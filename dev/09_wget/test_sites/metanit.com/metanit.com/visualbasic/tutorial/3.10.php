...
<html >
<head>
<title>Обобщенные типы в Visual Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Обобщенные типы</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Отличительной особенностью .NET 2.0 от первой версии платформы стала поддержка обобщенных типов (generics), равно как и обобщенных методов и делегатов. 
Чтобы разобраться в особенности данного нововведения, сначала посмотрим на проблему, которая могла возникнуть до появления обобщенных типов. Посмотрим 
на примере:</p>
<pre class="brush : vb;">
Dim x As Integer = 44
...
<p>В данном примере мы используем класс <b>ArrayList</b>, который представляет коллекцию объектов. Чтобы поместить объект в коллекцию, мы используем 
метод <span class="bb">Add</span>, а чтобы получить, указываем индекс элемента в скобках - <code>ar(0)</code>. Этот класс содержит коллекцию значений типа Object, 
а это значит, что в вызовах <code>ar.Add(x)</code> и <code>ar.Add(s)</code> значения переменных <code>x</code> и <code>s</code> сначала будут 
...
(boxing и unboxing) ведут к снижению производительности, так как системе надо осуществить необходимые преобразования.</p>
<p>Кроме того, существует другая проблема - проблема безопасности типов. Если мы напишем так, то естественно получим ошибку во время выполнения программы:</p>
<pre class="brush : vb;">
Dim y As Integer = ar(1) 'Вторым элементом является строка, а не число
</pre>
<p>Эти проблемы были призваны устранить обобщенные типы. Обобщенные типы позволяют указать конкретный тип, который будет использоваться. 
Например, используем обобщенный вариант класса <code>ArrayList</code> - класс <code>List</code>:</p>
<pre class="brush : vb;">
Dim x As Integer = 44
Dim s As String = "hello"
...
<p>Так как класс List является обобщенным, то нам нужно задать в выражении <b>(Of тип)</b> тип данных, для которого этот класс будет применяться. 
Далее мы добавляем число и строку в коллекцию. Однако если число будет добавлено в коллекцию arrGen, то на строке <code>arGen.Add(s)</code> 
...
выполнение и количество потенциальных ошибок.</p>
<p>Создадим обобщенный класс <b>Transaction</b>:</p>
<pre class="brush : vb;">
...
<P>С помощью буквы <b>T</b> в описании <code>Public Class Transaction(Of T)</code> мы указываем, что данный тип будет использоваться этим классом. 
...
Однако мы предполагаем, что вместо параметра T мы будем использовать интерфейс счета <code>IAccount</code>, который мы создали в предыдущих главах. 
Однако мы не можем знать, какой вид счета в банке в данном случае будет использоваться. Поэтому мы можем установить ограничение в виде типа <code>IAccount</code>:</P>
<pre class="brush : vb;">
...
<p>При этом мы можем задать множество ограничений. В этом случае они перечисляются после ключевого слова <b>As</b> в фигурных скобках:</p>
<pre class="brush : vb;">
Public Class Transaction(Of T As {IAccount, Client})
</pre>
<p>В качестве ограничения могут выступать как конкретные классы, так и интерфейсы. Кроме того, можно указать ограничение, чтобы использовались 
только структуры:</p>
<pre class="brush : vb;">
...
<p>А также можно задать в качестве ограничения класс или структуру, которые реализуют конструктор по умолчанию с помощью слова <b>New</b>:</p>
<pre class="brush : vb;">
Public Class Transaction(Of T As {Class, New})
</pre>
<p>Классами и структурами использование обобщений не ограничивается. Мы можем создавать также и обобщенные методы:</p>
<pre class="brush : vb;">
Sub GetInformation(Of T As Person)(emp As T)
    emp.Display()
End Sub
</pre>
<p>А затем также их использовать:</p>
<pre class="brush : vb;">
Sub Main()
