...
<html >
<head>
<title>Обработка исключений в Visual Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Обработка исключений</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>В процессе выполнения программы нередко возникают такие ситуации, которые трудно или даже невозможно предусмотреть во время создания программы. 
Например, при передаче файла по сети может неожиданно оборваться сетевое подключение. Такие ситуации называются <b>исключениями</b>. 
Чтобы программа неожиданно не зависла в таких ситуациях, программисту нужно вводить в нее обработку исключений. 
Для обработки исключений в языке VB.NET имеется конструкция <b>Try...Catch..Finally</b>. Ее использование выглядит следующим образом:</p>
 <pre class="brush : vb;">
Sub Main()
...
<p>В данном случае у нас как раз и возникает исключение, так как в массиве a только 4 элемента, а мы пытаемся присвоить значение шестому элементу. 
При использовании блока Try...Catch..Finally сначала выполняются все инструкции между операторами <span class="bb">Try</span> и 
<span class="bb">Catch</span>. 
И если вдруг в каком-то месте кода возникает исключение, то обычный порядок выполнения кода останавливается и программа переходит к выражению <b>Catch</b>. 
Это выражение имеет следующий синтаксис: <code>Catch имя_переменной As тип_исключения</code>. В данном случае у нас объявляется переменная ex, которая 
имеет тип <b>Exception</b>. При чем если возникшее исключение не является исключением типа, указанного в выражении Catch, то оно не обрабатывается, 
а программа просто зависает или выбрасывает сообщение об ошибке. Так как тип <code>Exception</code> является базовым типом для всех исключений, то в 
выражении <code>Catch ex As Exception</code> абсолютно все исключения.</p>
<p>В нашем случае мы просто выводим сообщение об исключении на экран с помощью свойства <code>Message</code>, определенное в классе <code>Exception</code>. 
Далее в любом случае выполняется блок после оператора <code>Finally</code>. Но этот блок необязательный, и его можно при обработке исключений опускать.</p>
<p>В случае, если в ходе программы исключений не возникнет, то программа не будет выполнять блок Catch, сразу перейдет к блоку Finally, 
если он имеется.</p>
<h4>Разработка собственных исключений</h4>
<p>Мы можем создать свои собственные классы исключений, которые будут обрабатывать нежелательные ситуации. Для примера возьмем класс 
<code>Algorithm</code>, который мы сделали в одной из глав предыдущей части:</p>
 <pre class="brush : vb;">
...
<p>Факториал и последовательность Фиббоначи определены только для положительных чисел, поэтому если мы передадим в функцию отрицательное число, 
возникнет исключительная ситуация. Чтобы ее обработать и создадим класс <code>NegativeNumberException</code>:</p>
 <pre class="brush : vb;">
...
<p>В этом классе по сути мы только передаем сообщение в конструктор базового класса. Теперь вызовем обработку исключения в методе Factorial 
с помощью ключевого слова <b>Throw</b>:</p>
<pre class="brush : vb;">
Public Shared Function Factorial(x As Integer) As Integer
...
<p>И теперь, если мы передадим в этот метод отрицательные значения, в блоке <code>Try...Catch</code> будет обрабатываться исключение <code>NegativeNumberException</code>:</p>
<pre class="brush : vb;">
Try
