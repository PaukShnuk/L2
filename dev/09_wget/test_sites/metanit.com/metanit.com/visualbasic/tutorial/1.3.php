...
<html >
<head>
<title>Visual Basic .NET | Компиляция в командной строке с .NET CLI</title>
<meta charset="utf-8" />
<meta name="description" content="Компиляция приложения на Visual Basic .NET в командной строке с помощью .NET CLI">
...
<h2>Компиляция в командной строке с .NET CLI</h2><div class="date">Последнее обновление: 16.11.2021</div>
...
<p>Для написания приложений на Visual Basic .NET удобно использовать Visual Studio, однако, хотя эта среда и облегчает разработку, 
в то же время скрывают множество мельчайших подробностей по созданию программ. И в реальности 
мы даже можем обойтись без той же Visual Studio, используя только инфраструктуру .NET CLI. Рассмотрим, как использовать .NET CLI и компилировать 
приложение из командной строки.</p>
<p>Для этого создадим на диске C какую-нибудь новую папку для проекта. Например, пусть мы создали для проекта папку <span class="b">C:\vbnet\helloapp</span>. Откроем командную строку и с помощью команды <span class="b">cd</span> перейдем к этой папке.</p>
<div class="console"><pre class="consoletext">C:\WINDOWS\system32&gt;cd C:\vbnet\helloapp</pre></div>
<p>Для создания проекта в .NET CLI применяется команда <span class="b">dotnet new</span>, которой передается шаблон проекта, а также дополнительные параметры. Итак, в введем в командной строке 
следующую команду:</p>
<div class="console"><pre class="consoletext">C:\vbnet\helloapp&gt;dotnet new console --language "vb"</pre></div>
<p>В данном случае мы указываем, что создаваемый проект будет представлять консольное приложение.</p>
<p>Кроме того, с помощью дополнительной опции <code>--language "vb"</code> необходимо указать, что мы создаем проект для языка Visual Basic .NET.</p>
<img src="./pics/1.14.png" alt="Создание проекта Visual Basic .NET в консоли" />
<p>После выполнения этой команды в папке helloapp будет создан проект с минимальным набором стандартных файлов и папок.</p>
<img src="./pics/1.5.png" alt=".NET CLI Создание проекта для Visual Basic" />
<p>В частности, мы можем найти в папке проекта файл <span class="b">helloapp.vbproj</span>. Это главный файл проекта, который определяет его 
конфигурацию. Мы можем открыть его в любом текстовом редакторе, просмотреть и  при необходимости изменить.</p>
<p>И, кроме того, по умолчанию создается главный файл программы <span class="b">Program.vb</span> со следующим содержимым:</p>
<pre class="brush:vb;">
Imports System
...
<p>Фактически это такой же минимальный проект, который создается в Visual Studio. И этот проект также уже можно запускать. Для запуска проекта введем в командной строке следующую команду:</p>
<div class="console"><pre class="consoletext">dotnet run</pre></div>
<img src="./pics/1.11.png" alt="Компиляция программы на Visual Basic .NET из командной строки" />
<p>После выполнения команды в проекте в папке bin\MCD\Debug\net6.0 появится файл программы helloapp.exe, который можно вручную запустить на компьютере, где установлен .NET 6.</p>
<p>И теперь перейдем непосредственно к изучению синтаксиса языка VB.NET.</p>
