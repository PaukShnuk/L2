...
<html >
<head>
<title>Преобразования базовых типов данных в VB.NET</title>
<meta charset="utf-8" />
...
<p>Итак, как вы выше увидели, каждый тип данных характеризуется определенным количеством байт, которое этот тип занимает в памяти. При этом для нас вполне логичным 
...
<p>Все дело в том, что в первом случае компилятор применяет преобразование типов: он преобразует данные типа Byte в данные типа Integer. 
Данный тип преобразований называется расширяющим (widening), так как тип Byte расширяет свой размер до размера типа Integer.</p>
...
<p>В этом случае переменные num1 и num2 типа Integer сужались до типа Byte. Однако с сужающими преобразованиями, особенно с теми, которые выполняются 
неявно, как в нашем случае, надо обращаться осторожно. Так, следующий код выбросит исключение OverflowException:</p>
<pre class="brush : vb;">
Dim num1 As Integer = 100
Dim num2 As Integer = 200
Dim res As Byte = num1 + num2
</pre>
<p>Что в принципе не удивительно, так как итоговое число - 300 больше максимального числа, которое может представлять тип Byte, то есть больше 255.</p>
<p>Другой момент, который надо затронуть, - это <span class="bb">явные (explicit conversion)</span> и <span class="bb">неявные (implicit conversion) преобразования</span>. Все выше перечисленные преобразования выполнялись у нас неявно. То есть 
компилятор сам выполнял преобразование из типа Integer в тип Byte без дополнительных команд. Мы можем явно запретить неявные сужающие преобразования, 
установив в самом верху кода программы оператор Option Strict On. Кроме того, мы можем установить этот оператор применительно ко всему проекту 
через свойства проекта (в меню выберите пункт Проект(Project) и в появившемся меню выберите пункт Свойства (Properties)):</p>
...
<li><p>Метод <span class="bb">CStr</span> преобразует в String любой числовой тип (включая Byte, SByte и типы перечисления), а также Boolean, Char, массив Char, Date, Object</p></li>
...
<p>Кроме того, для преобразования типов мы можем использовать функцию <span class="bb">CType</span>. Она принимает два параметра: первый параметр представляет переменную, 
которую надо преобразовать, а второй параметр - тип данных, в который надо эту переменную преобразовать. В следующем коде мы приводим переменную num3 к типу Byte:</p>
<pre class="brush : vb;">
Dim num2 As Integer = 2
