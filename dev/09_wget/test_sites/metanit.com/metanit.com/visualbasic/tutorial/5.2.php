...
<html >
<head>
<title>Работа с каталогами в Visual Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Работа с каталогами</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Для управления каталогами мы можем использовать сразу два класса <span class="bb">Directory</span> и 
<span class="bb">DirectoryInfo</span>, которые расположены в пространстве имен System.IO.</p>
<h3>Класс Directory</h3>
<p>Класс Directory содержит ряд статических методов, которые позволяют управлять каталогами. Основные из них:</p>
<ul>
<li><p><span class="bb">CreateDirectory(path)</span>: создает каталог по указанному пути path</p></li>
<li><p><span class="bb">Delete(path)</span>: удаляет каталог по указанному пути path</p></li>
<li><p><span class="bb">Exists(path)</span>: определяет, существует ли каталог по указанному пути path. Если существует, возвращается <code>true</code>, 
если не существует, то <code>false</code></p></li>
<li><p><span class="bb">GetDirectories(path)</span>: получает список каталогов в каталоге path</p></li>
<li><p><span class="bb">GetFiles(path)</span>: получает список файлов в каталоге path</p></li>
<li><p><span class="bb">Move(sourceDirName, destDirName)</span>: перемещает каталог по адресу sourceDirName на новый адрес destDirName</p></li>
<li><p><span class="bb">GetParent(path)</span>: получение родительского каталога по отношению к пути path</p></li>
</ul>
<h3>Класс DirectoryInfo</h3>
<p>Данный класс во многом похож на DirectoryInfo и предоставляет похожую функциональность. Некоторые из его свойств и методов:</p>
<ul>
<li><p>Метод <span class="bb">Create()</span> создает каталог</p></li>
<li><p>Метод <span class="bb">CreateSubdirectory(path)</span> создает подкаталог по указанному пути path</p></li>
<li><p>Метод <span class="bb">Delete()</span> удаляет каталог</p></li>
<li><p>Свойство <span class="bb">Exists</span> определяет, существует ли каталог</p></li>
<li><p>Метод <span class="bb">GetDirectories()</span>: получает список каталогов</p></li>
<li><p>Метод <span class="bb">GetFiles()</span> получает список файлов</p></li>
<li><p>Метод <span class="bb">MoveTo(destDirName)</span> перемещает каталог</p></li>
<li><p>Свойство <span class="bb">Parent</span> возвращает родительский каталог</p></li>
<li><p>Свойство <span class="bb">Root</span> возврашает корневой каталог</p></li>
</ul>
<p>Теперь используем эти классы в программе.</p>
...
<p>Прежде чем создать новый каталог, мы проверяем, имеется ли уже такой каталог. Если он существует, то создать его будет нельзя, и приложение 
выбросит ошибку. После создания каталога у нас получится следующий путь: "C:\SomeDir\program\avalon"</p>
...
надо удалять со всем содержимым:</p>
<pre class="brush:vb;">
...
<p>При перемещении надо учитывать, что перемещение может происходить только в пределах одного логического раздела - то есть из диска C мы можем переместить 
только в любое место на этом же диске C, но никак не на диск D и т.д. Кроме того, новый каталог, в который мы хотим перемесить все содержимое старого каталога, не должен существовать.</p>
