...
<html >
<head>
<title>Словарь Dictionary(Of T, V) в Visual Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Словарь Dictionary(Of T, V)</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Обобщенная коллекция Dictionary(Of T, V) является еще одним из наиболее используемых классов коллекций. 
Класс Dictionary(Of T, V) представляет словарь, который хранит объекты в виде пары ключ-значение. 
Каждый такой объект является объектом класса <b>KeyValuePair(Of TKey, TValue)</b>. С помощью свойств <code>Key</code> и 
<code>Value</code>, которые есть у данного класса, мы можем получить ключ и значение элемента в словаре.</p>
<p>Рассмотрим на примере использование словарей:</p>
<pre class="brush:vb;">
...
<p>Класс словарей также, как и другие коллекции, предоставляет методы Add и Remove для добавления и удаления элементов. 
Только в случае словарей в метод Add передаются два параметра: ключ и значение. А метод Remove удаляет не по индексу, а по ключу.</p>
<p>Поскольку в примере выше ключами является объекты типа <code>Integer</code>, а значениями - объекты типа <code>String</code>, то словарь 
в нашем случае будет хранить объекты <code>KeyValuePair(Of Integer, String)</code>. В цикле <code>For Each</code> также, как и у других коллекций, 
можно перебрать все элементы и извлечь их них ключ и значение.</p>
<p>Кроме того, мы можем получить отдельно коллекции ключей и значений словаря:</p>
<pre class="brush:vb;">
Module Module1
...
<p>В данном случае в качестве ключей выступают объекты типа <code>Char</code>, а значениями - объекты <code>Person</code>. С помощью свойства 
<code>Keys</code> мы можем получить ключи словаря, а с помощью свойства <code>Values</code> соответственно все значения в словаре.</p>
