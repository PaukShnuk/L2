...
<html >
<head>
<title>Перечисления в Visual Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Перечисления</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Перечисления представляют набор логически связанных констант. Объявление перечисления происходит с помощью оператора <b>Enum</b>, после которого указывается 
тип перечисления - он обязательно должен представлять целочисленный тип (Byte, Integer, Long, SByte, Short, UInteger, ULong или UShort). Если тип явным 
образом не указан, то умолчанию используется тип Integer. Затем идет список элементов перечисления:</p>
 <pre class="brush : vb;">
Enum days
    monday
    tuesday
...
<p>В данном случае элементу перечисления присваивается целочисленное значение, причем первый элемент будет иметь значение 0, второй - 1 и так далее. 
Мы можем также явным образом указать значения элементов, либо указав значение первого элемента:</p>
 <pre class="brush : vb;">
...
<p>Либо мы можем явным образом указать для всех них значения:</p>
 <pre class="brush : vb;">
Enum operation As Integer
    add = 20
    substract = 3
...
<p>Перечисления могут объявляться только на уровне модуля или класса, но никак не в процедуре или функции. Чтобы использовать перечисление, мы можем создать 
переменную с типом данного перечисления и присвоить ей значение следующим образом:</p>
 <pre class="brush : vb;">
Module Module2
	Enum days
        monday
...
<p>Посмотрим на примере, как использовать перечисления:</p>
 <pre class="brush : vb;">
Module Module1
...
<p>В данном случае у нас имеется перечисление <code>Operation</code>, которое представляет математические операции. Далее у нас объявлена процедура 
<code>MathOp</code>, которая в качестве параметров принимает два числа и типа операции. В основной процедуре Main мы два раза вызываем процедуру, передав 
в нее два числа - 5 и 10 и тип операции. Причем так как элементы перечисления представляют целые числа, то мы можем передать либо элемент перечисления, 
либо число, которое это константа представляет.</p>
