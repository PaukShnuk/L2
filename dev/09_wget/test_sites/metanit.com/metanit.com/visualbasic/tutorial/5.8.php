...
<html >
<head>
<title>Объект My.Computer.FileSystem в Visual Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Объект My.Computer.FileSystem</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Кроме ранее рассмотренных способов работы с файловой системой в Visual Basic.NET есть еще один способ, который представляет собой применение 
объекта <span class="bb">My.Computer.FileSystem</span>. В этом объекте определено ряд свойств и методов, позволяющих манипулировать файлами и каталогами. 
Рассмотрим основные из них:</p>
<ul>
<li><p>Свойство <code>CurrentDirectory</code> получает полное имя текущего каталога</p></li>
<li><p>Свойство <code>Drives</code> возвращает все диски компьютера:</p>
<pre class="brush:vb;">
For Each d In My.Computer.FileSystem.Drives
    Console.WriteLine("Имя диска: {0}  -  Размер: {1} байт", d.Name, d.TotalSize)
Next
</pre></li>
<li><p>Свойство <code>SpecialDirectories</code> получает все специальные каталоге на компьютере типа Program Files, Temp и др.</p></li>
<li><p>Метод <code>CopyDirectory</code> копирует один каталог в другой: 
<code>My.Computer.FileSystem.CopyDirectory("C:\SomeDir1","C:\SomeDir2", True)</code> - последний параметр задает перезапись совпадающих по имени файлов</p></li>
<li><p>Метод <code>CopyFile</code> копирует файл: <code>My.Computer.FileSystem.CopyFile("C:\SomeFile.txt","D:\NewFile.txt")</code></p></li>
<li><p>Метод <code>CreateDirectory</code> создает новый каталог: <code>My.Computer.FileSystem.CreateDirectory("C:\SomeFolder")</code></p></li>
<li><p>Метод <code>DeleteDirectory</code> удаляет каталог: <code>My.Computer.FileSystem.DeleteDirectory
("C:\SomeFolder", FileIO.DeleteDirectoryOption.DeleteAllContents)</code></p></li>
<li><p>Метод <code>DeleteFile</code> удаляет файл</p></li>
<li><p>Метод <code>DirectoryExists</code> возвращает True, если указанный каталог существует</p></li>
<li><p>Метод <code>FileExists</code> возвращает True, если указанный файл существует</p></li>
<li><p>Метод <code>FindInFiles</code> возвращает все файлы указанного каталога, в которых был найден определенный текст: 
<code>My.Computer.FileSystem.FindInFiles("C:\SomeFolder", "текст для поиска", True, FileIO.SearchOption.SearchAllSubDirectories)</code> - 
...
параметр позволяет задать поиск также и в подкаталогах.</p></li>
<li><p>Метод <code>GetDirectories</code> возвращает список каталогов указанного каталога</p></li>
<li><p>Метод <code>GetFiles</code> возвращает список файлов указанного каталога</p></li>
<li><p>Метод <code>GetParentPath</code> возвращает полное имя родительского каталога</p></li>
<li><p>Метод <code>MoveFile</code> перемещает файл из одного каталога в другой: <code>My.Computer.FileSystem.MoveFile("C:\SomeFile.txt","D:\SomeFile.txt", Treue)</code> - 
третий параметр позволяет задать перезапись файла, если в другом каталоге уже есть такой файл</p></li>
<li><p>Метод <code>MoveDirectory</code> перемещает каталог в другой: <code>My.Computer.FileSystem.MoveDirectory("C:\SomeSolder","D:\NewDir")</code></p></li>
<li><p>Метод <code>RenameDirectory</code> переименовывает каталог</p></li>
<li><p>Метод <code>RenameFile</code> переименовывает файл</p></li>
<li><p>Метод <code>ReadAllBytes</code> считывает все байты из файла в массив: 
<code>Dim bytes As Byte() = My.Computer.FileSystem.ReadAllBytes("C:\SomeFile.bin")</code></p></li>
<li><p>Метод <code>ReadAllText</code> считывает весь текст из файла: 
<code>Dim text As String = My.Computer.FileSystem.ReadAllText("C:\SomeFile.txt")</code></p></li>
<li><p>Метод <code>WriteAllBytes</code> записывает все байты в файл</p></li>
<li><p>Метод <code>WriteAllText</code> записывает весь текст в файл</code></p></li>
</ul>
<p>Например, создадим файл и запишем в него текст:</p>
<pre class="brush:vb;">
...
<div class="nav"><p><a href="5.7.php">Назад</a><a href="./">Содержание</a><a href="5.8.php">Вперед</a></p></div></div>
<div class="socBlock">
