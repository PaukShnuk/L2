...
<html >
<head>
<title>Перегрузка методов и операторов в VB.NET</title>
<meta charset="utf-8" />
...
<h2>Перегрузка методов и операторов</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Иногда возникает потребность создать один и тот же метод, но с разным набором параметров. И в зависимости от имеющихся параметров и использовать 
определенную версию метода. Допустим, в нашем классе State мы хотим определить метод для нападения на другое государство - метод <b>Attack</b>. 
...
<p>Но предположим, что мы хотим определить версию данного метода, где мы будем указывать не только государство, но количество войск, с помощью которых 
мы нападаем на врага. Тогда мы можем просто добавить вторую версию данного метода:</p>
 <pre class="brush : vb;">
Public Class State
...
<p>Либо в определении каждой версии метода мы можем использовать ключевое слово <b>Overloads</b> (тогда все версии этого метода должны быть помечены этим словом):</p> 
 <pre class="brush : vb;">
Public Class State
...
<p>Кроме методов мы можем также перегружать операторы. Поскольку перегружаемый оператор будет использоваться для всех объектов данного класса, 
то он объявляется с модификаторами <b>Public Shared</b>, после которых указывается ключевое слово <b>Operator</b> и далее знак оператора:</p>
<pre class="brush : vb;">
Public Shared Operator +(a As Object, b As Object) As Object

End Operator
</pre>
<p>Посмотрим на примере. У нас есть класс State или государство. Определим для этого класса оператор "+", который будет обозначать объединение двух государств, 
кроме того, предусмотрим операторы сравнения - "&gt;" и "&lt;", с помощью которых мы будем сравнивать два государства:</p>
 <pre class="brush : vb;">
Public Class State
...
<p>Обратите внимание, что поскольку мы применяем операцию сложения и сравнения над двумя объектами State, то мы должны передать эти объекты в качестве 
параметров. Тогда мы можем использовать операторы следующим образом:</p>
<pre class="brush : vb;">
Sub Main()
