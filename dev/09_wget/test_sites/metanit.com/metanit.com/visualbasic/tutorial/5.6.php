...
<html >
<head>
<title>Работа с бинарными файлами в Visual Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Работа с бинарными файлами</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Для работы с бинарными файлами в пространстве имен System.IO предназначены классы <b>BinaryWriter</b> и <b>BinaryReader</b>. Они 
позволяют записывать и считывать данные в двоичном формате.</p>
<h3>Некоторые методы класса BinaryWriter</h3>
<ul>
<li><p><span class="bb">Close()</span>: закрывает поток</p></li>
<li><p><span class="bb">Flush()</span>: очищает буфер, сбрасывая из него оставшиеся данные в файл</p></li>
<li><p><span class="bb">Seek()</span>: устанавливает позицию в потоке</p></li>
<li><p><span class="bb">Write()</span>: записывает данные в поток</p></li>
</ul>
<h3>Основные метода класса BinaryReader</h3>
<ul>
<li><p><span class="bb">Close()</span>: закрывает поток и освобождает ресурсы</p></li>
<li><p><span class="bb">ReadBoolean()</span>: считывает значение Boolean и перемещает указатель на один байт</p></li>
<li><p><span class="bb">ReadByte()</span>: считывает один байт и перемещает указатель на один байт</p></li>
<li><p><span class="bb">ReadChar()</span>: считывает значение Сhar, то есть один символ, и перемещает указатель на столько байтов, 
сколько занимает символ в текущей кодировке</p></li>
<li><p><span class="bb">ReadDecimal()</span>: считывает значение Decimal и перемещает указатель на 16 байт</p></li>
<li><p><span class="bb">ReadDouble()</span>: считывает значение Double и перемещает указатель на 8 байт</p></li>
<li><p><span class="bb">ReadInt16()</span>: считывает значение Short и перемещает указатель на 2 байта</p></li>
<li><p><span class="bb">ReadInt32()</span>: считывает значение Integer и перемещает указатель на 4 байта</p></li>
<li><p><span class="bb">ReadInt64()</span>: считывает значение Long и перемещает указатель на 8 байт</p></li>
<li><p><span class="bb">ReadSingle()</span>: считывает значение Single и перемещает указатель на 4 байта</p></li>
<li><p><span class="bb">ReadString()</span>: считывает значение String. Каждая строка предваряется значением длины строки, которое представляет 7-битное 
целое число</p></li>
</ul>
<p>Теперь рассмотрим, как мы можем использовать эти классы в реальной задаче. Допустим, у нас есть массив структур, которые мы хотим хранить в файле:</p>
<pre class="brush:vb;">
Imports System.IO
...
<p>В конце кода мы определили структуру State, которая имеет ряд полей. В методе Main создаем массив структур и записываем с помощью 
...
вызовом <code>File.Open(path, FileMode.OpenOrCreate)</code>.</p>
<p>Послк этого пробегаемся в цикле по массиву структур и записываем каждое поле структуры в поток. В том порядке, в каком они значения полей записываются, 
в том порядке они и будут размещаться в файле.</p>
<p>Затем считываем из записанного файла. Конструктор класса BinaryReader также в качестве параметра принимает объект потока, только в данном 
случае устанавливаем в качестве режима FileMode.Open: <code>new BinaryReader(File.Open(path, FileMode.Open))</code></p>
<p>В цикле <code>while</code> считываем данные. Вызов метода <code>PeekChar()</code> помогает нам установить конец потока. Этот метод считывает 
...
достигли конца файла. И для получения значений полей нам надо считать их из файла в том же порядке,в  каком они в него записывались.</p>
