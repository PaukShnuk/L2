...
<html >
<head>
<title>Статические члены в Visual Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Статические члены классов</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Обычно, чтобы использовать какой-нибудь класс, мы должны создать его объект. Однако если данный класс имеет статические или разделяемые методы, 
то чтобы получить к ним доступ, мы не обязательно должны использовать объекты. Например, создадим новый класс <span class="bb">Algorithm</span> и добавим в него 
две функции для вычисления числа Фибоначчи и факториала:</p>
 <pre class="brush : vb;">
...
<p>Обратите внимание на модификатор <b>Shared</b>, он указывает, что данные члены будут доступны для всего класса, то есть будут общими. В 
этом случае мы их можем использовать следующим образом:</p>
<pre class="brush : vb;">
Sub Main()
...
<p>В данном случае нам не надо создавать экземпляр или объект класса, мы можем обратиться к общему члену напрямую, указав только имя класса.</p> 
<h3>Статический конструктор</h3>
<p>Мы можем объявлять в классах также и статические конструкторы:</p>
<pre class="brush : vb;">
Public Class State
...
<p>Статические конструкторы выполняются при самом первом создании объекта данного класса:</p>
 <pre class="brush : vb;">
Sub Main()
...
<p>Чтобы понять смысл модификатора <b>Static</b>, сначала посмотрим на примере, в котором целесообразно его использование. Итак, у нас 
определен цикл:</p>
<pre class="brush : vb;">
...
<p>В этом цикле у нас определена переменная <code>temp</code>, которая после вывода на экран ее значения увеличивается на единицу. Однако 
...
мы заменим Dim на Static, то значение переменной при каждом новом проходе будет сохраняться, и при выводе на экран мы увидим, что ее значение увеличивается:</p>
<pre class="brush : vb;">
For i As Integer = 0 To 4
...
<p>Посмотрим на другом примере. Создадим метод со статической переменной, которая будет увеличиваться на единицу и отображаться на экране. 
И затем дважды вызовем этот метод:</p>
<pre class="brush : vb;">
Private Sub Display()
...
<p>Мы увидим, что несмотря на то, что вызов метода был завершен, значение переменной сохранилось, а при следующем вызове оно равно не 6, а 7. 
Тогда как если бы мы заменили слово Static на Dim, переменная после работы метода уничтожалась, а при следующем вызове метода создавалась бы вновь. В 
тоже время вместо переменной Static, мы можем использовать глобальную переменную на уровне класса или модуля.</p>
<p>И Static, и Shared могут использоваться в качестве модификаторов переменных. Однако отличие между ними состоит в том, что с помощью Static 
определяются переменные внутри методов, а с помощью Shared - переменные на уровне классов и модулей.</p>
