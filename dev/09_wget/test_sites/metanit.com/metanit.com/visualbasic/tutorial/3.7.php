...
<html >
<head>
<title>Абстрактные классы в Visaul Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Абстрактные классы</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Среди классов выделяются <i>базовые или абстрактные классы</i>. Мы не можем создать объект или экземпляр абстрактного класса. 
Основное его назначение - предоставить базовый функционал для других классов-наследников. При определении абстрактных классов используется ключевое слово 
<b>MustInherit</b>:</p>
<pre class="brush : vb;">
...
<p>Зачем нужны абстрактные классы? Допустим, мы разрабатываем программу для банка. В нашей программе мы можем определить три класса: 
<code>Person</code>, который описывает человека, <code>Employee</code>, который описывает сотрудника банка, и класс <code>Client</code>, 
который будет представлять клиента банка. Очевидно, что классы <code>Employee</code> и <code>Client</code> будут производными от класса <code>Person</code>. 
И так как все объекты будут представлять либо сотрудника банка, либо клиента, то напрямую мы от класса <code>Person</code> создавать объекты не будем. 
Поэтому имеет смысл сделать его абстрактным.</p>
<p>Абстрактный класс похож на обычный класс. Он также может иметь переменные, методы, конструкторы, свойства. Только от него напрямую нельзя образовывать 
объекты. Он представляет лишь базовый функционал, который потом реализуется в производных классах.
Кроме того, абстрактный класс в отличие от обычных классов может содержать абстрактные методы и свойства. Такие методы и свойства помечаются 
ключевым словом <b>MustOverride</b> и не имеют никакой реализации:</p>
<pre class="brush : vb;">
Public MustOverride Sub Display()
</pre>
<p>При этом производный класс обязан переопределить и реализовать все абстрактные методы и свойства, 
...
абстрактный. Теперь изменим нашу структуру классов и определим в ней абстрактный класс <code>Person</code>, в котором будет абстрактный метод 
<code>Display</code>:</p>
<pre class="brush : vb;">
