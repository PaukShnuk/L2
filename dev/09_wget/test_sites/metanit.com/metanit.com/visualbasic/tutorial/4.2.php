...
<html >
<head>
<title>Класс ArrayList в Visual Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Класс ArrayList</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Первый рассматриваемый нами класс <code>ArrayList</code> представляет необобщенную коллекцию объектов. И если стоит задача, создать коллекцию с разнотипными 
данными, то данный класс как раз для этого подходит. Рассмотрим пример использования ArrayList:</p>
<pre class="brush:vb;">
Module Module1
...
<p>Здесь вначале создается объект коллекции через конструктор как объект любого другого класса: <code>Dim objlist As New ArrayList()</code>. 
При необходимости можно было бы так же инициализировать коллекцию начальными данными, например, <code>Dim objlist As New ArrayList() From{1, 2, 5, "string", 7.7}</code></p>
<p>Затем последовательно добавляем различные значения. Данный класс коллекции, как и большинство других коллекций, имеет два способа добавления: 
одиночного объекта через метод <span class="bb">Add</span> и набора объектов - массива или другой коллекции через метод 
<span class="bb">AddRange</span></p>
<p>Так как класс ArrayList применяет интерфейс IEnumerable, то мы можем перебрать все хранящиеся в списке объекты через цикл <code>For Each</code>. 
...
<code>For Each item As Object In objlist</code></p>
<p>Класс ArrayList, как и большинство коллекций, реализует удаление с помощью методов Remove/RemoveAt. В данном примере мы удаляем первый элемент, 
передавая в метод RemoveAt индекс удаляемого элемента.</p>
<p>В конце программы опять же выводим элементы коллекции на экран только уже через обычный цикл For. В данном случае с перебором коллекций дело обстоит 
также, как и с массивами. А число элементов коллекции мы можем получить через свойство <code>Count</code></p>
<p>С помощью индексатора мы можем получить по индексу элемент коллекции так же, как и в массивах: <code>Dim firstObj As Object= objlist(0)</code></p>
