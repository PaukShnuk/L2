...
<html >
<head>
<title>Значение Nothing и Nullable-типы в VB.NET</title>
<meta charset="utf-8" />
...
<h2>Значение Nothing и Nullable-типы</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Все значимые типы, как например, Byte или Integer, должны иметь определенное значение. В отличие от значимых типов объекты ссылочных типов 
могут принимать значение <code>Nothing</code> или неопределенное значение. 
Например, <code>Dim d As Object = Nothing</code>. Объекты же типов значений так задать нельзя, и если мы напишем что-то наподобие 
<code>Dim x As Integer = Nothing</code>, то число x все равно будет определено - оно будет иметь значение по умолчанию, то есть 0.</p>
<p>Но иногда бывает удобно, чтобы объекты числовых типов в определенных ситуациях имели неопределенное значение, то есть Nothing. Например, 
при получении данных из базы данных мы точно не можем знать, установлены ли в бд данные или они имеют какое-то значение. </p>
<p>Чтобы задать объектам значимых типов неопределенное значение, надо использовать знак вопроса ? после указания типа. Например:</p>
<pre class="brush:vb;">
Dim d As Integer? = Nothing
Dim alive As Boolean? = Nothing
</pre>
<p>Но в действительности запись <code>?</code> является упрощенной формой использования структуры <span class="bb">Nullable(Of T)</span>. 
Параметр <code>T</code> в угловых скобках представляет универсальный параметр, вместо которого в приложении подставляется конкретный тип 
...
<p>Иногда перед присвоением одной переменной Nullable(Of T) другой обычной переменной требуется узнать, а имеет ли она определенное значение. 
Или ее значение равно Nothing, то присвоить обычной переменной некоторое значение по умолчанию. Это мы можем сделать следующим способом:</p>
<pre class="brush:vb;">
...
<p>Оператор If принимает два аргумента - если первый не равен Nothing, то возвращается он, иначе возвращается второй аргумент.</p>
<p>Но мы не можем написать следующим образом:</p>
<pre class="brush:vb;">
Dim z As Integer = 20
Dim t As Integer = If(z, 100)
</pre>
<p>Здесь переменная z не может использоваться в качестве первого параметра, так как она не представляет тип Nullable(Of T) и не может принимать значение Nothing.</p>
...
<div class="nav"><p><a href="3.15.php">Назад</a><a href="./">Содержание</a><a href="4.1.php">Вперед</a></p></div></div>
<div class="socBlock">
