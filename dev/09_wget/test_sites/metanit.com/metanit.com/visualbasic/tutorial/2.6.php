...
<html >
<head>
<title>Циклы в Visual Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Циклы</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Еще одним видом управляющих конструкций являются циклы. В VB.NET используется несколько видов циклов.</p>
<ul>
	<li><p>For...Next</p></li>
	<li><p>For Each...Next</p></li>
	<li><p>While</p></li>
	<li><p>Do</p></li>
</ul>
<h3>Цикл For...Next</h3>
<p>В этом цикл выполняется определенное число раз, причем это число задается счетчиком:</p>
<pre class="brush : vb;">
For i As Integer = 1 To 9
    Console.WriteLine("Квадрат числа {0} равен {1}", i, i * i)
Next
</pre>
<p>Здесь переменная i выполняет роль счетчика. После слова <b>To</b> мы помещаем максимальное значение счетчика. При каждом цикле значение счетчика увеличивается 
на единицу. И это значение сравнивается со значением после <b>To</b>. Если эти два значения равны, то цикла прекращает свою работу.</p>
<p>При работе с циклами мы можем увеличивать значение счетчика при каждом проходе не только на единицу, но и вообще на любое число. Для этого 
нужно либо использовать ключевое слово <b>Step</b> и после него указать шаг цикла, на который будет увеличиваться значение счетчика, либо можно 
увеличивать счетчик непосредственно в цикле:</p>
<pre class="brush : vb;">
...
<p>Обратите внимание, что в качестве шага в первом цикле выбрано отрицательное значение и значение счетчика с каждым проходом уменьшается на единицу. 
...
В итоге внутренний цикл отрабатывает пять раз, а внешний девять, то есть фактически получается 45 циклов.</p>
<h3>Цикл For Each...Next</h3>
<p>Цикл For Each осуществляет перебор элементов в определенной группе, например, в массиве или в коллекции. Предположим у нас есть некоторый массив 
типа Integer и нам надо инициализировать этот массив случайными значениями и затем вывести все его элементы на экран:</p>
<pre class="brush : vb;">
...
<p>В выражении For Each мы сначала указываем переменную, которая будет принимать значения элементов массива. А после ключевого слова <b>In</b> 
указываем группу, в которой надо перебрать все элементы.</p>
<h3>Цикл While</h3>
<p>В цикл While выполняется до тех пор, пока соблюдается определенное условие, указанное после слова While:</p>
<pre class="brush : vb;">
Dim j As Integer = 10
While j &gt; 0
    Console.WriteLine(j)
...
<h3>Цикл Do</h3>
<p>Цикл Do, также как и цикл While, выполняется, пока соблюдается определенное условие. Однако он имеет разные формы. Так, в следующем примере
сначала проверяется условие, а затем выполняется блок кода, определенный в цикле:</p>
<pre class="brush : vb;">
Dim j As Integer = 10
Do While j &gt; 0
    Console.WriteLine(j)
...
<p>В данном случае цикл выполняется, пока значение j больше нуля. Но есть еще одна запись, где вместо слова <span class="bb">While</span> используется слово 
<b>Until</b>, а цикл выполняется пока не соблюдено определенное условие, то есть пока значение j не станет меньше нуля:</p>
<pre class="brush : vb;">
Dim j As Integer = 10
Do Until j &lt; 0
    Console.WriteLine(j)
...
<p>Если изначально условие, заданное в цикле, неверно, то цикл не будет работать. Но мы можем определить проверку в конце цикла, и таким образом, 
наш цикл как минимум один раз отработает:</p>
<pre class="brush : vb;">
Dim j As Integer = -1
Do
    Console.WriteLine(j)
...
<h3>Операторы Continue и Exit</h3>
<p>Нередко возникает необходимость не дожидаться окончания цикла, а сразу же выйти из цикла, в случае соблюдения определенного условия. Для 
этого используют оператор <b>Exit</b>, после которого указывают тип цикла, из которого осуществляется выход, например, <code>Exit Do (Exit While)</code>:</p>
<pre class="brush : vb;">
Dim r As New Random()
...
<p>Существует и другая задача - осуществить выход не из цикла, а из текущего прохода или итерации и перейти к следующему. Для этого используют 
оператор <span class="bb">Continue</span>, после которого указывают тип цикла, из которого осуществляется выход, например, <code>Continue While</code>:</p>
<pre class="brush : vb;">
Dim r As New Random()
Dim num As Integer = r.Next(100)
...
<p>В данном случае мы в каждом проходе цикла вычитаем из num число 7 и затем смотрим, не принадлежит ли число num интервалу от 25 до 50. 
И если принадлежит, переходим к новой итерации цикла, а если нет, то выводим его на экран.</p>
