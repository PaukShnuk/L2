...
<html >
<head>
<title>Модификаторы доступа в Visual Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Модификаторы доступа</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Каждый из членов класса, будь то переменная или метод, да и сам класс имеют <span class="bb">модификаторы доступа</span>. Модификаторы доступа позволяют задать 
область видимости для членов класса. В VB.NET используются следующие модификаторы:</p>
<ul>
	<li><p><span class="bb">Public</span> - публичный или открытый класс или член класса. Доступ к члену класса можно получить из любого места в коде, а к открытому классу - 
	из других программ и сборок.</p></li>
	<li><p><span class="bb">Private</span> - закрытый класс или член класса. Доступ к данному классу или члену класса, можно получить только из кода в том же классе или контексте.</p></li>
	<li><p><span class="bb">Friend</span> - класс или член класса с данным модификатором доступен из текущей сборки или только в пределах данной программы.</p></li>
	<li><p><span class="bb">Protected</span> - применяется либо к членам класса, либо к вложенным классам. Такие классы или члены класа только из самого класса, либо 
	из наследующих классов.</p></li>
	<li><p><span class="bb">Protected Friend</span>  - применяется либо к членам класса, либо к вложенным классам. Данный модификатор объединяет контексты Friend и 
	Protected. Поэтому классы и члены класса с данным модификатором доступны либо из текущей программы, либо 
	из наследующих классов.</p></li>
</ul>
<p>Объявление переменных с оператором <span class="bb">Dim</span> аналогично объявлению этих переменных с модификатором <span class="bb">Private</span>.</p>
<p>Посмотрим на примере использование модификаторов доступа. Создадим следующий класс State:</p>
 <pre class="brush : vb;">
Public Class State
...
<p>Поскольку этот класс объявлен как <span class="bb">Public</span>, он будет доступен из любого места программы. У нас создано пять переменных для каждого уровня доступа
и одна переменная с оператором Dim. Также у нас созданы четыре метода, которые будут выводить значения переменных на экран. Обратите внимание, что 
так как все модификаторы позволяют использовать члены класса внутри данного класса, то и все переменные класса у нас доступны его методам. Теперь посмотрим, 
как мы сможем использовать переменные нашего класса в программе:</p>
 <pre class="brush : vb;">
Module Module1
...
<p>Таким образом, мы смогли установить только переменные <code>d, e</code> и <code>f</code>. И нам оказались доступны только два метода: 
<code>state1.Display_a()</code> и <code>state1.Display_b()</code>. Но так как мы переменные a и b не смогли установить, то эти методы вывели нули, 
так как значение переменных типа Integer по умолчанию равно нулю.</p>
<p>Несмотря на то, что модификаторы Friend и Public похожи по своему действию, но они имеют большое отличие. Классы и члены класса с модификатором 
Public также будут доступны и другим программам, если данных класс поместить в динамическую библиотеку dll и потом ее использовать в этих программах.</p>
<p>Благодаря такой системе модификаторов мы можем скрыть некоторые моменты реализации класса от других частей программы, такое сокрытие называется 
<span class="bb">инкапсуляцией</span>.</p>
