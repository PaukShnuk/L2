...
<html >
<head>
<title>Итераторы. Оператор Yield в Visual Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Итераторы. Оператор Yield</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Реализация интерфейса IEnumerable предполагает стандартную реализацию перечислителя. Однако мы можем не полагаться на стандартную 
реализацию, а создать свою логику итератора с помощью ключевых слов <span class="bb">Iterator</span> и <span class="bb">Yield</span>. 
Конструкция <b>итератора</b> представляет метод, в котором используется ключевое слово Yield для перебора по коллекции или массиву. Например, 
перепишем определенный в прошлой теме метод <code>GetEnumerator</code> в классе Library, применив итераторы:</p>
<pre class="brush:vb;">
...
<p>Метод <code>GetEnumerator()</code> теперь является итератором. И при переборе всех элементов в объекте Library через цикл For Each 
будет идти к обращение к вызову <code>Yield books(i)</code>. При обращении к оператору <code>Yield</code> будет сохраняться текущее 
местоположение. И когда метод For Each перейдет к следующей итерации для получения нового объекта, итератор начнет выполнения с этого местоположения.</p>
<p>И в основной программе в цикле For Each стандартным образом:</p>
<pre class="brush:vb;">
For Each b As Book In library
    Console.WriteLine(b.Name)
Next
</pre>
<p>Хотя выше в методе <code>GetEnumerator()</code> применялся перебор массива в цикле for, но это необязательно делать. 
Можно просто определить несколько вызовов оператора <code>Yield</code>:</p>
<pre class="brush:vb;">
Public Iterator Function GetEnumerator() As IEnumerator Implements IEnumerable.GetEnumerator
...
<p>Здесь также при вызове оператора <code>Yield</code> итератор запоминает текущее местоположение и при последующих вызовах начинает с него.</p>
<h3>Именованный итератор</h3>
<p>Хотя выше для создания итератора применялся метод <code>GetEnumerator</code>, но это необязательно, так как оператор <code>Yield</code> 
можно использовать внутри любого метода. Единственное ограничение - такой метод должен возвращать объект интерфейса <code>IEnumerable</code>. 
Подобные методы еще называют <span class="bb">именованными итераторами</span>.</p>
<p>Создадим такой именованный итератор в классе Library:</p>
<pre class="brush:vb;">
Class Book
...
<p>Здесь в качестве итератора выступает функция <code>Public Iterator Function GetBooks(max As Integer) As IEnumerable</code>, которая принимает в качестве параметра 
...
произошло ошибки, вызывается завершение функции с помощью операторов <b>Exit Function</b>.</p>
<p>Теперь применим итератор в программе:</p>
<pre class="brush:vb;">
...
<p>Вызов <code>library.GetBooks(5)</code> возвращает набор из не более чем 5 объектов Book. Но поскольку по умолчанию объект library содержит 
всего три таких объекта, то в методе <code>GetBooks</code> после трех операций сработает завершение функции <code>Exit Function</code>.</p>
...
<div class="nav"><p><a href="4.9.php">Назад</a><a href="./">Содержание</a><a href="5.1.php">Вперед</a></p></div></div>
<div class="socBlock">
