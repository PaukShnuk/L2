...
<html >
<head>
<title>Индексаторы и создание коллекций в Visual Basic</title>
<meta charset="utf-8" />
...
<h2>Индексаторы и создание коллекций</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>В предыдущих темах мы разобрали некоторые основные встроенные коллекции и посмотрели, как их использовать. Однако мы не ограничены только 
этими классами коллекций, и можем создавать свои собственные коллекции или хранилища объектов. Наиболее простым способом создания 
хранилища объектов является использование индексаторов.</p>
<p><b>Индексаторы</b> представляют специальные свойства, которые позволяют индексировать объекты и использовать их как массивы. По форме они 
напоминают свойства со стандартными методами <code>get</code> и <code>set</code>, которые возвращают и присваивают значение.</p>
<p>Посмотрим на примере. Допустим, у нас есть класс Book, представляющий книгу, и класс Library, который используется для хранения набора книг 
и использует индексаторы:</p>
<pre class="brush:vb;">
Module Module1
...
<p>В классе Library определен массив объектов Book, к которому затем будем обращаться с помощью индексатора. А само определение индексатора 
напоминает свойство: <code>Default Public Property Item(index As Integer) As Book</code>.</p>
<p>Во-первых, для определения индексатора используется ключевое слово <span class="bb">Default</span>. Во-вторых, в качестве возвращаемого 
или присваиваемого объекта указывается тип Book - тип объектов в хранилище. 
И в-третьих, в определении свойства указывается параметр <code>index As Integer</code>, через который будет осуществляться доступ к конкретным элементам.</p>
<p>И теперь мы сможем обращаться к объекту Library как к массиву: <code>library(0).Name</code>. Так как индексатор использует 
тип Book, то library(0) будет представлять объект Book.</p>
<p>Посмотрим более сложный пример. Допустим, у нас есть класс, в котором хранилище определено в виде двухмерного массива или матрицы:</p>
<pre class="brush:vb;">
Class Matrix
...
<p>В отличие от первого примера здесь для определения индексатора используются сразу два индекса - i и j. Соответственно в программе мы уже должны обращаться к объекту, используя два индекса:</p>
<pre class="brush:vb;">
