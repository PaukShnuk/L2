...
<html >
<head>
<title>Работа с текстовыми файлами в Visual Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Работа с текстовыми файлами</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Непосредственно для работы с текстовыми файлами в пространстве System.IO предназначены специальные классы <span class="bb">StreamReader</span> и <span class="bb">StreamWriter</span>.</p>
<h3>Чтение из файла и StreamReader</h3>
<p>Класс StreamReader позволяет читать из текстового файла весь текст или отдельные строки. Рассмотрим основные его методы:</p>
<ul>
<li><p>Метод <span class="bb">Close</span> закрывает файл и освобождает поток</p></li>
<li><p>Метод <span class="bb">Peek</span> возвращает следующий доступный символ, если символов больше нет, то возвращает -1</p></li>
<li><p>Метод <span class="bb">Read</span> считывает и возвращает следующий символ в численном представлении. Имеет перегруженную версию: 
<code>Read(array As Char(), index As Integer, count As Integer)</code>, где <code>array</code> - массив, куда считываются символы, <code>index</code> - индекс в массиве array, 
начиная с которого записываются считываемые символы, и count - максимальное количество считываемых символов</p></li>
<li><p>Метод <span class="bb">ReadLine</span> считывает одну строку из файла</p></li>
<li><p>Метод <span class="bb">ReadToEnd</span> считывает из файла весь текст</p></li>
</ul>
<p>Считаем текст из файла различными способами:</p>
<pre class="brush:vb;">
Imports System.IO
...
Module Module1
    ...
    Sub Main()
        ...
        Try
            ...
            Using reader As New StreamReader(path, System.Text.Encoding.UTF8)
                Dim archars As Char() = New Char(4) {}
                'считываем 4 символа
                reader.Read(archars, 0, 4)

                Console.WriteLine(archars)
            End Using
...
<p>Как и в случае с классом FileStream здесь используется конструкция <code>Using</code>.</p>
<p>В первом случае мы разом считываем весь текст с помощью метода <code>ReadToEnd()</code>.</p>
<p>Во втором случае в цикле While считываем построчно. Когда объект reader дойдет до конца файла и больше строк не останется, то 
метод <code>reade.ReadLine()</code> вернет значение Nothing.</p>
<p>В третьем случае считываем в массив четыре символа.</p>
<p>Обратите внимание, что в последних двух случаях в конструкторе StreamReader указывалась кодировка <code>System.Text.Encoding.UTF8</code>. 
...
Если кодировка не указана, то по умолчанию при чтении используется <code>UTF8</code>. Иногда важно указывать кодировку, так как она может 
отличаться от UTF8, и тогда мы получим некорректный вывод.</p>

<h3>Запись в файл и StreamWriter</h3>
<p>Для записи в текстовый файл применяется класс <code>StreamWriter</code>. Его основные методы:</p>
<ul>
<li><p><span class="bb">Close</span>: закрывает записываемый файл и освобождает все ресурсы</p></li>
<li><p><span class="bb">Flush</span>: записывает в файл оставшиеся в буфере данные и очищает буфер.</p></li>
<li><p><span class="bb">Write</span>: записывает в файл данные простейших типов, как Integer, Double и т.д.</p></li>
<li><p><span class="bb">WriteLine</span>: записывает в файл строку</p></li>
</ul>
<p>Посмотрим на использование класса StreamWriter на примере:</p>
<pre class="brush:vb;">
Imports System.IO
...
Module Module1
    ...
    Sub Main()
        ...
        Dim readPath As String = "C:\SomeDir\note.txt"
        Dim writePath As String = "C:\SomeDir\note2.txt"
        Dim text As String
        Try
...
<p>Вначале весь текст из файла считывается в переменную text, а затем содержание этой переменной с помощью объекта StreamWriter записывается в другой файл.</p>
<p>Класс StreamWriter имеет несколько конструкторов. Здесь мы использовали два из них: <code>New StreamWriter(writePath, False)</code>. 
...
будет файл дозаписываться или перезаписываться. Если этот параметр равен <code>True</code>, то новые данные добавляются в конце к уже имеющимся 
данным. Если <code>False</code>, то предыдущее содержимое стирается, и файл перезаписывается. И если в первом случае файл перезаписывается, то во втором делается дозапись в конец файла.</p>
<p>Третий параметр указывает кодировку, в которой записывается файл.</p>
