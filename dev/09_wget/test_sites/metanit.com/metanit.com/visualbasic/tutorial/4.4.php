...
<html >
<head>
<title>Двухсвязный список LinkedList(Of T) в VB.NET</title>
<meta charset="utf-8" />
...
<h2>Двухсвязный список LinkedList(Of T)</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Класс <code>LinkedList(Of T)</code> представляет двухсвязный список, в котором каждый элемент хранит указатель одновременно на предыдущий и на следующий 
элемент.</p>
<p>В обычном списке List(Of T) каждый элемент является объектом типа T. А в LinkedList(Of T) каждый элемент представляет 
объект класса <code>LinkedListNode(Of T)</code>. Этот класс имеет следующие свойства:</p>
<ul>
<li><p><span class="bb">Next</span>: указатель на следующий элемент типа LinkedListNode(Of T) в списке. Если следующий элемент отсутствует, то имеет значение Nothing</p></li>
<li><p><span class="bb">Previous</span>: указатель на предыдущий элемент типа LinkedListNode(Of T) в списке. Если предыдущий элемент отсутствует, то имеет значение Nothing</p></li>
<li><p><span class="bb">Value</span>: само значение узла, представленное типом T</p></li>
</ul>
<p>С помощью методов класса LinkedList(Of T) можно обращаться к различным элементам, как в конце, так и в начале списка:</p>
<ul>
<li><p><span class="bb">AddAfter(node As LinkedListNode(Of T), newNode As LinkedListNode(Of T))</span>: вставляет новый элемент newNode в список после элемента 
node.</p></li>
<li><p><span class="bb">AddAfter(node As LinkedListNode(Of T), value As T)</span>: добавляет в список новый элемент со значением value после элемента node</p></li>
<li><p><span class="bb">AddBefore(node As LinkedListNode(Of T), newNode As LinkedListNode(Of T))</span>: добавляет в список новый элемент newNode перед элементом node.</p></li>
<li><p><span class="bb">AddBefore(node As LinkedListNode(Of T), value As T)</span>: вставляет в список новый элемент со значением value перед элементом node.</p></li>
<li><p><span class="bb">AddFirst(node As LinkedListNode(Of T))</span>: вставляет новый элемент в начало списка</p></li>
<li><p><span class="bb">AddFirst(value As T)</span>: вставляет новый элемент со значением value в начало списка</p></li>
<li><p><span class="bb">AddLast(node As LinkedListNode(Of T))</span>: добавляет новый элемент в конец списка</p></li>
<li><p><span class="bb">AddLast(value As T)</span>: вставляет новый элемент со значением value в конец списка</p></li>
<li><p><span class="bb">RemoveFirst()</span>: удаляет первый элемент из списка, после чего первым элементом становится элемент, следующий за удаленным</p></li>
<li><p><span class="bb">RemoveLast()</span>: удаляет последний элемент из списка</p></li>
</ul>
<p>Используем двухсвязный список в программе:</p>
<pre class="brush:vb;">
Module Module1
...
<p>В данном примере создаются и используются два списка: для чисел и для объектов класса Person. С числами, наверное, все более менее просто, поэтому рассмотрим работу с классом Person.</p>
<p>Методы <code>AddLast</code>, <code>AddFirst</code> при добавлении в список возвращают ссылку на добавленный элемент 
LinkedListNode(Of T) (в нашем случае LinkedListNode (Of Person)). Затем с помощью свойств <code>Previous</code> и <code>Next</code> 
мы можем получить ссылки на предыдущий и следующий элементы в списке.</p>
