...
<html >
<head>
<title>Преобразование типов в Visual Basic.NET</title>
<meta charset="utf-8" />
...
<h2>Преобразование типов</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Ранее мы уже говорили о преобразовании простейших типов. Теперь затронем тему преобразования объектов различных классов. Итак, у нас есть 
следующая система классов:</p>
<pre class="brush : vb;">
'Классы
...
<p>В иерархии классов мы можем проследить следующую цепь наследования: Object (все классы неявно наследуются от типа Object) -&gt; Person -&gt; Employee|Client.</p>
<p>Из этой цепи видно, что объект класса <code>Client</code> одновременно является и представителем классов Person и Object. Поэтому мы можем 
создать объекты обоих классов следующим образом:</p>
<pre class="brush : vb;">
...
<p>В данном случае работает неявное преобразование, так как наши переменные представляют классы <code>Object</code> и <code>Person</code>, поэтому 
допустимо неявное восходящее преобразование - преобразование к типам, которые находятся вверху иерархии классов:</p>
<p><span style="margin-left:15px; background:silver;">Object</span>
<br><span style="margin-left:35px;">|</span>
<br><span style="margin-left:15px; background:silver;">Person</span>
<br><span style="margin-left:35px;">|</span>
<br><span style="background:silver;">Employee|Client</span>
<p>Таким образом, классы Employee и Client находятся в низу иерархии классов. Добавим еще класс <b>Manager</b>, который будет производным от Employee 
и поэтому будет находиться в самом низу иерархии классов.</p>
<pre class="brush : vb;">
...
<p>Значит, мы можем написать следующее, так как объект класса Manager в то же время является и сотрудником банка:</p>
<pre class="brush : vb;">
Dim man As Employee = New Manager("John", "Thompson", "City Bank")
man.Display()
</pre>
<P>В то же время, если мы применим нисходящее преобразование, как в следующем коде, то мы получим ошибку:</P>
<pre class="brush : vb;">
Dim emp As Manager = New Employee("John", "Thompson", "City Bank")
emp.Display()
</pre>
<p>Здесь мы преобразуем объект Emloyee к типу Manager, однако если Manager является объектом типа Emloyee, то 
объект Emloyee не является объектом типа Manager. Есть несколько способов избежать ошибки при выполнении программы. Во-первых, можно использовать 
оператор <b>TryCast</b> - он пытается преобразовать выражение к определенному типу, при этом не выбрасывает исключение. В случае неудачного преобразования 
выражение будет содержать значение <b>Nothing</b>:</p>
<pre class="brush : vb;">
...
<p>Второй способ заключается в отлавливании исключения <b>InvalidCastException</b>, которое возникнет в результате преобразования:</p>
<pre class="brush : vb;">
Try
    Dim emp As New Employee("John", "Thompson", "City Bank")
...
<p>Третий способ заключается в проверке допустимости преобразования с помощью ключевого слова <b>Is</b>:</p>
<pre class="brush : vb;">
Dim emp As New Employee("John", "Thompson", "City Bank")
...
<p>Выражение <code>TypeOf emp Is Manager</code> указывает, имеет ли переменная <code>emp</code> тип <code>Manager</code>. А поскольку 
такая проверка вернет False, то преобразование не сработает. Для явного преобразования типов используется оператор <code>DirectCast</code>, 
который пытается преобразовать переменную emp к типу Manager.</p>
<p>Все сказанное выше в отношении преобразования типов характерно и для интерфейсов. Поскольку класс <code>Client</code> реализует интерфейс 
<code>IAccount</code>, то переменная типа <code>IAccount</code> может хранить ссылку на объект типа <code>Client</code>:</p>
<pre class="brush : vb;">
