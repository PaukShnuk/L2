...
<html >
<head>
<title>PHP | Первый сайт на PHP</title>
<meta charset="utf-8" />
<meta name="description" content="Первый веб-сайт на PHP 8 с помощью веб-сервера Apache">
...
<h2>Первый сайт на PHP</h2><div class="date">Последнее обновление: 26.02.2021</div>
...
<p>Сейчас мы создадим небольшой сайтик, который призван дать начальное понимание работы с PHP.</p>
<p>Для создания программ на PHP нам потребуется текстовый редактор. Можно взять любой текстовый редактор. Наиболее популярным на 
сегодняшний день является программа <a href="https://code.visualstudio.com/download" rel="nofollow">Visual Studio Code</a></p>
<p>Перейдем к каталогу, который предназначен для хранения файлов веб-сайта (В прошлой теме для этой цели был создан каталог 
<span class="b">C:\localhost</span>.) Создадим в этом каталоге текстовый файл и назовем его <span class="b">index.html</span>. 
Откроем его в текстовом редакторе и добавим в него следующий код:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Код html содержит форму с двумя текстовыми полями. При нажатии на кнопку данные этой формы отсылаются скрипту <span class="b">display.php</span>, 
так как он указан в атрибуте <code>action</code>.</p>
<p>Теперь создадим этот скрипт, который будет обрабатывать данные. Добавим в папку <span class="b">C:\localhost</span> 
новый текстовый файл. Переименуем его в <span class="b">display.php</span>. По умолчанию файлы программ на php имеют расширение 
<span class="b">.php</span>.</p>
<img src="./pics/1.12.png" alt="Первый сайт на PHP на веб-сервере Apache" />
<p>Итак, добавим в файл <span class="b">display.php</span> следующий код:</p>
<pre class="brush:php;">
&lt;?php
...
<p>Для добавления выражений PHP используются теги <code>&lt;?php ..... ?&gt;</code>, между 
которыми идут инструкции на языке PHP. В коде php мы получаем данные отправленной формы и выводим их на страницу.</p>
<p>Каждое отдельное выражение PHP должно завершаться точкой с запятой. В данном случае у нас три выражения. Два из них получают переданные данные формы, 
например, <code>$name = $_POST["firstname"];</code>.</p>
<p><code>$name</code> - это переменная, которая будет хранить некоторое значение. Все переменные в PHP предваряются знаком $. И так как форма на странице index.html 
использует для отправки метод POST, то с помощью выражения <code>$_POST["firstname"]</code> мы можем получить значение, которое было введено в текстовое поле 
с атрибутом <code>name="firstname"</code>. И это значение попадает в переменную $name.</p>
<p>С помощью оператора <span class="b">echo</span> можно вывести на страницу любое значение или текст, которые идут после оператора. В данном случае (<code>echo "Ваше имя: &lt;b&gt;".$name . " " . $surname . "&lt;/b&gt;"</code>) 
с помощью знака точки текст в кавычках соединяется со значениями переменных $name и $surname и выводится на страницу.</p>
<p>Теперь обратимся к форме ввода, перейдя по адресу <span class="b">http://localhost/index.html</span> (или <span class="b">http://localhost</span>):</p>
<img src="./pics/1.13.png" alt="Первый веб-сайт на PHP" />
<p>Введем какие-нибудь данные и нажмем на кнопку отправить:</p>
<img src="./pics/1.14.png" alt="Отправка данных в PHP" />
<p>Итак, у нас сработал скрипт <span class="b">display.php</span>, который получил и вывел отправленные данные на страницу.</p>
...
<div class="nav"><p><a href="./1.5.php">Назад</a><a href="./">Содержание</a><a href="./2.1.php">Вперед</a></p></div></div>
<div class="socBlock">
