...
<html >
<head>
<title>PHP | Конструкции switch и match</title>
<meta charset="utf-8" />
<meta name="description" content="Конструкции switch и match в языке программирования PHP, операторы case и break, правила сравнения значений">
...
<h2>Конструкции switch и match</h2><div class="date">Последнее обновление: 07.03.2021</div>
...
<p>Конструкция <code>switch..case</code> является альтернативой использованию конструкции <code>if..elseif..else</code>. 
Оператор <span class="b">switch</span> получает некоторое выражение и сравнивает его с набором значений:</p>
<pre class="brush:php;">
switch (выражение){
...
сравнивается со значениями, помещенными после операторов <span class="b">сase</span>. И если совпадение будет найдено, то будет выполняться определенный блок сase.</p>
<p>Конструкция switch может содержать произвольное количество операторов case. В конце блока <span class="b">сase</span> ставится оператор <span class="b">break</span>, чтобы избежать выполнения других блоков.</p>
<p>Весь блок <span class="b">switch</span> помещается в фигурные скобкиб однако блок каждого отдельного оператора <span class="b">case</span> НЕ помещается в 
фигурные скобки.</p>
<p>Например, возьмем следующую конструкцию <code>if..elseif..else</code>:</p>
<pre class="brush:php;">
$a = 3;
...
<p>Теперь перепишем ее с помощь конструкции <code>switch..case</code>:</p>
<pre class="brush:php;">
$a = 3;
switch($a)
{
...
<p>То есть на вход в конструкцию switch передается переменная <code>$a<</code>, и ее значение последовательно сравнивается со значениями, 
указанными после операторов <code>case</code>. Так, в данном случае переменная <code>$a</code> равна 3, поэтому выполнение дойдет до блока</p>
<pre class="brush:php;">
case 3: 
	echo "умножение";
	break;
</pre>
<p>и этот блок и будет выполняться.</p>
<p>Если мы хотим также обработать ситуацию, когда совпадения не будет найдено, то можно добавить блок <span class="b">default</span>:</p>
<pre class="brush:php;">
$a = 3;
switch($a)
{
...
<p>Конструкция <code>switch..case</code> также поддерживает альтернативный синтаксис, при котором вместо открывающей блок фигурной скобки 
ставится двоеточие, а вместо закрывающей фигурной скобки - ключевое слово <span class="b">endswitch</span>:</p>
<pre class="brush:php;">
$a = 3;
switch($a):
	case 1: 
...
<h3>match</h3>
<p>Начиная с версии 8.0 в PHP была добавлена поддержка другой, похожей конструкции - <span class="b">match</span>. Она позволяет оптимизировать 
конструкцию <code>switch</code>. Конструкция <span class="b">match</span> также принимает некоторое выражение и сравнивает его с набором значений.</p>
<p>Например, пусть у нас есть следующий код:</p>
<pre class="brush:php;">
$a = 2;
switch($a)
{
...
<p>Перепишем этот пример с использованием <span class="b">match</span>:</p>
<pre class="brush:php;">
$a = 2;
$operation = match($a)
{
...
<p>Итак, <code>match</code> в скобках также принимает некоторое сравниваемое выражение (в данном случае это переменная $a). Сам блок кода match 
...
А вместо операторов <code>case</code> просто указываются значения, с которыми сравнивается выражение.</p>
<p>Но в отличие от <code>switch</code>, конструкция <span class="b">match</span> возвращает некоторый результат. 
Поэтому после каждого сравнимого значения ставится оператор <span class="b">=&gt;</span>, после которого идет возвращаемый результат.</p>
<p>То есть в данном случае переменная <code>$a</code> равна 2, поэтому в конструкции match будет выполнять блок</p>
<pre class="brush:php;">2 =&gt; "вычитание",</pre>
<p>Этот блок установит в качестве возвращаемого результата строку <code>"вычитание"</code>.</p>
<p>Поскольку конструкция match возвращает результат, то этот результат мы можем присвоить другой переменной:</p>
<pre class="brush:php;">$operation = match($a){
	//.................
}
</pre>
<p>В итоге в переменной <code>$operation</code> будет храниться строка <code>"вычитание"</code>.</p>
<p>Также мы можем переписать предыдущий пример следующим образом:</p>
<pre class="brush:php;">
$a = 2;
match($a)
{
...
<h4>Сравнение значений и типов</h4>
<p>Стоит отметить важное отличие конструкции <span class="b">switch</span> от <span class="b">match</span>: <span class="b">switch</span> сравнивает только значение, но не учитывает тип выражения. 
Тогда как <span class="b">match</span> также учитывает тип сравниваемого выражения. Рассмотрим разницу на примере. Пусть есть следующая конструкция 
switch:</p>
<pre class="brush:php;">
switch (8.0) {
  case "8.0":
...
<p>В конструкцию switch передается в качестве выажения число 8.0, но с точки зрения внутренней логики конструкции switch это 
выражение также соответствует строке "8.0". Поэтому в данном случае будет выполняться блок</p>
<pre class="brush:php;">
case "8.0":
    $result = "строка";
    break;
</pre>
<p>Теперь посмотрим, что будет в аналогичном примере с match:</p>
<pre class="brush:php;">
match (8.0) {
  "8.0" =&gt; $result = "строка",
  8.0 =&gt; $result = "число"
...
<p>Конструкция <span class="b">match</span> также будет учитывать тип выражения, а тип в данном случае float, поэтому будет выполняься блок:</p>
<pre class="brush:php;">8.0 =&gt; $result = "число"</pre>
