...
<html >
<head>
<title>PHP | Установка</title>
<meta charset="utf-8" />
<meta name="description" content="Установка PHP, загрузка пакета, Thread Safe, настройка интерпретатора">
...
<h2>Установка PHP</h2><div class="date">Последнее обновление: 04.06.2021</div>
...
<p>Есть разные способы установки всего необходимого программного обеспечения. Мы можем устанавливать компоненты по отдельности, а можем использовать уже готовые 
...
В качестве операционной системы будет использоваться Windows.</p>
<p>Что подразумевает установка PHP? Во-первых, нам нужен интерпретатор PHP. Во-вторых, необходим веб-сервер, например, Apache, с помощью которого мы сможем обращаться 
к ресурсам создаваемого нами сайта.</p>
<p>Для установки PHP перейдем на офсайт разработчиков <a href="https://www.php.net/downloads" rel="nofollow">https://www.php.net/downloads</a>. На странице загрузок мы можем найти различные дистрибутивы 
для операционной системы Linux. Если нашей операционной системой является Windows, то нам надо загрузить один из пакетов со страницы <a href="https://windows.php.net/download" rel="nofollow">https://windows.php.net/download</a>.</p>
<p>Загрузим zip-пакет последнего выпуска PHP, учитывая разрядность операционной системы, на которую надо установить PHP. Для 64-x разрядной:</p>
<img src="./pics/1.1.png" alt="Установка PHP на Windows" />
<p>Для 32-x разрядной:</p>
<img src="./pics/1.15.png" alt="Установка PHP на Windows 32х" />
<p>Интерпретатор PHP имеет две версии: Non Thread Safe и Thread Safe. В чем разниц между ними? 
Версия <code>Thread Safe</code> позволяет задействовать многопоточность, тогда как <code>Non Thread Safe</code> - однопоточная версия. 
Выбрем версию <span class="b">Thread Safe</span>.</p>
<p>Распакуем загруженный архив в папку, которую назовем <span class="b">php</span>. Пусть эта папка у нас будет располагаться в корне диска C.</p>
<p>Теперь нам надо выполнить минимальную конфигурацию PHP. Для этого зайдем в распакованный архив и найдем там файл <span class="b">php.ini-development</span>.</p>
<img src="./pics/1.2.png" alt="Установка интерпретатора PHP на Windows" />

<p>Это файл 
начальной конфигурации интерпретатора. Переименуем этот файл в <span class="b">php.ini</span> и затем откроем его в текстовом редакторе.</p>
<p>Найдем в файле строку:</p>
<pre class="brush:php;">;extension_dir = "ext"</pre>
<p>Эта строка указывает на каталог с подключаемыми расширениями для PHP. Расширения позволяют задействовать нам некоторую дополнительную функциональность, 
например, работу с базой данных. Все расширения находятся в распакованном  каталоге <span class="b">ext</span>.</p>
<p>Раскомментируем эту строку, убрав точку с запятой и укажем полный путь к папке расширений php:</p>
<pre class="brush:php;">extension_dir = "C:\php\ext"</pre>
<p>Остальное содержимое файла оставим без изменений.</p>
<p>Теперь установим веб-сервер.</p>
