...
<html >
<head>
<title>PHP | Основы синтаксиса</title>
<meta charset="utf-8" />
<meta name="description" content="Основы синтаксиса PHP, основные теги, рабоа интерпретатора">
...
<h1>Основы PHP</h1><h2>Основы синтаксиса</h2><div class="date">Последнее обновление: 27.02.2021</div>
...
<h3>Встраивание кода PHP</h3>
<p>При создании первой программы на PHP уже были затронуты некоторые основные принципы создания скриптов на языке PHP. Теперь, рассмотрим их более подробно.</p>
<p>Программа или скрипт на PHP, как правило, находится в файле  расширением <span class="b">.php</span>.  Хотя разработчики могут также вставлять код php и в файлы с расширениями 
<span class="b">.html/.htm</span>.</p>
<p>Документ PHP может содержать как разметку html, так и код на языке php. Для перехода от разметки html к коду php используются теги 
<span class="b">&lt;?php и ?&gt;</span>, между которыми идет код php. Данные теги служат указанием интерпретатору PHP, что их содержимое надо интерпретировать как код php, 
а не разметку html.</p>
<p>Например, определим папке, где хранятся файлы веб-сайта (исходя из прошлых тем это должна быть папка <code>c:\localhost</code>), 
файл <span class="b">hello.php</span>. Определим в этом файле следующий код:</p>
<pre class="brush:php;">
&lt;?php 
echo "Привет мир!";
?&gt;
</pre>
<p>Собственно код PHP состоит из набора инструкций. Здесь использована одна инструкция <code>echo "Привет мир!";</code>. Эта инструкция 
представляет встроенную команду <span class="b">echo</span>, которая выводит на веб-страницу некоторое значение. Выводимое значение указывается 
после команды <span class="b">echo</span> - в данном случае это строка "Привет мир!". Каждая отдельная инструкция в PHP завершается точкой с запятой.</p>
<p>Поскольку в данном случае файл "hello.php" находится в корневой папке веб-сервера, то для обращения к этому скрипту в 
адресной строке браузера надо ввести адрес <span class="b">http://localhost/hello.php</span>. В итоге при обращении к этому скрипту мы увидим в веб-браузере следующую страницу:</p>
<img src="./pics/2.1.png" alt="Встраивание PHP на веб-страницу" />
<p>Для оформления кода PHP также можно использовать краткую версию тегов: <code>&lt;? и ?&gt;</code>. Для этого в файле 
<span class="b">php.ini</span> надо изменить строку:</p> 
<pre class="brush:php;">short_open_tag = Off</pre>
<p>на</p>
<pre class="brush:php;">short_open_tag = On</pre>
<p>Когда пользователь обращается к скрипту в адресной строке браузера, набирая, например, <span class="b">http://localhost/hello.php</span>, то веб-сервер 
передает его интерпретатору PHP. Затем интерпретатор обрабатывает код и генерирует на его основе html-разметку. И затем сгенерированный html-код отправляется пользователю.</p>
<p>В случае выше определенного скрипта <code>hello.php</code> сгенерированная разметка будет выглядеть следующим образом:</p>
<pre class="brush:xml;">Привет мир!</pre>
<p>Но естественно мы можем также добавить некоторый код html. Например, изменим скрипт hello.php следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Как уже указывалось выше, интерпретатор с помощью тегов <span class="b">&lt;?php ... ?&gt;</span> сможет понять, что весь текст между 
этими тегами следует рассматривать как код PHP. А весь код вне этих тегов рассматривается как код html.</p>
<p>В этом случае интерпретатор сформирует следующий html-код:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/2.2.png" alt="Интерпретатор PHP" />
<p>При этом естественно мы можем использовать больше инструкций, а также встраивать код php в различных частях веб-страницы. 
Например:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<p>В данном случае код PHP встраивается в двух местах. В первом случае - внутри элемента <code>&lt;h1&gt;</code>. Во втором случае внутри элемента 
<code>&lt;div&gt;</code></p>
<p>Это даст нам следующий результат:</p>
<img src="./pics/2.3.png" alt="Теги php и html" />
<p>При чем при использовании функции <code>echo</code> мы можем включать в выводимый текст html-код, как в случае с выражением:</p>
<pre class="brush:php;">echo "&lt;h2&gt;Заголовок параграфа&lt;/h2&gt;";</pre>
<p>Хотя выше код php определялся в файле с расширением <span class="b">.php</span>, но равным образом мы также можем определять код в файлах с расширением 
<span class="b">.html</span>, и они также будут обрабатываться интерпретатором PHP.</p>
<h3>Сокращенная версия тегов php</h3>
<p>Если нам надо вывести на веб-страницу одно какое-нибудь значение, то мы можем использовать специальную форму тегов php - 
<span class="b">&lt;?= ... ?&gt;</span> - после знака = ("равно") ставится выводимое выражение. Например:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В первом случае выводится строка <code>&lt;?= "Первый сайт на PHP"; ?&gt;</code>.</p>
<p>Во втором случае выводится результат выражения <code>2 + 2</code>: <code>&lt;?= 2+2 ?&gt;</code>.</p>
<p>Результат работы скрипта:</p>
<img src="./pics/2.4.png" alt="Типы блоков кода и тегов PHP" />
<h3>Комментарии</h3>
<p>При создании веб-сайта мы можем использовать комментарии. Например, мы можем прокомментировать какое-либо действие, 
чтобы впоследствии иметь представление, что этот код делает:</p>
<pre class="brush:php;">
&lt;?php
...
<p>Знак <code>//</code> предваряет однострочный комментарий, и все что идет после этого знака на одной строке, будет считаться комментарием и не будет 
выполняться интерпретатором. При обработке интерпретатор просто будет пропускать комментарии.</p>
<p>Если нам надо закомментировать несколько строк, то мы можем использовать многострочный комментарий <code>/* текст комментария*/</code>:</p>
<pre class="brush:php;">
&lt;?php
echo "&lt;p&gt;Привет мир!&lt;/p&gt;"; // вывод сообщения
/*
...
<p>Все строки внутри комментария также не будут обрабатываться интерпретатором.</p>
