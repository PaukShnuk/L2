...
<html >
<head>
<title>PHP | Функции</title>
<meta charset="utf-8" />
<meta name="description" content="Функции в языке программирования PHP, оператор function, определение и вызов функций">
...
<h2>Функции</h2><div class="date">Последнее обновление: 11.03.2021</div>
...
<p>Функции представляют собой блок инструкций, которые многократно можно вызывать в различных частях программы. Функции позволяют разделять 
программуу на меньшие функциональные части.</p>
<p>Общий синтаксис определения функции выглядит следующим образом:</p>
<pre class="brush:php;">
...
<p>Определение функции начинается с ключевого слова <span class="b">function</span>, за которым следует имя функции. Имя функции должно начинаться с 
алфавитного символа или подчеркивания, за которыми может следовать любое количество алфавитно-цифровых символов или символов подчеркивания.</p>
...
<p>Данная функция назывется <code>hello</code>. Она не имеет параметров, поэтому после названия функции идут пустые скобки. Блок функции содержит 
только одну инструкцию, которая выводит сообщение "Hello PHP".</p>
<p>Чтобы функция сработала, ее надо вызвать. Для вызова функции указывается ее имя, после которого в скобках идут значения для ее параметров 
(если, кончено, она имеет параметры):</p>
<pre class="brush:php;">название_функции (значения_для_параметров_функции);</pre>
<p>Например, вызовем вышеопределенную функцию <code>hello</code>:</p>
<pre class="brush:php;">
&lt;?php
function hello()
{
...
<p>Поскольку для функции hello мы не определили никаких параметров, то при ее вызове указываем название функции и после нее пустые круглые скобки. Заканчивается 
вызов функции точкой с запятой.</p>
<p>В итоге браузер выведет сообщение:</p>
<pre class="browser">
Hello PHP
</pre>
<p>Преимуществом функций является то, что, определив однажды, мы можем многократно их вызывать в различных частях программы:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/2.7.png" alt="Функции в PHP" />
<p>Стоит отметить, что в данном случае сначала определяется функция, а затем она вызывается. Но мы также можно сделать наоборот:</p>
<pre class="brush:php;">
&lt;?php
hello();	// вызов функции
...
<p>Тем не менее есть исключения. В частности, при определении функции при определенном условии.</p>
<pre class="brush:php;">
...
<div class="nav"><p><a href="./2.16.php">Назад</a><a href="./">Содержание</a><a href="./2.17.php">Вперед</a></p></div></div>
<div class="socBlock">
