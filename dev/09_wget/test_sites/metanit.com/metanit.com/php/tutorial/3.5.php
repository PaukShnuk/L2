...
<html >
<head>
<title>PHP | Безопасность данных</title>
<meta charset="utf-8" />
<meta name="description" content="Безопасность отправленных данных в запросах типа GET и POST в языке программирования PHP, функции htmlentities, htmlspecialchars и strip_tags">
...
<h2>Безопасность данных</h2><div class="date">Последнее обновление: 15.03.2021</div>
...
<p>Большое значение в PHP имеет организация безопасности данных. Рассмотрим несколько простых механизмов, которые могут повысить безопасность нашего веб-сайта.</p>
<p>Но вначале возьмем форму из прошлой темы:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>И попробуем ввести в нее некоторые данные. Например, введем в поле для имени "&lt;script&gt;alert(hi);&lt;/script&gt;":</p>
<img src="./pics/3.6.png" alt="Безопасность в PHP" />
<p>После отправки данных в html разметку будет внедрен код javascript, который выводит окно с сообщением.</p>
<p>Это относительно простой и безвредный скрипт. Однако внедряемый код может быть более вредоносным. И чтобы избежать подобных проблем с 
безопасностью, рекомендуется применять функцию <span class="b">htmlentities()</span>. В качестве параметра она принимает значение, которое  надо экранировать:</p>
<pre class="brush:php;">
$name = "не определено";
$age = "не определен";
if(isset($_POST["name"])){
...
<p>И даже после ввода кода html или javascript все теги будут экранированы, и мы получим следующий вывод:</p>
<img src="./pics/3.7.png" alt="Функция htmlentities в PHP" />
<p>Еще одна специальная функция - <span class="b">htmlspecialchars()</span> похожа по действию на <span class="b">htmlentities</code>:</p>
<pre class="brush:php;">
$name = "не определено";
$age = "не определен";
if(isset($_POST["name"])){
...
<p>Еще одна функция - функция <span class="b">strip_tags()</span> позволяет полностью исключить теги html:</p>
<pre class="brush:php;">
$name = "не определено";
$age = "не определен";
if(isset($_POST["name"])){
...
<p>Результатом ее работы при том же вводе будет следующий вывод:</p>
<img src="./pics/3.8.png" alt="Функция strip_tags в PHP" />
...
<div class="nav"><p><a href="./3.1.php">Назад</a><a href="./">Содержание</a><a href="3.6.php">Вперед</a></p></div></div>
<div class="socBlock">
