...
<html >
<head>
<title>PHP | Подключение внешних файлов</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение внешних файлов в языке программирования PHP, оператор include, include_once, require, require_once, функция spl_autoload_register">
...
<h1>Базовые возможности PHP</h1><h2>Подключение внешних файлов</h2><div class="date">Последнее обновление: 29.03.2021</div>
...
<p>При разработке программ на PHP, возможно, какую-ту часть кода мы захотим использовать одновременно в других файлах с кодом PHP. В этом 
случае отдельные части кода можно распределить по отдельным файлам. Это позволить не писать один и тот же код по сто раз на сотнях скриптов, а будет достаточно подключить 
файл с кодом PHP. Кроме того, если потребуется изменить поведение подключаемого кода, достаточно будет изменить код в подключаемом файле.</p>
<p>Для подключения файлов PHP предоставляет ряд возможностей.</p>
<h3>Инструкция include</h3>
<p>Инструкция <span class="b">include</span> подключает в программу внешний файл с кодом php. Так, для примера определим файл <span class="b">welcome.php</span>:</p>
<pre class="brush:php;">
&lt;?php
...
<p>Здесь определена функция <code>welcome</code>, которая в качестве параметра принимает условное имя и использут его для вывода приветствия.</p>
<p>Теперь подключим данный файл в нашу программу, которая определена в другом файле в той же папке:</p>
<pre class="brush:php;">
&lt;?php
include "welcome.php";
...
<p>В место определения инструкции include будет вставляться весь код из файла <span class="b">welcome.php</span>. При этом вставка файла должна 
...
располагаются в одной папке.</p>
<p>Конструкция <code>include</code> может использовать как относительные, так и абсолютные пути. Например, выше использовался относительный путь. Или, к примеру, если мы имеем слующую структуру</p>
<ul>
<li><p>Файл <code>index.php</code></p></li>
<li><p>Папка <code>scripts</code></p>
	<ul>
	<li><p>Файл <code>welcome.php</code></p></li>
	</ul>
</li>
</ul>
<p>То чтобы подключить файл <code>welcome.php</code> из папки <code>scripts</code>, в файле index.php необходимо 
использовать следующий относительный путь:</p>
<pre class="brush:php;">include "scripts\welcome.php";</pre>
<p>Если файл <code>welcome.php</code> располагается по полному пути <code>C:\localhost\scripts\welcome.php</code>, то также можно было бы использовать абсолютный - 
полный путь:</p>
<pre class="brush:php;">include "C:\localhost\scripts\welcome.php";</pre>
<h3>Инструкция include_once</h3>
<p>Использование инструкции include имеет недостатки. Так, мы можем в разных местах кода неумышленно подключить один и тот же файл, 
что при выполнении кода вызовет ошибки.</p>
<p>Чтобы исключить повторное подключение файла, вместо инструкции <code>include</code> следует применять инструкцию <span class="b">include_once</span></p>
<pre class="brush:php;">
&lt;?php
...
<p>И теперь, если мы подключим этот же файл с помощью <code>include_once</code> еще где-нибудь ниже, то это подключение будет проигнорировано, так как файл уже подключен в программу.</p>
<h3>Инструкции require и require_once</h3>
<p>Действие инструкции <span class="b">require</span> подобно инструкции include: она также подключает внешний файл, вставляя в программу его содержимое. 
Только теперь, если данный файл не будет найден, действие программы прекратится (инструкция <code>include</code> в этом случае выбрасывает предупреждение):</p>
<pre class="brush:php;">
&lt;?php
require "welcome.php";
?&gt;
</pre>
<p>И также если у нас в коде встретятся несколько инструкций <code>require</code>, которые подключают один и тот же файл, то интерпретатор выдаст ошибку. И также 
чтобы избежать данной ситуации, следует использовать инструкцию <span class="b">require_once</span>:</p>
<pre class="brush:php;">
&lt;?php
require_once "welcome.php";
?&gt;
</pre>
<h3>Функция spl_autoload_register</h3>
<p>В больших приложениях количество подключаемых файлов может быть довольно большим. Однако встроенная функция <span class="b">spl_autoload_register()</span> 
в определенных ситуациях позволяет избежать большого количества инклудов. В качестве параметра она принимает функцию автозагрузки. 
Эта функция автоматически вызывается, когда в программе начинает использоваться неизвестный класс или интерфейс. И функция автозагруки пытается 
загрузить этот класс или интерфейс. В качестве параметра функция автозагрузки принимает название класса или интерфейса, которые надо загрузить.</p>
<p>Например, пусть у нас будет файл <span class="b">Person.php</span>, в котором располагается класс <span class="b">Person</span>:</p>
<pre class="brush:php;">
&lt;?php
class Person
{
...
<p>Обращаю внимание, что название файла соответствует названию класса.</p>
<p>Используем функцию автозагрузки для подключения подобного класса:</p>
<pre class="brush:php;">
&lt;?php
...
<p>Функция <code>spl_autoload_register()</code> в качестве параметра принимает название функции автозагрузки - в данном случае это функция <code>my_autoloader()</code>. 
...
программа встретит использование данного класса, она вызовет функцию <code>my_autoloader()</code>, в качестве параметра <code>$class</code> передаст в 
нее название класса Person.</p>
<p>Все действия функции автозагрузки мы определяем сами. В данном случае с помощью <code>echo</code> выводится некоторое диагностическое сообщение и 
подключается файл класса:</p>
<pre class="brush:php;">include $class . ".php";</pre>
<p>При этом в данном случае неважно какой класс, главное, чтобы он хранился в одноименном файле с расширением <code>.php</code>. 
В этого программа выведет следующее:</p>
<pre class="browser">
...
<div class="nav"><p><a href="./6.10.php">Назад</a><a href="./">Содержание</a><a href="./4.4.php">Вперед</a></p></div></div>
<div class="socBlock">
