...
<html >
<head>
<title>PHP | Проверка существования переменной</title>
<meta charset="utf-8" />
<meta name="description" content="Проверка существования переменной в языке программирования PHP, функция isset и empty и unset">
...
<h2>Проверка существования переменной</h2><div class="date">Последнее обновление: 13.03.2021</div>
...
<p>Если переменная объявлена, но ей изначально не присвоено никакого значения (иначе говоря она не инициализирована), или если переменная вовсе неопределена, то нам будет проблематично ее использовать. Например:</p>
<pre class="brush:php;">
&lt;?php
$a;
echo $a;
...
<p>При попытке вывести значение переменной мы получим диагностическое сообщение о том, что переменная не определена:</p>
<pre class="browser"><span class="b">Warning</span>: Undefined variable $a in <span class="b">C:\localhost\hello.php</span> on line <span class="b">10</span></pre>
<p>Ситуация может показатся искуственной. Тем не менее нередко переменные в PHP получают некоторые данные извне, например, ввод пользователя. Соответственно 
возникает необходимость перед использованием данных проверить, что эти данные определены, что они имеются.</p>
<p>Для проверки существования переменной PHP предоставляет ряд встроенных функций.</p>
<h3>Оператор isset</h3>
<p>Функция <span class="b">isset()</span> позволяет определить, инициализирована ли переменная или нет. Если переменная определена, 
то <code>isset()</code> возвращает значение <code>true</code>. Если переменная не определена, то <code>isset()</code> возвращает <code>false</code>. 
Также если переменная имеет значение <code>null</code> функция <code>isset()</code> также возвращает <code>false</code>.</p>
<p>Например:</p>
<pre class="brush:php;">
&lt;?php
$message;
...
<p>Здесь переменная $message не инициализирована, поэтому выражение <code>isset($message)</code> будет возвращать значение <code>false</code>.</p>
<pre class="browser">переменная message не определена</pre>
<p>Теперь пусть переменная $message имеет начальное значение:</p>
<pre class="brush:php;">
$message = "Hello PHP";
if(isset($message))
    echo $message;
...
<p>В этом случае выражение <code>isset($message)</code> будет возвращать <code>true</code>, поэтому будет выполняться инструкция <code>echo $message</code>:</p>
<pre class="browser">Hello PHP</pre>
<p>Однако если переменной присвоено начальное значение <code>null</code>, то опять же будет считаться, что эта переменная не установлена:</p>
<pre class="brush:php;">
$message = null;
if(isset($message))
    echo $message;
...
<pre class="browser">переменная message не определена</pre>
<h3>empty</h3>
<p>Функция <code>empty()</code> проверяет переменную на "пустоту". "Пустая" переменная - это переменная, значение которой равно 
<code>null</code>, 0, <code>false</code> или пустой строке - в этом случае функция <code>empty()</code> возвращает <code>true</code>:</p>
<pre class="brush:php;">
&lt;?php
$message = "";
if(empty($message))
...
<p>Здесь переменная <code>$message</code> хранит пустую строку, поэтому выражение <code>empty($message)</code> возвратит <code>true</code>.</p>
<pre class="browser">переменная message не определена</pre>
<p>При этом если строка содержит даже хотя бы один пробел и больше ничего (<code>$message = " "</code>), то такая строка уже не считается пустой.</p>
<h3>unset</h3>
<p>С помощью функции <span class="b">unset()</span> мы можем уничтожить переменную:</p>
<pre class="brush:php;">
&lt;?php
$a=20;
echo $a; // 20
...
<p>Но, как правило, необходимость в подобном удалении переменной возникает редко, так как PHP автоматически удаляет переменные, когда завершается 
выполнение контекста (например, функции), в котором определены эти переменные.</p> 
