...
<meta charset="utf-8" />
<meta name="description" content="Traits в языке программирования PHP, ключевое слово trait, оператор use">
...
<h2>Traits</h2><div class="date">Последнее обновление: 23.03.2021</div>
...
<p><span class="b">Traits</span> представляют группу методов, которые могут быть добавлены в классы. 
<span class="b">Traits</span> позволяют определять блоки функционала и многократно повторно использовать в классах без необходимости усложнять 
код классов, которые используют эти методы.</p> 
<p>Traits определяются с помощью ключевого словва <span class="b">trait</span>, который идет название трейта:</p>
<pre class="brush:php;">
trait Printer { }
</pre>
<p>Traits могут содержать только статические и нестатические методы:</p>
<pre class="brush:php;">
trait Printer
{
...
<p>Для применения классов трейта применяется оператор <span class="b">use</span>, после которого указывается добавляемый трейт:</p>
<pre class="brush:php;">
trait Printer
{
	public function printSimpleText($text) { echo "$text&lt;br&gt;"; }
...
<p>После добавления трейта с помощью оератора <code>use</code> класс может использовать его методы, как-будто они определены в самом этом классе.</p>
<p>Следует учитывать, что при наследовании методы трейта переопределяют унаследованные методы с тем же именем:</p>
<pre class="brush:php;">
class Data
{
