...
<html >
<head>
<title>PHP | Установка веб-сервера Apache и PHP на Mac OS</title>
<meta charset="utf-8" />
<meta name="description" content="Установка веб-сервера Apache и PHP 8 на Mac OS, настройка конфигурации, httpd.conf и httpd.exe, phpinfo, запуск простейшего скрипта на php, php_module">
...
<h2>Установка веб-сервера Apache и PHP на MacOS</h2><div class="date">Последнее обновление: 12.03.2022</div>
...
<p>Установка веб-сервера Apache и PHP на Mac OS несколько отличается от процесса установки на Windows. Прежде всего следует 
отметить, что Mac OS уже по умолчанию имеет встроенный Apache. Однако начиная с версии <span class="b">macOS Monterey</span> из операционной системы 
был удален PHP, который в предыдущих версиях устанавливался по умолчанию. Кроме того, настройка конфигурации встроенного веб-сервера 
несколько затруднена, каких-то пакетов может недостовать. И в этом случае более предпочтительный подход состоит в использовании пакетного менеджера <span class="b">Homebrew</span>, 
который значительно облегчает установку и настройку всех необходимых компонентов. Поэтому рассмотрим установку и настройку Apache и PHP с помощью пакетного менеджера Homebrew.</p>
<p>Вначале надо установить сам пакетный менеджер Homebrew через терминал с помощью команды:</p>
<div class="console">
<pre class="consoletext">
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
</pre>
</div>
<p> Чтобы убедиться, что Homebrew установлен, можно проверить его версию с помощью следующей команды:</p>
<div class="console">
<pre class="consoletext">
brew --version
</pre>
</div>
<p>В этом случае терминал должен отобразить что-то наподобие:</p>
<div class="console">
<pre class="consoletext">
Homebrew 3.3.1 Homebrew/homebrew-core (git revision 1362c572e16; last commit 2021-10-29)
</pre>
</div>
<p>Затем установим PHP с помощью следующей команды:</p>
<div class="console">
<pre class="consoletext">
brew install php
</pre>
</div>
<p>Далее установим веб-сервер Apache с помощью следующей команды:</p>
<div class="console">
<pre class="consoletext">
brew install httpd
</pre>
</div>
<p>После установки веб-сервера Apache вначале проверим его работоспособность. Для его запустим его с помощью следующей команды:</p>
<div class="console">
<pre class="consoletext">
brew services start httpd
</pre>
</div>
<p>При удачном запуске веб-сервера мы сможем обратиться к нему, введя в адресную строку веб-браузера адрес 
<span class="b">http://localhost:8080</span>. И в этом случае веб-браузер должен нам отобразить надпись "It works!":</p>
<img src="./pics/macos1.png" alt="запуск веб-сервера Apache на MacOS" />
<h3>Настройка  Apache</h3>
<p>Теперь настроим Apache. Для этого нам надо изменить файл конфигурации веб-сервера. В зависимости от архитектуры Mac расположение файла 
конфигурации будет отличаться:</p>
<ul><li><p>Для архитектуры Intel это путь <span class="b">/usr/local/etc/httpd/httpd.conf</span>.</p></li>
<li><p>Для архитектуры Apple M1 это путь <span class="b">/opt/homebrew/etc/httpd/httpd.conf</span>.</p></li>
</ul>
<p>Соответственно для изменения файла нам потребуется текстовый редактор.  Можно использовать встроенный текстовый редактор типа TextEdit.</p>
<p>Если архитектура - Intel, введем в терминале следующую команду:</p>
<div class="console">
<pre class="consoletext">
open -e /usr/local/etc/httpd/httpd.conf
</pre>
</div>
<img src="./pics/macos2.png" alt="Настройка конфигурации веб-сервера Apache на MacOS" />
<p>Если архитектура - Apple M1, введем в терминале следующую команду:</p>
<div class="console">
<pre class="consoletext">
open -e /opt/homebrew/etc/httpd/httpd.conf
</pre>
</div>
<p>Вначале изменим порт - по умолчанию он равен 8080. Для этого найдем следующую строку:</p>
<pre class="brush:php;">Listen 8080</pre>
<p>Параметр Listen указывает на порт, по которому можно будет обращаться к веб-серверу. Заменим эту строку на следующую:</p>
<pre class="brush:php;">Listen 80</pre>
<p>Далее настроим папку для файлов веб-сервера. По умолчанию это папка "/usr/local/var/www". Но изменим ее.</p>
<p>Создадим в папке текущего пользователя новую папку, которую назовем <span class="b">localhost</span>. Например, в моем текущий 
    пользователь называется <span class="b">eugene</span>, поэтому папка документов веб-сервера будет иметь путь 
    <span class="b">Users/eugene/localhost</span>. Теперь установим эту папку в качестве каталога для файлов веб-сервера.</p>
<p>Если архитектура Intel, найдем в файле следующие строки</p>
<pre class="brush:php;">
DocumentRoot "/usr/local/var/www"
&lt;Directory "/usr/local/var/www"&gt;
</pre>
<p>Если архитектура Apple M1, найдем в файле следующие строки</p>
<pre class="brush:php;">
DocumentRoot "/opt/homebrew/var/www"
&lt;Directory "/opt/homebrew/var/www"&gt;
</pre>
<p>Параметры DocumentRoot и Directory указывает на каталог файлов веб-сервера. Теперь изменим эти строки, передав данным 
    параметрам наш выше созданный каталог:</p>
<pre class="brush:php;">
    DocumentRoot "/Users/eugene/localhost"
    &lt;Directory "/Users/eugene/localhost"&gt;
</pre>
<p>Напоминаю, что вместо "eugene" в каждом конкретном случае будет идти имя текущего пользователя.</p>
<p>Затем установим имя сервера. Для этого найдем следующую строку:</p>
<pre class="brush:php;">#ServerName www.example.com:8080</pre>
<p>Заменим эту строку на следующую:</p>
<pre class="brush:php;">ServerName localhost</pre>
<p>Изменим пути файлам, в которые будут заноситься сведения об ошибках или посещении сайта.</p>
<p>Если архитектура Intel, найдем строку</p>
<pre class="brush:php;">ErrorLog "/usr/local/var/log/httpd/error_log"</pre>
<p>Если архитектура Apple M1, найдем следующую строку</p>
<pre class="brush:php;">ErrorLog "/opt/homebrew/var/log/httpd/error_log"</pre>
<p>И заменим ее на</p>
<pre class="brush:php;">ErrorLog "/Users/eugene/localhost/error.log"</pre>
<p>Далее изменим путь к логу посещений. Если архитектура Intel, найдем строку</p>
<pre class="brush:php;">CustomLog "/Users/eugene/localhost/access_log" common</pre>
<p>Если архитектура Apple M1, найдем следующую строку</p>
<pre class="brush:php;">CustomLog "/opt/homebrew/var/log/httpd/access_log" common</pre>
<p>И заменим ее на</p>
<pre class="brush:php;">CustomLog "/Users/eugene/localhost/access_log" common</pre>
...
<h4>Настройка PHP</h4>
<p>Теперь свяжем Apache с PHP. Вначале нам надо узнать версию PHP. Для этого введем в терминал команду</p>
<div class="console">
<pre class="consoletext">php -v</pre></div>
<p>Нам должно отобразиться что-то вроде следующего</p>
<div class="console">
<pre class="consoletext">
...
<p>Здесь нас будут интересовать первые две цифры версии. В моем случае выше это <span class="b">8.1</span>. В зависимости от архитектуры компьютера 
также настройка PHP будет отличаться.</p>
<p> найдем в файле строку</p>
<pre class="brush:php;">#LoadModule rewrite_module lib/httpd/modules/mod_rewrite.so</pre>
<p>Если архитектура Intel, после этой строки добавим следующую</p>
<pre class="brush:php;">LoadModule php_module /usr/local/opt/php@8.1/lib/httpd/modules/libphp.so</pre>
<p>Если же архитектура Apple M1, добавим следующую</p>
<pre class="brush:php;">LoadModule php_module /opt/homebrew/opt/php@8.1/lib/httpd/modules/libphp.so</pre>
<p>В данном случае добавляется модуль php. Обратите внимание на часть <span class="b">php@8.1</span>. Поскольку в моем 
    случае версия 8.1, соответственно я указываю данную версию модуля. То есть в итоге получится</p>
<pre class="brush:php;">
...
<p>Далее найдем блок &lt;IfModule mime_module&gt;:</p>
<pre class="brush:php;">
...
<p>И под строкой &lt;IfModule mime_module&gt; добавим две строчки:</p>
<pre class="brush:php;">
...
<p>Теперь наша задача - убедиться, что php подключен и работает правильно. Для этого перейдем в папку <span class="b">/Users/eugene/localhost</span>, которую мы создали для хранения 
...
<p>Теперь перезапустим Apache. Для этого последовательно введем в терминале две следующих команды:</p>
<div class="console">
<pre class="consoletext">
brew services stop httpd
brew services start httpd
</pre></div>
<p>И после перезапуска веб-сервера обратимся к выше созданному скрипту index.php, набрав в строке браузера 
адрес <span class="b">http://localhost/index.php</span></p>
<img src="./pics/macos3.png" alt="phpinfo" />
...
<span class="b">index.php</span>. И так как в файле httpd.conf в качестве хранилища документов веб-сервера указан каталог <span class="b">/Users/eugene/localhost</span>, то именно в этом каталоге и будет 
...
<p>Таким образом, теперь мы можем создавать свои сайты на php на MacOS.</p>
<h3>Настройка php</h3>
<p>Как и для Windows, конфигурация php хранится в файле php.ini. На MacOS  этот файл располагается по следующему пути:</p>
<pre class="brush:php;">
/usr/local/etc/php/[версия]/php.ini
</pre>
<p>Например, я установил версию php 8.1.1, поэтому в моем случай файл php.ini располагается по пути "/usr/local/etc/php/8.1/php.ini". Соответственно, если 
мне необходимо его отредактировать в текстовом редакторе по умолчанию TextEdit, я могу ввести в терминал команду</p>
<div class="console"><pre class="consoletext">open -e /usr/local/etc/php/8.1/php.ini</pre></div>
...
<div class="nav"><p><a href="./1.3.php">Назад</a><a href="./">Содержание</a><a href="./1.4.php">Вперед</a></p></div></div>
<div class="socBlock">
