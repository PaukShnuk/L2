...
<html >
<head>
<title>PHP | Многомерные массивы</title>
<meta charset="utf-8" />
<meta name="description" content="Многомерные массивы в языке PHP, перебор и обращение к элементам массива, цикл foreach">
...
<h2>Многомерные массивы</h2><div class="date">Последнее обновление: 11.03.2021</div>
...
<p>В предыдущих примерах рассматривались только одномерные массивы, где значения элементов представляли числа, строки. 
Но в PHP массивы могут также быть многомерными, то есть такими, где элемент массива сам является массивом.</p>
<p>Например, создадим многомерный массив с помощью функции <code>array()</code>:</p>
<pre class="brush:php;">
$families = array(array("Tom", "Alice"), array("Bob", "Kate"));
</pre>
<p>Сокращенное определение:</p>
<pre class="brush:php;">
$families = [["Tom", "Alice"], ["Bob", "Kate"]];
</pre>
<p>Выше определенный многомерный массив <code>$families</code> является двухмерным (частный случай многомерного массива), который можно представить в виде таблицы:</p>
<table>
<tr><td>Tom</td><td>Alice</td></tr>
<tr><td>Bob</td><td>Kate</td></tr>
</table>
<p>Для элементов данного массива не установлены явным образом ключи, поэтому каждый ключ элемента представляет числовой индекс, которые по умолчанию 
начинаются с нуля. Поэтому чтобы обратиться к первому элементу массива, надо использовать индекс 0 - <code>$families[0]</code>:</p>
<pre class="brush:php;">
$families = [["Tom", "Alice"], ["Bob", "Kate"]];
print_r($families[0]);	// Array ( [0] =&gt; Tom [1] =&gt; Alice )
</pre>
<p>Но поскольку элементами массива $families являются вложенные массивы, то чтобы обратиться к элементам внутри этих вложенных массивов, 
нам необходимо использовать второй индекс - <code>$families[0][0]</code>:</p>
<pre class="brush:php;">
$families = [["Tom", "Alice"], ["Bob", "Kate"]];
...
<p>То есть выражение <code>$families[0]</code> представляет обращение к первому элементу массива $families - к вложенному массиву <code>["Tom", "Alice"]</code>, 
а выражение <code>$families[0][0]</code> представляет обращение к первому элементу вложенного массива, то есть, в данном случае к элементу "Tom".</p>
<p>Перебор многомерного массива:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Для перебора применяется цикл <span class="b">foreach</span>. Цикл проходит по всем элементам массива $families, 
передавая каждый элемент в переменную <code>$family</code>:</p>
<pre class="brush:php;">foreach ($families as $family)</pre>
<p>Однако каждый элемент массива $families, который передается в переменную $family</code>, сам является массивом. 
И чтобы получить элементы из вложенного массива, применяется вложенный цикл:</p>
<pre class="brush:php;">foreach ($family as $user)</pre>
<p>В итоге при переборе циклом сформируется таблица:</p>
<img src="./pics/2.12.png" alt="Многомерные массивы в PHP" />
<p>Также можно определять многомерные ассоциативные массивы:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>И при выводе мы получим 3 списка:</p>
<img src="./pics/2.13.png" alt="Многомерные ассоциативные массивы в PHP" />
...
<img src="./pics/2.14.png" alt="Многомерные ассоциативные массивы в PHP и foreach" />
<p>Получение и изменение элемента в этом массиве:</p>
<pre class="brush:php;">
...
<p>Выше для инициализации массива применялась функция <code>array()</code>, но также можно использовать и сокращенный вариант с 
...
<div class="nav"><p><a href="./2.15.php">Назад</a><a href="./">Содержание</a><a href="./2.9.php">Вперед</a></p></div></div>
<div class="socBlock">
