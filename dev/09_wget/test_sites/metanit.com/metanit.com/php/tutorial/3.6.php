...
<html >
<head>
<title>PHP | Отправка массивов</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка массивов на сервер в запросах типа GET и POST в языке программирования PHP">
...
<h2>Отправка массивов</h2><div class="date">Последнее обновление: 16.03.2021</div>
...
<p>В прошлых темах была рассмотрена отправка на сервер отдельных значений. Однако отправка набора значений, то есть массивов в PHP 
может вызвать некоторые сложности. Рассмотрим, как можно отправить на сервер и соответственно получить на сервере массивы данных.</p>
<p>Например, определим следующий файл <span class="b">users.php</span>:</p>
<pre class="brush:xml;">
&lt;?php
$users = [];
if(isset($_GET["users"])){
...
<p>В данном случае мы предполагаем, что параметр "users", который передается в запросе типа GET, будет представлять массив. И соответствено мы сможем получить 
из него данные.</p>
<p>Чтобы передать массив этому скрипту, обратимся к нему со следующим запросом:</p>
<pre class="brush:php;">http://localhost/users.php?users[]=Tom&users[]=Bob&users[]=Sam</pre>
<p>Чтобы определить параметр строки запроса как массив, после названия параметра указываются квадраные скобки []. Затем мы можем присвоить некоторое значение: <code>users[]=Tom</code>. 
И сколько раз подобным образом будет присвоено значений, столько значений и будет в массиве. Все значения, как и обычно, отделяются амперсандом. Так, в данном случае в массив передаются три значения.</p>
<img src="./pics/3.10.png" alt="Передача массивов в PHP на сервер в запросе GET" />
<p>Подобным образом мы можем отправлять данные в запросе POST из формы. Например, определим следующий скрипт:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Как известно, название ключа передаваемых на сервер данных соответствует значению атрибута <code>name</code> у элемента формы. И чтобы указать, 
что какое-то поле ввода будет поставлять значение для массива, у атрибут <code>name</code> поля ввода в качестве значения принимает название массива с квадратными скобками:</p>
<pre class="brush:xml;">&lt;input type="text" name="users[]" /&gt;</pre>
<p>Соответственно, сколько полей ввода с одним и тем же именем массива мы укажем, столько значений мы сможем передать на сервер. Так, в данном случае 
на сервер передается три значения в массиве users:</p>
<img src="./pics/3.11.png" alt="Отправка массивов на сервер методом POST из формы в PHP" />
<p>Причем данный принцип применяется и к другим типам полей ввода формы html.</p>
<p>При этом в примерах выше передавался обычный массив, каждый элемент которого в качестве ключа имеет числовой индекс. Соотвенно, используя индекс, мы 
можем получить определенный элемент массива:</p>
<pre class="brush:php;">
$firstUser = $_POST["users"][0];
echo $firstUser;
</pre>
<p>Но также мы можем в элементах формы явным образом указать ключи:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Например, первое поле добавляет в массив элемент с ключом "first"</p>
<pre class="brush:xml;">&lt;input type="text" name="users[first]" /&gt;</pre>
<p>Поэтому на сервере мы можем с помощью данного ключа получить соответствующий элемент:</p>
<pre class="brush:xml;">$firstUser = $_POST["users"]["first"];</pre>
...
<div class="nav"><p><a href="./3.5.php">Назад</a><a href="./">Содержание</a><a href="./3.3.php">Вперед</a></p></div></div>
<div class="socBlock">
