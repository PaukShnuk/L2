...
<html >
<head>
<title>PHP | Копирование объектов классов</title>
<meta charset="utf-8" />
<meta name="description" content="Копирование объектов классов в языке программирования PHP, оператор clone, функция __clone">
...
<h2>Копирование объектов классов</h2><div class="date">Последнее обновление: 31.03.2021</div>
...
<p>При присваивании объекта класса другой переменной создается новая ссылка на тот же объект. Например:</p>
<pre class="brush:php;">
class Person{
...
<p>В данном случае после присваивания <code>$bob = $tom;</code> обе переменных будут указывать на один и тот же объект. Поэтому 
если мы поменяем свойство <code>$name</code> у одной переменной, то это измение затронет и другую переменую. Так как они ссылаются на один и тот же 
объект.</p>
<p>Однако такое поведение может быть нежелательным, если мы хотим, чтобы после копирования переменные представляли независимые друг от друга объекты. 
И для этого PHP предоставляет оператор <span class="b">clone</span>:</p>
<pre class="brush:php;">
class Person{
...
<p>При применении оператора <code>clone</code> все свойства, которые представляют примитивные типы и массивы, копируются в новый объект. 
Однако, что если у нас свойство класса представляет другой класс:</p>
<pre class="brush:php;">
class Company{
...
<p>Здесь в принципе мы сталкиваемся с той же ситуацией, что и в первом примере. Класс Person содержит свойство, которое представляет класс Company. 
При клонировании объекта:</p>
<pre class="brush:php;">$bob = clone $tom;</pre>
<p>Обе переменных $tom и $bob будут содержать ссылку на один и тот же объект Company. Соответственно если через одну переменную изменить 
свойства этого объекта:</p>
<pre class="brush:php;">$bob-&gt;company-&gt;name = "Google";</pre>
<p>то изменение затронет и другую переменную:</p>
<pre class="brush:php;">echo $tom-&gt;company-&gt;name;	// Google</pre>
<p>Чтобы выйти из этой ситуации, необходимо в классе определить метод <span class="b">__clone</span>. Он вызывается при клонировании и 
может применяться для клонирования вложенных объектов:</p>
<pre class="brush:php;">
class Company{
...
<div class="nav"><p><a href="./6.9.php">Назад</a><a href="./">Содержание</a><a href="./2.11.php">Вперед</a></p></div></div>
<div class="socBlock">
