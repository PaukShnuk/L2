...
<html >
<head>
<title>PHP | Стрелочные функции</title>
<meta charset="utf-8" />
<meta name="description" content="Стрелочные функции arrow function в языке программирования PHP, оператор fn">
...
<h2>Стрелочные функции</h2><div class="date">Последнее обновление: 11.03.2021</div>
...
<p>Стрелочные функции (arrow function) позволяют упростить запись анонимных функций, которые возвращают некоторое значение. И при этом стрелочные функции автоматически имеют доступ к переменным 
из внешнего окружения.</p>
<p>Стрелочная функция определяется с помощью оператора <span class="b">fn</span>:</p>
<pre class="brush:php;">fn(параметры) =&gt; действия;</pre>
<p>После оператора <code>fn</code> в скобках идет список параметров. Затем указывается оператор <span class="b">=&gt;</span>. А после него располагаются 
действия функции, которые возвращают некоторый результат.</p>
<p>Например:</p>
<pre class="brush:php;">
$a = 8; 
$b = 10;
...
<p>В данном случае определение стрелочной функции</p>
<pre class="brush:php;">$closure = fn($c) =&gt; $a + $b + $c;</pre>
<p>Фактически будет аналогично:</p>
<pre class="brush:php;">
...
<p>Только в отличие от стандартных анонимных функций стрелочный функции предоставляют более лаконичный синтаксис.</p>
<p>Также стрелочные функции могут применяться в качестве параметров функции:</p>
<pre class="brush:php;">
...
<div class="nav"><p><a href="./2.20.php">Назад</a><a href="./">Содержание</a><a href="./2.22.php">Вперед</a></p></div></div>
<div class="socBlock">
