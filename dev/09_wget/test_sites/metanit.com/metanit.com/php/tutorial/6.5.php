...
<html >
<head>
<title>PHP | Конструкторы и деструкторы</title>
<meta charset="utf-8" />
<meta name="description" content="Конструкторы и деструкторы классов в языке программирования PHP, функции __construct и __destruct, параметры конструктора, определение свойств в конструкторе">
...
<h2>Конструкторы и деструкторы</h2><div class="date">Последнее обновление: 18.03.2021</div>
...
<p>Конструкторы представляют специальные методы, которые выполняются при создании объекта и служат для начальной инициализации его свойств. Для создания 
конструктора надо объявить функцию с именем <span class="b">__construct</span> (с двумя подчеркиваниями впереди):</p>
<pre class="brush:php;">
&lt;?php
class Person
{ 
...
<p>Функция конструктора в данном случае принимает два параметра. Их значения передаются свойствам класса. И теперь чтобы создать объект, нам надо передать 
значения для соответствующих параметров:<p>
<pre class="brush:php;">$tom = new Person("Tom", 36);</pre>
<p>Вывод браузера:</p>
<pre class="browser">
Name: Tom; Age: 36
Name: Bob; Age: 41
...
<p>Чтобы сделать конструктор более гибким, мы можем обозначить один или несколько параметров в качестве необязательных. Тогда при создании объекта необязательно 
указывать все параметры. Например, изменим конструктор следующим образом:</p>
<pre class="brush:php;">
...
<p>Таким образом, если не будут заданы параметры, вместо них будут использоваться значения "Том" и 36. И теперь мы можем создать объект Person несколькими способами:</p>
<pre class="brush:php;">
&lt;?php
class Person
{ 
...
<h3>Объявление свойств через конструктор</h3>
<p>Начиная с версии PHP 8 в языке появилась возможность определить свойства через список параметров конструктора.
Любой параметр конструктора, который имеет модификатор доступа, например, <span class="b">public</span>, 
будет автоматически представлять новое свойство.</p>
<p>Например, определим свойства name и age напрямую через параметры конструктора:</p>
<pre class="brush:php;">
&lt;?php
class Person
{ 
...
<p>Хотя в данном случае в классе явным образом не определены свойства name и age, но поскольку в списке параметров конструктора заданы параметры 
с такими именами и с модификатором доступа (в данном случае <code>public</code>), то у класса автоматически создаются подобные переменные.</p>
<p>Можно сочетать оба способа объявления переменных:</p>
<pre class="brush:php;">
class Person
{ 
	public $name;
...
<p>При подобном объявлении свойств также можно передавать им значения по умолчанию:</p>
<pre class="brush:php;">
class Person
{ 
	public $name;
...
<h3>Деструкторы</h3>
<p>Деструкторы служат для освобождения ресурсов, используемых программой - для освобождения открытых файлов, открытых подключений к базам данных и т.д. 
Деструктор объекта вызывается самим интерпретатором PHP после потери последней ссылки на данный объект в программе.</p>
<p>Деструктор определяется с помощью функции <span class="b">__destruct</span> (два подчеркивания впереди):</p>
<pre class="brush:php;">
class Person
{
	public $name, $age;
...
<p>Функция деструктора определяется без параметров, и когда на объект не останется ссылок в программе, он будет уничтожен, и при этом будет вызван деструктор.</p>
...
<div class="nav"><p><a href="./6.1.php">Назад</a><a href="./">Содержание</a><a href="./6.6.php">Вперед</a></p></div></div>
<div class="socBlock">
