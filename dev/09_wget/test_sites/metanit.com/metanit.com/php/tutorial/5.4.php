...
<html >
<head>
<title>PHP | Отправка файлов на сервер</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка файлов на сервер в скриптах PHP, глобальный массив $_FILES, множественная загрузка файлов">
...
<h2>Отправка файлов на сервер</h2><div class="date">Последнее обновление: 17.03.2021</div>
...
<p>Чтобы загрузить файл на сервер, нам надо использовать форму с параметром <code>enctype="multipart/form-data"</code> и массив <code>$_FILES</code>. 
Итак, создадим файл <span class="b">upload.php</span> со следующим содержимым:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь определена форм с атрибутом <code>enctype="multipart/form-data"</code>. Форма содержит специальное поле для выбора файла.</p>
<p>Все загружаемые файлы попадают в ассоциативный массив <span class="b">$_FILES</span>. Чтобы определить, а есть ли вообще загруженные файлы, можно использовать 
конструкцию if: <code>if ($_FILES)</code></p>
<p>Массив $_FILES является двухмерным. Мы можем загрузить набор файлов, и каждый загруженный файл можно получить по ключу, который совпадает со значением 
атрибута <code>name</code>.</p>
<p>Так как элемент для загрузки файла на форме имеет <code>name="filename"</code>, то данный файл мы можем получить с помощью выражения <code>$_FILES["filename"]</code>.</p>
<P>У каждого объекта файла есть свои параметры, которые мы можем получить:</P>
<ul>
<li><p><code>$_FILES["file"]["name"]</code>: имя файла</p></li>
<li><p><code>$_FILES["file"]["type"]</code>: тип содержимого файла, например, <code>image/jpeg</code></p></li>
<li><p><code>$_FILES["file"]["size"]</code>: размер файла в байтах</p></li>
<li><p><code>$_FILES["file"]["tmp_name"]</code>: имя временного файла, сохраненного на сервере</p></li>
<li><p><code>$_FILES["file"]["error"]</code>: код ошибки при загрузке</p></li>
</ul>
<p>Также мы можем проверить наличие ошибок при загрузке. Если у нас нет ошибки, то поле <code>$_FILES["filename"]["error"]</code> содержит значение <code>UPLOAD_ERR_OK</code>.</p>
<p>При отправке файла на сервер он сначала загружается во временное место, из которого затем с помощью функции <span class="b">move_uploaded_file()</span> 
он перемещается в каталог сервера, где расположен скрипт "upload.php".</p>
<img src="./pics/3.18.png" alt="Загрузка файлов на сервер в PHP" />
<p>Также мы можем указать другой путь, например, допустим, на сервере есть папка "upload", тогда, чтобы загружать в нее файлы, необходимо указать 
соответствующий путь:</p>
<pre class="brush:xml;">
if ($_FILES && $_FILES["filename"]["error"]== UPLOAD_ERR_OK)
{
...
<p>Функция <code>move_uploaded_file()</code> принимает два параметра путь к загруженному временному файлу и путь, куда надо поместить загруженный файл.</p>
<h3>Ограничения и настройка загрузки</h3>
<p>По умолчанию размер загружаемых файлов ограничен 2 мб. Однако можно настроить данный показатель в файле конфигурации. Изменим этот показатель, например, 
до 10 мб. Для этого найдем в файле <span class="b">php.ini</span> следующую строку:</p>
<pre>upload_max_filesize = 2M</pre>
<p>Изменим ее на</p>
<pre>upload_max_filesize = 10M</pre>
<p>Также мы можем настроить папку для временных загружаемых файлов. Для этого в файле <span class="b">php.ini</span> найдем следующую строку:</p>
<pre>;upload_tmp_dir =</pre>
<p>Изменим ее на</p>
<pre>upload_tmp_dir = "C:/php/upload"</pre>
<p>Также в каталоге php нам надо создать папку <span class="b">upload</span>.</p>
<h3>Мультизагрузка</h3>
<p>Изменим скрипт <span class="b">upload.php</span> так, чтобы он поддерживал множественную загрузку:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Каждое поле выбора файла имеет атрибут <code>name="uploads[]"</code>, поэтому сервер будет рассматривать набор отправленных файлов как единый массив.</p>
<p>Затем используя цикл <code>foreach</code>, проходим по все файлам и сохраняем их в каталог веб-сайта.</p>
