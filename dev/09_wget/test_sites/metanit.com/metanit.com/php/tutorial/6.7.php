...
<html >
<head>
<title>PHP | Интерфейсы</title>
<meta charset="utf-8" />
<meta name="description" content="Интерфейсы в языке программирования PHP, ключевое слово interface, реализация в классах">
...
<h2>Интерфейсы</h2><div class="date">Последнее обновление: 22.03.2021</div>
...
<p>Интерфейс определяет абстрактный дизайн, которому должен соответствовать применяющий его класс. Интерфейс определяет методы без реализации. 
А класс затем применяет интерфейс и реализует эти методы. Применение интерфейса гарантирует, что класс имеет определенный функционал, описываемый интерфейсом.</p> 
<p>Интерфейс определяется с помощью ключевого слова <span class="b">interface</span>, за которым следует имя интерфейса и блок кода интерфейса в фигурных скобках:</p>
<pre class="brush:php;">
interface iMessenger {}
</pre>
<p>Здесь определен интерфейс <code>iMessenger</code>. Обычно имя интерфейса начинается со строчной буквы "i", но это условность.</p>
<p>Внутри блока интерфейса в фигурных скобках определяются сигнатуры методов. Причем все эти методы могут быть только публичными, то есть с модификатором 
<code>public</code>, либо без модификатора доступа (что аналоично модификатору public):</p>
<pre class="brush:php;">
interface iMessenger
{
	function send();
...
<p>Интерфейсы могут содержать лишь сигнатуры методов - наазвания функций и список параметров в скобках, после которого идет точка с запятой. 
Так, в данном случае объявлен метод <code>send()</code>. Он не имеет реализации - конкретную реализацию определит класс, который реализует этот интерфейс.</p>
<p>Для реализации классом интерфейса применяется ключевое слово <span class="b">implements</span>, после которого указывается имя применяемого интерфейса:</p>
<pre class="brush:php;">
&lt;?php
interface iMessenger
{
...
<p>В данном случае класс EmailMessenger реализует интерфейс iMessenger. Если класс применяет интерфейс, то он должен реализовать все методы этого интерфейса. 
Так, в данном случае класс EmailMessenger определяет метод <code>send()</code> с некоторой реализацией.</p>
...
<p>Когда нам могут понадобиться интерфейсы? Интерфейс - это контракт, который говорит, что класс обязательно реализует определенный функционал. 
И мы можем использовать это в своей программе. Например, определим следующий код:</p>
<pre class="brush:php;">
&lt;?php
interface iMessenger
{
...
<p>Для отправки сообщения здесь определена функция <code>sendMessage()</code>, которая в качестве первого параметра принимает объект мессандера, а 
...
должно реализовать интерфейс iMessenger. В самой функции мы знаем, что первый параметр - это объект, который обязательно реализует интерфейс iMessenger, 
поэтому мы можем вызвать у него метод <code>send()</code> для отправки сообщения:</p>
<pre class="brush:php;">
function sendMessage(iMessenger $messenger, $text)
{
	$messenger-&gt;send($text);
...
<h3>Множественное применение интерфейсов</h3>
<p>Класс может одновременно применять сразу несколько интерфейсов. В этом случае все интерфейсы перечисляются через запятую после слова 
<span class="b">implements</span>. А класс должен реализовать методы всех применяемых интерфейсов:</p>
<pre class="brush:php;">
interface iCamera
{
	function makeVideo();
...
<div class="nav"><p><a href="./6.4.php">Назад</a><a href="./">Содержание</a><a href="./6.8.php">Вперед</a></p></div></div>
<div class="socBlock">
