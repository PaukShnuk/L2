...
<html >
<head>
<title>PHP | Отправка форм. POST-запросы</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка данных форм html в запросах типа GET и POST в языке программирования PHP, глобальный массив $_POST">
...
<h2>Отправка форм</h2><div class="date">Последнее обновление: 15.03.2021</div>
...
<p>Одним из основных способов передачи данных веб-сайту является обработка форм. Формы представляют специальные элементы разметки HTML, которые содержат 
в себе различные элементы ввода - текстовые поля, кнопки и т.д. И с помощью данных форм мы можем ввести некоторые данные и отправить их на сервер. А сервер уже обрабатывает эти данные.</p>
<p>Создание форм состоит из следующих аспектов:</p>
<ul>
<li><p>Создание элемента <code>&lt;form&gt;&lt;form&gt;</code> в разметке HTML</p></li>
<li><p>Добавление в этот элемент одно или несколько поле ввода</li>
<li><p>Установка метода передачи данных. Чаще всего используются методы <code>GET</code> или <code>POST</code></p></li>
<li><p>Установка адреса, на который будут отправляться введенные данные</li>
</ul>
<h3>POST-запросы</h3>
<p>Итак, создадим новую форму. Для этого определим новый файл <span class="b">form.php</span>, в которое поместим следующее содержимое:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Атрибут <code>action="user.php"</code> элемента <code>form</code> указывает, что данные формы будет обрабатывать скрипт <span class="b">user.php</span>, 
который будет находиться с файлом <code>form.php</code> в одной папке. А атрибут <code>method="POST"</code> указывает, что в качестве метода передачи данных будет применяться метод POST.</p>
<p>Теперь определим файл <code>user.php</code>, который будет иметь следующее содержание:</p>
<pre class="brush:xml;">
&lt;?php
$name = "не определено";
$age = "не определен";
...
<p>Для обработки запросов типа POST в PHP используется встроенная глобальная переменная <span class="b">$_POST</span>. 
...
значения. Ключами в этом массиве являются значения атрибутов <code>name</code> у полей ввода формы.</p>
<p>Например, так как атрибут <code>name</code> поля ввода возраста имеет значение <code>age</code> (<code>&lt;input type="number" name="age" /&gt;</code>), 
то в массиве <code>$_POST</code> значение этого поля будет представлять ключ "age": <code>$_POST["age"]</code></p>
<p>И поскольку возможны ситуации, когда поле ввода будет не установлено, то в этом случае желательно перед обработкой данных проверять 
их наличие с помощью функции <span class="b">isset()</span>. И если переменная установлена, то функция isset() возвратит значение <code>true</code>.</p>
<p>Теперь мы можем обратиться к скрипту <span class="b">form.php<span> и ввести в форму какие-нибудь данные:</p>
<img src="./pics/3.3.png" alt="Обработка форм в PHP" />
<p>И по нажатию кнопки введенные данные методом POST будут отправлены скрипту <span class="b">user.php</span>:</p>
<img src="./pics/3.4.png" alt="массив $_POST в PHP" />
<p>Необязательно отправлять данные формы другому скрипту, можно данные формы обработать в том же файле формы. Для этого изменим файл <span class="b">form.php</span> 
следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<p>Поскольку в данном случае мы отправляем данные этому же скрипту - то есть по тому же адресу, то у элемента форма можно не устанавливать 
атрибут <code>action</code>.</p>
<img src="./pics/3.5.png" alt="Отправка формы в PHP" />
<p>Стоит отметить, что в принципе мы можем отправлять формы и запросом GET, в этом случае для получения тех же значений формы 
применяется массив <span class="b">$_GET</span>, который был рассмотрен в прошлой теме:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
