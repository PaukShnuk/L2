...
<meta charset="utf-8" />
<meta name="description" content="Массивы в языке PHP, определение массивов и функция array, количество элементов массива и функция count, ключи элементов массива, перебор и обращение к элементам массива, цикл foreach">
...
<h2>Массивы</h2><div class="date">Последнее обновление: 11.03.2021</div>
...
<p>Ранее мы рассмотрели, как в переменные можно сохранить одиночное значение, например, одно число или одну строку. Но кроме того, 
мы можем сохранить в переменную набор значений. И для этого используются <span class="b">массивы</span>.</p>
<p>Есть несколько способов определения массивов. Первый способ представляет использование функции <span class="b">array()</span>:</p>
<pre class="brush:php;">$numbers = array();</pre>
<p>В данном случае определяется пустой массив <code>$numbers</code>.</p>
<p>Второй способ представляет использование квадратных скобок <span class="b">[]</span>:</p>
<pre class="brush:php;">$numbers = [];</pre>
<p>При определении массива мы сразу можем передать ему начальные данные. Если применяются квадратные скобки, то элементы массива передаются внутри скобок:</p>
<pre class="brush:php;">$numbers = [1, 2, 3, 4];</pre>
<p>Аналогичное определение массива с помощью функции <code>array()</code>:</p>
<pre class="brush:php;">$numbers = array(1, 2, 3, 4);</pre>
<p>В качестве элементов массива могут выступать объекты любого типа.</p>
<p>Для обращения к элементам массива применяются <span class="b">ключи</span>. Ключ может представлять число или строку или одновременно и числа, и строки.</p>
<p>Для обычных массивов ключ представляет число. Числовые ключи еще называют индексами. Нумерация индексов начинается с нуля, то есть первый элемент массива имеет индекс 0, второй элемент - индекс 1 и так далее.</p>
<p>Например, обратимся к третьему элементу массива:</p>
<pre class="brush:php;">
&lt;?php
$numbers = [1, 4, 9, 16];
echo $numbers[2];	// 9
...
<p>Поскольку нумерация индексов начинается с нуля, то чтобы обратиться к третьему элементу, надо использовать индекс 2.</p>
<p>Таким образом мы получаем значение элемента массива. Но также мы можем его изменить:</p>
<pre class="brush:php;">
&lt;?php
$numbers = [1, 4, 9, 16];
$numbers[1] = 6;
...
<p>При этом нужно учитывать количество элементов массива. Так, мы не можем обратиться к элементу с несуществующим индексом:</p>
<pre class="brush:php;">
&lt;?php
$numbers = [1, 4, 9, 16];
echo $numbers[4];	// 6
...
<p>В данном случае в массиве $numbers всего 4 элемента, поэтому индексом последнего элемента будет 3. Таким образом, элемента с индексом 4 в массиве не существует, и 
при попытке получить его значение <code>echo $numbers[4]</code> PHP нам покажет предупреждение.</p>
<p>Тем не менее если мы хотим установить элемент по еще не существующему индексу, мы можем это сделать:</p>
<pre class="brush:php;">
&lt;?php
$numbers = [1, 4, 9, 16];
$numbers[5] = 76;
...
<p>Здесь устанавливается элемент с индексом 5. После установки мы можем получать его значение. При этом элемента с индексом 4 по прежнему не существует.</p>
<p>Для добавления нового элемента в массив мы можем, как в примере выше, просто установить новый элемент по еще не установленному индексу. Но есть и другой способ:</p>
<pre class="brush:php;">
&lt;?php
$numbers = [1, 4, 9, 16];
$numbers[] = 25;
...
<p>При таком способе новый элемент добавляется в конец массив, соответственно здесь, чтобы получить новый элемент, надо использовать индекс 4.</p>
<p>Чтобы получить полное наглядное представление о том, как в конкретном массиве сопоставляются ключи и значения элементов, можно использовать 
функцию <code>print_r</code>, в которую в качестве параметра передается массив:</p>
<pre class="brush:php;">
&lt;?php
$numbers = [1, 4, 9, 16];
$numbers[] = 25;
...
<p>Также следует отметить, что необязательно как-то специально инициализровать переменную массива - мы можем по ходу добавлять элементы в массив:</p>
<pre class="brush:php;">
&lt;?php
$numbers[] = 20;
$numbers[] = 120;
...
<h3>Оператор =&gt;</h3>
<p>Оператор <span class="b">=&gt;</span> позволяет сопоставить ключ с определенным значением. Хотя при определении массива выше нам не требовался 
подобный оператор, тем не менее мы можем его применять. Например, следующий массив:</p>
<pre class="brush:php;">$numbers = [1, 4, 9, 16];</pre>
<p>Будет аналогичен следующему массиву:</p>
<pre class="brush:php;">
$numbers = [0=&gt;1, 1=&gt;4, 2=&gt;9, 3=&gt;16];
// $numbers = array(0=&gt;1, 1=&gt;4, 2=&gt;9, 3=&gt;16);
</pre>
<p>Каждый элемент определяется в следующем формате: <code>ключ =&gt; значение</code></p>
<p>В дальнейшем мы также можем обращаться к элементам этого массива.</p>
<p>Этот оператор может понадобится, если мы хотим переопределить порядок индексов. Так, по умолчанию нумеация индексов начинается с нуля и 
каждый следующий элемент имеет индекс предыдущего элемента + 1. Оператор <span class="b">=&gt;</span> же позволяет определить свои индексы 
вручную для элементов, необязательно с нуля и необязательно по порядку:</p>
<pre class="brush:php;">
$numbers = [1=&gt; 1, 2=&gt; 4, 5=&gt; 25, 4=&gt; 16];
echo $numbers[2];	// 4
</pre>
<p>Также мы можем задать индекс лишь для одного элемента, тогда для последующих элементов индекс автоматически будет увеличиваться на единиицу:</p>
<pre class="brush:php;">
$numbers = [4=&gt; 16, 25, 36, 49, 64];
print_r($numbers);
</pre>
<p>В данном случае индексация начинается с числа 4 - это индекс элемента 16. Тогда для элемента 25 индексом будет число 5 и так далее.</p>
<p>Результат:</p>
<pre class="browser">
Array ( [4] =&gt; 16 [5] =&gt; 25 [6] =&gt; 36 [7] =&gt; 49 [8] =&gt; 64 )
</pre>
<h3>Перебор массива</h3>
<p>Для перебора массива мы можем применять стандартный метод <span class="b">for</span>:</p>
<pre class="brush:php;">
&lt;?php
...
<p>В данном случае мы не определяем вручную индексы элементов, поэтому индексация начинается с нуля с увеличением на единиицу для последующего элемента. 
Поэтому мы можем в цикле через переменную <code>$i</code> передавать индекс, начиная с нуля. Единственная трудность может заключаться в том, 
как определить индекс конечного элемента, чтобы установить потолок для переменной <code>$i</code>. В этом случае мы можем с помощью встроенной 
функции <span class="b">count()</span> получить длину массива. Эта функция в качестве параметра принимает массив и возвращает его длину.</p>
...
<h3>Цикл foreach</h3>
<p>Тем не менее выше использованный способ перебора не поможет, если индексы определяются вручную и отличаются они от соседних индексов не на единицу, а 
на произвольную величину. В этом случае мы можем использовать специальный цикл - <span class="b">foreach</span>:</p>
<pre class="brush:php;">
&lt;?php
...
<p>В цикле <span class="b">foreach</span> из массива последовательно извлекаются все элементы, и их значение помещается в переменную, 
указанную после ключевого слова <span class="b">as</span>. В данном случае в переменную <code>$element</code> по очереди помещаются 
все четыре значения из массива <code>$users</code>. Когда будет извлечен последний элемент из массива, цикл завершается.</p>
...
<p>Цикл <span class="b">foreach</span> позволяет извлекать не только значения, но и ключи элементов:</p>
<pre class="brush:php;">
&lt;?php
$users = [1 =&gt; "Tom", 4 =&gt; "Sam", 5 =&gt; "Bob", 21 =&gt; "Alice"];
$num = count($users);
...
<p>Здесь при переборе элементов цикла в переменную <code>$key</code> будет передаваться ключ элемента, а в переменную <code>$value</code> - 
его значение.</p>
<p>Результат работы скрипта:</p>
<pre class="browser">
1 - Tom
...
<div class="nav"><p><a href="./2.8.php">Назад</a><a href="./">Содержание</a><a href="./2.15.php">Вперед</a></p></div></div>
<div class="socBlock">
