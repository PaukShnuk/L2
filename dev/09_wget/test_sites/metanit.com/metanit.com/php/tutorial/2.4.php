...
<html >
<head>
<title>PHP | Константы</title>
<meta charset="utf-8" />
<meta name="description" content="Константы в языке программирования PHP, оператор const, определение константы с помощью функции define, проверка существования констант и defined">
...
<h2>Константы</h2><div class="date">Последнее обновление: 07.12.2021</div>
...
<p>Константы, как и переменные хранят определенное значение, только в отличие от переменных значение констант может быть установлено только один раз, и далее мы уже 
не можем его изменить. Константы обычно определяются для хранения значений, которые должны оставаться неизменными на протяжении всей работы скрипта.</p>
<h3>Оператор const</h3>
<p>Для определения константы применяется оператор <span class="b">const</span>, при этом в названии константы знак доллара $ (в отличие от переменных) не используется.</p>
<pre class="brush:php;">
&lt;?php
const PI = 3.14;
echo PI;
...
<p>Обычно названия констант используют заглавные символы, но это условность.</p>
<p>После определения константы мы можем ее использовать также, как и обычную переменную.</p>
<p>PHP позволяет устанавливать значения констант 
на основе вычисляемых выражений:</p>
<pre class="brush:php;">
&lt;?php
const PI = 2.1415 + 1;
echo PI;		// 3.1415
...
<p>Единственное исключение - мы не сможем изменить значение константы. То есть выражение <code>PI = 3.1415;</code>, которое должно изменить значение константы, не срабатает.</p>
<h3>Функция define</h3>
<p>Также для определения константы может применяться функция <span class="b">define()</span>, которая имеет следующую форму:</p>
<pre class="brush:php;">define(string $name, string $value)</pre>
<p>Параметр <code>$name</code> передает название константы, а параметр 
<code>$value</code> - ее значение. Значение константы может представлять тип <code>int, float, string, bool, null</code> или  массивы.</p>
<p>Например, определим числовую константу:</p>
<pre class="brush:php;">
&lt;?php
define("NUMBER", 22);
echo NUMBER;	// 22
...
<p>Кроме создаваемых программистом констант в PHP имеется еще несколько так называемых "магических" констант, которые есть в языке по умолчанию:</p>
<ul>
<li><p><span class="b">__FILE__</span>: хранит полный путь и имя текущего файла</p></li>
<li><p><span class="b">__LINE__</span>: хранит текущий номер строки, которую обрабатывает интерпретатор</p></li>
<li><p><span class="b">__DIR__</span>: хранит каталог текущего файла</p></li>
<li><p><span class="b">__FUNCTION__</span>: название обрабатываемой функции</p></li>
<li><p><span class="b">__CLASS__</span>: название текущего класса</p></li>
<li><p><span class="b">__TRAIT__</span>: название текущего трейта</p></li>
<li><p><span class="b">__METHOD__</span>: название обрабатываемого метода</p></li>
<li><p><span class="b">__NAMESPACE__</span>: название текущего пространства имен</p></li>
<li><p><span class="b">::class/span>: полное название текущего класса</p></li>
</ul>
<p>Например, выведем текущую выполняемую строку и название файла:</p>
<pre class="brush:php;">
&lt;?php
echo "Cтрока " . __LINE__ . " в файле " . __FILE__;
?&gt;
</pre>
<h3>Проверка существования константы</h3>
<p>Чтобы проверить, определена ли константы, мы можем использовать функцию <span class="b">bool defined(string $name)</span>. 
Если константа <code>$name</code> определена, то функция будет возвращать значение <code>true</code></p>:
<pre class="brush:php;">
&lt;?php
const PI = 3.14; 
if (!defined("PI"))
...
<div class="nav"><p><a href="./2.10.php">Назад</a><a href="./">Содержание</a><a href="./2.23.php">Вперед</a></p></div></div>
<div class="socBlock">
