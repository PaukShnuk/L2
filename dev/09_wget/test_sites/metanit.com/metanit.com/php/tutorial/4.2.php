...
<html >
<head>
<title>PHP | Работа с cookie</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с cookie в языке программирования PHP, функция setcookie, установка, получение и удаление куки, глобальный массив $_COOKIE">
...
<h2>Работа с cookie</h2><div class="date">Последнее обновление: 21.03.2021</div>
...
<p>Cookie (куки) представляют небольшие наборы данных (не более 4 кБайт), с помощью которых веб-сайт может сохранить на 
...
нет, отслеживать историю его визитов и т.д.</p>
<h3>Сохранение cookie</h3>
<p>Для установки куки на компьютере пользователя используется функция <span class="b">setcookie()</span>. Она должна вызываться перед 
тем, как будет отправлен ответ пользователю. Эта функция имеет следующее определение:</p>
<pre class="brush:php;">
bool setcookie(string $name, string $value, int $expire, 
	string $path, string $domain, bool $secure, bool $httponly);
</pre>
<p>Функция <code>setcookie()</code> может принимать следующие параметры:</p>
<ul>
<li><p><code>name</code>: имя cookie, которое будет использоваться для доступа к его значению</p></li>
<li><p><code>value</code>: значение или содержимое cookie - любой алфавитно-цифровой текст не более 4 кБайт</p></li>
<li><p><code>expire</code> (необязательный параметр): срок действия в секундах, после которого cookie уничтожаются. Если данный параметр не установлен или равен 0, 
...
<code>time()</code></p></li>
<li><p><code>path</code> (необязательный параметр): путь к каталогу на сервере, для которого будут доступны cookie. 
Если задать "/", cookie будут доступны для всего сайта. Если задать, например, <code>"/mydir/"</code>, cookie будут доступны только из 
каталога <span class="i">/mydir/</span> и всех его подкаталогов. По умолчанию значением является текущий каталог, в котором устанавливаются cookie.</p></li>
<li><p><code>domain</code> (необязательный параметр): задает домен, для которого будут доступны cookie. Если это домен второго уровня, например, 
<span class="i">localhost.com</span>, то cookie доступны для всего сайта localhost.com, в том числе и для его поддоменов типа <span class="i">blog.localhost.com</span>.</p>
<p>Если задан поддомен <span class="i">blog.localhost.com</span>, то cookie доступны только внутри этого поддомена.</p></li>
<li><p><code>secure</code> (необязательный параметр): указывает на то, что значение cookie должно передаваться по протоколу HTTPS. 
Если задано <code>true</code>, cookie от клиента будет передано на сервер, только если установлено защищенное соединение. По умолчанию равно <code>false</code>.</p></li>
<li><p><code>httponly</code> (необязательный параметр): если равно <code>true</code>, cookie будут доступны только через http протокол. 
То есть cookie в этом случае не будут доступны скриптовым языкам, например, JavaScript.  По умолчанию параметр равен <code>false</code></p></li>
</ul>
<p>Сохраним cookie:</p>
<pre class="brush:php;">
&lt;?php
...
<p>Здесь устанавливаются две куки: "name" и "age". Первая куки уничтожается после закрытия браузера, а вторая - через 3600 секунд, то есть через час.</p>
<p>При необходимости мы можем увидеть сохраненные куки в браузере с помощью инструментов разработчика. Например, вид куки в Google Chrome:</p>
<img src="./pics/4.1.png" alt="Куки cookie в PHP и  setcookie" />
<p>В cookie можно сохранить любую информацию, но не стоит сохранять важные с точки зрения безопасности данные, например, пароли. А если и сохранять какую-то важную информацию, то 
следует хранить ее в зашифрованном виде.</p>
<h3>Получение cookie</h3>
<p>Чтобы получить cookie, можно использовать глобальный ассоциативный массив <span class="b">$_COOKIE</span>, например, <code>$_COOKIE["name"]</code>. Так, получим ранее сохраненные куки:</p>
<pre class="brush:php;">
&lt;?php
...
<h3>Сохранение массивов в cookie</h3>
<p>Сохранение в куки массивов имеет некоторые особенности. Например, сохраним следующий массив:</p>
<pre class="brush:php;">
setcookie("lang[1]", "PHP");
setcookie("lang[2]", "C#");
setcookie("lang[3]", "Java");
</pre>
<p>Теперь получим его и выведем на страницу:</p>
<pre class="brush:php;">
...
<p>Для удаления cookie достаточно в качестве срока действия указать какое-либо время в прошлом:</p>
<pre class="brush:php;">
