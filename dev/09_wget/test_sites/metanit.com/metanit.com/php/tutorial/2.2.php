...
<meta charset="utf-8" />
<meta name="description" content="Переменные в PHP, правила определения и наименования переменных, регистрозависимость, присвоения значения">
...
<h2>Переменные</h2><div class="date">Последнее обновление: 28.02.2021</div>
...
<p>Переменные хранят отдельные значения, которые можно использовать в выражениях PHP. Для определения переменных 
применяется знак доллара <code>$</code>. Например:</p>
<pre class="brush:php;">
$num;
</pre>
<p>Здесь определена переменная <code>$num</code>. Поскольку определение переменной - это отдельная инструкция, она завершается 
точкой с запятой.</p>
<p>Как правило, названия переменный начинаются с маленькой буквы или символа подчеркивания. 
Стоит учитывать, что PHP является регистрозависимым языком, а значит, переменные <code>$num</code> и <code>$Num</code> 
будут представлять две разные переменные.</p>
<p>Также при наименовании переменных нам надо учитывать следующие правила:</p>
<ul>
<li><p>Имена переменных должны начинаться с алфавитного символа или с подчеркивания</p></li>
<li><p>Имена переменных могут содержать только символы: a–z, A–Z, 0–9, и знак подчеркивания</p></li>
<li><p>Имена переменных не должны включать в себя пробелы</p></li>
</ul>
<p>С помощью операции присвоения (<code>=</code>) переменной присваивается определенное значение:</p>
<pre class="brush:php;">
$num = 10;
</pre>
<p>Здесь определена переменная <code>$num</code>, которая хранит число 10.</p>
<p>После определения переменной и присвоения ей значения мы можем использовать ее в выражениях PHP. Например, вывести ее значение на  веб-страницу:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>С помощью функции <code>echo</code> значение переменной <code>$num</code> выводится на веб-страницу. И при обращении к скрипту мы увидим 
значение переменной <code>$num</code>:</p>
<img src="./pics/2.5.png" alt="Переменные в PHP" />
<p>Отличительной особенностью переменных является то, что мы можем изменять их значение:</p>
<pre class="brush:php;">
&lt;?php
// определение переменной $num
$num = 10;
...
<p>Также можно присваивать значение другой переменной:</p>
<pre class="brush:php;">
$a = 15;
$b = $a;
echo $b;
</pre>
<p>Если переменная объявлена, но ей изначально не присвоено никакого значения (иначе говоря она не инициализирована), то нам будет 
проблематично ее использовать. Например:</p>
<pre class="brush:php;">
&lt;?php
$num;
echo $num;
...
<p>Поэтому перед использованием переменной ей следует присвоить начальное значение.</p>
<h3>Вывод значения переменной</h3>
<p>В предыдущих примерах для вывода значения переменной применялась команда <span class="b">echo</span>, после которой указывалось выводимое значение. 
Однако есть и другой способ вывести значение переменной. Например, мы хотим одновременно вывести значения двух переменных:</p>
<pre class="brush:php;">
&lt;?php
$num_1 = 11;
$num_2 = 35;
...
<p>Здесь функции <code>echo</code> передается строка. Чтобы встроить в строку значение переменной, в этой сстроке указываем имя переменной вместе со знаком $. 
И кода в строке PHP встретит выражение <code>$num_1</code>, он заменит это выражение значением переменной $num_1. То же самое касается и 
переменной <code>$num_2</code>. В итоге при выполнении этого скрипта браузер отобразит значения обеих переменных:</p>
<pre class="browser">
num_1 = 11 num_2=35
</pre>
