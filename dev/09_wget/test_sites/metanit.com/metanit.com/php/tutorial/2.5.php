...
<html >
<head>
<title>PHP | Получение и установка типа переменной. Преобразование типов</title>
<meta charset="utf-8" />
<meta name="description" content="Получение и установка типа переменной в PHP, функции is_integer, is_string, is_double, is_null, is_numeric, settype, gettype, преобразование типов">
...
<h2>Получение и установка типа переменной. Преобразование типов</h2><div class="date">Последнее обновление: 12.03.2021</div>
...
<h3>Получение типа переменной</h3>
<p>Для получения типа переменной применяется функция <span class="b">gettype()</span>, которая возвращает название типа переменной, например, <code>integer</code> (целое число), 
<code>double</code> (число с плавающей точкой), <code>string</code> (строка), <code>boolean</code> (логическое значение), 
<code>null</code>, <code>array</code> (массив), <code>object</code> (объект) или <code>unknown type</code>. Например:</p>
<pre class="brush:php;">
&lt;?php
$a = 10;
$b = "10";
...
<p>Также есть ряд специальных функций, которые возвращают <code>true</code> или <code>false</code> в зависимости от того, представляет ли переменная 
определенный тип:</p>
<ul>
<li><p><span class="b">is_integer($a)</span>: возвращает значение <code>true</code>, если переменная $a хранит целое число</p></li>
<li><p><span class="b">is_string($a)</span>: возвращает значение <code>true</code>, если переменная $a хранит строку</p></li>
<li><p><span class="b">is_double($a)</span>: возвращает значение <code>true</code>, если переменная $a хранит действительное число</p></li>
<li><p><span class="b">is_numeric($a)</span>: возвращает значение <code>true</code>, если переменная $a представляет целое или действительное число или 
является строковым представлением числа. Например:</p>
<pre class="brush:php;">
$a = 10;
$b = "10";
echo is_numeric($a);
...
<p>Оба выражения <code>is_numeric()</code> возвратят true, так как переменная $a представляет число, а переменная $b является строковым представлением числа</p>
</li>
<li><p><span class="b">is_bool($a)</span>: возвращает значение <code>true</code>, если переменная $a хранит значение <code>true</code> или <code>FALSE</code></p></li>
<li><p><span class="b">is_scalar($a)</span>: возвращает значение <code>true</code>, если переменная $a представляет один из простых типов: строку, целое число, 
действительное число, логическое значение.</p></li>
<li><p><span class="b">is_null($a)</span>: возвращает значение <code>true</code>, если переменная $a хранит значение <code>null</code></p></li>
<li><p><span class="b">is_array($a)</span>: возвращает значение <code>true</code>, если переменная $a является массивом</p></li>
<li><p><span class="b">is_object($a)</span>: возвращает значение <code>true</code>, если переменная $a содержит ссылку на объект</p></li>
</ul>
<h3>Установка типа. Функция settype()</h3>
<p>С помощью функции <span class="b">settype()</span> можно установить для переменной определенный тип. Она принимает два параметра: 
<code>settype("Переменная", "Тип")</code>. В качестве первого параметра используется переменная, тип которой надо установить, а в качестве второго - 
строковое описание типа, которое возвращается функцией <code>gettype()</code>.</p>
<p>Если удалось установить тип, то функция возвращает <code>true</code>, если нет - то значение <code>false</code>.</p>
<p>Например, установим для переменной целочисленный тип:</p>
<pre class="brush:php;">
&lt;?php
$a = 10.7;
settype($a, "integer");
...
<p>Поскольку переменная $a представляет действительное число 10.7, то его вполне можно преобразовать в целое число через отсечение дробной части. 
Поэтому в данном случае функция <code>settype()</code> возвратит <code>true</code>.</p>
<h3>Преобразование типов</h3>
<p>По умолчанию PHP при необходимости автоматически преобразует значение переменной из одного типа в другой. По этой причине явные преобразования 
в PHP не так часто требуются. Тем не менее мы можем их применять.</p>
<p>Для явного преобразования перед переменной в скобках указыется тип, в который надо выполнить преобразование:</p>
<pre class="brush:php;">
$boolVar = false;
$intVar = (int)$boolVar; // 0
echo "boolVar = $boolVar&lt;br&gt;intVar = $intVar";
</pre>
<p>В данном случае значение "false" пробразуется в значение типа <code>int</code>, которое будет храниться в переменной <code>$intVar</code>. 
А именно значение <code>false</code> преобразуется в число 0. После этого мы сможем использовать данное значение как число.</p>
<p>При использовании выражения <code>echo</code> для вывода на страницу передаваемые значения автоматически преобразуются в строку. И поскольку 
переменная <code>boolVar</code> равна <code>false</code>, ее значение будет преобазовано в пустую строку. Тогда как значение 0 преобразуется в  
строку "0".</p>
<p>В PHP могут применяться следующие преобразования:</p>
<ul>
<li><p><span class="b">(int), (integer)</span>: преобразование в <code>int</code> (в целое число)</p></li>
<li><p><span class="b">(bool), (boolean)</span>: преобразование в <code>bool</code></p></li>
<li><p><span class="b">(float), (double), (real)</span>: преобразование в <code>float</code></p></li>
<li><p><span class="b">(string)</span>: преобразование в строку</p></li>
<li><p><span class="b">(array)</span>: преобразование в массив</p></li>
<li><p><span class="b">(object)</span>: преобразование в <code>object</code></p></li>
</ul>
...
<div class="nav"><p><a href="./2.23.php">Назад</a><a href="./">Содержание</a><a href="2.13.php">Вперед</a></p></div></div>
<div class="socBlock">
