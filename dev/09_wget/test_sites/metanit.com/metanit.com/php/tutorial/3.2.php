...
<html >
<head>
<title>PHP | Получение данных из строки запроса</title>
<meta charset="utf-8" />
<meta name="description" content="Получение данных из строки запроса в PHP, обработка запросов типа GET, глобальный массив $_GET">
...
<h1>Отправка данных на сервер</h1><h2>Получение данных из строки запроса</h2><div class="date">Последнее обновление: 15.03.2021</div>
...
<p>Самым простым способом передачи данных на сервер приложению PHP извне представляет передача данных через строку запроса.</p>
<p><span class="b">Строка запроса</span> представляет набор параметров, которые помещаются в адресе после вопросительного знака. 
При этом каждый параметр определяет название и значение. Например, в адресе:</p>
<pre class="brush:php;">http://localhost/user.php?name=Tom&age=36</pre>
<p>Часть <code>?name=Tom&age=36</code> представляет строку запроса, в которой есть два параметра <code>name</code> и <code>age</code>. 
Для каждого параметра определено имя и значение, которые отделяются знаком равно. Параметр <code>name</code> имеет значение "Tom", а параметр <code>age</code> - 
значение 36. Друг от друга параметры отделяются знаком амперсанда.</p>
<p>Например, определим следующий скрипт <span class="b">user.php</span> со следующим содержимым:</p>
<pre class="brush:php;">
&lt;?php
$name = "не определено";
$age = "не определен";
...
<p>Когда мы вводим в адресную строку браузера некий адрес и нажимаем на оправку, то серверу отправляется запрос типа <span class="b">GET</span>. 
В PHP по умолчанию определен глобальный ассоциативный массив <span class="b">$_GET</span>, который хранит все значения, 
передаваемые в запроса GET. Используя ключи передаваемых данных, мы можем из массива <span class="b">$_GET</span> получить передаваемые значения.</p>
<p>При отправки строки запроса ключами в этом массиве будут названия параметров, а значениями - значения параметров.</p>
<p>Например, в строке запроса передается параметр <code>name=Tom</code>. Соответственно, чтобы получить значение параметра name из запроса, обращаемся 
по соответствующему ключу:</p>
<pre class="brush:php;">$name = $_GET["name"];	// Tom</pre>
<p>Однако стоит учитывать, что в адресной строке необязательно будет использоваться строка запроса или конкретно данный параметр. Поэтому перед получением 
значения параметра сначала смотрим, а передан ли вообще такой параметр:</p>
<pre class="brush:php;">if(isset($_GET["name"])){</pre>
<p>Теперь обратимся к этому скрипту, например, так <span class="b">http://localhost/user.php?name=Tom&age=36</span>:</p>
<img src="./pics/3.1.png" alt="Массив $_GET в PHP и строка запроса" />
<p>Если мы не передадим значения какого-либо параметра, то соответствующая переменная будет использовать значение по умолчанию:</p>
<img src="./pics/3.2.png" alt="Данные строки запроса GET в PHP" />
...
<div class="nav"><p><a href="./2.13.php">Назад</a><a href="./">Содержание</a><a href="3.1.php">Вперед</a></p></div></div>
<div class="socBlock">
