...
<html >
<head>
<title>PHP | Работа с полями ввода форм</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с полями ввода форм checkbox, radiobutton, select в языке программирования PHP, отправка списков в запросе POST">
...
<h2>Работа с полями ввода форм</h2><div class="date">Последнее обновление: 17.03.2021</div>
...
<p>Формы могут содержать различные элементы - текстовые поля, флажки, переключатели и т.д., обработка которых имеет свои особенности.</p>
<h3>Обработка флажков</h3>
<p>Флажки или чекбоксы (html-элемент <code>&lt;input type="checkbox"/&gt;</code>) могут находиться в двух состояниях: отмеченном (checked) и 
неотмеченном. Например:</p>
<pre class="brush:xml;">Запомнить: &lt;input type="checkbox" name="remember" checked="checked" /&gt;</pre>
<img src="./pics/3.9.png" alt="Checkbox в PHP" />
<p>Если флажок находится в неотмеченном состоянии, например:</p>
<pre class="brush:xml;">Запомнить: &lt;input type="checkbox" name="remember" /&gt;</pre>
<p>то при отправке формы значение данного флажка не передается на сервер.</p>
<p>Если флажок отмечен, то при отправке на сервер для поля <code>remember</code> будет передано значение <code>on</code>:</p>
<pre class="brush:php;">$remember = $_POST["remember"];</pre>
<p>Если нас не устраивает значение <code>on</code>, то с помощью атрибута <code>value</code> мы можем установить нужное нам значение:</p>
<pre class="brush:xml;">Запомнить: &lt;input type="checkbox" name="remember" value="1" /&gt;</pre>
<p>Иногда необходимо создать набор чекбоксов, где можно выбрать несколько значений. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В этом случае значение атрибута name должно иметь квадратные скобки. И тогда после отправки сервер будет получать массив отмеченных значений:</p>
<pre class="brush:php;">
$technologies = $_POST["technologies"];
foreach($technologies as $item) echo "$item&lt;br /&gt;";
</pre>
<p>В данном случае переменная <code>$technologies</code> будет представлять массив, который можно перебрать и выполнять все другие операции с массивами.</p>
<img src="./pics/3.12.png" alt="передача массива Checkbox input на сервер в PHP" />
<h3>Переключатели</h3>
<p>Переключатели или радиокнопки позволяют сделать выбор между несколькими взаимоисключающими вариантами:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/3.13.png" alt="radiobutton in PHP" />
<p>На сервер передается значение атрибута <code>value</code> у выбранного переключателя. Получение переданного значения:</p>
<pre class="brush:php;">
if(isset($_POST["course"]))
{
    $course = $_POST["course"];
...
<h3>Список</h3>
<p>Список представляет элемент <code>select</code>, который предоставляет выбор одного или нескольких элементов:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Элемент <code>&lt;select&gt;</code> содержит ряд вариантов выбора в виде элементов <code>&lt;option&gt;</code>:</p>
<img src="./pics/3.14.png" alt="список select list в PHP" />
<p>Получить выбранный элемент в коде PHP как и обычное одиночное значение:</p>
<pre class="brush:php;">
if(isset($_POST["course"]))
{
    $course = $_POST["course"];
...
<p>Но элемент <code>&lt;select&gt;</code> также позволяет сделать множественный выбор. И в этом случае обработка выбранных 
значений изменяется, так как сервер получает массив значений:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Такие списки имеют атрибут <code>multiple="multiple"</code>. Для передачи массива также указываются в атрибуте <code>name</code> квадратные скобки: 
<code>name="courses[]"</code></p>
<img src="./pics/3.15.png" alt="Отправка списка с мнножественным выбором multiple select в PHP" />
...
<div class="nav"><p><a href="./3.6.php">Назад</a><a href="./">Содержание</a><a href="./3.4.php">Вперед</a></p></div></div>
<div class="socBlock">
