...
<html >
<head>
<title>PHP | Замыкания / Closure</title>
<meta charset="utf-8" />
<meta name="description" content="Замыкания / Closure в языке программирования PHP, оператор use, использование внешних переменных">
...
<h2>Замыкания / Closure</h2><div class="date">Последнее обновление: 12.03.2021</div>
...
<p>Замыкания в PHP представляют анонимную функцию, которая может использовать переменные из своего локального окружения. В отличие от обычных анонимных функций замыкания 
в PHP применяют выражение <span class="b">use</span>.</p>
<p>Вначале рассмотрим анонимную функцию, которая пытается использовать некоторую внешнюю переменную:</p>
<pre class="brush:php;">
&lt;?php
$number = 89;
...
<p>Анонимная функиция, которая представлена переменной <code>$showNumber</code>, пытается обратиться к внешней переменной $number. 
Однако при выполнении скрипта мы увидим в браузере сообщение об ошибке:</p>
<pre class="browser">
Warning: Undefined variable $number in C:\localhost\hello.php on line 13
</pre>
<p>То есть по умолчанию переменной $number для анонимной функции не существует. Из данной сиутации мы можем выйти, 
использовав оператор <span class="b">global</span> или массив <span class="b">$GLOBALS</span>, которые рассматриваются 
в одной из последующих тем. Тем не менее замыкания также позволяют решить эту проблему. Так, трансформируем функцию в замыкание:</p>
<pre class="brush:php;">
&lt;?php
$number = 89;
...
<p>Выражение <span class="b">use()</span> получает внешние переменные, которые анонимная функция собирается использовать. И теперь при ее выполении 
браузер выведет значение переменной <code>$number</code>.</p>
<p>Подобным образом функция-замыкание может захватывать и большее количество внешних переменных, а также как и другие функции применять параметры:</p>
<pre class="brush:php;">
$a = 8; 
$b = 10;
...
<div class="nav"><p><a href="./2.19.php">Назад</a><a href="./">Содержание</a><a href="./2.21.php">Вперед</a></p></div></div>
<div class="socBlock">
