...
<html >
<head>
<title>PHP | Ссылки</title>
<meta charset="utf-8" />
<meta name="description" content="Ссылки в языке программирования PHP, оператор & амперсанд, передача параметров по значению и по ссылке, получение ссылки на переменную, возвращение ссылки из функции">
...
<h2>Ссылки</h2><div class="date">Последнее обновление: 02.05.2021</div>
...
<p>Ссылки в PHP позволяют ссылаться на область памяти, где расположено значение переменной или параметра. Для создания ссылки перед переменной указывается 
символ амперсанда - <span class="b">&amp;</span>. Но перед рассмотрением ссылок сначала рассмотрим простой пример копирования переменных:</p>
<pre class="brush:php;">
&lt;?php
$tom = "Tom";
$sam = $tom;
...
<p>Здесь переменной <code>$sam</code> присваивается значение переменной <code>$tom</code> ("Tom"). Затем той же переменной <code>$sam</code> присваивается 
новое значение - "Sam". Результат программы ожидаем - обе переменные имеют различные значения:</p>
<pre class="browser">
tom = Tom
sam = Sam
</pre>
<p>Теперь при копировании значения переменных передадим ссылку:</p>
<pre class="brush:php;">
&lt;?php
$tom = "Tom";
...
<p>Здесь нас интересует выражение</p>
<pre class="brush:php;">$sam = &$tom;</pre>
<p>В данном случае переменной <code>$sam</code> передается не копия значения переменной <code>$tom</code>, как в предыдущем случае, а ссылка на область в памяти, 
которую занимает переменная <code>$tom</code>. То есть после выполнения инструкции</p>
<pre class="brush:php;">$sam = &$tom;</pre>
<p>Обе переменных будут указывать на один и тот же адрес в памяти. Это приведет к тому, что изменение значения одной из этих переменных приведет к изменению 
значения другой переменной. Потому что они ссылаются на один и тот же участок в памяти и соответственно имеют одно общее значение:</p>
<pre class="browser">
tom = Sam
sam = Sam
</pre>
<p>Присвоить ссылку на переменную можно двумя способами:</p>
<pre class="brush:php;">
$sam = &$tom;
// или так
$sam = & $tom;
...
<p>В примере в прошлых темах мы передавали параметры <span class="b">по значению</span>. Но в PHP есть и другая форма передачи параметров - <span class="b">по ссылке</span>. 
Рассмотрим два этих способа передачи параметров и сравним. Стандартная передача параметра по значению:</p>
<pre class="brush:php;">
&lt;?php
function square($a)
{
...
<p>В данном случае внутри функции <code>square()</code> значение параметра <code>$a</code> возводится в квадрат. То есть значение параметра внутри функции изменяется. 
Однако после вызова функции <code>square()</code> значение переменной <code>$number</code>, которое передается параметру $a, не изменится. 
Это и есть передача по значению, она никак не затрагивает переменную <code>$number</code>.</p>
<p>Теперь рассмотрим передачу параметра по ссылке:</p>
<pre class="brush:php;">
&lt;?php
function square(&$a)
{
...
<p>При передаче по ссылке перед параметром ставится знак амперсанда: <code>function square(&$a)</code>. Теперь интерпретатор будет передавать не 
...
В итоге, значение переменной <code>$number</code> после передачи параметру <code>&$a</code> также будет изменено.</p>
<h3>Возвращение ссылки из функции</h3>
<p>Функция также может возвращать ссылку. В этом случае при определении и вызове функции перед ее именем ставится знак амперсанда:</p>
<pre class="brush:php;">
&lt;?php
...
<p>В данном случае функция <code>checkName()</code> получает параметр по ссылке и возвращает ссылку - фактически ссылку, которая передается в функции. Для этого перед определением функции 
указан символ амперсанда:</p>
<p><code>function <span class="b">&</span>checkName(&$name)</code></p>
<p>Для имитации работы функция проверяет имя пользователя и изменяет его на некоторое стандартное, если оно равно "admin".</p>
<p>При вызове функции перед ее именем указывается символ амерсанда:</p>
<p><code>$checkedName = <span class="b">&</span>checkName($userName)</code></p>
<p>После выполнения функции переменная <code>$checkedName</code> фактически будет содержать ссылку на переменную <code>$userName</code>.</p>
...
<div class="nav"><p><a href="./2.22.php">Назад</a><a href="./">Содержание</a><a href="./2.10.php">Вперед</a></p></div></div>
<div class="socBlock">
