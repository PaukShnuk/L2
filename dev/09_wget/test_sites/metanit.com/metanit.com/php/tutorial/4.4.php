...
<html >
<head>
<title>PHP | Пространства имен</title>
<meta charset="utf-8" />
<meta name="description" content="Пространства имен в языке программирования PHP, оператор namespace, импорт пространств имен, классов, интерфейсов, констант, функций, оператор use и as, псевдонимы классов">
...
<h2>Пространства имен</h2><div class="date">Последнее обновление: 30.03.2021</div>
...
<p>Пространства имен позволяют избежать конфликта имен и сгруппировать функционал. Внутри пространства имен могут быть размещены классы, интерфейсы, функции и 
константы.</p>
...
<h3>Определение пространства имен</h3>
<p>Для создания пространства имен применяется директива <span class="b">namespace</span>, после которой идет название пространства имен:</p>
<pre class="brush:php;">
namespace base;
class Person
{
...
<p>В данном случае определено пространство имен "base". Обычно названия пространств указываются в нижнем регистре. Все, что расположено ниже этой 
директивы, относится к пространству имен "base". То есть класс Person принадлежит пространству имен base.</p>
<p>Стоит учитывать, что определение пространства имен должно быть расположено выше любого другого кода или разметки html. Например:</p>
<pre class="brush:xml;">
&lt;?php
namespace base;
class Person
...
<h3>Обращение к пространству имен</h3>
<p>Для обращения к конструкциям из пространства имен перед названием конструкции через слеш указывается ее пространство имен. Например, у нас есть файл <span class="b">Person.php</span>:</p>
<pre class="brush:xml;">
&lt;?php
namespace base;
class Person
...
<p>Теперь в другом файле подключим этот файл и обратимся к классу Person:</p>
<pre class="brush:php;">
&lt;?php
namespace work;
include "Person.php";
...
<p>Здесь определено другое пространство имен - "work". Сначала в нем подключается файл "Person.php". Затем создаем объект класса Person.</p>
<p>Если конструкции одного пространства имен используются в другом пространстве имен (в том числе в глобальном пространстве имен), то перед названием 
конструкции указывается название ее пространства имен. Так, поскольку класс Person расположен в другом пространстве имен - "base", то перед названием класса указываем его полное имя с учетом его пространства имен:</p>
<pre class="brush:php;">$tom = new \base\Person("Tom");</pre>
<p>Обратите внимание, что сначала идет слеш, потом название пространства имен и потом через слеш название класса (<code>\base\Person</code>).</p>
<p>Без указания пространства имен мы можем использовать конструкцию только в том же пространстве имен, в котором она определена.</p>
...
<p>Здесь класс Person определен в пространстве имен <code>base\classes\</code>. То есть теперь для обращения к классу Person в 
другом пространстве имен надо указывать <code>\base\classes\Person</code>:</p>
<pre class="brush:php;">
&lt;?php
namespace work;
include "Person.php";
...
<h3>Псевдонимы</h3>
<p>Если в одном пространстве имен используется какой-то класс из другого пространства имен, то писать каждый раз полное имя класса с учетом его пространства имен 
может быть утомительно, кроме того, перегружает код. Например:</p>
<pre class="brush:php;">
&lt;?php
namespace work;
include "Person.php";
...
<p>В этом случае мы можем использовать псевдонимы, которые задаются в виде</p>
<pre class="brush:php;">use полное_имя_класса as псевдоним;</pre>
<p>Конструкция <span class="b">use</span> импортирует класс из другого пространства имен, а оператор <span class="b">as</span> устанавливает для него псевдоним.</p>
<p>Так, сократим предыдущий код с помощью псевдонимов:</p>
<pre class="brush:php;">
&lt;?php
namespace work;
include "Person.php";
...
<p>В данном случае для класса <code>\base\classes\Person</code> установлен псевдоним <code>User</code>, соответственно теперь для обращения к классу 
Person мы можем использовать его псевдоним User. Результат тот же, но кода меньше.</p>
<p>Можно использовать только конструкцию <code>use</code> без указания псевдонима. Тогда класс можно будет исользовать по его непосредственному имени:</p>
<pre class="brush:php;">
&lt;?php
namespace work;
include "Person.php";
...
<h4>Подключение множества классов</h4>
<p>Подобным образом можно подключать сразу несколько классов. Например, пусть в файле <span class="b">Person.php</span> имеются следующие классы:</p>
<pre class="brush:php;">
&lt;?php
namespace base\classes;
class Person
...
<p>Подключение классов Person и Employee:</p>
<pre class="brush:php;">
&lt;?php
namespace work;
include "Person.php";
...
<p>Определение псевдонимов можно сократить:</p>
<pre class="brush:php;">use \base\classes\{Person as User, Employee as Employee};</pre>
<h3>Подключение констант и функций</h3>
<p>Для подключения констант применяется инструкция <span class="b">use const</span>, а для подключения функций - <span class="b">use function</span>.</p>
<p>Например, определение файла <span class="b">Person.php</span>:</p>
<pre class="brush:php;">
&lt;?php
namespace base\classes;
...
<p>Подключим класс Person вместе с константой adminName и функцией printPerson:</p>
<pre class="brush:php;">
&lt;?php
namespace work;
include "Person.php";
...
<div class="nav"><p><a href="./2.11.php">Назад</a><a href="./">Содержание</a><a href="./4.5.php">Вперед</a></p></div></div>
<div class="socBlock">
