...
<html >
<head>
<title>PHP | Типы данных</title>
<meta charset="utf-8" />
<meta name="description" content="Типы данных в PHP, bool, int, mixed, callable, object, array, динамическая типизация">
...
<h2>Типы данных</h2><div class="date">Последнее обновление: 28.02.2021</div>
...
<p>PHP является языком с динамической типизацией. Это значит, что тип данных переменной выводится во время выполнения, 
и в отличии от ряда других языков программирования в PHP не надо указывать перед переменной тип данных.</p>
<p>В PHP есть десять базовых типов данных:</p>
<ul>
<li><p>bool (логический тип)</p></li>
<li><p>int (целые числа)</p></li>
<li><p>float (дробные числа)</p></li>
<li><p>string (строки)</p></li>
<li><p>array (массивы)</p></li>
<li><p>object (объекты)</p></li>
<li><p>callable (функции)</p></li>
<li><p>mixed (любой тип)</p></li>
<li><p>resource (ресурсы)</p></li>
<li><p>null (отсутствие значения)</p></li>
</ul>
<p>Из этих типов данных первые четыре являются скалярными: <code>bool, int, float, string</code>. Поэтому вначале именно их и рассмотрим.</p>

<h3>int (целочисленный тип)</h3>
<p>Представляет целое число со знаком.</p>
<pre class="brush:php;">
$num = -100;
echo $num;
</pre>
<p>Здесь переменная <code>$num</code> представляет целочисленный тип, так как ей присваивается целочисленное значение.</p>
<p>Кроме десятичных целых чисел PHP обладает возможностью использовать также двоичные, восьмеричные и шестнадцатеричные числа. Шаблоны чисел для других систем:</p>
<ul>
<li><p>шестнадцатеричные : 0[xX][0-9a-fA-F]</p></li>
<li><p>восьмеричные : 0[0-7]</p></li>
<li><p>двоичные : 0b[01]</p></li>
</ul>
<p>Например:</p>
<pre class="brush:php;">
&lt;?php
...
<p>Переменная типа <code>int</code> занимает в памяти 32 бита, то есть может принимаь значения от -2 147 483 648 до 2 147 483 647. 
Если переменная получает числовое значение вне этого диапазона, то она трактуется как переменная типа <code>float</code></p>
<h3>Тип float (числа с плавающей точкой)</h3>
<p>Размер числа с плавающей точкой зависит от платформы. Максимально возможное значение, как правило, составляет 1.8*10<sup>308</sup> 
с точностью около 14 десятичных цифр. Например:</p>
<pre class="brush:php;">
&lt;?php
$a1 = 1.5; 
$a2 = 1.3e4; // 1.3 * 10^4 
...
<h3>Тип bool (логический тип)</h3>
<p>Переменные логического типа могут принимать два значения: <code>true</code> и <code>false</code> или иначе говоря <code>истина</code> и <code>ложь</code>. 
Чаще всего логические значения используются в условных конструкциях:</p>
<pre class="brush:php;">
&lt;?php
$foo = true;
$boo = false;
...
<h3>Тип string (строки)</h3>
<p>Для работы с текстом можно применять строки. Строки бывают двух типов: в двойных кавычках и одинарных. От типа кавычек 
зависит обработка строк интерпретатором. Так, переменные в двойных кавычках заменяются значениями, а переменные в одинарных кавычках остаются неизменными.</p>
<pre class="brush:php;">
&lt;?php
$a=10;
$b=5;
...
<p>В этом случае мы получим следующий вывод:</p>
<pre>
10+5 
$a+$b
</pre>
<p>Кроме обычных символов, строка может содержать специальные символы, которые могут быть неправильно интерпретированы. Например, нам надо добавить в строку кавычку:</p>
<pre class="brush:php;">
$text = "Модель "Apple II"";
</pre>
<p>Данная запись будет ошибочна. Чтобы исправить ошибку, мы можем сочетать различные типы кавычек ('Модель "Apple II"' или "Модель 'Apple III'") или использовать слеш, чтобы ввести кавычку в строку:</p>
<pre class="brush:php;">
$text = "Модель \"Apple II\"";
</pre>
<h3>Специальное значение null</h3>
<p>Значение <code>null</code> указывает, переменная не имеет значения. Использование данного значения полезно в тех случаях, когда мы 
...
то нам интерпретатор выдаст сообщение, что переменная не установлена:</p>
<pre class="brush:php;">
&lt;?php
$a;
echo $a;
...
<p>Использование значения null поможет избежать данной ситуации. Кроме того, мы сможем проверять наличие значения и в зависимости от результатов проверки 
производить те или иные действия:</p>
<pre class="brush:php;">
&lt;?php
$a = null;
echo "a = $a";
...
<p>Константа null не чувствительна к регистру, поэтому мы можем написать и так:</p>
<pre class="brush:php;">
$a = NULL;
</pre>
<h3>Динамическая типизация</h3>
<p>Поскольку PHP - язык с динамической типизацией, то мы можем присваивать одной и той же переменной значения разных типов:</p>
<pre class="brush:php;">
&lt;?php
$id = 123;
echo "&lt;p&gt;id = $id&lt;/p&gt;";
