...
<html >
<head>
<title>PHP | Генераторы</title>
<meta charset="utf-8" />
<meta name="description" content="Генераторы в языке программирования PHP, оператор yield и from, перебор в цикле foreach, отличие от массивов и увеличение производительности">
...
<h2>Генераторы</h2><div class="date">Последнее обновление: 11.03.2021</div>
...
<p><span class="b">Генератор</span> предоставляет функцию, которая генерирует набор значений.</p>
<p>Для возвращения значения из функции применяется оператор <span class="b">yield</span>. Но в отличие от <span class="b">return</span>  оператор 
<span class="b">yield</span> сохраняет состояние функции, позволяя ей продолжать работу с того места, когда остановилось ее выполнение.</p>
<p>Например, определим простейшую функицию генератора:</p>
<pre class="brush:php;">
function generator(){
...
<p>Здесь функция генератора фактически возвращает только одно число 21. Тем не менее мы можем перебирать результат функции генератора в цикле как 
стандартный массив:</p>
<pre class="brush:php;">
...
<p>Подобным образом генератор может возвращать и большее количество значений:</p>
<pre class="brush:php;">
...
<p>В данном случае функиция генератора <code>generateNumbers()</code> с помощью цикла генерирует значения от 0 до 5 включительно. Это все равно, если бы написали:</p>
<pre class="brush:php;">
function generateNumbers()
{
	yield 0;
...
<p>При переборе в цикле мы фактически перебираем результат функции как обычный массив, каждый элемент которого имеет числовой индекс, начиная с нуля:</p>
<pre class="brush:php;">
&lt;?php
function generateNumbers()
{
...
<p>С помощью оператора <span class="b">from</span> можно определять массив - источник данных для генератора:</p>
<pre class="brush:php;">
function generateNumbers()
{
	yield 1;
...
<p>В данном случае функция <code>generateNumbers()</code> для генерации часть данных берет из массива <code>[2, 3, 4]</code> с помощью выражения 
<code>yield from [2, 3, 4]</code>.</p>
<p>Функция генератора, как и любая функция может принимать параметры, что позволяет настраивать поведение генератора:</p>
<pre class="brush:php;">
...
<p>Но естественно может возникнуть вопрос: а зачем нужны генераторы? Разве мы не можем с тем же успехом перебират обычный массив? Например:</p>
<pre class="brush:php;">
$numbers = [1, 2, 3, 4, 5];
...
<p>Дело в том, что при работе с массивом весь массив загружается в память. При небольших объемах проблема может быть игнорироваться. 
Но чем больше размер массива, соответственно тем больше издержки и потери в производительности. Именно эту проблему и призваны решить генераторы, 
которые извлекают только одно значение одномоментно при обращении к функции, экономя тем самым вычислительные ресурсы.</p>
...
<div class="nav"><p><a href="./2.21.php">Назад</a><a href="./">Содержание</a><a href="./2.24.php">Вперед</a></p></div></div>
<div class="socBlock">
