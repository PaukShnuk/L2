...
<html >
<head>
<title>PHP | Абстрактные классы и методы</title>
<meta charset="utf-8" />
<meta name="description" content="Абстрактные классы и методы в языке программирования PHP, модификатор abstract, наследование в классах">
...
<h2>Абстрактные классы и методы</h2><div class="date">Последнее обновление: 23.03.2021</div>
...
<p>Абстрактный класс представляет частичную реализацию для классов-наследников.</p> 
<p>Абстрактный класс определяется с помощью модификатора <span class="b">abstract</span>, который ставится перед именем класса:</p>
<pre class="brush:php;">
abstract class Messenger { }
</pre>
<p>Одной из ключевых особенностей абстрактных классов является то, что мы не можем напрямую создать объекты абстрактного класса с помощью вызова его конструктора:</p>
<pre class="brush:php;">
abstract class Messenger { }
$telegram = new Messenger();	// эта строка не будет работать
</pre>
<p>Абстрактные классы, как и обычные классы, могут определять переменные и константы, методы и конструкторы.</p>
<p>Но другой особенностью абстрактных классов является то, что они могут содержать абстрактные методы. Это методы, которые не имеют реализации. Реализацию для них предоставляют 
классы-наследники. Например:</p>
<pre class="brush:php;">
abstract class Messenger
{
...
<p>При определении абстрактного метода перед словом <code>function</code> ставится модификатор <span class="b">abstract</span>. А после списка параметров метода - точка с запятой.</p>
<p>Абстрактные методы могут размещаться только в абстрактных классах. Обычный неабстрактный класс не может иметь абстрактных методов.</p>
<p>Если неабстрактный класс наследуется от абстрактного класса, то он обязан реализовать все его абстрактные методы.</p>
<p>Для наследования классом абстрактного класса, как и в общем случае при наследовании, применяется ключевое слово <span class="b">extends</span>. 
Например:</p>
<pre class="brush:php;">
&lt;?php
abstract class Messenger
...
<p>В данном случае класс EmailMessenger наследуется от абстрактного класса Messenger.</p>
<p>Абстрактный класс определяет абстрактный метод <code>send()</code>, поэтому класс-наследник EmailMessenger обычтельно должен предоставить реализацию 
для этого метода.</p>
<p>Так, в данном случае мы получим следующий вывод:</p>
<pre class="browser">
Outlook отправляет сообщение: Hello PHP 8
Выход из мессенджера...
</pre>
<p>Можно заметить, что абстрактные классы похожи на интерфейсы - и те, и другие могут определять методы без реализации, которые реализуются в других классах. 
Однако, абстрактные классы, как и обычные классы, могут иметь переменные, неабстрактные методы, конструкторы, а интерфейсы нет. Кроме того, в PHP один класс может 
наследоваться только от одного класса, тогда как один класс может применять сразу несколько интерфейсов.</p>
