...
<html >
<head>
<title>PHP и MySQL | Удаление данных в PDO</title>
<meta charset="utf-8" />
<meta name="description" content="Удаление данных в БД MySQL с помощью языка программирования PHP и библиотеки PDO, метод exec и execute, prepared statements, bindValue, параметры запросов">
...
<h2>Удаление данных в PDO</h2><div class="date">Последнее обновление: 07.06.2021</div>
...
<p>Для удаления данных применяется sql-команда <span class="b">DELETE</span>:</p>
<pre class="brush:sql;">
DELETE FROM Таблица
WHERE столбец = значение
</pre>
<p>Для удаления данных также может применяться метод <span class="b">exec()</span> объекта PDO. 
...
<p>Удалим из этой таблицы строку, где <code>id = 5</code>:</p>
<pre class="brush:php;">
&lt;?php
try {
...
<p>Результат метода <code>$conn-&gt;exec()</code> в данном случае количество удаленых строк. Однако опять же поскольку значение столбца, на основе которого происходит удаление, нередко приходит извне, 
то в этом случае лучше использовать параметризацию.</p>
<p>Итак, определим для вывода всех объектов из БД скрипт <span class="b">index.php</span>:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В таблицы для каждой строки определена форма, которая посылает данные в POST-запросе скрипту <span class="b">delete.php</span>. 
Чтобы передать в delete.php идентификатор удаляемого объекта, на форме определено скрытое поле для хранения <code>id</code> объекта.</p>
<p>Обратите внимание, что в данном случае применяется не ссылка для удаления типа
<pre class="brush:xml;">&lt;a href="http://адрес_нашего_сайта/delete.php?id=1"&gt;Удалить&lt;a/&gt;</pre>
<p>которая оправляет данные в GET-запросе, а именно форма, которая отправляет 
данные в POST-запросе. Почему? Подобные GET-запросы потенциально небезопасны. Допустим, нам пришло электронное письмо, в которое была внедрена картинка посредством тега:</p>
<pre class="brush:xml;">&lt;img src="http://адрес_нашего_сайта/delete.php?id=1" /&gt;</pre>
<p>В итоге при открытии письма 1-я запись в таблице может быть удалена. Уязвимость касается не только писем, но может проявляться и в других местах, но смысл один - GET-запрос к скрипту, который удаляет данные, несет потенциальную уязвимость.</p>
<p>Теперь определим сам скрипт <span class="b">delete.php</span>, который будет выполнять удаление:</p>
<pre class="brush:php;">
&lt;?php
if(isset($_POST["id"]))
{
...
<p>В данном случае скрипт получает через POST-запрос значение <code>id</code> и по этому идентификатору выполняет удаление. После чего происходит переадресация 
на скрипт <span class="b">index.php</span>.</p>
<img src="./pics/1.6.png" alt="Удаление из MySQL в PHP PDO" />
