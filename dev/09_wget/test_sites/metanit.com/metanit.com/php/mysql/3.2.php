...
<html >
<head>
<title>PHP и MySQL | Выполнение запросов в MySQLi. Создание базы данных и таблиц</title>
<meta charset="utf-8" />
<meta name="description" content="Создание базы данных и таблиц MySQL в языке PHP с помощью библиотеки MySQLi, выполнение запросов, функция mysqli_query и метод query">
...
<h2>Выполнение запросов в MySQLi. Создание базы данных и таблиц</h2><div class="date">Последнее обновление: 08.06.2021</div>
...
<p>Для выполнения запросов у объекта <span class="b">mysqli</span> вызывается метод <span class="b">query()</span>, в который передается выполняемая команда SQL:</p>
<pre class="brush:php;">
$conn = new mysqli("localhost", "root", "mypassword");
$conn-&gt;query(команда_sql);
</pre>
<p>Если выполнение запроса прошло успешно, то метод <span class="b">query()</span> возвращает <code>true</code>, если неудачно, то возвращается <code>false</code>.</p>
<h4>Процедурный подход</h4>
<p>При процедурном подходе для выполнения вызывается функция <span class="b">mysqli_query()</span>, в которую передается объект подключения и 
выполняемая команда SQL:</p>
<pre class="brush:php;">
$conn = mysqli_connect("localhost", "root", "mypassword");
mysqli_query($conn, $query);
</pre>
<p>Если выполнение запроса прошло успешно, то функция <span class="b">mysqli_query()</span> возвращает <code>true</code>, если неудачно, то возвращается <code>false</code>.</p>
<h3>Создание базы данных</h3>
<p>Для создания базы данных в MySQL применяется команда <span class="b">CREATE DATABASE</span>, после которой указывается имя создаваемой базы данных. 
Создадим базу данных с помощью PHP.</p>
<h4>Объектно-ориентированный подход</h4>
<pre class="brush:php;">
&lt;?php
$conn = new mysqli("localhost", "root", "mypassword");
...
<p>Здесь создается база данных <code>testdb2</code>. Если при выполнении команды возникнет ошибка, то мы можем получить информацию о ней через свойство <code>$conn-&gt;error</code>.</p>
<h4>Процедурный подход</h4>
<pre class="brush:php;">
&lt;?php
$conn = mysqli_connect("localhost", "root", "mypassword");
...
<p>Здесь создается база данных <code>testdb3</code>. Если при выполнении команды возникнет ошибка, то мы можем получить информацию о ней через функцию 
<code>mysqli_error()</code>, в которую передается объект подключения.</p>
<h3>Создание таблицы</h3>
<p>Создав базу данных, мы можем добавить в нее таблицы для хранения данных. Для создания таблицы в MySQL применяется команда <span class="b">CREATE TABLE</span>, 
после которой указывается имя создаваемой таблицы и в скобках определения столбцов. Например, создадим в ней таблицу, которая описывается следующим кодом</p>
<pre class="brush:sql;">CREATE TABLE Users (id INTEGER AUTO_INCREMENT PRIMARY KEY, name VARCHAR(30), age INTEGER);</pre>
<p>Здесь создается таблица под названием "users". Она будет хранить условных пользователей. В ней будет три столбца: id, name и age. Столбец 
<code>id</code> представляет числовой уникальный идентификатор строки - или идентификатор пользователя. Столбец <code>name</code> представляет строку - 
имя пользователя. А столбец <code>age</code> хранит число - возраст пользователя.</p>
<h4>Объектно-ориентированный подход</h4>
<pre class="brush:php;">
&lt;?php
...
<p>Если мы обращаемся уже непосредственно к базе данных, то ее имя передается в качетстве четвертого параметра в конструктор <span class="b">mysqli</span>: 
<code>new mysqli("localhost", "root", "mypassword", "testdb2");</code>.</p>
<h4>Процедурный подход</h4>
<pre class="brush:php;">
&lt;?php
...
<p>Если мы подключаемся к конкретной базе данных, то ее имя передается в качетстве четвертого параметра в функцию <span class="b">mysqli_connect()</span>: 
<code>mysqli_connect("localhost", "root", "mypassword", "testdb3")</code>.</p>
