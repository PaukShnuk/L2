...
<html >
<head>
<title>PHP и MySQL | Обновление данных в MySQLi</title>
<meta charset="utf-8" />
<meta name="description" content="Обновление данных в БД MySQL в языке PHP с помощью библиотеки MySQLi, команда UPDATE">
...
<h2>Обновление данных в MySQLi</h2><div class="date">Последнее обновление: 10.06.2021</div>
...
<p>В MySQL для обновления применяется sql-команда <span class="b">UPDATE</span>:</p>
<pre class="brush:sql;">
UPDATE Таблица
SET столбец1 = значение1, столбец2 = значение2,...
WHERE столбец = значение 
</pre>
<p>Для примера возьмем использованную в прошлых темах таблицу Users со следующим определением:</p>
<pre class="brush:sql;">
CREATE TABLE Users (id INTEGER AUTO_INCREMENT PRIMARY KEY, name VARCHAR(30), age INTEGER)
</pre>
<h4>Объектно-ориентированный стиль</h4>
<p>Сначала определим файл <span class="b">index.php</span>, который будет выводить список пользователей с ссылками на их обновление:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь используется команда SELECT, которая получает всех пользователей из таблицы Users. В таблице третий столбец хранит ссылку на скрипт update.php, который мы далее создадим и которому передается параметр id с идентификатором пользователя, которого надо изменить.</p>
<p>Для обновления данных определим скрипт <span class="b">update.php</span>:</p>
<pre class="brush:php;">
&lt;?php 
$conn = new mysqli("localhost", "root", "mypassword", "testdb2");
if($conn-&gt;connect_error){
...
<p>Весь код обновления структурно делится на две части. В первой части мы обрабатываем запрос GET. Когда пользователь нажимает на ссылку "Обновить" на странице <span class="b">index.php</span>, то отправляется запрос GET, в котором передается id редактируемого пользователя. </p>
<pre class="brush:php;">if($_SERVER["REQUEST_METHOD"] === "GET" && isset($_GET["id"]))</pre>
<p>И если это запрос GET, то нам надо вывести данные редактируемого пользователя в поля формы. Для этого получаем из базы данных объект по переданному id</p>
<pre class="brush:php;">
$userid = $conn-&gt;real_escape_string($_GET["id"]);
$sql = "SELECT * FROM Users WHERE id = '$userid'";
if($result = $conn-&gt;query($sql)){
</pre>
<p>Далее получаем полученные данные и, если они имеются, выводим их в поля формы. Таким образом, пользователь увидит на форме данные обновляемого объекта.</p>
<p>Вторая часть скрипта представляет обработку POST-запроса - когда пользователь нажимает на кнопку на форме, то будет отправляться POST-запрос с отправленными данными. Мы получаем эти данные и отправляем базе данных команду UPDATE с этими данными, используя при этом параметризацию запроса:</p>
<pre class="brush:php;">
$userid = $conn-&gt;real_escape_string($_POST["id"]);
$username = $conn-&gt;real_escape_string($_POST["name"]);
$userage = $conn-&gt;real_escape_string($_POST["age"]);
...
<p>Скрипт <span class="b">index.php</span>:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Скрипт <span class="b">update.php</span>:</p>
<pre class="brush:php;">
&lt;?php
$conn = mysqli_connect("localhost", "root", "_mypassword", "testdb3");
if (!$conn) {
	die("Ошибка: " . mysqli_connect_error());
}
?&gt;
...
<div class="nav"><p><a href="./3.7.php">Назад</a><a href="./">Содержание</a><a href="./3.6.php">Вперед</a></p></div></div>
<div class="socBlock">
