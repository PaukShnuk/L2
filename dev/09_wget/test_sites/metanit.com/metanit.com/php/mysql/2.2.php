...
<html >
<head>
<title>PHP и MySQL | Выполнение запросов в PDO. Создание базы данных и таблиц</title>
<meta charset="utf-8" />
<meta name="description" content="Выполнение запросов и создание базы данных и таблиц MySQL с помощью языка программирования PHP и библиотеки PDO, метод exec">
...
<h2>Выполнение запросов в PDO. Создание базы данных и таблиц</h2><div class="date">Последнее обновление: 07.06.2021</div>
...
<p>Для выполнения запросов к серверу базы данных у объекта PDO вызывается метод <span class="b">exec()</span>, в который передается выполняемое выражение SQL.</p>
<pre class="brush:php;">
$conn = new PDO("mysql:host=localhost", "root", "mypassword");
$conn-&gt;exec(команда_sql);
</pre>
<h3>Создание базы данных</h3>
<p>Для создания базы данных применяется SQL-команда <code>CREATE DATABASE</code>, после которой указывается имя создаваемой базы данных.</p>
<p>Создадим базу данных через PHP:</p>
<pre class="brush:php;">
&lt;?php
 try {
...
<p>В данном случае после подключения к серверу определяется переменная <code>$sql</code>, которая хранит команду на создание бд с именем "textdb1":</p>
<pre class="brush:php;">$sql = "CREATE DATABASE testdb1";</pre>
<p>Далее для выполнения этой команды передаем ее в метод <code>exec()</code>:</p>
<pre class="brush:php;">$conn-&gt;exec($sql);</pre>
<p>В итоге при успешном создании базы данных мы увидим в браузере сообщение об создании БД:</p>
<div class="browser">Database has been created</div>
<h3>Создание таблицы</h3>
<p>Подобным образом можно выполнять запросы на создание таблиц в базе данных. Для создания таблиц применяется SQL-команда <code>CREATE TABLE</code>, после которой указывается имя создаваемой таблицы и в скобках определения столбцов.</p>
<p>Так, возьмем выше созданную базу данных "testdb1". И создадим в ней 
таблицу, которая описывается следующим кодом</p>
<pre class="brush:sql;">
CREATE TABLE Users (id INTEGER AUTO_INCREMENT PRIMARY KEY, name VARCHAR(30), age INTEGER);
</pre>
<p>Здесь создается таблица под названием "users". Она будет хранить условных пользователей. 
...
Столбец name представляет строку - имя пользователя. А столбец age представляет число - возраст пользователя.</p>
<p>Для создания таблицы определим следующий скрипт php:</p>
<pre class="brush:php;">
&lt;?php
try {
    // подключаемся к серверу
...
<p>Обратите внимание, что по сравнению с предыдущим примером здесь в строке подключения указана база данных, в которой создается таблица: <code>"mysql:host=localhost;dbname=testdb1"</code></p>
<p>И после успешного выполнения запрос мы увидим в браузере сообщение об создании таблицы:</p>
<div class="browser">Table Users has been created</div>
