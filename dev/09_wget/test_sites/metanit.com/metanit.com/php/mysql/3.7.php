...
<html >
<head>
<title>PHP и MySQL | Фильтрация данных в MySQLi</title>
<meta charset="utf-8" />
<meta name="description" content="Фильтрация данных в БД MySQL в языке PHP с помощью библиотеки MySQLi, получение параметров для фильтрации">
...
<h2>Фильтрация данных в MySQLi</h2><div class="date">Последнее обновление: 09.06.2021</div>
...
<p>Подобно тому, как мы получаем все данные из таблицы, мы можем применять фильтрацию выбирать данные по определенному критерию. 
...
Например, получение всех данные из таблицы Users, где <code>id = 1</code>:</p>
<pre class="brush:sql;">
SELECT * FROM Users WHERE id = 1
</pre>
<p>Однако если данные для фильтрации приходят извне, нам необходимо учитывать их потенциальную опасность. Рассмотрим получение данных на примере таблицы Users, созданной в прошлых, которая имеет следующее определение:</p>
...
<p>Например, мы хотим получать в GET-запросе значение для id и по нему получать из базы данных нужные данные. Определим для этого следующий скрипт <span class="b">user.php</span>:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь через запрос GET получаем параметр id - идентификатор пользователя, который надо получить из базы данных. Однако поскольку это значение приходит извне, 
к нему применяется метод <span class="b">real_escape_string()</span>, который экранирует спецсимволы:</p>
<pre class="brush:php;">$userid = $conn-&gt;real_escape_string($_GET["id"]);</pre>
<p>Для упрощения обращения к скрипту определим скрипт <span class="b">index.php</span>, который будет выводить список пользователей с ссылкой на скрипт <code>user.php</code>:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Таким образом, при обращении к скрипту <span class="b">index.php</span> браузер отобразит список пользователей, а нажав на ссылку рядом с каждым пользователем, 
мы перейдем к подробному описанию:</p>
<img src="./pics/2.3.png" alt="Фильтрация данных из MySQL в PHP с помощью MySQLi" />
<h4>Процедурный стиль</h4>
<p>Скрипт <span class="b">user.php</span>:</p>
<pre class="brush:php;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
