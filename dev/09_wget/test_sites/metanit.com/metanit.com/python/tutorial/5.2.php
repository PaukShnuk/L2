...
<html >
<head>
<title>Python | Основные методы строк</title>
<meta charset="utf-8" />
<meta name="description" content="Основные методы строк в Python, поиск в строке, замена, разделение и объединение строк">
...
<h2>Основные методы строк</h2><div class="date">Последнее обновление: 05.02.2022</div>
...
<p>Рассмотрим основные методы строк, которые мы можем применить в приложениях:</p>
<ul>
<li><p><span class="b">isalpha()</span>: возвращает True, если строка состоит только из алфавитных символов</p></li>
<li><p><span class="b">islower()</span>: возвращает True, если строка состоит только из символов в нижнем регистре</p></li>
<li><p><span class="b">isupper()</span>: возвращает True, если все символы строки в верхнем регистре</p></li>
<li><p><span class="b">isdigit()</span>: возвращает True, если все символы строки - цифры</p></li>
<li><p><span class="b">isnumeric()</span>: возвращает True, если строка представляет собой число</p></li>
<li><p><span class="b">startswith(str)</span>: возвращает True, если строка начинается с подстроки str</p></li>
<li><p><span class="b">endswith(str)</span>: возвращает True, если строка заканчивается на подстроку str</p></li>
<li><p><span class="b">lower()</span>: переводит строку в нижний регистр</p></li>
<li><p><span class="b">upper()</span>: переводит строку в вехний регистр</p></li>
<li><p><span class="b">title()</span>: начальные символы всех слов в строке переводятся в верхний регистр</p></li>
<li><p><span class="b">capitalize()</span>: переводит в верхний регистр первую букву только самого первого слова строки</p></li>
<li><p><span class="b">lstrip()</span>: удаляет начальные пробелы из строки</p></li>
<li><p><span class="b">rstrip()</span>: удаляет конечные пробелы из строки</p></li>
<li><p><span class="b">strip()</span>: удаляет начальные и конечные пробелы из строки</p></li>
<li><p><span class="b">ljust(width)</span>: если длина строки меньше параметра width, то справа от строки добавляются пробелы, чтобы дополнить значение width, 
а сама строка выравнивается по левому краю</p></li>
<li><p><span class="b">rjust(width)</span>: если длина строки меньше параметра width, то слева от строки добавляются пробелы, чтобы дополнить значение width, 
а сама строка выравнивается по правому краю</p></li>
<li><p><span class="b">center(width)</span>: если длина строки меньше параметра width, то слева и справа от строки равномерно добавляются пробелы, 
чтобы дополнить значение width, а сама строка выравнивается по центру</p></li>
<li><p><span class="b">find(str[, start [, end])</span>: возвращает индекс подстроки в строке. Если подстрока не найдена, возвращается число -1</p></li>
<li><p><span class="b">replace(old, new[, num])</span>: заменяет в строке одну подстроку на другую</p></li>
<li><p><span class="b">split([delimeter[, num]])</span>: разбивает строку на подстроки в зависимости от разделителя</p></li>
<li><p><span class="b">join(strs)</span>: объединяет строки в одну строку, вставляя между ними определенный разделитель</p></li>
</ul>
<p>Например, если мы ожидаем ввод с клавиатуры числа, то перед преобразованием введенной строки в число можно проверить, с помощью метода isnumeric() введено ли в действительности число, 
и если так, то выполнить операцию преобразования:</p>
<pre class="brush:py;">
...
<p>Проверка, начинается или оканчивается строка на определенную подстроку:</p>
<pre class="brush:py;">
file_name = "hello.py"
...
<p>Удаление пробелов в начале и в конце строки:</p>
<pre class="brush:py;">
string = "   hello  world!  "
string = string.strip()
print(string)           # hello  world!
</pre>
<p>Дополнение строки пробелами и выравнивание:</p>
<pre class="brush:py;">
...
<p>Для поиска подстроки в строке в Python применяется метод <span class="b">find()</span>, который возвращает индекс первого вхождения подстроки в строку и 
имеет три формы:</p>
<ul>
<li><p><code>find(str)</code>: поиск подстроки str ведется с начала строки до ее конца</p></li>
<li><p><code>find(str, start)</code>: параметр start задает начальный индекс, с которого будет производиться поиск</p></li>
<li><p><code>find(str, start, end)</code>: параметр end задает конечный индекс, до которого будет идти поиск</p></li>
</ul>
<p>Если подстрока не найдена, метод возвращает -1:</p>
<pre class="brush:py;">
...
<p>Для замены в строке одной подстроки на другую применяется метод <span class="b">replace()</span>:</p>
<ul>
<li><p><code>replace(old, new)</code>: заменяет подстроку old на new</p></li>
<li><p><code>replace(old, new, num)</code>: параметр num указывает, сколько вхождений подстроки old надо заменить на new</p></li>
</ul>
<pre class="brush:py;">
...
<h3>Разделение на подстроки</h3>
<p>Метод <span class="b">split()</span> разбивает строку на список подстрок в зависимости от разделителя. В качестве разделителя может выступать любой символ или последовательность символов. 
Данный метод имеет следующие формы:</p>
<ul>
<li><p><code>split()</code>: в качестве разделителя используется пробел</p></li>
<li><p><code>split(delimeter)</code>: в качестве разделителя используется delimeter</p></li>
<li><p><code>split(delimeter, num)</code>: параметр num указывает, сколько вхождений delimeter используется для разделения. Оставшаяся часть строки добавляется 
в список без разделения на подстроки</p></li>
</ul>
<pre class="brush:py;">
...
<p>При рассмотрении простейших операций со строками было показано, как объединять строки с помощью операции сложения. Другую возможность для соединения 
строк представляет метод <span class="b">join()</span>: он объединяет список строк. Причем текущая строка, у которой вызывается данный метод, используется в качестве разделителя:</p>
<pre class="brush:py;">
...
<p>Вместо списка в метод join можно передать простую строку, тогда разделитель будет вставляться между символами этой строки:</p>
<pre class="brush:py;">
word = "hello"
