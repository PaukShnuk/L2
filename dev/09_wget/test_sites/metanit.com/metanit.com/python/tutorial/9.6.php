...
<html >
<head>
<title>Python | Поле ввода Entry</title>
<meta charset="utf-8" />
<meta name="description" content="Текстовое поле ввода Entry в tkinter и Python, его методы и параметры, отслеживание ввода текста">
...
<h2>Поле ввода Entry</h2><div class="date">Последнее обновление: 13.05.2017</div>
...
<p>Элемент Entry представляет поле для ввода текста. Конструктор Entry принимает следующие параметры:</p>
<pre class="brush:py;">Entry(master, options)</pre>
<p>Где master - ссылка на родительское окно, а options - набор следующих параметров:</p>
<ul>
<li><p><span class="b">bg</span>: фоновый цвет</p></li>
<li><p><span class="b">bd</span>: толщина границы</p></li>
<li><p><span class="b">cursor</span>: курсор указателя мыши при наведении на текстовое поле</p></li>
...
<li><p><span class="b">selectbackground</span>: фоновый цвет выделенного куска текста</p></li>
<li><p><span class="b">selectforeground</span>: цвет выделенного текста</p></li>
<li><p><span class="b">show</span>: задает маску для вводимых символов</p></li>
<li><p><span class="b">state</span>: состояние элемента, может принимать значения NORMAL (по умолчанию) и DISABLED</p></li>
...
<p>Определим элемент Entry и по нажатию на кнопку выведем его текст в отдельное окно с сообщением:</p>
<pre class="brush:py;">
from tkinter import *
...
<img src="./pics/9.10.png" alt="текстовое поле Entry в tkinter в Python" />
<p>Для вывода сообщения здесь применяется дополнительный модуль <span class="b">messagebox</span>, содержащий функцию <span class="b">showinfo()</span>, которая собственно и выводит введенный в текстовое поле текст. 
Для получения введенного текста используется комп StringVar, как было описано в одной из предыдущих тем.</p>
<p>Теперь создадим более сложный пример с формой ввода:</p>
<pre class="brush:py;">
from tkinter import *
from tkinter import messagebox
...
<img src="./pics/9.11.png" alt="Форма ввода на tkinter в Python" />
<h3>Методы Entry</h3>
<p>Элемент Entry имеет ряд методов. Основные из них:</p>
<ul>
<li><p><span class="b">insert(index, str)</span>: вставляет в текстовое поле строку по определенному индексу</p></li>
<li><p><span class="b">get()</span>: возвращает введенный в текстовое поле текст</p></li>
<li><p><span class="b">delete(first, last=None)</span>: удаляет символ по индексу first. Если указан параметр last, то удаление производится до индекса last. Чтобы удалить до конца, в качестве второго параметра можно 
использовать значение END.</p></li>
</ul>
<p>Используем методы в программе:</p>
<pre class="brush:py;">
from tkinter import *
from tkinter import messagebox
...
<p>При запуске программы сразу же в оба текстовых поля добавляется текст по умолчанию:</p>
<pre class="brush:py;">
name_entry.insert(0, "Tom")
surname_entry.insert(0, "Soyer")
</pre>
<p>Кнопка Clear очищает оба поля, вызывая метод delete:</p>
<pre class="brush:py;">
def clear():
    name_entry.delete(0, END)
    surname_entry.delete(0, END)
</pre>
<p>Вторая кнопка, используя метод get, получает введенный текст:</p>
<pre class="brush:py;">
def display():
    messagebox.showinfo("GUI Python", name_entry.get() + " " + surname_entry.get())
</pre>
<p>Причем, как видно из примера, нам необязательно обращаться к тексту в Entry через переменные типа StringVar, мы можем это сделать напрямую через метод get.</p>
...
<div class="nav"><p><a href="9.5.php">Назад</a><a href="./">Содержание</a><a href="9.7.php">Вперед</a></p></div></div>
<div class="socBlock">
