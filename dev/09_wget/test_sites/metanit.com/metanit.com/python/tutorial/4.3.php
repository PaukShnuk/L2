...
<html >
<head>
<title>Python | Файлы CSV</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с файлами CSV в Python, чтение и запись, методы csv.write и cvs.read">
...
<h2>Файлы CSV</h2><div class="date">Последнее обновление: 29.04.2017</div>
...
<p>Одним из распространенных файловых форматов, которые хранят в удобном виде информацию, является формат <span class="b">csv</span>. 
...
предоставляет специальный встроенный модуль <span class="b">csv</span>.</p>
<p>Рассмотрим работу модуля на примере:</p>
<pre class="brush:py;">
import csv
...
<p>В файл записывается двухмерный список - фактически таблица, где каждая строка представляет одного пользователя. А каждый пользователь 
содержит два поля - имя и возраст. То есть фактически таблица из трех строк и двух столбцов.</p>
<p>При открытии файла на запись в качестве третьего параметра указывается значение <code>newline=""</code> - пустая строка позволяет корректно считывать 
строки из файла вне зависимости от операционной системы.</p>
<p>Для записи нам надо получить объект <span class="b">writer</span>, который возвращается функцией <code>csv.writer(file)</code>. В эту функцию передается открытый файл. 
А собственно запись производится с помощью метода <code>writer.writerows(users)</code> Этот метод принимает набор строк. В нашем случае это двухмерный список.</p>
<p>Если необходимо добавить одну запись, которая представляет собой одномерный список, например, <code>["Sam", 31]</code>, то в этом случае можно вызвать метод 
<span class="b">writer.writerow(user)</span></p>
<p>В итоге после выполнения скрипта в той же папке окажется файл users.csv, который будет иметь следующее содержимое:</p>
<pre class="brush:py;">
Tom,28
Alice,23
Bob,34
...
<p>Для чтения из файла нам наоборот нужно создать объект <span class="b">reader</span>:</p>
<pre class="brush:py;">
import csv
...
<p>При получении объекта reader мы можем в цикле перебрать все его строки:</p>
<div class="console">
...
<p>В примере выше каждая запись или строка представляла собой отдельный список, например, <code>["Sam", 31]</code>. Но кроме того, модуль csv имеет 
специальные дополнительные возможности для работы со словарями. В частности, функция <span class="b">csv.DictWriter()</span> возвращает объект writer, 
который позволяет записывать в файл. А функция <span class="b">csv.DictReader()</span> возвращает объект reader для чтения из файла. Например:</p>
<pre class="brush:py;">
import csv
...
<p>Запись строк также производится с помощью методов <code>writerow()</code> и <code>writerows()</code>. Но теперь каждая строка представляет собой отдельный словарь, 
и кроме того, производится запись и заголовков столбцов с помощью метода <span class="b">writeheader()</span>, а в метод csv.DictWriter в качестве второго параметра 
передается набор столбцов.</p>
<p>При чтении строк, используя названия столбцов, мы можем обратиться к отдельным значениям внутри строки: <code>row["name"]</code>.</p>
