...
<html >
<head>
<title>Python | Класс object. Строковое представление объекта</title>
<meta charset="utf-8" />
<meta name="description" content="Класс object и его методы. Строковое представление объекта в Python, переопределение метода __str__">
...
<h2>Класс object. Строковое представление объекта</h2><div class="date">Последнее обновление: 28.01.2022</div>
...
<p>Начиная с 3-й версии в языке программирования Python все классы неявно имеют один общий суперкласс - <span class="b">object</span> и все классы по умолчанию наследуют его методы.</p>
<p>Одним из наиболее используемых методов класса object является метод <span class="b">__str__()</span>. Когда необходимо получить строковое представление 
объекта или вывести объект в виде строки, то Python как раз вызывает этот метод. И при определении класса хорошей практикой считается переопределение этого метода.</p>
...
<p>Это не очень информативная информация об объекте. Мы, конечно, можем выйти из положения, определив в классе Person дополнительный метод, 
который выводит данные объекта - в примере выше это метод display_info.</p>
<p>Но есть и другой выход - определим в классе Person метод <span class="b">__str__()</span> (по два подчеркивания с каждой стороны):</p>
<pre class="brush:py;">
class Person:
    def __init__(self, name, age):
        self.name = name  # устанавливаем имя
...
<p>Метод <code>__str__</code> должен возвращать строку. И в данном случае мы возвращаем базовую информацию о человеке. Если нам потребуется использовать эту 
информацию в других методах класса, то мы можем использовать выражение <code>self.__str__()</code>
...
<div class="nav"><p><a href="./7.6.php">Назад</a><a href="./">Содержание</a><a href="./2.11.php">Вперед</a></p></div></div>
<div class="socBlock">
