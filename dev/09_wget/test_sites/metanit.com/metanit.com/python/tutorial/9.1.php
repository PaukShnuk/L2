...
<html >
<head>
<title>Python | Tkinter. Создание окна приложения</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в создание графических прложений на Python с помощью модуля tkinter, окно приложения и его позиционирование и размеры">
...
<h1>Создание графического интерфейса</h1><h2>Tkinter. Создание окна приложения</h2><div class="date">Последнее обновление: 13.05.2017</div>
...
<p>Многие программы на сегодняшний день используют графический интерфейс, который более интуитивен и удобен для пользователя, чем консоль. 
...
который называется <span class="b">tkinter</span>.</p>
<p>Тулкит tkinter доступен в виде отдельного встроенного модуля, который содержит все необходимые графические компоненты - кнопки, текстовые поля и т.д.</p>
<p>Базовым моментом в построении графических программ является создание окна. Затем в окно добавляются все остальные компоненты графического интерфейса. 
Поэтому создадим вначале простейшее окно. Для этого определим следующий скрипт:</p>
<pre class="brush:py;">
from tkinter import *
...
<p>Для создания графического окна применяется конструктор <span class="b">Tk()</span>, который определен в модуле tkinter. Создаваемое окно 
присваивается переменной root, и через эту переменную мы можем управлять атрибутами окна. В частности, с помощью метода <span class="b">title()</span> 
можно установить заголовок окна.</p>
<p>С помощью метода <span class="b">geometry()</span> - размер окна. Для установки размера в метод <code>geometry()</code> 
передается строка в формате "Ширина x Высота". Если при создании окна приложения метод <code>geometry()</code> не вызывается, то окно занимает то пространство, 
которое необходимо для размещения внутреннего содержимого.</p>
<p>Для отображения окна надо вызвать у него метод <span class="b">mainloop()</span>, который запускает цикл обработки событий окна для 
взаимодействия с пользователем.</p>
<p>В результате при запуске скрипта мы увидим такое пустое окошко:</p>
<img src="./pics/9.1.png" alt="Графическая программа на Python" />
<h3>Начальная позиция окна</h3>
<p>По умолчанию окно позиционируется в верхний левый угол экрана. Но мы можем изменить его положение, передав нужные значения в метод <code>geometry()</code>:</p>
<pre class="brush:py;">
from tkinter import *
...
<p>Теперь строка в методе geometry имеет следующий формат: "Ширина x Высота + координатаX + координатаY". То есть при запуске окно будет находиться на 300 пикселей вправо и на 250 пикселей вниз от верхнего левого угла экрана.</p>
