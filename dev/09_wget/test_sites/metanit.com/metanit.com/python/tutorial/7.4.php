...
<html >
<head>
<title>Python | Переопределение функционала базового класса</title>
<meta charset="utf-8" />
<meta name="description" content="Полиморфизм в Python, переопределение и вызов методов суперкласса в подклассе, проверка типа объекта и функция isinstance">
...
<h2>Переопределение функционала базового класса</h2><div class="date">Последнее обновление: 28.01.2022</div>
...
<p>Но что, если мы хотим что-то изменить из этого функционала? Например, добавить работнику 
через конструктор, новый атрибут, который будет хранить компанию, где он работает или изменить реализацию метода display_info. Python позволяет переопределить функционал базового класса.</p>
...
<p>Здесь в классе Employee добавляется новый атрибут - <code>self.company</code>, который хранит компания работника. Соответственно метод <code>__init__()</code> 
...
мы хотим в производном классе изменить логику конструктора, то в конструкторе производного класса мы должны вызвать конструктор базового класса. То есть в конструкторе Employee надо вызвать конструктор класса Person.</p>
<p>Для обращения к базовому классу используется выражение <span class="b">super()</span>. Так, в конструкторе Employee выполняется вызов:</p>
<pre class="brush:py;">
super().__init__(name)
</pre>
<p>Это выражение будет представлять вызов конструктора класса Person, в который передается имя работника. И это логично. Ведь имя работника устанавливается именно 
в конструкторе класса Person. В самом конструкторе Employee лишь устанавливаем свойство company.</p>
<p>Кроме того, в классе Employee переопределяется метод <code>display_info()</code> - в него добавляется вывод компании работника. Причем мы могли определить этот метод 
следующим образом:</p>
<pre class="brush:py;">
def display_info(self):
...
<p>Но тогда строка вывода имени повторяла бы код из класса Person. Если эта часть кода совпадает с методом из класса Person, то нет смысла повторяться, 
поэтому опять же с помощью выражения <span class="b">super()</span> обращаемся к реализации метода display_info в классе Person:</p>
<pre class="brush:py;">
def display_info(self):
...
<p>При работе с объектами бывает необходимо в зависимости от их типа выполнить те или иные операции. И с помощью встроенной функции <span class="b">isinstance()</span> мы 
можем проверить тип объекта. Эта функция принимает два параметра:</p>
<pre class="brush:py;">isinstance(object, type)</pre>
<p>Первый параметр представляет объект, а второй - тип, на принадлежность к которому выполняется проверка. Если объект представляет указанный тип, то функция возвращает 
True. Например, возьмем следующую иерархию классов Person-Employee/Student:</p>
<pre class="brush:py;">
class Person:
...
<p>Здесь также определена функция <code>act</code>, которая проверяет с помощью функции <code>isinstance</code>, представляет ли параметр person определнный тип, и зависимости 
от результатов проверки обращается к определенному методу объекта.</p>
...
<div class="nav"><p><a href="./7.3.php">Назад</a><a href="./">Содержание</a><a href="./7.6.php">Вперед</a></p></div></div>
<div class="socBlock">
