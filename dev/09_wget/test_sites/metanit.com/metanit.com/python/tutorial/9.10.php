...
<html >
<head>
<title>Python | Меню</title>
<meta charset="utf-8" />
<meta name="description" content="Создание меню в tkinter и Python, его параметры, обработка нажатия на пункты меню, добавление пунктов меню и подменю">
...
<h2>Меню</h2><div class="date">Последнее обновление: 13.05.2017</div>
...
<p>Для создания иерархического меню в tkinter и Python применяется виджет <span class="b">Menu</span>.</p>
<p>Меню может содержать много элементов, причем эти элементы сами могут представлять меню и содержать другие элементы. В зависимости от того, какой тип элементов 
мы хотим добавить в меню, будет отличаться метод, используемый для их добавления. В частности, нам доступны следующие методы:</p>
<ul>
<li><p><span class="b">add_command(options)</span>: добавляет элемент меню через параметр options</p></li>
<li><p><span class="b">add_cascade(options)</span>: добавляет элемент меню, который в свою очередь может представлять подменю</p></li>
<li><p><span class="b">add_separator()</span>: добавляет линию-разграничитель</p></li>
<li><p><span class="b">add_radiobutton(options)</span>: добавляет в меню переключатель</p></li>
<li><p><span class="b">add_checkbutton(options)</span>: добавляет в меню флажок</p></li>
</ul>
<p>Создадим простейшее меню:</p>
<pre class="brush:py;">
from tkinter import *
...
<p>Для добавления пунктов меню у объекта Menu вызывается метод <span class="b">add_cascade()</span>. В этот метод передаются параметры пункта меню, в данном случае они представлены текстовой меткой, 
устанавливаемой через параметр <code>label</code>.</p>
<p>Но просто создать меню - еще недостаточно. Его надо установить для текущего окна с помощью параметра <span class="b">menu</span> в методе <span class="b">config()</span>. В итоге графическое окно будет иметь следующее меню:</p>
<img src="./pics/9.12.png" alt="Меню в tkinter и Python" />
...
<p>Здесь определяется подменю file_menu, которое добавляется в первый пункт основного меню благодаря установке опции <code>menu=file_menu</code>:</p>
<pre class="brush:py;">main_menu.add_cascade(label="File", menu=file_menu)</pre>
<img src="./pics/9.13.png" alt="Подменю и сепаратор в tkinter в Python" />
<p>Если необходимо настроить меню, то мы можем задать в конструкторе Menu следующие опции:</p>
<ul>
<li><p><span class="b">activebackground</span>: цвет активного пункта меню</p></li>
<li><p><span class="b">activeborderwidth</span>: толщина границы активного пункта меню</p></li>
<li><p><span class="b">activeforeground</span>: цвет текста активного пункта меню</p></li>
<li><p><span class="b">bg</span>: фоновый цвет</p></li>
<li><p><span class="b">bd</span>: толщина границы</p></li>
<li><p><span class="b">cursor</span>: курсор указателя мыши при наведении на меню</p></li>
<li><p><span class="b">disabledforeground</span>: цвет, когда меню находится в состоянии DISABLED</p></li>
<li><p><span class="b">font</span>: шрифт текста</p></li>
<li><p><span class="b">fg</span>: цвет текста</p></li>
<li><p><span class="b">tearoff</span>: меню может быть отсоединено от графического окна. В частности, при создании подменю а скриншоте 
можно увидеть прерывающуюся линию в верху подменю, за которую его можно отсоединить. Однако при значении <code>tearoff=0</code> подменю не сможет быть отсоединено.</p></li>
</ul>
<pre class="brush:py;">
from tkinter import *
...
<img src="./pics/9.14.png" alt="Настройка меню в Python" />
<h3>Взаимодействие с меню</h3>
<p>Отличительной особенностью элементов меню является способность реагировать на нажатия пользователя. Для этого у каждого элемента меню можно задать параметр 
<span class="b">command</span>, который устанавливает ссылку на функцию, выполняемую при нажатии.</p>
<pre class="brush:py;">
from tkinter import *
...
<div class="nav"><p><a href="9.9.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
