...
<html >
<head>
<title>Python | Консольный ввод и вывод</title>
<meta charset="utf-8" />
<meta name="description" content="Консольный ввод и вывод в языке программирования Python, вывод на консоль с помощью функции print, вывод в одну строку, параметр end, получение ввода пользователя с помощью функции input">
...
<h2>Консольный ввод и вывод</h2><div class="date">Последнее обновление: 15.01.2022</div>
...
<p>Для вывода информации на консоль предназначена встроенная функция <span class="b">print()</span>. При вызове этой функции ей в скобках передается 
...
<p>Здесь три вызова функции <code>print()</code> выводят некоторое сообщение. Причем при выводе на консоль каждое сообщение будет размещаться на отдельной 
строке:</p>
<div class="console">
...
<p>Такое поведение не всегда удобно. Например, мы хотим, чтобы все значения выводились на одной строке. Для этого нам надо настроить поведение 
функции с помощью параметра <span class="b">end</span>. Этот параметр задает символы, которые добавляются в конце к выводимой строке и . При применении 
параметра <span class="b">end</span> вызов функции <code>print()</code> выглядит следующим образом:</p>
<pre class="brush:py;">
print(значение, end = конечные_символы)
</pre>
<p>По умолчанию <code>end</code> равен символу <code>"\n"</code>, который задает перевод на следующую строку. Собственно поэтому функция print по умолчанию 
выводит передаваемое ей значение на отдельной строке.</p>
...
<p>В данном случае выводимые сообщения будут отделяться символами <code>" and "</code>:</p>
<div class="console">
<pre class="consoletext">
...
<p>Наряду с выводом на консоль мы можем получать ввод пользователя с консоли, получать вводимые данные. Для этого в Python определена функция 
<span class="b">input()</span>. В эту функцию передается приглашение к вводу. А результат ввода мы можем сохранить в переменную. Например, 
определим код для ввода пользователем имени:</p>
<pre class="brush:py;">
name = input("Введите свое имя: ")
print(f"Ваше имя: {name}")
</pre>
<p>В данном случае в функцию <code>input()</code> передается приглашение к вводу в виде строки "Введите свое имя: ". Результат функции - результат ввода 
пользователя передается в переменную <code>name</code>. Затем мы можем вывести значение этой переменной на консоль с помощью функции <code>print()</code>. 
Пример работы кода:</p>
<div class="console">
<pre class="consoletext">
...
<p>Стоит учитывать, что все введенные значения рассматриваются как значения типа <span class="b">str</span>, то есть строки. И даже если мы вводим число, 
как в втором случае в коде выше, то Python все равно будет рассматривать введенное значение как строку, а не как число.</p>
...
<div class="nav"><p><a href="./2.2.php">Назад</a><a href="./">Содержание</a><a href="./2.3.php">Вперед</a></p></div></div>
<div class="socBlock">
