...
<html >
<head>
<title>Python | guards или ограничения шаблонов</title>
<meta charset="utf-8" />
<meta name="description" content="guards или ограничения шаблонов в pattern matching и конструкция match..case в языке Python, проверка значений шаблонов">
...
<h2>guards или ограничения шаблонов</h2><div class="date">Последнее обновление: 01.02.2022</div>
...
<p><span class="b">Guard</span> или ограничения шаблонов позволяют установить дополнительные условия, которым должно соответсвовать 
выражение. Ограничение задается сразу после шаблона с помощью ключевого слова <span class="b">if</span>, после которого идет условие 
ограничения:</p>
<pre class="brush:py;">
class Person:
...
<p>Соответствует любому объекту Person, у которого атрибут age меньше 18. Собственно часть <code>if age &lt; 18</code> и 
...
то другое.</p>
<p>Подобным образом можно вводить дополнительные ограничения:</p>
<pre class="brush:py;">
class Person:
    def __init__(self, name, age):
        self.name = name
...
<p>Условия ограничений могут быть более сложными и составными по структуре:</p>
<pre class="brush:py;">
def check_data(data):
    match data:
...
<p>В данном случае функция получает кортеж data. Оба шаблона в конструкции match соответствуют двухэлементному кортежу. Но первый 
шаблон также применяет ограничение <code>name == "admin" or age not in range(1, 101)</code>, в соответствии с которым 
первый элемент кортежа должен иметь значение "admin", а второй должен находиться вне диапазона 1-101.</p>
...
<div class="nav"><p><a href="./2.25.php">Назад</a><a href="./">Содержание</a><a href="./2.27.php">Вперед</a></p></div></div>
<div class="socBlock">
