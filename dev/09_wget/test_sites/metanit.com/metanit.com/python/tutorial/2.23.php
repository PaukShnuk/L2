...
<html >
<head>
<title>Python | Массивы в pattern matching</title>
<meta charset="utf-8" />
<meta name="description" content="Массивы в pattern matching и конструкция match..case в языке программирования Python">
...
<h2>Массивы в pattern matching</h2><div class="date">Последнее обновление: 01.02.2022</div>
...
<p>В качестве шаблонов также могут выступать массивы. Подобным шаблоны также могут содержать либо конкретные значения, либо переменные, которые передаются элементы 
массивов, либо символ прочерка _, если элемент массива не важен:</p>
<pre class="brush:py;">
...
<p>В данном случае функция print_people принимает массив, который, как предполагается, состоит из трех элементов.</p>
<p>Первый шаблон предполагает, что элементы массива имеют определенные значения:</p>
<pre class="brush:py;">
case ["Tom", "Sam", "Bob"]:
    print("default people")
</pre>
<p>В данном случае первый элемент массива должен представлять строку "Tom", второй - строку "Sam" и третий - строку "Bob".</p>
<p>Второй шаблон предполагает, что первый элемент массива должен быть равне строке "Tom", остальные два элемента могут иметь произвольные 
значения:</p>
<pre class="brush:py;">
case ["Tom", second, _]:
    print(f"Second Person: {second}")
</pre>
<p>При этом значение второго элемента передается в переменную second, а значение третьего элемента не важно, поэтому вместо него применяется прочерк.</p>
<p>Третий шаблон соответствует любому массиву из трех элементов. При этом его элементы передаются в переменные first, second и third:</p>
<pre class="brush:py;">
case [first, second, third]:
    print(f"{first}, {second}, {third}")
</pre>
<p>В данном случае для соответствия любому из шаблонов массив должен был иметь три элемента. Но также можно определять шаблоны для массивов 
разной длины:</p>
<pre class="brush:py;">
def print_people(people):
    match people:
...
<h3>Массивы неопределенной длины</h3>
<p>Если необходимо сравнивать выражение с массивом неопределенной длины, то можно определить значения/переменные только для обязательных 
элементов массива, а на необязательные ссылаться с помощью символа * (звездочки):</p>
<pre class="brush:py;">
def print_people(people):
    match people:
        case [first, *other]:
...
<p>В примере выше применяется параметр <code>*other</code>, который соответствует всем остальным элементам. 
То есть шаблон <code>[first, *other]</code> соответствует любому массиву, который имеет как минимум один элемент, и этот элемент будет помещаться 
в параметр first. Все остальные элементы помещаются в параметр other, который представляет массив значений.</p>
<p>Если нам параметр с символом * (other) не важен, но мы по прежнему хотим, чтобы шаблон соответствовал массиву с одним и большим количеством элементов, 
...
<p>Если необходимо, чтобы элемент массива соответствовал набору значений, то эти значения можно перечислить через вертикальную черту:</p>
<pre class="brush:py;">
def print_people(people):
    match people:
...
<p>В данном случае первый шаблон соответствует массиву из трех элементов, где первый элемент равен или "Tom", или "Tomas", или "Tommy".</p>
<p>Также можно задать альтернативные значения для отдельных элементов, но и альтернативные массивы:</p>
<pre class="brush:py;">
def print_people(people):
    match people:
...
<p>В данном случае первый шаблон будет соответствовать двум массивам: <code>["Tom", "Sam", "Bob"]</code> и <code>["Tomas", "Sam", "Bob"]</code></p>
...
<div class="nav"><p><a href="./2.22.php">Назад</a><a href="./">Содержание</a><a href="./2.24.php">Вперед</a></p></div></div>
<div class="socBlock">
