...
<html >
<head>
<title>Python | Классы в pattern matching</title>
<meta charset="utf-8" />
<meta name="description" content="Классы в pattern matching и конструкция match..case в языке программирования Python, получение атрибутов в переменные">
...
<h2>Классы в pattern matching</h2><div class="date">Последнее обновление: 01.02.2022</div>
...
<p>Python позволяет использовать в pattern matching в качестве шаблонов объекты классов. Рассмотрим на примере:</p>
<pre class="brush:py;">
class Person:
...
<p>Здесь определен класс Person, который через конструктор принимает значения для атрибутов <code>self.name</code> и <code>self.age</code>.</p>
<p>Функция print_person принимает параметр Person, который, как предполагается, представляет объект класса Person. И внутри функции конструкция 
<span class="b">match</span> сравнивает значение параметра person с рядом шаблонов. Каждый шаблон представляет собой определение Person, где с каждым атрибутом 
сопоставляется некоторое значение. Например, первый шаблон строго определяет значения обоих атрибутов:</p>
<pre class="brush:py;">
case Person(name="Tom", age=37):
    print("Default Person")
</pre>
<p>Данный шаблон соответствует объекту Person, если у этого объекта атрибут name имеет значение "Tom", а атрибут age - значение 37.</p>
<p>Стоит отметить, что этот шаблон - это НЕ вызов конструктора Person. Шаблон просто устанавливает, как атрибуты сопоставляются со значениями.</p>
<p>Второй шаблон строго задает значение только для атрибута age:</p>
<pre class="brush:py;">
case Person(name=name, age=37):
    print(f"Name: {name}")
</pre>
<p>Для соответствия этому шаблону атрибут age должен быть равен 37. А атрибут name может иметь произвольное значение. И это значение передается 
переменной name. А запись <code>name=name</code> расшифровывается как  <code>атрибут_объекта=переменная</code>. А в вызове 
<code>print(f"Name: {name}")</code> на консоль выводится значение переменной name, которая получила значение атрибута name.</p>
<p>В данном случае и атрибут, и переменная имеют одинаковое значение, но это необязательно, и для переменной можно было использовать другое значение, например:</p>
<pre class="brush:py;">
case Person(name=person_name, age=37):      # переменной person_name передается значение атрибута name
    print(f"Name: {person_name}")
</pre>
<p>Третий шаблон соответствует объекту Person, у которого атрибут name равен строке "Tom". А значение атрибута age передается в переменную age:</p>
<pre class="brush:py;">
case Person(name="Tom", age=age):
    print(f"Age: {age}")
</pre>
<p>И в последнем шаблоне атрибуты name и age могут иметь произвольные значения. И эти значения передаются одноименным переменным:</p>
<pre class="brush:py;">
case Person(name=name, age=age):
    print(f"Name: {name}  Age: {age}")
</pre>
<p>При этом нам необязательно использоваться все атрибуты объекта Person. Также мы можем применить паттерн _, если нам надо обработать 
случаи, которые не соответствуют ни одному шаблону:</p>
<pre class="brush:py;">
class Person:
    def __init__(self, name, age):
        self.name = name
...
<p>В данном случае второй шаблон <code>Person(name=person_name)</code> соответствует любому объекту Person, при этом значение атрибута name 
передается переменной person_name</p>
<p>А последний шаблон обрабатывает случаи, когда передано значение, которое не представляет объект Person.</p>
<h3>Передача набора значений</h3>
<p>Также с помощью вертикальной черты можно определить набор значений, которые должен иметь атрибут:</p>
<pre class="brush:py;">
def print_person(person):
    match person:
...
<p>В данном случае первый шаблон соответствует объекту Person, у которого атрибут name имеет одно из трех значений: "Tom", "Tomas" или "Tommy".</p>
<p>Также можно задавать альтернативные значения для всего шаблона в том числе с помощью объектов других классов:</p>
<pre class="brush:py;">
class Person:
    def __init__(self, name, age):
        self.name = name
...
<p>Здесь первый шаблон</p>
<pre class="brush:py;">case Person(name="Tom") | Student(name="Tomas")</pre>
<p>соответствет любому объекту Person, у которого атрибут name = "Tom, и любому объекту Student, у которого атрибут name = "Tomas".</p>
<p>Второй шаблон -  <code>case Person(name=name) | Student(name=name)</code> соответствует любому объекту Person и Student.</p>
<h3>Позиционные параметры</h3>
<p>В примерах выше для определения атрибутов прописывалось их имя: <code>case Person(name="Tom", age=37)</code>. Но если используется куча шаблонов, и в каждом 
...
Но Python также позволяет использовать позиционные параметры:</p>
<pre class="brush:py;">
class Person:
...
<p>Обратите внимание в классе Person на вызов функции:</p>
<pre class="brush:py;">__match_args__ = ("name", "age")</pre>
<p>Благодаря этому Python будет знать, что при указании атрибутов атрибут name будет идти первым, а атрибут age - вторым.</p>
<p>И таким образом, в шаблонов не нужно указывать имя атрибута: <code>case Person("Tom", 37)</code> -  Python сам сопоставит атрибуты и 
значения/переменные на основе их позиции.</p>
...
<div class="nav"><p><a href="./2.24.php">Назад</a><a href="./">Содержание</a><a href="./2.26.php">Вперед</a></p></div></div>
<div class="socBlock">
