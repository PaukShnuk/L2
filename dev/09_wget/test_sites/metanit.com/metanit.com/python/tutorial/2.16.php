...
<html >
<head>
<title>Python | Оператор return и возвращение результата из функции</title>
<meta charset="utf-8" />
<meta name="description" content="Оператор return и возвращение результата из функции в языке программирования Python, выход из функции">
...
<h2>Оператор return и возвращение результата из функции</h2><div class="date">Последнее обновление: 16.01.2022</div>
...
<p>Функция может возвращать результат. Для этого в функции используется оператор <span class="b">return</span>, после которого указывается возвращаемое значение:</p>
<pre class="brush:py;">
...
<p>Здесь после оператора <span class="b">return</span> идет строка <code>"Hello METANIT.COM"</code> - это значение и будет возвращать функция 
<code>get_message()</code>.</p>
...
<p>После оператора return может идти и сложное вычислямое выражение, резлуьтат которого будет возвращаться из функции. Например, определим функцию, которая увеличивает число в два раза:</p>
<pre class="brush:py;">
def double(number):
    return 2 * number
</pre>
<p>Здесь функция double будет возвращать результат выражения <code>2 * number</code>:</p>
<pre class="brush:py;">
def double(number):
    return 2 * number
...
<h3>Выход из функции</h3>
<p>Оператор <span class="b">return</span> не только возвращает значение, но и производит выход из функции. Поэтому он должен определяться 
после остальных инструкций. Например:</p>
<pre class="brush:py;">
def get_message():
    return "Hello METANIT.COM"
...
<p>С точки зрения синтаксиса данная функция корректна, однако ее инструкция <code>print("End of the function")</code> не имеет смысла - она никогда 
выполнится, так как до ее выполнения оператор return возвратит значение и произведет выход из функции.</p>
<p>Однако мы можем использовать оператор <span class="b">return</span> и в таких функциях, которые не возвращают никакого значения. 
В этом случае после оператора return не ставится никакого возвращаемого значения. Типичная ситуация - в зависимости от опеределенных условий 
произвести выход из функции:</p>
<pre class="brush:py;">
...
<p>Здесь функция <code>print_person</code> в качестве параметров принимает имя и возраст пользователя. Однако в функции вначале мы проверяем, 
соответствует ли возраст некоторому диапазону (меньше 120 и больше 0). Если возраст находится вне этого диапазона, 
то выводим сообщение о недопустимом возрасте и с помощью оператора return выходим из функции. После этого функция заканчивает свою работу.</p>
