...
<html >
<head>
<title>Python | Обработка исключений</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка исключений в Python, блок try...except...finally, множественная обработка исключений, передача информации об ошибке">
...
<h1>Обработка ошибок и исключений</h1><h2>Конструкция try...except...finally</h2><div class="date">Последнее обновление: 25.01.2022</div>
...
в процессе ее выполнения. Подобные ошибки еще называются исключениями. Например, в прошлых темах мы рассматривали преобразование строки в число:</p>
<pre class="brush:py;">
string = "5"
number = int(string)
print(number)
...
<p>При выполнении этого скрипта будет выброшено исключение ValueError, так как строку "hello" нельзя преобразовать в число:</p>
<pre class="brush:py;">ValueError: invalid literal for int() with base 10: 'hello'</pre>
<p>С одной стороны, здесь очевидно, 
что строка не представляет число, но мы можем иметь дело с вводом пользователя, который также может ввести не совсем то, что мы ожидаем:</p>
<pre class="brush:py;">
...
<span class="b">try..except</span>.</p>
<h3>try..except</h3>
<p>Конструкция <span class="b">try..except</span> имеет следующее формальное определение:</p>
<pre class="brush:py;">
try:
	инструкции
...
<div class="nav"><p><a href="./7.5.php">Назад</a><a href="./">Содержание</a><a href="./2.20.php">Вперед</a></p></div></div>
<div class="socBlock">
