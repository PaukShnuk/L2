...
<html >
<head>
<title>Python | Словари в pattern matching</title>
<meta charset="utf-8" />
<meta name="description" content="Словари map в pattern matching и конструкция match..case в языке программирования Python, проверка ключей и получение элементов в переменные">
...
<h2>Словари в pattern matching</h2><div class="date">Последнее обновление: 01.02.2022</div>
...
<p>Pattern matching позволяет проверить наличие в словаре определнных ключей и значений:</p>
<pre class="brush:py;">
def look(words):
    match words:
...
<p>Здесь предполагается, что в функцию look передается словарь. Первый шаблон</p>
<pre class="brush:py;">
case {"red": "красный", "blue": "синий"}:  # если в словаре words слова red и blue
    print("Слова red и blue есть в словаре")
</pre>
<p>соответствует словарю, в котором есть два элемента со следующими ключами и значениями: <code>"red": "красный"</code> и 
<code>"blue": "синий"</code>.</p>
<p>Второй шаблон (<code>{"red": "красный"}</code>) соответствует любому словарю, где есть элемент <code>"red": "красный"</code>. 
Аналогично третий шаблон (<code>{"blue": "синий"}</code>) соответствует любому словарю, где есть элемент <code>"blue": "синий"</code></p>
<p>Четвертый шаблон - <code>case {}</code> соответствует в принципе любому словарю.</p>
<p>Последний шаблон соответствует любому значению и применяется на случай, если в функцию передан не словарь.</p>
<h3>Передача набора значений</h3>
<p>С помощью вертикальной черты | можно определить альтернативные значения:</p>
<pre class="brush:py;">
def look(words):
    match words:
...
<p>В данном случае шаблон <code>{"red": "красный" | "алый" | "червонный"}</code> соответствует словарю, в котором есть элемент с ключом 
"red" и значением "красный" или "алый" или "червонный".</p>
<p>Также можно задать альтернативный набор словарей:</p>
<pre class="brush:py;">
def look(words):
    match words:
...
<p>Первый шаблон - <code>{"red": "красный"} | {"blue": "синий"}</code> соответствует словарю, в котором есть либо элемент 
<code>{"red": "красный"}</code>, или <code>{"blue": "синий"}</code>, или оба.</p>
<p>Если нам важны сами ключи, но не важно значение ключей, то вместо конкретных значений можно передать шаблон _:</p>
<pre class="brush:py;">
def look(words):
    match words:
        case {"red": _, "blue": _}:
...
<h3>Получение значений по ключам</h3>
<p>Pattern matching позволяет получить значения элементов в переменные в виде:</p>
<pre class="brush:py;">{ключ: переменная}</pre>
<p>Например:</p>
<pre class="brush:py;">
def look(words):
    match words:
...
<p>В первом шаблоне значение элемента с ключом "red" попадает в переменную red, а элемента с ключом "blue" - в переменную blue.</p>
<h3>Получение всех значений</h3>
<p>С помощью символов ** (двойная звездочка) можно получить остальные элементы словаря:</p>
<pre class="brush:py;">
def look(words):
    match words:
        case {"red": red, **rest}:
...
<p>Здесь шаблон <code>{"red": red, **rest}</code> соответствует любому словарь, в котором есть элемент с ключом "red". Все остальные 
элементы словаря помещаются в параметр rest, который сам в свою очередь представляет словарь.</p>
...
<div class="nav"><p><a href="./2.23.php">Назад</a><a href="./">Содержание</a><a href="./2.25.php">Вперед</a></p></div></div>
<div class="socBlock">
