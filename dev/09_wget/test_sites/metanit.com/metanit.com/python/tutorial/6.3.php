...
<html >
<head>
<title>Python | Модуль locale</title>
<meta charset="utf-8" />
<meta name="description" content="Модуль locale в Python, установка текущей культуры, локализация и форматирование чисел и валют в соответствии с локальной культурой">
...
<h2>Модуль locale</h2><div class="date">Последнее обновление: 04.02.2022</div>
...
<p>При форматировании чисел Python по умолчанию использует англосаксонскую систему, при которой разряды целого числа отделяются друг от друга запятыми, а 
...
а дробная и целая часть - запятой:</p>
<pre class="brush:py;">
...
<p>И для решения проблемы форматирования под определенную культуру в Python имеется встроенный модуль <span class="b">locale</span>.</p>
<p>Для установки локальной культуры в модуле locale определена функция <span class="b">setlocale()</span>. Она принимает два параметра:</p>
<pre class="brush:py;">setlocale(category, locale)</pre>
<p>Первый параметр указывает на категорию, к которой применяется функция - к числам, валютам или и числам, и валютам. 
В качестве значения для параметра мы можем передавать одну из следующих констант:</p>
<ul>
<li><p><span class="b">LC_ALL</span>: применяет локализацию ко всем категориям - к форматированию чисел, валют, дат и т.д.</p></li>
<li><p><span class="b">LC_NUMERIC</span>: применяет локализацию к числам</p></li>
<li><p><span class="b">LC_MONETARY</span>: применяет локализацию к валютам</p></li>
<li><p><span class="b">LC_TIME</span>: применяет локализацию к датам и времени</p></li>
<li><p><span class="b">LC_CTYPE</span>: применяет локализацию при переводе символов в верхний или нижний регистр</p></li>
<li><p><span class="b">LC_COLLIATE</span>: применяет локаль при сравнении строк</p></li>
</ul>
<p>Второй параметр функции setlocale указывает на локальную культуру, которую надо использовать. На ОС Windows можно использовать код станы по ISO из двух символов, 
...
"en_US", для немецкого в Германии - "de_DE", для русского в России - "ru_RU". По умолчанию фактически используется культура "en_US".</p>
<p>Непосредственно для форматирования чисел и валют модуль locale предоставляет две функции:</p>
<ul>
<li><p><code>currency(num)</code>: форматирует валюту</p></li>
<li><p><code>format_string(str, num)</code>: подставляет число num вместо плейсхолдера в строку str</p>
<p>Применяются следующие плейсхолдеры:</p>
<ul>
<li><p><code>d</code>: для целых чисел</p></li>
<li><p><code>f</code>: для чисел с плавающей точкой</p></li>
<li><p><code>e</code>: для экспоненциальной записи чисел</p></li>
</ul>
<p>Перед каждым плейсхолдером ставится знак процента %, например:</p>
<pre class="brush:py;">"%d"</pre>
<p>При выводе дробных чисел перед плейсхолдером после точки можно указать, сколько знаков в дробной части должно отображаться:</p>
<pre class="brush:py;">%.2f		# два знака в дробной части</pre>
</li>
</ul>
<p>Применим локализацию чисел и валют в немецкой культуре:</p>
<pre class="brush:py;">
import locale
...
<p>Если вместо конкретного кода в качестве второго параметра передается пустая строка, то Python будет использовать культуру, которая применяется на 
текущей рабочей машине. А с помощью функции <span class="b">getlocale()</span> можно получить эту культуру:</p>
<pre class="brush:py;">
import locale
...
<p>Стоит отметить, что в зависимости от системы вывод может отличаться.</p>
