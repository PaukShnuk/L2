...
<html >
<head>
<title>Python | Установка и первая программа на Windows</title>
<meta charset="utf-8" />
<meta name="description" content="Первая программа на Python на Windows, установка и запуск интерпретатора, создание скрипта">
...
<h2>Установка и первая программа на Windows</h2><div class="date">Последнее обновление: 22.03.2022</div>
...
<h3>Установка</h3>
<p>Для создания программ на Python нам потребуется интерпретатор. Для его установки перейдем на страницу 
<a href="https://www.python.org/downloads/" rel="nofollow">https://www.python.org/downloads/</a> и найдем ссылку на загрузку последней версии языка:</p>
<img src="./pics/1.1.png" alt="Установка Python" />
<p>По нажатию на кнопку будет загружен соответствующей текущей ОС установщик Python. Следует учитывать, что Windows 7 и более ранние версии не поддерживаются.</p>
<p>На ОС Windows при запуске инсталлятора запускает окно мастера установки:</p>
<img src="./pics/1.3.png" alt="Установка дистрибутива Python 3.10 на Windows" />
<p>Здесь мы можем задать путь, по которому будет устанавливаться интерпретатор. Оставим его по умолчанию, то есть 
<span class="ii">C:\Users\[имя_пользователя]\AppData\Local\Programs\Python\Python310\</span>.</p>
<p>Кроме того, в самом низу отметим флажок "Add Python 3.10 to PATH", чтобы добавить путь к интерпретатору в переменные среды.</p>
<p>После этого мы можем проверить установку Python и его версию, запустив в командной строке/треминале команду</p>
<div class="console"><pre class="consoletext">python --version</pre></div>
<h3>Запуск интерпретатора</h3>
<p>После установки интерпретатора, как было описано в прошлой теме, мы можем начать создавать приложения на Python. Итак, создадим первую простенькую программу.</p>
<p>Как было сказано в прошлой теме, программа интерпретатора, если при установке не был изменен адрес, по умолчанию 
устанавливается на Linux по пути <span class="ii">usr/local/bin/python310</span>, а на Windows по пути 
<span class="ii">C:\Users\[имя_пользователя]\AppData\Local\Programs\Python\Python310\</span> и представляет файл под названием <span class="b">python.exe</span>.</p>
<img src="./pics/1.11.png" alt="Интерпретатор Python" />
<p>Запустим интерпретатор и введем в него следующую строку:</p>
<pre class="brush:py;">
print("hello world")
</pre>
<p>И консоль выведет строку "hello world":</p>
<img src="./pics/1.5.png" alt="Первая программа на Python" />
<p>Для этой программы использовалась функция <span class="b">print()</span>, которая выводит некоторую строку на консоль.</p>
<h3>Создание файла программы</h3>
<p>В реальности, как правило, программы определяются во внешних файлах-скриптах и затем передаются интерпретатору на выполнение. Поэтому создадим файл программы. 
Для этого на диске C или где-нибудь в другом месте файловой системы определим для скриптов папку <span class="b">python</span>. А в этой папке создадим новый текстовый файл, который 
назовем <span class="b">hello.py</span>. По умолчанию файлы с кодом на языке Python, как правило, имеют расширение <span class="b">py</span>.</p>
<img src="./pics/1.6.png" alt="Создание скрипта на языке Python" />
<p>Откроем этот файл в любом текстовом редакторе и добавим в него следующий код:</p>
<pre class="brush:py;">
name = input("Введите имя: ")
print("Привет,", name)
</pre>
<img src="./pics/1.8.png" alt="Python в Notepad++" />
<p>Скрипт состоит из двух строк. Первая строка с помощью функции <span class="b">input()</span> ожидает ввода пользователем своего имени. Введенное 
имя затем попадает в переменную <code>name</code>.</p>
<p>Вторая строка с помощью функции <code>print()</code> выводит приветствие вместе с введенным именем.</p>
<p>Теперь запустим командную строку/терминал и с помощью команды cd перейдем к папке, где находится файл с исходным кодом hello.py (например, в моем случае это папка C:\python).</p><div class="console">
<pre class="consoletext">
cd c:\python
</pre>
</div>
<p>Далее вначале введем полный путь к интерпретатору, а затем полный путь к файлу скрипта. К примеру, в моем случае в консоль надо будет вести:</p>
<div class="console">
<pre class="consoletext">
C:\Users\Eugene\AppData\Local\Programs\Python\Python310\python.exe hello.py
</pre>
</div>
<p>Но если при установке была указана опция <span class="b">"Add Python 3.10 to PATH"</span>, то есть путь к интерпретатору Python был добавлен в переменные среды, то вместо полного пути к интерпретатору можно просто написать python:</p>
<div class="console">
<pre class="consoletext">
python hello.py
</pre>
</div>
<p>Варианты с обоими способами запуска:</p>
<img src="./pics/1.10.png" alt="Первое приложение на Python" />
<p>В итоге программа выведет приглашение к вводу имени, а затем приветствие.</p>
