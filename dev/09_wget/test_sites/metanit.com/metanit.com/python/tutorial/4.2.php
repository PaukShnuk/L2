...
<html >
<head>
<title>Python | Текстовые файлы</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с текстовыми файлами в Python, их открытие, чтение и запись, методы write, read, readline, readlines">
...
<h2>Текстовые файлы</h2><div class="date">Последнее обновление: 21.06.2017</div>
...
<p>Чтобы открыть текстовый файл на запись, необходимо применить режим w (перезапись) или a (дозапись). Затем для записи применяется метод <span class="b">write(str)</span>, 
...
предварительно нужно конвертировать в строку.</p>
<p>Запишем некоторую информацию в файл "hello.txt":</p>
<pre class="brush:py;">
with open("hello.txt", "w") as file:
    file.write("hello world")
</pre>
<p>Если мы откроем папку, в которой находится текущий скрипт Python, то увидем там файл hello.txt. Этот файл можно открыть в любом текстовом редакторе и при желании изменить.</p>
<p>Теперь дозапишем в этот файл еще одну строку:</p>
<pre class="brush:py;">
with open("hello.txt", "a") as file:
    file.write("\ngood bye, world")
</pre>
<p>Дозапись выглядит как добавление строку к последнему символу в файле, поэтому, если необходимо сделать запись с новой строки, то можно использовать эскейп-последовательность "\n". 
В итоге файл hello.txt будет иметь следующее содержимое:</p>
<pre class="brush:py;">
hello world
good bye, world
</pre>
<p>Еще один способ записи в файл представляет стандартный метод <span class="b">print()</span>, который применяется для вывода данных на консоль:</p>
<pre class="brush:py;">
with open("hello.txt", "a") as hello_file:
    print("Hello, world", file=hello_file)
</pre>
<p>Для вывода данных в файл в метод print в качестве второго параметра передается название файла через параметр file. А первый параметр представляет записываемую 
в файл строку.</p>
<h3>Чтение файла</h3>
<p>Для чтения файла он открывается с режимом r (Read), и затем мы можем считать его содержимое различными методами:</p>
<ul>
<li><p><span class="b">readline()</span>: считывает одну строку из файла</p></li>
<li><p><span class="b">read()</span>: считывает все содержимое файла в одну строку</p></li>
<li><p><span class="b">readlines()</span>: считывает все строки файла в список</p></li>
</ul>
<p>Например, считаем выше записанный файл построчно:</p>
<pre class="brush:py;">
with open("hello.txt", "r") as file:
    for line in file:
        print(line, end="")
</pre>
<p>Несмотря на то, что мы явно не применяем метод <code>readline()</code> для чтения каждой строки, но в при переборе файла этот метод автоматически вызывается 
...
print передается значение <code>end=""</code>.</p>
<p>Теперь явным образом вызовем метод <code>readline()</code> для чтения отдельных строк:</p>
<pre class="brush:py;">
with open("hello.txt", "r") as file:
    str1 = file.readline()
    print(str1, end="")
...
<p>Метод readline можно использовать для построчного считывания файла в цикле while:</p>
<pre class="brush:py;">
with open("hello.txt", "r") as file:
    line = file.readline()
    while line:
...
<p>Если файл небольшой, то его можно разом считать с помощью метода <span class="b">read()</span>:</p>
<pre class="brush:py;">
with open("hello.txt", "r") as file:
    content = file.read()
    print(content)
</pre>
<p>И также применим метод <span class="b">readlines()</span> для считывания всего файла в список строк:</p>
<pre class="brush:py;">
with open("hello.txt", "r") as file:
...
<p>При чтении файла мы можем столкнуться с тем, что его кодировка не совпадает с ASCII. В этом случае мы явным образом можем указать кодировку с помощью 
параметра <span class="b">encoding</span>:</p>
<pre class="brush:py;">
filename = "hello.txt"
with open(filename, encoding="utf8") as file:
    text = file.read()
</pre>
<p>Теперь напишем небольшой скрипт, в котором будет записывать введенный пользователем массив строк и считывать его обратно из файла на консоль:</p>
<pre class="brush:py;">
# имя файла
