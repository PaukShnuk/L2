...
<html >
<head>
<title>Python | Диапазоны</title>
<meta charset="utf-8" />
<meta name="description" content="Диапазоны в Python, функция range и ее параметры и применение в цикле for, отличие диапазонов range от списков и кортежей">
...
<h2>Диапазоны</h2><div class="date">Последнее обновление: 29.01.2022</div>
...
<p>Диапазоны или range представляют неизменяемый последовательный набор чисел. Для создания диапазов применяетя <span class="b">range</span>, которая имеет следующие формы:</p>
<ul>
<li><p><span class="b">range(stop)</span>: возвращает все целые числа от 0 до stop</p></li>
<li><p><span class="b">range(start, stop)</span>: возвращает все целые числа в промежутке от start (включая) до stop (не включая).</p></li>
...
<p>Диапазоны чаще всего применяются в циклах <span class="b">for</span>. Например, выведем последовательно все числа от 0 до 4:</p>
<pre class="brush:py;">
for i in range(5):
    print(i, end=" ")
...
<p>Другой пример выведем таблицу умножения:</p>
<pre class="brush:py;">
for i in range(1, 10):
    for j in range(1, 10):
        print(i * j, end="\t")
...
<p>Если нам необходим последовательный список чисел, то для его создания удобно использовать функцию <span class="b">range</span>:</p>
<pre class="brush:py;">
...
<p>Преимуществом диапазонов над стандартными списками и кортежами является то, что диапазон всегда будет занимать одно и то же небольшое 
количество памяти вне зависимости от того, какой набор чисел представляет этот диапазон. В действительности диапазон хранит только начальное, конечное значение и приращение.</p>
...
<div class="nav"><p><a href="./3.2.php">Назад</a><a href="./">Содержание</a><a href="./3.3.php">Вперед</a></p></div></div>
<div class="socBlock">
