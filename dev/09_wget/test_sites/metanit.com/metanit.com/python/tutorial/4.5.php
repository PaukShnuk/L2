...
<html >
<head>
<title>Python | Модуль OS и работа с файловой системой</title>
<meta charset="utf-8" />
<meta name="description" content="Модуль OS и работа с файловой системой в Python, удаление и переименование файлов, создание и удаление каталогов">
...
<h2>Модуль OS и работа с файловой системой</h2><div class="date">Последнее обновление: 02.05.2017</div>
...
<p>Ряд возможностей по работе с каталогами и файлами предоставляет встроенный модуль <span class="b">os</span>. Хотя он содержит много функций, 
рассмотрим только основные из них:</p>
<ul>
<li><p><span class="b">mkdir()</span>: создает новую папку</p></li>
<li><p><span class="b">rmdir()</span>: удаляет папку</p></li>
<li><p><span class="b">rename()</span>: переименовывает файл</p></li>
<li><p><span class="b">remove()</span>: удаляет файл</p></li>
</ul>
<h3>Создание и удаление папки</h3>
<p>Для создания папки применяется функция <span class="b">mkdir()</span>, в которую передается путь к создаваемой папке:</p>
<pre class="brush:py;">
import os
...
<p>Для удаления папки используется функция <span class="b">rmdir()</span>, в которую передается путь к удаляемой папке:</p>
<pre class="brush:py;">
import os
...
<p>Для переименования вызывается функция <span class="b">rename(source, target)</span>, первый параметр которой - путь к исходному файлу, а второй - новое имя файла. 
В качестве путей могут использоваться как абсолютные, так и относительные. Например, пусть в папке <span class="ii">C://SomeDir/</span> располагается файл 
<span class="ii">somefile.txt</span>. Переименуем его в файл "hello.txt":</p>
<pre class="brush:py;">
import os
...
<p>Для удаления вызывается функция <span class="b">remove()</span>, в которую передается путь к файлу:</p>
<pre class="brush:py;">
import os

os.remove("C://SomeDir/hello.txt")
</pre>
<h3>Существование файла</h3>
<p>Если мы попытаемся открыть файл, который не существует, то Python выбросит исключение FileNotFoundError. Для отлова исключения мы можем использовать 
конструкцию try...except. Однако можно уже до открытия файла проверить, существует ли он или нет с помощью метода <span class="b">os.path.exists(path)</span>. 
В этот метод передается путь, который необходимо проверить:</p>
<pre class="brush:py;">
...
<div class="nav"><p><a href="4.6.php">Назад</a><a href="./">Содержание</a><a href="8.1.php">Вперед</a></p></div></div>
<div class="socBlock">
