...
<html >
<head>
<title>Python | Генерация исключений и создание своих типов исключений</title>
<meta charset="utf-8" />
<meta name="description" content="Генерация исключений и оператор raise в языке программирования Python, создание своих типов исключений">
...
<h2>Генерация исключений и создание своих типов исключений</h2><div class="date">Последнее обновление: 30.01.2022</div>
...
<h3>Генерация исключений и оператор raise</h3>
<p>Иногда возникает необходимость вручную сгенерировать то или иное исключение. Для этого применяется оператор <span class="b">raise</span>. Например, сгенерируем исключение</p>
<pre class="brush:py;">
try:
...
<p>Оператору <span class="b">raise</span> передается объект BaseException - в данном случае объект Exception. В конструктор 
...
raise, который сгенерирует исключение. В итоге управление программой перейдет к блоку <span class="b">except</span>, который обрабатывает 
исключения типа Exception:</p>
<div class="console">
...
<h3>Создание своих типов исключений</h3>
<p>В языке Python мы не ограничены только встроенными типами исключений и можем, применяя наследование, при необходимости создавать свои 
типы исключений. Например, возьмем следующий класс Person:</p>
<pre class="brush:py;">
class Person:
...
<p>Здесь класс Person в конструкторе получает значения для имени и возраста и присваивает их приватным переменным name и age. 
...
Одним из способов решения данной ситуации представляет генерация исключения при передаче некорректных значений.</p>
<p>Итак, определим следующий код программы:</p>
<pre class="brush:py;">
...
<p>В начале здесь определен класс PersonAgeException, который наследуется от класса Exception. Как правило, собственные классы исключений 
наследуются от класса Exception. Класс PersonAgeException предназначен для исключений, связанных с возрастом пользователя.</p>
<p>В конструкторе PersonAgeException получаем три значения - собственное некорректное значение, которое послужило причиной исключения, 
а также минимальное и максимальное значения возраста.</p>
<pre class="brush:py;">
class PersonAgeException(Exception):
    def __init__(self, age, minage, maxage):
        self.age = age
...
<p>В функции __str__  определяем текстовое представление класса - по сути сообщение об ошибке.</p>
<p>В конструкторе класса Persoon проверяем переданное для возраста пользователя значение. И если это значение не соответствует 
определенному диапазону, то генерируем исключение типа PersonAgeException:</p>
<pre class="brush:py;">raise PersonAgeException(age, minage, maxage)</pre>
<p>При применении класса Person нам следует учитывать, что конструктор класса может сгенерировать исключение при передаче некорректного 
значения. Поэтому создание объектов Person обертывается в конструкцию try..except:</p>
<pre class="brush:py;">
try:
    tom = Person("Tom", 37)
    tom.display_info()  # Имя: Tom 	Возраст: 37
...
<p>И если при вызове конструктора Person будет сгенерировано исключение типа PersonAgeException, то управление программой перейдет к 
блоку except, который обрабатывает исключения типа PersonAgeException в виде вывода информации об исключении на консоль.</p>
...
<div class="nav"><p><a href="./2.20.php">Назад</a><a href="./">Содержание</a><a href="./3.1.php">Вперед</a></p></div></div>
<div class="socBlock">
