...
<html >
<head>
<title>Python | Операции с датами</title>
<meta charset="utf-8" />
<meta name="description" content="Форматирование дат и времени, основные операции с датами в Python, класс timedelta, сравнение дат">
...
<h2>Операции с датами</h2><div class="date">Последнее обновление: 05.05.2017</div>
...
<h3>Фоматирование дат и времени</h3>
<p>Для форматирования объектов date и time в обоих этих классах предусмотрен метод <span class="b">strftime(format)</span>. Этот метод принимает только один 
параметр, указывающий на формат, в который нужно преобразовать дату или время.</p>
<p>Для определения формата мы можем использовать один из следующих кодов форматирования:</p>
<ul>
<li><p><span class="b">%a</span>: аббревиатура дня недели. Например, Wed - от слова Wednesday (по умолчанию используются английские наименования)</p></li>
<li><p><span class="b">%A</span>: день недели полностью, например, Wednesday</p></li>
<li><p><span class="b">%b</span>: аббревиатура названия месяца. Например, Oct (сокращение от October)</p></li>
<li><p><span class="b">%B</span>: название месяца полностью, например, October</p></li>
<li><p><span class="b">%d</span>: день месяца, дополненный нулем, например, 01</p></li>
<li><p><span class="b">%m</span>: номер месяца, дополненный нулем, например, 05</p></li>
<li><p><span class="b">%y</span>: год в виде 2-х чисел</p></li>
<li><p><span class="b">%Y</span>: год в виде 4-х чисел</p></li>
<li><p><span class="b">%H</span>: час в 24-х часовом формате, например, 13</p></li>
<li><p><span class="b">%I</span>: час в 12-ти часовом формате, например, 01</p></li>
<li><p><span class="b">%M</span>: минута</p></li>
<li><p><span class="b">%S</span>: секунда</p></li>
<li><p><span class="b">%f</span>: микросекунда</p></li>
<li><p><span class="b">%p</span>: указатель AM/PM</p></li>
<li><p><span class="b">%c</span>: дата и время, отформатированные под текущую локаль</p></li>
<li><p><span class="b">%x</span>: дата, отформатированная под текущую локаль</p></li>
<li><p><span class="b">%X</span>: время, форматированное под текущую локаль</p></li>
</ul>
<p>Используем различные форматы:</p>
<pre class="brush:py;">
...
<p>При выводе названий месяцев и дней недели по умолчанию используются английские наименования. Если мы хотим использовать текущую локаль, но то мы 
можем ее предварительно установить с помощью модуля locale:</p>
<pre class="brush:py;">
from datetime import datetime
import locale
...
<h3>Сложение и вычитани дат и времени</h3>
<p>Нередко при работе с датами возникает необходимость добавить к какой-либо дате определенный промежуток времени или, наоборот, вычесть некоторый период. И специально для 
таких операций в модуле datetime определен класс <span class="b">timedelta</span>. Фактически этот класс определяет некоторый период времени.</p>
<p>Для определения промежутка времени можно использовать конструктор timedelta:</p>
<pre class="brush:py;">timedelta([days] [, seconds] [, microseconds] [, milliseconds] [, minutes] [, hours] [, weeks])</pre>
<p>В конструктор мы последовательно передаем дни, секунды, микросекунды, миллисекунды, минуты, часы и недели.</p>
<p>Определим несколько периодов:</p>
<pre class="brush:py;">
...
<p>Используя timedelta, мы можем складывать или вычитать даты. Например, получим дату, которая будет через два дня:</p>
<pre class="brush:py;">
...
<p>Или узнаем, сколько было времени 10 часов 15 минут назад, то есть фактически нам надо вычесть из текущего времени 10 часов и 15 минут:</p>
<pre class="brush:py;">
from datetime import timedelta, datetime
...
<h3>Свойства timedelta</h3>
<p>Класс timedelta имеет несколько свойств, с помощью которых мы можем получить временной промежуток:</p>
<ul>
<li><p><span class="b">days</span>: возвращает количество дней</p></li>
<li><p><span class="b">seconds</span>: возвращает количество секунд</p></li>
<li><p><span class="b">microseconds</span>: возвращает количество микросекунд</p></li>
</ul>
<p>Кроме того, метод <span class="b">total_seconds()</span> возвращает общее количество секунд, куда входят и дни, и собственно секунды, и микросекунды.</p>
<p>Например, узнаем какой временной период между двумя датами:</p>
<pre class="brush:py;">
from datetime import timedelta, datetime
...
<h3>Сравнение дат</h3>
<p>Также как и строки и числа, даты можно сравнивать с помощью стандартных операторов сравнения:</p>
<pre class="brush:py;">
from datetime import datetime
