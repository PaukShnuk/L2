...
<html >
<head>
<title>Python | Преобразование типов</title>
<meta charset="utf-8" />
<meta name="description" content="Преобразование типов в Python, правила неявных преобразований, явные преобразования и функции int(), float(), str()">
...
<h2>Преобразование типов</h2><div class="date">Последнее обновление: 17.01.2022</div>
...
<p>В операциях с данными могут применяться значения различных типов. Например, складываются число типа <code>int</code> и число типа 
<code>float</code>:</p>
<pre class="brush:py;">
a = 2       # число int
b = 2.5     # число float
...
<p>В данном случае никакой ошибки не будет. Однако Python не всегда может автоматически производить операции, в которых участвуют 
данные разных типов. Рассмотрим, какие в данном случае действуют правила.</p>
<h3>Неявные преобразования</h3>
<p>Оба числа в арифметических операциях должны представлять один и тот же тип. Если же два операнда операции представляют разные типы данных, то 
Python пытается автоматически выполнить преобразования к одному из типов в соответствии со следующми правилами:</p>
<ul> 
<li><p>Если один из операндов операции представляет комплексное число (тип <span class="b">complex</span>), то другой операнд также преобразуется к 
типу complex.</p></li>
<li><p>Иначе, если один из операндов представляет тип <span class="b">float</span>, то второй операнд также преобразуется к типу float. Собственно так и произошло в примере выше, где 
значение переменной <code>a</code> было преобразовано в тип float</p></li>
<li><p>Иначе, оба операнда должны представлять тип <span class="b">int</span>, и в этом случае преобазование не требуется</p></li>
</ul>
...
<p>Мы ожидаем, что "2" + 3 будет равно 5. Однако этот код сгенерирует исключение, так как первое число на самом деле представляет строку. 
И мы увидим при выполнении кода что-то наподобие:</p>
<div class="console">
<pre class="consoletext">
Traceback (most recent call last):
  File "/Users/eugene/PycharmProjects/HelloApp/main.py", line 3, in <module>
    c = a + b
...
<ul>
<li><p><span class="b">int()</span>: преобразует значение в целое число</p></li>
<li><p><span class="b">float()</span>: преобразует значение в число с плавающей точкой</p></li>
<li><p><span class="b">str()</span>: преобразует значение в строку</p></li>
</ul>
<h4>int</h4>
...
<p>Примеры преобразований с помощью <code>int()</code>:</p>
<pre class="brush:py;">
a = int(15)     # a = 15
b = int(3.7)    # b = 3
c = int("4")    # c = 4
...
<p>Однако если значение не может быть преобразовано, то функция int выдаст ошибку <code>ValueError: invalid literal for int() with base 10</code>:</p>
<pre class="brush:py;">
b = int("a1c")    # Ошибка
c = int("4.7")    # Ошибка
</pre>
<h4>float</h4>
<p>Аналогичным образом действует функция  <span class="b">float()</span>, которая преобразует в число с плавающей точкой.</p>
<pre class="brush:py;">
a = "2.7"
b = 3
c = float(a) + b
...
<p>Примеры преобразований с помощью <code>float()</code>:</p>
<pre class="brush:py;">
a = float(15)       # a = 15.0
b = float(3.7)      # b = 3.7
c = float("4.7")    # c = 4.7
...
<pre class="brush:py;">
d = float("abc")  # Ошибка </pre>
<h4>str</h4>
<p>Функция <span class="b">str()</span> преобразует значение в строку:</p>
<pre class="brush:py;">
a = str(False)      # a = "False"
b = str(True)       # b = "True"
c = str(5)         # c = "5"
...
<p>Функция <code>str()</code> может быть актуальна, например, при добавлении к строке значения другого типа. Например, 
в следующем случае мы получим ошибку:</p>
<pre class="brush:py;">
age = 22
message = "Age: " + age     # Ошибка
print(message)
</pre>
<p>Если число складывается с число, то это стандартная операция сложения чисел. Если строка складывается со строкой, то 
...
<div class="nav"><p><a href="./2.18.php">Назад</a><a href="./">Содержание</a><a href="./2.9.php">Вперед</a></p></div></div>
<div class="socBlock">
