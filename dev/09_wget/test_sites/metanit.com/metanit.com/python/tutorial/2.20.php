...
<html >
<head>
<title>Python | except и обработка разных типов исключений</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка исключений в Python, блок except и обработка разных типов исключений, встроенные типы исключений, тип Exception, получение информации об ошибке">
...
<h2>except и обработка разных типов исключений</h2><div class="date">Последнее обновление: 30.01.2022</div>
...
<h3>Встроенные типы исключений</h3>
...
<p>В данном случае блок execpt обрабатывает только исключения типа <span class="b">ValueError</span>, которые могут возникнут при неудачном преобразовании 
строки в число.</p>
<p>В Python есть следующие базовые типы исключений:</p>
<ul>
<li><p><span class="b">BaseException</span>: базовый тип для всех встроенных исключений</p></li>
<li><p><span class="b">Exception</span>: базовый тип, который обычно применяется для создания своих типов исключений</p></li>
<li><p><span class="b">ArithmeticError</span>: базовый тип для исключений, связанных с арифметическими операциями (OverflowError, ZeroDivisionError, FloatingPointError).</p></li>
<li><p><span class="b">BufferError</span>: тип исключения, которое возникает при невозможности выполнить операцию с буффером</p></li>
<li><p><span class="b">LookupError</span>: базовый тип для исключений, которое возникают при обращении в коллекциях по некорректному ключу или индексу 
(например, IndexError, KeyError)</p></li>
</ul>
<p>От этих классов наследуются все конкретные типы исключений. В Python обладает довольно большим списком встроенных исключений. Весь  этот список можно 
посмотреть в <a href="https://docs.python.org/3/library/exceptions.html" rel="nofollow">документации</a>. Перечислю только некоторые наиболее часто встречающиеся:</p>
<ul>
<li><p><span class="b">IndexError</span>: исключение возникает, если индекс при обращении к элементу коллекции находится вне допустимого диапазона</p></li>
<li><p><span class="b">KeyError</span>: возникает, если в словаре отсутствует ключ, по которому происходит обращение к элементу словаря.</p></li>
<li><p><span class="b">OverflowError</span>: возникает, если результат арифметической операции не может быть представлен текущим числовым типом (обычно типом float).</p></li>
<li><p><span class="b">RecursionError</span>: возникает, если превышена допустимая глубина рекурсии.</p></li>
<li><p><span class="b">TypeError</span>: возникает, если операция или функция применяется к значению недопустимого типа.</p></li>
<li><p><span class="b">ValueError</span>: возникает, если операция или функция получают объект корректного типа с некорректным значением.</p></li>
<li><p><span class="b">ZeroDivisionError</span>: возникает при делении на ноль.</p></li>
<li><p><span class="b">NotImplementedError</span>: тип исключения для указания, что какие-то методы класса не реализованы</p></li>
<li><p><span class="b">ModuleNotFoundError</span>: возникает при при невозможности найти модуль при его импорте директивой <code>import</code></p></li>
<li><p><span class="b">OSError</span>: тип исключений, которые генерируются при возникновении ошибок системы (например, невозможно найти файл, память диска заполнена и т.д.)</p></li>
</ul>
<p>И если ситуация такова, что в программе могут быть сгенерированы различные типы исключений, то мы можем их обработать по отдельности, используя дополнительные 
выражения except. И при возникновении исключения Python будет искать нужный блок except, который обрабатывает данный тип исключения:</p>
<pre class="brush:py;">
try:
...
<p>Тип <span class="b">BaseException</span> представляет общее исключение, под которое попадают все исключительные ситуации. Поэтому в данном случае 
любое исключение, которое не представляет тип ValueError или ZeroDivisionError, будет обработано в блоке <code>except BaseException:</code>.</p>
<p>Однако, если в программе возникает исключение типа, для которого нет соответствующего блока <code>except</code>, то программа не сможет найти соответствующий блок except и 
сгенерирует исключение. Например, в следующем случае:</p>
<pre class="brush:py;">
try:
    number1 = int(input("Введите первое число: "))
    number2 = int(input("Введите второе число: "))
...
<p>Здесь предусмотрена обработка деления на ноль с помощью блока <code>except ZeroDivisionError</code>. Однако если пользователь вместо числа введет некорвертиуремую в число в строку, то 
возникнет исключение типа ValueError, для которого нет соответствующего блока except. И поэтому программа аварийно завершит свое выполнение.</p>
<p>Python позволяет в одном блоке except обрабатывать сразу несколько типов исключений. В этом случае все типы исключения передаются в скобках:</p>
<pre class="brush:py;">
try:
    number1 = int(input("Введите первое число: "))
    number2 = int(input("Введите второе число: "))
...
<div class="nav"><p><a href="./2.11.php">Назад</a><a href="./">Содержание</a><a href="./2.21.php">Вперед</a></p></div></div>
<div class="socBlock">
