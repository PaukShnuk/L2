...
<html >
<head>
<title>Python | Модуль shelve</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с бинарными файлами в Python и модуль shelve, сохранение и получение из файла элементов по ключу">
...
<h2>Модуль shelve</h2><div class="date">Последнее обновление: 22.06.2017</div>
...
<p>Для работы с бинарными файлами в Python может применяться еще один модуль - <span class="b">shelve</span>. 
Он сохраняет объекты в файл с определенным ключом. Затем по этому ключу может извлечь ранее сохраненный объект из файла. 
Процесс работы с данными через модуль shelve напоминает работу со словарями, которые также используют ключи для сохранения и извлечения объектов.</p>
<p>Для открытия файла модуль shelve использует функцию <span class="b">open()</span>:</p>
<pre class="brush:py;">open(путь_к_файлу[, flag="c"[, protocol=None[, writeback=False]]])</pre>
<p>Где параметр flag может принимать значения:</p>
<ul>
<li><p><span class="b">c</span>: файл открывается для чтения и записи (значение по умолчанию). Если файл не существует, то он создается.</p></li>
<li><p><span class="b">r</span>: файл открывается только для чтения.</p></li>
<li><p><span class="b">w</span>: файл открывается для записи.</p></li>
<li><p><span class="b">n</span>: файл открывается для записи Если файл не существует, то он создается. Если он существует, то он перезаписывается</p></li>
</ul>
<p>Для закрытия подключения к файлу вызывается метод <span class="b">close()</span>:</p>
<pre class="brush:py;">
import shelve
d = shelve.open(filename)
d.close()
</pre>
<p>Либо можно открывать файл с помощью оператора <span class="b">with</span>. Сохраним и считаем в файл несколько объектов:</p>
<pre class="brush:py;">
import shelve
...
<p>Запись данных предполагает установку значения для определенного ключа:</p>
<pre class="brush:py;">
states["London"] = "Great Britain"
</pre>
<p>А чтение из файла эквивалентно получению значения по ключу:</p>
<pre class="brush:py;">
print(states["London"])
</pre>
<p>В качестве ключей используются строковые значения.</p>
<p>При чтении данных, если запрашиваемый ключ отсутствует, то генерируется исключение. В этом случае перед получением мы можем проверять на наличие ключа с помощью 
оператора <span class="b">in</span>:</p>
<pre class="brush:py;">
with shelve.open(FILENAME) as states:
    key = "Brussels"
    if key in states:
...
<p>Также мы можем использовать метод <span class="b">get()</span>. Первый параметр метода - ключ, по которому следует получить значение, а второй - значение по умолчанию, которое возвращается, если ключ не найден.</p>
<pre class="brush:py;">
with shelve.open(FILENAME) as states:
    state = states.get("Brussels", "Undefined")
    print(state)
</pre>
<p>Используя цикл for, можно перебрать все значения из файла:</p>
<pre class="brush:py;">
with shelve.open(FILENAME) as states:
    for key in states:
        print(key," - ", states[key])
</pre>
<p>Метод <span class="b">keys()</span> возвращает все ключи из файла, а метод <span class="b">values()</span> - все значения:</p>
<pre class="brush:py;">
with shelve.open(FILENAME) as states:
...
<p>Еще один метод <span class="b">items()</span> возвращает набор кортежей. Каждый кортеж содержит ключ и значение.</p>
<pre class="brush:py;">
with shelve.open(FILENAME) as states:
...
<p>Для изменения данных достаточно присвоить по ключу новое значение, а для добавления данных - определить новый ключ:</p>
<pre class="brush:py;">
import shelve
...
<p>Для удаления с одновременным получением можно использовать функцию <span class="b">pop()</span>, в которую передается ключ элемента и значение по умолчанию, если ключ не найден:</p>
<pre class="brush:py;">
with shelve.open(FILENAME) as states:
...
<p>Также для удаления может применяться оператор <span class="b">del</span>:</p>
<pre class="brush:py;">
with shelve.open(FILENAME) as states:

    del states["Madrid"] 	# удаляем объект с ключом Madrid
</pre>
<p>Для удаления всех элементов можно использовать метод <span class="b">clear()</span>:</p>
<pre class="brush:py;">
with shelve.open(FILENAME) as states:
