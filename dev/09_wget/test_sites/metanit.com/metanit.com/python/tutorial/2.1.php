...
<html >
<head>
<title>Python | Введение в написание программ</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в написание программ на Python, основы синтаксиса, регистрозависимость, функции print и input, ввод данных и вывод на консоль, комментарии и shebang">
...
<h1>Основы Python</h1><h2>Введение в написание программ</h2><div class="date">Последнее обновление: 07.01.2022</div>
...
<p>Однако стоит учитывать, что некоторые конструкции языка могут состоять из нескольких строк. Например, условная конструкция <span class="b">if</span>:</p>
<pre class="brush:py;">
if 1 &lt; 2:
    print("Hello")
</pre>
<p>В данном случае если 1 меньше 2, то выводится строка "Hello". И здесь уже должен быть отступ, так как инструкция print("Hello") используется не сама по себе, а 
как часть условной конструкции if. Причем отступ, согласно <a href="https://www.python.org/dev/peps/pep-0008/" rel="nofollow">руководству по оформлению кода</a>, желательно делать из такого количество пробелов, которое кратно 4 (то есть 4, 8, 16 и т.д.) Хотя если отступов будет не 4, а 5, то программа также будет работать.</p>
...
<p>Python - регистрозависимый язык, поэтому выражения <code>print</code> и <code>Print</code> или <code>PRINT</code> представляют разные выражения. 
И если вместо метода print для вывода на консоль мы попробуем использовать метод Print:</p>
<pre class="brush:py;">Print("Hello World")</pre>
<p>то у нас ничего не получится.</p>
<h3>Комментарии</h3>
<p>Для отметки, что делает тот или иной участок кода, применяются комментарии. При трансляции и выполнении программы интерпретатор игнорирует комментарии, 
поэтому они не оказывают никакого влияния на работу программы. Комментарии в Python бывают блочные и строчные.</p>
<p>Строчные коментарии предваряются знаком решетки - <span class="b">#</span>. Они могут располагаться на отдельной строке:</p>
<pre class="brush:py;">
# Вывод на консоль 
# сообщения Hello World
print("Hello World")
</pre>
<p>Любой набор символов после знака # представляет комментарий. То есть в примее выше первые две строки кода являются комментариями.</p>
...
<p>В блочных коментариях до и после текста комментария ставятся три одинарные кавычки: <code>'''текст комментария'''</code>. Например:</p>
<pre class="brush:py;">
''' 
    Вывод на консоль
    сообщения Hello World
...
<p>Основной функцией для вывода информации на консоль является функция <span class="b">print()</span>. В качестве аргумента в эту функцию передается строка, которую мы хотим вывести:</p>
<pre class="brush:py;">print("Hello Python")</pre>
...
<p>Если функция print отвечает за вывод, то функция <span class="b">input</span> отвечает за ввод информации. В качестве необязательного параметра 
эта функция принимает приглашение к вводу и возвращает введенную строку, которую мы можем сохранить в переменную:</p>
<pre class="brush:py;">
