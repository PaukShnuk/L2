...
<html >
<head>
<title>Python | Кнопки</title>
<meta charset="utf-8" />
<meta name="description" content="Создание кнопок на Python с помощью модуля tkinter, обработка нажатия кнопки">
...
<h2>Кнопки</h2><div class="date">Последнее обновление: 13.05.2017</div>
...
<p>Тулкит tkinter содержит набор компонентов или виджетов, одним из которых является кнопка. Добавим кнопку в окно:</p>
<pre class="brush:py;">
from tkinter import *
...
<p>Для создания кнопки используется конструктор <span class="b">Button()</span>. В этом конструкторе с помощью параметра text можно установить текст кнопки.</p>
<p>Чтобы сделать элемент видимым, у него вызывается метод <span class="b">pack()</span>. В итоге вверху окна будет красоваться кнопка:</p>
<img src="./pics/9.2.png" alt="Кнопка в tkinter в Python" />
<p>Каждый виджет, в том числе кнопка, имеет ряд атрибутов, которые влияют на ее визуализацию и которые мы можем настроить через конструктор:</p>
<pre class="brush:py;">
from tkinter import *
...
<p>Параметры pady, padx, font принимают числовое значение, а параметры background и foreground получают шестнадцатеричное значение цвета. 
Параметр font содержит определение шрифта.</p>
<img src="./pics/9.4.png" alt="Настройка кнопки в tkinter и Python" />
<p>Всего же конструктор Button может принимать следующие параметры:</p>
<pre class="brush:py;">Button (master, options)</pre>
<p>Параметр master представляет ссылку на родительский контейнер. В случае выше это могло бы быть само графическое окно, и мы могли написать:</p>
<pre class="brush:py;">
root = Tk()
root.title("GUI на Python")
root.geometry("300x250")
...
<p>Однако если в коде создается одно окно, то кнопка и любой другой элемент уже по умолчанию размещаются в этом окне. Поэтому первый параметр мы можем опустить, как 
в примерах выше. Если бы у нас в коде создавалось бы несколько окон, тогда мы могли бы передать в конструктор Button ссылку на нужное окно.</p>
<p>Второй параметр options представляет набор на самом деле набор параметров, которые мы можем установить по их имени:</p>
<ul>
<li><p><span class="b">activebackground</span>: цвет кнопки, когда она находится в нажатом состоянии</p></li>
<li><p><span class="b">activeforeground</span>: цвет текста кнопки, когда она в нажатом состоянии</p></li>
<li><p><span class="b">bd</span>: толщина границы (по умолчанию 2)</p></li>
<li><p><span class="b">bg/background</span>: фоновый цвет кнопки</p></li>
<li><p><span class="b">fg/foreground</span>: цвет текста кнопки</p></li>
<li><p><span class="b">font</span>: шрифт текста, например, <code>font="Arial 14"</code> - шрифт Arial высотой 14px, 
или <code>font=("Verdana", 13, "bold")</code> - шрифт Verdana высотой 13px с выделением жирным</p></li>
<li><p><span class="b">height</span>: высота кнопки</p></li>
<li><p><span class="b">highlightcolor</span>: цвет кнопки, когда она в фокусе</p></li>
<li><p><span class="b">image</span>: изображение на кнопке</p></li>
...
<li><p><span class="b">padx</span>: отступ от границ кнопки до ее текста справа и слева</p></li>
<li><p><span class="b">pady</span>: отступ от границ кнопки до ее текста сверху и снизу</p></li>
<li><p><span class="b">relief</span>: определяет тип границы, может принимать значения SUNKEN, RAISED, GROOVE, RIDGE</p></li>
<li><p><span class="b">state</span>: устанавливает состояние кнопки, может принимать значения DISABLED, ACTIVE, NORMAL (по умолчанию)</p></li>
<li><p><span class="b">text</span>: устанавливает текст кнопки</p></li>
...
<li><p><span class="b">width</span>: ширина кнопки</p></li>
<li><p><span class="b">wraplength</span>: при положительном значении строки текста будут переносится для вмещения в пространство кнопки</p></li>
</ul>
<h3>Обработка нажатия на кнопку</h3>
<p>Для обработки нажатия на кнопку необходимо установить в конструкторе параметр <code>command</code>, присвоив ему ссылку на функцию, которая будет 
срабатывать при нажатии:</p>
<pre class="brush:py;">
from tkinter import *
...
<p>Здесь в качестве обработчика нажатия устанавливается функция click_button. В этой функции изменяется глобальная переменная clicks, которая хранит число кликов, 
и ее значение выводится в заголовок окна. Таким образом, при каждом нажатии кнопки будет срабатывать функция click_button, и количество кликов будет увеличиваться:</p>
<img src="./pics/9.3.png" alt="Настройка кнопки в tkinter и Python" />
