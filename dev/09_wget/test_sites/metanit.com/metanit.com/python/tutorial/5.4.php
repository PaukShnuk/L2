...
<html >
<head>
<title>Python | Программа подсчета слов</title>
<meta charset="utf-8" />
<meta name="description" content="Программа подсчета слов в Python, использование методов строк на примере">
...
<h2>Программа подсчета слов</h2><div class="date">Последнее обновление: 05.02.2022</div>
...
<p>Рассмотрим работу со строками на небольшом примере, который будет представлять программу подсчета слов.</p>
<p>Пусть весь код программы будет выглядеть следующим образом:</p>
<pre class="brush:py;">
...
<p>Здесь в функции <code>get_words()</code> производится начальная сегментация текста на слова. Пи этом все пунктуационные знаки удаляются, а переводы стоки заменяется на пробелы. 
Затем происходит разбитие текста на слова. В качестве разделителя по умолчанию применяется пробел.</p>
<p>Далее в функции <code>get_words_dict()</code> получаем словарь из слов, где ключ - это уникальное слово, а значение - количество вхождений данного слова в тексте.</p>
<p>В функции main осуществляется ввод пути к файлу и вызов выше определенных функций, а также вывод все статистики.</p>
...
<div class="nav"><p><a href="./5.3.php">Назад</a><a href="./">Содержание</a><a href="./2.13.php">Вперед</a></p></div></div>
<div class="socBlock">
