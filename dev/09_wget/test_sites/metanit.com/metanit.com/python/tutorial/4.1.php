...
<html >
<head>
<title>Python | Работа с файлами</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с файлами в Python, открытие и закрытие файлов, методы open и close, конструкция with...as, режимы открытия текстовых и бинарных файлов">
...
<h1>Работа с файлами</h1><h2>Открытие и закрытие файлов</h2><div class="date">Последнее обновление: 29.04.2017</div>
...
<p>Python поддерживает множество различных типов файлов, но условно их можно разделить на два виде: текстовые и бинарные. Текстовые файлы  - это к примеру 
...
В зависимости от типа файла работа с ним может немного отличаться.</p>
<p>При работе с файлами необходимо соблюдать некоторую последовательность операций:</p>
<ol>
<li><p>Открытие файла с помощью метода <span class="b">open()</span></p></li>
<li><p>Чтение файла с помощью метода <span class="b">read()</span> или запись в файл посредством метода <span class="b">write()</span></p></li>
<li><p>Закрытие файла методом <span class="b">close()</span></p></li>
</ol>
<h3>Открытие и закрытие файла</h3>
<p>Чтобы начать работу с файлом, его надо открыть с помощью функции <span class="b">open()</span>, которая имеет следующее формальное определение:</p>
<pre class="brush:py;">open(file, mode)</pre>
<p>Первый параметр функции представляет путь к файлу. Путь файла может быть абсолютным, то есть начинаться с буквы диска, например, <span class="ii">C://somedir/somefile.txt</span>. 
Либо можно быть относительным, например, <span class="ii">somedir/somefile.txt</span> - в этом случае поиск файла будет идти относительно 
расположения запущенного скрипта Python.</p>
<p>Второй передаваемый аргумент - <code>mode</code> устанавливает режим открытия файла в зависимости от того, что мы собираемся с ним делать. 
Существует 4 общих режима:</p>
<ul>
<li><p><span class="b">r</span> (Read). Файл открывается для чтения. Если файл не найден, то генерируется исключение FileNotFoundError</p></li>
<li><p><span class="b">w</span> (Write). Файл открывается для записи. Если файл отсутствует, то он создается. Если подобный файл уже есть, то он создается заново, и соответственно 
старые данные в нем стираются.</p></li>
<li><p><span class="b">a</span> (Append). Файл открывается для дозаписи. Если файл отсутствует, то он создается. Если подобный файл уже есть, 
то данные записываются в его конец.</p></li>
<li><p><span class="b">b</span> (Binary). Используется для работы с бинарными файлами. Применяется вместе с другими режимами - w или r.</p></li>
</ul>

<p>После завершения работы с файлом его обязательно нужно закрыть методом <code>close()</code>. Данный метод освободит все связанные с файлом используемые ресурсы.</p>
<p>Например, откроем для записи текстовый файл "hello.txt":</p>
<pre class="brush:py;">
myfile = open("hello.txt", "w")

myfile.close()
</pre>
<p>При открытии файла или в процессе работы с ним мы можем столкнуться с различными исключениями, например, к нему нет доступа и т.д. 
В этом случае программа выпадет в ошибку, а ее выполнение не дойдет до вызова метода close, и соответственно файл не будет закрыт.</p>
<p>В этом случае мы можем обрабатывать исключения:</p>
<pre class="brush:py;">
try:
...
<p>В данном случае вся работа с файлом идет во вложенном блоке try. И если вдруг возникнет какое-либо исключение, то в любом случае в блоке 
finally файл будет закрыт.</p>
<p>Однако есть и более удобная конструкция - конструкция <span class="b">with</span>:</p>
<pre class="brush:py;">
with open(file, mode) as file_obj:
	инструкции
</pre>
<p>Эта конструкция определяет для открытого файла переменную file_obj и выполняет набор инструкций. После их выполнения файл автоматически закрывается. 
Даже если при выполнении инструкций в блоке with возникнут какие-либо исключения, то файл все равно закрывается.</p>
<p>Так, перепишем предыдущий пример:</p>
<pre class="brush:py;">
...
<div class="nav"><p><a href="./2.27.php">Назад</a><a href="./">Содержание</a><a href="4.2.php">Вперед</a></p></div></div>
<div class="socBlock">
