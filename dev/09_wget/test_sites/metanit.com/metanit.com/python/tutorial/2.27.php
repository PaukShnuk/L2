...
<html >
<head>
<title>Python | Установка псевдонимов и паттерн AS</title>
<meta charset="utf-8" />
<meta name="description" content="Установка псевдонимов и паттерн AS в pattern matching и конструкция match..case в языке Python">
...
<h2>Установка псевдонимов и паттерн AS</h2><div class="date">Последнее обновление: 01.02.2022</div>
...
<p>Оператор <span class="b">as</span> позволяет установить псевдоним для значения шаблона или для всего шаблона. Простейший пример:</p>
<pre class="brush:py;">
...
<p>Здесь первый шаблон соответствует трем строкам: "Tom" | "Tomas" | "Tommy". После набора значений идет оператор 
<span class="b">as</span>, после которого указывается псевдоним. И вне зависимости от того, какая именно строка передана, она 
окажется в переменной name.</p>
<p>Псевдоним можно применять как для отдельного значения шаблона, так и для всего шаблона:</p>
<pre class="brush:py;">
def print_person(person):
    match person:
...
<p>Обычно псевдонимы более применимы в каких-то более сложных по структуре данных. Например:</p>
<pre class="brush:py;">
class Person:
...
<p>Здесь функция print_family принимает кортеж, который должен состоять из двух элементов Person. В первом шаблоне определяем 
для первого элемента псевдоним husband, а для второго - псевдоним wife. Затем, используя эти псевдонимы, мы можкем обращаться к их атрибутам.</p>
...
<div class="nav"><p><a href="./2.26.php">Назад</a><a href="./">Содержание</a><a href="./4.1.php">Вперед</a></p></div></div>
<div class="socBlock">
