...
<html >
<head>
<title>Python | Лямбда-выражения</title>
<meta charset="utf-8" />
<meta name="description" content="Лямбда-выражения, их определение и вызов в языке программирования Python, ключевое слово lambda, передача параметров в лямбду и получение результата">
...
<h2>Лямбда-выражения</h2><div class="date">Последнее обновление: 17.01.2022</div>
...
<p>Лямбда-выражения в языке Python представляют небольшие анонимные функции, которые определяются с помощью оператора <span class="b">lambda</span>. 
Формальное определение лямбда-выражения:</p>
<pre class="brush:py;">
...
<p>Здесь лямбда-выражение присваивается переменной message. Это лямбда-выражение не имеет параметров, ничего не возвращает и просто выводит 
строку "hello" на консоль. И через переменную message мы можем вызвать это лямбда-выражение как обычную функцию. Фактически оно аналогично следующей функции:</p>
<pre class="brush:py;">
def message(): 
    print("hello")
</pre>
<p>Если лямбда-выражение имеет параметры, то они определяются после ключевого слова <span class="b">lambda</span>. Если лямбда-выражение возвращает какой-то результат, 
то он указывается после двоеточия. Например, определим лямбда-выражение, которое возвращает квадрат числа:</p>
<pre class="brush:py;">
square = lambda n: n * n
...
<p>В данном случае лямбда-выражение принимает один параметр - n. Справа от двоеточия идет возвращаемое значение - <code>n* n</code>. Это лямбда-выражение 
аналогично следующей функции:</p>
<pre class="brush:py;">def square2(n): return n * n</pre>
...
<p>Хотя лямбда-выражения позволяют немного сократить определения функций, тем не менее они ограничены тем, что они могут выполнять только одну инструкцию. Однако они могут быть 
довольно удобны в тех случаях, когда необходимо использовать функцию для передачи в качестве параметра или 
возвращения в другой функции. Например, передача лямбда-выражения в качестве параметра:</p>
<pre class="brush:py;">
...
<div class="nav"><p><a href="./2.17.php">Назад</a><a href="./">Содержание</a><a href="./2.5.php">Вперед</a></p></div></div>
<div class="socBlock">
