...
<html >
<head>
<title>Python | Замыкания</title>
<meta charset="utf-8" />
<meta name="description" content="Замыкания closure в языке программирования Python, лексическое окружение функции">
...
<h2>Замыкания</h2><div class="date">Последнее обновление: 24.01.2022</div>
...
<p><span class="b">Замыкание</span> (closure) представляет функцию, которыая запоминает свое лексическое окружение даже в том случае, 
...
<p>Здесь функция <code>outer</code> определяет локальную переменную n - это и есть лексическое окружение для внутренней функции:</p>
<p>Внутри функции outer определена внутренняя функция - локальная функция inner, которая обращается к своему лексическому окружению - переменной n - 
увеличивает ее значение на единицу и выводит на консоль:</p>
<pre class="brush:py;">
def inner():      # локальная функция
    nonlocal n
    n += 1        # операции с лексическим окружением
...
<pre class="brush:py;">return inner</pre>
<p>В программе вызываем функцию outer и получаем в переменную <code>fn</code> локальную функцию inner:</p>
<pre class="brush:py;">fn = outer() </pre>
...
тем не менее она запомнила свое лексическое окружение и может к нему обращаться и изменять, что мы увидим по консольному выводу:</p>
<pre class="brush:py;">
fn()    # 6
fn()    # 7
fn()    # 8
...
<p>Вызов функции <code>multiply()</code> возвращает локальную функцию inner:</p>
<pre class="brush:py;">def inner(m): return n * m</pre>
...
<div class="nav"><p><a href="./2.9.php">Назад</a><a href="./">Содержание</a><a href="./7.1.php">Вперед</a></p></div></div>
<div class="socBlock">
