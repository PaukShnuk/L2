...
<html >
<head>
<title>Python | Бинарные файлы</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с бинарными файлами в Python, их открытие, чтение и запись, бинаный режим, модуль pickle, методы pickle.dump и pickle.load">
...
<h2>Бинарные файлы</h2><div class="date">Последнее обновление: 22.06.2017</div>
...
<p>Бинарные файлы в отличие от текстовых хранят информацию в виде набора байт. Для работы с ними в Python необходим встроенный модуль 
<span class="b">pickle</span>. Этот модуль предоставляет два метода:</p>
<ul>
<li><p><span class="b">dump(obj, file)</span>: записывает объект obj в бинарный файл file</p></li>
<li><p><span class="b">load(file)</span>: считывает данные из бинарного файла в объект</p></li>
</ul>
<p>При открытии бинарного файла на чтение или запись также надо учитывать, что нам нужно применять режим "b" в дополнение к режиму записи ("w") или чтения ("r"). 
Допустим, надо надо сохранить два объекта:</p>
<pre class="brush:py;">
import pickle
...
<p>С помощью функции dump последовательно записываются два объекта. Поэтому при чтении файла также последовательно посредством функции load мы можем считать 
эти объекты. Консольный вывод программы:</p>
<div class="console">
...
<p>Подобным образом мы можем сохранять и извлекать из файла наборы объектов:</p>
<pre class="brush:py;">
import pickle
...
<p>В зависимости от того, какой объект мы записывали функцией dump, тот же объект будет возвращен функцией load при считывании файла.</p>
...
<div class="nav"><p><a href="4.3.php">Назад</a><a href="./">Содержание</a><a href="4.6.php">Вперед</a></p></div></div>
<div class="socBlock">
