...
<html >
<head>
<title>Python | Работа с датами и временем</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с датами и временем в Python, модуль datetime, классы date, time, определение дат, их локализация">
...
<h1>Работа с датами и временем</h1><h2>Модуль datetime</h2><div class="date">Последнее обновление: 05.05.2017</div>
...
<p>Основной функционал для работы с датами и временем сосредоточен в модуле <span class="b">datetime</span> в виде следующих классов:</p>
<ul>
<li><p><span class="b">date</span></p></li>
<li><p><span class="b">time</span></p></li>
<li><p><span class="b">datetime</span></p></li>
</ul>
<h3>Класс date</h3>
<p>Для работы с датами воспользуемся классом <span class="b">date</span>, который определен в модуле datetime. Для создания объекта date мы можем 
использовать конструктор date, который последовательно принимает три параметра: год, месяц и день.</p>
<pre class="brush:py;">date(year, month, day)</pre>
<p>Например, создадим какую-либо дату:</p>
<pre class="brush:py;">
import datetime
...
<p>Если необходимо получить текущую дату, то можно воспользоваться методом <span class="b">today()</span>:</p>
<pre class="brush:py;">
...
<p>С помощью свойств day, month, year можно получить соответственно день, месяц и год</p>
<h3>Класс time</h3>
<p>За работу с временем отвечает класс <span class="b">time</span>. Используя его конструктор, можно создать объект времени:</p>
<pre class="brush:py;">time([hour] [, min] [, sec] [, microsec])</pre>
<p>Конструктор последовательно принимает часы, минуты, секунды и микросекунды. Все параметры необязательные, и если мы какой-то параметр не передадим, то 
соответствующее значение будет инициализироваться нулем.</p>
<pre class="brush:py;">
...
<h3>Класс datetime</h3>
<p>Класс <span class="b">datetime</span> из одноименного модуля объединяет возможности работы с датой и временем. Для создания объекта datetime 
можно использовать следующий конструктор:</p>
<pre class="brush:py;">datetime(year, month, day [, hour] [, min] [, sec] [, microsec])</pre>
<p>Первые три параметра, представляющие год, месяц и день, являются обязательными. Остальные необязательные, и если мы не укажем для них значения, то по умолчанию они инициализируются нулем.</p>
<pre class="brush:py;">
...
<p>Для получения текущих даты и времени можно вызвать метод <span class="b">now()</span>:</p>
<pre class="brush:py;">
from datetime import datetime
...
<p>С помощью свойств day, month, year, hour, minute, second можно получить отдельные значения даты и времени. А через методы <code>date()</code> и 
<code>time()</code> можно получить отдельно дату и время соответственно.</p>
<h3>Преобразование из строки в дату</h3>
<p>Из функциональности класса datetime следует отметить метод <span class="b">strptime()</span>, который позволяет распарсить строку и преобразовать ее в дату. Этот метод принимает два параметра:</p>
<pre class="brush:py;">strptime(str, format)</pre>
<p>Первый параметр str представляет строковое определение даты и времени, а второй параметр - формат, который определяет, как различные части даты и времени 
расположены в этой строке.</p>
<p>Для определения формата мы можем использовать следующие коды:</p>
<ul>
<li><p><span class="b">%d</span>: день месяца в виде числа</p></li>
<li><p><span class="b">%m</span>: порядковый номер месяца</p></li>
<li><p><span class="b">%y</span>: год в виде 2-х чисел</p></li>
<li><p><span class="b">%Y</span>: год в виде 4-х чисел</p></li>
<li><p><span class="b">%H</span>: час в 24-х часовом формате</p></li>
<li><p><span class="b">%M</span>: минута</p></li>
<li><p><span class="b">%S</span>: секунда</p></li>
</ul>
<p>Применим различные форматы:</p>
<pre class="brush:py;">
from datetime import datetime
...
<div class="nav"><p><a href="4.5.php">Назад</a><a href="./">Содержание</a><a href="8.2.php">Вперед</a></p></div></div>
<div class="socBlock">
