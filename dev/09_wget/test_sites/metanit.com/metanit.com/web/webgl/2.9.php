...
<html >
<head>
<title>Рисование линий в WebGL</title>
<meta charset="utf-8" />
<meta name="description" content="Рисование линий в WebGL с помощью примитивов gl.LINE_STRIP, gl.LINE_LOOP и gl.LINES">
...
<h2>Рисование линий</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Рисование линий во многом аналогично созданию треугольников, только теперь каждый примитив создается из двух вершин, что облегчает создание. 
Создадим набор линий:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="pics/2.6.png" alt="Рисование линий в WebGL" />
<p>Набор индексов <code>indices = [0, 1, 1, 2, 2, 3, 3, 4];</code> указывает последовательность вершин, из которых составляется четыре линии: 
...
визуально находятся вместе.</p>
<p>Другой способ создания линий представляют примитивы <code>gl.LINE_LOOP</code> и <code>gl.LINE_STRIP</code>. В отличие от 
<code>gl.LINES</code> здесь можно обойтись меньшим количеством вершин и индексов, поскольку в <code>gl.LINE_LOOP</code> линиии последовательно 
соединяются, образуя в конце замкнутый контур. В <code>gl.LINE_STRIP</code> вершины также последовательно соединяются линиями, только в конце не происходит 
соединения последней вершины с первой.</p>
<p>Возьмем предыдущий пример и изменим в нем всего две строки. Во-первых, изменим в функции <code>initBuffers</code> набор индексов 
на следующий: <code>indices = [0, 1, 2, 3, 4];</code>. 
Во-вторых, изменим в функции <code>draw</code> способ отрисовки примитивов на следующий: <code>gl.drawElements(gl.LINE_STRIP, indexBuffer.numberOfItems, gl.UNSIGNED_SHORT,0);</code></p>
<p>В итоге у нас должен получиться тот же эффект, что и на предыдущем примере. Если бы вместо gl.LINE_STRIP мы бы использовали 
gl.LINE_LOOP, то получили бы замкнутый контур:</p>
