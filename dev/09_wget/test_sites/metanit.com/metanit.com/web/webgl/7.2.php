...
<html >
<head>
<title>Работа с координатами текстуры</title>
<meta charset="utf-8" />
<meta name="description" content="Использование координат текстуры в WebGL, создание семплера и применение функции texture2D для получения цветов текстуры">
...
<h2>Работа с координатами текстуры</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Используя координаты текстуры мы моем более точно проецировать ее на поверхность объекта. Итак, создадим следующую страницу:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Данная веб-страничка будет иметь тот же эффект, что и из предыдущего параграфа, только в данном случае сопоставление текстуры с объектом будет идти 
по координатам. В конце функции инициализации буферов у нас есть такой код:</p>
<pre class="brush:js;">
// Координаты текстуры
var textureCoords = [
			0.0, 0.0,
...
<p>Каждая объявленная в массиве textureCoords точка на текстуре соответствует вершине двухмерного объекта:</p>
<img src="pics/7.4.png" />
<p>А создание буфера координат текстуры аналогично созданию буфера вершин.</p>
<p>Функция <code>setTextures()</code>, которая выполняет загрузку и настройку текстуры осталась та же, что и в прошлом параграфе за тем исключением, 
что теперь из нее вынесены методы <code>setupWebGL()</code> и <code>draw()</code> в функцию анимации.</p>
<p>После создания буфера координат текстуры нам надо его содержание передать в шейдер. И чтобы сделать это, нам, во-первых, надо создать атрибут 
(это делается в функции настройки шейдеров initShaders): <code>shaderProgram.vertexTextureAttribute = gl.getAttribLocation(shaderProgram, "aVertexTextureCoords");</code></p>
<p>Далее в функции отрисовки draw подобно тому, как мы создаем указатель на атрибут для вершинного буфера, то же самое проделываем с буфером координат 
текстуры:</p>
<pre class="brush:js;">
gl.bindBuffer(gl.ARRAY_BUFFER, textureCoordsBuffer);
  gl.vertexAttribPointer(shaderProgram.vertexTextureAttribute,
...
<p>То есть также создаем указатель на атрибут и подключаем его и далее делаем активной текстуру (gl.TEXTURE0) и связываем ее. WebGL поддерживает 
работу с несколькими текстурами одновременно, а использование gl.TEXTURE0 отсылает нас к первой текстуре.</p>
<p>И в завершении нам надо получить в шейдере атрибут aVertexTextureCoords и как-то его использовать. Для этого в вершинном шейдере заводим 
переменную для передачи значений текстуры во фрагментный шейдер, а данные она берет как раз из атрибута aVertexTextureCoords:</p>
<pre class="brush:js;">
attribute vec3 aVertexPosition;
attribute vec2 aVertexTextureCoords;
varying vec2 vTextureCoords;
...
<p>И во фрагментном шейдере так же проходим семплером: <code>gl_FragColor = texture2D(uSampler, vTextureCoords);</code></p>
