...
<html >
<head>
<title>Шейдеры в WebGL</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в шейдеры в WebGL, определение вершинного и фрагментного шейдера, язык GLSL (OpenGL ES Shading Language)">
...
<h1>Шейдеры</h1>
<h2>Введение в шейдеры</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Шейдеры являются одним из базовых элементов любой программы на WebGL. Без шейдеров сложно что-то сделать. На вход в графический процессов передается 
...
в итоге видим какие-нибудь трехмерные модели.</p>
<p>Если вы посмотрите на ранее использовавшиеся шейдеры (все они пока однотипны), то вы увидите, что в нем используется особый язык 
с Си-подобным синтаксисом. Например, код вершинного шейдера:</p>
<pre class="brush:js;">
...
<p>Этот язык называется <b>GLSL</b>, а если точнее, то язык шейдеров <b>OpenGL ES Shading Language</b>. В свою очередь GLSL основан на C++. 
...
более подробно ознакомиться с данным языком.</p>
<p>Этот язык используется в двух частях программы WebGL - в вершинном шейдере и фрагментном шейдере. Для написания кода на OpenGL ES Shading 
Language используются выражения языка JavaScript.</p>
<h3>Вершинный шейдер</h3>
<p>Практически в самом начале работы конвейера WebGL буфер вершин передается в вершинный шейдер. Вершинный шейдер проходит по всем переданным вершинам 
...
обработки дальше. До сих пор я использовал простейшее определение шейдера:</p>
<pre class="brush:js;">
attribute vec3 aVertexPosition;
void main(void) {
  gl_Position = vec4(aVertexPosition, 1.0);
...
<p>Атрибут <b>aVertexPosition</b>, имеющий тип <code>vec3</code>, как раз и передает координаты вершины из буфера вершин. И так как каждая вершина представлена 
тремя координатами x, y, z, то для передачи вершины используется трехмерный вектор vec3.</p>
<p>Как и каждая программа на С/С++, шейдер имеет основную процедуру <code>main</code>, в которой происходит генерация окончательных координат вершины, 
только уже в виде четырехмерного вектора. А <code>gl_Position</code> по сути это и есть преобразованные координаты вершины. 
В данном случае у нас никаких преобразований и трансформаций не производится, и финальная вершина будет иметь те же координаты, что и переданная в шейдер.</p>
<h3>Фрагментный шейдер</h3>
<p>Фрагментный шейдер уже наполняет набор примитивов цветом, раскрашивает их. Фрагменный шейдер никак не влияет на координаты вершины, он влияет только 
на цветовую составляющую, преобразуя вершины уже в пиксели или фрагменты.</p>
<p>Для простоты ранее также были использованы простейшие определения фрагментных шейдеров, например:</p>
<pre class="brush:js;">
void main(void) {
  gl_FragColor = vec4(1.0, 1.0, 1.0, 1.0);
}
</pre>
<p>Здесь также основная программа main, которая устанавливает финальный цвет для пикселя, представленного вершиной. Так как 
...
<code>vec4</code>. В данном случае каждый пиксель примитивов у нас окрашивается в белый цвет. А если бы мы захотели покрасить примитивы в 
зеленый цвет, то могли бы задать следующее определение цвета: <code>gl_FragColor = vec4(0.0, 1.0, 0.0, 1.0);</code></p>
