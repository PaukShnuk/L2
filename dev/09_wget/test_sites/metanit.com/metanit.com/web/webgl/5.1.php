...
<html >
<head>
<title>Матрицы и создание 3D</title>
<meta charset="utf-8" />
<meta name="description" content="Создание трехмерных объектов в WebGL, особенности отрисовки, матрицы вида, модели, проекции">
...
<h1>Матрицы и создание 3D</h1>
<h2>Первый 3D-объект</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Прошло несколько глав с самого начала, и возможно, у кого-то появятся вполне справедливые вопросы: а где же 3D? Все предыдущие примеры фактически 
...
Поэтому наконец создадим куб.</p>
<p>По сути куб формируют 8 точек, связанных между собою линиями. А с линиями мы уже познакомились в предыдущих главах. Итак, полный код будет следующим:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Итак, у нас есть 8 точек, которые соединены между собой линиями. Теперь мы можем запустить и увидим наш куб:</p>
<img src="pics/5.1.png" alt="Куб в WebGL" />
<p>Возможно, вы спросите меня, разве это куб? Да, это куб. Но так как мы смотрим прямо на него, то визуально мы будем видеть обычный прямоугольник. 
Однако повернув или изменив точку обзора, мы можем уже увидеть трехмерную проекцию куба. И для этого нам понадобятся матрицы.</p>
<h3>Введение в матрицы</h3>
<h4>Model-View Matrix</h4>
<p>Данная матрица, которую условно можно назвать <b>матрицей модели</b>, объединяет мировую матрицу и матрицу вида. Мировая матрица переводит 
...
видимое пространство камеры.</p>
<p>Камера в данном случае фактически и есть наш взгляд на трехмерную сцену. И поскольку, условно говоря, камера направлена определенным образом, мы 
видим не куб, а просто прямоугольник. Но стоит нам переместить камеру, и мы увидим куб.</p>
<h4>Projection Matrix</h4>
<p>Матрица проекции преобразует трехмерную систему координат объекта в двухмерные для отображения их на экране. Матрица проекции бывает двух типов: 
ортогональная и перспективная.</p>
<img src="pics/5.2.png" />
<p>При ортогональной проекции вне зависимости от значения координаты Z все объекты будут отрисовываться как есть с их текущими шириной и высотой. 
...
Что будет создавать некоторую иллюзию реалистичности сцены.</p>
<p>Познакомившись с матрицами, теперь мы можем переходить к их применению в приложении.</p>
...
<div class="nav"><p><a href="4.1.php">Назад</a><a href="./">Содержание</a><a href="5.2.php">Вперед</a></p></div></div>
<div class="socBlock">
