...
<html >
<head>
<title>Установка Viewport в WebGL</title>
<meta charset="utf-8" />
<meta name="description" content="Установка Viewport в WebGL для задания области отрисовки трехмерной сцены">
...
<h2>Установка Viewport</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Отрисовка примитивов с помощью методов <code>gl.drawElements</code> или <code>gl.drawArrays</code> еще не гарантирует, что эти примитивы 
...
<code>canvas</code> еще недостаточно, чтобы произвести отрисовку.</p>
<p>Для установки области рисования нам надо настроить <code>viewport</code> контекста WebGL. Во всех ранее использованных примерах настройка 
viewport была однотипна: сначала мы получали размеры области элемента canvas, которой представляет собой полотно рисования:</p>
<pre class="brush:js;">
if(gl){
...
<p>Свойства <code>gl.viewportWidth</code> и <code>viewportHeight</code> позволяли настроить ширину и высоту области рисования. Затем в функции отрисовки 
<code>draw</code> перед рисованием примитивов происходила установка области рисования: <code>gl.viewport(0, 0, gl.viewportWidth, gl.viewportHeight);</code>. 
Таким образом, система знала, какую часть элемента canvas надо использовать для рисования, а <code>gl.viewport</code> указывает на левый нижний 
угол этой области, а также ее ширину и высоту</p>
<p>Это распространенный способ установки viewport. Однако, это не значит, что надо обязательно делать так и никак иначе. Мы вполне можем задать 
другую область рисования: <code>gl.viewport(150, 200, 50, 50);</code>:</p>
<img src="pics/2.8.png" alt="Область viewport в WebGL"/>
<p>В этом случае вся отрисовка будет идти на указанной нами области.</p>
