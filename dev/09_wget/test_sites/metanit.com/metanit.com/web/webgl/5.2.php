...
<html >
<head>
<title>Использование матриц glMatrix для создания 3D</title>
<meta charset="utf-8" />
<meta name="description" content="Использование библиотеки glMatrix для создания трехмерных объектов в WebGL, создание и применение матриц к координатам, вращение и перемещение 3D-объектов">
...
<h2>Использование матриц glMatrix для создания 3D</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Поскольку javascript не имеет встроенных средств для работы с матрицами и векторами, то воспользуемся сторонними библиотеками. 
Одной из наиболее используемых подобных библиотек является <b>glMatrix</b>. Ее можно найти на официальном сайте разработчика 
по следующему адресу: <a href="http://glmatrix.net/" rel="nofollow">http://glmatrix.net/</a>. Она относительно небольшая - минимизированная версия 
весит около 27 кБ, полная - 105 кБ.</p>
<p>Это не единственная библиотека javascript, которую можно использовать в WebGL для работы с матрицами. Есть и другие, например, Sylvester, 
...
Я же в данном случае буду ориентироваться на последнюю версию glMatrix 2.0</p>
<p>Итак, все координаты и индексы у нас будут такими же, как и в прошлом примере, только теперь мы применим матрицы:</p>
<pre class="brush:js;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>И если мы теперь запустим веб-страничку, то результат уже будет другим:</p>
<img src="pics/5.3.png" />
<p>Теперь разберем узловые моменты кода. Первым делом, конечно, подключаем библиотеку glMatrix, в данном случае я использовал минимизированную версию.</p>
<p>Сначала определяем матрицы модели и проекции:</p>
<pre class="brush:js;">
var mvMatrix = mat4.create(); // матрица модели
var pMatrix = mat4.create(); // матрица проекции
</pre>
<p>Метод <code>mat4.create()</code> как раз представляет метод библиотеки glMatrix, используемый для создания матрицы 4х4.</p>
<p>В функции установки шейдеров initShaders() создаем две матрицы - по одной для матрицы модели и проекции. Эти матрицы будут передаваться в вершинный 
шейдер и там применяться к координатам объекта:</p>
<pre class="brush:js;">
shaderProgram.MVMatrix = gl.getUniformLocation(shaderProgram, "uMVMatrix");
shaderProgram.ProjMatrix = gl.getUniformLocation(shaderProgram, "uPMatrix");
</pre>
<p>Матрицы определяются как свойства в объекте shaderProgram с помощью метода gl.getUniformLocation(), который одинаков для обеих матриц.</p>
<p>Затем в отличие от предыдущих примеров мы добавляем две новых функции: setupWebGL() и setMatrixUniforms(), которые затем вызываем в основной функции программы.</p>
<p>Функция setupWebGL() содержит код по настройке трехмерной сцены, например, установку матриц и т.д. Вначале устанавливается матрица проекции: 
<code>mat4.perspective(pMatrix, 1.04, gl.viewportWidth / gl.viewportHeight, 0.1, 100.0);</code>. Первый параметр функции - это и есть выше созданная матрица проекции pMatrix, 
которая затем будет передаваться в шейдер.</p>
<p>Второй параметр - <code>1.04</code> - это угол обзора в радианах. Третий параметр <code>gl.viewportWidth / gl.viewportHeight</code> задает 
аспектное соотношение ширины к длине.</p>
<p>Четвертый и пятый параметр задают размеры видимой области - ближайшую и самую дальнюю ее точку.</p>
<p>Далее устанавливаем матрицу модели. Сначала для нее устанавливаем матрицу идентичности: <code>mat4.identity(mvMatrix);</code>. А затем применяем 
к ней перемещение и вращение:</p>
<pre class="brush:js;">
mat4.translate(mvMatrix,mvMatrix,[0, 0, -2.0]);
mat4.rotate(mvMatrix,mvMatrix, 1.9, [0, 1, 0]);	
</pre>
<p>Метод <code>mat4.translate</code> принимает в качестве первого параметра выходную матрицу, а в качестве второго - входную. Так как мы преобразуем одну матрицу, то 
...
на 2 единицы по оси Z в сторону от нас.</p>
<p>Метод <code>mat4.rotate</code> также в качестве первого и второго параметра принимает выходную и входную матрицу. Третьим параметром идет угол 
в радианах, на который выполняется вращение, а четвертый параметр - вектор, указывающий вдоль какой оси выполнять вращение. 
Поскольку в данном случае для оси Y указана единичка, а для других нули, то вращение будет идти только по оси Y.</p>
<p>И, наконец, метод <code>setMatrixUniforms()</code> выполняет передачу матриц в вершинный шейдер:</p>
<pre class="brush:js;">
gl.uniformMatrix4fv(shaderProgram.ProjMatrix,false, pMatrix);
gl.uniformMatrix4fv(shaderProgram.MVMatrix, false, mvMatrix);  
</pre>
<p>То есть фактически мы связываем матрицу shaderProgram.ProjMatrix с матрицей проекции pMatrix, а матрицу shaderProgram.MVMatrix с mvMatrix. 
И после этого значения матриц попадут в вершинный шейдер, где будут применены для определения конечных координат вершин:</p>
<pre class="brush:js;">
attribute vec3 aVertexPosition;
uniform mat4 uMVMatrix;
uniform mat4 uPMatrix;
...
<p>Обратите внимание на порядок преобразований:  матрица проекции умножается на матрицу модели, а не наоборот, и потом все умножается на вектор координат вершин.</p>
