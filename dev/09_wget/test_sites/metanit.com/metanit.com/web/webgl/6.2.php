...
<meta charset="utf-8" />
<meta name="description" content="Обработка пользовательского ввода в WebGL, реакция программы на нажатия клавиш, вращение и перемещение объекта клавиатурой">
...
<h2>Обработка пользовательского ввода</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Обработка пользовательского ввода предполагает обработку событий клавиатуры и мыши и не сильно отличается от общей модели, что используется в 
javascript, например, при создании игр с использованием элемента canvas.</p>
<p>Создадим программу, которая будет по нажатию клавиатуры перемещать куб вперед, назад, а также вращать его:</p>
<pre class="brush:js;">
...
<p>Вначале для фиксации перемещения задаем две переменных:</p>
<pre class="brush:js;">
var angle = 2.0;//	угол поворота в радианах
var zTranslation = -2.0; // смещение по оси Z
</pre>
<p>Дальше если у нас задан контекст WebGL, мы регистрируем обработчик события нажатия клавиши: <code>document.addEventListener('keydown', handleKeyDown, false);</code>. 
Соответственно добавляем в конце функцию обработчика:</p>
<pre class="brush:js;">
function handleKeyDown(e){
	switch(e.keyCode)
	{	
...
<p>С помощью свойства <code>e.keyCode</code> мы можем узнать код нажатой клавиши и в зависимости от этого выполнить определенные действия. 
