...
<html >
<head>
<title>gl.TRIANGLES</title>
<meta charset="utf-8" />
<meta name="description" content="Отрисовка треугольников в WebGL с помощью примитива gl.TRIANGLES">
...
<h2>gl.TRIANGLES</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Несмотря на то, что треугольник в принципе обычная фигура на плоскости, однако всю трехмерную фигуру мы можем представить как набор обычных треугольников.</p>
<p>Для каждого треугольника в буфере вершин определяется по три вершины. При этом, если нам надо отрисовать несколько треугольников, то вершины одного 
треугольника не используются для другого.</p>
<p>Ранее мы уже рассматривали пример отрисовки треугольника (например, в первой главе), теперь несколько модифицируем пример:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Результатом будет следующий вывод:</p>
<img src="pics/2.5.png" alt="gl.TRIANGLES" />
<p>В данном случае у нас есть пять вершин и шесть индексов. Поскольку построение треугольника идет по индексам через метод 
<code>gl.drawElements</code>, можно определить пять вершин. В данном случае главное, чтобы на каждый треугольник приходилось по три индекса,
которые указывают на три разных вершины.</p>
