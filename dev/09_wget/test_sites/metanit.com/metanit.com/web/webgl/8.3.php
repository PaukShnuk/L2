...
<html >
<head>
<title>Модель отражения Фонга. Код JavaScript</title>
<meta charset="utf-8" />
<meta name="description" content="Создание освещенного объекта по модели Фонга в WebGL, использование кода javascript для установки и передачи параметров в шейдеры">
...
<h2>Модель отражения Фонга. Код JavaScript</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>По сути вся логика освещения уже написана в шейдерах. Нам остается лишь установить и передать все необходимые параметры в шейдеры из кода javascript.</p>
<p>Я сразу приведу код javascript веб-странички:</p>
<pre class="brush:js;">
var gl;
var shaderProgram;
var vertexBuffer;
...
<p>Во-первых, к глобальным переменным добавилась трехмерная матрица нормалей: <code>var nMatrix = mat3.create();</code>. Это один из объектов, который 
затем передается в шейдер для установки освещения.</p>
<p>В функции <code>initShaders()</code> создаем все атрибуты и параметры, которые затем будем передавать в шейдеры:</p>
<pre class="brush:js;">
	shaderProgram.vertexPositionAttribute = gl.getAttribLocation(shaderProgram, "aVertexPosition"); 
	gl.enableVertexAttribArray(shaderProgram.vertexPositionAttribute);
...
<p>Таким образом, здесь мы настраиваем три атрибута, три матрицы и четыре параметра типа uniform, которые будут использоваться для настройки света.</p>
<p>Настройку параметров uniform производится в функции setupLights(), которая затем будет вызвана в основной функции:</p>
<pre class="brush:js;">
function setupLights() {
  gl.uniform3fv(shaderProgram.uniformLightPosition, [0.0, 10.0, 5.0]);
  gl.uniform3fv(shaderProgram.uniformAmbientLightColor, [0.1, 0.1, 0.1]);
...
<p>Здесь мы, во-первых, устанавливаем позицию источника света: в данном случае это точка с координатами (0.0, 10.0, 5.0). А также устанавливаем 
rgb-значения для трех отражений света.</p>
<p>В функции <code>setMatrixUniforms()</code> в конце добавляем установку матрицы нормалей. Саму эту матрицу мы получаем в функции 
<code>setupWebGL()</code> с помощью специального метода библиотеки glMatrix: <code>mat3.normalFromMat4(nMatrix, mvMatrix);</code></p>
<p>И в функции <code>initBuffers()</code> создаем буфер нормалей. Так как в данном случае объект по сути представляет куб, то тут очень просто определить нормали. 
Для более сложных ситуаций можно написать специальную функцию, чтобы вручную не высчитывать все 1024 нормали или, например, при моделировании 
объекта в Blender оттуда взять все данные по нормалям.</p>
<p>Установка буфера нормалей производится точно так же, как и буфера вершин.</p>
<p>Все остальное остается также: использование текстуры, матриц. И в итоге мы должны получить объект с признаками освещения:</p>
<img src="pics/8.5.png" alt="Создание освещения в WebGL" />
<br>
