...
<html >
<head>
<title>Отрисовка в WebGL</title>
<meta charset="utf-8" />
<meta name="description" content="Отрисовка в WebGL с помощью методов gl.drawArrays и gl.drawElements, виды примитивов - линии, точки, треугольники, способы их рендеринга">
...
<h2>Отрисовка в WebGL</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Для отрисовки фигур в WebGL используется следующие методы:</p>
<ul>
<li><p><code>gl.drawArrays()</code></p></li>
<li><p><code>gl.drawElements()</code></p></li>
</ul>
<p>Рассмотрим их подробнее.</p>
<h3>Метод gl.drawArrays()</h3>
<p>Метод <code>gl.drawArrays()</code> отрисовывает объекты последовательно по вершинам в буфере вершин. Данный метод принимает три параметра: <code>gl.drawArrays(mode, index, count)</code></p>
<ul>
<li><p><span class="bb">mode</span>: данный параметр указывает на отрисуемый примитив и может принимать следующие значения: 
gl.POINTS, gl.LINES, gl.LINE_LOOP, gl.LINE_STRIP, gl.TRIANGLES, gl.TRIANGLE_STRIP, gl.TRIANGLE_FAN. Далее дается более подробное описание примитивов.</p></li>
<li><p><span class="bb">index</span>: второй параметр указывает, какой номер вершины в буфере вершин будет первой для примитива.</p></li>
<li><p><span class="bb">count</span>: третий параметр указывает, сколько вершин будет использоваться для отрисовки.</p></li>
</ul>
<p>В WebGL определены следующие виды примитивов:</p>
<ul>
<li><p><span class="bb">gl.LINES</span>: набор линий, при этом все линии рисуются отдельно, если у двух линий определена общая точка, 
тогда визуально они соединяются, но фактически это две разные линии</p></li>
<li><p><span class="bb">gl.LINE_STRIP</span>: набор точек последовательно соединяются линиями, а незамкнутый контур, образуемый линиями, представляет единое целое</p></li>
<li><p><span class="bb">gl.LINE_LOOP</span>: то же самое, что и <code>gl.LINE_STRIP</code>, только последняя точка в наборе дополнительно еще соединяется с первой. 
Таким образом получается замкнутый контур</p></li>
<li><p><span class="bb">gl.TRIANGLES</span>: набор треугольников</p></li>
<li><p><span class="bb">gl.TRIANGLE_STRIP</span>: набор треугольников, при этом вершины последовательно соединяются в треугольники</p></li>
<li><p><span class="bb">gl.TRIANGLE_FAN</span>: набор треугольников, при этом для всех треугольников есть одна общая вершина в центре</p></li>
<li><p><span class="bb">gl.POINTS</span>: набор обычных точек, не соединенных между собой</p></li>
</ul>
<h3>Метод gl.drawElements()</h3>
<p>Если выше рассмотренный метод имеет дело с буфером вершин, то <code>gl.drawElements()</code> работает с буфером индексов. Он имеет следующую сигнатуру: 
<code>gl.drawElements(mode, count, type, offset)</code>:</p>
<ul>
<li><p><span class="bb">mode</span>: режим, указывающий на тип примитива. В качестве примитивов используются те же, что и для метода 
<code>gl.drawArrays()</code></p></li>
<li><p><span class="bb">count</span>: число элементов для отрисовки</p></li>
<li><p><span class="bb">type</span>: тип значений в буфере индексов. Может иметь значение  <code>UNSIGNED_BYTE</code> или <code>UNSIGNED_SHORT</code></p></li>
<li><p><span class="bb">offset</span>: смещение - с какого индекса будет проводиться отрисовка</p></li>
</ul>
<p>Например, в одном из примеров ранее использовалось: <code>gl.drawElements(gl.LINE_LOOP, indexBuffer.numberOfItems, gl.UNSIGNED_SHORT,0);</code></p>
