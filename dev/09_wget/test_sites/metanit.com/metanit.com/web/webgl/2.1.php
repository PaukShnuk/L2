...
<html >
<head>
<title>Основы работы с WebGL</title>
<meta charset="utf-8" />
<meta name="description" content="Получание контекста в программе WebGL и использование элемента canvas, настройка контекста">
...
<h1>Основы работы с WebGL</h1>
<h2>Создание контекста</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Для создания приложения на WebGL нам, во-первых, необходим элемент <code>canvas</code>. Этот элемент и будет тем полотном, на котором 
затем произойдет отрисовка сцены. Создадим простенькую страничку с данным элементом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Теперь нам надо получить контекст WebGL. Все действие по использование функциональности WebGL будет происходить в функции javascript, 
которая будет срабатывать при загрузке страницы. Тут есть два способа: либо повесить функцию на обработчик <code>onload</code> 
элемента <code>body</code>, либо использовать <code>window.onload</code>.</p>
<p>Итак, создадим перед закрывающим тегом body элемент <code>script</code> и изменим страницу следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Вначале мы объявляем глобальную переменную <code>var gl=null</code> - это и есть контекст WebGL, через который будет идти рисование.</p>
<p>Далее у нас идет функция, срабатывающая при загрузке страницы. Для начала мы устанавливаем контекст: 
<code>gl = canvas.getContext("webgl") || canvas.getContext("experimental-webgl");</code>. Элемент <code>canvas</code> поддерживает два контекста: 
<span class="bb">2d</span> (используется для простого рисования) и <span class="bb">webgl</span>. Естественно, нам надо получить последний контекст. 
...
если контекст webgl не установлен.</p>
<p>Далее идет закраска. Метод <code>gl.clearColor(0.5, 0.5, 0.5, 1.0)</code> принимает значения ARGB для цвета, которым буде закрашено полотно. 
И чтобы осуществить закраску, вызываем метод <code>gl.clear</code>. (Если быть точнее, очищаем цветовой буфер, используя константу gl.COLOR_BUFFER_BIT).</p>
<p>Результатом работы должен быть элемент canvas, окрашенный в серый цвет.</p>
