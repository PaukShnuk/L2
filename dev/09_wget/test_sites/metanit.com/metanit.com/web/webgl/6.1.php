...
<html >
<head>
<title>Анимация и пользовательский ввод</title>
<meta charset="utf-8" />
<meta name="description" content="Создание анимации трехмерных объектов в WebGL, использование функции requestAnimFrame">
...
<h1>Анимация и пользовательский ввод</h1><h2>Анимация объектов. Вращающийся куб</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>В качестве примера анимации создадим вращающийся куб. Итак, полный код веб-странички будет следующим:</p>
<pre class="brush:js;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В отличие от примера со статическим кубом здесь есть несколько изменений. Во-первых, мы добавили переменную <code>angle</code>, которая будет представлять 
угол поворота. С каждым интервалом времени данная переменная будет изменяться, и соответственно будет меняться угол поворота куба.</p>
<p>Во-вторых, в самом конце кода мы добавили функцию requestAnimationFrame для создания анимации.</p>
<h3>Использование requestAnimationFrame</h3>
<p>Для создания анимации раньше разработчики могли использовать специальные функции javascript:</p>
<ul>
<li><p><code>setTimeout(callback, timeoutInMilliseconds)</code></p></li>
<li><p><code>setInterval(callback, timeoutInMilliseconds)</code></p></li>
</ul>
<p>Обе функции в качестве параметра callback принимают функцию, которая срабатывала через определенное время timeoutInMilliseconds. Ну допустим относительно 
нашей задачи мы могли бы написать:</p>
<pre class="brush:js;">
window.onload=function(){
...
<p>Здесь функция setInterval вызывает функцию drawloop каждые 16,7 миллисекунд, то есть 60 раз в секунду. По сути данный подход будет равноценен предыдущему. Но..
Разработчики браузеров в последних версиях стали внедрять новое решение - метод <b>requestAnimationFrame</b>, который является рекомендуемым 
...
для создания более плавного эффекта браузер может замедлять анимации, эффективно определяя нужный интервал.</p>
<p>В данном случае мы задействуем requestAnimationFrame, и в качестве параметра данный метод получает функцию, выполняемую перед перерисовкой.</p>
<pre class="brush:js;">
window.requestAnimFrame = (function(){
      return  window.requestAnimationFrame       || 
              window.webkitRequestAnimationFrame || 
...
<p>Это кроссбраузерное определение метода. Далее в функции настройки контекста setupWebGL() увеличиваем угол на некоторое число и применяем его при повороте матрицы:</p>
<pre class="brush:js;">
angle += 0.01;
//.........................
mat4.rotate(mvMatrix,mvMatrix, angle, [0, 1, 0]);
</pre>
<p>В главной функции мы используем следующую конструкцию, в которую выносим функции по настройке матриц и отрисовке:</p>
<pre class="brush:js;">
(function animloop(){
	setupWebGL();
	setMatrixUniforms();
...
<p>Функция requestAnimFrame(animloop, canvas) как раз обращается к  window.requestAnimationFrame, передавая в параметрах функцию обратного вызова и элемент, 
который содержит всю анимацию, то есть в данном случае canvas.</p>
<p>Подобным образом мы можем задать анимацию перемещения или масштабирования. Либо также мы могли бы анимировать перемещение камеры, а не куба. Но 
общий принцип будет тот же.</p>
