...
<meta charset="utf-8" />
<meta name="description" content="Использование материалов при создании освещения по модели Фонга в WebGL">
...
<h2>Использование материалов</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Применение материалов в WebGL по сути не привносит ничего сложного. Мы просто определяем для каждого типа отражения свой материал и передаем его в вершинный 
шейдер. Код веб-странички будет следующим:</p>
<pre class="brush:js;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В плане кода настройка, передача в шейдер и использование материалов ничем не отличается от использования цветов освещения. В итоге наш куб будет выглядеть 
следующим образом:</p>
<img src="pics/8.6.png" alt="Использование материалов в WebGL" />
<p>Во-первых, здесь я убрал весь код, связанный с текстурами. В принципе он нам не особо нужен. Потому что цвет фигуры будут задавать цвета материалов.</p>
<p>Затем, также как и с цветовыми настройками освещения, мы задаем цвет материалов:</p>
<pre class="brush:js;">
// установка материалов
function setupMaterials() {
  gl.uniform3fv(shaderProgram.uniformAmbientMaterialColor, [0.0, 1.0, 0.0]);
...
<p>И в вершинном шейдере мы можем их получить и использовать при настройке освещения по модели отражения Фонга:</p>
<pre class="brush:js;">
vLightWeighting = uAmbientMaterialColor * uAmbientLightColor + 
				uDiffuseMaterialColor * uDiffuseLightColor * diffuseLightDot +
				uSpecularMaterialColor * uSpecularLightColor * specularLightParam;
