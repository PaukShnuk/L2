...
<html >
<head>
<title>Vue 3 | Флажки (Checkbox)</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с флажками во Vue 3, элементы checkbox и директива v-model, двусторонняя привязка к сложным объектам">
...
<h2>Флажки (Checkbox)</h2><div class="date">Последнее обновление: 02.11.2020</div>
...
<p>Флажок или checkbox представляет элемент управления, который может находиться в двух состояниях: отмеченном и неотмеченном. 
При установке привязки директива <code>v-model</code> использует его атрибут <code>checked</code> и событие <code>change</code> для 
отслеживания изменения значения. При этом начальное значение атрибута <code>checked</code>.</p>
<p>Одиночный флажок может быть привязан к логическому значению true или false:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/3.2.png" alt="Checkbox binding in Vue 3" />
<p>По умолчанию для отмеченного состояния используется значение true, а для неотмеченного - false. Но вместо них можно 
определить свои собственные значения для обоих состояний и между этими значениями переключаться:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
...
<p>Вместо значений true и false флажок привязывается к одному из свойств on или off. С помощью атрибута <code>v-bind:true-value="on"</code> 
...
попадает в свойство light.</p>
<img src="./pics/3.8.png" alt="Привязка к флажкам checkbox с помощью директивы v-model в Vue 3" />
<p>Во Vue 3 мы можем сократить эту конструкцию следующим образом:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
...
<p>Здесь элемент input применяет два атрибута, поставляемые Vue: <code>true-value</code> и <code>false-value</code>. Эти атрибуты имеют 
...
мы можем узнать, чему равно свойство light: <code>light==='on'</code>.</p>
<h3>Привязка к массиву</h3>
<p>Также группа флажков может быть привязана к одному массиву. В этом случае при отметке флажка его значение попадает в массив:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае 4 флажка привязаны к массиву selectedUsers. Отметив определенный флажок, мы добавим его значение в массив.</p>
<img src="./pics/3.1.png" alt="Checkbox и привязка к массиву в Vue 3" />
<h3>Привязка к объекту</h3>
<p>В примере выше значение каждого флажка представляло некоторую строку. Но также флажок может быть привязан к целому объекту. Для динамической привязки у 
элемента html применяется атрибут <span class="b">v-bind:value</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь динамически для каждого объекта в массиве формируется флажок, а при выборе флажка объект попадет в массив selectedUsers.</p>
<img src="./pics/3.11.png" alt="Привязка сложных объектов к input checkbox в Vue 3" />
