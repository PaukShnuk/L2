...
<html >
<head>
<title>Vue 3 | Элементы input и textarea</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с формами в Vue 3, элементы input и textarea, привязка данных с помощью v-model">
...
<h1>Работа с формами</h1><h2>Элементы input и textarea</h2><div class="date">Последнее обновление: 02.11.2020</div>
...
<p>Ранее в теме про <a href="//metanit.com/web/vuejs/1.5.php">Двустороннюю привязку</a> рассматривалась директива <span class="b">v-model</span>, 
...
подробно, как мы можем применять v-model и двустороннюю привязку для работы с формами. Например, используем ряд базовых элементов <code>input</code>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Директива v-model связывает атрибут <code>value</code> элемента input и свойство из приложения Vue и использует событие <code>input</code> у 
элемента для отслеживания изменения ввода. Если атрибут value у элемента input имеет 
некоторое значение, то оно игнорируется. В итоге при вводе в поле input свойство из приложения Vue подхватит введенное значение:</p>
<img src="./pics/3.9.png" alt="элементы формы и двусторонняя привязка с помощью v-model в Vue 3" />
<h3>textarea</h3>
<p>Элемент textarea позволяет вводить многострочный текст и также связывается со свойством во Vue через директиву v-model:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/3.10.png" alt="v-model и textarea во Vue 3" />
<p>В то же время есть некоторые особенности по работе с textarea. Прежде всего, если используется директива v-model, то внутри textarea мы не можем 
поместить текст с помощью интерполяции:</p>
<pre class="brush:xml;">&lt;textarea v-model="comment" class="form-control"&gt;{{comment}}&lt;/textarea&gt;</pre>
<p>И кроме того, при вводе текста в textarea с помощью клавиши Enter мы можем переносить строки. Чтобы переносы строк сохранялись при выводе 
текста на страницу, необходимо применять соответствующие стили css:</p>
<pre class="brush:xml;">
&lt;p style="white-space: pre-line"&gt;{{comment}}&lt;/p&gt;
</pre>
