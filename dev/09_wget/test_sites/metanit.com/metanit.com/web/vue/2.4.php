...
<html >
<head>
<title>Vue 3 | Фильтрация и сортировка массива</title>
<meta charset="utf-8" />
<meta name="description" content="Фильтрация и сортировка массива во Vue 3, взаимодействие со свойствами computed">
...
<h2>Фильтрация и сортировка массива</h2><div class="date">Последнее обновление: 02.11.2020</div>
...
<p>Одной из распространенных задач при работе с массивами является фильтрация. Обычно при фильтрации есть некоторый начальный список, а пользователю же 
возвращается некоторый временный результат. Начальный же список при этом не изменяется. И для этого во Vue лучше определить привязку к вычисляемому свойству:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае фильтрация работает по принципу живого поиска. При вводе значения в текстовое поле происходит повторное вычисление свойства filteredList. 
Это свойство представляет результат функции, которая возвращает те объекты, у которых поле company соответствует введенному значению. То есть идет фильтрация телефонов по компании производителя. 
Если же значение не введено, то возвращаем все элементы из массива phones.</p>
<p>В итоге динамически мы можем фильтровать элементы списка.</p>
<img src="./pics/29.png" alt="Фильтрация и живой поиск в Vue 3" />
<h3>Сортировка списка</h3>
<p>Для сортировки списка применяется похожая техника, что и для фильтрации:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Как и в случае с фильтрацией, привязка устанавливается к вычисляемому свойству. При нажатии на заголовок столбца в таблице происходит 
переустановка значения свойства sortParam, которое представляет критерий фильтрации. При его изменении повторно вычисляется свойство sortedList, 
которое сортирует массив phones в соответствии со значением в sortParam. Для фильтрации по трем критериям определены три вспомогательные функции sortByCompany, sortByPrice 
и sortByTitle.</p>
<img src="./pics/29.1.png" alt="Сортировка массивов в Vue 3" />
