...
<html >
<head>
<title>Vue 3 | Параметры маршрутов</title>
<meta charset="utf-8" />
<meta name="description" content="Параметры маршрутов во Vue 3, необязательные параметры, получение их значения в компонентах, объект $route.params">
...
<h2>Параметры маршрутов</h2><div class="date">Последнее обновление: 04.11.2020</div>
...
<p>Маршруты во Vue 3 могут содержать параметры, которые позволяют при навигации передавать некоторые данные. 
Для определения параметра в конец шаблона маршрута после двоеточия добавляется название параметра:</p>
<pre class="brush:xml;">{ path: '/products/:id', component: Products }</pre>
<p>Например, в данном случае определен параметр id. И такому маршруту , к примеру, будут соответствовать такие пути запроса как:</p>
<ul>
<li><p>/products/6-tom</p></li>
<li><p>/products/21</p></li>
<li><p>/products/phones</p></li>
</ul>
<p>Та часть, которая идет после последнего слеша, будет интерпретироваться как значение параметра id.</p>
<p>В компоненте Vue 3 мы можем получить параметры запроса через объект <span class="b">$route.params</span>, который содержит значения всех параметров. 
В частности, для обращения к параметру id нужно использовать выражение <code>$route.params.id</code>.</p>
<p>К примеру, определим следующую страницу index.html:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь маршрут, использующий компонент Products, применяет один параметр id:</p>
<img src="./pics/router5.png" alt="Параметры маршрутов в Vue 3 и $route.params" />
<p>Подобным образом можно использовать большее количество параметров:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае маршрут <code>{ path: '/products/:category/:id', component: Products }</code> применяет два параметра category и id, 
поэтому такому маршруту будут соответствовать следующие пути:</p>
<ul>
<li><p>/products/tablets/2</p></li>
<li><p>/products/phones/3</p></li>
</ul>
<img src="./pics/router6.png" alt="Передача параметров в vue-router" />
<h3>Регулярные выражения в параметрах</h3>
<p>В прошлой теме было рассмотрено использование параметров в маршрутов. Например, мы могли бы определить следующий маршрут:</p>
<pre class="brush:js;">{ path: '/products/:category/:id', component: Products }</pre>
<p>где применяется статический сегмент <code>/products</code> и динамические параметры category и id. Но система маршрутизации Vue 
также позволяет использовать при определении маршрутов регулярные выражения, что позволяет реализовать более изощренные сценарии 
при обработке маршрутов. По факту, когда, например, необходимо выделить из строки запроса параметр id (или другие динамические параметры), система маршрутизации 
применяет регулярное выражение <code>([^/]+)</code> (пока не встретится слеш /). Рассмотрим некоторые сценарии.</p>
<h3>Необязательные параметры</h3>
<p>Параметры могут быть необязательными. Для таких параметров необязательно передавать значения. Для их определения после названия параметра ставится вопросительный знак:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае в маршруте <code>{ path: '/products/:id?', component: Products }</code> параметр id определен как необязательный, 
поэтому компонент Products будет обрабатывать запросы как по пути "/products/3", так и по пути "/products".</p>
<img src="./pics/router4.png" alt="Необязательные параметры optional params в Vue 3" />
<h3>Параметр представляет только числа</h3>
<p>Выше параметр id мог представлять обычную строку. Но, допустим, у нас id должен представлять только числа. В этом случае мы можем 
использовать регулярное выражение "(\d+)":</p>
<pre class="brush:js;">
const routes = [
  { path: '/', component: Home },
...
<p>Также мы можем совместить оба условия - параметр может отсутствовать, но если он присутствует, он должен представлять число:</p>
<pre class="brush:js;">
const routes = [
  { path: '/', component: Home },
  { path: '/products/:id(\\d+)?', component: Products }
...
<div class="nav"><p><a href="6.2.php">Назад</a><a href="./">Содержание</a><a href="6.8.php">Вперед</a></p></div></div>
<div class="socBlock">
