...
<html >
<head>
<title>Vue 3 | Обработка ввода пользователя. События</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка ввода пользователя и событий в Vue 3, объект event, передача в обработчики параметров, директива v-on">
...
<h2>Обработка ввода пользователя. События</h2><div class="date">Последнее обновление: 31.10.2020</div>
...
<p>Для обработки ввода пользователя и событий элементов html в Vue 3 применяется директива <span class="b">v-on</span>, 
после которой через двоеточие указывается тип обрабатываемого события:</p>
<pre class="brush:js;">v-on:click="действия"</pre>
<p>В качестве типа события используется любое стандартное событие элементов html на веб-странице. Затем этой директиве в кавычках передаются те действия, 
которые должны выполняться при возникновении события. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В качестве действия директиве <code>v-on</code> можно передать любые валидные операции JavaScript. В данном случае, так как у нас определено во Vue свойство 
<code>counter</code>, то мы можем манипулировать значением этого свойства:</p>
<pre class="brush:xml;">
&lt;button v-on:click="counter++"&gt;+&lt;/button&gt;
&lt;span&gt;{{counter}}&lt;/span&gt;
&lt;button v-on:click="if(counter&gt;0) counter--;"&gt;-&lt;/button&gt;
</pre>
<p>Фактически директивам <code>v-on</code> передается кусок кода JavaScript, где свойства из Vue доступны как обычные переменные.</p>
<img src="./pics/6.png" alt="Обработка ввода пользователя и событий в Vue 3" />
<p>Также мы можем использовать сокращенный синтаксис для определения обработчиков событий:</p>
<pre class="brush:xml;">
&lt;button @click="counter++"&gt;+&lt;/button&gt;
&lt;span&gt;{{counter}}&lt;/span&gt;
&lt;button @click="if(counter&gt;0) counter--;"&gt;-&lt;/button&gt;
</pre>
<p>Однако если обработка событий предусматривает гораздо больше действий, то такой подход не является оптимальным. И в этом случае, 
как правило, все операции, которые необходимо вызывать при генерации события, выносятся в отдельные методы объекта Vue. Например, 
перепишем предыдущий пример следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>При нажатии на одну кнопку срабатывает метод increase, который увеличивает значение переменной counter. При нажатии на вторую кнопку вызывается метод 
decrease, который уменьшает это значение.</p>
<h3>Передача параметров в обработчик события</h3>
<p>Также мы можем передать в методы для обработки событий какие-то дополнительные значения:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае увеличение переменной counter будет идти на 4, а уменьшение - на 2.</p>
<h3>Получение объекта события</h3>
<p>При генерации события в его обработчике в качестве параметра мы можем получить объект, который инкапсулирует всю информацию о событии:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В качестве параметра в методы-обработчики события передается объект события. Это стандартный объект, который используется при 
обработке событий в обычном JavaScript. В зависимости от типа события этот объект может нести разную информацию.</p>
<img src="./pics/8.png" alt="Объект события в Vue 3" />
<p>Также можно совместить получение объекта $event и передачу других параметров в обработчике события:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<h3>Множественная обработка событий</h3>
<p>В примерах выше событию нажатию кнопки сопоставлялся один обработчик - в данном случае метод increase или decrease. Но также можно прикрепить 
к событию сразу несколько обработчиков:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае к событию нажатия привязаны два обработчика - методы <code>increase</code> и <code>inform</code>. И при нажатии кнопки 
будут срабатывать сразу оба этих метода. Метод inform с помощью выражения <code>new Date().toLocaleString()</code> получает текущее время (фактически время нажатия кнопки) и изменяет свойство <code>status</code>, 
которое выводится на веб-страницу:</p>
<img src="./pics/8.1.png" alt="Несколько обработчиков слушателей событий в Vue 3" />
...
<div class="nav"><p><a href="1.7.php">Назад</a><a href="./">Содержание</a><a href="1.5.php">Вперед</a></p></div></div>
<div class="socBlock">
