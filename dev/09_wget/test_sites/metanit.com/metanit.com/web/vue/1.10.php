...
<html >
<head>
<title>Vue 3 | Жизненный цикл</title>
<meta charset="utf-8" />
<meta name="description" content="Жизненный цикл приложения Vue 3, его основные этапы и их обработка, Lifecycle Hooks">
...
<h2>Жизненный цикл приложения Vue 3</h2><div class="date">Последнее обновление: 01.11.2020</div>
...
<p>В процессе своей работы приложение Vue 3 проходит через ряд этапов, которые составляют жизненный цикл приложения Vue 3. 
Кроме того, фреймворк Vue предоставляет ряд специальных методов, с помощью которых мы можем вызвать некоторые действия 
на различных этапах жизненного цикла приложения.</p>
<p>Весь жизненный цикл приложения Vue 3 можно описать следующим образом:</p>
<ol>
<li><p>Вызывается функция <code>Vue.createApp()</code> и метод <code>mount()</code>. Сначала происходит инициализация приложения Vue.</p></li>
<li><p>Сразу после инициализации приложения, но до установки свойств, методов и обработчиков событий объектов приложения вызывается метод <span class="b">beforeCreate()</span>.</p></li>
<li><p>Далее происходит создание объекта Vue, установка его свойств, обработчиков событий и прочих методов</p></li>
<li><p>После создания объекта приложения Vue и установки всех свойств и методов вызывается метод <span class="b">created()</span>. На этом этапе 
еще не произошло прикрепление объекта приложения Vue к элементу html веб-страницы.</p></li>
<li><p>Далее выполняется компиляция шаблона - валидную разметку с кодом html, которая представляет приложение Vue и внутри которой приложение будет работать.</p></li>
<li><p>Вызывается метод <span class="b">beforeMount()</span>. </p></li>
<li><p>Элемент html, который передается в метод <code>mount</code> в <code>Vue.createApp({}).mount()</code>, 
заменяется скомпилированным шаблоном.</p></li>
<li><p>Сразу после прикрепления приложения Vue к элементу html, вызывается метод <span class="b">mounted()</span>, и после этого шаблон считается прикрепленным к DOM, и мы можем с ним работать</p></li>
<li><p>Если в процессе работы мы обновляем данные приложения Vue, то происходит еще ряд событий:</p>
	<ol>
		<li><p>Данные изменяются</p></li>
		<li><p>Вызывается метод <span class="b">beforeUpdate()</span></p></li>
		<li><p>Производится повторный рендеринг DOM для его соответствия виртуальному DOM</p></li>
		<li><p>Вызывается метод <span class="b">updated()</span>. DOM на веб-странице обновлен, и мы продолжаем работать с приложением Vue</p></li>
	</ol>
</li>
<li><p>При откреплении объекта приложения Vue вызывается метод <span class="b">beforeUnmount()</span></p></li>
<li><p>И после открепления приложения Vue от html-страницы вызывается метод <span class="b">unmounted()</span>.</p></li>
</ol>
<p>Схематично жизненный цикл можно представить так:</p>
<img src="./pics/36.png" alt="Жизненный цикл приложения Vue.js" />
<p>Для примера используем методы управления жизненным циклом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Прежде всего обратите внимание, что все методы, которые обрабатывают события жизненного цикла, помещаются вне параметра <code>methods</code>.</p>
<p>Для вызова метода <code>unmount()</code> у объкта приложения, применяется встроенная функция JavaScript <code>setTimeout()</code>, которая вызывает метод 
<code>vueApp.unmount()</code> через 10000 миллисекунд после загрузки страницы.</p>
<p>В итоге после загрузки страницы мы увидим в консоли браузера сообщения, что сработало то или иное событие жизненного цикла. А при нажатии на кнопку Update выполняется обновление DOM, и вызываются методы <code>beforeUpdate()</code> и <code>updated()</code>.</p>
<img src="./pics/37.png" alt="Управление жизненным циклом и lifecycle hooks во Vue 3" />
<p>А через 10 секунд после загрузки веб-страницы будет выполняться метод <code>vueApp.unmount()</code>, который отвяжет объект приложения Vue 
от элемента веб-страницы и приведет к вызову методов <code>beforeUnmount()</code> и <code>unmonted()</code>. И после этого объект приложения Vue будет откреплен от веб-страницы, 
и работать с ним мы больше не сможем.</p>
<img src="./pics/37.1.png" alt="Unmounted во Vue 3" />
...
<div class="nav"><p><a href="1.11.php">Назад</a><a href="./">Содержание</a><a href="2.1.php">Вперед</a></p></div></div>
<div class="socBlock">
