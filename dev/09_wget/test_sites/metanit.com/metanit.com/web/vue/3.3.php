...
<html >
<head>
<title>Vue.js | Переключатели (radiobutton)</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с переключателями во Vue 3, элементы радиокнопки или radiobutton, привязка к сложным объектам с помощью директивы v-model">
...
<h2>Переключатели (radiobutton)</h2><div class="date">Последнее обновление: 02.11.2020</div>
...
<p>Переключатели (radiobutton или радиокнопки) позволяют определить группу альтернативных вариантов, из которых можно выбрать только один. 
При установке привязки, также как и для элементов <code>&lt;input type="checkbox"&gt;</code>, директива <code>v-model</code> использует его атрибут <code>checked</code> и событие <code>change</code> для 
отслеживания изменения значения, а начальное значение атрибута <code>checked</code>.
<p>Переключатели привязываются к одному свойству. При выборе переключателя связанное свойство получает выбранное значение:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/3.3.png" alt="Привязка к переключателям radioButtons в Vue 3" />
<h3>Привязка к объекту</h3>
<p>Также каждый из переключателей может быть привязан к сложному объекту. В этом случае используется атрибут <span class="b">v-bind:value</span>. 
Результатом выбора соответственно также будет сложный объект:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/3.12.png" alt="RadioButton и сложные объекты во Vue 3" />
<h3>Привязка к массиву объектов</h3>
<p>В случае выше было бы более оптимально хранить данные в массиве и динамически создавать по элементам массива группу радиокнопок:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Результат работы будет тот же самый, что и в предыдущем примере.</p>
