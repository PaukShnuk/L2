...
<html >
<head>
<title>Vue 3 | Объект приложения Vue</title>
<meta charset="utf-8" />
<meta name="description" content="Объект приложения Vue во Vue 3, функция его свойства и методы, параметры data и methods">
...
<h2>Объект приложения Vue</h2><div class="date">Последнее обновление: 30.09.2020</div>
...
<p>Для использования функциональности фреймворка Vue на веб-странице вначале необходимо создать объект приложения Vue. 
Для его создания во Vue 3 используется функция <span class="b">Vue.createApp</span>. При создании объекта приложения в эту функцию 
передается объект с параметрами, с помощью которых производится инициализация приложения:</p>
<pre class="brush:js;">
Vue.createApp(/* { параметры } */)
</pre>
<p>После создания объекта приложения у него можно вызвать метод <span class="b">mount()</span>, с помощью которого выполняется прикрепление 
созданного приложения Vue к опредленному элементу html на веб-странице. Для этого в метод <code>mount()</code> передается селектор данного элемента html:</p>
<pre class="brush:js;">
...
<p>В данном случае в функцию Vue.createApp передается пустой объект без каких либо параметров, но тем не менее этого достаточно, чтобы создать объект 
приложения Vue. То есть здесь переменная <code>vueApp</code> будет представлять объект приложения Vue 3. Затем этот объект прикрепляется к элементу 
div с идентификатором app.</p>
<h3>Данные приложения</h3>
<p>Теперь рассмотрим, как мы можем настроить и инициализировать приложение Vue.</p>
<p>Объект с параметрами, который передается в функцию Vue.createApp, использует функцию <span class="b">data()</span> для определения свойств приложения, которые будут хранить некоторые данные:</p>
<pre class="brush:js;">
Vue.createApp({
	data() {
	  return {
...
<p>Функция data возвращает объект со свойствами, предназначенными для хранения данных. В данном случае он определяет два свойства - name и age.</p>
<p>Используем этот объект на веб-странице:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Затем с помощью выражения <code>{{ название_свойства}}</code> (в двойные фигурные скобки помещается имя свойства) мы можем обратиться к свойству и 
вывести его значение на веб-страницу:</p>
<img src="./pics/2.1.png" alt="Data в Vue.createApp" />
<p>Данные можно также определять в виде внешнего объекта и затем возвращать из функции data:</p>
<pre class="brush:js;">
...
<p>Кроме того, используя объект Vue, мы можем обращаться к его свойствам по имени, получать и присваивать им значения:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
// получение данных
console.log(vueApp.age);	// 36
// изменение данных
vueApp.age = 25;
console.log(vueApp.age);	// 25
&lt;/script&gt;
&lt;/body&gt;
&lt;/html&gt;
</pre>
<p>Стоит отметить, что свойства объекта приложения Vue обладают "<span class="b">реактивностью</span>" - при изменении их значения, будет автоматически обновляться представление - элемент 
веб-страницы, связанный с данным объектом приложения Vue. Например, в примере выше веб-страница отобразит измененное значение свойства age.</p>
<img src="./pics/2.2.png" alt="Change Data properties в Vue.createApp" />

<h3>Методы</h3>
<p>Кроме хранения состояния приложение Vue может определять поведение в виде параметра <span class="b">methods</span>. Этот параметр указывает на набор методов. 
Каждый метод представляет стандартную функцию JavaScript:</p>
<pre class="brush:js;">
methods:{
	метод1(){
		// тело метода 1
...
<p>Например, определим ряд методов:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь определено два метода welcome и displayUser, которые возвращают некоторое значение. Для обращения к свойствам объекта Vue в методах используется ключевое слово <span class="b">this</span>, 
например, <code>this.age</code>.</p>
<p>И если метод возвращает некоторое значение, то мы можм получить его значение на веб-странице, обратившись к этому методу с помощью интерполяции:</p>
<pre class="brush:xml;">{{ welcome() }}</pre>
<img src="./pics/2.3.png" alt="Методы в Vue.js 3" />
<p>К методам также можно обращаться в коде JavaScript:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае опеределен метод incrementAge, который увеличивает значение свойства age на единицу. Кроме того, определен метод 
<code>incrementAgeThreeTimes()</code>, который три раза вызывает метод incrementAge - то есть фактически увеличивает значение свойства age на 3. 
Для обращения к методам объекта внутри самого объекта, также как и для обращения к свойствам, используется <code>this.</code></p>
<pre class="brush:js;">
incrementAgeThreeTimes(){
		this.incrementAge();
		//..........
</pre>
<p>И также, используя имя объекта мы можем вызвать его метод в коде JS:</p>
<pre class="brush:js;">
app.incrementAgeThreeTimes();
</pre>
<p>В итоге при выполнении примера переменная age увеличится на три:</p>
<img src="./pics/2.4.png" alt="Вызов методов в Vue.js 3" />
