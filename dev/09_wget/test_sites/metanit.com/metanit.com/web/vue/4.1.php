...
<html >
<head>
<title>Vue 3 | Создание компонентов</title>
<meta charset="utf-8" />
<meta name="description" content="Компоненты во Vue 3, создание и рендеринг компонентов, правила именования, метод app.component">
...
<h1>Компоненты</h1><h2>Создание компонентов</h2><div class="date">Последнее обновление: 02.11.2020</div>
...
<p>Компоненты представляют именованные объекты, к которым фреймворк Vue прикрепляет некоторое поведение. Компоненты позволяют инкапсулировать код и затем использовать 
его многократно в различных частях приложения.</p>
<p>Для создания компонента у объекта приложения Vue используется метод <span class="b">component(имя_компонента, конфигурация_компонента)</span>, где первый параметр 
представляет имя компонента, а второй параметр представляет конфигурацию компонента, которая собственно и определяет его данные и поведение.</p>
<p>Например, определим простейший компонент:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь определен компонент для элемента hello. И не важно, что такого элемента в html не существует, мы его сами создаем. Именно на место элемента 
<code>&lt;hello&gt;&lt;/hello&gt;</code> будет впоследствии рендерится компонент Vue. Более того мы не можем использовать для компонента 
встроенные элементы html типа div или h2.</p>
<p>В коде JavaScript, как обычно, определяется объект приложения Vue:</p>
<pre class="brush:js;">const app = Vue.createApp({});</pre>
<p>Только теперь мы не передаем в функцию <code>Vue.createApp()</code> никаких параметров для приложения, только пустой объект. Хотя это необязательно, мы могли бы определить 
и для объекта приложения какие-нибудь данные, методы и прочие параметры приложения.</p>
<p>Далее у объекта приложения Vue после его создания вызывается метод <code>app.component()</code></p>
<pre class="brush:js;">app.component('hello', { template: '<h2>Hello</h2>'});</pre>
<p>Первый аргумент метод - "hello" представляет название компонента, то есть наш компонент называется "hello". Стоит отметить, что между названием элемента и названием компонента 
есть соответствие: элемент <code>&lt;hello&gt;</code> и компонент "hello".</p>
<p>Есть два способа именования компонента: kebab-case и PascalCase. Например, у нас есть следующий элемент в разметке, в который будет рендерится компонент:</p>
<pre class="brush:xml;">
&lt;my-component-name&gt;&lt;/my-component-name&gt;
</pre>
<p>Тогда компонент должен называться либо "my-component-name" (kebab-case), либо "MyComponentName" (PascalCase).</p>
<p>Второй параметр метода <code>app.component()</code> устанавливает конфигурацию компонента. Так, через свойство <span class="b">template</span> можно задать 
html-разметку, которую будет содержать компонент. В итоге данная разметка будет вставляться 
вместо элемента <code>&lt;hello&gt;&lt;/hello&gt;</code>.</p>
<p>И при рендеринге страницы вместо элемента hello будет вставлено содержимое компонента:</p>
<img src="./pics/35.png" alt="Введение в компоненты в Vue 3" />
<p>При этом мы можем многократно использовать компонент в рамках приложения Vue, и в каждом случае будет происходить рендеринг компонента.</p>
<p>Причем компонент может использоваться только в рамках того элемента, к которому прикреплен объект Vue. То есть мы не можем написать наподобие, вынеся использование компонента во вне:</p>
<pre class="brush:xml;">
&lt;hello&gt;&lt;/hello&gt;
&lt;div id="app"&gt;
&lt;/div&gt;
