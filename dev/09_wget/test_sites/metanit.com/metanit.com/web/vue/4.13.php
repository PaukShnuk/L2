...
<html >
<head>
<title>Vue 3 | Передача функций обратного вызова в компоненты</title>
<meta charset="utf-8" />
<meta name="description" content="Передача функций обратного вызова от родительского компонента к дочернему во Vue 3">
...
<h2>Передача функций обратного вызова в компоненты</h2><div class="date">Последнее обновление: 03.11.2020</div>
...
<p>Функции обратного вызова представляют еще один способ взаимодействия между родительским и дочерним компонентами: родительские компоненты могут определять функции, 
а вызываются они в дочерних компонентах.</p>
...
<p>В данном случае объект приложения Vue определяет некоторый счетчик - переменную count. И также определеяет метод <code>increase()</code> для 
...
как обычное свойство под названием "changefn":</p>
<pre class="brush:xml;">&lt;counter :changefn="increase"&gt;&lt;/counter&gt;</pre>
<p>В самом компоненте counter по нажатию на кнопку вызывается метод, переданный через параметр changefn.</p>
<pre class="brush:xml;">&lt;button v-on:click="changefn()"&gt;+&lt;/button&gt;</pre>
<p>В итоге при нажатии на кнопку будет вызываться метод родительского компонента, и мы увидем увеличение значения переменной count.</p>
<img src="./pics/56.1.png" alt="Передача функций в дочерние компоненты в Vue 3" />
...
<p>Объект Vue выводит на страницу массив элементов и определяет два метода для управления элементами: add (для добавления) и remove (для удаления).</p>
<p>Для добавления нового элемента определен компонент userform, который представляет форму с полями ввода. В этот компонент передается метод add в виде функции addfn:</p>
<pre class="brush:xml;">&lt;userform :addfn="add"&gt;&lt;/userform&gt;</pre>
<p>Причем функция addfn определена в компоненте userform через props:</p>
<pre class="brush:js;">
Vue.component('userform', {
	props: ["addfn"],
</pre>
<p>При нажатии на кнопку эта функция будет вызываться, и ей будут передаваться введенные данные. Что фактически приведет к вызову метода add в родительском объекте Vue.</p>
<p>Аналогичная ситуация с компонентом useritem, который получает метод remove в виде функции removefn, которая также определяется через props. При нажатии на кнопку удаления вызывается 
функция removefn, ей передается индекс удаляемого элемента, и фактически будет идти вызов метода remove из объекта Vue.</p>
<p>Таким образом, в самих дочерних компонентах никаких действий по управлению массивом и объектами и по генерации событий не определено. Дочерние компоненты 
просто вызывают те методы, которые определены в родительском объекте.</p>
<img src="./pics/56.png" alt="Добавление в массив в Vue 3" />
...
<div class="nav"><p><a href="4.12.php">Назад</a><a href="./">Содержание</a><a href="4.15.php">Вперед</a></p></div></div>
<div class="socBlock">
