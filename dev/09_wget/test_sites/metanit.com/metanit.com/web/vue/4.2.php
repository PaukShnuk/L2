...
<html >
<head>
<title>Vue 3 | Локальная и глобальная регистрация компонентов</title>
<meta charset="utf-8" />
<meta name="description" content="Локальная и глобальная регистрация компонентов во Vue 3, примеры использования компонентов">
...
<h2>Локальная и глобальная регистрация компонентов</h2><div class="date">Последнее обновление: 02.11.2020</div>
...
<p>Компоненты могут быть зарегистрированы локально и глобально. Глобальные компоненты в отличие от локальных доступны для любого другого компонента в приложении Vue. Взаимодействие между 
компонентами будет рассмотрено позже, а пока посмотрим, как мы можем регистрировать в приложении компоненты.</p>
<h3>Глобальная регистрация</h3>
<p>Для глобальной регистрации применяется метод <code>component()</code> объекта приложения Vue:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае создается два компонента component-a и component-b.</p>
<img src="./pics/38.1.png" alt="Глобальные и локальные компоненты в Vue 3" />
<h3>Локальная регистрация компонентов</h3>
<p>При локальной регистрации компоненты задаются как обычные объкты JavaScript с необходимыми конфигурационными настройками. 
Затем для локальной регистрации компоненты передаются в объекта Vue устанавливается через свойство <span class="b">components</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае для каждого компонента определена только одна конфигурационная настройка - параметр template.</p>
<pre class="brush:js;">
const ComponentA = {
  template: '&lt;h2&gt;Component A&lt;/h2&gt;'
}
...
<p>Затем они передаются параметру <code>components</code> в объект приложения Vue:</p>
<pre class="brush:js;">
components: {
		'component-a': ComponentA,
		'component-b': ComponentB
...
<p>Здесь каждому компоненту назначается определеное имя, например, объекту ComponentA назначается имя "component-a" и соответственно для его рендеринга на веб-странице 
будет использоваться элемент <code>&lt;component-a&gt;</code>.</p>
<p>Если бы мы не передали объект компонента в свойство components, тогда мы не смогли бы его использовать.</p>
<p>В итоге мы получим тот же самый результат, что и в примере с глобальными компонентами:</p>
<img src="./pics/38.1.png" alt="Глобальные и локальные компоненты в Vue 3" />
