...
<html >
<head>
<title>Vue 3 | Props</title>
<meta charset="utf-8" />
<meta name="description" content="Свойства Props во Vue 3, передача компонентам данных извне">
...
<h2>Props</h2><div class="date">Последнее обновление: 03.11.2020</div>
...
<p>Каждый компонент может определять параметр <span class="b">props</span>, через который в компонент можно передать извне различные данные.
Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<p>Параметр <code>props</code> хранит массив ключей или свойств, которым извне можно передать значения. В данном случае определено одно свойство 
message. Это свойство также можно использовать в шаблоне компонента (<code>&lt;span&gt;{{ message }}&lt;/span&gt;</code>).</p>
<p>Чтобы передать свойству значение, у элемента компонента применяется атрибут, который называется также, как и свойство:</p>
<pre class="brush:xml;">&lt;message-comp message="hello"&gt;&lt;/message-comp&gt;</pre>
<img src="./pics/44.png" alt="Передача внешних данных в компонент через параметр props в Vue 3" />
<p>В некотором плане свойства props похожи на те свойства, которые определяются через параметр data. В принципе мы даже могли бы определить 
свойство с одним и тем же именем и в props, и в data:</p>
<pre class="brush:js;">
app.component('message-comp', {
  props: ['message'],
  data(){
...
<p>И в данном случае Vue 3 использовало свойство message из data, а не из параметра props, тем не менее в консоли мы бы увидели предупреждение, 
что свойство тем же именем также определено и в параметре props. Поэтому такой двойственности лучше избегать.</p>

<h3>Динамические свойства</h3>
<p>В примере выше свойству message передается статическое литеральное значение - строка "hello". Однако можно также устанавливать значения свойств динамически 
в зависимости от введенных в поля ввода данных:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Теперь свойство message устанавливается динамически в зависимости от введеного значения:</p>
<img src="./pics/45.png" alt="Динамические свойства props в Vue 3" />
<p>Для этого применяется механизм привязки - перед атрибутом указывается директива v-bind, которая определяет, к какому значению идет привязка:</p>
<pre class="brush:xml;">&lt;message-comp v-bind:message="welcome"&gt;&lt;/message-comp&gt;</pre>
<p>Также можно использовать сокращенную форму:</p>
<pre class="brush:xml;">&lt;message-comp :message="welcome"&gt;&lt;/message-comp&gt;</pre>
<h3>Привязка к сложным объектам</h3>
<p>Допустим, наш компонент выводит данные о пользователе - имя и возраст. Передадим эти данные в компонент:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь компонент получает значения для своих свойств name и age.</p>
<img src="./pics/46.png" alt="Передача в компоненты через props сложных объектов в Vue 3" />
<p>Но в данном случае свойства name и age можно оформить как единый объект и передавать этот объект целиком в компонент:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
...
<p>С помощью атрибута <code>v-bind="user"</code> объект user сразу передается компоненту и автоматически сопоставляется со свойствами name и age, 
...
результат работы будет тот же самый, что и в предыдущем случае. В то же время несмотря на то, что вроде бы идет привязка к объекту user, тем не менее компонент получает значения его свойств по отдельности.</p>
