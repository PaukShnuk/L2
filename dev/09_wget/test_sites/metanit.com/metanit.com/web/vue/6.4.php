...
<html >
<head>
<title>Vue  3 | Вложенные маршруты</title>
<meta charset="utf-8" />
<meta name="description" content="Вложенные маршруты (nested routes) во Vue 3, выбор между главным и дочерним маршрутом">
...
<h2>Вложенные маршруты</h2><div class="date">Последнее обновление: 04.11.2020</div>
...
<p>Одни маршруты могут быть вложенными по отношению к другим маршрутам (так называемые nested routes), то есть являются 
подмаршрутами. Для их использования определим следующую веб-страницу index.html:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь для одного маршрута установлено два подмаршрута. Для этого применяется свойство <span class="b">children</span>, которое в качестве 
параметров принимает массив подмаршрутов:</p>
<pre class="brush:js;">
{ 
	path: '/products', 
    component: Products,
...
<p>Родительский маршрут будет использовать компонент Products. Поэтому в шаблоне этого компонента определяется элемент <code></code>, 
в который будут помещаться компоненты, которые выбраны для обработки подмаршрутов:</p>
<pre class="brush:xml;">template: '&lt;div&gt;&lt;h1&gt;Товары&lt;/h1&gt;&lt;router-view&gt;&lt;/router-view&gt;&lt;/div&gt;'</pre>
<p>В итоге пути "products/tablets" и "products/phones" будут сопоставляться с подмаршрутами:</p>
<img src="./pics/router7.png" alt="Вложенные маршруты nested routes в vue 3" />
<p>Также можно задать подмаршрут, который будет обрабатывать запросы к корню родительского маршрута. Такой подмаршрут в качестве значения свойства 
path должен иметь пустую строку:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае компонент Index будет обрабатывать запросы по пути "/products":</p>
<img src="./pics/router9.png" alt="child routes in vue router" />
...
<div class="nav"><p><a href="6.8.php">Назад</a><a href="./">Содержание</a><a href="6.5.php">Вперед</a></p></div></div>
<div class="socBlock">
