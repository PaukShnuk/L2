...
<html >
<head>
<title>Vue 3 | Привязка данных</title>
<meta charset="utf-8" />
<meta name="description" content="Привязка данных в Vue3, интерполяция, директивы v-bind, v-once и v-html, привязки кода html и атрибутов">
...
<h2>Привязка данных</h2><div class="date">Последнее обновление: 31.10.2020</div>
...
<p>Vue 3 позволяет декларативным образом устанавливать привязку между элементами веб-страницы и данными объекта Vue. Есть различные формы привязки.</p>
<h3>Интерполяция</h3>
<p>Простейшую форму привязки представляет интерполяция - значение, к которому выполняется привязка, заключается в двойные фигурные скобки:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>При рендеринге выражения со скобками {{}} будут заменяться соответствующими значениями:</p>
<img src="./pics/3.png" alt="Привязка и интерполяция в Vue 3" />
<p>При этом в интерполяции могут использоваться любые валидные выражения JavaScript.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/4.1.png" alt="Интерполяция выражений JavaScript в Vue 3" />
<p>Также мы можем выводить значение, которое возвращает метод объекта Vue:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/4.2.png" alt="Интерполяция методов объектов Vue 3" />
<p>В данном случае метод sayHi просто возвращает некоторую строку. А вот метод welcome принимает параметр и в зависимости от значения этого параметра возвращает 
определенное сообщение. Соответственно при интерполяции мы можем передавать в метод разные параметры, тем самым влияя на его результат.</p>
<h3>v-text</h3>
<p>Для вывода простых значений в качестве альтернативы интерполяции можно использовать атрибут <span class="b">v-text</span> - ему передается 
значение, которое необходимо вывести в html-элемент:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<h3>Привязка к атрибутам</h3>
<p>Для привязки к атрибутам html-элементов предназначена директива <span class="b">v-bind</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Для привязки к атрибуту после v-bind через двоеточие указывается непосредственно сам атрибут, к которому надо выполнять привязку.</p>
<img src="./pics/4.png" alt="Привязка к атрибутам в Vue.js" />
<p>Для привязки атрибутов также можно использовать сокращенную форму:</p>
<pre class="brush:xml;">&lt;a :href="link" :title="title"&gt;{{text}}&lt;/a&gt;</pre>
<h3>Однократная привязка</h3>
<p>По умолчанию, если значение свойства меняется, то элементы визульного интерфейса обновляются и отображают обновленное значение. 
Но если необходимо, чтобы при изменении значения свойства элемент html отображал только первое значение свойства до его изменения, 
то применяется директива <span class="b">v-once</span/>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае несмотря на изменение значения свойства message на веб-странице будет отображаться его старое значение - строка "Hello". Данная возможность может 
быть полезна, если нам необходимо выводить на веб-страницу изначальное значение до всех его изменений.</p>
<h3>Привязка к html</h3>
<p>Для привязки элемента к коду html применяется директива <span class="b">v-html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>С помощью директивы v-html привязываемое значение будет рассматриваться как код html. В то же время при простой интерполяции код html будет интерпретироваться 
как обычная строка:</p>
<img src="./pics/5.png" alt="Привязка html в Vue 3" />
