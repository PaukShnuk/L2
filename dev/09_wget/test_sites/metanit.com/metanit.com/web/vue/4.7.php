...
<html >
<head>
<title>Vue 3 | Передача в компонент массивов и сложных объектов</title>
<meta charset="utf-8" />
<meta name="description" content="Передача массивов и сложных объектов через props в компоненты во Vue 3">
...
<h2>Передача массивов и сложных объектов</h2><div class="date">Последнее обновление: 03.11.2020</div>
...
<p>Отдельно рассмотрим передачу в компонент через props сложных объектов и массивов.</p>
<p>Например, передадим сложный объект:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь компонент получает данные в целом именно как один объект user. И после этого в шаблоне компонента можно обращаться к свойствам этого объекта через 
user.name и user.age.</p>
<img src="./pics/48.png" alt="Передача сложных объектов JavaScript через Props в компонент в Vue 3" />
<p>Но если мы не передадим значение для свойства user из props:</p>
<pre class="brush:xml;">&lt;user-info&gt;&lt;/user-info&gt;</pre>
<p>То в этом случае свойство user в компоненте будет неопределено, и программа завершит свою работу с ошибкой. В этом случае мы можем предусмотреть 
значение по умолчанию для этого свойства с помощью параметра <span class="b">default</span>. При этом для сложных объектов, а также для массивов этот параметр должен представлять функцию, которая 
возращает начальное значение для свойства.</p>
<p>В частности, изменим компонент следующим образом:</p>
<pre class="brush:js;">
app.component('user-info', {
  props: {
        user: {
...
<p>Теперь если извне не передается значение для props, программа будет использовать значение по умолчанию.</p>
<h3>Массивы</h3>
<p>Рассмотрим передачу массивов:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь извне компоненту передается массив users, состоящий из сложных объектов. В компоненте мы можем с помощью директивы v-for пробежаться 
по всему массиву и получить отдельные его элементы.</p>
<img src="./pics/49.png" alt="Передача массивов объектов через свойство props в компонент Vue 3" />
<p>Также можно выполнить более точную настройку свойства users, например, задать для него значение по умолчанию:</p>
<pre class="brush:js;">
app.component('userslist', {
	props: {
		users:{
