...
<html >
<head>
<title>Vue 3 | Программная навигация</title>
<meta charset="utf-8" />
<meta name="description" content="Программная навигация во Vue 3, метод router.push, передача параметров при переадресации">
...
<h2>Программная навигация</h2><div class="date">Последнее обновление: 05.11.2020</div>
...
<p>Для перехода к ресурсам внутри приложения Vue мы можем создавать ссылки с помощью элемента <code>&lt;<router-link>&gt;</code>. Однако иногда 
...
метод <span class="b">this.$router.push()</span>, в который передается информация о ресурсе, на который надо выполнить переход.</p> 
...
<p>В данном случае в компоненте Home по нажатию на кнопку выполняется переход по пути "/about" (то есть по сути к компоненту About):</p>
<pre class="brush:js;">router.push('/about')</pre>
<img src="./pics/router13.png" alt="Программная навигация в Vue 3" />
<p>В метод <code>router.push()</code> можно передавать не только прямой путь к другому ресурсу, но и другую информацию о ресурсе, на который надо ввыполнить переход:</p>
<pre class="brush:js;">
// простой путь
router.push('/about')
...
<p>Стоит учитывать, что если указан путь с помощью опция <code>path</code>, то опция <code>params</code>, которая представляет передаваемые параметры, 
игнорируется. Однако в связки с опцией path можно использовать опцию <code>query</code>.</p>
<p>Рассмотрим другой пример с передачей параметра и строки запроса:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае в компоненте Home по нажатию на кнопку вызывается метод</p>
<pre class="brush:xml;">router.push({ name: 'product', params: { cat: 'phone' }, query:{id: '1'} });</pre>
<p>Он приведет к переходу к маршруту с именем "product", который обрабатывается компонентом Product, а фактически к адресу "products/phone?id=1".</p>
<img src="./pics/router14.png" alt="метод router.push в Vue 3" />
