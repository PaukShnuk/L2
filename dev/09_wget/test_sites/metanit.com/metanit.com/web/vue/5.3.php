...
<html >
<head>
<title>Vue 3 | Слоты с ограниченной областью видимости</title>
<meta charset="utf-8" />
<meta name="description" content="Слоты с ограниченной областью видимости (scoped slots) во Vue 3, передача данных из дочернего компонента в родительский">
...
<h2>Слоты с ограниченной областью видимости</h2><div class="date">Последнее обновление: 04.11.2020</div>
...
<p>Иногда возникает необходимость использовать данные дочернего компонента в содержимом родительского компонента, которое 
передается в дочерний. Что-то наподобие:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
    &lt;user&gt;
...
<p>Данный пример, конечно, не будет работать, потому что для родительского компонента - в данном случае объекта приложения Vue объект user не существует, 
соответственно мы не можем обратиться к его свойствам name и age. Для решения этой задачи воспользуемся слотами с ограниченной областью видимости.</p>
<p>Слоты с ограниченной областью видимости (scoped slots) позволяют передавать данные между слотом и родительским компонентом. 
Подобные слоты определяют шаблон, в который можно передавать данные из дочернего компонента. И эти данные ограничены слотом.</p>
<p>Для определения шаблона в родительском компоненте применяется элемент <code>&lt;template&gt;</code>, 
в котором устанавливается специальный атрибут <span class="b">v-slot:имя_слота="данные"</span>. Значение этого атрибута - название объекта с данными, 
которые передаются от дочернего компонента.</p>
...
<p>К примеру, дочерний компонент через слот передает данные пользователя:</p>
<pre class="brush:xml;">&lt;slot v-bind:userinfo="user"&gt;&lt;/slot&gt;</pre>
<p>Здесь устанавливается привязка. То есть через слот в родительский компонент мы передаем объект user. В родительском компоненте мы можем получить этот объект 
через userinfo.</p>
<p>Для слота с ограниченной областью видимости в родительском компоненте определен шаблон:</p>
<pre class="brush:xml;">
&lt;template v-slot:default="props"&gt;
	&lt;h3&gt;Данные о пользователе&lt;/h3&gt;
	&lt;p&gt;Имя: {{ props.userinfo.name }}&lt;/p&gt;
...
<p>В дочернем компоненте для слота не указано имя, поэтому по умолчанию его имя будет "default". Поэтому для получения данных из дочернего 
компонента применяется атрибут <code>v-slot:default</code>, то есть v-slot:название_слота. Значение этого атрибута <code>props</code>, 
...
<code>props.userinfo.name</code>.</p>
<img src="./pics/62.png" alt="Слоты с ограниченной областью видимости Scoped slots in Vue 3" />
<p>Другой показательный пример представляет использование списков, где родительский компонент может определить шаблон для элемента списка:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае компонент userslist выводит список users. Для каждого элемента из этого списка создается свой элемент slot:</p>
<pre class="brush:xml;">
&lt;slot name="userdetails" v-bind:userinfo="user"&gt;&lt;/slot&gt;
</pre>
<p>Для слота определяется одно свойство - userdetails. Каждый объект user из массива передается в родительский компонент через ссылку userinfo.</p>
<p>В родительском компоненте определяется шаблон, в котором выводится каждый объект user:</p>
<pre class="brush:xml;">
&lt;template v-slot:userdetails="props"&gt;
	&lt;div&gt;
		&lt;p&gt;Имя: {{ props.userinfo.name}}&lt;/p&gt;
...
<p>Причем в данном случае родительский компонент сам определяет шаблон для элементов списка и правила их стилизации.</p>
<img src="./pics/63.png" alt="Передача списков через scoped slot в компонентах в Vue 3" />
