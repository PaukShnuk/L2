...
<html >
<head>
<title>Vue 3 | Слоты</title>
<meta charset="utf-8" />
<meta name="description" content="Слоты (slots) во Vue 3, передача содержимого из родительского компонента в дочерний">
...
<h1>Слоты</h1><h2>Введение в слоты</h2><div class="date">Последнее обновление: 04.11.2020</div>
...
<p>Слоты предоставляют механизм создания фиксированной структуры компонента, в которой содержимое для различных частей компонента 
определяет родительский компонент. Во Vue 3 слоты реализуются через элемент <span class="b">&lt;slot&gt;</span>, вместо которого 
...
<p>Здесь в коде html внутри элемента <code>&lt;child-component&gt;</code>, который представляет дочерний компонент, определено некоторое 
содержимое, а именно заголовок h2:</p>
<pre class="brush:xml;">&lt;child-component&gt;&lt;h2&gt;Vue App&lt;/h2&gt;&lt;/child-component&gt;</pre>
<p>Однако при рендеринге Vue полностью заменяет этот элемент со всем его содержимым на шаблон компонента, определенный параметром template:</p>
<img src="./pics/60.1.png" alt="Как работают слоты во Vue 3" />
<p>То есть в данном случае код родительского компонента, в данном сучае приложения Vue, помещенный внутрь элемента дочернего компонента, никак не влияет на 
рндеринг. Теперь применим слоты. Для их применения в произольном месте шаблона дочернего компонента вставляется элемент <code>&lt;slot&gt;&lt;/slot&gt;</code>:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
    &lt;child-component&gt;&lt;h2&gt;Vue App&lt;/h2&gt;&lt;/child-component&gt;
&lt;/div&gt;
...
<p>При рендеринге вместо элемента slot в шаблоне дочернего компонента будет помещаться тот контент, который передал родительский компонент в дочерний - 
в данном случае это заголовок h2 <code>&lt;h2&gt;Vue App&lt;/h2&gt;</code>:</p>
<img src="./pics/60.2.png" alt="Элемент slot и использование слотов в компонентах во Vue 3" />
<p>При этом содержимое, помещаемое в слоты, может быть более сложным, нежели один элемент (как в даном случае заголовок h2), то есть этом может быть 
группа элементов. Кроме того, в шаблоне дочернего компонента мы можем много раз использовать элемент <code>&lt;slot&gt;</code>, и все эти вставки будут 
заменяться контентом, переданным из родительского компонента:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
...
<img src="./pics/60.3.png" alt="Множественное использование слотов в компонентах во Vue 3" />
<h3>Содержимое слотов по умолчанию</h3>
<p>Может иметь место ситуация, когда родительский комопонент не предоставляет для дочернего никакого содержимого для слота в дочернем компоненте. 
В этом случае дочерний компонент может определить некоторое содержимое по умолчанию:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<pre class="brush:xml;">&lt;slot&gt;Здесь могла бы быть ваша реклама&lt;/slot&gt;</pre>
...
<img src="./pics/60.png" alt="Слоты в Vue 3" />
