...
<html >
<head>
<title>Vue 3 | Именованные слоты</title>
<meta charset="utf-8" />
<meta name="description" content="Именованные слоты (named slots) во Vue 3, передача данных из дочернего компонента в родительский">
...
<h2>Именованные слоты</h2><div class="date">Последнее обновление: 04.11.2020</div>
...
<p>Иногда возникает необходимость определить в компоненте несколько слотов, которые могли бы иметь 
...
Для этого в элементе <code>&lt;slot&gt;</code> используется атрибут <span class="b">name</span>.</p>
...
&lt;div id="app"&gt;
	&lt;container&gt;
		...
		&lt;template v-slot:footer&gt;
			&lt;p&gt;Для более подробной информации напишите по адресу contacts@gmail.com&lt;/p&gt;
		&lt;/template&gt;
	&lt;/container&gt;
&lt;/div&gt;
...
<p>Компонент container определяет ряд слотов. Причем один из слотов является обычным, а остальные два - именованными. Элементы <code>&lt;slot&gt;</code>, у которых явным образом не установлено имя (то еть атрибут name), 
по умолчанию имеют имя "default".</p>
<pre class="brush:xml;">
&lt;div&gt;
	&lt;header&gt;
		&lt;slot name="header"&gt;&lt;/slot&gt;
...
<p>То есть дочерний компонент определяетт структуру расположения контента. В родительском компоненте для слотов передается содержимое, которое 
оборачивается в элемент <code>&lt;template&gt;</code>. У этого элемента атрибут <code>v-slot</code> принимает имя слота, на место которого 
необходимо вставить содержимое.</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
	&lt;container&gt;
		&lt;template v-slot:header&gt;
...
<p>Например, содержимое элемента <code>&lt;template v-slot:header&gt;</code> будет вствляться на место слота с именем header, а элемент 
<code>&lt;template v-slot:default&gt;</code> предоставляет контент для слота без имени (по умолчанию имя default).</p>
<p>То есть в итоге сформируется следующая разметка:</p>
<pre class="brush:xml;">
&lt;div&gt;
  &lt;header&gt;
    &lt;h1&gt;Заголовок статьи&lt;/h1&gt;
...
<img src="./pics/61.png" alt="Именнованные слоты Named slots в Vue 3" />
<p>Также в дочернем компоненте можно предоставить контент по умолчанию, если родительский контейнер не предоставил для слотов никакого содержимого:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
app.component('container', { 
    template: `&lt;div&gt;
                &lt;header&gt;
                    &lt;slot name="header"&gt;&lt;h2&gt;Заголовок по умолчанию&lt;/h2&gt;
                    &lt;p&gt;Дата статьи: {{ new Date().toLocaleDateString() }}&lt;/p&gt;&lt;/slot&gt;
                &lt;/header&gt;
                &lt;article&gt;
                    &lt;slot&gt;&lt;/slot&gt;
                &lt;/article&gt;
                &lt;footer&gt;
                    &lt;slot name="footer"&gt;&lt;h5&gt;Copyright©2020&lt;/h5&gt;&lt;/slot&gt;
                &lt;/footer&gt;
            &lt;/div&gt;` 
});
app.mount('#app');
...
<img src="./pics/61.1.png" alt="Имена слотов в компонентах во Vue 3" />
