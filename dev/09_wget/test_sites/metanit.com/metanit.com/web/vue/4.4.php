...
<html >
<head>
<title>Vue 3 | Разделяемое состояние компонентов</title>
<meta charset="utf-8" />
<meta name="description" content="Разделяемое состояние и общие данные компонентов во Vue 3, особенности использования">
...
<h2>Разделяемое состояние компонентов</h2><div class="date">Последнее обновление: 03.11.2020</div>
...
<p>Для каждого компонента можно определить его собственное состояние. Но также можно определять некоторое общее состояние для нескольких компонентов. 
Для этого состояние определяется в виде внешнего объекта:</p>
<pre class="brush:js;">
let globalData = {
...
<p>Конкретно в данном примере у нас не будет разницы от вынесения объект data во вне. В то же время между двумя способами определения data все таки есть разница.</p>
...
<p>В данном случае приложение Vue использует два раза компонент counter. И каждый объект counter работает со своим состоянием. Изменение свойство count 
в одном компоненте никак не скажется на другом.</p>
<img src="./pics/42.png" alt="Состояние компонентов в Vue 3" />
<p>Рассмотрим другую ситуацию, когда компоненты используют одно и то же состояние:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В этом случае одно и то же состояние будет разделяться обоими компонентами. Поэтому изменения в состоянии в одном компоненте повлияют и на другой компонент.</p>
<img src="./pics/43.png" alt="Состояние компонентов в Vue 3" />
