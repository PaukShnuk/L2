...
<html >
<head>
<title>Vue 3 | Обработка ошибки 404/Not found</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка запросов к несущеcтвующим ресурсам во Vue 3, ошибка 404 и Not found">
...
<h2>Обработка ошибки 404/Not found</h2><div class="date">Последнее обновление: 04.11.2020</div>
...
<p>Распространенной задачей в веб-приложениях является обработка несущеcтвующих путей, когда приложение не может сопоставить запрошенный адрес ни с одним из ресурсов приложения. 
Рассмотрим, как это можно сделать во Vue 3.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Для обработки запросов к несуществующим ресурсам применяется компонент NotFound, который просто выводит соответствующую информацию:</p>
<pre class="brush:js;">const NotFound = { template: '&lt;h2&gt;Page Not Found&lt;/h2&gt;' }</pre>
<p>Этом компонент обрабатывает запросы по следующему маршруту:</p>
<pre class="brush:js;">
{ path: '/:pathMatch(.*)*', component: NotFound }
</pre>
<p>В данном случае шаблон маршрута выглядит несколько замыславато: "/:pathMatch(.*)*". Фактически он означает, что вся часть после слеша расценивается 
...
не соответствуют другие маршруты.</p>
<p>В итоге при обращении к несуществующему ресурсу запрос будет будет обрабатываться компонентом NotFound:</p>
<img src="./pics/router4.1.png" alt="Not Found in routing in Vue 3" />
