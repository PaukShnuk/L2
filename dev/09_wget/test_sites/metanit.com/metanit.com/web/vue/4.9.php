...
<html >
<head>
<title>Vue 3 | Получение состояния вложенного компонента</title>
<meta charset="utf-8" />
<meta name="description" content="Получение состояния вложенного компонента во Vue 3, объект this.$refs">
...
<h2>Получение состояния вложенного компонента</h2><div class="date">Последнее обновление: 03.11.2020</div>
...
<p>С помощью ссылки <span class="b">this.$refs</span> мы можем ссылаться на вложенные компоненты и, таким образом, обращаться к внутреннему состоянию компонента.</p>
<p>Например, изменим извне компонента одно из его свойств:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь для компонента userdetails с помощью атрибута <span class="b">ref</span> установлена ссылка details, через которую можно ссылаться на данный компонент.</p>
<pre class="brush:xml;">&lt;userdetails :user="user" ref="details"&gt;&lt;/userdetails&gt;</pre>
<p>В самом компоненте userdetails определено свойство visible, которое управляет видимостью частью шаблона компонента. Для изменения значения 
этого свойства в объекте Vue предусмотрена кнопка, по нажатию на которую срабатывает метод <code>toggle()</code>:</p>
<pre class="brush:js;">
toggle(){
	this.$refs.details.visible = !this.$refs.details.visible;
}
</pre>
<img src="./pics/51.png" alt="Изменение состояния компонентов  помощью $refs в Vue 3" />
<p>Стоит отметить, что ссылка <code>$refs</code> устанавливается только после рендеринга компонента. Поэтому не следует использовать 
$refs внутри шаблона компонента или в вычисляемых свойствах computed.</p>
