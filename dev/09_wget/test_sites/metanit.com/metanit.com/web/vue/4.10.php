...
<html >
<head>
<title>Vue 3 | Однонаправленный поток данных</title>
<meta charset="utf-8" />
<meta name="description" content="Однонаправленный поток данных в компонентах во Vue 3, изменение значений из props в дочернем компоненте">
...
<h2>Однонаправленный поток данных</h2><div class="date">Последнее обновление: 03.11.2020</div>
...
<p>При использовании props в компонентах следует учитывать, что данные в props представляют однонаправленный поток данных от родительского компонента к 
...
То есть поток данных идет только в одном направлении: родителя к потомкам. Props представяют свойства, доступные только для чтения.</p>
<p>Более того при любом обновлении родительского компонента у дочерних компонентов обновляются значения, передаваемые через props. 
При обратной ситуации, если мы попробуем изменить в дочерних компонентах значения, переданные через props, то на консоль браузера будет 
выведено предупреждение о том, что это не следует делать, а сами значения никак не изменятся. Например, рассмотрим следующую ситуацию:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Из родителя - объекта приложения Vue в компонент user-edit через props передается значение name. В этом компоненте мы можем попробовать управлять переданным значением через 
поле ввода и механизм двусторонней привязки:</p>
<pre class="brush:xml;">&lt;input type="text" v-model="user" /&gt;</pre>
<p>Но если мы попробуем изменить через поле ввода данное значение, то оно изменится только в рамках компонента user-edit, а на консоль будет выведено предупреждение:</p>
<img src="./pics/52.png" alt="Mutating props in Vue 3" />
<p>Однако могут быть ситуации, когда нам все таки нужно изменить значения в props. Что делать в этом случае? В этом случае мы можем определить локальную переменную. Через props устанавливается ее начальное значение, и затем компонент работает только с этой переменной:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Теперь все действия будут идти с локальной переменной userName. Значения из props в компоненте не будут изменяться, а изменения в родительском компоненте 
никак не повлияют на значение переменной userName.</p>
<img src="./pics/53.png" alt="Изменение значений из props в компонентах во Vue 3" />
<h3>Синхронизация значений родительского и дочернего компонентов</h3>
<p>Предложенное выше решение позволяет изменять внутри компонента данные, полученные от родительского компонента. Но тогда мы можем столкнуться с другой проблемой - 
а что, если родительский компонент также изменит данные:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае при измнении в родительском компоненте в дочерний компоненте по прежнему изменяется свойство "user" из props. Однако 
привязка элементов интерфейса идет не к свойству user из props, а к свойству userName из data. Поэтому компонент визуально никак не отразит изменения в 
родительском компоненте:</p>
<img src="./pics/53.1.png" alt="Изменение значений из props в родительском компоненте во Vue 3" />
<p>Что делать в этом случае? В этом случае мы можем отслеживать изменение user из props и соответственно обновлять свойство userName:</p>
<pre class="brush:xml;">
app.component('user-edit', {
    props: ["user"],
    data() {
...
<p>Теперь с помощью параметра watch определяется наблюдаемое свойство, которое называется также, как и отслеживаемое свойство user из props. 
Параметр newVal представляет новое значение, которое мы можем присвоить свойству userName и таким образом синхронизировать дочерний компонент с 
родительским.</p>
<img src="./pics/53.2.png" alt="Обновление данных props во Vue 3" />
