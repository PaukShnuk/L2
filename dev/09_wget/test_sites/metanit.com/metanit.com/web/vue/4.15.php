...
<html >
<head>
<title>Vue.js | Взаимодействие между сестринскими компонентами</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие между сестринскими компонентами одного уровня во Vue 3">
...
<h2>Взаимодействие между сестринскими компонентами.</h2><div class="date">Последнее обновление: 03.11.2020</div>
...
<p>Одной из частых задач при организации компонентов во Vue составляет организация взаимодействия между компонентами, 
которые находятся на одном уровне, то есть являются сестринскими.</p>
<p>В этом случае взаимодействие происходит через родительский компонент. Например, определим два дочерних компонента, которые взаимодействуют 
через родительский компонент:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Ключевым моментом здесь является то, что данные, которыми манипулируют оба компонента, хранятся как данные все приложения Vue. 
В данном случае оба дочерних компонента useredit и userinfo принимают от объекта приложения Vue имя пользователя. То есть оба компонента используют те данные, которые определены в объекте приложения Vue. 
Вместо объекта приложения Vue это мог бы быть другой компонент, который был бы родительским по отношению к userinfo и useredit.</p>
<p>Первый дочерний компонент - useredit получает данные от родителя через свойство <code>name</code></p>
<pre class="brush:js;">props: ["name"],</pre>
<p>Однако здесь мы сталкиваемся со следующей задачей. Параметр props передает свойства только для чтения, которые мы не можем изменять. 
Однако в компоненте useredit мы хотим изменять свойство name. Более того передавать измененное значение другому компоненту и при этом отслеживать, 
не изменил ли другой компонент сам значение свойства name. То есть необходимо передавать во вне изменения и при этом отслеживать, не произошли ли вне компонента изменения со 
свойством name.</p>
<p>Чтобы свойство name можно было изменять, компонент useredit определяет новое свойство username:</p>
<pre class="brush:js;">
data(){ 
		return { username: this.name }
	},
</pre>
<p>Это свойство username привязанно двусторонней привязкой к текстовому полю:</p>
<pre class="brush:xml;">&lt;input v-model="username" /&gt;&lt;button @click="save"&gt;Save&lt;/button&gt;</pre>
<p>При нажатии кнопки рядом с текстовым полем вызывается метод <code>save</code>, который, в свою очередь, 
генерирует событие <code>userchange</code>:</p>
<pre class="brush:js;">
save(){
	this.$emit("userchange", this.username);
}
</pre>
<p>Через событие userchange объект приложения Vue получит измененное имя пользователя и произведет необходимые изменения:</p>
<pre class="brush:js;">
change(name){
	this.user = name;
},
</pre>
<p>Чтобы компонент useredit также мог отслеживать внешние изменения со свойством name, он определяет наблюдаемое свойство watcher:</p>
<pre class="brush:js;">
watch: { 
      	name(newVal) {
          this.username = newVal;
...
<p>В качестве параметра свойству передается новое значение свойства name. В итоге при всех изменениях сработает это свойство, что приведет к 
переустановке значения в username.</p>
<p>Второй дочерний компонент - userinfo также через свойство name из props получает от приложения Vue имя пользователя:</p>
<pre class="brush:js;">props: ["name"]</pre>
<p>Однако этот компонент напрямую не изменяет значение name, а при нажатию на кнопку генерирует событие <code>userreset</code></p>
<pre class="brush:js;">
reset(){
	this.$emit("userreset");
}
</pre>
<p>Приложение Vue отслеживает это событие и при его возникновении сбрасывает имя пользователя в изначальное:</p>
<pre class="brush:js;">
reset(){
	this.user = 'Tom';
},
</pre>
<p>То есть объект приложения Vue здесь служит медиатором, а непосредственно между собой компоненты не взаимодействуют.</p>
<img src="./pics/59.png" alt="Взаимодействие между сестринскими компонентами одного уровня во Vue 3" />
...
<div class="nav"><p><a href="4.13.php">Назад</a><a href="./">Содержание</a><a href="4.14.php">Вперед</a></p></div></div>
<div class="socBlock">
