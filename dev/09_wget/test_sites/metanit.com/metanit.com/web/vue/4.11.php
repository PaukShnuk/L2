...
<html >
<head>
<title>Vue 3 | Создание событий</title>
<meta charset="utf-8" />
<meta name="description" content="Передача данных из дочернего компонента в родительский во Vue 3, определение своих событий, метод this.$emit, модификатор sync">
...
<h2>Создание событий</h2><div class="date">Последнее обновление: 03.11.2020</div>
...
<p>Через props мы можем передать данные от родительского компонента в дочерний. Но что если мы хотим также передавать данные и в обратном направлении: от дочернего 
компонента к родителю? В этом случае необходимо определить свои события.</p>
<p>Общая схема создания и вызова кастомного события выглядит следующим образом. Родительский компонент устанавливает у дочернего компонента 
атрибут <span class="b">v-on:название_события</span>:</p>
<pre class="brush:xml;">&lt;child-component v-on:myevent="action"&gt;&lt;/child-component&g;</pre>
<p>Где <code>myevent</code> - название события, которые мы сами определяем и которое будет отлавливать родительский компонент. А <code>action</code> - 
некоторы действия, которые будут вызываться при возникновении события myevent.</p>
<p>Дочерний компонент, в свою очередь, будет генерировать событие с помощью вызова в коде <span class="b">this.$emit(myevent)</span>, 
где "myevent" - это имя обытия, которое отслеживает родительский компонент с помощью атрибута <code>v-on:myevent="action"</code>.</p>
...
<p>В данном случае определяется событие <span class="b">userchange</span>. В реальности в JavaScript не существует подобного события, мы его определяем сами.</p>
<p>В шаблоне компонента элемент ввода связан двусторонней привязкой со значением со свойством userName. Также для этого элемента установлен обработчик события input:</p>
<pre class="brush:xml;">&lt;input type="text" v-model="userName" v-on:input="onUserChange" /&gt;</pre>
<p>То есть при вводе в это текстовое поле будет вызываться метод onUserChange. В этом методе через вызов <code>this.$emit('userchange', this.userName);</code> 
...
в текстовое поле изменится и свойство user).</p>
<pre class="brush:js;">
onUserChange(){
	this.$emit('userchange', this.userName);
}
</pre>
<p>Первый параметр метода <code>this.$emit()</code> должен представлять название события. В качестве последующих параметров мы можем передать произвольное 
количество данных - в данном случае только обновленное имя пользователя.</p>
<p>Чтобы отловить это событие в объекте Vue на дочернем компоненте определен атрибут <code>v-on:userchange</code>:</p>
<pre class="brush:xml;">&lt;useredit :user="name" v-on:userchange="change"&gt;&lt;/useredit&gt;</pre>
<p>То есть поскольку в дочернем компоненте генерируется событие userchange, то соответственно здесь и атрибут имеет определение <code>v-on:userchange</code>. 
Этому событию сопоставлен метод change:</p>
<pre class="brush:js;">
change(value){
	this.name = value;
}
</pre>
<p>Параметр value представляет то значение, которое передается в дочернем компоненте через вызов <code>this.$emit('userchange', this.userName);</code>, 
то есть this.userName.</p>
<p>В итоге при вводе данных в дочернем компоненте useredit сработает обработчик события input - метод onUserChange, который сгенерирует событие 
namechange, которое в свою очередь будет обработано в родительском объекте в методе change.</p>
<img src="./pics/54.png" alt="Определение своих событий для взаимодействия родительского и дочернего компонентов в Vue 3" />
