...
<html >
<head>
<title>Vue 3 | Модификаторы полей ввода</title>
<meta charset="utf-8" />
<meta name="description" content="Модификаторы ввода во Vue 3, .lazy, .number, .trim, объединение модификаторов">
...
<h2>Модификаторы полей ввода</h2><div class="date">Последнее обновление: 02.11.2020</div>
...
<p>Фреймворк Vue для работы с формами предоставляет также вспомогательный инструмент - модификаторы полей ввода, который 
позволяют управлять некоторыми моментами при вводе в элементы форм.</p>
<h3>.lazy</h3>
<p>По умолчанию директива <span class="b">v-model</span> обновляет связанные с полем ввода данные при 
каждом вызове события <span class="b">input</span>. То есть при вводе каждого символа в текстовое поле будут изменяться связанные данные. 
Но вместо этого мы также можем вызывать обновление не после события input, а после события <span class="b">change</span>. То есть связанные 
данные будут обновляться только после того, как текстовое поле потеряет фокус. Для этого применяется модификатор <span class="b">.lazy</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<h3>.number</h3>
<p>Модификатор <span class="b">.number</span> позволяет автоматически конвертировать при вводе вводимое значение в число. Поскольку даже если мы установим 
у поля input атрибут <code>type="number"</code>, то такое поле все равно будет возвращать строку:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
...
<h3>.trim</h3>
<p>Модификатор <span class="b">.trim</span> автоматически обрезает начальные и концевые пробелы при вводе строки:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
...
<h3>Объединение модификаторов</h3>
<p>Также можно объединять модификаторы:</p>
<pre class="brush:xml;">
