...
<html >
<head>
<title>Vue 3 | Изменение внешних массивов и объектов в компонентах</title>
<meta charset="utf-8" />
<meta name="description" content="Изменение внешних массивов и объектов в компонентах во Vue 3, стратегии взаимодействия дочернего и родительского компонентов">
...
<h2>Изменение внешних массивов и объектов в компонентах</h2><div class="date">Последнее обновление: 03.11.2020</div>
...
<p>В отличие от строк, чисел, логических значений true и false массивы и сложные объекты передаются в компоненты по ссылке. 
То есть получив ссылку, можно управлять значением, которое хранится по этой ссылке.</p>
<h3>Изменение массивов</h3>
<p>Рассмотрим, как мы можем изменять в дочернем компоненте массивы, переданные из родительского компонента. Определим следующую веб-страницу:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Для добавления нового элемента в массив определен компонент userform, который представляет форму добавления. Для вывода одного элемента из 
массива определен компонент useritem, который также содержит кнопку для удаления элемента.</p>
<p>По сути здесь используются две различные техники для работы с массивами. Компонент userform получает ссылку на массив и при нажатии на кнопку вызывает метод <code>userAdd()</code>, 
в котором через полученную ссылку в массив добавляется новый элемент.</p>
<p>Компонент useritem использует другую технику. Он получает элемент для вывода и по нажатию на кнопку удаления генерирует событие userdelete, 
передавая с ним индекс удаляемого элемента. Родительский объект получает это событие с индексом и по индексу собственно удаляет элемент из массива.</p>
<p>То есть мы можем либо напрямую передавать ссылку в дочерний компонент и в нем изменять значения в массиве, либо генерировать в дочернем компоненте 
событие, которое передается родительскому объекту.</p>
<img src="./pics/56.png" alt="Изменение массивов родительского компонента в дочернем в Vue 3" />
<h3>Изменение объектов</h3>
<p>Для изменения внешних объектов в компоненте можно использовать аналогичные техники. В частности, изменим html-страницу следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь компонент userform через props получает объект user, свойства которого связываются двухсторонней привязкой с полями ввода. 
При изменении введеных значений в этих полях изменится и значение в массиве:</p>
<img src="./pics/55.png" alt="Изменение объектов родительского компонента в дочернем в Vue 3" />
