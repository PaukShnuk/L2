...
<html >
<head>
<title>Vue 3 | Двусторонняя привязка и директива v-model</title>
<meta charset="utf-8" />
<meta name="description" content="Двусторонняя привязка в Vue 3 и ее использование и ограничения, директива v-model, two-way data binding">
...
<h2>Двусторонняя привязка и директива v-model</h2><div class="date">Последнее обновление: 31.10.2020</div>
...
<p>Ранее были рассмотрены примеры использования односторонней привязки с помощью интерполяции. Но кроме того, 
Vue 3 двустороннюю привязку. Для создания подобной привязки используется директива <span class="b">v-model</span>. Однако стоит отметить, что эта директива 
может применяться только к html-элементам <code>&lt;input&gt;</code>, <code>&lt;select&gt;</code>, <code>&lt;textarea&gt;</code> и к компонентам Vue.</p>
<p>Рассмотрим на простом примере:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь на странице определено текстовое поле <code>input</code>, которое с помощью директивы <code>v-model</code> привязано к свойству userName 
...
оно изменится автоматически при изменении текста в текстовом поле. То есть сработает двусторонняя привязка.</p>
<img src="./pics/9.png" alt="Директива v-model в Vue 3" />
<p>То есть с одной стороны, текстовое поле получает значение из объекта Vue, а с другой стороны, объект Vue исходя из введенного в текстовое поле значения изменяет свое свойство.</p>
<p>Рассмотрим другой пример - определим программу вычисления площади прямоугольника:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае одно поле ввода привязано к свойству width, а другое - к свойству height. При введении значения в одно из этих полей автоматически будет перевычисляться площадь прямоугольника, которая выводится ниже.</p>
<img src="./pics/10.png" alt="Двусторонняя привязка и v-model в Vue 3" />
<p>Стоит отметить, что директива <span class="b">v-model</span> игнорирует значения атрибутов <code>value</code>, <code>checked</code> и <code>selected</code> полей ввода, 
а для установки начального значения следует использовать свойства объекта из кода javascript.</p>
