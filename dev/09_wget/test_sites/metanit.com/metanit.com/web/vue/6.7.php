...
<html >
<head>
<title>Vue 3 | Переадресация</title>
<meta charset="utf-8" />
<meta name="description" content="Переадресация в приложении Vue 3, параметр redirect в маршутах, динамический редирект, передача параметров при переадресации">
...
<h2>Переадресация</h2><div class="date">Последнее обновление: 04.11.2020</div>
...
<p>В процессе работы приложения может сложиться ситуация, что какой-то ресурс будет перемещен с одного адреса на другой. В этом случае возникает необходимость 
установить переадресацию с одного адреса на другой.</p>
<p>Для выполнения переадресации у маршрута применяется параметр <span class="b">redirect</span>, который задает путь для редиректа:</p>
<pre class="brush:js;">
...
<p>Например, определим маршрут, на который будут переадресованы пользователи при попытке обращения к несуществующим адресам:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь компонент Home обрабатывает запросы по пути "/". Если запрошен адрес "/home", то происходит переадресация по пути "/". 
Если запрошен путь, происходит перенаправление на маршруту "home" - то есть по сути опять же по пути "/", который обслуживается компонентом Home.</p>
<h3>Переадресация с параметрами</h3>
<p>При этом при переадресации можно передавать параметры:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
    &lt;router-view&gt;&lt;/router-view&gt;
&lt;/div&gt;
...
<p>Здесь определена переадресация с адреса "/product/:id" на адрес "/ru/product/:id". При этом значение параметра id автоматически будет передаваться 
с первого маршрута второму.</p>
<h3>Динамическая переадресация</h3>
<p>Иногда возникает ситуация, когда надо динамически решить, куда выполнять переадресацию. В этом случае можно задать функцию, которая динамически 
определяет, куда переадресовывать пользователя:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае в зависимости от значения параметра маршрута id, который можно получить через выражения to.params.id, вычисляется, 
на какой маршрут перенаправить пользователя. Причем опять же мы можем выполнить переадресацию либо по пути, либо по маршруту по имени:</p>
<pre class="brush:js;">
{ path: '/product/:id', redirect: to =&gt; {
     if (to.params.id &gt; 10) {
        return '/404'
...
<p>В данном случае если id больше 10, идет переадресация о пути "/404" - по сути на компонент NotFound, а если id меньше 10, то идет переадресация 
на маршрут с именем "product", то есть по сути на компонент Product.</p>
...
<div class="nav"><p><a href="6.6.php">Назад</a><a href="./">Содержание</a><a href="6.9.php">Вперед</a></p></div></div>
<div class="socBlock">
