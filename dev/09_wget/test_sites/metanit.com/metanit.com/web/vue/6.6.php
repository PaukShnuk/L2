...
<html >
<head>
<title>Vue 3 | Именованные представления</title>
<meta charset="utf-8" />
<meta name="description" content="Именованные представления named views во Vue 3, одновременное использование нескольких компонентов router-vue">
...
<h2>Именованные представления</h2><div class="date">Последнее обновление: 04.11.2020</div>
...
const Footer = { template: '&lt;div&gt;&lt;p&gt;Copyright&copy;2020 All rights reserved.&lt;/p&gt;&lt;/div&gt;' }
...
<p>Первое представление имя "navigation". Второе представление не имеет имени. В этом случае неявно ему присваивается имя по умолчанию 
"default". Третье представление называется "footer".</p>
...
через который для каждого представления можно определить свой компонент. Так, для представления navigation будет использоваться компонент 
NavBar, который хранит набор ссылок.</p>
<p>Представление, которое не имеет имени, доступно здесь через псевдоним "default". Оно будет использовать компонент Home.</p>
<p>При этом необязательно для каждого представления устанавливать свой компонент. Так, в данном случае определяем сопоставление только для 
двух представлений - default и navigation.</p>
<p>Аналогично выглядит второй маршрут, только в нем компоненты устанавливаются для всех трех представлений:</p>
<pre class="brush:js;">
{ 
    path: '/about',
    components: {
...
<p>В данном случае представление navigation опять связывается с компонентом NavBar, для представления default применяется компонент About, 
а для предствления footer - одноименный компонент Footer.</p>
<p>Таким образом, при обращении по пути "/" отобразится набор навигационных ссылок и содержимое компонента Home:</p>
<img src="./pics/router11.png" alt="Именованные представления Named views в Vue 3" />
<p>А при переходе по пути "/about" будут отображаться все три представления с определенной информацией:</p>
<img src="./pics/router12.png" alt="Именованные представления во Vue 3" />
