...
<meta charset="utf-8" />
<meta name="description" content="Обработка событий в React, объект this в обработчиках событий, получение информации о событии">
...
<h2>События</h2><div class="date">Последнее обновление: 01.04.2022</div>
...
<p>Обработка событий элементов в React во многом похожа на обработку событий элементов DOM с помощью обычного JavaScript. 
В то же время есть небольшие отличия:</p>
<ul>
<li><p>События в React используют camelCase (в стандартном html "onclick", в React - "onClick")</p></li>
<li><p>В JSX в обработчик события передается функция компонента, а не строка</p></li>
</ul>
<p>В React можно применять разные способы определения и вызова событий. Возьмем простейшую задачу - обработка нажатия кнопки.</p>

<h3>События в функциональных компонентах</h3>
<p>Использование событий в функциональных компонентах несколько проще. Так, перепишем предыдущий компонент в функциональном стиле:</p>
<pre class="brush:js;">
...
<p>Здесь используется событие нажатия кнопки, которое задается через атрибут <span class="b">onClick</span> (не <code>onclick</code>).</p>
<p>Этому атрибуту в качестве обработчика события передавалась функция <code>press</code>, которая определена в функции компонента. 
И при нажатии на кнопку будет вызываться функция <code>press</code>, которая с помощью функции <code>alert</code> отображает окно с некоторым уведомлением.</p>
<p>Использование функционального компонента:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/2.8.png" alt="Обновление состояния и события в React" />
<h3>События в компонентах-классах</h3>
<p>При определении событий в классах-компонентах распространенный подход заключается в установке привязки события в конструкторе компонента:</p>
<pre class="brush:js;">
...
<p>Главная сложность при использовании событий в компонентах-классах - это работа с ключевым словом <span class="b">this</span>, которое указывает на текущий 
объект, в данном случае компонент. По умолчанию в функцию обработчика не передается текущий объект, поэтому <code>this</code> будет иметь значение <code>undefined</code>. И ни к каким свойствам и методам компонента 
...
прописывается вызов:</p>
<pre class="brush:js;">this.press = this.press.bind(this);</pre>
<p>И также стоит отметить, что при определении конструктора компонента в нем должен вызываться конструктор базового класса, в который передается объект <code>props</code>.</p>
<p>Использование класса-компонента на веб-странице аналогично:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Однако есть и другие способы определения и вызова события. Например, определение обработчика в виде публичного поля компонента, которое указывает на стрелочную 
функцию.</p>
<pre class="brush:js;">
class ClickButton extends React.Component {
...
<p>Либо мы можем определить функцию обработчика события как обычный метод класса, а вызывать с помощью стрелочной функции:</p>
<pre class="brush:js;">
class ClickButton extends React.Component {
...
<p>Однако в случае с использованием стрелочной функции есть вероятность столкнуться с проблемой производительности, если функция обработчика передается через свойства 
<span class="b">props</span> вложенным компонентам. Так как обработчик события будет создаваться каждый раз заново при каждом рендеринге компонента, что 
может привести к дополнительному повторному рендерингу вложенных компонентов, без которого можно было бы обойтись. Поэтому использование конструктора является более предпочтительной практикой.</p>

<h3>Получение информации о событии</h3>
<p>React использует концепцию SyntheticEvent - специальных объектов, которые представляют собой обертки для объектов событий, передаваемых в функцию события. И используя такой объект, мы можем 
получить в обработчике события всю информацию о событии:</p>
<pre class="brush:js;">
function ClickButton(props) {
...
<p>Параметр <span class="b">e</span> - это и есть информация о событии, которая передается в обработчик системой и которую мы можем использовать при обработке.</p>
<p>Аналогичный пример для класса-компонента:</p>
<pre class="brush:js;">
class ClickButton extends React.Component {
...
<p>Если необходимо передать в обработчик события некоторые аргументы, то в этом случае можно вызвать обработчик через стрелочную функцию:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/2.15.png" alt="Передача параметров в обработчик события в React" />
<p>Аналогичный пример для класса-компонента:</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./2.3.php">Назад</a><a href="./">Содержание</a><a href="./2.9.php">Вперед</a></p></div></div>
<div class="socBlock">
