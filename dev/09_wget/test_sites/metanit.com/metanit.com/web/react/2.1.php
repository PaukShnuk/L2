...
<html >
<head>
<title>React | Рендеринг элементов</title>
<meta charset="utf-8" />
<meta name="description" content="Рендеринг элементов в React, определение и обновление элемента">
...
<h2>Рендеринг элементов</h2><div class="date">Последнее обновление: 01.04.2022</div>
...
<p>Мельчайшими блоками приложения React являются элементы. Определение простейшего элемента:</p>
<pre class="brush:xml;">const element = &lt;h1&gt;Hello, React&lt;/h1&gt;;</pre>
<p>По сути элементы в React - это обычные объекты JavaScript, с которыми быстрее работать, чем с обычными элементами на веб-странице.</p>
<p>Для рендеринга элементов в React вначале необходимо определить корневой элемент, в который будет производиться рендеринг. Для этого применяется метод 
<span class="b">ReactDOM.createRoot()</span>. В него передается элемент веб-страницы, в котором будет производиться 
рендеринг</p>
<pre class="brush:xml;">
...
<p>Здесь константа <code>rootNode</code> представляет некоторый элемент html-страницы с <code>id="app"</code>. И в данном случае именно в этот элемент 
будет производиться рендеринг приложения React. Результатом метода <code>ReactDOM.createRoot</code> является 
объект ReactDOMRoot (в данном случае он представлен константой <code>root</code>), который представляет указатель на структуру данных, которую инфраструктура React 
использует для отслеживания дерева элементов для рендеринга.</p>
<p>Непосредственно для рендеринга у объекта ReactDOMRoot вызывается метод <span class="b">render()</span>. В этот метод 
передается компонент, который мы хотим отобразить на веб-странице. Например, рендеринг заголовка.</p>
<pre class="brush:xml;">
root.render(
        &lt;h1&gt;Hello React&lt;/h1&gt;,  // элемент, который мы хотим создать
     )
</pre>
<p>Элемент React может иметь только один корневой элемент, в который уже вкладываются все остальные элементы. Например, создадим новый проект. Определим в проекте 
следующий файл <span class="b">index.html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае корневым элементом является <code>&lt;div id="items"&gt;</code>, который будет добавляться на веб-страницу в элемент с id=app.</p>
<img src="./pics/2.1.png" alt="ReactElement и элементы в React" />
<p>При этом в один элемент DOM можно добавить только один элемент ReactElement, например, добавим еще один элемент на веб-страницу:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/2.2.png" alt="Рендеринг элементов в React" />
<h3>Обновление элементов</h3>
<p>Элементы React являются неизменяемыми (immutable). После создания элемента нельзя изменить его 
атрибуты или дочерние элементы. И единственный способ изменить интерфейс, определенный в элементе, это создать новый элемент и передать его в 
функцию ReactDOM.render(). Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/2.13.png" alt="ReactElement и рендеринг и обновление элементов в React" />
<p>В данном случае в элементе с id=header будет отображаться текущее время, однако, чтобы каждую секунду изменялось отображаемое значение, там надо 
повторно вызывать функцию для элемента.</p>
<p>При обновлении элемента виртуальный React DOM сравнивает текущее состояние элемента с его предыдущим состоянием и при наличии изменений применяет их 
к веб-странице и обновляет стандартный DOM. Но обновляться будет не весь пересоздаваемый элемент, 
а только текстовый узел, который содержит вывод текущего времени, так как только этот узел будет отличаться от предыдущего состояния.</p>
