...
<html >
<head>
<title>React | Дочерние маршруты</title>
<meta charset="utf-8" />
<meta name="description" content="Дочерние вложенные маршруты в React, класс IndexRoute">
...
<h2>Дочерние маршруты</h2><div class="date">Последнее обновление: 01.04.2022</div>
...
<p>В рамках маршрутов в React можно определять дочерние маршруты. Такие подмаршруты будут отсчитываться от главного маршрута. Но для построения подобной системы есть ряд подходов. Рассмотрим их.</p>
<h3>Определение подмаршрутов в коде компонента</h3>
<p>Для применения подмаршрутов возьмем из прошлой темы веб-страницу <span class="b">index.html</span> и изменим ее следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Для обработки запроса "/products" здесь определен маршрут, который обрабатывается компонентом Products:</p>
<pre class="brush:xml;">&lt;Route path="/products/*" element={&lt;Products /&gt;} /&gt;</pre>
<p>Обратите внимание на шаблон пути: <code>path="/products/*"</code>. Символ <span class="b">*</span> указывает, что компонент Products будет обрабатывать маршруты, которые начинаются 
 "/products/", но после слеша также могут идти и другие символы.</p> 
<p>Но сам этот компонент имеет вложенные маршруты:</p>
<pre class="brush:xml;">
function Products(){
	return &lt;div&gt;
			&lt;h2&gt;Товары&lt;/h2&gt;
...
<p>Вложенные маршруты отсчитываются фактически от главного маршрута "/products". То есть маршрут</p>
<pre class="brush:xml;">&lt;Route path="/phones" element={&lt;Phone /&gt;} /&gt;</pre>
<p>будет обрабатывать запросы по пути "/phones", который добавляется к пути главного компонента - "/products", то есть в итоге по пути 
"/products/phones". Аналогичным образом запросы по пути "/products/tablets" будут обрабатываться  компонентом Tablet.</p>
<p>Запустим приложение и пройдемся по разным адресам:</p>
<img src="./pics/4.17.png" alt="Дочерние маршруты в React" />
<img src="./pics/4.18.png" alt="Вложенные маршруты в React" />
<p>И что также можно заметить, то в обоих случаях выводится заголовок "Товары", так как он определен на главном компоненте Products. Остальное содержимое будет отличаться в зависимости от выбранного для 
обработки запроса компонента.</p>
<p>Аналогичный пример с использованием классов:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Однако данный подход имеет как минимум один недостаток - при обращении к любым адресам, которые начинаются с "products", запросы будет обрабатывать компонент 
Products. Но, к примеру, мы хотим, чтобы он обрабатывал свой основной маршрут и запросы по дочерним маршутам. Поэтому рассмотрим другой подход.</p>
<h3>Outlet</h3>
<p>При другом подходе маршруты определяются вне компонента, а их содержимое вставляется в главный компонент с помощью встроенного компонента <span class="b">Outlet</span>. Так, изменим предыдущий пример следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь надо отметить два момента. Прежле всего в коде компонента Products применяется компонент <span class="b">Outlet</span>:</p>
<pre class="brush:xml;">
function Products(){
	return &lt;div&gt;
			&lt;h2&gt;Товары&lt;/h2&gt;
...
<p>Здесь вместо элемента <code>&lt;Outlet /&gt;</code> будет вставляться содержимое компонентов, которые обрабатывают дочерние маршруты.</p>
<p>Во-вторых, дочерние маршруты определены внутри основного маршрута:</p>
<pre class="brush:xml;">
&lt;Route path="/products" element={&lt;Products /&gt;}&gt;
	&lt;Route path="phones" element={&lt;Phone /&gt;} /&gt;
	&lt;Route path="tablets" element={&lt;Tablet /&gt;} /&gt;
...
<p>Стоит отметить, что пути в дочерних маршрутах не должны начинаться со слеша. И в итоге дочерние маршруты также будут отсчитываються от главного маршрута "/products" и 
сопвадать с запросами "/products/phones" и "/products/tablets". Запрос по основному маршруту "/products" будет обрабатываться только компонентом 
Products. Все остальные запросы, которые не совпадают с основным и подмаршрутами, например, "/products/abc", будет обрабатываться самым последним маршрутом.</p>
...
<h3>Главный маршрут</h3>
<p>Нередко возникает необходимость среди дочерних маршрутов выделить основной подмаршрут. Для установки такого подмаршрута применяется атрибут 
<span class="b">index</span>. Например, изменим возьмем пример выше и изменим определение маршрутов:</p>
<pre class="brush:xml;">
&lt;Router&gt;
	&lt;div&gt;
		&lt;Routes&gt;
...
<p>В данном случае для маршрута с путем "/products" определен основной подмаршрут</p>
<pre class="brush:xml;">&lt;Route index element={&lt;h3&gt;Каталог товаров&lt;/h3&gt;} /&gt;</pre>
<p>Для простоты здесь явным образом не создается компонент для обработки этого маршрута, но естестественно можно также создать отдельный компонент. 
В любом случае содержимое этого компонента будет также вставляться в компонент Products на место элемента <code>Outlet</code>:</p>
<img src="./pics/4.23.png" alt="Outlet в React" />
