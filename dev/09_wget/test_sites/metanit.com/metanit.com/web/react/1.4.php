...
<html >
<head>
<title>React | Babel-cli</title>
<meta charset="utf-8" />
<meta name="description" content="Использование Babel-cli для компиляции кода JSX в JavaScript в React и запуск приложения с помощью веб-сервера Node.js">
...
<h2>Babel-cli</h2><div class="date">Последнее обновление: 01.04.2022</div>
...
<p>В прошлых темах для компиляции JSX в полноценный JavaScript в React.js на веб-страницу подключался 
компилятор Babel, который динамически при запуске веб-страницы выполнял компиляцию. Но существует и другой способ компиляции JSX в 
JavaScript - статическая компиляция, при которой мы явно создаем код на JavaScript.</p>
<p>Так, продолжим работу с проектом из прошлой темы  (или создадим новый). В этом проекте у нас есть файл <span class="b">index.jsx</span>, 
который содержит код на JSX:</p>
<pre class="brush:xml;">
...
<p>И также есть html-страница <span class="b">index.html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Стоит отметить, что в данном случае уже не подключается компилятор Babel как отдельный файл, более того сам файл приложения называется 
<span class="b">index.js</span>, а не <span class="b">index.jsx</span>. Хотя у нас определен именно файл index.jsx.</p>
<p>Далее добавим в папку проекта новый файл, который назовем <span class="b">package.json</span> и в котором определим следующий код:</p>
<pre class="brush:js;">
{
  "name": "helloapp",
  "version": "1.0.0",
...
<p>В секции devDependencies определены зависимости для компилятора Babel. 
А секция "babel" определяет пресеты - наборы плагинов, которые используются для работы с React.</p>
<p>Далее установим эти пакеты. И для этого перейдем в командной строке/терминале к папке проекта и выполним следующую команду</p>
<div class="console">
<pre class="consoletext">
npm install
</pre>
</div>
<img src="./pics/1.5.png" alt="Установка babel-cli в проект React" />
<p>Итак, у нас есть файл index.jsx с кодом JSX, теперь скомпилируем его в файл index.js. Для этого выполним следующую команду:</p>
<div class="console">
<pre class="consoletext">
npx babel index.jsx --out-file index.js
</pre>
</div>
<img src="./pics/1.6.png" alt="компиляции JSX в babel-cli" />
<p>Команда <span class="b">npx babel</span> позволяет определить местоположение скриптов babel в рамках проекта (вместо этой команды 
также можно использовать относительный путь к babel - <span class="b">.\node_modules\.bin\babel</span>).</p>
<p>В данном случае компилятору babel передается компилируемый файл - <code>index.jsx</code>, а после параметра <code>--out-file</code> указывается выходной файл. Более подробно про параметры компиляции 
можно прочитать на официальной странице по <a href="https://babeljs.io/docs/usage/cli/" rel="nofollow">babel-cli</a>. И после выполнения команды в папке проекта окажется файл index.js, который мы можем использовать.</p>

<img src="./pics/1.18.png" alt="компиляции JSX React в JavaScript в babel-cli" />
<p>В качестве сервера опять же возьмем определенное в прошлой теме приложение node.js в файле <span class="b">app.js</span>:</p>
<pre class="brush:js;">
...
<p>Теперь, как и в прошлой теме можно запустить проект на выполнение с помощью команды <span class="b">node app.js</span>, и в 
браузере мы увидим ту же страницу:</p>
<img src="./pics/1.4.png" alt="Компиляция React 17 с помощью Babel Cli 7" />
<p>Чтобы не запускать при запуске две команды (одна для компиляции файла javascript и другая для запуска сервера), 
определим скрипт в файле <span class="b">package.json</span>, изменив код файла следующим образом:</p>
<pre class="brush:js;">
{
  "name": "helloapp",
  "version": "1.0.0",
...
<p>Здесь в секции <code>scripts</code> определена команда <code>start</code>, которая позволяет последовательно запустить обе команды и сразу скомпилировать файл javascrript и 
запустить сервер. Теперь нам достаточно выполнить в командной строке/консоли команду <span class="b">npm start</span>:</p>
<img src="./pics/1.19.png" alt="Компиляция React с помощью Babel Cli 7 и запуск сервера Node.js" />
<p>Если мы хотим, чтобы при этом автоматически открывался браузер с запущенной страницей, то на Windows для этого можно добавить команду "start http://localhost:3000/":</p>
<pre class="brush:js;">
"scripts": {
...
<div class="nav"><p><a href="./1.6.php">Назад</a><a href="./">Содержание</a><a href="./1.5.php">Вперед</a></p></div></div>
<div class="socBlock">
