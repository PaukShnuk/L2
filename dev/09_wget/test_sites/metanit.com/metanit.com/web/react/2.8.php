...
<html >
<head>
<title>React | Составные компоненты. Поиск в списке и фильтрация</title>
<meta charset="utf-8" />
<meta name="description" content="Составные компоненты в React, поиск в списке и его фильтрация">
...
<h2>Составные компоненты. Поиск в списке</h2><div class="date">Последнее обновление: 01.04.2022</div>
...
<p>Компоненты могут состоять из других компонентов. Стандартный пример - какие-нибудь списки, например, меню. Каждый элемент меню может быть представлен 
отдельным компонентом, а само меню также может представлять компонент. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/2.4.png" alt="Составные компоненты в React" />
<p>В принципе мы могли бы определить все в одном компоненте, однако выделение отдельного компонента Item позволяет развивать и обновлять его разметку 
отдельно от родителького компонента. Например, если потребуется изменить структуру разметки html в компоненте, то достаточно это сделать в коде класса Item. 
К тому же мы можем повторно использовать Item в других компонентах.</p>
<p>В то же время в примере выше есть один минус - данные жестко закодированы в элементе Item и смешаны с представлением. Второй момент, который надо отметить, - 
нередко данные передаются от родительского компонента к дочернему, особенно в списках. Поэтому изменим код JSX в примере выше следующим образом:</p>
<pre class="brush:js;">
const propsValues = {
...
<p>Здесь отделены данные от представления. Из главного компонента ItemsList в дочерний компонент Item данные передаются через значение 
<code>this.props.data.items</code>.</p>
<img src="./pics/2.11.png" alt="Передача данных между компонентами в React" />
<p>Теперь пойдем дальше и добавим к списку поиск:</p>
<pre class="brush:js;">
const propsValues = {
	title: "Список смартфонов",
	items: [
...
<p>Теперь компонент ItemsList выводит в список не те объекты, которые передаются через <code>this.props.data.items</code>, а объекты из состояния 
<code>this.state.items</code>. В самом начале эти объекты совпадают.</p>
<p>Текстовое поле над списком использует функцию <code>filterList</code> для обработки события change - изменения ввода в текстовое поле. В функции 
filterList используя начальный список <code>this.props.data.items</code>, фильтруем его и переустанавливаем объекты в <code>this.state.items</code>. 
В итоге поле ввода будет изменяться состояние компонента ItemsList, что приведет повторному рендерингу компонента.</p>
<img src="./pics/2.12.png" alt="Фильтрация в списке в React" />
<p>Теперь пойдем еще дальше, что если мы захотим впоследствии сделать поиск не по вводу в текстовое поле, а по нажатию кнопки, которая будет добавлена в компонент. 
Либо, возможно, каким-то другим образом построить функционал поиска. В этом случае лучше все элементы, связанные с поиском выделить в отдельное функциональное звено, 
то есть в отдельный компонент. Поэтому изменим код следующим образом:</p>
<pre class="brush:js;">
const propsValues = {
	title: "Список смартфонов",
	items: [
...
<p>Таким образом, приложение для пользователя никак не изменилось - также по ввода данных в поле происходит фильтрация. Но теперь функционал ввода вынесен 
в отдельный компонент. В нем мы можем как угодно обрабатывать введенное значение, даже изменять его, проверять его значение на предмет некорретных слов или символов. 
Главному компоненту не важна вся эта предобработка, ему нужен только текст, по которому надо фильтровать.</p>
...
<div class="nav"><p><a href="./7.1.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
