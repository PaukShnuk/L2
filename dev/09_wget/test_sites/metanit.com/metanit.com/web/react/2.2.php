...
<html >
<head>
<title>React | Компоненты</title>
<meta charset="utf-8" />
<meta name="description" content="Компоненты в React, функциональный и объектно-ориентированный способ определения компонентов, класс React.Component, функция render">
...
<h2>Компоненты</h2><div class="date">Последнее обновление: 01.04.2022</div>
...
<p>ReactElement, который был рассмотрен в прошлой теме, позволяет создавать элементы, которые встраиваются на веб-страницу. Однако ReactElement плохо подходит для создания комплексной 
разметки HTML. Объекты ReactElement сложно использовать повторно в других аналогичных ситуациях, сложнее поддерживать.</p>
<p>И для решения этой проблемы в React используются компоненты. Компоненты проще обновлять и использовать повторно.</p>
<p>Компоненты аналогичны функциям JavaScript. Они хранят состояние с помощью свойств и возвращают элементы React, которые затем появляются на веб-странице.</p>
<h3>Создание компонентов</h3>
<p>Компоненты можно определить различными способами. Первый способ - функциональный. Например:</p>
<pre class="brush:js;">
function Hello() {
  return &lt;h1&gt;Привет, Eugene&lt;/h1&gt;;
}
</pre>
<p>Здесь определяется компонент Hello. Название компонентов должно начинаться с заглавной буквы.</p>

<p>Второй способ определения компонентов предполагает использование классов ES6:</p>
<pre class="brush:js;">
...
<p>Для рендеринга компонента в классе компонента обязательно должен быть определен метод <span class="b">render()</span>, который возвращает создаваемый элемент на JSX.</p>
<p>Также для определения мы можем использовать стрелочные функции (arrow functions):</p>
<pre class="brush:js;">
var Hello =() =&gt; {
    return (&lt;h1&gt;Привет, Eugene&lt;/h1&gt;);
}
</pre>
<p>В целом все описанные выше способы будут равноценны.</p>
<h3>Применение компонентов</h3>
<p>Используем выше определенный компонент:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/2.3.png" alt="Компоненты в React" />
<p>Если рассмотреть поэтапно, то рендеринг компонента будет происходить следующим образом:</p>
<ol>
<li><p>Сначала устанавливаем элемент, в который будет производиться рендеринг, с помощью функции <code>ReactDOM.createRoot()</code></p></li>
<li><p>Для рендеринга компонента <code>&lt;Hello /&gt;</code> вызывается функция <code>render()</code></p></li>
<li><p>React вызывает компонент Hello</p></li>
<li><p>Компонент Hello возвращает элемент <code>&lt;h1&gt;Привет, Eugene&lt;/h1&gt;</code></p></li>
<li><p>React DOM обновляет стандартный DOM веб-страницы, чтобы она содержала элемент <code>&lt;h1&gt;Привет, Eugene&lt;/h1&gt;</code></p></li>
</ol>
<p>Здесь важно отметить один момент. Вне зависимости от того, какой способ определения компонента мы используем, между названием класса/функции/стрелочной функции и элементом есть соответствие: класс/функция называется "Hello", 
и соответственно для рендеринга компонента применяется элемент <code>&lt;Hello /&gt;</code>. При этом учитывается регистр. 
