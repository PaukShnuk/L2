...
<meta charset="utf-8" />
<meta name="description" content="Свойства Props в React и их передача в компонент">
...
<h2>Props</h2><div class="date">Последнее обновление: 01.04.2022</div>
...
<p><span class="b">Props</span> представляет коллекцию значений, которые ассоциированы с компонентом. Эти значения позволяют создавать динамические компоненты, которые 
не зависят от жестко закодированных статических данных.</p>
...
<p>Параметр props, который передается в функцию компонента, инкапсулирует свойства объекта. В частности, свойство <code>name</code> и <code>age</code>. 
...
разметки html, наполненной разными данными.</p>
...
<p>Класс компонента также извне получает объект свойств, который доступен через <code>this.props</code>.</p>
<p>Использование стрелочных функций (arrow functions):</p>
<pre class="brush:js;">
const Hello = (props) =&gt; {
...
<p>При рендеринге React передает значения атрибутов в виде единого объекта "props". То есть значение из атрибута 
<code>name="Tom"</code> перейдет в свойство <code>props.name</code>.</p>
<p>В итоге будет создана следующая страница:</p>
<img src="./pics/2.7.png" alt="Свойства Props в React" />
...
<p>Если мы применяем классы, то для установки значений применяется статическое свойство <span class="b">defaultProps</span>:</p>
<pre class="brush:js;">
class Hello extends React.Component {
...
<p>При функциональном определении компонента также применяется свойство <code>defaultProps</code>:</p>
<pre class="brush:js;">
function Hello(props) {
	return &lt;div&gt;
		&lt;p&gt;Имя: {props.name}&lt;/p&gt;
...
<p>И в любом из этих случаев, если мы не определим какие-то свойства для компонента, то они будут брать значения из значений по умолчанию:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/2.14.png" alt="Свойства по умолчанию Props defaultProps в компонентах React" />
<h3>Обновление props</h3>
<p>Сам <span class="b">this.props</span> представляет неизменяемый объект, который предназначен только для чтения. Поэтому не следует его изменять, например:</p>
<pre class="brush:js;">
class Hello extends React.Component {
	render() {
		this.props.name = "Tom";
...
<p>Если же надо время то времени изменять какие-то внутренние данные компонента, то для хранения таких данных в React предназначен объект <span class="b">state</span>, 
который будет рассмотрен в следующей статье.</p>
