...
<html >
<head>
<title>React | Парсинг строки запроса</title>
<meta charset="utf-8" />
<meta name="description" content="Парсинг строки запроса в компоненте React.JS, получение параметров маршрута и строки запроса, объекты match и location">
...
<h2>Парсинг строки запроса</h2><div class="date">Последнее обновление: 02.04.2022</div>
...
<p>В прошлых темах использовались параметры маршрута, которые извлекались из запрошенного адреса url. 
Но кроме параметров маршрута приложение на React также может извлекать данные из строки запроса. Строка запроса - эта та часть адреса, 
которая идет в запрошенном адресе после вопросительного знака, например:</p>
<pre class="brush:js;">http://localhost:3000/2/?name=Bob&age=23</pre>
<p>Здесь часть "?name=Bob&age=23" как раз является строкой запроса, которая содержит два параметра. Строка запроса представляет 
набор пааметров в виде <code>ключ=значение</code>, которые разделяются друг от друга знаком амперсанда - &. Например, выше в строке запроса указаны два параметра: 
name и age. Параметр name имеет значение "Bob", а параметр age - значение 23.</p>
<p>Для получения параметров строки запроса в react-router v6 имеется функция-хук <span class="b">useSearchParams()</span>. 
Рассмотрим ее применение и для этого определим следующую веб-страницу:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/4.16.png" alt="Парсинг строки запроса в React.JS" />
<p>Для демонстрации парсинга строки запроса здесь используется только один компонент. Для маршрута компонента определен один параметр id.</p>
<p>Для тестирования определены две ссылки с помощью компонента <span class="b">Link</span>, которые содержат как параметры маршрута - пааметр id, так и параметры строки запроса - параметры name и age:</p>
<pre class="brush:xml;">
&lt;Link to="/2/?name=Bob&age=23"&gt;Bob&lt;/Link&gt; | 
&lt;Link to="/3/?name=Tim&age=33"&gt;Tim&lt;/Link&gt;
</pre>
<p>Параметр маршрута получаем через хук <span class="b">useParams()</span>:</p>
<pre class="brush:js;">
const params = useParams();
..............
&lt;p&gt;Id: {params.id}&lt;/p&gt;
</pre>
<p>Для получения параметров строки запроса получаем функция-хук <span class="b">useSearchParams</span>:</p>
<pre class="brush:js;">const useSearchParams  = ReactRouterDOM.useSearchParams;</pre>
<p>Затем обращаемся к ней:</p>
<pre class="brush:js;">const [searchParams, setSearchParams] = useSearchParams();</pre>
<p>Из нее мы можем получить два объекта. Объект <span class="b">searchParams</span> - по сути объект <span class="b">URLSearchParams</span>, 
который доступен в стандартном коде JavaScript и который позволяет получить из строки запроса параметры и управлять ими. И объект 
<span class="b">setSearchParams</span> - функция изменения объекта searchParams.</p>
<p>Используя функции <span class="b">get</span> через searchParams можно получить параметры name и age:</p>
<pre class="brush:js;">
&lt;p&gt;Name: {searchParams.get("name")}&lt;/p&gt;
&lt;p&gt;Age: {searchParams.get("age")}&lt;/p&gt;
</pre>
<p>Таким образом, мы можем работать с двумя типами параметров, как с параметрами маршрута, которые определяются непосредственно в маршруте, 
так и с параметрами строки запроса.</p>
