...
<meta charset="utf-8" />
<meta name="description" content="Жизненный цикл компонента в React, все этапы жизненного цикла и их обработка, getDerivedStateFromProps, getSnapshotBeforeUpdate, componentDidMount, componentDidUpdate, componentWillUnmount">
...
<h2>Жизненный цикл компонента</h2><div class="date">Последнее обновление:  01.04.2022</div>
...
<p>В процессе работы компонент проходит через ряд этапов жизненного цикла. На каждом из этапов вызывается определенная функция, в которой мы можем определить какие-либо действия:</p>
<ol>
<li><p><span class="b">constructor(props)</span>: конструктор, в котором происходит начальная инициализация компонента</p></li>
<li><p><span class="b">static getDerivedStateFromProps(props, state)</span>: вызывается непосредственно перед рендерингом компонента. 
Этот метод не имеет доступа к текущему объекту компонента (то есть обратиться к объкту компоненту через this) и должен возвращать объект для обновления объекта state или значение <code>null</code>, если нечего обновлять.</p></li>
<li><p><span class="b">render()</span>: рендеринг компонента</p></li>
<li><p><span class="b">componentDidMount()</span>: вызывается после рендеринга компонента. Здесь можно выполнять запросы к удаленным ресурсам </p></li>
<li><p><span class="b">componentWillUnmount()</span>: вызывается перед удалением компонента из DOM</p></li>
</ol>
<p>Кроме этих основных этапов или событий жизненного цикла, также имеется еще ряд функций, которые вызываются при обновлении состояния после начального рендеринга компонента, если в компоненте происходят обновления:</p>
<ol>
<li><p><span class="b">static getDerivedStateFromProps(props, state)</span></span></p></li>
<li><p><span class="b">shouldComponentUpdate(nextProps, nextState)</span>: вызывается каждый раз при обновлении объекта props или state. В качестве параметра 
передаются новый объект props и state. Эта функция должна возвращать true (надо делать обновление) или 
false (игнорировать обновление). По умолчанию возвращается true. Но если функция будет возвращать false, то тем самым мы отключим обновление компонента, а 
последующие функции не будут срабатывать.</p></li>
<li><p><span class="b">render()</span>: рендеринг компонента  (если shouldComponentUpdate возвращает true)</p></li>
<li><p><span class="b">getSnapshotBeforeUpdate(prevProps, prevState)</span>: вызывается непосредственно перед компонента. Он позволяет компоненту получить 
...
в качестве третьего параметра в метод <code>componentDidUpdate()</code> и может учитываться в componentDidUpdate при обновлении. Если нечего возвращать, то возвращается значение 
<code>null</code></p></li>
<li><p><span class="b">componentDidUpdate(prevProps, prevState, snapshot)</span>: вызывается сразу после обновления компонента (если shouldComponentUpdate возвращает true). 
В качестве параметров передаются старые значения объектов props и state. Третий параметр - значение, которое возвращает метод <code>getSnapshotBeforeUpdate</code></p></li>
</ol>
<p>Применим некоторые из событий жизненного цикла:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>При загрузке компонента сработают сразу четыре этапа жизненного цикла компонента:</p>
<img src="./pics/2.9.png" alt="Жизненный цикл компонентов в React" />
<p>При нажатии на кнопку сработает обработчик нажатия, который обновит объект state, что вызовет еще ряд функций жизненного цикла и повторный рендеринг:</p>
<img src="./pics/2.10.png" alt="Жизненный цикл и обновление компонентов в React" />
