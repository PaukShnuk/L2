...
<html >
<head>
<title>React | Управление ресурсами</title>
<meta charset="utf-8" />
<meta name="description" content="Управление ресурсами в React.js, работа с таймером, обработка событий жизненного цикла для управления ресурсами, componentDidMount и componentWillUnmount">
...
<h2>Управление ресурсами</h2><div class="date">Последнее обновление: 01.04.2022</div>
...
<p>В приложениях React при удалении компонентов важно освобождать ресурсы, которые используются этими компонентами. Для управления ресурсами можно задействовать функции жизненного цикла компонента.</p>
<p>Создание подобных ресурсов в компонентах React производится в функции <span class="b">componentDidMount()</span>. А для освобождения ресурсов применяется функция <span class="b">componentWillUnmount()</span>.</p>
<p>Стандартный пример - часы, которые выводят текущее время на страницу и для обновления времени используют таймер:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В функции <code>componentDidMount()</code> создается таймер - ресурс, используемый компонентом. Для его освобождения в функции 
<code>componentWillUnmount()</code> вызывается функция <code>clearInterval()</code>.</p>
<p>Для имитации удаления компонента с веб-страницы (и соответственно для вызова метода componentWillUnmount и удаления ресурсов) определена кнопка, 
по нажатию на которую вызывается функция <code>unmount()</code>. В этой функции вызывается функция <code>root.unmount()</code>, 
которая открепляет компонент от html-элемента на веб-странице.</p>
<img src="./pics/2.6.png" alt="Жизненный цикл компонента в React" />
<p>В итоге работа компонента будет выглядеть следующим образом:</p>
<ol>
<li><p>Когда элемент <code>&lt;Clock /&gt;</code> передается в функцию <code>root.render()</code>, React вызывает конструктор 
компонента Clock. В конструкторе компонента устанавливается объект this.state, который содержит текущую дату и время.</p></li>
<li><p>Затем React вызывает метод <code>render()</code> компонента Clock, и происходит обновление DOM, в который добавляется элемент, созданный компонентом Clock.</p></li>
<li><p>Когда элемент, сгенерированный компонентом Clock, добавляется в DOM, React вызывает метод <code>componentDidMount()</code>. 
Это приводит к запуску метода <code>tick()</code></p></li>
<li><p>Каждую секунду браузер вызывает метод <code>tick()</code>, в котором через вызов <code>setState()</code> 
обновляется значение <code>this.state.date</code>. При обновлении <code>this.state</code> React вызывает повторно метод <code>render()</code> и обновляет веб-страницу.</p></li>
<li><p>Если нажимем на кнопку, компонент Clock удаляется из DOM, и React вызывает метод <code>componentWillUnmount()</code>, в котором останавливается таймер.</p></li>
</ol>
