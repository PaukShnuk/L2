...
<html >
<head>
<title>Фильтры в AngularJS</title>
<meta charset="utf-8" />
<meta name="description" content="Использование фильтров в AngularJS для предобрабоки выводимых выражений, фильтр filter и параметры фильтрации">
...
<h2>Фильтры</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Фильтры позволяют провести некоторую предобработку перед выводом данных на страницу, например, отсортировать или как-то изменить набор данных.</p>
<p>Общий способ использования фильтров: <code>{{expression | filter}}</code></p>
<h3>Изменение регистра</h3>
<p>Используя фильтры <code>lowercase</code> и <code>uppercase</code>, мы можем приводить содержимое к нижнему и верхнему регистру соответственно. Например, 
<code>&lt;h3&gt;{{question.text | lowercase}}&lt;/h3&gt;</code></p>
<h3>Форматирование чисел</h3>
<p>Фильтр <code>number</code> позволяет форматировать числа. Например, <code>&lt;div&gt;{{234.5677 | number:1}}&lt;/div&gt;</code>. Результатом 
работы данного фильтра будет число <code>234.6</code>. Числовой параметр после двоеточия указывает, сколько останется чисел после запятой (условной, 
...
при выводе число дополняется нулями.</p>
<h3>Форматирование валюты</h3>
<p>Для форматирования валюты используется фильтр <code>currency</code>. Например: <code>&lt;div&gt;{{234.5 | currency}}&lt;/div&gt;</code>. 
...
<code>&lt;div&gt;{{234.5 | currency:'RUB'}}&lt;/div&gt;</code></p>
<h3>Форматирование дат</h3>
<p>Чтобы вывести дату в определенном формате применяется фильтр <code>date</code>. В качестве выражения используется количеств миллисекунд, 
прошедших с начала эпохи Unix (то есть с 1 января 1970 года). Например: <code>&lt;div&gt;{{1405405678756 | date:'yyyy-MM-dd'}}&lt;/div&gt;</code></p>
<p>Здесь на страницу будет выведена дата <code>2014-07-15</code>, соответствующая шаблону <code>yyyy-MM-dd</code>. Данный фильтр имеет очень много 
шаблонов, которые можно посмотреть в документации.</p>
<h3>Фильтры orderBy и limitTo</h3>
<p>Фильтр orderBy упорядочивает набор объектов по определенному свойству. Так, возьмем проект, созданный в прошлом разделе. Изменим строки, где 
происходит вывод объектов следующим образом:</p>
<pre class="brush:xml;">
&lt;div ng-repeat="answer in question.answers | orderBy:sortparam" class="answer"&gt;
&lt;!-- далее остальное содержимое --&gt;
</pre>
<p>Здесь у нас применяется фильтр orderBy, который использует параметр <code>sortparam</code>. Добавим определение этого параметра в ранее созданный контроллер 
QuestionController:</p>
<pre class="brush:js;">
...
<p>Строка <code>$scope.sortparam='rate';</code> указывает, что все объекты будут отсортированы при выводе по значению свойства rate в порядке возрастания. 
Если же нам надо поместить сначала те объекты, у которых свойство rate больше, то делаем обратный порядок: <code>$scope.sortparam='-rate';</code></p>
<p>Теперь применим фильтр limitTo, который ограничивает вывод определенным количеством объектов:</p>
<pre class="brush:xml;">
...
<h3>Фильтр filter</h3>
<p>Фильтр filter задает параметры и значения фильтрации. Посмотрим на примере. Возьмем проект, сделанный выше, и добавим в него привязку двух 
выпадающих списков с применением фильтра filter:</p>
<pre class="brush:xml;">
...
<p>Здесь первый элемент select привязывается к модели answer.author через ng-model. Второй же осуществляет фильтрацию по выбранной модели. При выборе автора ответа 
в первом списке во втором автоматически будут отображаться даты всех его ответов. Вывод в элемент select можно заменить выводом в div, ul или другой элемент.</p>
<img src="pics/10.png" />
<p>Рассмотрим еще пример с фильтром. Допустим, нам надо через специальное поле ввода динамически получать всех ответчиков, которые соответствуют определенному критерию:</p>
<pre class="brush:xml;">
&lt;div  ng-controller="QuestionController"&gt;
&lt;div class="quest"&gt;
&lt;h3&gt;{{question.text}}&lt;/h3&gt;
...
<img src="pics/filters.png" alt="Фильтры в AngularJS" />
<p>Тут все просто: текстовое поле осуществляет привязку к объекту member: <code>ng-model="member"</code>. Даже если в контроллере мы не установили этот 
объект, AngularJS создает его автоматически. А при выводе устанавливаем фильтр по этому объекту: <code>&lt;div ng-repeat="a in question.answers | filter:{author:member}" class="answer" &gt;</code>. 
Причем данный объект учитывается только для свойства author.</p>
