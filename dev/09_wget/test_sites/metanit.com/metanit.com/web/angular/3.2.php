...
<html >
<head>
<title>Сервис $http и ajax-запросы</title>
<meta charset="utf-8" />
<meta name="description" content="Создание ajax-запросов в AngularJS с помощью сервиса $http, пример использования сервиса">
...
<h2>Сервис $http и ajax-запросы</h2><div class="date">Последнее обновление: 27.04.2017</div>
...
<p>Сервис $http представляет ключевой сервис Angular, предназначенный для взаимодействия с удаленным HTTP-сервером через 
объект XMLHttpRequest или через JSONP. Посмотрим на примере, модифицировав приложение из прошлой темы. Во-первых, выделим все необходимые 
нам данные в какой-то внешний файл, например, <span class="b">question.json</span>, который будет находиться рядом со страницей index.html. Он будет 
содержать данные в формате json:</p>
<pre class="brush:js;">
{
    "question":{
...
<p>Что тут надо отметить? Во-первых, если в прошлой теме про сервисы передался сделанный нами сервис в функцию QuestionController, 
то теперь в качестве второго параметра мы передаем в нее сервис $http.</p>
<p>Во-вторых, надо отметить синтаксис использования сервиса:</p>
<pre class="brush:js;">
$http({method: 'GET', url: 'question.json'}).
            then(function success(response) {
                $scope.question=response.data.question;
...
<p>Для обращения к определенному ресурсу (в данном случае к файлу <span class="ii">question.json</span>) сервис $http использует метод <code>get</code>, 
...
скрипт на PHP, Ruby и т.д.</p>
<p>Далее идет функция <code>then</code>, в которую в качестве первого параметра передается функция, которая срабатывает, если обращение к ресурсу прошло 
...
передаем в представление.</p>
<p>Таким образом, данные передаются в представление, а саму страницу index.html при этом мы можем оставить такой же, как и в прошлой теме:</p>
<pre class="brush:xml;">
&lt;!doctype html&gt;
...
<p>Немного изменим пример. Пусть у нас данные загружаются по нажатию кнопки:</p>
<pre class="brush:js;">
questApp.controller('QuestionController',
    function QuestionController($scope, $http){
...
<p>А в блоке с данными создадим кнопку для загрузки данных:</p>
<pre class="brush:xml;">
&lt;!doctype html&gt;
&lt;html ng-app="questApp"&gt;
&lt;head&gt;
...
<p>А весь блок с данными теперь перемешен в отдельный элемент <code>div</code>, видимостью которого управляет объект loaded.</p>
...
<div class="nav"><p><a href="3.1.php">Назад</a><a href="./">Содержание</a><a href="3.7.php">Вперед</a></p></div></div>
<div class="socBlock">
