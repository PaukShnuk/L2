...
<html >
<head>
<title>Внедрение зависимостей в AngularJS</title>
<meta charset="utf-8" />
...
<h1>Внедрение зависимостей и сервис $injector</h1><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Внедрение зависимостей (<b>Dependency Injection</b>) представляет собой способ организации компонентов в приложении, при котором между компонентами 
отсутствуют жесткие связи: компоненты связаны посредством интерефейсов, а не конкретных реализаций. И в любое время мы можем динамически применить другую 
реализацию компонента.</p>
<p>В AngularJS ключевую роль во внедрении зависимостей играет сервис <code>$injector</code>. Его роль состоит в определении и установке зависимостей, которые используются функцией.</p>
<p>Свою функциональность сервис $injector реализует с помощью следующих методов:</p>
<ul>
<li><p><span class="bb">annotate(fn)</span>: возвращает набор зависимостей для функции fn</p></li>
<li><p><span class="bb">get(name)</span>: возвращает контретную реализацию сервиса по определенному названию сервиса</p></li>
<li><p><span class="bb">has(name)</span>: возвращает <code>true</code>, если для указанного объекта name установлена зависимость</p></li>
<li><p><span class="bb">invoke(fn, self, locals)</span>: вызывает функцию fn. Может принимать два необязательных параметра: <code>self</code> (позволяет установить 
аргумент <code>this</code> для вызываемой функции) и <code>locals</code> (представляет альтернативный способ передачи аргументов в вызываемую функцию)</p></li>
</ul>
<p>Рассмотрим на небольшом примере применение сервиса $injector:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html ng-app="myApp"&gt;
&lt;head&gt;
...
<p>На странице определена кнопка, нажатие которой обрабатывается методом <code>buttonClick()</code>. Данный метод определен в контроллере MyController.</p>
<p>Чтобы задействовать сервис $injector, он передается в качестве параметра в функцию контроллер.</p>
<p>Кроме того, здесь определена функция <code>getData</code>, которая зависит от сервиса dataService, а также от аргумента message.</p>
<p>В методе buttonClick мы можем получить все используемые функцией getData зависимости с помощью выражения <code>var deps = $injector.annotate(getData)</code>. 
После этого объект <code>deps</code> будет содержать набор используемых зависимостей, то есть dataService и message.</p>
<p>С помощью метода <code>$injector.has(deps[i])</code> определяем, является ли зависимость зарегистрированным сервисом. Нам нет смысла как-то переопределять сервис 
dataService, а вот объект message мы можем определить какой угодно.</p>
<p>Затем с помощью метода <code>$injector.get(deps[i])</code> получаем реализацию сервиса и добавляем в массив аргументов <code>args</code></p>
<p>В конце вызываем метод с помощью стандартной функции javascript apply, передавая в нее полученные компоненты: <code>getData.apply(null, args)</code></p>
<p>Теперь используем метод <code>$injector.invoke()</code>. Для этого изменим метод <code>buttonClick()</code>:</p>
<pre class="brush:js;">
$scope.buttonClick = function () {
...
<p>Поскольку опять же нам нет смысла устанавливать зависимости для сервиса dataService, поэтому мы устанавливаем зависимости только 
для параметра message. Затем установленные параметры передаются в функцию $injector.invoke.</p>
...
<div class="nav"><p><a href="4.3.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
