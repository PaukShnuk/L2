...
<html >
<head>
<title>Объект $scope в AngularJS</title>
<meta charset="utf-8" />
<meta name="description" content="Объект $scope в AngularJS, rootScope, их использование в контроллерах">
...
<h2>Объект $scope</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Объект $scope служит связующим звеном между представлением в виде кода html и контроллером. Объект $scope выполняет роль модели приложения. 
В $scope определяются любые объекты, которые можно использовать в представлении и к которым можно устанавливать привязку данных. Формально $scope представляет 
обычный javascript-объект.</p>
<p>Каждое приложение AngularJS имеет один объект <span class="bb">$rootScope</span>, который является родительским по отношению ко всем остальным объектам 
$scope, которые используются в контроллерах. И при запуске приложения создается привязка элемента, в котором используется директива <span class="bb">ng-app</span>, 
к объекту <code>$rootScope</code>. То есть это такой глобальный объект $scope, который мы можем использовать по всему приложению. Например:</p>
<pre class="brush:js;">
&lt;!doctype html&gt;
&lt;html ng-app="myApp"&gt;
&lt;head&gt;
...
<p>Здесь у модуля используется метод <span class="bb">run(callback)</span>, который регистрирует функцию, вызываемую при загрузке модуля. 
В этой функции устанавливается объект $rootScope.</p>
<p>Так как модуль применяется к корневому элементу страницы html (<code>&lt;html ng-app="myApp"&gt;</code>), то внутри этого элемента в любой части 
...
свойства и методы, определенные в $rootScope.</p>
<p>Подобно модулю контроллеры также могут устанавливать контекст и определять использовать свой объект $scope:</p>
<pre class="brush:xml;">
&lt;!doctype html&gt;
&lt;html ng-app="myApp"&gt;
&lt;head&gt;
...
<img src="pics/scope.png" alt="Scopes in AngularJS" />
<p>И при создании модуля, и при создании контроллера устанавливается свойство <code>message</code>, значение которого затем три раза выводится на странице.</p>
<p>Но для блока div устанавливается директива <span class="bb">ng-controller</span>, которая связывает его с контроллером myController. 
Директива ng-controller создает новый объект <code>$scope</code>, который является дочерним по отношению к глобальному $rootScope и который переопределяет 
его действие. В то же время область действия объекта $scope, установленного в контроллере, задается только этим блоком <code>div</code>, и вне 
его <code>$scope</code> уже не действует.</p>
...
<div class="nav"><p><a href="2.1.php">Назад</a><a href="./">Содержание</a><a href="2.8.php">Вперед</a></p></div></div>
<div class="socBlock">
