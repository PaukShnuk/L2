...
<html >
<head>
<title>Модули AngularJS</title>
<meta charset="utf-8" />
<meta name="description" content="Структура приложения в AngularJS, создание и использование модулей">
...
<h1>Основы AngularJS</h1>
<h2>Модули в AngularJS</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Основной формой организации приложений в AngularJS являются модули. Модуль представляет хранилище различной информации: директив, фильтров, 
контроллеров и т.д. При этом одно приложение может иметь несколько модулей. Например, разные модули могут представлять какую-либо специфическую 
функциональность.</p>
<p>Модули позволяют ассоциировать определенный участок html-страницы с приложением AngularJS. Модули также позволяют организовать и структурировать 
различные компоненты приложения AngularJS. Кроме того, модульность архитектуры приложения повышает тестируемость, и мы можем использовать различные 
части-модули приложения в других приложениях.</p>
<p>Для создания модуля используется метод <span class="bb">angular.module()</span>. Этот метод принимает три параметра, два из которых являются 
обязательными:</p>
<ol>
<li><p>Название модуля. Согласно соглашениям о наименовании модуль должен иметь суффикс <i>App</i></p></li>
<li><p>Набор других модулей в виде строкового массива, от которых данный модуль зависит</p></li>
<li><p>Конфигурационные настройки модуля</p></li>
</ol>
<p>В качестве результата метод <code>angular.module()</code> возвращает объект <span class="bb">Module</span>. Например, создадим модуль:</p>
<pre class="brush:js;">var myApp = angular.module('myApp', []);</pre>
<p>Здесь создается модуль myApp. Вместо набора модулей, передаваемых вторым параметром, мы можем отставить пустым массивом. В то же время 
было бы ошибкой вообще убрать этот параметр. Так как выражение <code>var myApp = angular.module('myApp');</code> будет представлять попытку 
получить модуль myApp в одноименную переменную, а не создать его.</p>
<p>Чтобы ассоциировать модуль с определенным куском html-страницы, нужно использовать директиву <b>ng-app</b>:</p>
<pre class="brush:xml;">
&lt;html ng-app="myApp"&gt;
</pre>
<p>Используя модуль, мы можем определить ряд компонентов, таких как контроллеры, сервисы и т.д., которые затем применяются в приложении. 
Для этого объект Module имеет ряд методов, наиболее используемые из них:</p>
<ul>
<li><p><code>config(callback)</code>: регистрирует функцию callback, которая используется для его конфигурации в процессе загрузки</p></li>
<li><p><code>constant(key, value)</code>: определяет сервис, который возвращает константное значение value</p></li>
<li><p><code>controller(name, constructor)</code>: создает контроллер</p></li>
<li><p><code>directive(name, factory)</code>: создает директиву, которая расширяет стандартную разметку html</p></li>
<li><p><code>factory(name, provider)</code>: создает службу</p></li>
<li><p><code>filter(name, factory)</code>: создает фильтр</p></li>
<li><p><code>provider(name, type)</code>: создает сервис</p></li>
<li><p><code>service(name, constructor)</code>: создает сервис</p></li>
</ul>
