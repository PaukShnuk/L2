...
<html >
<head>
<title>Отправка данных в AngularJS</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка данных с помощью ajax-запросов и метода $http.post в AngularJS и получение отправленных данных на стороне сервера">
...
<h2>Отправка данных</h2><div class="date">Последнее обновление: 27.04.2017</div>
...
<p>Отправка данных может производиться различными методами, но в данном случае мы рассмотрим отправку данных методом post().</p>
<p>Пусть у нас определена следующая страница:</p>
<pre class="brush:xml;">
&lt;!doctype html&gt;
...
<p>Кроме формы ввода я еще определил поля, которые привязываются к ответу с сервера.</p>
<p>Контроллер в файле <span class="b">AnswerController.js</span> тогда у нас будет выглядеть следующим образом:</p>
<pre class="brush:js;">
...
<p>Метод <code>$http.post()</code> отправляет объект answer по адресу <i>postAnswer.php</i></p>
<p>При удачной обработке ответа метод <code>success(function (answ)</code> отправленный сервером объект <code>answ</code> и устанавливает его в 
качестве значения <code>$scope.response</code></p>

<p>Скрипт <code>postAnswer.php</code> будет просто получать объект в json формате и его же отправлять в том же формате:</p>
<pre class="brush:php;">
&lt;?php
...
<img src="pics/post_angular.png" alt="Оправка в AngularJS" />
