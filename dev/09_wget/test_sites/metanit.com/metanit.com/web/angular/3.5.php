...
<html >
<head>
<title>Cервисы для работы с DOM в AngularJS</title>
<meta charset="utf-8" />
<meta name="description" content="Cервисы для работы с DOM в AngularJS">
...
<h2>Cервисы для работы с DOM</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>В прошлой главе мы рассматривали разные методы работы с DOM с помощью специального API jqLite. Кроме ранее расмотренных методов из этого API в AngularJS также определен ряд сервисов, 
которые представляют DOM API и которые позволяют работать со структурой веб-страницы:</p>
<ul>
<li><p><span class="bb">$anchorScroll</span>: перемещает полосу прокрутки к определенной ссылке на странице</p></li>
<li><p><span class="bb">$document</span>: предоставляет объект jqLite, который содержит стандартный объект <code>window.document</code></p></li>
<li><p><span class="bb">$interval</span>: представляет обертку вокруг функции <code>window.setInterval</code></p></li>
<li><p><span class="bb">$location</span>: предоставляет доступ к URL</p></li>
<li><p><span class="bb">$window</span>: предоставляет стандартный javascript-объект <code>window</code></p></li>
</ul>
...
<p>При использовании сервисов <code>$window</code> и <code>$document</code> в контроллере устанавливается для них зависимость. И затем мы можем использовать их функционал. 
В данном случае просто выводим окно сообщения, которое доступно в браузере через функцию <code>alert()</code>.</p>
<p>Выражение <code>$document.find("button").on("click", function (event)</code> установит для всех элементов button в качестве обработчика события "onclick" функцию</p>
<pre class="brush:js;">
function (event) {
	$window.alert(event.target.innerText);
}
</pre>
<p>При этом для кнопки "Не нажимай" также установлен обработчик "getAlert('Hell to World')" для события ng-click. Таким образом при нажатии на кнопку 
"Не нажимай" произойдет последовательный вызов двух обработчиков события "onclick": окно alert c текстом "Hell to World" и второе с текстом "Не нажимай".</p>
<img src="pics/window_service.png" alt="Сервис window в AngularJS" />
