...
<html >
<head>
<title>Директивы в AngularJS. ng-repeate</title>
<meta charset="utf-8" />
<meta name="description" content="Использование директив AngularJS для внедрения в код html новой функциональности, директивы ng-repeat, вывод на страницу массивов, использование встроенных переменных">
...
<h2>Директивы в AngularJS</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Как вы видели ранее, для добавления элементам html дополнительной функциональности AngularJS использует директивы. Например, 
в предыдущих разделах мы уже познакомились с рядом директив.</p>
<p>Как вы уже видели, директивы объявляются в виде атрибутов, например, <code>&lt;div ng-bind&gt;</code>. Однако уместен еще один 
вариант использования: <code>&lt;div class="ng-bind"&gt;</code></p>
<p>Некоторые директивы могут образовывать самостоятельные теги: <code>&lt;ng-form&gt; &lt;/ng-form&gt;</code></p>
<p>В AngularJS определено более 50 встроенных директив. Они охватывают широйчайший спектр функциональностей, которые востребованы во многих 
веб-приложениях: привязка данных, валидация форм, генерация шаблонов, обработка событий, управление html-элементами. Полный список директив вы можете посмотреть в документации. 
Сейчас же рассмотрим некоторые из них.</p>
<h3>Директива ng-repeat</h3>
<p>Директива <span class="b">ng-repeat</span> позволяет производить обход массивов в стиле цикла <code>for-each</code> и генерировать для них элементы html. 
Например, при рассмотрении контроллеров мы создали контроллер <span class="b">phoneController</span>. Изменим его так, чтобы он передавал в представление массив:</p>
<pre class="brush:js;">
...
<p>И чтобы вывести все элементы массива, применим директиву <b>ng-repeat</b>:</p>
<pre class="brush: xml;">
&lt;!doctype html&gt;
&lt;html ng-app="myApp"&gt;
&lt;head&gt;
...
<p>В данном случае с помощью выражения <code>&lt;li ng-repeat="phone in phones"&gt;</code> у нас будет создаваться для каждого объекта в наборе 
phones элемент <code>li</code>, который будет содержать определенную информацию.</p>
<img src="pics/4.png" alt="Директива ng-repeat в AngularJS" />
<h3>Встроенные переменные</h3>
<p>При обходе массивов мы можем использовать ряд встроенных переменных AngularJS:</p>
<ul>
<li><p><span class="bb">$index</span>: возвращает индекс элемента в массиве</p></li>
<li><p><span class="bb">$first</span>: возвращает true, если элемент является первым в массиве</p></li>
<li><p><span class="bb">$last</span>: возвращает true, если элемент является последним в массиве</p></li>
<li><p><span class="bb">$middle</span>: возвращает true, если элемент не является ни первым, ни последним в массиве</p></li>
<li><p><span class="bb">$even</span>: возвращает true, если элемент является четным</p></li>
<li><p><span class="bb">$odd</span>: возвращает true, если элемент является нечетным</p></li>
</ul>
<p>Используем пару переменных:</p>
<pre class="brush:xml;">
&lt;!doctype html&gt;
&lt;html ng-app="myApp"&gt;
&lt;head&gt;
...
<img src="pics/ng_repeat.png" alt="Директива ng-repeat в AngularJS" />
<h3>Обход внутренних массивов</h3>
<p>Возможна ситуация, когда объект в массиве, в свою очередь, сам содержит массив, и этот массив тоже надо обойти. В этом случае можно использовать вложеные директивы ng-repeat. 
Например, весь объект выглядит так:</p>
<pre class="brush:js;">
$scope.phones = [{
        name: 'Nokia',
        models:[{
...
<p>Для вывода вложенного массива можно использовать следующий код:</p>
<pre class="brush:xml;">
&lt;div ng-controller="phoneController"&gt;
	&lt;ul&gt;
