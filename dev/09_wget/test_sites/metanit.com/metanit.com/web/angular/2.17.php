...
<html >
<head>
<title>Обработка выражений в директивах AngularJS</title>
<meta charset="utf-8" />
...
<h2>Обработка выражений в директивах AngularJS</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>AngularJS предоставляет ряд сервисов, которые работают с выражениями AngularJS, в том числе и выражениями привязки. Разберем один из них - сервис 
<span class="bb">$compile</span>. Он преобразует HTML-код, содержащий выражения привязки и директивы, в функцию, вызываемую для генерации контента.</p>
<p>Чтобы применить сервис <code>$compile</code>, возьмем директиву answerList из прошлой темы и изменим ее следующим образом:</p>
<pre class="brush:js;">
...
<p>Сервис $compile представляет функцию, генерирующую контент из фрагментов html. Чтобы наполнить контент данными из контроллера, в функцию передается 
параметр scope: <code>compileFn(scope);</code></p>
<p>Завершает работу метод <code>element.append(answersElem)</code>, который добавляет сгенерированный контент в структру DOM веб-страницы.</p>
...
<div class="nav"><p><a href="2.16.php">Назад</a><a href="./">Содержание</a><a href="3.1.php">Вперед</a></p></div></div>
<div class="socBlock">
