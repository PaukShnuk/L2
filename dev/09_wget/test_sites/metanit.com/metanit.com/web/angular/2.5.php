...
<html >
<head>
<title>Cоздание собственных фильтров AngularJS</title>
<meta charset="utf-8" />
<meta name="description" content="Создание собственных фильтров в AngularJS">
...
<h2>Cоздание собственных фильтров</h2><div class="date">Последнее обновление: 26.04.2017</div>
...
<p>Возможно, кому-то набор встроенных фильтров покажется ограниченным. Но разработчики фреймворка предоставляют нам возможность создавать собственные фильтры.</p>
<h3>Определение фильтра</h3>
<p>Фильтр по сути представляет функцию, которая выполняется над переданным значением. Ее типизированное представление выглядит следующим образом:</p>
<pre class="brush:js;">
...
<p>В этой конструкции <code>module</code> - название модуля приложения. <code>filter_name</code> - произвольное название функции фильтра. 
Параметр возвращаемой функции - <code>param</code> - параметр, над которым будут производиться действия. И <code>some_value</code> - 
некоторое возвращаемое значение. В качестве значения может выступать текст самого параметра или какое-то иное значение. Посмотрим на примере.</p>
<p>Для начала создадим модуль в отдельном файле - назовем его <span class="b">app.js</span> и положим в папку <span class="b">js</span>:</p>
<pre class="brush:js;">
var questApp = angular.module('questApp', []);
</pre>
<p>Затем изменим контроллер QuestionController:</p>
<pre class="brush:js;">
...
<p>Теперь создадим фильтр. Его задачей будет фильтрация сообщений, и если сообщение содержит некорректное выражение, то его заменяем звездочками. Для 
этого создадим отдельный файл <span class="b">filter.js</span> в каталоге <span class="b">js</span> со следующим содержанием:</p>
<pre class="brush:js;">
...
<img src="pics/19.png" alt="AngularJS и создание своих фильтров" />
...
<p><a href="custom_filters.zip" rel="nofollow">Загрузить проект</a></p>
<p>И поскольку один из ответов к вопросу содержит некорректное слово, то при выводе оно будет заменяться звездочками:</p>
<img src="pics/12.png" />
