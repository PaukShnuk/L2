...
<html >
<head>
<title>Работа со структурой DOM в AngularJS</title>
<meta charset="utf-8" />
...
<h2>Работа со структурой DOM</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Подобно ряду многих других js-фреймворков и библиотек AngularJS позволяет работать со структорой DOM html-страницы: то есть добавлять, удалять, изменять html-элементы. 
Подобная функциональность реализована с помощью специального API, которое во многом напоминает jQuery и называется <b>jqLite</b> (jQuery - lite). Данное API 
реализовано в виде методов, которые позволяют управлять html-элементом. Рассмотрим ряд методов.</p>
<h3>Навигация по странице</h3>
<p>Для навигации по html-странице мы можем применять ряд методов:</p>
<ul>
<li><p><code>children()</code>: возвращает набор дочерних элементов</p></li>
<li><p><code>eq(index)</code>: возвращает элемент по определенному индексу из коллекции элементов</p></li>
<li><p><code>find(tag)</code>: получает все элементы по определенному тегу</p></li>
<li><p><code>next()</code>: возвращает следующий сестринский элемент</p></li>
<li><p><code>parent()</code>: возвращает родительский элемент</p></li>
<li><p><code>html()</code>: возвращает html-код элемента</p></li>
<li><p><code>text()</code>: возвращает текстовое содержимое элемента</p></li>
</ul>
<p>Например, возьмем структуру страницы из прошлых тем:</p>
<pre class="brush:xml;">
&lt;!doctype html&gt;
...
<p>Применим ряд методов для получения элементов:</p>
<pre class="brush:js;">
// получаем элент body
...
<p>Для конкретизации выборки используется метод javascript <code>document.querySelector(".quest")</code>, который в качестве параметра принимает 
селектор. Затем к найденному элементу применяется метод <span class="b">angular.element()</span>, 
который фактически создает обертку для найденного элемента. И после мы можем работать с полученным элементом с помощью методов angularjs.</p>
<p>Также обратите внимание, что свойство <code>questHeader.text()</code> будет содержать текст <code>{{question.text}}</code>, то есть текст до обработки 
его AngularJS.</p>
<h3>Манипуляция элементами</h3>
<ul>
<li><p><code>addClass(name)</code>: добавляет к элементу класс</p></li>
<li><p><code>attr(name) / attr(name, value)</code>: получает значение value атрибута name элемента и устанавливает его</p></li>
<li><p><code>css(name) / css(name, value)</code>: получает значение value стиля name у элемента и устанавливает его</p></li>
<li><p><code>hasClass(name)</code>: возвращает true, если элемент имеет класс name</p></li>
<li><p><code>prop(name) / prop(name, value)</code>: получает значение value свойстваа name элемента и устанавливает его</p></li>
<li><p><code>removeAttr(name)</code>: удаляет у элемента атрибут</p></li>
<li><p><code>removeClass(name)</code>: удаляет у элемента класс</p></li>
<li><p><code>text(value)</code>: устанавливает текстовое значение у элемента</p></li>
<li><p><code>toggleClass(name)</code>: переключает класс - класс name добавляется, а все ранее имевшиеся классы удаляются</p></li>
<li><p><code>val() / val(value)</code>: получает значение атрибута value и устанавливает его значение</p></li>
</ul>
...
<p>Следующие методы позволяют управлять созданием и удалением элементов:</p>
<ul>
<li><p><code>angular.element(html)</code>: создает определенный элемент с кодом html</p></li>
<li><p><code>after(elements)</code>: добавляет элементы elements после элемента, на котором вызывается данный метод</p></li>
<li><p><code>append(elements)</code>: добавляет элементы elements в качестве дочерних в конец элемента, на котором вызывается данный метод</p></li>
<li><p><code>clone()</code>: создает копию элемента, для которого вызывается метод</p></li>
<li><p><code>prepend(elements)</code>: добавляет элементы elements в качестве дочерних в начало элемента, на котором вызывается данный метод</p></li>
<li><p><code>remove()</code>: удаляет элемент</p></li>
<li><p><code>replaceWith(element)</code>: заменяет элемент, на котором вызывается метод, другим элементом, который передается в качестве параметра</p></li>
<li><p><code>wrap(element)</code>: обертывает элемент, на котором вызывается метод, новым элементом, передающимся в качестве параметра</p></li>
</ul>
...
<ul>
<li><p><code>on(events, handler)</code>: регистрирует обработчик для одного или нескольких событий</p></li>
<li><p><code>off(events, handler)</code>: удаляет ранее зарегистрированный обработчик для нескольких событий</p></li>
<li><p><code>triggerHandler(event)</code>: вызывает обработчики для события</p></li>
</ul>
<pre class="brush:js;">
