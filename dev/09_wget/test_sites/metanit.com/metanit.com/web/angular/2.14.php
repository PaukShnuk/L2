...
<html >
<head>
<title>Параметры директив в AngularJS</title>
<meta charset="utf-8" />
...
<h2>Параметры директив</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>С помощью дополнительных свойств мы можем настроить применение директивы. Рассмотрим два свойства: <span class="bb">link</span> и 
<span class="bb">restrict</span>.</p>
<p>Свойство <code>link</code> позволяет задать функцию директивы. А свойство <code>restrict</code> позволяет указать объект, к которому директива будет 
применяться. Оно может иметь следующие значения:</p>
<ul>
<li><p><code>E</code>: директива применяется к элементу</p></li>
<li><p><code>A</code>: директива применяется к атрибуту</p></li>
<li><p><code>C</code>: директива применяется к классу</p></li>
<li><p><code>M</code>: директива применяется к комментарию</p></li>
</ul>
<p>Данные значения можно комбинировать.</p>
<p>Например, возьмем директиву из прошлой темы и используем выше рассмотренные свойства:</p>
<pre class="brush:js;">
...
<p>В данном случае директива применяется ко всем четырем типам объектов. Рассмотрим, как применять директиву к отдельным объектам.</p>
<p><a href="directive_restrict_link.zip" rel="nofollow">Загрузить проект</a></p>
<h3>Применение директивы к элементам</h3>
<p>Для применения директивы к элементам, нам надо создать соответствующий элемент:</p>
<pre class="brush:xml;">&lt;answer-list source="question" /&gt;</pre>
<p>Здесь добавлен еще один атрибут - source, который принимает данные для директивы. Тогда в самой директиве нам надо изменить метод получения данных:</p>
<pre class="brush:js;">var data = scope[attrs["answerList"] ||attrs["source"]];</pre>
<h3>Применение директивы к классам</h3>
<p>Преобразуем элемент в следующий:</p>
<pre class="brush:xml;">&lt;div class="answer-list: question"&gt;&lt;/div&gt;</pre>
<p>Код директивы остается тем же.</p>
<h3>Применение директивы к комментариям</h3>
<p>Комментарий в коде html должен начинаться со слова <code>directive</code>, за которым следует двоеточие, имя директивы и объект, передаваемый в директиву:</p>
<pre class="brush:xml;">&lt;!-- directive: answer-list question --&gt;</pre>
<p>Тогда в коде директивы определяются два возможных пути: если директивы применяется к комментарию и остальные случаи</p>
<pre class="brush:js;">
if (element[0].nodeName == "#comment") {
	element.parent().append(divElem);
} 
