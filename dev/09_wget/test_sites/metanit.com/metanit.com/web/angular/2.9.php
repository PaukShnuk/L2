...
<html >
<head>
<title>Директивы шаблонов в AngularJS</title>
<meta charset="utf-8" />
<meta name="description" content="Директивы шаблонов в AngularJS, подключение представления через ng-include">
...
<h2>Директивы шаблонов в AngularJS</h2><div class="date">Последнее обновление: 26.04.2017</div>
...
<p>Рассмотрим еще ряд директив, который позволяют управлять разметкой html.</p>
<h3>ng-include</h3>
<p>Директива <code>ng-include</code> получает с веб-сервера фрагмент кода HTML, обрабатывает его и добавляет на html-страницу. 
Подобные фрагменты называются <b>частичными представлениями</b>.</p>
<p>Создадим в одной папке рядом с главной страницей index.html новый файл <span class="b">phonesList.html</span> со следующим содержимым:</p>
<pre class="brush:xml;">
&lt;ul&gt;
...
<p>Теперь подключим этот файл на главную страницу:</p>
<pre class="brush:xml;">
&lt;!doctype html&gt;
&lt;html ng-app="myApp"&gt;
&lt;head&gt;
...
<p>Директива <code>ng-include</code> может принимать три параметра:</p>
<ul>
<li><p><code>src</code>: url загружаемого файла</p></li>
<li><p><code>onload</code>: определяет выражение, которое рендерится при загрузке файла</p></li>
<li><p><code>autoscroll</code>: определяет, допускается ли прокрутка во время загрузки файла</p></li>
</ul>
<p>В папке <span class="ii">js/controllers/</span> по-прежнему будет файл phonesController.js с определением контроллера, из которого данные будут передаваться в представление:</p>
<pre class="brush:js;">
var myApp=angular.module('myApp', []);
...
<p>То есть в итоге весь проект будет выглядеть следующим образом:</p>
<img src="pics/16.png" alt="Проект AngularJS и представления html" />

<p>Поскольку для загрузки представления, в частности, файла phonesList.html AngularJS использует Ajax, то мы не сможем напрямую кинуть файл веб-страицы в веб-браузер. 
Нам надо обращаться к нему через веб-сервер:</p>
<img src="pics/17.png" alt="Проект AngularJS и ngInclude" />

<h3>Динамическая загрузка представлений</h3>
<p>Изменим приложение. Допустим, нам динамически надо подгружать одно из представлений в зависимости от выбора пользователя. Для начала изменим 
контроллер <span class="b">phoneController</span>:</p>
<pre class="brush:js;">
var myApp=angular.module('myApp');
myApp.controller('phoneController', function($scope) {
...
<p>Теперь контроллер передает в представление два разных списка - для смартфонов и планшетов. Также есть список modes, который содержит режимы выбора 
и пустой объект <code>data</code>. Кроме того, в $scope определена функция <code>setFile</code>, которая возвращает нужный файл в зависимости от значения 
<code>$scope.data.mode</code></p>
<p>Файл <span class="ii">phonesList.html</span> был сделан выше. А файл <span class="ii">tabletsList.html</span> будет выглядеть аналогично:</p>
<pre class="brush:xml;">
&lt;ul&gt;
...
<p>Теперь изменим главную страницу:</p>
<pre class="brush:xml;">
&lt;!doctype html&gt;
&lt;html ng-app="myApp"&gt;
&lt;head&gt;
...
<img src="pics/nginclude.png" alt="Директива ng-include в AngularJS" />
<P>Теперь привязка идет к результату метода setFile: <code>&lt;div ng-include="setFile()"&gt;</code>. Для переключения между представлениями определены две радиокнопки: 
<code>&lt;input type="radio" ng-model="data.mode" value="{{button.value}}"</code>. С помощью директивы <code>ng-model="data.mode"</code> происходит привязка 
свойства data.mode к значению радиокнопки, которое берется из button.value.</P>
