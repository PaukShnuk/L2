...
<html >
<head>
<title>Потенциально опасные данные в AngularJS</title>
<meta charset="utf-8" />
<meta name="description" content="Потенциально опасные данные в AngularJS, библиотека ng-sanitize, автоматическое экранирование символов, сервис $sanitize">
...
<h2>Потенциально опасные данные</h2><div class="date">Последнее обновление: 26.04.2017</div>
...
<p>При работе с данными какая-то часть может представлять набор кода html/javascript, что может нести потенциальные риски для пользователей. Но в AngularJS 
существуют механизмы, которые позволяют уменьшить риск, который несут в себе подобные данные. Главную роль в данном случае будут играть два сервиса:</p>
<ul>
<li><p><span class="bb">$sce</span>: удаляет потенциально опасные элементы и атрибуты из кода html</p></li>
<li><p><span class="bb">$sanitize</span>: заменяет потенциально опасные символы эскейп-последовательностями</p></li>
</ul>
<p>По умолчанию AngularJS использует такую функциональность, как <span class="b">SCE</span> (strict contextual escaping), которая защищает от привязки к потенциально 
опасному коду html. Используем сервис <code>$sanitize</code>:</p>
<pre class="brush:xml;">
&lt;!doctype html&gt;
&lt;html ng-app="myApp"&gt;
&lt;head&gt;
...
<p>Для использования привязки к html-коду и использования сервиса $sanitize нам надо вначале подключить файл <i>angular-sanitize.js</i> (либо его 
минимизированную версию), который идет в стандартном пакете фреймворка AngularJS.</p>
<p>Чтобы применить сервис, нам надо установить для него зависимость: <code>myApp.controller('helloController', function($scope, <b>$sanitize</b>){</code></p>
<p>Далее создаем модель с html-кодом, которая будет использоваться на веб-странице: <code>$scope.htmlcode="&lt;div&gt;Жми &lt;b&gt;тут&lt;/b&gt;&lt;/div&gt;";</code></p>
<p>Чтобы динамически отслеживать все изменения модели, применяется сервис <code>$scope.$watch</code>, который в качестве первого параметра принимает 
название модели, а в качестве второго - функцию, в которую передается новое значение. Вся работа функции сводится к использованию сервиса <code>$sanitize</code></p>
<p>В результате при вводе в текстовое поле новых символов, сервис $sanitize будет кодировать теги и некоторые другие символы:</p>
<img src="pics/sanitize_service.png" alt="Сервис $sanitize в AngularJS" />
