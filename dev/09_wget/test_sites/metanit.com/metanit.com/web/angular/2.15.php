...
<html >
<head>
<title>Шаблоны директив в AngularJS</title>
<meta charset="utf-8" />
<meta name="description" content="Шаблоны директив в AngularJS, динамическая загрузка шаблонов по условию">
...
<h2>Шаблоны директив</h2><div class="date">Последнее обновление: 26.04.2017</div>
...
<p>В прошлых темах мы рассмотрели создание директив, в которых применялось API jqLite с использованием метода <code>angular.element()</code> и 
...
<span class="bb">template</span>.</p>
<p>Используем шаблоны в директиве:</p>
<pre class="brush:js;">
...
<p>В шаблонах используются все те же стандартные директивы и выражения AngularJS.</p>
<p>Чтобы упростить использования шаблонов, мы можем использовать внешние файлы для их хранения. Создадим рядом с главной страницей index.html 
папку <span class="b">templates</span>. Добавим в нее новый файл <span class="b">answersTemplate.html</span> со следующим содержимым:</p>
<pre class="brush:xml;">
&lt;div class="answers"&gt;
...
<p>Теперь проект выглядит следующим образом:</p>
<img src="pics/21.png" alt="Шаблоны в проекте Angular JS" />
<p>По сути это весь тот код, который ранее был заключен в свойстве <code>template</code>. Теперь изменим директиву answerList следующим образом:</p>
<pre class="brush:js;">
questApp.directive("answerList", function () {
	return {
		link: function (scope, element, attrs) {
...
<p>Теперь вместо свойства <code>template</code> используется свойство <span class="b">templateUrl</span>, которое позволяет установить адрес шаблона.</p>
