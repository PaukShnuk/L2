...
<html >
<head>
<title>Валидация формы в AngularJS</title>
<meta charset="utf-8" />
<meta name="description" content="Особенности валидации форм в AngularJS, связывание с помощью директивы ng-model">
...
<h2>Валидация формы</h2><div class="date">Последнее обновление: 26.04.2017</div>
...
<p>При создании нового объекта нередко могут возникать ошибки. Но механизм валидации AngularJS позволяет нам информировать пользователя об ошибках. 
Возьмем, к примеру, html-страницу из прошлого раздела и добавим к ней форму:</p>
...
<p>За работу с этой формой будет отвечать контроллер AnswerController, который мы сейчас создадим и который мы подключаем внизу страницы. Чтобы 
связать создаваемый объект с полями, используется директива <span class="b">ng-model</span>. Поле с атрибутом <code>ng-model="answer.text"</code>, таким образом, будет отвечать 
за свойство text создаваемого формой объекта answer.</p>
<p>Так как нам надо, чтобы все поля перед отправкой были заполнены, устанавливаем для каждого поля атрибут <code>required</code>, который, 
в случае незаполнения поля, укажет пользователю, что данное поле надо обязательно заполнить.</p>
<p>Для поля для ввода даты также устанавливаем атрибут <code>ng-pattern="/\d\d/\d\d/\d\d\d\d/"</code>, который указывает на формат вводимой даты. 
Директива ng-pattern принимает регулярное выражение, которому должен соответствовать вводимое значение. То есть в данном случае регулярное выражение соответствует, например, выражению 12/11/2013.</p>
<p>И, наконец, для кнопки отправки установлено событие нажатия: <code>ng-click="save(answer, answerForm)"</code>, которое вызывает функцию save 
контроллера AnswerController, передавая в нее в качестве параметров объект answer, создаваемый, полями формы, и объект формы answerForm 
(так как у элемента form атрибут name="answerForm").</p>
<p>Теперь создадим простенький контроллер AnswerController:</p>
<pre class="brush:js;">
...
<p>С помощью выражения <code>answerForm.$valid</code> просматриваем, прошла форма валидацию или нет.</p> 
<p>Запустим и попробуем ввести различные значения:</p>
<img src="pics/13.png" alt="Валидация формы в AngularJS" />
<p>Как видно на рисунке, поле для ввода автора не заполнено, поэтому срабатывает атрибут required, который помечает поле - в данном случае это сообщение 
о необходимости заполнить поле. В других браузерах действие может быть иным, например, в Firefox просто выделяет границу поля красным цветом.</p>
<p>Если все поля заполнены правильно, то контроллер отобразит нам сообщение. Если же, например, значение поля даты не будет соответствовать 
установленному для него регулярному выражению, то форма не пройдет валидацию и сообщение, естественно, не отобразится.</p>
...
<div class="nav"><p><a href="2.5.php">Назад</a><a href="./">Содержание</a><a href="2.13.php">Вперед</a></p></div></div>
<div class="socBlock">
