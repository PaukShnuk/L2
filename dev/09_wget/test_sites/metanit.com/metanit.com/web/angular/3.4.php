...
<html >
<head>
<title>Сервис $timeout</title>
<meta charset="utf-8" />
<meta name="description" content="Использование сервиса $timeout в AngularJS для выполнения действия через определенный промежуток времени, а также остановка выполнения этого действия">
...
<h2>Сервис $timeout</h2><div class="date">Последнее обновление: 19.02.2016</div>
...
<p>Cервис $timeout выполняет некоторые действия по истечению определенного промежутка времени. Однако не следует воспринимать этот сервис как 
аналог стандартной функции javascript <code>setTimeout</code>, это разные вещи, хотя сходство между ними в функциональности есть. Например, сделаем так, чтобы контроллер из 
прошлой темы загружал данные через некоторый период:</p>
<pre class="brush:js;">
...
<p>Здесь функция, определенная в первом параметре, выполняется через промежуток времени, указанный во втором параметре - то есть через 3 секунды.</p>
<pre class="brush:js;">
var promiseObj = $timeout(function(){
    return dataService.getData();
}, 3000);
</pre>
<p>На выходе у нас получается объект promiseObj, который мы затем можем использовать. Например, если нам надо остановить выполнение сервиса 
$timeout, то мы можем вызвать следующий метод: <code>$timeout.cancel(promiseObj)</code></p>
