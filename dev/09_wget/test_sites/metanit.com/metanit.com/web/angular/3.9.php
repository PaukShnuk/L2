...
<html >
<head>
<title>Передача параметров в ajax-запрос в AngularJS</title>
<meta charset="utf-8" />
<meta name="description" content="Передача параметров в ajax-запрос в AngularJS, обработка ответа от сервера">
...
<h2>Передача параметров в запрос</h2><div class="date">Последнее обновление: 27.04.2017</div>
...
<p>Ранее мы рассмотрели отправку простых запросов. Теперь посмотрим, как мы можем в запросе отправлять параметры.</p>
<p>В прошлых темах использовались методы, которые могли увеличивать рейтинг ответов. Но все эти изменения происходили локально. И если 
мы загружаем данные с сервера, то соответственно надо также дать серверу знать о произведенных изменениях.</p>
<p>Для для взаимодействия с сервером нам надо уникально идентифицировать каждый пост с ответом. Поэтому вначале добавим в определение объектов в 
файле question.json параметр id:</p>
<pre class="brush:js;">
{
    "question":{
...
<p>Страница <i>index.html</i> остается такой же:</p>
<pre class="brush:xml;">
&lt;!doctype html&gt;
...
<p>И теперь определим отправку на сервер соответствующих запросов в контроллере QuestionController:</p>
<pre class="brush:js;">
...
<p>Для установки параметров используется параметр <code>params</code>, который принимает два параметра: id ответа и булевое значение, показывающее, было ли увеличение рейтинга.</p>
<p>В качестве ресурса для запроса определен скрипт <i>setAnswer.php</i>, но с учетом параметров фактический адрес запроса выглядит так: 
<i>setAnswer.php?id=[answer.id]&amp;up=[true или false]</i></p>
<p>Скрипт setAnswer.php может выглядеть так:</p>
<pre class="brush:php;">
&lt;?php
...
<div class="nav"><p><a href="3.8.php">Назад</a><a href="./">Содержание</a><a href="3.3.php">Вперед</a></p></div></div>
<div class="socBlock">
