...
<html >
<head>
<title>Angular | Иерархия сервисов</title>
<meta charset="utf-8" />
<meta name="description" content="Иерархия сервисов в Angular, сервисы уровня приложения, модуля и компонента, декоратор Injectable и параметр providedIn">
...
<h2>Иерархия сервисов</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Как было показано в предыдущих статьях, мы можем внедрять сервисы в компонентах и модулях. В зависимости от того, где внедряются сервисы, 
бывают различные уровни провайдеров сервисов. Уровень по сути определяет область действия и жизненный цикл сервиса. Есть три уровня провайдеров сервисов:</p>
<ul>
<li><p><span class="b">Глобальный или корневой уровень</span></p></li>
<li><p><span class="b">Уровень модуля</span></p></li>
<li><p><span class="b">Уровень компонента</span></p></li>
</ul>
<p>Для установки соответствующего уровня сервиса есть два способа: добавление сервиса в коллекцию <span class="b">providers</span> в модуле или компоненте и 
установка уровня с помощью параметра <span class="b">providedIn</span> в декораторе <span class="b">@Injectable</span>.</p>
<p>С одной стороны, может показаться, что нет смысла в таком разделение - почему бы для всех сервисов не сделать один корневой уровень, чтобы один объект сервиса был доступен 
для всего приложения по типу синглтона. Однако нередко возникает нобходимость разграничить функционал между отдельными функциональными частями приложения. Например, 
когда два компонента работают с разным набором данных - в этом случае они могут использовать один класс сервиса, но разные его объекты.</p>
<h3>Корневой уровень</h3>
<p>Корневой уровень (root level) предусматривает действие сервиса для всего приложения. Создается и используется один объект сервиса для всех частей приложения.</p>
<p>Подобный уровень устанавливается, если сервис добавляется в коллекцию providers главного модуля, который обычно называется AppModule и с запуска которого начинается работа приложения. 
Например:</p>
<pre class="brush:ts;">
...
<p>Если в данном случае AppModule является главным модулем, то для сервиса DataService определен корневой уровень. То есть во время работы приложения 
будет создаваться один объект данного сервиса для всего приложения.</p>
<p>Значение <span class="b">root</span> для параметра providedIn в декораторе <span class="b">Injectable</span> также позволяет установить корневой уровень действия:</p>
<pre class="brush:ts;">
...
<p>В этом случае мы можем не добавлять данный сервис в коллекцию providers у главного модуля.</p>
<h3>Уровень модуля</h3>
<p>Сервисы уровня модуля действуют только для текущего модуля и всех его классов - компонентов, директив, сервисов. Это все те сервисы, которые добавляются 
в коллекцию <span class="b">providers</span> во всех других модулях, кроме главного модуля. Например:<p>
<pre class="brush:ts;">
import { NgModule }      from '@angular/core';
import { BrowserModule } from '@angular/platform-browser';
import { FormsModule }   from '@angular/forms';
...
<p>Либо мы можем задать уровень модуля непосредствнно в сервисе:</p>
<pre class="brush:ts;">
import {Injectable} from '@angular/core';
...
<h3>Уровень компонента</h3>
<p>В этом случае действие сервиса ограничено текущим компонентом. Для каждого объекта компонента создается свой объект сервиса. Сам сервис добавляется также в коллекцию providers компонента:</p>
<pre class="brush:ts;">
