...
<html >
<head>
<title>Angular | Модули</title>
<meta charset="utf-8" />
<meta name="description" content="Модули в Angular, роль декоратора @NgModule, определение и использование модулей">
...
<h2>Модули</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Приложение Angular состоит из отдельных модулей. Как правило, приложения состоят из нескольких модулей. И каждое приложение Angular как минимум имеет 
один корневой модуль (root module), который, согласно условностям, называется <span class="b">AppModule</span>. Например, в первой главе использовался следующий корневой модуль:</p>
<pre class="brush:ts;">
...
<p>Для работы модуля ему необходимы ряд библиотек, поэтому в начале файла идет их подключение. 
Имя каждой библиотеки Angular начинается с префикса <span class="ii">@angular</span>.</p>
<p>Библиотеки устанавливаются через пакетный менеджер npm и импортируются с помощью директивы <code>import</code>. 
Например, импорт функциональности декоратора <code>NgModule</code> из библиотеки <span class="ii">@angular/core</span>:</p>
<pre class="brush:ts;">import { NgModule } from '@angular/core';</pre>
<p>То есть здесь мы должны импортировать все модули и классы, которые использует данный модуль. В частности, для AppModule необходимы:</p>
<ul>
<li><p><code>NgModule</code>: функциональность декоратора NgModule, без которой мы не сможем создать модуль</p></li>
<li><p><code>BrowserModule</code>: модуль, необходимый для работы с браузером</p></li>
<li><p><code>FormsModule</code>: модуль, необходимый для работы с формами html и, в частности, с элементами input. (Так как класс компонента работает с подобными 
элементами, то мы обязаны также импортировать этот модуль)</p></li>
<li><p><code>AppComponent</code>: функциональность корневого компонента приложения</p></li>
</ul>
<p>Непосредственно сам модуль представлен классом <code>AppModule</code>, который на первый взгляд ничего не делает и не содержит никакого функционала:</p>
<pre class="brush:ts;">export class AppModule { }</pre>
<p>Однако в Angular модуль это не просто класс. Каждый модуль должен определяться с декоратором <span class="b">@NgModule</span>.</p>
<p>NgModule представляет функцию-декоратора, которая принимает объект, свойства которого описывают метаданные модуля. Наиболее важные свойства:</p>
<ul>
<li><p><span class="b">declarations</span>: классы представлений (view classes), которые принадлежат модулю. Angular имеет три типа классов представлений: 
компоненты (components), директивы (directives), каналы (pipes)<p></li>
<li><p><span class="b">exports</span>: набор классов представлений, которые должны использоваться в шаблонах компонентов из других модулей<p></li>
<li><p><span class="b">imports</span>: другие модули, классы которых необходимы для шаблонов компонентов из текущего модуля<p></li>
<li><p><span class="b">providers</span>: классы, создающие сервисы, используемые модулем<p></li>
<li><p><span class="b">bootstrap</span>: корневой компонент, который вызывается по умолчанию при загрузке приложения<p></li>
</ul>
<p>В случае выше единственным классом представлений является компонент AppComponent. Поэтому он указывается для свойств declarations и bootstrap. 
И поскольку его действие зависит от модулей BrowserModule и FormsModule, то данные модули указываются для свойства imports.</p>
<p>Но при необходимости мы могли бы использовать и другие свойства:</p>
<pre class="brush:ts;">
@NgModule({
    imports:      [ BrowserModule, FormsModule ],
    declarations: [ AppComponent ],
