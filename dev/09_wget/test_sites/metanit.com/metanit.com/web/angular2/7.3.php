...
<html >
<head>
<title>Angular | Параметры маршрута</title>
<meta charset="utf-8" />
<meta name="description" content="Параметры маршрута в Angular, получение параметров, подписка на изменение параметров">
...
<h2>Параметры маршрута</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Маршруты могут определять параметры, через которые мы можем передавать компоненту какие-то данные извне. Для примера возьмем проект из прошлой темы:</p>
<img src="./pics/7.17.png" alt="Маршрутизация в Angular" />
<p>В папку src/app добавим новый файл <span class="b">item.component.ts</span> с новым компонентом:</p>
<pre class="brush:ts;">
...
<p>Для получения параметров маршрута нам необходим специальный сервис <span class="b">ActivatedRoute</span>. Он содержит информацию о маршруте, в частности, 
параметры маршрута, параметры строки запроса и прочее. Он внедряется в приложение через механизм dependency injection, поэтому в конструкторе мы можем получить его.</p>
<p>Свойство <code>snapshot</code> хранит состояние маршрута, а состояние маршрута содержит переданные параметры.</p>
<p>В данном случае мы предполагаем, что параметр будет называться "id", но это необязательно. Название параметра может быть любым.</p>
<p>Но пока нет никакого маршрута, который бы использовал данный компонент. Поэтому изменим класс модуля приложения:</p>
<pre class="brush:ts;">
...
<p>Здесь добавлен новый маршрут <code>{ path: 'item/:id', component: ItemComponent}</code></p>
<p>Токен <code>:id</code> представляет параметр маршрута. То есть мы сможем обратиться к компоненту с запросом типа 
<span class="ii">/item/6</span>, и число 6 будет представлять параметр id.</p>
<p>Теперь изменим главный компонент AppComponent:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>При определении ссылки на компонент, который использует параметры маршрута, все необходимые параметры заключаются в квадратные скобки:</p>
<pre class="brush:ts;">&lt;a [routerLink]="['item', '5']"&gt;item 5&lt;/a&gt;</pre>
<p>И при запуске приложения мы сможем по ссылке обращаться к компоненту ItemComponent и передавать ему id=5:</p>
<img src="./pics/7.5.png" alt="Параметры маршрутов в Angular 13" />
<p>Данный подход работает для одной ссылки, но что если у нас несколько ссылок на один и тот же компонент, но с разными id:</p>
<pre class="brush:xml;">
&lt;a [routerLink]="['item', '5']"&gt;item 5&lt;/a&gt;
&lt;a [routerLink]="['item', '8']"&gt;item 8&lt;/a&gt;
</pre>
<p>Если мы сначала перейдем по первой ссылке, то компонент ItemComponent получит id=5. Если после этого мы перейдем по второй ссылке, ItemComponent 
будет продолжать хранить прежнее значение id = 5.</p>
<p>Чтобы решить эту проблему, нам надо динамически изменять значение id в компоненте ItemComponent, поэтому изменим его код:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
import { ActivatedRoute} from '@angular/router';
...
<p>Метод <code>subscribe()</code> позволяет установить подписку на изменение параметра маршрута. В этом случае компонент будет получать новое значение, и 
проблем с навигацией по ссылкам с разными параметрами не возникнет.</p>
<p>Для решения выше озвученной проблемы также есть и другое решение. Изменим ItemComponent следующим образом:</p>
<pre class="brush:ts;">
...
<p>В данном случае получение id происходит в методе ngOnInit. Для этого происходит обращение к свойству <code>this.route.paramMap</code>, которое 
представляет объект <code>Observable&lt;ParamMap&gt;</code> (ParamMap представляет карту параметров -как параметров маршрута, так и строки запроса). 
Далее у Observable вызывается метод <code>pipe()</code>, который позволяет создать цепочку операторов rxjs.</p>
<p>В методе pipe() вызывается оператор <code>switchMap()</code>, который в качестве параметра принимает функцию с одним параметром - ParamMap. Таким образом мы 
извлекаем карту параметров и берем из нее значение параметра id. При изменении значения параметра switchMap позволяет получить новое значение.</p>
<p>По умолчанию значение параметра id представляет строку, поэтому далее с помощью метода subcribe мы подписываемся на объект Observable и приналичии данных извлекаем значение id. 
Плюс перед значением <code>+data</code> указывает на преобразование к числу.</p>
