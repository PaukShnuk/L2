...
<html >
<head>
<title>Angular | Сервисы и dependency injection</title>
<meta charset="utf-8" />
<meta name="description" content="Сервисы и dependency injection в Angular, внедрение сервиса через конструктор">
...
<h1>Сервисы и dependency injection</h1><h2>Сервисы</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Допустим, у нас есть следующая базовая структура проекта:</p>
<img src="./pics/4.5.png" alt="Сервисы в Angular и Typescript" />
<p>Добавим в папку <span class="ii">src/app</span> новый файл <span class="b">data.service.ts</span>. Этот файл будет содержать код сервиса. 
Согласно условностям в названии файла после названия самого сервиса должна идти подстрока <span class="ii">.service</span>. 
...
если сервис называется <code>SpecialSuperHeroService</code>, то файл сервиса будет называться <span class="ii">special-super-hero.service.ts</span>.</p>
<p>Определим в этом файле следующий сервис:</p>
<pre class="brush:ts;">
...
<p>В сервисе определен массив данных и методы для работы с ним. В реальном приложении эти данные можно получать с сервера или из какого-либо внешнего хранилища.</p>
<p>Теперь определим код компонента:</p>
<pre class="brush:ts;">
...
<p>Чтобы задействовать сервис в компоненте, его не только надо импортировать:</p>
<pre class="brush:ts;">import {DataService} from './data.service';</pre>
<p>Но также необходимо его добавить в коллекцию providers компонента:</p>
<pre class="brush:ts;">providers: [DataService]</pre>
<p>Все используемые сервисы добавляются в коллекцию providers. Тем самым определяется провайдер сервиса, который будет использоваться для его получения. 
И после этого мы сможем задействовать встроенный в Angular механизм 
dependency injection и получить объект сервиса в конструкторе компонента и затем использовать по необходимости:</p>
<pre class="brush:ts;">constructor(private dataService: DataService){}</pre>
<p>Для загрузки данных реализуется метод <code>ngOnInit()</code> интерфейса OnInit, который вызывается после конструктора.</p>
<p>И так как мы используем директиву ngModel для работы с элементами формы, то нам надо подключить модуль FormsModule в файле app.module.ts:</p>
<pre class="brush:ts;">
...
<p>В итоге мы сможем и выводить данные и добавлять новые элементы через сервис:</p>
<img src="./pics/4.1.png" alt="Сервисы в Angular 13" />
