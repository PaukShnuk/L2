...
<html >
<head>
<title>Angular | Программная навигация</title>
<meta charset="utf-8" />
<meta name="description" content="Программная навигация в Angular, сервис Router и его метод navigate, использование параметров при навигации">
...
<h2>Программная навигация</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Мы можем переходить по ресурсам внутри приложения с помощью ссылок. Но также Angular поддерживает программную навигацию. 
То есть программным образом из любого места приложения мы можем перейти к любому ресурсу. Для этого применяется сервис <span class="b">Router</span>, 
который определен в пакете "angular/router" и который передается в компоненты через механизм dependency injection.</p>
<p>Так, определим в компоненте кнопку и обработчик кнопки, который будет перенаправлять на определенный ресурс:</p>
<pre class="brush:ts;">
...
<p>Через конструктор получаем объект Router и в обработчике кнопки вызываем его метод <code>navigate()</code>. В этот метод передается путь перехода. 
...
можно было бы передать соответствующий путь: <code>this.router.navigate(['/about']);</code></p>
<h3>Параметры маршрута и строки запроса</h3>
<p>Изменим компонент AppComponent, добавив форму для ввода параметров:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
import { Router} from '@angular/router';
...
<p>Для передачи компоненту ItemComponent нужных параметров в метод <code>navigate()</code> первого параметра передается набор значений, первое из которых - собственно путь, 
...
строки запроса.</p>
<p>Код компонента <code>ItemComponent</code> оставим тем же, что и в прошлой теме:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>В итоге после ввода данных и нажатия на кнопку компонент ItemComponent будет получать введенные данные:</p>
<img src="./pics/7.8.png" alt="Query Params in Angular" />
