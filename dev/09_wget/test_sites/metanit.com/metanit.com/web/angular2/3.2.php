...
<html >
<head>
<title>Angular | Создание атрибутивных директив</title>
<meta charset="utf-8" />
<meta name="description" content="Создание атрибутивных директив и их использование в Angular">
...
<h2>Создание атрибутивных директив</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Атрибутивные директивы меняют поведение элемента, к которому они применяются. Например, директива <code>ngClass</code> позволяет установить для элемента 
класс CSS. При этом сама директива применяется к элементу в виде атрибута:</p>
<pre class="brush:xml;">&lt;div [ngClass]="{verdanaFont:true}"&gt;</pre>
<p>И при необходимости мы можем сами создавать какие-то свои директивы атрибутов для каких-то определенных целей. Итак, создадим свою директиву. 
Добавим в папку <span class="ii">src/app</span> новый файл, который назовем <span class="b">bold.directive.ts</span>:</p>
<img src="./pics/3.7.png" alt="Создание директив в Angular и Typescript" />
<p>Определим в файле <span class="b">bold.directive.ts</span> следующий код:</p>
<pre class="brush:ts;">
...
<p>Директива - это обычный класс на TS, к которому применяется декоратор <span class="b">Directive</span>, соответственно нам надо импортировать эту директиву из 
"angular/core". Кроме того, здесь импортируется класс "ElementRef". Он представляет ссылку на элемент, к которому будет применяться директива.</p>
<p>При применении декоратора <code>@Directive</code> необходимо определить селектор CSS, с которым будет ассоциирована директива. 
Селектор CSS для атрибута должен определяться в квадратных скобках. В данном случае в качестве селектора выступает <code>[bold]</code>.</p>
<p>Сам декоратор @Directive применяется к классу, который называется <code>BoldDirective</code>. Это собственно и есть класс директивы, который определяет ее логику.</p>
<p>Для получения элемента, к которому применяется данная директива, в классе определен конструктор, имеющий один параметр: <code>private elementRef: ElementRef</code>. 
Через этот параметр Angular будет передавать или инжектировать тот элемент из шаблона, в котором применяется директива.</p>
<p>Поскольку параметр определен с ключевым словом <code>private</code>, то для него будет создаваться одноименная приватная переменная, через которую мы можем получить объект ElementRef и 
произвести с ним какие-либо манипуляции. В частности, здесь идет обращение к вложенному свойству <code>nativeElement</code>, через которое у элемента 
устанавливается жирный шрифт:</p>
<pre class="brush:ts;">this.elementRef.nativeElement.style.fontWeight = "bold";</pre>
<p>Теперь возьмем код главного компонента и применим директиву:</p>
<pre class="brush:ts;">
...
<p>Здесь определено два параграфа, и к первому из них применяется директива. Поскольку в коде директивы был определен селектор "[bold]", то чтобы ее применить, в коде элемента применяется данный селектор.</p>
<p>Но сама по себе директива не заработает. Нам еще надо ее подключить в модуле приложения - классе AppModule:</p>
<pre class="brush:ts;">
...
<p>Как и компоненты, директивы также надо сначала импортировать из файла, где они объявлены:</p>
<pre class="brush:ts;">import { BoldDirective} from './bold.directive';</pre>
<p>Затем она добавляется в секцию <code>declarations</code>:</p>
<pre class="brush:ts;">declarations: [ AppComponent, BoldDirective],</pre>
<p>И если мы запустим приложение, то увидим применение директивы к первому параграфу:</p>
<img src="./pics/3.2.png" alt="Создание атрибутивных директив в Angular 13" />
<p>Для управления стилизацией элемента выше этот элемента извлекался через объект ElementRef в конструкторе директивы, и у него устанавливались стилевые свойства. 
Однако гораздо удобнее для управления стилем использовать рендерер. Так, изменим директиву следующим образом:</p>
<pre class="brush:ts;">
...
<p>Renderer2 представляет сервис, который также при вызове директивы автоматически передается в ее конструктор, и мы можем использовать данный сервис для 
стилизации элемента. А результат работы будет тот же, что и выше.</p>
