...
<html >
<head>
<title>Angular | Опциональные сервисы</title>
<meta charset="utf-8" />
<meta name="description" content="Опциональные, необязательные сервисы в Angular, декоратор Optional">
...
<h2>Опциональные сервисы</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Сервис может быть опциональным, необязательным. Например, в прошлой теме был создан сервис <span class="b">LogService</span>:</p>
<pre class="brush:ts;">
...
<p>И был другой сервис DataService, который использовал LogService:</p>
<pre class="brush:ts;">
...
<p>Допустим, по какой-то причине сервис LogService не доступен для инжектирования, например, мы не добавили в провайдеры компонента AppComponent:</p>
<pre class="brush:ts;">
...
<p>Если мы запустим приложение, то в этом случае мы получим ошибку. Так как для сервиса LogService не определен провайдер. В этом случае мы можем определить 
сервис LogService как опциональный, применяя декоратор <span class="b">Optional</span>. Для этого изменим код DataService:</p>
<pre class="brush:ts;">
...
<p>Итак, в конструкторе класса мы получаем сервис как опциональный:</p>
<pre class="brush:ts;">constructor(@Optional() private logService: LogService){}</pre>
<p>Далее при обращении к сервису мы можем проверить, установлен ли он, и если он установлен, использовать его:</p>
<pre class="brush:ts;">
if (this.logService) this.logService.write("операция получения данных");
</pre>
...
<div class="nav"><p><a href="./4.2.php">Назад</a><a href="./">Содержание</a><a href="./4.3.php">Вперед</a></p></div></div>
<div class="socBlock">
