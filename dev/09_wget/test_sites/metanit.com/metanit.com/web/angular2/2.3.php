...
<html >
<head>
<title>Angular | Загрузка приложения</title>
<meta charset="utf-8" />
<meta name="description" content="Загрузка приложения Angular, роль файла main.ts, загрузка модуля и главного компонента">
...
<h2>Загрузка приложения</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>При запуске приложения первым выполняется код, который определен в файле <span class="b">main.ts</span>. Этот файл имеет следующее содержимое:</p>
<pre class="brush:ts;">
...
<p>Первая строка импортирует функциональность модуля <code>platformBrowserDynamic</code> из пакета <code>angular/platform-browser-dynamic</code>. 
platformBrowserDynamic использует bootstrapModule для загрузки нужного модуля.</p>
<p>То есть фактически здесь platformBrowserDynamic запускает импортированный во второй строке модуль AppModule. После этого начинает работать вся логика, которая заложена в модуле 
AppModule, который представляет главный модуль приложения. Больше ничего файл main.ts не делает.</p>
<img src="./pics/1.6.png" alt="main.ts и AppModule и AppComponent в Angular" />
<p>В самом же AppModule нам надо указать, какой именно компонент будет использоваться в качестве основного при загрузке. Для этого используется параметр 
<span class="b">bootstrap</span> декоратора NgModule:</p>
<pre class="brush:ts;">
...
<p>То есть в данном случае значение <code>bootstrap: [ AppComponent ]</code> указывает, что модуль для загрузки в качестве основного компонента будет использовать 
класс AppComponent.</p>
<p>Если мы опустим компонент: <code>bootstrap: [ ]</code> или вовсе уберем параметр bootstrap, то на веб-страницу не будет загружаться представление из комопонента 
AppComponent.</p>
