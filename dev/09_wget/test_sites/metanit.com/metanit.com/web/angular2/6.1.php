...
<html >
<head>
<title>Angular | HttpClient и отправка запросов</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка get-запросов в Angular, модуль HttpClientModule, сервис HttpClient, работа с протоколом HTTP">
...
<h1>HTTP и взаимодействие с сервером</h1><h2>HttpClient и отправка запросов</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Для взаимодействия с сервером и отправки запросов по протоколу http применяется класс <span class="b">HttpClient</span>. Этот класс определяет ряд методов для отправки различного рода запросов: GET, POST, PUT, DELETE. 
Данный класс построен поверх стандартного объекта в JavaScript - XMLHttpRequest.</p> 
<p>Для использования этого класса в проект необходимо установить пакет "@angular/common":</p>
<pre class="brush:js;">
{
    "name": "helloapp",
    "version": "1.0.0",
...
<p>И также в файле модуля AppModule должен быть импортирован класс <span class="b">HttpClientModule</span> из пакета "@angular/common/http":</p>
<pre class="brush:ts;">
...
<p>Вначале рассмотрим выполнение простейших GET-запросов. Пусть у нас есть стандартная структура проекта:</p>
<img src="./pics/6.7.png" alt="HttpClientModule в Angular" />
<p>Определим обращение по сети к файлу json, который будет хранить некоторые данные. Папка собственно файлов приложения - это папка src. 
Поэтому в ней определим новую папку <span class="b">assets</span>. И далее добавим в папку <span class="b">src/assets</span> 
файл <span class="b">user.json</span>, который будет представлять данные:</p>
<pre class="brush:js;">
{
	"name": "Bob",
	"age": 28
...
<p>Для представления данных в папку <span class="ii">src/app</span> добавим новый файл <span class="b">user.ts</span> и определим в нем следующий код:</p>
<pre class="brush:ts;">
export class User{
    constructor(public name:string, public age:number){}
}
</pre>
<p>Для оправки запроса определим в компоненте AppComponent следуюший код:</p>
<pre class="brush:ts;">
...
<p>В данном случае в шаблоне выводятся данные объекта User, которые мы хотим получить с сервера. Однако загрузка данных, скажем, в конструкторе компонента 
не очень желательна. В этом плане метод <code>ngOnInit()</code>, который определен в интерфейсе OnInit и который вызывается при инициализации компонента представляет 
более предпочтительное место для загрузки данных. В конструкторе же мы просто получаем сервис HttpClient.</p>
<p>Далее в методе <code>ngOnInit()</code> получаем данные из сервиса. Сам метод <code>http.get()</code> возвращает объект 
<span class="b">Observable&lt;Object&gt;</span>. Observable представляет своего рода поток, и для прослушивания событий из этого потока применяется метод  <code>subscribe</code>. 
Этот метод определяет действие над результатом запроса - полученными с сервера данными. В данном случае действие определено в виде стрелочной функции. 
Причем поскольку между схемой класса User и данными из файла json есть прямое сопоставление, то получаемые данные мы можем передать в конструктор класса User:</p>
<pre class="brush:ts;">(data:any) =&gt; this.user=new User(data.name, data.age)</pre>
<p>Файл проекта после изменений:</p>
<img src="./pics/6.10.png" alt="Json as an asset in Angular 13" />
<p>Поскольку файл json представляет вспомогательный файл, то нам надо указать это angular cli в файле <span class="b">angular.json</span> с 
помощью параметра <code>"assets": ["src/assets"]</code>:</p>
<pre class="brush:js;">
{
  "version": 1,
  "projects": {
...
<p>В итоге при запуске веб-страницы мы увидим загруженные данные из файла user.json.</p>
<img src="./pics/6.1.png" alt="HttpService в Angular 13" />
<h3>Создание сервиса</h3>
<p>При взаимодействии с сервером, как правило, обращения к серверу происходят не непосредственно из компонента, а из вспомогательных сервисов. Поскольку сервис может определять 
дополнительную логику обработки полученных с сервера данных, которую могли бы сделать код компонента перегруженным. Кроме того, сервисы могут определять 
функционал, который будет использоваться несколькими компонентами. Компоненты же выступают в качестве потребителей данных, которые получены от 
сервисов.</p>
<p>Поэтому для работы с http добавим в папку <span class="b">src/app</span> новый файл <span class="b">http.service.ts</span> со следующим содержимым:</p>
<pre class="brush:ts;">
...
<p>Для отправки запросов сервис получает объект HttpClient. Поскольку сервис принимает в конструкторе параметр через механизм dependency injection, 
то к классу применяется декоратор <code>@Injectable</code>.</p>
<p>Для выполнения get-запроса у объекта HttpClient вызывается метод <code>get()</code>, в который передается адрес запроса - в нашем случае json-файл с данными.</p>

<p>Теперь используем этот сервис в компоненте AppComponent:</p>
<pre class="brush:ts;">
import { Component, OnInit} from '@angular/core';
...
<h3>Загрузка сложных данных</h3>
<p>В примерах выше определение json-файла соответствует определению класса User, поэтому простое присвоение <code>this.user=new User(data.name, data.age)</code> пройдет 
успешно. И подобным образом мы можем загружать и другие более сложные данные. Например, определим в папке <span class="b">src/assets</span> файл <span class="b">users.json</span>:</p>
<pre class="brush:js;">
{ 
    "userList":
    [{
...
<img src="./pics/6.8.png" alt="HttpClient в Angular 13" />
<p>Изменим в классе HttpService адрес загрузки данных:</p>
<pre class="brush:ts;">
getData(){
		return this.http.get('assets/users.json')
}
</pre>
<p>И изменим код компонента:</p>
<pre class="brush:ts;">
import { Component, OnInit} from '@angular/core';
import { HttpService} from './http.service';
import {User} from './user';
...
<p>В данном случае мы хотим получить массив объектов User. Но напрямую данные из users.json не соответствуют массиву. Массив в файле определен по ключу 
"userList". Поэтому, используя данный ключ, мы достаем нужные данные из ответа сервера: <code>this.users=data["userList"]</code>.</p>
<img src="./pics/6.2.png" alt="Загрузка данных в Angular 13" />
...
<div class="nav"><p><a href="./5.5.php">Назад</a><a href="./">Содержание</a><a href="./6.2.php">Вперед</a></p></div></div>
<div class="socBlock">
