...
<html >
<head>
<title>Angular | Получение параметров в директивах</title>
<meta charset="utf-8" />
<meta name="description" content="Получение входных параметров с помощью декоратора Input в директивах в Angular 13">
...
<h2>Получение параметров в директивах</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Директива как и компонент может получать некоторые входные параметры извне. Для этого также используется декоратор <span class="b">Input</span>. Итак, 
возьмем директиву BoldDirective из прошлой темы и, допустим, мы хотим, чтобы у текста при наведении также менялась высота шрифта. Но при этом чтобы 
нужную высоту шрифта можно было бы задать извне директивы. Для этого изменим ее следующим образом:</p>
<pre class="brush:ts;">
...
<p>В данном случае определяются два входных параметра:</p>
<pre class="brush:ts;">
@Input() selectedSize = "18px";
@Input() defaultSize = "16px";
</pre>
<p>Параметр selectedSize отвечает за высоту шрифта при наведении мыши, а в параметр defaultSize устанавливает высоту шрифта, когда указатель мыши 
находится вне границ элемента.</p>
<p>Теперь задействуем эти параметры, изменив код компонента:</p>
<pre class="brush:ts;">
...
<p>При применении директивы мы можем указать все входные параметры и их значения:</p>
<pre class="brush:xml;">&lt;p bold selectedSize="28px" [defaultSize]="'14px'"&gt;Hello Angular&lt;/p&gt;</pre>
<p>При этом есть две вариации применения праметров. Либо названия параметров заключаются в квадратные скобки, а их значения дополнительно заключаются в одинарные кавычки:</p>
<pre class="brush:xml;">[defaultSize]="'14px'"</pre>
<p>Либо названия параметров передаются без скобок, а их значения заключаются в двойные кавычки:</p>
<pre class="brush:xml;">selectedSize="28px"</pre>
<p>И при наведении на элемент автоматически будет меняться также и высота шрифта:</p>
<img src="./pics/3.3.png" alt="Input Parameters in Directive in Angular 13" />
<p>Теперь пойдем дальше и изменим первый входной параметр:</p>
<pre class="brush:ts;">@Input("bold") selectedSize = "18px";</pre>
<p>Здесь в декоратор Input передается селектор директивы  - bold. Поэтому чтобы установить этот параметр в шаблоне компонента мы можем напрямую использовать имя директивы:</p>
<pre class="brush:xml;">&lt;p [bold]="'28px'" [defaultSize]="'14px'"&gt;Hello Angular&lt;/p&gt;</pre>
