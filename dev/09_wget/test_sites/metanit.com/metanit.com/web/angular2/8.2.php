...
<html >
<head>
<title>Angular | Создание своих pipes</title>
<meta charset="utf-8" />
<meta name="description" content="Создание своих pipes в Angular, декоратор Pipe и интерфейс PipeTransform">
...
<h2>Создание своих pipes</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Если нам потребуется некоторая предобработка при выводе данных, дополнительное форматирование, то мы можем для этой цели написать свои собственные pipes.</p>
<p>Классы pipes должны реализовать интерфейс <span class="b">PipeTransform</span></p>
<pre class="brush:ts;">
interface PipeTransform {
  transform(value: any, ...args: any[]): any
}
</pre>
<p>Метод transform должен преобразовать входное значение. Этот метод в качестве параметра принимает 
значение, к которому применяется pipe, а также опциональный набор параметров. А на выходе возвращается отформатированное значение. Поскольку первый параметр представляет тип any, а второй параметр - массив типа any, 
то соответственно мы можем передавать данные любых типов. И также можем возвращать объект любого типа.</p>
<p>Рассмотрим простейший пример. Допустим, нам надо выводить число, в котором разделителем между целой и дробной частью является запятая, а не точка. 
Для этого мы можем написать небольшой pipe. Для этого добавим в проект в папку <span class="b">src/app</span> новый файл <span class="b">format.pipe.ts</span>:</p>
<img src="./pics/8.13.png" alt="Custom pipe in Angular" />
...
<p>К кастомному pipe должен применяться декоратор Pipe. Этот декоратор определяет метаданные, в частности, название pipe, по которому он будет использоваться:</p>
<pre class="brush:ts;">
@Pipe({
	name: 'format'
})
</pre>
<p>Применим FormatPipe в коде компонента:</p>
<pre class="brush:ts;">
...
<p>Но чтобы задействовать FormatPipe, его надо добавить в главном модуле приложения AppModule:</p>
<pre class="brush:ts;">
...
<img src="./pics/8.6.png" alt="Custom pipes in Angular 13" />
<h3>Передача параметров</h3>
<p>Добавим еще один pipe, который будет принимать параметры. Пусть это будет класс, который из массива строк будет создавать строку, принимая начальный и конечный индексы для выборки данных из массива. 
Для этого добавим в проект новый файл <span class="b">join.pipe.ts</span>, в котором определим следующее содержимое:</p>
<pre class="brush:ts;">
import { Pipe, PipeTransform } from '@angular/core';
...
<p>В метод transform класса JoinPipe первым параметром передается массив, второй необязательный параметр start представляет начальный индекс, с которого производится выборка, 
а третий параметр end - конечный индекс.</p>
<p>С помощью метода <code>slice()</code> получаем нужную часть массива, а с помощью метода <code>join()</code> соединяем массив в строку.</p>
<p>Применим JoinPipe:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>Опять же подключим JoinPipe в модуле приложения:</p>
<pre class="brush:ts;">
import { NgModule }      from '@angular/core';
import { BrowserModule } from '@angular/platform-browser';
import { AppComponent }   from './app.component';
...
<img src="./pics/8.7.png" alt="Передача параметров в pipe в Angular" />
