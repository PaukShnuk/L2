...
<html >
<head>
<title>Angular | Директивы ngClass и ngStyle</title>
<meta charset="utf-8" />
<meta name="description" content="Директивы ngClass и ngStyle в Angular, стилизация и привязка к классам CSS">
...
<h1>Директивы</h1><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Директивы определяют набор инструкций, которые применяются при рендеринге html-кода. Директива представляет класс с директивными метаданными. В TypeScript для прикрепления метаданных к классу применяется декоратор 
<span class="b">@Directive</span>.</p>
<p>В Angular есть три типа директив:</p>
<ul>
<li><p><span class="b">Компоненты</span>: компонент по сути также является директивой, а декоратор <code>@Component</code> расширяет возможности декоратора 
<code>@Directive</code> с помощью добавления функционала по работе с шаблонами.</p></li>
<li><p><span class="b">Атрибутивные</span>: они изменяют поведение уже существующего элемента, к которому они применяются. Например, <code>ngModel</code>, <code>ngStyle</code>, <code>ngClass</code></p></li>
<li><p><span class="b">Структурные</span>: они изменяют структуру DOM с помощью добавления, изменения или удаления элементов html. Например, это директивы <code>ngFor</code> и 
<code>ngIf</code></p></li>
</ul>
<h2>ngClass и ngStyle</h2>
<h3>ngClass</h3>
<p>Директива <span class="b">ngClass</span> позволяет определить набор классов, которые будут применяться к элементу. В качестве значения она принимает 
набор классов в следующем виде:</p>
<pre class="brush:ts;">
[ngClass]={
	"класс1": true/false,
	"класс2": true/false,
...
<p>Например, определим следующий компонент:</p>
<pre class="brush:ts;">
...
<p>В секции styles у компонента определены два класса, которые устанавливают различные стилевые свойства шрифта: verdanaFont и segoePrintFont.</p>
<p>В шаблоне для привязки класса к элементу применяется директива <code>[ngClass]="{verdanaFont:true}"</code>. Эта директива принимает js-объект, в котором 
...
случае класс verdanaFont будет применяться ко всему блоку div.</p>
<p>Однако в блоке div есть параграф, и мы, допустим, хотим, чтобы к этому параграфу применялся другой класс. А по умолчанию вложенный параграф унаследует стили от 
родительского блока div и также применяет класс segoePrintFont, в котором можно переопределить унаследованные стили.</p>
<img src="./pics/3.1.png" alt="Директива ngClass в Angular 13" />
<p>Вместо жестко закодированных значений true/false мы можем использовать привязку к выражениям:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>В качестве альтернативы мы можем использовать следующие выражения привязки:</p>
<pre class="brush:xml;">
...
<p>Выражение <code>[class.verdanaFont]="true"</code> указывает, что класс verdanaFont будет применяться для данного элемента.</p>
<p>Однако с помощью ngClass мы можем задать целый набор классов, которые применяются к элементу:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>В данном случае для элемента устанавливаются два класса verdanaFont и navyColor.</p>
<img src="./pics/3.9.png" alt="Определение классов для ngClass в Angular 13" />
<h3>ngStyle</h3>
<p>Директива <span class="b">ngStyle</span> позволяет задать набор стилей, которые применяются к элементу. В качестве значения директива принимает js-объект, 
в котором ключи - названия свойств CSS:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>Аналогично для установки стилей можно применять свойства объекта style:</p>
<pre class="brush:xml;">
...
<p>Также мы можем установить привязку </p>
<h3>Динамическое изменение стилей</h3>
<p>Директивы ngClass и ngStyle позволяют устанавливать привязку к выражениям, благодаря чему мы можем динамически менять стили или классы. Например:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>Выражение <code>[ngClass]="{invisible: visibility}"</code> устанавливает для класса invisible привязку к значению переменной <code>visibility</code>. По нажатию на кнопку мы можем переключать это свойство, 
тем самым управляя видимостью блока.</p>
<p>В качестве альтернативы также можно было бы использовать следующее выражение:</p>
<pre class="brush:xml;">&lt;div [class.invisible]="visibility"&gt;</pre>
<p>Либо также можно было бы написать так:</p>
<pre class="brush:xml;">&lt;div [style.display]="visibility?'block':'none'"&gt;</pre>
...
<div class="nav"><p><a href="./2.11.php">Назад</a><a href="./">Содержание</a><a href="./3.2.php">Вперед</a></p></div></div>
<div class="socBlock">
