...
<html >
<head>
<title>Angular | Взаимодействие с пользователем, HostListener и HostBinding</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие директивы с пользователем в Angular 13, обработка событий в директиве, применение декораторов HostListener и HostBinding">
...
<h2>Взаимодействие с пользователем, HostListener и HostBinding</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Кроме простой установки значений атрибутивная директива может взаимодействовать с пользователем. Для этого применяется декоратор HostListener.</p>
<p>Так, возьмем директиву BoldDirective из прошлой темы и добавим в нее взаимодействие с пользователем:</p>
<pre class="brush:ts;">
...
<p>Декоратор <span class="b">@HostListener</span> позволяет связать события DOM и методы директивы. В частности, в декоратор передается название события, 
по которому будет вызываться метод. В данном случае мы привязываем события mouseenter (наведения указателя мыши на элемент) и mouseleave (уведение указателя мыши с 
элемента) к методу <code>setFontWeight()</code>, который устанавливает стилевое свойство font-weight у элемента. Если мы наводим на элемент, то устанавливается 
выделение жирным. При отводе мыши выделение сбрасывается.</p>
<p>Кроме кода директивы менять больше ничего не надо, код компонента и модуля остаются теми же.</p>
<h3>HostBinding</h3>
<p>Еще один декоратор - <span class="b">HostBinding</span> позволяет связать обычное свойство класса со свойством элемента, к которому применяется директива. 
Например, изменим код директивы следующим образом:</p>
<pre class="brush:ts;">
...
<p>Инструкция <code>@HostBinding("style.fontWeight") get getFontWeight()</code> связывает со свойством "style.fontWeight" значение, которое возвращается 
этим геттером getFontWeight. А он возвращает значение свойства <code>fontWeight</code>, которое также меняется при наведении указателя мыши.</p>
<h3>Свойство host</h3>
<p>Вместо применения декораторов HostListener и HostBinding для реагирования директивы на действия пользователя мы можем определить обработчики событий в декораторе 
Directive с помощью его свойства <span class="b">host</span>. Так, перепишем директиву следующим образом:</p>
<pre class="brush:ts;">
...
<p>Результат работы директивы в данном случае будет аналогичен, только теперь все события и связанные с ними обработчики определяются с помощью параметра host:</p>
<pre class="brush:ts;">
host: {
	'(mouseenter)': 'onMouseEnter()',
	'(mouseleave)': 'onMouseLeave()'
