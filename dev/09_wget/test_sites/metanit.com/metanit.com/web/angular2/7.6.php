...
<html >
<head>
<title>Angular | Дочерние маршруты</title>
<meta charset="utf-8" />
<meta name="description" content="Дочерние маршруты (child routes) в Angular">
...
<h2>Дочерние маршруты</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>В прошлых темах рассматривалась работа с обычными маршрутами. То есть система маршрутизации выбирает один из компонентов на основе url и 
...
запроса. В этому случае нам надо использовать дочерние маршруты (child route).</p>
<p>К примеру, возьмем проект из прошлых тем:</p>
<img src="./pics/7.18.png" alt="Child routes in Angular" />
<p>Добавим в папку <span class="ii">src/app</span> два простеньких компонента. Вначале добавим файл <span class="b">item.details.component.ts</span>:</p>
<pre class="brush:ts;">
...
<p>И также добавим второй файл <span class="b">item.stat.component.ts</span>:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>Итак, есть два компонента ItemDetailsComponent и ItemStatComponent, которые условно раскрывают различные стороны одно и того же товара. Логически они относятся к 
ItemComponent, который также связан с определенным товаром.</p>
<p>Мы бы могли определить прямые маршруты к этим компонентам типа:</p>
<pre class="brush:ts;">
{ path: 'item/:id/details', component: ItemDetailsComponent},
{ path: 'item/:id/stat', component: ItemStatComponent},
</pre>
<p>Где в данном случае id - также параметр маршрута, представляющий условный id товара.</p>
<p>Но такие маршруты будут миновать компонент ItemComponent и никак его не затрагивают. Поэтому нам надо использовать другую организацию маршрутов. Для 
этого изменим модуль AppModule следующим образом:</p>
<pre class="brush:ts;">
...
<p>Каждый из дочерних маршрутов сопоставляется не совсем адресом url, а только с его частью. Далее чтобы применить такие маршруты, у маршрута для компонента 
ItemComponent применяется свойство <span class="b">children</span>:</p>
<pre class="brush:ts;">{ path: 'item/:id', component: ItemComponent, children: itemRoutes},</pre>
<p>Теперь изменим ItemComponent:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>Для вставки компонентов ItemDetailsComponent и ItemStatComponent здесь определен элемент <code>&lt;router-outlet&gt;&lt;/router-outlet&gt;</code>.</p>
<p>И изменим главный компонент AppComponent, добавив в него для тестирования ссылки на ItemDetailsComponent и ItemStatComponent:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>При переходе по подобным ссылкам будет срабатывать маршрутизация к компоненту ItemComponent и ему будет передаваться параметр маршрута - id. И также 
будет срабатывать маршрутизация к ItemDetailsComponent или ItemStatComponent, если после id идет какой-нибудь сегмент "details" или "stat":</p>
<img src="./pics/7.12.png" alt="Child routes in Angular" />
<img src="./pics/7.13.png" alt="Дочерние маршруты в Angular" />
