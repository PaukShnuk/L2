...
<html >
<head>
<title>Angular | Жизненный цикл компонента</title>
<meta charset="utf-8" />
<meta name="description" content="Жизненный цикл компонента в Angular, методы ngOnInit и ngOnChanges, управление жизненным циклов через lyfecycle hooks">
...
<h2>Жизненный цикл компонента</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>После создания компонента фреймворк Angular вызывает у этого компонента ряд методов, которые представляют различные этапы жизненного цикла:</p>
<img src="./pics/2.9.png" alt="Жизненный цикл компонентов в Angular 5" />
<ul>
<li><p><span class="b">ngOnChanges</span>: вызывается до метода <code>ngOnInit()</code> при начальной установке свойств, которые связаны механизмом привязки, а также при любой их переустановке или 
изменении их значений. Данный метод в качестве параметра принимает объект класса <code>SimpleChanges</code>, который содержит предыдущие и текущие значения 
свойства.</p></li>
<li><p><span class="b">ngOnInit</span>: вызывается один раз после установки свойств компонента, которые участвуют в привязке. 
Выполняет инициализацию компонента</p>
</li>
<li><p><span class="b">ngDoCheck</span>: вызывается при каждой проверке изменений свойств компонента сразу после методов <code>ngOnChanges</code> и <code>ngOnInit</code></p></li>
<li><p><span class="b">ngAfterContentInit</span>: вызывается один раз после метода <code>ngDoCheck()</code> после вставки содержимого в представление компонента кода html</p></li>
<li><p><span class="b">ngAfterContentChecked</span>: вызывается фреймворком Angular при проверке изменений содержимого, 
которое добавляется в представление компонента. Вызывается после метода <code>ngAfterContentInit()</code> и 
после каждого последующего вызова метода <code>ngDoCheck()</code>.</p></li>
<li><p><span class="b">ngAfterViewInit</span>: вызывается фреймворком Angular после инициализации представления компонента, а также представлений дочерних 
компонентов. Вызывается только один раз сразу после первого вызова метода <code>ngAfterContentChecked()</code></p></li>
<li><p><span class="b">ngAfterViewChecked</span>: вызывается фреймворком Angular после проверки на изменения в представлении компонента, а также проверки 
представлений дочерних компонентов. Вызывается после первого вызова метода <code>ngAfterViewInit()</code> и после каждого последующего вызова <code>ngAfterContentChecked()</code></p></li>
<li><p><span class="b">ngOnDestroy</span>: вызывается перед тем, как фреймворк Angular удалит компонент.</p></li>
</ul>
<p>Каждый такой метод определен в отдельном интерфейсе, который называется по имени метода без префикса "ng". Например, метод <code>ngOnInit</code> определен 
в интерфейсе <code>OnInit</code>. Поэтому, если мы хотим отслеживать какие-то этапы жизненного цикла компонента, то класс компонента должен применять соответствующие интерфейсы:</p>
<pre class="brush:ts;">
...
<h3>ngOnInit</h3>
<p>Метод <code>ngOnInit()</code> применяется для какой-то комплексной инициализации компонента. Здесь можно выполнять загрузку данных с сервера или из других источников данных.</p>
<p><code>ngOnInit()</code> не аналогичен конструктору. Конструктор также может выполнять некоторую инициализацию объекта, в то же время что-то сложное в конструкторе 
...
которая может занять продолжительное время, лучше делать в методе <code>ngOnInit</code>.</p>
<h3>ngOnDestroy</h3>
<p>Метод <code>ngOnDestroy()</code> вызывается перед удалением компонента. И в этом методе можно освобождать те используемые ресурсы, которые не удаляются автоматически сборщиком мусора. 
Здесь также можно удалять подписку на какие-то события элементов DOM, останавливать таймеры и т.д.</p>
<h3>ngOnChanges</h3>
<p>Метод <code>ngOnChanges()</code> вызывается перед методом <code>ngOnInit()</code> и при изменении свойств в привязке. С помощью параметра <code>SimpleChanges</code> в методе можно получить 
текущее и предыдущее значение измененного свойства. Например, пусть у нас будет следующий дочерний компонент:</p>
<pre class="brush:ts;">
...
<p>И пусть этот компонент используется в главном компоненте:</p>
<pre class="brush:ts;">
...
<p>То есть значение для свойства name передается в дочерний компонент ChildComponent из главного - AppComponent. Причем в главном компоненте тоже реализован 
метод <code>ngOnChanges()</code>.</p>
<p>И если мы запустим приложение, то сможем заметить, что при каждом изменении свойства name в главном компоненте вызывается метод ngOnChanges:</p>
<img src="./pics/2.11.png" alt="OnChanges в Angular 13" />
<p>В то же время надо отметить, что данный метод вызывается только при изменении входных свойств с декоратором <code>@Input</code>. Поэтому изменение свойства 
age в AppComponent здесь не будет отслеживаться.</p>
<h3>Реализация всех методов</h3>
<p>Определим следующий дочерний компонент:</p>
<pre class="brush:ts;">
import { Component, 
		 Input, 
		 OnInit,
...
<p>И используем этот компонент в главном компоненте:</p>
<pre class="brush:ts;">
...
<p>И при обращении к приложению мы получим следующую цепочку вызовов:</p>
<img src="./pics/2.12.png" alt="Lifecycle hooks in Angular 13" />
...
<div class="nav"><p><a href="./2.10.php">Назад</a><a href="./">Содержание</a><a href="./2.9.php">Вперед</a></p></div></div>
<div class="socBlock">
