...
<html >
<head>
<title>Angular | Создание структурных директив</title>
<meta charset="utf-8" />
<meta name="description" content="Создание структурных директив в Angular 13, TemplateRef и ViewContainerRef">
...
<h2>Создание структурных директив</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Создадим простейшую структурную директиву. Добавим в папку <span class="ii">src/app</span> новый файл <span class="b">while.directive.ts</span>:</p>
<img src="./pics/3.8.png" alt="Структурные директивы в Angular 13" />
<p>Этот файл будет содержать директиву. Определим в файле следующий код:</p>
<pre class="brush:ts;">
...
<p>Структурная директива должна применять декоратор Directive, в который передается название селектора директивы в квадратных скобках. В данном случае селектор - 
"while".</p>
<p>Для получения доступа к шаблону директивы применяется объект <span class="b">TemplateRef</span>. Этот объект автоматически передается в конструктор через механизм 
внедрения зависимостей. Кроме этого объекта в конструктор также передается объект рендерера - <span class="b">ViewContainerRef</span>. Ну и с помощью применения 
модификатора <code>private</code> для обоих этих параметров автоматически будут создаваться локальные переменные, к которым мы затем сможем обратиться.</p>
<pre class="brush:ts;">
constructor(private templateRef: TemplateRef&lt;any&gt;, 
				private viewContainer: ViewContainerRef) 
{ }
</pre>
<p>С помощью входного свойства-сеттера, к которому применяется декоратор <code>Input</code>, мы будем получать из вне некоторые значения, которые могут использоваться 
при создании разметки html. В данном случае мы получаем извне некоторое булевое значение:</p>
<pre class="brush:ts;">
@Input() set while(condition: boolean) {
    if (condition) {
        this.viewContainer.createEmbeddedView(this.templateRef);
...
<p>Если в данном случае condition равно true, то производим рендеринг шаблона через вызов <code>this.viewContainer.createEmbeddedView(this.templateRef);</code>. 
В итоге на веб-странице появится элемент, к которому применяется данная директива.</p>
<p>В обратном случае, если condition равно false, то, наоборот, удаляем элемент из разметки с помощью вызова <code>this.viewContainer.clear()</code>.</p>
<p>То есть по факту мы получили аналог директивы ngIf.</p>
<p>Далее применим директиву в главном компоненте AppComponent:</p>
<pre class="brush:ts;">
...
<p>И далее задействуем директиву в главном модуле AppModule:</p>
<pre class="brush:ts;">
...
<p>И после запуска приложения с помощью этой директивы мы сможем регулировать добавление или удаление определенного блока веб-страницы:</p>
<img src="./pics/3.6.png" alt="Structure directives in Angular 13" />
