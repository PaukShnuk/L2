...
<html >
<head>
<title>Angular | Модуль FormsModule и директива NgModel</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с формами в Angular, модуль FormsModule и директива NgModel">
...
<h1>Работа с формами</h1><h2>Модуль FormsModule и директива NgModel</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Для взаимодействия с пользователем в веб-приложениях, как правило применяются формы. В Angular прежде чем использовать формы в компонентах, нам надо 
импортировать в главном модуле AppModule модуль FormsModule, который позволяет работать с формами:</p>
<pre class="brush:ts;">
...
<p>Кроме того, в файле конфигурации приложения <span class="b">package.json</span> среди списка используемых зависимостей должен быть указан пакет <span class="b">"angular/forms"</span>:</p>
<pre class="brush:js;">
{
    "name": "helloapp",
    "version": "1.0.0",
...
<p>При работе с формами ключевым моментом является использование директивы <span class="b">NgModel</span>. 
Эта директива с помощью переданной модели создает объект <code>FormControl</code> и привязывает эту модель к созданному элементу формы.
Объект FormControl отслеживает значение модели, а также отвечает за валидацию этого значения и взаимодействие с пользователем.</p>
<p>Данная директива принимает переданную ей модель в качестве входного свойства. Причем мы можем использовать как однонаправленную, так и двунаправленную привязку.</p>
<p>Если нам надо просто вывести значение модели в поле ввода, то можно ограничиться и однонаправленной привязкой:</p>
<pre class="brush:xml;">&lt;input name="title" [ngModel]="title" /&gt;</pre>
<p>Это обычная привязка свойства, где в качестве модели используется некоторое свойство title, определенное в классе компонента.</p>
<p>Если нам надо отслеживать изменение введенных данных, то мы можем использовать двунаправленную привязку:</p>
<pre class="brush:xml;">&lt;input name="title" [(ngModel)]="title" /&gt;</pre>
<p>Рассмотрим применение NgModel на примере. Возьмем проект с базовой структурой:</p>
<img src="./pics/5.9.png" alt="NgModel и FormControl в Angular" />
<p>Определим в файле <span class="ii">app.component.ts</span> следующий компонент:</p>
<pre class="brush:ts;">
...
<p>Для представления данных здесь определен класс Phone, в котором есть три свойства. Класс компонента содержит массив объектов Phone. С помощью метода <code>addPhone()</code> 
в этот массив добавляется новый объект.</p>
<p>Для добавления данных в шаблоне определены три поля ввода. В каждом поле определены директивы типа <code>[(ngModel)]="title"</code>. Тем самым фактически определяются 
некоторые значения, которые привязаны к этим полям. В обработчике нажатия кнопки вызывается метод <code>addPhone()</code>, в который передаются эти значения.</p>
<p>В конце шаблона добавленные данные из массива phones выводятся на страницу:</p>
<img src="./pics/5.2.png" alt="Работа с формами и NgModel в Angular" />
<p>Все три поля привязаны к отдельным значениям, которые существуют сами по себе. Но мы можем пойти дальше и определить для формы ввода отдельную модель, которая будет инкапсулировать эти значения:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>Для полей ввода здесь создана отдельная переменная phone, к свойствам которой привязаны поля ввода. Стоит также обратить внимание на то, как добавляется новый 
объект в массив phones - здесь не добавляется напрямую переменная phone, а создается отдельный объект, который инициализируется значениями из переменной phone. 
А в остальном результат будет тем же, что и в предыдущем примере.</p>
