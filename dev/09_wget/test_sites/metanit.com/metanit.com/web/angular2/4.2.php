...
<html >
<head>
<title>Angular | Внедрение сервиса в другой сервис</title>
<meta charset="utf-8" />
<meta name="description" content="Внедрение сервиса в другой сервис в Angular, механизм dependency injection">
...
<h2>Внедрение сервиса в другой сервис</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Вполне вероятна ситуация, когда мы захотим использовать один сервис в другом сервисе. Например, в прошлой теме был создан сервис для работы с данными. 
Что если нам необходимо логгировать все операции с данными. Для логгирования определим новый сервис. Для этого добавим в папку <span class="ii">src/app</span> 
новый файл <span class="b">log.service.ts</span> со следующим содержимым:</p>
<pre class="brush:ts;">
...
<p>Для логгирования в сервисе определен метод <code>write</code>, который выводит некоторое сообщение на консоль.</p>
<p>Теперь используем этот сервис. Для этого изменим код в файле <span class="b">data.service.ts</span>:</p>
<pre class="brush:ts;">
...
<p>Чтобы указать, что сервис может использоваться в других сервисах, к классу сервиса применяется декоратор <span class="b">Injectable</span>. 
Данный декоратор гарантирует, что встроенный механизм внедрения зависимостей сможет создать объект этого класса и передать его в качестве зависимости в другой объект (в другой сервис или компонент).</p>
<p>Существует общая рекомендации от разработчиков Angular применять Injectable к любому классу сервиса, хотя в прошлой теме мы могли использовать сервис в компоненте без применения к сервису декоратора Injectable. 
Однако в практическом плане данный декоратор необходим, если внедряемый сервис сам имеет некоторые зависимости, как в примере с сервисом DataService выше.</p>
<p>И также в случае с DataService сервис LogService также надо зарегистрировать в списке провайдеров AppComponent:</p>
<pre class="brush:ts;">
...
<p>И несмотря на то, что напрямую LogService не используется в компоненте AppComponent, но он используется в DataService, который вызывается в AppComponent.</p>
<p>И при выполнении операций с данными в консоли браузера мы сможем увидеть работу сервиса LogService.</p>
<img src="./pics/4.2.png" alt="Injectable сервис в Angular" />
...
<div class="nav"><p><a href="./4.1.php">Назад</a><a href="./">Содержание</a><a href="./4.4.php">Вперед</a></p></div></div>
<div class="socBlock">
