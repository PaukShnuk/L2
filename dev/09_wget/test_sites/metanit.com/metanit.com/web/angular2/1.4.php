...
<html >
<head>
<title>Angular | Построение проекта</title>
<meta charset="utf-8" />
<meta name="description" content="Публикация приложения Angular 13, конфигурации angular cli, оптимизация, ng build production">
...
<h2>Построение проекта</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>В прошлой теме был создан первый простейший проект.</p>
<img src="./pics/1.4.1.png" alt="Angular 13 структура проекта" />
<p>Мы можем его запустить, поработать с ним, изменять его код, но как теперь получить те файлы, которые собственно представляют приложение и которые мы можем разместить на каком либо сервере? 
Для этого нам надо выполнить построение проекта.</p>
<p>В прошлой теме мы определили следующий файл <span class="b">angular.json</span>:</p>
<pre class="brush:js;">
{
  "version": 1,
  "projects": {
...
<p>Секция <code>build</code> описывает конфигурацию команды построения "ng build". В частности, параметр <code>"outputPath"</code> 
задает каталог для компилируемых файлов приложения.</p>
<p>То есть мы можем ввести в командную строку команду "ng build" для компиляции проекта</p>
<div class="console">
<pre class="consoletext">
C:\angular\helloapp&gt;ng build
</pre>
</div>
<p>После выполнения этой команды в каталоге проекта появится папка <span class="ii">dist/helloapp</span>, где мы сможем увидеть все файлы приложения. Мы можем расположить эти файлы на любой веб-сервер и так же обращаться к главной странице приложения.</p>
<img src="./pics/1.8.png" alt="Компиляция проекта в Angular 13 с помощью Angular CLI" />
<h3>Настройка построения</h3>
<p>По умолчанию Angular CLI использует ряд настроек при построении. Например, файлы компилируются в режиме production, то есть в режиме, когда они 
уже непосредственно готовы к развертыванию. Но мы также можем все настройки построения задать явным образом. Так, изменим файл <span class="b">angular.json</span> следующим образом:</p>
<pre class="brush:js;">
{
  "version": 1,
  "projects": {
...
<p>Теперь мы добавили для команды build подсекцию "configurations", которая задает дополнительные конфигурации проекта. И здесь указана одна конфигурация - 
"production" - то есть набор настроек, которые применяются к построению приложения, когда оно уже готово к развертыванию и полноценному использованию. И здесь определяется следующий набор настроек:</p>
<ul><li><p><code>optimization</code>: указывает, будет ли использоваться оптимизация</p></li>
<li><p><code>outputHashing</code>: указывает, будет ли в название генерируемого файла добавляться хеш-значение. Значение <code>all</code> 
говорит, что в названия всех генерируемых файлов добавляется хеш</p></li>
<li><p><code>sourceMap</code>: определяет, будут ли генерироваться файлы sourceMap</p></li>
<li><p><code>namedChunks</code>: определяет, будут ли использоваться имена файлов для именнованных подгружаемых чанков</p></li>
<li><p><code>vendorChunk</code>: определяет, будет ли создаваться для сторонних используемых в приложении библиотек отдельный файл</p></li>
<li><p><code>buildOptimizer</code>: подключает пакет <code>@angular-devkit/build-optimizer</code> 
для оптимизации при использовании опции <code>aot</code></p></li>
</ul>
<p>Стоит отметить, что эти конфигурации никак не влияют на процесс разработки и тестирования приложения. Мы также сможем запускать проект при помощи команды "ng serve". Она не будет применять никаких оптимизаций из конфигурации "production", 
иначе это бы увеличило процесс перекомпиляциии приложения при каждом изменении кода.</p>
<p>Теперь используем конфигурацию production для компиляции. Для этого команде build надо передать флаг <code>--configuration production</code>:</p>
<div class="console">
<pre class="consoletext">
C:\angular\helloapp&gt;ng build --configuration production
</pre>
</div>
<p>После этого мы увидим в папке dist/helloapp те же файлы, но которые были сгенерированы с помощью настроек из файла angular.json. Изменяя эти настройки, 
мы можем задать нужные нам в конкретной ситуации параметры построения проекта.</p>
...
<div class="nav"><p><a href="./1.1.php">Назад</a><a href="./">Содержание</a><a href="./1.2.php">Вперед</a></p></div></div>
<div class="socBlock">
