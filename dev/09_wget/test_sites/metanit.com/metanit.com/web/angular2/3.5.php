...
<html >
<head>
<title>Angular | Структурные директивы ngIf, ngFor, ngSwitch</title>
<meta charset="utf-8" />
<meta name="description" content="Структурные директивы ngIf, ngFor, ngSwitch и их использование в Angular">
...
<h2>Структурные директивы ngIf, ngFor, ngSwitch</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Структурные директивы изменяют структуру DOM с помощью добавления или удаления html-элементов. Рассмотрим три структурных директивы: ngIf, ngSwitch 
и ngFor.</p>
<h3>ngIf</h3>
<p>Директива <span class="b">ngIf</span> позволяет удалить или, наоборот, добавить элемент при определенном условии. Например, определим следующий компонент:</p>
<pre class="brush:ts;">
...
<p>В зависимости от значения свойства <code>condition</code> будет отображаться либо первый, либо второй параграф.</p>
<img src="./pics/3.4.png" alt="Структурные директивы в Angular 13" />
<p>Мы можем задавать альтернативные выражения с помощью директивы <span class="b">ng-template</span>. Так, предыдущий пример будет аналогичен следующему:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>Выражение <code>*ngIf="condition;else unset"</code> указывает, что если condition равно false, то срабатывает блок <code>&lt;ng-template #unset&gt;</code>.</p>
<p>Либо можно определить более изощренную логику. Так, изменим шаблон компонента следующим образом:</p>
<pre class="brush:xml;">
...
<p>В данном случае, если condition равно true, то отображается блок thenBlock, иначе отображается блок elseBlock.</p>
<h3>ngFor</h3>
<p>Директива <span class="b">ngFor</span> позволяет перебрать в шаблоне элементы массива. Например:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>В качестве значения директива принимает значение перебора аля-foreach: <code>let item of items</code>. Каждый перебираемый элемент помещается в 
переменную <code>item</code>, которую мы можем вывести на страницу.</p>
<img src="./pics/3.10.png" alt="Директивка ngFor в Angular 13" />
<p>При переборе элементов нам доступен текущий индекс элемента через переменную index, которую мы также можем использовать. Например:</p>
<pre class="brush:xml;">
&lt;div&gt;
	&lt;p *ngFor="let item of items; let i = index"&gt;{{i+1}}.{{item}}&lt;/p&gt;
&lt;/div&gt;
</pre>
<p>Надо учитывать, что индексация идет с нуля, поэтому, чтобы в данном случае отсчет шел с единицы, к переменной i прибавляется единица.</p>
<h3>Символ звездочки и синтаксический сахар</h3>
<p>Можно заметить, что при использовании директив ngFor и ngIf перед ними ставится символ звездочка. По факту это не более чем синтаксический сахар, который упрощает применение 
директивы. Так, определение <code>ngIf</code>:</p>
<pre class="brush:xml;">
&lt;p *ngIf="condition"&gt;
	Привет мир
&lt;/p&gt;
...
<p>по факту будет представлять следующий код:</p>
<pre class="brush:xml;">
...
<p>В итоге параграф и его текст перемещаются внутрь элемента <code>&lt;ng-template&gt;</code>. 
Сама директива помещается в тег <code>&lt;ng-template&gt;</code>, в котором применяется привязка свойства. Булевое значение привязанного свойство указывает, надо ли 
отображать соответствующий контент.</p>
<p>В итоге мы можем выбирать либо первый способ со звездочкой, который более компактный, либо второй способ с элементами ng-template.</p>
<p>То же самое касается и директивы ngFor:</p>
<pre class="brush:xml;">
&lt;ul&gt;
	&lt;li *ngFor="let item of items"&gt;{{item}}&lt;/li&gt;
&lt;/ul&gt
</pre>
<p>Этот код будет эквивалентен следующему:</p>
<pre class="brush:xml;">
&lt;ul&gt;
...
<h3>ngSwitch</h3>
<p>С помощью директивы <span class="b">ngSwitch</span> можно встроить в шаблон конструкцию switch..case и в зависимости от ее результата выполнения выводить тот или иной блок. Например:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>Директива ngSwitch в качестве значения принимает некоторое выражение. В данном случае это свойство count. В элемент <code>ng-template</code> 
помещается инструкция <code>ngSwitchCase</code>, которая сравнивает значение выражения из ngSwitch с другим выражением. Если оба выражения равны, то 
...
то вызывается инструкция <code>ngSwitchDefault</code>.</p>
