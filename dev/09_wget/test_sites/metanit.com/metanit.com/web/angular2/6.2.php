...
<html >
<head>
<title>Angular | Объект Observable и библиотека RxJS</title>
<meta charset="utf-8" />
<meta name="description" content="Объект Observable и библиотека RxJS в Angular, операторы React, преобразование данных с помощью метода map">
...
<h2>Объект Observable и библиотека RxJS</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Методы класса HttpClient после выполнения запроса возвращают объект <span class="b">Observable&lt;any&gt;</span>, 
...
(asynchronous observable). Так, выполнение запроса к серверу с помощью класса HttpClient выполняются в асинхронном режиме.</p>
<p>Естественно чтобы задействовать функционал RxJS в приложении, в проект должна быть добавлена соответствующая зависимость "rxjs":</p>
<pre class="brush:js;">
{
    "name": "helloapp",
    "version": "1.0.0",
...
<p>Используя специальные методы для объекта Observable, например, <span class="b">map</span> и <span class="b">filter</span>, можно произвести некоторую постобработку 
полученных от сервера результатов.</p>
<p>Так, возьмем проект из прошлой темы:</p>
<img src="./pics/6.8.png" alt="RxJS и Observable в Angular" />
<p>Например, определим в файле <span class="b">users.json</span> данные, которые напрямую не соответствуют массиву объектов User:</p>
<pre class="brush:js;">
{ 
	"userList":
	[{
...
<p>В качестве модели данных используем класс User:</p>
<pre class="brush:ts;">
export class User{
    constructor(public name:string, public age:number){}
}
</pre>
<p>То есть в данном случае у нас нет соответствия по именам свойствам: name - username и age - userage.</p>
<p>Определим следующий код сервиса, который будет получать данные из users.json:</p>
<pre class="brush:ts;">
...
<p>Смысл использования специального сервиса для работы с http заключается в сокрытии деталей отправки запросов. 
...
преобразовать данные из одного формата в другой.</p>
<p>У результата метода <code>get()</code> мы можем вызвать метод <span class="b">pipe()</span>, который позволяет обработать результаты запроса. Для этого 
метод pipe в качестве первого параметра принимает функцию обработки данных запроса. В данном случае в роли такой функции выступает оператор <span class="b">map</span>, 
который преобразует результаты запроса в новые объекты.</p>
<p>Но чтобы использовать элементы библиотеки RxJS, их надо импортировать:</p>
<pre class="brush:ts;">
import {Observable} from 'rxjs';
import { map } from 'rxjs/operators';
</pre>
<p>В итоге весь метод <code>getUsers()</code> возвращает объект <code>Observable&lt;User[]&gt;</code>.</p>
<p>Теперь используем сервис в классе компонента:</p>
<pre class="brush:ts;">
