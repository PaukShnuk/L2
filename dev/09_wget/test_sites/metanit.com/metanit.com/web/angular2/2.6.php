...
<html >
<head>
<title>Angular | Работа с компонентами</title>
<meta charset="utf-8" />
<meta name="description" content="Главный и дочерний компоненты в Angular 13, вставка одного компонента в другой, элемент ng-content">
...
<h2>Работа с компонентами</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Кроме основных компонентов в приложении мы также можем определять какие-то вспомогательные компоненты, которые управляют каким-то участком разметки 
html. Более того в приложении на странице может быть ряд разных блоков с какой-то определенной задачей. И для каждого такого блока можно создать отдельный компонент, чтобы упростить управление блоками на странице.</p>
<p>Добавим в проект второй компонент. Для этого добавим в папку <span class="b">src/app</span> новый файл <span class="b">child.component.ts</span>. В итоге весь проект будет выглядеть следующим образом:</p>
<img src="./pics/2.17.1.png" alt="Взаимодействие между компонентами в Angular" />
<p>Определим в файле <span class="b">child.component.ts</span> следующий код:</p>
<pre class="brush:ts;">
...
<p>Здесь определен класс ChildComponent. Опять же чтобы сделать этот класс компонентом, необходимо применить декоратор <code>@Component</code>.</p>
<p>Компонент будет управлять разметкой html, которая будет вставляться в элемент <code>child-comp</code>.</p>
<p>Шаблон представления будет просто выводить заголовок. В заголовке выводится имя, заданное через переменную name.</p>
<p>И кроме того, здесь определены стили для элементов h2 и p.</p>
<p>Теперь изменим код компонента AppComponent в файле <span class="b">app.component.ts</span>:</p>
<pre class="brush:ts;">
import { Component } from '@angular/core';
...
<p>Это основной компонент, который будет запускаться при запуске приложения, и через него мы будем использовать остальные компоненты. Так, 
компонент ChildComponent будет загружаться в элемент <code>child-comp</code>. И в шаблоне компонента AppComponent как раз определен такой элемент.</p>
<p>Кроме того, компонент определяет стили для тех же элементов на странице, и также, как и ChildComponent, определяет свойство name, только с другим значением.</p>
<p>Чтобы использовать все определенные в проекте компоненты, они должны быть указаны в главном модуле приложения. Определим в файле 
<span class="b">app.module.ts</span> следующий модуль:</p>
<pre class="brush:ts;">
...
<p>Запустим проект:</p>
<img src="./pics/2.3.png" alt="Работа с компонентами в Angular 13" />
<p>Результат показывает, что несмотря на то, что один компонент как бы включен в другой с помощью тега <code>&lt;child-comp&gt;</code>, но тем не менее стили 
одного компонента не применяются к другому. Каждый компонент очерчивает свою область с помощью шаблона, свое представление, которым он и управляет.</p>
<p>Также каждый компонент использует свое значение свойства name. То есть компоненты фактически существуют относительно независимо.</p>
<h3>ng-content</h3>
<p>Элемент <code>ng-content</code> позволяет родительским компонентам внедрять код html в дочерние компоненты. Так, изменим компонент ChildComponent 
следующим образом:</p>
<pre class="brush:ts;">
import { Component } from '@angular/core';
...
<p>Вместо элемента <code>&lt;ng-content&gt;</code> извне можно будет передать любое содержимое.</p>
<p>И изменим код главного компонента AppComponent:</p>
<pre class="brush:ts;">
import { Component } from '@angular/core';
...
<p>В элемент <code>&lt;child-comp&gt;</code> здесь передается заголовок <code>&lt;h2&gt;Добро пожаловать {{name}}!&lt;/h2&gt;</code>. 
Затем этот заголовок будет вставляться в дочерний компонент ChildComponent на место <code>&lt;ng-content&gt;</code>:</p>
<img src="./pics/2.5.png" alt="ng-content в Angular 13" />
<p>Причем управлять разметкой, которая вставляется в <code>&lt;ng-content&gt;</code>, будет AppComponent. Поэтому именно этот компонент задает стили и выражения привязки для вставляемого куска html.</p>
