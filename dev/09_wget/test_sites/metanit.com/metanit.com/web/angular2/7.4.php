...
<html >
<head>
<title>Angular | Параметры строки запроса</title>
<meta charset="utf-8" />
<meta name="description" content="Параметры строки запроса и их получение в Angular, создание ссылок с параметрами">
...
<h2>Параметры строки запроса</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Кроме параметров маршрута в запросе могут передаваться параметры строки запроса. Например, в запросе <span class="ii">http://localhost:4200/item?product=phone&price=200</span> 
часть <code>product=phone&amp;price=200</code> будет представлять параметры запроса - product и price. Такие параметры помещаются в конце адреса после 
вопросительного знака и разделяются амперсандом.</p>
<p>Параметры строки запроса не влияют на определение маршрутов, и их количество произвольно. Так, для работы возьмем проект из прошлой темы.</p>
<img src="./pics/7.20.png" alt="получение параметров query params из строки запроса в Angular" />
<p>В этом проекте определен 
компонент ItemComponent, который получает параметр маршрута. Добавим в него также получения параметров из строки запроса:</p>
<pre class="brush:ts;">
...
<p>Для хранения полученных параметров определены свойства product и price. Получение параметров из строки запроса аналогично получению данных из маршрута, 
только в данном случае применяется свойство <code>queryParams</code> класса ActivatedRoute.</p>
<p>И как и в случае с параметрами маршрута, результатом вызова <code>route.queryParams.subscribe()</code> является объект Subscription, который необходимо удалять при удалении компонента 
в методе ngOnDestroy.</p>
<p>И после этого мы сможем передавать через строку запроса данные в ItemComponent, например, с помощью запроса <span class="ii">http://localhost:4200/item/5?product=phone&price=200</span>.</p>
<p>Теперь изменим код AppComponent, определив ссылки с параметрами:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>Для передачи параметров к ссылке применяется директива <span class="b">queryParams</span>, которая принимает javascript-объект.</p>
<img src="./pics/7.7.png" alt="Параметры строки запроса в Angular 13" />
