...
<html >
<head>
<title>Angular | Один сервис для всех компонентов</title>
<meta charset="utf-8" />
<meta name="description" content="Создание общего обекта сервиса для всего приложения в Angular">
...
<h2>Один сервис для всех компонентов</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Вполне возможно, что в нашем приложении будет несколько различных компонентов, которые используют сервисы. При этом компоненты могут использовать одни и те же сервисы. Например, 
в прошлых темах был создан сервис DataService, который управляет данными. Определим специальный компонент для работы с данными. 
Для этого возьмем проект из прошлой темы и добавим в папку <span class="ii">src/app</span> новый файл <span class="b">data.component.ts</span>:</p>
<img src="./pics/4.6.png" alt="Использование сервиса в компонентах в Angular" />
...
<p>DataComponent загружает и добавляет данные. Для работы с сервисами декоратор Component определяет секцию providers:</p>
<pre class="brush:ts;">providers: [DataService, LogService]</pre>
<p>Используем этот компонент DataComponent в главном компоненте приложения AppComponent:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>Этот компонент через элемент <code>data-comp</code> вызывает компонент DataComponent. Причем вызывает два раза. То есть для обработки каждого элемента будет создаваться свой 
объект DataComponent.</p>
<p>И соответственно изменим главный модуль приложения AppModule:</p>
<pre class="brush:ts;">
...
<p>Теперь запустим приложение и попробуем добавить новый элемент:</p>
<img src="./pics/4.3.png" alt="Несколько сервисов для компонентов в Angular" />
<p>Как видно на скриншоте, при добавлении в одном компоненте новый элемент будет добавляться только для этого компонента. Потому что у нас два отдельных компонента, и 
для каждого из них будет создаваться свой набор сервисов DataService и LogService.</p>
<p>Такое поведение не всегда может быть предпочтительным. Возможно, потребуется, чтобы компоненты использовали один и тот же объект сервиса, вместо создания разных сервисов для каждого компонента. 
Для этого мы можем зарегистрировать все сервисы не в компоненте, а в главном модуле приложения AppModule:</p>
<pre class="brush:ts;">
import { NgModule }      from '@angular/core';
import { BrowserModule } from '@angular/platform-browser';
import { FormsModule }   from '@angular/forms';
...
<p>В этом случае мы уже можем убрать регистрацию сервисов из компонента DataComponent:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
import {DataService} from './data.service';
...
<p>После этого оба объекта DataComponent будут использовать один и тот же сервис DataService. Поэтому добавление объекта в одном компоненте автоматически отразится и на другом:</p>
<img src="./pics/4.4.png" alt="Один сервис на все классы в Angular" />
