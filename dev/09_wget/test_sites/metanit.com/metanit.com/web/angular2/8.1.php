...
<meta charset="utf-8" />
<meta name="description" content="Pipes и их встроенные типы в Angular, форматирование чисел, валют, дат">
...
<h1>Pipes</h1><h2>Работа с pipes</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Pipes представляют специальные инструменты, которые позволяют форматировать отображаемые значения. Например, нам надо вывести определенную дату:</p>
<pre class="brush:ts;">
...
<p>Здесь создается дата, которая дважды выводится в шаблоне. Во втором случае к дате применяется форматирование с помощью класса DatePipe.</p>
<img src="./pics/8.1.png" alt="Pipes in Angular 13" />
<h3>Встроенные pipes</h3>
<p>В Angular есть ряд встроенных pipes. Основные из них:</p>
<ul>
<li><p><span class="b">CurrencyPipe</span>: форматирует валюту</p></li>
<li><p><span class="b">PercentPipe</span>: форматирует проценты</p></li>
<li><p><span class="b">UpperCasePipe</span>: переводит строку в верхний регистр</p></li>
<li><p><span class="b">LowerCasePipe</span>: переводит строку в нижний регистр</p></li>
<li><p><span class="b">DatePipe</span>: форматирует дату</p></li>
<li><p><span class="b">DecimalPipe</span>: задает формат числа</p></li>
<li><p><span class="b">SlicePipe</span>: обрезает строку</p></li>
</ul>
<p>При применении классов суффикс <code>Pipe</code> отбрасывается (за исключением DecimalPipe - для его применения используется название "number"):</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<img src="./pics/8.2.png" alt="Пайпы Pipes в AngularJS 9" />
<h3>Параметры в pipes</h3>
<p>Pipes могут получать параметры. Например, пайп SlicePipe, который обрезает строку, может получать в качестве параметра, начальный и конечный индексы подстроки, 
которую надо вырезать:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>Все параметры в пайп передаются через двоеточие. В данном случае <code>slice:6:11</code> вырезает подстроку, начиная с 6 до 11 индекса. При этом, 
если начала выреза строки обязательно передавать, то конечный индекс необязателен. В этом случае в качестве конечного индекса выступает конец строки.</p>
<img src="./pics/8.3.png" alt="slice in Angular 13" />
<h4>Форматирование дат</h4>
<p>DatePipe в качестве параметра может принимать шаблон даты:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<p>DecimalPipe в качестве параметра принимает формат числа в виде шаблона:</p>
<pre class="brush:js;">{{ value | number [ : digitsInfo [ : locale ] ] }}</pre>
<ul>
<li><p><code>value</code>: само выводимое значение</p></li>
<li><p><code>digitsInfo</code>: строка в формате "minIntegerDigits.minFractionDigits-maxFractionDigits", где</p></li>
<ul>
<li><p><code>minIntegerDigits</code> - минимальное количество цифр в целой части</p></li>
<li><p><code>minFractionDigits</code> - минимальное количество цифр в дробной части</p></li>
<li><p><code>maxFractionDigits</code> - максимальное количество цифр в дробной части</p></li>
</ul>
<li>
<li><p><code>locale</code>: код применяемой культуры</p></li>
</ul>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<img src="./pics/8.4.png" alt="Форматирование чисел в Angular 13" />
<h4>Форматирование валюты</h4>
<p>CurrencyPipe может принимать ряд параметров:</p>
<pre class="brush:js;">{{ value | currency [ : currencyCode [ : display [ : digitsInfo [ : locale ] ] ] ] }}</pre>
<ul>
<li><p><code>value</code>: выводимая сумма</p></li>
<li><p><code>currencyCode</code>: код валюты согласно спецификации ISO 4217. Если не указан, то по умолчанию применяется USD</p></li>
<li><p><code>display</code>: указывает, как отображать символ валюты. Может принимать следующие значения:</p>
<ul>
<li><p><code>code</code>: отображает код валюты (например, USD)</p></li>
<li><p><code>symbol</code> (значение по умолчанию): отображает символ валюты (например, $)</p></li>
<li><p><code>symbol-narrow</code>: некоторые страны используют в качестве символа валюты несколько символов, например, канадский доллар - CA$, данный параметр 
позволяет получить собственно символ валюты - $</p></li>
<li><p><code>string</code>: отображает произвольную строку</p></li>
</ul>
</li>
<li><p><code>digitsInfo</code>: формат числа, который применяется в DecimalPipe</p></li>
<li><p><code>locale</code>: код используемой локали</p></li>
</ul>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<img src="./pics/8.5.png" alt="Форматирование валюты в Angular" />
<h3>Цепочки pipes</h3>
<p>Вполне возможно, что мы захотим применить сразу несколько pipes к одному значению, тогда мы можем составлять цепочки выражений, разделенные вертикальной чертой:</p>
<pre class="brush:ts;">
import { Component} from '@angular/core';
...
<div class="nav"><p><a href="./7.7.php">Назад</a><a href="./">Содержание</a><a href="./8.2.php">Вперед</a></p></div></div>
<div class="socBlock">
