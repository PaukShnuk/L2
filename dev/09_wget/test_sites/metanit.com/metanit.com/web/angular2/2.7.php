...
<html >
<head>
<title>Angular | Взаимодействие между компонентами</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие между компонентами в Angular, декоратор Input и определение входных свойств">
...
<h2>Взаимодействие между компонентами</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<h3>Передача данных в дочерний компонент</h3><p>В прошлой теме было рассмотрено, как вызывать компонент из главного компонента. Однако по умолчанию эти компоненты никак не взаимодействуют, они независимы. Каждый компонент 
определяет свои выражения привязки. Однако что, если мы хотим свойства дочернего компонента привязать к свойствам из главного компонента? Для этого определим следующий дочерний компонент:</p>
<pre class="brush:ts;">
...
<p>Ключевым моментом здесь является определение входных свойств с помощью декоратора <span class="b">@Input()</span>. И естественно чтобы использовать декоратор, 
его надо импортировать:</p>
<pre class="brush:ts;">
import { Input} from '@angular/core';
</pre>
<p>Ключевой особенностью таких входных свойств является то, что они могут устанавливаться извне, то есть извне получать значения, например, из главного компонента.</p>
<p>Теперь изменим код главного компонента:</p>
<pre class="brush:ts;">
...
<p>Поскольку свойство <code>userName</code> в дочернем компоненте определено как входное с декоратором Input, то в главном компоненте мы можем его использовать как атрибут и фактически 
применить привязку свойств:</p>
<pre class="brush:ts;">&lt;child-comp [userName]="name" [userAge]="age"&gt;&lt;/child-comp&gt;</pre>
<p>То же самое касается и свойства userAge.</p>
<p>В итоге свойства userAge и userName будут привязаны к значениям из главного компонента:</p>
<img src="./pics/2.10.png" alt="Взаимодействие компонентов в Angular 13" />
<h3>Привязка к сеттеру</h3>
<p>Кроме привязки к свойству мы можем установить привязку к сеттеру дочернего компонента. (Подробнее про сеттеры и вообще про модификаторы доступа можно 
посмотреть <a href="../typescript/3.10.php">здесь</a>). Это может быть необходимо, когда в дочернем компоненте надо осуществлять проверку или даже модификацию значения, получаемого от 
главного компонента.</p>
<p>Например, пусть в главном компоненте устанавливается возраст пользователя:</p>
<pre class="brush:ts;">
import { Component } from '@angular/core';
...
<p>А в дочернем компоненте будем получать переданный возраст через сеттер:</p>
<pre class="brush:ts;">
import { Input, Component} from '@angular/core';
...
<p>В главном компоненте мы можем ввести любое значение в поле ввода, в том числе и отрицательные числа. В дочернем компоненте через сеттер проверяем введенное значение и 
при необходимости корректируем его.</p>
<img src="./pics/2.4.png" alt="Передача данных между компонентами в Angular 13" />
...
<div class="nav"><p><a href="./2.6.php">Назад</a><a href="./">Содержание</a><a href="./2.10.php">Вперед</a></p></div></div>
<div class="socBlock">
