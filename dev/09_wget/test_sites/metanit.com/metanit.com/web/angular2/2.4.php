...
<html >
<head>
<title>Angular | Стили и шаблоны компонента</title>
<meta charset="utf-8" />
<meta name="description" content="Стилилизация компонента в Angular, способы определения стилей, подключени внешнего файла стилей, вынесение кода шаблона во внешний файл html">
...
<h2>Стили и шаблоны компонента</h2><div class="date">Последнее обновление: 04.11.2021</div>
...
<p>Стилизация компонента может производиться как с помощью установки стилей в самом компоненте, так и с помощью подключения внешних css-файлов.</p>
<p>Для установки стилей в директиве @Component определено свойство <span class="b">styles</span>:</p>
<pre class="brush:ts;">
...
@Component({
    ...
    template: `&lt;h1&gt;Hello Angular 13&lt;/h1&gt;
			&lt;p&gt;Angular 13 представляет модульную архитектуру приложения&lt;/p&gt;`,
	styles: [` 
			h1, h2{color:navy;}
...
<p>Параметр <code>styles</code> содержит набор стилей, которые будут использоваться компонентом:</p>
<img src="./pics/2.1.png" alt="Стили в компонентах в Angular 13" />
<p>При использовании стилей следует учитывать, что они применяются локально только к разметке, управляемой компонентом. Например, если на странице будут элементы вне 
области управления компонентом, то к ним уже не будут применяться стили. Например:</p>
<pre class="brush:xml;">
&lt;body&gt;
...
<p>Если бы заголовок h2 здесь располагался бы в шаблоне компонента, то к нему применялся бы стиль. А  так он не будет стилизован:</p>
<img src="./pics/2.2.png" alt="Стилизация в Angular 13" />
<h4>Селектор :host</h4>
<p>Селектор <span class="b">:host</span> ссылается на элемент, в котором хостится компонент. То есть в данном случае это элемент <code>&lt;my-app&gt;&lt;/my-app&gt;</code>. 
И селектор :host как раз позволяет применить стили к этому элементу:</p>
<pre class="brush:css;">
styles: [` 
	h1, h2{color:navy;}
	p{font-size:13px;}
...
<h3>Подключение внешних файлов</h3>
<p>Если стилей много, то код компонента может быть слишком раздут, и в этом случае их вынести в отдельный файл css. Так, создадим в одной папке с классом компонента 
(который по умолчанию располагается в папке app) новый файл <span class="b">app.component.css</span> со следующим содержимым:</p>
<pre class="brush:css;">
h1, h2{color:navy;}
p{font-size:13px;}
:host {
...
<p>Затем изменим код компонента:</p>
<pre class="brush:ts;">
import { Component } from '@angular/core';
...
<p>Параметр <code>styleUrls</code> позволяет указать набор файлов css, которые применяются для стилизации. В данном случае предполагается, что файл css 
располагается в проекте в папке app.</p>
<p>Похожим образом мы можем вынести шаблон в отдельный файл html. Также в папке app создадим новый файл <span class="b">app.component.html</span> со следующим кодом:</p>
<pre class="brush:xml;">
&lt;h1&gt;Hello Angular 13&lt;/h1&gt;
&lt;p&gt;Angular 13 представляет модульную архитектуру приложения&lt;/p&gt;
</pre>
<p>То есть здесь определен весь тот же код, что ранее был в шаблоне компонента. И теперь изменим сам компонент:</p>
<pre class="brush:ts;">
import { Component } from '@angular/core';
...
<img src="./pics/2.24.png" alt="Стили и шаблоны компонента в Angular" />
<p>Таким образом, за счет выноса кода css и html код самого компонента стал чище и проще.</p>
