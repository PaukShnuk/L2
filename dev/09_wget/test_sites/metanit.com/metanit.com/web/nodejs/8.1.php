...
<html >
<head>
<title>Node.JS | Подключение к MySQL</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение к MySQL в Node.js с помощью драйвера mysql2, метод createConnection, закрытие подключения">
...
<h1>MySQL</h1><h2>Подключение к MySQL</h2><div class="date">Последнее обновление: 28.04.2019</div>
...
<p>Для работы с сервером MySQL в Node.js можно использовать ряд драйверов. Самые популярные из них mysql и mysql2. По большей части они 
совместимы. В данном случае мы будем использовать mysql2, так как, судя по ряду тестов, он предоставляет большую производительность.</p>
<p>Итак, установим пакет mysql2:</p>
<div class="console">
<pre class="consoletext">
npm install --save mysql2
</pre></div>
<h3>Создание подключения</h3>
<p>Для создания подключения применяется метод <span class="b">createConnection()</span>, который в качестве параметра принимает настройки 
подключения и возвращает объект, представляющий подключение.</p>
<pre class="brush:js;">
...
<p>Передаваемые в метод настройки конфигурации могут содержать ряд параметров. Наиболее используемые из них:</p>
<ul>
<li><p><span class="b">host</span>: хост, на котором запущен сервер mysql. По умолчанию имеет значение "localhost"</p></li>
<li><p><span class="b">port</span>: номер порта, на котором запущен сервер mysql. По умолчанию имеет значение "3306"</p></li>
<li><p><span class="b">user</span>: пользователь MySQL, который используется для подключения</p></li>
<li><p><span class="b">password</span>: пароль для пользователя MySQL</p></li>
<li><p><span class="b">database</span>: имя базы данных, к которой идет подключение. Необязательный параметр. Если он не указан, то подключение 
идет в целом к северу</p></li>
<li><p><span class="b">charset</span>: кодировка для подключения, например, по умолчанию используется "UTF8_GENERAL_CI".</p></li>
<li><p><span class="b">timezone</span>: часовой пояс сервера MySQL. This is used to type cast server date/time values to JavaScript. 
По умолчанию имеет значение "local"</p></li>
</ul>
<p>Для установки подключения мы можем использовать метод <span class="b">connect()</span> объекта connection:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>Метод <code>connect()</code> принимает функцию, параметр которой содержит ошибка, которая возникла при подключении.</p>
<h4>Возможные ошибки при подключении</h4>
<p>Если при подключении к серверу mysql генерируется ошибка</p>
<div class="console">
<pre class="consoletext">
Client does not support authentication protocol requested by server; consider upgrading MySQL client
</pre>
</div>
<p>В этом случае необходимо в MySQL Workbench выполнить следующую команду:</p>
<pre class="brush:sql;">
ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY 'password'
</pre>
<p>Вместо 'password' должен быть указан пароль от MySQL для пользователя root.</p>
<h3>Закрытие подключения</h3>
<p>Для закрытия подключения применяется метод <span class="b">end()</span>:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>При запуске приложения и удачном подключении-закрытии подключения мы увидим на консоли:</p>
<div class="console">
<pre class="consoletext">
...
<p>Метод <span class="b">end()</span> гарантирует, что перед закрытием подключения к бд будут выполнены все оставшиеся запросы, которые не завершились к моменту 
вызова метода.</p>
<p>Если мы не вызовем этот метод, то подключение будет оставаться активным, и приложение Node.js продолжит свою работу, пока сервер MySQL не закроет подключение.</p>
<p>Если же нам надо немедленно закрыть подключение, не дожидаясь выполнения оставшихся запросов, то в этом случае можно применить метод 
<span class="b">destroy()</span>:</p>
<pre class="brush:js;">connection.destroy()</pre>
...
<div class="nav"><p><a href="6.9.php">Назад</a><a href="./">Содержание</a><a href="8.2.php">Вперед</a></p></div></div>
<div class="socBlock">
