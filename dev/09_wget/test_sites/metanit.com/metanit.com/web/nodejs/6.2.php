...
<html >
<head>
<title>Node.JS | Добавление данных в MongoDB</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление данных в MongoDB в Node.js, методы insertOne и insertMany, добавление одного и нескольких документов">
...
<h2>Добавление данных в MongoDB</h2><div class="date">Последнее обновление: 10.08.2021</div>
...
<p>В прошлой теме были рассмотрены некоторые базовые моменты по работе с MongoDB. Теперь более подробно рассмотрим как добавлять и получать данные из бд.</p>
<p>Для добавления мы можем использовать различные методы. Если нужно добавить один объект, то применяется метод <span class="b">insertOne()</span>. 
При добавлении набора объектов можно использовать метод <span class="b">insertMany()</span>.</p>
<h3>Добавление одного документа</h3>
<p>Для добавления одного документа применяется метод <span class="b">insertOne()</span>. В качестве первого параметра он принимает добавляемый документ. Рассмотрим на примере:</p>
<pre class="brush:js;">
...
<p>Добавляемый документ представляет объект. В данном случае это объект</p>
<pre class="brush:js;">{name: "Tom", age: 23};</pre>
<p>То есть в коллекцию users будет добавляться документ, который имеет для свойства: <code>name</code> и <code>age</code>, которые хранят соответветственно 
строку "Tom" и число 23.</p>
<p>В качестве второго параметра в метод <span class="b">insertOne()</span> передается функция обратного вызова, которая выполняется после добавления. В этой функции применяются два 
параметра: <span class="b">err</span> (ошибка, которая может возникнуть при операции) и <span class="b">result</span> (результат операции - добавленный объект).</p>
<p>В функции обратного вызова инспектируется добавленный объект с помощью свойства <span class="b">result</span>. Кроме того, выводим на консоль объект добавленный 
user, чтобы посмотреть какие изменение произошли с ним осле добавления в бд.</p>
...
<p>Здесь мы можем увидеть результат операции, в частности, с помощью свойства <code>result.
insertedId</code> получить идентификатор добавленного объекта. Кроме того, мы видим, кроме начальных свойств документ user после добавления в БД 
получили дополнительное свойство <span class="b">_id</span> - это уникальный идентификатор документа, 
который присваивается сервером при добавлении.</p>
<p>Аналогичный пример с применением <code>async\await</code>:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<p>Теперь используем метод <span class="b">insertMany()</span>. Добавим набор объектов и для этого изменим файл приложения app.js:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<p>Как и insertOne, метод <code>insertMany()</code> в качестве первого параметра принимает добавляемые данные - массив объектов, а в качестве второго - функцию обратного вызова, 
которая выполняется при добавлении данных. При удачном добавлении второй параметр функции - results будет содержать добавленные данные. А добавляемые объекты из массива users аналогичным образом получат свойство <code>_id</code>.</p>
...
c:\node\mongoapp&gt;node app
{
	...
	insertedCount: 3,
	insertedIds: {
		"0": new ObjectId("6112a80873c40dedd6d837c"),
		"1": new ObjectId("6112a80873c40dedd6d837d"),
		"2": new ObjectId("6112a80873c40dedd6d837e")
	}
}
...
<p>Теперь результат операции - объект <code>results</code> будет содержать количество добавленных документов и их идентификаторы.</p>
...
<div class="nav"><p><a href="./6.1.php">Назад</a><a href="./">Содержание</a><a href="./6.10.php">Вперед</a></p></div></div>
<div class="socBlock">
