...
<html >
<head>
<title>Node.JS | Модули</title>
<meta charset="utf-8" />
<meta name="description" content="Основы работы с Node.js, создание и подключение модулей, функция require, встроенные модули">
...
<h1>Основы работы с Node.js</h1><h2>Модули</h2><div class="date">Последнее обновление: 15.11.2018</div>
...
который может использоваться повторно в других модулях.</p>
<p>При необходимости мы можем подключать нужные нам модули. Какие встроенные модули есть в node.js и какую функциональность они предоставляют, можно узнать 
из <a href="https://nodejs.org/api/" rel="nofollow">документации</a>.</p>
...
был необходим модуль <span class="b">http</span>:</p>
<pre class="brush:js;">
const http = require("http");
</pre>
<p>После получения модуля мы сможем использовать весь определенный в нем функционал, который опять же можно посмотреть в <a href="https://nodejs.org/api/http.html" rel="nofollow">документации</a>.</p>
<p>Подобным образом мы можем загружать и использовать другие встроенные модули. Например, используем модуль <a href="https://nodejs.org/api/os.html" rel="nofollow">os</a>, который предоставляет информацию об окружении и операционной системе:</p>
<pre class="brush:js;">
const os = require("os");
...
<p>Мы не ограничены встроенными модулями и при необходимости можем создать свои. Так, в прошлой теме проект состоял из файла <span class="ii">app.js</span>, в котором создавался сервер, обрабатывающий 
запросы. Добавим в тот же каталог новый файл <span class="ii">greeting.js</span> и определим в нем следующий код:</p>
<pre class="brush:js;">console.log("greeting module");</pre>
<p>В файле <span class="ii">app.js</span> подключим наш модуль:</p>
<pre class="brush:js;">
...
<img src="pics/2.1.png" alt="Модули в Node.js" />
<p>На консоль выводится та строка, которая определена в файле greeting.js.</p>
<p>Теперь изменим файл <span class="ii">greeting.js</span>:</p>
<pre class="brush:js;">
...
модуля были доступны, необходимо определить их в объекте <span class="b">module.exports</span>. Объект <code>module.exports</code> - это то, что возвращает функция <code>require()</code> при получении модуля.</p>
<p>Вообще объект <span class="b">module</span> представляет ссылку на текущий модуль, а его свойство <span class="b">exports</span> определяет все свойства и методы модуля, 
которые могут быть экспортированы и использованы в других модулях. Подробнее определение загрузки модуля и все его функции можно посмотреть на странице <a href="https://github.com/nodejs/node/blob/master/lib/module.js" rel="nofollow">https://github.com/nodejs/node/blob/master/lib/module.js</a>.</p>
...
<p>Далее изменим файл <span class="ii">app.js</span>:</p>
<pre class="brush:js;">
const os = require("os");
const greeting = require("./greeting");
...
<img src="pics/2.3.png" alt="Подключение модулей в NodeJS" />
<h3>Определение конструкторов и объектов в модуле</h3>
...
Так, добавим в папку проекта новый файл <span class="ii">user.js</span>:</p>
<pre class="brush:js;">
...
<p>Подключим и используем этот модуль в файле <span class="b">app.js</span>:</p>
<pre class="brush:js;">
...
<p>Запустим приложение:</p>
<img src="pics/2.8.png" alt="Функция конструктора в Node.js" />
