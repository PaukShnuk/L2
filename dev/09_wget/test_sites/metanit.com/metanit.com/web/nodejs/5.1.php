...
<html >
<head>
<title>Node.JS | Тестирование</title>
<meta charset="utf-8" />
<meta name="description" content="Тестирование в Node.js, использование библиотеки Mocha">
...
<h1>Тестирование</h1><h2>Mocha</h2><div class="date">Последнее обновление: 17.01.2017</div>
...
<p>Важное место при разработке на Node.js занимает тестирование. И в данном случае гораздо легче воспользоваться имеющимися фреймворками, 
которые упрощают процесс тестирования. Одним из таких фреймворков является <span class="b">Mocha</span>. Подробнее о фреймворке можно узнать 
на официальной странице <a href="https://mochajs.org/" rel="nofollow">Mochajs</a>. В данном же случае мы рассмотрим некоторые базовые стороны работы с ним.</p>
<p>Определим в папке проекта новый файл <span class="ii">package.json</span> со следующим содержимым:</p>
<pre class="brush:js;">
{
  "name": "testapp",
  "version": "1.0.0"
...
<p>Так как фреймворк Mocha необходим только для тестирования приложения, то он добавляется в файле package.json в секцию <span class="b">devDependencies</span> 
с помощью команды --save-dev.</p>
<p>Для тестирования определим простейший модуль. Для этого добавим в проект файл <span class="ii">operations.js</span> со следующим содержимым:</p>
<pre class="brush:js;">
module.exports.multiply = function(x,y){return x * y;}
</pre>
<p>Здесь определена функция умножения двух чисел.</p>
<p>Для тестирования этого модуля добавим в проект новый файл <span class="ii">operations.test.js</span>:</p>
<pre class="brush:js;">
...
<p>Рассмотрим этот тест. Для тестирования результата применяется функция <span class="b">it()</span>, которая предоставляется фреймворком Mocha.</p>
...
<p>К примеру, нам надо проверить работу выше определенной функции <code>multiply()</code>, которая умножает два числа. 
Для этого в эту функцию надо передать два числа и сравнить ее результат с ожидаемым. Если результат не совпадает с ожидаемым значением, то генерируется ошибка.</p>
...
<p>Здесь добавляется секция "scripts", в которой определяется команда "test". Эта команда выполняет команду "mocha *.test.js", которая запускает тестирование с 
помощью mocha, передавая фреймворку все файлы, которые оканчиваются на ".test.js"</p>
<p>Если у нас один файл теста, то мы могли бы сразу указать полное имя файла, типа <code>mocha operations.test.js</code></p>
...
<img src="pics/5.1.png" alt="Mocha и тестирование в Node.js" />
...
<p>То тест не будет проходить, так как результат - 15 не равен ожидаемому результату - числу 16. И консоль уведомит об этом при повторном запуске теста:</p>
<img src="pics/5.2.png" alt="Создание тестов в Node.js" />
<p>Подобным образом мы можем определять и другие тесты. Например, изменим файл модуля <span class="ii">operations.js</span>:</p>
<pre class="brush:js;">
module.exports.multiply = function(x,y){return x * y;}
module.exports.add = function(x, y){ return x + y;}
</pre>
<p>Теперь в файле была добавлена функция для сложения чисел. Протестируем ее в <span class="ii">operations.test.js</span>:</p>
<pre class="brush:js;">
var operations = require("./operations");
...
<p>Запустим тест:</p>
<img src="pics/5.3.png" alt="Несколько тестов в Node.js" />
<h3>Тестирование асинхронных функций</h3>
<p>Немного отличается тестирование асинхронных функций. Например, определим в модуле <span class="ii">operations.js</span> асинхронную функцию:</p>
<pre class="brush:js;">
module.exports.multiply = function(x,y){return x * y;}
module.exports.add = function(x, y){ return x + y;}
...
<p>Протестируем эту функцию в <span class="ii">operations.test.js</span>:</p>
<pre class="brush:js;">
var operations = require("./operations");
...
<p>Особенностью тестирования асинхронных функций является то, что чтобы они завершились до завершения теста, в тестирующую функцию передается функция <code>done()</code>. 
Причем при окончании тестирования нам надо вызвать эту функцию. Тем самым через подобную функцию Mocha сможет контролировать выполнение теста.</p>
<img src="pics/5.4.png" alt="Тестирование асинхронных функций в Mocha и Node.js" />
