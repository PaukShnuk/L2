...
<meta charset="utf-8" />
<meta name="description" content="Stream в Node.js, потоки чтения и записи">
...
<h2>Stream</h2><div class="date">Последнее обновление: 17.11.2018</div>
...
<p>Stream представляет поток данных. Потоки бывают различных типов, среди которых можно выделить <a href="https://nodejs.org/api/stream.html#stream_readable_streams" rel="nofollow">потоки для чтения</a> и 
<a href="https://nodejs.org/api/stream.html#stream_writable_streams" rel="nofollow">потоки для записи</a>.</p>
<p>При создании сервера в первой главе мы уже сталкивались с потоками:</p>
<pre class="brush:js;">
...
<p>Параметры request и response, которые передаются в функцию и с помощью которых мы можем получать данные о запросе и управлять ответом, как раз представляют собой потоки: 
request - поток для чтения, а response - поток для записи.</p>
<p>Используя потоки чтения и записи, мы можем считывать и записывать информацию в файл. Например:</p>
<pre class="brush:js;">
const fs = require("fs");
...
<p>Для создания потока для записи применяется метод <span class="b">fs.createWriteStream()</span>, в который передается название файла. Если вдруг в папке нет такого файла, 
то он создается.</p>
<p>Запись данных производится с помощью метода <code>write()</code>, в который передаются данные. Для окончания записи вызывается метод <code>end()</code>.</p>
<p>После этого в папке проекта появляется файл hello.txt, который можно открыть в любом текстовом редакторе.</p>
<p>Для создания потока для чтения используется метод <span class="b">fs.createReadStream()</span>, в который также передается название файла. В качестве опционального параметра 
здесь передается кодировка, что позволит сразу при чтении кодировать считанные данные в строку в указанной кодировке.</p>
<p>Сам поток разбивается на ряд кусков или чанков (chunk). И при считывании каждого такого куска, возникает событие <span class="b">data</span>. С помощью метода <span class="b">on()</span> 
мы можем подписаться на это событие и вывести каждый кусок данных на консоль:</p>
<pre class="brush:js;">
readableStream.on("data", function(chunk){ 
	console.log(chunk);
});
</pre>
<p>Запустим файл на выполнение:</p>
<img src="pics/2.23.png" alt="Stream in Node.js" />
<p>Только работой с файлами функциональность потоков не ограничивается, также имеются сетевые потоки, потоки шифрования, архивации и т.д., но общие принципы работы с ними будут те же, что и у файловых потоков.</p>
