...
<html >
<head>
<title>Node.JS | Assert</title>
<meta charset="utf-8" />
<meta name="description" content="Assert и тестирование в Node.js">
...
<h2>Assert</h2><div class="date">Последнее обновление: 17.01.2017</div>
...
<p>В предыдущей теме для верификации результата теста полученное значение сравнивалось с ожидаемым результатом:</p>
<pre class="brush:js;">
if(result!==expectedResult){
	throw new Error(`Expected ${expectedResult}, but got ${result}`);
}
</pre>
<p>Подобное сравнение является довольно тривиальным и фактически используется во многих тестах с небольшими изменениями. И специально чтобы упростить 
верификацию результатов в тестах для Node.js был создан специальный модуль - <a href="https://nodejs.org/api/assert.html" rel="nofollow">assert</a>. 
Кроме того, есть различные библиотеки, которые еще называют <span class="b">Assertions</span> и которые служат той же цели: 
<a href="https://github.com/shouldjs/should.js" rel="nofollow">should.js</a>, <a href="https://github.com/Automattic/expect.js" rel="nofollow">expect.js</a> и т.д. 
Но в данном случае мы рассмотрим работу с модулем Assert.</p>
<h3>equal</h3>
<p>Функция <span class="b">assert.equal()</span> сравнивает два значения. Если они не равны, то генерируется ошибка. Например, возьмем определенную в 
модуле operations.js функцию умножения двух чисел:</p>
<pre class="brush:js;">
...
<p>Если оба значения будут равны, то мы опять же увидим в консоли, что тесты пройдены. Если же значения не будут равны, то мы увидим красные маркеры с сообщениями 
об ошибке:</p>
<img src="pics/5.5.png" alt="Модуль Assert в Node.js" />
<p>Определенно вариант с функцией assert.equal проще, чем если бы мы сравнивали вручную два значения и сами генерировали ошибку с некоторым сообщением.</p>
<p>В противоположность этой функции функция <span class="b">assert.notEqual()</span> генерирует ошибку, если оба значения равны:</p>
<pre class="brush:js;">
it("should multiply two numbers", function(){
