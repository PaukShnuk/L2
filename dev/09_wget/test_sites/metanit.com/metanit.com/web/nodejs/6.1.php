...
<html >
<head>
<title>Node.JS | Начало работы с MongoDB</title>
<meta charset="utf-8" />
<meta name="description" content="Начало работы с MongoDB в Node.js, поключение к базе данных, сохранение документов">
...
<h1>Node.js и MongoDB</h1><h2>Начало работы с MongoDB</h2><div class="date">Последнее обновление: 10.08.2021</div>
...
<p>Наиболее популярной системой управления базами данных для Node.js на данный момент является MongoDB. Для работы с этой платформой прежде всего необходимо установить сам сервер MongoDB. Подробнее как это сделать, описывается <a href="../../nosql/mongodb/1.2.php">здесь</a>. 
Кроме самого сервера Mongo для взаимодействия с Node.js нам необходим <a href="https://www.npmjs.com/package/mongodb" rel="nofollow">драйвер</a>.</p>
<p>При подключении и взаимодействии с бд в MongoDB можно выделить следующие этапы:</p>
<ol>
<li><p>Подключение к серверу</p></li>
<li><p>Получение объекта базы данных на сервере</p></li>
<li><p>Получение объекта коллекции в базе данных</p></li>
<li><p>Взаимодействие с коллекцией (добавление, удаление, получение, изменение данных)</p></li>
</ol>
<p>Итак, создадим новый проект. Для этого определим новый каталог, который будет называться <span class="b">mongoapp</span>.  Далее определим в этом каталоге новый файл <span class="ii">package.json</span>:</p>
<pre class="brush:js;">
{
  "name": "mongoapp",
  "version": "1.0.0",
...
<p>В данном случае последняя зависимость - "mongodb" как раз и представляет драйвер. Всю необходимую справочную информацию конкретно по данному драйверу 
можно найти на <a href="https://docs.mongodb.com/drivers/node/current/" rel="nofollow">https://docs.mongodb.com/drivers/node/current/</a>.</p>
...
<p>Ключевым классом для работы с MongoDB является класс <span class="b">MongoClient</span>, и через него будет идти все взаимодействия с хранилищем данных. 
Соответственно вначале мы должны получить MongoClient:</p>
<pre class="brush:js;">const MongoClient = require("mongodb").MongoClient;</pre>
<p>Для подключения к серверу mongodb применяется метод <span class="b">connect()</span>:</p>
<pre class="brush:js;">
...
<p>Вначале создается объект MongoClient. Для этого в его конструктор передается два параметра. Первый параметр - это 
адрес сервера. В качестве протокола адреса устанавливается "mongodb://". На локальной машине адресом будет localhost, после 
которого указывается номер порта. По умолчанию номер порта 27017.</p>
<p>Далее с помощью метода connect происходит подключение к серверу. В качестве параметра метод принимает функцию обратного вызова, 
которая срабатывает при установке подключения. Это функция принимает два параметра: 
<span class="b">err</span> (возникшая ошибка при установке соединения) и <span class="b">client</span> (ссылка на подключенный к серверу клиент).</p>
<p>Если при подключении возникли ошибки, то мы можем использовать значение err для получения ошибки.</p>
<p>Если же ошибки нет, то мы можем взаимодействовать с сервером через объект <span class="b">client</span>.</p>
<p>В конце завершения работы с бд нам надо закрыть соединение с помощью метода <span class="b">client.close()</span>.</p>
<p>Выше был рассмотрен пример с использованием функций обратного вызова. Но node.js также позволяет использовать другой подход - с использованием 
<span class="b">async/await</span>:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<h3>База данных</h3>
<p>Получив объект подлюченного клиента, мы можем обращаться к базе данных на сервере. Для этого используется метод</p>
<pre class="brush:js;">const db = client.db("название_бд");</pre>
<p>В качестве параметра в метод передается название базы данных, к которой мы хотим подключиться. Результатом функции является объект базы данных, 
через который мы далее уже сможем обращаться к данным в этой БД.</p>
<p>Для примера выполним ping к базе данных "admin", которая по умолчанию есть на сервере MongoDB:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<p>Для выполнения диагностических команд типа пинга объект базы данных <code>Db</code> предоставляет метод <code>command</code>, который в качестве первого параметра 
принимает объект диагностики. Например, объект <code>{ping: 1}</code>, указывает, что надо выполнить пинг к базе данных, то есть проверить подключение к ней.</p>
<p>В качестве второго параметра передается функция обратного вызова с двумя параметрами. Первый параметр этой функции - <code>err</code> представляет 
информацию об ошибке, если такая возникла. Второй параметр - <code>result</code> предоставляет результат проверки.</p>
<p>В самой функции обратного вызова мы можем проверить наличие ошибки и при ее отстутствии вывести информацию об успешном подключении и полученный результат:</p>
<pre class="brush:js;">
if(!err){
	console.log("Подключение с сервером успешно установлено");
	console.log(result);
...
<p>Для тестирования подключения перейдем на жестком диске к каталогу, в который установлена mongodb, а в этом каталоге перейдем к папке <span class="b">bin</span>:</p>
<img src="./pics/7.1.png" alt="MongoDB в Node.js" />
<p>Запустим сервер mongodb, который находится в этом каталоге и который представляет собой консольную программу <span class="b">mongod</span>.</p>
<img src="./pics/7.2.png" alt="Запуск MongoDB в Node.js" />
<p>Затем запустим наш файл app.js:</p>
<img src="./pics/7.3.png" alt="Документы MongoDB в Node.js" />
<p>Здесь мы видим, что полученный при пинге результат представляет объект <code>{ok: 1}</code></p>
<p>Пример пинга с <code>async/await</code>:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<h3>Коллекции</h3>
<p>База данных в MongoDB не имеет таблиц. Вместо этого все данные попадают в коллекции. И в рамках node.js для взаимодействия 
с базой данных (добавления, удаления, чтения данных) нам потребуется получить объект коллекции. Для этого применяется метод <span class="b">db.collection("название_коллекции")</span>, 
в который передается название коллекции и который возвращает объект класса <code>Collection</code>, через который мы можем взаимодействовать с этой коллекцией.</p>

<p>В отличие от таблиц в реляционных системах, где все данные хранятся в виде строк, в коллекциях в MongoDB данные хранятся в виде документов. Например, 
обратимся к коллекции в базе данных. Для этого определим в каталоге проекта следующий файл <span class="b">app.js</span>:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<p>В качестве базы данных здесь используется "usersdb". При этом не важно, что по умолчанию на сервере MongoDB нет подобной базы данных. При первом к ней обращении 
сервер автоматически ее создаст.</p>
<p>После подключения мы обращаемся к коллекции "users":</p>
<pre class="brush:js;">const collection = db.collection("users");</pre>
<p>Опять же неважно, что такой коллекции по умолчанию нет в бд usersdb, она также будет создана при первом обращении.</p>
<p>Получив коллекцию, мы можем использовать ее методы. В данном случае для простоты применяется метод <span class="b">countDocuments()</span>, который получает количество документов коллекции. 
...
параметра: <span class="b">err</span> (ошибка, которая может возникнуть при операции) и <span class="b">result</span> (результат операции - собственно количество документов).</p>
<p>В функции обратного вызова выводим количество документов. Результат работы:</p>
<div class="console">
<pre class="consoletext">
В коллекции users 0 документов
</pre>
</div>
<p>Аналогичный пример с <code>async/await</code>:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
