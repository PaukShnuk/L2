...
<html >
<head>
<title>Node.JS | Движок EJS</title>
<meta charset="utf-8" />
<meta name="description" content="Template engine EJS в Express и Node.js, создание представлений">
...
<h2>Движок EJS</h2><div class="date">Последнее обновление: 25.11.2018</div>
...
<p>В качестве альтернативы Handlebars вкратце рассмотрим применение другого движка представлений - <a href="https://www.npmjs.com/package/ejs" rel="nofollow">EJS</a>. 
Этот движок использует синтаксис, который во многом был заимствован из движка представлений Web Forms на платформе ASP.NET, который разрабатывался в компании Microsoft.</p>
<p>Вначале добавим пакеты ejs в проект с помощью команды:</p>
<div class="console">
...
<p>По факту здесь определен тот же код, что и в одной из предыдущих тем, только теперь устанавливается движок ejs:</p>
<pre class="brush:js;">app.set("view engine", "ejs");</pre>
<p>Далее добавим в папку <span class="ii">views</span> в папке проекта новый файл <span class="b">contact.ejs</span> (представления в EJS по умолчанию имеют расширение .ejs):</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Для работы с моделью представления используются выражения javascript, которые определяются с помощью тега <span class="b">&lt;% выражение %&gt;</span>. 
...
открывающая фигурная скобка и закрывающаяся:</p>
<pre class="brush:js;">
&lt;% for(var i=0; i&lt;emails.length;i++) {%&gt;
	.........................
&lt;%} %&gt;
...
<img src="pics/4.24.png" alt="Движок EJS в Node.js" />
<h3>Частичные представления</h3>
<p>EJS позволяет вставлять код одних представлений в другие с помощью функции <span class="b">include</span>. Например, определим в папке views 
каталог <span class="b">partials</span>, в который добавим два файла. Первый файл - <span class="b">menu.ejs</span> будет иметь следующий код:</p>
<pre class="brush:xml;">
&lt;nav&gt;&lt;a href="/"&gt;Главная&lt;/a&gt; | &lt;a href="/contact"&gt;Контакты&lt;/a&gt;&lt;/nav&gt;
</pre>
<p>И второй файл - <span class="b">footer.ejs</span> будет предствлять футер:</p>
<pre class="brush:xml;">&lt;footer&gt;&lt;p&gt;MyCorp - Copyright © 2017&lt;/p&gt;&lt;/footer&gt;</pre>
<p>Теперь подключим эти файлы в <span class="b">contact.ejs</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В функцию include передается путь к вставляемому представлению. Файл app.js при этом остается тем же, что и выше. В итоге содержимое 
подключаемых представлений будет добавлено на веб-страницу:</p>
<img src="pics/4.34.png" alt="Partials in EJS in Express in Node.js" />
...
<div class="nav"><p><a href="4.9.php">Назад</a><a href="./">Содержание</a><a href="4.17.php">Вперед</a></p></div></div>
<div class="socBlock">
