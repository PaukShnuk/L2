...
<html >
<head>
<title>Node.JS | Связь многие-ко-многим в Sequelize</title>
<meta charset="utf-8" />
<meta name="description" content="Связь один-к-одному в Sequelize в веб-приложении на Node.js">
...
<h2>Связь многие-ко-многим</h2><div class="date">Последнее обновление: 17.05.2019</div>
...
<p>Отношение многие-ко-многим предполагает, что сущность одного типа одновременно может иметь связи с множеством сущностей другого типа и наоборот. Например, 
...
есть в данном сслучае имеем связь многие ко многим.</p>
<p>Физически на уровне базы данных обычно для создания подобной связи создается промежуточная таблица, через которую связываются две основные таблицы. 
В Sequelize поэтому для связи двух сущностей отношением многие-ко-многим нам надо задать промежуточную модель. Так, возьмем пример с курсами и студентами:</p>
<pre class="brush:js;">
...
<p>Здесь в качестве промежуточной сущности выступает модель Enrolment - по сути данные успеваемости определенного студента по определенному курсу. В этой модели 
...
в этой модели мы могли бы определить какие-нибудь атрибуты, которые бы связывали студента с курсом, например, дату поступления на данный курс, дату окончания и т.д.</p>
<p>Непосредственно для создания связи многие-ко-многим применяется метод <span class="b">belongsToMany()</span>. Первый параметр метода - сущность, с которой 
надо установить связь. Второй параметр - объект конфигурации связи, который с помощью параметра <span class="b">through</span> обязательно должен 
задавать промежуточную сущность, через которую будут связаны обе основные сущности.</p>
<p>В итоге при выполнении данного кода в базе данных будут созданы три таблицы с помощью следующих SQL-команд:</p>
<pre class="brush:sql;">
...
<h3>Добавление связанных данных</h3>
<p>При установке связи многие ко многим модели могут использовать метод <code>addИМЯ_МОДЕЛИ()</code> для добавления объектов (например, 
<code>student.addCourse()</code> и <code>course.addStudent()</code>). Например, пусть у нас будет создано несколько 
объектов - студентов и курсов:</p>
<pre class="brush:js;">
...
<p>Добавим студенту с именем Tom курс по JavaScript:</p>
<pre class="brush:js;">
...
<p>Первым параметром в <code>student.addCourse()</code> передается добавляемый курс. Вторым параметром устанавливается значение для столбца grade в таблице enrolments. 
В итоге данный метод будет выполнять sql-команду:</p>
<pre class="brush:sql;">INSERT INTO `enrolments` (`id`,`grade`,`studentId`,`courseId`) VALUES (NULL,1,2,2);</pre>
<h3>Получение связанных данных</h3>
<p>Для получения связанных данных у каждой из моделей, участвующих в связи, мы можем использовать метод <code>getИМЯ_МОДЕЛИs()</code>. Например, 
получим все курсы студента по имени Tom:</p>
<pre class="brush:js;">
Student.findOne({where: {name: "Tom"}})
.then(student=&gt;{
    if(!student) return;
...
<p>Однако в реальности в данном случае мы получаем не просто курс из таблицы courses, а сводные данные на основании таблицы enrolments - 
выполняемая sql-команда в данном случае будет выглядеть следующим образом:</p>
<pre class="brush:sql;">
...
<p>То есть в данном случае мы сможем получить название и id курса, а также id и значение grade объекта Enrolment:</p>
<pre class="brush:js;">
Student.findOne({where: {name: "Tom"}})
.then(student=&gt;{
    if(!student) return;
...
<h3>Удаление связанных данных</h3>
<p>Для удаления связанных данных необходимо получить объект из промежуточной таблицы и удалить его. Например, удалим у студента по имени Tom курс 
JavaScript:</p>
<pre class="brush:js;">
Student.findOne({where: {name: "Tom"}})
.then(student=&gt;{
...
<div class="nav"><p><a href="9.6.php">Назад</a><a href="./">Содержание</a><a href="7.1.php">Вперед</a></p></div></div>
<div class="socBlock">
