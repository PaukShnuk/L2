...
<html >
<head>
<title>Node.JS | Определение моделей в Sequelize</title>
<meta charset="utf-8" />
<meta name="description" content="Определение моделей в Sequelize для работы с базами данных MySQL, MS SQL Server, Postgres в приложении на Node.js">
...
<h2>Определение моделей</h2><div class="date">Последнее обновление: 02.05.2019</div>
...
<p>Ключевым компонентом в работе Sequelize с бд являются модели. Модели описывают стуктуру хранящихся в бд данных. Кроме того, через модели в основном идет 
взаимодействие с бд.</p>
<p>Есть два способа определения моделей. Первый способ представляет использование метода <span class="b">define()</span>. Например, определим модель 
User, которая представляет пользователя:</p>
<pre class="brush:js;">
...
<p>Первый параметр метода - это название модели. В данном случае 
...
будет называться "users". Это следует учитывать при наименовании моделей.</p>
<p>Второй параметр метода <code>define()</code> задает структуру модели, то есть описывает ее поля. Для каждого поля определяется ряд атрибутов. 
Атрибут <code>type</code> указывает на тип поля. В данном случае поля id и age представляют тип INTEGER, то есть целое число, а 
поле name - тип STRING, то есть строку.</p>
<p>Сопоставление некоторых типов sequelize с типами SQL:</p>
<table class="tab">
<tr class="tabhead"><td><p>Sequelize</p></td><td><p>SQL</p></td></tr>
<tr><td><p>Sequelize.STRING</p></td><td><p>VARCHAR(255)</p></td></tr>
<tr><td><p>Sequelize.STRING.BINARY</p></td><td><p>VARCHAR BINARY</p></td></tr>
<tr><td><p>Sequelize.TEXT</p></td><td><p>TEXT</p></td></tr>
<tr><td><p>Sequelize.INTEGER</p></td><td><p>INTEGER</p></td></tr>
<tr><td><p>Sequelize.FLOAT</p></td><td><p>FLOAT</p></td></tr>
<tr><td><p>Sequelize.DOUBLE</p></td><td><p>DOUBLE</p></td></tr>
<tr><td><p>Sequelize.DECIMAL</p></td><td><p>DECIMAL</p></td></tr>
<tr><td><p>Sequelize.BOOLEAN</p></td><td><p>TINYINT(1)</p></td></tr>
</table>
<p>Полный набор типов и их сопоставление с типами SQL можно посмотреть в <a href="http://docs.sequelizejs.com/manual/data-types.html" rel="nofollow">документации</a>.</p>
<p>Дополнительно для поля можно задать еще ряд атрибутов. Атрибут <span class="b">allowNull</span> указывает, допускает ли поле отсутствие значение. По умолчанию 
имеет значение <code>true</code> - то есть у поля может отсутствовать значение. Значение <code>false</code> указывает, что поле обязательно должно иметь 
какое-либо значение.</p>
<p>Для поля id в данном случае также задается еще пара атрибутов. Атрибут <code>autoIncrement: true</code> указывает, что значение столбца в таблице в БД будет автоинкрементироваться, 
а атрибут <code>primaryKey: true</code> говорит о том, что соответствующий столбец в таблице будет выполнять роль первичного ключа.</p>
<p>Второй способ определения модели:</p>
<pre class="brush:js;">
...
<h3>Синхронизация с бд</h3>
<p>Перед началом взаимодействия с бд нам надо убедиться, что таблицы в базе данных соответствуют определению наших моделей. Для синхронизации выполняется 
метод <span class="b">sync</span>:</p>
<pre class="brush:js;">
...
<p>Метод <span class="b">sync()</span> синхронизирует структуру базы данных с определением моделей. Например, если для какой-то модели 
отстуствует соответствующая таблица в БД, то эта таблица создается.</p>
<p>Следует учитывать, что кроме собственно полей модели в таблице по умолчанию будут создаваться два дополнительных поля: <span class="b">createdAt</span> 
и <span class="b">updatedAt</span>, которые будут иметь тип <code>datetime</code> и будут представлять соответственно время создания и последнего обновления строки в таблице.</p>
<p>Например, при работе с базой данных MySQL фактически созданная таблица будет описываться следующим скриптом:</p>
<pre class="brush:sql;">
CREATE TABLE `users` (
...
<p>Более того, нам не обязательно определять в модели поле id, так как sequelize по умолчанию также будет создавать подобное поле, которое будет выполнять роль первичного 
ключа.</p>
<p>Также можно вручную создать таблицу на сервере и потом к ней подключаться без необходимости синхронизации.</p>
<p>Но, возможно, нам не нужны эти дополнительные столбцы createdAt и updatedAt. В этом случае при определении объекта Sequelize нам надо задать параметр 
<code>define: {timestamps: false}</code>:</p>
<pre class="brush:js;">
...
<p>Еси в бд есть подобная таблица, но она не соответствует определению модели, то мы можем использоать параметр <code>{force: true}</code>, 
чтобы удалить таблицы и создать их заново, но уже с нужной нам структурой:</p>
<pre class="brush:js;">
