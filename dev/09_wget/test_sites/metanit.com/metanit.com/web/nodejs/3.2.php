...
<html >
<head>
<title>Node.JS | Получение данных от клиента</title>
<meta charset="utf-8" />
<meta name="description" content="Получение данных от клиента в Node.js, обработка событий data и end объекта request, перебор асинхронного итератора в request, получение объектов в формате json">
...
<h2>Получение данных от клиента</h2><div class="date">Последнее обновление: 05.12.2021</div>
...
<p>Все данные запроса передаются через объект <span class="b">request</span> - первый параметр функции обратного вызова Функция, 
которая передается в функцию <span class="b">http.createServer()</span>. Однако получить из него отправленные клиентом данные может представлять сложность. 
Прежде всего потому, что функция обратного вызова в <span class="b">http.createServer()</span> вызывается при получении всех HTTP-заголовков, но не тела запроса. 
Кроме того, сам объект <span class="b">request</span> представляет поток, который получает данные отдельными чанками. Соответственно для получения 
данных из запроса нам необходимо прослушивать этот поток и последовательно извлекать их него полученные чанки информации.</p>
<p>В этом случае для получения данных мы можем использовать разные способы.</p>
<h3>Обработка событий data и end объекта request</h3>
<p>Первый способ представляет прослушивание события <span class="b">data</span> 
обработчик которого получает пришедшие чанки информации. А когда все данные получены, срабатывает событие 
<span class="b">end</span>. Например, определим следующий простейший сервер:</p>
<pre class="brush:js;">
...
<p>При обращении по адресу "/user" с помощью метода <span class="b">request.on</span> устанавливаем для события <span class="b">data</span> функцию-обработчик. 
Эта функция в качестве параметра получает очередной пришедший кусочек оправленных данных в виде параметра chunk. В данном случае мы ожидаем, что клиент будет отправлять 
текст. И в функции-обработчике добавляем полученные данные к переменной data. И каждый раз, когда приложение получит очередную порцию данных, будет 
срабатывать данные обработчик и добавлять данные к переменной data.</p>
<p>Когда все данные будут получены, сработает событие <span class="b">end</span>, в функции-обработчике которого мы уже будем знать, что все данные получены и мы 
можем производить с ними необходимые нам манипуляции. В данном случае полученные данные просто выводяться на консоль, а в ответ клиенту отправляется некоторое 
уведомление.</p>
<p>Для тестирования можно определить следующий клиент - страницу index.html:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь клиент отправляет на сервер некоторую строку, которую сервер после получения выведет на консоль:</p>
<img src="./pics/3.9.png" alt="Получение данных в Node.js" />
<h3>Перебор асинхронного итератора request</h3>
<p>Второй способ представляет перебор объекта <span class="b">request</span>, который также предоставляет асинхронный итератор для получаемых данных:</p>
<pre class="brush:js;">
const http = require("http");
const fs = require("fs");
...
<p>Здесь вначале определяем буфер для добавляемых данных:</p>
<pre class="brush:js;">const buffers = [];</pre>
<p>Далее перебираем объект request, получая из него данные и добавляя их в буфер buffers:</p>
<pre class="brush:js;">
for await (const chunk of request) {
	buffers.push(chunk);
}
</pre>
<p>И в конце с помощью функции <span class="b">Buffer.concat()</span> объединяем все полученные данные и преобразуем их в строку методом <code>toString()</code>:</p>
<pre class="brush:js;">
const data = Buffer.concat(buffers).toString();
</pre>
<p>При этом мы неограничены получением только текстовых данных. Например, получим данные в формате json:</p>
<pre class="brush:js;">
const http = require("http");
const fs = require("fs");
...
<p>Здесь предолагается, что клиент посылает серверу объект в формате JSON, у которого есть свойства name и age. И с помощью функции <code>JSON.parse()</code> 
мы можем расарсить полученные данные в формат json.</p>
<p>Код клиента для текстирования мог бы выглядеть следующим образом:</p>
<pre class="brush:js;">
sendUser();
...
<img src="./pics/3.10.png" alt="Получение данных в формате json в Node.js" />
...
<div class="nav"><p><a href="3.4.php">Назад</a><a href="./">Содержание</a><a href="3.3.php">Вперед</a></p></div></div>
<div class="socBlock">
