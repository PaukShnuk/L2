...
<html >
<head>
<title>Node.JS | Передача параметров приложению</title>
<meta charset="utf-8" />
<meta name="description" content="Передача параметров приложению в Node.js, объект process.argv">
...
<h2>Передача параметров приложению</h2><div class="date">Последнее обновление: 17.11.2018</div>
...
<p>При запуске приложения из терминала/командной строки мы можем передавать ему параметры. Для получения параметров в коде приложения применяется 
массив <span class="b">process.argv</span>. Это аналогично тому, как в языках C/C++/C#/Java в функцию main передается набор аргументов в виде строкового массива.</p>
<p>Первый элемент этого массива всегда указывает на путь к файлу node.exe, который вызывает приложение. Второй элемент массив всегда указывает на 
путь к файлу приложения, который выполняется.</p>
<p>К примеру, определим следующий файл <span class="ii">app.js</span>:</p>
<pre class="brush:js;">
...
<p>В данном случае мы ожидаем, что приложению будут переданы два параметра: name и age.</p>
<p>Теперь запустим приложение с помощью следующей команды:</p>
<div class="console">
<pre class="consoletext">
node app.js Tom 23
</pre>
</div>
<p>В данном случае "Tom" и "23" - это те значения, которые помещаются соответственно в <code>process.argv[2]</code> и <code>process.argv[3]</code>:</p>
<img src="pics/2.13.png" alt="process.argv в Node.js" />
