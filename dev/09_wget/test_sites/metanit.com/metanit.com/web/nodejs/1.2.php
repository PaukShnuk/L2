...
<html >
<head>
<title>Node.JS | Первое приложение</title>
<meta charset="utf-8" />
<meta name="description" content="Создание первого приложения на Node.JS, создание сервера, модуль http и метод createServer">
...
<h2>Первое приложение на Node.js</h2><div class="date">Последнее обновление: 14.11.2018</div>
...
<p>Напишем первое простейшее приложение для NodeJS. Для создания приложений можно использовать практически все стандартные конструкции языка JavaScript. 
Исключением является работа с DOM, так как приложение будет запускаться на сервере, а не в браузере, поэтому DOM и такие объекты как <code>window</code> или 
<code>document</code> в данном случае нам будут недоступны.</p>
<p>Для этого вначале создадим для приложения каталог на жестком диске. К примеру, я создал каталог 
<span class="ii">C:\node\helloapp</span>. В этом каталоге создадим файл <span class="ii">app.js</span>.</p>
...
<p>Вкратце разберем этот код.</p>
<p>На первой строке мы получаем модуль http, который необходим для создания сервера. Это встроенный модуль, и для его загрузки необходимо применить функцию <code>require()</code>:</p>
<pre class="brush:js;">const http = require("http");</pre>
<p>Далее с помощью метода <code>createServer()</code> создается новый сервер для прослушивания входящих подключений и обработки запросов. В качестве параметра 
...
для отправки ответа. В данном случае ответ представляет простую строку "Hello NodeJS!" и отправляется с помощью метода <code>response.end()</code>.</p>
<p>Но метод <code>http.createServer()</code> только создает сервер. Чтобы сервер начал прослушивать входящие подключения у него надо вызвать метод <code>listen</code>:</p>
<pre class="brush:js;">
.listen(3000, "127.0.0.1",function(){
	console.log("Сервер начал прослушивание запросов на порту 3000");
});
</pre>
<p>Этот метод принимает три параметра. Первый параметр указывает на локальный порт, по которому запускается сервер. Второй параметр указывает 
на локальный адрес. То есть в данном случае сервер будет запускаться по адресу 127.0.0.1 или localhost на порту 3000.</p>
<p>Третий параметр представляет функцию, которая запускается при начале прослушивания подключений. Здесь эта функция просто выводит диагностическое сообщение на консоль.</p>
<p>Теперь запустим сервер. Для этого откроем терминал (в OS X или Linux) или командную строку (в Windows). С помощью команды <span class="b">cd</span> 
перейдем к каталогу приложения:</p>
<div class="console">
<pre class="consoletext">
cd C:\node\helloapp
</pre>
</div>
<p>Затем вызовем следующую команду:</p>
<div class="console">
<pre class="consoletext">
node app.js
</pre>
</div>
<p>Она запускает сервер:</p>
<img src="pics/1.3.png" alt="Запуск сервера NodeJS" />
<p>Далее откроем браузер и введем в адресную стоку адрес <span class="ii">http://localhost:3000/</span>:</p>
<img src="pics/1.4.png" alt="Первое приложение на NodeJS" />
<p>И нам отобразится то сообщение, которое было послано в методе <code>response.end()</code>.</p>
