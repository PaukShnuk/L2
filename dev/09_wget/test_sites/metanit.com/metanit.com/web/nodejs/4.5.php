...
<html >
<head>
<title>Node.JS | POST-запросы и отправка форм</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка POST-запросов в Express и Node.js, создание парсера форм, body-parser deprecated, получение данных, urldecode параметры форм">
...
<h2>POST-запросы и отправка форм</h2><div class="date">Последнее обновление: 07.09.2021</div>
...
<p>При оправке каких-то сложных данных обычно используются формы. Рассмотрим, как получать отправленные данные в Express.</p>
<p>Вначале определим в папке проекта новый файл <span class="b">index.html</span> с формой ввода данных:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В файле <span class="b">app.js</span> определим следующий код:</p>
<pre class="brush:js;">
...
<pre class="brush:js;">const urlencodedParser = express.urlencoded({extended: false});</pre>
<p>Поскольку данные отправляются с помощью формы, то для создания парсера применяется функция <span class="b">urlencoded()</span>. 
В эту функцию передается объект, устанавливающий параметры парсинга. Значение <code>extended: false</code> указывает, что объект - результат парсинга 
будет представлять набор пар ключ-значение, а каждое значение может быть представлено в виде строки или массива.</p>
<p>При переходе по адресу "/" будет срабатывать метод app.get, который отправит пользователю файл index.html.</p>
<p>Так как данные отправляются с помощью метода POST, то для обработки определяем функцию <span class="b">app.post("/",...)</span>. 
Первый параметр функции - адрес, на который идет отправка - "/". В коде формы в файле index.html мы не указали адрес для отправки, поэтому по умолчанию 
данные будут отправляться по тому же адресу, по которому доступна веб-страница index.html - "/". Таким образом, с одним 
адресом "/" связаны две функции, только одна обрабатывает запросы get, а другая - запросы post. Второй параметр - выше созданный парсер. Третий параметр - обработчик:</p>
<pre class="brush:js;">
app.post("/", urlencodedParser, function (request, response) {
	if(!request.body) return response.sendStatus(400);
	console.log(request.body);
...
<p>Для получения самих отправленных данных используем выражения типа <span class="b">request.body.userName</span>, где request.body инкапсулирует данные формы, 
а userName - ключ данных, который соответствует значению атрибута <code>name</code> поля ввода на html-странице:</p>
<pre class="brush:xml;">&lt;input type="text" name="userName" /&gt;</pre>
<p>Запустим приложение и обратимся к странице index.html:</p>
<img src="pics/4.20.png" alt="POST запросы в Express и Node.js" />
<p>После отправки введенные данные будут получены в функции <code>app.post("/",...)</code>, которая выведет результат обработки:</p>
<img src="pics/4.21.png" alt="Отправка форм в Express и Node.js" />
...
<div class="nav"><p><a href="4.14.php">Назад</a><a href="./">Содержание</a><a href="4.15.php">Вперед</a></p></div></div>
<div class="socBlock">
