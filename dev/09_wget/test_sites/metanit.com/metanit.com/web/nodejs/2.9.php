...
<html >
<head>
<title>Node.JS | События</title>
<meta charset="utf-8" />
<meta name="description" content="События в Node.js, объект EventEmitter, определение событий, наследование">
...
<h2>События</h2><div class="date">Последнее обновление: 17.11.2018</div>
...
<p>Подавляющее большинство функционала Node.js применяет асинхронную событийную архитектуру, которая использует специальные объекты - эмиттеры для генерации различных событий, которые обрабатываются 
специальными функциями - обработчиками или слушателями событий. Все объекты, которые генерируют события, представляют экземпляры класса <span class="b">EventEmitter</span>.</p>
<p>С помощью функции <code>eventEmitter.on()</code> к определенному событию по имени цепляется функция обработчика. Причем для одного события можно указать множество обработчиков. 
Когда объект EventEmitter генерирует событие, происходит выполнение всех этих обработчиков.</p>
<p>Рассмотрим применение объекта EventEmitter и событий. Для этого определим следующий файл <span class="ii">app.js</span>:</p>
<pre class="brush:js;">
...
<p>Весь необходимый функционал сосредоточен в модуле <span class="b">events</span>, который необходимо подключить. С помощью функции <span class="b">on()</span> 
...
событие называется "greet".</p>
<p>Для генерации события и вызова связанных с ним обработчиков выполняется функция <code>emitter.emit()</code>, в которое передается название события.</p>
<p>И при запуске приложения будут вызваны все обработчики:</p>
<img src="pics/2.21.png" alt="EventEmitter в Node.js" />
<h3>Передача параметров событию</h3>
<p>При вызове события в качестве второго параметра в функцию emit можно передавать некоторый объект, который передается в функцию обработчика события:</p>
<pre class="brush:js;">
const Emitter = require("events");
let emitter = new Emitter();
let eventName = "greet";
...
<h3>Наследование от EventEmitter</h3>
<p>В приложении мы можем оперировать сложными объектами, для которых также можно определять события, но для этого их надо связать с объектом EventEmitter. Например:</p>
<pre class="brush:js;">
...
<p>Здесь определена функция конструктора User, которая представляет пользователя. Для прототипа User определяется метод sayHi, в котором генерируется 
событие "greet".</p>
<p>Но чтобы связать объект User с EventEmitter, надо вызвать функцию <code>util.inherits(User, EventEmitter);</code>. Она позволяет унаследовать 
классу User функционал от EventEmitter. Благодаря этому мы можем через метод <code>on()</code> добавить к событию объекта user какой-нибудь обработчик, который будет вызван при выполнении метода <code>user.sayHi()</code>.</p>
<img src="pics/2.22.png" alt="Наследование от EventEmitter в Node.js" />
<p>С помощью возможностей ES6 мы можем упростить выше пример:</p>
<pre class="brush:js;">
const EventEmitter = require("events");
...
<p>Результат будет тот же, но теперь не нужно использовать функцию util.inherits.</p>
