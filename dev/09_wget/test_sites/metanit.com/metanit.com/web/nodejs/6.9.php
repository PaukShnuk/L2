...
<html >
<head>
<title>Node.JS | Express и Mongoose</title>
<meta charset="utf-8" />
<meta name="description" content="Чтение, удаление, добавление и изменение данных в Mongoose и MongoDB в Node.js при обработке запросов в сервере Express">
...
<h2>Express и Mongoose</h2><div class="date">Последнее обновление: 11.08.2021</div>
...
<p>Рассмотрим, как совместить Mongoose и Express и выполнять операции с данными, когда приходят те или иные запросы к серверу. 
Для этого определим следующий файл приложения app.js:</p>
<pre class="brush:js;">
...
<p>По сути здесь производятся все те операции, которые были рассмотрены в прошлой теме. Единственное, что можно отметить, это запуск сервера 
после удачного подключения к базе данных usersdb в функции mongoose.connect:</p>
<pre class="brush:js;">
mongoose.connect("mongodb://localhost:27017/usersdb", { useUnifiedTopology: true, useNewUrlParser: true}, function(err){
	if(err) return console.log(err);
    app.listen(3000, function(){
...
<p>Поскольку Express в качестве хранилища 
статических файлов использует папку public, то при обращении к приложению по корневому маршруту <span class="ii">http://localhost:3000</span> 
клиент получит данный файл.</p>
<p>Запустим приложение, обратимся к приложению по адресу http://localhost:3000 и мы сможем взаимодействовать с базой данных MongoDB через Mongoose:</p>
...
<div class="nav"><p><a href="./6.8.php">Назад</a><a href="./">Содержание</a><a href="./8.1.php">Вперед</a></p></div></div>
<div class="socBlock">
