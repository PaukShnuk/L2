...
<html >
<head>
<title>Node.JS | Выполнение запросов к MySQL</title>
<meta charset="utf-8" />
<meta name="description" content="Выполнение запросов к MySQL в Node.js с помощью драйвера mysql2, метод query и execute	">
...
<h2>Выполнение запросов к MySQL</h2><div class="date">Последнее обновление: 28.04.2019</div>
...
<p>Для выполнения запросов у объекта подключения применяется метод <span class="b">query()</span>. Наиболее простая его форма:</p>
<pre class="brush:js;">query(sqlString, callback)</pre>
<p>Где <code>sqlString</code> - выполняемая SQL-команда, а <code>callback</code> - функция обратного вызова, через параметры которой мы можем 
получить результаты выполнения sql-команды или возникшую ошибку.</p>
<p>Например, получим все данные из таблицы:</p>
<pre class="brush:js;">
...
<p>В данном случае выполняется команда <code>SELECT</code>, которая извлекает все данные из таблицы "users". Функция обратного вызова принимает 
три параметра. Первый параметр передает ошибка, если она возникла при выполнении запроса. Второй параметр - <code>results</code> собственно 
представляет в виде массива те данные, которые получила команда SELECT. И третий параметр <code>fields</code> хранит метаданные полей таблицы и дополнительную служебную информацию.</p>
<p>Стоит отметить, что при выполнении запросов неявно устанавливается подключение, поэтому перед выполнением запроса нам в принципе необязательно у объекта подключения вызывать метод <span class="b">connect()</span>.</p>
<p>Также в mysql2 определен метод <span class="b">execute()</span>, который работает аналогичным образом:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>Если в запрос надо вводить данные, которые приходят извне, то для избежания sql-инъекций рекоммендуется использовать параметризацию.</p>
<p>При параметризации вместо конкретных данных в тексте запроса ставятся плейсхолдеры - знаки вопроса, вместо которых при выполнении 
запроса будут вставляться собственно данные. Например, добавление данных:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>В данном случае данные определены в виде массива user, которая в качестве параметра передается в метод <code>connection.query()</code>. При выполнении 
...
<code>INSERT INTO users(name, age) VALUES("Tom", 29)</code></p>
