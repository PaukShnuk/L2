...
<html >
<head>
<title>Node.JS | Удаление документов в MongoDB</title>
<meta charset="utf-8" />
<meta name="description" content="Удаление документов в MongoDB в Node.js, методы findOneAndDelete, deleteOne и deleteMany">
...
<h2>Удаление документов в MongoDB</h2><div class="date">Последнее обновление: 11.08.2021</div>
...
<p>Удалять документы в MongoDB можно различными способами. Здесь надо отметить следующие методы коллекции:</p>
<ul>
<li><p><span class="b">deleteMany()</span>: удаляет все документы, которые соответствуют определенному критерию</p></li>
<li><p><span class="b">deleteOne()</span>: удаляет один документ, который соответствует определенному критерию</p></li>
<li><p><span class="b">findOneAndDelete()</span>: получает и удаляет один документ, который соответствует определенному критерию</p></li>
<li><p><span class="b">drop()</span>: удаляет всю коллекцию</p></li>
</ul>
<h4>Удаление одного документа и метод deleteOne</h4>
<p>Метод <span class="b">deleteOne()</span> удаляет только объект:</p>
<pre class="brush:js;">
...
<p>Первый параметр в методе - фильтр документов, а второй - функция обратного вызова, в которой мы можем получить результат удаления. При этом результат удаления будет 
представлять объект, в котором с помощью свойства <code>deletedCount</code> можно увидеть количество удаленных документов:</p>
<div class="console">
...
<h4>Удаление нескольких объектов и метод deleteMany</h4>
<p>Метод <span class="b">deleteMany()</span> работает аналогичным образом, только удаляет все документы, которые соответствуют фильтру. Например, 
удалим всех пользователей, у которых имя "Tom":</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<h4>findOneAndDelete</h4>
<p>Метод <span class="b">findOneAndDelete()</span> удаляет один документ по определенному критерию, но по сравнению с методом deleteOne() он позволяет получить удаленный документ:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<p>Из консольного вывода мы видим, что удаленный документ прячется в свойстве <code>value</code> полученного результата.</p>
<p>Аналогичный пример с <code>async\await</code>:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<h4>drop</h4>
<p>Метод <span class="b">drop()</span> удаляет всю коллекцию:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<p>Стоит отметить, что если удаляемая коллекция - единственная коллекция в базе данных, то вместе с коллекцией удаляется и вся база данных.</p>
<p>Аналогичный пример с <code>async\await</code>:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<div class="nav"><p><a href="./6.10.php">Назад</a><a href="./">Содержание</a><a href="./6.4.php">Вперед</a></p></div></div>
<div class="socBlock">
