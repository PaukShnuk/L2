...
<html >
<head>
<title>Node.JS | Promise API в запросах к MySQL</title>
<meta charset="utf-8" />
<meta name="description" content="Выполнение запросов к MySQL в Node.js с помощью драйвера mysql2 и Promise API">
...
<h2>Promise API</h2><div class="date">Последнее обновление: 28.04.2019</div>
...
<p>Драйвер MySQL2 позволяет использовать промисы при выполнении запросов к бд. Например:</p>
<pre class="brush:js;">
...
<p>Для создания промиса при создании объекта connection вызывается метод <span class="b">promise()</span>. Затем при выполнении запроса 
с помощью метода <code>query()</code> мы можем к нему по цепочке подсоединить методы <span class="b">then()</span> и <span class="b">catch()</span>.<p> 
<p>Если при выполнении запроса возникла ошибка, то метод <code>catch()</code> выполняет функцию, которая передается в метод в качестве параметра. 
Эта функция принимает один параметр - собственно объект ошибки.</p>
<p>Если запрос выполнился успешно, то вызывается метод <code>then()</code>, который выполняет функцию, передаваемую в качестве параметра. 
Эта функция принимает в качестве параметра результат запроса. Результат запроса может оличаться в зависимости от посылаемой серверу команды SQL.</p>
<p>Для команды SELECT результат запроса фактически представляет массив из двух объектов, где первый объект - полученные из БД данные в виде массива, а второй - метаданные полей данных. 
То есть мы могли бы получить непосредственно сами данные так:</p>
<pre class="brush:js;">
connection.query("SELECT * FROM users")
          .then(result =&gt;{
...
<p>Консольный вывод в обоих случаях будет наподобие следующего:</p>
<div class="console">
...
<p>Если выполняется SQL-команда INSERT, UPDATE или DELETE, то результатом запроса будет объект, свойства которого описывают результат операции:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>Опять же результат запроса представляет массив из двух объектов, где собственно результаты операции описываются первым элементом. 
В частности, в данном случае консольный вывод будет наподобие следующего:</p>
<div class="console">
<pre class="consoletext">
C:\node\mysqlapp> node app.js
ResultSetHeader {
...
<p>Например, с помощью свойства <code>affectedRows</code> полученного объекта мы можем получить количество затронутых операцией (добавленных/
удаленных/обновленных) строк.</p>
<p>Все, что было выше сказано про метод <code>query()</code> относится и к методу <code>execute()</code>, который работает аналогичным образом:</p>
<pre class="brush:js;">
