...
<html >
<head>
<title>Node.JS | JSON и AJAX</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка данных в формате JSON в Express и Node.js, взаимодействие клиента и сервера с помощью Ajax">
...
<h2>JSON и AJAX</h2><div class="date">Последнее обновление: 18.04.2019</div>
...
<p>Пусть в папке проекта имеется файл <span class="b">index.html</span> со следующим кодом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь, как и в прошлой теме, определена форма с двумя полями для ввода имени и возраста пользователя. Но теперь с помощью 
...
с помощью ajax на адрес "/user".</p>
...
<p>Прежде всего для получения данных в формате json необходимо создать парсер с помощью функции json:</p>
<pre class="brush:js;">const jsonParser = express.json();</pre>
<p>И поскольку с клиентом мы взаимодействуем через формат json, то данные клиенту отправляются с помощью метода <code>response.json()</code>:</p>
<pre class="brush:js;">
app.post("/user", jsonParser, function (request, response) {
	console.log(request.body);
    if(!request.body) return response.sendStatus(400);
...
<p>Следует отметить, что в принципе мы можем отправить объект и с помощью стандартного метода <code>response.send(request.body)</code>. В реальности 
метод <code>response.json()</code> устанавливает для заголовка "Content-Type" значение "application/json", серилизует данные в json с помощью 
функции <code>JSON.stringify()</code> и затем отправляет данные с помощью <code>response.send()</code>.</p>
<p>Для получения данных, как и в случае с формами, используются выражения типа <span class="b">request.body.userName</span>, где request.body инкапсулирует данные формы, 
а userName - ключ данных.</p>
<p>При обращении к корню веб-приложения пользователю будет отправляться содержимое файла index.html с формой ввода данных.</p>
<p>Запустим приложение и обратимся к корню веб-сайта. Введем какие-нибудь данные и после отправки в консоли браузера отобразится ответ сервера:</p>
<img src="pics/4.22.png" alt="AJAX и JSON в Express и Node.js" />
