...
<html >
<head>
<title>Node.JS | Sequelize</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение к базам данных MySQL, MS SQL Server, Postgres через Sequelize в приложении на Node.js">
...
<h1>Sequelize</h1><div class="date">Последнее обновление: 02.05.2019</div>
...
<p>Sequelize - это ORM-библиотека для приложений на Node.js, которая осуществляет сопоставление таблиц в бд и отношений между ними с классами. При использовании 
...
MariaDB, SQLite, MS SQL Server.</p>
<p>Более подробно про работу sequelize можно посмотреть в <a href="http://docs.sequelizejs.com/" rel="nofollow">официальной документации</a>. А в данном случае мы сосредаточимся на основных моментах 
работы с данными на примере взаимодействия с БД MySQL.</p>
<p>Вначале установим пакет sequelize.</p>
<div class="console">
...
<p>Для подключения к базе данных прежде всего необходимо создать объект Sequelize:</p>
<pre class="brush:js;">
...
<p>Для создания объекта sequelize используется функция <span class="b">Sequelize</span>, которая принимает ряд параметров. 
Первый параметр - имя базы данных. Второй параметр - логин к бд, третий параметр - пароль. Это обязательные параметры.</p>
<p>Кроме того, с помощью четвертого параметра мы можем задать ряд дополнительных опций конфигурации. Четвертый параметр представляет объект, 
который имеет множество свойств. В данном случае используются только два. Первое свойство - <code>dialect</code> указывает на диалект языка SQL, который используется в запросах - 
в данном случае "mysql". Второе свойство <code>host</code> представляет адрес, по которому запущен сервер. По умолчанию <code>host</code> 
имеет значение "localhost", поэтому для подключения к локальной базе данных это свойство в принципе можно не указывать.</p>
<p>Для свойства <span class="b">dialect</span> в зависимости от того, с какой СУБД предстоит работать, могут использоваться следующие значения:</p>
<ul>
<li><p><code>mysql</code></p></li>
<li><p><code>mariadb</code></p></li>
<li><p><code>sqlite</code></p></li>
<li><p><code>postgres</code></p></li>
<li><p><code>mssql</code></p></li>
</ul>
<h3>Подключение к mysql</h3>
<p>Для работы с MySQL также должен быть установлен пакет <span class="b">mysql2</span>, работа с котором 
разбиралась в прошлой главе:<p>
<div class="console">
<pre class="consoletext">
npm install --save mysql2
</pre>
</div>
<p>И при создании объекта Sequelize в параметрах указывается <code>dialect: "mysql"</code>:</p>
<pre class="brush:js;">
const Sequelize = require("sequelize");
const sequelize = new Sequelize("usersdb2", "root", "123456", {
  dialect: "mysql",
...
<h3>Подключение к mssql</h3>
<p>Для работы с MS SQL Server в проекте должен быть установлен пакет <span class="b">tedious</span>:<p>
<div class="console">
<pre class="consoletext">
npm install --save tedious
</pre>
</div>
<p>И при создании объекта Sequelize в параметрах указывается <code>dialect: "mssql"</code>:</p>
<pre class="brush:js;">
const Sequelize = require("sequelize");
...
<p>Кроме диалекта в настройках конфигурации также указывается параметр <span class="b">host</span> - адрес сервера и <span class="b">port</span>. Если 
сервер запущен на порту 1433, то в принципе его необязательно указывать, так как этот порт Sequelize 
использует по умолчанию. Любой другой порт необходимо указывать.</p>
<p>При создании подключения к MSSQL Server мы можем столкнуться с рядом сложностей, которые вытекают в основном из конфигурации самого сервера. 
...
пункту <span class="b">Администрирование</span> и затем выберем пункт <span class="b">Управление компьютером</span></p>
<img src="pics/10.5.png" alt="Настройка MS SQL Server для Sequelize" />
<p>В открывшейся вкладке перейдем к пункту <span class="b">Службы и приложения -&gt; SQL Server Configuration Manager -&gt; SQL Server Network Configuration</span> 
и найдем нужный нам экземпляр сервера. Убедимся, что для включен параметр <span class="b">TCP/IP</span> (то есть имеет значение "Enabled").</p>
<img src="pics/10.6.png" alt="Настройка порта MS SQL Server для Sequelize" />
<p>Для использования логина и пароля в MS SQL Server убедимся, что он настроен на смешанную аутентификацию. Для этого подключимся к серверу через SQL Server Managemen Studio 
и перейдем к свойствам сервера:</p>
<img src="pics/10.7.png" alt="Настройка логина и пароля MS SQL Server для Sequelize" />
<p>Затем в открывшемся окне на вкладке <span class="b">Security</span> выбрем режим <span class="b">SQL Server and Windows Authentication mode</span>.</p>
<img src="pics/10.8.png" alt="Windows Authentication for MS SQL Server in Sequelize" />
<p>После применения всех настроек перезапустим сервер.</p>
<p>И естественно перед подключением необходимо создать базу данных, с которой будет идти взаимодействие.</p>
<h3>Подключение к postgres</h3>
<p>Для работы с СУБД Postgres также необходимо установить пакет <span class="b">pg</span>:<p>
<div class="console">
<pre class="consoletext">
npm install --save pg
</pre>
</div>
<p>И при создании объекта Sequelize в параметрах указывается <code>dialect: "postgres"</code>:</p>
<pre class="brush:js;">
const Sequelize = require("sequelize");
const sequelize = new Sequelize("usersdb2", "root", "123456", {
  dialect: "postgres"
...
<p>Все остальное взаимодействие с базой данных в большинстве случаев за некоторыми исключениями практически не зависит от конкретной СУБД. А это значит, что 
большую часть кода (если не весь код) мы можем использовать для подключения к разным СУБД.</p>
...
<div class="nav"><p><a href="8.6.php">Назад</a><a href="./">Содержание</a><a href="9.2.php">Вперед</a></p></div></div>
<div class="socBlock">
