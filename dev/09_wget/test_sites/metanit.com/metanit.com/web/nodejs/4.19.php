...
<html >
<head>
<title>Node.JS | Layout в Handlebars</title>
<meta charset="utf-8" />
<meta name="description" content="Файлы layout в Handlebars в Express и Node.js, создание единообазного вида сайта">
...
<h2>Layout в Handlebars</h2><div class="date">Последнее обновление: 15.02.2022</div>
...
<p>Файл layout или мастер-страница позволяет определить общий макет всех веб-страниц сайта. Благодаря чему гораздо проще обновлять сайт, определять и менять 
какие-то общие блоки кода.</p>
<p>Для работы с файлами layout установим в проект модуль <span class="b">express-handlebars</span> с помощью следующей команды:</p>
<div class="console"><pre class="consoletext">npm install express-handlebars</pre></div>
<p>Пусть в проекте в папке <span class="b">views/partials</span> будут определены два частичных представления для меню и футера. Первое представление - <span class="b">menu.hbs</span>:</p>
<pre class="brush:xml;">&lt;nav&gt;&lt;a href="/"&gt;Главная&lt;/a&gt; | &lt;a href="/contact"&gt;Контакты&lt;/a&gt;&lt;/nav&gt;</pre>
<p>И второе представление - <span class="b">footer.hbs</span>:</p>
<pre class="brush:xml;">
&lt;footer&gt;&lt;p&gt;MyCorp - Copyright &copy; 2017&lt;/p&gt;&lt;/footer&gt;
</pre>
<p>Создадим в проекте в папке views новый каталог <span class="b">layouts</span> и определим в нем файл <span class="b">layout.hbs</span>, 
который будет определять макет сайта:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь внедряются частичные представления menu.hbs и footer.hbs. И, кроме того, здесь также присутствует такое выражение, как <code>{{{body}}}</code>. 
Вместо этого выражения будет вставлять содержимое конкретных представлений.</p>
<p>Затем в папке views определим два обычных представления. Представление <span class="b">contact.hbs</span>:</p>
<pre class="brush:xml;">
&lt;h1&gt;{{title}}&lt;/h1&gt;
&lt;p&gt;Электронный адрес: {{email}}&lt;/p&gt;
&lt;p&gt;Телефон: {{phone}}&lt;/p&gt;
</pre>
<p>И представление <span class="b">home.hbs</span>:</p>
<pre class="brush:xml;">
&lt;h1&gt;Главная страница&lt;/h1&gt;
</pre>
<p>Эти представления не содержат элементов body, head, каких-то общих блоков, так как все они определены в файле layout.hbs.</p>
<p>Далее определим следующий файл <span class="b">app.js</span>:</p>
<pre class="brush:js;">
...
<p>Для использования файлов layout необходимо настроить движок hbs:</p>
<pre class="brush:js;">
app.engine("hbs", expressHbs.engine(
    {
        layoutsDir: "views/layouts", 
...
<p>Функция <code>expressHbs.engine()</code> осуществляет конфигурацию движка. В частности, свойство <code>layoutsDir</code> задает путь к папке с файлами layout относительно 
корня каталога проекта. Свойство <code>defaultLayout</code> указывает на название файла, который будет использоваться в качестве мастер-страницы. В нашем случае это 
файл <code>layout.hbs</code>, поэтому указываем название этого файла без расширения. И третье свойство - <code>extname</code> задает расширение файлов.</p>
<p>В итоге весь проект будет выглядеть следующим образом:</p>
<img src="./pics/4.32.png" alt="Layout in Handlebars in Express in Node.js" />
<p>Пример работы приложения:</p>
<img src="./pics/4.33.png" alt="Файлы Layout в Handlebars в Express в Node.js" />
...
<div class="nav"><p><a href="4.8.php">Назад</a><a href="./">Содержание</a><a href="4.9.php">Вперед</a></p></div></div>
<div class="socBlock">
