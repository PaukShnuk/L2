...
<html >
<head>
<title>Node.JS | Pipe</title>
<meta charset="utf-8" />
<meta name="description" content="Pipe в Node.js, каналы между потоками чтения и записи, архивация файла">
...
<h2>Pipe</h2><div class="date">Последнее обновление: 17.11.2018</div>
...
<p>Pipe - это канал, который связывает поток для чтения и поток для записи и позволяет сразу считать из потока чтения в поток записи. Для чего они нужны? 
Возьмем, к примеру проблему копирования данных из одного файла в другой.</p>
<p>Пусть в папке проекта определен некоторый файл <span class="ii">hello.txt</span>. Скопируем его содержимое в новый файл <span class="ii">some.txt</span>:</p>
<pre class="brush:js;">
const fs = require("fs");
...
<p>Однако задача записи в поток данных, считанных из другого потока, является довольно распространенной, и в этом случае pipes или каналы позволяют 
нам сократить объем кода:</p>
<pre class="brush:js;">
const fs = require("fs");
...
<p>У потока чтения вызывается метод <span class="b">pipe()</span>, в который передается поток для записи.</p>
<p>Рассмотрим другую проблему - архивацию файла. Здесь нам надо сначала считать файл, затем сжать данные и в конце записать сжатые данные в файл-архив. 
Pipes особенно удобно применять для подобного набора операций:</p>
<pre class="brush:js;">
const fs = require("fs");
...
<p>Для архивации подключается модуль <span class="b">zlib</span>. Каждый метод pipe() в цепочке вызовов возвращает поток для чтения, к которому 
опять же можно применить метод pipe() для записи в другой поток.</p>
...
<div class="nav"><p><a href="2.10.php">Назад</a><a href="./">Содержание</a><a href="3.1.php">Вперед</a></p></div></div>
<div class="socBlock">
