...
<html >
<head>
<title>Node.JS | Mongoose</title>
<meta charset="utf-8" />
<meta name="description" content="Использование Mongoose для работы с MongoDB в Node.js">
...
<h2>Mongoose</h2><div class="date">Последнее обновление: 11.08.2021</div>
...
<p>Mongoose представляет  специальную ODM-библиотеку (Object Data Modelling) для работы с MongoDB, которая позволяет сопоставлять объекты классов и документы коллекций из базы данных. 
Грубо говоря, Mongoose работает подобно инструментам ORM. Официальный сайт библиотеки, где можно посмотреть всю необходимую документацию: <a href="http://mongoosejs.com" rel="nofollow">http://mongoosejs.com</a></p>
...
<pre class="brush:js;">const mongoose = require("mongoose");</pre>
<p>Данные, которые используются в Mongoose, описываются определенной схемой. Например, в прошлых темах мы сохраняли в базу данных объекты с двумя 
свойствами <code>name</code> и <code>age</code>. Поэтому описываем здесь следующую схему:</p>
<pre class="brush:js;">
const Schema = mongoose.Schema;
...
<p>Схема содержит метаданные объектов. В частности, здесь устанавливаем, какие свойства будет иметь объект и какой у них будет тип данных. То есть это схема, которая 
описывает объект пользователя.</p>
<p>Затем, используя эту схему, создаем модель пользователя:</p>
<pre class="brush:js;">
const User = mongoose.model("User", userScheme);
</pre>
<p>Первый параметр в методе <span class="b">mongoose.model</span> указывает на название модели. Mongoose затем будет автоматически искать в 
...
"users". Если такая коллекция есть в бд, то с ней будет идти взаимодействие. Если такой коллекции в базе данных нет, то она будет создана автоматически.</p> 
<p>Второй параметр функции <code>mongoose.model</code> - собственно схема.</p>
...
<p>Для подключения к базе данных применяется метод <span class="b">mongoose.connect()</span>, в который передается адрес базы данных на сервере mongo:</p>
<pre class="brush:js;">mongoose.connect("mongodb://localhost:27017/usersdb", {useUnifiedTopology: true, useNewUrlParser: true});</pre>
<p>Второго параметра передается объект, который, во-первых, устанавливает топологию для подключения к MongoDB (<code>useUnifiedTopology: true</code>), а во-вторых, 
устанавливает использование нового парсера (<code>useNewUrlParser: true</code>) для парсинга адреса сервера MongoDB.</p>
<p>И в конце у объекта вызывается метод <span class="b">save</span>. Этот метод определен для всех создаваемых моделей, он сохраняет текущий объект в базу данных:</p>
<pre class="brush:js;">
user.save(function(err){
...
<p>С помощью метода <span class="b">mongoose.disconnect();</span> происходит отключение от бд.</p>
...
<p>С помощью метода <span class="b">then</span> мы можем получить данные, которые возвратил нам сервер и выполнить обработку результата.</p>
<p>Запустим приложение и выполним добавление объекта:</p>
<img src="./pics/7.10.png" alt="Mongoose в Node.js" />
