...
<html >
<head>
<title>Node.JS | Работа с файлами</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с файлами в Node.js, чтение и запись файлов, функции readFile, writeFile, appendFile">
...
<h2>Работа с файлами</h2><div class="date">Последнее обновление: 23.05.2019</div>
...
<p>Для работы с файлами в Node.js предназначен модуль <a href="https://nodejs.org/api/fs.html" rel="nofollow">fs</a>. Рассмотрим, как с ним работать.</p>
<h3>Чтение из файла</h3>
<p>Допустим, в одной папке с файлом приложения app.js расположен текстовый файл <span class="b">hello.txt</span> с простейшим текстом, например:</p>
<pre class="brush:js;">Hello Node JS!</pre>
<p>Для чтения файла в синхронном варианте применяется функция <span class="b">fs.readFileSync()</span>:</p>
<pre class="brush:js;">let fileContent = fs.readFileSync("hello.txt", "utf8");</pre>
<p>В метод передается путь к файлу относительно файла приложения app.js, а в качестве второго параметра указывается кодировка для получения текстового содержимого файла. 
На выходе получаем считанный текст.</p>
<p>Для асинхронного чтения файла применяется функция <span class="b">fs.readFile</span>:</p>
<pre class="brush:js;">
fs.readFile("hello.txt", "utf8", function(error,data){ });
</pre>
<p>Первый и второй параметр функции опять же соответственно путь к файлу и кодировка. А в качестве третьего параметра передается функция обратного вызова, 
которая выполняется после завершения чтения. Первый параметр этой функции хранит информацию об ошибке при наличии, а второй - собственно считанные данные.</p>
...
<img src="pics/2.17.png" alt="Чтение файла в Node.js" />
<p>И здесь стоит обратить внимание, что несмотря на то, что функция <code>fs.readFile()</code> вызывается первой, но так как она асинхронная, она не блокирует 
поток выполнения, поэтому ее результат выводится в самом конце.</p>
<h3>Запись файла</h3>
<p>Для записи файла в синхронном варианте используется функция <span class="b">fs.writeFileSync()</span>, которая в качестве параметра принимает путь к файлу 
и записываемые данные:</p>
<pre class="brush:js;">fs.writeFileSync("hello.txt", "Привет ми ми ми!")</pre>
<p>Также для записи файла можно использовать асинхронную функцию <span class="b">fs.writeFile()</span>, которая принимает те же параметры:</p>
<pre class="brush:js;">fs.writeFile("hello.txt", "Привет МИГ-29!")</pre>
...
<img src="pics/2.18.png" alt="Запись файла в Node.js" />
<p>Следует отметить, что эти методы полностью перезаписывают файл. Если надо дозаписать файл, то применяются методы <span class="b">fs.appendFile()/fs.appendFileSync()</span>:</p>
<pre class="brush:js;">
const fs = require("fs");
...
<img src="pics/2.19.png" alt="Дозапись в файл в Node.js" />
<h3>Удаление файла</h3>
<p>Для удаления файла в синхронном варианте используется функция <span class="b">fs.unlinkSync()</span>, которая в качестве параметра принимает путь к удаляемому файлу:</p>
<pre class="brush:js;">fs.unlinkSync("hello.txt")</pre>
<p>Также для удаления файла можно использовать асинхронную функцию <span class="b">fs.unlink()</span>, которая принимает путь к файлу и функцию, вызываемую при завершении удаления:</p>
<pre class="brush:js;">
