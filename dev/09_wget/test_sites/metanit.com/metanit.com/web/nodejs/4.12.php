...
<html >
<head>
<title>Node.JS | Отправка ответа в Express</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка ответа в Express и Node.js, методы send, sendFile, sendStatus, отправка файлов и статусных кодов">
...
<h2>Отправка ответа</h2><div class="date">Последнее обновление: 21.11.2018</div>
...
<p>Для отправки ответа в express у объекта response можно использовать ряд функций. Самый распространенный способ отправки ответа представляет 
функция <span class="b">send()</span>. В качестве параметра эта функция может принимать объект Buffer, строку, в том числе с html-кодом, объект javascript или массив.</p>
<p></p>
<pre class="brush:js;">
...
<p>Отправка объекта:</p>
<pre class="brush:js;">response.send({id:6, name: "Tom"});</pre>
<p>Отправка массива:</p>
<pre class="brush:js;">response.send(["Tom", "Bob", "Sam"]);</pre>
<p>Отправка объекта:</p>
<pre class="brush:js;">response.send(Buffer.from("Hello Express"));</pre>
<p>Объект Buffer формально представляет некоторые бинарные данные. Так, в случае выше при выполнении кода по умолчанию браузер загрузит файл, 
в котором будет строка "Hello Express".</p>
<h3>sendFile</h3>
<p>Метод send удобен для отправки строк, некоторого кода html небольшой длины, однако есть отправляемый код html довольно большой, то соответственно 
код приложения тоже становится громоздким. Например, мы можем написать так:</p>
<pre class="brush:js;">
const express = require("express");
const app = express();
...
<img src="pics/4.28.png" alt="Отправка кода html в Express в Node.js" />
<p>Однако гораздо лучше определять код html в отдельных файлах и затем эти файлы отправлять с помощью функции <span class="b">sendFile()</span>.</p>
<p>Например, определим в папке проекта новый файл <span class="b">index.html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
  &lt;head&gt;
...
<p>Отправим этот файл с помощью функции sendFile:</p>
<pre class="brush:js;">
const express = require("express");
const app = express();
...
<p>Следует учитывать, что в функцию sendFile необходимо передавать абсолютный путь к файлу, именно для этого с помощью <code>__dirname</code> получаем абсолютный путь 
к текущему проекту и затем добавляем к нему путь к файлу в рамках текущего проекта.</p>
<h3>Отправка статусных кодов</h3>
<p>Функция <span class="b">sendStatus()</span> отправляет пользователю определенный статусный код с некоторым сообщением по умолчанию. 
Например, отправим статусный код 404, который говорит, что ресурс не найден:</p>
<pre class="brush:js;">
const express = require("express");
const app = express();
...
<img src="pics/4.29.png" alt="sendStatus в Express в Node.js" />
<p>Как видно из скриншота, при отправке статусного кода 404 также отправляется сообщение "Not Found". Но, возможно, мы захотим отправлять какие-то свои более информативные 
сообщения. В этом случае можно использовать комбинацию функции <span class="b">status()</span>, которая также отправляет статусный код, и функции 
<span class="b">send()</span>:</p>
<pre class="brush:js;">
const express = require("express");
const app = express();
