...
<html >
<head>
<title>Node.JS | Обновление документов в MongoDB</title>
<meta charset="utf-8" />
<meta name="description" content="Обновление документов в MongoDB в Node.js, методы findOneAndUpdate, updateOne и updateMany, изменение одного и нескольких документов">
...
<h2>Обновление документов в MongoDB</h2><div class="date">Последнее обновление: 11.08.2021</div>
...
<ul>
<li><p><span class="b">updateOne</span>: обновляет один документ, который соответствует критерию фильтрации, и возвращает информацию об операции обновления</p></li>
<li><p><span class="b">updateMany</span>: обновляет все документы, которые соответствуют критерию фильтрации, и возвращает информацию об операции обновления</p></li>
<li><p><span class="b">findOneAndUpdate</span>: обновляет один документ, который соответствует критерию фильтрации, и возвращает обновленный документ</p></li>
</ul>
<h3>findOneAndUpdate</h3>
<p>Метод <span class="b">findOneAndUpdate()</span> обновляет один элемент. Он принимает следующие параметры:</p>
<ol>
<li><p>Критерий фильтрации документа, который надо обновить</p></li>
<li><p>Параметр обновления</p></li>
<li><p>Дополнительные опции обновления, которые по умолчанию имеют значение null</p></li>
<li><p>Функция обратного вызова, которая выполняется при обновлении</p></li>
</ol>
...
<p>Для обновления применяется объект <span class="b">{ $set: {age: 25}}</span>. Параметр <span class="b">$set</span> позволяет обновить значения для одного поля или группы полей. В данном случае 
изменяется поле age.</p>
<p>Третий параметр - функция обратного вызова выводит результат обновления. По умолчанию это старое состояние измененного документа:</p>
<div class="console">
...
<h3>Получение нового состояния обновленного документа</h3>
<p>Но, допустим, после обновления мы хотим получать не старое, а новое состояние измененного документа. Для этого мы можем задать 
дополнительну опцию <code>returnDocument: "after"</code>, которая указывает, что надо получить новое состояние документа. о умолчанию этот параметр имеет значение <code>before</code> (то есть возвращает 
состояние документа ДО обновления):</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<h3>updateMany</h3>
<p>Метод <span class="b">updateMany()</span> позволяет обновить все документы из коллекции, которые соответствуют критерию фильтрации:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<p>Аналогичный пример с использованием <code>async/await</code>:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<h3>updateOne</h3>
<p>Метод <span class="b">updateOne()</span> аналогичен методу updateMany за тем исключением, что обновляет только один элемент. В отличие от метода 
<code>findOneAndUpdate()</code> он не возвращает измененный документ:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
