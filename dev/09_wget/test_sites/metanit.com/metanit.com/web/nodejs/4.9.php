...
<html >
<head>
<title>Node.JS | Хелперы Handlebars</title>
<meta charset="utf-8" />
<meta name="description" content="Helpers в Handlebars в Express и Node.js">
...
<h2>Хелперы в Handlebars</h2><div class="date">Последнее обновление: 15.02.2022</div>
...
<p>Хелперы фактически представляют функции, которые возвращают некоторую строку. После получения эту строку можно добавить в любое место представления. Строка может представлять 
собой в том числе и код html.</p>
<p>Хелперы позволяют оптимизировать создание кода представлений. В частности, мы можем один раз определить функцию хелпера, а затем многократно применять ее 
в самых различных местах для генерации кода.</p>
...
<p>Хелпер определяется с помощью функции <span class="b">hbs.registerHelper()</span>. Первый параметр функции - название хелпера, а второй - функция, которая возвращает строку. 
В данном случае возвращается текущее время.</p>
<p>Далее определим представление <span class="ii">home.hbs</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/4.9.png" alt="Helper in Express and Node.js" />
<p>Хелпер может возвращать не просто строку, но и код html. Кроме того, хелперу можно передавать параметры, которые применяются при генерации результата. 
Например, определим в app.js еще один хелпер:</p>
<pre class="brush:js;">
const express = require("express");
const hbs = require("hbs");
...
<p>Здесь добавлено определение хелпера <code>createStringList()</code>, который в качестве параметра принимает некоторый массив строк и из них создает элемент "&lt;ul&gt;". 
Однако чтобы возвращаемое значение расценивалось именно как html, его надо обернуть в функцию <span class="b">hbs.SafeString()</span>.</p>
...
<img src="./pics/4.10.png" alt="Создание хелперов в Express и Node.js" />
...
<div class="nav"><p><a href="./4.19.php">Назад</a><a href="./">Содержание</a><a href="./4.10.php">Вперед</a></p></div></div>
<div class="socBlock">
