...
<html >
<head>
<title>Node.JS | Работа с модулями</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с модулями в Node.js, кэширование модулей, структура проекта">
...
<h2>Работа с модулями</h2><div class="date">Последнее обновление: 15.11.2018</div>
...
<p>Рассмотрим некоторые аспекты работы с модулями в Node.js. Прежде всего надо отметить, что подключаемые модули кэшируются. В частности, в файле 
<a href="https://github.com/nodejs/node/blob/master/lib/internal/modules/cjs/loader.js" rel="nofollow">https://github.com/nodejs/node/blob/master/lib/internal/modules/cjs/loader.js</a> есть такие строки:</p>
<pre class="brush:js;">
...
<p>Это, с одной стороны, увеличивает производительность, а с другой, может создать некоторые проблемы, если мы не будем учитывать этот 
аспект. Например, возьмем проект из прошлой темы, где в главный файл приложения <span class="b">app.js</span> подключается модуль 
<span class="b">greeting.js</span>. Изменим файл <span class="b">greeting.js</span> следующим образом:</p>
<pre class="brush:js;">module.exports.name = "Alice";</pre>
<p>В файле определена только одна строка, которая устанавливает свойство name.</p>
<p>Изменим код файла <span class="b">app.js</span>:</p>
<pre class="brush:js;">
...
<img src="pics/2.9.png" alt="Подключение нескольких модулей в Node.js" />
<h3>Структура модулей</h3>
<p>Нередко модули приложения образуют какие-то отдельные наборы или области. Такие наборы модулей лучше помещать в отдельные каталоги. Например, создадим в каталоге приложения подкаталог 
<span class="b">welcome</span> и создадим в нем три новых файла:</p>
<ul>
<li><p><span class="b">index.js</span></p></li>
<li><p><span class="b">morning.js</span></p></li>
<li><p><span class="b">evening.js</span></p></li>
</ul>
...
<ul style="list-style-type:none;">
	<li>
		...
		<ul style="list-style-type:none;">
			<li><p>index.js</p></li>
			<li><p>morning.js</p></li>
			<li><p>evening.js</p></li>
		</ul>
	</li>
	<li>
		<p>app.js</p>
...
<p>В файл <span class="b">morning.js</span> поместим следующую строку:</p>
<pre class="brush:js;">module.exports = "Доброе утро";</pre>
<p>Аналогично изменим файл <span class="b">evening.js</span>:</p>
<pre class="brush:js;">module.exports = "Добрый вечер";</pre>
<p>Эти два файла определяют сообщения приветствия в зависимости от времени суток.</p>
<p>И определим в файле <span class="b">index.js</span> следующий код:</p>
<pre class="brush:js;">
...
<p>Теперь используем этот модуль в файле <span class="b">app.js</span>:</p>
<pre class="brush:js;">
...
<p>Несмотря на то, что нет такого файла как welcome.js, но если в проекте есть каталог, который содержит файл с именем <span class="ii">index.js</span>, то мы 
можем обращаться к модулю по имени каталога, как в данном случае.</p>
<p>Запустим приложение, и на консоль будут выведены оба приветствия:</p>
<img src="pics/2.10.png" alt="index.js в node.js" />
