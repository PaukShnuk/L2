...
<html >
<head>
<title>Node.JS | Основные операции с данными MySQL</title>
<meta charset="utf-8" />
<meta name="description" content="Основные операции с данными MySQL в Node.js с помощью драйвера mysql2, создание базы данных и таблиц, добавление, получение, удаление и редактирование данных">
...
<h2>Основные операции с данными</h2><div class="date">Последнее обновление: 28.04.2019</div>
...
<p>Все операции с данными опираются на команды языка SQL, про которые подробно можно прочитать в соответствующем 
<a href="https://metanit.com/sql/mysql/">руководстве</a>. Тем не менее рассмотрим вкратце базовые операции с бд.</p>
<h3>Создание базы данных</h3>
<p>Создадим базу данных на сервере MySQL через Node.js:</p>
<pre class="brush:js;">
...
<p>В данном случае посредство команды <code>CREATE DATABASE</code> создается база данных usersdb2.</p>
<h3>Создание таблиц</h3>
<p>Теперь добавим в выше созданную базу данныз usersdb2 таблицу:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>Для создания таблицы применяется команда <code>CREATE TABLE</code>, которая создается таблицу users с тремя столбцами - id, name и age.</p>
<h3>Добавление данных</h3>
<p>Для добавления применяется SQL-команда <code>INSERT</code>. Добавим данные в ранее созданную таблицу users:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>В данном случае в таблицу добавляется одна строка, где столбец name имеет значение "Sam", столбец age - значение 31. С помощью параметра results 
в функции обратного вызова мы можем получить результаты операции. Например, в моем случае консольный вызов будет следующим:</p>
<div class="console">
...
<p>В данном случае мы видим, что возвращается объект, где можно выделить ряд свойств. Прежде всего, это <span class="b">affectedRows</span> - количество 
затронутых операцией строк (в данном случае количество добавленных строк) и <span class="b">insertId</span> - идентификатор (значение поля id) добавленной записи. 
Соответственно, если бы нам потребовалось получить id добавленной строки, то мы могли бы написать так:</p>
<pre class="brush:js;">
connection.query(sql, function(err, results) {
    if(err) console.log(err);
...
<h3>Добавление множества значений</h3>
<p>Добавим сразу несколько значений:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>При добавлении множества объектов в sql-запросе после <code>VALUES</code> указывается один вопросительный знак.</code> 
<p>И при успешном добавлении свойство <code>results.affectedRows</code> укажет, то добавлено три строки:</p>
<div class="console">
<pre class="consoletext">
C:\node\mysqlapp> node app.js
ResultSetHeader {
...
<p>Однако в этом случае следует учитывать, что мы не сможем получить id всех добавленных строк.</p>
<h3>Получение данных</h3>
<p>Для получения данных применяется sql-команда <code>SELECT</code>. Например, получим все данные из таблицы users:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>Объект <code>results</code> в функции обратного вызова будет представлять массив полученных из БД данных:</p>
<div class="console">
<pre class="consoletext">
C:\node\mysqlapp> node app.js
...
<p>Соответственно после получения мы сможем работать с этими данными как с обычным массивом объектов. Например, выведем только имя для каждого пользователя 
из базы данных:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>Выполним фильтрацию данных с применением выражения <code>WHERE</code>:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>Здесь запрос фактически будет выглядеть как <code>SELECT * FROM users WHERE name="Tom" AND age=29</code>, и в прицнипе мы могли бы напрямую ввести 
данные в запрос. Однако чтобы избежать sql-инъекций при передаче в запрос данных извне рекомендуется использовать параметризацию.</p>
<h3>Обновление</h3>
<p>Для обновления данных применяется sql-команда <code>UPDATE</code>:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>С помощью свойства <code>affectedRows</code> объекта results мы можем проверить, сколько строк было обновлено.</p>
<h3>Удаление</h3>
<p>Для удаления применяется sql-команда <code>DELETE</code>:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
