...
<html >
<head>
<title>Node.JS | Статические файлы</title>
<meta charset="utf-8" />
<meta name="description" content="Статические файлы в Express и Node.js, функция express.static">
...
<h2>Статические файлы</h2><div class="date">Последнее обновление: 22.11.2018</div>
...
<p>Рассмотрим, как в Node.js + Express отправлять пользователю статические файлы html. Для работы со статическими файлами в Express определен специальный компонент 
<span class="b">express.static()</span>, который указывает на каталог с файлами.</p>
<p>Создадим для статических файлов в проекте каталог <span class="b">public</span>, в который добавим новую html-страницу, которую назовем 
<span class="b">about.html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Чтобы встроить компонент express.static в процесс обработки запроса, вызывается функция <code>app.use()</code>. Эта функция позволяет добавлять различные 
компоненты, которые еще называются middleware, в конвейер обработки запроса:</p>
<pre class="brush:js;">app.use(express.static(__dirname + "/public"));</pre>
<p>Причем данный вызов помещается до всех остальных вызовов функции <code>app.get()</code>.</p>
<p>В саму же функцию <code>express.static()</code> передается путь к папке со статическими файлами. Специальное выражение <span class="b">__dirname</span> 
позволяет получить полный путь к папке.</p>
<p>Запустим приложение на выполнение и обратимся в браузере по пути "http://localhost:3000/about.html":</p>
<img src="pics/4.4.png" alt="Статические файлы html в Express и Node.js" />
<p>Следует отметить, что для обращения к файлу мы указываем только имя файла без названия каталога public. Также не следует путать подобное обращение к файлам 
с действием функции sendFile: в данном случае мы напрямую обращаемся к статическим файлам, а функция sendFile фактически берет содержимое из файла и отсылает его пользователю.</p>
...
<p>Теперь чтобы обратиться к файлу <span class="ii">about.html</span>, необходимо отправить запрос <span class="ii">http://localhost:3000/static/about.html</span>.</p>
...
<div class="nav"><p><a href="4.12.php">Назад</a><a href="./">Содержание</a><a href="4.3.php">Вперед</a></p></div></div>
<div class="socBlock">
