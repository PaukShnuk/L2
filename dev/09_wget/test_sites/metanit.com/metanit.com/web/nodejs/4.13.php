...
<html >
<head>
<title>Node.JS | Переадресация в Express</title>
<meta charset="utf-8" />
<meta name="description" content="Переадресация в Express и Node.js, метод redirect">
...
<h2>Переадресация</h2><div class="date">Последнее обновление: 21.11.2018</div>
...
<p>Для переадресации применяется метод <span class="b">redirect()</span>:</p>
<pre class="brush:js;">redirect([status,] path)</pre>
<p>В качестве параметра <code>path</code> передается путь, на который будет перенаправляться пользователь. Дополнительный параметр <code>status</code> 
задает статусный код переадресации. Если этот параметр не задан, тогда по умолчанию отправляется статусный код 302, который предполагает временную переадресацию.</p>
<p>С помощью данного метода можно выполнять переадресацию как по относительным путям, так и по абсолютным, в том числе на другие домены.</p>
<p>Переадресация по абсолютному пути:</p>
<pre class="brush:js;">
...
<p>В данном случае при обращении по пути "/index" будет идти переадресация на сайт https://metanit.com.</p>
<p>Переадресация по оносительным путям также не очень сложна, но здесь важно учитывать, как именно определяется адрес для редиректа. 
Рассмотрим редирект относительно текущего пути, с которого производится редирект. Например:</p>
<pre class="brush:js;">
const express = require("express");
const app = express();
...
<p>В данном случае будет идти перенаправление с ресурса "/home" на ресурс "/about", то есть, условно говоря, с http://localhost:3000/home на http://localhost:3000/about.</p>
<p>Теперь рассмотрим похожий пример:</p>
<pre class="brush:js;">
const express = require("express");
const app = express();
...
<p>Здесь с ресурса "/home/bar" также идет переадресация на ресурс "about", однако в реальности теперь это будет не http://localhost:3000/about, а http://localhost:3000/home/about. 
То есть мы как-бы поднимаемся на один уровень вверх - с "home/bar" на "home/" и затем к нему добавляется "about".</p>
<p>Если нам необходимо выполнить переадресацию не относительно текущего ресурса, а относительно корневого каталога приложения, то в начале адреса ставится слеш:</p>
<pre class="brush:js;">
const express = require("express");
const app = express();
...
<p>Еще несколько примеров. Переадресация относительно текущего адреса на адрес на том же уровне:</p>
<pre class="brush:js;">
app.use("/home/foo/bar",function (request, response) {
  response.redirect("./about")
});
</pre>
<p>Здесь идет переадресация с http://localhost:3000/home/foo/bar на http://localhost:3000/home/foo/about</p>
<p>Переадресация на адрес, который располагается уровнем выше:</p>
<pre class="brush:js;">
app.use("/home/foo/bar",function (request, response) {
  response.redirect("../about")
});
</pre>
<p>Здесь идет переадресация с http://localhost:3000/home/foo/bar на http://localhost:3000/home/about</p>

<p>Переадресация на уровень выше:</p>
<pre class="brush:js;">
app.use("/home/foo/bar",function (request, response) {
  response.redirect(".")
});
</pre>
<p>Здесь идет переадресация с http://localhost:3000/home/foo/bar на http://localhost:3000/home/foo</p>

<p>Переадресация на два уровня выше:</p>
<pre class="brush:js;">
app.use("/home/foo/bar",function (request, response) {
  response.redirect("..")
});
</pre>
<p>Здесь идет переадресация с http://localhost:3000/home/foo/bar на http://localhost:3000/home</p>
<p>По умолчанию при редиректе передается статусный код 302, который указывает, что ресурс временно доступен по новому адресу. 
Но мы можем указать статусный код 301, чтобы сделать переадресацию постоянной:</p>
<pre class="brush:js;">response.redirect(301, "/about");</pre>
...
<div class="nav"><p><a href="4.3.php">Назад</a><a href="./">Содержание</a><a href="4.14.php">Вперед</a></p></div></div>
<div class="socBlock">
