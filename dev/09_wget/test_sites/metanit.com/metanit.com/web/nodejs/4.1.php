...
<html >
<head>
<title>Node.JS | Начало работы с Express</title>
<meta charset="utf-8" />
<meta name="description" content="Начало работы с фреймворком Express в Node.js, создание веб-приложения">
...
<h1>Express</h1><h2>Начало работы с Express</h2><div class="date">Последнее обновление: 15.02.2022</div>
...
<p>В этой главе мы рассмотрим создание сервера с помощью фреймворка <span class="b">Express</span>. Казалось бы, зачем нам нужен дополнительный фреймворк, 
...
ряд готовых абстракций, которые упрощают создание сервера и серверной логики, в частности, обработка отправленных форм, работа с куками, CORS и т.д.</p>
<p>Исходный код фреймворка можно посмотреть в репозитории на гитхабе по адресу <a href="https://github.com/expressjs/express" rel="nofollow">https://github.com/expressjs/express</a>.</p>
<p>Создадим для проекта новый каталог, который назовем, к примеру, <span class="b">expressapp</span>. Для хранения информации обо всех зависимостях проекта определим в 
этом каталоге новый файл <span class="b">package.json</span>:</p>
<pre class="brush:js;">
{
  "name": "expressapp",
  "version": "1.0.0",
...
<img src="pics/4.1.png" alt="Добавление фреймворка Express в Node.js" />
<p>Создадим в каталоге проекта новый файл <span class="b">app.js</span>:</p>
<pre class="brush:js;">
// подключение express
...
<pre class="brush:js;">const app = express();</pre>
<p>Для обработки запросов в Express определено ряд встроенных функций, и одной из таких является функция <span class="b">app.get()</span>. Она обрабатывает GET-запросы протокола 
...
который будет вызываться, если запрос к серверу соответствует данному маршруту:</p>
<pre class="brush:js;">
app.get("/", function(request, response){
...
<p>Для запуска сервера вызывается метод <code>app.listen()</code>, в который передается номер порта.</p>
<p>Запустим проект и обратимся в браузере по адресу <span class="ii">http://localhost:3000/</span>:</p>
<img src="pics/4.2.png" alt="Запуск сервера Express в Node.js" />
<p>И что важно, Express опирается на систему маршрутов, поэтому все другие запросы, которые не соответствуют корневому маршруту "/", не будут обрабатываться:</p>
<img src="pics/4.3.png" alt="Маршруты Express в Node.js" />
<p>Теперь изменим файл <span class="ii">app.js</span>:</p>
<pre class="brush:js;">
const express = require("express");
...
<img src="pics/4.4.png" alt="Маршрутизация Express в Node.js" />
