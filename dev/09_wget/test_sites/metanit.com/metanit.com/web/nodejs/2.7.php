...
<html >
<head>
<title>Node.JS | Асинхронность</title>
<meta charset="utf-8" />
<meta name="description" content="Асинхронность в Node.js, определение функций обратного вызова (callback) и функция setTimeout">
...
<h2>Асинхронность в Node.js</h2><div class="date">Последнее обновление: 03.01.2017</div>
...
<p>Асинхронность представляет возможность одновременно выполнять сразу несколько задач. Асинхронность играет большую роль в Node.js.</p>
<p>Например, допустим в файле приложения <span class="ii">app.js</span> у нас расположен следующий код:</p>
<pre class="brush:js;">
...
<img src="pics/2.14.png" alt="Синхронность в Node JS" />
<p>Для рассмотрения асинхронности изменим код файла <span class="ii">app.js</span> следующим образом:</p>
<pre class="brush:js;">
...
<p>В начале также определяется функция display, но теперь кроме данных в качестве второго параметра она принимает функцию обратного вызова, которая и обрабатывает данные.</p>
<p>Эта функция callback принимает два параметра - информацию об ошибке и собственно данные. Это общая модель функций обратного вызова, которые передаются в асинхронные методы - 
первым идет параметр, представляющий ошибку, а второй - данные.</p>
...
<p>И последний важный момент - выполнение функции обратного вызова в функции <span class="b">setTimeout()</span>. Это глобальная функция, которая принимает в качестве первого параметра функцию обратного вызова, 
а в качестве второго - промежуток, через который функция обратного вызова будет выполняться. Для нашей задачи вполне подойдет промежуток в 0 миллисекунд.</p>
...
<img src="pics/2.15.png" alt="Асинхронность в Node.js" />
<p>Несмотря на то, что в setTimeout передается промежуток 0, фактическое выполнение функции display завершается после всех остальных функций, которые определены в программе. В итоге 
...
базой данных, выполнение которой может занять продолжительное время. То общее выполнение приложение не блокируется, а идет дальше.</p>
<p>Почему так происходит? Потому что все колбеки или функции обратного вызова в асинхронных функциях (в качестве таковой здесь используется функция setTimeout) помещаются в специальную очередь, и начинают выполняться после того, как все остальные синхронные вызовы в приложении 
завершат свою работу. Собственно поэтому выполнение колбека из функции setTimeout в примере выше происходит после выполнения вызова <code>console.log("Завершение работы программы");</code>. И стоит подчеркнуть, что в очередь колбеков переходит не функция, которая 
передается в display, а функция, которая передается в setTimeout.</p>
...
<img src="pics/2.16.png" alt="Асинхронность в NodeJS" />
