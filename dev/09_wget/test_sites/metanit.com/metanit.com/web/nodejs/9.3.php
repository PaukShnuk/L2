...
<html >
<head>
<title>Node.JS | Запросы к базе данных в Sequelize</title>
<meta charset="utf-8" />
<meta name="description" content="Запросы к базам данных MySQL, MS SQL Server, Postgres в Sequelize в приложении на Node.js">
...
<h2>Запросы к базе данных</h2><div class="date">Последнее обновление: 17.05.2019</div>
...
<p>Рассмотим базовые операции с данными на примере модели из прошлой темы:</p>
<pre class="brush:js;">
...
<p>Для добавления данных в бд у модели вызывается метод <span class="b">create()</span>, в который передается добавляемый объект:</p>
<pre class="brush:js;">
User.create({
    name: "Tom",
    age: 35
...
<p>Как мы видим, свойства передаваемого в метод create объекта соответствуют свойствам модели User за исключением свойства id, которое генерируется базой данных.</p>
<p>После операции добавления мы можем получить добавленный объект, в том числе его id, сгенерированный базой данных:</p>
<pre class="brush:js;">
User.create({
  name: "Bob",
  age: 31
...
<p>Для извлечения всех данных применяется метод <span class="b">findAll()</span>:</p>
<pre class="brush:js;">
User.findAll({raw:true}).then(users=&gt;{
  console.log(users);
}).catch(err=&gt;console.log(err));
</pre>
<p>В данном случае в метод передается необязательный объект <code>{raw:true}</code>, который позволяет получить непосредственно данны из БД в вид объектов 
без дополнительных метаданных. Например, консольный вывод в моем случае выглядел следующим образом:</p>
<div class="console">
...
<h3>Простейшая фильтрация</h3>
<p>Для применения фильтрации при получении данных (а также при обновлении и удалении) применяется оператор <span class="b">where</span>, который указывает на 
критерий фильтрации. Например, выберем из БД всех пользователей, у которых name="Tom":</p>
<pre class="brush:js;">
...
<p>Консольный вывод в данном случае:</p>
<div class="console">
<pre class="consoletext">
[ { id: 1, name: 'Tom', age: 35 }]
</pre>
</div>
<h3>Получение одного объекта</h3>
<p>Нередко необходимо получить лишь один объект из БД. В этом случае мы можем использовать такие методы как <span class="b">findByPk()</span> (получает 
объект по первичному ключу) и <span class="b">findOne()</span> (получает один объект по определенному критерию). Например, получим пользователя с id=2:</p>
<pre class="brush:js;">
User.findByPk(2)
.then(user=&gt;{
...
<p>Или получим одного пользователя с именем Tom:</p>
<pre class="brush:js;">
...
<h3>Обновление</h3>
<p>Для обновления применяется метод <span class="b">update()</span>, в который передается объект с новыми значениями и объект-критерий выборки обновляемых объектов:</p>
<pre class="brush:js;">
User.update({ age: 36 }, {
  where: {
    name: "Bob"
...
<p>В данном случае первый объект указывает, что все обновляемые объекты будут получать для поля age значение 36. Второй объект указывает на критерий выборки - 
обновляются только объекты, у которых name="Bob".</p>
<h3>Удаление</h3>
<p>Для удаления используется метод <span class="b">destroy()</span>, в который передается объект-критерий выборки удаляемых объектов:</p>
<pre class="brush:js;">
User.destroy({
  where: {
    name: "Bob"
...
<p>В данном случае удаляются объекты, у которых name="Bob".</p>
