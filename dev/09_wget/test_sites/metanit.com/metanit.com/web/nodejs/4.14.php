...
<html >
<head>
<title>Node.JS | Передача данных приложению. Параметры строки запроса</title>
<meta charset="utf-8" />
<meta name="description" content="Передача данных приложению в Express и в Node.js, получение параметров из строки запроса">
...
<h2>Передача данных приложению. Параметры строки запроса</h2><div class="date">Последнее обновление: 21.11.2018</div>
...
<p>Одним из способов передачи данных в приложение представляет использование параметров строки запроса. Строка запроса (query) - фактически это часть 
запрошенного адреса, которая идет после знака вопроса. Например, в запросе <code>http://localhost:3000/about?id=3&name=Tome</code> часть 
<code>id=3&name=Tome</code> представляет строку запроса.</p>
<p>Строку запроса образуют параметры. После названия каждого параметра после знака равно (=) идет его значение. Друг от друга параметры отделяются знаком амперсанда. 
Например, в адресе выше использовалось два параметра: параметр "id" имеет значение "3" и параметр "name" имеет значение "Tom".</p>
<p>В express мы можем получить параметра строки запроса через свойство <span class="b">query</span> объекта request, который передается в функцию 
обработки запроса. Например:</p>
<pre class="brush:js;">
...
<p>С помощью выражения <code>request.query</code> мы можем получить все параметры строки запрос в виде объекта javascript, а с помощью выражения 
<code>request.query.название_параметра</code> мы можем обратиться к каждому отдельному параметру.</p>
<img src="pics/4.25.png" alt="Параметры запроса в Express в Node.js" />
<h3>Передача массивов</h3>
<p>Подобным образом мы можем передавать массивы данных:</p>
<pre class="brush:js;">
const express = require("express");
...
<p>В данном случае в приложение будет передаваться параметр <code>name</code>, который представляет массив.</p>
<img src="pics/4.26.png" alt="Передача массивов в Express в Node.js" /> 
<h3>Передача сложных объектов</h3>
<p>Также можно передавать более сложые объекты, которые состоят из множества свойств:</p>
<pre class="brush:js;">
const express = require("express");
...
<p>В данном случае мы получае объект user, который содержит два свойства id и name, например, <code>user: { id: '7', name: 'Tom' } </code>.</p>
<img src="pics/4.27.png" alt="Передача сложных объектов в Express в Node.js" />
<p>При передаче в строке запроса свойства объекта помещаются в квадратные скобки: <code>user[id]</code>.</p>
...
<div class="nav"><p><a href="4.13.php">Назад</a><a href="./">Содержание</a><a href="4.5.php">Вперед</a></p></div></div>
<div class="socBlock">
