...
<html >
<head>
<title>Node.JS | Конвейер обработки запроса и middleware</title>
<meta charset="utf-8" />
<meta name="description" content="Конвейер обработки запроса в Express и Node.js, метод app.use и добавление компонентов middleware">
...
<h2>Конвейер обработки запроса и middleware</h2><div class="date">Последнее обновление: 21.11.2018</div>
...
<p>Когда фреймворк Express получает запрос, этот запрос передается в конвейер обработки. Конвейер состоит из набора компонентов или middleware, которые получают данные запроса и решают, 
как его обрабатывать.</p>
...
<p>Здесь конвейер обработки состоял из вызовов <code>app.get()</code>, которые сравнивали запрошенный адрес с маршрутом, и если между адресом и маршрутом было соответствие, то данный запрос обрабатывался 
методом <code>app.get()</code>.</p>
<p>При необходимости мы можем встроить в конвейер обработки запроса на любом этапе любую функцию middleware. Для этого применяется метод 
<span class="b">app.use()</span>. Так, изменим файл app.js следующим образом:</p>
<pre class="brush:js;">
const express = require("express");
...
<ul>
<li><p><span class="b">request</span>: данные запроса</p></li>
<li><p><span class="b">response</span>: объект для управления ответом</p></li>
<li><p><span class="b">next</span>: следующая в конвейере обработки функция</p></li>
</ul>
<p>Каждая из функций middleware просто выводит на консоль сообщение и в конце вызывает следующую функцию с помощью вызова <code>next()</code>.</p>
<p>При запуске приложения после обращения по адресу "http://localhost:3000/" последовательно отработают все три middleware:</p>
<img src="pics/4.11.png" alt="Middleware в Express и Node.js" />
...
<p>Теперь обработка завершается на Middleware 2, так как в этом методе происходит отправка ответа с помощью <code>response.send()</code>, а вызова 
следующей функции через <code>next()</code>:</p>
<img src="pics/4.12.png" alt="Обработка запроса в Express и Node.js" />
<p>Функции middleware также могут сопоставляться с определенными маршрутами. Например:</p>
<pre class="brush:js;">
const express = require("express");
...
<p>В данном случае вторая функция middleware явно сопоставляется с маршрутом "/about", поэтому она будет обрабатывать только запрос "http://localhost:3000/about". 
Первая функция middleware по прежнему обрабатывает все запросы:</p>
<img src="pics/4.13.png" alt="Middleware in Express" />
<h3>Пример middleware</h3>
<p>Middleware помогают выполнять некоторые задачи, которые должны быть сделаны до отправки ответа. Стандартная задача - логгирование запросов.</p>
...
<div class="nav"><p><a href="4.1.php">Назад</a><a href="./">Содержание</a><a href="4.12.php">Вперед</a></p></div></div>
<div class="socBlock">
