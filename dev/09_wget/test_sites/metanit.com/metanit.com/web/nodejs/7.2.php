...
<html >
<head>
<title>Node.JS | Паттерн MVC. Модели и представления</title>
<meta charset="utf-8" />
<meta name="description" content="Паттерн MVC в Node.js, определение моделей и представлений и их связь с контроллерами">
...
<h2>Модели и представления</h2><div class="date">Последнее обновление: 07.09.2021</div>
...
<p>Продолжим работу с проектом из прошлой темы и добавим в него остальные ключевые компоненты паттерна MVC - модели и представления. Итак, сейчас проект выглядит так</p>
<img src="pics/8.1.png" alt="MVC контроллеры в Node.js" />
<p>Прежде всего, добавим в проект пакет "hbs" для работы с представлениями. Вместо движка Handlebars естественно можно использовать любой другой движок представлений.</p>
<div class="console">
<pre class="consoletext">
npm install --save hbs
</pre>
</div>
<p>Добавим в проект новую папку <span class="b">models</span> и определим в ней новый файл <span class="b">user.js</span>, который будет 
хранить код следующей модели:</p>
<pre class="brush:js;">
const users = [];
...
<p>Модель фактически представляет класс User, который имеет два поля: name и age, которые представляют соответственно имя и возраст пользователя. 
...
либо получить пользователей из массива. Следует обратить внимание, что метод getAll определен как статический, поэтому относится в целом ко всему классу User.</p>
<p>Теперь добавим в проект представления. В данном случае в качесте движка представлений будет использоваться движок Handlebars, но естественно 
можно использовать любой предпочтительный движок представлений.</p>
<p>Итак, определим в проекте папку <span class="b">views</span>. Добавим в нее новый файл <span class="b">users.hbs</span>, который будет выводить список пользователей 
на веб-страницу:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<p>И также добавим в папку views второй файл - <span class="b">create.hbs</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Данное представление содержит форму для добавления нового пользователя.</p>
<p>Итоговый проект в итоге будет выглядеть следующим образом:</p>
<img src="pics/8.4.png" alt="MVC в Express и Node.js" />
<p>Теперь изменим контроллер <span class="b">userController</span>:</p>
<pre class="brush:js;">
...
<p>Контроллер теперь определяет три метода. Метод <code>addUser()</code> возвращает представление create.hbs для добавления нового пользователя.</p>
<p>Метод <code>postUser()</code> получает отправленные данные, создает объект User и вызывает у него метод save, тем самым сохраняя его в 
массив users из файла user.js. После добавления идет переадресация на список пользователей.</p>
<p>Метод <code>getUsers()</code> возвращает представление users.hbs, в которое передает список пользователей с помощью вызова 
метода <code>User.getAll()</code></p>
<p>Теперь изменим файл <span class="b">userRouter.js</span> для определения всех необходимых маршрутов:</p>
<pre class="brush:js;">
...
<p>И изменим главный файл приложения:</p>
<pre class="brush:js;">
const express = require("express");
const app = express();
...
<p>Запустим приложение и обратимся по адресу <span class="ii">localhost:3000/users/create</span> для добавления пользователя.</p>
<img src="pics/8.2.png" alt="Модель в MVC в Node.js" />
<p>И после добавления нас передресует на список пользователей</p>
<img src="pics/8.3.png" alt="Представления в MVC в Node.js" />
