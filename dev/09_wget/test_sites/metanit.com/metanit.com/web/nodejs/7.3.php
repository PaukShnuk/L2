...
<html >
<head>
<title>Node.JS | Паттерн MVC. Связь с базой данных MongoDB</title>
<meta charset="utf-8" />
<meta name="description" content="Паттерн MVC в Node.js, связь моделей и контроллеров с базой данных mongodb">
...
<h2>Связь с базой данных MongoDB</h2><div class="date">Последнее обновление: 07.09.2021</div>
...
<p>Продолжим работу с проектом из прошлой темы и в качестве базы данных вместо массива используем базу данных MongoDB. Для 
взаимодействия с MongoDB будем использовать Mongoose.</p>
<p>Итак, вначале добавим mongoose в проект:</p>
<div class="console">
<pre class="consoletext">
npm install mongoose --save
</pre>
</div>
<p>Проект в прошлой теме выглядел следующим образом:</p>
<img src="pics/8.4.png" alt="MVC в Express и Node.js" />
<p>Теперь нам надо добавить в него связь с mongodb. Вначале изменим в папке <span class="b">models</span> 
файл <span class="b">user.js</span>, где хранится определение используемой модели:</p>
<pre class="brush:js;">
...
<p>Теперь модель User представляет модель mongoose, которая определяется с помощью схемы userSheme. Поскольку модель mongoose уже 
...
как в прошлой теме.</p>
<p>Далее в папке <span class="b">controllers</span> изменим файл <span class="b">userController.js</span>:</p>
<pre class="brush:js;">
...
<p>В методе <code>getUsers()</code> вызывается метод <code>User.find()</code>, который получает данные из базы данных и передает их в представление 
users.hbs.</p>
<p>В методе <code>postUser()</code> из полученных данных создается объект User, и у него вызывается метод <code>save()</code>, который сохраняет 
объект в бд.</p>
<p>Файл <span class="b">userRouter.js</span> из папки <span class="b">routes</span>, который связывает маршруты и контроллер userController, 
остается тем же самым:</p>
<pre class="brush:js;">
...
<p>И в конце изменим главный файл приложения <span class="b">app.js</span>:</p>
<pre class="brush:js;">
const express = require("express");
const mongoose = require("mongoose");
const app = express();
...
<p>Теперь сервер начинает принимать запросы после установки подключения с базой данных MondoDB.</p>
<p>Все остальные файлы остаются без изменений.</p>
<p>Запустим приложение, обратимся по адресу <span class="ii">http://localhost:3000/users/create</span> и добавим одного пользователя:</p>
<img src="pics/8.6.png" alt="MongoDB в MVC в Node.js" />
<p>И после успешного добавления данные попадут в бд, и нас переадресуют на страницу, где мы увидим все ранее добавленные данные:</p>
<img src="pics/8.7.png" alt="Mongoose в MVC в Node.js" />
