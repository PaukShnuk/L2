...
<html >
<head>
<title>Node.JS | Связь один-к-одному в Sequelize</title>
<meta charset="utf-8" />
...
<h2>Связь один-к-одному</h2><div class="date">Последнее обновление: 17.05.2019</div>
...
<p>Отношение один к одному указывает, что одна сущность может владеть другой сущностью в единственном экземпляре. Например, у команды может быть только один тренер. С другой стороны, 
тренер может тренировать одновременно только одну команду.</p>
<p>Для создания подобной связи между моделями применяется метод <span class="b">hasOne()</span>. Например, определим модели тренера и команды:</p>
<pre class="brush:js;">
...
<p>В данном случае модель тренера (Coach) условно считается главной, а модель команды (Team) зависимой (но в данном случае деление на главную и зависимую модель 
достаточно условно). Поэтому метод <code>hasOne()</code> вызывается у модели Coach, и в качестве первого параметра передается модель Team. Хотя в данном случае не имеет значения, какая именно модель является главной или зависимой. 
Второй параметр метода задает конфигурацию связи, в частности, каскадное удаление.</p>
<p>В итоге при выполнении данного когда в MySQL будут созданы следующие таблицы:</p>
<pre class="brush:sql;">
...
<p>Как мы видим, в таблице teams создается дополнительный столбец coachId, через который данная таблица будет связана с таблицей coaches.</p>
<h3>Добавление и получение связанных данных</h3>
<p>Для установки связанных данных применяется метод <span class="b">setНАЗВАНИЕ_МОДЕЛИ()</span>. Например, добавим тренера и его команду:</p>
<pre class="brush:js;">
// добавляем тренера
...
<p>По факту метод <code>setМОДЕЛЬ()</code> будет вызывать SQL-команду UPDATE. То есть к моменту вызова данного метода связываемые сущности уже должны быть в 
базе данных.</p>
<p>Для получения связанных данных применяется метод <span class="b">getНАЗВАНИЕ_МОДЕЛИ()</span>. Например, получим тренера и его команду:</p>
<pre class="brush:js;">
...
<p>В данном случае на консоли мы получим:</p>
<div class="console"><pre class="consoletext">Tom Smith - Real Madrid</pre></div>
<p>Получение всех тренеров с включением связанных данных:</p>
<pre class="brush:js;">
Coach.findAll({
