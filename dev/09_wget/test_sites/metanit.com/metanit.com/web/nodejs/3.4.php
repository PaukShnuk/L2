...
<html >
<head>
<title>Node.JS | Отправка файлов</title>
<meta charset="utf-8" />
<meta name="description" content="Stream (потоки) и отправка файлов в Node.js, функция fs.createReadStream">
...
<h2>Отправка файлов</h2><div class="date">Последнее обновление: 18.05.2019</div>
...
<p>Отправка статических файлов - довольно частая задача в построении и функционировании веб-приложеия. Рассмотрим, как отправлять файлы в 
приложении на Node.js.</p>
<p>Пусть в каталоге поекта у нас будут три файла:</p>
<ul>
<li><p>app.js</p></li>
<li><p>about.html</p></li>
<li><p>index.html</p></li>
</ul>
<p>Наряду с файлом приложения app.js определим два html-файла. В файле <span class="b">index.html</span> определим следующий код:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Аналогично определим код в файле <span class="b">about.html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Наша задача будет заключаться в том, чтобы отправить их содержимое пользователю.</p>
<h3>Первый способ</h3>
<p>Для считывания файла может применяться метод <span class="b">fs.createReadStream()</span>, который считывает файл в поток, и затем с помощью метода <span class="b">pipe()</span> мы можем связать считанные файлы с потоком записи, то есть объектом response. 
Итак, поместим в файл <span class="b">app.js</span> следующий код:</p>
<pre class="brush:js;">
...
<p>Вначале мы получаем запрошенный адрес. Допустим, запрошенный адрес будет соответствовать напрямую пути к файлу на сервере. Затем с помощью асинхронной функции 
<span class="b">fs.access</span> проверяем доступность файла для чтения. Первый параметр функции - путь к файлу. Второй параметр - опция, относительно которой 
проверяется доступ. В данном случае значение <code>fs.constants.R_OK</code> говорит о том, что мы проверяем права на чтение из файла. Третий параметр функции - 
функция обратного вызова, которая получает объект ошибки. Если произошла ошибка (файл не досупен для чтения или вовсе не найден), посылаем статусный код 404.</p>
<p>Для отправки файла применяется цепочка методов</p>
<pre class="brush:js;">fs.createReadStream("some.doc").pipe(response);</pre>
<p>Метод <code>fs.createReadStream("some.doc")</code> создает поток для чтения - объект <span class="b">fs.ReadStream</span>. 
Для получения данных из потока вызывается метод <code>pipe()</code>, в который передается объект интерфейса <span class="b">stream.Writable</span> или поток для записи. А именно 
таким и является объект http.ServerResponse, который реализует этот интерфейс.</p>
<p>Запустим приложение и в браузере обратимся по адресу "http://localhost:3000/index.html":</p>
<img src="pics/3.2.png" alt="index.html в Node.js" />
<p>Аналогично мы можем обратиться по адресу "http://localhost:3000/about.html":</p>
<img src="pics/3.8.png" alt="index.html в Node.js" />
<p>В даном случае отправляются файлы html, но подобным образом мы можем отправлять самые разные файлы. Например, определим в проекте папку <span class="b">public</span>. 
И в ней создадим новый файл <span class="b">styles.css</span> со следующим содержимым:</p>
<pre class="brush:css;">body{ font-family: Verdana; color:rgb(48, 48, 92);}</pre>
<p>Применим эти стили на станице index.html:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>И затем обратимся к index.html:</p>
<img src="./pics/3.1.png" alt="Статические файлы в Node.js" />
<h3>Второй способ</h3>
<p>Второй способ представляет чтение данных с помощью функции <span class="b">fs.readFile()</span> и отправка с помощью метода <span class="b">response.end()</span>:</p>
<pre class="brush:js;">
const http = require("http");
const fs = require("fs");
