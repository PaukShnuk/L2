...
<html >
<head>
<title>Node.JS | Получение данных в MongoDB</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление данных в MongoDB в Node.js, методы find, toArray и findOne, фильтрация данных">
...
<h2>Получение данных в MongoDB</h2><div class="date">Последнее обновление: 11.08.2021</div>
...
<p>Метод find возвращает специальный объект <span class="b">FindCursor</span>, 
и чтобы получить все данные у этого объекта вызывается метод <span class="b">toArray()</span>. В этот метод передается функция обратного вызова со стандартными параметрами: 
err (информация об ошибке при ее наличии) и result (собственно результат выборки).</p>
...
<p>С помощью метода <code>find()</code> мы можем дополнительно отфильтровать извлекаемые документы. Например, нам надо найти всех пользователей, у которых 
имя - Tom:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<p>В качестве параметра в find передается объект, который устанавливает параметры фильтрации. В частности, что свойство name должно быть равно "Tom".</p>
<div class="console">
<pre class="consoletext">
c:\node\mongoapp&gt;node app
[
...
<h3>Фильтрация по нескольким критериям</h3>
...
<h3>Получение одного объекта и метод findOne</h3>
<p>Метод <span class="b">findOne()</span> работает аналогично, только позволяет получить один документ:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<p>И также в методе <code>findOne()</code> можно применять фильтрацию:</p>
<pre class="brush:js;">
...
<p>Применение <code>findOne()</code> с <code>async\await</code>:</p>
<pre class="brush:js;">
const MongoClient = require("mongodb").MongoClient;
...
<div class="nav"><p><a href="./6.2.php">Назад</a><a href="./">Содержание</a><a href="./6.3.php">Вперед</a></p></div></div>
<div class="socBlock">
