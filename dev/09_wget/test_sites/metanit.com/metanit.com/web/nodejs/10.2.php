...
<html >
<head>
<title>Node.JS | Настройка multer для загрузки файлов</title>
<meta charset="utf-8" />
<meta name="description" content="Настройка multer для загрузки файлов на сервер в  веб-приложении Node.js и Express">
...
<h2>Настройка multer</h2><div class="date">Последнее обновление: 22.05.2019</div>
...
<h3>Настройка параметров сохранения файла</h3>
<p>В прошлой теме при загрузке имени файла присваивается некоторое произвольное значение. Тем не менее, возможно, нас такие имена файлов не устраивают, и мы хотим сами 
контроллировать процесс именования файлов. В этом случае мы можем выполнить более точную настройку multer:</p>
<pre class="brush:js;">
...
<p>Для настройки сохранения файлов применяется функция <code>multer.diskStorage()</code>. Она принимает объект с двумя параметрами, каждый из которых представляет функцию:</p>
<ul>
<li><p><code>destination</code>: определяет место для сохранения загруженных файлов - в данном случае папка "uploads".</p></li>
<li><p><code>filename</code>: определяет имя для загруженных файлов - в данном случае это непосредственно имя загруженного файла - <code>file.originalname</code>. 
Хотя здесь при необходимости можно использовать другие методы, например, добавить к имени файла текущую дату:</p>
<pre class="brush:js;">filename: function (req, file, cb) {
    cb(null, file.originalname + "-" + Date.now())
}</pre></li>
</ul>
<p>Оба параметра получаю объект запроса <code>req</code>, из которого при необходимости мы можем получить какие-то другие данные запроса и использовать их при сохранении файла.</p>
<p>Затем необходимо установить параметр <span class="b">storage</span>:</p>
<pre class="brush:js;">app.use(multer({storage:storageConfig}).single("filedata"));</pre>
<h3>Фильтрация файлов</h3>
<p>При сохранении файлов мы можем столкнуться с необходимостью их фильтрации по типу. Возможно, нам нужны только файлы изображений или файлы pdf или какие-то 
другие типы файлов. В этом случае мы можем определить фильтр. Например, будем принимать только файлы изображений:</p>
<pre class="brush:js;">
const express = require("express");
const multer  = require("multer");
...
<p>Фильтр фактически представляет функцию, которая принимает три параметра: <code>req</code> (объект запроса), <code>file</code> (загруженный файл) и 
функция <code>cb()</code>:</p>
<pre class="brush:js;">
const fileFilter = (req, file, cb) =&gt; {
...
<p>С помощью значения <code>file.mimetype</code> мы можем проверить MIME-тип файла. Далее вызывается функция <code>cb()</code>. Если MIME-тип подходит, то есть 
мы хотим сохранить данный файл, то в качестве второго параметра в функцию cb передается <code>true</code>. Если же мы хотим отклонить файл, то передается значение 
<code>false</code>.</p>
<p>Когда выполняется вызов <code>cb(null, false)</code>, то при получении multer не устанавливает значение <code>req.file</code>, то есть фактически оно равно undefined. 
Соответственно эту ситуацию мы можем обработать при получении запроса:</p>
<pre class="brush:js;">
app.post("/upload", function (req, res, next) {
...
<p>И чтобы применить данный фильтр, его необходимо передать в объект multer параметру fileFilter:</p>
<pre class="brush:js;">app.use(multer({storage:storageConfig, fileFilter: fileFilter}).single("filedata"));</pre>
...
<div class="nav"><p><a href="10.1.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
