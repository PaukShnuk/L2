...
<html >
<head>
<title>Node.JS | Загрузка файлов с помощью multer</title>
<meta charset="utf-8" />
<meta name="description" content="Загрузка файлов на сервер с помощью пакета multer в Node.js и Express">
...
<h1>Загрузка файлов</h1><h2>Загрузка файлов с помощью multer</h2><div class="date">Последнее обновление: 22.05.2019</div>
...
<p>Одной из наиболее часто встречаемых задач в Node.js является загрузка файлов на сервер. В Node.js для этого нет встроенных инструментов, 
однако мы можем использовать дополнительные специальные пакеты. Одним из популярных подобных пакетов является <span class="b">multer</span>, который работает поверх 
Express.</p>
<p>Итак, для загрузки файлов создадим новый проект. Установим в него пакеты express и multer:</p>
<div class="console">
<pre class="consoletext">
install express multer
</pre>
</div>
<p>Определим в проекте новый файл <span class="b">index.html</span>, в котором будет располагаться форма для загрузки файла:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь важно отметить, что, чтобы на стороне сервера multer мог подхватить загруженный файл, у формы атрибут <span class="b">enctype</span> должен 
иметь значение <span class="b">multipart/form-data</span>.</p>
<p>В главном файле приложения <span class="b">app.js</span> используем multer для получения отправленного файла:</p>
<pre class="brush:js;">
...
<p>В данном случае multer добавляется в виде компонента middleware. Для конфигурации в функцию multer передается объект, в котором параметр <span class="b">dest</span> 
указывает на путь, по которому будет загружаться файл. В данном случае это папка <code>uploads</code>:</p>
<pre class="brush:js;">multer({dest:"uploads"}).single("filedata")</pre>
<p>Если внутри проекта такой папки нет, то она автоматически будет создана.</p>
<p>Далее вызывается функция <span class="b">single()</span> указывает, что загружаться будет один файл. Собственно на форме в index.html мы имеем поле 
...
называется "filedata", поэтому в функцию <code>single()</code> передается соответствующее значение.</p>
<p>Настроив multer, мы можем при обработке запроса получить файл через <span class="b">req.file</span>.</p>
<p>Для тестирования приложения запустим приложение, обратимся к странице <span class="ii">index.html</span> и загрузим какой-нибудь файл:</p>
<img src="pics/11.1.png" alt="Загрузка файла в Node.js" />
<p>После этого на консоли мы увидим что-то наподобие:</p>
<div class="console">
<pre class="consoletext">
{ fieldname: 'filedata',
...
<p>Здесь мы видим, какие метаданные получает сервер при загрузке файла, которые мы можем получить и как-нибудь использовать по своему усмотрению.</p>
<p>В результате в папке проекта будет создана подкаталог uploads, где мы найдем загруженный файл:</p>
<img src="pics/11.2.png" alt="Upload files in Express and Node.js" />
<p>Также нам необязательно встраивать multer глобально в конвейер обработки запросов в виде компонента middleware. Вместо этого мы можем определить использование 
multer только для отдельных функций обработки запросов. Например:</p>
<pre class="brush:js;">
const express = require("express");
const multer  = require("multer");
...
<p>Здесь в функции <code>app.post("/upload"...</code> второй параметр <code>upload.single("filedata")</code> опять же устанавливает использование multer для 
загрузки одного файла.</p>
...
<div class="nav"><p><a href="7.3.php">Назад</a><a href="./">Содержание</a><a href="10.2.php">Вперед</a></p></div></div>
<div class="socBlock">
