...
<html >
<head>
<title>Node.JS | Частичные представления и layout в Pug</title>
<meta charset="utf-8" />
<meta name="description" content="Частичные представления и layout в Pug в Express и Node.js, директивы include и extends">
...
<h2>Частичные представления и layout в Pug</h2><div class="date">Последнее обновление: 24.11.2018</div>
...
<h3>Частичные представления</h3>
<p>Pug позволяет внедрять одни предствления в другие. Например, определим в проекте в папке views новый файл <span class="b">footer.pug</span>:</p>
<pre class="brush:xml;">
&lt;footer&gt;&lt;p&gt;MyCorp - Copyright © 2017&lt;/p&gt;&lt;/footer&gt;
</pre>
<p>Подключим его в файл <span class="b">contact.pug</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Для подключения частичного представления применяется директива <span class="b">include</span>, после которой идет путь к подключаемому представлению.</p>
<p>Сам файл приложение остается тем же, что и в прошлой теме:</p>
<pre class="brush:js;">
...
<img src="pics/4.31.png" alt="Partials in Pug in Express in Node.js" />
<p>Подобным образом можно определить несколько представлений и подключать их в различные места стандартного представления.</p>
<h3>Layout</h3>
<p>Несколько представлений могут иметь общие блоки интерфейса, например, футер. Что если мы захотим пометь текст в футере? Тогда нам придется менять код во всех 
...
для других представлений.</p>
<p>Например, добавим в папку views новый файл <span class="b">layout.pug</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>С помощью директивы <span class="b">block</span> определяет блок, в который будет вставляться некоторое содержимое. На момент определения этого представления 
мы не знаем, что это будет за содержимое. Оно будет определяться в других представлениях, которые будут использовать данный layout. И в данном случае определяются два блока с именами title и content.</p>
<p>Также, как в и в любом представлении, мы можем подключать другие представления. В частности, здесь подключается футер из footer.pug, который обычно является общим элементом 
для большинства страниц веб-сайта.</p>
<p>Теперь определим представление <span class="b">contact.pug</span>, которое будет использовать данный layout:</p>
<pre class="brush:xml;">
extends layout.pug
...
<p>Для подключения мастер-страницы в представление применяется директива <span class="b">extends</span>, после которой указывается путь к файлу.</p>
<p>Поскольку на мастер-странице определены два блока title и content, то в данном случае мы определяем для каждого из этих блоков содержимое и 
это содержимое затем вставляется на место соответствующих блоков на мастер-сраницу.</p>
...
<div class="nav"><p><a href="4.17.php">Назад</a><a href="./">Содержание</a><a href="4.11.php">Вперед</a></p></div></div>
<div class="socBlock">
