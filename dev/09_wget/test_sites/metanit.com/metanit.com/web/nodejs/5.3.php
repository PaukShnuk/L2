...
<html >
<head>
<title>Node.JS | Тестирование Express</title>
<meta charset="utf-8" />
<meta name="description" content="Тестирование Express и маршрутов в Node.js, библиотека supertest">
...
<h2>Тестирование Express</h2><div class="date">Последнее обновление: 17.01.2017</div>
...
<p>Для тестирования функциональности, которая использует Express, мы можем воспользоваться специальным инструментом - <a href="https://www.npmjs.com/package/supertest" rel="nofollow">supertest</a>.</p>
...
<p>Первым делом определим простенький файл приложения <span class="ii">app.js</span>:</p>
<pre class="brush:js;">
...
<p>И чтобы задействовать данное приложение в тесте, оно оформляется в виде модуля: <code>module.exports.app = app;</code></p>
<p>Далее для тестов создадим в каталоге проекта новый файл <span class="ii">app.test.js</span>:</p>
<pre class="brush:js;">
...
<p>Для тестирования получаем модули supertest и нашего приложения и используем метод <code>it()</code> для получения результата.</p>
<p>Для настройки и выполнения теста в request передается функционал приложения:</p>
<pre class="brush:js;">request(app)</pre>
<p>устанавливаем маршрут, по которому будем обращаться в приложении:</p>
<pre class="brush:js;">get("/")</pre>
<p>Устанавливаем ожидаемый результат через метод <code>expect</code>:</p>
<pre class="brush:js;">expect("Hello Test")</pre>
<p>и с помощью метода <code>end()</code> выполняем тест:</p>
<pre class="brush:js;">end(done)</pre>
<p>Для запуска этого теста у нас опять же должна быть настроена должным образом команда <code>test</code> в файле package.json:</p>
<pre class="brush:js;">
{
  "name": "testapp",
  "version": "1.0.0",
...
<img src="pics/5.6.png" alt="Тестирование Express на Node.js" />
...
<p>Рассмотрим еще пару тестов. Для этого изменим файл <span class="ii">app.js</span> следующим образом:</p>
<pre class="brush:js;">
const express = require("express");
var app = express();
...
<p>Здесь определена обработка для трех маршрутов. Для их тестирования изменим файл <span class="ii">app.test.js</span>:</p>
<pre class="brush:js;">
const request = require("supertest");
...
<p>Если нам надо проверить статусный код, то также можем передать ожидаемый код статуса в метод <code>expect(404)</code></p>
<p>Если необходимо проверить какие-то комплексные объекты, которые отправляются в ответе клиенту, то в метод expect передается функция, в которую в 
качестве параметра передается объект ответа response. А через объект <code>response.body</code> можно получить весь ответ и сравнить его с ожидаемым значением. 
Для сравнения комплексных объектов можно применить метод <code>deepEqual()</code> или <code>deepStrictEqual()</code> библиотеки assert, рассмотренной в прошлой теме.</p>
<p>Запустим тесты и проверим результат:</p>
<img src="pics/5.7.png" alt="Тестирование ответа в Express и Node.js" />
