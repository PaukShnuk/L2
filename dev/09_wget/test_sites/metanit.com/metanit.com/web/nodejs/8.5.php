...
<html >
<head>
<title>Node.JS | Пулы подключений MySQL</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие с базой данных MySQL в Node.js с помощью драйвера mysql2 через пулы подключений, метод createPool">
...
<h2>Пулы подключений</h2><div class="date">Последнее обновление: 28.04.2019</div>
...
<p>Драйвер mysql2 позволяет создавать пулы подключений. Пулы подключений позволяют уменьшить время, затраченное на подключение к серверу MySQL, благодаря повторному использованию подключений. 
...
Это позволяет снизить издержки на создание новых подключений.</p>
<p>Пул подключений создается с помощью функции <span class="b">createPool()</span>. Например, создадим пул из пяти подключений:</p>
<pre class="brush:js;">
...
<p>Параметр <span class="b">connectionLimit</span> определяет максимальное количество подключений. Причем даже если мы определим 100 подключений, но приложениею требуется 
только 5 подключений, то соответственно будет создаваться и использоваться только 5 подключений.</p>
<p>Для закрытия всех подключений применяется метод <span class="b">end()</span>:</p>
<pre class="brush:js;">
...
<h3>Запросы к бд</h3>
<p>Запросы к бд через пул подключений выполняются через метод <span class="b">query()</span> также, как и в объекте connection, который рассматривался в прошлых темах:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>Обратите внимание, что в случае выше запросы выполняются не последовательно. Поскольку оба вызова метода query выполняются асинхронно, 
то соответственно нет никакой гарантии, что вначале произойдет добавление, а потом получение объектов.</p>
<p>То же самое следует учитывать и при вызове метода <span class="b">end()</span> у пула подключений:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>В данном случае вполне может сложиться ситуация, что первым отработает метод <code>pool.end()</code> - до того, как ранее вызванный метод 
<code>pool.query()</code> отправит запрос к базе данных. В итоге при таком сценарии мы получим ошибку.</p>
<h3>Promise API</h3>
<p>Так же, как и объкты подключений, пулы поддерживают работу с промисами:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
...
<p>Благодаря промисам мы можем обойти вышеописанную проблему с вызовом метода end после операции с данными:</p>
<pre class="brush:js;">
const mysql = require("mysql2");
