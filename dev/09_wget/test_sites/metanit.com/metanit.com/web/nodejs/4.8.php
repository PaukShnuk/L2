...
<html >
<head>
<title>Node.JS | Частичные представления</title>
<meta charset="utf-8" />
<meta name="description" content="Partial views в Handlebars в Express и Node.js, встраивание частичных представлений в обычные">
...
<h2>Частичные представления в Handlebars</h2><div class="date">Последнее обновление: 17.08.2020</div>
...
<p>Нередко вeб-страницы в приложении используют какие-то общие элементы. Это может быть меню, шапка сайта, футер, другие элементы. Однако здесь встает проблема: 
...
элемент в одном месте, а затем подключать на все страницы.</p>
<p>Решить эту проблему помогают частичные представления (partial views), которые представляют разделяемые общие элементы, которые можно добавлять на 
обычные представления.</p>
<p>К примеру, мы можем сделать общее меню и общий футер. Для этого создадим для частичных представлений в проекте подкаталог 
<span class="b">views/partials</span>.</p>
<p>Добавим в папку <span class="ii">views/partials</span> новый файл <span class="b">menu.hbs</span>:</p>
<pre class="brush:xml;">
&lt;nav&gt;&lt;a href="/"&gt;Главная&lt;/a&gt; | &lt;a href="/contact"&gt;Контакты&lt;/a&gt;&lt;/nav&gt;
</pre>
<p>Затем также добавим в <span class="ii">views/partials</span> новый файл <span class="b">footer.hbs</span>:</p>
<pre class="brush:xml;">
...
<p>В папке views определим обычное представление <span class="b">contact.hbs</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Для вставки частичного представления применяется выражение <code>{{&gt; menu}}</code>, в котором прописывается имя файла частичного представления без расширения.</p>
<p>Также добавим в папку views новое представление, которое назовем <span class="b">home.hbs</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Таким образом, у нас два представления, которые имеют общие элементы. И если нам потребуется добавить какой-нибудь новый пункт меню, то достаточно изменить файл menu.hbs.</p>
<p>В итоге весь проект будет выглядеть следующим образом:</p>
<ul style="list-style-type:none;">
<li><p>app.js</p><li>
<li><p>node_modules</p></li>
<li><p>views</p>
	<ul style="list-style-type:none;">
		<li><p>contact.hbs</p><li>
		<li><p>home.hbs</p><li>
		<li>
			<p>partials</p>
			<ul style="list-style-type:none;">
				<li><p>footer.hbs</p><li>
				<li><p>menu.hbs</p><li>
			</ul>
		<li>
	</ul>
...
<p>Для настройки функционала частичных представлений в коде используется вызов:</p>
<pre class="brush:js;">hbs.registerPartials(__dirname + "/views/partials");</pre>
<p>который устанавливает каталог, где располагаются частичные представления.</p>
<p>И если мы запустим проект и обратимся по одному из двух маршрутов: "/" или "/contact", то после рендеринга представления веб-страница будет 
содержать меню и футер:</p>
<img src="pics/4.8.png" alt="Partial Views in Express and Node.js" />
...
<div class="nav"><p><a href="4.7.php">Назад</a><a href="./">Содержание</a><a href="4.19.php">Вперед</a></p></div></div>
<div class="socBlock">
