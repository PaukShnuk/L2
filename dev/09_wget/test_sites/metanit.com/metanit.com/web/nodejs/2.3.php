...
<html >
<head>
<title>Node.JS | Объект global и глобальные переменные</title>
<meta charset="utf-8" />
<meta name="description" content="Объект global и глобальные переменные в модулях в Node.js">
...
<h2>Объект global и глобальные переменные</h2><div class="date">Последнее обновление: 15.11.2018</div>
...
<p>Node.js предоставляет специальный объект <span class="b">global</span>, который предоставляет доступ к глобальным, то есть доступным из каждого модуля приложения, переменным и функциям. Примерным аналогом данного объекта 
в javascript для браузера является объект <code>window</code>. Все доступные глобальные объекты можно посмотреть в <a href="https://nodejs.org/api/globals.html" rel="nofollow">документации</a>.</p>
<p>Для примера создадим следующий модуль <span class="b">greeting.js</span>:</p>
<pre class="brush:js;">
...
<p>Здесь, во-первых, происходит установка глобальной переменной <code>date</code>: <code>global.date = currentDate;</code></p>
<p>Во-вторых, в модуле получаем глобальную переменную name, которая будет установлена из вне. При этом обратиться к глобальной переменной name мы можем 
через объект global: <code>global.name</code>, либо просто через имя <code>name</code>, так как переменная глобальная.</p>
<p>Определим следующий файл приложения <span class="b">app.js</span>:</p>
<pre class="brush:js;">
...
<p>Здесь устанавливаем глобальную переменную name, которую мы получаем в модуле greeting.js. И также выводим на консоль глобальную переменную date. 
Причем все глобальные функции и объекты, например, <code>console</code>, также доступны внутри global, поэтому мы можем написать и 
<code>global.console.log()</code>, и просто <code>console.log()</code>.</p>
<p>Запустим файл app.js:</p>
<img src="pics/2.20.png" alt="global и глобальные объекты в Node.js" />
<p>Однако по возможности все таки рекомендуется избегать определения и использования глобальных переменных, и преимущественно ориентироваться на создание переменных, инкапсулированных в рамках отдельных модулей.</p>
