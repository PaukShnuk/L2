...
<html >
<head>
<title>Javascript | Выполнение кода</title>
<meta charset="utf-8" />
<meta name="description" content="Порядок выполнения кода на языке программирования JavaScript. Основы синтаксиса и добавление комментариев">
...
<h2>Выполнение кода javascript</h2><div class="date">Последнее обновление: 01.07.2021</div>
...
<p>Когда браузер получает веб-страницу с кодом html и javascript, то он ее интерпретирует. Результат интерпретации в виде различных элементов - кнопок, 
полей ввода, текстовых блоков и т.д., мы видим перед собой в браузере. Интерпретация веб-страницы происходит последовательно сверху вниз.</p>
<p>Когда браузер встречает на веб-странице элемент <code>&lt;script&gt;</code> с кодом javascript, то вступает в действие встроенный интерпретатор javascript. 
И пока он не закончит свою работу, дальше интерпретация веб-страницы не идет.</p>
<p>Рассмотрим небольшой пример и для этого изменим страницу index.html из прошлой темы следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь три вставки кода javascript - один в секции <code>&lt;head&gt;</code> и по одному после каждого заголовка.</p>
<p>Откроем веб-страницу в браузере и мы увидим, что браузер последовательно выполняет код веб-страницы:</p>
<img src="./pics/1.4.png" alt="Интерпретация кода JavaScript" />
<p>Здесь мы видим, что вначале выполняется код javascript из секции head, который выводит на веб-страницу некоторый текст:</p>
<pre class="brush:js;">document.write("Начальный текст");</pre>
<p>Далее выводится первый стандартный html-элемент <code>&lt;h2&gt;</code>:</p>
<pre class="brush:xml;">&lt;h2&gt;Первый заголовок&lt;/h2&gt;</pre>
<p>После этого выполняется вторая вставка кода на javascript:</p>
<pre class="brush:js;">document.write("Первый текст");</pre>
<p>Затем выводится второй html-элемент <code>&lt;h2&gt;</code>:</p>
<pre class="brush:xml;">&lt;h2&gt;Второй заголовок&lt;/h2&gt;</pre>
<p>И в конце выполняется последняя вставка кода на javascript:</p>
<pre class="brush:js;">document.write("Второй текст");</pre>
<p>После этого браузер закончит интерпретацию веб-страницы, и веб-страница окажется полностью загружена. Данный момент 
очень важен, поскольку может влиять на производительность. Поэтому нередко вставки кода javascript идут перед закрывающим тегом <span class="b">&lt;/body&gt;</span>, 
когда основная часть веб-страницы уже загружена в браузере.</p>
<h3>Основы синтаксиса javascript</h3>
<p>Прежде чем переходить к детальному изучению основ языка программирования javascript, рассмотрим некоторые базовые моменты его синтаксиса.</p>
<p>Код javascript состоит из инструкций. Каждая инструкция представляет некоторое действие. И для отделения инструкций друг от друга в javascript 
после инструкции ставится точка с запятой:</p>
<pre class="brush:js;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь в коде javascript определены три инструкции:</p>
<ol>
<li><pre class="brush:js;">document.write("2 + 5 =  ")</pre>
<p>Выводит на страницу текст "2 + 5 =  "</p></li>
<li><pre class="brush:js;">var sum = 2 + 5;</pre>
<p>С помощью оператора <span class="b">var</span> определяет переменную <span class="b">sum</span>, которая равна сумме 2 + 5</p></li>
<li><pre class="brush:js;">document.write(sum);</pre>
<p>Выводит на страницу значение переменной <span class="b">sum</span> (то есть суму 2 + 5)</p></li>
</ol>
<img src="./pics/1.5.png" alt="Инструкции в JavaScript" />
<p>Однако современные браузеры вполне могут различать отдельные инструкции, если они просто располагаются на отдельных строках без точки запятой:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Но чтобы улучшить читабельность кода и снизить число возможных ошибок, рекомендуется определять каждую инструкцию javascript на отдельной строчке и 
завершать ее точкой с запятой, как в первом варианте.</p>
<h3>Комментарии</h3>
<p>В коде javascript могут использоваться комментарии. Комментарии не обрабатываются интерпретатором javascript и никак не учитываются в работе программы. Они предназначены для ориентации по коду, чтобы указать, 
что делает тот или иной код.</p>
<p>Комментарии могут быть однострочными, для которых используется двойной слеш:</p>
<pre class="brush:js;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Кроме однострочных комментариев могут использоваться и многострочные. Такие комментарии заключаются между символами <code>/*текст комментария*/</code>. 
Например:</p>
<pre class="brush:js;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
