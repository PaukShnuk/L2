...
<html >
<head>
<title>JavaScript | Подключение внешнего файла</title>
<meta charset="utf-8" />
<meta name="description" content="Определение кода JavaScript во внешнем файле и его подключение на веб-страницу">
...
<h2>Подключение внешнего файла JavaScript</h2><div class="date">Последнее обновление: 01.07.2021</div>
...
<p>В прошлой статье код javascript непосредственно определялся на веб-странице. Но есть еще один способ подключения кода JavaScript, который представляет вынесение кода во внешние файлы и их подключение с помощью тега 
<code>&lt;script&gt;</code></p>
<p>Итак, в прошлой теме мы создали html-страницу <span class="b">index.html</span>, которая находится в каталоге <span class="b">app</span>. Теперь создадим в этом каталоге новый подкаталог. Назовем его <span class="b">js</span>. 
...
<span class="b">myscript.js</span>. Файлы с кодом javascript имеют расширение <span class="b">.js</span>. То есть у нас получится 
следующая структура проекта в папке <span class="b">app</span>:</p>
<img src="./pics/1.3.png" alt="Структура программы на JavaScript" />
<p>Откроем файл <span class="b">myscript.js</span> в текстовом редакторе и определим в нем следующий код:</p>
<pre class="brush:js;">
document.write("&lt;h2&gt;Первая программа на JavaScript&lt;/h2&gt;"); // выводим заголовок
document.write("Привет мир!");                             // выводим обычный текст
</pre>
<p>Здесь для добавления на веб-страницу некоторого содержимого применяется метод <code>document.write</code>. Первый вызов метода <code>document.write</code> выводит заголовок 
<code>&lt;h2&gt;</code>, а второй вызов - обычный текст.</p>
<p>Для совместимости с кодировкой страницы index.html для файла с кодом javascript также желательно устанавливать кодировку utf-8. При работе в Visual Studio Code этот 
редактор автоматически устанавливает кодировку UTF-8.</p>
<p>Теперь подключим этот файл на веб-страницу <span class="b">index.html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Чтобы подключить файл с кодом javascript на веб-страницу, применяется также тег <span class="b">&lt;script&gt;</span>, у которого устанавливается 
атрибут <span class="b">src</span>. Этот атрибут указывает на путь к файлу скрипта. В нашем случае используется относительный путь. 
Так как веб-страница находится в одной папке с каталогом js, то в качестве пути мы можем написать <span class="b">js/myscript.js</span>.</p>
<p>Также надо учитывать, что за открывающим тегом script должен обязательно следовать закрывающий <code>&lt;/script&gt;</code></p>
<p>И после открытия файла index.html в браузере отобразится сообщение:</p>
<img src="./pics/1.7.png" alt="Подключение внешних скриптов javascript" />
<p>В отличие от определения кода javascript вынесение его во внешние файлы имеет ряд преимуществ:</p>
<ul>
<li><p>Мы можем повторно использовать один и тот же код на нескольких веб-страницах</p></li>
<li><p>Внешние файлы javascript бразуер может кэшировать, за счет этого при следующем обращении к странице браузер снижает нагрузка на сервер, а браузеру надо загрузить меньший объем информации</p></li>
<li><p>Код веб-страницы становится "чище". Он содержит только html-разметку, а код поведения хранится во внешних файлах. В итоге можно отделить работу 
по созданию кода html-страницы от написания кода javascript</p></li>
</ul>
<p>Поэтому, как правило, предпочтительнее использовать код javascript во внешних файлах, а не в прямых вставках на веб-страницу с помощью элемента script.</p>
