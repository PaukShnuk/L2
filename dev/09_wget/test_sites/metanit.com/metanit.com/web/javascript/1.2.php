...
<html >
<head>
<title>JavaScript | Первая программа</title>
<meta charset="utf-8" />
<meta name="description" content="Создание первой программы на языке программирования JavaScript">
...
<h2>Первая программа на JavaScript</h2><div class="date">Последнее обновление: 01.0.2021</div>
...
<p>Создадим первую программу на javascript. Для написания и тестирования программ на JavaScript нам потребуются две вещи: тестовый редактор и веб-браузер.</p>
<p>В качестве текстового редактора можно взять любой, который нравится - Atom, Sublime Text, Visual Studio Code, Notepad++ и другие. В данном руководстве я буду ориентироваться 
на текстовый редактор <a href="https://code.visualstudio.com/download" rel="nofollow">Visual Studio Code</a>, поскольку он является наиболее популярным.</p>
<p>В качестве браузера также можно взять последние версии любого предпочтительного веб-браузера. В настоящем руководстве я буду преимущественно ориентироваться на 
Google Chrome.</p>
<p>Для начала определим для нашего приложения какой-нибудь каталог. Например, создадим на диске C папку <span class="b">app</span>. 
В этой папке создадим файл под названием <span class="b">index.html</span>. То есть данный файл будет представлять веб-страницу с кодом HTML.</p>
<img src="./pics/1.12.png" alt="Первый скрипт на JavaScript">
<p>Откроем этот файл в текстовом редакторе и определим в файле следующий код:</p>
<pre class="brush:js;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь мы определяем стандартные элементы html. В элементе <span class="b">head</span> определяется кодировка utf-8 и заголовок (элемент <code>title</code>). 
В элементе <span class="b">body</span> определяется тело веб-страницы, которое в данном случае состоит только из одного элемента 
<span class="b">&lt;script&gt;</span></p>
<p>Подключение кода javascript на html-страницу осуществляется с помощью тега <span class="b">&lt;script&gt;</span>. Данный тег следует размещать либо в 
заголовке (между тегами <code>&lt;head&gt;</code> и <code>&lt;/head&gt;</code>), либо в теле веб-странице (между тегами <code>&lt;body&gt;</code> 
и <code>&lt;/body&gt;</code>). Нередко подключение скриптов происходит перед закрывающим тегом <code>&lt;/body&gt;</code> для оптимизации загрузки веб-страницы.</p>
<p>Раньше надо было в теге <code>&lt;script&gt;</code> указывать тип скрипта, так как данный тег может использоваться не только 
для подключения инструкций javascript, но и для других целей. Так, даже сейчас вы можете встретить на некоторых веб-страницах такое определение элемента script:</p>
<pre class="brush:xml;">
&lt;script type="text/javascript"&gt;
</pre>
<p>Но в настоящее время предпочтительнее опускать атрибут type, так как браузеры по умолчанию считают, что элемент script содержит инструкции javascript.</p>
<p>Используемый нами код javascript содержит одно выражение:</p>
<pre class="brush:js;">
document.write("&lt;h2&gt;Первая программа на JavaScript&lt;/h2&gt;");
</pre>
<p>Код javascript может содержать множество инструкций и каждая инструкция завершается точкой с запятой. Наша инструкция вызывает метод 
<span class="b">document.write()</span>, который выводит на веб-страницу некоторое содержимое, в данном случае это заголовок <code>&lt;h2&gt;Первая программа на JavaScript&lt;/h2&gt;</code>.</p>
<p>Вид файла в текстовом редакторе Visual Studio Code:</p>
<img src="./pics/1.2.png" alt="Первая программа на JavaScript в Visual Studio Code">
<p>Теперь, когда веб-страница готова, откроем ее в веб-браузере:</p>
<img src="./pics/1.1.png" alt="Первая программа на JavaScript" />
<p>И веб-браузер отобразит заголовок, который мы передали в метод <code>document.write()</code> в коде javascript.</p>
