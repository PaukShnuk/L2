...
<html >
<head>
<title>CSS3 | Селекторы атрибутов</title>
<meta charset="utf-8" />
<meta name="description" content="Селекторы атрибутов в CSS 3, их типы и использование">
...
<h2>Селекторы атрибутов</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Кроме селекторов элементов мы также можем использовать селекторы их атрибутов. Например, у нас есть на веб-странице несколько полей input, а нам 
надо окрасить в красный цвет только текстовые поля. В этом случае мы как раз можем проверять значение атрибута <code>type</code>: если оно имеет 
значение <code>text</code>, то это текстовое поле, и соответственно его надо окрасить в красный цвет. Определение стиля в этом случае выглядело бы так:</p>
<pre class="brush:css;">
input[type="text"]{
...
<p>После элемента в квадратных скобках идет атрибут и его значение. То есть в данном случае мы говорим, что для текстового поля надо установить 
границу красного цвета 2 пикселя толщиной сплошной линией.</p>
<p>Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<img src="pics/4.11.png" alt="Селекторы атрибутов в CSS3" />
<p>Селекторы атрибутов можно применять не только к элементам, но и классам и идентификаторам. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/4.12.png" alt="Селекторы атрибутов в CSS" />
<p>Специальные символы позволяют конкретизировать значение атрибутов. Например символ ^ позволяет выбрать все атрибуты, которые начинаются на определенный текст. 
Например, нам надо выбрать все ссылки, которые используют протокол https, то есть ссылка должна начинаться на "https://". В этом случае можно применить 
следующий селектор:</p>
<pre class="brush:css;">
a[href^="https://"]{
...
<p>Если значение атрибута должно иметь в конце определенный текст, то для проверки используется символ $. Например, нам надо выбрать все изображения 
в формате jpg. В этом случае мы можем проверить, оканчивается ли значение атрибута <code>src</code> на текст ".jpg":</p>
<pre class="brush:css;">
img[src$=".jpg"]{
...
<p>И еще один символ "*" (звездочка) позволяет выбрать все элементы с атрибутами, которые в своем значении имеют определенный текст (не важно где - в начале, середине или конце):</p>
<pre class="brush:css;">
a[href*="microsoft"]{
...
<p>Данный атрибут выберет все ссылки, которые в своем адресе имеют текст "microsoft".</p>
...
<div class="nav"><p><a href="5.6.php">Назад</a><a href="./">Содержание</a><a href="5.9.php">Вперед</a></p></div></div>
<div class="socBlock">
