...
<html >
<head>
<title>HTML5 | flex-wrap</title>
<meta charset="utf-8" />
<meta name="description" content="Свойство flex-wrap во Flexbox в HTML5 и CSS3, перенос элементов на новую строку или столбец, обратный порядок элементов">
...
<h2>flex-wrap</h2><div class="date">Последнее обновление: 18.04.2017</div>
...
<p>Свойство <span class="b">flex-wrap</span> определяет, будет ли flex-контейнер несколько рядов элементов (строк или столбцов) в случае если его размеры недостаточны, чтобы вместить в один ряд все элементы. 
Это свойство может принимать следующие значения:</p>
<ul>
<li><p><span class="b">nowrap</span>: значение по умолчанию, которое определяет flex-контейнер, где все элементы раполагаются в одну строку (при расположении в виде строк) или один столбец (при расположении в столбик)</p></li>
<li><p><span class="b">wrap</span>: если элементы не помещаются во flex-контейнер, то создает дополнительные ряды в контейнере для размещения элементов. При расположении в  виде строки 
содаются дополнительные строки, а при расположении в виде столбца добавляются дополнительные столбцы</p></li>
<li><p><span class="b">wrap-reverse</span>: то же самое, что и значение wrap, только элементы располагаются в обратном порядке</p></li>
</ul>
<p>Например, возьмем значение по умолчанию <code>nowrap</code>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Здесь в каждом из flex-контейнеров по пять элементов, однако ширина контейнера может вмещать не все элементы, тогда они уходят за границу контейнера:</p>
<img src="pics/flexbox5.png" alt="flex-wrap nowrap в Flexbox и CSS3" />

<p>При установке значения <code>wrap</code> во flex-контейнере добавляются дополнительные ряды для помещения всех элементов в контейнере. Так, изменим значение 
свойства flex-wrap в контейнере:</p>
<pre class="brush:xml;">
.flex-container {
	display: flex;
	border: solid 0.25em #000;
...
<p>В этом случае появится дополнительная стока:</p>
<img src="pics/flexbox6.png" alt="flex-wrap в Flexbox и CSS3" />
<p>При расположении в виде столбца контейнер будет создавать дополнительные столбцы:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/flexbox7.png" alt="column flex-wrap в Flexbox и CSS3" />
