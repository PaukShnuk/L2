...
<html >
<head>
<title>CSS3 | Трансформации</title>
<meta charset="utf-8" />
<meta name="description" content="Трансформации элементов веб-страницы в HTML 5 и CSS, свойство transform, вращение, перемещение, наклон и масштабирование элементов">
...
<h1>Трансформации, переходы и анимации</h1><h2>Трансформации</h2><div class="date">Последнее обновление: 01.05.2016</div>
...
<p>Одним из нововведений CSS3 по сравнению с предыдущей версией является встроенная возможность трансформаций элемента. 
...
Для создания трансформаций в CSS3 применяется свойство <span class="b">transform</span>.</p>
<h3>Вращение</h3>
<p>Для поворота элемента свойство <code>transform</code> использует функцию <span class="b">rotate</span>:</p>
<pre class="brush:css;">
transform: rotate(угол_поворота deg);
</pre>
<p>После слова <span class="b">rotate</span> в скобках идет величина угла поворота в градусах. Например, повернем блок на 30 градусов:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/8.1.png" alt="Вращение в CSS3" />
<p>При этом можно отметить, что при повороте вращаемый элемент может накладываться на соседние элементы, так как сначала происходит установка положения 
элементов и только затем поворот.</p>
<p>Угол поворота может представлять как положительное, так и отрицательное значение. В случие отрицательного значения поворот производится в противоположную сторону.</p>
<h3>Масштабирование</h3>
<p>Применение масштабирования имеет следующую форму:</p>
<pre class="brush:css;">transform: scale(величина_масштабирования);</pre>
<p>Используем маштабирование:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/8.2.png" alt="Масштабирование в CSS3" />
<p>Значение больше 1 приводит к растяжению по вертикали и горизонтали, а меньше 1 - к сжатию. То есть значение 0.5 приводит к уменьшению в два раза, 
а значение 1.5 - к увеличению в полтора раза.</p>
<p>Можно также задать величины масштабирования отдельно для вертикали и горизонтали:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/8.3.png" alt="Настройка масштабирования в CSS3" />
<p>В данном случае по горизонтали будет идти масштабирование в 2 раза, а по вертикали - в 0.5 раз.</p>
<p>Также можно по отдельности задать параметры масштабирования: функция <span class="b">scaleX()</span> задает изменение по горизонтали, а 
<span class="b">scaleY()</span> - по вертикали. Например:</p>
<pre class="brush:css;">
.scale1{
	transform: scaleX(2);
}
</pre>
<p>Используя отрицательные значения, можно создать эффект зеркального отражения:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/8.4.png" alt="Трансформации и зеркальное отражение в CSS3" />
<h3>Перемещение</h3>
<p>Для перемещения элемента используется функция <span class="b">translate</span>:</p>
<pre class="brush:css;">
transform: translate(offset_X, offset_Y);
</pre>
<p>Значение <code>offset_X</code> указывает, на сколько элемент смещается по горизонтали, а <code>offset_Y</code> - по вертикали.</p>
<p>К примеру, сместим блок на 30 пикселей вниз и на 50 пикселей вправо:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/8.5.png" alt="Трансформация перемещения в CSS3" />
<p>В качестве единиц измерения смещения можно применять не только пиксели, но и любые другие единицы измерения длины в CSS - em, % и тд.</p>
<p>С помощью дополнительных функций можно отдельно применять смещения к горизонтали или вертикали: <span class="b">translateX()</span> (перемещение 
по горизонтали) и <span class="b">translateY()</span> (перемещение по вертикали). Например:</p>
<pre class="brush:css;">
transform: translateX(30px);
</pre>
<p>Кроме положительных значений также можно использовать и отрицательные - они перемещают элемент в противоположную сторону:</p>
<pre class="brush:css;">
transform: translateY(-2.5em);
</pre>
<h3>Наклон</h3>
<p>Для наклона элемента применяется функция <span class="b">skew()</span>:</p>
<pre class="brush:css;">
transform: skew(X, Y);
</pre>
<p>Первый параметр указывает, на сколько градусов наклонять элемент по оси X, а второй - значение наклона по оси Y.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/8.6.png" alt="transform: skew в CSS3" />
<p>Для создания наклона только по одной оси для другой оси надо использовать значение 0. Например, наклон на 45 градусов по оси X:</p>
<pre class="brush:css;">
transform: skew(45deg, 0);
</pre>
<p>Или наклон на 45 градусов только по оси Y:</p>
<pre class="brush:css;">
transform: skew(0,45deg);
</pre>
<p>Для создания наклона отдельно по оси X и по оси Y в CSS есть специальные функции: <span class="b">skewX()</span> и <span class="b">skewY()</span> соответственно.</p>
<pre class="brush:css;">
transform: skewX(45deg);
</pre>
<p>Также можно передавать отрицательные значения. Тогда наклон будет осуществляться в противоположную сторону:</p>
<pre class="brush:css;">
transform: skewX(-30deg);
</pre>
<h3>Комбинирование преобразований</h3>
<p>Если нам надо применить к элементу сразу несколько преобразований, скажем, вращаение и перемещение, то мы можем их комбинировать. Например, 
применение всех четырех преобразований:</p>
<pre class="brush:css;">
transform: translate(50px, 100px) skew(30deg, 10deg) scale(1.5) rotate(90deg);
</pre>
<p>Браузер применяет все эти функции в порядке их следования. То есть в данном случае сначала к элементу применяется перемещение, потом наклон, потом масштабирование и 
в конце вращение.</p>
<h3>Исходная точка трансформации</h3>
<p>По умолчанию при применении трансформаций браузер в качестве точки начала преобразования использует центр элемента. Но с помощью свойства 
<span class="b">transform-origin</span> можно изменить исходную точку. Это свойство в качестве значения принимает значения в пикселях, 
em и процентах. Также для установки точки можно использовать ключевые слова:</p>
<ul>
<li><p><code>left top</code>: левый верхний угол элемента</p></li>
<li><p><code>left bottom</code>: левый нижний угол элемента</p></li>
<li><p><code>right top</code>: правый верхний угол элемента</p></li>
<li><p><code>right bottom</code>: правый нижний угол элемента</p></li>
</ul>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<img src="pics/8.7.png" alt="Центр трансформаций в CSS3" />
...
<div class="nav"><p><a href="8.10.php">Назад</a><a href="./">Содержание</a><a href="9.2.php">Вперед</a></p></div></div>
<div class="socBlock">
