...
<html >
<head>
<title>HTML5 | Элементы форм</title>
<meta charset="utf-8" />
<meta name="description" content="Элементы форм в HTML5, их типы, тег input и его атрибут type, элементы select, button, textarea, роль атрибутов name и value">
...
<h2>Элементы форм</h2><div class="date">Последнее обновление: 08.04.2016</div>
...
<p>Формы состоят из определенного количества элементов ввода. Все элементы ввода помещаются между тегами <code>&lt;form&gt;</code> и <code>&lt;/form&gt;</code></p>
<p>Наиболее распространенным элементом ввода является элемент <span class="b">input</span>. Однако реальное действие этого элемента зависит от того, какое значение установлено 
у его атрибута <span class="b">type</span>. А он может принимать следующие значения:</p>
<ul>
<li><p><span class="b">text</span>: обычное текстовое поле</p></li>
<li><p><span class="b">password</span>: тоже текстовое поле, только вместо вводимых символов отображаются звездочки, поэтому в основном используется для ввода пароля</p></li>
<li><p><span class="b">radio</span>: радиокнопка или переключатель. Из группы радиокнопок можно выбрать только одну</p></li>
<li><p><span class="b">checkbox</span>: элемент флажок, который может находиться в отмеченном или неотмеченном состоянии</p></li>
<li><p><span class="b">hidden</span>: скрытое поле</p></li>
<li><p><span class="b">submit</span>: кнопка отправки формы</p></li>
<li><p><span class="b">color</span>: поле для ввода цвета</p></li>
<li><p><span class="b">date</span>: поле для ввода даты</p></li>
<li><p><span class="b">datetime</span>: поле для ввода даты и времени с учетом часового пояса</p></li>
<li><p><span class="b">datetime-local</span>: поле для ввода даты и времени без учета часового пояса</p></li>
<li><p><span class="b">email</span>: поле для ввода адреса электронной почты</p></li>
<li><p><span class="b">month</span>: поле для ввода года и месяца</p></li>
<li><p><span class="b">number</span>: поле для ввода чисел</p></li>
<li><p><span class="b">range</span>: ползунок для выбора числа из некоторого диапазона</p></li>
<li><p><span class="b">tel</span>: поле для ввода телефона</p></li>
<li><p><span class="b">time</span>: поле для ввода времени</p></li>
<li><p><span class="b">week</span>: поле для ввода года и недели</p></li>
<li><p><span class="b">url</span>: поле для ввода адреса url</p></li>
<li><p><span class="b">file</span>: поле для выбора отправляемого файла</p></li>
<li><p><span class="b">image</span>: создает кнопку в виде картинки</p></li>
</ul>
<p>Кроме элемента input в различных модификациях есть еще небольшой набор элементов, которые также можно использовать на форме:</p>
<ul>
<li><p><span class="b">button</span>: создает кнопку</p></li>
<li><p><span class="b">select</span>: выпадающий список</p></li>
<li><p><span class="b">label</span>: создает метку, которая отображается рядом с полем ввода</p></li>
<li><p><span class="b">textarea</span>: многострочное текстовое поле</p></li>
</ul>
<h3>Атрибуты name и value</h3>
<p>У всех элементов ввода можно установить атрибуты <code>name</code> и <code>value</code>. Эти атрибуты имеют важное значение. По атрибуту <code>name</code> 
мы можем идентифицировать поле ввода, а атрибут <code>value</code> позволяет установить значение поля ввода. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<p>Здесь текстовое поле имеет значение "Tom" (как указано в атрибуте value), поэтому при загрузке веб-страницы в этом поле мы увидим данный текст.</p>
<img src="pics/2.3.png" alt="Ввод в форму в HTML5" />
<p>Поскольку методом отправки данных формы является метод "get", то данные будут отправляться через строку запроса. Так как нам в данном случае не важно, как 
данные будут приниматься, не важен сервер, который получает данные, поэтому в качестве адреса я установил ту же самую страницу - то есть файл index.html. 
И при отправке мы сможем увидеть введенные данные в строке запроса:</p>
<img src="pics/2.4.png" alt="Ввод в форму в HTML5" />
<p>В строке запроса нас интересует следующий кусочек:</p>
<pre class="brush:xml;">login=Tom&amp;password=qwerty</pre>
<p>При отправке формы браузер соединяет все данные в набор пар "ключ-значение". В нашем случае две таких пары: <code>login=Tom</code> и 
<code>password=qwerty</code>. Ключом в этих парах выступает название поля ввода, которое определяется атрибутом <code>name</code>, а значением - собственно 
то значение, которое введено в поле ввода (или значение атрибута <code>value</code>).</p>
<p>Получив эти данные, сервер легко может узнать, какие значения в какие поля ввода были введены пользователем.</p>
