...
<html >
<head>
<title>HTML5 | Метки и автофокус</title>
<meta charset="utf-8" />
<meta name="description" content="Метки в HTML5, элемент label, установка автофокуса">
...
<h2>Метки и автофокус</h2><div class="date">Последнее обновление: 08.04.2016</div>
...
<p>Вместе с полями ввода нередко используются метки, которые представлены элементом <span class="b">label</span>. Метки создают аннотацию или заголовок к полю ввода, 
указывают, для чего это поле предназначено.</p>
<p>Для связи с полем ввода метка имеет атрибут <span class="b">for</span>, который указывает на id поля ввода:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<p>Так, текстовое поле здесь имеет атрибут <code>id="login"</code>. Поэтому у связанной с ним метки устанавливается атрибут <code>for="login"</code>. 
Нажатие на эту метку позволяет перевести фокус на текстовое поле для ввода логина:</p>
<img src="pics/2.10.png" alt="Label in HTML5" />
<p>Собственно на этом роль меток и заканчивается. Также мы можем установить автофокус по умолчанию на какое-либо поле ввода. Для этого применяется атрибут 
<span class="b">autofocus</span>:</p>
<pre class="brush:xml;">
&lt;form&gt;
	&lt;p&gt;
		&lt;label for="login"&gt;Логин: &lt;/label&gt;
...
<p>Здесь при запуске страницы фокус сразу же переходит на текстовое поле.</p>
