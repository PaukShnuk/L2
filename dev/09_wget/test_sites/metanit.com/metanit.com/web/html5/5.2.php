...
<html >
<head>
<title>CSS3 | Селекторы</title>
<meta charset="utf-8" />
<meta name="description" content="Селекторы в CSS 3, стилизация элементов по тегам, классам, идентификаторам, универсальный селектор">
...
<h2>Селекторы</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Определение стиля начинается с селектора. Например:</p>
<pre class="brush:css;">
div{
	width:50px; /* ширина */
	height:50px; /* высота */
...
<p>В данном случае селектором является <code>div</code>. Ряд селекторов наследуют название форматируемых элементов, например, div, p, h2 и т. д. 
...
применяться ко всем элементам <code>&lt;div&gt;</code> на веб-странице:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<p>Здесь на странице 3 элемента div, и все они будут стилизованы:</p>
<img src="pics/4.2.png" alt="Селекторы CSS" />
<h3>Классы</h3>
<p>Иногда для одних и тех же элементов требуется различная стилизация. И в этом случае мы можем использовать классы.</p>
<p>Для определения селектора класса в CSS перед названием класса ставится точка:</p>
<pre class="brush:css;">
.redBlock{
	background-color:red;
}
</pre>
<p>Название класса может быть произвольным. Например, в данном случае название класса - "redBlock". Однако при этом в имени класса разрешается использовать буквы, числа, дефисы и знаки подчеркивания, 
причем начинать название класса должно обязательно с буквы.</p>
<p>Также стоит учитывать регистр имен: названия "article" и "ARTICLE" будут представлять разные классы.</p>
<p>После определения класса мы можем его применить к элементу с помощью атрибута <span class="b">class</span>. Например:</p>
<pre class="brush:xml;">&lt;div class="redBlock"&gt;&lt;/div&gt;</pre>
<p>Определим и используем несколько классов:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/4.3.png" alt="Классы в CSS3" />
<h3>Идентификаторы</h3>
<p>Для идентификации уникальных на веб-станице элементов используются идентификаторы, которые определяются с помощью атрибутов id. Например, на странице 
может быть головной блок или шапка:</p>
<pre class="brush:xml;">&lt;div id="header"&gt;&lt;/div&gt;</pre>
<p>Определение стилей для идентификаторов аналогично определению классов, только вместо точки ставится символ решетки #:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/4.4.png" alt="Идентификаторы в CSS3" />
<p>Однако стоит заметить, что идентификаторы в большей степени относятся к структуре веб-странице и в меньшей степени к стилизации. Для стилизации 
преимущественно используются классы, нежели идентификаторы.</p>
<h3>Универсальный селектор</h3>
<p>Кроме селекторов тегов, классов и идентификаторов в css также есть так называемый <span class="b">универсальный селектор</span>, 
который представляет знак звездочки (*). Он применяет стили ко всем элементам на html-странице:</p>
<pre class="brush:css;">
*{		
	background-color: red;
}
</pre>
<h3>Стилизация группы селекторов</h3>
<p>Иногда определенные стили применяются к целому ряду селекторов. Например, мы хотим применить ко всем заголовкам подчеркивание. В этом случае 
мы можем перечислить селекторы всех элементов через запятую:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/4.5.png" alt="Группа селекторов в CSS3" />
<p>Группа селекторов может содержать как селекторы тегов, так и селекторы классов и идентификаторов, например:</p>
<pre class="brush:css;">
h1, #header, .redBlock{
