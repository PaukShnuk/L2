...
<html >
<head>
<title>HTML5 | Формы</title>
<meta charset="utf-8" />
<meta name="description" content="Формы в HTML5, элемент form и его атрибуты, автодополнение элементов форм">
...
<h1>Работа с формами</h1><h2>Формы</h2><div class="date">Последнее обновление: 08.04.2016</div>
...
<p>Формы в html представляют один из способов для ввода и отправки данных. Все поля формы помещаются между тегами <span class="b">&lt;form&gt;</span> 
и <span class="b">&lt;/form&gt;</span>. Например, создадим простейшую форму:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/2.1.png" alt="Формы в HTML5" />
<p>Для настройки форм у элемента form определены следующие атрибуты:</p>
<ul>
<li><p><code>method</code>: устанавливает метод отправки данных на сервер. Допустимы два значения: <code>post</code> и <code>get</code>.</p>
<p>Значение <code>post</code> позволяет передать данные на веб-сервер через специальные заголовки. А значение <code>get</code> позволяет 
передать данные через строку запроса.</p></li>
<li><p><code>action</code>: устанавливает адрес, на который передаются данные формы</p></li>
<li><p><code>enctype</code>: устанавливает тип передаваемых данных. Он свою очередь может принимать следующие значения:</p>
<ul>
<li><p><code>application/x-www-form-urlencoded</code>: кодировка отправляемых данных по умолчанию</p></li>
<li><p><code>multipart/form-data</code>: эта кодировка применяется при отправке файлов</p></li>
<li><p><code>text/plain</code>: эта кодировка применяется при отправке простой текстовой информации</p></li>
</ul>
</li>
</ul>
<p>В выше использованном примере:</p>
<pre class="brush:xml;">
&lt;form method="post" action="http://localhost:8080/login.php"&gt;
	&lt;input name="login"/&gt;
	&lt;input type="submit" value="Войти" /&gt;
...
<p>у формы установлен метод "post", то есть все значения формы отправляются в теле запроса, а адресом служит строка 
<i>http://localhost:8080/login.php</i>. Адрес здесь указан случайным образом.</p> 
<p>Как правило, по указанному адресу работает веб-сервер, который, используя одну из технологий серверной стороны (PHP, NodeJS, ASP.NET и т.д.), может 
получать запросы и возвращать ответ. В данном же случае мы не будем акцентировать внимание на технологиях серверной стороны, сосредоточимся лишь на тех средствах HTML, 
которые позволяют отправлять данные на сервер.</p>
<h3>Автодополнение</h3>
<p>Часто веб-браузеры запоминают вводимые данные, и при вводе браузеры могут выдавать список подсказок из ранее введенных слов:</p>
<img src="pics/2.2.png" alt="Autocomplete в HTML5" />
<p>Это может быть не всегда удобно, и с помощью атрибута  <span class="b">autocomplete</span> можно отключить автодополнение:</p>
<pre class="brush:xml;">
...
<p>Если нам надо включить автодополнение только для каких-то определенных полей, то мы можем применить к ним атрибут <code>autocomplete="on"</code>:</p>
<pre class="brush:xml;">
&lt;form method="post" autocomplete="off" action="http://localhost:8080/login.php"&gt;
	&lt;input name="login" /&gt;
...
<p>Теперь для всей формы, кроме второго поля, будет отключено автодополнение.</p>
