...
<html >
<head>
<title>HTML5 | Позиционирование</title>
<meta charset="utf-8" />
<meta name="description" content="Позиционирование элементов веб-страницы в HTML 5 и CSS, свойство position, абсолютная и относительная позиция">
...
<h2>Позиционирование</h2><div class="date">Последнее обновление: 28.04.2016</div>
...
<p>CSS предоставляет возможности по позиционированию элемента, то есть мы можем поместить элемент в определенное место на странице</p>
<p>Основным свойством, которые управляют позиционированием в CSS, является свойство <span class="b">position</span>. Это свойство может принимать 
одно из следующих значений:</p>
<ul>
<li><p><span class="b">static</span>: стандартное позиционирование элемента, значение по умолчанию</p></li>
<li><p><span class="b">absolute</span>: элемент позиционируется относительно границ элемента-контейнера, если у того свойство <code>position</code> не равно <code>static</code></p></li>
<li><p><span class="b">relative</span>: элемент позиционируется относительно его позиции по умолчанию. Как правило, основная цель 
...
абсолютного позиционированния вложенных в него элементов</p></li>
<li><p><span class="b">fixed</span>: элемент позиционируется относительно окна бразуера, это позволяет создать фиксированные элементы, которые не 
меняют положения при прокрутке</p></li>
</ul>
<p>Не следует одновременно применять к элементу свойство float и любой тип позиционирования, кроме static (то есть тип по умолчанию).</p>
<h3>Абсолютное позиционирование</h3>
<p>Область просмотра браузера имеет верхний, нижний, правый и левый края. Для каждого из этих четырех краев есть соответствующее 
свойство CSS: <code>left</code> (отступ от края слева), <code>right</code> (отступ от края справа), 
<code>top</code> (отступ от края контейнера сверху) и <code>bottom</code> (отступ снизу). Значения этих свойств указываются в пикселях, em или 
...
<p>Здесь элемент div с абсолютным позиционированием будет находиться на 100 пикселей слева от границы области просмотра и на 50 снизу.</p>
<img src="pics/7.23.png" alt="absolute position in css" />
<p>При этом не столь важно, что после этого элемента div идут какие-то другие элементы. Данный блок div в любом случае будет позиционироваться относительно 
границ области просмотра браузера.</p>
<p>Если элемент с абсоютным позиционированием располагается в другом контейнере, у которого в свою очередь значение свойства <code>position</code> 
не равно <code>static</code>, то элемент позиционируется относительно границ контейнера:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/7.24.png" alt="абсолютное позиционирование в css" />
<h3>Относительное позиционирование</h3>
<p>Относительное позиционирование также задается с помощью значения <span class="b">relative</span>. Для указания конкретной позиции, 
на которую сдвигается элемент, применяются те же свойства top, left, right, bottom:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<h3>Свойство z-index</h3>
<p>По умолчанию при совпадении у двух элементов границ, поверх другого отображается тот элемент, который определен в разметке html последним. 
Однако свойство <span class="b">z-index</span> позволяет изменить порядок следования элементов при их наложении. В качестве значения свойство принимает число. Элементы с большим значением этого 
свойства будут отображаться поверх элементов с меньшим значением z-index.</p>
<p>Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<img src="pics/7.26.png" alt="порядок наложения элементов в HTML" />
<p>Теперь добавим к стилю блока redBlock новое правило:</p>
<pre class="brush:css;">
.redBlock{
	z-index: 100;
...
<p>Здесь z-index равен 100. Но это необязательно должно быть число 100. Так как у второго блока z-index не определен и фактически равен нулю, то для redBlock мы можем 
установить у свойства z-index любое значение больше нуля.</p>
<p>И теперь первый блок будет накладываться на второй, а не наоборот, как было в начале:</p>
<img src="pics/7.27.png" alt="z-index в HTML и CSS" />
...
<div class="nav"><p><a href="8.8.php">Назад</a><a href="./">Содержание</a><a href="8.10.php">Вперед</a></p></div></div>
<div class="socBlock">
