...
<html >
<head>
<title>CSS3 | Стилизация таблиц</title>
<meta charset="utf-8" />
<meta name="description" content="Стилизация таблиц в CSS 3, границы ячеек, вертикальное выравнивание и свойство vertical-align">
...
<h2>Стилизация таблиц</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>CSS предоставляет ряд свойств, которые помогают стилизовать таблицу:</p>
<ul>
</ul>
<li><p><span class="b">border-collapse</span>: устанавливает, как будет стилизоваться граница смежных ячеек</p></li>
<li><p><span class="b">border-spacing</span>: устанавливает промежутки между границами смежных ячеек</p></li>
<li><p><span class="b">caption-side</span>: устанавливает положение элемента caption</p></li>
<li><p><span class="b">empty-cells</span>: задает режим отрисовки для пустых ячеек</p></li>
<li><p><span class="b">table-layout</span>: определяет размеры таблицы</p></li>
<h3>Установка таблицы</h3>
<p>Ранее для установки границы в таблице широко использовался атрибут <code>border</code>, например:</p>
<pre class="brush:xml;">
&lt;table border="2px" &gt;
</pre>
<p>Сейчас же тенденция для стилизации использовать только стили CSS, поэтому граница также задается через CSS с помощью стандартного свойства <span class="b">border</span>:</p>
<pre class="brush:css;">
table {
...
<p>При установке границ между столбцами с помощью свойства <span class="b">border-collapse</span> можно установить общую или раздельную границу между смежными ячейками:</p>
<ul>
<li><p><code>collapse</code>: смежные ячейки имеют общую границу</p></li>
<li><p><code>separate</code>: смежные ячейки имеют отдельные границы, которые разделяются пространством</p></li>
</ul>
<p>Если смежные ячейки имеют раздельные границы, то с помощью свойства <span class="b">border-spacing</span> можно установить пространство между границами:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/4.62.png" alt="Стилизация таблиц и их границ в CSS" />
<h3>Пустые ячейки</h3>
<p>Свойство <span class="b">empty-cells</span> позволяет стилизовать пустые ячейки с помощью одного из следующих значений:</p>
<ul>
<li><p><code>show</code>: пустые ячейки отображаются, значение по умолчанию</p></li>
<li><p><code>hide</code>: пустые ячейки не отображаются</p></li>
</ul>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<img src="pics/4.63.png" alt="Сокрытие пустых ячеек в CSS" />
<h3>Позиционирование заголовка</h3>
<p>Свойство <span class="b">caption-side</span> управляет позицией заголовка и может принимать следующие значения:</p>
<ul>
<li><p><code>top</code>: позиционирование заголовка вверху (значение по умолчанию)</p></li>
<li><p><code>bottom</code>: позиционирование заголовка внизу</p></li>
</ul>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<img src="pics/4.64.png" alt="Заголовоки таблиц в CSS" />
<h3>Управление размером таблицы</h3>
<p>С помощью свойства <span class="b">table-layout</span> можно управлять размером таблицы. По умолчанию это свойство имеет значение 
<code>auto</code>, при котором браузер устанавливает ширину столбцов таблицы автоматически, исходя из ширины самой широкой ячейки в столбце. А из ширины отдельных столбцов складывается 
ширина всей таблицы.</p>
<p>Однако с помощью другого значения - <code>fixed</code> можно установить фиксированную ширину:</p>
<pre class="brush:css;">
table {
	border: 1px solid #ccc;
	border-spacing: 3px;
...
<h3>Вертикальное выравнивание ячеек</h3>
<p>Как правило, содержимое ячеек таблицы выравнивается по центру ячейки. Но с помощью свойства <span class="b">vertical-align</span> это поведение можно переопределить. 
Это свойство принимает следующие значения:</p>
<ul>
<li><p><code>top</code>: выравнивание содержимого по верху ячейки</p></li>
<li><p><code>baseline</code>: выравнивание первой строки текста по верху ячейки</p></li>
<li><p><code>middle</code>: выравнивание по центру (значение по умолчанию)</p></li>
<li><p><code>bottom</code>: выравнивание по низу</p></li>
</ul>
<p>Свойство vertical-align применяется только к элементам <code>&lt;th&gt;</code> и <code>&lt;td&gt;</code>:</p>
<pre class="brush:css;">
td, th{
    border: solid 1px #ccc;
