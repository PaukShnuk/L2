...
<meta charset="utf-8" />
<meta name="description" content="Работа с видео в HTML5, элемент video, поддерживаемые форматы, кроссбраузерность, атрибуты и настройка элемента">
...
<h1>Мультимедиа</h1><h2>Видео</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Для воспроизведения видео в HTML5 используется элемент <span class="b">video</span>. Чтобы настроить данный элемент, мы можем использовать 
следующие его атрибуты:</p>
<ul>
<li><p><code>src</code>: источник видео, это может быть какой-либо видеофайл</p></li>
<li><p><code>width</code>: ширина элемента</p></li>
<li><p><code>height</code>: высота элемента</p></li>
<li><p><code>controls</code>: добавляет элементы управления воспроизведением</p></li>
<li><p><code>autoplay</code>: устанавливает автовоспроизведение</p></li>
<li><p><code>loop</code>: задает повторение видео</p></li>
<li><p><code>muted</code>: отключает звук по умолчанию</p></li>
</ul>
<p>Хотя мы можем установить ширину и высоту, но они не окажут никакого влияния на аспектное отношения ширины и высоты самого видео. 
Например, если видео имеет формат 375×240, то, к примеру, при настройках <code>width="375" height="280"</code> 
видео будет центрироваться на 280-пиксельном пространстве в HTML. Что позволяет избавить видео от искажений, которые были бы при растягивании.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/5.1.png" alt="Видео в HTML5" />
<p>Применим атрибуты <code>autoplay</code> и <code>loop</code>:</p>
<pre class="brush:xml;">
&lt;video src="cats.mp4" width="400" height="300" controls autoplay loop &gt;&lt;/video&gt;
</pre>
<p>Теперь видео будет автоматически проигрываться бесконечное число раз.</p>
<p>Если при воспроизведении надо отключить звук, то мы можем воспользоваться атрибутом <code>muted</code>:</p>
<pre class="brush:xml;">
&lt;video src="cats.mp4" width="400" height="300" controls muted &gt;&lt;/video&gt;
</pre>
<h3>Атрибут preload</h3>
<p>Еще один атрибут - <span class="b">preload</span> призван управлять загрузкой видео. Он принимает следующие значения:</p>
<ul>
<li><p><code>auto</code>: видео и связанные с ним метаданные будут загружаться до того, как видео начнет воспроизводиться.</p></li>
<li><p><code>none</code>: видео не будет загружаться в фоне, пока пользователь не нажмет на кнопку начала проигрывания</p></li>
<li><p><code>metadata</code>: в фоне до воспроизведения будут загружаться только метаданные (данные о формате, длительности и т.д), само видео не загружается</p></li>
</ul>
<pre class="brush:xml;">&lt;video src="cats.mp4" width="400" height="300" controls preload="auto"&gt;&lt;/video&gt;</pre>
<h3>Атрибут poster</h3>
<p>Атрибут poster позволяет установить изображение, которое будет отображаться до запуска видео. Этому атрибуту в качестве значения передается путь к изображению:</p>
<pre class="brush:xml;">&lt;video src="cats.mp4" width="400" height="300" controls poster="mycat.jpg"&gt;&lt;/video&gt;</pre>
<h3>Поддержка форматов видео</h3>
<p>Главной проблемой при использовании элемента <code>video</code> является поддержка различными веб-браузерами определенных форматов. 
С помощью вложенных элементов <span class="b">source</span> можно задать несколько источников видео, один из которых будет использоваться:</p>
<pre class="brush:xml;">
...
<p>Элемент <code>source</code> использует два атрибута для установки источника видео:</p>
<ul>
<li><p><code>src</code>: путь к видеофайлу</p></li>
<li><p><code>type</code>: тип видео (MIME-тип)</p></li>
</ul>
<p>Если браузер не поддерживает первый тип видео, то он пытается загрузить второй видеофайл. Если же и тип второго видеофайла не поддерживается, то браузер 
обращается к третьему видеофайлу.</p>
...
<div class="nav"><p><a href="11.3.php">Назад</a><a href="./">Содержание</a><a href="7.2.php">Вперед</a></p></div></div>
<div class="socBlock">
