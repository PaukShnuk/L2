...
<html >
<head>
<title>CSS3 | Псевдоклассы</title>
<meta charset="utf-8" />
<meta name="description" content="Псевдоклассы в CSS 3, стилизация ссылок, селекторы :not, :lang, :focus, :hover">
...
<h2>Псевдоклассы</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>В дополнение к селекторам тегов, классов и идентификаторов нам доступны селекторы псевдоклассов, которые несут дополнительные 
возможности по выбору нужных элементов.</p>
<p>Список доступных псевдоклассов:</p>
<ul>
<li><p><span class="b">:root</span>: позволяет выбрать корневой элемент веб-страницы, наверное наименее полезный селектор, так как на правильной веб-странице 
корневым элементом практически всегда является элемент <code>&lt;html&gt;</code></p></li>
<li><p><span class="b">:link</span>: применяется к ссылкам и представляет ссылку в обычном состоянии, по которой еще не совершен переход</p></li>
<li><p><span class="b">:visited</span>: применяется к ссылкам и представляет ссылку, по которой пользователь уже переходил</p></li>
<li><p><span class="b">:active</span>: применяется к ссылкам и представляет ссылку в тот момент, когда пользователь осуществляет по ней переход</p></li>
<li><p><span class="b">:hover</span>: представляет элемент, на который пользователь навел указатель мыши. Применяется преимущественно к ссылкам, однако может также применяться 
и к другим элементам, например, к параграфам</p></li>
<li><p><span class="b">:focus</span>: представляет элемент, который получает фокус, то есть когда пользователь нажимает клавишу табуляции или нажимает кнопкой мыши 
на поле ввода (например, текстовое поле)</p></li>
<li><p><span class="b">:not</span>: позволяет исключить элементы из списка элементов, к которым применяется стиль</p></li>
<li><p><span class="b">:lang</span>: стилизует элементы на основании значения атрибута <code>lang</code></p></li>
<li><p><span class="b">:empty</span>: выбирает элементы, которые не имеют вложенных элементов, то есть являются пустыми</p></li>
</ul>
<p>При применении псевдоклассов перед ними всегда ставится двоеточие. Например, стилизуем ссылки, используя псевдоклассы:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<h3>Селектор :not</h3>
<p>Селектор <span class="b">:not()</span> позволяет выбрать все элементы кроме определенных, то есть исключить некоторые элементы из выбора.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/4.21.png" alt="Селектор not в CSS 3" />
<p>Селектор <code>a:not(.blueLink)</code> применяет стиль ко всем ссылкам за исключением тех, которые имеют класс "blueLink". В скобки псевдоклассу 
not передается селектор элементов, которые надо исключить.</p>
<h3>Псевдокласс :lang</h3>
<p>Селектор <span class="b">:lang</span> выбирает элементы на основании атрибута <code>lang</code>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<div class="nav"><p><a href="5.8.php">Назад</a><a href="./">Содержание</a><a href="5.11.php">Вперед</a></p></div></div>
<div class="socBlock">
