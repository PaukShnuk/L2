...
<html >
<head>
<title>HTML5 | Размеры строк и столбцов в Grid Layout</title>
<meta charset="utf-8" />
<meta name="description" content="Высота и ширина строк и столбцов в Grid Layout в CSS3, фиксированные, автоматические и пропорциональные размеры">
...
<h2>Размеры строк и столбцов</h2><div class="date">Последнее обновление: 09.05.2017</div>
...
<h3>Фиксированные размеры</h3>
<p>В примерах, которые были рассмотрены в предыдущих статьях, ширина столбцов и длина строк устанавливались на основании фиксированных значений, которые 
передаются свойствам <span class="b">grid-template-columns</span> и <span class="b">grid-template-rows</span>. Для определения размеров мы можем использовать самые различные единицы 
измерения, которые нам доступны в CSS (px, em, rem, pt, %), например:</p>
<pre class="brush:xml;">
.grid-container {
...
<h3>Автоматические размеры</h3>
<p>Кроме точных размеров можно задавать автоматические размеры с помощью слова <span class="b">auto</span>. В этом случае ширина столбцов и высота строк вычисляются 
исходя из размеров содержимого:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Здесь задано три строки и ти столбца. Первый столбец имеет фиксированную ширину в 8em, а второй и третий столбцы - автоматическую ширину. 
И также первая и третья строки имеют автоматическую высоту, а вторая строка - фиксированную.</p>
<img src="pics/grid14.png" alt="Автоматические размеры в Grid Layout в CSS 3" />
<h3>Пропорциональные размеры</h3>
<p>Для установки пропорциональных размеров применяется специальная единица измерения <span class="b">fr</span>. 
Она представляет собой часть пространства (fraction), которое отводится для данного столбца или строки. Значение fr еще называют flex-фактором (flex factor).</p>
<p>Вычисление пропорциональных размеров производится по формуле:</p>
<pre class="brush:css;">flex-фактор * доступное_пространство  / сумма всех flex-факторов</pre>
<p>При этом под доступным пространством понимается все пространство grid-контейнера за исключением фиксированных значений строк и столбцов.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/grid15.png" alt="flex factor и fr в Grid Layout и CSS 3" />
<p>В данном случае имеются три столбца с шириной 2fr, 8em, 1fr. Поэтому ширина второго столбца будет вычисляться по формуле:</p>
<pre class="brush:css;">2 * (ширина_грида - 8em) / (2 + 1)</pre>
<p>Ширина третьего столбца будет вычисляться по формуле:</p>
<pre class="brush:css;">1 * (ширина_грида - 8em) / (2 + 1)</pre>
<p>И если первый столбец фиксированный с шириной 8em, то ширина второго и третьего столбца будут зависеть от ширины контейнера и будут автоматически масштабироваться при ее изменении.</p>
<p>В отношении строк все аналогично.</p>
