...
<html >
<head>
<title>CSS3 | Стилизация элемента details</title>
<meta charset="utf-8" />
<meta name="description" content="Стилизация элемента details и summary, атрибут open, настройка маркера, свойство list-style">
...
<h2>Стилизация элемента details</h2><div class="date">Последнее обновление: 14.07.2021</div>
...
<p>Рассмотрим некоторые возможности по стилизации элемента <span class="b">details</span>, который представляет раскрываемый блок.</p>
<h3>Атрибут open</h3>
<p>Прежде всего в раскрытом состоянии к элементу <span class="b">details</span> добавляется атрибут <span class="b">open</span>. Соответственно, используя 
атрибут, можно задать разные стили для элемента в скрытом и раскрытом состоянии. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>С помощью селектора <code>details[open]</code> мы можем обратиться к элементу <code>details</code> в раскрытом состоянии. Соответственно селектор</p>
<pre class="brush:css;">
details[open] &gt; summary {
	color:#eee; 
	background-color:#333;
...
<p>Позволяет задать стили для элемента <span class="b">summary</span> в раскрытом состоянии. То есть в данном случае при раскрытии элемента details мы переключаем 
цвет стиля и фона заголовка.</p>
<img src="./pics/6.46.png" alt="Details и summary в HTML и CSS, атрибут open" />
<h3>Настройка маркера</h3>
<p>По умолчанию элемент <span class="b">summary</span> в качестве маркера скрытости/раскрытости использует символ треугольника. Но его также можно настроить.</p>
<p>Для настройки изображения маркера можно использовать свойство <span class="b">list-style</span>, а также дополнительные свойства типа 
<span class="b">list-style-type</span> или <span class="b">list-style-image</span>, которые применяются для <a href="https://metanit.com/web/html5/6.7.php">стилизации списков</a>. 
Например, если необходимо убрать этот маркер, то можно применить стиль <code>list-style: none</code>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/6.47.png" alt="Details и summary в HTML и CSS, настройка маркера и list-style" />
<p>Другой пример - примение стиля <code>list-style-type: disc;</code>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/6.48.png" alt="Details и summary в HTML и CSS, настройка маркера и list-style-type" />
<h4>Настройка с помощью свойства content</h4>
<p>Но естественно свойством <code>list-style</code> мы не ограничены и по своему усмотрению можем более тонко управлять заголовком, например, с помощью свойства <span class="b">content</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае с помощью селектора <code>summary:before</code> устанавливаем содержимое, которое будет располагаться перед основным содержимым элемента <code>summary</code>. 
Селектор <code>details[open] summary:before</code> позволяет сделать то же самое, только в раскрытом виде. В итоге в скрытом виде маркер будет отображать символ +, а в раскрытом - 
символ -.</p>
<img src="./pics/6.49.png" alt="Details и настройка маркера summary в HTML и CSS" />
...
<div class="nav"><p><a href="./6.20.php">Назад</a><a href="./">Содержание</a><a href="./8.1.php">Вперед</a></p></div></div>
<div class="socBlock">
