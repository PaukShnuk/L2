...
<html >
<head>
<title>HTML5 | Разновидности синтаксиса</title>
<meta charset="utf-8" />
<meta name="description" content="Разновидности синтаксиса в HTML5, стиль XHTML, примеры разных стилей, использование валидатора для проверки правильности веб-страницы">
...
<h2>Разновидности синтаксиса HTML5</h2><div class="date">Последнее обновление: 18.11.2019</div>
...
<p>При создании документа HTML5 мы можем использовать два различных стиля: HTML и XML.</p>
<p>Стиль HTML предполагает следующие моменты:</p>
<ul>
<li><p>Начальные открывающие теги могут отсутствовать у элементов</p></li>
<li><p>Конечные закрывающие теги могут отсутствовать у элементов</p></li>
<li><p>Только пустые элементы (void elements) (например, <code>br</code>, <code>img</code>, <code>link</code>) могут закрываться с помощью слеша <code>/&gt;</code></p></li>
<li><p>Регистр названий тегов и атрибутов не имеет значения</p></li>
<li><p>Можно не заключать значения атрибутов в кавычки</p></li>
<li><p>Некоторые атрибуты могут не иметь значений (checked и disabled)</p></li>
<li><p>Специальные символы не экранируются</p></li>
<li><p>Документ должен иметь элемент DOCTYPE</p></li>
</ul>
<p>Это так называемый "разрешительный" стиль, основанный на послаблениях при создании документа.</p>
<p>Документ HTML5 также может быть описан с помощью синтаксиса XML. Такой стиль еще называют "XHTML". 
Он используется, если заголовок content-type имеет значение <code>application/xml+xhtml</code>. Для данного стиля характерны следующие правила:</p>
<ul>
<li><p>Каждый элемент должен иметь начальный открывающий тег</p></li>
<li><p>Непустые элементы (non-void elements) с начальным открывающим тегом также должны иметь конечный закрывающий тег</p></li>
<li><p>Любой элемент может закрываться с помощью слеша <code>/&gt;</code></p></li>
<li><p>Названия тегов и атрибутов регистрозависимы, как правило, используются в нижнем регистре</p></li>
<li><p>Значения атрибутов должны быть заключены в кавычки</p></li>
<li><p>Атрибуты без значений не допускаются (<code>checked="checked"</code> вместо просто <code>checked</code>)</p></li>
<li><p>Специальные символы должны быть экранированы</p></li>
</ul>
<p>Сравним два подхода. Подход HTML5:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<p>И аналогичный пример с использованием подхода XHTML:</p>
<pre class="brush:xml;">
...
<p>При использовании синтаксиса XHTML нам также надо указать пространство имен для данного документа: <code>&lt;html xmlns="http://www.w3.org/1999/xhtml"&gt;</code></p>
<p>Выбор конкретного стиля при написании html-документов зависит от предпочтений программиста или веб-дизайнера. Нередко используется смешенный стиль, 
который заимствует правила из первого, и из второго стилей.</p> 
<p>В то же время надо учитывать, что 
наличие у элемента закрывающего и открывающего тегов снижает вероятность, что элемент будет неправильно интерпретирован браузером.</p> 
<p>Также заключение значений атрибутов в кавычки поможет избежать потенциальных ошибок. Так, атрибут <code>class</code> может принимать несколько 
значений подряд. Например: <code>&lt;div class="navmenu bigdesctop"&gt;</code>. Но если мы опустим кавычки, то в качестве значения будет использоваться 
"navmenu", а "bigdesctop" браузер будет пытаться интерпретировать как отдельный атрибут.</p>
<p>Если же возникают затруднения, насколько правильной является создаваемая разметка html, то ее можно проверить с помощью валидатора по адресу 
<a href="https://validator.w3.org" rel="nofollow">https://validator.w3.org</a>:</p>
<img src="pics/1.6.png" alt="HTML5 Validator" />
<p>Мы можем вставить в текстовое поле код веб-страницы, и после нажатия на кнопку "Check" внизу валидатор либо отобразит нам ошибки красным цветом, либо зеленым цветом 
уведомит, что ошибок нет, и код прошел валидацию.</p>
