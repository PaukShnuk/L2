...
<html >
<head>
<title>CSS3 | Линейный градиент</title>
<meta charset="utf-8" />
<meta name="description" content="Линейный градиент в CSS 3, параметры градиента и функция linear-gradient, кроссбраузерность">
...
<h2>Линейный градиент</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Градиенты представляют плавный переход от одного цвета к другому. В CSS3 имеется ряд встроенных градиентов, которые можно использовать для создания фона элемента.</p>
<p>Градиенты в CSS не представляют какого-то специального свойства. Они лишь создают значение, которое присваивается свойству <span class="b">background-image</span>.</p>
<p>Линейный градиент распространяется по прямой от одного конца элемента к другому, осуществляя плавный переход от одного цвета к другому.</p>
<p>Для создания градиента нужно указать его начало и несколько цветов, например:</p>
<pre class="brush:css;">
background-image: linear-gradient(left,black,white);
</pre>
<p>В данном случае началом градиента будет левый край элемента, который обозначается значением <code>left</code>. Цвета градиента: черный (black) 
и белый (white). То есть начиная с левого края элемента до правого будет плавно идти переход из черного цвета в белый.</p>
<p>В использовании градиентов есть один недостаток - многообразие браузеров вынуждает использовать префикс вендора:</p>
<pre class="brush:css;">
...
<p>Так, полноценное использование градиента будет выглядеть следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/4.65.png" alt="Линейный градиент в CSS 3" />
<p>Для установки начала градиента можно использовать следующие значения: <code>left</code> (слева направо), <code>right</code> (справа налево), 
<code>top</code> (сверху вниз) или <code>bottom</code> (снизу вверх). Например, вертикальный градиент будет выглядеть следующим образом:</p>
<pre class="brush:css;">
background-image: linear-gradient(bottom,black,white);
</pre>
<p>Также можно задать диагональное направление с помощью двух значений:</p>
<pre class="brush:css;">
background-image: linear-gradient(top left,black,white);
</pre>
<img src="pics/4.66.png" alt="Настройка линейного градиента в CSS 3" />
<p>Кроме конкретных значений типа top или left также можно указать угол от 0 до 360, который определит направление градиента:</p>
<pre class="brush:css;">
background-image: linear-gradient(30deg,black,white);
</pre>
<p>После величины углы указывается слово <span class="b">deg</span>.</p>
<p>К примеру, 0deg означает, что градиент начинается в левой части и перемещается в правую часть, а при указании 45deg он начинается в нижнем 
левом углу и перемещается под углом 45° в верхний правый угол.</p>
<p>После определения начала градиента, можно указать применяемых цветов или опрные точки. Цветов не обязательно должно быть два, их может быть и больше:</p>
<pre class="brush:css;">
background-image: linear-gradient(top, red, #ccc, blue);
</pre>
<p>Все применяемые цвета распределяются равномерно. Однако можно также указать конкретные места фона для 
цветовых точек. Для этого после цвета добавляется второе значение, которое и определяет положение точки.</p>
<pre class="brush:css;">
background-image: linear-gradient(left, #ccc, red 20%, red 80%, #ccc);
</pre>
<h3>Повторяющийся градиент</h3>
<p>С помощью <code>repeating-linear-gradient</code> можно создавать повторяющиеся линейные градиенты. Например:</p>
<pre class="brush:css;">
...
<p>В данном случае градиент начинается с левого края элемента с полоски серого цвета (#ccc) шириной 20 пикселей, далее до 30 пикселей 
идет переход к красному цвету, а затем до 40 пикселей выполняется обратный переход к светло-синему цвету (rgba(0, 0, 126, .5)). 
После этого браузер повторяет градиент, пока не заполнит всю поверхность элемента.</p>
...
<div class="nav"><p><a href="6.18.php">Назад</a><a href="./">Содержание</a><a href="6.20.php">Вперед</a></p></div></div>
<div class="socBlock">
