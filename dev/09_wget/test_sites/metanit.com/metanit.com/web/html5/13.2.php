...
<html >
<head>
<title>HTML5 | Строки и столбцы в Grid Layout</title>
<meta charset="utf-8" />
<meta name="description" content="Создание строк и столбцов в Grid Layout в CSS3, свойства grid-template-columns и grid-template-rows">
...
<h2>Строки и столбцы</h2><div class="date">Последнее обновление: 09.05.2017</div>
...
<p>Грид образует сетку из строк и столбцов, на пересечении которых образуются ячейки. И для установки строк и столбцов в Grid Layout использовать следующие свойства CSS3:</p>
<ul>
<li><p><span class="b">grid-template-columns</span>: настраивает столбцы</p></li>
<li><p><span class="b">grid-template-rows</span>: настраивает строки</p></li>
</ul>
<h3>Столбцы</h3>
<p>Для определения столбцов используем у grid-контейнера стилевое свойство <span class="b">grid-template-columns</span>. Например, определим грид с двумя столбцами:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>В качестве значения свойству <span class="b">grid-template-columns</span> передается ширина столбцов. Сколько мы хотим иметь в гриде столбцов, столько 
и нужно передать значений этому свойству. Так, в случае выше грид содержит два столбца, поэтому свойству передаются 
два значения, которые указывают ширину столбцов:</p>
<img src="pics/grid4.png" alt="Установка столбцов в Grid Layout в CSS3" />
<p>Соответственно если мы хотим, чтобы в гриде было три столбца, то нам надо передать три значения, например:</p>
<pre class="brush:css;">grid-template-columns: 8em 7em 8em;</pre>
<p>Если столбцов больше чем элементов, то по умолчанию для их вмещения создаются новые строки.</p>
<h3>Строки</h3>
<p>Настойка строк во многом аналогичная настройке столбцов. Для этого у грид-контейнера необходимо установить свойство <span class="b">grid-template-rows</span>, 
которое задает количество и размеры строк:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Свойству <span class="b">grid-template-rows</span> передается высота каждой из строк. Так, в данном случае высота первой строки составляет 4em, а второй - 5em.</p>
<img src="pics/grid7.png" alt="Строки в Grid Layout CSS3" />
<p>В то же время, если элементов больше, чем ячеек грида, то образуются дополнительные строки (как в случае со столбцами). Поэтому, несмотря на то, что выше были определены 
настройки только для двух строк, в реальности строк в гриде будет три строки, причем, как видно на скриншоте, высота третьей строки необязательно будет 5em, как у других строк, 
она будет вычисляться автоматически.</p>
<p>Изменим стили grid-контейнера, добавив еще две строки:</p>
<pre class="brush:css;">
.grid-container {
	border: solid 2px #000;
	display: grid;
...
<p>Здесь определено три столбца и четыре строки:</p>
<img src="pics/grid8.png" alt="Столбцы и строки в grid в CSS 3" />
<p>И поскольку ячеек грида больше, чем элементов, то последняя строка оказывается пустой.</p>
