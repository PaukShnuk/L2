...
<html >
<head>
<title>CSS3 | Блочная модель</title>
<meta charset="utf-8" />
<meta name="description" content="Блочная модель в CSS 3, определение пространства и размеров элемента на веб-странице, просмотр блоковой модели элемента в браузере">
...
<h2>Блочная модель</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Для веб-браузера элементы страницы представляют небольшие контейнеры или блоки. Такие блоки могут иметь различное содержимое - текст, 
изображения, списки, таблицы и  другие элементы. Внутренние элементы блоков сами выступают в качестве блоков.</p>
<p>Схематично блочную модель можно представить следующим образом:</p>
<img src="pics/4.38.png" alt="Блочная модель в CSS 3" />
<p>Пусть элемент расположен в каком-нибудь внешнем контейнере. Это может быть элемент body, div и так далее. От других элементов он отделяется некоторым расстоянием - 
внешним отступом, которое описывается свойством CSS <span class="b">margin</span>. То есть свойство margin определяет расстояние от границы текущего элемента до других соседних элементов или 
до границ внешнего контейнера.</p>
<p>Далее начинается сам элемент. И в начале идет его граница, которая в CSS описывается свойством <span class="b">border</span>.</p>
<p>После границы идет внутренний отступ, который в CSS описывается свойством <span class="b">padding</span>. Внутренний отступ определяет расстояние от границы элемента до 
внутреннего содержимого.</p>
<p>Далее идет внутреннее содержимое, которое также реализует ту же блочную модель и также может состоять из других элементов, которые имеют внешние и внутренние отступы и 
границу.</p>
...
<p>После запуска веб-страницы в браузере мы можем посмотреть блочную модель конкретных элементов. Для этого надо нажать на нужный элемент правой кнопкой 
мыши и открывающемся контекстном меню выбрать пункт, который позволяет просмотреть исходный код элемента. Для разных браузеров этот пункт может называться по разному. 
К примеру в Google Chrome это <span class="b">Посмотреть код</span>:</p>
<img src="pics/4.40.png" alt="Просмотр кода элемента в Google Chrome" />
<p>В Mozilla Firefox этот пункт называется <span class="b">Исследовать элемент</span>.</p>
<p>И по выбору данного пункта браузер откроет панель, где будет показан код элемента его стили и блочная модель:</p>
<img src="pics/4.39.png" alt="Box Model в CSS 3" />
<p>В этой модели мы можем увидеть, как задаются отступы элемента, его граница, посмотреть отступы от других элементов и при необходимости 
динамически поменять значения их стилей.</p>
<p>Если мы явным образом не указываем значения свойств margin, padding и border, то браузер применяет предустановленные значения.</p>
...
<div class="nav"><p><a href="6.8.php">Назад</a><a href="./">Содержание</a><a href="6.10.php">Вперед</a></p></div></div>
<div class="socBlock">
