...
<html >
<head>
<title>HTML5 | Управление элементами. flex-basis, flex-shrink и flex-grow</title>
<meta charset="utf-8" />
<meta name="description" content="Управление элементами во Flexbox в HTML5 и CSS3, свойства flex-basis, flex-shrink и flex-grow, масштабирование элементов и их размеры">
...
<h2>Управление элементами. flex-basis, flex-shrink и flex-grow</h2><div class="date">Последнее обновление: 18.04.2017</div>
...
<ul>
<li><p><span class="b">flex-basis</span>: определяет начальный размер flex-элемента</p></li>
<li><p><span class="b">flex-shrink</span>: определяет, как flex-элемент будет уменьшаться относительно других flex-элементов во flex-контейнере</p></li>
<li><p><span class="b">flex-grow</span>: определяет, как flex-элемент будет увеличиваться относительно других flex-элементов во flex-контейнере</p></li>
</ul>
<h3>flex-basis</h3>
<p>Flex-контейнер может увеличиваться или уменьшаться вдоль своей центральной оси, например, при изменении размеров браузера, если контейнер имеет нефиксированные размеры. 
И вместе с контейнером также могут увеличиваться и уменьшаться его flex-элементы. Свойство <span class="b">flex-basis</span> определяет 
начальный размер flex-элемента до того, как он начнет изменять размер, подстраиваясь под размеры flex-контейнера.</p>
<p>Это свойство может принимать следующие значения:</p>
<ul>
<li><p><span class="b">auto</span>: начальный размер flex-элемента устанавливается автоматически</p></li>
<li><p><span class="b">content</span>: размер flex-элемента определяется по его содержимому, в то же время это значение поддерживается не всеми современными браузерами, поэтому его пока стоит избегать</p></li>
<li><p><span class="b">числовое значение</span>: мы можем установить конкретное числовое значение для размеров элемента</p></li>
</ul>
<p>Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
...
<img src="pics/flexbox14.png" alt="flex-basis в flexbox и css3" />
<p>У первого элемента у свойства <code>flex-basis</code> установлено значение auto. Поэтому первый элемент в качестве реального значения для ширины будет использовать 
значение свойства <code>width</code>.</p>
<p>У второго элемента у свойства <code>flex-basis</code> установлено значение auto, однако и свойство <code>width</code> имеет значение auto. Поэтому реальная ширина элемента будет устанавливаться 
по его содержимому.</p>
<p>У третьего элемента свойство <code>flex-basis</code> имеет конкретное значение, которое и используется. А свойство <code>width</code> в этом случае уже не играет никакой роли.</p>
<h3>flex-shrink</h3>
<p>Если flex-контейнер имеет недостаточно места для размещения элемента, то дальнейшее поведение этого элемента мы можем определить с помощью свойства 
<span class="b">flex-shrink</span>. Оно указывает, как элемент будет усекаться относительно других элементов.</p>
<p>В качестве значения свойство принимает число. По умолчанию его значение 1.</p>
<p>Рассмотрим действие этого свойства на примере:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/flexbox15.png" alt="flex-shrink в Flexbox и CSS3" />
<p>В данном случае начальная ширина каждого элемента равна 200px, то есть совокупная ширина составляет 600px. Однако ширина flex-контейнера составляет всего 
400px. То есть размер контейнера недостаточен для вмещения в него элементов, поэтому в действие вступает свойство <code>flex-shrink</code>, которое определено у элементов.</p>
<p>Для усечения элементов браузер вычисляет коэффициент усечения (shrinkage factor). Он вычисляется путем перемножения значения свойства 
<code>flex-basis</code> на <code>flex-shrink</code>. Таким образом, для трех элементов мы получим следующие вычисления:</p>
<pre class="brush:xml;">
// первый элемент
200px * 1 = 200
// второй элемент
...
<p>Таким образом, мы получаем, что для второго элемента коэффициент усечения в два раза больше, чем коэффициент для первого элемента. А для третьего элемента коэффициент больше в три раза, 
чем у первого элемента. Поэтому в итоге первый элемент при усечении будет в три раза больше, чем третий и в два раза больше, чем второй.</p>
<h3>flex-grow</h3>
<p>Свойство <span class="b">flex-grow</span> управляет расширением элементов, если во flex-контейнере есть дополнительное место. Данное свойство во многом 
похоже на свойство <code>flex-shrink</code> за тем исключением, что работает в сторону увеличения элементов.</p>
<p>В качестве значения свойство flex-grow принимает положительное число, которое указывает, во сколько раз элемент будет увеличиваться относительно 
других элементов при увеличении размеров flex-контейнера. По умолчанию свойство flex-grow равно 0.</p>
<p>Итак, используем свойство flex-grow:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Итак, для каждого элемента есть базовые начальные размеры. Здесь явным образом размеры для элементов не указаны, поэтому размер каждого элемента 
в данном случае будет складываться из размеров внутреннего содержимого, к которым добавляются внутренние отступы.</p>
<img src="pics/flexbox16.png" alt="flex-grow в flexbox и css3" />
<p>По мере растягивания контейнера будут увеличиваться элементы в соответствии со свойством flex-grow, которое указано для каждого элемента. 
Пространство, на которое растягивается контейнер, считается дополнительным пространством.</p>
<img src="pics/flexbox17.png" alt="flex-grow и растягивание элементов в flexbox и css3" />
<p>Так как у первого элемента свойство flex-grow равно 0, то первый элемент будет иметь константные постоянные размеры. 
...
2/3 дополнительного пространства.</p>
<h3>Свойство flex</h3>
<p>Свойство <span class="b">flex</span> является объединением свойств <code>flex-basis</code>, <code>flex-shrink</code> и <code>flex-grow</code> и имеет следующий формальный синтаксис:</p>
<pre class="brush:css;">flex: [flex-grow] [flex-shrink] [flex-basis]</pre>
<p>По умолчанию свойство <code>flex</code> имеет значение <code>0 1 auto</code>.</p>
<p>Кроме конкретных значений для каждого из подсвойств мы можем задать для свойства flex одно из трех общих значений:</p>
<ul>
<li><p><code>flex: none</code>: эквивалентно значению <code>0 0 auto</code>, при котором flex-элемент не растягивается и не усекается при увеличении и уменьшении контейнера</p></li>
<li><p><code>flex: auto</code>: эквивалентно значению <code>1 1 auto</code></p></li>
<li><p><code>flex: initial</code>: эквивалентно значению <code>0 1 auto</code></p></li>
</ul>
<p>Так, применим свойство flex:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/flexbox18.png" alt="Flex в CSS3" />
<p>Здесь каждый элемент имеет начальную ширину в 150 пикселей, так как у всех элементов свойство <code>flex-basis</code> имеет значение 0, что в целом для всех элементов будет составлять 600 пикселей.</p>
<p>При сжатии контейнера будут уменьшаться 3-й и 4-й элементы, так как у них свойство <code>flex-shrink</code> больше нуля. И так как у обоих элементов это свойство равно 1, 
то оба элемента будут уменьшаться в равных долях.</p>
<p>При растяжении контейнера будут увеличиваться 2-й и 4-й элементы, так как у этих элементов свойство <code>flex-grow</code> больше нуля. И также, так как это свойство равно 1, то 
эти элементы будут увеличиваться в равных долях.</p>
