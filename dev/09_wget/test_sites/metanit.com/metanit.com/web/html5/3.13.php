...
<html >
<head>
<title>HTML5 | Валидация форм</title>
<meta charset="utf-8" />
<meta name="description" content="Валидация форм в HTML5, проверка вводимых значений, использование атрибутов reqired, min, max, pattern">
...
<h2>Валидация форм</h2><div class="date">Последнее обновление: 08.04.2016</div>
...
<p>Итак, в нашем распоряжении имеются различные элементы, которые мы можем использовать на форме. Мы можем вводить в них различные значения. Однако нередко 
...
ввода в HTML5 существует механизм валидации.</p>
<p>Преимущество использования валидации в HTML5 заключается в том, что пользователь после некорректного ввода может сразу получить сообщение об ошибке и внести 
соответствующие изменения в введенные данные.</p>
<p>Для создания валидации у элементов форм HTML5 используется ряд атрибутов:</p>
<ul>
<li><p><span class="b">required</span>: требует обязательного ввода значения. Для элементов textarea, select, input (с типом text, password, checkbox, radio, 
file, datetime-local, date, month, time, week, number, email, url, search, tel)</p></li>
<li><p><span class="b">min</span> и <span class="b">max</span>: минимально и максимально допустимые значения. Для элемента input с типом datetime-local, 
date, month, time, week, number, range</p></li>
<li><p><span class="b">pattern</span>: задает шаблон, которому должны соответствовать вводимые данные. Для элемента input с типом text, password, email, url, search, tel</p></li>
</ul>
<h3>Атрибут required</h3>
<p>Атрибут required требует обязательного наличия значения:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<p>Если мы не введем в эти поля никаких данных, оставив их пустыми, и нажмем на кнопку отправки, то браузер высветит нам сообщения об ошибке, а данные не будут отправлены на сервер:</p>
<img src="pics/2.30.png" alt="Атрибут required в HTML5" />
<p>В зависимости от браузера визуализация сообщения может несколько отличаться. Также границы некорректного поля ввода могут окрашиваться в красный цвет. Например, поведение при отправке 
пустых сообщений в Firefox:</p>
<img src="pics/2.31.png" alt="Валидация HTML5 в Firefox" />
<h3>Атрибуты max и min</h3>
<p>Для ограничения диапазона вводимых значений применяются атрибуты max и min:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/2.32.png" alt="Ограничение диапазона чисел в HTML5" />
<h3>Атрибут pattern</h3>
<p>Атрибут pattern задает шаблон, которому должны соответствовать данные. Для определения шаблона используется язык так называемых 
<a href="//metanit.com/web/javascript/6.4.php">регулярных выражений</a>. Рассмотрим самые простейшие примеры:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<p>Здесь для ввода номера телефона используется регулярное выражение <code>\+\d-\d{3}-\d{3}-\d{4}</code>. Оно означает, что первым элементом в номере 
должен идти знак плюс +. Выражение <span class="b">\d</span> представляет любую цифру от 0 до 9. Выражение <code>\d{3}</code> означает три подряд идущих цифры, а 
<code>\d{4}</code> - четыре цифры подряд. То есть это выражение будет соответствовать номеру телефона в формате "+1-234-567-8901".</p>
<p>Если мы введем данные, которые не соответствуют этому шаблону, и нажмем на отправку, то браузер отобразит ошибку:</p>
<img src="pics/2.33.png" alt="Регулярные выражения в HTML5" />
<h3>Отключение валидации</h3>
<p>Не всегда валидация является желаемой, иногда требуется ее отключить. И в этом случае мы можем использовать либо у элемента формы атрибут <span class="b">novalidate</span>, 
либо у кнопки отправки атрибут <span class="b">formnovalidate</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
