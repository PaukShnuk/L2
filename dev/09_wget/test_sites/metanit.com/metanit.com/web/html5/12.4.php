...
<html >
<head>
<title>HTML5 | flex-flow. Порядок элементов</title>
<meta charset="utf-8" />
<meta name="description" content="Свойство flex-flow как объединение свойств flex-direction и flex-wrap, порядок элементов во Flexbox в HTML5 и CSS3">
...
<h2>flex-flow. Порядок элементов</h2><div class="date">Последнее обновление: 18.04.2017</div>
...
<h3>flex-flow</h3><p>Свойство <span class="b">flex-flow</span> позволяет установить значения сразу для обоих свойств <code>flex-direction</code> и <code>flex-wrap</code>. 
Оно имеет следующий формальный синтаксис:</p>
<pre class="brush:css;">
flex-flow: [flex-direction] [flex-wrap]
</pre>
<p>Причем второе свойство - flex-wrap можно в принципе опустить, тогда для него будет использоваться значение по умолчанию - <code>nowrap</code>.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Свойство <span class="b">order</span> позволяет установить группу для flex-элемента, позволяя тем самым переопределить его позицию внутри flex-контейнера. 
В качестве значения свойство принимает числовой порядок группы. К одной группе может принадлежать несколько элементов.</p>
<p>Например, элементы в группе 0 располагаются перед элементами с группой 1, а элементы с группой 1 располагаются перед элементами с группой 2 и так далее.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>В данном случае определены 3 группы. Первый отображается последний элемент, так как он имеет группу -1:</p>
<img src="pics/flexbox8.png" alt="order и flex-flow в Flexbox и CSS3" />
<p>По умолчанию если у элементов явным образом не указано свойство order, то оно имеет значение 0. И последними в данном случае отображаются второй и 
третий элемент, так как у них свойство order равно 1.</p>
