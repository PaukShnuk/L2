...
<html >
<head>
<title>HTML5 | Макет страницы на Flexbox</title>
<meta charset="utf-8" />
<meta name="description" content="Создание сложного макета страницы на Flexbox в HTML5 и CSS3, определение шапки, футера, сайдбара и блока навигации, адаптивная верстка макета">
...
<h2>Макет страницы на Flexbox</h2><div class="date">Последнее обновление: 19.04.2017</div>
...
<p>Теперь рассмотрим создание стандартного макета страницы, который состоит их шапки, футера и центральной части, в которой есть три столбца: основное содержимое и 
два сайдбара.</p>
<p>Для этого определим следующую веб-страницу:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/flexbox25.png" alt="Макет верстки на flexbox в HTML5 и CSS3" />
<p>Итак, flex-контейнером верхнего уровня здесь является элемент body. Его flex-элементами являются header, main и footer. Body располагает все свои элементы сверху вниз в столбик. 
Здесь также стоит отметить, что при ширине от 600px и выше для заполнения всего пространства браузера у body устанавливается стиль 
<code>height: 100vh;</code>.</p>
<p>Элементы header и footer аналогичны. Их свойство <code>flex: 0 0 5em;</code> указывают, что при любом изменении контейнера эти элементы будут иметь размер 
в 5em. То есть они имеют статический размер.</p>
<p>Более сложным является элемент main, который определяет основное содержимое. При этом будучи flex-элементом, он также является flex-контейнером для 
вложенных элементов и управляет их позиционированием. При ширине браузера до 600px он располагает элементы в столбик, что очень удобно на мобильных устройствах.</p>
<p>При ширине от 600px вложенные элементы nav, article и aside располагаются в виде строки. И поскольку при такой ширине браузера родительский элемент body 
заполняет по высоте  все пространство браузера, то для заполнения всей высоты контейнера body при его изменении у элемента main устанавливается свойство 
<code>flex: 1 1 auto;</code>.</p>
<p>У вложенных в main flex-элементов стоит отметить, что элемент навигации nav и элемент сайдбара aside будут иметь одинаковые размеры при масштабировании контейнера. 
А элемент article, содержащий основное содержимое, будет соответственно больше. При этом хотя nav определен после элемента article, но благодаря 
установке свойства <code>order: -1</code> блок навигации будет стоять до блока article.</p>
...
<div class="nav"><p><a href="12.9.php">Назад</a><a href="./">Содержание</a><a href="13.1.php">Вперед</a></p></div></div>
<div class="socBlock">
