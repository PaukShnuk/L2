...
<html >
<head>
<title>HTML5 | Элементы группировки</title>
<meta charset="utf-8" />
<meta name="description" content="Элементы группировки в HTML5, использование блоков div, span, параграфы">
...
<h2>Элементы группировки</h2><div class="date">Последнее обновление: 08.04.2016</div>
...
<p>Ряд элементов предназначен для группировки контента на веб-странице. </p>
<h3>Элемент div</h3>
<p>Элемент <span class="b">div</span> служит для структуризации контента на веб-странице, для заключения содержимого в отдельные блоки. Div создает блок, 
который по умолчанию растягивается по всей ширине браузера, а следующий после div элемент переносится на новую строку. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/1.7.png" alt="Блоки div в html5" />
<h3>Параграфы</h3>
<p>Параграфы создаются с помощью тегов <span class="b">&lt;p&gt;</span> и <span class="b">&lt;/p&gt;</span>, которые заключают некоторое содержимое. 
Каждый новый параграф располагается на новой строке. Применим параграфы:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/1.8.png" alt="Параграфы в html5" />
<p>Если в рамках одного параграфа нам надо перенести текст на другую строку, то мы можем воспользоваться элементом <span class="b">&lt;br&gt;</span>:</p>
<pre class="brush:xml;">&lt;p&gt;Первая строка.&lt;br/&gt;Вторая строка.&lt;/p&gt;</pre>
<h3>Элемент pre</h3>
<p>Элемент <span class="b">pre</span> выводит предварительно отформатированный текст так, как он определен:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/1.37.png" alt="Элемент pre в HTML5" />
<h3>Элемент span</h3>
<p>Элемент <span class="b">span</span> обтекает некоторый текст по всей его длине и служит преимущественно для стилизации заключенного в него текстового содержимого. В отличие от 
блоков div или параграфов <code>span</code> не переносит содержимое на следующую строку:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/1.9.png" alt="Элемент span в html5" />
<p>При этом стоит отметить, что сам по себе <code>span</code> ничего не делает. Так, во втором параграфе <code>span</code> никак не повлиял на внутренне текстовое 
содержимое. А в первом параграфе элемент <code>span</code> содержит атрибут стиля: <code>style="color:red;"</code>, который устанавливает для вложенного текста 
красный цвет фона.</p>
<p>При этом стоит отметить, что элементы <code>div</code> и <code>p</code> являются блочными, элемент div может содержать любые другие элементы, а элемент p - только строчные элементы. 
В отличие от них элемент <code>span</code> является строчным, то есть как бы встраивает свое содержимое во внешний контейнер - тот же div или параграф. Но при 
этом не следует помещать блочные элементы в строчный элемент span.</p>
