...
<html >
<head>
<title>CSS3 | Переменные</title>
<meta charset="utf-8" />
<meta name="description" content="Переменные в CSS3, их определение, функция var, резервные значения для переменных">
...
<h1>Переменные в CSS</h1><h2>Стилизация с помощью переменных</h2><div class="date">Последнее обновление: 18.11.2018</div>
...
<p>Как и в языках программирования, в CSS можно определять переменные. Переменные в CSS могут хранить стандартные значения, которые можно присвоить, 
обычным свойствам CSS, например, цвет фона, цвет шрифта, высоту шрифта, ширину и высоту элементов и так далее. Затем их можно многократно использовать 
в различных частях определения стилей.</p>
<p>Стоит отметить, что хотя часто используется термин "переменные" (css variables), официально они называются <span class="b">custom properties</span> 
(кастомные или настраиваемые свойства).</p>
<p>Определение переменных CSS должно начинаться с префикса <span class="b">--</span>, например, <code>--my-color</code>. Они могут быть определены для любого элемента. 
Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<p>Здесь в коде CSS для элемента div определены две переменныx: <code>--text-color</code> и <code>--text-size</code></p>
<pre class="brush:css;">
--text-color: #ff4757;
--text-size: 20px;
</pre>
<p>С помощью выражения <span class="b">var()</span> мы можем ссылаться на эти переменные в любой части кода CSS:</p>
<pre class="brush:css;">
#p1 {
	color: var(--text-color);
	font-size: var(--text-size);
...
<p>В итоге первый блок получит высоту шрифта из --text-size, второй блок получит цвет шрифта из --text-color, а третий блок получит оба этих компонента.</p>
<img src="pics/cssvariables1.png" alt="Custom properties and variables in CSS" />
<p>Если нам потребуется изменить цвет текста, достаточно будет изменить значение переменной.</p>
<p>Подобные переменные можно определить для любого элемента. При этом они наследуется дочерними элементами. Так, в примере выше мы видим, 
что вложенный элемент параграфа получает стили от родительского div.</p>
<p>Но если бы параграф находился бы вне блока div:</p>
<pre class="brush:xml;">
&lt;div id="div1"&gt;Первый блок.&lt;/div&gt;
&lt;div id="div2"&gt;Второй блок.&lt;/div&gt;
&lt;p id="p1"&gt;Третий блок.&lt;/p&gt;
</pre>
<p>Тогда бы значения из переменных к нему бы не применялись:</p>
<img src="pics/cssvariables2.png" alt="Переменные в CSS" />
<p>Если же необходимо, чтобы переменные могли бы использоваться глобально для всех элементов, тогда их определяют для элемента <span class="b">:root</span></p>
<pre class="brush:css;">
:root{
	--text-color: #ff4757;
	--text-size: 20px;
...
<h3>Резервные значения</h3>
<p>При определения переменных могут содержать какие-нибудь ошибки, например, переменная не определена (вообще не определена или в ее названии опечатка). 
В этом случае мы можем указать при применении переменной резервные значения, которые бдут применяться в случае, если значение переменной некорректно.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Второй параметр в функции <code>var()</code> позволяет задать резервное значение. Это может быть как точное значение, так и опять же значение другой переменной:</p>
<pre class="brush:css;">color: var(--text-color, var(--reserved-color));</pre>
<p>Для второй переменной можно также определить резервное значение, если она некорректна:</p>
<pre class="brush:css;">color: var(--text-color, var(--reserved-color, red));</pre>
...
<div class="nav"><p><a href="13.12.php">Назад</a><a href="./">Содержание</a><a href="14.2.php">Вперед</a></p></div></div>
<div class="socBlock">
