...
<html >
<head>
<title>CSS3 | Фон элемента</title>
<meta charset="utf-8" />
<meta name="description" content="Создание фона у элементов в CSS 3, свойства background и background-image, установка и настройка фоноого изображения">
...
<h2>Фон элемента</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Фон элемента описывается в CSS свойством <span class="b">background</span>. Фактически это свойство представляет сокращение набора следующих свойств CSS:</p>
<ul>
<li><p><span class="b">background-color</span>: устанавливает цвет фона</p>
<pre class="brush:css;">
background-color: #ff0507;
</pre>
</li>
<li><p><span class="b">background-image</span>: в качестве фона устанавливается изображение</p>
<pre class="brush:css;">
background-image: url(dubi.png);
</pre>
<p>Это свойство принимает одно значение: ключевое слово url, после которого в скобках идет путь к файлу изображения. В данном случае имеется в виду, 
что в одной папке рядом с веб-страницей находится файл <code>dubi.png</code>.</p>
<p>Также можно использовать абсолютные адреса URL, например:</p>
<pre class="brush:css;">
background-image: url(http://localhost.com/someimage.png);
</pre>
<p>Либо можно использовать относительные адреса - относительно html-документа или корневого каталога сайта:</p>
<pre class="brush:css;">
background-image: url(../images/someimage.png);	/* путь относительно html-документа */
</pre>
</li>
<li><p><span class="b">background-repeat</span>: устанавливает режим повторения фонового изображения по всей поверхности элемента</p></li>
<li><p><span class="b">background-size</span>: устанавливает размер фонового изображения</p></li>
<li><p><span class="b">background-position</span>: указывает позицию фонового изображения</p></li>
<li><p><span class="b">background-attachment</span>: устанавливает стиль прикрепления фонового изображения к элементу</p></li>
<li><p><span class="b">background-clip</span>: определяет область, которая вырезается из изображения и используется в качестве фона</p></li>
<li><p><span class="b">background-origin</span>: устанавливает начальную позицию фонового изображения</p></li>
</ul>
<p>Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
...
<p>Первый блок окрашен в оттенок красного цвета, а второй блок устанавливает в качестве фона изображение. Все содержимое блока накладывается поверх фона:</p>
<img src="pics/4.48.png" alt="Свойство background в CSS 3" />
<h3>Повторение изображения</h3>
<p>На выше приведенном скриншоте видно, что CSS должным образом масштабирует изображение, чтобы наиболее оптимально вписать его в пространство элемента. 
...
CSS начинает повторять изображение.</p>
<p>С помощью свойства <span class="b">background-repeat</span> можно изменить механизм повторения. Оно может принимать следующие значения:</p>
<ul>
<li><p><code>repeat-x</code>: повторение по горизонтали</p></li>
<li><p><code>repeat-y</code>: повторение по вертикали</p></li>
<li><p><code>repeat</code>: повторение по обеим сторонам (действие по умолчанию)</p></li>
<li><p><code>space</code>: изображение повторяется для заполнения всей поверхности элемента, но без создания фрагментов</p></li>
<li><p><code>round</code>: изображение должным образом масштабируется для полного заполнения всего пространства</p></li>
<li><p><code>no-repeat</code>: изображение не повторяется</p></li>
</ul>
<p>Например:</p>
<pre class="brush:css;">
div{
...
<p>Свойство <span class="b">background-size</span> позволяет установить размер фонового изображения. Для установки размера можно использовать либо 
единицы измерения, например, пиксели, либо проценты, либо одно из предустановленных значений:</p>
<ul>
<li><p><code>contain</code>: масштабирует изображение по наибольшей стороне, сохраняя аспектное отношение</p></li>
<li><p><code>cover</code>: масштабирует изображение по наименьшей стороне, сохраняя аспектное отношение</p></li>
<li><p><code>auto</code>: значение по умолчанию, изображение отображается в полный размер</p></li>
</ul>
<p>Если нужно масштабировать изображение таким образом, чтобы оно оптимальнее было вписано в фон, то для обеих настроек  можно установить 
значение 100%:</p>
<pre class="brush:css;">
background-size: 100% 100%;
</pre>
<p>Если задаются точные размеры, то вначале указывается ширина, а потом высота изображения:</p>
<pre class="brush:css;">
background-size: 200px 150px;	/* ширина 200 пикселей, высота 150 пикселей */
</pre>
<p>Можно задать точное значение для одного измерения - ширины или высоты, а для другого задать автоматические размеры, чтобы браузер сам выводил точные значения:</p>
<pre class="brush:css;">
...
<p>Во втором случае изображение будет масштабироваться до размеров 140х110. Поскольку у нас еще остается место на элементе, то по умолчанию изображение 
будет повторяться для заполнения всей поверхности:</p>
<img src="pics/4.49.png" alt="Свойство background-size в CSS 3" />
<h3>Позиция изображения</h3>
<p>Свойство <span class="b">background-position</span> управляет позицией фонового изображения внутри элемента. Оно может принимать отступы от верхнего левого угла 
элемента в единицах измерения, например, в пикселях в следующем формате:</p>
<pre class="brush:css;">
...
<img src="pics/4.50.png" alt="background-position в CSS 3" />
<p>Кроме того, данное свойство может принимать одно из следующих значений:</p>
<ul>
<li><p><code>top</code>: выравнивание по верхнему краю элемента</p></li>
<li><p><code>left</code>: выравнивание по левому краю элемента</p></li>
<li><p><code>right</code>: выравнивание по правому краю элемента</p></li>
<li><p><code>bottom</code>: выравнивание по нижнему краю элемента</p></li>
<li><p><code>center</code>: изображение располагается по центру элемента</p></li>
</ul>
<p>Например:</p>
<pre class="brush:css;">background-position: top right;	</pre>
<p>Здесь изображение выравнивается по верху и правому краю, то есть будет располагаться в правом верхнем углу элемента.</p>
<h3>background-attachment</h3>
<p>Свойство <span class="b">background-attachment</span> управляет, как фоновое изображение будет прикреплено к элементу. Это свойство может принимать следующие 
значения:</p>
<ul>
<li><p><code>fixed</code>: фон элемента фиксирован вне зависимости от прокрутки внутри элемента</p></li>
<li><p><code>local</code>: по мере прокрутки внутри элемента фон изменяется</p></li>
<li><p><code>scroll</code>: фон фиксирован и не меняется при прокрутке, но в отличие от fixed несколько элементов могут использовать свой фон, 
тогда как при fixed создается один фон для всех элементов</p></li>
</ul>
<p>Например:</p>
<pre class="brush:css;">
div{
...
<p>Свойство <span class="b">background-origin</span> указывает позицию на изображении, с которой будет начинаться собственно фоновое изображение для элемента. Оно может принимать 
следующие значения:</p>
<ul>
<li><p><code>border-box</code>: фон у элемента устанавливается начиная с его внешней границы, определяемой свойством border</p></li>
<li><p><code>padding-box</code>: фон устанавливается с учетом внутренних отступов</p></li>
<li><p><code>content-box</code>: фон устанавливается по содержимому элемента</p></li>
</ul>
<p>Используем все три значения:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/4.51.png" alt="Свойство background-origin в CSS 3" />
<h3>background-clip</h3>
<p>Свойство <span class="b">background-clip</span> определяет, какая часть изображения используется для фона. Он принимает те же значения:</p>
<ul>
<li><p><code>border-box</code>: изображение обрезается по границам элемента</p></li>
<li><p><code>padding-box</code>: из изображения исключается та часть, которая находится под границами элемента</p></li>
<li><p><code>content-box</code>: изображение обрезается по содержимому с учетом внутренних отступов</p></li>
</ul>
<p>Например, если к предыдущей разметке мы применим следующие стили:</p>
<pre class="brush:css;">
div{
	width: 200px;
	height: 200px;
...
<img src="pics/4.52.png" alt="Свойство background-clip в CSS 3" />
<h3>Свойство background</h3>
<p>Свойство background по сути является сокращением всех ранее рассмотренных свойств CSS в формате:</p>
<pre class="brush:css;">
background: &lt;background-color&gt; &lt;background-position&gt; &lt;background-size&gt;
&lt;background-repeat&gt; &lt;background-origin&gt; &lt;background-clip&gt; &lt;background-attachment&gt;
&lt;background-image&gt;
</pre>
<p>Например, если у нас есть следующий набор свойств:</p>
<pre class="brush:css;">
background-image: url(cats.jpg);
background-color: #eee;
background-repeat: no-repeat;
...
<p>То мы их можем сократить следующим образом:</p>
<pre class="brush:css;">
...
<div class="nav"><p><a href="6.13.php">Назад</a><a href="./">Содержание</a><a href="6.15.php">Вперед</a></p></div></div>
<div class="socBlock">
