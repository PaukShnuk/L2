...
<html >
<head>
<title>CSS3 | Стили CSS как хранилище данных</title>
<meta charset="utf-8" />
<meta name="description" content="Создание хранилища данных помощью переменных в CSS3, определение базы данных в стилях CSS">
...
<h2>Стили CSS как хранилище данных</h2><div class="date">Последнее обновление: 21.05.2021</div>
...
<p>Применение переменных в CSS не ограничивается настройкой стилизации. Их предназначение более широко - а именно хранение состояния. 
Благодаря этому мы даже можем определить с помощью стилей CSS и переменных своеобразную базу данных или хранилище данных, которое может в определенных сценариях 
применяться для хранения данных веб-страницы.</p>
<p>Например, определим файл стилей, который назовем <span class="b">user.css</span> со следующим содержимым:</p>
<pre class="brush:css;">
.user {
	--name: "Tom";
	--age: "37";
...
<p>Здесь для класса <code>user</code> определены четыре переменных, которые хранят некоторые значения. В данном случае все значения представляют строки.</p>
<p>Теперь определим следующую html-страницу, на которой будет подключаться вышеопределенный файл стилей:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Для вывода данных на страницу применяется элемент <code>&lt;div class="user"&gt;</code>, в котором определены четыре html-элемента: один заголовок h2 и три параграфа. И для каждого элемента 
определен свой класс.</p>
<p>Чуть выше на странице в элементе <code>&lt;style&gt;</code> определяются стили для этих элементов, которые применяют переменные из подключаемого файла <span class="b">user.css</span>. Все эти стили 
однотипны, они устанавливают текст элемента, который добавляется после уже имеющегося текстового содержимого. Например:</p>
<pre class="brush:css;">.user-name:after { content: var(--name); }</pre>
<p><span class="b">::after</span> создает псевдоэлемент в конце html-элемента (в данном случае в конце элемента с классом <code>user-name</code>). 
И у этого псевдоэлемента в качестве содержимого устанавливается текст из переменной <code>--name</code>.</p>
<p>Стили для остальных элементов идентичны.</p>

<p>Таким образом, мы ожидаем, что значения, которые храняться в файле <span class="b">user.css</span>, будут выводиться на веб-страницу. 
Кинем странице в браузер и увидем на ней данные из подключеннного файла css:</p>
<img src="./pics/cssvariables4.png" alt="Переключение тем в CSS и HTML" />
<p>Хранение текстовых данных и вывод их в текстовые блоки на веб-страницу, естественно, это только частный случай. В более широком смысле переменные CSS позволяют хранить 
некоторое состояние, которое применяется к html-странице.</p>
