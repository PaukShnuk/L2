...
<html >
<head>
<title>CSS3 | Селекторы потомков</title>
<meta charset="utf-8" />
<meta name="description" content="Селекторы потомков в CSS 3, стилизация вложенных элементов">
...
<h2>Селекторы потомков</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Веб-страница может иметь сложную организацию, одни элементы внутри себя могут определять другие элементы. Вложенные элементы иначе 
можно назвать потомками. А контейнер этих элементов - родителем.</p>
<p>Например, пусть элемент body на веб-странице имеет следующее содержимое:</p>
<pre class="brush:xml;">
&lt;body&gt;
	&lt;h2&gt;Заголовок&lt;/h2&gt;
	&lt;div&gt;
...
<p>Внутри элемента body определено три вложенных элемента: h2, div, p. Все эти элемены являются потомками элемента body.</p>
<p>А внутри элемента div определен только один вложенный элемент - p, поэтому элемент div имеет только одного потомка.</p>
<p>Используя специальные селекторы, мы можем стилизовать вложенные элементы или потомков внутри строго определенных элементов. Например, у нас 
на странице могут быть параграфы внутри блока с основным содержимым и внутри блока 
футера. Но для параграфов внутри блока основного содержимого мы захотим установить один шрифт, а для параграфов футера другой.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/4.6.png" alt="Селекторы вложенных элементов в CSS3" />
<p>Для применения стиля к вложенному элементу селектор должен содержать вначале родительский элемент и затем вложенный:</p>
<pre class="brush:css;">
#main p{
	font-size: 16px;
}
</pre>
<p>То есть данный стиль будет применяться только к тем элементам p, которые находятся внутри элемента с идентификатором main.</p>
...
<p>Здесь стиль применяется к элементам с классом "redLink", которые находятся внутри элемента <code>&lt;li&gt;</code>. И соответственно браузер 
окрасит эти элементы в красный цвет:</p>
<img src="pics/4.7.png" alt="Стилизация дочерних элементов в CSS3" />
<p>Но обратите внимание на пробел: <code>li .redLink</code>. Данный пробел играет большое значение и указывает как раз, что элементы с классом 
<code>redLink</code> должны быть вложенными по отношению к элементу <code>&lt;li&gt;</code></p>
<p>Но если мы уберем пробел:</p>
<pre class="brush:css;">
li.redLink{
	color: red;
}
</pre>
<p>то смысл селектора изменится. Теперь будет подразумеваться, что стиль применяется к элементам <code>&lt;li&gt;</code>, которые имеют класс <code>redLink</code>. 
Например, к следующему элементу:</p>
<pre class="brush:xml;">
&lt;li class="redLink"&gt;Microsoft: &lt;a&gt;Lumia 650&lt;/a&gt;&lt;/li&gt;
</pre>
<p>Но никак не к элементу:</p>
<pre class="brush:xml;">
&lt;li&gt;LG: &lt;a class="redLink"&gt;LG G5&lt;/a&gt;&lt;/li&gt;
</pre>
