...
<html >
<head>
<title>HTML5 | Фоновые изображения в Canvas</title>
<meta charset="utf-8" />
<meta name="description" content="Вывод изображений на Canvas в HTML5, принцип загрузки изображения и метод createPattern">
...
<h2>Фоновые изображения</h2><div class="date">Последнее обновление: 02.05.2016</div>
...
<p>Вместо конкретного цвета для заливки фигур, например, прямоугольников, мы можем использовать изображения. Для этого у кконтекста canvas имеется функция 
<span class="b">createPattern()</span>, которая принимает два параметра: изображение, которое будет использоваться в качестве фона, и принцип повторения 
...
<ul>
<li><p><code>repeat</code>: изображение повторяется для заполнения всего пространства фигуры</p></li>
<li><p><code>repeat-x</code>: изображение повторяется только по горизонтали</p></li>
<li><p><code>repeat-y</code>: изображение повторяется только по вертикали</p></li>
...
<p>Чтобы использовать изображение, нам надо создать элемент Image и установить источник изображения - локальный файл или ресурс в сети:</p>
<pre class="brush:js;">
var img = new Image();
img.src = "dubi.png";
</pre>
<p>В данном случае предполагается, что в одной папке с файлом html у меня находится файл изображения <i>dubi.png</i>. Однако загрузка изображения 
...
все действия по его использованию производятся в методе <code>img.onload</code>, который вызывается при завершении загрузки изображения:</p>
<pre class="brush:js;">
img.onload = function() {
...
<p>Метод <code>createPattern()</code> возвращает объект, который устанавливается в качестве стиля заполнения фигуры: <code>context.fillStyle = pattern;</code>. 
Отрисовка прямоугольника остается той же.</p>
<img src="pics/6.3.png" alt="Изображение как фон в canvas в HTML5" />
...
<div class="nav"><p><a href="10.9.php">Назад</a><a href="./">Содержание</a><a href="10.3.php">Вперед</a></p></div></div>
<div class="socBlock">
