...
<html >
<head>
<title>CSS3 | Цвета</title>
<meta charset="utf-8" />
<meta name="description" content="Цвета в CSS 3, форматы определения цветов, шестнадцатеричное значение, rgb, rgba, свойства color, background-color, border-color">
...
<h2>Цвет в CSS</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>В CSS широкое распространение находит использование цветов. Чтобы установить цвет текста, фона или границы, нам надо указать цвет.</p>
<p>Например, определим красный цвет для фона элемента div:</p>
<pre class="brush:css;">
div{
	background-color: red;
}
</pre>
<p>В CSS есть несколько различных свойств, которые в качестве значения требует определенный цвет. Например, за установку цвета текста отвечает 
свойство <span class="b">color</span>, за установку фона элемента - свойство <span class="b">background-color</span>, а за установку цвета границы - 
<span class="b">border-color</span>.</p>
<p>Существует несколько различных способов определения цвета текста.</p>
<ul>
<li><p><span class="b">Шестнадцатеричного значение</span>. Оно состоит из отдельных частей, которые кодируют в шестнадцатеричной системе 
значения для красного, зеленого и синего цветов.</p>
<p>Например, <span class="b">#1C4463</span>. Здесь первые два символа <code>1C</code> представляю значение красной компоненты цвета, далее 
<code>44</code> - значение зеленой компоненты цвета и <code>63</code> - значение уровня синего цвета. Финальный цвет, который мы видим на веб-странице, 
образуется с помощью смешивания этих значений.</p>
<p>Если каждое из трех двухзначных чисел содержит по два одинаковых символа, то их можно сократить до одного. Например, 
<code>#5522AA</code> можно сократить до <code>#52A</code>, или, к примеру, <code>#eeeeee</code> можно сократить до <code>#eee</code>. При этом 
не столь важно, в каком регистре будут символы.</p></li>

<li><p><span class="b">Значение RGB</span>. Значение RGB также представляет последовательно набор значений для красного, зеленого и синего цветов 
(Red — красный, Green — зеленый, Blue — синий). Значение каждого цвета кодируется тремя числами, которые могут представлять либо 
процентные соотношения (0–100%), либо число от 0 до 255.</p>
<p>Например</p>
<pre class="brush:css;">background-color: rgb(100%,100%,100%);</pre>
<p>Здесь каждый цвет имеет значение <code>100%</code>. 
И в итоге при смешивании этих значений будет создаваться белый цвет. А при значениях в 0% будет генерироваться черный цвет:</p>
<pre class="brush:css;">background-color: rgb(0%, 0%, 0%);</pre>
<p>Между 0 и 100% будут находиться все остальные оттенки.</p>
<p>Но, как правило, чаще применяются значения из диапазона от 0 до 255. Например,</p>
<pre class="brush:css;">background-color: rgb(28, 68, 99);</pre>
</li>

<li><p><span class="b">Значение RGBA</span>. Это тоже самое значение RGB плюс компонент прозрачности (Alpha). Компонент прозрачности 
имеет значение от 0 (полностью прозрачный) до 1 (не прозрачный). Например:</p>
<pre class="brush:css;">background-color: rgba(28, 68, 99, .6);</pre>
</li>

<li><p><span class="b">Значение HSL</span>. HSL представляет аббривиатуру: Hue — тон, Saturation — насыщенность и Lightness — осветленность. 
HSL задает три значения. Первое значение Hue угол в круге оттенков - значение в градусах от 0 до 360. 
Например, красный — 0 (или 360 при полном обороте круга). Каждый цвет занимает примерно 51°.</p>
<p>Второе значение - Saturation - представляет насыщенность, то указывает, насколько чистым является цвет. Насыщенность определяется в 
процентах от 0 (полное отсутствие насыщенности) до 100% (яркий, насыщенный цвет).</p>
<p>Третье значение - Lightness - определяет осветленность и указывается в процентах от 0 (полностью черный) до 100 (полностью белый). 
Для получения чистого цвет применяется значение 50 %.</p>
<p>Например:</p>
<pre class="brush:css;">
background-color: hsl(206, 56%, 25%);
</pre>
<p>Данный цвет является эквивалентом значений <code>#1C4463</code> и <code>rgb(28, 68, 99)</code></p>
</li>
<li><p><span class="b">Значение HSLA</span>. Аналогично RGBA здесь к HSL добавляется компонента прозрачности в виде значения от 0 (полностью прозрачный) до 1 (не прозрачный). Например:</p>
<pre class="brush:css;">background-color: hsl(206, 56%, 25%, .6);</pre>
</li>
<li><p><span class="b">Строковые значения</span>. Существует ряд константных строковых значений, например, red (для красного цвета) или green 
(для зеленого цвета). К примеру,</p>
<pre class="brush:css;">color: red;</pre>
<p>является эквивалентом</p>
<pre class="brush:css;">color: #ff0000;</pre>
<p>Полный перечень цветов можно найти на странице <a href="https://developer.mozilla.org/en-US/docs/Web/CSS/color_value" rel="nofollow">https://developer.mozilla.org/en-US/docs/Web/CSS/color_value</a></p></li>
</ul>
<p>В заключение следует отметить, что существует множество бесплатных онлайн-генераторов цвета, где можно настроить и посмотреть цвет в нужном формате. 
Например, <a href="https://developer.mozilla.org/ru/docs/Web/CSS/CSS_Colors/Color_picker_tool" rel="nofollow">генератор цвета на mdn</a>.</p>
<h3>Прозрачность</h3>
<p>Ряд настроек цвета позволяют установить значение для альфа-компоненты, которая отвечает за прозрачность. Но также в CSS есть специальное свойство, 
которое позволяет установить прозрачность элементов - свойство <span class="b">opacity</span>. В качестве значения оно принимает число от 0 (полностью прозрачный) 
до 1 (не прозрачный):</p>
<pre class="brush:css;">
div{
	width: 100px;
	height: 100px;
...
<div class="nav"><p><a href="./5.13.php">Назад</a><a href="./">Содержание</a><a href="./6.2.php">Вперед</a></p></div></div>
<div class="socBlock">
