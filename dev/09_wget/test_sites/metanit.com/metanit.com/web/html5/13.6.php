...
<html >
<head>
<title>HTML5 | Позиционирование элементов в Grid Layout</title>
<meta charset="utf-8" />
<meta name="description" content="Позиционирование элементов в ячейках в Grid Layout в CSS3, свойства grid-row-start, grid-row-end, grid-column-start, grid-column-end, grid-area">
...
<h2>Позиционирование элементов</h2><div class="date">Последнее обновление: 09.05.2017</div>
...
<p>Грид представляет собой набор ячеек, которые образуются на пересечении столбцов и строк. Но сами строки и столбцы образуются с помощью 
grid-линий, которые рассекают грид по вертикали и горизонтали:</p>
<img src="pics/grid20.png" alt="Grid-lines in Grid Layout CSS 3" />
<p>И по умолчанию каждый элемент в гриде позиционируется в одну ячейку по порядку. Но мы можем более точно настроить расположение элемента в гриде с помощью ряда свойств:</p>
<ul>
<li><p><span class="b">grid-row-start</span>: задает начальную горизонтальную grid-линию, с которой начинается элемент</p></li>
<li><p><span class="b">grid-row-end</span>: указывает, до какой горизонтальной grid-линии надо растягивать элемент</p></li>
<li><p><span class="b">grid-column-start</span>: задает начальную вертикальную grid-линию, от которой начинается элемент</p></li>
<li><p><span class="b">grid-column-end</span>: указывает, до какой вертикальной grid-линии нужно растягивать элемент</p></li>
</ul>
<p>Например, растянем элемент на несколько столбцов:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/grid5.png" alt="Растяжение элементов на столбцы в Grid Layout в CSS3" />
<p>Здесь определено четыре столбца, причем второму элементу присвоен особый класс special-item, который располагается, начиная со 2 grid-линии или 2-го столбца (<code>grid-column-start: 2</code>) 
до 5-й вертикальной grid-линии (<code>grid-column-end: 5</code>).</p>
<p>Второй элемент необязательно должен начинаться со второго столбца, это может быть любой другой: и 1-й, и 3-й и т.д. Например, если мы поместим второй элемент, 
начиная с 3-го столбца, то на месте второго столбца образуется пустота:</p>
<pre class="brush:css;">
.special-item{
	grid-column-start: 3;
	grid-column-end: 5;
...
<img src="pics/grid6.png" alt="grid-column-start в Grid Layout в CSS3" />
<p>Если для второго элемента установить начало с первого столбца, то второй элемент будет перенесен на следующую строку, и таким образом, он будет начинаться с 
первого столбца.</p>
<p>Вместо использования двух выше рассмотренных свойств мы можем использовать одно свойство - <span class="b">grid-column</span>, которое принимает 
значения grid-column-start и grid-column-end через слеш:</p>
<pre class="brush:css;">
grid-column: grid-column-start / grid-column-end;
</pre>
<p>Например, мы можем сократить стиль класса special-item следующим образом:</p>
<pre class="brush:css;">
.special-item{
	grid-column: 3 / 5;
}
</pre>
<p>Аналогично с помощью свойств <span class="b">grid-row-start</span> и <span class="b">grid-row-end</span> можно задать позиционирование элемента на несколько строк. 
Так, изменим класс special-item следующим образом:</p>
<pre class="brush:css;">
.special-item{
	grid-column-start:2;
	grid-row-start: 1;
...
<p>В данном случае второй элемент позиционируется во втором столбце первой строки и растягивается до 3-й строки.</p>
<img src="pics/grid9.png" alt="grid-row-start в Grid Layout и CSS 3" />
<p>Вместо использования пары свойств grid-row-start и grid-row-end можно использовать одно общее свойство <span class="b">grid-row</span>:</p>
<pre class="brush:css;">grid-row: grid-row-start / grid-row-end;</pre>
<p>Так, мы могли бы изменить стиль special-item следующим образом:</p>
<pre class="brush:css;">
.special-item{
	grid-column-start:2;
	grid-row: 1 / 3;
...
<h3>span</h3>
<p>С помощью специального слова <span class="b">span</span> можно задать растяжение элемента на несколько ячеек. После слова span указывается, на какое 
количество ячеек надо растянуть элемент:</p>
<pre class="brush:css;">
.special-item{
	grid-row: 1 / span 2;
...
<p>Элемент помещается в ячейку, которая находится на пересечении первой строки и второго столбца, и растягивается на две строки вниз и на два столбца вправо.</p>
<h3>grid-area</h3>
<p>Свойство <span class="b">grid-area</span> объединяет свойства <code>grid-column</code> и <code>grid-row</code>, позволяя сократить их запись:</p>
<pre class="brush:css;">grid-area: row-start / column-start / row-end / column-end</pre>
<p>Например, изменим стили класса special-item:</p>
<pre class="brush:xml;">
.special-item{
	grid-area: 1 / 2 / 3 / 4;
}
</pre>
<img src="pics/grid13.png" alt="grid-area в Grid Layout в CSS 3" />
