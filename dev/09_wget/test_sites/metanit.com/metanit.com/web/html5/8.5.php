...
<html >
<head>
<title>CSS3 | Свойство display</title>
<meta charset="utf-8" />
<meta name="description" content="Управление блоком элемента в HTML 5, свойство CSS display и значения block, inline, inline-block, none">
...
<h2>Свойство display</h2><div class="date">Последнее обновление: 27.04.2016</div>
...
<p>Кроме свойства <code>float</code>, которое позволяет изменять позицию элемента, в CSS есть еще одно важное свойство - <span class="b">display</span>. 
Оно позволяет управлять блоком элемента и также влиять на его позиционирование относительно соседних элементов.</p> 
<p>Это свойство может принимать следующие значения:</p>
<ul>
<li><p><code>inline</code>: элемент становится строчным, подобно словам в строке текста</p></li>
<li><p><code>block</code>: элемент становится блочным, как параграф</p></li>
<li><p><code>inline-block</code>: элемент располагается как строка текста</p></li>
<li><p><code>list-item</code>: элемент позиционируется как элемент списка обычно с добавление маркера  виде точки или порядкового номера</p></li>
<li><p><code>run-in</code>: тип блока элемента зависит от окружающих элементов</p></li>
<li><p><code>flex</code>: позволяет осуществлять гибкое позиционирование элментов</p></li>
<li><p><code>table</code>, <code>inline-table</code>: позволяет расположить элементы в виде таблицы</p></li>
<li><p><code>none</code>: элемент не виден и удален из разметки html</p></li>
</ul>
<p>Итак, значение <span class="b">block</span> позволяет определить блочный элемент. 
...
блочными и при визуализации веб-страницы визуально переносятся на новую строку.</p>
<p>Однако элемент <code>span</code> в отличие от элемента div блочным не является. Поэтому посмотрим, какие с ним произойдут изменения при применении значения 
<code>block</code>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<img src="pics/7.11.png" alt="Свойство display в CSS 3" />
<p>Здесь определено два элемента span, но один из них является блочным, так как к нему применяется стиль <code>display: block;</code>. Поэтому этот элемент 
span переносится на новую строку.</p>
<p>В отличие от блочных элементов строчные встраиваются в строку, так как имеют для свойства <code>display</code> значение <span class="b">inline</span>. Элемент 
span как раз по умолчанию имеет стиль <code>display: inline</code>, поэтому и встраивается в строку, а не переносится на следующую, как параграфы или div. 
И теперь произведем обратную процедуру - сделаем блочный элемент <code>div</code> строчным:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/7.12.png" alt="Блочные и строчные элементы в CSS 3" />
<p>Следует учитывать, что при применении значения <code>inline</code> браузер игнорирует некоторые свойства, такие как <code>width</code>, <code>height</code>, <code>margin</code>.</p>
<h3>inline-block</h3>
<p>Еще одно значение - <span class="b">inline-block</span> - представляет элемент, который обладает смесью признаков блочного и строчного элементов. 
...
Однако по отношению к вложенным элементам он рассматривается как блочный. И к такому элементу применяются свойства <code>width</code>, <code>height</code>, 
<code>margin</code>.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<img src="pics/7.13.png" alt="inline-block в CSS 3" />
<p>Первый элемент span является строчным, у него значение <code>inline</code>, поэтому для него бессмысленно применять свойства <code>width</code> и 
<code>height</code>. А вот второй элемент <code>span</code> имеет значение <code>inline-block</code>, поэтому к нему жуе применяются и ширина, и высота, и 
при необходимости еще можно установить отступы.</p>
<h3>run-in</h3>
<p>Значение <span class="b">run-in</span> определяет элемент, который зависит от соседних элементов. И здесь есть три возможных варианта:</p>
<ul>
<li><p>Элемент окружен блочными элементами, тогда фактически он имеет стиль <code>display: block</code>, то есть сам становится блочным<p></li>
<li><p>Элемент окружен строчными элементами, тогда фактически он имеет стиль <code>display: inline</code>, то есть сам становится строчным<p></li>
<li><p>Во всех остальных случаях элемент считается блочным<p></li>
</ul>
<h3>Табличное представление</h3>
<p>Значение <span class="b">table</span> по сути превращает элемент в таблицу. Посмотрим на примере списка:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/7.15.png" alt="display table в CSS 3" />
<p>Здесь список превращается в таблицу, а каждый элемент списка - в отдельную ячейку. Для этого у элемента списка устанавливается стиль <code>display: table-cell</code>. 
Фактически вместо этого списка мы могли бы использовать стандартную таблицу.</p>
<h3>Сокрытие элемента</h3>
<p>Значение <span class="b">none</span> позволяет скрыть элемент, которого как-будто нет на веб-странице:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/7.14.png" alt="display none в CSS 3" />
