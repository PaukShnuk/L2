...
<html >
<head>
<title>HTML5 | Элементы и атрибуты</title>
<meta charset="utf-8" />
<meta name="description" content="Элементы и атрибуты в HTML5, теги элемента, глобальные атрибуты">
...
<h2>Элементы и атрибуты HTML5</h2><div class="date">Последнее обновление: 08.04.2016</div>
...
<p>Прежде чем переходить непосредственно к созданию своих веб-страниц на HTML5, рассмотрим основные строительные блоки, кирпичики, из которых состоит веб-страница.</p>
<p>Документ HTML5, как и любой документ HTML, состоит из элементов, а элементы состоят из тегов. Как правило, элементы имеют открывающий и закрывающий тег, 
которые заключаются в угловые скобки. Например:</p>
<pre class="brush:xml;">
&lt;div&gt;Текст элемента div&lt;/div&gt;
</pre>
<p>Здесь определен элемент <code>div</code>, который имеет открывающий тег <code>&lt;div&gt;</code> и закрывающий тег <code>&lt;/div&gt;</code>. 
Между этими тегами находится содержимое элемента div. В данном случае в качестве содержимого выступает простой текст "Текст элемента div".</p>
<p>Элементы также могут состоять из одного тега, например, элемент <code>&lt;br /&gt;</code>, функция которого - перенос строки.</p>
<pre class="brush:xml;">
&lt;div&gt;Текст &lt;br /&gt; элемента div&lt;/div&gt;
</pre>
<p>Такие элементы еще называют пустыми элементами (void elements). Хотя я использовал закрывающий слеш, но его наличие согласно спецификации необязательно, 
и равнозначно использованию тега без слеша: &lt;br&gt;</p>
<p>Каждый элемент внутри открывающего тега может иметь <span class="b">атрибуты</span>. Например:</p>
<pre class="brush:xml;">
&lt;div style="color:red;"&gt;Кнопка&lt;/div&gt;
&lt;input type="button" value="Нажать"&gt;
</pre>
<p>Здесь определено два элемента: div и input. Элемент <code>div</code> имеет атрибут <span class="b">style</span>. После знака равно в кавычках 
пишется значение атрибута: <code>style="color:red;"</code>. В данном случае значение "color:red;" указывает, что цвет текста будет красным.</p>
<p>Второй элемент - элемент input, состоящий из одного тега, имеет два атрибута: <code>type</code> (указывает на тип элемента - кнопка) и <code>value</code> (определяет 
текст кнопки)</p>
<p>Существуют глобальные или общие для всех элементов атрибуты, как например, style, а есть специфические, применяемые к определенным элементам, как 
например, type.</p>
<p>Кроме обычных атрибутов существуют еще булевые или логические атрибуты (boolean attributes). Подобные атрибуты могут не иметь значения. Например, 
у кнопки можно задать атрибут <code>disabled</code>:</p>
<pre class="brush:xml;">
&lt;input type="button" value="Нажать" disabled&gt;
</pre>
<p>Атрибут disabled указывает, что данный элемент отключен.</p>
<h3>Глобальные атрибуты</h3>
<p>В HTML5 есть набор <span class="b">глобальных атрибутов</span>, которые применимы к любому элементу HTML5:</p>
<ul>
<li><p><span class="b">accesskey</span>: определяет клавишу для быстрого доступа к элементу</p></li>
<li><p><span class="b">class</span>: задает класс CSS, который будет применяться к элементу</p></li>
<li><p><span class="b">contenteditable</span>: определяет, можно ли редактировать содержимое элемента</p></li>
<li><p><span class="b">contextmenu</span>: определяет контекстное меню для элемента, которое отображается при нажатии на элемент правой кнопкой мыши</p></li>
<li><p><span class="b">dir</span>: устанавливает направление текста в элементе</p></li>
<li><p><span class="b">draggable</span>: определяет, можно ли перетаскивать элемент</p></li>
<li><p><span class="b">dropzone</span>: определяет, можно ли копировать переносимые данные при переносе на элемент</p></li>
<li><p><span class="b">hidden</span>: скрывает элемент</p></li>
<li><p><span class="b">id</span>: уникальный идентификатор элемента. На веб-странице элементы не должны иметь повторяющихся идентификаторов</p></li>
<li><p><span class="b">lang</span>: определяет язык элемента</p></li>
<li><p><span class="b">spellcheck</span>: указывает, будет ли для данного элемента использоваться проверка правописания</p></li>
<li><p><span class="b">style</span>: задает стиль элемента</p></li>
<li><p><span class="b">tabindex</span>: определяет порядок, в котором по элементам можно переключаться с помощью клавиши TAB</p></li>
<li><p><span class="b">title</span>: устанавливает дополнительное описание для элемента</p></li>
<li><p><span class="b">translate</span>: определяет, должно ли переводиться содержимое элемента</p></li>
</ul>
<p>Но, как правило, из всего этого списка наиболее часто используются три: <span class="b">class</span>, <span class="b">id</span> и <span class="b">style</span>.</p>
<h3>Пользовательские атрибуты</h3>
<p>В отличие от предыдущей версии языка разметки в HTML5 были добавлены пользовательские атрибуты (custom attributes). Теперь разработчик или создатель 
веб-страницы сам может определить любой атрибут, предваряя его префиксом <i>data-</i>. Например:</p>
<pre class="brush:xml;">
&lt;input type="button" value="Нажать" data-color="red" &gt;
</pre>
<p>Здесь определен атрибут <code>data-color</code>, который имеет значение "red". Хотя для этого элемента, ни в целом в html не существует подобного атрибута. 
Мы его определяем сами и устанавливаем у него любое значение.</p>
<h3>Одинарные или двойные кавычки</h3>
<p>Нередко можно встретить случаи, когда в html при определении значений атрибутов применяются как одинарные, так и двойные кавычки. Например:</p>
<pre class="brush:xml;">&lt;input type='button' value='Нажать'&gt;</pre>
<p>И одинарные, и двойные кавычки в данном случае допустимы, хотя чаще применяются именно двойные кавычки. Однако иногда само значение атрибута может 
содержать двойные кавычки, и в этом случае все значение лучше поместить в одинарные:</p>
<pre class="brush:xml;">&lt;input type="button" value='Кнопка "Привет мир"'&gt;</pre>
