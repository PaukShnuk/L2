...
<html >
<head>
<title>HTML5 | Флажки и переключатели</title>
<meta charset="utf-8" />
<meta name="description" content="Флажки и переключатели в HTML5, элементы input type="checkbox" и input type="radio", создание группы радиокнопок">
...
<h2>Флажки и переключатели</h2><div class="date">Последнее обновление: 08.04.2016</div>
...
<h3>Флажок</h3>
<p>Флажок представляет элемент, который может находиться в двух состояниях: отмеченном и неотмеченном. Флажок создается с помощью элемента <code>input</code> 
с атрибутом <span class="b">type="checkbox"</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/2.14.png" alt="Флажок/чекбокс в HTML5" />
<p>Атрибут <span class="b">checked</span> позволяет установить флажок в отмеченное состояние.</p>
<h3>Переключатели</h3>
<p>Переключатели или радиокнопки похожи на флажки, они также могут находиться в отмеченном или неотмеченном состоянии. Только для переключателей можно создать 
одну группу, в которой одновременно можно выбрать только один переключатель. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<p>Для создания радиокнопки надо указать атрибут <code>type="radio"</code>. И теперь другой атрибут <code>name</code> указывает не на имя элемента, а на имя 
группы, к которой принадлежит элемент-радиокнопка. В данном случае у нас две группы радиокнопок: <code>gender</code> и <code>tech</code>. Из каждой группы 
мы можем выбрать только один переключатель. Опять же чтобы отметить радиокнопку, у нее устанавливается атрибут <code>checked</code>:</p>
<img src="pics/2.15.png" alt="Радиокнопки в HTML5" />
<p>Важное значение играет атрибут <code>value</code>, который при отправке формы позволяет серверу определить, какой именно переключатель был отмечен:</p>
<img src="pics/2.16.png" alt="Radio button в HTML5" />
