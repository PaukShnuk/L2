...
<html >
<head>
<title>CSS3 | Стилизация шрифтов</title>
<meta charset="utf-8" />
<meta name="description" content="Стилизация шрифтов в CSS 3, различные семейства шрифтов, Arial, Verdana, использование свойств font-family, font-style, font-weight">
...
<h2>Стилизация шрифтов</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<h3>Семейство шрифтов</h3>
<p>Свойство <span class="b">font-family</span> устанавливает семейство шрифтов, которое будет использоваться. Например:</p>
<pre class="brush:css;">
body{
	font-family: Arial;
}
</pre>
<p>В данном случае устанавливается шрифт Arial.</p>
<p>Шрифт свойства font-family будет работать, только если у пользователя на локальном компьютере имеется такой же шрифт. По этой причине нередко выбираются стандартные шрифты, 
которые широко распространены, как Arial, Verdana и т.д.</p>
<p>Также нередко применяется практика нескольких шрифтов:</p>
<pre class="brush:css;">
body{
	font-family: Arial, Verdana, Helvetica;
}
</pre>
<p>В данном случае основным шрифтом является первый - Arial. Если он на компьютере пользователя не поддерживается, то выбирается второй и т.д.</p>
<p>Если название шрифта состоит из нескольких слов, например, Times New Roman, то все название заключается в кавычки:</p>
<pre class="brush:css;">
body{
	font-family: "Times New Roman";
}
</pre>
<p>Кроме конкретных стилей также могут использоваться общие универсальные шрифты, задаваемые с помощью значений <span class="b">sans-serif</span> и 
<span class="b">serif</span>:</p>
<pre class="brush:css;">
body{
	font-family: Arial, Verdana, sans-serif;
}
</pre>
<p>Так, если ни Arial, ни Verdana не поддерживаются на компьютере пользователя, то используется sans-serif - универсальный шрифт без засечек.</p>
<h3>Типы шрифтов</h3>
<h4>Шрифты с засечками</h4>
<p>Шрифты с засечками названы так, потому что на на концах основных штрихов имеют небольшие засечки. Считается, что они подходят для 
больших кусков текста, так как визуально связывают одну букву с другой, делая текст более читабельным.</p>
<p>Распространенные шрифты с засечками: Times, Times New Roman, Georgia, Garamond. Универсальный обобщенный шрифт с засечками представляет значение <span class="b">serif</span>.</p>
<h4>Шрифты без засечек</h4>
<p>В отличие от шрифтов с засечками шрифты из этой группы не имеют засечек. Наиболее распространенные шрифты этой группы: Arial, Helvetica, Verdana.</p>
<h4>Моноширинные шрифты</h4>
<p>Моноширинный шрифт преимущественно применяется для отображения программного кода и не предназначен для вывода стандартного текста статей. 
...
Consolas, Lucida Console.</p>
<p>Примеры шрифтов:</p>
<img src="pics/4.25.png" alt="Шрифты в CSS 3" />
<h3>Толщина шрифта</h3>
<p>Свойство <span class="b">font-weight</span> задает толщину шрифта. Оно может принимать 9 числовых значений: 100, 200, 300, 400,...900. 100 - очень тонкий шрифт, 
900 - очень плотный шрифт.</p>
<p>В реальности чаще для этого свойства используют два значения: <code>normal</code> (нежирный обычный текст) и <code>bold</code> (полужирный шрифт):</p>
<pre class="brush:css;">
font-weight: normal;
font-weight: bold;
</pre>
<h3>Курсив</h3>
<p>Свойство <span class="b">font-style</span> позволяет выделить текст курсивом. Для этого используется значение <code>italic</code>:</p>
<pre class="brush:css;">
p {font-style: italic;}
</pre>
<p>Если надо отменить курсив, то применяется значение <code>normal</code>:</p>
<pre class="brush:css;">
p {font-style: normal;}
</pre>
<h3>Цвет шрифта</h3>
<p>Свойство <span class="b">color</span> устанавливает цвет шрифта:</p>
<pre class="brush:css;">
p {
	color: red;
}
