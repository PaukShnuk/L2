...
<html >
<head>
<title>HTML5 | Кнопки</title>
<meta charset="utf-8" />
<meta name="description" content="Кнопки в HTML5, их типы, создание кнопок с помощью элементов input и button">
...
<h2>Кнопки</h2><div class="date">Последнее обновление: 08.04.2016</div>
...
<p>Кнопки представлены элементом <span class="b">button</span>. Они обладают широкими возможностями по конфигурации. Так, в зависимости от значения атрибута 
<code>type</code> мы можем создать различные типы кнопок:</p>
<ul>
<li><p><code>submit</code>: кнопка, используемая для отправки формы</p></li>
<li><p><code>reset</code>: кнопка сброса значений формы</p></li>
<li><p><code>button</code>: кнопка без какого-либо специального назначения</p></li>
</ul>
<p>Если кнопка используется для отправки формы, то есть у нее установлен атрибут <code>type="submit"</code>, то мы можем задать у нее ряд дополнительных атрибутов:</p>
<ul>
<li><p><code>form</code>: определяет форму, за которой закреплена кнопка отправки</p></li>
<li><p><code>formaction</code>: устанавливает адрес, на который отправляется форма. Если у элемента form задан атрибут <code>action</code>, то он переопределяется</p></li>
<li><p><code>formenctype</code>: устанавливает формат отправки данных. Если у элемента form установлен атрибут <code>enctype</code>, то он переопределяется</p></li>
<li><p><code>formmethod</code>: устанавливает метод отправки формы (post или get). Если у элемента form установлен атрибут <code>method</code>, то он переопределяется</p></li>
</ul>
<p>Например, определим на форме кнопку отправки и кнопку сброса:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/2.5.png" alt="Кнопки в HTML5" />
<p>Кроме элемента <code>button</code> для создания кнопок можно использовать элемент <code>input</code>, у которого атрибут равен <code>submit</code> или <code>reset</code>. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<p>И еще один элемент input с атрибутом <code>type="image"</code> позволяет использовать в качестве кнопки изображение:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/2.22.png" alt="Кнопка-изображение в HTML5" />
<p>Кроме наличия изображения в остальном эта кнопка будет аналогична стандартной кнопке отправки <code>input type="submit"</code> или <code>button type="submit"</code>.</p>
