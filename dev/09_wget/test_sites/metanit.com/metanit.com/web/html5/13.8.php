...
<html >
<head>
<title>HTML5 | Направление и порядок элементов в Grid Layout</title>
<meta charset="utf-8" />
<meta name="description" content="Направление и порядок элементов в Grid Layout в CSS3, свойства grid-auto-flow и order">
...
<h2>Направление и порядок элементов</h2><div class="date">Последнее обновление: 09.05.2017</div>
...
<h3>Свойство grid-auto-flow</h3>
<p>По умолчанию все элементы располагаются по порядку горизонтально, если места в строке больше нет, то элементы переносятся на следующую строку. Но 
с помощью свойства <span class="b">grid-auto-flow</span> можно изменить направление элементов. Это свойство принимает два значения:</p>
<ul>
<li><p><span class="b">row</span>: значение по умолчанию, элементы располагаются в строку друг за другом, если места в строке не хватает, 
элементы переносятся на следующую строку</p></li>
<li><p><span class="b">column</span>: элементы располагаются в столбик, если места в столбце не хватает, то элементы переходят в следующий столбец</p></li>
</ul>
<p>Применим данное свойство:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Свойство grid-auto-flow имеет значение <code>row</code>, поэтому элементы будут располагаться в строку.</p>
<img src="pics/grid10.png" alt="grid-auto-flow в CSS 3" />
<p>Теперь изменим стиль grid-контейнера:</p>
<pre class="brush:css;">
.grid-container {
	height: 100vh;
	display: grid;
...
<p>После этого элементы будут располагаться в столбец</p>
<img src="pics/grid11.png" alt="grid-auto-flow в Grid Layout CSS 3" />
<h3>Свойство order</h3>
<p>Свойство <span class="b">order</span> позволяет задать порядок элементов. По умолчанию для каждого элемента в гриде это свойство имеет значение 0. Поэтому 
элементы располагаются друг за другом как они определены в разметке html. Но мы можем переопределить этот порядок:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/grid12.png" alt="Порядок элементов в Grid Layout в CSS 3" />
<p>Элементы с классом last-item имеет порядок 1, поэтому они будут располагаться после других элементов, у которых порядок равен 0 или меньше.</p>
<p>Если же необходимо поставить одни элементы перед другими, то можно использовать отрицательное значение для свойства order. Так, у седьмого элемента 
порядок равен -1, что меньше, чем у других элементов. Поэтому последний в разметке элемент по факту будет располагаться перед всеми элементами.</p>
