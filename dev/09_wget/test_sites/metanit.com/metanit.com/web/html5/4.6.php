...
<html >
<head>
<title>HTML5 | Элемент main</title>
<meta charset="utf-8" />
<meta name="description" content="Элемент main в семантической разметке в HTML5, создание блока основного содержимого веб-страницы">
...
<h2>Элемент main</h2><div class="date">Последнее обновление: 08.04.2016</div>
...
<p>Элемент <span class="b">main</span> представляет основное содержимое веб-страницы. Он представляет уникальный контент, в который не следует 
включать повторяющиеся на разных веб-страницах элементы сайдбаров, навигационные ссылки, информацию о копирайте, логотипы и тому подобное.</p>
<p>Используем элемент main:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/3.5.png" alt="Элемент main в HTML5" />
<p>Не стоит думать, что абсолютно все содержимое надо обязательно помещать в элемент main. Нет мы также можем использовать вне его другие элементы, например, 
header и footer:</p>
<pre class="brush:xml;">
&lt;body&gt;
	&lt;header&gt;
...
<p>Однако надо помнить, что элемент main не может быть вложенным в такие элементы, как article, aside, footer, header, nav. Кроме того, на веб-странице допустимо наличие только одного элемента main.</p>
<p>Также стоит отметить, что на данный момент есть небольшие проблемы с поддержкой этого элемента в браузерах. В частности, IE 11 не поддерживает данный 
элемент (в остальных браузерах полная поддержка), поэтому в этом случае стоит использовать атрибут роли:</p>
<pre class="brush:xml;">
&lt;main role="main"&gt;
  ...
&lt;/main&gt;
</pre>
<p>Указание роли позволит IE11 и более старшим версиям IE должным образом интерпретировать элемент.</p>
