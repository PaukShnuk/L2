...
<html >
<head>
<title>HTML5 | Области грида в Grid Layout</title>
<meta charset="utf-8" />
<meta name="description" content="Области грида в Grid Layout в CSS3, сематическая связь названий областей с элементами страницы, свойство grid-template-areas">
...
<h2>Области грида</h2><div class="date">Последнее обновление: 09.05.2017</div>
...
<p>В рамках грида мы можем определять области (grid area). Области определятся с помощью двух вертикальных и двух горизонтальных grid-линий, 
...
Области особенно полезны для определения семантических отношений между различными частями макета страницы.</p>
<p>Для определения областей у grid-контейнера применяется свойство <span class="b">grid-template-areas</span>. Например, определим три области:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Здесь у grid-контейнера определяется два столбца и три строки:</p>
<pre class="brush:css;">
grid-template-columns: 150px 1fr;
grid-template-rows: 100px 1fr 100px;
</pre>
<p>То есть в итоге у нас будет в гриде 3 х 2 = 6 ячеек. Но в разметке страницы определено три элемента с одноименными областями: header, sidebar, content. 
И свойство grid-template-areas как раз устанавливает, как эти области будут располагаться в ячейках грида:</p>
<pre class="brush:css;">
grid-template-areas: 	"header header"
						"sidebar content"
						"sidebar content";
</pre>
<p>Выражение "header header" представляет первую строку и указывает, что область header занимает две ячейки подряд подряд.</p>
<p>Следующее выражение "sidebar content" указывает на вторую строку, где область sidebar занимает первую ячейку, а content - вторую ячейку. 
Третья строка повторяет вторую. То есть в итоге, если смотреть по ячейкам, то получится следующим образом:</p>
<img src="pics/grid21.png" alt="Grid areas в Grid Layout в CSS 3" />
<p>Для установки области у элементов задается свойство <span class="b">grid-area</span>:</p>
<pre class="brush:css;">
.header { 
...
<p>В итоге мы получим следующую страницу:</p>
<img src="pics/grid22.png" alt="области в Grid Layout в CSS 3" />
<p>Теперь рассмотрим другой более сложный пример. Пусть код веб-страницы выглядит следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Теперь грид содержит 5 строк и 5 столбцов, то есть в совокупности 25 ячеек, которые размещают 5 областей: header, menu, sidebar, content и footer.</p>
<p>Свойство <span class="b">grid-template-areas</span> в стилях grid-контейнера опять же содержит определение того, как все эти области сопоставляются с ячейками. 
...
указывается пять точек, между которыми ставятся пробелы. В итоге мы получим пять областей, между которыми будут располагаться незаполненные пространства:</p>
<img src="pics/grid23.png" alt="Пробелы между областями в Grid Layout в CSS3" />
...
<div class="nav"><p><a href="13.10.php">Назад</a><a href="./">Содержание</a><a href="13.12.php">Вперед</a></p></div></div>
<div class="socBlock">
