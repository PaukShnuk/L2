...
<html >
<head>
<title>CSS3 | Внешние отступы</title>
<meta charset="utf-8" />
<meta name="description" content="Внешние отступы в CSS 3, свойство margin">
...
<h2>Внешние отступы</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Свойство <span class="b">margin</span> определяет отступ элемента от других элементов или границы внешнего контейнера. Существуют специальные свойства CSS для задания 
отступов для каждой стороны:</p>
<ul>
<li><p><span class="b">margin-top</span>: отступ сверху</p></li>
<li><p><span class="b">margin-bottom</span>: отступ снизу</p></li>
<li><p><span class="b">margin-left</span>: отступ слева</p></li>
<li><p><span class="b">margin-right</span>: отступ справа</p></li>
</ul>
<p>Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
...
<img src="pics/4.41.png" alt="Margin в CSS 3" />
<p>Можно вместо четырех свойств задать одно:</p>
<pre class="brush:css;">
div{
...
<p>Свойство задается в формате:</p>
<pre class="brush:css;">margin: отступ_сверху отступ_справа отступ_снизу отступ_слева;</pre>
<p>Если значения для всех четырех отступов совпадает, то мы можем указать только одно значение:</p>
<pre class="brush:css;">
div{
	margin: 25px;
}
</pre>
<p>В этом случае для всех четырех отступов будет использоваться 25 пикселей.</p>
<p>Для установки отступов можно использовать точные значения в пикселях (px) или em, либо процентные отношения, либо значение <code>auto</code> (автоматическая 
установка отступов).</p>
<p>Например:</p>
<pre class="brush:css;">
margin-left: 2em;
</pre>
<p>Значение <code>2 em</code> определяет расстояние, которое в два раза больше размера шрифта элемента.</p>
<p>При использовании процентов веб-браузеры вычисляют размер отступов на основе ширины элемента-контейнера, в который заключен стилизуемый 
элемент.</p>
<p>В то же время, если несколько элементов у нас соприкасаются, то браузер выбирает наибольший отступ элемента, который затем и используется. 
Так, выше в примере использовался следующий стиль:</p>
<pre class="brush:css;">
div{
	margin-top: 30px; /* отступ сверху */
	margin-left: 25px; /* отступ слева */
...
<p>Между первым и вторым блоками расстояние будет равно 30 пикселям - значение свойства <code>margin-top</code> второго блока div, 
несмотря на то, что у первого блока div имеется свойство <code>margin-bottom</code>, равное 15 пикселям.</p>
