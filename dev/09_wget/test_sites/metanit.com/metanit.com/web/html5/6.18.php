...
<html >
<head>
<title>CSS3 | Прокрутка элементов</title>
<meta charset="utf-8" />
<meta name="description" content="Прокрутка элементов в CSS 3, свойство overflow, создание вертикальной и горизонтальной полос прокрутки">
...
<h2>Прокрутка элементов</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Нередко при создании веб-страниц можно столкнуться с ситуацией, когда содержимое блока занимает гораздо 
...
выходит за границы блока.</p>
<p>Однако свойство <span class="b">overflow</span> позволяет настроить поведение блока в подобной ситуации и добавить возможность прокрутки. 
Это свойство может принимать следующие значения:</p>
<ul>
<li><p><code>auto</code>: если контент выходит за границы блока, то создается прокрутка. В остальных случаях полосы прокрутки не отображаются</li>
<li><p><code>hidden</code>: отображается только видимая часть контента. Контент, который выходит за границы блока, не отображается, а полосы прокрутки не создаются</li>
<li><p><code>scroll</code>: в блоке отображаются полосы прокрутки, даже если контент весь помещается в границах блока, и таких полос прокрутки не требуется</li>
<li><p><code>visible</code>: значение по умолчанию, контент отображается, даже если он выходит за границы блока</li>
</ul>
<p>Рассмотрим применение двух значений:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/4.61.png" alt="Создание прокрутки в CSS" />
<p>Свойство <code>overflow</code> управляет полосами прокрутки как по вертикали, так и по горизонтали. С помощью дополнительных свойств 
<span class="b">overflow-x</span> и <span class="b">overflow-y</span> можно определить прокрутку соответственно по горизонтали и по вертикали. Данные свойства 
принимают те же значения, что и <code>overflow</code>:</p>
<pre class="brush:css;">
overflow-x: auto;
overflow-y: hidden;
</pre>
...
<div class="nav"><p><a href="6.17.php">Назад</a><a href="./">Содержание</a><a href="6.19.php">Вперед</a></p></div></div>
<div class="socBlock">
