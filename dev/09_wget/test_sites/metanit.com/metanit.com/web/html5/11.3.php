...
<html >
<head>
<title>HTML5 | Media Query в CSS</title>
<meta charset="utf-8" />
<meta name="description" content="Media Query в CSS 3 и в HTML 5, создание адаптивого дизайна">
...
<h2>Media Query в CSS</h2><div class="date">Последнее обновление: 03.05.2016</div>
...
<p>Другим важным элементом в построении адаптивого дизайна являются правила <span class="b">Media Query</span>, которые поволяют определить стиль в зависимости 
от размеров браузера пользователя.</p>
<p>Например, некоторые элементы могут </p>
<p>В CSS2 уже было решение в виде правила <code>media</code>, которое позволяет указать устройство, для которого используется данный стиль:</p>
<pre class="brush:xml;">
&lt;html&gt;
 &lt;head&gt;
...
<p>Правило <code>media="handheld"</code> указывает, что стили в mobile.css будут применяться к мобильным устройствам, в то время как правило 
<code>media="screen"</code> применяется к десктопным браузерам.</p>
<p>Однако многие современные мобильные браузеры по умолчанию считают, что страница предназначена для десктопов, поэтому в любом случае применяет 
правило <code>media="screen"</code>. Поэтому на подобное решение не стоит полагаться.</p>
<p>Для решения этой проблемы в CSS3 были введен механизм <span class="b">CSS3 Media Query</span>. Например, чтобы применить стиль только к мобильным устройствам 
мы можем написать так:</p>
<pre class="brush:xml;">
&lt;html&gt;
 &lt;head&gt;
  &lt;title&gt;Адаптивная веб-страница&lt;/title&gt;
...
<p>Значение атрибута media <code>(max-device-width:480px)</code> говорит нам о том, что стили из файла mobile.css будут 
применяться к тем устройствам, максимальная ширина экрана которых составляет 480 пикселей - то есть фактически это и есть мобильные устройства.</p>
<p>С помощью ключевого слова <span class="b">and</span> можно комбинировать условия, например:</p>
<pre class="brush:xml;">
&lt;link rel="stylesheet" type="text/css" media="(min-width:481px) and (max-width:768px)" href="mobile.css" /&gt;
</pre>
<p>Данный стиль будет применяться, если ширина браузера находится в диапазоне от 481 до 768 пикселей.</p>
<p>С помощью директивый <span class="b">@import</span> можно определить один css-файл и импортировать в него стили для определенных устройств:</p>
<pre class="brush:css;">
...
<p>Также можно не разлелять стили по файлам, а использовать правила CSS3 Media Query в одном файле css:</p>
<pre class="brush:css;">
body {
...
<p>Применяемые функции в CSS3 Media Query:</p>
<ul>
<li><p><span class="b">aspect-ratio</span>: отношение ширины к высоте области отображения (браузера)</p></li>
<li><p><span class="b">device-aspect-ratio</span>: отношение ширины к высоте экрана устройства</p></li>
<li><p><span class="b">max-width/min-width и max-height/min-height</span>: максимальная и минимальная ширина и высота области отображения (браузера)</p></li>
<li><p><span class="b">max-device-width/min-device-width и max-device-height/min-device-height</span>: максимальная и минимальная ширина и высота 
экрана мобильного устройства</p></li>
<li><p><span class="b">orientation</span>: ориентация (портретная или альбомная)</p></li>
</ul>
<p>Например, мы можем задать разные стили для разных ориентаций мобильных устройств:</p>
<pre class="brush:css;">
...
<p>Таким образом, мы меняем лишь определение стилей в зависимости от устройства, а сами стили css по сути остаются теми же, что мы используем для создания обычных сайтов.</p>
<p>Как правило, при определении стилей предпочтение отдается стилям для самых малых экранов - так называемый подход <span class="b">Mobile First</span>, 
хотя это необязательно. Например, определим следующую веб-страницу:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Сначала идут общие стили, которые актуальны прежде всего для мобильных устройств с небольшими экранами. Затем идут стили для устройств с экранами 
средней ширины: фаблеты, планшеты. И далее идут стили для десктопов.</p>
<p>И например, на эмуляторе Opera Mobile при эмуляции устройства с шириной в 480 пикселей страница приобретет красный цвет:</p>
<img src="pics/9.6.png" alt="Правила media query в CSS 3" />
<p>А в браузере обычного компьютера страница будет иметь синий цвет, как и определено в стилях:</p>
<img src="pics/9.7.png" alt="Правила media query в HTML 5" />
...
<div class="nav"><p><a href="11.2.php">Назад</a><a href="./">Содержание</a><a href="7.1.php">Вперед</a></p></div></div>
<div class="socBlock">
