...
<html >
<head>
<title>CSS3 | Высота шрифта</title>
<meta charset="utf-8" />
<meta name="description" content="Установка высоты шрифта в CSS 3, свойство font-size, форматы значений для высоты">
...
<h2>Высота шрифта</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Для установки размера шрифта используется свойство <span class="b">font-size</span>:</p>
<pre class="brush:css;">
div{
	font-size: 18px;
}
</pre>
<p>В данном случае высота шрифта составит 18 пикселей. Пиксели представляют наиболее часто используемые единицы измерения. Чтобы задать значение в пикселях, после самого значения идет сокращение "px".</p>
<p>Если к тексту явным образом не применяется высота шрифта, то используются значения браузера по умолчанию. Например, для простого текста в параграфах 
это 16 пикселей. Это базовый стиль текста.</p>
<p>Базовый стиль для разных элементов текста отличается: если для параграфов это 16 пикселей, то для заголовков h1 это 32 пикселя, для заголовков h2 - 24 пикселя и т..д.</p>
<p>Для измерения шрифта также можно использовать самые разные единицы измерения.</p>
<h3>Ключевые слова</h3>
<p>В CSS имеется семь ключевых слов, которые позволяют назначить размер шрифта относительно базового:</p>
<ul>
<li><p><span class="b">medium</span>: базовый размер шрифта браузера (16 пикселей)</p></li>
<li><p><span class="b">small</span>: 13 пикселей</p></li>
<li><p><span class="b">x-small</span>: 10 пикселей</p></li>
<li><p><span class="b">xx-small</span>: 9 пикселей</p></li>
<li><p><span class="b">large</span>: 18 пикселей</p></li>
<li><p><span class="b">x-large</span>: 24 пикселя</p></li>
<li><p><span class="b">xx-large</span>: 32 пикселя</p></li>
</ul>
<p>Например:</p>
<pre class="brush:css;">
font-size: x-large;
...
<h3>Проценты</h3>
<p>Проценты позволяют задать значение относительно базового или унаследованного шрифта. Например:</p>
<pre class="brush:css;">
font-size: 150%;
</pre>
<p>В данном случае высота шрифта будет составлять 150% от базового, то есть 16px * 1,5 = 24px</p>
<p>Наследование шрифта может изменить финальное значение. Например, следующую ситуацию:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Здесь элемент p наследует от контейнера - блока div шрифт высотой в 10 пикселей. То есть 10 пикселей теперь будет базовым для параграфа.</p>
<p>Далее для элемента p определяется новая высота шрифта в 150%. Это значит, что финальная высота будет равна 10px * 1,5 = 15px.</p>
<h3>Единица еm</h3>
<p>Единица измерения еm во многом эквивалентна процентам. Так, <code>1em</code> равен <code>100%</code>, <code>.5em</code> равно <code>50%</code> и т.д.</p>
