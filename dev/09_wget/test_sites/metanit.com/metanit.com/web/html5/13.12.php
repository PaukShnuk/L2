...
<html >
<head>
<title>HTML5 | Макет страницы в Grid Layout</title>
<meta charset="utf-8" />
<meta name="description" content="Создание адаптивного макета в Grid Layout в CSS3, использование областей для позиционирования элементов">
...
<h2>Макет страницы в Grid Layout</h2><div class="date">Последнее обновление: 09.05.2017</div>
...
<p>Рассмотрим создание простейшего адаптивного стандартного макета веб-страницы, который состоит из шапки, подвала, основного содержимого, блока навигации и сайдбара. 
Для этого определим следующую веб-станицу:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>В прошлой теме уже рассматривалось применение именованных областей. И здесь они также используются, так как очень удобно связывать стили с семантикой 
станицы через области.</p>
<p>Однако, так как предполагается, что веб-станицу могут просматривать на мобильных устройствах, где ширина экрана ограничена, то хотелось бы иметь 
адаптивный макет. Поэтому в стиля имеются два разных определения грида. Одно определение грида для мобильных устройств (условно в качестве ширины устройств выбрано значение в 468px):</p>
<pre class="brush:css;">
.grid-container {
	height:100vh;
	display: grid;
...
<p>В таком состоянии грид имеет только один столбец и 5 строк для каждой области плюс 4 строки-разделители.</p>
<img src="pics/grid28.png" alt="Макет веб-станицы с Grid Layout в CSS3" />
<p>При увеличении ширины экрана в действие вступает другое определение грида:</p>
<pre class="brush:css;">
.grid-container {
	height:100vh;
	display: grid;
...
<p>В этом случае столбцов и строк будет по пять:</p>
<img src="pics/grid27.png" alt="Адаптивный макет станицы с Grid Layout в CSS3" />
...
<div class="nav"><p><a href="13.11.php">Назад</a><a href="./">Содержание</a><a href="14.1.php">Вперед</a></p></div></div>
<div class="socBlock">
