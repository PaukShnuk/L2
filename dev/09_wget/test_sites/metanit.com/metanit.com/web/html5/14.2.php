...
<html >
<head>
<title>CSS3 | Создание тем с помощью переменных</title>
<meta charset="utf-8" />
<meta name="description" content="Создание светлой и темной тем с помощью переменных в CSS3, переключение тем">
...
<h2>Создание тем CSS с помощью переменных</h2><div class="date">Последнее обновление: 18.11.2018</div>
...
<p>Использование переменных в CSS открывает нам возможность создания и применения тем на веб-странице. Например, определим следующую веб-страницу:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
    ...
    &lt;body&gt;
		...
		&lt;/div&gt;
		&lt;footer&gt;
		  &lt;p&gt;Все права разрешены &copy;2018&lt;/p&gt;
		&lt;/footer&gt;
		&lt;script&gt;
...
<p>В стилях страницы определены фактически две темы, которые содержат три переменных:</p>
<pre class="brush:css;">
:root{
	--panel-bg-color: #dfe4ea;
	--container-bg-color: #f1f2f6;
...
<p>Первая тема - условно светлая, вторая - условно темная. При темной теме корневой элемент, то есть элемент <code>&lt;html&gt;</code> будет иметь атрибут <code>theme="dark"</code>.</p>
<p>Затем эти переменные используются для установки стилевых свойств отдельных элементов.</p>
<p>Для переключения тем у кнопки через несложный код javascript установлен обработчик нажатия, который проверяет наличие атрибута "theme" (что будет означать, 
что установлена темная схема). И при наличии атрибута убирает его, а при его отсутствии, наоборот, устанавливает.</p>
<pre class="brush:js;">
const toggleBtn = document.querySelector("#toggle-theme");
toggleBtn.addEventListener("click", function() {
	if(document.documentElement.hasAttribute("theme")){
...
<p>В результате при нажатии на кнопку произойдет глобальное переключение стилей веб-страницы:</p>
<img src="pics/cssvariables3.png" alt="Переключение тем в CSS и HTML" />
