...
<html >
<head>
<title>HTML5 | Таблицы</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с таблицами в HTML5, элементы table, tr, td, th, атрибуты и настройка таблиц, строк и столбцов">
...
<h2>Таблицы</h2><div class="date">Последнее обновление: 08.04.2016</div>
...
<p>Для создания таблиц в html используется элемент <span class="b">table</span>. Каждая таблица между тегами <code>&lt;table&gt;</code> и 
<code>&lt;/table&gt;</code> содержит строки, который представлены элементом <span class="b">tr</span>. А каждая строка между тегами <code>&lt;tr&gt;</code> и 
<code>&lt;/tr&gt;</code> содержит ячейки в виде элементов <span class="b">td</span>.</p>
<p>Создадим простейшую таблицу:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/1.29.png" alt="Таблицы в HTML5" />
<p>Здесь у нас в таблице 4 строки, и каждая строка имеет по три столбца.</p>
<p>При этом в данном случае первая строка выполняет роль заголовка, а остальные три строки собственно являются содержимым таблицы. Разделения 
заголовков, футера и тела таблицы в html предусмотрены соответственно элементы <span class="b">thead</span>, <span class="b">tfoot</span> и <span class="b">tbody</span>. Для их применения 
изменим таблицу следующим образом:</p>
<pre class="brush:xml;">
&lt;table&gt;
...
<p>В элемент <code>thead</code> заключается строка заголовков. Для ячеек заголовок используется не элемент <code>td</code>, а <code>th</code>. Элемент 
<code>th</code> выделяет заголовок жирным. А все остальные строки заключаются в <code>tbody</code></p>
<p>Элемент <code>tfoot</code> определяет подвал таблицы или футер. Здесь обычно выводится некоторая вспомогательная информация по отношению к таблице.</p>
<p>Кроме собственно заголовоков столбцов с помощью элемента <span class="b">caption</span> мы можем задать общий заголовок для таблицы.</p>
<img src="pics/1.30.png" alt="Заголовки таблиц в HTML5" />
<p>Также стоит отметить, что футер таблицы содержит только один столбец, который раздвигается по ширине трех столбцов с помощью атрибута <code>colspan="3"</code>.</p>
<p>Атрибут <span class="b">colspan</span> указывает на какое количество столбцов раздвигается данная ячейка. Также с помощью атрибута <span class="b">rowspan</span> 
мы можем раздвигать ячейку на определенное количество строк. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/1.31.png" alt="Растяжение ячеек таблицы в html" />
