...
<html >
<head>
<title>CSS3 | Селекторы дочерних элементов</title>
<meta charset="utf-8" />
<meta name="description" content="Селекторы дочерних элементов в CSS 3, отличие от стилизации потомков">
...
<h2>Селекторы дочерних элементов</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Селекторы дочерних элементов отличаются от селекторов потомков тем, что позволяют выбрать элементы только первого уровня 
вложенности. Например:</p>
<pre class="brush:xml;">
&lt;body&gt;
	&lt;h2&gt;Заголовок&lt;/h2&gt;
	&lt;div&gt;
...
<p>Хотя вложенными в элемент body элементами являются целых три - h2, div, p, но дочерними из них являются только два - div и h2, так как 
они находятся в первом уровне вложенности. А элемент p находится на втором уровне вложенности, так как вложен внутрь элемента div, а не просто элемента body.</p>
<p>Для обращения к дочерним элементам используется знак угловой скобки:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>В блоке с классом article есть два параграфа. Селектор <code>.article &gt; p</code> выбирает только те параграфы, который находятся непосредственно 
в блоке article:</p>
<img src="pics/4.13.png" alt="Селекторы дочерних элементов в CSS3" />
<p>Если бы мы использовали другой селектор без символа &gt;</p>
<pre class="brush:css;">
.article p{
...
<p>Тогда стиль бы применялся ко всем параграфам на всех уровнях вложенности:</p>
<img src="pics/4.14.png" alt="Селекторы вложенных элементов в CSS" />
...
<div class="nav"><p><a href="5.3.php">Назад</a><a href="./">Содержание</a><a href="5.8.php">Вперед</a></p></div></div>
<div class="socBlock">
