...
<html >
<head>
<title>HTML5 | Текстовые поля</title>
<meta charset="utf-8" />
<meta name="description" content="Текстовые поля в HTML5, элементов input type=text, атрибуты текстовых полей, поле ввода пароля">
...
<h2>Текстовые поля</h2><div class="date">Последнее обновление: 08.04.2016</div>
...
<p>Однострочное текстовое поле создается с помощью элемента <span class="b">input</span>, когда его атрибут <code>type</code> имеет значение <code>text</code>:</p>
<pre class="brush:xml;">&lt;input type="text" name="login" /&gt;</pre>
<p>С помощью ряда дополнительных атрибутов можно настроить текстовое поле:</p>
<ul>
<li><p><span class="b">dirname</span>: устанавливает направление текста</p></li>
<li><p><span class="b">maxlength</span>: максимально допустимое количество символов в текстовом поле</p></li>
<li><p><span class="b">pattern</span>: определяет шаблон, которому должен соответствовать вводимый текст</p></li>
<li><p><span class="b">placeholder</span>: устанавливает текст, который по умолчанию отображается в текстовом поле</p></li>
<li><p><span class="b">readonly</span>: делает текстовом поле доступным только для чтения</p></li>
<li><p><span class="b">required</span>: указывает, что текстовое поле обязательно должно иметь значение</p></li>
<li><p><span class="b">size</span>: устанавливает ширину текстового поля в видимых символах</p></li>
<li><p><span class="b">value</span>: устанавливает значение по умолчанию в текстовом поле</p></li>
</ul>
<p>Применим некоторые атрибуты:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/2.6.png" alt="Текстовые поля в HTML5" />
<p>В этом примере во втором текстовом поле сразу устанавливаются атрибуты <code>maxlength</code> и <code>size</code>. При этом size - то есть количество 
...
maxlength, мы не сможем.</p>
<p>В данном случае также важно различать атрибуты <code>value</code> и <code>placeholder</code>, хотя оба устанавливают видимый текст в поле. 
Однако <code>placeholder</code> устанавливает своего рода подсказку или приглашение к вводу, поэтому он обычно отмечается серым цветом. В то время как 
значение <code>value</code> представляет введенный в поле текст по умолчанию:</p>
<pre class="brush:xml;">
&lt;p&gt;&lt;input type="text" name="userName" value="Том" /&gt;&lt;/p&gt;
&lt;p&gt;&lt;input type="text" name="userPhone" placeholder="Номер телефона" /&gt;&lt;/p&gt;
</pre>
<img src="pics/2.7.png" alt="Placeholder в HTML5" />
<p>Атрибуты <code>readonly</code> и <code>disabled</code> делают текстовое поле недоступным, однако сопровождаются разным визуальным эффектом. В случае с 
disabled текстовое поле затеняется:</p>
<pre class="brush:xml;">
&lt;p&gt;&lt;input type="text" name="userName" value="Том" readonly /&gt;&lt;/p&gt;
&lt;p&gt;&lt;input type="text" name="userPhone" value="+12345678901" disabled /&gt;&lt;/p&gt;
</pre>
<img src="pics/2.8.png" alt="Disabled и readonly в HTML5" />
<p>Среди атрибутов текстового поля также следует отметить такой атрибут как <span class="b">list</span>. Он содержит ссылку на элемент <span class="b">datalist</span>, 
который определяет набор значений, появляющихся в виде подсказки при вводе в текстовое поле. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/2.9.png" alt="Datalist в HTML5" />
<p>Атрибут <code>list</code> текстового поля указывает на id элемента datalist. Сам элемент datalist с помощью вложенных элементов <code>option</code> 
определяет элементы списка. И при вводе в текстовое поле этот список отображается в виде подсказки.</p>
<h3>Поле поиска</h3>
<p>Для создания полей поиска предназначен элемент input с атрибутом <code>type="search"</code>. Формально он представляет собой простое текстовое поле:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img  src="pics/2.34.png" alt="Поиск в HTML5" />
<h3>Поле ввода пароля</h3>
<p>Для ввода пароля используется элемент input с атрибутом <code>type="password"</code>. Его отличительной чертой является то, что вводимые символы маскируются точками:</p>
<pre class="brush:xml;">
&lt;form&gt;
...
<img  src="pics/2.35.png" alt="Ввод пароля в HTML5" />
