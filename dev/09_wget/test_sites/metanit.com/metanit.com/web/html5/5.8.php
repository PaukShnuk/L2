...
<html >
<head>
<title>CSS3 | Селекторы элементов одного уровня</title>
<meta charset="utf-8" />
<meta name="description" content="Селекторы элементов одного уровня в CSS 3, стилизация сестринских элементов">
...
<h2>Селекторы элементов одного уровня</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Селекторы элементов одного уровня или смежных элементов позволяют выбрать элементы, которые находятся на одном уровне вложенности. Иногда такие 
элементы еще называют сиблинги (siblings) или сестринскими элементами. Например:</p>
<pre class="brush:xml;">
&lt;body&gt;
	&lt;h2&gt;Заголовок&lt;/h2&gt;
	&lt;div&gt;
...
<p>Здесь элементы h2 и оба блока div являются смежными, так как находятся на одном уровне. А элементы параграфов и заголовок h2 не являются смежными, так как 
параграфы вложены в блоки div.</p>
<p>Чтобы стилизовать первый смежный элемент по отношению к определенному элементу, используется знак плюса +:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Селектор <code>h2+div</code> позволяет определить стиль (в данном случае красный цвет текста) для блока div, который идет непосредственно после заголовка h2:</p>
<img src="pics/4.19.png" alt="Селекторы смежных элементов в CSS3" />
<p>Причем этот селектор будет стилизовать блок div, если он будет идти непосредственно после заголовка. Если же между заголовком и блоков div будет находиться 
еще какой-либо элемент, то к нему не будет применяться стиль, например:</p>
<pre class="brush:xml;">
&lt;h2&gt;Заголовок&lt;/h2&gt
...
<p>Если нам надо стилизовать вообще все смежные элементы одного уровня, неважно непосредственно идут они после определенного элемента или нет, то в этом случае 
вместо знака плюса необходимо использовать знак тильды "~":</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/4.20.png" alt="Стилизация элементов одного уровня в CSS" />
