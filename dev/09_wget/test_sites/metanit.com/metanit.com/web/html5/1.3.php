...
<html >
<head>
<title>HTML5 | Создание документа</title>
<meta charset="utf-8" />
<meta name="description" content="Создание документа в HTML5 в текстовом редакторе, первая веб-страница и ее открытие в браузере">
...
<h2>Создание документа HTML5</h2><div class="date">Последнее обновление: 08.04.2016</div>
...
<p>Элементы являются кирпичиками, из которых складывается документ html5. Для создания документа нам надо создать простой текстовый файл, а в качестве расширения файла указать 
<i>*.html</i></p>
<p>Создадим текстовый файл, назовем его <span class="ii">index</span> и изменим его расширение на <span class="b">.html</span>.</p>
<img src="pics/1.1.png" alt="Структура документа HTML5" />
<p>Затем откроем этот файл в любом текстовом редакторе, например, в Notepad++. Добавим в файл следующий текст:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<p>Для создания документа HTML5 нам нужны в первую очередь два элемента: DOCTYPE и html. Элемент <span class="b">doctype</span> или Document Type Declaration 
сообщает веб-браузеру тип документа. <code>&lt;!DOCTYPE html&gt;</code> указывает, что данный документ является документом html и что используется html5, а не html4 или какая-то другая версия языка разметки.</p>
<p>А элемент <code>html</code> между своим открывающим и закрывающим тегами содержит все содержимое документа.</p>
<p>Внутри элемента <code>html</code> мы можем разместить два других элемента: <span class="b">head</span> и <span class="b">body</span>. 
Элемент <code>head</code> содержит метаданные веб-страницы - заголовок веб-страницы, тип кодировки и т.д., а также ссылки на внешние ресурсы - стили, скрипты, если они использутся. 
Элемент <code>body</code> собственно определяет содержимое html-страницы.</p>
<p>Теперь изменим содержимое файла index.html следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<p>В элементе head определено два элемента:</p>
<ul>
<li><p>элемент <code>title</code> представляет заголовок страницы</p></li>
<li><p>элемент <code>meta</code> определяет метаинформацию страницы. Для корректного отображения символов предпочтительно указывать кодировку. 
В данном случае с помощью атрибута <code>charset="utf-8"</code> указываем кодировку utf-8.</p></li>
</ul>
<p>В пределах элемента элемента <code>body</code> используется только один элемент - <code>div</code>, который оформляет блок. 
Содержимым этого блока является простая строка.</p>
<p>Поскольку мы выбрали в качестве кодировки utf-8, то браузер будет отображать веб-страницу именно в этой кодировке. Однако необходимо чтобы сам текст 
...
Например, в Notepad++ надо зайти в меню <span class="b">Кодировки</span> и в открывшемся списке выбрать пункт <span class="b">Преобразовать в UTF-8 без BOM</span>:</p>
<img src="pics/1.3.png" alt="Установка кодировки html-файла" />
<p>После этого в статусной строке будет можно будет увидеть <span class="b">UTF-8 w/o BOM<span>, что будет указывать, что нужная кодировка установлена.</p>
<p>Сохраним и откроем файл <i>index.html</i> в браузере:</p>
<img src="pics/1.4.png" alt="Документ HTML5" />
<p>Таким образом, мы создали первый документ HTML5. Так как мы указали в элементе <code>title</code> заголовок "Документ HTML5", то именно такое название 
будет иметь вкладка браузера.</p>
<p>Так как указана кодировка utf-8, то веб-браузер будет корректно отображать кириллические символы.</p>
<p>А весь текст, определенный внутри элемента <code>body</code> мы увидим в основном поле браузера.</p>
