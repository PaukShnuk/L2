...
<meta charset="utf-8" />
<meta name="description" content="Работа с Canvas в HTML5, получение контекста кэнваса, рисование прямоугольников">
...
<h1>Canvas</h1><h2>Доступ к canvas и рисование прямоугольников</h2><div class="date">Последнее обновление: 02.05.2016</div>
...
<p>Одним из нововведений HTML5 стал элемент Canvas и продвинутые возможности по работе и манипуляции графикой. С помощью canvas можно рисовать 
как простейшие графические примитивы - линии, фигуры, текст, так и создавать сложные графические игры. В дополнение canvas позволяет манипулировать 
изображениями и даже видео.<p>
<p>Определим на веб-странице элемент canvas:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<p>Раньше, когда стандарты HTML5 еще только начинали внедряться, обычным делом было определение внутри элемента canvas напдписи о том, что браузер не поддерживает 
данный элемент. И если веб-браузер не поддерживает canvas, то пользователю отображалась эта надпись. Однако в настоящее время, когда стандарты HTML5, 
особенно canvas повсеместно внедрены, подобные надписи уже используются скорее по инерции.</p>
<p>Также, как правило, для элемента canvas задаются ширина и высота с помощью атрибутов <code>width</code> и <code>height</code>. Если не указать 
эти атрибуты, то по умолчанию ширина будет составлять 300, а высота - 150 пикселей.</p>
<h3>Получение контекста рисования</h3>
...
<p>Для получения контекста используется функция <code>getContext("2d")</code>. В данном случае мы получаем двухмерных контекст для создания двухмерной графики. 
Также контекст элемента canvas позволяет создавать трехмерную графику. Подробнее об этом можно почитать в руководстве по 
<a href="//metanit.com/web/webgl/">WebGL</a>. В данном же случае мы затронем только двухмерную графику.</p>
<h3>Рисование прямоугольников</h3>
<p>И наконец нам остается собственно нарисовать первую фигуру. Для рисования простейших фигур - прямоугольников нам могут понадобиться три метода:</p>
<ul>
<li><p><code>clearRect(x, y, w, h)</code>: очищает определенную прямоугольную область, верхний левый угол которой имеет координаты x и y, 
ширина равна w, а высота равна h</p></li>
<li><p><code>fillRect(x, y, w, h)</code>: заливает цветом прямоугольник, верхний леый угол которого имеет координаты x и y, 
ширина равна w, а высота равна h</p></li>
<li><p><code>strokeRect(x, y, w, h)</code>: рисует контур прямоугольника без заливки его каким-то определенным цветом</p></li>
</ul>
...
<img src="pics/6.1.png" alt="Элемент canvas в HTML5" />
<p>При рисовании в функции <code>strokeRect</code> и <code>fillRect</code> передаются координаты x и y верхнего левого угла прямоугольника относительно 
элемента canvas, а также ширина и высота прямоугольника. При этом началом координат считается верхний левый угол элемента canvas, ось 
X направлена вправо, а ось Y направлена вниз:</p>
<img src="pics/6.2.png" alt="Элемент canvas в HTML5" />
<p>В отличие от <code>strokeRect</code> и <code>fillRect</code> метод <code>clearRect</code> очищает определенную область, Фактически эта область приобретатет тот цвет, который у нее 
был бы, если бы к ней не применялись функции <code>strokeRect</code> и <code>fillRect</code>. Например:</p>
<pre class="brush:js;">
var canvas = document.getElementById("myCanvas"), 
    context = canvas.getContext("2d");
...
<p>В данном случае очищается небольшая часть прмоугольника в левом верхнем углу:</p>
<img src="pics/6.30.png" alt="Метод clearRect в canvas в HTML5" />
...
<div class="nav"><p><a href="7.3.php">Назад</a><a href="./">Содержание</a><a href="10.9.php">Вперед</a></p></div></div>
<div class="socBlock">
