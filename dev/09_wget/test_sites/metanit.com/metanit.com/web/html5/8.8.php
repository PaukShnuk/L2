...
<html >
<head>
<title>HTML5 | Создание простейшего макета</title>
<meta charset="utf-8" />
<meta name="description" content="Создание простейшего макета веб-страницы в HTML 5 и CSS, организация блоков и трехколоночный дизайн">
...
<h2>Создание простейшего макета</h2><div class="date">Последнее обновление: 28.04.2016</div>
...
<p>Используем полученные из прошлых тем сведения и создадим более менее осмысленный макет самой простейшей веб-страницы. Для начала определим базовую структуру веб-страницы:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
    ...
    &lt;body&gt;
		...
		&lt;div id="footer"&gt;
			...
			&lt;p&gt;Copyright &copy; MySyte.com, 2016&lt;/p&gt;
		&lt;/div&gt;
    &lt;/body&gt;
&lt;/html&gt;
</pre>
<p>В начале идет шапка сайта - блок с header, который содержит заголовок страницы и панель навигации. Далее идет блок wrapper, в котором два сайдбара и 
блок основного содержимого страницы. Сайдбары условно тоже содержат некоторое содержимое, но главное, что они определены до основного блока. И в самом низу 
небольшой футер.</p>
<p>В начале веб-страницы определено поключение файла <code>styles.css</code>, который будет стилизовать веб-страницу. Поэтому создадим в одном каталоге с 
веб-страницей файл <i>styles.css</i> и определим в нем следующее содержимое:</p>
<pre class="brush:css;">
* {
...
<p>Первые три стиля сбрасывают стилевые настройки по умолчанию для используемых нами элементов, а также устанавливат стиль элемента body.</p>
<p>Следующая пара стилей управляет отображением шапки (хедера) и заголовка страницы:</p>
<pre class="brush:css;">
#header{ 
	background-color: #f4f4f4;
}
...
<p>Следующий набор стилей управляет созданием горизонтальной панели навигации:</p>
<pre class="brush:css;">
#nav {
	background-color: #eee;
	border-top: 1px solid #ccc;
...
<p>В одной из прошлых тем подробно обсуждалось создание горизонтальной панели навигации. В принципе здесь ничего нового не добавляется: для элементов 
<code>&lt;li&gt;</code> устанавливается обтекание (<code>float: left;</code>), благодаря чему они размещаются в ряд, а каждая ссылка делается блочным элементом 
(<code>display: block;</code>)</p>
<p>Далее идет настройка средней части страницы, в частности, сайдбаров:</p>
<pre class="brush:css;">
.wrapper{
	background-color: #f7f7f7;
}
...
<p>Стиль класса wrapper позволяет установить фоновый цвет для боковых панелей. Для каждого сайдбара определяется ширина в 20% от ширины страницы. 
Процентные значения позволят автоматически подстраивать ширину блоков под ширину окна браузера при его расширении или сужении.</p>
<p>Далее следуют стили блока основного содержимого и футера:</p>
<pre class="brush:css;">
#article{
	background-color: #fafafa;
	border-left: 1px solid #ccc;
...
<p>Поскольку боковые панели имеют ширину в 20% каждая, то для главного блока устанавливается ширина в 60% и отступы справа и слева в 20%.</p>
<p>И в конце идет пара довольно важных стилей:</p>
<pre class="brush:css;">
#nav ul, #header h1, .wrapper, #footer p {
	max-width: 1200px;
	margin: 0 auto;	
...
<p>В начале для ряда селекторов определяется максимальная ширина в 1200 пикселей. Это значит, что основные элементы страницы не выйдут за 
пределы 1200 пикселей. А автоматический внешний отступ слева и справа позволит центрировать содержимое элементов. То есть при ширине браузера в 
1400 пикселей эти элементы с шириной в 1200 пикселей будут размещаться как бы по середине, а справа и слева будут отступы шириной в (1400-1200)/2 = 100 пикселей.</p>
<p>Второй стиль позволят сделать фиксированную минимальную ширину для ряда элементов. То есть в итоге при сжатии окна браузера сайдбары и основной блок будут 
выглядеть более менее, а при сжатии окна менее 768 пикселей образуется полоса прокрутки.</p>
<p>Данная модель размеров не идеальна. И далее мы рассмотрим более гибкие и распространенные концепции на основе адаптивной верстки.</p>
<p>В итоге у нас получится следующий простенький макет:</p>
<img src="pics/7.28.png" alt="Создание макета в HTML и CSS" />
...
<div class="nav"><p><a href="8.7.php">Назад</a><a href="./">Содержание</a><a href="8.9.php">Вперед</a></p></div></div>
<div class="socBlock">
