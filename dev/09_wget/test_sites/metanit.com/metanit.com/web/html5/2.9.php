...
<html >
<head>
<title>HTML5 | Ссылки</title>
<meta charset="utf-8" />
<meta name="description" content="Создание ссылок в HTML5, атрибут href, настройка ссылок и открытие документов">
...
<h2>Ссылки</h2><div class="date">Последнее обновление: 08.04.2016</div>
...
<p>Ссылки, которые представлены элементом <span class="b">&lt;a&gt;&lt;/a&gt;</span>, играют важную роль - они обеспечивают навигацию между отдельными 
документами. Этот элемент имеет следующие атрибуты:</p>
<ul>
<li><p><code>href</code>: определяет адрес ссылки</p></li>
<li><p><code>hreflang</code>: указывает на язык документа, на который ведет данная ссылка</p></li>
<li><p><code>media</code>: определяет устройство, для которого предназначена ссылка</p></li>
<li><p><code>rel</code>: определяет отношение между данным документом и ресурсом, на который ведет ссылка</p></li>
<li><p><code>target</code>: определяет, как документ по ссылке должен открываться</p></li>
<li><p><code>type</code>: указывает на mime-тип ресурса по ссылке</p></li>
</ul>
<p>Наиболее важным атрибутом является <code>href</code>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/1.32.png" alt="Ссылки в HTML5" />
<p>Здесь для ссылки используется относительный путь <i>content.html</i>. То есть в одной папке с данным документом должен находиться файл content.html, 
на который будет идти переход по нажатию на ссылку.</p>
<p>Также мы можем использовать абсолютные пути с полным указанием адреса:</p>
<pre class="brush:xml;">
&lt;a href="http://metanit.com/web/html5/"&gt;Учебник по HTML5&lt;/a&gt;
</pre>
<h3>Навигация внутри документа</h3>
<p>И также мы можем задать внутренние ссылки, которые будут переходить к определенным блокам внутри элементов:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<p>Чтобы определить внутреннюю ссылку, указывается знак решетки (#), после которого идет id того элемента, к которому надо осуществить переход. В данном 
случае переход будет идти к заголовкам h2.</p>
<h3>Атрибут target</h3>
<p>По умолчанию ресурсы, на которые ведут ссылке, открываются в том же окне. С помощью  атрибута <span class="b">target</span> можно переопределить 
это действие. Атрибут target может принимать следующие значения:</p>
<ul>
<li><code>_blank</code>: открытие html-документа в новом окне или вкладке браузера</p></li>
<li><code>_self</code>: открытие html-документа в том же фрейме (или окне)</p></li>
<li><code>_parent</code>: открытие документа в родительском фрейме, если ссылка расположена во внутреннем фрейме</p></li>
<li><code>_top</code>: открытие html-документа на все окно браузера</p></li>
<li><code>framename</code>: открытие html-документа во фрейме, который называется framename (В данном случае framename - только пример, название фрейма может быть произвольным)</p></li>
</ul>
<p>Например, открытие документа по ссылке в новом окне:</p>
<pre class="brush:xml;">
&lt;a href="http://metanit.com/web/html5/" target="_blank"&gt;Учебник по HTML5&lt;/a&gt;
</pre>
<p>Значение <code>_blank</code> как раз и указывает браузеру, что ресурс надо открыть в новой вкладке.</p>
<h3>Стилизация ссылок</h3>
<p>По умолчанию ссылка уже имеет некоторый цвет (один из оттенков синего), кроме того она имеет подчеркивание. При нажатии на ссылку 
...
Подобная стилизация задается многими браузерами по умолчанию, но мы можем ее переопределить. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Здесь определены стили для ссылок в различных состояниях. <code>a:link</code> применяется для ссылок в обычном состоянии, когда они не нажаты и на них не наведен указатель мыши.</p>
<p><code>a:visited</code> указывает на состояние ссылки, по которой уже был осуществлен переход.</p>
<p><code>a:hover</code> указывает на состояние ссылки, на которую навели указатель мыши.</p>
<p><code>a:active</code> указывает на ссылку в нажатом состоянии.</p>
<p>Стиль <code>color</code> устанавливает цвет ссылки. А стиль <code>text-decoration</code> устанавливает подчеркивание: если значение <code>underline</code>, то ссылка поддчеркнута, 
если <code>none</code>, то подчеркивание отсутствует.</p>
<h3>Ссылка-картинка</h3>
<p>Поместив внутрь элемента <code>&lt;a&gt;</code> элемент <code>&lt;img&gt;</code>, можно сделать ссылку-изображение:</p>
<pre class="brush:xml;">
&lt;a href="index.html"&gt;
