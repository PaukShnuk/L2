...
<html >
<head>
<title>HTML5 | Метатег Viewport</title>
<meta charset="utf-8" />
<meta name="description" content="Роль метатега viewport в создании адаптивного дизайна в HTML 5, настройка тега">
...
<h2>Метатег Viewport</h2><div class="date">Последнее обновление: 03.05.2016</div>
...
<p>Прежде всего рассмотрим один из ключевых моментов применения адаптивного дизайна - метатег <span class="b">viewport</span> (по сути с этого тега и начивается адаптиный дизайн). 
Пусть для начала у нас есть следующая веб-страница:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Это стандартная веб-страница, которая в обычном браузере будет выглядеть следующим образом:</p>
<img src="pics/9.4.png" alt="Не адаптивный дизайн в Google Chrome" />
<p>Однако если мы запустим ту же самую веб-страницу в эмуляторе мобильного устройства или на реальном мобильном устройстве, то мы с трудом сможем прочитать, что 
же на ней написано:</p>
<img src="pics/9.3.png" alt="Эмулятор Opera Mobile" />
<p>Применяя масштабирование, пользователь может наконец-то увидеть, что же там все таки написано. Однако это не очень удобно. При этом веб-страница 
имеет много пустого места, что не очень красиво.</p>
<p>Почему так происходит? Дело в том, что каждый мобильный браузер задает странице некоторые начальные размеры и потом пытается приспособить под размеры экрана текущего мобильного устройства.</p>
<p>Вся видимая область на экране браузера описывается понятием <span class="b">Viewport</span>. По сути viewport представляет область, в которую веб-браузер 
...
320 пикселей, то до этого размера потом будет сжата страница. И ко всем элементам страницы будет применен коэффициент масштабирования, равный 320/980.</p>
<p>Почему в данном случае используется именно 980 пикселей, а, скажем, не реальный размер экрана? Все дело в том, что по умолчанию браузер считает, что каждая веб-страница предназначена для десктопов. А обычной шириной десктопного сайта можно считать 980 пикселей.</p>
<p>При этом для каждого браузера устанавливается своя ширина области viewport, необязательно 980 пикселей. Другие браузеры могут поддерживать в 
качестве начальной ширины другие значения. Но они также будут выполнять масштабирование.</p>
<p>Чтобы избежать подобной не очень приятной картины, следует использовать метатег viewport. Он имеет следующее определение:</p>
<pre class="brush:xml;">&lt;meta name="viewport" content="параметры_метатега"&gt;</pre>
...
<img src="pics/9.5.png" alt="веб-страница с использованием тега viewport в HTML5" />
