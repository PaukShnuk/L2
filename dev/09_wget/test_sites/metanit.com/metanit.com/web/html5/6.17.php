...
<html >
<head>
<title>CSS3 | Обтекание элементов</title>
<meta charset="utf-8" />
<meta name="description" content="Обтекание элементов в CSS 3, свойство float, блочная верстка, запрет обтекания и свойство clear">
...
<h2>Обтекание элементов</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Как правило, все блоки и элементы на веб-странице в браузере появляются в том порядке, в каком они определены в коде html. Однако CSS предоставляет 
специальное свойство <span class="b">float</span>, которое позволяет установить обтекание элементов, благодаря чему мы можем создать более 
интересные и разнообразные по своему дизайну веб-страницы.</p>
<p>Это свойство может принимать одно из следующих значений:</p>
<ul>
<li><p><code>left</code>: элемент перемещается влево, а все содержимое, которое идет ниже его, обтекает правый край элемента</p></li>
<li><p><code>right</code>: элемент перемещается вправо</p></li>
<li><p><code>none</code>: отменяет обтекание и возвращает объект в его обычную позицию</p></li>
</ul>
<p>При применении свойства <span class="b">float</span> для стилизуемых элементов, кроме элемента img, рекомендуется установить свойство width.</p>
<p>Итак, представим, что нам надо на странице вывести слева от основного текста изображение, справа должен быть сайдбар, а все остальное место должно 
быть занято основным текстом статьи. Определим интерфейс страницы сначала без свойства float:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>В данном случае мы получим последовательное размещение элементов на странице:</p>
<img src="pics/4.57.png" alt="Обтекание элементов в CSS 3" />
<p>Теперь на той же странице применим свойство <code>float</code>, изменив стили следующим образом:</p>
<pre class="brush:css;">
.image { 
...
<p>Соответственно изменится и размещение элементов на странице:</p>
<img src="pics/4.58.png" alt="Свойство float в CSS 3" />
<p>Элементы, к которым применяется свойство <code>float</code>, еще называют floating elements или плавающими элементами.</p>
<p>Запрет обтекания. Свойство clear</p>
<p>Иногда возникает необходимость запретить обтекания. Подобная задача может быть актуальна, если какой-то блок должен переноситься вниз на новую строку, а не обтекать 
...
что не желательно.</p>
<p>Для запрета обтекания элементов в CSS применяется свойство <span class="b">clear</span>, которое указывает браузеру, что к стилизуемому элементу не должно применяться обтекание.</p>
<p>Свойство <code>clear</code> может принимать следующие значения:</p>
<ul>
<li><p><code>left</code>: стилизуемый элемент может обтекать плавающий элемент справа. Слева же обтекание не работает</p></li>
<li><p><code>right</code>: стилизуемый элемент может обтекать плавающий элемент только слева. А справа обтекание не работает</p></li>
<li><p><code>both</code>: стилизуемый элемент может обтекать плавающие элементы и относительно них смещается вниз</p></li>
<li><p><code>none</code>: стилизуемый элемент ведет себя стандартным образом, то есть принимает участие в обтекании справа и слева</p></li>
</ul>
<p>Например, пусть на веб-странице будет определен футер:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
    ...
    &lt;body&gt;
		...
		&lt;div class="footer"&gt;Copyright &copy; MyCorp. 2016&lt;/div&gt;
	&lt;/body&gt;
&lt;/html&gt;
</pre>
<p>Наличие обтекания будет создавать некорректное отображение, при котором футер смещается вверх:</p>
<img src="pics/4.59.png" alt="Запрет обтекания элементов в CSS" />
<p>Изменим стиль футера:</p>
<pre class="brush:css;">
.footer{
	border-top: 1px solid #ccc;
	clear: both;
...
<p>Теперь футер не будет обтекать изображение, а будет уходить вниз.</p>
<img src="pics/4.60.png" alt="Свойство clear в CSS" />
...
<div class="nav"><p><a href="6.16.php">Назад</a><a href="./">Содержание</a><a href="6.18.php">Вперед</a></p></div></div>
<div class="socBlock">
