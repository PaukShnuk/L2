...
<html >
<head>
<title>HTML5 | Рисование изображений на Canvas</title>
<meta charset="utf-8" />
<meta name="description" content="Рисование изображений на Canvas в HTML5, метод drawImage">
...
<h2>Изображения на canvas</h2><div class="date">Последнее обновление: 02.05.2016</div>
...
<p>Ранее уже рассматривалась установка изображений в качестве фона в прямоугольниках, но мы также может отдельно выводить изображения на canvas. 
Для этого применяется метод <span class="b">drawImage()</span>:</p>
<pre class="brush:js;">context.drawImage(image, x, y)</pre>
<p>Здесь параметр image передает выводимое изображение, а x и y - координаты верхнего левого угла изображения. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/6.24.png" alt="Вывод изображений на canvas в HTML5" />
<p>Опять же при выводе изображения нам надо быть уверенными, что изображение уже загружено браузером и готово к использованию, поэтому метод отрисовки изображения 
помещается в обработчик загрузки изображения <code>img.onload</code>.</p>
...
<img src="pics/6.25.png" alt="Вывод двух изображений на canvas в HTML5" />
<p>Метод <code>drawImage()</code> также имеет еще и третью форму:</p>
<pre class="brush:js;">drawImage(image, sx, sy, sWidth, sHeight, dx, dy, dWidth, dHeight)</pre>
<p>Где параметры <code>sx</code> и <code>sy</code> представляют координаты на изображении, с которого начиется обрезка изображения, а параметры 
<code>sWidth</code> и <code>sHeight</code> представляют соответственно ширину и высоту выреза относительно координат sx и sy.</p>
<p>Параметры <code>dx</code> и <code>dy</code> указывают координаты отрисовки обрезанного изображения на canvas, а <code>dWidth</code> и <code>dHeight</code> 
указывают соответственно на ширину и высоту изображения на canvas.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<img src="pics/6.26.png" alt="Настройка вывода изображения на canvas в HTML5" />
<h3>Захват изображений с других элементов</h3>
<p>Одной из замечательных функциональностей элемента canvas является возможность захвата изображения с другого элемента, например, элемента <code>video</code> 
или другого элемента canvas. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/6.34.png" alt="Получение изображения из видео на canvas" />
<p>По нажати на кнопку canvas будет получать текущий кадр воспроизводимого видео и фиксировать его в качестве изображения. При этом в метод <code>drawImage</code> 
в качестве первого параметра передается сам элемент, используемый как источник изображения.</p>
