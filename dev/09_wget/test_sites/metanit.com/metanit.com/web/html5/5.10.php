...
<html >
<head>
<title>CSS3 | Каскадность стилей</title>
<meta charset="utf-8" />
<meta name="description" content="Каскадность стилей в CSS 3, порядок применение селекторов, градация селекторов по типам значимости">
...
<h2>Каскадность стилей</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Когда к определенному элементу применяется один стиль, то все относительно просто. Однако если же к одному и тому же элементу применяется сразу несколько различных стилей, 
то возникает вопрос, какой же из этих стилей будет в реальности применяться?</p>
<p>В CSS действует механизм каскадности, которую можно определить как набор правил, определяющих последовательность применения множества стилей к одному и тому же элементу.</p>
<p>К примеру, у нас определена следующая веб-страница:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>В CSS определено три стиля и все они применяются к ссылке.</p>
<p>Если к элементу веб-страницы применяется несколько стилей, которые не конфликтуют между собой, то браузер объединяет их в один стиль.</p>
<p>Так, в данном случае, все три стиля не конфликтуют между собой, поэтому все эти стили будут применяться к ссылке:</p>
<img src="pics/4.24.png" alt="Каскадность в CSS 3" />
<p>Если же стили конфликтуют между собой, например, определяют разный цвет текста, то в этом случае применяется сложная система правил для вычисления значимости каждого стиля. 
Все эти правила описаны в спецификации по CSS: <a href="https://www.w3.org/TR/css3-selectors/#specificity" rel="nofollow">Calculating a selectors specificity</a>. 
Вкратце разберем их.</p>
<p>Для определения стиля к элементу могут применяться различные селекторы, и важность каждого селектора оценивается в баллах. Чем больше у селектора 
пунктов, тем он важнее, и тем больший приоритет его стили имеют над стилями других селекторов.</p>
<ul>
<li><p>Селекторы тегов имеют важность, оцениваемую в 1 балл<p></li>
<li><p>Селекторы классов, атрибутов и псевдоклассов оцениваются в 10 баллов<p></li>
<li><p>Селекторы идентификаторов оцениваются в 100 баллов<p></li>
<li><p>Встроенные inline-стили (задаваемые через атрибут <code>style</code>) оцениваются в 1000 баллов<p></li>
</ul>
<p>Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
...
<p>Здесь к ссылке применяется сразу три стиля. Эти стили содержат два не конфликтующих правила:</p>
<pre class="brush:css;">
font-size: 20px;
font-weight: bold;
</pre>
<p>которые устанавливают высоту шрифта 20 пикселей и выделение ссылки жирным. Так как каждое из эти правил определено только в одном стиле, то в итоге они будут суммироваться 
и применяться к ссылке без проблем.</p>
<p>Кроме того, все три стиля содержат определение цвета текста, но каждый стиль определяет свой цвет текста. Так как селекторы идентификаторов имеют больший удельный вес, 
то в конечном счете будет применяться темно-синий цвет, задаваемый селектором:</p>
<pre class="brush:css;">
#index {color: navy;}
</pre>
<p>Если селектор является составным, то происходит сложение баллов всех входящих в селектор подселекторов. Так, рассмотрим следующий пример:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>В CSS определено аж пять различных селекторов, которые устанавливают цвет ссылок. В итоге браузер выберет селектор <code>#menu a</code> и окрасит ссылки 
в темно-синий цвет. Но почему, на каком основании браузер выберет этот селектор?</p>
<p>Рассмотрим, как у нас будут суммироваться баллы по каждому из пяти селекторов:</p>
<table class="tab">
<tr class="tabhead"><td><p>Селектор</p></td><td><p>Идентификаторы</p></td><td><p>Классы</p></td><td><p>Теги</p></td><td><p>Сумма</p></td></tr>
<tr><td><p>.nav li a</p></td><td><p>0</p></td><td><p>1</p></td><td><p>2</p></td><td><p>12</p></td></tr>
<tr><td><p>#menu a</p></td><td><p>1</p></td><td><p>0</p></td><td><p>1</p></td><td><p>101</p></td></tr>
<tr><td><p>.nav .menuItem</p></td><td><p>0</p></td><td><p>2</p></td><td><p>0</p></td><td><p>20</p></td></tr>
<tr><td><p>a.menuItem:not(.newsLink)</p></td><td><p>0</p></td><td><p>2</p></td><td><p>1</p></td><td><p>21</p></td></tr>
<tr><td><p>div ul li a</p></td><td><p>0</p></td><td><p>0</p></td><td><p>4</p></td><td><p>4</p></td></tr>
</table>
<p>Итак, мы видим, что для селектора <code>#menu a</code> в колонке сумма оказалось больше всего баллов - 101. То есть в нем 1 идентификатор (100 баллов) 
и один тег(1 балл), которые в сумме дают 101 балл.</p>
<p>К примеру, в селекторе <code>.nav .menuItem</code> два селектора класса, каждый из которых дает 10 баллов, то есть в сумме 20 баллов.</p>
<p>При этом псевдокласс <code>:not</code> в отличие от других псевдокдассов не учитывается, однако учитывается тот селектор, который передается в псевдокласс not.</p>
<h3>Правило !important</h3>
<p>CSS предоставляет возможность полностью отменить значимость стилей. Для этого в конце стиля указывается значение <span class="b">!important</span>:</p>
<pre class="brush:css;">
a {font-size: 18px; color: red !important;}
#menu a {color: navy;}
</pre>
<p>В этом случае вне зависимости от наличия других селекторов с большим количеством баллов к ссылкам будет применяться красный цвет, определяемый первым стилем.</p>
...
<div class="nav"><p><a href="5.9.php">Назад</a><a href="./">Содержание</a><a href="./5.13.php">Вперед</a></p></div></div>
<div class="socBlock">
