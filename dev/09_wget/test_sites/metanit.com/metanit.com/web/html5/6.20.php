...
<html >
<head>
<title>CSS3 | Радиальный градиент</title>
<meta charset="utf-8" />
<meta name="description" content="Радиальный градиент в CSS 3, параметры градиента и функция radial-gradient, кроссбраузерность">
...
<h2>Радиальный градиент</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Радиальные градиенты в отличие от линейных распространяются от центра наружу по круговой схеме. Для создания радиального градиента достаточно 
указать цвет, который будет в центре градиента, и цвет, который должен быть снаружи. Эти цвета передаются в функцию <span class="b">radial-gradient()</span>. 
Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<img src="pics/4.67.png" alt="Радиальный градиент в CSS 3" />
<p>Как и в случае с линейным градиентом здесь также надо использовать префиксы вендоров для поддержки браузерами.</p>
<p>Радиальный градиент может иметь две формы: круговую и эллиптическую. Эллиптическая форма представляет распространение градиента в виде 
эллипса и задается с помощью ключевого слова <code>ellipse</code>:</p>
<pre class="brush:css;">background-image: radial-gradient(ellipse, white, black);</pre>
<p>Поскольку это значение для градиента по умолчанию, то оно может опускаться при использовании.</p>
<p>Круговая форма представляет распространение градиента в виде кругов от центра во вне. Для этого используется ключевое слово <code>circle</code>:</p>
<pre class="brush:css;">background-image: radial-gradient(circle, white, black);</pre>
<p>Как правило, центр радиального градиента расположен в центре элемента, но это поведение можно переопределить, указав значение для параметра 
<code>background-position</code>:</p>
<pre class="brush:css;">background-image: radial-gradient(25% 30%, circle, white, black);</pre>
<p>Числа <code>25% 30%</code> означают, что центр градиента будет находиться на расстоянии в 25% от левой границы и в 30% от верхней границы элемента.</p>
<img src="pics/4.68.png" alt="Позиционирование радиального градиента в CSS 3" />
<p>С помощью специальных дополнительных значений можно задать размер градиента:</p>
<ul>
<li><p><code>closest-side</code>: градиент распространяется из центра только до ближайшей к центру стороне элемента. То есть градиент остается 
внутри элемента</p></li>
<li><p><code>closest-corner</code>: ширина градиента вычисляется по расстоянию из его центра до ближайшего угла элемента, поэтому градиент может выйти за пределы элемента.</p></li>
<li><p><code>farthest-side</code>: градиент распространяется из центра до самой дальней стороны элемента</p></li>
<li><p><code>farthest-corner</code>: ширина градиента вычисляется по расстоянию из его центра до самого дальнего угла элемента</p></li>
</ul>
<p></p>
<pre class="brush:css;">
...
<div class="nav"><p><a href="6.19.php">Назад</a><a href="./">Содержание</a><a href="6.21.php">Вперед</a></p></div></div>
<div class="socBlock">
