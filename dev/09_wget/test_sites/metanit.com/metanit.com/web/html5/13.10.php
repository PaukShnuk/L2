...
<html >
<head>
<title>HTML5 | Именованные grid-линии и функция repeat в Grid Layout</title>
<meta charset="utf-8" />
<meta name="description" content="Именованные grid-линии и функция repeat в Grid Layout в CSS3, свойства grid-auto-flow и order">
...
<h2>Именованные grid-линии и функция repeat</h2><div class="date">Последнее обновление: 09.05.2017</div>
...
<p>С помощью ранее рассмотренной функции repeat мы можем растиражировать столбцы и строки, которые создаются между именованными grid-линиями:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Возьмем из данного примера определение столбцов:</p>
<pre class="brush:css;">grid-template-columns: 10px repeat(3, [column] 1fr [colgutter] 10px);</pre>
<p>Первый столбец будет иметь ширину в 10 пикселей. Затем происходит тиражирование столбцов с помощью функции repeat. Она создает подряд три копии двух столбцов. 
Первый столбец имеет ширину 1fr, то есть имеет пропорциональные размеры, и располагается между grid-линиями "column" и "colgutter". 
После grid-линии "colgutter" идет еще один столбец шириной в 10 пикселей. И эти два столбца будут повторяться три раза. 
То есть всего в гриде будет 7 столбцов.</p>
<p>Со строками будет во многом аналогично, только там создается 5 строк с помощью grid-линий "row" и "rowgutter".</p>
<p>При определении стиля элементов, используя имя grid-линий и их порядковый номер, мы можем явным образом указать с помощью свойств 
<span class="b">grid-column</span> и <span class="b">grid-row</span>, где именно должен располагаться элемент:</p>
<pre class="brush:css;">
.special-item{
...
<p>Причем свойство <code>grid-column: column 2</code> указывает на столбец в гриде, который начинается со второй grid-линии "column". В итоге мы получим следующий грид:</p>
<img src="pics/grid18.png" alt="Именованные столбцы и строки в Grid Layout и CSS 3" />
<p>Возможно, многие найдут такой подход более интуитивно понятным для определения позиции элемента.</p>
<p>И более того мы можем дополнительно добавлять новые именованные грид-линии вне функции repeat:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>И также стоит отметить, что вне зависимости от того, именованные строки,столбцы и grid-линии или неименованные, мы по прежнему можем позиционировать 
элементы, используя номера grid-линий, как в данном случае происходит в отношении сайдбара:</p>
<pre class="brush:css;">
.sidebar{
	grid-column: sidebarstart / sidebarend;
	grid-row: 2 / 5;
...
<img src="pics/grid19.png" alt="Сайдбар в Grid Layout в CSS3" />
...
<div class="nav"><p><a href="13.9.php">Назад</a><a href="./">Содержание</a><a href="13.11.php">Вперед</a></p></div></div>
<div class="socBlock">
