...
<html >
<head>
<title>CSS3 | Наследование стилей</title>
<meta charset="utf-8" />
<meta name="description" content="Наследование стилей в CSS 3, заимствование вложенными элементами стилей родительских элементов">
...
<h2>Наследование стилей</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Для упрощения определения стилей в CSS применяется механизм наследования стилей. Этот механизм предполагает, что вложенные элементы могут наследовать 
...
отдельно к параграфу и заголовку применить соответствующий стиль, который установит нужный цвет шрифта:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Однако поскольку и элемент p, и элемент h2 находятся в элементе body, то они наследуют от этого контейнера - элемента body многие стили. И чтобы не дублировать определение стиля, 
мы могли бы написать так:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>В итоге определение стилей стало проще, а результат остался тем же.</p>
<p>Если нам нежелателен унаследованный стиль, то мы его можем переопределить для определенных элементов:</p>
<pre class="brush: css;">
body {color: red;}
p {color: green;}
</pre>
<p>При нескольких уровнях вложенности элементы наследуют стили только ближайшего контейнера:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
    ...
    &lt;body&gt;
		&lt;div&gt;
			...
			&lt;p&gt;Текст про наследование стилей в CSS 3&lt;/p&gt;
		&lt;/div&gt;
		&lt;p&gt;Copyright &copy; MyCorp.com&lt;/p&gt;
    &lt;/body&gt;
&lt;/html&gt;
</pre>
<p>Здесь веб-страница имеет следующую структуру:</p>
<img src="pics/4.22.png" alt="Структура html и наследование стилей в css" />
<p>Для элемента div переопределяется цвет текста. И так как элемент h2 и один из параграфов находятся в элементе div, то они наследуют стиль именно элемента div. 
Второй параграф находится непосредственно в элементе body и поэтому наследует стиль элемента body.</p>
<img src="pics/4.23.png" alt="наследование стилей в css" />
<p>Однако не ко всем свойствам CSS применяется наследование стилей. Например, свойства, которые представляют отступы (margin, padding) и границы (border) элементов, не наследуются.</p>
<p>Кроме того, браузеры по умолчанию также применяют ряд предустановленных стилей к элементам. Например, заголовки имеют определенную высоту и т.д.</p>
