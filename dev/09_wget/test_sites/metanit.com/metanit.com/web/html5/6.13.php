...
<html >
<head>
<title>CSS3 | Размеры элементов. Box-sizing</title>
<meta charset="utf-8" />
<meta name="description" content="Определение ширины и высоты элементов в CSS 3, минимальные и максимальные размеры, свойство box-sizing">
...
<h2>Размеры элементов</h2><div class="date">Последнее обновление: 30.09.2021</div>
...
<p>Размеры элементов задаются с помощью свойств <span class="b">width</span> (ширина) и <span class="b">height</span> (высота).</p>
<p>Значение по умолчанию для этих свойств - <code>auto</code>, то есть браузер сам определяет ширину и высоту элемента. 
Можно также явно задать размеры с помощью единиц измерения (пикселей, em) или с помощью процентов:</p>
<pre class="brush:css;">
width: 150px;
width: 75%;
height: 15em;
</pre>
<p>Пиксели определяют точные ширину и высоту. Единица измерения <span class="b">em</span> зависит от высоты шрифта в элементе. Если 
...
Если же у элемента не определен размер шрифта, то он будет взят из унаследованных параметров или значений по умолчанию.</p>
<p>Процентные значения для свойства width вычисляются на основании ширины элемента-контейнера. Если, к примеру, 
ширина элемента body на веб-странице составляет 1000 пикселей, а вложенный в него элемент <code>&lt;div&gt;</code> имеет ширину 75%, 
то фактическая ширина этого блока <code>&lt;div&gt;</code> составляет 1000 * 0.75 = 750 пикселей. 
Если пользователь изменит размер окна браузера, то ширина элемента body и соответственно ширина вложенного в него блока div тоже изменится.</p>
<p>Процентные значения для свойства height работают аналогично свойству width, только теперь высота вычисляется по высоте элемента-контейнера.</p>
<p>Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<img src="pics/4.55.png" alt="Ширина и высота в CSS 3" />
<p>В то же время фактические размеры элемента могут в итоге отличаться от тех, которые установлены в свойствах <code>width</code> и <code>height</code>. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/4.56.png" alt="Фактические размеры в CSS 3" />
<p>Как видно на скриншоте, в реальности значение свойства <code>width</code> - 200px - определяет только ширину внутреннего содержимого элемента, 
...
230 пикселей, а ширина блока элемента с учетом внешних отступов составит 250 пикселей.</p> 
<p>Подобные расчеты следует учитывать при определении размеров элементов.</p>
<p>С помощью дополнительного набора свойств можно установить минимальные и максимальные размеры:</p>
<ul>
<li><p><span class="b">min-width</span>: минимальная ширина</p></li>
<li><p><span class="b">max-width</span>: максимальная ширина</p></li>
<li><p><span class="b">min-height</span>: минимальная высота</p></li>
<li><p><span class="b">max-height</span>: максимальная высота</p></li>
</ul>
<pre class="brush:css;">
min-width: 200px;
width:50%;
...
<p>В данном случае ширина элемента равна 50% ширины элемента-контейнера, однако при этом не может быть меньше 200 пикселей и больше 300 пикселей.</p>
<h3>Переопределение ширины блока</h3>
<p>Свойство <span class="b">box-sizing</span> позволяет переопределить установленные размеры элементов. Оно может принимать одно из следующих значений:</p>
<ul>
<li><p><code>content-box</code>: значение свойства по умолчанию, при котором браузер для определения реальных ширины и высоты элементов 
добавляет берет соответственно значения свойств <code>width</code> и <code>height</code> элемента</p>. Например:</li>
<pre class="brush:css;">
width: 200px;
height: 100px;
margin: 10px;
...
<p>В данном случае элемент будет иметь ширину в 200 пикселей и высоту в 100 пиксей.</p>
<li><p><code>padding-box</code>: указывает веб-браузеру, что ширина и высота элемента должны включать внутренние отступы как часть своего значения. Например, пусть у нас есть следующий стиль:</p>
<pre class="brush:css;">
width: 200px;
height: 100px;
margin: 10px;
...
<p>Здесь реальная ширина внутреннего содержимого блока будет равна 200px (width) - 10px (padding-left) - 10px (padding-right) = 180px.</p>
<p>Стоит отметить, что большинство современных браузеров не поддерживают данное свойство.</p></li>
<li><p><code>border-box</code>: указывает веб-браузеру, что ширина и высота элемента должны включать внутренние отступы и границы как часть своего значения. 
Например, пусть у нас есть следующий стиль:</p>
<pre class="brush:css;">
width: 200px;
height: 100px;
margin: 10px;
...
<p>Здесь реальная ширина внутреннего содержимого блока будет равна 200px (width) - 10px (padding-left) - 10px (padding-right) - 5px (border-left-width) 
- 5px (border-right-width) = 170px.</p></li>
</li>
</ul>
<p>Например, определим два блока, которые отличаются только значением свойства box-sizing:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>В первом случае при определении размеров блока к свойствам width и height будут добавляться толщина границы, а также внутренние и внешние отступы, поэтому 
первый блок будет иметь большие размеры:</p>
<img src="pics/4.78.png" alt="Box-sizing в CSS 3" />
...
<div class="nav"><p><a href="6.12.php">Назад</a><a href="./">Содержание</a><a href="6.14.php">Вперед</a></p></div></div>
<div class="socBlock">
