...
<html >
<head>
<title>HTML5 | Выравнивание плавающих элементов</title>
<meta charset="utf-8" />
<meta name="description" content="Выравнивание плавающих элементов в HTML 5, свойство box-sizing">
...
<h2>Выравнивание плавающих элементов</h2><div class="date">Последнее обновление: 27.04.2016</div>
...
<p>При работе с плавающими элементами и свойством <span class="b">float</span> довольно часто можно столкнуться с проблемой выпадения из страницы плавающих 
элементов. У этой проблемы есть различные аспекты и их решения. Рассмотрим эти аспекты.</p>
<p>Например, пусть у нас задан следующий блок:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>В сайдбаре довольно много текста, который, как ожидается, будет эффективно вписан в границы плавающего блока. Однако в реальности мы можем получить проблему:</p>
<img src="pics/7.19.png" alt="Выпадение плавающих блоков в HTML5" />
<p>Как видно на скриншоте буквы вылезают из плавающего блока за границу, несмотря даже на то, что по идее в плавающем блоке должен быть установлен еще и внутренний 
отступ в 10 пикселей от правой границы.</p>
<p>Почему так происходит? Зачастую браузеры своеобразно интерпретируют размеры элемента. В частности, у всех элементов по умолчанию для свойства <span class="b">box-sizing</span> используется 
значение <span class="b">content-box</span>, то есть при определении ширины и высоты элемента браузер будет прибавлять к значению свойств <code>width</code> и 
<code>height</code> также и внутренние отступы padding и ширину границы. В итоге это может привести к выпадению плавающих элементов из тех блоков, которые для них предназначены. 
Поэтому часто для всех элементов рекомендуется устанавливать для свойства <span class="b">box-sizing</span> значение <span class="b">border-box</span>, чтобы все элементы измерялись одинаково, 
а их ширина представляла только значение свойства <code>width</code>. Поэтому нередко в стилях добавляется следующий стиль:</p>
<pre class="brush:css;">
* {
	box-sizing: border-box;
}
</pre>
<p>То есть значение <code>box-sizing: border-box;</code> устанавливается для всех элементов, и все они интерпретируются браузером одинаково. 
К примеру, добавим этот стиль в выше определенную страницу и мы получим уже несколько другой результат:</p>
<img src="pics/7.20.png" alt="border-box и float в CSS3" />
<p>Рассмотрим другую проблему, которая связана с позиционированием плавающих элементов в контейнере:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/7.21.png" alt="навигация float в CSS3" />
<p>Несмотря на то, что панель навигации определена в блоке с id header, визуально она явно не находится в элементе head. Ну и кроме того, 
здесь также можно увидеть, что появляются непонятные отступы, а следующий после хедера блок залезает на меню.</p>
<p>Проблема отступов заключается в том, что браузер по умолчанию определяет для различных элементов встроенные стили. Поэтому может немного сбивать 
с толку, как и где эти стили определены, почему они применются. Нередко для решения этой проблемы разработчики просто сбрасывают некоторые наиболее 
значимые стили для большинства элементов:</p>
<pre class="brush:css;">
...
<p>Другая проблема - наложение элемента div с основным контентом на плавающий блок навигационной панели решается довольно просто - установкой для 
этого элемента div следующего стиля:</p>
<pre class="brush:css;">clear: both;</pre>
<p>Более сложной является проблема с выпадением плавающих элементов меню из границ блока-контейнера. Здесь есть два возможных варианта Решения. 
Первое решение состоит в добавлении к элементу, который представляет панель навигации, следующего стиля:</p>
<pre class="brush:css;">
ul:after {
	content: " ";
	display: table;
...
<p>Второе решение состоит в том, чтобы сделать сам блок панели навигации плавающим:</p>
<pre class="brush:css;">
#nav{
	background-color: #f4f4f4;
	border-top: 1px solid #ccc;
...
<p>Итак, с учетом выше сказанного изменим стили для веб-страницы (код html остается прежним):</p>
<pre class="brush:css;">
*{
	box-sizing: border-box;
}
...
<p>И теперь веб-страница будет выглядеть иначе, собственно как и должна:</p>
<img src="pics/7.22.png" alt="Выравнивание блока ссылок в HTML" />
...
<div class="nav"><p><a href="8.6.php">Назад</a><a href="./">Содержание</a><a href="8.8.php">Вперед</a></p></div></div>
<div class="socBlock">
