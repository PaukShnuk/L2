...
<html >
<head>
<title>HTML5 | Создание панели навигации</title>
<meta charset="utf-8" />
<meta name="description" content="Создание панели навигации в HTML 5, горизонтальный и вертикальный блок ссылок">
...
<h2>Создание панели навигации</h2><div class="date">Последнее обновление: 27.04.2016</div>
...
<p>Панель навигации играет важную роль на сайте, так как обеспечиват переходы между страницами сайта или на внешние ресурсы. Рассмотрим, как создать простенькую 
панель навигации.</p>
<p>Фактически панель навигации - это набор ссылок, часто в виде ненумерованного списка. Панели навигации бывают самыми различными: вертикальными и горизонтальными, 
одноуровневыми и многоуровневыми, но в любом случае в центре каждой навигации находится элемент <span class="b">&lt;a&gt;</span>. Поэтому при создании панели навигации 
мы можем столкнуться с рядом трудностей, которые вытекают из ограничений элемента ссылки. А именно, элемент <code>&lt;a&gt;</code> является строчным, 
а это значит, что мы не можем указать для него ширину, высоту, отступы. По ширине ссылка автоматически занимает то место, которое ей необходимо.</p>
<h3>Вертикальное меню</h3>
<p>Распространенное решение данной проблемы для создания вертикального меню состоит в том, чтобы сделать ссылку блочным элементом.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/7.16.png" alt="Стилизация ссылок и навигация в CSS3" />
<p>После установки свойства <code>display: block</code> мы можем определить у блока ссылки ширину, отступы и т.д.</p>

<h3>Горизонтальное меню</h3>
<p>Для создания горизонтального меню есть два метода. Первый заключается в применении свойства <code>float</code> и создании из ссылок плавающих элементов, 
которые обтекают друг друга с слева. И второй способ состоит в создании строки ссылок с помощью установки свойства <code>display: inline-block</code>.</p>
<h4>Использование float</h4>
<p>Алгоритм создания панели навигации с помощью float разделяется на два этапа. На первом этапе у элемента <span class="b">li</span>, в который заключена 
ссылка, устанавливается <code>float: left;</code>. Это позволяет расположить все элементы списка в ряд при достаточной ширине, когда правый элемент списка 
обтекает левый элемент списка.</p>
<p>Второй этап заключается в установке у элемента ссылки <code>display: block</code>, что дает нам возможность устанавливать ширину, отступы, вообщем все те признаки, 
которые характерны для блочных элементов.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/7.18.png" alt="Горизонтальный ряд ссылок в CSS3" />
<h4>inline и inline-block</h4>
<p>Для создания горизонтальной панели навигации нам надо сделать каждый элемент <span class="b">li</span> строчным, то есть установить 
для него <code>display: inline</code>. После этого для элемента ссылки, которая располагается в элементе <code>li</code>, мы можем 
установить <code>display: inline-block</code>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/7.17.png" alt="Горизонтальная панель навигации в CSS3" />
...
<div class="nav"><p><a href="8.5.php">Назад</a><a href="./">Содержание</a><a href="8.7.php">Вперед</a></p></div></div>
<div class="socBlock">
