...
<html >
<head>
<title>HTML5 | Media API. Управление видео из JavaScript</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с Media API в HTML5, управление воспроизведением медиа из JavaScript, создание своего элемента проигрывания видео">
...
<h2>Media API. Управление видео из JavaScript</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Вместе с новыми элементами audio и video в HTML5 был добавлен новый API в JavaScript для управления этими элементами. С помощью кода JavaScript 
мы можем получить элементы video и audio (как и любой другой элемент) и использовать их свойства. В JavaScript эти элементы 
представлены объектом <span class="b">HTMLMediaElement</span>, который с помощью свойств, методов и событий позволяет управлять воспроизведением аудио и видео. 
Отметим наиболее важные свойства, которые могут нам пригодиться для настройки этих элементов:</p>
<ul>
<li><p><span class="b">playbackRate</span>: устанавливает скорость воспроизведения. По умолчанию равно 1</p></li>
<li><p><span class="b">src</span>: возвращает название воспроизводимого ресурса, если он установлен в коде html элемента</p></li>
<li><p><span class="b">duration</span>: возвращает длительность файла в секундах</p></li>
<li><p><span class="b">buffered</span>: возвращает длительность той части файла, которая уже буферизирована и готова к воспроизведению</p></li>
<li><p><span class="b">controls</span>: устанавливает или возвращает наличие атрибута <code>controls</code>. Если он установлен, возвращается <code>true</code>, иначе возвращает <code>false</code></p></li>
<li><p><span class="b">loop</span>: устанавливает или возвращает наличие атрибута <code>loop</code>. Если он установлен, возвращается <code>true</code>, иначе возвращает <code>false</code></p></li>
<li><p><span class="b">muted</span>: устанавливает или возвращает наличие атрибута <code>muted</code></p></li>
<li><p><span class="b">preload</span>: устанавливает или возвращает наличие атрибута <code>preload</code></p></li>
<li><p><span class="b">volume</span>: устанавливает или возвращает уровень звука от 0.0 до 1.0</p></li>
<li><p><span class="b">currentTime</span>: возвращает текущее время воспроизведения</p></li>
</ul>
<p>Отдельно для элемента <code>video</code> мы можем использовать ряд дополнительных свойств:</p>
<ul>
<li><p><span class="b">poster</span>: устанавливает или возвращает атрибут <code>poster</code></p></li>
<li><p><span class="b">height</span>: устанавливает или возвращает атрибут <code>height</code></p></li>
<li><p><span class="b">width</span>: устанавливает или возвращает атрибут <code>width</code></p></li>
<li><p><span class="b">videoWidth, videoHeight</span>: для элемента video возвращают ширину и высоту видео</p></li>
</ul>
<p>Следует также отметить два метода, с помощью которых мы можем управлять воспроизведением:</p>
<ul>
<li><p><span class="b">play()</span>: начинает воспроизведение</p></li>
<li><p><span class="b">pause()</span>: приостанавливает воспроизведение</p></li>
</ul>
<p>Основные события элементов video и audio:</p>
<ul>
<li><p><span class="b">canplaythrough</span>: это событие срабатывает после загрузки страницы, если браузер определит, что он может воспроизводить это видео/аудио</p></li>
<li><p><span class="b">pause</span>: событие срабатывает, когда воспроизведение мультимедиа приостанавливается, и оно переводится в состояние "paused"</p></li>
<li><p><span class="b">play</span>: событие срабатывает, когда начинается воспроизведение файла</p></li>
<li><p><span class="b">volumechange</span>: срабатывает при изменении уровня звука мультимедиа</p></li>
<li><p><span class="b">ended</span>: срабатывает при окончании воспроизведения</p></li>
<li><p><span class="b">timeupdate</span>: срабатывает при изменении времени воспроизведения</p></li>
<li><p><span class="b">error</span>: генерируется при возникновении ошибки</p></li>
<li><p><span class="b">loadeddata</span>: срабатывает, когда будет загружен первый фрейм видеофайла</p></li>
<li><p><span class="b">loadedmetadata</span>: срабатывает после загрузки метаданных мультимедиа (длительность воспроизведения, размеры видео и т.д.)</p></li>
<li><p><span class="b">seeking</span>: срабатывает, когда пользователь начинает перемещать курсор по шкале воспроизведения для перемещения к новому месту аудио- или видеофайла</p></li>
<li><p><span class="b">seeked</span>: срабатывает, когда пользователь завершил перемещение к новому месту на шкале воспроизведения</p></li>
</ul>
<p>Теперь используем некоторые из этих свойств, событий и методов для управления элементом video:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
	&lt;head&gt;
...
<p>Вначале в коде JavaScript мы получаем все элементы. Затем, если браузер поддерживает видео и может его воспроизвести, то обрабатываем событие 
<code>canplaythrough</code>, устанавливая уровень звука и удаляя класс hidden:</p>
<pre class="brush:js;">
videoEl.addEventListener('canplaythrough', function () {
	vidControls.classList.remove('hidden');
	videoEl.volume = volumeControl.value;
...
<p>Чтобы запустить воспроизведение, нам надо обработать нажатие ссылки Play:</p>
<pre class="brush:js;">
playBtn.addEventListener('click', function () {
...
<p>Обрабатывая события запуска и остановки воспроизведения, мы можем изменять надпись на ссылке:</p>
<pre class="brush:js;">
videoEl.addEventListener('play', function () {
...
<p>Обрабатывая событие <code>input</code>, которое возникает при изменении значения ползунка, мы можем синхронизировать изменение ползунка и громкость 
видео:</p>
<pre class="brush:js;">
volumeControl.addEventListener('input', function () {
...
<p>Обработка события <code>ended</code> позволит сбросить время воспроизведения:</p>
<pre class="brush:js;">
videoEl.addEventListener('ended', function () {
...
<p>А обработчик события <code>timeupdate</code> позволит динамически изменять показатель времени воспроизведения:</p>
<pre class="brush:js;">
videoEl.addEventListener('timeupdate', function () {
	timePicker.innerHTML = secondsToTime(videoEl.currentTime);
}, false);
</pre>
<p>Для форматирования строки времени применяется вспомогательная функция <code>secondsToTime</code>. В итоге мы получим следующие элементы 
воспроизведения:</p>
<img src="pics/5.4.png" alt="Работа с видео в HTML5 и JavaScript" />
<p>Ну и подобным образом можно добавить другие элементы, например, шкалу воспроизведения, какие-то другие кнопки.</p>
...
<div class="nav"><p><a href="7.2.php">Назад</a><a href="./">Содержание</a><a href="10.1.php">Вперед</a></p></div></div>
<div class="socBlock">
