...
<html >
<head>
<title>CSS3 | Псевдоклассы :is() и :where()</title>
<meta charset="utf-8" />
<meta name="description" content="Псевдоклассы :is() и :where() в CSS 3, их отличие">
...
<h2>Псевдоклассы :is() и :where()</h2><div class="date">Последнее обновление: 06.07.2021</div>
...
<h3>Псевдокласс :is()</h3>
<p>Псевдокласс <span class="b">:is()</span> применяется для сокращения длинных селекторов. Функция псевдокласса <span class="b">:is()</span> 
принимает список селекторов для выбора элементов html.</p>
<p>Например, мы хотим применить стиль к параграфам, которые находятся в первом уровне вложенности в элементах <code>header</code>, <code>main</code>, 
<code>footer</code>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<p>В данном случае для параграфов устанавливается шрифт Verdana высотой 18px:</p>
<img src="./pics/is_1.png" alt="Псевдоклассы :is и :where в CSS" />
<p>Без использования псевдокласса <span class="b">:is</span> нам пришлось бы написать:</p>
<pre class="brush:css;">
header &gt; p, main &gt; p, footer &gt; p {font-size: 18px; font-family: Verdana;}
</pre>
<p>Псевдокласс <span class="b">:is</span> позволяет значительно сократить эту запись:</p>
<pre class="brush:css;">
:is(header, main, footer) &gt; p {font-size: 18px; font-family: Verdana;}
</pre>
<p>То есть в данном случае выражение <code>:is(header, main, footer)</code> выбирает все элементы header, main и footer и объединяет их с последуюзими селекторами.</p>
<p>Псевдокласс <code>:is</code> может быть вложенным, например, возьмем пример выше, только выберем теперь только те параграфы, которые находятся в элементах div:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/is_2.png" alt="Вложенный псевдокласс :is в CSS" />
<h3>Псевдокласс :where</h3>
<p>Псевдокласс <span class="b">:where()</span> работает подобно <code>:is()</code>, он также принимает набор селекторов и выбирает все соответстующие селекторы. Например, первый пример, только 
вместо <code>:is()</code> теперь используем <span class="b">:where</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>И мы получим тот же результат, что и для <code>:is()</code>:</p>
<img src="./pics/is_1.png" alt="Псевдоклассы :is и :where в CSS" />
<h3>Различие между :is() и :where()</h3>
<p>В чем же разница между <span class="b">:is()</span> и <span class="b">:where()</span>? Псевдокласс <span class="b">:is()</span> применяет каскадность стилей 
(selector specificity), которая определяется по селектору с самым большим рангом. А для стилей псевдокласса <span class="b">:where()</span> ранг селекторов всегда равен 0.</p> 
<p>Рассмотрим на схожем примере. Применение <code>:is()</code>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь ранг селекторов в выражении <code>:is(h2, #content)</code> будет вычисляться по наибольшему селектору - в данном случае идентификатору <code>#content</code>. Таким образом, ранг селекторов в 
псевдоклассе <code>:is()</code> будет составлять 100 баллов (селекторы тегов оцениваются в 1 балл).</p>
<p>Дальше идет селектор <code>div &gt; h2</code>, который переопределяет цвет фона. Но послкольку его ранг меньше, чем ранг <code>div &gt; :is(h2, #content)</code>, 
то этот стиль применяться не будет.</p>
<p>Третий селектор <code>div &gt; #content</code> имеет тот же ранг, что и <code>div &gt; :is(h2, #content)</code>, поэтому его стиль будет работать.</p>
<p>В итоге мы получим, что для элемента <code>&lt;h2&gt;</code> переопределение цвета фона не будет работать.</p>
<img src="./pics/is_3.png" alt="Каскадность стилей и псевдокласс :is в CSS" />

<p>Теперь изменим на <code>:where()</code>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Ранг селектора <code>:where(h2, #content)</code> будет равен 0, поэтому стиль, задаваемый селектором <code>div &gt; :where(h2, #content)</code> будет 
переопределен последующими стилями.</p>
<img src="./pics/is_4.png" alt="Каскадность стилей и псевдокласс :where в CSS" />
