...
<html >
<head>
<title>HTML5 | Трансформации на Canvas</title>
<meta charset="utf-8" />
<meta name="description" content="Трансформации на Canvas в HTML5, перемещение, вращение, масштабирование фигур, применение матрицы преобразований">
...
<h2>Трансформации</h2><div class="date">Последнее обновление: 31.10.2016</div>
...
<h3>Перемещение</h3>
<p>Перемещение осуществляется с помощью метода <span class="b">translate()</span>:</p>
<pre class="brush:js;">translate(x, y)</pre>
<p>Первый параметр указывает на смещение по оси X, а второй параметр - по оси Y.</p>
<pre class="brush:js;">
...
<img src="pics/6.36.png" alt="Перемещение на canvas в HTML5" />
<h3>Вращение</h3>
<p>Для поворота фигур на canvase применяется метод <span class="b">rotate()</span>:</p>
<pre class="brush:js;">rotate(angle)</pre>
<p>В этот метод в качестве параметра передается угол поворота в радианах относительно точки с координатами (0, 0).</p>
<pre class="brush:js;">
var canvas = document.getElementById("myCanvas"), 
    context = canvas.getContext("2d");
...
<img src="pics/6.38.png" alt="Поворот на canvas в HTML5" />
<h3>Масштабирование</h3>
<p>Для масштабирования фигур применяется метод <span class="b">scale()</span>:</p>
<pre class="brush:js;">scale(xScale, yScale)</pre>
<p>Параметр <code>xScale</code> указывает на масштабирование по оси X, а <code>yScale</code> - по оси Y.</p>
<pre class="brush:js;">
var canvas = document.getElementById("myCanvas"), 
    context = canvas.getContext("2d");
...
<img src="pics/6.39.png" alt="Масштабирование на canvas в HTML5" />
<h3>Матрица преобразований</h3>
...
<p>Но контекст элемента canvas также предоставляет метод <span class="b">transform()</span>, который позволяет задать матрицу преобразования:</p>
<pre class="brush:js;">transform(a, b, c, d, e, f)</pre>
<p>Все параметры этого метода последовательно представляют элементы матрицы преобразования:</p>
<ul>
 <li><p><code>a</code>: масштабирование по оси X</p></li>
 <li><p><code>b</code>: поворот вокруг оси X</p></li>
 <li><p><code>c</code>: поворот вокруг оси Y</p></li>
 <li><p><code>d</code>: масштабирование по оси Y</p></li>
 <li><p><code>e</code>: горизонтальное смещение</p></li>
 <li><p><code>f</code>: вертикальное смещение</p></li>
</ul>
<p>Например:</p>
<pre class="brush:js;">
var canvas = document.getElementById("myCanvas"), 
...
<img src="pics/6.40.png" alt="Матрица преобразований на canvas в HTML5" />
<h3>Замена трансформации</h3>
<p>При последовательном применении разных трансформаций они просто последовательно применяются к фигурам. Однако может возникнуть ситуация, когда надо применить трансформацию 
не вместе со другими, а вместо других, то есть заменить трансформацию. Для этого применяется метод <span class="b">setTransform()</span>:</p>
<pre class="brush:js;">setTransform(a, b, c, d, e, f)</pre>
<p>Его параметры представляют матрицу преобразования, и в целом его применение аналогино применению метода <code>transform()</code>. Например:</p>
<pre class="brush:js;">
var canvas = document.getElementById("myCanvas"), 
    context = canvas.getContext("2d");
var k = 0;
...
<img src="pics/6.43.png" alt="setTransform in HTML5" />
<h3>Сброс трансформаций</h3>
<p>При применении трансформаций вся последующая отрисовка фигур подвергается данным трансформациям. Но возможна ситуация, когда после одиночного применения трансформации 
нам больше не нужно ее применение. И для всей последующей отрисовки мы можем сбросить трансформации с помощью метода <span class="b"></span>:</p>
<pre class="brush:js;">
var canvas = document.getElementById("myCanvas"), 
    context = canvas.getContext("2d");
...
<img src="pics/6.41.png" alt="Сброс трансформаций на canvas в HTML5" />
