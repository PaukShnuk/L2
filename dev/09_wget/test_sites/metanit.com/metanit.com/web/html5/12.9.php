...
<html >
<head>
<title>HTML5 | Многоколоночный дизайн на Flexbox</title>
<meta charset="utf-8" />
<meta name="description" content="Многоколоночный дизайн на Flexbox в HTML5 и CSS3, создание макета с двумя и тремя колонками">
...
<h2>Многоколоночный дизайн на Flexbox</h2><div class="date">Последнее обновление: 19.04.2017</div>
...
<p>Теперь рассмотрим, как мы можем сделать простейшие многоколонночные макеты страницы с помощью Flexbox.</p>
<h3>Двуколоночный дизайн</h3>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Здесь flex-контейнером является элемент body. Так как на мобильных устройствах (особенно смартфонах) размер экрана не такой большой, поэтому по умолчанию устанавливаем 
расположение элементов в столбик. Однако для устройств с экраном от 600рх и выше действует правило media-query, которое устанавливает расположение в виде строки.</p>
<img src="pics/flexbox22.png" alt="Двуколоночный режим в Flexbox и CSS3" />
<h3>Трехколоночный режим</h3>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>В отличие от предыдущего примера здесь добавлен еще один элемент. Особенностью этого примера является то, что столбцы имеют одинаковые размеры. Для этого у них установлено 
свойство <code>flex: 1</code>, то есть при растяжении или уменьшении границ контейнера все элементы будут масштабироваться на равную величину.</p>
<p>И кроме того, при ширине экрана больше 600px у второго элемента устанавливается свойство <code>order: -1</code>, благодаря чему этот элемент помещается первым:</p>
<img src="pics/flexbox23.png" alt="Трехколоночный режим с равными столбцами в Flexbox и CSS3" />
<p>Подобным образом мы можем добавить и большее количество столбцов. Но в данном случае по умолчанию столбцы имеют одинаковую ширину. Но что делать, 
если один из столбцов (как правило, центральный) должен иметь ширину больше, чем у остальных? Для этого добавим в стили страницы следующее правило:</p>
<pre class="brush:css;">
.item:first-child {
	flex: 0 0 50%;
}
</pre>
<p>В этом случае первый элемент всегда будет занимать 50% пространства контейнера:</p>
<img src="pics/flexbox24.png" alt="Трехколоночный дизайн в Flexbox и CSS3" />
