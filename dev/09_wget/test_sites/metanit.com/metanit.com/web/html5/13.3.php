...
<html >
<head>
<title>HTML5 | Функция repeat и свойство grid в Grid Layout</title>
<meta charset="utf-8" />
<meta name="description" content="Повторное создание строк и столбцов с помощью функции repeat в Grid Layout в CSS3, свойство grid">
...
<h2>Функция repeat и свойство grid</h2><div class="date">Последнее обновление: 09.05.2017</div>
...
<h3>Повторение строк и столбцов</h3>
<p>Если у нас столбцов и(или) строк много и они имеют одинаковые размеры, то есть смыл использовать специальную функцию <span class="b">repeat()</span>, которая 
позволит настроить строки и столбцы. Так, в примере выше повторяется определение одинаковых строк и столбцов в grid-контейнере:</p>
<pre class="brush:css;">
grid-template-columns: 8em 8em 8em;
grid-template-rows: 5em 5em 5em 5em;
</pre>
<p>Здесь мы видим, что происходит повторение одних и тех же размеров - 8em и 5em для установки ширины столбцов и высоты строк. Поэтому перепишем 
стили, применив функцию repeat:</p>
<pre class="brush:css;">
.grid-container {
...
<p>Первый параметр функции repeat представляет число повторений, а второй - определение строк или столбцов. Например, свойство 
<code>grid-template-columns: repeat(3, 8em);</code> говорит, что необходимо определить 3 столбца шириной в 8em.</p>
<p>Соответственно выражение <code>grid-template-rows: repeat(4, 5em)</code> определяет 4 строки высотой по 5em.</p>
<p>Можно задавать повторение нескольких столбцов и строк:</p>
<pre class="brush:css;">
.grid-container {
	border: solid 2px #000;
	display: grid;
...
<p>В данном случае будет создано 4 столбца: два раза будут повторяться два столбца с шириной 7em и 8em.</p>
<p>В случае со строками будет создано 4 строки. Причем первая будет иметь высоту в 6em, а остальные три - 5em.</p>
<h3>Свойство grid</h3>
<p>Свойство <span class="b">grid</span> объединяет свойства <code>grid-template-rows</code> и <code>grid-template-columns</code> и разом позволяет задать настройки 
для строк и столбцов в следующем формате:</p>
<pre class="brush:css;">grid: grid-template-rows / grid-template-columns;</pre>
<p>К примеру, у нас есть следующее определение класса grid-контейнера:</p>
<pre class="brush:css;">
.grid-container {
	border: solid 2px #000;
	display: grid;
...
<p>Мы можем сократить этот класс следующим образом:</p>
<pre class="brush:css;">
.grid-container {
	border: solid 2px #000;
	display: grid;
...
<p>Либо опять же используя функцию <code>repeat()</code>, можно еще больше сократить определение грида:</p>
<pre class="brush:css;">
.grid-container {
	border: solid 2px #000;
	display: grid;
