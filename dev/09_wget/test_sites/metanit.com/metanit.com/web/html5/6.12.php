...
<html >
<head>
<title>CSS3 | Границы</title>
<meta charset="utf-8" />
<meta name="description" content="Определение границ элементов в CSS 3, свойства border, border-width, border-style, border-color, округление границ с помощью border-radius">
...
<h2>Границы</h2><div class="date">Последнее обновление: 21.04.2016</div>
...
<p>Граница отделяется элемент от внешнего по отношению к нему содержимого. При этом граница является частью элемента.</p>
<p>Для настройки границы могут использоваться сразу несколько свойств:</p>
<ul>
<li><p><span class="b">border-width</span>: устанавливает ширину границы</p></li>
<li><p><span class="b">border-style</span>: задает стиль линии границы</p></li>
<li><p><span class="b">border-color</span>: устанавливает цвет границы</p></li>
</ul>
<p>Свойство <code>border-width</code> может принимать следующие типы значений:</p>
<ul>
<li><p>Значения в единицах измерения, таких как em, px или cm</p>
<pre class="brush:css;">
border-width: 2px;
</pre>
</li>
<li><p>Одно из константных значений: <code>thin</code> (тонкая граница - 1px), <code>medium</code> (средняя по ширине - 3px), 
<code>thick</code> (толстая - 5px)</p>
<pre class="brush:css;">
border-width: medium;
</pre></li>
</ul>
<p>Свойство <code>border-color</code> в качестве значения принимает цвет CSS:</p>
<pre class="brush:css;">
border-color: red;
</pre>
<p>Свойство <code>border-style</code> оформляет тип линии границы и может принимать одно из следующих значений:</p>
<ul>
<li><p><code>none</code>: граница отсутствует</p></li>
<li><p><code>solid</code>: граница в виде обычной линии</p></li>
<li><p><code>dashed</code>: штриховая линия</p></li>
<li><p><code>dotted</code>: линия в виде последовательности точек</p></li>
<li><p><code>double</code>: граница в виде двух параллельных линий</p></li>
<li><p><code>groove</code>: граница имеет трехмерный эффект</p></li>
<li><p><code>inset</code>: граница как бы вдавливается во внутрь</p></li>
<li><p><code>outset</code>: аналогично inset, только граница как бы выступает наружу</p></li>
<li><p><code>ridge</code>: граница также реализует трехмерный эффект</p></li>
</ul>
<img src="pics/4.43.png" alt="border-style в CSS 3" />
<p>Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<img src="pics/4.44.png" alt="Граница в CSS 3" />
<p>При необходимости мы можем определить цвет, стиль и ширину границы для каждой из сторон используя следующие свойства:</p>
<pre class="brush:css;">
/* для верхней границы */
border-top-width
border-top-style
...
<p>Вместо установки по отдельности цвета, стиля и ширины границы мы можем использовать одно свойство - <span class="b">border</span>:</p>
<pre class="brush:css;">
...
<p>Для установки границы для отдельных сторон можно использовать одно из свойств:</p>
<pre class="brush:css;">
border-top
border-bottom
border-left
...
<p>Их использование аналогично:</p>
<pre class="brush:css;">
border-top: 2px solid red;
</pre>
<h3>Радиус границы</h3>
<p>Свойство <span class="b">border-radius</span> позволяет округлить границу. Это свойство принимает значение радиуса в пикселях или единицах em.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Теперь каждый угол будет скругляться по радиусу в 30 пикселей:</p>
<img src="pics/4.45.png" alt="border-radius в CSS 3" />
<p>Так как у элемента может быть максимально четыре угла, то мы можем указать четыре значения для установки радиуса у каждого углов:</p>
<pre class="brush:css;">
border-radius: 15px 30px 5px 40px;
</pre>
<img src="pics/4.46.png" alt="скругление углов в CSS 3" />
<p>Вместо общей установки радиусов для всех углов, можно их устанавливать по отдельности. Так, предыдущее значение border-radius можно переписать 
следующим образом:</p>
<pre class="brush:css;">
...
<p>Также <code>border-radius</code> поддерживает возможность создания эллиптических углов. То есть угол не просто скругляется, а использует два радиуса, образуя в итоге душу эллипса:</p>
<pre class="brush:css;">
border-radius: 40px/20px;
</pre>
<p>В данном случае полагается, что радиус по оси X будет иметь значение 40 пикселей, а по оси Y - 20 пикселей.</p>
<img src="pics/4.47.png" alt="эллиптическое скругление углов в CSS 3" />
...
<div class="nav"><p><a href="6.11.php">Назад</a><a href="./">Содержание</a><a href="6.13.php">Вперед</a></p></div></div>
<div class="socBlock">
