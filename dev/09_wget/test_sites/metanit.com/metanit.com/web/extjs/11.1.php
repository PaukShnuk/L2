...
<html >
<head>
<title>Работа с формами в ExtJS 4</title>
<meta charset="utf-8" />
<meta name="description" content="Использование компонента Ext.form.Panel и создание форм в ExtJS на примере большой формы запроса">
...
<h1>Работа с формами в ExtJS 4</h1>
<h2>Создание форм</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>В предыдущих главах мы познакомились с различными элементами и компонентами и способами компоновки. И, как мы узнали, для организации 
множества компонентов в приложении используются контейнеры - панели, окна. В ExtJS для упорядочивания и работы с 
компонентами используются формы.</p>
<p>Формы представлены компонентом <span class="bb">Ext.form.Panel</span>, который по сути является той же панелькой-контейнером для других компонентов, 
только еще реализует дополнительный функционал форм. Создадим какую-нибудь простейшую форму, например, форму авторизации:</p>
<pre class="brush:xml;">
&lt;style type="text/css"&gt;
    .field-margin {
        margin-left: 10%;
...
<img src="pics/11.1.png" alt="Пример формы в ExtJS 4" />
<p>По сути та же самая панель с текстовыми полями и кнопками, только теперь эти поля составляют единство - форму, подобно формам html.</p>
<h3>Доступ к полям формы</h3>
<p>Все компоненты формы сосредоточены в свойстве <code>items</code>. Чтобы получить доступ к определенному полю формы, мы можем 
использовать метод <code>get</code>, в который передается индекс компонента в контейнере. А с помощью метода <code>getValue</code> у компонента 
мы можем получить введенное значение.</p>
<p>Например, получим значение первого текстового поля вышеопределенной формы:</p>
<pre class="brush:xml;">
&lt;style type="text/css"&gt;
    .field-margin {
        margin-left: 10%;
...
<p>Также мы можем получить не значения отдельных элементов, а всю форму в целом, использовав метод <code>getForm()</code>. Например, 
мы можем вывести значения всех полей формы следующим способом:</p>
<pre class="brush:js;">
Ext.onReady(function(){
    var formPanel=Ext.create('Ext.form.Panel',{
        title: 'Форма авторизации',
