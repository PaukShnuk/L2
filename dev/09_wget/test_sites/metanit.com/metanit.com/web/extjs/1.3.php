...
<html >
<head>
<title>Ext.application и Ext.onReady в Ext JS</title>
<meta charset="utf-8" />
<meta name="description" content="Обзор и использование функций Ext.application и Ext.onReady">
...
<h2>Ext.application и Ext.onReady</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>При создании первого приложения в первой главе мы использовали следующий код:</p>
<pre class="brush:js;">
Ext.application({
    name: 'HelloExt',
    launch: function() {
...
<p>Метод <code>Ext.application</code> у нас инициализирует приложение ExtJS, внутри которого и происходит дальнейшее создание приложение и указание его 
параметров. Данный подход используется при создании приложений по шаблону MVC.</p>
<p>Но вместо этого метода можно употреблять другой <code>Ext.onReady</code>, особенно когда мы не планируем создавать полнофункциональное приложение MVC.</p>
<p>Этот метод также срабатывает после загрузки страницы и подобен функции jQuery. Он инициализирует все элементы ExtJS. Мы можем переписать предыдущий пример с использованием <code>Ext.onReady</code>:</p>
<pre class="brush:js;">
...
<p>И в данном случае оба подхода взаимозаменяемы. Хотя в принципе они не идентичны. Далее мы увидим, в чем они отличаются.</p>
