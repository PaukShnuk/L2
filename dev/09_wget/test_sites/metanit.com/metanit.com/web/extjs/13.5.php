...
<html >
<head>
<title>Редактирование в гриде в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Редактирование в гриде Ext JS с помощью плагинов Ext.grid.plugin.RowEditing и Ext.grid.plugin.CellEditing, настройка редактирования">
...
<h2>Редактирование в гриде</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<h3>Редактирование по строкам</h3>
<p>Грид в Ext JS позволяет нам редактировать данные с помощью плагина <code>Ext.grid.plugin.RowEditing</code>. Теперь применим плагин к гриду:</p>
<pre class="brush:js;">
...
<p>В данном случае у нас в грид было добавлено два момента. Во-первых, определение плагина:</p>
<pre class="brush:js;">
plugins:[{
			ptype:'rowediting',
			clicksToEdit: 1
...
<p>Параметр plugins принимает набор плагинов, применяемых к гриду. В данном случае у нас определен плагин <code>Ext.grid.plugin.RowEditing</code>. 
Поэтому в качестве значения параметра <code>ptype</code> используется значение <code>'rowediting'</code>. 
А параметр <code>clicksToEdit</code> указывает количество кликов мышью, которое активирует плагин.</p>
<p>Второй момент состоит в добавлении редакторов для столбцов с помощью опции editor:</p>
<pre class="brush:js;">
	editor: {
				xtype: 'datefield',
				allowBlank: false
...
<p>Редакторы столбцов задают компонент и правила, которые будут использоваться для редактирования значения. Например, для столбца даты у 
нас определен компонент datefield - то есть календарь, а параметр <code>allowBlank</code> говорит о том, что поле обязательно должно иметь значение.</p>
<p>Подобные редакторы столбцов мы можем определить для всех столбцов или только для некоторых.</p>
<p>В итоге, когда мы захотим редактировать, у нас отобразятся рядом со строкой две кнопки для редактирования - Update и Cancel. 
Введя новые значения в поля и нажав Update, мы увидим, что значение обновилось, причем обновилось не только в самой таблице, но и в хранилище, 
откуда берутся данные для таблицы:</p>
<img src="pics/13.5.png" />
<h3>Редактирование ячеек</h3>
<p>Кроме редактирования строк в целом мы можем также изменять значения отдельных ячеек. Для этого нам надо воспользоваться плагином 
<code>Ext.grid.plugin.CellEditing</code>. По сравнению с предыдущим примером таблицы мы можем просто поменять определение плагина:</p>
<pre class="brush:js;">
Ext.create('Ext.grid.Panel', {
        title: 'Пользователи',
		plugins:[{
...
<div class="nav"><p><a href="13.4.php">Назад</a><a href="./">Содержание</a><a href="13.6.php">Вперед</a></p></div></div>
<div class="socBlock">
