...
<html >
<head>
<title>Псевдонимы классов в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и использование псевдонимов классов с помощью свойства alias, использование псевдонимов визуальных компонентов с xtype">
...
<h2>Псевдонимы классов</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Псевдоним (alias) представляет сокращенное имя класса, по которому проще обращаться к этому классу. Менеджер классов в Ext JS автоматически 
при обращении к псевдониму автоматически сопоставляет данный псевдоним с определенным классом. Особенно распространено употребление псевдонимов 
у классом графических компонентов.</p>
<p>Чтобы использовать псевдоним, нам надо объявить в классе свойство <span class="bb">alias</span>, которое и будет хранить имя псевдонима:</p>
<pre class="brush:js;">
...
<p>И теперь мы сможем создавать объекты как используя полное имя класса, так и его псевдоним и оба способа будут равнозначны:</p>
<pre class="brush:js;">
...
<p>Очень удобно применять псевдонимы при создании визуальных компонентов. Например, определим какой-нибудь свой простенький класс панели:</p>
<pre class="brush:js;">
...
<p>Здесь использовался псевдоним 'widget.personpanel', в котором префикс 'widget' указывает, что это будет визуальный компонент. И 
затем мы сможем на него ссылаться с помощью свойства <span class="bb">xtype</span>:</p>
<pre class="brush:js;">
Ext.application({
    name: 'HelloExt',
    launch: function() {
...
<p>Здесь в качестве дочернего элемента контейнера Viewport используется класс Person.Panel, который мы получаем через свойство <code>xtype</code>.</p>
...
<div class="nav"><p><a href="2.8.php">Назад</a><a href="./">Содержание</a><a href="3.php">Вперед</a></p></div></div>
<div class="socBlock">
