...
<html >
<head>
<title>Круговая диаграмма в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Создание круговых диаграмм в ExtJS, использование свойства angleField, связь с хранилищем, кастомизация диаграммы">
...
<h2>Круговая диаграмма</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Для построения круговой диаграммы в параметре series надо указать опцию <code>type: 'pie'</code>:</p>
<pre class="brush:js;">
...
<img src="pics/15.11.png" alt="Круговая диаграмма в ExtJS 4" />
<p>В отличие от предыдущих видов диаграмм здесь нам уже не нужны оси. Размер каждого участка диаграммы или сектора зависит от 
значений поля, указанного в свойстве <code>angleField</code>. Здесь у нас указано поле <code>data</code>, поэтому именно это поле 
будет передавать данные для сектора.</p>
<p>Свойство <code>highlight</code> указывает конфигурацию, которая будет применяться при наведении на сектор. В данном случае мы определяем 
отступы от остальной части диаграммы текущего сектора.</p>
<p>Далее свойство <code>label</code> устанавливает надпись на секторе. Для надписи используется значение поля name (<code>field: 'name'</code>).</p>
<p>Последнее свойство <code>tips</code> определяет всплывающую подсказку, появляющуюся при наведении на сектор. При наведении 
в функцию в параметре <code>renderer</code> передается в качестве аргумента <code>storeItem</code> объект, связанный с текущим сектором, 
а затем динамически устанавливается всплывающая подсказка.</p>
...
<div class="nav"><p><a href="15.6.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
