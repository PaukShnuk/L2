...
<html >
<head>
<title>Отправка данных формы на сервер в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка данных формы ExtJS на сервер с помощью метода formPanel.getForm().submit и обработка на стороне сервера полученных данных на примере скрипта php">
...
<h2>Отправка данных формы на сервер</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>В прошлом разделе мы посмотрели, как заполнять форму различными способами. Но это только одна часть работы с формами. 
Как правило, формы нужны, чтобы отправлять данные на сервер, где они потом обрабатываются. Итак, возьмем для примера самую распространенную форму - 
форму ввода логина и пароля и добавим к ней функционал отправки на сервер:</p>
<pre class="brush:js;">
...
<p>Итак, у нас простая форма авторизации. На форме определены два текстовых поля - для ввода логина и пароля.</p> 
<p>По нажатию на кнопку у нас срабатывает заложенный в ней обработчик. В этом обработчике реализован метод формы <code>submit</code>:</p>
<pre class="brush:js;">
	loginForm.getForm().submit({
					url: 'login.php',
					success: function(form, action){
...
<p>Этот метод принимает в качестве параметра <code>url </code> ресурс, на который будет отсылаться форма. Здесь определен файл php, однако это может быть и ресурс asp.net mvc или веб-форма asp.net и т.д.</p>
<p>Также в качестве параметров метода submit можно добавить необязательные параметры success и failure. Оба они принимают функцию обработки ответа сервера. 
Только в первом случае обрабатывается успешный ответ с сервера, а во втором случае неудачный. Каждый из обработчиков просто отображает окно сообщения, извещающее 
пользователя.</p>
<p>Теперь создадим простейший файл <i>login.php</i>, который и будет принимать и обрабатывать полученные данные. В реальной ситуации можно было бы задать 
обращение к базам данных, проверку введенных логина и пароля, но для простоты мы ограничимся простым сравнением пароля и отправкой клиенту определенного ответа:</p>
<pre class="brush:php;">
&lt;?php
...
<p>Используя объект $_POST в php мы получаем отправленные данные формы. Поскольку у нас поле пароля имеет параметр <code>name: 'pass'</code>, 
то чтобы получить переданное значение используется выражение <code>$_POST['pass'];</code>. Тоже самое и для логина. Затем мы сравниваем полученный пароль 
с некоторым значением, и в зависимости от результата посылаем обратно клиенту ответ в виде объекта json.</p>
<p>При получении объекта <code>{"success": true, "message": "Добро пожаловать,' . $login.'"}</code> поскольку он уведомляет об успешной обрботке данных формы, 
этот ответ будет обрабатываться функции, заданной в параметре <code>success</code>. Второй параметр, передаваемый в окно сообщения - action.result.message, 
будет включать свойство message из объекта json.</p>
<img src="pics/11.3.png" alt="Отправка формы в ExtJS 4" />
