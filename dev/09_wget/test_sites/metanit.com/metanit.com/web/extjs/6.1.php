...
<html >
<head>
<title>Компоновка в Ext JS 4</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в компоновку компонентов в приложении, применение контейнеров и из свойства layout для расположения компонентов">
...
<h1>Компоновка в Ext JS 4</h1><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Предыдущие две главы были по сути вводными. Так как все преимущество фреймворка ExtJS 4 состоит прежде всего в использовании визуальных компонентов. 
Поэтому начнем работать с визуальной частью фреймворка.</p>
<h2>Введение в контейнеры</h2>
<p>В ExtJS 4 компоненты не могу находиться сами со себе, они обязательно должны заключаться в какой-нибудь контейнер. 
Контейнер - это по сути тоже компонент, который может включать в себя дочерние компоненты и управлять их расположением. 
Наиболее распространенные контейнеры - <code>Panel</code>, <code>Viewport</code>, <code>Window</code>.</p>
<p>Все контейнеры являются наследниками класса <strong>Ext.Container</strong>. И если нам потребуется создать новый контейнер,  
то нам надо будет образовать его именно от этого класса.</p>
<p>Используем наиболее распространенный контейнер - компонент <b>Panel</b> (панель). 
Определим на html-странице простейшую панельку, которая содержит еще несколько панелей:</p>
<pre class="brush:xml;">
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае мы не указали явным образом компоновку внутренних для панели компонентов. Но мы можем это сделать.</p>
<p>Все контейнеры обладают свойством <span class="bb">layout</span>, которое и задает внутренне расположение элементов или компоновку.</p>
<p>А свойство <span class="bb">items</span> задает коллекцию дочерних элементов контейнера.</p>
<p>Теперь на примере компонента Panel посмотрим, какие варианты компоновки имеются в ExtJs 4.</p>
...
<div class="nav"><p><a href="5.php">Назад</a><a href="./">Содержание</a><a href="6.2.php">Вперед</a></p></div></div>
<div class="socBlock">
