...
<html >
<head>
<title>Ext.XTemplate в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Использование Ext.XTemplate для вставки кода javascript и разметки на веб-страницу, управление параметрами объекта, передаваемого в Ext.XTemplate">
...
<h1>Ext.XTemplate</h1><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Фреймворк ExtJS обладает таким мощным средством, как шаблоны. Шаблоны позволяют выполнять отдельные куски кода JavScript и встраивать разметку, 
и тем самым определять содержимое элементов и компонентов.</p>
<p>Шаблоны определяются с помощью класса <code>Ext.XTemplate</code>. Теперь посмотрим, как можно использовать шаблоны. Например, определим следующий объект, 
который в свою очередь представляет массив некоторых объектов javascript:</p>
<pre class="brush:js;">
var tiobeData = [
	{
		position: 1,
...
<p>Теперь мы можем встроить его в разметку веб-страницы, используя объект Ext.XTemplate:</p>
<pre class="brush:xml;">
...
<p>И шаблон сгенерирует нам следующее наполнение веб-страницы:</p>
<img src="pics/10.1.png" />
<p>В данном случае мы использовали метод <code>overwrite</code> для вставки разметки на страницу. Этот метод принимает два обязательных параметра: 
элемент страницы, который надо заполнить данными (в данном случае весь элемент body веб-страницы), и объект данных, из которого ведется наполнение.</p>
<p>Выражение или тег <code>&lt;tpl for="."&gt;</code> вводит шаблон в разметку, объявляя цикл for. Таким образом, данный шаблон сгенерирует идентичную разметку для  каждого объекта в переменной 
tiobeData. Используя выражения в фигурных скобках, например, <code>{position}</code> подставляется соответствующее свойство объектов в переменной tiobeData.</p>
<p>Ext.XTemplate поддерживает разные способы работы с массивами:</p>
<pre class="brush:js;">
&lt;tpl for="."&gt;...&lt;/tpl&gt;       // цикл по массиву элементов в корневом узле, как в предыдущем случае
&lt;tpl for="data"&gt;...&lt;/tpl&gt;     // цикл по массиву в узле data
&lt;tpl for="data.sub"&gt;...&lt;/tpl&gt; // цикл по массиву в узле data.sub
&lt;tpl for="." between=","&gt;...&lt;/tpl&gt; // цикл по массиву в корневом узле и вставка знака запятой между элементами из массива
</pre>
<p>Например, определим некоторый объект, который у нас будет хранить массив животных:</p>
<pre class="brush:js;">
var allData = {
	animals: [
	{name: 'слон',
...
<p>То мы могли бы использовать следующий шаблон:</p>
<pre class="brush:js;">
var tpl = new Ext.XTemplate('&lt;h1&gt;Животные&lt;/h1&gt;', '&lt;ul&gt;', 
												'&lt;tpl for="animals"&gt;',
												'&lt;li&gt;{name}&lt;/li&gt;',
												'&lt;/tpl&gt;',
											'&lt;/ul&gt;');
tpl.overwrite(Ext.getBody(), allData);
</pre>
<h3>Условные конструкции в шаблонах</h3>
<p>С помощью XTemplate мы можем не просто подставлять нужные значения объектов, но и использовать более сложную логику. 
Так, мы можем использовать условные конструкции if как и в языках программирования.</p> 
<p>Изменим код шаблона, который выводит данные о языках программирования. Например, мы изменим стиль тех четных строк - окрасим их в другой цвет, 
и зададим логику вывода дополнительного столбца, содержание которого будет различаться в зависимости от позиции языка программирования:</p>
<pre class="brush:xml;">
Ext.onReady(function(){
...
<p>Данный код даст нам следующее содержание:</p>
<img src="pics/10.2.png" />

<p>Чтобы получить текущий передаваемый объект в цикле, в Ext.XTemplate имеется свойство <code>values</code>. Через это свойство затем мы можем 
обратиться уже непосредственно к свойствам объекта. Так, используя выражение <code>values.position</code>, мы обращаемся к свойству position 
Встроенное в определение строки <code>&lt;tr&gt;</code> выражение tpl позволяет в зависимости от значения свойства <code>position</code> 
определить цвет данной строки.</p>
<p>Далее идущая конструкция позволяет в зависимости от значения свойства использовать конструкцию if...elseif..else для вывода нужных нам данных.</p>
<p>Кроме свойства values Ext.XTemplate имеет еще ряд полезных свойств:</p>
<ul>
<li><p>parent - указывает на родителя текущего объекта. То есть в данном случае при переборе в цикле всех объектов, это свойство будет иметь 
в качестве значения объект, внутри которого и объявлен массив перебираемых объектов.</p></li>
<li><p>xindex - указывает на индекс текущего перебираемого элемента в массиве</p></li>
<li><p>xcount - показывает общее количество перебираемых объектов</p></li>
</ul>
<h3>Встраивание в шаблон кода JavaScript</h3>
<p>Класс XTemplate позволяет нам использовать функции javascript, которые встраиваются в шаблон, для обработки данных. 
Так, вернемся к нашей модели с языками программирования. Допустим, мы хотим сделать так, что если входное значение равно "C", то к нему 
должна добавляться подстрока "(ANSI)":</p>
<pre class="brush:xml;">
	var tpl = new Ext.XTemplate('&lt;h1&gt;TIOBE Rate&lt;/h1&gt;',
											...
											'&lt;/table&gt;',
											{
                                                 getStringValue: function(values){
                                                            if(values=="C"){return "C (ANSI)";}
                                                            else {return values}
                                                        }
                                            });
		tpl.overwrite(Ext.getBody(), tiobeData);
</pre>
<p>Таким образом, мы можем объявить некоторую функцию форматирования значения и затем использовать эту функцию вместо свойства, а само свойство 
передавать в качестве параметра функции.</p>
...
<div class="nav"><p><a href="9.2.php">Назад</a><a href="./">Содержание</a><a href="11.1.php">Вперед</a></p></div></div>
<div class="socBlock">
