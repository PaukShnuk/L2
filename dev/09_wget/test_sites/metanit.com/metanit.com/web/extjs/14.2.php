...
<html >
<head>
<title>Создание моделей и хранилища в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Создание модели и хранилища в ExtJS применительно к шаблону MVC, определение свойств модели и хранилища, создание источника данных">
...
<h2>Создание моделей и хранилища</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Первым делом создадим модели и хранилище, которое будет содержать объекты данной модели.</p>
<p>Файл модели будет называться <i>Book.js</i> и мы его поместим в каталог <i>BookApp/app/model/</i>:</p>
<pre class="brush:xml;">
...
<p>Тут все просто. Поскольку в прошлом параграфе в файле app.js мы определили имя приложения BookApp: </p>
<pre class="brush:js;">
name: 'BookApp',
appFolder: 'app',
</pre>
<p>то название <i>BookApp</i> фактически будет сопоставляться с каталогом app. Например, у нас модель находится в папке <i>app/model</i>, 
то ее название будет начинаться с <i>BookApp.model</i>, то есть в данном случае: <code>BookApp.model.Book</code>. </p>
<p>Модель Book определяет четыре поля: <code>'id','name', 'author', 'year'</code>.</p>

<p>В каталог <i>BookApp/app/store</i> поместим файл хранилища, который будет называться <i>BookStore.js</i>:</p>
<pre class="brush:js;">
...
<p>Опять же обратите внимание, поскольку файл хранилища расположен в папке <i>app/store</i>, то класс хранилища будет называться <i>BookApp.store.BookStore</i>. </p>
<p>Хранилище автоматически реализует загрузку данных из файла books.json.</p>
<p>Итак, добавим в каталог <i>BookApp/app/data/</i> файл books.json со следующим содержанием:</p>
<pre class="brush:js;">
{
    "success": true,
    "books": [
...
<p>На этом по сути работа с уровнем модели и данных закончена. Теперь перейдем к представлению.</p>
