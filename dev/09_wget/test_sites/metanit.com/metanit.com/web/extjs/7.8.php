...
<html >
<head>
<title>Ext.Button в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и использование кнопок в приложении Ext JS с помощью класса Ext.Button, обработчики кнопок">
...
<h2>Ext.Button</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Класс Ext.Button представляет кнопку:</p>
<pre class="brush:js;">
...
<p>Для использования кнопок внутри контейнера мы можем использовать xtype button:</p>
<pre class="brush:js;">
Ext.onReady(function(){
...
<p>Используя в вышеприведенном примере свойство <code>enableToggle: true</code>, при нажатии на кнопку она оказывается во включенном состоянии и выделяется 
цветом, при повторном нажатии мы снимаем включение.</p>
<p>Используя свойство menu, мы можем задать для кнопки меню:</p>
<pre class="brush:js;">
Ext.onReady(function(){
	Ext.create('Ext.Panel', {
		width:300,
...
<img src="pics/extbutton.png" />
<p>В первом примере мы устанавливали у кнопки свойство <code>handler</code>, которое определяет функцию, отрабатываемую при нажатии кнопки. 
Но мы также можем использовать свойство <code>listeners</code> и установить в нем обработчики других событий или слушатели:</p>
<pre class="brush:js;">
Ext.onReady(function(){
	Ext.create('Ext.Panel', {
		width:300,
