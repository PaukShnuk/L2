...
<html >
<head>
<title>Загрузка файлов на сервер в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Загрузка файлов на сервер в формах ExtJS">
...
<h2>Загрузка файлов на сервер</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>ExtJS 4 обладает достаточным функционалом для загрузки файлов на сервер. Для этого нам, во-первых, потребуется форма для выбора файла на стороне 
клиента, и, во-вторых, некоторый код на стороне сервера, который бы принимал файл.</p>
<p>На стороне клиента для выбора файла мы можем воспользоваться компонентом <b>Ext.form.field.File</b>, который имеет <code>xtype:'filefield'</code>.</p>
<pre class="brush:js;">
...
<p>При отправке сначала мы проверяем форму на валидность (<code>form.isValid()</code>), и потом только отправляем файл.</p>
<p>Параметр <code>waitMsg</code> хранит текст, который будет отображаться при в индикаторе при загрузке файла на сервер.</p>
<p>Файл upload.php обрабатывает загруженный файл и сохраняет его по определенному пути:</p>
<pre class="brush:php;">
&lt;?php
...
<p>При загрузке файла ExtJS создает скрытый элемент <code>&lt;form&gt;</code>, который помещается во временный элемент <code>&lt;frame&gt;</code>. 
При этом заголовок Content-Type у формы устанавливается в <code>multipart/form</code>. После окончания загрузки и получения ответа с сервера временный фрейм 
удаляется.</p>
...
<div class="nav"><p><a href="11.4.php">Назад</a><a href="./">Содержание</a><a href="12.1.php">Вперед</a></p></div></div>
<div class="socBlock">
