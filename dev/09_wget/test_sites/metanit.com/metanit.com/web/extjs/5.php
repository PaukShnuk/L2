...
<html >
<head>
<title>Анимация в Ext JS</title>
<meta charset="utf-8" />
<meta name="description" content="Применение функций анимации puff, switchOff, slideIn, slideOut, pulse. Создание собственной анимации">
...
<h1>Анимация в Ext JS</h1><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Также как и библиотека jQuery, Ext JS представляет прекрасные возможности для создания  различных анимаций элементов и компонентов.</p>
<p>Итак, создадим на веб-странице простой элемент <code>&lt;div id="blackQv"&gt;&lt;/div&gt;</code>, в виде черного квадрата, для чего определим для него стиль:</p>
<pre class="brush:xml;">
&lt;style&gt;
#blackQv{
	margin:100px;
...
<p>Чтобы применить анимацию, нам надо использовать один из методов, которые имеются в наличии у класса Ext.Element. Все они имеют ряд общих параметров:</p>
<ul>
<li><p><code>useDisplay</code>. Принимает булевое значение true/false, которое показывает, будет ли элемент скрыт после завершения 
анимации. Например, если мы установим значение <code>false</code>, то элемент будет скрыт методом hide и станет невидимым. Но если мы установим <code>true</code>, 
тогда элемент не только будет скрыт, но и перестанет использовать пространство страницы.</p></li>
<li><p><code>duration</code>. Этот параметр принимает в качестве значения число, которое показывает в миллисекундах, сколько будет длиться анимация.</p></li>
<li><p><code>easing</code>. Принимает в качестве значения строку. Он указывает на эластичность анимации и принимает следующие значения: <code>backIn, 
easeIn,easeOut, bounceOut, elasticeIn</code></p></li>
</ul>
<p>Используем первую функцию анимации - <b>puff</b>. Эта функция в течение определенного времени растягивает элемент, при этом постепенно делая его прозрачным:</p>
<pre class="brush:js;">
...
<p>Другая функция анимации - <b>switchOff</b> - действует противоположным образом, постепенно уменьшая элемент, пока он полностью не исчезнет:</p>
<pre class="brush:js;">
element.switchOff({
		easing: 'easeOut',
		duration: 3000,
...
<p>Еще две функции анимации - <b>slideIn</b> и <b>slideOut</b> производят скольжение элемента в указанном направлении. Если slideIn постепенно выдвигает 
элемент на страницу как бы из-за границ , то slideOut, наоборот, убирает его со страницы:</p>
<pre class="brush:js;">
element.slideIn('t', {
	easing: 'easeIn',
	duration: 2000
...
<p>Первый параметр здесь представляет направление анимации:</p>
<ul>
<li>t - анимация начинается сверху</li>
<li>b - снизу</li>
<li>r - справа</li>
<li>l - слева</li>
</ul>
<p>Функция <strong>fadeOut</strong> постепенно обесцвечивает элемент, делая его прозрачным:</p>
<pre class="brush:js;">
element.fadeOut({
	opacity: 0,
	easing: 'easeOut',
...
<p>В данном случае параметр <code>opacity</code> задает прозрачность, который обретет в конце анимации элемент. Здесь используется число 0, 
...
К примеру, чтобы сделать полупрозрачный элемент, надо указать число 0.5</p>
<p>Еще одна функция анимации - <b>pulse</b> задает пульсацию элемента:</p>
<pre class="brush:js;">
element.frame("#0000b9", 5, {
	duration: 1000
});
</pre>
<p>Первый параметр задает цвет пульсирующей границы, а второй - сколько раз элемент будет пульсировать.</p>
<h3>Анимация компонентов Ext JS</h3>
<p>Анимация компонентов похожа на анимацию обычных элементов. Например, используем анимацию кнопки:</p>
<pre class="brush:js;">
...
<p>Чтобы анимировать компонент, нам надо получить доступ к его элементу через метод <code>getEl</code>. Затем идет уже вышерассмотренное применение 
функций анимации с теми же параметрами.</p>
<h3>Создание своей анимации</h3>
<p>С помощью класса <strong>Ext.fx.Animator</strong> можно создавать пользовательские анимации. Создадим, например, анимацию прыгающего шара. 
Для начала определим элемент <code>&lt;div id="jump"&gt;&lt;/div&gt;</code> и его стили:</p>
<pre class="brush:xml;">
&lt;style&gt;
#jump{
	y: 100px;
...
<p>Теперь определим код анимации нашего шара:</p>
<pre class="brush:js;">
...
<p>Объект Ext.fx.Animator принимает ряд параметров, где <code>target</code> указывает на ID анимируемого элемента, а  <code>duration</code> - на время 
анимации в миллисекундах.</p>
<p>Третий параметр - <code>keyframes</code> задает ключевые кадры анимации. Здесь мы определяем ряд переходных состояний, в которых будет находится шар. 
Разберем первый ключевой кадр:</p>
<pre class="brush:js;">
0: {
		y: 100
	},
</pre>
<p>Число 0 указывает, что после 0 % времени анимации, определенной в параметре duration, элемент будет принимать характеристики, заключенные в 
анонимном объекте, то есть его координата у=100 пикселям.</p>
<p>На третьем кадре, который выполняется после 40% времени анимации, меняем значение координаты x, y и цвет элемента</p>
<pre class="brush:js;">
40: {
		y: 155,
		x:110,
...
<p>Также мы можем задать дополнительные параметры анимации:</p>
<ul>
<li><p><span class="bb">easing</span> - рассмотренный выше параметр, задающий эластичность анимации</p></li>
<li><p><span class="bb">iterations</span> - задает количество циклов анимации</p></li>
<li><p>события <span class="bb">beforeanimate/afteranimate</span> - представляют функции обработчиков, которые будут срабатывать перед началом анимации 
и после ее завершения</p></li>
<li><p><span class="bb">keyframe</span> - задает обработчик события keyframe, которое возникает между двумя ключевыми кадрами</p></li>
</ul>
...
<div class="nav"><p><a href="4.php">Назад</a><a href="./">Содержание</a><a href="6.1.php">Вперед</a></p></div></div>
<div class="socBlock">
