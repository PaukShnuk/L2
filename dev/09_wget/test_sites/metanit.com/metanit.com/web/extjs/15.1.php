...
<html >
<head>
<title>Рисование в ExtJS и диаграммы</title>
<meta charset="utf-8" />
<meta name="description" content="Рисование в ExtJS, использование спрайтов и их типы, класс Ext.draw.Sprite, отрисовка путей, текста, изображений на элементах">
...
<h1>Рисование в ExtJS и диаграммы</h1>
<h2>Введение в рисование. Спрайты</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Ext JS4 обладает довольно мощным функционалом для рисования, создания диаграмм и графиков. 
В основе создания графики в ExtJS лежит класс <b>Ext.draw.Component</b>. Он принимаем набор спрайтов, которые отрисовываются на странице. 
Для отрисовки спрайтов <code>Ext.draw.Component</code> использует класс <b>Ext.draw.Surface</b>, который представляет собой по сути поверхность рисования. 
В качестве способа рисования используется графика SVG (для браузеров, не поддерживающих SVG - IE 6-8, используется VML - SVG-подобный язык).</p>
<p>Каждый отдельный спрайт реализуется классом <code>Ext.draw.Sprite</code>. Мы можем использовать следующие типы спрайтов:</p>
<ul>
<li><p><code>circle</code>: рисует круг. С помощью параметра <code>radius</code> определяем радиус круга</p></li>
<li><p><code>rect</code>: рисует прямоугольник, у которого параметры <code>width</code> и <code>height</code> определяют соответственно ширину и высоту прямоугольника</p></li>
<li><p><code>text</code>: выводит текст</p></li>
<li><p><code>path</code>: отображает графический путь</p></li>
<li><p><code>image</code>: отображает изображение по определенному пути</p></li>
<li><p><code>ellipse</code>: рисует эллипс. В отличие от круга здесь задаются два радиуса: <code>radiusX</code> и <code>radiusY</code>, 
которые и определяют форму эллипса</p></li>
</ul>
<p>В качестве примера используем все спрайты вместе:</p>
<pre class="brush:js;">
...
<p>Свойство <code>viewBox</code> у класса <code>Ext.draw.Component</code> указывает, будут ли спрайты масштабироваться таким образом, чтобы 
подходить под пространство окружающего контейнера. Значение <code>false</code> указывает на то, что спрайты будут отрисовываться как есть, 
без всякого масштабирования.</p>
<p>Все спрайты принимают различный набор параметров, которые мы можем использовать:</p>
<ul>
<li><p><code>type</code>: тип спрайта. Может принимать значения: circle, rect, text, image, ellipse, path</p></li>
<li><p><code>width</code>: числовое значение ширины спрайта. Применяется для прямоугольных спрайтов, то есть, для rect, text, image</p></li>
<li><p><code>height</code>: числовое значение высоты спрайта. Применяется для прямоугольных спрайтов</p></li>
<li><p><code>radius</code>: радиус спрайта circle. В спрайте rect создает закругленные углы по заданному радиусу</p></li>
<li><p><code>radiusX</code>: радиус эллипса вдоль оси X</p></li>
<li><p><code>radiusY</code>: радиус эллипса вдоль оси Y</p></li>
<li><p><code>x</code>: значение Х-координаты спрайты</p></li>
<li><p><code>y</code>: значение У-координаты спрайта</p></li>
<li><p><code>opacity</code>: прозрачность спрайта, принимает значения с плавающей точкой от 0.0 до 1.0</p></li>
<li><p><code>fill</code>: цвет заливки спрайта. Может принимать как конкретное название цвета (например, 'blue'), так и его числовую константу 
(например, '#eee')</p></li>
<li><p><code>stroke</code>: цвет обводки границы спрайта</p></li>
<li><p><code>stroke-width</code>: толщина обводки</p></li>
<li><p><code>font</code>: шрифт CSS, используемый для спрайта text</p></li>
<li><p><code>text</code>: строка, которую выводит спрайт text</p></li>
<li><p><code>path</code>: набор точек, по которым спрайт path строит геометрический путь</p></li>
</ul>
<h3>Рисование путей</h3>
<p>Геометрические пути представляют более сложные и мощные средства для рисования, благодаря которым можно не ограничиваться геометрическими 
примитивами, а создать более сложные рисунки. Для рисования путей нам надо указать в качестве типа спрайта <code>path</code> и задать 
набор простейших примитивов, из которых будет строиться путь:</p>
<pre class="brush:js;">
Ext.onReady(function(){ 
...
<p>Параметр <code>path:'M 50 100 L 100 50 L 150 100'</code> указывает набор примитивов. Заглавная буква M указывает точку, с которой будет начинаться путь. 
То есть в данном случае координаты начальной точки будут равны x=50, y=100.</p>
<p>Далее следующая буква указывает, какой мы примитив будем использовать. В данном случае указана буква L, а это значит что от точки M  
чертится линия до точки с координатами x=100 и y=50. И дальше опять линия.</p>
<p>Надо заметить, что выражение <code>path:'M 50 100 L 100 50 L 150 100'</code> не будет аналогично выражению <code>path:'M 50 100 l 100 50 l 150 100'</code>. 
...
Малая же буква l указывает расстояние, на которое надо провести линию от текущей точки. То есть выражение <code>l 100 50</code> указывает, что надо провести линию от текущей точки 
на 100 единиц вправо по оси X и на 50 единиц вниз по оси Y.</p>
<h3>Динамическая установка атрибутов</h3>
<p>Используя метод <code>setAttributes</code>, мы можем динамически устанавливать значения параметров спрайта с перерисовкой:</p>
<pre class="brush:js;">
Ext.onReady(function(){ 
	var sprite = Ext.create('Ext.draw.Sprite', {
		type: 'rect',
...
<p>Метод <code>setAttributes</code> принимает набор параметров и их значений. Булевое значение <code>true</code> указывает системе, 
что сразу после установки значения надо перерисовать спрайт.</p>
...
<div class="nav"><p><a href="14.4.php">Назад</a><a href="./">Содержание</a><a href="15.2.php">Вперед</a></p></div></div>
<div class="socBlock">
