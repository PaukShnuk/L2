...
<html >
<head>
<title>Постраничный вывод в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Постраничный вывод данных в гриде Ext JS с помощью плагина Ext.toolbar.Paging и его настройка через свойство pageSize">
...
<h2>Постраничный вывод</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>В предыдущих параграфах мы выводили все данные в таблицу. Однако более детальная настройка связки хранилище+грид позволяет нам 
также настроить и вывод данных в таблицу. Так, мы можем задать ограничение на количество выводимых данных и как бы пролистывать их в гриде, 
то есть сделать постраничный вывод (paging).</p>
<p>Для этого нам, во-первых, надо настроить хранилище:</p>
<pre class="brush:js;">
...
<p>По сравнению с примерами хранилища в предыдущих параграфах здесь было использовано свойство <code>pageSize</code>. Оно показывает, 
сколько объектов будет получать за раз хранилище (то есть в данном случае 4 объекта).</p>
<p>Теперь настроим грид. Здесь нам надо применить плагин <code>Ext.toolbar.Paging</code>:</p>
<pre class="brush:js;">
...
<p>Само объявление тулбара находится в следующих строках:</p>
<pre class="brush:js;">
	dockedItems: [{
			xtype: 'pagingtoolbar',
			store:store,
...
<p>С помощью параметра <code>store</code> мы привязываем тулбар к хранилищу грида. Параметр dock задает положение тулбара на странице, а остальные 
параметры отвечают за вывод информации плагина.</p>
<p>Теперь перейдем к ресурсу на сервере, который будет посылать хранилищу данные в формате json. В нашем случае это файл <i>users.php</i>.</p>
<p>При применении плагина приложение будет отправлять запросы к users.php в виде GET-запроса типа <i>http://localhost/helloext/users.php?_dc=1372956099996&page=2&start=4&limit=4</i>, 
в котором приложение передает ряд параметров серверу для выборки данных.</p>
<p>Это параметры:</p>
<ul>
<li><p><code>page</code>: номер страницы для отображения в гриде</p></li>
<li><p><code>start</code>: номер объекта, с которого надо получить данные</p></li>
<li><p><code>limit</code>: размер данных, собственно, который и объявлен у нас в хранилище в параметре pageSize.</p></li>
</ul>
<p>Мы можем получить эти параметры в users.php и в соответствии с ними отправлять определенные данные в ответ приложению в виде объекта json:</p>
<pre class="brush:xml;">
&lt;?php
...
<p>В реальном приложении мы могли осуществлять запрос к бд, например, mysql, на получение limit данных, начиная с номера start.</p>


<p>В итоге, получив данные, мы можем использовать навигацию по страницам в гриде:</p>
