...
<html >
<head>
<title>Создание представлений в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Создание представлений в ExtJS, определение двух представлений, их свойства, применение стилей к представлениям">
...
<h2>Создание представлений</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>За визуальную часть или интерфейс пользователя будут отвечать представления. Представления у нас будут находиться в папке <i>BookApp/app/view</i>. 
Общая структура визуальной части нашего приложения будет следующей: у нас будет основная часть со списком книг и специальное окно, где можно изменять или создавать 
книгу.</p>
<p>Добавим в каталог <i>BookApp/app/view</i> файл <i>BookList.js</i>, который у нас будет отвечать за вывод таблицы книг, со следующим содержимым:</p>
<pre class="brush:js;">
...
<p>В данном случае мы создаем класс-компонент BookList, который будет унаследован от Ext.grid.Panel, то есть будет представлять собой таблицу. 
В качестве псевдонима определяем ему имя booklist (<code>alias: 'widget.booklist',</code>).</p>
<p>Определяем для параметра <code>initComponent</code> функцию, которая просто добавляет столбцы в грид. И привязываем его к созданному на прошлом 
этапе хранилищу: <code>store: 'BookStore'</code>.</p>
<p>Теперь определим класс окна, в котором будет происходить добавление/изменение/удаление книг. Этот класс у нас будет называться 
<i>Book</i>. Он также будет находиться в папке <i>BookApp/app/view</i> и будет иметь следующее содержание:</p>
<pre class="brush:js;">
...
<p>Во-первых, поскольку это окно, то оно наследуется от класса <code>Ext.window.Window</code>, а в качестве псевдонима, по которому 
к данному классу будем обращаться в приложении, мы указываем <code>bookwindow</code>.</p>
<p>Во-вторых, для параметра <code>initComponent</code> определяем функцию, которая инициализирует компоненты окна: основную форму с тремя полями 
для ввода, тулбар с тремя кнопками и одну кнопку внизу. В итоге визуально это будет выглядеть примерно так:</p>
<img src="pics/14.3.png" />
<p>Еще один момент, который бы я хотел отметить - применение стилей. В данном случае для создания иконок рядом с кнопками в тулбаре я применяю 
стили, например, <code>iconCls:'new-icon'</code>. Здесь используется класс new-icon. Для определения данного класса добавим в ранее созданную 
папку BookApp/styles три изображения, которые будут использоваться в качестве иконок на кнопках, и файл стилей (назовем его, например, <i>style.css</i>) примерно такого содержания:</p>
<pre class="brush:xml;">
...
<p>Ну и затем нам надо подключить эти стили к приложению в файле index.html:</p>
<pre class="brush:xml;">
&lt;html&gt;
&lt;head&gt;
&lt;title&gt;BookApp&lt;/title&gt;
...
<p>Теперь перейдем к ключевому звену приложения - к контроллеру.</p>
