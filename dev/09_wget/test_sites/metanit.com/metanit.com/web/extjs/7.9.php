...
<html >
<head>
<title>Флажки и переключатели в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Использование класса Ext.form.RadioGroup для создания радиокнопок или переключателей и создание флажков (чекбоксов) с помощью Ext.form.CheckboxGroup">
...
<h2>Флажки и переключатели</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Переключатели представляют собой группу флажков, из которых мы можем выбрать только один. Для формирования набора переключателей или радиокнопок, 
нам надо использовать класс Ext.form.RadioGroup, который имеет xtype=radiogroup. Объект этого класса принимает массив радиокнопок:</p>
<pre class="brush:js;">
...
<img src="pics/radiogroup.png" />
<p>Поле fieldLabel указывает на заголовок группы переключателей. Для отображения некоторого текста каждый переключатель имеет свойство <code>boxLabel</code>. 
Другое полезное свойство <code>inputValue</code> указывает на значение радиокнопки.</p>
<p>По умолчанию все радиокнопки размещаются в одну строку по одной в каждом столбце. Но мы можем сделать вертикальный набор столбцов, чтобы каждая 
кнопка начиналась с новой строки. Для этого нам надо установить у объекта RadioGroup <code>vertical: true</code> и указать нужное количество столбцов:</p>
<pre class="brush:js;">
Ext.onReady(function(){
	var radioGroupAutoLayout = Ext.create('Ext.form.Panel', {
		title: 'Переключатели',
...
<img src="pics/verticalradios.png" alt="вертикальная группа переключателей" />
<p>Установив <code>columns: 2</code>, мы сделали разбиение группы радиокнопок на два столбца.</p>
<p>В отличие от переключателей, в группе флажков или чекбоксов можно выбрать одновременно несколько элементов. В целом использование флажков похоже на использование 
радиокнопок: здесь мы также можем задать вертикальное или горизонтальное расположение элементов, разбить их на несколько столбцов:</p>
<pre class="brush:js;">
Ext.onReady(function(){
...
<img src="pics/extcheckbox.png" alt="Использование чекбоксов в ExtJs 4" />
