...
<html >
<head>
<title>Взаимодействие со спрайтами в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие со спрайтами в ExtJS, перемещение, вращение и масштабирование спрайтов, обработка событий и реакция спрайтов на собыия мыши">
...
<h2>Взаимодействие со спрайтами</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Применяя специальные параметры, мы можем проводить со спрайтами некоторые преобразования. Ext JS4 позволяет провести следующие преобразования: 
перемещение, вращение и масштабирование спрайтов.</p>
<h3>Вращение</h3>
<p>Для вращения используется параметр <code>rotation</code>, для которого указывается количество градусов, на которые поворачивается спрайт:</p>
<pre class="brush:js;">
...
<p>В данном случае вращение производится относительно центральной точки спрайта. Но мы можем также производить вращение относительно начала координат 
(да и вообще любой точки координатной плоскости):</p>
<pre class="brush:js;">
		{
			type: 'rect',
			width: 50,
...
<p>Параметры x и y указывают на координаты точки, относительно которой идет вращение.</p>
<h3>Перемещение</h3>
<p>Для перемещения мы указываем параметр <code>translate</code>, который принимает две опции: x и y, обозначающие на сколько перемещается спрайт 
вдоль соответствующих осей:</p>
<pre class="brush:js;">
		{
			type: 'rect',
			width: 50,
...
<p>Масштабирование задает параметр <code>scale</code>:</p>
<pre class="brush:js;">
	var drawComponent = Ext.create('Ext.draw.Component', {
		viewBox: false,
		items: [{
...
<p>Опции x и y в параметре <code>scale</code> указывают, во сколько раз надо масштабировать (увеличить) спрайт вдоль соответствующих осей:</p>
<img src="pics/15.4.png" />
<h3>Обработка событий спрайтов</h3>
<p>У каждого спрайта мы можем определить обработчики событий (например, нажатия) и тем самым внести в приложение интерактивность, сделав спрайты 
"отзывчивыми" на действия пользователя:</p>
<pre class="brush:js;">
...
<p>С помощью параметра <code>listeners</code> можно установить обработчики событий спрайтов. В данном случае мы обрабатываем события 
наведения мыши (<code>mouseover</code>) и увода указателя мыши за пределы спрайта (<code>mouseout</code>). В итоге у нас получается небольшой анимационный эффект.</p>
