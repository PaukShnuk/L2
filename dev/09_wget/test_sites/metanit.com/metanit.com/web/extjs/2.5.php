...
<html >
<head>
<title>Наследование в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Применение наследования с помощью параметра extend, переопределение методов базового класса и использование метода this.callParent">
...
<h2>Наследование</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>В Ext JS 4 поддерживается наследование. То есть мы можем на основе существующих классов создать новые. Допустим у нас есть класс Person:</p>
<pre class="brush:js;">
...
<p>Образуем от него класс <code>Manager</code>, это делается с помощью параметра <code>extend</code>:</p>
 <pre class="brush:js;">
...
<p>Данный класс добавляет всего одно свойство - <code>department</code> (отдел менеджера).</p>
<p>Здесь в секции <code>extend: 'Classes.Person'</code> мы указываем, от какого класса наследуется текущий класс. А в конструкторе 
...
то есть в данном случае конструктор) : <code>this.callParent([name, surname]);</code>.</p>
<p>В итоге новый класс будет содержать как свои свойства и методы, так и родительские:</p>
 <pre class="brush:js;">
...
<h3>Переопределение методов базового класса</h3>
 <p>В предыдущем примере, чтобы вывести сообщение о департаменте менеджера, нам приходилось использовать дополнительный метод. Однако мы можем 
 переопределить имеющийся метод <code>getinfo</code> и выводить всю информацию в нем:</p>
  <pre class="brush:js;">
Ext.define('Classes.Manager', {
	extend: 'Classes.Person',
	config: {
...
//alert("Department : " + eugene.getDepartment());
 ...
 <p>Вызов <code>this.callParent();</code> вызывает метод getinfo базового класса.</p>
 <h4>Сущность наследования</h4>
 <p>Когда мы создаем обычные классы, как, например, выше класс <code>Person</code>, и не используем при этом параметр <code>extend</code>, 
 создаваемый класс все равно неявно наследуется от базового для всех класса <code>Ext.Base</code>.</p>
 <p>Использование секции <code>extend</code> приводит к тому, что при создании класса на этапе предобработки (этап Extend) к создаваемому классу 
 присоединяется функционал класса-родителя. В итоге получается следующая цепочка наследования:</p>
