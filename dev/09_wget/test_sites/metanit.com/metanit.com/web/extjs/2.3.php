...
<html >
<head>
<title>Параметр Config в Ext JS</title>
<meta charset="utf-8" />
<meta name="description" content="Использование параметра Config для начальной инициализации объектов">
...
<h2>Параметр Config</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>В Ext JS 4 классы при объявлении членов класса могут использовать секцию <code>config</code>. Ее использование имеет некоторые преимущества по сравнению 
с обычным объявлением свойств класса, как это было в предыдущих разделах главы. В частности:</p>
<ul>
<li><p>Свойства в секции <code>config</code> (конфигурации) полностью инкапсулируются от других членов класса</p></li>
<li><p>Для каждого свойства в секции <code>config</code> автоматически создаются методы <code>get</code> и <code>set</code>, если эти методы 
еще не определены в классе</p></li>
<li><p>Для каждого свойства в секции <code>config</code> автоматически создается метод <code>apply</code>. Этот метод вызывается методом 
<code>set</code> перед установкой значения свойства. И если метод <code>apply</code> не возвращает никакое значение, то метод <code>set</code> 
не будет устанавливать для свойства новое значение.</p></li>
</ul>
<p>Таким образом, следующий класс</p>
<pre class="brush:js;">
...
<p>будет эквивалентен следующему за счет автоматического создания методов для свойств в секции <code>config</code>:</p>
<pre class="brush:js;">
Ext.define('Classes.Person', {
	name: 'Eugene',
    surname : 'Popov',
...
<p>Конфигурация класса происходит с помощью метода <strong>initConfig</strong> и передаваемого ему параметра. Этот метод определен у 
базового класса <code>Ext.Base</code> и поэтому содержится и у всех остальных классов (так как они по сути являются наследниками Ext.Base).</p>
<p>Поэтому мы вполне можем использовать автоматически создаваемые методы:</p>
<pre class="brush:js;">
...
<p>Если мы не хотим, чтобы использовался автоматически создаваемый метод, мы его можем задать явно. Например, мы не хотим, чтобы в качестве имени 
можно было указать пустую строку. И для этого переопределим метод <code>applyName</code>:</p>
<pre class="brush:js;">
Ext.define('Classes.Person', {
	config: {
			name: 'Eugene',
