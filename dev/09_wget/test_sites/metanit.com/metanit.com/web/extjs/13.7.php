...
<html >
<head>
<title>Группировка в гриде в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Настройка группировки в хранилище Ext JS с помощью свойства groupField  и  применение плагина Ext.grid.feature.Grouping для отображения группировки в гриде">
...
<h2>Группировка в гриде</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>У ряда объектов какие-то данные могут повторяться. И иногда бывает нужно сгруппировать данные по определенному критерию. 
Связка хранилище+грид позволяют нам настроить группировку данных. Во-первых, настроим хранилище:</p>
<pre class="brush:js;">
...
<p>Параметр <code>groupField</code> указывает по какому свойству будут группироваться объекты. После этого мы в принципе могли бы уже 
использовать грид, так как данные у нас уже будут сгруппированы.</p>
<p>Но грид позволяет настроить визуальное отображение групп с помощью плагина <code>Ext.grid.feature.Grouping</code>:</p>
<pre class="brush:js;">
...
<p>Для использования плагина мы применяем параметр feauture:</p>
<pre class="brush:js;">
	features: [Ext.create('Ext.grid.feature.Grouping', {groupHeaderTpl: 'Группа  {name} ({rows.length})' })],
</pre>
<p>Опция <code>groupHeaderTpl</code> указывает формат отображения в плагине группировки. В данном случае мы получаем через {name} значение 
свойства группировки (то есть свойства married), а с помощью выражения {rows.length} получаем количество строк в группе.</p>
<p>После этого мы получим сгруппированные данные в гриде:</p>
<img src="pics/13.7.png" />
...
<div class="nav"><p><a href="13.6.php">Назад</a><a href="./">Содержание</a><a href="14.1.php">Вперед</a></p></div></div>
<div class="socBlock">
