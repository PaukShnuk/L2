...
<html >
<head>
<title>Компоновка CardLayout в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Создание структуры приложения в виде слайдов с помощью компоновки CardLayout">
...
<h2>CardLayout</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Компоновка CardLayout позволяет разместить элементы по типу слайдов - каждая слайд располагается поверх другого, и единовременно виден только
один слайд. Но мы модем переключать слайды как вперед, так и назад.</p>
<pre class="brush:js;">
...
<p>Значение <code>layout:'card'</code> задает компоновку в виде слайдов - тем самым мы применяем к контейнеру класс <code>Ext.layout.container.Card</code>.</p>
<p>Все слайды у нас определены в свойстве items и представляют дочерние элементы панели.</p>
<p>Затем после определения логики переключения между слайдами мы используем параметр <code>bbar</code> для создания панели навигации из двух кнопок.</p>
