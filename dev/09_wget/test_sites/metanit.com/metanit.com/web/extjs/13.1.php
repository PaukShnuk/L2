...
<html >
<head>
<title>Ext.grid.Panel и отображение данных в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в элемент Ext.grid.Panel, его связь с моделью и хранилищем и отображение данных в виде таблицы">
...
<h1>Таблицы и Ext.grid.Panel</h1>
<h2>Ext.grid.Panel и отображение данных</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Для отображения и управления данными в виде таблицы в ExtJS 4 предназначен такой мощный элемент управления, как 
<code>Ext.grid.Panel</code>. Разберем на примере простейшее отображение данных в данном элементе.</p> 
<p>Итак, сначала определим сами данные. Это могут быть к примеру данные, находящиеся в формате json на сервере. Пусть это будет файл 
<i>users.json</i> следующего содержания:</p>
<pre class="brush:js;">
{
	"success": true,
	"users": [{
...
<p>Каждый элемент представляет пользователя с несколькими свойствами, как id, дата рождения, имя, фамилия, семейное положение и адрес электронной почты. 
Вторым шагом будет создание модели, соответствующей этим данным, и хранилище, которое будет загружать данные с сервера:</p>
<pre class="brush:js;">
Ext.define('User', {
...
<p>Хранилище <code>store</code> будет автоматически загружать данные из файла <i>users.json</i>. Теперь свяжем сам элемент <code>Ext.grid.Panel</code> 
с хранилищем:</p>
<pre class="brush:js;">
...
<p>Итак, элемент Ext.grid.Panel представляет собой панель в виде таблицы. Чтобы связать Ext.grid.Panel с хранилищем, устанавливаем свойство 
<code>store</code>. Затем с помощью свойства <code>columns</code> определяем массив столбцов, которые будут отображать данные из хранилища.</p>
<p>Для каждого столбца устанавливаются свойство <code>header</code>, которое обозначает название столбца, выводимое на веб-странице, и 
свойство <code>dataIndex</code>, которое в качестве значения принимает свойство модели, значение которого будет отображаться в данном столбце.</p>
<p>При определении столбца, выводящего дату, мы использовали свойство <code>xtype</code>. Позже мы посмотрим на его использование в столбцах, 
а пока надо понимать, что данное свойство использовалось прежде всего для явного указания типа столбца и преобразования даты в определенный формат.</p>
<p>Также надо отметить, что необязательно создавать столбцы для всех свойств модели. В итоге, мы получим следующий вывод:</p>
<img src="pics/13.1.png" alt="Отображение табличных данных в ExtJS 4" />
...
<div class="nav"><p><a href="12.7.php">Назад</a><a href="./">Содержание</a><a href="13.2.php">Вперед</a></p></div></div>
<div class="socBlock">
