...
<html >
<head>
<title>Взаимодействие с Local Storage HTML5 в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие с Local Storage HTML5, использование LocalStorageProxy для сохранения в локальном хранилище браузера объектов и их последующее извлечение и использование в программе">
...
<h2>Взаимодействие с Local Storage HTML5</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Ext JS 4 предоставляет специальный компонент в виде LocalStorageProxy (<code>Ext.data.proxy.LocalStorage</code>), который позволяет сохранять 
данные в браузере пользователя с помощью Local Storage API, которое имеется в HTML5.</p>
<p>Local Storage сохраняет данные в виде пары ключ-значение. А LocalStorageProxy обеспечивает взаимодействие с сохраненными данными - сохраняет и 
загружает из Local Storage, кроме того, выполняет их сериализацию и десериализацию.</p> 
<p>Рассмотрим подобное взаимодействие на следующем примере:</p>
<pre class="brush:js;">
...
<p>Чтобы взаимодействовать с локальным хранилищем, устанавливаем соответствующие настройки для параметра proxy у модели:</p>
<pre class="brush:js;">
		proxy: {
                 type: 'localstorage',
                 id: 'settings'
...
<p>Установка <code>type: 'localstorage'</code> подразумевает возможное сохранение объектов данной модели в Local Storage. Настройка 
<code>id: 'settings'</code> устанавливает ключ, по которому объекты будут сохраняться в локальном хранилище. Если в будущем мы сохраним 
несколько таких объектов, то к этому ключу будет добавляться id объекта, который мы будем устанавливать в поле модели <code>id</code>.</p>
<p>Далее мы создаем один объект такой модели и сохраняем в локальном хранилище с помощью метода <code>save</code>:</p>
<pre class="brush:js;">
var settings = Ext.create('User', {
             id: 1,
             name: 'Eugene22',
...
<p>Для последующей выгрузки данных из локального хранилища браузера в скрипт Ext JS нам надо использовать метод <code>load</code> модели:</p>
<pre class="brush:js;">  
      User.load(1, {
            callback: function(model, operation){
                    alert ('Ник:' + model.get('name'));
...
<p>В качестве первого параметра метода передается id объекта данной модели. Затем он соединяется с id, указанным в прокси модели и по получившемуся 
ключу происходит извлечение объекта (то есть "settings" + "1" в данном случае).</p>
<p>В качестве второго параметра выступает функция, вызывающаяся при извлечении объекта. С помощью первого параметра - model мы можем получить 
извлеченный из локального хранилища объект.</p>
...
<div class="nav"><p><a href="12.6.php">Назад</a><a href="./">Содержание</a><a href="13.1.php">Вперед</a></p></div></div>
<div class="socBlock">
