...
<html >
<head>
<title>Выбор модели в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Настройка формата выбора модели в Ext.grid.Panel с помощью классов Ext.selection.RowModel и Ext.selection.CellModel">
...
<h2>Выбор модели</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Благодаря выделению строки мы можем настроить выбор модели из грида. При этом мы можем настроить также и параметры выделения 
строки и даже отдельной ячейки с помощью моделей <code>Ext.selection.RowModel</code> и <code>Ext.selection.CellModel</code>.</p>
<p>Возьмем определение грида из предыдущего параграфа и применим Ext.selection.RowModel, заменив последний столбец следующим образом:</p>
<pre class="brush:js;">
{
	xtype:'actioncolumn',
	width:40,
...
<p>С помощью вызова <code>grid.getSelectionModel()</code> мы получаем модель выбора строки.</p>
<p>Метод <code>selectionModel.select(rowIndex);</code> выделяет строку (если она не выделена), а с помощью кода 
<code>record = selectionModel.getSelection()[0];</code> мы получаем модель, которая отображается на данную строку, 
в переменную record и затем модем уже ее манипулировать. Например, далее мы получаем значение свойства name: <code>record.get('name')</code></p>
<p>Несколько иначе работает класс <code>CellSelectionModel</code>, задающий выделение отдельной ячейки, а выделение всей строки при этом не происходит.</p>
<p>Для этого для всего грида установим формат выделения:</p>
<pre class="brush:js;">
