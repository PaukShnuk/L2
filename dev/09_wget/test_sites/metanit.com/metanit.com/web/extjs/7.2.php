...
<html >
<head>
<title>Жизненный цикл компонентов в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Описание жизненного цикла компонентов - создание, рендеринг и уничтожение объектов, а также основные этапы этих стадий">
...
<h2>Жизненный цикл компонентов</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Все компоненты Ext JS имеют определенный жизненный цикл, который включает следующие стадии:</p>
<ol>
<li><p><span class="bb">Инициализация</span></p></li>
<li><p><span class="bb">Рендеринг</span></p></li>
<li><p><span class="bb">Уничтожение</span></p></li>
</ol>
<p>Посмотрим на примере простого объекта панели, что они представляют:</p>
<pre class="brush:js;">
...
<h4>Инициализация</h4>
<p>На этом этапе происходит создание компонента, которое включает ряд шагов:</p>
<ol>
<li><p><i>Применение конфигурационных настроек</i>. На этом этапе компонент применяет все установленные параметры и свойства, которые мы передаем, 
например, через конструктор.</p>
<p>Так, на этом этапе определенные нами свойства <code>title</code>, <code>width</code> и <code>renderTo</code> копируются в объект панели.</p></li>
<li><p><i>Регистрация базовых событий компонента</i>. Каждый подкласс класса Ext.Component по умолчанию имеет некоторый набор основных событий: 
enable, disable, beforeshow, show, beforehide, hide, beforerender, render, beforedestroy, destroy.</p></li>
<li><p><i>Регистрация компонента</i>. Каждому создаваемому компоненту присваивается идентификатор, 
если мы его не определим явным образом в качестве свойства.</p>
<p>В случае с вышеопределенной панелькой используется определенный в качестве свойства id.</p></li>
<li><p><i>Загрузка плагинов</i>. Если для компонента нужны или определены какие-нибудь плагины, они загружаются на этом этапе. 
Если плагины не определены, как в случае с нашей панелью, то этап пропускается.</p></li>
<li><p><i>Вызов метода initComponent</i>. Этот метод выполняет большую часть работы по инициализации компонентов и классов</p></li>
<li><p><i>Добавление нового экземпляра класса в объект Ext.ComponentManager</i>. Благодаря чему затем можно будет получать компонент по его id 
через метод <code>Ext.getCmp</code>: <code>var myPanel = Ext.getCmp('myPanel');</code></p></li>
<li><p><i>Прямой рендеринг (если применяется)</i>. Если для компонента указан параметр <code>renderTo/applyTo</code>, то применяется рендеринг, иначе 
компонент не визуализируется, пока не будет вызван метод render</p></li>
</ol>
<h4>Рендеринг</h4>
<p>Стадия рендеринга также проходит через ряд этапов:</p>
<ol>
<li><p><i>Вызов собыия beforerender</i>. Компонент вызывает событие beforerender и проверяет результаты всех зарегистрированных обработчиков этого события. 
Если обработчик события возвращает <code>false</code>, то компонент останавливает рендеринг. </p></li>
<li><p><i>Установка свойства z-index</i>. Если компонент является "плавающим", например, меню или окно, то у него устанавливается CSS-свойство 
<code>z-index</code> для корректного отображения.</p></li>
<li><p><i>Установка контейнера</i>. Для компонента устанавливается контейнер, в котором будет производится рендеринг компонента.</p></li>
<li><p><i>Вызов метода onRender</i>. На этом этапе добавляются все необходимые элементы DOM к компоненту, и он приобретает те черты, 
которые мы потом можем лицезреть на экране</p></li>
<li><p><i>Компонент отображается на экране</i>. По умолчанию многие компоненты не отображаются и остаются скрытыми, используя стандартные классы 
Ext CSS, например, класс "x-hidden". Но если у компонента свойство <strong>autoShow</strong> имеет значение <code>true</code>, то все классы CSS, скрывающие 
элемент, удаляются</p></li>
<li><p><i>Применение пользовательских классов и стилей CSS</i>. На этом этапе применяются пользовательские стили и классы через использование свойств <code>cls</code> и 
<code>style</code></p></li>
<li><p><i>Инициализация события render</i>. На этом этапе все необходимые элементы уже добавлены в структуру DOM страницы, а стили уже применены. 
А событие уведомляет систему, что компонент готов к использованию.</p></li>
<li><p><i>Инициализация контента</i>. Существует несколько способов добавления контента: через свойство <code>html</code>, через 
свойство <code>contentEl</code> (которое содержит id элемента, используемого в качестве содержимого для компонента) и через свойство 
<code>tpl</code> (которое принимает шаблон контента)</p>
<p>В случае с использованной выше панелькой задано свойство <code>html</code>, поэтому контент для панели берется из него.</p></li>
<li><p><i>Вызов метода afterRender</i>. Этот метод автоматически вызывается в методе <code>render</code> компонента. В нем выполняется вся необходимая логика 
пострендеринга</p></li>
<li><p><i>Возникновение события afterRender</i>.</p></li>
<li><p><i>Скрытие или отключение компонента</i>. Если у компонента свойства <code>hidden</code> или <code>disabled</code> имеют значение <code>true</code>, 
то вызываются соответственно методы <code>hide</code> или <code>disable</code></p></li>
</ol>
<p>После этого компонент готов к принятию ввода от пользователя и с ним можно взаимодействовать.</p>
<h4>Уничтожение</h4>
<p>На этом этапе происходит удаление компонента из структуры DOM веб-страницы, отмена его регистрации и отмена регистрации 
обработчиков событий. Метод destroy, производящий уничтожение компонента, производится либо вручную в коде, либо вызывается контейнером и также 
предполагает ряд этапов:</p>
<ol>
<li><p><i>Инициализация события beforedestroy</i>. Если обработчик этого события возвращает <code>false</code>, то уничтожение компонента отменяется</p></li>
<li><p><i>Вызов метода beforeDestroy</i>. Выполняет некоторую логику до уничтожения компонента</p></li>
<li><p><i>Удаление элемента и всех зарегистрированных обработчиков событий элемента</i>.</p></li>
<li><p><i>Вызов метода onDestroy</i>. Этот метод выполняет дополнительные действия по уничтожению компонента, например, удаление хранилищ данных</p></li>
<li><p><i>Уничтожение всех плагинов компонента</i></p></li>
<li><p><i>Удаление всех связанных узлов из DOM</i>, если компонент прошел рендеринг</p></li>
<li><p><i>Инициализация события destroy</i>. Срабатывают все зарегистрированные обработчики данного события. которые сигнализируют, что компонент 
удален из структуры DOM</p></li>
<li><p><i>Удаление обработчиков событий компонента</i>.</p></li>
</ol>
<p>Пользуясь готовыми компонентами, не приходится ломать голову над всеми этими стадиями жизненного цикла. Однако при разработке собственных 
компонентов следует обратить на это внимание, чтобы потом избежать непредвиденных проблем в работе приложения. Например, разработчики могут забыть провести 
чистку разных объектов при уничтожении своего компонента - хранилищ данных, которых продолжат опрашивать веб-сервер на предмет получения данных и т.д.</p>
