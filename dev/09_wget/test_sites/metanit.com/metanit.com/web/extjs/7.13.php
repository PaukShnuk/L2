...
<html >
<head>
<title>Combobox в Ext JS</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и настройка выпадающих списков с помощью компонента Ext.form.ComboBox, подгрузка элементов списка с сервера в запросе ajax">
...
<h2>Combobox в Ext JS</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Функциональность выпадающих списков представлена классом Ext.form.ComboBox. Этот класс позволяет создавать списки, 
объекты которых загружаются как с сервера, либо могут быть объявлены непосредственно в коде:</p>
<pre class="brush:js;">
var yourData = [
     [1, 'JavaScript'],
    [2, 'PHP'],
...
<img src="pics/combobox.png" alt="Выпадающий список в ExtJS" />
<p>Итак, выпадающий список имеет xtype combobox. Параметр <code>fieldLabel</code> отображает метку-заголовок, который обозначает список.</p>
<p>Чтобы связать список с определенным набором данных, используется параметр <code>store</code>. Этот параметр принимает объект хранилища - Ext.data.Store 
(в данном случае взят подкласс Ext.data.SimpleStore, но могу использоваться и другие). Хранилище через свойство <code>data</code> мы связываем с некоторым объектом 
данных. Например, здесь объявлен в самом начале объект yourData, представляющий набор ключей-значений. Поле хранилища <code>fields</code> показывает, 
как будут отображаться данные из yourData.</p>
<p>Поле комбобокса <code>valueField</code> содержит ключ, определенный в поле <code>fields</code> хранилища, соответственно поле <code>displayField</code> 
отображает связанное с ключом значение.</p>
<p>В конце мы задаем режим с помощью свойства <code>queryMode</code>. Так как мы загружаем непосредственно из кода данные, то он имеет значение <code>local</code>. 
При загрузке с сервера надо установить режим <code>remote</code></p>

<h3>Загрузка данных с сервера</h3>
<p>Предположим, у нас на сервере находится следующий файл с данными в формате json <i>langData.json</i>:</p>
<pre class="brush:js;">
{
"success": true,
"languages": [{
...
<p>Затем у нам предстоит определить модель, связанную с хранилищем, а так же само хранилище. И связать хранилище с выпадающим списком, как в примере выше:</p>
<pre class="brush:js;">
Ext.onReady(function(){
	Ext.define('Language', {
...
<p>Во-первых, здесь мы создаем модель <code>Language</code>, которая является фактически расширением класса <b>Ext.data.Model</b>. Для этой модели 
определяем в параметре fields два поля: name с типом string и ID с типом int - как в нашем объекте json.</p>
<p>Второй шаг - создание хранилища, производного от Ext.data.Store. Для него мы указываем в поле model тип объектов - в данном случае это вышеопределенная модель. 
Затем в объекте proxy мы задаем файл, из которого модель будет наполняться данными в результате ajax-запроса и корневой узел объекта json на сервере, 
в котором будет идти поиск объектов.</p>
<p>На третьем этапе мы уже определяем сам комбобокс. В данном случае все как и в примере с загрузкой объектов из кода, только в качестве значения свойства 
<b>queryMode</b> устанавливается значение <code>remote</code>, а в качестве значения параметра store указывается ранее созданное хранилище.</p>
<h3>Автозаполнение списка</h3>
<p>Ext.form.ComboBox позволяет нам использовать возможности автозаполнения поля при вводе символов. Для этого надо установить свойство 
<code>typeAhead:true</code>:</p>
<pre class="brush:js;">
Ext.onReady(function(){
	var store = Ext.create('Ext.data.Store', {
		fields: ['name'],
...
<p>Кроме свойства typeAhead в данном случае использовался параметр <code>typeAheadDelay</code>, которое позволяет задать время задержки в миллисекундах 
перед автозаполнением, а параметр <code>hideTrigger: true</code> скрывает значок выбора из списка.</p>
<h3>Рендеринг содержимого</h3>
<p>Через свойство <code>listConfig</code> класса Ext.form.ComboBox мы можем задать параметры рендеринга содержимого, задав шаблон заполнения html:</p>
<pre class="brush:js;">
Ext.onReady(function(){
	var store = Ext.create('Ext.data.Store', {
...
<img src="pics/render_combobox.png" alt="Рендеринг содержимого комбобокса в ExtJS" />
<p>Что мы сделали в данном случае? В вышеприведенных примерах видно, что список отображает то свойство объекта, которое указано в параметре displayField 
комбобокса. То есть если у нас <code>displayField: 'name'</code>, то будет отображаться содержимое свойства name объекта.</p>
<p>В случае с рендерингом мы задаем параметр <code>listConfig</code>. Свойства, заданные в этом параметре, применяются к объекту списка комбобокса 
 а именно к объекту <b>Ext.view.BoundList</b>. Класс  BoundList имеет метод <code>getInnerTpl</code>, который и возвращает значение для параметра displayField.</p>
 <p>То есть определение параметра <code>displayField: 'name'</code> будет фактически эквивалентно</p>
<pre class="brush:js;">
 getInnerTpl: function(displayField) {
return '{' + displayField + '}';
},
</pre>
<p>где на место displayField подставляется name. Поэтому через метод  getInnerTpl можно переопределить шаблон рендеринга содержимого в комбобоксе.</p>
...
<div class="nav"><p><a href="7.12.php">Назад</a><a href="./">Содержание</a><a href="7.14.php">Вперед</a></p></div></div>
<div class="socBlock">
