...
<html >
<head>
<title>Динамическая загрузка классов в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Динамическая загрузка классов, применение функции Ext.require">
...
<h2>Динамическая загрузка классов</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>В ExtJS 4 существует возможность динамически подгружать нужные нам классы. Во-первых, выделим классы в отдельные файлы и расположим их в определенном 
месте на физическом диске.</p>
<p>Выделим следующий класс в файл <code>Person.js</code> (файлы должны иметь имена, соответствующие именам классов):</p>
<pre class="brush:js;">
Ext.define('Classes.Person', {
 ...
 <p>А класс Manager, который наследуется от класса Person, в файл <code>Manager.js</code>:</p>
 <pre class="brush:js;">
...
<p>Затем поместим их в определенном месте. Так как они находятся в пространстве имен <code>Classes</code>, то создадим в текущей директории, где находится 
наша html-страница, папку Classes и поместим в нее файлы Person.js и Manager.js:</p>
<img src="pics/2.3.png" />
<p>Исходя из скриншота моя html-страница находится в папке <code>helloext</code>, а выше упомянутые классы - в папке <code>helloext/Classes</code>.</p>
<p>Теперь изменим html-страницу следующим образом:</p>
<pre class="brush:xml;">
&lt;html&gt;
&lt;head&gt;
...
<p>Таким образом, мы вынесли весь функционал классов во внешние файлы. И используя класс <code>Ext.Loader</code>, мы сопоставляем физический путь 
с пространством имен (<code>paths: { 'Classes' : '<span class="bb">Classes</span>' }</code> - где жирным выделен путь, с которым сопоставляется пространство имен) и 
 загружает все классы по указанному пути.</p>
<p>Затем с помощью функции <strong>Ext.require</strong> загружаются в приложение классы. В качестве второго параметра указывается функция 
обратного вызова, которая срабатывает при загрузке класса.</p>
<p>Нам необязательно выполнять некоторые действия в качестве функции обратного вызова метода <code>Ext.require</code>. Мы можем сначала 
загрузить все нужные нам классы с помощью методов <code>Ext.require</code>, а потом с помощью обработки события <code>Ext.onReady</code> 
выполнить всю остальную логику приложения:</p>
<pre class="brush:xml;">
&lt;html&gt;
&lt;head&gt;
    &lt;title&gt;Hello Ext&lt;/title&gt;
...
&lt;/html&gt;
 ...
 <p>Кроме того, мы можем добавлять сопоставления классов с физическими путями дополнительно с помощью метода <strong>Ext.Loader.setPath</strong>:</p>
 <pre class="brush:js;">
