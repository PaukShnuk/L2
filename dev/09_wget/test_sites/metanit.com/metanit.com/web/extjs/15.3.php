...
<html >
<head>
<title>Диаграммы в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Создание диаграмм в ExtJS и класс Ext.chart.Chart, настройка осей графиков, связь диаграмм с хранилищем, создание легенды диаграммы">
...
<h2>Диаграммы</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Одним из существенных преимуществ фреймворка ExtJS 4 является удобный функционал по созданию разного рода диаграмм и графиков. 
За построение диаграмм отвечает класс <b>Ext.chart.Chart</b>. Создадим простейшую диаграмму:</p>
<pre class="brush:js;">
...
<img src="pics/15.5.png" alt="Пример диаграммы в ExtJS 4" />
<p>Данный график изображает статистику по использованию браузера Google Chrome. Логически мы можем разбить код на три части: создание модели, 
создание хранилища и сам график. С моделью и хранилищем все понятно, поэтому рассмотрим построение непосредственно графика.</p>
<p>Данные для графику берутся из хранилища, поэтому вначале устанавливаем свойство <code>store</code>.</p>
<h3>Создание осей</h3>
<p>Для определения осей и типов данных, которые отображаются на эти оси, используется свойство <code>axes</code>. Существует несколько типов осей:</p>
<ul>
<li><p><span class="bb">Category</span>: тип оси для нечисловых данных</p></li>
<li><p><span class="bb">Numerical</span>: тип оси для числовых данных (как в примере выше)</p></li>
<li><p><span class="bb">Time</span>: тип оси для временных периодов</p></li>
<li><p><span class="bb">Gauge</span>: тип оси для кольцевых диаграмм</p></li>
</ul>
<p>А свойство <code>fields</code> содержит массив полей модели. которые будут отображаться по данной оси.</p>
<p>Поскольку первое поле модели имеет тип <code>float</code>, то для него используется тип оси <code>Numeric</code>. А для строк 
используется тип <code>Category</code>:</p>
<pre class="brush:js;">
	axes: [{
            title: 'Доля браузера',
            type: 'Numeric',
...
<p>Если бы какое-то поле модели представляло бы тип <code>Date</code>, то в этом случае мы бы могли использовать тип оси <code>Time</code></p>
<p>Свойство <code>position</code> указывает на  расположение оси.</p>
<p>Свойство <code>position</code> указывает на положение оси. Для осей Х оно может иметь значения <code>bottom</code> и <code>top</code>, 
а для оси Y - значения <code>left</code> и <code>right</code>.</p>
<h4>Настройка сетки</h4>
<p>При помощи параметра <code>grid</code> у осей можно настроить сетку графика:</p>
<pre class="brush:js;">
...
<p>Параметр <code>grid: true</code> задает простое отображение сетки. Однако мы можем более детально настроить отображение сетки, например, в предыдущем примере 
настраивается отображение нечетных столбцов.</p>
<h3>Настройка легенды</h3>
<p>Используя параметр <code>legend</code>, мы можем настроить отображение легенды диаграммы:</p>
<pre class="brush:js;">
var chart = Ext.create('Ext.chart.Chart', {
	   renderTo: Ext.getBody(),
	   width: 400,
...
<p>Параметр <code>legend: {position: 'right'}</code> устанавливает легенду диаграммы в определенной стороне с помощью опции <code>position</code>. Она может принимать 
значения <code>left</code>, <code>right</code>, <code>top</code> и <code>bottom</code>. А опция <code>title</code> в объекте <code>series</code> 
устанавливает текст легенды:</p>
<img src="pics/15.6.png" alt="Легенда диаграммы в ExtJS 4" />

<h3>Тема диаграммы</h3>
<p>Используя параметр <code>theme</code> объекта <code>Ext.chart.Chart</code> можно настроить тему отображения:</p>
<pre class="brush:js;">
Ext.create('Ext.chart.Chart', {
	   renderTo: Ext.getBody(),
	   theme: 'Sky',
...
<p>ExtJS4 предлагает несколько встроенных тем:  'Base', 'Green', 'Sky', 'Red', 'Purple', 'Blue', 'Yellow' и шесть тем категорий от 
'Category1' до 'Category6'. По умолчанию используется тема 'Base'.</p>
<p>Теперь перейдем к рассмотрению различных видов диаграмм.</p>
