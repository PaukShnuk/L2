...
<html >
<head>
<title>Компонент Ext.panel.Panel в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Создание панелей с помощью класса Ext.Panel и прикрпеление к их сторонам других компонентов">
...
<h2>Компонент Ext.panel.Panel</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Контейнер Panel представлен классом <b>Ext.panel.Panel</b> и является наиболее часто используемым компонентом Ext JS:</p>
<pre class="brush:js;">
...
<p>Используя различные свойства панели, мы можем прикрепить элементы к одной из ее сторон. В частности мы можем использовать параметр 
<strong>dockedItems</strong>:</p>
<pre class="brush:js;">
...
<img src="pics/dockedpanel.png" alt="Компонент Ext.panel.Panel" />
<p>Свойство dockedItems принимает массив тулбаров, прикрепляемых к сторонам панели. Чтобы прикрепить тулбар к определенной стороне, мы устанавливаем свойство 
<code>dock</code>, которое можем принимать следующие значения: top, bottom, left, right. Каждый тулбар включает несколько кнопок.</p>
<p>Знак '-&gt;', используемый в описании тулбара, автоматически преобразуется в объект <strong>Ext.toolbar.Fill</strong> - специальный компонент, который 
растягивается по всей ширине тулбара, которая осталась после вычитания ширины всех остальных его компонентов.</p>
<p>другой используемый знак '-' преобразуется в объект <strong>Ext.toolbar.Separator</strong>. В визуальном плане он создает вертикальную черточку-разделитель.</p>
<p>Есть также альтернативные методы закрепления компонентов у границ панели. Так, мы могли бы использовать свойство <strong>fbar</strong> для прикрепления 
тулбара внизу:</p>
<pre class="brush:js;">
	Ext.create('Ext.panel.Panel', {
		title: 'Приложение',
...
<img src="pics/fbar.png" />
<p>Еще один метод позволяет закрепить тулбары по краям, используя свойства lbar, rbar, tbar, bbar:</p>
<pre class="brush:js;">
	Ext.create('Ext.panel.Panel', {
		title: 'Приложение',
		width: 400,
...
<img src="pics/dockedtoolbars.png" />
<h4>Метод addDocked</h4>
<p>Альтернативой использования параметра dockedItems является применение метода addDocked:</p>
<pre class="brush:js;">
...
<h3>Сворачивание и закрытие панели</h3>
<p>С помощью свойст <code>collapsible</code> и <code>closable</code> можно задать сворачивание и закрытие панели соответственно. В случае если 
эти свойства имеют значение <code>true</code>, в верхней части панели отобразятся значки, как в обычных оконных приложениях, 
через которые можно будет закрыть или свернуть панель:</p>
<pre class="brush:js;">
Ext.create('Ext.panel.Panel', {
		title: 'Приложение',
		width: 400,
...
<h3>Свойство tools</h3>
<p>Используя свойство <code>tools</code> можно задать иконки для панели, например:</p>
<pre class="brush:js;">
Ext.create('Ext.panel.Panel', {
		title: 'Приложение',
		width: 400,
...
<p>Эти иконки берутся из стандартного набора, в котором определено всего 25 иконок. Каждая иконка представляет экземпляр класса 
<strong>Ext.panel.Tool</strong> и позволяет связать с собой обработчик нажатия.</p>
