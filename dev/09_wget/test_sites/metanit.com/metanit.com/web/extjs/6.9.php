...
<html >
<head>
<title>Анкорная компоновка в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Описание анкорной компоновки и параметров ее использования">
...
<h2>Анкорная компоновка</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Анкорная компоновка позволяет определить размер дочернего элемента в зависимости от размера контейнера. Например, мы можем указать процентное 
отношение размеров дочернего элемента к размерам родителя:</p>
<pre class="brush:js;">
...
<p>Чтобы определить анкорную компоновку, мы задаем <code>layout:'anchor'</code>. Затем мы можем применить параметр <code>anchor</code> у дочерних 
элементов, чтобы задать их размеры. В данном случае используется значение <code>anchor: '60% 85%'</code>, то есть внутренняя панель будет иметь 
ширину в 60% от ширины контейнера, и высоту в 85% от высоты контейнера.</p>
<p>Также мы можем задать и другие варианты анкорной компоновки с помощью параметра <code>anchor</code>. Например, мы можем сделать смещение дочернего 
элемента от границ, использовав <code>anchor: '-50 -25'</code>, где 50 - смещение от правого края контейнера, а 25 - смещение от нижней границы контейнера:</p>
<pre class="brush:js;">
Ext.onReady(function(){
var panel= Ext.create('Ext.Panel', {
            title: 'Компоновка Anchor',
...
<p>Но мы также можем определить фиксированные размеры дочерней панели, указав <code>anchor: 'r b'</code>:</p>
<pre class="brush:js;">
Ext.onReady(function(){
var panel= Ext.create('Ext.Panel', {
            title: 'Компоновка Anchor',
