...
<html >
<head>
<title>Валидация полей формы в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Валидация полей формы ExtJS с помощью встроенных объектов VType, создание собственных объектов и правил валидации">
...
<h2>Валидация полей формы</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Когда мы рассматривали текстовые поля в одной из прошлых глав, мы видели, что можно проводить их валидацию с помощью таких свойств, как <code>allowBlank</code> 
и <code>minLength/maxLength</code>. Кроме того, для валидации полей формы в ExtJS мы можем применять синглтон <b>Ext.form.field.VTypes</b>.</p>
<p>VType предоставляет функцию валидации, сообщение об ошибках, которое появляется при неудаче валидации. Кроме того, с помощью VType можно установить 
маску регулярного выражения, которое позволит отсеять нежелательные символы.</p>
<p>Фреймворк уже имеет несколько шаблонных функций для валидации по умолчанию:</p>
<ul class="clear">
<li><p><span class="bb">alpha</span>: ограничивает ввод только алфавитными символами</p></li>
<li><p><span class="bb">alphnum</span>: позволяет вводить только алфавитно-цифровые символы</p></li>
<li><p><span class="bb">url</span>: проверяет правильность ввода адреса URL</p></li>
<li><p><span class="bb">email</span>: проверяет правильность ввода адреса электронной почты</p></li>
</ul>
<pre class="brush:js;">
...
<p>Чтобы использовать валидацию, мы присваиваем параметру <code>vtype</code> имя шаблонной функции валидации.</p>
<img src="pics/11.4.png" alt="Валидация в ExtJS 4" />
<h3>Создание своей логики валидации</h3>
<p>Так как набор встроенных способов валидации довольно ограничен, то нам может потребоваться создать свою логику валидации. Для этого нам надо переопределить 
VType. Этот объект состоит из трех частей:</p>
<ul>
<li><p>Функция, которая проверяет, соответствует ли введенное значение некому правилу</p></li>
<li><p>Текст сообщения, всплывающего при ошибке валидации</p></li>
<li><p>Маска ввода текста (необязательный параметр)</p></li>
</ul>
<p>При создании своих объектов VType используются некоторые соглашения по именованию. Посмотрим на примере. Допустим, создадим следующий объект vtype 
для валидации серии и номера паспорта:</p>
<pre class="brush:js;">
var passNumberVType = {
...
<p>Здесь определен vtype под именем passNumber. Поэтому маска ввода (passNumberMask) и параметр текста сообщения (passNumberText) имеют в своем названии 
в качестве префикса имя объекта.</p>
<p>Переменная passNumberRegex задает регулярное выражение для номера/серии паспорта, и если оно не соответствует переданному в параметре val значению, 
то ввод неудачен. Регулярное выражение очень простое - сначала идет 4 цифры (d{4}), потом знак пробела (\s), а потом еще 6 цифр.</p>
<p>Тогда форма ввода будет использовать vtype следующим образом:</p>
<pre class="brush:js;">
...
<p>Первая строка (<code>Ext.apply(Ext.form.field.VTypes, passNumberVType);</code>) применяет объект валидации синглтону 
<code>Ext.form.field.VTypes</code>, который и содержит все объекты валидации.</p>
<img src="pics/11.5.png" alt="Создание валидации в ExtJS 4" />
