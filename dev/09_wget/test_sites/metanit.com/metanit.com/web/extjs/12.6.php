...
<html >
<head>
<title>Ридеры хранилища в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Использование в хранилище ExtJS ридеров для получения и извлечения данных в формате json и xml">
...
<h2>Ридеры хранилища</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>В предыдущем разделе в одном из примеров мы загружали данные из json-файла в хранилище, используя свойство proxy. 
Но поскольку в качестве источника данных могут выступать наборы данных в различных форматах, ExtJS позволяет нам более точно настроить опции загрузки в 
хранилище. Так, мы можем воспользоваться ридерами.</p>
<h3>JSON</h3>
<p>Например, у нас данные на сервере находятся в следующем файле users.json:</p>
<pre class="brush:xml;">
{	
	"data":{
		"users":[{
...
<p>В данном случае интересующие нас данные находятся в узле <i>data.users</i>, поскольку узел users является вложенным узлом по отношению к 
узлу data. Тогда для того, чтобы загрузить данные в приложение, мы будет использовать следующее определение хранилища:</p>
<pre class="brush:xml;">
...
<p>По сравнению с кодом хранилища из прошлого раздела у нас в данном случае добавлен следующий участок кода:</p>
<pre class="brush:js;">
	reader:{
				type:'json',
				root: 'data.users'
...
<p>Параметр reader опредедяет ридер хранилища. В данном случае он имеет тип json (параметр type указывает на тип читаемых данных), 
а параметр root определяет узел данных, откуда будет вестись чтение. Так как у нас в файле json данные находятся по пути data.users, 
следовательно, параметр root и будет принимать это значение. Вложенность узлов при определении пути определяется точкой.</p>
<h3>XML</h3>
<p>Еще одним способом хранения данных является xml. Итак, определим следующий файл users.xml, который будет хранить данные:</p>
<pre class="brush:xml;">
&lt;users&gt;
    &lt;user&gt;
        &lt;id&gt;1&lt;/id&gt;
...
<p>В этом случае определение хранилища выглядело бы следующим образом:</p>
<pre class="brush:js;">
	Ext.define('UserStore', {
		extend: 'Ext.data.Store',
		model: 'User',
...
<p>Весь остальной код остался бы тем же самым. В данном случае опять же в опции reader мы устанавливаем параметр type (только теперь xml).</p>
<p>В параметре root устанавливаем корневой элемент, в котором находятся данные. А параметр record указывает на узел в файле xml, который 
будет сопоставляться с моделью хранилища (то есть в данном случае узел user).</p>
...
<div class="nav"><p><a href="12.3.php">Назад</a><a href="./">Содержание</a><a href="12.7.php">Вперед</a></p></div></div>
<div class="socBlock">
