...
<html >
<head>
<title>Компоновка FitLayout в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Использование компоновки FitLayout, которая заполняет все пространство контейнера внутренним дочерним элементом">
...
<h2>FitLayout</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>При использовании компоновки <code>FitLayout</code> внутренний элемент растягивается до границ родительского контейнера.</p>
<p>Например, разместим в панели внутреннюю панель, используя этот вариант компоновки:</p>
<pre class="brush:xml;">
&lt;html&gt;
&lt;head&gt;
...
<p>Свойство <code>layout: 'fit'</code> в данном случае показывает, что используется компоновка FitLayout и что внутренний элемент будет заполнять все 
содержимое родительского контейнера.</p>
<img src="pics/3.2.png" />
<p>FitLayout имеет значение только для первого дочернего элемента. Если бы у нас было бы две внутренних панели, как в прошлом разделе, то первая панель 
бы была растянута до границ контейнера, а вторую просто не было бы видно.</p>
