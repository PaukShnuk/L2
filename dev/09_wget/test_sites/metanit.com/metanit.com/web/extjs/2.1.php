...
<html >
<head>
<title>Система классов в Ext JS 4</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и использование классов в Ext JS, определение их свойств и методов">
...
<h1>Система классов в Ext JS 4</h1>
<h2>Определение и использование классов</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>JavaScript не является в чистом виде объектно-ориентированным языком. Но с помощью прототипов можно имитировать классы, которые играют в 
Ext JS 4 важную роль, поскольку на классах завязано большая часть функционала фреймворка.</p>
<p>Для определения новых классов используется метод <b>Ext.define</b>:</p>
<pre class="brush:js;">Ext.define(className, members, onClassCreated);</pre>
<p>Этот метод принимает следующие параметры:</p>
<ul>
<li><p><code>className</code> - имя класса в виде строки</p></li>
<li><p><code>members</code> представляет коллекцию членов класса в виде пар ключ-значение</p></li>
<li><p><code>onClassCreated</code> - необязательный параметр. Представляет функцию обратного вызова, которая вызывается после создания объекта данного класса</p></li>
</ul>
<p>Создадим примитивный класс. </p>
<pre class="brush:js;">
...
<p>Что у нас тут делается? Мы определили класс <code>Person</code>, который находится в пространстве имен <code>Classes</code>. Затем 
...
мы определили метод <code>getinfo</code>, который будет выводить собственно значения этих свойств.</p>
<p>Применим класс в приложению и создадим его объект. Для этого используется функция <strong>Ext.create</strong>. Пусть у нас будет следующая страница html:</p>
<pre class="brush:xml;">
&lt;html&gt;
&lt;head&gt;
...
<p>Строки после определения класса показывают его использование: сначала мы создаем его объект 
(<code>var eugene = Ext.create('Classes.Person');</code>) и затем можем вызвать его единственнный метод (<code>eugene.getinfo();</code>).</p>
<p>В итоге после загрузки страницы у нас будет появляться сообщение с информацией о полном имени созданного пользователя.</p>
<h4>Пространства имен</h4>
<p>Для организации классов в логические группы используются пространства имен. Выше использовалось пространство имен Classes, но пространства имен 
могут иметь несколько уровней вложенности, например, <code>Classes.Person.Manager</code> или <code>Company.Department.Division</code>.</p>
<h4>Изменение свойств</h4>
<p>Выше мы использовали объект с установленными по умолчанию значениями свойств. Но мы можем во время выполнения их изменить:</p>
<pre class="brush:js;">
&lt;script&gt;
		Ext.define('Classes.Person', {
			name: 'Eugene',
...
<h3>Сущность создания классов</h3>
<p>После определения каждый из классов превращается с помощью класса <code>Ext.ClassManager</code> в объект класса <code>Ext.Class</code>. 
Подобное превращение проходит два этапа: предобработку и постобработку, которые включают ряд действий.</p>
<p>На стадии предобработки инициализируются различные части класса. Таким образом, предобработчики запускаются до создания класса. 
Всего определены следующие предобработчики:</p>
<ul>
<li><p><span class="bb">className</span> : определяет пространство имен и имя класса</p></li>
<li><p><span class="bb">loader</span> : загружает все другие необходимые классы и зависимости, если таковые не имеются, данный этап пропускается</p></li>
<li><p><span class="bb">extend</span> : осуществляет наследование от классов, которые определены в секции <code>extend</code></p></li>
<li><p><span class="bb">statics</span> : обрабатывает и создает статические члены класса, которые определены в секции <code>statics</code></p></li>
<li><p><span class="bb">inheritableStatics</span> : наследует статические члены родительского класса, если таковые определены</p></li>
<li><p><span class="bb">config</span> : обрабатывает свойства, определенные в секции <code>config</code>. У этих свойств создаются методы 
<code>get/set/applay/reset</code></p></li>

<li><p><span class="bb">mixins</span> : добавляет к текущему классу функциональность миксин-классов, которые определены в секции <code>mixins</code></p></li>
<li><p><span class="bb">xtype</span> : определяет параметр xtype для нового класса</p></li>
<li><p><span class="bb">alias</span> : устанавливает псевдоним нового класса</p></li>
</ul>
<p>После завершения всех этапов предобработки начинается постобработка, на этапе которой срабатывают следующие обработчики:</p>
<ul>
<li><p><span class="bb">alias</span> : регистрирует новый класс и его псевдоним</p></li>
<li><p><span class="bb">singleton</span> : создает одиночный объект нового класса</p></li>
<li><p><span class="bb">alternateClassName</span> : определяет альтернативные имена для нового класса</p></li>
<li><p><span class="bb">uses</span> : импортирует классы, которые будут использоваться вместе с новым классом</p></li>
</ul>
<p>Мы можем сами вывести все возможные обработчики в вывод консоли, применив следующие выражения:</p>
<pre class="brush:js;">
