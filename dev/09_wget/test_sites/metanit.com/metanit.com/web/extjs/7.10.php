...
<html >
<head>
<title>Выбор дат. Ext.form.field.Date в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Использование класса Ext.form.field.Date для выбора дат, устанвка диапазонов и отключение возможности выбора даты">
...
<h2>Выбор дат. Ext.form.field.Date</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Для работы с датами в ExtJs предназначен класс <b>Ext.form.field.Date</b>. Он позволяет задать поле выбора даты:</p>
<pre class="brush:js;">
...
<img src="pics/extcalendar.png" alt="Выбор даты в ExtJs 4" />

<p>Свойство <code>format</code> задает формат даты. По умолчанию используется формат <i>m/d/Y</i>. Благодаря заданию формата при выборе даты 21 ноября 2012 
у нас в поле появится не 11/21/2012, как принято в США, а более удобная для нас форма 21/11/2012</p>
<p>Чтобы задать дату, мы используем объект <code>Date(год, месяц, день)</code>, причем отсчет месяцев начинается с 0, то есть февраль будет иметь номер 1.</p>
<p>Можно также отключить возможность выбора определенных дат, применяя свойство disabledDates:</p>
<pre class="brush:js;">
Ext.onReady(function(){
	var dateField=Ext.create('Ext.form.field.Date', {
		fieldLabel: 'Выбрать дату',
...
<p>С помощью свойств <code>maxValue</code> и <code>minValue</code> задается макcимально и минимально возможные даты для выбора. Если их не задать, нам 
будут доступны все даты.</p>
<p>Мы также можем отключить возможность выбора определенных дней недели. Например, чтобы отключить воскресенья и вторники, нам надо написать: 
<code>disabledDates: [0,2]</code>, так как воскресенье считается нулевым днем недели.</p>
<p>Чтобы задать дату, которая будет выбрана по умолчанию при загрузке страницы, можно использовать метод setValue: <code>dateField.setValue('21/12/2012');</code></p>
<h3>Альтернативные форматы</h3>
<p>Возможно, пользователи будут склонны задавать дату в том формате, какой им больше нравятся, а не в том, какой мы определили. В 
этом случае можно указать набор альтернативных форматов, которые, при вводе, будут автоматически изменяться на главный определенный нами формат:</p>
<pre class="brush:js;">
Ext.onReady(function(){
    var dateField=Ext.create('Ext.form.field.Date', {
        fieldLabel: 'Выбрать дату',
...
<p>Альтернативные форматы задаются с помощью свойства <code>altFormats</code> и перечисляются через вертикальную черту.</p>
