...
<html >
<head>
<title>Singleton в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Использование параметра singleton для создания только одной копии объекта класса">
...
<h2>Singleton</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>С помощью параметра <code>singleton</code> мы можем применить паттерн синглтон - то есть создание всего одного 
экземпляра объекта. Для этого нам надо всего лишь установить соответствующий параметр в <code>true</code>:</p>
<p>Например, применим параметр сиглтон к следующему классу:</p>
<pre class="brush:js;">
...
<p>Теперь попробуем использовать его</p>
<pre class="brush:js;">
...
<p>Если вызов <code>Classes.Person.getinfo();</code> сработает нормально, то следующее объявление объекта завершится с ошибкой, о чем 
можно будет увидеть в консоли веб-браузера. Так как у нас применяется параметр синглтон.</p>
<p>Несмотря на то, что мы не можем создавать объектов, такие классы-сиглтоны довольно удобны в использовании, например, для хранения константных 
значений.</p>
