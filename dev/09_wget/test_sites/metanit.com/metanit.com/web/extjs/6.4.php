...
<html >
<head>
<title>Компоновка HBoxLayout в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Использование компоновки HBoxLayout для создания горизонтально расположенных в ряд компонентов Ext JS">
...
<h2>HBoxLayout</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Компоновка <code>HBoxLayout</code> во многом похожа на вертикальный стек из прошлого раздела, только располагает элементы горизонтально, 
а вместо свойства height мы определяем свойство width:</p>
<pre class="brush:js;">
...
<p>Как и в прошлом разделе мы задаем у контейнера свойство <code>layout</code>, которое имеет похожие параметры.</p>
<h3>Параметр align</h3>
<p>Теперь этот параметр определяет вертикальное выравнивание дочерних элементов. Для данной компоновки этот параметр может принимать следующие значения:</p>
<ul>
<li><p><span class="bb">stretch</span> - растягивает элементы по вертикали до границ контейнера.</p></li>
<li><p><span class="bb">top</span> - значение по умолчанию, элементы прижимаются к верхней границе контейнера</p></li>
<li><p><span class="bb">middle</span> - элементы располагаются в центре контейнера</p></li>
<li><p><span class="bb">stretchmax</span> - позволяет растянуть все элементы до высоты самого высокого элемента, что позволяет нам указать высоту только у 
одного элемента:</p>
<pre class="brush:js;">
Ext.onReady(function(){
		Ext.create('Ext.panel.Panel', {
...
<h3>Параметр flex и гибкие размеры</h3>
<p>Здесь flex работает аналогично компоновке Vbox, только теперь автоматически устанавливается не высота, а ширина элемента, то есть параметр flex 
используется вместо параметра <code>width</code></p>
<h3>Параметр pack</h3>
<p>Параметр pack определяет выравнивание по ширине, но работает он, если не задан параметр flex ни в одном из элементов. Он может принимать следующие 
значения:</p>
<ul>
<li><p><span class="bb">start</span> - прижимает элементы к левой границе контейнера и используется по умолчанию</p></li>
<li><p><span class="bb">center</span> - элементы располагаются в центре контейнера</p></li>
<li><p><span class="bb">end</span> - элементы прижимаются к правой границе контейнера:</p></li>
</ul>
