...
<html >
<head>
<title>Компоновка ColumnLayout в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Использование компоновки ColumnLayout создания таблицы столбцов, где каждый дочерний элемент представлен в виде столбца. А также создание фреймов в приложении.">
...
<h2>ColumnLayout</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Компоновка <code>ColumnLayout</code> используется для создания таблицы столбцов - каждый дочерний элемент представляется в виде столбца:</p>
<pre class="brush:js;">
...
<p>Чтобы использовать данную компоновку, мы указываем для панели-контейнера значение <code>layout:'column'</code>. Затем чтобы внутренние панели отображались, 
...
<code>width: 200</code>. Второй способ подразумевает гибкие размеры: <code>columnWidth:.4</code>. Здесь для свойства columnWidth указывается ширина в 40 %. 
...
вычитается фиксированная ширина первой панели (200 пикселей) и затем умножается на значение columnWidth 0.4, что в итоге будет составлять 120 пикселей.</p>
