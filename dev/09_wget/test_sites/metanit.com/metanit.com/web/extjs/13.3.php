...
<html >
<head>
<title>Рендеринг столбцов в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Изменение формата отображения столбцов элемента Ext.grid.Panel с помощью параметра renderer и принимаемой им в качестве значения функции">
...
<h2>Рендеринг столбцов</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Кроме параметра <code>xtype</code> столбцы позволяют дополнительно настраивать отображение с помощью параметра <code>renderer</code> 
или рендер. Рендеры представляют собой функции, которые вызываются для каждого столбца со следующим набором параметров:</p>
<ul>
<li><p><span class="bb">Column index</span> : индекс текущего столбца</p></li>
<li><p><span class="bb">Metadata</span> : метаданные текущей ячейки или строки</p></li>
<li><p><span class="bb">Record</span> : запись текущей строки</p></li>
<li><p><span class="bb">Row index</span> : индекс текущей строки</p></li>
<li><p><span class="bb">Store</span> : хранилище грида</p></li>
<li><p><span class="bb">Value</span> : значение текущей ячейки</p></li>
<li><p><span class="bb">View</span> : текущее представление грида</p></li>
</ul>
<p>Посмотрим на примере и в качестве отправной точки возьмем пример из предыдущего раздела. Там создавали в гриде ряд столбцов. 
Теперь изменим объявление грида следующим образом (модель и хранилище остаются неизменными):</p>
<pre class="brush:js;">
...
<p>В моем случае я получу следующее отображение:</p>
<img src="pics/13.3.png" alt="Типы столбцов в ExtJS 4" />
<p>Итак, что изменилось? Во-первых, был добавлен еще один столбец, который будет отображать аватар пользователя. 
Предположим, что за каждым пользователем у нас закреплен соответствующий аватар, название которого формируется по принципу userId+".png". 
Тогда мы можем отобразить их в гриде следующим образом:</p>
<pre class="brush:js;">
{
    header: 'Аватар',
    dataIndex: 'userID',
...
<p>В функцию рендерера передается значение v. Так как у нас идет привязка к свойству <code>dataIndex: 'userID'</code>, то данное значение будет представлять 
значение этого свойства. Оператор return возвращает разметку для данной ячейки в виде тега <code>img</code>. И при наличии соответствующих картинок 
все они будут отображены в гриде.</p>
<p>Также мы изменили еще один столбец:</p>
<pre class="brush:js;">
		{
            text:'Женат/Замужем',
            xtype:'booleancolumn',
...
<p>Здесь мы сначала устанавливаем вес шрифта, используя в тернарной операции переданное значение <code>var fontweight = v ? 'normal' : 'bold';</code>: 
если v имеет значение true - то шрифт обычной толщины, иначе шрифт жирный. Строка <code>m.style='font-weight:'+fontweight;</code> уже непосредственно устанавливает 
вес шрифта для метаданных текущей ячейки.</p>
<p>Еще одна строка <code>v=v? 'Да' : 'Нет';</code> нужна, чтобы установить значения, а если мы ее уберем, то рендер по умолчанию 
будет устанавливать значения true и false.</p>
