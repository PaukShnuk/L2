...
<html >
<head>
<title>Создание меню в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Создание меню в ExtJS с помощью компонента Ext.menu.Menu, прикрепление меню в контейнере и наполение меню элементами">
...
<h2>Создание меню</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Для создания меню в ExtJS предназначен класс <b>Ext.menu.Menu</b>:</p>
<pre class="brush:js;">
...
<img src="pics/7.6.png" alt="Меню в ExtJS" />
<p>Указывая параметр <code>floating: false</code> мы фиксируем меню. А параметр <code>items</code> принимает массив объектов 
<b>Ext.menu.Item</b>, который используется в качестве пунктов меню. Причем в качестве объектов мы можем задавать разные компоненты, например, кнопки. 
В данном случае я указываю только свойство <code>text</code>.</p>
<p>Но также мы можем задать меню переключателей, указав группу и отмеченный вариант:</p>
<pre class="brush:js;">
Ext.onReady(function(){	
	var menu = Ext.create('Ext.menu.Menu', {
		width: 100,
...
<img src="pics/7.7.png" alt="Меню с переключателями в ExtJS" />
<p>Используя различные свойства <code>Ext.menu.Item</code>, можно применить различные свойства. Например, задать обработчики выбора</p>
<pre class="brush:js;">
Ext.onReady(function(){	
	var menu = Ext.create('Ext.menu.Menu', {
		width: 100,
...
<p>Поскольку само по себе меню мало полезно, поэтому посмотрим на его применение в контейнерах на примере панели:</p>
<pre class="brush:js;">
Ext.onReady(function(){	
	var menu = Ext.create('Ext.menu.Menu', {
		items: [{
...
<img src="pics/7.8.png" alt="Меню в панели в ExtJS" />
<p>С помощью псевдонима <code>xtype: 'splitbutton'</code> мы указываем явным образом, что будет создаваться кнопка с меню, а именно объект класса Ext.button.Split. 
Это по сути обычная кнопка, отличающаяся только тем, что для нее можно задать меню.</p>
...
<div class="nav"><p><a href="7.16.php">Назад</a><a href="./">Содержание</a><a href="8.php">Вперед</a></p></div></div>
<div class="socBlock">
