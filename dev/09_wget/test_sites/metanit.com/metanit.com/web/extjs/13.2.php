...
<html >
<head>
<title>Работа со столбцами Ext.grid.Panel в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Настройка столбцов элемента Ext.grid.Panel с помощью параметра xtype, типы столбцов, создание шаблонов для вывода данных">
...
<h2>Работа со столбцами Ext.grid.Panel</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Кроме <code>header</code> и <code>dataIndex</code> столбцы элемента Ext.grid.Panel имеют ряд свойств, которые позволяют настраивать 
отображение элемента.</p>
<p>Так, свойство <code>xtype</code> позволяет явным образом задать формат отображения данных для данного столбца. Среди возможных типов столбцов 
мы можем задать следующие:</p>
<ul>
<li><p>Ext.grid.column.Template (xtype: 'templatecolumn') : используя свойство <code>tpl</code> мы можем задать шаблон для отображения 
данных в столбце</p></li>
<li><p>Ext.grid.column.Date (xtype: 'datecolumn') : определяет столбец ля отображения дат, с помощью свойства <code>format</code> можно задать формат 
отображения даты</p></li>
<li><p>Ext.grid.column.Number (xtype:'numbercolumn') : определяет числовой тип столбца</p></li>
<li><p>Ext.grid.column.Boolean (xtype:'booleancolumn') : определяет булевый тип столбца и позволяет с помощью свойств <code>trueText</code> 
и <code>falseText</code> можно задать текст для отображения при значениях true и false соответственно</p></li>
<li><p>Ext.grid.column.Action (xtype:'actioncolumn') : этот тип столбца позволяет задать одну иконку или даже серию иконок, к которым можно прикрепить 
обработчики нажатия</p></li>
<li><p>Ext.grid.RowNumberer (xtype:'rownumberer') : с помощью этого типа можно определить первый столбец в виде числовых идентификаторов строк.</p></li>
</ul>
<p>Итак, продолжим работу с моделью и хранилищем из предыдущего параграфа. Наши изменения будут касаться только компонента Ext.grid.Panel. 
Обновим его следующим образом:</p>
<pre class="brush:js;">
...
<p>Здесь мы применили большую часть тех типов столбцов, о которых выше шла речь. Обратите внимание на определение шаблона во втором столбце: 
<code>tpl:'&lt;b&gt;{name} {surname} &lt;/b&gt;&lt;/br&gt;{email}'</code></p>
<p>Также обратите внимание на определение последнего столбца: <code>xtype:'actioncolumn</code>. С помощью свойства <code>items</code> мы 
определяем массив иконок, к которым можно прикрепить обработчики. В данном случае у нас определена одна иконка:</p>
<pre class="brush:js;">
{
	icon:'del.png',
	handler:function () {
...
<img src="pics/13.2.png" alt="Типы столбцов в ExtJS 4" />
