...
<html >
<head>
<title>Получение данных XML в ExtJS 4</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка на сервер асинхронных запросов с помощью функиции Ext.Ajax.request и получение ответа в виде xml, декодирование и обработка полученных xml-данных">
...
<h2>Получение данных XML в ExtJS 4</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Аналогично мы могли бы загрузить с помощью ajax-запроса xml-файл. Допустим, у нас на сервере есть файл <i>xmlData.xml</i>:</p>
<pre class="brush:xml;">
&lt;languages&gt;
	&lt;language&gt;
		&lt;title&gt;C&lt;/title&gt;
...
<p>Теперь в качестве ресурса url будет выступать файл xmlData.xml. Получить ответ с сервера в этом случае мы можем также с помощью выражения 
<code>response.statusText</code>, однако, как и в случае с json, у нас будут недекодированные ланные в виде строки. Поэтому используем свойство <code>responseXML</code>.</p>
<p>Свойство responseXML получает переданные xml-данные, по которым мы потом можем выполнить перебор и найти нужные нам элементы. 
Для доступа к элементам xml-файла мы можем использовать методы класса <strong>Ext.DomQuery</strong>. Например, получим 
текстовое содержимое первого элемента language в нашем файле xml:</p>
<pre class="brush:js;">
//..........................................
...
<p>Для получения данных из xml-узлов мы можем использовать следующие методы класса Ext.DomQuery:</p>
<ul>
<li><p><code>select(node,root)</code>: получает массив узлов, описанных в параметре node и определенных в родительском узле root</p></li>
<li><p><code>selectNode(node,root)</code>: получает первый узел по селектору node в родительском узле root</p></li>
<li><p><code>selectValue(node,root)</code>: получает значение узла node, определенного в родительском узле root</p></li>
</ul>
<p>Например, получить значения первого языка программирования из нашего файла xml мы можем таким способом:</p>
<pre class="brush:js;">
Ext.Ajax.request({
		url: 'xmlData.xml',
		success: function(response, options){
...
<p>Теперь используя методы Ext.DomQuery, выведем данные из xml-файла в виде таблицы на панель:</p>
<pre class="brush:js;">
...
<img src="pics/9.3.png"  alt='Получение и декодирование данных xml с сервера в ExtJS' />
...
<div class="nav"><p><a href="9.1.php">Назад</a><a href="./">Содержание</a><a href="10.php">Вперед</a></p></div></div>
<div class="socBlock">
