...
<html >
<head>
<title>Загрузка и сохранение моделей с помощью прокси в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Загрузка и сохранение моделей с помощью прокси, взаимодействие с внешними веб-ресурсами на примере PHP через ajax-запросы">
...
<h2>Загрузка и сохранение моделей с помощью прокси</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Одним из преимуществ использования моделей в ExtJS4 является поддержка объекта proxy (прокси), который позволяет взаимодействовать с 
сервером - загружать с него данные или, наоборот, отправлять обратно данные. Рассмотрим, как мы можем осуществлять взаимодействие модели с сервером через объект proxy.</p>
<p>Итак, продолжим работу с нашей моделью, определенной в прошлом параграфе, внеся в нее небольшие изменения:</p>
<pre class="brush:js;">
Ext.define('Person', {
...
<p>Здесь мы произвели по сравнению с моделью из прошлого параграфа два изменения. Во-первых, мы использовали параметр модели <code>idProperty</code>. 
...
значение.</p>
<p>Во-вторых, мы использовали свойство <code>proxy</code>, которое определено у модели. Параметр прокси позволяет обращаться к веб-ресурсам через 
ajax-запросы. Так, параметр <code>type: 'ajax'</code> указываетмодели, какой тип прокси использовать. В данном случае будет создаваться 
объект класса <code>Ext.data.proxy.Ajax</code> для взаимодействия с веб-ресурсами.</p>
<p>Задаваемый для прокси параметр <code>api</code> позволяет задать url-адреса для обработки запросов. Мы могли бы задать один универсальный адрес 
...
- создания объекта, чтения, обновления и удаления - используется определенный веб-ресурс.</p>
<p>К примеру файл <i>read.php</i>, который будет возвращать пользователю определенный объект, будет выглядеть таким образом:</p>
<pre class="brush:php;">
&lt;?php
...
<p>В данном случае скрипт просто создает один объект Person и возвращает его клиенту в формате json. В реальной ситуации это могла быть, например, выборка из бд mysql. 
После этого мы можем обратиться к этому ресурсу за получением нужного объекта по нужному нам id:</p>
<pre class="brush:js;">
Person.load(1, {
           callback: function(p, operation){
			alert(p.get('Name')); // вывод его отдельного свойства
...
<p>Обратите внимание, что функция Person.load будет обращаться к read.php, так как этот путь указан для чтения.</p>
<p>Для обновления некоторого объекта нам надо использовать его ID и затем свойство save - обращение будет идти к ресурсу update.php:</p>
<pre class="brush:js;">
...
<p>Однако если бы мы создавали объект без использования свойства PersonID, то обращение бы шло к ресурсу create.php:</p>
<pre class="brush:js;">
var person = Ext.create('Person', {
		Name: 'Eugene',
		Surname: 'Popov',
...
<p>И наконец метод удаления объекта:</p>
<pre class="brush:js;">
var person = Ext.create('Person', {
		PersonID: 1,
		Name: 'Eugene',
...
<p>Для удаления объекта мы также посылаем обект в POST-запросе на сервер, только в данном случае обращение идет к ресурсу delete.php.</p>
<p>Таким образом, используя свойства proxy у модели мы можем создать CRUD-интерфейс для работы с данными.</p>
...
<div class="nav"><p><a href="12.4.php">Назад</a><a href="./">Содержание</a><a href="12.3.php">Вперед</a></p></div></div>
<div class="socBlock">
