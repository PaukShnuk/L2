...
<html >
<head>
<title>Компоновка BorderLayout в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Описание компоновки BorderLayout, используемой для прикрепления компонентов к одной из четырех сторн контейнера">
...
<h2>BorderLayout</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>BorderLayout позволяет прикреплять элементы к одной из четырех сторон окна, либо закрепить его в центре:</p>
<pre class="brush:js;">
...
<img src="pics/3.9.png" alt="BorderLayout в ExtJS" />
<p>Значение <code>layout:'border'</code> устанавливает компоновку. Затем мы задаем панели, которые примыкают к определенным сторонам контейнера с 
помощью параметра <code>region</code>. Этот параметр может принимать следующие значения:</p>
<ul>
<li><p><span class="bb">center</span> - элемент размещается по центру</p></li>
<li><p><span class="bb">north</span> - элемент прижимается к верхней границе контейнера ("к северу")</p></li>
<li><p><span class="bb">south</span> - элемент прижимается к нижней границе контейнера</p></li>
<li><p><span class="bb">west</span> - элемент прижимается к левой стороне контейнера</p></li>
<li><p><span class="bb">east</span> - элемент прижимается к правой стороне контейнера</p></li>
</ul>
<p>В данном случае мы устанавливаем значения <code>region</code> для всех панелей, однако нам необязательно использовать панели для всех пяти сторон.</p>
<p>Подобная компоновка также интересна в плане дополнительных возможностей. Так, она позволяет использовать следующие параметры:</p>
<ul>
<li><p><strong>collapsed: true</strong> - означает, что панель будет скрыта при запуске</p></li>
<li><p><strong>collapsible: true</strong> - позволяет раскрыть/скрыть панель по нажатию на заголовок панели</p></li>
<li><p><strong>titleCollapse: true</strong> - скрывает панель</p></li>
<li><p><strong>split: true</strong> - позволяет раздвигать панель</p></li>
</ul>
<p>Используем новые возможности:</p>
<pre class="brush:js;">
Ext.onReady(function(){
	Ext.create('Ext.Panel', {
            width: 500,
