...
<html >
<head>
<title>Обработка событий элементов и компонентов в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Применение модели событий в Ext JS, использование обработчиков событий элементов и компонентов, делегация событий">
...
<h1>Обработка событий элементов и компонентов</h1><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Большое значение в Ext JS имеет модель событий. С помощью событий мы можем обрабатывать пользовательский ввод и контролировать ход выполнения приложения.</p>
<p>Для того, чтобы повесить обработчик события на элемент, нам надо использовать метод <code>on</code> класса <code>Ext.Element</code>, который является 
псевдонимом метода javascript <code>addListener</code>.</p>
<p>Итак, добавим для некоторого элемента <code>&lt;div id="content"&gt;Привет мир&lt;/div&gt;</code> обработчик события <code>click</code>:</p>
<pre class="brush:js;">
...
<p>Сначала функцией <code>Ext.get</code> мы получаем нужный элемент по id, потом в функции <code>element.on</code>. Первый аргумент этой функции - 
само событие, которое мы прослушиваем. Второй аргумент - функция обработчика этого события. В данном случае мы используем анонимную функцию. 
Третий параметр обозначает область действия обработчика (в данном случае это окно браузера).</p>
<p>Мы также можем передать в функцию <code>on</code> обработчик сразу для нескольких событий элемента:</p>
<pre class="brush:js;">
Ext.onReady(function(){
	var element = Ext.get('content');
	element.on({
...
<p>Чтобы добавить для элемента обработчики сразу для нескольких событий, мы создаем анонимный объект и внутри него объявляем пары ключ-значение, 
ключом является событие, а значением - функция обработчика этого события. Последний параметр - <code>scope</code> - указывает на область видимости.</p>
<h3>Обработчики событий компонентов</h3>
<p>Для добавления обработчиков событий у компонентов можно использовать параметр <b>listeners</b>, который, как и функция <code>on</code>, 
принимает в качестве анонимного объекта набор пар - событие и функция его обработчика:</p>
<pre class="brush:js;">
Ext.onReady(function(){
...
<p>Здесь мы создаем кнопку и добавляем ей обработчик события <code>click</code>. По нажатию на кнопку выводим сообщение.</p>
<h3>Делегация событий</h3>
<p>Делегация событий позволяет вешать обработчик события на родительский элемент, при этом события будут обрабатываться на дочерних элементах, 
для которых они были делегированы. Эта техника очень действенна, так как позволяет вместо прикрепления обработчиков к каждому отдельному элементу, 
обрабатывать события сразу нескольких элементов.</p>
<p>Определим к примеру следующий код разметки html:</p>
<pre class="brush:xml;">
&lt;div id="content" &gt;
	&lt;div class="text"&gt;Текст1&lt;/div&gt;
	&lt;div&gt;Текст2&lt;/div&gt;
	&lt;div class="text"&gt;Текст3&lt;/div&gt;
	&lt;div&gt;Текст4&lt;/div&gt;
&lt;/div&gt;
</pre>
<p>Добавим для элементов с классом <code>text</code> обработчик события click:</p>
<pre class="brush:js;">
Ext.onReady(function(){
	var element = Ext.get('content');
	element.on('click', function(e, target, options){
...
<p>С помощью параметра <strong>delegate</strong> мы прикрепляем обработчик события click к тем элементам, у которых <code>class="text"</code>. 
Используя переданный в качестве аргумента в обработчик параметр <code>target</code> получаем элемент, на котором произведено нажатие.</p>
<p>Технически при делегировании событий происходит проверка элемента, на котором произведено событие. Так, мы могли бы опустить параметр delegate и 
с помощью метода <code>getTarget</code> объекта <code>Ext.EventObject</code> проверять объект события:</p>
<pre class="brush:js;">
Ext.onReady(function(){
	var element = Ext.get('content');
	element.on('click', function(e, target, options){
...
<p>Комбинируя оба подхода, можно создать возможность выбора хода программы в зависимости от того, для какого элемента возникло событие. Например, 
определим на веб-странице простое меню:</p>
<pre class="brush:xml;">
&lt;ul id="menu"&gt;
...
<p>Теперь в зависимости от того, какой пункт меню был нажат, будем производить те или иные действия:</p>
<pre class="brush:js;">
Ext.onReady(function(){
...
<p>С помощью метода <code>getTarget</code> выясняем, над какой пункт меню нажали выводим определенное сообщение.</p>
...
<div class="nav"><p><a href="3.php">Назад</a><a href="./">Содержание</a><a href="5.php">Вперед</a></p></div></div>
<div class="socBlock">
