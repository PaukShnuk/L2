...
<html >
<head>
<title>Связь нескольких моделей в ExtJS 4</title>
<meta charset="utf-8" />
<meta name="description" content="Создание ассоциации нескольких моделей Ext JS друг с другом на примере связей one-to-many, many-to-one, one-to-one">
...
<h2>Связь нескольких моделей</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Ext JS 4 предоставляет возможность связать или ассоциировать несколько моделей друг с другом. Например, одна книга может иметь двух авторов, 
реализуя связь один-ко-многим (one-to-many).</p>
<p>Фреймворк Ext JS 4 три подобных отношения или типа связей:</p>
<ul>
<li><p>Один ко многим (One-to-many) (тип Ext.data.HasManyAssociation)</p></li>
<li><p>Многие к одному (Many-to-one) (тип Ext.data.BelongsToAssociation)</p></li>
<li><p>Один к одному (Has-one) (тип Ext.data.association.HasOne)</p></li>
</ul>
<p>Рассмотрим эти взаимосвязи. Допустим, у нас есть модель Company, представляющая компанию, у которой может быть несколько основателей. К примеру, 
компания Microsoft, которую основали Билл Гейтс и Полл Аллен. Теперь добавим соответствующие модели Company и Manager и соединим их 
связью один ко многим (one-to-many):</p>
<pre class="brush:js;">
...
<p>Теперь разберем код. Вначале мы создаем две модели Company и Manager и добавляем к ним определения полей. В отличии от тех определений моделей, с которыми 
мы имели дело в прошлых параграфах, в данном случае в модель company добавляется определение ассоциации с другими моделями:</p>
<pre class="brush:js;">
associations: [{
		type: 'hasMany',
		model: 'Manager',
...
<p>В данном случае, так как мы используем связь один-ко-многим, то используем конфигурацию объекта <b>Ext.data.HasManyAssociation</b>. Для этого 
указывается параметр type со значением <b>hasMany</b>. Кроме того, данный параметр в зависимости от выбранного типа ассоциации может принимать 
значения <b>hasOne</b> и <b>belongsTo</b>. </p>
<p>Параметр <code>model</code> принимает название модели, с которой ассоциируется текущая модель.</p>
<p>Параметр <code>name</code> принимает имя функции, которая создает дочернее хранилище. Таким образом, в данном случае при вызове функция 
<code>founders</code> будет возвращать хранилище, которое будет содержать связанные с текущим объектом модели Company данные.</p>
<p>Затем у хранилища, создаваемого функцией <code>company.founders()</code>, мы можем вызвать метод <code>add</code>, который добавляет новые объекты 
типа Manager  к текущему объекту Company.</p>
<h3>Ассоциация многие к одному </h3>
<p>Данный тип ассоциации действует подобным образом:</p>
<pre class="brush:js;">
Ext.onReady(function(){
...
<p>Данный тип связи похож на предыдущую, только в модели Manager мы указываем ассоциацию с моделью Company (<code>belongsTo: 'Company'</code>). 
...
Так, в данном случае это методы <code>getCompany</code> и <code>setCompany</code>, поскольку у нас связь идет к модели Company.</p>
 <p>В итоге ассоциированные данные загружаются после вызова метода <code>manager.setCompany</code>. А получить мы их можем с помощью метода 
 <code>manager.getCompany</code>.</p>
 
 <h3>Ассоциация один к одному </h3>
 <p>Последний тип ассоциации - один к одному:</p>
 <pre class="brush:js;">
Ext.onReady(function(){
	Ext.define('Company', {
		extend: 'Ext.data.Model',
...
<p>Сам код ассоциации, как и в случае с первой ассоциацией, включается в блоке</p>
 <pre class="brush:js;">
associations: [{
            type: 'hasOne',
            model: 'Manager',
...
<p>Также, как и в случае с предыдущей ассоциацией, используя метод setManager, мы устанавливаем для определенной компании менеджера. 
А используя метод getManager, мы получаем связанный с текущим объектом Company объект Manager.</p>
...
<div class="nav"><p><a href="12.5.php">Назад</a><a href="./">Содержание</a><a href="12.2.php">Вперед</a></p></div></div>
<div class="socBlock">
