...
<html >
<head>
<title>Работа с данными в ExtJS 4</title>
<meta charset="utf-8" />
<meta name="description" content="Использование данных в ExtJS 4, применение класса Ext.data.Model для создания моделей и обработка загружаемых данных">
...
<h1>Работа с данными в ExtJS 4</h1>
<h2>Создание моделей</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Для более удобной работы с данными в ExtJS 4 имеется класс <code>Ext.data.Model</code>, который представляет отдельную сущность в приложении или объект, обладающий 
определенными свойствами. Для хранения наборов подобных объектов используются хранилища данных. Итак, код нашего примера будет следующим:</p>
<pre class="brush:js;">
...
<p>Теперь разберем его по порядку. Прежде всего нам необходимо создать класс модели. Этот класс в принципе идентичен обычному классу Ext JS. 
И с помощью метода <code>Ext.define('Person', {
		extend: 'Ext.data.Model'....</code> мы расширяем функциональность класса <code>Ext.data.Model</code>, образуя новый класс Person. 
Определение полей новой модели находится в параметре <code>fields</code>. Этот параметр принимает массив объектов типа <b>Ext.data.Field</b>. 
С помощью свойства <code>type</code> определяется тип свойства, а с помощью свойства <code>name</code> - его имя.</p>
<p>Обратите внимание, что при указании свойства с типом <code>date</code> определяется формат даты, так как он в последствии используется методом 
<code>Ext.Date.parse</code> для надлежащего парсинга даты, особенно в тех случаях, когда она указана в нестандартных форматах.</p>
<p>Далее мы создаем объект данной модели с помощью метода <code>create</code>, заполняя его объектом javascript, который содержит набор пар ключ-значение, где в качестве ключа выступает имя свойства модели:</p>
<pre class="brush:js;">
var person = Ext.create('Person', {
		Name: 'Eugene',
		Surname: 'Popov',
...
<p>И уже при выводе значения данной модели на панель мы используем метод <code>get</code> у определенного нами объекта. Передавая в качестве параметра имя 
свойства, мы можем получить его значение (<code>person.get('Name')</code>)</p>
<p>Подобным образом мы можем динамически изменить значение свойства объекта с помощью метода set: <code>person.set('Name', 'Alexander');</code> или 
<code>person.set('Married', true);</code></p>

<h4>Предобработка модели</h4>
<p>Иногда возникает необходимость перед загрузкой обработать выводимые данные. Для этого в качестве параметра у поля модели мы можем определить 
параметр <code>convert</code>. Этот параметр принимает функцию, возвращающую значение данного свойства модели. Так, выше у нас в качестве дохода 
(свойство Salary) выводилось простое число. Теперь изменим это свойство:</p>
<pre class="brush:js;">
...........................
		{
			name: 'Salary',
...
<p>Функция, передаваемая в параметр <code>convert</code>, принимает два аргумента: v - передаваемое значение, record - текущий объект модели. 
Метод record.get('Name') получает имя объекта и затем выполняется конкатенация строк. В итоге мы получим при выводе строку "Eugene получает 300 рублей".</p>
<h3>Модели и формы</h3>
<p>В предыдущем примере мы использовали метод get для получения данных и вывода их на панель. Однако если мы работаем с формами, то нам необязательно 
получать значение каждого свойства таким же образом. Мы просто можем связать форму с моделью:</p>
<pre class="brush:js;">
Ext.onReady(function(){
    Ext.define('Person', {
        extend: 'Ext.data.Model',
...
<p>После нажатия на кнопку сброс у нас в поля формы будут загружены данные объекта модели person благодаря использованию 
метода <code>formPanel.getForm().loadRecord(person)</code>, который в качестве параметра принимает объект загружаемой модели.</p>
...
<div class="nav"><p><a href="11.5.php">Назад</a><a href="./">Содержание</a><a href="12.5.php">Вперед</a></p></div></div>
<div class="socBlock">
