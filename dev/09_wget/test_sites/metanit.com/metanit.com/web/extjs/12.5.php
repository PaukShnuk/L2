...
<html >
<head>
<title>Валидация полей модели в ExtJS</title>
<meta charset="utf-8" />
<meta name="description" content="Валидация полей модели в Ext JS 4, использование класса Ext.data.validations и создание собственных валидаторов для полей модели">
...
<h2>Валидация полей модели</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>При работе с объектами модели нередко бывает важно, чтобы они соответствовали определенным требованиям или ограничениям. Проверку на 
соответствие модели подобным ограничениям осуществляется с помощью валидации.</p>
<p>Ext JS 4 позволяет применять правила валидации напрямую к полям модели. А это значит, что мы можем избежать дублирования кода, упростить и 
оптимизировать приложение в целом. Создадим некоторую модель и определим в ней правила валидации:</p>
<pre class="brush:js;">
Ext.define('User', {
...
<P>Итак, мы добавили валидаторы полей модели в виде свойства <code>validations</code>. Это свойство содержит массив объектов-валидаторов. 
Каждый валидатор имеет поле <code>type</code>, которое указывает на тип валидатора, и поле <code>field</code>, которое указывает, к какому полю 
относится валидатор. Кроме того, используются дополнительные атрибуты.</p> 
<p>Затем массив валидаторов обрабатывается и сопоставляется с методами и свойствами, содержащимися в классе-синглтоне <code>Ext.data.validations</code>.</p>
<p>Фреймворк имеет всего шесть встроенных типов валидаторов:</P>
<ul>
<li><p><span class="bb">length</span>: проверяет длину поля, которая указывается в параметрах <code>min</code> и <code>max</code>, которые указывают 
соответственно на минимальную и максимальную длину значения поля.</p></li>
<li><p><span class="bb">presence</span>: проверяет на наличие значения у поля модели.</p></li>
<li><p><span class="bb">inclusion</span>: проверяет значение и возвращает true, если данное значение имеется в списке - в параметре list</p></li>
<li><p><span class="bb">exclusion</span>: проверяет значение и возвращает true, если данное значение отсутствует в списке - в параметре list</p></li>
<li><p><span class="bb">format</span>: проверяет, соответствует ли значение поля регулярному выражению в параметре <code>matcher</code></p></li>
<li><p><span class="bb">email</span>: проверяет, является ли действительным адрес электронной почты</p></li>
</ul>
<p>Теперь проверим валидаторы в действии - создадим объект данной модели и залоггируем в консоли вывод функции <code>validate</code> 
(указав в поле name заведомо невалидное значение - из одной буквы, хотя у нас минимальная длина должна составлять 2 символа):</p>
<pre class="brush:js;">
...
<p>В этом случае в консольном выводе мы увидим ошибку валидации:</p>
<pre>
....items:[{field:"name", message:"is wrong length"}],....
</pre>
<p>Это сообщение будет говорить о том, что данный объект модели невалидный, так как указанное поле имеет неправильную длину.</p>
<p>Хотя метод <code>validate</code> позволяет нам определить ошибки при валидации, но также мы можем применить метод <code>isValid()</code>, 
который сигнализирует о том, пройдена ли валидация:</p>
<pre class="brush:js;">
	if(person.isValid())
	{
...
<p>Таким образом, если валидация не пройдены, то мы можем вызвать метод validate для определения ошибок, а затем вывести информацию о каждой ошибке.</p>
<h3>Создание собственного валидатора</h3>
<p>Имеющиеся встроенные валидаторы покрывают большой объем случаев, но они все таки не всеобъемлющи. Иногда 
возникает необходимость в собственном валидаторе, заточенном под конкретные нужды. Например. в примере выше использовалось 
простое написание телефона. А что если телефон должен иметь определенное количество цифр, разделенных в некоторых местах дефисами. 
И в данном случае мы можем создать свой собственный валидатор.</p>
<p>Для создания нового валидатора, нам надо добавить для класса <code>Ext.data.validations</code> новые свойства и методы, 
которые будут выполнять функцию валидации:</p>
<pre class="brush:js;">
...
<p>В самом начали мы определили, во-первых, сообщение об ошибке - <code>Ext.data.validations.phoneMessage</code>, а во-вторых, 
функцию валидации. Функция валидации смотрит на формат строки и в зависимости от введенных данных возвращает true или false. 
 Будущий тип валидации должен будет иметь то же самое имя, что и функция валидации.</p>
<p>Теперь при создании и последующей валидации следующего объекта:</p>
<pre class="brush:js;">
var person = Ext.create('User', {
		name: 'E',
		age: 28,
...
<div class="nav"><p><a href="12.1.php">Назад</a><a href="./">Содержание</a><a href="12.4.php">Вперед</a></p></div></div>
<div class="socBlock">
