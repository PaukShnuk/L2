...
<html >
<head>
<title>Первое приложение на Ext JS 4</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и развертывание первого приложения на Ext JS">
...
<h2>Создание первого приложения</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Теперь чтобы непосредственно начать работать с Ext JS 4 создадим первое приложение. Оно будет очень простым и его единственная цель по сути будет 
состоять в выводе строки на экран. Итак, создадим следующий файл <code>index.html</code>, который у нас будет находится на веб-сервер 
в соседней папке по отношению к папке фреймворка extjs:</p>
<pre class="brush:xml;">
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае у нас по сути мало что делается: сначала мы подключаем стили ext js (<i>ext-all.css</i>), 
затем файл фреймворка (<i>ext-all-dev.js</i> или <i>ext-all.js</i>). И в последнюю очередь - сам файл приложения - <i>app.js</i>. Собственно в нем и будет происходить 
вся нехитрая магия.</p>
<p>Теперь создадим файл приложения <i>app.js</i>, который будет содержать следующий код javascript:</p>
<pre class="brush:js;">
Ext.application({
    name: 'HelloExt',
    launch: function() {
...
<p>То есть теперь приложение будет иметь следующую структуру (в моем случае папка приложения называется helloext, а в качестве примера 
используется сервер IIS):</p>
<img src="pics/1.1.png" />
<p>Теперь мы можем посмотреть приложение в действии, запустив в браузере нашу страницу <i>index.html</i>:</p>
<img src="pics/extjs_firstapp.png" />
<p>В будущем мы более подробно рассмотрим классы и виджеты Ext JS, а пока вкратце посмотрим по коду, что делает данное примитивное приложение. 
Итак, вначале мы инициализируем новое приложение вызывая функцию <code>Ext.application</code>. Мы указываем имя приложения (helloext) и 
те действия, которые будут производиться при его запуске.</p>
<p>В частности с помощью функции <code>Ext.create</code> создается новый объект класса <code>Ext.container.Viewport</code>, для чего загружается 
файл <code>Viewport.js</code>. И затем мы наполняем созданный контейнер элементами с помощью массива items и устанавливаем его расположение на 
странице.</p>
<p>Итак, мы познакомились вкратце с Ext JS 4. А теперь приступим к более подробному и детальному рассмотрению данного фреймворка.</p>
