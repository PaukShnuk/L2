...
<html >
<head>
<title>Специальные методы для обработки событий</title>
<meta charset="utf-8" />
<meta name="description" content="Специальные методы для обработки событий в jquery, такие как click, keydown, mouseenter и т.д. и примеры их использования">
...
<h2>Специальные методы для обработки событий</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Кроме методов bind/delegate/on jQuery предоставляет прямые методы для обработки событий. Эти методы, как правило, носят наименование обрабатываемого события, 
а в качестве параметра принимают функцию обработчика данного события:</p>
<ul>
<li><p><code>blur</code>: событие потери фокуса элементом</p></li>
<li><p><code>change</code>: событие изменение значение элемента</p></li>
<li><p><code>click</code>: событие нажатия</p></li>
<li><p><code>dblclick</code>: двойное нажатие</p></li>
<li><p><code>focus</code>: получение фокуса элементом</p></li>
<li><p><code>focusin</code>: получение фокуса элементом или одним из его дочерних элементов</p></li>
<li><p><code>focusout</code>: потеря фокуса элементом или одним из его дочерних элементов</p></li>
<li><p><code>hover</code>: наведение указателя мыши на элемент и выход за пределы элемента</p></li>
<li><p><code>keydown</code>: событие нажатия клавиши клавиатуры</p></li>
<li><p><code>keyup</code>: отжатие клавиши</p></li>
<li><p><code>mousedown</code>: нажатие мыши на элемент</p></li>
<li><p><code>mouseup</code>: отжатие кнопки мыши</p></li>
<li><p><code>mouseenter</code>: наведение указателя мыши на элемент</p></li>
<li><p><code>mouseleave</code>: выход указателя мыши за границы элемента</p></li>
<li><p><code>mousemove</code>: перемещение указателя мыши в пределах элемента</p></li>
<li><p><code>mouseout</code>: выход указателя мыши за границы элемента</p></li>
<li><p><code>mouseover</code>: наведение указателя мыши на элемент</p></li>
<li><p><code>ready</code>: загрузка структуры DOM элемента</p></li>
<li><p><code>resize</code>: измение размеров окна браузера</p></li>
<li><p><code>scroll</code>: событие прокрутки элемента</p></li>
<li><p><code>select</code>: выделение текста</p></li>
<li><p><code>submit</code>: отправка формы</p></li>
</ul>
<p>Принцип действий всех вышеперечисленных методов одинаков: если мы вызываем метод без параметра, то тем самым генерируем событие; 
а если передаем в качестве параметра функцию - то тем самым мы устанавливаем обработчик для данного события. Например:</p>
<pre class="brush:xml;">
...
<p>В данном случае для двух кнопок мы добавляем обработчики события <code>click</code>, то есть нажатия. Обработчик первой кнопки просто 
устанавливает цвет нажатой кнопки (которую мы получаем с помощью выражения <code>$(this)</code>).</p>
<p>Для второй кнопки также устанавливается обработчик в виде функции, передаваемой в метод <code>click</code>. Только теперь обработчик 
инициирует событие нажатия для первой кнопки: <code>$('button').first().click();</code>.</p>
<p>Таким образом, в зависимости от задачи и предпочтений мы можем использовать либо методы bind/delegate/on для регистрации обработчиков событий, либо 
вышеприведенные методы типа <code>click</code> или <code>mouseenter</code>.</p>
...
<div class="nav"><p><a href="5.5.php">Назад</a><a href="./">Содержание</a><a href="6.1.php">Вперед</a></p></div></div>
<div class="socBlock">
