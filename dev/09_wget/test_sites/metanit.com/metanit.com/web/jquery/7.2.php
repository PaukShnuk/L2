...
<html >
<head>
<title>Эффекты скольжения в jQuery</title>
<meta charset="utf-8" />
<meta name="description" content="Эффекты скольжения в jQuery, их применение для изменения высоты элементов, реализация с помощью методов slideUp, slideDown, slideToggle и примеры использования">
...
<h2>Эффекты скольжения</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Эффекты скольжения позволяют нам плавно скрыть или раскрыть элемент. Эффекты скольжения реализованы в виде методов <code>slideUp()</code>, 
<code>slideDown()</code> и <code>slideToggle()</code>.</p>
<p>Если метод slideUp скрывает элемент в направлении вверх, как бы скользя, то метод slideDown плавно раскрывает скрытый элемент в направлении вниз. 
Метод slideToggle комбинирует действие обоих методов: если элемент скрыт, но раскрывается, если раскрыт - скрывается.</p>
<p>Эти методы имеют одинаковые формы использования:</p>
<ul>
<li><p><code>slideUp/slideDown/slideToggle()</code>: метод без параметров</p></li>
<li><p><code>slideUp/slideDown/slideToggle([duration] [, easing][, complete])</code>. Параметр <code>duration</code> указывает 
как долго сокрытие элемента будет длиться. По умолчанию его значение равно 400 миллисекунд.</p>
...
<p>Параметр <code>complete</code> представляет функцию, вызываемую методом после завершения анимации.</p></li>
</ul>
<p>Допустим, пусть у нас будут две кнопки. По нажатию на первую кнопку она будет медленно скрываться, а по нажатию на вторую кнопку скрытая 
первая кнопка будет раскрываться:</p>
<pre class="brush:xml;">
...
<p>Подобным образом можно применить метод slideToggle:</p>
<pre class="brush:js;">
$(function() {
	$('#slideUp').click(function(){
