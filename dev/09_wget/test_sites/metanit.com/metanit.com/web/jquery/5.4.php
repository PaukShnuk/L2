...
<html >
<head>
<title>Методы delegate и on в jQuery</title>
<meta charset="utf-8" />
<meta name="description" content="Методы delegate и on для регистрации обработчиков событий jquery и отмена регистрации с помощью методов undelegate и off">
...
<h2>Методы delegate и on</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<h3>Метод delegate</h3>
<p>Ранее я упоминал, что метод <code>bind</code> добавляет обработчики событий только к тем элементам, которые на момент его действия уже существуют 
в структуре DOM. При добавлении новых элементов, например, с помощью метода <code>append</code> их события не будут обрабатываться.</p>
<p>Но jQuery предоставляет специальный метод <code>delegate</code>, который позволяет создать зарегистрировать обработчики и для будущих элементов, 
которых еще не в структуре DOM. Этот метод имеет следующие варианты использования:</p>
<ul>
<li><p><code>delegate('селектор','событие', обработчик_события)</code>: добавляет обработчик для события. Для фильтрации элементов, для которых добавляется обработчик, 
используется селектор.</p></li>
<li><p><code>delegate('селектор','событие', данные_события, обработчик_события)</code>: то же самое, что и в предыдущем случае, плюс параметр для передачи данных события в объект Event (e.data)</p></li>
</ul>
<p>Применение метода delegate:</p>
<pre class="brush:xml;">
&lt;button&gt;Жми1&lt;/button&gt;
&lt;button&gt;Жми2&lt;/button&gt;
...
<p>Здесь для всех элементов <code>button</code>, находящихся в элементе <code>body</code>, добавляется обработчик нажатия. И несмотря на то, что уже после этого 
добавляется новая кнопка, для нее также будет срабатывать обработчик.</p>
<p>Также мы можем отменить регистрацию обработчика события с помощью метода <code>undelegate</code>:</p>
<pre class="brush:js;">
$('body').undelegate('button','click');
</pre>
<h3>Метод on</h3>
<p>Метод <code>on</code> был введен в jQuery 1.7 и во многом является заменой методу delegate и имеет аналогичное действие. Он имеет похожие формы использования:</p>
<ul>
<li><p><code>on('событие', 'селектор', обработчик_события)</code></p></li>
<li><p><code>on('событие', 'селектор', данные_события, обработчик_события)</code></p></li>
</ul>
<p>По сути по сравнению с методом delegate в данном случае меняются лишь параметры селектора и события:</p>
<pre class="brush:js;">
...
<p>Для отмены регистрации обработчиков события можно использовать метод <code>off</code>:</p>
<pre class="brush:js;">
