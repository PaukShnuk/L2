...
<html >
<head>
<title>POST-запросы. Метод post</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка POST-запросов в jquery с помощью метода post, его параметры и примеры использования, отправка данных формы">
...
<h2>POST-запросы. Метод post</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>В отличие от GET-запросов данные POST-запросов передаются не в строке запроса, а в его теле. Распространенным примеров подобных запросов 
является отправка данных формы на сервер.</p>
<p>Для отправки POST-запросов предназначен метод <code>post</code>. Его объявление и использование в целом аналогично методу get. Он принимает 
...
<li><p><code>success(data, textStatus, jqXHR)</code>: необязательный параметр - функция обратного вызова, которая будет выполняться при 
успешном выполнении запроса. Она может принимать три параметра: <code>data</code> - данные, полученные с сервера, <code>textStatus</code> - 
- статус запроса и <code>jqXHR</code> - специальный объект jQuery, который представляет расширенный вариант объекта <b>XMLHttpRequest</b>.</p></li>
<li><p><code>dataType</code>: необязательный параметр, содержащий тип данных в виде строки, например, "xml" или "json"</p></li>
</ul>
<p>На выходе метод post возвращает объект <code>jqXHR</code>.</p>
...
<p>В данном случае мы передаем в качестве данных пароль и логин. На сервере мы можем получить данные и отправить ответ пользователю:</p>
<pre class="brush:php;">
&lt;?php
	$login=$_POST['login'];
	$pass=$_POST['password'];
...
<p>Поскольку наиболее часто запрос post используется при отправке данных формы, используем на стороне клиента форму:</p>
<pre class="brush:xml;">
&lt;html&gt;
	&lt;head&gt;
		&lt;meta charset='utf-8'&gt;
...
<p>Итак, серверная часть, к которой будет обращаться форма - файл ajax.php - у нас остается той же. Только в данном случае теперь 
для параметра <code>data</code>  в методе post мы данные берем из полей на этой форме.</p>
<p>Обратите внимание, что мы блокируем обычную отправку формы (<code>event.preventDefault();</code>), иначе у нас бы шла переадресация.</p>
<h3>Сериализация формы</h3>
<p>Посколкьу нередко формы не ограничиваются двумя полями, то проще применять сериализацию формы. Сериализация производится посредством метода 
<code>serialize</code> и в качестве результата создает объект javascript, где свойствам соответствуют поля формы. И значения хранят эти свойства те же, 
что имеют соответственные поля формы.</p>
<p>Итак, применим сеиализацию формы:</p>
<pre class="brush:xml;">
		 &lt;form action="ajax.php" id="loginForm"&gt;
...
<p>В отличие от предыдущего примера у нас здесь два отличия. Во-первых, обратите внимание, что поля для ввода имеют атрибут <code>name</code>. При указании параметра <code>data</code> мы производим 
сериализацию данных формы через метод <code>serialize</code>: <code>$("#loginForm").serialize()</code>. В данном методе в тело запроса 
передаются параметры. Причем имена параметров - это значения атрибутов name полей ввода. А значения параметров - соответственно введенные значения в текстовые поля.</p>
<p>И поэтому с помощью php мы можем извлечь эти значения: <code>$login=$_POST['login']</code>.</p>
