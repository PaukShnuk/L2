...
<html >
<head>
<title>Использование классов в jQuery</title>
<meta charset="utf-8" />
<meta name="description" content="Использование классов в jquery, добавление классов через метод addClass, удаление класса, проверка наличия класса с помощью метода hasClass и переключение классов с помощью метода toggleClass">
...
<h2>Использование классов в jQuery</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Для добавления к элементу класса применяется метод <code>addClass('названия_классов')</code>, который в качестве параметра принимает 
...
строку, содержащую несколько классов, разделенных пробелами:</p>
<pre class="brush:js;">
...
<p>Для удаления класса используется метод <code>removeClass('названия_классов')</code>. Данный метод также принимает набор имен классов, 
разделенных пробелами. И если у элемента имеется такой класс, то он удаляется:</p>
<pre class="brush:js;">
$('div').first().removeClass("black visible");
</pre>
<h3>Проверка классов. Метод hasClass</h3>
<p>Нередко бывает нужно проверить, имеет ли тот или иной элемент какой-нибудь класс, особенно это бывает необходимо, если классы добавляются динамически. 
В этом случае нам может помочь метод <code>hasClass('названия_классов')</code>. И если содержит класс, то данный метод возвращает true, иначе false.</p>
<pre class="brush:js;">
...
<h3>Переключение классов</h3>
<p>Переключение классов осуществляется с помощью метода <code>toggleClass('название_класса')</code>. В качестве параметра принимается название 
класса, который будет присвоен элементу. Например, сделаем переключение класса у кнопки по ее нажатию:</p>
<pre class="brush:xml;">
&lt;style&gt;
.redStyle {color:red;}
&lt;/style&gt;
...
<p>Здесь мы на кнопку вешаем обработчик события <code>click</code>. Сам обработчик передается в виде функции в качестве параметра. 
В этой функции переключаем класс redStyle с помощью метода <code>toggleClass("redStyle")</code> - если класс есть, он удаляется, а если его нет, то он добавляется.</p>
