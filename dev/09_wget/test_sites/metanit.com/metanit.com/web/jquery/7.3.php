...
<html >
<head>
<title>Эффекты прозрачности в jQuery</title>
<meta charset="utf-8" />
<meta name="description" content="Эффекты прозрачности в jQuery, их применение для изменения прозрачности элементов, реализация с помощью методов fadeIn, fadeOut, fadeTo, fadeToggle и примеры использования">
...
<h2>Эффекты прозрачности</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Эффекты прозрачности позволяют нам, плавно изменяя прозрачность элемента, скрыть его или отобразить. Эффекты прозрачности реализованы 
с помощью методов <code>fadeOut()</code>, <code>fadeIn()</code>, <code>fadeTo()</code> и <code>fadeToggle()</code>.</p>
<table class="tab">
<tr class="tabhead"><td><p>Метод</p></td><td><p>Описание</p></td></tr>
<tr><td><p>fadeOut</p></td><td><p>Скрывает элемент, уменьшая его прозрачность</p></td></tr>
<tr><td><p>fadeIn</p></td><td><p>Отображает элемент, увеличивая его прозрачность</p></td></tr>
<tr><td><p>fadeToggle</p></td><td><p>Сочетает методы fadeOut и fadeIn: если прозрачность равна нулю, то элемент отображается. Если элемент непрозрачен, 
то он скрывается</p></td></tr>
<tr><td><p>fadeTo</p></td><td><p>Изменение прозрачности до указанного уровня</p></td></tr>
</table>
<p>Методы <code>fadeOut()</code>, <code>fadeIn()</code> и <code>fadeToggle()</code> имеют похожие формы использования:</p>
<ul>
<li><p><code>fadeOut/fadeIn/fadeToggle()</code>: метод без параметров</p></li>
<li><p><code>fadeOut/fadeIn/fadeToggle([duration] [, easing][, complete])</code>. Параметр <code>duration</code> указывает 
как долго изменение прозрачности элемента будет длиться. По умолчанию его значение равно 400 миллисекунд.</p>
<p>Параметр <code>easing</code>, который принимает название функции плавности анимации в виде строки. По умолчанию его значение равно 
"swing". Также можно использовать значения <code>'slow'</code> и <code>'fast'</code>, которые соответствуют длительности эффекта в 600 и 200 миллисекунд.</p>
<p>Параметр <code>complete</code> представляет функцию обратного вызова, вызываемую методом по завершении анимации</p></li>
</ul>
<p>Метод <code>fadeTo</code> в отличие от других методов еще принимает и параметр opacity - оно принимает значение, до которого надо изменить прозрачность элемента: 
<code>fadeTo(duration, opacity [, easing][, complete])</code>. В качестве значение opacity принимается значение от 0 (полностью прозрачный) до 
1 (полностью видимый).</p>

<p>Допустим, пусть у нас на странице будет изображение и две кнопки, которые будут изменять прозрачность этого изображения:</p>
<pre class="brush:xml;">
...
<p>Обратите внимание, что метод fadeIn увеличивает прозрачность до того значения, которое было до использования метода fadeOut, а не обязательно до 1. То есть 
в данном случае, поскольку в начале был применен метод fadeTo и прозрачность изменена до 0.6, то и метод fadeIn будет увеличивать прозрачность до 0.6.</p>
