...
<html >
<head>
<title>Манипуляция элементами в jQuery</title>
<meta charset="utf-8" />
<meta name="description" content="Атрибуты и свойства элементов в jquery, их изменение и удаление, работа с пользовательскими атрибутами html5">
...
<h1>Манипуляция элементами в jQuery</h1><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>В предыдущей главе мы узнали, как получить элементы, используя селекторы и различные методы, как детализировать выборку, оставив в ней 
...
Поэтому посмотрим, какие манипуляции с элементами мы можем производить с помощью jQuery.</p>
<h2>Атрибуты и свойства элементов</h2>
<p>Библиотека jQuery предлагает нам инструментарий для манипуляции свойствами и атрибутами элементов. Но прежде чем переходить к непосредственным методам 
jQuery, надо отметить, чем отличаются свойства и атрибуты.</p>
<p>Атрибуты - это элементы разметки элементов, такие как id, style, class и ряд других. Свойства же представляют элементы объектов javascript.</p>
<p>Несмотря на такое различие, между свойствами и атрибутами имеет сопоставление. Так, атрибут id будет соответствовать свойству id. Например, у нас есть ссылка:</p>
<pre class="brush:xml;">
...
<p>То есть на консоль выведет значение атрибута id. В то же время не все для всех атрибутов есть одноименные классы. Так, атрибуту <code>class</code> 
соответствует свойство <code>className</code>.</p>
<h3>Изменение свойств</h3>
<p>Для работы со свойствами в jQuery имеется метод <code>prop()</code>. Чтобы получить значения свойства, нам надо передать в этот метод имя свойства. 
Например, так мы можем получить все адреса ссылок на странице:</p>
<pre class="brush:js;">
	$('a').each(function(index,elem){
		console.log($(elem).prop('href'));
	});
</pre>
<p>Чтобы изменить значение свойства, надо просто передать в качестве второго параметра новое значение:</p>
<pre class="brush:js;">
	$('a').first().prop('href','33.html');
</pre>
<p>После этого первая ссылка на странице будет иметь в качестве адреса страницу 33.html.</p>
<h3>Удаление свойств</h3>
<p>Для удаления свойств мы можем использовать метод <code>removeProp('имя_свойства')</code>:</p>
<pre class="brush:js;">
	$('a').first().removeProp('href');
</pre>
<p>После этого свойству будет присвоено новое значение: <code>undefined</code>, которое будет указывать, свойство не определено.</p>
<h3>Работа с атрибутами</h3>
<p>Работа с атрибутами во многом похожа на работу со свойствами. Так, чтобы получить значение атрибута элемента, нам надо использовать 
метод <code>attr('имя_атрибута')</code>:</p>
<pre class="brush:js;">
	$('a').each(function(index,elem){
		console.log($(elem).attr('href'));
	});
</pre>
<p>Действие данного метода аналогично тому, что приводилось для метода prop: вывод на консоль всех адресов ссылок.</p>
<p>А передавая в качестве второго параметра в метод attr некоторое значение, можно установить новое значение атрибута:</p>
<pre class="brush:js;">
	$('a').first().attr('href','33.html');
</pre>
<p>И также для удаления значений атрибутов мы модем использовать метод <code>removeAttr('имя_атрибута')</code>:</p>
<pre class="brush:js;">
$('a').first().removeAttr('href');
</pre>
<p>Обратите внимание, если в случае с удалением свойства через метод removeProp соответствующий свойству атрибут оставался, только ему присваивалось 
значение свойства, то есть undefined, то при удалении атрибута через removeAttr атрибут удаляется из разметки элемента.</p>
<h3>Пользовательские атрибуты html5</h3>
<p>В HTML5 была введена такая функциональность, как пользовательские атрибуты. Их суть в том, что мы можем применить к элементу дополнительные атрибуты, 
которые будут хранить некоторое дополнительное значение. Подобные атрибуты начинаются с префикса <i>data-</i>, после которого идет собственно название атрибута и его значение. 
Например, в следующем примере я добавляю атрибут data-year, который будет хранить год:</p>
<pre class="brush:xml;">
&lt;ul data-year="2010"&gt;
	&lt;li&gt;Java&lt;/li&gt;
	&lt;li&gt;C/C++&lt;/li&gt;
...
<p>Для работы с подобными атрибутами используется метод <code>data</code>. Он имеет следующие варианты использования:</p>
<ul>
<li><p><code>data('имя атрибута')</code>: получает значение атрибута первого элемента набора. В качестве параметра передается имя атрибута без префикса <i>data-</i></p>
<p>Так, мы можем получить значение атрибута из предыдущего примера следующим образом:</p>
<pre class="brush:js;">console.log($('ul').data('year'));</pre></li>
<li><p><code>data()</code>: возвращает объект javascript, который содержит набор атрибутов и их значений в виде пар ключ-значение.</p>
<p>Например, пусть у нас будет элемент с двумя атрибутами:</p>
<pre class="brush:xml;">&lt;ul data-year="2010" data-description="lang"&gt;</pre>
<p>Тогда метод <code>data()</code> вернул бы объект <code>{description:'lang', year:'2010'}</code>. И чтобы получить значение отдельного атрибута, можно написать так:</p>
<pre class="brush:js;">console.log($('ul').data().year);</pre></li>
<li><p><code>data('атрибут', 'новое значение')</code>: устанавливает в качестве значения атрибута строку, переданную в качестве второго параметра:</p>
<pre class="brush:js;">$('ul').first().data('year', '2012');</pre>
<p>В качестве нового значения может выступать и целых объект javascript, а данный атрибут будет содержать в качестве значения весь этот объект:</p>
<pre class="brush:js;">$('ul').first().data('lang', { rate: "tiobe", year: 2012 });</pre></li>
</ul>
<p>Если же мы захотим удалить атрибут, то для этого нужно применить метод <code>removeData('название_атрибута')</code>:</p>
<pre class="brush:js;">$('ul').removeData('year');</pre>
