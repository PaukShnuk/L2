...
<meta charset="utf-8" />
<meta name="description" content="Введение в библиотеку jQuery UI, ее загрузка, подключение и использование ">
...
<h1>jQuery UI</h1>
<h2>Введение в jQuery UI</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Фреймворк jQuery позволяет не только управлять элементами DOM, отправлять запросы AJAX, но представляет неплохой инструмент для построения 
пользовательского интерфейса с помощью виджетов. Для работы с интерфейсом предназначена специальная библиотека jQuery UI.</p>
<p>Библиотека jQuery UI загружается отдельно от основной библиотеки jQuery: ее можно найти по адресу <a href="https://jqueryui.com/download/" rel="nofollow">https://jqueryui.com/download/</a>. 
По этому адресу внизу страницы вы можете увидеть список тем библиотеки. Темы влияют на оформление применяемых компонентов. Но в начале работы выбор темы не столь 
важен, поэтому можно оставить стандартную тему UI lightness. Также на странице загрузок можно детализировать загрузку - что будет входить 
в загружаемый пакет, но можно оставить все установки по умолчанию и внизу страницы нажать кнопку Download для загрузки.</p>
<p>Также можно использовать сети CDN, поскольку библиотека довольно популярная, то она есть во многих CDN.</p>
<p>Распакуем пакет. В нем будут несколько папок и файлов в состав которых включены все опции, что мы выбрали на странице загрузки jQueryUI.</p>
<p>А также файл <i>index.html</i>, содержащий примеры использования виджетов и логики из библиотеки jQueryUI. Если мы откроем данный файл 
в текстовом редакторе, то увидим, что он ссылается на выбранную нами тему, библиотеку jQuery и библиотеку jQuery UI. В начале файла будет подключен 
файл стилей jquery-ui:</p>
<pre class="brush:xml;">&lt;link href="jquery-ui.css" rel="stylesheet"&gt;</pre>
<p>А ближе к концу файла будет поключена библиотека jQuery и библиотека jQuery UI:</p>

<pre class="brush:xml;">
&lt;script src="external/jquery/jquery.js"&gt;&lt;/script&gt;
&lt;script src="jquery-ui.js"&gt;&lt;/script&gt;
</pre>
<p>Как правило, чтобы воспользоваться возможностями jquery, нам нужно будет подключить эти три файла на нашу html-страницу. 
Их можно подключать как и из пакета, скачанного с официального сайта, так и из сетей CDN</p>
<p>Подключим библиотеку сначала из загруженного пакета. Для этого в распакованном пакете создадим следующую страницу html:</p>
<pre class="brush:xml;">
&lt;html&gt;
    &lt;head&gt;
...
<p>Здесь просто стандартный элемент ссылки заменяется на виджет button, и к нему добавляется обработчик нажатия. Сначала обязательно подключается основная библиотека jQuery, поскольку jQuery UI от нее зависит.</p>
<p>Также загрузим из CDN компании Microsft:</p>
<pre class="brush:xml;">
&lt;html&gt;
    &lt;head&gt;
...
<div class="nav"><p><a href="7.4.php">Назад</a><a href="./">Содержание</a><a href="8.2.php">Вперед</a></p></div></div>
<div class="socBlock">
