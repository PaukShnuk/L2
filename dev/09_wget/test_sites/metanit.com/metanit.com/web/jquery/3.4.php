...
<html >
<head>
<title>Содержимое элементов</title>
<meta charset="utf-8" />
<meta name="description" content="Содержимое элементов в jquery, работа с разметкой элементов и метод html, получение и установка текста с помощью метода text, работа с компонентами формы и метод val">
...
<h2>Содержимое элементов</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Библиотека jQuery представляет нам удобный инструментарий для работы с содержимым элемента в виде методов <code>html</code> и <code>text</code>.</p>
<h3>Работа с разметкой</h3>
<p>Для получения или установки разметки используется метод <code>html</code>, который имеет следующие варианты использования:</p>
<ul>
<li><p><code>html()</code>: получает разметку html первого элемента в наборе</p></li>
<li><p><code>html('новая_разметка')</code>: устанавливает в качестве разметки элемента код html, переданный в качестве параметра</p></li>
<li><p><code>html(функция)</code>: установка разметки с помощью функции</p></li>
</ul>
<p>Получим и изменим разметку элемента:</p>
<pre class="brush:js;">
...
<p>Новая разметка полностью затирает старую. Если же мы хотим просто добавить кусок разметки к старой, то мы просто можем присоединить новую разметку к старой:</p>
<pre class="brush:js;">
var oldHtml = $('div.header').html();
$('div.header').html(oldHtml+"&lt;p&gt; Новый параграф&lt;/p&gt;")
</pre>
<p>Также можно использовать функцию, которая пробегает по всем элементам выборки и в качестве параметров принимает текущий индекс 
элемента в выборке и его текущую разметку. С помощью функции мы можем переписать предыдущий пример следующим образом:</p>
<pre class="brush:js;">
...
<h3>Работа с текстом элемента</h3>
<p>Для работы с тексом используется метод <code>text()</code>, действие которого во многом похоже на действие метода html, только вместо разметки мы 
управляем текстом элемента:</p>
<ul>
<li><p><code>text()</code>: получает текст первого элемента в наборе</p></li>
<li><p><code>text('новый_текст')</code>: устанавливает в качестве текста элемента строку, переданную в качестве параметра</p></li>
<li><p><code>text(функция)</code>: установка текста с помощью функции</p></li>
</ul>
<pre class="brush:js;">
...
<p>Однако метод <code>text</code> следует использовать с осторожностью. Если элемент имеет внутреннюю разметку, то она полностью замещается новым 
текстовым содержимым. Поэтому в случае, если надо изменить текст без изменения разметки, то лучше применять другие методы.</p>
<h3>Работа с элементами форм</h3>
<p>Для получения значений элементов форм используется метод <code>val()</code>. По принципу действия он похож на методы html() и text(), 
только используется для установки значений компонентов форм.</p>
<ul>
<li><p><code>val()</code>: получает значение первого элемента в наборе</p></li>
<li><p><code>val('новое_значение')</code>: устанавливает в качестве значения элемента строку, переданную в качестве параметра</p></li>
<li><p><code>val(функция)</code>: установка значения с помощью функции</p></li>
</ul>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="3.3.php">Назад</a><a href="./">Содержание</a><a href="4.1.php">Вперед</a></p></div></div>
<div class="socBlock">
