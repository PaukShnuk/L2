...
<html >
<head>
<title>Работа с выборкой</title>
<meta charset="utf-8" />
<meta name="description" content="Размер выборки, ее перебор с помощью метода each и как массив javascript, получение элемента выборки по индексу, добавление элементов в набор через метод add">
...
<h2>Работа с выборкой</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Применение селекторов и фильтров возвращает нам набор элементов, к которому мы можем применить различные свойства и методы. Также поскольку 
набор элементов представляет собой массив, то мы можем обращаться с ним как с массивом.</p>
<h3>Количество выбранных элементов. Свойство length и метод size</h3>
<p>Свойство <code>length</code> возвращает нам количество выбранных элементов: <code>var num = $("tr:nth-child(odd)").length;</code></p>
<p>Альтернативой является использование метода <code>size</code>, который также возвращает число выбранных элементов: <code>var num = $("tr:nth-child(odd)").size();</code></p>
<h3>Получение элемента</h3>
<p>Поскольку возвращаемый в процессе выборки набор данных фактически является массивом, то мы можем получать элементы выборки по индексу: 
<code>var firstElement = $("tr:nth-child(odd)")[0];</code>. Например:</p>
<pre class="brush:js;">
$(function(){
	var array = $("tr:even");
...
<p>Альтернативой является использование метода <code>get(index)</code>, который в качестве параметра принимает индекс элемента: <code>var elem = $("tr:even").get(0);</code>.</p>
<h3>Функция eq</h3>
<p>Еще одним способом обратиться к элементу является функция <code>eq(index)</code>. Это функция в качестве параметра получает индекс элемента и возвращает 
из новый набор, состоящий из одного элемента по данному индексу. Например:</p>
<pre class="brush:js;">
$(function(){
	var array = $("tr:even");
...
<p>Преимуществом такого подхода является то, что мы можем для результата функции eq использовать методы jQuery (например, <code>css</code> для установки стиля, как в данном случае).</p>
<p>Кроме того, мы можем передать в функцию отрицательное значение. В этом случае отсчет будет производиться с конца.</p>
<h3>Получение первого и последнего элемента</h3>
<p>Поскольку разработчикам довольно часто приходилось в своих задачах получать последний и особенно первый элемент, то разработчики jQuery 
добавили специальные методы для их выборки: <code>last</code> и <code>first</code> соответственно.</p>
<pre class="brush:js;">
$(function(){
	var array = $("tr:even");
...
<h3>Перебор элементов выборки</h3>
<p>Кроме перебора в виде обычного массива в цикле, как в вышеприведенном варианте, мы можем использовать специальный метод <code>each</code>:</p>
<pre class="brush:js;">
$(function(){
...
<p>В метод <code>each</code> в качестве параметра передается безымянная функция, которая принимает два параметра: <code>index</code> - индекс элемента в наборе и 
<code>elem</code> - сам элемент.</p>
<h3>Индекс элемента</h3>
<p>Для определения индекса элемента в jQuery предназначен метод <code>index(элемент)</code>. В качестве параметра мы передаем элемент, индекс 
которого мы хотим определить:</p>
<pre class="brush:js;">
$(function(){
	var array = $("tr:even");
	var firstEl = array.first();
...
<p>Также метод index может принимать в качестве параметра результат выборки функции jQuery:</p>
<pre class="brush:js;">
$(function(){
	var array = $("tr");
...
<h3>Получение селектора</h3>
<p>С помощью свойства <code>selector</code> мы можем получить селектор выборки:</p>
<pre class="brush:js;">
$(function(){
	var array = $("tr:even");
...
<h3>Метод add и добавление новых элементов</h3>
<p>С помощью метода <code>add</code> мы можем создать новый набор. Данный метод имеет следующие варианты синтаксиса:</p>
<ul>
<li><p><code>add(селектор)</code>: добавление элементов, соответствующих селектору, в набор</p></li>
<li><p><code>add(селектор, контекст)</code>: добавление элементов, но в данном случае поиск элементов ведется в рамках контекста</p></li>
<li><p><code>add(элемент)</code>: добавление в набор элемента</p></li>
<li><p><code>add(массив элементов)</code>: добавление массива элементов</p></li>
<li><p><code>add(html)</code>: добавление в набор элемента, представленного разметкой html (при этом добавление идет только в набор, а не на страницу)</p></li>
<li><p><code>add(jQuery)</code>: добавление объекта, который является результатом функции jQuery</p></li>
</ul>
<p>Например, создадим начальную выборку из четных строк таблицы и добавим в нее первую строку из нечетных строк таблицы:</p>
<pre class="brush:js;">
$(function(){
...
<p>В то же время вместо метода <code>add</code> мы могли бы применить набор селекторов, который бы нам дал тот же результат.</p>
