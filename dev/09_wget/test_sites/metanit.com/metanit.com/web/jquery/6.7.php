...
<html >
<head>
<title>Подгрузка скриптов. Метод getScript</title>
<meta charset="utf-8" />
<meta name="description" content="Динамическая подгрузка и выполнение скриптов в jquery с помощью метода getScript, его параметры, примеры использования">
...
<h2>Подгрузка скриптов. Метод getScript</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Используя метод <code>getScript</code> мы можем дополнительно подгрузить скрипты на страницу. Этот метод принимает следующие параметры:</p>
<ul>
<li><p><code>url</code>: обязательный параметр, содержащий адрес скрипта, который будет подгружаться</p></li>
<li><p><code>success(script, textStatus, jqXHR)</code>: необязательный параметр, представляющий функцию обратного вызова, которая будет выполняться при 
успешном выполнении запроса.</p></li></ul>
<p>На выходе метод getScript возвращает объект <code>jqXHR</code>, связанный с текущем запросом.</p>
<p>Предположим на сервере у нас определен следующий простенький скрипт <i>redStyle.js</i>:</p>
<pre class="brush:js;">
$('body').css('background-color', 'red');
</pre>
<p>Скрипт просто окрашивает страницу в красный цвет. Теперь на странице определим его подгрузку, например, по нажатию кнопки:</p>
<pre class="brush:xml;">
...
<p>Теперь, после нажатия на кнопку, будет подгружаться и выполняться скрипт, который окрасит страницу в красный цвет.</p>
