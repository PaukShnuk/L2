...
<meta charset="utf-8" />
<meta name="description" content="Добавление в jquery элементов одного уровня с помощью методов before/after/insertBefore/insertAfter и примеры их использования">
...
<h2>Добавление сестринских элементов</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Если метод wrap имеет дело с родительскими элементами, а методы append/prepend - с дочерними, то для добавления элементов одного уровня или сестер(сиблингов) 
используются методы <code>before/after/insertBefore/insertAfter</code>.</p>
<h3>Метод before</h3>
<p>Метод <code>before</code> вставляет новый элемент перед каждым элементом выборки. Он имеет следующие способы использования:</p>
<ul>
<li><p><code>before('разметка html')</code>: добавляет в разметку перед каждым элементом выборки элемент, создаваемый из разметки html</p></li>
<li><p><code>before(элемент)</code>: добавляет в разметку перед каждым элементом выборки элемент html, переданный в качестве параметра</p></li>
<li><p><code>before(jQuery)</code>: добавляет в разметку перед каждым элементом выборки в объект jQuery</p></li>
<li><p><code>before(функция)</code>: добавляет в разметку перед каждым элементом выборки объект, возвращаемый функцией</p></li>
</ul>
<p>Допустим, у нас есть следующий список:</p>
<pre class="brush:xml;">
&lt;ul&gt;
...
<p>Используя метод before, мы можем добавить новый элемент списка в начало:</p>
<pre class="brush:js;">
...
<p>После добавления новые элементы окажутся в начале списка:</p>
<pre class="brush:xml;">
&lt;ul&gt;
...
<h3>Метод after</h3>
<p>Похожим образом работает метод <code>after</code>, за тем исключением, что добавление идет в конец списка.</p>
<ul>
<li><p><code>after('разметка html')</code>: добавляет после каждого элемента выборки элемент, создаваемый из разметки html</p></li>
<li><p><code>after(элемент)</code>: добавляет после каждого элемента выборки элемент html, переданный в качестве параметра</p></li>
<li><p><code>after(jQuery)</code>: добавляет после каждого элемента выборки в объект jQuery</p></li>
<li><p><code>after(функция)</code>: добавляет после каждого элемента выборки объект, возвращаемый функцией</p></li>
</ul>
<p>Возьмем предыдущий пример со списком и добавим в него пару элементов:</p>
<pre class="brush:js;">
	// добавление с помощью кода разметки
...
<h3>Метод insertBefore</h3>
<p>Метод <code>insertBefore</code> по своему действию похож на метод before, только в качестве параметра он принимает элемент, перед которым 
будет происходить добавление. Он имеет следующий синтаксис:</p>
<ul>
<li><p><code>insertBefore('разметка html')</code>: элемент добавляется перед элементом, создаваемым из разметки html</p></li>
<li><p><code>insertBefore(элемент)</code>: элемент добавляется перед элементом html, который передается в качестве параметра</p></li>
<li><p><code>insertBefore(jQuery)</code>: элемент добавляется перед объектом jQuery</p></li>
<li><p><code>insertBefore('селектор')</code>: элемент добавляется перед объектом, который соответствует селектору</p></li>
</ul>
<p>Фактически это тот же метод before, только здесь объект jQuery и элемент, относительно которого производится вставка, меняются местами:</p>
<pre class="brush:js;">
$('&lt;li class="lang"&gt;JavaScript&lt;/li&gt;').insertBefore('li.lang:first');
</pre>
<p>Здесь элемент <code>&lt;li class="lang"&gt;JavaScript&lt;/li&gt;</code> будет добавлен перед элементом, который соответствует селектору 
<code>li.lang:first</code></p>
<h3>Метод insertAfter</h3>
<p>Метод <code>insertAfter</code> похож на метод after, только, как и метод insertBefore, в качестве параметра он принимает элемент, после которого 
будет происходить добавление. Он имеет следующие варианты использования:</p>
<ul>
<li><p><code>insertAfter('разметка html')</code>: элемент добавляется после элемента, создаваемого из разметки html</p></li>
<li><p><code>insertAfter(элемент)</code>: элемент добавляется после элемента html, который передается в качестве параметра</p></li>
<li><p><code>insertAfter(jQuery)</code>: элемент добавляется после объекта jQuery</p></li>
<li><p><code>insertAfter('селектор')</code>: элемент добавляется после элемента, который соответствует селектору</p></li>
