...
<html >
<head>
<title>Получение данных JSON. Метод getJSON</title>
<meta charset="utf-8" />
<meta name="description" content="Получение данных JSON и их обработка в jquery с помощью метода getJSON, его параметры, примеры использования">
...
<h2>Получение данных JSON. Метод getJSON</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Ранее мы увидели, что мы можем использовать метод get для получения данных в формате json. Однако библиотека jQuery также предоставляет 
специальный метод <code>getJSON</code>. Этот метод отправляет на сервер GET-запрос и в ответ получает данные json.</p>
...
<li><p><code>success(data, textStatus, jqXHR)</code>: необязательный параметр, представляющий функцию обратного вызова, которая будет выполняться при 
успешном выполнении запроса.</p></li></ul>
<p>На выходе метод getJSON возвращает объект <code>jqXHR</code>, связанный с текущем запросом.</p>
<p>Допустим, на сервере у нас есть следующий файл <i>countries.json</i>, который содержит перечисление стран в формате json:</p>
<pre class="brush:js;">
{
	"Великобритания": 1,
	"Китай": 2,
...
<p>Добавим на страницу выпадающий список (элемент <code>select</code>) и загрузим в него данные из этого файла:</p>
<pre class="brush:xml;">
&lt;select id="countries"&gt;
...
<p>В качестве значения параметра <code>url</code> используется название файла json. Так как приходящие данные представляют собой набор, 
то мы можем пройтись по ним в цикле each и применить к ним функцию обработки.</p>
<p>Обрабатывающая функция вызывается для каждого элемента в наборе и принимает два параметра: название данных или ключ (key) и значение (val). 
То есть в строке <code>"Россия": 3</code> "Россия" - это ключ, а 3 - значение.</p>
<p>В данном случае у нас структура данных довольно проста. Теперь посмотрим на использование данных с более сложной структурой. Например, у нас определен 
такой файл <i>users.json</i>:</p>
<pre class="brush:js;">
{
	"users": [{
		"id" : 1,
...
<p>Попробуем вывести его в табличку на странице:</p>
<pre class="brush:xml;">
&lt;table id="users"&gt;
...
<p>Используя названия свойств объектов json, мы можем получить к ним доступ (например, <code>data.users[i].age</code>) и тем самым вывести их значения 
на страницу.</p>
