...
<html >
<head>
<title>Виджет Accordion</title>
<meta charset="utf-8" />
<meta name="description" content="Виджет Accordion и элементов-аккордеонов в jQuery-UI, использование виджета, его настройка и стилизация, его методы и свойства">
...
<h2>Виджет Accordion</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Виджет <code>accordion</code> используется для создания аккордеонов:</p>
<pre class="brush:xml;">
...
<img src="pics/8.9.png" alt="Виджет accordion в jquery ui" />
<h3>Стилизация виджета</h3>
<p>На предыдущем рисунке, возможно, кому-то покажется, что шрифт надписей виджета слишком велик. Возможно, потребуется настроить какие-либо еще 
свойства. И мы можем выполнить настройку стилей, применив их к классу <code>ui-accordion</code>.</p>
<p>Аккордеон структурно подразделяется на заголовок и основную часть. И кроме стилей аккордеона в целом, мы также можем настроить по отдельности 
стили заголовка, применив их к классу <code>ui-accordion-header</code>, и стили основной части - их применяют к классу <code>ui-accordion-content</code>:</p>
<pre class="brush:xml;">
&lt;html&gt;
&lt;head&gt;
&lt;meta charset='utf-8'&gt;
...
<img src="pics/8.10.png" alt="Стилизация аккордеона в jquery ui" />
<h3>Свойства аккордеона</h3>
<p>Аккордеоны в jquery ui имеет следующие свойства:</p>
<ul>
<li><p><code>collapsible</code>: при установке этого свойства в <code>true</code> при нажатии на заголовок мы можем скрывать содержимое блока (<code>$('#accordion').accordion({collapsible:false});</code>). 
По умолчанию имеет значение <code>false</code>, поэтому скрыть один блок мы можем только нажав на заголовок другого блока</p></li>
<li><p><code>active</code>: указывает, какой блок в текущий момент является активным. Например, <code>$('#accordion').accordion({active:2});</code> - здесь при создании виджета будет раскрыта третья вкладка, 
так как нумерация начинается с 0. По умолчанию активной является первая вкладка.</p></li>
<li><p><code>animate</code>: данное свойство указывает, будет ли виджет анимироваться, а если будет, то как. Данное свойство может принимать ряд значений.</p>
<p>Если свойство имеет значение <code>false</code>, то анимация отключается. Это проявляется в мгновенном открытии новых блоков, а при <code>true</code> 
подобное открытие происходит плавно.</p>
<p>Также данное свойство может иметь числовое значение, которое устанавливает время анимации в миллисекундах.</p>
<p>Также значение может представлять строковое название функции плавности анимации, например, <code>linear</code>, <code>swing</code>, <code>easeInCubic</code> 
и другие функции. Например: <code>$('#accordion').accordion({animate:'easeInCubic'})</code>.</p>
<p>И также можно установить в качестве значения объект, включающий как название функции плавности, так и длительность анимации в миллисекундах: 
<code>$('#accordion').accordion({animate: {easing:'easeInCubic', duration: 800}});</code>.</p>
</li>
...
<li><p><code>event</code>: данное свойство в качестве значения принимает событие, при возникновении которого будет происходить переключение 
блоков в аккордеоне: <code>$('#accordion').accordion({event:'mouseover'})</code> - здесь переключение блоков будет осуществляться при наведении мыши 
(событие mouseover) на заголовок блока. По умолчанию переключение происходит по событию <code>click</code>.</p></li>
<li><p><code>icons</code>: используя это свойство, мы можем задать иконку, отображаемую рядом с заголовком блока. По умолчанию это свойство имеет значение 
<code>{"header": "ui-icon-triangle-1-e", "activeHeader": "ui-icon-triangle-1-s"}</code> - в данном случае опция <code>ui-icon-triangle-1-e</code> 
указывает иконку для всех заголовков блоков, а опция <code>activeHeader</code> - для активных.</p>
<p>Мы можем переопределить иконку: <code>$('#accordion').accordion({icons: {"header": "ui-icon-plus", "activeHeader": "ui-icon-minus"}})</code>. 
Полный список используемых иконок можно найти по адресу <a href="https://api.jqueryui.com/theming/icons/" rel="nofollow">https://api.jqueryui.com/theming/icons/</a></p></li>
</ul>
<h3>Методы аккордеонов</h3>
<p>Виджет аккордеон имеет следующие методы:</p>
<ul>
<li><p>accordion("disable"): делает аккордеон недоступным (<code>$("#accordion").accordion("disable");</code>)</p></li>
<li><p>accordion("enable"): делает аккордеон доступным для взаимодействия</p></li>
<li><p>accordion("destroy"): удаляет функциональность аккордеона у элемента</p></li>
<li><p>accordion("widget"): возвращает объект jQuery, представляющий виджет (<code>var widget = $("#accordion").accordion("widget");</code>)</p></li>
<li><p>accordion("option"): позволяет получить или установить значения свойств аккордеона. Например, установим и получим значение свойства collapsible:</p>
<pre class="brush:js;">
	$('#accordion').accordion();
	// установка
...
<h3>События аккордеонов</h3>
<p>Аккордеоны имеют следующие события:</p>
<ul>
<li><p><code>activate(event, ui)</code>: событие возникает после выбора блока в аккордеоне, то есть его активации.</p>
<p>Если параметр <code>event</code> содержит объект события, то параметр <code>ui</code> представляет объект, который содержит информацию 
об активированном блоке, а также о том блоке, который до этого был активным. Этот параметр имеет следующие свойства:</p>
<ul>
<li><p>newHeader: заголовок блока, который активирован</p></li>
<li><p>oldHeader: заголовок блока, который до этого был активным</p></li>
<li><p>newPanel: панель блока, который активирован</p></li>
<li><p>oldPanel: панель блока, который был активным до выбора текущего активного блока</p></li>
</ul>
<p>Например, получим заголовки текущего активного блока и блока, который ранее был активным:</p>
<pre class="brush:js;">
	$('#accordion').accordion({
...
<li><p><code>beforeActivate(event, ui)</code>: возникает непосредственно перед активацией блока аккордеона</p></li>
</ul>
...
<div class="nav"><p><a href="8.4.php">Назад</a><a href="./">Содержание</a><a href="8.6.php">Вперед</a></p></div></div>
<div class="socBlock">
