...
<meta charset="utf-8" />
<meta name="description" content="Удаление элементов в jquery с помощью методов remove, detach, empty и примеры их использования">
...
<h2>Удаление элементов</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<h3>Метод remove</h3>
<p>Метод <code>remove([селектор])</code> удаляет элементы из структуры DOM. Например, у нас есть следующий список:</p>
<pre class="brush:xml;">
&lt;ul&gt;
...
<p>Мы можем удалить все четные элементы списков:</p>
<pre class="brush:js;">
$('li:even').remove();
</pre>
<p>А результатом будет следующий список:</p>
<pre class="brush:xml;">
&lt;ul&gt;
	&lt;li class="lang"&gt;C/C++&lt;/li&gt;
	&lt;li class="lang"&gt;JavaScript&lt;/li&gt;
...
<p>Также мы могли бы применить селектор для фильтрации удаляемых элементов:</p>
<pre class="brush:js;">$('li').remove(':even');</pre>
<h3>Метод detach</h3>
<p>Метод <code>detach</code> аналогичен методу remove за тем исключением, что при удалении объекта сохраняются все связанные с ним данные. 
Преимущество такого подхода заключается в том, что мы можем позднее вставить удаленные элемент в другом месте разметки html:</p>
<pre class="brush:js;">
...
<h3>Метод empty</h3>
<p>Метод <code>empty</code> очищает содержимое элементов, делая их пустыми:</p>
<pre class="brush:js;">
$('ul').empty();
</pre>
<p>В результате все элементы списка будут удалены, и разметка будет содержать пустой список: <code>&lt;ul&gt;&lt;/ul&gt;</code></p>
