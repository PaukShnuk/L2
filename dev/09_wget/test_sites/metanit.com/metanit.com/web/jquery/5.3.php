...
<meta charset="utf-8" />
<meta name="description" content="Модель всплытия событий jquery, остановка всплытия с помощью метода stopPropagation">
...
<h2>Всплытие событий</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Допустим, у нас есть следующая структура документа:</p>
<pre class="brush:xml;">
&lt;body&gt;
	&lt;div&gt;
		&lt;button&gt;Жми&lt;/button&gt;
...
<p>Таким образом, у нас трехуровневая структура: на самом высоком уровне - элемент body, ниже - элемент div, и в самом низу - элемент button. 
Теперь добавим для каждого последовательно обработчики нажатия:</p>
<pre class="brush:js;">
...
<p>Итак, если мы нажмем на кнопку, то сработает не только собственно обработчик кнопки, но и обработчики выше лежащих уровней. Так как нажатие на кнопку - 
это в то же время нажатие и на элемент div и на элемент body, так как кнопка находится внутри них. Произойдет <span class="bb">всплытие событие</span>. 
То есть с каждым уровнем выше будут просматриваться обработчики события click и, если они имеются, они будут вызываться.</p>
<p>Однако мы можем запретить всплытие. Например, нам надо, чтобы срабатывал только обработчик нажатия кнопки. В этом случае нам надо применить метод 
...
<p>Теперь обработчики вышележащих уровней не будут задействованы.</p>
