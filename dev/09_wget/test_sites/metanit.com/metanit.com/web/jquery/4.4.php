...
<html >
<head>
<title>Замена элементов</title>
<meta charset="utf-8" />
<meta name="description" content="Замена элементов в jquery с помощью методов replaceWith и replaceAll и примеры их использования">
...
<h2>Замена элементов</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Метод <code>replaceWith</code> замещает внутренне содержимое элемента новым содержимым. Он имеет следующие варианты использования:</p>
<ul>
<li><p><code>replaceWith('разметка html')</code>: замена содержимого элемента разметкой html</p></li>
<li><p><code>replaceWith(элемент)</code>: замена содержимого элемента некоторым элементом html</p></li>
<li><p><code>replaceWith(jQuery)</code>: замена содержимого элемента объектом jQuery</p></li>
<li><p><code>replaceWith(функция)</code>: замена содержимого элемента объектом, возвращаемым функцией</p></li>
</ul>
<p>Например, заменим первый элемент списка новым содержимым:</p>
<pre class="brush:js;">
$('li.lang').first().replaceWith('&lt;li class="lang"&gt;JavaScript&lt;/li&gt;');
</pre>
<p>Применим функцию для замены содержимого. Допустим, у нас есть следующая разметка:</p>
<pre class="brush:xml;">
&lt;div id="list"&gt;
...
<p>Но мы например захотели преобразовать этот элемент div в список. Тогда мы можем написать так:</p>
<pre class="brush:js;">
...
<p>Функция проходит по всем элементам выборки. Перебираемый элемент мы можем получить с помощью ключевого слова <code>this</code>. 
Затем заменяем элементы внутри div на элементы списка. На выходе у нас получится следующая разметка:</p>
<pre class="brush:xml;">
&lt;ol&gt;
...
<p>Метод <code>replaceAll(замещаемый_элемент)</code> замещает все элементы, определенные в параметре, новым элементом. 
В качестве замещаемого элемента можем передавать селектор, элемент html или объект jQuery. Например, заменим 
все элементы предыдущего списка другим содержимым:</p>
<pre class="brush:js;">
$('&lt;li class="lang"&gt;Элемент списка&lt;/li&gt;').replaceAll('li.lang');
</pre>
<p>В результате получится следующая разметка:</p>
<pre class="brush:xml;">
&lt;ol&gt;
