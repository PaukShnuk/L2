...
<meta charset="utf-8" />
<meta name="description" content="Регистрация и удаление обработчиков событий в jquery с помощью методов bind и unbind и примеры использования обработчиков событий. Однократная обработка событий и метод one">
...
<h1>События jQuery</h1>
<h2>Регистрация и удаление обработчиков событий</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>События позволяют нам отследить изменение состояния элемента и обработать пользовательский ввод. Используя стандартный код JavaScript 
мы могли бы обработать событие прямо в разметке элемента с помощью соответствующих атрибутов. Например, обработаем нажатие кнопки:</p>
<pre class="brush:xml;">
&lt;button onclick="alert('Привет мир');"&gt;Нажать&lt;/button&gt;
</pre>
<p>Для обработки нажатия кнопки задаются некоторые действия, например, как в данном случае вывод сообщения.</p>
<p>Библиотека jQuery предлагает нам эффектный способ регистрации обработчиков событий у элементов с помощью метода <code>bind</code>.</p>
<p>Метод bind имеет следующий синтаксис: <code>bind('тип_события', функция_обработчика_события)</code>. Например, обработаем нажатие кнопки 
с помощью jQuery:</p>
<pre class="brush:xml;">
...
<p>Используя в обработчике выражение <code>$(this)</code> мы можем получить элемент, на котором было произведено событие. Используя свойства элемента, 
...
<p>Таким образом, если у нас на странице несколько кнопок, то только в случае с одной по нажатию на нее будет добавляться на страницу список. 
В остальных случаях будет просто выводиться сообщение на экран.</p>
<p>Применяя метод bind, важно учитывать, что он добавляет обработчики для уже имеющихся элементов. К динамически добавленным элементам 
обработчики применяться не будут.</p>
<h3>Метод unbind</h3>
<p>Если по какой-то причине нам больше не нужно обрабатывать событие, то мы можем отменить регистрацию связанного с ним обработчика с помощью метода <code>unbind</code>:</p>
<pre class="brush:xml;">
...
<h3>Однократная обработка событий. Метод one</h3>
<p>Метод <code>one</code> позволяет задать для элемента обработчик события, который будет срабатывать только один раз. После отработки произвойдет 
удаление обработчика:</p>
<pre class="brush:xml;">
&lt;button&gt;Жми1&lt;/button&gt;
&lt;button&gt;Жми2&lt;/button&gt;
&lt;button&gt;Жми3&lt;/button&gt;
...
<p>В результате обработчик каждой кнопки сработает по разу.</p>
