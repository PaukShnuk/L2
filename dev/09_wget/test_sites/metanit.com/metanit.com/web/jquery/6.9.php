...
<html >
<head>
<title>Глобальные события ajax</title>
<meta charset="utf-8" />
<meta name="description" content="Использование глобальных событий в jquery (ajaxComplete, ajaxError, ajaxSend, ajaxStart, ajaxStop, ajaxSuccess), их обработка, параметр global">
...
<h2>Глобальные события ajax</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>В предыдущих параграфах мы посмотрели, как использовать функции обратного вызова для обработки ответа с сервера. Но кроме локальных функций, 
мы можем использовать также обрабатывать глобальные события AJAX. Для обработки данных глобальных событий в jQuery предоставляет следующие методы:</p>
<ul>
<li><p><code>ajaxComplete</code>: регистрирует обработчик завершения запроса AJAX</p></li>
<li><p><code>ajaxError</code>: регистрирует обработчик, который будет вызываться, если запрос завершился с ошибкой</p></li>
<li><p><code>ajaxSend</code>: регистрирует функцию, которая будет срабатывать перед отправкой запроса</p></li>
<li><p><code>ajaxStart</code>: регистрирует обработчик, который запускается при запросе ajax</p></li>
<li><p><code>ajaxStop</code>: регистрирует обработчик, который запускается при завершении запросов ajax</p></li>
<li><p><code>ajaxSuccess</code>: регистрирует функцию, которая запускается при успешном завершении запроса</p></li>
</ul>
<p>Все методы принимают в качестве параметра функцию обработчика. В свою очередь данная функция во всех методах, кроме 
<code>ajaxStart</code> и <code>ajaxStop</code> может принимать три параметра:</p>
<ul>
<li><p>объект <code>Event</code>, содержащий данные о событии</p></li>
<li><p>объект <code>jqXHR</code>, связанный с данным ajax-запросом</p></li>
<li><p>дополнительные параметры</p></li>
</ul>
<p>Для управления глобальными событиями предназначена опция <code>global</code> в методе ajax. По умолчанию она имеет значение true, то есть 
глобальные события будут генерироваться, и мы их сможем обработать. Если же этот параметр равен <code>false</code>, то события не будут генерироваться.</p>
<p>Начиная с версии jquery 1.8 глобальные события следует прикреплять к объекту <code>document</code>:</p>
<pre class="brush:js;">
$(function(){
	$(document).ajaxStart(function() {
		$("#log").append("&lt;p&gt;Сработало событие ajaxStart&lt;/p&gt;" );
	});
...
<p>При нажатии на кнопку будет выполняться ajax-запрос и вместе с этим будет инициировано событие ajaxStart, в результате чего в один элемент 
добавится запись о сработавшем событии, а в другой будет загружен ответ от сервера.</p>
<p>Подобным образом мы можем использовать остальные методы:</p>
<pre class="brush:js;">
$(document)
	.ajaxStart(function() {
		$("#log").append("&lt;p&gt;Начало запроса&lt;/p&gt;" );
	})
	.ajaxSend(function(event, jqxhr, settings) {
		$("#log").append("&lt;p&gt;Запрос отправлен по адресу: "+settings.url+"&lt;/p&gt;" );
	})
	.ajaxStop(function() {
		$("#log").append("&lt;p&gt;Все запросы завершены&lt;/p&gt;" );
	})
	.ajaxError(function(event, jqxhr, settings) {
		$("#log").append("&lt;p&gt;Ошибка при выполнении запроса&lt;/p&gt;" );
	})
	.ajaxComplete(function(event, jqxhr, settings) {
		$("#log").append("&lt;p&gt;Запрос завершен&lt;/p&gt;" );
	})
	.ajaxSuccess(function(event, jqxhr, settings) {
		$("#log").append("&lt;p&gt;С сервера получены данные: "+jqxhr.responseText+"&lt;/p&gt;" );
	});
	$('button').click(function() {
		$("#result").load('ajax.php');
	});
