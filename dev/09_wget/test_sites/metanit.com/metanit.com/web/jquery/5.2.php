...
<html >
<head>
<title>Параметры обработчика. Объект Event</title>
<meta charset="utf-8" />
<meta name="description" content="Передача параметров в обработчики событий jquery и использование объекта Event, его свойства и методы">
...
<h2>Параметры обработчика. Объект Event</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>С помощью объекта <code>Event</code> в обработчик события передается ряд параметров, связанных с событием. Затем в обработчике мы 
можем извлечь значения этих параметров и как-то применить при обработке.</p>
<p>Некоторые распространенные свойства объекта Event:</p>
<ul>
<li><p><code>currentTarget</code>: текущий элемент, на котором обрабатывается событие</p></li>
<li><p><code>data</code>:представляет собой объект, в который передаются дополнительные данные связанные с событием, например, в методе bind</p></li>
<li><p><code>target</code>: элемент, инициировавший событие</p></li>
<li><p><code>timestamp</code>: время наступления события</p></li>
<li><p><code>pageX</code>: координаты указателя мыши относительно левой границы документа</p></li>
<li><p><code>pageY</code>: координаты указателя мыши относительно верхней границы документа</p></li>
<li><p><code>which</code>: в обработчиках событий клавиатуры или мыши объект, содержащий данные о нажатой клавише или кнопке</p></li>
</ul>
<p>Например, обработаем нажатие на страницы клавиш клавиатуры и получим нажатую клавишу:</p>
<pre class="brush:js;">
$(function(){
...
<p>Здесь мы регистрируем обработчик клавиатуры для всего документа и в случае нажатия на консоль будет выведен числовой код клавиши. 
Получив данный числовой код, мы можем произвести некоторые действия.</p>
<h3>Передача данных в обработчик</h3>
<p>Используя метод bind мы можем передать данные в параметр data события в виде необязательного второго параметра:</p>
<pre class="brush:js;">
var mes = 'Привет мир!';
...
<p>В данном случае переменная mes и будет передаваться в свойство e.data, из которого мы потом и сможем получить ее значение.</p>
<p>Также объект Event предоставляет ряд методов:</p>
<ul>
<li><p><code>preventDefault()</code>: отменяет все действия, которые по умолчанию производит данное событие. Например, при нажатии на ссылку мы можем с помощью 
данного метода запретить переход по этой ссылке.</p></li>
<li><p><code>isDefaultPrevented()</code>: возвращает true, если для данного события уже ранее вызывался метод <b>preventDefault</b></p></li>
<li><p><code>stopPropagation()</code>: запрет всплытия события вверх по иерархии DOM и его последующей обработки.</p></li>
<li><p><code>isPropagationStopped()</code>: возвращает true, если ранее уже был вызван метод <b>stopPropagation</b></p></li>
</ul>
