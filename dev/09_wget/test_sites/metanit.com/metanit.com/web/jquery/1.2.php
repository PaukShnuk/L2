...
<html >
<head>
<title>Функция jQuery</title>
<meta charset="utf-8" />
<meta name="description" content="Роль функции jquery и различные способы ее использования. Загрузка кода jQuery на веб-страницу">
...
<h2>Функция jQuery</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Ключевым моментом в использовании данной библиотеки являетя функция jQuery. Эта функция запускается поле загрузки страницы. Поскольку, 
как правило, эта функция содержит код, манипулирующий элементами веб-страницы, то в этом случае естественно надо, чтобы к моменту срабатывания 
функции jquery все эти элементы были загружены. Поэтому принято помещать эту функцию в самый низ html-страницы, например, перед закрывающим тегом 
body.</p>
<p>Если же мы в предыдущем примере с двумя кнопками пометили бы код jquery в секцию head, где у нас идет подключение библиотеки и прочих скриптов, то в 
этом случае мы бы столкнулись с ошибкой, поскольку к моменту подключения используемая в скрипте кнопка была бы еще не создана.</p>
<p>Функция jQuery имеет следующий синтаксис:</p>
<pre class="brush:js;">
jQuery(document).ready(function(){
  // здесь код функции.
});
</pre>
<p>Формально описание функции jQuery: <code>jQuery(объект)</code>. В данном случае в качестве объекта испольузется объект <code>document</code>, 
представляющий фактически всю структуру DOM веб-страницы. И к нему применяется обработчик <code>ready</code>, который и сигнализирует о том, что DOM-модель веб-страницы загружена. 
В качестве параметра обработчика используется безымянная функция обратного вызова, которая и срабатывает при загрузке веб-страницы.</p>
<p>То есть фактически мы говорим веб-браузеру, что после загрузки всей объектной модели веб-страницы, представленной объектом <code>document</code>, он должен выполнить весь код, который мы вкладываем 
в функцию jQuery.</p>
<p>Есть также еще один способ объявления, который также равнозначен предыдущему:</p>
<pre class="brush:js;">
$(document).ready(function(){
  // здесь код функции.
});
</pre>
<p>Знак <code>$</code> представляет псевдоним jQuery.</p>
<p>Но мы можем использовать и сокращенные записи функции jQuery:</p>
<pre class="brush:js;">
$(function(){
  // здесь код функции.
});
...
<p>Все приведенные здесь четыре формы будут равнозначны.</p>
