...
<html >
<head>
<title>Виджет button</title>
<meta charset="utf-8" />
<meta name="description" content="Виджет button в jQuery-UI, его использование, методы и свойства, установка виджета для группы кнопок и buttonset">
...
<h2>Виджет button</h2><div class="date">Последнее обновление: 28.09.2017</div>
...
<p>Первый виждет, с которым мы познакомимся - это <code>button</code>. Виджет button создает кнопку и применяется к одиночным элементам. 
Например:</p>
<pre class="brush:xml;">
&lt;html&gt;
    &lt;head&gt;
...
<p>В итоге после действия виджета <code>button</code> первая ссылка получит следующую разметку:</p>
<pre class="brush:xml">
...
<p>В итоге визуально это будет выглядеть так:</p>
<img src="pics/8.1.png" alt="Виджет button в jquery" />
<p>В зависимости от выбранной темы подключаемых силей jquery-ui внешний вид создаваемых кнопок может немного отличаться, однако мы также сами 
...
свойства у класса <code>ui-button</code>:</p>
<pre class="brush:xml">
&lt;style&gt;
	.ui-button {font-size: 13px;}
&lt;/style&gt;
</pre>
<h3>Методы виджета button</h3>
<p>Мы можем управлять виджетом button через ряд методов:</p>
<ul>
<li><p>button("disable"): делает кнопку недоступной</p>
<pre class="brush:js">
$(function(){
...
<p>Обратите внимание, что если ранее не был применен к элементам виджет button, то нельзя будет использовать его методы.</p></li>
<li><p>button("enable"): делает кнопку доступной для взаимодействия</p></li>
<li><p>button("destroy"): удаляет функциональность виджета button у элемента</p></li>
<li><p>button("widget"): возвращает объект jQuery, представляющий виджет (<code>var widget = $(".button_jqui").button("widget");</code>)</p></li>
<li><p>button("refresh"): обновляет визуальное состояние элемента</p></li>
<li><p>button("option"): позволяет получить или установить значения свойств кнопки:</p>
<pre class="brush:xml;">
...
<p>Для получения значения в качестве второго параметра после "option" передается название свойства, которое надо получить. 
А для установки добавляется третий параметр, который указывает на новое значение свойства, переданного во втором параметре.</p></li>
</ul>
<h3>Свойства виджета button</h3>
<p>Кроме методов у виджета button определено также несколько свойств.</p>
<ul>
<li><p>Свойство <code>disabled</code>. Выше мы уже использовали это свойство для отключения кнопки. Но кроме того, мы можем использовать следующий синтаксис для 
его установки: <code>$('button.get_option').button({disabled:true})</code></p></li>
<li><p>Свойство <code>label</code>: это свойство устанавливает надпись кнопки. (<code>$('button.get_option').button({label:'Кнопка 1'})</code>). 
Если в разметке html кнопки задано изначальная надпись, то установка свойства label заменяет ее.</p></li>
<li><p>Свойство <code>icons</code> устанавливает иконку на кнопке. Весь список возможных иконок вы можете найти на офсайте по адресу 
<a href="https://api.jqueryui.com/theming/icons/" rel="nofollow">https://api.jqueryui.com/theming/icons/</a>. В загружаемом пакете с 
jquery-ui эти иконки находятся в папке <span class="ii">css/images</span>.</p>
<pre class="brush:xml;">
...
&lt;/script&gt;</pre>
<p>В данном случае устанавливается иконка после текста кнопки:</p>
<img src="pics/8.2.png" />

<p>Если нам надо установить иконку перед текстом, то используется опция <code>primary</code>: <code>$('button.help').button({icons: { primary: "ui-icon-help" }});</code>. 
Также при желании можно задать иконки до и после текста, использовав сразу две опции.</p></li>
<li><p>Свойство <code>showLabel</code> указывает, должна ли отображаться надпись на кнопке. 
<code>$('button.get_option').button({showLabel:false})</code> - в данном случае, если для кнопки установлено свойство <code>icons</code>, 
то есть заданы иконки, то надпись не будет отображаться, а будет отображаться заданная иконка. Если свойство icons не указано, то свойство showLabel игнорируется.</p></li>
</ul>
<h3>Группа кнопок и buttonset</h3>
<p>Если виджет <code>button</code> позволяет создать кнопку из одного элемента, то <code>buttonset</code> применяется для создания группы 
кнопок. Например, мы можем создать группу флажков (checkbox) или переключателей (radiobutton). При этом <code>buttonset</code> применяется 
не к самим кнопкам, а к их контейнерам:</p>
<pre class="brush:xml;">
&lt;div id="langs"&gt;
...
&lt;/script&gt;</pre>
<img src="pics/8.3.png" />
<p>Аналогично мы можем установить виджет и для группы флажков.</p>
