...
<meta charset="utf-8" />
<meta name="description" content="Виджет progressbar в jQuery-UI, его использование, создание индикатора процесса, его методы и свойства">
...
<h2>Виджет progressbar</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Виджет progressbar используется для создания индикатора процесса. Он сообщает пользователю о ходе выполнения какой-нибудь задачи. 
Например:</p>
<pre class="brush:xml;">
&lt;div id="progress"&gt;&lt;/div&gt;
...
&lt;/script&gt;</pre>
<h3>Свойства виджета progressbar</h3>
<p>Виджет обладает несколькими свойствами, которые помогают выполнить его настройку:</p>
<ul>
<li><p><span class="bb">value</span>: текущее значение прогрессбара. Например: <code>$('#progress').progressbar({value:33})</code></p>
<img src="pics/8.4.png" />
<p>По умолчанию значение свойства равно 0 и не может быть выше значения свойства <code>max</code></p>
<p>Также может принимать булевое значение: если имеет значение <code>false</code>, то прогрессбар имеет неопределенную шкалу значений.</p></li>
<li><p><span class="bb">disabled</span>: отключает прогрессбар, если установлено значение true (<code>$('#progress').progressbar({disabled:true, value:25})</code>). 
По умолчанию имеет значение false.</p></li>
<li><p><span class="bb">max</span>: указывает максимальное значение прогрессбара. Например, <code>$('#progress').progressbar({max:200})</code>. 
По умолчанию имеет значение 100.</p></li>
</ul>
<h3>Методы виджета</h3>
<p>Прогрессбар имеет следующие методы:</p>
<ul>
<li><p>progressbar("disable"): делает прогрессбар недоступным: <code>$("#progress").progressbar("disable");</code></p></li>
<li><p>progressbar("enable"): делает прогрессбар доступным для взаимодействия</p></li>
<li><p>progressbar("destroy"): удаляет функциональность виджета progressbar у элемента</p></li>
<li><p>progressbar("widget"): возвращает объект jQuery, представляющий виджет (<code>var widget = $("#progress").progressbar("widget");</code>)</p></li>
<li><p>progressbar("option"): позволяет получить или установить значения свойств прогрессбара:</p>
<pre class="brush:xml;">
&lt;div id="progress"&gt;&lt;/div&gt;
...
<p>С помощью первой кнопки мы получаем значение, передавая в качестве второго параметра после "option" название свойства, которое надо получить. 
А с помощью второй кнопки у свойства прогрессбара устанавливается значение, введенное в текстовое поле - его мы передаем в качестве третьего параметра метода.</p></li>
<li><p>Дополнительно виджет реализует метод progressbar("value"), который возвращает значение свойства <code>value</code>. 
Передавая в качестве второго параметра числовое значение, мы можем установить значение индикатора: <code>$('#progress').progressbar('value', 35);</code></p></li>
</ul>
<h3>События виджета</h3>
<p>Для прогрессбара определены три события:</p>
<ul>
<li><p><code>change(event, ui)</code>: возникает при изменении значения прогрессбара, например</p>
<pre class="brush:js;">
...
<li><p><code>complete(event, ui)</code>: возникает, когда значение прогрессбара будет равно <code>max</code>. Например:</p>
<pre class="brush:js;">
