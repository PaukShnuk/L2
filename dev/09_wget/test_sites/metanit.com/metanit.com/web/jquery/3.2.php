...
<html >
<head>
<title>Изменение стилей CSS</title>
<meta charset="utf-8" />
<meta name="description" content="Использование в jquery свойства css для получения и установки стиля элемента">
...
<h2>Изменение стилей CSS</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<h3>Получение стиля</h3>
<p>Для работы со стилями применяется метод <code>css()</code>. Чтобы получить значение нужного свойства, мы передаем в этот метод в качестве 
параметра название свойства:</p>
<pre class="brush:js;">
console.log($('body').css('font-size'));
</pre>
<h3>Изменение стиля</h3>
<p>Чтобы изменить стиль, во-первых, мы можем передать в метод css в качестве второго параметра новое значение свойства:</p>
<pre class="brush:js;">
$('a').css('font-weight', 'bold');
</pre>
<p>Во-вторых, мы можем изменить стиль с помощью функции, которая также передается в качестве второго параметра метода css. 
Например, изменим цвет шрифта ссылок:</p>
<pre class="brush:js;">
...
<p>В зависимости от того, какое было старое значение oldValue элемента для данного свойства, функция возвращает новое значение для каждого элемента выборки.</p>
<p>В-третьих, мы можем передать массив свойств для установки:</p>
<pre class="brush:js;">
$('a').css({'color':'red', 'cursor':'pointer', 'font-size': '14px'});
</pre>
<p>Здесь в качестве параметра мы передаем объект javascript, в котором устанавливаем для нужных свойств новые значения.</p>
<p>Иногда необходимо увеличить или уменьшать свойства относительно текущего значения на определенное количество. В этом случае мы можем 
написать так:</p>
<pre class="brush:js;">
$('a').css({'font-size': '-=1', 'margin-left': '+=10'});
</pre>
<h3>Установка ширины и высоты</h3>
<p>Несмотря на то, что мы можем манипулировать свойствами <code>width</code> и <code>height</code> через метод css, мы можем также 
использовать одноименные методы <code>width()</code> и <code>height()</code>:</p>
<pre class="brush:js;">
var div = $('div').first();
