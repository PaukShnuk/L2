...
<html >
<head>
<title>Метод trigger</title>
<meta charset="utf-8" />
<meta name="description" content="Синтаксис и использование метода trigger в jquery для ручного вызова обработчиков событий">
...
<h2>Метод trigger</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Метод <code>trigger</code> используется для вызова обработчиков событий вручную. Например, вызвав метод trigger для нужного элемента 
мы можем также и вызвать обработчик события. Он имеет следующие варианты использования:</p>
<ul>
<li><p><code>trigger('событие')</code>: вызов обработчика для данного события</p></li>
<li><p><code>trigger(объект_Event)</code>: вызов обработчика с использованием объекта Event, который содержит данные о том, какой именно обработчик надо вызвать</p></li>
</ul>
<p>Посмотрим на примере. Допустим, у нас есть таблица и есть два кнопки на форме, которые используют обработчики таблицы:</p>
<pre class="brush:xml;">
&lt;body&gt;
&lt;table&gt;
...
<p>Во-первых, у нас уже определены обработчики для строк таблицы для событий <code>mouseenter</code> и <code>mouseout</code>. Затем у нас определены 
...
события mouseenter, определенный для строк таблицы. А по нажатию второй кнопки - обработчик события mouseout.</p>
<img src="pics/5.1.png" alt="Использование метода trigger в jquery" />
