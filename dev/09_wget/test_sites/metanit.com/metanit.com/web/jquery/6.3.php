...
<html >
<head>
<title>GET-запросы. Метод get</title>
<meta charset="utf-8" />
<meta name="description" content="Использование метода get в jquery для осуществления GET-запросов AJAX, параметры метода и их применение">
...
<h2>GET-запросы. Метод get</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Метод <code>get</code> осуществляет GET-запрос к серверу, то есть все данные запроса передаются в строке запроса. Он принимает следующие параметры:</p>
<ul>
...
<p>На выходе метод get возвращает объект <code>jqXHR</code>, который будет инкапсулировать данные запроса. Позднее мы подробнее разберем этот объект.</p>
<p>Итак, перепишем пример из предыдущего параграфа с использованием метода <code>get</code>:</p>
<pre class="brush:js;">
$(function(){
...
<p>Здесь использованы два первых параметра: адрес ресурса и функция обратного вызова. В этой функции в качестве параметра data мы получаем принятые от сервера 
данные и загружаем их в качестве разметки в элемент выборки (<code>$('#news').html(data);</code>). По сути то же самой мы могли бы сделать с помощью 
метода <code>load</code>.</p>
<h3>Отправка данных</h3>
<p>Используя параметр data, мы можем отправить дополнительные данные. Например, можно отправить запрос на получение элемента из базы данных по id:</p>
<pre class="brush:js;">
$.get('ajax.php', { id: "1"});
</pre>
<p>А поскольку в GET-запросе все данные передаются в строке запроса, то данный код будет аналогичен следующему:</p>
<pre class="brush:js;">
$.get('ajax.php?id=1');
</pre>
<p>Соответственно на стороне сервера мы сможем получить этот параметр и произвести с ним какие-либо действия, например, получить элемент из бд по данному id:</p>
<pre class="brush:php;">
&lt;?php
	$id=$_GET['id'];
?&gt;
</pre>
<h3>Использование типа данных</h3>
<p>Нередко возникает такая ситуация, когда сервер отправляет данные в определенном формате, например, json или xml. Так, php-файл на стороне 
сервера мог бы выглядеть следующим образом:</p>
<pre class="brush:php;">
&lt;?php
	echo json_encode(array("event"=&gt;"Начало чемпионата России","date"=&gt;"13.07.2013")); 
?&gt;
</pre>
<p>В данном случае он возвращает объект json. Тогда на клиентской стороне мы можем явно прописать тип данных, а в функции обработать принятые данные:</p>
<pre class="brush:js;">
$.get('ajax.php', 
	function(data) {
