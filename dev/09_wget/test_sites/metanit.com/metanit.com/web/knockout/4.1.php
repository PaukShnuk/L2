...
<html >
<head>
<title>Knockout | Взаимодействие с сервером</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие с сервером в KnockoutJS, сериализация объектов в JavaScript и JSON">
...
<h1>Взаимодействие с сервером</h1><div class="date">Последнее обновление: 17.05.2016</div>
...
<p>Knockout не включает какие-либо функции по взаимодействию с сервером, отправки запросов и т.п. В основном подобные задачи ложатся 
на имеющиеся библиотеки типа jQuery, которая имеет весь необходимый для этого функционал. Собственно на примере использования функций jQuery мы и рассмотрим 
отправку и получение данных с сервера.</p>
<h2>Сериализация объектов</h2>
<p>Формат JSON является на сегодняшний день наиболее распространенной формой передачи данных между сервером и клиентом. И прежде чем отправлять некоторые 
данные на сервер, их надо сериализовать в JSON.</p>
<p>ViewModel, с которой мы работаем в Knockout, по сути представляет стандартный объект JavaScript, поэтому его можно легко сериализовать в 
JSON с помощью встроенной в JS функции <code>JSON.stringify</code>. Однако, как правило, внутри себя ViewModel содержит не только обычные объекты, но и 
...
сериализовать в json.</p>
<p>Для облегчения сериализации наблюдаемых объектов Knockout предоставляет две специальные функции:</p>
<ul>
<li><p><span class="b">ko.toJS</span>: создает объектный граф модели, заменяя все наблюдаемые объекты observable их текущими значениями</p></li>
<li><p><span class="b">ko.toJSON</span>: создает строковое представление ViewModel в формате JSON. В своей внутренней реализации эта функция вызывает 
<code>ko.toJS()</code>, а затем для полученного объекта использует встроенный сериализатор JSON (например, функцию <code>JSON.stringify()</code>)</p></li>
</ul>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
...
<img src="pics/21.png" alt="JSON в Knockout" />
