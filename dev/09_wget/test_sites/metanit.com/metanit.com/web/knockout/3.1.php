...
<html >
<head>
<title>Knockout | Работа с формами</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с формами в KnockoutJS, привязка value и textInput и значение элементов форм">
...
<h1>Работа с формами</h1><h2>Привязка value и textInput</h2><div class="date">Последнее обновление: 17.05.2016</div>
...
<p>Привязка value позволяет установить привязку для атрибута <code>value</code> элемента формы. Данная привязка может применяться в элементах 
<code>input</code>, <code>textarea</code>, <code>select</code>.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="pics/13.png" alt="Привязка к элементам форм в Knockout" />
<p>Привязка к атрибуту value во многом аналогична привязке к текстовому содержимого элемента: после ключевого слова value в качестве параметра идет наблюдаемый объект:</p>
<pre class="brush:xml;">&lt;input data-bind="value: userName" /&gt;</pre>
<h3>Привязка textInput</h3>
<p>Привязка textInput связывает текстовое поле (<code>&lt;input&gt;</code>) или элемент textarea со свойствами ViewModel, позволяя синхронно изменять 
...
значения по мере ввода в текстовое поле.</p>
<pre class="brush:xml;">
...
<p>Фактически этот тот же самый пример, что и с привязкой value, только теперь элемент span, который также привязан к свойству userName, будет мгновенно 
отражать изменения свойства ViewModel по мере ввода в текстовое поле.</p>
