...
<html >
<head>
<title>Knockout | Обратные вызовы привязки foreach</title>
<meta charset="utf-8" />
<meta name="description" content="Обратные вызовы привязки foreach в KnockoutJS, обработка добавления и удаления элемента в массив">
...
<h2>Обратные вызовы привязки foreach</h2><div class="date">Последнее обновление: 17.05.2016</div>
...
<p>Привязка foreach позволяет использовать ряд функций обратного вызова, которые вызываются при возникновении тех или иных событий:</p>
<ul>
<li><p><span class="b">afterRender</span>: вызывается, когда сработала самая первая инициализация привязки foreach, и также срабатывает при добавлении нового элемента в массив</p></li>
<li><p><span class="b">afterAdd</span>: срабатывает каждый раз при добавлении нового элемента в массив</p></li>
<li><p><span class="b">beforeRemove</span>: срабатывает перед удалением элемента из массива. Как правило, используется для анимации удаления</p></li>
<li><p><span class="b">beforeMove</span>: вызывается до перемещения элемента внутри массива</p></li>
<li><p><span class="b">afterMove</span>: вызывается после перемещения элемента внутри массива</p></li>
</ul>
<p>Как правило, функции обратного вызова служат для анимации элементов. Все они принимают три параметра:</p>
<ul>
<li><p>Изменяемый элемент структуры DOM на веб-странице, который был добавлен (afterAdd), удален (beforeRemove) или перемещен (beforeMove / afterMove)</p></li>
<li><p>Индекс изменяемого элемента в массиве</p></li>
<li><p>Изменяемый элемент самого массива, который был добавлен (afterAdd), удален (beforeRemove) или перемещен (beforeMove / afterMove)</p></li>
</ul>
<p>Пример анимации добавляемого элемента с помощью методов обратного вызова:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Для анимации вначале подключается библиотека jquery. Сама анимация определена в функции displayItem:</p>
<pre class="brush:js;">
displayItem: function(element, index, data) {
...
<p>То есть сначала устанавливает почти полная прозрачность, а затем она уменьшается в течение 2000 миллисекунд.</p>
<p>Сама же привязка принимает следующую форму:</p>
<pre class="brush:js;">data-bind="foreach: { data: phones, afterAdd: displayItem }"</pre>
