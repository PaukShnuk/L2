...
<html >
<head>
<title>Knockout | Привязка options</title>
<meta charset="utf-8" />
<meta name="description" content="Привязка options в KnockoutJS, работа с элементом select, создание списков">
...
<h2>Привязка options</h2><div class="date">Последнее обновление: 17.05.2016</div>
...
<p>Привязка <span class="b">options</span> применяется к выпадающему списку  или к спискам 
с множественным выбором, то есть к элементу <code>&lt;select&gt;</code>. Эта форма привязки служит для наполнения списка элементами.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="pics/17.png" alt="options binding in Knockout" />
<p>В качестве значения параметру options передается объект, представляющий массив.</p>
<p>Также вместе с параметром options можно еще ряд параметров:</p>
<ul>
<li><p><code>optionsCaption</code>: устанавливает заголовок в списке</p></li>
<li><p><code>optionsText</code>: устанавливает отображаемую элементом списка текстовую метку</p></li>
<li><p><code>optionsValue</code>: устанавливает значение элемента списка</p></li>
<li><p><code>optionsAfterRender</code>: назначает функцию обратного вызова, которая срабатывает при завершении рендеринга элементов списка</p></li>
</ul>
...
<p>В данном случае каждый элемент списка будет отображать значения свойства "model", а самим значением будет служить свойство "id".</p>
<h3>selectedOptions</h3>
<p>Параметр <span class="b">selectedOptions</span> также используется в паре с параметром <code>options</code> 
и позволяет определить, какие элементы в списках с множественным выбором будут выбраны по умолчанию:</p>
<pre class="brush:xml;">
&lt;p&gt; Выбрать модель:&lt;/p&gt;
&lt;select data-bind="options: phones,
...
<img src="pics/19.png" alt="selectedOptions in knockout" />
