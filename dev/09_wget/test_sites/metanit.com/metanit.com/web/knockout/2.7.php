...
<html >
<head>
<title>Knockout | Привязка if и ifnot</title>
<meta charset="utf-8" />
<meta name="description" content="Привязка if и ifnot в KnockoutJS, установка видимости элемента html">
...
<h2>Привязка if и ifnot</h2><div class="date">Последнее обновление: 17.05.2016</div>
...
<p>Привязка if позволяет скрыть, либо, наоборот, отобразить какой-то определенный элемент веб-страницы. Во многом она аналогична привязке visible:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>При опреелении привязки после ключевого слова <span class="b">if</span> идет выражение. Если это выражение возвращает значение <code>true</code>, 
то элемент, к которому применяется привязка, отобразается. Иначе элемент скрывается.</p>
<pre class="brush:js;">data-bind="if: display"</pre>
<p>Данный тип привязки также можно задать с помощью комментарных форм:</p>
<pre class="brush:xml;">
&lt;!-- ko if: display --&gt;
    &lt;p&gt;if-binding&lt;/p&gt;
&lt;!-- /ko --&gt;
</pre>
<p>Комментарии <code>&lt;!-- ko --&gt;</code> и <code>&lt;!-- /ko --&gt;</code> выполняют роль соответственно начального и конечного маркера, 
которые определяют "виртуальный элемент". Knockout также обрабатывает этот виртуальный элемент и выполняет к нему привязку также, как и к реальному элементу веб-страницы.</p>
<p>Привязка <span class="b">ifnot</span> работает аналогичным образом только в обратную сторону: если выражение привязки возвращает значение <code>false</code>, 
тогда привязанный элемент отображается. Например:</p>
<pre class="brush:xml;">
