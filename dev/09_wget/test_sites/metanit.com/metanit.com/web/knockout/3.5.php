...
<html >
<head>
<title>Knockout | Событие click</title>
<meta charset="utf-8" />
<meta name="description" content="Событие click в KnockoutJS, обработка нажатия на элемент">
...
<h2>Событие click</h2><div class="date">Последнее обновление: 17.05.2016</div>
...
<p>Привязка <span class="b">click</span> добавляет обработчик нажатия на любой связанный элемент веб-страницы. Как правило, в роли таких элементов выступают 
кнопки, ссылки, элементы input, но также это может быть любой видимый элемент веб-страницы.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="pics/20.png" alt="Обработка нажатий в Knockout" />
<h3>Передача параметра в обработчик</h3>
<p>При вызове функции обработчика Knockout передает ему в качестве первого параметра текущее значение модели:</p>
<pre class="brush:xml;">
...
<p>Также в качестве второго параметра мы можем получить данные о событии:</p>
<pre class="brush:js;">
var viewModel = {
    phones: ko.observableArray(["Honor 6", "Meizu Pro 6", "iPhone 6S"]),
...
<p>По умолчанию Knockout обрабатывает события последовательно начиная с вложенных элементов и заканчивая родительскими элементами, которые также могут иметь 
привязку click. Например:</p>
<pre class="brush:xml;">
...
<p>То есть при нажатии на кнопку сначала срабатывает функция <code>buttonClickHandler</code>, потом срабатывает функция родительского элемента - <code>divClickHandler</code>.</p>
<p>Чтобы избежать подобной множественной обработки события необходимо применять дополнительный параметр - <span class="b">clickBubble</span>, 
передав ему значение <code>false</code>:</p>
<pre class="brush:xml;">
&lt;div data-bind="click: divClickHandler"&gt;
