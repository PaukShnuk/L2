...
<html >
<head>
<title>Knockout | Событие submit</title>
<meta charset="utf-8" />
<meta name="description" content="Событие submit в KnockoutJS, обработка отправки форм">
...
<h2>Событие submit</h2><div class="date">Последнее обновление: 17.05.2016</div>
...
<p>Привязка <span class="b">submit</span> добавляет обработчик, который срабатывает при отправке формы:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В качестве параметра в обработчик передается элемент формы, у которого мы можем получить введенные значения.</p>
<p>Надо отметить, что при этом не происходит отправка формы. Чтобы она происходила, надо чтобы функция обработчика возвращала значение <code>true</code>:</p>
<pre class="brush:js;">
submitHandler : function(form) {
    console.log(form.elements["login"].value);
	console.log(form.elements["password"].value);
