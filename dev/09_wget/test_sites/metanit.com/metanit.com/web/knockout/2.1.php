...
<html >
<head>
<title>Knockout | Наблюдаемые объекты</title>
<meta charset="utf-8" />
<meta name="description" content="Функция ko.observable в KnockoutJS и создание привязки">
...
<h1>Основы привязки</h1><h2>Наблюдаемые объекты</h2><div class="date">Последнее обновление: 17.05.2016</div>
...
<p>KnockoutJS отслеживает изменения и при их наличии обновляет связанные компоненты. Но как KnockoutJS узнает, 
какие части view model были изменены? Для этого свойства модели надо определить как <span class="b">observables</span> или наблюдаемые объекты. 
Это специальные объекты JavaScript, которые могут извещать подписчиков об изменениях и автоматически отслеживать зависимости.</p>
<p>Для создания наблюдаемых объектов применяется метод <span class="b">ko.observable()</span>, в который передается значение объекта. Рассмотрим на примере:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="pics/7.png" alt="Observables в KnockoutJS" />
<p>Здесь во ViewModel определяются два наблюдаемых объекта в виде свойств phoneName и phoneCompany.</p>
<p>Чтобы динамически получить наблюдаемый объект в javascript, надо обратиться к этому объекту как к методу:</p>
<pre class="brush:js;">
var name = viewModel.phoneName();	// Lumia 650
var company = viewModel.phoneCompany();	// Microsoft
</pre>
<p>Чтобы установить новое значение, надо передать в этот объект новое значение в качестве параметра:</p>
<pre class="brush:js;">
viewModel.phoneName("Lumia 950 XL");
</pre>
<p>На первый взгляд такое определение ViewModel будет аналогично определению простого объекта:</p>
<pre class="brush:js;">
var viewModel = {
    phoneName: "Lumia 650",
...
<p>Однако наблюдаемые объекты позволяют также отслеживать изменения. Например, добавим на веб-страницу кнопку, по нажатию которой будем изменять значение наблюдаемого объекта:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В итоге при нажатии на кнопку у нас изменится не только сам объект, но и содержимое html-элемента, который привязан к этому объекту:</p>
<img src="pics/8.png" alt="Отслеживание изменений привязки в Knockout" />
<p>Если бы мы определили свойства во ViewModel как обычные объекты, то подобная синхронизация значений при их изменении не работала.</p>

<h3>Настройка подписки наблюдаемых объектов</h3>
<p>С помощью функции <span class="b">subscribe()</span> можно отслеживать изменения в наблюдаемых объектов и выполнять при этом определенные действия. Например, 
пусть при изменении значения новое значение объекта выводится в диалоговом окне:</p>
<pre class="brush:js;">
viewModel.phoneName.subscribe(function(newValue) {
    alert("Новое значение " + newValue);
});
</pre>
<p>Функция <code>subscribe</code> принимает функцию обратного вызова, которая вызывается при получении уведомления об изменении наблюдаемого объекта</p>
<p>С помощью метода <span class="b">dispose()</span> в любой момент времени можно прекратить подписку и отменить действие функции subscribe:</p>
<pre class="brush:js;">
