...
<html >
<head>
<title>Диаграммы с областями в D3.js</title>
<meta charset="utf-8" />
...
<h2>Диаграммы с областями</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Особую разновидность линейных диаграмм представляют диаграммы с областями (area chart). Для их построения используется метод <span class="bb">d3.svg.area()</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="pics/4.3.png" alt="Линейные диаграммы с областями в D3.js" />
<p>Здесь фактически также создается линейный график, только теперь все пространство между ним и осями закрашивается, образуя область. Для создания области применяется 
метод <code>d3.svg.area()</code>:</p>
<pre class="brush:js;">
var area = d3.svg.area()
	.x(function(d) { return d.x; })
	.y0(height - margin)
...
<p>Метод <code>x()</code> возвращает х-координату, <code>y1()</code> - y-координату, а метод <code>y0()</code> устанавливает константу, 
...
к высоте графика прибавить отступ сверху.</p>
<p>И также, как и с линиями, создаем по объектам area новый путь:</p>
<pre class="brush:js;">
g.append("path")
	.attr("d", area(data))
	.style("fill", "lightblue");
