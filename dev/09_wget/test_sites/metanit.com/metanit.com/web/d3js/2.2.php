...
<html >
<head>
<title>Функции для работы с данными в D3.js</title>
<meta charset="utf-8" />
...
<h2>Функции для работы с данными</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>При работе с данными мы можем полагаться на дополнительные функции, которые нам предлагает библиотека D3.js. Что представляют эти функции?</p>
<ul>
<li><p><code>d3.min(data)</code>: возвращает минимальное значение в массиве data</p></li>
<li><p><code>d3.max</code>: возвращает максимальное значение</p></li>
<li><p><code>d3.extent</code>: возвращает минимальное и максимальное значения в виде массива из двух элементов</p></li>
<li><p><code>d3.sum</code>: возвращает сумму всех элементов массива</p></li>
<li><p><code>d3.median</code>: возвращает медиану массива</p></li>
<li><p><code>d3.mean</code>: получает среднее значение</p></li>
<li><p><code>d3.ascending / d3.descending</code>: представляют функции, упорядочивающие массив по возрастанию / по убыванию</p></li>
<li><p><code>d3.quantile</code>: возвращает квантиль элементов массива, отсортированного по возрастанию</p></li>
<li><p><code>d3.bisect</code>: функция, которая возвращает позицию в отсортированном массиве, куда можно поместить определенный объект, не нарушая порядка возрастания.</p></li>
<li><p><code>d3.nest</code>: представляет функцию, которая преобразует некоторый массив объект в другую иерархическую структуру, более удобную для визуализации данных</p></li>
</ul>
<p>Например, в javascript определен следующий код:</p>
<pre class="brush:js;">
...
<p>Тогда мы получим следующий вывод:</p>
<pre class="brush:js;">
"d3.max: 21"
"d3.min: 1"
"d3.extent: 1,21"
...
<p>Отдельно разберем функцию nest, которая возвращает сгруппированные по определенному ключу данные:</p>
<pre class="brush:js;">
...
<p>Каждый элемент массива phones имеет три свойства, и по свойству "company" мы проводим группировку. То есть в данном случае у нас будет две группы объектов.</p>
<p>Для группировки применяется функция <code>d3.nest().key(...)</code>, которая возвращает свойство, служащее ключом. Сгруппированные наборы затем формируются с помощью 
функции <code>entries()</code>.</p>
<p>Для вывода объектов используются две вспомогательные функции <code>printArray()</code> и <code>printObject()</code>. Итоговый вывод будет выглядеть 
следующим образом:</p>
<pre class="brush:js;">
"Apple"
