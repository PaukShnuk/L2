...
<html >
<head>
<title>Трансформации и переходы в D3.js</title>
<meta charset="utf-8" />
...
<h2>Трансформации и переходы</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<h3>Трансформации</h3>
<p>Трансформации являются одним из ключевых аспектов работы с D3.js. Выделяются три типа трансформаций:</p>
<ul>
<li><p>Масштабирование</p></li>
<li><p>Перемещение</p></li>
<li><p>Вращение</p></li>
</ul>
<p>Все трансформации задаются для элемента с помощью атрибута <span class="bb">transform</span>. Для перемещения этот атрибут принимает значение 
<code>translate(x, y)</code>:</p>
<pre class="brush:js;">
var width = 400,
    height = 400;
...
<img src="pics/3.7.png" alt="Перемещение в D3.js" />
<p>Для масштабирования используется значение scale(s) (увеличение в s раз) или scale(sx, sy) (увеличение по горизонтали в sx раз и по вертикали в sy раз):</p>
<pre class="brush:js;">  
...
<img src="pics/3.8.png" alt="Масштабирование в D3.js" />
<p>Для вращения фигур применяется значение <code>rotate(degree,x,y)</code>, где первый параметр - угол в градусах поворота по часовой стрелке, а 
x и y - координаты центра точки поворота:</p>
<pre class="brush:js;">  
// оригинальный прямоугольник	
svg.append("rect")
	.style("fill", "none")
...
<img src="pics/3.9.png" alt="Вращение в D3.js" />
<p>При необходимости мы можем применять все или несколько трансформаций сразу:</p>
<pre class="brush:js;">  
svg.append("rect")
	.style("fill", "none")
	.style("stroke", "red")
...
<img src="pics/3.10.png" alt="Трансформации в D3.js" />
<h3>Переходы</h3>
<p>Переходы (transitions) в D3.js подразумевают динамическое изменение свойств объекта в течение некоторого промежутка времени, то есть его анимацию. 
Для создания перехода используют тройку методов <code>transition()</code>, <code>duration(duration)</code> (устанавливает время перехода) и <code>delay(duration)</code> 
(устанавливает период задержки перед выполнением перехода). Параметр duration в последних двух методах задает время в миллисекундах:</p>
<pre class="brush:js;">
var svg = d3.select("body").append("svg")
					.attr("height", 400)
					.attr("width", 400);    
...
<p>В данном случае устанавливается время анимации в 5000 миллисекунд и задержка в 200 миллисекунд. Если необходимость в функции <code>delay()</code> 
отсутствует, то мы ее можем опустить.</p>
<p>Далее определяется ряд стилей и атрибутов и их конченые значения. То есть в данном случае D3 будет осуществлять переход от цвета "black" к 
цвету "#ffffff". Библиотека автоматически рассчитает промежуточные показатели, которые будет принимать стилевое свойство fill между значениями 
"black" и "#ffffff".</p>
<p>То же самое относится к свойству "stroke" и атрибутам ширины и высоты.</p>
