...
<html >
<head>
<title>Координатные оси в D3.js</title>
<meta charset="utf-8" />
...
<h2>Координатные оси</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Для построения линейных графиков нам необходимы координатные оси. И поскольку линейные графики не мыслимы без осей, то в библиотеке d3.js 
имеется специальный метод <span class="bb">d3.svg.axis()</span>, который позволяет строить координатные оси. Рассмотрим на примере построения горизонтальной оси:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="pics/2.1.png" alt="Горизонтальная ось X в D3.js" />
<p>Вся логика по созданию оси X размещается в функции <code>createAxes</code> и разбивается на несколько этапов.</p>
<p>Во-первых, необходимо создать объект svg, который будет заключать ось:</p>
<pre class="brush:js;">
svg = d3.select("body").append("svg")
	.attr("class", "axis")
	.attr("width", width)
...
<p>Во-вторых, надо примерно осознавать разброс значений данных, которые должны быть отображены на графике. В нашем случае пока никаких данных нет, но 
грубо говоря представим, что ориентировочный диапазон данных будет от 0 до 1000. Эти значения должны накладываться на ось X, которая будет иметь длину в 450 пикселей - 
значение width (ширина объекта svg) минус отступы справа и слева по 25 пикселей. И для сопоставления значений задаем функцию линейной интерполяции:</p>
<pre class="brush:js;">
var scale = d3.scale.linear()
	.domain([0, 1000])
	.range([0, axisWidth]);
</pre>
<p>Далее собственно создаем горизонтальную ось:</p>
<pre class="brush:js;">
var axis = d3.svg.axis() 
		.scale(scale)
		.orient('bottom') // горизонтальная
...
<p>Чтобы создать ось, нужна функция интерполяции <code>scale</code> для сопоставления значений данных с экранными координатами. 
Так как оси могут быть горизонтальными и вертикальными, то надо задать ориентацию оси с помощью функции <code>orient()</code>.</p>
<p>Для создания горизонтальной оси в эту функцию в качестве параметра передается параметр 'bottom'. 'Bottom' также задает расположение текстовых меток по - 
под линией оси. Однако мы можем задать и над осевое расположение текста, передав вместо 'bottom' строку 'top'.</p>
<p>Построение оси Y будет отличаться от оси X только тем, что в функцию <code>orient()</code> будет передаваться параметр 'left' (расположение текстовых 
меток слева от оси) или 'right' (расположение меток справа от оси).</p>
<p>Последняя функция по цепочке <code>ticks()</code> задает количество делений на оси. Однако реальное число делений также будет зависеть от того, как 
алгоритмы D3.js оптимальным образом расположат деления по оси.</p>
<p>Финальная часть состоит в отрисовке оси:</p>
<pre class="brush:js;">
svg.append("g")
	.attr("transform", "translate("+margin+","+offset+")")
	.call(axis);
</pre>
<p>В контейнере svg оси должны размещаться внутри элемента <span class="bb">g</span>.  С помощью атрибута <code>transform</code>, которому в качестве 
значения задается <code>translate(25, 50)</code>, элемент g сдвигается по X на 25 единиц и по Y на 50 единиц. А функция <code>call()</code> передает и отрисовывает в элементе g ось.</p>
<p>Если мы обратимся к отрисованной ось в инспекторе элементов, то увидим ее сложную структуру:</p>
<img src="pics/2.2.png" alt="Координатные оси в D3.js" />
