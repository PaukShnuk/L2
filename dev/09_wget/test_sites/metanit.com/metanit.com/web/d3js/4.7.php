...
<html >
<head>
<title>Интерполяция и сглаживание графиков в D3.js</title>
<meta charset="utf-8" />
...
<h2>Интерполяция и сглаживание графиков</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>С помощью вспомогательной функции <span class="bb">interpolate</span> мы можем добиться интерполяции точек графика, при которой рассчитываются 
промежуточные переходы между двумя соседними точками. Эта функция в качестве параметра одной из следующих значений:</p>
<ul>
<li><p><code>basis</code>: образует b-сплайн с дублированием контрольных точек на концах отрезков</p></li>
<li><p><code>basis-open</code>: образует открытый b-сплайн</p></li>
<li><p><code>basis-closed</code>: образует закрытый b-сплайн</p></li>
<li><p><code>cardinal</code>: образует cardinal-сплайн с дублированием контрольных точек на концах отрезков</p></li>
<li><p><code>cardinal-open</code>: образует открытый cardinal-сплайн</p></li>
<li><p><code>cardinal-closed</code>: образует закрытый cardinal-сплайн</p></li>
<li><p><code>linear</code>: идентичен стандартному графику, где точки соединены линиями</p></li>
<li><p><code>linear-closed</code>: сегменты линий образуют замкнутые многоугольники</p></li>
<li><p><code>monotone</code>: кубическая интерполяция</p></li>
<li><p><code>step-before</code>: пошаговая интерполяция между вертикальными и горизонтальными сегментами</p></li>
<li><p><code>step-after</code>: пошаговая интерполяция между горизонтальными и вертикальными сегментами</p></li>
</ul>
<p>Используем параметр "basis":</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="pics/4.4.png" alt="Интерполяция графика в D3.js" />
<p>Причем интерполяцию графика мы можем применять как к набору линий <code>d3.svg.line().interpolate("basis")</code>, 
так и к набору областей, если они используются: <code>d3.svg.area().interpolate("basis")</code></p>
<p>Если мы используем другой параметр, то визуализация графика будет  отличаться. Например, параметр "step-after":</p>
<img src="pics/4.5.png" alt="Line chart interpolation in в D3.js" />
