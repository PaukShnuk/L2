...
<html >
<head>
<title>Несколько графиков в одной координатной системе в D3.js</title>
<meta charset="utf-8" />
...
<h2>Несколько графиков в одной координатной системе</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Теперь добавим второй график, а в качестве оси X используем шкалу временных отрезков:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="pics/4.11.png" alt="Графики в D3.js" />
<p>Для каждого графика определен свой массив данных: usdData и eurData. Чтобы автоматизировать построение оси Y по данным из этих массивов находится минимальное и максимальное 
значения:</p>
<pre class="brush:js;">
var maxValue = d3.max([d3.max(eurData, function(d) { return d.rate; }), 
					d3.max(usdData, function(d) { return d.rate; })]);
...
<p>Затем эти значения применяются для создания функции интерполяции значений на ось Y:</p>
<pre class="brush:js;">
var scaleY = d3.scale.linear()
            .domain([maxValue, minValue])
            .range([0, yAxisLength]);
</pre>
<p>На оси X будут располагаться даты, и так как даты обоих массивов совпадают, здесь просто берется минимальная и максимальная дата из первого массива. 
Хотя опять же можно было бы находить по двум и более массивам минимальную и максимальную даты.</p>
<p>При создании оси Х ее метки форматируются в определенный формат, а именно в формат '%e.%m' (день.месяц):</p>
<pre class="brush:js;">
var xAxis = d3.svg.axis()
             .scale(scaleX)
             .orient("bottom")
...
<p>И так как для создания двух графиков нам надо выполнить фактически одни и те же действия, то весь функционал по их созданию выделяется в 
дополнительную функцию <code>createChart()</code>.</p>
