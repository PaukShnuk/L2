...
<html >
<head>
<title>Гистограммы с нечисловыми данными в D3.js</title>
<meta charset="utf-8" />
...
<h2>Гистограммы с нечисловыми данными</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Нередко гистограммы отображают такие данные, которые имеют только одну числовую компоненту. В качестве второй компоненты могут выступать различные 
строки, например, названия. Допустим, нам надо вывести с помощью гистограмм набранные очки футбольных команд и их названия. 
Если по одной оси можно отобразить числовые значения - очки команд, то возникает вопрос, как поступить с названиями команд. 
Реализуем эту задачу на примере:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="pics/4.6.png" alt="Гистограммы в D3.js" />
<p>Так как по горизонтали располагаются не какие-то количественные показатели, а названия команд, выполняющие роль подписей к столбикам, то надо применить 
функцию <span class="bb">d3.scale.ordinal()</span>:</p>
<pre class="brush:js;">
var xScale = d3.scale.ordinal()
		.rangeRoundBands([0, xAxisLength + margin], .1)
		.domain(data.map(function(d) { return d.name; }));
</pre>
<p>Функция <code>rangeRoundBands()</code> принимает отрезок, на котором будут располагаться названия команд (<code>[0, xAxisLength + margin]</code>), 
а также коэффициент масштабирования столбиков - <code>0.1</code>.</p>
<p>А функция <code>domain()</code> получает данные, с которыми будет сопоставляться вышеопределенный отрезок [0, xAxisLength + margin].</p>
<p>Для задания цвета каждого столбика здесь определена функция <code>var color = d3.scale.category10();</code>, которая возвращает один из 10 цветов по порядку.</p>
<p>Как и при линейном графике, здесь также создаются координатные оси, но без вертикальных линий сетки, так как значения по оси Х нам не нужны. 
И ключевой момент заключается в создании самих столбиков гистограммы:</p>
<pre class="brush:js;">
svg.append("g")
	...
	.attr("x", function(d) { return xScale(d.name); })
	.attr("width", xScale.rangeBand())
	.attr("y", function(d) { return yScale(d.score) + margin; })
    .attr("height", function(d) { return yAxisLength - yScale(d.score); })
	.attr("fill", function(d) { return color(d.name); });
</pre>
