...
<html >
<head>
<title>Двумерные координаты в D3.js</title>
<meta charset="utf-8" />
...
<h2>Двумерные координаты</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Теперь добавим к горизонтальной оси и вертикальную, чтобы образовалась полноценная координатная плоскость.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="pics/2.3.png" alt="Координатная система в D3.js" />
<p>Здесь мы не устанавливаем количество делений с помощью метода <code>ticks</code> и при делении осей на отрезки полностью полагаемся на d3.js, 
которая поделит оси на интервалы автоматически, причем по оси X откладывается столько же делений, сколько и по оси Y:</p>
<pre class="brush:js;">
var xAxis = d3.svg.axis()
             .scale(scaleX)
             .orient("bottom");    
...
<p>Но при необходимости мы также могли бы настроить деление оси: <code>d3.svg.axis().scale(scaleX).orient("bottom").ticks(5)</code></p>
<p>Чтобы горизонтальная ось должным образом соприкасалась с вертикальной, надо сместить ее вниз по вертикали на величину, равную длине вертикальной, и 
соответственно у обеих осей будет одно и то же смещение по горизонтали:</p>
<pre class="brush:js;">
svg.append("g")       
     .attr("class", "x-axis")
     .attr("transform",  // сдвиг оси вниз и вправо
...
<p>Затем получаем выше добавленный элемент и выбираем в нем все элементы <code>g.tick</code>, которые добавляются автоматически и представляют отдельные деления, 
и добавляем по ним линии с помощью элемента <code>line</code>:</p>
<pre class="brush:js;">
d3.selectAll("g.x-axis g.tick")
    .append("line") // добавляем линию
    .classed("grid-line", true) // добавляем класс
...
<p>Каждая линия задается двумя точками с координатами (x1, y1) - начальная точка и (x2, y2) - конечная точка. Если первая точка определяется на 
оси X, то для определения y-составляющей второй точки используется yAxisLength, что аналогично выражению (height - 2 * margin). То есть линия будет идти с оси X вверх.</p>
<p>Построение горизонтальных линий будет аналогично.</p>
<p>Если мы обратимся к графику в инспекторе элементов, то увидим его сложную структуру:</p>
