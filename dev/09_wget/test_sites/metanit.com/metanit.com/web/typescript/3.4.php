...
<html >
<head>
<title>TypeScript | Модификаторы доступа</title>
<meta charset="utf-8" />
<meta name="description" content="Модификаторы доступа в TypeScript, private, protected, public, инкапсуляция и аксессоры, определение свойств через конструкторы">
...
<h2>Модификаторы доступа</h2><div class="date">Последнее обновление: 16.05.2021</div>
...
<p>Модификаторы доступа позволяют сокрыть состояние объекта от внешнего доступа и управлять доступом к этому состоянию. 
В TypeScript три модификатора: <span class="b">public</span>, <span class="b">protected</span> и <span class="b">private</span>.</p>
<p>Если к свойствам и функциям классов не применяется модификатор, то такие свойства и функции расцениваются как будто они определены с модификатором 
<code>public</code>. То есть следующее определение класса:</p>
<pre class="brush:ts;">
class Person {
...
<h3>private</h3>
<p>Если же к свойствам и методам применяется модификатор <code>private</code>, то к ним нельзя будет обратиться извне при создании объекта данного класса.</p>
<p>Например, создадим класс с приватными свойствами и методами:</p>
<pre class="brush:ts;">
class Person {
...
<p>Два свойства _name и _year используются с модификатором <code>private</code>, поэтому мы не можем их использовать вне класса, 
например, <code>console.log(tom._name)</code>. То же самое относится к функции <code>setYear()</code>. Остальные функции доступны.</p>
<h3>protected</h3>
<p>Модификатор <span class="b">protected</span> определяет поля и методы, которые из вне класса видны только в классах-наследниках:</p>
<pre class="brush:ts;">
class Person {
...
<p>В классе Employee будет доступно поле <code>name</code>, а также метод <code>printPerson()</code>, так как они имеют модификатор protected. 
А вот приватное свойство <code>year</code> и приватный метод <code>setYear()</code> будут недоступны.</p>
<h3>Определение полей через конструктор</h3>
<p>Использование модификаторов в параметрах конструктора позволяет сократить написание кода. Например, пусть у нас есть следующий класс:</p>
<pre class="brush:ts;">
class Person {
...
<p>Этот класс будет аналогичен следующему:</p>
<pre class="brush:ts;">
class Person {
...
<p>Используя модификаторы в параметрах конструктора, нам больше не надо явно создать свойства для этих параметров. Свойства создаются 
автоматически, называются они по имени параметров и имеют те же модификаторы, что и параметры.</p>
<p>Подобным образом, если мы хотим сделать свойства публичными, то следует использовать модификатор <span class="b">public</span>:</p>
<pre class="brush:ts;">
class Person {
...
<p>Если необходимо сделать поле для чтения, то к модификатору доступа добавляется модификатор <span class="b">readonly</span>:</p>
<pre class="brush:ts;">
class Person {
...
<div class="nav"><p><a href="./3.12.php">Назад</a><a href="./">Содержание</a><a href="./3.10.php">Вперед</a></p></div></div>
<div class="socBlock">
