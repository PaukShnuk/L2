...
<html >
<head>
<title>TypeScript | Заголовочные файлы для популярных библиотек</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение популярных библиотек на JavaScript в TypeScript на примере jQuery">
...
<h2>Заголовочные файлы для популярных библиотек</h2><div class="date">Последнее обновление: 20.05.2021</div>
...
<p>В прошлой теме было рассмотрено использование декларативных заголовочных файлов для внешнего кода на javascript. Однако, как правило, 
...
Этот репозиторий расположен на гитхабе: <a href="https://github.com/DefinitelyTyped/DefinitelyTyped/" rel="nofollow">https://github.com/DefinitelyTyped/DefinitelyTyped/</a></p>
<p>Как использовать js-библиотеки и заголовочные файлы для них? Рассмотрим на примере библиотеки <a href="https://jquery.com/download/" rel="nofollow">jquery</a>.</p>
<p>Вначале загрузим определения типов для библиотеки jquery с помощью следующей команды:</p>
<div class="console"><pre class="consoletext">npm install --save-dev @types/jquery</pre></div>
<p>В итоге в проекте будет создана папка <span class="b">node_modules/@types</span>, в которой каталог <span class="b">jquery</span> будет хранить 
заголовочные файлы для библиотеки jquery.</p>
<p>Кроме того, в проекте будет создан файл <span class="b">package.json</span> со следующим содержимым:</p>
<pre class="brush:js;">
{
  "devDependencies": {
    "@types/jquery": "^3.5.5"
...
<p>Здесь мы видим, что текущие определения для jquery работают с версией 3.5.5 и выше.</p>
<p>Теперь нам надо указать компилятору TypeScript с помощью параметра <span class="b">typeRoots</span>, где находятся заголовочные файлы - а это каталог <span class="b">node_modules/@types</span>. 
Для этого в проекте определим следующий файл конфигурации <span class="b">tsconfig.json</span>:</p>
<pre class="brush:js;">
{
    "compilerOptions": {
		"noImplicitAny": true,
...
<p>Далее в корневой папке проекта определим следующую веб-страницу <span class="b">index.html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь только подключается библиотека jquery из cdn, а также файл нашего приложения - app.js.</p>
<p>Далее в корневой папке проекта определим следующий файла <span class="b">app.ts</span>:</p>
<pre class="brush:js;">
$(document).ready(() =&gt; {
    $("#content").html("&lt;h1&gt;Привет TypeScript&lt;/h1&gt;");
});
</pre>
<p>В данном случае мы задействуем событие <code>document.ready</code>, которое определено в jquery и которое срабатывает при загрузке документа. 
И далее с помощью лямбда-выражения, которое определяет функцию обратного вызова, с помощью знакомого многим синтаксиса jquery на веб-страницу добавляется новый элемент.</p>
<p>Также мы могли бы использовать сокращенное определение функции:</p>
<pre class="brush:ts;">
$(() =&gt; {
    $("#content").html("&lt;h1&gt;Привет TypeScript&lt;/h1&gt;");
});
</pre>
<p>То есть в итоге проект будет выглядеть следующим образом:</p>
<img src="./pics/5.png" alt="Подключение jquery в TypeScript" />
<p>Теперь скомпилируем файл приложения с помощью команды</p>
<div class="console"><pre class="consoletext">tsc</pre></div>
<img src="./pics/9.png" alt="компиляция jquery в typescript" />
<p>В итоге при запуске веб-страницы index.html сработает код из файла app.ts:</p>
<img src="./pics/29.png" alt="Jquery and TypeScript" />
<p>Для примера еще обработаем событие нажатия кнопки. Допустим, в html-коде страницы есть такая кнопка:</p>
<pre class="brush:xml;">
&lt;button id="alertBtn"&gt;Жми&lt;/button&gt;
</pre>
<p>В коде на TS  мы могли бы так обработать нажатия на эту кнопку:</p>
<pre class="brush:ts;">
$(() =&gt; {
    $("#alertBtn").click((e) =&gt; { $("#content").html("&lt;h2&gt;Привет мир&lt;/h2&gt;"); });
});
</pre>
<p>Обработчики событий, например, <code>click</code> в качестве параметра также принимают лямбда-выражение, которое определяет 
набор инструкции, выполняемых при нажатии.</p>
...
<div class="nav"><p><a href="./4.1.php">Назад</a><a href="./">Содержание</a><a href="./6.1.php">Вперед</a></p></div></div>
<div class="socBlock">
