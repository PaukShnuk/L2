...
<html >
<head>
<title>TypeScript | Методы доступа get и set</title>
<meta charset="utf-8" />
<meta name="description" content="Методы доступа в TypeScript, get, set, инкапсуляция и аксессоры">
...
<h2>Методы доступа get и set</h2><div class="date">Последнее обновление: 16.05.2021</div>
...
<p>В стандарте JavaScript ECMAScript 5 была предложена концепция методов доступа: для доступа к свойству определяется пара методов - 
...
или в C#, где для доступа к приватным переменным создается свойство с двумя методами get/set.</p>
<p>К примеру, у нас есть следующий класс:</p>
<pre class="brush:ts;">
class Person {
    name: string;
	age: number;
...
<p>Здесь полю <code>age</code>, которое представляет условный возраст человека, присваивается некорретное значение.</p>
<p>Использование аксессоров или методов доступа позволяет управлять тем, как значение устанавливается и как оно возвращается. В частности, мы можем 
переписать предыдущих класс с использованием акссессоров следующим образом:</p>
<pre class="brush:ts;">
class Person {
    name: string;
	private _age: number;
...
<p>Методы доступа определяются как обычные методы, только перед ними ставятся ключевые слова <span class="b">get/set</span>. Set-метод контроллирует установку 
значения, а get-метод возвращает значение.</p>
<p>В данном случае мы опосредовали доступ к переменной <code>_age</code>, сделав ее приватной. Теперь к ней можно обратиться только через методы доступа. 
Метод <code>get</code> просто возвращает значение переменной:</p>
<pre class="brush:ts;">
public get age(): number {
        return this._age;
    }
</pre>
<p>А метод <code>set</code> устанавливает значение <code>_age</code>, но только в том случае, если оно укладывается в корректный диапазон.</p>
<p>Когда мы присваиваем значение:</p>
<pre class="brush:ts;">tom.age = 36;</pre>
<p>срабатывает set-метод</p>
<p>Когда мы получаем значение:</p>
<pre class="brush:ts;">console.log(tom.age);</pre>
<p>срабатывает get-метод</p>
...
<div class="nav"><p><a href="./3.4.php">Назад</a><a href="./">Содержание</a><a href="./3.13.php">Вперед</a></p></div></div>
<div class="socBlock">
