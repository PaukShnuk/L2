...
<html >
<head>
<title>TypeScript | Параметры компиляции noEmitOnError и target</title>
<meta charset="utf-8" />
<meta name="description" content="Настройка компилятора в TypeScript, параметр noEmitOnError и target, не компилировать при ошибке">
...
<h2>Параметры компиляции noEmitOnError и target</h2><div class="date">Последнее обновление: 10.05.2021</div>
...
<p>В прошлой теме было рассмотрено, что нельзя в одной области видимости несколько раз определить переменную с 
одним и тем же именем или что значение константы нельзя изменить.</p>
<p>Но давайте рассмотрим следующую небольшую программу на TypeScript:</p>
<pre class="brush:ts;">
let x = 10;
console.log(x);	// 10
...
<p>При компиляции ожидаемо мы увидим ряд ошибок:</p>
<img src="./pics/41.png" alt="Ошибки компиляции в TypeScript" />
<p>Тем не менее, если применяются настройки компиляции по умолчанию, javascript-файл вполне нормально скомпилируется, в частности, он будет иметь следующее содержание:</p>
<pre class="brush:js;">
var x = 10;
console.log(x); // 10
var x = 20; // ! Ошибка
...
<p>И в итоге в консоли браузера мы увидим результат работы этого скрипта:</p>
<img src="./pics/42.png" alt="noEmitOnError в TypeScript" />
<p>Но для компилятора TS можно с помощью параметра <span class="b">--noEmitOnError</span> можно указать, что не надо генерировать файл javascript, если 
при компиляции возникли ошибки. Этот параметр можно указать напрямую при вызове компилятора в консоли:</p>
<div class="console"><pre class="consoletext">tsc --noEmitOnError app.ts</pre></div>
<p>Либо в файле конфигурации <span class="b">tsconfig.json</span> для настройки <code>"noEmitOnError"</code> указать значение <code>true</code>:</p>
<pre class="brush:js;">
{
    "compilerOptions": {
        "target": "es5",
...
<h3>target</h3>
<p>Исправим предыдущий пример, чтобы он проходил компиляцию</p>
<pre class="brush:ts;">
let x = 10;
console.log(x);	// 10
...
<p>При компиляции с настройками по умолчанию мы получим следующий файл javascript:</p>
<pre class="brush:js;">
var x = 10;
console.log(x); // 10
var y = 20;
...
<p>Мы видим, что для определения всех значений применяется ключевое слово <code>var</code>, несмотря на то, что даже не самые современные браузеры уже давно 
поддерживают операторы <code>let</code> и <code>const</code>. Но компилятор typescript по умолчанию компилирует в стандарт ES3, для которого существует 
только оператор <code>var</code>. Но var/let/const - это частные случае. Как минимум, подавляющая часть стандарта ES2015 уже давно поддерживается во многих распространенных 
...
параметр <span class="b">-target/-t</span>.</p>
<p>Например, опять возьмем файл, который содержит ошибки:</p>
<pre class="brush:ts;">
let x = 10;
console.log(x);	// 10
...
<p>Скомпилируем в стандарт "ES2015":</p>
<div class="console"><pre class="consoletext">tsc app.ts -t ES2015</pre></div>
<p>Здесь не указан параметр "noEmitOnError", поэтому компилятор сообщит об ошибках, однако файл все равно скомпилирует. Скомпилированный файл 
практически не будет отличаться от кода на typescript:</p>
<pre class="brush:js;">
let x = 10;
console.log(x); // 10
var x = 120; // ! Ошибка
...
<p> Однако в браузере, который поддерживает let и const, все равно мы столкнемся с ошибкой:</p>
<img src="./pics/43.png" alt="ES2015 и компилятор TypeScript" />
<p>Также можно задать эту настройку в файле <span class="b">tsconfig.json</span>:</p>
<pre class="brush:js;">
{
    "compilerOptions": {
        "target": "es2015",
