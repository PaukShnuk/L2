...
<html >
<head>
<title>TypeScript | Первое приложение в Visual Studio</title>
<meta charset="utf-8" />
<meta name="description" content="Создание приложения на TypeScript в Visual Studio 2019 и 2022">
...
<h2>Первое приложение на TypeScript в Visual Studio</h2><div class="date">Последнее обновление: 09.05.2021</div>
...
<p>Среда разработки Visual Studio имеет встроенные возможности по созданию проектов с использованием TypeScript. Итак, создадим первое приложение на TypeScript в Visual Studio. 
Прежде всего нам необходимо с помощью установщика Visual Studio установить соответствующий компонент. Данный компонент называется <span class="b">Разработка Node.js</span>:</p>
<img src="./pics/35.png" alt="Установка TypeScript в Visual Studio" />
<p>При создании нового проекта в поле языка мы можем выбрать <span class="b">TypeScript</span> и увидеть доступные по умолчанию несколько шаблонов проектов, 
которые предполагают использование языка TypeScript:</p>
<img src="./pics/20.png" alt="TypeScript в Visual Studio" />
<p>Эти проекты позволяют использовать TypeScript в различных ситуациях - в связке с Node.js, Vue и т.д. В данном случае выберем самый простой 
тип проекта, который можно использовать для изучения TS - <span class="b">Blank Node.js Console Application</span>.</p>
<p>После выбора данного типа проекта дадим ему какое-нибудь имя, например, <span class="b">HelloApp</span> и укажем ему расположение:</p>
<img src="./pics/36.png" alt="Первый проект на TypeScript в Visual Studio" />
<p>В итоге Visual Studio создаст следующий проект:</p>
<img src="./pics/21.png" alt="Первое приложение на TypeScript" />
<p>Проект по умолчанию имеет следующие файлы:</p>
<ul>
<li><p><span class="b">app.ts</span>: файл с кодом на TypeScript</p></li>
<li><p><span class="b">package.json</span>: файл с определением пакетов и прочей конфигурации для Node.js</p></li>
<li><p><span class="b">tsconfig.json</span>: файл конфигурации TypeScript</p></li>
</ul>
<p>Кроме того, в узле <span class="b">npm/dev</span> можно увидеть применяемые для разработки пакеты npm - <span class="b">@types/node</span> и 
<span class="b">typescript</span>.</p>
<p>Ключевым элементом здесь является файл <span class="b">app.ts</span>, который выполняется при запуске приложения. Изменим его код следующим образом:</p>
<pre class="brush:ts;">
class User{
    name : string;
...
<p>Как и в прошлой теме, здесь определяется класс User, создается объект этого класса, и данные объекта выводятся на консоль.</p>
<p>Теперь перейдем к запуску проекта. Однако разработка в Visual Studio имеет ряд тонкостей. Прежде всего при обычном запуске консоль запуститься и захлопнется 
быстрее, чем мы там сможем что-то увидеть. Чтобы запущенная консоль не схлопывалась, перейдем в Visual Studio к пункту меню <span class="b">Tools -&gt; Options</span></p>
<img src="./pics/38.png" alt="Настройка TypeScript и Node.js в Visual Studio" />
<p>Далее в меню окна опций найдем пункт <span class="b">Node.js Tools/span>. Для этого пункта предусмотрены три опции - отметим все эти опции:</p>
<img src="./pics/37.png" alt="консольное приложение на TypeScript и Node.js в Visual Studio закрывается" />
<p>Запустим проект на выполнение с помощью комбинации клавиш <span class="b">Ctrl + F5</span> (то есть запуск без отладки), и перед нами откроется следующая консоль:</p>
<img src="./pics/22.png" alt="Первое консольное приложение на TypeScript и Node.js в Visual Studio" />
<p>Другая тонкость заключается в том, что по умлчанию кодировка файла app.ts может отличаться от UTF-8, из-за этого могут возникнуть проблемы с выводом 
кириллических символов. В этом случае мы можем в Visual Studio перейти к пункту меню <span class="b">File -&gt; Save As</span>. Далее в выпадающем меню у кнопки 
<span class="b">Save</span> выбрать пункт <span class="b">Save with Encoding...</span>:</p> 
<img src="./pics/39.png" alt="Русские символы в консольном приложении на TypeScript и Node.js в Visual Studio" />
<p>Затем в поле <span class="b">Encoding</span> укажем кодировку <code>Unicode (UTF-8 without signature)</code>.</p>
<img src="./pics/40.png" alt="Кодировка файлов Encoding в Visual Studio" />
