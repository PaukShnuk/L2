...
<html >
<head>
<title>TypeScript | Статические поля и методы</title>
<meta charset="utf-8" />
<meta name="description" content="Статические поля и методы в TypeScript, ключевое слово static, особенности определения и применения, передача статических методов при наследовании классов">
...
<h2>Статические поля и методы</h2><div class="date">Последнее обновление: 17.05.2021</div>
...
<p>Кроме обычных полей и методов класс может иметь статические. Статические поля и методы относятся не к отдельным объектам, а в целом к классу. И для обащения 
к статическим полям и методам применяется имя класса.</p>
<p>Статические поля и методы определяются с помощью ключевого слова <span class="b">static</span>:</p>
<pre class="brush:ts;">
class Person {
...
<p>Здесь в классе Person определено статическое поле <code>retirementAge</code>, которое условно хранит возраст выхода на пенсию для объектов Person. Поскольку это свойство будет общим 
для всех объектов Person, то имеет смысл сделать его статическим, то есть общим для всего класса, как в данном случае.</p>
<p>Для обращения к этому полю применяется имя класса:</p>
<pre class="brush:ts;">console.log(Person.retirementAge);</pre>
<p>Также определен статический метод <code>calculateYears()</code>, который условно вычисляет оставшее до пенсии врем (для упрощения задачи условимся, что передаваемый возраст меньше возраста выхода на пенсию). 
И также для обращения к этому методу применяется имя класса, а не объекта:</p>
<pre class="brush:ts;">let years = Person.calculateYears(36);</pre>
<p>Стоит отметить, что в статических методах мы можем обращаться к статическим полям или другим статическим методам класса, но мы не можем обращаться к 
нестатическим полям и методам и использовать ключевое слово <code>this</code>. Например, в следующем случае мы столкнемся с ошибкой при компиляции:</p>
<pre class="brush:ts;">
...
<p>Как и обычные поля и методы статические могут иметь модификаторы доступа:</p>
<pre class="brush:ts;">
class Person {
...
<p>Также статические поля и методы могут наследоваться, что позволяет обращаться к ним через имя производного класса:</p>
<pre class="brush:ts;">
class Person {
...
<div class="nav"><p><a href="./3.10.php">Назад</a><a href="./">Содержание</a><a href="./3.3.php">Вперед</a></p></div></div>
<div class="socBlock">
