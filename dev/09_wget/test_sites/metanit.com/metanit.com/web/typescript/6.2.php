...
<html >
<head>
<title>TypeScript | Декораторы методов и их параметров</title>
<meta charset="utf-8" />
<meta name="description" content="Декораторы методов и их параметров в TypeScript, их параметры, PropertyDescriptor и его свойства">
...
<h2>Декораторы методов и их параметров</h2><div class="date">Последнее обновление: 20.05.2021</div>
...
<p>Декоратор метода также представляет функцию, которая принимает три параметра:</p>
<pre class="brush:ts;">
function deprecated(target: any, propertyName: string, descriptor: PropertyDescriptor){ 
	console.log("Method is deprecated");
}
</pre>
<p>Декоратор принимает следующие параметры:</p>
<ol>
<li><p>Функция конструктора класса для статического метода, либо прототип класса для обычного метода.<p></li>
<li><p>Название метода.<p></li>
<li><p>Объект интерфейса PropertyDescriptor:</p>
<pre class="brush:ts;">
...
<p>Этот объект описывает изменение декорируемого метода. Применяется при компиляции в ES5 и выше, при ES3 имеет значение undefined.<p>
<p>Его свойство <code>value</code> содержит определение функции. Свойство <code>writable</code> указывает, является ли функция модифицируемой (если 
значение true, то является).</p></li>
</ol>
<p>Определим простейший декоратор для метода:</p>
<pre class="brush:ts;">
...
<p>Декоратор <code>readable</code> с помощью выражения <code>descriptor.writable = false;</code> устанавливает, что метод, к которому применяется 
данный декоратор, не может быть изменен.</p>
<p>В итоге после применения данного декоратора следующая инструкция</p>
<pre class="brush:ts;">tom.print = function(){console.log("print has been changed");}</pre>
<p>не имеет смысла и не будет работать. Однако если бы декоратор не применялся, то инструкция сработала бы.</p>
<h3>Параметры декоратора</h3>
<p>Декоратор может принимать параметры, которые позволяют настроить из вне поведение декоратора. Например, немного изменим предыдущий пример:</p>
<pre class="brush:ts;">
...
<p>Теперь параметр <code>readable</code> принимает параметр типа boolean:</p>
<pre class="brush:ts;">function readable(onlyRead : boolean){</pre>
<p>Значение этого параметра используется для установки свойства <code>descriptor.writable</code>:</p>
<pre class="brush:ts;">descriptor.writable = !onlyRead;</pre>
<p>При этом сам декоратор должен возратить функцию, которая соответствует выше рассмотренному определению функции декоратора.</p>
<p>При применении декоратора в скобках ему можно передать значения для параметров:</p>
<pre class="brush:ts;">
@readable(false)
print():void{
</pre>

<h3>Параметры и выходной результат метода</h3>
<p>Декоратор метода позволяет нам манипулировать параметрами и возвращаемым результатом метода. Например, определим следующий декоратор:</p>
<pre class="brush:ts;">
...
<p>Декоратор log логгирует (выводит на консоль) значения параметров и возвращаемый результат метода. Свойство <code>descriptor.value</code> 
позволяет получить начальное значение метода - то есть ту функцию, которую представляет метод.</p>
<pre class="brush:ts;">let originalMethod = descriptor.value;</pre>
<p>Затем происходит переустановка значения <code>descriptor.value</code>.</p>
<pre class="brush:ts;">
descriptor.value = function(...args: number[]){
}
</pre>
<p>Параметр <code>...args</code> - это все те параметры, которые будут передаваться в функцию. И мы можем логгировать эти параметры. Далее выывается оригинальная функция, 
которой передаются параметры args:</p>
<pre class="brush:ts;">
let returnValue = originalMethod.apply(this, args);
</pre>
<p>И таким образом мы можем получить результат вызова оригинальной функции и возвратить его из функции. То есть фактически получается, что мы берем оригинальную функцию, 
обертываем ее в какую-ту другую функцию, в которой опять же вызываем оригинальную функцию и возвращаем ее результат.</p>
<p>Таким образом, новое значение descriptor.value принимает те же параметры, возвращает тот же результат, что и оригинальная функция, но при этом добавляет некоторое дополнительное поведение.</p>
<p>Далее мы можем применить декоратор, например, к методу add класса Calculator и вызвать этот метод.</p>
<img src="./pics/33.png" alt="Получение параметов метода в декораторах TypeScript" />
<h3>Декораторы параметров методов</h3>
<p>Декоратор параметра метода представляет функцию, которая принимает три параметра:</p>
<pre class="brush:ts;">
function MyParameterDecorator(target: Object, propertyKey: string, parameterIndex: number){
	// код декоратора
}
</pre>
<p>Где первый параметр представляет конструктор класса, если метод статический, либо прототип класса, если метод нестатический. А второй параметр 
представляет имя метода. И третий параметр представляет порядковый индекс параметра в списке параметров.</p>
<p>Определим декоратор для параметра метода:</p>
<pre class="brush:ts;">
...
<p>Декоратор logParameter добавляет в прототип класса новое свойство <code>metadataKey</code>. Это свойство представляет массив, который содержит индексы декорированных параметров.</p>
<p>Для чтения метаданных из свойства metadataKey применяется декоратор метода logMethod, который перебирает все параметры метода, находит значения параметров 
по индексам, которые определены декоратором параметра, и выводит на консоль названия и значения декорированных параметров.</p>
<img src="./pics/31.png" alt="Декоратор параметра метода в TypeScript" />
