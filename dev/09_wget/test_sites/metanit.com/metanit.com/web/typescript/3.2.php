...
<html >
<head>
<title>TypeScript | Наследование</title>
<meta charset="utf-8" />
<meta name="description" content="Наследование в TypeScript, ключевое слово extends и super, переопределение базовых классов, их конструкторов и методов">
...
<h2>Наследование</h2><div class="date">Последнее обновление: 17.05.2021</div>
...
<p>Одним из ключевых моментов объектно-ориентированной парадигмы является наследование. В TypeScript наследование реализуется с помощью 
ключевого слова <span class="b">extends</span> (как в Java):</p>
<pre class="brush:ts;">
class Person {
...
<p>Класс Employee, который представляет работника, является подклассом или наследуется от класса Person. А класс Person называется родительским или базовым классом. При наследовании класс Employee перенимает весь 
функционал класса Person - все его свойства и функции и может их использовать. И также можно определить в подклассе новые свойства и методы, которых нет в классе Person.</p>
<pre class="brush:ts;">
let bob: Employee = new Employee("Bob");
bob.print();
</pre>
<p>Также мы можем расширить функциональность класса следующим образом:</p>
<pre class="brush:ts;">
class Person {
...
<p>Если подкласс определяет свой конструктор, то в нем должен быть вызван конструктор базового класса с помощью ключевого слова <span class="b">super</span>:</p>
<pre class="brush:ts;">
class Person {
...
<p>С помощью ключевого слова <span class="b">super</span> подкласс может обратиться к функционалу базового класса. В данном случае 
идет обращение к конструктору класса Person, который устанавливает значение свойства name: <code>super(name)</code></p>
<p>Причем даже если базовый класс не определяет явным образом никакого конструктора, в производном классе при определении 
конструктора все равно надо вызывать конструктор базового класса - в этом случае это будет вызов конструктора по умолчанию с помощью <code>super()</code>.</p>
<pre class="brush:ts;">
class Person {
    name: string;
}
...
<p>Также производные классы могут переопределять методы базовых классов:</p>
<pre class="brush:ts;">
class Person {
...
<p>В данном случае переопределяется метод <code>print()</code>, который кроме имени выводит также компанию сотрудника. Однако в данном случае реализация метода 
<code>print()</code> из базового класса повторяется в производном классе. И вместо того, чтобы дублировть код, мы можем с помощью ключевого слова <span class="b">super</span> 
вызвать реализацию этого метода из базового класса:</p>
<pre class="brush:ts;">
class Person {
...
<div class="nav"><p><a href="./3.1.php">Назад</a><a href="./">Содержание</a><a href="./3.12.php">Вперед</a></p></div></div>
<div class="socBlock">
