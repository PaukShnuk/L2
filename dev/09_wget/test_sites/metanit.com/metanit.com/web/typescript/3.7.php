...
<html >
<head>
<title>TypeScript | Миксины</title>
<meta charset="utf-8" />
<meta name="description" content="Реализация множественного наследования классов в TypeScript через миксины, трейты, их ограничения">
...
<h2>Миксины</h2><div class="date">Последнее обновление: 18.05.2021</div>
...
<p>TypeScript, как и многие объектно-ориентированные языки, как, например, Java или C#, не позволяет использовать напрямую множественное наследование. 
Мы можем реализовать множество интерфейсов в классе, но унаследовать его можем только от одного класса. Однако функциональность миксинов (<span class="b">mixins</span>) 
частично позволяют унаследовать свойства и методы сразу двух и более классов.</p>
<p>Рассмотрим на примере. Пусть, у нас есть класс Animal, который представляет животное, и класс Movable, который представляет транспортное средство. 
...
То есть для создания подобного класса было бы неплохо унаследовать его сразу и от класса Animal, и от класса Movable. Решим эту задачу на языке TypeScript:</p>
<pre class="brush:ts;">
class Animal {
...
<p>Чтобы класс Horse мог унаследовать методы классов Animal и Movable, определяется одноименный интерфейс Horse, 
который расширяет интерфейсы Animal и Movable:</p>
<pre class="brush:ts;">interface Horse extends Animal, Movable {}</pre>

<p>Но чтобы миксин мог унаследовать функционал, этого недостаточно. Нам еще надо использовать специальную функцию, которая перекопирует функционал из 
родительских классов в миксин:</p>
<pre class="brush:ts;">
function applyMixins(derivedCtor: any, baseCtors: any[]) {
    baseCtors.forEach(baseCtor =&gt; {
        Object.getOwnPropertyNames(baseCtor.prototype).forEach(name =&gt; {
...
<p>Затем применяем эту функцию:</p>
<pre class="brush:ts;">
applyMixins(Horse, [Animal, Movable]);
</pre>
<p>Первым параметром идет класс-миксин, а второй параметр - массив применяемых классов.</p>
