...
<html >
<head>
<title>TypeScript | Перечисление enum</title>
<meta charset="utf-8" />
<meta name="description" content="Перечисление enum в TypeScript, определение констант enum и их использование">
...
<h2>Перечисление enum</h2><div class="date">Последнее обновление: 15.05.2021</div>
...
<p>Тип <span class="b">enum</span> или перечисление позволяет определить набор именнованных констант, которые 
описывают определенные состояния.</p>
<p>Для определения перечисления применяется ключевое слово <span class="b">enum</span>. Например, объявим следующее перечисление:</p>
<pre class="brush:ts;">
enum Season { Winter, Spring, Summer, Autumn };
</pre>
<p>Перечисление называется <code>Season</code> и имеет четыре элемента. Теперь используем перечисление:</p>
<pre class="brush:ts;">
enum Season { Winter, Spring, Summer, Autumn };
...
<p>Здесь создается переменная <code>current</code>, которая имеет тип Season. При этом консоль браузера выведет нам число 2 - значение константы 
...
<pre class="brush:ts;">enum Season { Winter, Spring, Summer, Autumn };</pre>
<p>фактически эквивалентно следующему:</p>
<pre class="brush:ts;">
enum Season { Winter=0, Spring=1, Summer=2, Autumn=3 };
</pre>
<p>Хотя мы можем явным образом переопределить эти значения. Так, мы можем задать значение одной константы, тогда значения следующих констант будет увеличиваться на единицу:</p>
<pre class="brush:ts;">
enum Season { Winter=5, Spring, Summer, Autumn };			// 5, 6, 7, 8
</pre>
<p>Либо можно каждой константе задать свое значение:</p>
<pre class="brush:ts;">
enum Season { Winter=4, Spring=8, Summer=16, Autumn=32 };	// 4, 8, 16, 32
</pre>
<p>Также мы можем получить непосредственно текстовое значение:</p>
<pre class="brush:ts;">
enum Season { Winter=0, Spring=1, Summer=2, Autumn=3 };
var current: string = Season[2];	// 2 - числовое значение Summer
console.log(current);	// Summer
</pre>
<h3>Строковые перечисления</h3>
<p>Кроме числовых перечислений в TypeScript есть строковые перечисления, константы которых принимают строковые значения:</p>
<pre class="brush:ts;">
enum Season { 
	Winter = "Зима", 
	Spring = "Весна",
...
<h3>Смешанные гетерогенные перечисления</h3>
<p>Также можно определять смешанные перечисления, константы которых могут числа и строки.</p>
<pre class="brush:ts;">
enum Season { 
	Winter = 1, 
	Spring = "Весна",
...
<h3>Перечисления в функциях</h3>
<p>Перечисление может выступать в качестве параметра функции.</p>
<pre class="brush:ts;">
enum DayTime { 
	Morning, 
	Evening
...
<p>Каждая константа перечисления описывает некоторое состояние. И функция <code>welcome()</code> в виде параметра dayTime принимает это состояние и в зависимости 
от полученного значения выводит на консоль определенное значение.</p>
<p>Однако стоит отметить, что поскольку здесь перечисление DayTime представляет числовое перечисление, то в реальности в функцию <code>welcome()</code> мы можем 
передать числовые значения:</p>
<pre class="brush:ts;">welcome(1);	// Добрый вечер</pre>
<p>Либо даже определить параметр функции как числовой и передавать константы числового перечисления:</p>
<pre class="brush:ts;">
enum DayTime { 
	Morning, 
	Evening
...
<p>Пример параметра-строкового перечисления:</p>
<pre class="brush:ts;">
enum DayTimeMessage { 
	Morning = "Доброе утро", 
...
<p>При использовании строковых перечислений в отличие от числовых мы не можем передать переметру произвольную строку:</p>
<pre class="brush:ts;">welcome("Привет, ты спишь?")</pre>
<p>В этом случе компилятор выкатит нам ошибку при компиляции.</p>
<p>В то же время если параметр представляет тип <code>string</code>, то такому параметру можно передавать как строки, так и константы строкового перечисления:</p>
<pre class="brush:ts;">
enum DayTimeMessage { 
	Morning = "Доброе утро", 
	Evening = "Добрый вечер"
