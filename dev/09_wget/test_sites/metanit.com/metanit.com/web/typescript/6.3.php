...
<html >
<head>
<title>TypeScript | Декораторы свойств и методов доступа</title>
<meta charset="utf-8" />
<meta name="description" content="Декораторы свойств и методов доступа в TypeScript, их параметры, определение декоратора для геттера и сеттера">
...
<h2>Декораторы свойств и методов доступа</h2><div class="date">Последнее обновление: 20.05.2021</div>
...
<h3>Декораторы свойств</h3>
<p>Декоратор свойства представляет функцию, которая принимает два параметра:</p>
<pre class="brush:ts;">
function MyPropertyDecorator(target: Object, propertyKey: string){
	// код декоратора
}
</pre>
<p>Где первый параметр представляет конструктор класса, если свойство статическое, либо прототип класса, если свойство нестатическое. А второй параметр 
представляет имя свойства.</p>
<p>Определим простейший декоратор для свойства:</p>
<pre class="brush:ts;">
...
<p>Декоратор format выполняет небольшое форматирование значение свойства. Для этого вначале мы получаем значение свойства. Создаем геттер, который возвращает отформатированное значение. 
Далее определяется сеттер, который устанавливает новое значение для свойства. И в конце удаляется старое свойство и создается новое с геттером и сеттером.</p>
<img src="./pics/30.png" alt="Декоратор свойств в TypeScript" /><h3>Декоратор метода доступа</h3>
<p>Декоратор метода доступа принимает три параметра:</p>
<pre class="brush:ts;">
function decorator(target: Object, propertyName: string, descriptor: PropertyDescriptor){ 
	// код декоратора
}
</pre>
<p>Первый параметр представляет конструктора класса для статического метода, либо прототип класса для обычного метода.<p></li>
<p>Второй параметр представляет название метода.<p>
<p>Третий параметр представляет объект PropertyDescriptor.<p>
<p>Определим простейший декоратор метода доступа:</p>
<pre class="brush:ts;">
...
<p>Декоратор validator переопределяет поведение сеттера с помощью свойства <code>descriptor.set</code>. Если передаваемое сеттеру значение представляет строку 
"admin", то генерируется ошибка.</p>
<p>Декоратор достаточно применить только к геттеру или к сеттеру, в любом случае он будет сразу применяться к обоим аксессорам.</p>
<p>И, к примеру, при вызове инструкции <code>tom.name= "admin";</code> мы столкнемся с ошибкой:</p>
<img src="./pics/32.png" alt="Accessor decorator in TypeScript" />
