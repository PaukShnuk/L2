...
<html >
<head>
<title>TypeScript | Null и undefined</title>
<meta charset="utf-8" />
<meta name="description" content="Типы данных Null и undefined в TypeScript, оператор !, параметр компиляции strictNullChecks">
...
<h2>Null и undefined</h2><div class="date">Последнее обновление: 15.05.2021</div>
...
<p>Как и в JavaScript, в TypeScript есть специальные типы <span class="b">undefined</span> и <span class="b">null</span>, которые принимают соответствующие 
значения undefined и null.</p>
<p>Использование <code>undefined</code> и <code>null</code> зависит от параметра компиляции <span class="b">strictNullChecks</span>. 
По умолчанию он имеет значение <code>false</code> и не используется, а это значит, что по умолчанию мы можем применять эти типы, как и в javascript:</p>
<pre class="brush:ts;">
let a: undefined = undefined;
let b: null = null;
</pre>
<p>Но фактически мы можем присваивать значения undefined и null переменным других типов, например, number:</p>
<pre class="brush:ts;">
...
<p>В этом плане null и undefined выступают как подтипы других типов и полезны преимущественно в каких-то операциях, где неизвестен результат - то ли это будет число или строка, то ли 
это будет null. В этом случае, чтобы избежать возможной ошибки, мы можем проверить значение на undefined или null, собственно как и в javascript.</p>
<p>В то же время отсутствие проверки для этих типов со стороны компилятора является потенциальным источников багов, поэтому нередко при компиляции 
применяется параметр <span class="b">strictNullChecks</span>. Либо при установке флага при компиляции в консоли:</p>
<div class="console"><pre class="consoletext">--strictNullChecks</pre></div>
<p>Либо с помощью установки значения <code>true</code> в файле конфигурации <span class="b">tsconfig.json</span>:</p>
<pre class="brush:js;">
{
    "compilerOptions": {
        "target": "es2015",
...
<p>В этом случае при попытке присвоить значение типа <code>undefined</code> или <code>null</code> переменной другого типа компилятор при компиляции 
выбросит ошибку. Например, при компиляции предыдущего примера кода:</p>
<pre class="brush:ts;">
let x: number = undefined;
console.log(x);
x = null;
...
<p>Мы получим следующие ошибки:</p>
<img src="./pics/44.png" alt="strictNullChecks и undefined и null в TypeScript" />
<p>Также мы получим ошибку, если значение <code>undefined</code> присвоим переменной типа <code>null</code> или значение <code>null</code> переменной типа <code>undefined</code>.</p>
<p>Тем не менее нередки ситуации, когда мы точно не знаем, имеет ли какая-то переменная или параметр функции или конкретное значение или оно отсутствует. 
...
чтобы переменная или параметр могли принимать значение <code>null</code>. И в этом случае можно использовать объединения. Например:</p>
<pre class="brush:ts;">
...
<p>В данном случае переменная <code>userId</code> представляет объединение <code>number|null</code>, поэтому этой переменной можно присвоить как конкретное числовое 
значение, так и значение <code>null</code>.</p>
<p>Аналогично функция <code>printId</code> через параметр <code>id</code> может принимать как значение <code>null</code>, так и числовое значение.</p>
<h3>Оператор !</h3>
<p>Оператор <span class="b">!</span> (non-null assertion operator) позволяет указать, что объект не представляет значение <code>null</code> и <code>undefined</code>. Так, 
возьмем следующий пример:</p>
<pre class="brush:ts;">
const header: HTMLElement|null = document.getElementById("header");
header.innerText = "Hello Typescript!";
</pre>
<p>Встроенная функция <code>document.getElementById()</code> возвращает элемент веб-страницы по id, который представляет тип <span class="b">HTMLElement|null</span>. 
То есть он может иметь значение <code>null</code>, если html-элемента с подобным id нет на веб-странице. 
Получив элемент, мы пытаемся с помощью его свойства <code>innerText</code> изменить его текстовое содержимое.</p>
<p>Вроде все нормально, однако умолчанию с включенной опцией <code>strictNullChecks</code> при компиляции мы получим ошибку:</p>
<img src="./pics/45.png" alt="non-null assertion operator в TypeScript" />
<p>Чтобы избежать ошибки, используем оператор !:</p>
<pre class="brush:ts;">
const header: HTMLElement|null = document.getElementById("header");
header!.innerText = "Hello Typescript!";
</pre>
<p>Оператор <span class="b">!</span> ставится после объекта, который теоретически может принимать значение <code>null</code> перед обращением к его свойствам и методам:</p>
<pre class="brush:ts;">
object!.property
object!.method()
</pre>
<p>В то же время надо учитывать, что этот оператор не меняет значения объекта. Например, если объект имеет значение null или undefined, то данный оператор не поможет. 
Программа скомпилируется, но при выполнении скрипта программа все равно сгенерирует ошибку:</p>
<pre class="brush:ts;">
const header: HTMLElement|null = null;
header!.innerText = "Hello Typescript!";
</pre>
<p>Поэтому рекомендуется применять данный оператор, когда мы знаем, что объект не равен null или undefined.</p>
...
<div class="nav"><p><a href="./2.13.php">Назад</a><a href="./">Содержание</a><a href="./2.7.php">Вперед</a></p></div></div>
<div class="socBlock">
