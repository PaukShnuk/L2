...
<html >
<head>
<title>TypeScript | Абстрактные классы, методы и поля</title>
<meta charset="utf-8" />
<meta name="description" content="Наследование в TypeScript, Абстрактные классы, методы и поля">
...
<h2>Абстрактные классы, методы и поля</h2><div class="date">Последнее обновление: 17.05.2021</div>
...
<p>Абстрактные классы представляют классы, определенные с ключевым словом <span class="b">abstract</span>. Они во многом похожи на обычные классы за тем 
исключением, что мы не можем создать напрямую объект абстрактного класса, используя его конструктор.</p>
<pre class="brush:ts;">
...
<p>Как правило, абстрактные классы описывают сущности, которые в реальности не имеют конкретного воплощения. Например, геометрическая фигура может представлять 
...
и от него унаследовать классы конкретных геометрических фигур:</p>
<pre class="brush:ts;">
abstract class Figure {
	getArea(): void{
...
<p>В данном случае абстрактный класс определяет метод <code>getArea()</code>, который вычисляет площадь фигуры. Класс прямоугольника определяет 
свою реализацию для этого метода.</p>
<h3>Абстрактные методы</h3>
<p>Однако в данном случае метод getArea в базовом классе не выполняет никакой полезной работы, так как у абстрактной фигуры не может быть площади. 
И в этом случае подобный метод лучше определить как <span class="b">абстрактный</span>:</p>
<pre class="brush:ts;">
abstract class Figure {
...
<p>Абстрактный метод  не определяет никакой реализации. Если класс содержит абстрактные методы, то такой класс должен быть абстрактным. Кроме того, 
при наследовании производные классы обязаны реализовать все абстрактные методы.</p>
<h3>Абстрактные поля</h3>
<p>Также абстрактный метод может иметь абстрактные поля, то есть поля определенные с модификатором <span class="b">abstract</span>. При наследовании класс-наследник 
также обязан предоставить для них реализацию:</p>
<pre class="brush:ts;">
abstract class Figure {
	abstract x: number;
	abstract y: number;
...
<p>В данном случае класс Figure определяет два абстрактных поля <code>x</code> и <code>y</code>, которые условно представляют начальную точку фигуры:</p>
<pre class="brush:ts;">
abstract x: number;
abstract y: number;
</pre>
<p>Класс Rectangle предоставляет для них реализацию с помощью определения полей через параметры конструктора:</p>
<pre class="brush:ts;">constructor(public x: number, public y: number, public width: number, public height: number)</pre>
<p>По большому счету в данном случае большого смысла в абстрактных полях нет, тем не менее TypeScript позволяет их применять.</p>
