...
<html >
<head>
<title>TypeScript | Функции</title>
<meta charset="utf-8" />
<meta name="description" content="Функции в TypeScript, параметры функции и возвращение результата, передача значений, значения параметров по умолчанию">
...
<h2>Функции</h2><div class="date">Последнее обновление: 15.05.2021</div>
...
<p>В javascript функции определяются с помощью ключевого слова <span class="b">function</span>, например:</p> 
<pre class="brush:ts;">
function add(a, b) {
	return a + b;
}
...
<p>TypeScript также определяет функцию с помощью ключевого слова <code>function</code>, но при этом добавляет дополнительные возможности по работе с функциями. В частности, теперь мы можем определить тип передаваемых параметров и тип 
возвращаемого значения.</p>
<h3>Параметры функции</h3>
<p>Функция может иметь параметры, которые указываются после названия функции в скобках через запятую. Через двоеточие после имени параметра указывается 
его тип:</p>
<pre class="brush:ts;">
// определение функции
...
<p>Однако поскольку параметры имеют тип number, то при вызове функции</p>
<pre class="brush:ts;">
add("1", "2");
</pre>
<p>компилятор TS выдаст ошибку, так как параметры должны иметь тип number, а не тип string.</p>
<p>При этом функция может не только использовать передаваемые параметры, но и глобальные переменные, определенные во вне:</p>
<pre class="brush:ts;">
let koef: number = 1.5;
...
<h3>Результат функции</h3>
<p>Функция может возвращать значение определенного типа, который еще называется типом функции. Возвращаемый тип функции ставится после списка параметров через двоеточие:</p>
<pre class="brush:ts;">
...
<p>В данном случае функция будет возвращать значение типа <code>number</code>.</p>
<p>Если функция ничего не возвращает, то указывается тип <span class="b">void</span>:</p>
<pre class="brush:ts;">
...
<p>В принципе мы можем и не указывать тип, тогда он будет выводиться неявно на основе возвращаемого значения:</p>
<pre class="brush:ts;">
function add(a: number, b: number) {
    return a + b;
}
...
<p>В typescript при вызове в функцию должно передаваться ровно столько значений, сколько в ней определено параметров:</p>
<pre class="brush:ts;">
...
<p>Чтобы иметь возможность передавать различное число значений в функцию, в TS некоторые параметры можно объявить как необязательные. Необязательные параметры должны быть помечены вопросительным знаком <code>?</code>. Причем необязательные параметры 
должны идти после обязательных:</p>
<pre class="brush:ts;">
function getName(firstName: string, lastName?: string) {
    if (lastName)
        return firstName + " " + lastName;
...
<p>Во втором случае, когда в функцию передается только имя, второй используемый параметр будет иметь неопределенное значение 
или "undefined". Поэтому с помощью условной конструкции проверяется наличие значения для этого параметра.</p>
<h3>Значения параметров по умолчанию</h3>
<p>Параметры позволяют задать начальное значение по умолчанию. И если для такого параметра не передается значение, то он использует значение по умолчанию:</p>
<pre class="brush:ts;">
...
<p>Причем в качестве значения можно передавать результат другого выражения:</p>
<pre class="brush:ts;">
...
<div class="nav"><p><a href="./2.4.php">Назад</a><a href="./">Содержание</a><a href="./2.3.php">Вперед</a></p></div></div>
<div class="socBlock">
