...
<html >
<head>
<title>TypeScript | Фабрики декораторов</title>
<meta charset="utf-8" />
<meta name="description" content="Фабрики декораторов decorator factory в TypeScript, передача в декораторы параметров, декоратор валидации через регулярное выражение">
...
<h2>Фабрики декораторов</h2><div class="date">Последнее обновление: 20.05.2021</div>
...
<p>Декоратор класса, свойства, метода представляет обычную функцию, которая принимает заданное количество параметров. Но что если мы хотим 
...
фабрику декораторов (factory decorator). Фабрика декоратора представляет функцию, которая в свою очередь возвращает функцию декоратора.</p>
<p>Например, определим протейшую функцию декоратора:</p>
<pre class="brush:ts;">
...
<p>Декоратор regex принимает в качестве параметра регулярное выражение и при этом возвращает функцию декоратора свойства. В декораторе в сеттере 
при установке для свойства нового значения проверяется соответствие нового значения регулярному выражению. Если нет соответствия, то генерируется ошибка.</p>
<p>Далее в классе Account к его свойствам email и phone применяется декоратор regex, но в каждом случае в декоратор передается свое регулярное выражение, с помощью 
которого можно произвести проверку:</p>
<img src="./pics/34.png" alt="Decorator factory in TypeScript" />
