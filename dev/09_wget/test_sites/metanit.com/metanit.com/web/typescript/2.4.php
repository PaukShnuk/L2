...
<html >
<head>
<title>TypeScript | Type assertion. Преобразование к типу</title>
<meta charset="utf-8" />
<meta name="description" content="Type assertion. Преобразование к типу данных в TypeScript, оператор as">
...
<h2>Type assertion. Преобразование к типу</h2><div class="date">Последнее обновление: 15.05.2021</div>
...
<p>Type assertion представляет модель преобразования значения переменной к определенному типу. Обычно в некоторых ситуациях одна переменная может 
представлять какой-то широкий тип, например, <code>any</code> или <code>union</code>, которые по факту допускают значения различных типов. Однако при этом нам надо использовать переменную 
как значение строго определенного типа. И в этом случае мы можем привести к этому типу.</p>
<p>В качестве примера возьмем простейшую задачу - на веб-странице есть html-элемент с <code>id = header</code>, и мы хотим получить этот элемент, например, 
чтобы изменить его текст. Для получения элемента по id мы могли бы использовать встроенную js-функцию <code>document.getElementById()</code>:</p>
<pre class="brush:ts;">
const header = document.getElementById("header");
header.innerText = "Hello Typescript!";
</pre>
<p>Но в TypeScript эта функция возвращает объединение <code>HTMLElement|null</code>. То есть возвращаемое значение может представлять <code>null</code>, если 
соответствующий html-элемент отсутствует на веб-странице. Вследствие этого обращение к свойству <code>header.innerText</code> - к свойству объекта, который может быть <code>null</code>, 
содержит потенциальную ошибку. Поэтому компилятор при компиляции сгенерирует ошибку.</p>
<p>Однако ситуация может быть такова, что мы точно знаем, что у нас на странице есть такой элемент. Есть разные способы для решения этой проблемы, чтобы указать компилятору, 
что все нормально. И одним из ним является приведение типов с помощью <code>type assertion</code>.</p>
<p>Есть две формы приведения. Первая форма заключается в использовании угловых скобок:</p>
<pre class="brush:ts;">
const header = &lt;HTMLElement&gt;document.getElementById("header");
header.innerText = "Hello Typescript!";
</pre>
<p>Перед значением в угловых скобках указывается тип, к которому надо выполнить приведение. Так, в данном случае мы получаем объект типа <code>HTMLElement</code></p>
<p>Вторая форма заключается в применении оператора <span class="b">as</span>, после которого указывается тип, к которому надо выполнить преобразование:</p>
<pre class="brush:ts;">
const header = document.getElementById("header") as HTMLElement;
header.innerText = "Hello Typescript!";
</pre>
<p>Однако такие преобразования будут иметь силу, если мы точно знаем, что значение может быть преобразовано к целевому типу. Например, на странице есть элемент с 
id=header, поэтому мы можем преобразовать значение к типу HTMLElement. Однако если такого элемента нет, то во время выполнения мы опять же получим ошибку.</p>
...
<div class="nav"><p><a href="./2.14.php">Назад</a><a href="./">Содержание</a><a href="2.9.php">Вперед</a></p></div></div>
<div class="socBlock">
