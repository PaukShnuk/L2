...
<html >
<head>
<title>TypeScript | Массивы</title>
<meta charset="utf-8" />
<meta name="description" content="Массивы в TypeScript, определение массивов, тип Array&lt;T&gt; и ReadonlyArray, массив только для чтения">
...
<h2>Массивы</h2><div class="date">Последнее обновление: 15.05.2021</div>
...
<p>Массивы определяются с помощью выражения <code>[]</code> и также являются строго типизированными. То есть если изначально массив 
содержит строки, то в будущем он сможет работать только со строками.</p>
<pre class="brush:ts;">
...
<p>Как и в JavaScript, с помощью индексов можно обращаться к элементам массива.</p>
<p>Альтернативный способ определения массивов представляет применение типа <code>Array&lt;&gt;</code>, где в фигурных скобках указывается тип элементов массива:</p>
<pre class="brush:ts;">
let names: Array&lt;string&gt; = ["Tom", "Bob", "Alice"];
console.log(names[1]);	// Bob
</pre>
<p>Но фактически такие формы массивов, как <code>number[]</code> или <code>string[]</code> являются сокращением соответственно типов 
<code>Array&lt;number&gt;</code> или <code>Array&lt;string&gt;</code></p>
<p>В остальном массивы в TypeScript поддерживают все те же операции, что и <a href="https://metanit.com/web/javascript/5.3.php">массивы в JavaScript</a>.</p>
<h3>ReadonlyArray</h3>
<p>Массивы позволяют изменять значения своих элементов:</p>
<pre class="brush:ts;">
const people = ["Tom", "Bob", "Sam"];
people[1] = "Kate";
console.log(people[1]);	// Kate
</pre>
<p>Но TypeScript также позволяет определять массивы, элементы которых нельзя изменять. Для этого применяется тип <span class="b">ReadonlyArray&lt;&gt;</span>, 
для которого в угловых скобках указывается тип элементов массива.</p>
<p>В отличие от типа <code>Array</code> для типа <code>ReadonlyArray</code> мы не можем принимать конструктор, как в следующем случае:</p>
<pre class="brush:ts;">
const people: ReadonlyArray&lt;string&gt; = new ReadonlyArray("Tom", "Bob", "Sam");
</pre>
<p>Вместо этого необходимо передать значения в виде обычного массива:</p>
<pre class="brush:ts;">
const people: ReadonlyArray&lt;string&gt; = ["Tom", "Bob", "Sam"];
</pre>
<p>Для определения подобных массивов также можно использовать сокращение типа - <code>readonly Тип_данных[]</code>:</p>
<pre class="brush:ts;">const people: readonly string[]= ["Tom", "Bob", "Sam"];</pre>
<p>Массив ReadonlyArray поддерживаtт большинство тех же операций, что и обычные массивы, за тем исключением операций, которые изменяют массив и его элементы.
Так, мы не можем менять отдельные значения:</p>
<pre class="brush:ts;">
const people: ReadonlyArray&lt;string&gt; = ["Tom", "Bob", "Sam"];
people[1] = "Kate";		// ! Ошибка элементы массива ReadonlyArray нельзя изменить
</pre>
<p>Также мы не можем добавлять новые или удалять уже имеющиеся элементы:</p>
<pre class="brush:ts;">
const people: ReadonlyArray&lt;string&gt; = ["Tom", "Bob", "Sam"];
...
<p>Более того при компиляции компилятор сообщит нам, что для типа ReadonlyArray не определены методы <code>push()</code> и <code>pop()</code>. 
Это относится ко всем операциям, которые изменяют массив.</p>
<p>Все остальные операции, которые предусматривают чтение массива, мы по прежнему можем использовать:</p>
<pre class="brush:ts;">
...
<h3>Декомпозиция массивов</h3>
<p>Как и в javascript, массивы поддерживают декомпозицию на константы и переменные. Например:</p>
<pre class="brush:ts;">
...
<p>Здесь массив people раскладывается на три константы - first, second, third. Значения массива передаются константам/переменным по позиции, то есть первой константе - 
первый элемент массива, второй константе - второй элемент и так далее. Причем констант/переменных не больше, чем элементов массива.</p>
<p>С помощью оператора <span class="b">...</span> можно указать массив, в который будут помещаться все оставшиеся элементы раскладываемого массива, 
которые не вошли в предыдущие переменные/константы:</p>
<pre class="brush:ts;">
const people: string[]= ["Tom", "Bob", "Sam"];
...
<p>Здесь константа <code>first</code> принимает первый элемент массива - "Tom". Все оставшиеся элементы кортежа будут помещаться в массив 
<code>rest</code>.</p>
<p>Также можно оставить пустое место вместо переменной/константы, если мы хотим пропустить соответствующий элемент массива:</p>
<pre class="brush:ts;">
...
<div class="nav"><p><a href="./2.4.php">Назад</a><a href="./">Содержание</a><a href="./2.10.php">Вперед</a></p></div></div>
<div class="socBlock">
