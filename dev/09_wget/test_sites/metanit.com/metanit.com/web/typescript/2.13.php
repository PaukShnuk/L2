...
<html >
<head>
<title>TypeScript | Объединения union</title>
<meta charset="utf-8" />
<meta name="description" content="Тип данных union (объединения) в TypeScript, проверка типа данных">
...
<h2>Объединения union</h2><div class="date">Последнее обновление: 15.05.2021</div>
...
<p>Объединения или union не являются собственно типом данных, но они позволяют комбинировать или объединить другие типы. 
Так, с помощью объединений можно определить переменную, которая может хранить значение двух или более типов:</p>
<pre class="brush:ts;">
...
<p>Чтобы определить все типы, которые должно представлять переменная, все эти типы разделяются прямой чертой: <code>number | string</code>. 
В данном случае переменная <code>id</code> может представлять как тип <code>string</code>, то есть строку, так и число.</p>
<p>Подобным образом можно использовать union для определения параметров функции:</p>
<pre class="brush:ts;">
function printId(id: number|string){
	console.log(`Id: ${id}`);
}
</pre>
<p>Здесь функция <code>printId()</code> в качестве параметра id может принимать как значения типа <code>number</code>, так и значения типа <code>string</code>.</p>
<pre class="brush:ts;">
function printId(id: number|string){
	console.log(`Id: ${id}`);
}
...
<p>Тип union подходит для тех ситуаций, когда логика работы со всеми объединенными типами однообразна, например, как в случае выше, где значение 
извне встраивается в строку и выводится на консоли. Вне зависимости от типа действия одинаковы.</p>
<p>Однако иногда логика может различаться в зависимости от переданного типа. В этом случае можно использовать проверку на тип, чтобы разграничить логику 
для различных типов:</p>
<pre class="brush:ts;">
...
<p>В данном случае функция <code>printSentence()</code> может принимать как строку, так и массив строк, чтобы затем вывести строки на консоль. 
И в зависимости от типа данных действия отличаются. Так, для массива применяется метод <code>words.join(" ")</code>, который объединяет все элементы 
массива в одну строку, разделенные пробелом.</p>
...
<div class="nav"><p><a href="./2.3.php">Назад</a><a href="./">Содержание</a><a href="./2.8.php">Вперед</a></p></div></div>
<div class="socBlock">
