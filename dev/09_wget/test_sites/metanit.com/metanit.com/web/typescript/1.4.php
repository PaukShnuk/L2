...
<html >
<head>
<title>TypeScript | Настройки компиляции</title>
<meta charset="utf-8" />
<meta name="description" content="Параметры компиляции в TypeScript, настройка версии ECMAScript, установка настроек в Visual Studio и в командной строке">
...
<h2>Настройки компиляции</h2><div class="date">Последнее обновление: 09.05.2021</div>
...
<p>При компиляции файлов TypeScript из командной строки компилятор позволяет установить ряд конфигурационных настроек. Для установки нам доступна куча различных настроек, 
полный список которых можно найти на странице <a href="http://www.typescriptlang.org/docs/handbook/compiler-options.html" rel="nofollow">Параметры компиляции в TypeScript</a>. 
Рассмотрим лишь основные из них.</p>
<h3>Автоматическая перекомпиляция</h3>
<p>Опция <span class="b">--watch</span>, а также ее сокращенная версия <span class="b">-w</span> автоматически перекомпилирует файлы typescript, 
если в них были внесены какие-либо изменения. Благодаря чему не надо при каждом малейшем изменении вручную вводить команду в консоль для перекомпиляции.</p>
<div class="console"><pre class="consoletext">tsc -w app.ts</pre></div>
<h3>Версия ECMAScript</h3>
<p>С помощью параметра <span class="b">–-target</span> или его сокращенной версии <span class="b">–t</span> можно задать версию стандарта 
JavaScript, в которую будет компилироваться код TypeScript. Этот параметр может принимать следующие значения: 
"ES3" (по умолчанию), "ES5", "ES6" / "ES2015",  "ES7" / "ES2016", "ES2017", "ES2018", "ES2019", "ES2020" или "ESNext":</p>
<div class="console"><pre class="consoletext">tsc app.ts -t ES5</pre></div>
<h3>Удаление комментариев</h3>
<p>По умолчанию в файлы javascript переходят все комментариии, которыми снабжен код в файлах TS. Удаление комментариев при компиляции осуществляется с 
помощью параметра <span class="b">–-removeComments</span>:</p>
<div class="console"><pre class="consoletext">tsc app.ts --removeComments</pre></div>
<h3>Установка каталога</h3>
<p>С помощью параметра <span class="b">--outDir</span> можно задать папку для хранения скомпилированных файлов js:</p>
<div class="console"><pre class="consoletext">tsc --outDir D:\ts\js app.ts</pre></div>
<p>В данном случае скомпилированный файл app.js окажется в папке <span class="ii">D:\ts\js</span></p>
<h3>Объединение файлов</h3>
<p>Если у нас несколько файлов TS, то с помощью параметра <span class="b">--outFile</span> их можно объединить в один файл js:</p>
<div class="console"><pre class="consoletext">tsc --outFile output.js app.ts hello.ts</pre></div>
<p>Здесь файлы app.ts и hello.ts скомпилируются в один файл output.js</p>
<h3>Тип модуля</h3>
<p>С помощью параметра <span class="b">--module</span>, либо <span class="b">-m</span> можно указать тип модуля, который будет использоваться для компиляции. 
Эта опция может принимать следующие значения: "None", "CommonJS" (значение по умолчанию, если задана версия ECMAScript "ES3" или "ES5"), "AMD", "System", "UMD", "ES2015", "ES2020" и "ESNext".</p>
<div class="console"><pre class="consoletext">tsc -m commonjs app.ts</pre></div>
<h3>Несколько параметров</h3>
<p>Если надо задать несколько параметров, то они и их значения последовательно перечисляются через пробел.</p>
<div class="console"><pre class="consoletext">tsc -t ES5 --outDir js -m commonjs app.ts</pre></div>
<h3>Вызов справки</h3>
<p>И чтобы посмотреть все доступные параметры и справку по ним, можно воспользоваться параметром <span class="b">-h</span>:</p>
<div class="console"><pre class="consoletext">tsc -h</pre></div>
