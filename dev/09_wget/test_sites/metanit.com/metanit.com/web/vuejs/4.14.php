...
<html >
<head>
<title>Vue.js | Миксины</title>
<meta charset="utf-8" />
<meta name="description" content="Наследование компонентов и миксины во Vue.js, определение глобальных миксинов">
...
<h2>Миксины</h2><div class="date">Последнее обновление: 12.10.2017</div>
...
<img src="./pics/57.png" alt="Mixin in Vue.js" />
...
<p>Но в данном случае компоненты содержат таже частично повторяемый шаблон - две кнопки. И установку шаблона мы можем также перенести в миксин. Для 
этого воспользуемся событие жизненного цикла <span class="b">created</span>, которое вызывается после создания компонента и миксина.</p>
<p>При этом стоит учитывать, что вначале вызваются события жизненного цикла миксина, а потом события жизненного цикла компонента, который использует данный миксин. 
В частности, перепишем код миксина и компонентов:</p>
<pre class="brush:js;">
var toggleMixin = { 
	data: function(){
		return { state: true, on: "ON", off: "OFF"}
...
<p>С помощью значения <span class="b">this.$options.template</span> мы можем получить в миксине шаблон компонента и изменить его. В итоге 
результат работы компонентов будет тот же, что и ранее.</p>
<h3>Глобальные миксины</h3>
<p>Vue.js позволяет задавать миксины глобально, то есть для всех компонентов. Для этого используется функция <span class="b">Vue.mixin</span>:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
	&lt;message&gt;&lt;/message&gt;
&lt;/div&gt;
...
<img src="./pics/58.png" alt="Global mixin in Vue.js" />
<p>Поскольку в данном случае определен один компонент и объект Vue, то глобальный миксин будет вызываться два раза.</p>
