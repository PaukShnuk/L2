...
<html >
<head>
<title>Vue.js | Компоненты</title>
<meta charset="utf-8" />
<meta name="description" content="Компоненты во Vue.js, создание и рендеринг компонентов">
...
<h1>Компоненты</h1><h2>Создание компонентов</h2><div class="date">Последнее обновление: 10.09.2017</div>
...
<p>Компоненты представляют элементы, к которым компилятор Vue прикрепляет некоторое поведение. Компоненты позволяют инкапсулировать код и затем использовать 
его многократно в различных частях приложения.</p>
<p>Для создания компонента используется функция <span class="b">Vue.component(tagName, options)</span>, где параметр <span class="b">tagName</span> - кастомный элемент html, 
который будет представлять компонент, а параметр <span class="b">options</span> представляет конфигуацию компонента.</p>
...
<p>Здесь определен компонент для элемента hello. И неважно, что такого элемента в html нет, мы его сами создаем. Более того мы не можем использовать для компонента 
встроенные элементы html типа div или h2. То есть по сути компонент называется "hello".</p>
<p>В объекте options, который передается компоненту, через свойство template можно задать html-разметку, которую будет содержать компонент. В итоге данная разметка будет вставляться 
вместо элемента <code>&lt;hello&gt;&lt;/hello&gt;</code>.</p>
<p>При этом компонент должен быть определен до элемента Vue, в котором он используется.</p>
<p>И при рендеринге страницы вместо элемента hello будет вставлено содержимое компонента:</p>
<img src="./pics/35.png" alt="Введение в компоненты в Vue.js" />
