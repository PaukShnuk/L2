...
<html >
<head>
<title>Vue.js | Управление массивами</title>
<meta charset="utf-8" />
<meta name="description" content="Управление массивами в Vue.js, добавление и удаление элементов, изменение и фильтрация массива">
...
<h2>Управление массивами</h2><div class="date">Последнее обновление: 29.08.2017</div>
...
<p>Во Vue.js для каждого массива определено ряд методов, с помощью которых можно управлять элементами массива:</p>
<ul>
<li><p><span class="b">push()</span></p></li>
<li><p><span class="b">pop()</span></p></li>
<li><p><span class="b">shift()</span></p></li>
<li><p><span class="b">unshift()</span></p></li>
<li><p><span class="b">splice()</span></p></li>
<li><p><span class="b">sort()</span></p></li>
<li><p><span class="b">reverse()</span></p></li>
</ul>
<p>Эти методы являются обертками над одноименными <a href="https://metanit.com/web/javascript/5.3.php">стандартными методами</a> javascript для управления массивами и работают точно также. 
Единственное отличие их от стандартных методов состоит в том, что эти методы-обертки информируют систему Vue.js о том, что с массивом были произведены некоторые действия, и 
соответственно для этого массива может быть произведен повторный рендеринг на веб-странице.</p>
<p>Но кроме выше описанных методов, которые изменяют отдельные элементы массив, есть ряд методов в javascript, которые возвращают новый массив, типа 
<code>filter()</code>, <code>concat()</code>, <code>slice()</code>. Результат таких функций лучше привязывать к вычисляемому свойству, которое позволит произвести повторный рендеринг 
элементов веб-страницы.</p>
<h3>Добавление и удаление</h3>
<p>Определим код для добавления нового элемента в массив и удаления из массива:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>При нажатии на кнопку в массив будет добавлять введенное в текстовое поле значение, которое доступно через свойство newPhone. И в итоге произойдет 
обновление списка на веб-странице, и мы увидим добавленный элемент.</p>
<p>И кроме того, для каждого элемента предусмотрена кнопка, через которую по индексу можно удалить элемент из массива.</p>
<img src="pics/27.png" alt="Управление массивом в Vue.js" />
<h3>Установка элемента в массиве</h3>
<p>При работе с массивами мы можем столкнуться с некоторыми ограничениями. В частности, мы не можем просто по индексу переустановить элемент массива, 
присвоив ему новое значение:</p>
<pre class="brush:js;">this.phones[1] = 'Samsung Galaxy S8';</pre>
<p>Для установки значения нам надо использовать метод <span class="b">Vue.set()</span>:</p>
<pre class="brush:js;">Vue.set(массив, индекс_элемента, новое_значение);</pre>
<p>Например, обновим второй элемент массива:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
	&lt;ul&gt;
...
<h3>Возвращаение нового массива</h3>
<p>Ряд методов, такие как slice, concat, filter, не изменяют текущий массив, а возвращают новый. Одно из решений может заключаться в переустановке массива.  Например, 
используем метод slice, который возвращает часть массива:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
	&lt;ul&gt;
	  &lt;li v-for="phone in phones"&gt;
...
<p>Для фиксации нового массива присваиваем полученный массив свойству phones.</p>
<p>Однако данный способ не всегда является оптимальным. Особенно если мы хотим сохранить старый массив и изменять лишь его визуальное представление. 
И более идеальным вариантом, как правило, является разделение данных и представления этих данных. Для представления 
данных обычно определяется вычисляемое свойство-список, элементы которого выводятся на веб-страницу:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
...
<img src="pics/28.png" alt="Создание нового массива в Vue.js" />
