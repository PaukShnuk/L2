...
<html >
<head>
<title>Vue.js | Слоты с ограниченной областью видимости</title>
<meta charset="utf-8" />
<meta name="description" content="Слоты с ограниченной областью видимости (scoped slots) во Vue.js, передача данных из дочернего компонента в родительский">
...
<h2>Слоты с ограниченной областью видимости</h2><div class="date">Последнее обновление: 13.10.2017</div>
...
<p>Для определения шаблона в родительском компоненте применяется какой-нибудь элемент, как правило, это элемент <code>&lt;template&gt;</code>, 
в котором устанавливается специальный атрибут <span class="b">slot-scope</span>. Значение этого атрибута - название объекта с данными, 
...
<p>Для слота с ограниченной областью видимости здесь определен шаблон:</p>
<pre class="brush:xml;">
&lt;template slot-scope="props"&gt;
	&lt;h3&gt;Данные о пользователе&lt;/h3&gt;
	&lt;p&gt;Имя: {{ props.username }}&lt;/p&gt;
...
<p>К примеру, дочерний компонент вывод некоторые данные о пользователе. И через элемент template эти данные будут выводиться в слот.</p>
<p>Для получения данные в элементе определен атрибут <code>slot-scope="props"</code>. Props - это тот объект с данными, который передается от дочернего компонента.</p>
<p>Внутри шаблона мы можем обращаться через props к переданным данным.</p>
<p>Для определения данных в дочернем компоненте устанавливается элемент <code>&lt;slot&gt;</code> с соответствующими атрибутами:</p>
<pre class="brush:xml;">
&lt;div class="user"&gt;
	&lt;slot username="Tom" userage="31"&gt;&lt;/slot&gt;
&lt;/div&gt;
</pre>
<img src="./pics/62.png" alt="Scoped slots in Vue.js" />
...
<p>В данном случае компонент userslist получает извне список users. Для каждого элемента из этого списка создается свой элемент slot:</p>
<pre class="brush:xml;">
&lt;slot v-for="user in users" :username="user"&gt;&lt;/slot&gt;
</pre>
<p>Для слота определяется одно свойство - username.</p>
<p>В родительском компоненте определяется шаблон, в котором выводится значение свойсва username:</p>
<pre class="brush:xml;">
&lt;li slot-scope="props" class="user"&gt;
		{{ props.username }}
&lt;/li&gt;
...
<img src="./pics/63.png" alt="List in scoped slots in Vue.js" />
