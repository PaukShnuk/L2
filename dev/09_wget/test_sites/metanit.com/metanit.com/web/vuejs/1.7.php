...
<html >
<head>
<title>Vue.js | Шаблоны template</title>
<meta charset="utf-8" />
<meta name="description" content="Шаблоны в Vue.js и параметр template">
...
<h2>Шаблоны</h2><div class="date">Последнее обновление: 29.08.2017</div>
...
<p>Для работы с веб-страницей vue.js использует шаблоны. Шаблоны в Vue.js представляют валидную разметку с кодом html, которая 
компилируется в функции рендеринга виртуального DOM. Внутри шаблона может использоваться состояние приложения, то есть свойства объекта Vue, определенные через параметр <span class="b">data</span>.</p>
<p>Возьмем следующую страницу:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь все, что внутри элемента <code>&lt;div id="app"&gt;</code>, представляет шаблон Vue. То есть в данном случае шаблоном является код:</p>
<pre class="brush:xml;">&lt;p&gt;Имя: {{name}}   Возраст {{age}}&lt;/p&gt;</pre>
<p>Vue.js управляет именно этой частью веб-страницы. Вне шаблона Vue не работает.</p>
<p>Также шаблон можно задать через параметр <span class="b">template</span> в объекте Vue:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае результат будет тот же самый.</p>
