...
<html >
<head>
<title>Vue.js | Определение маршрутов</title>
<meta charset="utf-8" />
<meta name="description" content="Маршрутизация в Vue.js, определение маршрутов, объект VueRouter, компонент router-view">
...
<h1>Маршрутизация</h1><h2>Определение маршрутов</h2><div class="date">Последнее обновление: 29.10.2017</div>
...
<p>Vue.js имеет полноценную систему маршрутизации, которая позволяет сопоставлять запросы к приложению с определенными компонентами. 
За работу системы маршрутизации во Vue.js отвечает специальная библиотека - <span class="b">vue-router</span>.</p>
<p>Итак, создадим приложение, которое будет использовать маршруты. При этом следует учитывать, что чтобы использовать маршрутизацию в приложении 
Vue.js, необходимо запускать приложение на сервере, то есть просто кинуть страницу с кодом в браузер не получится. Поэтому для запуска будем использовать 
Node.js, как наиболее демократичный вариант.</p>
<p>Вначале создадим каталог на жестком диске, где будет размещаться проект. Добавим в этот каталог новый файл <span class="b">package.json</span>:</p>
<pre class="brush:js;">
{
    "name": "routerapp",
...
<p>Данный файл просто определяет пакет lite-server, который будет необходим для запуска приложения. Но вообще при желании можно использовать и другие веб-серверы, 
например, Apache, IIS и т.д.</p>
<p>Затем определим в каталоге проекта файл <span class="b">index.html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<pre class="brush:xml;">&lt;script src="https://unpkg.com/vue-router/dist/vue-router.js"&gt;&lt;/script&gt;</pre>
<p>В коде javascript вначале определяются несколько компонентов, которые будут обрабатывать запросы по определенным путям:</p>
<pre class="brush:js;">
const NotFound = { template: '&lt;h2&gt;Page Not Found&lt;/h2&gt;' }
const Home = { template: '&lt;h2&gt;Home Page&lt;/h2&gt;' }
const About = { template: '&lt;h2&gt;About Page&lt;/h2&gt;' }
...
запросы по пути "/about". А компонент NotFound будет обрабатывать все остальные пути, для этого для него свойство path имеет в качестве значения 
"*". Причем когда приложение получит запрос, то этот запрос будет последовательно сопоставляться со всеми маршрутами. Первый маршрут, у которого свойство path 
совпадет с путем запроса и будет выбран для обработки.</p>
<p>Затем создается объект маршрутизатора <span class="b">VueRouter</span>:</p>
<pre class="brush:js;">
const router = new VueRouter({
    mode: 'history',
    routes: routes
...
<p>У этого объекта устанавливаются два свойства. Свойство <span class="b">mode</span> указывает на используемый режим навигации. В частности, 
значение "history" применяет history.pushState API, которое позволяет использовать навигацию без перезагрузки страницы.</p>
<p>Далее свойство <span class="b">routes</span> устанавливает маршруты - это выше определенные маршруты.</p>
<p>И в конце объект маршрутизатора передается в объект Vue. И кроме того, в шаблоне объекта Vue помещается компонент router-view:</p>
<pre class="brush:xml;">&lt;router-view&gt;&lt;/router-view&gt;</pre>
<p>В этот элемент и будет помещаться выбранный для обработки запроса компонент.</p>
<p>И в конце перейдем к командной строке с помощью команды <span class="b">cd</span> к папке проекта и для установки пакета lite-server 
выполним команду</p>
<div class="console"><pre class="consoletext">npm install</pre></div>
<p>После ее выполнения запустим проект, введя в консоль команду</p>
<div class="console"><pre class="consoletext">npm start</pre></div>
<p>По умолчанию будет идти запрос к корню приложению, поэтому такой запрос будет обрабатываться компонентом Home:</p>
<img src="./pics/router1.png" alt="Маршрутизация в Vue.js" />
<p>А если мы обратимся по пути "/about", то запрос будет обработан компонентом About:</p>
<img src="./pics/router2.png" alt="Маршруты и VueRouter в Vue.js" />
<p>Запросы по остальным путям будет обрабатывать компонент NotFound.</p>
