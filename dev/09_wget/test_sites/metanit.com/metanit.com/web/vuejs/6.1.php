...
<html >
<head>
<title>Vue.js | Локальные и глобальные фильтры</title>
<meta charset="utf-8" />
<meta name="description" content="Локальные и глобальные фильтры во Vue.js, их определение и использование">
...
<h1>Фильтры</h1><h2>Локальные и глобальные фильтры</h2><div class="date">Последнее обновление: 21.10.2017</div>
...
<p>Фильтры позволяют отформатировать текст определенным образом перед его выводом на веб-страницу, то есть осуществляют предобработку значений.</p>
<p>Фильтры могут быть глобальными, то есть использоваться для любого компонента в приложении. И также фильтры могут быть локальными, 
то есть определенными только для одного компонента.</p>
<h3>Глобальные фильтры</h3>
<p>Для определения глобального фильтра во Vue.js вызывается метод <span class="b">Vue.filter</span>, который имет следующий формальный синтаксис:</p>
<pre class="brush:js;">
...
<p>Первый параметр представляет название фильтра. Второй параметр - функция, которая в качестве параметра принимает обрабатываемое значение. В самой 
функции некоторый образом обрабатывается это значение, из него создается новое значение, которое затем возвращается в качестве результата.</p>
<p>После определения фильтра его можно применить для форматирования какого-либо определенного значения. Это можно сделать в выражении интерполяции:</p>
<pre class="brush:js;">{message | filter}</pre>
<p>Или в выражениях атрибута <span class="b">v-bind</span>:</p>
<pre class="brush:js;">v-bind="message | filter"</pre>
<p>Также можно принять фильтр</p>
<p>Например, создадим фильтр, который будет делать первую букву заглавной:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае фильтр называется <code>capitalize</code>. Функция в фильтре вырезает первую букву переданного слова, делает ее заглавной и 
соединяет с остальной частью слова.</p>
<p>Для применения фильтра после определенного значения ставится прямая вертикальная черта, после которой идет название фильтра:</p>
<pre class="brush:js;">{{message | capitalize }}</pre>
<h3>Локальные фильтры</h3>
<p>Локальные фильтры определяются в компоненте через свойство <span class="b">filters</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь компонент message имеет параметр filters, который представляет объект. В этом объекте фильтры определяются в виде 
функцию, которые возвращают отформатированное значение. В данном случае определен только один фильтр uppercase, который просто возвращает строку с 
символами в верхнем регистре. И внутри компонента мы можем применять данный фильтр. Однако вне компонента мы не можем его применить, так как фильтр локальный. 
И в частности, применение фильтра в следующей строке:</p>
<pre class="brush:xml;">&lt;h2&gt;{{header | uppercase }}&lt;/h2&gt;</pre>
<p>работать не будет. И браузер в консоли выведет нам предупреждение, хотя ошибки в этом случае также не будет.</p>
<img src="./pics/64.png" alt="Фильтры во Vue.js" />
...
<div class="nav"><p><a href="5.3.php">Назад</a><a href="./">Содержание</a><a href="7.1.php">Вперед</a></p></div></div>
<div class="socBlock">
