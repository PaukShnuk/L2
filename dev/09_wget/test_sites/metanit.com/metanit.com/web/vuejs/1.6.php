...
<html >
<head>
<title>Vue.js | Вычисляемые свойства</title>
<meta charset="utf-8" />
<meta name="description" content="Вычисляемые свойства в Vue.js и параметр computed, отличие от функций">
...
<h2>Вычисляемые свойства</h2><div class="date">Последнее обновление: 06.08.2017</div>
...
<p>Кроме обычных свойств объект Vue может содержать вычисляемые свойства, который во многом аналогичны функциям, но в то же время отличаются от них. Рассмотрим небольшой пример:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь в зависимости от возраста пользователя функция checkAge возвращает некоторый результат. И при каждом изменении значения свойства age, функция checkAge будет пересчитывать свой результат. 
...
<img src="./pics/11.png" alt="Методы во Vue.js" />
...
<p>Здесь enabled представляет вычисляемое свойство. Его определение во многом аналогично методу checkAge. Но теперь при изменении состояния Vue будут анализироваться 
сделанные изменения, и если потребуется, то свойство enabled будет повторно вычисляться. Поэтому в данном случае, если мы изменим свойство age, изменится свойство enabled и повторно выполнится функция checkAge. 
Но если изменится свойство name, то свойство enabled не будет изменяться:</p>
<img src="./pics/12.png" alt="Computed in Vue.js" />
<h3>Сеттеры</h3>
...
<p>На уровне кода геттер задается через параметр <span class="b">get</span>, который представляет функцию, возвращающую значение. 
А сеттер представляет функцию, задаваемую через параметр <span class="b">set</span>. Причем каждое новое значение передается через параметр этой 
...
<div class="nav"><p><a href="1.5.php">Назад</a><a href="./">Содержание</a><a href="1.8.php">Вперед</a></p></div></div>
<div class="socBlock">
