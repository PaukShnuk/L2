...
<html >
<head>
<title>Vue.js | Именованные слоты</title>
<meta charset="utf-8" />
<meta name="description" content="Именованные слоты (named slots) во Vue.js, передача данных из дочернего компонента в родительский">
...
<h2>Именованные слоты</h2><div class="date">Последнее обновление: 13.10.2017</div>
...
<p>Каждому слоту можно назначить определенное имя и через это имя передавать в него некоторое содержимое из родительского компонента. 
...
<p>Компонент container определяет ряд слотов. Причем один из слотов является обычным, а остальные два - именованными. Именованные слоты как и обычные 
могут иметь содержимое по умолчанию:</p>
<pre class="brush:xml;">
&lt;div&gt;
	&lt;header&gt;
		&lt;slot name="header"&gt;Заголовок по умолчанию&lt;/slot&gt;
...
<p>В родительском компоненте для слотов передается содержимое, которое также помещается внутрь дочернео компонента. 
Причем для передачи контента для именованных слотов html-элементы должны определять атрибут <code>slot="имя_слота"</code>. 
Этому атрибуту передается имя слота.</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
	&lt;container&gt;
		&lt;h1 slot="header"&gt;Заголовок статьи&lt;/h1&gt;
...
<p>Те элементы, которые не имеют подобного атрибута, будут передаваться в качестве содержимого для неименованного обычного слота.</p>
<img src="./pics/61.png" alt="Named slots in Vue.js" />
