...
<html >
<head>
<title>Vue.js | Локальная и глобальная регистрация компонентов</title>
<meta charset="utf-8" />
<meta name="description" content="Локальная и глобальная регистрация компонентов во Vue.js, примеры использования компонентов">
...
<h2>Локальная и глобальная регистрация компонентов</h2><div class="date">Последнее обновление: 10.09.2017</div>
...
<p>Компоненты могут быть зарегистрированы локально и глобально. Глобальные компоненты доступны для любого объекта Vue на веб-странице. 
Локальные компоненты доступны только в рамках определенных объектов Vue.</p>
<p>Для локальной регистрации компонентов у объекта Vue устанавливается свойство <span class="b">components</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь определен глобальный компонент - section-header. Глобальный компонент определяется с помощью метода <span class="b">Vue.component()</span>.</p> 
<p>И также здесь определен компонент comp. По сути он представляет объект, который передается в качестве второго параметра в Vue.component() и может иметь все те же 
свойства, например, свойство <code>template</code>. Этот компонент локально регистрируется в объекте Vue:</p>
<pre class="brush:js;">
components:{
	'section-content':comp
}
</pre>
<p>Причем для его рендеринга будет использоваться элемент <code>&lt;section-content&gt;</code>.</p>
<img src="./pics/38.png" alt="Локальные компоненты в Vue.js" />
<p>Если бы компонент comp не был бы зарегистрирован в объекте Vue, то тогда мы бы его не могли использовать, либо пришлось бы делать его глобальным.</p>
...
<p>Здесь определено два объекта Vue. Компонент section-header является глобальным и поэтому доступен из любого объекта Vue. В дополнение к нему первый объект локально регистрирует 
два компонента - comp1 и footer, а второй объект Vue - один компонент comp2.</p>
<img src="./pics/39.png" alt="Глобальная и локальная регистрация компонентов" />
