...
<html >
<head>
<title>Vue.js | Жизненный цикл</title>
<meta charset="utf-8" />
<meta name="description" content="Жизненный цикл объекта Vue во Vue.js, его основные этапы и их обработка">
...
<h2>Жизненный цикл Vue</h2><div class="date">Последнее обновление: 26.08.2017</div>
...
<p>В процессе своей работы приложение Vue.js проходит через ряд этапов жизненного цикла. И с помощью специальных методов мы можем вызвать некоторые действия 
на этих различных этапах жизненного цикла.</p>
<p>Весь жизненный цикл приложения Vue.js можно описать следующим образом:</p>
<ol>
<li><p>Вызывается конструктор <code>new Vue()</code></p></li>
<li><p>Перед созданием объекта Vue вызывается метод <span class="b">beforeCreate()</span></p></li>
<li><p>Далее происходит инициализация объекта Vue, установка его данных и методов</p></li>
<li><p>После создания объекта Vue вызывается метод <span class="b">created()</span></p></li>
<li><p>Далее выполняется компиляция шаблона</p></li>
<li><p>Вызывается метод <span class="b">beforeMount()</span></p></li>
<li><p>Элемент html, к которому прикреплен объект Vue, заменяется скомпилированным шаблоном</p></li>
<li><p>Вызывается метод <span class="b">mounted()</span>, и после этого шаблон прикреплен к DOM, и мы можем с ним работать</p></li>
<li><p>Если в процессе работы мы обновляем данные объекта Vue, то происходит еще ряд событий:</p>
	<ol>
		...
		<li><p>Вызывается метод <span class="b">updated()</span>. DOM на веб-странице обновлен, и мы продолжаем работать с приложением Vue.js</p></li>
	</ol>
</li>
<li><p>При завершении работы приложения вызывается метод <span class="b">beforeDestroy()</span></p></li>
<li><p>И в конце вызывается метод <span class="b">destroyed()</span>. Объект Vue удалено из памяти, и больше с ним мы работать не можем.</p></li>
</ol>
...
<img src="./pics/37.png" alt="Управление жизненным циклом во Vue.js" />
<p>При нажатии на кнопку Update выполняется обновление DOM, и вызываются методы <code>beforeUpdate()</code> и <code>updated()</code>. Причем при повторном 
нажатии эти методы не будут в данном случае вызываться, так как виртуальный DOM никак не изменится, то есть заголовок получит тот же самый текст Update. Соответственно 
реальный DOM также не будет обновляться.</p>
<p>При нажатии на кнопку Destroy будет вызван метод destroy, в котором будет выполняться метод <code>this.$destroy()</code>. То есть мы явным образом 
вызываем у объекта Vue его уничтожение. Затем вызываются методы <code>beforeDestroy()</code> и <code>destroyed()</code>. И после этого объект Vue удален из памяти, 
и работать с ним мы больше не сможем, даже при том, что на веб-странице сохраняется его html-разметка.</p>
...
<div class="nav"><p><a href="1.12.php">Назад</a><a href="./">Содержание</a><a href="2.1.php">Вперед</a></p></div></div>
<div class="socBlock">
