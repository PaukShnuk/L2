...
<html >
<head>
<title>Vue.js | Однонаправленный поток данных</title>
<meta charset="utf-8" />
<meta name="description" content="Однонаправленный поток данных во Vue.js, изменение объекта props в компонентах">
...
<h2>Однонаправленный поток данных</h2><div class="date">Последнее обновление: 29.09.2017</div>
...
значения, переданные через props, то на консоль браузера будет выведено предупреждение о том, что это не следует делать.</p>
...
<p>Из родителя - объекта Vue в компонент useredit через props передается значение name. В этом компоненте мы можем попробовать управлять переданным значением через 
поле ввода и механизм двусторонней привязки:</p>
<pre class="brush:xml;">&lt;input type="text" v-model="user" /&gt;</pre>
<p>Но если мы попробуем изменить через поле ввода данное значение, то оно изменится только в рамках компонента useredit, а на консоль будет выведено предупреждение:</p>
<img src="./pics/52.png" alt="Mutating props in Vue.js" />
<p>Значение свойства name в родительском объекте Vue никак не будет затронуто. Почему так происходит? Дело в том, что данные простых типов - String, Number, Boolean передаются по значению, то есть в дочерний компонент 
...
<p>В отличие от простых типов сложные объекты и массивы передаются по ссылке. Поэтому из дочерних компонентов мы можем изменять массивы или сложные объекты, которые определены 
в родительских объектах.</p>
<p>Более того в объекте Vue посредством нажатия кнопки мы можем вызвать метод resetName, который изменит свойство name с Tom на Bob:</p>
<img src="./pics/53.png" alt="Mutating props in components in Vue.js" />
<p>То есть несмотря на то, что внутри компонента useredit мы меняем значение из props, но изменение родителя - объекта Vue приведет к изменению дочернего компонента.</p>
<p>Однако могут быть ситуации, когда нам все таки нужно изменить значения в props и при этом не допустить изменения этих значений извне. 
Что делать в этом случае? В этом случае мы можем определить локальную переменную. Через props устанавливается ее начальное значение, и затем компонент работает только с этой переменной:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
	&lt;h2&gt;Hello, {{name}}&lt;/h2&gt;
    &lt;useredit :user="name"&gt;&lt;/useredit&gt;
