...
<html >
<head>
<title>Vue.js | Объект Vue</title>
<meta charset="utf-8" />
<meta name="description" content="Объект Vue в Vue.js, его свойства и методы, параметр data">
...
<h2>Объект Vue</h2><div class="date">Последнее обновление: 27.07.2017</div>
...
<p>Для создания приложения в Vue.js используется объект <span class="b">Vue</span>. При создании этого объекта в его конструктор передается параметр, 
с помощью которых производится инициализация приложения:</p>
<pre class="brush:js;">
var vm = new Vue({
  // параметры
})
</pre>
<p>Касательно кода html приложение Vue.js представляет некоторую область на веб-странице. Для определения этой области на веб-станице можно определить 
какой-либо элемент. А для связи с объектом Vue в нем определяется параметр <span class="b">el</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>С помощью параметра <span class="b">data</span> можно определить данные, которые должно хранить приложение, то есть его состояние:</p>
<pre class="brush:js;">
var app = new Vue({
	el: '#app',
    data: {
...
<p>Здесь через параметр data определяются свойства name и age.</p>
<p>Данные можно определять в виде внешнего объекта и затем передавать параметру data:</p>
<pre class="brush:js;">
...
<p>Кроме того, используя объект Vue, мы можем обращаться к его свойствам по имени, получать и присваивать им значение: <code>app.age=30</code></p>
<h3>Методы</h3>
<p>Кроме хранения состояния Vue может определять поведение в виде параметра <span class="b">methods</span>. Этот параметр указывает на набор методов. Каждый метод представляет стандартную функцию JavaScript. 
Например, определим ряд методов:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь определено три метода. Первый метод welcome просто возвращает некоторое значение. Второй метод возвращает значение 
свойства name. Для обращения к свойствам объекта Vue в методах используется ключевое слово <span class="b">this</span>.</p>
<p>Третий метод принимает параметр - число и вычисляет для него факториал.</p>
<img src="./pics/7.png" alt="Методы в Vue.js" />
