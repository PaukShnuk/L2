...
<html >
<head>
<title>Vue.js | Обработка событий</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка событий в Vue.js, объет event, ередача в обработчики параметров, директива v-on">
...
<h2>Обработка событий</h2><div class="date">Последнее обновление: 27.07.2017</div>
...
<p>Для обработки событий элементов html в Vue.js используется директива <span class="b">v-on</span>, 
после которой через двоеточие указывается тип события:</p>
<pre class="brush:js;">v-on:click="действия"</pre>
<p>В качестве типа события используется любое стандартное событие элементов на веб-странице. Затем этой директиве в кавычках передаются те действия, 
которые должны выполниться при возникновении события. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В качестве действия директиве v-on можно передать любые валидные операции JavaScript. В данном случае, так как у нас определено во Vue свойство 
<code>counter</code>, то мы можем манипулировать значением этого свойства:</p>
<pre class="brush:xml;">
&lt;button v-on:click="counter++"&gt;+&lt;/button&gt;
&lt;button v-on:click="if(counter&gt;0) counter--;"&gt;-&lt;/button&gt;
</pre>
<p>Фактически мы передаем директивам v-on кусок кода JavaScript, где свойства из Vue доступны как обычные переменные.</p>
<img src="./pics/6.png" alt="События в Vue.js" />
...
<p>Однако в случае более сложных операций такой подход не является оптимальным. И в этом случае, как правило, все операции, которые надо вызывать при возникновении 
события, выносятся в отдельные методы объекта Vue. Например, перепишем предыдущий пример:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
...
<img src="./pics/8.png" alt="Объект события в Vue.js" />
<p>Кроме того, мы можем передать в методы для обработки событий какие-то дополнительные значения:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
...
<p>В данном случае увеличение переменной counter будет идти на 3, а уменьшение - на 2.</p>
<p>И также мы можем совместить передачу значений и объекта события:</p>
<pre class="brush:xml;">
&lt;div id="app"&gt;
...
<p>Для передачи объекта события в обработчик передается специальный объект <span class="b">$event</span>.</p>
