...
<html >
<head>
<title>Vue.js | Создание событий</title>
<meta charset="utf-8" />
<meta name="description" content="Передача данных из дочернего компонента в родительский во Vue.js, определение своих событий, метод this.$emit, модификатор sync">
...
<h2>Создание событий</h2><div class="date">Последнее обновление: 30.01.2018</div>
...
компонента к родителю? В этом случае необходимо определить свои события. Дочерний компонент будет генерировать событие с помощью вызова 
<span class="b">this.$emit(имя_события)</span>, а родительский компонент будет отлавливать это событие с помощью установки атрибута <span class="b">v-on:название_события</span> и 
при получении события поизводить определенные действия. Например:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/54.png" alt="Communication between child and parent component in Vue.js" />
<h3>Модификатор sync</h3>
<p>Начиная с версии 2.3 был добавлен модификатор <span class="b">sync</span>, который позволяет в ряде случаев сократить код использования событий. В частности, 
...
<p>В данном случае строка</p>
<pre class="brush:xml;">&lt;useredit :user.sync="name"&gt;&lt;/useredit&gt;</pre>
<p>будет сокращенем строки:</p>
<pre class="brush:xml;">&lt;useredit :user="name" v-on:update:user="val =&gt; name = val"&gt;&lt;/useredit&gt;</pre>
<p>Но в дочернем компоненте также необходимо вызывать данное событие:</p>
<pre class="brush:js;">
onUserChange: function(){
	this.$emit('update:user', this.userName)
}
