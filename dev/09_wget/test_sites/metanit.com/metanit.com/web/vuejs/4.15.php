...
<html >
<head>
<title>Vue.js | Взаимодействие между сестринскими компонентами. Шина событий</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие между сестринскими компонентами. Шина событий во Vue.js">
...
<h2>Взаимодействие между сестринскими компонентами. Шина событий</h2><div class="date">Последнее обновление: 30.01.2018</div>
...
<p>Во Vue.js также можно организовать взаимодействие между компонентами, которые находятся на одном уровне, то есть являются сестринскими.</p>
...
<p>В данном случае оба дочерних компонента принимают от объекта Vue имя пользователя. То есть оба компонента используют те данные, которые определены в объекте Vue. Если один компонент 
хочет изменить эти данные, то он генерирует событие. Объект Vue обрабатывает это событие и изменяет данные. После этого данные автоматически изменяются во втором компоненте. 
То есть объект Vue здесь служит медиатором, а непосредственно между собой компоненты не взаимодействуют.</p>
<img src="./pics/59.png" alt="Сестринские компоненты во Vue.js" />
<h3>Шина событий</h3>
<p>Еще один способ взаимодействия между компонентами одного уровня представлет применение шины событий. Ее суть заключается, в определении объекта Vue, 
который будет выполнять роль шины событий:</p>
<pre class="brush:js;">var eventBus = new Vue();</pre>
<p>В компоненте, который хочет изменить данные, мы вызываем через эту шину событие:</p>
<pre class="brush:js;">eventBus.$emit("событие", данные);</pre>
<p>А в другом компоненте в методе created устанавливаем обработчик этого события:</p>
<pre class="brush:js;">
created(){
...
<p>Например, рассмотрим следующий пример:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь объект Vue не обрабатывает никаких событий и просто передает в компоненты имя пользователя. В самих компонентах привяка идет не к свойству 
из props, а к свойству из параметра data. То есть в данном случае даже если бы мы, как в предыдущем примере, захотели бы изменить 
значения свойств через обработку событий в объекте Vue, то мы бы не смогли бы это сделать. Однако благодаря шине событий мы все же это можем сделать. 
Компонент useredit вызывает через шину событий событие userchange, а второй компонент - userinfo обрабатывает это событие.</p>
