...
<html >
<head>
<title>Vue.js | Именованные представления</title>
<meta charset="utf-8" />
<meta name="description" content="Именованные представления во Vue.js, одновременное использование нескольких компонентов router-vue">
...
<h2>Именованные представления</h2><div class="date">Последнее обновление: 29.10.2017</div>
...
<p>Для создания сложных макетов мы можем использовать в одном шаблоне сразу несколько элементов <code>&lt;router-view&gt;</code> или представлений. 
При этом для каждого представления можно определить свое имя с помощью атрибута <span class="b">name</span>. Затем, используя данное имя мы сможем 
сопоставить это представление с определенным компонентом, который будет определять содержимое представления.</p>
...
<p>Здесь в шаблоне объекта Vue определено три представления:</p>
<pre class="brush:xml;">
&lt;router-view&gt;&lt;/router-view&gt;
&lt;router-view name="ads"&gt;&lt;/router-view&gt;
&lt;router-view name="content"&gt;&lt;/router-view&gt;
</pre>
<p>Первое представление не имеет имени и, допустим, оно будет представлять навигацию по приложению. 
Второе представление называется "ads", и оно будет представлять рекламный блок. Третье представление называется "content", и оно представляет 
основное содержимое.</p>
<p>В коде javascript определяется ряд компонентов, которые будут использоваться. И затем идет определение маршрутов. Поскольку 
в данном случае мы имеем три представления, то соответственно мы можем задать для каждого представления свой компонент, которые будет определять 
содержимое представления.</p>
<p>Первый маршрут выглядит следующим образом:</p>
<pre class="brush:js;">
{ 
	path: '/',
    components: {
...
<p>Он использует путь '/'. Для установки используемых компонентов применяется параметр <span class="b">components</span>, 
через который для каждого представления можно определить свой компонент. Представление, которое не имеет имени, доступно здесь через псевдоним "default". 
Для обращения к остальным представлениям используется их имя. В то же время это делать необязательно для каждого представления устанавливать 
свой компонент. Так, в данном случае определяем сопоставление только для двух представлений - default и content.</p>
<p>Аналогично выглядит второй и третий маршруты, только в них компоненты устанавливаются для всех трех представлений:</p>
<pre class="brush:js;">
{ 
	path: '/phones',
    components: {
...
<p>То есть в данном случае первое безымянное представление будет отображать ссылку навигации, второе представление ads будет отображать 
рекламу выбранных товаров, а третье представление content будет отображать информацию о выбранных товаров.</p>
<p>Таким образом, при обращении по пути "/" отобразится список выбора типа товаров:</p>
<img src="./pics/router11.png" alt="Named views in Vue.js" />
<p>А при выборе определенного типа товара будут отображаться все три представления с определенной информацией:</p>
<img src="./pics/router12.png" alt="Именованные представления во Vue.js" />
<img src="./pics/router13.png" alt="router-view in Vue.js" />
