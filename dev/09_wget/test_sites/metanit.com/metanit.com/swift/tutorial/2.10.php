...
<html >
<head>
<title>Swift | Циклы</title>
<meta charset="utf-8" />
<meta name="description" content="Циклы в языке программирования Swift, цикл for-in, for, while, repeat-while, обход массивов, выход из цикла и ключевые слова break и continue">
...
<h2>Циклы</h2><div class="date">Последнее обновление: 29.12.2017</div>
...
<h3>Цикл for-in</h3>
<p>С помощью цикла <span class="b">for-in</span> мы можем перебрать элементы коллекции (массивы, множества, словари) или последовательности. Он 
...
<p>Например, переберем элементы массива:<p>
<pre class="brush:swift;">
for item in 1...5 {
...
<p>Выражение <code>1...5</code> образует последовательность из пяти чисел от 1 до 5. И цикл проходит по всем 
элементам последовательности. При каждом проходе он извлекает одно число и передает его в переменную item. Таким образом, 
цикл сработает пять раз.</p>
<p>С помощью оператора  <span class="b">where</span> можно задавать условия выборки из последовательности элементов:</p>
<pre class="brush:swift;">
...
<p>Здесь из последовательности <code>0...10</code>  извлекаются только те элементы, которые соответствуют условию после оператора 
where - <code>i % 2 == 0</code>, то есть четные числа.</p>
<h3>Цикл while</h3>
<p>Оператор <code>while</code> проверяет некоторое условие, и если оно возвращает true, то выполняет блок кода. Этот цикл имеет следующую форму:</p>
<pre class="brush:swift;">
while условие {
...
<p>При этом надо внимательно подходить к условию. Если оно всегда будет возвращать true, то мы получим бесконечный цикл, и приложение может зависнуть.</p>
<h3>Цикл repeat-while</h3>
<p>Цикл <code>repeat-while</code> сначала выполняет один раз цикл, и если некоторое условие возвращает true, то продолжает выполнение цикла. Он имеет 
следующую форму:</p>
<pre class="brush:swift;">
repeat {
...
<p>Например, перепишем предыдущий цикл <code>while</code>:</p>
<pre class="brush:swift;">
var i = 10
...
<p>Здесь цикл также выполнится 10 раз, пока значение переменной i не станет равно 0.</p>
...
<p>Несмотря на то, что переменная i меньше 0, но цикл выполнится один раз.</p>
...
<p>Поскольку в цикле идет проверка, равно ли значение переменной i числу 5, то когда перебор дойдет до числа 5, 
сработает оператор break, и цикл завершится.</p>
<p>Теперь поставим себе другую задачу. А что если мы хотим, чтобы при проверке цикл не завершался, а просто переходил к следующему элементу. 
...
<p>В этом случае цикл, когда дойдет до числа 5, которое не удовлетворяет условию проверки, просто пропустит это 
число и перейдет к следующему элементу последовательности.</p>
