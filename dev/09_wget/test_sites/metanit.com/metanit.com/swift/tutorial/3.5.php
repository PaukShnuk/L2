...
<html >
<head>
<title>Swift | Сабскрипты</title>
<meta charset="utf-8" />
<meta name="description" content="Сабскрипты в языке программирования Swift, превращение классов в коллекции, использование индексаторов">
...
<h2>Сабскрипты</h2><div class="date">Последнее обновление: 02.01.2018</div>
...
<p>Классы, структуры и перечисления могут определять сабскрипты (subscripts). Сабскрипты используются для доступа к элементам коллекции или 
последовательности. В ряде языков программирования есть похожая концепция - индексаторы. Сабскрипты позволяют обращаться с объектом класса или структуры как с 
отдельной коллекцией.</p>
<p>Для определения сабскрипта используется ключевое слово <span class="b">subscript</span>:</p>
...
<p>После ключевого слова <code>subscript</code> в скобках идут параметры, которые используются для получения элементов. Нередко параметры представляют 
числовой индекс, по которому надо получить объект. Далее указывается тип элементов, с которыми мы работаем.</p>
<p>Сабскрипт может состоять из двух блоков: get и set. Блок get возвращает элемент, а блок set устанавливает новое значение, которое передается через параметр newValue.</p>
<p>Например, создадим класс библиотеки. Упрощенно библиотека представляет некоторый набор книг. То есть мы можем представить класс библиотеки как 
коллекцию книг и использовать сабскрипты для получения книг по индексу:</p>
<pre class="brush:swift;">
...
<p>Здесь сабскрипт предназначен для работы с типом Book. В блоке get происходит получение объекта Book по индексу из массива books. В блоке set устанавливаем 
объект Book в массиве books.</p>
<p>В итоге в программе мы сможем обращаться к библиотеке как к массиву по индексу для получения нужной книги:</p>
<pre class="brush:swift;">var firstBook: Book = myLibrary[0]</pre>
<p>Есть два типа сабскриптов:</p>
<ul>
<li><p>Сабскрипты, которые поддерживают чтение и запись (то есть с блоками get и set, как в примере выше)</p></li>
<li><p>Сабскрипты только для чтения (только с блоком get)</p></li>
</ul>
<p>Изменим класс библиотеки, чтобы применить сабскрипт только для чтения:</p>
<pre class="brush:swift;">
class Library{
...
<div class="nav"><p><a href="2.12.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
