...
<html >
<head>
<title>Swift | Структуры</title>
<meta charset="utf-8" />
<meta name="description" content="Структуры в языке программирования Swift, встроенные инициализаторы структур, методы mutating">
...
<h2>Структуры</h2><div class="date">Последнее обновление: 02.01.2017</div>
...
<p>Во многом похожи на классы другие конструкции - <span class="b">структуры</span>. Все базовые типы данных, такие как Int, массивы, коллекции - все 
они представляют структурами.</p>
<p>Для определения структуры используется ключевое слово <span class="b">struct</span>, после которого идет название структуры:</p>
<pre class="brush:swift;">
struct User {

}
</pre>
<p>В данном случае структура называется User. Как и класс, структура по сути представляет новый тип данных. Все тело 
структуры заключается в фигурные скобки.</p>
<p>Для создания объекта User нам надо использовать инициализатор по умолчанию:</p>
<pre class="brush:swift;">
var tom: User = User()
</pre>
<p>Структура, как и класс, может содержать свойства и методы:</p>
<pre class="brush:swift;">
struct User {
    var name: String = "Tom"
    var age: Int = 18
...
<p>Одним из отличий структур от классов состоит в том, что для создания структур мы можем в инициализатор передать значения для всех его свойств:</p>
<pre class="brush:swift;">
var bob  = User(name: "Bob", age: 23)
print(bob.getInfo())
</pre>
<p>При этом не важно, что в самой структуре нет инициализатора, который принимает два параметра. По умолчанию для 
каждой структуры мы можем передать в инициализаторе значения для всех ее свойств. Параметры инициализатора в этом случае соответствуют свойствам структуры. При этом мы 
не можем задать только одно или два свойства, нам обязательно надо указать значения для всех свойств структуры.</p>
<p>Но мы также можем задать и дополнительные инициализаторы в самой структуре:</p>
<pre class="brush:swift;">
struct User {
    var name: String
    var age: Int
...
<p>Для обращения к свойствам, методам, инициализаторам структуры, также как и в классе, может использоваться ключевое слово <span class="b">self</span>.</p>
<h3>mutating-методы</h3>
<p>Еще одним отличием структуры от классов является невозможность изменения в методах структуры свойств этой же 
...
<p>Чтобы выйти из этой ситуации, перед именем метода необходимо указать ключевое слово mutating:</p>
<pre class="brush:swift;">
struct User {
    var name: String
    var age: Int
