...
<html >
<head>
<title>Swift | Типы Character и String</title>
<meta charset="utf-8" />
<meta name="description" content="Типы Character и String в языке Swift, операции со строками, программирование для iOS">
...
<h2>Типы Character и String</h2><div class="date">Последнее обновление: 25.12.2017</div>
...
<p>Для работы с текстом применяются два типа данных: <span class="b">Character</span> и 
<span class="b">String</span>. Character представляет отдельный символ, а String - строку из нескольких символов. 
При этом надо отметить, что String - это не просто набор объектов Character, это отдельный тип, который по функциональности отличается.</p>
<p>Самый простой способ определения строки и символов представляет использование строковых литералов - значений в двойных кавычках:</p>
<pre class="brush:swift;">
var a: Character = "a"
var hello: String = "hello"
</pre>
<p>В отличие от строки в переменную типа <code>Character</code> мы не можем засунуть больше одного символа, то есть в следующем случае мы столкнемся с ошибкой:</p>
<pre class="brush:swift;">
var a: Character = "abc"
</pre>
<p>Строки могут быть пустыми, то есть по сути не содержать ничего, но при этом быть инициализированными:</p>
<pre class="brush:swift;">
var str1: String = ""
var str2: String = String()
</pre>
<p>Пустая строка создается при помощи присвоения <code>""</code> или при помощи инициализатора <code>String()</code></p>
<h3>Управляющие последовательности</h3>
<p>Строка может содержать специальные символы, которые называются управляющими последовательностями или эскейп-последовательностями. 
Основные из них:</p>
<ul>
<li><p><code>\n</code>: перевод на новую строку</p></li>
<li><p><code>\t</code>: табуляция</p></li>
<li><p><code>\"</code>: кавычка</p></li>
<li><p><code>\\</code>: обратный слеш</p></li>
</ul>
<p>Например, мы хотим определить многострочный текст, в котором будут использоваться кавычки и обратные слеши:</p>
<pre class="brush:swift;">
let text = "ООО \"Рога и копыта 777\" \nГен. директор: Иванов"
print(text)
</pre>
<p>XCode Playground интерпретирует данную строку следующим образом:</p>
<pre class="browser">
ООО "Рога и копыта 777" 
Ген. директор: Иванов
</pre>
<p>Но, начиная с версии языка 4.0, для создания многострочных строк можно заключать строку в тройные кавычки """:</p>
<pre class="brush:swift;">
let text = """
ООО "Рога и копыта 777"
Ген. директор: Иванов
...
<p>Результат будет тот же, что и в примере выше. Кроме того, внутри многострочных строк кавычки не надо экранировать с помощью обратного слеша.</p>

<h3>Преобразование в строку</h3>
<p>Для преобразования значений других типов данных к строке надо передать преобразуемое значение в инициализатор:</p>
<pre class="brush:swift;">
var number: Int = 22
var str: String = String(number)	// "22"
</pre>
<h3>Конкатенация строк</h3>
<p>Для конкатенации (объединения) строк используется оператор <code>+</code> (плюс):</p>
<pre class="brush:swift;">
var str: String = ""	// ""
str += "hello"			// "hello"
...
<p>Интерполяция представляет собой объединение в строку различных литералов и значений, которые могут представлять другие типы данных. 
Для интерполяции объекты других типов данных заключаются в скобки, перед которыми ставится слеш:</p>
<pre class="brush:swift;">
var age: Int = 22
...
<p>Также интерполяция может содержать сложные выражения и операции, например:</p>
<pre class="brush:swift;">
let x1 = 5
let x2 = 6
let str = "x1 + x2 = \(x1 + x2)"
...
<div class="nav"><p><a href="2.20.php">Назад</a><a href="./">Содержание</a><a href="2.4.php">Вперед</a></p></div></div>
<div class="socBlock">
