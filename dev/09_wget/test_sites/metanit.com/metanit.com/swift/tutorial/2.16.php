...
<html >
<head>
<title>Swift | Функция как значение. Тип функции</title>
<meta charset="utf-8" />
<meta name="description" content="Тип функции в языке программирования Swift, функции как параметры и возвращаемые результаты других функций">
...
<h2>Тип функции</h2><div class="date">Последнее обновление: 01.01.2018</div>
...
<p>Каждая функция имеет определенный тип, который складывается из типов параметров функции и типа 
возвращаемого значения.</p>
<p>Например, пусть у нас есть следующая функция:</p>
...
<p>Эта функция имеет тип <code>(Int, Int) -&gt; Int</code></p>
<p>Или, например, функция:</p>
<pre class="brush:swift;">
...
<p>Она имеет тип <code>(String) -&gt; Void</code></p>
<p>Используя тип функции, мы можем определять переменные или константы этого типа и затем динамически назначать их конкретные функции:</p>
<pre class="brush:swift;">
...
<p>Итак, здесь у нам определены две функции sum и subtract, которые имеют одни и те же параметры и возвращаемые типы значений, но отличаются конкретными действиями: 
в одном случае идет сложение, а в другом - вычитание чисел.</p>
<p>Также определена переменная <code>someFunc</code>, которая имеет тип - функцию с двумя параметрами типа Int и возвращаемым значением типа Int. То есть 
...
<p>Фактически здесь будет вызываться функция sum. Поэтому в качестве результата мы получим сумму 4 + 5.</p>
<p>Затем динамически мы можем приравнять переменную другой функции:</p>
<pre class="brush:swift;">
someFunc = subtract
var result = someFunc(5, 4)	// 1
</pre>
<p>Подобная функциональность открывает нам большие возможности. В частности, мы можем использовать типы функций в качестве типов параметров или 
в качестве возвращаемых типов в других функциях.</p>
<h3>Типы функций в качестве типов параметров</h3>
<pre class="brush:swift;">
func sum(_ a: Int, _ b: Int) -&gt; Int{
...
<p>Здесь функция <code>getResult</code> в качестве первого параметра принимает функцию. Типу этого параметры соответствуют выше определенные функции sum и substract, 
поэтому они могут использоваться при вызове функции getResult:</p>
<pre class="brush:swift;">getResult(sum, 13, 10)</pre>
<h3>Типы функций как типы возвращаемых значений</h3>
<pre class="brush:swift;">
...
<p>Возвращаемым типом функции <code>select</code> является тип <code>(Int, Int) -&gt; Int</code>. Это значит, что 
...
Поэтому в select мы возвращаем не конкретное значение, а одну из этих функций в зависимости от значения параметра n.</p>
<p>Возвращаемый результат мы можем присвоить переменной или константе:</p>
<pre class="brush:swift;">
var someFunc = select(1)    // add
</pre>
<p>И затем через эту переменную можно будет обращаться к возвращенной функции.</p>
