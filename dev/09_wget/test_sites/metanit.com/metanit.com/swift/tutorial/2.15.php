...
<html >
<head>
<title>Swift | Дополнительно о параметрах функций</title>
<meta charset="utf-8" />
<meta name="description" content="Передача параметров по ссылке в языке программирования Swift, выходные параметры и ключевое слово inout, произвольное количество параметров">
...
<h2>Дополнительно о параметрах функций</h2><div class="date">Последнее обновление: 01.01.2018</div>
...
<p>С помощью оператора многоточия (...) можно устанавливать произвольное количество параметров одного типа.</p>
...
<p>Если параметр именнованный, то указывается его имя, после которого после двоеточия указываются через запятую все значения:</p>
<pre class="brush:swift;">
...
<h3>Выходные параметры inout</h3>
<p>По умолчанию все параметры функции представляют константные значения, и мы не можем их изменить. Например, при определении 
следующей функции мы получим ошибку:</p>
<pre class="brush:swift;">
func increase(_ n : Int){
    n += 10
}
</pre>
<p>По умолчанию параметр n представляет константу, и его значение мы не можем изменить. Однако в ряде задач такое поведение 
может потребоваться. И если все таки необходимо изменить значение параметра, то его надо определить с ключевым 
словом <span class="b">inout</span>:</p>
<pre class="brush:swift;">
...
<p>При передачи переменных в функцию к ним добавляется знак амперсанда.То есть фактически мы передаем ссылку на переменную, а не просто ее значение. 
И, таким образом, сможем изменить значение переменной, определенной вне функции.</p>
<p>Другой показательный пример - обмен значений переменных:</p>
<pre class="brush:swift;">
