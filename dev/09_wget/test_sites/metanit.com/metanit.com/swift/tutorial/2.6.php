...
<html >
<head>
<title>Swift | Условная конструкция If. Тернарный оператор</title>
<meta charset="utf-8" />
<meta name="description" content="Ветвление в языке Swift, тернарный оператор, конструкция if..else, программирование для iOS и MacOS">
...
<h2>Условная конструкция If. Тернарный оператор</h2><div class="date">Последнее обновление: 29.12.2017</div>
...
<p>В языках программирования часто бывает необходимо произвести некоторые действия в зависимости от выполнения тех или иных условий. Если определенные условия 
...
какие условные конструкции есть в языке Swift.</p>
<h3>Конструкция if/else</h3>
<p>Конструкция <span class="b">if</span> проверяет истинность некоторого условия и в зависимости от результатов проверки выполняет определенный код:</p>
...
<p>Здесь если первое число больше второго, то сработает весь код в блоке if, который располагается между открывающей и закрывающей фигурными скобками. 
Если же первое число окажется меньше второго, тогда действия в конструкции if работать не будут.</p>
<p>Важно, что после слова <span class="b">if</span> в этой конструкции должно идти значение типа <span class="b">Bool</span>. Поскольку результат операции сравнения 
...
<p>Если при проверке условия нам надо выполнить какие-либо альтернативные действия, то мы можем использовать блок <span class="b">else</span>:</p>
<pre class="brush:swift;">
let num1 = 22
let num2 = 15
if num1 &gt; num2{
...
<p>Но при сравнении чисел мы можем насчитать три состояния: первое число больше второго, первое число меньше второго и числа равны. Используя конструкцию 
<span class="b">else if</span>, мы можем обрабатывать дополнительные условия:</p>
<pre class="brush:swift;">
let num1 = 22
let num2 = 15
if num1 &gt; num2{
...
<p>Тернарный оператор аналогичен простой конструкции if и имеет следующий синтаксис:</p>
<pre class="brush:swift;">[первый операнд - условие] ? [второй операнд] : [третий операнд]</pre>
<p>В зависимости от условия тернарный оператор возвращает второй или третий операнд: если условие равно true, то возвращается второй операнд; если условие 
равно false, то третий. Например:</p>
<pre class="brush:swift;">
var num1 = 10
var num2 = 6
var num3 = num1 &gt; num2 ? num1 - num2 : num1 + num2
</pre>
<p>В данном случае num3 будет равно 4, так как num1 больше num2, поэтому будет выполняться второй операнд: <code>num1 - num2</code>.</p>
