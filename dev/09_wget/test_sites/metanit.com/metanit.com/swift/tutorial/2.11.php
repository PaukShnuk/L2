...
<html >
<head>
<title>Swift | Множества</title>
<meta charset="utf-8" />
<meta name="description" content="Множества set в языке программирования Swift, операции с множествами, добавление и удаление элементов">
...
<h2>Множества</h2><div class="date">Последнее обновление: 05.01.2018</div>
...
<p>Множества (Set) представляют собой неупорядоченные коллекции уникальных элементов. В отличие от массивов в множествах элементы 
должны быть уникальными, мы не можем определить несколько элементов с одним и тем же значением.</p>
<h3>Определение</h3>
<p>Для определения множества у переменной или константы устанавливается тип <code>Set&lt;Element&gt;</code>, где Element - это тип данных:</p>
<pre class="brush:swift;">
var numbers: Set&lt;Int&gt; = [5, 6, 7, 8]
</pre>
<p>При определении можно опускать тип:</p>
<pre class="brush:swift;">
var numbers: Set = [5, 6, 7, 8]
</pre>
<p>Либо можно использовать функцию инициализатора <code>Set&lt;Element&gt;</code>:</p>
<pre class="brush:swift;">
var numbers = Set&lt;Int&gt;(arrayLiteral: 5, 6, 7, 8)
</pre>
<p>Создание пустого множества:</p>
<pre class="brush:swift;">
var numbers = Set&lt;Int&gt;()
// или
// var numbers: Set&lt;Int&gt; = []
</pre>
<h3>Работа с множествами</h3>
<p>Метод <code>insert()</code> позволяет добавить новый элемент в множество:</p>
<pre class="brush:swift;">
var numbers: Set&lt;Int&gt; = [5, 6, 7, 8]
numbers.insert(10);
print(numbers)	// [5, 6, 7, 8, 10]
</pre>
<p>Также множества поддерживают ряд операций по удалению:</p>
<ul>
<li><p><span class="b">removeAtIndex()</span>: удаляет элемент по определенному индексу</p></li>
<li><p><span class="b">removeFirst()</span>: удаляет первый элемент</p></li>
<li><p><span class="b">remove()</span>: удаляет какой-то определенный элемент по значению</p></li>
<li><p><span class="b">removeAll()</span>: удаляет все элементы</p></li>
</ul>
<pre class="brush:swift;">
var numbers: Set&lt;Int&gt; = [5, 6, 7, 8]
numbers.remove(7);
...
<p>Еще один метод <span class="b">contains()</span> позволяет проверить наличие в множестве элемента:</p>
<pre class="brush:swift;">
var numbers: Set&lt;Int&gt; = [5, 6, 7, 8]
var isPresent = numbers.contains(7);	// true - элемент есть
isPresent = numbers.contains(34);	// false - элемент отсутствует
</pre>
<p>Хотя множество представляет неупорядоченный набор, но с помощью метода <span class="b">sorted()</span> мы можем отсортировать его:</p>
<pre class="brush:swift;">
var numbers: Set&lt;Int&gt; = [4, 7, 2, 6]
print(numbers.sorted())	// [2, 4, 6, 7]
</pre>
<h3>Операции над множествами</h3>
<p>Особую ценность среди операций представляют операции, предназначенные специально для множеств: объединение, пересечение, разность множеств.</p>
<img src="pics/2.1.png" alt="Операции над множествами в Swift" />
<ul>
<li><p><span class="b">intersection()</span>: пересечение множеств, возвращает общие для обоих множеств элементы</p></li>
<li><p><span class="b">symmetricDifference()</span>: возвращает не общие (не пересекающиеся) для обоих множеств элементы (симметрическая разность)</p></li>
<li><p><span class="b">union()</span>: объединение двух множеств</p></li>
<li><p><span class="b">subtract()</span>: разница множеств, возвращает элементы первого множества, которые отсутствуют во втором</p></li>
</ul>
<pre class="brush:swift;">
var a: Set = [1, 2, 3, 4, 5]
var b: Set = [4, 5, 6, 7, 8]
