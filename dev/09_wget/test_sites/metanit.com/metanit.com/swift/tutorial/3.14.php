...
<html >
<head>
<title>Swift | Преобразование типов</title>
<meta charset="utf-8" />
<meta name="description" content="Преобразование типов в языке программирования Swift, нисходящее преобразование от базового типа к производному, операторы as!, as? и is">
...
<h2>Преобразование типов</h2><div class="date">Последнее обновление: 04.01.2018</div>
...
<p>При определении иерархии классов мы можем использовать объекты производного класса везде, где требуеются объекты базового класса. 
Например, следующие классы:</p>
...
<p>Поскольку класс Employee наследуется от класса Person, то везде, где требуется объект Person, мы можем использовать 
объект Employee:</p>
<pre class="brush:swift;">
...
<p>И в данном случае никаких ошибок не возникнет. И компилятор автоматически преобразует объекты Employee к типу Person.</p>
<p>Но теперь рассмотрим другую ситуацию:</p>
<pre class="brush:swift;">
let bob: Person = Employee(name: "Bob", age: 28, company: "Apple")
print(bob.company)      // ! Ошибка: у типа Person нет свойства company
bob.work()              // ! Ошибка: у типа Person нет метода work
</pre>
<p>В данном случае константа bob представляет тип Person, но хранит ссылку на объект Employee, у которого есть свойство company и 
метод work. Однако у типа Person их нет. И в данном случае константа bob воспринимается имеено объект Person, объект Person 
не обязательно должен представлять объект Employee.</p>
...
<p>Функция getInfo принимает объект Employee, однако константа bob представляет именно объект Person, несмотря на то, что 
хранит ссылку на объект Employee, поэтому автоматически мы ее передать в этот метод не можем.</p>
<h3>Нисходящее преобразование</h3>
<p>Что же делать, если переменная / константы представляет объект базового типа, однако нам необходимо его использовать как 
объект производного типа? В этом случае необходимо применить нисходящее преобразование типов (downcasting). Для 
этого применяется оператор <span class="b">as!</span>:</p>
<pre class="brush:swift;">
func getInfo(p: Employee){
    p.display()
}
...
<h3>Безопасное преобразование</h3>
<p>Хотя оператор <span class="b">as!</span> позволяет преобразовать объект одного типа в другой, тем не менее мы можем столкнуться с 
ошибкой:</p>
<pre class="brush:swift;">
let tom: Person = Person(name:"Tom", age: 23)
let tomEmpl: Employee = tom as! Employee
</pre>
<p>Здесь константа tom хранит ссылку на объект Person, а не Employee. Поэтому при попытке преобразования к типу Employee 
мы получим ошибку. Чтобы избежать подобных ошибок, следует проверять тип перед преобразованием типов с помощью оператора 
<span class="b">is</span>:</p>
<pre class="brush:swift;">
let tom: Person = Person(name:"Tom", age: 23)
if tom is Employee{
    let tomEmpl: Employee = tom as! Employee
...
<p>В качестве альтернативы мы можем преобразовывать объект в тип Optional с помощью оператора <span class="b">as?</span>, а затем проверять на nil:</p>
<pre class="brush:swift;">
let tom: Person = Person(name:"Tom", age: 23)
...
<p>Также можно сократить код следующим образом с помощью оператора <span class="b">?.</span>:</p>
<pre class="brush:swift;">
let tom: Person = Person(name:"Tom", age: 23)
