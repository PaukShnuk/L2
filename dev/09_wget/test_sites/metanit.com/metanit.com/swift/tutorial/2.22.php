...
<html >
<head>
<title>Swift | Последовательность</title>
<meta charset="utf-8" />
<meta name="description" content="Последовательности range  в языке программирования Swift, определение и работа с последовательностями, их основные методы">
...
<h1>Коллекции</h1><h2>Последовательность</h2><div class="date">Последнее обновление: 05.01.2018</div>
...
<p>Последовательность (range) представляет набор значений, который определяется начальной и конечной точкой. Есть два способа 
определения последовательности с помощью специальных операторов:</p>
<ul>
<li><p><span class="b">...</span>: этот оператор принимает начальное и конечное значения последовательности и создает набор значений, 
который включает оба значения</p>
<pre class="brush:swift;">
let range = 1...5   // 1 2 3 4 5
</pre>
<p>Таким образом, с помощью выражения <code>1...5</code> будет создаваться последовательность 1 2 3 4 5. При создании 
последовательности используется в качестве шага число 1, которое добавляется к предыдущему значению.</p>
<p>Также последовательность может идти в обратную сторону, если начальное значение меньше конечного, то есть 
применяется шаг -1:</p>
<pre class="brush:swift;">
let range = -5 ... -1   // -5 -4 -3 -2 -1
</pre>
</li>
<li><p><span class="b">..&lt;</span>: этот оператор также принимает начальное и конечное значения последовательности 
и создает набор значений, который не включает конечное значение.</p></li>
</ul>
<p>Последовательности имееют ряд методов, рассмотрим некоторые из них.</p>
<p>Метод <span class="b">reversed()</span> возвращает перевернутую наоборот последовательность:</p>
<pre class="brush:swift;">
...
<p>Метод <span class="b">contains()</span> позволяет проверить наличие элемента в последовательности. Если элемент присутствует, 
то метод возвращает true:</p>
<pre class="brush:swift;">
let range = 5 ... 8   // 5 6 7 8
...
<p>Метод <span class="b">start(with:)</span> позволяет проверить, начинается ли последовательность с подпоследовательности, 
которая передается через параметр with:</p>
<pre class="brush:swift;">
let range = 5 ... 8   // 5 6 7 8
...
<p>Метод <span class="b">overlaps</span> возвращает true, если две последовательности хотя бы частично совпадают:</p>
<pre class="brush:swift;">
let range = 5 ... 8   // 5 6 7 8
...
<div class="nav"><p><a href="3.10.php">Назад</a><a href="./">Содержание</a><a href="2.9.php">Вперед</a></p></div></div>
<div class="socBlock">
