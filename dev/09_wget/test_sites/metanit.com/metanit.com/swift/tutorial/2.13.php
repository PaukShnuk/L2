...
<html >
<head>
<title>Swift | Функции</title>
<meta charset="utf-8" />
<meta name="description" content="Функции в языке программирования Swift, определение функций, параметры, значения параметров по умолчанию">
...
<h2>Функции</h2><div class="date">Последнее обновление: 31.12.2017</div>
...
<p>Функция представляет набор инструкций, который имеет имя (имя функции) и может использоваться повторно в различных местах программы. Функция имеет 
следующее формальное определение:</p>
...
<p>Сначала идет ключевое слово <span class="b">func</span>, после которого идет имя функции. Для именования функции применяются в принципе те же правила, 
что и при именовании переменных. Для имени функции используется режим camel case.</p> 
<p>После имени функции в скобках указываются параметры. Если параметров нет, то просто идут пустые скобки.</p>
<p>Если функция возвращает какое-либо значение, то после параметров в скобках идет стрелка и тип возвращаемого значения. И в конце в фигурных скобках собственно 
идет блок кода, который и представляет функцию.</p>
...
<p>Здесь функция называется <code>printName</code>. Эта функция ничего не возвращает, поэтому тут после скобок сразу идут фигурные скобки с набором операторов. 
Данная функция просто выводит строку "Меня зовут Том".</p>
<p>Далее по имени функции мы можем ее вызвать многократно. Для вызова функции указывается ее имя, после которого в скобках 
перечисляются значения для ее параметров:</p>
<pre class="brush:swift;">
func printName(){
...
<p>В частности, здесь функция вызывается три раза.</p>
<h3>Параметры функции</h3>
<p>Теперь используем параметры:</p>
<pre class="brush:swift;">
...
<p>Количество параметров может быть произвольным. В данном случае мы используем два параметра: для передачи имени и возраста. Для каждого параметра 
определено имя и тип. Например, первый параметр называется <code>name</code> и имеет тип String.</p>
<p>При вызове функции нам надо учитывать имя и тип параметров. При вызове функции ей необходимо передать значения для всех 
ее параметров по имени. То есть указывается имя параметра и через двоеточие его значение: <code>name: "Tom"</code>, 
причем передаваемое значение должно соответствовать параметру по типу:</p>
<pre class="brush:swift;">printInfo(name: "Tom", age: 18)</pre>
<p>Хотя мы можем указать, что при вызове функции не надо указывать имена параметров при ее вызове. Для этого перед именем параметра нужно поставить подчеркивание. Между подчеркиванием и названием параметра 
должен идти пробел:</p>
<pre class="brush:swift;">
...
<p>При вызове подобной функции значения параметрам передаются по позиции. При этом опять же передаваемое значение должно 
соответствовать параметру по типу.</p>
<h3>Значения параметров по умолчанию</h3>
<p>Если мы определили два параметра в функции, то соответственно при ее вызове мы также должны передать ей два значения для параметров. Однако мы также 
можем установить для параметров значения по умолчанию:</p>
<pre class="brush:swift;">
...
<p>И если при вызове функции мы не передадим для какого-то параметра значение, то этот параметр будет использовать значение по умолчанию.</p>
...
<div class="nav"><p><a href="2.10.php">Назад</a><a href="./">Содержание</a><a href="2.14.php">Вперед</a></p></div></div>
<div class="socBlock">
