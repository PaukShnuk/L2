...
<html >
<head>
<title>Swift | Наследование</title>
<meta charset="utf-8" />
<meta name="description" content="Наследование в языке программирования Swift, базовые суперклассы и подклассы, переопределение методов, свойств и инициализаторов">
...
<h2>Наследование</h2><div class="date">Последнее обновление: 02.01.2018</div>
...
<p>Одним из ключевых механизмов объектно-ориентированного программирования является наследование. 
В Swift классы могут наследовать функционал от других классов.</p>
<p>Класс-наследник еще называют подклассом, а класс, от которого наследуется функционал, - базовым классом или суперклассом.</p>
<p>Классы в Swift имеют полноценный доступ ко всем методам, свойствам, которые определены в суперклассе. Однако при необходимости 
подклассы могут переопределять наследуемый функционал  суперклассом, например, изменять поведение методов или свойств.</p>
<p>Общий синтаксис наследования классов выглядит следующим образом:</p>
<pre class="brush:swift;">
class SubClass: SuperClass{
}
</pre>
<p>Для примера наследования рассмотрим простейшую ситуацию. Пусть у нас есть класс человека и класс служащего:</p>
<pre class="brush:swift;">
class User{
...
<p>Здесь класс Emplyee наследуется от класса User. Ведь класс сотрудника по сути будет повторять функционал класса человека, так как каждый сотрудник имеет 
имя и фамилию. И наследование в данном случае помогает избежать ненужного повторения при определении свойств и методов.</p>
<p>А после создания объекта Employee мы можем через него обращаться к свойствам и методам базового класса User:</p>
<pre class="brush:swift;">
...
<p>Ключевое слово <span class="b">super</span> позволяет обращаться из подкласса к свойствам и методам базового класса. В выше приведенном примере 
ключевое слово super используется для обращения к инициализатору базового класса ля передачи ему значений.</p>
<h3>Переопределение методов</h3>
<p>Подкласс может перенимать полностью функционал базового класса, а может и переопределять его. Для переопределения используется ключевое слово 
<span class="b">override</span>.</p>
<pre class="brush:swift;">
class User{
...
<p>В данном случае мы переопределяем метод <code>getFullInfo()</code>. Теперь кроме имени и фамилии он также возвращает данные о компании, в которой 
сотрудник работает. И всегда, когда мы будем вызывать метод <code>getFullInfo()</code> у объекта Employee, будет срабатывать именно переопределенная версия метода.</p>
<p>Используя ключевое super, мы можем переопределить метод по-другому:</p>
<pre class="brush:swift;">
override func getFullInfo() -&gt; String{
...
<p>Подобным образом мы можем переопределять свойства:</p>
<pre class="brush:swift;">
class User{
...
<p>В данном случае переопределяется свойство <code>fullInfo</code>.</p>
<h3>Переопределение инициализаторов</h3>
<p>При переопределении инициализатора необходимо вызвать инициализатор базового класса для инициализации тех свойств, которые определены в базовом классе. 
Кроме того, если в подклассе есть собственные свойства, их необходимо инициализировать до вызова инициализатора базового класса:</p>
<pre class="brush:swift;">
class User{
...
<h3>Обязательные инициализаторы</h3>
<p>В предыдущем примере нам было необязательно переопределять инициализатор класса User в классе Employee. Однако с помощью ключевого слова <span class="b">required</span> 
мы можем отметить этот инициализатор как обязательный для переопределения в подклассах:</p>
<pre class="brush:swift;">
class User{
...
<p>С помощью ключевого слова <span class="b">final</span> мы можем запретить переопределение свойств, методов, сабскриптов в производном классе:</p>
<pre class="brush:swift;">
class User{
...
<p>Теперь свойство <code>fullInfo</code> нельзя будет переопределить в производном классе.</p>
<p>Более того мы можем вообще запретить наследование класса, поставив перед его определением ключевое слово final:</p>
<pre class="brush:swift;">
final class User{
