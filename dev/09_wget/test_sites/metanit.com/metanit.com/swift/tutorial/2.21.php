...
<html >
<head>
<title>Swift | Перегрузка функций</title>
<meta charset="utf-8" />
<meta name="description" content="Перегрузка функций (function overloading) в языке программирования Swift, правила перегрузки функций">
...
<h2>Перегрузка функций</h2><div class="date">Последнее обновление: 01.01.2018</div>
...
<p>В Swift нам доступен механизм перегрузки функций, то есть мы можем определять функции с одним и тем же именем, но 
разным количеством или типом параметров:</p>
...
<p>В данном случае все три функции называются sum, но так как они отличаются либо по количеству параметров, 
либо по их типу. При вызове данной функции Swift по типу и количеству параметров сможет распознать, какую именно версию 
функции sum надо использовать.</p>
<p>Также перегруженные версии одной функции могут отличаться по типу возвращаемого значения:</p>
<pre class="brush:swift;">
...
<p>В данном случае определены две версии функции sum, которые совпадают по типу и количеству параметров, но различаются по типу 
возвращаемого значения.</p>
<p>Константа <code>a</code> представляет тип Int, поэтому в выражении <code>let a : Int = sum(1, 2)</code> компилятор увидит, 
что необходима версия, которая возвращает значение Int, и будет использовать эту версию. Аналогично в выражении 
<code>let b : Double = sum(1, 2)</code> константа <code>b</code> представляет тип Double, поэтому здесь будет применяться версия функции 
sum, которая возвращает значение Double. То есть в данном случае у нас ошибок не возникнет.</p>
<p>Однако теперь рассмотрим другую ситуацию:</p>
<pre class="brush:swift;">
func sum(_ x: Int, _ y : Int) -> Int{
    return x + y
}
...
<p>Здесь явным образом не указано, какой тип представляют константы a и b, поэтому их тип будет выводиться изрезультата 
вызова <code>sum(1, 2)</code>. Но теперь компилятор не знает, какую именно версию функции sum использовать, так как тип констант неизвестен. В итоге 
в данном случае мы столкнемся с ошибкой.</p>
...
<div class="nav"><p><a href="2.17.php">Назад</a><a href="./">Содержание</a><a href="2.18.php">Вперед</a></p></div></div>
<div class="socBlock">
