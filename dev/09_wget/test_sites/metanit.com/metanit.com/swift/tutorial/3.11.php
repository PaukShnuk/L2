...
<html >
<head>
<title>Swift | Свойства и методы класса</title>
<meta charset="utf-8" />
<meta name="description" content="Свойства и методы класса в языке программирования Swift, их наследование и переопределение, отличие от статических методов и свойств">
...
<h2>Свойства и методы класса</h2><div class="date">Последнее обновление: 02.01.2018</div>
...
<p>Ранее уже рассматривались статические свойства и методы, которые являются общими для всего типа. Однако у них есть одна проблема: 
их нельзя переопределить в производных классах:</p>
<pre class="brush:swift;">
class Exchange{
    static var usd = 59.0
    static func convert(sum: Double) -&gt; Double{
        return sum / usd
    }
}
...
<p>Например, в данном случае класс Exchange представляет обменник, который с помощью метода convert обменивает средства одной валюты 
    на usd. Но, к примеру, в банке действует обменник, в котором будет отличаться курс доллара и который будет при обмене 
вычитать дополнительные проценты за обмен. Однако переопределить функционал класса Exchange мы не сможем, так как и 
свойство и метод являются статическими.</p>
<p>Для решения этой проблемы в классе могут 
быть определены свойства и методы класса (class properties/class methods). Они похожи на статические, то есть относятся 
ко всему классу в целом, а не к отдельным его объектам. Для определения свойств и методов класса применяется 
ключевое слово <span class="b">class</span>:</p>
<pre class="brush:swift;">
class Exchange{
...
<p>К таким свойствам и методам обращение идет по имени класса, как и в случае со статическими.</p>
<p>При переопределении свойств класса следует учитывать, что свойства класса должны быть вычисляемыми, как в данном случае.</p>
<p>Также стоит учитывать, что при переопределении мы можем определить свойства и методы как статические, как в случае выше, 
тогда они также будут недоступны для переопрееления для классов-наследников от BankExchange. Либо же мы также можем 
определить их как свойства и методы класса:</p>
<pre class="brush:swift;">
class BankExchange : Exchange{
...
<div class="nav"><p><a href="3.6.php">Назад</a><a href="./">Содержание</a><a href="3.12.php">Вперед</a></p></div></div>
<div class="socBlock">
