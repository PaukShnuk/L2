...
<html >
<head>
<title>Go | Добавление данных</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление данных в БД в веб-приложениях на языке программирования Go, отправка форм и обработка запросов POST">
...
<h2>Добавление данных</h2><div class="date">Последнее обновление: 03.03.2018</div>
...
<p>Процесс добавления данных разбивается на две части. Вначале нам надо показать пользователю форму для ввода данных. Затем, когда пользователь отправит 
введенные данные, нам надо их получить и добавить в базу данных.</p>
<p>Возьмем проект из прошлой темы. И прежде всего добавим в папку templates новый файл <span class="b">create.html</span>, который будет содержать 
форму для добавления:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/4.2.png" alt="Create in Database in Golang" />
...
<p>Для добавления данных определена функция <code>CreateHandler()</code>. Поскольку нам надо, с одной стороны, отображать пользователю форму для добавления, 
...
"GET", то мы будем возвращать форму для добавления. Если запрос имеет тип "POST", то парсим данные полученной формы и извлекаем из них нужные элементы.</p>
<p>Для извлечения нужных данных из полученных форм применяется метод <code>r.FormValue()</code>. В качестве параметра этому методу передается название данных. То есть, например, на форме есть 
следующее поле:</p>
<pre class="brush:xml;">
&lt;input type="text" name="model" /&gt;
</pre>
<p>Атрибут name указывает, что название этого поля - "model". Следовательно, чтобы получить введенные в него данные, необходимо использовать выражение 
<code>model := r.FormValue("model")</code>.</p>
...
<p>После этого выполняется переадресация на корень сайта с помощью функции <span class="b">http.Redirect</span>:</p>
<pre class="brush:go;">http.Redirect(w, r, "/", 301)</pre>
<p>Третий параметр указывает путь переадресации. В данном случае "/", по которому выводится список объектов из БД. 
Четвертый параметр представляет статусный код переадресации. В данном случае код 301 указывает, что переадресация временная.</p>
...
<img src="pics/4.3.png" alt="Добавление данных в бд в GO" />
<p>И после отправки формы приложение получит данные, добавит их в БД  и переадресует на главную страницу:</p>
<img src="pics/4.4.png" alt="Переадресация redirect в GO" />
