...
<html >
<head>
<title>Go | Редактирование данных</title>
<meta charset="utf-8" />
<meta name="description" content="Редактирование данных в БД в веб-приложениях на языке программирования Go, получение идентификатора редактируемого объекта и его изменение в базе данных">
...
<h2>Редактирование данных</h2><div class="date">Последнее обновление: 03.03.2018</div>
...
<p>Редактирование данных, как и добавление, разбивается на две части. Вначале нам надо отобразить пользователю форму для изменения выбранного объекта. 
Потом нам надо получить отправленные данные и сохранить их в базу данных.</p>
<p>Прежде всего определим форму для редактирования. Для этого в папке <span class="b">templates</span> создадим файл <span class="b">edit.html</span>.</p>
<img src="pics/4.5.png" alt="Edit data in Golang" />
<p>Определим в файле <span class="b">edit.html</span> следующий код:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Чтобы указать, какой объект будет редактироваться, мы будем передавать через адрес id этого объекта. И для упрощения маршрутизации в 
данном случае мы будем использовать пакет <span class="b">gorilla/mux</span>.</p>
...
<p>На случай, если в базе данных не окажется объекта с подобным id, с помощью функции <code>http.Error()</code> возвращаем статусный код 404, который 
указывает, что объект не найден.</p>
...
<p>В функции main эти функции EditPage и EditHandler связываются с определенными маршрутами. По сути они привязаны к одному и тому же маршруту, однако для 
разного типа запросов: EditPage для запросов GET, а EditHandler - для запросов POST.</p>
<pre class="brush:go;">
router.HandleFunc("/edit/{id:[0-9]+}", EditPage).Methods("GET")
router.HandleFunc("/edit/{id:[0-9]+}", EditHandler).Methods("POST")
</pre>
<p>Стоит отметить, что добавление данных, которое представлено в данном случае функцией CreateHandler, также фактически выполняет два действия в зависимости от типа запроса: отображает страницу для добавления и 
собственно добавляет данные. И в прицнипе организацию добавления можно сделать также, как и редактирование, разделив на две функции для каждого типа запросов.</p>
<p>Для упрощения управлением объектами изменим файл <span class="b">index.html</span>, добавив в него ссылки на редактирование:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<img src="pics/4.6.png" alt="Редактирование данных в БД в Go" />
