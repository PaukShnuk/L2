...
<html >
<head>
<title>Go | Первое веб-приложение</title>
<meta charset="utf-8" />
<meta name="description" content="Создание первого веб-приложения на языке программирования Go, функция ListenAndServe, определение веб-сервера">
...
<h1>Основы веб-программирования в Go</h1><div class="date">Последнее обновление: 28.02.2018</div>
...
<p>Особую сферу разработки на Go занимает веб-программирование, которое представляет создание различных веб-приложений, в том числе различных веб-сайтов, 
...
производительности, либо когда важны какие-то определенные возможности Go, например, параллельная обработка запросов.</p>
<h2>Первое веб-приложение</h2>
<p>Основной функционал для создания веб-приложений в Go расположен в пакете <span class="b">net/http</span>. В частности, чтобы запустить веб-приложение, 
которое могло бы принимать входящие запросы, достаточно вызвать функцию <span class="b">http.ListenAndServe</span>:</p>
<pre class="brush:go;">func ListenAndServe(addr string, handler Handler) error</pre>
<p>Первый параметр указывает, запросы по какому пути будут обслуживаться веб-приложением. Второй параметр определяет обработчик запроса в виде интерфейса Handler. 
Этот интерфейс определяет функцию <code>ServeHTTP</code>:</p>
<pre class="brush:go;">
type Handler interface { 
	ServeHTTP(ResponseWriter, *Request) 
}
</pre>
<p>Функция ServeHTTP принимает два параметра. Первый параметр - объект ResponseWriter представляет поток ответа, в который мы моем записать любые данные, которые мы хотим отправить в ответ пользователю. 
Второй параметр - Request инкапсулирует всю информацию о запросе.</p>
<p>Например, определим простейшее веб-приложение. Для этого создадим на жестком диске каталог для хранения файлов с исходным кодом на языке Go, допустим, он 
будет называться <span class="ii">golang</span>. И определим в этом каталоге файл <span class="b">server.go</span> со следующим кодом:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>В данном случае определен кастомный тип msg на основе типа string, который реализует метод ServeHTTP интерфейса Handler. В самом методе с помощью 
вызова <code>fmt.Fprint(resp, m)</code> в поток ответа resp пишется сообщение, которое хранится в строке m. Таким образом, пользователю отправляется ответ.</p>
...
<p>Первый параметр указывает, что веб-приложение будет запускаться по адресу <code>localhost:8181</code>. Номер порта необязательно должен быть 8181. 
Это может быть любой незанятый порт.</p>
<p>Запустим веб-приложение:</p>
<img src="pics/1.1.png" alt="Веб-сервер в Golang" />
<p>И затем в любом браузере обратимся по адресу "http://localhost:8181/":</p>
<img src="pics/1.2.png" alt="Веб-приложение в языке Go" />
