...
<html >
<head>
<title>Go | Чтение с консоли</title>
<meta charset="utf-8" />
<meta name="description" content="Чтение с консоли с помощью объекта os.Stdin в языке программирования Go, функции fmt.Scan, fmt.Scanf, fmt.Scanln, получение данных в переменные">
...
<h2>Чтение с консоли</h2><div class="date">Последнее обновление: 15.01.2018</div>
...
<p>В Go имеется объект <span class="b">os.Stdin</span>, который реализует интерфейс io.Reader и позволяет считывать данные с консоли. Например, мы можем использовать 
функцию <code>fmt.Fscan()</code> для считывания с консоли с помощью os.Stdin:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>При запуске программы мы сможем вводить данные с консоли, и они перейдут в переменные name и age:</p>
<div class="console">
...
<p>Однако также для получения ввода с консоли мы можем использовать встроенные функции <span class="b">fmt.Scan()</span>, 
<span class="b">fmt.Scanln()</span> и <span class="b">fmt.Scanf()</span>, которые аналогичны соответственно функциям <code>fmt.Fscan()</code>, 
<code>fmt.Scanln()</code> и <code>fmt.Scanf()</code>:</p>
<pre class="brush:go;">
...
<p>В случае если вводятся сразу несколько значений, то разделителем между ними является пробел. Хотя теоретически строка может включать внутренние пробелы, тем не менее 
данные функции считывают значение строки и других типов данных до пробела:</p>
<div class="console">
<pre class="consoletext">
