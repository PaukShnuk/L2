...
<html >
<head>
<title>Go | Первая программа</title>
<meta charset="utf-8" />
<meta name="description" content="Первая программа на языке программирования Go в консоли, использование компилятора">
...
<h2>Первая программа</h2><div class="date">Последнее обновление: 03.12.2021</div>
...
<p>Создадим первую программу на языке Go. Для написания кода нам потребуется какой-нибудь текстовый редактор. Можно взять любой 
редактор, например, встроенный блокнот или популярный Notepad++ или любой другой. Для трансляции исходного кода в приложение необходим компилятор.</p>
<h3>Создание программы</h3>
<p>Определим на жестком диске папку для хранения файлов с исходным кодом. Допустим, в моем случае это будет папка <span class="b">C:\golang</span>. 
В этой папке создадим новый текстовый файл, который переименуем в <span class="b">hello.go</span>.</p>
<img src="./pics/1.6.png" alt="Первая программа на языке Go" />
<p>Откроем этот файл в любом текстовом редакторе и определим в нем следующий код:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Что в этой программе делается? Программа на языке Go определяется в виде пакетов. Программный код должен быть определен в каком-то определенном пакете. Поэтому 
в самом начале файла с помощью оператора <span class="b">package</span> указывается, к какому пакету будет принадлежать файл. В данном случае это пакет main:</p>
<pre class="brush:go">package main</pre>
<p>Причем пакет должен называться именно main, так как именно данный пакет определяет исполняемый файл.</p>
<p>При составлении программного кода нам может потребоваться функционал из других пакетов. В Go есть множество встроенных пакетов, которые содержат код, 
выполняющий определенные действия. Например, в нашей программе мы будем выводить сообщение на консоль. И для этого нам нужна функция <span class="b">Println</span>, 
которая определена в пакете <span class="b">fmt</span>. Поэтому второй строкой с помощью директивы <span class="b">import</span> мы подключаем этот пакет:</p>
<pre class="brush:go;">import "fmt"</pre>
<p>Далее идет функция main. Это главная функция любой программы на Go. По сути все, что выполняется в программе, выполняется именно функции main.</p>
<p>Определение функции начинается со слова <span class="b">func</span>, после которого следует название функции, то есть main. 
После названия функции в скобках идет перечисление параметров. Так как функция main не принимает никаких параметров, то в данном случае указываются пустые скобки.</p>
<p>Затем в фигурных скобках определяется тело функции main - те действия, которые собственно и выполняет функция.</p>
<pre class="brush:go;">func main() {</pre>
<p>В нашем случае функция выводит на консоль строку "Hello Go!". Для этого применяется функция <span class="b">Println()</span>, которая определена в пакете fmt. 
Поэтому при вызове функции вначале указывается имя пакета и через точку имя функции. А в скобках функции передается то сообщение, которое она должна выводить на консоль:</p>
<pre class="brush:go;">fmt.Println("Hello Go!")</pre>
<h3>Компиляция и выполнение программы</h3>
<p>Теперь скомпилируем и выполним данную программу. Для этого необходимо передать файл с исходным кодом компилятору go.exe и указать нужную команду. 
Для этого откроем командную строку(терминал) и перейдем в ней с помощью команды <span class="b">cd</span> к папке, где храниться файл с исходным кодом 
<span class="ii">hello.go</span> (в моем случае это папка <span class="ii">C:\golang</span>):</p>
<div class="console"><pre class="consoletext">cd C:\golang</pre></div>
<p>Затем выполним программу с помощью следующей команды:</p>
<div class="console"><pre class="consoletext">go run hello.go</pre></div>
<p><span class="b">go</span> - это компилятор. Поскольку при установке путь к компилятору автоматически прописывается в переменную PATH в переменных окружения, 
то нам не надо указывать полный путь <span class="b">C:\Go\bin\go.exe</span>, а достаточно написать просто имя приложения go. Далее идет параметр 
<span class="b">run</span>, который говорит, что мы просто хотим выполнить программу. И в конце указывается собственно файл программы hello.go.</p>
<p>В итоге после выполнения на консоль будет выведено сообщение "Hello Go!".</p>
<img src="./pics/1.7.png" alt="Выполнение программы на языке Go" />
<p>Данная команды выполняет, но не компилирует программу в отдельный исполняемый файл. Для компиляции выполним другую команду:</p>
<div class="console"><pre class="consoletext">go build hello.go</pre></div>
<p>После выполнения этой команды в папке с исходным файлом появится еще один файл, который будет называться hello.exe и который мы можем запускать. 
После этого опять же мы можем выполнить программу, запустив в консоли этот файл:</p>
<img src="./pics/1.8.png" alt="Компиляция программы на языке Go в Windows" />
