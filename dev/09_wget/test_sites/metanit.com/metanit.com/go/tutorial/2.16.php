...
<html >
<head>
<title>Go | Анонимные функции</title>
<meta charset="utf-8" />
<meta name="description" content="Анонимные функции в языке программирования Go, доступ к окружению и замыкания, определение анонимных функций в виде переменных и передача их в качестве параметров в функции">
...
<h2>Анонимные функции</h2><div class="date">Последнее обновление: 26.12.2017</div>
...
<p>Анонимные функции - это функции, которым не назначено имя. Они отличаются от обычных функций также тем, что они могут определяться внутри других функций и также могут 
иметь доступ к контексту выполнения.</p>
<p>Анонимные функции позволяют нам определить некоторое действие непосредственно там, где оно применяется. Например, нам надо выполнить сложение двух чисел, но больше нигде 
это действие в программе не нужно:</p>
<pre class="brush:go;">
package main
...
<p>Фактически переменная f определена как:</p>
<pre class="brush:go;">var f func(int, int) int = func(x, y int) int{ return x + y}</pre>
<p>То есть переменной f можно присвоить любую функцию, которая соответствует типу <code>func(int, int) int</code>.</p>
<h3>Анонимная функция как аргумент функции</h3>
<p>Очень удобно использовать анонимные функции в качестве аргументов других функций:</p>
<pre class="brush:go;">
package main
...
<h3>Анонимная функция как результат функции</h3>
<p>Анонимная функция может быть результатом друой функции:</p>
<pre class="brush:go;">
package main
...
<h3>Доступ к окружению</h3>
<p>Преимуществом анонимных функций является то, что они имеют доступ к окружению, в котором они вызываются. Например:</p>
<pre class="brush:go;">
package main
...
<p>Здесь функция square определяет локальную переменную x и возвращает анонимную функцию. Анонимная функция увеливичает значение переменной x и возвращает ее квадрат.</p>
<p>Таким образом, мы можем зафисировать у внешней функции square состояние в виде переменной x, которое будет изменяться в анонимной функции.</p>
