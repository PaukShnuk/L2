...
<html >
<head>
<title>Go | Отправка запросов по HTTP</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка запросов по HTTP в языке программирования Go">
...
<h2>Отправка запросов по HTTP</h2><div class="date">Последнее обновление: 20.01.2018</div>
...
<p>Особую область применения Go представляют запросы по протоколу HTTP. 
...
http были выделены в отдельный пакет <a href="https://golang.org/pkg/net/http/" rel="nofollow">net/http</a>.</p>
<p>Для отправки запросов в пакете net/http определен ряд функций:</p>
<pre class="brush:go;">
...
<ul>
<li><p><span class="b">Get()</span>: отправляет запрос GET</p></li>
<li><p><span class="b">Head()</span>: отправляет запрос HEAD</p></li>
<li><p><span class="b">Post()</span>: отправляет запрос POST</p></li>
<li><p><span class="b">PostForm()</span>: отправляет форму в запросе POST</p></li>
</ul>
<p>Рассмотрим выполнение самого простого запроса - запроса GET, для которого применяется одноименный метод:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>Метод <code>Get()</code> в качестве параметра принимает адрес ресурса, к которому надо выполнить запрос, и возвращает объект <code>*http.Response</code>, 
...
интерфейс <code>io.ReadCloser</code>. А это значит, что это поле по сути является потоком для чтения, и 
...
<code>defer resp.Body.Close()</code> и в цикле считываем через метод Read данные и выводим на консоль.</p>
<img src="pics/8.1.png" alt="HttpClient in Golang" />
<p>Поскольку в данном случае ответ от веб-ресурса все равно выводится на консоль, то мы можем сократить код:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
