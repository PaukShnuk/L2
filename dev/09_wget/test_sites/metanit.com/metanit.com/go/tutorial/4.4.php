...
<html >
<head>
<title>Go | Методы</title>
<meta charset="utf-8" />
<meta name="description" content="Методы в языке программирования Go, отличие от функций, методы в структурах">
...
<h2>Методы</h2><div class="date">Последнее обновление: 26.12.2017</div>
...
<p>Метод представляет функцию, связанную с определенным типом. Методы определяются также как и обычные функции за тем исключением, что в определении 
метода также необходимо указать получателя или receiver. Получатель - это параметр того типа, к которому прикрепляется метод:</p>
<pre class="brush:go;">
func (имя_параметра тип_получателя) имя_метода (параметры) (типы_возвращаемых_результатов){
	тело_метода
}
</pre>
<p>Допустим, у нас будет определен именованный тип, представляющий срез из строк:</p>
<pre class="brush:go;">type library []string</pre>
<p>Для вывода всех элементов из среза мы можем определить следующий метод:</p>
<pre class="brush:go;">
...
<p>Та часть, которая расположена между ключевым словом <code>func</code> и именем метода и представляет определение <span class="ii">получателя</span>, 
для которого будет определен этот метод: <code>(l library)</code>. Используя параметр получателя (в данном случае l), мы можем обращаться к получателю. 
...
<p>lib является объектом типа library, поэтому для него мы можем вызвать метод print. В данном случае объект lib - это и будет то значение, которое будет 
передаваться в функцию print через параметр <code>(l library)</code>.</p>
<h3>Методы структур</h3>
<p>Подобным образом мы можем определять методы и для структур:</p>
<pre class="brush:go;">
package main
...
<p>В данном случае для структуры person определены две функции: print и eat. Функция print выводит информацию о текущем объекте person. А функция 
eat имитирует употребление пищи. Каждая из этих функций определяет объект и тип структуры, к которой функция относится:</p>
<pre class="brush:go;">
func (p person) имя_функции
</pre>
<p>С помощью объекта p мы можем обращаться к свойствам структуры person. В остальном это обычные функции, которые могут принимать параметры и возвращать результат.</p>
<p>Для обращения к функциям структуры указывается переменная структуры и через точку идет вызов функциии.</p>
<pre class="brush:go;">
tom.print()
tom.eat("борщ")
</pre>
<p>В данном случае tom - это и будет объект <code>p person</code> в определении функции.</p>
