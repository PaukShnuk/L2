...
<html >
<head>
<title>Go | Реляционные базы данных</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с реляционными база данных в языке программирования Go, пакет database/sql">
...
<h1>Базы данных</h1><h2>Работа с реляционными база данных</h2><div class="date">Последнее обновление: 31.01.2018</div>
...
<p>Для работы с реляционными базами данных в языке Go применяется встроенный пакет 
<a href="https://golang.org/pkg/database/sql/" rel="nofollow">database/sql</a>. Однако он не используется сам по себе. Он лишь предоставляет универсальный интерфейс для работы с базами данных. 
Для работы с конкретной СУБД нам также необходим драйвер. Список доступных драйверов можно найти <a href="https://github.com/golang/go/wiki/SQLDrivers" rel="nofollow">здесь</a>. 
Однако поскольку драйвера реализуют одни и те же интерфейсы, то в принципе работа с различными СУБД будет идентична.</p>
<p>Для того, чтобы начать работу с базой данных, необходимо открыть подключение с помощью функции <span class="b">Open()</span>:</p>
<pre class="brush:go;">func Open(driverName, dataSourceName string) (*DB, error)</pre>
<p>Эта функция принимает в качестве параметров имя драйвера и имя источника данных, к которому надо подключаться. Возвращает функция 
...
ошибке.</p>
<p>Затем взаимодействие с базой данных осуществляется посредством методов объекта DB.</p>
<pre class="brush:go;">
...
<ul>
<li>
<p>Метод <span class="b">Exec()</span> выполняет некоторое sql-выражение, которое передается через первый параметр, не возвращая никакого результата. 
...
абстрактная операция добавления данных в БД, которая предполагает выполнение команды INSERT:</p>
<pre class="brush:go;">
result, err := db.Exec("INSERT INTO Products (model, company, price) VALUES ('iPhone X', 'Apple', 72000)")
</pre>
<p>Способ вставки дополнительных параметров в SQL-выражение зависит от конкретного драйвера. Также этот метод подходит для выполнения команд UPDATE (обновление) и DELETE (удаление).</p>
<p>Метод возвращает объект <code>Result</code>. Это интерфейс определяет два метода:</p>
<pre class="brush:go;">
...
<li><p>Метод <span class="b">Query()</span> для выполнения запроса, который возвращает какие-либо данные. Обычно это запросы, которые содержат команду SELECT.</p>
<pre class="brush:go;">
rows, err := db.Query("SELECT name FROM users WHERE age=23")
</pre>
<p>Результатом запроса является объект <span class="b">*Rows</span> - по сути набор строк. С помощью ряда его методов можно извлечь полученные данные:</p>
<pre class="brush:go;">
...
<p>Общий принцип чтения набора строк выглядит примерно следующим образом:</p>
<pre class="brush:go;">
...
<p>Вначале выполняем запрос к базе данных с помощью метода <code>db.Query</code>, затем с помощью метода <code>Next()</code> последовательно считываем все строки из набора. 
Если строк в наборе нет, то метод возвращает <code>false</code>, и происходит ыход из цикла. Если строки еще есть, то указатель *Rows переходит к следующей строке. 
И затем мы можем считать в переменные с помощью метода <code>Scan()</code> данные из текущей строки.</p>
</li>
<li><p>Метод <span class="b">QueryRow()</span> возвращает одну строку в виде объекта <span class="b">*Row</span>. Как правило, этот метод применяется для получение единичного объекта, 
например, по id. Этот объект имеет метод <code>Scan()</code>, который позволяет извлечь данные из строки:</p>
<pre class="brush:go;">func (r *Row) Scan(dest ...interface{}) error</pre>
</li>
</ul>
<p>Также стоит отметить, что язык Go поддерживает создание запросов с помощью объекта <span class="b">Stmt</span>, в который можно вводить различные данные и который повышает производительность. 
И также в Go имеется поддержка транзакций в виде объекта <span class="b">Tx</span>.</p>
<p>Все эти вещи по разному реализуются в различных драйверах для конкретных систем управления базами данных. Но общие принципы будут одни и те же. То есть общая 
структура работы с различными база данных благодаря единому интерфейсу будут совпадать.</p>
