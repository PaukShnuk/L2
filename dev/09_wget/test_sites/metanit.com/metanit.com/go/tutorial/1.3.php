...
<html >
<head>
<title>Go | LiteIDE</title>
<meta charset="utf-8" />
<meta name="description" content="Первая программа на языке программирования Go в интегрированной среде разработки LiteIDE">
...
<h2>Go в LiteIDE</h2><div class="date">Последнее обновление: 20.12.2017</div>
...
<p>Использование интегрированных сред разработки (IDE) в ряде случаев упрощает управление проектом и создание приложения. Для языка Go 
...
Официальный сайт IDE - <a href="http://liteide.org/en/" rel="nofollow">http://liteide.org/en/</a>.</p>
<p>Непосредственно загрузить все файлы данной IDE можно по ссылке: <a href="https://sourceforge.net/projects/liteide/files/" rel="nofollow">https://sourceforge.net/projects/liteide/files/</a>.</p>
<img src="pics/1.9.png" alt="LiteIDE X в GoLang" />
<p>На этой странице можно найти ссылку для загрузки. Причем ничего не надо устанавливать. По ссылке загружается zip-архив, который после загрузки достаточно распаковать. 
После распаковки архива в корневой папке в каталоге bin можно найти файл исполняемой программы <span class="b">liteide.exe</span> (на Windows), через который и можно запустить среду разработки:</p>
<img src="pics/1.10.png" alt="LiteIDE и язык программирования Go" />
<p>Запустим этот файл, и по умолчанию нам отроется стартовая странца приветствия. LiteIDE имеет локализованные версии (в том числе русифицированную версию), и через 
параметры можно переключить язык среды на любой из предложенных.</p>
<p>Для создания нового проекта перейдем к меню <span class="b">File(Файл)</span> -&gt; <span class="b">New (Создать)</span>:</p>
<img src="pics/1.11.png" alt="Первый проект на языке Go в LiteIDE" />
<p>После этого отобразится окно выбора шаблона проекта:</p>
<img src="pics/1.12.png" alt="Шаблоны проектов в LiteIDE" />
<p>В качестве шаблона выберем <span class="b">Empty File</span>. А внизу в поле <span class="b">Name</span> укажем для файла имя <span class="ii">hello.go</span>. 
В другом поле можно увидеть путь к каталогу, где будет располагаться файл. Этот путь мы также можем изменить. И затем проект будет создан, и откроется окно, где надо будет подтвердить открытие проекта:</p>
<img src="pics/1.13.png" />
<p>После этого в центральной части окна программы откроется текстовый редактор. Введем в него следующий код:</p>
<pre class="brush:go;">
package main
...
<img src="pics/1.14.png" alt="Создание проекта на языке Go в LiteIDE X" />
<p>Для сборки и запуска проекта нажмем на панели инструментов кнопку <span class="b">BR</span>. И внизу окна IDE отобразится 
поле вывода, где мы можем увидеть результат выполнения программы.</p>
<p>Также в LiteIDE можно создавать и использовать другие типы проектов. Например, создадим проект <span class="b">Go1 Command Project</span>, который пусть называется 
<span class="ii">hello</span>:</p>
<img src="pics/1.15.png" />
<p>В результате будет создан проект, который располагается в отдельной папке и который по умолчанию состоит из двух файлов. И данный проект подобным образом мы можем запустить:</p>
<img src="pics/1.16.png" alt="Проект Go1 Command Project в LiteIDE X" />
