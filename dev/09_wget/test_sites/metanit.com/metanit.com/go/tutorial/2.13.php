...
<html >
<head>
<title>Go | Срезы</title>
<meta charset="utf-8" />
<meta name="description" content="Срезы (slice) в языке программирования Go, определение срезов, добавление, перебор и удаление элементов">
...
<h2>Срезы</h2><div class="date">Последнее обновление: 21.12.2017</div>
...
<p>Срезы (slice) представляют последовательность элементов одного типа переменной длины. В отличие от массивов длина в срезах не иксирована и динамически может меняться, 
то есть можно добавлять новые элементы или удалять уже существующие.</p>
<p>Срез определяется также, как и массив, за тем исключением, что у него не указывается длина:</p>
<pre class="brush:go;">
var users []string
</pre>
<p>Также срез можно инициализировать значениями:</p>
<pre class="brush:go;">
var users = []string{"Tom", "Alice", "Kate"}
// или так
users2 := []string{"Tom", "Alice", "Kate"}
</pre>
<p>К элементам среза обращение происходит также, как и к элементам массива - по индексу и также мы можем перебирать все элементы с помощью цикла 
for:</p>
<pre class="brush:go;">
...
<p>С помощью функции <span class="b">make()</span> можно создать срез из нескольких элементов, которые будут иметь значения по умолчанию:</p>
<pre class="brush:go;">
...
<h3>Добавление в срез</h3>
<p>Для добавления в срез применяется встроенная функция <span class="b">append(slice, value)</span>. Первый параметр функции - срез, в который надо добавить, а второй параметр - 
значение, которое нужно добавить. Результатом функции является увеличенный срез.</p>
<pre class="brush:go;">
...
<h3>Оператор среза</h3>
<p>Оператор среза s[i:j] создает из последовательности s новый срез, который содержит элементы последовательности s с i по j-1. 
При этом должно соблюдаться условие <code>0 &lt;= i &lt;= j &lt;= cap(s)</code>. В качестве исходной последовательности, из которой берутся элементы, 
может использоваться массив, указатель на массив или другой срез. В итоге в полученном срезе будет j-i элементов.</p>
<p>Если значение i не указано, то применяется по умолчанию значение 0. Если значение j не указано, то вместо него используется длина исходной последовательности s.</p>
<pre class="brush:go;">
// исходный массив
...
<p>Что делать, если необходимо удалить какой-то определенный элемент? В этом случае мы можем комбинировать функцию append и оператор среза:</p>
<pre class="brush:go;">
...
<div class="nav"><p><a href="2.18.php">Назад</a><a href="./">Содержание</a><a href="2.14.php">Вперед</a></p></div></div>
<div class="socBlock">
