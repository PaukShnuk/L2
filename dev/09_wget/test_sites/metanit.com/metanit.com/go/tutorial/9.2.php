...
<html >
<head>
<title>Go | Сервер. Обработка подключений</title>
<meta charset="utf-8" />
<meta name="description" content="Создание сервера в языке программирования Go, функция net.Listen,  интерфейс net.Listener и обработка входящих подключений">
...
<h2>Сервер. Обработка подключений</h2><div class="date">Последнее обновление: 16.01.2018</div>
...
<p>Для прослушивания и приемы входящих запросов в пакете net определена функция <span class="b">net.Listen</span>:</p>
<pre class="brush:go;">func Listen(network, laddr string) (net.Listener, error)</pre>
<p>Функция принимает два параметра: network - протокол, по которому приложение будет получать запросы, и laddr представляет локальный адрес, по 
которому будет запускаться сервер. Протокол должен представлять одно из значений: "tcp", "tcp4", "tcp6", "unix", "unixpacket". Локальный адрес может содержать только номер порта, например, ":8080". В этом случае приложение будет обслуживать по всем.</p>
<p>В случае успешного выполнения функция возвращает объект интерфейса <span class="b">net.Listener</span>, который представляет функционал для приема входящих подключений. 
В зависимости от типа используемого протокола возвращаемый объект Listener может представлять тип <code>net.TCPListener</code> или 
<code>net.UnixListener</code> (оба этих типа реализуют интерфейс net.Listener).</p>
<p>Основные методы, которые представляет net.Listener: <span class="b">Accept()</span> (принимает входящее подключение) и <span class="b">Close()</span> (закрывает подключение).</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>Вначале в функции <code>net.Listen("tcp", ":4545")</code> устанавливается 4545 порт для прослушивания подключений по протоколу TCP. После вызова этой функции 
сервер запущен и готов принимать подключения. Затем в бесконечном цикле for получаем входящие подключения с помощью вызова <code>listener.Accept()</code>. Этот метод возвращает объект net.Conn, который представляет 
...
принимает срез байтов, то любые сообщения надо транслировать в срез байтов: <code>conn.Write([]byte(message))</code></p>
<p>Для тестирования сервера определим еще одну программу - клиент:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>Поскольку сервер будет запущен на локальном компьтере на порте 4545, то клиент подлючается к этому адресу: <code>net.Dial("tcp", "127.0.0.1:4545")</code></p>
<p>После этого к серверу будет отправляться запрос, и с помощью вызова <code>io.Copy(os.Stdout, conn)</code> выводим полученный ответ на консоль.</p>
<p>Вначале запустим сервер. На Windows может отобразиться окно с предложением разрешить доступ:</p>
<img src="pics/9.2.png" alt="Запуск сервера на Go" />
<p>И после этого будет запущен сервер:</p>
<img src="pics/9.3.png" alt="Запуск сервера TCP на Golang" />
<p>Затем запустим клиент:</p>
<img src="pics/9.4.png" alt="Подключение клиента к серверу TCP на Go" />
<p>После запуска клиент подключится к серверу и получит от него сообщение.</p>
