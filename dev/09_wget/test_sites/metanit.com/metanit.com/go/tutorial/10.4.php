...
<html >
<head>
<title>Go | SQLite</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с базой данных SQLite в языке программирования Go, операции с данными, пакет github.com/mattn/go-sqlite3">
...
<h2>SQLite</h2><div class="date">Последнее обновление: 31.01.2018</div>
...
<p>Для работы с SQLite в Go нам потребуется драйвер <a href="https://github.com/mattn/go-sqlite3" rel="nofollow">go-sqlite3</a>. Для использования 
драйвера вначале установим его, выполнив в командной строке/терминале следующую команду:</p>
<div class="console"><pre class="consoletext">go get github.com/mattn/go-sqlite3</pre></div>
<p>Пусть в папке со скриптом программы на Go у нас будет создана база данных SQLite, которая называется <span class="b">store.db</span>, и в ней будет таблица products, которая описывается следующим скриптом:</p>
<pre class="brush:sql;">
...
<img src="./pics/11.5.png" alt="Работа с базой данных SQLite в Go" />
<h3>Открытие подключения</h3>
<p>Для открытия соединения с базой данных в функцию <span class="b">sql.Open()</span> передается имя драйвера "sqlite3" и путь к файлу базы данных:</p>
<pre class="brush:go;">
...
<p>Для добавления используется метод <span class="b">Exec()</span> объекта DB:</p>
<pre class="brush:go;">
package main
import (
	"database/sql"
...
<p>Через дополнительные параметры метода <code>db.Exec()</code> можно передавать значения выполняемому sql-выражению через 
плейхолдеры $1, $2 и так далее, вместо которых вставляются значения дополнительных параметров метода db.Exec.</p>
<p>У результата операции есть метод <code>result.LastInsertId()</code>, который возвращает id последнего добавленного объекта, и метод <code>result.RowsAffected()</code>, который позволяет получить 
количество добавленных строк:</p>
<img src="./pics/11.6.png" alt="Добавление в базу данных SQLite в языке Go" />
<h3>Получение данных</h3>
<p>Для получения данных применяется метод <code>db.Query()</code>, который возвращает набор строк, либо <code>db.QueryRow()</code>, который возвращает 
одну строку:</p>
<pre class="brush:go;">
package main
import (
...
<img src="./pics/11.7.png" alt="Получение данных из базы данных SQLite в языке Go" />
