...
<html >
<head>
<title>Go | Отображения</title>
<meta charset="utf-8" />
<meta name="description" content="Отображения (map) в языке программирования Go, определение отображений, ключи и значения, добавление, перебор и удаление элементов, хеш-таблица">
...
<h2>Отображения</h2><div class="date">Последнее обновление: 21.01.2017</div>
...
<p>Отображение или map представляет ссылку на хеш-таблицу - структуру данных, где каждый элемент представляет пару "ключ-значение". При этом каждый элемент имеет уникальный ключ, по которому можно получить 
значение элемента. Отображение определяется как объект типа <span class="b">map[K]V</span>, где К представляет тип ключа, а V - тип значения. 
Причем тип ключа K должен поддерживать операцию сравнения ==, чтобы оображение могло сопоставить значение с одним из ключей и хеш-таблицы.</p>
...
<p>Установка значений отображения:</p>
<pre class="brush:go;">
...
<p>Как и в массиве или в срезе элементы помещаютя в фигурные скобки. Каждый элемент представляет пару ключ -значение. Вначале идет ключ и через двоеточие значение. Определение элемента завершается запятой.</p>
<p>Для обращения к элементам нужно применять ключи:</p>
<pre class="brush:go;">
var people = map[string]int{ 
	"Tom": 1,
	"Bob": 2,
...
<p>Для проверки наличия элемента по определенному ключу можно применять выражение if:</p>
<pre class="brush:go;">
var people = map[string]int{ 
	"Tom": 1,
	"Bob": 2,
...
<p>Если значение по заданному ключу имеется в отображении, то переменная ok будет равна true, а переменная val будет хранить полученное значение. 
Если переменная ok равна false, то значения по ключу в отображении нет.</p>
<p>Для перебора элементов применяется цикл for:</p>
<pre class="brush:go;">
var people = map[string]int{ 
	"Tom": 1,
	"Bob": 2,
...
<p>Функция <span class="b">make</span> представляет альтернативный вариант создания отображения. Она создает пустую хеш-таблицу:</p>
<pre class="brush:go;">people := make(map[string]int)</pre>
<h3>Добавление и удаление элементов</h3>
<p>Для добавления элементов достаточно просто выполнить установку значения по новому ключу и элемент с этим ключом будет добавлен в коллекцию:</p>
<pre class="brush:go;">
var people = map[string]int{ "Tom": 1, "Bob": 2}
people["Kate"] = 128
fmt.Println(people)		// map[Tom:1  Bob:2  Kate:128]
</pre>
<p>Для удаления применяется встроенная функция <span class="b">delete(map, key)</span>, первым параметром которой является отображение, а вторым - ключ, по которому надо 
удалить элемент.</p>
<pre class="brush:go;">
