...
<html >
<head>
<title>Go | Структуры</title>
<meta charset="utf-8" />
<meta name="description" content="Структуры в языке программирования Go, определение и использование структур, доступ к полям структур, указатели на структуры">
...
<h2>Структуры</h2><div class="date">Последнее обновление: 24.12.2017</div>
...
<p>Структуры представляют тип данных, определяемый разработчиком и служащий для представления каких-либо объектов. Структуры содержат набор полей, 
которые представляют различные атрибуты объекта. Для определения структуры применяются ключевые слова <span class="b">type</span> и <span class="b">struct</span>:</p>
<pre class="brush:go;">
type имя_структуры struct{
	поля_структуры
}
</pre>
<p>Каждое поле имеет название и тип данных, как переменная. Например, определим структуру, которая представляет человека:</p>
<pre class="brush:go;">
type person struct{
	name string
	age int
...
<p>Структура называется person. Она имеет два поля: name (имя человека, представляет тип string) и age (возраст человека, представляет тип int).</p>
<h3>Создание и инициализация структуры</h3>
<p>Структура представляет новый тип данных, и мы можем определить переменную данного типа:</p>
<pre class="brush:go;">
var tom person
</pre>
<p>С помощью инициализатора можно передать структуре начальные значения:</p>
<pre class="brush:go;">
var tom person = person{"Tom", 23}
</pre>
<p>Инициализатор представляет набор значений в фигурных скобках. Причем эти значения передаются полям структуры в том порядке, в 
...
передается второму полю - age.</p>
<p>Также мы можем явным образом указать какие значения передаются свойствам:</p>
<pre class="brush:go;">
var alice person = person{age: 23, name: "Alice"}
</pre>
<p>Также можно использовать сокращенные способы инициализации переменной структуры:</p>
<pre class="brush:go;">
var tom = person {name: "Tom", age: 24}
bob := person {name: "Bob", age: 31}
</pre>
<p>Можно даже не указывать никаких значений, в этом случае свойства структуры получат значения по умолчанию:</p>
<pre class="brush:go;">undefined := person {}	// name - пустая строка, age - 0</pre>
<h3>Обращение к полям структуры</h3>
<p>Для обращения к полям структуры после переменной ставится точка и указывается поле структуры:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Как и в случае с обычными переменнами, можно создавать указатели на структуры.</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Для инициализации указателя на структуру необязательно присваивать ему адрес переменной. Можно присвоить адрес безымянного объекта следующим образом:</p>
<pre class="brush:go;">
var tom *person = &person{name:"Tom", age:23}
var bob *person = new(person)
</pre>
<p>Для обращения к полям структуры через указатель можно использовать операцию разыменования (<code>(*tomPointer).age</code>), либо напрямую обращаться по указателю 
(<code>tomPointer.age</code>).</p>
<p>Также можно определять указатели на отдельные поля структуры:</p>
<pre class="brush:go;">
tom := person {name: "Tom", age: 22}
