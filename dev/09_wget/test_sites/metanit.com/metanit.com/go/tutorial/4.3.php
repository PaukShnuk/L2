...
<html >
<head>
<title>Go | Вложенные структуры</title>
<meta charset="utf-8" />
<meta name="description" content="Вложенные структуры в языке программирования Go, хранение в структуре ссылки на саму себя">
...
<h2>Вложенные структуры</h2><div class="date">Последнее обновление: 24.12.2017</div>
...
<p>Поля одних структур могут представлять другие структуры. Например:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>В данном случае структура person имеет поле contactInfo, которое представляет другую структуру <code>contact</code>.</p>
<p>Можно сократить определение поля следующим образом:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Поле contact в структуре person фактические эквивалентно свойству <code>contact contact</code>, то есть свойство называется contact и 
представляет тип contact. Это позволяет нам сократить путь к полям вложенной структуры. Например, мы можем написать <code>tom.email</code>, а не 
<code>tom.contact.email</code>. Хотя можно использовать и второй вариант.</p>
<h3>Хранения ссылки на структуру того же типа</h3>
<p>При этом надо учитывать, что структура не может иметь поле, которое представляет тип этой же структуры. Например:</p>
<pre class="brush:go;">
type node struct{
	value int
	next node
...
<p>Подобное определение будет неправильным. Вместо этого поле должно представлять указатель на структуру:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Здесь определена структура node, которая представляет типичный узел односвязного списка. Она хранит значение в поле value и ссылку на следующий 
узел через указатель next.</p>
<p>В функции main создаются три связанных структуры, и с помощью цикла for и вспомогательного указателя current выводятся их значения.</p>
<div class="console">
