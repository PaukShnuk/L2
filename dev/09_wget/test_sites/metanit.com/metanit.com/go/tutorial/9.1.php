...
<html >
<head>
<title>Go | Отправка запросов</title>
<meta charset="utf-8" />
<meta name="description" content="Сетевое программирование в языке Go, пакет net, объект net.Conn, создание подключение и отправка запросов к серверу, функция net.Dial">
...
<h1>Сетевое программирование</h1><div class="date">Последнее обновление: 23.01.2018</div>
...
<p>Одной из ключевых возможностей языка Go является возможность работы с сетевыми сервисами: отправлять запросы к ресурсам в сети и, наоборот, обрабатывать входящие запросы. 
Основной функционал по работе с сетью представлен пакетом <a href="https://golang.org/pkg/net" rel="nofollow">net</a>. 
Этот пакет предоставляет различные низкоуровневые сетевые примитивы, через которые идет взамодействие по сети.</p>
<h2>Отправка запросов</h2>
<p>Для отправки запросов к ресурсам в сети применяется функция <span class="b">net.Dial()</span>:</p>
<pre class="brush:go;">func Dial(network, address string) (Conn, error)</pre>
<p>Эта функция принимает два параметра: network - тип протокола и address - адрес ресурса.</p>
<p>Есть следующие типы протоколов:</p>
<ul>
<li><p><code>tcp, tcp4, tcp6</code>: протокол TCP. tcp по умолчанию представляет tcp4, цифра в конце указывает, какой тип адресов будет использоваться: IPv4 или IPv6</p></li>
<li><p><code>udp, udp4, udp6</code>: протокол UDP. udp по умолчанию представляет udp4</p></li>
<li><p><code>ip, ip4, ip6</code>: протокол IP. ip по умолчанию представляет ip4</p></li>
<li><p><code>unix, unixgram, unixpacket</code>: сокеты Unix</p></li>
</ul>
<p>Второй параметр представляет сетевой адрес ресурса (для адресов в сети интернет это домен). Это может быть числовой сетевой адрес, например, <code>"127.0.0.1"</code>. Он может включать указание порта, например, 
<code>"127.0.0.1:80"</code>. Это также может быть адрес в формате IPv6, например, <code>"::1"</code> или <code>"[2516:b7f0:3421:b16::71]:80"</code>.</p>
<p>Функция возращает объект, который реализует интерфейс <span class="b">net.Conn</span>. Этот интерфейс, в свою очередь, применяет интерфейсы 
...
в виде типов <span class="b">IPConn</span>, <span class="b">UDPConn</span>, <span class="b">TCPConn</span>. В зависимости от используемого протокола возвращается соответствующий тип.</p>
<p>Таким образом, используя данную функцию, мы можем отправлять запросы по протоколу TCP и UDP. Например:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>В данном случае мы фактически обращаемся к веб-ресурсу golang.ru. Так как <code>net.Conn</code> реализует интерфейсы <code>io.Reader</code> и <code>io.Writer</code>, то в данный объект можно записывать данные - фактически посылать по сети данные и можно считывать с него данные - 
получать данные из сети. Например, <code>conn.Write([]byte(httpRequest))</code> посылает данные, которые здесь представлены переменной httpRequest. Так как метод 
Write отправляет срез байтов, то любые данные надо преобразовать в срез байтов.</p>
<p>Как и любой объект io.Reader, мы можем передать net.Conn в функцию io.Copy и считать полученные по сети данные, например, на консоль: <code>io.Copy(os.Stdout, conn) </code>.</p>
<img src="pics/9.1.png" alt="net.Dial и TCP, отправка запроса в Golang" />
<p>Стоит отметить, что в примере выше осуществляет запрос к сетевому ресурсу сети интернет по протоколу TCP. Однако для этой же цели куда более удобнее использовать возможности 
пакета net/http, который предназначен специально для протокола HTTP, который работает поверх TCP.</p>
...
<div class="nav"><p><a href="8.9.php">Назад</a><a href="./">Содержание</a><a href="9.2.php">Вперед</a></p></div></div>
<div class="socBlock">
