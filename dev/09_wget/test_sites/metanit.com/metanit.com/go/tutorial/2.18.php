...
<html >
<head>
<title>Go | defer и panic</title>
<meta charset="utf-8" />
<meta name="description" content="Функции defer и panic в языке программирования Go, отложенное выполнение функций, генерация ошибок">
...
<h2>defer и panic</h2><div class="date">Последнее обновление: 21.01.2017</div>
...
<h3>defer</h3>
<p>Оператор <span class="b">defer</span> позволяет выполнить определенную функцию в конце программы, при этом не важно, где в 
реальности вызывается эта функция. Например:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Здесь функция finish вызывается с оператором defer, поэтому данная функция в реальности будет вызываться в самом конце выполнения программы, несмотря на то, что 
ее вызов определен в начале функции main. В частности, мы получим следующий консольный вывод:</p>
<div class="console">
...
<p>Если несколько функций вызываются с оператором defer, то те функции, которые вызываются раньше, будут выполняться позже всех. Например:</p>
<pre class="brush:go;">
package main
import  "fmt"
...
<h3>panic</h3>
<p>Оператор <span class="b">panic</span> позволяет сгенерировать ошибку и выйти из программы:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Оператору panic мы можем передать любое сообщение, которое будет выводиться на консоль. Например, в данном случае в функции divide, если второй параметр равен 0, то 
осуществляется вызов <code>panic("Division by zero!")</code>.</p>
<p>В функции main в вызове <code>fmt.Println(divide(4, 0))</code> будет выполняться оператор panic, поскольку второй параметр функции divide равен 0. И в этом случае все последующие операции, 
которые идут после этого вызова, например, в данном случае это вызов <code>fmt.Println("Program has been finished")</code>, не будут выполняться. В этом случае 
мы получим следующий консольный вывод:</p>
<div class="console">
<pre class="consoletext">
3
panic: Division by zero!
...
<p>И в конце вывода будет идти диагностическая информация о том, где возникла ошибка.</p>
...
<div class="nav"><p><a href="2.17.php">Назад</a><a href="./">Содержание</a><a href="2.13.php">Вперед</a></p></div></div>
<div class="socBlock">
