...
<html >
<head>
<title>Go | Горутины</title>
<meta charset="utf-8" />
<meta name="description" content="Параллельное программирование и горутины (goroutine) в языке программирования Go, определение и вызов горутин">
...
<h1>Параллельное программирование. Горутины</h1><h2>Горутины</h2><div class="date">Последнее обновление: 12.01.2018</div>
...
<p>Горутины (goroutines) представляют параллельные операции, которые могут выполняться независимо от функции, в которой они запущены. Главная особенность горутин 
...
ядрах процессора, тем самым горутины будут выполняться паралелльно, и программа завершится быстрее.</p>
<p>Каждая горутина, как правило, представляет вызов функции, и последовательно выполняет все свои инструкции. Когда мы запускаем программу на Go, мы уже 
работаем как минимум с одной горутиной, которая представлена функцией main. Эта функция последовательно выполняет все инструкции, которые определены внутри нее.</p>
<p>Для определения горутин применяется оператор <span class="b">go</span>, который ставится перед вызовом функции:</p>
<pre class="brush:go;">go вызов_функции</pre>
<p>Например, определим несколько горутин, вычисляющих факториал числа:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>В цикле последовательно запускаются шесть горутин с помощью вызова <code>go factorial(i)</code>. То есть фактически это обычный вызов функции с оператором go.</p>
<p>Однако вместо шести факториалов на консоли при вызове программы мы можем увидеть только строку "The End":</p>
<div class="console">
<pre class="consoletext">
The End
</pre>
</div>
<p>"Можем увидеть" означает, что поведение программы в данном случае недетерминировано. Например, вывод может быть и таким:</p>
<div class="console">
<pre class="consoletext">
2 - 2
1 - 1
...
<p>Почему так происходит? После вызова <code>go factorial(i)</code> функция main запускает горутину, которая начинает выполняться в своем контексте, независимо от функции main. То есть фактически main и factorial 
...
main, и соответственно мы сможем увидеть на консоли их результат. Но может сложиться ситуация, что функция main выполнится раньше вызовов функции factorial. Поэтому мы можем не увидеть на консоли факториалы чисел.</p>
<p>Чтобы все таки увидеть результат выполнения горутин, поставим в конце функции main вызов функции <code>fmt.Scanln()</code>, которая ожидает ввода пользователя с консоли:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Теперь мы сможем увидеть результаты всех вызовов функции factorial:</p>
<div class="console">
<pre class="consoletext">
1 - 1
3 - 6
...
<p>Стоит отметить, что так как каждая горутина запускается в своем собственном контексте и выполняется независимо и паралелльно по сравнению с другими горутинами, 
то в данном случае нельзя четко детерминировать, какая из горутин завершится раньше. Например, горутина <code>go factorial(2)</code> запускается 
до <code>go factorial(5)</code>, однако может завершиться после.</p>
<p>Горутины также могут представлять вызовы анонимных функций:</p>
<pre class="brush:go;">
package main
import "fmt"
