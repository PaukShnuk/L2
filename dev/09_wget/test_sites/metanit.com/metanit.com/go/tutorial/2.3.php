...
<html >
<head>
<title>Go | Типы данных</title>
<meta charset="utf-8" />
<meta name="description" content="Типы данных в языке программирования Go, строки, числа, логический тип, bool, int, string, byte, rune, неявная типизация переменной">
...
<h2>Типы данных</h2><div class="date">Последнее обновление: 20.12.2017</div>
...
<p>Все данные, которые хранятся в памяти, по сути представляют просто набор битов. И именно тип данных определяет, как будут интерпретироваться эти данные 
и какие операции с ними можно произодить. Язык Go является статически типизированным языком, то есть все используемые в программе данные имеют определенный тип.</p>
<p>Go имеет ряд встроенных типов данных, а также позволяет определять свои типы. Рассмотрим базовые встроенные типы данных, 
которые мы можем использовать.</p>
<h3>Целочисленные типы</h3>
<p>Ряд типов представляют целые числа:</p>
<ul>
<li><p><span class="b">int8</span>: представляет целое число от -128 до 127 и занимает в памяти 1 байт (8 бит)</p></li>
<li><p><span class="b">int16</span>: представляет целое число от -32768 до 32767 и занимает в памяти 2 байта (16 бит)</p></li>
<li><p><span class="b">int32</span>: представляет целое число от -2147483648 до 2147483647 и занимает 4 байта (32 бита)</p>
<li><p><span class="b">int64</span>: представляет целое число от –9 223 372 036 854 775 808 до 9 223 372 036 854 775 807 и занимает 8 байт (64 бита)</p></li>

<li><p><span class="b">uint8</span>: представляет целое число от 0 до 255 и занимает 1 байт</p></li>
<li><p><span class="b">uint16</span>: представляет целое число от 0 до 65535 и занимает 2 байта</p></li>
<li><p><span class="b">uint32</span>: представляет целое число от 0 до 4294967295 и занимает 4 байта</p></li>
<li><p><span class="b">uint64</span>: представляет целое число от 0 до 18 446 744 073 709 551 615 и занимает 8 байт</p></li>


<li><p><span class="b">byte</span>: синоним типа <code>uint8</code>, представляет целое число от 0 до 255 и занимает 1 байт</p></li>
<li><p><span class="b">rune</span>: синоним типа <code>int32</code>, представляет целое число от -2147483648 до 2147483647 и занимает 4 байта</p></li>

<li><p><span class="b">int</span>: представляет целое число со знаком, которое в зависимости о платформы может занимать либо 4 байта, либо 8 байт. То есть соответствовать либо 
int32, либо int64.</p></li>
<li><p><span class="b">uint</span>: представляет целое беззнаковое число только без знака, которое, аналогично типу int, в зависимости о платформы может занимать либо 4 байта, либо 8 байт. То есть соответствовать либо 
uint32, либо uint64.</p></li>
</ul>
<p>Здесь несложно запомнить, что есть типы со знаком (то есть которые могут быть отрицательными) и есть безнаковые положительные типы, которые начинаются на префикс u (uint32). Ну и также есть byte - синоним для uint8 и rune - синоним для int32.</p>
<p>Стоит отметить типы int и uint. Они имеют наиболее эффективный размер для определенной платформы (32 или 64 бита). Это наиболее используемый тип для представления целых чисел в программе. Причем различные 
компиляторы могут предоставлять различный размер для этих типов даже для одной и той же платформы.</p>
<p>Примеры определения переменных, которые представляют целочисленные типы:</p>
<pre class="brush:go;">
var a int8 = -1
var b uint8 = 2
...
<p>Для представления дробных чисел есть два типа:</p>
<ul>
<li><p><span class="b">float32</span>: представляет число с плавающей точкой от 1.4*10<sup>-45</sup> до 3.4*10<sup>38</sup>(для положительных). 
Занимает в памяти 4 байта (32 бита)</p></li>
<li><p><span class="b">float64</span>: представляет число с плавающей точкой от 4.9*10<sup>-324</sup> до 1.8*10<sup>308</sup> (для положительных) и 
занимает 8 байт.</p></li>
</ul>
<p>Тип float32 обеспечивает шесть десятичных цифр точности, в то время как точность, обеспечиваемая типом float64, составляет около 15 цифр</p>
<p>Примеры использования типов float32 и float64:</p>
<pre class="brush:go;">
var f float32 = 18
var g float32 = 4.5
var d float64 = 0.23
...
<p>В качестве разделителя между целой и дробной частью применяется точка.</p>
<h3>Комплексные числа</h3>
<p>Существуют отдельные типы для представления комплексных чисел:</p>
<ul>
<li><p><span class="b">complex64</span>: комплексное число, где вещественная и мнимая части представляют числа float32</p></li>
<li><p><span class="b">complex128</span>: комплексное число, где вещественная и мнимая части представляют числа float64</p></li>
</ul>
...
<h3>Тип bool</h3>
<p>Логический тип или тип <span class="b">bool</span> может иметь одно из двух значений: <span class="b">true</span> (истина) или 
<span class="b">false</span> (ложь).</p>
<pre class="brush:go;">
var isAlive bool = true
var isEnabled bool = false
</pre>
<h3>Строки</h3>
<p>Строки представлены типом <span class="b">string</span>. В Go строке соответствует строковый литерал - последовательность символов, заключенная в 
двойные кавычки:</p>
<pre class="brush:go;">var name string = "Том Сойер"</pre>
<p>Кроме обычных символов строка может содержать специальные последовательности (управляющие последовательности), которые начинаются с обратного слеша \. Наиболее 
распространенные последовательности:</p>
<ul>
<li><p><span class="b">\n</span>: переход на новую строку</p></li>
<li><p><span class="b">\r</span>: возврат каретки</p></li>
<li><p><span class="b">\t</span>: табуляция</p></li>
<li><p><span class="b">\"</span>: двойная кавычка внутри строк</p></li>
<li><p><span class="b">\\</span>: обратный слеш</p></li>
</ul>
<h3>Значение по умолчанию</h3>
<p>Если переменной не присвоено значение, то она имеет значение по умолчанию, которое определено для ее типа. Для числовых типов это число 0, для логического типа - false, 
для строк - ""(пустая строка).</p>
<h3>Неявная типизация</h3>
<p>При определении переменной мы можем опускать тип в том случае, если мы явно инициализируем переменную каким-нибудь значением:</p>
<pre class="brush:go;">var name = "Tom"</pre>
<p>В этом случае компилятор на основании значения неявно выводит тип переменной. Если присваивается строка, то то соответственно переменная будет представлять тип string, 
если присваивается целое число, то переменная представляет тип int и т.д.</p>
<p>То же самое по сути происходит при кратком определении переменной, когда также явным образом не указывается тип данных:</p>
<pre class="brush:go;">name := "Tom"</pre>
<p>При этом стоит учитывать, что если мы не указываем у переменной тип, то ей обязательно надо присвоить некоторое начальное значение. Объявление 
переменной одновременно без указания типа данных и начального значения будет ошибкой:</p>
<pre class="brush:go;">var name	// ! Ошибка</pre>
<p>Надо либо указать тип данных (в этом случае переменная будет иметь значение по умолчанию):</p>
<pre class="brush:go;">var name string</pre>
<p>Либо указать начальное значение, на основании которого выводится тип данных:</p>
<pre class="brush:go;">var name = "Tom"</pre>
<p>Либо и то, и другое одновременно:</p>
<pre class="brush:go;">var name string = "Tom"</pre>
<p>Неявная типизация нескольких переменных:</p>
<pre class="brush:go;">
var (
		name = "Tom"
		age = 27
