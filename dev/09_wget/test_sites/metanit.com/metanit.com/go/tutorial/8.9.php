...
<html >
<head>
<title>Go | Буферизированный ввод-вывод</title>
<meta charset="utf-8" />
<meta name="description" content="Буферизированный ввод-вывод в языке программирования Go, пакет bufio и объекты Writer и Reader">
...
<h2>Буферизированный ввод-вывод</h2><div class="date">Последнее обновление: 16.01.2018</div>
...
<p>Большиство встроенных операций ввода-вывода не используют буфер. Это может иметь отрицательный эффект для производительности приложения. Для 
буферизации потоков чтения и записи в Go опредеелены ряд возможностей, которые сосредоточены в пакете <span class="b">bufio</span>.</p>
<h3>Запись через буфер</h3>
<p>Для записи в источник данных через буфер в пакете bufio определен тип <span class="b">Writer</span>. Чтобы записать данные, можно воспользоваться 
одним из его методов:</p>
<pre class="brush:go;">
...
<ul>
<li><p><span class="b">Write()</span>: записывает срез байтов</p></li>
<li><p><span class="b">WriteByte()</span>: записывает один байт</p></li>
<li><p><span class="b">WriteRune()</span>: записывает один объект типа rune</p></li>
<li><p><span class="b">WriteString()</span>: записывает строку</p></li>
</ul>
<p>При выполнении этих методов данные вначале накапливаются в буфере, а чтобы сбросить их в источник данных, необходимо вызвать метод <span class="b">Flush()</span>.
<p>Для создания потока вывода через буфер применяется функция <span class="b">bufio.NewWriter()</span>:</p>
...
<h3>Чтение через буфер</h3>
<p>Для чтения из источника данных через буфер в пакете bufio определен тип <span class="b">Reader</span>. Для чтения данных можно 
воспользоваться одним из его методов:</p>
<pre class="brush:go;">
...
<ul>
<li><p><span class="b">Read(p []byte)</span>: считывает срез байтов и возвращает количество прочитанных байтов</p></li>
<li><p><span class="b">ReadByte()</span>: считывает один байт</p></li>
<li><p><span class="b">ReadBytes(delim byte)</span>: считывает срез байтов из потока, пока не встретится байт delim</p></li>
<li><p><span class="b">ReadLine()</span>: считывает строку в виде среза байт</p></li>
<li><p><span class="b">ReadRune()</span>: считывает один объект типа rune</p></li>
<li><p><span class="b">ReadSlice(delim byte)</span>: считывает срез байтов из потока, пока не встретится байт delim</p></li>
<li><p><span class="b">ReadString(delim byte)</span>: считывает строку, пока не встретится байт delim</p></li>
</ul>
<p>Для создания потока ввода через буфер применяется функция <span class="b">bufio.NewReader()</span>:</p>
<pre class="brush:go;">func NewReader(rd io.Reader) *Reader</pre>
<p>Она принимает объект io.Reader - это может быть любой объект, с которого производится чтение: os.Stdin, файл и т.д. В качестве результата 
возвращается объект bufio.Reader:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>В данном случае идет считывания из ранее записанного файла. Для этого объект файла os.File передается в функцию bufio.NewReader, на основании которого создается 
...
<div class="nav"><p><a href="8.8.php">Назад</a><a href="./">Содержание</a><a href="9.1.php">Вперед</a></p></div></div>
<div class="socBlock">
