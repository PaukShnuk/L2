...
<html >
<head>
<title>Go | Форматируемый ввод</title>
<meta charset="utf-8" />
<meta name="description" content="Форматируемый ввод из файла в языке программирования Go, функции fmt.Fscan, fmt.Fscanf, fmt.Fscanln">
...
<h2>Форматируемый ввод</h2><div class="date">Последнее обновление: 15.01.2018</div>
...
<p>Пакет <span class="b">fmt</span> чтение из объекта, который реализует интерфейс <span class="b">io.Reader</span>. 
Для этого применяются следующие функции: <span class="b">Fscan()</span>, <span class="b">Fscanln()</span> и <span class="b">Fscanf()</span>.</p>
<h3>Функции Fscan и Fscanln</h3>
<p>Через параметры функций Fscan и Fscanln можно получить вводимые значения:</p>
<pre class="brush:go;">
func Fscan(r io.Reader, a ...interface{}) (n int, err error)
func Fscanln(r io.Reader, a ...interface{}) (n int, err error)
</pre>
<p>В качестве первого параметра передается объект io.Reader, из которого надо считывать данные, а второй параметр представляет объекты, в которые считываются данные. 
В качестве результата обе функции возвращают количество считанных байт и информацию об ошибке. Например:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<h3>Fscanf</h3>
<p>Функция <span class="b">fmt.Fscanf()</span> считывает данные с применением форматирования:</p>
<pre class="brush:go;">func Fscanf(r io.Reader, format string, a ...interface{}) (n int, err error)</pre>
<p>Первый параметр функции представляет объект io.Reader. Второй параметр - строка форматирования, которая содержит спецификаторы и определяет последовательность 
считывания данных. Третий параметр - набор объектов, в которые надо считать данные. Например:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>Здесь вначале данные структуры person записываются в файл, а затем считываются из него в три переменных. При записи данных файл мы знаем его структуру. 
...
передаются их адреса:</p>
<pre class="brush:go;">fmt.Fscanf(file, "%s %d %f\n", &name, &age, &weight)</pre>
<p>При определении строки форматирования и передаче объктов для считывания действуют те же правила, что и при записи с помощью fmt.Fprintf. Так, первый спецификатор 
связан с первым объектом, второй спецификатор - со вторым объектом и так далее. И также спецификаторы должны соответствовать объетам по типу.</p>
<p>В итоге при выполнении этой программы на консоль будет выведено:</p>
<div class="console"><pre class="consoletext">Tom	24	68.50</pre></div>
<p>При этом объекты, в которые производится считывание, необязательно должны представлять переменные примитивных типов. Например, это может быть и структура:</p>
<pre class="brush:go;">
func readData(filename string){
...
<p>Сначала функция writeData записывает в файл набор объектов person. А затем в функции readData из файла считываются данные в бесконечном цикле. Когда 
файл закончится, функция Fscanf возвратит ошибку io.EOF.</p>
