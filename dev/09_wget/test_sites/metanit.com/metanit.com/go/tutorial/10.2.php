...
<meta charset="utf-8" />
<meta name="description" content="Работа с базой данных MySQL в языке программирования Go, операции с данными">
...
<h2>MySQL</h2><div class="date">Последнее обновление: 31.01.2018</div>
...
<p>Для работы с MySQL будем использовать драйвер <a href="https://github.com/go-sql-driver/mysql/" rel="nofollow">Go MySQL Driver</a>. 
Прежде всего нам надо добавить данный драйвер к переменной $GOPATH. Для этого выполним в командной строке/терминале следующую команду:</p>
<pre class="brush:go;">go get github.com/go-sql-driver/mysql</pre>
<p>Вначале создадим на сервере MySQL базу данных productdb и в ней таблицу products. Для этого можно использовать следующие выражений SQL</p>
<pre class="brush:sql;">
...
<p>Созданная база данных пуста, поэтому добавим в нее какие-нибудь данные:</p>
<pre class="brush:go;">
package main
import (
	"database/sql"
...
<p>Вначале подключаем все нужные нам пакеты. Для работы с реляционной базой данных необходим пакет "database/sql". И так как мы используем mysql, 
...
пакета инициализировать его с помощью вызова функции init.</p>
<p>Далее открываем подключение функцией Open:</p>
<pre class="brush:go;">sql.Open("mysql", "root:password@/productdb")</pre>
<p>Первый аргумент функции - название драйвера, в данном случае это "mysql". Второй параметр имеет форму "логин:пароль@/база_данных". Логин и пароль должны быть те, 
которые были указаны для mysql при его установке. В моем случае логин - root, а пароль - password. Название базы данных -productdb - та, которая была создана выше.</p>
<p>Эта функция возвращает объект базы данных - DB. И для добавления данных у этого объекта вызывается метод <span class="b">Exec()</span>:</p>
<pre class="brush:go;">
result, err := db.Exec("insert into productdb.Products (model, company, price) values (?, ?, ?)", 
		"iPhone X", "Apple", 72000)
</pre>
<p>Первый аргумент функции - это sql-выражение, которое добавляет строку в таблицу Products. Знаки вопроса в этом выражении представляют плейсхолдеры, 
...
параметра. Все дополнительные параметры передаются в sql-выражение на место плейсхолдеров по позиции - первый параметр вместо первого плейсхолдера и так далее.</p>
<p>Результат выполнения функции попадает в переменную result, которая хранит результат выполнения операции в базе данных. В частности, через метод 
<span class="b">result.LastInsertId()</span> мы можем получить id последнего добавленного объекта, а с помощью метода  
<span class="b">result.RowsAffected()</span> - количество добавленных строк.</p>
<p>И при выполнении данного скрипта получим следующие результаты:</p>
<img src="pics/11.1.png" alt="Добавление в базу данных MySQL в языке Go" />
<p>При этом необязательно определять все добавляемые данные через параметры, можно ввести их напрямую в sql-выражение:</p>
<pre class="brush:go;">
result, err := db.Exec("insert into productdb.Products (model, company, price) values ('Pixel 2', 'Google', 64000)")
</pre>
<h3>Получение данных</h3>
<p>Теперь получим ранее добавленные данные:</p>
<pre class="brush:go;">
package main
import (
	"database/sql"
...
<img src="pics/11.2.png" alt="Получение данных из базы данных MySQL в языке Go" />
