...
<html >
<head>
<title>Go | Переменные</title>
<meta charset="utf-8" />
<meta name="description" content="Переменные в языке программирования Go, определение переменной, правила именования, тип данных и инициализация переменных">
...
<h2>Переменные</h2><div class="date">Последнее обновление: 20.12.2017</div>
...
<p>Для хранения данных в программе применяются переменные. Переменная представляет именованный участок в памяти, который может хранить некотоое значение. 
Для определения переменной применяется ключевое слово <span class="b">var</span>, после которого идет имя переменной, а затем указывается ее тип:</p>
<pre class="brush:go;">var имя_переменной тип_данных</pre>
<p>Имя переменной представляет произвольный идентификатор, который состоит из алфавитных и цифровых символов и символа подчеркивания. При этом первым символом 
...
range, return, select, struct, switch, type, var.</p>
<p>Например, простейшее определение переменной:</p>
<pre class="brush:go;">var hello string</pre>
<p>Данная переменная называется hello и она представляет тип <span class="b">string</span>, то есть некоторую строку.</p>
<p>Можно одновременно объявить сразу несколько переменных через запятую:</p>
<pre class="brush:go;">var a, b, c string</pre>
<p>В данном случае определены переменные a, b, c, которые имеют тип <span class="b">string</span>. 
В этом случае опять же в конце указывается тип данных, и все переменные принадлежат этому типу.</p>
<p>После определения переменной ей можно присвоить какое-либо значение, которое соответствует ее типу:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Поскольку переменная hello представляет тип string, ей можно присвоить строку. В данном случае переменная hello хранит строку "Hello world". С помощью 
функции Println значение этой переменной выводится на консоль.</p>
<p>Также важно учитывать, что Go - регистрозависимый язык, то есть переменные с именами "hello" и "Hello" будут представлять разные переменные:</p>
<pre class="brush:go;">
var hello string
hello = "Hello world"
fmt.Println(Hello)	// ! Ошибка переменной Hello нет, есть переменная hello
</pre>
<p>Также можно сразу при объявлении переменной присвоить ей начальное значение. Такой прием называется инициализацией:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Если мы хотим сразу определить несколько переменных и присвоить им начальные значения, то можно обернуть их в скобки:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Отличительной особенностью переменных является то, что их значение можно многократно изменять:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<h3>Краткое определение переменной</h3>
<p>Также допустимо краткое определение переменной в формате:</p>
<pre class="brush:go;">имя_переменной := значение</pre>
<p>После имени переменной ставится двоеточие и равно и затем указывается ее значение.</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>В этом случае тип данных явным образом не указывается, он выводится автоматически из присваиваемого значения.</p>
