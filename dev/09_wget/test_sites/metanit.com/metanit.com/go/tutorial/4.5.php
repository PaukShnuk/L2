...
<html >
<head>
<title>Go | Методы указателей</title>
<meta charset="utf-8" />
<meta name="description" content="Определение методов для указателей на структуры в языке программирования Go">
...
<h2>Методы указателей</h2><div class="date">Последнее обновление: 26.12.2017</div>
...
<p>При вызове метода, объект структуры, для которого определен метод, передается в него по значению. Что это значит? Рассмотрим следующий пример:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Для структуры person определен метод updateAge, который принимает параметр newAge и изменяет значение поля age у структуры. То есть при вызове 
этого метода мы ожидаем, что возраст человека изменится. Однако консольный вывод нам показывает, что значение поля age не изменяется:</p>
<div class="console">
...
<p>Потому что при вызове <code>tom.updateAge(33)</code> метод updateAge получает копию структуры tom. То есть структура tom копируется в другой участок памяти, 
и далее метод updateAge работает с копией, никак не затрагивая оригинальную структуру tom.</p>
<p>Однако такое поведение может быть нежелальтельным. Что если мы всетаки хотим таким образом изменять состояние структуры? В этом случае необходимо использовать указатели на структуры:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Теперь метод updateAge принимает указатель на структуру person: <code>p *person</code>, то есть фактически адрес структуры в памяти. С помощью операции разыменования получаем значение 
по этому адресу в памяти и меняем поле age:</p>
<pre class="brush:go;">
(*p).age = newAge
</pre>
<p>В функции main определяем указатель на структуру person и передаем ему адрес структуры tom:</p>
<pre class="brush:go;">
var tomPointer *person = &tom
</pre>
<p>Затем вызываем метод updateAge:</p>
<pre class="brush:go;">tomPointer.updateAge(33)</pre>
<p>Таким образом, метод updateAge получит адрес, который хранится в tomPointer и по этому адресу обратиться к структуре tom, изменив значение ее свойства age.</p>
<div class="console">
<pre class="consoletext">
before 24
after 33
...
<p>Стоит отметить, что несмотря на то, что метод updateAge определен для указателя на структуру person, мы по прежнему можем вызывать этот метод и для объекта person:</p>
<pre class="brush:go;">
var tom = person { name: "Tom", age: 24 }
...
<div class="nav"><p><a href="4.4.php">Назад</a><a href="./">Содержание</a><a href="5.1.php">Вперед</a></p></div></div>
<div class="socBlock">
