...
<html >
<head>
<title>Go | Рекурсивные функции</title>
<meta charset="utf-8" />
<meta name="description" content="Рекурсивные функции в языке программирования Go, рекурсивная функция факториала">
...
<h2>Рекурсивные функции</h2><div class="date">Последнее обновление: 26.12.2017</div>
...
<p>Рекурсивная функция представляет такую функцию, которая вызывает саму себя. Рекурсивные функции представляют мощный инструмент для обработки рекурсивных структур данных, например, 
различных деревьев.</p>
<p>Например, определим функцию вычисления факториала числа, которая получает результат рекурсивным способом:</p>
<pre class="brush:go;">
package main
...
<p>Здесь функция factorial получает некоторое положительное число, для которого надо вычислить факториал. Полученный результат возвращается из функции. 
Вначале идет условие, что если число равно 0, то функция возвращает 1. Иначе функция возвращает произведение числа n на результат этой же функции для числа n-1.</p>
<p>При создании рекурсивной функции в ней обязательно должен быть некоторый базовый вариант, который использует оператор return и помещается в 
начале функции. В случае с факториалом это <code>if x == 0 {return 1}</code>.</p>
<p>И, кроме того, все рекурсивные вызовы должны обращаться к подфункциям, которые в конце концов сходятся к базовому варианту. Так, при передаче в 
...
конце концов мы дойдем до ситуации, когда число будет равно 0, и будет использован базовый вариант.</p>
<p>Например, вызов <code>factorial(4)</code> фактически можно расписать следующим образом:</p>
<ul>
<li><p>factorial(4)</p></li>
<li><p>4 * factorial(3)</p></li>
<li><p>4 * 3 * factorial(2)</p></li>
<li><p>4 * 3 * 2 * factorial(1)</p></li>
<li><p>4 * 3 * 2 * 1 * factorial(0)</p></li>
<li><p>4 * 3 * 2 * 1 * 1</p></li>
</ul>
<p>Другим распространенным показательным примером рекурсивной функции служит функция, вычисляющая числа Фибоначчи. n-й член последовательности 
Фибоначчи определяется по формуле: f(n)=f(n-1) + f(n-2), причем f(0)=0, а f(1)=1.</p>
<pre class="brush:go;">
package main
