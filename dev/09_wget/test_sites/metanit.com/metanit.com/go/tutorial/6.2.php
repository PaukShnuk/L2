...
<html >
<head>
<title>Go | Соответствие интерфейсу</title>
<meta charset="utf-8" />
<meta name="description" content="Соответствие структур интерфейсу в языке программирования Go, правила реализации интерфейсов, вложенные интерфейсы">
...
<h2>Соответствие интерфейсу</h2><div class="date">Последнее обновление: 28.12.2017</div>
...
<p>Чтобы тип данных соответствовал интерфейсу, он должен реализовать все методы этого интерфейса. Например:</p>
<pre class="brush:go;">
package main
...
<p>Здесь определен интерфейс Stream, который условно представляет некоторый поток данных и определяет три метода: close, read и write. И также есть две 
...
соответствует интерфейсу Stream. Поэтому везде, где требуется объект Stream мы можем использовать объект типа <code>*File</code>, но никак не *Folder.</p>
<p>Например, в функцию <code>closeStream()</code>, которая условно закрывает поток, в качестве параметра передается объект Stream. При вызове в эту функцию 
можно передать объект типа *File, который соответствует интерфейсу Stream:</p>
<pre class="brush:go;">closeStream(myFile)</pre>
<p>А вот объект *Folder передать нельзя:</p>
<pre class="brush:go;">closeStream(myFolder)		// Ошибка: тип *Folder не реализует интерфейс Stream</pre>
<p>Но мы по-прежнему можем вызывать у объекта *Folder метод close, но он будет рассматриваться как собственный метод, который не имеет 
никакого отношения к интерфейсу Stream.</p> 
<p>Еще важно отметить, что в данном случае методы реализованы именно для типа <span class="b">*File</span>, то есть указателя на объект File, 
а не для типа <span class="b">File</span>. Это два разных типа. Поэтому тип File по-прежнему НЕ соответствует интерфейсу Stream. И мы, к примеру, 
не можем написать следующее:</p>
<pre class="brush:go;">
myFile2 := File{}
closeStream(myFile2)		// ! Ошибка: тип File не соответствует интерфейсу Stream
</pre>
<h3>Реализация нескольких интерфейсов</h3>
<p>При этом тип данных необязательно должен реализовать только методы интерфейса, для типа данных можно определить его собственные методы или также 
реализовать методы других интерфейсов. Например:</p>
<pre class="brush:go;">
package main
...
<p>В данном случае для типа *File реализованы методы обоих интерфейсов - Reader и Writer. Соответственно мы можем использовать объекты типа *File в качестве 
объектов Reader и Writer.</p>
<h3>Вложенные интерфейсы</h3>
<p>Одни интерфейсы могут содержать другие интерфейсы.</p>
<pre class="brush:go;">
type Reader interface{
	read()
}
...
<p>В этом случае для соответствия подобному интерфейсу типы данных должны реализовать все его вложенные интерфейсы. 
Например:</p>
<pre class="brush:go;">
package main
...
<p>В данном случае определено три интерфейса. Причем интерфейс ReaderWriter содержит интерфейсы Reader и Writer. Чтобы структура File соответствовала интерфейсу 
ReaderWriter, она должна реализовать методы read и write, то есть методы обоих вложенных интерфейсов, что в принципе здесь и сделано.</p>
