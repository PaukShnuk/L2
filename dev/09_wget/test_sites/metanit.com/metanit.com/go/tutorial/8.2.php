...
<html >
<head>
<title>Go | Создание и открытие файлов</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и открытие файлов в языке программирования Go, функции os.Open, os.Create, os.OpenFile и os.Close">
...
<h2>Создание и открытие файлов</h2><div class="date">Последнее обновление: 15.01.2018</div>
...
<p>Для работы с файлами мы можем использовать функциональность пакета <a href="https://golang.org/pkg/os/" rel="nofollow">os</a>. 
Все файлы в Go представлены типом <span class="b">os.File</span>. Этот тип реализует ряд интерфейсов, например, io.Reader и io.Writer, 
которые позволяют читать содержимое файла и сохранять данные в файл.</p>
<p>С помощью функции <span class="b">os.Create()</span> можно создать файл по определенному пути. Путь к файлу передается в качестве параметра. 
Если подобный файл уже существует, то он перезаписывается:</p>
<pre class="brush:go;">file, err := os.Create("hello.txt")</pre>
<p>Функция возвращает объект os.File для работы с файлом и информацию об ошибке, которая может возникнуть при создании файла.</p>
<p>Ранее созданный файл можно открыть с помощью функции <span class="b">os.Open()</span>:</p>
<pre class="brush:go;">
file, err := os.Open("hello.txt")
</pre>
<p>Эта функция также возвращает объект os.File для работы с файлом и информацию об ошибке, которая может возникнуть при открытии файла.</p>
<p>Также в нашем распоряжении есть функция <span class="b">os.OpenFile()</span>, которая открывает файл, а если файла нет, то создает его. Она принимает три параметра:</p>
<ul>
<li><p>путь к файлу</p></li>
<li><p>редим открытия файла (для чтения, для записи и т.д.)</p></li>
<li><p>разрешения для доступа к файлу</p></li>
</ul>
<p>Например:</p>
<pre class="brush:go;">
...
<p>После окончания работы с файлом его следует закрыть с помощью метода <span class="b">Close()</code>.</p>
<pre class="brush:go;">
package main
import (
"fmt"
...
<p>С помощью функции <span class="b">os.Exit()</span> можно выйти из программы. А метод <code>Name()</code>, определенный для типа os.File, позволяет получить имя файла.</p>
