...
<meta charset="utf-8" />
<meta name="description" content="Отправка запросов по HTTP с помощью http.Client в языке программирования Go">
...
<h2>http.Client</h2><div class="date">Последнее обновление: 20.01.2018</div>
...
<p>Для осуществления HTTP-запросов также может применяться структура <span class="b">http.Client</span>. Чтобы отправить запрос к веб-ресурсу, можно использовать 
один из ее методов:</p>
<pre class="brush:go;">
...
<p>Во многом они аналогичны тем одноименным функциям (за исключением метода Do), которые определены в пакете net/http и которые были рассмотрены в прошлой теме. Например, 
выполнение самого простого запроса GET:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>Структура http.Client имеет ряд полей, которые позволяют настроить ее поведение:</p>
<ul>
<li><p><span class="b">Timeout</span>: устанавливает таймаут для запроса</p></li>
<li><p><span class="b">Jar</span>: устанавливает куки, отправляемые в запросе</p></li>
<li><p><span class="b">Transport</span>: определяет механиз выполнения запроса</p></li>
</ul>
<p>Установка таймаута:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>Свойство Timeout представляет объект <code>time.Duration</code>, и в данном случае оно равно 6 секундам.</p>
<h3>Request</h3>
<p>Для управления запросом и его параметрами в Go используется объект <span class="b">http.Request</span>. Он позволяет установить различные настройки, 
добавить куки, заголовки, определить тело запроса. Для создания объекта http.Request применяется функция <span class="b">http.NewRequest()</span>:</p>
<pre class="brush:go;">func NewRequest(method, url string, body io.Reader) (*Request, error)</pre>
<p>Функция принимает три параметра. Первый параметр - тип запроса в виде строки ("GET", "POST"). Второй параметр - адрес ресурса. Третий параметр - тело запроса.</p>
<p>Для отправки объекта Request можно применять метод <span class="b">Do()</span>:</p>
