...
<html >
<head>
<title>Go | Закрытие канала</title>
<meta charset="utf-8" />
<meta name="description" content="Закрытие канала в языке программирования Go, правила взаимодействия с закрытым каналом, получение состояния канала">
...
<h2>Закрытие канала</h2><div class="date">Последнее обновление: 12.01.2018</div>
...
<p>После инициализации канал готов передавать данные. Он находится в открытом состоянии, и мы можем с ним взаимодействовать, пока не будет закрыт с помощью встроенной функции 
<span class="b">close()</span>:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>После закрытия канала мы не сможем послать в него новые данные. Если мы попробуем это сделать, то получим ошибку. Однако мы можем получить ранее добавленные данные. 
...
третье значение, которого нет в канале, мы получим значение по умолчанию - число 0.</p>
<p>Чтобы не столкнуться с проблемой, когда канал уже закрыт, мы можем проверять состояние канала. В частности, из канала мы можем получить два значения:</p>
<pre class="brush:go;">val, opened:= &lt;-intCh</pre>
<p>Первое значение - val - это собственно данные из канала, а opened представляет логическое значение, которое равно true, если канал открыт и мы можем успешно считать из него данные. Например, 
мы можем проверять с помощью условной конструкции состояние канала:</p>
<pre class="brush:go;">
package main
import "fmt"
