...
<html >
<head>
<title>Go | Синхронизация</title>
<meta charset="utf-8" />
<meta name="description" content="Синхронизация горутин через каналы в языке программирования Go, закрытие канала и передача данных горутине">
...
<h2>Синхронизация</h2><div class="date">Последнее обновление: 12.01.2018</div>
...
<p>Использование каналов открывает нам возможности по синхронизации между различными горутинами. Например, одна горутина производит некоторое действие, результат которой 
...
<p>Канал не обязательно должен нести данные, которые представляют некоторый результат, от которого зависит дальнейшее выполнение горутины. Иногда это может быть 
холостой объект, например, пустая структура, которая необходима только для синхронизации горутин:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>В данном случае функция factorial по-прежнему вычисляет факториал, но помещает все факториалы чисел от 1 до n в отображение results, где 
ключи представляют числа, а значения - факториалы чисел.</p>
<p>Канал, через который горутины взаимодействуют, представляет тип <code>chan struct{}</code>. Причем функция factorial не отправляе конкретные данные в канал, а просто закрывает его после 
выполнения всех своих инструкций с помощью вызова <code>defer close(ch)</code>. После закрытия канала в функции main получае соответствующий сигнал в строке <code>&lt;-structCh</code>, и 
после этого функция main продолжает свою работу.</p>
