...
<html >
<head>
<title>Go | Вывод на консоль</title>
<meta charset="utf-8" />
<meta name="description" content="Вывод на консоль с помощью объекта os.Stdout в языке программирования Go, функции fmt.Println, fmt.Print, fmt.Printf">
...
<h2>Вывод на консоль</h2><div class="date">Последнее обновление: 15.01.2018</div>
...
<p>Стандартным потоком вывода в Go является объект <span class="b">os.Stdout</span>, который фактически представляет консоль. Например, мы могли бы 
вывести в этот поток данные следующим образом:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>Здесь используется рассмотренная в прошлой теме функция <code>Fprintln()</code>, которая выводит в поток вывода набор значений. То есть фактически 
в данном случае идет запись или вывод на консоль.</p>
<p>Однако поскольку запись в стандартный поток os.Stdout - довольно распространенная задача, то вместо функций <code>Fprint/Fprintln/Fprintf</code> применяются их 
двойники: <span class="b">Println()</span>, <span class="b">Print()</span> и <span class="b">Printf()</span> соответственно, которые по умолчанию 
выводят данные в os.Stdout:</p>
<pre class="brush:go;">
package main
import "fmt"
