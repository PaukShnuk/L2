...
<html >
<head>
<title>Go | Пакеты</title>
<meta charset="utf-8" />
<meta name="description" content="Пакеты в языке программирования Go, разбиение кода на модули, импорт пакетов, встроенные пакеты.">
...
<h1>Пакеты и модули</h1><h2>Пакеты и их импорт</h2><div class="date">Последнее обновление: 24.12.2017</div>
...
<p>Весь код в языке Go организуется в пакеты. Пакеты представляют удобную организацию разделения кода на отдельные части или модули. Модульность позволяет 
определять один раз пакет с нужной функциональностью и потом использовать его многкратно в различных программах.</p>
<p>Код пакета располагается в одном или нескольких файлах с расширением <span class="b">go</span>. Для определения пакета применяется ключевое слово <span class="b">package</span>. Например:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>В данном случае пакет называется <span class="b">main</span>. Определение пакета должно идти в начале файла.</p>
<p>Есть два типа пакетов: исполняемые (executable) и библиотеки (reusable). Для создания исполняемых файлов пакет должен иметь имя <span class="b">main</span>. Все остальные пакеты 
не являются исполняемыми. При этом пакет main должен содержать функцию <span class="b">main</span>, которая является входной точкой в приложение.</p>
<h3>Импорт пакетов</h3>
<p>Если уже есть готовые пакеты с нужной нам функциональностью, которую мы хотим использовать, то для их использования мы можем их импортировать в программу с 
помощью оператора <span class="b">import</span>. Например, в примере выше задействуется функциональность вывода сообщения на консоль с помощью функции Println, 
которая определена в пакете <span class="b">fmt</span>. Соответственно чтобы использовать эту функцию, необходимо импортировать пакет fmt:</p>
<pre class="brush:go;">import "fmt"</pre>
<p>Нередко программы подключают сразу несколько внешних пакетов. В этом случае можно последовательно импортировать каждый пакет:</p>
<pre class="brush:go;">
package main
import "fmt"
import "math"
...
<p>В данном случае подключается встроенный пакет math, который содержит функцию <code>Sqrt()</code>, возвращающую квадратный корень числа.</p>
<p>Либо чтобы сократить определение импорта пакетов можно заключить все пакеты в скобки:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>Подобным образом мы можем импортирвать как встроенные пакеты, так и свои собственные. Полный список встроенных пакетов в Go можно найти по адресу 
<a href="https://golang.org/pkg/" rel="nofollow">https://golang.org/pkg/</a>.</p>
<h3>Пакет из нескольких файлов</h3>
<p>Один пакет может состоять из нескольких файлов. Например, определим в папке два файла:</p>
<img src="pics/2.1.png" alt="Multiple files in Go build" />
<p>В файле <span class="b">factorial.go</span> определим функцию для подсчета факториала:</p>
<pre class="brush:go;">
package main
...
<p>Данный файл принадлежит пакету main.</p>
<p>В файле <span class="b">main.go</span> используем функцию для вычисления факториала:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Данный файл также принадлежит пакету main. Файлов может и быть и больше. Теперь скомпилируем из этих файлов программу. Для этого перейдем в 
консоли к папке проекта и выполним команду:</p>
<div class="console"><pre class="consoletext">go build -o main.exe factorial.go main.go</pre></div>
<p>Флаг <span class="b">-o</span> указывает, как будет называться выходной файл - в данном случае main.exe. Затем идут все компилируемые файлы. После выполнения этой 
команды будет создан файл main.exe, который мы сможем запустить в консоли:</p>
<img src="pics/2.2.png" alt="Компиляция файлов Go" /> 
<h3>Файлы в разных пакетах</h3>
<p>Теперь рассмотрим другую ситуацию, когда файлы нашей программы разделены по разным пакетам. Определим в папке проекта каталог <span class="b">computation</span>. 
Затем в каталог computation добавим следующий файл factorial.go:</p>
<pre class="brush:go;">
package computation
...
<p>Код файла factorial.go принадлежит пакету computation. Важно отметить, что название функции начинается с заглавной буквы. Благодаря этому данная 
функция будет видна в других пакетах.</p>
<img src="pics/2.4.png" alt="Импорт пакетов в Go" />
<p>И чтобы использовать функцию factorial, надо импортировать этот пакет в файле main.go:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>Путь "./computation" указывает, что пакет находится в папке computation.</p>
...
<img src="pics/2.3.png" alt="Компиляция проекта Go из разных пакетов" />
