...
<html >
<head>
<title>Go | Передача потоков данных</title>
<meta charset="utf-8" />
<meta name="description" content="Передача потоков данных между горутинами через каналы в языке программирования Go, горутины в цикле for">
...
<h2>Передача потоков данных</h2><div class="date">Последнее обновление: 12.01.2018</div>
...
<p>Нередко одна горутина транслирует другой горутине через канал не одиночные значения, а некоторый поток данных. В этом случае общий алгоритм состоит в том, что горутина-отправитель в течение 
некоторого периода отправляет данные. Когда данные для отправки закончились, работа сделала, отправитель закрывает канал.</p>
<p>Горутина-получатель в бесконечном цикле получает данные из канала. Если будет получен маркер закрытия канала, то осуществляется выход из бесконечного цикла.</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>В данном случае функция main и горутина factorial взаимодействуют через канал intCh. Функция factorial последовательно вычисляем факториалы чисел от 1 до n. 
И все вычисленные значения передаются в канал. По завершении функции factorial канал закрывается вызывом <code>defer close(ch)</code>.</p>
<p>В функции main в бесконечом цикле отправленные данные извлекаются из канала. При этом также проверяется, открыт ли канал. И ели вдруг канал закрыт и соответственно нет смысла получать 
из него данные, происходит выход из бесконечного цикла:</p>
<pre class="brush:go;">
for { 
	num, opened := &lt;- intCh 
	if !opened { 
...
<p>При извлечении значений из канала мы можем использовать ту же форму цикла for, которая применяется для перебора коллекций:</p>
<pre class="brush:go;">
...
<p>Когда канал будет закрыт, то автоматически произойдет выход из цикла for.</p>
