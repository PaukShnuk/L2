...
<html >
<head>
<title>Go | Указатели и функции</title>
<meta charset="utf-8" />
<meta name="description" content="Указатели и функции в языке программирования Go, передача параметров по ссылке, передача в функцию указателя и возвращение из функции указаелей">
...
<h2>Указатели и функции</h2><div class="date">Последнее обновление: 24.12.2017</div>
...
<h3>Указатели как параметры функции</h3>
<p>По умолчанию все параметры передаются в функцию по значению. Например:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Функция changeValue изменяет значение параметра, возводя его в квадрат. Но после вызова этой функции мы видим, что значение переменной d, которая передается в changeValue, не изменилось. 
Ведь функция получает копию данной переменной и работает с ней независимо от оригинальной переменной d. Поэтому d никак не изменяется.</p>
<p>Однако что, если нам все таки надо менять значение передаваемой переменной? И в этом случае мы можем использовать указатели:</p>
<pre class="brush:go;">
package main
...
<p>Теперь функция changeValue принимает в качестве параметра указатель на объект типа int. При вызове функции changeValue в нее передается адрес переменной 
d (<code>changeValue(&d)</code>). И после ее выполнения мы видим, что значение переменной d изменилось.</p>
<h3>Указатель как результат функции</h3>
...
<p>В данном случае функция createPointer возвращает указатель на объект int.</p>
