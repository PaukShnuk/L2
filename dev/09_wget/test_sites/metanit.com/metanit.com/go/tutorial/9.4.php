...
<html >
<head>
<title>Go | Установка таймаута при передачи по сети</title>
<meta charset="utf-8" />
<meta name="description" content="Установка таймаута при взаимодействии клиента и сервера в языке программирования Go, методы SetDeadline, SetReadDeadline, SetWriteDeadline">
...
<h2>Установка таймаута</h2><div class="date">Последнее обновление: 18.01.2018</div>
...
<p>При взаимодействии клиента и сервера мы можем устанавливать таймаут, по истечении которого соединение между сервером и клиентом при отсутствии взаимодействия будет разорвано. 
Для этого у типа <span class="b">net.Conn</span> определены следующие методы:</p>
<ul>
<li><p><span class="b">SetDeadline(t time.Time) error</span>: устанавливает таймаут на все операции ввода-вывода. Для установки времени применяется структура <span class="b">time.Time</span></p></li>
<li><p><span class="b">SetReadDeadline(t time.Time) error</span>: устанавливает таймаут на операции ввода в поток</p></li>
<li><p><span class="b">SetWriteDeadline(t time.Time) error</span>: устанавливает таймаут на операции вывода из потока</p></li>
</ul>
<p>В каком случае они могут пригодиться? В прошлой теме было рассмотрено взаимодействие сервера и клиента. Для чтения данных от клиента сервер использовал буфер фиксированного 
размера:</p>
<pre class="brush:go;">
input := make([]byte, (1024 * 4)) 
n, err := conn.Read(input)
</pre>
<p>Однако в ряде ситуаций это не лучший способ, особенно когда размер передаваемых данных превышает размер буфера. Мы можем точно не знать, сколько данных возвратит нам сервер. 
Поэтому определим следующий код клиента:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>Теперь получение данных выделено в отдельный цикл for:</p>
<pre class="brush:go;">
for{
	buff := make([]byte, 1024)
	n, err := conn.Read(buff)
...
<p>Поэтому даже если сервер передал больше 1024 байт, все они все равно будут обработаны. Но кроме того, здесь также устанавливается таймаут на чтение данных. 
Перед самим циклом устанавливается таймаут в 5 секунд:</p>
<pre class="brush:go;">conn.SetReadDeadline(time.Now().Add(time.Second * 5))</pre>
<p>Это значит, что клиент может ожидать данные на чтение от сервера в течении 5 секунд. По истечении этого времени операция чтения генерирует ошибку и соответственно 
...
700 милисекунд сервер не пришлет никаких данных, то происходит выход из цикла и соответственно чтение данных заканчивается.</p>
<p>Важно понимать роль подобных задержек, так как они позволяют сгенерировать ошибку при чтении данных. А значит мы можем получить эту ошибку и должным образом обработать ее, 
...
а клиент ожидал данных от сервера также в операции чтения. И была бы своего рода блокировка.</p>
<p>Код сервера остается тем же, что и в прошлой теме:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
