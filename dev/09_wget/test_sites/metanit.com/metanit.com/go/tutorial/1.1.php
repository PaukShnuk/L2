...
<html >
<head>
<title>Go | Введение в язык</title>
<meta charset="utf-8" />
<meta name="description" content="Основные особенности языка программирования Go, для чего он нужен, установка компилятора">
...
<h1>Введение в язык Go</h1><h2>Что такое Go</h2><div class="date">Последнее обновление: 15.03.2022</div>
...
<p>Go представляет компилируемый статически типизированный язык программирования от компании Google. Язык Go предназначен для 
создания различного рода приложений, но прежде всего это веб-сервисы и клиент-серверные приложения. Хотя также язык обладает возможностями по работе с графикой, низкоуровневыми возможностями и т.д.</p>
<p>Работа над языком Go началась в 2007 в недрах компании Google. Одним из авторов является Кен Томпсон, который, к слову, является и одним из 
авторов языка Си (наряду с Денисом Ритчи). 10 ноября 2009 года язык был анонсирован, а в марте 2012 года вышла версия 1.0. 
При этом язык продолжает развиваться. Текущей версией на момент написания данной статьи является версия 1.18, которая вышла в марте 2022 года.</p>
<p>Язык Go развивается как open source, то есть представляет поект с открытым исходным кодом, и все его коды и компилятор можно найти и использовать бесплатно. 
Официальный сайт проекта - <a href="https://go.dev/" rel="nofollow">https://go.dev/</a>, где можно много полезной информации о языке.</p>
<p>Go является кроссплатформенным, он позволяет создавать программы под различные операционные системы - Windows, Mac OS, Linux, FreeBSD. Код обладает переносимостью: 
программы, написанные для одной из этих операционных систем, могут быть легко с перекомпиляцией перенесены на другую ОС.</p>
<p>Основные особенности языка Go:</p>
<ul>
<li><p>компилируемый - компилятор транслирует программу на Go в машинный код, понятный для определенной платформы</p></li>
<li><p>статически типизированный</p></li>
<li><p>присутствует сборщик мусора, который автоматически очищает память</p></li>
<li><p>поддержка работы с сетевыми протоколами</p></li>
<li><p>поддержка многопоточности и параллельного программирования</p></li>
</ul>
<p>В настоящее время Go находит широкое применение в различных сферах. В частности, среди известных проектов, которые применяют Go, 
можно найти следующие: Google, Dropbox, Netflix, Kubernetes, Docker, Twitch, Uber, CloudFlare и ряд других.</p>
<p>Что нужно для работы с Go? Прежде всего необходим текстовый редактор для набора кода и компилятор для преобразования кода в исполняемый файл. 
...
Существуют плагины для Go для других IDE, в частности, IntelliJ IDEA и Netbeans.</p>
<h3>Установка компилятора</h3>
<p>Пакет для установки компилятора можно загрузить с официального сайта <a href="https://go.dev/dl/" rel="nofollow">https://go.dev/dl/</a>.</p>
<img src="./pics/1.1.png" alt="Установка компилятора для языка программирования Go" />
<p>По этому адресу пакеты установщиков для различных операционных систем. Так, при загрузки и запуске установщика для Windows открывается следующее окно:</p>
<img src="./pics/1.2.png" alt="Установка компилятора Go на Windows" />
<p>После принятия лицензионного соглашения отобразится окно для выбора места установки:</p>
<img src="./pics/1.3.png" alt="Путь к компилятору Go на Windows" />
<p>На Windows, например, по умолчанию используется путь "C:\Program Files\Go\". Оставим путь по умолчанию и перейдем к следующему окну, на котором нажмем на кнопку Install:</p>
<img src="./pics/1.4.png" alt="Установка компилятора Go на Windows" />
<p>После успешной установки в папке установки будут установлены все файлы, необходимые для работы с Go. В частности, в подкаталоге 
<span class="ii">bin</span> можно найти файл <span class="b">go</span> (go.exe на Windows), который выполняет роль компилятора:</p>
<img src="./pics/1.5.png" alt="Компилятор GoLang" />
<p>После установки мы можем проверить версию языка, запустив следующую команду в консоли/командной строке:</p>
<div class="console"><pre class="consoletext">go version</pre></div>
<img src="./pics/1.22.png" alt="Версия компилятора Go" />
