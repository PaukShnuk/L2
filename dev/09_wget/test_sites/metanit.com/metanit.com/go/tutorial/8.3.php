...
<html >
<head>
<title>Go | Чтение и запись файлов</title>
<meta charset="utf-8" />
<meta name="description" content="Чтение и запись файлов в языке программирования Go, функции WriteString, Write, Read и получение данных в поток байтов">
...
<h2>Чтение и запись файлов</h2><div class="date">Последнее обновление: 15.01.2018</div>
...
<h3>Запись в файл</h3>
<p>Для записи текстовой информации в файл можно применять метод <span class="b">WriteString()</span> объекта os.File, который заносит в файл строку:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>В данном случае создается файл hello.txt, в который записывается строка "Hello Gold!".</p>
<p>Для записи нетекстовой бинарной информации в виде набора байт применяется метод <span class="b">Write()</span> (реализация интерфейса io.Writer):</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>Поскольку тип <code>io.File</code> реализует интерфейс <code>io.Reader</code>, то для чтения из файла мы можем использовать метод <span class="b">Read()</span>. 
Этот метод позволяет получить содержимое файла в виде набора байт:</p>
<pre class="brush:go;">
package main
import (
	"fmt"
...
<p>Для считывания данных определяется срез из 64 байтов. В бесконечном цикле содержимое файла считывается в срез, а когда будет достигнут конец файла, 
...
то для вывода текста на консоль преобразуем срез байтов в строку: <code>string(data[:n])</code>.</p>
