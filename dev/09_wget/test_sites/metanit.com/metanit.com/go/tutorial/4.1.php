...
<html >
<head>
<title>Go | Объявление типов</title>
<meta charset="utf-8" />
<meta name="description" content="Объявление типов в языке программирования Go, оператор type, свои кастомные типы данных">
...
<h1>Производные типы</h1><h2>Объявление типов</h2><div class="date">Последнее обновление: 24.12.2017</div>
...
<p>Оператор <span class="b">type</span> позволяет определять именованный тип. Именнованный тип основывается на уже существующем типе. Например:</p>
<pre class="brush:go;">type mile int</pre>
<p>В данном случае определяется тип mile, который основывается на типе int. По сути mile представляет тип int и работа с ним будет производиться также, как и с типом int. 
Однако в то же время фактически это новый тип.</p>
<p>Мы можем определять переменные данного типа, работать с ними как с объектами базового типа int:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Но может возникнуть вопрос, а зачем это нужно, зачем создавать новый тип, если он все равно ведет себя как тип int? Рассмотрим следующую ситуацию:</p>
<pre class="brush:go;">
package main
import "fmt"
...
<p>Здесь определены два именованных типа: mile и kilometer, которые по сути представляют тип int и которые предназначены для выражения расстояния в милях 
и километрах соответственно. И также определена функция <code>distanceToEnemy()</code>, которая отображает расстояние в милях до условного противника. 
В качестве параметра принимает значение mile - именно значение типа mile, а не типа int. Это позволит нам уменьшить вероятность передачи некорректных данных. То есть 
передаваемые данные должны быть явным образом определены в программе как значение типа mile, а не типа int или типа kilometer. Таким образом, с помощью именнованных типов 
мы придаем типу некоторый дополнительный смысл.</p>
...
<p>Здесь определен именованный тип library, который по сути представляет срез из строк. Данный тип будет представлять своего рода библиотеку, 
которая включает книги в виде их строковых названий. С помощью функции printBooks можно вывести все книги из этой библиотеки. При этом функция 
printBooks работает именно с типом library, а не с любым срезом из строк.</p>
