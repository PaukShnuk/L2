...
<html >
<head>
<title>Go | Тип функции</title>
<meta charset="utf-8" />
<meta name="description" content="Тип функции в языке программирования Go, функция-значение, передача функции в качестве параметра и возвращение функций в качестве результата">
...
<h2>Тип функции</h2><div class="date">Последнее обновление: 26.12.2017</div>
...
<p>Каждая функция имеет определенный тип, который складывается из списка типов параметров и списка типов возвращаемых рехультатов. Например, возьмем следующую 
функцию:</p>
<pre class="brush:go;">
...
</p>Эта функция представляет тип <span class="b">func(int, int) int</span>. Этому же типу будет соответствовать следующая функция:</p>
<pre class="brush:go;">
func multiply(x int, y int) int{
	return x * y
}
</pre>
<p>Хотя эта функция называется иначе, выполняет другие действия, но по типу параметров и по типу возвращаемого результата она соответствует выше указанному типу функции.</p>
<p>Возьмем еще одну функцию:</p>
<pre class="brush:go;">
func display(message string){
	fmt.Println(message)
}
</pre>
<p>Эта функция имеет тип <span class="b">func(string)</span>. То есть опять же вначале идет слово func, а затем в скобках типы параметров. Так как функция не возвращает никакого результата, 
то соответственно возвращаемый тип не указывается.</p>
<p>Но какое значение имеет тип функции? Это значит, что мы можем определять переменные или параметры функций, которые будут представлять определенный тип функциии. То есть фактически переменная может быть функцией. Например:</p>
<pre class="brush:go;">
package main
...
<p>Здесь переменная f имеет тип <code>func(int, int) int</code>, то есть представляет любую функцию, которая принимает два параметра типа int и 
возвращает значение типа int. Поэтому мы можем присвоить этой переменной функцию add, которая соответствует данному типу:</p>
<pre class="brush:go;">var f func(int, int) int = add</pre>
<p>После этого мы можем вызывать присвоенную функцию через переменную, передавая нужные значения для ее параметров:</p>
...
<p>Также функция может передаваться в качестве параметра в другую функцию. Например:</p>
<pre class="brush:go;">
package main
...
<p>Здесь функция <code>action</code> принимает три параметра. Первые два параметра - числа, а третий параметр - функция, которая соответствует типу: 
<code>func(int , int) int</code>. То есть третий параметр представляет некоторое действие и может быть представлен любой функцией, которая принимает два значения типа int и возвращает также 
...
мы сможем вызывать данную функцию.</p>
...
<p>Здесь функция sum вычисляет сумму элементов среза. Но н всех элементов, а только тех, которые соответствуют условию. Условие передается в виде функции в качестве второго параметра. 
Условие должно соответствовать функции типа <code>func(int) bool</code>. То есть функция должна принимать в качестве параметра значение типа int и возвращать значение типа bool, которое указывает, 
соответствовует ли переданное число условию.</p>
<p>Для примера здесь также определены две вспомогательные функции: isEven (возвращает true, если число четное) и isPositive (возвращает true, если число 
положительное). Эти функции соответствуют типу <code>func(int) bool</code>, поэтому их можно использовать в качестве условия.</p>
...
<p>Здесь в зависимости от значения параметра функция selectFn возвращает одну из трех функций: add, subtract или multiply.</p>
...
<div class="nav"><p><a href="2.12.php">Назад</a><a href="./">Содержание</a><a href="2.16.php">Вперед</a></p></div></div>
<div class="socBlock">
