...
<html >
<head>
<title>Kotlin | Тип функции</title>
<meta charset="utf-8" />
<meta name="description" content="Тип функции в языке программирования Kotlin, передача функции переменной, вызов функции через переменную, оператор ::">
...
<h2>Тип функции</h2><div class="date">Последнее обновление: 26.05.2021</div>
...
<p>В Kotlin все является объектом, в том числе и функции. И функции, как и другие объекты, имеют определенный тип. Тип функции определяется следующим образом:</p>
<pre class="brush:kt;">(типы_параметров) -&gt; возвращаемый_тип</pre>
<p>Возьмем функцию которая не принимает никаких параметров и ничего не возвращает:</p>
<pre class="brush:kt;">
fun hello(){
...
<p>Она имеет тип</p>
<pre class="brush:kt;">() -&gt; Unit</pre>
<p>Если функция не принимает параметров,  определении типа указываются пустые скобки, а в качестве типа возвращаемого значения - тип <code>Unit</code>.<p>
<p>Возьмем другую функцию:</p>
<pre class="brush:kt;">
fun sum(a: Int, b: Int): Int{
    return a + b
}
</pre>
<p>Эта функция принимает два параметра типа Int и возвращает значение типа Int, поэтому она имеет тип</p>
<pre class="brush:kt;">(Int, Int) -&gt; Int</pre>
<p>Что дает нам знание типа функции? Используя тип функции, мы можем определять переменные и параметры других функций, которые будут представлять функции.</p>
<h3>Переменные-функции</h3>
<p>Переменная может представлять функцию. С помощью типа функции можно определить, какие именно функции переменная может представлять:</p>
<pre class="brush:kt;">
fun main() {
...
<p>Здесь переменная <code>message</code> представляет функцию с типом <code>() -&gt; Unit</code>, то есть функцию без параметров, которая ничего не возвращает. 
Далее определена как раз такая функция - <code>hello()</code>, соответственно мы можем передать функцию hello переменной message.</p>
<p>Чтобы передать функцию, перед названием функции ставится оператор <span class="b">::</span></p>
<pre class="brush:kt;">message = ::hello</pre>
<p>Затем мы можем обращаться к переменной <code>message()</code> как к обычной функции:</p>
<pre class="brush:kt;">message()</pre>
<p>Так как переменная message ссылается на функцию hello, то при вызове <code>message()</code> фактически будет вызываться функция <code>hello()</code>.</p>
<p>При этом тип функции также может выводится исходя из присваемого переменной значения:</p>
<pre class="brush:kt;">val message = ::hello // message имеет тип () -&gt; Unit</pre>
<p>Рассмотрим другой пример, когда переменная ссылается на функцию с параметрами:</p>
<pre class="brush:kt;">
fun main() {
...
<p>Переменная operation представляет функцию с типом <code>(Int, Int) -&gt; Int</code>, то есть функцию с двумя параметрами типа Int и возвращаемым значением типа Int. 
Соответственно такой переменной мы можем присвоить функцию <code>sum</code>, которая соответствует этому типу.</p>
<p>Затем через имя переменной фактически можно обращаться к функции <code>sum()</code>, передавая ей значения для параметров и получая ее результат:</p>
<pre class="brush:kt;">val result = operation(3, 5)</pre>
<p>При этом динамически можно менять значение, главное чтобы оно соответствовало типу переменной:</p>
<pre class="brush:kt;">
fun main() {
