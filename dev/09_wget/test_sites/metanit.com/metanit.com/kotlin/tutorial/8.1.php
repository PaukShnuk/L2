...
<html >
<head>
<title>Kotlin | Введение в корутины</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в корутины в языке программирования Kotlin, асинхронность, функции launch и coroutineScope, задержка выполнения с помощью функции delay, модификатор suspend">
...
<h1>Корутины</h1><h2>Введение в корутины</h2><div class="date">Последнее обновление: 22.01.2022</div>
...
<p>Далее на вкладке "Project Settings" перейдем к пункту <span class="b">Libraries</span>. В центральном поле отобразятся библиотеки, добавленные в проект. И 
для добавления новой библиотеки нажмем на знак плюса:</p>
<img src="./pics/coroutine2.png" alt="Add to console project kotlinx.coroutines" />
<p>Нас перебросит к папке <span class="b">lib</span>, которая содержит устанавливаемые по умолчанию библиотеки Kotlin. 
Если перебрасывает к другой папке, то мы можем вручную перейти к папке, где хранятся библиотеки по умолчанию. 
На Windows это будет папка <code>C:\Users\[Имя_пользователя]\AppData\Roaming\JetBrains\IdeaIC2021.3\plugins\Kotlin\kotlinc\lib</code></p> 
...
<img src="./pics/coroutine3.png" alt="Добавление в проект kotlinx-coroutines-core-jvm.jar" />
<p>После добавления в проекте в узле <span class="b">External Libraries / KotlinJavaRuntime</span> мы увидим добавленную библиотеку:</p>
<img src="./pics/coroutine4.png" alt="Add to console Kotlin project kotlinx-coroutines-core.jar" />
<p>В других типах проектов для подключения <code>kotlinx.coroutines</code> может использоваться Gradle или другие способы. Так, если проект 
использует Gradle, то в файл gradle добавляется зависимость:</p>
<pre class="brush:kt;">
dependencies {
...
<p>Сама функция <span class="b">delay()</span> то же является подобной функцией, которая определена с модификатором <span class="b">suspend</span>. А любая функция с модификатором 
...
<div class="nav"><p><a href="./7.15.php">Назад</a><a href="./">Содержание</a><a href="./8.2.php">Вперед</a></p></div></div>
<div class="socBlock">
