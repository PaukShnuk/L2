...
<html >
<head>
<title>Kotlin | Перечисления enums</title>
<meta charset="utf-8" />
<meta name="description" content="Перечисления enums в языке программирования Kotlin, определение и использование перечислений, применение интерфейсов, анонимные классы в перечислениях">
...
<h2>Перечисления enums</h2><div class="date">Последнее обновление: 29.05.2021</div>
...
<p>Enums или перечисления представляют тип данных, который позволяет определить набор логически связанных констант. Для определения перечисления 
применяются ключевые слова <span class="b">enum class</span>. Например, определим перечисление:</p>
<pre class="brush:kt;">
enum class Day{
    MONDAY, TUESDAY, WEDNESDAY, THURSDAY, FRIDAY, SATURDAY, SUNDAY
}
</pre>
<p>Данное перечисление Day представляет день недели. Внутри перечисления определяются константы. В данном случае это названия семи дней недели. Константы определяются 
через запятую. Каждая константа фактически представляет объект данного перечисления.</p>
<pre class="brush:kt;">
fun main() {
...
<p>Классы перечислений как и обычные классы также могут иметь конструктор. Кроме того, для констант перечисления также может вызываться конструктор для их инициализации.</p>
<pre class="brush:kt;">
...
<p>В примере выше у класса перечисления через конструктор определяется свойство value. Соответственно при определении констант перечисления необходимо каждую из этих констант инициализировать, передав значение для свойства value.</p>
<p>При этом перечисления - это не просто список значений. Они могут определять также свойства и функции. Но если класс перечисления содержит свойства или функции, 
то константы должны быть отделены точкой с запятой.</p>
<pre class="brush:kt;">
enum class Day(val value: Int){
    MONDAY(1), TUESDAY(2), WEDNESDAY(3),
...
<p>В данном случае в перечислении определена функция <code>getDuration()</code>, которая вычисляет разницу в днях между двумя днями недели.</p>
<h3>Встроенные свойства и вспомогательные методы</h3>
<p>Все перечисления обладают двумя встроенными свойствами:</p>
<ul>
<li><p><span class="b">name</span>: возвращает название константы в виде строки</p></li>
<li><p><span class="b">ordinal</span>: возвращает порядковый номер константы</p></li>
</ul>
<pre class="brush:kt;">
enum class Day(val value: Int){
    MONDAY(1), TUESDAY(2), WEDNESDAY(3),
...
<p>Кроме того, в Kotlin нам доступны вспомогательные функции:</p>
<ul>
<li><p><code>valueOf(value: String)</code>: возвращает объект перечисления по названию константы</p></li>
<li><p><code>values()</code>: возвращает массив констант текущего перечисления</p></li>
</ul>
<pre class="brush:kt;">
fun main() {
...
<h3>Анонимные классы и реализация интерфейсов</h3>
<p>Константы перечисления могут определять анонимные классы, которые могут иметь собственные методы и свойства или реализовать абстрактные методы класса перечисления:</p>
<pre class="brush:kt;">
enum class DayTime{
    DAY{
...
<p>В данном случае класс перечисления DayTime определяет абстрактный метод <code>printName()</code> и две переменных - <code>startHour</code> (начальный час) и <code>endHour</code> (конечный час). 
А константы определяют анонимные классы, которые реализуют эти свойства и функцию.</p>
<p>Также, классы перечислений могут применять интерфейсы. Для этого для каждой константы определяется анонимный класс, который содержат все реализуемые свойства и функции:</p>
<pre class="brush:kt;">
interface Printable{
    fun printName()
}
...
<p>Нередо перечисления применяются для хранения состояния в программе. И в зависимоси от этого состояния мы можем направить действие программы по определенному пути. 
Например, определим перечисление, которое представляет арифметические операции, и функцию, которая в зависимости от переданной операции выполняет то или иное действие:</p>
<pre class="brush:kt;">
...
<p>Функция <code>operate()</code> принимает два числа - операнды операции и тип операции в виде перечисления Operation. И в зависимоси от значения перечисления 
возвращает либо сумму, либо разность, либо произведение двух чисел.</p>
...
<div class="nav"><p><a href="./4.12.php">Назад</a><a href="./">Содержание</a><a href="./4.4.php">Вперед</a></p></div></div>
<div class="socBlock">
