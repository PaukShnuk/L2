...
<html >
<head>
<title>Kotlin | Получение части элементов</title>
<meta charset="utf-8" />
<meta name="description" content="Получение части элементов коллекций и последовательностей в языке Kotlin, функции slice, take, takeWhile, drop, dropWhile, chuncked">
...
<h2>Получение части элементов</h2><div class="date">Последнее обновление: 24.01.2022</div>
...
<h3>slice</h3>
<p>Функция <span class="b">slice()</span> возвращает часть коллекции, элементы которой располагаются на определенных индексах. Индексы передаеются в функцию 
либо в виде диапазона <code>IntRange</code>, либо коллекции значений <code>Iterable&lt;Int&gt;</code>. Результатом функции является список 
List&lt;T&gt;, который содержит элементы по указанным индексам:</p>
<pre class="brush:kt;">
...
<p>Стоит отметить, что для последовательностей эта функция не доступна.</p>
<h3>Take и takeWhile</h3>
<p>Функция <span class="b">take()</span> извлекает из начала коллекции/последовательности определенное количество элементов</p>
<pre class="brush:kt;">
take(n: Int): List&lt;T&gt;/Sequence&lt;T&gt;
</pre>
<p>Например, выберем 3 первых элементов:</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Bob", "Sam", "Kate", "Alice", "Mike")
println(people.take(3))     // ["Tom", "Bob", "Sam"]
</pre>
<p>Для коллекций также доступна функция <span class="b">takeLast()</span>, которая извлекает определенное количество элементов из конца коллекции</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Bob", "Sam", "Kate", "Alice", "Mike")
// получаем три последних элемента
println(people.takeLast(3))     // ["Kate", "Alice", "Mike"]
</pre>
<p>Функция <span class="b">takeWhile()</span> отбирает элементы с начала коллекции/последовательности, которые соответвуют условию предиката:</p>
<pre class="brush:kt;">
takeWhile(predicate: (T) -&gt; Boolean): List&lt;T&gt;       // для коллекций
takeWhile(predicate: (T) -&gt; Boolean): Sequence&lt;T&gt;   // для последовательностей
</pre>
<p>функция предиката получает в качестве параметра текущий элемент и возращает true, если элемент соответствует условию:</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice", "Mike")
val part = people.takeWhile{it.length == 3}
println(part)     // ["Tom", "Sam"]
</pre>
<p>В данном случае условие предиката возвращает true, если длина строки равна 3: <code>{it.length == 3}</code>, 
...
<code>["Tom", "Sam"]</code>. Когда функция <code>takeWhile()</code> встретит хоть один элемент, который не соответствует условию, она завершит работу. 
Хотя в списке people есть еще элементы с длиной в три символа (элемент <code>"Bob"</code>). Поэтому если в списке первый элемент уже не соответствует условию предиката, то функция возвратит пустой список:</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice", "Mike")
val part = people.takeWhile{it.length == 4}     // выбираем строки с длиной в 4 символа
println(part)     // [] - пустой список
</pre>
<p>Для коллекций также доступа функция <span class="b">takeLastWhile()</span>, которая работает аналогичным образом, только выбирает элементы с конца списка</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice", "Mike")
val part = people.takeLastWhile{it.length !=3} // длина НЕ равна 3
println(part)     // ["Alice", "Mike"]
</pre>
<h3>drop и dropWhile</h3>
<p>Функция <span class="b">drop()</span> позволяет пропустить определенное количество элементов в коллекции/последовательности:</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice", "Mike")
val part = people.drop(3) // пропускаем первые 3 элемента
println(part)     // [Bob, Alice, Mike]
</pre>
<p>Для коллекций также доступна функция <span class="b">dropLast()</span>, которая проупскает определенное количество элементов из конца коллекции</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice", "Mike")
val part = people.dropLast(2) // пропускаем последние 2 элемента
println(part)     // [Tom, Sam, Kate, Bob]
</pre>
<p>Функция <span class="b">dropWhile()</span> пропускает элементы с начала коллекции/последовательности, которые соответвуют условию предиката:</p>
<pre class="brush:kt;">
...
<p>В данном случае условие предиката возвращает true, если длина строки равна 3: <code>{it.length == 3}</code>, 
поэтому функция <code>dropWhile()</code> пропускает строки, пока они соответствуют этому условию. Так, в данном случае в результирующем списке окажется первые два элемента: 
<code>[Kate, Bob, Alice, Mike]</code>. Когда функция <code>dropWhile()</code> встретит хоть один элемент, который не соответствует условию, она завершит работу. 
Хотя в списке people есть еще элементы с длиной в три символа (элемент <code>"Bob"</code>). Поэтому если в списке первый элемент уже не соответствует условию предиката, то функция возвратит список со всеми элементами.</p>
<p>Для коллекций также доступа функция <span class="b">dropLastWhile()</span>, которая работает аналогичным образом, только пропускает элементы с конца списка</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice", "Mike")
val part = people.dropLastWhile{it.length !=3} // длина НЕ равна 3
println(part)     // ["Tom", "Sam", "Kate", "Bob"]
</pre>
<h3>Разделение на части</h3>
<p>Функция <span class="b">chuncked()</span> позволяет разбить коллекцию/последовательность на списки определенной длины:</p>
<pre class="brush:kt;">
chunked(size: Int): List&lt;List&lt;T&gt;&gt;       // для коллекций
chunked(size: Int): Sequence&lt;List&lt;T&gt;&gt;  // для последовательностей
</pre>
<p>В качестве параметра в функцию передается размер списков, в которые будут помещаться элементы:</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice", "Mike")
val parts = people.chunked(3)
println(parts)     // [[Tom, Sam, Kate], [Bob, Alice, Mike]]
</pre>
<p>В данном случае разбиваем коллекцию на списки по 3 элемента, соответственно результирующая коллекция будет содержать два списка по три элемента.</p>
<p>Дополнительная версия функции в качестве второго параметра принимает функцию преобразования элементов:</p>
<pre class="brush:kt;">
...
<p>То есть сначала коллекция/последовательность разбивается на несколько списков длиной size, затем каждый список передается в функцию преобразования, 
где на его основе можно сгенерировать новое значение. Например:</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice", "Mike")
val parts = people.chunked(3){it.first()}
println(parts)     // [Tom, Bob]
</pre>
<p>В данном случае коллекция разделяется на два списка длиной по 3 элемента. В функции преобразования из каждого списка выбирается и возвращается 
первый элемент каждого списка. Соответственно результатом функции будет список из двух первых элементов.</p>
...
<div class="nav"><p><a href="./7.13.php">Назад</a><a href="./">Содержание</a><a href="./7.15.php">Вперед</a></p></div></div>
<div class="socBlock">
