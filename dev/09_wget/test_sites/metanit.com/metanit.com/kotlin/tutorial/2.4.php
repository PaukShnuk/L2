...
<html >
<head>
<title>Kotlin | Операции с числами</title>
<meta charset="utf-8" />
<meta name="description" content="Операции с числами в языке программирования Kotlin, арифметические и поразрядные операции, инкремент, декремент">
...
<h2>Операции с числами</h2><div class="date">Последнее обновление: 23.05.2021</div>
...
<ul>
<li><p><span class="b">+</span> (сложение): возвращает сумму двух чисел.</p>
<pre class="brush:kt;">
val x = 5
val y = 6
val z = x + y
...
<li><p><span class="b">-</span> (вычитание): возвращает разность двух чисел.</p>
<pre class="brush:kt;">
val x = 5
val y = 6
val z = x - y  // z = -1
...
<li><p><span class="b">*</span> (умножение): возвращает произведение двух чисел.</p>
<pre class="brush:kt;">
val x = 5
val y = 6
val z = x * y  // z = 30
...
<li><p><span class="b">/</span> (деление): возвращает частное двух чисел.</p>
<pre class="brush:kt;">
val x = 60
val y = 10
val z = x / y  // z = 6
</pre>
<p>При этом если в операции деления оба операнда представляют целые числа, то результатом тоже будет целое число, а если в процессе деления образовалась дробная часть, то она отбрасывается:</p>
<pre class="brush:kt;">
fun main() {
...
<p>Так в данном случае, хотя если согласно стандартной математике разделить 11 на 5, то получится 2.2. Однако поскольку оба операнда представляют целочисленный тип, а именно тип <code>Int</code>, 
то дробная часть - 0.2 отрабрасывается, поэтому результатом будет число 2, а переменная <code>z</code> будет представлять тип <code>Int</code>.</p>
...
<p>В данном случае переменная <code>y</code> представляет тип <code>Double</code>, поэтому результатом деления будет число 2.2, а переменная <code>z</code> также 
будет представлять тип <code>Double</code></p>

</li>
<li><p><span class="b">%</span>: возвращает остаток от целочисленного деления двух чисел.</p>
<pre class="brush:kt;">
val x = 65
val y = 10
val z = x % y  // z = 5
...
<li><p><span class="b">++</span> (инкремент): увеличивает значение на единицу.</p>
...
<li><p><span class="b">--</span> (декремент): уменьшает значение на единицу.</p>
...
<p>Постфиксный декремент возвращает значение до уменьшения на единицу:</p>
<pre class="brush:kt;">
var x = 5
val y = x--
println(x)      // x = 4
...
<p>Также есть ряд операций присвоения, которые сочетают арифметические операции и присвоение:</p>
<ul>
<li><p><span class="b">+=</span>: присваивание после сложения. Присваивает левому операнду сумму левого и правого операндов: <span class="b">A += B</span> эквивалентно 
<span class="b">A = A + B</span></p></li>
<li><p><span class="b">-=</span>: присваивание после вычитания. Присваивает левому операнду разность левого и правого операндов: 
<span class="b">A -= B</span> эквивалентно <span class="b">A = A - B</span></p></li>
<li><p><span class="b">*=</span>: присваивание после умножения. Присваивает левому операнду произведение левого и правого операндов: 
<span class="b">A *= B</span> эквивалентно <span class="b">A = A * B</span></p></li>
<li><p><span class="b">/=</span>: присваивание после деления. Присваивает левому операнду частное левого и правого операндов: 
<span class="b">A /= B</span> эквивалентно <span class="b">A = A / B</span></p></li>
<li><p><span class="b">%=</span>: присваивание после деления по модулю. Присваивает левому операнду остаток от целочисленного деления левого операнда на правый: 
<span class="b">A %= B</span> эквивалентно <span class="b">A = A % B</span></p></li>
</ul>
<h3>Поразрядные операции</h3>
<p>Ряд операций выполняется над двоичными разрядами числа. Здесь важно понимать, как выглядит двоичное представление тех или иных чисел. 
В частности, число 4 в двоичном виде - 100, а число 15 - 1111.</p>
<p>Есть следующие поразрядные операторы (они применяются только к данным типов Int и Long):</p>
<ul>
<li><p><span class="b">shl</span>: сдвиг битов числа со знаком влево</p>
<pre class="brush:kt;">
...
<p>В данном случае число сдвигается на два разряда влево, поэтому справа число в двоичном виде дополняется двумя нулями. То есть в двоичном виде 3 представляет 11. Сдвигаем 
на два разряда влево (дополняем справа двумя нулями) и получаем 1100, то есть в десятичной системе число 12.</p>
</li>
<li><p><span class="b">shr</span>: сдвиг битов числа со знаком вправо</p>
<pre class="brush:kt;">
...
<p>Число 12 сдвигается на два разряда вправо, то есть два числа справа факически отбрасываем и получаем число 11, то есть 3 в десятичой системе.</p>
</li>
<li><p><span class="b">ushr</span>: сдвиг битов беззнакового числа вправо</p>
<pre class="brush:kt;">
val z = 12 ushr 2     // z = 1100 &gt;&gt; 2 = 11
println(z)          // z = 3
</pre>
</li>
<li><p><span class="b">and</span>: побитовая операция AND (логическое умножение или конъюнкция). Эта операция сравнивает соответствующие разряды двух чисел и возвращает единицу, если эти разряды обоих чисел равны 1. 
Иначе возвращает 0.</p>
<pre class="brush:kt;">
val x = 5   // 101
val y = 6   // 110
...
<li><p><span class="b">or</span>: побитовая операция OR (логическое сложение или дизъюнкция). Эта операция сравнивают два соответствуюших разряда обоих чисел и 
возвращает 1, если хотя бы один разряд равен 1. Если оба разряда равны 0, то возвращается 0.</p>
<pre class="brush:kt;">
val x = 5   // 101
val y = 6   // 110
...
<li><p><span class="b">xor</span>: побитовая операция XOR. Сравнивает два разряда и возвращает 1, если один из разрядов равен 1, а другой равен 0. 
Если оба разряда равны, то возвращается 0.</p>
<pre class="brush:kt;">
val x = 5   // 101
val y = 6   // 110
...
<li><p><span class="b">inv</span>: логическое отрицание или инверсия - инвертирует биты числа</p>
<pre class="brush:kt;">
val b = 11  // 1011
val c = b.inv()
println(c)      // -12
...
<div class="nav"><p><a href="./2.10.php">Назад</a><a href="./">Содержание</a><a href="./2.5.php">Вперед</a></p></div></div>
<div class="socBlock">
