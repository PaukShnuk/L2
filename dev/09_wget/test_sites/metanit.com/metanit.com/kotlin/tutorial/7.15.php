...
<html >
<head>
<title>Kotlin | Получение отдельных элементов</title>
<meta charset="utf-8" />
<meta name="description" content="Получение отдельных элементов коллекций и последовательностей в языке Kotlin, выборка элементов по условию и по позиции, функции elementAt, elementAtOrElse, elementAtOrNull, first, last, random">
...
<h2>Получение отдельных элементов</h2><div class="date">Последнее обновление: 24.01.2022</div>
...
<h3>Получение элемента по индексу</h3>
<p>Для получения элемента по индексу применяется функция <span class="b">elementAt()</span>, в которую передается индекс элемента:</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice")
// получаем элемент по индексу 1
println(people.elementAt(1))     // Sam
</pre>
<p>Однако если переданный индекс выходит за границы коллекции/последовательности, то приложение сгенерирует ошибку. В этом случае мы можем использовать 
функцию <span class="b">elementAtOrNull()</span>, которая возвращает null, если индекс вне границ:</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice")
println(people.elementAtOrNull(1))     // Sam
println(people.elementAtOrNull(6))     // null
</pre>
<p>Также в этом случае можно использовать функцию <span class="b">elementAtOrElse()</span>:</p>
<pre class="brush:kt;">
elementAtOrElse(index: Int, defaultValue: (Int) -&gt; T): T
</pre>
<p>Первый параметр функции - индекс элемента, а второй - функция, которая получает индекс и возвращает значение по умолчанию, если индекс находится вне границ 
коллекции/последовательности.</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice")
...
<h3>Получение по условию</h3>
<p>Если надо получить первый или последний элементы коллекции/последовательности, то можно использовать соответственно функции <span class="b">first()</span> 
и <span class="b">last()</span>:</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice")
println(people.first())     // Tom
println(people.last())     // Alice
</pre>
<p>В качестве параметра эти функции могут принимать функцию предиката</p>
<pre class="brush:kt;">
first(predicate: (T) -&gt; Boolean): T
last(predicate: (T) -&gt; Boolean): T
</pre>
<p>Эти функции возвращают первый и последний элементы, которые соответствуют условию предиката:</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice")
...
<p>Однако если коллеккция пуста, или в коллекции/последовательности нет элементов, которые соответствуют условию, то программа генерирует ошибку. В этом случае 
можно применять функции <span class="b">firstOrNull()</span> и <span class="b">lastOrNull()</span>, которые возвращают null, если коллекция пуста или 
в ней нет элементов, соответствующих условию:</p>
<pre class="brush:kt;">
firstOrNull(): T?
...
<h3>Выбор случайного элемента</h3>
<p>Для извлечения случайного элемента применяется функция <span class="b">random()</span></p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice")
println(people.random())
</pre>
<p>Однако если коллекция/последовательность пуста, то эта функция генерирует ошибку. В этом случае можно использовать функцию <span class="b">randomOrNull()</span>, 
которая в этом случае возвращает null:</p>
<pre class="brush:kt;">
...
<div class="nav"><p><a href="./7.14.php">Назад</a><a href="./">Содержание</a><a href="./8.1.php">Вперед</a></p></div></div>
<div class="socBlock">
