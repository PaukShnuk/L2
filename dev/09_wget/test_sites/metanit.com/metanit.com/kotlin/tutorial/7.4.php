...
<meta charset="utf-8" />
<meta name="description" content="Коллекция Map в Kotlin, MutableMap, методы mapOf, mutableMapOf, hashMapOf, linkedMapOf, классы HashMap и LinkedHashMap, перебор коллекции, получение ключей и значений по ключу, изменение карты">
...
<h2>Map</h2><div class="date">Последнее обновление: 24.01.2022</div>
...
<p>Коллекция <span class="b">Map</span> представляет коллекцию объектов, где каждый элемент имеет ключ и сопоставляемое с ним значение. 
При этом все ключи в коллекции являются уникальными. В отличие от List и Set интерфейс Map не расширяет интерфейс Collection.</p>
<p><span class="b">Map</span> представляет неизменяемую коллекцию, для создания которой применяется метод <span class="b">mapOf()</span>.</p>
<pre class="brush:kt;">
val people = mapOf(1 to "Tom", 5 to "Sam", 8 to "Bob")
</pre>
<p>Функция mapOf принимает набор элементов, каждый из которых с помощью оператора <span class="b">to</span> сопоставляет ключ со значением, например, 
<code>1 to "Tom"</code> (с условным идентификатором пользователя сопоставляется его имя). В данном случае переменная <code>people</code> представляет 
объект <code>Map&lt;Int, String&gt;</code>, где первый тип - Int представляет тип ключей (идентификатор пользователя), а второй тип - String 
представляет тип значений.</p>
<p>Для перебора Map можно использовать цикл for, при этом каждый элемент будет представлять объект <code>Map.Entry&lt;K, V&gt;</code>. Через его 
свойство <span class="b">key</span> можно получить ключ, а с помощью <span class="b">value</span> - значение:</p>
<pre class="brush:kt;">
val people = mapOf(1 to "Tom", 5 to "Sam", 8 to "Bob")
...
<h3>Обращение к элементам Map</h3>
<p>Для получения элементов по ключу может применяться метод <span class="b">get()</span>, в который передается ключ элемента:</p>
<pre class="brush:kt;">
val dictionary = mapOf("red" to "красный", "blue" to "синий", "green" to "зеленый")
val blue = dictionary.get("blue")
println(blue)   // синий
</pre>
<p>В данном случае переменная <code>dictionary</code> представляет объект <code>Map&lt;String, String&gt;</code>, где ключи представляют строки, а значения - 
то же строки (условный перевод слова). Для получения значения по ключу "blue", применяется выражение <code>dictionary.get("blue")</code></p>
<p>Также можно сократить получение элемента с помощью квадратных скобок:</p>
<pre class="brush:kt;">
val dictionary = mapOf("red" to "красный", "blue" to "синий", "green" to "зеленый")
val blue = dictionary["blue"]
println(blue)   // синий
</pre>
<p>Если в Map нет элемента с указанным ключом, то возвращается null:</p>
<pre class="brush:kt;">
...
<p>Такое поведение не всегда может быть предпочтительно. И в этом случае можно использовать пару других методов для получения элементов. Так, метод 
<span class="b">getOrDefault()</span> позволяет задать значение по умолчанию, которое будет возврашаться, если по указанному ключу нет элементов:</p>
<pre class="brush:kt;">
val dictionary = mapOf("red" to "красный", "blue" to "синий", "green" to "зеленый")
...
<p>Еще один метод - <span class="b">getOrElse()</span> в качестве второго параметра принимает функцию, которая задает значение на случай, 
если по указанному ключу нет элементов:</p>
<pre class="brush:kt;">
val dictionary = mapOf("red" to "красный", "blue" to "синий", "green" to "зеленый")
...
<p>Кроме получения отдельных элементов Map позволяет получить отдельно ключи и значения с помощью свойств :</p>
<pre class="brush:kt;">
val dictionary = mapOf("red" to "красный", "blue" to "синий", "green" to "зеленый")
println(dictionary.values)  // [красный, синий, зеленый]
println(dictionary.keys)   // [red, blue, green]
</pre>
<p>Также с помощью методов <span class="b">containsKey()</span> и <span class="b">containsValue()</span> можно проверить наличие в Map определенного ключа и 
значения соответственно:</p>
<pre class="brush:kt;">
val dictionary = mapOf("red" to "красный", "blue" to "синий", "green" to "зеленый")
...
<h3>MutableMap</h3>
<p>Изменяемые коллекции представлены интерфейсом <span class="b">MutableMap</span>, который расширяет интерфейс Map. Для создания объекта 
MutableMap применяется функция <span class="b">mutableMapOf()</span>.</p>
<pre class="brush:kt;">
val people = mutableMapOf(220 to "Tom", 225 to "Sam", 228 to "Bob") // MutableMap&lt;Int, String&gt;
</pre>
<p>Интерфейс MutableMap реализуется рядом коллекций:</p>
<ul>
<li><p><span class="b">HashMap</span>: простейшая реализация интерфейса MutableMap, не гарантирует порядок элементов в коллекции. Создается функцией <span class="b">hashMapOf()</span></p></li>
<li><p><span class="b">LinkedHashMap</span>: представляет комбинацию HashMap и связанного списка, создается функцией <span class="b">linkedMapOf()</span></p></li>
</ul>
<pre class="brush:kt;">
...
<p>Для изменения Map можно применять ряд методов:</p>
<ul>
<li><p><span class="b">put(key: K, value: V)</span>: добавляет элемент с ключом key и значением value</p></li>
<li><p><span class="b">putAll()</span>: добавляет набор объектов типа <code>Pair&lt;K, V&gt;</code>. В качестве такого набора могут выступать объекты 
Iterable, Sequence и Array</p></li>
<li><p><span class="b">set(key: K, value: V)</span>: устанавливает для элемента с ключом key значение value</p></li>
<li><p><span class="b">remove(key: K): V?</span>: удаляет элемент с ключом key. Если удаление успешно произведено, то возвращается значение удаленного элемента, иначе возвращается null.</p>
<p>Дополнительная версия метода</p><pre class="brush:kt;">remove(key: K, value: V): Boolean</pre>
<p>удаляет элемент с ключом key, если он имеет значение value, и возвращает true, если элемент был успешно удален</p></li>
</ul>
...
<p>Вместо метода set для установки значения могут применяться квадратные скобки, в которые передается ключ элемента:</p>
<pre class="brush:kt;">
val people = mutableMapOf(1 to "Tom", 2 to "Sam", 3 to "Bob")
...
<p>Причем если с указанным ключом нет элемента, то он добавляется, как в примере выше в случае с ключом 5.</p>
