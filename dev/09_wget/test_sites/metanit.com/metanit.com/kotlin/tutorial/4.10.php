...
<html >
<head>
<title>Kotlin | Переопределение методов и свойств</title>
<meta charset="utf-8" />
<meta name="description" content="Переопределение методов и свойств при наследовании в языке программирования Kotlin, аннотации open и override, правила переопределения">
...
<h2>Переопределение методов и свойств</h2><div class="date">Последнее обновление: 30.05.2021</div>
...
<p>Kotlin позволяет переопределять в производном классе функции и свойства, которые определенны в базовом классе. 
Чтобы функции и свойства базового класа можно было переопределить, к ним применяется аннотация 
<span class="b">open</span>. При переопределении в производном классе к этим функциям применяется аннотация <span class="b">override</span>.</p>
<h3>Переопределение свойств</h3>
<p>Чтобы указать, что свойство можно переопределить в производном классе, перед его определением указывается ключевое слово <span class="b">open</span>:</p>
<pre class="brush:kt;">
open class Person(val name: String){
    open var age: Int = 1
}
</pre>
<p>В данном случае свойство <code>age</code> доступно для переопределения.</p>
<p>Если свойство определяется через первичный конструктор, то также перед его определением ставится аннотация <span class="b">open</span>:</p>
<pre class="brush:kt;">
open class Person(val name: String, open var age: Int = 1){
}
</pre>
<p>В производном классе для переопределения свойства перед ним указывается аннотация <span class="b">override</span>.</p>
<pre class="brush:kt;">
open class Person(val name: String, open var age: Int = 1){
}
...
<p>Здесь переопределение заключается в изменении начального значения для свойства age.</p>
<p>Также переопределить свойство можно сразу в первичном конструкторе:</p>
<pre class="brush:kt;">
open class Person(val name: String, open var age: Int = 1){
}
...
<h3>Переопределение геттеров и сеттеров</h3>
<p>Также можно переопределять геттеры и сеттеры свойств:</p>
<pre class="brush:kt;">
open class Person(val name: String){
...
<p>Здесь класс Employee переопределяет геттер свойства <code>fullInfo</code> и сеттер свойства <code>age</code></p>
<h3>Переопределение методов</h3>
<p>Чтобы функции базового класа можно было переопределить, к ним применяется аннотация <span class="b">open</span>. При переопределении в 
производном классе к этим функциям применяется аннотация <span class="b">override</span>:</p>
<pre class="brush:kt;">
open class Person(val name: String){
    open fun display(){
        println("Name: $name")
...
<p>Функция display определена в классе Person с аннотацией <span class="b">open</span>, поэтому в производных классах его можно переопределить. 
В классе Employee эта функция переопределена с применением аннотации <span class="b">override</span>.</p>
<h3>Переопределение в иерархии наследования классов</h3>
<p>Стоит учитывать, что переопределить функции можно по всей иерархии наследования. Например, у нас может быть класс Manager, унаследованный от Employee:</p>
<pre class="brush:kt;">
open class Person(val name: String){
    open fun display(){
        println("Name: $name")
...
<p>В данном случае класс Manager переопределяет функцию display, поскольку среди его базовых классов есть класс Person, который определяет эту функцию 
с ключевым словом <span class="b">open</span>.</p>
<h3>Запрет переопределения</h3>
<p>В это же время иногда бывает необходимо запретить дальнейшее переопределение функции в классах-наследниках. Для этого применяется ключевое слово 
<span class="b">final</span>:</p>
<pre class="brush:kt;">
open class Person(val name: String){
    open fun display(){
        println("Name: $name")
...
<h3>Обращение к реализации из базового класса</h3>
<p>С помощью ключевого слова <span class="b">super</span> в производном классе можно обращаться к реализации из базового класса.</p>
<pre class="brush:kt;">
open class Person(val name: String){
...
<p>В данном случае производный класс Employee при переопределении свойства и функции применяет реализацию из базового класса Person. Например, через 
<code>super.fullInfo</code> возвращается значение свойства из базового класса (то есть значение свойства name), а с помощью вызова <code>super.display()</code> 
вызывается реализация функции display из класса Person.</p>
...
<div class="nav"><p><a href="./4.2.php">Назад</a><a href="./">Содержание</a><a href="./4.11.php">Вперед</a></p></div></div>
<div class="socBlock">
