...
<html >
<head>
<title>Kotlin | Вариантность, ковариантность и контравариантность</title>
<meta charset="utf-8" />
<meta name="description" content="Вариантность, ковариантность и контравариантность в языке программирования Kotlin, создание ковариантных и контравариантных обобщенных типов, операторы in и out">
...
<h2>Вариантность, ковариантность и контравариантность</h2><div class="date">Последнее обновление: 02.05.2021</div>
...
<p>Вариантность описывает, как обобщенные типы, типизированные классами из одной иерархии наследования, соотносятся друг с другом.</p>
<h3>Инвариантность</h3>
<p>Инвариантность предполагает, что, если у нас есть классы Base и Derived, где Derived - производный класс от Base, то класс C&lt;Base&gt; не 
является ни базовым классом для С&lt;Derived&gt;, ни производным. Например, у нас есть следующие типы:</p>
<pre class="brush:kt;">
...
<p>В данном случае мы не можем присвоить объект <code>Messenger&lt;EmailMessage&gt;</code> переменной типа <code>Messenger&lt;Message&gt;</code> и наоборот, они никак между собой 
не соотносятся, несмотря на то, что EmailMessage наследуется от Message:</p>
<pre class="brush:kt;">
...
<p>Мы можем присвоить переменным по умолчанию только объекты их типов:</p>
<pre class="brush:kt;">
fun changeMessengerToDefault(obj: Messenger&lt;Message&gt;){
...
<h3>Ковариантость</h3>
<p>Ковариантость предполагает, что, если у нас есть классы Base и Derived, где Base - базовый класс для Derived, то класс SomeClass&lt;Base&gt; 
является базовым классом для SomeClass&lt;Derived&gt;</p>
<p>Для определения обобщенного типа как ковариантного параметр обощения определяется с ключевым словом <span class="b">out</span>:</p>
<pre class="brush:kt;">
interface Messenger&lt;out T: Message&gt;
open class Message(val text: String)
class EmailMessage(text: String): Message(text)
</pre>
<p>В данном случае интерфейс Messenger является <span class="b">ковариантным</span>, так как его параметр определен со словом out: <code>interface Messenger&lt;out T&gt;</code>. 
И теперь переменной типа <code>Messenger&lt;Message&gt;</code> мы можем присвоить значение типа <code>Messenger&lt;EmailMessage&gt;</code></p> 
<pre class="brush:kt;">
fun changeMessengerToEmail(obj: Messenger&lt;EmailMessage&gt;){
    val messenger: Messenger&lt;Message&gt; = obj
}
</pre>
<p>Вообще не случайно используется именно слово <span class="b">out</span>. Оно указывает, что обобщенный тип может возвращать из функции значение типа T:</p>
<pre class="brush:kt;">
fun main() {
...
<p>В данном случае обобщенный интерфейс Messenger определяет функцию <code>writeMessage()</code> для генерации объекта Message. Класс EmailMessenger применяет интерфейс Messenger и реализует 
эту функцию. То есть в данном случае тип EmailMessenger по сути представляет тип <code>Messenger&lt;EmailMessage&gt;</code>.</p>
<p>Поскольку в Messenger параметр T определен с аннотацией <span class="b">out</span>, то мы можем присвоить переменной типа <code>Messenger&lt;Message&gt;</code> 
значение типа <code>EmailMessenger</code> (а по сути значение типа <code>Messenger&lt;EmailMessage&gt;</code>)</p>
<pre class="brush:kt;">val messenger: Messenger&lt;Message&gt; = EmailMessenger()</pre>
<p>В то же время тип T нельзя использовать в качестве типа входных параметров функции. Например, в следующем случае компилятор известит нас об ошибке:</p>
<pre class="brush:kt;">
interface Messenger&lt;out T: Message&gt;{
    fun writeMessage(text: String): T
...
<p>Контравариантость предполагает в какой-то степени обратную ситуацию. Контравариантость предполагает, что, если у нас есть классы Base и Derived, где 
Base - базовый класс для Derived, то объекту SomeClass&lt;Derived&gt; мы можем присвоить значение SomeClass&lt;Base&gt; (при ковариантности, наоборот, - объекту 
SomeClass&lt;Base&gt; можно присвоить значение SomeClass&lt;Derived&gt;)</p>
<p>Для определения обобщенного типа как контравариантного параметр обобщения определяется с ключевым словом <span class="b">in</span>:</p>
<pre class="brush:kt;">
interface Messenger&lt;in T: Message&gt;
open class Message(val text: String)
class EmailMessage(text: String): Message(text)
</pre>
<p>В данном случае интерфейс Messenger является <span class="b">контравариантным</span>, так как его параметр определен со словом <span class="b">in</span>: <code>interface Messenger&lt;in T&gt;</code>. 
И теперь переменной типа <code>Messenger&lt;EmailMessage&gt;</code> мы можем присвоить значение типа <code>Messenger&lt;Message&gt;</code></p> 
<pre class="brush:kt;">
fun changeMessengerToDefault(obj: Messenger&lt;Message&gt;){
    val messenger: Messenger&lt;EmailMessage&gt; = obj
}
</pre>
<p>Применение аннотации <span class="b">in</span> означает, что обобщенный тип может получать значение типа T через параметр функции:</p>
<pre class="brush:kt;">
fun main() {
...
<p>В данном случае обобщенный интерфейс Messenger определяет функцию <code>sendMessage()</code>, которая принимает объект Message в качестве параметра. 
Класс InstantMessenger применяет интерфейс Messenger и реализует эту функцию. То есть в данном случае тип InstantMessenger по сути представляет тип 
<code>Messenger&lt;Message&gt;</code>.</p>
<p>Поскольку в интерфейсе Messenger параметр T определен с аннотацией <span class="b">in</span>, то мы можем присвоить переменной типа 
<code>Messenger&lt;EmailMessage&gt;</code> 
значение типа <code>InstantMessenger</code> (то есть значение типа <code>Messenger&lt;Message&gt;</code>)</p>
<pre class="brush:kt;">val messenger: Messenger&lt;EmailMessage&gt; = InstantMessenger()</pre>
<p>В то же время тип T нельзя использовать в качестве типа результата функции. Например, в следующем случае компилятор известит нас об ошибке:</p>
<pre class="brush:kt;">
interface Messenger&lt;in T: Message&gt;{
...
<div class="nav"><p><a href="./6.2.php">Назад</a><a href="./">Содержание</a><a href="./5.2.php">Вперед</a></p></div></div>
<div class="socBlock">
