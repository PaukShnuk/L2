...
<html >
<head>
<title>Kotlin | Абстрактные классы и методы</title>
<meta charset="utf-8" />
<meta name="description" content="Абстрактные классы и методы в языке программирования Kotlin, модификатор abstract">
...
<h2>Абстрактные классы и методы</h2><div class="date">Последнее обновление: 29.05.2021</div>
...
<p>Абстрактные классы - это классы, определенные с модификатором <span class="b">abstract</span>. Отличительной особенностью абстрактных классов является то, что 
мы не можем создать объект подобного класса. Например, определим абстрактный класс Human:</p>
<pre class="brush:kt;">abstract class Human(val name: String)</pre>
<p>Абстрактный класс, как и обычный, может иметь свойства, функции, конструкторы, но создать его объект напрямую вызвав его конструктор мы не можем:</p>
<pre class="brush:kt;">
val kate: Human		// норм, просто определение переменной
val alice: Human = Human("Alice")	// ! ошибка, создать объект нельзя
</pre>
<p>Такой класс мы можем только унаследовать:</p>
<pre class="brush:kt;">
...
<p>Стоит отметить, что в данном случае перед абстрактным классом не надо указывать аннотацию <span class="b">open</span>, как при наследовании неабстрактных классов.</p>
<pre class="brush:kt;">
...
<p>Абстрактные классы могут иметь абстрактные методы и свойства. Это такие функции и свойства, которые определяются с ключевым словом <span class="b">abstract</span>. 
Абстрактные методы не содержат реализацию, то есть у них нет тела. А для абстрактных свойств не указывается значение. 
При этом абстрактные методы и свойства можно определить только в абстрактных классах:</p>
<pre class="brush:kt;">
abstract class Human(val name: String){
...
<p>Если класс наследуется от абстрактного класса, то он должен либо реализовать все его абстрактные методы и свойства, либо также быть абстрактным.</p>
<p>Так, в данном случае класс Person должен обязательно 
определить реализацию для функции <code>hello()</code> и свойства <code>age</code>. При этом, как и при переопределении обычных методов и свойств, применяется аннотация <span class="b">override</span>.</p>
<p>Абстрактные свойства также можно реализовать в первичном конструкторе:</p>
<pre class="brush:kt;">
abstract class Human(val name: String){
...
<p>Зачем нужны абстрактные классы? Классы обычно отражают какие-то сущности реального мира. Но некоторые из этих сущностей представляют абстракцию, которая 
...
можем определить абстрактный класс фигуры и затем от него унаследовать все остальные классы фигур:</p>
<pre class="brush:kt;">
...
<div class="nav"><p><a href="./4.10.php">Назад</a><a href="./">Содержание</a><a href="./4.8.php">Вперед</a></p></div></div>
<div class="socBlock">
