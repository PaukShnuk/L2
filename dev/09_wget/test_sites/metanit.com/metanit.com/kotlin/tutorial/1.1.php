...
<html >
<head>
<title>Kotlin | Введение в язык. Первая программа</title>
<meta charset="utf-8" />
<meta name="description" content="Основные особенности языка программирования Kotlin, для чего он нужен, первая программа, компиляция в консоли">
...
<h1>Введение в язык Kotlin</h1><h2>Что такое Kotlin. Первая программа</h2><div class="date">Последнее обновление: 22.05.2021</div>
...
<p><span class="b">Kotlin</span> представляет современный, статически типизированный и один из самых быстроразвивающихся языков 
...
так и браузерные клиентские приложения - фронтенд. Kotlin также можно применять для создания десктопных приложений, для Data Science и так далее.</p>
<p>Таким образом, круг платформ, для которых можно создавать приложения на Kotlin, чрезвычайно широк - Windows, Linux, Mac OS, iOS, Android.</p>

<p>Самым популярным направлением, где применяется Kotlin, является прежде всего разработка под ОС Android. Причем настолько популярным, 
...
Java и C++), а инструменты по работе с данным языком были по умолчанию включены в функционал среды разработки Android Strudio начиная с версии 3.0.</p>
<p>Официальный сайт языка - <a href="https://kotlinlang.org/" rel="nofollow">https://kotlinlang.org/</a>, где можно найти самую последнюю и самую подробную информацию по языку.</p>
<p>Первая версия языка вышла 15 февраля 2016 года. Хотя сама разработка языка велась с 2010 года. Текущей версией языка на данный момент 
является версия 1.5, которая вышла 5 мая 2021 года.</p>
<p>Kotlin испытал влияние многих языков: Java, Scala, Groovy, C#, JavaScript, Swift и позволяет писать программы как в объектно-ориентированном, так и в функциональном стиле. 
Он имеет ясный и понятный синтаксис и довольно легок для обучения.</p>
<p>Но Kotlin - это не просто очередной язык программирования. На сегодняшний день это целая экосистема:</p>
<img src="./pics/1.0.png" alt="Экосистема языка Kotlin" />
<p>Ядро этой экосистемы - <span class="b">Common Kotlin</span>, которое включает в себя собственно язык, основные библиотеки и базовые инструменты для построения программ.</p>
<p>Для взаимодействия с конкретной платформой имеются предназначенные для этой платформы версия Kotlin: <span class="b">Kotlin/JVM</span>, 
<span class="b">Kotlin/JS</span> и <span class="b">Kotlin/Native</span>. Эти специфические версии представляют расширения для языка Kotlin, 
а также специфичные для конкретной платформы бибилиотеки и инструменты разработки.</p>
<p>В будущем вся эта экосистема будет объединена в единую платформу <span class="b">Kotlin Multiplatform</span>, которая на данный момент находится в альфа-версии.</p>
<p>Также стоит отметить, что Kotin развивается как opensource, исходный код проекта можно посмотреть в репозитории на github по адресу 
<a href="https://github.com/JetBrains/kotlin/" rel="nofollow">https://github.com/JetBrains/kotlin/</a>.</p>

<h3>Первая программа на Kotlin</h3>
<p>Создадим первую программу на языке Kotlin. Что для этого необходимо? Для набора кода программы понадобится текстовый редактор. Это может быть любой тестовый редактор, 
например, Notepad++ или Visual Studio Code. И для компиляции программы необходим компилятор.</p>
<p>Кроме того, необходимо установить JDK (Java Development Kit). Загрузить пакеты JDK для установки можно с сайта компании Oracle - 
...
<p>Загрузить компилятор непосредственно для самого языка Kotlin можно по адресу <a href="https://github.com/JetBrains/kotlin/releases/latest/" rel="nofollow">https://github.com/JetBrains/kotlin/releases/latest/</a>. 
...
Загрузим файл <span class="b">kotlin-compiler-1.5.0.zip</span>:</p>
<img src="./pics/1.9.png" alt="Компилятор Kotlin" />
<p>По выше указанному адресу можно найти архив. Загрузим и распакуем из архива папку <span class="b">kotlinc</span>. В распакованном архиве в папке bin мы можем найти утилиту <span class="b">kotlinc</span>, 
с помощью которой и будет производиться компиляция:</p>
<img src="./pics/1.10.png" alt="Компилятор для языка Kotlin" />
<p>Теперь определим на жестком диске каталог для файлов с исходным кодом. Например, в моем случае каталог будет находиться по пути <span class="b">c:/kotlin</span>. 
В этом каталоге создадим текстовый файл и переименуем его в <span class="b">app.kt</span>. Расширение <span class="b">kt</span> - это расширение файлов на языке Kotlin.</p>
<p>Далее определим в этом файле код, который будет выводить некоторое сообщение на консоль:</p>
<pre class="brush:kt;">
fun main(){
...
<img src="./pics/1.11.png" alt="Первая программа на Kotlin в текстовом редакторе" />
<p>Откроем командную строку. Вначале с помощью команды <span class="b">cd</span> перейдем к папке, где находится файл <span class="b">app.kt</span>. 
Затем для компиляции программы введем следующую команду:</p>
<div class="console">
<pre class="consoletext">
c:\kotlinc\bin\kotlinc app.kt -include-runtime -d app.jar
</pre>
</div>
<p>В данном случае мы передаем компилятору <span class="b">c:\kotlin\bin\kotlinc</span> для компиляции файл app.kt. (Чтобы не писать полный путь к компилятору, 
путь к нему можно добавить в переменную PATH в переменных среды). Далее с помощью параметра <code>-include-runtime</code> указывается, что создаваемый файл будет включать 
среду Kotlin. А параметр <code>-d</code> указывает, как будет называться создаваемый файл приложения, то есть в данном случае это будет <code>app.jar</code>.</p>
<p>После выполнения этой команды будет создан файл app.jar. Теперь запустим его на выполнение. Для этого введем команду</p>
<div class="console">
<pre class="consoletext">
java -jar app.jar
</pre>
</div>
<p>В данном случае считается, что путь к JDK, установленном на компьютере, прописан в переменной PATH в переменных среды. Иначе вместо "java" 
придется писать полный путь к утилите java.</p>
<p>В итоге при запуске файла мы увидим на консоли строку "Hello Kotlin".</p>
<img src="./pics/1.12.png" alt="Запуск первой программы на Kotlin и JVM" />
