...
<html >
<head>
<title>Kotlin | List</title>
<meta charset="utf-8" />
<meta name="description" content="Коллекция List в Kotlin, MutableList, типы и функции списков, получение элементов списка">
...
<h2>List</h2><div class="date">Последнее обновление: 23.01.2022</div>
...
<p><span class="b">List</span> представляет последовательный список элементов. При этом List представляет неизменяемую (immutable) коллекцию, которая в основном 
только обеспечивает получение элементов по позиции.</p>
<p>Интерфейс List расширяет интерфейс <span class="b">Collection</span>, поэтому перенимает его возможности.</p>
<p>Для создания объекта List применяется метод <span class="b">listOf()</span>:</p>
<pre class="brush:kt;">
var numbers = listOf(1, 2, 3, 4, 5)     // объект List&lt;Int&gt;
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice")   // объект List&lt;String&gt;
</pre>
<p>Списки поддерживают перебор с помощью цикла for, кроме для списка по умолчанию задача реализация toString, которая выводит все элементы списка в 
удобочитаемом виде:</p>
<pre class="brush:kt;">
...
<h4>Методы списков</h4>
<p>Кроме унаследованных методов класс List имеет ряд специфичных. Рассмотрим некоторые из них.</p>
<p>Для получения элемента по индексу можно применять метод <span class="b">get(index)</span>, который возвращает элемент по индексу</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice")
...
<p>Вместо метода get для обращения по индексу можно использовать квадратные скобки <span class="b">[]</span>:</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice")
val first = people[0]
val second = people[1]
...
<p>Однако, если индекс выходит за границы списка, то при использовании метода  <code>get()</code> и квадратных скобок генерируется исключение. 
Чтобы избежать подобной ситуации, можно применять метод <span class="b">getOrNull()</span>, который возвращает null, если индекс находится вне границ списка:</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice")
...
<p>Либо в качестве альтернативы можно применять метод <span class="b">getOrElse()</span>:</p>
<pre class="brush:kt;">
getOrElse(index: Int, defaultValue: (Int) -&gt; T): T
</pre>
<p>Первый параметр представляет индекс, а второй параметр - функция, которая получает запрошенный индекс и возвращает значение, которое возвращается, если индекс выходит 
за границы списка:</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice")
...
<h4>Получение части списка</h4>
<p>Метод <span class="b">subList()</span> возвращает часть списка и в качестве параметров принимает начальный и конечный индексы извлекаемых элементов:</p>
<pre class="brush:kt;">subList(fromIndex: Int, toIndex: Int): List&lt;E&gt;</pre>
<p>Например, получим подсписок с 1 по 4 индексы:</p>
<pre class="brush:kt;">
val people = listOf("Tom", "Sam", "Kate", "Bob", "Alice", "Mike")
val subPeople = people.subList(1, 4)
println(subPeople)      // [Sam, Kate, Bob]
</pre>
<h3>Изменяемые списки</h3>
<p>Изменяемые списки представлены интерфейсом <span class="b">MutableList</span>. Он расширяет интерфейс List и позволяют добавлять и удалять элементы. 
Данный интерфейс реализуется классом <span class="b">ArrayList</span>.</p>
<p>Для создания изменяемых списков можно использовать ряд методов:</p>
<ul>
<li><p><span class="b">arrayListOf()</span>: создает объект ArrayList</p></li>
<li><p><span class="b">mutableListOf()</span>: создает объект MutableList</p></li>
</ul>
<p>Создание изменяемых списков:</p>
<pre class="brush:kt;">
var numbers : ArrayList&lt;Int&gt; = arrayListOf(1, 2, 3, 4, 5)
var numbers2: MutableList&lt;Int&gt; = mutableListOf(5, 6, 7)
</pre>
<p>Если необходимо добавлять или удалять элементы, то надо использовать методы MutableList:</p>
<ul>
<li><p><span class="b">add(index, element)</span>: добавлят элемент по индексу</p></li>
<li><p><span class="b">add(element)</span>: добавляет элемент</p></li>
<li><p><span class="b">addAll(collection)</span>: добавляет коллекцию элементов</p></li>
<li><p><span class="b">remove(element)</span>: удаляет элемент</p></li>
<li><p><span class="b">removeAt(index)</span>: удаляет элемент по индексу</p></li>
<li><p><span class="b">clear()</span>: удаляет все элементы коллекции</p></li>
</ul>
<pre class="brush:kt;">
fun main() {
