...
<html >
<head>
<title>Kotlin | Перегрузка функций</title>
<meta charset="utf-8" />
<meta name="description" content="Перегрузка функций в языке программирования Kotlin, правила перегрузки, function overloading">
...
<h2>Перегрузка функций</h2><div class="date">Последнее обновление: 25.05.2021</div>
...
<p>Перегрузка функций (function overloading) представляет определение нескольких функций с одним и тем же именем, но с различными параметрами. 
Параметры перегруженных функций могут отличаться по количеству, типу или по порядку в списке параметров.</p>
<pre class="brush:kt;">
...
<p>В данном случае для одной функции <code>sum()</code> определено пять перегруженных версий. Каждая из версий отличается либо по типу, либо количеству, либо по порядку параметров. 
При вызове функции sum компилятор в зависимости от типа и количества параметров сможет выбрать для выполнения нужную версию:</p>
<pre class="brush:kt;">
fun main() {
...
<p>При этом при перегрузке не учитывает возвращаемый результат функции. Например, пусть у нас будут две следующие версии функции sum:</p>
<pre class="brush:kt;">
fun sum(a: Double, b: Int) : Double{
    return a + b
}
...
<p>Они совпадают во всем за исключением возвращаемого типа. Однако в данном случае мы сталкивамся с ошибкой, так как перегруженные версии 
должны отличаться именно по типу, порядку или количеству параметров. Отличие в возвращаемом типе не имеют значения.</p>
...
<div class="nav"><p><a href="./3.4.php">Назад</a><a href="./">Содержание</a><a href="./3.9.php">Вперед</a></p></div></div>
<div class="socBlock">
