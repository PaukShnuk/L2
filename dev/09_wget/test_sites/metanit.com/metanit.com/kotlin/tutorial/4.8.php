...
<h2>Интерфейсы</h2><div class="date">Последнее обновление: 29.05.2021</div>
...
<p>Интерфейсы представляют контракт, который должен реализовать класс. Интерфейсы могут содержать объявления свойств и функций, а также их реализацию 
по умолчанию.</p>
<p>Для определения интерфейса применяется ключевое слово <span class="b">interface</span>. Например:</p>
<pre class="brush:kt;">
interface Movable{
...
<p>Например, в данном случае интерфейс Movable представляет функцонал транспортного средства. Он содержит две функции и одно свойство. 
Функция <code>move()</code> представляет абстрактный метод - она не имеет реализации. Вторая функция <code>stop()</code> имеет реализацию по умолчанию.</p>
<p>При определении свойств в интерфейсе име не присваиваются значения.</p>
<p>Мы не можем напрямую создать объект интерфейса, так как интерфейс не поддерживает конструкторы и просто представляет шаблон, которому класс должен соответствовать.</p>
<p>Определим два класса, которые применяют интерфейс:</p>
<pre class="brush:kt;">
class Car : Movable{
...
<p>Для применения интерфейса после имени класса ставится двоеточие, за которым следует название интерфейса. При применении интерфейса класс должен 
реализовать все его абстрактные методы и свойства, а также может предоставить свою реализацию для тех свойств и методов, которые уже имеют реализацию по умолчанию. 
При реализации функций и свойств перед ними ставится ключевое слово <span class="b">override</span>.</p>
<p>Так, класс <code>Car</code> представляет машину и применяет интерфейс <code>Movable</code>. Так как интерфейс содержит абстрактный метод <code>move()</code>, то 
класс <code>Car</code> обязательно должен его реализовать.</p>
<p>Тоже касается свойства <code>speed</code> - класс Car должен его определить. Здесь реализация свойства заключается в установке для него начального значения.</p>
<p>А вот функцию <code>stop()</code> класс Car может не реализовать, так как она уже содержит реализацию по умолчанию.</p>
<p>Класс Aircraft представляет самолет и тоже применяет интерфейс Movable. При этом класс Aircraft реализует обе функции интерфейса.</p>
<p>В последствии в программе мы можем рассматривать объекты классом Car и Aircraft как объекты Movable:</p>
<pre class="brush:kt;">
fun main() {
...
<p>Рассмотрим еще пример. Определим интерфейс Info, который объявляет ряд свойств:</p>
<pre class="brush:kt;">
interface Info{
    val model: String
        get() = "Undefined"
...
<p>Первое свойство имеет геттер, а это значит, что оно имеет реализацию по умолчанию. При применении интерфейса такое свойство необязательно реализовать. 
Второе свойство - number является абстрактным, оно не имеет ни геттера, ни сеттера, то есть не имеет реализации по умолчанию, поэтому классы его обязаны реализовать.</p>
<p>Для реализации интерфейса возьмем выше определенный класс Car:</p>
<pre class="brush:kt;">
...
<p>Теперь класс Car применяет два интерфейса. Класс может применять несколько интерфейсов, в этом случае они указываются через запятую, и все эти интерфейсы класс должен реализовать. 
Класс Car реализует оба свойства. При этом при реализации свойств в классе необязательно указывать геттер или сеттер. Кроме того, можно реализовать свойства 
в первичном конструкторе, как это сделано в случае со свойствами model и number</p>
...
<p>В Kotlin мы можем наследовать класс и применять интерфейсы. При этом мы можем одновременно и наследоваться от класса, и применять один или несколько интерфейсов. 
Однако что, если переопределяемая функция из базового класса имеет то же имя, что и функция из применяемого интерфейса:</p>
<pre class="brush:kt;">
open class Video {
...
<p>Здесь класс Video и интерфейс AudioPlayable определяют функцию play. В этом случае класс MediaPlayer, который наследуется от Video и применяет 
интерфейс AudioPlayable, обязательно должен определить функцию с тем же именем, то есть play. С помощью конструкции <code>super&lt;имя_типа&gt;.имя_функции</code> 
можно обратиться к опредленной реализации либо из базового класса, либо из интерфейса.</p>
...
<div class="nav"><p><a href="./4.11.php">Назад</a><a href="./">Содержание</a><a href="./4.7.php">Вперед</a></p></div></div>
<div class="socBlock">
