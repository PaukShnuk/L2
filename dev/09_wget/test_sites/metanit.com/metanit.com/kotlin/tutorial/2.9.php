...
<html >
<head>
<title>Kotlin | Структура программы</title>
<meta charset="utf-8" />
<meta name="description" content="Структура программы на языке Kotlin, функция main как точка входа в программу, инструкции и блоки кода, типы комментариев">
...
<h1>Основы языка Kotlin</h1><h2>Структура программы</h2><div class="date">Последнее обновление: 23.05.2021</div>
...
<h3>Функция main</h3>
<p>Точкой входа в программу на языке Kotlin является функция <span class="b">main</span>. Именно с этой функции начинается выполнение программы на Kotlin, поэтому эта функция 
должна быть в любой программе на языке Kotlin.</p>
<p>Так, в прошлой теме была определена следующая функция main:</p>
<pre class="brush:kt;">
fun main(){
    println("Hello Kotlin")
}
</pre>
<p>Определение функции <code>main()</code> (в принципе как и других функций в Kotlin) начинается с ключевого слова <span class="b">fun</span>. По сути оно 
указывает, что дальше идет определение функции. После <code>fun</code> указывается имя функции. В данном случае это <span class="b">main</span>.</p>
<p>После имени функции в скобках идет список параметров функции. Здесь функция main не принимает никаких параметров, поэтому после имени функции идут пустые скобки.</p>
<p>Все действия, которые выполняет функция, заключаются в фигурные скобки. В данном случае единственное, что делает функция main, - вывод на консоль некоторого сообщения с помощью 
другой встроенной функции <span class="b">println()</span>.</p>
<p>Стоит отметить, что до версии 1.3 в Kotlin функция main должна была принимать параметры:</p>
<pre class="brush:kt;">
fun main(args: Array&lt;String&gt;) {
    println("Hello Kotlin")
}
</pre>
<p>Параметр <code>args: Array&lt;String&gt;</code> представляет миссив строк, через который в программу можно передать различные данные.</p>
<p>Начиная с версии 1.3 использовать это определение функции с параметрами необязательно. Хотя мы можем его использовать.</p>
<h3>Инструкции и блоки кода</h3>
<p>Основным строительным блоком программы на языке Kotlin являются <span class="b">инструкции</span> (statement). 
Каждая инструкция выполняет некоторое действие, например, вызовы функций, объявление переменных и присвоение им значений. Например:</p>
<pre class="brush:kt;">
println("Hello Kotlin!");
</pre>
<p>Данная строка представляет встроенной функции <code>println()</code>, которая выводит на консоль, некоторое сообщение 
(в данном случае строку "Hello Kotlin!").</p>
<p>Стоит отметить, что в отличие от других похожих языков программирования, например, Java, в Kotlin не обязательно ставить после инструкции точку запятой. 
Каждая инструкция просто размещается на новой строке:</p>
<pre class="brush:kt;">
fun main(){
...
<p>Тем не менее, если инструкции располагаются на одной строке, то чтобы их отделить друг от друга, надо указывать после инструкции точку с запятой:</p>
<pre class="brush:kt;">
fun main(){
...
<p>В Kotlin есть два типа комментариев: однострочный и многострочный. Однострочный комментарий размещается на одной строке после двойного слеша //. 
