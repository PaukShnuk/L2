...
<html >
<head>
<title>Kotlin | Модификаторы видимости</title>
<meta charset="utf-8" />
<meta name="description" content="Модификаторы видимости в языке программирования Kotlin, public, private, protected, internal">
...
<h2>Модификаторы видимости</h2><div class="date">Последнее обновление: 28.05.2021</div>
...
<p>Все используемые типы, а также компоненты типов (классы, объекты, интерфейсы, конструкторы, функции, свойства) имеют определеннй уровень видимости, 
...
В Kotlin есть следующие модификаторы видимости:</p>
<ul>
<li><p><span class="b">private</span>: классы, объекты, интерфейсы, а также функции и свойства, определенные вне класса, с этим модификатором видны только в том файле, в котором они определены. 
Члены класса с этим модификатором видны только в рамках своего класса</p></li>
<li><p><span class="b">protected</span>: члены класса с этим модификатором видны в классе, в котором они определены, и в классах-наследниках</p></li>
<li><p><span class="b">internal</span>: классы, объекты, интерфейсы, функции, свойства, конструкторы с этим модификатором видны в любой части модуля, в котором они определены. Модуль представляет набор файлов Kotlin, скомпилированных вместе в одну структурную единицу. 
Это может быть модуль IntelliJ IDEA или проект Maven</p></li>
<li><p><span class="b">public</span>: классы, функции, свойства, объекты, интерфейсы с этим модификатором видны в любой части программы. (При этом если функции или классы с этим модификатором 
определены в другом пакете их все равно нужно импортировать)</p></li>
</ul>
<p>Для установки уровня видимости модификатор ставится перед ключевыми словами <code>var/val/fun</code> в самом начале определения свойства или функции.</p>
<p>Если модификатор видимости явным образом не указан, то применяется модификатор <span class="b">public</span>. То есть следующий класс</p>
<pre class="brush:kt;">
class Person(){
...
<p>Будет эквивалентен следующему определению класса:</p>
<pre class="brush:kt;">
class Person(){
...
<p>Если свойства объявляются через первичный конструктор и для них явным образом не указан модификатор видимости:</p>
<pre class="brush:kt;">class Person(val name: String, val age: Int){
	public fun printPerson(){
        println("Name: $name  Age: $age")
    }
}</pre>
<p>То также к таким свойствам автоматически применяется <code>public</code>:</p>
<pre class="brush:kt;">class Person(public val name: String, public val age: Int){
	public fun printPerson(){
        println("Name: $name  Age: $age")
    }
}</pre>
<p>Соответственно мы можем обращаться к подобным компонентам класса в любом месте программы:</p>
<pre class="brush:kt;">
fun main() {
...
<h3>private</h3>
<p>Если же к свойствам и методам применяется модификатор <span class="b">private</span>, то к ним нельзя будет обратиться извне - вне данного класса.</p>
<pre class="brush:kt;">
...
<p>В данном случае класс Person определяет два свойства <code>name</code> (имя человека) и <code>age</code> (возраст человека). 
...
определены с модификатором <code>private</code>, то мы можем к ним обращаться только внутри этого класса. Вне класса обращаться к ним нельзя:</p>
<pre class="brush:kt;">println(tom.name)   // Ошибка! - свойство name - private</pre>
<p>Также в классе определены три функции <code>printPerson()</code>, <code>printAge()</code> и <code>printName()</code>. Последние две функции выводят значения свойств.
А функция printPerson выводит информацию об объекте, вызывая предыдущие две функции.</p>
<p>Однако функции <code>printAge()</code> и <code>printName()</code> определены как приватные, поэтому их можно использовать только внутри класса:</p>
<pre class="brush:kt;">tom.printAge()  // Ошибка! - функция printAge - private</pre>
<h3>protected</h3>
<p>Модификатор <span class="b">protected</span> определяет свойства и функции, которые из вне класса видны только в классах-наследниках:</p>
<pre class="brush:kt;">
fun main() {
...
<p>Здесь в классе Person свойство <code>name</code> определенно как <code>protected</code>, поэтому оно доступно в классе-наследнике Employee (однако вне базового и производного класса - например, в функции main оно 
недоступно). А вот свойство <code>age</code> - приватное, поэтому оно доступно только внутри класса Person.</p>
<p>Также в классе Employee будет доступна функция <code>printPerson()</code>, так как она имеет модификатор protected</code>, а функции <code>printAge()</code> и 
<code>printName()</code> с модификатором <code>private</code> будут недоступны.</p>

<h3>Модификаторы конструкторов</h3>
<p>Конструкторы как первичные, так и вторичные также могут иметь модификаторы. Модификатор указывается перед ключевым словом <span class="b">constructor</span>. 
По умолчанию они имеют модификатор <code>public</code>. Если для первичного конструктора необходимо явным образом установить 
модификатор доступа, то конструктор определяется с помощью ключевого слова <span class="b">constructor</span>:</p>
<pre class="brush:kt;">
fun main() {
...
<p>Стоит отметить, что в данном случае, поскольку конструктор приватный мы не можем его использовать вне класса ни для создания объекта класса в функции main, 
ни при наследовании. Но мы можем использовать такой конструктор в других конструкторах внутри класса:</p>
<pre class="brush:kt;">
fun main() {
...
<p>Здесь вторичный конструктор класса Person, который имеет модификатор <code>protected</code> (то есть доступен в текущем классе и классах-наследниках) 
вызывает первичный конструктор класса Person, который имеет модификатор <code>private</code>.</p>
<h3>Модификаторы объектов и типов верхнего уровня</h3>
<p>Классы, а также переменные и функции, которые определены вне других классов, также могут иметь модификаторы public, private и internal.</p>
<p>Допустим, у нас есть файл <span class="b">base.kt</span>, который определяет одноименный пакет:<p>
<pre class="brush:kt;">
package base
...
<p>Внутри данного файла мы можем использовать его приватные переменные, функции классы. Однако при подключении этого пакета в другие файлы, приватные переменные, 
функции и классы будут недоступны:</p>
<pre class="brush:kt;">
import base.*
...
<p>Однако даже внутри одного файла есть ограничения на использование приватных классов:</p>
<pre class="brush:kt;">
package email
...
<p>Здесь мы столкнемся с ошибкой, так как публичная функция не может принимать параметр приватного класса. И в данном случае нам надо либо сделать класс 
Message публичным, либо функцию send приватной.</p>
...
<div class="nav"><p><a href="./4.9.php">Назад</a><a href="./">Содержание</a><a href="./4.2.php">Вперед</a></p></div></div>
<div class="socBlock">
