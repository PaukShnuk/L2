...
<html >
<head>
<title>Kotlin | Пакеты и импорт</title>
<meta charset="utf-8" />
<meta name="description" content="Пакеты в языке программирования Kotlin, package, подключение пакетов с помощью директивы import">
...
<h2>Пакеты и импорт</h2><div class="date">Последнее обновление: 27.05.2021</div>
...
<p>Пакеты в Kotlin представляют логический блок, который объединяет функционал, например, классы и функции, используемые для решения близких по характеру задач. 
...
поместить в другие пакеты.</p>
<p>Для определения пакета применяется ключевое слово <span class="b">package</span>, после которого идет имя пакета:</p>
<pre class="brush:kt;">package email</pre>
<p>Определение пакета помещается в самое начало файла. И все содержимое файла рассматривается как содержимое этого пакета.</p>
<p>Например, добавим в проект новый файл <span class="b">email.kt</span>:</p>
<img src="./pics/4.1.png" alt="Пакеты в Kotlin" />
<p>И определим в нем следующий код:</p>
<pre class="brush:kt;">
package email
...
<p>Пакет называется "email". Он содержит класс <code>Message</code>, который содежит одно свойство <code>text</code>. Условно говоря, это класс представляет email-сообщение, а свойство text - его текст.</p>
<p>Также в этом пакете определена функция <code>send()</code>, которая условно отправляет сообшение на некоторый адрес.</p>
<p>Допустим, мы хотим использовать функционал этого пакета в другом файле. Для подключения сущностей из пакета необходимо применить директиву 
<span class="b">import</span>. Здесь возможны различные способы подключения функционала из пакета. Можно подключить в целом весь пакет:</p>
<pre class="brush:kt;">import email.*</pre>
<p>После названия пакета ставится точка и звездочка, тем самым импортируются все типы из этого пакета. Например, возьмем другой файл проекта - 
<span class="ii">app.kt</span>, который определяет функцию main, и используем в нем функционал пакета email:</p>
<pre class="brush:kt;">
import email.*
...
<p>Поскольку в начале файла импортированы все типы из пакета email, то мы можем использовать класс Message и функцию send в функции main.</p>
...
<p>Также можно импортировать типы, определенные в пакете, по отдельности:</p>
<pre class="brush:kt;">
import email.send
import email.Message
</pre>
<h3>Псевдонимы</h3>
<p>С помощью оператора <span class="b">as</span> можно определять псевдоним для подключаемого типа и затем обращаться к этому типу через его псевдоним:</p>
<pre class="brush:kt;">
...
<p>Здесь для функции <code>send()</code> определен псевдоним <code>sendEmail</code>. И далее для обращения к этой функции надо использовать ее псевдоним:</p>
<pre class="brush:kt;">sendEmail(myMessage, "tom@gmail.com")</pre>
<p>Также для класса Message определен псевдоним EmailMessage. Соответственно при использовании класса необходимо применять его псевдоним, а не оригинальное имя:</p>
<pre class="brush:kt;">val myMessage = EmailMessage("Hello Kotlin")</pre>
<p>Псевдонимы могут нам особенно пригодится, если у нас импортируются из разных пакетов типы с одним и тем же именем. Например, пусть в проекте есть файл 
<span class="b">sms.kt</span>:</p>
<pre class="brush:kt;">
package sms
...
<p>Здесь определен пакет <code>sms</code> также с классом Message и функцией send для отправке сообшения по sms.</p>
<p>Допустим, в файле <span class="b">app.kt</span> мы одновременно хотим использовать класс Message и функцию send и из файла <span class="b">email.kt</span>, и 
из файла <span class="b">sms.kt</span>:</p>
<pre class="brush:kt;">
import email.send as sendEmail
import email.Message as EmailMessage
...
<h3>Встроенные пакеты</h3>
<p>Kotlin имеет ряд встроенных пакетов, которые подключаюся по умолчанию в любой файл на языке Kotlin:</p>
<ul>
<li><p><code>kotlin.*</code></p></li>
<li><p><code>kotlin.annotation.*</code></p></li>
<li><p><code>kotlin.collections.*</code></p></li>
<li><p><code>kotlin.comparisons.*</code></p></li>
<li><p><code>kotlin.io.*</code></p></li>
<li><p><code>kotlin.ranges.*</code></p></li>
<li><p><code>kotlin.sequences.*</code></p></li>
<li><p><code>kotlin.text.*</code></p></li>
</ul>
<p>Поэтому если возникнет необходимость использовать какие-то типы, определенные в этих пакетах, то явным образом эти пакеты не нужно импортировать.</p>
...
<div class="nav"><p><a href="./4.3.php">Назад</a><a href="./">Содержание</a><a href="./4.9.php">Вперед</a></p></div></div>
<div class="socBlock">
