...
<html >
<head>
<title>Kotlin | Отличие последовательности от коллекций Iterable</title>
<meta charset="utf-8" />
<meta name="description" content="Отличие последовательности Sequence от коллекций Iterable в языке программирования Kotlin, процесс обработки элементов при наборе операций в коллекциях и последовательностях">
...
<h2>Отличие последовательности от коллекций Iterable</h2><div class="date">Последнее обновление: 24.01.2022</div>
...
<p>Здесь создается коллекция - список people (объект типа List), который содержит объекты Person. Далее к этому списку применяются две операции фидльтрации в виде 
метода <code>filter()</code>. Сначала получаем все объекты Person, у которых свойство <code>age</code> больше 30:</p>
<pre class="brush:kt;">people.filter { println("Age filter: ${it}"); it.age &gt; 30 }</pre>
...
<pre class="brush:kt;">filter{ println("Name filter: ${it}"); it.name.length == 3 }</pre>
