...
<html >
<head>
<title>Jetpack Compose | Модификатор Modifier.toggleable</title>
<meta charset="utf-8" />
<meta name="description" content="Модификатор Modifier.toggleable и его параметры в Jetpack Compose на языке программирования Kotlin, создание переключаемого компонента checkbox">
...
<h2>Modifier.toggleable</h2><div class="date">Последнее обновление: 06.10.2021</div>
...
<p>Модификатор <span class="b">Modifier.toggleable</span> устанавливает для компонента два состояния и позволяет переключаться между этими состояниями. 
Подобные компоненты в этом плане похожи на флажки (checkbox), в которых можно поставить отметку, а можно ее снять, тем самым изменив состояние флажка. 
Функция модификатора принимает следующие параметры:</p>
<pre class="brush:kt;">
...
<ul>
<li><p><code>value</code> хранит состояние компонента в виде объекта <code>Boolean</code>, поэтому состояние может принимать только два значения: <code>true</code> и <code>false</code></p></li>
...
<li><p><code>onValueChange</code> представляет функцию типа <code>(Boolean) -&gt; Unit</code>, которая вызывается при нажатии на компонент</p></li>
</ul>
<p>Создадим переключаемый компонент на основе компонента Text:</p>
<pre class="brush:kt;">
...
<p>Для отслеживания состояния создается переменная <code>checked</code>, которая по умолчанию хранит значение <code>false</code>. Это значение передается в 
функцию модификатора <code>toggleable</code> параметру <code>value</code>. А с помощью другого параметра - <code>onValueChange</code> задаем функцию, которая получает новое состояние через 
параметр <code>it</code> и передает его переменной <code>checked</code>. Таким образом, при нажатии состояние компонента будет переключаться.</p>
<img src="./pics/4.27.png" alt="Modifier.toggleable и переключение компонента в Jetpack Compose и Kotlin в Android" />
...
<div class="nav"><p><a href="./4.3.php">Назад</a><a href="./">Содержание</a><a href="./4.4.php">Вперед</a></p></div></div>
<div class="socBlock">
