...
<html >
<head>
<title>Jetpack Compose | Что такое модификаторы</title>
<meta charset="utf-8" />
<meta name="description" content="Модификаторы и визуальный интерфейс в Jetpack Compose и Kotlin и Android, объект Modifier, его функции, параметр modifier, изменение внешнего вида компонентов">
...
<h1>Модификаторы и визуальный интерфейс</h1><h2>Что такое модификаторы</h2><div class="date">Последнее обновление: 27.09.2021</div>
...
<p>Для настройки внешнего вида большинства встроенных компонентов в Jetpack Compose применяются так называемые <span class="b">модификаторы</span>. 
Модификаторы представляют функции, которые задают какой-то отдельный аспект для компонентов (или иными словами "модифицируют" внешний вид компонента), например, установка размеров компонента или его фонового цвета и т.д.</p>
<p>Большинство встронных компонентов поддерживают применение модификаторов через параметр <span class="b">modifier</span>. Например, 
возьмем ранее встречавшийся простой компонент <span class="b">Text</span>, который выводит некоторый текст. Данный компонент, как и другие компоненты 
в Jetpack Compose, определен в виде функции с аннотацией <span class="b">@Compose</span>, которая имеет ряд параметров:</p>
<pre class="brush:kt;">
@Composable
fun Text(
    value: String,
...
<p>Из определения функции компонента мы видим, что она имеет параметр <code>modifier</code>, который представляет тип (а если точнее интерфейс) 
<span class="b">Modifier</span>. И этот тип предоставляет огромное количество встроенных функций-модификаторов, изменяющих отдельные аспекты компонентов. 
Далее отдельно рассмотрим отдельные группы этих функций. А пока посмотрим на примере компонента <span class="b">Text</span>, как применять модификаторы.</p>
<pre class="brush:kt;">
...
<img src="./pics/3.1.png" alt="Modifier и модификаторы в Jetpack Compose и Kotlin в Android" />
<p>Здесь для компонента Text задается три параметра. Первый параметр представляет отображаемый текст. Второй параметр - <code>fontSize</code> задает 
размер шрифта - 22 единицы. Третий параметр задает модификаторы, которые применяются к компоненту:</p>
<pre class="brush:kt;">modifier = Modifier.background(Color.Yellow)</pre>
<p>В данном случае применяется только один модификатор - функция <span class="b">background()</span>, которая устанавливает фоновый цвет. В нее передается определение 
цвета - здесь желтый цвет, предоставленный встроенным значением <code>Color.Yellow</code>.</p>
<p>Стоит учитывать, что функции модификаторы, как и используемые ими значения, необходимо импортировать:</p>
<pre class="brush:kt;">
import androidx.compose.foundation.background
import androidx.compose.ui.Modifier
import androidx.compose.ui.graphics.Color
</pre>
<h3>Применение нескольких модификаторов</h3>
<p>К компоненту можно применять множество модификаторов по цепочке:</p>
<pre class="brush:kt;">Modifier.модификатор1().модификатор2().модификатор3()...модификаторN()</pre>
<p>Например, применим к компоненту Text несколько модификаторов:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь к компоненту <code>Text</code> применяются три функции-модификатора. Первая задает фоновый цвет:</p>
<pre class="brush:kt;">Modifier.background(Color.Yellow)</pre>
<p>Второй модификатор устанавливает размер, а точнее растягивает компонент по всей длине контейнера:</p>
<pre class="brush:kt;">fillMaxSize()</pre>
<p>Третий модификатор устанавливает отступы (30 единиц) от границ контейнера:</p>
<pre class="brush:kt;">padding(30.dp)</pre>
<img src="./pics/3.2.png" alt="Установка модификаторов для компонентов в Jetpack Compose и Kotlin в Android" />
...
<div class="nav"><p><a href="./1.3.php">Назад</a><a href="./">Содержание</a><a href="./3.2.php">Вперед</a></p></div></div>
<div class="socBlock">
