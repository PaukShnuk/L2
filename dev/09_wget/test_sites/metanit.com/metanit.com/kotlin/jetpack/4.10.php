...
<html >
<head>
<title>Jetpack Compose | Панели приложения TopAppBar и BottomAppBar</title>
<meta charset="utf-8" />
<meta name="description" content="Панели инстументов приложения TopAppBar и BottomAppBar в Jetpack Compose на языке программирования Kotlin в Android, IconButton и Spacer">
...
<h2>Панели приложения TopAppBar и BottomAppBar</h2><div class="date">Последнее обновление: 20.10.2021</div>
...
<p>По умолчанию Jetpack Compose предоставляет два компонента для создания верхней и нижней панели в приложении.</p>
<h3>TopAppBar</h3>
<p>Компонент <span class="b">TopAppBar</span> позволяет определить верхнюю панель. Он имеет следующие параметры:</p>
<pre class="brush:kt;">
@Composable
fun TopAppBar(
...
<ul>
<li><p><code>modifier</code>: представляет объект <span class="b">Modifier</span>, который определяет модификаторы панели</p></li>
<li><p><code>backgroundColor</code>: значение типа <code>Color</code>, которое задает фоновый цвет панели. По умолчанию равно <code>MaterialTheme.colors.primarySurface</code></p></li>
<li><p><code>contentColor</code>: значение типа <code>Color</code>, которое устанавливает предпочтительный цвет для содержимого панели. 
По умолчанию предоставляет вызов <code>contentColorFor(backgroundColor)</code>, которое для установки использует значение параметра <code>backgroundColor</code></p></li>
<li><p><code>elevation</code>: объект типа <span class="b">ButtonElevation?</span>, который определяет анимацию для содержимого панели. По умолчанию равно <code>AppBarDefaults.TopAppBarElevation</code></p></li> 	
<li><p><code>contentPadding</code>: объект типа <span class="b">PaddingValues</span>, который устанавливает отступы между границами панели и ее содержимым. По умолчанию равно <code>AppBarDefaults.ContentPadding</code></p></li> 	
<li><p><code>content</code>: задает контент панели. По умолчанию содержимое располагается в виде строки с помощью контейнера 
<code>Row</code>. Часто в роли содержимого выступают кнопки с иконками, например, в виде объектов IconButton.</p></li> 	
</ul>
<p>Определим простейшую панель TopAppBar:</p>
<pre class="brush:kt;">
...
<img src="./pics/4.36.png" alt="TopAppBar в Jetpack Compose и Kotlin в Android" />
<p>Нередко элементы панели инстументов распределяются по всей ширине панели таким образом, что первая прижата к одному краю, а последняя - другому краю панели, 
даже если между ними довольно много пространства. Чтобы создать подобный эффект, можно использовать компонент <span class="b">Spacer</span>, растянув 
его на все оставшееся пространство панели:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/4.37.png" alt="Spacer в TopAppBar в Jetpack Compose и Kotlin в Android" />
<h3>BottomAppBar</h3>
<p>Компонент <span class="b">BottomAppBar</span> позволяет определить нижнюю панель. Он имеет следующие параметры:</p>
<pre class="brush:kt;">
@Composable
fun BottomAppBar(
    modifier: Modifier = Modifier,
...
<li><p><code>contentColor</code>: значение типа <code>Color</code>, которое устанавливает предпочтительный цвет для содержимого панели. 
По умолчанию предоставляет вызов <code>contentColorFor(backgroundColor)</code>, которое для установки использует значение параметра <code>backgroundColor</code></p></li>
<li><p><code>cutoutShape</code>: представляет объект типа <span class="b">Shape</span>, который задает форму.</p></li> 	
<li><p><code>elevation</code>: объект типа <span class="b">ButtonElevation?</span>, который определяет анимацию для содержимого панели. По умолчанию равно <code>AppBarDefaults.BottomAppBarElevation</code></p></li> 	
<li><p><code>contentPadding</code>: объект типа <span class="b">PaddingValues</span>, который устанавливает отступы между границами панели и ее содержимым. По умолчанию равно <code>AppBarDefaults.ContentPadding</code></p></li> 	
<li><p><code>content</code>: задает контент панели. По умолчанию содержимое располагается в виде строки с помощью контейнера 
<code>Row</code>.</p></li> 	
</ul>
<p>BottomAppBar применяется похожим образом, что и TopAppBar:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/4.38.png" alt="BottomAppBar в Jetpack Compose и Kotlin в Android" />
<p>В данном случае для визуального эффекта и имитации прикрепления панели BottomAppBar к нижней кромке приложения данная панель обернута в строку Row, растянутую по всей высоте с выравниванием контента 
по низу. Однако Jetpack Compose также предоставляет компоненты, которые позволяют автоматически разместить верхнюю и нижнюю панель соответственно вверху и внизу окна приложения и которые мы далее рассмотрим.</p>
