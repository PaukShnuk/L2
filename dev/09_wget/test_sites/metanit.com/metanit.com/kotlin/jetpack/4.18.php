...
<html >
<head>
<title>Jetpack Compose | Индикаторы прогресса</title>
<meta charset="utf-8" />
<meta name="description" content="Индикаторы прогресса CircularProgressIndicator и LinearProgressIndicator в Jetpack Compose на языке программирования Kotlin в Android, настройка цвета, бесконечная индикация">
...
<h2>Индикаторы прогресса</h2><div class="date">Последнее обновление: 26.10.2021</div>
...
<h3>CircularProgressIndicator</h3>
<p>Компонент <span class="b">CircularProgressIndicator</span> представляет круговой индикатор процесса. Он имеет следующее определение:</p>
<pre class="brush:kt;">
@Composable
...
<ul>
<li><p><code>progress</code>: значение индикатора в виде объекта Float. Это значение находится в диапазаоне от 0.0 (процесс еще не начался) 
до 1.0 (завершение процесса).</p></li> 	
<li><p><code>modifier</code>: представляет объект <span class="b">Modifier</span>, который определяет модификаторы компонента</p></li>  	
<li><p><code>color</code>: цвет индикатора</p></li> 	
<li><p><code>strokeWidth</code>: ширина индикатора</p></li> 		
</ul>
<p>Определим индикацию процесса с помощью CircularProgressIndicator:</p>
<pre class="brush:kt;">
...
<p>В данном случае значение компонента CircularProgressIndicator привязано к переменной <code>progress</code>. Для имитации некоторого процесса с помощью 
функции <span class="b">rememberCoroutineScope</span> определяем контекст корутины и по нажатию на кнопку с его с помощью создаем и заускаем корутину. В 
...
одну секунду с помощью вызова <code>delay()</code>:</p>
<pre class="brush:kt;">
onClick = {
	scope.launch {
		while (progress &lt; 1f) {
...
<img src="./pics/4.58.png" alt="CircularProgressIndicator в Jetpack Compose и Kotlin в Android" />
<p>Также компонент имеет еще одну версию:</p>
<pre class="brush:kt;">
@Composable
fun CircularProgressIndicator(
    modifier: Modifier = Modifier,
...
<p>Эта версия предоставляет бесконечно прокручиваемый индикатор процессе.</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<h3>LinearProgressIndicator</h3>
<p>Компонент <span class="b">LinearProgressIndicator</span> представляет линейный индикатор процесса. Он имеет следующее определение:</p>
<pre class="brush:kt;">
@Composable
...
до 1.0 (завершение процесса).</p></li> 	
<li><p><code>modifier</code>: представляет объект <span class="b">Modifier</span>, который определяет модификаторы компонента</p></li>  	
<li><p><code>color</code>: цвет закрашенной части индикатора (пройденная часть)</p></li> 	
<li><p><code>backgroundColor</code>: цвет незакрашенной части индикатора (еще непройденная часть)</p></li> 	
</ul>
<p>Определим индикацию процесса с помощью LinearProgressIndicator:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь параметр progress компонента LinearProgressIndicator привязан к значению переменной <code>progress</code>. 
Для имитации некоторого процесса с помощью функции <span class="b">rememberCoroutineScope</span> определяем контекст корутины и по нажатию на кнопку с его с помощью создаем и заускаем корутину. В 
этой корутине увеличиваем значение прогресса progress на 0.1f пока не дойдем до значения 1.0. Для имитации долговременной работы применяется задержка на 
одну секунду с помощью вызова <code>delay()</code>.</p>
<img src="./pics/4.59.png" alt="LinearProgressIndicator в Jetpack Compose и Kotlin в Android" />
<p>Параметры color и backgroundColor позволяют настроить цветовую схему индикатора:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/4.60.png" alt="индикация процесса и LinearProgressIndicator в Jetpack Compose и Kotlin в Android" />
<p>Также компонент имеет еще одну версию:</p>
<pre class="brush:kt;">
@Composable
fun LinearProgressIndicator(
...
<p>Эта версия предоставляет бесконечно прокручиваемый индикатор процессе. Пример ее применения:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<div class="nav"><p><a href="./4.17.php">Назад</a><a href="./">Содержание</a><a href="./7.1.php">Вперед</a></p></div></div>
<div class="socBlock">
