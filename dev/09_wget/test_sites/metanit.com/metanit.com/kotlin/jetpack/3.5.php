...
<html >
<head>
<title>Jetpack Compose | Создание прокрутки</title>
<meta charset="utf-8" />
<meta name="description" content="Создание прокрутки компонентов с помощью модификаторов horizontalScroll и verticalScroll в Jetpack Compose и Kotlin и Android, тип rememberScrollState">
...
<h2>Создание прокрутки</h2><div class="date">Последнее обновление: 28.09.2021</div>
...
<p>Вполне может оказаться, содержимое какого-либо компонента больше размера контейнера. В этом случае очевидным решением будет создание прокрутки. 
Однако по умолчанию она отстутсвует, ее необходимо добавлять вручную. Рассмотрим, как это сделать</p>
<h3>Прокрутка по горизонтали</h3>
<p>Для добавления прокрутки содержимого по горизонтали применяется модификатор <span class="b">Modifier.horizontalScroll</span>:</p>
<pre class="brush:kt;">
...
<p>Параметры модификатора:</p>
<ul>
<li><p><code>state</code>: представляет объект <span class="b">ScrollState</span> и описывает состояние полосы прокрутки</p></li> 	
<li><p><code>enabled</code>: значение типа <code>Boolean</code>, которое указывает, будет ли прокрутка доступна. По умолчанию имеет значение  <code>true</code> 
<li><p><code>flingBehavior</code>: представляет объект <span class="b">FlingBehavior</span> и описывает поведение при завершении прокрутки. 
По умолчанию имеет значение <code>ScrollableDefaults.flingBehavior</code></p></li>
<li><p><code>reverseScrolling</code>: устанавливает направление. При значении <code>true</code> прокрутка идет в обратном направлении. По умолчанию имеет значение <code>false</code></p></li>
</ul>
<p>Применим горизонтальную прокрутку к компоненту Text:</p>
<pre class="brush:kt;">
...
<p>Здесь в модификатор <code>.horizontalScroll(ScrollState(0))</code> передается объект <span class="b">ScrollState</span>, которые инициализирован числом 
0 - это начальное положение полосы прокрутки, а именно 0 пикселей. Соответственно, если мы хотим, чтобы скролл устанавливался на каком-то другом положении, 
то можно передать другое значение.</p>
<h3>Тип rememberScrollState</h3>
<p>Хотя мы можем сами определять состояние полосы покрутки с помощью объекта ScrollState, но Jetpack Compose также предлагает встроенную реализацию в виде 
типа <span class="b">rememberScrollState</span>:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<h3>Прокрутка по вертикали</h3>
<p>Для добавления прокрутки содержимого по вертикали применяется модификатор <span class="b">Modifier.verticalScroll</span>:</p>
<pre class="brush:kt;">
...
<p>Данный модификатор принимает те же значения и работает в приниципе также, как и <code>horizontalScroll</code>, только по вертикали. 
Применим данный модификатор:</p>
<pre class="brush:kt;">
package com.example.helloapp
        ...
        setContent{
            Text(
                "What is Lorem Ipsum?\n" +
                   ...
                   "...............",
                fontSize=22.sp,
                modifier = Modifier.background(color= Color.Yellow).verticalScroll(rememberScrollState())
            )
        }
    }
}
</pre>
<h3>Сочетание прокруток по горизонтали и вертикали</h3>
<p>Применяя оба модификатора - <code>horizontalScroll()</code> и <code>verticalScroll()</code>, можно совместить прокрутку по горизонтали и вертикали:</p>
<pre class="brush:kt;">
Text(
	"What is Lorem Ipsum?\n" +
		"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.\n" +
