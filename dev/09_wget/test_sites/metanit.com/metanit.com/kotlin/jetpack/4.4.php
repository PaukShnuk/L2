...
<html >
<head>
<title>Jetpack Compose | Checkbox</title>
<meta charset="utf-8" />
<meta name="description" content="Флажок Checkbox и его параметры в Jetpack Compose на языке программирования Kotlin в Android, обработка выделения компонента">
...
<h2>Checkbox</h2><div class="date">Последнее обновление: 04.10.2021</div>
...
<p>Компонент <span class="b">Checkbox</span> представляет флажок, который может быть в отмеченном и неотмеченном состоянии. Его функция принимает следующие параметры:</p>
<pre class="brush:kt;">
@Composable
fun Checkbox(
    checked: Boolean,
...
<ul>
<li><p><code>checked</code>: указывает, будет ли отмечен флажок. Представляет значение <code>Boolean</code>. Если равен <code>true</code>, 
то флажок отмечен.</p></li>
<li><p><code>onCheckedChange</code>: представляет функции типа <code>(Boolean) -&gt; Unit</code>, которая выполняется при изменении состояния флажка (установки или снятия отметки). В качестве параметра в функцию передается новое состояние флажка.</p></li>
<li><p><code>modifier</code>: объект <span class="b">Modifier</span>, который устанавливает для флажка модификаторы</p></li>
<li><p><code>enabled</code>: указывает, будет ли доступен флажок. Представляет значение <code>Boolean</code> и по умолчанию равен <code>true</code> (то есть флажок будет доступен).</p></li>	
<li><p><code>interactionSource</code>: объект <span class="b">MutableInteractionSource</span>, который задает поток взаимодействий для флажка. 
По умолчанию равен <code>remember { MutableInteractionSource() }</code>.</p></li>
<li><p><code>colors</code>: объект <span class="b">CheckboxColors</span>, который задает цвета для флажка. 
По умолчанию равен <code>CheckboxDefaults.colors()</code>.</p></li>
</ul>
<p>Простейший флажок:</p>
<pre class="brush:kt;">
...
<p>Здесь надо отметить два момента. Прежде всего для компонента Checkbox необходимо задать как минимум два параметра: <code>checked</code> и <code>onCheckedChange</code>. 
Для этого в примере выше определена переменная <code>checkedState</code>, которая представляет состояние флажка (отмечен или нет). 
Эта переменная представляет объект типа <span class="b">MutableState&lt;Boolean&gt;</span>, который создается функцией 
<span class="b">mutableStateOf()</span>. В эту функцию передается собственно хранимое значение - в данном случае значение <code>true</code>, 
которое затем можно получить с помощью свойства <code>value</code> объекта <code>MutableState&lt;T&gt;</code>. А функция <span class="b">remember</span> позволяет сохранить это значение.</p>
<p>В коде с помощью свойства <code>value</code> мы привязываем значение переменной к свойству checked:</p>
<pre class="brush:kt;">checked = checkedState.value</pre>
<p>А в функции обработки изменения состояния флажка с помощью параметра <code>it</code> передаем в переменную новое состояние флажка (если отмечен - true, если отметка отсутвует - false):</p>
<pre class="brush:kt;">onCheckedChange = { checkedState.value = it }</pre>
<p>Второй момент, который надо отметить, флажок не предоставляет встроенных возможностей по установке текстовой метки. Однако, как правило, при флажке идет некоторый 
текст, который некоторым образом объясняет назначение флажка. Но в Jetpack Compose подобную текстовую метку нам надо устанавливать дополнительно:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/4.18.png" alt="Флажок Checkbox в Jetpack Compose и Kotlin в Android" />
<p>Из параметров следует отметить параметр <code>colors</code>, который представляет тип <code>CheckboxColors</code> и устанавливает цветовую гамму флажка. 
По умолчанию он хранит компонент <code>CheckboxDefaults.colors</code></p>
<pre class="brush:kt;">
@Composable
fun colors(
...
<p>Его параметры:</p>
<ul>
<li><p><code>checkedColor</code>: цвет флажка, когда он находится в отмеченном состоянии</p></li>
<li><p><code>uncheckedColor</code>: цвет флажка, когда он неотмечен</p></li>
<li><p><code>checkmarkColor</code>: цвет отметки флажка</p></li>
<li><p><code>disabledColor</code>: цвет флажка, когда он не доступен для нажатия</p></li>
<li><p><code>disabledIndeterminateColor</code>: цвет флажка типа TriStateCheckbox, когда он не доступен и одновременно находится в состоянии <code>ToggleableState.Indeterminate</code></p></li>	
</ul>
<p>Используем данный параметр для настройки цветов:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/4.19.png" alt="Цвет Checkbox в Jetpack Compose и Kotlin в Android" />
<h3>TriStateCheckbox</h3>
<p>Компонент <span class="b">TriStateCheckbox</span> расширяет Checkbox, добавляя возможность установить флажок в третье - неопределенное состояние</p>
<pre class="brush:kt;">
@Composable
...
<p>Он принимает почти те же параметры, что и Checkbox, за исключением двух параметров. Так, параметр <code>state</code> представляет тип <span class="b">ToggleableState</span> 
и устанавливает состояние флажка с помощью следующих значений:</p>
<ul>
<li><p><code>ToggleableState.Indeterminate</code>: неопределенное состояние</p></li>	
<li><p><code>ToggleableState.Off</code>: отметка снята</p></li>
<li><p><code>ToggleableState.On</code>: флажок отмечен</p></li>
</ul>
<p>Второй отличающийся параметр - <code>onClick</code> задает функцию изменения состояния флажка.</p>
<p>Определим простейший компонент TriStateCheckbox:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/4.20.png" alt="TriStateCheckbox в Jetpack Compose и Kotlin в Android" />
...
<div class="nav"><p><a href="./4.7.php">Назад</a><a href="./">Содержание</a><a href="./4.6.php">Вперед</a></p></div></div>
<div class="socBlock">
