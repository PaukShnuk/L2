...
<html >
<head>
<title>Jetpack Compose | Компонент Image</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с изобажениями и компонент Image в Jetpack Compose на языке программирования Kotlin в Android, ContentScale и масштабирование изобажения">
...
<h1>Работа с изображениями</h1><h2>Компонент Image</h2><div class="date">Последнее обновление: 24.10.2021</div>
...
<p>Для выведения изображений в приложении в Jetpack Compose предназначен компонент <span class="b">Image</span>. Он имеет ряд версий. Первая версия компонента:</p>
<pre class="brush:kt;">
@Composable
fun Image(
    bitmap: ImageBitmap,
...
<ul>
<li><p><code>bitmap</code>: объект типа <code>ImageBitmap</code>, которое собственно представляет изображение для отрисовки</p></li>
<li><p><code>contentDescription</code>: представляет строковое описание для изображения, которое применяется сервисами accessibility в служебных целях</p></li>
<li><p><code>modifier</code>: представляет объект <span class="b">Modifier</span>, который определяет модификаторы компонента</p></li>
<li><p><code>alignment</code>: представляет объект типа <span class="b">Alignment</span>, которое задает выравнивание изображения. Значение по умолчанию - <code>Alignment.Center</code>.</p></li> 	
<li><p><code>contentScale</code>: объект типа <span class="b">ContentScale</span>, который принцип масштабирования изображения. По умолчанию равно <code>ContentScale.Fit</code></p>
<p>Может принимать следующие значения:</p>
<ul>
<li><p><code>ContentScale.Crop</code>: масштабирует изображение с сохранением аспектного отношения (отношение высоты и ширины) таким образом, что ширина и высота оказываются равными или больше сторон контейнера.</p></li>
<li><p><code>ContentScale.FillBounds</code>: неравномерно масштабирует изображение для полного заполнения пространства контейнера.</p></li>
<li><p><code>ContentScale.FillHeight</code>: масштабирует изображение с сохранением аспектного отношения таким образом, что 
высота изображения равна высоте контейнера.</p></li>
<li><p><code>ContentScale.FillWidth</code>: масштабирует изображение с сохранением аспектного отношения таким образом, что 
ширина изображения равна ширине контейнера.</p></li>
<li><p><code>ContentScale.Fit</code>: масштабирует изображение с сохранением аспектного отношения (отношение высоты и ширины) 
таким образом, что ширина и высота оказываются равными или меньше сторон контейнера.</p></li>
<li><p><code>ContentScale.Inside</code>: масштабирует изображение с сохранением аспектного отношения 
(отношение высоты и ширины) таким образом, чтобы вместить изображение внутри контейнера, если ширина и(или) высота изобажения больше ширины и(или) высоты контейнера.</p></li>
<li><p><code>ContentScale.None</code>: масштабирование отсутствует</p></li>
</ul>
<img src="./pics/6.13.png" alt="ContentScale в Image и масштабирование изобажений в Jetpack Compose и Kotlin в Android" />
</li> 	
<li><p><code>alpha</code>: задает прозрачность изображения в виде значения типа <code>Float</code></p></li> 	
<li><p><code>colorFilter</code>: устанавливает применяемые к изображению цветовые фильтры в виде объекта <span class="b">ColorFilter</span></p></li> 	
<li><p><code>filterQuality</code>: задает алгоритм выборки пикселей из изображения. Представляет объект типа <span class="b">FilterQuality</span> и по умолчанию имеет значение <code>FilterQuality.Low</code></p></li> 	
</ul>
<p>Вторая версия Image:</p>
<pre class="brush:kt;">
@Composable
fun Image(
...
<p>Здесь применяются практические те же параметры, кроме первого - <code>imageVector</code>, который устанавливает отображаемый рисунок и 
представляет объект <span class="b">ImageVector</span>.</p>
<p>Третья версия:</p>
<pre class="brush:kt;">
@Composable
fun Image(
...
<p>Здесь опять же меняется только первый параметр, который задает изображение. В данном случае это параметр <code>painter</code>, который представляет 
объект <span class="b">Painter</span>.</p>
<p>Какой бы вариант компонента Image мы не выбрали, в любом случае необходимо установить первый параметр, который задает изображение, и параметр <code>contentDescription</code>.</p>
...
<p>В данном случае для установки изображения применяется параметр <code>painter</code>, который в качестве значения принимает объект <code>ColorPainter</code>. 
...
окрашенный в красный цвет.</p>
<img src="./pics/6.1.png" alt="Image в Jetpack Compose и Kotlin в Android" />
...
<div class="nav"><p><a href="./7.3.php">Назад</a><a href="./">Содержание</a><a href="./6.2.php">Вперед</a></p></div></div>
<div class="socBlock">
