...
<html >
<head>
<title>Jetpack Compose | Векторная графика и ImageVector</title>
<meta charset="utf-8" />
<meta name="description" content="Векторная графика и ImageVector в Jetpack Compose на языке программирования Kotlin в Android, отображение иконок и векторной графики в Image">
...
<h2>Векторная графика и ImageVector</h2><div class="date">Последнее обновление: 24.10.2021</div>
...
<p>Для отображения векторной графики в компоненте Image у этого компонента применяется параметр <code>imageVector</code>, который представляет 
объект класса <span class="b">ImageVector</span>.</p>
<p>Например, встроенные в Jetpack Compose иконки как раз представляют векторную графику, которую мы можем вывести в компоненте Image:</p>
<pre class="brush:kt;">
...
<img src="./pics/6.8.png" alt="ImageVector в Image в Jetpack Compose и Kotlin в Android" />
<h3>Ресурсы векторной графики</h3>
<p>Также мы можем использовать ImageVector для загрузки векторной графики, которая хранится в ресурсах приложения. Для хранения векторной графики, как и вообще изображений, в проекте предназначена папка <span class="b">res/drawable</span>:</p>
<img src="./pics/6.2.png" alt="drawable resorces in Jetpack Compose" />
<p>По умолчанию в этой папке уже имеется ресурсы векторной графики в виде файлов <code>ic_launcher_background.xml</code> и 
<code>ic_launcher_foreground.xml</code>, которые применяются для создания иконок приложения.</p>
<p>Для загрузки ресурса векторной графики и создания из него объекта ImageVector применяется функция <span class="b">ImageVector.vectorResource()</span>, в которую передается идентификатор загружаемого 
ресурса. Например, загрузим имеющийся по умолчанию файл <code>ic_launcher_background.xml</code></p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Для примера определим свой ресурс векторной графики. Для этого в папку <span class="b">res/drawable</span> добавим новый ресурс. Для 
этого нажмем правой кнопкой мыши на папку <span class="b">res/drawable</span> и в появившемся контекстном меню выберем пункт 
<span class="b">New -&gt; Drawable Resource File</span>:</p>
<img src="./pics/6.9.png" alt="Vector Graphics in Android Studio и Jetpack Compose" />
<p>Далее в появившемся окошке в поле <span class="b">File name</span> введем <span class="b">star</span> - это будет название файла:</p>
<img src="./pics/6.10.png" alt="Добавление векторной графики в Android Studio для Jetpack Compose" />
<p>Остальные настройки оставим по умолчанию и нажмем на OK.</p>
<p>После этого в папку <span class="b">res/drawable</span> будет добавлен файл <span class="b">star.xml</span>:</p>
<img src="./pics/6.11.png" alt="Vector Graphics in Android Studio and Jetpack Compose" />
<p>Изменим содержимое этого файла на следующий код:</p>
<pre class="brush:xml;">
...
<p>Я не буду подробно останавливаться на том, как создавать подобную графику, но вкратце данная графика представляет красную звезду. Для простоты можно взять 
какое-нибудь изображение в формате svg, который довольно распространен, и конвертировать его в данный формат с помощью онлайн-конвертеров.</p>
<p>Теперь изменим код в файле <span class="b">MainActivity.kt</sp> для загрузки этой звезды:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/6.12.png" alt="ImageVector и vectorResource и получение векторной графики в Android Studio и Jetpack Compose и Kotlin" />
...
<div class="nav"><p><a href="./6.2.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
