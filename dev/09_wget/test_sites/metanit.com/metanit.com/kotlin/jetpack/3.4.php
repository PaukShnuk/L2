...
<html >
<head>
<title>Jetpack Compose | Установка отступов и смещения</title>
<meta charset="utf-8" />
<meta name="description" content="Установка отступов и смещения у компонентов с помощью модификаторов padding и offset в Jetpack Compose и Kotlin и Android, порядок применения модификаторов">
...
<h2>Установка отступов и смещения</h2><div class="date">Последнее обновление: 28.09.2021</div>
...
<h3>Отступы</h3>
<p>Для установки отступов внутри компонента применяется модификатор <span class="b">padding()</span>, которые имеет несколько вариантов:</p>
<pre class="brush:kt;">
...
<p>В качестве значения отступов применяются единицы <span class="b">dp</span>. Применим данный модификатор:</p>
<pre class="brush:kt;">
...
<p>В данном случае устанавливается отступ в 30 единиц между границами компонента Text и его содержимым.</p>
<img src="./pics/3.7.png" alt="Отступы и модификатор padding в Jetpack Compose и Kotlin в Android" />
<p>Установка отступов по отдельности:</p>
<pre class="brush:kt;">Modifier.padding(start = 30.dp, top=30.dp, bottom = 25.dp)</pre>
<p>Причем необязательно устанавливать отступы от всех четырех сторон, можно задать только те отступы, которые нужны.</p>
<p>Установка отступов по горизонтали и вертикали:</p>
<pre class="brush:kt;">Modifier.padding(horizontal = 25.dp, vertical = 25.dp) </pre>
<h4>PaddingValues</h4>
<p>Тип PaddingValues применяет значения для установки отступов:</p>
<pre class="brush:kt;">
...
<h4>Порядок применения отступов</h4>
<p>При использовании отступов следует учитывать очередность их применения, например:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь компонент Text растягивается по всей длине и ширине экрана. При этом к компоненту применяются два отступа. Но один отступ применяется до 
установки фонового цвета, а другой отступ - после.</p>
<img src="./pics/3.8.png" alt="Порядок применения отступов и модификатор padding в Jetpack Compose и Kotlin в Android" />
<p>В итоге получится, что сначала применяется модификатор <code>padding(20.dp)</code>, который устанавливает отступ в 20 единиц между границами компонента и закрашиваемой областью. 
Затем применяется модификатор <code>padding(30.dp)</code>, который устанавливает отступ в 30 единиц между границами закрашенной области и содержимым - текстом.</p>

<h3>Сдвиг</h3>
<p>Для сдвига содержимого компонента по горизонтали и вертикали применяется модификатор <span class="b">offset</span>. Он имеет следующие версии:</p>
<pre class="brush:kt;">
fun Modifier.offset(x: Dp = 0.dp, y: Dp = 0.dp): Modifier
fun Modifier.offset(offset: Density.() -&gt; IntOffset): Modifier
</pre>
<p>Посмотрим на использование первой версии модификатора:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В данном случае сдвигаем содержимое компонента Text на 30 единиц по горизонтали и на 50 единиц по вертикали.</p>
<img src="./pics/3.9.png" alt="Сдвиг и модификатор offset в Jetpack Compose и Kotlin в Android" />
<p>Как и в случае с отступами следует учитывать порядок применения модификаторов. Так, изменим из предыдущего примера компонент Text следующим образом:</p>
<pre class="brush:kt;">
Text(
	"Hello METANIT.COM",
	fontSize=22.sp,
...
<p>В предыдущем примере сначала применялся фоновый цвет, а лишь затем смещение. Теперь же, наоборот, соответственно мы увидим, что сначала будет производиться 
смешение содержимого компонента, а затем окрашивание фоновым цветом:</p>
<img src="./pics/3.10.png" alt="Порядок смещения компонента и модификатор offset в Jetpack Compose и Kotlin в Android" />
<p>При этом можно передавать не только положительные, но отрицательные значения. В этом случае сдвиг идет в противоположном направлении.</p>
