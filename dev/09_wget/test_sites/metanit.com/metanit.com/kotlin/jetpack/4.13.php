...
<html >
<head>
<title>Jetpack Compose | Выдвижная панель drawer в Scaffold</title>
<meta charset="utf-8" />
<meta name="description" content="Выдвижная панель drawer в Scaffold в Jetpack Compose на языке программирования Kotlin в Android, управление компонентом drawer и DrawerState, настройка drawer">
...
<h2>Выдвижная панель drawer в Scaffold</h2><div class="date">Последнее обновление: 22.10.2021</div>
...
<p>Jetpack Compose позволяет определить для компонента Scaffold панель Drawer и управлять ею. Drawer - это выдвижная панель, которая может отображаться и быть скрыта.</p>
<p>Фактически панель Drawer в Scaffold - это произвольный набор компонентов который помещается в компонент Column с помощью параметра <code>drawerContent</code>:</p>
<pre class="brush:kt;">drawerContent: ColumnScope.() -&gt; Unit = null,</pre>
<p>Для управления выдвижной панелью в Jetpack Compose определен класс <span class="b">DrawerState</span>, который предоставляет ряд свойств и методов. Отметим некоторые:</p>
<ul>
<li><p>Метод <code>close()</code> скрывает выдвижную панель</p></li>
<li><p>Метод <code>open()</code> отобажает выдвижную панель</p></li>
<li><p>Свойство <code>isOpen</code> возвращает <code>true</code>, если выдвижная панель отображается</p></li>
<li><p>Свойство <code>isClosed</code> возвращает <code>true</code>, если выдвижная панель скрыта</p></li>
</ul>
<p>Определим простейшую панель Drawer:</p>
<pre class="brush:kt;">
...
<img src="./pics/4.47.png" alt="Drawer в Scaffold в Jetpack Compose и Kotlin в Android" />
<p>Здесь выдвижная панель предствлена набором из трех компонентов Text:</p>
<pre class="brush:kt;">
drawerContent={
	Text("Пункт меню 1", fontSize = 28.sp)
	Text("Пункт меню 2", fontSize = 28.sp)
...
<p>По умолчанию эта панель скрыта. Чтобы ее открыть, нам надо обратиться к функции <code>open()</code> объекта <span class="b">DrawerState</span>. Внутри Scaffold это состояние 
хранится внутри состояния самого Scaffold - ScaffoldState. Поэтому сначала получаем состояние самого Scaffold с помощью встроенной функции 
<span class="b">rememberScaffoldState()</span>:</p>
<pre class="brush:kt;">val scaffoldState = rememberScaffoldState()</pre>
<p>Далее из состояния Scaffold мы можем обратиться к состоянию drawer и его методам и свойствам:</p>
<pre class="brush:kt;">scaffoldState.drawerState.open()</pre>
<p>Но поскольку <code>drawerState.open()</code> - это suspend-функция, ее необходимо запускать в рамках корутины. И для этого вначале создаем контекст корутины с помощью встроенной функции 
<code>rememberCoroutineScope()</code>:</p>
<pre class="brush:kt;">val scope = rememberCoroutineScope()</pre>
<p>И затем в рамках запущеной корутины запускаем отображение выдвижной панели:</p>
<pre class="brush:kt;">
scope.launch{ scaffoldState.drawerState.open()}
</pre>
<p>Аналогично с помощью функции <code>scaffoldState.drawerState.close()</code> можно программно скрыть панель.</p>
<h3>Взаимодействие с Drawer</h3>
<p>Выше была определена выдвижная панель с тремя условными пунктами меню, представленными компонентами Text. Посмотрим, как мы можем взаимодействовать с этой панелью, 
например, выбирать тот или иной пункт меню:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/4.48.png" alt="Обработка выбора пунктов в Drawer в Scaffold в Jetpack Compose и Kotlin в Android" />
<p>Здесь выбранный пункт хранится в переменной <code>selectedItem</code>. При создании меню к каждому компоненту Text добавляется модификатор 
<code>clickable</code>, в котором обрабатывается нажатие посредством передачи переменной <code>selectedItem</code> текущего элемента. После чего выдвижная панель скрывается.</p>
<h3>Настройка визуального вида drawer</h3>
<p>Scaffold предоставляет ряд дополнительных параметров для управления панелью drawer:</p>
<ul>
<li><p><code>drawerGesturesEnabled</code>: указывает, будет ли для выдвижной панели доступна 
поддержка жестов. По умолчанию равно значению <code>true</code> (поддержка доступна)</p></li>
<li><p><code>drawerShape</code>: объект <span class="b">Shape</span>, который задает форму для выдвижной панели. По умолчанию равно <code>MaterialTheme.shapes.large</code></p></li>
<li><p><code>drawerElevation</code>: задает анимацию для выдвижной панели. По умолчанию равно <code>DrawerDefaults.Elevation</code></p></li>
<li><p><code>drawerBackgroundColor</code>: задает фоновый цвет для выдвижной панели. По умолчанию равно <code>MaterialTheme.colors.surface</code></p></li>
<li><p><code>drawerContentColor</code>: задает цвет содержимого для выдвижной панели. По умолчанию равно вызову функции <code>contentColorFor(drawerBackgroundColor)</code>, который применяет для установки цвета параметр <code>drawerBackgroundColor</code></p></li>
<li><p><code>drawerScrimColor</code>: задает цвет затенения Scaffold, когда панель открыта. 
По умолчанию равно <code>DrawerDefaults.scrimColor</code></p></li>
</ul>
<p>Например, изменение цветовой гаммы:</p>
<pre class="brush:kt;">
package com.example.helloapp
