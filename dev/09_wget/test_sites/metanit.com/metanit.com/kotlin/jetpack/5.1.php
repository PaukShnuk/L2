...
<html >
<head>
<title>Jetpack Compose | Взаимодействие с кодом Kotlin</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие с кодом Kotlin в Jetpack Compose, использование переменных и функций в компонентах">
...
<h1>Взаимодействие с кодом Kotlin и состояние компонентов</h1><h2>Взаимодействие с кодом Kotlin</h2><div class="date">Последнее обновление: 13.10.2021</div>
...
<p>Jetpack Compose предполагает применение декларативного стиля при определении визуального интерфейса и работы с ним. В этом отношении иногда возникает вопрос как определять какие-то свои 
собственные данные в виде переменных или действия в виде функций и использовать эти переменные и функции в компонентах для создания графического интерфейса. Рассмотрим некоторые случаи.</p>
<h3>Использование переменных</h3>
<p>Как и в общем случае переменные можно определять на уровне класса (например, на уровне класса MainActivity), либо на уровне отдельного метода/функции. Однако 
не всегда очевидно, что, в ом чсле можно определять переменные на уровне функций, которые принимают компоненты в качестве параметров. Например, используем пру переменных:</p>
<pre class="brush:kt;">
...
<img src="./pics/5.4.png" alt="Переменные в компонентах в Jetpack Compose и Kotlin" />
<p>Второй параметр функции <span class="b">setContent()</span> представляет функцию типа <code>@Composable () -&gt; Unit</code>, то есть 
функцию, которая принимает в качестве параметра компонент или объект с аннотацией <code>@Composable</code>. Kotlin позволяет определить эту функцию после названия функции setContent 
и внутрь нее передать нуюный компонент:</p>
<pre class="brush:kt;">
setContent {
...
<p>Но также в рамках этой функции мы также можем использовать все определения и инструкции, которые поддерживает язык Kotlin, в том числе определять переменные, как в примере выше. 
А затем эти переменные передать в компонент в качестве значений для его параметров.</p>
<p>Это относится ко всем функциям, которые принимают компоненты в качестве параметров. Причем функция каждого компонента определяет свою область видимости, 
в рамках которой доступны ее переменные. Вложенные компоненты имеют доступ к переменным, определенным в родительских компонентах. Например:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/5.5.png" alt="Переменные в компонентах в Jetpack Compose и Kotlin и Android" />
<p>Здесь иерархию компонентов можно представить следующим образом: <code>setContent -&gt; Column -&gt; Row -&gt; Text</code>. На уровне функции <code>setContent()</code> 
определяется переменная <code>setContentMessage</code>, которая видна во всех вложенных компонентах. На уровне комонента Column определяется 
переменная <code>columnMessage</code>, которая видна на уровне компонентов Row.</p>
<p>Два последних компонента Row определяют переменную <code>rowMessage</code>, которые доступны во вложенных компонентах Text, но недоступны вне соответствующих компонентов Row.</p>
<h3>Условные конструкции</h3>
<p>Подобным образом можно использовать внутри компонентов управляющие конструкции, например, условные конструкции:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь в зависимости от переменной hour в качестве внутренного содержимого установливается определенный компонент.</p>
<h3>Циклы</h3>
<p>Если необходимо создать ряд отднотиных компонентов, то удобно определить различающиеся данные в виде списка, а в цикле по этому списку создать набор компонентов:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь определяем данные в виде списке langs. В столбце пробегаемся по этому списку и создаем для каждого свой компонент Text</p>
<img src="./pics/5.6.png" alt="создание компонентов в цикле в Jetpack Compose и Kotlin и Android" />
<p>Причем в данном случае мы также могли бы применить и встроенную функцию <code>forEach()</code> списка, которая позволяет перебрать все объекты:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Определим функцию и используем ее в комонентах:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>В данном случае функция <code>createMessage()</code> в зависимости от переданного в нее значения возвращает то или иное сообщение, которое затем можно 
применить для установки текста в компоненте Text.</p>
<p>Подобным образом можно было бы определить функцию в виде анонимной функции или лямбда-выражения:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<div class="nav"><p><a href="./2.3.php">Назад</a><a href="./">Содержание</a><a href="./5.2.php">Вперед</a></p></div></div>
<div class="socBlock">
