...
<html >
<head>
<title>Jetpack Compose | Scaffold</title>
<meta charset="utf-8" />
<meta name="description" content="Scaffold в Jetpack Compose на языке программирования Kotlin в Android, установка верхней и нижней панелей TopAppBar и BottomAppBar, floatingActionButton">
...
<h2>Scaffold</h2><div class="date">Последнее обновление: 22.10.2021</div>
...
<p><span class="b">androidx.compose.material.Scaffold</span> представляет сложный компонент, который устанавливает общую структуру страницы. И мы можем встроить в эту структуру 
различные компоненты. В некотором роде <span class="b">Scaffold</span> выступает в роли еще одного компонента-контейнера. Определение функции компонента:</p>
<pre class="brush:kt;">
@Composable
fun Scaffold(
...
<li><p><code>scaffoldState</code>: : представляет объект <span class="b">ScaffoldState</span>, который определяет состояние компонента Scaffold. По умолчанию равен результату вызова 
<code>rememberScaffoldState()</code></p></li>
<li><p><code>topBar</code>: устанавливает верхнюю панель окна</p></li>
<li><p><code>bottomBar</code>: устанавливает нижнюю панель окна</p></li> 
<li><p><code>snackbarHost</code>: представляет компонент, который содержит отображаемые на экран объекты <span class="b">Snackbar</span>. Обычно это 
компонент <span class="b">SnackbarHost</span>.</p></li>
<li><p><code>floatingActionButton</code>: устанавливает кнопку для выполнения некоторого действия. Обычно в качестве подобной кнопки выступает компонент 
<span class="b">FloatingActionButton</span></p></li>
<li><p><code>floatingActionButtonPosition</code>: задает позицию кнопки из параметра <code>floatingActionButton</code>. 
Представляет объект типа <span class="b">FabPosition</span> и по умолчанию равно значению <code>FabPosition.End</code></p></li> 
<li><p><code>isFloatingActionButtonDocked</code>: указывает, будет ли кнопка из параметра <code>floatingActionButton</code> перекрывать нижнюю панель. 
По умолчанию равно <code>false</code></p></li>
<li><p><code>drawerContent</code>: задает содержимое выдвижной панели Drawer</p></li>
<li><p><code>drawerGesturesEnabled</code>: указывает, будет ли для выдвижной панели Drawer доступна 
поддержка жестов. По умолчанию равно значению <code>true</code> (поддержка доступна)</p></li>
<li><p><code>drawerShape</code>: объект <span class="b">Shape</span>, который задает форму для выдвижной панели Drawer. По умолчанию равно <code>MaterialTheme.shapes.large</code></p></li>
<li><p><code>drawerElevation</code>: задает анимацию для выдвижной панели Drawer. По умолчанию равно <code>DrawerDefaults.Elevation</code></p></li>
<li><p><code>drawerBackgroundColor</code>: задает фоновый цвет для компонента Drawer. По умолчанию равно <code>MaterialTheme.colors.surface</code></p></li>
<li><p><code>drawerContentColor</code>: задает цвет содержимого для компонента Drawer. По умолчанию равно вызову функции <code>contentColorFor(drawerBackgroundColor)</code>, который применяет для установки цвета параметр <code>drawerBackgroundColor</code></p></li>
<li><p><code>drawerScrimColor</code>: задает цвет затенения содержимого в компоненте Drawer, когда этот компонент раскрыт. 
По умолчанию равно <code>DrawerDefaults.scrimColor</code></p></li>
<li><p><code>backgroundColor</code>: фоновый цвет компонента Scaffold. По умолчанию равно значению <code>MaterialTheme.colors.background</code></p></li>
<li><p><code>contentColor</code>: цвет содержимого компонента Scaffold. По умолчанию равно вызову функции <code>contentColorFor(backgroundColor)</code>, который применяет для установки цвета параметр <code>backgroundColor</code></p></li>	
<li><p><code>content</code>: содержимое Scaffold. Представляет вызов функции типа <code>(PaddingValues) -&gt; Unit</code></p></li>
</ul>
<p>Определим простейший Scaffold:</p>
<pre class="brush:kt;">
...
<p>В данном случае Scaffold содержит один компонент Text.</p>
<img src="./pics/4.39.png" alt="Scaffold в Jetpack Compose и Kotlin в Android" />
<p>Теперь добавим в Scaffold верхнюю и нижнюю панели:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<img src="./pics/4.40.png" alt="TopAppBar и BottomAppBar и Scaffold в Jetpack Compose и Kotlin в Android" />
<h3>Установка кнопки floatingActionButton</h3>
<p>Компонент Scaffold может включать кнопку, которая устанавливается через параметр <code>floatingActionButton</code> и по нажатию на которую выполняется некоторое действие. 
Обычно это кнопки типа <span class="b">FloatingActionButton</span> или <span class="b">ExtendedFloatingActionButton</span>. Добавим подобную кнопку:</p>
<pre class="brush:kt;">
package com.example.helloapp
...
<p>Здесь параметр <code>floatingActionButton</code> получает кнопку FloatingActionButton, которая переключает значение переменной <code>isAdded</code> с 
true на false и обратно. В зависимости от этого значения устанавливается иконка данной кнопки, а также текст содержимого в Scaffold.</p>
<p>Кроме того, с помощью параметра <code>floatingActionButtonPosition</code> для кнопки установлено позиционирование по центру. Этот параметр принимает одно из двух значений: 
<code>FabPosition.Center</code> (позиционирование по центру) и <code>FabPosition.End</code> (позиционирование по нижнем углу).</p>
<p>И еще один параметр - <code>isFloatingActionButtonDocked</code> задает прикрепление к нижней панели - значение <code>true</code> (при значении <code>false</code> кнопка была бы откреплена и 
располагалась бы выше нижней панели).</p>
<img src="./pics/4.41.png" alt="FloatingActionButton и Scaffold в Jetpack Compose и Kotlin в Android" />
