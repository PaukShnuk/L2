...
<html >
<head>
<title>Jetpack Compose | Обработка нажатий</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка нажатий компонентов с помощью модификатора Modifier.clickable в Jetpack Compose и Kotlin и Android">
...
<h2>Обработка нажатий</h2><div class="date">Последнее обновление: 03.10.2021</div>
...
<p>Объект <span class="b">Modifier</span> позволяет добавить компонентам возможность обработки нажатия. Для этого он предоставляет функцию <span class="b">clickable()</span>:</p>
<pre class="brush:kt;">
...
<ul>
<li><p><code>enabled</code>: значение типа <code>Boolean</code>, которое указывает, будет ли доступен компонент для нажатия. По умолчанию имеет значение 
<code>true</code>, то есть компонент будет доступен для нажатия. При значении <code>false</code> обработка нажатий оключена.</p></li>
<li><p><code>onClickLabel</code>: предоставляет значение типа <code>String?</code> и задает метку, с помощью которой можно быстро обратиться к компоненту. 
По умолчанию равен <code>null</code></p></li>
<li><p><code>role</code>: объект типа <span class="b">Role?</span>, который устанавливает тип визуального элемента. По умолчанию равен <code>null</code></p></li>	
<li><p><code>onClick</code>: функция типа <code>() -&gt; Unit</code>, которая собственно и обрабатывает нажатие.</p></li>
</ul>
<p>Например, определим простейшую обработку нажатия для компонента <span class="b">Text</span>:</p>
<pre class="brush:kt;">
...
<p>В данном случае мы определяем переменную <code>count</code>, которая будет хранить количество нажатий. 
Однако это не просто число. Она будет предоставлять - объект типа <span class="b">MutableState&lt;Int&gt;</span>, который создается функцией 
...
<p>Обратившись к свойству <code>value</code> мы можем получить значение и вывести его в строке в компоненте Text:</p>
<pre class="brush:kt;">Text("Clicks: ${count.value}"</pre>
<p>А в функцию <code>Modifier.clickable</code> через параметр <code>onClick</code> передаем обработчик нажатия, котором это значение увеличивается на единицу:</p>
<pre class="brush:kt;">onClick = { count.value += 1 }</pre>
<p>В итоге при нажатии на компонент изменится значение в переменной count и соответственно изменится текст компонента Text.</p>
<img src="./pics/2.13.png" alt="Modifier.clickable, onClick и обработка нажатия в Jetpack Compose и Kotlin в Android" />
...
<div class="nav"><p><a href="./3.5.php">Назад</a><a href="./">Содержание</a><a href="./2.1.php">Вперед</a></p></div></div>
<div class="socBlock">
