...
<html >
<head>
<title>Выборка документов MongoDB в PHP</title>
<meta charset="utf-8" />
<meta name="description" content="Выборка документов MongoDB в PHP, использование методов find и findOne, фильтрация выборки, применение регулярных выражений и класса MongoRegex">
...
<h2>Выборка документов</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Также как и при работе в консоли, в php получить документ мы можем с помощью методов <span class="bb">find</span> и <span class="bb">findOne</span>. 
Например, получим единичный объект:</p>
<pre class="brush:php;">
$con = new MongoClient();
...
<p>Результатом опять же будет объект в виде ассоциативного массива:</p>
<img src="pics/3.1.png" alt="Получение документа из MongoDB в PHP" />
<p>Опять же мы можем обратиться к отдельным свойствам и получить их значения:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<p>База данных вернула нам первый попавшийся документ. Но, как и при работе в консоли, мы можем настроить фильтрацию:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<p>Также мы можем искать по значению поля во вложенном объекте:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<p>По тому же принципу работает метод <code>find</code>, который выбирает набор документов из бд. Он возвращает объект класса 
<b>MongoCursor</b>, который позволяет в цикле перебрать все документы выборки:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<p>Метод <code>getNext</code> возвращает следующий документ в выборке $list.</p>
<p>И также мы можем указать условия фильтрации для выборки:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<p>При настройке фильтров выборки мы можем использовать регулярные выражения, которые обогащают возможности поиска. Регулярные выражения 
применяются через специальный класс <b>MongoRegex</b>:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<p>В качестве параметра в конструктор класса <code>MongoRegex</code> передается регулярное выражение. В данном случае идет поиск всех 
документов, значение поля name в которых начинается с "Ma".</p>
