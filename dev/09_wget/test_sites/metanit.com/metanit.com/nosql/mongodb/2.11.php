...
<html >
<head>
<title>БД MongoDB | Установка ссылок</title>
<meta charset="utf-8" />
<meta name="description" content="Установка ссылок в БД MongoDB, ручное и автоматическое связывание по ключу, функциональность DBRef">
...
<h2>Установка ссылок в БД</h2><div class="date">Последнее обновление: 08.08.2021</div>
...
<p>В реляционных базах данных можно устанавливать внешние ключи, когда поля из одной таблицы ссылаются на поля в другой таблице. В MongoDB 
...
которая хранит список компаний, в которых работают пользователи.</p>
<h3>Ручная установка ссылок</h3>
<p>Ручная установка ссылок сводится к присвоению значения поля _id одного документа полю другого документа. Допустим, у нас могут быть коллекции, 
представляющие компании и работников, работающих в этих компаниях. Итак, сначала добавим в коллекцию companies документ представляющий компанию:</p>
<pre class="brush:js;">
&gt; db.companies.insertOne({"_id" : "microsoft", year: 1974})
</pre>
<p>Теперь добавим в коллекцию users документ, который представляет работника. В этом документе будет поле company, представляющее компанию, где работает работник. 
И очень важно, что в качестве значения для этого поля мы устанавливаем не объект company, а значение ключа <code>_id</code> добавленного выше документа:</p>
<pre class="brush:js;">
&gt; db.users.insertOne({name: "Tom", age: 28, company: "microsoft"})
</pre>
<p>Теперь получим документ из коллекции users:</p>
<pre class="brush:js;">
&gt; user = db.users.findOne()
</pre>
<p>В данном случае имеется в виду, что выше добавленный элемент будет единственным в коллекции. Иначе надо сделать выборку по _id последнего добавленого документа.</p>
<p>После этого консоль выводит полученный документ. И теперь найдем ссылку на его компанию в коллекции companies:</p>
<pre class="brush:js;">
&gt; db.companies.findOne({_id: user.company})
</pre>
<p>И если документ с таким идентификатором обнаружен, он отображается на консоли:</p>
<img src="./pics/2.9.png" alt="установка ссылок и внешних ключей MongoDB" />
<h3>Автоматическое связывание</h3>
<p>Используя функциональность <span class="b">DBRef</span>, мы можем установить автоматическое связывание между документами. Посмотрим на примере применение данной 
функциональности. Вначале добавим новый документ в коллекцию companies:</p>
<pre class="brush:js;">
&gt; google = db.companies.insertOne({name : "google", year: 1998})
</pre>
<p>Обратите внимание, что мы получаем результат добавления в переменную <span class="b">google</span>. 
При добавлении нового документа генерирует _id, которыймы можем получить с помощью свойства <code>insertedId</code> результата функции.</p>
<p>Теперь создадим новый документ для коллекции users, у которого ключ company свяжем с только что добавленным документом apple:</p>
<pre class="brush:js;">
&gt; sam = ({name: "Sam", age: 25, company: { "$ref" : "companies", "$id" : google.insertedId}})
&gt; db.users.insertOne(sam)
</pre>
<p>И мы можем протестировать:</p>
<pre class="brush:js;">
&gt; db.companies.findOne({_id: sam.company.$id})
</pre>
<img src="./pics/2.10.png" alt="Установка отношений между коллекциями в mongodb" />
<p>Посмотрев на примере, теперь разберем организацию ссылок между документами. Для связывания с документом google использовалось следующее 
выражение <code>{ "$ref" : "companies", "$id" : google.insertedId}</code>. Формальный синтаксис DBRef следующий:</p>
<pre class="brush:js;">
{ "$ref" : название_коллекции, "$id": значение [, "$db" : название_бд ]}
</pre>
<p>Первый параметр $ref указывает на коллекцию, где хранится связанный документ. Второй параметр указывает на значение, которое и будет представлять 
что-то типа внешнего ключа. Третий необязательный параметр указывает на базу данных.</p>
<p>При тестировании в качестве запроса на выборку указывается выражение <code>_id: sam.company.$id</code>. Так как sam.company 
представляет теперь объект <code>{ "$ref" : "companies", "$id" : google.insertedId}</code>, то нам надо конкретизировать параметр <code>sam.company.<span class="b">$id</span></code></p>
