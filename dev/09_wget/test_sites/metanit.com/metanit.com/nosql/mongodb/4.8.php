...
<html >
<head>
<title>Интерфейс IFindFluent и его методы в MongoDB в C#</title>
<meta charset="utf-8" />
<meta name="description" content="Использование интерфейса IFindFluent и его методов для создания запросов к базе данных MongoDB в C#">
...
<h2>Интерфейс IFindFluent и его методы</h2><div class="date">Последнее обновление: 10.06.2017</div>
...
<p>Метод <code>Find()</code> объекта <code>IMongoCollection</code> возвращает объект <span class="bb">IFindFluent</span>, 
определяющий ряд методов, которые мы можем использовать при получении объектов из БД. Рассмотрим некоторые его основные методы:</p>
<ul>
<li><p><span class="bb">Sort()</span>: задает параметры сортировки</p></li>
<li><p><span class="bb">SortBy()</span>: задает сортировку по возрастанию по определенному свойству</p></li>
<li><p><span class="bb">SortByDescending()</span>: задает сортировку по убыванию</p></li>
<li><p><span class="bb">Skip()</span>: пропускает при выборке определенное количество объектов</p></li>
<li><p><span class="bb">Limit()</span>: устанавливает максимальное количество документов, которое можно получить</p></li>
<li><p><span class="bb">CountAsync()</span>: возвращает число документов в выборке</p></li>
<li><p><span class="bb">FirstAsync()/FirstOrDefaultAsync()</span>: возвращает первый документ в выборке</p></li>
<li><p><span class="bb">Projection()</span>: проекция объектов одних типов на другие</p></li>
</ul>
<p>Отсортируем все документы по возрастанию по полю Age:</p>
<pre class="brush:c#;">
private static async Task SortPeople()
{
...
<p>Соответственно для сортировки по убыванию нужно написать <code>Sort("{Age:-1}")</code>.</p>
<p>Если мы работаем с классами типа Person, то можно использовать методы SortByDescending и SortBy с лямбда-выражениями:</p>
<pre class="brush:c#;">
var collection = database.GetCollection&lt;Person&gt;("people");
//сортировка по убыванию по свойству Name
var result1 = await collection.Find(new BsonDocument()).SortByDescending(e =&gt; e.Name);
//сортировка по возрастанию по свойству Age
var result2 = await collection.Find(new BsonDocument()).SortBy(e =&gt; e.Age);
...
var people = await collection.Find(new BsonDocument()).Sort(sort).ToListAsync();
</pre>
<p>Пропуск и лимитация выборки:</p>
<pre class="brush:c#;">
...
<p>Количество документов выборки:</p>
<pre class="brush:c#;">
var filter = new BsonDocument();
long number = await collection.Find(filter).CountAsync();
</pre>
<p>Если у нас документы имеют сложную структуру, которая нам не очень подходит для вывода, то мы можем использовать проекции:</p>
<pre class="brush:c#;">
private static async Task ProjectPeople()
{
...
<p>Выражение <code>Project("{Name:1, Age:1, _id:0}")</code> указывает, что из всех полей в финальную выборку будут попадать только поля Name и Age, так как 
для ни установлена единица. Все остальные поля не добавляются. Исключение только _id, поэтому если мы не хотим видеть данное поле в выборке, то 
для него нало установить ноль.</p>
<p>Альтернативный способ - применение класса ProjectionDefinitionBuilder:</p>
<pre class="brush:c#;">
...
<p>С помощью методов <code>Include()</code> указываем, какие поля надо включить. Соответственно метод <code>Exclude()</code> исключает 
поле из выборки.</p>
<p>Теперь осуществим выборку объектов Person и проецируем их на какой-нибудь другой тип. Для проекции определим дополнительный класс Employee:</p>
<pre class="brush:c#;">
class Employee
{
...
<p>Теперь при выборке осуществим проекцию из Person в Employee:</p>
<pre class="brush:c#;">
var client = new MongoClient("mongodb://localhost");
var database = client.GetDatabase("test");
var collection = database.GetCollection&lt;Person&gt;("people");
