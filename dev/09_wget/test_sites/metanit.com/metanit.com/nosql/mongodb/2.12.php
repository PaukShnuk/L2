...
<html >
<head>
<title>MongoDB | Работа с индексами</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с индексами в БД MongoDB, метод createIndex, получение индексов коллекции с помощью функции getIndexes и их удаление через dropIndex">
...
<h2>Работа с индексами</h2><div class="date">Последнее обновление: 08.08.2021</div>
...
<p>При поиске документов в небольших коллекциях мы не испытаем особых проблем. Однако когда коллекции содержат миллионы документов, а нам надо 
...
В этом случае нам могут помочь индексы.</p>
<p>Индексы позволяют упорядочить данные по определенному полю, что впоследствии ускорит поиск. Например, если мы в своем приложении или задаче, 
как правило, выполняем поиск по полю <code>name</code>, то мы можем индексировать коллекцию по этому полю:</p>
<pre class="brush:js;">
&gt; db.users.createIndex({"name" : 1})
</pre>
<p>Таким образом с помощью метода <code>createIndex</code> устанавливается индекс по полю name. MongoDB позволяет установить до 64 индексов на одну коллекцию.</p>
<h3>Настройка индексов</h3>
<p>Если мы просто определим индекс для коллекции, например, <code>db.users.createIndex({"name" : 1})</code>, то мы все еще сможем 
добавлять в коллекцию документы с одинаковым значением ключа <code>name</code>. Однако, если нам потребуется, чтобы в коллекцию можно было добавлять 
документ с одним и тем же значением ключа только один раз, мы можем установить флаг <code>unique</code>:</p>
<pre class="brush:js;">
&gt; db.users.createIndex({"name" : 1}, {"unique" : true})
</pre>
<p>Теперь, если мы попытаемся добавить в коллекцию два документа с одним и тем же значением name, то мы получим ошибку.</p>
<p>В тоже время тут есть свои тонкости. Так, документ может не иметь ключа <code>name</code>. В этом случае для добавляемого документа автоматически 
...
ключ name со значением null уже присутствует в коллекции.</p>
<p>Также можно задать уникальный индекс сразу для двух полей:</p>
<pre class="brush:js;">
&gt; db.users.createIndex({"name" : 1, "age" : 1}, {"unique" : true})
</pre>
<p>Однако в этом случае все добавляемые документы должны иметь уникальные значения для обоих полей.</p>
<p>Кроме того, тут есть свои ограничения. Например, значение поля, по которому идет индексация, не должно быть больше 1024 байт.</p>
<h3>Управление индексами</h3>
<p>Все индексы базы данных хранятся в системной коллекции <span class="b">indexes</span>. Для обращения к ней мы можем использовать функцию 
<code>getIndexes</code>, например, чтобы вывести всю информацию об индексах для конкретной коллекции:</p>
<pre class="brush:js;">
&gt; db.users.getIndexes()
</pre>
<p>Данная команда вернет вывод наподобие следующего:</p>
<pre class="brush:js;">
[
	{
		"v" : 1,
...
<p>Как мы видим, здесь для коллекции users (из бд test) определено 2 индекса: id и name. Поле <code>key</code> используется для поиска максимального и минимального значений, 
для различных операций, где надо применять данный индекс. Поле <code>name</code> применяется в качестве идентификатора для операций администрирования, 
например, для удаления индекса:</p>
<pre class="brush:js;">
