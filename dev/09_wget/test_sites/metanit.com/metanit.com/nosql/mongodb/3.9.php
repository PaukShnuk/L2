...
<html >
<head>
<title>Работа с DBRef в PHP</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с DBRef в PHP, ручное и автоматическое связывание документов в MongoDB, метод MongoDBRef::create">
...
<h2>Работа с DBRef</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>В предыдущей главе мы посмотрели, как с помощью DBRef связывать одни документы с другими. В PHP также можно использовать подобную функциональность.</p>
<h3>Ручное связывание</h3>
<p>При ручном связывании в одном документе устанавливается в качестве значения поля id другого документа:</p>
<pre class="brush:php;">
// новая компания
...
<p>После добавления в бд документ $microsoft будет содержать поле _id, по которому мы сможем установить ссылку в поле <code>"company" =&gt; $microsoft['_id']</code></p>
<h3>Автоматическое связывание</h3>
<p>Автоматическое связывание предусматривает применение метода <code>create</code> для создания связи. Этот метод принимает три параметра:</p>
<ul>
<li><p><code>collection</code>: название коллекции, где хранится информация</p></li>
<li><p><code>id</code>: id документа, с которым надо установить связь</p></li>
<li><p><code>database</code>: название базы данных</p></li>
</ul>
<pre class="brush:php;">
// новая компания
