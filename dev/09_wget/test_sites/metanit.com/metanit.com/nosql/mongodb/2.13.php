...
<meta charset="utf-8" />
<meta name="description" content="Управление коллекциями в БД MongoDB, создание и переименование коллекций, capped collection">
...
<h2>Управление коллекцией</h2><div class="date">Последнее обновление: 08.08.2021</div>
...
<p>В предыдущих темах коллекция создавалась неявно автоматически при добавлении в нее первых данных. Но мы также можем создать ее явным образом, 
...
<p>В процессе работы, возможно, потребуется изменить название коллекции. Например, если при первом добавлении данных в ее названии была опечатка. И 
чтобы не удалять и затем пересоздавать коллекцию, следует использовать функцию <code>renameCollection</code>:</p>
<pre class="brush:js;">
...
всегда гарантируется, так как данные могут быть удалены, перемещены, изменены. Поэтому в MongoDB существует понятие <span class="b">ограниченной коллекции</span> 
...
<code>profiles</code> и зададим для нее размер в 9500 байт:</p>
<pre class="brush:js;">
...
<p>Однако при таком способе создания коллекции следует учитывать, что если все место под коллекцию заполнено (например, выделенные нами 
9500 байтов), а количество документов еще не достигло максимума, в данном случае 150, то в этом случае при добавлении нового документа 
самый старый документ будет удаляться, а на его место будет вставляться новый документ.</p>
...
<h3>Подколлекции</h3>
<p>Для упрощения организации данных в коллекциях мы можем использовать подколлекции. Например, данные по коллекции users надо разграничить на 
профили и учетные данные. И мы можем использовать создать коллекции <code>db.users.profiles</code> и <code>db.users.accounts</code>. 
При этом они не будут никак связаны с коллекцией <code>users</code>. То есть в итоге будут три разные коллекции, однако в плане 
логической организации хранения данных, возможно, для кого-то так будет проще.</p>
...
<div class="nav"><p><a href="./2.12.php">Назад</a><a href="./">Содержание</a><a href="./2.5.php">Вперед</a></p></div></div>
<div class="socBlock">
