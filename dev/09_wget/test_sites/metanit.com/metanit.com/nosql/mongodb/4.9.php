...
<html >
<head>
<title>Агрегация и группировка в MongoDB в C#</title>
<meta charset="utf-8" />
<meta name="description" content="Агрегация и группировка результатов выборки из базы данных MongoDB в C#">
...
<h2>Агрегация и группировка</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Метод <code>Aggregate</code> объекта <span class="bb">IMongoCollection</span> позволяет объединять выборки, выполнять группировку и фильтрацию.</p>

<h3>Группировка</h3>
<p>Группировка позволяет сгруппировать документы в выборке по определенному критерию. Для группировки используется метод <code>Group()</code>:</p>
<pre class="brush:c#;">
var people = await collection.Aggregate()
    .Group(new BsonDocument { { "_id", "$Age" }, { "count", new BsonDocument("$sum", 1) } }).ToListAsync();
foreach (BsonDocument p in people)
{
    Console.WriteLine("{0} - {1}", p.GetValue("_id"), p.GetValue("count"));
}
</pre>
<p>Метод <code>Group</code> в качестве параметра принимает BsonDocument, который описывает, как должна проводиться группировка. При этом 
нам надо установить параметр "_id" - ключ группировки. В данном случае для ключа выбрано свойство Age, при этом к свойству добавляется знак 
доллара: "$Age". То есть каждая группа объектов будет иметь одно и то же значение свойства Age.</p>
<p>И также можно установить дополнительные поля для группы. Так, здесь к группе добавляется поле <code>count</code>, которое будет содержать количество 
объектов в группе. Для подсчета количества применяется функция <code>$sum</code></p>
<p>Несмотря на то, что коллекция (в данном случае объект <code>collection</code>) может хранить данные различных моделей, например, Person, но в 
результате группировки мы будем получать объекты <code>BsonDocument</code>, которые представляют каждую отдельную группу. Поэтому, чтобы получить значения 
ключей в группах, используется метод <code>GetValue()</code>.</p>
<h3>Фильтрация</h3>
<p>Для фильтрации объектов используется метод <span class="bb">Match()</span>.</p>
<pre class="brush:c#;">
var people = await collection.Aggregate()
...
<p>Метод Match принимает объект BsonDocument с ключами и их значениями, которым должны соответствовать элементы выборки. 
Также можно сочетать фильтрацию и группировку:</p>
<pre class="brush:c#;">
var people = await collection.Aggregate()
