...
<html >
<head>
<title>Метод BulkWriteAsync в MongoDB в C#</title>
<meta charset="utf-8" />
<meta name="description" content="Использование метода BulkWriteAsync для оптимизации запросов в базе данных MongoDB в C#">
...
<h2>Метод BulkWriteAsync</h2><div class="date">Последнее обновление: 10.06.2017</div>
...
<p>В предыдущих темах было рассмотрено, как использовать различные методы для операций записи, то есть добавления, редактирования, удаления. 
Для каждой операции есть пара методов, которая работает с одним или несколькими документами: <code>UpdateOneAsync() / UpdateManyAsync()</code>, 
<code>InsertOneAsync() / InsertManyAsync()</code> и <code>DeleteOneAsync() / DeleteManyAsync()</code>. Однако для увеличения производительности 
для массовой записи данных мы также можем применять метод <span class="bb">BulkWriteAsync()</span></p>
<pre class="brush:c#;">
using System;
...
<p>Метод <code>BulkWriteAsync()</code> в качестве параметра принимает массив объектов <span class="b">WriteModel</span>, который описывают разные операции. В данном случае в этом массиве только одна операция - 
удаление документа, описываемая с помощью объекта <span class="b">DeleteOneModel</span>. В качестве параметра в конструктор этот объект принимает 
фильтр элементов, которые надо удалить. И также, как и в прошлых темах, фильтр задается либо с помощью BsonDocument, либо с помощью FilterDefinition.</p>
<p>Кроме объекта DeleteOneModel для операций записи можно использовать еще ряд объектов:</p>
<ul>
<li><p><span class="bb">DeleteOneModel / DeleteManyModel</span>: удаление</p></li>
<li><p><span class="bb">UpdateOneModel / UpdateManyModel</span>: обновление</p></li>
<li><p><span class="bb">InsertOneModel / ReplaceOneModel</span>: добавление</p></li>
</ul>
<p>Используем набор операций:</p>
<pre class="brush:c#;">
...
Console.WriteLine("удалено: {0}; добавлено: {1}; обновлено: {2}", 
    result.DeletedCount, result.InsertedCount, result.ModifiedCount);
</pre><p>В качестве результата асинхронной операции записи мы получаем объект <span class="b">BulkWriteResult</span>, у которого можно получить информацию 
о выполненных операциях:</p>
<pre class="brush:c#;">
Console.WriteLine("удалено: {0}; добавлено: {1}; обновлено: {2}", 
    result.DeletedCount, result.InsertedCount, result.ModifiedCount);
</pre>
