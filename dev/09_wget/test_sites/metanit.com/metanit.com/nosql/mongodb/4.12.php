...
<html >
<head>
<title>MongoDB и ASP.NET MVC. Создание проекта</title>
<meta charset="utf-8" />
...
<h2>MongoDB и ASP.NET MVC. Создание проекта</h2><div class="date">Последнее обновление: 11.09.2017</div>
...
<p>Ранее мы рассмотрели по отдельности основные моменты работы MongoDB в C#. Теперь объединим все в одно приложение на ASP.NET MVC 5.</p>
<p>Создадим новый проект ASP.NET MVC 5 с типом аутентификации No Athentication.</p>
<p>Через Nuget установим в проект следующие пакеты для работы с MongoDB:</p>
<ul>
<li><p><span class="b">MongoDB.Driver</span>:</p></li>
<li><p><span class="b">MongoDB.Driver.GridFS</span>:</p></li>
</ul>
<p>Далее в папку <span class="b">Models</span> добавим модель Computer, объекты которой будут храниться в базе данных:</p>
<pre class="brush:c#;">
using MongoDB.Bson;
...
<p>И сразу же определим в файле <i>web.config</i> подключение к базе данных:</p>
<pre class="brush:xml;">
&lt;connectionStrings&gt;
    &lt;add name="MongoDb" connectionString="mongodb://localhost:27017/computerstore" /&gt;
&lt;/connectionStrings&gt;
</pre>
<p>Теперь добавим в папку Models новый класс, через который мы будем выполнять все операции с базой данных. Назовем 
этот класс ComputerContext:</p>
<pre class="brush:c#;">
using System;
using System.Collections.Generic;
using MongoDB.Bson;
...
<p>Также добавим в папку Models две вспомогательные модели, которые помогут в организации приложения:</p>
<pre class="brush:c#;">
...
<p>Класс ComputerFilter поможет в организации фильтрации выборки по модели компьютера и его году выпуска. А класс ComputerList 
будет использоваться как модель представления.</p>
