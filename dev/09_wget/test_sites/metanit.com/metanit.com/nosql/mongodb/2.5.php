...
<html >
<head>
<title>MongoDB | Работа с данными в MongoDB Compass</title>
<meta charset="utf-8" />
<meta name="description" content="Управление коллекциями и документами в БД MongoDB через графический клиент MongoDB Compass">
...
<h2>Работа с данными в MongoDB Compass</h2><div class="date">Последнее обновление: 09.08.2021</div>
...
<p>В предыдущих темах рассматривалась работа с сервером MongoDB через консольную оболочку <span class="b">mongo</span>. Но также мы можем работать с данными через графический клиент 
<span class="b">MongoDB Compass</span>. Данный графический клиент довольно прост и инутивно понятен в использовании, а графическое представление данных для кого-то может быть проще для понимание. Соответственно 
кому-то, возможно, через графический клиент будет проще и удобнее работать.</p>
<p>Для базовых операций, как то: создание/удаление коллекций, добавление, просмотр, изменения и удаления документов есть соответствующие элементы графического интерфейса:</p>
<img src="./pics/2.11.png" alt="Управление базой данных MongoDB Compass" />
<p>При выборе определенной коллекции и отображении ее данных над список данных есть кнопка для добавления данных, в том числе для импорта из внешнего файла. 
А напротив каждого документа есть опции для его просмотра, изменения и удаления:</p>
<img src="./pics/2.16.png" alt="Добавление, изменение и удаление документов в MongoDB Compass" />
<p>Но несмотря на наличие графических возможностей для управления данными, они могут оказаться недостаточными для каких-то более сложных сценариев работы с 
данными. И для этого в MongoDB Compass есть встроенный консольный клиент <span class="b">Mongosh</span>, в котором можно вводить почти все те же команды 
для работы с данными, что и в консольной оболочке mongo:</p>
<img src="./pics/2.17.png" alt="Консоль Mongosh в MongoDB Compass" />
<p>Но также стоит отметить, что консольный клиент в Compass несколько упрощает написание запросов. Прежде всего, мы можем копировать и вставлять запросы в mongosh. Также mongosh 
имеет функцию автодополнения, которая позволяет нам предложить варианты доступых функций:</p>
<img src="./pics/2.18.png" alt="Консольный клиент Mongosh в MongoDB Compass" />
<p>Кроме того, как видно на скриншоте выше, mongosh позволяет нас уведомить об устаревших функциях. Так, выше на скриншоте применялась устаревшая 
функция <code>insert()</code> для добвления в базу данных документа. Хотя mongosh выполнил данную функцию, тем не менее сопроводил ее выполнение 
соответствующими сообщениями. А в консольной оболочке mongo выполнение подобной функции прошло бы без каких-либо проблем.</p>
...
<div class="nav"><p><a href="./2.13.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
