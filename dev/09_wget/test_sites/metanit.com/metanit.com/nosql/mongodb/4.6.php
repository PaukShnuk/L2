...
<html >
<head>
<title>Выборка из базы данных в MongoDB и C#</title>
<meta charset="utf-8" />
<meta name="description" content="Выборка из базы данных в MongoDB и C#, методы Find и FindAsync">
...
<h2>Выборка из базы данных</h2><div class="date">Последнее обновление: 09.06.2017</div>
...
<p>Для получения документов из базы данных мы можем использовать методы <span class="b">Find()</span> и <span class="b">FindAsync</span>, которые определены в интерфейсе IMongoCollection. 
При создании запроса этим методам в качестве параметра передается объект <code>BsonDocument</code>, который определяет критерии запроса.</p>
<p>Метод <code>FindAsync()</code> предоставляет результат в виде объекта <span class="b">IAsyncCursor</span>, который 
представляет собой курсор, осуществляющий перебор объектов.</p>
<p>Метод <code>Find()</code> возвращает объект <span class="b">IFindFluent</span>. Для получения результата выборки из этого объекта можно использовать метод 
<span class="b">ToListAsync()</span>, возвращающий список извлеченных из коллекции документов.</p>
<p>Итак, в прошлой теме в коллекцию people в базу данных test было добавлено несколько элементов. Теперь используем метод FindAsync() для их извлечения:</p>
<pre class="brush:c#;">
class Program
{
...
<p>Если мы хотим выбрать все документы, как в данном случае, то в качестве фильтра определяем пустой BsonDocument:</p>
<pre class="brush:c#;">var filter = new BsonDocument();</pre>
<p>Для создания запросов драйвер C# для MongoDB применяет асинхронный API, и нам надо это учитывать. И так как метод <code>FindAsync()</code> 
возвращает не просто IAsyncCursor, а Task&lt;IAsyncCursor&gt;, то затем нам еще надо его получить:</p>
<pre class="brush:c#;">var cursor = await collection.FindAsync(filter)</pre>
<p>Для перехода к порции данных используется асинхронный метод <code>MoveNextAsync()</code>, возвращающий объект <code>Task&lt;bool&gt;</code>. 
Логическое значение в данном случае будет показывать, есть ли еще данные.</p>
<p>И для получения извлеченной коллекции элементов применяется выражение <code>cursor.Current</code>, которое возвращает объект <code>IEnumerable&ltComputer&gt;</code>.</p>
<p>Используя метод Find, можно сократить код получения данных:</p>
<pre class="brush:c#;">
private static async Task FindDocs()
{
	string connectionString = "mongodb://localhost";
...
<p>Хотя с методом Find получается немного меньше кода, но он не всегда является идеальным решением, так как он загружает все данные из базы данных в память, что 
может быть нежелательно, особенно если в бд хранятся миллионы документов.</p>
<p>Подобным образом можно получать вместо объектов BsonDocument объекты стандартных классов:</p>
<pre class="brush:c#;">
class Program
{
    static void Main(string[] args)
...
<p>И аналогичный пример с использованием метода <span class="b">Find</span>:</p>
<pre class="brush:c#;">
class Program
{
    static void Main(string[] args)
...
<p>В данном случае мы можем использовать коллекцию как набор объектов BsonDocument или Person. Однако все же BsonDocument является более гибким, чем стандратные классы, 
используемые для моделей данных. Например, работая с BsonDocument, мы можем сохранять значения разных полей, определять новые дополнительные поля. В случае же, например, 
объектов Person, если при извлечении данных система столкнется с объектом, который, например, имеет поля, для которых нет соответствующих свойств в классе Person, 
то программа выдаст ошибку о невозможности связать поля документов из бд со свойствами Person.</p>
