...
<html >
<head>
<title>MongoDB | Графический клиент Compass</title>
<meta charset="utf-8" />
<meta name="description" content="Использование графического клиента Compass для работы с базами данных MongoDB">
...
<h2>Графический клиент Compass</h2><div class="date">Последнее обновление: 06.08.2021</div>
...
<p>Для работы с MongoDB также можно использовать официальный графический клиент <span class="b">MongoDB Compass</span>. 
Для его загрузки перейдем по адресу <a href="https://www.mongodb.com/try/download/compass" rel="nofollow">https://www.mongodb.com/try/download/compass</a>. 
На этой странице мы можем выбрать опции для загрузки - версию Compass и целевую операционную систему. Рассмотрим на примере установке на Windows.</p>
<p>Для Windows нам доступны две опции: загрузка в виде установщика msi и загрузка архива zip. В реальности нам достаточно загрузить архив и распаковать его 
в нужном месте. Поэтому выберем эту опцию:</p>
<img src="./pics/compass1.png" alt="Compass in MongoDB" />
<p>После загрузки zip-архива распакуем его и перейдем к распакованной папке.</p>
<img src="./pics/compass2.png" alt="Установка Compass в MongoDB и MongoDBCompass.exe" />
<p>Здесь мы можем найти файл <span class="b">MongoDBCompass.exe</span>. Именно он и представляет исполняемый файл программы. Запустим его:</p>
<img src="./pics/compass3.png" alt="Подключение в Compass к локальному серверу MongoDB" />
<p>По умолчанию нам откроется окно для создания нового подключения. Оно имеет только одно поле, куда надо ввести строку подключения. 
Строка подключения может отличаться в зависимости от места запуска сервера MongoDB (локально на текущем компьютере или где-то на другом сетевом ресурсе), 
используемого логина и пароля и других настроек. В данном случае мы рассмотрим подключение к локально запущеному серверу mongodb, установка которого 
была рассмотрена в прошлой теме.</p>
<p>Для подключения к локальному серверу MongoDB, который по умолчанию запускаетсяна порту 27017, введем <span class="b">mongodb://localhost:27017</span> и 
нажмем на кнопку Connect.</p>
<p>После успешного подключения нам откроется список баз данных, которые есть на сервере:</p>
<img src="./pics/compass5.png" alt="Databases in MongoDB through Compass" />
...
<img src="./pics/compass6.png" alt="Коллекции в БД в MongoDB" />
<p>В данном случае на скриншоте отображается список коллекций базы данных test, созданных в прошлой теме с помощью оболочки mongo.</p>
<p>Нажав на определенную коллекцию, можно увидеть графически все данные, которые есть в коллекции:</p>
<img src="./pics/compass7.png" alt="Данные в коллекции в MongoDB" />
