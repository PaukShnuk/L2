...
<html >
<head>
<title>MongoDB и ASP.NET MVC. Работа с файлами изображений</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с файлами изображений, сохранение и удаление файлов в базе данных MongoDB и GridFS в ASP.NET MVC 5">
...
<h2>MongoDB и ASP.NET MVC. Работа с файлами изображений</h2><div class="date">Последнее обновление: 11.09.2017</div>
...
<p>Добавим в контроллер следующие методы, которые позволят управлять прикреплением файлов изображений:</p>
<pre class="brush:c#;">
...
<p>Действие <code>AttachImage</code> возвращает пользователю представления для прикрепления изображения, а его post-версия добавляет прикрепленное 
изображение в базу данных. Для управления файлами здесь используются методы, которые определены в классе ComputerContext.</p>
<p>Также для вывода изображения в представление определен метод <code>GetImage</code>. Теперь создадим представление 
<span class="ii">AttachImage.cshtml</span> с функционал для прикрепления изображения к определенной модели:</p>
<pre class="brush:xml;">
@model MongoDBApp.Models.Computer

@{
    ViewBag.Title = "Прикрепить изображение";
}
...
@using (Html.BeginForm(null, null, FormMethod.Post, new { enctype = "multipart/form-data" }))
{
    ...
    &lt;div&gt;
        ...
        &lt;div&gt;
            @Html.LabelFor(model =&gt; model.ImageId, "Загрузить изображение")
            &lt;div&gt;
...
<p>В случае, если изображение уже прикреплено к модели, то оно выводится на страницу с помощью хелпера 
<code>Url.Action("GetImage", new { id=@Model.ImageId})"</code>.</p>
<img src="pics/4.8.png" alt="Загрузка изображений в C# и MongoDB" />
...
<div class="nav"><p><a href="4.14.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
