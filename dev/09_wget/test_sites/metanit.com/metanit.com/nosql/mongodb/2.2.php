...
<html >
<head>
<title>MongoDB | Установка и администрирование базы данных</title>
<meta charset="utf-8" />
<meta name="description" content="Установка и администрирование базы данных MongoDB, команды use и db, получение статистики по бд">
...
<h2>Установка и администрирование базы данных</h2><div class="date">Последнее обновление: 07.08.2021</div>
...
<p>В этой и дальнейших статьях мы рассмотрим базовые операции с данными в MongoDB с примением как консольной оболочки <span class="b">mongo</span>, 
так и графического клиента <span class="b">MongoDB Compass</span>. Однако в любом случае при начале работы с сервером следует не забывать запускать сам сервер - то есть приложение <span class="b">mongod</span>.</p>
<p>Начиная работать с MongoDB в консольной оболочке mongo, первым делом надо установить нужную нам базу данных в качестве текущей, 
чтобы затем ее использовать. Для этого надо использовать команду <span class="b">use</span>, после которой идет название базы данных. 
При этом не важно, существует ли такая бд или нет. Если ее нет, то MongoDB автоматически создаст ее при добавлении в нее данных.</p>
<p>Итак, запустим консольную оболочку mongo.exe и введем там следующую команду:</p>
<pre class="brush:js;">
&gt; use test
</pre>
<img src="./pics/2.2.png" alt="Создание базы данных в MongoDB" />
<p>Теперь в качестве текущей будет установлена БД test.</p>
<p>Если вы вдруг не уверены, а существует ли уже база данных с таким названием, то с помощью команды <span class="b">show dbs</span> можно вывести названия 
всех имеющихся бд на консоль:</p>
<img src="./pics/2.3.png" alt="Вывод списка бд в MongoDB" />
<p>Для базы данных можно задать любое имя, однако есть некоторые ограничения. Например, в имени не должно быть символов 
<code>/, \, ., ", *, &lt;, &gt;, :, |, ?, $</code>. Кроме того, имена баз данных ограничены 64 байтами.</p>
<p>Также есть зарезервированные имена, которые нельзя использовать: local, admin, config. Эти имена представляют базы данных, которые уже имеют по умолчанию на сервере и предназначны для служебных целей.</p>
<p>Причем как вы видите, бд test в данном списке нет, так как я в нее еще не добавил данные.</p>
<p>Если мы хотим узнать, какая бд используется в текущей момент, то мы можем воспользоваться командой <span class="b">db</span>:</p>
<pre class="brush:js;">
&gt; db
test
</pre>
<p>Кроме баз данных мы можем просмотреть список всех коллекций в текущей бд с помощью команды <code>show collections</code>.</p>
<h3>Получение статистики</h3>
<p>Используя команду <span class="b">db.stats()</span>, можно получить статистику по текущей базе данных. Например, у нас в качестве текущей установлена 
база данных test:</p>
<img src="./pics/2.12.png" alt="Вывод статистики по бд в MongoDB" />
<p>Похожим образом мы можем узнать всю статистику по отдельной коллекции. Например, узнаем статистику по коллекции users: <code>db.users.stats()</code></p>
