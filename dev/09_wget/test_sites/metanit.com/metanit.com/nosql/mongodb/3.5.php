...
<html >
<head>
<title>Управление выборкой MongoDB в PHP</title>
<meta charset="utf-8" />
<meta name="description" content="Управление выборкой MongoDB в PHP, использование курсоров и класса MongoCursor, ограничение количесва докумнетов и пропуск, сортировка выборки">
...
<h2>Управление выборкой</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Класс MongoCursor имеет ряд методов, которые позволяют управлять результатами выборки и настраивать выборку.</p>
<h4>Подсчет количества документов</h4>
<p>Метод <code>count</code> класса MongoCursor позволяет подсчитать общее количество документов в выборке:</p>
<pre class="brush:php;">
$con = new MongoClient();
...
<h4>Ограничение выборки</h4>
<p>Метод <code>limit</code> позволяет ограничить выборку определенным количеством документов. Например, ограничим вывод двумя документами:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<h4>Пропуск документов</h4>
<p>Метод <code>skip</code> позволяет пропустить определенное количество документов:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<p>Оператор $slice сочетает возможности limit и skip, но при этом его основная деятельность и преимущество - ограничение вывода элементов 
массива, который может быть установлен в качестве значения ключа. Например, в каждом документе может быть массив accounts для хранения номеров банковских 
счетов. Их может быть несколько, но нам для задачи может потребоваться выбрать только один. И в этом нам может помочь $slice.</p>
<p>Оператор $slice принимает два параметра. Первый параметр указывает на общее количество возвращаемых документов. Второй параметр 
необязательный, но если он используется, тогда первый параметр указывает на смещение относительно начала (как функция skip), а второй - на ограничение количества извлекаемых документов.</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<p>Передавая в параметр <code>$slice</code> число 1, мы говорим системе, взять один элемент массива languages с начала массива. Мы также 
могли бы взять первый с конца, передав отрицательное  число: -1. И также мы можем ограничить выборку, передав второй параметр:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<h4>Сортировка</h4>
<p>Также как и в консоли, в PHP, применяя метод <code>sort</code> класса MongoCursor можно настроить сортировку:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<p>Метод sort в качестве параметра принимает поле, по которому надо упорядочить выборку. Если мы хотим, чтобы в выборки все записи упорядочивались по возрастанию относительно поля age, то мы указываем единицу: <code>$param=array("age"=&gt;1);</code>. 
Если мы хотим, чтобы выборка упорядочивалась по убыванию, то указываем -1: <code>array("age"=&gt; -1);</code></p>
