...
<html >
<head>
<title>Сохранение документов в базу данных в MongoDB в C#</title>
<meta charset="utf-8" />
<meta name="description" content="Сохранение документов в базу данных в MongoDB с помощью C#, методы InsertOneAsync и InsertManyAsync">
...
<h2>Сохранение документов в базу данных</h2><div class="date">Последнее обновление: 09.06.2017</div>
...
<p>Для добавления данных в коллекцию используется метод <span class="b">InsertOneAsync</span>, определенный в интерфейсе <span class="bb">IMongoCollection</span>. Например, 
добавим в коллекцию people один документ:</p>
<pre class="brush:c#;">
using System;
...
<p>Кроме метода <code>InsertOneAsync</code> мы также можем использовать для сохранения документов метод <code>InsertManyAsync()</code>, который в качестве 
параметра принимает набор объектов:</p>
<pre class="brush:c#;">
private static async Task SaveDocs()
{
    string connectionString = "mongodb://localhost";
...
<p>Однако мы можем работать не только с объектами BsonDocument, но и со стандартными классами C#. Допустим, нам надо сохранить объекты следующих классов:</p>
<pre class="brush:c#;">
using MongoDB.Bson;
using System.Collections.Generic;
...
<p>Теперь добавим объект Person в коллекцию people:</p>
<pre class="brush:c#;">
private static async Task SaveDocs()
{
    string connectionString = "mongodb://localhost";
...
<P>Так как коллекция будет хранить данные типа Person, то она типизируется данным типом: <code>MongoCollection&lt;Person&gt;</code>. В данном случае 
объекты Person будут выступать в роли документов.</P>
<p>При добавлении, если для объекта не установлен идентификатор "_id", то он автоматически генерируется. И затем мы его можем получить:</p>
<pre class="brush:c#;">
await collection.InsertOneAsync(person1);
