...
<html >
<head>
<title>MongoDB | Добавление данных</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление данных в MongoDB с помощью команды insertOne и insertMany, создание документов, загузка данных из файла">
...
<h2>Добавление данных</h2><div class="date">Последнее обновление: 08.08.2021</div>
...
<p>Установив бд, теперь мы можем добавить в нее данные. Все данные хранятся в бд в формате BSON, который близок к JSON, поэтому нам надо также вводить данные в 
этом формате. И хотя у нас, возможно, на данный момент нет ни одной коллекции, но при добавлении в нее данных она автоматически создается.</p>
...
<p>Документ представляет набор пар ключ-значение. В данном случае добавляемый документ имеет три ключа: name, age, languages, и каждому из них 
сопоставляет определенное значение. Например, ключу languages в качесте значения сопоставляется массив.</p>
...
<p>При добавлении данных, если мы явным образом не предоставили значение для поля "_id" (то есть уникального идентификатора документа), 
то оно генерируется автоматически. Но в принципе мы можем сами установить этот идентификатор при добавлении данных:</p>
<pre class="brush:js;">
...
<p>По умолчанию он выводит все документы коллекции:</p>
<img src="./pics/2.14.png" alt="insertOne in mongodb" />
...
<img src="./pics/2.15.png" alt="insertMany in mongodb" />
...
<h3>Загрузка данных из файла</h3>
<p>Данные для базы данных mongodb можно определять в обычном текстовом файле, что довольно удобно, поскольку мы можем переносить или пересылать этот 
файл независимо от базы данных mongodb. Например, определим где-нибудь на жестком диске файл <span class="b">users.js</span> со следующим содержимым:</p>
<pre class="brush:js;">
db.users.insertMany([
...
<p>То есть здесь с помощью метода insertMany добавляются три документа в коллекцию users.</p>
<p>Для загузки файла в текущую базу данных применяется функция <span class="b">load()</span>, в которую в качестве параметра передается путь к файлу:</p>
<pre class="brush:js;">
&gt; load("D:/users.js")
</pre>
<p>В данном случае предполагается, что файл располагается по пути "D:/users.js".</p>
