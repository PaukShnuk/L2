...
<html >
<head>
<title>Условные операторы MongoDB в PHP</title>
<meta charset="utf-8" />
<meta name="description" content="Условные операторы MongoDB в PHP ($lt, $gt, $slice, $exists), выборка по условию, поиск в массивах">
...
<h2>Условные операторы</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>Также как и в консоли mongo, в php можно использовать условные операторы <code>$lt</code> (меньше чем), <code>$gt</code> (больше чем), 
<code>$lte</code> (меньше или равно) и <code>$gte</code> (больше или равно) для операций сравнения над значениями. Эти операторы аналогичны операциям <code>&lt;, &gt;, &lt;= и &gt;=</code> соответственно.</p>
<p>Найдем документы, в которых значение age меньше 24:</p>
<pre class="brush:php;">
$con = new MongoClient();
...
<p>Соответственно, если бы мы хотели найти документы с age со значением больше, чем 24, то мы могли бы использовать следующее выражение: 
<code>$cond=array("age"=&gt; array('$gt' =&gt; 24))</code></p>
<p>Оператор <code>$ne</code> позволяет найти документы, которые не соответствуют некоторому условию. Например, найдем все документы со значением 
поля age не равным 25:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<h3>Поиск в массивах</h3>
<p>Оператор <code>$in</code> определяет массив значений, одному из которых должно соответствовать значение поля:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<p>Оператор <code>$all</code> определяет массив значений, только теперь поле документа должно иметь все эти значения:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<p>Оператор $or определяет набор пар ключ-значение, которые должны иметься в документе. Если документ имеет хоть одну такую пару 
ключ-значение, то он соответствует данному запросу:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
...
<p>Оператор <code>$exists</code> позволяет проверить наличие в документах ключа. Например, получим все документы, в который есть ключ company:</p>
<pre class="brush:php;">
$con = new MongoClient();
$collection= $con-&gt; test-&gt; persons;
