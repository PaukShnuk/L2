...
<html >
<head>
<title>PHP и MongoDB</title>
<meta charset="utf-8" />
<meta name="description" content="Использование MongoDB в связке с PHP, установка драйвера и настройка инструментария, тестирование подключения к базе данных">
...
<h1>PHP и MongoDB</h1><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>В предыдущей главе был дан обзор основным концепциям и функциональностям MongoDB. Однако в реальности мало кто будет работать через консоль, 
вбивая в базу данных по одному документу. Как правило, база данных будет получать поток данных от внешнего приложения или наоборот возвращать ему эти данные. 
Поэтому нам потребуется организовать работу между MongoDB и приложением, написанным на одном из языков программирования. В качестве языка программирования, 
на примере которого будем рассматривать, подобное взаимодействие, выбран PHP, как один из наиболее распространенных языков.</p>
<h2>Установка драйвера для PHP</h2>
<p>Установим драйвер для языка PHP. Непосредственно сам драйвер можно загрузить со страницы проекта на гитхабе: 
<a href="https://github.com/mongodb/mongo-php-driver" rel="nofollow">https://github.com/mongodb/mongo-php-driver</a>.</p>
<p>После загрузки драйвера и распаковки его пакета на Unix-системах следует ввести следующие команды:</p>
<pre>
phpize
./configure
make
...
<p>И затем добавить в файл php.ini следующую строку:</p>
<pre>extension=mongo.so</pre>
<p>Для установки драйвера на ОС Windows надо загрузить последний пакет со страницы <a href="https://s3.amazonaws.com/drivers.mongodb.org/php/index.html">https://s3.amazonaws.com/drivers.mongodb.org/php/index.html</a>. 
На текущий момент последним является пакет <i>php_mongo-1.4.5.zip</i>.</p>
<p>Итак, загрузим и распакуем этот архивный пакет. В нем будут находиться драйвера для возможных версий php. Надо выбрать ту, которая соответствует вашей версии 
php и положить соответствующий файл драйвера в каталог <i>php/ext</i>, где хранятся все расширения для php.</p>
<p>Затем надо внести изменение в файл <i>php.ini</i>, добавив в него следующую строчку в секцию, где идет подключение расширений:</p>
<pre>
extension=php_mongo-[версия].dll
</pre>
<p>Например, в моем случае c версией php 5.4 Tread Safe я добавил в каталог <i>php/ext</i> файл <i>php_mongo-1.4.5-5.4-vc9.dll</i>. 
И поэтому в php.ini я добавляю <code>extension=php_mongo-1.4.5-5.4-vc9.dll</code></p>
<p>Затем перегружаем веб-сервер и тестируем. Для тестирования подключения драйвера mongodb можно воспользоваться стандартной функцией 
<code>phpinfo()</code>, выводящей всю информацию о php на веб-сервер. В нашем случае она должна вывести следующее:</p>
<img src="./pics/1.5.png" alt="Подключение MongoDB к PHP" />
<h3>Подключение к MongoDB из PHP</h3>
<p>Подключив драйвер к php, теперь мы можем устанавливать соединение с базой данных и взаимодействовать с ней из кода php.</p>
<p>Чтобы установить соединение с бд, достаточно прописать в коде одну строку:</p>
<pre class="brush:php;">
$connection = new Mongo();
</pre>
<p>В данном случае мы подключаемся к серверу, который запущен на localhost на стандартном порту 27017. Однако мы можем подключиться и к внешнему 
серверу:</p>
<pre class="brush:php;">
$connection = new Mongo("somesite.com");
</pre>
<p>Здесь мы говорим подключиться к серверу MongoDB, который запущен на домене somesite.com и также на порту 27017. Если порт используется другой, 
то мы его также можем указать:</p>
<pre class="brush:php;">
$connection = new Mongo("somesite.com:888");
</pre>
<p>После того, как работа с базой данных завершена, мы можем закрыть соединение, используя метод <code>close</code>:</p>
<pre class="brush:php;">
$connection-&gt;close();
</pre>
<h3>Создание документов в PHP</h3>
<p>Ранее мы видели, что документ в mongodb фактически представляет объект в формате, аналогичном JSON. Он может хранить ключи, которым сопоставляются 
некоторые значения. Сами значения также могут представлять сложные объекты из ключей и значений. Так, мы создавали документ, представляющий 
...
<p>Создадим аналогичный документ в PHP. Для этого воспользуемся ассоциативными массивами:</p>
<pre class="brush:php;">
$steve = array(	
	"name" =&gt; "Steve", 
	"age" =&gt; 25, 
