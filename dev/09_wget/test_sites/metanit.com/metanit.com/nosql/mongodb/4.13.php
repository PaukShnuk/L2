...
<html >
<head>
<title>MongoDB и ASP.NET MVC. Чтение и добавление данных</title>
<meta charset="utf-8" />
<meta name="description" content="Чтение и добавление данных в MongoDB в ASP.NET MVC 5">
...
<h2>MongoDB и ASP.NET MVC. Чтение и добавление данных</h2><div class="date">Последнее обновление: 11.09.2017</div>
...
<p>Теперь добавим в проект новый контроллер HomeController и определим в нем методы Index и Create для выода списка и добвления объектов:</p>
<pre class="brush:c#;">
...
<p>Практически нам ничего не надо делать в контроллере кроме как вызывать методы класса ComputerContext, который и выполняет всю основную работу.</p>
<p>Представление со списком объектов будет предоставлять возможности фильтрации по названию и по году выпуска модели. Поэтому 
мы будем работать не напрямую с моделью Computer, а через ComputerList.</p>
<p>Метод <code>Filter</code> возвращает отфильтрованные данные. Для фильтрации используется поиск с помощью регулярных выражений 
по имени и по году.</p>
<p>Теперь добавим для метода Index новое представление:</p>
<pre class="brush:xml;">
@model MongoDBApp.Models.ComputerList
...
@foreach (var item in Model.Computers) {
    &lt;tr&gt;
        ...
        &lt;td&gt;
            ...
            @Html.ActionLink("Изображение", "AttachImage", new { id = item.Id })
        &lt;/td&gt;
    &lt;/tr&gt;
}
&lt;/table&gt;
</pre>
<p>Представление типизируется моделью ComputerList, в таблицу выводятся все модели компьютеров через свойство Model.Computers, 
а форма поиска привязывается к свойству Model.Filter.</p>
<p>А для метода Create определим представление <span class="b">Create.cshtml</span> с формой для добавления данных:</p>
<pre class="brush:xml;">
@model MongoDBApp.Models.Computer

@{
    ViewBag.Title = "Новая модель";
}
&lt;h2&gt;Новая модель&lt;/h2&gt;
...
Запустим сервер MongoDB и затем запустим проект. При первом обращении к приложению у нас естественно нет никаких данных, и на главной странице мы увидим пустую таблицу. Поэтому обратимся к методу Create добавим какие-нибудь данные:</p>
<img src="pics/4.9.png" alt="MongoDB и ASP.NET MVC 5" />
<p>И таким образом добавим несколько объектов Computer:</p>
<img src="pics/4.6.png" />
<p>А при фильтрации на страницу будут выводиться только те объекты, которые соответствуют критериям поиска:</p>
