...
<html >
<head>
<title>MongoDB | Установка на Windows</title>
<meta charset="utf-8" />
<meta name="description" content="Особенности установки MongoDB под различные платформы, обзор инсталляционного пакета, запуск mongodb">
...
<h2>Установка и начало работы с MongoDB на Windows</h2><div class="date">Последнее обновление: 08.08.2021</div>
...
<p>Для установки MongoDB загрузим один распространяемых пакетов с официального сайта <a href="https://www.mongodb.com/try/download/community" rel="nofollow">https://www.mongodb.com/try/download/community</a>.</p>
<p>Официальный сайт предоставляет пакеты дистрибутивов для различных платформ: Windows, Linux, MacOS, Solaris. И каждой платформы доступно несколько дистрибутивов. 
Причем есть два вида серверов - Community и Enterprise. В данном случае надо установить версию Community. Хотя Enterprise-версия обладает несколько большими возможностями, 
но она доступна только в триальном режиме или по подписке.</p>
<p>На момент написания данного материала последней версией платформы была версия <span class="b">5.0</span>, которая увидела свет в июле 2021 года и для которой постоянно выходят подверсии. 
Использование конкретной версии может несколько отличаться от применения иных версий платформы MongoDB.</p>
<p>Для загрузки всех необходимых файлов выберем нужную операционную систему и подходящий тип пакета. Рассмотрим на примере установки на ОС Windows.</p>
<p>MongoDB можно загрузить в ряде вариантов. Так, для Windows доступна загрузка установщика msi и также доступна загрузка zip-пакета. В реальности нам достаточно загрузить 
zip-архив и распаковать в нужной нам папке. Поэтому выберем этот вариант загрузки:</p>
<img src="./pics/1.6.png" alt="Установка MongoDB" />
<p>Если до установки уже была установлена более ранняя версия MongoDB, то ее необходимо удалить.</p>

<p>После загрузки архивного пакета распакуем его в папку <span class="b">C:\mongodb</span>.</p>
<h3>Содержимое пакета MongoDB</h3>
<p>Если после установки мы откроем папку <span class="b">bin</span> в распакованном архиве (<span class="ii">C:\mongodb\bin</span>), то сможем найти там кучу приложений, которые выполняют определенную роль. Вкратце рассмотрим их.</p>
<img src="./pics/1.1.png" alt="Каталог MongoDB" />
<ul>
<li><p><span class="b">mongo</span>: представляет консольный интерфейс для взаимодействия с базами данных, своего рода консольный клиент</p></li>
...
<h3>Создание каталога для БД и запуск MongoDB</h3>
<p>После установки надо создать на жестком диске каталог, в котором будут находиться базы данных MongoDB.</p>
<p>В ОС Windows по умолчанию MongoDB хранит базы данных по пути <span class="b">C:\data\db</span>, поэтому, если вы используете Windows, вам надо создать 
соответствующий каталог.</p>
<p>Если же возникла необходимость использовать какой-то другой путь к файлам, то его можно передать при запуске MongoDB во флаге 
<code>--dbpath</code>.</p>
<p>Итак, после создания каталога для хранения БД можно запустить сервер MongoDB. Сервер представляет приложение <span class="b">mongod</span>, которое находится 
в папке bin. Для этого запустим терминал/командную строку и там введем соответствующие команды. Для ОС Windows это 
будет выглядеть так:</p>
<img src="./pics/1.2.png" alt="Запуск сервера MongoDB mongod" />
<p>Командная строка отобразит нам ряд служебной информации, например, что сервер запускается на localhost на порту 27017.</p>
<p>И после удачного запуска сервера мы сможем производить операции с бд через оболочку <span class="b">mongo</span>. Эта оболочка представляет 
файл <span class="b">mongo.exe</span>, который располагается в выше рассмотренной папке установки. Запустим этот файл:</p>
<img src="./pics/1.3.png" alt="Запуск оболочки MongoDB" />
...
<p>Первая команда <span class="b">use test</span> устанавливает в качестве используемой базу данных test. Даже если такой бд нет, 
то она создается автоматически. И далее <code>db</code> будет представлять текущую базу данных - то есть базу данных test. После db идет 
...
<p>С помощью метода <span class="b">db.users.insertOne()</span> в коллекцию users базы данных test добавляется объект <code>{ name: "Tom" }</code>. 
...
<img src="./pics/1.4.png" alt="Добавление в бд MongoDB" />
...
<h3>Установка драйверов MongoDB</h3>
<p>В дальнейшем в рамках данного руководства мы будет рассматривать взаимодействие с сервером MongoDB преимущественно через выше использованную оболочку 
<span class="b">mongo</span>. Однако, мы также можем взаимодействовать с mongodb в наших приложениях, написанных на PHP, C++, C# и других языках программирования. И для этой цели необходим специальный драйвер.</p>
<p>На офсайте на странице <a href="https://docs.mongodb.com/ecosystem/drivers/" rel="nofollow">https://docs.mongodb.com/ecosystem/drivers/</a> можно найти список драйверов 
для всех поддерживаемых языков программирования, в частности, для PHP, C, C++, C#, Java, Go, Python, Rust, Ruby, Scala, Swift, а также для Node.js.</p>
<p>Работа с драйверами на конкретных языках программирования будет рассмотрена в соответствующих разделах, посвященных этим языкам..</p>
...
<div class="nav"><p><a href="./1.1.php">Назад</a><a href="./">Содержание</a><a href="./1.4.php">Вперед</a></p></div></div>
<div class="socBlock">
