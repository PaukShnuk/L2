...
<html >
<head>
<title>Работа с GridFS в MongoDB в C#</title>
<meta charset="utf-8" />
<meta name="description" content="Сохранение файлов в базе данных MongoDB с помощью системы GridFS в C#">
...
<h2>Работа с GridFS</h2><div class="date">Последнее обновление: 10.06.2017</div>
...
<p>Для хранения больших объемов информации, в частности, файлов, в MongoDB  используется система GridFS. Для работы с ней прежде всего необходимо установить через Nuget 
пакет <span class="b">MongoDB.Driver.GridFS</span>:</p>
<img src="pics/4.11.png" alt="GridFS в MongoDB и C#" />
<p>Чтобы получить объект GridFS, определенный у базы данных, нам надо создать объект <span class="b">IGridFSBucket</span>, определенный в пространстве имен <code>MongoDB.Driver.GridFS</code>. Для 
его создания можно использовать конструктор класса <span class="b">GridFSBucket</span>, в который передается объект базы данных:</p>
<pre class="brush:c#;">
var client = new MongoClient("mongodb://localhost");
var database = client.GetDatabase("test");
IGridFSBucket gridFS = new GridFSBucket(database);
</pre>
<p>Класс GridFSBucket предоставляет функциональность по управлению хранилищем, в частности, по добавлению, изменению и удалению файлов.</p>
<h3>Загрузка файлов</h3>
<p>Для добавления файлов в GridFS используется ряд методов, которые позволяют загружать либо данные из потока, либо массив байтов:</p>
<ul>
<li><p><span class="b">UploadFromBytes/UploadFromBytesAsync</span>: загружает файл в GridFS в виде массива байтов.</p></li>
<li><p><span class="b">UploadFromStream/UploadFromStreamAsync</span>: получает данные из потока</p></li>
</ul>
<p>Например, загрузим файл с локальной машины в хранилище:</p>
<pre class="brush:c#;">
using System;
...
<p>С помощью объекта FileStream создается файловый поток, а затем этот поток передается в метод <code>gridFS.UploadFromStreamAsync()</code> в качестве второго параметра. 
Первый параметр метода - имя файла - произвольный идентификатор. Метод возвращает id добавленного файла в GridFS в виде объекта ObjectId.</p>
<p>Если мы посмотрим содержание базы данных через Robomongo, то сможем увидеть в коллекции <span class="b">fs.files</span> информацию о сохраненном файле:</p>
<img src="pics/4.5.png" alt="Сохранение файлов в MongoDB и GridFS" />

<h3>Чтение файлов из GridFS</h3>
<p>Для загрузки файла из GridFS также применяется ряд методов в зависимости от того, во что мы загружаем файл - в массив байтов или в поток:</p>
<ul>
<li><p><span class="b">DownloadToStream/DownloadToStreamAsync</span>: загрузка файла по id из GridFS в поток</p></li>
<li><p><span class="b">DownloadAsBytes/DownloadAsBytesAsync</span>: загрузка файла по id в виде массива байтов</p></li>
<li><p><span class="b">DownloadToStreamByName/DownloadToStreamByNameAsync</span>: загрузка файла по имени из GridFS в поток</p></li>
<li><p><span class="b">DownloadAsBytesByName/DownloadAsBytesByNameAsync</span>: загрузка файла по id в виде массива байтов</p></li>
</ul>
<p>Первые две пары методов в качестве первого параметра принимают id файла, который был присвоен при загрузке в GridFS. Вторая пара методов загружает 
файла по имени. И третьяя пара методов загружает файл в виде массива байтов.</p>
<p>Извлечем ранее загруженный в GridFS файл:</p>
<pre class="brush:c#;">
using System;
using System.Threading.Tasks;
using MongoDB.Driver;
...
<p>В первом примере при загрузке в GridFS файлу было присвоено имя "cat.jpg". Теперь мы поэтому имени наоборот извлекаем файл из GridFS. Извлеченный файл передается в поток 
и сохраняется на диске D под именем "cat_new.jpg".</p>
<p>Подобным образом мы можем использовать перегрузки метода и загрузить файл, по определенному id.</p>
<h3>Поиск файла</h3>
<p>С помощью методов <span class="b">Find/FindAsync</span> мы можем найти файл в GridFS, а точнее получить различные его свойства. В качестве параметра 
эти методы принимают объект <span class="b">FilterDefinition&lt;GridFSFileInfo&gt;</span>, который определяет критерии поиска. Результатом методов является объект <span class="b">IAsyncCursor&lt;GridFSFileInfo&gt;</span> - 
список информации о файлах.</p>
<p>Например, найдем информацию по ранее добавленному файлу cat.jpg:</p>
<pre class="brush:c#;">
...
<p>Фильтр для поиска создается тем же образом, что и фильтр для выборки из обычной базы данных MongoDB. 
...
класс GridFSFileInfo, а имя файла хранится в его свойстве Filename, которое представляет строку. Поэтому метод <code>Eq</code> типизируется типом string. 
В него в качестве первого параметра передается лямбда-выражение, которое возвращает свойство Filename. А второй параметр представляет значение, 
которое должно иметь это свойство.</p>
<p>Необязательно искать именно по имени файла. Можно было взять любое свойство класса GridFSFileInfo в качестве критерия.</p>
<p>По одному и тому же критерию в бд может находиться несколько файлов, поэтому метод Find фактически возвращает набор, из которого мы можем получить отдельные объекты 
GridFSFileInfo и их свойства.</p>
<h3>Замена файла</h3>
<p>Для замены файла нам надо повторно его загрузить с помощью одного из методов UploadXXX и при загрузке указать параметр замены. Например, заменим 
файл по id:</p>
<pre class="brush:c#;">
...
<p>Для замены в метод UploadFromStreamAsync передается объект <span class="b">GridFSUploadOptions</span>, который описывает дополнительные опции 
загрузки. В частности, его свойство Metadata указывает на критерий замены. В данном случае замена будет производиться для файлов, у которых поле 
"filename" равно "cat.jpg".</p>
<h3>Удаление файлов</h3>
<p>Для удаления файла используются методы <span class="b">Delete/DeleteAsync</span>, которые удаляют файл по id:</p>
<p>Например, удалим ранее сохраненный файл новым:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="4.16.php">Назад</a><a href="./">Содержание</a><a href="4.12.php">Вперед</a></p></div></div>
<div class="socBlock">
