...
<html >
<head>
<title>Работа с GridFS в PHP</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с GridFS в PHP, сохранение информации в файловой системе mongoDB и функция storeUpload, классы MongoGridFS, MongoGridFSFile и MongoGridFSCursor и примеры их использования">
...
<h2>Работа с GridFS</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>В первой главе вкратце упоминалась система GridFS, которая позволяет хранить большие объекмы информации и файлы. Драйвер PHP для MongoDB предоставляет 
набор классов, необходимых для работы с GridFS:</p>
<ul>
<li><p><span class="bb">MongoGridFS</span>: позволяет сохранять и извлекать информацию из бд</p></li>
<li><p><span class="bb">MongoGridFSFile</span>: позволяет работать с отдельными файлами</p></li>
<li><p><span class="bb">MongoGridFSCursor</span>: работает с курсорами</p></li>
</ul>
<h3>Сохранение информации</h3>
<p>Для сохранения файла в системе GridFS используется функция <b>storeUpload</b>, которая принимает два параметра: название файла и метаданные и возвращает 
идентификатор <code>_id</code> сохраненного файла. Например, у нас есть некоторая форма для загрузки файла:</p>
<pre class="brush:xml;">
&lt;html&gt;
&lt;head&gt;
&lt;/head&gt;
...
<p>И скрипт ./, который обрабатывает форму:</p>
<pre class="brush:php;">
$con = new MongoClient();
$db= $con-&gt; test;
...
<p>Метод <b>getGridFS</b> позволяет получить объект класса MongoGridFS и вызвать для полученного файла метод <code>storeUpload</code>. 
Данный метод возвращает идентификатор, который мы можем использовать по своему усмотрению, например, связать с каким-нибудь объектом в коллекции.</p>
<h3>Извлечение файлов</h3>
<p>Для получения файлов из бд мы можем воспользоваться опять же методом <code>findOne</code> класса GridFS (или методом find). 
Метод findOne возвращает объект класса <b>MongoGridFSFile</b>, который инкапсулирует всю функциональность для работы с файлом. 
...
<p>Для получения файла идет фильтрация по id - объект MongoId получаем id и делаем выборку. Можно было бы применить и другой фильтр, например, по названию: 
<code>$file = $gridFS -&gt; findOne('22.txt')</code> или вообще не передавать никакой фильтр и взять первый файл из выборки.</p>
<p>Класс MongoGridFSFile обладает некоторыми методами, которые позволяют манипулировать файлом:</p>
<ul>
<li><p><code>getBytes</code>: возвращает содержимое сайта в виде набора байтов</p></li>
<li><p><code>getFilename</code>: возвращает имя файла</p></li>
<li><p><code>getResource</code>: возвращает ресурс, который может использоваться для чтения сохраненного файла</p></li>
<li><p><code>getSize</code>: возвращает размер файла</p></li>
<li><p><code>write</code>: пишет файл на диск</p></li>
</ul>
<p>В данном случае для отдачи файла пользователю мы используем метод <code>getBytes</code>, и пользователь сможет увидеть текстовый файл в браузере.</p>
<h3>Удаление файла</h3>
<p>Удаление файла из системы GridFS производится с помощью метода <code>delete</code>, в который передается id файла:</p>
<pre class="brush:php;">
$con = new MongoClient();
$db= $con-&gt; test;
