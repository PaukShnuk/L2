...
<html >
<head>
<title>C# и MongoDB</title>
<meta charset="utf-8" />
<meta name="description" content="Использование MongoDB в связке с C# и ASP.NET MVC, установка драйвера и настройка инструментария, тестирование подключения к базе данных">
...
<h1>C# и MongoDB</h1><h2>Установка драйвера и подключение к базе данных</h2><div class="date">Последнее обновление: 09.06.2017</div>
...
<p>Фреймворк .NET и в частности язык C# благодаря наличию драйвера также могут работать с MongoDB. Исходные файлы драйвера, а также разную необходимую 
информацию, например, что добавлено в новой версии, какие были сделаны улучшения и т.д., все это можно найти на странице на гитхабе: 
<a href="https://github.com/mongodb/mongo-csharp-driver/releases" rel="nofollow">https://github.com/mongodb/mongo-csharp-driver/releases</a>. 
Оттуда можно загрузить исходный код драйвера, либо его бинарные файлы.</p>
<p>Но для установки всех необходимых файлов и библиотек драйвера в проект проще использовать пакетный менеджер NuGet:</p>
<img src="pics/4.1.png" alt="Установка MongoDB через NuGet" />
<p>Текущей версией драйвера является версия 2.4. Версию следует учитывать при работе, так как функционал отдельных версий может отличаться.</p>
<p>Итак, возьмем простой консольный проект на C# и через NuGet установим в него пакет драйвера MongoDB.</p>
<p>И после этого в проект будет добавлен ряд библиотек: <span class="b">MongoDB.BSON.dll</span>, 
<span class="b">MongoDB.Driver</span> и <span class="b">MongoDB.Driver.Core</span>:</p>
<img src="pics/4.2.png" alt="Библиотеки MongoDB в C#" />
<p>Библиотека <span class="b">MongoDB.BSON.dll</span> содержит код документы и значения BSON, а также код для сопоставления документов BSON с объектами 
классов на C#.</p>
<p>Библиотека <span class="b">MongoDB.Driver.Core.dll</span> содержит функционал для подключения к серверу.</p>
<p>Библиотека <span class="b">MongoDB.Driver.dll</span> представляет легковесную обертку для взаимодействия кода на C# с сервером MongoDB.</p>
<p>Затем в коде программы мы можем подключить все необходимые пространства имен:</p>
<pre class="brush:c#;">
using MongoDB.Bson;
using MongoDB.Driver;
</pre>
<h3>Подключение к базе данных</h3>
<p>Для подключения к базе данных необходимо использовать ряд классов:</p>
<pre class="brush:c#;">
string connectionString = "mongodb://localhost:27017";
MongoClient client = new MongoClient(connectionString);
IMongoDatabase database = client.GetDatabase("test");
</pre>
<p>Для установки подключения первым делом нам надо использовать класс <span class="b">MongoClient</span>, в конструктор которого надо передать строку подключения:</p>
<pre class="brush:c#;">
string connectionString = "mongodb://localhost:27017"; // адрес сервера
MongoClient client = new MongoClient(connectionString);
</pre>
<p>Строка подключения имеет следующий вид: <code>mongodb://[username:password@]hostname[:port][/[database][?options]]</code>. Например, 
при указании всех параметров строка подключения могла бы выглядеть так: <code>mongodb://user:pass@localhost/db1?authSource=userDb</code> - в данном 
случае пользователь с логином user и паролем pass подключается к базе данных db1 на хосте localhost. Кроме того, также задается дополнительный параметр 
<code>authSource</code> со значением <code>userDb</code>.</p>
<p>Если порт не указан, то по умолчанию используется порт 27017.</p>
<p>После получения клиента с его помощью мы можем уже непосредственно обращаться 
к конкретным базам данных. Для получения базы данных применяется метод <code>GetDatabase</code>, в который передается название базы. Этот метод 
возвращает объект <span class="b">IMongoDatabase</span>:</p>
<pre class="brush:c#;">
IMongoDatabase database = client.GetDatabase("test");
</pre>
<p>В данном случае идет получение ссылки на базу данных test, которая имеется в пакете mongodb по умолчанию. Получив объект базы данных, мы 
можем обращаться к конкретным коллекциям и проводить различные операции с данными.</p>
<p>После выполнения всех необходимых операций нам необязательно закрывать подключение, как, например, в случае с подключениями к другим базам данных, так 
как MongoDB сама выполнит всю работу.</p>
<h3>Строки подключения в файлах конфигураций</h3>
<p>Кроме жесткого кодирования строк подключения в коде можно также их определить в конфигурационных файлах: app.config (для десктопных приложений) и 
web.config (для веб-приложений) в секции connectionString:</p>
<pre class="brush:xml;">
&lt;connectionStrings&gt;
    &lt;add name="MongoDb" connectionString="mongodb://localhost/test" /&gt;
&lt;/connectionStrings&gt;
</pre>
<p>Атрибут <code>connectionString</code> устанавливает параметры подключения, в частности сервер - тот же localhost и название базы данных - test.</p>
<p>Тогда на стороне сервера получение строки подключения из файла будет выглядеть так:</p>
<pre class="brush:xml;">
using System;
...
<p>Для использования класса ConfigurationManager необходимо добавить в проект библиотеку <span class="b">System.Configuration.dll</span>:</p>
<img src="pics/4.10.png" alt="Строка подключения к MongoDB в C#" />
<h3>Получение всех бд с сервера</h3>
<p>Создадим небольшое консольное приложение, которое будет выводить на экран все базы данных сервера:</p>
<pre class="brush:c#;">
using System;
using System.Collections.Generic;
using System.Linq;
...
<p>В этом случае в файле <span class="ii">App.config</span> должно быть прописано определение строки подключения:</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="3.10.php">Назад</a><a href="./">Содержание</a><a href="4.2.php">Вперед</a></p></div></div>
<div class="socBlock">
