...
<html >
<head>
<title>Добавление данных в MongoDB в PHP</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление данных в MongoDB через скрипт PHP, метод insert класса MongoClient и его параметры">
...
<h2>Добавление данных</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>В предыдущей главе, чтобы добавить новый документ в консоли mongo, использовалась функция <code>insert</code>. В PHP все точно также. 
Сначала создаем документ, получаем объект коллекции и вызываем у него метод insert:</p>
<pre class="brush:php;">
$steve = array(	
	"name" =&gt; "Steve", 
	"age" =&gt; 25, 
...
<p>Все довольно просто. Метод <code>insert</code> может также принимать второй необязательный аргумент, устанавливающий дополнительные опции 
добавления. В этом аргументе мы можем определить следующие параметры:</p>
<ul>
<li><p><code>fsync</code>: принимает булевое значение. По умолчанию равно <code>false</code>. Если значение равно <code>true</code>, 
то перед подтверждением удачного добавления данных в бд, они в обязательном порядке записываются на жесткий диск.</p></li>
<li><p><code>j</code>: подобно параметру <code>fsync</code> принимает булевое значение. По умолчанию равно <code>false</code>. 
Если значение равно <code>true</code>, то перед подтверждением удачного добавления данных в бд, они в обязательном порядке журналируются.</p></li>
<li><p><code>w</code>: если равно 1, то можно проводить операции записи-удаления-обновления. Если равно 0, то подобные операции запрещены. 
По умолчанию равно 1.</p></li>
<li><p><code>wtimeout</code>: указывает время в миллисекундах, которое сервер будет ожидать подтверждения. По умолчанию равно 10000 миллисекунд</p></li>
<li><p><code>timeout</code>: указывает время в миллисекундах, которое клиент будет ожидать ответ от базы данных</p></li>
</ul>
<pre class="brush:php;">
$bill = array(	
	"name" =&gt; "Bill", 
