...
<html >
<head>
<title>MongoDB | Удаление данных</title>
<meta charset="utf-8" />
<meta name="description" content="Удаление данных в MongoDB, функции remove, drop, dropDatabase, deleteMany, deleteOne, их особенности, параметры и примеры использования, удаление одного и всех документов, коллекции и базы данных">
...
<h2>Удаление данных</h2><div class="date">Последнее обновление: 08.08.2021</div>
...
<p>Для удаления документов в MongoDB предусмотрены функции <span class="b">deleteOne()</span> - удаляет один документ и <span class="b">deleteMany()</span> - 
позволяет удалить несколько документов. В качестве параметра в эти функции передается фильтр удаляемых документов.</p>
<p>Например, удалим документ, в котором <code>name="Tom"</code>:</p>
<pre class="brush:js;">
&gt; db.users.deleteOne({name : "Tom"})
</pre>
<p>В итоге первый найденный документ с name=Tom будет удален. Для удаления всех документов, которые соответствуют фильтруб применяется 
функция <span class="b">deleteMany()</span>:</p>
<pre class="brush:js;">
&gt; db.users.deleteMany({name : "Tom"})
</pre>
<p>Причем, как и в случае с find, мы можем задавать условия выборки для удаления 
различными способами (в виде регулярных выражений,  в виде условных конструкций и т.д.):</p>
<pre class="brush:js;">
...
<h3>Удаление коллекций и баз данных</h3>
<p>Мы можем удалять не только документы, но и коллекции и базы данных. Для удаления коллекций используется функция <span class="b">drop</span>:</p>
<pre class="brush:js;">
&gt; db.users.drop()
</pre>
...
<p>Чтобы удалить всю базу данных, надо воспользоваться функцией <span class="b">dropDatabase()</span>:</p>
<pre class="brush:js;">
&gt; db.dropDatabase()
</pre>
