...
<html >
<head>
<title>Настройка модели с помощью атрибутов в MongoDB в C#</title>
<meta charset="utf-8" />
<meta name="description" content="Настройка модели с помощью атрибутов в MongoDB в C#, BsonId, BsonRepresentation и BsonIgnore">
...
<h2>Настройка модели с помощью атрибутов</h2><div class="date">Последнее обновление: 09.06.2017</div>
...
<h3>Установка Id</h3>
<p>Каждый объект в базе данных имеет поле <code>_id</code>, которое выполняет роль уникального идентификатора объекта. Используя атрибут <span class="bb">BsonId</span> 
мы можем явно установить свойство, которое будет выполнять роль идентификатора:</p>
<pre class="brush:c#;">
class Person
{
    [BsonId]
...
<p>Хотя в данном случае свойство называется PersonId и имеет тип int, при создании документа данное свойство будет представлять в документе поле 
<code>_id</code></p>
<p>Используя атрибуты, мы можем управлять настройкой классов моделей и их сериализацией в документы mongodb. Например:</p>
<pre class="brush:c#;">
...
<p>Атрибут <span class="b">BsonIgnore</span> позволяет не учитывать свойство Surname при сериализации объекта в документ. А атрибут 
<span class="b">BsonElement</span> позволяет задать настройки элемента для данного свойства. В частности, здесь изменяется название элемента с Name на 
First Name. Поэтому при создании документа:</p>
<pre class="brush:c#;">
...
<h3>Игнорирование значений по умолчанию</h3>
<p>В примере выше для объекта Person задается объект Company. Однако в какой-то ситуации для объекта Person данный объект может отсутствовать. Например, 
...
только у него будет значение <code>null</code>. Чтобы избежать добавление в документ элементов, которые имеют значения, можно использовать 
атрибут <span class="b">BsonIgnoreIfNull</span>:</p>
<pre class="brush:c#;">
class Person
{
    public string Name { get; set; }
...
<p>В этом случае объект:</p>
<pre class="brush:c#;">
Person p = new Person { Name = "Bill", Surname = "Gates", Age = 48 };
</pre>
<p>будет представлять следующий документ:</p>
<div class="console">
<pre class="consoletext">
{
    "Name": "Bill",
...
<p>Так как данный атрибут работает только для свойств, которые могут иметь значение <code>null</code>, то есть ссылочных типов, то он не подходит 
для объектов значимых типов. Например, нам неизвестен возраст человека: <code>Person p = new Person { Name = "Bill", Surname = "Gates"};</code>. 
Но даже если мы не указали свойство Age, оно будет присутствовать в документе со значением 0 - то есть значением по умолчанию. Чтобы этого избежать, 
для свойства Age использован атрибут <span class="b">BsonIgnoreIfDefault</span>.</p>
<h3>BsonRepresentation</h3>
<p>Еще один атрибут <span class="b">BsonRepresentation</span> отвечает за представление свойства в базе данных. Например:</p>
<pre class="brush:c#;">
class Person
{
...
<p>В этом случае для свойства Id указывается, что оно будет выполнять роль идентификатора и в базе данных соответствующее поле будет иметь тип <code>ObjectId</code>. 
А вот свойству целочисленному Age в базе данных будет соответствовать строковое поле Age из-за применения атрибута <code>[BsonRepresentation(BsonType.String)]</code>.</p>
<h3>BsonClassMap</h3>
<p>Для настройки сопоставления классов C# с коллекциями MongoDB можно использовать класс <span class="b">BsonClassMap</span>, который регистрирует 
принципы сопоставления. Например, возьмем тот же класс Person:</p>
<pre class="brush:c#;">
...
<p>С помощью метода <span class="b">RegisterClassMap()</span> определяется карта сопоставления объектов Person и BsonDocument. В частности, в данном случае для свойство Name 
будет сопоставляться с полем name.</p>
<h3>Конвенции</h3>
<p>Конвенции наряду с атрибутами и BsonClassMap представляют еще один способ определения сопоставления классов и объектов BsonDocument. 
Конвенции определяются в виде набора - объекта <span class="b">ConventionPack</span>. Этот объект может содержать набор конвенций. 
Каждая конвенция представляет объект класса, производного от ConventionBase. Например, переведем все ключи в BsonDocument в нижний регистр:</p>
<pre class="brush:c#;">
using System;
using MongoDB.Bson;
...
<p>В данном случае применяется конвенция <code>CamelCaseElementNameConvention</code>, которая переводит названия свойства в camel case - имена начинаются со строчной буквы.</p>
<p>Конвенция добавляется с помощью метода <code>Add()</code> класса ConventionPack. И чтобы она сработала, необходимо вызвать метод 
<span class="b">ConventionRegistry.Register()</span>, который зарегистрирует конвенцию. Первый параметр метода представляет название конвенции, 
второй - объект ConventionPack, а третий - условие, при котором применяется конвенция. Здесь в качестве условия просто установлено ключевое слово <span class="b">true</span>, 
то есть конвенция будет применяться ко всем свойствам.</p>
<p>В результате при выводе на консоль названия всех ключей в документе будут переведены в нижний регистр:</p>
<div class="console">
<pre class="consoletext">
