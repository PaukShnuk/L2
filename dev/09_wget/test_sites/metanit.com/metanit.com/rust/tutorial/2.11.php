...
<html >
<head>
<title>Rust | Параметры функции</title>
<meta charset="utf-8" />
<meta name="description" content="Параметры функции в языке программирования Rust, передача аргументов в функцию по позиции">
...
<h2>Параметры функции</h2><div class="date">Последнее обновление: 20.04.2021</div>
...
<p>Функция может принимать параметры. Параметры позволяют передать в функцию некоторые данные. 
Параметры перечисляются в скобках после имени функции имеют следующее определение:</p>
<pre class="brush:rs;">название_параметра : тип_данных</pre>
<p>После названия параметра через двоеточие указывается его тип.</p>
<p>Например, определим функцию с одним параметром:</p>
<pre class="brush:rs;">
fn square(n: i32)
{
	let result = n * n;
...
<p>Функция <code>square</code> имеет один параметр, который называется <code>n</code> и который имеет тип <code>i32</code>, 
то есть представляет целое число. Внутри функции вычисляем квадрат этого числа и выводим его на консоль.</p>
<p>Поскольку функция принимает параметр, то при вызове функции нам обязательно надо передать для этого параметра значение, которрое соответствует его типу. 
Значение которое передается параметру при вызове функции назывется <span class="b">аргументом</span>. То есть в данном случае 
нам надо передаться в функцию при ее вызове какое-нибудь целое число.</p>
<pre class="brush:rs;">
fn main(){
	square(5);
	square(6);
...
<p>В данном случае функция <code>square</code> вызывается три раза, и каждый раз ей передается разное значение. 
...
<p>Если для функции надо определить несколько параметров, то они перечисляются через запятую. Например, определим функцию с двумя параметрами:</p>
<pre class="brush:rs;">
fn main(){
	show_user("Tom", 36);
}
...
<p>Первый параметр функции <code>show_user()</code> называется <code>name</code> и представляет тип <code>&amp;str</code>, то есть грубо говоря 
строку. А второй параметр - <code>age</code> представляет целое число. Поэтому при вызове функции в нее надо передать два значения - строку и число. Причем значения передаются параметрам 
<span class="b">по позиции</span>. То есть первый параметр представляет тип <code>&amp;str</code>, поэтому первым аргументом в функцию 
передается строка. Второй параметр представляет целое число, поэтому вторым аргументом передается целое число. То есть передаваемые аргументы 
соответствует параметрам по типу.</p>
<h3>Изменение значения параметра<h3>
<p>В некоторых ситуациях может потребоваться изменить значение параметра. Но здесь надо помнить, что как и переменные, значения параметров не изменяемы. 
Например, при компиляции следующей программы мы получим ошибку:</p>
<pre class="brush:rs;">
fn main(){
    square(5);
    square(6);
...
<p>Здесь мы пытаемся заменить значение параметра <code>n</code> квадратом его первоначального значения. Но при компиляции компилятор 
радостно сообщит нам "cannot assign to immutable argument".</p>
<p>Если нам действительно необходимо изменять значение параметра, то такой параметр необходимо определить с ключевым словом <span class="b">mut</span>:</p>
<pre class="brush:rs;">
fn main(){
    square(5);
    square(6);
