...
<html >
<head>
<title>Rust | Оператор ?</title>
<meta charset="utf-8" />
<meta name="description" content="Оператор ? и обработка ошибок c помощью перечиcления Result в языке программирования Rust, Result в функции main, error propagation">
...
<h2>Оператор ?</h2><div class="date">Последнее обновление: 07.05.2021</div>
...
<p>Оператор <span class="b">?</span> после значения <code>Result</code> указывает, что если функция возвращает объект Result, который представляет константу <span class="b">Ok</span>, 
то возвращается значение из константы <span class="b">Ok</span>. Однако если значение <code>Result</code> предоставляет константу <span class="b">Err</span>, 
то <span class="b">Err</span> будет возвращаться из всей вызывающей функции, в которой получен объект Result.</p>
...
<p>Функция <code>create_person()</code> возвращает объект Result. Поэтому при ее вызове в <code>main</code> к ней можно применить оператор <span class="b">?</span>:</p>
<pre class="brush:rs;">let tom = create_person("Tom", 36)?;</pre>
<p>То есть здесь нам не надо применять никаких конструкций match или дополнительных методов типа <code>unwrap()</code> или <code>expect()</code>, 
чтобы получить требуемый объект Person.</p>
<p>Однако в этом случае функция, которая вызывает функцию <code>create_person()</code> также должна возвращать объект Result. То есть в данном случае функция 
<code>main()</code> должна определять в качестве возвращаемого типа тип <span class="b">Result</span>. Но функция main является особой функцией, 
которая может возращать только тип <span class="b">()</span>.  Этот тип еще называется <span class="b">unit type</span> и аналогичен 
типу <code>void</code> в других языках программирования.</p>
<p>Чтобы выйти из ситуации первый параметр типа в <code>Result&lt;T, E&gt;</code> (параметр T) 
представляет тип <code>()</code>, а второй - тип ошибки (то есть тип String):</p>
<pre class="brush:rs;">fn main() -&gt;Result&lt;(), String&gt;{</pre>
<p>Кроме того, в конце функция возвращает константу <code>Ok()</code>, в которую передается <code>()</code>:</p>
<pre class="brush:rs;">Ok(())</pre>
<p>При этом возвращать отдельно константу <code>Err()</code> не надо, поскольку это делает автоматически оператор <code>?</code></p>
<p>Стоит отметить, что в реальности оператор <code>?</code> можно применять к функциям, которые возвращают объекты типов <span class="b">Result</span> или 
<span class="b">Option</span> или других типов, которые реализуют трейт <span class="b">std::ops::Try</span>.</p>
...
<div class="nav"><p><a href="./10.4.php">Назад</a><a href="./">Содержание</a><a href="./9.1.php">Вперед</a></p></div></div>
<div class="socBlock">
