...
<html >
<head>
<title>Rust | Перечисления Enum</title>
<meta charset="utf-8" />
<meta name="description" content="Перечисления Enum в языке программирования Rust, определение констант, передача им значений, использование в конструкции match">
...
<h2>Перечисления Enum</h2><div class="date">Последнее обновление: 12.04.2021</div>
...
<p><span class="b">Перечисления</span> или тип <span class="b">enum</span> представляет набор констант или состояний, в которых может пребывать переменная перечисления. 
И в зависимости от текущего состояния мы можем выполнить те или иные действия.</p>
<p>Формальное определение перечисления:</p>
<pre class="brush:rs;">
...
<p>После названия перечисления в фигурных скобках через запятую перечисляются константы перечисления.</p>
<p>Например, определим перечисление, которое назовем <code>OperationType</code> и которое будет представлять арифметические операции:</p>
<pre class="brush:rs;">
enum OperationType{
...
<p>Для обращения к константе перечисления указывается название перечисления и через два двоеточия его константа:</p>
<pre class="brush:rs;">название_перечисления::константа</pre>
<p>Например, мы можем определить переменную перечисления:</p>
<pre class="brush:rs;">let op = OperationType::Multiply;</pre>
<p>В данном случае переменная <code>op</code> представляет константу <code>OperationType::Multiply</code>.</p>
<p>Как было сказано в начале, перечисления удобны для определения состояний. И в зависимости от текущего состояния выполняется то или иное действие. 
Например:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Здесь в конструкции <code>match</code> проверяется состояние переменной <code>op</code>. В зависимости от значения этой переменной выводится то или иное сообщение на консоль. 
Поскольку эта переменная равна <code>OperationType::Multiply</code>, то будет выполняться выражение:</p>
<pre class="brush:rs;">OperationType::Multiply =&gt; println!("Умножение"),</pre>
<p>Если в будущем потребуется изменить значение переменной перечисления, то такая переменная должна быть определена с помощью ключевого слова <span class="b">mut</span>. 
Например, применим перечисление OperationType для выполнения арифметических операций:</p>
<pre class="brush:rs;">
fn main(){
...
<p>В данном случае все арифметические операции вынесены в отдельную функцию <code>get_result()</code>, которая в качестве параметров принимает 
...
Результат операции возвращается функцией.</p>
<p>В функции main вызывается функция <code>get_result()</code>, в которую передаются различные типы операций, и соответственно функция будет возвращать различный результат:</p>
<div class="console">
...
<h3>Установка типа значений перечисления</h3>
<p>Константы перечисления могут хранить некоторые значения, которые соответствуют их типу. Выше в перечислении OperationType тип для хранимых значений констант не определялся, но мы 
можем его определить. Например:</p>
<pre class="brush:rs;">
enum DayTime{
...
<p>Здесь обе константы перечисления DayTime представляют тип <span class="b">String</span>, то есть по сути они могут хранить некоторую строку.</p>
<p>А это значит, что при создании объекта этого перечисления нам надо передать его строковое значение:</p>
<pre class="brush:rs;">let dt = DayTime::Morning("Доброе утро".to_string());</pre>
<p>Значение передается в скобках после названия константы. Поскольку в данном случае константа <code>DayTime::Morning</code> представляет тип <code>String</code>, 
то переданная строка преобразуется с помощью метода <code>to_string()</code> к типу <code>String</code>.</p>
<p>Теперь рассмотрим, как мы можем использовать это значение:</p>
<pre class="brush:rs;">
fn main(){
 
	let mut dt = DayTime::Morning("Доброе утро".to_string());
	show_message(dt);
	
	dt = DayTime::Evening("Добрый вечер".to_string());
	show_message(dt);
}
fn show_message(dt: DayTime){
	match dt
	{
...
<p>Здесь определена функция <code>show_message()</code>, которая в качестве параметра принимает перечисление DayTime и в зависимости от его значения выводит то или 
иное сообщение на консоль. Причем при сравнении значений в конструкции match мы можем вытащить значение константы перечисления в отдельную переменную:</p>
<pre class="brush:rs;">DayTime::Evening(message)=&t; println!("{}", message),</pre>
<p>В данном случае в скобках определяется переменная <code>message</code>, которая получит значение из перечисления. И затем мы можем как-то использовать 
это значение, например, как в данном случае вывести на консоль.</p>
<p>Но возможно, нам значение соответствующей константы перечисления не важно, оно нам не нужно и мы не собираемся его использовать. В этом случае мы можем поставить прочерк:</p>
<pre class="brush:rs;">DayTime::Morning(_)=&gt; println!("Добрый день!"),</pre>
...
<pre class="brush:rs;">
let mut dt = DayTime::Morning(0, 12);
</pre>
<p>Тогда в конструкции <code>match</code> определяются две переменные:</p>
<pre class="brush:rs;">
fn show_message(dt: DayTime){
	match dt
	{
...
<div class="nav"><p><a href="./2.17.php">Назад</a><a href="./">Содержание</a><a href="./2.19.php">Вперед</a></p></div></div>
<div class="socBlock">
