...
<html >
<head>
<title>Rust | Структура программы</title>
<meta charset="utf-8" />
<meta name="description" content="Структура программы на языке Rust, функция main, макрос println!, типы комментариев, блоки кода и инструкции">
...
<h1>Основы Rust</h1><h2>Структура программы</h2><div class="date">Последнее обновление: 03.04.2021</div>
...
<h3>Инструкции</h3>
<p>Программа на Rust состоит из набора инструкций. Каждая инструкция выполняет определенное действие. В конце инструкции в Rust ставится 
точка с запятой (;). Данный знак указывает компилятору на завершение инструкции. Например:</p>
<pre class="brush:rs;">println!("Hello Rust!");</pre>
<p>Данная строка выводит на консоль строку "Hello Rust!", является инструкцией и поэтому завершается точкой с запятой.</p>
...
<h3>Функция main</h3>
<p>Каждая программа на языке Rust начинает свое выполнение с функции <span class="b">main()</span>. Соответственно эта функция обязательно должна быть 
в коде программы.</p>
<p>Функция также является блоком кода, поэтому ее тело обрамляется фигурными скобками, между которыми определяется набор инструкций. 
В частности, при создании первой программы использовалась следующая функция main:</p>
<pre class="brush:rs;">
...
<p>Определение функции в Rust начинается с ключевого слова <span class="b">fn</span>, за которым следует названия функции, то есть 
<span class="b">main</span> (название функции фиксированное). 
После названия в скобках идет список параметров. В данном случае функция main не принимает никаких параметров, поэтому после названия указаны пустые скобки.</p>
...
<p>В языке Rust есть два типа комментариев: однострочный и многострочный. Однострочный комментарий размещается на одной строке после двойного слеша //:</p>
<pre class="brush:rs;">
fn main() 						// определяем функцию main
{								// начало функции
    println!("Hello Rust!");	// выводим строку на консоль
