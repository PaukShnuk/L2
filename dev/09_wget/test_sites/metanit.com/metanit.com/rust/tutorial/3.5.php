...
<html >
<head>
<title>Rust | Изменяемые ссылки</title>
<meta charset="utf-8" />
<meta name="description" content="Изменяемые ссылки reference в языке программирования Rust, оператор &mut, изменение значения по ссылке">
...
<h2>Изменяемые ссылки</h2><div class="date">Последнее обновление: 15.04.2021</div>
...
<p>По умолчанию ссылки, как и переменные, нельзя изменять. То есть по умолчанию мы не можем изменить значение, на которое ссылается ссылка. Например:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Здесь для изменения значения по ссылке определена функция <code>change_message()</code>. У структуры String есть метод <code>push()</code>, который добавляет 
...
мы получим ошибку, поскольку по умолчанию значение по ссылке нельзя изменять:</p>
<img src="./pics/3.6.png" alt="Mutable reference и изменение ссылки в Rust" />
<p>Чтобы значение по ссылке все таки можно было изменять, необходимо использовать <span class="b">изменяемую ссылку</span> (mutable reference), 
которая определяется с помощью оператора <span class="b">&amp;mut</span>:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Здесь надо отметить три момента. Прежде всего, поскольку будет изменяться значение переменной, она должна быть определена с оператором <span class="b">mut</span>:</p>
<pre class="brush:rs;">let mut message = "hello".to_string();</pre>
<p>Во-вторых, параметр-ссылка функции, через который надо изменять значение, определяется как изменяемая ссылка с <span class="b">&amp;mut</span>:</p>
<pre class="brush:rs;">fn change_message(mes: &mut String){</pre>
<p>В-третьих, при передаче ссылки в функцию значения перед ним также указывается оператор <span class="b">&amp;mut</span>:</p>
<pre class="brush:rs;">change_message(&mut message);</pre>

<p>В итоге, эта программа будет нормально компилироваться и одарит нас следующим консольным выводом:</p>
<div class="console">
...
<p>Подобным образом можно определять переменные-изменяемые ссылки:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Следует отметить, что изменяемые ссылки имеют одно ограничение: в рамках одной области видимости может существовать только одна изменяемая ссылка. 
Например, следующий код не скомпилируется:</p>
<pre class="brush:rs;">
fn main(){
...
<p>В данном случае в одной области видимости - функции main определены две изменяемые ссылки - s2 и s3. Однако мы можем использовать вложенные области 
видимости, которые располагаются до определения изменяемых ссылок во внешних областях видимости:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Одна переменная-изменяемая ссылка - s3 определена в анонимном блоке, который заканчивает свою работу ДО определения второй изменяемой ссылки - s2. Соответственно 
никаких конфликтов между ссылками не возникнет.</p>
