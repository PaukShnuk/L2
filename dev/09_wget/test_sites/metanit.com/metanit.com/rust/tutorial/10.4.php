...
<html >
<head>
<title>Rust | Обработка нескольких типов ошибок</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка нескольких типов ошибок c помощью перечиcления Result в языке программирования Rust">
...
<h2>Обработка нескольких типов ошибок</h2><div class="date">Последнее обновление: 04.05.2021</div>
...
<p>Выше функция использовала один тип ошибки, что введен некорретный возвраст. Однако, что если мы захотим разграничить несколько различных ситуаций, 
которые могут вызывать ошибку. Например, мы могли бы определять отдельные ошибки при вводе некорректного возраста и некорректного имени пользователя. Рассмотрим 
возможную реализацию:</p>
<pre class="brush:rs;">
...
<p>Здесь для представления ошибок определена структура PersonError, которая имеет одно поле - <code>kind</code>, которое описывает тип ошибки и представляет 
перечисление ErrorKind.</p>
<p>Теперь функция <code>create_person</code> возвращает объект <code>Result&lt;Person, PersonError&gt;</code>, а это значит, что в константу 
<code>Err</code> необходимо передать объект PersonError.</p>
<p>В коде функции в зависимости от типа некорретных данных в PersonError передается та или иная константа перечисления ErrorKind:</p>
<pre class="brush:rs;">
Result::Err(
			PersonError{kind: ErrorKind::InvalidName(
				String::from("Некорректное имя. Длина имени должна быть больше 2-х символов")
...
<p>Используем функцию create_person в функции main:</p>
<pre class="brush:rs;">
fn main() {
...
<p>Для обработки конкретного типа ошибки применяются две конструкции match. В первой - внешней конструкции match узнаем, какой именно результат пришел 
из функции create_person:</p>
<pre class="brush:rs;">
let tom = match tom_result{
		Ok(person) =&gt; person,
		Err(error) =&gt; //.........
...
<p>Если результат представляет константу <code>Result::Ok</code>, то получем и возвращем объект Person. И затем мы сможем к нему обращаться через 
переменную <code>tom</code>.</p>
<p>Если результат представляет константу <code>Result::Err</code>, то используем вторую - вложенную конструкцию match, чтобы определить конкретный тип 
ошибки - какую константу перечисления ErrorKind она представляет:</p>
<pre class="brush:rs;">
Err(error) =&gt; match error.kind {
            ErrorKind::InvalidAge(message) =&gt; panic!("{}", message),
            ErrorKind::InvalidName(message) =&gt; panic!("{}", message)
...
<p>Поскольку каждая константа перечисления ErrorKind хранит значение типа String (в программе выше это сообщение об ошибке), то мы его можем 
получить в переменную message и передать в макрос <code>panic</code>.</p>
<p>Пример работы программы при передаче некорректного имени:</p>
<img src="./pics/10.5.png" alt="ErrorKind и типы и обработка ошибок в Rust" />
<p>И также мы можем сократить данный пример с помощью метода <code>unwrap_or_else()</code>:</p>
<pre class="brush:rs;">
fn main() {
