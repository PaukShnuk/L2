...
<html >
<head>
<title>Rust | Последовательность Range</title>
<meta charset="utf-8" />
<meta name="description" content="Последовательность Range в языке программирования Rust, оператор .., перебор последовательности, получение подмассива, использование в конструкции match">
...
<h2>Последовательность Range</h2><div class="date">Последнее обновление: 16.04.2021</div>
...
<p>Тип <span class="b">Range</span> или <span class="b">последовательность</span> представляет некоторый диапазон элементов. В Rust последовательности используются чаще всего 
для получения какой-то части набора элементов, например, массива или строки. Хотя этим применение последовательностей не ограничивается.</p>
<p>Для создания последовательности применяется оператор <span class="b">..</span> (две точки). Его общая форма:</p>
<pre class="brush:rs;">начальное_значение .. конечное_значение</pre>
<p>Слева от оператора <span class="b">..</span> указывается начальное значение диапазона, а справа - конечное значение. 
При этом конечное значение не включается в диапазон.</p>
<p>Также стоит отметить, что если начальное значение больше конечного, то последовательность будет пустой.</p>
<p>Например:</p>
<pre class="brush:rs;">
fn main(){
...
<p>В данном случае образуется последовательность чисел от 1 до 9 (число 9 не включается в последовательность). А с помощью цикла <span class="b">for</span> 
можно перебрать последовательность.</p>
<p>Хотя мы можем использовать оператор <span class="b">..</span> для создания последовательности, но в реальности она представляет структуру <span class="b">std::ops::Range</span>, которая имеет 
два поля: <span class="b">start</span> (начальное значение) и <span class="b">end</span> (конечное значение). Например, предыдущий пример мы могли 
бы переписать так:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Также для определения последовательности можно использовать модификацию оператора - <span class="b">..=</span>, которая позволяет включить в набор конечное значение:</p>
<pre class="brush:rs;">
fn main(){
...
<h3>Использование последовательности</h3>
<p>Последовательности очень удобно применять, если нам нужно взять какую-то часть массива. Например:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Более подробно использование последовательностей для извлечения поднабора рассматривается в теме про тип <a href="./3.6.php">slice</a>.</p>
...
<div class="nav"><p><a href="./2.18.php">Назад</a><a href="./">Содержание</a><a href="./2.25.php">Вперед</a></p></div></div>
<div class="socBlock">
