...
<html >
<head>
<title>Rust | Контекст/область видимости</title>
<meta charset="utf-8" />
<meta name="description" content="Контекст/область видимости scope переменных и констант в языке программирования Rust">
...
<h1>Ссылки и Ownership</h1><h2>Контекст/область видимости</h2><div class="date">Последнее обновление: 14.04.2021</div>
...
<p>Для каждого значения имеется свой констекст или область видимости (scope), в рамках которой мы можем использовать данное значение. Как определяется контекст/область видимости? 
Обычно границами области видимости служат фигурные скобки <span class="b">{}</span>, которые определяют блок кода - открывающая фигурная скобка определяет начало контекста, а закрывающая - 
конец контекста. Соответственно циклы и условные конструкции, функции и анонимные блоки кода образуют отдельную область видимости. Также есть глобальная область видимости - 
вне функции main, однако в этой обасти видимости можно определять только константы.</p>
<p>Объект, который определено в рамках какой-то области видимости, вне этой области видимости мы использовать не можем. Например:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Здесь переменная <code>n</code> определена в анонимном блоке кода, который ограничен фигурными скобками:</p>
<pre class="brush:rs;">
...
<p>После определения и инициализации переменной <code>n</code> мы можем ее использовать вплоть до окончания ее контекста - закрывающей фигурной скобки анонимного блока.</p>
<p>После завершения контекста (закрывающей фигурной скобки) значение переменной удаляется, и ее мы больше не можем использовать.</p>
<p>Объекты могут использоваться во вложенных контекстах, но не во внешних. Например:</p>
<pre class="brush:rs;">
...
<p>Здесь константа <code>A</code> определена в глобальной области видимости, поэтому она доступна в любой функции, определенной в том же файле, в том 
числе в любом месте функции <code>main</code>.</p>
<p>Переменная <code>x</code>, определенная в функции main, имеет в качестве области видимости - функцию main. 
Поэтому ее значение может использоваться в любом месте функции с момента инициализации переменной до конца функции, в том числе во вложенном в функцию анонимном блоке.</p>
<p>Переменная <code>n</code>, определенная в анонимном блоке, имеет в качестве области видимости данный анонимный блок. Поэтому ее значение может 
применяться в любом месте анонимного блока с момента инициализации переменной до конца блока.</p>
...
<div class="nav"><p><a href="./2.22.php">Назад</a><a href="./">Содержание</a><a href="./3.2.php">Вперед</a></p></div></div>
<div class="socBlock">
