...
<html >
<head>
<title>Rust | Аннотации ссылок в определениях методов</title>
<meta charset="utf-8" />
<meta name="description" content="Аннотации и определение времени жизни ссылок в методах структур в языке программирования Rust, зависимость от параметров и возвращаемых данных">
...
<h2>Аннотации ссылок в определениях методов</h2><div class="date">Последнее обновление: 26.04.2021</div>
...
<p>Если структура имеет поля, которые хранят ссылки, то при определении методов потребуется использовать аннотации времени жизни для ссылок.</p>
<p>Формальное применение аннотаций времени жизни в определении метода:</p>
<pre class="brush:rs;">
impl&lt;'a&gt; структра&lt;'a&gt; {
    fn метод(&self){
...
<p>Вначале посмотрим, что будет если мы не используем аннотации в определении метода:</p>
<pre class="brush:rs;">
struct Person&lt;'a&gt; {
    name: &amp;'a str,
}
...
<p>Здесь для структуры Person, которая применяет аннотацию для ссылки, определен метод <code>print()</code>. Однако при определении метода не применяется 
никаких аннотаций, поэтому мы получим ошибку при компиляции:</p>
<img src="./pics/3.13.png" alt="reference lifetime annotation in method in Rust" />
<p>Теперь применим аннотации в определении метода:</p>
<pre class="brush:rs;">
struct Person&lt;'a&gt; {
    name: &amp;'a str,
}
...
<p>Теперь ошибки не возникнет, и программа будет прекрасно работать.</p>
<h3>Аннотации для результата метода</h3>
<p>Кроме того, параметры и результаты методы могут использовать свои аннотации:</p>
<pre class="brush:rs;">
struct Person&lt;'a&gt; {
    name: &amp;'a str,
}
...
<p>По умолчанию в методах время возвращаемой ссылки соответствует времени жизни объекта структуры, который вызвал метод и который передается в метод через параметр 
<code>&amp;self</code>, однако в данном случае для возвращения результата применяется другой параметр-ссылка - строковый слайс <code>words</code>.</p>
<p>К параметру words и возвращаемой ссылке применяется аннотация <code>'b</code>, которая устанавливает, как и в случае с обычными функциями, что время жизни возвращаемой ссылки 
соответствует времени жизни объекта из параметра <code>words</code>. Причем в таком случае время жизни возвращаемой ссылки может быть большим, чем у объекта Person, 
который будет вызывать метод:</p>
<pre class="brush:rs;">
struct Person&lt;'a&gt; {
    name: &amp;'a str,
}
...
<p>Однако если метод возвращает ссылку на основе поля из структуры, в этом случае аннотации можно не использовать, поскольку возвращаемая ссылка автоматически будет иметь соответствовать 
времени жизни объекта структуры:</p>
<pre class="brush:rs;">
struct Person&lt;'a&gt; {
    name: &amp;'a str,
}
