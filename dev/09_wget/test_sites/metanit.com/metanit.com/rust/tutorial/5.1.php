...
<html >
<head>
<title>Rust | Методы</title>
<meta charset="utf-8" />
<meta name="description" content="Методы в языке программирования Rust, оператор impl, ссылка &self, текущий объект, определение и вызов метода">
...
<h1>Объектно-ориентированное программирование</h1><h2>Методы</h2><div class="date">Последнее обновление: 18.04.2021</div>
...
<p>Методы представляют функции, которые определены для структур или перечислений enum. Они позволяют добавить структурам и 
перечислениям некоторое поведение - некоторые выполняемые действия.</p>
<p>Формальное определение методов:</p>
<pre class="brush:rs;">
...
<p>Определение методов заключается в конструкцию, которая определяется с помощью ключевого слова <span class="b">impl</span>. После него идет название 
структуры или перечисления, для которого определяются методы.</p>
<p>Первый и обязательный параметр метода всегда представляет 
...
<p>Например, пусть у нас есть некоторая структура Person:</p>
<pre class="brush:rs;">
struct Person
{
    name: String,
...
<p>Добавим к этой структуре метод, который будет выводить ее данные на консоль. Для этого пропишем следующий код:</p>
<pre class="brush:rs;">
impl Person{
...
<p>После оператора <span class="b">impl</span> указывается название структуры, для которой создается метод - структуры Person.</p>
<p>Метод называется <code>display()</code>. Он имеет только один параметр - ссылку на объект структуры, который будет вызывать данный метод - <code>&amp;self</code>.</p>
<p>В самом методе с помощью ссылки на объект структуры мы можем обратиться к ее данным:</p>
<pre class="brush:rs;">println!("Name: {}  Age: {}", &self.name, &self.age);</pre>
<p>Вызов метода происходит в следующем формате:</p>
<pre class="brush:rs;">название_объекта.название_метода();</pre>
<p>После названия объекта структуры через точку указывается имя метода, после которого в скобках перечисляются значения для параметров метода. (Для параметра <code>&amp;self</code> значение передается автоматически)</p>
<p>Теперь используем выше определенные структуру и метод:</p>
<pre class="brush:rs;">
fn main(){
...
<p>В данном случае создается два объекта структуры Person: tom и bob. При вызове</p>
<pre class="brush:rs;">tom.display();</pre>
<p>В качестве ссылки на текущий объект <code>&amp;self</code> передается ссылка на объект tom, поскольку этот объект вызывает данный метод. 
При этом при вызове метода для параметра <code>&amp;self</code> не надо передавать никаких значений. Поэтому при вызове метода мы не передаем ему никаких параметров.</p>
<p>Аналогично при вызове</p>
<pre class="brush:rs;">bob.display();</pre>
<p>В метод будет передаваться ссылка на объект bob, поэтому этот вызов метода выведет на консоль данные для объекта bob.</p>
<p>Стоит отметить, что в принципе мы здесь используем еще один метод - метод <code>to_string()</code>, который преобразует значение к типу <code>String</code>:</p>
<pre class="brush:rs;">name: "Tom".to_string(),</pre>
<h3>Передача параметров в метод</h3>
<p>Как и любая функция метод может принимать и другие параметры Например, добавим метод, с помощью которого один объект Person будет приветствовать 
другой объект Person:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Итак, здесь добавлен метод <code>say_hello()</code>, который принимает три параметра. Первый параметр традиционно - ссылка на текущий объект. Второй 
параметр - <code>other</code> представляет ссылку на структуру Person - человека, которого надо приветствовать. И третий параметр - <code>hour</code> - текущий час. В самом методе в зависимости 
от текущего часа выводим на консоль то или иное приветствие.</p>
<p>Поскольку метод принимает три параметра, то при вызове метода в него надо передать два значения - для второго и третьего параметра (как уже говорилось выше, 
для первого параметра передавать никаких значений не надо):</p>
<pre class="brush:rs;">tom.say_hello(&bob, 12);</pre>
<p>Консольный вывод программы:</p>
<div class="console">
<pre class="consoletext">
Добрый день, Bob!
...
<h3>Возвращение результата из метода</h3>
<p>Также, как и обычные функции, метод может возвращать некоторое значение. Например, определим метод, который будет сравнивать два объекта Person по возрасту - кто старше и 
в зависимости от проверки возвращать некоторый результат:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Метод сравнения на возраст называется <code>is_older()</code>. В качестве параметра он принимает объект Person, с которым проводится сравнение. В качестве результата 
метод возвращает значение типа <code>bool</code>: <code>true</code>, если возраст текущего объекта Person больше возраста объекта other, либо 
<code>false</code>, если возраст меньше или равен.</p>
<p>При вызове в метод передается значение типа Person:</p>
<pre class="brush:rs;">let is_older = tom.is_older(&bob);</pre>
<p>Полученный из метода результат сохраняется в переменную <code>is_older</code>. Поскольку этот результат представляет тип <code>bool</code>, то мы можем 
использовать в конструкции <code>if..else</code> и в зависимости от его значения вывести на консоль то или иное значение. 
В данном случае консольный вывод будет следующим:</p>
<div class="console"><pre class="consoletext">Tom младше чем Bob или одного возраста с ним</pre></div>
<h3>Изменение текущего объекта в методе</h3>
<p>Если необходимо изменять текущий объект в методе, то первый параметр метода определяется как изменяемая ссылка - <span class="b">&mut self</span>. 
Например:</p>
<pre class="brush:rs;">
fn main(){
...
<p>В данном случае определен метод <code>change_age()</code>, который изменяет возраст текущего объекта Person с помощью второго параметра. Соответственно первый 
параметр определен как <span class="b">&amp;mut self</span>. Кроме того, переменная, которая представляет текщий объект определена с <span class="b">mut</span>:</p>
<pre class="brush:rs;">let mut tom = Person { name: "Tom".to_string(), age: 36};</pre>
...
<div class="nav"><p><a href="./3.6.php">Назад</a><a href="./">Содержание</a><a href="./5.2.php">Вперед</a></p></div></div>
<div class="socBlock">
