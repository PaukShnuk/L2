...
<html >
<head>
<title>Rust | Методы unwrap и expect типа Result</title>
<meta charset="utf-8" />
<meta name="description" content="Методы unwrap и expect и unwrap_or_else перечиcления Result в языке программирования Rust и обработка ошибок">
...
<h2>Методы unwrap и expect типа Result</h2><div class="date">Последнее обновление: 04.05.2021</div>
...
<p>Для перехвата и обработки ошибки в прошлой теме применялась конструкция <code>match</code>:</p>
<pre class="brush:rs;">
fn main() {
...
<p>Причем само получение результата функции как бы разделяется на две части. Сначала получаем собственно результат функции - объект Result:</p>
<pre class="brush:rs;">let tom_result = create_person("Tom", 36);</pre>
<p>Затем применяем конструкцию <code>match</code> для получения объекта Person:</p>
<pre class="brush:rs;">let tom = match tom_result{ ..... };</pre>
<p>Такой способ может показаться слишком избыточым в плане кода. Нам надо определить слишком много действий, чтобы получить требуемый объект Person. 
Однако тип <span class="b">Result</span> позволяет упростить работу, предоставляя специальные методы.</p>
<h3>unwrap</h3>
<p>Одним из таких методов является <span class="b">unwrap()</span>. Если возвращенный объект Result представляет константу <span class="b">Ok</span>, 
то метод <code>unwrap()</code> возвращает объект, который содержится в константе <span class="b">Ok</span>. Но если объект Result представляет 
константу <span class="b">Err</span>, то метод <code>unwrap()</code> вызывает макрос <code>panic!</code>.</p>
<p>Так, перепишем предыдущий пример с применением метода <code>unwrap()</code>:</p>
<pre class="brush:rs;">
fn main() {
...
<p>Если объект Person будет успешно создан, то через переменную <code>tom</code> мы сможем использовать далее в программе.</p>
<p>В случае ошибки мы получим следующее сообщение:</p>
<img src="./pics/10.6.png" alt="Result и метод unwrap в Rust" />
<h3>expect</h3>
<p>Метод <span class="b">expect()</span> работает аналогично за тем исключением, что позволяет установить сообщение, которое будет передаваться в макрос <code>panic</code>:</p>
<pre class="brush:rs;">
fn main() {
...
<h3>unwrap_or_else</h3>
<p>Еще одну возможность предоставляет метод <span class="b">unwrap_or_else()</span>. Если возвращенный объект Result представляет константу 
<span class="b">Ok</span>, то метод <code>unwrap_or_else()</code> возвращает объект, который содержится в константе <span class="b">Ok</span>.</p>
<p>Но если объект Result представляет константу <span class="b">Err</span>, то метод вызывает вызывает передаваемую в качестве параметра функцию. 
Эта функция в качестве параметра получает объект ошибки из константы <code>Err</code> и сама определяет логику обработки ошибки. Например:</p>
<pre class="brush:rs;">
fn main() {
...
<p>Здесь в метод <code>unwrap_or_else()</code> передается функция <code>|error| { panic!("Возникла проблема: {}", error); }</code>. 
Параметр <code>error</code> - это, что передается в константу <code>Err</code> - в данном случае объект String с сообщением об ошибке.</p>
