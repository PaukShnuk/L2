...
<html >
<head>
<title>Rust | Константы</title>
<meta charset="utf-8" />
<meta name="description" content="Константы в языке программирования Rust, оператор const, определение и использование констант, отличие от переменных">
...
<h2>Константы</h2><div class="date">Последнее обновление: 09.04.2021</div>
...
<p>Для хранения значений, которые в течении работы программы не должны меняться, в Rust предназначены <span class="b">константы</span>. Константы определяются 
с помощью ключевого слова <span class="b">const</span> в следующем формате:</p>
<pre class="brush:rs;">const название_константы : тип_данных = значение;</pre>
<p>Определим константу и выведем ее значение на консоль:</p>
<pre class="brush:rs;">
fn main(){
	const PI : f32 = 3.14;
    println!("PI = {}", PI);
...
<p>Здесь определена константа <code>PI</code>, которая имеет значение 3.14. Согласно условностям названия констант указываются в 
верхнем регистре.</p>
<p>Поскольку константа хранит неизменяемое значение, то при попытке его изменить, мы столкнемся с ошибкой, как в следующем случае:</p>
<pre class="brush:rs;">
fn main(){
	const PI : f32 = 3.14;
    println!("PI = {}", PI);
...
<p>Может возникнуть вопрос, в чем отличие констант от обычных переменных, значение которых по умолчанию тоже нельзя изменять.</p>
<p>Прежде всего, отличия констант и переменных в синтаксисе: для определения констант используется ключевое слово <span class="b">const</span>, и при этом 
для констант обязательно надо указать тип данных. А переменные определяются с помощью ключевого слова <span class="b">let</span>, а их тип можно не указывать, если при 
объявлении переменной ей тут же присваивается значение. Кроме того, переменные можно определить с ключевым словом <span class="b">mut</span> и сделать их 
изменяемыми, что нельзя сделать с константами.</p>
<pre class="brush:rs;">
let mut age = 36;
const PI : f32 = 3.14;
</pre>
<p>Другое отличие состоит в том, что переменную можно вначале объявить, а потом присвоить ей значение (по крайней мере до ее использования):</p>
<pre class="brush:rs;">
let age: u32;
age	= 36;
</pre>
<p>А константе надо присвоить значение сразу же при ее объявлении:</p>
<pre class="brush:rs;">
const PI : f32; 	// Ошибка!! Константе должно быть присвоено значение!
PI = 3.14;
</pre>
<p>Третье отличие заключается в том, что константа принимает только константное значение, которое должно быть известно уже во время компиляции. 
А переменной можно присвоить значение, которое может вычисляться уже в процессе работы программы, например, результат функции.</p>
<pre class="brush:rs;">
fn main(){
...
<p>Другой пример динамической установки данных, который не сработает с константами:</p>
<pre class="brush:rs;">
fn main(){
	let a = 10;
	let b = 5;
...
<p>В данном случае константа <code>NUM</code> получает значение из конструкции <code>if..else</code>. Но если в случае с переменной <code>number</code> 
проблем не возникнет. То при определения значения константы компилятор возвестит нас об ошибке. Хотя если вместо вычислений из конструкции 
<code>if..else</code> передавались бы константные значения, то ошибки бы не возникло:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Четвертое отличие - константа может быть определена в любой области видимости, в том числе в глобальной области видимости.</p>
<pre class="brush:rs;">
const NUM : i32 = 225;
...
<p>В данном случае в глобальной области видимости (вне функции main) определены константа NUM и переменная number. 
Однако если константу мы можем определить в глобальной области видимости, то переменную НЕ можем. Поэтому при компиляции 
данной программы возникнет ошибка.</p>
...
<div class="nav"><p><a href="./2.12.php">Назад</a><a href="./">Содержание</a><a href="./2.20.php">Вперед</a></p></div></div>
<div class="socBlock">
