...
<html >
<head>
<title>Rust | Паттерны и конструкция if let</title>
<meta charset="utf-8" />
<meta name="description" content="Паттерны и конструкция if let в языке программирования Rust, структуры, кортежи и перечисления в if let, декомпозиция на переменные">
...
<h2>Паттерны и конструкция if let</h2><div class="date">Последнее обновление: 29.04.2021</div>
...
<p>Конструкция <span class="b">if let</span> позволяет объединить синтаксис конструкции <span class="b">if</span> и определение переменной 
с помощью оператора <span class="b">let</span>.</p>
<p><span class="b">if let</span> принимает шаблон и выражение, разделенные знаком равно.</p>
<pre class="brush:rs;">
...
<h3>Кортежи</h3>
<p>Например, нам надо выполнить некоторые действия, если элементы кортежа имеют определенные значения:</p>
<pre class="brush:rs;">
fn main() {
...
<p>Здесь <code>("Tom", 36)</code> представляет шаблон, а кортеж user - выражение, с которым сравниваются значения из шаблона.</p>
<p>В данном случае условие</p>
<pre class="brush:rs;">
if let ("Tom", 36) = user 
</pre>
<p>говорит, что если первый элемент кортежа user равен строке "Tom", а второй - числу 36, то выполняется вывод на консоль.</p>
<p>По большому счету данная конструкция соответствует следующей конструкции <code>match</code>:</p>
<pre class="brush:rs;">
match user{
...
<p>Однако конструкция match получается более многосложной за счет того, что нам надо определить действия по умолчанию на случай, если кортеж 
не соответствует ни одному из значений.</p>
<p>Также можно определить альтернативные действия на случай, если значения из шаблона не соответствуют выражению:</p>
<pre class="brush:rs;">
fn main() {
...
<p>Эта конструкция также хороша тем, что мы можем в шаблоне определить переменные и затем использовать их в теле конструкции:</p>
<pre class="brush:rs;">
fn main() {
...
<p>В данном случае, если первый компонент кортежа user равен строке "Tom", то второй компонент кортежа user помещается в переменную age, которую 
затем можно использовать в блоке конструкции <code>if let</code>.</p>
<h3>Перечисления</h3>
<p>Подобным образом можно использовать в конструкции <code>if let</code> перечисления:</p>
<pre class="brush:rs;">
fn main(){
...
<p>В данном случае в конструкции <code>if let</code> переменная <code>morning</code> сопоставляется с шаблоном <code>DayTime::Morning(message)</code>:</p>
<pre class="brush:rs;">
if let DayTime::Morning(message) = morning{
</pre>
<p>При сопоставлении значение константы <code>DayTime::Morning</code> попадает в переменную <code>message</code>, которую далее можно использовать.</p>
<h3>Структуры</h3>
<p>Подобным образом можно использовать и структуры:</p>
<pre class="brush:rs;">
struct Person
{
    name: String,
...
<p>Здесь шаблон <code>Person{name: username, age: 33, height: _}</code> сопоставляется с переменной <code>tom</code>. Следует учитывать, что в 
шаблонах нельзя использовать вызовы функций, поэтому создание объекта String вынесено в отдельную переменную <code>username</code>.</p>
<p>Если нам не важно значение какого-то поля, то для него можно указать прочерк, как в случае с полем height.</p>
...
<div class="nav"><p><a href="./2.25.php">Назад</a><a href="./">Содержание</a><a href="./3.1.php">Вперед</a></p></div></div>
<div class="socBlock">
