...
<html >
<head>
<title>Rust | Функции</title>
<meta charset="utf-8" />
<meta name="description" content="Функции в языке программирования Rust, оператор fn, определение и вызов функций">
...
<h2>Функции</h2><div class="date">Последнее обновление: 08.04.2021</div>
...
<p>Функции представляют именованный блок инструкций, которые выполняют некоторые действия. Функции позволяют разделить программу на 
отдельные функциональные части, которые выполняют какую-то определенную задачу, и многократно их вызывать в различных частях программы.</p>
<p>Формальное определение функции:</p>
<pre class="brush:rs;">
fn имя_функции(параметры) 
{
}
</pre>
<p>Определение функции начинается с ключевого слова <span class="b">fn</span>, после которого идет название функции. 
После названия функции в скобках указывается список параметров.</p>
<p>По умолчанию при создании любой программы на Rust мы уже сталкиваемся с одной специальной функцией - <span class="b">main</span>, с которой начинается 
выполнение программы на языке Rust. Определим еще одну функцию:</p>
<pre class="brush:rs;">
fn main()
{
...
<p>Здесь кроме функции <code>main</code> определена еще одна функция, которая называется "hello". Она, как и функция main, не принимает никаких параметров, 
поэтому после ее названия идут пустые скобки. И все, что эта функция делает, это выводит на консоль строку "Hello Rust".</p>
<p>Функция определяется вне функции main, однако где именно - до или после функции main - это без разницы.</p>
<p>Тем не менее если мы скомпилируем и запустим программу, то ничего не произойдет, и на консоль не будет выводиться никакой строки 
"Hello Rust", потому что функция <code>hello</code> нигде не запускается. Более того при компиляции компилятор выведет предупреждение, 
что функция <code>hello</code> не используется.</p>
<p>Сама по себе функция hello не будет выполняться. По умолчанию Rust выполняет только функцию main. 
Поэтому, чтобы выполнить функцию hello, ее необходимо вызвать в функции main. Вызов функции осуществляется в форме:</p>
<pre class="brush:rs;">имя_функции(аргументы);</pre>
<p>После имени функции указываются скобки, в которых перечисляются аргументы - значения для параметров функции.</p>
<p>Например, вызовем функцию hello два раза:</p>
<pre class="brush:rs;">
fn main(){
	hello();
    hello();
...
<p>В итоге программа два раза выведет на консоль строку "Hello Rust":</p>
<div class="console">
...
<p>Подобным образом можно определять и вызывать и другие функции:</p>
<pre class="brush:rs;">
fn main(){
	hello();
    welcome();
