...
<html >
<head>
<title>Rust | Аннотации и время жизни ссылки</title>
<meta charset="utf-8" />
<meta name="description" content="Аннотации и время жизни ссылки в языке программирования Rust, область видимости ссылок, lifetime и scope">
...
<h1>Время жизни ссылки</h1><h2>Аннотации и время жизни ссылки</h2><div class="date">Последнее обновление: 25.04.2021</div>
...
<p>Каждая ссылка в Rust имеет свое время жизни, которое обычно областью видимости или контекстом, в рамках которого ссылку можно 
использовать. И большинстве случаев Rust сам выводит для ссылки время жизни, опять же исходя из ее области видимости. Рассмотрим простейший пример:</p>
<pre class="brush:rs;">
fn main(){
	let x;
...
<p>Здесь во внутреннем блоке кода переменная <code>x</code> получает ссылку на данные переменной <code>n</code>: <code>x = &amp;n;</code></p>
<p>При завершении внутреннего блока кода память под переменную <code>n</code> освобождается, соответственно ссылка 
<code>&amp;n</code> становится недействительной, а переменная <code>x</code> теперь ссылается на несуществующие данные. 
Однако чуть ниже идет попытка обратиться к данным переменной <code>x</code>, которые уже являются недействительными:</p>
<pre class="brush:rs;">println!("x: {}", x);</pre>
<p>К счастью компилятор Rust умееет отслеживать подробные ситуации, поэтому при компиляции мы получим ошибку:</p>
<img src="./pics/3.7.png" alt="reference does not live long enough в Rust" />
<p>Если бы обе переменные были определены в одной области видимости, то такой проблемы не возникло:</p>
<pre class="brush:rs;">
fn main(){
	let x;
	let n = 2;
...
<p>Либо, если бы переменной <code>x</code> только применялась в той области видимости, в которой действительно ее значение:</p>
<pre class="brush:rs;">
fn main(){
	let x;
	{
...
<p>Хотя в большинстве случаев Rust сам может вывести время жизни ссылки, в некоторых случаях это время необходимо указывать 
явным образом. Так, рассмотрим другой пример - функцию, которая возвращает строковый слайс <code>&amp;str</code>:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Здесь функция <code>get_message()</code> возвращает объект <code>&amp;str</code>. Однако Rust не может определить время жизни 
данной ссылки, поэтому при компиляции будет сгенерирована ошибка <span class="b">missing lifetime specifier</span>:</p>
<img src="./pics/3.8.png" alt="missing lifetime specifier в Rust" />
<p>И в данной ситуации нам необходимо указать явным образом время жизни для возвращаемой ссылки.</p>
<h3>Установка времени жизни ссылки</h3>
<p>Для установки времени жизни ссылки применяются аннотации, которые должны начинаться с апострофа ', например, аннотация <code>'a</code>.</p>
<p>Аннотация помещается после символа амперсанда <span class="b">&amp;</span>, но до определения типа данных:</p>
<pre class="brush:rs;">
&amp;i32        // обычная ссылка
&amp;'a i32     // ссылка с указанным временем жизни
&amp;'a mut i32 // изменяемая ссылка с указанным временем жизни
</pre>
<p>Так, изменим предыдущий пример, применив аннотации времени жизни:</p>
<pre class="brush:rs;">
fn main(){
	let message = get_message();
	println!("message: {}", message);
...
<p>После названия функции необходимо указать названи аннотации. В данном случае аннотация называется <code>'a</code>.</p>
<pre class="brush:rs;">fn get_message&lt;'a&gt;()</pre>
<p>Далее аннотация применяется к возвращаемому результату:</p>
<pre class="brush:rs;">-&gt; &amp;'a str </pre>
<p>Теперь программа будет успешно компилироваться и запускаться.</p>
<p>Стоит учесть, что аннотации времени жизни сами по себе они не имеют большого значения. В реальности они не меняют время жизни ссылки. 
Их назначение - указать компилятору, как аннотации времени жизни соотносятся друг с другом. То есть 
не существует в языке Rust некой встроенной аннотации <code>'a</code> - это название мы даем аннотации сами.</p>
<p>Однако какое же время жизни будет у результата, который возвращается данной функцией? Если время жизни возвращаемой ссылки НЕ привязано к времени жизни параметров функции, 
то тогда время жизни возвращаемой ссылки соответствует времени жизни возвращаемого значения.</p>
<p>В данном случае в функции параметров нет, а возвращаемое значение - строковый литерал "hello". Строковые литералы сохраняются в памяти в течение всей работы программы, 
поэтому время жизни подобной ссылки - все время работы программы.</p>
