...
<html >
<head>
<title>Rust | Ссылки</title>
<meta charset="utf-8" />
<meta name="description" content="Ссылки reference в языке программирования Rust, определение ссылки на переменную, параметр-ссылка, оператор &">
...
<h2>Ссылки</h2><div class="date">Последнее обновление: 15.04.2021</div>
...
<p>В прошлой теме рассматривалась тема владения или ownership, в частности, рассматривалась смена владения, после которого переменную-прошлого владельца нельзя было 
использовать без повторной инициализации:</p>
<pre class="brush:rs;">
fn main(){
...
<p>В данном случае после присвоения <code>let s2 = s1</code> происходит смена владельца строки "hello" с переменной <code>s1</code> на <code>s2</code>. После этого 
переменную <code>s1</code> без повторной инициализации мы не можем использовать.</p>
<p>Такое поведение не всегда может быть желательно. Возможно, мы затем сохранить владение для переменной <code>s1</code>, но при этом также присвоить ее значение переменной 
<code>s2</code>. Одним из решений данной проблемы является применение <span class="b">ссылок (reference)</span>.</p>
<p>Для получения ссылки на переменную применяется оператор <span class="b">&amp;</span> (амперсанд), который ставится перед переменной:</p>
<pre class="brush:rs;">&переменная</pre>
<p>После установки ссылки на переменную другими переменными, эта переменная сохраняет владение своим значением. Рассмотрим на примере:</p>
<pre class="brush:rs;">
fn main(){
...
<p>В данном случае переменная s2 в качестве значения получает не само значение переменной s1 (то есть строку "hello"), а ссылку на него:</p>
<pre class="brush:rs;">let s2 = &s1;</pre>
<p>Поскольку здесь присваивается не сам объект String, а ссылка на него, то типом данных переменной <code>s2</code> будет тип <span class="b">&amp;String</span>. То 
есть фактически мы могли бы написать:</p>
<pre class="brush:rs;">let s2: &amp;String = &s1;</pre>
<p>Упрощенно визуально это можно представить следующим образом:</p>
<img src="./pics/3.5.png" alt="Ссылки reference в Rust и ownership" />
<p>Переменная <code>s2</code> ссылается на объект String <code>s1</code>, а тот хранит указатель на строку "hello" в куче. При этом s1 сохраняет владение и мы ее можем и дальше использовать:</p>
<div class="console">
...
<p>При этом когда завершается область видимости переменной-ссылки s2, то значение, на которое она ссылается, не удаляется. Поскольку переменная-ссылка не владеет этим значением:</p>
<pre class="brush:rs;">
fn main(){
...
<h3>Параметры-ссылки</h3>
<p>Параметры функций также могут представлять ссылки на другие объекты. В этом случае определение параметра выглядит следующим образом:</p>
<pre class="brush:rs;">параметр: &amp;тип_данных</pre>
<p>Например, передадим в функцию ссылку:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Здесь параметр функции <code>display_message	()</code> представляет ссылку на объект <code>String</code> и поэтому имеет тип <code>&amp;String</code>:</p>
<pre class="brush:rs;">fn display_message(message: &amp;String){</pre>
<p>При вызове функции этому параметру необходимо передать ссылку на объект String (но никак не сам объект String):</p>
<pre class="brush:rs;">display_message(&s1);	// message: hello</pre>
