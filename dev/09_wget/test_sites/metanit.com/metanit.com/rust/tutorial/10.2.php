...
<html >
<head>
<title>Rust | Тип Result</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка ошибок и перечисление Result в языке программирования Rust">
...
<h2>Тип Result</h2><div class="date">Последнее обновление: 04.05.2021</div>
...
<p>Многие ошибки не требуют завершения программы. Такие ошибки можно обработать и продолжить выполнение программы. Для этой цели в Rust 
предназначен тип <span class="b">Result</span>. Данный тип представляет enum или перечисление, которое определяет две константы: 
<span class="b">Ok</span> и <span class="b">Err</span>.</p>
<pre class="brush:rs;">
enum Result&lt;T, E&gt; {
    Ok(T),
    Err(E),
...
<p>Это перечисление типизировано двумя параметрами типа: <span class="b">T</span> и <span class="b">E</span>. <span class="b">T</span> представляет тип 
значения, который будет возвращаться в случае успешного выполнения. А параметр <span class="b">E</span> представляет тип ошибки, 
которая будет возвращаться через константу <code>Err</code> в случае возникновения ошибки.</p>
<p>Рассмотрим, как использовать перечисление Result. Допустим, у нас есть функция, которая создает объект некоторой структуры:</p>
<pre class="brush:rs;">
...
<p>Итак, у нас есть структура Person, которая содержит два поля: name - имя пользователя и age - возраст пользователя. И также определена функция 
<code>create_person()</code>, которая принимает данные для имени и возраста пользователя и по ним создает объект Person. Однако данная 
функция возвращает не просто объект Person, а объект перечисления <span class="b">Result&lt;Person, String&gt;</span>. 
А это значит, что функция должна возвращать либо константу Ok, в которую передается значение Person, либо константу Err, в которую должно передаваться значение String.</p>
<p>Логика функции проста: если возраст корректен (в данном случае меньше 110), то объект Person создается. Если возраст некорретен, то нет смысла создавать 
объект Person.</p>
<p>Если действия завершились успешно, то возвращается константа <span class="b">Ok</span>, которая содержит созданный объект Person.</p> 
<pre class="brush:rs;">Result::Ok(new_person)</pre>
<p>Если действия по созданию объекта Person завершились неудачно, то возвращается константа <span class="b">Err</span>, 
которая содержит информацию об ошибке в виде объекта String - некоторое сообщение об ошибке.</p>
<pre class="brush:rs;">Result::Err(String::from("Некорректный возраст. Возраст должен быть меньше 110"))</pre>
<p>Теперь применим это в программе:</p>
<pre class="brush:rs;">
fn main() {
...
<p>Вначале передаем корректные данные:</p>
<pre class="brush:rs;">let tom_result = create_person("Tom", 36);</pre>
<p>Чтобы узнать, что собой представляет полученный результат <code>tom_result</code>, используем конструкцию <code>match</code>:</p>
<pre class="brush:rs;">
match tom_result{
	Ok(tom) =&gt; println!("Name: {}  Age: {}", tom.name, tom.age),
	Err(err_message) =&gt; println!("{}", err_message)
...
<p>Если создание объекта Person завершилось успешно, то мы получим константу Ok. В данном случае из константы в переменную <code>tom</code> получаем 
возвращенный объект Person и выводим его данные на консоль:</p>
<pre class="brush:rs;">Ok(tom) =&gt; println!("Name: {}  Age: {}", tom.name, tom.age),</pre>
<p>Если создание Person завершилось неудачно, то выполняется выражение</p>
<pre class="brush:rs;">Err(err_message) =&gt; println!("{}", err_message)</pre>
<p>которое помещает в переменную <code>err_message</code> объект String с сообщением об ошибке и выводит его на консоль.</p>
<p>Для второго вызова функции логика аналогична. В итоге мы получим следующий консольный вывод:</p>
<div class="console">
...
<h3>Вызов макроса panic</h3>
<p>Как именно обрабатывать ошибку, зависит от разных условий. Например, в примере выше предполагалось, что отсутствие объекта Person некритично. 
Однако вероятны ситуации, когда ошибка может повлиять на возможность дальнейшего выполнения программы. Вполне возможно, что 
если объект Person не создан, то дальше нет смысла продолжать работу программы. И в этом случае ее можно завершить макросом <span class="b">panic!</span>:</p>
<pre class="brush:rs;">
fn main() {
...
<p>Здесь, используя синтаксис паттернов, получаем из результата tom_result значение Person. Если результат представляет константу <code>Ok()</code>, 
то передаем из нее в переменную person объект Person и возвращаем его:</p>
<pre class="brush:rs;">Ok(person) =&gt; person</pre>
<p>В итоге это значение перейдет в переменную <code>tom</code>. И далее мы сможем, например, вывести полученные данные на консоль или как-то иным образом использовать 
полученный объект Person.</p>
<p>Если результат представляет константу <code>Err</code>, то вызываем макрос <code>panic!</code>, в котором выводит сообщение на консоль. Соответственно 
при выполнении этого макроса программа завершает свое выполнение.</p>
<p>Пример выполнения макроса panic при вводе некорректных данных:</p>
<img src="./pics/10.4.png" alt="enum Result and OK and Err in Rust" />
