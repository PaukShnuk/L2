...
<html >
<head>
<title>Rust | Модули</title>
<meta charset="utf-8" />
<meta name="description" content="Модули в языке программирования Rust, приватные функции, ключевое слово pub, определение и использование модулей">
...
<h1>Модули</h1><h2>Определение модулей. Приватность и публичность</h2><div class="date">Последнее обновление: 29.04.2021</div>
...
<p>Модули позволяют организовать код - определения функций, структур, трейтов, перечислений и т.д. в функциональные группы. 
Затем эти группы можно повторно использовать в различных частях программы как единое целое. Кроме того, модули позволяют управлять приватностью отдельных функциональных блоков, 
определять, какой функционал будет доступен вне модуля, а какой нет.</p>
<p>Определение начинается с ключевого слова <span class="b">mod</span>, за которым следует имя модуля и в фигурных скобках функционал модуля:</p>
<pre class="brush:rs;">
mod имя_модуля{
	// функционал модуля
}
</pre>
<p>Функционал модуля представляют функции, структуры, перечисления, трейты, перечисления, определения методов, которые в нем определены. Например, определение простейшего 
модуля:</p>
<pre class="brush:rs;">
mod my_module{			// определяем модуль
	pub fn test(){		// определяем публичную функцию - с атрибутом pub
		println! ("Hello from my_module");
	}
}
</pre>
<p>В данном случае модуль называется <code>my_module</code>. В этом модуле определена одна функция - <code>test()</code>, которая выводит строку на консоль. 
Определение этой функции начинается с ключевого слова <span class="b">pub</span>, которое позволяет использовать данную функцию вне модуля.</p>
<p>Для обращения к функционалу модуля применяется следующий синтаксис:</p>
<pre class="brush:rs;">имя_модуля::функциональность</pre>
<p>После имени модуля через два двоеточия :: указывается функциональность (например, функция), к которой происходит обращение.</p>
<p>Например, вызовем из выше определенного модуля функцию <code>test()</code>:</p>
<pre class="brush:rs;">
mod my_module{	// определяем модуль
	pub fn test(){		// определяем публичную функцию - с атрибутом pub
		println! ("Hello from my_module");
...
fn main(){
	my_module::test();	 // обращаемся к функции из модуля
}
</pre>
<p>Обращение к функции из модуля происходит также, как в общем случае, только названием функции указывается имя модуля и оператор <code>::</code>.</p>
<h3>Приватность и публичность</h3>
<p>Кроме общей организации функционала в общие группы модули позволяют установить приватность и публичность для вложенного функционала.</p>
<p>По умолчанию весь функционал модуля является приватным и доступен только внутри самого. Например, возьмем предыдущий пример и уберем в нем 
ключевое слово <code>pub</code> из определения функции:</p>
<pre class="brush:rs;">
mod my_module{
	fn test(){
		println!("Hello from my_module");
...
<p>При компиляции такой программы мы получим ошибку <code>"function `test` is private"</code>:</p>
<img src="./pics/8.1.png" alt="Модули и приватность в Rust" />
<p>Такую функцию мы можем использовать только внутри ее модуля.</p>
<p>Если какой-то функционал нужно использовать вне модуля, то в его определении необходимо использовать ключевое слово <span class="b">pub</span>, которое делает соответствующий 
функционал видимым вне его модуля:</p>
<pre class="brush:rs;">
mod my_module{
	fn private_test(){
		println!("Private test");
	}
	pub fn public_test(){
		println!("Public test");
		private_test();
...
fn main(){
	my_module::public_test();
	// my_module::private_test();	// !Ошибка -  функция private_test приватная
}
</pre>
<p>Здесь в модуле my_module определены две функции. Функция private_test приватная и может использоваться только внутри этого модуля. Например, 
мы можем вызвать ее в другой функции - public_test. А функция public_test - публичная и может вызываться вне модуля, например, в функции main.</p>
<h3>Структуры и методы в модулях</h3>
<p>При определении структур, их полей и методов следует учитывать, что они также по умолчанию приватные. Поэтому, если мы хотим, чтобы они были доступны 
вне модуля, также к ним необходимо применять ключевое слово <span class="b">pub</span>:</p>
<pre class="brush:rs;">
mod my_module{
...
<p>При обращении к структурам из другого модуля, перед их названием также указывается название модуля и оператор <code>::</code>.</p>
<h3>Перечисления в модулях</h3>
<p>Особенностью перечислений является то, что перед их константами не надо указывать слово <span class="b">pub<span>, чтобы сделать их доступными вне модуля:</p>
<pre class="brush:rs;">
mod my_module{
...
fn main(){
	let morning = my_module::DayTime::Morning;
	
    if let morning = my_module::DayTime::Morning{
...
<div class="nav"><p><a href="./7.3.php">Назад</a><a href="./">Содержание</a><a href="./8.2.php">Вперед</a></p></div></div>
<div class="socBlock">
