...
<html >
<head>
<title>Rust | Устройство памяти. Стек и куча</title>
<meta charset="utf-8" />
<meta name="description" content="Устройство памяти. Стек и куча heap в языке программирования Rust, размешение переменных в памяти">
...
<h2>Устройство памяти в Rust. Стек и куча</h2><div class="date">Последнее обновление: 14.04.2021</div>
...
<p>Вся память в Rust делится на два типа: <span class="b">стек (stack)</span> и <span class="b">куча (heap)</span>.</p>
<p>Стек устроен по принципу "LIFO" (last in, first out). Каждый новый добавляемый элемент располагается поверх предыдущего. Удаление начиется с верхушки стека. 
Первым удаляется элемент, который добавлен последним. А те элементы, которые были добавлены в стек первыми, удаляются последними. 
При этом данные, помещаемые в стек, должны иметь известный фиксированный размер. Таким образом, в стек помещаются данные примитивных типов 
i8, i16, i32, i64, isize, u8, u16, u32, u64, usize, f32, f64, bool, char, массивы, кортежи.</p>
<p>При вызове функции значения всех ее параметров и переменных помещаются в стек. Когда функция закончит свою работу, все данные удаляются 
из стека.</p>
<p>Например:</p>
<pre class="brush:rs;">
fn main(){
...
<p>В данном случае все три переменных имеют фиксированные размеры: целочисленные переменные x и y представляют тип <code>i32</code> (4 байта), а переменная z - тип 
<code>f64</code> (8 байт). Поскольку числовые данные имеют фиксированный размер, они размещаются в стеке. И в данном случае мы можем представить стек следующим образом:</p>
<img src="./pics/3.1.png" alt="Stack in Rust" />
<p>Куча или хип (heap) работает иначе. При размещении данных в куче распределитель памяти (memory allocator) находит в куче 
кусок памяти достаточного размера, чтобы вместить данные, и возвращает указатель - адрес этого куска в памяти, куда помещены данные.</p>
<p>Указатель имеет фиксированный размер - 8 байт, поэтому помещается в стеке. Однако непосредственные данные размещаются в куче. А для обращения к этим данным сначала идет обращение к указателю в стеке, а через указатель 
к самим данным, размещенным в куче.</p>
<p>Размещение данных в стеке происходит быстрее, чем размещение в куче. Так как распределителю памяти не надо искать свободное место 
достаточного размера, а данные всегда располагаются поверх уже имеющихся. В то время как выделение памяти в куче требует большей работы. 
Кроме того, доступ к данным в куче происходит медленнее, чем в стеке.</p>
...
<p>Здесь переменная <code>s1</code> представляет тип <span class="b">String</span>. Встроенная структура <span class="b">String</span> представляет строку, которые можно динамически изменять. А это значит, что нам точно неизвестно, 
сколько именно памяти будет занимать переменная этого типа. Поэтому непосредственно данные строки (набор байт) 
помещаются в кучу (heap).</p>
<p>Чтобы получить объект типа String, у строкового литерала "hello" вызывается метод <code>to_string()</code>.</p>
<p>В данном случае стек и кучу мы можем представить визуально следующим образом:</p>
<img src="./pics/3.2.png" alt="Куча heap in Rust" />
<p>В стек помещаются те данные переменной s1, которые имеют фиксированный размер. Это размер строки (<code>len</code>) - в данном случае 5 символов, 
размер буфера для строки (<code>capacity</code>) - в данном случае также 5 символов и указатель - адрес в куче, где собственно размещена 
строка. Например, это может быть адрес <code>0x2061aa10770</code>, по которому располагается строка "hello".</p>
