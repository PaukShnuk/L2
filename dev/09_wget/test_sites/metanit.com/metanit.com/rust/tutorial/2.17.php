...
<html >
<head>
<title>Rust | Структуры-кортежи</title>
<meta charset="utf-8" />
<meta name="description" content="Структуры-кортежи tuple structs в языке программирования Rust, оператор struct, определение структур-кортежей и обращение к их полям, ">
...
<h2>Структуры-кортежи</h2><div class="date">Последнее обновление: 12.04.2021</div>
...
<p>Отдельный тип представляют <span class="b">структуры-кортежи</span> (<span class="b">tuple structs</span>). 
Как и структуры они определяются с помощью ключевого слова <span class="b">struct</span>, но для их полей не указываются имена - только типы данных:</p>
<pre class="brush:rs;">struct название_структуры
{
	тип_поля1,
	тип_поля2,
	....
...
<p>В данном случае определена структура <code>Point</code>, которая представляет условную точку на плоскости и состоит из двух координат - двух полей 
типа <code>i32</code>.</p>
<pre class="brush:rs;">struct Point(i32, i32);</pre>
<p>При инициализации переменной структуры ее полям передаются некоторые значения, которые соответствуют типу полей:</p>
<pre class="brush:rs;">let start = Point(2, 4);</pre>
<p>Для обращения к полям такой структуры применяется тот же подход, что и в кортежах - обращение идет по индексу полей. То есть для обращения к первому полю применяется 
индекс 0</p>
<pre class="brush:rs;">start.0</pre>
<p>Для обращения ко второму полю применяется индекс 1 и так далее</p>
<pre class="brush:rs;">start.1</pre>
...
<p>Могут быть и более сложные конструкции, когда одна структура-кортеж использует другие:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Здесь структура-кортеж Distance использует другую структуру-кортеж Point. Соответственно чтобы обратитьсяк полям структуры Point внутри структры Distance, 
необходимо использовать два индекса:</p>
<pre class="brush:rs;">dist.0.0</pre>
<p><code>dist.0</code> указывает на первое поле структуры Distance, а <code>dist.0.0</code> на первое поле стуктуры Point, которая представляет первое поле структуры Distance.</p>
