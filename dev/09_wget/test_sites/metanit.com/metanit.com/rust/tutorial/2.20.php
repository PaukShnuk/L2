...
<html >
<head>
<title>Rust | Анонимные функции</title>
<meta charset="utf-8" />
<meta name="description" content="Анонимные функции в языке программирования Rust, опрееделение параметров, возвращение результата">
...
<h2>Анонимные функции</h2><div class="date">Последнее обновление: 20.04.2021</div>
...
<p><span class="b">Анонимные функции</span> в отличие от обычных функций не имеют имени и могут сохраняться в переменную.</p>
<p>Формальное определение анонимной функции:</p>
<pre class="brush:rs;">let переменная = | параметры | { 
	// выполняемые действия
 };</pre>
<p>Анонимная функция определяется без оператора <code>fn</code>. Ее определение фактически начинается с определения параметров, 
которые помещаются между двумя вертикальными чертами: <code>| параметры |</code>. Затем в фигурных скобках идут выполняемые 
действия. Причем после закрывающей фигурной скобки ставится точка с запятой. При этом анонимная функция сохраняется в переменной.</p>
<p>Вызов анонимной функции похож на вызов обычной функции, только для вызова применяется имя переменной:</p>
<pre class="brush:rs;">название_замыкания(значения_для_параметров);</pre>
...
<p>Здесь определена переменная <code>square</code>, которая хранит анонимную функцию. Эта функция принимает один параметр типа <code>i32</code> и 
выводит на консоль его квадрат.</p>
...
<p>Анонимные функции также могут возвращать значение:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Здесь определена переменная <code>sum</code>, которая представляет анонимную функцию, принимающую два параметра - два числа и возвращающую их сумму. 
И по имени переменной мы можем вызвать эту анонимную функцию и получить ее результат.</p>
<p>Если анонимная функция не принимает никаких параметров, то просто указываются две вертикальных черты:</p>
<pre class="brush:rs;">
...
<h3>Выведение типов</h3>
<p>В отличие от обычной функции в анонимной можно не указывать типы параметров и возвращаемого результата. Для анонимной функции компилятор сам 
способен определить типы параметров и результата. Например, мы могли бы переписать вышеопределенную анонимную функцию <code>sum</code> следующим образом:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Более того, так как анонимная функция содержит лишь одно выполняемое действие, то мы можем и фигурные скобки убрать:</p>
<pre class="brush:rs;">
fn main(){
    let sum = |a, b| a + b;
	let sum_of_5_and_4 = sum(5, 4);
...
<p>Однако стоит учитывать, что типы устанавливаются при первом вызове анонимной функции. И при последующих вызовах типы параметров и результатов 
анонимной функции должны соответствовать первому вызову. Например, в следующем случае мы получим ошибку при компиляции:</p>
<pre class="brush:rs;">
fn main(){
    let sum = |a, b| a + b;
...
<p>Потому что после первого вызова анонимной функции <code>sum()</code> компилятор ожидает, что далее она будет принимать два целых числа. Однако во втором вызове 
в данном случае передаются два числа с плавающей точкой.</p>
...
<div class="nav"><p><a href="./2.13.php">Назад</a><a href="./">Содержание</a><a href="./2.21.php">Вперед</a></p></div></div>
<div class="socBlock">
