...
<meta charset="utf-8" />
<meta name="description" content="Ownership в языке программирования Rust, смена владельца значения при присвоении переменных и передаче значений параметрам функции">
...
<h2>Ownership</h2><div class="date">Последнее обновление: 14.04.2021</div>
...
<p>Для управления данными в куче в Rust применяется такая концепция как <span class="b">ownership</span> 
(можно перевести как "владение", "принадлежность"). Она является одной из характерных возможностей и 
особенностей языка Rust, которая позволяет гарантировать безопасность памяти.</p>
<p>Ownership имеет следующие аспекты:</p>
<ul>
<li><p>Каждое значение в Rust имеет переменную, которая называется "владельцем" (owner).</p></li>
<li><p>У значения в один момент времени может быть только один владелец.</p></li>
<li><p>Когда владелец выходит за пределы области видимости/контекста, в котором он определен, его значение удаляется из памяти. Для этого 
Rust автоматически вызывает специальную функцию - <span class="b">drop()</span>, которая очищает память.</p></li>
</ul>
<p>Рассмотрим простейшую ситуацию:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Вначале определяем переменную <code>s1</code>, которая является владельцем строки "hello". Строка "hello" хранится в куче.</p>
<img src="./pics/3.2.png" alt="Куча heap in Rust" />
<p>Потом меняем владельца, присваивая значение s1 переменной s2.</p>
<pre class="brush:rs;">let s2 = s1;</pre>
<p>Но у значения может быть только один владелец. Поэтому s1 перестает быть владельцем. Таким образом, владение строкой "hello" переходит от переменной s1 к переменной s2.</p>
<img src="./pics/3.3.png" alt="Ownership in Rust" /> 
<p>А переменная <code>s1</code> теперь становится неинициализированной, и соответственно нам ее нельзя использовать. В итоге при компиляции этого кода мы получим 
ошибку на строке</p>
<pre class="brush:rs;">println!("s1: {}", s1);	// !Ошибка - s1 теперь нельзя использовать</pre>
<img src="./pics/3.4.png" alt="Ownership in Rust" />
<p>Хотя мы можем заново инициализировть переменную s1, присвоив ей какое-либо значение:</p>
<pre class="brush:rs;">
fn main(){
...
<h3>Смена владельца в функции</h3>
<p>Передача значения параметру в функцию аналогична присвоению значения переменной:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Здесь значение переменной <code>s1</code> передается параметру <code>message</code> при вызове функции <code>display_message()</code>:</p> 
<pre class="brush:rs;">display_message(s1);</pre>
<p>Это приводит к тому, что опять же переменная <code>s1</code> теряет владение над строкой "hello", которое переходит к параметру <code>message</code>.</p>
<p>В итоге опять же при компиляции мы получим ошибку на строке:</p>
<pre class="brush:rs;">println!("s1: {}", s1);		// !Ошибка - переменная s1 неинициализирована</pre>
