...
<html >
<head>
<title>Rust | Вложенные модули и ключевое слово super</title>
<meta charset="utf-8" />
<meta name="description" content="Вложенные модули и ключевое слово super в языке программирования Rust, обращение к родительскому модулю">
...
<h2>Вложенные модули и ключевое слово super</h2><div class="date">Последнее обновление: 29.04.2021</div>
...
<h3>Вложенные модули</h3>
<p>Одни модули могут содержать другие:</p>
<pre class="brush:rs;">
mod my_module{
...
<p>Стоит учитывать, что вложенный модуль, как и другой функционал модуля, по умолчанию приватный. Поэтому опять же, чтобы сделать модуль доступным из вне, он определяется 
с ключевым словом <span class="b">pub</span>. А при обращении к функционалу модуля указываются весь путь к функционалу с учетом внешнего и вложенных модулей:</p>
<pre class="brush:rs;">my_module::inner_module::test();</pre>
<h3>super</h3>
<p>Ключевое слово <span class="b">super</span> применяется для обращения к родительскому модулю. Через <code>super</code> мы можем обращаться в том числе и к приватным функциям родительского модуля:</p>
<pre class="brush:rs;">
mod parent_module{
...
<p>Здесь родительский модуль <code>parent_module</code> содержит приватную функцию <code>hello</code> и публичный модуль <code>child_module</code>. 
Внутри вложенного модуля <code>child_module</code> через <span class="b">super</span> мы можем обратиться к родительскому модулю <code>parent_module</code>, 
например, к его функции hello:</p>
<pre class="brush:rs;">super::hello();</pre>
