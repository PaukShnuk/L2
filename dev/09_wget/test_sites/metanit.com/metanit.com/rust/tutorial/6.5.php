...
<html >
<head>
<title>Rust | Статическое время жизни</title>
<meta charset="utf-8" />
<meta name="description" content="Статическое время жизни ссылок в функциях в языке программирования Rust, аннотация static">
...
<h2>Статическое время жизни</h2><div class="date">Последнее обновление: 25.04.2021</div>
...
<p>Rust имеет одну специальную встроенную аннотацию, которая называется <span class="b">'static</span> и которая означает, что ссылка будет существовать 
в течение всей программы.</p>
<p>Например, ранее была рассмотрена следующая программа:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Фактически здесь мы имеем дело со статическим временем жизни ссылки, которое равно времени выполнения программы. Почему? 
Строковые литералы сохраняются в бинарном виде и доступны в течение всего времени выполнения программы. Поэтому время жизни описывается как 
статическое. То есть ссылка живет в течение всей работы программы. И мы могли бы здесь применить аннотацию <span class="b">'static</span>:</p>
<pre class="brush:rs;">
fn main(){
	let message = get_message();
	println!("message: {}", message);
...
<p>Причем не важно, в какой области видимости создается ссылка, она будет доступна в течение все работы программы. Например:</p>
<pre class="brush:rs;">
fn main(){
	let message;
	{	
...
<p>Здесь ссылка возвращается во вложенном блоке кода, который по идее создается свою область видимости, однако по-прежнему проблем с 
компиляцией и выполнением программы не возникнет. Поскольку возвращается строковый слайс в виде строкового литерала со статическим временем жизни.</p>
