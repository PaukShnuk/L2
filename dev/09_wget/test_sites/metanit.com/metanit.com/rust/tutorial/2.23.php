...
<html >
<head>
<title>Rust | Тип функции</title>
<meta charset="utf-8" />
<meta name="description" content="Тип функции в языке программирования Rust, определение присвоение переменным функций">
...
<h2>Тип функции</h2><div class="date">Последнее обновление: 05.05.2021</div>
...
<p>В Rust функция может представлять отдельный тип данных. И в реальности мы даже можем определять переменные с типом функции и присваивать им конкретные функции, также 
как и анонимные. Что представляет тип функции? Тип функции состоит из типов параметров и типа возвращаемого результата 
функции.</p>
<pre class="brush:rs;">fn(тип_параметра1, тип_параметра2 ... тип_параметраN) -&gt; тип_результата</pre>
...
<p>Эта функция не имеет никаких параметров и ничего не возвращает. Тип такой функции можно описать как <span class="b">fn()</span> - после ключевого 
слова <code>fn</code> в скобках указываются типы параметров. Но в данном случае функция не имеет параметров, поэтому скобки пустые.</p>
<p>Допустим, у нас есть следующая анонимная функция:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Анонимная функция, которая представлена переменной <code>display</code>, в реальности имеет тип <span class="b">fn()</span>. То ест мы можем написать:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Но, допустим, функция принимает параметры:</p>
<pre class="brush:rs;">
fn message(text:&str){
	println!("{}", text);
}
</pre>
<p>Типом такой функции является <span class="b">fn(&amp;str)</span> - в скобках указывется тип параметра. То есть мы можем написать так:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Теперь рассмотрим пример, когда функция возвращает результат:</p>
<pre class="brush:rs;">
fn multiply(a: i32, b: i32) -&gt; i32{
	a * b
}
</pre>
<p>Функция принимает два параметра типа <code>i32</code> и возврашает объект типа <code>i32</code>. Тип результата также включается в тип функции. Поэтому 
типом данной функции будет <span class="b">fn(i32, i32) -&gt; i32</span>. То есть мы можем написать так:</p>
<pre class="brush:rs;">
fn main(){
