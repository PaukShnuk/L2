...
<html >
<head>
<title>Rust | Конструкция if..else</title>
<meta charset="utf-8" />
<meta name="description" content="Конструкция if..else в языке программирования Rust, выражение let-if, else if, определение альтернативных условий программы">
...
<h2>Конструкция if..else</h2><div class="date">Последнее обновление: 05.04.2021</div>
...
<p>Условная конструкция <span class="b">if..else</span> направляет ход программы по одному из возможных путей в зависимости от условия.</p>
<h3>Конструкция if</h3>
<p>Конструкция <span class="b">if</span> проверяет истинность условия, и если оно истинно, выполняет блок инструкций. 
Этот оператор имеет следующую сокращенную форму:</p>
<pre class="brush:rs;">
if условие
{
	инструкции;
...
<p>В качестве <span class="ii">условия</span> используется условное выражение, которое возвращает значение типа <span class="b">bool</span>: <span class="b">true</span> или 
<span class="b">false</span>. Если условие возвращает true, то выполняются последующие инструкции, которые входят в блок if. 
Если условие возвращает false, то инструкции из блока <code>if</code> не выполняются. Блок инструкций заключается в фигурные скобки.</p>
<p>Например:</p>
<pre class="brush:rs;">
fn main(){
	let number = 5;
...
<p>Здесь конструкция if проверяет истинность условия <code>number == 5</code>. Если оно возвращает <code>true</code>, то выполняется инструкция 
<code>println!("Условие истинно: number равно 5");</code>, которая выполняет вывод строки на консоль. И поскольку в данном случае переменная 
<code>number</code> действительно равна 5, то будут выполняться инструкции из блока <code>if</code>, и мы увидим на консоли соответствующее сообшение:</p>
<div class="console"><pre class="consoletext">Условие истинно: number равно 5</pre></div>
...
<p>Здесь конструкция <code>if</code> проверяет истинность условия <code>number &lt; 3</code> (меньше ли значение переменной number числа 3). Поскольку number равно 5, 
то это условие ложно и возвращает <code>false</code>, соответственно инструкции из блока if не будут выполняться.</p>
<h3>else</h3>
...
<p>После оператора else мы можем определить набор инструкций, которые выполняются, если условие после оператора if возвращает <code>false</code>. То есть если 
<span class="ii">условие</span> истинно, выполняются инструкции блока <code>if</code>, а если это выражение ложно, то выполняются инструкции блока <code>else</code>.</p>
<pre class="brush:rs;">
fn main(){
	let number = 5;
...
<p>В данном случае условие <code>number &gt; 6</code> ложно, то есть возвращает false, поэтому будет выполняться блок else. 
И в итоге на консоль будет выведена строка</p>
<div class="console"><pre class="consoletext">Условие ложно: number НЕ больше 6</pre></div>
<h3>else if</h3>
<p>Однако нередко надо обработать не два возможных альтернативных варианта, а гораздо больше. Например, в случае выше можно насчитать три условия: 
переменная number может быть больше 6, меньше 6 и равна 6. Для проверки альтернативных условий мы можем вводить выражение <span class="b">else if</span>, 
после которого указывается альтернативное условие:</p>
<pre class="brush:rs;">
fn main(){
	let number = 5;
...
<p>То есть в данном случае мы получаем три ветки развития событий в программе. И поскольку условие в выражении <code>else if number &lt; 6</code> возвращает 
<code>true</code>, то будут выполняться инструкции из блока <code>else if</code>:</p>
<div class="console"><pre class="consoletext">number меньше 6</pre></div>
<p>При необходимости можно добавлять и большее количество выражений <code>else if</code> для проверки альтернативных условий:</p>
<pre class="brush:rs;">
fn main(){
	let number = 5;
...
<h3>if let</h3>
<p>Конструкция <span>let-if</span> позволяет получить в переменную результат конструкции <code>if..else</code>:</p>
<pre class="brush:rs;">
...
<p>Здесь если условие, которое представлено переменной <code>condition</code> равно <code>true</code>, то number равно 4. Если condition равно 
<code>false</code>, то number будет равно 5. И поскольку здесь <code>condition = true</code>, то number будет равно 4</p>
...
<p>В данном случае если переменная operation равна <span class="b">1</span>, то number равно результату операции <code>a + b</code>. 
Если operation равна <span class="b">2</span>, то number равно <code>a - b</code>. В остальных случаях number равно <code>a * b</code>.</p>
<p>Поскольку здесь <code>operation = 2</code>, то <code>number = a - b</code>, то есть 5 - 2 = 3.</p>
