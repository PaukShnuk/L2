...
<html >
<head>
<title>Rust | Конструкция match</title>
<meta charset="utf-8" />
<meta name="description" content="Конструкция match в языке программирования Rust, возвращение результата">
...
<h2>Конструкция match</h2><div class="date">Последнее обновление: 06.04.2021</div>
...
<p>Конструкция <span class="b">match</span> оценивает некоторое выражение и сравнивает его с набором значений. И совпадении значений выполняет определенный код.</p>
<p>Конструкция <span class="b">match</span> имеет следующее формальное определение:</p>
<pre class="brush:rs;">
match выражение 
{
	значение1 =&gt; действия1,
...
<p>После оператора <span class="b">match</span> указывается сравниваемое выражение, например, переменная. Далее идет перечисление значений, с 
которыми идет сравнение.</p>
<p>После каждого значения после оператора <span class="b">=&gt;</span> идут действия, которые выполняются, если это значение соответствует сравниваемому выражению.</p>
<p>В конце после всех значений указывается универсальное значение <code>_</code>, действия которого выполняются, если ни одно из значений не соответствуют сравниваемому выражению.</p>
<p>Например:</p>
<pre class="brush:rs;">
fn main(){
	let num = 2;
...
<p>Здесь в конструкции <code>match</code> значение переменной <code>num</code> последовательно сравнивается со значениями 1, 2, 3. Поскольку переменная 
<code>num</code> равна 2, то она соответствует значению</p>
<pre class="brush:rs;">2=&gt;println! ("два"),</pre>
<p>Соответственно на консоль будет выведена строка "два".</p>
<p>Возьмем другую ситуацию:</p>
<pre class="brush:rs;">
fn main(){
	let num = 23;
	match num
...
<p>Здесь значение переменной num не соответствует ни одному из значений в конструкции match, поэтому будет применяться код</p>
<pre class="brush:rs;">_=&gt;println! ("непонятно")</pre>
<p>Конструкция match может возвращать значение. В этом случае мы можем присвоить его переменной:</p>
<pre class="brush:rs;">
fn main(){
	let num = 3;
...
<p>Здесь результат конструкции match присваивается переменной <code>result</code>. Возвращаемые значения указываются после оператора <span class="b">=&gt;</span>. 
В данном случае это строки "один", "два", "три", "непонятно". Поскольку переменная num = 3, то конструкция match возвратит строку "три".</p>
