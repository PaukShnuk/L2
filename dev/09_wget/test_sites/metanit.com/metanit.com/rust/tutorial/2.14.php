...
<html >
<head>
<title>Rust | Кортежи</title>
<meta charset="utf-8" />
<meta name="description" content="Кортежи tuple в языке программирования Rust, доступ к элементам кортежа, декомпозиция кортежа на переменные, кортежи в конструкции match">
...
<h1>Составные типы данных</h1><h2>Кортежи</h2><div class="date">Последнее обновление: 27.04.2021</div>
...
<p><span class="b">Кортеж (tuple)</span> представляет способ группировки данных разных типов в один составной тип. 
Кортежи имеют фиксированный тип в соответствии с вложенными данными. 
После определения кортежа его размер нельзя ни уменьшить, ни увеличить.</p>
<p>Формальный синтаксис определения кортежа:</p>
<pre class="brush:rs;">let tuple = (значение1, значение2, ... значениеN);</pre>
<p>Кортеж определяется в круглых скобках, внутри которых через запятую перечисляются составляющие кортеж значения.</p>
<p>Определение простейшего кортежа:</p>
<pre class="brush:rs;">
let user = ("Tom", 36, 1.78);
</pre>
<p>Здесь переменная <code>user</code> представляет кортеж, который состоит из трех элементов.</p>
<p>В примере выше тип элементов кортежа выводится автоматически, но также мы можем явно задать их типы:</p>
<pre class="brush:rs;">
let user: (&str, u8, f32) = ("Tom", 36, 1.78);
</pre>
<p>То есть в данном случае кортеж имеет тип <code>(&str, u8, f32)</code>, а это значит, что первый его элемент должен представлять строку, 
второй элемент - число типа <code>u8</code>, а третий - число с плавающей точкой типа <code>f32</code>.</p>
<p>После объявления кортежа мы можем обращаться к его элементам в виде:</p>
<pre class="brush:rs;">
название_кортежа.индек_элемента
</pre>
<p>После названия кортежа через точку указывается индекс элемента кортежа, при этом индексация начинается с нуля. Например:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Также мы можем изменять элементы кортежа, если его переменная определена с ключевым словом <span class="b">mut</span>:</p>
<pre class="brush:rs;">
fn main(){
...
<h3>Декомпозиция</h3>
<p>Rust предоставляет такую возможность, как декомпозиция (destructuring) кортежа. Например, мы могли бы следующим образом 
присвоить значения кортежа переменным:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Но декомпозиция позволяет сократить код:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Но, возможно, нам нужны не все данные из кортежа. Например, нам не нужно получать рост человека. Тем не менее мы не можем написать, например, так:</p>
<pre class="brush:rs;">let (name, age) = user;	// !Ошибка</pre>
<p>Нам обязательно надо указать столько же переменных, сколько значений в кортеже. Однако если какое-то значение нам не нужно, мы можем вместо соответствующей переменной указать прочерк:</p>
<pre class="brush:rs;">let (name, age, _) = user;</pre>
<h3>Сравнение кортежей</h3>
<p>Для сравнения кортежей применяется операция <code>==</code>. Кортежи считаются равными, если равны их соответствующие значения. Пример сравнения кортежа:</p>
<pre class="brush:rs;">
fn main() {
...
<div class="nav"><p><a href="./2.24.php">Назад</a><a href="./">Содержание</a><a href="./2.15.php">Вперед</a></p></div></div>
<div class="socBlock">
