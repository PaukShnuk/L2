...
<html >
<head>
<title>Rust | Макрос panic!</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка ошибок в языке программирования Rust, Макрос panic!">
...
<h1>Обработка ошибок</h1><h2>Макрос panic!</h2><div class="date">Последнее обновление: 04.05.2021</div>
...
<p>В Rust все ошибки делятся на две категории: <span class="b">recoverable (обрабатываемые)</span> и <span class="b">unrecoverable (необрабатываемые)</span>. 
 При возникновении обрабатываемой ошибки, (например, отсутствие запрошенного файла) можно сообщить о проблеме 
 пользователю и вернуться к нормальной работе программы (если файл отсуствует - то можно обработать - создать новый файл). Необрабатываемые ошибки (например, обращение к несуществующему элементу вектора) нередко являются показателями наличия в программе багов, в такой ситуации сложно как-то обработать ситуацию.</p>
 <p>Для обработки обрабатываемых ошибок Rust предоставляет специальный тип <span class="b">Result&lt;T, E&gt;</span>, а для обработки ситуации, 
 когда возникает необрабатываемая ошибка - макрос <span class="b">panic!</span>. Хотя оба средства также могут сочетаться в рамках обработки одной ошибки.</p>
<h2>Макрос panic!</h2>
<p>Для обработки ситуаций, когда возникает необрабатываемая ошибка, применяется макрос <span class="b">panic!</span>. При его выполнении программа выводит на консоль сообщение об ошибке, очищает стек и завершает выполнение программы.</p>
<p>Ряд потенциальных необрабатываемых ошибок Rust может отловить на этапе компиляции, например, возьмем ситуацию, когда мы обращаемся к несуществующему элементу массива:</p>
<pre class="brush:rs;">
fn main() {
	let numbers = [1, 2, 3, 4];
    println!("{}", numbers[10]);
...
<p>В данном случае в массиве numbers 4 элемента, соответственно элемента с индексом 10, который мы пытаемся вывести на консоль, в массиве не существует. 
И при компиляции мы получим ошибку:</p>
<img src="./pics/10.2.png" alt="Uncoverable errors in Rust" />
<p>Однако изменим тип данных с массива на вектор:</p>
<pre class="brush:rs;">
fn main() {
...
<p>В векторе также 4 элемента, а элемента с индексом 10 не существует. Однако вектор может увеличиваться, и компилятор не сможет отловить возникновение ошибки. 
Поэтому компиляция завершится успешно, однако при выполнении программы мы все равно столкнемся с ошибкой:</p>
<img src="./pics/10.3.png" alt="panic! in Rust" />
<p>Это как раз и результат выполнения макроса <code>panic!</code>, которое прописано в исходном коде. В выводимом на консоль макросом сообщении 
...
исправления.</p>
<p>Но мы сами можем вызывать данный макрос в различных частях программы для обработки подобных ситуаций. 
Например, для имитации ошибки просто применим этот макрос:</p>
<pre class="brush:rs;">
fn main() {
    panic!("непредвиденная ошибка");
}
</pre>
<p>В макрос <code>panic!</code> передается сообщение, которое затем будет выводиться на консоль при возникновении ошибки. Так, при выполнении этой программы 
мы увидим на консоли наше сообщение и краткую информацию о возникшей ошибке:</p>
<img src="./pics/10.1.png" alt="Макрос panic в Rust" />
<p>Рассмотрим другой более практичный пример: у нас есть функция, которая извне получает некоторые данные. Однако эти данные могу быть некорректными. 
И в этом случае мы можем вызвать макрос <code>panic</code>:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Здесь функция <code>create_person()</code> получает извне имя и возраст пользователя и по ним создает объект структуры Person, который возвращается 
...
макрос <code>panic!</code>, который с помощью передаваемого сообщения раскравает причину ошибки.</p>
...
<div class="nav"><p><a href="./8.4.php">Назад</a><a href="./">Содержание</a><a href="./10.2.php">Вперед</a></p></div></div>
<div class="socBlock">
