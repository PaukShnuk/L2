...
<html >
<head>
<title>Rust | Trait как параметр и результат функции</title>
<meta charset="utf-8" />
<meta name="description" content="Trait как параметр и результат функции в языке программирования Rust, оператор impl">
...
<h2>Trait как параметр и результат функции</h2><div class="date">Последнее обновление: 21.04.2021</div>
...
<p>Как и другие типы, трейты могут использоваться в качестве типа данных для параметров и возвращаемого результата функции.</p>
<h3>Trait как тип параметров</h3>
<p>Формальная установка трейта в качестве типа данных для параметра:</p>
<pre class="brush:rs;">название_параметра: &impl название_трейта</pre>
<p>После двоеточия идет оператор <span class="b">impl</span>, а затем указывается название трейта.</p>
<p>Причем обычно передается не просто объект трейта, а ссылка на него, чтобы не менять владельца данных. Поэтому перед оператором <span class="b">impl</span> указывается 
амперсанд <code>&amp;</code></p>
<p>Например:</p>
<pre class="brush:rs;">
...
<p>В данном случае имеются две структуры Person и Message. Они разные по своему характеру, представляют совершенно разные данные: одна структура представляет 
...
<code>Printer</code> с одним методом <code>print()</code></p>
<pre class="brush:rs;">
trait Printer{
	fn print(&self);
}
</pre>
<p>Каждая из структур по своему реализует этот метод.</p>
<p>Затем в функции <code>display()</code> мы можем получить ссылку на объект трейта:</p>
<pre class="brush:rs;">
fn display(printable: &impl Printer) {
    printable.print();
}
</pre>
<p>Причем для этой функции не важно, ссылка на какой объект будет передаваться параметру. Главное, чтобы он реализовал методы трейта Printer. Сообственно 
запись <code>&impl Printer</code> и указывается, что это должна быть ссылка на объект, который реализует трейт Printer.</p>
<p>Затем в функции main создаются два объекта, ссылки на которые передаются в функцию <code>display()</code>:</p>
<pre class="brush:rs;">
let tom = Person {name: String::from("Tom"), age: 36};
let hello = Message { title: String::from("Hello Rust")};
...
<h3>Trait как тип результата</h3>
<p>Трейт как тип результата определяется следующим образом:</p>
<pre class="brush:rs;">fn название_функции() -&gt; impl название_трейта {</pre>
<p>После оператора <span class="b">-&gt;</span> указывается ключевое слово <span class="b">impl</span>, а затем идет название трейта. 
То есть, как бы говориться, что возвращаемый объект должен реализовать указанный трейт.</p>
<p>Например:</p>
<pre class="brush:rs;">
...
<p>В данном случае структура TextMessage, которая представляет некоторое текстовое сообщение, реализует трейт Sender, в частности, его метод <code>send()</code>.</p>
<p>Определенная здесь функция <code>create_message()</code> в качестве результата возвращает объект трейта Sender:</p>
<pre class="brush:rs;">
fn create_message(addr: &str) -&gt; impl Sender{
	TextMessage {address: String::from(addr), text: String::from("Привет, ты спишь?") }
}
</pre>
<p>Поскольку структура TextMessage реализует трейт Sender, то функция может возвратить объект этой структуры.</p>
<p>В функции main вызываем функцию <code>create_message()</code>, получаем ее результат - объект структуры TextMessage и выполняем ее метод <code>send()</code>:</p>
<pre class="brush:rs;">
let text_message = create_message("sam@gmail.com");
text_message.send();
</pre>
<p>Однако, эта возможность имеет существенные ограничения. Так, функция должна возвращать объкты только одного и того же типа. Например, 
в следующем случае мы получим ошибку на этапе компиляции:</p>
<pre class="brush:rs;">
...
<p>В данном случае добавлена структура VoiceMessage, которая тоже реализует трейт Sender. А функция <code>create_message()</code> теперь в зависимости 
...
разных типов, что не допускается.</p>
