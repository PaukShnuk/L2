...
<html >
<head>
<title>Rust | Введение</title>
<meta charset="utf-8" />
<meta name="description" content="Общий обзор языка программирования Rust, его основные особенности, история развития, установка на Windows">
...
<h1>Введение в язык Rust</h1><h2>Общий обзор языка программирования Rust. Установка</h2><div class="date">Последнее обновление: 05.03.2022</div>
...
<p>Rust представляет язык программирования, разработанный в компании Mozilla для большей производительности и безопасности. Его сфера применения довольнно широка. 
...
его можно использовать для написания прикладных программ и веб-приложений.</p>
<p>На сегодняшний день довольно много компаний применяют Rust в своих разработках. В частности, достаточно назвать такие, как Dropbox, Microsoft, Mozilla, Coursera, Facebook, 
Amazon, Discord, Deno.</p>
<p>Некоторые особенности языка Rust:</p>
<ul>
<li><p>Rust является компилируемым языком со статической типизацией. А это значит, что типы всех используемых в программе данных определяются в момент компиляции, 
благодаря чему становится сложнее написать некоректные программы.</p></li>
<li><p>В плане синтаксиса Rust напоминает сиподобные языки, в частности, С/С++. Поэтому тем, кто знаком с каким-то сиподобным языком, особенно, если это 
C или C++ будет проще при знакомстве с синтаксисом Rust.</p></li>
<li><p>Имеющаяся в Rust система управления памятью в отличие от C/C++ позволяет более безопасно работать с памятью.</p></li>
</ul>
<h4>История развития</h4>
<p>Изначальный автор языка - Грейдоном Хоаром (Graydon Hoare), который начал развивать Rust как личный проект в 2006 году и который работал в компании 
Mozilla. В 2009 году Mozilla начала спонсоровать проект.</p>
<p>Первая стабильная версия языка - Rust 1.0 вышла 15 мая 2015 года. На момент написания данной статьи последней версией является версия Rust 1.59.</p>
<p>8 февраля 2021 года пятью компаниями (AWS, Huawei, Google, Microsoft и Mozilla) была образована независимая некоммерческая организация 
Rust Foundation, основной задаче которой является поддержание языка Rust и развитие его экосистемы.</p>
<p>Стоит отметить, что Rust также является opensource-проектом - весь связанный с ним код можно найти в репозитории на github - <a href="https://github.com/rust-lang" rel="nofollow">https://github.com/rust-lang</a></p>
<h3>Установка для Windows</h3>
<p>Для установки необходимого инструментария для программирования на языке Rust перейдем на страницу 
<a href="https://www.rust-lang.org/tools/install" rel="nofollow">https://www.rust-lang.org/tools/install</a>. 
Найдем на странице кнопку <span class="b">Download Rustup-init.exe (64-bit)</span>, если у нас 64-битная версия (как в большинстве случаев), 
либо <span class="b">Download Rustup-init.exe (32-bit)</span> (если система 32-х битная):</p>
<img src="./pics/1.1.png" alt="Установка Rust и Rustup на Windows" />
<p>После нажатия на кнопку на компьютер будет загружен файл установщика. После загрузки нажмем на этот файл:</p>
<img src="./pics/1.2.png" alt="Установщик Rustup на Windows" />
<p>Установщик предложит нам на выбор три опции. Опция с номером <span class="b">1</span> представляет установку. Поэтому введем единицу:</p>
<img src="./pics/1.3.png" alt="Установка Rust" />
<p>В конце программа установки радостно нам сообщит, что Rust успешно установлен, и предложит нам нажать на Enter для завершения работы мастера установки:</p>
<img src="./pics/1.4.png" alt="Rust и Visual Studio" />
<p>Нажмем на Enter, и все - мы можем приступать к созданию программ на Rust.</p>
<h3>Установка для Mac OS и Linux</h3>
<p>Для установки необходимого инструментария для программирования на языке Rust на Mac OS/Linux в терминале необходимо выполнить команду:</p>
<div class="console"><pre class="consoletext">curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh</pre></div>
<h3>Офлайн-установка</h3>
<p>При необходимости также можно выполнить офлайн-установку с помощью одного из установщиков для каждой из поддерживаемых операционных систем и архитектур со странице <a href="https://forge.rust-lang.org/infra/other-installation-methods.html" rel="nofollow">https://forge.rust-lang.org/infra/other-installation-methods.html</a></p>
<h3>Версия и обновление Rust</h3>
<p>Стоит отметить, что постоянно выходят новые подверсии Rust, и чтобы обновить необходимый инструментарий, необходимо в консоли набрать следующую команду:</p>
<div class="console"><pre class="consoletext">rustup update</pre></div>
<p>Проверить номер текущей установленной версии можно с помощью следующей команды:</p>
<div class="console"><pre class="consoletext">rustc --version</pre></div>
