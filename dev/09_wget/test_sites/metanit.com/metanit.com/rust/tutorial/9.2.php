...
<html >
<head>
<title>Rust | Загрузка и использование внешних зависимостей</title>
<meta charset="utf-8" />
<meta name="description" content="Загрузка и использование внешних зависимостей с помощью Cargo в языке программирования Rust, пакет rand и генерация случайных чисел">
...
<h2>Загрузка и использование внешних зависимостей</h2><div class="date">Последнее обновление: 05.05.2021</div>
...
<p>Рассмотрим, как мы можем с помощью Cargo загружать и использовать внешние зависимости.</p>
<p>Следует сказать, что кроме собственно стандартной библиотеки типов и функциональностей, которая поставляется вместе с компилятором Rust и которые мы можем 
использовать по умолчанию в нашей программе, также есть куча различных пакетов, которые разрабатываются различными разработчиками по всему миру. 
Для хранения подобных пакетов существует централизованная система - <a href="https://crates.io/" rel="nofollow">https://crates.io/</a>. И мы можем использовать эти пакеты в своей программе.</p>
<p>Рассмотрим на примере пакета <span class="b">https://crates.io/crates/rand</span>, который позволяет задействовать в Rust случайные числа. Возьмем проект Cargo, созданный в прошлой теме 
(или создадим новый).</p>
<p>Если проект уже запускался, он будет выглядеть следующим образом:</p>
<img src="./pics/9.5.png" alt="Dependency in Rust" />
<p>В любом случае нас в проекте будет интересовать файл конфигурации проекта <span class="b">Cargo.toml</span> и файл кода <span class="b">main.rs</span> из папки <span class="b">src</span>.</p>
<p>Прежде всего укажем в файле <span class="b">Cargo.toml</span> зависимость, которую надо загрузить и которую мы собираемся использовать. По умолчанию он выглядит примерно таким образом:</p>
<pre class="brush:rs;">
[package]
name = "hello"
version = "0.1.0"
...
<p>Конкретные данные в случае имени проекта и авторов могут отличаться, однако секция <code>[dependencies]</code> по умолчанию пуста. Именно в этой секции 
указываются внешние пакеты, которые надо загрузить.</p>
<p>Изменим данную секцию следующим образом:</p>
<pre class="brush:rs;">
[dependencies]
rand = "0.8.3"
</pre>
<p>Вначале идет название крейта или пакета - "rand", а после знака равно его версия. То есть в моем случае весь файл Cargo.toml будет выглядеть следующим образом:</p>
<pre class="brush:rs;">
[package]
name = "hello"
version = "0.1.0"
...
<p>Теперь перейдем к файлу <span class="b">main.rs</span> и изменим его код следующим образом:</p>
<pre class="brush:rs;">
...
<p>Вначале здесь подключается тип (трейт) <span class="b">rand::Rng</span> из пакета <code>rand</code>. Трейт <code>Rng</code> определяет ряд методов, в частности, 
метод <span class="b">gen_range()</span>, который принимает в качестве параметра последовательность и возвращает случайное число из этой последовательности.</p>
<p>Чтобы получить случайное число, сначала обращаемся к функции <span class="b">rand::thread_rng()</span>, которая возврашает объект структуры 
<span class="b">rand::rngs::ThreadRng</span>. Для этой структуры реализован трейт <code>rand:Rng</code>, и соответственно через ее объект можно вызвать 
метод <code>gen_range()</code> для генерации случайного числа:</p>
<pre class="brush:rs;">let random_number = rand::thread_rng().gen_range(0..10);</pre>
<p>То есть в данном случае мы получим случайное число из диапазона от 0 до 10.</p>
<p>Теперь построим и запустим проект командой</p>
<div class="console"><pre class="consoltext">cargo run</pre></div>
<img src="./pics/9.6.png" alt="Загрузка и компиляция пакетов в Rust через Cargo" />
<p>При построении проекта Cargo проверяет секцию <code>[dependencies]</code> и загружает все крейты (пакеты), которые отсутствуют. Кроме того, загружаются 
пакеты, от которых зависит загружаемый пакет. После загузки пакетов Rust компилирует их и затем компилирует проект.</p>
<p>И таким образом, после запуска проекта мы увидим на консоли некоторое случайное число.</p>
<p>Если после последнего запуска никаких изменений в файл Cargo.toml не было произведено, то Cargo ничего заново не загружает и не компилирует, а использует 
ранее загруженные и скомпилированные пакеты. Если в код не было внесено никаких изменений, то при запуске через Cargo он также не перекомпилируется.</p>
<p>Однако что, если вышла новая версия пакета, и мы хотим использовать ее вместо старой? В этом случае нам достаточно изменить версию пакета в <code>Cargo.toml</code> 
и выполнить команду:</p>
<div class="console"><pre class="consoletext">cargo update</pre></div>
...
<div class="nav"><p><a href="./9.1.php">Назад</a><a href="./">Содержание</a><a href="./11.1.php">Вперед</a></p></div></div>
<div class="socBlock">
