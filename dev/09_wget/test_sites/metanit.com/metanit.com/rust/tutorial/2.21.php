...
<html >
<head>
<title>Rust | Замыкания</title>
<meta charset="utf-8" />
<meta name="description" content="Анонимные функции и замыкания closure в языке программирования Rust, лексическое окружение функций">
...
<h2>Замыкания</h2><div class="date">Последнее обновление: 20.04.2021</div>
...
<p><span class="b">Замыкания</span> или closures в Rust представляют анонимные функции, которые могут запоминать свое окружение. По сути любая анонимная функция может 
запоминать свое окружение и поэтому является замыканием.</p>
<p>Когда замыкание получает значение из своего окружения, оно сохраняется в памяти для дальнейшего использования в теле замыкания. Рассмотрим простейший 
пример:</p>
<pre class="brush:rs;">
fn main(){
...
<p>В данном случае переменная <code>message</code> представляет часть лексического окружения, которое доступно замыканию.</p>
<pre class="brush:rs;">let message = "Hello Rust";	// переменная - внешнее окружение замыкания</pre>
<p>Далее определяется замыкание <code>hello</code>, которое может обращаться к этому окружению:</p>
<pre class="brush:rs;">
let hello =||{				// определение замыкания
	println!("{}", message);
};
</pre>
<p>Затем мы можем вызвать замыкание, которое использует переменную из внешнего окружения:</p>
<pre class="brush:rs;">hello();	// вызов замыкания</pre>
<p>В итоге на консоль будет выведена строка "Hello Rust".</p>
<p>Если замыкание должно изменять значение внешней переменной, то и эта внешняя переменная и переменная-замыкание должны быть определены со ключевым словом 
...
<p>Здесь замыкание <code>increase()</code> увеличивает значение переменной <code>n</code>. Соответственно и переменная <code>n</code>, и переменная <code>increase</code> 
определены как изменяемые.</p>
...
n = 7
n = 8
Переменная n в функции main равна 8
</pre>
</div>
...
<div class="nav"><p><a href="./2.20.php">Назад</a><a href="./">Содержание</a><a href="./2.23.php">Вперед</a></p></div></div>
<div class="socBlock">
