...
<html >
<head>
<title>Rust | Аннотации ссылок в структурах</title>
<meta charset="utf-8" />
<meta name="description" content="Аннотации и определение времени жизни ссылок в структурах в языке программирования Rust, зависимость от передаваемых данных">
...
<h2>Аннотации ссылок в структурах</h2><div class="date">Последнее обновление: 25.04.2021</div>
...
<p>Подобно тому, как аннотации времени жизни применяются в функциях, они также могут применяться в структурах. Формальное применение аннотаций:</p>
<pre class="brush:rs;">
struct название_структуры&lt;'аннотация&gt; {
    поле_структуры: &amp;'аннотация str,
}
</pre>
<p>После названия структуры в угловых скобках идет название аннотации. Если поле структуры хранит ссылку, то после знака амперсанда &amp;, но перед типом данных указывается название аннотации.</p>
<p>Сначала рассмотрим проблему, с которой мы можем столкнуться. Пусть у нас есть структура, поле которой представляет строковый слайс, то есть ссылку:</p>
<pre class="brush:rs;">
struct Person {
    name: &amp;str,
}
...
<p>Но при компиляции компилятор Rust укажет, что необходимо установить аннотации времени жизни для поля <code>name</code> в структуре Person:</p>
<img src="./pics/3.11.png" alt="reference lifetime in struct in Rust" />
<p>Теперь применим аннотации:</p>
<pre class="brush:rs;">
struct Person&lt;'a&gt; {
    name: &amp;'a str,
}
...
<p>Здесь структура применяет аннотацию <code>'a</code>. Применение аннотации означает, что объект структуры будет доступен пока доступна ссылка из 
поля name. Что это значит? Рассмотрим следующий пример:</p>
<pre class="brush:rs;">
struct Person&lt;'a&gt; {
    name: &amp;'a str,
}
...
<p>Здесь переменная tom определена с ключевым словом <code>mut</code>, чтобы теоретически можно было поменять значения ее полей.</p>
<p>Во вложенном блоке кода создается объект <code>username</code>, который представляет тип <code>String</code>. То есть время жизни этого объекта 
ограничено вложенным блоком кода.</p> 
<p>Далее полю <code>name</code> объекта структуры присваивается строковый слайс, полученный с помощью метода <code>as_str()</code> из объекта username.</p>
<pre class="brush:rs;">tom = Person { name: username.as_str()};</pre>
<p>После завершения вложенного блока кода мы пытаемся изменить значения поля <code>name</code> - переменная tom же определена как изменяемая. Однако проблема в том, 
что мы не можем использовать объект Person, представленный переменной tom, вне области видимости переменной <code>username</code>. Соответственно при компиляции мы получим ошибку:</p>
<img src="./pics/3.12.png" alt="время жизни ссылки в структурах в Rust" />
<p>Мы могли бы исправать ситуацию, перенеся весь код использования объекта tom в область видимости переменной username:</p>
<pre class="brush:rs;">
struct Person&lt;'a&gt; {
    name: &amp;'a str,
}
...
<p>Либо создать новый объект структуры Person и присвоить его переменной tom:</p>
<pre class="brush:rs;">
struct Person&lt;'a&gt; {
    name: &amp;'a str,
}
...
<p>Правда, в данном случае мы уже будем работать с уже другим объектом.</p>
