...
<html >
<head>
<title>Rust | Типы данных</title>
<meta charset="utf-8" />
<meta name="description" content="Типы данных в языке программирования Rust, целочсленные типы i8, i16, i32, i64, isize, u8, u16, u32, u64, usize, двоичны, восьмеричный и шестнадцатеричный формат, дробные числа f32 и f64, bool, char, символы и строки &str">
...
<h2>Типы данных</h2><div class="date">Последнее обновление: 03.04.2021</div>
...
<p>Rust является языком со статической типизацией, а это значит, что для каждой переменной должен быть определен тип данных. 
...
могут занимать.</p>
<p>Типы данных бывают примитивными (или скалярными) и составными. Рассмотрим примитивные типы данных.</p>
<h3>Целочисленные типы данных</h3>
<p>Ряд типов данных может хранить целые числа:</p>
<ul>
<li><p><span class="b">i8</span>: хранит целое число со знаком и занимает в памяти 8 бит (диапазон значений от -128 до 127)</p></li>
<li><p><span class="b">u8</span>: хранит положительное целое число и занимает в памяти 8 бит (диапазон значений от 0 до 255)</p></li>
<li><p><span class="b">i16</span>: хранит целое число со знаком и занимает в памяти 16 бит</p></li>
<li><p><span class="b">u16</span>: хранит положительное целое число и занимает в памяти 16 бит</p></li>
<li><p><span class="b">i32</span>: хранит целое число со знаком и занимает в памяти 32 бита</p></li>
<li><p><span class="b">u32</span>: хранит положительное целое число и занимает в памяти 32 бита</p></li>
<li><p><span class="b">i64</span>: хранит целое число со знаком и занимает в памяти 64 бита</p></li>
<li><p><span class="b">u64</span>: хранит положительное целое число и занимает в памяти 64 бита</p></li>
<li><p><span class="b">i128</span>: хранит целое число со знаком и занимает в памяти 128 бит</p></li>
<li><p><span class="b">u128</span>: хранит положительное целое число и занимает в памяти 128 бит</p></li>
<li><p><span class="b">isize</span>: хранит целое число со знаком, размерность которого зависит от архитектуры компьютера - 
64 бита на 64-битной архитектуре, и 32 бита на 32-битной архитектуре</p></li>
<li><p><span class="b">usize</span>: хранит положительное целое число, размерность которого зависит от архитектуры компьютера - 
64 бита на 64-битной архитектуре, и 32 бита на 32-битной архитектуре</p></li>
</ul>
<p>Как видно из списка типов у них логичные названия - сначала идет буква "i", если число может быть положительным и отрицательным, либо буква 
"u", если число может быть только положительным. Далее идет размерность числа в битах - какой размер оно занимает в памяти.</p>
<p>Примеры применения типов:</p>
<pre class="brush:rs;">
fn main(){
	let a: i8  = 10;
    println!("a: {}", a);
...
<p>Если тип не указан, то считается, что числовое значение имеет тип <span class="b">i32<span>:</p>
<pre class="brush:rs;">
let a = 10; // переменная a имеет тип i32
</pre>
<p>Целые числа могут указываться в двоичном, восьмеричном, десятичном и шестнадцатеричном формате.</p>
<p>Двоичный формат - перед числом ставится <code>0b</code>:</p>
<pre class="brush:rs;">let a = 0b0101;		// 5 в десятичной системе</pre>
<p>Восьмеричный формат - перед числом ставится <code>0o</code>:</p>
<pre class="brush:rs;">let a = 0o11;		// 9 в десятичной системе</pre>
<p>Шестнадцатеричный формат - перед числом ставится <code>0x</code>:</p>
<pre class="brush:rs;">let a = 0xA1;		// 161 в десятичной системе</pre>
<h3>Дробные числа</h3>
<p>Дробные числа или точнее числа с плавающей точкой представлены двумя типами:</p>
<ul>
<li><p><span class="b">f32</span>: хранит число с плавающей точкой, которое занимает в памяти 32 бита</p></li>
<li><p><span class="b">f64</span>: хранит число с плавающей точкой, которое занимает в памяти 64 бита</p></li>
</ul>
<p>В качестве разделителя целой и дробной частей используется точка. Если тип явно не указан, то по умолчанию считается, что дробное значение 
имеет тип <span class="b">f64</span>:</p>
<pre class="brush:rs;">
fn main(){
	let a: f32 = 2.04;
    println!("a: {}", a);
...
<p>Логический тип или <span class="b">bool</span> может хранить одно из двух значений: <span class="b">true</span> (истина) или <span class="b">false</span> (ложь).</p>
<pre class="brush:rs;">
fn main(){
	let a: bool = true;
    println!("a: {}", a);
...
<p>Символьный тип или <span class="b">char</span> может хранить отдельные символы Unicode. Каждый символ заключается в одинарные кавычки. 
В памяти он занимает 4 байта.</p>
<pre class="brush:rs;">
fn main(){
	let a: char = 'a';
    println!("a: {}", a);
...
<h3>Строки</h3>
<p>Строки не относятся к примитивным типам, тем не менее рассмотрим их, так как они довольно часто используется. Строка хранит набор символов в кодировке UTF-8, который заключается в двойные кавычки. 
В качестве определителя типа применяется <code>&amp;str</code></p>
<pre class="brush:rs;">
fn main(){
	let a: &str = "hello";
    println!("a: {}", a);
