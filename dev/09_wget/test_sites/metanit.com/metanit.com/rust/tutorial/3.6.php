...
<meta charset="utf-8" />
<meta name="description" content="Slice в языке программирования Rust, получение подмассива и подстроки, string slice, синтаксис последовательностей range">
...
<h2>Slice</h2><div class="date">Последнее обновление: 16.04.2021</div>
...
<p>Тип <span class="b">Slice</span> представляет часть набора элементов, например, часть массива.</p>
<p>Формальное определение slice:</p>
<pre class="brush:rs;">let slice = &amp;набор_элементов[начальный_индекс..конечный_индекс];</pre>
<p>Для извлечения набора элементов применяется синтаксис последовательностей - оператор <span class="b">..</span>, слева от которого указывается начальный индекс, а справа - конечный индекс.</p>
<p>При этом слайс определяется как ссылка, поэтому он не владеет полученным поднабором элементов, а лишь содержит ссылку на этот поднабор.</p>
<p>Например, определим slice, который представляет часть массива:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Выражение <code>numbers[1..5]</code> указывает, что мы хотим взять из массива элементы, начиная с 1-го индекса по 5-й. 
Так как индексация в массиве начинается с нуля, а конечный индекс в результат не включается, то из массива <code>[1, 2, 3, 4, 5, 6, 7, 8]</code> 
мы получим подмассив <code>[2, 3, 4, 5]</code>.</p>
<p>После инициализации слайса можно обращаться к его элементам. Например, как в массиве по индексу:</p>
<pre class="brush:rs;">println!("slice[0]={}", slice[0]);	// выведем первый элемент слайса</pre>
<p>Также можно перебрать элементы в цикле <code>for</code>.</p>
...
<p>Если надо взять элементы, начиная с индекса 0, то его можно не указывать. Также можно не указывать конечный индекс, если необходимо взять элементы вплоть 
до последнего. Таким образом, можно применять следующие комбинации:</p>
<ul>
<li><p><code>numbers[..]</code> эквивалентно <code>[1, 2, 3, 4, 5, 6, 7, 8]</code></p></li>
<li><p><code>numbers[..3]</code> эквивалентно <code>[1, 2, 3 ]</code></p></li>
<li><p><code>numbers[..=3]</code> эквивалентно <code>[1, 2, 3, 4]</code></p></li>
<li><p><code>numbers[1..]</code> эквивалентно <code>[2, 3, 4, 5, 6, 7, 8]</code></p></li>
<li><p><code>numbers[1..3]</code> эквивалентно <code>[2, 3]</code></p></li>
<li><p><code>numbers[1..=3]</code> эквивалентно <code>[2, 3, 4]</code></p></li>
</ul>
<p>Также через слайс можно изменять элементы поднабора (и соответственно начального набора). В этом случае слайс должен быть определен как изменяемая ссылка с оператором 
<span class="b">&amp;mut</span>:</p>
<pre class="brush:rs;">
fn main(){
...
<h3>String slice</h3>
<p>Подобным образом можно получить поднабор и из других наборов элементов, например, из строки - типа String:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Стоит отметить, что для представления строкового слайса в Rust есть специальный тип - <span class="b">&amp;str</span>, который уже ранее использовался 
в предыдущих статьях. То есть в данном случае мы могли бы написать:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Поэтому когда мы присваивали переменной обычную строку:</p>
<pre class="brush:rs;">let hello = "hello world";</pre>
<p>Фактически здесь переменная <code>hello</code> представляла строковый слайс или тип <code>&amp;str</code>.</p>
