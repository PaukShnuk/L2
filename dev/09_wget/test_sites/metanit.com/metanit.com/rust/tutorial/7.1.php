...
<html >
<head>
<title>Rust | Вектор</title>
<meta charset="utf-8" />
<meta name="description" content="Вектор Vec&lt;T&gt; в языке программирования Rust, отличие от массивов, создание вектора, добавление, удаление и получение его элементов, перебор вектора в цикле for">
...
<h1>Коллекции</h1><h2>Вектор</h2><div class="date">Последнее обновление: 28.04.2021</div>
...
<p>Для хранения набора однотипных данных в языке Rust предназначены массивы. Однако они имеют недостаток - после их определения мы не можем изменить их длину, не можем 
ни добавить новый элемент, ни удалить уже существующий. Этот недостаток призван устранить вектор, который представлен типом 
<span class="b">Vec&lt;T&gt;</span> (а точнее структурой <code>std::vec::Vec&lt;T&gt;</code>). Вектор как раз и представляет динамически изменяемый набор однотипных данных.</p>
<h3>Определение вектора</h3>
<p>Есть разные способы определения вектора. Первый способ - вызов функции <span class="b">Vec::new()</span>, которая создает пустой вектор:</p>
<pre class="brush:rs;">let v: Vec&lt;тип_данных&gt; = Vec::new();</pre>
<p>Например:</p>
<pre class="brush:rs;">let v: Vec&lt;i32&gt; = Vec::new();</pre>
<p>В данной случае также указан тип вектора - <span class="b">Vec&lt;i32&gt;</span> - то есть вектор, который хранит значения типа i32.</p>
<p>Второй способ представляет применение специального макроса <span class="b">vec!</span>, после которого в квадратных скобках через запятую 
перечисляются элементы вектора:</p>
<pre class="brush:rs;">let v = vec! [значение1, значени2, ... значениеN];</pre>
<p>Например:</p>
<pre class="brush:rs;">let v = vec![1, 2, 3];</pre>
<p>Здесь для вектора определены три элемента типа i32, поэтому вектор неявно будет иметь тип <code>Vec&lt;i32&gt;</code></p>
<p>При этом вектор может быть и пустым:</p>
<pre class="brush:rs;">let v: Vec&lt;i32&gt;= vec![];</pre>
<p>Третий способ представляет заполнение вектора значениями по умолчанию:</p>
<pre class="brush:rs;">let v = vec![val; repeat];</pre>
<p>В квадратных скобках после макроса <code>vec!</code> указываются два значения. Первое значение указывает на значение по умолчанию, которым заполняется вектор, а второе значение - 
начальное количество элементов. Например:</p>
<pre class="brush:rs;">let v = vec![5; 3];</pre>
<p>Этот вектор будет аналогичен следущему:</p>
<pre class="brush:rs;">let v = vec![5, 5, 5];</pre>
<h3>Добавление элементов</h3>
<p>Для добавления элементов в вектор применяется метод <span class="b">push()</span>:</p>
<pre class="brush:rs;">
...
<p>Причем если предполагается изменение вектора, то его переменная должна быть определена с помощью оператора <span class="b">mut</span>.</p>
<p>При это в примере выше указан тип вектора, так как Rust на основе добавляемых значений может определить тип. Однако все добавляемые значения, должны иметь 
один и тот же тип данных.</p>
<h3>Обращение к элементам вектора</h3>
<p>Обращение к элементам вектора производится так же, как и к элементам массива с помощью индексов:</p>
<pre class="brush:rs;">вектор[индекс_элемента]</pre>
<p>Например, получим и изменим некоторые элементы вектора:</p>
<pre class="brush:rs;">
fn main(){
...
<h3>Перебор вектора</h3>
<p>Для перебора вектора можно использовать цикл <span class="b">for</span>:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Обратите внимания, что циклу for передается не сам объект вектора users, а ссылка на него. Дело в том, что передача объекта в цикл меняет владение вектора. После чего мы 
не сможем ссылаться на этот вектор через переменную users. Проблему можно увидеть на следующем примере:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Здесь в цикл передается не ссылка, а полноценный объект вектора. В итоге владение вектором перемещается от переменной users в цикл for. 
Поэтому на строке</p>
<pre class="brush:rs;">users.push("Alice");</pre> 
<p>при компиляции мы получим ошибку, так как переменная users больше не представляет объект вектора.</p>
<h3>Длина вектора</h3>
<p>Длина вектора возвращается методом <span class="b">len()</span>:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Для удаления элемента по индексу применяется метод <span class="b">remove()</span>:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Еще один метод - <span class="b">pop()</span> удаляет элемент из конца вектора и возвращает его:</p>
<pre class="brush:rs;">
fn main(){
...
<p>В реальности метод <code>pop()</code> возвращает объект перечисления <code>Option&lt;T&gt;</code>. И чтобы получить из него содержащийся в нем элемент, 
применяется метод <code>unwrap()</code>: <code>removed_element.unwrap()</code>.</p>
<p>Это только часть функционала векторов. Все методы вектора можно посмотреть в документации - <a href="https://doc.rust-lang.org/std/vec/struct.Vec.html" rel="nofollow">https://doc.rust-lang.org/std/vec/struct.Vec.html</a></p>
