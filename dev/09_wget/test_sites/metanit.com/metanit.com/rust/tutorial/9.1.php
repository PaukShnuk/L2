...
<html >
<head>
<title>Rust | Создание проекта с помощью Cargo</title>
<meta charset="utf-8" />
<meta name="description" content="Создание проекта с помощью Cargo в языке программирования Rust, построение и запуск проектов, управление пакетами">
...
<h1>Cargo</h1><h2>Создание проекта с помощью Cargo</h2><div class="date">Последнее обновление: 30.04.2021</div>
...
<p><span class="b">Cargo</span> представляет систему построения и управления пакетами в Rust. В большинстве случаев применение этой системы для компиляции программ 
более предпочтительно, чем использование одного компилятора rustc, поскольку Cargo выполняет кучу задач - выполняет построение программы, 
загружает и управляет внешними зависимостями - внешними библиотеками, от которых зависит работа приложения. 
Соответственно при написании более сложных программ на Rust, которые используют какие-то внешние библиотеки, использование Cargo 
позволит существенно упростить создание приложений.</p>
<p>При установке Rust вместе с ним устанавливается и Cargo. Чтобы проверить наличие cargo и его текущую версию, в командной строке/терминале 
необходимо выполнить команду:</p>
<div class="console"><pre class="consoletext">cargo --version</pre></div>
<h3>Создание проекта</h3>
<p>Создадим простейший проект. Выберем для проектов какой-нибудь каталог на жестком диске. Например, в моем случае это каталог <span class="b">C:\rust</span>. 
И с помощью команды <code>cd</code> перейдем в консоли/командной строке к этому каталогу:</p>
<div class="console"><pre class="consoletext">cd c:\rust</pre></div>
<p>Для создания нового проекта в cargo предназначена команда <span class="b">new</span>. Допустим, наш проект будет называться <span class="b">hello</span>. 
Так, для создания проекта выполним команду:</p>
<div class="console"><pre class="consoletext">cargo new hello</pre></div>
<p>После этого в папке c:\rust будет создан каталог <span class="b">hello</span>, который будет содержать файлы проекта.</p>
<img src="./pics/9.2.png" alt="Создание проекта Cargo в Rust" />
<p>Перейдем в этот каталог командой cd:</p>
<div class="console"><pre class="consoletext">cd hello</pre></div>
<p>Что представляет проект Cargo, создаваемый по умолчанию:</p>
<img src="./pics/9.1.png" alt="Проект Cargo в Rust" />
<ul>
<li><p>Скрытая папка <span class="b">.git</span> предназначена для работы с Git<p></li>
<li><p>Папка <span class="b">src</span> предназначена для хранения файлов с исходным кодом. По умолчанию в ней уже 
есть один файл - <span class="b">main.rs</span> со простейшим кодом:<p>
<pre class="brush:rs;">
fn main() {
    println!("Hello, world!");
}
...
<li><p>Файл <span class="b">.gitignore</span> также предназначен для работы с Git и устанавливает, какие папки и файлы не будут загружаться 
на git-репозиторий<p></li>
<li><p>Файл <span class="b">Cargo.toml</span> представляет конфигурацию проекта Cargo. По умолчанию в моем случае он имеет следующий код:<p>
<pre class="brush:rs;">
[package]
name = "hello"
version = "0.1.0"
...
<p>По умолчанию он определяет ряд настроек. В каждом конкретном случае они могут отличаться. Так, секция <span class="b">[package]</span> 
устанавливает данные пакета приложения. В частност, здесь определены следующие опции пакета:</p>
<ul>
<li><p><code>name</code> — имя пакета</p></li>
<li><p><code>version</code> — версия пакета</p></li>
<li><p><code>authors</code> — авторы пакета</p></li>
<li><p><code>edition</code> — версия Rust</p></li>
</ul>
<p>Также по умолчанию в файле определена секция <span class="b">[dependencies]</span>. В данном случае он пуст. В этой секции определяются внешние 
библиотеки, используемые в проекте.</p>
</li>
</ul>
<h3>Построение и запуск проекта</h3>
<p>После того, как мы закончили работу с файлами проекта, его надо построить - создать исполняемый файл приложения. Для построения в Cargo определена команда <span class="b">build</span>. 
Так, по умолчанию у нас уже есть файл main.rs, который содержит некоторый код. Выполним построение этого проекта:</p>
<div class="console"><pre class="consoletext">cargo build</pre></div>
<p>Эта команда создает исполняемый файл приложения в папке <span class="b">target/debug</span>. То есть на Linux/MacOS это файл 
<code>target/debug/hello</code>, а на Windows - <code>target\debug\hello.exe</code>. И после построения мы можем запустить этот файл. 
На Linux/MacOS с помощью команды:</p>
<div class="console"><pre class="consoletext">./target/debug/hello</pre></div>
<p>На Windows с помощью команды:</p>
<div class="console"><pre class="consoletext">.\target\debug\hello</pre></div>
<img src="./pics/9.3.png" alt="Построение и запуск проекта Cargo в Rust" />
<p>Также Cargo предоставляет команду <span class="b">run</span>, которая позволяет сразу построить проект и запустить созданный файл:</p>
<div class="console"><pre class="consoletext">cargo run</pre></div>
<img src="./pics/9.4.png" alt="Команды Cargo в Rust" />
<p>Когда проект полностью готов, мы можем выполнить его построение с флагом <code>--release</code>:</p>
<div class="console"><pre class="consoletext">cargo build --release</pre></div>
<p>В этом случае исполняемый файл приложения создается в папке <span class="b">target/release</span> (<span class="b">target\release</span> на Windows), 
вместо <code>target/debug</code>.</p>
...
<div class="nav"><p><a href="./10.5.php">Назад</a><a href="./">Содержание</a><a href="./9.2.php">Вперед</a></p></div></div>
<div class="socBlock">
