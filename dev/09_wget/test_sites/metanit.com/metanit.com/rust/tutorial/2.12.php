...
<html >
<head>
<title>Rust | Возвращение значения из функции</title>
<meta charset="utf-8" />
<meta name="description" content="Возвращение значения из функции в языке программирования Rust, оператор -&gt;, присвоение результата функции переменной">
...
<h2>Возвращение значения из функции</h2><div class="date">Последнее обновление: 11.04.2021</div>
...
<p>Функция может возвращать некоторый результат. Для подобной функции при ее определении необходимо указать тип возвращаемого 
значения с помощью оператора <span class="b">-&gt;</span>:</p>
<pre class="brush:rs;">
fn имя_функции(параметры) -&gt; тип_возвращаемого_значения
{
}
</pre>
<p>Тип возвращаемого значения указывается после стрелки <code>-&gt;</code> после списка параметров.</p>
<p>Например, определим функцию, которая возвращает сумму двух чисел:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Здесь функция <code>sum()</code> принимает два числа (а точнее два значения типа <code>i32</code>). Кроме того, она возвращает 
значение типа <code>i32</code> (<code>-&gt; i32</code>).</p>
<p>Возвращение значения выглядит довольно просто - результат сложения двух чисел:</p>
<pre class="brush:rs;">a + b</pre>
<p>Обратите внимание, что, во-первых, для возвращения значения не используется никаких дополнительных операторов, как например, оператор 
<code>return</code> в некоторых языках программирования. Во-вторых, после возвращаемого значения не ставится точка с запятой.</p>
<p>Поскольку функция <code>sum()</code> возвращает значение - число типа i32, то мы можем присвоить это значение переменной:</p>
<pre class="brush:rs;">let num1 = sum(2, 3);	// 5</pre>
<p>Исходя из присваиваемого значения Rust установит для переменной <code>num1</code> тип <code>i32</code>.</p>
<p>Важно, что если мы указали для функции тип возвращаемого результата, то функция обязательно должна возвращат некоторое значение. 
Например, чуть изменим предыдущий пример:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Точка с запятой после <code>a + b</code> в функции <code>sum()</code> указывает, что это выражение НЕ представляет возвращаемый результат. 
Соответственно мы столкнемся с ошибкой:</p>
<img src="./pics/2.3.png" alt="Возвращение результата из функции в Rust" />
<p>Другой пример возможной ошибочной ситуации при возвращении результата - возвращение значения неправильного типа. Например:</p>
<pre class="brush:rs;">
fn main(){
...
<p>В данном случае для функции <code>sum()</code> в качестве типа возвращаемого значения определен тип 
<span class="b">u32</span>. Но при сложении двух значений типа <code>i32</code> образуется значение того же типа <span class="b">i32</span>. 
То есть появляется несовместимость типов - <span class="b">u32</span> и <span class="b">i32</span>, поэтому мы опять же столкнемся с ошибкой:</p>
<img src="./pics/2.4.png" alt="Возвращение из функции в Rust" />
<p>При этом функция может содержать и другие инструкции, только в отличие от выражения возвращения результата они должны заканчивться точкой с запятой. Например, 
напишем функцию проверки корректности возраста:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Функция <code>check_age()</code> в качестве параметра принимает некоторый возраст. Тип возвращаемого значения - <span class="b">bool</span>, 
то есть, если возраст прошел проверку, возвращается <code>true</code>, если не прошел - то <code>false</code>.</p>
<p>Условимся, что возраст больше 110 является недействительным. Поэтому возвращение значения выглядит следующим образом:</p>
<pre class="brush:rs;">age &lt;= 110</pre>
...
<p>Теперь немного изменим пример, чтобы функция могла возвращать два разных значения:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Функция <code>set_age()</code> опять же в качестве параметра принимает некоторый возраст и, если он является недействительным (больше 110), 
то устанавливает в качестве возраста некоторое значение по умолчанию - в данном случае число 25:</p>
<pre class="brush:rs;">
else{
		25
	}
</pre>
<p>Если возраст прошел проверку, то просто возвращается значение параметра:</p>
<pre class="brush:rs;">
if age &lt;= 110{
		age
	}
</pre>
<p>Важно отметить, что вне зависимости какое значение возвращается, все они должны соответствовать возвращаемому типу функции - в данном случае типу <code>u8</code>.</p>
...
<p>Оператор <span class="b">return</span> представляет иной способ возвращения значения. В этом случае возвращаемое значение указывается 
после оператора <span class="b">return</span>, а затем идет точка с запятой - как в ряде других языков программирования:</p>
<pre class="brush:rs;">return возвращаемое_значение;</pre>
<p>Например:</p>
<pre class="brush:rs;">
fn main(){
