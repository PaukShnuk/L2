...
<html >
<head>
<title>Rust | Структуры</title>
<meta charset="utf-8" />
<meta name="description" content="Структуры в языке программирования Rust, оператор struct, определение структур и обращения к ее полям, передача значений из другой структуры">
...
<h2>Структуры</h2><div class="date">Последнее обновление: 27.04.2021</div>
...
<p><span class="b">Структуры</span> представляют составной тип данных, который представляет какую-то определенную сущность и который объединяет 
набор данных, описывающих эту сущность.</p>
<p>Для определения структуры применяется ключевое слово <span class="b">struct</span>:</p>
<pre class="brush:rs;">
...
<p>После слова <span class="b">struct</span> указывается имя структуры, а затем внутри фигурных скобок через запятую перечисляются ее переменные. Переменные структуры еще называются 
полями. Для каждого поля определяется название и его тип.</p>
<p>Например, определение простейшей структуры:</p>
<pre class="brush:rs;">
...
<p>В данном случае определена структура Person, которая представляет сущность "человек". Названия структур обычно начинаются с большой буквы. Эта структура 
имеет три поля, которые описывают соответственно имя, возраст и рост человек.</p>
<p>Обратите внимание, что имя человека - поле <code>name</code> представляет тип <span class="b">String</span>, который представляет расширяемую строку и который также 
является встроенной в Rust структурой. Таким образом, одни структуры могут быть полями других структур.</p>
<p>Создав структуру, мы можем определить ее переменную. При инициализации такой переменной необходимо указать значения для всех полей структуры:</p>
<pre class="brush:rs;">
let tom = Person{
	name: "Tom".to_string(),
	age: 36,
...
<p>Здесь обращаю внимание, что значение для поля указывается после двоеточия (:), а не после знака равно, как при обычном присвоении. Кроме того, 
полю <code>name</code> присваивается не просто строка "Tom", а результат метода <code>"Tom".to_string()</code>, который преобразует строковый литерал к 
типу <code>String</code>.</p>
<p>Затем через имя переменной структуры мы сможем обращаться к ее полям:</p>
<pre class="brush:rs;">название_переменной.поле_структуры</pre>
<p>После названия переменной через точку указывается поле структуры.</p>
<p>Например, используем вышеописанную структуру в программе:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Если мы хотим изменять значения полей структуры, то переменная структуры должна быть определена с ключевым словом <span class="b">mut</span>:</p>
<pre class="brush:rs;">
fn main(){
...
<h3>Передача значений по имени</h3>
<p>Если полям структуры передаются значения переменных или параметров метода, имена которых совпадают с именами полей структуры, то можно 
сократить инициализацию структуры.</p>
<pre class="brush:rs;">
fn main(){
...
<p>Здесь названия переменных <code>age</code> и <code>height</code> совпадает с названиями полей. Поэтому вместо</p>
<pre class="brush:rs;">
let age = 22;
let height = 1.68;
...
<h3>Передача значений из другой структуры</h3>
<p>Иногда возникает необходимость передать одной переменной структуры при инициализации значения другой переменной этой структуры. И тут Rust тоже позволяет 
сократить код:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Выражение <code>..bob</code> указывает, что переменная структуры будет наполняться из переменной <code>bob</code>. Если мы хотим для отдельных полей 
определить другие значения, то мы можем их прописать явно:</p>
<pre class="brush:rs;">
let tom = Person{
	name: "Tom".to_string(),
	..bob
...
<p>В данном случае переменная tom берет из переменной bob значения для полей age и height, а для поля name определяет свое значение.</p>
<h3>Декомпозиция структуры</h3>
<p>Rust позволяет раскладывать структуры на переменные. Для этого применяется оператор <span class="b">let</span>:</p>
<pre class="brush:rs;">
let структура{поле_структуры: переменная} = объект_структуры;
</pre>
<p>Рассмотрим пример с декомпозицией структуры Person:</p>
<pre class="brush:rs;">
struct Person
{
    name: String,
...
<p>Здесь выражение</p>
<pre class="brush:rs;">let Person{name: username, age: userage, height: _} = tom;</pre>
<p>указывает, что из структуры <code>tom</code> значение поля <code>name</code> надо передать в переменную <code>username</code>, а значение поля <code>age</code> - в 
переменную <code>userage</code>. Переменные username и userage определяются автоматически. А поле <code>height</code>, к примеру, мы не хотим использовать, поэтому ему передается прочерк <code>_</code>.</p>
<p>И далее мы сможем использовать переменные username и userage.</p>
...
<div class="nav"><p><a href="./2.15.php">Назад</a><a href="./">Содержание</a><a href="./2.17.php">Вперед</a></p></div></div>
<div class="socBlock">
