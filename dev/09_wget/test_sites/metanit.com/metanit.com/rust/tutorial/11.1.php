...
<html >
<head>
<title>Rust | Ввод с клавиатуры</title>
<meta charset="utf-8" />
<meta name="description" content="Ввод с клавиатуры в языке программирования Rust, пакет std::io и функция stdin(), структура Stdin, метод read_line">
...
<h1>Ввод и вывод</h1><h2>Ввод с клавиатуры</h2><div class="date">Последнее обновление: 07.05.2021</div>
...
<p>Для считывания ввода с клавиатуры применяется структура <span class="b">std::io::Stdin</span>. Чтобы получить объект этой структуры, используется 
метод <span class="b">std::io::stdin()</pre>:</p>
<pre class="brush:rs;">let handle: std::io::Stdin  = std::io::stdin();</pre>
<p>Далее через полученный объект мы можем использовать ряд методов. Так, для считывания ввода в строку применяется метод <span class="b">read_line()</span> из трейта 
<span class="b">BufRead</span>:</p>
<pre class="brush:rs;">pub fn read_line(&self, buf: &mut String) -&gt; Result&lt;usize&gt;</pre>
<p>Функция считывает все байты, пока не встретит символ перевода строки (байт 0xA) или пока не достигнет конца потока. После этого считанные байты 
добавляются в буфер. Считанные данные мы можем получить через второй параметр - ссылку на объект String, в которую считывается ввод</p>
<p>Если выполнение функции завершилось успешно, то она возвращает количество считанных байтов.</p>
<p>Если при считывании данных функция достигла конца файла (EOF), то она возвращает значение <span class="b">Ok(0)</span>.</p>
<p>Однако, если считываемые данные не представляют символы в кодировке UTF-8, то функция возвращает ошибку.</p>
<p>Например, считываем некоторые данные с консоли:</p>
<pre class="brush:rs;">
use std::io;
...
<p>В данном случае для получения введенных данных определяется переменная input, которая передается в метод <code>read_line()</code>. Для обработки возможной ошибки 
после вызова функции указываем оператор <span class="b">?</span>.</p>
...
<div class="nav"><p><a href="./9.2.php">Назад</a><a href="./">Содержание</a><a href="./12.1.php">Вперед</a></p></div></div>
<div class="socBlock">
