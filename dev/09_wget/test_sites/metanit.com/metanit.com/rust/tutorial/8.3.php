...
<html >
<head>
<title>Rust | Оператор use и подключение модулей</title>
<meta charset="utf-8" />
<meta name="description" content="Оператор use и подключение модулей в языке программирования Rust, определение псевдонимов и оператор as">
...
<h2>Оператор use и подключение модулей</h2><div class="date">Последнее обновление: 29.04.2021</div>
...
<p>Оператор <span class="b">use</span> позволяет добавить путь к функционалу из других модулей к текущую область видимости или, грубо говоря, 
импортировать функционал. Формальное применение:</p>
<pre class="brush:rs;">use модуль::функция/структура/перечисление/трейт;</pre>
<p>Особенно данный оператор может быть полезен, если нам приходится по многу раз функционал модуля. Например, вызовем функцию модуля:</p>
<pre class="brush:rs;">
mod my_module{	
...
<p>Функция <code>hello()</code> прекрасно работает, но вот прописывать полный путь к этой функции с учетом всех модулей может быть утомительно, кроме того, 
это само по себе утяжеляет код. Теперь применим оператор <code>use</code> для подключения или импорта функции в текущую область видимости:</p>
<pre class="brush:rs;">
mod my_module{	
	pub mod inner_module{
		pub fn hello(){
...
<p>Инструкция</p>
<pre class="brush:rs;">use my_module::inner_module::hello;</pre>
<p>добавляет в текущую область видимости функцию <code>hello</code> из модуля <code>my_module::inner_module::</code>, благодаря чему далее 
при вызове функции можно не прописывать весь набор модулей.</p>
<h3>Подключение всего модуля</h3>
<p>Возможно, в некотором модуле определено несколько функций, структур, перечислений. И мы хотим, чтобы они все были доступны в рамках 
используемой области видимости:</p>
<pre class="brush:rs;">
mod my_module{	
	pub mod inner_module{
		pub fn hello(){
...
<p>Символ звездочка (*) позволяет подключить всю функциональность модуля:</p>
<pre class="brush:rs;">use my_module::inner_module::*;</pre>
<h3>Подключение нескольких компонентов одного модуля</h3>
<p>Возможно, мы захотим подключить не весь модуль, а только несколько компонентов из него:</p>
<pre class="brush:rs;">
mod my_module{	
	pub mod inner_module{
		pub fn hello(){
...
<p>Здесь инструкция</p>
<pre class="brush:rs;">use my_module::inner_module::{hello, bye};</pre>
<p>подключает две функции - hello и bye - набор подключаемых компонентов одного и того же модуля указывается в фигурных скобках через запятую. 
Все остальные компоненты этого модуля (как например, в данном случае функция welcome) будут не доступны.</p>
<p>Но, возможно, в модуле есть еще какие-то компоненты, которые мы также хотим подключить, но обращаться к ним через имя модуля. Тогда для подключения 
модуля можно использовать ключевое слово <span class="b">self</span>:</p>
<pre class="brush:rs;">
mod my_module{	
	pub mod inner_module{
		pub fn hello(){
...
<h3>Одноименный функционал в модулю</h3>
<p>Выше подключалась функция <code>hello()</code>, но что если в области видимости, в которую подключается функция <code>hello()</code> из другого модуля, 
уже есть своя функция <code>hello()</code>:</p>
<pre class="brush:rs;">
mod my_module{	
	pub mod inner_module{
		pub fn hello(){
...
<p>В этом случае мы получим при компиляции ошибку. И, чтобы избежать подобных ситуаций, можно подключить модуль, в котором определена функция, 
и обращать к функции через имя модуля:</p>
<pre class="brush:rs;">
mod my_module{	
	pub mod inner_module{
		pub fn hello(){
...
<h3>Установка псевдонимов с помощью оператора as</h3>
<p>Оператор <span class="b">as</span> позволяет установить для подключаемого функционала псевдоним:</p>
<pre class="brush:rs;">
mod my_module{
...
<p>Здесь инструкция</p>
<pre class="brush:rs;">use my_module::Person as User;</pre>
<p>указывает, что для обращения к структуре Person теперь применяется имя "User":</p>
<pre class="brush:rs;">let tom = User{ name: String::from("Tom"), age: 36 };</pre>
<p>С помощью псевдонимов мы могли бы решить проблему в предыдущем примере с одноименными функциями:</p>
<pre class="brush:rs;">
mod my_module{	
	pub mod inner_module{
		pub fn hello(){
...
<p>Здесь для функции <code>hello()</code> из модуля <code>my_module::inner_module</code> устанавливается псевдоним "simple_hello", 
через который мы можем к ней обращаться.</p>
