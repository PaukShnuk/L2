...
<html >
<head>
<title>Rust | Массивы</title>
<meta charset="utf-8" />
<meta name="description" content="Массивы в языке программирования Rust, способы определения массивов, доступ к элементам массива, перебор массива и метод iter">
...
<h2>Массивы</h2><div class="date">Последнее обновление: 11.04.2021</div>
...
<p>Массивы представляют набор элементов, при этом все элементы набора должны представлять один и тот же тип данных.</p>
<p>В Rust массив можно определять различными способами. Простейшее формальное объявление массива:</p>
<pre class="brush:rs;">let название_массива: [тип_данных; размер];</pre>
<p>После названия массива через точку с запятой в квадратных скобках сначала указывается тип данных элементов массива. А потом через точку с запятой 
размер массива, то есть число его элементов.</p>
<p>Например, объявление массива, который имеет 7 элементов типа <code>i32</code>:</p>
<pre class="brush:rs;">let numbers: [i32; 7];</pre>
<p>Фактически выражение <code>[i32; 7]</code> выступает в качестве типа массива. То есть фактически мы определили переменную <code>numbers</code> и 
указали ее тип - <code>[i32; 7]</code> - массив из 7 элементов типа i32. Но такая переменная еще неинициализирована. Поэтому использовать 
мы ее не можем.</p>
<p>Для инициализации переменной массива необходимо ей присвоить набор значений. Простейшее определение с инициализацией:</p>
<pre class="brush:rs;">let переменная_массива: [тип_данных; размер] = [элемент1, элемент2, ... элементN];</pre>
<p>В качестве значения в квадратных скобках через запятую перечисляются элементы массива. Например:</p>
<pre class="brush:rs;">let numbers: [i32; 7] = [1, 2, 3, 5, 8, 13, 21];</pre>
<p>В данном случае массив <code>numbers</code> имеет 7 элементов: 1, 2, 3, 5, 8, 13, 21.</p>
<p>Как и в случае с обычной переменной можно сначала объявить переменную, а затем инициализировать ее:</p>
<pre class="brush:rs;">
let numbers: [i32; 7];
numbers	= [1, 2, 3, 5, 8, 13, 21];
</pre>
<p>Также если массив инициализируется при его определении, то можно не указывать тип массива:</p>
<pre class="brush:rs;">let numbers = [1, 2, 3, 5, 8, 13, 21];</pre>
<p>В этом случае Rust сам выведет тип - <code>[i32; 7]</code></p>
<h3>Обращение к элементам массива</h3>
<p>Для обращения к элементам массива используется их числовой индекс:</p>
<pre class="brush:rs;">название_массива[индекс_элемента]</pre>
<p>Каждый элемент массива имеет числовой индекс. Индексация начинается с нуля. То есть первый элемент массива будет иметь индекс 0, второй элемент - индекс 1 и так далее.</p>
<p>Например, выведем на консоль первый и третий элементы массива:</p>
<pre class="brush:rs;">
fn main()
{
...
<p>Подобным образом можно не только получить, но и изменять элемент массива. Однако, если мы собираемся изменять элементы массива, 
то он должен определен с модификатором <span class="b">mut</span>:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Стоит учитывать, что мы не можем обратиться к несуществующему элементу. Например, выше в примере в массиве есть всего три элемента. 
Соответственно индексом последнего элемента является число 2. Поэтому к элементам с индексами 3, 4 и больше мы обратиться не можем, так как их просто не существует. 
Например, в следующем случае при попытке обращения к несуществующему элементу мы столкнемся с ошибкой на этапе компиляции:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Также следует учитывать, что массивы в Rust имеют фиксированный размер. После их определения мы не можем ни уменьшить, ни увеличить их размер.</p>
<h3>Перебор массива</h3>
<p>Для перебра массива применяется цикл <span class="b">for</span>:</p>
<pre class="brush:rs;">
fn main(){
...
<p>Стоит отметить, что напрямую перебирать массив в цикле for мы не можем - у массива необходимо вызывать метод <span class="b">iter()</span>: <code>users.iter()</code></p>
...
<h3>Заполнение массива значениями по умолчанию</h3>
<p>Массивы также имеют еще одну форму определения:</p>
<pre class="brush:rs;">let название_массива: [тип_данных; размер] = [значение_по умолчанию; размер];</pre>
<p>В качестве значения в квадратных скобках сначала указывается значение по умолчанию, которым заполняется массив. А затем 
через точку с запятой количество элементов массива, которые должны принять это значение.</p>
<pre class="brush:rs;">let numbers: [i32; 5] = [2; 5];</pre>
<p>В данном случае каждый из пяти элементов массива numbers будет иметь значение "2".</p>
<p>Также можно сократь определение массива и не указывать тип:</p>
<pre class="brush:rs;">
fn main(){
...
<div class="nav"><p><a href="./2.14.php">Назад</a><a href="./">Содержание</a><a href="./2.16.php">Вперед</a></p></div></div>
<div class="socBlock">
