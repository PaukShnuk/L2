...
<html >
<head>
<title>С | Введение в массивы и строки</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в массивы и строки в языке программирования Си, определение и инициализация массивов, многомерные массивы, особенности строк как набора символов">
...
<h2>Введение в массивы и строки</h2><div class="date">Последнее обновление: 20.05.2017</div>
...
<p>Массив представляет набор однотипных значений. Объявление массива выглядит следующим образом:</p>
<pre class="brush:c;">тип_переменной название_массива [длина_массива]</pre>
...
<pre class="brush:c;">int numbers[4];</pre>
...
<p>Индексы начинаются с нуля, поэтому для обращения к первому элементу необходимо использовать выражение <code>numbers[0]</code>.</p>
<p>Также мы можем сразу объявить и инициализировать массив значениями. Для этого переменной массива присваивается набор значений через запятую в фигурных скобках:</p>
<pre class="brush:c;">
int numbers[4] = { 1, 2, 3, 5 };
printf("numbers[2] = %d", numbers[2]);	// 3
</pre>
<p>При инициализации массива можно явно не указывать его длину, в этом случае длина массива будет вычисляться исходя из количества его элементов:</p>
<pre class="brush:c;">
int numbers[] = { 1, 2, 3, 5 };
</pre>
...
<pre class="brush:c;">int numbers[5] = { 10, 12};	// 10, 12, 0, 0, 0</pre>
...
<p>Массивы могут быть многомерными. Элементы таких массивов сами в свою очередь являются массивами, в которых также элементы могут быть массивами. 
Например, определим двухмерный массив чисел:</p>
<pre class="brush:c;">int numbers[3][2] = { {1, 2}, {4, 5}, {7, 8} };</pre>
<p>Здесь массив numbers имеет три элемента, но каждый из этих элементов сам представляет массив из двух элементов. Такой массив еще можно представить в виде таблицы:</p>
...
<h3>Строки</h3>
<p>Выше рассматривались массивы чисел, но с массивами остальных типов данных все будет аналогично. Но отдельно стоит остановиться на массивах символов. 
...
по сути строка - это и есть набор символов. Например, определим строку:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>Но стоит отметить, что кроме самих символов, которые заключены двойные кавычки, каждая строка в качестве завершающего символа содержит символ '\0' 
или нулевой символ. Поэтому в строке "Hello" на самом деле будет не 5 символов, а 6. К примеру, переберем все символы строки и выведем их десятичный код ASCII:</p>
<pre class="brush:c;">
char welcome[] = "Hello";
for(int i=0; i&lt;6; i++)
{
...
<p>Если бы мы определяли массив welcome не как строку, а именно как массив символов, то последним элементом должен был бы идти нулевой символ:</p>
<pre class="brush:c;">char welcome[] = {'H', 'e', 'l', 'l', 'o', '\0'};</pre>
