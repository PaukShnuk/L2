...
<html >
<head>
<title>С | Массивы указателей, строки и многоуровневая адресация</title>
<meta charset="utf-8" />
<meta name="description" content="Массивы указателей, строки и многоуровневая адресация в языке программирования Си, указатели на другие указатели">
...
<h2>Массивы указателей, строки и многоуровневая адресация</h2><div class="date">Последнее обновление: 27.05.2017</div>
...
<p>Указатели сами по себе представляют значения, которые можно хранить в массивах. То есть в итоге получится массив указателей.</p>
<p>Массив указателей определяется одним из трех способов:</p>
<pre class="brush:c;">
тип *имя_массива [размер];
тип *имя_массива [] = инициализатор;
тип *имя_массива [размер] = инициализатор;
</pre>
<p>Используем все эти способы:</p>
<pre class="brush:c;">
int a[] = {1, 2, 3, 4};
int *p1[3];
int *p2[] = { &a[1], &a[2], &a[0] };
...
<p>Массив указателей p1 состоит из трех элементов, но он не инициализирован и является пустым.</p>
<p>Массивы p2 и p3 в качестве элементов хранят адреса на элементы массива a.</p>
<h3>Строки и указатели</h3>
<p>Ранее мы рассмотрели, что строка по сути является набором символов, окончанием которого служит нулевой символ '\0'. И фактически строку можно представить в виде массива:</p>
<pre class="brush:c;">
char[] hello = "Hello World";
</pre>
<p>Но в языке Си также для представления строк можно использовать указатели на тип <span class="b">char</span>:</p>
<pre class="brush:c;">
char *hello = "Hello World!";
printf("%s", hello);
</pre>
<p>Оба определения строки - с помощью массива и указателя будут равнозначны.</p>
<p>Соответственно массив указателей типа <span class="b">char</span> представляет собой набор строк:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<h3>Указатели на указатели</h3>
<p>Кроме обычных указателей в языке Си мы можем создавать указатели на другие указатели. Такие ситуации еще называются многоуровневой адресацией.</p>
<p>Например:</p>
<pre class="brush:c;">int **ptr;</pre>
<p>Переменная ptr представляет указатель на указатель на объект типа <span class="b">int</span>. Две звездочки в определении указателя говорят о 
том, что мы имеем дело с двухуровневой адресацией. Например:</p>
<pre class="brush:c;">
int  x = 22;
int *ptr1;
int **ptr2;
...
<p>Здесь указатель <code>ptr2</code> хранит адрес указателя <code>ptr1</code>. Поэтому через выражение <code>*ptr2</code> можно получить значение, 
которое хранится в указателе ptr - адрес переменной x. А через выражение <code>**ptr2</code> можно получить значение по адресу из ptr1, то есть значение переменной x.</p>
