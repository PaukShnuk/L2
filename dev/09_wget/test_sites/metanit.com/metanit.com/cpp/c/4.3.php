...
<html >
<head>
<title>С | Возвращение результата из функции</title>
<meta charset="utf-8" />
<meta name="description" content="Возвращение результата из функции в языке программирования Си, оператор return">
...
<h2>Возвращение результата из функции</h2><div class="date">Последнее обновление: 27.05.2017</div>
...
<p>В прошлой теме была определена функция factorial, которая в качестве возвращаемого типа имела тип <span class="b">void</span>, то есть по факту ничего не возвращает:</p>
<pre class="brush:c;">
...
<p>Хотя эта функция не использует явно оператор <span class="b">return</span>, но компилятор сам его добавляет автоматически в конец функции:</p>
<pre class="brush:c;">
void factorial(int n)
{
	int result = 1;
...
<p>Эти оба определения фактически равнозначны.</p>
<p>Но также функция может возвращать какое-то конкретное значение. Например, изменим функцию factorial, чтобы она возвращала вычисляемое значение:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>Теперь функция factorial в качестве возвращаемого типа имеет тип <span class="b">int</span>. Если тип функции отличен от <span class="b">void</span>, 
то функция обязательно должна возвращать значение данного типа. Для возвращения значения применяется оператор <span class="b">return</span>, 
после которого указывается возвращаемое значение.</p>
<p>В данном случае функция factorial возвращает значение переменно result, которая содержит факториал числа.</p>
<p>Если функция возвращает какое-либо значение, то при ее вызове мы можем получить это значение в переменную:</p>
<pre class="brush:c;">f = factorial(arg);</pre>
<p>Как и в случае с параметрами и аргументами функции возвращаемое значение должно соответствовать типу функции. Если подобного соответствия нет, 
компилятор пытается выполнить преобразование возвращаемого значения к типу функции:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>В данном случае функция exchange возвращает объект типа <span class="b">int</span>, но возвращаемое значение, описываемое выражением <code>sum/rate</code> будет представлять 
тип <span class="b">double</span>, поэтому в данном случае компилятор автоматически выполнит преобразование:</p>
<pre class="brush:c;">return (int)sum/rate;</pre>
