...
<html >
<head>
<title>С | Внешние объекты</title>
<meta charset="utf-8" />
<meta name="description" content="Внешние объекты в языке программирования Си, ключевое слово extern, прототип функции, подключение переменных и функций из внешних файлов">
...
<h2>Внешние объекты</h2><div class="date">Последнее обновление: 31.05.2017</div>
...
<p>Переменные, используемые в программе, могут быть не только глобальными, локальными или статическими, но также могут представлять внешние объекты. Такие 
...
И в этом случае встает вопрос организации взаимодействия между ними. Рассмотрим пример такого взаимодействия.</p>
<p>Пусть у нас в программе будет два файла: главный файл <span class="b">app.c</span> и вспомогательный файл <span class="b">main.c</span>.</p>
<img src="./pics/4.3.png" alt="Внешние объекты в языке программирования Си" />
<p>Файл <span class="b">app.c</span> будет содежать следующий код:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>И содержимое файла <span class="b">main.c</span>:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>Оба файла используют функции и переменные друг друга. Если переменная определена во внешнем файле, то перед ее использованием должно идти ее описание с 
ключевым словом <span class="b">extern</span>. В частности, в файле app.c определена переменная x. Но чтобы ее использовать в файле main.c, нам надо в этом файле 
поместить ее описание:</p>
<pre class="brush:c;">extern int x;</pre>
<p>Описание переменной в принципе совпадает с ее определением, только при описании ей нельзя присваивать какое-либо значение.</p>
<p>Если мы хотим использовать внешние функции, то перед их использованием надо определить их прототип. Так, в файле app.c определена функция func1, поэтому в файле main.c 
перед вызовом этой функции помещается ее прототип:</p>
<pre class="brush:c;">
void func1(void);
</pre>
<p>То же самое касается использования переменных и функций из файла main.c в файле app.c.</p>
<p>При компиляции в Visual Studio ничего дополнительно не надо делать - среда разработки автоматически скомпилирует оба файла в одну программу.</p>
<p>При компиляции через GCC компилятору надо последовательно через пробел передать имена файлов:</p>
<div class="console">
...
<div class="nav"><p><a href="4.5.php">Назад</a><a href="./">Содержание</a><a href="5.1.php">Вперед</a></p></div></div>
<div class="socBlock">
