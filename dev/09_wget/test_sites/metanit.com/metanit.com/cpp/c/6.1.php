...
<html >
<head>
<title>С | Структуры</title>
<meta charset="utf-8" />
<meta name="description" content="Структуры в языке программирования Си, ключевое слово struct, определение структур с помощью typedef и директивы #define, работы со структурами">
...
<h1>Структуры</h1><h2>Определение структур</h2><div class="date">Последнее обновление: 01.06.2017</div>
...
<p>Структура в языке программирования Си представляет собой производный тип данных, который объединяет в единое целое множество компонентов. 
При этом в отличие от массива эти компоненты могут представлять различные типы данных.</p>
<p>Для определения структуры применяется ключевое слово <span class="b">struct</span>, а сам формат определения выглядит следующим образом:</p>
<pre class="brush:c;">
...
<p><span class="ii">Имя_структуры</span> представляет произвольный идентификатор, к которому применяются те же правила, что и при наименовании переменных.</p>
<p>После имени структуры в фигурных скобках помещаются <span class="ii">Компоненты_структуры</span>, которые представляют набор описаний объектов, которые составляют структуру.</p>
<p>Следует отметить, что в отличие от функции при определении структуры после закрывающей фигурной скобки идет точка с запятой.</p>
<p>Например, определим простейшую структуру:</p>
<pre class="brush:c;">
struct person
{
	int age;
...
<p>Здесь определена структура <code>person</code>, которая имеет два элемента: <code>age</code> (представляет тип int) и <code>name</code> 
(представляет указатель на тип char).</p>
<p>Все элементы структуры объявляются как обычные переменные. Но в отличие от переменных при определении элементов структуры для них не выделяется память, 
и их нельзя инициализировать. По сути мы просто определяем новый тип данных.</p>
<p>После определения структуры мы можем ее использовать. Для начала мы можем определить объект структуры - по сути обычную переменную, которая будет 
представлять выше созданный тип:</p>
<pre class="brush:c;">struct person tom;</pre>
<p>Здесь определена переменная tom, которая представляет структуру person. И при каждом определении переменной типа структуры ей будет выделяться 
память, необходимая для хранения ее элементов.</p>
<p>При определении переменной структуры ее можно сразу инициализировать, присвоив какое-нибудь значение:</p>
<pre class="brush:c;">struct person tom = {23, "Tom"};</pre>
<p>Инициализация структур аналогична инициализации массивов: в фигурных скобках передаются значения для элементов структуры по порядку. Так как 
...
И так далее для всех элементов структуры по порядку.</p>
<p>Также после создания переменной структуры можно обращаться к ее элементам - получать их значения или, наоборот, присваивать им новые значения. 
Для обращения к элементам структуры используется операция "точка":</p>
<pre class="brush:c;">имя_переменной_структуры.имя_элемента</pre>
<p>Теперь объединим все вместе в рамках программы:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>С элементами структуры можно производить все те же операции, что и с переменными тех же типов. Например, добавим ввод с консоли:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>Мы можем одновременно совмещать определение типа структуры и ее переменных:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>После определения структуры, но до точки запятой мы можем через запятую перечислить набор переменных. А затем присвоить их элементам значения.</p>
<p>При подобном определении мы можем даже не указывать имя структуры:</p>
<pre class="brush:c;">
struct
{
	int age;
...
<p>В этом случае компилятор все равно будет знать, что переменные tom, bob и alice представляют структуры с двумя элементами name и age. И соответственно 
мы также с этими переменными сможем работать. Другое дело, что мы не сможем задать новые переменные этой структуры в других местах программы.</p>
<h3>typedef</h3>
<p>Еще один способ определения структуры представляет ключевое слово <span class="b">typedef</span>:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>В конце определения структуры после закрывающей фигурной скобки идет ее обозначение - в данном случае <code>person</code>. 
В дальнейшем мы можем использовать это обозначение для создания переменной структуры. При этом в отличие от примеров выше здесь не надо при определении переменной не надо 
использовать слово <span class="b">struct</span>.</p>
<h3>Директива define</h3>
<p>Еще один способ определить структуру представляет применение препроцессорной директивы <span class="b">#define</span>:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>В данном случае директива define определяет константу PERSON, вместо которой при обработке исходного кода препроцессором будет вставляться 
код структуры <code>struct {int age; char name[20];}</code></p>
