...
<html >
<head>
<title>С | Рекурсивные функции</title>
<meta charset="utf-8" />
<meta name="description" content="Рекурсивные функции в языке программирования Си, функция факториала или числа Фибоначчи, особенности построения рекурсии">
...
<h2>Рекурсивные функции</h2><div class="date">Последнее обновление: 27.05.2017</div>
...
<p>Рекурсивные функции - это функции, которые вызывают сами себя.</p>
<p>Например, представим вычисление факториала в виде рекурсивной функции:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>В функции factorial задано условие, что если число n не равно 1, то это число умножается на результат этой же функции, 
в которую в качестве параметра передается число n-1. То есть происходит рекурсивный спуск. И так далее, пока не дойдем того момента, 
когда значение параметра не будет равно единице.</p>
<p>Рекурсивная функция обязательно должна иметь некоторый базовый вариант, который использует оператор <span class="b">return</span> и 
к которому в итоге сходятся все остальные вызовы. В случае с факториалом  базовый вариант представляет условие, когда n = 1, в этом случае сработает 
инструкция <code>return 1;</code>.</p>
<p>И также все рекурсивные вызовы должны обращаться к подфункциям, которые в конце концов сходятся к базовому варианту. Так, при передаче в 
функцию положительного числа при дальнейших рекурсивных вызовах подфункций в них будет передаваться каждый раз число, меньшее на единицу. 
И в конце концов мы дойдем до ситуации, когда число будет равно 1, и будет использован базовый вариант.</p>
<p>Однако рекурсивный способ решения задачи не всегда является оптимальным. Надо учитывать, что при каждом обращении к рекурсивной функции 
выделяется новый участок памяти для всех ее параметров и переменных. И в данном случае ту же задачу по нахождению факториала куда оптимальнее решить с помощью циклов.</p>
<p>Другим распространенным показательным примером рекурсивной функции служит функция, вычисляющая числа Фиббоначчи. n-й член последовательности Фибоначчи определяется по формуле: f(n)=f(n-1) + f(n-2), причем f(0)=0, а f(1)=1. 
Значения f(0)=0 и f(1)=1 определяют базовые варианты для данной функции:</p>
<pre class="brush:c;">
int fibonachi(int n)
{
    if (n == 0)
