...
<html >
<head>
<title>С | GCC. Первая программа на Linux</title>
<meta charset="utf-8" />
<meta name="description" content="Компилятор GCC. Первая программа на языке Си на Linux Ubuntu, установка компилятора">
...
<h2>GCC. Первая программа на Linux</h2><div class="date">Последнее обновление: 18.05.2017</div>
...
<p>Рассмотрим создание первой программы на Linux, в частности, в среде Ubuntu 16.04. Как правило, многие дистрибутивы Linux, в том числе и Ubuntu, 
уже по умолчанию содержат установленный компилятор gcc, который мы сразу же можем использовать. Но даже если вдруг он не установлен, то его можно доустановить в терминале через команду:</p>
<pre class="brush:c;">sudo apt-get install gcc</pre>
...
<p>Определим в файловой системе каталог для исходных файлов с кодом на Си и создадим в нем новый файл <span class="b">hello.c</span> со следующим кодом:</p>
<pre class="brush:c;">
...
<p>В итоге в папке с файлом hello.c появился скомпилированный файл, который по умолчанию называется <span class="b">a.out</span>. Запустим его с помощью следующей команды:</p>
 ...
 <img src="./pics/gcc1.png" alt="Компиляция программы в GCC в Ubuntu" />
