...
<html >
<head>
<title>С | Чтение и запись бинарных файлов</title>
<meta charset="utf-8" />
<meta name="description" content="Чтение и запись бинарных файлов в языке программирования Си, функции getc и putc, посимвольный обмен с файлами">
...
<h2>Чтение и запись бинарных файлов</h2><div class="date">Последнее обновление: 07.06.2017</div>
...
<p>Работа с бинарными файлами производится посредством посимвольного чтения-записи.</p>
<p>Для чтения одного символа применяется функция <span class="b">getc()</span> (функция <span class="b">fgetc()</span> аналогична). Она имеет 
следующий прототип:</p>
<pre class="brush:c;">int getc(FILE *stream);</pre>
<p>В качестве параметра в функцию передается указатель на файловый поток, а возвращаемым значением является считанный из файла символ, точнее его числовой код.</p>
<p>Для записи символа в файл применяется функция <span class="b">putc()</span>, которая имеет следующий прототип:</p>
<pre class="brush:c;">int putc(int c, FILE *stream);</pre>
<p>В качестве параметров в функцию передаются указатель на файловый поток и символ, который надо записать. Результатом функции является записанный символ.</p>
<p>При чтении из файла и записи в него одного символа указатель текущей позиции в потоке увеличивается, обеспечивая тем самым продвижение по файлу.</p>
<p>Применим эти функции в программе:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>Для записи и чтения файла определены функции write и read соответственно. В функции write пробегаемся по массиву символов и каждый из них заносим в файл, передавая символ в вызов 
<code>putc()</code>.</p>
<p>В функции read считываем по одному символу из файла через вызов <code>getc()</code>. Здесь стоит обратить внимание на процесс считывания файла. Когда будет достигнут конец файла, функция getc() возвратит значение <span class="b">EOF</span>, 
которое определено в файле stdio.h. Поэтому мы можем считывать файл, пока не получим значение EOF.</p>
<p>В итоге на консоль будет выведено ранее записанное сообщение, а на диске D мы сможем найти файл data.txt с этим сообщением.</p>
