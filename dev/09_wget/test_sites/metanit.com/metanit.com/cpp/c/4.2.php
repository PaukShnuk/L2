...
<html >
<head>
<title>С | Передача параметров в функцию</title>
<meta charset="utf-8" />
<meta name="description" content="Передача параметров в функцию в языке программирования Си, аргументы, выделение памяти для параметров, преобразование типов">
...
<h2>Передача параметров в функцию</h2><div class="date">Последнее обновление: 27.05.2017</div>
...
<pre class="brush:c;">тип название_параметра</pre>
<p>Например, определим функцию вычисления факториала:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>Функция factorial принимает один параметр, который имеет имя n и тип <span class="b">int</span>.</p>
<p>В методе main эта функция вызывается два раза, и каждый раз ей передается новое значение для параметра n. Передаваемое значение для параметра 
еще называется <span class="b">аргументом</span>. В итоге функция консоль факториалы чисел 4 и 6:</p>
<div class="console">
...
<p>Прототип функции factorial выглядел бы следующим образом:</p>
<pre class="brush:c;">void factorial(int n);</pre>
<p>Если необходимо несколько параметров, то они перечисляются через запятую. Например, напишем программу имитирующую обмен валюты:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>С помощью функции <span class="b">scanf()</span> осуществляется ввод с консоли для переменных rate и sum, затем эти переменные в качестве аргументов 
передаются в функцию exchange. Переменные, используемые в качестве аргументов, необязательно должны называться именно так, как параметры функции. 
Соответствие между аргументами и параметрами устанавливается по их расположению: первый аргумент передает значение для первого параметра, второй аргумент - для второго параметра и так далее. 
И кроме того, здесь второй вызов функции exchange в качестве аргументов получает числовые константы.</p>
...
<p>Между передаваемыми в функцию аргументами и ее параметрами должно быть соответствие по типу. Если же типы не совпадают, то компилятор пытается автоматически выполнить преобразование типов.</p>
<p>Например, выше использовалась функция <code>void factorial(int n)</code>. Если мы передадим в factorial значения другого типа, то компилятор будет пытаться привести их 
к типу int:</p>
<pre class="brush:c;">
factorial(4.56);
double f = 0.53e+1;
...
<p>Фактически это будет аналогично следующему коду:</p>
<pre class="brush:c;">
factorial((int)4.56);
double f = 0.53e+1;
factorial((int)f);
</pre>
<h3>Процесс передачи параметров</h3>
<p>При передаче параметров следует учитывать, то они передаются в функцию по значению. То есть функция не изменяет значения передаваемых аргументов. Рассмотрим следующий пример:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>В функцию add передаются три числа, где к первому и ко второму число последовательно прибавляется третье. В функции main выводим перед и 
после выполнения функции add значения переменных a и b, 
которые передаются в add в качестве аргументов.</p>
<p>И при выполнении мы увидим, что изменения аргументов в функции add действуют только в рамках этой функции. Вне ее значения переменных a и b остаются неизменными:</p>
<div class="console">
<pre class="consoletext">
...
<p>Почему так происходит? При компиляции функции для ее параметров выделяются отдельные участки памяти. При этом для параметров типа <span class="b">float</span> формируются 
в памяти объекты типа <span class="b">double</span>, а для параметров типов <span class="b">char</span> и <span class="b">short int</span> - объекты типа 
<span class="b">int</span>.</p>
<p>При вызове функции вычисляются значения аргументов, которые передаются на место параметров. И затем значения аргументов заносятся в участки памяти, 
отведенные для параметров. При этом значения типа <span class="b">float</span> преобразуются 
в объекты типа <span class="b">double</span>, а объекты типов <span class="b">char</span> и <span class="b">short int</span> - в объекты типа 
<span class="b">int</span>.</p>
<p>После завершения работы функции память, выделенная для параметров, освобождается.</p>
