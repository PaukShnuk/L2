...
<html >
<head>
<title>С | Директива #define</title>
<meta charset="utf-8" />
<meta name="description" content="Директивы #define и #undef в языке программирования Си, определение идентификаторов и констант">
...
<h2>Директива #define</h2><div class="date">Последнее обновление: 22.05.2017</div>
...
<p>Директива <span class="b">#define</span> определяет идентификатор и последовательность символов, которые будут подставляться вместо 
идентификатора каждый раз, когда он встретится в исходном файле. Формальное определение директивы:</p>
<pre class="brush:c;">#define идентификатор последовательность_символов</pre>
<p>Используем директиву #define:</p>
<pre class="brush:c;">
#define BEGIN {
#define END }
#define N 23
...
<p>Здесь определены три идентификатора BEGIN, END, N. Как правило, идентификаторы имеют имена в верхнем регистре, хотя это необязательно, можно определять имена и строчными символами.</p>
<p>В итоге все вхождения последовательности символов "BEGIN" будут заменяться на открывающую фигурную скобку, а "END" - на закрывающую, а символ "N" на число 23.</p>
<p>Таким образом, после обработки препроцессора программа приобретет следующий вид:</p>
<pre class="brush:c;">
int main(void)
{
	int x = 23;
...
<p>Для идентификатора необязательно определять последовательность символов, можно ограничиться одним идентификаторов:</p>
<pre class="brush:c;">#define DEBUG</pre>
<p>Особенно удобно использовать директиву #define для определения размеров массивов:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>В данном случае если мы захотим глобально поменять размер массива, то достаточно изменить значение N в директиве define.</p>
<p>Следует учитывать, что директива препроцессор не заменяет последовательности символов в двойных и одинарных кавычках и в комментариях:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>Причем если идентификатор должен представлять одно слово, то его последовательность символов может состоять из нескольких слов или символов, разделенных пробелами:</p>
<pre class="brush:c;">#define REAL long double</pre>
<h3>Директива #undef</h3>
<p>В процессе работы мы можем многократно определять новое значение для одного идентификатора:</p>
<pre class="brush:c;">
#define N 23
#define N 32
#define N 55
</pre>
<p>Но некоторые компиляторы, в частности, gcc, могут выдавать предупреждения при повторном определении идентификатора, и чтобы выйти из этой ситуации, мы можем 
использовать директиву <span class="b">#undef</span> для отмены действия макроса. Эта директива имеет следующее определение:</p>
<pre class="brush:c;">#undef идентификатор</pre>
<p>Например:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
