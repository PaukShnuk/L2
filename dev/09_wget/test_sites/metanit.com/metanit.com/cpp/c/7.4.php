...
<html >
<head>
<title>С | Чтение и запись текстовых файлов</title>
<meta charset="utf-8" />
<meta name="description" content="Чтение и запись текстовых файлов в языке программирования Си, функции gets и puts">
...
<h2>Чтение и запись текстовых файлов</h2><div class="date">Последнее обновление: 07.06.2017</div>
...
<p>При работе с текстовыми файлами граздо проще работать с данными не как с отдельными символами, а как со строками с помощью 
функций <span class="b">gets()</span> и <span class="b">puts()</span>.</p>
<p>Функция <span class="b">puts()</span> записывает в файл строку, то есть набор символов, который завершается символом '\0'. Она имеет следующий прототип:</p>
<pre class="brush:c;">int fputs(const char *s, FILE *stream);</pre>
<p>Первый параметр функции - записываемая строка, а второй - указатель на файловый поток. В качестве результата функция возвращает неотрицательное целое число. 
При ошибках в процессе записи возвращается значение <span class="b">EOF</span>.</p>
<p>При записи строки нулевой символ '\0' в файл не записывается.</p>
<p>Функция <span class="b">fgets()</span> считывает из файла одну строку и имеет следующий прототип:</p>
<pre class="brush:c;">char * fgets(char *s, int n, FILE *sream);</pre>
<p>Первый параметр <code>char *s</code> представляет строку, в которую производится считывание. Второй параметр - число <code>n</code> указывает, сколько символов необходимо считывать. 
И третий параметр представляет файловый поток, из которого производится считывание.</p>
<p>При вызове функция считывает из файла не более n-1 символов. Функция прекращает чтение, когда прочтет n-1 символов или встретит 
символ переноса строки \n. Все считанные символы записываются в строку s, в том числе символ \n. И также конец каждой строки дописывается нулевой символ '\0'.</p>
<p>При успешном завершении функция возвращает указатель s, а при ошибке или достижении конца файла возвращается значение <span class="b">NULL</span>.</p>
<p>Применим функции в программе:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>Вначале открываем файл на запись и с помощью вызова <code>fputs(message, fp)</code> записываем в файл искомую строку. 
Затем открываем файл на чтение и в цикле считываем из файла по 256 символов и выводим их на консоль с помощью вызова <code>fgets(cc, 256, fp)</code>. 
Когда данные в файле закончатся, функция возвратит NULL, и произойдет выход из цикла.</p>
<p>Другой пример работы с текстовыми файлами - копирование содержимого из одного файла в другой:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
