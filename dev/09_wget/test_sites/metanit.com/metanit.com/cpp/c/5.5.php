...
<html >
<head>
<title>С | Указатели и массивы</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с массивами с помощью указателей в языке программирования Си, получение адреса массива в памяти">
...
<h2>Указатели и массивы</h2><div class="date">Последнее обновление: 27.05.2017</div>
...
<p>В языке Си массивы и указатели тесно связаны. С помощью указателей мы также легко можем манипулировать элементами массива, как и с помощью индексов.</p>
<p>Имя массива без индексов в Си является адресом его первого элемента. Соответственно через операцию разыменования мы можем получить значение по этому адресу:</p>
<pre class="brush:c;">
int a[] = {1, 2, 3, 4, 5};
printf("a[0] = %d", *a);	// a[0] = 1
</pre>
<p>Мы можем пробежаться по всем элементом массива, прибавляя к адресу определенное число:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>Со сложением и вычитанием здесь действуют те же правила, что и в операциях с указателями. Добавление единицы означает прибавление к адресу 
значения, которое равно размеру типа массива. Так, в данном случае массив представляет тип <span class="b">int</span>, размер которого, как правило, составляет 4 байта, 
...
<p>В итоге в моем случае я получу следующий результат работы программы:</p>
<div class="console">
...
<p>В то же время имя массива это не стандартный указатель, мы не можем изменить его адрес, например, так:</p>
<pre class="brush:c;">
int a[5] = {1, 2, 3, 4, 5};
...
<p>Так как в данном случае мы имеем дело с двухмерным массивом, то адресом первого элемента будет выражение <code>a[0]</code>. Соответственно указатель указывает на 
...
<p>Но в любом случае программа вывела бы следующий результат:</p>
<div class="console">
<pre class="consoletext">
1	2	3	4
5	6	7	8
