...
<html >
<head>
<title>С | Структуры как элементы структур</title>
<meta charset="utf-8" />
<meta name="description" content="Структуры как элементы структур в языке программирования Си, обращение к вложенным структурам">
...
<h2>Структуры как элементы структур</h2><div class="date">Последнее обновление: 01.06.2017</div>
...
<p>Кроме базовых примитивных типов данных как int или char, массивов и указателей в качестве элементов структуры можно использовать другие 
структуры. Рассмотрим на примере.</p>
...
<p>Использованные здесь структуры отражают объекты реального мира. Структура company представляет некоторую компанию, ее элементы 
<code>name</code> и <code>country</code> представляют название и страну компании соответственно. 
А структура smartphone представляет смартфон, в котором определено три элемента: title - название, price - цена и manufacturer - компания-производитель. 
Последний элемент в свою очередь представляет структуру company.</p>
<p>Таким образом, получается, что структура phone имеет три элемента, но так как последний элемент составной и сам содержит два элемента, то при инициализации 
структуры smartphone нам надо ей передать 4 значения:</p>
<pre class="brush:c;">struct smartphone phone = {"iPhone 8", 56000, "Apple", "USA"};</pre>
<p>Что можно формализовать следующим образом:</p>
<pre class="brush:c;">struct smartphone phone = {title, price, manufacturer.name, manufacturer.country};</pre>
<p>Опять же передача значений идет в том порядке, в котором элементы определены в структуре, в том числе во вложенных структурах.</p>
<p>И чтобы обратиться к полям вложенной структуры, надо сначала обратиться к самой этой структуре:</p>
<pre class="brush:c;">phone.manufacturer.name</pre>
<p>И подобное обращение будет применяться при всех операциях с элементами вложенной структуры company. Например, рассмотрим ввод данных с консоли:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>Для ввода значения, например, имени компании-производителя смартфона, используется получение адреса соответствующего элемента <code>&phone.manufacturer.name</code>.</p>
