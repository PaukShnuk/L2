...
<html >
<head>
<title>С | Арифметические операции</title>
<meta charset="utf-8" />
<meta name="description" content="Арифметические операции в языке программирования Си, операнды, инкремент, декремент">
...
<h2>Арифметические операции</h2><div class="date">Последнее обновление: 18.05.2017</div>
...
<p>Язык программирования Си поддерживает все основные арифметические операции. Эти операции производятся над числами. Значения, которые участвуют в операции, называются операндами. 
Список имеющихся в Си операций:</p>
<ul>
<li><p>+</p>
...
<li><p>/</p>
<p>Операция деления двух чисел:</p>
<pre class="brush:c;">
int a = 15 / 3;			// 5
float d = 34.0 / 3.0;	// 11.333333
</pre>
<p>При делении стоит быть внимательным, так как если в операции участвуют два целых числа, то результат деления будет округляться до целого числа, даже если результат 
присваивается переменной float:</p>
<pre class="brush:c;">
float a = 10 / 4;
printf("a = %f \n", a);		// 2.0
</pre>
<p>Результат представлял вещественное число, один из операндов также должен представлять вещественное число:</p>
<pre class="brush:c;">
float a = 10.0 / 4;
printf("a = %f \n", a);		// 2.5
</pre>
...
<li><p>%</p>
<p>Возвращает результат деления двух чисел по модулю (получение остатка от деления):</p>
<pre class="brush:c;">
...
<p>Здесь результатом будет число 43:</p>
<ol>
<li><p>5 * b = 35</p></li>
<li><p>a + 35 = 43</p></li>
</ol>
...
<p>Здесь результатом будет число 91:</p>
<ol>
<li><p>a + 5 = 13</p></li>
<li><p>13 * b = 91</p></li>
</ol>
<p>Также в Си есть специальные унарные операции над одним числом: ++ (инкремент) и -- (декремент). Каждая из операций имеет две разновидности: 
префиксная и постфиксная.</p>
<ul>
<li><p>Префиксный инкремент (++x). Увеличивает значение переменной на единицу и полученное значение используется как значение выражения ++x<p>
<pre class="brush:c;">
int a = 8;
int b = ++a;
printf("a = %d \n", a);	// 9
...
<li><p>Постфиксный инкремент (x++). Увеличивает значение переменной на единицу, но значением выражения x++ будет то, которое было до увеличения на единицу:<p>
<pre class="brush:c;">
int a = 8;
int b = a++;
printf("a = %d \n", a);	// 9
...
<li><p>Префиксный декремент (--x). Уменьшает значение переменной на единицу, и полученное значение используется как значение выражения --x<p>
<pre class="brush:c;">
int a = 8;
int b = --a;
printf("a = %d \n", a);	// 7
...
<li><p>Постфиксный декремент (x--). Уменьшает значение переменной на единицу, но значением выражения x-- будет то, которое было до уменьшения на единицу:<p>
<pre class="brush:c;">
int a = 8;
int b = a--;
printf("a = %d \n", a);	// 7
...
<pre class="brush:c;">int c = (a++) + b;</pre>
<p>Аналогично будет расцениваться выражения с постфиксным декрементом: <code>int c = a---b;</code>.</p>
