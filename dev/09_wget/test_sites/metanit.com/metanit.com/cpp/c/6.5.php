...
<html >
<head>
<title>С | Структуры и функции</title>
<meta charset="utf-8" />
<meta name="description" content="Использование структур в функциях в языке программирования Си, передача и получение указателя на структуры">
...
<h2>Структуры и функции</h2><div class="date">Последнее обновление: 02.06.2017</div>
...
<p>Как и любой другой объект, структура может использоваться в качестве параметра функции и также может быть возвращаемым объектом функции.</p>
<p>Рассмотрим использование структур в функциях на примере:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>В начале программы определена структура time, которая представляет время и содержит три элемента для хранения значений для часов, минут и секунд.</p>
<p>В конце программы определена функция <code>addminutes()</code>, прибавляет к дате некоторое количество минут. Первый параметр функции представляет структуру time, а второй - 
количество минут, которое надо прибавить. Возвращаемое значение функции также представляет структуру time. В самой функции с помощью некоторой простейшей логики 
создаем новый объект структуры time, присваиваем ему значения, переданные через первый параметр, и добавляем нужное количество минут.</p>
<p>Затем в функции main последовательно вызываем функцию addminutes, добавляя к некоторому времени разное количество минут.</p>
...
<h3>Указатели на структуру как параметры</h3>
<p>При использовании структур в качестве параметров в функции следует учитывать, что при вызове функции для структуры, также как и для параметров типа int или char, 
...
не можем.</p>
<p>Для примера определим функцию, где попытаемся изменить структуру:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>Если мы запустим программу, то увидим передаваемая структура current_time в функцию addminutes после выполнения этой функции не изменится:</p>
<div class="console">
<pre class="consoletext">
...
<p>Если мы действительно хотим изменить структуру в функции, то на надо передавать не саму структуру, а указатель на нее:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>Теперь в функцию передается указатель на структуру p_time, который содержит адрес структуры current_time. В итоге любое изменение значений по этому указателю 
приведет к изменению самой структуры current_time. И результаты программы будут уже отличаться:</p>
<div class="console">
<pre class="consoletext">
In addminutes	17:59:10
In main			17:59:10
...
<h3>Указатели на структуру как результаты функций</h3>
<p>И еще один вариант использования структур в функции - это возвращение из функции указателя на структуру. Например, 
определим функцию, которая будет возвращать указатель на структуру time:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
#include &lt;stdlib.h&gt;
...
<p>В функции input вначале выделяем память для структуры, получая указатель на выделенный блок памяти.</p>
<p>Потом поочередно вводим значения для элементов структуры и возвращаем указатель из функции.</p>
<p>В функции main вызываем функцию input, получая из нее указаетель. Используя полученный указатель, можно получить значения элементов структуры. 
И в конце освобождаем выделенную под структуру динамическую память.</p>
<p>Консольный вывод программы:</p>
<div class="console">
<pre class="consoletext">
Enter hour: 22
