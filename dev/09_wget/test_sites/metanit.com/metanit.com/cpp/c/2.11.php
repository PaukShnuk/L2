...
<html >
<head>
<title>С | Условные конструкции</title>
<meta charset="utf-8" />
<meta name="description" content="Условные конструкции в языке программирования Си, выражения if..else, switch..case, тернарный оператор, ветвление программы">
...
<h2>Условные конструкции</h2><div class="date">Последнее обновление: 20.05.2017</div>
...
<p>Условные конструкции позволяют направить ход программы по одному из возможных путей в зависимости от условия. В языке Си есть несколько операторов, 
которые позволяют это сделать.</p>
<h3>Оператор if</h3>
...
<pre class="brush:c;">if (выражение_условия) инструкция;</pre>
<p>В качестве <span class="ii">выражения_условия</span> может использоваться арифметическая, логическая операция или операция сравнения. И если эта операция возвращает ненулевое значение (то есть выражение истинно), 
то выполняется последующая инструкция:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
int main(void)
{
...
<p>Здесь используются две условных конструкции if. Они проверят больше или меньше значение переменной x, чем определенное значение. В качестве инструкции 
в обоих случаях выполняется функция printf.</p>
<p>В первом случае <code>x &gt; 50</code> условие истинно, так как значение переменной x действительно больше 50, поэтому эта операция сравнения 
возвратит 1, и поэтому будет выполнятся последующая инструкция.</p>
<p>Во вором случае операция сравнения <code>x &lt; 30</code> возвратит 0, так как условие ложно, поэтому последующая инструкция не будет выполняться. 
Поэтому на консоль будет выведена всего одна строка</p>
<div class="console"><pre class="consoletext">x greater than 50</pre></div>
...
<p>Также мы можем использовать полную форму оператора if:</p>
<pre class="brush:c;">
...
<p>После оператора else мы можем определить набор инструкций, которые выполняются, если выражение_условия ложно, то есть возвращает 0. То есть если 
<span class="ii">выражение_условия</span> истинно, выполнится инструкция после оператора if, а если это выражение ложно, то выполняется инструкция после оператора else.</p>
<pre class="brush:c;">
int x = 60;
if(x &gt; 60) 
...
<p>Однако в данном случае у нас есть три условия: переменная x может быть больше 60, меньше 60 и равна 60. Для проверки альтернативных условий 
мы можем вводить выражения <span class="b">else if</span>:</p>
<pre class="brush:c;">
int x = 60;
...
сравнивается со значениями, помещенными после оператора <span class="b">сase</span>. И если совпадение будет найдено, то будет выполняться 
определенный блок <span class="b">сase</span>.</p>
<p>В конце конструкции switch может стоять блок <span class="b">default</span>. Он необязателен и выполняется в том случае, если ни одна совпадения в блоках case не было найдено. Например:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
int main(void)
{
...
<p>Чтобы избежать выполнения последующих блоков case/default, в конце каждого блока ставится оператор <span class="b">break</span>.</p>
...
1 (то есть истинно), то возвращается второй операнд; если условие равно 0 (то есть ложно), то третий. Например:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
int main(void)
{
...
<p>Здесь результатом тернарной операции является переменная z. И если переменная op содержит знак "+", то z будет равно второму операнду - (x+y). Иначе z 
