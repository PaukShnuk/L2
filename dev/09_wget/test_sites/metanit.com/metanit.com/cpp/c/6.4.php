...
<html >
<head>
<title>С | Массивы структур</title>
<meta charset="utf-8" />
<meta name="description" content="Массивы структур в языке программирования Си, перебор массива с помощью указателя">
...
<h2>Массивы структур</h2><div class="date">Последнее обновление: 01.06.2017</div>
...
<p>Из структур можно создавать массивы также, как массивы других типов. И все форматы определения массива структур будут аналогичны определению массивов других типов:</p>
<pre class="brush:c;">struct person people[10];</pre>
<p>В данном случае определен массив структур person из 10 элементов.</p>
<p>Используем массив структур в программе:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>В массиве people определено 4 объекта person. Хотя при инициализации в массив передается 8 значений <code>{23, "Tom", 32, "Bob", 26, "Alice", 41, "Sam"}</code>, 
но так как каждая структура состоит из двух элементов, то соответственно из этих значений по порядку получается 4 объекта person.</p>
<p>Обращение к элементам массива структур происходит по индексу <code>people[0]</code>. А чтобы обратиться к элементу структуры из массива, 
после индекса указывается имя элемента структуры: <code>people[i].name</code></p>
...
<p>И также как с массивами других типов с массивами структур можно использовать указатели:</p>
<pre class="brush:c;">
#include &lt;stdio.h&gt;
...
<p>Здесь в массиве people те же 4 элемента person. Для их просмотра создан указатель *p, который устанавливается на начало массива people. И в цикле 
...
И эти действия продолжаются пока указатель не дойдет до конца массива, который можно получить через выражение people+n.</p>
