...
<html >
<head>
<title>C++ | Виртуальные функции и их переопределение</title>
<meta charset="utf-8" />
<meta name="description" content="Виртуальные функции и их переопределение в языке програмирования C++, модификатор virtual и override, вызов реализации базового класса, полиморфный класс">
...
<h2>Виртуальные функции и их переопределение</h2><div class="date">Последнее обновление: 10.10.2017</div>
...
<p>Класс Employee наследует функцию display и может использовать ее для вывода информации о сотруднике. Однако реализация этой функции 
...
которая хранит компанию, в которой работает объект Employee. И было бы хорошо, если бы для объекта Employee функция display также выводила бы значения переменной company.</p>
<p>Мы можем решить эту проблему с помощью виртуальных функций. То есть чтобы в производном классе можно было бы переопределить 
функцию, в базовом классе подобная функция должна быть объявлена с ключевым словом <span class="b">virtual</span>:</p>
<pre class="brush:cpp;">
class Person
{
public:
...
<p>Теперь функция display может быть переопределена в производных классах. Причем производный класс может и не переопределять функцию, а 
использовать унаследованный функционал.</p>
<p>Для переопределения виртуальной функции в производном классе она должна быть определена с ключевым словом <span class="b">override</span>, которое помещается после списка параметров функции:</p>
<pre class="brush:cpp;">
class Employee : public Person
{
public:
...
<p>Класс, который определяет или наследует виртуальную функцию, еще назвается <span class="b">полиморфным</span> (polymorphic class). 
То есть в данном случае Person и Employee являются полиморфными классами.</p>
<p>При переопределении виртуальных функций следует учитывать, что переопределенная версия функции в производном классе должна иметь тот же набор параметров и возвращать объект того же типа, 
что и виртуальная функция в базовом классе.</p>
<h3>Обращение к базовому классу</h3>
<p>Теперь в классе Employee функция display выводит имя и компанию сотрудника. Однако функция display в классе Person выводит еще и возраст человека. 
Но поскольку переменная age в классе Person определена как закрытая, то класс Employee не имеет к ней доступа.</p>
<p>Мы могли бы в данном случае определить переменную age как защищенную, то есть со спецификатором protected, однако есть и другой способ - мы можем 
просто обратиться к реализации функции базового класса. Применим данный способ:</p>
<pre class="brush:cpp;">
class Employee : public Person
{
public:
...
<p>С помощью вызова <code>Person::display();</code> будет выполняться реализация функции display из класса Person, которая выведет на консоль 
имя и возраст. Соответственно нам остается только определить код для вывода компании. Причем таким образом мы можем обращаться к любому функционалу базового класса за исключением приватных переменных и функций.</p>
...
<h3>Сокрытие функций</h3>
<p>Производный класс может определить функцию с тем же именем, что и виртуальная функция в базовом классе, с тем же или другим списком параметров. 
...
<p>В данном случае функция display представляет совершенно новую реализацию, которая не имеет ничего общего с базовым классом Person. Она скрывает по сути виртуальную функцию из базового класса.</p>
<p>Нередко подобные функции являются результатом невнимательности - хотели переопределить функцию базового класса, но забыли поставить спецификатор 
override. В то же время ошибки в данном случае не будет. И мы также сможем использовать данную функцию:</p>
<pre class="brush:cpp;">
Employee bob("Bob", 31, "Microsoft");
bob.display();
</pre>
<h3>Запрет переопределения</h3>
<p>С помощью спецификатора <span class="b">final</span> мы можем запретить определение в производных классах функций, которые имеют то же самое имя, возвращаемый тип и список параметров, что и виртуальная функция 
в базовом классе. Например:</p>
<pre class="brush:cpp;">
class Person
{
public:
...
<div class="nav"><p><a href="5.10.php">Назад</a><a href="./">Содержание</a><a href="5.12.php">Вперед</a></p></div></div>
<div class="socBlock">
