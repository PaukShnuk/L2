...
<html >
<head>
<title>C++ | Forward_list</title>
<meta charset="utf-8" />
<meta name="description" content="Контейнер Forward_list в языке програмирования C++, односвязный список, все операции со списком">
...
<h2>Forward_list</h2><div class="date">Последнее обновление: 03.11.2017</div>
...
<p>Контейнер <span class="b">forward_list</span> представляет односвязный список. Для его использования необходимо подключить заголовочный файл <span class="b">forward_list</span>.</p>
...
<p>Напрямую в списке forward_list можно получить только первый элемент. Для этого применяется функция <span class="b">front()</span>. Для 
перебора элементов также можно использовать цикл:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Также для перебора и получения элементов можно использовать итераторы. Причем класс forward_list добавляет ряд дополнительных функций 
для получения итераторов: <span class="b">before_begin()</span>  и <span class="b">cbefore_begin()</span>. Обе функции возвращают итератор (вторая функция возвращает константный итератор <code>const_iterator</code>) на 
несуществующий элемент непосредственно перед началом списка. К значению по этому итератору обратиться нельзя.</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;forward_list&gt;
...
<h3>Размер списка</h3>
<p>По умолчанию класс forward_list не определяет никаких функций, которые позволяют получить размер контейнера. 
В этом классе только функция <span class="b">max_size()</span>, которая позволяет получить масимальный размер контейнера.</p>
<p>Функция <span class="b">empty()</span> позволяет узнать, пуст ли список. Если он пуст, то функция возвращает значение true, иначе возвращается значение false:</p>
<pre class="brush:cpp;">
std::forward_list&lt;int&gt; numbers = { 1, 2, 3, 4, 5 };
if (numbers.empty())
...
<p>Для изменения размера контейнера можно использовать функцию <span class="b">resize()</span>, которая имеет две формы:</p>
<ul>
<li><p><span class="b">resize(n)</span>: оставляет в списке n первых элементов. Если список содержит больше элементов, то он усекается до первых n элементов. 
Если размер списка меньше n, то добавляются недостающие элементы и инициализируются значением по умолчанию</p></li>
<li><p><span class="b">resize(n, value)</span>: также оставляет в списке n первых элементов. Если размер списка меньше n, то добавляются 
...
<h3>Изменение элементов списка</h3>
<p>Функция <span class="b">assign()</span> позволяет заменить все элементы списка определенным набором. Она имеет следующие формы:</p>
<ul>
...
<p>Функция <span class="b">swap()</span> обменивает значениями два списка:</p>
<pre class="brush:cpp;">
...
<ul>
<li><p><span class="b">push_front(val)</span>: добавляет объект val в начало списка</p></li>
<li><p><span class="b">emplace_front(val)</span>: добавляет объект val в начало списка</p></li>
<li><p><span class="b">emplace_after(p, val)</span>: вставляет объект val после элемента, на который указывает итератор p. Возвращает итератор на вставленный элемент. 
Если p представляет итератор на позицию после конца списка, то результат неопределен.</p></li>
<li><p><span class="b">insert_after(p, val)</span>: вставляет объект val после элемента, на который указывает итератор p. Возвращает итератор на вставленный элемент.</p></li>
<li><p><span class="b">insert_after(p, n, val)</span>: вставляет n объектов val после элемента, на который указывает итератор p. Возвращает итератор на последний вставленный элемент.</p></li>
<li><p><span class="b">insert_after(p, begin, end)</span>: вставляет после элемента, на который указывает итератор p, набор объектов из другого контейнера, начало и конец которого определяется итераторами 
begin и end. Возвращает итератор на последний вставленный элемент.</p></li>
<li><p><span class="b">insert_after(p, il)</span>: вставляет после элемента, на который указывает итератор p, список инициализации il. 
Возвращает итератор на последний вставленный элемент.</p></li>
</ul>
...
<ul>
<li><p><span class="b">clear()</span>: удаляет все элементы</p></li>
<li><p><span class="b">pop_front()</span>: удаляет первый элемент</p></li>
<li><p><span class="b">erase_after(p)</span>: удаляет элемент после элемента, на который указывает итератор p. Возвращает итератор на элемент после удаленного</p></li>
<li><p><span class="b">erase_after(begin, end)</span>: удаляет диапазон элементов, на начало и конец которого указывают соответственно итераторы begin и end. 
Возвращает итератор на элемент после последнего удаленного</p></li>
</ul>
...
<div class="nav"><p><a href="7.5.php">Назад</a><a href="./">Содержание</a><a href="7.8.php">Вперед</a></p></div></div>
<div class="socBlock">
