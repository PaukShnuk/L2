...
<html >
<head>
<title>C++ | Передача аргументов по значению и по ссылке</title>
<meta charset="utf-8" />
<meta name="description" content="Передача аргументов в функцию по значению и по ссылке в языке программирования C++, различия между этими двумя способами передачи параметров">
...
<h2>Передача аргументов по значению и по ссылке</h2><div class="date">Последнее обновление: 22.09.2017</div>
...
<p>Аргументы могут передаваться по значению (by value) и по ссылке (by reference). При передаче аргументов по значению внешний объект, который передается в 
качестве аргумента в функцию, не может быть изменен в этой функции. В функцию передается само значение этого объекта. Например:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Функция square принимает два числа и возводит их в квадрат. В функции main перед и после выполнения функции square происходит вывод на консоль 
значений переменных a и b, которые передаются в square в качестве аргументов.</p>
<p>И при выполнении мы увидим, что изменения аргументов в функции square действуют только в рамках этой функции. Вне ее значения переменных a и b остаются неизменными:</p>
<div class="console">
...
<p>Почему так происходит? При компиляции функции для ее параметров выделяются отдельные участки памяти. При вызове функции вычисляются значения аргументов, которые передаются на место параметров. И затем значения аргументов заносятся в эти участки памяти. То есть функция 
манипулирует копиями значений объектов, а не самими объектами.</p>
<h3>Передача параметров по ссылке</h3>
<p>При передаче параметров по ссылке передается ссылка на объект, через которую мы можем манипулировать самим объектов, а не просто его значением. 
Так, перепишем предыдущий пример, используя передачу по ссылке:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Теперь параметры a и b передаются по ссылке. Ссылочный параметр связывается непосредственно с объектом, поэтому через ссылку можно менять сам объект.</p>
...
<p>Передача по ссылке позволяет возвратить из функции сразу несколько значений. Также передача параметров по ссылке является более эффективной при передаче очень больших объектов. 
Поскольку в этом случае не происходит копирования значений, а функция использует сам объект, а не его значение.</p>
...
<p>Если функция принимает аргументы по значению, то изменение параметров внутри функции также никак не скажется на внешних объектах, даже если при вызове функции 
в нее передаются ссылки на объекты.</p>
<div class="console">
<pre class="consoletext">
Before square: a = 4 	b = 5
In square: a = 16		b = 25
...
<p>Передача параметров по значению больше подходит для передачи в функцию небольших объектов, значения которых копируются в определенные участки памяти, которые потом использует функция.</p>
<p>Передача параметров по ссылке больше подходит для передачи в функцию больших объектов, в этом случае не нужно копировать все содержимое объекта в 
участок памяти, за счет чего увеличивается производительность программы.</p>
