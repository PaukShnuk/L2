...
<html >
<head>
<title>C++ | Арифметические операции</title>
<meta charset="utf-8" />
<meta name="description" content="Арифметические операции в языке программирования C++, инкремент, декремент, приоритеты операций">
...
<h2>Арифметические операции</h2><div class="date">Последнее обновление: 12.09.2017</div>
...
<p>Арифметические операции производятся над числами. Значения, которые участвуют в операции, называются операндами. 
...
К бинарным операциям относят следующие:</p>
<ul>
<li><p><span class="b">+</span></p>
<p>Операция сложения возвращает сумму двух чисел:</p>
<pre class="brush:cpp;">
int a = 10;
int b = 7;
int c = a + b;	// 17
...
<li><p><span class="b">*</span></p>
<p>Операция умножения возвращает произведение двух чисел:</p>
<pre class="brush:cpp;">
int a = 10;
int b = 7;
int c = a * b;	// 70
...
<li><p><span class="b">/</span></p>
<p>Операция деления возвращает частное двух чисел:</p>
<pre class="brush:cpp;">
int a = 20;
int b = 5;
int c = a / b;	// 4
...
<p>При делении стоит быть внимательным, так как если в операции участвуют два целых числа, то результат деления будет округляться до целого числа, даже если результат присваивается переменной float или double:</p>
<pre class="brush:cpp;">
double k = 10 / 4;     // 2
std::cout &lt;&lt; k;
</pre>
<p>Чтобы результат представлял числос плавающей точкой, один из операндов также должен представлять число с плавающей точкой:</p>
<pre class="brush:cpp;">
double k = 10.0 / 4;     // 2.5
std::cout &lt;&lt; k;
</pre>
...
<li><p><span class="b">%</span></p>
<p>Операция получения остатка от целочисленного деления:</p>
<pre class="brush:cpp;">
int a = 33;
int b = 5;
int c = a % b;	// 3
...
<p>Также есть две унарные арифметические операции, которые производятся над одним числом: ++ (инкремент) и -- (декремент). 
Каждая из операций имеет две разновидности: префиксная и постфиксная:</p>
<ul>
<li><p><span class="b">Префиксный инкремент</span>.</p>
<p>Увеличивает значение переменной на единицу и полученный результат используется как значение выражения ++x</p>
<pre class="brush:cpp;">
int a = 8;
int b = ++a;
std::cout &lt;&lt; a &lt;&lt; "\n";	// 9
...
<p>Увеличивает значение переменной на единицу, но значением выражения x++ будет то, которое было до увеличения на единицу</p>
<pre class="brush:cpp;">
int a = 8;
int b = a++;
std::cout &lt;&lt; a &lt;&lt; "\n";	// 9
...
<li><p><span class="b">Префиксный декремент</span>.</p>
<p>Уменьшает значение переменной на единицу, и полученное значение используется как значение выражения --x</p>
<pre class="brush:cpp;">
int a = 8;
int b = --a;
std::cout &lt;&lt; a &lt;&lt; "\n";	// 7
...
<li><p><span class="b">Постфиксный декремент</span>.</p>
<p>Уменьшает значение переменной на единицу, но значением выражения x-- будет то, которое было до уменьшения на единицу</p>
<pre class="brush:cpp;">
int a = 8;
int b = a--;
std::cout &lt;&lt; a &lt;&lt; "\n";	// 7
...
<p>Арифметические операции вычисляются слева направо. Одни операции имеют больший приоритет чем другие и поэтому выполняются вначале. 
Операции в порядке уменьшения приоритета:</p>
<table>
<tr><td>+ (инкремент), - (декремент)</td></tr>
<tr><td>* (умножение), / (деление), % (остаток от деления)</td></tr>
<tr><td>+ (сложение), - (вычитание)</td></tr>
</table>
...
<p>Хотя операции выполняются слева направо, но вначале будет выполняться операция инкремента <code>++b</code>, которая увеличит значение переменной 
b и возвратит его в качестве результата, так как эта операция имеет больший приоритет. Затем выполняется умножение <code>5 * ++b</code>, 
и только в последнюю очередь выполняется сложение <code>a + 5 * ++b</code></p>
<p>Скобки позволяют переопределить порядок вычислений. Например:</p>
<pre class="brush:cpp;">
int a = 8;
int b = 7;
int c = (a + 5) * ++b;      // 104
...
<p>Несмотря на то, что операция сложения имеет меньший приоритет, но вначале будет выполняться именно сложение, а не умножение, так как операция сложения заключена в скобки.</p>
