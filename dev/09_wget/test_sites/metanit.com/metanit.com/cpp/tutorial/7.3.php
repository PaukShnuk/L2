...
<html >
<head>
<title>C++ | Итераторы</title>
<meta charset="utf-8" />
<meta name="description" content="Итераторы в языке програмирования C++, их использование, функции begin и end, перебор элементов контейнера">
...
<h2>Итераторы</h2><div class="date">Последнее обновление: 03.11.2017</div>
...
<p>Итераторы обеспечивают доступ к элементам контейнера. С помощью итераторов очень удобно перебирать элементы. Итератор описывается типом 
<span class="b">iterator</span>. Но для каждого контейнера конкретный тип итератора будет отличаться. Так, итератор для контейнера 
<code>list&lt;int&gt;</code> представляет тип <span class="b">list&lt;int&gt;::iterator</span>, а итератор контейнера <code>vector&lt;int&gt;</code> 
представляет тип <span class="b">vector&lt;int&gt;::iterator</span> и так далее.</p>
<p>Для получения итераторов контейнеры в C++ обладают такими функциями, как <span class="b">begin()</span> и <span class="b">end()</span>. 
Функция <span class="b">begin()</span> возвращает итератор, который указывает на первый элемент контейнера (при наличии в контейнере элементов). 
Функция <span class="b">end()</span> возвращает итератор, который указывает на следующую позицию после последнего элемента, то есть по сути на конец контейнера. 
...
<p>В данном случае создается вектор - контейнер типа vector, который содержит значения типа int. И этот контейнер инициализируется набором {1, 2, 3, 4}. 
И через метод <code>begin()</code> можно получить итератор для этого контейнера. Причем этот итератор будет указывать на первый элемент контейнера.</p>
<h3>Операции с итераторами</h3>
<p>С итераторами можно проводить следующие операции:</p>
<ul>
<li><p><span class="b">*iter</span>: получение элемента, на который указывает итератор</p></li>
<li><p><span class="b">++iter</span>: перемещение итератора вперед для обращения к следующему элементу</p></li>
<li><p><span class="b">--iter</span>: перемещение итератора назад для обращения к предыдущему элементу. Итераторы контейнера <span class="b">forward_list</span> 
не поддерживают операцию декремента.</p></li>
<li><p><span class="b">iter1 == iter2</span>: два итератора равны, если они указывают на один и тот же элемент</p></li>
<li><p><span class="b">iter1 != iter2</span>: два итератора не равны, если они указывают на разные элементы</p></li>
</ul>
...
<p>При работе с контейнерами следует учитывать, что добавление или удаление элементов в контейнере может привести к тому, что все текущие итераторы для 
данного контейнера, а также ссылки и указатели на его элементы станут недопустимыми.</p>
...
<h3>Константные итераторы</h3>
<p>Если контейнер представляет константу, то для обращения к элементам этого контейнера можно использовать только константный итератор (тип 
<span class="b">const_iterator</span>). Такой итератор позволяет считывать элементы, но не изменять их:</p>
<pre class="brush:cpp;">
...
<p>Для получения константного итератора также можно использовать функции <span class="b">cbegin()</span> и <span class="b">cend</span>. При этом даже если контейнер не 
представляет константу, но при этом для его перебора используется константный итератор, то опять же нельзя изменять значения элементов этого контейнера:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;vector&gt;
...
<h3>Реверсивные итераторы</h3>
<p>Реверсивные итераторы позволяют перебирать элементы контейнера в обратном направлении. Для получения реверсивного итератора применяются функции 
<span class="b">rbegin()</span> и <span class="b">rend()</span>, а сам итератор представляет тип <span class="b"></span>:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;vector&gt;
...
<p>Если надо обеспечить защиту от изменения значений контейнера, то можно использовать константный реверсивный итератор, который представлен типом 
<span class="b">const_reverse_iterator</span> и который можно получить с помощью функций <span class="b">crbegin()</span> и <span class="b">crend()</span>:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;vector&gt;
...
<ul>
<li><p><span class="b">iter + n</span>: возвращает итератор, который смещен от итератора iter на n позиций вперед</p></li>
<li><p><span class="b">iter - n</span>: возвращает итератор, который смещен от итератора iter на n позиций назад</p></li>
<li><p><span class="b">iter += n</span>: перемещает итератор на n позиций вперед</p></li>
<li><p><span class="b">iter -= n</span>: перемещает итератор на n позиций назад</p></li>
<li><p><span class="b">iter1 - iter2</span>: возвращает количество позиций между итераторами iter1 и iter2</p></li>
<li><p><span class="b">&gt;, &gt;=, &lt;, &lt;=</span>: операции сравнения. Один итератор больше другого, если указывает на элемент, который ближе к концу</p></li>
</ul>
<p>Применение некоторых операций:</p>
<pre class="brush:cpp;">
std::vector&lt;int&gt; v = {1, 2, 3, 4, 5};
auto iter1 = v.begin();
auto iter2 = iter1 + 2;
