...
<html >
<head>
<title>C++ | Вектор</title>
<meta charset="utf-8" />
<meta name="description" content="Вектор в языке програмирования C++, тип vector, его конструкторы и функции, создание и использование векторов. получение и изменение элементов вектора">
...
<h2>Вектор</h2><div class="date">Последнее обновление: 02.10.2017</div>
...
<p>Вектор представляет контейнер, который содержит коллекцию объектов одного типа. Для работы с векторами необходимо включить заголовок:</p>
<pre class="brush:cpp;">#include &lt;vector&gt;</pre>
<p>Определим простейший вектор:</p>
<pre class="brush:cpp;">
std::vector&lt;int&gt; numbers;
</pre>
<p>В угловых скобках указывается тип, объекты которого будут храниться в векторе. То есть вектор numbers хранит объекты типа int. 
Однако такой вектор пуст. Он не содержит никаких элементов.</p>
...
<p>При этом можно хранить в векторе элементы только одного типа, который указан в угловых скобках. Значения дугих типов в вектор сохранить нельзя, как например, в следующем случае:</p>
<pre class="brush:cpp;">std::vector&lt;int&gt; v{5, "hi"};</pre>
<h3>Обращение к элементам и их перебор</h3>
<p>Для обращения к элементам вектора можно использовать разные способы:</p>
<ul>
<li><p><span class="b">[index]</span>: получение элемента по индексу (также как и в массивах), индексация начинается с нуля</p></li>
<li><p><span class="b">at(index)</span>: функция возращает элемент по индексу</p></li>
...
<p>При этом следует учитывать, что индексация не добавляет элементов. Например, если вектор содержит 5 элементов, то мы не можем 
обратиться к шестому элементу:</p>
<pre class="brush:cpp;">
std::vector&lt;int&gt; numbers= {1, 2, 3, 4, 5};
numbers[5] = 9;
</pre>
<p>При таком обращении результат неопределен. Некоторые комиляторы могут генерировать ошибку, некоторые продолжат работать, но даже в этом случае 
такое обращение будет ошибочно, и оно в любом случае не добавит в вектор шестой элемент.</p>
<p>Чтобы избежать подобных ситуаций, можно использовать функцию <span class="b">at()</span>, которая хотя также возвращает элемент по индексу, 
но при попытке обращения по недопустимому индексу будет генерировать исключение <span class="b">out_of_range</span>:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;vector&gt;
#include &lt;stdexcept&gt;
