...
<html >
<head>
<title>C++ | Область видимости объектов</title>
<meta charset="utf-8" />
<meta name="description" content="Область видимости объектов в языке программирования C++, глобальные и локальные объекты, автоматические и статические переменные">
...
<h2>Область видимости объектов</h2><div class="date">Последнее обновление: 22.09.2017</div>
...
<p>Область видимости (scope) представляет часть программы, в пределах которой можно использовать объект. Как правило, область видимости ограничивается блоком кода, который заключается в 
фигурные скобки В зависимости от области видимости создаваемые объекты могут быть глобальными, локальными или автоматическими.</p>
...
<h3>Локальные объекты</h3>
<p>Объекты, которые создаются внутри блока кода (он может представлять функцию или какую-либо конструкцию типа циклов), называются локальными. 
Такие объекты доступны в пределах только того блока кода, в котором они определены.</p>
<h4>Автоматические объекты</h4>
<p>Локальные объекты, которые существуют только во время выполнения того блока, в котором они определены, являются <span class="b">автоматическими</span>.</p>
<p>При входе в блок для подобных переменных выделяется память, а после завершения работы этого блока, выделенная память освобождается, а 
объекты удаляются.</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Здесь в функции print определена локальная переменная n. В функции main определена автоматическая переменная z. Вне своих функций 
...
print. Соответственно также мы не можем использовать переменную z в функции print, так как эта переменная ограничена фукцией main.</p>
<p>Параметры функции также, как и локальные переменные, существуют, пока выполняется функция, вне функции они не доступны.</p>
...
<p>Для каждой области видимости доступны все те объекты, которые определены во внешней области видимости или во внешнем контексте. Глобальная область видимости является внешней для функции, 
...
Однако переменные, определенные в блоке кода, вне этого блока использовать нельзя.</p>
<h3>Сокрытие объектов</h3>
<p>Локальные объекты, определенные внутри одного контекста, могут скрывать объекты с тем же именем, определенные во внешнем контексте:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Здесь определено три переменных с именем <span class="b">n</span>. Переменная n, определенная на уровне функции main 
(<code>int n = 10;</code>) скрывает глобальную переменную n. А переменная n, определенная на уровне блока, скрывает переменную, определенную 
на уровне функции main.</p>
<h3>Статические объекты</h3>
<p>Кроме автоматических есть особый тип локальных объектов - статические объекты. Они определяются на уровне функций с помощью ключевого 
слова <span class="b">static</span>. Если автоматические переменные определяются и инициализируются при каждом входе в функцию, то статические переменные инициализируются только один раз, а при последующих вызовах функции используется старое значение статической переменной.</p>
...
<p>Функция display вызывается три раза, и при каждом вызове программа повторно будет выделять память для переменной i, которая определена в 
функции. А после завершения работы display, память для переменной i будет освобождаться. Соответственно ее значение при каждом вызове будет неизменно:</p>
<div class="console">
...
<p>К переменной был добавлено ключевое слово static, поэтому при завершении работы функции display переменная не уничтожается, ее память не очищается, наоборот, она сохраняется в памяти. И соответственно результат работы программы будет иным:</p>
<div class="console">
<pre class="consoletext">
i=1
i=2
