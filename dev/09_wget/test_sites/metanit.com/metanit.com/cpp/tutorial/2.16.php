...
<html >
<head>
<title>C++ | Строки</title>
<meta charset="utf-8" />
<meta name="description" content="Строки в языке программирования C++, тип std::string, определение, сравнение и объединение строк">
...
<h2>Строки</h2><div class="date">Последнее обновление: 30.09.2017</div>
...
<p>Для хранения строк в C++ применяется тип <span class="b">string</span>. Для использования этого типа его необходимо подключить в код с помощью директивы 
<span class="b">include</span>:</p>
<pre class="brush:cpp;">
#include &lt;string&gt;
#include &lt;iostream&gt;
...
<p>Тип string определен в стандартной библиотеке и при его использовании надо указывать пространство имен <span class="b">std</span>.</p>
<p>Либо можно использовать выражение using, чтобы не указывать префикс std:</p>
<pre class="brush:cpp;">using std::string;</pre>
<p>В данном случае значение переменной hello, которая представляет тип string, выводится на консоль.</p>
<p>При компиляции через g++ может потребоваться указать флаг <span class="ii">-static</span>. То есть если код определен в файл hello.cpp, 
то команда на компиляцию для g++ может выглядеть следующим образом:</p>
<pre class="brush:cpp;">g++ hello.cpp -o hello -static</pre>
<p>Для инициализации строк можно использовать различные способы:</p>
<pre class="brush:cpp;">
#include &lt;string&gt;
#include &lt;iostream&gt;
...
<p>Если при определении переменной типа string мы не присваиваем ей никакого значения, то по умолчанию данная переменная содержит пустую строку:</p>
<pre class="brush:cpp;">std::string s1;</pre>
<p>Также можно инициализировать переменную строчным литералом, который заключается в двойные кавычки:</p>
<pre class="brush:cpp;">std::string s2 = "hello";</pre>
<p>В качестве альтернативы можно передавать строку в скобках после определения переменной:</p>
<pre class="brush:cpp;">std::string s3("welcome");</pre>
<p>Если необходимо, чтобы строка содержала определенное количесто определеных символов, то можно указать в скобках количество символов и сам символ:</p>
<pre class="brush:cpp;">std::string s4(5, 'h');</pre>
<p>И также можно передать переменной копию другой строки:</p>
<pre class="brush:cpp;">std::string s5 = s2;</pre>
<h3>Конкатенация строк</h3>
<p>Над строками можно выполнять ряд операций. В частности, можно объединять строки с помощью стандартной операции сложения:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
using std::cout;
using std::endl;
...
<p>К строкам можно применять операции сравнения. Оператор <span class="b">==</span> возвращает true, если все символы обеих строк равны.</p>
<pre class="brush:cpp;">
std::string s1 = "hello";
std::string s2 = "world";
...
<p>При этом символы должны совпадать в том числе по регистру.</p>
<p>Операция <span class="b">!=</span> возвращает true, если две строки не совпадают.</p>
<pre class="brush:cpp;">
std::string s1 = "hello";
std::string s2 = "world";
...
<p>Остальные базовые операции сравнения <span class="b">&lt;</span>, <span class="b">&lt;=</span>, <span class="b">&gt;</span>, <span class="b">&gt;=</span> 
...
А строка "a" больше строки "A". Если первые символы строки равны, то сравниваются последующие символы:</p>
<pre class="brush:cpp;">
std::string s1 = "Aport";
std::string s2 = "Apricot";
bool result = s1 &gt; s2;		// false
</pre>
<p>В данном случае условие s1 &gt; s2 ложно, то есть s2 больше чем s1, так как при равенстве первых двух символов ("Ap") третий символ второй 
строки ("o") стоит в алфавите до третьего символа второй строки ("r"), то есть "o" меньше чем "r".</p>
<h3>Размер строки</h3>
<p>С помощью метода <span class="b">size()</span> можно узнать размер строки, то есть из скольких символов она состоит:</p>
<pre class="brush:cpp;">
std::string s1 = "hello";
std::cout &lt;&lt; s1.size() &lt;&lt; std::endl;	// 5
</pre>
<p>Если строка пустая, то она содержит 0 символов. В этом случае мы можем применить метод <span class="b">empty()</span> - он возвращает true, если строка пустая:</p>
<pre class="brush:cpp;">
std::string s1 = "";
if(s1.empty())
	std::cout &lt;&lt; "String is empty" &lt;&lt; std::endl;
</pre>
<h3>Чтение строки с консоли</h3>
<p>Для считывания введенной строки с консоли можно использовать объект std::cin:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;string&gt;
...
<p>Однако если при данном способе ввода строка будет содержать подстроки, разделенные пробелом, то std::cin будет использовать только первую подстроку:</p>
<div class="console">
<pre class="consoletext">
...
<p>Чтобы считать всю строку, применяется метод <span class="b">getline()</span>:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;string&gt;
...
<p>Метод getline принимает два объекта - std::cin и переменную, в которую надо считать строку.</p>
<p>Консольный вывод:</p>
<div class="console">
<pre class="consoletext">
Input your name: Tom Smith
...
<h3>Получение и изменение символов строки</h3>
<p>Подобно массиву мы можем обращаться с помощью индексов к отдельным символам строки, получать и изменять их:</p>
<pre class="brush:cpp;">
...
<h3>Символьные массивы</h3>
<p>Массив символов, последний элемент которого представляет нулевой символ '\0', может использоваться как строка:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Данный код выведет на консоль строку "hello". Подобное определение массива строк будет также эквивалентно следующему:</p>
<pre class="brush:cpp;">char letters[] = "hello";</pre>
<p>Однако подобное использование массива строк унаследовано от языка Си, а при написании программ на С++ при работе со строками 
следует отдавать предпочтение встроенному типу string, а не массиву символов.</p>
...
<div class="nav"><p><a href="2.15.php">Назад</a><a href="./">Содержание</a><a href="3.1.php">Вперед</a></p></div></div>
<div class="socBlock">
