...
<title>C++ | Array</title>
<meta charset="utf-8" />
<meta name="description" content="Контейнер array в языке програмирования C++, сравнение с массивом, функции типа array">
...
<h2>Array</h2><div class="date">Последнее обновление: 03.11.2017</div>
...
<p>Контейнер <span class="b">array</span> представляет аналог массива. Он также имеет фиксированный размер.</p>
<p>Для создания объекта array в угловых скобках после названия типа необходимо передать его тип и размер:</p>
<pre class="brush:cpp;">
std::array&lt;int, 6&gt; numbers;		// состоит из 6 чисел
std::array&lt;std::string, 5&gt; strings;	// состоит из 5 строк
</pre>
<p>Фиксированный размер накладывает ограничение на инициализацию и использование подобных контейнеров. В частности, для инициализации мы не можем 
...
<p>Если при инициализации мы передадим меньшее количество элементов, то для недостающих элементов будут использоваться значения по умолчанию 
(например, для целочисленных типов это число 0). Однако если при инициализации мы предадим большее количество элементов, нежели размер контейнера, то мы столкнемся с ошибкой.</p>
<p>В контейнер array нельзя добавлять новые элементы, так же как и удалять уже имеющиеся. Основные функции типа array, которые мы можем использовать:</p>
<ul>
<li><p><span class="b">size()</span>: возвращает размер контейнера</p></li>
<li><p><span class="b">at(index)</span>: возвращает элемент по индексу index</p></li>
<li><p><span class="b">front()</span>: возвращает первый элемент</p></li>
<li><p><span class="b">back()</span>: возвращает последний элемент</p></li>
<li><p><span class="b">fill(n)</span>: присваивает всем элементам контейнера значение n</p></li>
</ul>
...
<p>Несмотря на то, что объекты array похожи на обычные массивы, однако тип array более гибок. Например, мы не можем присваивать одному массиву напрямую значения второго массива. 
В то же время объекту array мы можем передавать данные другого объекта array:</p>
<pre class="brush:cpp;">
