...
<meta charset="utf-8" />
<meta name="description" content="Первая программа на языке С++ в Visual Studio, установка, настройка проекта и его компиляция">
...
<h2>Первая программа в Visual Studio</h2><div class="date">Последнее обновление: 28.08.2017</div>
...
<p>Для создания программы на C++ нам нужны, как минимум, две вещи: текстовый редактор для набора кода и компилятор для превращения этого кода в приложение. 
...
а также имеют еще множество разных вспомогательных возможностей.</p>
<p>Для программирования под Windows наиболее популярной средой разработки, если говорить о C++, является Visual Studio. Данную среду можно найти по ссылке 
<a href="https://www.visualstudio.com/ru/vs/" rel="nofollow">https://www.visualstudio.com/ru/vs/</a>. Она доступна в ряде редакций. В частности, 
можно использовать бесплатную и полнофункциональную версию Visual Studio 2017 Community.</p>
...
<img src="pics/vs1.png" alt="Установка С++ в Visual Studio 2017" />
<p>Выбрав все необходимые пункты, нажмем ОК для запуска установки. После установки Visual Studio создадим первый проект. Для этого перейдем в меню 
<span class="b">File (Файл) -&gt; New (Создать) -&gt; Project... (Проект)</span>, и нам откроется окно создания нового проекта. В нем перейдем в левой части окна к 
языку С++ и выберем его подсекцию <span class="b">General</span>:</p>
<img src="pics/vs2.png" alt="Первый проект на Си в Visual Studio" />
<p>В центральной части окна в качестве типа проекта выберем <span class="b">Empty Project</span>, а внизу окна в поле для имени проекта дадим проекту имя HelloApp и нажмем на ОК для создания проекта.</p>
...
<img src="pics/vs5.png" alt="Добавление файла на С++ в Visual Studio 2017" />
<p>Затем нам откроется окно для добавления нового элемента:</p>
<img src="pics/vs6.png" alt="Добавление исходного файла на C++ в Visual Studio 2017" />
<p>Здесь нам надо выбрать пункт <span class="b">C++ File(.cpp)</span>, а внизу окна укажем для файла имя <span class="b">Hello.cpp</span>. Как правило, 
исходные файлы на C++ имеют расширение <span class="ii">.сpp</span>.</p>
<p>После добавления файла изменим опции проекта. Для этого перейдем к пункту меню <span class="b">Project -&gt; Properties</span>:</p>
<img src="pics/vs3.png" alt="Свойства проекта С++ в Visual Studio" />
<p>И в открывшемся окне свойств проекта в левой части перейдем к секции <span class="b">Linker -&gt;System</span> и далее для 
...
<p>Тем самым мы указываем, что мы хотим создать консольное приложение. После установки этого значения нажмем на кнопку "Применить", чтобы новые настройки конфигурации вступили в силу.</p>
<p>После добавления файла проект будет иметь следующую структуру:</p>
<img src="pics/vs7.png" alt="Структура проекта на C++ в Visual Studio 2017" />
...
<p>Теперь собственно создадим первую программу и определим в файле Hello.cpp простейший код, который будет выводить строку на консоль:</p>
<pre class="brush:c;">
...
<p>Здесь использован весь тот код, который был рассмотрен в предыдущих темах про компиляцию с помощью g++.</p>
...
<img src="pics/vs8.png" alt="Запуск программы на C++ в Visual Studio" />
...
<img src="pics/vs10.png" alt="Первое приложение на языке программирования C++" />
<p>После этого на жестком диске в проекте в папке Debug можно увидеть скомпилированный файл exe, который мы можем запускать независимо от Visual Studio:</p>
<img src="pics/vs11.png" alt="Компиляция кода на языке программирования C++" />
<p>В данном случае файл <span class="b">HelloApp.exe</span> как раз и представляет скомпилированный исполняемый файл. 
И, кроме того, в той же папке автоматически генерируются два вспомогательных файла:</p>
<ul>
