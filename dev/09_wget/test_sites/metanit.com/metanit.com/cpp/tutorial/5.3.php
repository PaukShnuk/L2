...
<html >
<head>
<title>C++ | Объявление и определение функций класса</title>
<meta charset="utf-8" />
<meta name="description" content="Разделение объявления и определения функций в языке програмирования C++, оператор ::">
...
<h2>Объявление и определение функций класса</h2><div class="date">Последнее обновление: 05.10.2017</div>
...
<p>В языке C++ можно разделять объявление и определение функций в том числе по отношению к функциям, которые создаются в классах. Для 
этого используется выражение <code>имя_класса::имя_функции(параметры) { тело_функции}</code>.</p>
<p>Например, возьмем следующий класс Person:</p>
<pre class="brush:cpp;">
class Person
{
public:
...
<p>Разобъем класс, вынеся реализацию его методов во вне:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;string&gt;
using std::string;
...
<p>Теперь функции класса Person (в данном случае конструктор и функция move) в самом классе имеют только объявления. Реализации функций размещены вне класса Person. </p>
