...
<html >
<head>
<title>C++ | Разделение программы на файлы</title>
<meta charset="utf-8" />
<meta name="description" content="Разделение программы на файлы в языке программирования C++, заголовочные файлы и их подключение">
...
<h2>Разделение программы на файлы</h2><div class="date">Последнее обновление: 22.09.2017</div>
...
<p>Если программа содержит много кода, то более оптимально было бы разнести отдельные части кода по отдельным файлам. Например, одни функции могут храниться в одном файле исходного кода, 
другие функции - в другом файле.</p>
<p>Например, определим файл <span class="b">factorial.cpp</span>, который будет иметь следующий код:</p>
<pre class="brush:cpp;">
int factorial(int n)
{
	if (n &gt; 1)
...
<p>Это функция вычисления факториала.</p>
<p>Добавим еще один файл <span class="b">factorial.h</span>, который будет содержать объявление функции factorial:</p>
<pre class="brush:cpp;">int factorial(int);</pre>
<p>И также определим главный файл, который назовем <span class="b">app.cpp</span>:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include "factorial.h"
...
<p>Функция main вызывает функцию factorial для вычислеия факториала числа. Но чтобы использовать функцию, определенную в другом файле, необходимо добавить 
...
заголовок функции. Файл factorial.h еще называется заголовочным файлом (header file).</p>
<p>Можно было бы и не подключать файл factorial.h и вообще не создавать его, а объявление функции поместить непосредственно в файл app.cpp. 
...
В данном же случае достаточно изменить объявление функции в одном файле - factorial.h.</p>
<img src="pics/4.1.png" alt="header files in c++" />
<p>При компиляции в Visual Studio все файлы автоматически компилируются в один. При компиляции через g++ необходимо передать все файлы через пробел компилятору:</p>
<div class="console">
<pre class="consoletext">
g++ app.cpp factorial.cpp -o app
</pre>
</div>
<p>На выходе будет сгенерирован единый файл app.</p>
