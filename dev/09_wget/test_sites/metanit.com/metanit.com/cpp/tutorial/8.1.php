...
<html >
<head>
<title>C++ | Базовые типы для работы с потоками</title>
<meta charset="utf-8" />
<meta name="description" content="Базовые типы для работы с потоками в языке програмирования C++, потоки ostream и istream, операторы ввода и вывода, объекты cout и cin">
...
<h1>Потоки и система ввода-вывода</h1><h2>Базовые типы для работы с потоками</h2><div class="date">Последнее обновление: 25.10.2017</div>
...
<p>Все инструменты для работы с системой ввода-вывода и потоками в языке С++ определены в стандартной библиотеке. 
Заголовочный файл iostream определяет следующие базовые типы для работы с потоками:</p>
<ul>
<li><p><span class="b">istream</span> и <span class="b">wistream</span>: читают данные с потока</p></li>
<li><p><span class="b">ostream</span> и <span class="b">wostream</span>: записывают данные в поток</p></li>
<li><p><span class="b">iostream</span> и <span class="b">wiostream</span>: читают и записывают данные в поток</p></li>
</ul>
<p>Для каждого типа определен его двойник, который начинается на букву w и который предназначен для поддержки данных типа wchar_t.</p>
<p>Эти типы являются базовыми для других классов, управляющих потоками ввода-вывода.</p>
<p>Объект типа <span class="b">ostream</span> получает значения различных типов, преобразует их в последовательность символов и 
передает их через буфер в определенное место для вывода (консоль, файл, сетевые интерфейсы и т.д.)</p>
<img src="pics/8.1.png" alt="Поток вывода ostream в С++" />
<p>Поток <span class="b">istream</span> получает через буфер из определенного места последовательности символов (с консоли, из файла, из сети и т.д.) 
...
затем передаются объекту istream.</p>
<img src="pics/8.2.png" alt="Поток ввода istream в С++" />
<p>По умолчанию в стандартной библиотеке определены объекты этих классов - <span class="b">cout</span>, <span class="b">cin</span>, 
<span class="b">cerr</span>, которые работают с консолью.</p>
<h3>Запись в поток</h3>
<p>Для записи данных в поток ostream применяется оператор <span class="b">&lt;&lt;</span>. Этот оператор получает два 
операнда. Левый операнд представляет объект типа ostream, а правый операнд - значение, которое надо вывести в поток.</p>
<p>К примеру, по умолчанию стандартная библиотека C++ предоставляет объект <span class="b">cout</span>, который представляет тип 
ostream и позволяет выводить данные на консоль:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Так как оператор <span class="b">&lt;&lt;</span> возвращает левый операнд - cout, то с помощью цепочки операторов мы 
можем передать на консоль несколько значений:</p>
<pre class="brush:cpp;">std::cout &lt;&lt; "Hello" &lt;&lt; " world" &lt;&lt; std::endl;</pre>
<h3>Чтение данных</h3>
<p>Для чтения данных из потока применяется оператор ввода <span class="b">&gt;&gt;</span>, который принимает два операнда. 
Левый операнд представляет поток istream, с которого производится считывание, а правый операнд - объект, в который считываются данные.</p>
<p>Для чтения с консоли применяется объект <span class="b">cin</span>, который представляет тип istream.</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Однако такой способ не очень подходит для чтения строк с консоли особенно когда считываемая строка содержит пробельные символы. В этом случае 
лучше использовать встроенную функцию <span class="b">getline()</span>, которая в качестве параметра принимает поток istream и переменную типа string, в которую надо считать данные:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;string&gt;
...
<h3>Вывод ошибок</h3>
<p>Для вывода сообщения об ошибке на консоль применяется объект <span class="b">cerr</span>, который представляет объект типа ostream:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<h3>Потоки символов wchar_t</h3>
<p>Для работы с потоками данных типов wchar_t в стандартной библиотеке определены объекты <span class="b">wcout</span> 
(тип wostream), <span class="b">wcerr</span> (тип wostream) и <span class="b">wcin</span> (тип wistream), которые являются аналогами 
для объектов cout, cerr и cin и работают аналогично</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<div class="nav"><p><a href="7.8.php">Назад</a><a href="./">Содержание</a><a href="8.2.php">Вперед</a></p></div></div>
<div class="socBlock">
