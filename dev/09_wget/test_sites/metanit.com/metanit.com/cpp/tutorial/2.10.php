...
<html >
<head>
<title>C++ | Ввод и вывод в консоли</title>
<meta charset="utf-8" />
<meta name="description" content="Ввод и вывод на консоль в языке программирования C++, потоки istream и ostream, объекты cout и cin">
...
<h2>Ввод и вывод в консоли</h2><div class="date">Последнее обновление: 14.09.2017</div>
...
<p>По умолчанию язык C++ не содержит встроенных средств для ввода с консоли и вывода на консоль, эти средства предоставляются библиотекой 
<span class="b">iostream</span>. В ней определены два типа: <span class="b">istream</span> и <span class="b">ostream</span>. istream представляет 
поток ввода, а ostream - поток вывода.</p>
<p>Вобще сам темин "поток" в данном случае представляет последовательность символов, которая записывается на устройство ввода-вывода или считывается с него. 
И в данном случае под устройством ввода-вывода рассматривается консоль.</p>
<p>Для записи или вывода символов на консоль применяется объект <span class="b">cout</span>, который представляет тип ostream. А для чтения с консоли 
используется объект <span class="b">cin</span></p>
...
<p>Для вывода на консоль применяется оператор <span class="b">&lt;&lt;</span>. Этот оператор получает два операнда. Левый операнд представляет 
объект типа ostream, в данном случае объект cout. А правый операнд - значение, которое надо вывести на консоль.</p>
<p>Так как оператор &lt;&lt; возвращает левый операнд - cout, то с помощью цепочки операторов мы можем передать на консоль несколько значений. 
Например, определим простейшую программу вывода на консоль:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Оператору &lt;&lt; передаются различные значения - строки, значения переменных, которые выводятся на консоль.</p>
<p>Строки могут содержать управляющие последовательности, которые интерпретируются определенным образом. Например, последовательность 
...
как табуляция.</p>
<p>Также цепочку операторов &lt;&lt; можно завершать значением <span class="b">std::endl</span>, которое вызывает перевод на новую строку и 
...
немедлено будут выведены на консоль.</p>
<h3>Ввод с консоли</h3>
<p>Для считывания с консоли данных применяется оператор ввода <span class="b">&gt;&gt;</span>, который принимает два операнда. Левый операнд представляет 
объект типа istream (в данном случае объект cin), с которого производится считывание, а правый операнд - объект, в который считываются данные.</p>
...
<p>Здесь после приглашений к вводу программа ожидает ввода значений для переменных age и weight.</p>
...
<p>Стоит отметить, что так оператор ввода в первом случае будет добавлять данные в целочисленную переменную age, то он ожидает ввода числа. В случае с 
...
В этом случае программа может выдать некорректный результат.</p>
<p>Оператор ввода &gt;&gt; возвращает левый операнд - объект cin, поэтому мы можем по цепочке считывать данные в различные переменные:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>После ввода одного из значений надо будет ввести пробел и затем вводить следующее значение.</p>
