...
<html >
<head>
<title>C++ | Структуры</title>
<meta charset="utf-8" />
<meta name="description" content="Структуры в языке програмирования C++, определение и использование структур, их отличие от классов, ключевое слово struct, аггрегатные классы">
...
<h2>Структуры</h2><div class="date">Последнее обновление: 09.10.2017</div>
...
<p>Ранее для определения классов мы использовали ключевое слово <span class="b">class</span>. Однако C++ предоставляет еще один способ 
для определения пользовательских типов, который заключается в использовании структур. Данный способ был унаследован языком С++ еще от языка Си.</p>
<p>Структура в языке C++ представляет собой производный тип данных, который представляет какую-то определенную сущность, также как и класс. 
Нередко структуры применителько к С++ также называют классами. И в реальности различия между ними не такие большие.</p>
...
<p>После имени структуры в фигурных скобках помещаются <span class="ii">Компоненты_структуры</span>, которые представляют набор описаний объектов и функций, которые составляют структуру.</p>
...
<pre class="brush:cpp;">имя_переменной_структуры.имя_элемента</pre>
<p>По сути структура похожа на класс, то есть с помощью структур также можно определять сущности для использования в программе. В то же время 
...
Тогда как в классе все его члены, для которых не указан спецификатор доступа, являются закрытыми (private).</p>
<p>Кроме того мы можем инициализировать структуру, присвоив ее переменным значения с помощью синтаксиса инициализации:</p>
<pre class="brush:cpp;">person tom = { 34, "Tom" };</pre>
...
<p>При этом любой класс мы можем представить в виде структуры и наоборот. Возьмем, к примеру, следующий класс:</p>
<pre class="brush:cpp;">
class Person
{
public:
...
<p>Данный класс определяет сущность человека и содержит ряд приватных и публичных переменных и функции. Вместо класса для определения той же сущности мы могли 
бы использовать структуру:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;string&gt;
...
<p>И в плане конечного результата программы мы не увидели бы никакой разницы.</p>
<p>Когда использовать структуры? Как правило, структуры используются для описания таких данных, которые имеют только набор публичных атрибутов - открытых переменных. Например, как та же структура person, которая 
была определена в начале статьи. Иногда подобные сущности еще называют <span class="b">аггрегатными классами</span> (aggregate classes).</p>
