...
<html >
<head>
<title>C++ | Определение и объявление функций</title>
<meta charset="utf-8" />
<meta name="description" content="Определение и объявление функций в языке программирования C++, их вызов, функция main">
...
<h1>Функции</h1><h2>Определение и объявление функций</h2><div class="date">Последнее обновление: 22.09.2017</div>
...
<p>Функция определяет действия, которые выполняет программа. Функции позволяют выделить набор инструкций и придать ему имя. А затем многократно 
по присвоенному имени вызывать в различных частях программы. По сути функция - это именованный блок кода.</p>
...
<p>Первая строка представляет заголовок функции. Вначале указывается возвращаемый тип функции. Если функция не возвращает никакого значения, то используется тип <span class="b">void</span>.</p>
<p>Затем идет имя функции, которое представляет произвольный идентификатор. К именованию функции применяются те же правила, что и к 
именованию переменных.</p>
...
<p>После заголовка функции в фигурных скобках идет тело функции, которое содержит выполняемые инструкции.</p>
<p>Для возвращения результата функция применяет оператор <span class="b">return</span>. Если функция имеет в качестве возвращаемого типа любой тип, кроме void, то она должна обязательно с помощью оператора return 
возвращать какое-либо значение.</p>
<p>Например, определение функции main, которая должна быть в любой программе на языке C++ и с которой начинается ее выполнение:</p>
<pre class="brush:cpp;">
int main()
{
    return 0;
...
<p>Возвращаемым типом функции является тип <span class="b">int</span>, поэтому функция должна использовать оператор <span class="b">return</span> и возвращать какое-либо значение, которое соответствует типу int. 
Возвращаемое значение ставится после оператора return.</p>
<p>Но если функция имеет тип <span class="b">void</span>, то ей не надо ничего возвращать. Например, мы могли бы определить следующую функцию:</p>
<pre class="brush:cpp;">
void hello()
{
	std::cout &lt;&lt; "hello\n";
...
<p>Здесь определена функция hello, которая вызывается в функции main два раза. В этом и заключается преимущество функций: 
мы можем вынести некоторые общие действия в отдельную функцию и затем вызывать многократно в различных местах программы. 
В итоге программа два раза выведет строку "hello".</p>
<div class="console">
...
<p>При использовании функций стоит учитывать, что компилятор должен знать о функции до ее вызова. Поэтому вызов функции должен происходить 
...
примеру, мы сначала вызовем, а потом определим функцию, то мы получим ошибку на этапе компиляции, как в следующем случае:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>В этом случае перед вызовом функции надо ее дополнительно объявить. Объявление функции еще называют прототипом. Формальное объявление 
выглядит следующим образом:</p>
<pre class="brush:cpp;">тип имя_функции(параметры);</pre>
<p>Фактически это заголовок функции. То есть для функции hello объявление будет выглядеть следующим образом:</p>
<pre class="brush:cpp;">void hello();</pre>
<p>Используем объявление функции:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>В данном случае несмотря на то, что определение функции идет после ее вызова, но так как функция уже объявлена до ее вызова, то 
компилятор уже будет знать о функции hello, и никаких проблем в работе программы не возникнет.</p>
...
<div class="nav"><p><a href="2.16.php">Назад</a><a href="./">Содержание</a><a href="3.2.php">Вперед</a></p></div></div>
<div class="socBlock">
