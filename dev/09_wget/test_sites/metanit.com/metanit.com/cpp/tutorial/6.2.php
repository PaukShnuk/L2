...
<meta charset="utf-8" />
<meta name="description" content="Тип exception в языке програмирования C++, генерация исключения, азличия в реализации комиляторов">
...
<h2>Тип exception</h2><div class="date">Последнее обновление: 05.10.2017</div>
...
<p>Все исключения в языке C++ описываются типом <span class="b">exception</span>, который определен в заголовочном файле <span class="b">&lt;exception&gt;</span>. 
И при обработке исключений мы также можем использовать данный класс. Например:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;exception&gt;
...
<p>Прежде всего, оператору throw передается объект типа <span class="b">std::exception</span>. Данный объект инициализируется строкой "Division by zero!", 
которая передает информацию о характере ошибки.</p>
<pre class="brush:cpp;">throw std::exception("Division by zero!");</pre>
<p>Если мы хотим отловить исключения типа exception, то нам надо в выражении catch определить переменную этого типа:</p>
<pre class="brush:cpp;">catch (const std::exception& err)</pre>
<p>То есть здесь err представляет переменную типа exception. Не обязательно определять переменную именно как константную ссылку, можно определить ее и так:</p>
<pre class="brush:cpp;">catch (std::exception err)</pre>
<p>Если мы не собираемся использовать эту переменную в блоке catch, то можно указать просто тип исключения:</p>
<pre class="brush:cpp;">
catch (std::exception)
{
...
<p>Также стоит отметить различия в реализации компиляторов. Так, Visual Studio позволяет инициализировать объект exception строкой. Например, можно передать сообщение об ошибке:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;exception&gt;
...
<p>С помощью метода <span class="b">what()</span> в блоке catch можно получить переданное сообщение и, к примеру, вывести его на консоль:</p>
<pre class="brush:cpp;">
catch (std::exception err)
{
	std::cout &lt;&lt; err.what() &lt;&lt; std::endl;
...
<p>Но в g++ этот код работать не будет, так как тип exception для g++ не может быть инициализирован строкой.</p>
