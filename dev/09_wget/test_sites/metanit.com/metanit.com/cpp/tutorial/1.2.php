...
<html >
<head>
<title>С++ | Первая программа на Windows. Компилятор g++</title>
<meta charset="utf-8" />
<meta name="description" content="Компилятор g++. Первая программа на языке С++ на Windows, набор компиляторов GCC Mingw-w64 и его установка">
...
<h2>Первая программа на Windows. Компилятор g++</h2><div class="date">Последнее обновление: 28.08.2017</div>
...
который принимает файл с исходным кодом и компилирует его в исполняемый файл.</p>
<p>В качестве компилятора мы будем использовать один из наиболее популярных на сегодня компиляторов - <span class="b">g++</span>, 
который доступен для разных платформ. Более подобному информацию о g++ можно получить на официальном сайте проекта <a href="https://gcc.gnu.org/" rel="nofollow">https://gcc.gnu.org/</a>.</p>
<p>Набор компиляторов g++ распространяется в различных версиях. Одной из наиболее популярных версий является пакет средств для разработки от 
некоммерческого проекта <a href="https://mingw-w64.org/doku.php" rel="nofollow">Mingw-w64</a>. Этот пакет содержит не только g++, но и ряд других компиляторов 
...
<a href="https://sourceforge.net/projects/mingw-w64/files/Toolchains%20targetting%20Win32/Personal%20Builds/mingw-builds/installer/mingw-w64-install.exe/download">https://sourceforge.net/projects/mingw-w64/files/Toolchains%20targetting%20Win32/Personal%20Builds/mingw-builds/installer/mingw-w64-install.exe/download</a></p>
...
<p>В частности, файл <span class="b">g++.exe</span> как раз и будет представлять компилятор для языка С++.</p>
...
<p>Для упрощения запуска компилятора можно добавить путь к нему в <span class="b">Переменные среды</span>. Для этого перейдем к окну 
...
<p>Таким образом, мы установили компилятор и теперь мы можем написать первую программу. Для набора исходного кода потребуется текстовый редактор. 
Можно взять распространенный редактор Notepad++ или даже обычный встроенный Блокнот, либо какой-то другой редактор.</p>
<p>Итак, создадим на жестком диске С папку для исходных файлов. А в этой папке создадим новый текстовый файл, который переименуем в 
<span class="b">hello.cpp</span>. То есть по сути файлы исходного кода на С++ - это обычные текстовые файлы, которые, как правило, имеют расширение 
<span class="b">cpp</span>.</p>
<img src="pics/mingw8.png" alt="Первая программа на С++" />
<p>В моем случае файл hello.cpp находится в папке C:\cpp.</p>
<p>Теперь определим в файле hello.cpp простейший код, который будет выводить строку на консоль:</p>
<pre class="brush:cpp;">
...
С помощью оператора <span class="b">&lt;&lt;</span> в этот поток (в данном случае фактически на консоль) передается строка символов, которую надо вывести на консоль, то есть "Hello world!".</p>
...
<p>Каждая строка снабжена комментарием. Все, что написано после двойного слеша // представляет комментарий. Комментарий не учитывается при компиляции приложения, и 
не является частью программного кода, а служат лишь для его описания. Комментарий позволяет понять, что делает программа.</p>
<img src="pics/mingw9.png" alt="Язык программирования С++ в Notepad++" />
...
<div class="console"><pre class="consoletext">cd C:\cpp</pre></div>
<p>Чтобы скомпилировать исходный код, необходимо компилятору gcc передать в качестве параметра файл hello.cpp:</p>
<div class="console"><pre class="consoletext">g++ hello.cpp -o hello</pre></div>
<p>Дополнительный необязательный параметр <code> -o hello</code> указывает, что скомпилированный файл будет называться hello.exe. Если не указать этот параметр, то файл будет называться по умолчанию - a.exe.</p>
<p>После выполнения этой команды будет скомпилирован исполняемый файл, который в Windows по умолчанию называется hello.exe. И мы можем обратиться к этому файлу, и в этом случае консоль выведет 
строку "Hello World!", собственно как и прописано в коде.</p>
<img src="pics/mingw6.png" alt="Запуск компилятора G++ на Windows" />
