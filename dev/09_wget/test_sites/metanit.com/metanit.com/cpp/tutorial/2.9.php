...
<html >
<head>
<title>C++ | Операции присваивания</title>
<meta charset="utf-8" />
<meta name="description" content="Операции присваивания в языке программирования C++, приоритет операций присвоения">
...
<h2>Операции присваивания</h2><div class="date">Последнее обновление: 13.09.2017</div>
...
<p>Операции присваивания позволяют присвоить некоторое значения. Эти операции выполняются над двумя операндами, причем левый операнд 
может представлять только модифицируемое именованное выражение, например, переменную.</p>
<p>Базовая операция присваивания <span class="b">=</span> позволяет присвоить значение правого операнда левому операнду:</p>
<pre class="brush:cpp;">
int x; 
x = 2
</pre>
<p>То есть в данном случае переменная x (левый операнд) будет иметь значение 2 (правый операнд).</p>
<p>Стоит отметить, что тип значения правого операнда не всегда может совпадать с типом левого операнда. В этом случае компилятор пытается преобразовать 
значение правого операнда к типу левого операнда.</p>
<p>При этом операции присваивания имеют правосторонний порядок, то есть выполняются справа налево. И, таким образом, можно выполнять 
множественное присваивание:</p>
<pre class="brush:cpp;">
int a, b, c;	
a = b = c = 34;
</pre>
<p>Здесь сначала вычисляется значение выражения <code>c = 34</code>. Значение правого операнда - <code>34</code> присваивается левому операнду с. 
Далее вычисляется выражение <code>b = c</code>: значение правого операнда <span class="b">c</span> (34) присваивается левому операнду 
<span class="b">b</span>. И в конце вычисляется выражение <code>a = b</code>: значение правого операнда <span class="b">b</span> (34) присваивается левому операнду 
<span class="b">a</span>.</p>
<p>Кроме того, следует отметить, что операции присваивания имеют наименьший приоритет по сравнению с другими типами операций, поэтому выполняются 
в последнюю очередь:</p>
<pre class="brush:cpp;">
int x;
x = 3 + 5;
</pre>
<p>В соответствии с приоритетом операций вначале выполняется выражение <code>3 + 5</code>, и только потом его значение присваивается переменной x.</p>
...
<span class="b">A %= B</span> эквивалентно <span class="b">A = A % B</span></p></li>
<li><p><span class="b">&lt;&lt;=</span>: присваивание после сдвига разрядов влево. Присваивает левому операнду результат сдвига его битового представления влево на определенное количество разрядов, равное значению 
правого операнда: <span class="b">A &lt;&lt;= B</span> эквивалентно <span class="b">A = A &lt;&lt; B</span></p></li>
<li><p><span class="b">&gt;&gt;=</span>: присваивание после сдвига разрядов вправо. Присваивает левому операнду результат сдвига его битового представления вправо на определенное количество разрядов, равное значению 
правого операнда: <span class="b">A &gt;&gt;= B</span> эквивалентно <span class="b">A = A &gt;&gt; B</span></p></li>
<li><p><span class="b">&=</span>: присваивание после поразрядной конъюнкции. Присваивает левому операнду результат поразрядной конъюнкции его 
битового представления с битовым представлением правого операнда: <span class="b">A &= B</span> эквивалентно <span class="b">A = A & B</span></p></li>
<li><p><span class="b">|=</span>: присваивание после поразрядной дизъюнкции. Присваивает левому операнду результат поразрядной дизъюнкции его 
битового представления с битовым представлением правого операнда: <span class="b">A |= B</span> эквивалентно <span class="b">A = A | B</span></p></li>
<li><p><span class="b">^=</span>: присваивание после операции исключающего ИЛИ. Присваивает левому операнду результат операции исключающего ИЛИ его 
битового представления с битовым представлением правого операнда: <span class="b">A ^= B</span> эквивалентно <span class="b">A = A ^ B</span></p></li>
</ul>
