...
<html >
<head>
<title>C++ | Операторы преобразования типов</title>
<meta charset="utf-8" />
<meta name="description" content="Операторы преобразования типов в языке програмирования C++, перегрузка операций, явные (explicit) преобразования">
...
<h2>Операторы преобразования типов</h2><div class="date">Последнее обновление: 23.10.2017</div>
...
<p>Операторы преобразования (conversion operator) определяют преобразование объекта одного типа в другой. Они имеют следующий 
общий синтаксис:</p>
<pre class="brush:cpp;">
operator тип() const;
</pre>
<p>В данном случае <code>тип</code> - это тот тип, в который надо преобразовать. В качестве типа нельзя использовать массив или функцию, хотя 
указатель на функцию вполне можно использовать.</p>
<p>Операции преобразования не должны изменять преобразуемый объект, поэтому функции операторов определяются с ключевым словом const.</p>
...
<p>В днном случае в классе Counter определен оператор преобразования от типа Counter к типу int:</p>
<pre class="brush:cpp;">
operator int() const
{
	return seconds;
...
<p>Операции преобразования также могут быть явными. В этом случае операторы определяются с ключевым словом <span class="b">explicit</span>. 
А при их применении необходимо использовать операцию приведения типов:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Здесь в классе Counter определен оператор преобразования объекта из типа Counter в тип Clock, то есть по сути из секунд 
необходимо получить часы, минуты и секунды. Этот оператор определен как явная операция преобразования:</p>
<pre class="brush:cpp;">
explicit operator Clock() const
{
	int h = seconds / 3600;
...
<p>Поэтому при использовании данной операции необходимо перед преобразуемым типом ставить операцию приведения - в скобках указывается тип, 
к которому надо пребразовать объект:</p>
<pre class="brush:cpp;">Clock clock = (Clock)counter;</pre>
<p>Также можно определить обратное образование из типа Clock в тип Counter. Для этого можно определить конструктор в классе Counter, 
который применяет ссылку на объект Clock:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<div class="nav"><p><a href="5.14.php">Назад</a><a href="./">Содержание</a><a href="6.1.php">Вперед</a></p></div></div>
<div class="socBlock">
