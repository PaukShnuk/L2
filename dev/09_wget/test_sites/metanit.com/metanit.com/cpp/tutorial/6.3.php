...
<html >
<head>
<title>C++ | Тип exception</title>
<meta charset="utf-8" />
<meta name="description" content="Типы исключений в языке програмирования C++, обработка нескольких видов исключений, stdexcept">
...
<h2>Типы исключений</h2><div class="date">Последнее обновление: 05.10.2017</div>
...
<p>Кроме типа exception в C++ есть еще несколько производных типов исключений, которые могут использоваться при различных ситуациях. Основные из них:</p>
<ul>
<li><p><span class="b">runtime_error</span>: общий тип исключений, которые возникают во время выполнения</p></li>
<li><p><span class="b">range_error</span>: исключение, которое возникает, когда полученный результат превосходит допустимый диапазон</p></li>
<li><p><span class="b">overflow_error</span>: исключение, которое возникает, если полученный результат превышает допустимый диапазон</p></li>
<li><p><span class="b">underflow_error</span>: исключение, которое возникает, если полученный в вычислениях результат имеет недопустимые отрицательное значение (выход за нижнюю допустимую границу значений)</p></li>
<li><p><span class="b">logic_error</span>: исключение, которое возникает при наличии логических ошбок к коде программы</p></li>
<li><p><span class="b">domain_error</span>: исключение, которое возникает, если для некоторого значения, передаваемого в функцию, не определено результата</p></li>
<li><p><span class="b">invalid_argument</span>: исключение, которое возникает при передаче в функцию некорректного аргумента</p></li>
<li><p><span class="b">length_error</span>: исключение, которое возникает при попытке создать объект большего размера, чем допустим для данного типа</p></li>
<li><p><span class="b">out_of_range</span>: исключение, которое возникает при попытке доступа к элементам вне допустимого диапазона</p></li>
</ul>
<img src="pics/6.1.png" alt="Типы исключений в C++ и std::exception" />
<p>Большинство этих типов определено в заголовочном файле <span class="b">stdexcept</span>, за исключением класса <span class="b">bad_alloc</span>, 
который определн в файле <span class="b">new</span>, и класса <span	class="b">bad_cast</span>, который определен в файле 
<span class="b">type_info</span>.</p>
<p>В отличие от классов exception, bad_alloc и bad_cast в конструкторы других типов можно передать строку, то есть таким образом можно передать сообщение об ошибке.</p>
<p>Конструкция try...catch может использовать несколько блоков catch для обработки различных типов исключений. При возникновении исключения 
для его обработки будет выбран тот, который использует тип возникшего исключения.</p>
<p>При использовании нескольких блоков catch вначале помещаются блоки catch, которые обрабатывают более частные исключения, а только потом блоки catch с более 
общими типами исключений:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;exception&gt;
#include &lt;stdexcept&gt;
...
<p>Здесь функция divide, если параметр b равен 0, выбрасывает исключение типа <span class="b">runtime_error</span>. Исключение инициализируется 
сообщением об ошибке "Division by zero!".</p>
<p>В функции main конструкция try..catch использует три блока catch. Причем последний блок представляет самый общий тип исключений exception. Второй блок 
...
runtime_error.</p>
<p>Также все типы исключений имеют метод <span class="b">what()</span>, который возвращает информацию об ошибке. И в данном случае программа выдаст следующий результат:</p>
<div class="console">
