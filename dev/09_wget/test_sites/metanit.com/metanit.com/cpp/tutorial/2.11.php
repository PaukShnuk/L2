...
<html >
<head>
<title>C++ | Пространства имен и using</title>
<meta charset="utf-8" />
<meta name="description" content="Пространства имен и оператор using в языке программирования C++, префикс std, область видимости">
...
<h2>Пространства имен и using</h2><div class="date">Последнее обновление: 14.09.2017</div>
...
<p>При чтении и записи в предыдущих темах использовались объекты <code>std::cout</code> и <code>std::cin</code> соответственно. Причем они использовались с 
префиксом <span class="b">std::</span>. Этот префикс указывает, что объекты cout, cin, endl определены в пространствен имен 
<span class="b">std</span>. А само двойное двоеточие <span class="b">::</span> представляет оператор области видимости (scope operator), который позволяет указать, в каком пространсте 
имен определен объект. И без префикса эти объекты по умолчанию мы использовать не можем.</p>
<p>Однако подобная запись может показаться несколько громоздкой. И в этом случае можно использовать оператор <span class="b">using</span>, 
который позволяет ввести в программу объекты из различных пространств имен.</p>
<p>Использование оператора using имеет следующй формат:</p>
<pre class="brush:cpp;">
using пространство_имен::объект
</pre>
<p>Например, пусть у нас есть следующая программа:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Здесь используются сразу три объекта из програнства имен std: cout, cin и endl. Перепишем программу с использованием using:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
using std::cin;
using std::cout;
...
<p>Для каждого объекта из пространства std определяется свое выражение using. При этом программа будет работать также как и раньше.</p>
