...
<html >
<head>
<title>C++ | Перечисления</title>
<meta charset="utf-8" />
<meta name="description" content="Перечисления в языке програмирования C++, определение и использование перечислений, ключевое слово enum">
...
<h2>Перечисления</h2><div class="date">Последнее обновление: 09.10.2017</div>
...
<p>Перечисления (enum) представляют еще один способ определения своих типов. Их отличительной особенностью является то, что они содержат набор числовых констант.</p>
...
<p>Для определения перечисления применяется ключевое слово <span class="b">enum</span>, после которого идет название перечисления. Затем в 
...
То есть в данном случае spring=0, a winter=3.</p>
...
<p>Мы можем определить переменную типа seasons и присвоить этой переменной значение одной из констант, объявленных в перечислении. Но фактически это будет числовое значение. 
В частности, консольный вывод данной программы:</p>
<pre class="brush:cpp;">Season: 2</pre>
<p>В то же время перечисления - это отдельный тип, поэтому мы не можем присвоить переменной напрямую числовое значение:</p>
<pre class="brush:cpp;">seasons currentSeason = 2;	// ошибка</pre>
<p>Если нас не устраивают значения по умолчанию для констант, то мы можем явным образом задать значения. Например, установить начальное значение:</p>
<pre class="brush:cpp;">
enum seasons
{
	spring = 1,
...
<p>Когда необходимы перечисления? Перечисления могут использоваться, когда у нас есть ряд логически связанных констант, которые естественно 
лучше определить в одном общем типе данных. Например:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
enum operations
{
...
<p>В данном случае все арифметические операции хранятся в перечислении operations. В зависимости от выбранной операции в конструкции switch 
производятся определенные действия.</p>
