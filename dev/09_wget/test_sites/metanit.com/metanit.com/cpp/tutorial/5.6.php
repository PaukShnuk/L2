...
<html >
<head>
<title>C++ | Ключевое слово this</title>
<meta charset="utf-8" />
<meta name="description" content="Ключевое слово this в языке програмирования C++, обращение к членам класса, возвращение ссылки на текущий объект в классе">
...
<h2>Ключевое слово this</h2><div class="date">Последнее обновление: 07.10.2017</div>
...
<p>Ключевое слово this представляет указатель на текущий объект данного класса. Соответственно через this мы можем обращаться внутри класса к любым его членам.</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>В данном случае определен класс Point, который представляет точку на плоскости. И для хранения координат точки в классе определены переменные x и y.</p>
<p>Для обращения к переменным используется указатель <span class="b">this</span>. Причем после this ставится не точка, а стрелка <span class="b">-&gt;</span>.</p>
<p>В большинстве случаев для обращения к членам класса вряд ли поднадобится ключевое слово this. Но оно может быть необходимо, если параметры функции или переменные, которые 
...
<p>Здесь метод move с помощью указателя this возвращает ссылку на объект текущего класса, осуществляя условное перемещение точки. Таким образом, мы можем по цепочке для одного и того же объекта вызывать метод move:</p>
<pre class="brush:cpp;">p1.move(10, 5).move(10);</pre>
<p>Здесь также важно отметить возвращение не просто объекта Point, а ссылки на этот объект. Так, в данном случае выге определенная строка фактически будет аналогично следующему коду:</p>
<pre class="brush:cpp;">
p1.move(10, 5);
p1.move(10, 10);
</pre>
...
<p>То вызов <code>p1.move(10, 5).move(10)</code> был бы фактически эквивалентен следующему коду:</p>
<pre class="brush:cpp;">
