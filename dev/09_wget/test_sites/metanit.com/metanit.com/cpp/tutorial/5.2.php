...
<html >
<head>
<title>C++ | Конструкторы и инициализация объектов</title>
<meta charset="utf-8" />
<meta name="description" content="Конструкторы классов и инициализация объектов в языке програмирования C++, списки инициализаторов">
...
<h2>Конструкторы и инициализация объектов</h2><div class="date">Последнее обновление: 08.10.2017</div>
...
<p>И мы можем установить значения для переменных класса Person, можем получить их значения во внешние переменные. Однако если 
мы попробуем получить значения переменных name и age до их установки, то результаты будут неопределенными:</p>
<pre class="brush:cpp;">
Person person;
cout &lt;&lt; "Name: " &lt;&lt; person.name &lt;&lt; "\tAge: " &lt;&lt; person.age &lt;&lt; endl;
</pre>
<p>Чтобы избежать подобной ситуации применяются специальные функции инициализации или конструкторы. Они позволяют инициализировать объект класса. 
Так, изменим код программы следующим образом:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;string&gt;
using std::string;
...
<p>По сути конструктор представляет функцию, которая может принимать параметры и которая должна называться по имени класса. 
В данном случае конструктор принимает два параметра и передает их значения полям name и age.</p>
<p>Если в классе определены конструкторы, то при создании объекта этого класса необходимо вызвать один из его конструкторов.</p>
<p>Вызов конструктора получает значения для параметров и возвращает объект класса:</p>
<pre class="brush:cpp;">Person person = Person("Tom", 22);</pre>
<p>После этого вызова у объекта person для поля name будет определено значение "Tom", а для поля age - значение 22. Вполедствии мы также сможем 
обращаться к этим полям и переустанавливать их значения.</p>
<p>Тажке можно использовать сокращенную форму инициализации:</p>
<pre class="brush:cpp;">Person person("Tom", 22);</pre>
...
<p>В классе Person определено три конструктора, и в функции все эти конструкторы используются для создания объектов:</p>
<div class="console">
<pre class="consoletext">
Name: Tom	Age: 22
Name: Sam	Age: 18
...
<p>Хотя пример выше прекрасно работает, однако мы можем заметить, что все три конструктора выполняют фактически одни и те же действия - устанавливают значения переменных name и age. И в C++ можем сократить их определения, 
вызова из одного конструктора другой и тем самым уменьшить объем кода:</p>
<pre class="brush:cpp;">
class Person
{
public:
...
<p>Запись <code>Person(string n): Person(n, 18)</code> представляет вызов конструктора, которому передается значение параметра n и число 18. То есть второй конструктор делегирует действия по инициализации переменных первому 
...
<pre class="brush:cpp;">Person bob = Person();</pre>
<p>будет использовать третий конструктор, который в свою очередь вызывает второй конструктор, а тот обращается к первому конструктору.</p>
<h3>Инициализация констант и ссылок</h3>
<p>В теле конструктора мы можем передать значения переменным класса. Однако константы и ссылки требуют особого отношения. Например, вначале определим следующий класс:</p>
<pre class="brush:cpp;">
class Person
{
public:
...
<p>Этот класс не будет компилироваться, так как здесь есть две ошибки - отсутствие инициализации константы name и ссылки ageRef. Хотяя их значения устанавливаются в конструкторе, 
но к моменту, когда код инструкции из тела конструктора начнут выполняться, константы и ссылки уже должны быть инициализированы. И для этого необходимо использовать списки инициализации:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;string&gt;
using std::string;
...
<p>Списки инициализации представляют перечисления инициализаторов для каждой из переменных и констант через двоеточие после списка параметров конструктора:</p>
<pre class="brush:cpp;">
Person(string n, int a) : name(n), age(a), ageRef(age)
{
}
