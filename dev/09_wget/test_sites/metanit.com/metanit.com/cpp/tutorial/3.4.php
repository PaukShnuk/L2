...
<html >
<head>
<title>C++ | Константные параметры</title>
<meta charset="utf-8" />
<meta name="description" content="Константные параметры в функции в языке программирования C++, передача в функцию констант, константные ссылки">
...
<h2>Константные параметры</h2><div class="date">Последнее обновление: 22.09.2017</div>
...
<p>Несмотря на то, что при вызове функции ей передаются константы, но так как сами параметры не являются константными, 
то функция может изменять их значения.</p>
<h3>Константные ссылки</h3>
<p>Если функция получает аргументы по ссылке, то чтобы передать в функцию константу, параметры тоже должны представлять ссылку на константу:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>И если в функцию необходимо передать большие объекты, которые не должны изменяться, то определение параметров именно как константных ссылок больше всего подходит для данной задачи.</p>
