...
<html >
<head>
<title>C++ | Перегрузка операторов</title>
<meta charset="utf-8" />
<meta name="description" content="Перегрузка операторов в языке програмирования C++, перегрузка операций сравнения, присвоения, инкремента и декремента">
...
<h2>Перегрузка операторов</h2><div class="date">Последнее обновление: 20.10.2017</div>
...
<p>Перегрузка операторов позволяет определить действия, которые будет выполнять оператор. Перегрузка подразумевает создание 
функции, название которой содержит слово <span class="b">operator</span> и символ перегружаемого оператора. Функция оператора может 
быть определена как член класса, либо вне класса.</p>
<p>Перегрузить можно только те операторы, которые уже определены в C++. Создать новые операторы нельзя.</p>
<p>Если функция оператора определена как отдельная функция и не является членом класса, то количество параметров такой функции совпадает с количеством операндов оператора. 
...
операнд - второму параметру. При этом как минимум один из параметров должен представлять тип класса</p>
...
<p>Здесь функция оператора не является частью класса Counter и определена вне его. Данная функция перегружает оператор сложения для типа Counter. Она является бинарной, 
...
который хранит общее количесто секунд. То есть по сути здесь операция сложения сводится к сложению секунд обоих объектов:</p>
<pre class="brush:cpp;">
Counter operator + (Counter c1, Counter c2)
{
	return Counter(c1.seconds + c2.seconds);
...
<p>При этом необязательно возвращать объект класса. Это может быть и объект встроенного примитивного типа. И также мы можем определять дополнительные перегруженные 
функции операторов:</p>
<pre class="brush:cpp;">
...
<p>Данная версия складывает объект Counter с числом и возвращает также число. Поэтому левый операнд операции должен представлять тип Counter, 
...
<p>Также функции операторов могут быть определены как члены классов. Если функция оператора определена как член класса, то левый 
...
<p>Какие операторы где переопределять? Операторы присвоения, индексирования ([]), вызова (()), доступа к члену класса по указателю (-&gt;) 
...
обычно также определяются в виде функций-членов класса. Все остальные операторы чаще определяются как отдельные функции, а не члены класса.</p>
<h3>Операторы сравнения</h3>
<p>Ряд операторов перегружаются парами. Например, если мы определяем оператор <span class="b">==</span>, то необходимо также определить 
и оператор <span class="b">!=</span>. А при определении оператора <span class="b">&lt;</span> надо также определять функцию для оператора 
<span class="b">&gt;</span>. Например, перегрузим данные операторы:</p>
<pre class="brush:cpp;">
...
<p>Особую сложность может представлять переопределение операций инкремента и декремента, поскольку нам надо определить и префиксную, 
и постфиксную форму для этих операторов. Определим подобные операторы для типа Counter:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Префиксные операторы должны возвращать ссылку на текущий объект, который можно получить с помощью указателя this:</p>
<pre class="brush:cpp;">
Counter& operator++ ()
{
	seconds += 5;
...
<p>В самой функции можно определить некоторую логику по инкременту значения. В данном случае количество секунд увеличивается на 5.</p>
<p>Постфиксные операторы должны возвращать значение объекта до инкремента, то есть предыдущее состояние объекта. Чтобы постфиксная форма 
отличалась от префиксной постфиксные версии получают дополнительный параметр типа int, который не используется. Хотя в принципе мы можем его использовать.</p>
<pre class="brush:cpp;">
Counter operator++ (int)
{
	Counter prev = *this;
