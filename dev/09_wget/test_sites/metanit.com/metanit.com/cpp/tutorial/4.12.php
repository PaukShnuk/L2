...
<html >
<head>
<title>C++ | Динамические массивы</title>
<meta charset="utf-8" />
<meta name="description" content="Динамические массивы в языке программирования C++, инициализация массива, выделение и освобождение памяти под массив, операторы new и delete">
...
<h2>Динамические массивы</h2><div class="date">Последнее обновление: 11.11.2017</div>
...
<p>Кроме отдельных динамических объектов в языке C++ мы можем использовать динамические массивы. Для выделения памяти под динамический массив 
также используется оператор <span class="b">new</span>, после которого в квадратных скобках указывается, сколько массив будет содержать объектов:</p>
<pre class="brush:cpp;">int *numbers = new int[4]; // динамический массив из 4 чисел</pre>
<p>Причем в этом случае оператор new также возвращает указатель на объект типа int - первый элемент в созданном массиве.</p>
<p>В данном случае определяется массив из четырех элементов типа int, но каждый из них имеет неопределенное значение. Однако мы также можем инициализировать массив значениями:</p>
<pre class="brush:cpp;">
...
<p>В последнем случае при инициализации массива конкретными значениями следует учитывать, что если значений в фигурных скобках 
...
для которых не предоставлены значения, иницилизируются значением по умолчанию.</p>
<p>После создания динамического массива мы сможем с ним работать по полученному указателю, получать и изменять его элементы:</p>
<pre class="brush:cpp;">
...
<p>Для удаления динамического массива и освобождения его памяти применяется специальная форма оператора <span class="b">delete</span>:</p>
<pre class="brush:cpp;">
...
<div class="nav"><p><a href="4.11.php">Назад</a><a href="./">Содержание</a><a href="5.1.php">Вперед</a></p></div></div>
<div class="socBlock">
