...
<html >
<head>
<title>C++ | Оператор return и возвращение результата</title>
<meta charset="utf-8" />
<meta name="description" content="Возвращение результата из функции в языке программирования C++, оператор return и его формы использования">
...
<h2>Оператор return и возвращение результата</h2><div class="date">Последнее обновление: 22.09.2017</div>
...
<p>Для возвращения результата из функции применяется оператор <span class="b">return</span>. Этот оператор имеет две формы:</p>
<pre class="brush:cpp;">
return;
return выражение;
</pre>
<p>Первая форма используется, если в качестве возвращаемого типа функции применяется тип <span class="b">void</span>. Например:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>В данном случае функция factorial вычисляет факториал переданного числа. Однако если число меньше 1, то функция выводит соответствующее сообщение, и 
с помощью оператора return осуществляется выход из функции.</p>
<p>Вторая форма оператора return применяется для возвращения результата из функции. Если функция имеет в качестве возвращаемого типа любой тип, отличный от void, то 
...
должно соответствовать возвращаемому типу функции, либо допускать неявное преобразование в этот тип.</p>
<p>Например, возвратим из функции факториал числа:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Так как функция factorial возвращает значение, то ее результат можно присвоить какой-нибудь переменной или константе:</p>
<pre class="brush:cpp;">int result = factorial(n);</pre>
<h3>Возвращение ссылки</h3>
<p>Не следует возвращать ссылку на локальный объект, который создается внутри функции. Поскольку все создаваемые в функции объекты удаляются после ее завершения, 
а их память очищается, то возвращаемая ссыла будет указывать на несуществующий объект, как в следующем случае:</p>
<pre class="brush:cpp;">
int &factorial(int n)
{
	int result = 1;
