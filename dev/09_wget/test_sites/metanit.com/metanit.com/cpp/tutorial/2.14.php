...
<html >
<head>
<title>C++ | Ссылки</title>
<meta charset="utf-8" />
<meta name="description" content="Ссылки (reference) в языке программирования C++, установка ссылок и ссылки на константы, особенности определения">
...
<h2>Ссылки</h2><div class="date">Последнее обновление: 17.09.2017</div>
...
<p>Ссылка (reference) представляет способ манипулировать каким-либо объектом. Фактически ссылка - это альтернативное имя для объекта. 
Для определения ссылки применяется знак амперсанда &:</p>
<pre class="brush:cpp;">
int number = 5;
int &refNumber = number;
</pre>
<p>В данном случае определена ссылка refNumber, которая ссылается на объект number. При этом в определении ссылки используется тот же тип, который представляет объект, на который ссылка ссылается, то есть 
в данном случае int.</p>
<p>При этом нельзя просто определить ссылку:</p>
<pre class="brush:cpp;">
int &refNumber;
</pre>
<p>Она обязательно должна указывать на какой-нибудь объект.</p>
<p>Также нельзя присвоить ссылке литеральное значение, например, число:</p>
<pre class="brush:cpp;">
int &refNumber = 10;
</pre>
<p>После установления ссылки мы можем через нее манипулировать самим объектом, на который она ссылается:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Изменения по ссылке неизбежно скажутся и на том объекте, на который ссылается ссылка.</p>
<p>Можно определять не только ссылки на переменные, но и ссылки на константы. Но при этом ссылка сама должна быть константной:</p>
<pre class="brush:cpp;">
const int number = 5;
...
<p>Инициализировать неконстантную ссылку константным объектом мы не можем:</p>
<pre class="brush:cpp;">
const int number = 5;
int &refNumber = number; // ошибка
</pre>
<p>Также константная ссылка может указывать и на обычную переменную, только значение по такой ссылке мы не сможем изменить:</p>
<pre class="brush:cpp;">
int number = 5;
const int &refNumber = number;
std::cout &lt;&lt; refNumber &lt;&lt; std::endl; // 5
...
<p>В данном случае несмотря на то, что мы не можем напрямую изменить значение по константной ссылке, тем не менее мы можем изменить сам объект, что 
приведет естественно к изменению константной ссылки.</p>
