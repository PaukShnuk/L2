...
<html >
<head>
<title>C++ | Константы</title>
<meta charset="utf-8" />
<meta name="description" content="Константы в языке программирования C++, инициализация констант">
...
<h2>Константы</h2><div class="date">Последнее обновление: 12.09.2017</div>
...
<p>Отличительной особенностью переменных является то, что мы можем многократно в течение работы программы изменять их значение:</p>
<pre class="brush:cpp;">
int x = 7;
x = 9;
x = 5;
</pre>
<p>Но кроме переменных в языке программирования C++ можно определять константы. Их значение устанавливается один раз и впоследствии мы его не 
...
слово <span class="b">const</span>. Например:</p>
<pre class="brush:cpp;">
const int x = 22;
std::cout &lt;&lt; x;
</pre>
<p>Если же мы захотим после определения константы присвоить ей некоторое значение, то компилятор не сможет скомпилировать программу и выведет ошибку:</p>
<pre class="brush:cpp;">
const int x = 22;
x = 78;
</pre>
<p>То есть такой код не будет работать. И так как нельзя изменить значения константы, то ее всегда необходимо инициализировать, если мы хотим, чтобы она имела некоторое значение.</p>
<p>Если константа не будет инициализирована, то компилятор также выведет ошибку и не сможет скомпилировать программу, как в следующем случае:</p>
<pre class="brush:cpp;">const int x;</pre>
<p>В качестве значения константам можно передавать как обычные литералы, так и динамически вычысляемые значения, например, значения переменных или других констант:</p>
<pre class="brush:cpp;">
int a = 10;
const int b = 7;
const int d = b;
