...
<html >
<head>
<title>C++ | Шаблоны функций</title>
<meta charset="utf-8" />
<meta name="description" content="Определение и использование шаблонов функций в языке програмирования C++, function template, typename">
...
<h2>Шаблоны функций</h2><div class="date">Последнее обновление: 23.11.2017</div>
...
<p>Кроме шаблонов классов мы можем применять шаблоны функций (function template). Например, рассмотрим следующий пример:<p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Данный пример отлично работает, производит вычисления, как и должен. Однако в данном случае мы сталкиваемся с тем, что функция add фактически дублируется. 
Обе ее версии фактически выполняют одно и то же действие, единственно что отличается тип параметров и возвращаемого значения: в одном случае это тип int, а в другом - тип double.</p>
<p>Теперь применим шаблоны функций. Шаблоны функций представляют некоторый образец, по которому можно создать конкретную 
функцию, специфическую для определенного типа:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
...
<p>Определение шаблона функции, как и шаблона класса, начинается с ключевого слова <span class="b">template</span>, после которого указываются угловые скобки. 
...
<p>Но в данном случае важно, чтоб тип, который будет применяться вместо параметра T, поддерживал операцию сложения, которая возвращала бы объект этого же типа. Если вдруг используемый тип не будет 
применять операцию сложения, то на этапе компиляции мы столкнемся с ошибкой.</p>
<p>И при вызове функции add в нее можно передавать объекты и типа int, и типа double, и любого другого типа. При вызове функции компилятор 
на основании типа аргументов выведет конкретный тип, связанный с параметром шаблона T.</p>
...
<p>В данном случае при вызове <code>transact("id1234", "id5678", 2804, 5000);</code> вместо параметра T будет подставляться тип 
string, а вместо параметра K - тип int.</p>
