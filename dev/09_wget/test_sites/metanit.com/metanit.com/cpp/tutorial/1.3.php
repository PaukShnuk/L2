...
<html >
<head>
<title>С++ | Первая программа на Linux. Компилятор g++</title>
<meta charset="utf-8" />
<meta name="description" content="Компилятор g++. Первая программа на языке С++ на Linux Ubuntu, установка компилятора">
...
<h2>Первая программа на Linux. Компилятор g++</h2><div class="date">Последнее обновление: 28.08.2017</div>
...
<p>В прошлой теме было рассмотрено создание первой программы на Windows. Теперь рассмотрим создание первой программы на Linux, в 
...
этот компилятор.</p>
<p>Как правило, многие дистрибутивы Linux, в том числе и Ubuntu, уже по умолчанию содержат установленный компилятор g++, который мы сразу же можем использовать. Но даже если вдруг он не установлен, то его можно доустановить в терминале через команду:</p>
<pre class="brush:cpp;">sudo apt-get install g++</pre>
<p>В остальном, если мы будем использовать для компиляции компилятор g++, все будет аналогично созданию программы на Windows.</p>
<p>Определим в файловой системе каталог для исходных файлов с кодом на С++ и создадим в нем новый файл <span class="b">hello.cpp</span> со следующим кодом:</p>
<pre class="brush:cpp;">
...
<p>И это тот же код, что был в случае с Windows, потому что программы на С++ на уровне исходного кода в большей степени обладают переносимостью.</p>
...
<img src="pics/ubuntu1.png" alt="Первая программа на C++ на Linux" />
...
<div class="console"><pre class="consoletext">g++ hello.cpp -o hello</pre></div>
...
<p>И на консоль будет выведена строка "Hello World!".</p>
<img src="pics/ubuntu2.png" alt="Компиляция программы в G++ в Ubuntu" />
...
<div class="nav"><p><a href="./1.2.php">Назад</a><a href="./">Содержание</a><a href="./1.6.php">Вперед</a></p></div></div>
<div class="socBlock">
