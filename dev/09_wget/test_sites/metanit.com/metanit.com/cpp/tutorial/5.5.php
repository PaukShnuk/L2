...
<html >
<head>
<title>C++ | Дружественные функции и классы</title>
<meta charset="utf-8" />
<meta name="description" content="Дружественные функции и классы в языке програмирования C++, ключевое слово friend">
...
<h2>Дружественные функции и классы</h2><div class="date">Последнее обновление: 05.10.2017</div>
...
<p>Дружественные функции - это функции, которые не являются членами класса, однако имеют доступ к его закрытым членам - переменным и функциям, которые имеют спецификатор 
private.</p>
<p>Для определения дружественных функций используется ключевое слово <span class="b">friend</span>. Например, определим следующую программу:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;string&gt;  
...
<p>Здесь определен класс Auto, который представляет автомобиль. У этого класса определены приватные закрытые переменные name (название автомобиля) и price 
...
Обе этих функции принимают в качестве параметра ссылку на объект Auto.</p>
<p>Когда мы объявляем дружественные функции, то фактически мы говорим компилятору, что это друзья класса и они имеют доступ ко всем членам этого класса, в том числе закрытым.</p>
<p>При этом для дружественных функций не важно, определяются они под спецификатором public или private. Для них это не имеет значения.</p>
<p>Определение этих функций производится вне класса. И поскольку эти функции являются дружественными, то внутри этих функций мы можем через переданную ссылку Auto 
обратиться ко всем его закрытым переменным.</p>
...
<h3>Определение дружественных функций в классе</h3>
<p>Дружественные функции могут определяться в другом классе. Например, определим класс Person, который использует объект Auto:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;string&gt; 
...
<p>Вначале определен класс Person, который представляет человека. Однако поскольку класс Person использует класс Auto, то перед классом Person идет объявление класса Auto.</p>
...
<p>Причем поскольку данные функции будут определены в классе Person, то названия этих функций предваряется префиксом "Person::".</p>
<p>И поскольку в этих функциях предполагается использовать объект Auto, то ко времени определения этих функций все члены объекта Auto должны быть известны, 
...
функциях к закрытым членам класса Auto.</p>
<p>Консольный вывод программы:</p>
<div class="console">
<pre class="consoletext">
Tom drives Tesla
...
<h3>Дружественные классы</h3>
<p>В случае выше класс Person использует только две функции из класса Auto. Но допустим впоследствии возникла необходимость добавить в класс Auto еще ряд 
...
И в этом случае целесообразно определять не отдельные дружественные функции, а определить дружественным весь класс Person:</p>
<pre class="brush:cpp;">
#include &lt;iostream&gt;
#include &lt;string&gt; 
...
<p>Единственно, что в данном случае изменилось по сравнению с предыдущим примером это то, что в классе Auto определение дружественных функций было заменено 
определением дружественного класса:</p>
<pre class="brush:cpp;">friend class Person;</pre>
<p>То есть тем самым мы опять же говорим, что класс Person - это друг класса Auto, поэтому объекты Person могут обращаться к приватным переменным класса Auto. 
После этого в классе Person можно обращаться к закрытым членам класса Auto из любых функций.</p>
