...
<html >
<head>
<title>C++ | Арифметика указателей</title>
<meta charset="utf-8" />
<meta name="description" content="Арифметика указателей в языке программирования C++, инкемент и декремент указателей, арифметические операции с указателями">
...
<h2>Арифметика указателей</h2><div class="date">Последнее обновление: 23.09.2017</div>
...
<p>Указатели могут участвовать в арифметических операциях (сложение, вычитание, инкремент, декремент). Однако сами операции производятся 
немного иначе, чем с числами. И многое здесь зависит от типа указателя.</p>
<p>К указателю можно прибавлять целое число, и также можно вычитать из указателя целое число. Кроме того, можно вычитать из одного указателя другой указатель.</p>
...
<p>Здесь видно, что после инкремента значение указателя увеличилось на 4: с 0x60fe98 до 0x60fe9c. А после декремента, то есть уменьшения на 
единицу, указатель получил предыдущий адрес в памяти.</p>
...
<p>После изменения адреса мы можем получить значение, которое находится по новому адресу, однако это значение может быть неопределенным, как показано в случае выше.</p>
...
<pre class="brush:cpp;">pd = pd + 2;</pre>
...
<pre class="brush:cpp;">pc = pc - 3;</pre>
...
чем адрес из второго указателя (0x60fe8c + 4 = 0x60fe90). Так как размер <span class="b">одного</span> объекта int равен 4 байтам, то 
расстояние между указателями будет равно (0x60fe90 - 0x60fe8c)/4 = 1.</p>
