...
<html >
<head>
<title>Создание бандлов. Библиотеки из CDN</title>
<meta charset="utf-8" />
<meta name="description" content="Пример создания своих бандлов в ASP.ET MVC 4, их подключение в приложении, подключение библиотек из сетей CDN">
...
<h2>Создание бандлов. Библиотеки из CDN</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Создадим свой бандл. Например, я определил в папке со скриптами подкаталог MyScripts для каких-то своих скриптов, которые я хочу подключать в 
своем бандле:</p>
<img src="mvcpics/15.4.png" />
<p>В файле <i>BundleConfig.cs</i> я добавляю свой бандл с моими файлами:</p>
<pre class="brush:c#;">
using System.Web;
...
<p>Выражение <code>"~/Scripts/MyScripts/*.js"</code> подключит в бандл все файлы js, которые есть в каталоге Scripts/MyScripts. 
Далее переходим к разметке и подключаем там новый бандл:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>И после этого бандл будет подключаться в приложение. При подключении бандлов не стоит забывать об очередности: если у вас есть бандлы, 
файлы которых зависят от других файлов в других бандлов, например, от jquery, то, естественно, зависимые бандлы надо подключать после основных.</p>
<h3>Подключение библиотек из сетей CDN</h3>
<p>Обычным делом на сегодняшний день является подключение различных популярных библиотек javascript из сетей CDN. Если мы хотим подключать, например, 
библиотеку jquery не со своего приложения, а с какой-нибудь сети CDN, то мы должны включить поддержку CDN в бандлах в файле <i>BundleConfig.cs</i>:</p>
<pre class="brush:c#;">
using System.Web;
using System.Web.Optimization;
...
<p>Далее подключаем бандл обычным способом. Кроме того, мы можем задать возможность подключения библиотеки из своего приложения, если 
библиотеку не удалось загрузить с этой сети:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<div class="nav"><p><a href="15.1.php">Назад</a><a href="./">Содержание</a><a href="16.1.php">Вперед</a></p></div></div>
<div class="socBlock">
