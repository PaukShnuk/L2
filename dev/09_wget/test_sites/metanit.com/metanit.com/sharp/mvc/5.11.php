...
<html >
<head>
<title>Работа с моделями со связью многие-ко-многим</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с моделями со связью многие-ко-многим в ASP.NET MVC">
...
<h2>Работа с моделями со связью многие-ко-многим</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Простой вывод связанных данных со связью "многие-ко-многим" не представляет особого труда. Однако не совсем понятно, как делать создание и 
...
студентов.</p>
<p>В итоге форма редактирования у нас будет выглядеть примерно так:</p>
<img src="mvcpics/5.22.png" />
<p>Итак, добавим в контроллер HomeController следующее действие Edit:</p>
<pre class="brush:c#;">
...
<p>Действие Edit представляет два метода - для запроса get и для запроса post. Метод, обрабатывающий запрос get, стандартный - он передает 
в представление редактируемую модель, а также список всех курсов через <code>ViewBag.Courses = db.Courses.ToList();</code>, чтобы мы могли 
затем эти курсы вывести в представлении.</p>
<p>Метод post принимает полученные данные, только кроме модели Student сюда также передаются все выбранные курсы в виде массива id курсов. В 
этот массив и будут помещаться все значения всех отмеченных на форме флажков. В самом методе мы устанавливаем новые значения свойств модели.</p>
<p>Затем нам надо установить в коллекции Courses у студента все отмеченные курсы. Для этого сначала удаляем у студента все курсы, затем проходим по всем курсам из базы данных, и если они были 
отмечены на форме и отсутствуют в списке, то добавляем их. Неотмеченные курсы, если они раньше имелись у студента, таким образом, будут удалены. В итоге информация в базе данных будет соотвествующим образом обновлена.</p>
<p>И напоследок добавим само представление <i>Edit.cshtml</i>:</p>
<pre class="brush:xml;">
...
<p>Ну а сохранение модели будет во многом идентично редактированию.</p>
...
<div class="nav"><p><a href="5.10.php">Назад</a><a href="./">Содержание</a><a href="5.7.php">Вперед</a></p></div></div>
<div class="socBlock">
