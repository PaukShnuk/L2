...
<html >
<head>
<title>Шаблоны формирования</title>
<meta charset="utf-8" />
<meta name="description" content="Шаблоны формирования - упрощенный способ создания разметки представлений для добавления, изменения, удаления и просмотра записей из БД">
...
<p>Поскольку зачастую разработчики вынуждены создавать представления для одних и тех же действий: добавления, изменения, удаления и просмотра записей из 
БД, то команда разработчиков MVC внедрила такую полезную функцию, как <span class="bb">шаблоны формирования (scaffolding templates)</span>. Эти шаблоны позволяют 
по заданной модели и контексту данных сформировать всю необходимую разметку для представлений и контроллера, с помощью которых можно управлять 
записями в БД.</p>
<p>Тут надо сразу оговориться, что для корректного применения шаблонов формирования надо, чтобы использовалась одна из связок MVC 4 + EntityFramework 5 или 
MVC 5 + Entity Framework 6.</p>
<p>Итак, применим шаблон формирования. Добавим новый контроллер. В окне создания нового контроллера установим следующие параметры:</p>
<img src="mvcpics/5.8.png" />
<p>В секции <span class="bb">Template</span> нам предлагается выбрать шаблон формирования кода. По умолчанию нам доступны следующие шаблоны:</p>
<ul>
<li><p><span class="bb">Empty MVC Controller</span>. Этот шаблон добавляет в папку Controllers класс, производный от класса Controller с указанным именем. 
Единственное действие, которое выполняет этот контроллер - это действие Index, которое по умолчанию не заключает никакого функционала. 
Данный шаблон не создает представлений</p></li>
<li><p><span class="bb">MVC Controller with Empty Read/Write Actions</span>. Этот шаблон добавляет в проект контроллер, который содержит методы Index, 
Details, Create, Edit и Delete. Однако эти методы фактически никакой полезной работы не выполняют, и необходимо самим создать для них 
код и представления для каждого из этих методов.</p></li>
<li><p><span class="bb">MVC Controller with Read/Write Actions and Views, Using Entity Framework</span>. Этот шаблон создает контроллер с методами  Index, Details, Create, Edit и Delete, 
а также все необходимые представления для этих действий и добавляет код для извлечения информации из базы данных. Что очень полезно, так как 
нам не надо писать соответствующий код, как в предыдущем разделе.</p></li>
<li><p><span class="bb">Empty API Controller</span>. Этот шаблон пустой контроллер, производный от класса ApiController. Класс ApiController был введен в MVC 4 
и используется в проектах типа Web API - своего рода веб-служб.</p></li>
<li><p><span class="bb">API Controller with Empty Read/Write Actions</span>. Этот шаблон добавляет в проект контроллер, производный от класса ApiController, который содержит методы Index, 
Details, Create, Edit и Delete.</p></li>
<li><p><span class="bb">API Controller with Read/Write Actions and Views, Using Entity Framework</span></p><p>Этот шаблон, также как и в случае с обычным контроллером, 
создает контроллер, только в данном случае производный от класса ApiController, который содержит методы  Index, Details, Create, Edit и Delete, 
а также все необходимые представления для этих действий.</p></li>
</ul>
<p>В нашем случае мы выбираем пункт <span class="bb">MVC Controller with Read/Write Actions and Views, Using Entity Framework</span>.</p>
<p>Далее нам будет предложено выбрать в поле <span class="bb">Model class</span> модель, на основе которой будут создаваться представления. Выберем созданную ранее 
модель Book (либо какую-то другую имеющуюся модель).</p>
<p>В завершении в поле <span class="bb">Data context class</span> надо выбрать класс контекста данных, с помощью которого мы получаем все данные модели из БД. 
В нашем случае - это ранее созданный класс BookContext.</p>
<p>Установив все необходимые параметры, нажмем кнопку Add, и в проект будет добавлен новый контроллер. Он будет выглядеть примерно следующим образом:</p>
<pre class="brush:c#;">
using System;
...
<p>А в папке <span class="ii">Views/Book</span> мы увидим все необходимые представления со всем необходимым кодом, который теперь нам не надо набирать вручную. 
мы можем запустить проект и перейти в адресной строке браузера к нашему контроллеру, чтобы убедиться, что все работает как надо:</p>
<img src="mvcpics/5.9.png" />
<p>Благодаря шаблонам формирования мы можем не думать о создании кода для стандартных операций. Нам остается после 
генерации кода лишь изменить автоматически сгенерированные названия на свои (например, название страницы, автоматические генерируемых ссылок и др.)</p>
...
<div class="nav"><p><a href="5.9.php">Назад</a><a href="./">Содержание</a><a href="5.6.php">Вперед</a></p></div></div>
<div class="socBlock">
