...
<html >
<head>
<title>Ajax-Формы в ASP.NET MVC 4</title>
<meta charset="utf-8" />
...
<h2>Ajax-Формы</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Итак, после настройки и подключения всех необходимых скриптов мы можем приступить непосредственно к работе с Ajax. Допустим, у нас 
есть класс <code>Book</code>, содержащий данные о книге, а в БД у нас может находиться несколько книг одного автора. И нам надо реализовать поиск 
всех книг определенного автора.</p>
<p>Казалось бы, зачем в данном случае Ajax, если мы можем, например, в форму вводить имя автора и отправлять на сервер, а сервер в качестве ответа 
возвратит нам страницу с нужным результатом. Но, как выше уже говорилось, AJAX поможет нам избежать перезагрузки всей страницы и выполнить 
загрузку данных в <strong>асинхронном режиме</strong>, что несомненно повышает производительность приложения.</p>
<p>Для начала определим действие контроллера, которое будет отвечать за извлечение из БД нужной информации и передавать извлеченную информацию в 
частичное представление:</p>
<pre class="brush:c#;">
...
<p>Итак, действие получает в качестве параметра имя автора и по нему осуществляет поиск в БД. Теперь добавим к представлениям данного контроллера 
частичное представление <span class="ii">BookSearch.cshtml</span>, поскольку частичные представления довольно удобны для работы с AJAX:</p>
<pre class="brush:xml;">
...
<p>В данном случае представление типизируется для модели <code>IEnumerable&lt;AjaxMvcApplication.Models.Book&gt;</code>, которая и будет 
передаваться в представление. А затем в элемент <code>div</code> будут выводиться результаты поиска в виде списка при условии, конечно, если модель 
не равна <code>null</code>.</p>
<p>Теперь перейдем к самому представлению, которое и будет отображаться пользователю:</p>
<pre class="brush:xml;">
@{
    ViewBag.Title = "Index";
}
...
<p>Хелпер <strong>Ajax.BeginForm</strong> похож на хелпер <code>Html.BeginForm</code> - он также создает элемент <code>form</code>, который используется 
...
<code>BookSearch</code>, которое возвращает частичное представление с данными. Если действие находится не в текущем контроллере, а в другом мы также можем 
указать имя контроллера: <code>Ajax.BeginForm("BookSearch", "Home", new AjaxOptions....</code></p>
<p>Второй параметр более интересный. Он представляет объект <strong>AjaxOptions</strong>, который влияет на отображение результатов. Он принимает ряд параметров, 
из которых мы в данном случае использовали только <code>UpdateTargetId</code>. Этот параметр указывает, что у нас при получении результатов от сервера 
на странице будет обновляться элемент с <code>id="results"</code>, в который помещется html-код выше созданного частичного представления.</p>

<p>Теперь мы можем запустить приложение и осуществить поиск с помощью AJAX:</p>
<img src="mvcpics/10.2.png" />
<img src="mvcpics/10.3.png" />
<p>Обратите внимание на создаваемую разметку для данной формы:</p>
<pre class="brush:xml;">
...
<p>Создается как и в случае с хелпером <code>Html.BeginForm</code> элемент <code>form</code>. Но теперь в специальном атрибуте указывается, 
что это Ajax-форма: <code>data-ajax="true"</code>. Остальные параметры формы являются передачей в html параметров объекта AjaxOptions.</p>
