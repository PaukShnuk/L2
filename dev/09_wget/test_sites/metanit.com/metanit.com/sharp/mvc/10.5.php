...
<html >
<head>
<title>Параметры объекта AjaxOptions</title>
<meta charset="utf-8" />
...
<h2>Параметры объекта AjaxOptions</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Выше мы использовали только один параметр объекта AjaxOptions. Но он имеет еще ряд параметров, которые позволяют настроить отображение результатов 
запроса:</p>
<ul>
	<li><p><span class="bb">Confirm</span> - настраивает сообщение о подтверждении отправки запроса на сервер. Если пользователь не подтвердит, то 
	запрос не будет отправлен</p></li>
	<li><p><span class="bb">HttpMethod</span> - устанавливает метод (Get или Post), с помощью которого выполняется запрос</p></li>
	<li><p><span class="bb">InsertionMode</span> - устанавливает, как полученные результаты будут отображаться на странице. Может принимать одно из трех значений 
	перечисления <code>InsertionMode</code>: <code>InsertAfter</code>, <code>InsertBefore</code> и <code>Replace</code>(по умолчанию).</p></li>
	<li><p><span class="bb">LoadingElementId</span>- устанавливает id элемента html-страницы, который будет отображаться во время запроса. 
	Обычно это какая-нибудь анимация, которая дает знать, что некоторая работа выполняется в фоновом режиме</p></li>
	<li><p><span class="bb">LoadingElementDuration</span> - устанавливает количество миллисекунд, через которое появится элемент, указанный в параметре LoadingElementId</p></li>
	<li><p><span class="bb">OnBegin</span> - задает обратный вызов перед отправкой запроса. Соотносится с событием <code>beforeSend</code> библиотеки jQuery</p></li>
	<li><p><span class="bb">OnComplete</span> - задает обратный вызов, который вызывается после выполнения запроса (как удачного, так и неудачного). Соотносится с событием <code>complete</code> библиотеки jQuery</p></li>
	<li><p><span class="bb">OnFailure</span> - задает обратный вызов, который вызывается после неудачного выполнения запроса. Соотносится с событием <code>error</code> библиотеки jQuery</p></li>
	<li><p><span class="bb">OnSuccess</span> - задает обратный вызов, который вызывается после удачного выполнения запроса. Соотносится с событием <code>success</code> библиотеки jQuery</p></li>
	<li><p><strong>UpdateTargetId</strong> - указывает на id элемента, в котором будут выводиться результаты запроса</p></li>
	<li><p><span class="bb">Url</span> - устанавливает адрес Url сервера, на который отправляется запрос. Установив данное свойство, можно не использовать название 
	имя контроллера и его действие в качестве параметров <code>Ajax.BeginForm</code></p></li>
</ul>
<h3>LoadingElementId и LoadingElementDuration</h3>
<p>Эти параметры используются для создания некой анимации или визуализации, которая извещает пользователей о процессе запроса. Если запрос идет долго, 
то пользователь может долго не получать результаты запроса и может подумать, что страница зависла. Чтобы пользователь видел, что процесс выполняется, 
и нужно настраивать данные параметры.</p>
<pre class="brush:xml;">
...
<img src="mvcpics/10.4.png" />
<p>Данный элемент отображается на странице только во время запроса, поэтому для него устанавливается атрибут <code>display:none</code>. 
И после выполнения запроса данный элемент снова скрывается.</p>
<p>Очень популярен способ индикации процесса с помощью картинки:</p>
<pre class="brush:xml;">
@using (Ajax.BeginForm("BookSearch", new AjaxOptions { UpdateTargetId = "results", LoadingElementId="loading", 
                                                        LoadingElementDuration=1000}))
{
...
<h3>Confirm</h3>
<p>Этот параметр настраивает сообщение о подтверждении отправки запроса на сервер. Если пользователь не подтвердит, то запрос не будет 
отправлен.</p>
<pre class="brush:xml;">
@using (Ajax.BeginForm("BookSearch", new AjaxOptions { UpdateTargetId = "results", Confirm="Выполнить AJAX-запрос?"}))
{
    &lt;input type="text" name="name" /&gt;
    &lt;input type="submit" value="Поиск" /&gt;
}
...
<img src="mvcpics/10.5.png" />
<h3>InsertionMode</h3>
<p>Этот параметр указывает на способ вставки результатов запроса на страницу. Он принимает одно из трех значений одноименного перечисления 
<code>InsertionMode</code>: <code>InsertAfter</code> (после остальных результатов), <code>InsertBefore</code> (перед остальными результатами) 
и <code>Replace</code> (полность замещает результаты предыдущего AJAX-запроса - используется по умолчанию).</p>
<p>Так, при установке значения параметра <code>InsertionMode=InsertionMode.InsertBefore</code> мы будем наблюдать следующую картину:</p>
<img src="mvcpics/10.6.png" />
<h3>Обратные вызовы</h3>
<p>Параметры <code>OnBegin</code>, <code>OnComplete</code>, <code>OnFailure</code> и <code>OnSuccess</code> помогают задать обратные вызовы, 
которые будут выполняться перед запросом, либо в случае удачного или неудачного выполнения запроса.</p>
<p>Чтобы их использовать, определим соответствующие обработчики javascipt в представлении и укажем их в параметрах объекта <code>AjaxOptions</code>:</p>
<pre class="brush:xml;">
...
@using (Ajax.BeginForm("BookSearch", new AjaxOptions { UpdateTargetId = "results", OnBegin="OnBegin", 
                                            OnFailure="OnFailure", OnSuccess="OnSuccess", OnComplete="OnComplete"}))
{
    &lt;input type="text" name="name" /&gt;
    &lt;input type="submit" value="Поиск" /&gt;
}
...
<h5>Обратный вызов OnBegin</h5>
<img src="mvcpics/10.7.png" />
<h5>Обратный вызов OnSuccess</h5>
<img src="mvcpics/10.8.png" />
<h5>Обратный вызов OnComplete</h5>
<img src="mvcpics/10.9.png" />
<h5>Обратный вызов OnFailure</h5>
<img src="mvcpics/10.10.png" />
