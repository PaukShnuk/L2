...
<html >
<head>
<title>Метаданные и валидация модели</title>
<meta charset="utf-8" />
<meta name="description" content="Использование атрибутов и аннотаций для отображение свойств модели">
...
<h1>Метаданные и валидация модели</h1><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>При работе с моделью в MVC большее значение играют метаданные. 
Они позволяют указать некоторую дополнительную информацию об объекте, например, 
о том, как отображать его свойства в представлении, или о том, как осуществлять валидацию вводимых данных.</p>
<h2>Аннотации данных для отображения свойств</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p>
<p>Аннотации данных представляют собой атрибуты, которые вы можете найти в пространстве имен <code>System.ComponentModel.DataAnnotations</code> 
(хотя несколько атрибутов определено в других пространствах). </p>
<h3>Атрибут Display</h3>
<p>Итак, предположим у нас имеется некоторая модель Book:</p>
<pre class="brush:c#;">
public class Book
{
...
<p>И мы пытаемся получить из базы данных один объект этой модели:</p>
<pre class="brush:c#;">
    public class HomeController : Controller
    {
        ...
        {
            var firstBook = db.Books.ToList&lt;Book&gt;().First();
            return View(firstBook);
        }
    }
</pre>
<p>Используя соответствующий хелпер, мы выводим эту модель представлении: <code>@Html.DisplayForModel()</code>:</p>
 <pre class="brush:xml;">
...
<p>Обычный код, но тут мы сталкиваемся с проблемой:</p>
<img src="mvcpics/7.1.png" />
<p>Как видно, заголовки у свойств точно такие же, как и названия свойств, что не очень удобно. Ведь мы могли назвать свойства как угодно. 
К тому же мы не хотим, чтобы названия на русскоязычном сайте были на английском языке.<p> 
<p>Конечно, в этом случае мы могли бы использовать <code>@Html.LabelFor</code> 
...
вывести всю информацию о модели. К тому же может потребоваться в нескольких представлениях выводить информацию о модели. Поэтому данный подход не оптимален.</p>
<p>И тут нам на помощь приходят аннотации данных, а именно атрибут <span class="bb">Display</span>. Нам достаточно поправить модель, указав у нее параметры отображения:</p>
<pre class="brush:c#;">
...
<p>Свойство <code>Name</code> атрибута Display содержит строку, которая будет отображаться вместо имени свойства. 
Больше нам ничего не надо менять. Запустим и увидим новые названия:</p>
<img src="mvcpics/7.2.png" />
<h3>Атрибут HiddenInput</h3>
<p>В предыдущем примере у нас осталась одна проблемка - это поле Id. Иногда, конечно, может потребоваться вывод поля Id. Но, например, 
если бы мы выводили модель в режиме редактирования с помощью хелпера <code>@Html.EditorForModel()</code>, то данное поле было бы доступно для редактирования, 
что не очень хорошо, особенно когда идентификаторы не должны меняться. Чтобы скрыть это поле мы можем применить атрибут <span class="bb">HiddenInput</span>:</p>
<pre class="brush:c#;">
using System.ComponentModel.DataAnnotations;
...
<p>Свойство <code>DisplayValue=false</code> указывает, что надо скрыть данное поле. В итоге вы его не увидите:</p>
<img src="mvcpics/7.3.png" />
<p>При использовании хелперов редактирования (<code>Html.EditorFor/Html.EditorForModel</code>) для данного свойства будет сгенерировано 
скрытое поле: <code>&lt;input type="hidden" id="Id" name="Id" value="1" /&gt;</code></p>
<h3>Атрибут ScaffoldColumn</h3>
<p>При редактировании модели атрибут HiddenInput полностью не скрывает поля, так как мы можем посмотреть исходный код страницы и найти 
соответствующие поля. Чтобы полностью скрыть свойство от хелперов, используется атрибут <span class="bb">ScaffoldColumn</span>:</p>
<pre class="brush:c#;">
       [ScaffoldColumn(false)]
        public virtual int Id { get; set; }
</pre>
<p>Теперь хелперы редактирования не увидят данное свойство и не создадут для него даже скрытое поле на странице.</p>
<h3>Атрибут DataType</h3>
<p>Атрибут <span class="bb">DataType</span> позволяет предоставлять среде выполнения информацию об использовании свойства. Например, допустим, у нас есть 
свойство Password:</p>
<pre class="brush:c#;">
[DataType(DataType.Password)]
public string Password { get; set; }
</pre>
<p>Для свойства с атрибутом DataType.Password HTML-хелперы создают элемент ввода, у которого атрибут type имеет значение "password". 
Тогда в браузере вы при вводе данных вы не увидите вводимые символы:</p>
<img src="mvcpics/7.4.png" />
<p>Перечисление DataType может принимать несколько различных значений:</p>
<table class="tab">
	<tr class="tabhead"><td><p>Значение</p></td><td><p>Описание</p></td></tr>
	<tr><td><p><code>Currency</code></p></td><td><p>Отображает текст в виде валюты</p></td></tr>
	<tr><td><p><code>DateTime</code></p></td><td><p>Отображает дату и время</p></td></tr>
	<tr><td><p><code>Date</code></p></td><td><p>Отображает только дату, без времени</p></td></tr>
	<tr><td><p><code>Time</code></p></td><td><p>Отображает только время</p></td></tr>
	<tr><td><p><code>Text</code></p></td><td><p>Отображает однострочный текст</p></td></tr>
	<tr><td><p><code>MultilineText</code></p></td><td><p>Отображает многострочный текст (элемент textarea)</p></td></tr>
	<tr><td><p><code>Password</code></p></td><td><p>Отображает символы с использованием маски</p></td></tr>
	<tr><td><p><code>Url</code></p></td><td><p>Отображает строку URL</p></td></tr>
	<tr><td><p><code>EmailAddress</code></p></td><td><p>Отображает электронный адрес</p></td></tr>
</table>
<h3>Атрибут UIHint</h3>
<p>Данный атрибут указывает, какой будет использоваться шаблон отображения при создании разметки html для данного свойства. Шаблон управляет, 
как свойство будет рендерится на странице.</p>
<p>Имеются следующие встроенные шаблоны:</p>
<ul>
	<li><p><span class="bb">Boolean</span></p>
	...
	<li><p><span class="bb">Collection</span></p>
	...
	<li><p><span class="bb">String</span></p>
	<p>Хелперы редактирования создают однострочное текстовое поле</p>
	</li>
	<li><p><span class="bb">Url</span></p>
...
<div class="nav"><p><a href="6.6.php">Назад</a><a href="./">Содержание</a><a href="7.2.php">Вперед</a></p></div></div>
<div class="socBlock">
