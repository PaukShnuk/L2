...
<html >
<head>
<title>Работа с маршрутами</title>
<meta charset="utf-8" />
<meta name="description" content="Создание маршрутов с помощью метода MapRoute, работа с объектом Route и коллекцией маршрутов RouteCollection">
...
<h2>Работа с маршрутами</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>Создание новых маршрутов</h3>
<P>Создать свой маршрут мы можем, например, как было уже показано, с помощью метода <strong>MapRoute</strong>. Например:</P>
<pre class="brush:c#;">
...
<p>Либо мы можем создать объект Route и потом добавить его в коллекцию маршрутов RouteCollection. Определим два маршрута:</p>
<pre class="brush:c#;">
public class RouteConfig
{
    public static void RegisterRoutes(RouteCollection routes)
...
<p>Итак, здесь определены два маршрута. Первый - маршрут <code>Default</code> сопоставляется с запросами, имеющими три сегмента. Второй - 
<code>newRoute</code> сопоставляется с запросами, имеющими только два сегмента. В итоге вызов <span class="ii">http://localhost:5555/Home/Index/1</span> 
будет соответствовать первому маршруту, так как в нем определено три сегмента. А вызов <span class="ii">http://localhost:5555/Home/Index</span> - второму маршруту. 
Вызов <span class="ii">http://localhost:5555/Home</span> не будет соответствовать ни одному маршруту, так как у нас не определен маршрут, принимающий только один сегмент 
в шаблоне URL.</p>
<p>Правда, ситуация с этими двумя запросами в большей степени является искусственной, так как мы можем совместить их, просто определив параметр id 
как необязательный: <code>routes.MapRoute(name: "Default", url: "{controller}/{action}/{id}", defaults: new { id = RouteParameter.Optional });</code>.</p>
<p>При передаче значений по умолчанию для параметров мы должны учитывать позицию параметра. Движок маршрутизации использует значение по умолчанию 
только тогда, когда все последующие параметры также имеют значения по умолчанию. Так, если мы зададим следующий маршрут: 
<code>routes.MapRoute(name: "Default", url: "{controller}/{action}/{id}", defaults: new { action = "Index" });</code>, 
то запрос, содержащий два сегмента, например, <span class="ii">Home/2</span> не будет сопоставляться с данным маршрутом. Поэтому нам надо в данном случае указать 
значение по умолчанию также и для параметра id.</p>
<h3>Сопоставление запросов с файлами на диске</h3>
<p>Ранее мы обращались к ресурсам сайта через маршруты. Но мы можем адресовать запросы также отдельным файлам сайта. Работа механизма маршрутизации такова, 
что сначала он смотрит, совпадает ли запрос с определенным файлом, хранящимся на сервере, и если такого файла не находит, тогда он начинает 
сопоставлять запрос с определенными маршрутами.</p>
<p>Итак, добавим в папку Content проекта новую html-страницу и назовем ее, например, <span class="ii">Welcome.html</span></p>
<img src="mvcpics/6.1.png" />
<p>Определим какое-нибудь простенькое содержание страницы. Теперь мы можем обращаться к этой странице напрямую через запрос <span class="ii">Content/Welcome.html</span>.</p>
<h4>Использование префиксов в строке запроса</h4>
<p>Сегменты строки запроса необязательно должны нести только значения для параметров, определенных в маршруте. Мы также можем использовать различные 
префиксы в строке запроса и соответствующим образом настроить маршрут для обработки подобных запросов. Например, мы хотим, чтобы запрос содержал 
префикс <code>Ru</code>: <span class="ii">http://localhost:49326/Ru/Home/Index/1</span>. Тогда обрабатывающий этот запрос маршрут может выглядеть следующим образом:</p>
<pre class="brush:c#;">routes.MapRoute(name: "Default", url: "Ru/{controller}/{action}/{id}", defaults: new { id = RouteParameter.Optional }</pre>
<p>Кроме того, мы можем добавлять префиксы не в качестве отдельного сегмента, а к самому сегменту:</p>
<pre class="brush:c#;">routes.MapRoute(name: "Default", url: "Ru{controller}/{action}/{id}", defaults: new { id = RouteParameter.Optional }</pre>
<p>В этом случае строка запроса, соответствующая данному маршруту, может выглядеть так: <span class="ii">http://localhost:49326/RuHome/Index</span></p>
<h3>Порядок определения новых маршрутов</h3>
<p>При добавлении новых маршрутов важно учитывать их порядок. Обычно более специфические маршруты помещаются перед более общими. Например, предположим, 
мы определили следующие маршруты:</p>
<pre class="brush:c#;">
public static void RegisterRoutes(RouteCollection routes)
{
    routes.IgnoreRoute("{resource}.axd/{*pathInfo}");
...
<p>Если мы отправим приложению запрос типа <span class="ii">http://localhost:49326/RuHome/Index</span> и если у нас в приложении не определен контроллер <span class="bb">RuHome</span>, 
то приложение вернет ошибку. Почему? Потому система маршрутизации пытается сопоставить запрос сначала с первым маршрутом. Если запрос не соответствует 
первому маршруту - тогда со вторым и так далее по списку маршрутов, пока не найдет нужный. В данном случае входящий запрос соответствует и первому и второму 
маршрутам. Однако первый маршрут будет искать контроллер по имени <span class="bb">RuHomeController</span> и, не найдя его, вернет ошибку. Поэтому чтобы подобная ситуация 
не произошла, надо сначала определить маршрут Default2, который более специфичен и не конфликтует с первым:</p>
<pre class="brush:c#;">
public static void RegisterRoutes(RouteCollection routes)
{
    routes.IgnoreRoute("{resource}.axd/{*pathInfo}");
...
<p>Кроме того, мы можем использовать префиксы в качестве псевдонимов для контроллера и его метода:</p>
<pre class="brush:c#;">routes.MapRoute( name: "Default2", url: "Store/Buy", defaults: new { controller = "Home", action="Index" } );</pre>
<p>Здесь для контроллера Home мы используем псевдоним <code>Store</code>, а для действия Index - псевдоним <code>Buy</code>. В итоге данный маршрут будет 
сопоставляться с таким запросом, как <span class="ii">Store/Buy</span>. А система маршрутизации будет обращаться по такому запросу к методу Index контроллера Home.</p>
<h3>Получение переданных параметров</h3>
<p>Чтобы получить переданные значения для параметров маршрута, мы можем воспользоваться объектом <strong>RouteData</strong>. Например, 
если у нас определен стандартный маршрут <code>routes.MapRoute(name: "Default", url: "{controller}/{action}/{id}", defaults: new { id = RouteParameter.Optional });</code>, 
то мы можем получить значение параметра controller следующим образом:</p>
<pre class="brush:c#;">
public string Index()
{
...
<p>Значения, передаваемые в качестве дополнительных значений, например, для параметра <code>id</code>, мы можем извлекать из самого параметра метода:</p>
<pre class="brush:c#;">
...
<h4>Передача различного числа параметров в запросе</h4>
<p>Ранее мы ограничивались только тремя сегментами. Но если у нас скажем метод принимает два и более параметров:</p>
<pre class="brush:c#;">
...
<p>То мы просто можем добавить в маршрут нужное нам количество параметров:</p>
<pre class="brush:c#;">
...
<p>Тогда данный маршрут будет обрабатывать запрос типа <span class="ii">Home/Index/1/name</span>, что эквивалентно следующей строке запроса: <span class="ii">Home/Index?id=1&name=name</span></p>
<p>Кроме того, мы можем обозначить любое количество сегментов в запросе, чтобы не быть жестко привязанным к числу сегментов с помощью параметра 
<span class="bb">{*catchall}</span>:</p>
<pre class="brush:c#;">
...
<p>Теперь мы можем обрабатывать запросы с любым количеством сегментов:</p>
<table class="tab">
<tr class="tabhead"><td><p>Запрос</p></td><td><p>Параметры запроса</p></td></tr>
<tr><td><p>mysyte.com</p></td><td><p>controller=Home</p>
<p>action=Index</p></td></tr>
<tr><td><p>mysyte.com/Book</p></td><td><p>controller=Book</p>
<p>action=Index</p></td></tr>
<tr><td><p>mysyte.com/Book/Show</p></td><td><p>controller=Book</p>
<p>action=Show</p></td></tr>
<tr><td><p>mysyte.com/Book/Show/2</p></td><td><p>controller=Book</p><p>action=Show</p><p>id=2</p></td></tr>
<tr><td><p>mysyte.com/Book/Show/2/Oldedition</p></td><td><p>controller=Book</p><p>action=Show</p><p>id=2</p><p>catchall=Oldedition</p></td></tr>
<tr><td><p>mysyte.com/Book/Show/2/Oldedition/1960</p></td><td><p>controller=Book</p><p>action=Show</p><p>id=2</p><p>catchall=Oldedition/1960</p></td></tr>
</table>
<p>После получения значения для параметра catchall мы сами должны обработать его и получить уже значения для отдельных сегментов.</p>
