...
<p>Рассмотрим атрибуты, участвующие в валидации модели на примере проекта приложения, созданного в прошлом параграфе данной главы.</p>
...
<img src="mvcpics/7.8.png" />
<p>Таким образом, если мы явным образом не установим текст сообщения, то при выводе ошибки будет отображаться стандартный текст сообщения.</p>
...
<img src="mvcpics/7.9.png" />
<h3>Атрибут RegularExpression</h3>
<p>Применение данного атрибута предполагает, что вводимое значение должно соответствовать указанному в этом атрибуте регулярному выражению.</p>
<p>Наиболее распространенный пример - это проверка корректности адреса электронной почты. Допустим, в некоторой модели у нас есть свойство <code>Email</code>:</p>
<pre class="brush:c#;">
...
<p>Атрибут Remote в отличие от предыдущих атрибутов находится в пространстве имен <strong>System.Web.Mvc</strong>. Он позволяет выполнять валидацию на стороне клиента с обратными вызовами на сервер.</p>
<p>Например, два пользователя не могут одновременно иметь одно и тоже значение <code>UserName</code>. 
Но с помощью валидации на стороне клиента трудно гарантировать, что введенное значение будет уникальным. 
А с помощью атрибута Remote мы можем послать значение свойства <code>UserName</code> на сервер, а там оно уже сравнивается со значениями, находящимися в базе данных.</p>
<pre class="brush:c#;">
[Remote("CheckUserName", "Account")]
public string UserName { get; set; }
</pre>
<p>В атрибуте можно установить имя действия и имя контроллера, которые должны вызываться кодом на стороне клиента. Клиентский код посылает 
введенное пользователем значение для свойства <code>UserName</code> автоматически, а перегруженный конструктор атрибута позволяет указать 
...
<p>Атрибут Compare также находится в пространстве имен System.Web.Mvc. Он гарантирует, что два свойства объекта 
