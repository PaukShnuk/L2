...
<html >
<head>
<title>Отображение ошибок валидации</title>
<meta charset="utf-8" />
<meta name="description" content="Отображение ошибок валидации с помощью хелпера Html.ValidationSummary">
...
<h2>Отображение ошибок валидации</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для отображения сообщений об ошибках при валидации модели мы можем использовать хелпер <strong>Html.ValidationSummary</strong>:</p>
<pre class="brush:xml;">
...
<p>В данном случае сообщения об ошибках выводятся над полями для ввода значений свойств:</p>
<img src="mvcpics/7.13.png" />
<p>Но данный хелпер имеет перегруженные версии, которые помогают настроить более точное отображение сообщений об ошибках:</p>
<table class="tab">
	<tr class="tabhead"><td><p>Перегруженная версия</p></td><td><p>Описание</p></td></tr>
	<tr><td><p><code>Html.ValidationSummary()</code></p></td><td><p>Отображает общий список ошибок сверху</p></td></tr>
	<tr><td><p><code>Html.ValidationSummary(bool)</code></p></td><td><p>Если параметр равен <code>true</code>, то вверху будут отображаться только 
	сообщения об ошибках уровня модели, а специфические ошибки будут отображаться рядом с полями ввода. Если же параметр равен <code>false</code>, то вверху отображаются все ошибки.</p></td></tr>
	<tr><td><p><code>Html.ValidationSummary(string)</code></p></td><td>Данная перегруженная версия хелпера отображает перед списком ошибок сообщение, которое передается в параметр <code>string<code><p></p></td></tr>
	<tr><td><p><code>Html.ValidationSummary(bool, string)</code></p></td><td><p>Сочетает две предыдущие перегруженные версии</p></td></tr>
</table>
<p>Также мы можем использовать хелпер Html.ValidationMessageFor для вывода сообщения об ошибке для отдельного свойства, 
причем рядом с полем для ввода свойства, а не в сводке ошибок сверху:</p>
<pre class="brush:xml;">
@Html.ValidationSummary(true)
...
<h3>Стилизация ошибок</h3>
<p>Еще один важный момент отображения ошибок - это их стилизация. То, что мы видим ошибки в красном цвете и границы полей ввода также в красном цвете, 
не жестко установлено, и мы все это можем изменить. В файле стилей <span class="bb">Site.css</span> в проектах, созданных по шаблону Basic и Internet Application, мы можем найти 
секцию стилей, которая как раз и отвечает за стилизацию (при использования проекта по типу Empty нам самим придется добавить стили):</p>
<pre class="brush:xml;">
...
<p>Это классы используются полями, генерируемыми хелперами валидации. И изменяя данные стили, мы можем управлять отображением ошибок. 
Итак, изменим два стиля - стиль класса <code>.field-validation-error</code> (он управляет отображением ошибок рядом с текстовыми полями) и 
стиль класса стиль класса <code>.input-validation-error</code>, который управляет отображением поля ввода в режиме ошибки:</p>
<pre class="brush:xml;">
.field-validation-error {
    color: #6699CC;
    font-weight:bold;
...
<p>В итоге при возникновении ошибок валидации мы увидим следующую картину:</p>
<img src="mvcpics/7.14.png" />
