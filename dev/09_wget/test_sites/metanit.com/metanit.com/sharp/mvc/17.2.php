...
<html >
<head>
<title>Первое приложение с SignalR</title>
<meta charset="utf-8" />
<meta name="description" content="Создание первого приложения на SignalR, использование хабов, построение серверной и клиентской части, регистрация в Global.asax">
...
<h2>Первое приложение с SignalR</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Создадим простенькое приложение с использованием SignalR - небольшой чат. Итак, создадим новое приложение ASP.NET MVC4 по шаблону Basic. 
Я назвал свое приложение <span class="bb">SignalRMvc</span>.</p>
<p>После создания проекта найдем через пакетный менеджер NuGet библиотеку <span class="bb">Microsoft ASP.NET SignalR</span>:</p>
<img src="mvcpics/17.1.png" alt="Microsoft ASP.NET SignalR" />
...
<p>Поскольку приложение чата оперирует пользователями, то создадим модель пользователей. Добавим в папку <i>Models</i> класс <i>User</i>:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Затем в папку <i>Controllers</i> добавим пустой контроллер <i>HomeController</i>, где будет один метод Index:</p>
<pre class="brush:c#;">
using System;
using System.Web;
...
<p>Больше нам ничего не надо, всю логику приложения будет содержать хаб.</p>
...
класс <i>ChatHub.cs</i>. Он будет иметь следующий код:</p>
<pre class="brush:c#;">
using System;
using System.Web;
...
<p>Затем для метода Index ранее созданного контроллера Home создадим представление <i>Index.cshtml</i>:</p>
<pre class="brush:xml;">
@{
    ViewBag.Title = "Чат";
}
...
<p>Теперь перейдем к коду клиентской части. Обратите внимание, что весь javascript у нас находится в специальной секции скриптов, 
которая подключается в мастер-страницу после библиотеки jQuery. Если у вас несколько другая структура приложения, то перед данным скриптом 
следует подключать основную библиотеку jQuery.</p> 
<p>Вначале подключаем библиотеку SignalR. (Также обратите внимание, что новейшая версия библиотеки может отличаться от использованной здесь.) После этого идет подключение скрипта хаба, 
который будет автоматически сгенерирован при запуске приложения.</p>
<p>Весь код находится в функции jQuery, за ее пределами определены две функции - <code>htmlEncode</code> (для кодирования тегов, чтобы 
...
<p>И в заключении нам надо зарегистрировать хаб. Для этого откроем файл <i>Global.asax</i> и добавим в метод <code>Application_Start</code> 
строку <code>RouteTable.Routes.MapHubs()</code>, которая и производит регистрацию:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="17.1.php">Назад</a><a href="./">Содержание</a><a href="17.3.php">Вперед</a></p></div></div>
<div class="socBlock">
