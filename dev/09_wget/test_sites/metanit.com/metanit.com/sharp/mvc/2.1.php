...
<html >
<head>
<title>Создание первого приложения в MVC 4</title>
<meta charset="utf-8" />
<meta name="description" content="Создание первого приложения на mvc 4, которое призвано дать некоторое начальное понимание работы mvc">
...
<h1>Создание первого приложения на ASP.NET MVC 4</h1>
<h2>Создание проекта</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Сейчас мы создадим первое примитивное приложение на mvc 4, которое призвано дать некоторое начальное понимание работы mvc. 
В качестве шаблона проекта мы выберем шаблон Empty и создадим мини-приложение с нуля.</p>
<p>Что будет делать наше приложение? Приложение будет представлять примитивную эмуляцию книжного магазина, в котором мы сможем просматривать список книг 
и оформлять покупку.</p>
<p>Итак, откроем Visual Studio или Visual Web Developer, в меню выберем <span class="bb">File -&gt; New Project..</span> и среди шаблонов проектов выберем шаблон 
<span class="bb">ASP.NET MVC 4 Web Application</span> и как-нибудь назовем новый проект, например, BookStore. Затем в окне создания нового проекта 
ASP.NET MVC 4 Web Application выберем шаблон Empty:</p>
<img src="mvcpics/2.1.png" />
<p>После этого будет создан по сути пустой проект, в котором еще ничего нет. Нам предстоит наполнить его.</p>
<p>Во-первых, нам предстоит работать с данными, которые должны быть представлены некими моделями. Для нашей задачи мы можем выделить две области данных: 
информация о книге и информация, связанная с оформлением покупки. Соответственно, мы создадим две модели. Итак, в нашем приложении уже определена папка <span class="bb">Models</span>. 
В ней будут находиться наши модели. Нажмем на эту папку правой кнопкой мыши и в появившемся меню выберем <span class="bb">Add-&gt;Class...</span>. Назовем новый класс или модель <span class="bb">Book</span> 
и добавим в него код, описывающий модель книги:</p>
<pre class="brush:c#;">
...
<p>Теперь таким же образом создадим класс для оформления покупки - класс <span class="bb">Purchase</span>:</p>
<pre class="brush:c#;">
using System;
namespace BookStore.Models
{
...
<p>В данном случае надо сказать, что в данном случае действуют некоторые условности. Поскольку база данных и соответствующая ей модель должна 
...
определение ключа по названию свойства. Для этого модель должна содержать свойство с именем <span class="bb">Имя_моделиId</span> или просто <span class="bb">Id</span>. Так, у нас 
в модели Purchase определено свойство PurchaseId, то есть данное свойство является первичным ключом. Второй способ состоял в определении ключа с помощью 
атрибута Key, установленным над нужным свойством. Более подробно об этом мы поговорим в главе, где будем обсуждать модели.</p>
<h3>Создание контекста данных и EntityFramework</h3>
<p>Для более удобного доступа к данным рекомендуется использовать фреймворк Entity Framework, хотя его использование необязательно и всецело зависит от 
предпочтений разработчика. Этот фреймворк позволяет абстрагироваться от структуры конкретной базы данных и вести все операции с данными через модель.</p>
<p>Поскольку мы создали проект по шаблону Empty, который практически не содержит никаких файлов, то нам надо будет 
добавить в проект файлы EntityFramework для взаимодействия с БД. Если бы мы создавали проект по другому шаблону, например, 
Basic или Internet Application, то библиотека EntityFramework уже была бы добавлена по умолчанию.</p>
<p>Итак, добавим в проект поддержку EntityFramework и для этого нажмем правой кнопкой мыши в структуре проекта на узел References (который содержит подключенные 
в проект библиотеки) и в появившемся меню выберем <span class="bb">Manage NuGet Packages...</span></p>
<img src="mvcpics/2.2.png" />
<p>В окне управления пакетами NuGet в правом верхнем углу введите в поле поиска <span class="bb">Entity</span> и нажмите Enter. После этого все найденные пакеты, которые
имеют отношение к запросу, будут отображены в среднем столбце, как показано на рисунке:</p>
<img src="mvcpics/2.3.png" />
<p>Затем устанавливаем пакет, и после того, как закончится установка, мы можем использовать все преимущества EntityFramework для 
управлением данными.</p>
<p>После установки создадим контекст данных. Он нужен для облегчения доступа к БД на основе некоторой модели. Добавим в папку Models новый класс 
<span class="bb">BookContext</span> со следующим содержанием:</p>
<pre class="brush:c#;">
using System;
...
Свойства типа <code>public DbSet&lt;Book&gt; Books { get; set; }</code> помогают получать набор данных определенного типа (в данном случае 
типа Book).</p>
<p>В итоге EntityFramework сам возьмет на себя заботу по созданию базы данных. То есть в данном случае мы будем использовать так называемый подход 
<b>Code First</b> - у нас есть модели, и по ним фреймворк будет создавать таблицы в базе данных.</p>
<p>Наконец, укажем строку подключения. Откроем файл <i>web.config</i>, найдем секцию <i>configSections</i> и после нее вставим 
секцию <i>connectionStrings</i>:</p>
<pre class="brush:xml;">
&lt;connectionStrings&gt;
...
<p>Тем самым мы определяем путь к базе данных, которая потом будет создаваться. Выражение <code>|DataDirectory|</code> представляет заместитель, 
