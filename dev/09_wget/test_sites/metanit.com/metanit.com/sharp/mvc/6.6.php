...
<html >
<head>
<title>Создание собственного обработчика маршрутов</title>
<meta charset="utf-8" />
<meta name="description" content="Описание механизма маршрутизации и создание обработчика маршрутов через реализацию интерфейса IHttpHandler">
...
<h2>Создание собственного обработчика маршрутов</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Пред тем как приступить к созданию собственного обработчика маршрутов, посмотрим, что в целом представляет собой процесс маршрутизации.</p>
<p>Процесс маршрутизации состоит из следующих этапов:</p>
<ol>
<li><p>Модуль <code>UrlRoutingModule</code> пытается сопоставить текущий запрос с маршрутами в таблице <code>RouteTable</code>.</p></li>
<li><p>Если сопоставление завершилось удачно, то модуль маршрутизации выбирает обработчик маршрутов сопоставленного маршрута -  объект <strong>IRouteHandler</strong>.</p></li>
<li><p>Затем у объекта <code>IRouteHandler</code> вызывается метод <code>GetHandler</code>, который возвращает объект <code>IHttpHandler</code>, 
используемый для обработки запроса.</p></li>
<li><p>У обработчика <code>IHttpHandler</code> вызывается метод <code>ProcessRequest</code> для обработки запросов.</p></li>
</ol>
<p>По умолчанию обработчик запросов или объект <code>IRouteHandler</code> представляет экземпляр класса <code>MvcRouteHandler</code>, 
который возвращает объект <code>MvcHandler</code>, применяющий интерфейс <code>IHttpHandler</code>. Этот объект <code>MvcHandler</code> 
отвечает за инициализацию контроллера, который потом уже вызывает одно из своих действий.</p>
<p>Однако мы можем переопределить это поведение, указав при определении маршрута свой обработчик маршрутов. Итак, сначала создадим сам обработчик. 
Для обработчика создадим какую-нибудь специальную папку в проекте и добавим в нее новый класс. Назовем его <code>MyRouteHandler</code>:</p>
<pre class="brush:c#;">
using System.Web;
...
<p>Как было уже сказано про этапы маршрутизации, нам нужно собственно два класса: класс, реализующий интерфейс IHttpHandler, который и будет 
обрабатывать запрос;  и класс, реализующий интерфейс IRouteHandler, который сопоставляется с маршрутом, и вызывает первый класс.</p>
<p>Теперь в классе <code>RouteConfig</code> пропишем маршрут, который будет обрабатываться нашим обработчиком, не забывая при этом импортировать 
пространство имен, в котором объявлен наш обработчик:</p>
<pre class="brush:c#;">
...
<p>Теперь мы можем адресовать нашему приложению соответствующий запрос, например, <span class="ii">Home/Index</span> и браузер выведет нам нашу строку с посланием.</p>
...
<div class="nav"><p><a href="6.5.php">Назад</a><a href="./">Содержание</a><a href="7.1.php">Вперед</a></p></div></div>
<div class="socBlock">
