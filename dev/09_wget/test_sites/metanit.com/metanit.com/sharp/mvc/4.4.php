...
<html >
<head>
<title>Мастер-страницы</title>
<meta charset="utf-8" />
<meta name="description" content="Использование мастер-страниц для создания едиообразия сайта, использование секций и представлений ViewStart">
...
<p>Для создания единообразного вида сайта применяются мастер-страницы. Мастер-страницы - это по сути те же самые представления. 
Мы можем определить на мастер-странице некоторые элементы, которые будут отображаться 
на всех страницах сайта. А также мы можем определить заполнители или плейсхолдеры, содержание которых обеспечивают другие представления.</p>
<p>При создании первого приложения в предыдущих главах мы использовали следующую мастер-страницу _Layout.chtml:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>На вид это обычное представление за одним исключением - вызова метода <code>@RenderBody()</code>. Этот вызов является плейсхолдером, 
на место которого другие представления, которые используют эту мастер-страницу, будут подставлять свое содержимое. И таким образом, мы 
можем легко установить для представлений веб-приложения единообразный стиль.</p>
<p>Чтобы применить мастер-страницу к представлению, мы должны в секции <span class="bb">Layout</span> указать путь к мастер-странице. Например, наше представление Index.cshtml, 
использующее эту мастер-страницу, начинается так:</p>
<pre class="brush:xml;">
@{
    Layout = "~/Views/Shared/_Layout.cshtml";
}
</pre>
<p>Если мы не используем мастер-страницу, то мы указываем <code>Layout = null;</code>.</p>
...
<p>Однако, при таком подходе, если у нас есть куча представлений, и мы вдруг захотели определить новую секцию на мастер-странице, нам придется 
изменить все имеющиеся представления, что не очень удобно. Но мастер-страницы предлагают нам варианты гибкой настройки секций.</p>
...
<h3>ViewStart</h3>
<p>Если у нас в проекте пара-тройка представлений, мы легко можем изменить для каждого описание мастер-страницы в секции Layout, если, например, 
мы решим использовать другую мастер-страницу. Однако, если у нас много представлений, то это делать будет не очень удобно.</p>
...
<p>При создании проекта ASP.NET MVC 3/4 по шаблону Basic или Internet в каталоге Views уже находится файл <code>_ViewStart.cshtml</code>. 
Этот файл определяет мастер-страницу, используемую по умолчанию. При создании нового проекта ASP.NET MVC 3/4 по шаблону Empty, нам надо добавить 
новое представление <span class="bb">_ViewStart.cshtml</span> в папку Views и удалить весь ранее автоматически сгенерированный текст и добавить туда следующие строки:</p>
<pre class="brush:xml;">@{
    Layout = "~/Views/Shared/_Layout.cshtml";
}
</pre>
<p>Этот код выполняется до любого другого кода, определенного в представлении, поэтому из других представлений мы можем удалить секцию <code>Layout</code>. 
