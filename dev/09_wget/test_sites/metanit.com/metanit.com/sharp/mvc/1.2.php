...
<html >
<head>
<title>Начало работы с ASP.NET MVC 4</title>
<meta charset="utf-8" />
<meta name="description" content="Для создания приложений на ASP.NET MVC нужна среда разработки - Visual Studio 2010 (Visual Studio 2012) и фреймворк MVC 3/4">
...
<h2>Начало работы с ASP.NET MVC 4</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<P>Итак, чтобы создавать приложения на ASP.NET MVC нам потребуется среда разработки - Microsoft Visual Studio Community 2013 или Microsoft Visual Studio Express 2013 for Web, которые можно найти на 
странице <a href="https://www.visualstudio.com/ru-ru/downloads/download-visual-studio-vs#DownloadFamilies_2" rel="nofollow">Visual Studio 2013</a>. Также мы можем использовать один из 
выпусков Visual Studio 2012, которые тоже включают средства разработки для MVC 4.</P>
<p>После установки всего необходимого инструментария мы можем создать первое приложение. Итак, откроем Microsoft Visual Studio Community 2013 (или 
другой имеющийся выпуск Visual Studio 2012 / 2013). В меню <span class="bb">File (Файл)</span> выберем пункт <span class="bb">New Project... (Создать проект)</span>. 
Перед нами откроется диалоговое окно создания проекта. Cреди типов проекта выбрем 
Web-&gt;Visual Studio 2012, а в центральном поле - шаблон ASP.NET MVC 4 Web Application:</p>
<img src="mvcpics/1.5.png" alt="Создание проекта MVC 4 в Visual Studio 2013" />
<p>После выбора типа проекта перед нами откроется окно выбора шаблона нового приложения:</p>
<img src="mvcpics/1.3.png" />
<p>Выберем шаблон <span class="bb">Internet Application</span>. Что представляют все эти шаблоны?</p>
<h4>Шаблоны приложения MVC 4</h4>
<ul>
<li><p><span class="bb">Internet Application</span>. Данный шаблон представляет некоторую базовую функциональность, например, управление учетными записями, 
которую будет содержать новое приложение. </p></li>
<li><p><strong>Intranet Application</strong>. Данный шаблон похож на шаблон Internet Application. Отличие состоит в том, что в Intranet Application для 
управления учетными записями используется аутентификация Windows, а не система ASP.NET Membership. </p></li>
<li><p><span class="bb">Empty</span>. Этот шаблон представляет абсолютно чистое приложение без каких-либо файлов и структуры папок.</p></li>
<li><p><span class="bb">Basic</span>. Этот шаблон также представляет пустой проект. Правда, он содержит базовые каталоги, CSS, инфраструктуру приложения MVC. 
Ранее в MVC 3 этот шаблон назывался Empty.</p></li>
<li><p><span class="bb">Mobile Application</span>. Это новинка MVC 4, предназначенная для создания веб-приложений для мобильных платформ.</p></li>
<li><p><span class="bb">Web API</span>. Предназначен для создания сервисов и для работы с фреймворком Web API.</p></li>
<li><p><span class="bb">Single Page Application</span>. Шаблон, предназначенный для создания сайтов-одностраничников.</p></li>
<li><p><span class="bb">Facebook Application</span>. Предназначен для создания веб-приложений под социальную сеть Facebook.</p></li>
</ul>
<p>Кроме того, данное диалоговое окно предлагает нам выбрать движок представления в секции <strong>View Engine</strong> и задать опции тестирования.</p>
 <p>При выборе движка представления нам, как и в MVC 3, доступны две опции - движок <strong>Razor</strong> и ASPX (традиционный движок Web Forms). В данном случае мы 
 выберем Razor. Опции тестирования мы пока трогать не будем.</p>
 <p>Итак, нажимаем кнопку OK и создаем новый проект. Мы сразу же можем запустить проект на выполнение и нам предстанет чудная картинка в браузере 
 с нашим приложением:</p>
 <img src="mvcpics/1.4.png" />
 <p>Таким образом, весь функционал, который содержит проект по умолчанию, реализуется в такое приложение. Теперь давайте посмотрим, что представляет 
 структура проекта.</p>
 <h3>Структура проекта MVC 4</h3>
 <p>Справа в окне Solution Explorer (Обозреватель решений) мы можем увидеть структуру проекта MVC 4. Тот, кто раньше работал с предыдущими версиями MVC, 
 заметит некоторые отличия. Итак, пройдемся по папкам и файлам проекта.</p>
 <h4>App_Data</h4>
 <p>В этой папке хранятся все необходимые файлы и ресурсы, например, базы данных, используемые приложением. После развертывания приложения только 
 непосредственно приложение может работать с этой папкой, доступ же простых пользователей в эту папку запрещен.</p>
 <h4>Файл Global.asax и папка App_Start</h4>
 <p>В mvc 4 была добавлена папка App_Start. Она включает весь функционал конфигурации приложения, который в предыдущих версиях содержался в файле 
 ...
 приложения, выполняющуюся при запуске.</p>
  <h4>Файл Web.config</h4>
 <p>Файл конфигурации приложения, который находится в корневой папке приложения</p>
 <h4>Content</h4>
 <p>Содержит некоторые вспомогательные файлы, которые не включают код на c# или javascript, и которые развертываются вместе с приложением. В частности, 
 ...
 стили css и изображения для определенных тем.</p>
 <h4>Controllers</h4>
 <p>Содержит контроллеры - классы, отвечающие за работу приложения. По умолчанию здесь находятся два контроллера - HomeController и AccountController.</p>
<h4>Папки Images и Scripts</h4>
<p>Папки Images и Scripts содержат соответственно изображения и скрипты на JavaScript, используемые в приложении. По умолчанию эти папки уже содержат 
файлы, в частности, в папку Scripts уже помещены файлы библиотеки jQuery.</p>
<h4>Models</h4>
<p>Содержит модели, используемые приложением. По умолчанию здесь определена одна модель - AccountModel, которая представляет отдельную учетную запись.</p>
 <h4>Views</h4>
<p>Здесь размещаются представления. Представления группированы по папкам, каждая из которых соответствует одному контроллеру. После получения и обработки запроса 
...
папка Shared</p>
<p>Итак, мы посмотрели на некоторые базовые части проекта и теперь создадим первое приложение.</p>
