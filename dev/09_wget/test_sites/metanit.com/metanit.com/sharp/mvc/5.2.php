...
<meta charset="utf-8" />
<meta name="description" content="Чтобы хранить данные, нам естественным образом нужна база данных. Как правило, в качестве базы данных используется MS SQL Server.">
...
<p>Чтобы хранить данные, нам естественным образом нужна база данных. Как правило, в качестве базы данных используется MS SQL Server, на примере которого мы и 
посмотрим весь процесс создания БД и подключения к ней.</p>
<p>Мы можем создать базу данных прямо в проекте, либо же создать ее на сервере MS SQL. Для хранения баз данных  проекте у нас предназначена папка <strong>App_Data</strong>. Для этого нажмем правой кнопкой 
мыши на папку App_Data и в появившемся контекстном меню выберем <span class="bb">Add-&gt; New Item...</span>. В появившемся окне добавления нового элемента 
выберем <strong>SQL Server</strong><span class="bb"> Database</span> и назовем новую базу данных Bookstore.mdf:</p>
<img src="mvcpics/5.1.png" />
<p>Мы можем создать базу данных равнозначным образом и на сервере. После этого база данных добавляется в проект, и мы можем увидеть ее в папке App_Data. Теперь в обозревателе баз данных (окно <span class="bb">Database Explorer</span>) 
мы можем подключиться к ней и создать таблицы, которые будут хранить данные.</p>
<p>Раскроем узел Bookstore.mdf и найдем узел Tables. Нажмем на этот узел правой кнопкой мыши и в появившемся меню выберем пункт <span class="ii">Add New Table</span>. И 
...
Entity Framework должны соответствовать имени модели. То есть, так как наша модель называется <code>Book</code>, то таблица будет называться <code>Books</code>. 
А Entity Framework автоматически распознает, что таблица <code>Books</code> соответствует классу <code>Book</code>.</p>
<p>Итак, создадим структуру таблицы:</p>
<img src="mvcpics/5.2.png" />
<p>Не забудьте установить ниже в окне Properties (в Visual Stidio 2010 - окно Column Properties) для столбца Id соответствующие параметры для первичного ключа:</p>
<img src="mvcpics/5.3.png" />
<p>После этого, если мы работаем с Visual Studio 2010, нам будет предложено просто ввести имя таблицы - введем имя Books, 
и затем таблица добавляется в БД.</p>
<p>А в Visual Studio Express 2012 for Web нам надо сгенерировать таблицу на основе заданного выше 
определения. Для этого нажмем на кнопку Update:</p>
<img src="mvcpics/5.3.1.png" />
<p>В появившемся диалоговом окне нажмем на кнопку Update Database. После этого в нашу базу данных добавляется 
только что сгенерированная таблица. Подобным образом определим таблицу Purchases для модели Purchase:</p>
<img src="mvcpics/5.4.png" />
<p>Добавим в таблицу Books несколько записей:</p>
<img src="mvcpics/5.5.png" />
<p>Теперь, во-первых, чтобы взаимодействовать с БД, нам нужен класс контекста данных, пусть это будет следующий класс BookContext:</p>
<pre class="brush:c#;">
using System;
...
<p>Во-вторых, определим строку подключения к БД. Для этого откроем файл Web.config и добавим в конец секции 
<span class="bb">configuration</span> определение строки подключения. Однако тут надо сразу заметить, 
что для Visual Studio 2010 строка подключения будет отличаться от строки подключения, которая используется в Visual Studio 2012.</p> 
<p>Итак, строка подключения для <u>Visual Studio 2010</u> будет выглядеть так:</p>
<pre class="brush:xml;">
&lt;configuration&gt;
.........................
    &lt;connectionStrings&gt;
...
<p>Для <u>Visual Studio 2012</u> будет выглядеть определение строки подключения будет выглядеть следующим образом:</p>
<pre class="brush:xml;">
&lt;configuration&gt;
.........................
    &lt;connectionStrings&gt;
...
<p>В Visual Studio 2012 в отличие от 2010-й версии мы можем использовать режим LocalDB, который предназначен прежде всего для разработчиков, 
представляя некоторую упрощенную версию. Поэтому в данном случае в качестве источника данных используется <b>(LocalDB)\v11.0</b>.</p>
<p>Обратите внимание, что в обоих случаях свойство <code>name="BookContext"</code> должно содержать название контекста данных.</p>
<p>Использование подстановки <i>|DataDirectory|</i> позволяет опустить полный физический путь к базе данных, которая хранится в папке App_Data.</p>
<p>Теперь мы можем получить содержимое таблицы Books в контроллере Home:</p>
<pre class="brush:c#;">
...
<p>И вывести данные в представлении Index.cshtml:</p>
<pre class="brush:xml;">
