...
<html >
<head>
<title>Введение в создание сайтов на ASP.NET MVC</title>
<meta charset="utf-8" />
<meta name="description" content="ASP NET MVC представляет собой платформу для создания сайтов и веб-приложений с использованием паттерна (или шаблона) MVC (model - view - controller)">
...
<h1>Введение в ASP.NET MVC</h1>
<h2>Особенности платформы ASP.NET MVC</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>ASP.NET MVC представляет собой платформу для создания сайтов и веб-приложений с использованием паттерна (или шаблона) MVC (model - view - controller). </p>
<p>Работа над новой платформой была начата в 2007 году, а в 2009 году появилась первая версия. В итоге к текущему моменту (2012 год) уже было выпущено 
4 версии платформы, а сам фреймворк обрел большую популярность по всему миру благодаря своей гибкости и адаптивности.</p>
<p>Шаблон MVC, лежащий в основе новой платформы, подразумевает взаимодействие трех компонентов: контроллера (controller), модели (model) и 
представления (view). Что же представляют эти компоненты?</p>
<p><span class="bb">Контроллер</span> (controller) представляет класс, с которого собственно и начинается работа приложения. Этот класс обеспечивает связь между 
...
соответствующее представление.</p>
<p><span class="bb">Представление</span> (view) - это собственно визуальная часть или пользовательский интерфейс приложения - например, html-страница, через которую пользователь, зашедший на сайт, 
взаимодействует с веб-приложением.</p>
<p><span class="bb">Модель</span> (model) представляет набор классов, описывающих логику используемых данных.</p>
<p>Общую схему взаимодействия упрощенно можно представить следующим образом:</p>
<img src="mvcpics/1.1.png" alt="Взаимодействие компонентов паттерна MVC" />
<h3>ASP.NET MVC и ASP.NET Web Forms</h3>
<P>ASP.NET MVC является в некотором роде конкурентом для традиционных веб-форм и имеет по сравнению с ними следующие преимущества:</P>
<ul>
<li><p><span class="bb">Разделение ответственности</span>. В MVC приложение состоит из трех частей: контроллера, представления и модели, каждая из которых выполняет 
свои специфичные функции. В итоге приложение будет легче поддерживать  модифицировать в будущем.</p></li>
<li><p>В силу разделения ответственности приложения mvc обладают <span class="bb">лучшей тестируемостью</span>. И мы можем тестировать отдельные компоненты 
независимо друг от друга.</p></li>
<li><p><span class="bb">Соответствие протоколу HTTP</span>. Приложения MVC в отличие от веб-форм не поддерживают объекты состояния (ViewState). Ясность и простота 
платформы позволяют добиться большего контроля над работой приложения</p></li>
<li><p><span class="bb">Гибкость</span>. Вы можете настраивать различные компоненты платформы по своему усмотрению. Изменять какие-либо части конвейера работы MVC 
или адаптировать его к своим нуждам и потребностям.</p></li>
</ul>
<p>В то же время не стоит однозначно сбрасывать со счетов ASP.NET WebForms. Поскольку она также имеет свои сильные стороны, например, модель событий, которая 
будет ближе тем разработчикам, которые ранее занимались созданием клиентских приложений.</p>
<p>В традиционных веб-формах вы имеете контроль над разметкой и можете в реальном времени в визуальном редакторе Visual Studio увидеть, как будет выглядеть 
та или иная страница. При работе с MVC Visual Studio подобного не позволяет делать.</p>
<p>В любом случае вы вольны выбирать ту платформу, которая 
...
Тем более, что ASP.NET Web Forms еще не умирает и также продолжает развиваться.</p>
