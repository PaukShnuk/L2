...
<html >
<head>
<title>OAuth-аутентификация</title>
<meta charset="utf-8" />
<meta name="description" content="Понятие OAuth-аутентификации, ее применение в приложении на примере авторизации через учетную запись в сервисе google">
...
<h2>OAuth-аутентификация</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В современном мире велика популярность различных социальных сервисов и соцсетей - google+, twitter, facebook и т.д. Это привело разработчиков 
к мысли использовать в качестве логина на сайт учетную запись в данных сервисах и соцсетях. До MVC4 это сложно было сделать. Но в MVC4  
появились соответствующие механизмы, которые позволяют предельно упростить аутентификацию с помощью соцсетей.</p>
<p>В ряде случаев нам надо будет зарегистрировать свое приложение в социальных сервисах, чтобы воспользоваться возможностью подобной авторизации 
(как например в Facebook). А некоторые сервисы как Goggle и Yohoo, позволяют использовать возможность аутентификации на основе социальных 
сервисов без всякой регистрации.</p>
<p>Итак, в новом проекте по шаблону Internet Apllication мы уже можем найти уже готовую инфраструктуру для использования OAuth-аутентификации. 
Если вы посмотрите на файл <strong>AuthConfig.cs</strong>, который находится в папке App_Start, то вы найдете в нем такой код:</p>
<pre class="brush:c#;">
using System;
...
<p>Так, настройка аутентификации через Facebook:</p>
<pre class="brush:c#;">
//OAuthWebSecurity.RegisterFacebookClient(
//    appId: "",
//    appSecret: "");
</pre>
<p>Если ваше веб-приложение зарегистрировано в соцсети facebook, то вы затем можете ввести в параметр <code>appId</code> идентификатор вашего 
приложения в этой сети, а в параметр <code>appSecret</code> секретное слово. Значения для обоих параметров выдаются уже самой соцсетью при регистрации приложения.</p>
<p>В данном случае воспользуемся аутентификации с помощью Google и раскомментируем строку <code> OAuthWebSecurity.RegisterGoogleClient();</code>.</p>

<p>Контроллер AccoutController уже содержит ряд действий для работы с подобной схемой авторизации: <code>ExternalLogin</code> (для авторизации 
с помощью внешних сервисов), <code>Manage</code> (для управления учетной записью), <code>ExternalLoginConfirmation</code> (для подтверждения 
авторизации) и ряд других.</p>
<p>Затем в папке <i>Views/Account</i> можно увидеть ряд представлений, предназначенных для работы с OAuth. Например, частичное представление 
<i>_ExternalLoginListPartial.cshtml</i> - здесь можно настроить отображение списка сервисов,  <i>ExternalLoginConfirmation</i> - 
для отображения формы подтверждения логина и <i>ExternalLoginFailure</i> - для отображения формы ошибки авторизации.</p>
<p>При обращении к странице логина после этого у нас будет выбор способа авторизации - с помощью стандартных механизмов членства, либо 
с помощью учетной записи Google.</p>
<img src="mvcpics/11.18.png" />
<p>Нажмем на кнопку Google и мы попадем на страницу авторизации в самом сервисе google.</p>
<img src="mvcpics/11.19.png" />
<p>Введем логин и пароль, и после успешной авторизации на google нас попросят принять некоторые разрешения:</p>
<img src="mvcpics/11.20.png" />
<p>Нажмем на кнопку Принять и после этого мы попадем на страницу нашего приложения, где нам надо будет ассоциировать свою учетную запись 
внутри нашего приложения с учетной записью на google.</p>
<img src="mvcpics/11.21.png" />
<p>Нажмем на кнопку Register и тем самым свяжем учетные записи.</p>
<img src="mvcpics/11.22.png" />
...
<div class="nav"><p><a href="11.12.php">Назад</a><a href="./">Содержание</a><a href="12.1.php">Вперед</a></p></div></div>
<div class="socBlock">
