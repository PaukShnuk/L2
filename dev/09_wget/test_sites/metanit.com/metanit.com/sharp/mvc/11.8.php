...
<html >
<head>
<title>Создание своих провайдеров членства/ролей</title>
<meta charset="utf-8" />
<meta name="description" content="Создание своей логики авторизации, переопределение стандартных провайдеров членства и ролей, настройка своей базы данных">
...
<h2>Создание своих провайдеров членства и ролей</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>ASP.NET MVC предоставляет нам законченную модель провайдеров членства/ролей (в виде универсальных провайдеров или в виде SimpleMembershipPrivider API), 
которые предлагают нам некоторую стандартную инфраструктуру с уже определенными таблицами. Однако иногда стандартные механизмы членства 
могут не удовлетворять наши потребности. Например, некоторым модель членства кажется довольно тяжеловесной. А кто-то хотел бы применить 
свою схему таблиц и авторизации. Поэтому в данном случае нам поможет переопределение стандартных механизмов членства и ролей.</p>
<p>Переопределение членства предполагает, что мы должны переопределить методы абстрактного класса <strong>MembershipProvider</strong>, 
который лежит в основе провайдеров членства. А переопределение провайдера ролей включает реализацию абстрактного класса <strong>RoleProvider</strong>. 
Посмотрим на пример. Создадим новый проект ASP.NET MVC 4 по шаблону Basic. 
Назовем его, например, <span class='bb'>CustomAuthorization</span>.</p>
<p>Первым делом определим модели, которые будут описывать учетные записи. Это у нас будут модели пользователей и ролей, а также контекст 
данных для доступа к ним. Итак, добавим в папку Models следующие классы:</p>
<pre class="brush:c#;">
using System;
...
<p>Во-первых, здесь определен класс пользователя, который будет иметь следующие поля: id, адрес электронной почты, дату регистрации, 
пароль и ссылку на выполняемую роль в системе. Предполагается, что пользователи в качестве логина на сайте будут использовать адрес 
электронной почты.</p>
<p>Во-вторых, у нас имеется класс роли в системе, а также контекст UserContext, через который мы будем получать данные из БД.</p>
<p>В отличие от провайдеров членства, рассмотренных в прошлых разделах, здесь мы сами создадим базу данных, структуру которой мы определим сами. 
Эта структура будет полностью соответствовать вышеописанным моделям.</p>
<p>Итак, создадим новую базу данных для нашего приложения, поместив ее в папку <span class="ii">App_Data</span>. Назовем ее, например, <span class="bb">StoreDB</span>.</p>
<p>Создадим таблицу Roles, которая будет хранить роли пользователей:</p>
<img src="mvcpics/11.15.png" />
<p>Создадим таблицу Users, которая будет хранить пользователей:</p>
<img src="mvcpics/11.16.png" />
<p>Итак, столбцы в таблицах в точности соответствуют определению моделей, а в качестве ссылки на роль в таблице Roles в таблице Users 
используется внешний ключ. Это и все в плане хранения данных, что нам потребуется для аутентификации и авторизации пользователей.</p>
<p>Добавим в таблицу Roles пару ролей (пусть это будут роли администратора и простого пользователя):</p>
<img src="mvcpics/11.17.png" />
<p>И в файле web.config добавим определение строки подключения:</p>
<pre class="brush:xml;">
&lt;connectionStrings&gt;
...
<p>Теперь в следующей теме создадим свой провайдер членства.</p>
