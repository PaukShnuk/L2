...
<html >
<head>
<title>Работа со сложными моделями</title>
<meta charset="utf-8" />
<meta name="description" content="Управление сложными моделями: их добавление, редактирование и удаление">
...
<h2>Работа со сложными моделями</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В прошлом разделе мы создали две модели Player и Team и вывели элементы из таблицы Players на страницу. Теперь посмотрим, как 
проделать остальные операции с моделями - редактирование, удаление, добавление. Эти действия не сильно отличаются от логики работы с 
простыми моделями. Единственное отличие - мы должны учитывать навигационное свойство, имеющееся в сложной модели.</p>
<h3>Добавление модели</h3>
<p>Рассмотрим логику добавления модели Player в таблицу Players. Вначале пропишем в контроллере следующее действие Create:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>Первый вариант действия Create обрабатывает GET-запрос и выдает представление, передавая в него объект <strong>SelectList</strong> - 
список всех команд.</p> 
<p>Второй вариант действия получает введенную пользователем в представлении модель и добавляет ее в БД. Рассмотрим само представление 
<i>Create.cshtml</i>:</p>
<pre class="brush:xml;">
...
<p>Как и в случае с простыми моделями, мы привязываем поля к определенному свойству. </p>
<p>Тут следует лишь отметить создание выпадающего списка, из которого мы выбираем команду. Выбираемое значение в этом списке привязывается 
к свойству модели TeamId.</p>
<img src="mvcpics/5.16.png" />
<h3>Редактирование модели</h3>
<p>Редактирование работает подобным способом. Определим в контроллере действия, которые отвечают за редактирование:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>Здесь мы также формируем список команд, которые мы получаем из БД, в объекте SelectList. И после получения запроса на редактирование 
определенной модели Player контроллер передает эту модель и список команд в представление <i>Edit.cshtml</i>:</p>
<pre class="brush:xml;">
@model NavigationProperty.Models.Player
@{
    ViewBag.Title = "Edit";
...
<p>Ну и удаление производится также, как и в случае с обычной моделью.</p>
