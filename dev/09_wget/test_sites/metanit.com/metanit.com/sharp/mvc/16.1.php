...
<html >
<head>
<title>Мультиязычный сайт и локализация</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в локализацию в ASP.NET MVC, создание ресурсов для различных культур, настройки ресурсов, их название и связь с конкретными культурами">
...
<h1>Мультиязычный сайт и локализация</h1>
<h2>Введение в мультиязычные сайты. Ресурсы</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Сегодня никого не удивишь мультиязычными сайтами. Благодаря мультиязычночти сайт становится доступным гораздо большему числу потенциальных 
пользователей, то сулит большие выгоды, например, увеличение дохода, увеличение аудитории и т.д. Посмотрим, как в ASP.NET MVC 4 мы можем создать мультиязычный сайт.</p>
<p>Перед созданием мультиязычного сайта надо определиться с концепцией.</p>
<ul>
<li><p>Мы можем сделать для каждой отдельной культуры свое представление и затем в зависимости от выбранной культуры рендерить его в ответ клиенту.</p></li>
<li><p>Второй вариант предполагает использование ресурсов, когда у нас одно представление для всех культур, и в зависимости от выбранной культуры 
подгружаются определенные для нее ресурсы.</p></li>
</ul>
<p>Используем второй вариант. Создадим проект по типу Basic (в моем случае он будет называться MultilingualSite). И первым делом создадим модель. 
Допустим, наше приложение будет работать со списком пользователей. Добавим в папку Models модель <span class="bb">Person</span>:</p>
<pre class="brush:c#;">
using System;
...
<p>В данном случае для упрощения примера я использовал всего три поля. К двоим применены атрибуты Required и Display. Только в данном случае 
атрибут Required указывает на тип ресурса, который будет применяться (<b>ErrorMessageResourceType</b>) и имя ресурса (<b>ErrorMessageResourceName</b>).</p>
<p>Определение <code>ErrorMessageResourceType = typeof(Resources.Resource), ErrorMessageResourceName = "CountryRequired"</code> 
...
уже конкретный ресурс, значение которого будет использоваться в качестве сообщения об ошибке при валидации. То же самое можно прописать и для других атрибутов валидации, например, для атрибута StringLength.</p>
<p>Пока у нас нет никаких ресурсов, но сейчас мы их создадим.</p>
<p>То же самое характерно и для атрибута <code>Display</code>, только в данном случае его свойство <code>Name</code> указывает на имя ресурса, 
который будет использоваться в качестве названия для соответствующего поля.</p>
<p>Также создадим какой-нибудь простенький контекст данных <code>PersonContext</code>:</p>
<pre class="brush:c#;">
sing System;
...
<p>Теперь создадим ресурсы. Ресурсы могут содержать не только сообщения об ошибках валидации, но и другие данные, например, заголовки страниц и т.д.</p>
<p>Создадим в проекте папку <i>Resources</i>. В эту папку добавим файл ресурсов <i>Resource.resx</i>. Это будет файл ресурсов по умолчанию, который будет 
использоваться для русского языка:</p>
<img src="mvcpics/16.1.png" alt="Добавление ресурса в проект ASP.NET MVC" />
<p>Перед использованием ресурсов укажем некоторые их настройки для каждого ресурса в окне Свойств (Propertires) внизу под окном проекта:</p>
<img src="mvcpics/16.4.png" alt="Параметры ресурсов в ASP.NET MVC" />
<p>Здесь нам надо установить следующие параметры:</p>
<ul>
<li><p><span class="bb">Build Action</span> - в качестве типа построения выберем значение <span class="bb">Embedded Resource</span></p></li>
<li><p><span class="bb">Custom Tool</span> - в качестве инструмента создания ресурсов введем в поле <span class="bb">PublicResXFileCodeGenerator</span></p></li>
<li><p><span class="bb">Custom Tool Namespace</span> - в качестве пространства имен укажем <span class="bb">Resources</span>. 
...
мы указали <code>ErrorMessageResourceType = typeof(Resources.Resource)</code>, где в качестве пространства имен выступает Resources.</p></li>
</ul>
<p>Теперь перейдем к заполнению самих ресурсов. Нажмем на файл <i>Resource.resx</i> и в открывшуюся таблицу внесем названия ресурсов и их значения:</p>
<img src="mvcpics/16.3.png" alt="Таблица ресурсов" />
<p>Это и есть весь набор используемых ресурсов, и мы можем определить множество разных ресурсов. Подобный набор надо указать и у других ресурсов, 
только указав значения на соответствующем языке. Так, на рисунке указан ресурс <code>NameRequired</code>. При определении модели в атрибуте я указал, 
что именно этот ресурс будет использоваться в качестве сообщения об ошибке валидации.</p>
<p>Кроме того, мы можем в коде представлений обратиться к ресурсам, например: <code>@Resources.Resource.NameRequired</code>. Здесь 
...
использовать и другие ресурсы.</p>
<p>Теперь добавим ресурсы для других культур. Для этого можно скопировать файл Resource.resx и вставить его копии в папку Resources. 
Я сделал две копии файла и переименовал их следующим образом:</p>
<ul>
<li><p>Resource.en.resx : файл ресурсов для английского языка</p></li>
<li><p>Resource.de.resx : файл ресурсов для немецкого языка</p></li>
</ul>
<p>Можно ограничиться и одним, например, только для английского языка.</p>
<p>Теперь структура проекта выглядит так:</p>
<img src="mvcpics/16.2.png" alt="Структура мультиязычного сайта в ASP.NET MVC" />
<h3>Условности при создании ресурсов</h3>
<p>При локализации имеют значение в наименовании ресурсов суффиксы, обозначающие локали. Так, мы можем использовать нейтральные культуры, 
например: <code>Resource.en.resx</code>. Здесь суффикс <code>en</code> обозначает английский язык в целом, все англоязычные культуры. 
Но мы можем конкретизировать культуру, например, задать ресурс только для американской версии английского: <code>Resource.en-US.resx</code>.</p>
<p>Суффикс культуры имеет следующий формат: <code>languagecode-country/regioncode</code>, где <code>languagecode</code> - код языка, а 
<code>country/regioncode</code> - код субкультуры. В моем случае я использую только нейтральные культуры без субкультур.</p>
<p>Откроем файл Resource.en.resx и переведем все ресурсы на английский язык:</p>
<img src="mvcpics/16.7.png" />
<p>С ресурсами мы завершили, теперь перейдем к построению самой логики приложения.</p>
...
<div class="nav"><p><a href="15.2.php">Назад</a><a href="./">Содержание</a><a href="16.2.php">Вперед</a></p></div></div>
<div class="socBlock">
