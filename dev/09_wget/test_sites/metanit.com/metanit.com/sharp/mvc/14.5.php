...
<html >
<head>
<title>Режимы отображения DisplayMode</title>
<meta charset="utf-8" />
<meta name="description" content="Использование функциональности DisplayMode для создания мобильного веб-приложения на asp.net mvc 4, регистрация дополнительных режимов отображения, переключение между мобильным и десктопным видами сайта">
...
<h2>Режимы отображения DisplayMode</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В предыдущем параграфе мы создали мобильное веб-приложение на основе встроенного шаблона. Однако есть еще один способ создания мобильного приложения. 
Допустим, у нас уже есть проект сайта, но только для десктопов, и нам нужно создать его копию, но только для мобильных приложений.</p>
<p>Инфраструктура ASP.NET MVC 4 содержит такую функциональность как режимы отображения или <b>DisplayMode</b>. Например, 
у нас есть некое представление Index.cshtml. Оно было предназначено для десктопов. И мы хотим рядом с ним сделать копию представления, но только 
уже для мобильных устройств. Для этого нам достаточно создать в проекте копию данного представления, добавив к его имени суффикс 
<span class="bb">Mobile</span>. То есть представление, которое ориентировано на мобильные устройства, у нас будет называться Index.<span class="bb">Mobile</span>.cshtml.</p> 
<p>И больше нам ничего не надо менять: контроллеры и их действия остаются теми же. Но теперь этот суффикс Mobile будет показывать 
фреймворку, что представление <i>Index.Mobile.cshtml</i> надо использовать в ответ на запросы, исходящие от мобильных устройств. Во всех остальных 
случаях будет использоваться обычное представление <i>Index.cshtml</i>.</p>
<p>Теперь посмотрим на примере. Возьмем обычный проект asp.net mvc 4 и добавим ко всем его представлениям (в том числе к мастер-странице) двойники 
с суффиксом Mobile:</p>
<img src="mvcpics/14.6.png" />
<p>Теперь, если мы запустим проект на выполнение и обратимся к приложению из обычного браузера Opera (либо другого браузера, предназначенного для персональных компьютеров), то 
мы увидим представление Index.cshtml:</p>
<img src="mvcpics/14.7.png" />
<p>В то же время если мы обратимся к приложению из браузера Opera Mobile, то обращение уже будет идти к представлению Index.Mobile.cshtml:</p>
<img src="mvcpics/14.8.png" />
<h3>Переопределение режима DisplayMode</h3>
<p>Благодаря суффиксу Mobile фреймворк узнавал о том, что страница предназначена для мобильных устройств. Однако этим работа функциональности 
DisplayMode не исчерпывается. Так, мы можем переопределить ее действие. Зачем это надо? К примеру ряд браузеров и устройств могут не поддерживать 
какие-то определенные теги, стили, вследствие этого страницы могут отображаться некорректно.</p>
<p>Так, в настоящее время существует большое число пользователей, которые используют Internet Explorer 8. И мы, допустим, хотим, чтобы наш сайт 
при запросах от IE 8 использовал только определенные представления - как и в случае с мобильными устройствами.</p>
<p>Для этого вначале нам надо зарегистрировать в файле Global.asax в методе Application_Start новый режим отображения для Internet Explorer 8:</p>
<pre class="brush:c#;">
...
<p>Здесь мы смотрим, содержит ли свойство UserAgent у объекта Request "MSIE 8" - то есть был ли запрос послан браузером IE 8. 
В этом случае мы используем новый режим DefaultDisplayMode("IE8"). Параметр "IE8" означает, что наши специфичные представления будут иметь названия типа [имя_представления].IE8.cshtml. 
Собственно также, как и с мобильными приложениями. Поэтому добавим в проект представление Index.IE8.cshtml и сэмулируем запрос от IE 8, например, в IE 10 в режиме совместимости:</p>
<img src="mvcpics/14.9.png" />
<h3>Переключение между мобильным и десктопным вариантом</h3>
<p>Несмотря на то, что автоматическое переключение представлений между мобильной и десктопной версиями очень удобно, все же подобный автоматизм 
может вызвать неудобства. Десктопные сайты предлагают, как правило, пользователю гораздо больше функционала, чем мобильные. И некоторым пользователям, возможно, 
больше нравится десктопная версия даже при обзоре на мобильном устройстве.</p>
<p>Чтобы решить эту проблему, ряд сайтов предлагает переключение режимов отображения - с мобильного на десктопный и обратно. Подобную функциональность 
мы можем использовать и в приложениях asp.net mvc. Для этого вначале добавим в проект с помощью пакетного менеджера NuGet библиотеку <b>jQuery.Mobile.MVC</b>:</p>
<img src="mvcpics/14.10.png" />
<p>Этот пакет добавляет в проект ряд файлов:</p>
<ul>
<li><p>Во-первых, добавляется библиотека <code>jquery.mobile.js</code> и ее минимизированная версия, а также используемые этой библиотекой файлы стилей css.</p></li>
<li><p>Во-вторых, добавляется контроллер <code>ViewSwitcherController</code>, который переключает режимы отображения.</p></li>
<li><p>В-третьих, добавляется мастер-страница <code>_Layout.Mobile.cshtml</code> и файл стилей <code>Site.Mobile.css</code> для мобильного вида (при их отсутствии).</p></li>
<li><p>А также добавляется частичное представление <code>_ViewSwitcher</code> с разметкой переключения режимов.</p></li>
</ul>
<p>Кроме того, в каталог App_Start проекта добавляется файл <i>BundleMobileConfig.cs</i>, который содержит объявление мобильного бандла, включающего библиотеку jquery.mobile и все сопутствующие 
файлы стилей. Если мы в будущем собираемся использовать этот бандл, то нам надо зарегистрировать его в файле <i>Global.asax</i> в методе App_Start:</p>
<pre class="brush:c#;">
protected void Application_Start()
{
...
<p>Теперь, чтобы задействовать функционал переключения, добавим в мастер-страницы layout (и в обычную, и в мобильную версии) вызов 
частичного представления <code>_ViewSwitcher</code> (естественно мы можем локализовать представление _ViewSwitcher.cshtml, настроив нужным нам образом отображения переключения режимов):</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>После этого, обратившись к приложению на мобильном устройстве, нам будет доступна функция переключения режимов отображения:</p>
<img src="mvcpics/14.11.png" />
<p>Таким образом, в приложении ASP.NET MVC 4 мы легко можно связать обычную десктопную версию вею-приложения с мобильной и при необходимости осуществлять переключение 
между ними.</p>
...
<div class="nav"><p><a href="14.4.php">Назад</a><a href="./">Содержание</a><a href="15.1.php">Вперед</a></p></div></div>
<div class="socBlock">
