...
<html >
<head>
<title>Основы валидации</title>
<meta charset="utf-8" />
<meta name="description" content="Элементы валидации на стороне сервера и на стороне клиента">
...
<h2>Основы валидации</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Итак, чтобы посмотреть, что из себя представляет валидация, создадим проект по шаблону Basic Application.</p>
<p>Добавим в наше приложение новую модель Book, которую частично мы разрабатывали ранее в этой главе:</p>
<pre class="brush:c#;">
    public class Book
    {
...
<p>Обратите внимание, что по сравнению с предыдущей версией данной модели, мы добавили атрибут <span class="bb">Required</span>. Теперь создадим контроллер, который 
будет управлять объектами данной модели. Сделаем контроллер (пусть контроллер называется допустим <span class="ii">BookController</span>) типизированным: в качестве шаблона выберем 
<span class="ii">MVC controller with read/write actions and views, using Entity Framework</span>, а в качестве класса модели укажем нашу модель Book:</p>
<img src="mvcpics/7.6.png" />
<p>В итоге у нас будет по умолчанию создан набор представлений по управлению объектами модели. Запустим приложение и обратимся к действию Create 
нашего контроллера (запрос <i>Book/Create</i>), которое должно добавлять новую запись в БД. Не заполняя поля, мы можем сразу нажать на кнопку отправки формы:</p>
<img src="mvcpics/7.7.png" />
...
<ol>
	<li><p>Использование атрибутов валидации при объявлении модели. Мы указали для каждого свойства атрибут <code>Required</code>, 
	благодаря чему фреймворк знает, что данное свойство обязательно должно содержать некоторое значение.</p></li>
	<li><p>Использование хелперов валидации. При каждом свойстве мы используем хелпер валидации <code>Html.ValidationMessageFor</code>:</p>
	<pre class="brush:xml;">
		...
		<p>Благодаря чему и отображается сообщение об ошибке.</p></li>
		<li><p>Чтобы настроить стиль отображения на стороне клиента, в файле Site.css указан стиль для соответствующих классов:</p>
		<pre class="brush:xml;">
	...
	<li><p>Валидация на стороне сервера. Эта была валидация на клиентской стороне. Однако если у вас отключен javascript а браузере, то 
	вы не увидите буковок с ошибками, а форма благополучно отправится на сервер. Но во фреймворке предусмотрена также валидация на стороне 
	сервера:</p>
	<pre class="brush:c#;">
        [HttpPost]
...
<p>С помощью свойства <strong>ModelState.IsValid</strong> мы узнаем, проходит модель валидацию или нет, и зависимости от результата совершаем те или 
иные действия.</p></li>	
</ol>
<p>Для просто демонстрации выбрано приложение по шаблону Basic, однако если вы работаете с проектом приложения по шаблону Empty, то вам придется 
добавлять все эти файлы jquery, код css и прочее, чтобы создать механизм валидации.</p>
<p>Таков в общем механизм валидации приложения по умолчанию. Теперь более подробно поговорим об основных моментах валидации. Начнем с атрибутов.</p>
