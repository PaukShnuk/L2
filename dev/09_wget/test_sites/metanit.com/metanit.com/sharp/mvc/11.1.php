...
<html >
<head>
<title>Авторизация и аутентификация в MVC 4</title>
<meta charset="utf-8" />
<meta name="description" content="Основные моменты авторизации и аутентификации в ASP,NET MVC, а также понятия членства, ролей, профили, провайдеров членства">
...
<h1>Авторизация и аутентификация в MVC 4</h1>
<h2>Введение в авторизацию и аутентификацию</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Большую роль в веб-приложениях играют механизмы авторизации и аутентификации. Они позволяют разграничить доступ для различных групп пользователей, 
а также идентифицировать пользователей.</p>
<p>Аутентификация - это процесс идентицификации пользователя, то есть грубо говоря мы узнаем, что за пользователь посетил веб-приложение. 
А авторизация уже представляет процесс определения прав, которые могут быть даны аутентифицированному пользователю, его возможностей по 
доступу к ресурсам веб-приложения.</p>
<p>В главе, посвященной фильтрам, мы уже разбирали один момент, связанный с авторизацией - фильтр авторизации. Он используется в виде атрибута 
<code>Authorize</code>, который устанавливается либо для одиночного действия контроллера, либо для всего контроллера.</p>
<pre class="brush:c#;">
[Authorize]
...
<p>И после установки атрибута к данному ресурсу получат доступ только те пользователи, которые успешно осуществили вход в веб-приложение.</p>
<p>Мы также можем разрешить неавторизованным пользователям доступ к ресурсам, пометив их атрибутом <code>AllowAnonymous</code>.</p>
<p>Авторизация в MVC опирается на три ключевых области, которые помогают управлять пользователями в системе. Это членство (membership), 
роли и профили.</p>
<P>Членство представляет управление регистрационными данными пользователей, которые затем используются для авторизации и аутентификации.</P>
<p>Роли - группы пользователей, которые используются для авторизации и разделения доступа к ресурсам.</p>
<p>Профили представляют различную информацию о пользователях, как например, имя, фамилию, рост, возраст и т.д.</p>
<p>Управление пользователями, членством и ролям производится с помощью провайдеров членства и ролей. В MVC 4 можно использовать различные провайдеры: 
как в виде API SimpleMembershipProvider, так и в виде универсальных провайдеров. При необходимости гибкая среда ASP.NET MVC позволяет создавать 
и использовать свои провайдеры членства/ролей и профилей со своей уникальной логикой.</p>
...
<div class="nav"><p><a href="10.9.php">Назад</a><a href="./">Содержание</a><a href="11.2.php">Вперед</a></p></div></div>
<div class="socBlock">
