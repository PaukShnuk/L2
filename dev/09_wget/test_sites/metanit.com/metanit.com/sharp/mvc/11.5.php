...
<html >
<head>
<title>Настройка использования SimpleMembershipProvider</title>
<meta charset="utf-8" />
<meta name="description" content="Настройка использования SimpleMembershipProvider, определение собственной базы данных и таблицы профилей, создание инфраструктуры для авторизации">
...
<h2>Настройка использования SimpleMembershipProvider</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>SimpleMembershipProvider является довольно гибким классом и его использовать довольно легко. Так мы можем переопределить таблицу пользователей, 
которая по умолчанию называется <i>UserProfile</i> и ее определение. 
Например, мы можем создать в базе данных таблицу и использовать ее в качестве хранилища данных пользователей, потому что 
стандартная таблица хранит только логины и id пользователей, а нам захотелось ее расширить: например, чтобы пользователь в качестве логина 
использовал email, а также добавить в таблицу ряд дополнительных столбцов.</p> 
<p>Поэтому добавим в папку <i>App_Data</i> новую базу данных, назовем ее к примеру StoreDB и соответственно изменим строку подключения 
к этой бд в файле web.config, чтобы бд использовалась по умолчанию:</p>
<pre class="brush:xml;">
&lt;connectionStrings&gt;
...
<p>Добавим в базу данных таблицу <i>Users</i> и создадим для нее следующее определение полей:</p>
<img src="mvcpics/11.7.png" />
<p>Далее перейдем в файл <i>InitializeSimpleMembershipAttribute.cs</i> и изменим строку инициализации подключения к БД в соответствии с 
определением нашей таблицы</p>
<pre class="brush:c#;">
WebSecurity.InitializeDatabaseConnection("DefaultConnection", "Users", "Id", "Email", autoCreateTables: true);
</pre>
<p>Поскольку наша таблица называется Users, идентификатор пользователя содержится в столбце Id, а его логин в столбце Email.</p>
<p>Теперь нам надо изменить используемые по умолчанию модели, чтобы они соответствовали определению столбцов таблицы пользователей. 
Поэтому изменим следующие классы в файле <i>AccoutModels.cs</i>. Класс контекста:</p>
<pre class="brush:c#;">
...
<p>Класс пользователей User, который содержит данные об отдельном пользователе:</p>
<pre class="brush:c#;">
[Table("Users")]
public class User
{
...
<p>Классы моделей регистрации и логина:</p>
<pre class="brush:c#;">
...
<p>Теперь изменим код контроллера AccountController, чтобы он регистрировал нового пользователя должным образом:</p>
<pre class="brush:c#;">
[AllowAnonymous]
...
<p>Если в случае с действием Login  мы по сути просто меняем имя свойства, которое определяет логин, то в действии Register мы 
также добавляем в БД переданные данные о пользователе через анонимный объект: 
<code>WebSecurity.CreateUserAndAccount(model.Email, model.Password, new { FirstName=model.FirstName, LastName=model.LastName, Year=model.Year});</code></p>
<p>И последнее, что осталось изменить - это представления логина и регистрации. Представление логина:</p>
<pre class="brush:xml;">
...
<p>И представление регистрации:</p>
<pre class="brush:xml;">
...
<p>После запуска и регистрации пользователя со всеми данными он окажется в базе данных.</p>
