...
<h2>AJAX-ссылки</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Другим часто используемым AJAX-хелпером является <strong>Ajax.ActionLink</strong>. Он во многом похож на хелпер Ajax.BeginForm за тем исключением, что генерирует специальные 
ajax-ссылки. Посмотрим на примере. Добавим в представление следующую разметку:</p>
<pre class="brush:xml;">
&lt;div id="bestbook"&gt;
    @Ajax.ActionLink("Лучшая книга", "BestBook", new AjaxOptions{ UpdateTargetId="bestbook"})
&lt;/div&gt;
</pre>
<p>В данном случае мы также используем объект AjaxOptions для установки параметров асинхронной ссылки, который принимает все те же 
параметры, что и в случае с ajax-формой.</p>
<p>Мы помещаем ссылку в элемент с id, который указывается в параметре <code>UpdateTargetId</code>. Таким образом, результат запроса будет замещать ссылку. 
Но для работы нам также необходимо, как и в случае с ajax-формой, действие контроллера и вызываемое им частичное представление.</p>
<p>Метод контроллера, в данном случае <code>BestBook</code>, будет просто возвращать первую книгу:</p>
<pre class="brush:c#;">
...
<p>А вызываемое им частичное представление <span class="ii">BestBook.cshtml</span> будет выводить информацию об этой книге:</p>
<pre class="brush:xml;">
@model AjaxMvcApplication.Models.Book
&lt;h3&gt;Лучшая книга&lt;/h3&gt;
@Html.DisplayForModel()
</pre>
<p>AJAX-ссылка будет отображена при запуске как обычная ссылка:</p>
<img src="mvcpics/10.11.png" />
<p>А после нажатия она будет заменена полученным от сервера контентом:</p>
<img src="mvcpics/10.12.png" />
<p>В качестве примера можно также изменить код представления из предыдущего раздела про ajax-формы, применив ajax-ссылки:</p>
<pre class="brush:xml;">
...
<p>В итоге все авторы, передаваемые в представление через ViewBag, выводятся в виде ссылок внизу страницы. А нажатие на одну из этих ссылок 
будет иметь то же действие, что и поиск по кнопке:</p>
<img src="mvcpics/10.13.png" />
