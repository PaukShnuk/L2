...
<p>Модель необязательно состоит только из свойств, кроме того, она может иметь конструктор, какие-нибудь вспомогательные метода. Но главное не перегружать 
...
<p>Данные моделей хранятся в базе данных. Чтобы взаимодействовать с базой данных, очень удобно пользоваться фреймворком <strong>Entity Framework</strong>. 
...
<p>Выпуск ASP.NET MVC 4 уже включает Entity Framework 5.0, однако в проектах по типу Empty вам придется подключать фреймворк через пакетный менеджер NuGet.</p>
<p>Чтобы подключиться к базе данных через Entity Framework, нам нужен <strong>контекст данных</strong>. Контекст данных представляет собой класс, производный от 
...
данных. Допустим, создадим контекст данных для работы с вышеприведенными моделями Book и Purchase:</p>
<pre class="brush:c#;">
using System;
