...
<html >
<head>
<title>HttpContext. Контекст запроса. Куки</title>
<meta charset="utf-8" />
...
<h2>HttpContext. Контекст запроса. Куки</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Те, кто работал с php, вероятно, знают такой объект как <code>$_SERVER</code> и ряд переменных, с помощью которых можно получить данные о запросе: 
о браузере, об ip-адресе пользователя и еще ряд других. ASP.NET MVC также предоставляет подобную функциональность, инкапсулированную в объекте 
<b>HttpContext</b>.</p>
<p>Возможно, вас может смутить тот факт, что также в контроллере вы можете обратиться к объекту <code>ControllerContext</code>, который имеет свойство 
HttpContext и по сути предоставляет доступ к той же функциональности. В то же время между ними есть некоторые различия. 
...
http-запроса по отношению к данному контроллеру.</p>
<p>Информация о запросе содержится в свойстве <code>Request</code>. <b>HttpContext.Request</b> представляет объект класса, 
унаследованного от <code>HttpRequestBase</code>, и поэтому содержит все его свойства. Рассмотрим некоторые из них.</p>
<ul>
...
Поскольку реферер может быть не определен, то сначала смотрим, не равен ли он null</p></li>
</ul>
<p>Например:</p>
<pre class="brush:c#;">
public string Index()
...
<p>HttpContext.Request имеет еще ряд свойств, которые можно посмотреть в документации.</p>
<h3>Работа с куки</h3>
...
<p>Однако прежде чем получать значения куки, их естественно надо установить. Для этого нам служит другое свойство - свойство 
<code>HttpContext.Response</code>. Например, установим в куки значение "id":</p> 
<pre class="brush:c#;">HttpContext.Response.Cookies["id"].Value = "ca-4353w";</pre>
<h3>Отправка ответа</h3>
<p>Свойство <code>HttpContext.Response</code> содержит объект <code>HttpResponse</code>, который передает на сторону клиента некоторые значения. 
Например, куки, как было описано выше. Также мы можем передать с помощью этого свойства другие объекты, например, кодировку ответа: 
<code>HttpContext.Response.Charset = "iso-8859-2";</code></p>
<p>Мы также можем управлять ответом с помощью методов. К примеру метод <code>AddHeader</code> позволяет добавить к ответу дополнительный заголовок.</p>
...
проще использовать методы, создающие объекты ActionResult, например, представления.</p>
<h3>Определение пользователя</h3>
<p>Также объект HttpContext позволяет определить пользователя:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="3.9.php">Назад</a><a href="./">Содержание</a><a href="3.6.php">Вперед</a></p></div></div>
<div class="socBlock">
