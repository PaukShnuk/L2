...
<html >
<head>
<title>Стилизация приложения и мастер-страницы в MVC 4</title>
<meta charset="utf-8" />
<meta name="description" content="Пример стилизации приложения asp net mvc 4 с помощью стилей css">
...
<h2>Стилизация приложения</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Теперь в нашем приложении не хватает одной детали - стилизации. Сделаем какую-нибудь простенькую стилизацию. Для этого нам надо определить файл стилей для 
нашего приложения. Все файлы стилей принято хранить в папке Content, поэтому вначале добавим в проект папку Content. Для этого нажмем на название проекта 
правой кнопкой мыши и в появившемся меню выберем <span class="bb">Add (Добавить)-&gt;New Folder (Новая папка)</span>.</p>
<p>Теперь в новую папку добавим файл стилей. Назовем его Site.css. Для этого нажмем правой кнопкой мыши на папку Content и  в появившемся меню 
выберем <span class="bb">Add (Добавить)-&gt;New Item (Новый элемент)</span>. Затем в списке шаблонов найдем шаблон <span class="bb">Style Sheet</span> и дадим новому файлу имя Site.css.</p>
<p>Теперь добавим в файл Site.css следующее содержание:</p>
<pre class="brush:css;">
body {
    font-size: 13px;
...
<p>Класс <code>.menu</code> в данном случае будет служить в качестве класса для навигационного меню на сайте.</p>
<p>Теперь мы можем использовать стили. Поскольку наши представления по сути обычные веб-страницы, мы можем как и на любой странице html добавить в 
секции head ссылки на файл стилей:</p>
<pre class="brush:xml;">
&lt;head&gt;
    &lt;meta name="viewport" content="width=device-width" /&gt;
...
<p>Если в приложении одно представление, или если разные представления используют разные стили и другие ресурсы, то такой подход оправдан. 
Однако в нашем случае у нас есть два представления, у которых должно быть одно меню и которые должны быть одинаково стилизованы. В этом случае очень удобно 
использовать мастер-страницы. Мастер-страница задает некий единый шаблон для других использующих его представлений.</p>
<p>Итак, создадим мастер-страницу. По сути это простое представление. Вначале добавим в папку Views проекта новую папку Shared. После 
этого в папку Shared добавим новое представление. Назовем его <span class="bb">_Layout.cshtml</span>:</p>
<img src="mvcpics/2.12.png"  />
<p>Добавим в файл _Layout.cshtml следующий текст:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь мы использовали другой способ подключения стилей - мы использовали метод Url.Content, в который передали путь к файлу. Оба способа - и этот, и 
использованный ранее равноправны.</p>
<p>Затем мы создали меню сайта с помощью методов Html.ActionLink, который генерирует элемент-ссылку и принимает название ссылки, метод контроллера и имя контроллера. 
В конце идет метод <span class="bb">RenderBody()</span>, при вызове ресурса в эту часть мастер-страницы и будут подставляться содержимое остальных представлений.</p>

<p>Теперь адаптируем представления к использованию их мастер-страницей. Изменим представление Index.cshtml следующим образом:</p>
<pre class="brush:xml;">
@{
...
<p>Сущность изменений состоит в том, что в секции <code>Layout = "~/Views/Shared/_Layout.cshtml"; </code> мы указываем путь к мастер-странице. 
После этого нам не нужны секции head и body. Запустив приложение, мы увидим, что теперь к нашему сайту применена стилизация:</p>
<img src="mvcpics/2.13.png" alt="Стилизация в ASP NET MVC 4" />
