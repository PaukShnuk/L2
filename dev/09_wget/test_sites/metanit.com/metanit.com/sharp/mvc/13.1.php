...
<html >
<head>
<title>Публикация приложения</title>
<meta charset="utf-8" />
<meta name="description" content="Публикация веб-приложения на сервере, основные части пакета приложения, возможность развертывания сайта на веб-серверах IIS и nginx и на облачной платформе Azure">
...
<h1>Публикация приложения</h1>
<h2>Среда публикации и составные части приложения</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>После того, как приложение написано и протестировано локально, его можно опубликовать, чтобы оно стало доступно широким массам.</p>
<p>Для начала нужно выбрать среду размещения. Мы можем выбрать одну из следующих возможностей:</p>
<ul>
<li><p><span class="bb">Веб-сервер IIS</span></p></li>
<li><p><span class="bb">Веб-сервер nginx</span></p></li>
<li><p><span class="bb">Облачная платформа Windows Azure</span></p></li>
</ul>
<p>В зависимости от выбранной среды будут различаться условия конфигурации приложения и его администрирования.</p>
<p>Теперь рассмотрим основные составные части публикуемого сайта, из чего непосредственно будет состоять работающее веб-приложение.</p>
<h3>Статические ресурсы</h3>
<p>Статические ресурсы в данном случае - это файлы стилей css, скрипты JavaScript, файлы изображений. В стандартной структуре проекта ASP.NET 
MVC эти файлы располагаются в каталогах <span class="ii">Content</span>, <span class="ii">Scripts</span> и <span class="ii">Images</span> 
соответственно. Эти файлы не компилируются в динамические сборки, поэтому являются статическими ресурсами и переносятся на сайт при публикации 
как есть.</p>
<h3>Сборки приложения</h3>
<p>Сборки приложения представляют по сути ключевую часть приложения - именно в них компилируются наши классы и контроллеры. 
При компиляции сборки приложения попадают в проекте в папку <i>bin</i>, где их собственно можно найти. Так, если приложение называется BookStore, 
то в после компиляции в папке bin окажется файл <i>BookStore.dll</i>, который и будет представлять ключевой файл приложения.</p>
<p>При развертывании приложения важно, чтобы оно имело доступ к библиотекам фреймворка ASP.NET MVC. Для этого надо либо установить на сервере 
данный фреймворк, либо включить все нужные библиотеки фреймворка в пакет приложения. В последнем случае все нужные нам библиотеки также 
оказываются в папке bin.</p>
<p>В Visual Studio 2012 (в том числе в Express-версии) копирование библиотек в папку bin происходит 
автоматически). Поэтому все динамические библиотеки фреймворка вы уже сможете увидеть в папке bin после компиляции.</p>
<p>А в Visual Studio 2010 необходимо воспользоваться специальной опцией <strong>Add Deployable Dependencies...</strong>. Для этого нажмем 
в структуре проекта на название проекта правой кнопкой мыши и в появившемся меню выберем <span class="bb">Add Deployable Dependencies...</span>.</p>
<img src="mvcpics/13.1.png" />
<p>После этого в появившемся окне выберем <span class="bb">ASP.NET MVC</span> и нажмем ОК. После этого все необходимые библиотеки фреймворка 
будут добавлены в папку bin.</p>
<img src="mvcpics/13.2.png" />
<h3>Файлы Global.asax и Web.config</h3>
<p>Файлы, содержащие настройки приложения. При развертывании важно учитывать, что некоторые настройки могут меняться в зависимости от сервера. 
Так, при публикации, возможно, придется менять строку подключения к БД.</p>
<h3>Представления</h3>
<p>Представления, которые содержатся в каталоге Views, включается в пакет приложения как есть, в отличие от основной сборки приложения. 
Затем уже в процессе обращения к приложению в ходе JIT-компиляции представления компилируются и отдаются пользователю в ответ на запрос.</p>
<p>В итоге если у нас такая структура проекта в Visual Studio:</p>
<img src="mvcpics/13.6.png" />
<p>То при публикации (например, в файловой системе) у нас получается примерно такая структура:</p>
<img src="mvcpics/13.7.png" />
...
<div class="nav"><p><a href="12.3.php">Назад</a><a href="./">Содержание</a><a href="13.2.php">Вперед</a></p></div></div>
<div class="socBlock">
