...
<h1>Фильтры</h1>
<h2>Фильтры. Введение</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
так и к отдельным его методам, свойствам и полям. В качестве примера вы можете создать новый проект 
приложения по шаблону Internet Application - в этом случае в папке Contollers у вас по умолчанию будет создан контроллер <strong>AccountController</strong>:</p>
<pre class="brush:c#;">
   [Authorize]
...
<p>В данном случае <strong>атрибут [Authorize]</strong> как раз и будет представлять фильтр авторизации. Если бы мы не применили 
...
<p>В MVC реализовано четыре типа фильтров:</p>
<table class="tab">
	...
	<tr><td><p>Фильтры авторизации</p></td><td><p>IAuthorizationFilter</p></td><td><p>AuthorizationAttribute</p></td><td><p>Фильтр, определяющий, аутентифицирован ли клиент и имеет ли он доступ к данному ресурсу. 
	Данный фильтр запускается до выполнения любого другого фильтра или метода действий</p></td></tr>
	...
	<tr><td><p>Фильтры результатов действий</p></td><td><p>IResultFilter</p></td><td><p>ResultFilterAttribute</p></td><td><p>Фильтр, применяемый к результатам действий. Может запускаться как до, так и после выполнения результата действия</p></td></tr>
...
<p>Теперь разберем первый фильтр - фильтр авторизации.</p>
