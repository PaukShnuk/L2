...
<html >
<head>
<title>Создание ограничений для маршрутов</title>
<meta charset="utf-8" />
<meta name="description" content="Создание ограничений маршрутов через интерфейс IRouteConstraint и игнорирование маршрутов с помощью метода IgnoreRoute">
...
<h2>Создание ограничений для маршрутов</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Иногда возникает необходимость, более точно задать совпадение строки запроса для данного маршрута. Например, нам надо задать, чтобы имя 
контроллера обязательно начиналось с буквы "H". Тогда нам надо определить соответствующее ограничение для имени с помощью регулярных выражений:</p>
<pre class="brush:c#;">
routes.MapRoute(
    name: "Default",
...
<p>С помощью параметра <code>constraints</code> мы задаем ограничения маршрута. И если мы направим приложению запрос <span class="ii">Book/Index</span>, 
...
а другого маршрута, которому бы соответствовал запрос <span class="ii">Book/Index</span>, у нас не задано.</p>
<p>Подобным образом мы можем задать ограничения и для других параметров. 
Например, пусть параметр <code>id</code> состоит как минимум из двух цифр:</p>
<pre class="brush:c#;">
routes.MapRoute(
    name: "Default",
...
<p>Теперь даже запрос <span class="ii">Home/Index/1</span>  у нас не будет сопоставляться с маршрутом, так как параметр id состоит из одной цифры, а не из двух.</p>
<p>Также мы можем задать ограничение по типу метода. Например, мы хотим, чтобы обрабатывались только запросы для методов с типом GET:</p>
<pre class="brush:c#;">
routes.MapRoute(
    name: "Default",
    url: "{controller}/{action}/{id}/{*catchall}",
...
<p>Теперь будут обрабатываться только те запросы, которые представляют запросы типа GET.</p>
<h3>Создание собственных ограничений</h3>
<p>Чтобы создать собственное ограничение, нужно реализовать интерфейс <code>IRouteConstraint</code> с одним единственным методом <code>Match</code>, который имеет следующее 
определение:</p>
<pre class="brush:c#;">public interface IRouteConstraint
{
  bool Match(HttpContextBase httpContext, Route route, string parameterName,   
    RouteValueDictionary values, RouteDirection routeDirection);
}
</pre>
<p>Ограничение маршрута применяет этот интерфейс IRouteConstraint, что вынуждает затем движок маршрутизации вызвать для ограничения маршрута метод 
IRouteConstraint.Match, чтобы определить, применяется ли данное ограничение к данному запросу или нет. Например, создадим ограничение, которое не будет 
пропускать запросу по некоторому url. Итак, добавим в приложение следующий класс:</p>
<pre class="brush:c#;">
...
<p>Здесь мы говорим, что если запрашиваемый ресурс совпадает со значением свойства <code>httpContext.Request.Url.AbsolutePath</code>, 
то запрос не будет сопоставляться с маршрутом. Тогда определение маршрута может выглядеть следующим образом:</p>
<pre class="brush:c#;">
routes.MapRoute(
    name: "Default",
    url: "{controller}/{action}/{id}/{*catchall}",
...
<p>Теперь запрос <span class="ii">/Home/Index/12</span> не будет обрабатываться, даже если он удовлетворяет всем остальным условиям и ограничениям.</p>
<h3>Игнорирование запросов</h3>
<p>По умолчанию в методе <code>RegisterRoutes</code> класса <code>RouteConfig</code> определена такая строка <code>routes.IgnoreRoute("{resource}.axd/{*pathInfo}");</code>. 
...
где использовали ограничение для маршрута в виде запрета пути /Home/Index/12, следующим образом:</p>
<pre class="brush:c#;">
...
<p>Теперь также запрос <span class="ii">Home/Index/12</span> не будет обрабатываться и сопоставляться с определенным ресурсом на сервере.</p>
