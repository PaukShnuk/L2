...
<html >
<head>
<title>Добавление базы данных на хостинг somee.com</title>
<meta charset="utf-8" />
<meta name="description" content="Создание сайта и базы данных на хостинге somee.com, перенос данных с помощью скрипта из локальной базы данных на сервер">
...
<h2>Добавление базы данных на хостинг somee.com</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Теперь развернем наш сайт на хостинге. В начале добавим на хостинг базу данных, если она используется. Для этого в панели аккаунта 
слева находим узел Databases и нажимаем на него. После этого переходим в правой части к созданию новой базы данных. 
Вводим название для базы данных и выбираем версию SQL-Servera. И после этого нажимаем на кнопку <span class="bb">Create empty database</span></p>
<img src="mvcpics/13.20.png" alt="Создание базы данных на somee.com" />
<p>Затем нас известят, что создание пустой базы данных прошло успешно, и далее мы можем наполнять ее. Создадим на сервере таблицы по 
тем таблицам, которые у нас в БД. Во-первых, откроем в Visual Studio определение таблицы бд:</p>
<img src="mvcpics/13.27.png" />
<p>Под визуальным отображением определения таблицы мы увидим скрипт создания таблицы. Например, в моем случае он выглядит так:</p>
<pre>
...
<p>Теперь найдем на панели учетной записи на somee.com в левом меню пункт <span class="bb">New SQL Query</span>. Нажмем на него и нам 
откроется текстовое поле для sql-команды. Скопируем в него скрипт таблицы и нажмем справа на кнопку <span class="bb">Run Single Query</span></p>
<img src="mvcpics/13.28.png" />
<p>После этого будет сгенерирована таблица. Затем, если мы хотим перенести на сервер также и данные из таблицы, то отобразим в 
Visual Studio данные таблицы и по ним сгенерируем скрипт, нажав повыше таблицы на значок Script:</p>
<img src="mvcpics/13.25.png" alt="Генерация скрипта добавления данных в бд" />
<p>После этого в отдельном окне в Visual Studio будет сгенерирован скрипт добавления данных, который мы также запустим на выполнения с помощью <span class="bb">Run Single Query</span>. 
Так, в моем случае скрипт данных будет следующим):</p>
<pre>
...
<p>Если таблиц несколько, то повторим данную последовательность действий для всех таблиц.</p>
