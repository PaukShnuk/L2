...
<html >
<head>
<title>Работа с SignalR</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в работу с SignalR, для чего предназначена библиотека, поддерживаемые платформы, модели взимодействия клиента и сервера, способы передачи данных">
...
<h1>Работа с SignalR</h1>
<h2>Введение в SignalR</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В последнее время в веб-среде все чаще создаются веб-приложения, использующие технологии коммуникаций в реальном времени: это и простые чаты, и более сложные многопользовательские 
видеоконференции. Такие приложения могут использовать различные приемы работы: технологию Web Socket, опросы long polling и т.д. Для 
упрощения работы с коммуникациями реального времени была создана специальная библиотека под названием <b>SignalR</b>.</p>
<p>При этом SignalR не является библиотекой, предназначенной только для использования в ASP.NET MVC. Ее также можно использовать и в веб-формах, а также 
в консольных приложениях, десктопных приложения и в WPF.</p>
<p>SignalR предоставляет простой API для создания функционала, который позволяет вызывать функции JavaScript на стороне клиента из 
серверного кода, написанного с помощью языков платформы .NET. SignalR значительно упрощает работу с коммуникациями реального времени. Библиотека 
обрабатывает все подключения и автоматически рассылает сообщения всем подключенным клиентам, либо каким-нибудь специфическим клиентам.</p>
<p>Фактически библиотека SignalR состоит из API серверной стороны, который применяется в коде на C#, и из клиентских библиотек JavaScript.</p>
<p>SignalR предоставляет разработчикам две модели: постоянные подключения (<b>Persistent Connection</b>) и хабы (<b>Hubs</b>).</p>
<p>Постоянные подключения (Persistent Connection API) представляют разработчикам прямой доступ к низкоуровневому протоколу коммуникации. 
Подключения в этой модели представляют конечную точку, к которой подключаются клиенты, наподобие модели подключений в WCF.</p>
<p>Хабы же предоставляют протокол взаимодействия более высокого уровня. Они представляют верхний слой над Persistent Connection API  и 
позволяют клиенту и серверу напрямую вызывать методы друг друга.</p>
<p>Если вы будете работать с SignalR, то скорее всего будете преимущественно использовать хабы, как более удобные инструменты.</p>
<h4>Поддерживаемые платформы</h4>
<p>SignalR поддерживается наиболее распространенными серверными платформами на базе ОС Windows:</p>
<ul>
<li><p><span class="bb">Windows Server 2012</span></p></li>
<li><p><span class="bb">Windows Server 2008 r2</span></p></li>
<li><p><span class="bb">Windows 8</span></p></li>
<li><p><span class="bb">Windows 7</span></p></li>
<li><p><span class="bb">Windows Azure</span></p></li>
</ul>
<p>Для работы на стороне сервера также необходима версия фреймворка .net от 4.0 и выше.</p>
<p>Поддерживаемые клиентские платформы (браузеры) тоже многообразны:</p>
<ul>
<li><p>Microsoft Internet Explorer 8, 9 и 10. Версии Modern, Desktop и Mobile</p></li>
<li><p>Mozilla Firefox: как на ОС Windows, так и на Mac ОС</p></li>
<li><p>Google Chrome: как на ОС Windows, так и на Mac ОС</p></li>
<li><p>Safari:  как на ОС Windows, так и на Mac ОС</p></li>
<li><p>Opera: ОС Windows</p></li>
<li><p>Android-браузер</p></li>
</ul>
<p>Также следует учитывать, что браузер должен поддерживать библиотеку jQuery 1.6.4 или более поздние версии (например, 1.7.2, 1.8.2, 1.9.1 и т.д.). </p>
<h4>Транспорт передачи данных</h4>
<p>Для обмена данными между клиентом и сервером SignalR использует тот способ передачи или тот транспорт, который наиболее подходит к данной ситуации. 
Однако разработчики могут переопределить способ передачи. SignalR предоставляет следующие типы технологий для взаимодействия сервера и клиента:</p>
<ul>
<li><p><span class="bb">WebSockets</span></p></li>
<li><p><span class="bb">Server-sent events</span></p></li>
<li><p><span class="bb">Forever Frames</span></p></li>
<li><p><span class="bb">Long polling</span></p></li>
</ul>
<h4>Выбор транспорта</h4>
<p>При создании подключения SignalR выбирает, если доступно, технологию WebSocket, так как это наиболее оптимальная технология для SignalR, наиболее 
эффективно использующая память сервера. В то же время WebSocket может использоваться только на серверах под управлением Windows Server 2012 или Windows 8 
и при наличии установленного .NET Framework 4.5. При этом также технология WebSocket должна поддерживаться и браузером клиента. И если технология 
WebSocket недоступна на сервере или клиенте, то выбирается другой транспорт.</p>
<p>Если WebSocket, то SignalR использует технологию Server Sent Events, при ее поддержке сервером и клиентом.</p>
<p>Если технология Server Sent Events также недоступна, то используются скрытые фреймы - Forever Frames.</p>
<p>И если Forever Frames также недоступны, то используется Long Polling. Например, если на стороне клиента используется браузер IE 8 и ниже, 
то используется Long Polling.</p>
<p>Теперь рассмотрим работу с библиотекой на конкретных примерах.</p>
...
<div class="nav"><p><a href="16.2.php">Назад</a><a href="./">Содержание</a><a href="17.2.php">Вперед</a></p></div></div>
<div class="socBlock">
