...
<html >
<head>
<title>Валидация модели в контроллере</title>
<meta charset="utf-8" />
<meta name="description" content="Валидация на стороне сервера с помощью проверки значения свойства ModelState.IsValid">
...
<h2>Валидация модели в контроллере</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Кроме валидации на стороне клиента, мы можем осуществлять валидацию и внутри контроллера. Делается это с помощью проверки значения свойства <strong>ModelState.IsValid</strong>.</p> 
<p>Объект ModelState сохраняет все значения, которые пользователь ввел для свойств модели, а также все ошибки, 
связанные с каждым свойством и с моделью в целом. Если в объекте ModelState имеются какие-нибудь ошибки, то свойство ModelState.IsValid возвратит false:</p>
<pre class="brush:c#;">
[HttpPost]
...
<p>Допустим, значение для свойства Name у нас не указано, и мы отсылаем значения модели на сервер. Поскольку к свойству применяется атрибут 
Required, то после привязки модели при валидации мы получим следующие значения:</p>
<pre class="brush:c#;">
...
<p>В данном случае метод <code>ModelState.AddModelError</code> добавляет для свойства, указанного в качестве первого параметра (в данном случае 
<code>Name</code>) ошибку "Недопустимая длина строки". При использовании хелперов впоследствии мы можем вывести данное сообщение об ошибке:</p>
<pre class="brush:xml;">
...
<img src="mvcpics/7.12.png" />
<p>Кроме ошибок для конкретного свойства мы можем указывать ошибки на уровне модели. Выше мы видели, что у нас одно свойство имеет некорректное значение. 
Ошибки же на уровне модели связывают несколько свойств. Например,</p>
<pre class="brush:c#;">
...
<p>Оба свойства могут быть корректными в других ситуациях, но не вместе. Понятно, что Лев Толстой не мог написать, скажем, книги после своей смерти. 
Поэтому мы добавляем ошибку на уровне модели и в качестве первого параметра опускаем название свойства и указываем просто кавычки.</p>
