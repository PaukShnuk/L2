...
<html >
<head>
<title>Генерация исходящих адресов URL</title>
<meta charset="utf-8" />
<meta name="description" content="Генерация исходящих адресов с помощью хелперов Html.ActionLink, Html.RouteLink и RouteUrl">
...
<h2>Генерация исходящих адресов URL</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Еще одной стороной механизма маршрутизации является генерации исходящих адресов в приложении, например, при выводе их в представлении. 
Наиболее простой способ вывода адреса является использование якоря - элемента <code>a</code>:</p>
<pre class="brush:xml;">&lt;a href="Home/Index/3" /&gt;</pre>
<p>Но также мы можем использовать специальные хелперы рендеринга - <span class="bb">Html.ActionLink</span> и <span class="bb">Html.RouteLink</span></p>
...
<pre class="brush:c#;">@Html.ActionLink("Жми здесь", "Show")
</pre>
<p>Что создаст вам следующую разметку:</p>
<pre class="brush:xml;">&lt;a href="/Home/Show"&gt;Жми здесь&lt;/a&gt;
</pre>
<p>Когда надо указать ссылку на действие из другого контроллера, то в хелпере <code>ActionLink</code> в качестве третьего аргумента 
...
<p>Кроме того, если у нас в некотором методе Index контроллера Book определено несколько параметров:</p> 
<pre class="brush:c#;">
...
аргументов для имени контроллера и имени действия. Так, первый пример с <code>ActionLink</code> эквивалентен следующему коду:</p>
<pre class="brush:c#;">@Html.RouteLink("Все книги", new { controller = "Book", action = "Index", author = "Толстой", id = 10 }, new { id = "Tolstoi", @class = "link" })</pre>
...
Например, возьмем стандартный маршрут Default:</p>
<pre class="brush:c#;">
routes.MapRoute(
    name: "Default",
...
<pre class="brush:c#;">@Html.RouteLink("Все книги","Default",new { action = "Show" })</pre>
<h3>URL-хелперы</h3>
<p>URL-хелперы похожи на хелперы <code>ActionLink</code> и <code>RouteLink</code> за тем исключением, что они не возвращают HTML, 
а создают пути URL и возвращают их в виде строк. Имеется три типа URL-хелперов:</p>
<ul>
<li><code>Action</code></li>
<li><code>Content</code></li>
<li><code>RouteUrl</code></li>
</ul>
<p>Хелпер <code>Action</code> похож на ActionLink за тем исключением, что но не возвращает тег якоря. Например, следующий код отображает 
адрес URL, но не саму ссылку:</p>
<pre class="brush:c#;">@Url.Action("Index", "Book", new { author = "Толстой", id = 10 }, null)</pre>
<p>Хелпер <code>RouteUrl</code> использует тот же шаблон, что и <code>Action</code>, но как и <code>RouteLink</code>, принимает имя маршрута 
и аргументы для параметров маршрута:</p>
<pre class="brush:c#;">@Url.RouteUrl(new { controller = "Book", action = "Index", author = "Толстой", id = 10 })</pre>
<p>Хелпер <code>Content</code> преобразует относительные пути в абсолютные. Пример использования хелпера Content можно увидеть в представлении _Layout:</p>
<pre class="brush:xml;">
       &lt;script src="@Url.Content("~/Scripts/jquery-1.5.1.min.js")" 
        type="text/javascript"&gt;&lt;/script&gt;
</pre>
<p>С помощью тильды (<code>~</code>) хелпер Content генерирует необходимый URL относительно расположения приложения. Без тильды URL мог бы стать 
некорректным, если бы вы перенесли приложение в другой виртуальный каталог.</p>
