...
<html >
<head>
<title>ASP.NET MVC 4 | Движок представлений и Razor</title>
<meta charset="utf-8" />
<meta name="description" content="Движок представлений и Razor в ASP.NET MVC 4">
...
<h2>Движок представлений и Razor</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>При использовании элементов кода С# в представлениях мы использовали знак @. Это знак, используемый движком представлений Razor для перехода к коду на 
языке C#/VB.NET. Чтобы понять суть движка Razor и его синтаксиса, посмотрим сначала, что такое движки представлений.</p>
<h3>Движок представлений</h3>
...
<p>До MVC 3 использовался только движок <strong>Web Forms</strong>, 
который использовал стандартный синтаксис ASP.NET. Например:</p>
<pre class="brush:xml;">
&lt;% foreach(BookStore.Models.Book b in ViewBag.Books) { %&gt;
    &lt;li&gt;Книга: &lt;%: b.Name %&gt;&lt;/li&gt;
&lt;% } %&gt;
</pre>
<p>Введение в MVC 3 движка Razor позволило уменьшить синтаксис при вызове кода C#, сделать код более "чистым". 
Аналогичный пример на Razor выгдядит так:<code>@foreach(BookStore.Models.Book b in ViewBag.Books) {&lt;li&gt;Книга: b.Name &lt;/li&gt;}</code>
Поэтому его использование более предпочтительно. В конечном счете, выбор движка представлений зависит от личных предпочтений, и оба движка по функциональности равноценны. </p>
<p>В данном случае важно понимать, что движок представлений, например, Razor - это не какой-то новый язык, это лишь способ рендеринга представлений, 
который имеет определенный синтаксис для перехода от разметки html к коду C#.</p>
<p>Кроме движка Razor можно использовать и движки сторонних производителей. Например, движок Spark</a>, использующий декларативный синтаксис, или Brail, который использует язык Boo.</p>
