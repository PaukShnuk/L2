...
<html >
<head>
<title>Аутентификация форм</title>
<meta charset="utf-8" />
<meta name="description" content="Основы аутентификации форм в asp.net mvc, ее основные моменты и параметры, определение в файле web.config, сохранение данных в виде куки-наборов">
...
<p>Еще один способов аутентификации пользователей представляет аутентификация форм. Она более гибкая по сравнению с аутентификацией Windows, 
хотя, возможно, и чуть более сложная в понимании. Она основывается она выдаче аутентифицированному пользователю куки-наборов, по которым он 
в дальнейшем верифицируется.</p>
<p>Чтобы установить режим аутентификации на основе форм, в файл конфигурации web.config заносится следующие строки в секцию <i>system.web</i>:</p>
<pre class="brush:xml;">
  &lt;system.web&gt;
	..................
...
<p>Блок <i>authentication</i> определяет параметры аутентификации, в частности, блок forms задает в параметре <span class="bb">loginUrl</span> 
ресурс, на который будет направлен пользователь, который не был аутентифицирован.</p>
<p>Для рассмотрения аутентификации форм возьмем веб-приложение по шаблону Internet Application, который содержит всю базовую инфраструктуру авторизации пользователей.</p>
<p>Это приложение в файле конфигурации по умолчанию содержит подключение к бд. При желании мы можем сами создать свою БД и переопределить подключение. 
А база данных для этого подключения, создаваемая при первом обращении пользователя в папке App_Data, хранит все данные об учетных 
записях, зарегистрированных в системе. Из этой же базы будут браться данные для подтверждения аутентификации пользователя.</p>
<p>Вся логика аутентификации заключена в методах Login контроллера AccountController, которые осуществляют вход пользователя в систему. В зависимости 
от используемой версии Visual Studio. Так, в <u>Visual Studio 2010</u> метод Login выглядит следующим образом:</p>
<pre class="brush:c#;">
[AllowAnonymous]
...
<p>Тогда как в проекте, созданном в Visual Studio 2012 этот метод будет выглядеть следующим образом:</p>
<pre class="brush:c#;">
[AllowAnonymous]
public ActionResult Login(string returnUrl)
{
...
<p>Несмотря на некоторые различия фактически они производят те же действия в виде аутентификации форм. Сама аутентификация разделяется на два действия:</p>
<ul>
<li><p><span class="bb">Валидация пользователя</span>: в первом случае это делает метод <code>Membership.ValidateUser</code>. И если пользователь находится 
в нашей базе данных, то далее приложение переходит ко второму этапу.</p></li>
<li><p>Второй этап представляет <span class="bb">создание аутентификационного тикета</span> - то есть некоторого билета безопасности, 
...
<b>.AUTHPATH</b> с помощью метода <code>FormsAuthentication.SetAuthCookie</code></p></li>
</ul>
<p>В этом и по сути и состоит сущность аутентификации форм.</p>
<p>Несмотря на то, что во втором случае - в коде для Visual Studio 2012 не используются явно эти методы, а вместо них применяется только один - 
<code>WebSecurity.Login</code>, но фактически он скрывает ту же функциональность в своей внутренней реализации:</p>
<pre class="brush:c#;">
...
<h3>Создание куки-набора</h3>
<p>Важно отметить, что при установке куки-наборов их содержимое шифруется при помощи машинных ключей, которую автоматически создает IIS на 
сервере. После успешной авторизации на сервере приложения мы можем посмотреть в браузере добавленный куки-набор:</p>
<img src="mvcpics/11.5.png" alt="Установка куки-наборов в ASP.NET MVC" />
<p>Как вы видите, первый куки-набор <span class="bb">.AUTHPATH</span>, который содержит данные авторизованного пользователя, совершенно нечитаем, 
что уменьшает вероятность взломов простой подделкой занчений куки.</p>
<p>Второй куки-набор - <b>__RequestVerificationToken</b> - представляет специальный куки-набор, который препятствует взлому в виде CSRF-атак.</p>

<h3>Настройка аутентификации</h3>
<p>Используя атрибуты узла <i>forms</i> в файле конфигурации, мы можем настроить параметры аутентификации. В частности мы можем применить 
...
<p>Например, мы можем задать следующее определение аутентификации форм:</p>
<pre class="brush:xml;">
  &lt;system.web&gt;
	..................
    &lt;authentication mode="Forms"&gt;
...
<div class="nav"><p><a href="11.2.php">Назад</a><a href="./">Содержание</a><a href="11.4.php">Вперед</a></p></div></div>
<div class="socBlock">
