...
<html >
<head>
<title>Добавление файлов приложения на somee.com</title>
<meta charset="utf-8" />
<meta name="description" content="Развертывание веб-приложения на хостинге somee.com, выбор способа публикации, публикация в файловую систему">
...
<h2>Добавление файлов приложения на somee.com</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Теперь, когда у нас уже есть база данных на сервере, перейдем к публикации сайта. Нажмем в Visual Studio на проект и выберем в появившемся меню пункт Publish. В мастере публикации 
выберем профиль (или создадим, если нет ни одного профиля). И перейдем дальше на вкладку Connection (Подключение). Здесь нам надо будет выбрать 
способ публикации и указать настройки:</p>
<img src="mvcpics/13.26.png" alt='Мастер публикации приложения в Visual Studio 2012' />
<p>Вначале в опции <span class="bb">Publish method</span> предстоит выбрать способ публикации. Какой способ выбрать в данном случае? 
В зависимости от ресурса можно использовать различные способы публикации. Так, на Windows Azure можно выбрать способ <strong>Web Deploy</strong>. 
При наличии доступа по ftp можно выбрать соответствующий способ - <span class="bb">FTP</span>.</p> 
<p>Нам вполне мог бы помочь способ <strong>Web Deploy Package</strong>, только создаваемый на выходе пакет приложения имеет слишком витиеватую 
структуру и включает ненужные файлы, в частности, каталог App_Data с бд нам уже не нужен.</p>
<p>Поэтому воспользуемся способом <span class="bb">File System</span>, который предполагает, что все содержимое сайта будет опубликовано на жестком 
диске в файловой системе. Затем, выбрав нужные файлы, мы создадим из него архив и перенесем его на хостинг.</p>
<p>Теперь в поле <span class="bb">Target location</span> введем каталог на локальном диске, в котором будет опубликовано приложение. 
А в поле <span class="bb">Destination URL</span> укажем адрес нашего сайта, который мы создали на somee.com. И нажмем на кнопку Publish.</p>
<p>После этого, если публикация прошла успешно, по указанному пути в поле <i>Target location</i> - каталогу публикации - на жестком диске будет опубликовано приложение. 
Перейдем по этому пути:</p>
<img src="mvcpics/13.29.png" />
<p>Во-первых, изменим строку подключения в файле <i>web.config</i> (который у нас в опубликованном каталоге) на новую, поскольку мы создали на хостинге новую бд.  Чтобы найти 
строку подключения к новой базе данных, зайдем в учетную запись на somee.com и слева в меню раскроем узел Databases и нажмем на нашу бд. 
После этого справа появятся настройки подключения к бд, в том числе и строка подключения:</p>
<img src="mvcpics/13.30.png" />
<p>После изменения строки подключения создадим из всех файлов и папок в каталоге публикации, кроме папки App_Data, архивный пакет, например, zip. 
Затем перейдем к панели учетной записи на somee.com и в меню слева выберем File Manager. В центральной части отобразится файловая система 
нашего сайта. Если в ней есть какие-нибудь файлы по умолчанию, то их можно удалить. Теперь нажмем на кнопку Upload и с помощью кнопки 
Обзор выберем только что созданный архивный пакет приложения. И после этого нажмем на ссылку 
<span class="bb">Upload and Unzip archives</span>.</p>
<img src="mvcpics/13.31.png" alt="Загрузка сайта на somee.com" />
<p>После этого выбранный пакет приложения будет загружен и автоматически распакован. И теперь мы можем работать с приложением и 
обращаться к нему из браузера.</p>
<p>Возможно, все шаги по созданию учетной записи и публикации приложения покажутся слишком долгими и скучными, но в реальности это занимает не более 10 минут.</p>
<p>Хостинг somee.com выбран только в качестве примера. Этапы от регистрации до публикации на каждом конкретном хостинге могут отличаться.</p>
...
<div class="nav"><p><a href="13.4.php">Назад</a><a href="./">Содержание</a><a href="14.1.php">Вперед</a></p></div></div>
<div class="socBlock">
