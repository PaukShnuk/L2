...
<html >
<head>
<title>Публикация на веб-сервере IIS</title>
<meta charset="utf-8" />
<meta name="description" content="Развертывание приложения на веб-сервере IIS, мастер публикации в Visual Studio 2012, настройка IIS">
...
<h2>Публикация на веб-сервере IIS</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Как правило, большинство веб-приложений ASP.NET публикуются на веб-сервер IIS. Поэтому посмотрим на примере, как размещать 
веб-приложение на IIS-севере.</p>
<p>В начале сконфигурируем веб-сервер. Для этого откроем средство администрирования IIS: зайдем в Панель управления, затем выберем 
Администрирование-&gt;Диспетчер служб IIS. И нам откроется консоль управления IIS:</p>
<img src="mvcpics/13.8.png" alt="Консоль IIS" />
<p>Будем размещать свой сайт в узле по умолчанию (в моем случае это <i>Default Web Site</i>). И для этого вначале создадим в каталоге этого 
узла папку для нашего приложения. По умолчанию каталогом для стандартного веб-узла является каталог <i>C:\inetpub\wwwroot</i>. Перейдем в нее 
и создадим в нем папку BookStore, которая будет содержать наше приложение.</p>
<p>Теперь нажмем правой кнопкой мыши на имя узла по умолчанию и выберем в появившемся меню 
пункт <span class="bb">Добавить приложение</span>:</p>
<img src="mvcpics/13.9.png" alt="Добавление приложения в IIS" />
<p>В появившемся окне введем соответствующие настройки (в качестве физического пути приложения созданный выше каталог):</p>
<img src="mvcpics/13.10.png" alt="Настройка приложения для IIS" />
<p>Сайт у нас практически создан. Теперь осталось разместить в каталоге <i>C:\inetpub\wwwroot\BookStore</i> наше приложение.</p>
<p>Перейдем к приложению в Visual Studio. Нажмем правой кнопкой на название проекта и в появившемся меню выберем <span class="bb">Publish</span>:</p>
<img src="mvcpics/13.11.png" />
<p>Перед нами откроется мастер публикации, который предложит нам пройти несколько этапов. В начале выберем профиль:</p>
<img src="mvcpics/13.3.png" alt="Мастер публикации сайта в Visual Studio 2012" />
<p>Если не одного профиля не определено, то создадим, нажав на ссылку <span class="bb">New...</span> и выбрав какое-нибудь название.</p>
<p>После создания профиля нажмем на Next и перейдем к следующему этапу - Connection. На этом этапе для опции <span class="bb">Publish Method</span> 
выберем <span class="bb">File System</span></p>
<p>Для опции <span class="bb">Target Location</span> определим физический путь к каталогу  нашего сайта. В данном случае это путь 
<i>C:\inetpub\wwwroot\BookStore</i>.</p>
<p>А для поля <span class="bb">Destination URL</span> указываем url, по которому будет доступно приложение, а именно  
<i>http://localhost/BookStore</i>.</p>
<img src="mvcpics/13.5.png" />
<p>После установки всех свойств жмем на кнопку Publish. После этого в Visual Studio в окне Output студия выдаст сводку об успешности или 
неуспешности публикации. Возможно, вы увидите сообщения "Отказано в доступе", из-за чего публикация пройдет неудачно:</p>
<img src="mvcpics/13.12.png" alt="Консольный вывод публикации сайта" />
<p>В этом случае нам надо открыть доступ к папке C:\inetpub\wwwroot\BookStore на запись (по умолчанию стоит доступ только на чтение) и повторить 
публикацию.</p>
<p>Либо как вариант, если мы не хотим раскрывать доступ на запись, то можно вручную скопировать и перенести все необходимые файлы в указанную выше папку. Как нам показывает окно Output, все файлы 
для публикации сохраняются в проекте в каталоге <i>obj\Release\Package\PackageTmp</i>. И мы можем просто скопировать все файлы из этого 
каталога и перенести их в каталог <i>C:\inetpub\wwwroot\BookStore</i>.</p>
<p>Если мы при создании веб-приложения использовали движок базы данных localdb, то нам надо разместить базу данных на MS SQL Servere и соответсвенно 
изменить строку подключения к базе данных, чтобы использовать уже не localdb, который предназначен только для целей разработки, а 
MS SQL Server.</p>
<p>Теперь запустим Службу веб-публикаций через консоль Служб или через консоль IIS и можем обращаться к сайту по url <i>http://localhost/BookStore</i>.</p>
...
<div class="nav"><p><a href="13.1.php">Назад</a><a href="./">Содержание</a><a href="13.3.php">Вперед</a></p></div></div>
<div class="socBlock">
