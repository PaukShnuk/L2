...
<html >
<head>
<title>Введение в Web API</title>
<meta charset="utf-8" />
<meta name="description" content="Сущность Web API как веб-службы, архитектура REST, создание проекта Web API, работа с методами контроллера, маршрутизация в Web API">
...
<h1>Web API</h1>
<h2>Введение в Web API</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Одним из преимуществ стека технологий на платформе .NET является возможность создания сервисов. Так, в ASP.NET мы могли создавать 
ASMX-веб-службы. Кроме того, мы можем использовать более функциональную в этом плане технологию WCF для создания веб-служб.</p>
<p>Но с последним релизом MVC 4 и вообще всей платформы .NET 4.5 в нашем распоряжении оказался еще один инструмент для создания веб-служб - 
Web API. Концепция Web API (которую кстати можно использовать не только в MVC, но и в веб-формах) новый подход к реализации веб-приложений.</p>
<p>Создадим простое приложение ASP.NET MVC 4, указав в качестве шаблона проекта <span class="bb">Web API</span>:</p>
<img src="mvcpics/12.1.png" />
<p>Среда создаст обычный mvc-проект со стандартной структурой. По умолчанию нам будет создано два контроллера. Один из них стандартный контроллер 
HomeController. Второй контроллер - <strong>ValuesController</strong>, который и реализует функционал Web API:</p>
<pre class="brush:c#;">
...
<p>Как видно, определение контроллера Web API отличается от обычного контроллера. Во-первых, он образован от класса <code>ApiController</code>, 
который не связан с базовым классом обычных контроллеров - <code>Controller</code></p>
<p>Во-вторых, контроллеры Web API применяют стиль REST. Вообще поддержка архитектуры REST (Representation State Transfer или 
"передача состояния представления") является одним из основных узловых пунктов технологии Web API.</p>
<p>Для взаимодействия с сервером в REST-архитектуре используются методы HTTP:</p>
<ul>
<li><p>GET</p></li>
<li><p>POST</p></li>
<li><p>PUT</p></li>
...
<p>У нас тут нет обычных методов действий, как в традиционных контроллерах, которые возвращают ActionResult. А определенные в 
контроллере ValuesContoller методы сопоставляются с одноименными методами HTTP.</p>
<h3>Маршрутизация в Web API</h3>
<p>Поскольку в Web API методы контроллера не являются прямыми ресурсами и сопоставляются с методами HTTP, то и весь механизм маршрутизации 
действует не как при определении обычных маршрутов. Все определения маршрутов для Web API находятся в файле <i>WebApiConfig.cs</i> 
(в папке <i>App_Start</i>):</p>
<pre class="brush:c#;">
...
коллекцию элементов <code>string</code>.</p>
<p>Если же сервер получит запрос PUT с адресом <i>api/values</i>, то такой запрос будет сопоставлен с методом Put.</p>
<p>А запросу GET <i>api/values/7</i> будет соответствовать метод <code>string Get(int id)</code>, так как этот метод принимает параметр.</p>
...
<p>Однако мы можем использовать и любые другие имена без префиксов, но в этом случае нам надо будет явно указать метод HTTP в виде атрибута, например:</p>
<pre class="brush:c#;">
public class ValuesController : ApiController
{
...
<p>Таким образом, если первые два действия соответствуют соглашениям о наименовании (в начале содержат название метода HTTP), 
...
<div class="nav"><p><a href="11.11.php">Назад</a><a href="./">Содержание</a><a href="12.2.php">Вперед</a></p></div></div>
<div class="socBlock">
