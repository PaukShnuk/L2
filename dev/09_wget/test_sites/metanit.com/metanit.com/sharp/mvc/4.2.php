...
<html >
<head>
<title>Строго типизированные представления</title>
<meta charset="utf-8" />
...
<p>В предыдущих примерах, чтобы передать информацию из БД в представление, мы использовали объект ViewBag:</p>
<pre class="brush:xml;">
...
у нас не задан явно и выводится компилятором. Либо мы могли бы указать тип модели явно, применив полное имя типа модели:</p>
<pre class="brush:xml;">
...
<p>Объект <span class="bb">Model</span> представляет тип модели, указанной в директиве <code>@model</code>, и будет хранить переданные из контроллера данные.</p>
 <p>Чтобы не писать полностью и содержит передаваемые в параметре данные. Но чтобы не писать полностью имя типа модели, мы можем импортировать пространство имен 
в представлении:</p>
<pre class="brush:xml;">
...
<img src="mvcpics/4.1.png" />
<p>В поле модели укажем какой-нибудь имеющийся класс модели, а в поле шаблонов выберем любой шаблон, кроме Empty. И если 
мы, к примеру, выберем шаблон List, то автоматически сгенерированное представление будет по своему функционалу идентично ранее рассмотренному представлению 
с выводом книг.</p>
