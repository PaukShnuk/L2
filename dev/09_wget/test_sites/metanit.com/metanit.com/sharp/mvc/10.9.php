...
<html >
<head>
<title>Работа с jQuery UI</title>
<meta charset="utf-8" />
...
<h2>Работа с jQuery UI</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Одним из наиболее популярных плагинов jQuery является jQuery UI, поэтому его и включили в стандартный набор скриптов. Этот плагин 
предназначен для работы с пользовательским интерфейсом и содержит, во-первых, ряд интересных визуальных эффектов, типа 
bounce, explode, fade, pulsate и shake. Во-вторых, он содержит набор виджетов, как accordion, autocomplete, button, datepicker, dialog, progressbar, slider и tabs.</p>
<p>Чтобы начать работать с jQuery UI, надо прежде всего подключить эту библиотеку:</p>
<pre class="brush:xml;">
&lt;link href="@Url.Content("~/Content/themes/base/jquery-ui.css")" rel="stylesheet" type="text/css" /&gt;
&lt;script src="@Url.Content("~/Scripts/jquery-ui-1.8.20.min.js")" type="text/javascript"&gt;&lt;/script&gt;
</pre>
<p>Обратите внимание, что в данном случае нам также нужно подключить и файл стилей <span class="ii">jquery-ui.css</span>, иначе плагин просто не будет работать. 
...
по умолчанию включена одна тема - <code>base</code>. Все используемые ее файлы располагаются в папке <span class="ii">Content/themes/base</span>. 
Здесь есть изображения, нужные для визуализации. А также файлы стилей. Файл <span class="ii">jquery-ui.css</span> является основным, поэтому мы его подключаем к приложению.</p>
<h3>Виджет Autocomplete</h3>
<p>Теперь добавим в наше приложение виджет Autocomplete, который реализует функциональность автозаполнения.</p>
<p>Для начала определим в представлении элемент ввода, для которого и будет производиться автозаполнение:</p>
<pre class="brush:xml;">
...
<p>Мы просто извлекаем все книги из базы, по ним формируем новый анонимный объект - автора, и получившийся массив новых объектов посылаем обратно 
...
либо оба этих свойства. Свойство label используется при отображении текста пользователю. При выборе пользователем определенного элемента в 
...
<p>И в конце надо добавить скрипт, поместив его где-нибудь представлении или во внешний js-файл, который вызывает функцию autocomplete для элемента заполнения:</p>
<pre class="brush:js;">
$(function () {
...
<div class="nav"><p><a href="10.8.php">Назад</a><a href="./">Содержание</a><a href="11.1.php">Вперед</a></p></div></div>
<div class="socBlock">
