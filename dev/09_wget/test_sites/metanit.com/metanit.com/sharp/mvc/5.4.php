...
<h2>Редактирование модели</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В прошлом разделе мы увидели, что для редактирования полей модели удобно применять хелпер Editor/EditorFor. Теперь посмотрим, как сделать саму 
логику редактирования модели. Пусть в некотором действии контроллера мы получаем объект модели по Id и выводим ее поля для редактирования в представлении:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>На случай, если пользователи не укажут id, мы устанавливаем в качестве параметра не int, а <code>int?</code>. И если такой параметр не передан, то 
возвращаем результат метода HttpNotFound.</p>
...
<p>С помощью строки <code>db.Entry(book).State = EntityState.Modified;</code> мы указываем, что объект существует <code>book</code> уже в базе данных, 
...
сгенерировать и выполнить команду UPDATE для обновления модели в БД.</p>
<p>Обратимся к методу EditBook, например, <code>Home/EditBook/1</code>:</p>
<img src="mvcpics/5.7.png" />
<p>Хелпер <code>Html.EditorFor</code> сгенерировал нам поля для редактирования. Мы можем изменить модель, и отправить ее на сервер, где произойдет ее сохранение.</p>
...
<div class="nav"><p><a href="5.3.php">Назад</a><a href="./">Содержание</a><a href="5.9.php">Вперед</a></p></div></div>
<div class="socBlock">
