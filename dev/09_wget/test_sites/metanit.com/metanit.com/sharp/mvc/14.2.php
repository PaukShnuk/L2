...
<html >
<head>
<title>Метатег Viewport</title>
<meta charset="utf-8" />
<meta name="description" content="Использование метатега Viewport для создания мобильных веб-приложений и его параметры">
...
<h2>Метатег Viewport</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Очевидно, что размеры экрана мобильных устройств ставят перед нами одну из наиболее важных проблем при создании сайта для мобильных устройств. 
В данном случае избежать этой проблемы нам поможет тег Viewport. Возьмем, к примеру, следующую разметку:</p>
<pre class="brush:xml;">
&lt;!doctype html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Запустив данную страничку в эмуляторе Opera Mobile, получим следующую картину:</p>
<img src="mvcpics/14.1.png" alt="веб-страница без использования тега viewport" />
<p>Используя масштабирование, пользователь может наконец-то увидеть, что же там все таки написано. Однако это не очень удобно. 
При этом сайт страница имеет много пустого места, что не очень красиво.</p>
<p>Почему так происходит? Дело в том, что каждый мобильный браузер задает странице некоторые начальные размеры и потом пытается приспособить под размеры 
экрана текущего мобильного устройства.</p>
<p>Вся видимая область на экране браузера описывается понятием <span class="bb">Viewport</span>. По сути viewport представляет область, в которую 
веб-браузер пытается "впихнуть" веб-страницу. Например, браузер Safari на iPone и iPod определяет ширину viewport по умолчанию равной 
980 пикселям. Затем, получив страницу с сервера и вписав в viewport с шириной 980 пикселей, браузер сжимает ее 
до размеров мобильного устройства. Например, если ширина экрана смартфона составляет 320 пикселей, то до этого размера потом будет 
сжата страница. И ко всем элементам страницы будет применен коэффициент масштабирования, равный 320/980.</p>
<p>Почему в данном случае используется именно 980 пикселей, а, скажем, не реальный размер экрана? Все дело в том, что по умолчанию браузер считает, 
что каждая веб-страница предназначена для десктопов. А обычной шириной десктопного сайта можно считать 980 пикселей.</p>
<p>При этом для каждого браузера устанавливается своя ширина области viewport, необязательно 980 пикселей, например, IE поддерживает в качестве начальной ширины 1024 пикселя. Но общая картина будет та же.</p>
<p>Чтобы избежать подобной не очень приятной картины, следует использовать метатег viewport. Использование этого метатега во многом даст понять 
браузеру, что он имеет дело не с десктопной страницей, а предназначенной в том числе и для мобильных устройств.</p>
<p>Общее объявление метатега следующее:</p>
<pre class="brush:xml;">
&lt;meta name="viewport" content="параметры_метатега"&gt;
</pre>
<p>В атрибуте content мета-тега мы можем определить следующие параметры:</p>
<table class="tab">
<tr class="tabhead"><td><p>Параметр</p></td><td><p>Значения</p></td><td><p>Описание</p></td></tr>
<tr><td><p>width</p></td><td><p>Принимает целочисленное значение в пикселях или значение <code>device-width</code></p></td><td><p>Устанавливает ширину области viewport</p></td></tr>
<tr><td><p>height</p></td><td><p>Принимает целочисленное значение в пикселях или значение <code>device-height</code></p></td><td><p>Устанавливает высоту области viewport</p></td></tr>
<tr><td><p>initial-scale</p></td><td><p>Число с плавающей точкой от 0.1 и выше</p></td><td><p>Задает коэффициент масштабирования начального размера viewport. 
Значение 1.0 задает отсутствие масштабирования</p></td></tr>
<tr><td><p>user-scalable</p></td><td><p>no/yes</p></td><td><p>Указывает, может ли пользователь с помощью жестов масштабировать страницу</p></td></tr>
<tr><td><p>minimum-scale</p></td><td><p>Число с плавающей точкой от 0.1 и выше</p></td><td><p>Задает минимальный масштаб размера viewport. 
Значение 1.0 задает отсутствие масштабирования</p></td></tr>
<tr><td><p>maximum-scale</p></td><td><p>Число с плавающей точкой от 0.1 и выше</p></td><td><p>Задает максимальный масштаб размера viewport. 
Значение 1.0 задает отсутствие масштабирования</p></td></tr>
</table>
<p>Теперь изменим предыдущий пример веб-страницу, использовав метатег:</p>
<pre class="brush:xml;">
&lt;!doctype html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="mvcpics/14.2.png" alt="веб-страница с использованием тега viewport" />
<p>Веб-страничка определенно выглядит лучше благодаря использованию метатега viewport. Используя параметр <code>width=device-width</code> 
...
viewport и ширина одинаковы.</p>
<p>Мы также можем использовать другие параметры, например, запретить пользователю масштабировать размеры страницы:</p>
<pre class="brush:xml;">
