...
<html >
<head>
<title>Авторизация в MVC 4 и SimpleMembershipProvider</title>
<meta charset="utf-8" />
<meta name="description" content="Авторизация на примере Internet Application, провайдер членства SimpleMembershipProvider и провайдер ролей SimpleRoleProvider, таблицы для хранения учетных записей">
...
<h2>Авторизация в MVC 4 и SimpleMembershipProvider</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В ASP.NET MVC 4 был введен новый провайдер членства, который стал использоваться по умолчанию в проектах по типу Internet Application 
...
которые содержатся в основе ASP.NET API. Так, класс <strong>SimpleRoleProvider</strong> просто применяет абстрактный базовый класс <code>RoleProvider</code>.</p>
<p>Класс SimpleMembershipProvider также применяет традиционный провайдер членства: сначала абстрактный класс ExtendedMembershipProvider наследует 
MembershipProvider, а затем сам SimpleMembershipProvider наследуется от ExtendedMembershipProvider.</p>
 <p>Для облегчения работы с новым провайдером членства используется класс-хелпер <strong>WebSecurity</strong> из пространства имен 
 <code>WebMatrix.WebData</code>. Через этот класс мы можем инициализировать подключение или проводить верифицикацию пользователей.</p>
<p>В качестве примера для рассмотрения выберем проект по типу Internet Application. После создания проекта в каталоге Filters в структуре проекта мы можем найти файл <strong>InitializeSimpleMembershipAttribute.cs</strong>, 
который содержит объявление класса <span class="bb">InitializeSimpleMembershipAttribute</span>. Этот класс является производным от 
ActionFilterAttribute, то есть фактически представляет класс фильтра действия, применяемого к контроллеру AccountController.</p>
<p>InitializeSimpleMembershipAttribute содержит определение класса-инициализатора SimpleMembershipProvider, который создает соединение 
с базой данных следующим вызовом:</p>
<pre class="brush:c#;">
WebSecurity.InitializeDatabaseConnection("DefaultConnection", "UserProfile", "UserId", "UserName", autoCreateTables: true);
</pre>
<p>По умолчанию этот метод определяет следующие значения:</p>
<ul>
<li><p>"DefaultConnection" - имя подключение, которое содержится в файле Web.config</p></li>
<li><p>"UserProfile" - название таблицы пользователей в БД (по умолчанию содержит столбцы UserId и UserName). Именно эта таблица хранит профили пользователей.</p></li>
<li><p>"UserId" - имя столбца в таблице пользователей, который содержит Id пользователя</p></li>
<li><p>"UserName" - имя столбца в таблице пользователей, который содержит имя пользователя</p></li>
<li><p>autoCreateTables: true - создает весь необходимый для провайдера Simple Membership набор таблиц, если они еще не существуют.</p></li>
</ul>
<p>При первом обращении к БД, если ее не существовало, она будет создана и автоматически заполнена таблицами с определенными полями. </p>
<img src="mvcpics/11.6.png" alt="Таблицы в БД по умолчанию в ASP.NET MVC 4" />
<p>Эти таблицы имеют следующее определение:</p>
<h4>webpages_Membership</h4>
<table class="tab">
<tr class="tabhead"><td><p>Столбец</p></td><td><p>Тип данных</p></td></tr>
<tr><td><p>UserId</p></td><td><p>int</p></td></tr>
<tr><td><p>CreateDate</p></td><td><p>datetime</p></td></tr>
<tr><td><p>ConfirmationToken</p></td><td><p>nvarchar(128)</p></td></tr>
<tr><td><p>IsConfirmed</p></td><td><p>bit</p></td></tr>
<tr><td><p>LastPasswordFailureDate</p></td><td><p>datetime</p></td></tr>
<tr><td><p>PasswordFailuresSinceLastSuccess</p></td><td><p>int</p></td></tr>
<tr><td><p>Password</p></td><td><p>nvarchar(128)</p></td></tr>
<tr><td><p>PasswordChangedDate</p></td><td><p>datetime</p></td></tr>
<tr><td><p>PasswordSalt</p></td><td><p>nvarchar(128)</p></td></tr>
<tr><td><p>PasswordVerificationToken</p></td><td><p>nvarchar(128)</p></td></tr>
<tr><td><p>PasswordVerificationTokenExpirationDate</p></td><td><p>datetime</p></td></tr>
</table>
<p>Таблица webpages_Membership используется для управления учетными записями пользователей, то есть членством.</p>
<h4>webpages_OAuthMembership</h4>
<table class="tab">
<tr class="tabhead"><td><p>Столбец</p></td><td><p>Тип данных</p></td></tr>
<tr><td><p>Provider</p></td><td><p>nvarchar(30)</p></td></tr>
<tr><td><p>ProviderUserId</p></td><td><p>nvarchar(100)</p></td></tr>
<tr><td><p>UserId</p></td><td><p>int</p></td></tr>
</table>
<h4>webpages_Roles</h4>
<table class="tab">
<tr class="tabhead"><td><p>Столбец</p></td><td><p>Тип данных</p></td></tr>
<tr><td><p>RoleId</p></td><td><p>int</p></td></tr>
<tr><td><p>RoleName</p></td><td><p>nvarchar(256)</p></td></tr>
</table>
<h4>webpages_UsersInRoles</h4>
<table class="tab">
<tr class="tabhead"><td><p>Столбец</p></td><td><p>Тип данных</p></td></tr>
<tr><td><p>UserId</p></td><td><p>int</p></td></tr>
<tr><td><p>RoleId</p></td><td><p>int</p></td></tr>
</table>
<p>Последние две таблицы используются для управления ролями и пользователей.</p>
<p>Почему создаются таблицы именно с этими именами? Эти имена определены в коде самого <code>SimpleMembershipProvider</code>:</p>
<pre class="brush:c#;">
//..................................		
...
<p>Другие две таблицы закодированы в коде <code>SimpleRoleProvider</code>:</p>
<pre class="brush:c#;">
...
<p>Чтобы сменить способ организации данных в таблицах нам придется создать собственные провайдеры членства и ролей.</p>
<p>Однако таблицу определения пользователей, которая по умолчанию называется <i>UserProfile</i>, мы вполне можем переопределить. 
В следующем разделе мы это попробуем сделать.</p>
<p>По умолчанию регистрация пользователя в методе Register контроллера AccountController проводится с помощью следующего кода: 
<code>WebSecurity.CreateUserAndAccount(model.UserName, model.Password)</code>. Передавая в метод введенные данные, мы создаем две записи 
...
webpages_Membership в соответствующее поле.</p>
<p>Затем уже по логину и паролю мы можем войти на сайт - за это отвечает строка <code>WebSecurity.Login(model.UserName, model.Password)</code> 
...
то пользователь аутентифицируется.</p>
<p>SimpleMembershipProvider, будучи довольно гибким классом, может работать с различными типами SQL Server: 
SQL Server, SQL Azure, SQL Server CE, SQL Server Express, LocalDB. В тоже время он имеет ограничения: так он не может работать с 
 СУБД MySQL, базами данных NoSQL, поскольку использует специфичные для MS SQL Serverа выражения SQL для управления данными.</p>
...
<div class="nav"><p><a href="11.3.php">Назад</a><a href="./">Содержание</a><a href="11.5.php">Вперед</a></p></div></div>
<div class="socBlock">
