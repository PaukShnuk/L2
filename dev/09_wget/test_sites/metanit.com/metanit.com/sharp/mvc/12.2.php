...
<html >
<head>
<title>Создание и тестирование приложения Web API</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и настройка приложения Web API, его тестирование с помощью средства веб-отладки Fiddler">
...
<h2>Создание и тестирование приложения Web API</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Итак, создадим новое приложение MVC 4 по типу Web API:</p>
<img src="mvcpics/12.1.png" />
<p>Назовем его, его например, BookStore. Допустим, наше приложение будет работать с книгами.</p>
<p>Сначала создадим модель данных, описывающую книгу. Добавим в папку Models новый класс Book:</p>
<pre class="brush:c#;">
public class Book
{
...
<p>Затем добавим в папку Models контекст данных BookContext, через который будем взаимодействовать с базой данных:</p>
<pre class="brush:c#;">
...
<p>И добавим в папку <i>App_Data</i> базу данных для нашего приложения, где будут храниться данные. В моем случае база данных будет 
называться BookDB. Создадим в ной базе данных таблицу Books для моделей книг:</p>
<img src="mvcpics/12.2.png" />
<p>И наполним таблицу некоторыми данными:</p>
<img src="mvcpics/12.3.png" />
<p>И в завершении настройки модельной части изменим строку подключения в файле <i>web.config</i>:</p>
<pre class="brush:xml;">
&lt;connectionStrings&gt;
...
<p>Таким образом, у нас получился практически стандартный контроллер, реализующий CRUD-модель, то есть добавление, получение, обновление 
и удаление данных.</p>
<p>Кстати чтобы создать аналогичный функционал мы можем не писать вручную код, а воспользоваться шаблонами формирования при создании контроллера. Для этого просто при 
...
<img src="mvcpics/12.4.png" />
<p>И шаблон формирование создаст нам аналогичный функционал с небольшими изменениями в коде.</p>
<p>Итак, теперь мы можем перейти к построению клиентской части, но перед этим протестируем приложение. Для тестирования GET-запросов нам 
достаточно простого веб-браузера, но воспользуемся более удобным инструментом web-отладки как <strong>Fiddler</strong>.</p>
<p><a href='http://fiddler2.com/' rel='nofollow'>Fiddler</a> позволяет отправлять различные запросы к веб-ресурсам. Итак, 
...
к нашему сервису. Например, в моем случае локальный сервер запустился по адресу <i>http://localhost:2512/</i>, 
поэтому строка запроса будет выглядеть так: <i>http://localhost:2512/api/values/</i>. Поскольку у нас слева от адреса стоит метод GET, 
поэтому в данном случае мы будем осуществлять запрос к методу <code>public IEnumerable&lt;Book&gt; GetBooks()</code>.</p>
<img src="mvcpics/12.5.png" />

<p>После ввода адреса нажмем на Enter и перейдем на вкладку Inspectors, выбрав JSON в качестве формата отображения ответа, то увидим 
результат запроса - а именно список книг, который Fiddler получает в сериализованном виде:</p>
<img src="mvcpics/12.6.png" />
<p>Таким образом, мы убедились, что приложение работает. Подобным образом мы можем задать в Fiddler и другие запросы. 
А теперь перейдем к построению клиентской части приложения.</p>
