...
<html >
<head>
<title>Добавление и удаление модели</title>
<meta charset="utf-8" />
...
<h2>Добавление и удаление модели</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В предыдущей теме мы посмотрели, как редактировать модель. Продолжим работу с моделью Book и теперь посмотрим, как мы можем ее добавлять и удалить из БД. 
Для добавления модели вначале определим пару действий:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>Первый метод возвращает пользователю представление с формой для добавления, а второй - принимает данные этой формы. Теперь создадим представление.</p>
<p>А представление будет выглядеть следующим образом:</p>
<pre class="brush:xml;">
...
<p>При получении модели book в действии Create метод <code>db.Books.Add(book)</code> будет устанавливать значение <code>Added</code> в качестве состояния 
модели. Поэтому метод <code>db.SaveChanges()</code> сгенерирует выражение INSERT для вставки модели в таблицу. То есть метод Create мы могли 
бы переписать следующим образом:</p>
<pre class="brush:c#;">
[HttpPost]
public ActionResult Create(Book book)
{
...
<p>Теперь самая важная часть - удаление модели. Даже не в плане реализации, сколько в плане безопасности. Добавим простое действие, которое удаляет 
модель из базы данных:</p>
<pre class="brush:c#;">
...
<p>Вначале мы проверяем, а есть ли такой объект в бд, и если есть, то вызываем метод <code>db.Books.Remove(b)</code>. Он установит статус модели в <code>Deleted</code>, 
благодаря чему EntityFramework при вызове метода <code>db.SaveChanges</code> сгенерирует sql-выражение DELETE. Но мы можем сами указать статус явным образом:</p>
<pre class="brush:c#;">
public ActionResult Delete(int id)
{
...
<p>Подобный подход имеет один плюс - мы избегаем первого запроса к бд, который у нас был в выражении <code>Book b = db.Books.Find(id);</code>. То есть 
вместо двух запросов к БД теперь у нас только один. Но в целом подобный метод на удаление имеет один минус в плане безопасности.</p>
<p>Допустим, нам пришло электронное письмо, в которое была внедрена картинка посредством тега:</p>
<p><code>&lt;img src="http://адрес_нашего_сайта/Home/Delete/1" /&gt;</code></p>
<p>В итоге при открытии письма 1-я запись в таблице может быть удалена. Уязвимость качается не только писем, но может проявляться и в других местах, 
но смысл один - GET-запрос к методу Delete несет потенциальную уязвимость. Поэтому переделаем метод следующим образом:</p>
<pre class="brush:c#;">
[HttpGet]
public ActionResult Delete(int? id)
{
...
<p>Теперь вместо одного метода Delete целых два. Атрибут <code>ActionName("Delete")</code> указывает, что  метод DeleteConfirmed будет восприниматься 
как действие Delete. Первый метод передает удаляемую модель в представление. На представлении с помощью нажатия кнопки мы сможем 
подтвердить удаление. И id уйдет второму методу по запросу POST. Таким образом, мы уйдем от уязвимости GET-запроса. Ну и само представление:</p>
<pre class="brush:c#;">
@{
    Layout = "~/Views/Shared/_Layout.cshtml";
}
...
<div class="nav"><p><a href="5.4.php">Назад</a><a href="./">Содержание</a><a href="5.5.php">Вперед</a></p></div></div>
<div class="socBlock">
