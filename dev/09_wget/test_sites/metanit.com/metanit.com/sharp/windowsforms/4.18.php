...
<html >
<head>
<title>Элемент NotifyIcon в Windows Forms | Visual C#</title>
<meta charset="utf-8" />
...
<h2>Элемент NotifyIcon</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Элемент NotifyIcon позволяет задать значок, который будет отображаться при запуске приложения в панели задач.</p>
<p>Рассмотрим основные его свойства:</p>
<ul>
<li><p><span class="bb">BallonTipIcon</span>: иконка, которая будет использоваться на всплывающей подсказке. Это свойство может иметь следующие значения: 
<code>None</code>, <code>Info</code>, <code>Warning</code>, <code>Error</code>.</p></li>
<li><p><span class="bb">BalloonTipText</span>: текст, отображаемый во всплывающей подсказке</p></li>
<li><p><span class="bb">BalloonTipTitle</span>: загаловок всплывающей подсказки</p></li>
<li><p><span class="bb">ContextMenuStrip</span>: устанавливает контекстное меню для объекта NotifyIcon</p></li>
<li><p><span class="bb">Icon</span>: задает значок, который будет отображаться в системном трее</p></li>
<li><p><span class="bb">Text</span>: устанавливает текст всплывающей подсказки, которая появляется при нахождении указателя мыши над значком</p></li>
<li><p><span class="bb">Visible</span>: устанавливает видимость значка в системном трее</p></li>
</ul>
<p>Чтобы добавить на форму NotifyIcon, перенесем данный элемент на форму с панели инструментов. После этого добавленный компонент NotifyIcon 
отобразится внизу дизайнера формы.</p>
<p>Затем зададим у NotifyIcon для свойства Icon какую-нибудь иконку в формате <i>.ico</i>. И также установим для свойства <code>Visible</code> 
значение <code>true</code>.</p>
<p>Далее также зададим у NotifyIcon для свойства <code>Text</code> какой-нибудь текст, например, "Показать форму". Этот текст отобразится при прохождении указателя мыши над 
значком NotifyIcon в системном трее.</p>
<p>Чтобы можно было открыть форму по клику на значок в трее, надо обработать событие Click у NotifyIcon. Поэтому в коде формы определим обработчик для 
этого события:</p>
<pre class="brush:c#;">
...
<p>В обработчике просто переводим форму из минимизированного состояния в обычное.</p>
<p>И кроме того, чтобы форма не отображалась на панели задач, у нее задаем свойство <span class="bb">ShowInTaskbar = false</span>.</p>
<p>В итоге после запуска приложения в трее будет отображаться значок NotifyIcon, нажав на который при свернутой форме, мы ее можем заново открыть.</p>
<img src="pics/4.57.png" alt="NotifyIcon в Windows Forms" />
<p>Теперь используем всплывающую подсказку. Для этого изменим конструктор формы:</p>
<pre class="brush:c#;">
public partial class Form1 : Form
{   
    public Form1()
...
<p>И при запуске отобразится всплывающая подсказка:</p>
<img src="pics/4.58.png" />
...
<div class="nav"><p><a href="4.17.php">Назад</a><a href="./">Содержание</a><a href="4.19.php">Вперед</a></p></div></div>
<div class="socBlock">
