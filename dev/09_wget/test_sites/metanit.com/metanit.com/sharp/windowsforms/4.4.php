...
<html >
<head>
<title>Элемент MaskedTextBox в Windows Forms | Visual C#</title>
<meta charset="utf-8" />
...
<h2>Элемент MaskedTextBox</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Элемент MaskedTextBox по сути представляет обычное текстовое поле. Однако данные элемент позволяет контролировать ввод пользователя и проверять 
его автоматически на наличие ошибок.</p>
<p>Чтобы контролировать вводимые в поле символы, надо задать маску. Для задания маски можно применять следующие символы:</p>
<ul>
<li><p><code>0</code>: Позволяет вводить только цифры</p></li>
<li><p><code>9</code>: Позволяет вводить цифры и пробелы</p></li>
<li><p><code>#</code>: Позволяет вводить цифры, пробелы и знаки '+' и '-'</p></li>
<li><p><code>L</code>: Позволяет вводить только буквенные символы</p></li>
<li><p><code>?</code>: Позволяет вводить дополнительные необязательные буквенные символы</p></li>
<li><p><code>A</code>: Позволяет вводить буквенные и цифровые символы</p></li>
<li><p><code>.</code>: Задает позицию разделителя целой и дробной части</p></li>
<li><p><code>,</code>: Используется для разделения разрядов в целой части числа</p></li>
<li><p><code>:</code>: Используется в временных промежутках - разделяет часы, минуты и секунды</p></li>
<li><p><code>/</code>: Используется для разделения дат</p></li>
<li><p><code>$</code>: Используется в качестве символа валюты</p></li>
</ul>
<p>Чтобы задать маску, надо установить свойство <code>Mask</code> элемента. Найдя это свойство в окне свойств(Porperties), нажмем на него и нам 
отобразится окно для задания одного из стандартных шаблонов маски. В частности мы можем выбрать Phone number (Телефонный номер), который подразумевает 
ввод в текстовое поле только телефонного номера:</p>
<img src="pics/4.8.png" alt="Шаблоны элемента MaskedTextBox" />
<p>Теперь при запуске мы сможем ввести в текстовое поле только цифры, получив в итоге телефонный номер.</p>
<p>Теперь сделаем свою маску. Например, создадим маску для ввода инициалов имени и отчества и фамилий ограниченной длины в текстовое поле. Для 
этого присвоим свойству <code>Mask</code> значение <code>L.L.L?????????</code>. Тогда ввод в текстовое поле будет выглядеть следующим образом:</p>
<img src="pics/4.9.png" alt="Элемент MaskedTextBox в Windows Forms" />
<p>Данный элемент также представляет нам ряд свойств, которые можно использовать для управления вводом. Так, свойство <span class="bb">BeepOnError</span> 
при установке значения <code>true</code> подает звуковой сигнал при введении некорректного символа.</p>
<p>Свойство <span class="bb">HidePromptOnLeave</span> при установке в <code>true</code> при потери текстовым полем фокуса скрывает, указанные в PromptChar</p>
<p>Свойство <span class="bb">PromptChar</span> указывает на символ, который отображается в поле на месте ввода символов. По умолчанию стоит знак подчеркивания.</p>
<p>Свойство <span class="bb">AsciiOnly</span> при значении <code>true</code> позволяет вводить только asci-символы, то есть символы из диапазона A-Z и a-z.</p>
