...
<html >
<head>
<title>Размеры элементов и их позиционирование в контейнере | Visual C#</title>
<meta charset="utf-8" />
...
<h2>Размеры элементов и их позиционирование в контейнере</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для каждого элемента управления мы можем определить свойство <code>Location</code>, которое задает координаты верхнего левого угла элемента относительно контейнера. 
...
координаты положения элемента:</p>
<img src="pics/3.7.png" alt="Положение элемента на форме" />
<p>Также мы можем установить позицию элемента в коде:</p>
<pre class="brush:c#;">
...
<p>С помощью свойства <span class="bb">Size</span> можно задать размеры элемента:</p>
<img src="pics/3.9.png" alt="размеры элемента в Windows Forms" />
<p>Дополнительные свойства <code>MaximumSize</code> и <code>MinimumSize</code> позволяют ограничить минимальный и максимальный размеры.</p>
<p>Установка свойств в коде:</p>
<pre class="brush:c#;">
...
<h3>Свойство Anchor</h3>
<p>Дополнительные возможности по позиционировании элемента позволяет определить свойство Anchor. Это свойство определяет расстояние между одной из сторон элемента 
и стороной контейнера. И если при работе с контейнером мы будем его растягивать, то вместе с ним будет растягиваться и вложенный элемент.</p>
<p>По умолчанию у каждого добавляемого элемента это свойство равно <code>Top, Left</code>:</p>
<img src="pics/3.8.png" alt="Свойство Anchor" />
<p>Это значит, что если мы будем растягивать форму влево или вверх, то элемент сохранит расстояние от левой и верхней границы элемента до границ контейнера, в качестве которого 
выступает форма.</p>
<p>Мы можем задать четыре возможных значения для этого свойства или их комбинацию:</p>
<ul>
<li><p>Top<p></li>
<li><p>Bottom<p></li>
<li><p>Left<p></li>
...
<p>Например, если мы изменим значение этого свойства на противоположное - <code>Bottom, Right</code>, тогда у нас будет неизменным расстояние между правой и нижней стороной элемента и формой.</p>
<p>При этом надо отметить, что данное свойство учитывает расстояние до границ контейнера, а не формы. То есть если у нас на форме есть элемент Panel, а на 
...
на контейнер Panel.</p>
<p>Чтобы задать это свойство в коде, надо использовать перечисление <span class="bb">AnchorStyles</span>:</p>
<pre class="brush:c#;">
...
<h3>Свойство Dock</h3>
<p>Свойство Dock позволяет прикрепить элемент к определенной стороне контейнера. По умолчанию оно имеет значение None, но также позволяет задать еще пять значений:</p>
<img src="pics/3.10.png" alt="Свойство Dock в Windows Forms" />
<ul>
<li><p><code>Top</code>: элемент прижимается к верхней границе контейнера</p></li>
<li><p><code>Bottom</code>: элемент прижимается к нижней границе контейнера</p></li>
<li><p><code>Left</code>: элемент прижимается к левой стороне контейнера</p></li>
<li><p><code>Right</code>: элемент прикрепляется к правой стороне контейнера</p></li>
<li><p><code>Fill</code>: элемент заполняет все пространство контейнера</p></li>
</ul>
