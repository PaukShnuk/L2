...
<html >
<head>
<title>Текстовое поле TextBox в Windows Forms | Visual C#</title>
<meta charset="utf-8" />
...
<h2>Текстовое поле TextBox</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для ввода и редактирования текста предназначены текстовые поля - элемент TextBox. Так же как и у элемента Label текст элемента TextBox 
можно установить или получить с помощью свойства Text.</p>
<p>По умолчанию при переносе элемента с панели инструментов создается однострочное текстовое поле. Для отображения больших объемов информации в 
текстовом поле нужно использовать его свойства <code>Multiline</code> и <code>ScrollBars</code>. 
При установке для свойства <code>Multiline</code> значения true, все избыточные символы, которые выходят за границы поля, будут переноситься на 
новую строку.</p>
<p>Кроме того, можно сделать прокрутку текстового поля, установив для его свойства <code>ScrollBars</code> одно из значений:</p>
<ul>
<li><p><span class="bb">None</span>: без прокруток (по умолчанию)</p></li>
<li><p><span class="bb">Horizontal</span>: создает горизонтальную прокрутку при длине строки, превышающей ширину текстового поля</p></li>
<li><p><span class="bb">Vertical</span>: создает вертикальную прокрутку, если строки не помещаются в текстовом поле</p></li>
<li><p><span class="bb">Both</span>: создает вертикальную и горизонтальную прокрутку</p></li>
</ul>
<img src="pics/4.6.png" alt="Элемент TextBox в Windows Forms" />
<h3>Автозаполнение текстового поля</h3>
<p>Элемент TextBox обладает достаточными возможностями для создания автозаполняемого поля. Для этого нам надо привязать свойство 
<b>AutoCompleteCustomSource</b> элемента TextBox к некоторой коллекции, из которой берутся данные для заполнения поля.</p>
<p>Итак, добавим на форму текстовое поле и пропишем в код события загрузки следующие строки:</p>
<pre class="brush:c#;">
...
<img src="pics/4.7.png" alt="Автозаполнение текстового поля" />
<p>Режим автодополнения, представленный свойством <span class="bb">AutoCompleteMode</span>, имеет несколько возможных значений:</p>
<ul>
<li><p><span class="bb">None</span>: отсутствие автодополнения</p></li>
<li><p><span class="bb">Suggest</span>: предлагает варианты для ввода, но не дополняет</p></li>
<li><p><span class="bb">Append</span>: дополняет введенное значение до строки из списка, но не предлагает варианты для выбора</p></li>
<li><p><span class="bb">SuggestAppend</span>: одновременно и предлагает варианты для автодополнения, и дополняет введенное пользователем значение</p></li>
</ul>
<h3>Перенос по словам</h3>
<p>Чтобы текст в элементе TextBox переносился по словам, надо установить свойство <b>WordWrap</b> равным <code>true</code>. То есть если одно 
слово не умещается на строке, то но переносится на следующую. Данное свойство будет работать только для многострочных текстовых полей.</p>
<h3>Ввод пароля</h3>
<p>Также данный элемент имеет свойства, которые позволяют сделать из него поле для ввода пароля. Так, для этого надо использовать <span class="bb">PasswordChar</span> 
и <span class="bb">UseSystemPasswordChar</span>. </p>
<p>Свойство PasswordChar по умолчанию не имеет значение, если мы установим в качестве него какой-нибудь символ, то этот символ будут отображаться 
при вводе любых символов в текстовое поле.</p>
<p>Свойство UseSystemPasswordChar имеет похожее действие. Если мы установим его значение в <code>true</code>, то вместо введенных символов 
в текстовом поле будет отображаться знак пароля, принятый в системе, например, точка.</p>
<h3>Событие TextChanged</h3>
<p>Из всех событий элемента TextBox следует отметить событие <code>TextChanged</code>, которое срабатывает при изменении текста в элементе. Например, поместим 
на форму кроме текстового поля метку и сделаем так, чтобы при изменении текста в текстовом поле также менялся текст на метке:</p>
<pre class="brush:c#;">
public partial class Form1 : Form
{
    public Form1()
...
<img src="pics/4.10.png" alt="Событие TextChanged" />
