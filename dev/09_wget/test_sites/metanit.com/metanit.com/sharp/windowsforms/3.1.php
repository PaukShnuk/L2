...
<html >
<head>
<title>Контейнеры в Windows Forms | Visual C#</title>
<meta charset="utf-8" />
...
<h1>Контейнеры в Windows Forms</h1><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для организации элементов управления в связанные группы существуют специальные элементы - контейнеры. Например, Panel, FlowLayoutPanel, 
...
придает форме определенный визуальный стиль.</p>
<p>Все контейнеры имеют свойство <code>Controls</code>, которое содержит все элементы данного контейнера. Когда мы переносим какой-нибудь 
...
Либо мы также можем добавить элемент управления динамически с помощью кода в эту же коллекцию.</p>
<h2>Динамическое добавление элементов</h2>
<p>Добавим на форму кнопку динамически. Для этого добавим событие загрузки формы, в котором будет создаваться новый элемент управления. 
Это можно сделать либо с помощью кода, либо визуальным образом.</p>
<p>С помощью перетаскивания элементов с Панели Инструментов мы можем легко добавить новые элементы на форму. Однако такой способ довольно ограничен, 
поскольку очень часто требуется динамически создавать (удалять) элементы на форме.</p>
<p>Для динамического добавления элементов создадим обработчик события загрузки формы в файле кода:</p>
<pre class="brush:c#;">
private void Form1_Load(object sender, EventArgs e)
{
}
</pre>
<p>Теперь добавим в него код добавления кнопки на форму:</p>
<pre class="brush:c#;">
private void Form1_Load(object sender, EventArgs e)
{
...
<p>Сначала мы создаем кнопку и устанавливаем ее свойства. Затем, используя метод <code>Controls.Add</code> мы добавляем ее в коллекцию элементов 
формы. Если бы мы это не сделали, мы бы кнопку не увидели, поскольку в этом случае для нашей формы ее просто не существовало бы.</p>
<p>С помощью метода <code>Controls.Remove()</code> можно удалить ранее добавленный элемент с формы:</p>
<pre class="brush:c#;">
this.Controls.Remove(helloButton);
</pre>
<p>Хотя в данном случае в качестве контейнера использовалась форма, но при добавлении и удалении элементов с любого другого контейнера, например, 
GroupBox, будет применяться все те же методы.</p>
