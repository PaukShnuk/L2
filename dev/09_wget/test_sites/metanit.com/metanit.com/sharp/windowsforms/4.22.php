...
<html >
<head>
<title>ErrorProvider в Windows Forms | Visual C#</title>
<meta charset="utf-8" />
<meta name="description" content="Вывод ошибок с помощью ErrorProvider в Windows Forms и Visual C#">
...
<h2>ErrorProvider</h2><div class="date">Последнее обновление: 09.11.2016</div>
...
<p>ErrorProvider не является полноценным визуальным компонентом, тем не менее он позволяет настраивать визуальное отображение ошибок при вводе 
пользователя. Этот элемент применяется преимущестенно для проверки и индикации ошибок.</p>
<p>Так, определим на форме два текстовых поля с именами nameBox и ageBox. И далее перетащим с панели инструментов на форму элемент ErrorProvider.</p>
<p>ErrorProvider оторазится под формой, а в окне свойств мы также сможем управлять его свойствами:</p>
<img src="pics/4.63.png" alt="ErrorProvider в Windows Forms" />
<p>Среди его свойств можно выделить следующие:</p>
<ul>
<li><p><span class="b">BlinkRate</span>: задает частоту мигания значка ошибки</p></li>
<li><p><span class="b">BlinkStyle</span>: задает, когда значок ошибки будет мигать</p></li>
<li><p><span class="b">Icon</span>: устанавливает сам значок ошибки. По умолчанию это красный кружок с восклицательным знаком, но можно установить любую 
другую иконку.</p></li>
</ul>
<p>В коде формы приложения пропишем следующее:</p>
<pre class="brush:c#;">
using System;
...
<p>Здесь для обоих текстовых полей задано событие Validating, которое срабатывает при вводе пользователя и призвано управлять валидацией вода. Это событие 
...
то с помощью метода <code>errorProvider1.SetError()</code> для определенного элемента добавляем ошибку. Если все условия соблюдены, то, наоборот, удаляем все ошибки 
с помощью метода <code>errorProvider1.Clear()</code>.</p>
<p>Запустим приложение и при некорректном вводе мы увидим ошибку:</p>
<img src="pics/4.64.png" alt="Валидация в Windows Forms" />
<p>Здесь есть небольшая деталь - валидация элемента будет происходить, когда мы завершим ввод и перейдем на другой элемент. Если же нам надо валидировать 
элемент по мере ввода, то тогда мы можем обрабатывать событие TextChanged у тех же текстовых полей.</p>
...
<div class="nav"><p><a href="4.21.php">Назад</a><a href="./">Содержание</a><a href="5.1.php">Вперед</a></p></div></div>
<div class="socBlock">
