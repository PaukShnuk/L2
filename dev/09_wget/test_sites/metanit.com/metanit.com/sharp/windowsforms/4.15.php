...
<html >
<head>
<title>DateTimePicker и MonthCalendar в Windows Forms | Visual C#</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с датами в Windows Forms и Visual C#, элементы DateTimePicker и MonthCalendar">
...
<h2>DateTimePicker и MonthCalendar</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для работы с датами в Windows Forms имеются элементы DateTimePicker и MonthCalendar.</p>
<h3>DateTimePicker</h3>
<p>DateTimePicker представляет раскрывающийся по нажатию календарь, в котором можно выбрать дату.  собой элемент, который с помощью перемещения ползунка позволяет вводить числовые значения.</p>
<img src="pics/4.47.png" alt="Элемент DateTimePicker в Windows Forms" />
<p>Наиболее важные свойства DateTimePicker:</p>
<ul>
<li><p><span class="b">Format</span>: определяет формат отображения даты в элементе управления. Может принимать следующие значения:</p>
<p><code>Custom</code>: формат задается разработчиком</p>
<p><code>Long</code>: полная дата</p>
<p><code>Short</code>: дата в сокращенном формате</p>
<p><code>Time</code>: формат для работы с временем</p></li>
<li><p><span class="b">CustomFormat</span>: задает формат отображения даты, если для свойства <code>Format</code> установлено 
значение <code>Custom</code></p></li>
<li><p><span class="b">MinDate</span>: минимальная дата, которую можно выбрать</p></li>
<li><p><span class="b">MaxDate</span>: наибольшая дата, которую можно выбрать</p></li>
<li><p><span class="b">Value</span>: определеяте текущее выбранное значение в DateTimePicker</p></li>
<li><p><span class="b">Text</span>: представляет тот текст, который отображается в элементе</p></li>
</ul>
<p>При выборе даты элемент генерирует событие <code>ValueChanged</code>. Например, обработаем данное собатие и присвоим выбранное значение 
тексту метки:</p>
<pre class="brush:c#;">
...
<p>Свойство Value хранит объект DateTime, поэтому с ним можно работать как и с любой другой датой. В данном случае выбранная дата 
преобразуется в строку времени.</p>
<img src="pics/4.48.png" alt="Время в DateTimePicker" />
<p>В вышеприведенном случае значение <code>dateTimePicker1.Value.ToLongTimeString()</code> аналогично тому тексту, который отображается в элементе. 
И мы могли бы написать так:</p>
<pre class="brush:c#;">
label1.Text = String.Format("Вы выбрали: {0}", dateTimePicker1.Text);
</pre>
<h3>MonthCalendar</h3>
<p>С помощью MonthCalendar также можно выбрать дату, только в данном случае этот элемент представляет сам календарь, который не надо раскрывать:</p>
<img src="pics/4.49.png" alt="MonthCalendar в Windows Forms" />
<p>Рассмотрим некоторые основные свойства элемента.</p>
<p>Свойства выделения дат:</p>
<ul>
<li><p><span class="b">AnnuallyBoldedDates</span>: содержит набор дат, которые будут отмечены жирным в календаре для каждого года</p></li>
<li><p><span class="b">BoldedDates</span>: содержит набор дат, которые будут отмечены жирным (только для текущего года)</p></li>
<li><p><span class="b">MonthlyBoldedDates</span>: содержит набор дат, которые будут отмечены жирным для каждого месяца</p></li>
</ul>
<p>Добавление выделенных дат делается с помощью определенных методов (как и удаление):</p>
<pre class="brush:c#;">
...
<img src="pics/4.50.png" alt="Выделение дат в MonthCalendar" />
<p>Для снятия выделения можно использовать аналоги этих методов:</p>
<pre class="brush:c#;">
...
<p>Свойства для определения дат в календаре:</p>
<ul>
<li><p><span class="b">MinDate</span>: определяет минимальную дату для выбора в календаре</p></li>
<li><p><span class="b">MaxDate</span>: задает наибольшую дату для выбора в календаре</p></li>
<li><p><span class="b">FirstDayOfWeek</span>: определяет день недели, с которого должна начинаться неделя в календаре</p></li>
<li><p><span class="b">SelectionRange</span>: определяет диапазон выделенных дат</p></li>
<li><p><span class="b">SelectionEnd</span>: задает конечную дату выделения</p></li>
<li><p><span class="b">SelectionStart</span>: определяет начальную дату выделения</p></li>
<li><p><span class="b">ShowToday</span>: при значении true отображает внизу календаря текущую дату</p></li>
<li><p><span class="b">ShowTodayCircle</span>: при значении true текущая дата будет обведена кружочком</p></li>
<li><p><span class="b">TodayDate</span>: определяет текущую дату. По умолчанию используется системная дата на компьютере, но с помощью данного 
свойства мы можем ее изменить</p></li>
</ul>
<p>Например, при установке свойств:</p>
<pre class="brush:c#;">
...
<p>будет следующее отображение календаря:</p>
<img src="pics/4.51.png" />
<p>Наиболее интересными событиями элемента являются события <code>DateChanged</code> и <code>DateSelected</code>, которые возникают 
при изменении выбранной в элементе даты. Однако надо учитывать, что выбранная дата будет представлять первую дату из диапазона выделенных дат:</p>
<pre class="brush:c#;">
public partial class Form1 : Form
{
    public Form1()
...
<img src="pics/4.52.png" alt="Выбор даты в MonthCalendar" />
