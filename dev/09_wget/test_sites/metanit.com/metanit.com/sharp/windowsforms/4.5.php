...
<html >
<head>
<title>Элементы Radiobutton и CheckBox в Windows Forms | Visual C#</title>
<meta charset="utf-8" />
...
<h2>Элементы Radiobutton и CheckBox</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>CheckBox</h3>
<p>Элемент CheckBox или флажок предназначен для установки одного из двух значений: отмечен или не отмечен. Чтобы отметить флажок, надо установить 
у его свойства <span class="bb">Checked</span> значение <code>true</code>.</p>
<p>Кроме свойства <code>Checked</code> у элемента CheckBox имеется свойство <span class="bb">CheckState</span>, которое позволяет задать для флажка 
одно из трех состояний - Checked (отмечен), Indeterminate (флажок не определен - отмечен, но находится в неактивном состоянии) и Unchecked (не отмечен)</p>
<img src="pics/4.11.png" alt="Элемент CheckBox в Windows Forms" />
<p>Также следует отметить свойство <code>AutoCheck</code> - если оно имеет значение <code>false</code>, то мы не можем изменять состояние флажка. 
По умолчанию оно имеет значение <code>true</code>.</p>
<p>При изменении состояния флажка он генерирует событие <span class="bb">CheckedChanged</span>. Обрабатывая это событие, мы можем получать измененный флажок 
и производить определенные действия:</p>
<pre class="brush:c#;">
...
<h3>Radiobutton</h3>
<p>На элемент CheckBox похож элемент RadioButton или переключатель. Переключатели располагаются группами, и включение одного переключателя 
означает отключение всех остальных.</p>
<p>Чтобы установить у переключателя включенное состояние, надо присвоить его свойству <code>Checked</code> значение <code>true</code>.</p>
<p>Для создания группы переключателей, из которых можно бы было выбирать, надо поместить несколько переключателей в какой-нибудь контейнер, например, 
в элементы GroupBox или Panel. Переключатели. находящиеся в разных контейнерах, будут относиться к разным группам:</p>
<img src="pics/4.12.png" alt="Элемент RadioButton в Windows Forms" />
<p>Похожим образом мы можем перехватывать переключение переключателей в группе, обрабатывая событие <code>CheckedChanged</code>. 
Связав каждый переключатель группы с одним обработчиком данного события, мы сможем получить тот переключатель, который в данный момент выбран:</p>
<pre class="brush:c#;">
