...
<html >
<head>
<title>TrackBar, Timer и ProgressBar в Windows Forms | Visual C#</title>
<meta charset="utf-8" />
<meta name="description" content="Элементы TrackBar, Timer и ProgressBar в Windows Forms и Visual C#">
...
<h2>TrackBar, Timer и ProgressBar</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>TrackBar</h3>
<p>TrackBar представляет собой элемент, который с помощью перемещения ползунка позволяет вводить числовые значения.</p>
<img src="pics/4.42.png" alt="Элемент TrackBar в Windows Forms" />
<p>Некоторые важные свойства TrackBar:</p>
<ul>
<li><p><span class="bb">Orientation</span>: задает ориентацию ползунка - расположение по горизонтали или по вертикали</p></li>
<li><p><span class="bb">TickStyle</span>: задает расположение делений на ползунке</p></li>
<li><p><span class="bb">TickFrequency</span>: задает частоту делений на ползунке</p></li>
<li><p><span class="bb">Minimum</span>: минимальное возможное значение на ползунке (по умолчанию 0)</p></li>
<li><p><span class="bb">Maximum</span>: максимальное возможное значение на ползунке (по умолчанию 10)</p></li>
<li><p><span class="bb">Value</span>: текущее значение ползунка. Должно находиться между Minimum и Maximum</p></li>
</ul>
<p>Свойство <code>TickStyle</code> может принимать ряд значений:</p>
<ul>
<li><p><code>None</code>: деления отсутствуют</p></li>
<li><p><code>Both</code>: деления расположены по обеим сторонам ползунка</p></li>
<li><p><code>BottomRight</code>: у вертикального ползунка деления находятся справа, а у горизонтального - снизу</p></li>
<li><p><code>TopLeft</code>: у вертикального ползунка деления находятся слева, а у горизонтального - сверху (применяется по умолчанию)</p></li>
</ul>
<p>К наиболее важным событиям элемента следует отнести событие <span class="bb">Scroll</span>, которое позволяет обработать перемещение 
...
зависимости от значения ползунка, либо какике-нибудь другие настройки:</p>
<pre class="brush:c#;">
...
<img src="pics/4.43.png" alt="Перемещение ползунка в Windows Forms" />

<h3>Timer</h3>
<p>Timer является компонентом для запуска действий, повторяющихся через определенный промежуток времени. Хотя он не является 
визуальным элементом, но его аткже можно перетащить с Панели Инструментов на форму:</p>
<img src="pics/4.44.png" alt="Timer в Windows Forms" />
<p>Наиболее важные свойства и методы таймера:</p>
<ul>
<li><p>Свойство <span class="bb">Enabled</span>: при значении true указывает, что таймер будет запускаться вместе с запуском формы</p></li>
<li><p>Свойство <span class="bb">Interval</span>: указывает интервал в миллисекундах, через который будет срабатывать обработчик события Tick, которое есть у таймера</p></li>
<li><p>Метод <span class="bb">Start()</span>: запускает таймер</p></li>
<li><p>Метод <span class="bb">Stop()</span>: останавливает таймер</p></li>
</ul>
<p>Для примера определим простую форму, на которую добавим кнопку и таймер. В файле кода формы определим следующий код:</p>
<pre class="brush:c#;">
public partial class Form1 : Form
{
    int koef = 1;
...
<p>Здесь в конструкторе формы устанавливаются начальные значения для таймера, кнопки и формы.</p>
<p>Через каждый интервал таймера будет срабатывать обработчик <code>timer1_Tick</code>, в котором изменяется положение 
кнопки по горизонтали с помощью свойства <code>button1.Left</code>. А с помощью дополнительной переменной <code>koef</code> 
можно управлять направлением движения.</p>
<p>Кроме того, с помощью обраотчика нажатия кнопки <code>button1_Click</code> можно либо остановить таймер (и вместе с ним движение 
кнопки), либо опять его запустить.</p>
<img src="pics/4.45.png" />
<h3>Индикатор прогресса ProgressBar</h3>
<p>Элемент ProgressBar служит для того, чтобы дать пользователю информацию о ходе выполнения какой-либо задачи.</p>
<img src="pics/4.46.png" alt="ProgressBar в Windows Forms" />
<p>Наиболее важые свойства ProgressBar:</p>
<ul>
<li><p><span class="bb">Minimum</span>: минимальное возможное значение</p></li>
<li><p><span class="bb">Maximum</span>: максимальное возможное значение</p></li>
<li><p><span class="bb">Value</span>: текущее значение элемента</p></li>
<li><p><span class="bb">Step</span>: шаг, на который изменится значение Value при вызове метода <code>PerformStep</code></p></li>
</ul>
<p>Для имитации работы прогрессбара поместим на форму таймер и в коде формы определим следующий код:</p>
<pre class="brush:c#;">
public partial class Form1 : Form
{        
    public Form1()
...
<div class="nav"><p><a href="4.13.php">Назад</a><a href="./">Содержание</a><a href="4.15.php">Вперед</a></p></div></div>
<div class="socBlock">
