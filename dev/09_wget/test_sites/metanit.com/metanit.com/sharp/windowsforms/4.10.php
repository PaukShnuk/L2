...
<html >
<head>
<title>Элементы NumericUpDown и DomainUpDown в Windows Forms | Visual C#</title>
<meta charset="utf-8" />
...
<h2>Элементы NumericUpDown и DomainUpDown</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>NumericUpDown</h3>
<p>Элемент NumericUpDown представляет пользователю выбор числа из определенного диапазона. Для определения диапазона чисел для выбора NumericUpDown имеет два свойства: 
<span class="bb">Minimum</span> (задает минимальное число) и <span class="bb">Maximum</span> (задает максимальное число).</p>
<p>Само значение элемента хранится в свойстве <span class="bb">Value</span>:</p>
<img src="pics/4.22.png" alt="NumericUpDown в Windows Forms" />
<p>По умолчанию элемент отображает десятичные числа. Однако если мы установим его свойство <span class="bb">Hexadecimal</span> равным <code>true</code>, 
то элемент будет отображать все числа в шестнадцатеричной системе.</p>
<img src="pics/4.23.png" alt="Шестнадцатиричные числа в NumericUpDown" />
<p>Даже если мы в коде установим обычное десятичное значение:</p>
<pre class="brush:c#;">numericUpDown1.Value = 66;</pre>
<p>то элемент все равно отобразит его в шестнадцатиричной системе.</p>
<p>Если мы хотим отображать в поле дробные числа, то можно использовать свойство <span class="bb">DecimalPlaces</span>, которое указывает, 
сколько знаков после запятой должно отображаться. По умолчанию это свойство равно нулю.</p>
<p>Также можно задать отображение тысячного разделителя. Для этого для свойства <span class="bb">ThousandsSeparator</span> надо установить значение 
<code>true</code>. Например, numericUpDown при <code>Value=1000,03</code>, <code>DecimalPlaces=2</code> и <code>ThousandsSeparator=true</code>:</p>
<img src="pics/4.24.png" />
<p>При этом надо учитывать, что если мы устанавливаем значение для свойства Value в окне свойств, то там в качестве разделителя цеой и дробной части 
используется запятая. Если же мы устанавливаем данное свойство в коде, тогда в качестве разделителя используется точка.</p>
<p>По умолчанию при нажатии на стрелочки вверх-вниз на элементе значение будет увеличиваться, либо уменьшаться на единицу. Но с помощью свойства 
<span class="bb">Increment</span> можно задать другой шаг приращения, в том числе и дробный.</p>
<p>При работе с NumericUpDown следует учитывать, что его свойство <code>Value</code> (как и свойства <code>Minimum</code> и <code>Maximum</code>) 
хранит значение decimal. Поэтому в коде мы также должны с ним работать как с decimal, а не как с типом int или double.</p>

<h3>DomainUpDown</h3>
<p>Элемент DomainUpDown предназначен для ввода текстовой информации. Он имеет текстовое поле для ввода строки и две стрелки для перемещения по списку строк:</p>
<img src="pics/4.25.png" alt="Элемент DomainUpDown в Windows Forms" />
<p>Список для DomainUpDown задается с помощью свойства <span class="bb">Items</span>. Список можно сразу упорядочить по алфавиту. Для этого надо свойству 
<span class="bb">Sorted</span> присвоить значение <code>true</code>.</p>
<p>Чтобы можно было циклично перемещаться по списку, то есть при достижении конца или начала списка его просмотр начинался с первого или последнего элемента, 
надо установить для свойства <span class="bb">Wrap</span> значение <code>true</code>.</p>
<p>В коде выбранное значение в DomainUpDown доступно через свойство Text. Например, добавим программно список строк в DomainUpDown и обработаем изменение выбора 
в списке:</p>
<pre class="brush:c#;">
...
<p>Для обработки изменения текста здесь таке, как и для элемента TextBox, можно использовать событие <code>TextChanged</code>, в обработчике которого 
мы выводим выбранный текст в сообщение.</p>
