...
<html >
<head>
<title>Создание непрямоугольных форм. Закрытие формы | Visual C#</title>
<meta charset="utf-8" />
<meta name="description" content="Создание непрямоугольных форм в Windows Forms и Visual C#">
...
<h2>Создание непрямоугольных форм. Закрытие формы</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>По умолчанию все формы в Windows Forms являются прямоугольными. Однако мы можем создавать и непрямоугольные произвольные формы. 
Для этого используется свойство <span class="bb">Region</span>. В качестве значения оно принимает объект одноименного класса Region.</p>
<p>При создании непрямоугольных форм, как правило, не используются границы формы, так как границы задаются этим объектом 
Region. Чтобы убрать границы формы, надо присвоить у формы свойству <code>FormBorderStyle</code> значение <code>None</code>.</p>
<p>И еще один аспект, который надо учитывать, заключается в перемещении, закрытии, максимизации и минимизации форм. То есть в данном случае , 
...
дополнительно определять для этого программную логику.</p>
<p>Итак, перейдем к коду формы и изменим его следующим образом:</p>
<pre class="brush:c#;">
using System;
...
<img src="pics/2.6.png" alt="Непрямоугольные формы в Windows Forms" />
<p>Создание области формы происходит в обработчике события Form1_Load. Для создания области используется графический путь - объект класса 
<code>System.Drawing.Drawing2D.GraphicsPath</code>, в который добавляется эллипс. Графический путь позволяет создать фигуру любой формы, 
поэтому, если мы захотим форму в виде морской звезды, то нам просто надо должным образом настроить используемый графический путь.</p>
<p>Для закрытия формы в обработчике события нажатия кнопки <code>button1_Click</code> форма закрывается программным образом: <code>this.Close()</code></p>
<p>Для перемещения формы обрабатываются два события формы - событие нажатия кнопки мыши и событие перемещения указателя мыши.</p>
