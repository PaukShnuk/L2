...
<html >
<head>
<title> C# и Windows Forms | Создание графического приложения</title>
<meta charset="utf-8" />
...
<h2>Создание графического приложения</h2><div class="date">Последнее обновление: 13.11.2021</div>
...
<h3>Графический редактор Windows Forms</h3>
<p>Одним из преимуществ разработки в Visual Studio приложений Windows Forms является наличие графического редактора, который позволяет в графическом виде представить 
создаваемую форму и в принципе упрощает работу с графическими компонентами.</p>
<p>Для открытия формы в режиме графического дизайнера нажмем на в структуре проекта на файл <span class="b">Form1.cs</span> либо левой кнопкой мыши двойным кликом, 
либо правой кнопкой мыши и в появившемся контекстном меню выберем <span class="b">View Designer</span> (также можно использовать комбинацию клавиш Shift+F7)</p>
<img src="./pics/1.2.png" alt="первое приложение Windows Forms на C#" />
<p>После этого в Visual Studio откроется выбранная форма в графическом виде.</p>
<p>В то же время следует отметить, что на более слабых компьютерах окно дизайнера можно открываться довольно долго, а на некоторых - довольно слабых компьютерах может и вообще не открыться.</p>
<p>При выборе формы в окне дизайнера внизу справа под структурой проекта мы сможем найти окно <span class="b">Properties</span>(Свойства). 
Так как у меня в данный момент выбрана форма как элемент управления, то в этом поле отображаются свойства, связанные с формой.</p>
<p>Теперь найдем в этом окне свойство формы Text и изменим его значение на любое другое:</p>
<img src="./pics/1.3.png" alt="окно свойств и Windows forms в Visual Studio" />
<p>Таким образом мы поменяли заголовок формы. И подобным образом мы можем поменять другие свойства формы, которые доступны в окне свойств.</p>
<p>Но Visual Studio имеет еще одну связанную функциональность. Она обладает панелью графических инструментов. И мы можем, вместо создания элементов управления в коде C#, 
...
найдем в левой части Visual Studio вкладку <span class="b">Toolbox (Панель инструментов)</span>. Нажмем на эту вкладку, и у нас откроется панель с элементами, 
откуда мы можем с помощью мыши перенести на форму любой элемент:</p>
<img src="./pics/1.4.png" alt="панель инструментов Toolbox и Windows forms в Visual Studio" />
<p>Найдем среди элементов кнопку и, захватив ее указателем мыши, перенесем на форму:</p>
<img src="./pics/1.5.png" alt="Button from Toolbox и Windows forms в Visual Studio" />
<p>Причем при выборе кнопки она открывается в окне свойств и, как и для всей формы, для кнопки в окне свойств мы можем изменить значения различных свойств.</p>
<p>Кроме того, если после переноса кнопки на форму мы откроем файл <span class="b">Form1.</span></p>
<pre class="brush:c#;">
 namespace HelloApp
{
    partial class Form1
...
<p>Мы видим, что в класс Form1 была добавлена переменная <code>button1</code> типа Button и для этой переменной, как и для объекта формы, задан ряд свойств. 
И если в окне свойств мы поменяем значения этих свойств, то в этом файле также изменяться их значения. Как в случае с текстом формы:</p>
<pre class="brush:c#;">this.Text = "METANIT.COM";</pre>
<p>Это визуальная часть. Теперь приступим к самому программированию. Добавим простейший код на языке C#, который бы выводил сообщение по нажатию кнопки. 
Для этого перейдем в файл кода <span class="b">Form1.cs</span>, который связан с этой формой. По умолчанию после создания проекта он имеет код типа следующего:</p>
<pre class="brush:c#;">
namespace HelloApp
{
...
<p>Кнопка обладает событием <span class="b">Click</span>, которое генерируется при нажатии. В данном случае в конструкторе формы мы подвязываем к кнопке button1 в качестве обработчика события 
нажатия метод <code>button1_Click</code>, в котором с помощью метода <span class="b">MessageBox.Show</span> выводит сообщение. Текст сообщения передается в метод в качестве параметра.</p>
<p>Стоит отметить, что графический дизайнер позволяет автоматически сгенерировать обработчик нажатия кнопки. Для этого надо 
в окне дизайнера нажать на кнопку на форме двойным щелчком мыши.</p>
<p>Теперь запустим проект и мы увидим форму с кнопкой, на которую мы можем нажать и получить сообщение:</p>
<img src="./pics/1.6.png" alt="Первое приложение в Windows Forms и C# в Visual Studio" />
