...
<html >
<head>
<title>Панель вкладок TabControl и SplitContainer в Windows Forms | Visual C#</title>
<meta charset="utf-8" />
...
<h2>Панель вкладок TabControl и SplitContainer</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>TabControl</h3>
<p>Элемент TabControl позволяет создать элемент управления с несколькими вкладками. И каждая вкладка будет хранить некоторый набор других элементов управления, 
как кнопки, текстовые поля и др. Каждая вкладка представлена классом <b>TabPage</b>.</p>
<p>Чтобы настроить вкладки элемента TabControl используем свойство <b>TabPages</b>. При переносе элемента TabControl с панели инструментов на форму 
по умолчанию создаются две вкладки - tabPage1 и tabPage2. Изменим их отображение с помощью свойства TabPages:</p>
<img src="pics/3.11.png" alt="Свойство TabPages" />
<p>Нам откроется окно редактирования/добавления и удаления вкладок:</p>
<img src="pics/3.12.png" alt="Изменение вкладок" />
<p>Каждая вкладка представляет своего рода панель, на которую мы можем добавить другие элементы управления, 
а также заголовок, с помощью которого мы можем переключаться по вкладкам. Текст заголовка задается с помощью свойства Text.</p>
<img src="pics/3.13.png" />
<h4>Управление вкладками в коде</h4>
<p>Для добавления новой вкладки нам надо ее создать и добавить в коллекцию tabControl1.TabPages с помощью метода Add:</p>
<pre class="brush:c#;">
//добавление вкладки
...
<p>Получая в коллекции tabControl1.TabPages нужную вкладку по индексу, мы можем ей легко манипулировать:</p>
<pre class="brush:c#;">
// изменение свойств
tabControl1.TabPages[0].Text = "Первая вкладка";
</pre>
<h3>SplitContainer</h3>
<p>Элемент SplitContainer позволяет создавать две разделенные сплитером панели. Изменяя положение сплитера, можно изменить размеры этих панелей.</p>
<img src="pics/3.15.png" alt="Элемент SplitContainer" />
<p>Используя свойство <code>Orientation</code>, можно задать горизонтальное или вертикальное отображение сплитера на форму. В данном случае 
это свойство принимает значения Horisontal и Vertical соответственно.</p>
<p>В случае, когда надо запретить изменение положения сплиттера, то можно присвоить свойству <code>IsSplitterFixed</code> значение <code>true</code>. 
Таким образом, сплитер окажется фиксированным, и мы не сможем поменять его положение.</p>
<p>По умолчанию при растяжении формы или ее сужении также будет меняться размер обеих панелей сплитконтейнера. Однако мы можем закрепить за одной панелью 
...
SplitContainer свойство <code>FixedPanel</code>. В качестве значения оно принимает панель, которую надо зафиксировать:</p>
<img src="pics/3.14.png" alt="Фиксированная панель в SplitContainer" />
<p>Чтобы изменить положение сплитера в коде, мы можем управлять свойством <span class="bb">SplitterDistance</span>, которое задает положение сплиттера в пикселях 
от левого или верхнего края элемента SplitContainer. А с помощью свойства <code>SplitterIncrement</code> можно задать шаг, на который будет 
перемещаться сплиттер при движении его с помощью клавиш-стрелок.</p>
<p>Чтобы скрыть одну из двух панелей, мы можем установить свойство <code>Panel1Collapsed</code> или <code>Panel2Collapsed</code> в 
<code>true</code></p>
