...
<html >
<head>
<title>Метки и ссылки в Windows Forms | Visual C#</title>
<meta charset="utf-8" />
...
<h2>Метки и ссылки</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>Label</h3>
<p>Для отображения простого текста на форме, доступного только для чтения, служит элемент Label. Чтобы задать отображаемый текст метки, 
надо установить свойство <code>Text</code> элемента.</p>
<h3>LinkLabel</h3>
<p>Особый тип меток представляют элементы LinkLabel, которые предназначены для вывода ссылок, которые аналогичны ссылкам, размещенным на стандартных веб-станиц.</p>
<p>Также, как и с обычными ссылками на веб-страницах, мы можем по отношению к данному элементу определить три цвета:</p>
<ul>
<li><p>Свойство <span class="bb">ActiveLinkColor</span> задает цвет ссылки при нажатии</p></li>
<li><p>Свойство <span class="bb">LinkColor</span> задает цвет ссылки до нажатия, по которой еще не было переходов</p></li>
<li><p>Свойство <span class="bb">VisitedLinkColor</span> задает цвет ссылки, по которой уже были переходы</p></li>
</ul>
<p>Кроме цвета ссылки для данного элемента мы можем задать свойство <span class="bb">LinkBehavior</span>, которое управляет поведением ссылки. Это свойство 
принимает четыре возможных значения:</p>
<ul>
<li><p><span class="bb">SystemDefault</span>: для ссылки устанавливаются системные настройки</p></li>
<li><p><span class="bb">AlwaysUnderline</span>: ссылка всегда подчеркивается</p></li>
<li><p><span class="bb">HoverUnderline</span>: ссылка подчеркивается только при наведении на нее курсора мыши</p></li>
<li><p><span class="bb">NeverUnderline</span>: ссылка никогда не подчеркивается</p></li>
</ul>
<p>По умолчанию весь текст на данном элементе считается ссылкой. Однако с помощью свойства <span class="bb">LinkArea</span> мы можем изменить область ссылки. 
Например, мы не хотим включать в ссылку первые шесть символов. Для этого задаем подсвойство <code>Start</code>:</p>
<img src="pics/4.4.png" alt="Настройка элемента LinkLabel в Windows Forms" />
<p>Чтобы выполнить переход по ссылке по нажатию на нее, надо дополнительно написать код. Данный код должен обрабатывать событие <code>LinkClicked</code>, 
которое есть у элемента LinkLabel. Например, пусть у нас на форме есть элемент ссылки называется linkLabel1 и который содержит некоторую ссылку:</p>
<img src="pics/4.5.png" alt="LinkLabel в Windows Forms" />
<p>Чтобы перейти по ссылке, зададим обработчик LinkClicked:</p>
<pre class="brush:c#;">
...
<p>Метод <code>System.Diagnostics.Process.Start()</code> откроет данную ссылку в веб-браузере, который установлен в системе браузером по умолчанию.</p>
