...
<html >
<head>
<title>Окно сообщения MessageBox в Windows Forms | Visual C#</title>
<meta charset="utf-8" />
...
<h2>Окно сообщения MessageBox</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Как правило, для вывода сообщений применяется элемент MessageBox. Однако кроме сообственно вывода строки сообщения данный элемент может устанавливать 
ряд настроек, которые определяют его поведение.</p>
<p>Для вывода сообщения в классе MessageBox предусмотрен метод <span class="bb">Show</span>, который имеет различные версии и может принимать ряд параметров. 
Рассмотрим одну из наиболее используемых версий:</p>
<pre class="brush:c#;">
...
<p>Здесь применяются следующие параметры:</p>
<p><code>text</code>: текст сообщения</p>
<p><code>caption</code>: текст заголовка окна сообщения</p>
<p><code>buttons</code>: кнопки, используемые в окне сообщения. 
Принимает одно из значений перечисления <span class="bb">MessageBoxButtons</span>:</p>
<ul>
<li><p><code>AbortRetryIgnore</code>: три кнопки Abort (Отмена), Retry (Повтор), Ignore (Пропустить)</p></li>
<li><p><code>OK</code>: одна кнопка OK</p></li>
<li><p><code>OKCancel</code>: две кнопки OK и Cancel (Отмена)</p></li>
<li><p><code>RetryCancel</code>: две кнопки Retry (Повтор) и Cancel (Отмена)</p></li>
<li><p><code>YesNo</code>: две кнопки Yes и No</p></li>
<li><p><code>YesNoCancel</code>: три кнопки Yes, No и Cancel (Отмена)</p></li>
</ul>
<p>Таким образом, в зависимости от выбора окно сообщения может иметь от одной до трех кнопок.</p>
<p><code>icon</code>: значок окна сообщения. Может принимать одно из следующих значений перечисления <span class="bb">MessageBoxIcon</span>:</p>
<ul>
<li><p><code>Asterisk, Information</code>: значок, состоящий из буквы i в нижнем регистре, помещенной в кружок</p></li>
<li><p><code>Error, Hand, Stop</code>: значок, состоящий из белого знака "X" на круге красного цвета.</p></li>
<li><p><code>Exclamation, Warning</code>: значок, состоящий из восклицательного знака в желтом треугольнике</p></li>
<li><p><code>Question</code>: значок, состоящий из вопросительного знака на периметре круга</p></li>
<li><p><code>None</code>: значок у сообщения отсутствует</p></li>
</ul>
<p><code>defaultButton</code>: кнопка, на которую по умолчанию устанавливается фокус. Принимает одно из значений перечисления <span class="bb">MessageBoxDefaultButton</span>:</p>
<ul>
<li><p><code>Button1</code>: первая кнопка из тех, которые задаются перечислением MessageBoxButtons</p></li>
<li><p><code>Button2</code>: вторая кнопка</p></li>
<li><p><code>Button3</code>: третья кнопка</p></li>
</ul>
<p><code>options</code>: параметры окна сообщения. Принимает одно из значений перечисления <span class="bb">MessageBoxOptions</span>:</p>
<ul>
<li><p><code>DefaultDesktopOnly</code>: окно сообщения отображается на активном рабочем столе.</p></li>
<li><p><code>RightAlign</code>: текст окна сообщения выравнивается по правому краю</p></li>
<li><p><code>RtlReading</code>: все элементы окна располагаются в обратном порядке справа налево</p></li>
<li><p><code>ServiceNotification</code>: окно сообщения отображается на активном рабочем столе, даже если в системе не зарегистрирован ни один пользователь</p></li>
</ul>
<p>Нередко используется один параметр - текст сообщения. Но посмотрим, как использовать остальные параметры. Пусть у нас есть кнопка, в обработчике 
нажатия которой открывается следующее окно сообщения:</p>
<pre class="brush:c#;">
...
<img src="pics/4.59.png" alt="MessageBox в Windows Forms" />
<p>Однако нам не просто дается возможность установки кнопок в окне сообщения. Метод <code>MessageBox.Show</code> возвращает объект 
<span class="bb">DialogResult</span>, с помощью которого мы можем узнать, какую кнопку в окне сообщения нажал пользователь. DialogResult представляет 
перечисление, в котором определены следующие значения:</p>
<ul>
<li><p><code>Abort</code>: нажата кнопка Abort</p></li>
<li><p><code>Retry</code>: нажата кнопка Retry</p></li>
<li><p><code>Ignore</code>: нажата кнопка Ignore</p></li>
<li><p><code>OK</code>: нажата кнопка OK</p></li>
<li><p><code>Cancel</code>: нажата кнопка Cancel</p></li>
<li><p><code>None</code>: отсутствие результата</p></li>
<li><p><code>Yes</code>: нажата кнопка Yes и No</p></li>
<li><p><code>No</code>: нажата кнопка No</p></li>
</ul>
<p>Используем обработку выбора пользователя, изменив обработчик нажатия кнопки следующим образом:</p>
<pre class="brush:c#;">
private void button1_Click(object sender, EventArgs e)
{
...
<p>И теперь, если в окне сообщения мы выберем выриант Yes, то кнопка окрасится в красный цвет.</p>
...
<div class="nav"><p><a href="4.18.php">Назад</a><a href="./">Содержание</a><a href="4.20.php">Вперед</a></p></div></div>
<div class="socBlock">
