...
<html >
<head>
<title>Элемент ListBox в Windows Forms | Visual C#</title>
<meta charset="utf-8" />
...
<h2>ListBox</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Элемент ListBox представляет собой простой список. Ключевым свойством этого элемента является свойство <span class="bb">Items</span>, которое как раз и хранит набор 
всех элементов списка.</p>
<p>Элементы в список могут добавляться как во время разработки, так и программным способом. В Visual Studio в окне Properties (Свойства) для 
элемента ListBox мы можем найти свойство Items. После двойного щелчка на свойство нам отобразится окно для добавления элементов в список:</p>
<img src="pics/4.13.png" alt="Добавление объектов в элемент ListBox" />
<p>В пустое поле мы вводим по одному элементу списка - по одному на каждой строке. После этого все добавленные нами элементы окажутся 
в списке, и мы сможем ими управлять:</p>
<img src="pics/4.14.png" alt="Элемент ListBox в Windows Forms" />
<h3>Программное управление элементами в ListBox</h3>
<h4>Добавление элементов</h4>
<p>Итак, все элементы списка входят в свойство <code>Items</code>, которое представляет собой коллекцию. Для добавления нового элемента в 
эту коллекцию, а значит и в список, надо использовать метод Add, например: <code>listBox1.Items.Add("Новый элемент");</code>. 
При использовании этого метода каждый добавляемый элемент добавляется в конец списка.</p>
<p>Можно добавить сразу несколько элементов, например, массив. Для этого используется метод <code>AddRange</code>:</p>
<pre class="brush:c#;">
string[] countries = { "Бразилия", "Аргентина", "Чили", "Уругвай", "Колумбия" };
listBox1.Items.AddRange(countries);
</pre>
<h4>Вставка элементов</h4>
<p>В отличие от простого добавления вставка производится по определенному индексу списка с помощью метода <code>Insert</code>:</p>
<pre class="brush:c#;">
listBox1.Items.Insert(1, "Парагвай");
</pre>
<p>В данном случае вставляем элемент на вторую позицию в списке, так как отсчет позиций начинается с нуля.</p>
<h4>Удаление элементов</h4>
<p>Для удаления элемента по его тексту используется метод <code>Remove</code>:</p>
<pre class="brush:c#;">
listBox1.Items.Remove("Чили");
</pre>
<p>Чтобы удалить элемент по его индексу в списке, используется метод <code>RemoveAt</code>:</p>
<pre class="brush:c#;">
listBox1.Items.RemoveAt(1);
</pre>
<p>Кроме того, можно очистить сразу весь список, применив метод <code>Clear</code>:</p>
<pre class="brush:c#;">
listBox1.Items.Clear();
</pre>
<h4>Доступ к элементам списка</h4>
<p>Используя индекс элемента, можно сам элемент в списке. Например, получим первый элемент списка:</p>
<pre class="brush:c#;">
string firstElement = listBox1.Items[0];
</pre>
<p>Свойство <code>Count</code> возвращает количество элементов в списке:</p>
<pre class="brush:c#;">
int number = listBox1.Items.Count;
</pre>
<h4>Выделение элементов списка</h4>
<p>При выделении элементов списка мы можем ими управлять как через индекс, так и через сам выделенный элемент. Получить выделенные 
элементы можно с помощью следующих свойств элемента ListBox:</p>
<ul>
<li><p><span class="bb">SelectedIndex</span>: возвращает или устанавливает номер выделенного элемента списка. Если выделенные элементы отсутствуют, тогда 
свойство имеет значение -1</p></li>
<li><p><span class="bb">SelectedIndices</span>: возвращает или устанавливает коллекцию выделенных элементов в виде набора их индексов</p></li>
<li><p><b>SelectedItem</b>: возвращает или устанавливает текст выделенного элемента</p></li>
<li><p><span class="bb">SelectedItems</span>: возвращает или устанавливает выделенные элементы в виде коллекции</p></li>
</ul>
<p>По умолчанию список поддерживает выделение одного элемента. Чтобы добавить возможность выделения нескольких элементов, 
надо установить у его свойства <code>SelectionMode</code> значение <code>MultiSimple</code>.</p>
<p>Чтобы выделить элемент програмно, надо применить метод <code>SetSelected(int index, bool value)</code>, 
где index - номер выделенного элемента. Если второй параметр - value имеет значение <code>true</code>, то элемент по указанному индексу 
выделяется, если <code>false</code>, то выделение наоборот скрывается:</p>
<pre class="brush:c#;">
listBox1.SetSelected(2, true); // будет выделен третий элемент
</pre>
<p>Чтобы снять выделение со всех выделенных элементов, используется метод <code>ClearSelected</code>.</p>
<h3>Событие SelectedIndexChanged</h3>
<p>Из всех событий элемента ListBox надо отметить в первую очередь событие <code>SelectedIndexChanged</code>, которое возникает при изменении выделенного элемента:</p>
<pre class="brush:c#;">
...
<p>В данном случае по выбору элемента списка будет отображаться сообщение с выделенным элементом.</p>
