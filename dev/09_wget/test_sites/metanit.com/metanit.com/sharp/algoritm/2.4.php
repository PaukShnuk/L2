...
<html >
<head>
<title>Структуры данных в C# | Стек</title>
<meta charset="utf-8" />
<meta name="description" content="Структура данных Стек в c#, построение стека на основе связного списка и основные операции с ним">
...
<h2>Стек</h2><div class="date">Последнее обновление: 02.09.2016</div>
...
<p>В прошлой теме была реализована структура Стек на основе массивов. Однако в подобной структуре мы сталкиваемся с необходимостью 
динамически изменять размер массива, выделяя для него новую память, или, наоборот, уменьшая ее. Альтернативой является создание стека на основе односвязного списка. 
Структура "Односвязный список" является простой и понятной и в то же время достаточно эффективной для создания других структур данных.</p>
<p>Каждый элемент в стеке, как и в односвязном списке, будет представлен классом Node:</p>
<pre class="brush:c#;">
public class Node&lt;T&gt;
{
    public Node(T data)
...
<p>На основе класса Node мы можем реализовать структуру стек:</p>
<pre class="brush:c#;">
using System;
...
<p>В отличие от предыдущей реализации стека, построенной на массивах, здесь мы уже уходим от ограничения, которые накладывали массивы - нам не надо пересоздавать массив 
при добавление в него или удалении из него данных. Нам достаточно переустановить ссылку на элемент head, который является верхушкой стека. 
При этом алгоритмическая сложность обоих методов Push и Pop составляет <span class="b">O(1)</span>.</p>
<p>Таким образом, мы получили относительно простое и достаточно эффективное решение.</p>
