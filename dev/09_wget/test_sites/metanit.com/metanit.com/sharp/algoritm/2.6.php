...
<html >
<head>
<title>Структуры данных в C# | Дек</title>
<meta charset="utf-8" />
<meta name="description" content="Структура данных дек или двусторонняя чередь в С#, основные операции с деком">
...
<h2>Дек</h2><div class="date">Последнее обновление: 02.09.2016</div>
...
<p>Дек (deque) представляет двустороннюю очередь, в которой элементы можно добавлять как в начало, так и в конец. Удаление также может идти как с начала, так и с конца.</p>
<p>Поскольку реализовать добавление и удаление нужно с двух сторон, то за основу реализации можно взять организацию двусвязного список. Так, каждый элемент в деке будет представлен 
следующим классом:</p>
<pre class="brush:c#;">
...
<p>Класс дека будет следующим:</p>
<pre class="brush:c#;">
using System;
...
<p>В отличие от класса очереди здесь определены два метода для добавления и два для удаления.</p>
<p>Для добавления в начало очереди переустанавливаем ссылку на переменную head:</p>
<pre class="brush:c#;">
public void AddFirst(T data)
{
    DoublyNode&lt;T&gt; node = new DoublyNode&lt;T&gt;(data);
...
<p>Добавление элемента в конец дека вызывает аналогичную переустановку переменной tail:</p>
<pre class="brush:c#;">
public void AddLast(T data)
{
    DoublyNode&lt;T&gt; node = new DoublyNode&lt;T&gt;(data);
...
<p>При удалении из начала дека надо переустановить ссылку на первый элемент:</p>
<pre class="brush:c#;">
public T RemoveFirst()
{
    if (count == 0)
...
<p>При удалении последнего элемента переустанавливается переменная tail на предпоследний элемент:</p>
<pre class="brush:c#;">
public T RemoveLast()
{
    if (count == 0)
...
<p>Сложность алгоритмов всех методов добавления и удаления из дека составляет <span class="b">O(1)</span>.</p>
<img src="pics/deck.png" alt="Дек в C#" />
<p>Применение дека:</p>
<pre class="brush:c#;">
