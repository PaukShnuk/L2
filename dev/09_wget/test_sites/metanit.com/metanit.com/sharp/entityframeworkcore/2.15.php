...
<html >
<head>
<title>Entity Framework Core | Управление схемой БД и миграции</title>
<meta charset="utf-8" />
<meta name="description" content="Управление схемой БД и миграции в Entity Framework Core и C#, создание и выполнение миграций, создание и удаление базы данных с помощью Database.EnsureCreated и Database.EnsureDeleted">
...
<h2>Управление схемой БД и миграции</h2><div class="date">Последнее обновление: 15.11.2020</div>
...
операции с моделью User, то мы столкнемся с ошибкой.</p>
<img src="./pics/2.15.png" alt="Ручное изменение таблицы в Entity Framework Core" />
<img src="./pics/2.7.png" alt="Изменение модели в Entity Framework Core" />
<p>Так как модель User изменилась, то нам надо привести в соответствие соответствующую таблицу в БД. В зависимости от конкретной ситуации можно использовать ряд подходов для этого.</p>
<h3>Ручное изменение базы данных</h3>
<p>В самых простых случаях мы можем написать sql-скрипт для добавления столбцов или таблиц, либо же даже можем вручную в режиме дизайнера таблицы в Visual 
Studio добавить столбцы и/или таблицы. Для этого перейдем к дизайнеру таблицы:</p>
<p></p>
<img src="./pics/2.8.png" alt="Изменение таблицы в Entity Framework Core" />
<p>Например, свойству Position с типом string будет соответствовать столбец Position с типом NVARCHAR. После добавления столбца нажмем на кнопку Update и 
далее на кнопку Update Database:</p>
<img src="./pics/2.9.png" />
<p>В итоге будет добавлен новый столбец. И теперь таблица Users находится в соответствии с классом User. Больше никаких проблем при выполнении программы не возникнет.</p>
...
<p>Другой очень простой способ изменения схемы данных представляет применение пары методов <span class="b">Database.EnsureCreated</span> и 
<span class="b">Database.EnsureDeleted</span>, которые определены в классе контекста. <span class="b">Database.EnsureCreated</span> создает базу 
данных, а <span class="b">Database.EnsureDeleted</span> удаляет ее.</p> 
<p>Первый метод уже ранее использовался для создания бд в конструкторе контекста данных. Подобным образом можно вызвать оба метода:</p>
<pre class="brush:c#;">
public class ApplicationContext : DbContext
{
	public DbSet&lt;User&gt; Users { get; set; }
...
<p>То есть мы сначала удаляем базу данных со старой схемой, а потом создаем ее заново.</p>
<p>Преимуществом этих методов является то, что мы можем вызвать их в любом месте приложения, создав объект контекста.</p>
<p>В то же время при удалении происходит полное удаление данных, что в ряде случаев может быть нежелательным. И в этом случае лучше использовать 
миграции.</p>
<h3>Миграция</h3>
...
<p>Теперь для создания и выполнения миграции перейдем в Visual Studio к окну Package Manager Console. Вначале введем команду</p>
<pre>
...
<li><p><span class="b">XXXXXXXXXXXXXX_InitialCreate.Designer.cs</span>: файл метаданных миграции, которые используются Entity Frameworkом</p></li>
...
<p>После выполнения миграции мы найдем сгенерированную базу данных. Следует отметить, что кроме основных таблиц (в случае выше таблицы Users) база данных 
...
<pre class="brush:c#;">myDbContext.Database.Migrate();</pre>
...
<p>Как получать конфигурацию подключения из файла, описывалось в статье <a href="https://metanit.com/sharp/entityframeworkcore/2.2.php">Конфигурация подключения</a></p>
<p>При выполнении миграции для такого контекста данных мы получим ошибку:</p>
<pre>
