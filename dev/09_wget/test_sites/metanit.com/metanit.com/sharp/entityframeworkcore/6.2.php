...
<html >
<head>
<title>Entity Framework Core | Хранимые функции</title>
<meta charset="utf-8" />
<meta name="description" content="Хранимые функции в Entity Framework Core, определение и вызов функций из базы данных в C#, метод HasDbFunction">
...
<h2>Хранимые функции</h2><div class="date">Последнее обновление: 19.11.2020</div>
...
<p>Наряду с выполнением обычных sql-запросов мы также можем выполнять хранимые функции и процедуры, которые имеются в базе данных. 
Рассмотрим вызов хранимой функции в приложении на C#.</p>
<p>Вначале создадим функцию. Пусть наша база данных описывается следующим контекстом данных и моделями:</p>
<pre class="brush:c#;">
...
<p>Теперь в Visual Studio в окне <span class="b">SQL Server Object Explorer</span> откроем узел базы данных. В узле базы данных откроем подузел 
Programmability -&gt; Functions -&gt; Table-valued Functions</span>:</p>
<img src="./pics/6.4.png" alt="Entity Framework Core и Table-valued Functions" />
...
<p>Как видно, я получил ожидаемые результаты, значит, функция работает правильно.</p>
<p>Теперь обратимся к ней из кода C#:</p>
<pre class="brush:c#;">
...
<p>В этом случае я получу те же результаты, что и при выполнении скрипта выше.</p>
<h3>Упрощение вызова в EntityFramework Core 5.0</h3>
<p>В EntityFramework Core 5.0 была добавлена возможность создания в классе контекста метода, который проецируется на 
...
<p>Далее в переопрепределенном методе <code>OnModelCreating()</code> класса контекста нам надо зарегестрировать метод GetUsersByAge с помощью вызова <span class="b">HasDbFunction()</span>:</p>
