...
<html >
<head>
<title>Entity Framework Core | Определение моделей</title>
<meta charset="utf-8" />
<meta name="description" content="Определение моделей в Entity Framework Core, включение и исключение классов из модели">
...
<h2>Определение моделей</h2><div class="date">Последнее обновление: 15.11.2020</div>
...
сопоставляются с таблицами в базе данных. Но кроме того, в модель также включаются типы, на которые есть ссылки в сущностях, которые 
...
<p>После создания базы данных в ней будут созданы две таблицы: Products и Company.</p>
<img src="./pics/2.2.png" alt="Навигационное свойство в Entity Framework Core" />
<p>Поскольку для типа Product определен набор DbSet, то для имени таблицы будет применяться имя этого набора, а для второй таблицы будет использоваться 
имя класса Company.</p>
<p>Еще один способ включения сущности в модель представляет вызов <code>Entity()</code> объекта ModelBuilder в методе <code>OnModelCreating()</code>:</p>
<pre class="brush:c#;">
public class ApplicationContext : DbContext
{
    public DbSet&lt;Product&gt; Products { get; set; }
...
<p>И если мы сейчас создадим и выполним миграции, то в базе данных будут уже три таблицы:</p>
<img src="./pics/2.3.png" alt="Fluent API в Entity Framework Core" />
<h3>Исключение из модели</h3>
<p>Иногда возникают ситуации, когда надо, наоборот, исключить сущность из модели. Например, в примере выше сущность Product ссылается на класс Company, и, допустим, 
...
<p>При исключении сущности Company в базе данных будет только одна таблица Products, причем она не будет содержать столбца, который бы сопоставлялся со свойством 
Manufacturer класса Product:</p>
<img src="./pics/2.4.png" alt="Добавление моделей в Entity Framework Core" />
