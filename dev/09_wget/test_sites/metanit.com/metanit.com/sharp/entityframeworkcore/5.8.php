...
<html >
<head>
<title>Entity Framework Core | Выполнение запросов</title>
<meta charset="utf-8" />
<meta name="description" content="Стадии выполнения запросов в Entity Framework Core, отслеживание объектов">
...
<h2>Выполнение запросов</h2><div class="date">Последнее обновление: 18.11.2020</div>
...
<p>Рассмотрим, как EF обрабатывает запросы на получение данных из БД. Вначале выражения LINQ обрабатываются Entity Framework Core, и на их основе создается объект запроса в той форме,
в которой он может обрабатываться провайдером базы данных. Созданный объект запроса кэшируется, что позволяет не 
пересоздавать его при повторном его выполнении.</p>
<p>Затем этот объект запроса передается провайдеру базы данных, который транслирует его на язык, понятный для базы 
данных (например, SQL). База данных обрабатывает запрос и возвращает определенный результат.</p>
<p>EF Core получает результат обработки, и дальше его действия зависят от того, отслеживаются ли результаты запроса или нет.</p>
<p>Если запрос является отслеживаемым, то десь есть два альтернативных варианта:</p>
<ul>
<li><p>Если данные, полученные из бд, представляют объекты, которые уже отслеживаются, то есть они уже есть в 
контексте, то EF возвращает те объекты, которые уже есть в контексте.</p>
</li>
<li><p>Если данные, полученные из бд, представляют объекты, которые еще не отслеживатся, их нет в контексте, 
то EF создает по этим данным новые объекты, добавляет в контекст, начинает их отслеживать и возвращает их пользователю.</p>
</li>
</ul>
<p>Если запрос является не отслеживаемым, то есть отслеживание отключено с помощью метода <span class="b">AsNoTracking()</span> или 
установки свойства <code>ChangeTracker.QueryTrackingBehavior = QueryTrackingBehavior.NoTracking</code>, то EF 
создает по этим данным новые объекты и возвращает их пользователю. В отличие от отслеживаемых запросов созданные объекты не добавляются в контекст и не отслеживаются.</p>
<p>Рассмотрим несколько примеров. Для этого возьмем следующую модель User и контекст данных:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Так как запрос <code>db.Users.FirstOrDefault()</code> является отслеживаемым, то при получении данных, по ним будет создаваться объект user1, 
который добавляется в контекст и начинает отслеживаться.</p>
<p>Далее повторно вызывается данный запрос для получения объекта user2. Этот запрос то же является отслеживаемым, поэтому 
EF увидит, что такой объект уже есть в контексте, он уже отслеживается, и возвратит ссылку на тот же объект. Поэтому все изменения с переменной user1 
затронут и переменную user2.</p>
...
<pre class="consoletext">
Before User1: Tom   User2: Tom
After User1: Sam   User2: Tom
</pre>
</div>
<p>С первым объектом user1 все по прежнему: он также попадает в контекст и отслеживается. А вот второй запрос теперь является неотслеживаемым, так как использует 
метод AsNoTracking. Поэтому для данных, полученных в результате этого запроса, будет создаваться новый объект, который никак не будет связан с 
объектом user1. И изменения одного из этих объектов никак не повлияют на второй объект.</p>
...
<div class="nav"><p><a href="5.7.php">Назад</a><a href="./">Содержание</a><a href="5.6.php">Вперед</a></p></div></div>
<div class="socBlock">
