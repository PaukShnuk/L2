...
<html >
<head>
<title>Entity Framework Core | MySQL</title>
<meta charset="utf-8" />
<meta name="description" content="MySQL в Entity Framework Core, провавайдер MySql.Data.EntityFrameworkCore, метод UseMySQL">
...
<h2>MySQL</h2><div class="date">Последнее обновление: 18.11.2020</div>
...
<p>На данный момент для работы с MySQL имеется несколько провайдеров, но в данном случае мы будем использовать провайдер 
<span class="b">Pomelo.EntityFrameworkCore.MySql</span> от Pomelo Foundation Project. Стоит отметить, что на данный момент (18 ноября 2020) 
для версии EF Core он пока в предрелизном состоянии, поэтому мы будем использовать предрелизную версию.</p>
<p>Для подключения к MySQL создадим новый консольный проект .NET Core и добавим через Nuget пакет <span class="b">Pomelo.EntityFrameworkCore.MySql</span>:</p>
<img src="./pics/7.2.png" alt="MySQL в Entity Framework Core" />
<p>Стоит отметить, что также есть официальный провайдер от Oracle - <span class="b">MySql.Data.EntityFrameworkCore</span>, но он развивается довольно медленно (на данный момент 
не поддерживает EF Core 5.x), кроме того, не имеет поддержки ряда функционала.</p>
<p>Для работы определим модель User:</p>
<pre class="brush:c#;">
public class User
{
...
<p>И также определим контекст данных - класс ApplicationContext:</p>
<pre class="brush:c#;">
...
<p>Для работы с MySQL вызывается метод <span class="b">UseMySql()</span>, в который передается строка подключения. В строке подключения 
указываются адрес сервера (параметр server), имя пользователя в субд (User), его пароль (Password) и имя базы данных (Database).</p>
<p>В качестве второго параметра передается номер версии MySQL в виде объекта <span class="b">MySqlServerVersion</span> - в его конструктор передается 
объект <code>Version</code>, который собственно содержит номер установленной версии MySQL. Например, в моем случае это версия <span class="b">8.0.11</span>, 
соответственно я передаю объект <code>new MySqlServerVersion(new Version(8, 0, 11))</code>.</p>
<p>И для тестирования определим в файле Program.cs добавление и вывод данных:</p>
<pre class="brush:c#;">
using System;
using System.Linq;
