...
<html >
<head>
<title>Entity Framework Core | Подключение к существующей базе данных</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение к существующей базе данных на Entity Framework Core, подход Datebase First">
...
<h2>Подключение к существующей базе данных</h2><div class="date">Последнее обновление: 14.11.2020</div>
...
<p>Для подключения к существующей базе данных создадим новый проект. Как и в прошлой теме это будет проект по типу <span class="b">Console App (.NET Core)</span>, 
который назовем FirstApp.</p>
<img src="./pics/1.9.png" alt="Подключение к существующей базе данных в EF Core" />
<p>Для работы с существующей БД MS SQL Server нам надо добавить два пакета:</p>
<ul>
<li><p><span class="b">Microsoft.EntityFrameworkCore.SqlServer</span> (представляет функциональность Entity Framework для работы с MS SQL Server)</p></li>
...
<img src="./pics/1.5.png" alt="Реверсинг базы данных в Entity Framework" />
<p>К примеру возьмем базу данных, созданную в прошлой теме. Эта база данных называется helloappdb.mdf и имеет одну таблицу Users с тремя столбцами Id, Name и 
Age.</p>
<img src="./pics/1.6.png" alt="Определение базы данных в Entity Framework" />
...
необходимые классы по базе данных. Чтобы воспользоваться этой функцией перейдем в Visual Studio к окну <span class="b">Package Manager Console</span>. 
Его открыть можно, перейдя в меню <span class="b">Tools</span> –&gt; <span class="b">NuGet Package Manager</span> –&gt; <span class="b">Package Manager Console</span></p>
<p>Далее в Package Manager Console выполним следующую команду:</p>
<pre class="brush:c#;">Scaffold-DbContext "Server=(localdb)\mssqllocaldb;Database=helloappdb;Trusted_Connection=True;" Microsoft.EntityFrameworkCore.SqlServer</pre>
<p>Здесь в качестве параметра команде Scaffold-DbContext передается строка подключения с указанием сервера и названием базы данных.</p>
<img src="./pics/1.7.png" alt="Reverse Engineering in Entity Framework Core" />
<p>После выполнения этой команды в проект будет добавлен класс Users:</p>
<pre class="brush:c#;">
...
<p>И после генерации данных классов мы сможем работать с базой данных. Для этого изменим код класса Program:</p>
<pre class="brush:c#;">
using System;
using System.Linq;
