...
<html >
<head>
<title>Entity Framework Core | Отслеживание объектов и AsNoTracking</title>
<meta charset="utf-8" />
<meta name="description" content="Отслеживание объектов в контексте в Entity Framework Core, метод AsNoTracking, свойство ChangeTracker">
...
<h2>Отслеживание объектов и AsNoTracking</h2><div class="date">Последнее обновление: 18.11.2020</div>
...
отслеживаемыми. Когда контекст данных извлекает данные из базы данных, Entity Framework помещает извлеченные 
объекты в кэш и отслеживает изменения, которые происходят с этими объектами вплоть до использования метода 
SaveChanges()/SaveChangesAsync(), который фиксирует все изменения в базе данных. Но нам не всегда необходимо отслеживать изменения. 
...
применяется к набору IQueryable. При его применении 
возвращаемые из запроса данные не кэшируются. То есть запрос является неотслеживаемым. А это означает, что Entity 
Framework не производит какую-то дополнительную обработку и не выделяет дополнительное место для хранения 
...
<p>Пусть в базе данных есть есть несколько моделей User и Company:</p>
<pre class="brush:c#;">
...
<p>Причем в данном случае мы можем и не вызывать метод SaveChanges, элемент уже и так будет закэширован. Метод SaveChanges 
необходим, чтобы применить все изменения с объектами в базе данных.</p>
<p>Но если бы мы использовали AsNoTracking, то результат был бы другой:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
