...
<html >
<head>
<title>Entity Framework Core | Выполнение SQL-запросов</title>
<meta charset="utf-8" />
<meta name="description" content="Выполнение SQL-запросов в Entity Framework Core, методы FromSqlRaw и ExecuteSqlRaw, определение параметров в запросах">
...
<h1>SQL в Entity Framework Core</h1><h2>Выполнение SQL-запросов</h2><div class="date">Последнее обновление: 18.11.2020</div>
...
<p>LINQ to Entities позволяет создавать запросы к источнику данных. Однако в случае, если запрос очень сложный по своей структуре или нам его 
легче описать на языке SQL, то в Entity Framework Core выполнить напрямую sql-запрос.</p>
<h3>FromSqlRaw</h3>
...
<p>Класс <span class="b">SqlParameter</span> из пространства имен System.Data.SqlClient позволяет задать параметр, который затем передается в запрос sql.</p>
...
новые записи. Для этой цели применяется метод <span class="b">ExecuteSqlRaw()</span>, который возвращает количество затронутых командой строк. 
...
<span class="b">ExecuteSqlInterpolated()</span> соответственно, которые позволяют использовать интерполяцию строк для передачи параметров. Пример с 
<code>FromSqlInterpolated</code>:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
	var name = "%Tom%";
...
<p>Использование <code>ExecuteSqlInterpolated</code>:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
...
<h3>Асинхронные версии</h3>
<p>Методы <code>Database.ExecuteSqlInterpolated()</code> и <code>Database.ExecuteSqlRaw</code> имеют асинхронные версии:</p>
<pre class="brush:c#;">
using System;
using System.Linq;
using Microsoft.EntityFrameworkCore;
