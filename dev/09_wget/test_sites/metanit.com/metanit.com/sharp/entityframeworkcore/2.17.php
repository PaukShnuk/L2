...
<html >
<head>
<title>Entity Framework Core | Настройка индексов</title>
<meta charset="utf-8" />
<meta name="description" content="Настройка индексов в Entity Framework Core, составные индексы, фильтры индексов, метод HasIndex и HasFilter">
...
<h2>Настройка индексов</h2><div class="date">Последнее обновление: 16.11.2020</div>
...
Но с помощью Fluent API мы можем переопределить это поведение. Для установки индекса применяется метод <span class="b">HasIndex()</span>:</p>
<pre class="brush:c#;">
...
<p>В данном случае для индекса будет использоваться столбец PhoneNumber, а называться он будет "PhoneIndex".</p>
...
<p>Начиная с версии EntityFramework Core 5.0 во фреймворк был добавлен атрибут <span class="b">IndexAttribute</span>, который позволяет через атрибут 
настроить индекс. Например:</p>
<pre class="brush:c#;">
[Index("PhoneNumber")]
public class User
{
...
<p>Первый и обязательный параметр атрибута указывает на свойство, с которым будет ассоциирован индекс. В данном случае это свойство PhoneNumber. 
Но также он может принимать набор свойств, для которых создается индекс. В этом случае названия свойств просто перечисляются через запятую:</p>
<pre class="brush:c#;">
...
<p>В данном случае индекс будет называться Phone_Index и для него установлена уникальность.</p>
