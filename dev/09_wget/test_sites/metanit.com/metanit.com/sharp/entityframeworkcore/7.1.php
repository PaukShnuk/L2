...
<html >
<head>
<title>Entity Framework Core | PostgreSQL</title>
<meta charset="utf-8" />
<meta name="description" content="PostgreSQL в Entity Framework Core, провавайдер Npgsql.EntityFrameworkCore.PostgreSQL, метод UseNpgsql">
...
<h1>Провайдеры баз данных</h1><h2>PostgreSQL</h2><div class="date">Последнее обновление: 18.11.2020</div>
...
<p>Для работы с базой данных PostgreSQL создадим новый консольный проект .NET Core. Для работы с этой СУБД вместо с Entity Framework Core в проект необходимо добавить 
через Nuget пакет <span class="b">Npgsql.EntityFrameworkCore.PostgreSQL</span>:</p>
<img src="./pics/7.1.png" alt="PostgreSQL в Entity Framework Core" />
<p>После установки пакета определим в проекте класс User:</p>
<pre class="brush:c#;">
public class User
{
...
<p>И также определим класс контекста данных:</p>
<pre class="brush:c#;">
...
<p>Для установки подключения к базе данных в методе OnConfiguring вызывается метод <span class="b">UseNpgsql()</span>, в который передается строка подключения. 
Строка подключения содержит адрес сервера (параметр Host), порт (Port), название базы данных на сервере (Database), имя пользователя в рамках сервера PostgreSQL (Username) и его пароль (Password). 
В зависимости от настроек сервера PostgreSQL параметры могут отличаться.</p>

<p>Теперь определим в файле <span class="b">Program.cs</span> простейшую программу по добавлению и извлечению объектов из базы данных:</p>
<pre class="brush:c#;">
using System;
using System.Linq;
...
<h3>Миграции</h3>
<p>Выше для создания базы данных использовался метод Database.EnsureCreated. Теперь изменим класс контекста данных - уберем вызов Database.EnsureCreated и 
изменим название база данных:</p>
<pre class="brush:c#;">
public class ApplicationContext : DbContext
{
	public DbSet&lt;User&gt; Users { get; set; }
...
<p>Посмотрим теперь, как использовать миграции. Прежде всего нам надо добавить в проект через 
Nuget пакет <span class="b">Microsoft.EntityFrameworkCore.Tools</span> для поддержки миграций.</p>
<p>Для создания базы данных создадим и выполним миграции. Для этого в окне <span class="b">Package Manager Console</span> введем команду:</p>
<div class="console">
<pre class="consoletext">
Add-Migration Initial
</pre>
</div>
<p>После генерации файла миграции для создания базы данных выполним команду:</p>
<div class="console">
<pre class="consoletext">
Update-Database
</pre>
</div>
<p>После этого на сервере будет создана база данных, и мы сможем с ней взаимодействовать.</p>
<h3>Подключение к существующей базе данных</h3>
<p>Для подключения к существующей базе данных в PostgreSQL необходимо в Package Manager Console выполнить команду Scaffold-DbContext, 
которой передается строка подключения и название провайдера, то есть <span class="b">Npgsql.EntityFrameworkCore.PostgreSQL</span> (для выполнения этой команды 
тоже необходим пакет <span class="b">Microsoft.EntityFrameworkCore.Tools</span>). Сначала вводится команда Scaffold-DbContext и строка подключения:</p>
<pre class="brush:c#;">Scaffold-DbContext "Host=localhost;Port=5432;Database=usersdb;Username=postgres;Password=password"</pre>
<p>Затем в консоли появится слово <span class="b">Provider</span>, после которого надо будет ввести название провайдера, то есть</p>
<pre class="brush:c#;">Npgsql.EntityFrameworkCore.PostgreSQL</pre>
<p>Причем на данный момент название провайдера вводится вручную.</p>
<img src="./pics/7.3.png" alt="миграции PostgreSQL в Entity Framework Core" />
<p>И если все прошло удачно, то EntityFramework Core автоматически сгенерирует все необходимые классы моделей и контекста.</p>
