...
<html >
<head>
<title>Entity Framework Core | Ограничения свойств</title>
<meta charset="utf-8" />
<meta name="description" content="Ограничения свойств по длине в Entity Framework Core, обязательные значения свойств, атрибуты MaxLength и Required, методы IsRequired и HasMaxLength">
...
<h2>Ограничения свойств</h2><div class="date">Последнее обновление: 16.11.2020</div>
...
<h3>Обязательные свойства</h3>
<p>По умолчанию свойство является необязательным к установке, если оно допускает значение <span class="b">null</span>. Это свойства, которые имеют, например, такие типы как 
<code>string</code>, <code>int?</code>, <code>byte[]</code>, объекты классов и т.д. Хотя мы также можем настроить эти свойства как обязательные.</p>
<p>Свойство является обязательным, если значение <code>null</code> не является для него корректным. Это свойства типов <code>int</code>, <code>decimal</code>, <code>bool</code> 
и т.д.</p>
<h4>Атрибут Required</h4>
<p>Атрибут <span class="b">Required</span> указывает, что данное свойство обязательно для установки, то есть будет иметь определение NOT NULL в БД::</p>
<pre class="brush:c#;">
public class User
{
...
<p>Если мы не установим свойство Name у объекта User и попытаемся добавить этот объект в бд, то получим ошибку. А столбец Name в базе данных будет определен как NOT NULL.</p>
<h4>Метод IsRequired</h4>
<p>То же самое можно сделать и через Fluent API с помощью метода <span class="b">IsRequired()</span>:</p>
<pre class="brush:c#;">
...
<p>По умолчанию для строкового свойства в таблице создается столбец с типом <code>nvarchar(MAX)</code>, то есть строка неограниченной длины (исключение - если строковое 
свойство представляет ключ, тогда длина составляет 450 символов). Используя аннотации данных или Fluent API, мы можем ограничить строку по длине.</p>
...
<p>В этом случае для столбец Name в базе данных будет иметь тип <code>nvarchar(50)</code>.</p>
