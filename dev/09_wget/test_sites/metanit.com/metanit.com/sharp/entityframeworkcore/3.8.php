...
<html >
<head>
<title>Entity Framework Core | Explicit loading</title>
<meta charset="utf-8" />
<meta name="description" content="Явная загрузка связанных данных в Entity Framework Core, Explicit loading и метод Load, методы Collection и Reference">
...
<h2>Explicit loading</h2><div class="date">Последнее обновление: 16.11.2020</div>
...
<p>Explicit loading предполагает явную загрузку данных с помощью метода <span class="b">Load()</span>. Допустим, у нас имеются следующие модели:</p>
<pre class="brush:c#;">
...
<p>Выражение <code>db.Users.Where(p=&gt;p.CompanyId==company.Id).Load()</code> загружает всех пользователей в контекст. 
...
Метод Collection применяется, если навигационное свойство представляет коллекцию:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
    Company company = db.Companies.FirstOrDefault();
...
<p>Если навигационное свойство представляет одиночный объект, то можно применять метод Reference:</p>
<pre class="brush:c#;">
using(ApplicationContext db = new ApplicationContext())
{
