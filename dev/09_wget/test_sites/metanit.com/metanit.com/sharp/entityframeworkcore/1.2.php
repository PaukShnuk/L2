...
<html >
<head>
<title>Entity Framework Core | Первое приложение</title>
<meta charset="utf-8" />
<meta name="description" content="Cоздание первого приложения на Entity Framework Core, миграции и создание новой базы данных, подход Code First">
...
<h2>Первое приложение на EF Core</h2><div class="date">Последнее обновление: 14.11.2020</div>
...
<p>Платформу Entity Framework Core можно применять в различных технологиях стека .NET. В данном случае мы будем рассматривать базовые моменты платформы на примере 
консольных приложений, как наиболее простых и не содержащих никакого лишнего кода. Но в последствии также затронем применение EF Core и в других технологиях на конкретных примерах.</p>
<p>Итак, создадим первое приложение с использованием Entity Framework Core. Для этого создадим в Visual Studio 2019 новый проект по типу 
<span class="b">Console App (.NET Core)</span>, который назовем HelloApp:</p>
<img src="./pics/1.1.png" alt="Первое приложение на Entity Framework Core" />
<img src="./pics/1.10.png" alt="Создание первого приложения на Entity Framework Core 5" />
...
<span class="ii">Program.cs</span>. И чтобы начать работать с EntityFramework Core, нам необходимо вначале 
...
Так, в данном случае мы будем использовать MS SQL Server в качестве СУБД, поэтому нам надо искать пакет <span class="b">Microsoft.EntityFrameworkCore.SqlServer</span>:</p>
<img src="./pics/1.2.png" alt="Microsoft.EntityFrameworkCore.SqlServer и начало работы с EntityFramework Core 5" />
...
<pre class="brush:c#;">Install-Package Microsoft.EntityFrameworkCore.SqlServer</pre>
<p>После выполнения этой команды выполним вторую команду:</p>
<pre class="brush:c#;">Install-Package Microsoft.EntityFrameworkCore.Tools</pre>
...
<p>В любом приложении, работающим с БД через Entity Framework, нам нужен будет контекст (класс производный от DbContext). 
...
<p>Кроме того, для настройки подключения нам надо переопределить метод <span class="b">OnConfiguring</span>. Передаваемый в него параметр класса DbContextOptionsBuilder 
с помощью метода <code>UseSqlServer</code> позволяет настроить строку подключения для соединения с MS SQL Server. В данном случае мы определяем, что  в качестве 
сервера будет использоваться движок localdb, который предназначен специально для разработки, (<code>"Server=(localdb)\\mssqllocaldb"</code>), 
а файл базы данных будет называться helloappdb (<code>"Database=helloappdb"</code>).</p>
...
<p>Теперь определим сам код программы, который будет взаимодействовать с созданной БД. Для этого изменим класс Program следующим 
образом:</p>
<pre class="brush:c#;">
using System;
using System.Linq;
...
<p>И после добавления мы можем найти базу данных в окне SQL Server Object Explorer (открыть данное окно можно через меню 
<span class="b">View-&gt;SQL Server Object Explorer</span>). Там мы можем раскрыть и посмотреть содержимое ее таблиц:</p>
