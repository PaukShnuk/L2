...
<html >
<head>
<title>Entity Framework Core и C# | Настройка ключей</title>
<meta charset="utf-8" />
<meta name="description" content="Настройка ключей в Entity Framework Core, первичные и альтернативные ключи, составной ключ, метод HasKey, имя ключа в базе данных, атрибут Key">
...
<h2>Настройка ключей</h2><div class="date">Последнее обновление: 18.11.2021</div>
...
<p>По умолчанию в качестве ключа используется свойство, которое называется <span class="b">Id</span> или <span class="b">[имя_класса]Id</span>. 
Например:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Для установки свойства в качестве первичного ключа с помощью аннотаций применяется атрибут <span class="b">[Key]</span>:</p>
<pre class="brush:c#;">
...
<p>Для конфигурации ключа с Fluent API применяется метод <span class="b">HasKey()</span>:</p>
<pre class="brush:c#;">
...
<p>Дополнительно с помощью Fluent API можно настроить имя ограничения, которое задается для первичного ключа. Для этого применяется метод <span class="b">HasName()</span>:</p>
<pre class="brush:c#;">
protected override void OnModelCreating(ModelBuilder modelBuilder)
{
...
<h3>Составные ключи</h3>
<p>С помощью Fluent API можно создать составной ключ из нескольких свойств:</p>
<pre class="brush:c#;">
public class ApplicationContext : DbContext
{
...
<p>Составной ключ можно создать только с помощью Fluent API. Применение подобного ключа:</p>
<pre class="brush:c#;">
...
<p>На уровне базы данных в случае с SQLite будет создаваться следующая таблица:</p>
<pre class="brush:sql;">
CREATE TABLE "Users" (
...
<h3>Альтернативные ключи</h3>
<p>Альтернативные ключи представляют свойства, которые также, как и первичный ключ, должны иметь уникальное значение. В то же время альтернативные ключи не 
являются первичными. На уровне базы данных это выражается в установке для соответствующих столбцов ограничения на уникальность.</p>
<p>Для установки альтернативного ключа используется метод <span class="b">HasAlternateKey()</span>:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>В данном случае свойство Passport (серия и номер паспорта) будет альтернативным ключом. Созданная таблица Users в случае SQLite будет описываться следующим 
SQL-скриптом:</p>
<pre class="brush:sql;">
CREATE TABLE "Users" (
	"Id"	INTEGER NOT NULL,
...
<p>Альтернативные ключи также могут быть составными:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>В этом случае в SQLite будет создаваться следующая таблица:</p>
<pre class="brush:sql;">
CREATE TABLE "Users" (
	"Id"	INTEGER NOT NULL,
	"Name"	TEXT,
