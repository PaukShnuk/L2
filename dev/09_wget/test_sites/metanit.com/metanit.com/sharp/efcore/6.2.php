...
<html >
<head>
<title>Entity Framework Core и C# | Хранимые функции</title>
<meta charset="utf-8" />
<meta name="description" content="Хранимые функции в Entity Framework Core, определение и вызов функций из базы данных в C#, проекция хранимой функции на методы класса, метод HasDbFunction">
...
<h2>Хранимые функции</h2><div class="date">Последнее обновление: 22.11.2021</div>
...
<p>Вместе с обычными sql-запросами Entity Framework Core также позволяет выполнять хранимые функции, которые созданы в базе данных. 
Рассмотрим вызов хранимой функции в приложении на C# на примере БД MS SQL Server.</p>
<p>Пусть наша база данных описывается следующим контекстом данных и сущностями:</p>
<pre class="brush:c#;">
...
<p>Вначале создадим саму хранимую функцию, к которой мы будем обращаться. Для этого в Visual Studio в окне <span class="b">SQL Server Object Explorer</span> откроем узел базы данных. В узле базы данных откроем подузел 
<span class="b">Programmability -&gt; Functions -&gt; Table-valued Functions</span>:</p>
<img src="./pics/6.4.png" alt="Entity Framework Core в C# и Table-valued Functions" />
<p>Далее нажмем на этот узел правой кнопкой мыши и выберем <span class="b">Add New -&gt; Table-valued Function</span>:</p>
<p>После этого Visual Studio генерирует и открывает файл скрипта со следующим содержимым:</p>
<pre class="brush:sql;">
...
<p>Изменим скрипт следующим образом:</p>
<pre class="brush:sql;">
...
<p>Выражение <code>CREATE FUNCTION [dbo].[GetUsersByAge]</code> создае новую функцию GetUsersByAge. После ее названия определяется список параметров. Пусть наша функция принимает 
только один параметр <code>@age</code>, который имеет тип <code>int</code>, то есть целочисленное значение.</p>
<p>Затем идет определение возвращаемого объекта-таблицы в выражении <code>RETURNS @returntable TABLE(...)</code>. В скобках идет перечисление столбцов возвращаемой 
таблицы. В данном случае они совпадают с определением таблицы Users. То есть таблица будет содержать объекты класса User.</p>
<p>Между выражениями <code>BEGIN</code> и <code>END</code> идет собственно выполнение функции. В данном случае с помощью оператора WHERE  функция будет находить все 
строки, у которых столбец Age содержит меньшее значение, чем в параметре @age.</p>
<p>Теперь добавим функцию в базу данных. Для этого нажмем на кнопку Update:</p>
<img src="./pics/6.5.png" alt="Добавление хранимой функции в базу данных" />
<p>Затем в появившемся окне нажмем на кнопку <span class="bb">Update Database</span></p>
<img src="./pics/6.6.png" alt="Хранимые функции в MS SQL Server" />
<p>После этого в базу данных будет добавлена функция, и ее можно будет использовать. Но перед обращением к ней из кода 
c# мы ее протестируем, чтобы убедиться, что она работает как надо. Для этого нажмем на функцию правой кнопкой мыши и в появившемся меню выберем пункт <span class="b">Execute Function</span>:</p>
<img src="./pics/6.7.png" />
<p>После этого откроется окно для установки входных параметров функции. Введем в поле Value какое-нибудь число, которое будет передаваться в функцию в качестве параметра:</p>
<img src="./pics/6.8.png" />
<p>И Visual Studio сгенерирует и сразу же выполнит скрипт с функцией и переданным в нее параметром:</p>
<img src="./pics/6.9.png" />
<p>Как видно, я получил ожидаемые результаты, значит, функция работает правильно. Теперь обратимся к функции в коде на C#. Для этого мы можем воспользоватся одним из двух 
подходов.</p>
<h3>Обращение к функции в запросе SQL</h3>
<p>Первый подход предполагает обращение к хранимой функции в запросе SQL, который отправляется из кода C#:</p>
<pre class="brush:c#;">
...
<p>В данном случае в запросе вместо таблицы указываем имя вызов функции с переданными ей параметрами: <code>GetUsersByAge (@age)</code></p>
<p>В итоге результат данного запроса будет таким же, что и при выполнении скрипта выше.</p>
<h3>Проецирование хранимой функции на метод класса</h3>
<p>Второй подход предполагает определение в классе контекста метода, который проецируется на 
хранимую функцию и через который можно вызывать данную функцию.</p>
<p>Например, выше была определена хранимая табличная функция <code>GetUsersByAge</code>, которая в качестве параметра принимает некоторое число - возраст и возвращает набор 
пользователей (по сути набор объектов User). Создадим для этой функции метод. Для этого изменим класс контекста следующим образом:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>Здесь добавлен метод <code>GetUsersByAge()</code>, который соответствует хранимой функции в БД. Он принимает параметр типа int и возвращает 
объект <code>IQueryable&lt;User&gt;</code>. Этот метод с помощью встроенного в классе DbContext метода <code>FromExpression</code> вызывает 
<code>GetUsersByAge(age)</code>.</p>
<pre class="brush:c#;">public IQueryable&lt;User&gt; GetUsersByAge(int age)  =&gt; FromExpression(() =&gt; GetUsersByAge(age));</pre>
<p>Далее в переопрепределенном методе <code>OnModelCreating()</code> класса контекста нам надо зарегистрировать метод GetUsersByAge с помощью вызова <span class="b">HasDbFunction()</span>:</p>
<pre class="brush:c#;">modelBuilder.HasDbFunction(() =&gt; GetUsersByAge(default));</pre>
<p>Далее мы можем обратиться к хранимой функции GetUsersByAge в коде следующим образом:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
