...
<html >
<head>
<title>Entity Framework Core и C# | Конфигурация подключения</title>
<meta charset="utf-8" />
...
<h2>Конфигурация подключения</h2><div class="date">Последнее обновление: 16.11.2021</div>
...
<p>В этот метод передается объект класса <span class="b">DbContextOptionsBuilder</span>, который позволяет установить параметры подключения. 
...
использовать. Например, для установки подключения к SQLite вызывается метод <code>UseSqlite()</code>, в который передается строка подключения.</p>
<p>Это способ вполне рабочий и может использоваться. Единственно, что хочется отметить, что нам необязательно жестко определять строку подключения внутри контекста, 
мы можем получать ее извне:</p>
<pre class="brush:c#;">
...
<p>Затем при создании объекта контекста передать строку подключения:</p>
<pre class="brush:c#;">
...
<h3>Установка конфигурации в конструкторе</h3>
...
<p>Для этого добавим в проект новый элемент <span class="b">JavaScript JSON Configuration File</span>, который назовем <span class="b">appsettings.json</span>:</p>
...
<p>После добавления файла в Visual Studio для его копирования в каталог приложения в окне свойств необходимо установить для опции 
<span class="b">Copy to Output Directory</span> значение "Copy if newer" (или "Copy always")</p>
<img src="./pics/1.11.png" alt="Файл конфигурации JSON для Entity Framework Core в C#" />
<p>Для работы с БД возьмем ранее определенный класс контекста:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>Далее в файле <span class="b">Program.cs</span> определим следующий код:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>Для создания конфигурации применяется класс <span class="b">ConfigurationBuilder</span>. Метод <span class="b">AddJsonFile()</span> добавляет все настройки из файла конфигурации. 
