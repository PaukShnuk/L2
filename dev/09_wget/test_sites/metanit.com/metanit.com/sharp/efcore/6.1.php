...
<html >
<head>
<title>Entity Framework Core и C# | Выполнение SQL-запросов</title>
<meta charset="utf-8" />
<meta name="description" content="Выполнение SQL-запросов в Entity Framework Core и C#, методы FromSqlRaw и ExecuteSqlRaw, определение параметров в запросах">
...
<h1>SQL в Entity Framework Core</h1><h2>Выполнение SQL-запросов</h2><div class="date">Последнее обновление: 21.11.2021</div>
...
<p>Кроме использования инфраструктуры LINQ to Entities для создания запросов Entity Framework Core также позволяет писать запросы к базе данных 
...
не очень оптимальный sql-запрос.</p>
<h3>FromSqlRaw</h3>
<p>Для получения данных из БД у объектов DbSet определен метод <span class="b">FromSqlRaw()</span>, который принимает в качестве параметра sql-выражение и набор параметров. 
В качестве результата FromSqlRaw возвращает набор полученных из бд объектов.</p>
<p>При этом надо учитывать, что передаваемое в метод FromSqlRaw SQL-выражение не должно извлекать связанные данные, а полученные значения должны соответствовать определению какого-либо класса.</p>
<p>Для примера возьмем следующие модели и контекст данных:</p>
<pre class="brush:c#;">
...
<p>Пусть ранее у нас были добавлены следующие данные:</p>
<pre class="brush:c#;">
...
<p>Например, получим все объекты из таблицы Companies:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
...
<p>Выражение <code>SELECT</code> извлекает данные из таблицы. Так как эта таблица сопоставляется с моделью Company и хранит объекты этой модели, то 
в результате мы получим набор объектов класса Company.</p>
<p>При этом мы можем добавлять к методу другие методы LINQ, которые все вместе будут конкатенироваться в одно общее SQL-выражение:</p>
<pre class="brush:c#;">
var comps = db.Companies.FromSqlRaw("SELECT * FROM Companies").OrderBy(x=&gt;x.Name).ToList();
foreach (var company in comps)
    Console.WriteLine(company.Name);
</pre>
<p>В итоге будет выполняться следующее SQL-выражение:</p>
<pre class="brush:sql;">
SELECT "c"."Id", "c"."Name"
FROM (
...
<p>Также можно использовать метод Include для подгрузки связанных данных:</p>
<pre class="brush:c#;">
...
<h3>Параметры</h3>
<p>Другая версия метода <code>FromSqlRaw()</code> позволяет использовать параметры. Например, выберем из бд все модели, в названии которых есть подстрока "Tom":</p>
<pre class="brush:c#;">
...
<p>Поскольку выше приведенный контекст данных подключается к бд SQLite, то для представления параметров применяется тип <span class="b">SqliteParameter</span> из 
пространства имен <span class="b">Microsoft.Data.Sqlite</span>. Конструктор SqliteParameter имеет ряд версий. В данном случае в конструктор передается два параметра. Первый параметр - название параметра, через который на него можно ссылаться в строке запроса - 
в данном случае "@name". Второй параметр - значение параметра.</p>
<p>Если бы подключение шло к бд MS SQL Server, то для представления параметров применялся бы класс <span class="b">SqlParameter</span> из пространства 
имен <span class="b">Microsoft.Data.SqlClient</span>:</p>
<pre class="brush:c#;">
...
<p>Также мы можем определять параметры как простые переменные:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
	var name = "%Tom%";
...
<h3>ExecuteSqlRaw</h3>
<p>Метод <code>FromSqlRaw()</code> осуществляет выборку из БД, но кроме выборки нам, возможно, придется удалять, обновлять уже существующие или вставлять 
новые записи. Для этой цели применяется метод <span class="b">ExecuteSqlRaw()</span> и его асинхронная версия - <span class="b">ExecuteSqlRawAsync()</span>, которые возвращают количество затронутых командой строк. 
Для его вызова у контекста данных используется свойство <code>Database</code>:</p>
<pre class="brush:c#;">
// вставка
...
<p>Для методов FromSqlRaw и ExecuteSqlRaw в EF Core определены их двойники - методы <span class="b">FromSqlInterpolated()</span> и 
<span class="b">ExecuteSqlInterpolated()</span> (асинхронная версия - <span class="b">ExecuteSqlInterpolatedAsync()</span>), которые позволяют использовать интерполяцию строк для передачи параметров. Пример с 
<span class="b">FromSqlInterpolated</span>:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
	var name = "%Tom%";
...
<p>В этом случае на стороне сервера также будут создаваться параметры для передачи значений name и age наподобие:</p>
<pre class="brush:sql;">
@p0='%Tom%'
@p1=30
...
<p>Использование <span class="b">ExecuteSqlInterpolated()/ExecuteSqlInterpolatedAsync()</span>:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
...
<div class="nav"><p><a href="./5.8.php">Назад</a><a href="./">Содержание</a><a href="./6.2.php">Вперед</a></p></div></div>
<div class="socBlock">
