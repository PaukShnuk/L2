...
<html >
<head>
<title>Entity Framework Core и C# | Конструкторы сущностей</title>
<meta charset="utf-8" />
<meta name="description" content="Конструкторы сущностей в языке C# и их использование в Entity Framework Core, соответствие параметров конструктора и свойств сущностей">
...
<h2>Конструкторы сущностей</h2><div class="date">Последнее обновление: 19.11.2021</div>
...
<p>Когда EF Core создает объект сущности, например, при после получения данных из БД, он вначале вызывае конструктор по умолчанию, который не имеет параметров, 
и затем передает каждому свойству полученные из бд значения.</p>
<p>Если EF Core находит конструктор с параметрами, где названия и типы параметров соответствуют устанавливаемым свойствам, 
...
типу и имени за тем исключением, что названия могут отличаться по регистру, например, свойство <code>Name</code> и параметр <code>name</code>. 
Рассмотрим на примере. Допустим, у нас есть следующая сущность User:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Класс User имеет три свойства и через конструктор устанавливает два из них.</p>
<p>Пусть у нас будет стандартный контекст данных:</p>
<pre class="brush:c#;">
...
<p>И, допустим, в программе создаем несколько объектов User, добавляем их в БД и получаем обратно из БД:</p>
<pre class="brush:c#;">
...
<p>Здесь при получении данных при выполнении метода <code>db.Users.ToList()</code> EF Core будет вызывать для каждой полученной строки из таблицы 
...
<p>Здесь надо учитывать несколько моментов:</p>
<ul>
<li><p>Необязательно для всех свойств определять в конструкторе свои параметры. Например, свойство Id не устанавливается в конструкторе. Те свойства, для 
которых в конструкторе не определено параметров, устанавливаются напрямую, как в общем случае.</p></li>
<li><p>Параметры и свойства должны соответствовать по имени и типу за исключением регистра имени.</p></li>
<li><p>Конструкторы могут иметь любой модификатор доступа, в том числе, <code>private</code>.</p></li>
<li><p>EF Core НЕ устанавливает таким обазом навигационные свойства, которые представляют другие сущности и имеют конструктор:</p>
<pre class="brush:c#;">
public class User
{
    public int Id { get; set; }
...
<p>При этом класс может определять несколько конструкторов с разным количеством параметров:</p>
<pre class="brush:c#;">
public class User
{
    public int Id { get; set; }
