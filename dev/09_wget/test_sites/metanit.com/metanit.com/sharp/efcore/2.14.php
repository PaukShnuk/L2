...
<html >
<head>
<title>Entity Framework Core и C# | Инициализация БД начальными данными</title>
<meta charset="utf-8" />
<meta name="description" content="Инициализация базы данных начальными данными в Entity Framework Core и языке программирования C#, метод HasData">
...
<h2>Инициализация БД начальными данными</h2><div class="date">Последнее обновление: 04.02.2022</div>
...
<p>Иногда необходимо, чтобы при первом обращении база данных уже содержала некоторые данные. И 
Entity Framework Core позволяет инициализировать базу данных при ее создании некоторыми начальными 
данными. Благодаря этому к моменту первого использования базы данных она уже будет содержать начальные данные, которые мы сможем тут же использовать. 
И нам не потребуется вручную или программно добавлять в БД нужные нам данные.</p>
<p>Для инициализации БД при конфигурации определенной модели вызывается метод <span class="b">HasData()</span>, в который передаются добавляемые данные:
<pre class="brush:c#;">
...
<p>Например, инициализируем БД набором данных:</p>
<pre class="brush:c#;">
...
<p>У объекта ModelBuilder, который передается в OnModelCreating в качестве параметра, вызывается метод <span class="b">Entity&lt;T&gt;()</span>. 
Этот метод типизируется типом, для которого будут добавляться начальные данные. То есть в данном случае данные будут добавляться в таблицу, 
где хранятся объекты User. Поэтому Entity типизируется типом User.</p>
<p>Далее по цепочке вызывается метод <span class="b">HasData()</span>, который собственно и определяет начальные данные. 
В данном случае это набор из трех объектов User. При этом для каждого объекта необходимо установить значение первичного ключа - в данном случае значение свойства Id. 
Причем вне зависимости, генерирует ли база данных для данных автоматически индентификатор или нет, нам в любом случае его надо установить - это основное ограничение 
при инициализации БД начальными данными.</p>
<p>При этом следует учитывать, что инициализация начальными данными будет выполняться только в двух случаях:</p>
<ul>
<li><p>При выполнении миграции. (При создании миграции добавляемые данные автоматически включаются в скрипт миграции)</p></li>
<li><p>При вызове метода <span class="b">Database.EnsureCreated()</span>, который создает БД при ее отсутствии</p></li>
</ul>
<p>В случае выше в конструкторе применяется метод <span class="b">Database.EnsureCreated()</span>, поэтому при создании контекста данных</p>
<pre class="brush:c#;">
ApplicationContext db = new ApplicationContext();
</pre>
<p>будет автоматически производиться инициализация бд начальными данными.</p>
<p>Аналогично можно инициализировать данные нескольких сущностей, в том числе связанных между собой:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
