...
<html >
<head>
<title>Entity Framework Core и C# | Cортировка и проекция из базы данных</title>
<meta charset="utf-8" />
<meta name="description" content="Cортировка и проекция из базы данных в LINQ to Entities в Entity Framework Core и C#, методы OrderBy, ThenByDescending, Select">
...
<h2>Cортировка и проекция из базы данных</h2><div class="date">Последнее обновление: 21.11.2021</div>
...
<h3>Проекция</h3>
<p>Проекция позволяет получить из набора объектов одного типа набор объектов другого типа.</p>
<p>Пусть у нас есть те же классы, что и в прошлой теме:</p>
<pre class="brush:c#;">
...
<p>Допустим, нам надо добавить в результат выборки название компании. Мы можем использовать метод <span class="b">Include</span> 
для подсоединения к объекту связанных данных из другой таблицы: <code>var users = db.Users.Include(p=&gt;p.Company)</code>. Но не всегда нужны все свойства 
выбираемых объектов. В этом случае мы можем применить метод <span class="b">Select</span> для проекции извлеченных данных на новый тип:</p>
<pre class="brush:c#;">
...
<p>В итоге метод Select из полученных данных спроецирует новый тип. При выполнении этот запрос будет трансформироваться в следующее выражение SQL:</p>
<pre class="brush:sql;">
SELECT "u"."Name", "u"."Age", "c"."Name" AS "Company"
FROM "Users" AS "u"
INNER JOIN "Companies" AS "c" ON "u"."CompanyId" = "c"."Id"
</pre>
<p>В даном случае мы получим данные анонимного типа, но это также может быть определенный пользователем тип. Например:</p>
<pre class="brush:c#;">
...
<p>И спроецируем выборку на этот тип:</p>
<pre class="brush:c#;">
...
<h3>Сортировка</h3>
<p>Чтобы отсортировать полученные из бд данных по определенному критерию по возрастанию применяется метод <span class="b">OrderBy</span>, либо 
оператор <span class="b">orderby</span>. Например, отсортируем объекты по возрастанию по свойству Name:</p>
<pre class="brush:c#;">
using(ApplicationContext db = new ApplicationContext())
{
...
<p>В результате Entity Framework будет генерировать следующее выражение SQL, которое будет упорядочивать данные:</p>
<pre class="brush:sql;">
SELECT "u"."Id", "u"."Age", "u"."CompanyId", "u"."Name"
FROM "Users" AS "u"
ORDER BY "u"."Name"
</pre>
<p>В качестве альтернативы методу OrderBy можно использовать оператор <span class="b">orderby</span>:</p>
<pre class="brush:c#;">
...
<p>Для сортировки по убыванию применяется метод <span class="b">OrderByDescending()</span>:</p>
<pre class="brush:c#;">
var users = db.Users.OrderByDescending(u=&gt;u.Name);
</pre>
<p>В этом случае к выражению SQL будет добавляться оператор DESC:</p>
<pre class="brush:sql;">
SELECT "u"."Id", "u"."Age", "u"."CompanyId", "u"."Name"
FROM "Users" AS "u"
ORDER BY "u"."Name" DESC
</pre>
<p>При необходимости упорядочить данные сразу по нескольким критериям можно использовать методы <span class="b">ThenBy()</span>(для сортировки по возрастанию) и 
<span class="b">ThenByDescending()</span>. Например, отсортируем по двум значениям:</p>
<pre class="brush:c#;">
