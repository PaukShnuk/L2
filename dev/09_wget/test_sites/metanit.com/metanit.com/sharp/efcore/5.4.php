...
<html >
<head>
<title>Entity Framework Core и C# | Операции с множествами: объединение, пересечение, разность</title>
<meta charset="utf-8" />
<meta name="description" content="Операции с множествами: объединение, пересечение, разность в LINQ to Entities в Entity Framework Core и C#, методы Union, Intersect, Except">
...
<h2>Операции с множествами: объединение, пересечение, разность</h2><div class="date">Последнее обновление: 21.11.2021</div>
...
<p>Ряд методов Linq позволяют работать с результатами выборки как со множествами, производя операции на объединение, пересечение, разность двух выборок.</p>
<p>Но перед использованием данных методов надо учитывать, что они проводятся над однородными выборками с одинаковым определением строк, то есть которые совпадают по составу столбцов.</p>
<p>Для примеров возьмем модели из прошлых тем:</p>
<pre class="brush:c#;">
...
<p>На примере следующих данных:</p>
<pre class="brush:c#;">
...
<h3>Объединение</h3>
<p>Для объединения двух выборок используется метод <span class="b">Union()</span>:</p>
<pre class="brush:c#;">
using(ApplicationContext db = new ApplicationContext())
{
...
<p>Метод Union в качестве параметра принимает результаты второй выборки и объединяет ее с исходной. В результате мы получим два подзапроса SQL, результаты которых 
объединяются в общий набор:</p>
<pre class="brush:sql;">
...
<p>При этом мы не можем объединить две разнородные выборки, например, таблицу, пользователей и таблицу компаний. Однако уместна следующая запись:</p>
<pre class="brush:c#;">
var result = db.Users.Select(p =&gt; new { Name = p.Name })
    .Union(db.Companies.Select(c =&gt; new { Name = c.Name }));
</pre>
<p>Первая выборка после метода Select будет формировать набор элементов с одним столбцом Name. Вторая выборка из таблицы компаний после метода Select 
также будет формировать набор элементов с одним столбцом Name. Поэтому строки в обоих выборках будут однородны, и мы их сможем объединять.</p>
<h3>Пересечение</h3>
<p>Чтобы найти пересечение выборок, то есть те элементы, которые присутствуют сразу в двух выборках, используется метод <span class="b">Intersect()</span>. 
Например, выберем все пользователей, у которых возраст больше 30 и в имени содержится подстрока "Tom":</p>
<pre class="brush:c#;">
using(ApplicationContext db = new ApplicationContext())
{
	var users = db.Users.Where(u =&gt; u.Age &gt; 30)
...
<p>Несмотря на то, что операция пересечения не равна операции объединения, рассмотренной выше, но при выполнении пересечение будет выполнять те же SQL-выражения:</p>
<pre class="brush:sql;">
SELECT "u"."Id", "u"."Age", "u"."CompanyId", "u"."Name"
FROM "Users" AS "u"
WHERE "u"."Age" &gt; 30
...
<h3>Разность</h3>
<p>Если нам надо найти элементы первой выборки, которые отсутствуют во второй выборке, то мы можем использовать метод <span class="b">Except</span>:</p>
<pre class="brush:c#;">
using(ApplicationContext db = new ApplicationContext())
{
...
<p>В результате будет формироваться следующий SQL-запрос:</p>
<pre class="brush:sql;">
SELECT "u"."Id", "u"."Age", "u"."CompanyId", "u"."Name"
FROM "Users" AS "u"
WHERE "u"."Age" &gt; 30
