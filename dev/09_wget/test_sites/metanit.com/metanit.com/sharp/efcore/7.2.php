...
<html >
<head>
<title>Entity Framework Core и C# | MySQL</title>
<meta charset="utf-8" />
<meta name="description" content="MySQL в Entity Framework Core и C#, провайдер Pomelo.EntityFrameworkCore.MySql, метод UseMySQL, поделючение и операции с базой данных">
...
<h2>MySQL</h2><div class="date">Последнее обновление: 22.11.2021</div>
...
<p>Для подключения к MySQL добавим через Nuget пакет <span class="b">Pomelo.EntityFrameworkCore.MySql</span>:</p>
<img src="./pics/7.2.png" alt="MySQL в Entity Framework Core" />
<p>Стоит отметить, что также есть официальный провайдер от Oracle - <span class="b">MySql.EntityFrameworkCore</span>, но он развивается довольно 
медленно, кроме того, не имеет поддержки ряда функционала. Поэтому предпочтительнее использовать пакет <span class="b">Pomelo.EntityFrameworkCore.MySql</span>.</p>
<p>Для работы определим класс User:</p>
<pre class="brush:c#;">
public class User
{
...
объект <code>Version</code>, который собственно содержит номер установленной версии MySQL. Например, в моем случае это версия <span class="b">8.0.25</span>, 
соответственно я передаю объект <code>new MySqlServerVersion(new Version(8, 0, 25))</code>. Версию MySQL можно узнать, например, через MySQL Workbench</p>
<p>И для тестирования определим в файле <span class="b">Program.cs</span> добавление и вывод данных:</p>
<pre class="brush:c#;">
// добавление данных
