...
<html >
<head>
<title>Entity Framework Core и C# | Использование полей сущности</title>
<meta charset="utf-8" />
<meta name="description" content="Сопоставление полей моделей в Entity Framework Core в языке программирования C# со столбцами таблицы, приватные поля">
...
<h2>Использование полей сущности</h2><div class="date">Последнее обновление: 18.11.2021</div>
...
<p>Кроме свойств Entity Framework также может использовать поля класса (в том числе приватные) для сопоставления со столбцами. 
Например, возьмем следующую сущность:</p>
<pre class="brush:c#;">
public class User
{
    int id;
...
<p>Здесь в классе User определено три поля. Все они приватные, недоступные извне. Кроме того, есть два свойства для чтения, которые возвращают 
значения полей. Два поля - name и age устанавливаются только через конструктор. Третье поле - id, как мы ожидаем, будет устанавливаться при добавлении 
объекта сущности в базу данных.</p>
<p>Настроим класс контекста для применения этих полей:</p>
<pre class="brush:c#;">
...
<p>Для сопоставления полей со свойствами и столбцами применяется Fluent API. Чтобы использовать поле в качестве свойства при сопоставлении со столбцами применяется 
метод <span class="b">Property()</span>, в который передается название поля:</p>
<pre class="brush:c#;">modelBuilder.Entity&lt;User&gt;().Property("name");</pre>
<p>То есть в данном случае мы говорим, что мы хотим, чтобы поле "name" выступало в качестве свойства сущности и сопоставлялось со столбцом в таблице бд.</p>
<p>С помощью метода <span class="b">HasField()</span> устанавливается поле, которое используется для свойства. Так, в выражении</p>
<pre class="brush:c#;">modelBuilder.Entity&lt;User&gt;().Property("Id").HasField("id");</pre>
<p>Для свойства Id будет использоваться поле id</p>
<p>В итоге Entity Framework создаст следующую таблицу:</p>
<img src="./pics/2.18.png" alt="Поля класса в Entity Framework и C#" />
<p>В программе мы можем создать объект User и добавить в бд:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./2.10.php">Назад</a><a href="./">Содержание</a><a href="./2.6.php">Вперед</a></p></div></div>
<div class="socBlock">
