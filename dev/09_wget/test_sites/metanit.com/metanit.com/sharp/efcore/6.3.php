...
<html >
<head>
<title>Entity Framework Core и C# | Хранимые процедуры</title>
<meta charset="utf-8" />
<meta name="description" content="Хранимые процедуры в Entity Framework Core, определение и вызов хранимых процедур из базы данных в C#, выходные параметры процедур">
...
<h2>Хранимые процедуры</h2><div class="date">Последнее обновление: 22.11.2021</div>
...
<p>Рассмотрим, как вызывать хранимые процедуры из кода на C# через Entity Framework Core на примере БД MS SQL Server.</p>
<p>Пусть наша база данных, как и в прошлой теме, описывается следующим контекстом данных и сущностями:</p>
<pre class="brush:c#;">
...
<p>Вначале создадим саму хранимую процедуру, к которой мы будем обращаться. Для этого в Visual Studio в окне <span class="b">SQL Server Object Explorer</span> откроем узел базы данных. В узле базы данных откроем подузел 
Programmability -&gt; Stored Procedures</span>:</p>
<img src="./pics/6.1.png" alt="Entity Framework Core и Хранимые процедуры" />
<p>Далее нажмем на этот узел правой кнопкой мыши и выберем <span class="b">Add New Stored Procedure</span>:</p>
<p>После этого Visual Studio генерирует и октрывает файл скрипта хранимой процедуры со следующим содержимым:</p>
<pre class="brush:sql;">
...
<p>Данная процедура ищет все строки, где значение столбца название компании равно строке, переданной через параметр @name.</p>
<p>Далее нажмем на кнопку Update для добавления хранимой процедуры:</p>
<img src="./pics/6.2.png" alt="Создание хранимой процедуры в Entity Framework Core" />
<p>И затем в появившемся окошке нажимаем кнопку Update Database:</p>
<img src="./pics/6.3.png" alt="Добавление хранимой процедуры в MS SQL Server" />
<p>После этого в узле <span class="b">Stored Procedures</span> появится новая хранимая процедура, и мы сможем ее использовать.</p>
<p>Теперь обратимся к процедуре в коде C#:</p>
<pre class="brush:c#;">
...
<p>Параметр в методе <code>FromSqlRaw</code> принимает название процедуры, после которого идет перечисление параметров: <code>GetUsersByCompany @name</code></p>
<h3>Выходные параметры процедуры</h3>
<p>В некоторых случаях процедура может возвращать не набор данных из таблиц, а какие-то отдельные данные. Как правило, для этого используются выходные параметры. 
Например, нам надо получить имя пользователя с максимальным возрастом. Для этого определим следующую хранимую процедуру:</p>
<pre class="brush:sql;">
...
<p>Параметр <code>name</code> здесь определен как выходной с ключевым словом <span class="b">OUTPUT</span>. Через него будет передаваться имя пользователя.</p>
<p>Обратимся к этой процедуре в коде C#:</p>
<pre class="brush:c#;">
using Microsoft.Data.SqlClient;
using Microsoft.EntityFrameworkCore;
...
<p>Здесь параметр userName определен как выходной. Так как нам в данном случае не надо возвращать набор данных, который соответствует одной из моделей, то 
для выполнения запроса используется метод <code>db.Database.ExecuteSqlRaw()</code>. И после его выполнения через свойство <span class="b">param.Value</span> мы 
сможем получить значение, переданное через параметр.</p>
...
<div class="nav"><p><a href="./6.2.php">Назад</a><a href="./">Содержание</a><a href="./2.11.php">Вперед</a</p></div></div>
<div class="socBlock">
