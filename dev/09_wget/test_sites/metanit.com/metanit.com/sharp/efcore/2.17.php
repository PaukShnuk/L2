...
<html >
<head>
<title>Entity Framework Core и C# | Настройка индексов</title>
<meta charset="utf-8" />
<meta name="description" content="Настройка индексов в Entity Framework Core и языке программирования C#, составные индексы, фильтры индексов, метод HasIndex и HasFilter">
...
<h2>Настройка индексов</h2><div class="date">Последнее обновление: 18.11.2021</div>
...
<p>Для увеличения производительности поиска в базе данных применяются индексы. По умолчанию индекс создается для каждого свойства, которое используется в качестве внешнего ключа. 
Однако Entity Framework также позволяет создавать свои индексы.</p>
<h3>Настройка индексов с помощью атрибутов</h3>
<p>Для создания индекса можно использовать атрибут <span class="b">[Index]</span>. Например:</p>
<pre class="brush:c#;">
...
<p>Первый и обязательный параметр атрибута указывает на свойство (или набор свойств), с которым будет ассоциирован индекс. В данном случае это свойство PhoneNumber.</p> 
<p>Но также он может принимать набор свойств, для которых создается индекс. В этом случае названия свойств просто перечисляются через запятую:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>С помощью дополнительных параметров можно настроить уникальность и имя индекса:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>В данном случае индекс будет называться Phone_Index, а значение <code>IsUnique = true</code> указывает, что индекс должен быть уникальным.</p>
<h3>Настройка индексов с помощью Fluent API</h3>
<p>Для создания индекса через Fluent API применяется метод <span class="b">HasIndex()</span>:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<h4>Уникальность индексов</h4>
<p>С помощью дополнительного метода <span class="b">IsUnique()</span> можно указать, что индекс должен иметь уникальное значение. 
Тем самым мы гарантируем, что в базе данных может быть только один объект с определенным значением для свойства-индекса:</p>
<pre class="brush:c#;">
protected override void OnModelCreating(ModelBuilder modelBuilder)
{
...
<p>Также можно определить индексы сразу для нескольких свойств:</p>
<pre class="brush:c#;">
protected override void OnModelCreating(ModelBuilder modelBuilder)
{
...
<h4>Имя индекса</h4>
<p>Для установки имени индекса применяется метод <span class="b">HasDatabaseName()</span>, в который передается имя индекса:</p>
<pre class="brush:c#;">
protected override void OnModelCreating(ModelBuilder modelBuilder)
{
...
<p>В данном случае для индекса будет использоваться свойство PhoneNumber, а называться он будет "PhoneIndex".</p>
<h4>Фильтры индексов</h4>
<p>Некоторые системы управления базами данных позволяют определять индексы с фильрами или частичные индексы, которые позволяют выполнять индексацию только 
по ограниченному набору значений, что увеличивает производительность и уменьшает использование дискового простанства. И EntityFramework Core также 
позволяет создавать подобные индексы. Для этого применяется метод <span class="b">HasFilter()</span>, в который передается sql-выражение, которое определяет условие фильтра. 
Например:</p>
<pre class="brush:c#;">
protected override void OnModelCreating(ModelBuilder modelBuilder)
{
...
<p>В данном случае в качестве индекса будет использоваться столбец PhoneNumber. Причем только для тех строк, у которых в столбце PhoneNumber значение не равно NULL.</p>
