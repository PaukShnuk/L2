...
<html >
<head>
<title>Entity Framework Core и C# | Модели, Fluent API и аннотации данных</title>
<meta charset="utf-8" />
...
<h1>Создание модели в Entity Framework Core</h1><h2>Fluent API и аннотации данных</h2><div class="date">Последнее обновление: 17.11.2021</div>
...
<p><span class="b">Модель</span> в Entity Framework представляет набор всех сущностей и связей между ними, которыми управляет контекст данных. 
Все сущности, с которыми работает Entity Framework Core и которые хранятся в базе данных, определяются в C# в виде классов. При этом 
Entity Framework применяет ряд условностей для сопоставления классов с таблицами. Например, названия столбцов должны 
...
<h3>Fluent API</h3>
<p><span class="b">Fluent API</span> представляет набор методов, которые определяют сопоставление между классами и их свойствами и таблицами и их столбцами. Для использования 
функционала Fluent API переопределяется метод <span class="b">OnModelCreating()</span>:</p>
<pre class="brush:c#;">
...
<h3>Аннотации</h3>
<p>Аннотации представляют настройку классов сущностей с помощью атрибутов. Большинство подобных атрибутов располагаются в пространстве 
<span class="b">System.ComponentModel.DataAnnotations</span>, которое нам надо подключить перед использованием аннотаций. Например:</p>
<pre class="brush:c#;">
...
<p>В данном случае атрибут <code>Column</code> представляет аннотацию, которая указывает, что свойство Id будет сопоставляться со столбцом "user_id" (а не Id, как бы было по умолчанию).</p>
<p>Таким образом, мы можем использовать три подхода к определению модели:</p>
<ul>
...
<div class="nav"><p><a href="./7.3.php">Назад</a><a href="./">Содержание</a><a href="./2.3.php">Вперед</a></p></div></div>
<div class="socBlock">
