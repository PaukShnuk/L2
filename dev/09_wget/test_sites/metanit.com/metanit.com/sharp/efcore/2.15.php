...
<html >
<head>
<title>Entity Framework Core и C# | Обязательные и необязательные свойства</title>
<meta charset="utf-8" />
<meta name="description" content="Обязательные и необязательные свойства в Entity Framework Core и языке программирования C#, атрибут Required и метод IsRequired в Fluent API, ссылочные nullable-типы">
...
<h2>Обязательные и необязательные свойства</h2><div class="date">Последнее обновление: 19.11.2021</div>
...
<p>По умолчанию свойство является необязательным к установке, если оно допускает значение <span class="b">null</span>. Это свойства, которые представляют nullable-типы, 
например, <span class="b">string?</span>, <span class="b">int?</span> и т.д. Хотя мы также можем настроить эти свойства как обязательные.</p>
<p>Свойство является обязательным, если оно не допускает значение <span class="b">null</span>.</p>
<p>Например, возьмем следующую модель:</p>
<pre class="brush:c#;">
public class User
{
...
<p>В данном случае свойство Name не представляет nullable-тип, поэтому оно рассматривается как обязательное (как и свойство Age). А свойство 
Company представляет nullable-тип - <code>string?</code>, соответственно является необязательным. Поэтому для этой сущности в SQLite будет сгенерирована 
следующая таблица:</p>
<pre class="brush:sql;">
CREATE TABLE "Users" (
	"Id"	INTEGER NOT NULL,
...
<p>Здесь мы видим, что столбец Company допускает значение NULL, а столбец Name - не допускает благодаря установке атрибута <span class="b">NOT NULL</span>. 
Хотя здесь приведен пример бд SQLite, но для других систем баз данных будет действовать аналогичная логика.</p>
<h3>Атрибут Required</h3>
<p>Атрибут <span class="b">Required</span> указывает, что данное свойство обязательно для установки, то есть будет иметь определение NOT NULL в БД, даже если оно представляет 
nullable-тип:</p>
<pre class="brush:c#;">
...
<p>А столбец Name в базе данных будет определен как NOT NULL.</p>
<p>Если мы не установим свойство Name у объекта User и попытаемся добавить этот объект в бд, то получим во время выполнения исключение типа <span class="b">Microsoft.EntityFrameworkCore.DbUpdateException</span>:</p>
<pre class="brush:c#;">
