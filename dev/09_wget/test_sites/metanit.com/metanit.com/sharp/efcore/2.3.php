...
<html >
<head>
<title>Entity Framework Core и C# | Определение моделей</title>
<meta charset="utf-8" />
<meta name="description" content="Определение моделей в Entity Framework Core, включение и исключение классов из модели, nullable и контекст данных, методы Entity и Ignore, атрибут NotMapped">
...
<h2>Определение моделей</h2><div class="date">Последнее обновление: 18.11.2021</div>
...
<h3>Включение сущностей в модель</h3>
<p>По умолчанию все типы сущностей, для которых определены в контексте данных наборы <span class="b">DbSet</span>, включаются в модель и в дальнейшем 
сопоставляются с таблицами в базе данных. Например:</p>
<pre class="brush:c#;">
...
<p>В данном случае, поскольку для класса User в классе контекста определено свойство типа DbSet</p>
<pre class="brush:c#;">public DbSet&lt;User&gt; Users { get; set; } = null!;</pre>
<p>Таким образом, для сущности User будет создана таблица в бд.</p>
<h3>Ссылочные nullable-типы и DbSet</h3>
<p>Класс <span class="b">DbSet</span>, как и другие типы, является ссылочным. А, начиная с C# 10 и .NET 6 автоматически применяется функциональность ссылочных 
nullable-типов. И переменные/свойства тех типов, которые не являются nullable, следует инициализировать некотором значением перед их использованием. Например, 
если мы напишем:</p>
<pre class="brush:c#;">public DbSet&lt;User&gt; Users { get; set; }</pre>
<p>То мы столкнемся с предупреждением:</p>
<img src="./pics/2.16.png" alt="Nullable и DbSet в Entity Framework Core" />
<p>То есть нам надо инициализировать свойство типа DbSet. Хотя в этом нет большого смысла, так как контструктор базового класса DbContext 
гарантирует, что все свойства типа <span class="b">DbSet</span> будут инициализированы и соответственно в принципе не будут иметь 
значение <span class="b">null</span>.</p>
<p>Тем не менее проблема остается, поскольку мы сталкиваемся с предупреждением. Чтобы выйти из этой ситуации мы можем инициализировать свойство 
с помощью выражения <span class="b">null!</span>, которое говорит, что данное свойство в принципе не будет иметь значение null:</p>
<pre class="b">public DbSet&lt;User&gt; Users { get; set; } = null!;</pre>
<p>Другое решение - инициализировать свойство значением типа <span class="b">Set&lt;T&gt;</span>:</p>
<pre class="brush:c#;">public DbSet&lt;User&gt; Users =&gt; Set&lt;User&gt;();</pre>

<h3>Включение сущностей в модель без DbSet</h3>
<p>Но кроме того, в модель также включаются типы, на которые есть ссылки в сущностях, которые 
уже включены в модель, например, через свойства DbSet.</p>
<p>Например, пусть у нас определены следующие сущности:</p>
<pre class="brush:c#;">
public class User
{
    public int Id { get; set; }
...
<p>И пусть у нас будет класс контекста данных:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>После создания базы данных в ней будут созданы две таблицы: Users и Company. А третий класс - Country никак не используется в сущностях User и Company, для Country нет свойства 
DbSet в классе контекста, поэтому она не будет включена в контекст и для нее не будет создаваться таблица в бд.</p>
<img src="./pics/2.2.png" alt="Навигационное свойство в Entity Framework Core и C#" />
<p>Поскольку для типа User определен набор DbSet, то для имени таблицы будет применяться имя этого набора, а для второй таблицы будет использоваться 
имя класса Company.</p>
<p>Еще один способ включения сущности в модель представляет вызов <span class="b">Entity()</span> объекта <span class="b">ModelBuilder</span> в методе <span class="b">OnModelCreating()</span>:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>И если мы сейчас создадим и выполним миграции, то в базе данных будут уже три таблицы для сущностей:</p>
<img src="./pics/2.3.png" alt="Fluent API в Entity Framework Core и C#" />
<h3>Исключение из модели</h3>
<p>Иногда возникают ситуации, когда надо, наоборот, исключить сущность из модели. Например, в примере выше сущность Company ссылается на класс Company, и, допустим, 
мы не хотим, чтобы в базе данных была таблица Company. В этом случае мы можем использовать Fluent API или аннотации данных.</p>
<p>Применение Fluent API заключается в вызове метода <span class="b">Ignore()</span>:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>Аннотации данных предполагают установку над классом атрибута <span class="b">[NotMapped]</span>:</p>
<pre class="brush:c#;">
...
<p>При исключении сущности Company в базе данных будет только одна таблица Users, причем она не будет содержать столбца, который бы сопоставлялся со свойством 
Company класса User:</p>
<img src="./pics/2.4.png" alt="Исключение сущностей из модели и Ignore и NotMapped в Entity Framework Core" />
...
<div class="nav"><p><a href="./2.1.php">Назад</a><a href="./">Содержание</a><a href="./2.4.php">Вперед</a></p></div></div>
<div class="socBlock">
