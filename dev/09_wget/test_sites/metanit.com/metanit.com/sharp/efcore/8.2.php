...
<html >
<head>
<title>Entity Framework Core и C# | Проекция запросов на представления</title>
<meta charset="utf-8" />
<meta name="description" content="Query Types в Entity Framework Core и C#, сопоставление классов с представлением в базе данных, DbQuery, CREATE VIEW">
...
<h2>Проекция запросов на представления</h2><div class="date">Последнее обновление: 22.11.2021</div>
...
<p>Обычно таблица в базе данных сопоставляется с определенным классом модели в Entity Framework. Но EF Core также 
позволяет сопоставлять наборы DbSet в контексте данных с представлениями в базе данных, а классы C# - с данными, которые возвращаются этими представлениями.</p>
<p>При определении классов, которые будут сопоставляться с данными представлений, следует учитывать следующие ограничения:</p>
<ul>
<li><p>Для них не надо определять первичный ключ</p></li>
<li><p>Изменения в подобных типах не отлеживаются контекстом данных.</p></li>
<li><p>Мы можем получать данные подобных типов. Но мы не можем добавлять, изменять или удалять объекты подобных типов.</p></li>
<li><p>Классы сущностей не могут содержать навигационные свойства на подобные типы.</p></li>
</ul>
<p>Рассмотрим простой пример. Допустим у нас есть следующие классы сущностей товара и производящей его компании:</p>
<pre class="brush:c#;">
...
<p>Каждая из этих сущностей будет сопоставляться с определенной таблицей в бд. Но, допустим, мы хотим группировать выборку по каждой 
компании и получать количество товаров компании и их полную сумму. Для этого мы можем использовать разные способы. Например, написать SQL-запрос и 
получать его результаты или написать представление в бд. В данном случае мы создадим представление. Но в начале определим следующий класс, который будет сопоставляться с представлением:</p>
<pre class="brush:c#;">
...
<p>Данный класс и будет представлять тип, который будет сопоставляться не с таблицей, а с данными представления, которое мы создадим позже.</p>
<p>Далее определим контекст данных:</p>
<pre class="brush:c#;">
...
<p>В классе контекста данных мы видим, что определено свойство <code>public DbSet&lt;CompanyProductsGroup&gt; ProductsByCompany { get; set; }</code>. 
Однако в отличие от других наборов DbSet оно будет сопоставляться с представлением. Для этого для этого типа в методе <code>OnModelCreating()</code> 
определяются две важные настройки.</p>
<p>Во-первых, указывается, что данная сущность (CompanyProductsGroup) не будет содержать ключа:</p>
<pre class="brush:c#;">pc.HasNoKey();</pre>
<p>Во-вторых, указывается, что данныя сущность будет сопоставляться с представлением БД:</p>
<pre class="brush:c#;">pc.ToView("View_ProductsByCompany");</pre>
<p>В данном случае представление называется "View_ProductsByCompany". Оно уже может существовать в базе данных, а может отсутствовать. Далее мы создадим это представление.</p>
<p>Теперь используем все это в программе:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>Для ясности я разделил блок инициализации БД и блок выборки из БД. В первом блоке using, где происходить инициализация базы начальными данными, 
также добавляется представление с помощью команды <code>CREATE VIEW</code>. (Предсталение можно естественно создать и вручную непосредственно в самой базе данных). Это представление как раз и отвечает за группировку данных. 
...
со структорой данных, которые возвращает представление.</p>
<pre class="brush:c#;">
db.Database.ExecuteSqlRaw(@"CREATE VIEW View_ProductsByCompany AS 
							SELECT c.Name AS CompanyName, Count(p.Id) AS ProductCount, Sum(p.Price * p.TotalCount) AS TotalSum
							FROM Companies c
...
<p>Далее мы можем получить данные через это представление также, как мы получаем данные из таблиц:</p>
<pre class="brush:c#;">var companyProducts = db.ProductsByCompany.ToList();</pre>
...
<p>И после выполнения кода в базе данных мы сможем увидеть две таблицы и одно представление:</p>
<img src="./pics/6.14.png" alt="Предсталения View в Entity Framework Core" />
