...
<html >
<head>
<title>Entity Framework Core и C# | Свойства сущности</title>
<meta charset="utf-8" />
<meta name="description" content="Свойства моделей в Entity Framework Core и языке C#, исключение сопоставления свойств сущностей со столбцами в базе данных, метод Ignore и атрибут NotMapped">
...
<h2>Свойства сущности</h2><div class="date">Последнее обновление: 18.11.2021</div>
...
<p>По умолчанию модель включает все свойства сущности, которые определены как публичные и которые открыты для записи и чтения. Например,</p>
<pre class="brush:c#;">
public class User
{
...
<p>Класс User имеет три публичных свойства, поэтому при чтении или записи в базу данных Entity Framework будет автоматически сопоставлять столбцы из таблицы с 
этими свойствами по имени. Но такое поведение не всегда необходимо. Иногда требуется, наоборот, исключить определенное свойство, чтобы для него не создавался столбец в таблице.</p>
<p>Исключение с помощью Fluent API производится через метод <span class="b">Ignore()</span>:</p>
<pre class="brush:c#;">
...
<p>Здесь свойство Address исключается из модели, и для него не будет создаваться столбец в таблице Users.</p>
<p>Исключение с помощью аннотаций данных:</p>
<pre class="brush:c#;">
...
<p>В обоих случаях при миграции будет создана таблица Users, которая не будет содержать столбца для свойства Address, и оно не будет участвовать в сопоставлениях 
при операциях с бд:</p>
<img src="./pics/2.5.png" alt="Column Mapping и NotMapped в Entity Framework Core" />
<h3>Использование полей класса</h3>
<p>В примере выше применялись автосвойства, которые представляют сокращенную версию свойств без полноценных блоков get и set. Однако свойства не обязательно 
должны представлять именно автосвойства. Для хранения значений они могут использовать поля класса и иметь полноценные блоки get и set. Например:</p>
<pre class="brush:c#;">
public class User
{
    string name;
...
<div class="nav"><p><a href="./2.3.php">Назад</a><a href="./">Содержание</a><a href="./2.10.php">Вперед</a></p></div></div>
<div class="socBlock">
