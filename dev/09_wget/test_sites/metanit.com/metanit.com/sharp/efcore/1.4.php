...
<html >
<head>
<title>Entity Framework Core и C# | Основные операции с данными</title>
<meta charset="utf-8" />
<meta name="description" content="Основные операции с данными в Entity Framework Core 6.0 в языке программирования C#, интерфейс CRUD">
...
<h2>Основные операции с данными. CRUD</h2><div class="date">Последнее обновление: 15.11.2021</div>
...
<p>Для примера проект консольного приложения на C# и для работы с бд SQLite через Entity Framework добавим в него NuGet-пакет 
<span class="b">Microsoft.EntityFrameworkCore.Sqlite</span>.<p>
...
<p>Далее в файле <span class="b">Program.cs</span> определим все базовые операции с данными:</p>
<pre class="brush:c#;">
// Добавление
...
<p>Но надо отметить, что в данном случае действие контекста данных ограничивается пределами конструкции using, которая определяет область действия объекта ApplicationContext. Но рассмотрим другой пример. Мы получаем объект в одном месте,а  обновляем в другом. 
Например:</p>
<pre class="brush:c#;">
User? user = null;
using (ApplicationContext db = new ApplicationContext())
...
<p>Несмотря на то, что объект user не равен null, имеется в базе данных, но во втором блоке using обновления соответствующего объекта в БД не произойдет. Потому что объект User больше не отслеживается контекстом данных. 
И в этом случае во второй конструкции using, где происходит редактирование, нам надо использовать метод Update:</p>
<pre class="brush:c#;">
// Редактирование
using (ApplicationContext db = new ApplicationContext())
{
...
<h3>Асинхронный API</h3>
<p>Вместо метода <span class="b">SaveChanges()</span> для асинхронного выполнения зароса к бд можно использовать его асинхронный двойник - 
<span class="b">SaveChangesAsync()</span>. Также, для добавления данных определены асинхронные методы <span class="b">AddAsync</span> и 
<span class="b">AddRangeAsync</span>. Пример применения асинхронного API:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./2.16.php">Назад</a><a href="./">Содержание</a><a href="./1.5.php">Вперед</a></p></div></div>
<div class="socBlock">
