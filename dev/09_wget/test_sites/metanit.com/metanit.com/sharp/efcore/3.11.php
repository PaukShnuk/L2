...
<html >
<head>
<title>Entity Framework Core и C# | Иерархические данные</title>
<meta charset="utf-8" />
<meta name="description" content="Иерархические данные в Entity Framework Core и C#, ссылка сущности на саму себя, хранение древовидых структур">
...
<h2>Иерархические данные</h2><div class="date">Последнее обновление: 23.11.2021</div>
...
<p>Некоторые данные представляют иерархическую структуру по типу деревьев, например, меню, где один пункт меню может иметь подменю. Или файловая система, 
где в каталоге могут быть другие каталоги. Рассмотрим, как в Entity Framework мы можем создать и использовать подобную иерархическую структуру.</p>
<p>Определим следующую сущность MenuItem:</p>
<pre class="brush:c#;">
...
<p>MenuItem представляет пункт меню, который может иметь подменю. Свойство Title представляет некоторый текст пункта меню. Свойство <code>Parent</code> 
хранит ссылку на родительский пункт меню при его наличии, а <code>ParentId</code> - id родительского пункта меню. Причем поскольку у пунктов меню верхнего уровня 
не может быть родительских меню, то свойства <code>ParentId</code> и <code>Parent</code> определены как необязательные. А свойство <code>Children</code> 
представляет пункты подменю.</p>
<p>И определим следующий контекст данных:</p>
<pre class="brush:c#;">
...
<p>В итоге при создании базы данных будет генерироваться следующая таблица:</p>
<pre class="brush:sql;">
