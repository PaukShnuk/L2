...
<html >
<head>
<title>Entity Framework Core и C# | Ограничения свойств</title>
<meta charset="utf-8" />
<meta name="description" content="Ограничения свойств по длине в Entity Framework Core и языке программирования C#, атрибут MaxLength, методы HasCheckConstraint и HasMaxLength в Fluent API">
...
<h2>Ограничения свойств</h2><div class="date">Последнее обновление: 19.11.2021</div>
...
<h3>Установка ограничений</h3>
<p>С помощью метода <span class="b">HasCheckConstraint()</span> можно установить ограничение для столбца. На уровне базы данных это приведет к установке для столбца 
атрибута <span class="b">CHECK</span>, который задает ограничение.</p>
<p>В метод <span class="b">HasCheckConstraint()</span> передается название столбца и sql-выражение, которое выполняет проверку корректности передаваемого значения. 
Например, у пользователя есть возраст, который должен укладываться в некоторые допустимые рамки. К примеру, возраст не может быть меньше 0 и больше 120:</p>
<pre class="brush:c#;">
...
<p>В выражении</p>
<pre class="brush:c#;">HasCheckConstraint("Age", "Age &gt; 0 AND Age &lt; 120");</pre>
<p>в качестве первого параметра передается название столбца - в данном случае столбец "Age", а в качестве второго параметра - выражение SQL, которое будет 
использоваться в качестве ограничения.</p>
<p>В этом случае на уровне базы данных SQLite для сущности User будет создаваться следующая таблица:</p>
<pre class="brush:sql;">
CREATE TABLE "Users" (
	"Id"	INTEGER NOT NULL,
	"Name"	TEXT,
	...
	CONSTRAINT "CK_Users_Age" CHECK("Age" &gt; 0 AND "Age" &lt; 120)
);
</pre>
<p>Соответственно мы не сможем добавить в бд объект User, у которого значение свойства Age будет меньше 0 или больше 120.</p>
<pre class="brush:c#;">
...
<p>В качестве третьего параметра в <code>HasCheckConstraint</code>, передается делегат Action, который принимает объект CheckConstraintBuilder 
для настройки ограничия. В частности, мы можем задать имя ограничения:</p>
<pre class="brush:c#;">
protected override void OnModelCreating(ModelBuilder modelBuilder)
{
	modelBuilder.Entity&lt;User&gt;()
        .HasCheckConstraint("Age", "Age &gt; 0 AND Age &lt; 120", c =&gt; c.HasName("CK_User_Age"));
}
</pre>
<h3>Ограничения по длине</h3>
<p>Как правило, по умолчанию для строкового свойства в таблице создается столбец для хранения строки неограниченной длины. 
Используя аннотации данных или Fluent API, мы можем ограничить строку по длине.</p>
<p>Ограничение максимальной длины применяется только к строкам и к массивам, например, <code>byte[]</code>.</p>
<h4>Атрибут MaxLength</h4>
<p>В аннотациях данных ограничение по длине устанавливается с помощью атрибута <span class="b">MaxLength</span>:</p>
<pre class="brush:c#;">
...
<p>Стоит отметить, что данное ограничение будет действовать только для тех систем баз данных, которые поддерживают данную возможность. 
Например, для бд SQLite это не будет иметь никакого значения. А в случае с бд MS SQL Server столбец Name в базе данных будет иметь тип <code>nvarchar(50)</code> и тем самым иметь ограничение по длине.</p>
<p>Надо отметить, что также есть атрибут <span class="b">[MinLength]</span>, который устанавливает минимальную длину, но он на определение таблицы не влияет.</p>
<h4>Метод HasMaxLength</h4>
<p>В Fluent API ограничение по длине устанавливается с помощью метода <span class="b">HasMaxLength()</span>:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<div class="nav"><p><a href="./2.8.php">Назад</a><a href="./">Содержание</a><a href="./2.13.php">Вперед</a></p></div></div>
<div class="socBlock">
