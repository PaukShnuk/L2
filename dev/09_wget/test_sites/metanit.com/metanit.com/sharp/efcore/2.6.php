...
<html >
<head>
<title>Entity Framework Core и C# | Сопоставление таблиц и столбцов</title>
<meta charset="utf-8" />
<meta name="description" content="Сопоставление моделей с таблицами и свойств классов со столбцами в Entity Framework Core и языке программирования C#, атрибуты Table и Column, применение методов ToTable и HasColumnName">
...
<h2>Сопоставление таблиц и столбцов</h2><div class="date">Последнее обновление: 18.11.2021</div>
...
<h3>Сопоставление таблиц</h3>
<p>Каждая сущность по умолчанию сопоставляется с таблицей, которая называется по имени свойства <span class="b">DbSet&lt;T&gt;</span> в контексте данных, представляющего данную сущность. 
Если в контексте данных подобного свойства не определено, то для названия таблицы используется имя класса сущности.</p>
<h4>Атрибут Table</h4>
<p>Атрибут Table позволяет переопределить сопоставление с таблицей по имени:</p>
<pre class="brush:c#;">
...
<p>Теперь сущность User будет сопоставляться с таблицей "People".</p>
<h4>Метод ToTable</h4>
<p>Аналогичное переопределение можно произвести через Fluent API с помощью метода <span class="b">ToTable()</span>:</p>
<pre class="brush:c#;">
...
<p>С помощью дополнительного параметра <span class="b">schema</span> можно определить схему, к которой будет принадлежать таблица:</p>
<pre class="brush:c#;">
modelBuilder.Entity&lt;User&gt;().ToTable("People", schema: "userstore");
</pre>
<h3>Сопоставление столбцов</h3>
<p>По умолчанию каждое свойство сопоставляется с одноименным столбцом.</p>
<h4>Атрибут Column</h4>
<p>Атрибут <span class="b">Column</span> переопределяет сопоставление:</p>
<pre class="brush:c#;">
public class User
{
	[Column("user_id")]
...
<p>Теперь свойство Id будет сопоставляться со столбцом "user_id".</p>
<h4>Метод HasColumnName</h4>
<p>Также сопоставление можно переопределить в Fluent API с помощью метода <span class="b">HasColumnName</span>:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<div class="nav"><p><a href="./2.5.php">Назад</a><a href="./">Содержание</a><a href="./2.15.php">Вперед</a></p></div></div>
<div class="socBlock">
