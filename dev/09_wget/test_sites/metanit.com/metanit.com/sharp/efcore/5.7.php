...
<html >
<head>
<title>Entity Framework Core и C# | Отслеживание объектов и AsNoTracking</title>
<meta charset="utf-8" />
<meta name="description" content="Отслеживание объектов в контексте в Entity Framework Core и C#, метод AsNoTracking, свойство ChangeTracker">
...
<h2>Отслеживание объектов и AsNoTracking</h2><div class="date">Последнее обновление: 21.11.2021</div>
...
<p>Запросы могут быть отслеживаемыми и неотслеживаемыми. По умолчанию все запросы, которые возвращают объекты классов моделей являются 
отслеживаемыми. Когда контекст данных извлекает данные из базы данных, Entity Framework Core помещает извлеченные 
объекты в кэш и отслеживает изменения, которые происходят с этими объектами вплоть до использования метода 
<span class="b">SaveChanges()/SaveChangesAsync()</span>, который фиксирует все изменения в базе данных. Но нам не всегда необходимо отслеживать изменения. 
Например, нам надо просто вывести данные для просмотра.</p>
<p>Чтобы данные не помещались в кэш, применяется метод <span class="b">AsNoTracking()</span>. Этот метод 
применяется к объекту <span class="b">IQueryable</span>. При его применении 
возвращаемые из запроса данные не кэшируются. То есть запрос является неотслеживаемым. А это означает, что Entity 
Framework Core не производит какую-то дополнительную обработку и не выделяет дополнительное место для хранения 
извлеченных из БД объектов. И поэтому такие запросы работают быстрее.</p>
<p>Небольшой пример. Допустим, у нас есть следующие модели и контекст данных:</p>
<pre class="brush:c#;">
...
<p>Пусть в базе данных есть есть несколько объектов User:</p>
<pre class="brush:c#;">
...
<p>Мы видим, что в наборе users первый элемент имеет у свойства Age значение 18.</p>
<p>Причем в данном случае мы можем и не вызывать метод <span class="b">SaveChanges</span>, элемент уже и так будет закэширован. Метод <span class="b">SaveChanges</span> 
необходим, чтобы применить все изменения с объектами в базе данных.</p>
<p>Но если бы мы использовали метод <span class="b">AsNoTracking()</span>, то результат был бы другой:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
...
<p>Консольный вывод будет тот же, что и в предыдущем случае:</p>
<div class="console">
<pre class="consoletext">
Tom (18)
Bob (39)
...
<p>Так как при получении первого элемента используется AsNoTracking, он не будет отслеживаться, и поэтому вызов 
db.SaveChanges() никак не повлияет на базу данных, а первый элемент сохранит свое первоначальное значение у свойства Age.</p>
<h3>Свойство ChangeTracker</h3>
<p>Кроме использования метода AsNoTracking, можно отключить отслеживание в целом для объекта контекста. Для этого надо установить 
значение <span class="b">QueryTrackingBehavior.NoTracking</span> для свойства <span class="b">db.ChangeTracker.QueryTrackingBehavior</span>:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
...
<p>Также можно отключить отслеживание на уровне всего контекста данных, например, в его конструкторе</p>
<pre class="brush:c#;">
...
<p>Вобще через свойство <span class="b">ChangeTracker</span> мы можем управлять отслеживанием объектом и получать разнообразную информацию. 
Например, мы можем узнать, сколько объектов отслеживается в текущий момент:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
	var users = db.Users.ToList();
