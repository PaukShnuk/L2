...
<html >
<head>
<title>Entity Framework Core и C# | Загрузка связанных данных. Метод Include</title>
<meta charset="utf-8" />
...
<h2>Загрузка связанных данных. Метод Include</h2><div class="date">Последнее обновление: 20.11.2021</div>
...
<p>Для загрузки связанных данных используется метод <span class="b">Include</span>:</p>
<pre class="brush:c#;">
...
<h3>Загрузка сущностей со сложной многоуровневой структурой</h3>
...
<p>И пусть есть следующий контекст данных</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
надо использовать метод <span class="b">ThenInclude()</span>, через который затем подгружаются страны компаний.</p>
<p>При загрузке связанных данных EF Core гарантирует, что если связанная сущность не установлена (например, свойство CompanyId в объекте User равно null) 
то данное навигационное свойство просто будет игнорироваться. Соответственно никакой ошибки в процессе получения данных не произойдет. 
Но поскольку компилятор не знает об этом, то он выдает предупреждение, например, в следующем случае:</p>
<pre class="brush:c#;">.ThenInclude(c =&gt; c.Country)    // 'c' may be null here </pre>
<p>В этом случае мы можем использовать оператор <span class="b">!</span> (null-forgiving оператор), чтобы указать, что значение null в данной ситуации невоможно.</p>
<pre class="brush:c#;">.ThenInclude(c =&gt; c!.Country)    // норм, компилятор доволен </pre>
<p>В итоге на уровне базы данных это выльется в следующий код SQL:</p>
<pre class="brush:sql;">
...
<h4>Include</h4>
<p>Также мы можем использовать тот же метод <span class="b">Include</span> для загрузки данных далее по цепочке:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
    var users = db.Users
...
<p>В этом случае будет формироваться такой же sql-запрос, и мы получим аналогичный результат.</p>
<h3>Многоуровневая система данных</h3>
<p>И в конце рассмотрим более сложную многоуровневую структуру сущностей:</p>
<pre class="brush:c#;">
// столица страны
public class City
{
...
которая хранит ссылку на столицу в виде объекта City.</p>
<p>Для взаимодействия с бд определим следующий контекст данных:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>Теперь добавим начальные и данные и загрузим пользователей с детальными данными:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
