...
<html >
<head>
<title>Entity Framework Core и C# | Управление базой данных</title>
<meta charset="utf-8" />
<meta name="description" content="Управление базой данных в Entity Framework Core и C#, создание и удаление базы данных с помощью Database.EnsureCreated и Database.EnsureDeleted, проверка подключения с помощью CanConnect">
...
<h2>Управление базой данных</h2><div class="date">Последнее обновление: 23.11.2021</div>
...
<p>Для управления базой данной в классе <span class="b">DbContext</span> определено свойство <span class="b">Database</span>, 
которое представляет тип <span class="b">Microsoft.EntityFrameworkCore.Infrastructure.DatabaseFacade</span> и которое предоставляет некоторый функционал для управления базой данных.</p>
<h3>Database.EnsureCreated</h3>
<p>Метод <span class="b">Database.EnsureCreated()</span> и его асинхронная версия <span class="b">Database.EnsureCreatedAsync()</span> 
гарантируют, что база данных будет создана.</p>
<p>Если база данных отсутствует, то данный метод создает ее.</p>
<p>Если база данных имеется, но она НЕ имеет таблиц, то этот метод создает таблицы, которые соответствуют схеме данных.</p>
<p>Если база данных имеется и она имеет таблицы, то этот метод не оказывает никакого влияния.</p>
<p>Данный метод можно вызвать в коде контекста данных, например, в конструкторе, чтобы гарантировать, что при вызове операций с данными БД уже создана:</p>
<pre class="brush:c#;">
...
<p>Либо можно вызывать вне класса контекста данных там, где нам доступен его объект:</p>
<pre class="brush:c#;">
...
<p>Метод возвращает <span class="b">true</span>, если бд создана в результате вызова этого метода. Если база данных уже есть, то возвращается <span class="b">false</span>.</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
...
<h3>Database.EnsureDeleted</h3>
<p>Метод <span class="b">Database.EnsureDeleted()</span> и его асинхронная версия <span class="b">Database.EnsureDeletedAsync()</span> 
гарантируют, что база данных будет удалена.</p>
<p>Если база данных имеется, то она удаляется. Если база данных отсутствует, то этот метод не оказывает никакого влияния.</p>
<p>Данный метод можно вызвать в коде контекста данных, так и вне, где доступен объект контекста данных. 
Обычно данных метод применяется в паре с <code>Database.EnsureCreated</code> для пересоздания БД, 
например, когда нм надо иметь чистую бд без каких-либо данных для прототипироания и тестирования, например:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>Либо вне класса контекста данных:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
...
<p>Метод возвращает <span class="b">true</span>, если бд была удалена в результате вызова этого метода. Если база уже до вызова метода отсутствует, то возвращается <span class="b">false</span>.</p>

<h3>Database.CanConnect</h3>
<p>Еще один метод, который стоит отметить, это <span class="b">Database.CanConnect()</span> и его асинхронная версия <span class="b">Database.CanConnectAsync()</span>. 
Данный метод возвращает <span class="b">true</span>, если бд доступна, и <span class="b">false</span>, если бд не доступна:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
