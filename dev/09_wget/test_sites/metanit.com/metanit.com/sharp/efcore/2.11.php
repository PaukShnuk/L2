...
<html >
<head>
<title>Entity Framework Core и C# | Параллелизм</title>
<meta charset="utf-8" />
<meta name="description" content="Проблема параллельного доступа в Entity Framework Core и C#, атрибут ConcurrencyCheck и Timestamp, исключение DbUpdateConcurrencyException, методы IsConcurrencyToken и IsRowVersion">
...
<h1>Дополнительные статьи</h1><h2>Параллелизм</h2><div class="date">Последнее обновление: 22.11.2021</div>
...
<p>В ситуации, когда множество пользователей одновременно имеют доступ к одинаковому набору данных и могут эти данные изменять, 
мы можем столкнуться с проблемой параллелизма. Например, два пользователя независимо друг от друга начнут редактировать один и тот же объект. 
И после сохранения объекта первым пользователем второй пользователь уже будет работать с неактуальными данными.</p>
<p>Для решения проблемы параллелизма Entity Framework Core предлагает использовать токены параллелизма или <span class="b">concurrency token</span>.</p>
<p>Если свойство установлено в качестве concurrency token (токен параллелизма), Entity Framework перед сохранением изменений будет проверять его значение. 
Если значение отличается, то какой-то другой пользователь уже произвел изменения над данными. В этом случае EF Core применяет принцип оптимистичного параллелизма 
(optimistic concurrency), при котором, если сохраняемые данные уже были кем-то изменены, то выбрасывается ошибка.</p>
<h3>Атрибут ConcurrencyCheck</h3>
<p>Атрибут <span class="b">ConcurrencyCheck</span> позволяет решить проблему параллелизма, когда с одной и той же записью в таблице могут работать 
одновременно несколько пользователей. Например:</p>
<pre class="brush:c#;">
...
<p>В обычном режиме Entity Framework Core при обновлении смотрит на Id и если Id записи в таблице совпадает с Id в передаваемой модели User, то строка 
в таблице обновляется. При использовании атрибута ConcurrencyCheck EF смотрит не только на Id, но и на исходное значение свойства Name. 
И если оно совпадает с тем, что имеется в таблице, то запись обновляется. Если же не совпадает (то есть кто-то уже успел обновить), 
то EF генерирует исключение <span class="b">DbUpdateConcurrencyException</span>:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<h3>Метод IsConcurrencyToken</h3>
<p>В Fluent API токен параллелизма настраивается с помощью метода <span class="b">IsConcurrencyToken()</code>:</p>
<pre class="brush:c#;">
using System.ComponentModel.DataAnnotations;
using Microsoft.EntityFrameworkCore;
...
<h3>Атрибут Timestamp</h3>
<p>Другой механизм по отслеживанию изменений объекта в БД представляет атрибут <span class="b">Timestamp</span>.</p>
<pre class="brush:c#;">
using System.ComponentModel.DataAnnotations;
using Microsoft.EntityFrameworkCore;
...
<p>Атрибут Timestamp указывает, что значение свойства Timestamp будет включаться в создаваемое Entity Frameworkом SQL-выражение WHERE при отправке в 
базу данных команд на обновление и удаление. То есть значение данного свойства генерируется при добавлении объекта, а также каждый раз, когда пользователь будет обновлять его. 
В качестве типа для свойства используется массив байтов.</p>
<p>И если два пользователя одновременно начнут редактировать одну и ту же строку, то после сохранения модели первым пользователем, 
второй пользователь получит исключение DbUpdateConcurrencyException, которое соответственно надо обработать.</p>
<p>Стоит отметить, что реализация этой функциональности зависит от провайдера БД. Так, для SQLite это не окажет никакого влияния, поэтому в примере выше 
подключение идет к MS SQL Server.</p>
<h3>Fluent API и метод IsRowVersion</h3>
<p>Вместо атрибута Timestamp можно во Fluent API использовать метод <span class="b">IsRowVersion</span>, который установит, что столбец представляет версию строки. 
Но опять же работа данной функциональности зависит от конкретного провайдера бд, например, для MS SQL Server эта функциональность работает:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<div class="nav"><p><a href="./6.3.php">Назад</a><a href="./">Содержание</a><a href="./2.12.php">Вперед</a></p></div></div>
<div class="socBlock">
