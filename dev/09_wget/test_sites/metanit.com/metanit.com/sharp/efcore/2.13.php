...
<html >
<head>
<title>Entity Framework Core и C# | Конфигурация моделей</title>
<meta charset="utf-8" />
<meta name="description" content="Конфигурация моделей в Entity Framework Core, интерфейс IEntityTypeConfiguration, вынесение конфигурации моделей во внешние классы">
...
<h2>Конфигурация моделей</h2><div class="date">Последнее обновление: 24.11.2021</div>
...
<p>С помощью атрибутов и Fluent API для сущостей и их свойств можно установить многочисленные настройки. 
Однако, если настроек очень много, то они могут утяжелять класс контекста и сущностей. В этом случае Entity Framework Core позволяет вынести конфигурацию 
сущностей в отдельные классы.</p>
<p>Для вынесения конфигурации во вне необходимо создать класс конфигурации, реализующий интерфейс <span class="b">EntityTypeConfiguration&lt;T&gt;</span>.</p>
<p>К примеру, пусть у нас есть следующий класс контекста и моделей:</p>
<pre class="brush:c#;">
...
<p>Вся конфигурация здесь определена в методе <span class="b">OnModelCreating()</span>. В принципе он не содержит много кода, однако при наличии 
гораздо большего количества сущностей и более изощренной их конфигурации с помощью Fluent API данный метод мог бы сильно раздуться в размерах. 
И теперь изменим определение контекста, применив конфигурации:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>Теперь конфигурация моделей вынесена в отдельные классы. А для добавления конкретных конфигураций в контекст используется метод 
<span class="b">modelBuilder.ApplyConfiguration()</span>, которому передается нужный объект конфигурации. В итоге по своему действию первый и 
второй варианты контекста будут идентичны.</p>
<p>В качестве альтернативы мы могли бы использовать еще один вариант. Вместо выделения отдельных классов конфигураций определить конфигурацию в виде 
отдельных методов в том же классе контекста данных:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata.Builders;
...
<p>Здесь конфигурация определяется для каждого типа в отдельном методе, который в качестве параметра принимает объект <code>EntityTypeBuilder&lt;T&gt;</code>. 
Затем метод передается в вызов <code>modelBuilder.Entity&lt;T&gt;()</code> для соответствующей модели.</p>
<h3>Атрибут EntityTypeConfiguration</h3>
<p>Еще один альтернативный вариант применения конфигураций представляет атрибут <span class="b">EntityTypeConfiguration</span>, который применяется к сущности и который получает 
тип класса конфигурации:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata.Builders;
...
<div class="nav"><p><a href="./2.9.php">Назад</a><a href="./">Содержание</a><a href="./2.14.php">Вперед</a></p></div></div>
<div class="socBlock">
