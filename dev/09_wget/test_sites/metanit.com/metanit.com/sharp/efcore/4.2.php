...
<html >
<head>
<title>Entity Framework Core и C# | Наследование. Подход TPT - Table Per Type</title>
<meta charset="utf-8" />
<meta name="description" content="Наследование в Entity Framework Core и C#, подход Table Per Type, таблица для каждого производного класса, организация базы данных">
...
<h2>Подход TPT - Table Per Type</h2><div class="date">Последнее обновление: 18.11.2020</div>
...
<p>В прошлой статье был рассмотрен подход TPH - одна таблица для всей иерархии наследования классов. Но также EntityFramework Core 
позволяет использовать другой подход - TPT или Table Per Type, который предполагает создание отдельной таблицы для каждого класса из иерархии. 
Для реализации  подхода TPT можно использовать два способа: атрибуты или Fluent API.</p>
<h3>Применение TPT на основе атрибутов</h3>
<p>С помощью атрибута <span class="b">[Table]</span> мы можем указать для каждого класса свою таблицу:</p>
<pre class="brush:c#;">
...
<p>В этом случае в бд будут создаваться следующие три таблицы:</p>
<pre class="brush:sql;">
CREATE TABLE "Users" (
	"Id"	INTEGER NOT NULL,
	"Name"	TEXT,
...
<p>Здесь мы видим, что все свойства базового класса User будут храниться в одной таблице, а те данные, которые относятся только к производным классам, хранятся в отдельных таблицах.</p>
...
<h3>Применение TPT на основе Fluent API</h3>
<p>Также мы можем настроить TPT с помощью метода <span class="b">ToTable()</span> во Fluent API:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>В остальном применение классов будет аналогично примеру с атрибутом <code>[Table]</code>.</p>
...
<div class="nav"><p><a href="4.1.php">Назад</a><a href="./">Содержание</a><a href="5.1.php">Вперед</a></p></div></div>
<div class="socBlock">
