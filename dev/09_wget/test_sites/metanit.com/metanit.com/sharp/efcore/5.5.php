...
<html >
<head>
<title>Entity Framework Core и C# | Агрегатные операции</title>
<meta charset="utf-8" />
<meta name="description" content="Агрегатные операции в LINQ to Entities в Entity Framework Core и C#, методы All, Any, Count, Sum, Min, Max, Average">
...
<h2>Агрегатные операции</h2><div class="date">Последнее обновление: 21.11.2021</div>
...
<p>Entity Framework Core поддерживает ряд агрегатных операций, например, нахождение суммы по столбцу, количества элементов в выборке и т.д. 
Эти операции представлены такими методами как <span class="b">All, Any, Count, Sum, Min, Max, Average</span>. Для рассмотрения агрегатных операций возьмем модели из прошлой темы:</p>
<pre class="brush:c#;">
...
<h3>Наличие элементов</h3>
<p>Метод <span class="b">Any()</span> позволяет проверить, есть ли в базе данных элемент с определенными признаками, и если есть, то метод возвратит значение 
true. Например, проверим, есть ли в базе данных пользователи, которые работают в компании Google:</p>
<pre class="brush:c#;">bool result = db.Users.Any(u=&gt;u.Company!.Name=="Google");</pre>
<p>При выполнении это выражение преобразуется в следующий SQL-запрос:</p>
<pre class="brush:sql;">
SELECT EXISTS (
	SELECT 1
    FROM "Users" AS "u"
...
<p>Метод <span class="b">All()</span> позволяет проверит, удовлетворяют ли все элементы в базе данных определенному критерию. Например, проверим, 
все ли пользователи работают в компании Microsoft:</p>
<pre class="brush:c#;">bool result = db.Users.All(u=&gt;u.Company!.Name=="Microsoft");</pre>
<p>При вызове будет выполняться следующий SQL-запрос:</p>
<pre class="brush:sql;">
SELECT NOT EXISTS (
	SELECT 1
	FROM "Users" AS "u"
...
<h3>Количество элементов в выборке</h3>
<p>Метод <code>Count()</code> позволяет найти количество элементов в выборке:</p>
<pre class="brush:c#;">
...
<p>В результате будут выполняться варажения SQL наподобие:</p>
<pre class="brush:sql;">
SELECT COUNT(*)
FROM "Users" AS "u"
...
<h3>Минимальное, максимальное  и среднее значения</h3>
<p>Для нахождения минимального, максимального и среднего значений по выборке применяются функции <span class="b">Min()</span>, <span class="b">Max()</span> 
и <span class="b">Average()</span> соответственно. Найдем минимальный, максимальный и средний возраст пользователей:</p>
<pre class="brush:c#;">
using(ApplicationContext db = new ApplicationContext())
{
    // минимальный возраст
...
<p>Эти запросы трансформируются в SQLite в следующие SQL-выражения:</p>
<pre class="brush:sql;">
SELECT MIN("u"."Age")
FROM "Users" AS "u"
...
<p>Для получения суммы значений используется метод <span class="b">Sum()</span>:</p>
<pre class="brush:c#;">
using(ApplicationContext db = new ApplicationContext())
{
...
<p>Во втором случае будет выполняться следующее SQL-выражение:</p>
<pre class="brush:sql;">
...
<div class="nav"><p><a href="./5.4.php">Назад</a><a href="./">Содержание</a><a href="./5.7.php">Вперед</a></p></div></div>
<div class="socBlock">
