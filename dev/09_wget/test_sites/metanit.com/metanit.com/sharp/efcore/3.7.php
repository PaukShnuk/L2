...
<html >
<head>
<title>Entity Framework Core и C# | Комплексные типы</title>
<meta charset="utf-8" />
<meta name="description" content="Комплексные типы в Entity Framework Core и C#, их определение, метод OwnsOne, хранение в базе данных">
...
<h2>Комплексные типы</h2><div class="date">Последнее обновление: 21.11.2021</div>
...
<p>Обычно Entity Framework Core сопоставляет каждый класс с отдельной таблицей. Однако бывают случаи, когда определенный класс существует не сам по себе, 
а несет некоторую дополнительную информацию по отношению к другой главной модели. Данная функциональность предполагает, что у нас есть несколько типов, но один из них является владельцем (owner). Другие же 
зависимые или собственные типы (owned entity types) являются частью типа-владельца и без него существовать не могут.</p>
<p>Для создания связки "главный тип - зависимые типы" можно использовать либо атрибут <span class="b">OwnedAttribute</span>, либо настройки Fluent API.</p>
<h3>OwnedAttribute</h3>
<p>Атрибут <span class="b">OwnedAttribute</span> позволяет установить зависимый тип. Например:</p>
<pre class="brush:c#;">
...
<p>В данном случае есть тип-владелец User и есть владеемый им тип UserProfile. Класс User определяет основные настройки учетной записи - логин и пароль, а UserProfile - 
всторостепенные данные - имя, возраст и т.д. Чтобы указать, что User владеет типом UserProfile, над UserProfile указывается атрибут OwnedAttribute. 
Кроме того, UserProfile не содержит ключа.</p>
<p>В итоге при создании базы данных данные обоих типов будут содержаться в одной таблице:</p>
<pre class="brush:sql;">
CREATE TABLE "Users" (
	"Id"	INTEGER NOT NULL,
	"Login"	TEXT,
...
<h3>Fluent API</h3>
<p>Также для настройки связи можно использовать Fluent API, в частности, метод <span class="b">OwnsOne()</span>:</p>
<pre class="brush:c#;">
...
<p>В методе <code>OwnsOne()</code> указывается навигационное свойство, которое представляет зависимый тип.</p>
<h3>Приватные типы</h3>
<p>Вполне возможно, что мы захотим сделать навигационное свойство Profile приватным. В этом случае понадобится дополнительная настройка:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>В этом случае в метод <code>OwnsOne</code> передается тип навигационного свойства и само имя навигационного свойства. Применение:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
...
<h3>Вложенные собственные типы</h3>
<p>Одни собственные типы могут иметь другие собственные типы, увеличивая сложность структуры классов. Например, определим следующие классы:</p>
<pre class="brush:c#;">
using System.ComponentModel.DataAnnotations;
...
<p>Главной сущностью здесь является класс User, который содержит объект класса UserProfile. Класс UserProfile, в свою очередь, также содержит 
объекты еще одного класса - Claim, который представляет отдельные данные по принципу ключ-значение. Обратите внимание, что свойство Profile является <span class="b">обязательным</span>.</p>
<p>Теперь определим контекст данных:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>С помощью метода <span class="b">OwnsOne()</span> устанавливается включение через свойства собственных типов. И после создания базы данных 
мы получим следующую таблицу User, которая будет содержать данные, представленные собственными типами.</p>
<pre class="brush:sql;">
CREATE TABLE "Users" (
	"Id"	INTEGER NOT NULL,
	"Login"	TEXT,
...
<p>Для работы с собственными типами определим следующий код:</p>
<pre class="brush:c#;">
using (ApplicationContext db = new ApplicationContext())
{
	User user1 = new User
...
<div class="nav"><p><a href="./3.6.php">Назад</a><a href="./">Содержание</a><a href="./3.11.php">Вперед</a></p></div></div>
<div class="socBlock">
