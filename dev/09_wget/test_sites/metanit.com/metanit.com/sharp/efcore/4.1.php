...
<html >
<head>
<title>Entity Framework Core и C# | Наследование. TPH - Table Per Hierarchy</title>
<meta charset="utf-8" />
<meta name="description" content="Наследование в Entity Framework Core и C#, подход Table Per Hierarchy, одна таблица для всей иерархии классов, столбец Discriminator">
...
<h1>Наследование</h1><h2>Подход TPH - Table Per Hierarchy</h2><div class="date">Последнее обновление: 21.11.2021</div>
...
<p>По умолчанию при работе с цепочками наследования классов Entity Framework Core использует подход TPH (Table Per Hierarchy / Таблица на одну иерархию классов).
При использовании данного подхода TPH для всех классов из одной иерархии в базе данных создается одна таблица. А чтобы определить, к какому именно классу относится 
строка в таблице, в этой же таблице создается дополнительный столбец - дискриминатор.</p>
<p>Например, у нас есть следующая иерархия классов:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Есть базовый класс User, который представляет пользователя и от которого наследуются класс Employee - класс работника и Manager - класс управляющего.</p>
<p>Определим контекст данных:</p>
<pre class="brush:c#;">
...
<p>Чтобы включить все классы из иерархии наследования в базу данных, в контексте данных для каждого типа должен быть определен набор DbSet.</p>
<p>Сгенерированная база данных будет содержать для всех типов одну таблицу Users. 
Кроме всех свойств классов User, Employee и Manager здесь также появляется еще один столбец - Discriminator. Он имеет тип nvarchar (то есть строка), 
а в качестве значения он принимает название класса, к которому относится строка в таблице. В итоге в бд будет создаваться следующая таблица:</p>
<pre class="brush:sql;">
CREATE TABLE "Users" (
	"Id"	INTEGER NOT NULL,
	"Name"	TEXT,
...
<p>И в итоге данные таблицы Users в бд будут выглядеть следующим образом:</p>
<img src="./pics/4.2.png" alt="TPH in Entity Framework Core и C#" />
<p>При необходимости мы даже можем добавить в класс User свойство Discriminator:</p>
<pre class="brush:c#;">
public class User
{
    public int Id { get; set; }
...
<p>Однако надо учитывать, что на уровне базы данных соответствующий столбец установлен как readonly, то есть только для чтения. Поэтому мы сможем только получать его значения, 
но не изменять.</p>
...
<div class="nav"><p><a href="./3.7.php">Назад</a><a href="./">Содержание</a><a href="./4.2.php">Вперед</a></p></div></div>
<div class="socBlock">
