...
<html >
<head>
<title>Создание первого приложения (Silverlight)</title>
<meta charset="utf-8" />
<meta name="description" content="Создание первого приложения на Silverlight 5 - онлайн-калькулятора">
...
<p>Наш первый проект не просто так назван SilverlightCalculator, поэтому создадим калькулятор, который будет более наглядным примером, чем HelloWorld. 
Тем более, что HelloWorld - это не приложение, это всего лишь HelloWorld.</p>
<p>Логика нашего калькулятора будет очень проста. У нас будут три глобальных переменные: две для левого и правого операндов, и одна для 
знака операции. При нажатии на число, оно будет добавляться как строка справа к текущему операнду. При нажатии на кнопку со знаком операции 
у нас будут производиться вычисления. Также нам понадобится кнопка CLEAR для очиcтки все переменных и текстового поля. В итоге калькулятор будет выглядеть следующим образом:</p>
<img src="silpics/1.5.jpg" />
<p>Итак, в разметке XAML у нас следующий код:</p>
<pre class="brush:xml;">
...
<p>А файл кода должен выглядеть следующим образом:</p>
<pre class="brush:c#;">
using System;
...
<p>Запускаем приложение на выполнение и получаем примитивный калькулятор.</p>
...
<div class="nav"><p><a href="1.1.php">Назад</a><a href="./">Содержание</a><a href="2.php">Вперед</a></p></div></div>
<div class="socBlock">
