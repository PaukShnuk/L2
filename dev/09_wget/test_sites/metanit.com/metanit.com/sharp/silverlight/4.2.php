...
<html >
<head>
<title>Специальные контейнеры</title>
<meta charset="utf-8" />
<meta name="description" content="Использование в Silverlight контейнеров ScrollViewer, ViewBox и Border, их свойства и примеры использования">
...
<h2>Специальные контейнеры</h2><div class="date">Последнее обновление: 1.11.2015</div>
...
<h3>ScrollViewer</h3>
<p>Этот элемент обеспечивает прокрутку содержимого. Он может вмещать в себя только один элемент, поэтому все элементы, помещаемые внутрь ScrollViewer необходимо облачить в еще один контейнер. 
Поддерживает как вертикальную, так и горизонтальную прокрутку. Ее можно установить с помощью свойств <span class="bb">HorizontalScrollBarVisibility</span> и 
<span class="bb">VerticalScrollBarVisibility</span>. Эти свойства принимают одно из следующих значений:</p>
<ul>
<li><p><span class="bb">Auto</span> - наличие полос прокрутки устанавливается автоматически.</p></li>
<li><p><span class="bb">Visible</span> - полосы прокрутки отображаются в окне приложения.</p></li>
<li><p><span class="bb">Hidden</span> - полосы прокрутки не видно, но прокрутка возможна с помощью клавиш клавиатуры.</p></li>
<li><p><span class="bb">Disabled</span> - полосы прокрутки не используются, а сама прокрутка даже с помощью клавиатуры невозможна.</p></li>
</ul>
<p>В качестве примера обернем предыдущий код в элемент ScrollViewer:</p>
<pre class="brush:xml;">
...
<img src="silpics/4.7.jpg" />
<h3>ViewBox</h3>
<p>Элемент ViewBox автоматически масштабирует размеры своего содержимого под свои размеры. Среди свойств ViewBox следует отметить свойство <span class="bb">Stretch</span>, 
которое позволяет растягивать содержимое и может принимать одно из следующих значений: <span class="bb">None</span>, <span class="bb">Uniform</span> (по умолчанию), <span class="bb">Fill</span> и <span class="bb">UniformToFill</span>.
 Например:</p>
<pre class="brush:xml;">&lt;Viewbox Stretch="Fill"&gt;
            &lt;Grid&gt;
                &lt;Button Width="60" Height="30" Margin="10"&gt;Fill&lt;/Button&gt;
                &lt;Ellipse Width="58" Height="30" Fill="AliceBlue" Opacity="0.5" /&gt;
            &lt;/Grid&gt;
   &lt;/Viewbox&gt;</pre>
<img src="silpics/4.8.jpg" />
<h3>Border</h3>
<p>Ряд элементов управления не имеют рамки или возможности установить границы. Для этого предназначен элемент Border. 
Этот элемент скорее можно назвать декоратором, чем контейнером. Он создает рамку вокруг вложенного содержимого. 
Его свойство <strong>CornerRadius</strong> позволяет создавать рамки с закругленными краями.</p>
<pre class="brush:xml;">&lt;Border BorderBrush="Black" BorderThickness="2" VerticalAlignment="Center" CornerRadius="10"&gt;
            &lt;StackPanel&gt;
