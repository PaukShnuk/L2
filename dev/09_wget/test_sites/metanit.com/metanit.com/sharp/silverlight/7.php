...
<html >
<head>
<title>Ресурсы (Silverlight)</title>
<meta charset="utf-8" />
<meta name="description" content="Использование статических и динамических ресурсов в xaml и в коде. Добавление в приложение словарей ресурсов">
...
<h1>Концепция ресурсов в Silverlight</h1><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Здесь под ресурсами подразумеваются не дополнительные файлы, как, например, аудиофайлы, файлы с изображениями, которые добавляются в проект. 
Silverlight. Это свойство включает коллекцию, точнее словарь (Resource dictionary) отдельных ресурсов. Каждый ресурс является каким-нибудь 
объектом, например, элемент управления, кисть, коллекция объектов и т.д. Смысл использования ресурсов в их гибкости, их можно многократно 
использовать в различных частях приложения, динамически менять. Кроме того, некоторые объекты, например, коллеции пользовательских типов 
невозможно в xaml-коде объявить иначе, как с помощью ресурсов.</p>
<p>Поскольку ресурсы содержатся в словаре, каждому присваивается уникальный ключ с помощью свойства <span class="bb">x:Key</span>. По этому ключу можно потом в коде найти ресурс.</p>
<p>Например, объявим кнопку в качестве ресурса и затем используем ее в проекте</p>
<pre class="brush:xml;">
...
<p>В данном случае мы создаем ресурс с ключом <code>x:Key="R2D2Button"</code> и затем присваиваем этот ресурс свойству Background кнопки в проекте.</p>
<img src="silpics/7.1.jpg" />
<p>Ресурсы могут объявляться внутри различных элементов управления - UserControl, Grid, Button. 
Также возможно определение ресурсов для все приложения в файле xaml-разметки приложения.</p>
<p>Общий синтаксис задания ресурса в качестве значения свойства объекта такой: в фигурных скобках после ключевого слова <span class="bb">StaticResource</span> или <span class="bb">DynamicResource</span> идет ключ ресурса:</p>
<p><span class="bb">Свойство="{StaticResource ключ_ресурса}"</span></p>
<p>Мы можем получить объект ресурса, обратившись к коллекции ресурсов и получив из нее по ключу нужный объект:</p>
<pre class="brush: c#;">RadialGradientBrush rdb = (RadialGradientBrush)this.Resources["R2D2Button"];</pre>
<P>В то же время мы не можем изменить ресурс или присвоить ему новый объект, так как, в отличие от WPF, Silverlight не поддерживает 
динамические ресурсы.</P>
<h3>Словари ресурсов</h3>
<p>Кроме прописывания ресурсов на уровне определенного элемента, например, UserControl или Grid, можно воспользоваться словарем ресурсов. 
Для этого надо добавить в проект новый объект под названием Resource Dictionary. Он представляет собой обычный xaml-файл.</p>
<p>Нажмите правой кнопкой на название проекта и выберите в пункт <span class="bb">Add</span>-&gt;<span class="bb">New Item...</span>. Затем среди шаблонов выберите 
<b>Silverlight Resource Dictionary</b>. Выберите имя Dictionary1.xaml и нажмите OK. В проект будет добавлен словарь ресурсов. 
Теперь вынесем ресурс кисти с радиальным градиентом из ресурсов UserControl в наш словарь ресурсов:</p>
<pre class="brush:xml;">
&lt;ResourceDictionary
    xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" 
    xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"&gt;
...
<p>Если наш словарь находится в файле <code>Dictionary1.xaml</code>, то его можно подключить в файле разметки приложения таким способом:</p>
...
<p>Запустите проект и вы увидите, что несмотря на то, что мы удалили ресурсы UserControl из разметки, кнопка все равно применяет ресурс, но теперь 
уже из словаря ресурсов.</p>
...
<div class="nav"><p><a href="6.php">Назад</a><a href="./">Содержание</a><a href="8.php">Вперед</a></p></div></div>
<div class="socBlock">
