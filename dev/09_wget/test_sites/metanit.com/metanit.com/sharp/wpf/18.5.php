...
<html >
<head>
<title>C# и WPF | Камера</title>
<meta charset="utf-8" />
<meta name="description" content="Камера в трехмерных сценах в WPF, ортогональная и перспективная виды камер, настройка положения и углов обзора камеры">
...
<h2>Камера</h2><div class="date">Последнее обновление: 13.03.2016</div>
...
<p>Теперь перейдем к последнему моменту в нашем приложении - к камере. От настроек камеры зависит, как трехмерная сцена будет отображаться на экране. </p>
<p>В WPF имеются три типа типа камер:</p>
<ul>
<li><p><span class="b">PerspectiveCamera</span>. Создает перспективную проекцию трехмерной сцены. В итоге те предметы, которые находятся дальше, выглядят меньше. 
	А те, которые ближе, выглядят больше - в принципе как и в реальной жизни.</p></li>
<li><p><span class="b">OrthographicCamera</span>. Создает ортогональную проекцию сцены, в которой не применяется перспектива. И предметы сохраняют свои размеры.</p></li>
<li><p><span class="b">MatrixCamera</span>. Позволяет задать матрицу преобразования трехмерной сцены в двухмерное содержимое для вывода на экран.</p></li>
</ul>
<img src="./pics/18.8.png" alt="Камера в WPF" />
<p>PerspectiveCamera и OrthographicCamera имеют следующие свойства, которые позволяют настроить камеру:</p>
<ul>
<li><p><span class="b">Position</span>. Задает точку трехмерного пространства, в котором находится камера. Например:</p>
<pre class="brush:xml;">
&lt;Viewport3D&gt;
    &lt;Viewport3D.Camera&gt;
...
<p>Здесь камера устанавливается в точку (0,0,2), то есть фактически находится впереди треугольника на 2 единицы - как-бы между нами и экраном монитора, на котором изображен треугольник.</p></li>
<li><p><span class="b">LookDirection</span>. Определяет вектор направления камеры. Чтобы правильно определить направление камеры, нам надо выбрать точку, на которую 
будет направлена камера. И затем из координат этой точки вычесть координаты в свойстве Position.</p></li>
<img src="./pics/18.9.png" alt="Координаты камеры в WPF" />
	<li><p><span class="b">UpDirection</span>. Задает вектор вертикальной ориентации камеры, точнее ее наклон. В нашем случае данное свойство не было задано, 
	поэтому оно приняло значения по умолчанию - (0,1,0). Данное значение указывает, что камера направлена прямо вверх. Но мы можем наклонить 
	камеру немного по оси Х, указав, например, следующее значение:</p>
<pre class="brush:xml;">&lt;PerspectiveCamera Position="0,0,2" LookDirection="0,0,-2" UpDirection="-0.15,1,0" /&gt;</pre></li>
<img src="./pics/18.10.png" />
<li><p><span class="b">NearPlaneDistance</span>. Это свойство задает переднюю плоскость отсечения. А все та часть сцены, которая находится к камере ближе значения 
	этого свойства, отображаться не будет. По умолчанию имеет значение 0.125</p></li>
<li><p><span class="b">FarPlaneDistance</span>. Это свойство задает заднюю плоскость отсечения. А все та часть сцены, которая находится к камере дальше значения 
	этого свойства, отображаться не будет. По умолчанию имеет значение <code>Double.PositiveInfinity</code></p></li>
</ul>
<p>Кроме этих свойств камеры имеют еще свойство, которое задает ширину обзора или угол обзора. Только для различных камер оно называется по разному. 
Для PerspectiveCamera это свойство <span class="b">FieldOfView</span>. Для OrthographicCamera это свойство <span class="b">Width</span>. Чем больше значение этого свойства, 
тем угол обзора камеры.</p>
...
<div class="nav"><p><a href="18.4.php">Назад</a><a href="./">Содержание</a><a href="18.2.php">Вперед</a></p></div></div>
<div class="socBlock">
