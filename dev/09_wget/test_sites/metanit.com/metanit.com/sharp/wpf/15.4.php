...
<html >
<head>
<title>C# и WPF | Фиксированные документы</title>
<meta charset="utf-8" />
<meta name="description" content="Фиксированные документы в WPF, элемент DocumentViewer и FixedDocument, формат XPS и работа с классом XpsDocument, загрузка и сохранение xps-файлов">
...
<h2>Фиксированные документы</h2><div class="date">Последнее обновление: 29.02.2016</div>
...
<p>Фиксированные документы характеризуются точной неизменной компоновкой и предназначены преимущественно для печати, но могут использоваться также 
и для чтения текста с экрана. К таким документам в WPF относятся документы на основе XPS. Для просмотра XPS-документа используется элемента 
<span class="b">DocumentViewer</span>. В качестве содержимого DocumentViewer принимает элемент <span class="b">FixedDocument</span>, который как раз 
и представляет фиксированный документ.</p>
<p>Например:</p>
<pre class="brush:xml;">
...
<p>Фиксированный документ FixedDocument может принимать различное количество страниц. Каждая страница представляет элемент <span class="b">PageContent</span>. В этот элемент 
помещается объект <span class="b">FixedPage</span>, в который в свою очередь помещаются другие элементы - это могут эллипсы, текстовые поля и т.д. Так, в данном случае у нас 
две страницы. Используя свойства Width и Height, мы можем управлять размером страниц.</p>
<p>Также, здесь мы разместили две кнопки для загрузки и сохранения документа. Теперь в файле кода c# определим для них обработчики событий:</p>
<pre class="brush:c#;">
...
<p>Для сохранения и открытия документа применяется класс <span class="b">XpsDocument</span>. Для его использования нам надо добавить в проект библиотеки 
<span class="b">ReachFramework.dll</span> и <span class="b">System.Printing.dll</span>.</p>
<p>Чтобы сохранить документ, получаем объект <code>XpsDocumentWriter</code> и вызываем его метод <code>Write()</code>.</p>
<p>Для открытия документа просто используем метод <code>GetFixedDocumentSequence()</code> объекта XpsDocument.</p>
<p>И после запуска приложения мы сможем увидеть наш определенный в разметке xaml документ:</p>
<img src="./pics/15.12.png" alt="FixedDocument в WPF" />
<p>Как видно из скриншота, контейнер фиксированных документов DocumentViewer уже имеет некоторую базовую функциональность (например, масштабирование, поиск), которые мы можем использовать.</p>
<p>Для эксперимента можно также попробовать создать простейшие xps-файлы в MS Word (для этого стандартный документ docx можно экспортировать в формат xps) 
и загрузить этот документ в нашу программу.</p>
...
<div class="nav"><p><a href="15.3.php">Назад</a><a href="./">Содержание</a><a href="15.5.php">Вперед</a></p></div></div>
<div class="socBlock">
