...
<html >
<head>
<title>C# и WPF | Трехмерные трансформации и анимации</title>
<meta charset="utf-8" />
<meta name="description" content="Трехмерные трансформации в WPF, применение анимации к трехмерным объектам">
...
<h2>Трехмерные трансформации и анимации</h2><div class="date">Последнее обновление: 13.03.2016</div>
...
<p>В прошлой теме в примере в кубом мы использовали вращение - связывали значение движения ползунка с углом поворота куба. Для трехмерных объектов доступны 
в принципе те же самые трансформации, что и для объектов двухмерных. Мы можем применить трансформацию как на уровне элемента <span class="b">ModelVisual3D</span>, 
так и на уровне элемента <span class="b">GeometryModel3D</span>.</p>
<p>Для вращения применяется трансформация <span class="b">RotateTransform3D</span>:</p>
<pre class="brush:xml;">
...
<p>Свойство <span class="b">Axis</span> задает ось вращения. Поскольку координаты X и Z имеют 0, куб будет вращаться только вокруг оси Y. Но мы можем вращать 
сразу вокруг нескольких осей как бы по-диагонали, установив свойство, например, так: <code>Axis="1 1 0"</code>. А свойство <span class="b">Angle</span> 
определяет угол, на который поворачивается объект. По умолчанию он равен 0.</p>
<p>Для перемещения применяется трансформация <span class="b">TranslateTransform3D</span>:</p>
<pre class="brush:xml;">
&lt;ModelVisual3D.Transform&gt;
	&lt;TranslateTransform3D x:Name="translate"  OffsetZ="0" /&gt;
&lt;/ModelVisual3D.Transform&gt;
</pre>
<p>Код ползунка, устанавливающего перемещение по оси Z, мог бы выглядеть так:</p>
<pre class="brush:xml;">
&lt;Slider Minimum="-10" Maximum="10" Value="{Binding ElementName=translate, Path= OffsetZ}" /&gt;
</pre>
<p>Эта трансформация имеет свойства <span class="b">OffsetX</span>, <span class="b">OffsetY</span> и <span class="b">OffsetZ</span>, устанавливающих перемещение объекта вдоль осей X,Y и Z 
соответственно.</p>
<p>Для масштабирования применяется трансформация <span class="bb">ScaleTransform3D</span>:</p>
<pre class="brush:xml;">
&lt;ModelVisual3D.Transform&gt;
	&lt;ScaleTransform3D x:Name="scale" ScaleX="0.8" /&gt;
&lt;/ModelVisual3D.Transform&gt;
</pre>
<p>Эта трансформация имеет свойства <span class="bb">ScaleX</span>, <span class="bb">ScaleY</span> и <span class="bb">ScaleZ</span>, позволяя масштабировать объект вдоль осей X,Y и Z 
соответственно.</p>
<p>Также имеется трансформация <span class="bb">MatrixTransform3D</span>, которая с помощью свойства <span class="bb">Matrix</span> позволяет задать трансформацию объекта.</p>
<p>Кроме того, мы можем комбинировать несколько трансформаций вместе, поместив их в объект <span class="bb">Transform3DGroup</span>.</p>
<pre class="brush:xml;">
&lt;ModelVisual3D.Transform&gt;
	&lt;Transform3DGroup&gt;
		&lt;RotateTransform3D&gt;
...
<h3>Анимация трехмерных объектов</h3>
<p>WPF предоставляет нам специальные классы для анимации в трехмерной сцене. Такие анимации могут быть полезны, например, если мы хотим вращать камеру вокруг 
3D-объекта. В частности, WPF предоствляет специальные классы анимаций по ключевым кадрам, которые могут применяться к трехмерным структурам: 
<span class="b">Point3DAnimationUsingKeyFrames</span> и <span class="b">Vector3DAnimationUsingKeyFrames</span>:</p>
<pre class="brush:xml;">
...
<p>В данном случае при загрузке окна будут срабатываеть определенные в нем триггеры, которые будут запускать анимацию. Здесь определено две анимации по ключевым 
кадрам: Point3DAnimationUsingKeyFrames - для анимации свойства Position объекта камеры и Vector3DAnimationUsingKeyFrames - для анимации вектора LookDirection.</p>
<p>В итоге при запуске окна мы получим перемещение камеры относительно трехмерного объекта.</p>
...
<div class="nav"><p><a href="18.2.php">Назад</a><a href="./">Содержание</a><a href="18.7.php">Вперед</a></p></div></div>
<div class="socBlock">
