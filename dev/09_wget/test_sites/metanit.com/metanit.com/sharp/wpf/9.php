...
<html >
<head>
<title>C# и WPF | Ресурсы</title>
<meta charset="utf-8" />
<meta name="description" content="Использование ресурсов в WPF, их преимущества, примеры, работа с ресурсами в коде XAML и C#">
...
<h1>Ресурсы</h1><h2>Концепция ресурсов в WPF</h2><div class="date">Последнее обновление: 27.01.2016</div>
...
<p>В WPF важное место занимают <span class="b">ресурсы</span>. В данном случае под ресурсами подразумеваются не дополнительные файлы (или 
<span class="b">физические ресурсы</span>), как, например, аудиофайлы, файлы с изображениями, которые добавляются в проект. Здесь речь идет о <span class="b">логических ресурсах</span>, 
...
свойства <span class=b">Resources</span>. Данное свойство опредлено в базовом классе <span class="b">FrameworkElement</span>, поэтому его имеют 
большинство классов WPF.</p>
<p>В чем смысл использования ресурсов? Они повышают эффективность: мы можем определить один раз какой-либо ресурс и затем многократно использовать его в различных местах приложения. 
В связи с этим улучшается поддержка - если возникнет необходимость изменить ресурс, достаточно это сделать в одном месте, и изменения произойдут глобально в приложении.</p>
<p>Свойство <span class="b">Resources</span> представляет объект <span class="b">ResourceDictionary</span> или словарь ресурсов, 
где каждый хранящийся ресурс имеет определенный ключ.</p>
<h3>Определение ресурсов</h3>
<p>Определим ресурс окна и ресурс кнопки:</p>
<pre class="brush:xml;">
...
<p>Здесь у окна определяются два ресурса: redStyle, который представляет объект SolidColorBrush, и gradientStyle, который представляет кисть с линейным градиентом. 
У кнопки определен один ресурс darkStyle, представляющий кисть SolidColorBrush. Причем каждый ресурс обязательно имеет свойство <span class="b">x:Key</span>, 
которое и определяе ключ в словаре.</p>
<p>А в свойствах Background соответственно у грида и кнопки мы можем применить эти ресурсы: <code>Background="{StaticResource gradientStyle}"</code> - здесь после 
выражения <code>StaticResource</code> идет ключ применяемого ресурса.</p>
<img src="./pics/9.1.png" alt="Ресурсы в WPF" />

<h3>Управление ресурсами в коде C#</h3>
<p>Добавим в словарь ресурсов окна градиентную кисть и установим ее для кнопки:</p>
<pre class="brush:c#;">
...
<p>С помощью свойства <code>Add()</code> объект кисти и его произвольный ключ добавляются в словарь. Далее с помощью метода <code>TryFindResource()</code> мы 
пытаемся найти ресурс в словаре и установить его в качестве фона. Причем, так как этот метод возвращает object, необходимо выполнить приведение типов.</p>
<p>Всего у ResourceDictionary можно выделить следующие методы и свойства:</p>
<ul>
<li><p>Метод <span class="b">Add(string key, object resource)</span> добавляет объект с ключом key в словарь, причем в словарь можно добавить любой объект, главное ему сопоставить ключ</p></li>
<li><p>Метод <span class="b">Remove(string key)</span> удаляет из словаря ресурс с ключом key</p></li>
<li><p>Свойство <span class="b">Uri</span> устанавливает источник словаря</p></li>
<li><p>Свойство <span class="b">Keys</span> возвращает все имеющиеся в словаре ключи</p></li>
<li><p>Свойство <span class="b">Values</span> возвращает все имеющиеся в словаре объекты</p></li>
</ul>
<p>Для поиска нужного ресурса в коллекции ресурсов у каждого элемента определены методы <code>FindResource()</code> и <code>TryFindResource()</code>. Она оба возвращают 
ресурс, соответствующий определенному ключу. Единственное различие между ними состоит в том, что <code>FindResource()</code> генерирует исключение, если ресурс с нужным ключом не был 
найден. А метод <code>TryFindResource()</code> в этом случае просто возвращает null.</p>
<h3>Разделяемые ресурсы</h3>
<p>Когда один и тот же ресурс используется в разных местах, то фактически мы используем один и тот же объект. Однако это не всегда желательно. Иногда 
...
изспользовать выражение <code>x:Shared="False"</code>:</p>
<pre class="brush:xml;">&lt;SolidColorBrush x:Shared="False" x:Key="redStyle" Color="BlanchedAlmond" /&gt;</pre>
<h3>Примеры использования ресурсов</h3>
<p>Рассмотрим еще пару примеров применения ресурсов. К примеру, если мы хотим, чтобы ряд кнопок обладал одинаковыми свойствами, то мы можем определить одну общую кнопку в качестве 
ресурса:</p>
<pre class="brush:xml;">
&lt;Window x:Class="ResourcesApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
...
<p>Другой пример - определение списка объектов для списковых элементов:</p>
<pre class="brush:xml;">
&lt;Window x:Class="ResourcesApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
