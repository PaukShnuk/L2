...
<html >
<head>
<title>C# и WPF | ItemsPanel. Установка панели элементов</title>
<meta charset="utf-8" />
<meta name="description" content="Панель данных в WPF, переопределение панели элементов в ListBox и ListView, свойство ItemsPanel и объект ItemPanelTemplate">
...
<h2>ItemsPanel. Установка панели элементов</h2><div class="date">Последнее обновление: 26.02.2016</div>
...
<p>В примере из прошлой темы про шаблоны данных DataTemplate была одна проблема - по умолчанию ListBox размещает все элементы в один вертикальный столбик, 
поэтому если мы, к примеру, раздвинем окно приложения по ширине, то у нас появится много незаполненного пространства справа:</p>
<img src="./pics/14.6.png" alt="Панель элементов в WPF" />
<p>Такой дизайн сложно назвать удачным. Ведь гораздо удобнее было бы в данном случае, если бы элементы при растяжении по ширине автоматически заполняли новое пространство.</p>
<p>Дело в том, что такие элементы как ListBox и ListView по умолчанию инкапсулируют все элементы списка в специальную панель VirtualizingStackPanel, 
которая располагает все элементы по вертикали. Но с помощью свойства <span class="b">ItemsPanel</span> можно переопределить панель элементов в внутри списка.</p>
<p>Например, в ситуации выше было бы более эффективно использовать панель WrapPanel:</p>
<pre class="brush:xml;">
...
<p>Для установки панели применяется элемент <span class="b">ItemsPanelTemplate</span>, который собственно и устанавливает нужную нам панель. 
И также в данном случае с помощью свойства <code>ScrollViewer.HorizontalScrollBarVisibility="Disabled"</code> убирается горизонтальная прокрутка, чтобы она 
нам не мешала. 
В итоге окно приложения будет выглядеть следующим образом:</p>
<img src="./pics/14.7.png" alt="ItemsPanel и ItemsPanelTemplate в WPF" />
<p>И также, как и в случае с шаблонами данных, шаблон панели мы также можем вынести во внешний ресурс и затем этот ресурс подключать:</p>
<pre class="brush:xml;">
&lt;Window x:Class="DataApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<div class="nav"><p><a href="14.3.php">Назад</a><a href="./">Содержание</a><a href="14.5.php">Вперед</a></p></div></div>
<div class="socBlock">
