...
<html >
<head>
<title>C# и WPF | Создание свойств зависимостей</title>
<meta charset="utf-8" />
<meta name="description" content="Создание своих dependency property - свойств зависимостей в WPF, проверка и валидация значений свойств">
...
<h2>Создание свойств зависимостей</h2><div class="date">Последнее обновление: 12.02.2017</div>
...
<p>В предыдущих темах мы рассмотрели теоретические основы свойств зависимостей, теперь посмотрим, как мы можем определять какие-то свои свойства зависимостей. 
Итак, определим в нашем проекте новый класс Phone:</p>
<pre class="brush: c#;">
...
<p>Если мы хотим применять свойства зависимостей, то нам надо унаследовать свой класс от абстрактного класса <code>DependencyObject</code>. 
В нашем классе мы определяем два свойства зависимостей: <code>TitleProperty</code> и <code>PriceProperty</code>. 
Обратите внимание, что они объявляются с модификаторами <code>public static readonly</code>.</p>
<p>Затем свойства регистрируются в статическом конструкторе нашего класса с помощью метода <span class="b">Register</span>. И в конце 
для них создаются обычные свойства-обертки, в которых мы получаем доступ к значению свойств с помощью методов GetValue и SetValue.</p>
<p>Теперь используем наш класс. Для этого определим следующую разметку XAML:</p>
<pre class="brush: xml;">
...
<p>Здесь в ресурсах окна определяется объект Phone с установкой его свойств Price и Name:</p>
<pre class="brush:c#;">&lt;local:Phone Price="600" Title="iPhone 6S" x:Key="iPhone6s" /&gt;</pre>
<p>Данный ресурс имеет ключ <code>iPhone6s</code>, по которому мы можем к нему обратиться. Далее для контейнера Grid мы задаем этот ресурс как контекст данных:</p>
<pre class="brush:xml;">&lt;Grid x:Name="grid1" DataContext="{StaticResource iPhone6s}"&gt;</pre>
<p>Установка контекста данных позволяет внутри грида привязать отдельные элеметы к свойства ресурса, то есть объекта Phone:</p>
<pre class="brush:xml;">&lt;TextBox Text="{Binding Price, Mode=TwoWay}" Grid.Column="1" Grid.Row="1"  /&gt;</pre>
<p>То же самое и для элемента TextBlock. Однако для TextBox у нас действует не просто привязка, а двусторонняя привязка, обозначенная параметром <code>Mode=TwoWay</code>. 
А это значит, что любые изменения свойства Price в ресурсе будут отображаться в текстовом поле. И наоборот - любые изменения в текстовом поле будут менять 
значения в ресурсе.</p>
<p>В этом состоит одно из преимуществ использования свойств зависимостей - для обычных свойств подобные привязки бы не работали.</p>
<p>Для проверки значения ресурса я добавил кнопку, у которой установил обработчик нажатия - Button_Click. И также добавим код этого обработчика в файл кода C#:</p>
<pre class="brush:c#;">
...
<p>Запустим приложение, изменим значение в текстовом поле, например, на 800 и нажмем на кнопку:</p>
<img src="./pics/13.1.png" alt="Dependency Property in WPF" />
<h3>Добавление валидации</h3>
<p>WPF предоставляет два способа валидации значения свойства:</p>
<ul>
<li><p><span class="b">ValidateValueCallback</span>: делегат, который возвращает true, если значение проходит валидацию, и false - если не проходит</p></li>
<li><p><span class="b">CoerceValueCallback</span>: делегат, который может подкорректировать уже существующее значение свойства, если оно вдруг не попадает в диапазон допустимых значений</p></li>
</ul>
<p>При установке значения сначала срабатывает делегат <code>ValidateValueCallback</code>, который возвращает true, если значение проходит валидацию. 
Далее срабатывает делегат <code>CoerceValueCallback</code>, который может модифицировать это значение, если оно вдруг является некорректным. 
В случае, если эти два делегата успешно отработали, то срабатывает делегат PropertyChangedCallback, который извещает систему об изменении значени свойства.</p>
<p>При этом нам необязательно использовать сразу оба эти делеата, можно применять лишь один из них, а можно их комбинировать вместе.</p>
<p>Вначале применим делегат ValidateValueCallback. Для этого изменим код класса Phone на следующий:</p>
<pre class="brush: c#;">
public class Phone : DependencyObject
{
    public static readonly DependencyProperty TitleProperty;
...
<p>Делегат <code>ValidateValueCallback</code> указывает на метод <code>ValidateValue</code>, который в качестве параметра принимает новое значение свойства. 
Если оно равно нулю или выше, так как отрицательные цены не имеют смысла, то значение проходит валидацию, и метод возвращает true.</p>
<p>При подобных проверках надо учитывать, что в процессе запуска приложения значения ресурса Phone устанавливаются два раза. Первый раз свойство Price получает 
значение по умолчанию для типов int, то есть число 0. И только потом на следующем этапе инициализации оно получает число 600, которое и установлено в ресурсе Phone. Поэтому 
если мы, к примеру, изменим логику валидации:</p>
<pre class="brush:c#;">
if (currentValue &gt; 0)
    return true;
</pre>
<p>То мы получим ошибку. Ведь число 0 теперь не является допустимым, но на начальной инициализации объекта Phone, именно 0 устанавливается в качестве 
значения для свойства Price. Поэтому подобные моменты надо учитывать.</p>
<p>Теперь, если мы запустим приложение и попробуем установить число меньше 0, то оно не пройдет валидацию, и ресурс Phone сохранит старое значение для свойства Price.</p>
<p>Теперь применим делегат <span class="b">CoerceValueCallback</code>:</p>
<pre class="brush: c#;">
public class Phone : DependencyObject
{
    public static readonly DependencyProperty TitleProperty;
...
<p>В данном случае мы будем корректировать значение свойства Price, так как оно изменяется через текстовое поле. Для этого делегат CoerceValueCallback 
будет вызывать метод <code>CorrectValue</code>. Данный метод должен принимать два параметра: <span class="b">DependencyObject</span> - валидируемый объект (в данном случае объект Phone) и 
<span class="b">object</span> - новое значение для свойства Price (либо для другого устанавливаемого свойства).</p>
<p>В самом методе <code>CorrectValue()</code> мы получаем новое значение и модифицируем его, если оно некорректно.</p>
<p>Единственное, что надо учитывать при работе с делегатом CoerceValueCallback, то, что он вызывается только тогда, когда вызов делегата ValidateValueCallback возвращает значение <code>true</code>.</p>
<p>Больше нам ничего менять не надо, разметка xaml остается прежней. И если мы запустим приложение и введем в текстоое поле некорректное значение, 
то сработает этот делегат, который подкорректирует значение.</p>
...
<div class="nav"><p><a href="13.2.php">Назад</a><a href="./">Содержание</a><a href="6.php">Вперед</a></p></div></div>
<div class="socBlock">
