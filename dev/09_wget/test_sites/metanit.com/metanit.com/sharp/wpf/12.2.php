...
<html >
<head>
<title>C# и WPF | Создание и использование шаблонов</title>
<meta charset="utf-8" />
<meta name="description" content="Шаблоны в WPF, их создание и использование, элемент ControlTemplate, триггеры в шаблонах">
...
<h2>Создание и использование шаблонов</h2><div class="date">Последнее обновление: 15.02.2016</div>
...
<p>Все визуальные элементы в WPF уже имеют встроенные шаблоны, которые определяют визуальное дерево, структуру и даже поведение элементов. Однако мощь шаблонов 
состоит в том, что мы можем их переопределить по своему вкусу. Например, сделать круглое окно, а не квадратное, или кнопку в виде морской звезды.</p>
<p>К примеру создадим округлый элемент Button:</p>
<pre class="brush:xml;">
...
<img src="./pics/8.5.png" alt="ControlTemplate в WPF" />
<p>Мы можем определять шаблоны с через стили, а можем в виде отдельных ресурсов. Так, в данном случае с помощью элемента <span class="b">ControlTemplate</span> 
определяется ресурс с ключом "btTemplate".</p>
<p>В ControlTemplate вложены элементы Border и ContentControl, которые через свои свойства определяют, как будет выглядеть кнопка.</p>
<h3>TemplateBinding</h3>
<p>Вышеопределенный шаблон устанавливал цвет, ряд других параметров, которые мы не могли бы изменить. Например, если мы установим в кнопке цвет фона, то этот цвет никак не повлияет, 
так как в реальности будет работать только цвет, определенный в элементе Border:</p>
<pre class="brush:xml;">&lt;Button x:Name="myButton" Template="{StaticResource btTemplate}" Background="Red"&gt;Привет&lt;/Button&gt;</pre>
<p>Чтобы мы могли из элемента, к которому применяется шаблон, влиять на свойства, определенные в этом шаблоне, нам надо использовать элемент 
<span class="b">TemplateBinding</span>. Он служит для установки в шаблоне привязки к свойствам элемента. Так, изменим шаблон следующим образом:</p>
<pre class="brush:xml;">
&lt;Window x:Class="ControlsTemplateApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<p>Выражение типа <code>Background="{TemplateBinding Background}"</code> в элементе Border указывает, что фон элемента Border будет привязан к свойству 
Background элемента Button. Таким образом, здесь практически все свойства Border и ContentControl (кроме свойства CornerRadius) устанавливаются из разметки элемента Button</p>
<h3>Свойство Template</h3>
<p>Используя свойство <span class="b">Template</span>, можно определить шаблон напрямую в самом элементе:</p>
<pre class="brush:xml;">
...
<h3>Триггеры в шаблонах</h3>
<p>Если мы возьмем стандартные кнопки (или другие элементы управления), то мы можем заметить, что они меняют свои свойства в различных состояниях. 
Например, кнопка изменяет фон при нажатии. Рассмотрим, как мы сможем сделать подобные вещи.</p>
<p>Для реагирования на изменение свойств в шаблонах, как и в стилях, используютс триггеры. Для триггеров в элементе ControlTemplate определена коллекция 
<span class="b">Triggers</span>. Например, добавим к ранее определенному шаблону несколько триггеров:</p>
<pre class="brush:xml;">
&lt;Button x:Name="myButton" Content="Привет" Height="40" Width="100" Background="LightPink"&gt;
    &lt;Button.Template&gt;
        &lt;ControlTemplate TargetType="Button"&gt;
...
<p>Здесь для элемента Border определено свойство <code>x:Name="buttonBorder"</code>, благодаря чему мы можем ссылаться на этот элемент в триггерах.</p>
<p>В коллекции триггеров шаблона определено три триггера. Первый срабатывает, когда свойство <span class="b">IsMouseOver</span> элемена Button будет равно true. То есть при 
наведении указателя мыши на кнопку. В этом случае триггер делает жирным шрифт кнопки:</p>
<pre class="brush:xml;">
&lt;Trigger Property="IsMouseOver" Value="true"&gt;
    &lt;Setter Property="FontWeight" Value="Bold" /&gt;
&lt;/Trigger&gt;
</pre>
<p>Второй триггер срабатывает при нажатии на кнопку, а третий - когда свойство <span class="b">IsEnabled</span> равно <code>false</code>. Причем в этих 
триггерах свойство <span class="b">TargetName</span>, что позволяет установить свойства не самой кнопки, а элемента Border в шаблоне.</p>
...
<div class="nav"><p><a href="12.php">Назад</a><a href="./">Содержание</a><a href="12.3.php">Вперед</a></p></div></div>
<div class="socBlock">
