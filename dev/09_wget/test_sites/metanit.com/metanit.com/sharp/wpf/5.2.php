...
<html >
<head>
<title>C# и WPF | Кнопки</title>
<meta charset="utf-8" />
<meta name="description" content="Использование кнопок в WPF, их типы - Button, RepeatButton, ToggleButton, CheckBox, RadioButton, их основные свойства и события">
...
<h2>Кнопки</h2><div class="date">Последнее обновление: 18.01.2016</div>
...
<p>В WPF кнопки представлены целым рядом классов, которые наследуются от базового класса ButtonBase:</p>
<img src="./pics/4.49.png" alt="Кнопки Button в WPF" />
<h3>Button</h3>
<p>Элемент Button представляет обычную кнопку:</p>
<pre class="brush:xml;">&lt;Button x:Name="button1" Width="60" Height="30" Background="LightGray" /&gt;</pre>
<p>От класса ButtonBase кнопка наследует ряд событий, например, Click, которые позволяют обрабатывать пользовательский ввод.</p>
<p>Чтобы связать кнопку с обработчиком события нажатия, нам надо определить в самой кнопке атрибут <code>Click</code>. А значением этого атрибута 
будет название обработчика в коде C#. А затем в самом коде C# определить этот обработчик.</p>
<p>Например, код xaml:</p>
<pre class="brush:xml;">
&lt;Button x:Name="button1" Width="60" Height="30" Content="Нажать" Click="Button_Click" /&gt;
</pre>
<p>И обработчик в коде C#:</p>
<pre class="brush:c#;">
...
<p>Либо можно не задавать обработчик через атрибут, а стандартным образом для C# прописать в коде: <code>button1.Click+=Button_Click;</code></p>
<p>К унаследованным свойствам кнопка имеет такие свойства как <span class="b">IsDefault</span> и <span class="b">IsCancel</span>, которые принимают значения true и false.</p>
<p>Если свойство IsDefault установлено в true, то при нажатии клавиши Enter будет вызываться обработчик нажатия этой кнопки.</p>
<p>Аналогично если свойство IsCancel будет установлено в true, то при нажатии на клавишу Esc будет вызываться обработчик нажатия этой кнопки.</p>
<p>Например, определим код xaml:</p>
<pre class="brush:xml;">
...
<p>А в коде MainWindow.xaml.cs определим следующий код C#:</p>
<pre class="brush:c#;">
...
<p>Теперь при нажатии на клавишу Enter будет отображаться сообщение, а при нажатии на Esc будет происходить выход из приложения и закрытие окна.</p>

<h3>RepeatButton</h3>
<p>Отличительная особенность элемента RepeatButton - непрерывная генерация события <code>Click</code>, пока нажата кнопка. Интервал генерации 
события корректируется свойствами <span class="b">Delay</span> и <span class="b">Interval</span>.</p>
<p>Сам по себе элемент RepeatButton редко используется, однако он может служить основой для создания ползунка в элементах ScrollBar и ScrollViewer, в которых 
нажатие на ползунок инициирует постоянную прокрутку.</p>
<h3>ToggleButton</h3>
<p>Представляет элементарный переключатель. Может находиться в трех состояниях - true, false и "нулевом" (неотмеченном) состоянии, а его значение представляет значение типа 
<code>bool?</code> в языке C#. Состояние можно установить или получить с помощью свойства <span class="b">IsChecked</span>. 
Также добавляет три события - <span class="b">Checked</span> (переход в отмеченное состояние), <span class="b">Unchecked</span> (снятие отметки) 
и <span class="b">Intermediate</span> (если значение равно null). Чтобы отрабатывать все три события, надо установить свойство <span class="b">IsThreeState="True"</span></p>
<p>ToggleButton, как правило, сам по себе тоже редко используется, однако при этом он служит основой для создания других более функциональных элементов, таких 
как checkbox и radiobutton.</p>
...
<div class="nav"><p><a href="5.19.php">Назад</a><a href="./">Содержание</a><a href="5.18.php">Вперед</a></p></div></div>
<div class="socBlock">
