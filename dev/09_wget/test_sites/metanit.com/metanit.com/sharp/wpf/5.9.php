...
<html >
<head>
<title>C# и WPF | ListView</title>
<meta charset="utf-8" />
<meta name="description" content="Использование элемента ListView в WPF, его взаимодействие с коллекциями и массивами, отображение данных, особенности работы">
...
<h2>ListView</h2><div class="date">Последнее обновление: 10.01.2018</div>
...
<p>Этот элемент управления отображает информацию на множестве строк и столбцов. Он унаследован от класса <span class="b">ListBox</span>, поэтому может вести себя простой список:</p>
<pre class="brush:xml;">
&lt;ListView&gt;
...
<p>Но чтобы создать более сложные по структуре данные используется свойство <span class="b">View</span>. Это свойство принимает в качестве 
значения объект GridView, который управляет отображением данных. GridView определяет коллекцию определений столбцов - <span class="b">GridViewColumn</span>, 
которое с помощью свойства Header определяет название столбца, а с помощью свойства DisplayMemberBinding можно определить привязку столбца к определенному 
свойству добавляемого в ListView объекта.</p>
<pre class="brush:xml;">&lt;GridViewColumn DisplayMemberBinding="{Binding Path=Company}"&gt;Компания&lt;/GridViewColumn&gt;</pre>
<p>Допустим у нас в проекте определен класс Phone:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Создадим в xaml-коде коллекцию объектов <code>Phone</code> (в принципе это можно было бы сделать и в файле кода) и объявим привязку столбцов ListView к свойствам объектов Phone:</p>
<pre class="brush: xml;">
...
<img src="./pics/4.17.png" alt="Элемент ListView в WPF" />
