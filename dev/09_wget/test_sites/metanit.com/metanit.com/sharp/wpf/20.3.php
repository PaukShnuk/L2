...
<html >
<head>
<title>C# и WPF | Диалоговые окна</title>
<meta charset="utf-8" />
<meta name="description" content="Создание диалоговых модальных окон в WPF, метод ShowDialog и DialogResult">
...
<h2>Диалоговые окна</h2><div class="date">Последнее обновление: 21.03.2016</div>
...
<p>WPF поддерживает возможность создания модальных диалоговых окон. При вызове модальное окно блокирует доступ к родительскому окну, пока пользователь 
не закроет модальное окно.</p>
<p>Для работы добавим в проект новое окно, которое назовем <span class="b">PasswordWindow</span>. Это окно будет выполнять роль модального.</p>
<p>Изменим интерфейс PasswordWindow:</p>
<pre class="brush:xml;">
...
<p>Здесь определено текстовое поле для ввода пароля и две кнопки. Вторая кнопка с атрибутом <code>IsCancel="True"</code> будет выполнять роль отмены. 
А первая кнопка будет подтверждать ввод.</p>
<p>Для подтверждения ввода и успешного выхода из модального окна определим в файле кода PasswordWindow определим обработчик первой кнопки Accept_Click:</p>
<pre class="brush:c#;">
...
<p>Для успешного выхода из модального диалогового окна нам надо для свойства <span class="b">DialogResult</span> установить значение <code>true</code>. Для 
второй кнопки необязательно определять обработчик, так как у нее установлен атрибут <code>IsCancel="True"</code>, следовательно, ее нажатие будет эквивалентно 
результату <code>this.DialogResult = false;</code>. Этот же результат будет при закрытии диалогового окна на крестик.</p>
<p>Кроме того, здесь определяется свойство <code>Password</code>, через которое мы можем извне получить введенный пароль.</p>
<p>И изменим главную форму MainWindow, чтобы из нее запускать диалоговое окно. Во-первых, определим кнопку:</p>
<pre class="brush:xml;">
...
<p>И определим обработчик для этой кнопки:</p>
<pre class="brush:c#;">
...
<p>В итоге при нажатии на кнопку будет отображаться следующее диалоговое окно:</p>
<img src="./pics/20.4.png" alt="Модальные диалоговые окна в WPF" />
<p>И в зависимости от результатов ввода будет отображаться то или иное сообщение.</p>
...
<div class="nav"><p><a href="20.2.php">Назад</a><a href="./">Содержание</a><a href="22.1.php">Вперед</a></p></div></div>
<div class="socBlock">
