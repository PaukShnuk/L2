...
<html >
<head>
<title>C# и WPF | Плавность анимации</title>
<meta charset="utf-8" />
<meta name="description" content="Функции плавности анимации в WPF, типы Easing functions, роль свойства EasingMode, создание своей функции анимации">
...
<h2>Плавность анимации</h2><div class="date">Последнее обновление: 04.03.2016</div>
...
<p>Для создания плавности анимации используется различные функции плавности или эластичности (Easing functions). Их основная задача - сделать ход анимации 
более плавным, более естественным.</p>
<p>Рассмотрим самую простейшую функцию анимации - <span class="b">ElasticEase</span>. 
С помощью ее свойства <span class="b">Oscillations</span> можно определить количество колебательных движений во время анимации, в итоге анимируемый 
элемент как бы пружинится.</p>
<p>Свойство <span class="b">Springiness</span> позволяет установить жесткость, и чем больше это значение, тем быстрее затухают колебания.</p> 
<p>Еще одно свойство <span class="b">EasingMode</span> указывает на режим эластичности и может принимать одно из трех значений:</p>
<ul>
<li><p><span class="b">EaseIn</span>: функция плавности применяется в начале анимации</p></li>
<li><p><span class="b">EaseOut</span>: функция плавности применяется в конце анимации</p></li>
<li><p><span class="b">EaseInOut</span>: функция плавности применяется в начале и в конце анимации</p></li>
</ul>
<p>Например,</p>
<pre class="brush:xml;">
...
<p>Настройка функции анимации <code>Oscillations="5"</code> указывает, что будет пять колебаний. А свойсто <code>EasingMode="EaseOut"</code> устанавливает, 
что эти колебания будут в конце анимации. Если бы мы захотели увидить колебания в начале, то соответственно надо было бы использовать настройку <code>EasingMode="EaseIn"</code></p>
<p>В WPF определено всего 11 функций плавности анимации:</p>
<ul>
<li><p><span class="b">BackEase</span> возвращает анимацию назад, а ее свойство <span class="b">Amplitute</span> задает амплитуду возврата</p></li>
<li><p><span class="b">BounceEase</span> создает эффект отскока, а свойство <span class="b">Bounces</span> устанавливает количество отскоков.</p></li>
<li><p><span class="b">ElasticEase</span> создает анимацию осцилляций пружины назад и вперед до момента ее полной остановки</p></li>
<li><p><span class="b">SineEase</span> создает анимацию с помощью формулы синуса</p></li>
<li><p><span class="b">ExponentialEase</span> создает анимацию с помощью экспоненциальной формулы.</p></li>
<li><p><span class="b">CircleEase</span> создает анимацию с помощью цикличной функции.</p></li>
<li><p><span class="b">QuadraticEase</span> функция плавности на основе квадратичной функции f(t) = t<sup>2</sup></p></li>
<li><p><span class="b">CubicEase</span> функция плавности на основе кубической функции f(t) = t<sup>3</sup></p></li>
<li><p><span class="b">QuarticEase</span> создает анимацию с помощью формулы f(t) = t<sup>4</sup></p></li>
<li><p><span class="b">QuinticEase</span> создает анимацию с помощью формулы f(t) = t<sup>5</sup></p></li>
<li><p><span class="b">PowerEase</span> ускоряет анимацию вначале и замедляет в конце, используя функцию f(t) = t<sup>p</sup>. В зависимости от значения 
экспоненты p посредством этой функции мы можем замещать функции QuadraticEase, QuarticEase и QuinticEase</p></li>
</ul>
<p>Например, применение функции PowerEase:</p>
<pre class="brush:xml;">
&lt;DoubleAnimation.EasingFunction&gt;
    &lt;PowerEase Power="6" /&gt;
&lt;/DoubleAnimation.EasingFunction&gt;
</pre>
<p>Или функция BounceEase:</p>
<pre class="brush:xml;">
&lt;DoubleAnimation.EasingFunction&gt;
    &lt;BounceEase EasingMode="EaseOut" Bounces="3" /&gt;
&lt;/DoubleAnimation.EasingFunction&gt;
</pre>
<h3>Создание своей функции анимации</h3>
<p>WPF имеет ограниченный набор функций плавности, и, возможно, в какой-то момент их станет недостаточно. В этом случае можно созать свою функцию. К счастью, 
определение функции плавности представляет довольно тривиальный процесс.</p>
<p>Например, определим функцию, которая использует для выисления значений функцию f(t) = t<sup>6</sup>. Для этого добавим в проект следующий класс:</p>
<pre class="brush:c#;">
...
<p>Метод <code>EaseInCore()</code> выполняет всю работу функции плавности. Он получает текущее значение анимируемого свойства в виде параметра normalizedTime. 
Здесь мы просто возводит его в шестую степень. А метод <code>CreateInstanceCore()</code> должен возвращать объект функции анимации.</p>
<p>Применение функции в XAML:</p>
<pre class="brush:xml;">
&lt;DoubleAnimation Storyboard.TargetProperty="Width" 
                To="150"
                Duration="0:0:5"&gt;
...
<div class="nav"><p><a href="16.4.php">Назад</a><a href="./">Содержание</a><a href="20.1.php">Вперед</a></p></div></div>
<div class="socBlock">
