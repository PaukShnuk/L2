...
<html >
<head>
<title>C# и WPF | Создание вкладок и TabControl</title>
<meta charset="utf-8" />
<meta name="description" content="Создание вкладок в WPF, элемент TabControl, объекты TabPage, создание, изменение вкладок, особенности визуализации">
...
<h2>Создание вкладок и TabControl</h2><div class="date">Последнее обновление: 18.01.2016</div>
...
<p>Для создания вкладок в WPF, как и в WinForms, предназначен элемент TabControl, а отдельная вкладка представлена элементом TabItem:</p>
<pre class="brush:xml;">
&lt;TabControl&gt;
...
<p>Элемент TabItem является элементом управления содержимым, поэтому в него можно вложить другие элементы:</p>
<pre class="brush:xml;">
...
<img src="./pics/4.18.png" alt="Элемент TabControl в WPF" />
<p>Класс TabItem наследуется от класса HeaderedContentControl, поэтому кроме свойства <code>Content</code>, определедяющее содержимое вкладки, имеет также 
свойство <span class="b">Header</span>, которое определяет заголовок. И в этот заголовок мы можем вложить различное содержимое, как в примере выше.</p>
<p>И также, как и в случае с ListBoxItem и ComboBoxItem, мы можем вложить в TabControl и другие элементы, которые неявно образуют отдельные вкладки:</p>
<pre class="brush:xml;">
&lt;TabControl&gt;
...
<h3>Программное добавление вкладок</h3>
<p>Допустим, у нас на форме есть TabControl:</p>
<pre class="brush:xml;">
&lt;TabControl x:Name="products"&gt;
&lt;/TabControl&gt;
</pre>
<p>Через код C# добавим в него вкладку:</p>
<pre class="brush:c#;">
