...
<html >
<head>
<title>C# и WPF | Основы анимаций</title>
<meta charset="utf-8" />
<meta name="description" content="Основы анимаций в WPF, их типы, программное создание и настройка анимации">
...
<h1>Анимация</h1><h2>Основы анимаций</h2><div class="date">Последнее обновление: 04.03.2016</div>
...
<p>Наличие встроенных возможностей анимации представляют одну из ключевых особенностей платформы WPF. Анимации в WPF  - это действиетельно мощное средство, 
которое при этом очень легко использовать. Но перед тем, как перейти к созданию анимаций, сразу надо сказать об ограничениях:</p>
<ul>
<li><p>Одна анимация выполняется только для одного свойства зависимостей<p></li>
<li><p>Для анимации свойства необходим класс анимации, который бы поддерживал тип этого свойства. Например, для изменения таких свойств, как длина, ширина, которые представляют тип <code>double</code>, предназначен класс 
<span class="b">DoubleAnimation</span>. Для изменения цвета фона или шрифта - <span class="b">ColorAnimation</span>, для изменения свойства <code>Margin</code> -  <span class="b">ThiknessAnimation</span>.<p></li>
</ul>
<p>За анимацию в WPF отвечает пространство имен <code>System.Windows.Media.Animation</code>. Оно содержит довольно большой набор классов, позволяющих 
анимировать различные свойства. Но в реальности все классы анимаций можно разделить условно на три группы:</p>
<ul>
<li><p>Классы, которые производят линейную интерполяцию значений, благодаря чему при анимации свойство плавно изменяет свое значение. Как правило, такие классы 
называются по шаблону <code>TypeAnimation</code>, где <code>Type</code> - тип данных, которое представляет анимируемое свойства, например, <code>DoubleAnimation</code>:</p>
<ul> 
<li><p>ByteAnimation</p></li>
<li><p>ColorAnimation</p></li>
<li><p>DecimalAnimation</p></li>
<li><p>DoubleAnimation</p></li>
<li><p>Int16Animation</p></li>
<li><p>Int32Animation</p></li>
<li><p>Int64Animation</p></li>
<li><p>PointAnimation</p></li>
<li><p>Point3DAnimation</p></li>
<li><p>QuarternionAnimation</p></li>
<li><p>RectAnimation</p></li>
<li><p>Rotation3DAnimation</p></li>
<li><p>SingleAnimation</p></li>
<li><p>SizeAnimation</p></li>
<li><p>ThicknessAnimation</p></li>
<li><p>VectorAnimation</p></li>
<li><p>Vector3DAnimation</p></li>
</ul>

</li>
<li><p>Классы, которые производят анимацию по ключевым кадрам или фреймам (покадровая анимация). Такие классы, как правило, называются по шаблону 
<code>TypeAnimationUsingKeyFrames</code>, например, <code>DoubleAnimationUsingKeyFrames</code></p>
<ul>
<li><p>BooleanAnimationUsingKeyFrames</p></li>
<li><p>ByteAnimationUsingKeyFrames</p></li>
<li><p>CharAnimationUsingKeyFrames</p></li>
<li><p>ColorAnimationUsingKeyFrames</p></li>
<li><p>DecimalAnimationUsingKeyFrames</p></li>
<li><p>DoubleAnimationUsingKeyFrames</p></li>
<li><p>Int16AnimationUsingKeyFrames</p></li>
<li><p>Int32AnimationUsingKeyFrames</p></li>
<li><p>Int64AnimationUsingKeyFrames</p></li>
<li><p>MatrixAnimationUsingKeyFrames</p></li>
<li><p>ObjectAnimationUsingKeyFrames</p></li>
<li><p>PointAnimationUsingKeyFrames</p></li>
<li><p>Point3DAnimationUsingKeyFrames</p></li>
<li><p>QuarternionAnimationUsingKeyFrames</p></li>
<li><p>RectAnimationUsingKeyFrames</p></li>
<li><p>Rotation3DAnimationUsingKeyFrames</p></li>
<li><p>SingleAnimationUsingKeyFrames</p></li>
<li><p>SizeAnimationUsingKeyFrames</p></li>
<li><p>StringAnimationUsingKeyFrames</p></li>
<li><p>ThicknessAnimationUsingKeyFrames</p></li>
<li><p>VectorAnimationUsingKeyFrames</p></li>
<li><p>Vector3DAnimationUsingKeyFrames</p></li>
</ul>

</li>
<li><p>Классы, которые используют для анимации объект PathGeometry. Такие классы, как правило, называются по шаблону 
<code>TypeAnimationUsingPath</code>, например, <code>DoubleAnimationUsingPath</code></p>

<ul>
<li><p>DoubleAnimationUsingPath</p></li>
<li><p>MatrixAnimationUsingPath</p></li>
<li><p>PointAnimationUsingPath</p></li>
</ul>

</li>
</ul>
<p>Анимацию можно создать и использовать как декларативно в коде XAML, так и программно в коде C#.</p>
<h3>Программная анимация</h3>
<p>Пусть в XAML определена кнопка:</p>
<pre class="brush:xml;">
&lt;Button x:Name="helloButton" Width="70" Height="30" Content="Hello" /&gt;
</pre>
<p>Проанимируем ее свойство <code>Width</code>:</p>
<pre class="brush:c#;">
using System;
...
<p>В данном случае мы сначала задаем тип анимации и создаем ее. Поскольку изменяется свойство Width, это будет <span class="bb">DoubleAnimation</span>.</p>
<p>У любого класса линейной анимации есть набор свойств, с помощью которых мы можем управлять анимацией:</p>
<ul>
<li><p><span class="b">From</span>: начальное значение, с которого будет начинается анимация</p></li>
<li><p><span class="b">To</span>: конечное значение</p></li>
<li><p><span class="b">Duration</span>: время анимации в виде объекта TimeSpan</p></li>
<li><p><span class="b">By</span>: значение, которое указывает, насколько должно увеличиться анимируемое свойство. Свойство By используется вместо свойства To</p></li>
<li><p><span class="b">RepeatBehavior</span>: позволяет установить, как анимация будет повторяться</p></li>
<li><p><span class="b">AccelerationRatio</span>: задает ускорение анимации</p></li>
<li><p><span class="b">DecelerationRatio</span>: устанавливает замедление анимации</p></li>
<li><p><span class="b">SpeedRatio</span>: устанавливает скорость анимации. По умолчанию значение 1.0</p></li>
<li><p><span class="b">AutoReverse</span>: при значении true анимация выполняется в противоположную сторону</p></li>
<li><p><span class="b">FillBehavior</span>: определеяет поведение после окночания анимации. Если оно имеет значение <code>Stop</code>, то после окончания 
анимации объект возвращает прежние значения: <code>buttonAnimation.FillBehavior = FillBehavior.Stop</code>. Если же оно имеет значение <code>HoldEnd</code>, 
то анимация присваивает анимируемому свойству новое значение.</p></li>
</ul>
<p>Чтобы запустить анимацию, у элемента вызывается метод <code>BeginAnimation()</code>. В этот метод передается свойство зависимости, которое надо анимировать, и сам объект анимации.</p>
<p>В данном случае произойдет изменение ширины кнопки от текущего значения до 150 пикселей. И данное изменение будет длиться 3 секунды.</p>
<p>Если мы хотим повторения анимации, можно использовать свойство RepeatBehavior:</p>
<pre class="brush:c#;">
DoubleAnimation buttonAnimation = new DoubleAnimation();
buttonAnimation.From = helloButton.ActualWidth;
buttonAnimation.To = 150;
...
<p>В данном случае анимация будет повторяться два раза. Также мы можем задать время повторения:</p>
<pre class="brush:c#;">
buttonAnimation.Duration = TimeSpan.FromSeconds(3);
buttonAnimation.RepeatBehavior = new RepeatBehavior(TimeSpan.FromSeconds(7));
</pre>
<p>Здесь время повторения - 7 секунд. Анимация длится 3 секунды, а это значит, что будет 7 / 3 повторений: два полноценных повторения и в последнем случае 
ширина увелится только до трети требуемой ширины.</p>
<p>Чтобы задать плавное изменение свойства в обратную сторону, применим свойство AutoReverse:</p>
<pre class="brush:c#;">
buttonAnimation.AutoReverse = true;
buttonAnimation.RepeatBehavior = new RepeatBehavior(5);
</pre>
<h3>Событие Completed</h3>
<p>При завершении анимации генерируется событие Completed, которое мы можем обработать:</p>
<pre class="brush:c#;">
public MainWindow()
{
    InitializeComponent();
...
<h3>Анимация нескольких свойств</h3>
<p>Одна анимация может работать только с одним свойством объекта. Однако если нам надо анимировать сразу несколько свойств, то мы можем задать для каждоо свой объект анимации:</p>
<pre class="brush:c#;">
// анимация для ширины
...
<div class="nav"><p><a href="18.2.php">Назад</a><a href="./">Содержание</a><a href="16.2.php">Вперед</a></p></div></div>
<div class="socBlock">
