...
<html >
<head>
<title>C# и WPF | Шаблоны элементов управления</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в шаблоны WPF, логическое и визуальное дерево, использование классов LogicalTreeHelper и VisualTreeHelper, изменение шаблонов через ControlTemplate и ContentPresenter">
...
<h1>Шаблоны элементов управления</h1><h2>Логическое и визуальное дерево</h2><div class="date">Последнее обновление: 15.02.2016</div>
...
<p>Несмотря на то, что стили существенно облегчают манипулирование внешним видом элементов управления, 
...
Чтобы понять шаблоны, необходимо осознать общую концепцию визуализации в WPF.<p>
<p>Визуализация в WPF тесно связана с такими понятиями как логическое и визуальное дерево. Эти деревья являются своего рода каркасом приложения. 
Так мы можем представить приложение как некий набор вложенных элементов. Возьмем, к примеру, следующую простейшую разметку xaml:</p>
<pre class="brush:xml;">
...
<p>Структуру элементов здесь можно представить следующей схемой:</p>
<img src="./pics/8.1.png" alt="Логическое дерево в WPF" />
<p>То есть в Window есть Grid, в Gride - элемент Button, в кнопке в качестве содержимого установлен некоторый текст в виде объекта String. В итоге 
получается некое дерево элементов, которое называется <span class="b">логическим</span>. В WPF оно представлено классом 
<span class="b">System.Windows.LogicalTreeHelper</span>. Логическое дерево имеет дело с визуализацией как таковой, оно образует модель доступа к дочерним элементам.</p>
<p>От него отличается <span class="b">визуальное дерево</span>, представленное классом <span class="b">System.Windows.Media.VisualTreeHelper</span>. Так, визуальное дерево для вышеприведенной разметки xaml будет 
выглядеть следующим образом:</p>
<img src="./pics/8.2.png" alt="Visual Tree в WPF" />
<p>Визуальное дерево получается гораздо сложнее, оно показывает, как с визуальной точки зрения устроен элемент, из каких частей он состоит.</p>
<p>Visual Studio имеет встроенные средства для просмотра визуального дерева элементов. Для этого нам надо запустить проект в режиме отладки и в меню 
выбрать пункт <span class="b">Debug -&gt; Windows -&gt; Live Visual Tree</span>. После нажатия на этот пункт Visual Studio откроет окно с визуальным деревом, в 
котором мы можем посмотреть, как устроен элемент:</p>
<img src="./pics/8.3.png" alt="Визуальное дерево в Visual Studio в WPF" />
<p>Если мы нажмем правой кнопкой мыши на элемент в этом дереве и в контекстном меню выберем пункт <span class="b">Show Properties</span>, справа в 
Visual Studio откроется окно свойств для этого элемента.</p>
<img src="./pics/8.4.png" />
<p>Визуальное дерево элемента управления опредлеляет, как будет выглядеть этот элемент или иными словами его <span class="b">шаблон</span>. Шаблон элемента - это своего рода визуальный скелет элемента управления. 
Например, для элемента Button упрощенно шаблон выглядит следующим образом:</p>
<pre class="brush:xml;">
...
<p>Полное описание шаблона кнопки можно найти по ссылке <a href="https://msdn.microsoft.com/en-us/library/ms753328%28v=vs.100%29.aspx" rel="nofollow">https://msdn.microsoft.com/en-us/library/ms753328%28v=vs.100%29.aspx</a>.</p>
<p>Здесь мы можем увидеть, что сам шаблон задается с помощью стиля при определении сеттера для свойства <span class="b">Template</span>. Чтобы 
определить сам, шаблон, применяется элемент <span class="b">ControlTemplate</span>. И как раз в этом элементе мы можем увидеть и элемент Border, и элемент 
ContentPresenter, которые составляют визуальное дерево кнопки.</p>
...
<div class="nav"><p><a href="3.3.php">Назад</a><a href="./">Содержание</a><a href="12.2.php">Вперед</a></p></div></div>
<div class="socBlock">
