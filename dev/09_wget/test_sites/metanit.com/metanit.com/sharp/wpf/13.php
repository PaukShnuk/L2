...
<html >
<head>
<title>C# и WPF | DependencyObject и свойства зависимостей</title>
<meta charset="utf-8" />
<meta name="description" content="Обзор и использование класса DependencyObject и свойств зависимостей в WPF">
...
<h1>DependencyObject и свойства зависимостей</h1><h2>Введение в Dependency Property</h2><div class="date">Последнее обновление: 24.01.2016</div>
...
<p>В прошлой главе были рассмотрены базовые элементы WPF и их основные свойства. Однако рассмотренные свойства элементов, как например, Width или Height, 
являются не просто стандартными свойствами языка C#. Они фактически скрывают <span class="b">свойства зависимостей</span> или <span class="b">dependency property</span>. 
Без свойств зависимостей были бы невозможны многие ключевые особенности WPF, как привязка данных, стили, анимация и т.д.</p>
<p>Рассмотрим, как они определяются. Возьмем, к примеру, элемент TextBlock, у которого есть свойство <span class="b">Text</span>:</p>
<pre class="brush:c#;">
...
<p>Статическое свойство <code>TextProperty</code> является свойством зависимостей, представляя объект <span class="b">System.Windows.DependencyProperty</span>. 
По соглашениям по именованию все свойства зависимостей представляют статические публичные поля (public static) с суффиксом <i>Property</i>.</p>
<p>Затем в статическом конструкторе класса происходит регистрация свойства с помощью метода <span class="b">DependencyProperty.Register()</span>, 
в который передается ряд параметров:</p>
<ul>
<li><p><span class="b">имя свойства</span> (в данном случае "Text"). Как правило, соответствует названию свойства зависимостей без суффикса <i>Property</i></p></li>
<li><p><span class="b">тип свойства</span> (в данном случае string)</p></li>
<li><p><span class="b">тип, который владеет свойством</span> -  собственно тот тип, в котором свойство определено или в данном случае тип TextBlock</p></li>
<li><p>Необязательный параметр <span class="b">FrameworkPropertyMetadata</span> устанавливает дополнительные настройки свойства</p></li>
<li><p>В качестве пятого необязательного параметра может использоваться ссылка на метод, который производит валидацию свойства. В данном случае этот параметр опущен.</p></li>
</ul>
<p>Первые три обязательных свойства довольно просты, поэтому подробнее остановимся на четвертом необязательном параметре, представляющем объект 
<span class="b">FrameworkPropertyMetadata</span>. Данный объект содержит ряд свойств для конфигурации свойства зависимостей:</p>
<ul>
<li><p><span class="b">AffectsArrange</span>: если имеет значение <code>true</code>, то свойство зависимостей будет влиять на процесс компоновки элемента</p></li>
<li><p><span class="b">AffectsMeasure</span>: если имеет значение <code>true</code>, то свойство зависимостей будет учитываться при установке размеров элемента при компоновке</p></li>
<li><p><span class="b">AffectsParentArrange</span>: если имеет значение <code>true</code>, то свойство зависимостей будет влиять на процесс компоновки в родительском элементе</p></li>
<li><p><span class="b">AffectsParentMeasure</span>: если имеет значение <code>true</code>, то свойство зависимостей будет учитываться при установке размеров родительского элемента при его компоновке</p></li>
<li><p><span class="b">AffectsRender</span>: если имеет значение <code>true</code>, то свойство зависимостей будет влиять на рендеринг и визуализацию элемента</p></li>
<li><p><span class="b">BindsTwoWayByDefault</span>: если имеет значение <code>true</code>, то свойство зависимостей будет использовать двустороннюю привязку данных</p></li>
<li><p><span class="b">CoerceValueCallback</span>: хранит ссылку на метод, который применяется для проверки допустимости значения до его валидации. Если 
значение не допустимо, то оно может корректироваться, чтобы соответствовать допустимым диапазонам.</p></li>
<li><p><span class="b">DefaultValue</span>: устанавливает значение по умолчанию для свойства зависимостей</p></li>
<li><p><span class="b">Inherits</span>: если имеет значение <code>true</code>, то вложенные элементы применительно к себе могут изменять значение свойства зависимостей. Например, если контейнер Windows задает свойство 
<code>FontSize</code>, то TextBlock автоматически подхватываетего  значение, если в нем самом это свойство не установлено</p></li>
<li><p><span class="b">IsAnimationProhibited</span>: если имеет значение <code>true</code>, то свойство зависимостей не применяется при анимации</p></li>
<li><p><span class="b">IsNotDataBindable</span>: если имеет значение <code>true</code>, то свойство зависимостей не будет поддерживать привязку данных</p></li>
<li><p><span class="b">Journal</span>: если имеет значение <code>true</code>, то значение свойства зависимостей будет журналироваться (сохраняться)</p></li>
<li><p><span class="b">PropertyChangedCallback</span>: хранит ссылку на метод, который вызывается при изменении значения свойства</p></li>
<li><p><span class="b">SubPropertiesDoNotAffectRender</span>: если имеет значение <code>true</code>, то элемент не будет перерисовываться, если если какое-то подсвойства у 
свойства зависимостей изменит свое значение</p></li>
</ul>
<p>В данном случае применяется один из конструкторов:</p>
<pre class="brush:c#;">
new FrameworkPropertyMetadata(string.Empty, 
	FrameworkPropertyMetadataOptions.AffectsMeasure | FrameworkPropertyMetadataOptions.AffectsRender, 
    new PropertyChangedCallback(OnTextChanged), new CoerceValueCallback(CoerceText)))
</pre>
<p>Этот конструктор устанавливает в качестве значени по умолчанию пустую строку, указывает, что при изменении значения элемент будет перерисовываться (собственно, 
что мы и видим - при изменении значения свойства Text новое значение отображается), и при изменении значения свойства будут вызываться методы OnTextChanged и CoerceText.</p>
<p>Далее после регистрации свойства идет обертка - обычное свойство .NET, которое имеет сеттер и геттер и которое вызывает методы <span class="b">GetValue</span> и 
<span class="b">SetValue</span> для получения и установки значения соответственно. Эти методы определены в классе <span class="b">System.Windows.DependencyObject</span>, 
который является базовым для всех элементов WPF, в том числе и для TextBlock.</p>
<p>Последнее обстоятельство привносит ограничение - свойства зависимостей могут быть определены только в тех классах, которые наследутся от DependencyObject.</p>
<p>Кроме того, DependencyObject поддерживает еще ряд свойств для управления свойствами зависимостей:</p>
<ul>
	<li><p><span class="b">ClearValue</span>: очищает значение объекта DependencyProperty</p></li>
	<li><p><span class="b">InvalidateProperty</span>: повторно вычисляет действующее значение объекта DependencyProperty</p></li>
	<li><p><span class="b">ReadLocalValue</span>: считывает значение объекта DependencyProperty</p></li>
</ul>
<p>Например:</p>
<pre class="brush:c#;">
...
<h3>Провайдеры свойств</h3>
<p>В WPF определение значения свойств представляет многоэтапный процесс. На различных стадиях этого процесса применяются различные провайдеры свойств, которые помогают получить значение 
для свойства зависимостей.</p>
<p>При извлечении значения свойства система использует 10 провайдеров:</p>
<ol>
<li><p>Получение локального значение свойства (то есть то, которое установлено разработчиком через XAML или через код C#)</p></li>
<li><p>Вычисление значения с помощью триггеров из шаблона родительского элемента</p></li>
<li><p>Вычисление значения из шаблона родительского элемента</p></li>
<li><p>Вычисление значения с помощью триггеров из применяемых стилей</p></li>
<li><p>Вычисление значения с помощью триггеров из применяемого шаблона</p></li>
<li><p>Получение значения из сеттеров применяемых стилей</p></li>
<li><p>Вычисление значения с помощью триггеров из применяемых тем</p></li>
<li><p>Получение значения из сеттеров применяемых тем</p></li>
<li><p>Получение унаследованного значения (если свойство <code>FrameworkPropertyMetadata.Inherits</code> имеет значение <code>true</code>)</p></li>
<li><p>Извлечение значения по умолчанию, которое устанавливается через объект FrameworkPropertyMetadata</p></li>
</ol>
<p>Все эти этапы выполняются последовательно сверху вниз. Если на одном этапе было получено значения, то этапы ниже уже не выполняются. 
...
представляет сложный многоэтапный процесс. И даже если в XAML для элемента не установлено значение какого-либо свойства, то все равно оно может иметь значение, полученное на одном из выше перечисленных шагов.</p>
<p>Все десять перечисленных этапов обычно объединяются в одну стадию - <span class="b">получение базового значения</span>.</p>
<p>Но кроме получения значения есть еще процесс установки значения. Он вовлекает ряд дополнительных шагов:</p>
<ol>
<li><p>Вышеописанные 10 шагов - получение базового значения</p></li>
<li><p>Если значение свойства, полученное на шаге 1, представляет собой сложное выражение (например, выражение привязки данных), то WPF вычисляет значение этого выражения и получает конкретный результат</p></li>
<li><p>Если для свойства применяется анимация, то далее она используется для получения нового значения</p></li>
<li><p>После получения значения WPF применяет делегат <span class="b">CoerceValueCallback</span>, который задается в объекте <code>FrameworkPropertyMetadata</code> 
...
задается новое значение</p></li>
<li><p>В конце применяется делегат <span class="b">ValidateValueCallback</span> (если он указан при регистрации свойства в качестве пятого параметра), 
который выполняет валидацию. Метод, на который ссылается делегат, возвращает <code>true</code> при прохождении валидации. Иначе возвращается 
<code>false</code> и генерируется исключение</p></li>
</ol>
...
<div class="nav"><p><a href="5.17.php">Назад</a><a href="./">Содержание</a><a href="13.2.php">Вперед</a></p></div></div>
<div class="socBlock">
