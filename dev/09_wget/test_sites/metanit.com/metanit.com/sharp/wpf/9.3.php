...
<html >
<head>
<title>C# и WPF | Словари ресурсов</title>
<meta charset="utf-8" />
<meta name="description" content="Определение словарей ресурсов в WPF, элемент ResourceDictionary, подключением словаря ресурсов в коде XAML и C#">
...
<h2>Словари ресурсов</h2><div class="date">Последнее обновление: 27.01.2016</div>
...
<p>Мы можем определять ресурсы на уровне отдельных элементов окна, например, как ресурсы элементов Window, Grid и т.д. 
Однако есть еще один способ определения ресурсов, который предполагает использование <span class="b">словаря ресурсов</span>.</p>
<p>Нажмем правой кнопкой мыши на проект и в контекстном меню выберем <span class="b">Add -&gt; New Item...</span>, И в окне добавления выберем пункт <span class="b">Resource Dictionary (WPF)</span>:</p>
<img src="./pics/9.3.png" alt="Добавление словаря ресурсов в WPF" />
<p>Оставим у него название по умолчанию Dictionary1.xaml и нажмем на кнопку OK.</p>
<p>После этого в проект добавляется новый файл. Он представляет собой обычный xaml-файл с одним корневым элементом <i>ResourceDictionary</i>:</p>
<pre class="brush:xml;">
...
<p>Изменим его код, добавив какой-нибудь ресурс:</p>
<pre class="brush:xml;">
&lt;ResourceDictionary xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
                    xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
                    xmlns:local="clr-namespace:ResourcesApp"&gt;
...
<p>После определения файла ресурсов его надо подсоединить к ресурсам приложения. Для этого откроем файл <i>App.xaml</i>, который есть в проекте по умолчанию и 
изменим его:</p>
<pre class="brush:xml;">
...
<p>Элемент <code>ResourceDictionary.MergedDictionaries</code> здесь представляет колекцию объектов <code>ResourceDictionary</code>, то есть словарей ресурсов, которые добавляются 
к ресурсам приложения. Затем в любом месте приложения мы сможем сослаться на этот ресурс:</p>
<pre class="brush:xml;">&lt;Button Content="OK" MaxHeight="40" MaxWidth="80" Background="{StaticResource buttonBrush}" /&gt;</pre>
<p>При этом одновременно мы можем добавлять в коллекцию ресурсов приложения множество других словарей или параллельно с ними определять еще какие-либо ресурсы:</p>
<pre class="brush:xml;">
&lt;Application x:Class="ResourcesApp.App"
             xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
             xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<h3>Загрузка словаря ресурсов</h3>
<p>Нам необязательно добавлять словарь ресурсов через ресурсы приложения. У объекта ResourceDictionary имеется свойство <code>Source</code>, 
через которое мы можем связать ресурсы конкретного элемента со словарем:</p>
<pre class="brush:xml;">
...
<p>Также мы можем загружать словарь динамически в коде C#. Так, загрузим в коде C# вышеопределенный словарь:</p>
<pre class="brush:c#;">this.Resources = new ResourceDictionary() { Source = new Uri("pack://application:,,,/Dictionary1.xaml") };</pre>
<p>При динамической загрузке, если мы определяем ресурсы через xaml, то они должны быть динамическими:</p>
<pre class="brush:xml;">&lt;Button Content="OK" MaxHeight="40" MaxWidth="80" Background="{DynamicResource buttonBrush}" /&gt;</pre>
...
<div class="nav"><p><a href="9.2.php">Назад</a><a href="./">Содержание</a><a href="11.php">Вперед</a></p></div></div>
<div class="socBlock">
