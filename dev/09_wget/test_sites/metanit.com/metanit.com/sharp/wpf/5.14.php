...
<html >
<head>
<title>C# и WPF | DataGrid</title>
<meta charset="utf-8" />
<meta name="description" content="Использование элемента DataGrid для создания таблиц в WPF, определение его столбцов, заполнение данными и взаимодействие с коллекциями, параметры отображения столбцов и строк">
...
<h2>DataGrid</h2><div class="date">Последнее обновление: 19.01.2016</div>
...
<p>DataGrid во многом похож на ListView, но более сложный по характеру и допускает редактирование содержимого таблицы.</p>
<p>В разделе о ListView мы создали класс Phone, объекты которого выводили в список:</p>
<pre class="brush: c#;">
public class Phone
{
...
<p>Теперь же выведем объекты в таблицу DataGrid. Чтобы DataGrid автоматически разбивал таблицу на столбцы, установим свойство 
<code>AutoGenerateColumns="True"</code>:</p>
<pre class="brush:xml;">
...
<p>В данном случае префикс <code>local</code> ссылается на пространство имен текущего проекта, в котором определен класс Phone 
(xmlns:local="clr-namespace:Controls"), а <code>col</code> - префикс-ссылка на пространство имен System.Collections 
(xmlns:col="clr-namespace:System.Collections;assembly=mscorlib"). И это даст в итоге следующий вывод:</p>
<img src="./pics/4.26.png" alt="Элемент DataGrid в WPF" />
<p>Программная установка источника для DataGrid:</p>
<pre class="brush:c#;">
...
<table class="tab">
<tr><td><p><span class="b">RowBackground</span> и <span class="b">AlternatingRowBackground</span></p></td>
<td><p>Устанавливают фон строки. Если установлены оба свойства, цветовой фон чередуется: <span class="b">RowBackground</span> - для нечетных строк 
и <span class="b">AlternatingRowBackground</span> - для четных</p></td>
</tr>
<tr><td><p><span class="b">ColumnHeaderHeight</span></td><td><p>Устанавливает высоту строки названий столбцов.</p></td></tr>
<tr><td><p><span class="b">ColumnWidth</span></p></td><td><p>Устанавливает ширину столбцов.</p></td></tr>
<tr><td><p><span class="b">RowHeight</span></p></td><td><p>Устанавливает высоту строк.</p></td></tr>
<tr><td><p><span class="b">GridLinesVisibility</span></p></td><td><p>Устанавливает видимость линий, разделяющих столбцы и строки. 
Имеет четыре значения - <code>All</code> - видны все линии, <code>Horizontal</code> - видны только горизонтальные линии, <code>Vertical</code> - видны только вертикальные линии, <code>None</code> - линии отсутствуют</p></td></tr>
<tr><td><p><span class="b">HeadersVisibility</span></p></td><td><p>Задает видимость заголовков</p></td></tr>
<tr><td><p><span class="b">HorizontalGridLinesBrush</span> и <span class="b">VerticalGridLinesBrush</span></p></td><td><p>Задает цвет горизонтальных и вертикальных линий соответственно</p></td></tr></table>
<p>Хотя предыдущий пример довольно прост, в нем есть несколько недочетов. Во-первых, у нас нет возможности повлиять на расстановку столбцов. 
Во-вторых, заголовки определены по названиям свойств, которые на английском языке, а хотелось бы на русском. В этом случае 
мы должны определить свойства отображения столбцов сами. Для этого надо воспользоваться свойством <span class="b">DataGrid.Columns</span> 
и определить коллекцию столбцов для отображения в таблице.</p>
<p>Причем можно задать также и другой тип столбца, отличный от текстового. DataGrid поддерживает следующие варианты столбцов:</p>
<table class="tab">
<tr><td><p>DataGridTextColumn</p></td><td><p>Отображает элемент TextBlock или TextBox при редактировании</p></td></tr>
<tr><td><p>DataGridHyperlinkColumn</p></td><td><p>Представляет гиперссылку и позволяет переходить по указанному адресу</p></td></tr>
<tr><td><p>DataGridCheckBoxColumn</p></td><td><p>Отображает элемент CheckBox</p></td></tr>
<tr><td><p>DataGridComboBoxColumn</p></td><td><p>Отображает выпадающий список - элемент ComboBox</p></td></tr>
<tr><td><p>DataGridTemplateColumn</p></td><td><p>Позволяет задать специфичный шаблон для отображения столбца</p></td></tr>
</table>
<p>Перепишем предыдущий пример с учетом новой информации:</p>
<pre class="brush:xml;">
...
<img src="./pics/4.27.png" />
<p>Среди свойств DataGrid одним из самых интересных является <span class="b">RowDetailsTemplate</span>. Оно позволяет задать шаблон отображения 
дополнительной информации касательно данной строки. Измени элемент DataGrid:</p>
<pre class="brush:xml;">
&lt;DataGrid x:Name="phonesGrid" AutoGenerateColumns="False" HorizontalGridLinesBrush="DarkGray"
    RowBackground="LightGray" AlternatingRowBackground="White"&gt;
...
<img src="./pics/4.28.png" alt="DataGrid.RowDetailsTemplate в WPF" />
