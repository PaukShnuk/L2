...
<html >
<head>
<title>C# и WPF | Основы трехмерной графики</title>
<meta charset="utf-8" />
<meta name="description" content="Создание трехмерной графики в WPF, основные моменты, особенности работы с классом Viewport3D">
...
<h1>Трехмерная графика</h1><h2>Основы работы с трехмерной графикой</h2><div class="date">Последнее обновление: 13.03.2016</div>
...
<p>Работать с графикой в принципе очень интересно, но работать с трехмерной - особенно интересно. И WPF предлагает нам удобный инструментарий для этого. 
...
возможности WPF вполне сойдут.</p>
<p>Для создания трехмерной сцены в приложении WPF требуется несколько компонентов:</p>
<ul>
<li><p><span class="b">Окно просмотра (Viewport3D)</span>, которое и содержит трехмерную сцену.</p></li>
<li><p><span class="b">Сам объект или геометрия</span>.</p></li>
<li><p><span class="b">Камера</span>, которая устанавливает, как сцена или объект будет отображаться.</p></li>
<li><p><span class="b">Освещение</span>, которое и содержит трехмерную сцену.</p></li>
<li><p><span class="b">Материал</span>, который вместе с освещением определяет внешний вид трехмерного объекта.</p></li>
</ul>
<p>Итак, контейнером верхнего уровня для трехмерной сцены является объект <span class="b">Viewport3D</span>. Формально это 
такой же объект со всеми свойствами, как кнопка или текстовое поле, которое мы можем позиционировать на форме как угодно.</p>
<p>Для отображения трехмерной сцены в Viewport3D надо установить камеру с помощью свойства <span class="b">Camera</span>. Без камеры мы не сможем лицезреть трехмерные 
объекты.</p>
<p>Для добавления самих трехмерных объектов в Viewport3D предусмотрено свойство <span class="b">Children</span>, в котором 
определяются один или несколько объектов <span class="b">ModelVisual3D</span>. А каждый из этих объектов ModelVisual3D, который добавляется в Viewport3D, 
имеет свойство <span class="b">Content</span>. Это свойство принимает в качестве значения один объект <span class="b">GeometryModel3D</span>, либо объект 
<span class="b">Model3DGroup</span>, который содержит группу объектов <span class="b">GeometryModel3D</span>, которые уже непосредственно устанавливают форму трехмерного объекта.</p>
<p>Рассмотрим использование Viewport3D и создание простейшей трехмерной фигуры:</p>
<pre class="brush:xml;">
...
<p>Фактически в этом коде мы можем выделить четыре узловые части:</p>
<ol>
<li><p>Определение камеры. Без камеры мы не сможем увидеть объект</p></li>
<li><p>Определение источника света.</p></li>
<li><p>Определение самого трехмерного объекта</p></li>
<li><p>Определение материала (цвета) этого объекта</p></li>
</ol>
<p>В итоге мы получим треугольник:</p>
<img src="./pics/18.1.png" alt="Трехмерная сцена и Viewport3D в WPF" />
<p>Нарисованный нами треугольник вряд ли можно назвать трехмерным, но на самом деле так и есть. Треугольник является примитивом, из которых состоят 
...
сложным объектам.</p>
<p>Среди свойств Viewport3D следует отметить свойство <span class="b">ClipToBounds</span>, которое по умолчанию равно <code>True</code>. Из-за этого 
...
<code>ClipToBounds</code>, то трехмерный объект накладыватся поверх смежных элементов. Данный эффект можно увидеть, если, например, задать отступ для Viewport3D:</p>
<pre class="brush:xml;">
&lt;Grid&gt;
    &lt;Viewport3D ClipToBounds="False" Margin="0 40 0 0"&gt;
</pre>
<p>Теперь рассмотрим по отдельности все части определени трехмерной сцены.</p>
...
<div class="nav"><p><a href="17.4.php">Назад</a><a href="./">Содержание</a><a href="18.3.php">Вперед</a></p></div></div>
<div class="socBlock">
