...
<html >
<head>
<title>C# и WPF | Взаимодействие команд и событий</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие команд и событий в паттерне MVVM в WPF, использование триггеров событий для вызова команд">
...
<h2>Взаимодействие команд и событий</h2><div class="date">Последнее обновление: 18.10.2016</div>
...
<p>Кнопка поддерживает команды, и вместо обработки события Click мы можем прикрепить к ней команду. Но что делать, если элемент не поддерживает команду, а нам 
надо обработать какое-то его действие.</p>
<p>Например, у нас определен элемент ListBox, и мы хотим отслеживать выбор объекта в списке. Если бы мы использовали событийную модель, то 
мы бы обрабатывали событие <code>SelectionChanged</code>. Но мы знаем, что выделение объекта в списке ведет к изменению свойства <code>SelectedItem</code> 
...
к свойству <code>SelectedItem</code> у ListBox.</p>
<p>Аналогично у элемента TextBox есть событие <code>TextChanged</code>, которое вызывается при изменении вводимого текста. Но опять же изменение текста приводит к изменению свойства <code>Text</code> 
у элемента TextBox. Поэтому мы могли бы установить привязку для этого элемента к свойству Text:</p>
<pre class="brush:xml;">&lt;TextBox Text="{Binding Title, UpdateSourceTrigger=PropertyChanged}" /&gt;</pre>
<p>А в классе Phone (либо в специальной ViewModel, которая инкапсулирует объект Phone) обрабатывать изменение свойства Title, которое бы синхронно менялось при вводе новых символов в текстовое поле:</p>
<pre class="brush:c#;">
public string Title
{
...
<p>В тоже время подобные действия можно установить не для всех событий. И если мы хотим связать события с командами, необходимо использовать дополнительный функционал.</p>
<p>В частности, нам надо добавить в проект специальную библиотеку <span class="b">System.Windows.Interactivity.dll</span>:</p>
<img src="./pics/22.5.png" alt="Команды и события в WPF" />
<p>Затем добавим в класс ApplicationViewModel новую команду:</p>
<pre class="brush:c#;">
...
<p>Смысл этой команды заключается в добавлении копии объекта в список.</p>
<p>Далее добавим в представление кнопку, которая будет вызывать данную команду. Но, допустим, мы хотим выполнять эту команду, если по кнопке был совершен 
двойной щелчок, то есть если произошло событие <code>MouseDoubleClick</code>:</p>
<pre class="brush:xml;">
...
<p>Для добавленной кнопки устанавливается свойство <code>Interaction.Triggers</code>, которое позволяет связать триггеры событий с командами и передать этим командам 
параметры:</p>
<pre class="brush:c#;">
&lt;Button Content="2x"&gt;
    &lt;i:Interaction.Triggers&gt;
...
<p>Теперь по двойному нажатию на эту кнопку произойдет дублирование в списке выделенного элемента:</p>
<img src="./pics/22.6.png" alt="EventTrigger и Command в WPF" />
...
<div class="nav"><p><a href="22.4.php">Назад</a><a href="./">Содержание</a><a href="22.6.php">Вперед</a></p></div></div>
<div class="socBlock">
