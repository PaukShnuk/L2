...
<html >
<head>
<title>C# и WPF | Прикрепляемые свойства</title>
<meta charset="utf-8" />
<meta name="description" content="Прикрепляемые свойства (attached properties) в WPF, их определение и использование">
...
<h2>Прикрепляемые свойства</h2><div class="date">Последнее обновление: 24.01.2016</div>
...
<p>Прикрепляемые свойства (attached properties) также являются свойствами зависимостей с той разницей, что они определяются в одном классе, а применяются 
в другом. Например, при установке столбца или строки грида, в которых размещается элемент управления, используются свойства <code>Grid.Row</code> и 
<code>Grig.Column</code>, которые как раз и представляют прикрепляемые свойства. То есть эти свойства определены в классе Grid, но используются в других 
вложенных элементах:</p>
<pre class="brush:xml;">
&lt;Grid&gt;
    &lt;Grid.RowDefinitions&gt;
        &lt;RowDefinition /&gt;
...
<p>В коде C# установка значения для прикрепленных свойств производится с помощью статических методов в формате <code>Класс.Set[Свойство]()</code>. 
Например, установим для выше определенной кнопки столбец и строку грида:</p>
<pre class="brush:c#;">
Grid.SetRow(button1, 1); // вторая строка
Grid.SetColumn(button1, 1); // второй столбец
</pre>
<p>С помощью методов типа <code>Класс.GetСвойство()</code> мы можем получать в коде значения прикрепляемых свойств:</p>
<pre class="brush:c#;">
int column = Grid.GetColumn(button1); // получаем номер столбца
</pre>
<p>Если более детально взглянуть на прикрепляемые свойства, то можно увидеть, что их определение немного отличается от стандартных свойств зависимостей. 
Для регистрации прикрепляемого свойства применяется метод <code>RegisterAttached()</code>:</p>
<pre class="brush:c#;">
...
<p>Метод <code>RegisterAttached()</code> принимает в прицнипе те же параметры, что и метод <code>Register()</code>.</p>
<p>Другое отличие от обычных свойств зависимостей состоит в том, что для прикрепляемых свойств не создается обертка в виде стандартного свойства C#. 
Вместо этого используется пара статических методов <code>SetСвойство()</code> <code>GetСвойство()</code>:</p>
<pre class="brush:c#;">
...
<p>В принципе выше мы уже рассмотрели их применение на примере Grid.SetColumn/Grid.SetRow и Grid.GetColumn.</p>
...
<div class="nav"><p><a href="13.php">Назад</a><a href="./">Содержание</a><a href="13.3.php">Вперед</a></p></div></div>
<div class="socBlock">
