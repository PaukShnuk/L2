...
<html >
<head>
<title>C# и WPF | Подключение к базе данных</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение к базе данных в приложении WPF, работа с ADO.NET и MS SQL Server, особенности добавления, удаления и редактирования данных, CRUD-интерфейс">
...
<h2>Подключение к базе данных</h2><div class="date">Последнее обновление: 28.02.2016</div>
...
<p>В прошлой теме была создана база данных, теперь подключимся к ней из приложения. В любом проекте WPF, как и в ряде других типов проектов для .NET, по 
умолчанию есть файл конфигурации, который называется <i>app.config</i> и который имеет следующее содержимое:</p>
<pre class="brush:xml;">
...
<p>Добавим в него строку подключения к бд, изменив файл следующим образом:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;configuration&gt;
    &lt;startup&gt; 
...
<p>Для определения всех подключений в программе в пределах узла <code>&lt;configuration&gt;</code> добавляется новый узел <code>&lt;connectionStrings&gt;</code>. 
В этом узле определяются строки подключения с помощью элемента <code>&lt;add&gt;</code>. Каждая строка подключения имеет название, определяемое с помощью 
атрибута <code>name</code>. В данном случае строка подключения называется "DefaultConnection". Название может быть произвольное.</p>
<p>Атрибут connectionString собственно хранит строку подключения. Он состоит из трех частей:</p>
<ul>
<li><p><code>Data Source=.\SQLEXPRESS</code>: указывает на название сервера. По умолчанию для MS SQL Server Express используется ".\SQLEXPRESS"</p></li>
<li><p><code>Initial Catalog=mobiledb</code>: название базы данных. Так как база данных называется mobiledb, то соответственно здесь данное название и указываем</p></li>
<li><p><code>Integrated Security=True</code>: задает режим аутентификации</p></li>
</ul>
<p>Так как мы будем подключаться к базе данных MS SQL Server, то соответственно мы будем использовать провайдер для SQL Server, функциональность которого заключена в пространстве имен System.Data.SqlClient.</p>
<p>Далее определим код графического интерфейса в xaml:</p>
<pre class="brush:xml;">
...
<p>Здесь определен довольно простой интерфейс: датагрид для отображения данных, и две кнопки для обновления данных в бд и для удаления. В итоге приложение 
будет выглядеть следующим образом:</p>
<img src="./pics/19.8.png" />
<p>Теперь определим код подключения и все обработчики кнопок в файле кода c#:</p>
<pre class="brush:c#;">
using System;
...
<p>Вся работа с бд производится стандартными средствами ADO.NET и прежде всего классом SqlDataAdapter. Вначале мы получаем в конструкторе строку 
подключения, которая определена выше в файле <i>app.config</i>:</p>
<pre class="brush:c#">connectionString = ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString;</pre>
<p>Чтобы задействовать эту функциональность, нам надо добавить в проект библиотеку <span class="b">System.Configuration.dll</span>.</p>
<p>Далее в обработчике загрузки окна <code>Window_Loaded</code> создаем объект SqlDataAdapter:</p>
<pre class="brush:c#;">adapter = new SqlDataAdapter(command);</pre>
<p>В качестве команды для добавления объекта устанавливаем ссылку на хранимую процедуру:</p>
<pre class="brush:c#;">adapter.InsertCommand = new SqlCommand("sp_InsertPhone", connection);</pre>
<p>Получаем данные из БД и осуществляем привязку:</p>
<pre class="brush:c#;">
adapter.Fill(phonesTable);
phonesGrid.ItemsSource = phonesTable.DefaultView;
</pre>
<p>За обновление отвечает метод <code>UpdateDB()</code>:</p>
<pre class="brush:c#;">
private void UpdateDB()
{
    SqlCommandBuilder comandbuilder = new SqlCommandBuilder(adapter);
...
<p>Чтобы обновить данные через SqlDataAdapter, нам нужна команда обновления, которую можно получить с помощью объекта SqlCommandBuilder. Для самого 
обновления вызывается метод <code>adapter.Update()</code>.</p>
<p>Причем не важно, что мы делаем в программе - добавляем, редактируем или удаляем строки. Метод adapter.Update сделает все необходимые действия. Дело в том, 
что при загрузке данных в объект DataTable система отслеживает состояние загруженных строк. В методе <code>adapter.Update()</code> состояние строк используется 
...
здесь: <a href="http://metanit.com/sharp/adonet/3.4.php">Обновление БД из DataSet вручную</a></p>
<p>В обработчике кнопки обновления просто вызывается этот метод UpdateDB, а в обработчике кнопки удаления предварительно удаляются все выделенные строки.</p>
<p>Таким образом, мы можем вводить в DataGrid новые данные, редактировать там же уже существующие, сделать множество изменений, и после этого нажать на кнопку обновления, 
и все эти изменения синхронизируются с базой данных.</p>
<p>Причем важно отметить действие хранимой процедуры - при добавлении нового объекта данные уходят на сервер, и процедура возвращает нам id добавленной записи. 
Этот id играет большую роль при генерации нужного sql-выражения, если мы захотим эту запись изменить или удалить. И если бы не хранимая процедура, то нам пришлось бы 
после добавления данных загружать заново всю таблицу в datagrid, только чтобы у новой добавленной записи был в datagrid id. И хранимая процедура избавляет нас от этой работы.</p>
<p>Также здесь мы могли бы выполнять обновление данных сразу после редактирования строки. Для этого нужно задействовать событие <code>RowEditEnding</code> элемента 
DataGrid:</p>
<pre class="brush:c#;">
public MainWindow()
{
...
<p>И если после окончания редактирования мы нажмем на Enter, то срабатает обработчик события RowEditEnding, который обновит базу данных.</p>
<p>Итак, здесь рассмотрен простейший способ работы с базой данных в WPF. Далее мы рассмотрим еще один способ, который подразумевает применение технологии 
Entity Framework.</p>
...
<div class="nav"><p><a href="19.1.php">Назад</a><a href="./">Содержание</a><a href="19.3.php">Вперед</a></p></div></div>
<div class="socBlock">
