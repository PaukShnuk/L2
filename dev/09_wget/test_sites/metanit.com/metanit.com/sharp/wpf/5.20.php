...
<html >
<head>
<title>C# и WPF | ScrollViewer. Создание прокрутки</title>
<meta charset="utf-8" />
...
<h2>ScrollViewer. Создание прокрутки</h2><div class="date">Последнее обновление: 18.01.2016</div>
...
<p>Элемент ScrollViewer обеспечивает прокрутку содержимого. Может вмещать в себя только один элемент, поэтому все элементы, помещаемые внутрь ScrollViewer необходимо облачить в еще один контейнер. 
Например:</p>
<pre class="brush:xml;" >
...
<img src="./pics/4.43.png" alt="ScrollViewer in WPF" />
<p>ScrollViewer поддерживает как вертикальную, так и горизонтальную прокрутку. Ее можно установить с помощью свойств 
<span class="b">HorizontalScrollBarVisibility</span> и <span class="b">VerticalScrollBarVisibility</span>. Эти свойства принимают одно из следующих значений:</p>
<ul>
...
<p>Среди свойств нужно отметить еще <span class="b">CanContentScroll</span>. Если оно установлено в True, то прокрутка осуществляется не на несколько пикселей, а к началу следующего элемента.</p>
<p>Кроме того, прокрутку можно организовать программным способом - с помощью следующих методов элемента ScrollViewer:</p>
<ul>
<li><p><span class="b">LineUp(), LineDown(), LineRight(), LineLeft()</span>: прокрутка соответственно вверх, вниз, вправо, влево.</p></li>
<li><p><span class="b">ScrollToEnd(), ScrollToHome()</span>: прокрутка в конец окна и в начало.</p></li>
<li><p><span class="b">ScrollToRightEnd(), ScrollToLeftEnd()</span>: прокрутка в правый и левый конец окна.</p></li>
</ul>
<p>В качестве примера обернем несколько элементов RadioButton в элемент ScrollViewer:</p>
<pre class="brush:xml;">
&lt;Window x:Class="ControlsApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<p>А в файле кода C# пропишем обработчики кнопок, которые будут выполнять программно прокрутку:</p>
<pre class="brush:c#;">
...
<img src="./pics/4.11.png" alt="ScrollViewer в WPF" />
