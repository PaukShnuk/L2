...
<html >
<head>
<title>C# и WPF | Команды в MVVM</title>
<meta charset="utf-8" />
<meta name="description" content="Определение команд и их использование в паттерне MVVM в WPF, связь с представлением">
...
<h2>Команды в MVVM</h2><div class="date">Последнее обновление: 18.10.2016</div>
...
<p>Для взаимодействия пользователя и приложения в MVVM используются команды. Это не значит, что вовсе не можем использовать события и событийную модель, однако везде, где возможно, вместо 
событий следует использовать команды.</p>
<p>В WPF команды представлены интерфейсом ICommand:</p>
<pre class="brush:c#;">
...
<p>Однако WPF имеет в качестве реализации этого интерфейса имеет класс <code>System.Windows.Input.RoutedCommand</code>, который ограничен по функциональности. 
Поэтому, как правило, придется реализовывать свои собственные команды с помощью реализации ICommand.</p>
<p>Для использования команд продолжим работу с проектом из прошлой темы и добавим в него новый класс, который назовем RelayCommand:</p>
<pre class="brush:c#;">
using System;
...
<p>Класс реализует два метода:</p>
<ul>
<li><p><span class="b">CanExecute</span>: определяет, может ли команда выполняться</p></li>
<li><p><span class="b">Execute</span>: собственно выполняет логику команды</p></li>
</ul>
<p>Событие <span class="b">CanExecuteChanged</span> вызывается при изменении условий, указывающий, может ли команда выполняться. Для этого используется 
событие <code>CommandManager.RequerySuggested</code>.</p>
<p>Ключевым является метод Execute. Для его выполнения в конструкторе команды передается делегат типа <code>Action&lt;object&gt;</code>. При этом 
класс команды не знает какое именно действие будет выполняться. Например, мы можем написать так:</p>
<pre class="brush:c#;">
var cmd = new RelayCommand(o =&gt; { MessageBox.Show("Команда" + o.ToString()); });
cmd.Execute("1");
</pre>
<p>В результате вызова команды будет выведено окно с надписью "Команда1". Но мы могли также передать любое другое действие, которое бы соответствовало 
делегату <code>Action&lt;object&gt;</code>.</p>
<h3>Применение команд</h3>
<p>Для ряда визуальных элементов WPF, например, для кнопок, определена поддержка команд. Однако сами команды определяются в ViewModel и затем через 
механизм привязки устанавливаются для элементов управления. Например, изменим код ApplicationViewModel следующим образом:</p>
<pre class="brush:c#;">
...
<p>Здесь добавлена команда на добавление объекта:</p>
<pre class="brush:c#;">
private RelayCommand addCommand;
public RelayCommand AddCommand
{
...
<p>Команда хранится в свойстве AddCommand и представляет собой объект выше определенного класса RelayCommand. Этот объект в конструкторе принимает действие - делегат 
Action&lt;object&gt;. Здесь действие представлено в виде лямбда-выражения, которое добавляет в коллекцию Phones новый объект Phone и устанавливает его в качестве выбранного.</p>
<p>Используем эту команду. Для этого изменим код представления в <span class="ii">MainWindow.xaml</span>:</p>
<pre class="brush:xml;">
...
<p>Здесь добавлена кнопка, свойство <code>Command</code> которой приязано к свойству AddCommand объекта ApplicationViewModel:</p>
<pre class="brush:xml;">&lt;Button Command="{Binding AddCommand}"&gt;+&lt;/Button&gt;</pre>
<p>И нам не надо писать никаких обработчиков нажатия. Автоматически при нажатии на кнопку сработает команда, которая добавит в список еще один объект. А код 
в файле <span class="ii">MainWindow.xaml.cs</span> остается прежним:</p>
<pre class="brush:c#;">
...
<p>И при нажатии на кнопку в список будет добавлен новый объект, который мы сразу сможем отредактировать в текстовых полях справа:</p>
<img src="./pics/22.3.png" alt="Команды в паттерне MVVM в WPF" />
