...
<html >
<head>
<title>C# и WPF | Анимации по ключевым кадрам</title>
<meta charset="utf-8" />
<meta name="description" content="Покадровая анимация в WPF, определение кадров и элемент KeyFrame, примеры анимаций">
...
<h2>Анимации по ключевым кадрам</h2><div class="date">Последнее обновление: 04.03.2016</div>
...
Есть еще второй род анимаций - анимации по ключевым кадрам, которые анимирует те же свойства, что и обычные анимации. Например, 
<span class="b">ColorAnimationUsingKeyFrames</span> анимирует цвет, а <span class="b">DoubleAnimationUsingKeyFrames</span> - свойства, представляющие тип double. </p>
<p>Анимации по ключевым кадрам имеет практически все те же свойства, что и обычные анимации. Основное отличие - покадровые анимации имеют свойство <span class="b">KeyFrames</span>, которое 
позволяет установить коллекцию кадров. 
Каждый кадр (KeyFrame) определяет значение для анимируемого свойства с помощью свойства <span class="b">Value</span> и продолжительность кадра с помощью 
свойства <span class="b">KeyTime</span></p>
<pre class="brush: xml;">
...
<p>Тип кадра анимации определяется следующим образом: тип анимации задает тип изменяемого значения, например, <code>ColorAnimationUsingKeyFrames</code> 
изменяемое значение типа <code>Color</code>. Тип кадра состоит из трех частей: вид кадра, например, линейный (Linear), тип изменяемого значения, и 
ключевого слова <code>KeyFrame</code>. То есть выше тип кадра был <code>LinearColorKeyFrame</code> - линейный кадр, изменяющий значение Color.</p>
<p>Свойство <code>KeyTime</code> позволяет задать время, с которого начинается кадр. Например, <code>KeyTime="66%"</code> - действие кадра начинается, когда будет пройдено 
66% времени анимации. Или <code>KeyTime="0:0:2"</code> - кадр начинается со 2 секунды.</p>
<p>Кроме линейных есть еще несколько видов кадров анимации:</p>
<ul>
<li><p>Дискретные кадры, например, <span class="b">DiscreteColorKeyFrame</span>, они обеспечивают резкую смену кадров.</p></li>
<li><p>Кадры, на основе сплайнов, например, <span class="b">SplineDoubleKeyFrame</span>. В этом случае анимация строиться на основе кривой Безье. 
Свойство <span class="b">KeySpline</span> определяет начальную и конечную точки участка сплайна. При чем начальная точка сплайна находится в начале координат (0,0), а конечная - в конце (1,1). 
Получаемый сплайн является отношением между временем (координата X) и анимируемым значением (координата Y). </p></li>
</ul>
<p>Например, используем сплайновую анимацию:</p>
<pre class="brush:xml;">
&lt;DoubleAnimationUsingKeyFrames 
		Storyboard.TargetProperty="(Canvas.Bottom)"
		Duration="0:0:3" AutoReverse="True" 
