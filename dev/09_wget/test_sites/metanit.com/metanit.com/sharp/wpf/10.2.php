...
<html >
<head>
<title>C# и WPF | Триггеры</title>
<meta charset="utf-8" />
<meta name="description" content="Триггеры в WPF, их виды, триггеры свойств, данных, событий, установка обработчиков событий, выполнение анимации">
...
<h2>Триггеры</h2><div class="date">Последнее обновление: 07.02.2016</div>
...
<p>Тригерры позволяют декларативно задать некоторые действия, которые выполняются при изменении свойств стиля. Существует три вида триггеров:</p>
<ul>
<li><p><span class="b">Триггеры свойств</span>: вызываются в ответ на изменения свойствами зависимостей своего значения</p></li>
<li><p><span class="b">Триггеры данных</span>: вызываются в ответ на изменения значений любых свойств (они необязательно должны быть свойствами зависимостей)</p></li>
<li><p><span class="b">Триггеры событий</span>: вызываются в ответ на генерацию событий</p></li>
<li><p><span class="b">Мультитриггеры</span>: вызываются при выполнении ряда условий</p></li>
</ul>
<h3>Триггеры свойств</h3>
<p>Простые триггеры свойств задаются с помощью объекта <span class="b">Trigger</span>.  Они следят за значением свойств и в случае их изменения с помощью 
объекта <span class="b">Setter</span> устанавливают значение других свойств.</p>
<p>Например, в следующем примере по наведению на кнопку высота шрифта устанавливается в 14, 
а цвет шрифта становится красным:</p>
<pre class="brush:xml;">
...
<img src="./pics/10.2.png" alt="Триггеры в WPF" />
<p>Здесь объект Trigger применяет два свойства: Property и Value. Свойство Property указывает на отслеживаемое свойство, а свойство Value указывает на 
значение, по достижении которого триггер начнет действовать. Например, в данном случае триггер отслеживает свойство <code>IsMouseOver</code> - если оно будет 
равно <code>true</code>, тогда сработает триггер.</p>
<p>Trigger имеет вложенную коллекцию Setters, в которой можно определить сеттеры, реализующие логику триггера.</p>
<h3>MultiTrigger</h3>
<p>При необходимости отслеживания не одного, а сразу нескольких свойств используют объект <span class="b">MultiTrigger</span>. 
Он содержит коллекцию элементов <span class="b">Condition</span>, каждый из которых, как и обычный триггер, определяет отслеживаемое свойство и его 
значение:</p>
<pre class="brush:xml;">
&lt;Window x:Class="TriggersApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
...
<p>В данном случае если удовлетворены одновременно оба условия:</p>
<pre class="brush:xml;">
&lt;Condition Property="IsMouseOver" Value="True" /&gt;
&lt;Condition Property="IsPressed" Value="True" /&gt;
</pre>
<p>то срабатывают сеттеры мультитриггера:</p>
<pre class="brush:xml;">
&lt;Setter Property="FontSize" Value="14" /&gt;
&lt;Setter Property="Foreground" Value="Red" /&gt;
</pre>
<h3>EventTrigger</h3>
<p>Если простой триггер наблюдает за изменением свойства, то EventTrigger реагирует на определенные события совсем как обработчики событий. Правда, 
триггеры событий более ограничены в своих возможностях.</p>
<p>EventTrigger определяет анимацию и, если событие происходит, запускает ее на выполнение.</p>
<pre class="brush:xml;">
&lt;Window x:Class="TriggersApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<p>В данном случае свойство <span class="b">RoutedEvent</span> задает событие, на которое подписывается объект EventTrigger. Затем определяется свойство 
<code>EventTrigger.Actions</code>, которое задает анимацию, производимую в случае возникновения события.</p>
<p>Объект <span class="b">BeginStoryboard</span> начинает анимацию, которая задается объектом <span class="b">Storyboard</span>. Сама анимация 
определяется в объекте <span class="b">DoubleAnimation</span>. Его свойство <code>Storyboard.TargetProperty</code> указывает на свойство элемента, 
изменяемое в процессе анимации, <span class="b">Duration</span> задает время анимации, а свойство <span class="b">To</span> - финальное значение свойства, 
на котором анимация заканчивается.</p>
<p>Суть данной анимации - значение свойства, указанного в To, сравнивается с текущим значением свойства и, если значение в To больше, то свойство увеличивает значение, иначе уменьшается.</p>
<h3>Триггер данных DataTrigger</h3>
<p>DataTrigger отслеживает изменение свойств, которые необязательно должны представлять свойства зависимостей. Для соединения с отслеживаемыми свойства триггеры 
данных используют выражения привязки:</p>
<pre class="brush:xml;">
&lt;Window x:Class="TriggersApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<p>С помощью свойства <code>Binding</code> триггер данных устанавливает привязку к отслеживаемому свойству. Свойство <span class="b">Value</span> задает значение 
отлеживаемого свойства, при котором сработает триггер. Так, в данном случае, если чекбокс будет отмечен, то сработает триггер, который установит 
у кнопки <code>IsEnabled="False"</code>.</p>
<img src="./pics/10.3.png" alt="DataTrigger в WPF" />
...
<div class="nav"><p><a href="10.php">Назад</a><a href="./">Содержание</a><a href="10.3.php">Вперед</a></p></div></div>
<div class="socBlock">
