...
<html >
<head>
<title>Создание команд в WPF</title>
<meta charset="utf-8" />
<meta name="description" content="Создание новых команд в WPF на основе интерфейса ICommand и использование команд в паттерне MVVM">
...
<h2>Создание новых команд</h2><div class="date">Последнее обновление: 27.01.2016</div>
...
<p>В WPF определено много команд, но даже их может оказаться недостаточно. Поэтому нередко разработчики создают свои собственные команды. 
Наиболее простой способ создания команды - использование готовых классов <span class="b">RoutedCommand</span> и <span class="b">RoutedUICommand</span>, в которых уже реализован 
интерфейс <code>ICommand</code>. Итак, в файле кода под классом окна создадим новый класс, назовем его <code>WindowCommands</code>, который 
будет содержать новые команды:</p>
<pre class="brush: c#;">
...
<p>В данном случае команда называется Exit и представляет собой объект RoutedCommand. В конструктор этого объекта в данном случае передается название команды и элемент-владелец 
команды (здесь объект MainWindow). Причем команда определяется как статическое свойство.</p>
<p>Теперь в коде XAML установим привязку к этой команде и задействуем ее:</p>
<pre class="brush:xml;">
...
<p>Итак, здесь есть пункт меню и есть кнопка, которые вызывают эту команду. Так как команды определены в локальном пространстве имен, которое соответствует 
префиксу <code>local</code> и представляют статические свойства, то к ним обращаемся с помощью выражения <code>local:WindowCommands.Exit</code>. Привязка команды связывает ее с обработчиком <code>Exit_Executed</code>, 
который определим в коде c#:</p>
<pre class="brush: c#;">
...
<p>Таким образом, при нажатии на кнопку или пункт меню будет выполняться команда Exit, которая будет вызывать вышеопределенный обработчик. В нем происходит 
запись в лог о дате и времени выхода и собственно осуществляется выход из приложения.</p>
...
<div class="nav"><p><a href="7.1.php">Назад</a><a href="./">Содержание</a><a href="8.php">Вперед</a></p></div></div>
<div class="socBlock">
