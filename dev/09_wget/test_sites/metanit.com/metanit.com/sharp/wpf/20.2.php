...
<html >
<head>
<title>C# и WPF | Взаимодействие между окнами</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие между окнами в WPF, свойство Owner и создание дочерних окон, передача в окно данных">
...
<h2>Взаимодействие между окнами</h2><div class="date">Последнее обновление: 21.03.2016</div>
...
<p>Рассмотрим, как мы можем взаимодействовать с несколькими окнами в WPF. Для этого создадим новый проект. По умолчанию он уже содержит одно главное окно MainWindow. 
Теперь добавим еще одно окно. Для этого в окне добавления нового элемента нам надо выбрать тип "Window (WPF)":</p>
<img src="./pics/20.2.png" alt="Добавление окна в WPF" />
<p>Назовем его <span class="b">TaskWindow</span>.</p>
<p>Теперь определим на главном окне MainWindow кнопку для открытия нового окна:</p>
<pre class="brush:xml;">
...
<p>Обработчик нажатия кнопки Button_Click будет выглядеть так:</p>
<pre class="brush:c#;">
...
<p>Для открытия нового окна создаем его объект и затем вызываем метод <span class="b">Show()</span>.</p>
<img src="./pics/20.3.png" alt="Взаимодействие между окнами в WPF" />
<p>При нажатии на кнопку открывается окно TaskWindow.</p>
<p>Используя ссылку на окно, мы можем взаимодействовать с ним, например, передавать ему данные из главной формы или вызывать его методы. Например, изменим код C# класса 
TaskWindow:</p>
<pre class="brush:c#;">
...
<p>Здесь добавлено свойство ViewModel и метод, который отображает его содержимое. Теперь изменим обработчик Button_Click в главном окне MainWindow:</p>
<pre class="brush:c#;">
private void Button_Click(object sender, RoutedEventArgs e)
{
    TaskWindow taskWindow = new TaskWindow();
...
<p>Здесь у окна TaskWindow устанавливается свойство ViewModel и вызывается его метод.</p>
<p>Важно отметить, что после открытия эти окна существуют независимо друг от друга. Мы можем закрыть главное окно 
MainWindow, и второе окно TaskWindow все равно продолжит свою работу. Однако мы можем задать и другое поведение.</p>
<p>У всех окон есть свойство <span class="b">Owner</span>, которое указывает на главное окно, владеющее текущим окном. Так, изменим обработчик 
Button_Click в главном окне:</p>
<pre class="brush:c#;">
private void Button_Click(object sender, RoutedEventArgs e)
{
    TaskWindow taskWindow = new TaskWindow();
...
<p>Теперь текущий объект MainWindow является владельцем taskWindow. Если, к примеру, мы закроем MainWindow, то закроется и TaskWindow.</p>
<p>Кроме того, мы можем обращаться из TaskWindow к своему владельцу:</p>
<pre class="brush:c#;">
public partial class TaskWindow : Window
{
...
<p>С другой стороны все зависимые окна доступны в главном окне-владельце через свойство <span class="b">OwnedWindows</span>:</p>
<pre class="brush:c#;">
private void Button_Click(object sender, RoutedEventArgs e)
{
    TaskWindow taskWindow = new TaskWindow();
...
<h3>Класс App и свойство Windows</h3>
<p>Еще одним способ для взаимодействия с окнами предоставляет класс App - главный класс приложения. Он содержит свойство <span class="b">Windows</span>, 
которое хранит информацию обо всех открытых окнах приложения. И в любом месте программы мы можем получить эту информацию:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="20.1.php">Назад</a><a href="./">Содержание</a><a href="20.3.php">Вперед</a></p></div></div>
<div class="socBlock">
