...
<html >
<head>
<title>C# и WPF | Взаимодействие с трехмерными объектами</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие с трехмерными объектами в WPF, обработка событий, работа с классами ModelUIElement3D и Viewport2DVisual3D">
...
<h2>Взаимодействие с трехмерными объектами</h2><div class="date">Последнее обновление: 13.03.2016</div>
...
<h3>ModelUIElement3D</h3>
<p>Подобно классам двухмерных элементов в WPF имеются их трехмерные аналоги - Visual3D и UIElement3D. И мы можем использовать два их класса-наследника: 
 <span class="b">ModelUIElement3D</span> и <span class="b">ContainerUIElement3D</span>. В чем их преимущество перед ModelVisual3D? Поскольку ModelUIElement3D является фактически 
 своего рода элементом управления, мы можем прикрутить к нему события и, например, проверять нажатия мыши по нему. Иначе нам пришлось бы писать гораздо больше 
 кода. Например, изменим код с кубом, применив ModelUIElement3D:</p>
 <pre class="brush:xml;">
...
&lt;/Window&gt;
 ...
 <p>Таким образом, мы связываем нажатие по кубу с обработчиком события MouseDown, в котором затем мы можем определить все необходимые действия:</p>
<pre class="brush:c#;">
private void ModelUIElement3D_MouseDown(object sender, MouseButtonEventArgs e)
{
 ...
 <p>Элемент ContainerUIElement3D является контейнером для других элементов Visual3D, то есть он может вмещать несколько различных элементов 
 ModelUIElement3D, ModelVisual3D и Viewport2DVisual3D</p>
 <h3>Viewport2DVisual3D</h3>
 <p>Элемент Viewport2DVisual3D позволяет помещать на поверхности трехмерного объекта двухмерный интерактивный контент, например, кнопки или текстовые поля. 
 При этом данный интерактивный контент может реагировать на действия пользователя. Например, кнопка может реагировать на нажатия.</p>
 <p>У элемента Viewport2DVisual3D можно выделить следующие свойства:</p>
 <ul>
 <li><p><span class="b">Geometry</span>: определяет меш, который устанавливает поверхность трехмерного объекта</p></li>
 <li><p><span class="b">Visual</span>: определяет двухмерные элементы, которые будут помещаться на трехмерный объект</p></li>
 <li><p><span class="b">Material</span>: материал, используемый для рендеринга двухмерных элементов</p></li>
 <li><p><span class="b">Transform</span>: трасформация, применяемая к 3D-объекту</p></li>
 </ul>
<p>Изменим приложение, применив Viewport2DVisual3D:</p>
<pre class="brush:xml;">
&lt;Grid&gt;
    &lt;Grid.RowDefinitions&gt;
...
<img src="./pics/18.15.png" alt="Viewport2DVisual3D в WPF" />
...
<div class="nav"><p><a href="18.6.php">Назад</a><a href="./">Содержание</a><a href="16.php">Вперед</a></p></div></div>
<div class="socBlock">
