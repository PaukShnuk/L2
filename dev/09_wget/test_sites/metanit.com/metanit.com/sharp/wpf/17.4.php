...
<html >
<head>
<title>C# и WPF | Трансформации</title>
<meta charset="utf-8" />
<meta name="description" content="Трансформации в WPF, классы трансформаций RotateTransform, ScaleTransform, TranslateTransform, SkewTransform, различие между свойствами RenderTransform и LayoutTransform">
...
<h2>Трансформации</h2><div class="date">Последнее обновление: 04.03.2016</div>
...
<p>Трансформации представляют инструмент изменения положения или размера элементов WPF. Трансформации могут быть полезны в тех ситуациях, когда надо изменить положение элемента, 
либо анимировать. Все трансформации наследуются от абстрактного базового класса 
<span class="b">System.Windows.Media.Transform</span> и представляют следующие классы:</p>
<ul>
<li><p><span class="b">TranslateTransform</span>: сдвигает элементы по горизонтали и вертикали</p></li>
<li><p><span class="b">RotateTransform</span>: вращает элемент</p></li>
<li><p><span class="b">ScaleTransform</span>: выполняет операции масштабирования</p></li>
<li><p><span class="b">SkewTransform</span>: изменяет позицию элемента путем наклона на определенное количество градусов</p></li>
<li><p><span class="b">MatrixTransform</span>: изменяет координатную систему в соответствии с определенной матрицей</p></li>
<li><p><span class="b">TransformGroup</span>: представляет группу трансформаций</p></li>
</ul>
<h3>RotateTransform</h3>
<p>RotateTransform поворачивает элемент вокруг оси на определенное количество градусов. Данный объект принимает три основых параметра:</p>
<ul>
<li><p><code>Angle</code>: угол поворота</p></li>
<li><p><code>CenterX</code>: устанавливает центр вращения по оси X</p></li>
<li><p><code>CenterY</code>: устанавливает центр вращения по оси Y</p></li>
</ul>
<pre class="brush:xml;">
...
<img src="./pics/16.8.png" alt="RotateTransform в WPF" />
<h3>TranslateTransform</h3>
<p>TranslateTransform позволяет сместить положение элемента по оси X, с помощью свойства X, и по оси Y - с помощью свойства Y.</p>
<pre class="brush:xml;">
&lt;Rectangle Width="100" Height="30" Stroke="Blue" Fill="LightBlue"&gt;
    &lt;Rectangle.RenderTransform&gt;
...
<img src="./pics/16.9.png" alt="TranslateTransform в WPF" />

<h3>ScaleTransform</h3>
<p>Обеспечивает масштабирование элемента на определенную величину. Для изменения ширины надо задать свойство <span class="b">ScaleX</span>, 
а для изменения длины - свойство <span class="b">ScaleY</span>. Кроме того, также имеются свойства <span class="b">CenterX</span> и 
<span class="b">CenterY</span>, позволяющие позиционировать элемент.</p>
<p>Например, увеличение прямоугольника в полтора раза:</p>
<pre class="brush:xml;">
&lt;Rectangle Width="100" Height="30" Stroke="Blue" Fill="LightBlue"&gt;
    &lt;Rectangle.RenderTransform&gt;
...
<img src="./pics/16.10.png" alt="ScaleTransform в WPF" />
<h3>SkewTransform</h3>
<p>SkewTransform позволяет задать наклон элемента вдоль оси X с помощью свойства <span class="b">AngleX</span>, и по оси Y - с помощью свойства 
<span class="b">AngleY</span>. А с помощью свойств <span class="b">CenterX</span> и <span class="b">CenterY</span> можно изменить положение элемента относительно 
осй X и Y:</p>
<pre class="brush:xml;">
&lt;Rectangle Width="100" Height="30" Stroke="Blue" Fill="LightBlue"&gt;
    &lt;Rectangle.RenderTransform&gt;
...
<img src="./pics/16.11.png" alt="SkewTransform в WPF" />
<h3>MatrixTransform</h3>
<p>Осуществляет матричное преобразование элемента. В свойстве <span class="b">Matrix</span> мы задаем первые два столбца, которые применяются при 
преобразовании. Последний столбец по умолчанию имеет значения {0 0 1}.</p>
<pre class="brush:xml;">
&lt;Rectangle Width="100" Height="30" Stroke="Blue" Fill="LightBlue"&gt;
    &lt;Rectangle.RenderTransform&gt;
...
<img src="./pics/16.12.png" alt="MatrixTransform в WPF" />
<h3>TransformGroup</h3>
<p>TransformGroup позволяет комбинировать различные трансформации вместе:</p>
<pre class="brush:xml;">
&lt;Rectangle Width="100" Height="30" Stroke="Blue" Fill="LightBlue"&gt;
    &lt;Rectangle.RenderTransform&gt;
        &lt;TransformGroup&gt;
...
<h3>RenderTransform и LayoutTransform</h3>
<p>Для применения трансформаций у фигур и стандартных элементов управления WPF используются свойства <span class="b">RenderTransform</span> и 
<span class="b">LayoutTransform</span>. Несмотря на то, что для обоих свойств трансформации задаются одинаково, их действие различается. Так, свойство 
LayoutTransform применяется до компоновки элемента, а RenderTransform - после, поэтому одинаковые трансформации для этих свойств могут давать немного разные результаты:</p>
<pre class="brush:xml;">
&lt;Grid&gt;
...
<img src="./pics/16.13.png" alt="LayoutTransform в WPF" />
...
<div class="nav"><p><a href="17.3.php">Назад</a><a href="./">Содержание</a><a href="18.1.php">Вперед</a></p></div></div>
<div class="socBlock">
