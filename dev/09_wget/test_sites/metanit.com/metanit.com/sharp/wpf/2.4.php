...
<html >
<head>
<title>C# и WPF | Пространства имен из C# в XAML</title>
<meta charset="utf-8" />
<meta name="description" content="Использование в WPF в XAML классов из пространств имен плафтормы .NET и C#">
...
<h2>Пространства имен из C# в XAML</h2><div class="date">Последнее обновление: 04.02.2016</div>
...
<p>По умолчанию в WPF в XAML подключается предустановленный набор пространств имен xml. Но мы можем задействовать 
любые другие пространства имен и их функциональность в том числе и стандартные пространства имен платформы .NET, например, System или System.Collections. 
Например, по умолчанию в определении элемента Window подключается локальное пространство имен:</p>
<pre class="brush:xml;">xmlns:local="clr-namespace:XamlApp"</pre>
...
<p>Так как пространство имен проекта проецируется на префикс <span class="b">local</span>, то все классы проекта используются в форме 
<code>local:Название_Класса</code>. Так в данном случае объект Phone устанавливается в качестве содержимого кнопки через свойство Content. 
Для сложных объектов это свойство принимает их строковое представление, которое возвращается методом ToString():</p>
<img src="./pics/2.2.png" alt="Локальные классы в XAML" />
<p>Мы можем подключить любые другие пространства имен, классы которых мы хотим использовать в приложении. Например:</p>
<pre class="brush:xml;">
&lt;Window x:Class="XamlApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<p>Здесь определены два дополнительных пространства имен:</p>
<pre class="brush:xml;">
xmlns:col="clr-namespace:System.Collections;assembly=mscorlib"
xmlns:sys="clr-namespace:System;assembly=mscorlib"
</pre>
<p>Благодаря этому нам становятся доступными объекты из пространств имен System.Collections и System. И затем используя префикс, мы можем 
использовать объекты, входящие в данные пространства имен: <code>&lt;col:ArrayList....</code></p>
<p>Общий синтаксис подключения пространств имен следующий: <code>xmlns:Префикс="clr-namespace:Пространство_имен;assembly=имя_сборки"</code>. 
Так в предыдущем случае мы подключили пространство имен System.Collections, классы которого находятся в сборке mscorlib. И данное подключенное пространство 
имен у нас отображено на префикс col.</p>
...
<div class="nav"><p><a href="2.3.php">Назад</a><a href="./">Содержание</a><a href="4.php">Вперед</a></p></div></div>
<div class="socBlock">
