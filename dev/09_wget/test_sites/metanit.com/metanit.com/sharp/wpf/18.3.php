...
<html >
<head>
<title>C# и WPF | Определение трехмерного объекта</title>
<meta charset="utf-8" />
<meta name="description" content="Создание трехмерных объектов в WPF, работа с классами ModelVisual3D, GeometryModel3D и MeshGeometry3D, использование материалов в трехмерных объектах">
...
<h2>Определение трехмерного объекта</h2><div class="date">Последнее обновление: 13.03.2016</div>
...
<p>Для создания трехмерного объекта мы можем использовать различные виды элементов:</p>
<table class="tab">
<tr class="tabhead"><td><p>3D-элемент</p></td><td><p>2D-аналог</p></td><td><p>Описание</p></td></tr>
<tr><td><p>Visual3D</p></td><td><p>Visual</p></td><td><p>Базовый класс для всех трехмерных объектов, которые визуализируются в Viewport3D. В частности, 
классы <span class="b">ModelVisual3D</span> и <span class="b">ModelUIElement3D</span>, объекты которых добавляются в Viewport3D через свойство Children, как раз наследуются от 
Visual3D</p></td></tr>
<tr><td><p>Model3D</p></td><td><p>Drawing</p></td><td><p>Представляет трехмерную модель, которая визуализируется элементом Visual3D</p></td></tr>
<tr><td><p>Geometry3D</p></td><td><p>Geometry</p></td><td><p>Абстрактный класс, который представляет трехмерную поверхность элемента. Для использования нам 
доступен его производный класс MeshGeometry3D</p></td></tr>
<tr><td><p>GeometryModel3D</p></td><td><p>GeometryDrawing</p></td><td><p>Класс, производный от Model3D. Объединяет объект Geometry3D, преставляющий поверхность 3D-объекта, и объект Material, 
определяющий материал для покрытия трехмерного объекта</p></td></tr>
<tr><td><p>Transform3D</p></td><td><p>Transform</p></td><td><p>Определяет трансформации 3D-объектов</p></td></tr>
<tr><td><p>UIElement3D</p></td><td><p>UIElement</p></td><td><p>Базовый класс для трехмерных объектов, которые могут вести себя как обычные элементы управления, например, 
реагировать на события</p></td></tr>
</table>
<p>В прошлой теме для задания трехмерного объекта мы использовали следующую разметку xaml:</p>
<pre class="brush:xml;">
&lt;ModelVisual3D&gt;
...
<p>Объект <span class="b">ModelVisual3D</span> предсталяет визуализируемый трехмерный объект. Для его установки используется свойство <span class="b">Content</span>.</p>
<p>В качестве содержимого этого свойства устанавливается элемент <span class="b">GeometryModel3D</span>. Этот элемент объединяет два аспекта: геометрию трехмерного объекта и 
его материал. Для определения материала применяется свойство <span class="b">Material</span>, а для установки геометрии элемента - свойство <span class="b">Geometry</span>.</p>

<p>В качестве геометрии устанавливается элемент <span class="b">MeshGeometry3D</span>, который преставляет собой <span class="b">меш</span> - один из фундаментальных концептов 
разработки трехмерных объектов. Для определения самого трехмерного объекта у MeshGeometry3D доступны следующие свойства:</p>
<ul>
<li><p><span class="b">Positions</span>: определяет координаты вершин треугольников. В нашем случае фигура представляет один треугольник, поэтому для его 
определения потребуется всего три точки. Для каждой точки определяется три координаты x, y, z.</p></li>
<li><p><span class="b">TriangleIndices</span>: определяет треугольник, принимая набор верши. Например, <code>TriangleIndices="0,2,1"</code> - треугольник 
строиться от первой вершины, далее к третьей и далее ко второй, которая соединяется с первой.</p></li>
<li><p><span class="b">Normals</span>: векторы нормалей, которые применяются для вычисления освещенности</p></li>
<li><p><span class="b">TextureCoordinates</span>: определяет, как двухмерная текстура будет накладываться на трехмерный объект</p></li>
</ul>

<p>Итак, чтобы нарисовать объект, мы должны задать его вершины. Для этого в объекте MeshGeometry3D мы устанавливаем свойство Positions. В случае одно треугольника 
нам нужны только три вершины, которые в координатной системе размещены следующим образом:</P>
<img src="./pics/18.2.png" />
<p>Затем с помощью свойства TriangleIndices мы устанавливаем, в какой последовательности эти вершины будут соединены линиями. В данном случае мы указываем, 
что сначала мы соединыем первую и третью вершины (точку -0.5,0,0 с точкой 0.5,0,0), потом от третьей вершины проводим линию до второй (до точки 
0,0.5,0). Почему мы не написали TriangleIndices="0,1,2", что вроде было бы более логично? В WPF действует правило, согласно которому мы должны перечислять 
точки против часовой стрелки относительно оси Z, поэтому, если бы мы написали 0,1,2, то у нас бы ничего не получилось.</p>
<p>При этом надо отметить, что ось Z направлена на нас, то есть положительные значения после условного нуля будут ближе к нам, а отрицательные - дальше от нас.</p>
<h3>Материалы</h3>
<p>Кроме геометрии в объекте GeometryModel3D мы еще должны определить материал. Материалы бывают нескольких видов:</p>
<ul>
<li><p><span class="b">DiffuseMaterial</span>. Создает матовую поверхность объекта, на которой свет распространен равномерно во всех направлениях. 
Яркость же объекта зависит от угла, под которым свет падает на поверхность.</p></li>
<li><p><span class="b">SpecularMaterial</span>. Имитирует блестящую поверхность, отражая свет зеркально в противоположном направлении. Зачастую он используется 
для наложения бликов на темной поверхности, к которой ранее применялся DiffuseMaterial</p></li>
<p>Для использования материала SpecularMaterial изменим код материала следующим образом:</p>
<pre class="brush:xml;">
&lt;GeometryModel3D.Material&gt;
    &lt;MaterialGroup&gt;
...
<p>Материал SpecularMaterial имеет свойство SpecularPower, которое позволяет задать размытость цвета. При большем значении цвет становится ярче, 
а при меньшем значении, наоборот, мы получаем более мягкий, более размытый эффект.</p>
<img src="./pics/18.3.png" alt="Материалы в WPF" />
<li><p><span class="b">EmissiveMaterial</span>. Создает светящуюся поверхность.</p></li>
<pre class="brush:xml;">
&lt;GeometryModel3D.Material&gt;
    &lt;MaterialGroup&gt;
...
<p>При наложении различных цветов они смешиваются, создавая средний оттенок.</p>
<img src="./pics/18.4.png" />
<li><p><span class="b">MaterialGroup</span>. Создает комбинацию нескольких материалов.</p></li>
</ul>
<p>Кроме свойства Material в объекте GeometryModel3D определено свойство BackMaterial, которое настраивает внешний вид также с помощью материалов 
для задней поверхности предмета. Но поскольку в данном случае нас задняя поверхность треугольника не интересует, мы не используем это свойство. 
Но оно в принципе аналогично свойству Material.</p>
...
<div class="nav"><p><a href="18.1.php">Назад</a><a href="./">Содержание</a><a href="18.4.php">Вперед</a></p></div></div>
<div class="socBlock">
