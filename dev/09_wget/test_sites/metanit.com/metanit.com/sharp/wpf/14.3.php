...
<html >
<head>
<title>C# и WPF | ItemTemplate и DataTemplate</title>
<meta charset="utf-8" />
<meta name="description" content="Шаблоны данных в WPF, переопределение шаблонов отображения элементов в ListBox и ListView, свойство ItemTemplate и объект DataTemplate">
...
<h2>ItemTemplate и DataTemplate</h2><div class="date">Последнее обновление: 26.02.2016</div>
...
<p>Все элементы управления списками имеют свойство <span class="b">ItemTemplate</span>, которое позволяет задать свой шаблон отображения данных. В качестве 
значения оно принимает объект <span class="b">DataTemplate</span>. В качестве самих данных, которые отображаются в DataTemplate, как правило, выступают объекты пользовательских классов, создаваемых самим разработчиком. 
Сам шаблон данных представляет разметку xaml, которая управляет визуализацией элемента.</p>
<p>Итак, добавим в проект следующий класс Phone, который будет представлять модель телефона:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>В коде xaml определим шаблон данных для списка смартфонов:</p>
<pre class="brush:xml;">
...
<p>Итак, здесь шаблон данных представлен следующей разметкой:</p>
<pre class="brush:xml;">
&lt;DataTemplate&gt;
    &lt;StackPanel Margin="5"&gt;
        &lt;Image Width="100" Height="75" Source="{Binding Path=ImagePath}" /&gt;
...
<p>Для каждого элемента применяется привязка к определенному свойству объекта Phone. То есть фактически в визуальном плане каждый элемент будет представлять 
собой StackPanel с набором элементов Image и TextBlock.</p>
<p>Далее создадим в коде C# коллекцию смартфонов и установим ее в качестве источника данных:</p>
<pre class="brush:c#;">
...
<p>Здесь создается 4 объекта Phone. Для каждого объекта устанавливаются необходимые значения. Причем свойство ImagePath указывает на путь к изображению 
в папке Images. Предполагается, что в проект добавлена папка Images, в которой есть четыре изображения для каждой из моделей.</p>
<p>И также здесь определяется обработчик выбора элемента в списке. Несмотря на то, что список представляет сложные данные, мы все равно сможем обработать выбор пользователя 
и получить выбранный объект. И так как DataTemplate отображает объекты Phone, то выделенный элемент мы можем привести к типу Phone.</p>
<p>В итоге получится следующее приложение:</p>
<img src="./pics/14.5.png" alt="DataTemplate в WPF" />
<p>Одним из преимуществ шаблонов данных является то, что их можно вынести во вне, например, в ресурсы. В этом случае мы повторно сможем использовать одни и тот же 
шаблон данных для разных элементов в разных частях программы:</p>
<pre class="brush:xml;">
&lt;Window x:Class="DataApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<h3>Триггеры данных</h3>
<p>С помощью триггеров данных (data triggers) можно задать дополнительную логику визуализации, которая срабатывает, если свойство привязанного объекта принимает то или 
иное значение. Например, в случае с примером выше, допустим, мы хотим, если у отображаемого телефона свойство Company равно Microsoft, то этот элемент 
выделяется жирным или выделяется красным цветом. И для этого добавим в шаблон данных DataTemplate триггер данных:</p>
<pre class="brush:xml;">
&lt;Window x:Class="DataApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<p>Для визуализации изображение помещено в элемент Border, который имеет имя "phoneImageBorder". С помощью триггера данных у этого элемента изменяется 
ширин и цвет границы. Кроме того, меняется жирность шрифта, который выводит название модели:</p>
<pre class="brush:xml;">
&lt;DataTrigger Binding="{Binding Company}" Value="Microsoft"&gt;
    &lt;Setter TargetName="phoneTitle" Property="FontWeight" Value="Bold" /&gt;
    &lt;Setter TargetName="phoneImageBorder" Property="BorderBrush" Value="Red" /&gt;
...
<p>В итоге мы получим следующий визуальный эффект:</p>
<img src="./pics/14.14.png" alt="DataTrigger в DataTemplate" />
