...
<html >
<head>
<title>C# и WPF | Приложение и класс Application</title>
<meta charset="utf-8" />
<meta name="description" content="Описание класса Application и его роль в приложении WPF, использования файлов App.xaml и App.xaml.cs и базовых свойств и  событий приложения">
...
<h1>Приложение и класс Application</h1><h2>Класс Application</h2><div class="date">Последнее обновление: 13.02.2016</div>
...
<p>В предыдущих главах мы работали непосредственно с окном приложения, которое представлено классом Window, его разметкой, добавляли в нее 
элементы, создавали для него код на C#. Однако само приложение начинается не с класса <span class="b">Window</span>, а с класса 
<span class="b">Application</span>. По умолчанию при создании проекта WPF создается файл App.xaml и класс связанного кода App.xaml.cs:</p>
<img src="./pics/12.1.png" alt="Класс Application в WPF" />
<p>Файл App.xaml выглядит примерно так:</p>
<pre class="brush:xml;">
...
<p>С помощью атрибута <code>x:Class</code> элемент Application задает полное название производного класса приложения. По умолчанию класс называется <span class="b">App</span>, с указанием названия проекта, то есть 
в данном случае <span class="b">LifecycleApp.App</span></p>
<p>Как правило, основная задача данного файла состоит в определении ресурсов, общих для приложения. Поэтому тут по умолчанию определен пустой элемент <code>Application.Resources</code>, 
в который, собственно, и помещаются ресурсы.</p>
<p>Также здесь декларативным путем можно прикрепить к событиям приложения обработчики.</p>
<p>С помощью атрибута <span class="b">StartupUri</span> устанавливается путь к разметке xaml, 
с которого начинается выполнение приложения. По умолчанию это разметка окна MainWindow, определенного в файле <i>MainWindow.xaml</i>. Если у нас в 
приложении несколько окон, тут мы можем указать то, которое будет запускаться при вызове приложения.</p>
<p>файл <i>App.xaml.cs</i> также содержит определение класса App. По умолчанию этот класс совершенно пустой:</p>
<pre class="brush:c#;">
public partial class App : Application
{
}
</pre>
<p>В нем мы можем задать обработчики событий приложения, либо какую-то другую глобальную для всего приложения логику.</p>
<p>Таким образом, у нас получается, что оба файла <i>App.xaml</i> и <i>App.xaml.cs</i> содержат определение одного и тоже класса App. 
Однако в конечном счете они будут компилироваться в один файл приложения <i>App.g.cs</i>, который вы можете 
найти после компиляции приложения в каталоге проекта <i>obj/Debug</i> и который будет выглядеть примерно так:</p>
<pre class="brush:c#;">
...
<p>Входной точкой в программу (как и в любое другое приложение на C#) является метод Main, в котором создается экземпляр приложения и производится 
начальная инициализация и вызов главного окна программы.</p>
<p>Так как WPF требует, чтобы главный поток работал в однопоточном подразделении (Single-threaded apartment), то метод Main помечается атрибутом 
<code>STAThreadAttribute</code>. Однопоточное подразделение содержит один поток, в данном случае главный. Это означает, что к элементам, созданным 
в этом потоке можно обратиться только из этого же потока. В то же время WPF предлагает эффективный способ взаимодействия между потоками, о котором мы позже поговорим.</p>
<p>С помощью метода InitializeComponent происходит инициализация приложение: установка главного окна для запуска. И далее само приложение запускается 
через вызов <code>app.Run()</code>.</p>
<p>Однако это не жестко заданная организация файлов приложения, и мы ее можем переопределить и установить точку входа в приложение сами. Для этого, во-первых, изменим код App.xaml.cs:</p>
<pre class="brush:c#;">
public partial class App : Application
{
    App()
...
<p>Фактически файл App.xaml.cs стал похож на компилируемый файл <i>App.g.cs</i>. Здесь мы сразу определяем метод Main и запускаем главное окно MainWindow.</p>
<p>После этого изменим файл <i>App.xaml</i> следующим образом:</p>
<pre class="brush:xml;">
&lt;Application x:Class="LifecycleApp.App"
             xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
             xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<p>В данном случае убран атрибут StartupUri, так как главное окно приложения теперь запускается из самого класса, и определен стиль для элемента Grid, 
который по умолчанию имеется в MainWindow.xaml.</p>
<p>Далее в окне свойств для файла <i>App.xaml</i> изменим значение в поле <span class="b">Build Action</span> на <span class="b">Page</span>:</p>
<img src="./pics/12.2.png" alt="Buil Action в WPF" />
<p>И после запуска нам отобразится окно с гридом, окрашенным в красный цвет.</p>
...
<div class="nav"><p><a href="10.3.php">Назад</a><a href="./">Содержание</a><a href="3.2.php">Вперед</a></p></div></div>
<div class="socBlock">
