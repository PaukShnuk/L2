...
<html >
<head>
<title>C# и WPF | Интерфейс INotifyPropertyChanged</title>
<meta charset="utf-8" />
<meta name="description" content="Привязка в WPF, извещение свойства об изменении значения, интерфейс INotifyPropertyChanged">
...
<h2>Интерфейс INotifyPropertyChanged</h2><div class="date">Последнее обновление: 7.02.2016</div>
...
<p>В прошлой теме использовался объект Phone для привязки к текстовым блокам. Однако если мы изменим его, содержимое текстовых блоков не изменится. 
Например, добавим в окно приложения кнопку:</p>
<pre class="brush:xml;">
...
<p>И в файле кода для этой кнопки определим обработчик, в котором будет меняться свойства ресурса:</p>
<pre class="brush:c#;">
...
<p>Сколько бы мы не нажимали на кнопку, текстовые блоки, привязанные к ресурсу, не изменятся. Чтобы объект мог полноценно реализовать механизм привязки, 
нам надо реализовать в его классе интерфейс INotifyPropertyChanged. И для этого изменим класс Phone следующим образом:</p>
<pre class="brush:c#;">
...
<p>Когда объект класса изменяет значение свойства, то он через событие PropertyChanged извещает систему об изменении свойства. А система обновляет все привязанные объекты.</p>
<img src="./pics/11.5.png" alt="Интерфейс INotifyPropertyChanged в WPF" />
...
<div class="nav"><p><a href="11.php">Назад</a><a href="./">Содержание</a><a href="11.3.php">Вперед</a></p></div></div>
<div class="socBlock">
