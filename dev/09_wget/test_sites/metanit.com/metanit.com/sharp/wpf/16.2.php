...
<html >
<head>
<title>C# и WPF | Анимация в XAML</title>
<meta charset="utf-8" />
<meta name="description" content="Создание анимации в коде XAML в WPF, использование триггеров EventTrigger для запуска анимации, роль объекта Storyboard">
...
<h2>Анимация в XAML</h2><div class="date">Последнее обновление: 04.03.2016</div>
...
<p>Для определения анимации в XAML применяется объект <span class="b">EventTrigger</span> или триггер событий. Этот объект имеет свойство 
<span class="b">Actions</span>, которое определяет ряд действий, возникающих в результате генерации события. Само возникающее действие описывается через элемент 
<span class="b">BeginStoryboard</span>, который и запускает анимацию.</p>
<p>Непосредственно для определения анимации используется объект <span class="b">Storyboard</span>. Он объявляет объект анимации со всеми ее свойствами и 
параметрами. Например, проанимируем ширину кнопки:</p>
<pre class="brush: xml;">
...
<p>У объекта EventTrigger с помощью атрибута <code>RoutedEvent</code> определяется событие, которое будет запускать анимацию. В данном случае это событие Loaded - загрузка окна.</p>
<p>Ряд настроек анимации устанавливает элемент Storyboard: <code>TargetName</code> задает анимируемый элемент, а <code>TargetProperty</code> определяет свойство элемента, 
которое будет анимироваться.</p>
<p>В принципе эти настройки также можно было бы вынести в объект анимации в виде прикрепляемых свойств:</p>
<pre class="brush:xml;">
&lt;Storyboard&gt;
    &lt;DoubleAnimation Storyboard.TargetProperty="Width" Storyboard.TargetName="helloButton" 
						From="70" To="150"  ...&gt;
</pre>
<p>И далее в самом объекте Storyboard определяется объект анимации DoubleAnimation с рядом настроек. Все настройки в прицнипе тут те же, что использовались для анимации в прошлой теме. Правда, при определении значений свойств здесь есть некоторые отличия.</p>
<p>Так, свойство <span class="b">RepeatBehavior</span> инициализируется временем - "0:0:10", которое будет повторяться анимация. Чтобы указать число 
повторов, структуру <span class="b">RepeatBehavior</span> надо инициализировать так: <span class="b">RepeatBehavior="2x"</span>, 
 где 2 - количество повторов, а x - просто префикс, указывающий, что речь идет о количестве итераций, без него бы число интерпретировалось как количество 
 дней. Третий способ задания этого свойства - <span class="b">RepeatBehavior="Forever"</span> - в этом случае анимация будет продолжаться все время работы приложения.</p>
<p>Например, ниже установлено <span class="b">RepeatBehavior="Forever"</span>, а свойство <span class="b">DecelerationRatio</span> замедляет анимацию, что создает эффект подскока шара в верх, где он, достигая максимальной точки, теряет скорость, а при падении вновь ее увеличивает:</p>
<pre class="brush: xml;">
&lt;Window x:Class="AnimationApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<p>В данном случае так как в EventTrigger в качестве события определено <code>Button.Click</code>, то анимация будет запускаться по нажатию на кнопку.</p>
<p>По умолчанию анимация вызывается 60 раз в секунду, однако с помощью прикрепленного свойства <span class="b">Timeline.DesiredFrameRate</span> можно задать частоту кадров в секунду, как в предыдущем примере.</p>
<h3>Анимация свойств вложенных объектов</h3>
<p>Анимация может применяться и к свойствам вложенных объектов, которые являются свойствами, например:</p>
<pre class="brush: xml;">
...
<p>Здесь свойство <span class="b">Fill</span> элемента Ellipse инициализируется кистью RadialGradientBrush, которая имеет коллекцию <span class="b">GradientStops</span>. 
Анимация же применяется ко второму объекту коллекции и его свойству <span class="b">Color</span>.</p>
<h3>Комплексные анимации</h3>
<p>С помощью объекта Storyboard можно создавать и более комплексные анимации. Например, сделаем кнопку, которая одновременно меняет ширину, длину и цвет:</p>
<pre class="brush:xml;">
...
<h3>Определение анимации в стиле</h3>
<p>При этом необязательно знать имя элемента для анимации, можно прикрепить анимацию ко всем элементам одного типа и установить ее через стиль:</p>
<pre class="brush: xml;">
&lt;Window x:Class="AnimationApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<p>Так как у стиля задан атриубут <code>TargetType="Button"</code>, то анимация внутри триггера будет применяться ко всем кнопкам. 
...
а у этого объекта есть свойство <code>Color</code>. Напрямую анимировать свойство Background мы не можем, так как ColorAnimation требует типа Color.</p>
<p>Так как стиль применяется глобально к кнопкам, то в настройках анимации название анимируемого элемента не указывается. Также не указывается свойство <code>From</code>, 
а это значит, что анимаця будет идти от текущего цвета кнопки.</p>
<p>Теперь после нажатия натажатая кнопка будет ненадолго окрашиваться в красный, а затем возвращать исходный цвет.</p>
...
<div class="nav"><p><a href="16.php">Назад</a><a href="./">Содержание</a><a href="16.3.php">Вперед</a></p></div></div>
<div class="socBlock">
