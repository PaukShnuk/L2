...
<html >
<head>
<title>C# и WPF | Контейнеры GroupBox и Expander</title>
<meta charset="utf-8" />
<meta name="description" content="Обзор специальных контейнеров - элементов ScrollViewer, Expander, GroupBox, особенности работы с ними, их основные свойства и события">
...
<h2>Контейнеры GroupBox и Expander</h2><div class="date">Последнее обновление: 18.01.2016</div>
...
<p>Особая группа элементов управления образована от класса <span class="b">HeaderedContentControl</span>, который является подклассом <code>ContentControl</code>. Эта группа отличается 
тем, что позволяет задать заголовок содержимому. В эту группу элементов входят GroupBox и Expander.</p>
<h3>GroupBox</h3>
<p>Элемент GroupBox организует наборы элементов управления в отдельные группы. При этом мы можем определить у группы заголовок:</p>
<pre class="brush:xml;">
...
<p>GroupBox включает группу различных элементов, однако, как и всякий элемент управления содержимым, он принимает внутри себя только один контейнер, 
поэтому сначала мы вкладываем в GroupBox общий контейнер, а в него уже все остальные элементы.</p>
<img src="./pics/4.8.png" alt="Элемент GroupBox в WPF" />
<p>Однако заголовок GroupBox необязательно представляет простой текст. Мы можем пойти дальше и изменить предыдущий пример, засунув кнопку заказа прямо в заголовок:</p>
<pre class="brush:xml;">
&lt;GroupBox Padding="5"&gt;
	&lt;GroupBox.Header&gt;
...
<p>Осталось добавить обработчик нажатия кнопки Click для обработки заказа и можно заказывать блюда.</p>

<h3>Expander</h3>
<p>Представляет скрытое содержимое, раскрывающееся по нажатию мышкой на указатель в виде стрелки. Причем содержимое опять же может быть самым разным: кнопки, текст, картинки и т.д.</p> 
<p>С помощью свойства <span class="bb">IsExpanded</span> можно задать раскрытие узла при старте приложения. По умолчанию узел скрыт. Пример использования:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<img src="./pics/4.10.png" alt="Элемент Expander в WPF" />
<p>Опять же мы можем изменить заголовок, вложив в него, например, кнопку или изображение:</p>
<pre class="brush:xml;">
&lt;Expander&gt;
    &lt;Expander.Header&gt;
...
<p>Если мы хотим обработать открытие экспандера, то нам надо обработать событие Expanded (а при обработке закрытия - событие Collapsed). 
Данные события вызываются до самого действия, поэтому мы можем перед открытием, например, динамически устанавливать содержание экспандера:</p>
<pre class="brush:xml;">
&lt;Expander Expanded="Expander_Expanded" Collapsed="Expander_Collapsed"&gt;
</pre>
<p>А обработка событий в файле C# могла бы выглядеть так:</p>
<pre class="brush:c#;">
...
<p>В итоге при раскрытии элемента вместо начального содержимого там будет определенная в коде кнопка.</p>
<p>Программное создание Expandera:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="5.3.php">Назад</a><a href="./">Содержание</a><a href="5.20.php">Вперед</a></p></div></div>
<div class="socBlock">
