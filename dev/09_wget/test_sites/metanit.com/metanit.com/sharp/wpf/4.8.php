...
<html >
<head>
<title>C# и WPF | Свойства компоновки элементов</title>
<meta charset="utf-8" />
<meta name="description" content="Свойства компоновки элементов WPF, установка ширины и высоты, выравнивание по горизонтали (HorizontalAlignment) и вертикали (VerticalAlignment), оступы margin и padding, zindex.">
...
<h2>Свойства компоновки элементов</h2><div class="date">Последнее обновление: 16.01.2016</div>
...
<p>Элементы WPF обладают набором свойств, которые помогают позиционировать данные элементы. Рассмотрим некоторые из этих свойств.</p>
<h3>Ширина и высота</h3>
<p>У элемента можно установить ширину с помощью свойства <code>Width</code> и высоту с помощью свойства <code>Height</code>. Эти свойства 
принимают значение типа <code>double</code>. Хотя общая рекомендация 
состоит в том, что желательно избегать жестко закодированных в коде ширины и высоты.</p>
<p>Также мы можем задать возможный диапазон ширины и высоты с помощью свойств <code>MinWidth/MaxWidth</code> и <code>MinHeight/MaxHeight</code>. 
И при растяжении или сжатии контейнеров элементы с данными заданными свойствами не будут выходить за пределы установленных значений.</p>
<p>Возможно, возникает вопрос, а в каких единицах измерения устанавливаются ширина и высота? Да и в общем какие единицы измерения используются? 
В WPF можно использовать несколько единиц измерения: сантиметры (<code>cm</code>), точки (<code>pt</code>), дюймы (<code>in</code>) и пиксели 
(<code>px</code>). Например, зададим размеры в других единицах: <code>&lt;Button Content="Кнопка" Width="5cm" Height="0.4in" /&gt;</code></p>
<p>Если единица измерения не задана явно, а просто стоит число, то используются по умолчанию пиксели. Но эти пиксели не равны обычным пикселям, а 
являются своего рода "логическими пикселями", независимыми от конкретного устройства. Каждый такой пиксель представляет 1/96 дюйма вне зависимости от 
разрешения экрана.</p>
<h3>Выравнивание</h3>
<h4>HorizontalAlignment</h4>
<p>С помощью специальных свойств мы можем выровнять элемент относительно определенной стороны контейнера по горизонтали или вертикали.</p>
<p>Свойство <span class="b">HorizontalAlignment</span> выравнивает элемент по горизонтали относительно правой или левой стороны контейнера и соответственно 
может принимать значения Left, Right, Center (положение по центру), Stretch (растяжение по всей ширине). Например:</p>
<pre class="brush:xml;"> 
&lt;Grid&gt;
...
<img src="./pics/3.15.png" alt="Свойство HorizontalAlignment в WPF" />

<h4>VerticalAlignment</h4>
<p>Также мы можем задать для элемента выравнивание по вертикали с помощью свойства <span class="b">VerticalAlignment</span>, которое принимает 
следующие значения: Top (положение в верху контейнера), Bottom (положение внизу), Center (положение по центру), Stretch (растяжение по всей высоте). Например:</p>
<pre class="brush:xml;"> 
&lt;Grid&gt;
...
<img src="./pics/3.16.png" alt="Свойство VerticalAlignment в WPF" />
<h3>Отступы margin</h3>
<p>Свойство <span class="b">Margin</span> устанавливает отступы вокруг элемента. Синтаксис: Margin="левый_отступ верхний_отступ правый_отступ нижний_отступ". 
Например, установим отступы у одной кнопки слева и сверху, а  у другой кнопки справа и снизу:</p>
<pre class="brush:xml;"> 
&lt;Grid&gt;
...
<img src="./pics/3.14.png" alt="Свойство Margin в WPF" />
<p>Если мы зададим свойство таким образом: Margin="20", то сразу установим отступ для всех четырех сторон.</p>
<h3>Panel.ZIndex</h3>
<p>При создании интерфейса возможна ситуация, когда одни элементы будут полностью или частично перекрывать другие. По умолчанию те элементы, которые определены 
 позже, перекрывают те элементы, которые определены ранее. Однако мы можем изменить подобное действие с помощью свойства <b>Panel.ZIndex</b>.</p>
 <p>По умолчанию для всех создаваемых элементов <code>Panel.ZIndex="0"</code>. Однако назначив данному свойству более высокое значение, мы можем передвинуть его 
 на передний план. Элементы с большим значением этого свойства будут перекрывать те элементы, у которых меньшее значение этого свойства:</p>
<pre class="brush:xml;">
&lt;Grid&gt;
...
&lt;/Grid&gt;</pre>
<img src="./pics/3.13.png" alt="Свойство Panel.ZIndex в WPF" />
...
<div class="nav"><p><a href="4.7.php">Назад</a><a href="./">Содержание</a><a href="5.1.php">Вперед</a></p></div></div>
<div class="socBlock">
