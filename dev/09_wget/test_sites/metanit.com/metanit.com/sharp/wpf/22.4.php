...
<html >
<head>
<title>C# и WPF | Передача параметров команде</title>
<meta charset="utf-8" />
<meta name="description" content="Передача параметров команде в паттерне MVVM в WPF">
...
<h2>Передача параметров команде</h2><div class="date">Последнее обновление: 18.10.2016</div>
...
<p>Команда может принимать один параметр типа object, вместо которого мы можем передать любой объект или даже коллекцию объектов. 
Например, продолжим работу с проектом из прошлой темы и добавим в него удаление объекта из списка. Для этого изменим код ApplicationViewModel следующим 
образом:</p>
<pre class="brush:c#;">
...
<p>Здесь добавлена команда удаления объекта из списка:</p>
<pre class="brush:c#;">
private RelayCommand removeCommand;
public RelayCommand RemoveCommand
{
...
<p>Здесь предполагается, что в качестве параметра в команду будет передаваться удаляемый объект Phone. Ну а поскольку в реальности параметр имеет тип object, то 
его еще надо привести к типу Phone.</p>
<p>Стоит отметить, что в качестве второго параметра в конструктор RelayCommand передается делегат Func&lt;obj, bool&gt;, который позволяет указать условие, 
при котором будет доступна команда. В нашем случае нет смысла удалять элементы из списка, если в списке нет элементов.</p>
<p>И также изменим код <span class="ii">MainWindow.xaml</span>:</p>
<pre class="brush:xml;">
...
<p>Здесь добавлена кнопка удаления. Для нее установлена привязка к команде RemoveCommand. Кроме того, с помощью атрибута <code>CommandParameter</code> 
кнопка устанавливает объект, который передается команде. В данном случае это объект из свойства SelectedPhone:</p>
<pre class="brush:xml;">
&lt;Button Command="{Binding RemoveCommand}" 
		CommandParameter="{Binding SelectedPhone}"&gt;-&lt;/Button&gt;</pre>
<p>Теперь мы сможем удалять элементы из списка. Причем, если в списке не будет элементов, то кнопка будет недоступна, благодаря тому, что в конструктор 
RelayCommand выше было передано выражение <code>(obj) =&gt; Phones.Count &gt; 0</code>, которое устанавливает условие выполнения команды:</p>
<img src="./pics/22.4.png" alt="Параметры команд в MVVM в WPF" />
