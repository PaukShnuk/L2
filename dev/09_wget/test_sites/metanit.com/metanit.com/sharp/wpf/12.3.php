...
<html >
<head>
<title>C# и WPF | Визуальные состояния</title>
<meta charset="utf-8" />
<meta name="description" content="Управление состоянием шаблонов в WPF, объект VisualStateManager и ViewState, анимация при смене состояний в элементе управления">
...
<h2>Визуальные состояния</h2><div class="date">Последнее обновление: 15.02.2016</div>
...
Например, для кнопки это может быть состояние <span class="b">MouseOver</span>, когда указатель мыши наведен на кнопку, или состояние <span class="b">Pressed</span>, когда кнопка нажата. 
У большинства элементов управления есть несколько состояний. И мы можем управлять переходом в эти состояния.</p>
<p>Чтобы изменить внешний вид в зависимости от состояния элемента используется объект <span class="b">ViewState</span>. Объект ViewState через другой 
объект - <span class="bb">Storyboard</span> позволяет определить анимацию, изменяющую внешний вид элемента.</p>
<p>Так, используем несколько состояний, определенных в элементе Button:</p>
<pre class="brush: xml;">
...
<p>При применении данного шаблона при наведении мыши на кнопку она будет приобретать светло-синий цвет, а если мышь покинет пределы кнопки, цвет опять 
станет розовой.</p>
<p>За счет чего это происходит? Корневой элемент Border имеет объект <span class="b">VisualStateManager</span>, у которого, в свою очередь, есть 
коллекция объектов <span class="b">VisualStateGroup</span>. В данном случае у нас используется только один объект VisualStateGroup - <span class="b">CommonStates</span>. 
Данная группа содержит такие состояния как <span class="b">MouseOver</span>, <span class="b">Pressed</span>, <span class="b">Normal</span>. 
А объекты VisualState как раз и задают состояния и применяемую к ним анимацию.</p>
<p>Анимацией управляет объект <span class="b">Storyboard</span>. Его свойство <span class="b">TargetName</span> указывает на имя объекта, который 
анимируется, а <span class="b">TargetProperty</span> - свойство объекта, которое анимируется.</p>
<p>Если объект VisualState используется для визуализации элемента в определенном состоянии, то объект <span class="b">VisualTransition</span> 
визуализирует элемент в момент перехода от одного состояния в другое. Так, если мы задали два визуальных состояния, то мы можем определить визуальный 
переход из одного состояния в другое:</p>
<pre class="brush: xml;">
&lt;Button x:Name="myButton" Content="Привет" Height="40" Width="100"&gt;
    &lt;Button.Template&gt;
        &lt;ControlTemplate TargetType="Button"&gt;
...
<p>В данном случае в момент перехода опять же анимируется фон кнопки, только теперь вся анимация длится 1.5 секунды, в течение которых кнопка последовательно приоретает 
желтый, красный и зеленый цвет, после чего возвращается в исходное состояние.</p>
