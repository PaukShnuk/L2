...
<html >
<head>
<title>C# и WPF | События клавиатуры</title>
<meta charset="utf-8" />
<meta name="description" content="События клавиатуры в WPF, примеры их обработки, валидация ввода в текстовые поля, объект KeyEventArgs">
...
<h2>События клавиатуры</h2><div class="date">Последнее обновление: 22.01.2016</div>
...
<p>К событиям клавиатуры можно отнести следующие события:</p>
<table class="tab">
<tr class="tabhead"><td><p>Событие</p></td><td><p>Тип события</p></td><td><p>Описание</p></td></tr>
<tr><td><p><span class="b">KeyDown</span></p></td><td><p>Поднимающееся</p></td><td><p>Возникает при нажатии клавиши</p></td></tr>
<tr><td><p><span class="b">PreviewKeyDown</span></p></td><td><p>Туннельное</p></td><td><p><p>Возникает при нажатии клавиши</p></td></tr>
<tr><td><p><span class="b">KeyUp</span></p></td><td><p>Поднимающееся</p></td><td><p>Возникает при освобождении клавиши</p></td></tr>
<tr><td><p><span class="b">PreviewKeyUp</span></p></td><td><p>Туннельное</p></td><td><p>Возникает при освобождении клавиши</p></td></tr>
<tr><td><p><span class="b">TextInput</span></p></td><td><p>Поднимающееся</p></td><td><p>Возникает при получении элементом текстового ввода (генерируется не только клавиатурой, но и стилусом)</p></td></tr>
<tr><td><p><span class="b">PreviewTextInput</span></p></td><td><p>Туннельное</p></td><td><p>Возникает при получении элементом текстового ввода</p></td></tr>
</table>
<p>Большинство событий клавиатуры (KeyUp/PreviewKeyUp, KeyDown/PreviewKeyDown) принимает в качестве аргумента объект KeyEventArgs, у которого можно отметить следующие свойства:</p>
<ul>
<li><p><span class="b">Key</span> позволяет получить нажатую или отпущенную клавишу</p></li>
<li><p><span class="b">SystemKey</span> позволяет узнать, нажата ли системная клавиша, например, Alt</p></li>
<li><p><span class="b">KeyboardDevice</span> получает объект KeyboardDevice, представляющее устройство клавиатуры</p></li>
<li><p><span class="b">IsRepeat</span> указывает, что клавиша удерживается в нажатом положении</p></li>
<li><p><span class="b">IsUp</span> и <span class="b">IsDown</span> указывает, была ли клавиша нажата или отпущена</p></li>
<li><p><span class="b">IsToggled</span> указывает, была ли клавиша включена - относится только к включаемым клавишам Caps Lock, Scroll Lock, Num Lock</p></li>
</ul>
<p>Например, обработаем событие KeyDown для текстового поля и выведем данные о нажатой клавише в текстовый блок:</p>
<pre class="brush:xml;">
...
<p>А в файле кода пропишем обработчик <code>TextBox_KeyDown</code>:</p>
<pre class="brush:c#;">
...
<p>Здесь в текстовый блок добавляется текстовое представление нажатой клавиши в текстовом поле:</p>
<img src="./pics/6.4.png" alt="События клавиатуры в WPF" />
<p>Правда, в данном случае реальную пользу от текстового представления мы можем получить только для алфавитно-цифровых клавиш, в то время как при нажатии специальных клавиш или кавычек 
будут добавляться их полные текстовые представления, например, для кавычек - OemQuotes.</p>
<p>Если нам надо отловить нажатие какой-то опредленной клавиши, то мы можем ее проверить через перечисление <span class="b">Key</span>:</p>
<pre class="brush:c#;">
...
<p>Объект <code>KeyboardDevice</code> позволяет нам получить ряд дополнительных данных о собыиях клавиатуры через ряд свойств и методов:</p>
<ul>
<li><p><span class="b">Modifiers</span> позволяет узнать, какая клавиша была нажата вместе с основной (Ctrl, Shift, Alt)</p></li>
<li><p><span class="b">IsKeyDown()</span> определяет, была ли нажата определенная клавиша во время события</p></li>
<li><p><span class="b">IsKeyUp()</span> позволяет узнать, была ли отжата определенная клавиша во время события</p></li>
<li><p><span class="b">IsKeyToggled()</span> позволяет узнать, была ли во время события включена клавиша Caps Lock, Scroll Lock или Num Lock</p></li>
<li><p><span class="b">GetKeyStates()</span> возвращает одно из значений перечисления KeyStates, которое указывает на состояние клавиши</p></li>
</ul>
<P>Пример использования KeyEventArgs при одновременном нажатии двух клавиш Shift и F1:</P>
<pre class="brush: c#;">
private void TextBox_KeyDown(object sender, KeyEventArgs e)
{
...
<p>События TextInput/PreviewTextInput в качестве параметра принимают объект <span class="b">TextCompositionEventArgs</span>. Из его свойств стоит отметить, пожалуй, 
только свойство <code>Text</code>, которое получает введенный текст, именно текст, а не текстовое представление клавиши. Для этого добавим к текстовому полю обработчик:</p>
<pre class="brush:xml;">
&lt;TextBox Height="40" Width="260" PreviewTextInput="TextBox_TextInput" /&gt;
</pre>
<p>И определим обработчик в файле кода:</p>
<pre class="brush:c#;">
...
<p>Причем в данном случае я обрабатываю именно событие PreviewTextInput, а не TextInput, так как элемент TextBox подавляет событие TextInput, и вместо него генерирует 
событие TextChanged. Для большинства других элементов управления, например, кнопок, событие TextInput прекрасно срабатывает.</p>
<h3>Валидация текстового ввода</h3>
<p>События открывают нам большой простор для валидации текстового ввода. Нередко при вводе используются те или иные ограничения: нельзя вводить цифровые символы или, наоборот, можно только цифровые и т.д. Посмотрим, 
как мы можем провети валидацию ввода. К примеру, возьмем ввод номера телефона. Сначала зададим обработку двух событий в xaml:</p>
<pre class="brush:xml;">&lt;TextBox PreviewTextInput="TextBox_PreviewTextInput" PreviewKeyDown="TextBox_PreviewKeyDown"  /&gt;</pre>
<p>И определим в файле кода обработчики:</p>
<pre class="brush:c#;">
...
<p>Для валидации ввода нам надо использовать обработчики для двух событий - PreviewKeyDown и PreviewTextInput. Дело в том, что нажатия не всех клавиш PreviewTextInput 
обрабатывает. Например, нажатие на клавишу пробела не обрабтывается. Поэтому также применяется обработка и PreviewKeyDown.</p>
<p>Сами обработчики проверяют ввод и если ввод соответствует критериям, то он отклоняется с помощью установки <span class="b">e.Handled = true</span>. Тем самым мы 
говорим, что событие обработано, а введенные текстовые сиволы не будут появляться в текстовом поле. Конкретно в данном случае пользователь может вводить только 
цифровые символы и пробел в соответствии с форматом телефонного номера.</p>
...
<div class="nav"><p><a href="6.php">Назад</a><a href="./">Содержание</a><a href="6.3.php">Вперед</a></p></div></div>
<div class="socBlock">
