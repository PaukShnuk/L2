...
<html >
<head>
<title>C# и WPF | Графические пути и геометрии</title>
<meta charset="utf-8" />
<meta name="description" content="Графические пути в WPF, создание сложных фигур с помощью объекта Path, классы геометрий, комбинирование геометрий посредством классов GeometryGroup и CombinedGeometry">
...
<h2>Пути и геометрии</h2><div class="date">Последнее обновление: 04.03.2016</div>
...
Поэтому для этих целей применяется класс <span class="b">Path</span>, который представляет геометрический путь. Он также, как и фигуры, наследуется от 
класса Shape, но может заключать в себе совокупность объединенных фигур. Класс Path имеет свойство <span class="b">Data</span>, которое определяет объект Geometry  - геометрический объект для отрисовки. Этот объект задает фигуру или совукупность фигур для отрисовки.</p>
...
<li><p><span class="b">EllipseGeometry</span> представляет эллипс, эквивалент фигуры Ellipse</p></li>
...
<li><p><span class="b">CombinedGeometry</span> создает фигуру, состоящую из двух объектов Geometry</p></li>
<li><p><span class="b">StreamGeometry</span> - специальный объект Geometry, предназначенный для сохранения всего геометрического пути в памяти</p></li>
</ul>
<p>Например, использование LineGeometry:</p>
<pre class="brush:xml;">
&lt;Path Stroke="Blue"&gt;
    &lt;Path.Data&gt;
...
<p>будет аналогично следующему объекту Line:</p>
<pre class="brush:xml;">
&lt;Line X1="100" Y1="30" X2="200" Y2="130" Stroke="Blue" /&gt;
</pre>
<p>Свойства <code>StartPoint</code> и <code>EndPoint</code> задают начальную и конечную точки линии.</p>
...
<p>Свойство <code>Rect</code> задает параметры прямоугольника в формате "координата X, координата Y  ширина, высота". Также с помощью свойств <code>RadiusX</code> и 
<code>RadiusY</code> можно задать радиус скругления углов прямоугольника.</p>
<img src="./pics/16.14.png" alt="RectangleGeometry в WPF" />
<p>EllipseGeometry:</p>
<pre class="brush:xml;">
...
<p>Свойство <code>Center</code> устанавливает цетр овала, а свойста <code>RadiusX</code> и <code>RadiusY</code> - радиусы.</p>
<p>GeometryGroup объединяет несколько геометрий:</p>
<pre class="brush: xml;">
&lt;Path Fill="LightPink" Stroke="LightBlue"&gt;
    &lt;Path.Data&gt;
...
<p>Объект GeometryGroup устанавливает свойство <span class="b">FillRule</span>. Если оно равно <span class="b">EvenOdd</span> (значение по умолчанию), 
то перекрывающиеся поверхности двух геометрий являются прозрачными. А при значении <span class="b">FillRule="Nonzero"</span> (как в данном случае), 
перекрывающиеся поверхности геометрий будут окрашены также, как и остальные части пути.</p>
<img src="./pics/16.15.png" alt="GeometryGroup в WPF" />
<h3>CombinedGeometry</h3>
<p>CombinedGeometry состоит из двух геометрий. В этом он похож на GeometryGroup, который также может объединять две геометрии. Однако между ними есть различия. 
Отличие состоит в том, что объект CombinedGeometry имеет свойство <span class="b">GeometryCombinedMode</span>, которое указывает модель перекрытия двух 
геометрий:</p>
<ul>
<li><p><span class="b">Union</span>: фигура включает обе геометрии</p></li>
<li><p><span class="b">Intersect</span>: фигура включает область, которая одновременно принадлежит обеим геометриям</p></li>
<li><p><span class="b">Xor</span>: фигура включает только непересекающие области геометрий</p></li>
<li><p><span class="b">Exclude</span>: фигура включает первую геометрию с исключением тех областей, которые принадлежат также и второй геометрии</p></li>
</ul>
<p>Применим все способы:</p>
<pre class="brush:xml;">
&lt;Grid&gt;
    &lt;Grid.RowDefinitions&gt;
        &lt;RowDefinition /&gt;
...
<img src="./pics/16.16.png" alt="CombinedGeometry в WPF" />
...
<div class="nav"><p><a href="17.php">Назад</a><a href="./">Содержание</a><a href="17.3.php">Вперед</a></p></div></div>
<div class="socBlock">
