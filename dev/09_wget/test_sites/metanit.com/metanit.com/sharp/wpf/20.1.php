...
<html >
<head>
<title>C# и WPF | Окна и класс Window</title>
<meta charset="utf-8" />
<meta name="description" content="Создание окон в WPF, класс Window, его свойства для настройки окон, жизненный цикл и основные события окон приложения">
...
<h1>Окна</h1><h2>Класс Window</h2><div class="date">Последнее обновление: 21.03.2016</div>
...
<p>Ключевым элементом в системе графического интерфейса в WPF является окно, которое содержит все необходимые элементы управления. 
Окно в WPF представлено классом <span class="b">Window</span>, который является производным от класса ContentControl. Поэтому окно является элементом управления содержимым, и как, к примеру, 
кнопка, может содержать в себе один дочерний элемент. Как правило, в его качестве выступает один из элементов компоновки, например, Grid.</p>
<p>Класс Window привносит ряд свойств, которые позволяют настроить окно приложения:</p>
<ul>
<li><p><span class="b">AllowsTransparency</span>: при значении <code>true</code> позволяет установить прозрачный фон окна</p></li>
<li><p><span class="b">Icon</span>: представляет иконку, которая отображается в левом верхнем углу окна и в панели задач. Если иконка не установлена, 
то система будет использовать стандартную иконку по умолчанию.</p></li>
<li><p><span class="b">Top</span>: устанавливает отступ окна приложения от верхней границы экрана</p></li>
<li><p><span class="b">Left</span>: устанавливает отступ окна приложения от левой границы экрана</p></li>
<li><p><span class="b">ResizeMode</span>: задает режим изменения размеров окна. Может принимать следующие значения:</p>
	<ul>
	<li><p><code>CanMinimize</code>: окно можно только свернуть</p></li>
	<li><p><code>NoResize</code>: у окна нельзя изменить начальные размеры</p></li>
	<li><p><code>CanResize</code>: у окна можно изменять размеры</p></li>
	<li><p><code>CanResizeWithGrip</code>: в правом нижнем углу окна появляется визуализация того, что у окна можно изменять размеры</p></li>
	</ul>
</li>
<li><p><span class="b">RestoreBounds</span>: возвращает границы окна</p></li>
<li><p><span class="b">ShowInTaskbar</span>: при значении <code>true</code> иконка окна отображается на панели задач</p></li>
<li><p><span class="b">SizeToContent</span>: позволяет автоматически масштабировать размеры окна в зависимости от содержимого. Может принимать 
следующие значения:</p>
<ul>
	<li><p><code>Width</code>: автоматически масштабируется только ширина</p></li>
	<li><p><code>Height</code>: автоматически масштабируется только высота</p></li>
	<li><p><code>WidthAndHeight</code>: автоматически масштабируются высота и ширина</p></li>
	<li><p><code>Manual</code>: автоматическое масштабирование отсутствует</p></li>
	</ul>
</li>
<li><p><span class="b">Title</span>: заголовок окна</p></li>
<li><p><span class="b">Topmost</span>: при значении <code>true</code> окно устанавливается поверх других окон приложения</p></li>
<li><p><span class="b">WindowStartupLocation</span>: устанавливает стартовую позицию окна. Может принимать следующие значения:</p>
<ul>
	<li><p><code>CenterOwner</code>: если данное окно было запущено другим окном, то данное окно позиционируется относительно центра запустившего его окна</p></li>
	<li><p><code>CenterScreen</code>: окно помещается в центре экрана</p></li>
	<li><p><code>Manual</code>: позиция устанавливается вручную с помощью свойств Top и Left</p></li>
</ul>
</li>
<li><p><span class="b">WindowState</span>: состояние окна. Возможные значения:</p>
<ul>
	<li><p><code>Maximized</code>: раскрыто на весь экран</p></li>
	<li><p><code>Minimized</code>: свернуто</p></li>
	<li><p><code>Normal</code>: стандартное состояние</p></li>
</ul>
</li>
</ul>

<h3>Жизненный цикл</h3>
<p>В процессе работы окно в WPF проходит ряд этапов жизненного цикла, которые доступны нам через обработку событий класса Window:</p>
<ol>
<li><p><span class="b">Initialized</span>: это событие возникает при инициализации окна, когда у него устанавливаются 
...
То есть событие Initialized окна приложения генерируется только после того, как отработает событие Initialized для всех вложенных элементов.</p></li>
<li><p><span class="b">Loaded</span>: возникает после полной инициализации окна и применения к нему стилей и привязки данных. 
После генерации этого события происходит визуализация элемента, и окно отображается на экране и становится видимым для пользователя</p></li>
<li><p><span class="b">Closing</span>: возникает при закрытии окна</p></li>
<li><p><span class="b">Closed</span>: возникает, когда окно становится закрытым</p></li>
<li><p><span class="b">Unloaded</span>: возникает после закрытия окна при выгрузке всех связанных ресурсов из памяти</p></li>
</ol>
<img src="./pics/20.1.png" alt="Жизненный цикл окна приложения в WPF" />
<p>Соответственно, если нам надо выполнить некоторые действия при загрузке или при закрытии окна, мы можем обработать события Loaded и Closing/Closed. Например, 
запишем в текстовый лог события жизненного цикла:</p>
<pre class="brush:c#;">
using System;
...
<div class="nav"><p><a href="16.5.php">Назад</a><a href="./">Содержание</a><a href="20.2.php">Вперед</a></p></div></div>
<div class="socBlock">
