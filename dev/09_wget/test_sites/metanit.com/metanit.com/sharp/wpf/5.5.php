...
<html >
<head>
<title>C# и WPF | Текстовые элементы</title>
<meta charset="utf-8" />
<meta name="description" content="Обзор элементов, которые предназначены для вывода или ввода текста - TextBlock, TextBox, RichTextBox и PasswordBox">
...
<h2>Текстовые элементы управления</h2><div class="date">Последнее обновление: 18.01.2016</div>
...
<h3>TextBlock</h3>
<p>Элемент предназначен для вывода текстовой информации, для создания простых надписей:</p> 
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>Ключевым свойством здесь является свойство <code>Text</code>, которое задает текстовое содержимое. Причем в случае <code>&lt;TextBlock&gt;Текст1&lt;/TextBlock&gt;</code> 
данное свойство задается неявно.</p>
<p>С помощью таких свойств, как FontFamily, TextDecorations и др., мы можем настроить отображение текста. Однако мы можем задать и более сложное форматирование, например:</p>
<pre class="brush:xml;">
...
<img src="./pics/4.12.png" alt="TextBlock в WPF" />
<p>Элементы <code>Run</code> представляют куски обычного текста, для которых можно задать отдельное форматирование.</p>
<p>Для изменения параметров отображаемого текста данный элемент имеет такие свойства, как <span class="b">LineHeight</span>, 
<span class="b">TextWrapping</span> и <span class="b">TextAlignment</span>.</p>
<p>Свойство <span class="b">LineHeight</span> позволяет указывать высоту строк.</p>
<p>Свойство <span class="b">TextWrapping</span> позволяет переносить текст при установке этого свойства <code>TextWrapping="Wrap"</code>. По умолчанию это свойство имеет 
значение <code>NoWrap</code>, поэтому текст не переносится.</p>
<p>Свойство <span class="b">TextAlignment</span> выравнивает текст по центру (значение Center), правому (Right) или левому краю (Left): <code>&lt;TextBlock TextAlignment="Right"&gt;</code></p>
<p>Для декорации текста используется свойство <span class="b">TextDecorations</span>, например, если <code>TextDecorations="Underline"</code>, то текст будет подчеркнут.</p>
<p>Если нам вдруг потребуется перенести текст на другую строку, то тогда мы можем использовать элемент <code>LineBreak</code>:</p>
<pre class="brush:xml;">
&lt;TextBlock&gt;
...
<h3>TextBox</h3>
<p>Если TextBlock просто выводит статический текст, то этот элемент представляет поле для ввода текстовой информации.</p>
<p>Он также, как и TextBlock, имеет свойства <span class="bb">TextWrapping</span>, <span class="bb">TextAlignment</span> и <span class="bb">TextDecorations</span>.</p>
<p>С помощью свойства <span class="bb">MaxLength</span> можно задать предельное количество вводимых символов.</p>
<pre class="brush:xml;">
&lt;TextBox MaxLength="250" TextChanged="TextBox_TextChanged"&gt;Начальный текст&lt;/TextBox&gt;
</pre>
<p>В коде C# мы можем обработать событие изменения текста:</p>
<pre class="brush:xml;">
...
<p>По умолчанию, если вводимый текст превышает установленные границы поля, то текстовое поле растет, чтобы вместить весь текст. Но визуально это не очень 
...
TextWrapping="Wrap".</p>
<p>Чобы переводить по нажатию на клавишу Enter курсор на следующую строку, нам надо установить свойство <code>AcceptsReturn="True"</code>.</p>
<p>Также мы можем добавить полю возможность создавать табуляцию с помощью клавиши Tab, установив свойство <code>AcceptsTab="True"</code></p>
<p>Для отображения полос прокрутки TextBox поддерживает свойства <span class="bb">VerticalScrollBarVisibility</span> и <span class="bb">НоrizontalScrollBarVisibility</span>:</p>
<pre class="brush:xml;">
...
<p>Возможно, при создании приложения нам потребуется сделать текстовое поле недоступным для ввода (на время в зависимости от условий или вообще), 
тогда для этого нам надо установить свойство <code>IsReadOnly="True"</code>.</p> 
<p>Для выделения текста есть свойства <span class="bb">SelectionStart</span>, <span class="bb">SelectionLength</span> и <span class="bb">SelectionText</span>. 
Например, выделим программно текст по нажатию кнопки:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>TextBox обладает встроенной поддержкой орфографии. Чтобы ее задействовать, надо установить свойство <code>SpellCheck.IsEnabled="True"</code>. 
...
явным образом указать через свойство <span class="b">Language</span>:</p>
<pre class="brush:xml;">
&lt;DockPanel&gt;
    &lt;TextBox SpellCheck.IsEnabled="True" Language="ru-ru"&gt;Привет, как дила?&lt;/TextBox&gt;
&lt;/DockPanel&gt;
</pre>
<img src="./pics/4.44.png" alt="Проверка орфографии в WPF" />
<h3>Метка (Label)</h3>
<p>Главной особенностью меток является поддержка мнемонических команд-клавиш быстрого доступа, которые передают фокус связанному элементу. Например, </p>
<pre class="brush:xml;">&lt;Label Target="{Binding ElementName=TextBox1}"&gt;_привет&lt;/Label&gt;
&lt;TextBox Name="TextBox1" Margin="0 30 0 0" Height="30" Width="100"&gt;&lt;/TextBox&gt;
</pre>
<p>Теперь, нажав на клавишу "п", мы переведем фокус на связанное текстовое поле. При вызове приложения подчеркивание не отображается, чтобы отображать подчеркивание, надо нажать на клавишу Alt. 
Тогда чтобы перевести фокус на связанное текстовое поле необходимо будет нажать сочетание Alt + "п". 
Если не предполагается использование клавиш быстрого доступа, то для вывода обычной текста вместо меток лучше использовать элемент TextBlock.</p>
<h3>PasswordBox</h3>
<p>Элемент предназначен для ввода парольной информации. По сути это тоже текстовое поле, только для ввода символов используется маска. 
Свойство <span class="b">PasswordChar</span> устанавливает символ маски, отображаемый при вводе пароля. Если это свойство не задано, то по умолчанию 
для маски символа используется черная точка. 
Свойство <span class="b">Password</span> устанавливает парольную строку, отображаемую по умолчанию при загрузке окна приложения.</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<img src="./pics/4.45.png" alt="PasswordBox в WPF" />
<h3>RichTextBox</h3>
<p>Для вывода текстового содержимого, насыщенного форматированием, графикой, предназначен RichTextBox. Можно даже сказать, что он выводит не 
...
простой текстовый редактор, мы поговорим в главе, посвященной документам.</p>
...
<div class="nav"><p><a href="5.20.php">Назад</a><a href="./">Содержание</a><a href="5.6.php">Вперед</a></p></div></div>
<div class="socBlock">
