...
<html >
<head>
<title>C# и WPF | Работа с Entity Framework</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с Entity Framework в WPF, подключение к базе данных MS SQL Server, создание CRUD-интерфейса">
...
<h2>Работа с Entity Framework</h2><div class="date">Последнее обновление: 28.02.2016</div>
...
<p>Entity Framework представляет ORM-технологию, которая позволяет абстрагироваться от структуры базы данных и может выполнять автоматически сопоставление 
таблиц и их данных с моделями классов, определенных разработчиком. Более подробно про эту технологию можно прочитать в руководстве по <a href="../entityframework/">Entity Framework</a>, 
здесь же мы рассмотрим лишь, как сделать простейшее приложение с использованием этого инструмента.</p>
<p>Вначале создадим новый проект. Пусть он называется <span class="b">EfDbApp</span>. Первым делом нам надо добавить все необходимые пакеты Entity Framework в наш проект. 
Для этого в структуре проекта нажмем правой кнопкой мыши на узел <span class="b">References</span> (Библиотеки) и в контекстном меню выберем пункт <span class="b">Manage NuGet Packages...</span>:</p>
<img src="./pics/19.9.png" alt="Добавление Entity Framework в WPF" />
<p>В открывшемся менеджере пакетов NuGet выберем пакет Entity Framework и установим его:</p>
<img src="./pics/19.10.png" alt="NuGet в WPF" />
<p>Затем определим строку подключения в файле <i>App.config</i>:</p>
<pre class="brush:xml;">
...
<p>В данном случае мы будем использовать ту же базу данных <span class="b">mobiledb</span>, которую создали в предыдущих темах. Однако даже если у нас нет базы данных mobiledb, то при первом обращении к базе данных Entity Framework создаст ее автоматически.</p>
<p>Теперь добавим в проект новую папку, которую назовем <i>Models</i> и в которой определим новый класс Phone:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Для взаимодействия с базой данных через Entity Framework нам нужен контекст данных, поэтому добавим в папку Models еще один класс, который назовем 
MobileContext:</p>
<pre class="brush:c#;">
...
<p>Класс контекста наследуется от класса DbContext. В своем конструкторе он передает в конструктор базового класса название строки подключения из файла 
<i>App.config</i>. Также в контексте данных определяется свойство по типу <code>DbSet&lt;Phone&gt;</code> - через него мы будем взаимодействовать с таблицей, 
которая хранит объекты Phone.</p>
<p>Теперь определим разметку интерфейса:</p>
<pre class="brush:xml;">
...
<p>Как и в прошлой теме, на форме будет один элемент DataGrid и две кнопки для обновления и удаления.</p>
<img src="./pics/19.8.png" />
<p>Теперь определим в файле кода c# привязку данных и обработчики кнопок:</p>
<pre class="brush:c#;">
...
<p>С помощью следующих строк здесь мы загружаем данные из бд и выполняем привязку к ним элемента DataGrid:</p>
<pre class="brush:c#;">
db.Phones.Load();
phonesGrid.ItemsSource = db.Phones.Local.ToBindingList();
</pre>
<p>После окончания работы с контекстом данных нам нужно его утилизовать, а именно вызвать его метод <code>Dispose()</code>. Подходящим местом для этого 
является обработчик события <code>Closing</code> класса MainWindow.</p>
<p>И как и в прошлой теме, здесь нам достаточно ввести данные в строку грида, чтобы создать новый объект. Также мы можем изменять уже существующие значения 
в ячейках грида. И чтобы сохранить все изменения, в обработчике кнопки обновления вызывается метод <span class="b">db.SaveChanges()</span>.</p>
<p>Для удаления данных сначала вызывается метод <code>db.Phones.Remove(phone)</code> для всех удаляемых объектов, а затем также метод db.SaveChanges.</p>
...
<div class="nav"><p><a href="19.2.php">Назад</a><a href="./">Содержание</a><a href="15.php">Вперед</a></p></div></div>
<div class="socBlock">
