...
<html >
<head>
<title>C# и WPF | Валидация данных</title>
<meta charset="utf-8" />
<meta name="description" content="Механизм валидации, использовнаие класса ExceptionValidationRule, создание своей логики валидации через IDataErrorInfo, вывод ошибок валидации">
...
<h2>Валидация данных</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>При работе с данными важную роль играет валидация данных. Прежде чем использовать полученные от пользователя данные, нам надо убедиться, 
...
<span class="b">ExceptionValidationRule</span>. Этот класс обозначает введенные данные как некорректные, если в процессе ввода возникает какое-либо исключение, 
например, исключение преобразования типов.</p>
<p>Итак, допустим, у нас определен следующий класс:</p>
<pre class="brush: c#;">
...
<p>Этот класс представляет человека и предполагает три свойства: имя, возраст и должность. Понятно, что возраст должен представлять числовое значение. 
Однако пользователи могут ввести что угодно. Мы можем обрабатывать ввод с клавиатуры, а можем воспользоваться классом <code>ExceptionValidationRule</code>, 
который в случае неудачи преобразования строки в число установит красную границу вокруг текстового поля.</p>
<P>Сначала создадим в файле кода объект нашего класса PersonModel и установим контекст данных окна:</P>
<pre class="brush: c#;">
...
<p>Теперь установим привязку в xaml-коде:</p>
<pre class="brush:xml;">
...
<p>В данном случае мы задаем объект Binding для свойства Text. Данный объект имеет коллекцию правил валидации вводимых данных - <span class="b">ValidationRules</span>. 
Эта коллекция принимает только одно правило валидации, представленное классом <span class="b">ExceptionValidationRule</span>. Запустим приложение на выполнение 
...
что в вводимых данных имеются ошибки.</p>
<img src="./pics/14.10.png" alt="Валидация данных в WPF" />
<p>Мы также можем реализовать свою логику валидации для класса модели. Для этого модель должна реализовать интерфейс <span class="b">IDataErrorInfo</span>. Этот интерфейс 
...
<p>Допустим, мы хотим ограничить возраст человека только положительными значениями от 0 до 100. Тогда валидация модели будет выглядеть следующим образом:</p>
<pre class="brush: c#;">
...
<p>И последнее - нам осталось немного подкорректировать xaml-код. Теперь нам надо использовать в качестве правила валидации класс 
<span class="b">DataErrorValidationRule</span>:</p>
<pre class="brush:xml;">
&lt;TextBox Grid.Column="1" Grid.Row="1" Height="30"  Margin="0 0 15 0"&gt;
    &lt;TextBox.Text&gt;
        &lt;Binding Path="Age"&gt;
...
<img src="./pics/14.11.png" />
<p>Так как число 450 больше 100 и поэтому не является валидным, то текстовое поле выделяется красным.</p>
<h3>Настройка внешнего вида при ошибке валидации</h3>
<p>Но это еще не все. Мы можем сами управлять через шаблоны отображением ошибки ввода. В предыдущем случае у нас граница текстового поля при ошибке 
окрашивалась в красный цвет. Попробуем настроить данное действие. Для этого нам нужно использовать элемент <span class="b">AdornedElementPlaceholder</span>. 
Итак изменим разметку приложения следующим образом, добавив в нее шаблон элемента управления:</p>
<pre class="brush:xml;">
&lt;Window x:Class="DataValidationApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<p>С помощью свойства <span class="b">Validation.ErrorTemplate</span> мы получаем шаблон, который будет отрабатывать при ошибке валидации. 
...
восклицательный знак красного цвета. Запустим приложение и попробуем ввести в текстовое поле какое-нибудь некорректное значение. В результате сработает наш шаблон:</p>
<img src="./pics/14.12.png" alt="Настройка валидации в WPF" />
<p>Мы также можем определить поведение и визуализацию через триггер при установке свойства <span class="bb">Validation.HasError</span> в True. А с помощью свойства 
ToolTip можно создать привязку к сообщению ошибки:</p>
<pre class="brush:xml;">
...
<img src="./pics/14.13.png" alt="Всплывающая подсказка при валидации в WPF" />
<h3>Обработка событий валидации</h3>
<p>WPF предоставляет механизм обработки ошибки валидации с помощью события <code>Validation.Error</code>. Данное событие можно использовать в любом элементе управления. 
Например, пусть при ошибке валидации при вводе в текстовое поле выскакивает сообщение с ошибкой. Для этого изменим текстовое поле следующим образом:</p>
<pre class="brush:xml;">
...
<p>Здесь, во-первых, надо отметить установку свойства <code>NotifyOnValidationError="True"</code>:</p>
<pre class="brush:xml;">&lt;Binding Path="Age" NotifyOnValidationError="True"&gt;</pre>
<p>Это позволит вызывать событие валидации.</p>
<p>И также устанавливается сам обработчик события валидации:</p>
<pre class="brush:xml;">
&lt;TextBox Grid.Column="1" Grid.Row="1" Height="30"  Margin="0 0 15 0" 
	Validation.Error="TextBox_Error"&gt;
</pre>
<p>При этом следует отметить, что событие <code>Validation.Error</code> является поднимающимся (bubbling events), поэтому мы можем установить для него обработчик и в контейнере Grid или в любых других контейнерах, 
в которых находится это текстовое поле. И в случае ошибки событие также будет генерироваться и обрабатываться.</p>
<p>И в конце определим в файле кода c# сам обработчик:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="14.8.php">Назад</a><a href="./">Содержание</a><a href="19.1.php">Вперед</a></p></div></div>
<div class="socBlock">
