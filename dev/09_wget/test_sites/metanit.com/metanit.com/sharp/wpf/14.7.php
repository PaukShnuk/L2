...
<html >
<head>
<title>C# и WPF | XmlDataProvider</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение к файлу XML в WPF, применение объекта XmlDataProvider">
...
<h2>XmlDataProvider</h2><div class="date">Последнее обновление: 26.02.2016</div>
...
<p>XmlDataProvider используется для подключения к xml-документам. Причем xml-документом в данном случае может быть и локальный xml-файл, и xml из интернета. 
Для организации связи с источником данных XmlDataProvider использует следующие свойства:</p>
<ul>
<li><p><span class="b">Source</span>: устанавливает источник данных</p></li>
<li><p><span class="b">XPath</span>: задает путь внутри документа xml к целевому набору данных</p></li>
<li><p><span class="b">Document</span>: устанавливает ссылку на xml-документ</p></li>
<li><p><span class="b">IsAsynchronous</span>: при значении true загружает данные асинхронно</p></li>
</ul>
<p>К примеру, определим в проекте файл <i>phones.xml</i>:</p>
<pre class="brush:xml;">
...
<p>Теперь обратимся к этому файлу через XmlDataProvider:</p>
<pre class="brush:xml;">
...
<p>XmlDataProvider определяется как ресурс. Его свойству <code>Source</code> присваивается название xml-файла. Свойство <code>XPath</code> 
указывает, что все данные для провайдера в файле будет находиться в рамках элемента <code>&lt;phones&gt;</code>.</p>
<p>В элементе ListBox как обычно устанавливаем свойство <code>ItemsSource</code>, при этом используя параметр привязки <span class="b">XPath</span>. В данном случае он 
указывает на элемент из xml, на основе которого будут создаваться элементы в ListBox, то есть элемент <code>&lt;phone&gt;</code>.</p>
<p>Здесь важно отметить, что в XPath используются регистрозависимые строки. То есть в файле xml определен элемент <code>&lt;phones&gt;</code> - с маленькой буквы, 
поэтому в XPath также используется название с маленькой буквы: <code>XPath="phones"</code>. То же самое касается и свойства <code>DisplayMemberPath="title"</code>.</p>
<p>В итоге у нас получится следующий список:</p>
<img src="./pics/14.9.png" alt="XmlDataProvider в WPF" />
...
<div class="nav"><p><a href="14.6.php">Назад</a><a href="./">Содержание</a><a href="14.8.php">Вперед</a></p></div></div>
<div class="socBlock">
