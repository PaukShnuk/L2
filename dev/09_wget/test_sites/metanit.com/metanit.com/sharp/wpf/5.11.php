...
<html >
<head>
<title>C# и WPF | Меню</title>
<meta charset="utf-8" />
<meta name="description" content="Создание меню в приложении WPF, использование элементов Menu и MenuItem, работа с контекстным меню и класс ContextMenu,">
...
<h2>Меню</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>Menu</h3>
<p>Данный элемент служит для создания стандартных меню:</p>
<pre class="brush: xml;">
...
<img src="./pics/4.19.png" alt="Элемент Menu в WPF" />
<p>Элемент Menu включает набор элементов MenuItem, которые опять же являются элементами управления содержимым и могут включать другие элементы MenuItem 
и не только. Также мы можем вложить в меню и другие элементы, которые неявно будут преобразованы в MenuItem. Например:</p>
<pre class="brush:xml;">
&lt;Menu Height="25" VerticalAlignment="Top"&gt;
    &lt;MenuItem Header="File"&gt;
		&lt;Button Content="Exit" /&gt;
...
<p>Также для разделения отдельных пунктов меню можно включать элемент <code>Separator</code>, как в примере выше.</p>
<p>Мы также можем настроить внешний вид отображения, задав свойство <b>MenuItem.Header</b> или использовав свойство Icons:</p>
<pre class="brush:xml;">
...
<p>Чтобы обработать нажатие пункта меню и произвести определенное действие, можно использовать событие <code>Click</code>, однако в будущем мы познакомимся 
с еще одним инструментом под названием команды, который также широко применяется для реакции на нажатие кнопок меню. А пока свяжем обработчик c событием:</p>
<pre class="brush:xml;">
&lt;MenuItem Header="View"  Click="MenuItem_Click"&gt;&lt;/MenuItem&gt;
</pre>
<p>И определим сам обработчик в коде C#:</p>
<pre class="brush:c#;">
...
<h3>ContextMenu</h3>
<p>Класс ContextMenu служит для создания контекстных всплывающих меню, отображающихся после нажатия на правую кнопку мыши. Этот элемент также содержит коллекцию элементов <code>MenuItem</code>. 
Однако сам по себе ContextMenu существовать не может и должен быть прикреплен к другому элементу управления. Для этого у элементов есть свойство 
<code>ContextMenu</code>:</p>
<pre class="brush:xml;">
...
<img src="./pics/4.21.png" alt="Элемент ContextMenu в WPF" />
<p>И при нажатии правой кнопкой мыши на один из элементов отобразится контекстное меню.</p>
