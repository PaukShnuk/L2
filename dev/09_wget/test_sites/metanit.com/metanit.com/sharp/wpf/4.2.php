...
<html >
<head>
<title>C# и WPF | Grid</title>
<meta charset="utf-8" />
<meta name="description" content="Контейнер Grid в WPF, создание строк и столбцов, установка их ширины и высоты, позиционирование дочерних элементов внутри грида">
...
<h2>Grid</h2><div class="date">Последнее обновление: 16.01.2016</div>
...
<p>Это наиболее мощный и часто используемый контейнер, напоминающий обычную таблицу. Он содержит столбцы и строки, количество которых задает разработчик. 
Для определения строк используется свойство <span class="b">RowDefinitions</span>, а для определения столбцов - свойство <span class="b">ColumnDefinitions</span>:</p>
<pre class="brush:xml;">
&lt;Grid.RowDefinitions&gt;
...
<p>Каждая строка задается с помощью вложенного элемента <code>RowDefinition</code>, который имеет открывающий и закрывающий тег. При этом задавать 
дополнительную информацию необязательно. То есть в данном случае у нас определено в гриде 3 строки.</p>
<p>Каждая столбец задается с помощью вложенного элемента <code>ColumnDefinition</code>. Таким образом, здесь мы определили 3 столбца. ТО есть в итоге 
у нас получится таблица 3х3.</p>
<p>Чтобы задать позицию элемента управления с привязкой к определенной ячейке Grid, в разметке элемента нужно прописать значения свойств <span class="b">Grid.Column</span> и 
<span class="b">Grid.Row</span>, тем самым указывая, в каком столбце и строке будет находиться элемент. 
...
<p>Атрибут <code>ShowGridLines="True"</code> у элемента Grid задает видимость сетки, по умолчанию оно равно False.</p>
<p>То есть у нас получится следующая картина:</p>
<img src="./pics/3.2.png" alt="Элемент Grid в WPF" />
...
Есть несколько вариантов настройки размеров.</p>
<h4>Автоматические размеры</h4>
<p>Здесь столбец или строка занимает то место, которое им нужно</p></li>
<pre class="brush:xml;">
&lt;ColumnDefinition Width="Auto" /&gt;
&lt;RowDefinition Height="Auto" /&gt;
</pre>
<h4>Абсолютные размеры</h4>
<p>В данном случае высота и ширина указываются в единицах, независимых от устройства:</p></li>
<pre class="brush:xml;">
&lt;ColumnDefinition Width="150" /&gt;
&lt;RowDefinition Height="150" /&gt;
</pre>
<p>Также абсолютные размеры можно задать в пикселях, дюймах, сантиметрах или точках:</p>
<table border="1"><tr><td><p>пиксели</p></td><td><p>px</p></td><tr>
...
<p>Если строка или столбец имеет высоту, равную *, то данная строка или столбце будет занимать все оставшееся место. Если у нас есть несколько 
...
на сумму коэффициентов.</p>
<p>Например, если у нас три столбца:</p>
<pre class="brush:xml;">
&lt;ColumnDefinition Width="*" /&gt;
&lt;ColumnDefinition Width="0.5*" /&gt;
&lt;ColumnDefinition Width="1.5*" /&gt;
</pre>
<p>В этом случае сумма коэффициентов равна 1* + 0.5* + 1.5* = 3*. Если у нас грид имеет ширину 300 единиц, то для коэфициент 1* будет соответствовать пространству 
300 / 3 = 100 единиц. Поэтому первый столбец будет иметь ширину в 100 единиц, второй - 100*0.5=50 единиц, а третий - 100 * 1.5 = 150 единиц.</p>
<p>Можно комбинировать все типы размеров. В этом случае от ширины/высоты грида отнимается ширина/высота столбцов/строк с абсолютными или автоматическими размерами, и 
затем оставшееся место распределяется между столбцами/строками с пропорциональными размерами:</p>
<img src="./pics/3.18.png" alt="Размеры в Grid в WPF" />
<h3>UniformGrid</h3>
<p>Аналогичен контейнеру Grid контейнер <span class="b">UniformGrid</span>, только в этом случае все столбцы и строки одинакового размера и используется упрощенный синтаксис для их определения: 
<pre class="brush:xml;">
...
<img src="./pics/3.3.png" alt="Контейнер UniformGrid в WPF" />
...
<div class="nav"><p><a href="4.php">Назад</a><a href="./">Содержание</a><a href="4.3.php">Вперед</a></p></div></div>
<div class="socBlock">
