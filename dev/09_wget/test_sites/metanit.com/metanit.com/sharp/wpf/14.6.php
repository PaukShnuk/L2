...
<html >
<head>
<title>C# и WPF | Провайдеры данных. ObjectDataProvider</title>
<meta charset="utf-8" />
<meta name="description" content="Провайдеры данных в WPF, применение объекта ObjectDataProvider для подключения к источникам данных">
...
<h2>Провайдеры данных. ObjectDataProvider</h2><div class="date">Последнее обновление: 26.02.2016</div>
...
<p>Для упрощения работы с источниками данных в WPF есть такая функциональность, как <span class="b">провайдеры данных</span>. Они позволяют связывать источники данных и 
элементы интерфейса. По умолчанию доступно два провайдера данных: ObjectDataProvider (для работы с объектами) и XmlDataProvider (для работы с данными из xml-файлов).</p>
<p><span class="b">ObjectDataProvider</span> позволяет использовать в качестве базы данных какой-нибудь локальный объект. Для установки подключения к объекту провайдер 
применяет следующие свойства:</p>
<ul>
<li><p><span class="b">ObjectType</span>: указывает на тип объекта, который будет выполнять роль источника</p></li>
<li><p><span class="b">MethodName</span>: указывает на метод объекта из свойства ObjectType</p></li>
<li><p><span class="b">ObjectInstance</span>: непосредственно сам объект-источник. В приложение можно одновременно использовать только одно из свойств: ObjectType, либо 
ObjectInstance</p></li>
<li><p><span class="b">IsAsynchronous</span>: при значении true получает данные от источника в асинхронном режиме</p></li>
</ul>
<p>К примеру, определим в файле кода класс, который представляет модель данных, и класс, который будет поставлять данные:</p>
<pre class="brush:c#;">
...
<p>Итак, класс окна MainWindow здесь абсолютно пустой, в нем не устанавливается никаких привязок или источников данных.</p>
<p>В качестве модели данных применяется класс Phone, а всю работу по поставке данных берет на себя класс PhoneRepository. Это стандартный репозиторий, 
усеченный до одного метода <code>GetPhones()</code>. Данный метод фактически выполняет роль источника данных.</p>
<p>Теперь в коде разметки xaml определим ObjectDataProvider, который свяжем с PhoneRepository:</p>
<pre class="brush:xml;">
...
<p>ObjectDataProvider определяется как обычный ресурс. В качестве типа объекта указываем тип PhoneRepository, а в качестве метода - GetPhones. Таким образом, 
ObjectDataProvider будет связан с той коллекцией объектов Phone, которая возвращается методом GetPhones.</p>
<p>Затем этот ресурс мы можем использовать как источник данных в элементе ListBox.</p>
<img src="./pics/14.8.png" alt="ObjectDataProvider в WPF" />
<p>Подобным образом можно использовать свойство ObjectInstance класса ObjectDataProvider для определения источника. Для этого для ObjectInstance надо 
установить ссылку на ресурс - коллекцию объектов:</p>
<pre class="brush:xml;">
&lt;Window x:Class="DataProvidersApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<div class="nav"><p><a href="14.5.php">Назад</a><a href="./">Содержание</a><a href="14.7.php">Вперед</a></p></div></div>
<div class="socBlock">
