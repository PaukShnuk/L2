...
<html >
<head>
<title>C# и WPF | DockPanel</title>
<meta charset="utf-8" />
<meta name="description" content="Использование контейнера DockPanel в WPF, прижатие элемента к определенному краю окна, особенности компоновки внутри DockPanel">
...
<h2>DockPanel</h2><div class="date">Последнее обновление: 16.01.2016</div>
...
<p>Этот контейнер прижимает свое содержимое к определенной стороне внешнего контейнера. Для этого у вложенных элементов надо установить сторону, 
к которой они будут прижиматься с помощью свойства <code>DockPanel.Dock</code>. Например,</p>
<pre class="brush:xml;">
...
<p>В итоге получаем массив кнопок, каждая из которых прижимается к определенной стороне элемента DockPanel:</p>
<img src="./pics/3.8.png" alt="Контейнер DockPanel в WPF" />
<p>Причем у последней кнопки мы можем не устанавливать свойство <code>DockPanel.Dock</code>. Она уже заполняет все оставшееся пространство. 
Такой эффект получается благодаря установке у DockPanel свойства <code>LastChildFill="True"</code>, которое означает, что последний элемент заполняет все оставшееся место. 
Если у этого свойства поменять True на False, то кнопка прижмется к левой стороне, заполнив только о место, которое ей необходимо.</p>
<p>Также обратите внимание на порядок прикрепления к кнопкам свойства <code>DockPanel.Dock</code>. Например, если мы изменим порядок на:</p>
<pre class="brush:xml;">
&lt;DockPanel LastChildFill="True"&gt;
    &lt;Button DockPanel.Dock="Top" Background="AliceBlue" Content="Верхняя кнопка" /&gt;
    &lt;Button DockPanel.Dock="Left" Background="Aquamarine" Content="Левая кнопка" /&gt;
...
<p>В этом случае нижняя кнопка уже будет заполнять меньшее место.</p>
<p>Мы также можем прижать к одной стороне сразу несколько элементов. В этом случае они просто будут располагаться по порядку:</p>
<pre class="brush:xml;">
&lt;DockPanel LastChildFill="True"&gt;
...
<img src="./pics/3.9.png" />
<p>Контейнер DockPanel особенно удобно использовать для создания стандартных интерфейсов, где верхнюю и левую часть могут занимать какие-либо меню, 
нижнюю - строка состояния, правую - какая-то дополнительная информация, а в центре будет находиться основное содержание.</p>
