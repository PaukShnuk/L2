...
<html >
<head>
<title>C# и WPF | Привязка данных и контекст данных</title>
<meta charset="utf-8" />
<meta name="description" content="Привязка данных в WPF, подключение к контексту данных, свойство DataContext, вложенные контексты данных">
...
<h1>Работа с данными</h1><h2>Привязка данных и контекст данных</h2><div class="date">Последнее обновление: 26.02.2016</div>
...
<p>Большую роль при работе с данными играет механизм привязки. Ранее в одной из прошлых тем рассматривалась привязка элементов и их свойств. В этой же 
теме сделаем больший упор на привязку данных.</p>
<p>Для создания привязки применяется элемент <span class="b">Binding</span> и его свойства:</p>
<ul>
<li><p><span class="b">ElementName</span>: имя элемента, к которому идет привязка. Если мы говорим о привязке данных, то данное свойство задействуется редко за 
исключением тех случаев, когда данные определены в виде свойства в определенном элементе управления</p></li>
<li><p><span class="b">Path</span>: ссылка на свойство объекта, к которому идет привязка</p></li>
<li><p><span class="b">Source</span>: ссылка на источник данных, который не является элементом управления</p></li>
</ul>
<p>Свойства элемента Binding помогают установить источник привязки. Для установки источника или контекста данных в элементах управления WPF предусмотрено 
свойство <span class="b">DataContext</span>. Рассмотрим на примерах их использование.</p>
<p>Пусть у нас в проекте определен следующий класс Phone:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Это сложный класс, который включает кроме простых данных типа string и decimal также и сложный объект Company.</p>
<p>Определим в классе окна MainWindow свойство, которое будет представлять объект Phone:</p>
<pre class="brush:c#;">
...
<p>Здесь установлено свойство <code>DataContext</code> класса MainWindow, после чего мы сможем получить значения из MyPhone в любом элементе в пределах MainWindow:</p>
<pre class="brush:xml;">
...
<p>Причем контекст данных переходит от корневых элементов к вложенным вниз по логическому дереву. Так, мы установили в качестве контекста для всего окна объект MyPhone. 
Однако элементы внутри окна могут конкретизировать контекст, взять какую-то его часть:</p>
<pre class="brush:xml;">&lt;TextBlock DataContext="{Binding Path=Company}" Text="{Binding Path=Title}" /&gt;</pre>
<p>Данный текстовый блок устанавливает в качестве контекста объект Company из общего контекста MyPhone:</p>
<img src="./pics/14.1.png" alt="Контекст данных для элементов WPF" />
<p>В тоже время нам необязательно конкретизировать контекст для текстового блока, вместо этого мы могли бы с помощью нотации точки обратиться к 
вложенным свойствам:</p>
<pre class="brush:xml;">&lt;TextBlock Text="{Binding Path=Company.Title}" /&gt;</pre>
<p>При этом надо учитывать, что когда мы определяем простое свойство объекта в коде c#, то установить в качестве контекста данных мы можем его только там же в коде C#, 
...
мы сможем устанавливать контекст данных и в коде xaml:</p>
<pre class="brush:c#;">
public partial class MainWindow : Window
{
...
<p>В данном случае контекст данных уже не устанавливается, а вместо обычного свойства определено свойство зависимостей. Тогда в коде xaml мы можем обратиться 
к этому свойству следующим образом:</p>
<pre class="brush:xml;">
&lt;Window x:Class="DataApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<p>Для ссылки на свойство необходимо установить имя окна: <code>Name="mainWindow"</code>. Также мы могли бы сделать то же самое, используя контекст данных:</p>
<pre class="brush:xml;">
...
<h3>Подключение к ресурсам</h3>
<p>Также в качестве контекста данных можно установить какой-нибудь ресурс. Например:</p>
<pre class="brush:xml;">
&lt;Window x:Class="DataApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<img src="./pics/14.2.png" alt="Ресурсы и DataContext в WPF" />
...
<div class="nav"><p><a href="12.4.php">Назад</a><a href="./">Содержание</a><a href="14.2.php">Вперед</a></p></div></div>
<div class="socBlock">
