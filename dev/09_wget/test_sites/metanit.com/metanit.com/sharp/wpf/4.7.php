...
<html >
<head>
<title>C# и WPF | Canvas</title>
<meta charset="utf-8" />
<meta name="description" content="Контейнер Canvas в WPF, расположение элементов по точным координатам, использование свойств Canvas.Top, Canvas.Bottom, Canvas.Right и Canvas.Left для позиционирования">
...
<h2>Canvas</h2><div class="date">Последнее обновление: 16.01.2016</div>
...
<p>Контейнер Canvas является наиболее простым контейнером. Для размещения на нем необходимо указать для элементов точные координаты относительно сторон Canvas. 
Для установки координат элементов используются свойства <code>Canvas.Left</code>, <code>Canvas.Right</code>, <code>Canvas.Bottom</code>, 
<code>Canvas.Top</code>. Например, свойство Canvas.Left указывает, на сколько единиц от левой стороны контейнера будет находиться элемент, 
а свойство Canvas.Top - насколько единиц ниже верхней границы контейнера находится элемент.</p>
<p>При этом в качестве единиц используются не пиксели, а независимые 
от устройства единицы, которые помогают эффективно управлять масштабированием элементов. Каждая такая единица равна 1 /96 дюйма, и при стандартной установке 
в 96 dpi эта независимая от устройства единица будет равна физическому пикселю, так как 1/96 дюйма * 96 dpi (96 точек на дюйм) = 1. В тоже время при работе на других 
мониторах или при других установленных размеры, установленные в приложении, будут эффективно масштабироваться. Например, при разрешении 
в 120 dpi одна условная единица будет равна 1,25 пикселя, так как 1/96 дюйма * 120 dpi= 1,25 пикселя.</p>
<p>Если элемент не использует свойства Canvas.Top и другие, то по умолчанию свойства <code>Canvas.Left</code> и <code>Canvas.Top</code> 
будут равны нулю, то есть он будет находиться в верхнем левом углу.</p>
<p>Также надо учитывать, что нельзя одновременно задавать Canvas.Left и Canvas.Right или Canvas.Bottom и Canvas.Top. Если подобное произойдет, 
то последнее заданное свойство не будет учитываться. Например:</p>
<pre class="brush:xml;"> 
...
<img src="./pics/3.12.png" alt="Контейнер Canvas в WPF" />
