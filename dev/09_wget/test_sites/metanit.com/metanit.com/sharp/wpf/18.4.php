...
<html >
<head>
<title>C# и WPF | Освещение</title>
<meta charset="utf-8" />
<meta name="description" content="Освещение трехмерных объектов в WPF, работа с различными источниками света, DirectionalLight, AmbientLight, SpotLight, PointLight, создание световых эффектов">
...
<h2>Освещение</h2><div class="date">Последнее обновление: 13.03.2016</div>
...
<p>Установив геометрию и настроив внешний вид объекта, мы можем задать освещение. Без освещения мы бы не могли увидеть объект, кроме того, 
освещение также влияет на внешний вид, а именно на цвет объекта, наравне с материалами. В WPF система освещения призвана имитировать свет в окружающем 
нас мире. В то же время эта имитация все-таки далека от реальной. Так, свет отраженный от одного объекта, не будет виден на другом объекте. Также объекты 
не обрасывают на другие объекты тени.</p>
<p>Свет в WPF определяется следующим образом: сначала вычисляется освещенность для отдельной вершины треугольника, а затем эта освещенность интерполируется на 
весь треугольник, сглаживаясь с освещенностью двух других вершин.</p>
<p>Для определения света в WPF используется базовый класс Light, но в реальности мы будем работать с производными от него классами:</p>
<ul>
<li><p><span class="b">DirectionalLight</span>: создает прямые параллельные лучи, которые идут в направлении, указанном в свойстве Direction. 
В качестве примера подобного источника света можно привести солнце.</p></li>
<li><p><span class="b">AmbientLight</span>: создает источник рассеянного света. Свет исходит со всех сторон и равномерно покрывает поверхность объекта</p></li>
<li><p><span class="b">PointLight</span>: создает точечный источник света.</p></li>
<li><p><span class="b">SpotLight</span>: создает источник, который испускает свет на трехмерную сцену по конусу.</p></li>
</ul>
<h3>DirectionalLight</h3>
<p>Для установки освещения в прошлой теме мы использовали объект DirectionalLight:</p>
<pre class="brush:xml;">
&lt;ModelVisual3D&gt;
...
<p>Использованный нами в примере источник освещения задает вектор направления света. Он начинается в точке (0,0,0) и направлен в точку (-1,-1,-2). 
При этом нам важен угол падения света, а не длина вектора, которая может быть и (-2,-2,-4), но будет означать то же самое, что и направление вектора (-1,-1,-2), 
поскольку оба вектора эквивалентны.</p>
<h3>AmbientLight</h3>
<p>Представляет рассеяный свет. Также широко распространенный источник света.</p>
<pre class="brush:xml;">
 &lt;AmbientLight Color="Yellow" /&gt;
</pre>
<p>Кроме того, для создания рассеянного света мы можем использовать свойство <span class="b">AmbientColor</span> у объекта DiffuseMaterial:</p>
<pre class="brush:xml;">
 &lt;DiffuseMaterial Brush="LightCoral" AmbientColor="Yellow" /&gt;
</pre>
<img src="./pics/18.5.png" alt="Рассеянный свет AmbientLight в WPF" />
<h3>PointLight</h3>
<p>Для создания точечного света нам надо указать точку, в которой находится источник света:</p>
<pre class="brush:xml;">
 &lt;PointLight Color="White" Position="0,0.8,0.25" /&gt;
</pre>
<img src="./pics/18.6.png" alt="PointLight в WPF" />
<h3>SpotLight</h3>
<p>Данный источник света похож на PointLight, также лучи света направлены в одну точку, только теперь освещенность ограничивается углом, который определяется конусом:</p>
<pre class="brush:xml;">
&lt;SpotLight Color="White" Position="1,0.2,0.5"
        Direction="-0.5,-0.5,-0.5" InnerConeAngle="45" OuterConeAngle="90" /&gt;
</pre>
<img src="./pics/18.7.png" alt="SpotLight в WPF" />
<p>Свойства <code>InnerConeAngle</code> и <code>OuterConeAngle</code> определяют форму конуса, используемого для определения освещенности. Если эти свойства равны, 
то фактически мы получаем стандартный точечный свет PointLight. А свойства Position и Direction также указывают соответственно на положение источника света и направление света.</p>
...
<div class="nav"><p><a href="18.3.php">Назад</a><a href="./">Содержание</a><a href="18.5.php">Вперед</a></p></div></div>
<div class="socBlock">
