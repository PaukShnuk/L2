...
<html >
<head>
<title>C# и WPF | Работа с классом Application</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с классом Application в WPF, его основные свойства и события, загрузка только одной копии приложения, создание экрана-заставки">
...
<h2>Работа с классом Application</h2><div class="date">Последнее обновление: 13.02.2016</div>
...
<h3>Свойство ShutdownMode</h3>
<p>Свойство <span class="b">ShutdownMode</span> указывает на способ выхода из приложения и может принимать одно из следующих значений:</p>
<ul>
<li><p><code>OnMainWindowClose</code>: приложение работает, пока открыто главное окно</p></li>
<li><p><code>OnLastWindowClose</code>: приложение работает, пока открыто хотя бы одно окно</p></li>
<li><p><code>OnExplicitShutdown</code>: приложение работает, пока не будет явно вызвано Application.Shutdown()</p></li>
</ul>
<p>Задать свойство ShutdownMode можно в коде xaml:</p>
<pre class="brush:xml;">
...
<p>либо определить в App.xaml.cs:</p>
<pre class="brush:c#;">
...
<p>Класс Application определяет ряд событий, который могут использоваться для всего приложения:</p>
<ul>
<li><p><span class="b">Startup</span>: происходит после вызова метода Application.Run() и перед показом главного окна</p></li>
<li><p><span class="b">Activated</span>: происходит, когда активизируется одно из окон приложения</p></li>
<li><p><span class="b">Deactivated</span>: возникает при потере окном фокуса</p></li>
<li><p><span class="b">SessionEnding</span>: происходит при завершении сеанса Windows при перезагрузке, выключении или выходе из системы текущего пользователя</p></li>
<li><p><span class="b">DispatcherUnhandledException</span>: возникает при возникновении необработанных исключени</p></li>
<li><p><span class="b">LoadCompleted</span>: возникает при завершении загрузки приложения</p></li>
<li><p><span class="b">Exit</span>: возникает при выходе из приложения Это может быть закрытие главного или последнего окна, метод Application.Shutdown() или завершение сеанса</p></li>
</ul>
<p>Так же, как и для элементов, обработчики события определяются для одноименных атрибутов в разметке xaml. Например, обработаем событие Startup:</p>
<pre class="brush:xml;">
&lt;Application x:Class="LifecycleApp.App"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<p>Затем в файле связанного кода <i>App.xaml.cs</i> пропишем обработчик события:</p>
<pre class="brush: c#;">
public partial class App : Application
{
...
<p>В результате при запуске каждой новой копии данного приложение обработчик события будет определять, запущена ли уже приложение, 
и если оно запущено, то данная копия завершит свою работу.</p>
<h3>Создание заставки приложения</h3>
<p>Приложения бывают разные, одни открываются быстро, другие не очень. И чтобы пользователя как-то известить о том, что идет загрузка приложения, 
нередко используют заставку или сплеш-скрин. В WPF простые заставки на основе изображений очень легко сделать. Для этого добавьте в проект какое-нибудь 
изображение и после добавления установите для него в окне Properties (Свойства) для свойства <span class="b">BuildAction</span> 
значение <span class="b">SplashScreen</span>. И после запуска до появления окна приложения на экране будет висеть изображение заставки.</p>
<h3>Обращение к текущему приложению</h3>
<p>Мы можем обратиться к текущему приложению из любой точки данного приложения. Это можно сделать с помощью свойства <span class="bb">Current</span>, определенном в 
классе Application. Так, изменим код нажатия кнопки на следующий:</p>
<pre class="brush: c#;">
...
<p>Таким образом, при нажатии на кнопку мы получим одно или несколько сообщений с именами всех окон данного приложения.</p>
...
<div class="nav"><p><a href="3.php">Назад</a><a href="./">Содержание</a><a href="3.3.php">Вперед</a></p></div></div>
<div class="socBlock">
