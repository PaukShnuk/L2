...
<html >
<head>
<title>C# и WPF | Стили</title>
<meta charset="utf-8" />
<meta name="description" content="Использование стилей WPF, сеттеры, назначение стилей элементам через свойство TargetType и обработчиков событий через EventSetter">
...
<h1>Стили, триггеры и темы</h1><h2>Стили</h2><div class="date">Последнее обновление: 07.02.2016</div>
...
<p>Стили позволяют определить набор некоторых свойств и их значений, которые потом могут применяться к элементам в xaml. 
...
Аналогом стилей могут служить каскадные таблицы стилей (CSS), которые применяются в коде html на веб-страницах.</p>
<p>Зачем нужны стили? Стили помогают создать стилевое единообразие для определенных элементов. Допустим, у нас есть следующий код xaml:</p>
<pre class="brush:xml;">
...
<p>Здесь обе кнопки применяют ряд свойств с одними и теми же значениями:</p>
<img src="./pics/10.1.png" alt="Стили в WPF" />
<p>Однако в данном случае мы вынуждены повторяться. Частично, проблему могло бы решить использование ресурсов:</p>
<pre class="brush:xml;">
...
<p>Однако в реальности код раздувается, опть же приходится писать много повторяющейся информации. И в этом плане стили предлагают более элегантное решение:</p>
<pre class="brush:xml;">
&lt;Window x:Class="StylesApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<p>Результат будет тот же, однако теперь мы избегаем ненужного повторения. Более того теперь мы можем управлять всеми нужными нам свойствами как единым целым - одним стилем.</p>
<p>Стиль создается как ресурс с помощью объекта <span class="b">Style</span>, который представляет класс <span class="b">System.Windows.Style</span>. 
И как любой другой ресурс, он обязательно должен иметь ключ. С помощью коллекции <span class="b">Setters</span> определяется группа свойств, входящих в 
стиль. В нее входят объекты <span class="b">Setter</span>, которые имеют следующие свойства:</p>
<ul>
<li><p><span class="b">Property</span>: указывает на свойство, к которому будет применяться данный сеттер. Имеет следующий синтаксис: 
<code>Property="Тип_элемента.Свойство_элемента"</code>. Выше в качестве типа элемента использовался Control, как общий для всех элементов. 
Поэтому данный стиль мы могли бы применить и к Button, и к TextBlock, и к другим элементам. Однако мы можем и конкретизировать элемент, например, Button:</p>
<pre class="brush:xml;">&lt;Setter Property="Button.FontFamily" Value="Arial" /&gt;</pre></li>
<li><p><span class="b">Value</span>: устанавливает значение</p></li>
</ul>
...
<h3>TargetType</h3>
<p>Hам необязательно прописывать для всех кнопок стиль. Мы можем в самом определении стиля с помощью свойства <span class="b">TargetType</span> 
задать тип элементов. В этом случае стиль будет автоматически применяться ко всем кнопкам в окне:</p>
<pre class="brush:xml;">
&lt;Window x:Class="StylesApp.MainWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<p>Причем в этом случае нам уже не надо указывать у стиля ключ <code>x:Key</code> несмотря на то, что это ресурс.</p>
<p>Также если используем свойство TargetType, то в значении атрибута <code>Property</code> уже необязательно указывать тип, то есть <code>Property="Control.FontFamily"</code>. 
И в данном случае тип можно просто опустить: <code>Property="FontFamily"</code></p>
...
<pre class="brush:xml;">&lt;Button x:Name="button2" Content="Кнопка 2" Style="{x:Null}" /&gt;</pre>
<h3>Определение обработчиков событий с помощью стилей</h3>
<p>Кроме коллекции <code>Setters</code> стиль может определить другую коллекцию - <span class="b">EventSetters</span>, которая содержит объекты 
<span class="b">EventSetter</span>. Эти объекты позволяют связать события элементов с обработчиками. Например, подключим все кнопки к одному 
обработчику события <code>Click</code>:</p>
<pre class="brush:xml;">
&lt;Style TargetType="Button"&gt;
...
<p>Соответственно в файле кода c# у нас должен быть определен обработчик Button_Click:</p>
<pre class="brush:c#;">
...
<p>У класса Style еще есть свойство <span class="b">BasedOn</span>, с помощью которого можно наследовать и расширять существующие стили:</p>
<pre class="brush:xml;">
&lt;Window.Resources&gt;
...
<p>Cвойство <code>BasedOn</code> в качестве значения принимает существующий стиль, определяя его как статический ресурс. В итоге он объединяет весь 
функционал родительского стиля со своим собственным.</p>
<p>Если в дочернем стиле есть сеттеры для свойств, которые также используются в родительском стиле, как в данном случае сеттер для свойства <code>Button.FontFamily</code>, 
то дочерний стиль переопределяет родительский стиль.</p>
<h3>Стили в C#</h3>
<p>В C# стили представляют объект System.Windows.Style. Используя его, мы можем добавлять сеттеры и устанавливать стиль для нужных элементов:</p>
<pre class="brush:c#;">
...
<p>При создании сеттера нам надо использовать свойство зависимостей, например, <code>Property = Control.FontFamilyProperty</code>. Причем для свойства 
<code>Value</code> у сеттера должен быть установлен объект именно того типа, которое хранится в этом свойстве зависимости. 
Так, свойство зависимости <code>MarginProperty</code> хранит объект типа <code>Thickness</code>, поэтому определение сеттера выглядит следующим образом:</p>
...
<div class="nav"><p><a href="11.3.php">Назад</a><a href="./">Содержание</a><a href="10.2.php">Вперед</a></p></div></div>
<div class="socBlock">
