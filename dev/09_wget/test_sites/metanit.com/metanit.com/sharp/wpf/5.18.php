...
<html >
<head>
<title>C# и WPF | CheckBox и RadioButton</title>
<meta charset="utf-8" />
<meta name="description" content="Применение элементов CheckBox и RadioButton в WPF, их основные свойства и примеры использования">
...
<h2>CheckBox и RadioButton</h2><div class="date">Последнее обновление: 18.01.2016</div>
...
<h3>CheckBox</h3>
<p>Элемент CheckBox представляет собой обычный флажок. Данный элемент является производным от класса ToggleButton и поэтому может принимать также три состояния: 
<span class="b">Checked</span>, <span class="b">Unchecked</span> и <span class="b">Intermediate</span>.</p>
<p>Чтобы получить или установить определенное состояние, надо использовать свойство <b>IsChecked</b>, которое также унаследовано от ToggleButton:</p>
<pre class="brush:xml;">
...
<p>Установка свойства <code>IsChecked="{x:Null}"</code> задает неопределенное состояние для элемента checkbox. Остальные два состояния задаются с помощью 
True и False. В данном примере также привязан к двум флажкам обработчик события <code>Checked</code>. Это событие возникает при установке checkbox 
в отмеченное состояние.</p>
<p>А атрибут <code>IsThreeState="True"</code> указывает, что флажок может находиться в трех состояниях.</p>
<img src="./pics/4.3.png" alt="Элемент checkbox в WPF" />
<p>Ключевыми событиями флажка являются события <span class="b">Checked</span> (генерируется при установке флажка в отмеченное состояние), 
<span class="b">Unchecked</span> (генерируется при снятии отметки с флажка) и <span class="b">Indeterminate</span> (флажок переведен в неопределенное состояние).  
Например, определим флажок:
<pre class="brush:xml;">
...
<p>А в файле кода C# пропишем для него обработчики:</p>
<pre class="brush:c#;">
...
<p>Программное добавление флажка:</p>
<pre class="brush:c#;">
...
<h3>RadioButton</h3>
<p>Элемент управления, также производный от ToggleButton, представляющий переключатель. Главная его особенность - поддержка групп. Несколько 
элементов RadioButton можно объединить в группы, и в один момент времени мы можем выбрать из этой группы только один переключатель. Например,</p>
<pre class="brush:xml;">
&lt;StackPanel x:Name="stackPanel"&gt;
...
<img src="./pics/4.4.png" alt="Элемент RadioButton в WPF" />
<p>Чтобы включить элемент в определенную группу, используется свойство <code>GroupName</code>. В данном случае у нас две группы 
- Languages и Technologies. Мы можем отметить не более одного элемента RadioButton в пределах одной группы, зафиксировав тем самым выбор из нескольких возможностей.</p>
<p>Чтобы проследить за выбором того или иного элемента, мы также можем определить у элементов событие <code>Checked</code> и его обрабатывать в коде:</p>
<pre class="brush:xml;">
&lt;RadioButton GroupName="Languages" Content="VB.NET" Checked="RadioButton_Checked" /&gt;
</pre>
<p>Обработчик в файле кода:</p>
<pre class="brush:c#;">
...
<p>Программное добавление элемента RadioButton:</p>
<pre class="brush:c#;">
using System.Windows;
using System.Windows.Controls;
