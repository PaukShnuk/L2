...
<html >
<head>
<title>C# и WPF | Элементы управления содержимым</title>
<meta charset="utf-8" />
<meta name="description" content="Элементы управления содержимым в WPF, класс ContentControl, его особенности, свойство Content">
...
<h2>Элементы управления содержимым</h2><div class="date">Последнее обновление: 18.01.2016</div>
...
<p>Элементы управления содержимым (content controls) представляют такие элементы управления, которые содержат в себе другой элемент. Все элементы управления 
содержимым наследуются от класса <span class="b">ContentControl</span>, который в свою очередь наследуется от класса System.Window.Controls.Control.</p>
<img src="./pics/4.38.png" alt="ContentControl в WPF" />
<p>К элементам управления содержимым относятся такие элементы как Button, Label, ToggleButton, ToolTip, RadioButton, CheckBox, GroupBox, TabItem, 
Expander, ScrollViewer. Также элементом управления содержимым является и главный элемент окна - Window.</p>
<p>Отличительной чертой всех этих элементов является наличие свойства <span class="b">Content</span>, которое и устанавливает вложенный элемент. В этом 
элементы управления содержимым схожи с контейнерами компоновки. Только контейнеры могут иметь множество вложенных элементов, а элементы управления содержимым только один.</p>
<p>Свойство Content может представлять любой объект, который может относиться к одному из двух типов:</p>
<ul>
<li><p>Объект класса, не наследующего от UIElement. Для такого объекта вызывается метод <code>ToString()</code>, который возвращает строковое преставление 
объекта. Затем эта строка устанавливается в качестве содержимого.</p></li>
<li><p>Объект класса, наследующего от UIElement. Для такого объекта вызывается метод <code>UIElement.OnRender()</code>, который выполняет отрисовку внутри элемента 
управления содержимым.</p></li>
</ul>
<p>Рассмотрим на примере кнопки, которая является элементом управления содержимым:</p>
<pre class="brush:xml;">
&lt;Button Content="Hello World!" /&gt;
</pre>
<p>В качестве содержимого устанавливается обычная строка. Этот же пример мы можем в XAML прописать иначе:</p>
<pre class="brush:xml;">
&lt;Button&gt;
    &lt;Button.Content&gt;
        Hello World!
...
<p>Либо мы можем использовать сокращенное неявное определения свойства Content:</p>
<pre class="brush:xml;">
&lt;Button&gt;
    Hello World!
&lt;/Button&gt;
</pre>
<p>Возьмем другой пример. Определим кнопку с именем button1:</p>
<pre class="brush:xml;">
...
<p>А в файле коде MainWindow.xaml.cs присвоим ее свойству Content какой-либо объект:</p>
<pre class="brush:c#;">
using System;
...
<p>В итоге мы получим следующую кнопку:</p>
<img src="./pics/4.39.png" />
<p>В итоге число конвертируется в строку и устанавливается в качесте содержимого.</p>
<p>Иначе все будет работать, если мы в качестве содержимого используем объект, унаследованный от UIElement:</p>
<pre class="brush:xml;">
&lt;Button x:Name="button1"&gt;
    &lt;Button Content="Hello" /&gt;
&lt;/Button&gt;
</pre>
<p>Теперь в качестве содержимого будет использоваться другая кнопка, для которой при визуализации будет вызываться метод <code>OnRender()</code>:</p>
<img src="./pics/4.40.png" />
<p>Для создания той же кнопки через код C# мы бы могли прописать следующее выражение:</p>
<pre class="brush:c#;">button1.Content = new Button { Content = "Hello" };</pre>
<p>В отличие от контейнеров компоновки для элементов управления содержимым мы можем задать только один вложенный элемент. Если же нам надо вложить в элемент управления содержимым несколько 
элементов, то мы можем использовать те же контейнеры компоновки:</p>
<pre class="brush:xml;">
&lt;Button x:Name="button1"&gt;
    &lt;StackPanel&gt;
...
<img src="./pics/4.41.png" />
<p>То же самое мы могли бы прописать через код C#:</p>
<pre class="brush:c#;">
...
<h3>Позиционирование контента</h3>
<h4>Content Alignment</h4>
<p>Выравнивание содержимого внутри элемента задается свойствами <span class="b">HorizontalContentAlignment</span> (выравнивание по горизонтали) и <span class="b">VerticalContentAlignment</span> (выравнивание по вертикали), аналогичны свойствам VerticalAlignment/HorizontalAlignment. 
Свойство <code>HorizontalContentAlignment</code> принимает значения Left, Right, Center (положение по центру), Stretch (растяжение по всей ширине). Например:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p><code>VerticalContentAlignment</code> принимает значения Top (положение в верху), Bottom (положение внизу), Center (положение по центру), Stretch (растяжение по всей высоте)</p>
<h4>Padding</h4>
<p>С помощью свойства Padding мы можем установить отступ содержимого элемента:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<img src="./pics/4.42.png" />
<p>Свойство Padding задается в формате <code>Padding="отступ_слева отступ_сверху отступ_справа отступ_снизу"</code>.</p>
<p>Если со всех четырех сторон предполагается один и тот же отступ, то, как и в случае с Margin, мы можем задать одно число:</p>
<pre class="brush:xml;">
&lt;Button x:Name="button2" Padding="20"  Content="Hello World" /&gt;
</pre>
<p>Важно понимать, от какой точки 
задается отступ. В случае с первой кнопкой в ней контект выравнивается по левому краю, поэтому отступ слева будет предполагать отступ от левого края элемента Button. 
А вторая кнопка располагается по центру. Поэтому для нее отступ слева предполагает отступ от той точки, в которой содержимое бы находилось при центрировании без применения Padding.</p>
<p>Комбинация значений свойств HorizontalContentAlignment/VerticalContentAlignment и Padding позволяет оптимальным образом задать расположение содержимого.</p>
...
<div class="nav"><p><a href="5.1.php">Назад</a><a href="./">Содержание</a><a href="5.2.php">Вперед</a></p></div></div>
<div class="socBlock">
