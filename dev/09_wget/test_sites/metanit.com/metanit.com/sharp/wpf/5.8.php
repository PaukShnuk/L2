...
<html >
<head>
<title>C# и WPF | ComboBox</title>
<meta charset="utf-8" />
<meta name="description" content="Использование элемента ComboBox в WPF для создания выпажающего списка, его использование, применение событий и свойств">
...
<h2>ComboBox</h2><div class="date">Последнее обновление: 18.01.2016</div>
...
<p>ComboBox содержит коллекцию элементов и образует выпадающий список:</p> 
<pre class="brush:xml;">
...
<img src="./pics/4.47.png" alt="ComboBox в WPF" />
<h3>ComboBoxItem</h3>
<p>В качестве элементов в ComboBoxe мы можем использовать различные компоненты, но наиболее эффективным является применение элемента <span class="b">ComboBoxItem</span>. 
ComboBoxItem представляет элемент управления содержимым, в который через свойство Content мы можем поместить другие элементы. Например:</p>
<pre class="brush: xml;">
...
<img src="./pics/4.16.png" alt="Элемент ComboBox в WPF" />
...
ComboBoxItem имеет преимущество, так как мы можем выделить данный элемент, установив свойство <code>IsSelected="True"</code>, либо можем сделать 
недоступным с помощью установки свойства <code>IsEnabled="False"</code>.</p>
<h3>Событие SelectionChanged</h3>
<p>Обрабатывая событие <code>SelectionChanged</code>, мы можем динамически получать выделенный элемент:</p>
<pre class="brush: xml;">
&lt;ComboBox Height="25" Width="150" SelectionChanged="ComboBox_Selected"&gt;
&lt;!-- остальное содержимое списка--&gt;
&lt;/ComboBox&gt;
</pre>
<p>Обработка события в коде C#:</p>
<pre class="brush: xml;">
...
<p>Правда, для элементов со сложным содержимым подобный способ может не пройти, и если мы захотим получить текст, до него придется добираться, спускаясь по 
дереву вложенных элементов.</p>
<h3>Свойства</h3>
<p>Установка свойства <code>IsEditable="True"</code> позволяет вводить в поле списка начальные символы, а затем функция автозаполнения подставит 
подходящий результат. По умолчанию свойство имеет значение <code>False</code>.</p>
<p>Это свойство работает в комбинации со свойством <code>IsReadOnly</code>: оно указывает, является поле ввода доступным только для чтения. 
По умолчанию имеет значение <code>False</code>, поэтому если <code>IsEditable="True"</code>, то мы можем вводить туда произвольный текст.</p>
<p>Еще одно свойство <code>StaysOpenOnEdit</code> при установке в <code>True</code> позволяет сделать список раскрытым на время ввода значений в поле ввода.</p>
