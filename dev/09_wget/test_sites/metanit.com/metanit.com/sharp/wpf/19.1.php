...
<html >
<head>
<title>C# и WPF | Взаимодействие с базой данных</title>
<meta charset="utf-8" />
<meta name="description" content="Создание базы данных для приложения WPF, особенности работы с базами данных и SQL Server Management Studio">
...
<h1>Взаимодействие с базой данных</h1><h2>Создание базы данных</h2><div class="date">Последнее обновление: 28.02.2016</div>
...
<p>Как правило, для хранения больших наборов данных используются различные базы данных. В WPF, как и в целом в .NET, мы можем работать с различными системами управления баз данных - 
MS SQL Server, Oracle, MySQL, MongoDB, Postgres и т.д. В данном случае рассмотрим взаимодействие приложения на WPF и базы данных MS SQL Server. Хотя 
общие принципы работы с другими БД не будут теми же.</p>
<p>Прежде всего для работы с базами данных должен быть установлен и запущен MS SQL Server.  Все необходимые файлы для загрузки можно найти на офсайте по 
ссылке <a href="https://www.microsoft.com/ru-ru/download/details.aspx?id=42299" rel="nofollow">MS SQL Server 2014 Express</a></p>
<p>Итак, создадим новый проект. Пусть он будет называться <span class="b">DbApp</span>. Вначале создадим базу данных, к которой мы будем подключаться. 
Для этого откроем специальную программу <span class="b">SQL Server 2014 Management Studio</span>, которая устанавливается вместе с MS SQL Server и которая 
служит для управления базами данных. В левом древовидном меню найдем пункт <span class="b">Databases</span>, нажмем на него правой кнопкой мыши и в появившемся 
контекстном меню выберем пункт <span class="b">New Database...</span>:</p>
<img src="./pics/19.1.png" alt="Создание базы данных в WPF" />
<p>Назовем базу данных <span class="b">mobiledb</span> и нажмем на кнопку OK:</p>
<img src="./pics/19.2.png" />
<p>После этого в левом меню появится узел с только что созданной базой данных. Теперь добавим в нее таблицу. Правой кнопкой мыши нажмем на подузел Tables и в контекстном 
меню выберем пункт <span class="b">Tables...</span>:</p>
<img src="./pics/19.3.png" alt="Создание таблицы в WPF"  />
<p>Затем нам открывается дизайнер таблицы:</p>
<img src="./pics/19.4.png" alt="Дизайнер таблицы в SQL Management Studio"  />
<p>В нем надо указать четыре столбца: Id, Title, Company и Price, которые будут представлять соответственно уникальный идентификатор телефона, название его модели, производителя и цену. У первого и четвертого столбца надо указать тип int 
(то есть целочисленный), а у столбцов Title и Company - тип nvarchar (строковый).</p>
<p>Кроме того, в окне свойств таблицы в поле Name надо ввести имя таблицы - Phones, а в поле <span class="b">Identity Column</span> ввести Id, 
так как столбец Id будет идентификатором.</p>
<p>И в конце нам надо установить курсор на столбец Id и в панели инструментов программы нажать на золотой ключик. После этого напротив поля Id также должен появиться золотой ключик. Этот ключик будет указывать, что столбец Id будет выполнять роль первичного ключа.</p>
<p>После этого нажмем на сохранение и затем на клавишу F5 (обновление), и в узле нашей базы данных появится новая таблица, которая будет называться dbo.Phones:</p>
<img src="./pics/19.5.png" />
<p>Итак, мы создали базу данных и таблицу, и сделаем последний шаг - добавим в базу данных харнимую процедуру, которая осуществляет добавление нового объекта в базу данных. 
Для этого выберем в узле базы данных пункт <span class="b">Programmability-&gt;Stored Procedures</span>. Нажмем на этот узел правой кнопкой мыши и в 
контекстном меню выберем пункт <span class="b">Stored Procedure...</span>:</p>
<img src="./pics/19.6.png" alt="Хранимая процедура и WPF" />
<p>В центральной части программы открывается код процедуры, который генерируется по умолчанию. Заменим этот код следующим::</p>
<pre class="brush:sql;">
...
<p>Эта процедура выполняет добавление данных. После выражения <code>CREATE PROCEDURE</code> идет название процедуры. Процедура называется 
"sp_InsertPhone", и по этому названию мы ее будем вызывать в коде C#. Название процедуры может быть любое.</p>
<p>Процедура имеет три входных параметра: <code>@title</code>, <code>@company</code> и <code>@price</code>. Через эти параметры будут передаваться 
значения для объекта в таблице Phones. В самом теле процедуры после выражения AS идет стандартное sql-выражение INSERT, которое выполняет добавление 
данных. И в конце с помощью выражения SELECT возвращается результат. Выражение <code>SCOPE_IDENTITY()</code> возвращает id добавленной записи, который присваивается 
четвертому выходному параметру <code>@Id</code>. Поэтому на выходе из процедуры мы получим id новой записи. И завершается процедура ключевым словом GO.</p>
<p>Вообще мы бы могли обойтись и без хранимой процедуры. Однако при подключении из кода C# она нам упростит работу, и в дальнейшем мы посмотрим, в чем ее преимущества. 
И затем нажмем на кнопку Execute. После этого в базу данных добавляется хранимая процедура.</p>
<img src="./pics/19.7.png" />
<p>Теперь подключимся к ней из приложения на WPF.</p>
...
<div class="nav"><p><a href="14.php">Назад</a><a href="./">Содержание</a><a href="19.2.php">Вперед</a></p></div></div>
<div class="socBlock">
