...
<html >
<head>
<title>C# и WPF | Работа с датами. Calendar и DatePicker</title>
<meta charset="utf-8" />
<meta name="description" content="Обзор элементов для работы с датами в WPF - Calendar и DatePicker и их основных свойств">
...
<h2>Работа с датами. Calendar и DatePicker</h2><div class="date">Последнее обновление: 19.01.2016</div>
...
<p>Элементы для работы с данными представлены следующими классами: <span class="b">Calendar</span> и <span class="b">DatePicker</span>.</p>
...
<table class="tab"><tr><td><p><span  class="b">BlackoutDates</span></p></td><td><p>Принимает в качестве значения объект <span class="b">CalendarDateRange</span>, задающий с помощью свойств <span class="b">Start</span> и <span class="b">End</span> диапазон дат, которые будут зачеркнуты в календаре.</td></tr>
<tr><td><p><span class="b">DisplayDateStart</span> и <span class="b">DisplayDateEnd </span></p></td><td><p>Задают соответственно начальную и конечную дату диапазона, который будет отображаться в календаре.</p></td></tr>
<tr><td><p><span class="b">IsTodayHighlighted</span></p></td><td><p>Отмечает, будет ли выделена текущая дата</p></td></tr>
<tr><td><p><span class="b">SelectedDate</span>(<span class="b">SelectedDates</span>)</p></td><td><p>Задает выделенную дату (диапазон выделенных дат)</p></td></tr>
<tr><td><p><span class="b">FirstDayOfWeek</span></p></td><td><p>Задает первый день недели</p></td></tr>
</table>
<p>Также Calendar имеет еще два важных свойства: <span class="b">DisplayMode</span>(формат отображения дат) и <span class="b">SelectionMode</span> (способ выделения).</p> 
<p>DisplayMode может принимать одно из следующих значений:</p> 
<ul>
<li><p><span class="b">Month</span> (по умолчанию) отображает все дни текущего месяца</p></li>
<li><p><span class="b">Decade</span> отображает все года текущего десятилетия</p></li>
<li><p><span class="b">Year</span> отображает все месяцы текущего года</p></li>
</ul>
<p>SelectionMode может принимать одно из следующих значений:</p>
<ul>
<li><p><span class="b">SingleDate</span> (по умолчанию) выделяет только одну дату</p></li>
<li><p><span class="b">None</span> запрещает выделение</p></li>
<li><p><span class="b">SingleRange</span> по нажатию на Ctrl выделяет несколько последовательно идущих дат</p></li>
<li><p><span class="b">MultipleRange</span> по нажатию на Ctrl выделяет несколько не последовательно идущих диапазонов дат</p></li>
</ul>
<p>Например:</p>
<pre class="brush:xml;">
...
<p>Обратите внимание, что при задании даты мы сначала пишем месяц, а потом число.</p>
<p>Чтобы использовать в программе выбор даты пользователем, мы можем обработать событие <code>SelectedDatesChanged</code> 
в коде c#:</p>
<pre class="brush:c#;">
...
<img src="./pics/4.31.png" alt="Элемент Calendar в WPF" />
<p>Для создания набора выделенных дат нам надо подключить в xaml пространство имен System:</p>
<pre class="brush:xml;">
...
<img src="./pics/4.32.png" alt="Календарь в WPF" />
<h3>DatePicker</h3>
<p>Кроме выше названных свойств элемент DatePicker определяет еще ряд свойств, среди которых <span class="b">IsDropDownOpen</span> 
(показывает, будет ли связанный с элементом всплывающий календарь оставаться открытым после выбора даты) и <span class="b">SelectedDateFormat</span> 
(формат даты, принимает значение Short - формат в виде дд.мм.гггг или мм.дд.гггг и значение Long - с полными названиями месяцев):</p>
<pre class="brush:xml;">
&lt;DatePicker Name="datePicker1" IsDropDownOpen="True" SelectedDateFormat="Short" /&gt;
</pre>
<img src="./pics/4.33.png" alt="Элемент DatePicker в WPF" />
<p>И при нажатии на элемент отобразится тот же календарь для дальнейшего выбора даты.</p>
...
<div class="nav"><p><a href="5.15.php">Назад</a><a href="./">Содержание</a><a href="5.17.php">Вперед</a></p></div></div>
<div class="socBlock">
