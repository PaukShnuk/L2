...
<html >
<head>
<title>C# и WPF | Работа с диалоговыми окнами</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с диалоговыми окнами в паттерне MVVM в WPF, открытие и сохранение файлов">
...
<h2>Работа с диалоговыми окнами</h2><div class="date">Последнее обновление: 18.10.2016</div>
...
<p>Работа с диалоговыми окнами при использовании паттерна MVVM может вызывать некоторые трудности. Например, если мы хотим по клику на кнопку или на пункт 
меню сохранять или открывать объекты с помощью диалоговых окон, не всегда может быть ясно, как вписать взаимодействие с диалоговыми окнами в MVVM.</p>
<p>Итак, для работы с диалоговыми окнами продолжим работу с проектом из прошлой темы. Вначале добавим в него новый интерфейс IDialogService, который будет 
определять функционал для работы с диалоговыми окнами:</p>
<pre class="brush:c#;">
...
<p>Далее добавим в проект реализацию этого интерфейса в виде класса DefaultDialogService:</p>
<pre class="brush:c#;">
...
<p>Для получения пути файла для открытия/сохранения данный класс использует стандартные классы <code>OpenFileDialog</code> и 
<code>SaveFileDialog</code>, которые определены в пространстве имен <code>Microsoft.Win32</code>. Кроме того, для отображения сообщения здесь используется метод 
<code>MessageBox.Show()</code>.</p>
<p>Для работы с файлами одного функционала по открытию/сохранению файла мало. Нам еще надо выполнять сами действия по считыванию информации из файла или ее сохранению в файл. 
Подобные действия, конечно, можно определить и в ViewModel. Однако для работы с информацией мы можем использовать различные типы файлов - бинарные файлы, xml, json и т.д. 
Для json в .NET мы можем использовать один функционал, для xml - другой, для текстовых файлов - третий и так далее. Поэтому в этом случае для работы с файлами 
определим в проекте общий интерфейс IFileService:</p>
<pre class="brush:c#;">
...
<p>Первый метод предназначен для открытия файла. Он принимает путь к файлу и возвращает список объектов. 
Второй метод сохраняет данные из списка в файле по определенному пути.</p>
<p>В качестве типа файлов мы будем использовать файлы json. Поэтому для работы с ними добавим в проект библиотеку <span class="ii"></span>:</p>
<img src="./pics/22.7.png" alt="Сериализация в JSON в WPF" />
<p>Затем добавим в проект следующий класс JsonFileService:</p>
<pre class="brush:c#;">
using System.Collections.Generic;
using System.IO;
...
<p>С помощью класса DataContractJsonSerializer здесь производится сериализация/десериализация объектов в файл json в виде набора List&lt;Phone&gt;.</p>
<p>Далее изменим код ApplicationViewModel:</p>
<pre class="brush:c#;">
...
<p>Для работы с файлами в конструктор ApplicationViewModel передаются объекты IDialogService и IFileService:</p>
<pre class="brush:c#;">
public ApplicationViewModel(IDialogService dialogService, IFileService fileService)
{
    this.dialogService = dialogService;
...
<p>Затем эти объекты используются в командах OpenCommand и SaveCommand.</p>
<p>Также изменим код <span class="ii">MainWindow.xaml</span>:</p>
<pre class="brush:xml;">
...
<p>В отличие от предыдущей темы здесь добавлено меню с двумя пунктами, которые привязаны к командам SaveCommand и OpenCommand:</p>
<pre class="brush:xml;">
&lt;Menu Grid.ColumnSpan="2" &gt;
    &lt;MenuItem Header="Файл"&gt;
        &lt;MenuItem Header="Открыть" Command="{Binding OpenCommand}" /&gt;
...
<p>И в конце изменим файл связанного кода <span class="ii">MainWindow.xaml.cs</span>:</p>
<pre class="brush:c#;">
using System.Windows;
...
<p>В конструктор ApplicationViewModel для работы с файлами передаются объекты DefaultDialogService и JsonFileService.</p>
<img src="./pics/22.8.png" alt="Диалоговые окна в MVVM в WPF" />
...
<div class="nav"><p><a href="22.5.php">Назад</a><a href="./">Содержание</a><a href="21.1.php">Вперед</a></p></div></div>
<div class="socBlock">
