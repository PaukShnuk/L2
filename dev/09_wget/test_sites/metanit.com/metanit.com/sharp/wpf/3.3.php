...
<html >
<head>
<title>C# и WPF | Ресурсы приложения</title>
<meta charset="utf-8" />
<meta name="description" content="Ресурсы приложения WPF, установка путей к ресурсам внутри приложения, извлечение ресурсов">
...
<h2>Работа с ресурсами приложения</h2><div class="date">Последнее обновление: 13.02.2016</div>
...
<p>Большинство приложений WPF так или иначе используют различные бинарные ресурсы - файлы изображений, мультимедиа и т.д. Для добавления ресурсов достаточно 
добавить уже существующий файл в проект. Нередко для оптимизации структуры проекта для хранения ресурсов создаются отдельные папки в проекте.</p>
<p>При добавлении ресурсов в проект мы можем установить одну из двух опций компиляции в окне свойств в поле <span class="b">Build Action</span>:</p>
<ul>
<li><p><span class="b">Resources</span>: ресурс встраивается в сборку (значение по умолчанию)</p></li>
<li><p><span class="b">Content</span>: ресурс физически находится отдельно от сборки, однако сборка содержит информацию о его местоположении</p></li>
</ul>
<img src="./pics/12.3.png" alt="Бинарные ресурсы в WPF" />
<h3>Доступ к ресурсам</h3>
<p>Для доступа к ресурсам в WPF можно использовать объект <span class="b">Uri</span>. Например, в проекте в папке Images помещен ресурс - файл river.jpg. 
И чтобы обратиться к нему из элемента Image мы можем использовать относительный путь <i>Images/river.jpg</i>:</p>
<pre class="brush:xml;">
&lt;Image x:Name="myImage" Source="Images/river.jpg" /&gt;
</pre>
<p>Для доступа к ресурсу здесь применяется относительный путь с указанием папки и названия файла. Однако в реальности все используемые в xaml пути трансформируются в объекты Uri. 
Так, если бы нам пришлось устанавливать изображение в коде c#, то нам пришлось бы написать что-то вроде следующего:</p>
<pre class="brush:c#;">myImage.Source = new BitmapImage(new Uri("Images/river.jpg", UriKind.Relative));</pre>
<p>Конструктор Uri принимает два параметра: собственно относительный путь и значение из перечисления UriKind, которое указывает, что путь стоит расценивать как относительный.</p>
<p>Кроме относительных путей мы также можем использовать абсолютные:</p>
<pre class="brush:c#;">myImage.Source = new BitmapImage(new Uri(D://forest.jpg, UriKind.Absolute));</pre>
<p>Кроме относительных и абсолютных путей в WPF можно использовать определения "упакованных" URI (packageURI). Фактически относительные пути являются сокращениями 
упакованных Uri. Так путь "Images/river.jpg" является сокращением пути "pack://application:,,,/images/river.jpg":</p>
<pre class="brush:c#;">myImage.Source = new BitmapImage(new Uri("pack://application:,,,/images/river.jpg"));</pre>
<p>Аналог в xaml:</p>
<pre class="brush:xml;">&lt;Image x:Name="myImage" Source="pack://application:,,,/images/river.jpg" /&gt;</pre>
<p>Синтаксис упакованных URI заимствован из спецификации стандарта XML Paper Specification (XPS). Для ресурсов, встроенных в сборку приложения, он имеет следующую форму:</p>
<pre>pack://application:,,,/[относительный _путь_к_ресурсу]</pre>
<p>Три запятых здесь фактически передают три слеша. То есть по факту начало пути выглядит так: <code>application:///</code></p>
<p>Если же наши ресурсы находятся вне приложения, например, это некий файл по пути <i>D://forest.jpg</i>, тогда вместо <code>application</code> использется 
название логического диска:</p>
<pre class="brush:c#;">myImage.Source = new BitmapImage(new Uri("pack://D:,,,forest.jpg"));</pre>
<h3>Ресурсы в других сборках</h3>
<p>Может сложиться ситуация, когда наше приложение использует внешнюю сборку, в которой были упакованы ресурсы. И мы также можем обратиться к этим ресурсам, 
используя синтаксис "упакованных" Uri. В этом случае путь будет иметь следующий формат:</p>
<pre>pack://application:,,,/[название_библиотеки];component/[путь_к_ресурсу]</pre>
<p>Допустим, файл сборки называется libs.dll, а ресурс, а сам ресурс в ней находится по пути <i>images/forest.jpg</i>, тогда получить его мы сможем следующим образом:</p>
<pre class="brush:c#;">myImage.Source = new BitmapImage(new Uri("pack://application:,,,/libs;component/images/river.jpg"));</pre>
<p>Либо можно использовать эквивалентный способ доступа к ресурсу:</p>
<pre class="brush:c#;">myImage.Source = new BitmapImage(new Uri("libs;component/images/river.jpg", UriKind.Relative));</pre>
<h3>Получение ресурсов</h3>
<p>Используя выше рассмотренные пути можно в программе получать сам файл ресурса и производить с ним манипуляции. Для получения ресурса приложения 
применяется метод <span class="b">Application.GetResourceStream()</span>, например:</p>
<pre class="brush:c#;">
...
<p>В данном случае мы считываем ресурс приложения и выполняем его копирование по новому пути вне приложения.</p>
...
<div class="nav"><p><a href="3.2.php">Назад</a><a href="./">Содержание</a><a href="12.php">Вперед</a></p></div></div>
<div class="socBlock">
