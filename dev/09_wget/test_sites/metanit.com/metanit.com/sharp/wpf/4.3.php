...
<html >
<head>
<title>C# и WPF | GridSplitter</title>
<meta charset="utf-8" />
<meta name="description" content="Использование элемента GridSplitter в WPF для настройки размеров столбцов и строк грида">
...
<h2>GridSplitter</h2><div class="date">Последнее обновление: 16.01.2016</div>
...
<p>Элемент <span class="bb">GridSplitter</span> помогает создавать интерфейсы наподобие элемента SplitContainer в WinForms, только более 
функциональные. Он представляет собой некоторый разделитель между столбцами или строками, путем сдвига которого можно регулировать ширину 
столбцов и высоту строк. В качестве примера можно привести стандартный интерфейс проводника в Windows, где разделительная полоса отделяет 
древовидный список папок от панели со списком файлов. Например,</p>
<pre class="brush:xml;">
&lt;Grid&gt;
...
<img src="./pics/3.4.png" alt="Элемент GridSplitter в WPF" />
<p>Двигая центральную линию, разделяющую правую и левую части, мы можем устанавливать их ширину.</p>
<p>Итак, чтобы использовать элемент GridSplitter, нам надо поместить его в ячейку в Gride. По сути это обычный элемент, такой же, как кнопка. 
Как выше, у нас три ячейки (так как три столбца и одна строка), и GridSplitter помещен во вторую ячейку. Обычно строка или столбец, в которые помещают 
элемент, имеет для свойств Height или Width значение <code>Auto</code>. </p>
<p>Если у нас несколько строк, и мы хотим, чтобы разделитель распространялся на несколько строк, то мы можем задать свойство <code>Grid.RowSpan</code>:</p>
<pre class="brush:xml;">
&lt;Grid.ColumnDefinitions&gt;
    &lt;ColumnDefinition Width="*" /&gt;
    &lt;ColumnDefinition Width="Auto" /&gt;
...
<p>В случае, если мы задаем горизонтальный разделитель, то тогда соответственно надо использовать свойство <code>Grid.ColumnSpan</code></p>
<p>Затем нам надо настроить свойства. Во-первых, надо настроить ширину (Width) для вертикальных сплитеров и высоту (Height) для горизонтальных. 
Если не задать соответствующее свойство, то сплитер мы не увидим, так как он изначально очень мал.</p>
<p>Затем нам надо задать выравнивание. Если мы хотим, что сплитер заполнял всю высоту доступной области (то есть если у нас вертикальный сплитер), то нам надо установить 
для свойства <span class="b">VerticalAlignment</span> значение <code>Stretch</code>.</p></li>
<p>Если же у нас горизонтальный сплитер, то надо установить свойство <span class="b">HorizontalAlignment</span> в <code>Stretch</code></p>
<p>Также в примере выше используется свойство <span class="b">ShowsPreview</span>. Если оно равно False, то изменение границ кнопок будет происходить сразу же при 
перемещении сплитера. Если же оно равно True, тогда изменение границ будет происходить только после того, как перемещение сплитера завершится, и при перемещении сплиттера 
мы увидим его проекцию.</p>
<p>В отличие от элемента SplitContainer в WinForms, в WPF можно установить различное количество динамически регулируемых частей окна. Немного усовершенствуем предыдущий пример: </p>
<pre class="brush:xml;">
&lt;Grid&gt;
    &lt;Grid.ColumnDefinitions&gt;
        &lt;ColumnDefinition Width="*" /&gt;
...
<p>Здесь у нас сразу два сплитера: один между двумя верхними и нижней панелями, а второй - между правой и левой панелями.</p>
