...
<html >
<head>
<title>C# и WPF | Всплывающие подсказки ToolTip и Popup</title>
<meta charset="utf-8" />
<meta name="description" content="Всплывающие подсказки и окна в WPF, настройка элемента ToolTip и его свойства, управление элементом Popup">
...
<h2>Всплывающие подсказки ToolTip и Popup</h2><div class="date">Последнее обновление: 18.01.2016</div>
...
<h3>ToolTip</h3>
<p>Элемент ToolTip представляет всплывающую подсказку при наведении на какой-нибудь элемент. Для определения всплывающей подсказки у элементов 
уже есть свойство ToolTip, которому можно задать текст, отображаемый при наведении:</p>
<pre class="brush:xml;">
&lt;Button Content="Tooltip" ToolTip="Всплывающая подсказка для кнопки" Height="30" Width="80" /&gt;
</pre>
<p>Также мы можем более точно настроить всплывающую подсказку с помощью свойства <code>Button.ToolTip</code>:</p>
<pre class="brush:xml;">
...
<p>Всплывающие подсказки можно применять не только кнопкам, но и ко всем другим элементам управления, например, к текстовому блоку:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>Оба определения всплывающей подсказки будут аналогичны.</p>
<p>Поскольку ToolTip является элементом управления содержимого, то в него можно встроить другие элементы для создания более богатой функциональности. 
Например:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<img src="./pics/4.5.png" alt="Элемент ToolTip в WPF" />
<p>Здесь у нас два переключателя, и на одном из них определен расширенный элемент ToolTip: а именно в него вложен элемент Image, выводящий изображение, 
и элемент TextBlock. Таким образом, можно создавать всплывающие подсказки с различным наполнением.</p>
<p>Изображение для элемента Image в данном случае было добавлено в проект.</p>
<h3>Свойства ToolTip</h3>
<p>Некоторые полезные свойства элемента Tooltip:</p>
<ul>
<li><p><span class="b">HasDropShadow</span>: определяет, будет ли всплывающая подсказка отбрасывать тень.</p></li>
<li><p><span class="b">Placement</span>: определяет, как будет позиционироваться всплывающая подсказка на окне приложения. 
По умолчанию ее верхний левый угол позиционируется на указатель мыши.</p></li>
<li><p><span class="b">HorizontalOffset/VerticalOffset</span>: определяет смещение относительно начального местоположения. </p></li>
<li><p><span class="b">PlacementTarget</span>: определяет позицию всплывающей подсказки относительно другого элемента управления.</p></li> 
</ul>
<p>Применим свойства:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>Здесь у нас три переключателя. У первого мы задаем свойства через элемент ToolTip. Для второго переключателя мы также можем задать свойства, 
несмотря на то, что здесь мы всплывающую подсказку задаем просто <code>ToolTip="Цена: 29990 рублей" Content="Nexus 5X"</code>. В этом случае 
мы можем использовать прикрепленные свойства класса <span class="b">ToolTipService</span>:</p>
<ul>
<li><p><code>InitialShowDelay</code>: задает задержку перед отображением всплывающей подсказки</p></li>
<li><p><code>ShowDuration</code>: устанавливает время отображения всплывающей подсказки</p></li>
<li><p><code>BetweenShowDelay</code>: устанавливает время, в течение которого пользователь сможет перейти к другому элементу с подсказкой, и для этого элемента не будет 
работать свойство InitialShowDelay (если оно указано)</p></li>
<li><p><code>ToolTip</code>: устанавливает содержимое всплывающей подсказки</p></li>
<li><p><code>HasDropShadow</code>: определяет, будет ли подсказка отбрасывать тень</p></li>
<li><p><code>ShowOnDisabled</code>: устанавливает поведение всплывающей подсказки для недоступного элемента (со значением <code>IsEnabled="True"</code>). 
Если это свойство равно true, то подсказка отображается для недоступных элементов. По умолчанию равно false.</p></li>
<li><p><code>Placement / HorizontalOffset / VerticalOffset / PlacementTarget</code>: те же свойства, что и у элемента ToolTip, которые устанавливают положение 
всплывающей подсказки</p></li>
</ul>
<h4>Программное создание всплывающей подсказки</h4>
<p>Допустим, в коде XAML у нас определена следующая кнопка:</p>
<pre class="brush:xml;">&lt;Button x:Name="button1" Content="Hello" /&gt;</pre>
<p>Тогда в файле кода C# мы могли бы определить всплывающую подсказку для кнопки так:</p>
<pre class="brush:c#;">
...
<h3>Popup</h3>
<p>Элемент Popup также представляет всплывающее окно, только в данном случае оно имеет другую функциональность. Если Tooltip отображается автоматически 
при наведении и также автоматически скрывается через некоторое время, то в случае с Popup все эти действия нам надо задавать вручную.</p>
<p>Так, чтобы отразить при наведении мыши на элемент всплывающее окно, нам надо соответственным образом обработать событие <code>MouseEnter</code>.</p>
<p>Второй момент, который надо учесть, это установка свойства <code> StaysOpen="False"</code>. По умолчанию оно равно True, а это значит, 
что при отображении окна, оно больше не исчезнет, пока мы не установим явно значение этого свойства в False.</p>
<p>Итак, создадим всплывающее окно:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>И обработчик наведения курсора мыши на кнопку в коде c#:</p>
<pre class="brush:xml;">
...
<img src="./pics/4.7.png" alt="Элемент Popup в WPF" />
<p>И при наведении указателя мыши на элемент появится всплывающее окно с сообщением.</p>
