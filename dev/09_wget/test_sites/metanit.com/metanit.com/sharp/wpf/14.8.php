...
<html >
<head>
<title>C# и WPF | Иерархические данные и HierarchicalDataTemplate</title>
<meta charset="utf-8" />
<meta name="description" content="Отображение иерархических данных в WPF, шаблон HierarchicalDataTemplate и привязка данных к элементам TreeView и Menu">
...
<h2>Иерархические данные и HierarchicalDataTemplate</h2><div class="date">Последнее обновление: 10.08.2016</div>
...
<p>В отличие от простых списковых элементов типа ListBox или ComboBox, элементы TreeView и Menu способны отображать иерархические данные, 
построенные по образу дерева. Как у дерева могут быть ветви, у которых, в свою очередь, также могут быть ветви, так и у TreeView могут быть узлы высшего уровня, которые могут содержать подузлы, а в подузлах также могут храниться подузлы.</p>
<p>Для работы именно с иерархическими данными в WPF имеется специальный тип шаблонов данных - <span class="b">HierarchicalDataTemplate</span>. 
Этот шаблон задает формат отображения уровня данных. Рассмотрим на примере.</p>
<p>Пусть у нас есть следующий класс:</p>
<pre class="brush:c#;">
public class Node
{
...
<p>Этот класс представляет иерархические данные - объект Node может содержать несколько других объектов Node.</p>
<p>Теперь в коде xaml определим элемент TreeView для отображения данных:</p>
<pre class="brush:xml;">
...
<p>С помощью свойства <code>ItemTemplate</code> у TreeView задается шаблон отображения для каждого объекта Node в виде HierarchicalDataTemplate.</p>
<p>Атрибут <code>ItemsSource="{Binding Path=Nodes}"</code> осуществляет привязку к внутренней коллекции Nodes, которая имеется в объекте Node.</p>
<p>Далее элемент <code>&lt;TextBlock Text="{Binding Name}" /&gt;</code> устанавливает привязку к свойству Name объекта Node.</p>
<p>Теперь в файле кода окна определим коллекцию объектов Node, которая будет отображаться в TreeView:</p>
<pre class="brush:c#;">
...
<p>Здесь просто создается список, который привязывается к TreeView. В итоге у нас получится следующее отображение:</p>
<img src="./pics/14.15.png" alt="HierarchicalDataTemplate в WPF" />
<p>Рассмотрим другой пример. Пусть объекты узлой будут отличаться по типу. Для этого определим следующие классы:</p>
<pre class="brush:c#;">
...
<p>Компания-производитель содержит список произведенных моделей телефонов.</p>
<p>Теперь изменим код TreeView в xaml:</p>
<pre class="brush:xml;">
&lt;TreeView x:Name="treeView1"&gt;
    &lt;TreeView.ItemTemplate&gt;
...
<p>Теперь для отображения внутренних данных, то есть объектов Phone задается дополнительный шаблон:</p>
<pre class="brush:xml;">
&lt;HierarchicalDataTemplate.ItemTemplate&gt;
    &lt;DataTemplate&gt;
        &lt;TextBlock Text="{Binding Title}" /&gt;
...
<p>Опять же в коде C# можно создать и установить источник данных для TreeView:</p>
<pre class="brush:c#;">
...
<img src="./pics/14.16.png" alt="Иерархические данные для TreeView в WPF" />
<p>Аналогичным образом можно определить шаблон для элемента Menu:</p>
<pre class="brush:xml;">
...
<img src="./pics/14.17.png" alt="HierarchicalDataTemplate в меню в WPF" />
<h3>Получение данных из xml</h3>
<p>Для представления иерархических данных очень удобен формат xml. И в WPF мы можем легко связн данные из xml с иерархическим элементом - TreeView или Menu. 
Допусти, у нас есть в проекте следующий файл <span class="ii">nodes.xml</span>:</p>
<pre class="brush:xml;">
...
<p>С помощью XmlDataProvider мы можем подключить этот файл к TreeView:</p>
<pre class="brush:xml;">
&lt;Window x:Class="DataApp.DataWindow"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
<div class="nav"><p><a href="14.7.php">Назад</a><a href="./">Содержание</a><a href="14.php">Вперед</a></p></div></div>
<div class="socBlock">
