...
<html >
<head>
<title>C# и WPF | ProgressBar и Slider</title>
<meta charset="utf-8" />
<meta name="description" content="Обзор элементов WPF, основанных на диапазонах значений (Range Controls) - ProgressBar и Slider, работа с ними, их основные свойства и события">
...
<h2>ProgressBar и Slider</h2><div class="date">Последнее обновление: 19.01.2016</div>
...
<p>ProgressBar и Slider представляют элементы, основанные на диапазонах значений. То есть они хранят и отображают числовые данные на определенном диапазоне.</p>
<p>Все они являются наследниками класса <span class="b">RangeBase</span>, поэтому наследуют такие его свойства, как:</p>
<ul>
<li><p><span class="b">Value</span> указывает на текущее значение элемента (представлено типом <code>Double</code>)</p></li>
<li><p><span class="b">Maximum/Minimum</span> указывает на максимальное/минимальное значение элемента</p></li>
</ul>
<h3>Slider</h3>
<p>Представляет собой обычный ползунок. Он добавляет следующие свойства:</p>
<ul>
<li><p><span class="b">Orientation</span>: указывает ориентацию ползунка - горизонтальную(Horizontal) или вертикальную (Vertical)</p></li>
<li><p><span class="b">Delay</span>: указывает время в миллисекундах, по истечении которого ползунок переместится на одну единицу после щелчка.</p></li>
<li><p><span class="b">Interval</span>: указывает время в миллисекундах, по истечении которого ползунок может перемещаться</p></li>
<li><p><span class="b">TickPlacement</span>: задает визуализацию шкалы ползунка. По умолчанию имеет значение None (отсутствие шкалы). Значение BottomRight создают шкалу в нижней части ползунка,
TopLeft - в верхней, Both - по обоим сторонам.</p></li>
<li><p><span class="b">TickFrequency</span>: указывает частоту появления отметок на шкале ползунка.</p></li>
<li><p><span class="b">IsSelectionRangeEnabled</span>: задает затенение участка ползунка. Если оно установлено в True, то начальная и конечная отметка 
затенения задаются с помощью свойств <span class="b">SelectionStart</span> и <span class="b">SelectionEnd</span>.</p></li>
</ul>
<p>Простейший слайдер:</p>
<pre class="brush:xml;">
...
<p>В файле кода мы можем прописать обработчик Slider_ValueChanged, который будет срабатывать при возникновении события ValueChanged - изменении значения слайдера. 
В данном случае обработчик этого события будет изменять выделение слайдера:</p>
<pre class="brush:c#;">
...
<img src="./pics/4.29.png" alt="Элемент Slider в WPF" />
<h3>ProgressBar</h3>
<p>ProgressBar представляет индикатор, отображающий выполнение определенного процесса. Также имеет свойство <span class="b">Orientation</span>, 
которое устанавливает вертикальное или горизонтальное расположение индикатора. Для связи с реальными процессами вся логика изменения индикатора, его свойства Value должна отрабатываться в коде.</p>
<pre class="brush:xml;">
&lt;ProgressBar Minimum="0" Maximum="100" Value="50" Height="25" /&gt;
</pre>
<img src="./pics/4.30.png" alt="Элемент ProgressBar в WPF" />
