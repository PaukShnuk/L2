...
<html >
<head>
<title>TCP-сервер. Класс TcpListener в C# и .NET</title>
<meta charset="utf-8" />
<meta name="description" content="Создание TCP-сервера в C#, использование класса TcpListener">
...
<h2>TCP-сервер. Класс TcpListener</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Класс TcpListener прослушивает входящие подключения по определенному порту.</p>
<p>Для создания объекта слушателя надо передать ему в конструктор объект IPAddress и порт, по которому будет вестись прослушивание входящих подключений:</p> 
<pre class="brush:c#;">
...
<p>Для запуска сервера вызывается метод <span class="bb">Start()</span>.</p>
<p>Когда к серверу обращается клиент, то мы можем использовать один из двух методов <span class="bb">AcceptSocket</span> или 
<span class="bb">AcceptTcpClient</span> для получения соответственно объекта <span class="bb">Socket</span> или <span class="bb">TcpClient</span>, 
которые будут использоваться для взаимодействия с подключенным клиентом.</p>
<pre class="brush:c#;">
TcpClient client = server.AcceptTcpClient();
// или
Socket socket = server.AcceptSocket();
</pre>
<p>Методы AcceptSocket и AcceptTcpClient блокируют выполняющий поток, пока сервер не обслужит подключенного клиента. Затем через методы, определенные 
в классах TcpClient и Socket, можно взаимодействовать с подключенным клиентом: получать от него данные или, наоборот, отправлять ему.</p>
<p>В конце по завершению работы сервера надо вызвать метод <span class="bb">Stop()</span>.</p>
<p>В прошлой теме был создан TCP-клиент. Теперь создадим примитивный TCP-сервер, к которому будет подключаться клиент:</p>
<pre class="brush:c#;">
using System;
using System.Net;
...
<p>В данном случае сервер прослушивает все подключения по порту 8888. В бесконечном цикле он получает входящие подключения.</p>
<p>У входящих объектов TcpClient получает сетевой поток через метод <code>client.GetStream()</code> и затем его использует для отправки сообщения.</p>
<p>И мы можем совместить клиента из прошлой темы с этим сервером. Тогда сервер будет отправлять клиентам данные, а клиенты - получать их.</p>
