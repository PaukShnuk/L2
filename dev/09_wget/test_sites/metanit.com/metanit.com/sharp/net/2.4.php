...
<html >
<head>
<title>Обработка ошибок при запросах в C# и .NET</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка ошибок при отправке запросов с помощью WebRequest в C#, типы исключений, примеры обработки">
...
<h2>Обработка ошибок при запросах</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>При работе с классами WebRequest и WebResponse могут возникать исключения. Непосредственно к работе с сетью относится класс исключений 
<span class="bb">WebException</span>. Его ключевым свойством является свойство <span class="bb">Status</span>, которое позволяет получит тип 
исключения.</p>
<p>Это свойство принимает одно из значений перечисления <span class="bb">WebExceptionStatus</span>:</p>
<ul>
<li><p><code>ConnectFailure</code>: невозможно подключиться к ресурсу</p></li>
<li><p><code>ConnectionClosed</code>: подключение было преждевременно закрыто</p></li>
<li><p><code>KeepAliveFailure</code>: сервер закрыл подключение, для которого был установлен заголовок Keep-Alive</p></li>
<li><p><code>MessageLengthLimitExceeded</code>: запрос превышает допустимый размер</p></li>
<li><p><code>NameResolutionFailure</code>: служба DNS не может сопоставить имя хоста с ip-адресом</p></li>
<li><p><code>ProtocolError</code>: запрос был завершен, но возникла ошибка на уровне протокола, например, запрошенный ресурс не был найден</p></li>
<li><p><code>ReceiveFailure</code>: от удаленного сервера не был получен полный ответ</p></li>
<li><p><code>RequestCanceled</code>: запрос был отменен</p></li>
<li><p><code>SecureChannelFailure</code>: при установлении подключения с использованием SSL произошла ошибка</p></li>
<li><p><code>SendFailure</code>: полный запрос не был передан на удаленный сервер</p></li>
<li><p><code>ServerProtocolViolation</code>: ответ сервера не являлся допустимым ответом HTTP</p></li>
<li><p><code>Timeout</code>: ответ не был получен в течение определенного времени</p></li>
<li><p><code>TrustFailure</code>: сертификат сервера не может быть проверен</p></li>
<li><p><code>UnknownError</code>: возникло исключение неизвестного типа</p></li>
</ul>
<p>Обработка исключения и возможное логгирование ошибок позволит определить природу неполадки, что в дальнейшем поможет найти и исправить ошибку. 
Например, обратимся к несуществующему ресурсу:</p>
<pre class="brush:c#;">
try
{
...
<p>И консоль выведет сообщение об ошибке:</p>
<div class="console">
...
<div class="nav"><p><a href="2.3.php">Назад</a><a href="./">Содержание</a><a href="3.1.php">Вперед</a></p></div></div>
<div class="socBlock">
