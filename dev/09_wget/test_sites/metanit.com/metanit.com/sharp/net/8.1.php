...
<html >
<head>
<title>Работа с электронной почтой в C# и .NET</title>
<meta charset="utf-8" />
<meta name="description" content="Использование класса SmtpClient для отправки почты в C#, добавление к письму файлов">
...
<h2>Отправка почты. SmtpClient</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для отправки почты в среде интернет используется протокол SMTP (Simple Mail Transfer Protocol). Данный протокол указывает, как почтовые сервера 
взаимодействуют при передаче электронной почты.</p>
<p>Для работы с протоколом SMTP и отправки электронной почты в .NET предназначен класс <span class="bb">SmtpClient</span> из 
пространства имен <span class="bb">System.Net.Mail</span>.</p>
<p>Этот класс определяет ряд свойств, которые позволяют настроить отправку:</p>
<ul>
<li><p><code>Host</code>: smtp-сервер, с которого производится отправление почты. Например, smtp.yandex.ru</p></li>
<li><p><code>Port</code>: порт, используемый smp-сервером. Если не указан, то по умолчанию используется 25 порт.</p></li>
<li><p><code>Credentials</code>: аутентификационные данные отправителя</p></li>
<li><p><code>EnableSsl</code>: указывает, будет ли использоваться протокол SSL при отправке</p></li>
</ul>
<p>Еще одним ключевым классом, который используется при отправке, является <span class="bb">MailMessage</span>. Данный класс представляет 
собой отправляемое сообщение. Среди его свойств можно выделить следующие:</p>
<ul>
<li><p><code>Attachments</code>: содержит все прикрепления к письму</p></li>
<li><p><code>Body</code>: непосредственно текст письма</p></li>
<li><p><code>From</code>: адрес отправителя. Представляет объект MailAddress</p></li>
<li><p><code>To</code>: адрес получателя. Также представляет объект MailAddress</p></li>
<li><p><code>Subject</code>: определяет тему письма</p></li>
<li><p><code>IsBodyHtml</code>: указывает, представляет ли письмо содержимое с кодом html</p></li>
</ul>
<p>Используем эти классы и выполним отправку письма:</p>
<pre class="brush:c#;">
using System;
using System.Net;
using System.IO;
...
<p>Для отправки применяется метод <code>Send()</code>, в который передается объект MailMessage.</p>
<p>Также мы можем использовать асинхронную версию отправки с помощью метода <code>SendMailAsync</code>:</p>
<pre class="brush:c#;">
using System;
using System.Net;
using System.IO;
...
<h3>Добавление вложений</h3>
<p>К письму мы можем прикрепить вложения с помощью свойства Attachments. Каждое вложение представляет объект <span class="bb">System.Net.Mail.Attachment</span>:</p>
<pre class="brush:c#;">
MailAddress from = new MailAddress("somemail@gmail.com", "Tom");
...
<div class="nav"><p><a href="7.1.php">Назад</a><a href="./">Содержание</a><a href="9.1.php">Вперед</a></p></div></div>
<div class="socBlock">
