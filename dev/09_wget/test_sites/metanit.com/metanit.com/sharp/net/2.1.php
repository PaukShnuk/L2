...
<html >
<head>
<title>Класс WebClient в C# и .NET</title>
<meta charset="utf-8" />
<meta name="description" content="Использование класса WebClient в C# для отправки запросов к сетевым ресурсам. Создание асинхронных запросов">
...
<h2>Класс WebClient</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для загрузки файлов с определенных ресурсов предназначен класс <span class="bb">WebClient</span>, который находится в пространстве имен 
System.Net.</p>
<p>Самый простой способ загрузки предоставляет метод <span class="bb">DownloadFile()</span>. Например, загрузим файл с какого-нибудь сайта:</p>
<pre class="brush:c#;">
WebClient client = new WebClient();
client.DownloadFile("http://somesite.com/book.pdf", "myBook.pdf");
Console.WriteLine("Файл загружен");
</pre>
<p>Метод <code>DownloadFile()</code> принимает два параметра. Первый параметр указывает на файл в интернете, который надо загрузить, а второй параметр 
определяет имя загруженного файла на локальном компьютере. После загрузки в папке с приложением появится файл myBook.pdf.</p>
<p>WebClient также позволяет получить файл в качестве потока и затем манипулировать этим потоком в процессе загрузки:</p>
<pre class="brush:c#;">
using System;
using System.Net;
using System.IO;
...
<p>Для открытия потока используется метод <span class="bb">OpenRead()</span>, в который передается адрес файла.</p>
<p>Для чтения потока применяется класс <code>StreamReader</code>. Затем прочитанные строки выводятся на консоль.</p>
<p>Загрузку файла можно осуществлять в асинхронном режиме:</p>
<pre class="brush:c#;">
using System;
using System.Net;
