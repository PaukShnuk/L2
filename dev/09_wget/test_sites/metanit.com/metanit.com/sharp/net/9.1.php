...
<html >
<head>
<title>Протокол FTP в C# и .NET</title>
<meta charset="utf-8" />
...
<h1>Протокол FTP</h1>
<h2>FtpWebRequest и FtpWebResponse</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Протокол FTP (File Transfer Protocol) предназначен для передачи файлов по сети. Он работает поверх протокола TCP и использует 21 порт. Однако так как 
...
уже есть готовые решения. Эти решения представляют классы <span class="bb">FtpWebRequest</span> и <span class="bb">FtpWebResponse</span>. 
Эти классы являются производными от WebRequest и WebResponse и позволяют отправлять запрос к FTP-серверу и получать от него ответ.</p>
<p>FtpWebRequest позволяет отправить запрос к серверу. Для настройки запроса мы можем использовать следующие его свойства:</p>
<ul>
<li><p><code>Credentials</code>: задает или возвращает аутентификационные данные пользователя</p></li>
<li><p><code>EnableSsl</code>: указывает, надо ли использовать ssl-соединение</p></li>
<li><p><code>Method</code>: задает команду протокола FTP, которая будет использоваться в запросе</p></li>
<li><p><code>UsePassive</code>: при значении true устанавливает пассивный режим запроса к серверу</p></li>
<li><p><code>UseBinary</code>: указывает тип данных, которые будут использоваться в запросе. Значение true указывает, что передаваемые данные являются двоичными, а 
значение false - что данные будут представлять текст. Значение по умолчанию — true</p></li>
</ul>
<p>Например, загрузим текстовый файл с фтп-сервера:</p>
<pre class="brush:c#;">
using System;
using System.IO;
using System.Net;
...
<p>В данном случае идет обращение к фтп-серверу "ftp://127.0.0.1", который я поднял на локальном компьютере, но это может быть любой адрес рабочего фтп-сервера. 
Если нам надо просто загрузить файл, то мы можем использовать метод</p>
<pre class="brush:c#;">request.Method = WebRequestMethods.Ftp.DownloadFile</pre>
<p>При отправке запроса нам надо указать соответствующий метод.</p>
<p>Если фтп-сервер требует установки логина и пароля, то применяется свойство Credentials:</p>
<pre class="brush:c#;">request.Credentials = new NetworkCredential("login", "password");</pre>
<p>Если сервер использует ssl-соединение, то надо его установить в запросе: <code>request.EnableSsl = true</code></p>
<p>После отправки запроса мы можем получить ответ в  виде FtpWebResponse:</p>
<pre class="brush:c#;">
FtpWebResponse response = (FtpWebResponse)request.GetResponse();
Stream responseStream = response.GetResponseStream();
</pre>
<p>Получив поток ответа, мы можем им манипулировать. В данном случае с помощью FileStream сохраняем файл в папку программы под именем newTest.txt.</p>
...
<div class="nav"><p><a href="8.1.php">Назад</a><a href="./">Содержание</a><a href="9.2.php">Вперед</a></p></div></div>
<div class="socBlock">
