...
<html >
<head>
<title>Чат c широковещательной рассылкой на Windows Forms в C#</title>
<meta charset="utf-8" />
<meta name="description" content="Создание графического чата на C# для протокола UDP, использование широковещательной рассылки">
...
<h2>Чат c широковещательной рассылкой на Windows Forms</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Использовать консольные чаты и подобные приложения для работы в сети не очень удобно. Гораздо проще использовать графические программы, 
поэтому создадим новый проект на Windows Forms, который также будет использовать широковещательную рассылку и который будет называться UdpChat.</p>
<p>Главная и единственная форма приложения будет выглядеть следующим образом:</p>
<img src="pics/udpchatproject.png" alt="Проект чата с широковещательной рассылкой на UDP" />
<p>На форме есть две кнопки loginButton и logoutButton соответственно для входа и выхода из чата, а также кнопка sendButton для отправки сообщений. 
Для ввода имени, которое будет использоваться в чате, есть текстовое поле userNameTextBox. Для ввода сообщения внизу имеется текстовое поле 
messageTextBox, которое является многострочным, то есть у которого свойство MultiLine имеет значение true.</p>
<p>И в самом центре размещено многострочное текстовое поле chatTextBox, в которое будут выводиться полученные сообщения.</p>
<p>Весь код формы:</p>
<pre class="brush:c#;">
using System;
using System.Net;
...
<p>Все клиенты будут подключаться к общему адресу "235.5.5.1". По нажатию на кнопку входа будет происходить отправка сообщения о входе нового пользователя 
и запускаться новая задача на прием входящих сообщений.</p>
<p>Для приема сообщений будет использоваться метод <code>ReceiveMessages()</code>. Так как в этой задаче мы обращаемся к элементам, созданным в главном потоке, то для добавления сообщения 
надо использовать делегат MethodInvoker:</p>
<pre class="brush:c#;">
this.Invoke(new MethodInvoker(() =&gt;
{
    string time = DateTime.Now.ToShortTimeString();
...
<p>В качестве параметра он принимает либо ссылку на метод, либо лямбда-выражение.</p>
<p>По нажатию на кнопку sendButton происходит отправка сообщения. И по нажатию на кнопку выхода или по закрытию формы срабатывает метод <code>ExitChat()</code>, 
которые прекращает поток и осуществляет выход из чата.</p>
<p>При выходе мы можем столкнуться с проблемой. При прослушивании входящих сообщений задача останавливается на строке</p>
<pre class="brush:c#;">byte[] data = client.Receive(ref remoteIp);</pre>
<p>В ожидании приема сообщений. Когда происходит выход из чата по закрытию формы, то всем, в том числе и этому ожидающему клиенту посылается 
сообщение о выходе. Однако при этом идет закрытие и уничтожение формы, в связи с чем мы можем столкнуться с исключением ObjectDisposedException. Поэтому при приеме сообщений 
включается дополнительный блок обработки исключений:</p>
<pre class="brush:c#;">
catch (ObjectDisposedException)
{
    if (!alive)
...
<p>Поэтому если исключение было вызвано при закрытии потока, то мы его не обрабатываем. Иначе выбрасываем ошибку.</p>
<p>Запустим чат на нескольких машинах в рамках одной подсети и мы сможем взаимодействовать со всеми подключенными клиентами:</p>
<img src="pics/udpchat.png" alt="Чат с широковещательной рассылкой на UDP в Windows Forms" />
