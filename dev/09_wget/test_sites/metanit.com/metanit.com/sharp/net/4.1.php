...
<html >
<head>
<title>Протокол TCP в C# и .NET</title>
<meta charset="utf-8" />
<meta name="description" content="Основы работы с протоколом TCP в C#, создание TCP-клиента">
...
<h1>Протокол TCP</h1><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Огромное количество трафика и взаимодействия в сети сейчас происходит по протоколу TCP (Transmission Control Protocol). Этот протокол гарантирует 
доставку сообщений и широко используется в различных существующих на сегодняшний день программах.</p>
<p>Для работы с протоколом TCP в .NET предназначены классы <span class="bb">TcpClient</span> и <span class="bb">TcpListener</span>. 
Эти классы строятся поверх класса <span class="bb">System.Net.Sockets.Socket</span>.</p>
<h2>TCP-клиент. Класс TcpClient</h2>
<p>Для создания клиентской программы, работающей по протоколу TCP, предназначен класс <span class="bb">TcpClient</span>.</p>
<p>Для подключения к серверу TCP, в этом классе определен метод <code>Connect()</code>, которому передается название хоста и порт:</p>
<pre class="brush:c#;">
TcpClient tcpClient = new TcpClient ();
tcpClient.Connect ("www.microsoft.com", 80);
</pre>
<p>Чтобы взаимодействовать с сервером TcpClient определяет метод <code>GetStream()</code>, который возвращает объект <span class="bb">NetworkStream</span>. 
Через данный объект можно передавать сообщения серверу или, наоборот, получать данные с сервера.</p>
<pre class="brush:c#;">
NetworkStream netStream = tcpClient.GetStream();
</pre>
<p>После окончания работы с TcpClient его надо закрыть методом <code>Close()</code>.</p>
<pre class="brush:c#;">
tcpClient.Close();
</pre>
<p>Для отправки данных у потока используется метод <code>Write()</code>:</p>
<pre class="brush:c#;">
...
<p>Для чтения используется метод <code>Read()</code> класса NetworkStream:</p>
<pre class="brush:c#;">
NetworkStream stream = tcpClient.GetStream();
...
<p>При этом в обоих случаях нам надо учитывать тип кодировки: он должен совпадать с кодировкой, в которой сервер отправляет или получает сообщения. 
В данном случае используется кодировка UTF8.</p>
<p>Полноценный код клиента, выполняющий чтение из потока, мог бы выглядеть так:</p>
<pre class="brush:c#;">
using System;
...
<p>В данном случае предполагается, что по локальному адресу 127.0.0.1 на порту 8888 запущен tcp-сервер.</p>
<p>Мы ожидаем, что сервер будет передавать строку в кодировке UTF8, и для создания строки определяется объект StringBuilder.</p>
<p>Для считывания данных создаем массив байтов: <code>byte[] data = new byte[256]</code>. Однако поскольку данные могут иметь больший размер, 
то считываем их из потока в цикле do..while. При этом проверяется значение <code>stream.DataAvailable</code> на наличие данных в потоке.</p>
