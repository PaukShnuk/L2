...
<html >
<head>
<title>Широковещательная рассылка на UDP в C# и .NET</title>
<meta charset="utf-8" />
<meta name="description" content="Широковещательная рассылка с помощью протокола UDP в C#, построение многопользовательского чата">
...
<h2>Широковещательная рассылка</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Протокол UDP позволяет рассылать сообщения с помощью широковещательной групповой рассылки. При такой рассылке клиенту достаточно отправить одно сообщение, и 
его получат все остальные клиенты, которые подключены к группе. И нет надобности отправлять одно сообщение каждому отдельному клиенту.</p> 
<p>При использовании широковещательной передачи надо учитывать, что передача не идет дальше локальных сетей, так как маршрутизаторы подобные рассылки не пропускают.</p>
<p>Ключевым в данном случае является метод <span class="bb">JoinMulticastGroup()</span>, который позволяет присоединиться клиенту к группе в локальной сети. Благодаря этому объект UdpClient 
сможет получать дейтаграммы, которые предназначаются всей группы объектов UdpClient.</p>
<p>Для присоединения к группе в метод JoinMulticastGroup передается адрес группы. В качестве адреса может использоваться один из адресов в диапазоне 
от 224.0.0.0 до 239.255.255.255.</p>
<p>Создадим приложение, которое будет использовать широковещательную рассылку. Если оталкиваться от проекта из прошлой темы, то для поддержки групповой рассылки 
достаточно добавить в потоке получения данных вызов метода <code>JoinMulticastGroup()</code>:</p>
<pre class="brush:c#;">
...
<p>Метод <code>JoinMulticastGroup()</code> принимает два параметра: адрес групповой рассылки (должен быть одним и тем же для всей группы клиентов) и 
число проходов через маршрутизаторы. То есть пока сообщение достигнет получателя, оно может пройти через некоторое число маршрутизаторов, которые направят его 
по нужному пути. В данном случае число 50 означает, что чтобы пройти от компьютера-отправителя к компьютеру-получателю, дейтаграмма может миновать 
50 маршрутизаторов.</p>
<p>На основе Udp-чата из прошлой темы сделаем консольный чат с применением широковещательной рассылки:</p>
<pre class="brush:c#;">
using System;
using System.Net;
...
<p>Теперь для групповой рассылки в локальной сети будет использоваться адрес "235.5.5.11", в качестве порта - 8001. Причем поскольку отправленное пользователем 
сообщение будет транслироваться всем в группе, в том числе и ему самому, то чтобы сообщение не дублировать в консоли, то в данном случае проверяется адрес 
отправителя с помощью вспомогательного метода LocalIPAddress(). Если адрес отправителя и получателя совпадает, то ему сообщение не выводится.</p>
<p>После построения проекта мы можем запустить скомпилированное приложение на разных машинах в локальной сети.</p>
<p>Первый клиент:</p>
<div class="console">
