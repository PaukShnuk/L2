...
<html >
<head>
<title>Сетевые потоки бинарных данных в C# и .NET</title>
<meta charset="utf-8" />
<meta name="description" content="Использование классом BinaryReader и BinaryWriter для приема и отправки данных по сети в C#, пример финансового приложения, работающего по сети">
...
<h2>Потоки бинарных данных</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Создадим небольшое финансовое клиент-серверное приложение для демонстрации того, как мы можем отправлять и получать данные примитивных типов. 
Для работы с примитивными типами в .NET используются классы BinaryReader и BinaryWriter. Подобно тому, как с их помощью можно писать данные в файл, 
также можно добавлять данные в поток NetworkStream.</p>
<p>Наше приложение будет регистрировать вклады от клиентов. Клиент направляет запрос на открытие вклада, а сервер создает вклад и посылает ответ.</p>
<p>Вначале создадим проект сервера. И первым делом добавим в него класс Account, который будет представлять модель вклада:</p>
<pre class="brush:c#;">
...
<p>Затем добавим класс ClientObject, который будет выполнять обработку запроса от клиента:</p>
<pre class="brush:c#;">
...
<p>Для считывания данных обертываем объект NetworkStream в BinaryReader. Затем с помощью методов считываем отдельные данные примитивных типов из потока.</p>
<p>После считывания создаем объект счета Account. При необходимости его можно сохранить в базу данных, выполнить с ним какие-либо действия, но в данном 
случае просто выводим сообщение на консоль.</p>
<p>И далее с помощью BinaryWriter пишем в ответ клиенту номер нового счета.</p>
<p>И теперь используем этот класс в главном классе программы:</p>
<pre class="brush:c#;">
using System;
using System.Net;
...
<p>И также создадим проект клиента:</p>
<pre class="brush:c#;">
using System;
using System.Net.Sockets;
using System.IO;
...
<p>Принципы работы клиента будут те же самые, что и сервера.</p>
<p>Запустим программы сервера и клиента. Клиент:</p>
<div class="console">
<pre class="consoletext">
Для регистрации вклада введите данные!
Укажите ваше имя: Евгений
Укажите сумму вклада: 300
Укажите период вклада в месяцах: 7
Номер вашего счета 874364398dc-23dc-8841-6043829843489
</pre>
</div>
<p>Сервер:</p>
<div class="console">
...
Евгений зарегистрировал счет на сумму 300
</pre>
</div>
