...
<html >
<head>
<title>Адреса в .NET</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с ip-адресами в C#, использование DNS-служб, классы IPAddress и IPHostEntry">
...
<h2>Адреса в .NET</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Прежде чем отправить запрос к какому-нибудь ресурсу, компьютер обращается к DNS-северу, чтобы по имени ресурса получить его ip-адрес. И затем 
уже обращается по этому ip-адресу.</p>
<p>Все ip-адреса представляют 32-битное (протокол IPv4) или 128-битное значение (протокол IPv6), например, <code>31.170.165.181</code>.</p>
<p>В системе классов .NET ip-адрес представлен классом <span class="bb">IPAddress</span>. Этот класс позволяет управлять адресами с помощью следующих свойств и методов:</p>
<ul>
<li><p>Метод <code>Parse()</code>: преобразует строковое представление адреса в IPAddress</p>
<pre class="brush:c#;">IPAddress ip = IPAddress.Parse("127.0.0.1"); // ip указывает на локальный адрес</pre></li>
<li><p>Статическое свойство <code>Loopback</code>: возвращает объект IPAddress для адреса 127.0.0.1. Аналогично вышеприведенному коду</p></li>
<li><p>Статическое свойство <code>Any</code>: возвращает объект IPAddress для адреса 0.0.0.0</p></li>
<li><p>Статическое свойство <code>Broadcast</code>: возвращает объект IPAddress для адреса 255.255.255.255</p></li>
</ul>
<p>Также для получения адреса в сети используется класс <span class="bb">IPHostEntry</span>. Он содержит информацию об определенном компьютере-хосте.</p>
<p>С помощью свойства <span class="bb">HostName</span> этот класс возвращает имя хоста, а с помощью свойства <span class="bb">AddressList</span> - 
все ip-адреса хоста, так как один компьютер может иметь в сети несколько ip-адресов.</p>
<p>Для взаимодействия с dns-сервером и получения ip-адреса применяется класс <span class="bb">Dns</span>. Для получения информации о хосте компьютера 
и его адресах у него имеется метод <span class="bb">GetHostEntry()</span>:</p>
<pre class="brush:c#;">
...
<img src="pics/1.2.png" alt="Получение ip-адреса в C# и .NET" />
