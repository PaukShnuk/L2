...
<html >
<head>
<title>Отправка данных в запросе в C# и .NET</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка параметров и данных в запросе с помощью WebRequest в C#">
...
<h2>Отправка данных в запросе</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Если мы хотим отправить данные в запросе, то нам надо добавить их в поток запроса. Например, мы хотим обратиться к веб-приложению ASP.NET MVC, 
в котором определен контроллер Home и метод PostData, принимающий данные:</p>
<pre class="brush:c#;">
...
<p>В данном случае конкретная технология не важна. Это мог бы быть и скрипт PHP, который принимает данные:</p>
<pre class="brush:php;">
&lt;?php
$sName = "Не известно";
...
<p>Теперь передадим ресурсу данные:</p>
<pre class="brush:c#;">
...
<p>Тут надо отметить следующий момент. Мы точно знаем, что параметр, который получает веб-ресурс, должен называться "sName". Поэтому при запросе 
формируются данные в формате <i>название_параметра=данные</i>: <code>"sName=Hello world!"</code></p>
<p>Еще также надо отметить, что для кодирования данных устанавливается тип содержимого "application/x-www-form-urlencoded".</p>
<p>После получения потока запроса ()<code>Stream dataStream = request.GetRequestStream()</code>) данные добавляются в запрос.</p>
<p>А после обращения к веб-ресурсу консоль отобразит нам ответ:</p>
<div class="console">
...
<p>Если веб-ресурс должен принять несколько параметров, например:</p>
<pre class="brush:c#;">
[HttpPost]
...
<p>То при отправке параметры разделяются амперсандом:</p>
<pre class="brush:c#;">
WebRequest request = WebRequest.Create("http://localhost:5374/Home/PostData");
request.Method = "POST";
...
<p>Ну и если нам надо отправить запрос GET, то все параметры можно передать в строке запроса:</p>
<pre class="brush:c#;">
