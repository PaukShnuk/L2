...
<html >
<head>
<title>Протокол HTTP в C# и .NET</title>
<meta charset="utf-8" />
<meta name="description" content="Основы работы с протоколом HTTP в C# и .NET, использование класса HttpListener для обработки запосов по HTTP">
...
<h1>Протокол HTTP</h1>
<h2>HttpListener</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для прослушивания подключений по протоколу HTTP и ответа на HTTP-запросы предназначен класс <span class="bb">HttpListener</span>. Данный класс построен на базе библиотеки 
HTTP.sys, которая является слушателем режима ядра, обрабатывающим весь трафик HTTP для Windows.</p>
<p>Для работы с HttpListener нам надо вначале задать адреса, которые будут использоваться для обращения к приложению. 
Адреса задаются через свойство <span class="bb">Prefixes</span> класса HttpListener. При этом адрес должен оканчиваться на слеш, например: 
<code>http://somesite.com:8888/connection/</code></p>
<p>Чтобы начать прослушивать входящие подключения, надо вызвать метод <code>Start()</code> и затем метод <code>GetContext()</code> класса HttpListener.</p>
<p>Напишем небольшое приложение:</p>
<pre class="brush:c#;">
using System;
using System.Net;
using System.IO;
...
<p>В данном случае наша программа будет прослушивать все обращения по локальному адресу <i>http://localhost:8888/connection/</i></p>
<p>При вызове метода <code>listener.GetContext()</code> текущий поток блокируется, и слушатель начинает ожидать входящие подключения. Этот метод возвращает 
объект <span class="bb">HttpListenerContext</span>, с помощью которого можно получить доступ к объектам запроса и ответа.</p>
<p>Для получения запроса используется свойство <code>context.Request</code>, которое возвращает объект <span class="bb">HttpListenerRequest</span>, 
а для получения объекта ответа - свойство <code>context.Response</code>. После получения ответа получаем выходной поток и пишем в него массив байтов:</p>
<pre class="brush:c#;">
Stream output = response.OutputStream;
output.Write(buffer, 0, buffer.Length);
output.Close();
</pre>
<p>В итоге, если мы запустим приложение и обратимся в каком-нибудь браузере по адресу <i>http://localhost:8888/connection/</i>, то нам отобразится 
сформированный в приложении код html:</p>
<img src="pics/httplistener.png" alt="HttpListener в C# и .NET" />
<p>Также мы использовать асинхронную версию метода <code>GetContext()</code> - <code>GetContextAsync()</code>:</p>
<pre class="brush:c#;">
...
<p>И также чтобы постоянно прослушивать всходящие подключения, можно заключить код слушателя в бесконечный 
цикл while:</p>
<pre class="brush:c#;">
private static async Task Listen()
{
...
<div class="nav"><p><a href="6.2.php">Назад</a><a href="./">Содержание</a><a href="8.1.php">Вперед</a></p></div></div>
<div class="socBlock">
