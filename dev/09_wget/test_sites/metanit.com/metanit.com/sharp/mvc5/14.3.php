...
<html >
<head>
<title>Компоненты Bootstrap</title>
<meta charset="utf-8" />
...
<h2>Компоненты Bootstrap</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Bootstrap имеет ряд компонентов, которые не сводятся к стандартным кнопкам или текстовым полям, а представляют более сложные элементы. 
Полный список компонентов Bootstrap можно найти на официальной странице проекта <a href="http://getbootstrap.com/components/" rel="nofollow">http://getbootstrap.com/components/</a>. 
Рассмотрим вкратце некоторые из них.</p>
<h3>Навигационные панели</h3>
<p>Первый компонент Bootstrap, с которым мы сталкиваемся в проекте, это навигационная панель. Данный компонент располагается на мастер-странице 
_Layout:</p>
<pre class="brush:xml;">
...
<p>И даже если мы сузим границы веб-браузера или запустим сайт на мобильном устройстве, то мы увидим, что панель навигации остается достаточно функциональной:</p>
<img src="./pics/14.9.png" />
<p>Компонент представляет класс <b>navbar</b>. Чтобы панель была фиксирована по верху страницы, используется также класс <span class="bb">navbar-fixed-top</span>. 
Если бы мы, наоборот, захотели фиксировать панель по низу, то в этом случае использовали бы класс <span class="bb">navbar-fixed-bottom</span></p>
<p>И также в объявлении блока навигации используется класс <code>navbar-inverse</code>, который инвертирует цвета по умолчанию. Вместо этого класса 
мы могли бы использовать <code>navbar-default</code>, тогда в этом случае у нас бы было меню стандартных светлых тонов.</p>
<p>Для создания ссылок навигации применяется класс <span class="bb">nav</span>. Bootstrap представляет несколько классов для оформления ссылок навигации. 
По умолчанию используется класс <code>navbar-nav</code>, но мы можем использовать и другие возможности.</p>
<p>Для создания навигации по типу вкладок применяется класс <span class="bb">nav-tabs</span>. Так, например следующее меню:</p>
<pre class="brush:xml;">
...
<p>даст такой эффект:</p>
<img src="./pics/14.10.png" />

<p>Еще один вариант создания панели ссылок представляет класс <span class="bb">nav-pills</span>:</p>
<pre class="brush:xml;">
...
<p>А комбинируя классы nav-pills и nav-stacked, мы можем создать вертикальное меню:</p>
<pre class="brush:xml;">
&lt;ul class="nav nav-pills nav-stacked"&gt;
	...............................
&lt;/ul&gt;
</pre>
<img src="./pics/14.12.png" />
<h4>Пагинация</h4>
<p>Класс <span class="bb">pagination</span> позволяет создать панель ссылок в виде постраничной навигации:</p>
<pre class="brush:xml;">
&lt;ul class="pagination"&gt;
...
<img src="./pics/14.13.png" />
<h4>Заглавия</h4>
<p>Для создания ссылок заголовков применяется класс <span class="bb">breadcrumb</span>:</p>
<pre class="brush:xml;">
&lt;ul class="breadcrumb"&gt;
    &lt;li class ="active"&gt;@Html.ActionLink("Home", "Index", "Home")&lt;/li&gt;
    &lt;li&gt;@Html.ActionLink("About", "About", "Home")&lt;/li&gt;
...
<img src="./pics/14.14.png" />
<h3>Кнопки</h3>
<p>Для создания кнопок Bootstrap имеет класс <span class="bb">btn</span>. Как правило, кнопки офрмляются в группу с помощью класса <span class="bb">btn-group</span>:</p>
<pre class="brush:xml;">
...
<img src="./pics/14.15.png" />
<h4>Кнопка с выпадающим списком</h4>
<p>Для создания выпадающего списка по примеру элемента нам надо использовать кнопку вместе со списком, который должен иметь класс <span class="bb">dropdown-menu</span>:</p>
<pre class="brush:xml;">
&lt;div class="btn-group"&gt;
    &lt;button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown"&gt;
        Язык программирования &lt;span class="caret"&gt;&lt;/span&gt;
    &lt;/button&gt;
...
<img src="./pics/14.18.png" />
<h3>Метки</h3>
<p>Для оформления кусков текста в качестве меток мы можем использовать класс <span class="bb">label</span>. Также мы можем использовать 
дополнительные классы меток, чтобы конкретизировать тип сообщения:</p>
<pre class="brush:xml;">
...
<img src="./pics/14.16.png" />
<p>Похожую функциональность предлагает класс <span class="bb">alert</span>:</p>
<pre class="brush:xml;">
&lt;div class="alert alert-success"&gt;Задача успешно завершена&lt;/div&gt;
&lt;div class="alert alert-info"&gt;Дополнительная информация&lt;/div&gt;
&lt;div class="alert alert-warning"&gt;Внимание!&lt;/div&gt;
&lt;div class="alert alert-danger"&gt;Опасно!&lt;/div&gt;
</pre>
<img src="./pics/14.17.png" />
<p>Это только некоторые компоненты, которые предлагает Bootstrap. Но уже по ним можно увидеть, что Bootstrap довольно гибок, и даже сложные по структуре компоненты 
может легко адаптировать и приспособить под конкретные устройства.</p>
