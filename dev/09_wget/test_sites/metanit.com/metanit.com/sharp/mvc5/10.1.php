...
<html >
<head>
<title>JavaScript и AJAX в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h1>JavaScript и AJAX</h1>
<h2>Использование JavaScript</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Современные веб-приложения практически невозможно представить без языка клиентской части - JavaScript. Даже при использовании таких серверных 
...
Ему на смену приходят специальные библиотеки, в частности, jQuery. Применительно к ASP.NET MVC при создании веб-приложений библиотеки jQuery играют очень большую роль.</p>
<h3>Подключение файлов JavaScript/jQuery</h3>
<p>По умолчанию проект ASP.NET MVC 5 уже содержит необходимый базовый набор скриптов, в том числе библиотеки jQuery:</p>
<img src="./pics/10.1.png" alt="Библиотеки javascript в ASP.NET MVC 5" />
<p>Большинство скриптов по умолчанию имеют свои двойники с суффиксом <span class="ii">min</span>, например, <span class="ii">jquery-1.10.2.js</span> и <span class="ii">jquery-1.10.2.min.js</span>. 
Оба скрипта представляют одну и ту же функциональность. Но вторая версия представляет минимизированную версию (поэтому и идет с суффиксом <span class="ii">min</span>). 
Подобные минимизированные скрипты гораздо меньше по объему (в среднем на 60%), поэтому в реальным приложениях предпочтительнее 
использовать именно минимизированные скрипты, так как пользователь тратит меньше времени и трафика на их загрузку. В то же время их не 
очень удобно читать. Поэтому для большего удобства разработчиков полные и минимизированные скрипты базовых библиотек идут вместе.</p>
<p>Вкратце посмотрим, зачем нужны большинство скриптов, идущих по умолчанию в проекте MVC 5 с типом аутентификации Individual User Accounts:</p>
<ul>
<li><p><i>jquery-1.10.2.js</i> - базовая библиотека jQuery, на которую опираются большинство других скриптов. В данном случае используется 
	версия 1.10.2. Однако библиотека постоянно обновляется, поэтому можно использовать более новые версии, которые можно добавить вручную или через NuGet.</p></li>
<li><p><i>modernizr-2.6.2.js</i> - библиотека, позволяющая определить, поддерживает ли браузер те или иные возможности HTML5 и CSS3</p></li>
<li><p><i>bootstrap.js</i> - библиотека, позволяющая создавать адаптивные веб-приложения с использованием css-фреймворка bootstrap</p></li>
<li><p><i>respond.js</i> - позволяет использовать правила media queries CSS3 в старых браузерах, которые напрямую не поддерживают данную возможность</p></li>
<li><p><i>jquery.validate.js</i> - представляет функционал для валидации на стороне клиента</p></li>
<li><p><i>jquery.validate.unobtrusive.js</i> - предоставляет поддержку ненавязчивой валидации модели</p></li>
<li><p><i>jquery-1.10.2.intellisense.js</i> и <i>jquery.validate-vsdoc.js</i> - используются для поддержки документации и IntelliSense 
	по соответствующим библиотекам в Visual Studio</p></li>
</ul>
<p>В некоторых типах проектов ряд распространенных скриптов может отсутствовать, например, если делать проект MVC 5 с типом аутентификации No Athentication, то в нем будет отсутствовать 
скрипты валидации. В этом случае мы можем найти недостающий скрипт в NuGet и добавить его в проект:</p>
<img src="./pics/10.2.png" />
<p>Чтобы подключить файл javascript используется метод <code>Render</code> класса <code>System.Web.Optimization.Scripts</code>:</p> 
<p><code>@Scripts.Render("~/scripts/jquery.validate.min.js")</code></p>
<p>Этот метод принимает в качестве параметра строку - полный путь к скрипту.</p>
<p>Также для подключения скриптов мы можем использовать хелпер <code>Url.Content</code>:</p> <p><code>&lt;script src="@Url.Content("~/scripts/jquery.validate.min.js")" type="text/javascript"&gt;&lt;/script&gt;</code></p>
<p>Если файлы javascript будут использоваться большинством представлений приложения, то удобнее всего добавить их сразу на мастер-страницу.</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Если же скрипты используются не на всех, а на отдельных предствлениях, то удобно определить в мастер-странице специальную секцию для скрипов, 
как в примере выше. А затем в представлении подключать скрипты в этой секции:</p>
<pre class="brush:xml;">
@{
    ViewBag.Title = "Index";
}
