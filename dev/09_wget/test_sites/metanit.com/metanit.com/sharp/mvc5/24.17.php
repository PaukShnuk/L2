...
<html >
<head>
<title>ASP.NET MVC 5 | Работа с MySQL</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение к MySQL в ASP.NET MVC 5, работа с базой данных через Entity Framework">
...
<h2>Работа с MySQL</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>MySQL является одной из самых распространенных баз данных, которая может использоваться также и в связке с ASP.NET MVC. Для работы с 
MySQL нам нужен естественно сам MySQL Server, на котором будет находиться база данных. Кроме того, поскольку мы будем работать с C#, 
то на целевом компьютере должен быть установлен также <a href="http://downloads.mysql.com/archives/c-net/">MySQL Connector.NET 6.9.6</a>. Но необязательно 
устанавливать раздельно, поскольку при установке сервера MySQL в последних версиях можно сразу установить и MySQL Connector.</p>
<p>Поскольку мы будем взаимодействовать с базой данных через Entity Framework, то соответственно добавим нужный пакет через NuGet. И также через NuGet 
добавим в проект еще один пакет, который называется <span class="bb">MySql.Data.Entity</span>:</p>
<img src="./pics/mysql1.png" alt="MySql.Data.Entity в ASP.NET MVC 5" />
<p>Вместе с этим пакетом в проект также будет установлен другой пакет - <span class="bb">MySql.Data</span>.</p>
<p>Затем в файле <span class="bb">web.config</span> определим подключение, которое будет использоваться:</p>
<pre class="brush:c#;">
&lt;connectionStrings&gt;
...
<p>В данном случае база данных будет называться mobiledb. При этом важно указать точные данные для параметра UserId и Password - соответственно 
логин и пароль для подключения к базе данных.</p>
<p>Далее добавим простенькую модель и контекст данных:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>При создании контекста данных к нему надо применить атрибут <span class="bb">DbConfigurationType(typeof(MySql.Data.Entity.MySqlEFConfiguration))</span>. 
В остальном особых различий при подключении к mysql по сравнению с MSSQL нет.</p>
<p>Например, пусть у нас есть два действия по добавлению и выводу объектов:</p>
<pre class="brush:c#;">
...
<p>Добавим несколько объектов:</p>
<img src="./pics/mysql2.png" />
<p>И затем с помощью дополнительных утилит, например, MySQL Workbench, можно уидеть созданную азу данных, ее структуру и получить с помощью запроса 
хранящиеся там объекты:</p>
...
<div class="nav"><p><a href="./24.16.php">Назад</a><a href="./">Содержание</a><a href="./24.18.php">Вперед</a></p></div></div>
<div class="socBlock">
