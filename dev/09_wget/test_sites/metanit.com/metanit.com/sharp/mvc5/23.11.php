...
<html >
<head>
<title>ASP.NET Identity в многоуровневой архитектуре в ASP.NET MVC 5. Часть 2</title>
<meta charset="utf-8" />
...
<h2>ASP.NET Identity в многоуровневой архитектуре.  Часть 2</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 11.11.2015</div>
...
<p>Теперь добавим в решение новый проект по типу <span class="b">Class Library</span>, который назовем <span class="b">UserStore.BLL</span>.</p>
<p>Так как наше приложение будет использовать систему ASP.NET Identity, то в этот проект через Nuget пакеты <span class="b">Microsoft.AspNet.Identity.Core</span> и 
<span class="b">Microsoft.AspNet.Identity.EntityFramework</span>.</p>
<p>И также добавим в проект ссылку на проект DAL.</p>
<p>Вначале определим в проекте новую папку, назовем ее <span class="b">DTO</span>, и добавим в нее новый класс <span class="b">UserDTO</span>:</p>
<pre class="brush:c#;">
...
<p>Через данный класс мы будем передавать информацию о пользователях на уровень представления или, наоборот, получать с этого уровня данные. 
Данный класс содержит все основные свойства, соответствующие свойствам моделей ApplicationUser и ClientProfile.</p>
<p>Также добавим в проект папку <span class="b">Infrastructure</span>, которая будет хранить различные вспомогательные классы. 
Определим в ней один класс <span class="b">OperationDetails</span>:</p>
<pre class="brush:c#;">
...
<p>Данный класс будет хранить информацию об успешности операции. Свойство <code>Succedeed</code> указывает, успешна ли операция, а свойства 
<code>Message</code> и <code>Property</code> будут хранить соответственно сообщение об ошибке и свойство, на котормо произошла ошибка.</p>
<p>Далее добавим в проект папку <i>Interfaces</i>. Поместим в нее новый интерфейс IUserService:</p>
<pre class="brush:c#;">
using System;
...
<p>Через объекты данного интерфейса уровень представления будет взаимодействовать с уровнем доступа к данным. Здесь определены только три метода: 
Create (создание пользователей), Authenticate (аутентификация пользователей) и SetInitialData (установка начальных данных в БД - админа и списка ролей).</p>
<p>Далее добавим в папку Interfaces новый интерфейс <span class="b">IServiceCreator</span>:</p>
<pre class="brush:c#;">
namespace UserStore.BLL.Interfaces
{
...
<p>В данном случае для упрощения примера я не буду использовать контейнеры внедрения зависимостей, а вместо этого воспользуюсь абстрактной фабрикой, 
которую будет представлять интерфейс IServiceCreator. Хотя естественнно можно также использовать для внедрения зависимостей DI-контейнеры типа Ninject.</p>
<p>И в конце добавим в проект папку <i>Services</i>. В ней определим класс <span class="b">UserService</span>:</p>
<pre class="brush:c#;">
using UserStore.BLL.DTO;
using UserStore.BLL.Infrastructure;
...
<p>С помощью объекта IUnitOfWork сервис будет взаимодействовать с базой данных.</p>
<p>И определим в этой же папке класс фабрики, которая будет создавать этот сервис:</p>
<pre class="brush:c#;">
using UserStore.BLL.Interfaces;
...
<p>В итоге получился проект со следующей структурой:</p>
<img src="./pics/23.13.png" alt="ASP.NET Identity in Business Logic Layer" />
...
<div class="nav"><p><a href="./23.10.php">Назад</a><a href="./">Содержание</a><a href="./23.12.php">Вперед</a></p></div></div>
<div class="socBlock">
