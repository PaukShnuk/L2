...
<html >
<head>
<title>Настройка валидации пароля в ASP.NET Identity</title>
<meta charset="utf-8" />
...
<h2>Настройка валидации пароля в ASP.NET Identity</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>При использовании системы аутентификации и авторизации ASP.NET Identity мы можем столкнуться со следующей проблемой: при вводе пароля 
срабатывает встроенная логика валидации - длина пароля не может быть меньше 6 символов, и мы эту логику никак не можем изменить, используя 
стандартный механим атрибутов валидации. Но что, если нам надо установить другую минимальную длину пароля? Или что если мы хотим, чтобы в 
пароле могли бы использоваться только цифровые или только алфавитные символы?</p>
<p>И несмотря на то, что встроенная логика валидации скрыта от наших глаз, мы ее можем переопределить. За механизм валидации отвечает свойство 
<span class="b">PasswordValidator</span> класса <span class="b">UserManager</span>. Оно хранит объект валидации, который мы можем изменить.</p>
<p>Если мы перейдем в файл <i>IdentityConfig.cs</i> в папке App_Start, то в определении класса <code>ApplicationUserManager</code> мы 
можем увидеть применение валидатора пароля:</p>
<pre class="brush:c#;">
...
<p>Для определения валидации пароля здесь применяется встроенный класс <span class="bb">PasswordValidator</span>, который имеет следующие свойства:</p>
<ul>
...
<p>Для большинства случаев данного функционала вполне хватит, мы можем настроить минимальную длину пароля, ввод алфавитных, цифровых или неалфавитно-цифровых 
символов. Однако если нам потребуется более сложая логика валидации, то придется определять свой класс валидатора.</p>
<p>Объект валидатора представляет объект интерфейса <code>IIdentityValidator</code>. В данном случае в качестве такого объекта используется 
экземпляр класса <code>PasswordValidator</code>, который устанавливает минимальную длину пароля и ряд других правил. 
Однако больше ничего с этим валидатором мы сделать не можем. Поэтому для создания более изощренной логики валидации нам надо создать отдельный класс.</p>
<p>Итак, создадим следующий класс:</p>
<pre class="brush:c#;">
...
<p>Класс валидатора реализует интерфейс <code>IIdentityValidator</code>, который имеет один метод <code>ValidateAsync</code>. 
Этот метод принимает строковый параметр, на место которого затем в программе будет передаваться строка валидируемого пароля.</p>
<p>Кроме проверки количества символов пароль здесь также проверяется на соответствие регулярному выражению, которое говорит о том, что все символы 
должны представлять цифры (<code>string pattern = "^[0-9]+$"</code>). Таким образом, мы можем определить в методе различные способы проверок.</p>
<p>И затем также в определении класса ApplicationUserManager мы можем его использовать:</p>
<pre class="brush:c#;">
var manager = new ApplicationUserManager
					(new UserStore&lt;ApplicationUser&gt;
						(context.Get&lt;ApplicationDbContext&gt;()));
...
<div class="nav"><p><a href="./12.6.php">Назад</a><a href="./">Содержание</a><a href="./12.13.php">Вперед</a></p></div></div>
<div class="socBlock">
