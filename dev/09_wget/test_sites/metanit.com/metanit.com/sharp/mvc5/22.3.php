...
<html >
<head>
<title>Примеры правил URL Rewriting в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Примеры правил URL Rewriting</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Рассмотрим некоторые примеры правил, которые могут часто использоваться.</p>
<p>Переадресация с домена без www на домен с www:</p>
<pre class="brush:xml;">
...
<p>Например, мы хотим перенаправлять с home на home/index, так как по сути эти два url ведут на один ресурс и тем самым избежать дублей:</p>
<pre class="brush:xml;">
...
<p>Удаление завершающего строку запроса слеша (например, <i>http://localhost:1234/home/index/1/</i>)):</p>
<pre class="brush:xml;">
...
<p>Если у нас изменился адрес, то мы можем использовать переадресацию со старого адреса на новый:</p>
<pre class="brush:xml;">
...
<p>Предотвращение Image Hotlinking - ссылок сторонних сайтов на изображения на нашем сайте:</p>
<pre class="brush:xml;">
...
<p>Если сайт доступен по протоколу https, то при обращении к тем же ресурсам сайта, но по протоколу http, сервер будет отправлять ошибку 403. Чтобы избежать этого 
и просто перенаправить пользователя на тот же запрошенный им ресурс, но только по протоколу https, можно использовать следующее правило:</p>
<pre class="brush:xml;">
...
<p>Перевод строки запроса в нижний регистр:</p>
<pre class="brush:xml;">
...
<p>Блокирование запроса по IP:</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./22.2.php">Назад</a><a href="./">Содержание</a><a href="./23.1.php">Вперед</a></p></div></div>
<div class="socBlock">
