...
<html >
<head>
<title>Аутентификация OWIN и ClaimsIdentity | ASP.NET MVC</title>
<meta charset="utf-8" />
...
<h2>Аутентификация OWIN и ClaimsIdentity</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 16.11.2015</div>
...
<p>В этой статье рассмотрим, как мы можем аутентифицировать пользователя и разграничить доступ по ролям без всяких провайдеров и ASP.NET Identity средствами 
инфраструктуры OWIN. И для этого вначале создадим новый проект без аутентификации.</p>
<p>Прежде всего, нам необходимы модели пользователя и роли, а также контекст данных. Поэтому добавим в папку Models следующие классы:</p>
<pre class="brush:c#;">
public class User
{
...
<p>И также добавим класс контекста данных и инициализатор:</p>
<pre class="brush:c#;">
...
<p>Пусть в файле web.config у нас будет определена строка подключения DefaultConnection, а в файле Global.asax вызывается вышеопределенный инициализотор БД.</p>
<p>Теперь нам надо подключить OWIN  в проект. Для этого добавим через Nuget следующие пакеты:</p>
<pre>
Microsoft.Owin.Host.SystemWeb
Microsoft.Owin.Security.Cookies
</pre>
<p>Они имеют ряд зависимостей, и вместе с ними будут добавлены пакеты OWIN, Microsoft.Owin и Microsoft.Owin.Security.</p>
<p>Затем добавим в проект в папку App_Start класс OWIN, который назовем Startup:</p>
<pre class="brush:c#;">
...
<p>С помощью метода расширения <code>UseCookieAuthentication()</code> здесь устанавливается, что при аутентификации пользователей будут использоваться куки: <code>AuthenticationType = "ApplicationCookie"</code>.</p>
<p>Теперь сделаем функционал для входа на сайт. Вначале добавим в проект папку ViewModels и в ней определим класс LoginViewModel - модель входа на сайт:</p>
<pre class="brush:c#;">
...
<p>Затем добавим в папку Controllers новый контроллер AccountController:</p>
<pre class="brush:c#;">
using System.Linq;
using System.Web;
...
<p>Первым делом с помощью контекста OWIN мы получаем объект <span class="b">IAuthenticationManager</span>. Что это за объект? Данный объект управляет аутентификационными куками и 
выполняет ряд других действий.</p>
<p>В POST-методе Login мы получаем из представления объект LoginModel и по нему определяем пользователя. Если пользователь был найден, то создается объект 
<span class="b">ClaimsIdentity</span>:</p>
<pre class="brush:c#;">ClaimsIdentity claim = new ClaimsIdentity("ApplicationCookie", ClaimsIdentity.DefaultNameClaimType, ClaimsIdentity.DefaultRoleClaimType);</pre>
<p>Первый параметр - "ApplicationCookie" указывате на тип аутентификации, то есть куки. И в конструктор также передаются константы ClaimsIdentity.DefaultNameClaimType и ClaimsIdentity.DefaultRoleClaimType, 
которые представляют название клейма логина пользователя и название клейма роли.</p>
<p>Далее в объект ClaimsIdentity последовательно добавляются клеймы, которые содержат логин пользователя, его id, роль:</p>
<pre class="brush:c#;">
claim.AddClaim(new Claim(ClaimTypes.NameIdentifier, user.Id.ToString(), ClaimValueTypes.String));
claim.AddClaim(new Claim(ClaimsIdentity.DefaultNameClaimType, user.Email, ClaimValueTypes.String));
if (user.Role!=null)
...
<p>И также нам надо добавить специальный клейм провайдера:</p>
<pre class="brush:c#;">
claim.AddClaim(new Claim("http://schemas.microsoft.com/accesscontrolservice/2010/07/claims/identityprovider", 
    "OWIN Provider", ClaimValueTypes.String));
</pre>
<p>Причем второй параметр представляет в данном случае произвольное название провайдера - "OWIN Provider".</p>
<p>По похожей схеме происходит создание аутентификационного тикета в ASP.NET Identity, правда, там провайдер четко определен - "Asp Net Identity".</p>
<p>Далее с помощью методов <code>AuthenticationManager.SignOut()</code> и <code>AuthenticationManager.SignIn()</code> куки удаляются и вновь устанавливаются.</p>
<p>Таким образом, аутентификационный тикет будет содержать данные о логине пользователя, его роли и id. Мы уже по умолчанию  можем получать логин пользователя: <code>User.Identity.Name</code>. 
Но на данный момент в проекте нет никакого способа определять id пользователя и роль. И для этого добавим в папку Models новый класс IdentityExtensions:</p>
<pre class="brush:c#;">
using System;
...
<p>Данный класс создает два метода расширения GetUserId и GetUserRole для получения из аутентификационного тикета id и роли пользователя.</p>
<p>И мы можем, например, использовать разграничение входа и эти методы в имеющемся у нас HomeController:</p>
<pre class="brush:c#;">
using System.Web.Mvc;
...
<div class="nav"><p><a href="./12.19.php">Назад</a><a href="./">Содержание</a><a href="./12.21.php">Вперед</a></p></div></div>
<div class="socBlock">
