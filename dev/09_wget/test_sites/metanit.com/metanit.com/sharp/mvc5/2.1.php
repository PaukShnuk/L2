...
<html >
<head>
<title>Создание первого приложения ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Создание первого приложения на ASP.NET MVC 5 и языке программирования C#, определение моделей">
...
<h1>Создание первого приложения ASP.NET MVC 5</h1><h2>Создание проекта</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Мы посмотрели некоторые основные понятия паттерна MVC, теперь создадим первое приложение. Это будет очень простенькое приложение, 
цель которого - дать некоторое начальное понимание работы с ASP.NET MVC 5.</p>
<p>Это приложение будет эмулировать работу книжного магазина: оно будет предоставлять нам выбор книг, а пользователь, зашедший на сайт, сможет оформить покупку. 
Для начала, я думаю, достаточно.</p>
<p>Итак, откроем Visual Studio 2013 <span class="bb">File -&gt; New Project..</span> и создадим новый проект. 
Назовем новый проект, например, BookStore. Затем в окне создания нового проекта выберем MVC. А в правой части окна изменим тип аутентификации приложения 
на <span class="bb">No Authentication</span> (так как пока нам система аутентификации не нужна):</p>
<img src="./pics/2.2.png" />
<p>После этого будет создан по сути проект, который практически не обладает никакой функциональностью, хотя уже имеет базовую структуру.</p>
<p>Первым делом определим модели данных нашего приложения. Поскольку речь идет о книжном магазине, то такими моделями могут быть 
модель книги и модель покупки книги.</p>
<p>В проекте уже по умолчанию определена папка <span class="bb">Models</span>. 
В ней будут находиться наши модели. Нажмем на эту папку правой кнопкой мыши и в появившемся меню выберем <span class="bb">Add-&gt;Class...</span>. 
Назовем первый новый класс <span class="bb">Book</span> и добавим в него код, описывающий модель книги:</p>
<pre class="brush:c#;">
...
<p>Конечно, в реальном приложении подобная модель могла бы также включать изображение книги, количество экземпляров, год издания и т.д. Но 
для нашей задачи вполне сойдет и этот набор полей.</p>
<p>Подобным образом второй класс - модель <span class="bb">Purchase</span>, которая будет отвечать за отдельную совершаемую покупку книги:</p>
<pre class="brush:c#;">
using System;
namespace BookStore.Models
{
...
<h3>Условности при создании моделей</h3>
<p>Как вы видите, модель представляет обычный класс на языке C#. Все модели здесь имеют набор свойств, описывающие реальные свойства объекта. 
В то же время при создании моделей следует соблюдать некоторые условности. Поскольку мы будем использовать для хранения моделей базу данных SQL Server, 
то для манипуляции над объектами в базе данных нам надо определить для них первичный ключ (Primary Key), который выполняет роль универсального 
идентификатора объекта. Поэтому первым свойством в каждой модели идет свойство Id, предназначенное для хранения первичного ключа.</p> 
<p>И тут вступают в силу условности: свойство идентификатора модели должна иметь имя либо <span class="bb">Имя_моделиId</span>, либо 
просто <span class="bb">Id</span>. Так, у нас в модели Book определено свойство Id, то есть данное свойство является первичным ключом. 
А в случае с моделью Purchase свойство носит название PurchaseId.</p>
<p>Второй способ состоял в определении ключа с помощью атрибута Key, установленным над нужным свойством.</p>
<h3>EntityFramework</h3>
<p>Для работы с данными в ASP.NET MVC рекомендуется использовать фреймворк Entity Framework, хотя его использование необязательно и всецело зависит от 
предпочтений разработчика. Преимущество этого фреймворка состоит в том, что он позволяет абстрагироваться от структуры конкретной базы 
данных и вести все операции с данными через модель.</p>
<p>Сейчас наш проект не содержит библиотек EntityFramework. И чтобы их добавить в проект, воспользуемся пакетным менеджером NuGet. 
Итак, окне Solution Explorer (Обозреватель решений) нажмем правой кнопкой мыши в структуре проекта на узел References и в появившемся меню выберем <span class="bb">Manage NuGet Packages...</span></p>
<p>В окне управления пакетами NuGet в правом верхнем углу введите в поле поиска <span class="bb">EntityFramework</span> и нажмите Enter. 
После этого в среднем столбце будут отображены все найденные пакеты, которые имеют отношение к запросу, а самым первым будет пакет самого фреймворка EntityFramework, 
который нам и надо установить:</p>
<img src="./pics/2.1.png" />
<p>Запустим процесс установки пакета, нажав на кнопку Install.</p>
<h3>Создание контекста данных</h3>
<p>После завершения установки создадим контекст данных. Контекст данных использует EntityFramework для доступа к БД на основе некоторой модели. 
Итак, добавим в папку Models новый класс <span class="bb">BookContext</span>:</p>
<pre class="brush:c#;">
using System;
...
<p>Чтобы создать контекст, нам надо унаследовать новый класс от класса <b>DbContext</b>. 
Свойства наподобие <code>public DbSet&lt;Book&gt; Books { get; set; }</code> помогают получать из БД набор данных определенного типа 
(например, набор объектов Book).</p>
<h3>CodeFirst</h3>
<p>Хотя мы будем использовать базу данных, но создавать явным образом мы ее не будем. За нас все сделает EntityFramework. 
Это так называемый подход <span class="b">Code First</span> - у нас есть модели, и по ним фреймворк будет создавать таблицы в базе данных.</p>
<p>И в заключении работы над модельной частью установим строку подключения. Для этого откроем файл <span class="b">web.config</span>, найдем секцию 
<span class="ii">configSections</span> и сразу <span class="bb">после</span> нее вставим секцию <span class="b">connectionStrings</span>:</p>
<pre class="brush:xml;">
&lt;connectionStrings&gt;
...
<p>В этой секции мы определяем путь к базе данных, которая затем будет создаваться. Выражение <code>|DataDirectory|</code> представляет заместитель, 
который указывает, что база данных будет создаваться в проекте в папке <code>App_Data</code>. Позднее мы подробнее разберем настройки подключения 
к БД.</p>
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://www.youtube.com/watch?v=kfjgNLxJTdw"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 3. Первое приложение. Обработка запросов в ASP.NET MVC 5</span></a></li>
<li><a title="Видео по статье" rel="nofollow" href="https://www.youtube.com/watch?v=GkJ3qBTniPM"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 4. Первое приложение. Модели в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
