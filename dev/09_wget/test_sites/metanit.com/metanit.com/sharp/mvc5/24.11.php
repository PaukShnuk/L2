...
<html >
<head>
<title>ASP.NET MVC 5 | Подгрузка с прокруткой списка в MVC и Web API</title>
<meta charset="utf-8" />
<meta name="description" content="Создание постоянной подгрузки новых элементов при прокрутке списка в ASP.NET MVC 5 и Web API">
...
<h2>Подгрузка с прокруткой списка в ASP.NET MVC и Web API</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>На многих сайтах можно встретить такую функциональность как inifinite scrolling или бесконечный кроллинг, который предполагает постепенную подгрузку контента 
по мере прокрутки пользователем браузера. Стандартный пример: прокрутка тем в вконтакте, когда происходит плавная подгрузка постов. Попробуем реализовать 
подобную возможность.</p>

<p>Определим модель данных, которые будут подгружаться:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Контроллер будет выглядеть следующим образом:</p>
<pre class="brush:c#;">
...
<p>Переменная <code>pageSize</code> указывает, сколько элементов будет на странице. Метод Index получает номер страницы, и по нему отдает нужные данные.</p>
<p>При первом обращении к сайту метод Index будет возвращать представление, в которое будет передаваться результат функции <code>GetItemsPage()</code></p>
<p>Функции <code>GetItemsPage()</code> по сути выполняет пагинацию данных, выбирая ту часть, которая соответствует номеру странице и количеству элементов на странице.</p>
<p>Если же запрос представляет Ajax-запрос, то метод Index возвращает частичное представление _Items, в которое опять де передается результат функции <code>GetItemsPage()</code>.</p>
<p>Создадим частичное представление "_Items.cshtml":</p>
<pre class="brush:xml;">
...
<p>Оно выводит список переданных в него объектов.</p>
<p>И создадим главное представление <i>Index.cshtml</i>:</p>
<pre class="brush:xml;">
@{
    Layout = null;
}
...
<p>Для загрузки данных в представлении определен блок с id="scrolList". Он по умолчанию содержит вызвов частичного представления _Items.cshtml: <code>@Html.Partial("_Items")</code>, 
благодаря чему уже при первом обращении у нас будет некоторое начальное количество элементов.</p>
<p>И также здесь используется специальный блок с id="loading" для отображения индикатора загрузки с помощью gif-анимации:</p>
<img src="./pics/loading_spinner.gif" alt="gif-анимация" />
<P>Всю основную работу делает javascript. Чтобы отследить прокрутку, применяем обработчик <code>$(window).scroll()</code>, который вызывает функцию 
<code>loadItems()</code></P>
<p>В функции <code>loadItems()</code> с помощью ajax-запроса происходит подгрузка дополнительных данных.</p>
<h3>Подгрузка данных в Web Api</h3>
<p>В Web Api все будет аналогично, за исключением отправки данных. Допустим, у нас есть следующий контроллер Web API:</p>
<pre class="brush:c#;">
...
<p>И загрузка данных на html-странице:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<P>В представлении все то е самое, только данные получаем не в виде частичного представления, а в виде объектов javascript.</P>
...
<div class="nav"><p><a href="./24.10.php">Назад</a><a href="./">Содержание</a><a href="./24.12.php">Вперед</a></p></div></div>
<div class="socBlock">
