...
<html >
<head>
<title>Bootstrap в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h1>Bootstrap в ASP.NET MVC 5</h1>
<h2>Введение в Bootstrap</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Bootstrap представляет собой css-фреймворк для создания адаптивных веб-приложений. Проект Bootstrap изначально был создан и развивался 
разработчиками из Twitter для собственных нужд, однако затем вышел за границы твиттера и в настоящее время развивается как open source-проект и является 
одним из самых популярных фреймворков для создания веб-приложений. Что в принципе и неудивительно: в настоящее время все растет количество пользователей 
мобильного интернета, все больше людей для веб-серфинга используют смартфоны, планшеты. Что в итоге привело к увеличению числа сайтов с адаптивным дизайном и 
соответственно к увеличению популярности средств для разработки подобных сайтов, в том числе фреймворка Bootstrap.</p>
<p>Проекты ASP.NET MVC 5 по умолчанию уже содержат все необходимые файлы Bootstrap:</p>
<img src="./pics/14.1.png" />
<p>Функциональность Bootstrap заключена в файле стилей <i>bootstrap.css</i> и скрипте <i>bootstrap.js</i>. И если мы откроем мастер-страницу _Layout.cshtml, 
которая идет по умолчанию в проекте, то мы увидим подключение соответствующих библиотек:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Выражение <code>@Styles.Render("~/Content/css")</code> подключает необходимые стили, в том числе и bootstrap, а выражение <code>@Scripts.Render("~/bundles/bootstrap")</code> 
подключает скрипт bootstrap. Мы можем это увидеть, открыв файле BundleConfig.cs:</p>
<pre class="brush:c#;">
...
<p>Также на мастер-странице мы можем увидеть использование таких классов css, как navbar, navbar-inverse, navbar-fixed-top и т.д. - все это классы bootstrap, 
которые позволяют адаптировать интерфейс страницы к различным устройствам. Например, запустим страницу в веб-браузере на ПК:</p>
<img src="./pics/14.2.png" />
<p>И если мы также запустим данное приложение на мобильном телефоне или эмуляторе, то мы увидим, что интерфейс по прежнему выглядит довольно неплохо:</p>
<img src="./pics/14.3.png" />
<p>Собственно в этом и преимущество использования Bootstrap. Основой для адаптивного дизайна стали правила media queries, которые описывают 
стилевые свойства элементов для различных разрешений экрана. С точки зрения разработчиков Bootstrap все устройства можно поделить на следующие группы:</p>
<ul>
<li><p>Очень маленькие, ширина экрана которых меньше 768 пикселей</p></li>
<li><p>Небольшие - их ширина от 768 пикселей и выше</p></li>
<li><p>Средние с шириной от 992 пикселя и выше</p></li>
<li><p>И большие с шириной от 1200 пикселей и выше</p></li>
</ul>
<p>Это деление мы можем увидеть, если откроем файл bootstrap.css и найдем там правила media, например:</p>
<pre class="brush:css;">
@media (min-width: 768px) {
  ....................
}
</pre>
<p>А теперь рассмотрим некоторые классы данного фреймворка.</p>
...
<div class="nav"><p><a href="./13.2.php">Назад</a><a href="./">Содержание</a><a href="./14.2.php">Вперед</a></p></div></div>
<div class="socBlock">
