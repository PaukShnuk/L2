...
<html >
<head>
<title>Методы действий и их параметры в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Методы действий и их параметры в ASP.NET MVC 5">
...
<h2>Методы действий и их параметры</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p><span class="b">Методы действий (action methods)</span> представляют такие методы контроллера, которые обрабатывают запросы 
по определенному URL. Например, возьмем проект из предыдущей главы. В нем был определен следующий контроллер:</p>
<pre class="brush:c#;">
...
<p>Здесь методы Index и Buy являются методами действий или просто действиями контроллера. При получении запроса типа 
<span class="ii">/Home/Index</span> контроллер передает обработку запроса действию Index.</p>
<p>Так как запросы бывают разных типов, например, GET и POST, фреймворк ASP.NET MVC позволяет определить тип обрабатываемого запроса для действия, применив к нему соответствующий атрибут: 
<code>[HttpGet], [HttpPost], [HttpDelete]</code> или <code>[HttpPut]</code>. Так, действие Buy разбито на два метода, по одному для каждого типа запроса.</p>
<p>Однако не все методы контроллера являются методами действий. Методы действий всегда имеют модификатор <span class="bb">public</span>. Закрытых приватных 
методов действий не бывает. Но контроллер может также включать и обычные методы, которые могут использоваться в вспомогательных целях. Например,</p>
<pre class="brush:c#;">
[HttpPost]
public string Buy(Purchase purchase)
{
...
<p>Соответственно мы не можем отправить из браузера запрос <i>Home/getToday/</i>, потому что метод getToday не является методом действия.</p>
<h3>Передача данных в контроллеры и параметры</h3>
<p>В приложении из предыдущей главы метод Buy использовал параметр purchase. Так как данный метод обрабатывает POST-запросы, то мы можем отправить ему следующую форму:</p>
<pre class="brush:xml;">
...
<p>Значение атрибута name у всех полей на этой форме соответствует названию свойства модели, поэтому система автоматически свяжет значения полей 
с соответствующими свойствами. А в методе Buy весь этот набор свойств превратится в модель Purchase.</p>
<p>Кроме POST-запросов у нас есть также GET-запросы, при которых все параметры передаются в строке запроса. Например, вторая версия метода Buy в качестве 
параметра принимает значение типа int: <code>public ActionResult Buy(int id)</code>. Стандартный get-запрос принимает примерно следующую форму: 
<i>название_ресурса?параметр1=значение1&amp;параметр2=значение2</i>. То есть запрос к данному методу мог бы выглядеть так: 
<i>Home/Buy?id=2</i>. Название параметров метода должно совпадать с названием параметров в строке запроса. Благодаря этому система сможет их 
автоматически связать. А в самом методе мы сможем получить этот параметр и использовать его по своему усмотрению.</p>
<p>Кроме того, система маршрутизации позволяет создавать маршруты. Например, по умолчанию в проекте MVC определяется следующий маршрут: 
<i>Контроллер/Метод/id</i>. Последний параметр является опциональным. И благодаря этому мы можем передать параметр id и так: <i>Home/Buy/2</i></p>
<p>Для примера определим действие, которое будет подсчитывать площадь треугольника:</p>
<pre class="brush:c#;">
...
<p>В этом случае мы можем обратиться к действию, набрав в адресной строке <i>Home/Square?a=10&amp;h=3</i>, и приложение выдало бы нам нужный результат.</p>
<p>Мы также можем задать для параметров значения по умолчанию:</p>
<pre class="brush:c#;">
...
<p>В этом случае при запросе страницы мы можем указать только один параметр или вообще не указывать(<span class="ii">Home/Square?h=5</span>).</p>
<h4>Получение данных из контекста запроса</h4>
<p>Кроме того, мы можем получить параметры, да и не только параметры, но и другие данные, связанные с запросом, из объектов контекста запроса. 
Нам доступны следующие объекты контекста: <span class="bb">Request</span>, <span class="bb">Response</span>, <strong>RoutedData</strong>, <strong>HttpContext</strong> и <span class="bb">Server</span>.</p>
<p>Объект Request содержит коллекцию Params, которая хранит все параметры, переданные в запросы. И мы их можем получить:</p>
<pre class="brush:c#;">
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/F7rhqyphToQ"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 9. Методы контроллеров и их параметры в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
