...
<html >
<head>
<title>Фильтры в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Фильтры и их виды в ASP.NET MVC 5, применение фильтров">
...
<h1>Фильтры</h1>
<h2>Фильтры в ASP.NET MVC 5</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Иногда возникает необходимость внести в приложение некоторую логику, которая должна отрабатывать до вызова действий контроллера. В частности, 
например, пользователь вызывает определенное действие некоторого контроллера, и нам необходимо проверять, авторизовался ли данный пользователь, и 
потом уже выполнять определенные действия. Для этого и применяются фильтры.</p>
<p>Фильтры реализованы как атрибуты, благодаря чему позволяют уменьшить объем кода в контроллере. Данные атрибуты могут применяться как ко всему классу, 
так и к отдельным его методам, свойствам и полям. Например, если у нас есть некоторый контроллер HomeController, и мы хотим, чтобы к нему имели доступ только авторизованные пользователи. 
В этом случае мы можем использовать следующий атрибут:</p>
<pre class="brush:c#;">
[Authorize]
...
<p>Атрибут <span class="bb">[Authorize]</span> в данном случае как раз и будет задействовать фильтр авторизации. Если бы мы не применили 
данный фильтр, то нам бы пришлось вручную писать логику проверки, авторизован ли пользователь или нет.</p>
<h3>Применение фильтров</h3>
<p>Как вы видели из примера выше, для применения фильтра, надо атрибут поставить над классом контроллера - в этом случае данный фильтр будет применяться ко 
всем методам данного контроллера.</p>
<p>Но, как уже выше говорилось, мы можем применить фильтр на уровне отдельного метода:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{   
    [Authorize]
...
<h3>Базовые типы фильтров</h3>
<p>В MVC 5 имеется несколько типов фильтров:</p>
<table class="tab">
<tr class="tabhead"><td><p>Тип фильтров</p></td><td><p>Реализуемый интерфейс</p></td><td><p>Стандартная реализация</p></td><td><p>Описание</p></td></tr>
<tr><td><p>Фильтры аутентификации</p></td><td><p>IAuthenticationFilter</p></td><td><p>Отсутствует</p></td><td><p>Фильтр, определяющий, 
аутентифицирован ли клиент. Данный фильтр запускается до выполнения любого другого фильтра или метода действий</p></td></tr>
<tr><td><p>Фильтры авторизации</p></td><td><p>IAuthorizationFilter</p></td><td><p>AuthorizeAttribute</p></td><td><p>Фильтр, определяющий, имеет ли пользователь доступ к данному ресурсу. 
Данный фильтр запускается после фильтра аутентификации, но до любого другого фильтра или метода действия</p></td></tr>
<tr><td><p>Фильтры действий</p></td><td><p>IActionFilter</p></td><td><p>ActionFilterAttribute</p></td><td><p>Фильтр, применяемый к действиям. Может запускаеться как до, так и после выполнения метода действий</p></td></tr>
<tr><td><p>Фильтры результатов действий</p></td><td><p>IResultFilter</p></td><td><p>ActionFilterAttribute</p></td><td><p>Фильтр, применяемый к результатам действий. Может запускаться как до, так и после выполнения результата действия</p></td></tr>
<tr><td><p>Фильтры исключений</p></td><td><p>IExceptionFilter</p></td><td><p>HandleErrorAttribute</p></td><td><p>Атрибут для обработки исключений, выбрасываемых 
методом действий и результатом действий</p></td></tr>
</table>
<p>При вызове метода действий фреймворк MVC сначала смотрит, применяются ли к данному действию какие-либо атрибуты, и если применяются, то в начале отрабатывают данные атрибуты.</p>
<p>Теперь разберем первый фильтр - фильтр аутентификации.</p>
...
<div class="nav"><p><a href="./7.6.php">Назад</a><a href="./">Содержание</a><a href="./8.2.php">Вперед</a></p></div></div>
<div class="socBlock">
