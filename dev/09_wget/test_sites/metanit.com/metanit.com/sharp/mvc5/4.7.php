...
<html >
<head>
<title>Строго типизированные хелперы в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Строго типизированные хелперы в ASP.NET MVC 5, их применение, привязка к модели">
...
<h2>Строго типизированные хелперы</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Кроме базовых хелперов в ASP.NET MVC имеются их двойники - строго типизированные хелперы. Этот вид хелперов принимает в качестве параметра 
лямбда-выражение, в котором указывается то свойство модели, к которому должен быть привязан данный хелпер. Важно учитывать, что строго типизированные 
хелперы могут использоваться только в строго типизированных представлениях, а тип модели, которая передается в хелпер, должен 
быть тем же самым, что указан для всего представления с помощью директивы @model.</p>
<p>Посмотрим на примере. Во второй главе мы использовали модель <code>Purchase</code> для оформления покупки книги:</p>
<pre class="brush:c#;">
...
<p>И для ее использования применялась следующая форма:</p>
<pre class="brush:xml;">
...
<p>Перепишем этот пример с использованием хелперов:</p>
<pre class="brush:xml;">
...
<img src="./pics/4.7.png" />
<p>Строго типизированный хелпер похож на обычный, только в конце прибавляется суффикс <span class="bb">For</span>: <code>LabelFor</code>. 
...
модель, которая будет использоваться: <code>@model BookStore.Models.Purchase</code>. То есть, в вызове 
<code>@Html.TextBoxFor(m=&gt;m.Person)</code> параметр <code>m</code> представляет переменную модели Purchase. А лямбда-выражение <code>m=&gt;m.Person</code> 
указывает, что данный хелпер будет генерировать текстовое поле для свойства <code>Person</code>. 
Таким образом, хелпер <code>@Html.TextBoxFor(m=&gt;m.Person)</code> сгенерирует текстовое поле <code>&lt;input id="Person" name="Person" type="text" value="" /&gt;</code>.</p>
<p>Для каждого базового встроенного хелпера имеется свой строго типизированный хелпер:</p>
<ul>
<li><p><span class="bb">Html.CheckBoxFor</span></p>
<p>Выражение <code>@Html.CheckBoxFor(m=&gt;m.Enable, false)</code> cоздает разметку:</p>
<pre class="brush:xml;">
&lt;input id="Enable" name="Enable" type="checkbox" value="true" /&gt;
&lt;input name="Enable" type="hidden" value="false" /&gt;
</pre></li>
<li><p><span class="bb">Html.HiddenFor</span></p>
<p>Выражение <code>@Html.HiddenFor(m=&gt; m.Name)</code> создает разметку:</p> 
<p><code>&lt;input id="Name" name="Name" type="hidden" value="[значение_m.Name]" /&gt;</code></p></li>
<li><p><span class="bb">Html.LabelFor</span></p>
<p>Хелпер <code>@Html.LabelFor(m =&gt; m.Name,"Имя")</code> генерирует разметку:</p> 
<p><code>&lt;label for="Name"&gt;Имя&lt;/label&gt;</code></p></li>
<li><p><span class="bb">Html.PasswordFor</span></p>
<p>Хелпер <code>@Html.PasswordFor(m =&gt; m.Password)</code> оздает разметку:</p> 
<p<<code>&lt;input id="Password" name="Password" type="password" /&gt;</code></p></li>
<li><p><span class="bb">Html.RadioButtonFor</span></p>
...
<li><p><span class="bb">Html.TextBoxFor</span></p>Выражение <p><code>@Html.TextBoxFor(m =&gt; m.Name)</code> создает разметку:</p> 
<p><code>&lt;input id="Name" name="Name" type="text" /&gt;</code></p></li>
<tr><td><p><span class="bb">Html.TextAreaFor</span></p>
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/S76Z2jnRiOo"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 22. Строго типизированные хелперы в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
