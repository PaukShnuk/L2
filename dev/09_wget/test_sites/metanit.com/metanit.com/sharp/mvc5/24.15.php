...
<html >
<head>
<title>ASP.NET MVC 5 | Сжатие изображений с помощью ImageResizer</title>
<meta charset="utf-8" />
<meta name="description" content="Сжатие изображений с помощью ImageResizer в ASP.NET MVC 5, создание различных версий изображений">
...
<h2>Сжатие изображений с помощью ImageResizer</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Для сжатия изображений при загрузке на сервере в приложении ASP.NET MVC можно использовать стандартные классы из пространства 
имен System.Drawing, например, Image и Bitmap, и вручную проводить все масштабирование. Но можно также воспользоваться готовыми решениями. В частности библиотекой 
<span class="bb">ImageResizer</span>.</p> 
<p>Чтобы добавить ее функциональность в проект, необходимо через NuGet добавить последовательно ряд пакетов:</p>
<pre>
ImageResizer.WebConfig
ImageResizer.Plugins.DiskCache
ImageResizer.Plugins.PrettyGifs
</pre>
<img src="./pics/imageresizer.png" alt="Библиотека ImageResizer в ASP.NET MVC 5" />
<p>После добавления пакетов в файл конфигурации web.config добавляется определение модуля для работы с изображениями:</p>
<pre class="brush:xml;">
&lt;modules&gt;
    &lt;add name="ImageResizingModule" type="ImageResizer.InterceptModule" /&gt;
&lt;/modules&gt;
</pre>
<p>Тепер применим библиотеку при загрузке файла:</p>
<pre class="brush:c#;">
using ImageResizer;
//...........................
...
<p>Для создания нового изображения применяется метод <code>ImageBuilder.Current.Build()</code>, который в качестве параметра принимает 
объект <span class="bb">ImageJob</span>. Конструктор данного объекта принимает ряд перегруженных версий.</p>
<p>В данном случае первый параметр указывает на поток, который надо сохранить.</p>
<p>Второй параметр определяет полный путь к сохраняемому файлу, включая его имя.</p>
<p>Третий параметр задает инструкции сохранения. В чаcтности, инструкция "maxwidth=600&amp;maxheight=600" задает максимальные размеры файла. 
И если размеры загружаемого файлы выходят за эти границы, то изображение должным образом масштабируется. В данном случае 
в качестве максимально допустимых размеров задаются ширина в 600 пикселей и высота также в 600 пикселей.</p>
<p>Четвертый булевый параметр задает, будет ли вызываться после использования объекта, представляющего изображение, метод Dispose (
если иточник изображения представляет объект IDisposable).</p>
<p>Пятый параметр задает добавление формата. Например, мы могли бы использовать следующую инструкцию: 
<code>"maxwidth=600&amp;maxheight=600&amp;format=jpg"</code>. И если данный параметр имел бы значение <code>true</code>, 
...
использовало бы формат jpg вне зависимости от того, какой формат у него был ранее.</p>
<p>Если необходимо создавать по несколько версий для каждого изображения: малый, средний и большой форматы (например, при показе эскизов 
отображать только изображения малого формата, а при загрузке изображения отдавать файл в большом формате), то в этом случае мы 
моем воспользоваться набором инструкций:</p>
<pre class="brush:c#;">
[HttpPost]
public ActionResult Upload(HttpPostedFileBase upload)
{
...
<p>В этом случае в папке для загруженных файлов появятся по три копии для каждого файла с именами: 
<i>filename_large.ext</i>, <i>filename_medium.ext</i> и <i>filename_small.ext</i></p>
...
<div class="nav"><p><a href="./24.14.php">Назад</a><a href="./">Содержание</a><a href="./24.16.php">Вперед</a></p></div></div>
<div class="socBlock">
