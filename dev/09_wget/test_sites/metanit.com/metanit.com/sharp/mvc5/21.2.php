...
<html >
<head>
<title>IoC-контейнер Ninject в ASP.NET MVC</title>
<meta charset="utf-8" />
<meta name="description" content="IoC контейнер Ninject и внедрение зависимостей (Dependency Injection) в ASP.NET MVC 5">
...
<h2>IoC-контейнер Ninject</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 28.10.2017</div>
...
<p>Итак, мы объявили зависимость контроллера от объекта IRepository. Теперь необходимо внедрить зависимость в контроллер.</p>
<p>Для внедрения зависимостей, как правило, используются IoC-контейнеры. Существует множество IoC-контейнеров:  Castle Windsor, Spring.Net, Autofac, Ninject, Unity. 
Используем Ninject.</p>
<p>Для этого вначале все необходимые библиотеки в проект. Установить необходимый пакет можно через NuGet:</p>
<img src="./pics/ninject.png" alt="Ninject in ASP.NET MVC 5" />
<p>Нужный нам пакет: <span class="bb">Ninject.MVC5</span>,  вместе с которым устанавливается ряд зависимых пакетов.</p>
<p>После установки пакета мы уже можем использовать Ninject в проекте. Например, изменим конструктор контроллера следующим образом:</p>
<pre class="brush:c#;">
using Ninject;
//.....................
...
<p>Чтобы управлять зависимостями через Ninject, вначале надо создать объект <code>Ninject.IKernel</code> с помощью встроенной реализации этого интерфейса - 
класса StandardKernel:</p>
<pre class="brush:c#;">
IKernel ninjectKernel = new StandardKernel();
</pre>
<p>Далее нужно установить отношения между интерфейсами и их реализациями:</p>
<pre class="brush:c#;">
ninjectKernel.Bind&lt;IRepository&gt;().To&lt;BookRepository&gt;();
</pre>
<p>Данное выражение указывает, что объекты IRepository должны будут рассматриваться как BookRepository.</p>
<p>И в конце создается объект интерфейса через метод Get:</p>
<pre class="brush:c#;">
repo = ninjectKernel.Get&lt;IRepository&gt;();
</pre>
<p>Поскольку выше мы установили сопоставление между IRepository и BookRepository, то метод <code>ninjectKernel.Get&lt;IRepository&gt;()</code> будет создавать экземпляр класса 
BookRepository.</p>
<h3>Глобальная регистрация завимостей</h3>
<p>Зарегистрируем зависимости глобально для всех контроллеров и для этого создадим в проекте новую папку Util и поместим в нее новый 
класс NinjectRegistrations:</p>
<pre class="brush:c#;">
using IoSApp.Models;
...
<p>Класс NinjectRegistrations наследуется от класса NinjectModule и фактически представляет модуль Ninject.  Он переопределяет метод 
<code>Load()</code>, который вызывается при загрузке модуля. И с помощью вызова <code>Bind&lt;IRepository&gt;().To&lt;BookRepository&gt;();</code> 
собственно устанавливается сопоставление между интерфейсом-зависимостью  и конкретным классом этого интерфейса.</p>
<p>И в конце необходимо инициализировать сопоставление зависимостей происходило при запуске приложения. Для этого перейдем к файлу 
<span class="b">Global.asax.cs</span>, который запускается при старте приложения. Изменим его содержимое 
следующим образом:</p>
<pre class="brush:c#;">
using Ninject;
using Ninject.Modules;
...
<p>Этот класс вызывается при запуске приложения. В конце метода создается модуль Ninject - объект NinjectRegistrations. 
И затем с помощью метода <code>DependencyResolver.SetResolver()</code> регистрируется ранее созданный сопоставитель зависимостей.</p>
<p>И теперь изменим контроллер:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
    IRepository repo;
...
<p>Несмотря на то, что в конструктор контроллера не передается никакой конкретной реализации, но все будет работать. Рассмотрим поэтапно, как происходит 
в данном случае внедрение зависимостей:</p>
<ol>
<li><p>Фреймворк MVC получает запрос и обращается к контроллеру HomeController</p></li>
<li><p>Фреймворк MVC обращается к классу сопоставления зависимостей (в данном случае класс NinjectDependencyResolver), 
чтобы тот создал новый объект HomeController, передавая параметр Type в метод GetService (в класс NinjectDependencyResolver)</p></li>
<li><p>Сопоставитель зависимостей вызывает инфраструктуру Ninject для создания нового объекта HomeController, передавая тип создаваемого объекта в метод TryGet</p></li>
<li><p>Ninject смотрит на конструктор HomeController и видит, что там используется зависимость от интерфейса IRepository, 
для которого он устанавливает сопоставление с конкретной реализацией</p></li>
<li><p>Ninject создает экземпляр класса BookRepository и затем использует его для создания контроллера HomeController</p></li>
<li><p>Ninject передает созданный объект HomeController сопоставителю зависимостей, который, в свою очередь, передает его фреймворку MVC. И далее происходит обработка запроса.</p></li>
</ol>
<p>Таким образом, мы избегаем использования конкретных реализаций и работаем с объектами на уровне интерфейсов. А сопоставление интерфейсов с конкретными реализациями перекладывается на класс NinjectDependencyResolver.</p>
...
<div class="nav"><p><a href="./21.1.php">Назад</a><a href="./">Содержание</a><a href="./21.3.php">Вперед</a></p></div></div>
<div class="socBlock">
