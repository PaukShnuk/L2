...
<html >
<head>
<title>Явная привязка модели в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Явная привязка модели</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>При использовании параметра в методе действия привязка модели работает неявно. Но мы можем вызвать на контроллере и явную привязку модели 
с помощью методов <strong>UpdateModel</strong> и <strong>TryUpdateModel</strong>. 
Если модель не прошла валидацию, то метод <code>UpdateModel</code> выбрасывает исключение. Ниже показан пример использования метода 
<code>UpdateModel</code> в действии <code>Edit</code> вместо применения параметра:</p>
<pre class="brush:c#;">
[HttpPost]
...
<p><code>TryUpdateModel</code> также вызывает привязку модели, но не выбрасывает исключение. Этот метод возвращает значение типа <code>bool</code> - 
если это значение равно <code>true</code>, модель прошла привязку, если <code>false</code>, то валидация прошла неудачно.</p>
<pre class="brush:c#;">
[HttpPost]
public ActionResult Edit()
{
