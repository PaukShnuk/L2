...
<html >
<head>
<title>ASP.NET MVC 5 | Динамическая локализация</title>
<meta charset="utf-8" />
<meta name="description" content="Локализация дат в ASP.NET MVC 5 в зависимости от выбранного пользователем языка, динамическое изменение настроек локализации">
...
<h2>Динамическая локализация</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>В статьях <a href="//metanit.com/sharp/mvc5/24.7.php">Локализация чисел decimal</a> и 
<a href="//metanit.com/sharp/mvc5/17.10.php">Локализация дат в ASP.NET MVC</a> была рассмотрена статическая локализация, когда приложение делается 
под определенную культуру со всеми ее особенностями (запятая или точка в качестве разделителя, определенный формат даты). Однако если у нас веб-приложение 
расчитано на несколько языков или культур, такая ситуация может оказаться нежелательной. Например, пользователю из США будет привычнее использовать в качестве разделителя 
точку и формат даты месяц/день/год, а пользователь из Германии, наоборот, использует формат день/месяц/год и запятую в качестве разделителя. 
Поэтому рассмотрим, как менять динамически настройки локализации в зависимости от выбранной пользователем культуры.</p>
<p>Сначала создадим в проекте инфраструктуру для переключения языка. Подробно данный момент описан в статье <a href="//metanit.com/sharp/mvc/16.2.php">Логика мультиязычного сайта. Фильтр локализации</a>. 
Для этого определим добавим в файл <span class="b">Global.asax</span>, обработчик события <span class="bb">BeginRequest</span>, который будет переключать текущую культуру приложения:</p>
<pre class="brush:c#;">
using System;
...
<p>Обработчик Application_BeginRequest вызывается в начале жизненного цикла запроса. В нем который получаем установленную культуру из куков. 
Затем мы смотрим, есть ли такая культура в списке. Если культуры нет, то используем рускоязычную культуру по умолчанию.</p>
<p>Далее в контроллере создадим метод, который будет сохранять выбранную пользователем культуру в куки</p>
<pre class="brush:c#;">
...
<p>В файле _Layout.cshtml определим переключение языка:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>С помощью выпадающего списка переключаем культуру. В данном случае опустим локализацию названий полей с помощью ресурсов.</p>
<p>Теперь добавим в проект модель User:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Также добавим в контроллер действие Create, которое будет получать введенные пользователем данные:</p>
<pre class="brush:c#;">
...
<p>В post-методе будем получать модель и в случае успешной валидации выводить введенную дату рождения.</p>
<p>Теперь самое главное - определим представление для создания модели, причем нам надо учесть возможность выбора одного из двух форматов дат в 
соответствии с заданной культурой. Фактически нам надо два набора настроек для виджета datepicker, который позволит установить дату: для русскоязычной и для англоязычной культры. 
Для упрощения работы вынесем настройки в отдельный файлы javascript.</p>
<p>Добавим в папку Scripts файл <i>datepicker.en.js</i> со следующим содержимым:</p>
<pre class="brush:js;">
$(function () {
	$("input[type='date']")
...
<p>Для англоязычной культуры используется формат 'm/d/yy'.</p>
<p>Также добавим в папку Scripts файл <i>datepicker.ru.js</i>:</p>
<pre class="brush:js;">
$(function () {
    $("input[type='date']")
...
<p>Здесь уже другой формат "dd/mm/yy" плюс куча дополнительный настроек.</p>
<p>И определим представление Create.cshtml:</p>
<pre class="brush:xml;">
...
<p>С помощью свойства <span class="bb">System.Threading.Thread.CurrentThread.CurrentCulture</span> мы можем получить текущую культуру и 
в зависимости от ее значения подключить тот или иной скрипт.</p>
<p>Выберем русскоязычную культуру и введем дату:</p>
<img src="./pics/local1.png" />
<p>При получении данных модель будет успешно валидирована в контроллере, и браузер выведет дату в соответствии с культурой:</p>
<img src="./pics/local2.png" />
<p>Теперь изменим культуру на англоязычную и соответветственно используем другой формат даты:</p>
<img src="./pics/local5.png" />
<p>И также валидация модели пройдет успешно :</p>
<img src="./pics/local4.png" />
...
<div class="nav"><p><a href="./24.18.php">Назад</a><a href="./">Содержание</a><a href="./25.1.php">Вперед</a></p></div></div>
<div class="socBlock">
