...
<html >
<head>
<title>Создание чата на AJAX и jQuery. Часть 2</title>
<meta charset="utf-8" />
...
<h2>Создание чата на AJAX и jQuery. Часть 2</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В прошлый раз мы определили контроллер и модель, а теперь создадим всю остальную часть чата. Вначале определим представления.</p>
<p>Код главного представления <i>Index.cshtml</i> будет выглядеть следующим образом:</p>
<pre class="brush:xml;">
...
<p>Это представление без мастер-страницы. Элемент <code>div</code> с id="Username" будет отображать ник пользователя. 
Блок с <code>id="LastRefresh</code> предназначен для отображения времени последнего обновления чата, а <code>div</code> c id="Error" - для 
вывода ошибок, которые могут возникнуть.</p>
<p>И для собственно логина у нас есть текстовое поле и кнопка. Однако при вводе ника и нажатии на кнопку данные сразу не будут передаваться на сервер, 
так как это не форма. Реальная отправка данных на сервер будет происходить с помощью ajax-ссылки, которая будет иметь id = "LoginButton":</p>
<pre class="brush:c#;">
@Ajax.ActionLink("Login", "Index", new { user = "" }, 
        new AjaxOptions { UpdateTargetId = "container", OnFailure = "LoginOnFailure", OnSuccess = "LoginOnSuccess" }, 
        new { @id = "LoginButton", @style = "visibility:hidden;" })
</pre>
<p>При нажатии на ссылку будет происходить обновление блока с <code>id="container"</code>. За счет этого будет происходить частичное обновление текущей страницы 
...
стиль <code>"visibility:hidden;"</code></p> 
<p>Нажатие будет происходить автоматически в поключаемом в представлении файле <i>chat.js</i>:</p>
<pre class="brush:js;">
...
<p>Чтобы отправить данные по логину и нажать на ссылку, в обработчике нажатия мы прописываем следующие строки:</p>
<pre class="brush:js;">
// формируем ссылку с параметрами, по которой идет обращение
var href = "/Home?user=" + encodeURIComponent(nickName);
href = href + "&amp;logOn=true";
...
<p>Поскольку в запросе к серверу передается параметр logOn=true, то метод Index при обработке этого запроса будет отправлять клиенту частичное представление 
<i>ChatRoom</i>: <code>return PartialView("ChatRoom", chatModel);</code>. Создадим это представление:</p>
<pre class="brush:xml;">
...
<p>Это частичное представление, типизированное моделью ChatModel, которая передается из метода Index. Оно содержит поля для ввода и отправки 
сообщений, а также кнопку выхода из чата.</p>
<p>Кроме того, здесь есть блок <code>div id="RefreshArea"</code>, предназначенный непосредственно для вывода сообщений и пользователей. 
Для рендеринга этой информации в нем идет обращение к частичному представлению <i>History</i>, которое мы сейчас создадим. И также тут есть невидимая ajax-ссылка, 
обновляющая блок <code>div id="RefreshArea"</code>:</p>
<pre class="brush:c#;">
@Ajax.ActionLink("ActionLink", "Index", new { user = "", logOn = "", logOff = "", chatMessage = "" }, 
new AjaxOptions { UpdateTargetId = "RefreshArea", OnSuccess = "ChatOnSuccess", OnFailure = "ChatOnFailure" }, 
new { @id = "ActionLink", @style = "visibility:hidden;" })
</pre>
<p>В вышерассмотренном файле javacript при удачном логине вызывается функция <code>LoginOnSuccess</code>, которая в свою очередь вызывает 
функцию <code>Refresh()</code>:</p>
<pre class="brush:js;">
function Refresh() {
    var href = "/Home?user=" + encodeURIComponent($("#Username").text());
...
<p>Эта функция опять же формирует строку запроса с параметрами для ajax-ссылки и производит автоклик на нее. В результате блок <code>div id="RefreshArea"</code> 
будет обновляться новыми данными каждые пять секунд.</p>
<p>Подобное действие будет происходить также и при нажатии на кнопку отправки сообщения в чат:</p>
<pre class="brush:js;">
var href = "/Home?user=" + encodeURIComponent($("#Username").text());
href = href + "&chatMessage=" + encodeURIComponent(text);
$("#ActionLink").attr("href", href).click();
</pre>
<p>И поскольку в обоих случаях при передаче параметров в метод Index контроллера параметры logOn и logOff будут не установлены, то метод 
Index в ответ будет возвращать частичное представление <i>History.cshtml</i>:<code>return PartialView("History", chatModel);</code></p>
<p>Теперь создадим само частичное представление <i>History.cshtml</i>:</p>
<pre class="brush:xml;">
@using AjaxChat.Models
@model ChatModel
&lt;div id="Users"&gt;
...
<p>В заключении можно еще как-нибудь стилизовать приложение. В итоге должно получиться примерно так. При запуске мы увидим поле логина:</p>
<img src="./pics/10.14.png" alt="Создание чата на ASP.NET MVC" />
<p>И при удачном входе в чат уже сможем читать и посылать сообщения:</p>
<img src="./pics/10.15.png"  alt="Создание чата на jQuery и AJAX" />
