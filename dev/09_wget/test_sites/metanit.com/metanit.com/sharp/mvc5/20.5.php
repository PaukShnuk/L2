...
<html >
<head>
<title>ASP.NET MVC 5 | Кэширование статического контента</title>
<meta charset="utf-8" />
<meta name="description" content="Кэширование статического контента в ASP.NET MVC 5, статическое и динамическое сжатие">
...
<h2>Кэширование статического контента</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Кроме кэширования результатов действий большое значение для оптимизации работы приложения имеет кэширование статического контекта. В данном случае под 
сттаическим контентом подразумеваются файлы изображений, скрипты javascript, файлы стилей css, какие-то другие файлы, например, видео или аудио-файлы, 
используемые на странице.</p>
<p>При использовании изображений можно их дополнительно оптимизировать с помощью различных сервисов, например, <a href="https://tinypng.org/" rel="nofollow">tinypng.org</a>, 
или составлять спрайты, что уменьшит объем загружаемых данных и снизит количество запросов к серверу.</p>
<p>При работе со скриптами javascript и стилями css их можно минимизировать с помощью специальных минификаторов. Кроме того, из стилей и скриптов можно составлять 
бандлы, что также оптимизирует производительность и уменьшает количество запросов к серверу.</p>
<p>Хотя эти методы и оптимизируют производительность, но их также бывает недостаточно, особенно когда мы имеем дело с высоконагруженными приложениями. 
К тому же еще может быть куча других типов статических файлов. В этом случае мы можем еще больше увеличить производительность, добавив кэширование.</p>
<p>Например, в приложении используется стили, скрипты и изображения. При первом обращении к ресурсу сервер отправит все необходимые файлы вместе с заголовком OK 200:</p>
<img src="./pics/20.4.png" />
<p>Как правило, браузеры сохраняют все загруженные статические файлы и при повторном запросе проверяют, изменился ли файл на сервере. Если сервер отдает заголовок 304 Not Modified, 
что знаичт, что файлы не изменялись, браузер использует ранее сохраненные файлы на локальном компьютере. Если файлы на сервере были изменены, то они повторно загружаются 
на локальный компьютер:</p>
<img src="./pics/20.5.png" />
<p>При большой нагрузке подобные запросы также могут влиять на работу сервера. И чтобы браузер кэшировал статические файлы и использовал их по умолчанию, 
нам надо настроить файл web.config.</p>
<p>В файле конфигурации в пределах узла <code>&lt;configuration&gt;</code> добавим новый элемент:</p>
<pre class="brush:xml;">
&lt;system.webServer&gt;
    &lt;staticContent&gt;
...
<p>Элемент <code>staticContent</code> определяет секцию, относящуюся к статическим файлам. В частности, с помощью элемента <code>&lt;clientCache&gt;</code> 
можно задать параметры кэширования на клиенте. Атрибут <code>httpExpires</code> задает максимальную дату, до которой контент в браузере будет кэшироваться. 
В данном случае это пятница 8 мая 2015 года. При определении даты мы должны следовать определенному формату.</p>
<p>Атрибут <code>cacheControlMode</code> принимает одно из значений, которые указывают на режим кэширования:</p>
<ul>
<li><p><code>NoControl</code>: используется по умолчанию, не добавляет в ответ клиенту никаких загловоков кэширования</p></li>
<li><p><code>DisableCache</code>: добавляет в ответ клиенту заголовок <code>Cache-Control: no-cache</code> - запрет кэширования</p></li>
<li><p><code>UseMaxAge</code>: добавляет в ответ клиенту заголовок <code>Cache-Control: max-age="временной отрезок"</code> - в качестве временного отрезка 
используется значение атрибута CacheControlMaxAge</p></li>
<li><p><code>UseExpires</code>: добавляет в ответ клиенту заголовок <code>Expires: дата</code>, где в качестве даты применяется значение атрибута 
httpExpires</p></p></li>
</ul>
<p>Теперь при повторном обращении приложение не будет посылать серверу запрос, изменились ли файлы или нет, а будет извлекать эти файлы из кэша.</p>
<img src="./pics/20.6.png" alt="Caching in ASP.NET MVC 5" />
<p>Однако параметр <code>httpExpires</code> не всегда удобен, особенно когда мы хотим задать определенное время кэширования, например, на месяц или на день, вне зависимости от 
текущей даты. Для этого изменим элемент <code>clientCache</code> следующим образом:</p>
<pre class="brush:xml;">
&lt;clientCache cacheControlCustom="public;max-age" 
                   cacheControlMaxAge="10.00:00:00" 
    cacheControlMode="UseMaxAge" /&gt;
</pre>
<p>Атрибут <code>cacheControlMaxAge</code> задает период кэширования в виде объекта TimeSpan. А параметр <code>cacheControlCustom</code> указывает 
на дополнительную директиву, которая будет использоваться в ответе клиенту.</p>
<p>Опять же в данном случае файлы будут браться из кэша без отправки дополнительных запросов серверу.</p>
<h3>Http-компрессия</h3>
<p>Применение сжатия позволит дополнительно оптимизировать нагрузку на сайт. Для этого в изменим вышеопределенный элемент <code>system.webServer</code>:</p>
<pre class="brush:xml;">
&lt;system.webServer&gt;
...
<p>Элемент <code>&lt;urlCompression&gt;</code> позволяет настроить сжатие с помощью следующих атрибутов:</p>
<ul>
<li><p><code>doDynamicCompression</code>: если имеет значение true, то применяется динамическое сжатие. По умолчанию true</p></li>
<li><p><code>doStaticCompression</code>: если имеет значение true, то применяется статическое сжатие. По умолчанию true</p></li>
<li><p><code>dynamicCompressionBeforeCache</code>:  если имеет значение true, то динамическое сжатие производится до помещения ответа в выходной кэш. По умолчанию false</p></li>
</ul>
<p>В результате сжатия браузер получит такой заголовок <code>Content-Encoding: gzip</code>. Соответственно если мы хотим отключить компрессию, то 
всем атрибутам надо поставить значение <code>false</code>.</p>
...
<div class="nav"><p><a href="./20.4.php">Назад</a><a href="./">Содержание</a><a href="./20.6.php">Вперед</a></p></div></div>
<div class="socBlock">
