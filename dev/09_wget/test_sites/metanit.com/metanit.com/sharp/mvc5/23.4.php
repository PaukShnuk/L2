...
<html >
<head>
<title>AutoMapper в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="AutoMapper в ASP.NET MVC 5, сопоставление и маппинг классов">
...
<h2>AutoMapper</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 20.02.2020</div>
...
<p>Automapper позволяет проецировать одну модель на другую, что позволяет сократить объемы кода и упростить программу.</p>
<p>Рассмотрим на примере. Пусть у нас есть следующая модель домена User, объекты которой хранятся в базе данных:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Для работы с моделью определены следующие классы контекста данных и репозитория:</p>
<pre class="brush:c#;">
...
<p>В контроллере предусмотрена базовая CRUD-функциональность, и для каждого представления созданы свои модели view model:</p>
<pre class="brush:c#;">
...
<p>Таким образом, есть одна модель домена и три модели представления, которые отличаются по набору свойств. Теперь добавим в проект 
через NuGet пакет Automapper:</p>
<img src="./pics/automapper.png" alt="Automapper in ASP.NET MVC 5" />
<p>После добавления применим Automapper в контроллере:</p>
<pre class="brush:c#;">
...
<p>Вначале создается объект репозитория IRepository. Затем в каждом методе производится конфигурация Automapper и сопоставление объектов.</p>
<p>Чтобы выполнить сопоставление типов, вначале надо определеить конфигурацию маппера, которой передается, какие именно типы и как именно будут сопоставляться. 
Для создания конфигурации используется конструктор <code>MapperConfiguration</code>:</p>
<pre class="brush:c#;">
var config = new MapperConfiguration(cfg =&gt; cfg.CreateMap&lt;User, IndexUserViewModel&gt;());
</pre>
<p>В этот конструктор передается параметр конфигурации, который с помощью различных методов, в частности, метода CreateMap, настраивает сопоставление классов. 
...
так как в методе Index получаем из бд объекты User и из них создаем объекты IndexUserViewModel, то первым в <code>cfg.CreateMap&lt;User,IndexUserViewModel&gt;</code> 
идет User. При конфигурации сопоставляются одноименные свойства User и IndexUserViewModel.</p>
<p>Однако при создании, например, набор свойств моделей CreateUserViewModel и User отличается, поэтому нам надо использовать дополнительный 
метод <code>ForMember()</code> для проецирования отдельных свойств одного класса на свойства другого класса:</p>
<pre class="brush:c#;">
var config = new MapperConfiguration(cfg =&gt; cfg.CreateMap&lt;CreateUserViewModel, User&gt;()
                    .ForMember("Name", opt =&gt; opt.MapFrom(c =&gt; c.FirstName + " " + c.LastName))
                    .ForMember("Email", opt =&gt; opt.MapFrom(src =&gt; src.Login)));
</pre>
<p>То же самое и с редактированием.</p>
<p>После определения конфигурации создается объект маппера, в конструктор которого передается объект конфигурации:</p>
<pre class="brush:c#;">var mapper = new Mapper(config);</pre>
<p>При сопоставлении объектов используется метод <span class="bb">mapper.Map()</span>:</p>
<pre class="brush:c#;">
User user = mapper.Map&lt;EditUserViewModel, User&gt;(model);
</pre>
<p>Причем также можно создавать коллекции объектов:</p>
<pre class="brush:c#;">
var users =
    apper.Map&lt;IEnumerable&lt;User&gt;, List&lt;IndexUserViewModel&gt;&gt;(repo.GetAll());
</pre>
<p>Таким образом, мы можем использовать AutoMapper для проецирования одних классов на другие.</p>
...
<div class="nav"><p><a href="./23.3.php">Назад</a><a href="./">Содержание</a><a href="./23.5.php">Вперед</a></p></div></div>
<div class="socBlock">
