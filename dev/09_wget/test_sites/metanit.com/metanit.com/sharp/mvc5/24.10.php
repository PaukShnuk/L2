...
<html >
<head>
<title>ASP.NET MVC 5 | Жизненный цикл приложения</title>
<meta charset="utf-8" />
<meta name="description" content="Жизненный цикл приложения в ASP.NET MVC">
...
<h2>Жизненный цикл приложения</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Отправляя из браузера запрос к приложению, нередко мы практически моментально получаем нужный контент. Однако в реальности обработка 
запроса проходит кучу различных этапов. И в этой статье мы рассмотрим основные этапы жизненного цикла приложения на ASP.NET MVC 5.</p>
<ol>
<li><p>После получения IIS запроса на обработку, при первом обращении к ресурсу, работающему под управлением CLR, создается объект класса 
<span class="bb">ApplicationManager</span>, который представляет домен приложения, внутри которого обрабатывается запрос. Домены приложений 
изолируют выполняющиеся приложения друг от друга. Уже внутри домена приложения создается объект класса <span class="bb">HostingEnvironment</span>, 
который предоставляет доступ к информации о приложении, в частности, он сообщает имя и каталог приложения.</p></li> 
<li><p>После создания домена приложения также создаются и инициализируются такие объекты, как <span class="bb"> HttpContext, HttpRequest</span> 
и <span class="bb">HttpResponse</span>, которые инкапсулируют всю информацию, связанную с текущим запросом к приложению.</p></li>
<li><p>На следующей стадии уже непосредственно запускается приложение, которое представляет экземпляр класса <span class="bb">HttpApplication</span>. 
Если в приложении определен файл <i>Global.asax</i>, то среда ASP.NET в качестве приложения создает объект класса <span class="bb">Global.asax</span>, 
который в свою очередь наследуется от класса <code>HttpApplication</code>. На этой же стадии происходит начальная инициализация приложения 
в методе <code>Application_Start</code>, который находится в файле Global.asax.cs.</p></li>
<li><p>После этого запрос начинает обрабатываться в конвейере класса <code>HttpApplication</code> - начинается собственно обработка запроса приложением. И 
первым шагом здесь является установка маршрута. Запрос перехватывается специальным HTTP-модулем под названием <span class="bb">UrlRoutingModule</span>. 
Этот модуль выбирает маршрут, который соответствует входящему запросу.</p>
<p>Весь набор маршрутов определяется в файле RouteConfig.cs:</p>
<pre class="brush:c#;">
...
<p>А сама регистрация маршрутов происходит в методе Application_Start строкой <code>RouteConfig.RegisterRoutes(RouteTable.Routes);</code>. Для каждого приложения имеется только один объект коллекции маршрутов RouteTable.</p></li>
<li><p>Если модулю UrlRoutingModule удастся сопоставить запрос с одним из маршрутов в коллекции RouteTable, то затем UrlRoutingModule 
выбирает обработчик маршрутов сопоставленного маршрута - объект IRouteHandler. По умолчанию объект IRouteHandler представляет экземпляр класса MvcRouteHandler.</p></li>
<li><p>Затем у объекта <code>IRouteHandler</code> вызывается метод GetHandler, который возвращает объект интерфейса <span class="bb">IHttpHandler</span>, используемый для обработки запроса. По умолчанию в качестве IHttpHandlera 
используется объект класса MvcHandler.</p></li>
<li><p>У обработчика IHttpHandler вызывается метод ProcessRequest для обработки запросов:</p>
<pre class="brush:c#;">
...
<p>На этом этапе уже происходит непосредственно создание контроллера, который представляет объект интерфейса IController. 
За создание контроллера отвечает объект интерфейса <code>IControllerFactory</code>, представляющий фабрику контроллеров. В качестве класса 
по умолчанию для фабрики контроллеров выступает класс <span class="bb">System.Web.Mvc.DefaultControllerFactory</span></p></li>
<li><p>После этого начинается собственно выполнение кода контроллера. После инициализации для запуска метода для обработки запроса 
контроллер вызывает метод InvokeAction():</p>
<pre class="brush:c#;">
public virtual bool InvokeAction(ControllerContext controllerContext, string actionName)
</pre></li>
<li><p>Далее привязчик модели (по умолчанию класс 
<span class="bb">System.Web.Mvc.DefaultModelBinder</span>) извлекает данные из запроса, производит из преобразование, форматирование, 
валидацию и связывает их с определенными параметрами вызываемого метода.</p></li>
<li><p>При вызове метода в ASP.NET MVC 5 запускается фильтр аутентификации, представляющий объект интерфейса IAuthenticationFilter. Он 
аутентифицирует пользователя</p></li>
<li><p>После фильтра аутентификации в ASP.NET MVC 5 запускается фильтр авторизации, представляющий реализацию интерфейса IAuthorizationFilter. 
До MVC 5 фильтры аутентификации и авторизации объединялись в один фильтр и срабатывали вместе. Фильтр авторизации управляет доступом пользователя к определенным 
ресурсам на основе его учетных данных.</p></li>
<li><p>Перед непосредственным выполнением метода контроллера запускается метод <code>OnActionExecuting</code> фильтра действий. 
Фильтр действий представляет объект интерфейса IActionFilter. Кроме того, также после выполнения метода контроллера запускается 
другой метод фильтра действий - метод <code>OnActionExecuted</code></p></li>
<li><p>Собственно выполнение метода контроллера. Он выполняет определенную логику и на выходе генерирует результат обработки в виде объекта 
ActionResult.</p></li>
<li><p>При обработке результата срабатывает другой фильтр - фильтр результатов - объект интерфейса IResultFilter. Его метод 
<code>OnResultExecuting</code> срабатывает до обработки результата, а метод <code>OnResultExecuted</code> после.</p></li>
<li><p>Генерация результата представляет создание объекта одного из классов результатов действий - ViewResult, ContentResult, FileResult, RedirectResult и др.</p>
<p>На заключительном этапе у каждого объекта ActionResult вызывается метод <code>ExecuteResult</code>, который обрабатывает результат действия. 
Для объектов ViewResult и PartialViewResult это выражается в поиске необходимого представления и его рендеринге движком представлений (как правило, движком Razor), 
который представляет объект интерфейса IViewEngine. Результат обработки в виде html-страницы посылается пользователю.</p>
<p>Для других объектов ActionResult (ContentResult, RedirectRefult и др.) происходит простая отправка результата в выходной поток.</p></li>
<li><p>И в конце пользователь получает результат обработки своего запроса.</p></li>
</ol>
<p>Схематично весь процесс конвейера приложения на ASP.NET MVC 5 можно представить так:</p>
<img src="./pics/lifecycle.png" alt="Обработка запросов в ASP.NET MVC 5" />
...
<div class="nav"><p><a href="./24.9.php">Назад</a><a href="./">Содержание</a><a href="./24.11.php">Вперед</a></p></div></div>
<div class="socBlock">
