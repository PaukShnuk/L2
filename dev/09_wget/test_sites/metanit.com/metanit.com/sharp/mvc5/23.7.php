...
<html >
<head>
<title>Data Access Layer в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Data Access Layer</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Первым уровнем, который мы реализуем, будет Data Access Layer или уровень доступа к данным. Этот уровень обычно содержит все модели данных, хранящихся 
в БД, а также классы, через которые идет взаимодействие с БД. И вначале создадим новый проект ASP.NET MVC без аутентификации. При этом решение 
назовем <span class="bb">NLayerApp</span>, а проект <span class="bb">NLayerApp.WEB<span>:</p>
<img src="./pics/23.3.png" alt="N-Layer ASP.NET application" />
<p>Проект NLayerApp.WEB будет представлять уровень представления.</p>
<p>После создания решения и проекта добавим в решение новый проект по типу Class Library, который назовем <span class="bb">NLayerApp.DAL</span>. 
Этот проект будет представлять уровень доступа к данным.</p>
<p>Первым делом определим модели, объекты которых будут храниться в бд. Для этого в проект добавим новую папку <i>Entities</i>, а в этой папке 
определим следующий класс модели Phone:
<pre class="brush:c#;">
...
<p>Теперь добавим в проект через NuGet пакет Entity Framework и также определим в проекте новую папку, которая пусть будет называться <i>EF</i>. 
В эту папку добавим класс контекста данных:</p>
<pre class="brush:c#;">
using System.Collections.Generic;
...
<p>Как и в проекте с монолитной архитектурой здесь также используется инициализатор БД, который теперь выполняется в статическом конструкторе контекста. 
Кроме того, для строки подключения конструктор контекста принимает строковый параметр connectionString.</p>
<p>Для увеличения гибкости подключения к БД используются репозитории. Поэтому вначале определим в проекте еще одну папку <i>Interfaces</i>. И в нее 
добавим интерфейс репозиториев IRepository:</p>
<pre class="brush:c#;">
using System;
using System.Collections.Generic;
...
<p>Поскольку мы будем использовать несколько репозитория для каждой сущности, то для упрощения использования подключения к бд будем использовать паттерн Unit Of Work. 
И для этого также в папку <i>Interfaces</i> добавим новый интерфейс IUnitOfWork:</p>
<pre class="brush:c#;">
using NLayerApp.DAL.Entities;
using System;
...
<p>Для хранения реализаций данных интерфейсов определим в проекте еще одну папку <i>Repositories</i>. Добавим в нее класс репозитория для смартфонов 
PhoneRepository:</p>
<pre class="brush:c#;">
using System;
using System.Collections.Generic;
...
<p>Также добавим в эту же папку класс репозитория для заказов OrderRepository:</p>
<pre class="brush:c#;">
using System;
using System.Collections.Generic;
using System.Data.Entity;
...
<p>И также в папку <i>Repositories</i> добавим класс EFUnitOfWork:</p>
<pre class="brush:c#;">
using System;
using NLayerApp.DAL.EF;
using NLayerApp.DAL.Interfaces;
...
<p>Класс EFUnitOfWork в конструкторе принимает строку - названия подключения, которая потом будет передаваться в конструктор контекста данных. 
Собственно через EFUnitOfWork мы и будем взаимодействовать с базой данных.</p>
<p>В итоге структура проекта будет выглядеть следующим образом:</p>
<img src="./pics/23.4.png" alt="Data Access Layer in ASP.NET MVC" />
<p>Собственно, и весь уровень доступа к данным, который мы могли бы сделать для проекта из прошлой темы, и теперь перейдем к построению бизнес-логики.</p>
...
<div class="nav"><p><a href="./23.6.php">Назад</a><a href="./">Содержание</a><a href="./23.8.php">Вперед</a></p></div></div>
<div class="socBlock">
