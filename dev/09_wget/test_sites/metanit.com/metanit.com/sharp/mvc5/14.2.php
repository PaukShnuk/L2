...
<html >
<head>
<title>Позиционирование элементов в Bootstrap</title>
<meta charset="utf-8" />
...
<h2>Позиционирование элементов</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>При использовании Bootstrap очень удобно позиционировать интерфейс в виде таблицы или сетки, используя строки и столбцы. Так, если мы 
...
расположены как бы в три столбца и составляют одну строку:</p>
<img src="./pics/14.4.png" />
<p>Если мы откроем код представления, то мы можем увидеть использование классов bootstrap:</p>
<pre class="brush:xml;">
&lt;div class="row"&gt;
    &lt;div class="col-md-4"&gt;
...
<p>Класс <span class="bb">row</span> задает расположение отдельных блоков в виде одной строки. Строка в Bootstrap может иметь до 12 столбцов. Строк может быть сколько угодно, 
но в данном случае у нас одна строка.</p>
<p>Для создания отдельного столбца строки используется класс <code>col-md-4</code>. <code>col</code>, как ясно из названия, обозначает столбец.</p>
<p>Дальше идет <code>md</code> - идентификатор устройства. Как мы увидели в прошлой теме, bootstrap делит все устройства условно на четыре группы в зависимости от 
...
единиц в строке будет занимать данный блок. Таким образом, получается, что класс <code>col-md-4</code> означает, что данный блок будет занимать 4 условных единицы из 12 в строке, то есть треть ширины 
экрана устройства с экраном шириной от 992 пикселей.</p>
<p>И поскольку у нас есть четыре группы, то для каждой группы имеются свои классы. Например, для очень маленьких устройств с экраном меньше 768 пикселей (то есть мобильных телефонов), 
подобный класс мог бы быть таким <span class="bb">col-xs-4</span>.</p>
<p>Все типы классов:</p>
<ul>
<li><p><span class="bb">col-xs-*</span>: для устройств с шириной экрана меньше 768 пикселей</p></li>
<li><p><span class="bb">col-sm-*</span>: для устройств с шириной экрана от 768 пикселей и выше</p></li>
<li><p><span class="bb">col-md-*</span>: для устройств с шириной экрана от 992 пикселя и выше</p></li>
<li><p><span class="bb">col-lg-*</span>: для устройств с шириной экрана от 1200 пикселей и выше</p></li>
</ul>
<p>Хотя даже на мобильных устройствах блок с классом <code>col-md-4</code> будет выглядеть вполне неплохо, но мы можем установить сразу два класса, 
чтобы еще больше детализировать отображение на различных устройствах, например:</p>
<pre class="brush:css;">
&lt;div class="col-xs-12 col-md-4"&gt;
</pre>
<p>И хотя в представлении Index.cshtml по умолчанию все три блока имеют относительную ширину в 4 единицы, составляя в целом 12 единиц, мы можем задать 
любую другую ширину, позиционируя их по собственному усмотрению. Например:</p>
<img src="./pics/14.5.png" />
<h3>Отступы</h3>
<p>Специальные классы <code>col-md(sm|lg)-offset-*</code> позволяют задать смещение относительно левого блока или начала строки в условных единицах. 
Например, у нас есть такая строка:<p>
<pre class="brush:xml;">
&lt;div class="row"&gt;
    &lt;div class="col-md-4"&gt;
        &lt;h2&gt;Левый блок&lt;/h2&gt;
...
<p>Класс <code>col-md-offset-4</code> будет смещать правый блок на 4 условных единицы вправо:</p>
<img src="./pics/14.6.png" />
<p>Некоторые примеры по использованию смещения:</p>
<img src="./pics/14.7.png" />
<h3>Порядок столбцов</h3>
<p>С помощью классов <code>col-md(sm|lg)-push-*</code> и <code>col-md(sm|lg)-pull-*</code> мы можем переопределить порядок блоков в строке. 
Например, класс <code>col-md-push-4</code> сдвигает блок на четыре единицы вправо от текущего положения. И наоборот, класс <code>col-md-pull-4</code> 
сдвигает блок влево от текущей позиции. Так, предыдущий пример со смещением мы могли бы переписать следующим образом:</p>
<pre class="brush:xml;">
&lt;div class="row"&gt;
    &lt;div class="col-md-4"&gt;
        &lt;h2&gt;Левый блок&lt;/h2&gt;
...
<p>И у нас был бы тот же эффект. Но мы можем также полностью переупорядочить порядок следования блоков:</p>
<pre class="brush:xml;">
&lt;div class="row"&gt;
...
<p>Теперь правый блок сместится влево на четыре единицы, а левый блок - вправо на 8 единиц:</p>
<img src="./pics/14.8.png" />
<p>Таким образом, используя класс row и классы столбцов, мы можем задать общее расположение элементов, а адаптивность bootstrap гарантирует, что  
на любых устройствах подобная сетка элементов будет выглядеть вполне нормально. Теперь рассмотрим некоторые компоненты, которые предлагает нам Bootstrap.</p>
