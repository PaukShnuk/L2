...
<html >
<head>
<title>Регистрация и создание пользователей в ASP.NET Identity в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Регистрация и создание пользователей в ASP.NET Identity</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 07.12.2015</div>
...
<p>Класс IdentityUser определяет множество свойств, однако нам необязательно всех их устанавливать. И в данном случае для создания пользователей 
нам лучше воспользоваться связующей моделью, которая установит все необходимые свойства. Итак, добавим в папку Models класс, который будет представлять 
пользователя:</p>
<pre class="brush:c#;">
using System;
...
<p>Теперь добавим в папку <i>Controllers</i> новый контроллер AccountController:</p>
<pre class="brush:c#;">
using System;
...
<p>Первым делом в контроллере создается свойство UserManager, возвращающее объект ApplicationUserManager. Через него мы будем взаимодействовать с 
хранилищем пользователей. Для получения хранилища применяется выражение <code>HttpContext.GetOwinContext().GetUserManager&lt;ApplicationUserManager&gt;()</code></p>
<p>Для собственно регистрации определен метод Register в Get- и Post-версиях. Post-версия представляет асинхронный метод, поскольку для создания пользователя 
здесь используется асинхронный вызов <code>UserManager.CreateAsync()</code>. Этот метод возвращает объект <span class="bb">IdentityResult</span>.</p>
<p>Если создание пользователя прошло успешно, то его свойство <code>Succeeded</code> будет равно <code>true</code>.</p>
<p>И в конце создадим представление для регистрации <i>Register.cshtml</i>:</p>
<pre class="brush:xml;">
...
<img src="./pics/aspnetidentity.png" alt="Регистрация в Asp.Net Identity" />
<p>И после ввода данных данные о пользователе попадут в таблицу AspNetUsers:</p>
<img src="./pics/aspnetidentity2.png" alt="Создание пользователей в Asp.Net Identity" />
...
<div class="nav"><p><a href="./12.15.php">Назад</a><a href="./">Содержание</a><a href="./12.17.php">Вперед</a></p></div></div>
<div class="socBlock">
