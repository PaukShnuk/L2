...
<html >
<head>
<title>Редактирование и удаление пользователей в ASP.NET MVC</title>
<meta charset="utf-8" />
...
<h2>Редактирование и удаление пользователей</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 07.12.2015</div>
...
<p>Редактирование и удаление пользователей в ASP.NET Identity представляется довольно простой задачей благодаря классу UserManager. Так, возьмем проект из 
прошлой темы, где у нас уже есть механизм регистрации и логина пользователей. Добавим в контроллер AccountController следующие методы:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>Методы Delete и DeleteConfirmed отображают пользователю представление для удаления и принимают выбор пользователя об удалении. Для удаления используется 
метод <code>UserManager.DeleteAsync()</code>. Он возвращает объект <span class="bb">IdentityResult</span>, который позволяет отследить успех операции.</p>
<p>Метод Edit также отображает представление для редактирования, передавая в него модель EditModel, которую мы далее создадим. В данном случае мы редактируем только 
значение свойства Year. POST-версия метода 
принимает данные модели и устанавливает значения ее свойств для пользователя. Редактирование также производится одним методом - методом <code>UserManager.UpdateAsync()</code></p>
<p>Далее создадим модель EditModel:</p>
<pre class="brush:c#;">
...
<p>Пусть редактирование будет производиться только для года рождения пользователя.</p>
<p>Представление для удаления могло бы выглядеть так:</p>
<pre class="brush:xml;">
@{
...
<p>И представление для редактирования:</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./12.17.php">Назад</a><a href="./">Содержание</a><a href="./12.19.php">Вперед</a></p></div></div>
<div class="socBlock">
