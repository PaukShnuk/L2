...
<html >
<head>
<title>Добавление и удаление данных в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Добавление и удаление данных</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В предыдущей теме мы посмотрели, как редактировать данные. Продолжим работу с моделью Book и теперь посмотрим, как мы можем ее добавлять и удалить из БД. 
Для добавления данных вначале определим пару действий:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>При получении объекта book в действии Create метод <code>db.Books.Add(book)</code> будет устанавливать значение <code>Added</code> в качестве состояния 
объекта модели. Поэтому метод <code>db.SaveChanges()</code> сгенерирует выражение INSERT для вставки данных модели в таблицу. То есть метод Create мы могли 
...
<p>Теперь самая важная часть - удаление данных. Даже не в плане реализации, сколько в плане безопасности. Добавим простое действие, которое удаляет 
объект из базы данных:</p>
<pre class="brush:c#;">
...
<p>Вначале мы проверяем, а есть ли такой объект в бд, и если есть, то вызываем метод <code>db.Books.Remove(b)</code>. Он установит статус объекта модели в <code>Deleted</code>, 
...
<p>В итоге при открытии письма 1-я запись в таблице может быть удалена. Уязвимость касается не только писем, но может проявляться и в других местах, 
...
подтвержить удаление. И удаляемый id уйдет второму методу по запросу POST. Таким образом, мы уйдем от уязвимости GET-запроса. Ну и само представление:</p>
<pre class="brush:c#;">
@{
    Layout = "~/Views/Shared/_Layout.cshtml";
}
