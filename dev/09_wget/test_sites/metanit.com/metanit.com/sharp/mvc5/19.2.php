...
<html >
<head>
<title>Модули в ASP.NET MVC</title>
<meta charset="utf-8" />
<meta name="description" content="Модули в ASP.NET MVC 5, использование встроенных модулей">
...
<h2>Модули</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В ASP.NET MVC модули представляют специальные классы, реализующие интерфейс <span class="bb">System.Web.IHttpModule</span>. Их инициализация происходит 
после создания объекта приложения. С помощью модулей мы можем управлять обработкой запроса в приложении.</p>
<p>Фреймворк ASP.NET MVC уже использует ряд встроенных модулей для различных задач:</p>
<ul>
<li><p><span class="bb">AnonymousIdentification</span>: представлен классом <code>System.Web.Security.AnonymousIdentificationModule</code>. Отвечает за 
идентификацию запросов даже в тех случаях, когда пользователь не аутентифицирован - то есть для анонимных запросов</p></li>
<li><p><span class="bb">DefaultAuthentication</span>: представлен классом <code>System.Web.Security.DefaultAuthenticationModule</code>. Отвечает за 
установку свойства User у объекта HttpContext</p></li>
<li><p><span class="bb">FileAuthorization</span>: представлен классом <code>System.Web.Security.FileAuthorizationModule</code>, который используется при аутентификации Windows</p></li>
<li><p><span class="bb">FormsAuthentication</span>: представлен классом <code>System.Web.Security.FormsAuthenticationModule</code>. Данный модуль используется при 
аутентификации форм и устанавливает значение свойства <code>HttpContext.User</code>, с помощью которого мы в приложении можем получить аутентифицированного пользователя</p></li>
<li><p><span class="bb">OutputCache</span>: представлен классом <code>System.Web.Caching.OutputCacheModule</code>. Отвечает за 
кэширование ответа клиенту</p></li>
<li><p><span class="bb">PageInspectorHttpModule</span>: этот модуль поддерживает функциональность Page Inspector, которая имеется в Visual Studio и с помощью которой 
осуществляется отладка HTML и CSS</p></li>
<li><p><span class="bb">Profile</span>: представляет класс <code>System.Web.Profile.ProfileModule</code>, который связывает данные профиля пользователя с данными в запросе</p></li>
<li><p><span class="bb">RoleManager</span>: представлен классом <code>System.Web.Security.RoleManagerModule</code>, который управляет назначением ролей</p></li>
<li><p><span class="bb">ScriptModule-4.0</span>: представлен классом <code>System.Web.Handlers.ScriptModule</code>. Предназначен для поддержки Ajax-запросов</p></li>
<li><p><span class="bb">ServiceModel-4.0</span>: представлен классом <code>System.ServiceModel.Activation.ServiceHttpModule</code>. Этот модуль используется веб-службами ASP.NET</p></li>
<li><p><span class="bb">Session</span>: реализован классом <code>System.Web.SessionState.SessionStateModule</code>. Предназначен для связи данных сессии с запросами</p></li>
<li><p><span class="bb">UrlAuthorization</span>: представлен классом <code>System.Web.Security.UrlAuthorizationModule</code>, который обеспечивает авторизованный доступ к ресурсам</p></li>
<li><p><span class="bb">UrlMappingsModule</span>: представлен классом <code>System.Web.UrlMappingsModule</code>. Предназначен для поддержки функции URL Mappings, 
которая в ASP.NET MVC не применяется</p></li>
<li><p><span class="bb">UrlRoutingModule-4.0</span>: реализован классом <code>System.Web.Routing.UrlRoutingModule</code>, который используется системой маршрутизации</p></li>
<li><p><span class="bb">WebPageHttpModule</span>: данный модуль сопоставляет запросы с файлами представлений</p></li>
<li><p><span class="bb">WindowsAuthentication</span>: представляет класс <code>System.Web.Security.WindowsAuthenticationModule</code>. Отвечает за аутентификацию Windows</p></li>
</ul>
<p>В принципе мы можем сами получить список используемых модулей в своем приложении. Для этого в контроллере получим все модули через свойство <span class="bb">HttpContext.ApplicationInstance.Modules</span>. 
Допустим, у нас следующий метод получает модули и передает их в представление:</p>
<pre class="brush:c#;">
...
<p>И в представлении осуществим простой вывод массива:</p>
<pre class="brush:xml;">
