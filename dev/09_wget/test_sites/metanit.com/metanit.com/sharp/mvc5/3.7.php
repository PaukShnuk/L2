...
<html >
<head>
<title>Контекст запроса HttpContext. Куки. Сессии в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Контекст запроса HttpContext, объекты Request и Response, получение данных о запросе и управление ответом. Работа с куками и сессиями в ASP.NET MVC 5">
...
<h2>Контекст запроса HttpContext. Куки. Сессии</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Нередко для обработки запроса требуется информация о контексте запроса: какой у пользователя браузер, ip-адрес, с какой страницы или сайта пользователь 
попал к нам. И ASP.NET MVC позволяет получить всю эту информацию, используя объект <b>HttpContext</b>.</p>
<p>Хотя в контроллере мы также можем обратиться к объекту <code>ControllerContext</code>, который имеет свойство 
HttpContext и по сути предоставляет доступ к той же функциональности и информации. Но в то же время между ними есть некоторые различия. 
Объект HttpContext описывает данные конкретного http-запроса, который обрабатывается приложением. А ControllerContext описывает данные 
http-запроса непосредственно по отношению к данному контроллеру.</p>
<p>Вся информация о контексте запроса доступна нам через свойства объекта HttpContext. Так, все данные запроса содержится в свойстве 
<code>Request</code>. <b>HttpContext.Request</b> представляет объект класса, унаследованного от <code>HttpRequestBase</code>, и поэтому содержит все его свойства. Рассмотрим некоторые из них:</p>
<ul>
<li><p>Получение браузера пользователя: <span class="bb">HttpContext.Request.Browser</span></p></li>
<li><p>Иногда просто браузера недостаточно, тогда можно обратиться к агенту пользователя: <b>HttpContext.Request.UserAgent</b></p></li>
<li><p>Получение url запроса: <span class="bb">HttpContext.Request.RawUrl</span></p></li>
<li><p>Получение IP-адреса пользователя: <span class="bb">HttpContext.Request.UserHostAddress</span></p></li>
<li><p>Получение реферера: <code>HttpContext.Request.UrlReferrer == null ? "" : HttpContext.Request.UrlReferrer.AbsoluteUri</code> 
Так как реферер может быть не определен, то сначала смотрим, не равен ли он null</p></li>
</ul>
<p>Например:</p>
<pre class="brush:c#;">
public string Index()
...
<p>Если HttpContext.Request содержит информацию о запросе, то свойство <code>HttpContext.Response</code> управляет ответом. Оно представляет 
объект <code>HttpResponse</code>, который передает на сторону клиента некоторые значения: куки, служебные заголовки, сам ответ в виде кода html. 
Например, установим кодировку ответа: 
<code>HttpContext.Response.Charset = "iso-8859-2";</code></p>
<p>Методы объекта HttpResponse позволяют управлять ответом. Например, метод <code>AddHeader</code> позволяет добавить к ответу дополнительный заголовок.</p>
<p>Кроме того, нам необязательно вызывать метод <code>View</code> в действия контроллера, чтобы отправить клиенту ответ запроса. Мы вполне 
можем воспользоваться методом <code>HttpContext.Response.Write</code>:</p>
<pre class="brush:c#;">
...
<p>Метод <code>HttpContext.Response.Write</code> здесь добавляет в поток определенное содержимое, переданное в качестве параметра. Но в реальности, конечно, 
проще использовать методы, генерирующие объекты ActionResult, например, представления.</p>
<h3>Определение пользователя</h3>
<p>Также объект HttpContext содержит информацию о пользователе в свойстве <span class="bb">HttpContext.User</span>:</p>
<pre class="brush:c#;">
...
<p>Подробнее об идентификации пользователя можно узнать в главе про аутентификацию и авторизацию.</p>
<h3>Работа с куки</h3>
<p>Чтобы получить куки, нам надо воспользоваться свойством <code>HttpContext.Request.Cookies</code>:</p> 
<pre class="brush:c#;">string id =HttpContext.Request.Cookies["id"].Value;</pre>
<p>В данном случае, если у нас установлена на стороне клиента куки "id", то мы получим ее значение.</p>
<p>Однако прежде чем получать значения куки, их естественно надо установить. Для установки значения куки мы можем использовать 
свойство <code>HttpContext.Response</code>. Например, установим в куки значение "id":</p> 
<pre class="brush:c#;">HttpContext.Response.Cookies["id"].Value = "ca-4353w";</pre>
<h3>Сессии</h3>
<p>Сессии также, как и куки, используются для хранения некоторых данных, которые можно получить в любом месте приложения. Для работы с ними используется объект <span class="bb">Session</span>. Например, 
установим в одном методе контроллера мы можем установить значение сессии:</p>
<pre class="brush:c#;">
...
<p>А получить можно в другом методе:</p>
<pre class="brush:c#;">
...
<p>Если мы хотим удалить значение сессии для ключа name, мы можем просто присвоить значение null: <code>Session["name"]=null;</code></p>
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/9_yJz8f-YRU"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 14. Контекст запроса, куки, сессии в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
