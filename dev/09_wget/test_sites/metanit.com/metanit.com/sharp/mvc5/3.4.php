...
<html >
<head>
<title>ViewResult и генерация представлений в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Класс ViewResult и генерация представлений в ASP.NET MVC 5, передача данных в представление через объекты ViewBag, ViewData, TempData">
...
<h2>ViewResult и генерация представлений</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Класс ViewResult является наиболее часто возвращаемым результатом действий контроллера. Он производит рендеринг представления в 
веб-страницу и возвращает ее в виде ответа клиенту.</p>
<p>Чтобы возвратить объект ViewResult используется метод <span class="bb">View</span>:</p>
<pre class="brush:c#;">
...
<p>Вызов метода View возвращает объект ViewResult. Затем уже ViewResult производит рендеринг определенного представления в ответ. По умолчанию 
контроллер производит поиск представления в проекте по следующему пути: <span class="ii">/Views/Имя_контроллера/Имя_представления.cshtml</span></p>
<p>Согласно настройкам по умолчанию, если представление не указано явным образом, то в качестве представления будет использоваться то, имя которого 
совпадает с именем действия контроллера. Например, вышеопределенное действие Index по умолчанию будет производить поиск представления Index.cshtml 
в папке <span class="ii">/Views/Home/</span>.</p>
<p>Однако можно также задать имя представления явным образом:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
...
<p>В итоге в качестве представления будет выбрано представление <span class="ii">/Views/Home/Index.cshtml</span>. Мы также можем полностью переопределить путь, 
по которому система будет искать представление:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
    public ViewResult SomeMethod()
...
<p>Правда, если такого пути не окажется, то приложение выбросит ошибку.</p>
<h3>Передача данных из контроллера в представление</h3>
<p>Существуют различные способы передачи данных из контроллера в представление. Один из них представляет использование объекта 
<b>ViewData</b>.</p> 
<p>ViewData представляет словарь из пар ключ-значение:</p>
<pre class="brush:c#;">
public ViewResult SomeMethod()
{
...
<p>В этом случае в представлении SomeView.cshtml можно получить передаваемую строку следующим образом:</p>
<pre class="brush:xml;">
@{
    Layout = null;
}
...
<p>Еще один способ передачи данных представляет объект <b>ViewBag</b>. Этот объект позволяет определить различные свойства и присвоить им любое значение. Так, мы могли бы переписать 
предыдущий пример следующим образом:</p>
<pre class="brush:c#;">
public ViewResult SomeMethod()
{
...
<p>И таким же образом изменить представление:</p>
<pre class="brush:xml;">
............................
&lt;body&gt;
    &lt;div&gt;
...
<p>И не важно, что изначально объект ViewBag не содержит никакого свойства Head, оно определяется динамически. При этом свойства ViewBag могут 
содержать не только простые объекты типа string или int, но и сложные данные. Так, в примере из прошлой главы мы добавляли в объект ViewBag список объектов модели:</p>
<pre class="brush:c#;">
public ActionResult Index()
{
...
<p>Хотя ViewData и ViewBag и похожи, в то же время они не полностью эквивалентны. Так, например, нельзя передавать динамические значения из 
ViewBag в методы расширения в представлениях. Например, мы не можем написать 
<code>@Html.TextBox("name", ViewBag.Name)</code>, так как компилятор C# должен знать тип каждого параметра во время компиляции, чтобы выбрать 
нужный метод расширения. В этом случае нам надо либо использовать ViewData:<code>@Html.TextBox("name", ViewData["Name"])</code>, либо применить 
приведение типов: <code>@Html.TextBox("name", (string)ViewBag.Name)</code></p>
<p>И еще один способ предлагает объект <span class="b">TempData</span>. TempData представляет словарь, хранящий пары ключ-значение, как и ViewData, 
но его использование немного отличается. TempData позволяет сохранять переданное значение в течении всего текущего запроса. Использование TempData аналогично 
работе с ViewData.</p>
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/hI7j5ibHjQQ"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 11. ViewResult. Генерация представлений в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
