...
<html >
<head>
<title>Редактирование данных в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Редактирование простой модели в ASP.NET MVC 5, состояние EntityState.Modified">
...
<h2>Редактирование данных</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В предыдущей теме мы посмотрели, как с помощью шаблонных хелперов отображать значения модели. Теперь же займемся логикой редактирования 
данных. Вначале добавим в контроллер действие, которые будет получать по Id объект модели и выводить в представление ее свойства для редактирования:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>На случай, если пользователи не укажут id, мы устанавливаем в качестве параметра не int, а int?. И если такой параметр не передан, то возвращаем результат метода <code>HttpNotFound</code>.</p>
<p>А представление у нас будет содержать набор хелперов EditorFor для некоторых полей модели:</p>
<pre class="brush:xml;">
@{
...
<p>Так как уникальный идентификатор id книги нам не надо редактировать, то поле для его отображения сделаем скрытым, то есть воспользуемся 
хелпером <code>Html.HiddenFor</code>.</p>
<p>Теперь нам нужен сам код сохранения. Определим в контроллере действие EditBook, которое будет обрабатывать POST-запросы:</p>
<pre class="brush:c#;">
[HttpPost]
...
<p>С помощью строки <code>db.Entry(book).State = EntityState.Modified;</code> мы указываем, что объект <code>book</code> уже существует в базе данных, 
и для него надо внести в базу измененное значение, а не создавать новую запись. После чего перенаправляемся на главную страницу.</p>
<p>Стоит отметить, что хотя Entity Framework позволяет нам абстрагироваться от запросов sql и структуры бд, но на низком уровне, когда мы устанавливаем 
значение <code>db.Entry(book).State = EntityState.Modified;</code>, то мы тем самым указываем методу <code>db.SaveChanges()</code>, что надо 
сгенерировать и выполнить комаду UPDATE для обновления данных в БД.</p>
<p>Обратимся к методу EditBook, например, <code>Home/EditBook/1</code>:</p>
<img src="./pics/5.9.png" />
<p>Хелпер <code>Html.EditorFor</code> сгенерировал нам поля для редактирования. Мы можем изменить объект модели, и отправить его на сервер, где произойдет ее сохранение.</p>
