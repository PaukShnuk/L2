...
<html >
<head>
<title>ASP.NET MVC 5 | MemoryCache</title>
<meta charset="utf-8" />
<meta name="description" content="MemoryCache и кэширование объектов в памяти в ASP.NET MVC 5">
...
<h2>MemoryCache</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Начиная с версии .NET 4.0 во фреймворк был добавлен класс <span class="bb">MemoryCache</span>, который предназначен для кэширования контента. Данный класс 
не предназначен только для создания веб-приложений в ASP.NET, его можно использовать и при написании графических и даже консольных приложений.</p>
<p>При использовании MemoryCache следует учитывать, что объект этого класса привязан к домену приложения. И также надо учитывать, что этот объект будет уничтожаться веб-сервером IIS при перезапуске пула приложения в следующих случаях:</p>
<ul>
<li><p>Если в течение 20 минут не было запросов к приложению</p></li>
<li><p>Если с момента запуска (последнего перезапуска) приложения прошло 1740 минут - временной интервал для перезапуска приложений в IIS по умолчанию</p></li>
<li><p>Если в папку проекта на IIS была добавлена новая копия приложения (идет автоматический перезапуск)</p></li>
</ul>
<p>Для использования MemoryCache надо добавить в проект библиотеку <span class="bb">System.Runtime.Caching.dll</span>.</p>
<p>Пусть у нас есть некоторая модель Phone и класс контекста данных PhoneContext:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Добавим новый класс, который будет использовать функционал MemoryCache:</p>
<pre class="brush:c#;">
...
<p>Во многом данный класс напоминает стандартный репозиторий. Теперь применим его для получения объекта Phone в методе контроллера:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./20.5.php">Назад</a><a href="./">Содержание</a><a href="./21.1.php">Вперед</a></p></div></div>
<div class="socBlock">
