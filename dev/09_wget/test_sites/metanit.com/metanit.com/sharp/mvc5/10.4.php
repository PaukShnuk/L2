...
<html >
<head>
<title>Параметры объекта AjaxOptions в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Параметры объекта AjaxOptions в ASP.NET MVC 5 и его настройка для отправки ajax-запросов">
...
запроса:</p>
<ul>
	<li><p><span class="b">Confirm</span> - настраивает сообщение о подтверждении отправки запроса на сервер. Если пользователь не подтвердит, то 
	запрос не будет отправлен</p></li>
	<li><p><span class="b">HttpMethod</span> - устанавливает метод (Get или Post), с помощью которого выполняется запрос</p></li>
	<li><p><span class="b">InsertionMode</span> - устанавливает, как полученные результаты будут отображаться на странице. Может принимать одно из трех значений 
	перечисления <code>InsertionMode</code>: <code>InsertAfter</code>, <code>InsertBefore</code> и <code>Replace</code>(по умолчанию).</p></li>
	<li><p><span class="b">LoadingElementId</span>- устанавливает id элемента html-страницы, который будет отображаться во время запроса. 
	Обычно это какая-нибудь анимация, которая дает знать, что некоторая работа выполняется в фоновом режиме</p></li>
	<li><p><span class="b">LoadingElementDuration</span> - устанавливает количество миллисекунд, через которое появится элемент, указанный в параметре LoadingElementId</p></li>
	<li><p><span class="b">OnBegin</span> - задает обратный вызов перед отправкой запроса. Соотносится с событием <code>beforeSend</code> библиотеки jQuery</p></li>
	<li><p><span class="b">OnComplete</span> - задает обратный вызов, который вызывается после выполнения запроса (как удачного, так и неудачного).
	 Соотносится с событием <code>complete</code> библиотеки jQuery</p></li>
	<li><p><span class="b">OnFailure</span> - задает обратный вызов, который вызывается после неудачного выполнения запроса. Соотносится с событием <code>error</code> библиотеки jQuery</p></li>
	<li><p><span class="b">OnSuccess</span> - задает обратный вызов, который вызывается после удачного выполнения запроса. Соотносится с событием <code>success</code> библиотеки jQuery</p></li>
	<li><p><strong>UpdateTargetId</strong> - указывает на id элемента, в котором будут выводиться результаты запроса</p></li>
	<li><p><span class="b">Url</span> - устанавливает адрес Url сервера, на который отправляется запрос. Установив данное свойство, можно не использовать название 
...
<p>Чтобы их использовать, определим соответствующие обработчики javascript в представлении и укажем их в параметрах объекта <code>AjaxOptions</code>:</p>
<pre class="brush:xml;">
...
<img src="./pics/10.9.png" />
<h5>Обратный вызов OnSuccess</h5>
<img src="./pics/10.10.png" />
<p>Таким образом, объект AjaxOptions позволяет настроить различные параметры ajax-запроса.</p>
...
<div class="nav"><p><a href="./10.3.php">Назад</a><a href="./">Содержание</a><a href="./10.5.php">Вперед</a></p></div></div>
<div class="socBlock">
