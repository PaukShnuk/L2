...
<html >
<head>
<title>Авторизация пользователей в ASP.NET Identity</title>
<meta charset="utf-8" />
...
<h2>Авторизация пользователей в ASP.NET Identity</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 07.12.2015</div>
...
<p>Для создания инфраструктуры для входа пользователей на сайт вначале добавим в проект в папку Models специальную модель LoginModel:</p>
<pre class="brush:c#;">
...
<p>В прошлой теме у нас уже был добавлен контроллер AccountController. Теперь же добавим в него следующие строки:</p>
<pre class="brush:c#;">
...
<p>Вначале определяется новое свойство, представляющее объект <span class="bb">IAuthenticationManager</span>, с помощью которого мы будем управлять входом на сайт. 
Для этого интерфейс IAuthenticationManager определяет два метода:</p>
<ul>
<li><p><code>SignIn()</code>: создает аутентификационные куки</p></li>
<li><p><code>SignOut()</code>: удаляет аутентификационные куки</p></li>
</ul>
<p>В Get-версии метода Login мы получаем адрес для возврата и передаем его в преставление.</p>
<p>В Post-версии метода Login получаем данные из представления в виде модели LoginModel и по ним пытаемся получить пользователя из бд с помощью метода 
<code>UserManager.FindAsync(model.Email, model.Password)</code>. Этот метод возвращает объект ApplicationUser в случае успеха поиска.</p>
<p>AspNet Identity использует аутентификацию на основе объектов claim. Поэтому нам надо сначала создать объект <span class="bb">ClaimsIdentity</span>, 
который представляет реализацию интерфейса IIdentity в AspNet Identity. Для создания ClaimsIdentity применяется метод <code>CreateIdentityAsync()</code></p>
<p>И на финальном этапе вызывается метод <code>AuthenticationManager.SignIn()</code>, в который передается объект конфигурации аутентификации 
<code>AuthenticationProperties</code>, а также ранее созданный объект ClaimsIdentity. Свойство <code>IsPersistent</code> позволяет сохранять аутентификационные данные в браузере 
даже после закрытия пользователем браузера.</p>
<p>И метод <code>Logout</code> просто удаляет аутентификационные куки в браузере, как бы делая выход из системы.</p>
<p>Представление логина может выглядеть так:</p>
<pre class="brush:c#;">
...
<img src="./pics/aspnetidentity3.png" alt="Логин в Asp.Net Identity" />
...
<div class="nav"><p><a href="./12.16.php">Назад</a><a href="./">Содержание</a><a href="./12.18.php">Вперед</a></p></div></div>
<div class="socBlock">
