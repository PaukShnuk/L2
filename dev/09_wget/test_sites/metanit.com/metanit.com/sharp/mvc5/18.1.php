...
<html >
<head>
<title>Тестирование в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Тестирование в ASP.NET MVC 5, что такое юнит-тесты, понятие Test-Driven Development">
...
<h2>Введение в тестирование. Test-Driven Development</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Одно из преимуществ разработки на платформе ASP.NET MVC предоставляют богатые возможности по тестированию веб-приложения. Можно самим выполнять тестирование 
тех или иных моментов вручную, а можно использовать специальные небольшие программы, которые называются <b>юнит-тесты</b>.</p>
<p>Юнит-тесты позволяют быстро и автоматически протестировать отдельные участки кода независимо от остальной части программы. При надлежащем составлении 
юнит-тесты вполне могут покрыть большую часть кода приложения.</p>
<p>Большинство юнит-тестов так или иначе имеют ряд следующих признаков:</p>
<p><span class="bb">Тестирование небольших участков кода ("юнитов")</span></p>
<p>При создании юнит-тестов выбираются небольшие участки кода, которые надо протестировать. Как правило, тестируемый участок должен быть меньше класса, 
а в большинстве случаев тестируется отдельный метод класса. Упор на небольшие участки позволяет довольно быстро писать простенькие тесты.</p>
<p>Однажды написанный код нередко читают многократно, поэтому важно писать понятный код. Особенно это важно в юнит-тестах, где в случае неудачи при 
...
значительно упрощает подобную работу.</p>
<p><span class="bb">Тестирование в изоляции от остального кода</span></p>
<p>При тестировании важно изолировать тестируемый код от остальной программы, с которой он взаимодействует, чтобы потом четко определить возможность 
ошибок именно в этом изолированном коде. Что упрощает и повышает контроль над отдельными компонентами программы.</p></li>
<p><span class="bb">Тестирование только общедоступных конечных точек</span></p>
<p>Всего лишь небольшие изменения в классе могут привести к неудаче многих юнит-тестов, поскольку реализация используемого класса изменилась. 
...
В итоге уменьшается вероятность, что изменения в классах могут привести к провалу юнит-тестов.</p>
<p><span class="bb">Автоматизация тестов</span></p>
<p>Написание юнит-тестов для небольших участков кода ведет к тому, что количество этих юнит-тестов становится очень велико. 
И если процесс получения результатов и проведения тестов не автоматизирован, то это может привести к непроизводительному расходу рабочего времени и 
...
Для автоматизации процесса разработчики обычно обращаются к фреймворкам юнит-тестирования</p>
<h4>Фрейморки тестирования</h4>
<ul>
<li><p><b>MS Test</b>: фреймворк юнит-тестирования от компании Microsoft, который по умолчанию включен в Visual Studio (начиная с VS 2012 во все версии)</p></li>
<li><p><b>NUnit</b>: портированный фреймворк с JUnit для платформы .NET</p></li>
<li><p><b>xUnit.net</b>: еще один фреймворк тестирования от создателей NUnit для платформы .NET</p></li>
</ul>
<h3>Разработка через тестирование (Test-Driven-Development)</h3>
<p>Разработка через тестирование (TDD) процесс применения юнит-тестов, при котором сначала пишутся тесты, а потом уже программный код, достаточный для выполнения этих тестов.</p>
<p>Использование TDD позволяет снизить количество потенциальных багов в приложении. Создавая тесты перед написанием кода, мы тем самым описываем 
...
момент создания теста еще не существует). Таким образом, тесты помогают оформить и описать API будущих компонентов.</p>
<p>Порядок написания кода при TTD довольно прост:</p>
<ul>
<li><p>Пишем юнит-тест</p></li>
<li><p>Запускаем его и видим, что он завершился неудачей (программный код ведь еще не написан)</p></li>
<li><p>Пишем некоторое количество кода, достаточное для запуска теста</p></li>
<li><p>Снова запускаем тест и видим его результаты</p></li>
</ul>
<p>Этот цикл повторяется снова и снова, пока не будет закончена работа над программным кодом. Так как большинство фреймворков юнит-тестирования 
...
(опять же выводится текст зеленого цвета), то данный цикл часто называют <i>красным/зеленым циклом</i>.</p>
<h3>Интеграционные тесты</h3>
<p>Даже если вы охватите юнит-тестами практически всю логику приложения, все равно могут быть моменты, которые будут работать не так, как надо. Кроме того, 
сложно протестировать представления. Подобные вещи тестируются с помощью интеграционных тестов, выполняющихся на уровне веб-браузера. Для создания подобных тестов 
в Visual Studio 2013 в версиях Ultimate и Premium доступен такой тип проекта как <span class="bb">Coded UI Test Project</span>. Кроме того, доступны 
open-source решения для создания интеграционных тестов:</p>
<ul>
<li><p><a href="http://watin.org" rel="nofollow">WatiN</a> - эмулирует поведение пользователя в веб-браузере. Поддерживает в том числе сайты, использующие ajax</p></li>
<li><p><a href="http://seleniumhq.org" rel="nofollow">SeleniumHQ</a> - набор инструментов для тестирования в браузерах на различных платформах</p></li>
</ul>
<h3>Нагрузочные тесты</h3>
<p>Еще один вид тестов представляют <b>нагрузочные тесты</b>, которые призваны протестировать работу сайта в условиях высокой нагрузки. Подобные тесты 
позволяют выявить узкие места при работе с базой данных или при обращении к диску и ряд других проблем, которые сложно выявить другими способами.</p>
<p>Visual Studio Ultimate также имеет инструментарий для создания нагрузочных тестов.</p>
...
<div class="nav"><p><a href="./17.10.php">Назад</a><a href="./">Содержание</a><a href="./18.2.php">Вперед</a></p></div></div>
<div class="socBlock">
