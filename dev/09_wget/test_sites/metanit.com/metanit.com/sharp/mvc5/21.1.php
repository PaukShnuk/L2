...
<html >
<head>
<title>Внедрение зависимостей в ASP.NET MVC</title>
<meta charset="utf-8" />
<meta name="description" content="Внедрение зависимостей в ASP.NET MVC, инверсия управления, Inversion of Control, IoS и DI контейнеры">
...
<h1>Внедрение зависимостей</h1><h2>Инверсия управления</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В небольших приложениях на ASP.NET MVC мы относительно легко можем заменить одни классы на другие, вместо одного контекста данных использовать другой. 
Однако в крупных приложениях это уже будет проблематично сделать, особенно если у нас десятки контроллеров с сотней методов. В этой ситуации нам на помощь 
может прийти такой механизм как <b>внедрение зависимостей</b></p>
<p>Рассмотрим простой пример. Допустим, у нас есть некая модель Book:</p>
<pre class="brush:c#;">
public class Book
{
...
<p>И пусть у нас определен контекст данных и репозиторий для управления объектами данной модели:</p>
<pre class="brush:c#;">
...
<p>Репозиторий содержит ссылку на контекст данных и с его помощью управляет моделями, например, добавляет в бд и т.д. Теперь рассмотрим использование 
репозитория в контроллере:</p>
<pre class="brush:c#;">
...
<p>Контроллер содержит ссылку на BookRepository и в данном случае он <span class="bb">зависит</span> от BookRepository. А сам объект 
BookRepository является зависимостью.</p>
<p>Контроллер ссылается на конкретную реализацию репозитория, поэтому между ними существует жесткая связь (tight coupling). На первый взгляд ничего 
плохого здесь нет, но, например, у меня изменилась схема подключения к базе данных: вместо MS SQL Servera я решил использовать MongoDB или MySql. Кроме того, 
может потребоваться динамически менять один класс на другой.</p>
<p>В данном случае жесткая связь привязывает контроллер к конкретной реализации репозитория. Такой код по мере расширения приложения сложнее поддерживать и сложнее тестировать. 
Поэтому рекомендуется уходить от использования жесткосвязанных компонентов к слабосвязанным. Для этого определим новый интерфейс IRepository и изменим 
класс репозитория:</p>
<pre class="brush:c#;">
...
<p>Параллельно с этим я могу определить и другой класс репозитория, например, для использования MySql:</p>
<pre class="brush:c#;">
...
<p>И теперь мы можем избавить контроллер от жесткой зависимости от компонента:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
    IRepository repo;
...
<p>Теперь контроллер ничего не знает и не зависит от конкретной реализации репозитория. Теперь его создание выносится из класса контроллера во вне. 
Подобный процесс еще называется <b>инверсией управления (inversion of control (IoC))</b></p>
