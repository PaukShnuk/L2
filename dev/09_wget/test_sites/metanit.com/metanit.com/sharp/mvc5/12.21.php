...
<html >
<head>
<title>Авторизация на основе Claims | ASP.NET MVC</title>
<meta charset="utf-8" />
...
<h2>Авторизация на основе Claims</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 06.12.2015</div>
...
<p>Инструмент Claims в рамках ASP.NET Identity представляет нам прекрасный способ для создания системы авторизации, которая основана 
на самых различных свойствах объекта или так называемая <span class="b">Claims-Based Authorization</span>. Например, если мы хотим разграничить 
доступ к ресурсам в зависимости от возраста пользователя, то с помощью Claims мы это легко можем сделать.</p>
<p>Итак, создадим новый проект с типом авторизации Individual User Accounts. Пусть нам надо добавить разграничение доступа по возрасту пользователя. 
Для этого прежде всего изменим подсистему регистрации.</p>
<p>В модель RegisterModel из файла <i>Models/AccountViewModels.cs</i> добавим свойство Year для хранения года рождения пользователя:</p>
<pre class="brush:c#;">
...
<p>И также добавим в представление дл регистрации <i>Register.cshtml</i> код для создания поля ввода года рождения:</p>
<pre class="brush:xml;">
...
<p>Итак, в модели RegisterModel определено свойство Year, а в представлении ля него имеется поле ввода. Теперь изменим post-версию метода Register, 
чтобы использовать это свойство:</p>
<pre class="brush:c#;">
[HttpPost]
[AllowAnonymous]
...
<p>После создания пользователя вначале формируем объект <span class="b">IdentityUserClaim</span>:</p>
<pre class="brush:c#;">var identityClaim = new IdentityUserClaim { ClaimType = "Year", ClaimValue = model.Year.ToString() };</pre>
<p>Чтобы задействовать этот класс, надо подключить пространство имен <code>Microsoft.AspNet.Identity.EntityFramework</code>. Свойство 
<code>ClaimType</code> отвечает за тип данных. Поскольку здесь сохраняется год, то и тип называется Year. А свойство <code>ClaimValue</code> 
хранит значение для этого типа в виде строки.</p>
<p>Далее claim добавляется пользователю, и происходит обновление:</p>
<pre class="brush:c#;">
user.Claims.Add(identityClaim);
await UserManager.UpdateAsync(user);
</pre>
<p>В итоге claim будет сохранен в базе данных. Для хранения claim в ASP.NET Identity используется таблица <span class="b">AspNetUserClaims</span>. А 
сохраненые данные будут выглядеть примерно так:</p>
<img src="./pics/aspnetclaims.png" alt="Claims Based Authorization" />
<p>В данном случае определяется claim "Year" для хранения года рождения, но также мы можем сохранять и любую другую информацию о пользователе.</p>
<p>Затем изменим класс <span class="b">ApplicationUser</span>, чтобы он сохранял данные своего claim в куках:</p>
<pre class="brush:c#;">
...
<p>И в конце нам надо создать атрибут, который бы позволил устанавливать ограничения по доступу. Для этого добавим в проект новую папку Util, 
и затем в нее добавим новый класс <span class="b">ClaimsAuthorizeAttribute</span>:</p>
<pre class="brush:c#;">
using System;
...
<p>Данный класс наследуется от атрибута авторизации <span class="b">AuthorizeAttribute</span> и добавляет свойство <code>Age</code>, 
которое позволит устанавливать ограничения по возрасту.</p>
<p>В самом классе с помощью получаемого контекста запроса httpContext извлекаем claim и устанавливаем, имеет ли текущий пользователь 
доступ к ресурсу. Если пользователь проходит авторизацию, то возвращается <code>true</code>, если нет, то <code>false</code>.</p>
<p>И после этого мы сможем использовать данный атрибут в контроллере. Например:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./12.20.php">Назад</a><a href="./">Содержание</a><a href="./13.1.php">Вперед</a></p></div></div>
<div class="socBlock">
