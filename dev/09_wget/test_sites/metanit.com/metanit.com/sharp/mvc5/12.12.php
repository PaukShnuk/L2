...
<html >
<head>
<title>ASP.NET MVC 5 | Провайдер ролей</title>
<meta charset="utf-8" />
<meta name="description" content="Переопределение универсальных провайдеров. Провайдер ролей в ASP.NET MVC 5">
...
<h2>Создание провайдера ролей</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 30.11.2015</div>
...
<p>Универсальные провайдеры предоставляют уже готовый функционал авторизации. Но в то же время эти провайдеры обладают достаточной гибкостью - в частности 
мы можем их переопределить по своему усмотрению. При этом нам необязательно переопределять и использовать все четыре провайдера. Что довольно удобно, особенно в ситуации, 
когда нам не нужны все навороты ASP.NET Identity, а требуется построить очень простенькую систему авторизации.</p>
<p>Итак, создадим свой провайдер ролей. Для этого вначале создадим новый проект и добавим в него через NuGet Entity Framework.</p>
...
данных для доступа к ним. Поэтому добавим в папку Models следующие классы:</p>
<pre class="brush:c#;">
using System;
...
<p>Здесь определен класс пользователя и класс ролей. Класс пользователя содержит ссылку на выполняемую роль в системе. И также определен 
контекст UserContext, через который мы будем получать данные из БД. Контекст данных связывается с подключение к бд, которое определено в файле web.config. При желании можно определить любое другое подключение.</p>
<p>Кроме того, чтобы инициализировать базу данных начальными данными, через класс инициализатора UserDbInitializer мы добавляем пару ролей и одного 
пользователя в базу данных. И чтобы инициализатор сработал, добавим его вызов в файл global.asax:</p>
<pre class="brush:c#;">
...
<p>Теперь определим сам провайдер ролей. Для этого сначала добавим в проект папку <span class="b">Providers</span> и затем в него добавим новый класс CustomRoleProvider:</p>
<pre class="brush:c#;">
using System;
using System.Linq;
...
<p>В целях демонстрации переопределено три метода. Первый из них - GetRolesForUser позволяет получать набор ролей для определенного пользователя. 
Второй метод - - IsUserInRole - определяет, выполняет ли пользователь определенную роль в системе.</p>
<p>Чтобы использовать провайдер ролей в приложении, надо добавить его определение в файл конфигурации. Откроем файл <span class="b">web.config</span> 
и в пределах узла <code>system.web</code> добавим наш провайдер:</p>
<pre class="brush:xml;">
...
<p>И в конце добавим контроллер AccountController для логина и регистрации:</p>
<pre class="brush:c#;">
using System.Linq;
...
<p>В методе Register при добавлении нового пользователя мы ему будем присваивать в роль user. Модели, используемые методами Login и Register, выглядят так:</p>
<pre class="brush:c#;">
...
<p>И теперь мы можем разграничить доступ к методам различных контроллеров с помощью атрибута Authorize:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./12.11.php">Назад</a><a href="./">Содержание</a><a href="./12.15.php">Вперед</a></p></div></div>
<div class="socBlock">
