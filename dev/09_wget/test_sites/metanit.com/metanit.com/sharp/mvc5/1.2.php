...
<html >
<head>
<title>Начало работы с ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Начало работы с ASP.NET MVC 5, создание и запуск первого проекта, его параметры и структра, типы аутентификации проекта">
...
<h2>Начало работы с ASP.NET MVC 5</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 24.10.2018</div>
...
<p>Для создания веб-приложений на платформе ASP.NET MVC 5 необходима среда разработки - Visual Studio, которую можно найти по адресу <a href="https://www.visualstudio.com/ru/downloads/" rel="nofollow">Visual Studio 2017</a>.</p>
<p>После установки откроем Visual Studio 2017 и в меню <span class="bb">File (Файл)</span> выберем пункт <span class="b">New Project... (Создать проект)</span>. 
Перед нами откроется диалоговое окно создания проекта, где нам надо перейти на подвкладку Web и выбрать шаблон <span class="b">ASP.NET Web Application (.NET Framework)</span>:</p>
<img src="./pics/vs2017.png" alt="ASP.NET MVC 5 в Visual Studio 2017" /> 
<p>Дадим какое-нибудь имя проекту и нажмем ОК. После этого отобразится окно выбора шаблона нового приложения:</p>
<img src="./pics/1.3.png"  alt="Шаблоны проектов ASP.NET MVC 5" />
<p>По умолчанию уже выбран шаблон MVC.</p>
<p>Кроме того, данное диалоговое окно позволяет задать опции тестирования.</p>
<p>Также нам доступен в правой части окна выбор механизма аутентификации в приложении (кнопка <span class="bb">Change Authentication</span>). 
По умолчанию установлен тип <span class="b">No Authentication</span>, который подразумевает отсутствие какой-либо систмы аутентификации. 
Не будем его изменять. Но если мы нажмем на кнопку Change Authentication, 
то нам будет доступен выбор из следующих типов аутентификации:</p>
<img src="./pics/1.4.png" alt="Типы аутентификации в ASP.NET MVC 5" />
<p>Что они представляют?</p>
<ul>
<li><p><span class="b">No Authentication</span>: приложение не требует аутентификации пользователя</p></li>
<li><p><span class="b">Individual User Accounts</span>: требуется индивидуальная аутентификация, учетные записи пользователей хранятся 
в базе данных, кроме того, доступна аутентификация с помощью социальных сетей</p></li>
<li><p><span class="b">Work or School Accounts</span>: в основном предназначено для корпоративных приложений, которые используют Active Directory или Office 365</p></li>
<li><p><span class="b">Windows Authentication</span>: для управления учетными записями используется аутентификация Windows, так называемые intranet-приложения</p></li>
</ul>
 <p>Позже мы подробнее поговорим о механизмах аутентификации в приложении.</p>
 <p>Нажимаем кнопку OK, и создается новый проект. Он уже содержит разветвленную структуру и имеет некоторое наполнение по умолчанию. Запустим проект на выполнение, и 
 нам отобразится некоторый контент, который уже имеется по умолчанию в приложении:</p>
 <img src="./pics/1.5.png" alt="Первый проект на ASP NET MVC" />
 <h3>Структура проекта MVC 5</h3>
 <p>Весь этот функционал обеспечивается следующей структурой проекта:</p>
  <img src="./pics/1.6.png" alt="Стуктура проекта в ASP.NET MVC 5" />
 <p>Вкратце рассмотрим, для чего нужны все эти папки и файлы.</p>
<ul>
<li><p><span class="b">App_Data</span>: содержит файлы, ресурсы и базы данных, используемые приложением</p></li>
<li><p><span class="b">App_Start</span>: хранит ряд статических файлов, которые содержат логику инициализации 
 приложения при запуске</p></li>
<li><p><span class="b">Content</span>: содержит вспомогательные файлы, которые не включают код на c# или javascript, и которые развертываются вместе с приложением, например, 
файлы стилей css</p></li>
<li><p><span class="b">Controllers</span>: содежит файлы классов контроллеров. По умолчанию в эту папку добавляются два контроллера - HomeController и AccountController</p></li>
<li><p><span class="b">fonts</span>: хранит дополнительные файлы шрифтов, используемых приложением</p></li>
<li><p><span class="b">Models</span>: содержит файлы моделей. По умолчанию Visual Studio добавляет пару моделей, описывающих учетную запись и служащих для 
аутентификации пользователя</p></li>
<li><p><span class="b">Scripts</span>: каталог со скриптами и библиотеками на языке javascript</p></li>
<li><p><span class="b">Views</span>: здесь хранятся представления. Все представления группируются по папкам, каждая из которых соответствует одному контроллеру. 
После обработки запроса контроллер отправляет одно из этих представлений клиенту. 
Также здесь имеется каталог Shared, который содержит общие для всех представления</p></li>
<li><p><span class="b">Global.asax</span>: файл, запускающийся при старте приложения и выполняющий начальную инициализацию. Как правило, 
здесь срабатывают методы классов, определенных в папке App_Start</p></li>
<li><p><span class="b">packages.config</span>: файл, который содержит установленные в проект пакеты Nuget</p></li>
<li><p><span class="b">Web.config</span>: файл конфигурации приложения</p></li>
</ul>
<p>Конкретная структура каждого отдельного приложения, естественно, будет отличаться, а гибкость MVC позволяет изменять структуру, приспосабливая, 
ее к своим потребностям. Но описанные выше моменты будут общими для большинства проектов.</p>
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://www.youtube.com/watch?v=tgY2sBHeNDI"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 2. Начало работы. Первое приложение на ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
