...
<html >
<head>
<title>Поиск по таблице в JqGrid</title>
<meta charset="utf-8" />
...
<h2>Поиск по таблице в JqGrid</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Теперь добавим поиск по таблице. Для этого изменим представление из предыдущей темы следующим образом:</p>
<pre class="brush:xml;">
@{
    Layout = null;
}
...
<p>Во-первых, мы можем управлять визуальными свойствами элементов грида, устанавливая стили <code>.ui-jqgrid .ui-jqgrid-pager {height:50px;}</code>. 
В некоторых случаях это желательно делать, так как встроенные стили могут оказаться не совсем оптимальными. Элемент <code>.ui-jqgrid-pager</code> как раз представляет нижнюю панель 
с кнопками и пагинацией.</p>
<p>Для управления нижней панелью используется функция <code>$("#jqg").jqGrid('navGrid', '#jpager',...</code>. В ней задаются параметры нижней панели. 
...
<code>false</code>: <code>add: false</code></p>
<p>Нам нужны только кнопки поиска и обновления, поэтому для них задается значение true, и опционально мы можем задать и текст (значение текста 
также можно оставить пустым: <code>refreshtext: ""</code>)</p>
<p>Обновление в данном случае не обращается к серверу, а обновляет данные в гриде, которые могли подвергнуться сортировке.</p>
<p>И затем при нажатии на кнопку поиска отобразится окно, в котором по разным критериям мы сможем найти нужные объекты:</p>
<img src="./pics/17.9.png" alt="Поиск в JqGrid" />
<p>Окно поиска имеет набор полей с некоторыми значениями, которые при желание мы можем изменить. Для этого нам надо перейти к файлу <code>grid.locale-ru.js</code> и 
найти параметр search, который и задает все опции окна поиска:</p>
<pre class="brush:js;">
search : {
		caption: "Поиск...",
		Find: "Найти",
