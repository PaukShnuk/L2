...
<html >
<head>
<title>HTML-хелперы в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и использование html-хелперов (html helpers) в ASP NET MVC 5">
...
<h2>HTML-хелперы</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 22.09.2016</div>
...
<p>При отсутствии подобного хелпера, то нам бы пришлось по сути дублировать один и тот же html-код для создания списка. Однако этот хелпер еще довольно простой, 
...
случае его лучше вынести в отдельный файл кода. Для этого создадим в проекте папку под названием <span class="b">App_Code</span>. "App_Code" - это 
специальное зарезервированное имя для папки, которая содержит html-хелперы, поэтому мы должны использовать данное название.</p>
<p>Далее добавим в эту папку новый файл - представление под названием <span class="ii">MyHelpers.cshtml</span>. В этом файле определим код хелпера:</p>
<pre class="brush:xml;">
...
<p>И затем мы сможем применить этот хелпер в любом представлении в нашем проекте:</p>
<pre class="brush:xml;">
@MyHelpers.HtmlList(new string[] { "Лондон", "Париж", "Берлин" })
</pre>
<h3>Определение хелпера в классе C#</h3>
<p>Также для определения хелпера мы можем использовать стандартные классы на C#. Для этого нам надо создать новый класс с методом расширения - 
...
<ul>
<li><p>Свойство <span class="b">InnerHtml</span> позволяет установить или получить содержимое тега в виде строки</p></li>
<li><p>Метод <span class="b">MergeAttribute (string, string, bool)</span> позволяет добавить к элементу один атрибут. Для получения всех атрибутов можно использовать коллекцию 
<span class="bb">Attributes</span></p></li>
<li><p>Метод <span class="b">SetInnerText(string)</span> устанавливает текстовое содержимое внутри элемента</p></li>
<li><p>Метод <span class="b">AddCssClass(string)</span> добавляет класс css к элементу</p></li>
</ul>
...
<h3>Самозакрывающиеся элементы</h3>
<p>Ряд элементов html могут быть самозакрывающимися без парного закрывающего тега, например, элемент <code>&lt;img /&gt;</code>. Чтобы обозначить элемент как 
самозакрывающийся, мы можем применить значение <span class="b">TagRenderMode.SelfClosing</span>. К примеру, добавим в папку Helpers новый класс, 
который назовем <span class="b">ImageHelper</span>:</p>
<pre class="brush:c#;">
using System.Web.Mvc;
...
<p>Здесь метод <code>Image()</code> также представляет собой метод расширения для класса HtmlHelper, и кроме того, он принимает еще два параметра - для атрибутов <code>src</code> 
и <code>alt</code>.</p>
<p>Для создания объекта MvcHtmlString здесь применяется статический метод <code>MvcHtmlString.Create()</code>, в который также передается текстовое содержимое 
TagBuilder. Но так как элемент img самозакрывающийся, здесь также применяется <code>TagRenderMode.SelfClosing</code>.</p>
<p>Затем данный хелпер мы сможем также использовать в представлении, как и любой другой:</p>
<pre class="brush:xml;">
@using BookStore.Helpers
...
<p>Стандартные реализации хелперов позволяют добавить к создаваемым элементам атрибуты, например, атрибут class, id и другие. 
В кастомные хелперы мы также можем добавлять атрибуты. Например, изменим хелпер CreateList следующим образом:</p>
<pre class="brush:c#;">
...
<p>Теперь в качестве второго параметра передается объект htmlAttributes, свойства которого должны представлять собой передаваемые в хелпер атрибуты. 
С помощью метода <code>HtmlHelper.AnonymousObjectToHtmlAttributes()</code> данный объект преобразуется в атрибуты html, которые затем добавляются к элементу.</p>
<p>И в этом случае в коде представления мы можем передать в хелпер значения для атрибутов:</p>
<pre class="brush:xml;">
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/iMc8jiJcSu4"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 20. HTML helpers в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
