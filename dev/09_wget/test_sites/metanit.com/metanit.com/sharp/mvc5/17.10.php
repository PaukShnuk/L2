...
<html >
<head>
<title>ASP.NET MVC 5 | Локализация дат</title>
<meta charset="utf-8" />
<meta name="description" content="Локализация дат в ASP.NET MVC 5, валидация дат с помощью jquery, глобализация приложения">
...
<h2>Локализация дат</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Поскольку в разных регионах применяются разные форматы дат, то использование дат в приложение имеет определенные особенности. Например, пусть имеется следующая модель:</p>
<pre class="brush:c#;">
public class User
{
...
<p>В данном случае мы ориентированы на формат dd/MM/yyyy, как показывает атрибут <code>DisplayFormat</code>.</p>
<p>В представлении используются скрипты валидации и виджет jQuery DatePicker для выбора дат:</p>
<pre class="brush:xml;">
...
<p>В контроллере перед сохранением модель проходит валидацию:</p>
<pre class="brush:c#;">
[HttpPost]
...
<p>И при стандартном ходе событий несмотря на то, что мы введем обычную вроде дату, мы получим ошибку:</p>
<img src="./pics/localdate1.png" alt="Локализация дат в ASP.NET MVC 5" />
<p>Потому что стандартная библиотека валидации javascript, которую предоставляет Microsoft, ориентируется на формат mm/dd/yy, у нас же datepicker использует 
формат dd/mm/yy.</p>
<p>Чтобы решить проблему, надо переопределить функцию валидатора. Для этого после определения настроек datepicker переопределим функцию валидатора для выбора даты:</p>
<pre class="brush:xml;">
@section Scripts {
    @Scripts.Render("~/bundles/jqueryval")
    &lt;link href='@Url.Content("~/Content/themes/base/all.css")' rel="stylesheet" type="text/css" /&gt;
...
<p>Теперь для проверки используется функция <code>$.datepicker.parseDate</code>. Если виджет сможет распарсить дату, то возвращается true. Если же 
в процессе парсинга возникнет ошибка, то возвращается false.</p>
<p>После исправления при запуске проекта в Visual Studio все заработает. Но при размещении на зарубежном хостинге, например, на том же somee.com мы можем столкнуться с другой проблемой:</p>
<img src="./pics/localdate2.png" alt="Валидация дат в ASP.NET MVC" />
<p>Несмотря на то, что на локальной машине все работало, на хостинге не работает. Потому что текущая культура может отличаться, и в соответствии с 
текущей культурой может использоваться другой формат даты. В данном случае ошибка будет возникать при валидации на сервере.</p>
<p>Рещением в данной ситуации будет явное указание культуры с помощью элемента <span class="bb">globalization</span> в файле web.config в узле <i>system.web</i>:</p>
<pre class="brush:xml;">
&lt;system.web&gt;
    &lt;globalization culture="ru-RU" uiCulture="ru" /&gt;
	&lt;!-- остальное содержимое system.web--&gt;
</pre>
<p>Теперь на стороне сервера также будет использоваться дата в формате dd/MM/yyyy.</p>
...
<div class="nav"><p><a href="./17.9.php">Назад</a><a href="./">Содержание</a><a href="./18.1.php">Вперед</a></p></div></div>
<div class="socBlock">
