...
<html >
<head>
<title>Взаимодействие между модулями. События модулей</title>
<meta charset="utf-8" />
...
<h2>Взаимодействие между модулями. События модулей</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>При регистрации модулей все они попадают в коллекцию <span class="bb">Modules</span>, которая имеется у класса приложения HttpApplication. Получив один модуль 
из этой коллекции, мы можем с ним взаимодействовать - получать его поля и свойства и подписываться на события. Например, изменим модуль из предыдущей темы 
следующим образом:</p>
<pre class="brush:c#;">
using System;
...
<p>В отличие от старой версии модуля из прошлой темы здесь добавляется событие и осуществляется его вызов в обработчике события <code>EndRequest</code>.</p>
<p>Теперь добавим еще один модуль. Назовем его <i>TotalTimeModule</i>. Он будет рассчитывать количество запросов и общее время обработки запросов на основе 
данных из предыдущего модуля:</p>
<pre class="brush:c#;">
...
<p>Так как предыдущий модуль называется у нас Timer (как определено в web.config), то по этому имени мы можем получить данный модуль: <code>app.Modules["Timer"]</code>. 
И затем подписаться на его событие. Обработка события проста: изменяем значение переменных totalTime и requestCount.</p>
<p>Также добавим новый модуль в файл web.config:</p>
<pre class="brush:xml;">
&lt;system.webServer&gt;
  &lt;modules&gt;
...
<p>И поскольку второй модуль зависит от первого - берет его данные для подсчета своих полей, то модуль Total объявляется вторым. И теперь можем запустить 
приложение:</p>
...
<div class="nav"><p><a href="./19.3.php">Назад</a><a href="./">Содержание</a><a href="./19.5.php">Вперед</a></p></div></div>
<div class="socBlock">
