...
<html >
<head>
<title>Переопределение HTTP-обработчика</title>
<meta charset="utf-8" />
...
<h2>Переопределение HTTP-обработчика</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Ранее мы посмотрели, как можно назначить обработчик для обработки различных маршрутов. Но мы также можем переопределить базовый обработчик запроса при наличии некоторых 
условий.</p>
<p>С помощью модулей мы можем переопределить действия http-обработчика, не внося больших изменений в код приложения. При переопределении обработчика нам надо учитывать, 
что обработчик запроса выбирается при возникновении события <code>MapRequestHandler</code>. Поэтому переопределение обработчика должно происходить на более ранней стадии.</p>
<p>Итак, добавим в проект следующий модуль:</p>
<pre class="brush:c#;">
...
<p>Поскольку нам надо переопределить обработчик запроса до события <code>MapRequestHandler</code>, то мы это делаем при обработке другого события - <code>PostResolveRequestCache</code>, 
которое возникает до <code>MapRequestHandler</code>. Модуль сравнивает данные, полученные из контекста запроса, в частности, сравниваем контроллер и метод. 
И если окажется, что запрос предназначался к методу Index контроллера HomeController, то вызывается http-обработчик <code>UserInfoHandler</code>, 
который я взял из одной из предыдущих тем.</p>
<p>Для переопределения http-обработчика используется метод <code>RemapHandler()</code>, который имеется у объекта контекста <code>HttpContext</code>.</p>
<p>И после определения модуля зарегистрируем его в <i>web.config</i>:</p>
<pre class="brush:xml;">
&lt;system.webServer&gt;
    &lt;modules&gt;
...
<p>И если мы перейдем по запросу <i>Home/Index</i>, то данный запрос будет перехвачен модулем, и его обработчик будет изменен на UserInfoHandler.</p>
<p>Мы можем проконтролировать 
выбор обработчика, добавив в ответ клиенту название http-обработчика. Для этого добавим в метод <code>Init()</code> модуля обработчик события <code>EndRequest</code>:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./19.6.php">Назад</a><a href="./">Содержание</a><a href="./19.8.php">Вперед</a></p></div></div>
<div class="socBlock">
