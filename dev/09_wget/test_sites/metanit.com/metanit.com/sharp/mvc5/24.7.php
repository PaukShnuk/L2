...
<html >
<head>
<title>ASP.NET MVC 5 | Локализация чисел decimal</title>
<meta charset="utf-8" />
<meta name="description" content="Локализация чисел decimal в ASP.NET MVC, в атрибуте Range и использование в числах в качестве разделителя точки и запятой">
...
<h2>Локализация чисел decimal</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>При работе с данными типа decimal при использовании клиентской валидации через javascript у нас может возникнуть проблема: так как у нас 
русскоязычная культура, то в качестве разделителя дробной и целой частей в числах будет использоваться запятая, однако библиотеки валидации 
javascript при вводе дробного числа с запятой будут определять подобный ввод как ошибку. Например, у нас есть следующая модель:</p>
<pre class="brush:c#;">
public class Book
{
    [Required]
...
<p>И представляение для создания объекта данной модели:</p>
<pre class="brush:xml;">
...
<p>В секции Scripts подключаются все необходимые для валидации скрипты. Но при валидации на стороне клиента атрибут Range не будет 
должным образом срабатывать и любой ввод с запятой будет считаться ошибочным. Здесь есть два решения.</p>
<h3>Решение 1</h3>
<p>Чтобы числа с запятой нормально валидировались, нам надо изменить секцию скриптов в представлении следующим образом:</p>
<pre class="brush:js;">
@section Scripts {
    @Scripts.Render("~/bundles/jqueryval")
	&lt;script&gt;
...
<p>С помощью данного кода javascript мы переопределяем действие библиотеки jquery.validate.js. Поэтому данный код следует помещать после включения данной 
библиотеки.</p>
<h3>Решение 2</h3>
<p>Мы решили по каким-то причинам отказаться от запятой в качестве разделителя и хотим использовать точку. В этом случае нам надо явно задать 
англоязычную культуру для приложения. Для этого переходим к файлу <i>web.config</i> в нашем проекте и добавляем в секцию 
<span class="bb">system.web</span> следующее определение культуры:</p>
<pre class="brush:xml;">
&lt;globalization culture="en-US" uiCulture="en" /&gt;
</pre>
<p>Соответственно также нам надо будет в атрибуте Range устанавливать числа не с запятой, а с точкой: 
<code>[Range(typeof(decimal),"5.0", "100.6", ErrorMessage = "Наименьшая цена - 5 рублей"]</code></p>
...
<div class="nav"><p><a href="./24.6.php">Назад</a><a href="./">Содержание</a><a href="./24.8.php">Вперед</a></p></div></div>
<div class="socBlock">
