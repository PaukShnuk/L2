...
<html >
<head>
<title>Результаты действий в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Результаты действий в ASP.NET MVC 5, создание своего класса результата на основе ActionResult">
...
<h2>Результаты действий</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Когда пользователь обращается к ресурсу, как правило, он ожидает получить определенный ответ, например, в виде веб-страницы с некоторыми данными. 
На стороне сервера метод контроллера, получая параметры, обрабатывает их и формирует некоторый ответ в виде результата действия.</p>
<p>В прошлой теме в примере с вычислением площади треугольника мы возвращали html-код в виде строки. Но, как правило, возвращаемым результатом 
является объект класса, производного от <span class="bb">ActionResult</span>. 
ActionResult представляет собой абстрактный класс, в котором определен один метод <span class="bb">ExecuteResult</span>, переопределяемый в классах-наследниках:</p>
<pre class="brush:c#;">
...
<p>Создадим свои результаты действий. Они будут очень простыми. Возьмем какой-нибудь проект, например, из прошлой главы, и добавим в него новую папку 
Util, которая будет содержать новые классы. После добавления папки добавим в нее первый класс. Назовем его <span class="bb">HtmlResult</span>. Он у нас будет 
...
<p>В конструкторе класса HtmlResult получаем переданный html-код, а в методе Execute вставляем его в общее окружение, чтобы получилась полноценная html-страница, 
и пишем ее в выходной поток: <code>context.HttpContext.Response.Write(fullHtmlCode);</code></p>
<p>Чтобы использовать этот класс подключим в контроллер пространство имен нового класса: <code>using BookStore.Util;</code> и добавим новый метод:</p>
<pre class="brush:c#;">
...
<p>И обратившись к этому методу из браузера, например, <i>Home/GetHtml</i>, мы получим html-страничку. Хотя данный пример довольно примитивен, но в целом он демонстрирует, как работают классы 
результатов действий.</p>
<p>Создадим еще один класс результатов. Добавим в папку Util новый класс <span class="bb">ImageResult</span>:</p>
<pre class="brush:c#;">
using System.Web.Mvc;
...
<p>Данный класс не сложнее предыдущего и просто отдает изображение к html-коде. Тогда метод, использующий данный результат действий, мог бы выглядеть так:</p>
<pre class="brush:c#;">
...
<p>Здесь предполагается, что в проекте есть папка Images, в которой имеется изображение <code>visualstudio.png</code>. И тогда, 
если мы в браузере обратимся к этому действию, например, <i>Home/GetImage</i>, то сможем увидеть изображение.</p>
<h3>Встроенные классы, производные от ActionResult</h3>
<p>В реальности нам вряд ли потребуется часто создавать свои классы для обработки результата действия. Фрейморк <strong>ASP.NET MVC</strong> 
предлагает нам богатую палитру классов результатов действий, которые охватывают большинство, если не все возможные ситуации.</p>
<ul>
<li><p><span class="bb">ContentResult</span>: пишет указанный контент напрямую в ответ в виде строки, практически как предыдущие примеры</p>
<p>Так, следующий пример:</p>
<pre class="brush:c#;">
...
<p>Можно переписать с использованием ContentResult следующим образом:</p>
<pre class="brush:c#;">
...
<p>Даже если мы оставим в качестве возвращаемого результата тип string, то фреймворк увидит, что возвращаемый тип не является объектом 
ActionResult. И тогда автоматически создается объект ContentResult для возвращаемой строки.</p></li>
<li><p><span class="bb">EmptyResult</span>: по сути ничего не делает, отправляет пустой ответ</p></li>
<li><p><span class="bb">FileResult</span>: является базовым классом для всех объектов, пишущих бинарный ответ в выходной поток. Предназначен для отправки файлов</p></li>
<li><p><span class="bb">FileContentResult</span>: класс, производный от <code>FileResult</code>, пишет в ответ массив байтов</p></li>
<li><p><span class="bb">FilePathResult</span>: также производный от <code>FileResult</code> класс, пишет в ответ файл, находящийся по заданному пути</p></li>
<li><p><span class="bb">FileStreamResult</span>: класс, производный от <code>FileResult</code>, пишет бинарный поток в выходной ответ</p></li>
<li><p><span class="bb">HttpStatusCodeResult</span>: результат действия, который возвращает клиенту определенный статусный код HTTP</p></li>
<li><p><span class="bb">HttpUnauthorizedResult</span>: класс, производный от <code>HttpStatusCodeResult</code>. Возвращает клиенту ответ в 
...
<li><p><span class="bb">HttpNotFoundResult</span>: производный от <code>HttpStatusCodeResult</code>. Возвращает клиенту ответ в виде статусного кода HTTP 404, 
указывая, что запрошенный ресурс не найден</p></li>
<li><p><span class="bb">JavaScriptResult</span>: возвращает в ответ в качестве содержимого код JavaScript</p></li>
<li><p><span class="bb">JsonResult</span>: возвращает в качестве ответа объект или набор объектов в формате JSON</p></li>
<li><p><span class="bb">PartialViewResult</span>: производит рендеринг частичного представления в выходной поток</p></li>
<li><p><span class="bb">RedirectResult</span>: перенаправляет пользователя по другому адресу URL, возвращая статусный код 302 для временной переадресации или код 301 
для постоянной переадресации зависимости от того, установлен ли флаг <code>Permanent</code>.</p></li>
<li><p><span class="bb">RedirectToRouteResult</span>: класс работает подобно <code>RedirectResult</code>, но перенаправляет пользователя по 
определенному адресу URL, указанному через параметры маршрута</p></li>
<li><p><span class="bb">ViewResult</span>: производит рендеринг представления и отправляет результаты рендеринга в виде html-страницы клиенту</p></li>
</ul>
<p>Рассмотрим подробнее работу некоторых из этих классов.</p>
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/dHHFuQO0t7Q"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 10. Результаты действий в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
