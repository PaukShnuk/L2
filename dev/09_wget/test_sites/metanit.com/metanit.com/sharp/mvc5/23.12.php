...
<html >
<head>
<title>ASP.NET Identity в многоуровневой архитектуре в ASP.NET MVC 5. Часть 3</title>
<meta charset="utf-8" />
...
<h2>ASP.NET Identity в многоуровневой архитектуре. Уровень представления</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 11.11.2015</div>
...
<p>Вернемся к самому первому проекту, который был создан, - к проекту <span class="b">UserStore.WEB</span> и вначаеле добавим в него следующие пакеты Nuget:</p>
<pre>
...
<p>И также добавим в проект ссылку на проект BLL.</p>
<p>Данные пакеты позволят нам использовать аутентификацию AspNet Identity вместе с OWIN. И вначале в папку <i>App_Start</i> добавим класс <span class="b">Startup</span>:</p>
<pre class="brush:c#;">
...
<p>С помощью фабрики сервисов здесь создается сервис для работы с сервисами:</p>
<pre class="brush:c#;">IServiceCreator serviceCreator = new ServiceCreator();</pre>
<p>Потом сервис региструется контекстом OWIN:</p>
<pre class="brush:c#;">app.CreatePerOwinContext&lt;IUserService&gt;(CreateUserService);</pre>
<p>При этом здесь предполагается, что в файле web.config имеется строка подключения DefaultConnection, которая передается в метод <code>serviceCreator.CreateUserService("DefaultConnection")</code>.</p>
<p>Если бы создали бы проект с аутентификацией, то примерно также работала регистрация контекстом OWIN менеджера пользователей.</p>
<p>Но опять же нам необязательно использовать абстрактную фабрику для создания сервиса, мы также могли бы воспользоваться контейнерами внедрения зависимостей для 
установки всех зависимостей.</p>
<p>Далее добавим в папку Controllers новый контроллер AccountController:</p>
<pre class="brush:c#;">
...
<p>Поскольку ранее мы зарегитрировали сервис пользователей через контекст OWIN, то теперь мы можем получить этот сервис с помощью метода</p>
<pre class="brush:c#;">HttpContext.GetOwinContext().GetUserManager&lt;IUserService&gt;()</pre>
<p>В методе Login производится вход пользователя на сайт. Для этого используется модель LoginModel:</p>
<pre class="brush:c#;">
...
<p>Причем, чтобы гарантировать, что в базе данных будут начальные данные, в методе Login также запускается метод <code>SetInitialDataAsync()</code>.</p>
<p>Далее с помощью метода <code>UserService.Authenticate()</code> получаем объект ClaimsIdentity, который затем используется для создания аутентификационного тикета, сохраняемого в куках.</p>
<p>В методе Register для регистрации используется модель RegisterModel:</p>
<pre class="brush:c#;">
...
<p>В самом методе Register с помощью вызова <code>UserService.Create()</code> в базу данных будет добавляться новый пользователь.</p>
<p>Ну и естественнно для методов Login и Register должны быть определены представления, которые используют соответственно модели LoginModel и RegisterModel.</p>
<p>И после определения системы аутентификации мы можем использовать атрибут Authorize и роли для разграничения доступа.</p>
<p>В итоге проект будет выглядеть следующим образом:</p>
<img src="./pics/23.14.png" alt="ASP.NET Identity in Presentation Layer" />
<p>Ссылка на итоговый <a href="https://cloud.mail.ru/public/4pk6/EozhuDVxQ" rel="nofollow">проект</a></p>
...
<div class="nav"><p><a href="./23.11.php">Назад</a><a href="./">Содержание</a><a href="./24.1.php">Вперед</a></p></div></div>
<div class="socBlock">
