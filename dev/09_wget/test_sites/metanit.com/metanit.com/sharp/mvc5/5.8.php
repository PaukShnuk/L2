...
<html >
<head>
<title>Работа со сложными моделями в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<p>В предыдущей теме мы создали две модели Player и Team и вывели элементы из таблицы Players на страницу. Теперь посмотрим, как 
...
навигационные свойства, которые есть в сложной модели.</p>
<h3>Добавление модели</h3>
<p>При добавлении модели, имеющий внешний ключ, характерно все то же самое, что и для обычной модели. Единственное дополнение - нам надо также передавать в 
представление набор значений для связи внешнего ключа с другой таблицей. Итак, добавим в контроллер следующее действие Create:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>Первый метод обрабатывает GET-запрос и возвращает представление, передавая в него объект <span class="bb">SelectList</span> - 
список всех команд.</p> 
<p>Второй метод получает введенную пользователем в представлении модель и добавляет ее в БД. А теперь создадим представление <i>Create.cshtml</i>:</p>
<pre class="brush:xml;">
...
<img src="./pics/5.19.png" />
<h3>Редактирование модели</h3>
<p>Редактирование работает подобным способом. Определим в контроллере следующее действие Edit:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>Здесь также в виде объекта SelectList создается список команд, которые извлекаются из БД. И после получения запроса на редактирование 
