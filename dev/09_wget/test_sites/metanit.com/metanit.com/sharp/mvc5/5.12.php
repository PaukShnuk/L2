...
<html >
<head>
<title>Миграция баз данных</title>
<meta charset="utf-8" />
...
<h2>Миграция баз данных</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Нередко возникает ситуация, когда модель меняется. Например, мы решили внести в нее новые свойства. Но при этом у нас уже имеется существующая база данных, 
в которой есть какие-то данные. И чтобы без потерь обновить базу данных ASP.NET MVC предлагает нам такой механизм как миграции. Например, у нас есть простая модель User:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Соответсвенно есть контекст данных, через который мы работаем с бд:</p>
<pre class="brush:c#;">
...
<p>И допустим, у нас есть вся инфраструктура для работы с этой моделью - представления, контроллеры, и у нас есть уже в базе данных несколько объектов данной модели. 
Но в какой-то момент мы решили изменить модельную базу приложения. Например, мы добавили еще одно поле в модель User:</p>
<pre class="brush:c#;">
public class User
{
    public int Id { get; set; }
...
<p>Кроме того, мы решили добавить еще одну модель, например:</p>
<pre class="brush:c#;">
...
<p>Таким образом, контекст данных у нас уже меняется следующим образом:</p>
<pre class="brush:c#;">
public class UserContext : DbContext
{
    public UserContext() : 
...
<p>Мы можем добавить в представления для модели User дополнительное поле для свойства Age, можем создать контроллер и представления для новой модели, 
но при попытке добавить новый объект в бд, мы будем получать ошибку:</p>
<img src="./pics/5.23.png" />
<p>Контекст данных изменился, и теперь нам надо провести миграцию от старой схемы базы данных к новой. И первым делом найдем внизу Visual Studio 
окно Package Manager Console, введем в нем команду: <code>enable-migrations</code> и нажмем Enter:</p>
<img src="./pics/12.13.png" />
<p>После выполнения этой команды Visual Studio в проекте будет создана папка Migrations, в которой можно найти файл <i>Configuration.cs</i>. 
...
namespace MigrationApp.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration&lt;MigrationApp.Models.UserContext&gt;
    {
...
введем команду:</p>
<pre>PM&GT; Add-Migration "MigrateDB"</pre>
...
<p>В методе <code>Up</code> с помощью вызова метода CreateTable создается таблица "dbo.Companies" и производится ее настройка: создание столбцов, 
установка ключей. И также добавляется новый столбец Age в уже имеющуюся таблицу. Метод <code>Down</code> удаляет столбец и таблицу на случай, если они существуют. 
Фактически эти методы равнозначные выражению ALTER в языке SQL, которое меняет структуру базы данных и ее таблиц.</p>
...
<img src="./pics/5.24.png" />
<p>Итак, миграция выполнена, и мы можем уже использовать обновленные модели и контекст данных.</p>
