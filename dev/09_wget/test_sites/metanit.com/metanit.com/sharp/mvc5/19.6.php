...
<html >
<head>
<title>Создание асинхронных HTTP-обработчиков</title>
<meta charset="utf-8" />
...
<h2>Создание асинхронных HTTP-обработчиков</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>При обработке различных запросов есть смысл сделать обработчики асинхронными, например, когда речь идет о получении данных из бд или от внешнего ресурса.</p>
<p>Асинхронные обработчики отличаются от синхронных тем, что класс обработчика наследуется от класса <span class="bb">HttpTaskAsyncHandler</span>.</p>
<p>Итак, создадим новый класс UserInfoAsyncHandler:</p>
<pre class="brush:c#;">
...
<p>В данном случае мы получаем третий сегмент строки запроса, который должен содержать id нужной нам записи. И получив id, выводим из базы данных соответствующие ему данные.</p>
<p>Также зарегистрируем новый маршрут, запросы по которому будут обрабатываться данным обработчиком, в файле RouteConfig.cs:</p>
<pre class="brush:c#;">
...
<p>Новый маршрут будет соответствовать запросам типа <i>http://localhost:8989/user/1</i>, где последний сегмент будет извлекаться обработчиком и использоваться 
для получения информации из базы данных.</p>
...
<div class="nav"><p><a href="./19.5.php">Назад</a><a href="./">Содержание</a><a href="./19.7.php">Вперед</a></p></div></div>
<div class="socBlock">
