...
<html >
<head>
<title>Параметры кэширования в ASP.NET MVC</title>
<meta charset="utf-8" />
...
<h2>Параметры кэширования</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Используя параметры VaryByHeader, VaryByCustom и VaryByParam атрибута OutputCache, мы можем управлять версиями кэша и в зависимости от 
условий предоставлять одну или другую версию кэшированных данных. Применение вариантов кэширования повышает гибкость приложения.</p>
<h3>Дифференциация кэша по заголовкам</h3>
<p>При получении запроса мы получаем различные заголовки и можем определить вид заголовка, по которому будет идти дифференциация кэша. Например:</p>
<pre class="brush:c#;">
...
<p>И если мы обратимся к приложению одновременно из разных браузеров, которые будут посылать разные заголовки для параметра user-agent, 
то мы увидим разные результаты.</p>
<h3>Дифференциация кэша по параметрам запроса</h3>
<p>Параметр <code>VaryByParam</code> задает набор параметров, для которых будут генерироваться различные версии кэшированных данных:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
    static int x = 9;
...
<p>Если мы хотим генерировать различные версии кэша для нескольких параметров, то эти параметры указываются через точку с запятой. Также можно 
указать с помощью знака <code>*</code>, чтобы генерировались версии кэша для всех возможных наборов параметров.</p>
<p>В данном случае для различных значений параметров title и id будут создаваться разные версии кэшированных данных.</p>
<p>И если в двух разных браузерах мы, например, наберем в адресной строке <i>http://localhost:2000/Home/Index/title=mvc</i>, то они отобразят одно и то же 
значение из кэша.</p>
<h3>Дифференциация кэша по пользовательским параметрам</h3>
<p>С помощью свойства <code>VaryByCustom</code> мы можем сами определить значения, по которым будут генерироваться разные версии кэша. Для этого перейдем к файлу <i>Global.asax</i> и переопределим 
в классе <b>MvcApplication</b> метод <code>GetVaryByCustomString</code>:</p>
<pre class="brush:c#;">
...
<p>Метод прост: если переданный параметр имеет значение "browser", то возвращается название браузера, если нет, то идет обращение к вызову метода из родительского 
класса.</p>
<p>Теперь установим соответствующим образом атрибут OutputCache:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
    static int x = 9;
...
<p>Значение атрибута как раз и представляет то значение, которое в качестве параметра будет передаваться в метод <code>GetVaryByCustomString()</code>. В 
итоге при обращении к ресурсу из различных браузеров мы будем получать различные версии кэша.</p>
