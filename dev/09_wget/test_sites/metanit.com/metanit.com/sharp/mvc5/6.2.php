...
<html >
<head>
<title>Работа с маршрутами в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<P>Для создания маршрутов можно использовать метод <span class="bb">MapRoute</span>. Например:</P>
<pre class="brush:c#;">
...
<p>Либо можно сначала создать объект Route, а потом добавить его в коллекцию маршрутов RouteCollection. Определим два маршрута:</p>
<pre class="brush:c#;">
public class RouteConfig
{
    public static void RegisterRoutes(RouteCollection routes)
...
<code>newRoute</code> сопоставляется с запросами, имеющими только два сегмента. Так, вызов <span class="ii">http://localhost:5555/Home/Index/1</span> 
...
как необязательный: <code>routes.MapRoute(name: "Default", url: "{controller}/{action}/{id}", defaults: new { id = UrlParameter.Optional });</code>.</p>
<p>При передаче значений по умолчанию для параметров важно учитывать позицию параметра. Движок маршрутизации использует значение по умолчанию 
только в том случае, если все последующие параметры также имеют значения по умолчанию. Так, если мы зададим следующий маршрут: 
...
<p>Мы посмотрели, как сопоставлять маршруты с контроллерами и их методами. Однако мы можем адресовать запросы также и отдельным файлам сайта, 
например, статическим html-страницам. Работа механизма маршрутизации такова, что сначала он смотрит, совпадает ли запрос с определенным файлом, 
хранящимся на сервере, и если такого файла не находит, тогда он начинает сопоставлять запрос с определенными маршрутами.</p>
...
<p>Для тестирования создадим какой-нибудь контент на этой странице. И после ее добавления в проект мы можем к ней обращаться напрямую через 
запрос <span class="ii">Content/Welcome.html</span>.</p>
<h4>Использование префиксов в строке запроса</h4>
<p>Сегменты строки запроса необязательно должны нести только значения для параметров, определенных в маршруте. Можно также использовать различные 
...
<pre class="brush:c#;">routes.MapRoute(name: "Default", url: "Ru/{controller}/{action}/{id}", defaults: new { id = UrlParameter.Optional }</pre>
<p>Кроме того, можно добавлять префиксы не в качестве отдельного сегмента, а к самому сегменту:</p>
<pre class="brush:c#;">routes.MapRoute(name: "Default", url: "Ru{controller}/{action}/{id}", defaults: new { id = UrlParameter.Optional }</pre>
...
<p>При отправлении приложению запроса типа <span class="ii">http://localhost:49326/RuHome/Index</span>, если у нас в приложении не определен контроллер <span class="bb">RuHomeController</span>, 
приложение вернет ошибку. Почему? Потому система маршрутизации пытается сопоставить запрос сначала с первым маршрутом. Если запрос не соответствует 
...
<p>Также мы можем использовать префиксы в качестве псевдонимов для контроллера и его метода:</p>
<pre class="brush:c#;">routes.MapRoute( name: "Default2", url: "Store/Buy", defaults: new { controller = "Home", action="Index" } );</pre>
<p>Здесь для контроллера HomeController используется псевдоним <code>Store</code>, а для действия Index - псевдоним <code>Buy</code>. В итоге данный маршрут будет 
...
<p>Чтобы получить переданные значения для параметров маршрута, мы можем воспользоваться объектом <b>RouteData</b>. Например, 
если у нас определен стандартный маршрут <code>routes.MapRoute(name: "Default", url: "{controller}/{action}/{id}", defaults: new { id = UrlParameter.Optional });</code>, 
...
<p>Значения, передаваемые в качестве дополнительных значений, например, для параметра <code>id</code>, можно извлекать из самого параметра метода:</p>
<pre class="brush:c#;">
...
<h4>Передача произвольного количества параметров в запросе</h4>
