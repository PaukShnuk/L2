...
<html >
<head>
<title>Подтверждение телефона по SMS в ASP.NET Identity 2.0</title>
<meta charset="utf-8" />
...
<h2>Подтверждение телефона по SMS в ASP.NET Identity 2.0</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 02.12.2015</div>
...
<p>ASP.NET MVC 5 поддерживает также подтверждение регистрации по sms. Но чтобы включить данную функциональность в приложении, нам надо связать 
наше приложение с сервисом, который будет отправлять sms через специальный API. Одним из таких сервисов является <a href="https://www.twilio.com/" rel="nofollow">Twilio</a></p>
<p>Итак, создадим проект и первым делом подключим через NuGet пакет, который позволит взаимодействовать с этим сервисом:</p>
<img src="./pics/12.19.png" alt="Twilio в ASP.NET MVC 5" />
<p>Теперь надо зарегистрироваться на самом этом сервисе и получить идентификатор аккаунта ACCOUNT SID, токен AUTH TOKEN и привязанный к приложению номер телефона, 
с которого будут отправляться sms.</p>
<p>Значения ACCOUNT SID и AUTH TOKEN можно найти в личном кабинете:</p>
<img src="./pics/12.20.png" />
<p>Через пункт меню Numbers можно увидеть номер телефона, привязанный к аккаунту:</p>
<img src="./pics/12.21.png" />
<p>Также нам потребуется подключить все те страны, с телефонными кодами которых мы будем взаимодействовать. Это можно сделать в личном кабинете 
в меню Geographic Permissions на вкладке Messaging. Здесь нам надо отметить все необходимые страны:</p>
<img src="./pics/12.33.png" />
<p>После создания учетной записи в Twilio первым делом подключим этот сервис к себе в приложение. Для этого перейдем в файл <i>IdentityConfig.cs</i>, 
который располагается в папке <span class="bb">App_Start</span>. В нем уже приготовлен специальный класс для отправки sms, но пока он по сути ничего не делает:</p>
<pre class="brush:c#;">
...
<p>Там же в файле в методе Create происходит установка этого сервиса для менеджера учетных записей: <code>manager.SmsService = new SmsService();</code></p>
<p>И теперь подключим в начало файла функциональность twilio: <code>using Twilio;</code> и изменим код сервиса следующим образом:</p>
<pre class="brush:c#;">
public class SmsService : IIdentityMessageService
{
    public Task SendAsync(IdentityMessage message)
...
<p>Переменная <span class="bb">AccountSid</span> будет хранить идентификатор вашего аккаунта ACCOUNT SID, переменная <span class="b">AuthToken</span> 
содержит токен AUTH TOKEN, а <span class="bb">twilioPhoneNumber</span> - номер телефона, привязанного к вашему аккаунту. В данном случае все эти значения 
из моего аккаунта, для вашего аккаунта Twilio они будут отличаться.</p>

<p>Теперь надо создать инфраструктуру для отправки sms и верификации полученных через sms кодов.</p>
<p>Модель пользователя <span class="b">ApplicationUser</span> уже имеет свойство <code>PhoneNumber</code>, которое представляет телефон пользователя, а 
также свойство <code>PhoneNumberConfirmed</code>, указывающее, был ли подтвержден номер телефона. Однако напрямую с этими свойствами при подтверждении мы не будем 
...
для работы. Во-первых, метод AddPhoneNumber, который принимает номер телефона и отправляет на него код:</p>
<pre class="brush:c#;">
...
<p>Во-вторых, это метод <code>VerifyPhoneNumber</code>, который верифицирует код:</p>
<pre class="brush:c#;">
...
<p>Метод <code>AddPhoneNumber</code> в GET-версии возвращает представление, которое предназначено для ввода телефона. А POST-версия метода принимает 
введенный номер в виде модели AddPhoneNumberViewModel и на основе этого номера генерирует код: 
<code>var code = await UserManager.GenerateChangePhoneNumberTokenAsync(User.Identity.GetUserId(), model.Number);</code></p>
<p>Затем формируется сообщение: <code>var message = new IdentityMessage { Destination = model.Number, Body = "Your security code is: " + code};</code>. 
Это сообщение отправляется <code>await UserManager.SmsService.SendAsync(message);</code>, а фактически передается вышеопределенному классу 
<b>SmsService</b> для непосредственной отправки.</p>
<p>После этого пользователь перенаправляется на страницу ввода кода, а на телефон пользователя приходит код. После ввода кода данные обрабатываются post-методом 
VerifyPhoneNumber, в котором с помощью вызова <code>var result = await UserManager.ChangePhoneNumberAsync(User.Identity.GetUserId(), model.PhoneNumber, model.Code);</code> 
введенный код сравнивается с токеном из бд. Если сравнение прошло успешно, то <code>result.Succeeded</code> возвратит значение <code>true</code></p>
<p>Для данных методов в файле <i>ManageViewModels.cs</i> уже определены все необходимые модели:</p>
<pre class="brush:c#;">
...
<p>А в папке <i>Views/Manage</i> уже имеются представления <i>AddPhoneNumber.cshtml</i> и <i>VerufyPhoneNumber.cs</i>.</p>
<p>Запустим приложение, зарегистрируемся и перейдем к добавлению номера по ссылке Manage/AddPhoneNumber:</p>
<img src="./pics/12.22.png" />
<p>Номер вводится с плюсиком, например, +79262221133.</p>
<p>После ввода номера мы получим смс на телефон следующего вида:</p>
<img src="./pics/12.23.png" />
<p>Введем полученный код:</p>
<img src="./pics/12.24.png" />
<p>И после успешной проверки нам отобразится соответствующая страница:</p>
<img src="./pics/12.32.png" />
<p>И все, мы прошли проверку. Теперь в базе данных для данного пользователя в поле <code>PhoneNumber</code> будет значится номер телефона, и в программе 
мы его сможем получить и использовать: <code>user.PhoneNumber</code></p>
...
<div class="nav"><p><a href="./12.8.php">Назад</a><a href="./">Содержание</a><a href="./12.10.php">Вперед</a></p></div></div>
<div class="socBlock">
