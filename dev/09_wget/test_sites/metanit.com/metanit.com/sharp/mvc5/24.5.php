...
<html >
<head>
<title>ASP.NET MVC 5 | Генерация sitemap.xml</title>
<meta charset="utf-8" />
<meta name="description" content="Генерация файла sitemap.xml в ASP.NET MVC 5">
...
<h2>Генерация sitemap.xml</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 02.01.2017</div>
...
<p>Рассмотрим механизм создания карты сайта. Вряд ли здесь можно предложить какое-то универсальное решение, все зависит непосредственно от веб-приложения, 
от того, как в нем устроена система маршрутов и ссылок, но в принципе можно в некоторой степени автоматизировать процесс создания файла sitemap.xml.</p>
<p>Допустим, у нас есть простой контроллер HomeController:</p>
<pre class="brush:c#;">
using System.Text;
...
<p>Здесь у нас четыре метода, кроме того, стоит отметить, что метод Product получает элементы из бд по id. Таким образом, в базе данных еще могут храниться множество 
элементов, для каждого из которых должен формироваться свой путь типа "Home/Product/5".</p>
<p>Теперь добавим в проект новый класс Generator, который будет генерировать файл <span class="ii">sitemap.xml</span>:</p>
<pre class="brush:c#;">
...
<p>Для упрощения создания ссылки по маршруту и его параметрам здесь определен метод расширения <span class="b">AbsoluteRouteUrl</span>, 
который принимает название маршрута и его параметры.</p>
<p>Метод <code>GetSitemapNodes()</code> класса Generator генерирует все ссылки по методам контроллера. Для получения всех id товаров из базы данных применяется 
вспомогательный сервис. Для тестирования можно определить следующий класс:</p>
<pre class="brush:c#;">
...
<p>При варианте, когда все ссылки создаются вручную или программно, но при этом сохраняются в базе данных, задача сильно облегчается, так как в этом случае 
достаточно получить все эти ссылки из бд.</p>
<p>Метод <code>GetSitemapDocument()</code> создает собственно карту сайта.</p>
<p>Теперь добавим в HomeController метод, который будет отдавать сгенерированную карту сайта:</p>
<pre class="brush:c#;">
...
<p>Далее в файле RouteConfig.cs определим дополнительный маршрут для sitemap.xml:</p>
<pre class="brush:c#;">
routes.MapRoute(
    name: "sitemap",
    url: "sitemap.xml",
...
<p>Кроме того, добавим в файл конфигурации <span class="ii">web.config</span> дополнительный узел, который разрешит обработку системой маршрутизации 
запросов к sitemap.xml:</p>
<pre class="brush:xml;">
&lt;system.webServer&gt;
    &lt;handlers&gt;
...
<p>И теперь при обращении по пути sitemap.xml мы получим сгенерированную карту:</p>
<img src="./pics/sitemap.png" alt="Sitemap.xml в ASP.NET MVC 5" />
...
<div class="nav"><p><a href="./24.4.php">Назад</a><a href="./">Содержание</a><a href="./24.6.php">Вперед</a></p></div></div>
<div class="socBlock">
