...
<html >
<head>
<title>Глобальные фильтры и переопределение фильтров</title>
<meta charset="utf-8" />
...
<h2>Глобальные фильтры и переопределение фильтров</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Мы посмотрели на различные виды фильтров. Как вы видели, мы можем применить фильтры как к отдельным действиям, так и ко всему контроллеру - то есть 
ко всем его действиям. Однако если мы хотим применить фильтр ко всем контроллерам, а контроллеров у нас много? В данном случае в mvc 
реализована функциональность глобальных фильтров. Эти фильтры применяются ко всем действиям всех контроллеров.</p> 
<p>Откроем файл <span class="ii">FilterConfig.cs</span>, который находится в папке <span class="ii">App_Start</span>. В этом файле определен класс <span class="bb">FilterConfig</span>, 
который в методе <span class="bb">RegistreGlobalFilters</span> осуществляет регистрацию глобальных фильтров в коллекцию filters. Эта 
коллекция представляет собой объект <code>GlobalFilterCollection</code>, который передается в качестве параметра при вызове метода в файле 
Global.asax.</p>
<p>Итак, мы можем зарегистрировать глобальные фильтры так:</p>
<pre class="brush:c#;">
...
<h3>Переопределение фильтров</h3>
<p>Одним из нововведений MVC 5 стала такая функциональность как переопределение фильтров. Чтобы понять ее суть, рассмотрим следующую ситуацию. 
Например, у нас есть контроллер с ограничением доступа:</p>
<pre class="brush:c#;">
...
<p>Атрибут авторизации на уровне контроллера <code>[Authorize(Users = "eugene")]</code> ограничивает доступ к методам контроллера: в данном случае 
они доступны только пользователю с логином eugene. И очевидно, что второй атрибут авторизации для метода Test работать не будет.</p>
<p>Однако такая возможность была бы очень плезна, особенно при использовании глобальных фильтров, когда нам надо открыть или переопределить доступ. 
Проблема касается не только фильтров авторизации, но и любых других.</p>
<p>И чтобы решить данную проблему в MVC 5 была введена такая функциональность, как переопределение фильтров. Она подразумевает реализацию 
интерфейса <b>IOverrideFilter</b>:</p>
<pre class="brush:c#;">
...
<p>Единственное свойство интефрейса позволяет указать тип фильтров, которые надо переопределить. Итак, создадим новый класс, реализующий данный интерфейс:</p>
<pre class="brush:c#;">
...
<p>Так как нам надо переопределить поведение фильтров авторизации, то в качестве типа указывается IAuthorizationFilter. И теперь мы можем переписать 
контроллер следующим образом:</p>
<pre class="brush:c#;">
[Authorize(Users = "eugene")]
public class HomeController : Controller
{
...
<p>Таким образом, фильтр авторизации <code>[Authorize(Users = "eugene")]</code> будет применяться ко всем действиям контроллера, кроме метода Test.</p>
...
<div class="nav"><p><a href="./8.8.php">Назад</a><a href="./">Содержание</a><a href="./8.7.php">Вперед</a></p></div></div>
<div class="socBlock">
