...
<html >
<head>
<title>DefaultModelBinder в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Привязка DefaultModelBinder в ASP.NET MVC 5">
...
<h2>DefaultModelBinder</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Привязчик <span class="bb">DefaultModelBinder</span> используется по умолчанию, если для данного типа не определен другой привязчик. Чтобы получить значения 
для параметров, привязчик просматривает следующие объекты строго по порядку:</p>
<ol>
<li><p><code>Request.Form</code>. В данном случае привязчик получает значения, отправленные с помощью форм</p></li>
<li><p><code>RouteData.Values</code>. Здесь получает значения с помощью маршрутов приложения</p></li>
<li><p><code>Request.QueryString</code>. В данном случае привязчик извлекает значения из строки запроса</p></li>
<li><p><code>Request.Files</code>. В данном случае используются загруженные на сервер файлы</p></li>
</ol>
<p>Если в одном из этих объектов будет обнаружен соответствующий элемент, то на нем поиск прекращается, а параметру передается найденное значение. 
Например, привязчик ищет значения для параметров следующего действия:</p>
<pre class="brush:c#;">
...
<p>Чтобы DefaultModelBinder мог связать значение с параметром, элемент данных запроса должен обязательно иметь то же имя, что и параметр. То 
есть в данном случае они оба должны иметь имя <code>id</code>.</p>
<p>Итак, привязчик DefaultModelBinder просматривает в поиске значения для параметра id следующие пути:</p>
<ol>
<li><p><code>Request.Form["id"]</code></p></li>
<li><p><code>RouteData.Values["id"]</code></p></li>
<li><p><code>Request.QueryString["id"]</code></p></li>
<li><p><code>Request.Files["id"]</code></p></li>
</ol>
<p>При привязке простых типов DefaultModelBinder преобразует строковое выражение параметра из данных запроса к типу параметра с помощью 
класса <code>System.ComponentModel.TypeDescriptor</code>. Если же значение нельзя преобразовать, то привязка оканчивается неудачей. Поэтому 
есть смысл использовать параметры, допускающие значение <code>null</code>, а в метод действия вкладывать соответствующую логику по проверке значения:</p>
<pre class="brush:c#;">
public ActionResult Edit(int? id)
{
    // ...
...
<p>При получении значений для сложных типов, например, для той же модели Book, <code>DefaultModelBinder</code> с помощью рефлексии 
просматривает объект и находит все свойства объекта, которые могут использоваться при привязке. Например, <code>DefaultModelBinder</code> видит, 
что объект <code>Book</code> имеет свойство <code>Author</code>, то привязчик будет искать в запросе параметр "Author".</p>
<p>Если свойство представляет простой тип, то для него используется, как было указано выше преобразование с помощью класса 
класса <code>System.ComponentModel.TypeDescriptor</code>. Если же свойство представляет сложный тип - то опять повторяется рефлексия и весь цикл.</p>
<h3>Выборочная привязка</h3>
<p>Иногда возникает возможность исключить некоторые свойства из привязки модели. Мы можем это сделать с помощью атрибута <code>Bind</code>.</p>
<p>Для включения только определенных свойств мы можем использовать свойство <code>Include</code> данного атрибута:</p>
<pre class="brush:c#;">
...
<p>В данном случае мы указываем, что в привязке будут участвовать только свойства <code>Name</code> и <code>Author</code>. Остальные же свойства 
из привязки исключаются.</p>
<p>Либо мы можем использовать свойство <code>Exclude</code> атрибута Bind, чтобы исключить свойство из привязки:</p>
<pre class="brush:c#;">
...
<p>В данном случае выборочная привязка применена к методу Create. Что если нам надо осуществить выборочную привязку глобально во всем приложении? 
Тогда мы можем применить атрибут непосредственно к модели, и в этом случае атрибут будет применен по умолчанию ко всем методам действий контроллеров проекта<p>
<pre class="brush:c#;">
	[Bind (Exclude="Year")]
    public class Book
    {
