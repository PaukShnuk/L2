...
<html >
<head>
<title>Атрибуты валидации в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Атрибуты валидации</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Рассмотрим атрибуты, которые используются для валидации модели в приложении на ASP.NET MVC.</p>
...
<p>Чтобы при валидации мы не получали несуразных сообщений об ошибке, этот атрибут позволяет настроить текст сообщения:</p>
<pre class="brush:c#;">
public class Book
{
...
<img src="./pics/7.7.png" alt="Настройка сообщений об ошибке в ASP.NET MVC 5" />
<p>А если мы явным образом не установим текст сообщения, то при выводе ошибки будет отображаться стандартный текст сообщения.</p>
...
<img src="./pics/7.9.png" alt="Атрибут RegularExpression в Asp.net mvc 5" />
<h3>Атрибут StringLength</h3>
<p>Чтобы пользователь не мог ввести очень длинный текст, используется атрибут <code>StringLength</code>. 
Особенно это актуально, если в базе данных установлено ограничение на размер хранящихся строк.</p>
<p>Первым параметром идет максимальная допустимая длина строки. 
Именованные параметры, в частности <code>MinimumLength</code> и <code>ErrorMessage</code>, позволяют задать дополнительные опции отображения.</p>
<pre class="brush:c#;">
public class Book
{
    [HiddenInput(DisplayValue = false)]
...
<p>Атрибут <code>Range</code> может работать как с целочисленными значениями, так и с числами с плавающей точкой. 
А еще одна перегруженная версия его конструктора принимает параметр Type и две строки (которые позволяют создать диапазон возрастов).</p>
<pre class="brush:c#;">
[Range(typeof(decimal), "0.00", "49.99")]
public decimal Price { get; set; }
</pre>
<h3>Атрибут Compare</h3>
<p>Атрибут Compare гарантирует, что два свойства объекта 
модели имеют одно и то же значение. Если, например, надо, чтобы пользователь ввел пароль дважды:</p>
<pre class="brush:c#;">
...
<p>Таким образом, используя атрибуты валидации модели, мы можем управлять валидацией и заключать несложную логику проверки значений свойств уже в атрибуты свойств, 
не прибегая к коду.</p>
<h3>Атрибут Remote</h3>
<p>Атрибут Remote в отличие от других классов атрибутов находится в пространстве имен <b>System.Web.Mvc</b>. Он позволяет выполнять валидацию на стороне клиента с обратными вызовами на сервер.</p>
<p>Например, название книги, представленое свойством <code>Name</code>, не должно быть равно определенному значению. 
При валидации на стороне клиента трудно гарантировать соблюдение ряда проверок, особенно если мы задействуем базы данных. 
А с помощью атрибута Remote мы можем послать значение свойства <code>Name</code> на сервер, а там уже проверить.</p>
<pre class="brush:c#;">
public class Book
{
    [Required]
...
<p>В атрибуте можно установить имя действия и имя контроллера, которые должны вызываться кодом на стороне клиента, а также еще ряд именованных параметров, 
в частности, сообщение об ошибке с помощью параметра <code>ErrorMessage</code>. Клиентский код посылает 
введенное пользователем значение для свойства <code>Name</code> автоматически, а перегруженный конструктор атрибута позволяет указать 
дополнительные поля, значения которых надо посылать на сервер.</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>Это действие контроллера принимает в качестве параметра имя свойства, подлежащего валидации, и возвращает true или false в форме объекта 
в формате JSON. При этом если возвращается false, то мы увидим сообщение об ошибке, заданное парааметром ErrorMessage.</p>
<img src="./pics/remote.png" alt="Атрибут Remote в ASP.NET MVC5" />
