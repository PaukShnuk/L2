...
<html >
<head>
<title>Создание привязчика модели в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Создание привязчика модели</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Чтобы создать привязчик модели, нам надо реализовать интерфейс <span class="bb">IModelBinder</span>:</p>
<pre class="brush:c#;">
...
<p>Данный интерфейс содержит единственный метод <code>BindModel</code>, который мы и реализуем в новом классе.</p>
<p>Сначала мы получаем поставщик значений, а затем с его помощью получаем значения для отдельных свойств будущего 
объекта <code>Book</code>. Чтобы корректным образом получить значение, мы преобразуем его к надлежащему типу: <code>int id = (int)valueProvider.GetValue("Id").ConvertTo(typeof(int));</code>. 
И в конце возвращаем созданный объект <code>Book</code>.</p>
<p>Теперь нам надо зарегистрировать привязчик модели. Это можно сделать двумя способами. Первый способ - регистрация в файле Global.asax:</p>
<pre class="brush:c#;">
...
<p>Альтернативный способ регистрации привязчика - использование соответствующего атрибута для модели:</p>
<pre class="brush:c#;">
...
<p>Теперь для модели Book будет использоваться привязчик BookModelBinder. Ну и в любом методе, который принимает в качестве параметра 
объект Book, можно протестировать привязчик:</p>
<pre class="brush:c#;">
[HttpPost]
