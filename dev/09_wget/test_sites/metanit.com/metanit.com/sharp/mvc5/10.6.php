...
<html >
<head>
<title>Формат JSON в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Формат JSON</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Как уже писалось в начале главы, технология AJAX все больше вместо передачи данных в формате xml использует формат JSON (JavaScript Object Notation). 
JSON не зависит от языка программирования, он более удобен и легче обрабатывается. 
Например, объект Book в формате JSON мог бы выглядеть бы так: <code>{"Name":"Война и мир", "Author":"Л. Толстой", "Year":"1863"}</code>.</p>
<p>В JSON каждый отдельный объект заключается в фигурные скобки и представляет собой набор пар ключ-значение, разделенных запятыми, где ключом является название свойства 
объекта, а значением соответственно значение этого свойства.</p>
<p>Ранее мы в ajax-форме обращались к методу, который возвращал данные в виде обычной разметки html. Теперь изменим его или определим новый, 
чтобы он возвращал данные в формате JSON:</p>
<pre class="brush:c#;">
...
<p>Новое действие теперь возвращает объект <span class="bb">JsonResult</span>, который принимает объект с результатами запроса (в данном случае объект jsondata). 
Второй необязательный параметр представляет значение перечисления <code>JsonRequestBehavior</code> и может принимать два значения: 
<code>AllowGet</code> (разрешить Get-запросы) и <code>DenyGet</code> (запретить Get-запросы). В данном случае мы разрешаем действию посылать результаты 
в JSON-формате в ответ на запросы Get.</p>
<p>Теперь изменим ajax-форму в нашем представлении:</p>
<pre class="brush:xml;">
...
<p>Поскольку параметр <code>UpdateTargetId</code> не будет приниматься во внимание при получении данных в формате JSON, поэтому вся логика 
по выводу данных на страницу закладывается в обратный вызов <code>OnSuccess</code>. В обработчике <code>OnSuccess</code> мы последовательно 
добавляем полученные данные в нужный элемент html-страницы.</p>
