...
<html >
<head>
<title>ASP.NET MVC 5 | Приложение на Angular</title>
<meta charset="utf-8" />
<meta name="description" content="Создание приложения на Angular в ASP.NET MVC 5 с помощью webpack">
...
<h2>Приложение на Angular</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.05.2019</div>
...
<p>В данной статье рассмотрим подключение Angular 8 в ASP.NET MVC 5. Более подробно про Angular 2/8 можно почитать в соответствующем руководстве - 
<a href="https://metanit.com/web/angular2/">https://metanit.com/web/angular2/</a>. В данном же случае рассмотрим только те аспекты, которые характерны 
именно для ASP NET MVC.</p>
<p>Для работы с Angular 2+ в ASP.NET MVC 5 предварительно необходимо установить TypeScript, а также Node.JS и NPM.</p>
<p>Создадим новый проект на ASP NET MVC 5. Пусть он называется AngularApp:</p>
<img src="./pics/angular1.png" alt="Angular 2 в ASP.NET MVC 5" />
<p>Далее добавим в корень проекта новый элемент <span class="b">npm Configuration File</span>, который назовем <span class="b">package.json</span>:</p>
<img src="./pics/angular2.png" alt="npm в ASP.NET MVC 5" />
...
<p>Нажмем правой кнопкой мыши на package.json и в появившемся меню выберем пункт "Restore Packages":</p>
<img src="./pics/angular3.png" alt="node package.json в ASP.NET MVC 5" />
<p>После этого в проект будет добавлен каталог <span class="ii">node_modules</span>, который будет содержать все необходимые пакеты.</p>
<p>Далее добавим в корень проекта файл конфигурации TypeScript - элемент <span class="b">TypeScript JSON Configuration File</span>, который назовем 
<span class="ii">tsconfig.json</span>:</p>
<img src="./pics/angular5.png" alt="Добавление TypeScript в ASP.NET MVC 5" />
<p>Определим в нем следующее содержимое:</p>
<pre class="brush:js;">
{
  "compilerOptions": {
...
<p>Теперь собственно определим файлы приложения на Angular. Для этого добавим в проект новую папку <span class="b">src</span>. И затем 
в ней создадим каталог <span class="b">app</span>. В папке <span class="ii">src/app</span> определим новый файл на языке TypeScript, 
который назовем <span class="b">app.component.ts</span>:</p>
<pre class="brush:ts;">
...
<p>Также добавим в папку app новый файл <span class="b">app.module.ts</span>:</p>
<pre class="brush:ts;">
...
<p>Далее уровнем выше - в папку <span class="ii">src</span> добавим файл <span class="b">main.ts</span>:</p>
<pre class="brush:ts;">
...
<p>И в эту же папку <span class="ii">src</span> добавим файл <span class="b">polyfills.ts</span>:</p>
<pre class="brush:ts;">
import 'core-js';
import 'zone.js/dist/zone';
</pre>
<p>В конце добавим в корень проекта новый файл javascript, который назовем <span class="b">webpack.config.js</span> и в котором определим следующее содержимое:</p>
<pre class="brush:js;">
...
<p>Данный файл определяет конфгурацию сборщика webpack, который позволяет сгенерировать сборки на языке javascript из файлов TypeScript.</p>
<p>В итоге весь проект будет выглядеть так:</p>
<img src="./pics/angular6.png" alt="Применяем Angular 8 в ASP.NET" />
<p>С помощью команды <code>run build</code>, которая с помощью сборщика webpack сгенерирует из файлов на языке TypeScript два файла app.js и polyfills.js. 
...
Но можно автоматичзировать этот процесс. Для этого перейдем к свойствам проекта и далее перейдем на вкладку <span class="b">Build Events</span>:</p>
<img src="./pics/angular8.png" alt="Angular 8 и Webpack в ASP NET MVC 5" />
<p>В поле <span class="b">Pre-build event command line</span> введем команду <code>npm run build</code> и после этого сохраним проект. То есть когда будет 
выполняться построение проекта в Visual Studio автоматически запускается командная строка, в которой будет выполняться данная команда.</p>
<p>Теперь задействуем файлы приложения Angular в представлениях. Так как приложение запускается с метода Index контроллера Home, который использует 
представление Index.cshtml, то изменим это представление следующим образом:</p>
<pre class="brush:xml;">
@{
...
<p>И после запуска приложения сработает код, определенный в app.component.ts:</p>
<img src="./pics/angular7.png" alt="Добавление Angular 2 в ASP.NET MVC 5" />
...
<div class="nav"><p><a href="./24.3.php">Назад</a><a href="./">Содержание</a><a href="./24.5.php">Вперед</a></p></div></div>
<div class="socBlock">
