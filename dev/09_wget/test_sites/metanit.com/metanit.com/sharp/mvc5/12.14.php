...
<html >
<head>
<title>Базовые классы AspNet Identity в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Базовые классы AspNet Identity</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Систему AspNet Identity образует множество различных классов, предназначенных для различных задач. Рассмотрим основные классы.</p>
<h3>Контекст данных IdentityDbContext</h3>
<p>Для взаимодействия с базой данных в пространстве имен <code>Microsoft.AspNet.Identity.EntityFramework</code> определен класс 
<span class="bb">IdentityDbContext</span>. Это обычный контекст данных, производный от DbContext, который уже содержит свойства, необходимые для 
управления пользователями и ролями: свойства <code>Users</code> и <code>Roles</code>. Хотя в реальном приложении мы будем иметь дело с классами, производными от 
IdentityDbContext. Например, у нас есть такой контекст данных:</p>
<pre class="brush:c#;">
...
<p>Выведем всех пользователей из бд:</p>
<pre class="brush:c#;">
...
<h3>Пользователи</h3>
<p>Функциональность пользователей в AspNet Identity сосредоточена в классе <b>IdentityUser</b>, который определен в 
пространстве имен <code>Microsoft.AspNet.Identity.EntityFramework</code>. IdentityUser реализует интерфейс <b>IUser</b> и определяет следующие свойства:</p>
<ul>
<li><p><span class="bb">Claims</span>: возвращает коллекцию claims - специальных атрибутов, которыми обладает пользователь и которые хранят о пользователе 
определенную информацию</p></li>
<li><p><span class="bb">Email</span>: email пользователя</p></li>
<li><p><span class="bb">Id</span>: уникальный идентификатор пользователя</p></li>
<li><p><span class="bb">Logins</span>: возвращает коллекцию логинов пользователя</p></li>
<li><p><span class="bb">PasswordHash</span>: возвращает хэш пароля</p></li>
<li><p><span class="bb">Roles</span>: возвращает коллекцию ролей пользователя</p></li>
<li><p><span class="bb">PhoneNumber</span>: возвращает номер телефона</p></li>
<li><p><span class="bb">SecurityStamp</span>: возвращает некоторое значение, которое меняется при каждой смене настроек аутентификации для данного пользователя</p></li>
<li><p><span class="bb">UserName</span>: возвращает ник пользователя</p></li>
<li><p><span class="bb">AccessFailedCount</span>: число попыток неудачного входа в систему</p></li>
<li><p><span class="bb">EmailConfirmed</span>: возвращает <code>true</code>, если email был подтвержден</p></li>
<li><p><span class="bb">PhoneNumberConfirmed</span>: возвращает <code>true</code>, если телефонный номер был подтвержден</p></li>
<li><p><span class="bb">TwoFactorEnabled</span>: если равен <code>true</code>, то для данного пользователя включена двухфакторная авторизация</p></li>
</ul>
<p>Как правило, для управления пользователями определяют класс, производный от IdentityUser:</p>
<pre class="brush:c#;">
...
<p>Непосредственное управление пользователями осуществляется с помощью класса <span class="bb">UserManager&lt;T&gt;</span>, которое находится 
в пространстве имен <code>Microsoft.AspNet.Identity</code>.</p>
<p>Этот класс определяет множество методов, которые имеют как синхронные, так и асинхронные версии. Перечислим основные:</p>
<ul>
<li><p><span class="bb">ChangePassword(id, old, new) / ChangePasswordAsync(id, old, new)</span>: изменяет пароль пользователя</p></li>
<li><p><span class="bb">Create(user) / CreateAsync(user)</span>: создает нового пользователя</p></li>
<li><p><span class="bb">Delete(user) / DeleteAsync(user)</span>: удаляет пользователя</p></li>
<li><p><span class="bb">Find(user, pass) / FindAsync(user, pass)</span>: ищет пользователя по определенному логину и паролю</p></li>
<li><p><span class="bb">FindById(id) /FindByIdAsync(id)</span>: ищет пользователя по id</p></li>
<li><p><span class="bb">FindByEmail(email) / FindByEmailAsync(email)</span>: ищет пользователя по email</p></li>
<li><p><span class="bb">FindByName(name) / FindByNameAsync(name)</span>: ищет пользователя по нику</p></li>
<li><p><span class="bb">Update(user) / UpdateAsync(user)</span>: обновляет пользователя</p></li>
<li><p><span class="bb">Users</span>: возвращает всех пользователей</p></li>
<li><p><span class="bb">AddToRole(id, name) / AddToRoleAsync(id, name)</span>: добавляет для пользователя с определенным id роль name</p></li>
<li><p><span class="bb">GetRoles(id) / GetRolesAsync (id)</span>: получает все роли пользователя по id</p></li>
<li><p><span class="bb">IsInRole(id, name) / IsInRoleAsync(id, name)</span>: возвращает true, если пользователь с данным id имеет роль name</p></li>
<li><p><span class="bb">RemoveFromRole(id, name) / RemoveFromRoleAsync(id, name)</span>: удаляет роль name у пользователя по id</p></li>
</ul>
<p>Создадим класс для управления пользователями:</p>
<pre class="brush:c#;">
...
<p>Для создания класса используется объект <span class="bb">UserStore&lt;ApplicationUser&gt;</span>. Класс <code>UserStore</code> 
представляет реализацию интерфейса <code>IUserStore&lt;T&gt;</code>. В свою очередь, чтобы создать объект <code>UserStore</code>, 
необходимо использовать контекст данных ApplicationContext.</p>
<h3>Роли в AspNet Identity</h3>
<p>Каждая роль в ASP.NET Identity представляет объект интерфейса IRole. В Entity Framework имеется встроенная реализация данного интерфейса в виде 
класса <span class="bb">IdentityRole</span>.</p>
<p>Класс IdentityRole имеет следующие свойства:</p>
<ul>
<li><p><code>Id</code>: уникальный идентификатор роли</p></li>
<li><p><code>Name</code>: название роли</p></li>
<li><p><code>Users</code>: коллекция объектов IdentityUserRole, который связывают пользователя и роль</p></li>
</ul>
<p>Для управления ролями в ASP.NET Identity имеется класс <span class="bb">RoleManager&lt;T&gt;</span>, где T представляет реализацию интерфейса 
<span class="bb">IRole</span>. Класс RoleManager управляет ролями с помощью следующих синхронных и асинхронных методов:</p>
<ul>
<li><p><code>Create(role) / CreateAsync(role)</code>: создает новую роль с именем role</p></li>
<li><p><code>Delete(role) / DeleteAsync(role)</code>: удаляет роль с именем role</p></li>
<li><p><code>FindById(id) / FindByIdAsync(id)</code>: ищет роль по id</p></li>
<li><p><code>FindByName(name) / FindByNameAsync(name)</code>: ищет роль по названию</p></li>
<li><p><code>RoleExists(name) / RoleExistsAsync(name)</code>: возвращает true, если роль с данным названием существует</p></li>
<li><p><code>Update(role) / UpdateAsync(role)</code>: обновляет роль</p></li>
<li><p><code>Roles</code>: возвращает набор всех имеющихся ролей</p></li>
</ul>
<p>Теперь рассмотрим применение этих классов в приложении на ASP.NET MVC.</p>
...
<div class="nav"><p><a href="./12.1.php">Назад</a><a href="./">Содержание</a><a href="./12.2.php">Вперед</a></p></div></div>
<div class="socBlock">
