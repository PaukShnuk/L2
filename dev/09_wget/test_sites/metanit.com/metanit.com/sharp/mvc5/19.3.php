...
<html >
<head>
<title>Создание собственного модуля в ASP.NET MVC</title>
<meta charset="utf-8" />
...
<h2>Создание собственного модуля</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Интерфейс IHttpModule определяет ряд методов, которые классы модулей должны реализовать:</p>
<ul>
<li><p><span class="bb">Init(app)</span>: вызывается при создании модуля и его передаче объекту HttpApplication, в котором этот модуль регистрируется</p></li>
<li><p><span class="bb">Dispose()</span>: вызывается, когда обработка запроса завершена. В этом методе происходит освобождение связанных ресурсов</p></li>
</ul>
<p>Создадим простейший модуль, который будет измерять время обработки запроса. Для этого добавим в проект новый каталог <i>Modules</i>, в котором будут 
находиться наши модули. Затем добавим в этот каталог новый класс. Назовем его <code>TimerModule</code>:</p>
<pre class="brush:c#;">
...
<p>Для замеров времени используется класс <code>Stopwatch</code> из пространства имен <code>System.Diagnostics</code>.</p>
<P>В методе <code>Init()</code> устанавливаются обработчики для событий начала и завершения обработки запроса. Оба обработчика принимают два параметра: 
<code>src</code> - объект приложения HttpApplication и <code>args</code> - аргументы события EventArgs.</P>
<p>Обработчик начала обработки запроса просто запускает таймер. А обработчик завершения обработки запроса получает прошедшее время и добавляет информацию о нем 
в ответ клиенту через метод <code>Response.Write()</code>.</p>
<p>Метод <code>Dispose()</code> оставлен пустым, так как нам не надо освобождать никаких ресурсов, связанных с модулем.</p>
<p>После определения модуля его надо зарегистрировать в файле <i>web.config</i>. Для этого добавим в файл в пределах узла 
<code>configuration</code> следующую секцию:</p>
<pre class="brush:xml;">
&lt;system.webServer&gt;
  &lt;modules&gt;
...
<p>В данном случае у меня приложение называется LifeCycleApp, а модуль TimerModule находится в папке Modules, поэтому полное название типа будет LifeCycleApp.Modules.TimerModule.</p>
<p>И в конце мы можем протестировать модуль:</p>
<img src="./pics/19.1.png" alt="Модули в ASP.NET MVC" />
<p>И внизу страницы мы увидим, что к общему содержимому страницы добавлено время выполнения запроса.</p>
...
<div class="nav"><p><a href="./19.2.php">Назад</a><a href="./">Содержание</a><a href="./19.4.php">Вперед</a></p></div></div>
<div class="socBlock">
