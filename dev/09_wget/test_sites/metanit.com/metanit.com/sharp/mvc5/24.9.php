...
<html >
<head>
<title>ASP.NET MVC 5 | Работа конвейера веб-сервера IIS</title>
<meta charset="utf-8" />
<meta name="description" content="Работа конвейера веб-сервера IIS в ASP.NET MVC">
...
<h2>Работа конвейера веб-сервера IIS</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Ключевую роль при обработке запросов к приложениям ASP.NET играет веб-сервер IIS. IIS прослушивает входящие запросы, определяет, как они должны обрабатываться, и 
отправляет пользователю определенный ответ.</p>
<p>IIS определяет число запросов, которые могут обрабатываться единовременно на одном ядре процессора компьютера. По умолчанию число единовременно обрабатываемых 
...
статусный код ошибки 503. Но это в целом, теперь рассмотрим более детально обработку запросов.</p>
<p>Но прежде чем рассмотреть, как IIS обрабатывает запросы, познакомимся с отдельными компонентами, которые играют ключевую роль в обработке запросов в веб-сервере IIS.</p>
<h3>HTTP.sys</h3>
<p>Драйвер уровня ядра HTTP.sys является посредником между приложением и операционной системой. В архитектуре IIS он выполняет две задачи:</p>
<ul>
<li><p>HTTP.sys является протокольным слушателем (protocol listeners) по умолчанию в IIS. То есть HTTP.sys прослушивает все запросы, 
которые, используют протоколы HTTP и HTTPS, и затем передает эти запросы другим компонентам IIS для дальнейшей обработки.</p></li>
<li><p>HTTP.sys также представляет стек протокола HTTP. А именно HTTP.sys выполняет кэширование, постановку запросов в очередь, предобработку и ряд других функций.</p></li>
</ul>
<h3>Порт завершения ввода-вывода</h3>
<p>Для максимизации числа одновременно обрабатываемых запросов IIS использует специальное API, которое называется портом завершения ввода-вывода (Input/Output Completion Port). 
IIS имеет выделенный пул потоков для обработки операций ввода/вывода и управления очередью запросов. При обработке запросов этот API взаимодействует с драйвером HTTP.sys.</p>
<h3>World Wide Web Publishing Service (W3SVC)</h3>
<p>World Wide Web Publishing Service или Служба веб-публикации является адаптером над драйвером HTTP.sys и выполняет следующие функции:</p>
<ul>
<li><p>Конфигурирование HTTP.sys</p></li>
<li><p>Мониторинг показателей производительности</p></li>
</ul>
<P>До версии IIS 7.0 служба веб-публикации также управляла рабочими процессами приложений. Но начиная с IIS 7.0 за эту функцию отвечает Windows Process Activation Service.</P>
<h3>Windows Process Activation Service (WAS) </h3>
<p>Данная служба была выделена начиная с IIS 7.0. Ее предназначение - управление рабочими процессами. WAS состоит из трех основных компонентов:</p>
<ul>
<li><p><span class="bb">менеджер конфигурации</span>: считывает конфигурационную информацию о веб-приложениях  и пулах приложений из файла 
<i>applicationhost.config</i></p></li>
<li><p><span class="bb">менеджер процессов</span>: выполняет сопоставление между пулами приложений и рабочими процессами. Если соответствующий 
пулу приложения рабочий процесс еще не запущен, то менеджер процессов его запускает.</p></li>
<li><p><span class="bb">интерфейс неуправляемых адаптеров прослушивателей</span> (unmanaged listener adaptor interface): предоставляет 
интерфейс для прослушивателей запросов, которые не относятся к протоколу HTTP</p></li>
</ul>
<h3>Обработка запросов в IIS</h3>
<p>Теперь рассмотрим поэтапно работу конвейера IIS по обработке входящих запросов.</p>
<ol>
<li><p>Пользователь посылает HTTP-запрос, обращаясь через браузер к определенному ресурсу на сервере. Этот запрос перехватывается драйвером HTTP.sys.</p></li>
<li><p>Драйвер HTTP.sys обращается к WAS для получения конфигурационных данных для запрошенного адреса URL</p></li>
<li><p>Менеджер конфигурации WAS считывает данные из файла <i>applicationhost.config</i>, в частности пул приложения и конфигурационные настройки сайта</p></li>
 <li><p>Считанные данные WAS передает службе веб-публикации IIS (служба W3SVC)</p></li>
<li><p>Служба веб-публикации использует полученные от WAS данные для конфигурации HTTP.sys. Затем драйвер HTTP.sys помещает пришедший запрос в очередь порта завершения ввода-вывода (I/O completion port), которую обрабатывает WAS.</p></li>
<li><p>WAS использует выделенный пул потоков для обработки очереди. По умолчанию данный пул потоков может использовать до 250 потоков на одно ядро компьютера. 
Если к данному моменту не был запущен рабочий процесс, ассоциированный с запрошенным URL, то WAS запускает приложение <b>w3wp.exe</b>, 
в рамках которого работает рабочий процесс обработки запросов.</p></li>
<li><p>В рамках запущенного рабочего процесса ASP.NET проверяет, сколько запросов обрабатывается в текущий момент времени. Если их число превышает лимит по умолчанию в 5000 запросов, 
...
статусный код ошибки 503.</p>
<p>Если запрос направлен к статическому файлу, который не содержит кода .NET-языков, то ASP.NET посылает содержимое этого файла в порт 
завершения ввода-вывода IIS, а оттуда - пользователю, сделавшему запрос. В остальных случаях ASP.NET отправляет запрос в пул потоков CLR.</p>
<p>CLR обрабатывает запрос. На этой стадии запускается непосредственно код нашего веб-приложения или веб-сайта, размещенного на веб-сервере.</p>
<p>После того, как среда CLR закончит обработку запроса, она посылает результаты драйверу HTTP.sys, а тот - на порт завершения ввода-вывода IIS.</p></li>

<li><p>Пользователь получает результат обработки</p></li>
</ol>
<p>Схематично весь процесс можно представить следующим образом:</p>
<img src="./pics/iis.png" alt="Обработка запросов в IIS" />
...
<div class="nav"><p><a href="./24.8.php">Назад</a><a href="./">Содержание</a><a href="./24.10.php">Вперед</a></p></div></div>
<div class="socBlock">
