...
<html >
<head>
<title>ASP.NET MVC 5 | Настройка страниц ошибок</title>
<meta charset="utf-8" />
<meta name="description" content="Настройка страниц ошибок в ASP.NET MVC 5 с помощью элементов customErrors и httpErrors в файле конфигурации web.config">
...
<h2>Настройка страниц ошибок</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 22.07.2016</div>
...
<p>С помощью двух секций customErrors и httpErrors в файле конфигурации мы можем задать в ASP.NET MVC 5 обработку статусных кодов ошибок.</p>
<p>Секция <span class="b">customErrors</span> в web.config позволяет задать собственные страницы ошибок для различных статусных кодов HTTP. Правда, применение этой секции имеет ограничения.</p>
<p>Вначале добавим в проект свои страницы ошибок. Например, для обработки ошибки 404 добавим в корень проекта файл <span class="ii">404.html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Подобным образом добавим файлы и для других ошибок.</p>
<p>Далее найдем в файле конфигурации <span class="ii">web.config</span> секцию <span class="b">system.web</span> и поместим в нее подсекцию 
<code>customErrors</code> (если эта подсекция уже есть, изменим ее):</p>
<pre class="brush:xml;">
&lt;system.web&gt;
...
<p>Внутри секции <code>customErrors</code> помещаются элементы <code>error</code>, каждый из которых задает код ошибки (атрибут <code>statusCode</code>) 
и страницу переадресации при ошибке (атрибут <code>redirect</code>)</p>
<p>Если мы сейчас запустим приложение и обратимся к несуществующему ресурсу, то нам отобразится наша страница с ошибкой:</p>
<img src="./pics/24.4.png" alt="customErrors в ASP.NET MVC 5" />
<p>Однако при такой переадресации мы сталкиваемся с рядом проблем. Во-первых, в качестве адреса используется не оригинальный путь, к которому идет 
обращение, а <i>404.html</i>. То есть вместо строки запроса <span class="ii">/Home/Some</span>, мы получаем <span class="ii">/404.html?aspxerrorpath=/Home/Some</span>.
Во-вторых, при отдаче ответа клиент получает статусный код 200, то есть запрос успешно обработан, вместо кода 404.</p>
<p>Чтобы решить первую проблему, добавим к определению секции атрибут <code>redirectMode="ResponseRewrite"</code>:</p>
<pre class="brush:xml;">
...
<p>Теперь страницу с ошибкой можно будет увидеть напрямую по запрошенному пути. Однако клиенту все равно будут отдаваться статусный код 200.</p>
<p>Более того у нас есть еще и другая проблема. Если мы напрямую возвратим из метода контроллера статусный код ошибки, то этот код никак не будет обрабатываться, и настройки в 
customErrors на него не подействуют. Например:</p>
<pre class="brush:c#;">
...
<p>При обращении к нему мы получим стандартное сообщение об ошибке:</p>
<img src="./pics/24.5.png" alt="NotFound in ASP.NET MVC 5" />
<p>И для решения возникших проблем нам надо использовать не <code>customErrors</code>, а элемент <span class="b">httpErrors</span>.</p>
<p>Вначале для обработки ошибок добавим в приложение новый контроллер ErrorController:</p>
<pre class="brush:c#;">
...
<p>Добавим представление для метода NotFound:</p>
<pre class="brush:xml;">
@{
...
<p>Подобным образом добавим представление и для других методов.</p>
<p>Далее добавим элемент <code>httpErrors</code>. Этот элемент помещается внутрь секции <span class="b">system.webServer</span>. Если 
этой секции внутри web.config нет, то ее можно добавить. А узел <code>customErrors</code> мы можем удалить или закомментировать:</p>
<pre class="brush:xml;">
&lt;system.web&gt;
   &lt;compilation debug="true" targetFramework="4.6" /&gt;
   &lt;httpRuntime targetFramework="4.6" /&gt;
...
<p>Элемент httpErrors имеет ряд настроек. Чтобы протестировать настройки локально, устанавливается атрибут <code>errorMode="Custom"</code>. 
...
Атрибут <code>responseMode</code> может принимать три значения:</p>
<ul>
<li><p><code>ExecuteURL</code>: производит рендеринг ответа полученного при вызове адреса url из атрибута path</p></li>
<li><p><code>Redirect</code>: выполняет переадресацию со статусным кодом 302</p></li>
<li><p><code>File</code>: рассматривает адрес url из атрибута path как статическую страницу и использует ее в качестве ответа</p></li>
</ul>
...
<p>И теперь, если мы запустим приложение и обратимся по несуществующему пути или к методу, который возвращает статусный код 404, то у нас отобразится ответ от метода NotFound контроллера 
ErrorController:</p>
<img src="./pics/24.6.png" alt="Настройка страниц ошибок в ASP.NET MVC 5" />
<p>Установка атрибута <code>responseMode="File"</code> позволяет использовать в качестве ответа статические файлы. Например, используем ранее определенные файлы ошибок:</p>
<pre class="brush:xml;">
&lt;httpErrors errorMode="Custom" existingResponse="Replace"&gt;
    &lt;remove statusCode="404"/&gt;
    &lt;remove statusCode="403"/&gt;
