...
<html >
<head>
<title>Области в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Области</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<img src="./pics/6.2.png" alt="Области в ASP.NET MVC 5" />
...
<p>Автоматически сгенерированное определение маршрута в этом файле показывает, как входящие запросы будут сопоставляться с контроллерами и действиями, определенными в данной области Store. 
Однако чтобы сопоставление запросов с областью происходило, также надо зарегистрировать все области в файле Global.asax. 
...
Вы можете открыть файл Global.asax и увидеть изменения:</p>
<pre class="brush:c#;">
............................
...
И также определить для действий контроллера представления. Допустим, у нас в области Store определен следующий контроллер ShopController:</p>
<pre class="brush:c#;">
...
<p>Тогда при запуске приложения мы можем обратиться к методу Index контроллера по адресу <span class="ii">/Store/Shop/Index</span>, указав сначала имя области, а потом 
как обычно имя контроллера и его метода.</p>
<p>Но что будет, если мы захотим добавить в область контроллер Home с методом Index, как и в основном приложении. Так как у нас определен стандартный маршрут, 
...
<p>Чтобы избежать подобной двойственности, в файле <span class="ii">RouteConfig.cs</span> проекта надо указать пространство имен контроллера 
...
<p>В данном случае основное приложение по умолчанию находится в пространстве имен <code>AreasTestApp</code> (так как проект называется AreasTestApp), 
а контроллер Home - в пространстве имен <code>AreasTestApp.Controllers</code>. 
...
бы указать соответствующее пространство имен, которое в моем случае - <code>AreasTestApp.Areas.Store.Controllers</code></p>
