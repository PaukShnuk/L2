...
<html >
<head>
<title>ASP.NET MVC 5 | Управление иерархическими данными</title>
<meta charset="utf-8" />
<meta name="description" content="Управление иерархическими данными в ASP.NET MVC 5 и Entity Framework 6 на примере построения меню">
...
<h2>Управление иерархическими данными</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 04.08.2016</div>
...
<p>В этой статье мы рассмотрим управление иерархическими данными в Entity Framework 6 и использование их в ASP.NET MVC 5 на примере построения меню. 
...
очередь, также может содержать различные подпункты.</p>
<p>Вначале определим модель для пункта меню:</p>
<pre class="brush:c#;">
...
<p>Также определим класс инициализатора, который добавить некоторые начальные пункты меню:</p>
<pre class="brush:c#;">
...
<p>Теперь мы можем использовать эти объекты в качестве меню в приложении. Для этого определим в контроллере дополнительный метод, который будет возвращать 
частичное представление с меню:</p>
<pre class="brush:c#;">
...
<p>И определим для метода само частичное представление <span class="ii">Menu.cshtml</i>:</p>
<pre class="brush:c#;">
...
<p>Для создания меню применяется хелпер <code>BuildMenu()</code>, в котором из переданного списка получаем коллекцию объектов меню для определенного 
родительского id и при наличии в этой коллекции объектов выводим их в список. Затем рекурсивно вызываем хелпер <code>BuildMenu</code> для вложенных пунктов меню.</p>
<p>Поскольку элементы меню самого верхнего уровня имеют в качестве id родительского пункта меню значение null, то при вызове хелпера для второго параметра передается значение null в качестве значения 
по умолчанию.</p>
<p>И в конце где-нибудь на мастер-сранице <span class="ii">_Layout.cshtml</i> мы можем определить обращение к методу Menu:</p>
<pre class="brush:c#;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В итоге получится вот такое меню, которое остается только стилизовать:</p>
<img src="./pics/5.37.png" alt="Иерархические данные в ASP.NET MVC 5 и Entity Framework 6" />
...
<div class="nav"><p><a href="./5.17.php">Назад</a><a href="./">Содержание</a><a href="./6.1.php">Вперед</a></p></div></div>
<div class="socBlock">
