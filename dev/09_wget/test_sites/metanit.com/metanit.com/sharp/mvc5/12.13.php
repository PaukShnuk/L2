...
<html >
<head>
<title>Валидация пользователя в ASP.NET Identity в ASP.NET MVC</title>
<meta charset="utf-8" />
...
<h2>Валидация пользователя в ASP.NET Identity</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Кроме валидации пароля в ASP.NET Identity также используется валидация пользователя. Если мы перейдем в файл <i>IdentityConfig.cs</i> в папке 
App_Start, то в определении класса <code>ApplicationUserManager</code> мы найдем валидацию пользователя:</p>
<pre class="brush:c#;">
...
<p>Для валидации пользователя используется встроенный класс <span class="bb">UserValidator</span>, который имеет следующие свойства:</p>
<ul>
<li><p><code>AllowOnlyAlphanumericUserNames</code>: если равно true, то юзернейм должен содержать только алфавитно-цифровые символы</p></li>
<li><p><code>RequireUniqueEmail</code>: если равно true, то email пользователя должен быть уникальным</p></li>
</ul>
<p>Возможности встроенного валидатора довольно ограничены, но мы можем переопределить его поведения.  Для этого нам надо унаследовать 
класс от <code>UserValidator&lt;ApplicationUser&gt;</code>. Итак, создадим следующий класс:</p>
<pre class="brush:c#;">
...
<p>В валидаторе мы можем использовать те же свойства <code>AllowOnlyAlphanumericUserNames</code> и <code>RequireUniqueEmail</code>. В частности, 
в конструкторе устанавливается <code>AllowOnlyAlphanumericUserNames = false</code></p>
<p>Чтобы написать свою логику валидации, надо переопределить метод <code>ValidateAsync</code>. В нашем случае он проверяет значения полей UserName 
...
<code>IdentityResult</code>, который сожержит все возникшие ошибки.</p>
<p>И затем можно в классе ApplicationUserManager изменить используемый валидатор пользователя:</p>
<pre class="brush:c#;">
var manager = new ApplicationUserManager
					(new UserStore&lt;ApplicationUser&gt;
						(context.Get&lt;ApplicationDbContext&gt;()));
...
<p>Для целей тестирования я определил дополнительное свойство Name в модели регистрации и добавил поле для его ввода в представление регистрации. 
А в контроллере AccountController при создании нового пользователя применяется это свойство для UserName:</p>
<pre class="brush:c#;">
...
<p>И теперь, если я введу некорректные значения, приложение отобразит мне ошибки:</p>
<img src="./pics/uservalidator.png" alt="Валидация пользователя в ASP.NET Identity" />
...
<div class="nav"><p><a href="./12.7.php">Назад</a><a href="./">Содержание</a><a href="./12.8.php">Вперед</a></p></div></div>
<div class="socBlock">
