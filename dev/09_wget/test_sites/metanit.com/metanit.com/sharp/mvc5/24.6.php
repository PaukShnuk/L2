...
<html >
<head>
<title>ASP.NET MVC 5 | Загрузка файлов на сервер</title>
<meta charset="utf-8" />
<meta name="description" content="Загрузка файлов на сервер в ASP.NET MVC 5">
...
<h2>Загрузка файлов на сервер</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 13.09.2014</div>
...
<p>Во-первых, создадим в проекте папку, в которую будем загружать файлы. Пусть она будет называться <i>Files</i>.</p>
<p>Потом создадим код разметки в представлении, с помощью которого пользователь мог бы загружать файл:</p>
<pre class="brush:xml;">
@{
...
<p>Хелпер <code>Html.BeginForm</code> сгенерирует форму. Так как первые два параметра указывают, что форма будет обрабатываться на сервере методом Upload 
контроллера HomeController.</p>
<p>Благодаря установке атрибута формы <code>enctype="multipart/form-data"</code> браузер будет знать, что надо загрузить файл.</p>

<p>Тогда мы можем получить загруженный файл в методе действия <code>Upload</code>:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>Переданный с клиента на сервер файл передается через объект <span class="bb">HttpPostedFileBase</span>.</p>
<p>С помощью метода <code>SaveAs</code> мы сохраняем файл по определенному пути на сервере. А помочь определить точный путь для сохранения поможет 
метод <code>Server.MapPath()</code>.</p>
<p>С помощью свойства <code>InputStream</code> мы можем получить поток файла и манипулировать им как обычными потоками:</p>
<pre class="brush:c#;">
...
<p>При множественной загрузке файлов создаем в представлении набор элементов с типом <code>file</code> с одинаковым значением атрибута <code>name</code>:</p>
<pre class="brush:xml;">
@using (Html.BeginForm("Upload", "Home", FormMethod.Post, new {enctype="multipart/form-data"}))
{
...
<p>Теперь в метода контроллера мы уже будем получать коллекцию объектов HttpPostedFileBase:</p>
<pre class="brush:c#;">
[HttpPost]
...
<h3>Ограничение размера файлов</h3>
<p>Даже если мы все сделаем правильно, мы можем получить ошибку, если попытаемся загрузить файл размером больше 4 МБ. 4 мегабайта - ограничение, 
действующее на стороне сервера. Однако мы можм его переопределить.</p>
<p>Для этого найдем в файле конфигурации <i>web.config</i> узел <code>httpRuntime</code> (он вложен в элемент <code>system.web</code>). Чтобы установить новое 
ограничение по размеру, нам надо указать атрибут <code>maxRequestLength</code>. Например, зададим в качестве нового размера около 20 мегабайт:</p>
<pre class="brush:xml;">
&lt;system.web&gt;
...
<div class="nav"><p><a href="./24.5.php">Назад</a><a href="./">Содержание</a><a href="./24.7.php">Вперед</a></p></div></div>
<div class="socBlock">
