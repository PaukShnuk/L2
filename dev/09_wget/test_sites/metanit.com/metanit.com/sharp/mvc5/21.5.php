...
<html >
<head>
<title>IoC-контейнер Castle Windsor в ASP.NET MVC</title>
<meta charset="utf-8" />
...
<h2>IoC-контейнер Castle Windsor</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Еще одним распространенным IoC-контейнером является Castle Windsor.</p>
<p>Возьмем привычный пример с репозиторием:</p>
<pre class="brush:c#;">
...
<p>Во-первых, добавим через NuGet соответствующий пакет:</p>
<img src="./pics/castlewindsor.png" alt="Внедрение зависимостей с Castle Windsor в ASP.NET MVC" />
<P>Добавим в проект в какую-нибудь специальную папку новый класс CastleControllerFactory:</P>
<pre class="brush:c#;">
...
<p>Класс CastleControllerFactory представляет реализацию фабрики контроллеров. Его цель - установка контейнера Castle Windsor и 
получение контроллера для обработки запроса. Также добавим новый класс <code>ApplicationCastleInstaller</code>:</p>
<pre class="brush:c#;">
...
<p>Данный класс реализует интерфейс IWindsorInstaller, предоставляемый IoC-контейнером и регистрирует зависимости компонентов. В данном случае регистрируется 
сопоставление объектов IRepository с типом BookRepository, а также регистрируются все контроллеры, которые входят в сборку.</p>
<p>И чтобы внедрение зависимостей заработало, изменим файл Global.asax:</p>
<pre class="brush:c#;">
...
<p>Суть проведенных изменений состоит в создании контейнера и установки фабрики контроллеров для обработки запросов.</p>
<h3>Установка параметров и свойств</h3>
<p>Если зависимость в виде репозитория принимает в конструкторе параметр:</p>
<pre class="brush:c#;">
public class BookRepository : IDisposable, IRepository
{
...
<p>То мы можем установить значение для этого параметра следующим образом:</p>
<pre class="brush:c#;">
container.Register(Component.For&lt;IRepository&gt;().ImplementedBy&lt;BookRepository&gt;()
                .DynamicParameters((r, k) =&gt;{ k["context"] = new BookContext(); }));
</pre>
<p>Метод <code>DynamicParameters</code> принимает делегат, через который устанавливается значение параметра следующим образом: <code>k["название_параметра"]=значение</code></p>
<p>Установка зависимостей в свойствах будет несколько сложнее. Во-первых, изменим интерфейс репозитория, чтобы он принимал контекст в виде свойства:</p>
<pre class="brush:c#">
...
<p>Установим значение для этого свойства:</p>
<pre class="brush:c#;">
public class ApplicationCastleInstaller : IWindsorInstaller
{
    public void Install(IWindsorContainer container, IConfigurationStore store)
...
<p>Чтобы установить свойство, нам надо создать нужный объект <code>var repo = container.Resolve&lt;IRepository&gt;();</code>. Затем мы можем обращаться к нему 
также, как в обычной программе.</p>
...
<div class="nav"><p><a href="./21.4.php">Назад</a><a href="./">Содержание</a><a href="./22.1.php">Вперед</a></p></div></div>
<div class="socBlock">
