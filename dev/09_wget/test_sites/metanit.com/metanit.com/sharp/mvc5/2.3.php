...
<html >
<head>
<title>Стилизация приложения и мастер-страницы в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Стилизация приложения и мастер-страницы в ASP.NET MVC 5 и C#">
...
<h2>Стилизация приложения и мастер-страницы</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Итак, добавим в наше приложение небольшую примитивную стилизацию. Для этого определим файл стилей. По умолчанию Visual Studio уже добавляет 
файл стилей <span class="b">Site.css</span> в папку Content:</p>
<img src="./pics/2.8.png"  />
<p>Кроме файла Site.css, в папке Content находится файл css-фреймворка Bootstrap, но нам он пока не понадобится. Откроем файл Site.css и изменим 
его содержание на следующее:</p>
<pre class="brush:css;">
body {
    font-size: 13px;
...
<p>Класс <code>.menu</code> в данном случае будет служить в качестве класса для навигационного меню на сайте. Хотя наше приложении не очень большое, поэтому 
там будет только ссылка на главную страницу. Но при необходимости вы можете добавить в него дополнительные пункты.</p>
<p>Чтобы использовать стили, мы можем их просто подключить в секции head, как в любой обычной html-страницу:</p>
<pre class="brush:xml;">
&lt;head&gt;
    &lt;meta name="viewport" content="width=device-width" /&gt;
...
<p>В нашем случае достаточно вставить данный код на оба наших представления. Однако это не самый лучший подход, так как стили для обоих представлений общие, 
...
на всех представлениях.</p>
<p>И чтобы выйти из этой проблемы фреймворк ASP.NET MVC предоставляет нам такую функциональность, как мастер-страницы. Мастер-страница задает 
единый шаблон для других использующих его представлений.</p>
<p>По умолчанию в проекте уже имеется мастер-страница, которая называется <span class="bb">_Layout.cshtml</span>. Ее можно найти в папке 
<i>Views -&gt; Shared</i>:</p>
<img src="./pics/2.9.png"  />
<p>Файл _Layout.cshtml уже имеет некоторое содержимое по умолчанию. Изменим его на следующее:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Для подключения стилей здесь использовался другой способ - метод Url.Content, в который передается путь к файлу. Впоследствии мы познакомимся с еще одним 
способом - подключение бандлов, который является более предпочтительным.</p>
<p>После секции head на мастер-странице идет создание меню. Так как у нас всего два представления, 
...
Он генерирует элемент-ссылку и принимает название ссылки, метод контроллера и имя контроллера.</p>
<p>И далее идет вызов метода <span class="bb">RenderBody()</span> - с помощью этого метода в это место будет подставляться разметка уже конкретных представлений.</p>
<p>Теперь изменим представления так, чтобы они использовали мастер-страницу. Обновленное представление Index.cshtml:</p>
<pre class="brush:xml;">
@{
...
<p>Также изменим представление Buy.cshtml:</p>
<pre class="brush:xml;">
@{
    Layout = "~/Views/Shared/_Layout.cshtml";
}
...
<p>Чтобы указать используемую мастер-страницу, в начале представления прописывается путь к мастер-странице: 
<code>Layout = "~/Views/Shared/_Layout.cshtml"; </code>. И теперь нам больше не нужны секции head и body. Мы их можем удалить.</p>
<p>Запустим на выполнение обновленный проект и убедимся, что к нашему приложению применена стилизация и мастер-страницы:</p>
<img src="./pics/2.10.png" alt="Стилизация в ASP.NET MVC 5" />
<p>На этом работа над приложением закончена, и теперь мы можем перейти к более детальному обсуждению основных компонентов приложения MVC.</p>
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/M-t_CXexWhc"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 7. Первое приложение. Стилизация в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./2.2.php">Назад</a><a href="./">Содержание</a><a href="./3.1.php">Вперед</a></p></div></div>
<div class="socBlock">
