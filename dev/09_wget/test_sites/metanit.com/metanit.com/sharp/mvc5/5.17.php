...
<html >
<head>
<title>ASP.NET MVC 5 | Domain Model и View Model</title>
<meta charset="utf-8" />
<meta name="description" content="Модель представления View Model и модель домена Domain Model в ASP.NET MVC 5">
...
<h2>Domain Model и View Model</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Модели, которые используются в приложении, бывают разными. В прошлой теме мы рассмотрели вывод и одновременно фильтрацию объектов Player:</p>
<pre class="brush:c#;">
public class Player
{
...
<p>Модель Player в данном случае представляет <b>модель домена</b>, ее объекты хранятся в базе данных и чтобы их получить, мы 
...
данных в представление нам пришлось создать дополнительную модель:</p>
<pre class="brush:c#;">
...
<p>Это модель представления или <b>view model</b>, которая предназначена конкретно для вывода данных в некоторое представление. И важно 
понимать отличие domain model от view model.</p>
<p>Другая часто встречающаяся ситуация - редактирование. Нам надо редактировать игрока, но надо изменить не все свойства, а только часть. Опять же в этом случае создается 
дополнительная модель представления, которая содержит те свойства, которые надо изменить. Например:</p>
<pre class="brush:c#;">
...
<p>И представление для редактирования тогда использует не модель Player, а модель PlayerViewModel. Тогда обработка введенных данных 
могла выглядеть следующим образом:</p>
<pre class="brush:c#;">
[HttpPost]
...
<p>В некоторых случаях модель домена может совпадать с моделью представления. Например, при создании нового игрока мы можем определить следующее представление:</p>
<pre class="brush:xml;">
...
<p>Представление использует модель Player, метод контроллера также может получать эту модель и сразу сохранять ее в базу данных. То есть здесь domain model = view model. 
Для маленьких приложений это вполне допустимо. Но, как правило, такие ситуации в реальных приложениях встречаются не часто и то в основном в очень небольших.</p>
<p>Для больших приложений или даже небольших, но которые постоянно расширяются, подобная ассоциация моделей может вылиться в проблемы 
с поддержкой и развитием приложения. Поэтому рекомендуется для каждого отдельного представления создавать свою ViewModel.</p>
...
<div class="nav"><p><a href="./5.16.php">Назад</a><a href="./">Содержание</a><a href="./5.18.php">Вперед</a></p></div></div>
<div class="socBlock">
