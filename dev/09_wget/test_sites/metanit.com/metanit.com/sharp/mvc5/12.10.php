...
<html >
<head>
<title>Сброс пароля в ASP.NET Identity 2.0</title>
<meta charset="utf-8" />
<meta name="description" content="Сброс пароля в ASP.NET Identity 2.0">
...
<h2>Сброс пароля в ASP.NET Identity 2.0</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Рассмотрим механизм сброса пароля в ASP.NET Identity. Для Identity 2.0 уже имеется необходимая инфраструктура для сброса пароля: это представления и методы контроллера.</p>
<p>В файле <span class="ii">AccountViewModels.cs</span> уже описаны модели, которые используются для сброса пароля: <span class="bb">ForgotPasswordViewModel</span> и 
<span class="b">ResetPasswordViewModel</span></p>
<p>А в контроллере AccountController определены методы, необходимые для сброса пароля. В частности, методы <code>ResetPassword</code> и 
<code>ForgotPassword</code>.</p>
<p>Для сброса пароля мы будем обращаться к методу <code>ForgotPassword</code>, который будет вызывать представление <i>ForgotPassword.cshtml</i>. 
Данное представление содержит форму для ввода электронного адреса, который будет использоваться для сброса пароля.</p>
<p>Данные формы обрабатываются post-версией метода <code>ForgotPassword</code>, который по умолчанию не содержит функционал для сброса пароля. Поэтому 
...
<p>После отправки письма пользователь перенаправляется к методу ForgotPasswordConfirmation. Все остальные действия по сбросу пароля выполняет метод 
<code>ResetPassword</code>, стандартная реализация которого в принципе не требует изменения.</p>
<p>Но чтобы была возможной отправка электронных сообщений, должен быть настроен соответствующим образом сервис отправки почты. Для этого  перейдем к файлу 
<i>IdentityConfig.cs</i> и найдем в нем класс <code>EmailService</code>, который и будет отправлять электронные сообщения. Изменим его следующим образом:</p>
<pre class="brush:c#;">
...
<p>Итак, допустим, у нас в системе уже есть зарегистрированный с почтовым адресом пользователь. Попробуем сбросить пароль и обратимся к методу ForgotPassword:</p>
<img src="./pics/12.25.png" alt="Сброс пароля в ASP.NET MVC" />
<p>И после этого мы будем перенаправлены по методу ForgetPasswordConfirmation:</p>
<img src="./pics/12.26.png" alt="Сброс пароля в ASP.NET Identity" />
<p>Тем временем на указанную электронную почту приходит письмо с ссылкой. После перехода по этой ссылке срабатывает метод ResetPassword, 
и мы оказываемся на странице сброса пароля:</p>
<img src="./pics/12.27.png"  />
<p>Указываем ту же почту и новый пароль и отправляем форму на сервер. После этого post-метод ResetPassword обрабатывает данные и перенаправляет нас на 
метод ResetPasswordConfirmation:</p>
<img src="./pics/12.28.png" alt="Подтверждение пароля в ASP.NET Identity"  />
<p>И после переустановки пароля мы сможем заходить на сайт под тем же электронным адресом и новым паролем.</p>
...
<div class="nav"><p><a href="./12.9.php">Назад</a><a href="./">Содержание</a><a href="./12.11.php">Вперед</a></p></div></div>
<div class="socBlock">
