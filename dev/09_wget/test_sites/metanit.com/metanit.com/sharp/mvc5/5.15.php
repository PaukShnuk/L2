...
<html >
<head>
<title>Переопределение шаблонов отображения и редактирования в ASP.NET MVC</title>
<meta charset="utf-8" />
...
<h2>Переопределение шаблонов отображения и редактирования</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>При использовании методов <span class="bb">DisplayFor()/EditorFor()</span> фреймворк MVC сам определяет, каким образом и какую разметку html создавать для 
отображения и редактирования полей модели. Но мы можем переопределить подобное поведение, указав, как именно MVC должен работать с отдельными типами данных.</p>
<p>Допустим, у нас есть следующая модель:</p>
<pre class="brush:c#;">
public class Book
{
...
<p>Модель содержит свойства трех типов данных: int, string, decimal. Переопределим создание разметки для этих свойств.</p>
<p>Для этого добавим в проект в каталог <i>Views/Shared</i> две новых папки: <span class="bb">DisplayTemplates</span> (для шаблонов отображения) и 
<span class="bb">EditorTemplates</span> (для шаблонов редактирования):</p>
<img src="./pics/5.31.png" alt="Display and Editor Templates in ASP.NET MVC 5" />
<p>Так как класс использует свойства трех разных типов данных, в шаблонах отображения в папке DisplayTemplates я создал по файлу для каждого типа. Названия 
файлов носят названия классов, которые представляют данный тип. Например, файл <i>Int32.cshtml</i>, который используется для визуализации значений int, 
...
<p>Директива <code>@model Int32</code> указывает, что в качестве модели будет использоваться тип int или Int32. А само содержимое модели делается жирным шрифтом 
с помощью тега <code>&lt;b&gt;</code></p>
<p>Файл <i>String.cshtml</i> имеет следующее содержимое:</p>
<pre class="brush:xml;">
@model String

&lt;b&gt;"@Model"&lt;/b&gt;
</pre>
<p>Тут тоже самое, только название указывается в кавычках. В общем-то мы можем определить разные теги или структуру тегов, можно, например, задать классы 
(<code>&lt;span class="string"&gt;"@Model"&lt;/span&gt;</code>) и т.д.</p>
<p>Файл <i>Decimal.cshtml</i>:</p>
<pre class="brush:xml;">
@model decimal
...
<p>Здесь уже определяется логика форматирования значения в качестве денежной единицы.</p>
<p>Теперь если мы используем хелпер DisplayFor() для вывода свойств модели:</p>
<pre class="brush:xml;">
...
<p>То мы получим следующую картину:</p>
<img src="./pics/5.32.png" />
<p>В папку <i>EditorTemplates</i> я добавил только один шаблон форматирования - <i>String.cshtml</i>:</p>
<pre class="brush:xml;">
@model string

&lt;input type="text" name="name" style="border-color:red; height:40px; background-color:#eee;" value="@Model" /&gt;
</pre>
<p>И допустим, у нас есть следующее представление для редактирования данных:</p>
<pre class="brush:xml;">
@model CustomDisplayEditorTemplates.Models.Book
...
<p>То мы получим следующую страницу:</p>
<img src="./pics/5.33.png" />
<p>При создании подобных шаблонов на основе элементов ввода следует учитывать их ограниченность: в данном случае элемент имеет атрибут 
<code>name="name"</code>, поэтому его значение будет сопоставляться со свойством Name. Если у нас она модель в которой есть только одно строковое свойство, 
которое называется Name, то подобный подход еще сработает. Но у нас может быть несколько свойств с типом string, которые могут называться по-разному. В этом случае 
мы можем определить шаблон вывода или редактирования модели целиком.</p>
<p>Например, добавим в папку DisplayTemplates следующий файл <i>Book.cshtml</i>:</p>
<pre class="brush:xml;">
@model CustomDisplayEditorTemplates.Models.Book
...
<p>CustomDisplayEditorTemplates - в данном случае это название моего проекта, в котором определен класс Book.</p>
<p>Значения всех свойств модели выводятся через таблицу. Тогда представление для отображения модели будет выглядеть так:</p>
<pre class="brush:xml;">
@model CustomDisplayEditorTemplates.Models.Book
@{
    Layout = null;
...
<img src="./pics/5.34.png" />
<p>Также добавим в каталог EditorTemplates шаблон для редактирования <i>Book.cshtml</i>:</p>
<pre class="brush:xml;">
@model CustomDisplayEditorTemplates.Models.Book
...
<p>Используем этот шаблон для создания формы редактирования:</p>
<pre class="brush:xml;">
@model CustomDisplayEditorTemplates.Models.Book
...
<div class="nav"><p><a href="./5.14.php">Назад</a><a href="./">Содержание</a><a href="./5.16.php">Вперед</a></p></div></div>
<div class="socBlock">
