...
<html >
<head>
<title>ASP.NET MVC 5 | PostgreSQL</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с базой данных PostgreSQL, выборка данных в ASP.NET MVC 5">
...
<h2>PostgreSQL</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Для подключения к базе данных PostgreSQL в ASP.NET MVC вначале добавим в проект пакеты Entity Framework и Npgsql.EntityFramework:</p>
<img src="./pics/postgres.png" alt="PostgreSQL в ASP.NET MVC 5" />
<p>После добавления пакетов надо изменить файл конфигурации web.config. Вначале добавим строку подключения, определив в файле секцию connectionStrings:</p>
<pre class="brush:xml;">
&lt;connectionStrings&gt;
...
<p>В строке подключения надо указать адрес сервера (параметр Server), id пользователя (по умолчанию postgres), пароль для данного пользователя, 
номер порта (по умолчанию 5432) и название базы данных.</p>
<p>После добавления пакетов из NuGet внизу файла web.config у нас будет следующая секция <code>entityFramework</code>:</p>
<pre class="brush:xml;">
&lt;entityFramework&gt;
...
<p>В этой секции изменим вторую строку, чтобы в качестве провайдера по умолчанию использовался Npgsql:</p>
<pre class="brush:xml;">
&lt;defaultConnectionFactory type="Npgsql.NpgsqlServices, Npgsql.EntityFramework" /&gt;
</pre>
<p>И затем под секцией <code>entityFramework</code> добавим следующую секцию:</p>
<pre class="brush:xml;">
&lt;system.data&gt;
  &lt;DbProviderFactories&gt;
...
<p>Это вся необходимая конфигурация для работы с PostgreSQL. Все остальные принципы работы будут теми же. Так же определяем класс контекста и модели:</p>
<pre class="brush:c#;">
...
<p>И в методе контроллера добавляем данные в БД и передаем их в представление:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./24.13.php">Назад</a><a href="./">Содержание</a><a href="./24.15.php">Вперед</a></p></div></div>
<div class="socBlock">
