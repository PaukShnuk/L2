...
<html >
<head>
<title>ASP.NET MVC 5 | Использование Datepicker jQuery</title>
<meta charset="utf-8" />
<meta name="description" content="Использование виджета jQueryUI Datepicker в ASP.NET MVC для создания элемента выбора дат в виде календаря">
...
<h2>Использование Datepicker jQuery</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>В веб-приложениях нередко приходится сталкиваться с датами. Например, у нас в проекте есть такая модель Person:</p>
<pre class="brush:c#;">
public class Person
{
...
<p>Она имеет поля: Id, имя, страна и дата рождения. Все просто, но теперь создадим представление для создания объекта этой модели. 
Допустим, у нас будет простенький контроллер:</p>
<pre class="brush:c#;">
...
<p>И само представление <i>Create.cshtml</i>, в котором и будет происходить создание объекта:</p>
<pre class="brush:xml;">
...
<p>Если мы запустим в Опере 12 или в Хроме, то браузер создаст нам поле для ввода даты в виде календарика, с помощью которого довольно удобно выбирать 
дату. Например, в Хроме это выглядит так:</p> 
<img src="./pics/datapicker1.png" />
<p>Вроде неплохо, но IE или Firefox, например, создадут для ввода даты обычное текстовое поле, а не что-то типа календаря. И тут нам на помощь приходит виджет jquery-ui datepicker.</p>
<p>Этот виджет определен в библиотеке jquery ui. В проектах Basic и Internet Application эта библиотека уже по умолчанию имеется в папке Script и 
добавлена в бандл jqueryui. Если вы в качестве типа проекта используете шаблон Empty, то надо будет дополнительно подключить соответствующие библиотеки, 
например, с помощью NuGet.</p>
<p>Итак, чтобы добавить datepicker в приложение, нам надо подключить библиотеку jquery-ui и сопутствующие файлы стилей. Я это сделаю в 
мастер-странице <i>_Layout.cshtml</i>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Сначала подключаем бандлы скриптов и стилей jquery-ui, которые у нас объявлены в файле <i>BundleConfig.cs</i> в папке <i>App_Start</i> 
(стили - <code>@Styles.Render("~/Content/themes/base/css")</code> и скрипты - <code>@Scripts.Render("~/bundles/jqueryui")</code>).</p>
<p>После этого, с помощью дополнительного скрипта, используя ранее подключенную библиотеку modernizr, мы проверяем, создает ли браузер 
специальное поле по типу календарика. Если нет, то добавляем в разметку datepicker.</p>
<p>Обратите внимание на формат даты в datepicker: <code>dateFormat: 'dd/mm/yy'</code>. Этот формат должен совпадать с форматом даты, 
который содержит данное поле модели, то есть в данном случае <code>DataFormatString = "{0:dd'/'MM'/'yyyy}"</code>.</p>
<p>И, обратившись к представлению,  например, в браузере Firefox, вы сможете увидеть, что он использует элемент datepicker для выбора дат:</p>
<img src="./pics/datapicker2.png" />
<p>Тут у нас остается только одна проблема, все надписи и календарь на английском. Чтобы руссифицировать, изменим вышеопределенную функцию в коде 
javascript следующим образом:</p>
<pre class="brush:xml;">
&lt;!--если браузер не создает специального поля для date, то добавляем datepicker--&gt;
&lt;script type="text/javascript"&gt;
    $(function () {
...
<div class="nav"><p><a href="./17.7.php">Назад</a><a href="./">Содержание</a><a href="./17.10.php">Вперед</a></p></div></div>
<div class="socBlock">
