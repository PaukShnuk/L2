...
<html >
<head>
<title>Создание приложения с ASP.NET Identity с нуля в ASP.NET MVC</title>
<meta charset="utf-8" />
<meta name="description" content="Создание приложения с ASP.NET Identity с нуля в ASP.NET MVC">
...
<h2>Создание приложения с ASP.NET Identity с нуля</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 07.12.2015</div>
...
<p>При создании проекта с типом аутентификации Individual User Accounts в него по умолчанию добавляется все необходимые файлы для работы с AspNet Identity. 
Однако, как правило, редко востребован весь стандартный функционал. Если нам нужна только регистрация и логин, то остальные файлы и неиспользуемый код будут просто висеть в проекте, 
либо их придется удалять. Однако мы можем и выбрать любой другой тип проекта и в него уже добавить вручную функционал AspNet Identity, причем только тот, 
который нам будет нужен. Это даст нам больший контроль над тем кодом, который размещается в проекте.</p>
<p>Например, создадим обычный проект MVC с типом аутентификации <code>No Authentication</code>. 
<p>Чтобы добавить в проект AspNet Identity, нам надо добавить следующие NuGet-пакеты:</p>
<ul>
<li><p>Microsoft.AspNet.Identity.EntityFramework</p></li>
<li><p>Microsoft.AspNet.Identity.OWIN</p></li>
<li><p>Microsoft.Owin.Host.SystemWeb</p></li>
</ul>
<p>После добавления пакетов надо обновить файл <i>web.config</i>: добавим строку подключения:</p>
<pre class="brush:xml;">
&lt;connectionStrings&gt;
  &lt;add name="IdentityDb" providerName="System.Data.SqlClient"
    connectionString="Data Source=(localdb)\MSSQLLocalDB;AttachDbFilename=|DataDirectory|\IdentityDb.mdf;Integrated Security=True;"/&gt;
&lt;/connectionStrings&gt;
</pre>
<p>Теперь добавим классы пользователей и контекста данных в папку Models. Класс контекста будет наследоваться от IdentityDbContext:</p>
<pre class="brush:c#;">
...
<p>В классе пользователя кроме унаследованных от IdentityUser свойств также определяется и свойство Year для хранения года рождения пользователя.</p>
<p>Поскольку вся работа с пользователями идет не напрямую, а через менеджер пользователей, то также добавим в папку Models соответствующий класс:</p>
<pre class="brush:c#;">
...
<p>Класс менеджера пользователей наследуется от <code>UserManager</code>. В конструкторе он принимает объект хранилища пользователей IUserStore. 
А статический метод <code>Create()</code> создает экземпляр класса ApplicationUserManager с помощью объекта контекста <code>IOwinContext</code></p>
<p>Последний шаг в первоначальной настройке проекта для AspNet Identity состоит в добавлении в проект файла запуска приложения OWIN. Итак, 
добавим в папку App_Start файл <i>Startup.cs</i> со следующим содержанием:</p>
<pre class="brush:c#;">
using Microsoft.Owin;
using Owin;
...
<p>Интерфейс <code>IAppBuilder</code> определяет множество методов, в данном случае нам достаточно трех методов. Метод 
<code>CreatePerOwinContext</code> регистрирует в OWIN менеджер пользователей ApplicationUserManager и класс контекста ApplicationContext.</p>
<p>Метод <code>UseCookieAuthentication</code> с помощью объекта <code>CookieAuthenticationOptions</code> устанавливает аутентификацию на основе куки 
в качестве типа аутентификации в приложении. А свойство <code>LoginPath</code> позволяет установить адрес URL, по которому будут перенаправляться неавторизованные 
пользователи. Как правило, это адрес <i>/Account/Login</i>.</p>
<p>Это минимально необходимая настройка проекта для использования AspNet Identity, на основе которой мы уже сможем создавать всю остальную систему авторизации и аутентификации.</p>
...
<div class="nav"><p><a href="./12.12.php">Назад</a><a href="./">Содержание</a><a href="./12.16.php">Вперед</a></p></div></div>
<div class="socBlock">
