...
<html >
<head>
<title>Использование объектов Claim</title>
<meta charset="utf-8" />
...
<h2>Использование объектов Claim</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Объекты <b>Claim</b> позволяют нам сохранять дополнительную информацию, связанную с текущим пользователем. Например, возьмем новый стандартный 
проект, использующий аутентификацию Individual User Accounts и изменим в нем класс <span class="bb">ApplicationUser</span>, 
который находится в файле <i>IdentityModels.cs</i>:</p>
<pre class="brush:c#;">
...
<p>Мы определили два дополнительных свойства в классе ApplicationUser. И также для каждого дополнительного свойства добавили соответсвующий объект claim. 
Вызов метода <code>GenerateUserIdentityAsync</code>, который уже имеется в классе по умолчанию, возвращает нам объект <code>ClaimsIdentity</code>, 
для которого сообственно и производится добавление claims.</p>
<p>Добавление очень просто: используем метод <span class="bb">AddClaim</span>, в который передаем новый Claim. Объект Claim принимает два зачения: 
тип клейма в виде строки и его строковое значение. По умолчанию фреймворк содержит ряд констнатных значений для типов claim в перечислении 
<span class="bb">ClaimTypes</span>, однако этот набор ограниченный, поэтому для сохранения возраста используется строка "age", так как ClaimTypes 
не содержит нужного типа.</p>
<p>Чтобы пользователь смог задать эти данные при регистрации, изменим модель регистрации <span class="bb">RegisterViewModel</span> (нахдящаяся в файле 
<i>AccountViewModels.cs</i>), которая будет содержать все эти свойства:</p>
<pre class="brush:c#;">
...
<p>Соответственно нам надо изменить и представление регистрации, например, следующим образом:</p>
<pre class="brush:xml;">
...
<p>И в методе Register контроллера AccountController изменим создание нового пользователя:</p>
<pre class="brush:c#;">
[HttpPost]
[AllowAnonymous]
...
<p>И чтобы проверить в действии этот функционал, определим в контроллере HomeController новое действие:</p>
<pre class="brush:c#;">
...
<p>Итак, зарегистрируемся:</p>
<img src="./pics/12.17.png" />
<p>Теперь обратимся к действию GetInfo:</p>
<img src="./pics/12.18.png" />
<p>Таким образом, благодаря использованию claims мы можем сохранять ряд важной информации, связанной с текущим пользователем, и уменьшить 
количество обращений к бд для получения этой информации.</p>
