...
<html >
<head>
<title>Persistent Connection API в SignalR 2</title>
<meta charset="utf-8" />
...
<p>Создадим проект ASP.NET MVC 5 с типом аутентификации No Authentication. Назовем его, например, <span class="b">PersistentConMvc</span>. Затем через менеджер NuGet добавим в проект библиотеку SignalR.</p>
...
<p>Теперь создадим серверную инфраструктуру для взаимодействия с клиентом. Создадим в проекте папку <span class="ii">Connection</span> и добавим в нее новый 
класс <span class="b">ChatConnection</span>:</p>
<pre class="brush:c#;">
using System;
...
<p>При создании класса мы можем выбрать уже имеющийся шаблон файла:</p>
<img src="./pics/16.7.png" alt="Persistent Connection API в SignalR 2" />
...
<p>Чтобы подключить функциональность SingnalR, добавим в проект новый класс <i>Startup</i>:</p>
<pre class="brush:c#;">
using System;
using System.Threading.Tasks;
...
<p>В данном случае мы регистрируем маршрут, по которому затем будем обращаться из клиентской части.</p>
<p>Теперь клиентская часть. Так как в проекте по умолчанию есть контроллер HomeController, то мы можем использовать предназначенные для его методов 
представления. Изменим представление <i>Index.cshtml</i> следующим образом:</p>
<pre class="brush:xml;">
@{
    Layout = null;
}
