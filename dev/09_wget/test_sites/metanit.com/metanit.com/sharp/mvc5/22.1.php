...
<html >
<head>
<title>URL Rewriting в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h1>URL Rewriting</h1>
<h2>Введение в URL Rewriting</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Кроме определения маршрутов в приложении ASP.NET MVC 5 нам может потребоваться такой механизм как <b>URL Rewriting</b> или переопределение URL-адресов, которые используются для доступа к 
ресурсам приложения. Например, приложение использовало один домен, потом переехало на другой - надо сделать редирект. Или стандартная проблема: 
перенаправление с домена с www на домен без www и наоборот. Для этого делается переопределение URL-адресов.</p>
<p>За переопределение URL отвечает специальный модуль <b>URL Rewrite Module</b>. Само переопределение производится в файле конфигурации <i>web.config</i> в элементе <code>&lt;system.webServer&gt;</code>, 
который определяется внутри элемента <code>&lt;configuration&gt;</code></p>
<p>Например, определим автоматическое перенаправление с адресов с www на адреса без www:</p>
<pre class="brush:xml;">
...
<p>Все правила переопределения URL определяются в элементе <code>&lt;rewrite&gt;</code>. В нем мы можем использовать два типа наборов правил:</p>
<ul>
<li><p><span class="bb">&lt;globalRules&gt;</span>: глобальные правила, которые задаются на стороне сервера и по отношению ко всему серверу. 
Они определяются в файле <i>ApplicationHost.config</i> и не могут быть переопределены или отключены на более низком уровне (на уровне отдельных 
сайтов и веб-приложений). Эти правила срабатывают во время события <code>PreBeginRequest</code> на этапе обработки запроса.</p></li>
<li><p><span class="bb">&lt;rules&gt;</span>: это те правила, которые задаются на более низких уровнях конфигурационной иерархии, в частности, на 
уровне приложения в файле web.config и которые были использованы в примере выше. Эти правила срабатывают во время события <code>BeginRequest</code></p></li>
</ul>
<p>Каждое правило, представленное элементом <code>&lt;rule&gt;</code>, состоит из трех частей:</p>
<ul>
<li><p><span class="bb">Pattern</span> – выражение, которому должна соответствовать строка запроса и которое задается в элементе <code>&lt;match&gt;</code></li>
<li><p><span class="bb">Conditions</span> – различные дополнительные условия, которым должен соответствовать URL-адрес. 
Например, значения HTTP-заголовков, пути к файлам и т.д.</p></li>
<li><p><span class="bb">Action</span> – действие, которое должно выполняться, если строка URL соответствует регулярному выражению в Pattern и 
...
создаваемый данным правилом, передается в конвейер IIS, а другие правила не будут обрабатываться.</p>
...
<h3>Pattern</h3>
<p>С помощью атрибута <code>url</code> определяется выражение, с которым сравнивается строка запроса.</p>
<p>Тип синтаксиса этого выражения задается атрибутом <code>patternSyntax</code> элемента <code>rule</code>, который может принимать следующие значения:</p>
<ul>
<li><p><span class="bb">ECMAScript</span>: стандарт синтаксиса регулярных выражений ECMAScript. Если мы не используем атрибут patternSyntax, то 
данное значение используется для правила по умолчанию.</p></li>
<li><p><span class="bb">Wildcard</span>: для поиска соответствия используются метасимволы</p></li>
</ul>
<p>Кроме атрибута url элемент match имеет еще ряд атрибутов:</p>
<ul><li><p><span class="bb">negate</span>: если имеет значение true, то правило применяется, если URL <span class="bb">НЕ</span> соответствует 
выражению в атрибуте <code>url</code> - то есть обратное действие. По умолчанию имеет значение false.</p></li>
<li><p><span class="bb">ignoreCase</span>: если имеет значение false, то правило учитывает регистр строки URL. 
По умолчанию имеет значение true, то есть регистр не учитывается.</p></li>
</ul>
<p>В выше использованном примере в качестве Pattern использовался следующий несложный элемент:</p>
<pre class="brush:xml;">
&lt;rule name="Remove WWW" patternSyntax="Wildcard" stopProcessing="true"&gt;
        &lt;match url="*" /&gt;
</pre>
<p>Звездочка означает, что строка запроса может состоять из любых символов.</p>
<h3>Определение условий</h3>
<p>Условия, задаваемые элементом <code>&lt;conditions&gt;</code>, определяют дополнительную логику оценки URL на соответствие правилу. Правило 
можно и не содержать элемента <code>&lt;conditions&gt;</code>, если он не нужен. В этом элементе определяется атрибут 
<span class="bb">logicalGrouping</span>, который может иметь одно из двух значений:</p>
<ul>
<li><p><code>MatchAll</code>: при таком значении для применения правила к URL должны быть соблюдены все условия</p></li>
<li><p><code>MatchAny</code>: для применения правила к URL должно быть соблюдено хотя бы одно условие</p></li>
</ul>
<p>Каждое отдельное условие задается с помощью элемента <code>&lt;add &gt;</code> и настраивается с помощью следующих атрибутов:</p>
<ul>
...
<code>input="{CACHE_URL}"</code>, где "CACHE_URL" представляет переменную сервера, хранящую запрошенный адрес url. Тут также могут использоваться и другие переменные сервера.</p></li>
...
<ul>
<li><p><span class="bb">Rewrite</span>: заменяет текущую строку запроса URL другой строкой</p></li>
<li><p><span class="bb">Redirect</span>: выполняет редирект, посылая клиенту статусный код 3хх.</p></li>
<li><p><span class="bb">CustomResponse</span>: отправляет клиенту определенный статусный код, а также может отправлять специфическое сообщение</p></li>
<li><p><span class="bb">AbortRequest</span>: сбрасывает подключение для текущего клиента</p></li>
</ul>
...
<p>В нашем случае происходит переадресация:</p>
<pre class="brush:xml;">
&lt;conditions&gt;
    &lt;add input="{CACHE_URL}" pattern="*://www.*" /&gt;
&lt;/conditions&gt;
...
<p>При создании условия в атрибуте pattern используются две звездочки, которые инкапсулируют два сегмента до и после "www". Эти два сегмента 
передаются в действие {C:1} и {C:2}. И с их помощью мы можем построить выходной адрес URL.</p>
...
<div class="nav"><p><a href="./21.5.php">Назад</a><a href="./">Содержание</a><a href="./22.2.php">Вперед</a></p></div></div>
<div class="socBlock">
