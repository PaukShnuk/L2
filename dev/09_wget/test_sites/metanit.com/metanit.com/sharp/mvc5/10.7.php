...
<html >
<head>
<title>AJAX-запросы с помощью jQuery в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="AJAX-запросы с помощью jQuery в ASP.NET MVC 5, обработка ответа сервера">
...
<h2>AJAX-запросы с помощью jQuery</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 10.11.2015</div>
...
<p>Кроме использования AJAX-форм и AJAX-ссылок мы можем выполнить запрос средствами самой библиотеки jQuery, не прибегая к ajax-хелперам.</p>
<p>Для примера создадим представление, в котором и будут осуществляться запросы AJAX:</p>
<pre class="brush:xml;">
...
<p>В данном случае мы в функции <code>jQuery</code> вешаем на элемент с <code>id=submit</code> (а это кнопка) обработчик события <code>click</code>. 
Строкой <code>var name = $('#search').val();</code> мы получаем введенное в текстовое поле значение. Следующей строчкой обрабатываем его - 
кодируем пробельные символы, так как иначе, если введенное слово будет содержать пробельные символы, то передаваемое в действие контроллера значение будет некорректно.</p>
<p>И в строке <code>$('#results').load('@Url.Action("BookSearch", "Home")?name='+name)</code> мы загружаем в элемент с <code>id=results</code> контент, 
полученный с помощью метода <code>load</code>.</p>
<p>Метод, к которому мы обращаемся, будет получать строковый параметр, осуществлять по нему поиск и передавать найденные данные в частичное представление:</p>
<pre class="brush:c#;">
...
<p>Вызываемое частичное представление могло бы выглядеть так:</p>
<pre class="brush:xml;">
...
<p>Теперь, мы можем осуществить поиск, введя какое-либо значение и нажав на кнопку поиска:</p>
<img src="./pics/10.5.png" />
<p>Выше был представлен только один способ проведения ajax-запросов с помощью jquery. Более подробно об ajax-запросах и jquery вы можете прочитать здесь: <a href="//metanit.com/web/jquery/6.1.php">Технология Ajax</a></p>
