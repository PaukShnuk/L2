...
<html >
<head>
<title>ASP.NET MVC 5 | Подключение React</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение React JS в ASP.NET MVC 5, определение компонентов">
...
<h1>React.JS</h1><h2>Подключение React в ASP.NET MVC 5</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 06.03.2019</div>
...
<p>В рамках руководства по <a href="//metanit.com/web/react">React</a> были рассмотрены основные моменты работы с библиотекой, в данной же главе 
рассмотрим, как применять React в связке с ASP.NET MVC 5, особенности работы именно с ASP.NET. Для работы с React в рамках ASP.NET MVC 5 нам потребуется библиотека <a href="https://reactjs.net" rel="nofollow">ReactJS.NET</a>.</p>
<p>Итак, создадим новый проект ASP.NET MVC 5 без аутентификации. Прежде всего добавим в него все необходимые пакеты. Для этого перейдем к пакетному менеджеру Nuget и 
через него установим пакет <span class="b">React.Web.Mvc4</span>:</p>
<img src="./pics/react1.png" alt="Установка ReactJS в ASP.NET MVC 5" />
<p>Вместе с этоим пакетом автоматически будет установлен и ряд других пакетов, таких как <span class="b">React.Core</span> и <span class="b">React.Web</span>. 
Причем неважно, что там стоит цифра 4 - React.Web.Mvc<span class="b">4</span>, данный пакет подходит и для MVC 4, и для MVC 5.</p>

<p>Также необходимо установить какой-нибудь JS-движок (рекомендуется использовать V8 или ChakraCore). Например, используем движок V8. Для этого нам также надо установить через 
Nuget следующие пакеты:</p>
<ul>
<li><p>JavaScriptEngineSwitcher.V8</p></li>
<li><p>JavaScriptEngineSwitcher.V8.Native.win-x64</p></li>
<li><p>JavaScriptEngineSwitcher.V8.Native.win-x86</p></li>
</ul>
<img src="./pics/react5.png" alt="React и JavaScriptEngineSwitcher.V8 в ASP.NET MVC 5" />
<p>После установки пакетов, зарегистрируем движок. Для этого перейдем к файлу <span class="b">ReactConfig.cs</span> в папке <span class="b">App_Start</span> 
(этот файл генерируется в проекте автоматически при установки пакета React.Web.Mvc4). По умолчанию данный файл выглядит примерно следующим образом:</p>
<pre class="brush:c#;">
using React;
...
<p>Изменим его содержимое следующим образом:</p>
<pre class="brush:c#;">
using React;
...
<p>В данном случае мы зарегистрировали движок V8.</p>
<p>Теперь добавим в проект в папку <span class="b">Scripts</span>, которая предназначена для скриптов js, новый файл <span class="b">app.jsx</span>:</p>
<img src="./pics/react2.png" alt="JSX и ReactJS в Visual Studio 2019" />
<p>Определим в файле <span class="ii">app.jsx</span> следующее содержимое:</p>
<pre class="brush:js;">
...
<p>Здесь определен компонент Hello, который выводит заголовок. Для определения компонента применяется код JSX. Подробнее про создание компонентов и прочие особенности работы с React можно посмотреть в соответствующем 
<a href="//metanit.com/web/react">руководстве</a>.</p>
<p>А метод <span class="b">ReactDOM.render</span> загружает компонент на веб-страницу в элемент с id="content".</p>
<p>Теперь создадим саму веб-страницу. Для этого возьмем представление <span class="b">Index.cshtml</span>, которое создается по умолчанию в папке Views/Home. Изменим 
код этого представления следующим образом:</p>
<pre class="brush:xml;">
@{
    Layout = null;
}
...
<p>Поскольку браузеры не воспринимает код JSX, кроме того не все функциональные возможности стандарта ES2015 поддерживаются браузерами, то для генерации из 
кода JSX стандартного кода JavaScript, который понятен браузерам, в представлении подключаются две библиотеки из cdn:</p>
<pre class="brush:xml;">
&lt;script crossorigin src="https://cdnjs.cloudflare.com/ajax/libs/react/16.8.0/umd/react.development.js"&gt;&lt;/script&gt;
&lt;script crossorigin src="https://cdnjs.cloudflare.com/ajax/libs/react-dom/16.8.0/umd/react-dom.development.js"&gt;&lt;/script&gt;
</pre>
<p>Кроме того, здесь определен элемент div c id="content", в котором будет визуализироваться код компонента. И в конце подключается сам файл app.jsx.</p>
<p>Запустим приложение на выполнение:</p>
<img src="./pics/react3.png" alt="React в ASP.NET MVC 5" />
...
<div class="nav"><p><a href="./24.19.php">Назад</a><a href="./">Содержание</a><a href="./25.2.php">Вперед</a></p></div></div>
<div class="socBlock">
...
$(function() { 
  ...
  $("#toggle-theme").on("click", function() {
	...
	$("li:contains('Глава 25.')").removeClass('lastExpandable').addClass('lastCollapsable').children('ul').css({"display":"block"});
	$("li:contains('Глава 25.')").children('div').removeClass('lastExpandable-hitarea').addClass('lastCollapsable-hitarea');
