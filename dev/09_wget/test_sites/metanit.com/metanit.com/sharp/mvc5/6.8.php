...
<html >
<head>
<title>Маршрутизация и вложенные ресурсы</title>
<meta charset="utf-8" />
...
<h2>Маршрутизация и вложенные ресурсы</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Нередко в различных соцсетях или других сервисах можно встретить ссылки такого типа <i>https://twitter.com/SomeName/following</i> или, например, 
<i>https://plus.google.com/u/0/12344566556/posts</i>. Эти две ссылки объединяет похожая схема маршрутизации - сначала идет общий путь, затем идентификатор 
пользователя (SomeName или 12344566556), и далее какое-то свойство, относящееся к пользователю - following (подписки в твиттере) или posts (все посты 
в Google+). Если мы посмотрим с точки зрения организации данных, то получится, что в некоторый класс пользователя хранит объекты постов или подписок. 
Таким образом, эти объекты оказываются как бы вложенными в объект пользователя. 
Посмотрим, как мы сможем создать систему маршрутов, чтобы обращаться к этим объектам как к вложенным и в asp.net mvc.</p>
<p>Допустим, у нас есть следующие классы, которые связаны по внешнему ключу и объекты которых храняться в базе данных:</p>
<pre class="brush:c#;">
public class Book
{
...
<p>Каждый объект Book хранит ссылку на связанного автора из таблицы авторов. Контекст данных мог бы выглядеть так:</p>
<pre class="brush:c#;">
...
<p>Теперь наша задача состоит в том, чтобы передать пользователю информацию о какой-либо книге и о ее авторе. Для этого создадим в контроллере HomeController 
два действия:</p>
<pre class="brush:c#;">
...
<p>Каждое действие получает нужную книгу по id и передает в строго типизированное представление объект книги или объект автора книги.</p>
<p>Теперь добавим нужные маршруты. Изменим код в файле RouteConfig.cs следующим образом:</p>
<pre class="brush:c#;">
...
<p>Итак, здесь мы добавляем два маршрута. Первый маршрут будет обрабатывать запрос типа <i>book/1/author</i> и сопоставлять его с 
методом <code>GetAuthor</code> (фактически будет представлять запрос <i>home/getauthor/1</i>). А второй маршрут будет обрабатывать запрос типа <i>home/book/1</i> и сопоставлять его с 
методом <code>GetBook</code>.</p>
<p>И теперь мы можем запустить приложения и получить информацию о первой книге, набрав в браузере <i>book/1/</i>:</p>
<img src="./pics/6.4.png" />
<p>А если мы наберем <i>book/1/author</i>, то получим информацию об авторе этой книги:</p>
<img src="./pics/6.5.png" />
<p>И мы также могли использовать атрибуты маршрутизации, чтобы создать подобные маршруты:</p>
<pre class="brush:c#;">
[Route("book/{id}")]
public ActionResult GetBook(int? id)
{
...
<p>А код в файле RouteConfig.cs выглядел бы следующим образом:</p>
<pre class="brush:c#;">
public class RouteConfig
{
    public static void RegisterRoutes(RouteCollection routes)
