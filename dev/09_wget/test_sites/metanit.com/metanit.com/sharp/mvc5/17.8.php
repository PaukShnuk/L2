...
<html >
<head>
<title>Валидация в диалоговых окнах в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Валидация в модальных диалоговых окнах с помощью jquery в ASP.NET MVC 5">
...
<h2>Валидация в диалоговых окнах</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В предыдущей теме было рассмотрено создание диалоговых окон для добавления и редактирования моделей. Но не был затронут вопрос валидации моделей при их 
создании в диалоговых окон. Рассмотрим, как мы можем добавить валидацию.</p>
<p>Возьмем модель Computer из прошлой темы и добавим ей несколько атрибутов валидации:</p>
<pre class="brush:c#;">
...
<p>Теперь изменим код контроллера для принятия и вывода моделей:</p>
<pre class="brush:c#;">
...
<p>Для простоты вместо бд используется статический список, и имеются всего лишь два действия - для вывода списка и для создания новой модели. При создании 
модели в зависимости от результата возвращается одно из частичных представлений.</p>
<p>Во-первых, добавим в проект библиотеку jQuery-UI.</p>
<p>Теперь добавим представления. Основное представление <i>Index.cshtml</i> для вывода списка:</p>
<pre class="brush:xml;">
...
<p>В отличие от механизма создания диалогового окна в прошлой теме, здес к диалоговому окну добавляется кнопка. И этой кнопке назначается обработчик нажатия, 
в котором срабатывает ajax-запрос с помощью функции <code> $.ajax()</code>. Функция сериализует данные формы и отправляет по адресу, указанному в параметре 
<code>url</code>. При успешной отправке с помощью функции <code>success: function (result) {...</code> мы можем обработать ответ от сервера. В данном случае 
ответ сервера просто добавляется в блок диалогового окна: <code>$("#dialogContent").html(result)</code>.</p>
<p>Теперь добавим частичное представление <i>Create.cshtml</i>:</p>
<pre class="brush:xml;">
...
<p>Здесь стандартная форма с хелперами валидации. Единственное отличие от стандартных форм - отстутствие кнопки отправки, так как мы ее создаем в коде javascript 
в представлении Index.cshtml.</p>
<p>И затем добавим частичное представление <i>Success.cshtml</i>, которое будет генерироваться при успешном добавлении модели:</p>
<pre class="brush:xml;">
...
<p>Здесь выводится сообщение о добавлении, и затем запускается скрипт, котоырй перенаправляет опять же на главную страницу, чтобы добавленные данные 
отобразились на странице. В качестве альтернативы можно было бы сделать загрузку данных таблицы моделей через ajax, чтобы не перегружать всю страницу.</p>
<p>Итак, запустим приложение и попробуем ввести такие данные, которые не пройдут валидацию:</p>
<img src="./pics/17.15.png" />
<p>И если модель пройдет валидацию, то мы получим сообщение о ее добавлении, и страница Index.cshtml будет перезагружена.</p>
