...
<html >
<head>
<title>Работа с ролями в AspNet Identity</title>
<meta charset="utf-8" />
...
<h2>Работа с ролями в AspNet Identity</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Кроме логинов и паролей система авторизации и аутентификации предлагает еще такой компонент для разграничения доступа, как роли. Роли 
позволяют создать группы пользователей с определенными правами и в зависимости от принадлежности к той или иной группе, разграничить доступ к ресурсам приложения. 
Посмотрим на примере.</p>
<p>Возьмем стандартный проект ASP.NET MVC 5 с моделью аутентификации Individual User Accounts. По умолчанию система AspNet Identity включает все необходимые средства 
для управления ролями. В начале главы мы уже разбирались с механизмом AspNet Identity. В стандартном проекте MVC 5 каждая роль представлена 
объектом класса IdentityRole, который реализует интерфейс IRole. По аналогии с управлением пользователями и классом UserManager для 
управления ролями предназначен класс RoleManager, который использует в качестве хранилища ролей объект RoleStore.</p>
<p>Задействуем систему ролей в проекте. Например, нам надо, чтобы у нас в базе данных 
уже был один пользователь, выполняющий роль админа.  Итак, проинициализируем базу данных начальными значениями для ролей и пользователей. Для этого добавим в папку Models новый класс </p>
<pre class="brush:c#;">
...
<p>Так как у нас по умолчанию уже определен в проекте контекст данных для управления пользователями и ролями ApplicationDbContext, то мы его указываем при создании 
объектов UserManager и RoleManager.</p>
<p>Метод <code>roleManager.Create</code> позволяет добавить роль в бд в таблицу AspNetRoles, а метод <code>userManager.AddToRole</code> устанавливает 
определенную роль для пользователя с переданным в метод id.</p>
<p>Обратите внимание, что мы можем добавить для одного пользователя множество ролей, как в данном случае</p>
<p>Ну и чтобы инициализация базы данных заработала, добавим ее вызов в файл Global.asax в метод App_Start:</p>
<pre class="brush:c#;">
...
<p>Теперь при запуске приложения в базе данных окажется один пользователь и две роли. И теперь мы можем устанавливать разграничение доступа по ролям на уровне контроллеров и 
методов:</p>
<pre class="brush:c#;">
...
<p>Теперь к методу About могут обратиться только администраторы. Но пойдем дальше. У нас есть еще одна роль, и мы, например, хотим, чтобы при регистрации 
всем пользователям присваивалась роль "user". Для этого изменим метод Register следующим образом:</p>
<pre class="brush:c#;">
[HttpPost]
[AllowAnonymous]
...
<p>Теперь выведем на веб-страницу все роли текущего пользователя:</p>
<pre class="brush:c#;">
[Authorize]
...
<p>С помощью метода <code>GetRoles()</code> класса UserManager мы можем получить набор ролей определенного пользователя. </p>
<p>Для удаления роли у пользователя используется метод <code>RemoveFromRole()</code>, который также определен у класса UserManager. Например, удалим 
у текущего пользователя роль "user":</p>
<pre class="brush:c#;">
