...
<html >
<head>
<title>Модели и база данных в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h1>Модели</h1>
<h2>Модели и БД</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Все сущности в приложении принято выделять в отдельные модели. В зависимости от поставленной задачи и сложности приложения 
...
книги и класс для покупки книги.</p>
<p>Модели представляют собой простые классы и располагаются в проекте в каталоге <span class="ii">Models</span>. Модели описывают логику данных. 
Например, модель представляющая книгу и ее покупку:</p>
<pre class="brush:c#;">
public class Book
{
    // ID книги
...
<p>Модель необязательно состоит только из свойств, кроме того, она может иметь конструктор, какие-нибудь вспомогательные методы. Но главное не перегружать 
класс модели и помнить, что его предназначение - описывать данные. Манипуляции с данными и бизнес-логика - это больше сфера контроллера.</p>
<p>Данные моделей, как правило, хранятся в базе данных. Для работы с базой данных очень удобно пользоваться фреймворком <span class="b">Entity Framework</span>, 
который позволяет абстрагироваться от написания sql-запросов, от строения базы данных и полностью сосредоточиться на логике приложения.</p>
<p>Если при создании проекта MVC 5 вы выберете в качестве типа аутентификации "No Authentication", то после создания проекта в него надо будет подключить 
EntityFramework через пакетный менеджер NuGet, как описывалось во второй главе.</p>
<p>В качестве альтернативы NuGet можно использовать консоль пакетного менеджера. Для этого в меню Visual Studio выберем View -&gt; Other Windows -&gt; 
Package Manager Console. После этого внизу студии откроется консоль пакетного менеджера. В ней введем такую команду:</p>
<p><code>PM&gt; Install-Package EntityFramework -Version 6.0.2</code></p>
<p>После ввода команды будет загружен и установлен пакет Entity Framework. Иногда этой консолью предпочтительнее пользоваться при установке пакетов, чем менеджером NuGet, 
...
NuGet может предложить только текущую версию.</p>
<p>Entity Framework поддерживает подход "Code first", который предполагает сохранение или извлечение информации из БД на SQL Server без создания схемы 
...
сохранять объекты этих классов.</p>
<p>Для подключения к базе данных через Entity Framework, нам нужен посредник - <strong>контекст данных</strong>. Контекст данных представляет собой класс, производный от 
класса <span class="bb">DbContext</span>. Контекст данных содержит одно или несколько свойств типа DbSet&lt;T&gt;, где T представляет тип объекта, хранящегося в базе 
данных. Например, создадим контекст данных для работы с вышеприведенными моделями:</p>
<pre class="brush:c#;">
using System;
...
<p>С помощью свойств <code>Books</code> и <code>Purchases</code> мы получаем доступ к данным соответствующих моделей, которые хранятся в базе данных.</p>
