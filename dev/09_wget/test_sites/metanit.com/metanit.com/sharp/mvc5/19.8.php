...
<html >
<head>
<title>Выполнение действий по таймеру</title>
<meta charset="utf-8" />
...
<h2>Выполнение действий по таймеру</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Одной из наиболее распространенных задач при создании модулей и обработчиков является выполнение некоторых повторяющихся действий по таймеру, 
например, почтовых рассылок, записей в логи и т.д. Создадим свой модуль, который будет нести подобную функциональность.</p>
<p>Например, определим модуль, который будет отправлять письмо на почту в определенное время:</p>
<pre class="brush:c#;">
...
<p>Данный модуль настроен на отправку письма в 1:30. В методе <code>Init()</code> запускается таймер, который каждые 30000 миллисекунд (30 секунд) вызывает метод 
<code>SendEmail()</code></p>
<p>Метод <code>SendEmail()</code> с помощью оператора <code>lock</code> определяет критическую секцию, доступ к которой одновременно возможен только для одного 
потока.</p>
<p>Для управления отправкой мы проверяем текущее время и значение переменной <code>sent</code> и в заивисимости от результатов проверки отправляем письмо.</p>
<p>Подобным образом можно определить и другие действия, выполняемые с определенной периодичностью.</p>
...
<div class="nav"><p><a href="./19.7.php">Назад</a><a href="./">Содержание</a><a href="./20.1.php">Вперед</a></p></div></div>
<div class="socBlock">
