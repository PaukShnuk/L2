...
<html >
<head>
<title>Создание проекта для юнит-тестов в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Создание проекта для юнит-тестов</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Посмотрим на примере, как создавать юнит-тесты. По умолчанию при создании проекта в любой версии Visual Studio 2013 для создаваемого проекта веб-приложения 
нам уже предлагается включить дополнительный проект с тестами с помощью опции <span class="bb">Add unit tests</span>:</p>
<img src="./pics/18.1.png" alt="Создание проекта тестов в Visual Studio" />
<p>Отметим данный флажок и создадим новый проект.</p>
<p>Если вдруг нам надо добавить тесты к уже существующему проекту, то мы можем добавить в решение новый тип проекта <span class="bb">Unit Test Project</span>. 
Как правило, для проекта тестов название выбирается по следующей схеме <i>[название_главного_проекта].Tests</i>:</p>
<img src="./pics/18.10.png" alt="Добавление тестов к проекту в Visual Studio" />
<p>При одновременном создании обоих проектов по умолчанию тестовый проект уже содержит ряд тестов:</p>
<img src="./pics/18.2.png" alt="Юнит-тесты в Visual Studio" />
<p>В частности для контроллера HomeController в проекте тестов будет создан класс <span class="bb">HomeControllerTest</span>:</p>
<pre class="brush:c#;">
[TestClass]
...
<p>Класс тестов имеет атрибут <code>[TestClass]</code>, а каждый метод, проводящий тестирование, - атрибут <code>[TestMethod]</code>.</p>
...
<p>Далее в секции Act выполняет само действие, которое надо протестировать, а именно генерация представления: <code> ViewResult result = controller.Index() as ViewResult</code>. Так как метод Index контроллера 
возвращает объект ActionResult, то его еще надо привести к объекту ViewResult.</p>
<p>Чтобы проверить на выполнение предыдущее действие, в секции Assert вызывается метод <code>Assert.IsNotNull(result)</code>. Этот метод проверяет, был ли сгенерирован объект представления 
в предыдущей секции. Класс Assert еще имеет ряд методов, которые позволяют верифицировать результат.</p>
<p>Модель тестов <span class="bb">Arrange-Act-Assert</span> представляет просто особенность тестирования в Visual Studio, а целую парадигму тестирования:</p>
<ul>
<li><p><span class="bb">Arrange</span>: подготовка среды, в которой выполняется код</p></li>
<li><p><span class="bb">Act</span>:  тестирование кода (обычно представляет одну строку кода)</p></li>
<li><p><span class="bb">Assert</span>: убеждаемся, что результат теста именно тот, что мы и ожидали</p></li>
</ul>
<p>Этот простейший стандартный тест не охватывает всех возможных ошибок, например, были ли сгенерировано нужное представление. Он просто призван дать 
начальное понимание работы тестов.</p>
<p>Теперь запустим тест на выполнение. Для этого перейдем в окно <b>Test Explorer</b> и нажмем в нем на кнопку <span class="bb">Run All</span>:</p>
<img src="./pics/18.3.png" alt="Test Explorer в Visual Studio" />
<p>Если оно по умолчанию не открыто, запустить тесты можно через меню <span class="bb">Test-&gt;Run-&gt;All Tests</span>:</p>
<img src="./pics/18.4.png" />
<p>Если все нормально, то обозреватель тестов сигнализирует нам зеленым цветом, что все тесты успешно пройдены:</p>
<img src="./pics/18.5.png" />
<p>Теперь создадим собственные тесты.</p>
...
<div class="nav"><p><a href="./18.1.php">Назад</a><a href="./">Содержание</a><a href="./18.3.php">Вперед</a></p></div></div>
<div class="socBlock">
