...
<html >
<head>
<title>Строго типизированные представления в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Строго типизированные представления в ASP.NET MVC 5, определение модели представлений, view model">
...
<h2>Строго типизированные представления</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В предыдущих примерах для передачи информации из контроллера в представление использовался объект ViewBag:</p>
<pre class="brush:xml;">
...
<p>Здесь мы получаем доступ к элементам коллекции, заключенной в ViewBag.Books, с помощью переменной с ключевым словом var - то есть тип переменной 
у нас не задан явно и выводится компилятором. То же самое мы могли бы указать тип модели явно, применив полное имя типа модели:</p>
<pre class="brush:xml;">
...
<p>Хотя примеры с объектом ViewBag работают как надо, но есть и другой способ, иногда более предпочтительный, который заключается в использовании <span class="bb">строго типизированных представлений</span>. 
Подобные представления позволяют передавать данные не через объект ViewBag, а напрямую в представление через параметр метода View. 
Код метода контроллера мог бы выглядеть так:</p>
<pre class="brush:c#;">
...
<p>Теперь, чтобы связать представление с передаваемым параметром, надо добавить в представление директиву <span class="bb">@model</span> с указанием типа передаваемых 
данных. Поскольку books представляет тип IEnumerable&lt;Book&gt;, то представление будет выглядеть так:</p>
<pre class="brush:xml;">
...
<p>Объект <span class="bb">Model</span> представляет тип модели, указанной в директиве <code>@model</code>, и будет хранить переданные из контроллера данные.</p>
 <p>Но чтобы не писать полностью имя типа модели, мы можем импортировать пространство имен 
в представлении:</p>
<pre class="brush:xml;">
...
<p>Кроме того, мы можем автоматически создать строго типизированное представление, указав в диалоговом окне при создании представления 
соответствующие параметры:</p>
<img src="./pics/4.2.png" alt="Строго типизированное представление в ASP.NET MVC 5" />
<p>Для этого в поле Template надо выбрать любой другой шаблон, кроме Empty (without model), и после этого указать нужный класс модели и контекста данных. И если 
мы выберем шаблон List, то автоматически сгенерированное представление будет по своему функционалу идентично ранее рассмотренному представлению 
с выводом книг.</p>
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/NelSzou6lqU"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 17. Строго типизированные представления в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
