...
<html >
<head>
<title>Фильтры авторизации в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Фильтры авторизации в ASP.NET MVC 5">
...
<h2>Фильтры авторизации</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Фильтры авторизации срабатывают после фильтров аутентификации и до запуска остальных фильтров и вызова методов действий. Цель фильтров авторизации - 
разграничить доступ пользователей, чтобы к определенным ресурсам приложения имели доступ только определенные пользователи.</p>
<p>Фильтры авторизации реализуют интерфейс <code>IAuthorizationFilter</code>:</p>
<pre class="brush:c#;">
...
<p>В ASP.NET MVC 5 имеется встроенная реализация данного фильтра - <span class="bb">AuthorizeAttribute</span>. Он применяется посредством установки атрибута 
[Authorize] над контроллером или методом контроллера. Например:</p>
<pre class="brush:c#;">
[Authorize]
...
<p>Атрибут <span class="bb">AllowAnonymous</span> позволяет разрешить доступ к ресурсам для анонимных, не авторизованных пользователей. 
Например, если к контроллеру применяется атрибут Authorize, то фреймворк сначала будет смотреть, есть ли у пользователя необходимые права для доступа к методу контроллера. 
...
<p>Система авторизации в mvc позволяет настроить доступ к ресурсам сайта для отдельных пользователей или групп пользователей. 
Для подобной настройки можно использовать два свойства атрибута AuthorizeAtribute:</p>
<ul>
...
<p>Массивы пользователей и ролей помогают провести детальную авторизацию. В базовом классе атрибута определены переменные для ролей и пользователей, 
...
Так, если у нас используются следующие роли: <code>Roles="admin, moderator"</code>, то свойство <code>base.Roles</code> также будет иметь 
значение <code>"admin, moderator"</code>. И, выполняя манипуляции со строкой - разбиение по запятой, отсечение начальных и конечных пробелов, мы можем 
поместить все переданные роли  в массив.</p>
<p>Другим моментом использования атрибута является применение объекта контекста HttpContextBase. С его помощью 
можно получить контекст запроса и информацию о пользователе, например, аутентифицирован ли пользователь в системе.</p>
<p>Применение фильтра подобно применению стандартной версии:</p>
<pre class="brush:c#;">
...
<p>Но можно и сочетать наследование атрибута и переопределение интерфейса IAuthorizationFilter:</p>
<pre class="brush:c#;">
...
<p>Хотя в предыдущих примерах переопределение авторизации по сути не имело смысла, однако все-таки может потребоваться реализация своего атрибута. 
...
<p>Переопределенный атрибут также позволяет использовать роли и пользователей:</p>
<pre class="brush:c#;">
