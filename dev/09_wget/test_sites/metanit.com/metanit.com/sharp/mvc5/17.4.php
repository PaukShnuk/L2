...
<html >
<head>
<title>Пагинация в JqGrid</title>
<meta charset="utf-8" />
...
<h2>Пагинация в JqGrid</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Изменим представление из предыдущей темы следующим образом:</p>
<pre class="brush:xml;">
@{
    Layout = null;
}
...
<p>В отличие от предыдущего примера здесь добавлен еще один скрипт - скрипт локализации grid.locale-ru.js, который установился в проект вместе с другими файлами 
JqGrid в каталог <i>Scripts/i18n</i>. При необходимости можно выбрать любой другой файл локализации. В частности, есть также локализации для белорусского и украинского языков. 
Кроме того, можно самим создать на основе имеющейся собственную локализацию.</p>
<p>Также на страницу добавлен новый элемент, который будет содержать функционал пагинации: <code>&lt;div id="jpager"&gt;&lt;/div&gt;</code></p>
<p>А в определении грида указывается, что именно в этом элементе будет применяться пагинация для грида: <code>pager: '#jpager',</code></p>
<p>В итоге мы получим грид с элементами постраничной навигации:</p>
<img src="./pics/17.8.png" alt="Пагинация и pager в JqGrid" />
