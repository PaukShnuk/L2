...
<html >
<head>
<title>Архитектура приложений ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Onion-архитектура. Часть 1</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Термин "Onion Architecture" ("луковая" архитектура) был предложен Джеффри Палермо (Jeffrey Palermo) еще в 2008 году. 
Спустя годы данная концепция стала довольно  популярной и является одной из наиболее применяемых типов архитектуры при построении 
приложения на ASP.NET.</p>
<p>Onion-архитектура представляет собой разделение приложения на уровни. При чем есть один независимый уровень, который находится в центре 
архитектуры. От этого уровня зависит второй уровень, от второго - третий и так далее. 
То есть получается, что вокруг первого независимого уровня наслаивается второй-зависимый. Вокруг второго наслаивается третий, который также может зависеть и от первого. 
Образно это может быть выражено в виде лука, в котором также есть сердцевина, вокруг которого наслаиваются все остальные слои, вплоть до шелухи.</p>
<p>Количество уровней может отличаться, но в центре всегда находится модель домена (Domain Model), то есть те классы моделей, которые используются в приложении и объекты которых хранятся в базе данных:</p>
<img src="./pics/onion_architecture.png" alt="Onion-архитектура в ASP.NET" />
<p>Первый уровень вокруг модели домена образуют интерфейсы, которые управляют работой с моделью домена. Обычно это интерфейсы репозиториев, через которые 
мы взаимодействуем с базой данных.</p>
<p>Внешний уровень представляет такие компоненты, которые очень часто изменяются. Обычно внешний уровень образуют пользовательский 
интерфейс, тесты, какие-то вспомогательные классы инфраструктуры приложения. К этому уровню также относятся конкретные реализации интерфейсов, объявленных на нижележащих уровнях. 
Например, реализация интерфейса репозитория, который объявлен на уровне Domain Services. Вообще все внутрение уровни, которые можно объединить 
в Application Core, определяют только интерфейсы, а конкретная реализация этих интерфейсов располагается на внешнем уровне.</p>
<p>Также стоит отметить, что все внешние хранилища, как базы данных, файлы, внешние веб-сервисы, от которых мы можем получать данные, - все это является 
внешним по отношению к архитектуре.</p>
<p>Для более подробного рассмотрения данного типа архитектуры создадим обычный проект ASP.NET MVC 5, который будет называться OnionApp.</p>
<img src="./pics/onion_architecture2.png" />
<p>Пока это монолитное приложение, в котором весь код размещен в одном проекте. Теперь добавим в <span class="bb">решение</span> (не в проект) новую 
папку. Назовем ее <span class="bb">Domain</span>. Затем добавим в папку новый проект. В качестве типа проекта выберем тип <span class="bb">Class Library</span>, 
а в качестве его названия укажем <span class="bb">OnionApp.Domain.Core</span>:</p>
<img src="./pics/onion_architecture3.png" />
<p>Добавим в новый проект класс, представляющий книгу, который и будут представлять Domain Model:</p>
<pre class="brush:c#;">
...
<img src="./pics/onion_architecture4.png" />
<p>Затем добавим в папку Domain новый проект также по типу Class Library, а в качестве его названия укажем <span class="bb">OnionApp.Domain.Interfaces</span>. 
Затем добавим в этот проект ссылку на вышеопределенный проект <code>OnionApp.Domain.Core</code> и также добавим новый интерфейса:</p>
<pre class="brush:c#;">
using System;
...
<p>Этот интерфейс и составляет уровень <span class="bb">Domain Services</span> и зависит от уровня Domain Model.</p>
<p>И на данный момент проекты выглядят так:</p>
<img src="./pics/onion_architecture5.png" />
<p>При создании архитектуры приложения надо понимать, что реальное количество уровней здесь весьма условно. В зависимости от масштаба задач 
уровней может быть и больше, и меньше. Однако важно понимать сам принцип, что в центре у нас модели домена, а все остальное зависит от них. Каждый внешний 
уровень может зависеть от внутреннего, но не наоборот.</p>
<p>На выше представленной схеме между внешним уровнем и уровнем Domain Services есть еще уровень API или интерфейсов бизнес-логики 
приложения - уровень Application Services. Этот уровень может включать интерфейсы вспомогательных классов. Например, покупка книги может представлять собой объект, 
который в зависимости от способа оплата (наличкой, через кредитную карту, через электронные деньги) может включать тот или иной функционал. 
И, возможно, было бы неплохо определить общий интерфейс покупки, а в зависимости от типа магазина использовать его конкретную реализацию. Поэтому добавим в решение новую папку, которую 
назовем <span class="bb">Services</span>. В эту папку добавим новый проект по типу Class Library, который назовем <span class="bb">OnionApp.Services.Interfaces</span></p>
<p>И добавим в этот проект интерфейс IOrder:</p>
<pre class="brush:c#;">
using OnionApp.Domain.Core;
...
<img src="./pics/onion_architecture6.png" />
<p>Данный проект также имеет зависимость от классов проекта <code>OnionApp.Domain.Core</code>. А интерфейс IOrder, представляющий процесс 
покупки и оформления заказа, использует эти классы в методе <code>MakeOrder()</code>. Предполагается, что в метод передается объект купленной книги.</p>
<p>Теперь перейдем к созданию внешнего уровня, который и будет реализовывать данные интерфейсы. Для этого добавим в решение папку 
<span class="bb">Infrastructure</span> и затем в нее добавим новый проект по типу Class Library, который назовем <span class="bb">OnionApp.Infrastructure.Data</span>.</p>
<p>Данный проект будет реализовывать интерфейсы, объявленные на нижних уровнях, и связывать их с хранилищем данных. В качестве хранилища данных будет 
использоваться бд MS SQL Server, с которой мы будем взаимодействовать через Entity Framework. Поэтому добавим в этот проект через nuGet все пакеты Entity Framework. 
Также добавим в проект ссылки на проекты OnionApp.Domain.Core и OnionApp.Domain.Interfaces.</p>
<p>После этого добавим в проект новый класс <span class="bb">OrderContext</span>:</p>
<pre class="brush:c#;">
using System;
using System.Collections.Generic;
...
<p>Также добавим класс репозитория BookRepository:</p>
<pre class="brush:c#;">
using System;
using System.Collections.Generic;
using System.Linq;
...
<img src="./pics/onion_architecture7.png" />
...
<div class="nav"><p><a href="./22.3.php">Назад</a><a href="./">Содержание</a><a href="./23.2.php">Вперед</a></p></div></div>
<div class="socBlock">
