...
<html >
<head>
<title>Добавление ролей в ASP.NET Identity в ASP.NET MVC</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление ролей в ASP.NET Identity в ASP.NET MVC 5, ограничение доступа к ресурсам в зависимости от роли пользователя">
...
<h2>Добавление ролей в ASP.NET Identity</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Сущность ролей в ASP.NET Identity представлена классом IdentityRole, который реализует интерфейс IRole. И мы можем продолжать использовать IdentityRole, 
но при необходимости также можем переопределить, добавив в него новые свойства. Итак, возьмем проект из прошлой темы и добавим в него функциональность 
управления ролями.</p>
<p>Для начала добавим в папку Models новый класс ApplicationRole:</p>
<pre class="brush:c#;">
...
<p>Новый класс наследует весь функционал от IdentityRole плюс добавляет новое свойство <code>Description</code>, которое будет содержать описание роли.</p>
<p>Для управления ролями используется менеджер ролей RoleManager.  Поэтому добавим в папку Models новый класс ApplicationRoleManager:</p>
<pre class="brush:c#;">
...
<p>Опять же наследуем функционал от уже имеющегося класса RoleManager. Метод <code>Create</code> позволит классу приложения OWIN создавать 
экземпляры менеджера ролей для обработки каждого запроса, где идет обращение к хранилищу ролей RoleStore.</p>
<p>И теперь нам надо зарегистрировать менеджер ролей в классе приложения OWIN. Поэтому откроем файл <i>Startup.cs</i>, который по предыдущим темам 
у нас находился в папке App_Start, и изменим его содержимое следующим образом:</p>
<pre class="brush:c#;">
...
<p>Благодаря регистрации менеджер ролей будет использовать тот же контекст данных, что и менеджер пользователей.</p>
<p>Если мы создаем систему ролей в приложении после первого запуска приложения, то нам надо будет произвести миграцию, так как контекст данных 
у нас изменился в силу изменения системы ролей. Поэтому в Visual Studio в окне Package Manager Console введем команду: enable-migrations и нажмем Enter.</p>
<p>Теперь нам надо создать саму миграцию. Там же в консоли Package Manager Console введем команду:</p>
<pre>
PM&gt; Add-Migration "DataMigration"
</pre>
<p>Visual Studio автоматически сгенерирует класс миграции:</p>
<pre class="brush:c#;">
...
<p>Теперь создадим контроллер, который будет выполнять стандартные действия с ролями:</p>
<pre class="brush:c#;">
...
<p>Это стандартный CRUD-контроллер, выполняющий чтение, удаление, редактирование и добавление ролей. Вначале для взаимодействия с менеджером ролей 
мы получаем его объект из контекста OWIN:</p>
<pre class="brush:c#;">
HttpContext.GetOwinContext().GetUserManager&lt;ApplicationRoleManager&gt;()
</pre>
<p>Затем методы менеджера ролей используются для управления ролями. Обратите внимание, что из представлений в методы Create и Edit мы получаем не объект 
ApplicationRole, а специальные модели EditRoleModel и CreateRoleModel, который могут выглядеть так:</p>
<pre class="brush:c#;">
...
<p>Использование моделей позволит избежать различных проблем с контекстом данных и управлением объектами, которые могли возникнуть, если бы мы напрямую 
использовали бы ApplicationRole.</p>
<p>И представления будут выглядеть стандартно. Представление <i>Index.cshtml</i>:</p>
<pre class="brush:xml;">
...
<p>Представление <i>Create.cshtml</i>:</p>
<pre class="brush:xml;">
...
<p>И представление для редактирования <i>Edit.cshtml</i>:</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./12.18.php">Назад</a><a href="./">Содержание</a><a href="./12.20.php">Вперед</a></p></div></div>
<div class="socBlock">
