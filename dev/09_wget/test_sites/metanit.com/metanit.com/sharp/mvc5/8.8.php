...
<html >
<head>
<title>Примеры фильтров действий в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Примеры фильтров действий</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Рассмотрим некоторые фильтры действий, которые могут использоваться в различных ситуациях.</p>
<h3>Фильтр кэширования</h3>
<pre class="brush:c#;">
using System.Web;
...
<p>Данный фильтр во многом аналогичен встроенному фильтру кэширования OutputCache за тем исключением, что в данном случае мы сами можем 
определить все параметры кэширования при различных условиях. Например, определить разную логику кэширования для разных браузеров и т.д.</p>
<p>Для управления временем задается свойство Duration, принимающее время в секундах. Применение фильтра:</p>
<pre class="brush:c#;">
[Cache (Duration=300)]
...
<p>В итоге при обращении к методу мы получим следующие заголовки:</p>
<pre>
HTTP/1.1 200 OK
Cache-Control: public, must-revalidate, proxy-revalidate, max-age=300
//..............................
</pre>
<h3>Фильтр компрессии</h3>
<pre class="brush:c#;">
using System.Web;
using System.Web.Mvc;
...
<p>С помощью заголовка Accept-Encoding в фильтре получаем, какой алгоритм сжатия поддерживается браузером: deflate или gzip (нередко поддерживаются оба, 
тогда выбираем gzip). Затем в зависимости от условий используем либо класс GZipStream, либо DeflateStream для сжатия выходного потока.</p>
<h3>Фильтр пробелов</h3>
<pre class="brush:c#;">
using System;
...
<p>Данный фильтр получает выходной поток в виде объекта <code>filterContext.HttpContext.Response.Filter</code> и передает его для обработки в объект 
<code>SpaceCleaner</code>. SpaceCleaner с помощью регулярных выражений находит все пробелы, удаляет их и измененные данные записывает в выходной поток. 
Таким образом, происходит минификация html-контента, объем данных немного сжимается.</p>
<h3>Фильтр логгирования</h3>
<p>Вначале определим модель, которая будет описывать посетителя сайта, и контекст данных:</p>
<pre class="brush:c#;">
...
<p>Тогда фильтр, производящий логгирование в базу данных, будет выглядеть следующим образом:</p>
<pre class="brush:c#;">
using System.Web.Mvc;
...
<img src="./pics/8.1.png" alt="Custom Action filters in ASP.NET MVC 5" />
...
<div class="nav"><p><a href="./8.5.php">Назад</a><a href="./">Содержание</a><a href="./8.6.php">Вперед</a></p></div></div>
<div class="socBlock">
