...
<html >
<head>
<title>ASP.NET MVC 5 | Рисовалка на SignalR</title>
<meta charset="utf-8" />
<meta name="description" content="Рисовалка на SignalR в ASP.NET MVC 5">
...
<h2>Рисовалка на SignalR</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Чат - наиболее показательное решение, которое можно сделать с помощью SignalR. Однако чатами все не ограничивается. И попробуем создать 
графическое приложение с использованием SignalR, в котром есть общее полотно, и каждый подключившийся к приложению может что-то нарисовать, и все будут 
это видеть.</p>
<p>Итак, создадим проект ASP.NET MVC 5 с типом аутентификации No Authentication. Я назвал свой проект <i>SignalRDraw</i>. И сразу же 
через менеджер NuGet добавим в проект библиотеку SignalR.</p>
<p>Сразу же создадим клиента для приложения. Для этого добавим в проект новый файл <i>index.html</i> прямо в корень проекта. Затем нажмем на этот 
файл правой кнопкой мыши и в контекстном меню выберем <span class="bb">Set as Start Page</span> (Сделать начальной страницей). Таким образом, 
мы не будем использовать ни контроллеры, ни их представления. Весь функционал клиента у нас будет содержать эта веб-страничка. А каталоги 
Controllers и Views в принципе в обще можно удалить.</p>
<p>Теперь изменим код файла <i>index.html</i> на следующий:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
...
<p>Разметка html содержит всего один элемент canvas - полотно для рисования, больше нам ничего не надо.</p>
<p>В коде javascript вначале определяются две модели: <code>drawGame</code> - для использования внутри клиента и <code>data</code> - для взаимодействия с сервером.</p>
<p>Чтобы непоседственно рисовать примитивы на полотне, нам надо получить canvas и его контекст:</p>
<pre class="brush:xml;">
var canvas = document.getElementById('drawingpad');
var ctx = canvas.getContext('2d');
</pre>
<p>Далее получаем ссылку на хаб. В данном случае класс хаба на сервере, который мы позже создадим, будет называться DrawHub. А с помощью функции 
<code>addLine</code> клиент будет получать с сервера данные и по ним рисовать линии.</p>
<pre class="brush:xml;">
var chat = $.connection.drawHub;
chat.client.addLine = function (data) {
...
<p>Затем в функции <code>$.connection.hub.start().done(function ()...</code> добавляются обработчики событий мыши, с  поомщью которых можно рисовать линии и 
отправлять сообщения об этих линиях на сервер.</p>
<p>Теперь определим на уровне сервера модель, которая будет представлять данные, получаемые от клиента. Добавим в папку Models новый класс Data:</p>
<pre class="brush:c#;">
using System;
...
<p>Каждое свойство для связи с моделью data, определенной в javscript, имеет атрибут <span class="bb">JsonProperty</span>.</p>
<p>Теперь добавим в проект прямо в корень новый класс хаба:</p>
<pre class="brush:c#;">
using System;
...
<p>С помощью метода Clients.AllExcept() мы можем исключить из рассылки сообщений того клиента, который собственно и прислал сообщений, и таким образом, 
избежать ситуации, когда у него будет дублироваться нарисованная линия. А метод <code>addLine</code> собственно и выполняет рыссылку всем 
подключенным клиентам и задействует на клиенте одноименный метод <code>addLine</code>, который выполняет рисование.</p>
<p>Ну и также добавим в проект класс Startup, чтобы задействовать SignalR:</p>
<pre class="brush:c#;">
using System;
...
<img src="./pics/16.8.png" />
<p>И теперь мы можем запустить проект и открыть запущенную страницу в разных браузерах и протестировать приложение:</p>
<img src="./pics/16.9.png" />
...
<div class="nav"><p><a href="./16.3.php">Назад</a><a href="./">Содержание</a><a href="./16.5.php">Вперед</a></p></div></div>
<div class="socBlock">
