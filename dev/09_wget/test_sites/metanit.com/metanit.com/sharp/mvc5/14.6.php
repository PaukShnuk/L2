...
<html >
<head>
<title>Модальные окна в Bootstrap и ASP.NET MVC</title>
<meta charset="utf-8" />
...
<h2>Модальные окна</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Фреймворк Bootstrap предлагает прекрасные возможности по созданию модальных окон. По своей функциональности они напоминают модальные окна в jQuery UI.</p>
<p>Для использования модальных окон создадим новый проект. Наш проект будет использовать модель, описывающую модель компьютера:</p>
<pre class="brush:c#;">
...
<p>Изменим контроллер HomeController следующим образом:</p>
<pre class="brush:c#;">
...
<p>Есть обычный метод Index, который возвращает стандартное представление со списком, и есть метод Details, который генерирует частичное представление с данными 
о выбранной модели. Это частичное представление и будет использоваться модальным окном.</p>
<p>Теперь создадим представления. Изменим представление <i>Index.chtml</i> следующим образом:</p>
<pre class="brush:xml;">
...
<p>Нажатие на любой элемент списока приведет к тому, что сработает обработчик, определенный в коде javascript. Этот обработчик будет загружать 
содержимое частичное представления, которое мы чуть позже определим. Данные загружаются в элемент с <code>id="dialogContent"</code>. 
И чтобы этот элемент расценивался как модальное диалоговое окно, у него устанавливается класс <code>modal-dialog</code>.</p>
<p>Этот блок определен внутри блока <code>modDialog</code>, для которого установлены классы <code>modal fade</code>.</p>
<p>Теперь создадим частичное представление <i>Details.cshtml</i>, которое будет использоваться модальным окном:</p>
<pre class="brush:xml;">
...
<p>Здесь опять же используются классы bottstrap для формирования содержимого модального окна.</p>
<p>Все содержимое помещается в блок div с классом <code>modal-content</code>. Для создания заголовочной части модального окна используется блок с классом 
<code>modal-header</code>. В частности кроме собственно заголовка мы можем определить кнопку закрытия окна, присвоив ей класс <code>close</code>. 
И также здесь указываем с помощью атрибута <code>data-dismiss</code> элемент, который должен "закрываться" - то есть блок div с классом <code>modal</code>.</p>
<p>Непосредственно содержимое помещается в блок <code>modal-body</code>.</p>
<p>Теперь мы можем протестировать приложение. При нажатии на любую ссылку моделей должно появиться модальное окно с подробной информацией:</p>
<img src="./pics/14.24.png" />
<p>И при нажатии на крестик, модальное окно закроется. Таким образом, мы можем использовать bootstrap для создания модальных окон.</p>
...
<div class="nav"><p><a href="./14.5.php">Назад</a><a href="./">Содержание</a><a href="./16.1.php">Вперед</a></p></div></div>
<div class="socBlock">
