...
<html >
<head>
<title>ASP.NET MVC 5 | Onion-архитектура. Часть 2</title>
<meta charset="utf-8" />
<meta name="description" content="Многоуровневая Onion архитектура в ASP.NET MVC 5, взаимодействие между уровнями">
...
<h2>Onion-архитектура. Часть 2</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 21.03.2018</div>
...
<p>Также у нас есть интерфейс IOrder. И для его реализации определим в папке Infrastructure дополнительный проект по типу Class Library, 
который назовем <span class="b">OnionApp.Infrastructure.Business</span>. Добавим к нему ссылки на проекты OnionApp.Domain.Core и 
OnionApp.Services.Interfaces.</p>
<img src="./pics/onion_architecture8.png" />
<p>Здесь мы можем определить разные реализации интерфейса IOrder. Например:</p>
<pre class="brush:c#;">
...
<p>Для простоты примера я опустил код. Подобным образом можно создать и другие возможные реализации:</p>
<pre class="brush:c#;">
using OnionApp.Domain.Core;
using OnionApp.Services.Interfaces;
...
<p>Теперь, когда все необходимые проекты готовы, мы можем перейти к созданию веб-интерфейса для управления функционалом. Для этого 
будем использовать имеющийся по умолчанию проект OnionApp. В этом проекте уже есть стандартный HomeController. Изменим его таким образом, чтобы 
он использовал зависимости из ранее созданных проектов:</p>
<pre class="brush:c#;">
using System.Web;
...
<p>Но чтобы связать зависимости, воспользуемся контейнером зависимостей Ninject. Добавим в проект OnionApp через NuGet пакет Ninject.MVC5 также, 
как это было показано в статье <a href="//metanit.com/sharp/mvc5/21.2.php">IoC-контейнер Ninject</a>.</p>
<p>Чтобы зарегистрировать зависимости, создадим в проекте OnionApp каталог Util, в который добавим новый класс <span class="b">NinjectRegistrations</span>:</p>
<pre class="brush:c#;">
...
<p>Данный класс связывает IBookRepository с BookRepository и IOrder с CacheOrder.</p>
<p>Затем перейдем к файлу <span class="b">Global.asax</span> и изменим его следующим образом:</p>
<pre class="brush:c#;">
using Ninject;
using Ninject.Modules;
...
<p>И, поскольку наше веб-приложение использует Entity Framework, то добавим в проект OnionApp пакет Entity Framework и в файле web.config пропишем 
...
<p>То есть несмотря на то, что класс контекста данных OrderContext определен в проекте OnionApp.Infrastructure.Data, обращение к контексту 
данных через репозиторий будет происходить в проекте OnionApp, поэтому в нем и прописываем строку подключения.</p>
<p>В итоге у нас получилось шесть проектов:</p>
<img src="./pics/onion_architecture9.png" />
<p>Таким образом, мы создали приложение с onion-архитектурой, которую схематически можно выразить следующим образом:</p>
<img src="./pics/onion_architecture10.png" />
