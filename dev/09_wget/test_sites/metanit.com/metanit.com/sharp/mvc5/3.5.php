...
<html >
<head>
<title>Переадресация в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Переадресация в ASP.NET MVC 5, классы RedirectResult, RedirectToAction, RedirectToRoute, отправка кодов статуса и ошибок с помощью HttpStatusCodeResult">
...
<h2>Переадресация и отправка кодов статуса и ошибок</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Существует два вида переадресации: временная и постоянная. И в зависимости от вида переадресации при ее выполнении сервер посылает 
браузерам один из двух кодов HTTP:</p>
<ul>
<li><p>статусный код 301 представляет постоянную переадресацию. При данном типе переадресации предполагается, что запрашиваемый документ 
окончательно перемещен в другое место. После получения данного статусного кода браузер может автоматически настраивать запросы на новый ресурс, 
даже если старый ресурс со временем перестанет применять переадресацию. Поэтому данный способ использовать нежелательно.</p></li>
<li><p>статусный код 302 представляет временную переадресацию. При временной переадресации считается, что запрашиваемый документ 
временно перемещен на другую страницу.</p></li>
</ul>
<p>В обоих случаях для переадресации будет использоваться объект <strong>RedirectResult</strong>, однако метод, возвращающий данный объект, будет отличаться.</p>
<p>Для временной переадресации применяется метод <span class="bb">Redirect</span>:</p>
<pre class="brush:c#;">
...
<p>Для постоянной переадресации подобным образом используется метод RedirectPermanent:</p>
<pre class="brush:c#;">
public RedirectResult SomeMethod()
{
...
<p>При этом нам необязательно возвращать из метода объект RedirectResult. Нередко возникает ситуация, когда в зависимости от некоторых условий требуется направить 
пользователя по одному адресу, либо переадресовать на другой ресурс. Типичная ситуация: авторизация пользователя - если он авторизован, то ему 
отображается требуемая веб-страница, а если нет, то он перенаправляется на страницу для логина. Например:</p>
<pre class="brush:c#;">
...
<p>Если в качестве параметра будет передано число больше 3, то произойдет редирект на Home/Index. В остальных случаях пользователю будет возвращаться представление.</p>
<p>Еще один класс для создания переадресации -  <span class="bb">RedirectToRouteResult</span> - позволяет выполнить более детальную настройку перенаправлений. 
Он возвращается двумя методами: <span class="bb">RedirectToAction</span> и <span class="bb">RedirectToRoute</span>.</p> 
<p>Метод RedirectToRoute позволяет произвести перенаправление по определенному маршруту внутри домена:</p>
<pre class="brush:c#;">
...
<p>Метод <span class="bb">RedirectToAction</span> позволяет перейти к определенному действию определенного контроллера. Он также позволяет задать передаваемые 
параметры:</p>
<pre class="brush:c#;">
public RedirectToRouteResult SomeMethod()
{
...
<p>Методы <code>Redirect/RedirectToAction</code> представлют временную переадресацию. Но они имеют свои двойники для создания постоянной 
переадресации: <code>RedirectPermanent/RedirectToActionPermanent</code>. Их действие аналогично, разница лишь в том, что они отправляют браузеру статусный код 301. 
...
способствовать полному выпадению сайта из поиска.</p>
<h3>Отправка ошибок и статусных кодов</h3>
<p>Иногда возникает необходимость отправить сообщения об ошибках при доступе к тому или иному ресурсу. Обычно, если ресурс недоступен, mvc-фреймворк 
автоматически отреагирует на эту ситуацию, отправив соответствующий статусный код. Но в некоторых ситуациях нам нужно более тонко реагировать на полученный запрос. 
Например, у нас есть контент, к которому установлены возрастные ограничения. Мы смотрим введенный возраст, и если он попадает под ограничение, 
мы можем выслать статусный код ошибки:</p>
<pre class="brush:c#;">
...
<p>Подобным образом мы можем послать браузеру любой другой статусный код.</p>
<p>В качестве альтернативы также можно возвращать объект HttpNotFoundResult с помощью метода <span class="bb">HttpNotFound</span></p>
<pre class="brush:c#;">
public ActionResult Check(int age)
{
    if (age &lt; 21)
...
<p>И еще один класс, предназначенный для отправки статусных кодов - класс HttpUnauthorizedResult. Он извещает пользователя, что тот не имеет права доступа к ресурсу, 
отправляя браузеру статусный код 401:</p>
<pre class="brush:c#;">
public ActionResult Check(int age)
{
    if (age &lt; 21)
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/PicvO4DyUjw"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 12. Переадресация и отправка статусных кодов в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
