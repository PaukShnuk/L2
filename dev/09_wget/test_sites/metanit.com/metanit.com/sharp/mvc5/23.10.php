...
<html >
<head>
<title>ASP.NET Identity в многоуровневой архитектуре в ASP.NET MVC 5. Часть 1</title>
<meta charset="utf-8" />
...
<h2>ASP.NET Identity в многоуровневой архитектуре. Часть 1</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 11.11.2015</div>
...
<p>Ранее мы рассмотрели создание трехуровневой архитектуры приложения. Тепеь рассмотрим, как мы можем использовать в этой архитектуре встроенную 
систему аутентификации и авторизации ASP.NET Identity.</p>
<p>Итак, создадим новое решение. Пусть оно будет называться <span class="b">UserStore</span> и в этом решении определим новый проект 
<span class="b">UserStore.WEB</span> по типу ASP.NET MVC 5 без аутентификации.</p>
<img src="./pics/23.10.png" alt="ASP.NET Identity in NLayer Architecture" />
<p>После этого добавим в решение новый проект по типу Class Library, который назовем UserStore.DAL. Этот проект будет представлять слой доступа к 
данным.</p>
<p>Так как мы будем использовать систему ASP.NET Identity, то нам вначале надо добавить соответствующие пакеты NuGet: <span class="b">EntityFramework</span>, 
<span class="b">Microsoft.AspNet.Identity.Core</span> и <span class="b">Microsoft.AspNet.Identity.EntityFramework</span></p>
<img src="./pics/23.11.png" alt="ASP.NET Identity Core в многоуровневой архитектуре" />
<p>Вначале добавим в DAL-проект новую папку, которую назовем <i>Entities</i>. В этой папке будут храниться сущности приложения. В данном случае ограничимся тремя сущностями: 
классом пользователя, классом роли и классом профиля пользователя.</p>
<p>Итак, добавим в папку Entities новый класс <span class="b">ApplicationUser</span>:</p>
<pre class="brush:c#;">
...
<p>И также добавим класс профиля пользователя <span class="b">ClientProfile</span>:</p>
<pre class="brush:c#;">
...
<p>Класс пользователя и его профиля связаны отношением один-к-одному. Разделение данных о пользователя на два класса позволит независимо друг от друга рассматривать 
аутентификационные данные и вспомогательные данные, то есть профиль пользователя, которые не играют никакой роли при аутентификации.</p>
<p>И также добавим в эту папку класс роли <span class="b">ApplicationRole</span>:</p>
<pre class="brush:c#;">
using Microsoft.AspNet.Identity.EntityFramework;
...
<p>Важно отметить, что эти классы зависят от пространства имен <span class="b">Microsoft.AspNet.Identity.EntityFramework</span>.</p>
<p>Далее добавим в проект новую папку, которую назовем <i>EF</i> и в которую поместим новый класс <span class="b">ApplicationContext</span>, 
то есть контекст данных:</p>
<pre class="brush:c#;">
...
<p>Данный класс наследуется от класса IdentityDbContext и поэтому уже имеет свойства Users и Roles, позволяющие взаимодействовать с таблицами пользователей 
и ролей. Поэтому здесь мы добавляем только свойство для ClientProfile.</p>
<p>Теперь добавим в проект еще три папки: <i>Identity</i>, <i>Interfaces</i> и <i>Repositories</i>. В папку Identity добавим класс ApplicationUserManager:</p>
<pre class="brush:c#;">
using UserStore.DAL.Entities;
...
<p>Данный класс будет управлять пользователями: добавлять их в базу данных и аутентифицировать.</p>
<p>Также добавим в папку Identity новый класс ApplicationRoleManager:</p>
<pre class="brush:c#;">
using UserStore.DAL.Entities;
using Microsoft.AspNet.Identity;
using Microsoft.AspNet.Identity.EntityFramework;
...
<p>Это стандартные для ASP.NET Identity классы по управлению ролями и пользователями. По сути эти классы выполняют роль репозиториев.</p>
<p>В папку Interfaces добавим интерфейс управления профилями пользователей <span class="b">IClientManager</span>:</p>
<pre class="brush:c#;">
using UserStore.DAL.Entities;
using System;
...
<p>Данный интерфейс содержит один метод для создания нового профиля пользователя.</p>
<p>И также определим интерфейс для UnitOfWork:</p>
<pre class="brush:c#;">
...
<p>Объект UnitOfWork будет содержать ссылки на менеджеры пользователей и ролей, а также на репозиторий пользователей.</p>
<p>В папку Repositories добавим два класса - конкретные реализации этих интерфейсов. Класс управления профилями ClientManager:</p>
<pre class="brush:c#;">
...
<p>И класс IdentityUnitOfWork:</p>
<pre class="brush:c#;">
using UserStore.DAL.EF;
using UserStore.DAL.Entities;
using UserStore.DAL.Interfaces;
...
<p>Класс инкапсулирует все менеджеры для работы с сущностями в виде свойств и хранит общий контекст данных.</p>
<p>В итоге проект будет выглядеть следующим образом:</p>
<img src="./pics/23.12.png" alt="Data Access Layer in ASP.NET Identity" />
...
<div class="nav"><p><a href="./23.9.php">Назад</a><a href="./">Содержание</a><a href="./23.11.php">Вперед</a></p></div></div>
<div class="socBlock">
