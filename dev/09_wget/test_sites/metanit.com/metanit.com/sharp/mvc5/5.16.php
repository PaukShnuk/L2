...
<html >
<head>
<title>Фильтрация данных в ASP.NET MVC</title>
<meta charset="utf-8" />
<meta name="description" content="Фильтрация данных в веб-приложении на ASP.NET MVC 5 и C#">
...
<h2>Фильтрация данных</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Рассмотрим, как фильтровать данные по определенным критериям в ASP.NET MVC. Возьмем один из ранее использовавшихся примеров с 
футболистами и командами, где у нас есть следующие модели:</p>
<pre class="brush:c#;">
public class Player
{
...
<p>Допустим, при выводе списка футболистов нам надо также предусмотреть возможность для их фильтрации по некоторым критериям, например, по команде 
и по позиции на поле.</p>
<p>Так как мы будем выводить в одном представлении и список игроков, и критерии для выбора, которые представляют списки команд и 
позиций, то добавим в проект специальную модель:</p>
<pre class="brush:c#;">
using System;
...
<p>Три свойства - для списка игроков, команд и позиций.</p>
<p>Для вывода данных модели определим представление:</p>
<pre class="brush:xml;">
...
<p>Перед таблицей со списком игроков, определена форма, которая с помощью запросов <span class="b">GET</span> посылает выбранные в выпадающих 
списках значения на сервер.</p>
<p>Теперь на стороне контроллера определим метод, который будет выводить список игроков и производить фильтрацию:</p>
<pre class="brush:c#;">
...
<p>При обращении к методу Index вне зависимости были ли переданы параметры team и position, все равно в представление будет выводиться 
список игроков. Если определенный параметр был передан, то к объекту <code>IQueryable</code>, представляющего выборку, добавляется 
выражение <code>Where</code>: <code>players = players.Where(p=&gt;p.TeamId==team);</code>.</p>
<p>Для формирования выпадающего списка команд в представлении, получаем его из БД:</p>
<pre class="brush:c#;">List&lt;Team&gt; teams = db.Teams.ToList();</pre>
<p>Для возможности выбора игроков из любой команды, добавляем в этот список новый пункт:</p>
<pre class="brush:c#;">teams.Insert(0, new Team { Name = "Все", Id = 0 });</pre>
<p>Все полученные и созданные списки используются для формирования объекта <code>PlayersListViewModel</code>, который передается в 
представление.</p>
<img src="./pics/5.36.png" alt="Фильтрация в ASP.NET MVC 5" />
