...
<html >
<head>
<title>Переменные сервера и обратные ссылки в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Переменные сервера и обратные ссылки</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для IIS URL представляется в следующем виде: <code>http(s)://&lt;host&gt;:&lt;port&gt;/&lt;path&gt;?&lt;querystring&gt;</code>. 
...
<p>{C:1} и {C:2} - это обратные ссылки, которые представляют сегменты выражения <code>pattern="*://www.*"</code>.</p>
<p>Принцип создания обратных ссылок зависит от типа синтаксиса выражения - ECMAScript или Wildcard. Так, в примере выше использовался тип Wildcard, 
и каждая звездочка в выражении будет создавать из соответствующего сегмента обратную ссылку.</p>
<p>При использовании типа ECMAScript и стандартного синтаксиса регулярных выражений, если бы у нас было следующее условие:</p>
<pre class="brush:xml;">
&lt;add input="{CACHE_URL}" pattern="([a-z]+)/([a-z]+)/([0-9]+)" /&gt;
</pre>
<p>То из строки запроса "home/index/2" генерировались бы три обратных ссылки "home", "index" и "2"</p>
...
доступны через выражения типа <span class="bb">{R:N}</span>, где N - число от 0 до 9. При этом значение <code>{R:0}</code> 
представляет всю попадающую под условие строку.</p>
...
<p>Например, при запросе <i>http://www.somesite.com/home/index/2/article</i> мы получим следующие сегменты:</p>
<p>{R:0} = "home/index/2/article" </p>
<p>{R:1} = "2" </p>
<p>{R:2} = "article" </p>
<p>В итоге будет формироваться следующая строка URL: <i>http://www.somesite.com/home/index?id=2&name=article</i></p>
<p>Для тестирования определил следующий метод, который выводит текущую строку запроса:</p>
<pre class="brush:c#;">
...
<p>Теперь обратимся по запросу <i>http://localhost:5347/home/index/23/article</i>:</p>
<img src="./pics/urlrewriting.png" alt="URL Rewriting" />
