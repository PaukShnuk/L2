...
<html >
<head>
<title>Авторизация через внешние сервисы</title>
<meta charset="utf-8" />
...
<h2>Авторизация через внешние сервисы</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Инфраструктура OWIN и AspNet Identity позволяют производить авторизацию через внешние сервисы с помощью аккаунтов в Google, Twitter, Facebook и т.д. Хотя 
подобная функциональность была доступна и предыдущей версии - в MVC 4, в MVC 5 ее механизм существенно изменился, и за аутентификацию отвечают компоненты OWIN.</p>
<p>Возьмем стандартный проект MVC 5, использующий AspNet Identity, то есть имеющий в качестве типа аутентификации Individual User Accounts. В проекте 
в папке App_Start мы можем увидеть файл <span class="bb">Startup.Auth.cs</span>, который отвечает за настройку конфигурации OWIN:</p>
<pre class="brush:c#;">
using System;
...
<p>В конце метода ConfigureAuth можно увидеть ряд закомментированных по умолчанию строк, предназначенных для аутентификации через внешние сервисы. Например:</p>
<pre class="brush:c#;">
//app.UseFacebookAuthentication(
//   appId: "",
//   appSecret: "");
</pre>
<p>При регистрации нашего приложения в социальной сети Facebook мы получаем идентификатор приложения в рамках этой соцсети и секретное слово. 
...
в приложении через этот аккаунт. Довольно удобно - всего три строчки, а все остальное за нас сделает инфраструктура ASP.NET MVC.</p>
<p>Для примера выберем самый простой вариант - аутентификацию через Google. Чтобы подключаться к провайдерам аутентификации Google или 
Facebook, нам надо настроить проект на использование SSL. Для этого перейдем к панели свойств проекта и установим там параметр <span class="bb">SSL Enabled</span> равным 
True.</p>
<img src="./pics/ssl_in_aspnetmvc.png" alt="SSL in ASP.NET MVC" />
<p>Вместе с установкой SSL также Visual Studio устанавливает и новый адрес для SSL-соединения: <i>https://localhost:44300/</i>, который мы 
будем использовать для аутентификации Google.</p>
<p>Теперь нам надо изменить адрес запуска проекта. Для этого скопируем ssl-адрес и перейдем в настройки проекта на вкладку Web. Вставим скопированный адрес в поле 
Project Url:</p>
<img src="./pics/googleauth8.png" alt="Изменение адреса запуска в ASP.NET MVC" />
<p>Прежде всего нам надо будет создать в аккаунте Google некоторые настройки, которые будут использоваться нашим приложением. Для этого авторизуемся в Google и перейдем к странице <a href="https://console.developers.google.com/project" rel="nofollow">Google Developers Console</a>:</p>
<img src="./pics/googleauth.png" alt="Авторизация Google в Asp.Net MVC 5" />
<p>На странице нажмем на кнопку <span class="bb">Create Project</span>. В появившемся окне введем название проекта и его ID:</p>
<img src="./pics/googleauth2.png" alt="Авторизация Google в Oath2" />
<p>Google автоматически генерирует некоторые значения, как на рисунке выше. И в принципе мы можем эти значения оставить. И через некоторое время будет 
создан новый проект, а перед нами откроется панель настроек проекта.</p>
<p>В левой колонке меню выберем пункт <span class="bb">APIs &amp; auth</span> и далее выберем <span class="bb">Consent screen</span>. Здесь нам надо установить 
название приложение в поле Product Name, а также укажем email:</p>
<img src="./pics/googleauth3.png" />
<p>В моем случае название приложения TestAppForGoogleAuth. Сохраним, нажав внизу страницы на кнопку Save. После этого перейдем 
в левой панели меню к пункту <span class="bb">Credentials</span> и на открывшейся странице нажмем на кнопку <span class="bb"> Create New Client ID</span>. 
Нам отобразится следующее диалоговое окно:</p>
<img src="./pics/googleauth4.png" />
<p>Тут есть несколько опций, которые надо установить. Во-первых, в поле APPLICATION TYPE оставим <span class="bb">Web Application</span></p>
<p>В поле <span class="bb">Authorized JavaScript</span> введем ssl-адрес нашего приложения, который был установлен выше. Как правило, это 
<i>https://localhost:44300/</i></p>
<p>После это в поле <span class="bb">Authorized redirect URI</span> введем <i>https://localhost:44300/signin-google</i></p>
<p>После ввода всех данных нажмем на кнопку <span class="bb">Create Client ID</span>. После этого Google сгенерирует нам ряд аутентификационных настроек, которые мы будем 
использовать в приложении:</p>
<img src="./pics/googleauth5.png" alt="Подключение к Google в ASP.NET MVC" />
<P>После получения аутентификационных настроек обновим файл Startup.Auth.cs:</P>
<pre class="brush:c#;">
app.UseGoogleAuthentication(new GoogleOAuth2AuthenticationOptions()
{
...
<p>Затем перейдем в меню на вкладку <span class="bb">APIs</span>. Найдем пункт Google+ API и нажмем на кнопку OFF:</p>
<img src="./pics/googleauth6.png" alt="Авторизация Google+ и ASP.NET MVC" />
<p>После всех настроек установим соответствующий атрибут ssl для контроллера HomeController:</p>
<pre class="brush:c#;">
[RequireHttps]
...
<p>Запустим проект и обратимся к методу Login, увидим, что теперь нам доступно два способа входа на сайт: стандартный и через Google.</p>
<img src="./pics/12.8.png" />
<p>После этого приложение попросит принять некоторые разрешения для доступа к нашему аккаунту::</p>
<img src="./pics/googleauth10.png" />
<p>Нажмем на кнопку Принять, и после этого мы перейдем к поледнему шагу - регистрации внешнего логина, который будет отображаться на сайте для данного аккаунта:</p>
<img src="./pics/googleauth11.png" />
<p>И после этого в меню начнет отображаться логин</p>
<img src="./pics/googleauth12.png" />
<p>За отображение логина в представлении отвечает частичное представление <i>_ExternalLoginsListPartial</i>.</p>
<p>Данные пользователя после завершения визита на сайт не пропадают. Если мы откроем в окне Server Explorer используемую базу данных, то мы сможем увидеть, 
что в таблицу AspNetUserLogins, которая отвечает за связь логинов сайта с внешними аккаунтами, была добавлена новая запись:</p>
<img src="./pics/googleauth13.png" />
<p>Кроме того, так как был добавлен новый пользователь, соответствующая запись добавляется в таблицу AspNetUsers.</p>
<p>Таким образом, мы можем воспользоваться преимуществами OWIN и OAuth2 и добавить в приложение идентификацию с помощью внешних сервисов.</p>
...
<div class="nav"><p><a href="./12.14.php">Назад</a><a href="./">Содержание</a><a href="./12.3.php">Вперед</a></p></div></div>
<div class="socBlock">
