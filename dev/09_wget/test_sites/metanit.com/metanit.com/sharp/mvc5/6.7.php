...
<html >
<head>
<title>Атрибуты маршрутизации в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Атрибуты маршрутизации</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Одним из нововведений MVC 5 стала такая функциональность как атрибуты маршрутизации (attribute routing). Данные атрибуты позволяют сопоставить 
определенный маршрут с методом контроллера. Например, у нас есть такой метод в контроллере HomeController:</p>
<pre class="brush:c#;">
...
<p>Ему мог бы соответствовать следующий маршрут, определенный в файле RouteConfig.cs:</p>
<pre class="brush:c#;">
...
<p>Параметр <code>constraints: new { id="\d+"}</code> ограничивает первый сегмент строки запроса числами, таким образом, у нас не будет конфликта между двумя маршрутами. То есть запрос вида 
<i>http://localhost:6392/2/volga</i> будет сопоставляться с первым маршрутом, а если вместо числа 2 будет идти строка - то со вторым.</p>
<p>Но атрибуты маршрутизации позволяют не определять дополнительный маршрут, а указать сопоставление прямо в коде контроллера:</p>
<pre class="brush:c#;">
[Route("{id:int}/{name}")]
public string Test(int id, string name)
{
...
<p>Кроме того, нам надо изменить код в файле RouteConfig.cs следующим образом:</p>
<pre class="brush:c#;">
public class RouteConfig
{
    public static void RegisterRoutes(RouteCollection routes)
...
<p>Здесь вместо определения маршрута мы используем метод <code>routes.MapMvcAttributeRoutes();</code>, который подключает в систему маршрутизации приложения 
функциональность атрибутов маршрутизации.</p>
<p>Мы также можем явно указать имя контроллера или метода или параметра в атрибуте: <code>[Route("Home/{id:int}/{name}")]</code>. Данный маршрут 
будет соответствовать запросу <i>Home/4/somename</i></p>
