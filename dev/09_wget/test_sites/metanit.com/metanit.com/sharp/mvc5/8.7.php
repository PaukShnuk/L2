...
<html >
<head>
<title>Дополнительные встроенные фильтры | ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Дополнительные встроенные фильтры</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>Фильтр RequireHttps</h3>
<p>Фильтр RequireHttps заставляет использовать протокол HTTPS, а браузер перенаправит пользователя на то же действие, только с префиксом <code>https</code>.</p>
<p>Данный фильтр применяется только к GET-запросам.</p>
<h3>Фильтр OutputCache</h3>
<p>Данный фильтр говорит mvc-фреймворку кэшировать вывод метода действия, чтобы полученный контент можно было в дальнейшем использовать повторно.</p>
<p>Кэширование может увеличить производительность, особенно когда идет речь о выборке из базы данных, которая может занимать значительное время.</p>
<p>С помощью параметра Duration мы можем настроить время (в секундах):</p>
<pre class="brush:c#;">
...
<h3>Атрибут ValidateAntiforgeryToken</h3>
<p>Фильтр ValidateAntiforgeryToken предназначен для противодействия подделке межсайтовых запросов, производя верификацию токенов при обращении к методу действия. 
Наиболее частым случаем является применение данного фильтра к методам, отвечающим за авторизацию, например:</p>
<pre class="brush:c#;">
[HttpPost]
[AllowAnonymous]
...
<p>Кроме того, нам надо использовать хелпер <span class="bb">@Html.AntiForgeryToken()</span> для использования токена:</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./8.6.php">Назад</a><a href="./">Содержание</a><a href="./8.9.php">Вперед</a></p></div></div>
<div class="socBlock">
