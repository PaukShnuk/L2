...
<html >
<head>
<title>Введение в AJAX в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Введение в AJAX</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>AJAX (Асинхронный JavaScript и XML) представляет собой технологию гибкого взаимодействия между клиентом и сервером. Благодаря ее 
использованию мы можем осуществлять асинхронные запросы к серверу без перезагрузки всей страницы. Правда, в настоящее время все больше вместо 
формата XML используется формат JSON для взаимодействия между клиентом и сервером.</p>
<p>Применительно к ASP.NET MVC использование AJAX вылилось в целую концепцию под названием <span class="bb">"ненавязчивого AJAX"</span> и 
<span class="bb">ненавязчивого JavaScript</span> (<span class="ii">unobtrusive Ajax/JavaScript</span>). Смысл этой концепции заключается в том, что весь необходимый код 
JavaScript используется не на самой веб-странице, а помещается в отдельные файлы с расширением <span class="ii">*.js</span>. А затем с помощью тега 
<code>&lt;script&gt;</code> мы в веб-станице ссылаемся на данный файл кода.</p>
<p>Таким образом мы отделяем визуализацию от логики приложения. Что имеет свои плюсы. Так, выделение скрипта в отдельный загружаемый файл 
увеличивает производительность сайта, поскольку файл сохраняется в кэше и затем от туда подгружается.</p>
<h3>Настройка ненавязчивого JavaScript/ AJAX</h3>
<p>Во-первых, соответствующие настройки должны быть указаны в файле Web.config:</p>
<pre class="brush:xml;">
&lt;appSettings&gt;
...
<p>Во-вторых, нам надо подключить соответствующие файлы JavaScript:</p>
<pre class="brush:xml;">
@Scripts.Render("~/scripts/jquery-1.10.2.js")
@Scripts.Render("~/scripts/jquery.unobtrusive-ajax.js")
</pre>
<p>Если у нас в проекте отсуствует библиотека <i>jquery.unobtrusive-ajax.js</i>, то мы ее опять же можем добавить через NuGet:</p>
<img src="./pics/10.3.png" />
<p>Первый файл - общая библиотека jQuery. Второй файл (<span class="ii">jquery.unobtrusive-ajax.js</span>) подключает к приложению функциональность Ajax-хелперов, 
например, Ajax-форм. Главное не забыть, подключить основную библиотеку jQuery перед остальными файлами, так как все остальные скрипты, как правило, зависят от нее.</p>
<h3>AJAX-хелперы</h3>
<p>Для осуществления ajax-запросов в ASP.NET MVC присутствует такая функциональность, как ajax-хелперы:</p>
<ul>
	<li><p><span class="bb">Ajax.ActionLink</span>: создает гиперссылку на действие контроллера, по нажатию на которую происходит ajax-запрос к этому действию</p></li>
	<li><p><span class="bb">Ajax.RouteLink</span>: похож на хелпер Ajax.ActionLink, только ссылка создается на определенный маршрут, а не на действие контроллера</p></li>
	<li><p><span class="bb">Ajax.BeginForm</span>: создает html-форму, которая отправляет ajax-запросы к определенному действию определенного контроллера</p></li>
	<li><p><span class="bb">Ajax.BeginRouteForm</span>: похож на Ajax.BeginForm, только ajax-запросы направляются не к действию контроллера, к по определенному маршруту</p></li>
	<li><p><span class="bb">Ajax.GlobalizationScript</span>: создает ссылку на скрипт, который содержит информацию о культуре</p></li>
	<li><p><span class="bb">Ajax.JavaScriptStringEncode</span>: кодирует строку для использования в JavaScript</p></li>
</table>
<p>Наиболее часто используются <span class="bb">Ajax.ActionLink</span> и <span class="bb">Ajax.BeginForm</span>, поэтому далее мы рассмотрим эти хелперы.</p>
