...
<html >
<head>
<title>ASP.NET MVC 5 | Переопределение системы Identity</title>
<meta charset="utf-8" />
<meta name="description" content="Переопределение системы Identity, добавление новых свойств и таблиц в ASP NET MVC 5">
...
<h2>Переопределение системы AspNet Identity</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Ранее мы рассмотрели основные моменты AspNet Identity и то, как ее использовать. Однако встроенная система не всегда в полной мере удовлетворяет нуждам разработчиков. 
И может потребоваться переопределить ее: добавить новые поля к модели, добавить новые таблицы и т.д. Рассмотрим, как переопределять базовый механизм 
AspNet Identity.</p>
<p>Как мы увидели в прошлых темах, при использовании AspNet Identity по умолчанию создается база данных со стандартным набором таблиц. Для взаимодействия 
с базой данных в проекте определен контекст данных ApplicationDbContext, который можно найти в файле <i>IdentityModels.cs</i> в папке Models:</p>
<pre class="brush:c#;">
...
<p>Кроме того, здесь также описывается класс пользователей ApplicationUser, который наследует всю логику от IdentityUser. При регистрации контекст данных заносит 
в бд логин и хеш пароля пользователя. Однако, что если мы хотим определить еще ряд полей, например, возраст и т.д.? Изменим модели в файле <i>IdentityModels.cs</i> 
следующим образом:</p>
<pre class="brush:c#;">
public class ApplicationUser : IdentityUser
{
...
<p>Во-первых, мы добавили новое свойство в класс ApplicationUser. Во-вторых, мы также добавили новый класс Book и свойство Books в контекст данных.</p>
<p>Поскольку у нас изменился контекст данных и модель пользователя, то созданная ранее база данных уже не будет отражать наши модели, и ее надо перестроить. 
В этом нам помогут миграции. Итак, внизу Visual Studio в окне Package Manager Console введем команду: <code>enable-migrations</code> и нажмем Enter:</p>
<img src="./pics/12.13.png" />
<p>В результате выполнения этой команды Visual Studio создаст в проекте папку Migrations, в которой можно найти файл <i>Configuration.cs</i>. 
Этот файл содержит объявление одноименного класса Configuration, который устанавливает настройки конфигурации:</p>
<pre class="brush:c#;">
...
<p>В методе Seed можно проинициализировать базу данных начальными данными. Теперь нам надо создать саму миграцию. Там же в консоли Package Manager Console 
введем команду:</p>
<pre>PM&GT; Add-Migration "DataMigration"</pre>
<p>После этого Visual Studio автоматически сгенерирует класс миграции:</p>
<pre class="brush:c#;">
namespace CustomIdentity.Migrations
{
    using System;
...
<p>И чтобы выполнить миграцию, применим этот класс, набрав в той же консоли команду:</p>
<pre>PM&GT; Update-Database</pre>
<p>После этого, если мы посмотрим на состав базы данных, то увидим, что к ней были применены изменения в соответствии с выполненной миграцией:</p>
<img src="./pics/12.15.png" />
<p>Так как у нас появилось новое свойство в ApplicationUser, изменим модель регистрации в файле <i>AccountViewModels.cs</i>, который находится в папке Models:</p>
<pre class="brush:c#;">
...
<p>И также добавим в представление для регистрации Register.cshtml пару новых полей:</p>
<pre class="brush:xml;">
...
<p>Внешний вид страницы регистрации в итоге будет выглядеть так:</p>
<img src="./pics/12.14.png" />
<p>И изменим код добавления нового пользователя в методе Register контроллера AccountController. Сейчас он выглядит так:</p>
<pre class="brush:c#;">
...
<p>Теперь мы можем добавлять новых пользователей, указывая у них дополнительные свойства. И также при желании сделать функционал по добавлению 
объектов Book в таблицу Books или создать новые свойства моделей и новые таблицы.</p>
