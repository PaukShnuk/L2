...
<html >
<head>
<title>Тестирование создания модели и переадресации</title>
<meta charset="utf-8" />
...
<h2>Тестирование создания модели и переадресации</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Рассмотрим на примере тестирование создания модели и переадресации. Во-первых, нам нужен минимальный код для запуска тестов. 
Для этого возьмем проект веб-приложения из прошлой темы и определим в нем стандартный метод Create:</p>
<pre class="brush:c#;">
...
<p>Это минимальный код, достаточный для написания тестов. Теперь в класс тестов HomeControllerTest добавим ряд методов:</p>
<pre class="brush:c#;">
[TestMethod]
...
<p>В данном методе проверяется механизм валидации. Через объект контроллера мы можем добавить ряд ошибок к свойству ModelState. И в случае ошибок, 
идет обращение к представлению Create.</p>
<p>Если мы запустим, то тест не сработает, так как никакой валидации в методе контроллера нет. Поэтому исправим метод Create:</p>
<pre class="brush:c#;">
[HttpPost]
public ActionResult Create(Computer c)
{
...
<p>Запустим тест снова. Теперь он сработает успешно, и мы можем переходить к новому тесту. Добавим в класс тестов следующий метод:</p>
<pre class="brush:c#;">
[TestMethod]
...
<p>Тестирование переадресации делается также просто, как и тестирование представлений. Объект <code>RedirectToRouteResult</code>, который представляет переадресацию, 
имеет свойство <code>RouteValues</code>, которое позволяет получить данные маршрута - в данном случае получаем метод и сравниваем его с ожидаемым.</p>
<p>И поскольку данный тест опять завершится неудачно, снова изменим метод Create:</p>
<pre class="brush:c#;">
[HttpPost]
public ActionResult Create(Computer c)
{
...
<p>Запустим еще раз тест и убедимся, что он завершился успешно. И теперь добавим третий метод тестов:</p>
<pre class="brush:c#;">
[TestMethod]
...
Но не все методы возвращают определенные значения. Некоторые методы возвращают тип <code>void</code>, однако и тоже необходимо тестировать. 
И чтобы протестировать подобные методы, в классе Mock определен метод <code>Verify</code>.</p>
<p>Его синтаксис прост: <code>mock.Verify(a=&gt;a.Method(parameter))</code>, где mock - это объект <code>Mock</code>, а <code>parameter</code> - значение, 
передаваемое в качестве параметра в метод.</p>
<p>В данном случае вызывается метод Create, в который передается модель для сохранения, и метод Save.</p>
<p>Опять же тест не сработает, поэтому изменим метод Create следующим образом:</p>
<pre class="brush:c#;">
[HttpPost]
public ActionResult Create(Computer c)
{
...
<p>Запустим тест и убедимся, что он завершится успешно.</p>
<p>Таким образом, мы можем протестировать сохранение модели и механизм переадресации.</p>
...
<div class="nav"><p><a href="./18.5.php">Назад</a><a href="./">Содержание</a><a href="./19.1.php">Вперед</a></p></div></div>
<div class="socBlock">
