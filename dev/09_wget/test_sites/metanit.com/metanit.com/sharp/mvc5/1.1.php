...
<html >
<head>
<title>ASP.NET MVC 5 | Введение в создание сайтов</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в создание сайтов и веб-приложений на ASP.NET MVC 5">
...
<h1>Введение в ASP.NET MVC 5</h1><h2>Особенности ASP.NET MVC. Что нового в MVC 5</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Платформа ASP.NET MVC представляет собой фреймворк для создания сайтов и веб-приложений с помощью реализации паттерна MVC.</p>
<p>Концепция паттерна (шаблона) MVC (model - view - controller) предполагает разделение приложения на три компонента:</p>
<p><span class="bb">Контроллер</span> (controller) представляет класс, обеспечивающий связь между пользователем и системой, представлением 
...
определенный вывод, например, в виде представления.</p>
<p><span class="bb">Представление</span> (view) - это собственно визуальная часть или пользовательский интерфейс приложения. Как правило, 
html-страница, которую пользователь видит, зайдя на сайт.</p>
<span class="bb">Модель</span> (model) представляет класс, описывающий логику используемых данных.</p>
<p>Общую схему взаимодействия этих компонентов можно представить следующим образом:</p>
<img src="./pics/1.1.png" alt="Паттерн MVC в ASP.NET" />
<p>В этой схеме модель является независимым компонентом - любые изменения контроллера или представления не затрагивают модель. Контроллер и 
представление являются относительно независимыми компонентами, и нередко их можно изменять независимо друг от друга.</p>
<p>Благодаря этому реализуется концепция <span class="bb">разделение ответственности</span>, в связи с чем легче построить работу над отдельными 
...
<p>Конкретные реализации и определения данного паттерна могут отличаться, но в силу своей гибкости и простоты он стал очень популярным 
в последнее время, особенно в сфере веб-разработки.</p>
<p>Свою реализацию паттерна представляет платформа ASP.NET MVC. 2013 год ознаменовался выходом новой версии ASP.NET MVC - MVC 5, а также релизом 
Visual Studio 2013, которая предоставляет инструментарий для работы с MVC5.</p>
<p>Хотя во многих аспектах MVC 5 не слишком сильно будет отличаться от MVC 4, многое из одной версии вполне применимо к другой, но в то же время 
есть и существенные отличия:</p>
<ul>
<li><p>В MVC 5 изменилась концепция аутентификации и авторизации. Вместо SimpleMembershipProvider была внедрена система ASP.NET Identity, 
которая использует компоненты OWIN и Katana.</p></li>
<li><p>Для создания адаптивного и расширяемого интерфейса в MVC 5 используется css-фреймворк Bootstrap</p></li>
<li><p>Добавлены фильтры аутентификации, а также появилась функциональность переопределения фильтров</p></li>
<li><p>В MVC 5 также добавлены атрибуты маршрутизации</p></li>
</ul>
<p>Это наиболее важные нововведения в MVC 5. Кроме того, есть еще ряд менее значимых, например, использование по умолчанию Entity Framework 6, 
некоторые изменения при создании проекта (концепция One ASP.NET), дополнительные компоненты и т.д.</p>
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://www.youtube.com/watch?v=PJRDpA9NpDs"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 1. Введение в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
