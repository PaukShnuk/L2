...
<html >
<head>
<title>Поставщики значений в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Поставщики значений</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Как уже говорилось в начале главы, для поиска значений привязчик модели использует специальные компоненты - <b>поставщики значений (value 
providers)</b>. Поставщики значений предоставляют доступ к информации, которая затем используется в привязке модели.</p> 
<p>Поставщики значений поставляются <span class="bb">фабриками поставщиков значений</span> (value provider factories) и системой поиска данных для этих 
...
Разработчики могут создавать свои собственные фабрики и поставщики значений и добавлять их в список фабрик <code>ValueProviderFactories.Factories</code>. 
Для предоставления дополнительного источника данных для привязки модели разработчики могут применить фабрику или поставщик значений.</p>
<p>Все поставщики значений реализуют интерфейс <span class="bb">IValueProvider</span>:</p>
<pre class="brush:c#;">
...
<p>В конкретных реализациях метод <code>ContainPrefix</code> сопоставляет некоторые данные с указанным префиксом, а метод <code>GetValue</code> 
получает данные для определенного ключа.</p>
<p>Создадим свой поставщик значений. Пусть, к примеру, он будет получать для нас название браузера пользователя. Определим поставщик:</p>
<pre class="brush:c#;">
...
<p>В методе <code>GetValue</code>, если ключ совпадает с префиксом, то мы возвращаем объект <code>ValueProviderResult</code>, в конструктор которого 
...
А в третьем параметре передается информация о культуре, с которое ассоциируется значение. В данном случае второй и третий параметр нам не столь важны.</p>
<p>Как уже писалось выше, чтобы задействовать свой поставщик значений, нам надо определить фабрику поставщика:</p>
<pre class="brush:c#;">
...
<p>И в конце нам надо зарегистрировать фабрику поставщика значений в файле Global.asax:</p>
<pre class="brush:c#;">
...
<p>В данном случае мы добавляем наш поставщик в конец коллекции <code>ValueProviderFactories.Factories</code>. 
...
то следующие после него уже не используются.</p>
<p>В нашем случае это не имеет значения. Однако если возникнет необходимость поставить свою фабрику поставщика значений на первое место в списке, то мы 
можем сделать это следующим образом: <code>ValueProviderFactories.Factories.Insert(0, new BrowserValueProviderFactory());</code></p>
<p>Теперь мы можем определить метод действия, в котором протестируем работу поставщика значений:</p>
<pre class="brush:c#;">
...
<p>Итак, отправив запрос данному методу действий, мы получим в качестве ответа название нашего браузера.</p>
<p>Как вы видите, мы не оправляем в форме или в строке запроса параметр <code>browser</code> и его значение. Его находит сам наш поставщик.</p>
