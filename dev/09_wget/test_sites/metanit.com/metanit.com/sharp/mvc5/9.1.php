...
<html >
<head>
<title>Привязка модели в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h1>Привязка модели</h1>
<h2>Введение в привязку моделей</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В предыдущих главах мы рассмотрели модели и узнали, как передавать в методы контроллера значения из представлений. Но за кадром остался весь механизм, 
который способствует сопоставлению значений с используемыми в методах контроллера параметрами. И этот механизм называется <span class="ii">привязкой модели</span>.</p>
<p>Чтобы понять смысл привязки, посмотрим на примере. Допустим, мы отправляем на сервер форму с некоторыми данными для модели Book. И в 
некотором методе контроллера мы можем получить переданные данные из формы:</p>
<pre class="brush:c#;">
[HttpPost]
...
<p>Однако свойств модели Book, которые надо получить из запроса, может быть множество. Кроме того, вы можете заметить, что поля для ввода свойств 
модели имеют имя (атрибут <code>name</code>), совпадающее с именем свойства модели. Чтобы облегчить процесс получения присланных данных и был создан 
механизм привязки. Благодаря чему мы можем просто написать:<p>
<pre class="brush:c#;">
[HttpPost]
...
<p>При этом привязка модели не ограничена POST-запросами и сложными параметрами типа объекта Book. Привязка может также работать и с более простыми 
параметрами, например со следующим действием Edit, обрабатывающим HTTP-запрос GET:</p>
<pre class="brush:c#;">
...
<h3>Привязчики моделей</h3>
<p>После сопоставления запроса с некоторым маршрутом контролллеров вызываются специальные компоненты - <span class="bb">активаторы действий</span> (action invoker), 
которые вызывают нужное действие контроллера и передают в него значения из контекста запроса. 
А чтобы сопоставить полученные значения с конкретными параметрами активаторы действий, в частности, активатор по умолчанию - <code>ControllerActionInvoker</code>, 
используют <span class="bb">привязчики модели (model binder)</span>. Привязчики моделей и осуществляют собственно привязку.</p>
<p>Все привязчики моделей должны реализовать интерфейс <b>IModelBinder</b>:</p>
<pre class="brush:c#;">
...
<p>В принципе для каждого отдельного типа может существовать свой привязчик модели. При просмотре параметров метода действия активатор действий 
ищет для каждого типа параметра соответствующий привязчик и вызывает его метод <code>BindModel</code>. В случае, если соответствующего данному типу 
привязчика не обнаружится, то используется привязчик по умолчанию - <code>DefaultModelBinder</code>.</p>
<p>Затем привязчик использует специальные компоненты - <strong>поставщики значений (value provider)</strong> для поиска значений в различных частях запроса.</p>
