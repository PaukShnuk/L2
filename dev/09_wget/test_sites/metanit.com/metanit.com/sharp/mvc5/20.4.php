...
<html >
<head>
<title>Динамическое управление кэшем в ASP.NET MVC</title>
<meta charset="utf-8" />
...
<h2>Динамическое управление кэшем</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Кроме применения атрибута <code>OutputCache</code> можно управлять кэшем динамически с помощью свойства <span class="bb">Response.Cache</span>. 
Это свойство представляет объект класса <span class="bb">HttpCachePolicy</span>, в котором определены следующие методы и свойства:</p>
<ul>
<li><p><code>SetCacheability(policy)</code>: устанавливает у ответа заголовок <code>Cache-Control</code> с помощью перчисления HttpCacheability. Устанавливая данный параметр, 
мы можем определить место хранения кэшируемых данных - клиент или сервер</p></li>
<li><p><code>SetExpires(date)</code>: устанавливает с помощью объекта DateTime у ответа заголовок <code>Expires</code>, который определяет срок окончания кэширования</p></li>
<li><p><code>SetLastModified(date)</code>: устанавливает с помощью объекта DateTime у ответа заголовок <code>Last-Modified</code></p></li>
<li><p><code>SetMaxAge(period)</code>: устанавливает с помощью объекта TimeSpan у заголовка <code>Cache-Control</code> флаг <code>max-age</code></p></li>
<li><p><code>SetNoServerCaching()</code>: отключает для данного ответа кэширование на стороне сервера</p></li>
<li><p><code>SetNoStore()</code>: добавляет к заголовку <code>Cache-Control</code> флаг <code>no-store</code></p></li>
<li><p><code>SetNoTransforms()</code>: добавляет к заголовку <code>Cache-Control</code> флаг <code>no-trans</code></p></li>
<li><p><code>SetProxyMaxAge(period)</code>: устанавливает с помощью объекта TimeSpan у заголовка <code>Cache-Control</code> флаг <code>s-maxage</code></p></li>
<li><p><code>SetVaryByCustom(name)</code>: устанавливает параметр, в зависимости от которого будет выбираться версия кешированных данных. Аналогично применению параметра VaryByCustom 
в атрибуте OutputCache</p></li>
<li><p><code>VaryByHeaders</code>: определяет заголовки, которые используются для выбора версии кэша</p></li>
<li><p><code>VaryByParams</code>: определяет параметры строки запроса или форм, которые используются для выбора версии кэша</p></li>
</ul>
<p>Используем некоторые из этих методов:</p>
<pre class="brush:c#;">
...
<p>Для кэширования в данном случае не нужен атрибут OutputCache, так как все делается через свойство <code>Response.Cache</code>. Выражение <code>Response.Cache.SetExpires(DateTime.Now.AddSeconds(30))</code> 
определяет 30-ти секундный период кэширования. А выражение <code>Response.Cache.SetCacheability(HttpCacheability.Public)</code> устанавливает в ответе для 
заголовка <code>Cache-Control</code> значение <code>public</code>, которое указывает, что контент кэшируется как на клиенте, так и на прокси-сервере. 
Его действие аналогично применению значения <code>OutputCacheLocation.Downstream</code> для параметра <code>Location</code> атрибута <code>OutputCache</code></p>
<p>Кроме значения <code>HttpCacheability.Public</code> перечисление <span class="bb">HttpCacheability</span> может иметь еще несколько значений:</p>
<ul>
<li><p><span class="bb">NoCache</span>: устанавливает в ответе для заголовка <code>Cache-Control</code> значение <code>no-cache</code>, которое указывает, 
что контент не будет кэшироваться. При использовании атрибута OutputCache аналог - <code>OutputCacheLocation.None</code></p></li>
<li><p><span class="bb">Private</span>: устанавливает в ответе для заголовка <code>Cache-Control</code> значение <code>private</code>, которое указывает, 
что контент будет кэшироваться только на клиенте. Аналог - <code>OutputCacheLocation.Client</code></p></li>
<li><p><span class="bb">Server</span>: устанавливает в ответе для заголовка <code>Cache-Control</code> значение <code>no-cache</code>. Контент будет 
кэшироваться только в выходном кэше ASP.NET. Аналог - <code>OutputCacheLocation.Server</code></p></li>
<li><p><span class="bb">ServerAndNoCache</span>: объединяет действие значений <code>Server</code> и <code>NoCache</code></p></li>
<li><p><span class="bb">ServerAndPrivate</span>: объединяет действие значений <code>Server</code> и <code>Private</code> (кэширование на клиенте и сервере)</p></li>
</ul>
<p>Например, используем только выходной кэш сервера без кэширования на клиенте:</p>
<pre class="brush:c#;">
public string Index()
{
    Response.Cache.SetExpires(DateTime.Now.AddSeconds(30));
