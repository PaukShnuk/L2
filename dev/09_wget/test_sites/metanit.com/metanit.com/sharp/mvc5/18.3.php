...
<html >
<head>
<title>Создание юнит-тестов в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Создание юнит-тестов в ASP.NET MVC 5">
...
<h2>Создание юнит-тестов</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Возьмем тот же проект из прошлый темы (либо создадим новый) и добавим в главный проект веб-приложения в папку Contollers новый контроллер StoreController:</p>
<pre class="brush:c#;">
...
<p>Контроллер имеет только один метод, который устанавливает свойство ViewBag.Message и генерирует объект ActionResult. А также добавим для метода Index представление.</p>
<p>Теперь перейдем к проекту тестов и добавим в него новый класс тестов. Для этого мы можем добавить либо стандартный класс, либо использовать специальный шаблон файлов. 
Для этого в проекте тестов нажмем правой кнопкой мыши на каталог Controllers и в появившемся контекстном меню выберем <span class="b">Add-&gt;Unit Test...</span>:</p>
<img src="./pics/18.7.png" alt="Добавление тестов в проект ASP.NET MVC 5" />
<p>По умолчанию добавляет класс UnitTest1. Во-первых, изменим название класса и файла на <span class="b">StoreControllerTest</span>. 
Затем изменим следующим образом сам класс:</p>
<pre class="brush:c#;">
using System;
...
Метод <code>IndexViewEqualIndexCshtml</code> проверяет название вызываемого представления с помощью вызова <code>Assert.AreEqual</code>. А метод 
<code>IndexStringInViewbag</code> проверяет значение строки в свойстве ViewBag.Message.</p>
<p>Хотя у нас только один метод в контроллере, для него мы создали три тестовых метода для теста каждого отдельного тестового действия. Подобная изоляция 
облегчает тестирования отдельных участков кода.</p>
<p>Перед запуском тестов перестроим главный проект. И запустим тесты. В этом случае мы увидим, что один тест не пройден - тот, который верифицирует представление:</p>
<img src="./pics/18.6.png" alt="Создание юнит-тестов в ASP.NET MVC 5" />
<p>Тест не пройден, потому что при вызове метода <code>View</code> нам надо явным образом указывать представление. Поэтому изменим метод Index в главном проекте 
следующим образом:</p>
<pre class="brush:c#;">
public ActionResult Index()
{
...
<p>Снова запустим тесты. И теперь уже все тесты должны быть успешно пройдены.</p>
<p>Все три действия имеют одну и ту же секцию Arrange, и, возможно, было бы неплохо сразу установить все начальные настройки для всех методов. Для этого 
изменим в тестовом проекте класс <code>StoreControllerTest</code> следующим образом:</p>
<pre class="brush:c#;">
[TestClass]
public class StoreControllerTest
{
...
<p>Атрибут <span class="b">TestInitialize</span> позволяет задать метод, который выполняет начальную инициализацию для каждого отдельного теста. Благодаря 
этому код сокращен, а в тестовых методах оставлены только части Assert. Однако подобный подход надо принимать с осторожностью, так как он осложняет возможности по 
изменению кода. В данном случае общий контекст очень прост, но если при изменении методов будет изменяться и их контекст, то придется вносить большие изменения 
во всех классах тестов, а не только в отдельный метод для тестов.</p>
<h3>Класс Assert и тестирование результата</h3>
<p>Класс <span class="bb">Assert</span> из пространства имен <code>Microsoft.VisualStudio.TestTools.UnitTesting</code> 
с помощью своих статических методов позволяет верифицировать результат выполнения некоторого действия. Ранее уже было рассмотрено несколько методов, в частности, 
метод <code>Assert.IsNotNull()</code>, проверяющий, не равен ли некоторый объект значению null. Кроме того, при тестировании нам доступен еще 
ряд методов:</p>
<ul>
<li><p><code>AreEqual(object expected, object actual)</code>: проверяет, равны ли оба объекта. Имеет различные перегруженные версии, позволяющие 
сравнивать различные типы объектов</p></li>
<li><p><code>AreEqual&lt;T&gt;(T expected, T actual)</code>: обобщенная версия предыдущего метода. Например, <code>Assert.AreEqual&lt;string&gt;("Index", result.MasterName)</code></p></li>
<li><p><code>AreNotEqual(object expected, object actual)</code>: проверяет, не равны ли оба объекта. Тест проходит успешно, если объекты не равны</p></li>
<li><p><code>AreNotEqual&lt;T&gt;(T expected, T actual)</code>: обобщенная версия предыдущего метода</p></li>
<li><p><code>AreSame(object expected, object actual)</code>: проверяет, указывают ли оба объекта на один и тот же объект в памяти</p></li>
<li><p><code>AreNotSame(object expected, object actual)</code>: проверяет, указывают ли оба объекта на разные объекты в памяти. Если они указывают на один и тот же объект, то тест заканчивается неудачно</p></li>
<li><p><code>Equals(object objA, object objB)</code>: проверяет на равенство оба объекта</p></li>
<li><p><code>IsFalse(bool condition)</code>: проверяет, равно ли условие condition значению false</p></li>
<li><p><code>IsTrue(bool condition)</code>: проверяет, равно ли условие condition значению true</p></li>
<li><p><code>IsNull(object value)</code>: проверяет, имеет ли объект value значение null</p></li>
<li><p><code>IsInstanceOfType(object value, Type expectedType)</code>: проверяет, представляет ли объект value тип expectedType</p></li>
</ul>
<p>Используя эти методы, мы можем проверить различные ситуации в своем приложении.</p>
...
<div class="nav"><p><a href="./18.2.php">Назад</a><a href="./">Содержание</a><a href="./18.4.php">Вперед</a></p></div></div>
<div class="socBlock">
