...
<html >
<head>
<title>Подключение к базе данных в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение к базе данных в ASP.NET MVC 5">
...
<h2>Подключение к базе данных</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для хранения данных приложению нужна база данных. Мы можем использовать различные СУБД, но, как правило, в качестве базы данных в связке с ASP.NET MVC 
используется база данных MS SQL Server, на примере которого мы и посмотрим весь процесс создания БД и подключения к ней.</p>
<p>Мы можем создать базу данных прямо в проекте, либо же создать ее на сервере MS SQL. Для хранения баз данных в проекте предназначена папка <span class="bb">App_Data</span>. 
Итак, нажмем на папку App_Data правой кнопкой мыши и в появившемся контекстном меню выберем <span class="bb">Add-&gt; New Item...</span>. В 
появившемся окне добавления нового элемента выберем <span class="bb">SQL Server Database</span> и назовем новую базу данных Bookstore.mdf:</p>
<img src="./pics/5.1.png" />
<p>После этого в папку папку App_Data будет добавлена база данных, и мы можем начинать с ней работать - добавлять таблицы и данные. Но перед 
этим посмотрим, что у нас будет хранится в БД. Для работы с БД возьмем модели из предыдущих глав - модель Book:</p>
<pre class="brush:c#;">
public class Book
{
...
<p>Пусть для работы с ними определен следующий контекст данных:</p>
<pre class="brush:c#;">
...
<p>Чтобы связать приложение, контекст данных и БД, добавим в файл <span class="ii">web.config</span> строку подключения к этой базе данных. Для этого после 
секции configSections вставим следующую секцию:</p>
<pre class="brush:xml;">
&lt;connectionStrings&gt;
...
<p>Обратите внимание, что значени атрибута name (<code>name="BookContext"</code>) в качестве значения имеет название контекста данных.</p>
<p>Есть также второй способ. Например, если у нас строка подключения имеет другое название, например <code>&lt;add name="DefaultConnection"....</code>, и мы не хотим его менять, 
то в этом случае для связи с контекстом данных мы можем передать название в конструкор базового класса:</p>
<pre class="brush:c#;">
public class BookContext : DbContext
{
...
<p>В Visual Studio мы можем использовать LocalDB. LocalDB представляет облегченную версию движка баз данных SQL Sever Express, 
которая специально нацелена на разработчиков. Поэтому в данном случае в качестве источника данных указываем <span class="b">(LocalDB)\MSSQLLocalDB</span>.</p>
<p>Использование подстановки <span class="ii">|DataDirectory|</span> позволяет опустить полный физический путь к базе данных, которая хранится в папке App_Data.</p>
<p>Если мы используем не движок localdb, а полноценный MS SQL Server, то тогда в строку подключения включается название сервера:</p>
<pre class="brush:xml;">
&lt;connectionStrings&gt;
...
<p>Кроме того, здесь также вместо AttachDbFilename используется название каталога бд "Initial Catalog".</p>
<p>Для других СУБД - MySQL, MongoDB, Postgre и т.д. строка подключения также может отличаться. Но в данном случае мы будем использовать LocalDB.</p>
<p>Теперь создадим сами таблицы. Для этого перейдем в окно <span class="bb">Server Explorer</span> и раскроем в нем узел Data Connections. 
Далее мы увидим узел BookContext, который представляет нашу БД, и также раскроем его:</p>
<img src="./pics/5.2.png" />
<p>Добавим первую таблицу. Для этого нажмем правой кнопкой мыши на узел Tables и в появившемся меню выберем пункт <span class="ii">Add New Table</span>. 
...
то таблица будет называться <code>Books</code>. А Entity Framework автоматически распознает, что таблица <code>Books</code> соответствует 
классу <code>Book</code>.</p>
<p>Итак, создадим структуру таблицы:</p>
<img src="./pics/5.3.png" />
<p>Также поскольку предполагается, что столбец Id будет инкрементироваться с добавлением каждого нового объекта, то установим для него автоинкремент 
в окне Properties:</p>
<img src="./pics/5.4.png" />
<p>И последний шаг - генерация базы данных. Для этого нажмем на кнопку Update:</p>
<img src="./pics/5.5.png" />
<p>В появившемся диалоговом окне нажмем на кнопку Update Database. После этого в нашу базу данных добавится новая таблица. 
Подобным образом определим таблицу Purchases для модели Purchase:</p>
<img src="./pics/5.6.png" />
<p>Добавим в дизайнере баз данных в таблицу Books несколько записей. Для этого нажмем в окне Server Explorer на узел Books и выберем в появившемся списке 
пункт Show Table Data. Добавим, к примеру, следующий набор записей:</p>
<img src="./pics/5.7.png" />
<p>Теперь мы можем получить эти данные в контроллере Home и передать их в представление:</p>
<pre class="brush:c#;">
...
<p>Вывод данных в представлении Index.cshtml:</p>
<pre class="brush:xml;">
...
<p>Чтобы наверняка быть уверенным, что подключение к базе данных закрыто, следует вызывать метод <code>Dispose</code> у контекста данных:</p>
<pre class="brush:c#;">
...
<p>Это переопределенная версия метода Dispose контроллера, которая вызывается при уничтожении объекта контроллера. В нее 
помещается вызов <code>db.Dispose()</code>, который уничтожает все связанные с контекстом данных ресурсы и подключения.</p>
