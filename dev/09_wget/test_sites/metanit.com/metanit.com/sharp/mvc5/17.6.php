...
<html >
<head>
<title>CRUD-операции в JqGrid</title>
<meta charset="utf-8" />
...
<h2>CRUD-операции в JqGrid</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>CRUD операции предполагают возможность чтения отдельных записей, добавление, редактирование, а также удаление. Добавим подобную функциональность 
и в JqGrid. Для этого изменим представление следующим образом:</p>
<pre class="brush:xml;">
@{
    Layout = null;
}
...
<p>В отличие от представления прошлой темы в определении столбцов добавлена следующая опция: <code>editable: true,</code>. Она позволит при генерации 
форм добавления/редактирования создать для данного столбца поле. Поскольку свойство Id нам не надо менять или устанавливать вручную, то для 
него эта опция не указывается.</p>
<p>Также здесь подключена функциональность просмотра отдельных записей, добавления, редактирования и удаления: 
<code>add: true, del: true, edit: true, view: true,</code>, кроме того, также устанавливается текст меток. Опять же можно было бы и не устанавливать текст, 
тогда бы были просто иконки для данных функциональностей.</p>
<p>Далее определяются функции для каждого действия, кроме просмотра. Так как реализация всех трех действий похожа, то весь код определен в одной 
функции <code>update</code>. При этом вызов функций идет в определенном порядке: сначала редактирование, потом добавление и в конце удаление. 
В качестве параметра функция update принимает строковое описание действия, и в зависимости от его значения в обработчике 
отправки формы <code>onclickSubmit</code> обращается к тому или иному методу контроллера:</p>
<pre class="brush:js;">
onclickSubmit: function (params) {
    var list = $("#jqg");
    var selectedRow = list.getGridParam("selrow");
...
<p>В данном случае переменная <code>params.url</code> хранит адрес, по которому форма с данными будет отправляться. При этом нам не надо явным образом создавать 
какую-либо форму, плагин JqGrid все сделает за нас.</p>
<p>И также определена функция <code>afterSubmit</code>, вызываемая после получения ответа от сервера после отправки формы. Она выполняет обновление грида:</p>
<pre class="brush:js;">
afterSubmit: function (response, postdata) {
    $(this).jqGrid('setGridParam', { datatype: 'json' }).trigger('reloadGrid')
    return [true, "", 0]
...
<p>Она возвращает объект в виде массива, где первый элемент показывает, что запрос выполнился удачно, поэтому true, в случае ошибке можно было 
указать false. Второй параметр указывает на сообщение от сервера. А третий параметр должен содержать id добавленной или обновленной или удаленной записи. 
Для простоты примера я сделал сообщение пустым, а вместо id указал число 0.</p>
<p>Теперь изменим код контроллера, добавив в него новые методы, к которым будет идти обращение из представления. Итак, добавим следующие методы:</p>
<pre class="brush:c#;">
[HttpPost]
...
<p>Запустив приложение, мы можем просмотреть отдельную запись:</p>
<img src="./pics/17.10.png" />
<p>Добавить новую или редактировать старую (форма редактирования аналогична форме добавления):</p>
<img src="./pics/17.11.png" alt="Add to JqGrid" />
<p>Удалить старую:</p>
<img src="./pics/17.12.png" alt="Delete in JqGrid" />
<p>Таким образом, мы рассмотрели некоторые аспекты использования плагина JqGrid для создания гридов с богатой функциональностью.</p>
...
<div class="nav"><p><a href="./17.5.php">Назад</a><a href="./">Содержание</a><a href="./17.7.php">Вперед</a></p></div></div>
<div class="socBlock">
