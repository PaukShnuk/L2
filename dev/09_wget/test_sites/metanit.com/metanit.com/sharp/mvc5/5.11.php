...
<html >
<head>
<title>Передача массивов и сложных данных в контроллер ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Передача массивов и сложных данных в контроллер</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Из предыдущих тем мы узнали, как передавать отдельные объекты из представления в методы действия контроллера в качестве параметров. 
Но в реальности может возникнуть ситуация, что потребуется передать в метод не один объект типа <code>int</code> или какой-нибудь модели, а 
сразу несколько объектов. Посмотрим на некоторые возможные случаи.</p>
...
<p>Поэтому при отправке формы будет формироваться коллекция из <code>names</code>, состоящая из четырех элементов. И в методе контроллера 
...
<p>Мы можем передать в представление массив объектов некоторой модели, например, модели <code>Book</code>:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>Так мы сгенерируем для каждого объекта набор полей для редактирования его свойств. И после нажатия кнопки весь данный массив отправится 
на сервер, где его можно получить таким образом:</p>
<pre class="brush:c#;">
[HttpPost]
...
<h3>Передача разных объектов одной модели</h3>
<p>В предыдущем случае мы передавали коллекцию объектов модели <code>Book</code>. Однако может возникнуть ситуация, когда мы должны разграничить 
как-то переданные объекты, а не рассматривать их как одну коллекцию. Например, метод контроллера может выглядеть так:</p>
<pre class="brush:c#;">
[HttpPost]
...
<p>Вторую модель - <code>myBook</code> мы создаем уже в самом представлении. Все поля модели генерируются с помощью хелпера <code>@Html.EditorFor(m=&gt;myBook)</code>. 
Обратите внимание на имя модели - <span class="bb">myBook</span>. Так как мы ожидаем, что данный объект должен быть передан в метод в качестве параметра <code>myBook</code>, 
то он должен иметь точно такое же имя, а не произвольное.</p>
...
<p>Модель автора содержит ссылку на коллекцию книг. В контроллере мы получаем эту модель:</p>
<pre class="brush:c#;">
[HttpPost]
...
<p>Тогда представление могло бы выглядеть так:</p>
<pre class="brush:c#;">
...
<p>Так как модель Author содержит набор книг в свойстве Books, то атрибут name соответствующих текстовых элементов должно иметь следующую форму: 
<code>name="Books[0].name"</code>. С помощью кода javascript мы можем динамически добавить новые элементы:</p>
<img src="./pics/send_array_tocontroller.png" alt="Отправка в контроллер MVC массивов" />
