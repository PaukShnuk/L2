...
<html >
<head>
<title>ASP.NET MVC 5 | Внедрение зависимостей с параметрами в Ninject</title>
<meta charset="utf-8" />
<meta name="description" content="Внедрение зависимостей с параметрами с помощью IoC контейнера Ninject в ASP.NET MVC 5">
...
<h2>Внедрение зависимостей с параметрами</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 28.10.2017</div>
...
<p>Используемый при внедрении зависимостей объект BookRepository был довольно простым. Немного усложним его, добавив конструктор с параметром:</p>
<pre class="brush:c#;">
...
<p>Теперь при сопоставлении типов надо устанавливать аргумент, передаваемый в конструктор. Для этого используется метод <code>WithConstructorArgument</code>. 
Изменим в классе NinjectRegistrations метод Load:</p>
<pre class="brush:c#;">
...
<p>Метод WithConstructorArgument принимает два параметра: название параметра конструктора и значение.</p>
<p>Подобным образом мы могли бы вынести контекст данных в репозитории в отдельное свойство:</p>
<pre class="brush:c#;">
public class BookRepository : IDisposable, IRepository
{
    private BookContext db;
...
<p>Тогда установить нужное значение для свойства можно с помощью метода <span class="b">WithPropertyValue</span>:</p>
<pre class="brush:c#;">
public class NinjectRegistrations : NinjectModule
{
	public override void Load()
...
<p>Метод WithPropertyValue также принимает два параметра: название свойства и значение для него.</p>
...
<div class="nav"><p><a href="./21.2.php">Назад</a><a href="./">Содержание</a><a href="./21.4.php">Вперед</a></p></div></div>
<div class="socBlock">
