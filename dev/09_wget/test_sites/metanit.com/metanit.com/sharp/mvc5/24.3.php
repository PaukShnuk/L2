...
<html >
<head>
<title>ASP.NET MVC 5 | Удаленное тестирование приложения</title>
<meta charset="utf-8" />
<meta name="description" content="Запуск и тестирование веб-приложения на ASP.NET MVC 5 в локальной подсети из под мобильных и других удаленных устройств">
...
<h2>Удаленное тестирование приложения</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 20.08.2016</div>
...
<p>В данной статье мы рассмотрим запуск и тестирование приложения в локальной подсети, то есть как сделать приложение, запущенное в Visual Studio, доступным для всех устройств, 
подключенных к одной и той же сети. В данном случае речь идет не о полноценной публикации, а только о доступе в рамках подсети.</p>
<p>Для чего это может быть нужно? Это может быть нужно, если мы хотим протестировать наше приложение, запущенное на Visual Studio, из других устройств 
подсети. К примеру, я сделал мобильное приложение, которое обращается к моему же веб-приложению на ASP.NET. И мне надо протестировать взаимодействие между ними. 
Разворачивать полноценно веб-приложение на каком-то внешнем хостинге или локальном IIS только для этой цели не имеет смысла. И гораздо проще было бы 
запустить на Visual Studio веб-приложение и тут же его протестировать.</p>
<p>По умолчанию даже если мы запустим проект из Visual Studio на выполнение, и он запустится по какому-нибудь адресу типа <span class="ii">http://localhost:xxxx</span>, 
то на других устройствах, подключенных к одной подсети, он все равно будет не доступен. И чтобы сделать его доступным, необходимо выполнить ряд шагов. Рассмотрим два способа.</p>
<h3>Первый способ</h3>
<p>Прежде всего нам надо точно узнать адрес в под сети для того компьютера, на котором будет запускаться приложение. Это можно сделать в командной строке с помощью 
команды <span class="b">ipconfig</span>:</p>
<img src="./pics/24.7.png" alt="ipconfig in Windows 10" />
<p>Здесь нас интересует <span class="b">IPv4-адрес</span>, который в моем случае равен "192.168.1.18". То есть этот адрес компьютера в локальной сети.</p>
<p>Теперь надо настроить приложение, чтобы оно было доступно по этому адресу.</p>
<p>При создании проекта asp.net mvc 5 или asp net web api соответствующие данные о нем заносятся в специальный конфигурационный файл. При работе с VS 2013 этот файл находитя по пути 
<span class="ii">C:\Users\[Имя Пользователя]\Documents\IISExpress\config\applicationhost.config</span>. А при создании приложений в VS 2015 файл будет находиться на жестком диске в каталоге проекта по пути 
<span class="b">/{каталог проекта}/.vs/config/applicationhost.config</span>.</p>
<p>Откроем нужный файл в зависимости от версии Visual Studio и найдем в нем узел, который устанавливает настройки для нашего проекта. Например, в моем случае проект называется 
XamarinService, и нужный мне узел в файле <span class="ii">applicationhost.config</span> будет выглядеть следующим образом:</p>
<pre class="brush:xml;">
...
<p>В данном случае нас интересует узел <code>bindings</code>, который устанавливает привязку к определенному адресу. Добавим в него новый элемент binding, в 
котором будет указан наш локальный адрес в подсети:</p>
<pre class="brush:xml;">
&lt;bindings&gt;
    &lt;binding protocol="http" bindingInformation="*:57792:localhost" /&gt;
...
<p>Следующий шаг будет состоять в открытии портов для брандмауэра Windows. Для этого нужно запустить командную строку от имени администратора и последовательно ввести 
следующие команды:</p>
<div class="console">
...
<p>Эта команда добавляет для утилиты http.sys необходимый адрес, по которому запускается приложение. В можем случае это адрес "http://192.168.1.18:57792/". 
...
<code>user=Все</code>. "Все" - это группа пользователей. Но надо отметить, что данная часть команды будет отличаться для разных локализаций. Например, для англоязычной версии было бы:</p>
<div class="console">
<pre class="consoletext">
netsh http add urlacl url=http://192.168.1.18:57792/ user=everyone
</pre>
</div>
<p>А для ОС на немецком языке</p>
<div class="console">
<pre class="consoletext">
netsh http add urlacl url=http://192.168.1.18:57792/ user=Jeder
</pre>
</div>
<p>И после этого надо ввести вторую команду:</p>
<div class="console">
<pre class="consoletext">
...
<p>Эта команда добавляет к файерволлу Windows новое правило, которое разрешает все входящие подключения для порта 57792 (по которому в моем случае 
запускается приложение) для всех устройств из локальной подсети.</p>
<p>И после этой настройки мы можем обратиться к запущенному приложению с любого устройства в подсети, например, с подключенного через WI-FI смартфона:</p>
<img src="./pics/24.8.png" alt="Тестирование приложения ASP.NET MVC 5 из смартфона" />
<p>Если в будущем возникнет необходимость удалить эти установки, то это можно сделать с помощью команды:</p>
<div class="console">
<pre class="consoletext">
...
<h3>Второй способ</h3>
<p>Второй способ заключается в применении специального пакета для NodeJS, который называется <span class="b">iisexpress-proxy</span>. В этом случае нам не надо 
изменять файл applicationhost.config и добавлять исключения в файерволл Windows. Соответственно, гораздо меньше вероятности где-то ошибиться, однако при этом 
на целевом компьютере, на котором запускается веб-приложения, должен быть установлен NodeJS.</p>
<p>Для установки iisexpress-proxy выполним в командной строке следующую команду:</p>
<div class="console">
<pre class="consoletext">
npm install -g iisexpress-proxy
</pre>
</div>
<p>Далее, чтобы указать перенаправление с локального порта на прокси-порт, нам надо выполнить команду: <span class="ii">iisexpress-proxy [localPort] to [proxyPort]</span>. 
<code>localPort</code> - это локальный порт нашего приложения, а <code>proxyPort</code> - это прокси-порт (от 1025 до 65535), по которому мы будем обращаться 
к приложению. Например, в моем случае это будет команда:</p>
<div class="console">
<pre class="consoletext">
iisexpress-proxy 57792 to 3000
</pre>
</div>
<p>То есть все вызовы будут направляться с localhost:57792 на 192.168.1.18:3000.</p>
<p>При вводе этой команды может отобразиться окошко с призывом добавить исключение для файерволла:</p>
<img src="./pics/24.9.png" alt="iisexpress-proxy в ASP.NET MVC 5" />
<p>Добавим исключение, и после этого начнет работать проксирование одного адреса на другой:</p>
<img src="./pics/24.10.png" alt="iisexpress-proxy" />
<p>При запуске указывается, по какому адресу будет доступно приложение. То есть теперь, чтобы обратиться к приложению с другого устройства подсети, я должен буду использовать адрес 
<span class="ii">http://192.168.1.18:3000</span>.</p>
<p>Чтобы прекратить проксирование, достаточно нажать на Ctrl-C.</p>
...
<div class="nav"><p><a href="./24.2.php">Назад</a><a href="./">Содержание</a><a href="./24.4.php">Вперед</a></p></div></div>
<div class="socBlock">
