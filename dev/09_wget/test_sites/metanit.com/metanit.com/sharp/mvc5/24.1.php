...
<html >
<head>
<title>ASP.NET MVC 5 | Действия по расписанию и Quartz.NET</title>
<meta charset="utf-8" />
<meta name="description" content="Действия по расписанию в ASP.NET MVC 5 с помощью Quartz.NET, cron">
...
<h1>Дополнительные статьи</h1><h2>Действия по расписанию и Quartz.NET</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 27.02.2018</div>
...
<p>Quartz.NET представляет открытый фреймворк для выполнения действий по расписанию в среде ASP.NET. Используем его в своем приложении. 
Для начала нам надо добавить Nuget-пакет, который называется по имени фреймворка:</p>
<pre>
Quartz.NET
</pre>
<img src="./pics/24.1.png" alt="Quartz.NET в ASP.NET MVC 5" />
<p>Что надо для работы с Quartz.NET? Надо, во-первых, определить работу, которая будет выполняться. И, во-вторых, надо определить триггер, который будет 
управлять выполнением работы, запускать ее, конфигурировать.</p>
<p>Итак, добавим в проект новую папку, которую назовем Jobs. В эту папку добавим новый класс <span class="b">EmailSender</span>, который будет представлять работу - 
рассылку электронной почты. Этот класс будет выглядеть следующим образом:</p>
<pre class="brush:c#;">
using Quartz;
using System.Net;
...
<p>Класс работы должен реализовать интерфейс <span class="b">IJob</span>, который определяет метод <code>Execute()</code>, собственно выполняющий некоторую работу. 
...
виртуального пользователя user@yandex.ru.</p>
<p>В зависимости от почтового сервера настройки объекта SmtpClient, который отправляет письмо, могут различаться.</p>
<p>Далее также добавим в эту папку Jobs новый класс <span class="b">EmailSheduler</span>, который будет запускать работу:</p>
<pre class="brush:c#;">
using Quartz;
using Quartz.Impl;
...
<p>В статическом методе <code>Start()</code> вначале создаем объект расписания <span class="b">IScheduler</span> и запускаем его вызовом <code>scheduler.Start()</code>.</p>
<p>Затем создаем саму работу, которая будет выполняться в виде объекта <span class="b">IJobDetail</span>.</p>
<p>Следующим шагом идет создание и конфигурация триггера, который запускает работу. Здесь возможны различные опции. Но в данном случае указывается, 
что триггер будет называться "trigger1" (произвольное название) и будет принадлежать к группе "group1". Он будет запускать работу сразу после начала выполнения и будет выполнять ее раз 
в минуту. То есть пользователю будут раз в минуту приходить письма от администратора. И эти действия будут повторяться бесконечно.</p>
<p>И в конце происходит запуск всего действия:</p>
<pre class="brush:c#;">scheduler.ScheduleJob(job, trigger);</pre>
<p>Несколько слов по поводу возможных настроек отправки. Вместо минутного интервала выполнения мы можем задать еще ряд других:</p>
<ul>
<li><p><code>WithInterval(milliseconds)</code>: интервал выполнения в миллисекундах</p></li>
<li><p><code>WithIntervalInSeconds(seconds)</code>: интервал в секундах</p></li>
<li><p><code>WithIntervalInHours(hours)</code>: интервал в часах</p></li>
<li><p><code>WithRepeatCount(number)</code>: определяет количество повторов</p></li>
</ul>
<p>Момент запуска, который задается в триггере вызовом <code>StartNow()</code>, также имеет различные варианты:</p>
<ul>
<li><p><code>StartNow()</code>: запуск сразу же после начала выполнения</p></li>
<li><p><code>StartAt()</code>: определяет время, когда триггер начинает запускать работу</p></li>
<li><p><code>EndAt</code>: определяет время, когда триггер перестает запускать работу</p></li>
</ul>
<p>И чтобы работа начала выполняться по расписанию со стартом приложения, изменим класс Global.asax.cs:</p>
<pre class="brush:c#;">
...
<p>И после старта приложения раз в минуту начнут отправляться письма.</p>
...
<div class="nav"><p><a href="./23.12.php">Назад</a><a href="./">Содержание</a><a href="./24.2.php">Вперед</a></p></div></div>
<div class="socBlock">
