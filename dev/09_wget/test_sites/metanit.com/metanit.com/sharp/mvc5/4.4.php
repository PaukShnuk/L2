...
<html >
<head>
<title>Частичные представления в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Частичные представления partial views в ASP.NET MVC 5, метод PartialView и Html.Partial, встраивание частичных представлений в обычные">
...
<h2>Частичные представления</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Кроме обычных представлений метод действия может также возвращать частичные представления. Их отличительной особенностью является то, что 
...
результатов AJAX-запроса.</p>
<p>За рендеринг частичных представлений отвечает объект <code>PartialViewResult</code>, который возвращается методом <span class="b">PartialView</span>. 
Итак, определим в контроллере новое действие Partial:</p>
<pre class="brush:c#;">
...
<p>Теперь добавим новое представление <code>Partial.cshtml</code>. Для этого при создании представления в настройках укажем, что оно будет частичным:</p> 
<img src="./pics/4.3.png" alt="Создание частичных представлений в ASP.NET MVC 5" />
<p>После этого в проект будет добавлен пустой файл частичного представления. Итак, добавим в частичное представление следующую строку:</p>
<p><code>&lt;h2&gt;@ViewBag.Message&lt;/h2&gt;</code></p>
<p>После этого мы можем к обратиться к действию Partial, как к обычному действию контроллера, и оно нам вернет частичное представление:</p>
<img src="./pics/4.4.png" />
...
<p>Но смысл применения частичных представлений состоит не в этом, иначе они бы никак не отличались от обычных. Поэтому теперь встроим его в какое-нибудь другое представление. 
Для этого нам надо применить в любом месте обычного представления хелпер <span class="b">Html.Partial</span>:</p>
<p><code>@Html.Partial("Partial")</code></p>
<p>В данном случае в качестве параметра мы указываем имя частичного представления без расширения файла. Но в этом случае надо учитывать, что 
передать <code>ViewBag.Message</code> из метода Partial, как в предыдущем примере, мы уже не сможем. И если мы все таки хотим передать его в частичное представление, 
...
в представлении <span class="ii">Index.сshtml</span>, то в методе Index мы можем написать:</p>
<pre class="brush:c#;">
...
<p>И, таким образом, сообщение во <code>ViewBag.Message</code> будет передано как главному, так и частичному представлению.</p>
<p>Кроме хелпера <code>Html.Partial</code> частичное представление можно встроить с помощью другого хелпера - <code>Html.RenderPartial</code>. 
...
обрамляется фигурными скобками:</p>
<p><code> @{Html.RenderPartial("Partial");}</code></p>
<p>Еще одно отличие между двумя способами заключается в том, что <code>Html.RenderPartial</code> напрямую пишет вывод 
в выходной поток, поэтому может работать чуть быстрее, чем <code>Html.Partial</code> .</p>
<p>Также как и в случае с обычными представлениями, мы можем создавать строго типизированные частичные представления, указав в шапке файла директиву @model:</p>
<pre class="brush:xml;">
...
<p>Тогда мы можем вызвать это представление так: <code>@Html.Partial("Partial", new string[] { "Russia", "USA", "Canada", "France" })</code></p>
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/TzIFvHmXnNE"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 19. Частичные представления в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
