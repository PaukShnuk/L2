...
<html >
<head>
<title>ASP.NET MVC 5 | Взаимодействие React.JS и ASP.NET</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие React.JS и ASP.NET MVC 5, выполнение и обработка ajax-запросов">
...
<h2>Взаимодействие React.JS и ASP.NET</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 03.02.2017</div>
...
<p>В рамках данной статьи продолжим работу с проектом из прошлой темы. На примере ASP.NET MVC 5 создадим простейший интерфейс управления данными. 
Хотя здесь используется MVC, но все те же действия могут быть легко перенесены и в Web API.</p>
<p>Добавим в проект класс модели Phone, с которой будем работать:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>И изменим код контроллера HomeController:</p>
<pre class="brush:c#;">
...
<p>В качестве источника данных здесь используется массив объектов Phone, но при необходимости его можно заменить на контекст EF для работы с базой данных.</p>
<p>Метод Index обрабатывает запрос к главной странице. Метод <code>GetPhones()</code> возвращает набор объектов в формате json. Метод <code>AddPhone()</code> 
обрабатывает post-запросы и выполняет добавление объекта в массив. И метод <code>DeletePhone()</code> удаляет объект из массива по id, обрабатывая 
запросы DELETE.</p>
<p>ASP.NET MVC 5 имеет одну особенность, которая состоит в том, что по умолчанию запросы PUT, DELETE, OPTIONS отключены. И нам надо их явным образом подключить, 
если мы хотим использовать их в своем проекте. Для этого в файл конфигурации <span class="ii">web.config</span> в узел <span class="b">&lt;system.webServer&gt;</span> 
надо добавить обработчик:</p>
<pre class="brush:xml;">
...
<p>То есть полностью узел будет выглядеть следующим образом:</p>
<pre class="brush:xml;">
&lt;system.webServer&gt;
	&lt;handlers&gt;
		&lt;remove name="Babel" /&gt;
...
<p>Если мы используем Web API, то там соответственно ничего не надо добавлять в файл конфигурации.</p>
<p>В проекте в папке Scripts определим следующий файл <span class="b">app.jsx</span>:</p>
<pre class="brush:js;">
...
<p>Здесь определен компонент PhonesList, который выводит список объектов. Для вывода каждого отдельного объекта предназначен компонент Phone. 
Для создания формы добавления нового объекта используется компонент PhoneForm.</p>
<p>Для отправки запросов к серверу здесь применяется стандартный объект <span class="b">XMLHttpRequest</span>. Хотя также можно использовать функции из jquery или какие-то специальные библиотеки.</p>
<p>При рендеринге компоненту PhonesList передается ряд адресов для взаимодействия с сервером. После завершения рендеринга компонента срабатывает метод 
<code>componentDidMount()</code>, в котором производится загрузка начальных данных.</p>
<p>В методе <code>onAddPhone()</code> получаем пришедший из компонента PhoneForm объект и посылаем его на сервер.</p>
<p>В методе <code>onRemovePhone()</code> получаем пришедший из компонента Phone объект и также посылаем его на сервер.</p>
<p>В обоих этих методах в случае успешного запроса выполняем повторную загрузку данных и переустанавливаем состояние компонента. В то же время надо отметить, 
что методы контроллера HomeController при добавлении/удалении возвращают добавленный/удаленный объект, и в принципе вместо повторной загрузки всех данных 
мы можем получать этот объект и уже в самом компоненте добавлять или удалять его из массива phones, тем самым снизив количество запросов и объем передаваемых данных.</p>
<p>Представление <span class="ii">Index.cshtml</span> остается тем же, что и в прошлой теме:</p>
<pre class="brush:xml;">
@{
    Layout = null;
}
...
<p>Запустим приложение, и мы сможем просматривать, добавлять и удалять данные:</p>
<img src="./pics/react4.png" alt="Использование React JS в ASP.NET MVC 5" />
...
<div class="nav"><p><a href="./25.1.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
