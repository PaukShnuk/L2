...
<html >
<head>
<title>Мастер-страницы в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Мастер-страницы _Layout.cshtml в ASP.NET MVC 5, вставка содержимого представлений с помощью RenderBody, определение секций через метод RenderSection">
...
<h2>Мастер-страницы</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Мастер-страницы используются для создания единообразного, унифицированного вида сайта. По сути мастер-страницы - это те же самые представления, 
но позволяющие включать в себя другие представления. Например, можно определить на мастер-странице общие для всех остальных представлений элементы, а также подключить общие стили и скрипты. 
В итог нам не придется на каждом отдельном представлении прописывать путь к файлам стилей, а потом при необходимости его изменять. 
А используемые на мастер-страницах заполнители или плейсхолдеры позволят вставить на их место другие представления.</p>
<p>По умолчанию при создании нового проекта ASP.NET MVC 5 в проект уже добавляется мастер-страница под названием <span class="ii">_Layout.chtml</span>, которую 
можно найти в каталоге <span class="ii">Views/Shared</span>. В приложении из второй главы мы ее изменили следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>На первый взгляд это обычное представление за одним исключением: здесь используется метод <code>@RenderBody()</code>, который является заместителем и на место которого 
потом будут подставляться другие представления, использующие данную мастер-страницу. В итоге мы 
сможем легко установить для представлений веб-приложения единообразный стиль.</p>
<p>Чтобы в представлении использовать мастер-страницу, нам надо в секции <span class="bb">Layout</span> указать путь к нужной мастер-странице. 
Например, в представлении Index.cshtml можно определить мастер-страницу так:</p>
<pre class="brush:xml;">
@{
    Layout = "~/Views/Shared/_Layout.cshtml";
}
</pre>
<p>Если мы не используем мастер-страницу, то указываем <code>Layout = null;</code>.</p>
<p>Также необязательно для всех представлений использовать одну и ту же мастер-страницу. Можно определить несколько мастер-страниц, например, одну для форума, 
одну для блога и т.д., и в зависимости от раздела сайта подключать нужную. Добавляются в проект они также как и обычные представления.</p>
<p>Мастер-страница может иметь несколько секций, куда представления могут поместить свое содержимое. Например, добавим к мастер-странице секцию footer:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Теперь при запуске предыдущего представления Index мы получим ошибку, так как секция Footer не определена. 
По умолчанию представление должно передавать содержание для каждой секции мастер-страницы. 
Поэтому добавим вниз представления Index секцию footer. Это мы можем сделать с помощью выражения <code>@section</code>:</p>
<pre class="brush:xml;">
@{
    Layout = "~/Views/Shared/_Layout.cshtml";
}
...
<p>Но при таком подходе, если у нас есть куча представлений, и мы вдруг захотели определить новую секцию на мастер-странице, нам придется 
изменить все имеющиеся представления, что не очень удобно. В этом случае мы можем воспользоваться одним из вариантов гибкой настройки секций.</p>
<p>Первый вариант заключается в использовании перегруженной версии метода <span class="bb">RenderSection</span>, которая позволяет указать, что данную секцию 
не обязательно определять в представлении. Чтобы отметить секцию <code>Footer</code> в качестве необязательной, надо передать в метод 
...
<h3>ViewStart</h3>
<p>Если у нас в проекте пара-тройка представлений, мы легко можем изменить вручную для каждого описание мастер-страницы в секции Layout, если, например, 
мы решим использовать другую мастер-страницу. Но если у нас много представлений, то это делать будет не очень удобно.</p>
<p>Для более гибкой настройки представлений предназначена страница <code>_ViewStart.cshtml</code>. 
Код этой страницы выполняется до кода любого из представлений, расположенных в том же каталоге. Данный файл последовательно применяется 
к каждому представлению, находящемуся в одном каталоге.</p>
<p>При создании проекта ASP.NET MVC 5 в каталог Views уже по умолчанию добавляется файл <code>_ViewStart.cshtml</code>. 
Этот файл определяет мастер-страницу, используемую по умолчанию:</p>
<pre class="brush:xml;">@{
    Layout = "~/Views/Shared/_Layout.cshtml";
}
</pre>
<p>Данный код выполняется до любого другого кода, определенного в представлении, поэтому из других представлений мы можем удалить секцию <code>Layout</code>. 
Если же представление должно использовать другую мастер-страницу, то мы просто переопределяем свойство <code>Layout</code>, дописывая его 
определение в начало представления.</p>
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/C3864w5Y5LI"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 18. Мастер-страницы в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
