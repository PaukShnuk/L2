...
<h2>Фреймворк Moq</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Кроме собственно фреймворков для создания и проведения юнит-тестов при тестировании часто бывают полезны такие фреймворки, которые 
...
и одним из самых популярных является <b>Moq</b>.</p>
<p>Итак, возьмем проект из прошлой темы и подключим Moq через NuGet в <span class="bb">проект тестов</span> (а не в проект веб-приложения):</p>
<img src="./pics/18.9.png" alt="Moq фреймворк в ASP.NET MVC 5" />
<p>После этого в узел References проекта тестов будет добавлена библиотека Moq, содержащая всю основную функциональность.</p>
<p>Теперь изменим класс тестов следующим образом:</p>
<pre class="brush:c#;">
using Moq;
.................
...
<p>Для доступа к функциональности Moq вначале подключается соответствующее пространство имен <code>using Moq;</code>.</p>
<p>Moq предназначен для имитации объектов. В данном случае имитируется функциональность репозитория. Для этого объект Mock типизируется 
соответствующим типом: <code>var mock = new Mock&lt;IRepository&gt;()</code></p>
<p>Затем выполняется настройка mock объекта с помощью метода <code>Setup</code>. Так как нам надо имитировать возвращение методом <code>GetComputerList()</code> 
набора объектов, то данный метод вызывается в методе Setup, а с помощью метода <code>Returns</code> определяем данный набор объектов.</p>
...
<p>Теперь запустим тест. Он должен завершиться неудачей, так как у нас не передается в методе Index в представление никакой модели. Поэтому изменим метод 
Index:</p>
<pre class="brush:c#;">
...
<p>И если мы сейчас запустим тест, то он пройдет успешно.</p>
<p>Теперь для примера добавим в класс тестов еще один метод:</p>
<pre class="brush:c#;">
[TestMethod]
...
<p>Цель данного метода - проверить сообщение, передаваемое через ViewBag. Причем я хочу, чтобы сообщение передавалось, если 
в базе данных больше 0 объектов. Для этого список, возвращаемый методом <code>GetComputerList()</code>, инициализируется одним объектом. А 
метод <code>Assert.AreEqual</code> проверяет оба сообщения.</p>
<p>Запустим тест и увидим, что он завершился неудачей. Теперь нам надо изменить метод Index в главном проекте, чтобы он соответствовал тесту:</p>
<pre class="brush:c#;">
public ActionResult Index()
{
    var model = repo.GetComputerList();
...
<p>Поскольку с помощью Moq мы имитировали в методе контроллера список с одним элементом, то если мы сейчас запустим тест, то он пройдет успешно, так как 
строка в тесте и значение ViewBag.Message будут совпадать.</p>
...
<div class="nav"><p><a href="./18.4.php">Назад</a><a href="./">Содержание</a><a href="./18.6.php">Вперед</a></p></div></div>
<div class="socBlock">
