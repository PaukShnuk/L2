...
<html >
<head>
<title>Шаблоны формирования в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h2>Шаблоны формирования</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Так как большинство приложений так или иначе основываются на стандартных CRUD-операция (Create - Read - Update - Delete), 
то зачастую разработчики вынуждены многократно создавать контроллеры и представления для одних и тех же действий: добавления, изменения, 
удаления и просмотра записей из БД. И чтобы облегчить разработчикам жизнь, команда MVC добавила такую полезную функциональность, как 
<span class="bb">шаблоны формирования</span> (scaffolding templates). Эти шаблоны позволяют 
по заданной модели и контексту данных сформировать весь необходимый базовый код контроллеров, а также всю разметку для представлений, с помощью которых можно управлять 
записями в БД.</p>
<p>Чтобы воспользоваться данной функциональностью, добавим новый контроллер. Нажмем правой кнопкой мыши на папку Controllers и выберем 
<span class="bb">Add -&gt; Controller...</span>. Далее в окне добавления нового контроллера нам будет предложено выбрать шаблон контроллера:</p>
<img src="./pics/5.10.png" alt="scaffolding templates в asp.net mvc 5" />
<p>Собственно к MVC относятся только первые три шаблона:</p>
<ul>
<li><p><span class="bb">MVC 5 Controller - Empty</span>. Этот шаблон добавляет в папку Controllers пустой контроллер, который имеет один 
единственный метод Index. Данный шаблон не создает представлений</p></li>
<li><p><span class="bb">MVC 5 Controller with read/write actions</span>. Данный шаблон добавляет в проект контроллер, который содержит методы Index, 
Details, Create, Edit и Delete. Однако эти методы не содержат никакой логики работы с базой данных. И нам предлагается самим создать для них код и представления.</p></li>
<li><p><span class="bb">MVC 5 Controller with views, using Entity Framework</span>. Это уже более интересный шаблон, который создает контроллер с 
методами  Index, Details, Create, Edit и Delete, а также все необходимые представления для этих действий и добавляет код для извлечения информации из базы данных.</p></li>
</ul>
<p>Выберем последний пункт, то есть <span class="bb">MVC 5 Controller with views, using Entity Framework</span>.</p>
<p>После этого откроется окно добавления нового контроллера, в котором нам будет предложено установить некоторые настройки:</p>
<ul>
<li><p><span class="bb">Controller name</span>: имя контроллера</p></li>
<li><p><span class="bb">Use async controller actions</span>: будут ли автоматические сгенерированные методы контроллера асинхронными. Установим 
данную опцию.</p></li>
<li><p><span class="bb">Model class</span>: класс модели. Выберем созданную ранее 
модель Book (либо какую-то другую имеющуюся модель)</p></li>
<li><p><span class="bb">Data context class</span>: класс контекста данных. Выберем контекст данных для выбранной модели.</p></li>
<li><p><span class="bb">Generate views</span>: надо ли генерировать представления к создаваемым действиям контроллера. При установке этой опции становятся 
доступными две следующие опции. Установим все эти опции.</p></li>
<li><p><span class="bb">Reference script libraries</span>: будут ли подключать представления библиотеки jquery и другие необходимые файлы javascript</p></li>
<li><p><span class="bb">Use a layout page</span>: будут ли генерируемые представления использовать мастер-страницу</p></li>
</ul>
<img src="./pics/5.11.png" />
<p>Установив все опции, нажмем кнопку Add, и в проект будет добавлен новый контроллер. Он будет выглядеть примерно следующим образом:</p>
<pre class="brush:c#;">
using System;
...
<p>А в папке <span class="ii">Views/Book</span> мы найдем все необходимые представления со всем необходимым кодом, который теперь нам не надо набирать вручную. 
И теперь мы можем запустить проект и перейти в адресной строке браузера к нашему контроллеру:</p>
<img src="./pics/5.12.png" />
<p>Благодаря шаблонам формирования мы можем не думать о создании кода для стандартных операций, что позволяет сэкономить уйму времени. Правда, 
после генерации кода все равно придется вносить правки, изменять автоматически сгенерированные названия на свои (например, название страницы, автоматические генерируемых ссылок и др.), 
однако от основной работы мы уже будем избавлены.</p>
