...
<html >
<head>
<title>Бандлы и минификация в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Бандлы и минификация в ASP.NET MVC 5, ScriptBundle и StyleBundle">
...
<h1>Бандлы и минификация</h1><h2>Введение в бандлы и минификацию</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В ASP.NET MVC 4 была введена концепция бандлов, которая помогает организовать файлы скриптов и стилей более эффективным путем для снижения 
издержек при передаче клиенту. Посмотрим, что представляют собой бандлы.</p>
<p>В предыдущих версиях MVC мы могли стандартным образом подключать скрипты и стили, например:</p>
<pre class="brush:xml;">
&lt;head&gt;
    &lt;meta name="viewport" content="width=device-width" /&gt;
...
<p>Фреймворк MVC для настройки путей предлагает нам использовать URL-хелперы:</p>
<pre class="brush:xml;">
&lt;head&gt;
    &lt;meta name="viewport" content="width=device-width" /&gt;
...
<p>Использование бандлов представляет совсем другой подход к использованию скриптов и стилей. При создании нового проекта MVC 5 
функциональность бандлов уже по умолчанию включается в приложение. Если вы посмотрите на стандартный код мастер-страницы <i>_Layout.cshtml</i>, то увидите совсем  
иной способ подключения скриптов и стилей:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
&lt;/head&gt;
&lt;body&gt;
    &lt;!-- Основное содержимое мастер-страницы --&gt;
...
<p>В данном случае подключаются не конкретные скрипты и стили, а именно бандлы. С помощью хелпера <code>Styles.Render</code> подключаются бандлы 
стилей, а с помощью хелпера <code>Scripts.Render</code> подключаются бандлы скриптов. Эти хелперы принимают в качестве параметра имя бандла.</p>
<p>По умолчанию проекты MVC 5 уже содержат регистрацию бандлов - в файле <span class="ii">BundleConfig.cs</span>, который находится в папке <span class="ii">App_Start</span>:</p>
<pre class="brush:c#;">
using System.Web;
...
<p>Здесь метод <code>RegisterBundles</code> добавляет все создаваемые бандлы в коллекцию <code>bundles</code>. Объявление бандла выглядит следующим образом: 
<code>new ScriptBundle("~/bundles/jquery").Include("~/Scripts/jquery-{version}.js")</code>.</p>
<p>В конструктор <code>ScriptBundle</code> передается виртуальный путь бандла. А с помощью метода <code>Include</code> уже включаются в данный бандл 
конкретные файлы скриптов.</p>
<p>В выражении <code>"~/Scripts/jquery-{version}.js"</code> параметр {version} является заменителем для любого символьного обозначения версии скрипта. 
...
так как система уже автоматически примет новую версию.</p>
<p>Выражение <code>"~/Scripts/jquery.validate*"</code> с помощью знака звездочки заменяет остальную часть строки. Например, это выражение 
подключит в бандл сразу два файла: <i>jquery.validate.js</i> и <i>jquery.validate.unobtrusive.js</i> (и их минимизированные версии), так как 
их названия начинаются с <code>jquery.validate*"</code>.</p>
<p>То же самое касается и создания бандлов стилей, только в этом случае используется класс <code>StyleBundle</code>.</p>
<p>Однако само объявление бандлов в файле <i>BundleConfig.cs</i> еще не подключает автоматически их в проект. Для этого в файле <i>Global.asax</i> 
прописывается соответствующая директива:</p>
<pre class="brush:c#;">
...
<p>Таким образом, мы и регистрируем бандлы для нашего приложения.</p>
<h3>Минификация</h3>
<p>Ключевым моментом концепции бандлов является минификация. Ее суть состоит в том, что при развертывании приложения клиенту отдается не полная, а минимизированная 
версия скриптов или стилей. За счет чего экономятся ресурсы сервера, так как идет отдача файла с меньшим объемом. В то же время в процессе отладки 
приложение отдает обычную версию, поскольку благодаря ей, нам проще разобраться в возможных ошибках. Посмотрим на примере. Вот у меня есть 
некоторая мастер-страничка, которая подключает кучу бандлов:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае я подключаю пять бандлов. Если запустить теперь приложение, то через средства разработчика в InternetExplorer 11 я могу увидеть 
скрипты, которые несут бандлы:</p>
<img src="./pics/13.1.png" />
<p>В данном случае у нас по умолчанию приложение находится в режиме отладки. В файле <i>web.config</i> можно найти следующую секцию:</p>
<pre class="brush:xml;">
&lt;configuration&gt;
  &lt;!---------------------------------------&gt;
  &lt;system.web&gt;
...
<p>Параметр <code>compilation debug="true"</code> указывает, что приложение запускается в режиме отладки. Теперь изменим его значение на false: 
<code>debug="false"</code> и перезапустим проект. Теперь мы увидим несколько иной результат:</p>
<img src="./pics/13.2.png" />
<p>И если вы посмотрите, например, в том же IE11 в средствах разработчика заголовки ответа, то вы увидите, что в этот раз в режиме развертывания сервер отдает нам минимизированный файл:</p>
<img src="./pics/13.3.png" />
<p>Таким образом, использование бандлов в приложениях на ASP.NET MVC повышает эффективность и производительность.</p>
...
<div class="nav"><p><a href="./12.21.php">Назад</a><a href="./">Содержание</a><a href="./13.2.php">Вперед</a></p></div></div>
<div class="socBlock">
