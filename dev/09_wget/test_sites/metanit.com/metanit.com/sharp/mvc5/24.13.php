...
<html >
<head>
<title>ASP.NET MVC 5 | Динамическая загрузка списка по выбору из другого списка</title>
<meta charset="utf-8" />
<meta name="description" content="Динамическая загрузка одного списка по выбору из второго через Ajax в ASP.NET MVC 5 и Web API">
...
<h2>Динамическая загрузка списка по выбору из другого списка</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 1.11.2015</div>
...
<p>Нередко у нас появляется необходимость динамически связать два выпадающих списка на форме. Рассмотрим, как это сделать</p>
<p>Допустим, у нас есть следующие модели, представляющие страну и город, а также класс контекста:</p>
<pre class="brush:c#;">
...
<p>В классе контроллера определим метод, который будет отдавать представление с двумя списками:</p>
<pre class="brush:c#;">
...
<p>Пусть по умолчанию в списке стран будет выбран первый элемент, поэтому также через ViewBag передаем данные для второго списка, который содержит 
города первой страны.</p>
<p>Также здесь определен метод, возвращающий частичное представление, в которое передается список городов по id страны.</p>
<p>Добавим для этого метода частичное представление <i>GetItems.cshtml</i>:</p>
<pre class="brush:xml;">
...
<p>Здесь формируется элемент select, то есть выпадающий список, по переданному списку городов.</p>
<p>И теперь определим главное представление:</p>
<pre class="brush:xml;">
@{
...
<p>Здесь подразумевается, что на мастер-странице перед секцией scripts уже подключены файлы jquery. При изменении выбора в спиcке городов идет ajax-запрос к методу 
GetItems, а возвращенное частичное представление заменяет имеющийся html-код списка городов.</p>
...
<div class="nav"><p><a href="./24.12.php">Назад</a><a href="./">Содержание</a><a href="./24.14.php">Вперед</a></p></div></div>
<div class="socBlock">
