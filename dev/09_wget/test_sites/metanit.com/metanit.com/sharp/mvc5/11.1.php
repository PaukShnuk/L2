...
<html >
<head>
<title>OWIN и Katana в ASP.NET MVC 5</title>
<meta charset="utf-8" />
...
<h1>OWIN и Katana</h1>
<h2>Введение в OWIN и Katana</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Одним из нововведений в .NET 4.5.1 стало внедрение спецификации OWIN и проекта Katana. Благодаря этим нововведениям становятся доступными новые способы 
построения веб-приложений. Рассмотрим, что они представляют.</p>
<p><span class="bb">OWIN</span> или <span class="bb">Open Web Interface for .NET</span> представляет собой спецификацию, определяющую взаимодействие 
между веб-приложением и веб-сервером. Современным веб-приложениям требуется больше гибкости. И ответом на эти вызовы стало появление спецификации OWIN.</p>
<p>Но OWIN - это спецификация, которая создавалась как открытый проект вне стен Microsofta. Непосредственной реализацией этой спецификации стал 
проект <span class="bb">Katana</span>.</p>
<p>Всю архитектуру проекта Katana можно представить следующим образом:</p>
<img src="./pics/11.1.png" alt="Архитектура Katana и OWIN" />
<p>Рассмотрим основные слои этой архитектуры:</p>
<ul>
<li><p><span class="bb">Хост</span>: процесс, который обрабатывает приложение. Если в качестве сервера приложения используется IIS, то подобным 
процессом является процесс самого сервера IIS. Однако им может быть и сам файл приложения. Основная его задача - запуск, загрузка компонентов OWIN и корректное завершение.</p></li>
<li><p><span class="bb">Сервер</span>: реализует спецификацию OWIN, выполняет наполнение словаря окружения из данных запроса и обрабатывает 
запрос на конвейере OWIN.</p></li>
<li><p><span class="bb">Промежуточное ПО или слой Middleware</span>: набор компонентов, обрабатывающих запросы в конвейере OWIN.</p></li>
<li><p>И само <span class="bb">приложение</span>, выполняющее определенную задачу.</p></li>
</ul>
<p>Что нам дает использование Katana в своих проектах? Katana позволяет создавать приложение, использующее различные веб-технологии, и запускать его где 
угодно. Посмотрим на простом примере.</p>
<p>В Visual Studio 2013 создадим новое приложение. В моем случае оно будет называться <i>KatanaApp</i>. В качестве шаблона приложения выберем 
шаблон <span class="bb">Empty</span>:</p>
<img src="./pics/11.2.png" alt="Создание проекта Katana и OWIN" />
<p>Итак, Visual Studio создаст абсолютно пустой проект. Функционал инфраструктуры OWIN содержится в библиотеке <i>Microsoft.Owin.Host.SystemWeb</i>. 
Для добавления библиотеки в проект воспользуемся менеджером NuGet:</p>
<img src="./pics/11.3.png" />
<p>Теперь добавим в проект класс запуска OWIN. Все приложения OWIN должны иметь подобный класс, в котором определяются компоненты конвейера приложения. 
Есть различные способы определения данного класса. Мы воспользуемся самым простым - просто добавим в проект соответствующий класс. Для этого нажмем 
правой кнопкой мыши на проект в окне Solution Explorer, в появившемся меню выберем <span class="bb">Add-&gt;New Item...</span> и 
в окне добавления нового компонента выберем <span class="bb">OWIN Startup class</span>:</p>
<img src="./pics/11.4.png" alt="OWIN Startup class" />
<p>Теперь изменим добавленный класс следующим образом:</p>
<pre class="brush:c#;">
using System;
...
<p>Во-первых, чтобы приложение знало, где у нас находится класс OWIN Startup, мы используем атрибут <code>[assembly: OwinStartup(typeof(KatanaApp.Startup1))]</code>. 
Так, теперь оно знает, что таковым классом является KatanaApp.Startup1.</p>
<p>Этот класс содержит один метод <span class="bb">Configuration</span>, который в качестве параметра принимает интерфейс <span class="bb">IAppBuilder</span>. 
...
которая имеет в качестве параметра делегат <span class="bb">Func&lt;IOwinContext,Task&gt;</span>. С помощью объекта <code>context.Response</code> 
создаем ответ и посылаем его клиенту.</p>
<p>При запуске мы увидим в браузере нашу строку:</p>
<img src="./pics/11.5.png" />
<p>В данном случае в качестве веб-сервера выступил IIS, он же выступает в качестве хоста. Однако одной из сторон проекта Katana и OWIN является 
то, что приложение само будет выступать и в качестве хоста, и в качестве сервера.</p>
<p>В основе такой организации лежит класс <span class="bb">HttpListener</span>, который может прослушивать входящие http-запросы и отвечать на них. 
То есть фактически может выступать в качестве веб-сервера. И хотя мы в большей степени говорим о веб-приложениях в рамках ASP.NET MVC, но чтобы посмотреть 
преимущества OWIN, создадим простое консольное приложение в Visual Studio, например, в Visual Studio 2013 for Desktop.</p>
<p>Вначале добавим через NuGet библиотеку <span class="bb">Microsoft.Owin.SelfHost</span>:</p>
<img src="./pics/11.6.png" />
<p>Теперь добавим в консольное приложение вышеопределенный класс Startup1 (не забывая изменить пространство имен класса на пространство имен нашего 
приложения). И изменим код класса Program следующим образом:</p>
<pre class="brush:c#;">
using System;
...
<p>Теперь мы можем запустить приложение. Оно будет фактически работать как веб-сервер на 8000 порту, и в браузере мы можем обратиться к нему, 
набрав в адресной строке <i>http://localhost:8000</i>.</p>
<h3>OWIN и Katana в проектах MVC 5</h3>
<p>Мы рассмотрели общую концепцию проектов OWIN и Katana, но как же они применяются в MVC 5? При создании проекта ASP.NET MVC 5, использующего идентификацию 
пользователей, мы сможем заметить применение OWIN и Katana в приложении:</p>
<img src="./pics/11.7.png" />
<p>Во-первых, так как проект использует Katana, то в нем имеется файл <i>Startup.cs</i> с одноименным классом:</p>
<pre class="brush:c#;">
using Microsoft.Owin;
using Owin;
...
<p>Этот тот же класс, что мы рассматривали ранее, только в данном случае он частичный (partial) и вызывает метод <code>ConfigureAuth</code>. Этот 
метод определен в другом файле - в файле <i>Startup.Auth.cs</i>, который находится в папке App_Start. В этом файле также находится определение частичного класса 
Startup:</p>
<pre class="brush:c#;">
...
<p>В методе ConfigureAuth производится аутентификация с помощью куки. По закомментированным строкам вы можете увидеть, что с помощью 
OWIN производится аутентификация с помощью внешних сервисов, таких как facebook, twitter и др. Кроме того, как мы дальше увидим, вся 
базовая система аутентификации и авторизации AspNet Identity базируется на компонентах OWIN и Katana.</p>
...
<div class="nav"><p><a href="./10.9.php">Назад</a><a href="./">Содержание</a><a href="./12.1.php">Вперед</a></p></div></div>
<div class="socBlock">
