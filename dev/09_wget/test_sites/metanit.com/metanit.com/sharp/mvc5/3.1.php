...
<html >
<head>
<title>Контроллеры в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Контроллеры в приложении ASP.NET MVC 5, добавление контроллеров, определение методов">
...
<h1>Контроллеры</h1><h2>Основы контроллеров</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
суффикс "Controller", остальная же часть до этого суффикса считается именем контроллера.</p>
...
<p>Однако Visual Studio предлагает нам более удобные средства для создания контроллеров, предполагающие их гибкую настройку. Чтобы ими воспользоваться, 
нажмем на папку Controllers правой кнопкой мыши и в появившемся меню выберем <i>Add -&gt; Controller...</i>. После этого нам отобразится окно создания 
нового контроллера:</p>
<img src="./pics/3.1.png" alt="Создание нового контроллера в ASP.NET MVC 5" />
<p>Собственно к контроллерам MVC 5 здесь непосредственное отношение имеют первые три пункта. Остальные больше относятся к Web API 2. В этом 
списке выберем первый пункт - <span class="bb">MVC 5 Controller - Empty</span>, который подразумевает создание пустого контроллера. Остальные два 
пункта позволяют сгенерировать классы с CRUD-функциональностью на основе шаблонов формирования, о которых мы поговорим в разделе о моделях.</p>
<p>Далее нам будет предложено ввести имя, и после этого новый контроллер с единственным методом Index будет добавлен в проект. При таком добавлении в отличие от предыдущих примеров 
для данного контроллера будет автоматически создан каталог в папке Views, который будет хранить все представления, связанные с действиями этого контроллера.</p>
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/WZq5NNj9Bi4"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 8. Основы контроллеров в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
