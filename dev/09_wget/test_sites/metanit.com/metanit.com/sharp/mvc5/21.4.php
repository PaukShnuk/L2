...
<html >
<head>
<title>IoC-контейнер Autofac в ASP.NET MVC</title>
<meta charset="utf-8" />
...
<h2>IoC-контейнер Autofac</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>IoC-контейнер Autofac предоставляет аналогичную функциональность, что и Ninject, предлагая альтернативный способ внедрения зависимостей.</p>
<p>Возьмем тот же пример с репозиторием:</p>
<pre class="brush:c#;">
...
<p>И пусть у нас будет тот же контроллер:</p>
<pre class="brush:c#;">
...
<p>Используем для установки зависимостей IoC-контейнер Autofac. Для этого добавим в проект через NuGet два пакета: <i>Autofac</i> и 
<i>Autofac ASP.NET MVC 5 Integration</i>:</p>
<img src="./pics/autofac1.png" alt="Внедрение зависимостей с Autofac в ASP.NET MVC" />
<P>Каждый из этих пакетов надо устанавливать отдельно.</P>
<p>После добавления Autofac определим в проекте, например, в какой-нибудь дополнительной папке Util новый класс AutofacConfig:</p>
<pre class="brush:c#;">
using Autofac;
using Autofac.Core;
...
<p>Класс AutofacConfig определяет один статический метод <code>ConfigureContainer()</code>, который и выполняет всю настройку.</p>
<p>После этого остается вызвать данный метод в global.asax:</p>
<pre class="brush:c#;">
...
<p>Как и Ninject, Autofac предоставляет возможность устанавливать параметры и свойства у зависимостей. Например, класс репозитория принимает параметр:</p>
<pre class="brush:c#;">
public class BookRepository : IDisposable, IRepository
{
...
<p>Для установки параметров используется метод <span class="bb">WithParameter</span>:</p>
<pre class="brush:c#;">
builder.RegisterType&lt;BookRepository&gt;()
        .As&lt;IRepository&gt;()
        .WithParameter("context", new BookContext());
</pre>
<p>Первым параметром в метод передается название параметра, а вторым - значение для него. Если у нас используется несколько параметров 
(например, <code>public BookRepository(BookContext context, string connectionStr)</code>), то применяется метод 
<code>WithParameters()</code>:</p>
<pre class="brush:c#;">
builder.RegisterType&lt;BookRepository&gt;()
    .As&lt;IRepository&gt;()
...
<p>Для установки зависимостей в свойствах применяются методы <code>WithProperty/WithProperties</code>, деействие которых аналогично. Например, 
в репозитории есть свойство:</p>
<pre class="brush:c#">
public class BookRepository : IDisposable, IRepository
{
    private BookContext db;
...
<div class="nav"><p><a href="./21.3.php">Назад</a><a href="./">Содержание</a><a href="./21.5.php">Вперед</a></p></div></div>
<div class="socBlock">
