...
<html >
<head>
<title>Переопределение шаблонов формирования в ASP.NET MVC</title>
<meta charset="utf-8" />
...
<h2>Переопределение шаблонов формирования</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Ранее мы рассмотрели функциональность шаблонов формирования, которые позволяют автоматизировать процесс создания контроллера и представлений. Однако иногда их функциональности бывает недостаточно, и возникает необходимость переопределить их поведение.</p>
<p>Для начала посмотрим, что представляют имеющиеся шаблоны. Все шаблоны для Visual Studio 2013 хранятся в папке <i>C:\Program Files (x86)\Microsoft Visual Studio 12.0\Common7\IDE\Extensions\Microsoft\Web\Mvc\Scaffolding\Templates</i>. 
 Эта папка содержит следующие подкаталоги:</p>
<img src="./pics/5.26.png" alt="scaffolding in ASP.NET MVC 5" />
<p>Все подкаталоги представляют шаблоны, на основе которых генерируется код контроллеров или представлений. В частности, каталог <i>MvcView</i> содержит 
шаблоны для генерации представлений:</p>
<img src="./pics/5.27.png" />
<p>Шаблон представляет собой файл с расширением <i>.t4</i>, который содержит ряд директив, по которым затем генерируется код. Например, наиболее маленький 
шаблон <i>Empty.cs.t4</i> имеет следующий код:</p>
<pre class="brush:c#;">
...
<p>Все директивы и управляющие блоки шаблонов помещаются между тегами <code>&lt;#</code> и <code>#&gt;</code>. Например, директива <code>&lt;#@ output extension=".cshtml" #&gt;</code> указывает, 
что выходной файл после обработки должен иметь расширение <i>.cshtml</i>.</p>
<p>Блоки обычного текста располагаются вне тегов <code>&lt;#</code> и <code>#&gt;</code>. Блоки текста не обрабатываются и помещаются в выходной файл как есть.</p>
<p>Мы можем изменить какой-либо шаблон из существующих, однако изменение затронет все последующие случаи использования данного шаблона. И чтобы не менять глобально, 
мы можем добавить нужные шаблоны в проект в Visual Studio. Шаблоны из проекта в Visual Studio переопределяют поведение глобальных шаблонов из каталога 
<i>C:\Program Files (x86)\Microsoft Visual Studio 12.0\Common7\IDE\Extensions\Microsoft\Web\Mvc\Scaffolding\Templates</i> для данного проекта.</p>
<p>Чтобы переопределить шаблон, нам надо создать в проекте папку с именем <i>CodeTemplates</i>. А в нее затем поместить те шаблоны, которые надо переопределить. 
Например, переопределим шаблон <i>Delete.cs.t4</i>:</p>
<img src="./pics/5.28.png" />
<p>Поскольку шаблон <i>Delete.cs.t4</i> находится в папке <i>MvcView</i>, то в прокте надо создать соответствующую папку в CodeTemplates и затем скопировать 
туда шаблон. Кроме того, я добавил еще пару файлов, от которых зависит <i>Delete.cs.t4</i> и которые также находятся в папке 
<i>C:\Program Files (x86)\Microsoft Visual Studio 12.0\Common7\IDE\Extensions\Microsoft\Web\Mvc\Scaffolding\Templates\MvcView</i></p>
<p>Теперь изменим код шаблона <i>Delete.cs.t4</i> на следующий:</p>
<pre class="brush:c#;">
&lt;#@ template language="C#" HostSpecific="True" #&gt;
&lt;#@ output extension=".cshtml" #&gt;
&lt;#@ include file="Imports.include.t4" #&gt;
...
<p>Этот шаблон по сути ничего не делает кроме того, что выводит заголовок.</p>
<p>Первая директива указывает, что будет использоваться язык C#. Вторая строка говорит, что выходной файл будет иметь расширение <i>.cshtml</i>, 
а третья строка подключает файл.</p>
<p>Теперь, если мы попытаемся создать представление по шаблону <span class="bb">Delete</span>, то Visual Studio сгенерирует файл со следующим содержанием:</p>
<pre class="brush:xml;">
&lt;h2&gt;Шаблон удаления&lt;/h2&gt;
</pre>
<p>Теперь создадим уже свой более осмысленный шаблон. Для этого добавим в проект в папку <i>CodeTemplates/MvcView</i> какой-нибудь файл или другой шаблон и 
переименуем его на <span class="bb">MyTemplate</span>:</p>
<img src="./pics/5.29.png" />
...
<p>Этот шаблон принимает модель из контроллера и выводит все значения ее свойств в виде таблицы. Теперь мы можем использовать этот шаблон при создании 
представлений:</p>
<img src="./pics/5.30.png" />
...
<div class="nav"><p><a href="./5.13.php">Назад</a><a href="./">Содержание</a><a href="./5.15.php">Вперед</a></p></div></div>
<div class="socBlock">
