...
<html >
<head>
<title>Отправка файлов в ASP.NET MVC 5</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка файлов в ASP.NET MVC 5 с помощью метода File и классов FileResult, FileContentResult, FilePathResult">
...
<h2>Отправка файлов в ASP.NET MVC 5</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для отправки клиенту файлов предназначен <span class="bb">FileResult</span>. Однако так как это абстрактный класс, то фактически мы будем иметь дело с его наследниками:</p>
<ul>
<li><p><span class="bb">FileContentResult</span>: отправляет клиенту массив байтов, считанный из файла</p></li>
<li><p><span class="bb">FilePathResult</span>: представляет простую отправку файла напрямую с сервера</p></li>
<li><p><span class="bb">FileStreamResult</span>: данный класс создает поток - объект System.IO.Stream, с помощью которого считывает и отправляет файл клиенту</p></li>
</ul>
<p>Во всех трех случаях для отправки файлов применяется метод <span class="bb">File</span>, который и возвращает объект FileResult. 
Только в зависимости от выбранного способа используется соответствующая перегруженная версия этого метода.</p>
<p>Чтобы отправить файл из файловой системы (то есть использование объекта FilePathResult), нам надо указать в методе File три параметра: 
путь к файлу на стороне сервера, тип содержимого и имя файла для принимающей стороны (имя файла необязательно, и можно обойтись в принципе только двумя 
параметрами).</p>
<pre class="brush:c#;">
...
<p>Предполагается, что у нас в проекте есть папка Files, в которой лежит файл PDFIcon.pdf. Метод <span class="bb">Server.MapPath</span> позволяет построить 
полный путь к ресурсу из каталога в проекте. Но также можно использовать и абсолютные пути, 
обращаясь к любому файлу в файловой системе, например, <code>string file_path = @"C:\Book\PDFIcon.pdf";</code></p>
<p>И, при обращении, например, по пути <i>Home/GetFile</i> нам будет предложено сохранить данный файл на локальном компьютере.</p>
<p>Похожим образом работает и классы FileContentResult, только вместо имени файла в методе File указывается массив байтов, в который был считан файл:</p>
<pre class="brush:c#;">
...
<p>И если мы хотим возвратить объект FileStreamResult, то в качестве первого аргумента в методе File идет объект Stream для отправляемого файла:</p>
<pre class="brush:c#;">
// Отправка потока
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/MqJb2af8kWc"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 13. Отправка файлов в ASP.NET MVC 5</span></a></li>
</ul>
</fieldset></div>
