...
<html >
<head>
<title>Создание пагинации в ASP.NET MVC</title>
<meta charset="utf-8" />
...
<h2>Создание пагинации</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Одной из распространенных задач при работе с данными является проблема пагинации или разделения данных на несколько блоков - страниц, которые выводятсяв представление 
по отдельности и которые сопровождаются удобной навигацией по страницам. Для создания пагинации мы можем воспользоваться готовыми плагинами. Но в данном случае мы сами 
создадим механизм для постраничного вывода.</p>
<p>Вначале опрелелим модель, данные которой будем выводить:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Также определим модель, которая будет описывать механизм пагинации, а также дополнительную модель для вывода данных в представление:</p>
<pre class="brush:c#;">
...
<p>Так как вместе с данными моделей телефонов потребуется также использовать в представлении модель пагинации, то они инкапсулируются классом <code>IndexViewModel</code>.</p>
<p>Теперь определим контроллер и его метод для вывода данных:</p>
<pre class="brush:c#;">
...
<p>Для простоты примера я создаю обычный список в конструкторе контроллера, однако это мог бы быть и вывод из базы данных.</p>
<p>Метод <code>Index</code> в качестве параметра принимает номер страницы. По умолчанию номер страницы будет равняться единице.</p>
<p>В самом методе с помощью комбинации методов <code>Skip</code> и <code>Take</code> происходит отбор нужной части данных: метод <code>Skip()</code> 
пропускает определенное количество данных, которое передается в параметре, а метод <code>Take()</code> извлекает определенное количество.</p>
<p>Для создания механизма пагинации в представлении удобно использовать хелперы. Создадим собственный хелпер. Для этого добавим в проект папку 
<i>Helpers</i> и затем добавим в нее следующий класс:</p>
<pre class="brush:c#;">
using System.Text;
using System.Web;
...
<p>Данный хелпер просто создет блок ссылок, а также добавляет им классы для визуализации. Классы могут быть любыми, но в данном случае использовались стандартные 
классы bootstrap.</p>
<p>И теперь определим представление, которое использовать пагинацию:</p>
<pre class="brush:xml;">
...
<p>Поскольку хелпер пагинации находится в папке Helpers, то необходимо вначале представления подключить данное пространство имен <code>using ManualPaginApp.Helpers</code>. 
И хелпер пагинации <code>Html.PageLinks</code> сделают всю работу по формированию блока ссылок в сответствии с заложенной в нем логикой..</p>
<p>В итоге получим примерно следующую картину:</p>
<img src="./pics/5.25.png" />
...
<div class="nav"><p><a href="./5.12.php">Назад</a><a href="./">Содержание</a><a href="./5.14.php">Вперед</a></p></div></div>
<div class="socBlock">
