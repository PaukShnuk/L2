...
<html >
<head>
<title>Навигация в приложении в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h1>Навигация в приложении</h1>
<h2>Введение в навигацию</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Чтобы понять механизм навигации в приложении WP 8.1, рассмотрим некоторые общие моменты, на которых основана навигация.</p>
<p>При запуске приложения ему выделяется окно или экран смартфона. Окно представлено объектом <span class="bb">Window</span>. И если на планшетах и десктопах 
под управлением ОС Windows 8.1 мы можем использовать более одного окна, то  на смартфоне мы ограничены только одним окном.</p>
<p>Окно содержит один единственный фрейм, который занимает все пространство окна. Фрейм представлен объектом Frame. А фрейм уже содержит страницы (представленные 
классом Page), которые, как правило, занимают все пространство фрейма. Таким образом, в один момент времени мы можем видеть только одну страницу.</p>
<p>Создание фрейма происходит в методе <code>OnLaunched</code>, который определен в классе App:</p>
<pre class="brush:c#;">
...
<p>Выполнение метода фактически начинается с получения фрейма: <code>Frame rootFrame = Window.Current.Content as Frame</code>. И если фрейм окажется 
равен null, то он создается и устанавливается в качестве фрейма окна: <code>Window.Current.Content = rootFrame</code>.</p>
<p>Затем с помощью фрейма осуществляется переход к певой странице: <code>rootFrame.Navigate(typeof(MainPage), e.Arguments)</code></p>
<p>В конце активируется окно с фреймом, если оно неактивно: <code>Window.Current.Activate()</code></p>
<p>Для сохранения истории переходов между страницами объект Frame имеет свойство <span class="bb">BackStack</span>, которое представляет объект 
<code>IList&lt;PageStackEntry&gt;</code>.</p>
<p>Теперь рассмотрим создание системы навигации в приложении.</p>
...
<div class="nav"><p><a href="6.4.php">Назад</a><a href="./">Содержание</a><a href="7.2.php">Вперед</a></p></div></div>
<div class="socBlock">
