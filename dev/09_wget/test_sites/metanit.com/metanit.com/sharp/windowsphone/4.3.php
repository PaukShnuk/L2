...
<html >
<head>
<title>CheckBox и RadioButton в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h2>CheckBox и RadioButton</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>CheckBox</h3>
<p>Элемент CheckBox представляет собой флажок, который может принимать три состояния: Checked, Unchecked и Intermediate. 
Чтобы получить или установить определенное состояние, надо использовать свойство IsChecked:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>Значение свойства <code>IsChecked="{x:Null}"</code> устанавливает неопределенное состояние для элемента checkbox. Остальные два состояния задаются 
с помощью True и False. Хотя визуальной разницы между неотмеченным и неопределенным состоянием вы не увидите:</p>
<img src="pics/4.4.png" alt="CheckBox in Windows Phone 8.1" />
<p>Также обратите внимание на свойство <span class="bb">IsThreeState</span>. По умолчанию оно равно False, что значит, что элемент может переключаться в 
два состояния - отмеченное и неотмеченное. При установке <code>IsThreeState="True"</code> элемент может переключаться в третье неопределенное состояние.</p>
<p>В коде c# мы можем проинспектировать состояние флажка, например, получая его в обработчике кнопки:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>Также мы можем определить изменение состояние отдельного флажка с помощью обработки его события <span class="bb">Checked</span>:</p>
<pre class="brush:xml;">
&lt;CheckBox x:Name="checkBox1" Checked="checkBox_Checked" Content="Enabled" /&gt;
</pre>
<p>И в коде обработчика, например, изменим цвет кнопки:</p>
<pre class="brush:c#;">
...
<h3>RadioButton</h3>
<p>RadioButton представляет переключатель или радиокнопку. Элемент RadioButton, как и CheckBox, может быть в отмеченном, неотмеченном и неопределенном состоянии.
И в один момент времени мы можем выбрать из группы элементов RadioButton только один переключатель. Например,</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>Все переключатели образуют одну группу, где можно выбрать только один элемент. Но, используя свойство <span class="bb"></span> мы можем задать несколько групп:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>И в обработчике, например, изменяем текст текстового блока:</p>
<pre class="brush:c#;">
