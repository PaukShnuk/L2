...
<html >
<head>
<title>Приложение и его жизненный цикл в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h1>Приложение и его жизненный цикл</h1>
<h2>Роль класса App и жизненный цикл приложения</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Работа приложения Windows Phone начинается с класса <span class="bb">App</span>, который определен в файлах <i>App.xaml</i> и <i>App.xaml.cs</i>. 
...
Кроме того, в классе App можно перехватить исключения, которые могут возникнут в процессе работы.</p>
<p>ОС WP8.1 запускает приложение в виде класса App, а тот уже использует объект Frame для загрузки отдельных страниц Page и обеспечивает навигацию между 
ними.</p>
<p>В процессе работы приложение проходит различные этапы жизненного цикла. Обрабатывая события жизненного цикла, можно управлять работой приложения.</p>
<p>Жизненный цикл приложений Windows Phone 8.1 будет аналогичен жизненному циклу приложений для Windows 8.1, но в то же время он будет отличаться от жизненного цикла 
в Windows Phone 8.</p>
<p>При создании приложений надо учитывать, одновременно может быть запущено несколько приложений, но только одно из них будет активным на экране. Также одновременно может быть активной только одна страница, 
а единственный способ активизировать страницу - это переход на нее.</p>
<p>При запуске приложения объект класса App вызывает метод <span class="bb">OnLaunched</span>, а само приложение переходит в состояние <span class="bb">Running</span>. 
Если мы откроем в проекте файл <i>App.xaml.cs</i>, то мы увидим в классе App следующий метод:</p>
<pre class="brush:c#;">
...
<p>Когда пользователь уходит из приложения, используя кнопки Back (Назад), Start (Пуск) или кнопку поиска, то приложение приостанавливается, но окончательно не 
завершается. Минимальные данные о нем сохраняются в памяти в специальном хранилище под названием <b>backstack</b>. Приложение переходит в состояние 
<span class="bb">Suspended</span>. А объект App генерируется событие <span class="bb">Suspending</span>, обработчик которого также определен в файле <i>App.xaml.cs</i>:</p>
<pre class="brush:c#;">
...
<p>Метод <code>OnSuspending</code> позволяет обработать приостановку работы приложения. Мы можем определить в этом методе ряд действий, который, например, 
...
негативно можно повлиять на UX (user experiense / опыт взаимодействия пользователя с приложением).</p>
<p>Если приложение было приостановлено, то мы можем вновь к нему перейти и возобновить с ним работу, например, с помощью кнопки Back (назад) или через 
Диспетчер задач. При возобновлении работы приложение генерирует событие <span class="bb">Resuming</span>. По умолчанию оно никак не обрабатывается, но 
мы можем добавить соответствующий обработчик в класс App:</p>
<pre class="brush:c#;">
public sealed partial class App : Application
{
    public App()
...
<p>Здесь опять же мы можем добавить некоторую логику, например, загрузки ранее сохраненных данных, проверку каких-то условий и т.д. Но также, как и в предыдущем случае, 
код должен по возможности максимально быстро работать.</p>
<h3>Закрытие приложения</h3>
<p>Для полного закрытия приложения WP 8 надо нажать на кнопку закрытия (крестик):</p>
<img src="pics/6.7.png" />
<p>В этом случае приложение переходит в состояние <span class="bb">ClosedByUser</span> и удаляется из памяти.</p>
<p>Есть и второй вариант завершения работы приложения. Допустим, оно было приостановлено и перешло в состояние <code>Suspended</code>. В этот момент 
...
удалит все данные о ранее приостановленном приложении. И тогда приложение перейдет в состояние <span class="bb">Terminated</span></p>
<p>В подобных случаях возможно ситуация, что вместе с завершением приложения ОС также удалит из памяти какие-либо важные данные, с которыми пользователь ранее работал. 
...
данные, которые были до ухода из приложения. В таких случаях опять же можно сохранять данные, обрабатывая событие <code>Suspending</code>, а при запуске 
приложения загружать эти данные в приложение.</p>
