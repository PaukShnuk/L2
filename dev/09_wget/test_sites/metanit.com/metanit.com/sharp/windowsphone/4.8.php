...
<html >
<head>
<title>Slider и Progressbar в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h2>Slider и Progressbar</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>Slider</h3>
<p>Элемент Slider представляет собой ползунок:</p>
<pre class="brush:xml;">
&lt;Slider Value="15" Minimum="0" Maximum="100" VerticalAlignment="Top" /&gt;
</pre>
<img src="pics/4.20.png" alt="Slider in Windows Phone 8.1" />
<p>Свойства <code>Minimum</code> и <code>Maximum</code> задают соответственно минимальное и максимальное значения, а свойство Value - текущее значение.</p>
<p>С помощью свойства <span class="bb">Orientation</span> можно определить ориентацию ползунка - горизонтальную(Horizontal) или вертикальную (Vertical)</p></li>
<p>Обрабатывая событие <code>ValueChanged</code>, мы можем обработать изменение значение слайдера. Например, разметка со слайдером:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>И обработка события в коде c#:</p>
<pre class="brush:c#;">
...
<h3>ProgressBar</h3>
<p>ProgressBar представляет индикатор процесса:</p>
<pre class="brush:xml;">
&lt;ProgressBar Foreground="Red" x:Name="progressbar1" Minimum="0" Maximum="100" 
				Value="25" VerticalAlignment="Top" Height="30" /&gt;
</pre>
<img src="pics/4.21.png" alt="ProgressBar in Windows Phone 8.1" />
<h3>ProgressRing</h3>
<p>ProgressRing, также, как и ProgressBar, представляет индикатор процесса, только в виде анимационного кружка. Например, запустим индикацию с помощью кнопки:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>Обработчик нажатия кнопки в коде c#:</p>
<pre class="brush:c#;">
...
<img src="pics/4.22.png" alt="ProgressRing in Windows Phone 8.1" />
