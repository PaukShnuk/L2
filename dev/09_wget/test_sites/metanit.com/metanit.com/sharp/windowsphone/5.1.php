...
<html >
<head>
<title>Ресурсы и стили в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h1>Ресурсы и стили</h1>
<h2>Концепция ресурсов в Windows Phone 8.1</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Ресурсы позволяют определить объекты, которые могут многократно использоваться в приложении в различных его частях. Итак, определим пару ресурсов и используем их:</p>
<pre class="brush:xml;">
&lt;Page
    ...
    &lt;/Page.Resources&gt;
    &lt;Grid&gt;
        &lt;Button x:Name="button1" Background="{StaticResource grayBrush}" 
               Foreground="{StaticResource darkRedBrush}" Content="Кнопка" 
                VerticalAlignment="Top" /&gt;
    &lt;/Grid&gt;
&lt;/Page&gt;
</pre>
<p>Получится такая кнопка:</p>
<img src="pics/5.1.png" alt="Resources in Windows Phone 8.1" />
<p>Ресурсы определяются в свойстве <span class="bb">Resources</span>, которое есть у компонентов визуального интерфейса. В данном случае ресурсы определены 
как ресурсы элемента Page внутри тегов <code>&lt;Page.Resources&gt; &lt;/Page.Resources&gt;</code>.</p>
<p>Каждый ресурс должен иметь уникальный идентификатор, которым выступает свойство <span class="bb">x:Key</span>.</p>
<p>Чтобы задать ресурс в качестве значения свойство, используется выражение <code>{StaticResource ключ_ресурса}</code></p>
<p>В качестве ресурсов можно определить и более сложное содержимое, например:</p>
<pre class="brush:xml;">
&lt;Page
    x:Class="HelloApp.MainPage"
    xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    &lt;Page.Resources&gt;
        &lt;SolidColorBrush x:Key="darkRedBrush" Color="DarkRed" /&gt;
        &lt;SolidColorBrush x:Key="grayBrush" Color="DarkGray" /&gt;
        &lt;StackPanel x:Key="R2D2Button" Orientation="Horizontal"&gt;
            &lt;Ellipse Fill="Red" Width="30" Height="30" Stroke="Black" /&gt;
            &lt;TextBlock Text="Record" FontSize="22" Width="100" Height="30" Margin="8" /&gt;
        &lt;/StackPanel&gt;
    &lt;/Page.Resources&gt;
    &lt;Grid&gt;
        &lt;Button Background="{StaticResource grayBrush}" VerticalAlignment="Top"
               Foreground="{StaticResource darkRedBrush}" Content="{StaticResource R2D2Button}" /&gt;
    &lt;/Grid&gt;
&lt;/Page&gt;
</pre>
<img src="pics/5.2.png" />
<p>Если у нас в проекте несколько страниц Page, и нам надо использовать одинаковые ресурсы на каждой из них, то мы можем определить данные ресурсы в качестве ресурсов всего 
приложении. Для этого откроем файл <span class="bb">App.xaml</span> и добавим в него определение ресурсов:</p>
<pre class="brush:xml;">
&lt;Application
...
<p>В этом случае данные ресурсы будут доступны из любого места приложения.</p>
<h3>Темы</h3>
<p>Кроме подключения ресурсов через StaticResource в разметке по умолчанию можно увидеть в определении Page такое выражение 
<code>Background="{ThemeResource  ApplicationPageBackgroundThemeBrush}"</code>. <span class="bb">ThemeResource</span> позволяет подключить ранее созданную тему для 
приложения. Подобных тем существует довольно много для разных типов элементов:</p>
