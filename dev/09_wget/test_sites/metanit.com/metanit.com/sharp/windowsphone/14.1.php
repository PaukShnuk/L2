...
<html >
<head>
<title>Работа с SQLite. Часть 1 в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h1>SQLite в Windows Phone 8.1</h1>
<h2>Работа с SQLite. Часть 1</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p><b>SQLite</b> представляет одну из наиболее распространенных реляционных СУБД, которая обладает такими преимуществами, как компактность, 
...
как мы можем использовать SQLite при программирования для Windows Phone 8.1.</p>
<h3>Добавление расширений</h3>
<p>Для работы с SQLite создадим проект по типу Blank App. И вначале нам надо подключить в Visual Studio расширения. Для этого в главном меню Visual Studio выберем пункты 
<span class="bb">TOOLS -&gt; Extensions and Updates</span></p>
<p>Здесь нам надо установить два расширения: <b>SQLite for Windows Phone 8.1</b> и <b>SQLite for Windows Runtime (Windows 8.1)</b></p>
<img src="pics/14.1.png" alt="Установка расширений для Visual Studio" />
<p>После их установки добавим в проект соответствующую библиотеку. Для этого нажмем правой кнопкой мыши на узел <span class="bb">References -&gt;Add Reference</span> 
и в списке выберем библиотеку <b>SQLite for Windows Phone 8.1</b>:</p>
<img src="pics/14.2.png" alt="SQLite для Windows Phone" />
<p>После этого в проект будет добавлена данная библиотека. Для взаимодействия с кодом c# также надо добавить еще одну библиотеку, которая 
позволит нам создавать запросы к базе данных. Для этого нажмем правой кнопкой мыши на узел <span class="bb">References -&gt;Manage NuGet Packages...</span> 
и в поле поиска введем <i>sqlite-pcl</i>:</p>
<img src="pics/14.3.png" />
<p>В списке найдем библиотеку <b>Portable Class Library for SQLite</b> и установим ее.</p>
<p>После добавления в узле References окажутся две одинаковые библиотеки, но с разными версиями:</p>
<img src="pics/14.4.png" />
<p>Библиотеку с более ранней версией мы можем удалить.</p>
<p>Рядом со второй библиотекой будет отображаться желтый треугольник, который показывает, что библиотека не может применяться к любому процессору. 
А если мы посмотрим на настройки отладки проекта, то увидим, что для проекта установлено <span class="bb">Any CPU</span>. Так как цель нашего приложения 
- смартфон, то изменим тип процессора на <span class="bb">ARM</span>:</p>
<img src="pics/14.5.png" />
<p>Создаваемое приложение будет очень простым. Пусть оно будет выводить список книг из библиотеки, а также предоставлять функционал для управления обектами - 
их созданием, удалением, редактированием и т.д.</p>
<p>И первым делом добавим в проект класс модели, которая будет использоваться - класс Book:</p>
<pre class="brush:c#;">
public class Book
{
...
<p>Чтобы легче управлять объектами, было бы лучше создать какую-нибудь абстракцию наподобие репозитория. Поэтому добавим в проект 
еще один класс BookRepository:</p>
<pre class="brush:c#;">
using SQLitePCL;
using System;
...
<p>Этот класс производит основную работу с базой данных, и все взаимодействие будет идти через его методы.</p>
<p>Для управления бд в классе определяется переменная типа <span class="bb">SQLiteConnection</span></p>
<p>Метод <code>CreateTable()</code> создает таблицу в бд, если таблица еще не создана. Для выполенния sql-запросов используются методы 
<code>Prepare</code> и <code>Step()</code>:</p>
<pre class="brush:c#;">
using (var statement = con.Prepare(sql))
{
    statement.Step();
...
<p>Методы <code>Insert()</code>, <code>Update()</code> и <code>Delete()</code> соответственно добавляют, обновляют и удаляют объекты. Здесь 
...
<code>Bind</code>. Он связывает значения с параметрами, например, <code>statement.Bind(1, book.Name)</code></p>
<p>И методы <code>GetBooks()</code> и <code>GetBook()</code> соответственно получают набор объектов и один единственный объект по id.</p>
<p>Теперь добавим в проект страницы и определим логику для манипуляции объектами из пользовательского интерфейса.</p>
