...
<html >
<head>
<title>Интерфейс INotifyPropertyChanged в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h2>Интерфейс INotifyPropertyChanged</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В прошлой теме использовался объект Tablet для привязки к текстовым блокам. Однако если мы изменим его, содержимое текстовых блоков не изменится. Чтобы объект 
мог полноценно реализовать механизм привязки, нам надо реализовать в его классе интерфейс <span class="bb">INotifyPropertyChanged</span>.</p>
<p>Итак, изменим класс Tablet следующим образом:</p>
<pre class="brush:c#;">
...
<p>Когда объект класса изменяет значение свойства, то он через событие PropertyChanged извещает систему об изменении свойства. А система обновляет все привязанные 
объекты.</p>
<p>Так, в xaml мы можем добавить кнопку, которая будет изменять значение объекта:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Обработчик кнопки в файле MainPage.xaml.cs просто изменяет ресурс:</p>
<pre class="brush:c#;">
...
<p>Обратите внимание, что мы не изменяем напрямую текст в текстовом блоке. Однако при изменении свойства Name и после генерации события PropertyChanged система 
сама изменит содержимое текстового блока.</p>
