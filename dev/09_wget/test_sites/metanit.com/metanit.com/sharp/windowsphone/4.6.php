...
<html >
<head>
<title>ListBox в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<p>ListBox представляет собой обычный список. Содержит коллекцию элементов ListBoxItem. 
Также ListBox может содержать любые другие элементы, например, текстовые блоки, кнопки, тогда в этом случае они неявно обертываются в элемент ListBoxItem:</p> 
<pre class="brush: xml;">
...
<img src="pics/4.13.png" alt="Элемент ListBox в Windows Phone 8.1" />
<p>Все эти элементы будут находиться в коллекции <code>Items</code> и, таким образом, по счетчику можно к ним обращаться, например, 
<code>listbox1.Items[0]</code> - первый элемент ListBox, который в данном случае представляет TextBlock. Также мы можем установить элемент: 
<code>listbox1.Items[2]="Германия";</code><p>
<p>Так как ListBoxItem является элементом управления содержимым, то мы можем задавать в качестве элемента списка и более сложные композиции элементов, например:</p>
<pre class="brush: xml;">
...
<p>Чтобы обработать выбор элемента в ListBoxе, мы можем добавить обработчик к событию <code>SelectionChanged</code>:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>В коде C# мы можем использовать свойство <span class="bb">SelectedItem</span>, чтобы получить выбранный элемент:</p>
<pre class="brush:c#;">
