...
<html >
<head>
<title>Файлы отделенного кода</title>
<meta charset="utf-8" />
...
<h2>Файлы отделенного кода</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>По умолчанию вместе с файлом MainPage.xaml Visual Studio также создает файл отделенного кода (code-behind) <span class="bb">MainPage.xaml.cs</span>, 
который содержит логику на C#, связанную с файлом MainPage.xaml. Файлы XAML позволяют нам определить визуальный интерфейс, но для создания логики, например, для определения обработчиков событий элементов управления, все равно придется воспользоваться кодом C#.</p>
<p>Если мы посмотрим на файл MainPage.xaml.cs, то увидим, что он определяет частичный класс <code>MainPage</code>:</p>
<pre class="brush:c#;">
public sealed partial class MainPage : Page
</pre>
<p>С помощью атрибута <code>x:Class</code> объект Page в XAML ассоциируется с частичным классом, определенным в MainPage.xaml.cs. Затем при комплияции 
компилятор соединяет оба файла в один класс.</p>
<p>Класс MainPage по умолчанию практически ничего не делает, кроме начальной инициализации компонентов:</p>
<pre class="brush:c#;">
using System;
...
<p>Через вызываемый в конструкторе метод <code>InitializeComponent()</code> класс MainPage получает скомпилированный ранее код XAML, разбирает его и по нему строит графический интерфейс страницы.</p>
<h3>Взаимодействие кода C# и XAML</h3>
<p>В приложении часто требуется обратиться к какому-нибудь элементу управления. Для этого надо установить у элемента в XAML атрибут <code>x:Name</code>.</p>
...
<p>Например, определим в XAML кнопку с двумя текстовыми полями:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>И теперь мы можем добавить в файл отделенного кода метод <code>button1_Click</code> - обработчик события Click:</p>
<pre class="brush:c#;">
using System;
using System.Collections.Generic;
using System.IO;
...
<p>Хотя XAML позволяет довольно просто создавать элементы, но мы можем также определять элементы динамически в коде c#. Например, пусть элемент Grid в XAML имеет следующее 
имя:</p>
<pre class="brush:xml;">
&lt;Grid x:Name="grid1"&gt;
&lt;/Grid&gt;
</pre>
<p>Теперь в файле отделенного кода в конструкторе MainPage можно добавить в этот грид разные элементы:</p>
<pre class="brush:c#;">
public MainPage()
{
    this.InitializeComponent();
...
<p>Это аналогично тому, как если бы создали ту же кнопку через xaml:</p>
<pre class="brush:xml;">
&lt;Grid x:Name="grid1"&gt;
