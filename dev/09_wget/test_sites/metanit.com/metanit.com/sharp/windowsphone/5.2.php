...
<html >
<head>
<title>Стили в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h2>Стили</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Стили применяются для создания единообразного отображения элементов, также они упрощают подключения группы ресурсов к отдельным элементам. Например:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Фактически стиль также является ресурсом и определяется в коллекции ресурсов элемента. Стиль создается с помощью объекта Style. Затем указывается его 
ключ и тип, элементов, к которым он относится (свойство <span class="bb">TargetType</span>). Внутри стиля создаются объекты Setter. Они определяют свойства, подлежащие стилизации, и передаваемые им значения.
Свойство Property имеет следующий синтаксис: <code>Property="Свойство_элемента"</code>.</p>
<p>В данном случае мы определили два стиля. Теперь с помощью кнопок создадим логику переключения стилей. Для этого добавим в код c# следующий обработчик:</p>
<pre class="brush:c#;">
...
<p>Теперь по умолчанию к кнопкам будет применен первый стиль:</p>
<img src="pics/5.3.png" />
<p>После нажатия на вторую кнопку мы сможем переключиться ко второму стилю:</p>
