...
<html >
<head>
<title>Работа с жизненным циклом. Практика в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h2>Работа с жизненным циклом. Практика.</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Теперь рассмотрим общие способы работы с событиями жизненного цикла. Для этого создадим новый проект по типу Blank App. Назовем его, к примеру, LifecycleApp.</p>
<p>Проект по умолчанию содержит страницу MainPage, но нам он не понадобится. Для управления навигацией и ее историей нам нужны страницы по типу <span class="bb">Basic Page</span>, 
а не <span class="bb">Blank Page</span>. Итак, удалим файл <i>MainPage.xaml</i> и после этого добавим в проект новый элемент по типу Basic Page:</p>
<img src="pics/6.8.png" alt="Basic Page in Windows Phone 8.1" />
<p>При добавлении элемента появится сообщение о том, что вместе с этим элементом надо добавить и некоторые другие связанные компоненты:</p>
<img src="pics/6.9.png" />
<p>После этого, во-первых, будет добавлена новая страница Page1.xaml, которая будет уже иметь некоторое содержимое. Во-вторых, в проект также будет добавлена 
папка <span class="bb">Commom</span> с рядом файлов. В итоге структура проекта должна выглядеть так:</p>
<img src="pics/6.10.png" alt="Lifecycle in Windows Phone 8.1" />
<p>Сделаем довольно примитивное приложение, суть которого будет заключаться в сохранении и восстановлении данных текстового поля. Для этого изменим разметку 
xaml страницы Page1, добавив в нее элемент TextBox и убрав лишние элементы:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Первым делом, нам надо сохранить данные приложения, когда оно переходит в приостановленное состояние. Для управления данными приложения мы можем 
использовать объект <b>Windows.Storage.ApplicationData</b>. Данный объект поддерживает два варианта сохранения данных: через свойство 
<span class="bb">RoamingSettings</span> и через свойство <span class="bb">LocalSettings</span>.</p>
<p>RoamingSettings позволяет сохранять данные в каталоге, который синхронизирован с различными устройствами, например, в облачном хранилище. 
А свойство LocalSettings позволят сохранить данные на текущем устройстве. Для данной задачи мы используем LocalSettings.</p>
<p>Итак, добавим в файл отделенного кода <i>Page1.xaml.cs</i> следующий код обработчика изменения текста:</p>
<pre class="brush:c#;">
...
<p>Таким образом, введенное значение будет сохраняться при каждом изменении текста в текстовом поле.</p>
<p>Теперь загрузим ранее сохраненное значение в текстовое поле. Для этого найдем в файле <i>Page1.xaml.cs</i> метод <code>NavigationHelper_LoadState</code>:</p>
<pre class="brush:c#;">
private void NavigationHelper_LoadState(object sender, LoadStateEventArgs e)
{
}
</pre>
<p>И изменим его следующим образом:</p>
<pre class="brush:c#;">
private void NavigationHelper_LoadState(object sender, LoadStateEventArgs e)
{
    Windows.Storage.ApplicationDataContainer localSettings =
...
<p>Метод <code>NavigationHelper_LoadState</code> является обработчиком события <code>LoadState</code> класса <span class="bb">NavigationHelper</span>, 
который находится в проекте в каталоге Common. Сам обработчик прикрепляется в конструкторе странице.</p>
<p>Теперь мы можем ввести некоторое значение в текстовое поле, приостановить приложение или полность закрыть и открыть его снова. И мы вновь увидим ранее введенное 
значение. Мы можем эмулировать завершение приложения в Visual Studio после запуска с помощью опции <span class="bb">Suspend and shutdown</span>:</p>
<img src="pics/6.11.png" />
