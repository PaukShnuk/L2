...
<html >
<head>
<title>Геолокация в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h1>Геолокация</h1><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Одной из отличительных черт смартфонов является возможность использования систем навигации типа GSM или GLONASS для определения координат. 
Рассмотрим, как мы в приложении на Windows Phone 8.1 можем осуществлять геолокацию.</p>
<p>Среди стандартных элементов управления в Visual Studio нам доступен такой элемент как Map, представляющий географическую карту. Чтобы его использовать, 
создадим по шаблону Blank App новый проект, который назовем MapApp. Изменим, стандартную разметку xaml страницы MainPage.xaml на следующую:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>В отличие от стандартной разметки здесь добавлены две строчки. Во-первых, импортировано пространство имен <code>xmlns:Maps="using:Windows.UI.Xaml.Controls.Maps"</code>. 
Во-вторых, добавлен собственно элемент map: <code>&lt;Maps:MapControl x:Name="map"&gt;&lt;/Maps:MapControl&gt;</code></p>
<p>Если мы запустим приложение, то увидим фактически готовую карту:</p>
<img src="pics/11.1.png" alt="Maps in Windows Phone 8.1" />
<p>Но просто созерцать карту не очень интересно, поэтому попробуем с ней что-нибудь сделать. Для начала мы можем установить свое местоположение. Для 
этого первым делом перейдем к файлу <span class="bb">Package.appxmanifest</span>. В нем перейдем на вкладку <span class="bb">Capabilities</span>. 
На ней установим отметку в поле <span class="bb">Location</span>:</p>
<img src="pics/11.2.png" alt="Определение местоположения Windows Phone 8.1" />
<p>Далее перейдем к файлу кода MainPage.xaml.cs и изменим в нем метод <code>OnNavigatedTo()</code>, который срабатывает при переходе к данной странице:</p>
<pre class="brush:c#;">
...
<p>Свойство <code>MapServiceToken</code> устанавливает токен аутентификации, который при релизе приложения задается через Центр Разработчика Windows Phone, 
но для целей тестирования здесь мы можем указать любое значение.</p>
<p>Вначале надо получить объект геолокатора, который позволяет установить координаты телефона. В качестве точности геолокации я использовал 150 метров, 
хотя можно использовать и другие значения. После получения координат, надо определить эти координаты на карте. Для этого в метод <code>map.TrySetViewAsync()</code> 
передаются координаты и значение приближения.</p>
<p>И затем карта отобразит наше текущее местонахождение, причем точность определения довольно высока:</p>
<img src="pics/11.4.png" />
<p>Теперь изменим приложение. Во-первых, изменим файл MainPage.xaml следующим образом:</p>
<pre class="brush:xml;">
&lt;Page
    x:Class="MapApp.MainPage"
    xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
...
<p>Кроме элемента карты здесь определен слайдер для динамической настройки масштаба карты, также здесь определены два текстовых поля для отображения текущих 
географических координат.</p>
<p>Также добавим в файл MainPage.xaml.cs обработчики событий:</p>
<pre class="brush:c#;">
...
<p>Также в начале файле в секции using подключим пространство имен <span class="bb">Windows.UI.Xaml.Controls.Maps</span></p>
<p>И теперь мы можем динамически контролировать текущие координаты на карте и изменение масштаба через слайдер:</p>
<img src="pics/11.3.png" />
<h3>Свойства элемента Map</h3>
<p>Рассмотрим некоторые свойства элемента Map:</p>
<ul>
<li><p><span class="bb">Center</span>: центральная точка на карте. Мы можем как получить ее значения, так и установить:</p>
<pre class="brush:c#;">
...
<li><p><span class="bb">ZoomLevel</span>: масштаб, например, <code>map.ZoomLevel = 18</code></p></li>
<li><p><span class="bb">Style</span>: стиль карты, использует константы из перечисления <span class="bb">MapStyle</span>, например, <code>map.Style = MapStyle.Terrain</code></p></li>
<li><p><span class="bb">ColorScheme</span>: цветовая схема карты, использует константы перечисления <span class="bb">MapColorScheme</span>: <code>map.ColorScheme = MapColorScheme.Dark</code></p></li>
<li><p><span class="bb">TrafficFlowVisible</span>: указывает, надо ли отображать интенсивность дорожного трафика, например, <code>map.TrafficFlowVisible = true</code> - 
в этом случае дороги на карте будут отображены в том или ином цвете, сигнализирующем об интенсивности движения на дорогах</p></li>
</ul>
...
<div class="nav"><p><a href="11.1.php">Назад</a><a href="./">Содержание</a><a href="13.1.php">Вперед</a></p></div></div>
<div class="socBlock">
