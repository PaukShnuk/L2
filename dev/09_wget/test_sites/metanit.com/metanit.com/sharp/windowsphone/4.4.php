...
<html >
<head>
<title>Всплывающие окна в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h2>Всплывающие окна</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>Popup</h3>
<p>Для создания всплывающих окон мы можем использовать элемент Popup. Он имеет свойство StaysOpen, которое позволяет управлять его отображением. 
Так, по умолчанию оно равно True, а это значит, что при отображении окна, оно больше не исчезнет, пока мы не установим явно значение этого свойства в 
False. Например, добавим элемент Popup:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>Popup может хранить сложное содержимое, другие элементы управления, но в данном случае ограничимся текстовым блоком.</p>
<p>И с помощью обработчика нажатия кнопки управляем отображением окна:</p>
<pre class="brush:c#;">
...
<img src="pics/4.9.png" alt="Popup in Windows Phone 8.1" />
<h3>MessageDialog</h3>
<p>Еще один способ создания окон заключается в применении класса <span class="bb">MessageDialog</span>. В данном случае нам не надо ничего определять в xaml, 
все делается в коде c#. Например, определим в коде следующий обработчик нажатия кнопки:</p>
<pre class="brush:c#;">
...
<p>Класс MessageDialog находится в пространстве имен <span class="bb">Windows.UI.Popups</span>, поэтому нам также надо подключить его в начале файла директивой using: 
<code>using Windows.UI.Popups;</code></p>
<p>Для отображения окна используется метод <span class="bb">ShowAsync()</span>, который является асинхронным. Поэтому нам в данном случае надо использовать операторы 
<code>async</code> и <code>await</code>.</p>
<p></p>
