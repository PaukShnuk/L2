...
<html >
<head>
<title>Компоновка и позиционирование элементов</title>
<meta charset="utf-8" />
...
<h1>Компоновка и позиционирование элементов</h1>
<h2>Контейнер Grid</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>При создании нового проекта по шаблону <span class="bb">Blank App</span> разметка страницы в xaml уже содержит элемент <span class="bb">Grid</span>, 
в который затем помещаются все остальные компоненты. Однако элемент Grid служит не просто контейнером, но и управляет позиционированием вложенных элементов.</p>
<p>Для более точной настройки места положения элементов элемент Grid использует свойства <span class="bb">RowDefinitions</span> и 
<span class="bb">ColumnDefinitions</span>. RowDefinitions задает определения строк, а ColumnDefinitions - определения столбцов. Например:</p>
<pre class="brush:xml;">
&lt;Grid&gt;
    &lt;Grid.RowDefinitions&gt;
        &lt;RowDefinition /&gt;
...
<p>Каждая строка задается с помощью вложенного элемента RowDefinition, то есть в данном случае в гриде определено 
3 строки.</p>
<p>Каждый столбец задается с помощью вложенного элемента ColumnDefinition. Таким образом, здесь мы определили 3 столбца. То есть в итоге у нас получится таблица 3х3.</p>
<p>Чтобы задать позицию элемента управления с привязкой к определенной ячейке Grid, в разметке элемента нужно прописать значения свойств 
<span class="bb">Grid.Column</span> и <span class="bb">Grid.Row</span>, то есть указав, в каком столбце и строке будет находиться элемент. Кроме того, 
если мы хотим растянуть элемент управления на несколько строк или столбцов, то можно указать свойства <span class="bb">Grid.ColumnSpan</span> и 
<span class="bb">Grid.RowSpan</span>, как в следующем примере:</p>
<pre class="brush:xml;">
&lt;Page
...
<img src="pics/3.1.png" alt="Grid in Windows Phone 8" />
<p>Отсчет строк и столбцов начинается с нуля. Если мы не определим свойство Grid.Column, то по умолчанию элемент будет помещен в первый столбец. И также если 
мы не определим свойство Grid.Row, элемент будет размещаться в первой строке. Например:</p>
<pre class="brush:xml;">
&lt;Grid&gt;
    &lt;Grid.RowDefinitions&gt;
        &lt;RowDefinition /&gt;
...
<p>В предыдущем примере строки имеют одинаковую высоту, а столбцы - одинаковую ширину, которая определяется путем деления всего доступного пространства на количество строк/столбцов.</p>
<p>Но есть различные варианты настройки размеров.</p>
<h4>Автоматически размеры</h4>
<p>Автоматические размеры задаются с помощью значения <code>Auto</code>, а столбец или строка занимает то место, которое им нужно:</p>
<pre class="brush:xml;">
&lt;ColumnDefinition Width="Auto" /&gt;
&lt;RowDefinition Height="Auto" /&gt;
</pre>
<h4>Абсолютные размеры</h4>
<p>В данном случае высота и ширина указываются в пикселях:</p>
<pre class="brush:xml;">
&lt;ColumnDefinition Width="120" /&gt;
&lt;RowDefinition Height="80" /&gt;
</pre>
<h4>Пропорциональные размеры</h4>
<p>Для определения пропорциональных размеров используется символ *. Например, ниже задаются два столбца, второй из которых имеет ширину в четверть от ширины первого:</p>
<pre class="brush:xml;">
&lt;ColumnDefinition Width="*" /&gt;
&lt;ColumnDefinition Width="0.25*" /&gt;
</pre>
<p>Или, например, установим пропорциональную высоту строк:</p>
<pre class="brush:xml;">
&lt;Grid.RowDefinitions&gt;
...
<p>В данном случае вся высота грида будет разделена на 6 частей, из которых под первую строку отводится одна часть, под вторую - две, а под третью - три.</p>
