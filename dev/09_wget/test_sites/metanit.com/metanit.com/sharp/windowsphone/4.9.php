...
<html >
<head>
<title>Элемент Flyout в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h2>Flyout</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Элемент Flyout позволяет реализовать функциональность всплывающих окон, которое можно убрать, коснувшись любого места за пределами этого окна.</p>
<p>Как и элемент Popup, Flyout может принимать сложное содержимое. Однако он существует не сам по себе, а прикрепляется к какому-нибудь элементу 
FrameworkElement. Например:</p>
<pre class="brush:xml;">
...
<p>В данном случае для прикрепления всплывающего сообщения к кнопке используется свойство <code>Button.Flyout</code>. И по нажатию на кнопку произойдет отображение 
сообщения:</p>
<img src="pics/4.26.png" alt="Flyout in Windows Phone 8.1" />
<p>Во Flyout можно определить различные элементы и прикрепить к ним обработчики событий, как в примере выше с кнопкой.</p>
<p>По умолчанию элемент отображается в верхней части экрана. Однако мы можем также заполнить всплывающим окном все пространство экрана, установив 
для свойства <span class="bb">Placement</span> значение <code>Full</code>:</p>
<pre class="brush:xml;">
&lt;Flyout Placement="Full"&gt;
	&lt;!----&gt;
&lt;/Flyout&gt;
</pre>
<p>Но не у всех элементов есть прикрепленное свойство <code>Flyout</code>, как у кнопки. При чтобы прикрепить к подобным элементам Flyout, 
надо использовать свойство <span class="bb">FlyoutBase.AttachedFlyout</span>. Правда, в этом случае также надо будет самим реализовать отображения 
всплывающего окна. Например:</p>
<pre class="brush:xml;">
...
<p>Но чтобы увидеть окно, надо еще определить следующий обработчик для события <code>Tapped</code>:</p>
<pre class="brush:c#;">
