...
<html >
<head>
<title>Сохранение данных в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h1>Сохранение данных</h1><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Ранее мы уже сталкивались с некоторыми моментами сохранения данных в телефоне, теперь рассмотрим этот вопрос более подробно.</p>
<p>В программе на Windows Phone 8.1 мы можем сохранить данные в сериализованном виде, например, в формате xml или json.</p>
<p>На устройстве Windows Phone 8.1 каждому приложению отводится свое место в папке <span class="bb">Local</span>. Чтобы получить этот хранилище в приложении, 
мы можем использовать объект <span class="bb">Windows.Storage.ApplicationData.Current.LocalFolder</span>. Этот объект обладает рядом методов для 
чтения/записи файлов, позволяя, тем самым, управлять сохраненными данными. Но среди его методов можно выделить следующие два:</p>
<ul>
<li><p><span class="bb">OpenStreamForWriteAsync()</span> - сохраняет данные в файл</p></li>
<li><p><span class="bb">OpenStreamForReadAsync()</span> - считывает данные из файла</p></li>
</ul>
<p>Первый метод принимает два параметра: имя файла и перечисление <span class="bb">Windows.Storage.CreationCollisionOption</span>. Данное 
перечисление может иметь следующие значения:</p>
<ul>
<li><p><span class="bb">OpenIfExists</span>: предполагает открытие файла</p></li>
<li><p><span class="bb">FailIfExists</span>: генерирует исключение, если файл с таким именем уже существует</p></li>
<li><p><span class="bb">GenerateUniqueName</span>: генерирует новый файл с уникальным именем, если заданный файл уже существует</p></li>
<li><p><span class="bb">ReplaceExisting</span>: перезаписывает имеющийся файл</p></li>
</ul>
<p>А второй принимает имя файла для чтения.</p>
<p>Для примера создадим новое приложение по типу Blank App. И сначала добавим в проект модель, которую мы будем сохранять и затем восстанавливать. 
Пусть это будет простенький класс User:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Определим на главной странице MainPage.xaml следующую разметку:</p>
<pre class="brush:xml;">
&lt;Page
    x:Class="SerializeDataApp.MainPage"
    xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
    xmlns:local="using:SerializeDataApp"
    ...
    &lt;Grid&gt;
        &lt;StackPanel&gt;
            &lt;TextBlock x:Name="infoBlock" Text="Введите данные" /&gt;
            &lt;TextBox x:Name="nameBox" PlaceholderText="Имя" /&gt;
            &lt;TextBox x:Name="surnameBox" PlaceholderText="Фамилия" /&gt;
            &lt;TextBox x:Name="companyBox" PlaceholderText="Компания" /&gt;
            &lt;StackPanel Orientation="Horizontal"&gt;
                &lt;Button x:Name="saveButton" Content="Сохранить" Click="saveButton_Click" /&gt;
                &lt;Button x:Name="getButton" Content="Извлечь" Click="getButton_Click" /&gt;
            &lt;/StackPanel&gt;
        &lt;/StackPanel&gt;
    &lt;/Grid&gt;
&lt;/Page&gt;
</pre>
<p>У нас есть три текстовых поля для ввода данных и две кнопки, которые будет сохранять и восстанавливать данные.</p>
<p>Опеределим в файле mainPage.xaml.cs обработчики для этих кнопок:</p>
<pre class="brush:c#;">
...
<p>Кроме объекта ApplicationData.Current.LocalFolder здесь используется другой ключевой класс - класса-сериализатор <span class="bb">DataContractSerializer</span>. 
Он позволяет сериализовать данные в файл xml, а также потом восстановить данные из этого файла.</p>
<p>При создании объекта класса DataContractSerializer мы задаем ему тип объекта, с которым ему необходимо работать: <code>new DataContractSerializer(typeof(User))</code>. 
Если бы мы сохраняли список объектов User, то соответственно могли бы указать <code>new DataContractSerializer(typeof(List&lt;User&gt;))</code></p>
<p>С помощью метода <code>WriteObject()</code> объект DataContractSerializer сохраняет данные, а с помощью метода <code>ReadObject()</code> извлекает. 
Этими методами функциональность класса не ограничивается, но они являются основными для механизма сериализации и десериализации.</p>
<p>И теперь запустив приложение и введя данные, мы можем по нажатию кнопки сохранить их в памяти смартфона. Затем через некоторое время перезапустив приложение 
мы можем восстановить ранее сохраненные данные.</p>
<p>И если мы захотим вместо xml-формата использовать json, тогда мы можем использовать класс <span class="bb">DataContractJsonSerializer</span>, 
действие которого аналогично классу DataContractSerializer. Например, сохранение:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="10.2.php">Назад</a><a href="./">Содержание</a><a href="12.1.php">Вперед</a></p></div></div>
<div class="socBlock">
