...
<html >
<head>
<title>Кнопки и текстовые поля в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h2>Кнопки и текстовые поля</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>TextBlock</h3>
<p>Элемент TextBlock служит для вывода простой текстовой информации, например, надписи, метки:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>Для изменения параметров отображаемого текста данный элемент имеет такие свойства, как <span class="bb">LineHeight</span>, <b>TextWrapping</b> и <span class="bb">TextAllignment</span>.</p>
<p>Свойство LineHeight позволяет указывать высоту строк.</p>
<p>Свойство TextWrapping позволяет переносить текст при установке этого свойства TextWrapping="Wrap". По умолчанию это свойство имеет 
значение <code>NoWrap</code>, поэтому текст не переносится.</p>
<p>Свойство TextAllignment выравнивает текст по центру (значение Center), правому (Right) или левому краю (Left): <code>&lt;TextBlock TextAlignment="Right"&gt;</code></p>
<p>Для оформления текста используется свойство <span class="bb">TextDecorations</span>, например, если <code>TextDecorations="Underline"</code>, то текст будет подчеркнут.</p>
...
<h3>Button</h3>
<p>Элемент Button представляет стандартную кнопку:</p>
<pre class="brush:xml;">
&lt;Button x:Name="button1" Content="Кнопка" Width="60" Height="30" Background="Red" /&gt;
</pre>
<p>Как правило, кнопки служат для инициирования пользователем какого-либо действия по нажатию. Для обработки нажатия надо привязать к событию <code>Click</code> 
кнопки обработчик события. Например, код xaml:</p>
<pre class="brush:xml;">
&lt;Button x:Name="button1" Content="Кнопка" Width="60" Height="30" Background="Red" Click="button1_Click" /&gt;
</pre>
<p>Тогда в файле отделенного кода для данной страницы надо прописать обработчик:</p>
<pre class="brush:c#;">
...
<p>Так как Button является элементом управления содержимым и имеет свойство Content, мы можем задать более сложные композиции кнопок. Например:</p>
<pre class="brush:xml;">
...
<p>Однако такой подход имеет ограничения, так как поле для ввода текста будет недоступно, а при попутке ввести текст, будет инициироваться нажатие кнопки.</p>
<h3>TextBox</h3>
<p>Элемент TextBox служит для ввода текстовой информации.</p>
<p>Он также, как и TextBlock, имеет свойства <span class="bb">TextWrapping</span>, <span class="bb">TextAllignment</span> и <span class="bb">TextDecorations</span>.</p>
<p>С помощью свойства <span class="bb">MaxLength</span> можно задать предельное количество вводимых символов.</p>
<p>Свойство <span class="bb">PlaceholderText</span> задает текст по умолчанию, который отображается в поле до ввода в него символов.</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
Свойство <span class="bb">PasswordChar</span> устанавливает символ маски, отображаемый при вводе пароля. 
Свойство <span class="bb">Password</span> устанавливает парольную строку, отображаемую по умолчанию при загрузке окна приложения.</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<img src="pics/4.3.png" alt="PasswordBox на Windows Phone 8.1" />
