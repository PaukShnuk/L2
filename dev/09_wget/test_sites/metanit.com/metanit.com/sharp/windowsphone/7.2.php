...
<html >
<head>
<title>Создание навигации в Windows Phone 8.1</title>
<meta charset="utf-8" />
...
<h2>Создание навигации</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Создадим новый проект по типу <span class="bb">Blank App</span> и назовем его, например, NavigationApp.</p>
<p>Новый проект уже имеет по умолчанию одну страницу MainPage. Добавим еще пару страниц по типу <span class="bb">Blank Page</span>. Назовем их 
Page2 и Page3:</p>
<img src="pics/7.1.png" alt="Navigation in Windows Phone 8.1" />
<p>Добавим в первую страницу MainPage заголовок и кнопку для перехода:</p>
<pre class="brush:xml;">
&lt;Grid&gt;
    &lt;StackPanel&gt;
...
<p>Теперь добавим в файл <i>MainPage.xaml.cs</i> обработчик нажатия кнопки:</p>
<pre class="brush:c#;">
...
<p>Чтобы перейти к другой странице, мы можем использовать метод <code>Frame.Navigate()</code>, в который передается тип объекта. В данном случае мы переходим на вторую страницу Page2.</p>
<p>Перегрузка метода <code>Frame.Navigate()</code> позволяет передать при переходе какой-то объект. Например, передадим строку:</p>
<pre class="brush:c#;">
private void Forward_Click(object sender, RoutedEventArgs e)
{
...
<p>Теперь займемся страницей Page2. Определим на ней следующий код xaml:</p>
<pre class="brush:xml;">
&lt;Grid&gt;
    &lt;StackPanel&gt;
...
<p>Теперь перейдем к коду Page2.xaml.cs и вначале получим переданный из первой странице объект. В коде c# каждой страницы уже по умолчанию определен 
метод <span class="bb">OnNavigatedTo(NavigationEventArgs e)</span> и изменим его следующим образом:</p>
<pre class="brush:c#;">
...
<p>То есть объект <code>e.Parameter</code> - это тот объект, который передается во втором параметре метода <code>Frame.Navigate(typeof(Page2), "(переход из MainPage)")</code></p>
<p>И при переходе из первой страницы в заголовок добавится переданное из MainPage сообщение:</p>
<img src="pics/7.2.png" />
<p>Нам необязательно передавать именно строку. Это может быть объект любого класса. Например,  у нас есть следующий класс PageInfo:</p>
<pre class="brush:c#;">
class PageInfo
{
...
<p>Тогда на странице MainPage мы можем передать объект этого класса так:</p>
<pre class="brush:c#;">
private void Forward_Click(object sender, RoutedEventArgs e)
{
...
<p>А на странице Page2 получаем переданные данные так:</p>
<pre class="brush:c#;">
protected override void OnNavigatedTo(NavigationEventArgs e)
{
    if(e.Parameter!=null)
...
<p>Объект Frame поддерживает возврат на предыдущую страницу с помощью метода <span class="bb">Frame.GoBack()</span>, либо переход на следующую 
(метод <span class="bb">Frame.GoForward()</span>), которая имеется в истории переходов.</p>
<p>Итак, пусть страница MainPage содержит кнопку:</p>
<pre class="brush:xml;">
&lt;Button Content="Вперед" Click="Forward_Click" /&gt;
</pre>
<p>В коде обработчика нажатия кнопки пропишем следующий код:</p>
<pre class="brush:c#;">
private void Forward_Click(object sender, RoutedEventArgs e)
{
...
<p>Таким образом, если в истории переходов уже есть страница, на которую переходили с MainPage (то есть свойство Frame.CanGoForward равно true), то 
сработает переход к этой странице с помощью <code>Frame.GoForward()</code>. Иначе произойдет переход к странице Page2.</p>
<p>На странице Page2 определим две кнопки:</p>
<pre class="brush:xml;">
&lt;Button Content="Вперед" Click="Forward_Click" /&gt;
&lt;Button Content="Назад" Click="Back_Click" /&gt;
</pre>
<p>И определим в файле <i>Page2.xaml.cs</i> для них обработчики:</p>
<pre class="brush:c#;">
...
<p>На третьей странице Page3 определим одну кнопку:</p>
<pre class="brush:xml;">
&lt;Button Content="Назад" Click="Back_Click" /&gt;
</pre>
<p>И также добавим обработчик для этой кнопки:</p>
<pre class="brush:c#;">
private void Back_Click(object sender, RoutedEventArgs e)
{
    if (Frame.CanGoBack)
...
<img src="pics/7.3.png" alt="Навигация в Windows Phone 8.1" />
<p>Таким образом, мы можем создать навигацию последовательно для нескольких страниц и переходить по ним.</p>
<h3>Обработка кнопки Назад</h3>
<p>По умолчанию, если мы попытаемся перейти из второй страницы на главную с помощью экранной кнопки Назад, то нас выбросит из приложения. Но мы можем обработать 
событие <span class="bb">HardwareButtons.BackPressed</span>, тем самым переопределив действие кнопки Назад в нашем приложении. Для этого определим у второй 
страницы конструктор и метод:</p>
<pre class="brush:c#;">
public Page2()
{
...
<p>Если у нас в приложении есть история переходов, то осуществляем переход назад и после этого устанавливаем для свойства 
<code>BackPressedEventArgs.Handled</code> значение <code>true</code></p>
