...
<html >
<head>
<title>UWP | Основные операции с базой данных SQLite</title>
<meta charset="utf-8" />
<meta name="description" content="Основные операции с базой данных в Universal Windows Platform, использование Entity Framework Core 1.0 для работы с бд, добавление, удаление, редактирование и чтение данных">
...
<h2>Основные операции с базой данных SQLite</h2><div class="date">Последнее обновление: 14.03.2016</div>
...
<p>После создания моделей и добавления Entity Framework создадим собственно элементы управления, которые позволят нам добавлять, обновлять, удалять и просматривать 
данные из базы данных.</p>
<p>По умолчанию у нас уже есть страница MainPage.xaml. Пусть она будет выводить список компаний, то есть объектов класса Company, а также будет содержать элементы 
для управления этими объектами. Итак, изменим MainPage.xaml следующим образом:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Для отображения списка определен элемент ListView, а для управления компаниями в нижней панели определены три кнопки. Теперь в файле кода MainPage.xaml.cs 
определим для них обработчики нажатия:</p>
<pre class="brush:c#;">
...
<p>При загрузке страницы срабатывает обработчик <code>MainPage_Loaded</code>, в котором получаем список компаний из базы данных и устанавливаем его в качестве 
источника данных для ListView:</p>
<pre class="brush:c#;">companiesList.ItemsSource = db.Companies.ToList();</pre>
<p>Обработчик кнопки добавления <code>Add_Click</code> просто перенаправляет на страницу CompanyPage, которую мы чуть позже создадим.</p>
<p>Обработчик кнопки редактирования <code>Edit_Click</code> получает выделенный объект Company в списке и передает его id на страницу CompanyPage.</p>
<p>Обработчик кнопки удаления <code>Delete_Click</code> получает выделенный объект Company и передает его в метод Remove:</p>
<pre class="brush:c#;">db.Companies.Remove(company);</pre>
<p>Этот метод позволяет сгенерировать SQL-выражение DELETE для удаления данных из таблицы компаний. После этого надо сохранить все изменения, то есть 
выполнить это SQL-выражение с помощью вызова:</p>
<pre class="brush:c#;">db.SaveChanges();</pre>
<p>И далее переустанавливаем источник данных для ListView.</p>
<p>В итоге главная страница будет выглядеть примерно так:</p>
<img src="pics/17.7.png" alt="Работа с базой данных в UWP" />
<p>Для добавления/редактирования добавим новую страницу по типу BlankPage, которую назовем CompanyPage:</p>
<img src="pics/17.6.png" alt="CRUD операции в UWP" />
<p>Так как модель Company имеет по сути одно значимое свойство - Name, то много нам добавлять и редактировать не надо, и для обеих операций нам хватит одной страницы. 
Тем более эти операции у нас будут отличаться только наличием Id при редактировании.</p>
<p>Изменим начальный код страницы <i>CompanyPage.xaml</i> на следующий:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Здесь одно текстовое поле для редактирования названия компании и две кнопки: для сохранения изменений и отмены действий.</p>
<p>Далее изменим код C#, добавив в него обработчики кнопок:</p>
<pre class="brush:c#;">
...
<p>При переходе на страницу срабатывает метод <code>OnNavigatedTo()</code>. В нем мы смотрим на переданный параметр. Если он не равен null, значит, нам надо отредактировать 
сушность, идентификатор которой равен параметру. В ином случае надо выполнить добавление. По этому идентифкатору мы получаем компанию из бд:</p>
<pre class="brush:c#;">company = db.Companies.FirstOrDefault(c =&gt; c.Id == id);</pre>
<p>При нажатии на кнопку сохранения смотрим, была ли получена ранее компания. Если она была получена, значит, нам надо отредактировать ее. Для этого объект компании 
передается в метод <code>Update()</code>:</p>
<pre class="brush:c#;">db.Companies.Update(company);</pre>
<p>Он генерирует выражение SQL UPDATE для обновления записей в базе данных.</p>
<p>Если же компания при загрузке страницы не была установлена, то выполняем добавление в базе данных с помощью метода <code>Add()</code>:</p>
<pre class="brush:c#;">db.Companies.Add(new Company { Name = nameBox.Text });</pre>
<p>Далее выполняем сгенерированное SQL-выражение с помощью метода <code>db.SaveChanges()</code>. И после всех изменений возвращаемся на главную страницу.</p>
<p>В итоге CompanyPage будет выглядеть примерно так:</p>
<img src="pics/17.8.png" alt="Добавление в базу данных на Universal Windows Platform" />
<p>Таким образом, мы создали простейшее приложение, которое выполняет все базовые операции с данными: чтение, добавление, обновление и удаление.</p>
