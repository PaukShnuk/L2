...
<html >
<head>
<title>UWP | Работа с папками</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с каталогами в файловой системе в Universal Windows Platform, управление папками, их создание, удаление, переименование, перемещение, получение файлов">
...
<h1>Файловая система</h1><h2>Работа с папками</h2><div class="date">Последнее обновление: 24.02.2016</div>
...
<p>Windows 10 предоставляет новый API для работы с файлами. Основу этого API составляет пространство имен <span class="b">Windows.Storage</span>, в котором и 
сосредоточен весь функционал для работы с файлами.</p>
<p>Одними из важнейших классов в этом API являются StorageFile и StorageFolder. Класс <span class="b">StorageFile</span> возвращает информацию об определенном 
файле и его содержимом, а также предоставляет способы для манипуляции с этим файлом. А класс <span class="b">StorageFolder</span> предназначен для управления папками пользователя.</p>
<p>При работе с файлами и папками надо учитывать, что нам доступны определенные виды хранилищ файлов и папок, с которыми мы можем взаимодействовать. 
...
в облаке. Все типы папок вне зависимости от местонахождения представлены классом <span class="b">StorageFolder</span>.</p>
<p>По умолчанию разработчик имеет доступ к следующим папкам в системе пользователя:</p>
<ul>
<li><p>Папка, в которую установлено приложение. Эта папка содержит все файлы и каталоги, которые были созданы в проекте (это любые папки, в которых 
для файлов у свойства <span class="b">Build Action</span> установлено значение "Content"). Так как эта папка полностью удаляется или пересоздается 
при каждом удалении/переустановке/обновлении приложения, то все сделанные изменения в этой папке будут удалены вместе с переустановкой или удалением приложения.</p>
<p>Чтобы получить эту папку, можно использовать следующее выражение:</p>
<pre class="brush:c#;">
StorageFolder installedLocation = ApplicationModel.Package.Current.InstalledLocation;
</pre>
<p>Кроме того, мы можем напрямую обратиться к файлам из этой папки, используя специальные адреса Uri с префиксом "ms-appx:///":</p>
<pre class="brush:c#;">
StorageFile file = await StorageFile.GetFileFromApplicationUriAsync("ms-appx:///file.txt");
</pre>
</li>
<li><p>Каталоги, предназначенные для хранения данных. Это три папки: локальная (local), перемещаемая (roaming) и временная (temp). Эти папки создаются 
при установке приложения. Для получения доступа к этим папкам, можно использовать следующие выражения:</p>
<pre class="brush:c#;">
...
<p>Также мы можем, используя специальные адреса Uri, обратиться к файлам из этих папок:</p>
<pre class="brush:c#;">
...
<p>При использовании папки TemporaryFolder следует учитывать, что содержимое этой папки в любое время может быть удалено системой, либо самим пользователем 
через средства удаления временных файлов, как Disk Cleanup.</p>
</li>
</ul>
<p>Класс StorageFolder, который представляет все эти папки, позволяет управлять ими через ряд своих методов:</p>
<ul>
<li><p><span class="b">GetFolderAsync()</span>: возвращает вложенную папку по имени</p></li>
<li><p><span class="b">GetFoldersAsync()</span>: возвращает все вложенные папки</p></li>
<li><p><span class="b">GetFilesAsync()</span>: возвращает все вложенные файлы</p></li>
<li><p><span class="b">GetFileAsync()</span>: возвращает один файл по имени</p></li>
<li><p><span class="b">CreateFileAsync()</span>: создает новый файл</p></li>
<li><p><span class="b">CreateFolderAsync()</span>: создает новую папку</p></li>
<li><p><span class="b">DeleteAsync()</span>: удаляет текущую папку</p></li>
<li><p><span class="b">GetItemsAsync()</span>: возвращает все вложенные папки и файлы</p></li>
<li><p><span class="b">GetItemAsync()</span>: возвращает по имени либо файл, либо папку, в зависимости, что этому имени соответствует</p></li>
<li><p><span class="b">GetParentAsync()</span>: возвращает родительскую папку</p></li>
<li><p><span class="b">OpenStreamForReadAsync()</span>: открывает поток для чтения</p></li>
<li><p><span class="b">OpenStreamForWriteAsync()</span>: открывает поток для записи</p></li>
<li><p><span class="b">RenameAsync()</span>: переименовывает папку</p></li>
</ul>
<p>С помощью свойств StorageFolder мы можем получить информацию о папке:</p>
<ul>
<li><p><span class="b">Attributes</span>: возвращает атрибуты данной папки</p></li>
<li><p><span class="b">DateCreated</span>: возвращает дату создания папки</p></li>
<li><p><span class="b">DisplayName</span>: возвращает отображаемое имя папки</p></li>
<li><p><span class="b">Name</span>: возвращает имя папки</p></li>
<li><p><span class="b">Path</span>: возвращает полный путь к данной папке в файловой системе</p></li>
</ul>
<p>Для примера создадим проект, в который поместим папку с файлами:</p>
<img src="pics/12.2.png" alt="Работа с папками на Universal Windows Platform" />
<p>Здесь в проект добавлена папка Data, в которой есть два файла. Получим этим файлы в программе. Для этого в коде разметки xaml определим ListBox:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>А в файле кода C# определим обработчик события загрузки страницы:</p>
<pre class="brush:c#;">
using System;
...
<p>Так как папка Data у нас в проекте, то при установке она попадает в текущую папку приложения, поэтому для ее получения сначала получаем текущую папку приложения 
с помощью выражения <code>Windows.ApplicationModel.Package.Current.InstalledLocation</code>. Получив все файлы, выводим их в ListBox:</p>
<img src="pics/12.3.png" />
...
<div class="nav"><p><a href="4.19.php">Назад</a><a href="./">Содержание</a><a href="12.2.php">Вперед</a></p></div></div>
<div class="socBlock">
