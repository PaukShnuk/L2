...
<html >
<head>
<title>UWP | Представления XAML</title>
<meta charset="utf-8" />
<meta name="description" content="Использование XAML Views в Universal Windows Platform для создания адаптивного дизайна, определение представления для каждого вида устройств">
...
<h2>Представления XAML</h2><div class="date">Последнее обновление: 20.02.2016</div>
...
<p>В предыдущих темах было рассмотрено два способа адаптации внешнего вида под разные размеры устройств - обработка события SizeChanged и использование Visual State. 
...
несколько разных представлений для одной и той же страницы, которые будут использоваться на разных устройствах.</p>
<p>Есть два способа определения таких представлений:</p>
<ol>
<li><p>Можно создать в проекте отдельные папки для хранения представлений, предназначенных для отдельных устройств. Эти папки должны называться по шаблону 
<i>DeviceFamily-Тип</i>, где "Тип" - это тип устройства (Desktop, Mobile).</p></li>
<li><p>Можно просто добавить представления, названия которых должны оканчиваться на <i>DeviceFamily-Тип.xaml</i>, где "Тип" также это тип устройства.</p></li>
</ol>
<p>Первый подход является более предпочтительным, так как позволяет организовать представления по отдельным каталогам, в связи с чем с ними проще работать. Поэтому применим данный подход. 
Создадим новое приложение Universal Windows Platform. И изменим код xaml его главной страницы MainPage.xaml:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Главная страница выводит горизонтальный стек розовых кнопок:</p>
<img src="pics/10.6.png" />
<p>Теперь добавим в этот проект новую папку, которую назовем <i>DeviceFamily-Mobile</i>. И далее в нее добавим новую страницу, которую назовем как и главную - 
MainPage.xaml:</p>
<img src="pics/10.7.png" />
<p>Изменим код этой страницы, чтобы он выводил вертикальный стек светло-синих кнопок:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>И теперь если мы запустим приложение на мобильном устройстве, то соответственно сработает MainPage.xaml из папки DeviceFamily-Mobile:</p>
<img src="pics/10.8.png" alt="Адаптивный дизайн на Windows 10 Mobile" />
<p>При этом без разницы какую ориентацию мобильного устройства мы используем - портретную или альбомную - в любом случае будет срабатывать представление из 
папки DeviceFamily-Mobile. Во всех остальных случаях будет работать представление MainPage.xaml из корня проекта.</p>
<p>Если бы мы использовали второй подход к созданию адаптивных представлений, тогда нам надо было добавить в корень проекта преставление с именем 
<i>MainPage.DeviceFamily-Mobile.xaml</i>.</p>
