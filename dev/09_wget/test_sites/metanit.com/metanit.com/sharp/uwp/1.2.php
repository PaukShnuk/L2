...
<html >
<head>
<title>UWP | Начало работы. Первое приложение</title>
<meta charset="utf-8" />
<meta name="description" content="Первое приложение на платформе UWP с помощью языка программирования C#, особенности Universal Windows Platform">
...
<h2>Начало работы. Первое приложение</h2><div class="date">Последнее обновление: 12.11.2017</div>
...
<p>Итак, начнем работу с Universal Windows Platform и создадим первый проект. Для этого откроем Visual Studio 2017 и в меню выберем пункт 
<span class="b">File-&gt;New-&gt;Project...</span>. Перед нами откроется окно создания нового проекта:</p>
<img src="pics/1.3.png" alt="Проект Universal Windows Platform" />
<p>В этом окне нам надо выбрать для языка C# выбрать шаблон <span class="b">Blank App (Universal Windows)</span>. 
Внизу окна установим путь к проекту и дадим имя проекта, например, HelloApp. И нажем на кнопку OK.</p>
<p>После этого нам отображается с выбором тех версий SDK, на которые будет ориентировано приложение:</p>
<img src="pics/1.9.png" alt="SDK Universal Windows Platform" />
<p>Как правило, эти версии отражают глобальные обновления ОС Windows, например, Creators Update, Anniversary Update и т.д. В качестве опции 
<span class="b">Target Version</span> (целевой среды приложения) рекомендуется устанавливать последнюю доступную версию SDK, которая обычно уже установлена по умолчанию. 
При выборе минимальной версии следует учитывать, что выбранное значение задает минимальную версию платформы UWP, с которой проект может работать.</p>
<p>Оставим данные опции по умолчанию и нажмем на OK. И Visual Studio создает новый проект:</p>
<img src="pics/1.4.png" alt="Проект Universal Windows Platform" />
<p>Рассмотрим все узлы, из которых состоит проект:</p>
<ul>
<li><p><span class="b">AssemblyInfo.cs</span>: файл кода на c#, который устанавливает информацию о сборке приложения</p></li>
<li><p><span class="b">Default.rd.xml</span>: файл, который содержит директивы .NET Native - технологии, которая позволяет компилировать код C# в машинный код</p></li>
<li><p><span class="b">References</span>: стандартный узел с подключенными библиотеками</p></li>
<li><p><span class="b">Assets</span>: предназначен для хранения различных ресурсов приложения. По умолчанию здесь несколько файлов изображений</p></li>
<li><p><span class="b">App.xaml</span> и <span class="b">App.xaml.cs</span>: главный файл приложения с кодом XAML и связанный с ним файл кода C#. Они устанавливают 
некоторую общую логику, общие ресурсы для всего приложения</p></li>
<li><p><span class="b">HelloApp_TemporaryKey.pfx</span>: временный ключ, которым подписываются приложения во время запуска на локальном компьютере разработчика</p></li>
<li><p><span class="b">MainPage.xaml</span> и <span class="b">MainPage.xaml.cs</span>: файлы главной страницы приложения - графическое представление в виде кода XAML и логика в виде 
кода C#</p></li>
<li><p><span class="b">Package.appxmanifest</span>: файл манифеста, устаналивающий различные настройки приложения</p></li>
</ul>
<p>Теперь сделаем простейшее приложение. Визуально приложение UWP представлено через страницы - отдельные объекты Page. По умолчанию в проекте есть 
только одна страница - MainPage. Код этой страницы и определяет то, что мы увидим на экране при запуске приложения. И теперь немного изменим ее.</p>
<p>Итак, откроем файл MainPage.xaml:</p>
<img src="pics/1.5.png" alt="MainPage in Universal Windows Platform" />
<p>Снизу у нас окажется разметка на языке XAML - языке для декларативного определения графического интерфейса, а сверху визуальное представления кода XAML 
в виде прямоугольника, представляющего окно приложения.</p>
<p>Внесем небольшие изменения. Изменим код XAML на следующий:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Суть сделанных нами изменений состоит в том, что мы добавили элемент <code>&lt;TextBlock Text="Hello World!" FontSize="30" /&gt;</code> - обычную текстовую метку с надписью 
"Hello World!". И чтобы сделать текст крупным, у текстовой метки устанавливается шрифт высотой в 30 единиц. После изменения также должны произойти автоматические изменения в визуальном представлении:</p>
<img src="pics/1.6.png" alt="Код XAML in UWP" />
<p>Теперь запустим проект на выполнение, нажав на F5. И у нас отобразиться такое вот окошко, представляющее MainPage:</p>
<img src="pics/1.7.png" alt="Первое приложение на UWP" />
<p>При запуске это приложение устанавливается на рабочий компьютер разработчика, и мы можем найти его в меню Пуск и оттуда запускать:</p>
<img src="pics/1.8.png" alt="Установка приложения Universal Windows Platform" />
<h3>Запуск на Windows 10 Mobile</h3>
<p>Также мы можем развернуть и простестировать приложение на мобильном устройстве под управлением Windows 10 Mobile. Но для этого надо подключить мобильное устройство 
к компьютеру с помощью кабеля USB, а на самом смартфоне 
установить режим разработчика на телефоне можно, перейдя в <span class="b">Параметры</span> -&gt; 
<span class="b">Обновление и безопасность</span> -&gt; <span class="b">Для разработчиков</span> и выбрав пункт "Режим разработчика":</p>
<img src="pics/14.3.png" alt="Режим разработчика на смартфоне Windows 10 Mobile" />
