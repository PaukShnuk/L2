...
<html >
<head>
<title>UWP | Специальные папки</title>
<meta charset="utf-8" />
<meta name="description" content="Работа со специальными папками в Universal Windows Platform, получение доступа к каталогам документов, музыки и видео и получение из них файлов">
...
<h2>Специальные папки</h2><div class="date">Последнее обновление: 24.02.2016</div>
...
<p>Кроме описанных в прошлой теме папок, которые доступны разработчику, мы также можем использовать еще ряд папок, которые являются специальными и 
предназначены для хранения файлов пользователя. Доступ к ним можно получить через свойства класса <span class="b">Windows.Storage.KnownFolders</span>:</p>
<ul>
<li><p><span class="b">DocumentsLibrary</span>: представляет папку Documents (Документы)</p></li>
<li><p><span class="b">HomeGroup</span>: каталог HomeGroup</p></li>
<li><p><span class="b">MediaServerDevices</span>: папка Media Server Devices</p></li>
<li><p><span class="b">MusicLibrary</span>: папка Music (Музыка)</p></li>
<li><p><span class="b">PicturesLibrary</span>: папка Pictures (Изображения)</p></li>
<li><p><span class="b">RemovableDevices</span>: папка Removable Devices</p></li>
<li><p><span class="b">VideosLibrary</span>: папка Videos (Видео)</p></li>
<li><p><span class="b">SavedPictures</span>: папка для сохраненных скриншотов</p></li>
</ul>
<p>Но просто так обратиться к этим папкам, например, написав:</p>
<pre class="brush:c#;">StorageFolder folder = KnownFolders.PicturesLibrary;</pre>
<p>мы не можем. Чтобы с ними работать, нам надо изменить файл манифеста.</p>
<p>Итак, откроем файл <span class="b">Package.appxmanifest</span> и перейдем на вкладку <span class="b">Capabilities</span>. И здесь нам надо отметить те специальные 
папки, к которым мы будем обращаться:</p>
<img src="pics/12.4.png" alt="Capabilities в Package.appxmanifest в UWP" />
<p>В данном случае я отметил пункт "Pictures Library", тем самым сделав доступной для моего кода папку изображений. И теперь я могу к ней обращаться:</p>
<pre class="brush:c#;">
...
<img src="pics/12.5.png" alt="Получение папки изображений в UWP" />
<h3>Папка загрузок</h3>
<p>Еще одна папка, которую мы можем использовать, - это папка загрузок или Downloads. Для работы с ней предназначен специальный класс <span class="b">DownloadsFolder</span>. 
Однако возможности этого класса ограничены преимущественно функциональностью записи. Так, нам доступны следующие методы:</p>
<ul>
<li><p><span class="b">CreateFileAsync()</span>: создает файл в папке загрузок</p></li>
<li><p><span class="b">CreateFileForUserAsync()</span>: создает файл для определенного пользователя</p></li>
<li><p><span class="b">CreateFolderAsync()</span>: создает папку</p></li>
<li><p><span class="b">CreateFolderForUserAsync()</span>: создает папку для определенного пользователя</p></li>
</ul>
<p>Например, создание новой папки и файла в папке загрузок:</p>
<pre class="brush:c#;">
// создание папки Data
