...
<html >
<head>
<title>UWP | Фигуры и рисование</title>
<meta charset="utf-8" />
<meta name="description" content="Рисование в Universal Windows Platform, использование фигур Ellipse, Line, Rectangle, Polygon, Polyline, создание сложных графических рисунков">
...
<h2>Фигуры и рисование</h2><div class="date">Последнее обновление: 29.02.2016</div>
...
<p>Фигуры представляют один из способов построения простейшей двухмерной графики и фактически являются обычными элементами как, к примеру, кнопка. 
К фигурам относятся <span class="b">Polygon</span> (многоугольник), <span class="b">Ellipse</span> (эллипс), <span class="b">Rectangle</span> (прямоугольник), 
<span class="b">Line</span> (простая линия), <span class="b">Polyline</span> (несколько линий), <span class="b">Path</span> (геометрический путь). 
Все они наследуются от абстрактного класса <span class="b">Shape</span>, который определяет ряд общих свойств:</p>
<ul>
<li><p><span class="b">Fill</span>: заполняет фон фигуры с помощью кисти аналогично свойству <span class="b">Background</span> у других элементов</p></li>
<li><p><span class="b">Stroke</span>: устанавливает кисть, которая отрисовывает границу фигуры</p></li>
<li><p><span class="b">StrokeThikness</span>: задает толщину границы фигуры</p></li>
<li><p><span class="b">StrokeStartLineCap</span> и <span class="b">StrokeEndLineCap</span>: создают для незамкнутых фигур (Line) контур в начале 
и в конце линии соответственно</p></li>
<li><p><span class="b">StrokeDashArray</span>: задает границу фигуры в виде штриховки, создавая эффект пунктира</p></li>
<li><p><span class="b">StrokeDashOffset</span>: задает расстояние до начала штриха</p></li>
<li><p><span class="b">StrokeDashCap</span>: задает форму штрихов</p></li>
</ul>
<h3>Ellipse</h3>
<p>Ellipse представляет овал:</p>
<pre class="brush:xml;">&lt;Ellipse Fill="LightBlue" Width="200" Height="200" /&gt;</pre>
<p>При одинаковой ширине и высоту получается круг:</p>
<img src="pics/13.4.png" alt="Ellipse в Universal Windows Platform" />
<h3>Rectangle</h3>
<p>Rectangle представляет прямоугольник:</p>
<pre class="brush:xml;">
...
<img src="pics/13.5.png" alt="Rectangle в Universal Windows Platform" />
<h3>Line</h3>
<p>Line представляет простую линию. Для создания линии надо указать координаты в ее свойствах X1, Y1, X2 и Y2. При этом надо учитывать, что началом координатной системы 
является верхний левый угол:</p>
<pre class="brush:xml;">
&lt;Line X1="100" Y1="30" X2="200" Y2="150" Stroke="Red" /&gt;
</pre>
<img src="pics/13.7.png" alt="Line в UWP" />
<h3>Polygon</h3>
<p>Polygon представляет многоугольник. С помощью коллекции <span class="b">Points</span> элемент устанавливает набор точек - объектов типа <span class="b">Point</span>, которые последовательно соединяются 
линиями, причем последня точка соединяется с первой:</p>
<pre class="brush:xml;">
&lt;Polygon Fill="LightPink" Points="50, 150, 150, 50, 250, 150" /&gt;
</pre>
<img src="pics/13.6.png" alt="Polygon в Universal Windows Platform" />
<p>В данном случае у нас три точки (50, 150), (150, 50) и (250, 150), которые образуют треугольник.</p>
<h3>Polyline</h3>
<p>Polyline представляет набор точек, соединенных линиями. В этом плане данный элемент похож на Polygon за тем исключением, что первая и последняя точка не соединяются:</p>
<pre class="brush:xml;">
&lt;Polyline Stroke="Red" Points="50, 150, 150, 50, 250, 150" /&gt;
</pre>
<img src="pics/13.8.png" alt="Polyline в Universal Windows Platform" />
<h3>Path</h3>
<p>Элемент Path предназначен для создания сложных геометрических путей. Класс Path имеет свойство <span class="b">Data</span>, которое определяет объект 
<span class="b">Geometry</span> - геометрический объект для отрисовки. Этот объект задает фигуру или совукупность фигур для отрисовки.</p>
<p>Geometry является абстрактным классом, поэтому в качестве объекта используется один из производных классов:</p>
<ul>
<li><p><span class="b">LineGeometry</span> представляет линию, эквивалент фигуры Line</p></li>
<li><p><span class="b">RectangleGeometry</span> представляет прямоугольник, эквивалент фигуры Rectangle</p></li>
<li><p><span class="b">EllipseGeometry</span> представляет овал, эквивалент фигуры Ellipse</p></li>
<li><p><span class="b">PathGeometry</span> представляет путь, образующий сложную геометрическую фигуру из простейших фигур</p></li>
<li><p><span class="b">GeometryGroup</span> создает фигуру, состоящую из нескольких объектов Geometry</p></li>
</ul>
<p>Например, создадим набор фигур:</p>
<pre class="brush:xml;">
&lt;Path Stroke="Red"&gt;
    &lt;Path.Data&gt;
...
<img src="pics/13.9.png" alt="Geometry в UWP" />
<p>Номинально здесь три фигуры: линия, овал и прямоугольник. Однако так как они объединены в один путь, то мы можем управлять ими как единым целым.</p>
<p>Свойство <span class="b">FillRule</span> позволяет установить режим закрашивания фигур в геометрическом пути. Если он равен <code>EvenOdd</code>, 
то  (значение по умолчанию), то перекрывающие поверхности являются прозрачными. Если же установить <code>FillRule="Nonzero"</code>, то перекрывающиеся 
поверхности геометрий будут окрашены также, как и остальные части пути. Цвет в данном случае определяется свойством <span class="b">Fill</span> класса Path.</p>
<h3>PathGeometry</h3>
<p><span class="b">PathGeometry</span> содержит один или несколько компонентов <span class="b">PathFigure</span>. Объект <span class="b">PathFigure</span> в свою очередь формируется из сегментов. 
...
<img src="pics/13.10.png" alt="Геометрический путь в UWP" />
<p>Существует также сокращенный вариант записи сегментов путей. Так, следующее описание пути:</p>
<pre class="brush:xml;">
&lt;Path Stroke="Red"&gt;
    &lt;Path.Data&gt;
        &lt;PathGeometry&gt;
...
<tr><td><p>S x1,y1, x,y</p></td><td><p>Создает новый объект BezierSegment по указанным точкам</p></td></tr>
</table>
...
<div class="nav"><p><a href="14.1.php">Назад</a><a href="./">Содержание</a><a href="13.1.php">Вперед</a></p></div></div>
<div class="socBlock">
