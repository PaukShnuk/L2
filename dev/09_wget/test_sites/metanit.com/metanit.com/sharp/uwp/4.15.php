...
<html >
<head>
<title>UWP | ComboBox</title>
<meta charset="utf-8" />
<meta name="description" content="Элемент ComboBox в Universal Windows Platform, его основные свойства и методы, примеры использования, работа с выпадающими списками в UWP">
...
<h2>ComboBox</h2><div class="date">Последнее обновление: 14.04.2017</div>
...
<p>ComboBox представляет раскрывающийся список. Он применяется, чтобы предоставить пользователю возможность выбрать одно из значений 
в списке. ComboBox больше подходит для выбора элементов, которые представляют обычные строки. Хотя более сложные данные также можно добавлять в ComboBox.</p>
<p>Если элементов в списке меньше пяти, то оптимальнее использовать для выбора набор элементов RadioButton или CheckBox.</p>
<p>Например, определим простейший элемент ComboBox:</p>
<pre class="brush:xml;">
&lt;Page
...
<img src="pics/4.46.png" alt="ComboBox в Universal Windows Platform" />
<img src="pics/4.47.png" alt="ComboBox в UWP" />
<h3>ComboBoxItem</h3>
<p>В ComboBox можно добавлять различные компоненты, например, текстовые блоки, но кроме того специально для ComboBox определен 
класс <span class="b">ComboBoxItem</span>, который представляет отдельный элемент списка. ComboBoxItem представляет элемент управления содержимым, в который через свойство Content мы можем поместить 
другие элементы. Например:</p>
<pre class="brush:xml;">
&lt;ComboBox Width="200"&gt;
...
<img src="pics/4.48.png" alt="Выпадающий список в Universal Windows Platform" />
<p>Для создания первого элемента использовался элемент ComboBoxItem. Для второго и третьего такие элементы создаются неявно. Однако использование 
<code>ComboBoxItem</code> имеет преимущество, так как мы можем выделить данный элемент, установив свойство <code>IsSelected="True"</code>, либо можем сделать 
недоступным с помощью установки свойства <code>IsEnabled="True"</code>.</p>
<p>Но хотя мы можем выводить в ComboBox сложные объекты с изображениями и различными свойствами, но для этого все же лучше использовать более специализированные элементы типа ListView или GridView.</p>
<p>Во всем остальном, что касается обработки выбора элемента, привязки к сложным коллекциям, использования DataTemplate ComboBox работает также, как и другие элементы-списки, например, ListBox.</p>
...
<div class="nav"><p><a href="11.3.php">Назад</a><a href="./">Содержание</a><a href="4.16.php">Вперед</a></p></div></div>
<div class="socBlock">
