...
<html >
<head>
<title>UWP | Элементы управления</title>
<meta charset="utf-8" />
<meta name="description" content="Элементы управления в Universal Windows Platform, типы элементов, их общие свойства">
...
<h1>Элементы управления</h1><h2>Обзор элементов управления в Universal Windows Platform</h2><div class="date">Последнее обновление: 11.02.2016</div>
...
<p>Universal Windows Platform предоставляет большой выбор элементов, которые условно можно объединить в несколько групп:</p>
<ul>
<li><p><span class="b">Элементы управления содержимым</span>: различные кнопки, например, элементы Button, ToggleButton, RadioButton, CheckBox и др. Отличительной 
особенностью этих элементов является свойство <code>Content</code>, которое устанавливает содержимое элемента</p></li>
<li><p><span class="b">Текстовые элементы</span>: это элементы TextBlock, TextBox, PasswordBox, которые управляют некоторым текстом через свойство 
<code>Text</code></p></li>
<li><p><span class="b">Элементы управления списками</span>: эта группа элементов отличается тем, что хранит в себе список других элементов в виде свойства 
<code>Items</code>. К подобным элементам можно отнести ListBox, ListView, GridView, ComboBox, FlipView</p></li>
<li><p><span class="b">Специальные контейнеры</span>: они не являются элементами компоновки, однако могут вмещать в себя другие элементы и влиять на их позиционирование. 
В частности, это элементы Border и ScrollViewer</p></li>
<li><p><span class="b">Элементы для работ с датами и временем</span>: эти элементы управляет датой и(или) временем в виде объекта DateTimeOffset, они представлены 
классами DatePicker, TimePicker, CalendarDatePicker, CalendarView</p></li>
<li><p><span class="b">Элементы, основанные на диапазонах значений</span>: это элементы Slider, ProgressBar, которые служат для визуализации некоторого диапазона значений</p></li>
<li><p>Все остальные элементы, не попавшие в предыдущие группы</p></li>
</ul>
<p>Хотя данное разделение довольно условно, однако в ряде случаев элементы из одной группы имеют некоторые общие черты в поведении.</p>
<p>Большинство элементов в Universal Windows Platform наследуется от класса Control, а все дерево наследования можно представить следующим образом:</p>
<img src="pics/4.53.png" alt="Элементы управления в Universal Windows Platform" />
<p>Все элементы имеют ряд общих свойств.</p>
<h3>Свойство Name</h3>
<p>Это свойство устанавливает имя элемента и позволяет на него ссылаться в коде xaml или c#. Например, пусть в xaml установлена следующая кнопка:</p>
<pre class="brush:xml;">&lt;Button x:Name="myButton" Content="MyButton" /&gt;</pre>
<p>Тогда в файле кода C# мы можем обратиться к этой кнопке:</p>
<pre class="brush:c#;">myButton.Content = "Привет!";</pre>
<h3>FieldModifier</h3>
<p>Свойство <span class="b">FieldModifier</span> задает модификатор доступа к объекту (<code>private, protected, internal, protected internal</code> и <code>public</code>):</p>
<pre class="brush:xml;">
...
<p>В данном случае объявление кнопок с модификаторами будет равноценно следующему их определению в коде:</p>
<pre class="brush:xml;">
private Button button1;
internal Button button2;
</pre>
<p>Если атрибут FieldModifier не определен, то по умолчанию элемент использует модификатор "protected internal".</p>
<h3>Свойство Visibility</h3>
<p>Это свойство устанавливает параметры видимости элемента и может принимать одно из двух значений:</p>
<ul>
<li><p><code>Visible</code>: элемент виден и участвует в компоновке</p></li>
<li><p><code>Collapsed</code>: элемент не виден и не участвует в компоновке, то есть при размещении других элементов в контейнере он не учитывается, как будто его и нет</p></li>
</ul>
<h3>Свойства шрифтов</h3>
<p>Специальная группа свойств позволяет настроить шрифт:</p>
<ul>
<li><p><span class="b">FontFamily</span>: определяет семейство шрифтов. Например, "Verdana", "Arial". Установка в коде C#:</p>
<pre class="brush:c#;">button1.FontFamily = new FontFamily("Verdana");</pre>
</li>
<li><p><span class="b">FontSize</span>: определяет высоту шрифта</p></li>
<li><p><span class="b">FontStyle</span>: определяет наклон шрифта, принимает одно из трех значений - Normal, Italic, Oblique. Установка в коде C#:</p>
<pre class="brush:c#;">button1.FontStyle = Windows.UI.Text.FontStyle.Italic;</pre>
</li>
<li><p><span class="b">FontWeight</span>: определяет толщину шрифта и принимает ряд значений, как Black,Bold и др. Установка в коде C#:</p>
<pre class="brush:c#;">button1.FontWeight = new Windows.UI.Text.FontWeight() { Weight = 999 };</pre>
</li>
<li><p><span class="b">FontStretch</span>: определяет, как будет растягивать или сжимать текст, например, значение Condensed сжимает текст, а Expanded - растягивает. Установка в коде C#:</p>
<pre class="brush:c#;">button1.FontStretch = Windows.UI.Text.FontStretch.Expanded;</pre>
</li>
</ul>
<p>Например:</p>
<pre class="brush:xml;">&lt;Button Content="Hello World!" FontFamily="Verdana" FontSize="13" FontStretch="Expanded" /&gt;</pre>
<h3>Свойство FlowDirection</h3>
...
<img src="pics/4.54.png" alt="FlowDirection в Universal Windows Platform" />
<h3>Цвета фона и шрифта</h3>
<p>Свойства Background и Foreground задают соответственно цвет фона и текста элемента управления.</p>
<p>Одним из способов задания цвета представляет использование его числового кода, например: <code>Background="#ffffff"</code>. 
В качестве значения свойство Background (Foreground) может принимать запись в виде шестнадцатеричного значения в формате <span class="b">#rrggbb</span>, 
где rr - красная составляющая, gg - зеленая составляющая, а bb - синяя. Также можно при установке цвета учитывать альфа-компонент, используя формат #aarrggbb.</p>
<p>Но проще всего использовать уже готовые константы из класса <span class="b">Windows.UI.Colors</span>:</p>
<pre class="brush:xml;">&lt;Button Background="LightGray" Foreground="Red" Content="Hello" /&gt;</pre>
<p>Однако при компиляции будет создаваться объект <span class="b">SolidColorBrush</span>, который и будет задавать цвет элемента. То есть определение кнопки выше фактически будет равноценно следующему:</p>
<pre class="brush:xml;">
