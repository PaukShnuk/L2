...
<html >
<head>
<title>UWP | ItemTemplate и DataTemplate</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с данными в Universal Windows Platform, свойство ItemTemplate и элемент DataTemplate, добавление данные в элементы UWP">
...
<h2>ItemTemplate и DataTemplate</h2><div class="date">Последнее обновление: 14.04.2017</div>
...
<p>Элемент ListBox позволяет легко установить привязку к коллекции объектов, однако по умолчанию каждый элемент ListBox отображает строку. 
Если же нам необходимо отобразить более сложные объекты в ListBox, то для этого надо использовать объект DataTemplate.</p>
<p>Ключевым свойством для отображения данных во всех элементах-списках (не только в ListBox) является свойство <span class="b">ItemTemplate</span>. Оно задает шаблон 
данных. В качестве значения этого свойства устанавливается объект <span class="b">DataTemplate</span> - именно этот объект и управляет отображением данных.</p>
<p>В качестве самих данных, которые отображаются в DataTemplate, как правило, выступают объекты пользовательских классов, создаваемых самим разработчиком. Например, 
определим в проекте следующий класс Phone, который будет представлять модель телефона:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Далее создадим в классе MainPage свойство, которое будет представлять коллекцию смартфонов:</p>
<pre class="brush:c#;">
using System;
...
<p>Здесь создается 4 объекта Phone. Для каждого объекта устанавливаются необходимые значения. Причем свойство <code>ImagePath</code> указывает на 
путь к изображению в папке Assets. Предполагается, что в папке Assets есть четыре изображения для каждой из моделей.</p>
<p>Теперь изменим разметку MainPage.xaml:</p>
<pre class="brush:xml;">
&lt;Page
            ...
            &lt;ListBox.ItemTemplate&gt;
                &lt;DataTemplate x:DataType="local:Phone"&gt;
                    &lt;StackPanel Margin="10"&gt;
                        &lt;Image Width="100" Height="80" Source="{x:Bind ImagePath}" /&gt;
...
<p>С помощью атрибута <code>ItemsSource="{x:Bind Phones}"</code> задаем привязку к свойству Phones из MainPage. А с помощью свойства <code>ListBox.ItemTemplate</code> задаем, как этот список Phones будет отображаться.</p>
<p>Элемент DataTemplate устанавливает, как будет отображаться один элемент Phone из списка Phones. Чтобы создать привязку именно к объекту Phone, в 
DataTemplate устанавливается атрибут <code> x:DataType="data:Phone"</code>. То есть префикс <code>data</code> указывает на то пространство имен, в 
котором определен класс Phone. После этой установки мы можем задать привязку у вложенных элементов к конкретным свойствам класса Phone:</p>
<pre class="brush:xml;">&lt;Image Width="200" Height="150" Source="{x:Bind ImagePath}" /&gt;</pre>
<p>А с помощью атрибута <code>SelectionChanged="phonesList_SelectionChanged"</code> устанавливается обработчик выделения элементов. Определим в классе MainPage этот обработчик:</p>
<pre class="brush:c#;">
private async void phonesList_SelectionChanged(object sender, SelectionChangedEventArgs e)
{
    Phone selectedPhone = (Phone)phonesList.SelectedItem;
...
<p>Мы можем получить выделенный элемент через свойство <code>phonesList.SelectedItem</code>. И так как DataTemplate отображает объекты Phone, то выделенный элемент 
мы можем привести к типу Phone.</p>
<p>В итоге получится следующее приложение:</p>
<img src="pics/11.1.png" alt="Data Binding и DataTemplate в UWP" />
...
<div class="nav"><p><a href="4.14.php">Назад</a><a href="./">Содержание</a><a href="11.2.php">Вперед</a></p></div></div>
<div class="socBlock">
