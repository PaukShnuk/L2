...
<html >
<head>
<title>UWP | Элементы управления содержимым</title>
<meta charset="utf-8" />
<meta name="description" content="Элементы управления содержимым в Universal Windows Platform, класс ContentControl, роль свойства Content, основные свойства">
...
<h2>Элементы управления содержимым</h2><div class="date">Последнее обновление: 11.02.2016</div>
...
<p>Все элементы управления содержимым инкапсулируют в себе какой-то другой элемент. Все они наследуются от класса 
<span class="b">ContentControl</span>, который определяет ряд общих методов и свойств, в частности, свойство <code>Content</code>, которое позволяет вложить в 
этот элемент другой элемент. К таким элементам управления содержимым можно отнести Button, ToggleButton, ToolTip, 
RadioButton, CheckBox и ряд других. Также элементом управления содержимым является и главный элемент окна - Window</p>
<p>Отличительной чертой элементов управления содержимым является наличие свойства <span class="b">Content</span>, которое и позволяет добавить внутри элемента 
...
а элементы управления содержимым только один.</p>
<p>В качестве содержимого свойство Content может принимать два вида объектов:</p>
<ul>
<li><p>Объект класса, который не наследуется от UIElement. Для такого объекта вызывается метод ToString(), который возвращает строковое преставление объекта. 
Затем эта строка устанавливается в качестве содержимого.</p></li>
<li><p>Объект класса, который наследуются от UIElement. У такого объекта вызывается метод UIElement.OnRender(), который выполняет отрисовку внутри элемента 
...
<p>Содержимое элемента Button здесь представляет обычную строку, то есть объект типа, который не наследуется от UIElement. Этот же пример мы можем в XAML 
прописать иначе:</p>
<pre class="brush:xml;">	
&lt;Button&gt;
    &lt;Button.Content&gt;
...
<p>Также можно использовать сокращенное определение свойства Content:</p>
<pre class="brush:xml;">&lt;Button&gt;Hello UWP!&lt;/Button&gt;</pre>
<p>Возьмем другой пример. Определим кнопку с именем myButton:</p>
<pre class="brush:xml;">	
&lt;Page
...
<p>А в файле кода MainPage.xaml.cs присвоим свойству Content какой-либо объект:</p>
<pre class="brush:c#;">
...
<p>В итоге получится следующую кнопку:</p>
<img src="pics/4.55.png" alt="Content Control в Universal Windows Platform" />
<p>В данном случае число конвертируется в строку, которая устанавливается в качестве содержимого. Иначе все будет работать, если мы в качестве содержимого 
используем объект, унаследованный от UIElement:</p>
<pre class="brush:xml;">
&lt;Button x:Name="myButton" HorizontalAlignment="Center"&gt;
    &lt;Button Content="Universal Windows Platform" /&gt;
&lt;/Button&gt;
</pre>
<p>Теперь в качестве содержимого будет использоваться другая кнопка, для которой при визуализации будет вызываться метод OnRender():</p>
<img src="pics/4.56.png" />
<p>То есть в данном случае получается кнопка внутри кнопки, и во внутреннюю кнопку мы можем вложить еще какой-нибудь элемент.</p>
<p>Чтобы создать подобную кнопку в коде C#, можно было бы использовать следующее выражение:</p>
<pre class="brush:c#;">myButton.Content = new Button { Content = "Universal Windows Platform" };</pre>
<p>Но необходимо помнить, что элементы управления содержимым могут содержать только один объект. Но что, если нам захочется поместить в кнопку сразу несколько 
других кнопок? В этом случае можно использовать контейнеры компоновки:</p>
<pre class="brush:xml;">
&lt;Button x:Name="myButton" HorizontalAlignment="Center"&gt;
    &lt;StackPanel&gt;
...
<h4>HorizontalContentAlignment</h4>
<p>Свойство HorizontalContentAlignment служит для выравнивания содержимого по горизонтали внутри элемента относительно его границ. Оно принимает следующие значения: 
Left (положение слева), Right (справа), Center (положение по центру), Stretch (растяжение по всей ширине).</p>
<pre class="brush:xml;">
...
<img src="pics/4.57.png" alt="HorizontalContentAlignment в Universal Windows Platform" />
<h4>VerticalContentAlignment</h4>
<p>Свойство VerticalContentAlignment служит для выравнивания содержимого элемента по вертикали. Оно принимает следующие значения: 
Top (положение в верху), Bottom (внизу), Center (по центру), Stretch (растяжение по всей высоте).</p>
