...
<html >
<head>
<title>UWP | Checkbox, RadioButton и ToggleSwitch</title>
<meta charset="utf-8" />
<meta name="description" content="Элементы Checkbox, ToggleSwitch и RadioButton в Universal Windows Platform, создание флажков и переключателей, их основные свойства, методы и события, примеры использования, обработка изменения состояния элемента">
...
<h2>Checkbox, RadioButton и ToggleSwitch</h2><div class="date">Последнее обновление: 12.04.2017</div>
...
<h3>CheckBox</h3>
<p>Флажок или CheckBox применяется для выбора из двух вариантов по принципу "да-нет", а также когда нам надо выбрать несколько опций из группы опций.</p>
<p>Формально CheckBox может принимать три состояния: <code>Checked</code>, <code>Unchecked</code> и <code>Intermediate</code>. Для установки 
состояния применяется свойство <span class="b">IsChecked</span>:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<img src="pics/4.6.png" alt="CheckBox in Universal Windows Platform" />
<p>Значение свойства <code>IsChecked="{x:Null}"</code> устанавливает флажок в неопределенное состояние. Остальные два состояния задаются с помощью значений <code>true</code> 
и <code>false</code>. Кроме того, чтобы можно было переключаться между всеми тремя состояниями, надо установить свойство <code>IsThreeState="True"</code></p>
<p>С помощью событий <span class="b">Checked</span> (генерируется при установке флажка в отмеченное состояние), <span class="b">Unchecked</span> (генерируется при снятии отметки с флажка) и 
<span class="b">Indeterminate</span> (флажок переведен в неопределенное состояние) мы можем отследить изменение состояния флажка. Например, определим элемент checkBox: </p>
<pre class="brush:xml;">
...
<p>И в файле кода c# определим все обработчики:</p>
<pre class="brush:c#;">
...
<img src="pics/4.7.png" alt="Обработка состояния CheckBox в UWP" />
<p>Также для нас представляет интерес еще два события - <span class="b">Tapped</span> и <span class="b">Click</span>, которые возникают при нажатии на флажок. Мы можем обработать одно из них:</p>
<pre class="brush:xml;">
...
<h3>RadioButton</h3>
<p>RadioButton представляет переключатель или радиокнопку. Переключатель используется для выбора из двух и более взаимоисключающих вариантов. Элемент RadioButton, как и CheckBox, может быть в отмеченном, неотмеченном и неопределенном 
состоянии. Отличительной чертой этих элементов является свойство <span class="b">GroupName</span>, которое задает группу радиокнопок. И в один момент времени 
мы можем отметить в одной группе только одну радиокнопку. Например,</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<img src="pics/4.8.png" alt="RadioButton in Universal Windows Platform" />
<p>Если бы радиокнопки принадлежали бы к двум разным группам, то соответственно мы могли бы выбрать две радиокнопки:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
    &lt;RadioButton GroupName="phones" Content="iPhone 6s Plus" IsChecked="True" /&gt;
    &lt;RadioButton GroupName="phones" Content="Nexus 6P" /&gt;
...
<img src="pics/4.9.png" />
<p>Чтобы проследить за выбором того или иного элемента, мы также можем определить у элементов событие Checked и его обрабатывать в коде:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
    &lt;TextBlock x:Name="textBlock1" /&gt;
...
<p>А в файле кода C# зададим обработчик:<p>
<pre class="brush:c#;">
...
<img src="pics/4.10.png" />
<h3>ToggleSwitch</h3>
<p><span class="b">ToggleSwitch</span> служит для переключения между двумя состояниями, которые вступают в силу сразу после изменения состояния переключателя. В этом плане элемент похож на CheckBox. Для определения текущего состояния используется 
свойство <span class="b">IsOn</span>:</p>
<pre class="brush:xml;">&lt;ToggleSwitch IsOn="True" Header="On/Off" VerticalAlignment="Top" /&gt;</pre>
<img src="pics/4.11.png" alt="ToggleSwitch in Universal Windows Platform" />
<p>Свойство <span class="b">Header</span> позволяет задать заголовок. При этом кроме заголовка элемент хранит текстовые метки для включенного и выключенного состояний. 
По умолчанию это "on" и "off". При выборе текстовые метки отображаются рядом с элементом. Но с помощью свойств <span class="b">OnContent</span> и 
<span class="b">OffContent</span> мы можем изменить эти метки или вложить более сложное содержимое, чем обычный текст:</p>
<pre class="brush:xml;">&lt;ToggleSwitch IsOn="True" OnContent="Включено" OffContent="Выключено" VerticalAlignment="Top" /&gt;</pre>
<img src="pics/4.12.png" alt="Кастомизация ToggleSwitch в UWP" />
<p>Несмотря на то, что можно задать длинные метки в качестве текста, рекомендуется использовать короткие метки для обозначения состояния, не длиннее 4 символов, например, вкл. и выкл.</p>
<p>Для отслеживания изменения состояния элементы мы можем задействовать событие <span class="b">Toggled</span>:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
    &lt;TextBlock x:Name="textBlock1" /&gt;
...
<p>А в коде c# пропишем обработчик:</p>
<pre class="brush:c#;">
...
<p>Программное создание ToggleSwitch:</p>
<pre class="brush:c#;">
