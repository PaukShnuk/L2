...
<html >
<head>
<title>UWP | ListBox</title>
<meta charset="utf-8" />
<meta name="description" content="Элемент ListBox в Universal Windows Platform, его основные свойства и методы, примеры использования, класс ListBoxItem">
...
<h2>ListBox</h2><div class="date">Последнее обновление: 14.04.2017</div>
...
<p>Элемент ListBox представляет собой обычный список. Содержит коллекцию элементов <span class="b">ListBoxItem</span>, которые являются элементами управления содержимым. 
Также ListBox может содержать любые другие элементы, например:</p>
<pre class="brush:xml;">
...
<img src="pics/4.45.png" alt="ListBox в Universal Windows Platform" />
<p>Все эти элементы будут находиться в коллекции <code>phonesList.Items</code> и, таким образом, по счетчику мы сможем к ним обратиться, например, 
<code>phonesList.Items[0]</code> - первый элемент ListBox, который в данном случае представляет TextBlock. Также мы можем установить элемент:</p> 
<pre class="brush:c#;">phonesList.Items[2]="Google Pixel";</pre>
<p>Компонент ListBoxItem представляет элемент управления содержимым, поэтому также мы можем задавать через его свойство Content более сложные композиции элементов, например:</p>
<pre class="brush:xml;">
...
<img src="pics/4.44.png" alt="ListBox в UWP" />
<p>В данном случае предполагается, что в папке Assets у нас находятся три файла изображений. Мы можем использовать элементы как внутри ListBoxItem, 
так и непосредственно вставлять их в список.</p>
<p>В в реальности, конечно, редко придется прибегать к подобному определению элементов списка. Как правило, весь список задается через привязку через свойство ItemsSource, 
как было расмотрено в прошлой теме. А для отображения сложных данных обычно применяются шаблоны DataTemplate, которые будут рассмотрены в следующей теме.</p>
<h3>Выделение элементов</h3>
<p>ListBox поддерживает множественный выбор. Для этого нужно установить свойство <code>SelectionMode="Multiple"</code> или 
<code>SelectionMode="Extended"</code>. В последнем случае, чтобы выделить несколько элементов, необходимо держать нажатой клавишу Ctrl или Shift. 
По умолчанию <code>SelectionMode="Single"</code>, то есть допускается только единственное выделение.</p>
...
<div class="nav"><p><a href="4.13.php">Назад</a><a href="./">Содержание</a><a href="11.1.php">Вперед</a></p></div></div>
<div class="socBlock">
