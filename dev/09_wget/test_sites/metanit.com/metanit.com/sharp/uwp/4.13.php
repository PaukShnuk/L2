...
<html >
<head>
<title>UWP | Элементы управления данными</title>
<meta charset="utf-8" />
<meta name="description" content="Элементы управления данными в Universal Windows Platform, их основные свойства и методы, создание список в UWP, установка источника данных">
...
<h1>Работа с данными</h1><h2>Элементы управления данными</h2><div class="date">Последнее обновление: 25.11.2017</div>
...
<p>Для работы с данными в Universal Windows Platform есть целый пласт элементов управления в виде списков. Они являются производными от класса 
<span class="b">Windows.UI.Xaml.Controls.ItemsControl</span>, который в свою очередь является наследником класса Control. Все они содержат 
коллекцию элементов. Это такие элементы управления как ListBox, ComboBox, ListView, GridView, FlipView, AutoSuggestBox.</p>
...
&lt;Page
    ...
    &lt;Grid Background="{ThemeResource ApplicationPageBackgroundThemeBrush}"&gt;
        &lt;ListBox Name="phonesList"&gt;
            ...
            &lt;x:String&gt;LG G 6&lt;/x:String&gt;
            &lt;x:String&gt;Xiaomi Mi&lt;/x:String&gt;
        &lt;/ListBox&gt;
    &lt;/Grid&gt;
&lt;/Page&gt;
</pre>
<img src="pics/4.42.png" alt="Элементы управления данными в UWP" />
<p>Все элементы, размещенные внутри ListBox, да и внутри любого другого спискового элемента, представляют элементы списка.</p>
...
<p>Например, применительно к выше определенному списку мы бы могли написать в коде C#:</p>
<pre class="brush:c#;">
phonesList.Items.Add("HTC U Ultra");
phonesList.Items.RemoveAt(1); // удаляем второй элемент
</pre>
<h3>ItemsSource</h3>
<p>Для списочных элементов необязательно их вручную заполнять объектами. Вместо этого можно воспользоваться свойством <span class="b">ItemsSource</span>, 
задав в его значения коллекцию, из которой будет формироваться список. Например:</p>
<pre class="brush:xml;">
&lt;ListBox Name="phonesList" /&gt;
</pre>
<p>На данный момент это пока пустой список. А в файле отделенного кода выполним наполнение этого списка:</p>
<pre class="brush:c#;">
...
<p>Свойство ItemsSource в качестве значения принимает массив, хотя это может быть и список типа List. И каждый элемент этого массива переходит в ListBox.</p>
...
&lt;Page
    ...
    &lt;Grid Background="{ThemeResource ApplicationPageBackgroundThemeBrush}"&gt;
        &lt;ListBox Name="phonesList" DisplayMemberPath="Title"&gt;
            &lt;local:Phone Title="iPhone 7" Company="Apple" Price="49000" /&gt;
            &lt;local:Phone Title="Samsung Galaxy S8" Company="Samsung" Price="50000" /&gt;
            &lt;local:Phone Title="Huawei P10" Company="Huawei" Price="29990" /&gt;
        &lt;/ListBox&gt;
    &lt;/Grid&gt;
&lt;/Page&gt;
</pre>
<p>Так как здесь используется класс из текущего проекта, то соответственно у нас обязательно должно быть подключено пространство имен проекте: 
<code>xmlns:local="using:DataApp"</code>. В принципе по умолчанию UWP уже его подключает. Кроме того, чтобы не возникало проблем с разметкой XAML, 
желательно сделать перестроение проекта. И в итоге окно нам выведет названия смартфонов:</p>
<img src="pics/4.43.png" alt="Свойство DisplayMemberPath в UWP" />
<p>То же самое мы бы могли сделать программным способом:</p>
<pre class="brush:c#;">
phonesList.ItemsSource = new List&lt;Phone&gt;
{
...
<h3>Выбор элемента в списке</h3>
<p>Все элементы управления списками поддерживают выделение элементов. Выделенный(е) элемент(ы) можно получить с помощью свойств <span class="b">SelectedItem(SelectedItems)</span>, 
а получить индекс выделенного элемента - с помощью свойства <span class="b">SelectedIndex</span>. Свойство <span class="b">SelectedValue</span> 
позволяет получить значение выделенного элемента.</p>
<p>При выделении элемента в списке генерируется событие SelectionChanged, которое мы можем обработать. Например, возьмем предыдущий список:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
    &lt;ListBox Name="phonesList" DisplayMemberPath="Title" SelectionChanged="phonesList_SelectionChanged" &gt;
        &lt;local:Phone Title="iPhone 7" Company="Apple" Price="49000" /&gt;
        &lt;local:Phone Title="Samsung Galaxy S8" Company="Samsung" Price="50000" /&gt;
        &lt;local:Phone Title="Huawei P10" Company="Huawei" Price="29990" /&gt;
    &lt;/ListBox&gt;
...
<p>Необходимо иметь в виду, что так как в списке в xaml определены элементы Phone, то в коде мы можем привести выделенный элемент 
<code>phonesList.SelectedItem</code> к типу Phone.</p>
<p>Если какая-то обработка выбора элемента не требуется, и нам надо посто вывести данные выбранного элемента, то мы можем просто использовать 
мехунизм привязки к свойству SelectedItem:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
	&lt;ListBox Name="phonesList" DisplayMemberPath="Title"&gt;
		&lt;local:Phone Title="iPhone 7" Company="Apple" Price="49000" /&gt;
		...
		&lt;local:Phone Title="Huawei P10" Company="Huawei" Price="29990" /&gt;
	&lt;/ListBox&gt;
	&lt;StackPanel DataContext="{Binding ElementName=phonesList, Path=SelectedItem}"&gt;
...
<img src="pics/11.5.png" alt="Привязка к выбранному элементу в UWP" />
...
<div class="nav"><p><a href="10.4.php">Назад</a><a href="./">Содержание</a><a href="4.14.php">Вперед</a></p></div></div>
<div class="socBlock">
