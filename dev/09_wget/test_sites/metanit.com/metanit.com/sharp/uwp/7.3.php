...
<html >
<head>
<title>UWP | Создание и использование шаблонов</title>
<meta charset="utf-8" />
<meta name="description" content="Шаблоны элементов в UWP, определение и использование шаблонов, свойство Template, элемент TemplateBinding">
...
<h2>Создание и использование шаблонов</h2><div class="date">Последнее обновление: 20.02.2016</div>
...
<p>Визуальные элементы в Universal Windows Platform уже имеют встроенные предопределенные шаблоны, которые определяют визуальное дерево элементов. Но 
...
шаблонов все использующиеся элементы сохраняли стилевое единообразие.</p>
<p>Итак, для примера создадим округлую кнопку:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>В результате у нас получится следующая кнопка:</p>
<img src="pics/7.5.png" alt="ControlTemplate в UWP" />
<p>Шаблоны можно в виде отдельных ресурсов. Так, здесь с помощью элемента <span class="b">ControlTemplate</span> определяется ресурс с ключом 
"btTemplate". В ControlTemplate вложены элементы Border и ContentPresenter, которые с помощью атрибутов определяют внешний вид кнопки.</p>
<h3>TemplateBinding</h3>
<p>Вышеопределенный шаблон устанавливал цвет и ряд других параметров, которые мы не могли бы изменить из кода самой кнопки. Например, если мы установим в кнопке цвет фона, то этот цвет никак не повлияет 
на кнопку:</p>
<pre class="brush:xml;">&lt;Button x:Name="myButton" Template="{StaticResource btTemplate}" Background="Red" HorizontalAlignment="Center"&gt;UWP&lt;/Button&gt;</pre>
<p>Чтобы можно было устанавливать ряд свойств из самого элемента, к которому применяется шаблон, необходимо использовать элемент 
...
<code>Background</code> элемента Button. Таким образом, здесь практически все свойства Border и ContentControl (кроме свойства CornerRadius) 
устанавливаются из разметки элемента Button.</p>
<h3>Свойство Template</h3>
<p>С помощью свойства <span class="b">Template</span> можно определить шаблон напрямую в самом элементе:</p>
<pre class="brush:xml;">
