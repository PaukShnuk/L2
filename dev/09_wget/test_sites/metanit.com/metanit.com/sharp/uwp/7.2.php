...
<html >
<head>
<title>UWP | Шаблоны, визуальное и логическое дерево</title>
<meta charset="utf-8" />
<meta name="description" content="Шаблоны элементов в Universal Windows Platform, логическое и визуальное дерево, элемент ControlTemplate">
...
<h2>Шаблоны, визуальное и логическое дерево</h2><div class="date">Последнее обновление: 20.02.2016</div>
...
<p>Наряду со стилями Universal Windows Platform предоставляет еще оно средство для кастомизации элементов управления - <span class="b">шаблоны (control template)</span>. 
Но чтобы понять, что представляют шаблоны, вначале необходимо ознакомиться с такими понятиями как <span class="b">логическое дерево</span> и <span class="b">визуальное дерево</span>.</p>
<p>Логическое и визуальное деревья являются своего рода каркасом графического интерфейса приложения. Мы можем представить приложение как некий набор 
вложенных элементов. Возьмем, к примеру, следующую простейшую разметку xaml:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Здесь структуру визуальных элементов можно описать следующим образом:</p>
<img src="pics/7.2.png" alt="Логическое дерево в Universal Windows Platform" />
<p>Так, в элементе Page есть вложенный элемент Grid, в Gride - элемент Button, в кнопке в качестве содержимого установлен некоторый текст в виде объекта 
String. И все это находится во фрейме, который устанавливается в качестве содержимого в объекте Window. В итоге получается некое дерево элементов, которое является <span class="b">логическим</span>. Логическое дерево образует модель доступа к дочерним элементам.</p>
<p>От него отличается <span class="b">визуальное дерево</span>, которое для той же разметки XAML будет выглядеть следующим образом:</p>
<img src="pics/7.3.png" alt="Визуальное дерево в Universal Windows Platform" />
<p>Визуальное дерево сложнее по своей структуре, оно показывает, как с визуальной точки зрения устроен элемент, из каких частей он состоит.</p>
...
выбрать <span class="b">Debug -&gt; Windows -&gt; Live Visual Tree</span>. После выбора этого пункта Visual Studio откроет окно со схемой визуального 
дерева, на котором мы можем увидеть, как устроен элемент:</p>
<img src="pics/7.4.png" alt="Визуальное дерево в UWP" />
<p>Визуальное дерево элемента управления определяет шаблон элемента - своего рода визуальный скелет элемента управления. Например, для элемента Button 
упрощенно шаблон выглядит следующим образом:</p>
<pre class="brush:xml;">
...
<p>Полностью встроенный шаблон и стиль кнопки можно найти по ссылке 
<a href="https://msdn.microsoft.com/en-us/library/windows/apps/mt299109.aspx" rel="nofollow">https://msdn.microsoft.com/en-us/library/windows/apps/mt299109.aspx</a></p>
<p>И здесь мы можем увидеть, что сам шаблон задается с помощью элемента <span class="b">ControlTemplate</span>. Атрибут <code>TargetType="Button"</code> как раз 
...
визуальное дерево элемента Button.</p>
