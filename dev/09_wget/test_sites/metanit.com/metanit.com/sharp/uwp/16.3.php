...
<html >
<head>
<title>UWP | Работа со сложными данными SQLite</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с данными со связью "один ко многим" в Universal Windows Platform, чтение и сохранение данных в бд SQLite">
...
<h2>Работа со сложными данными</h2><div class="date">Последнее обновление: 26.11.2017</div>
...
<p>В прошлой теме мы рассмотрели работу с простой моделью Company, объекты которой хранятся в базе данных. Но у нас еще есть другая модель - Phone, которая связана с 
Company отношением "один-ко-многим". Рассмотрим вкратце, как мы можем работать и с этой моделью.</p>
<p>Для работы с моделью Phone добавим в проект новую страницу. Назовем ее <span class="b">PhonesListPage.xaml</span> и установим у нее следующее содержимое:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Сначала на странице идет набор полей для добавления нового объекта Phone. И под ними располагается элемент ListView, который выводит все добавленные объекты. 
Для упрощения примера добавление и вывод происходят на одной и той же страницу.</p>
<p>Также изменим код C# у страницы, чтобы добавлять объекты Phone и выводить их в списке:</p>
<pre class="brush:c#;">
using System;
...
<p>При загрузке страницы мы получаем два списка - список телефонов для их отображения в ListView и список компаний для выпадающего списка ComboBox, чтобы при добавлении 
пользователь мог выбрать компанию.</p>
<p>В обработчике кнопки добавления создаем новый объект Phone по введенным в поля данным и добавляем его в базу данных. В общем-то тут добавление проходит также, 
как и добавление обычной модели. Только в данном случае нам надо еще установить внешний ключ на связанную модель Company с помощью свойства CompanyId.</p>
<p>В итоге страница будет выглядеть следующим образом:</p>
<img src="pics/17.9.png" alt="Связь один ко многим в Entity Framework и UWP" />
<p>И чтобы эту страницу можно было вызвать с главной, добавим в <span class="ii">MainPage.xaml</span> еще одну кнопку:</p>
<pre class="brush:xml;">
&lt;Page.BottomAppBar&gt;
    &lt;CommandBar&gt;
...
<p>И определим для этой кнопки обработчик:</p>
<pre class="brush:c#;">
