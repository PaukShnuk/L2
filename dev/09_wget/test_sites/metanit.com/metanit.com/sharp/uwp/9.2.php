...
<html >
<head>
<title>UWP | История навигации</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с историей навигации в Universal Windows Platform, свойство Frame.BackStack, переход вперед и назад по журналу навигации, методы Frame.GoForward и Frame.GoBack, ">
...
<h2>История навигации</h2><div class="date">Последнее обновление: 13.02.2016</div>
...
<p>Объект Frame хранит историю навигации, благодаря чему поддерживает возврат на предыдущую страницу с помощью метода <span class="b">Frame.GoBack()</span>, 
либо переход на следующую в истории переходов страницу посредством метода <span class="b">Frame.GoForward()</span>.</p>
<p>Для практики с историей навигации продолжим работу с проектом из прошлой темы и добавим в него новую страницу по типу <span class="b">Blank Page</span>, 
которую назовем Page3 (либо создадим новый проект, в котором будут три страницы MainPage, Page2 и Page3).</p>
<img src="pics/9.4.png" alt="Страницы в UWP" />
<p>Итак, пусть страница MainPage содержит кнопку:</p>
<pre class="brush:xml;">&lt;Button Content="Вперед" Click="Forward_Click" Margin="10" /&gt;</pre>
...
<p>Таким образом, если в истории переходов уже есть страница, на которую переходили с MainPage (то есть свойство Frame.CanGoForward равно true), 
то сработает переход к этой странице с помощью Frame.GoForward(). Иначе произойдет переход к странице Page2.</p>
...
<p>И также добавим обработчик для этой кнопки в файл <i>Page3.xaml.cs</i>: </p>
<pre class="brush:c#;">
private void Back_Click(object sender, RoutedEventArgs e)
{
    if (Frame.CanGoBack)
...
<p>И после этого мы сможем переходить от одной страницы к другой.</p>
<img src="pics/9.5.png" alt="Навигация в UWP" />
<p>При переходе от одной страницы к другой с помощью метода <span class="b">Navigate()</span> в специальный стек в объекте Frame поверх остальных добавляется информация о страницах. 
Этот стек мы можем получить с помощью свойства <span class="b">Frame.BackStack</span>. Так, в нашем случае при цепи переходов MainPage-&gt;Page2-&gt;Page3, 
...
метода <span class="b">Frame.GoBack()</span> система извлекает из стека первую страницу и осуществляет к ней переход.</p>
<p>Мы можем вручную управлять стеком, например, изменим метод кнопки на странице Page3 таким образом, чтобы переход шел к самой первой странице в стеке:</p>
<pre class="brush:c#;">
private void Back_Click(object sender, RoutedEventArgs e)
{
...
<p>Каждый объект в стеке представляет тип PageStackEntry, из которого мы можем получить данные о странице. То есть в данном случае переход будет идти обратно к MainPage, 
...
переходе к MainPage в стек будет добавлена информация о странице Page3.</p>
