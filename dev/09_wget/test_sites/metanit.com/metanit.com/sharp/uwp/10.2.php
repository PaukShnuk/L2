...
<html >
<head>
<title>UWP | Адаптивные триггеры и визуальные состояния</title>
<meta charset="utf-8" />
<meta name="description" content="Visual States в Universal Windows Platform, настройка свойств элементов для различной ширины экрана с помощью элементов AdaptiveTrigger">
...
<h2>Адаптивные триггеры и визуальные состояния</h2><div class="date">Последнее обновление: 20.02.2016</div>
...
<p>В прошлой теме была рассмотрена адаптация элементов управления к изменениям ширины экрана на основе обработки события SizeChanged. Но Universal 
Windows Platform также предоставляет и другое более эффективное решение - <span class="b">Visual States</span> или <span class="b">визуальные состояния</span>.</p>
<p>Элементы визуальных состояний используются для представления внешнего вида элементов интерфейса, когда они находятся в каком-то определенном состоянии. 
Для управления визуальными состояниями применяется объект <span class="b">VisualStateManager</span>. Для примера возьмем код приложения из прошлой темы и применим к нему визуальные состояния:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Все визуальные состояния определяются в корневом элементе компоновки, в данном случае в StackPanel. Для определения визуальных состояний используется 
свойство <span class="b">VisualStateManager.VisualStateGroups</span>. В него помещается элемент <span class="b">VisualStateGroup</span>, который представляет 
набор состояний. Каждое визуальное состояние задается с помощью объекта <span class="b">VisualState</span>. Он имеет имя. В данном случае у нас два состояния: 
Mobile и Desktop.</p>
<p>Для каждого состояния определяется набор адаптивных триггеров с помощью коллекции <span class="b">VisualState.StateTriggers</span> и объектов 
<span class="b">AdaptiveTrigger</span>. Адаптивные триггеры задают условие, при достижении которого приложение переходит в данное визуальное состояние. 
По большому счету мы можем задать только два условия - минимальную ширину окна приложения через свойство <span class="b">MinWindowWidth</span> и и минимальную высоту 
окна с помощью свойства <span class="b">MinWindowHeight</span>.</p>
<p>В данном случае приложение попадает в первое визуальное состояние Desktop, когда ширина окна от 500 единиц и больше, а во второе состояние - когда ширина окна больше 0.</p>
<p>И также каждый объект VisualState содержит коллекцию <span class="b">VisualState.Setters</span>. Каждый объект Setter в этой коллекции устанавливает значение для какого-то 
определенного свойства элемента. Само свойство указывается через атрибут <span class="b">Target</span>, например, <code>Target="buttonStack.Orientation"</code>. Для ссылки на элемент используется 
его имя buttonStack.</p>
<p>Теперь в файле кода <i>MainPage.xaml.cs</i> нам не надо устанавливать обработчик события SizeChanged или писать какую-то другую логику. Все будет делаться автоматически 
при изменении размеров окна приложения.</p>
<img src="pics/10.3.png" alt="Адаптивный интерфейс в Universal Windows Platform" />
