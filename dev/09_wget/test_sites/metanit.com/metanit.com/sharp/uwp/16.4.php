...
<html >
<head>
<title>UWP | Использование существующей БД SQLite</title>
<meta charset="utf-8" />
<meta name="description" content="Использование существующей БД SQLite в Universal Windows Platform для работы с данными, особенности применения">
...
<h2>Использование существующей БД SQLite</h2><div class="date">Последнее обновление: 14.03.2016</div>
...
<p>В предыдущих темах мы рассмотрели процесс работы с базой данных SQLite, которая создается при первом запуске приложения. 
Теперь рассмотрим ситуацию, если у нас уже есть некоторая база данных, в которой имеются начальные данные, и нам эту базу данных надо подключить к проекту.</p>
<p>Продолжим работу с проектом из прошлой темы и вначале создадим саму базу данных SQLite. Для этого воспользуемся специальной программой <span class="b">Sqlitebrowser</span>, которая предоставляет графический 
интерфейс для работы с базой данных SQLite. Эту программу можно найти по адресу <a href="http://sqlitebrowser.org" rel="nofollow">http://sqlitebrowser.org</a>.</p>
<p>После установки Sqlitebrowser откроем его и нажмем в меню на кнопку <span class="b">New Database</span>:</p>
<img src="pics/17.10.png" alt="Sqlitebrowser и Universal Windows Platform" />
<p>Определим для базы данных место и имя. Пусть она будет называться <span class="b">phones.db</span>.</p>
<p>После этого Sqlitebrowser открывает новую базу данных. В ней еще ничего нет, никаких таблиц. Поэтому перейдем на вкладку <span class="b">Execute SQL</span> и введем на ней 
в поле ввода следующие sql-выражения:</p>
<pre class="brush:sql;">
CREATE TABLE Phones(
...
<img src="pics/17.11.png" alt="Создание таблиц в Sqlitebrowser" />
<p>Эти sql-выражения позволят создать две таблицы: Company и Phone, которые будут связаны внешним ключом.</p>
<p>После ввода sql-выражений нажмем на панели на кнопку "Execute" или на клавишу F5 на клавиатуре, чтобы выполнить sql-выражения. 
И после этого будут созданы необходимые таблицы. Выберем таблицу Company и добавим в нее какие-нибудь данные:</p>
<img src="pics/17.12.png" />
<p>Также добавим начальные данные и в таблицу Phone:</p>
<img src="pics/17.13.png" />
<p>Итак, база данных у нас уже имеется. Теперь добавим ее в проект и установим у нее в панели свойств для параметра <span class="b">Build Action</span> 
значение <span class="b">Content</span>:</p>
<img src="pics/17.14.png" alt="Добавление базы данных в UWP" />
<p>Теперь изменим класс контекста данных MobileContext, чтобы он использовал эту бд:</p>
<pre class="brush:c#;">
...
<p>База данных будет устанавливаться вместе с приложением в папку . Однако базы данных приложение берет из папки ApplicationData.Current.LocalFolder. Поэтому 
при первом запуске приложения нам надо выполнить копирование базы данных в ApplicationData.Current.LocalFolder. Для этого изменим класс App:</p>
<pre class="brush:c#;">
using System;
...
<p>Весь остальной код с прошлой темы можно не изменять. И при запуске приложения мы увидим на главной странице наши начальные данные:</p>
<img src="pics/17.15.png" />
...
<div class="nav"><p><a href="16.3.php">Назад</a><a href="./">Содержание</a><a href="17.1.php">Вперед</a></p></div></div>
<div class="socBlock">
