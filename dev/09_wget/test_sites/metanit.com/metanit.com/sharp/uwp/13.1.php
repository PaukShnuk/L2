...
<html >
<head>
<title>UWP | Работа с мультимедиа</title>
<meta charset="utf-8" />
<meta name="description" content="Работа со звуком и аудиофайлами в Universal Windows Platform, управление воспроизведением аудио">
...
<h1>Работа с мультимедиа</h1><h2>Аудио</h2><div class="date">Последнее обновление: 16.04.2017</div>
...
<p>Universal Windows Platform имеет широкие возможности для работы с аудио, со звуками, с их обработкой. Для воспроизведения аудиофайлов мы можем использовать 
в рамках UWP такой элемент как <span class="b">MediaElement</span>.</p>
<p>MediaElement представляет простейший элемент для воспроизведения аудио. С помощью свойства <span class="b">Source</span> или метода <span class="b">SetSource()</span> (программным способом) 
можно установить источник аудио.</p>
<p>К примеру, создадим в проекте новую папку для хранения аудиофайлов, которую назовем <i>Audio</i>, и добавим в нее какой-нибудь аудиофайл:</p>
<img src="pics/15.1.png" alt="Работа со звуком в UWP" />
<p>В данном случае аудиофайл называется "beethoven_fur_elise.mp3". Тогда, чтобы его воспроизвести, я могу использовать следующую разметку:</p>
<pre class="brush:xml;">&lt;MediaElement x:Name="media" Source="ms-appx:///audio/beethoven_fur_elise.mp3" AutoPlay="True" /&gt;</pre>
<p>Для проигрывания файлов из проекта применяется Uri в формате "ms-appx:///путь к файлу", либо просто путь без префикса, например, "audio/beethoven_fur_elise.mp3". 
Также, мы можем в качестве источника указывать интернет-ресурсы.</p>
<p>Установка источника программным способом:</p>
<pre class="brush:c#;">
using System;
...
<p>Элемент MediaElement имеет множество свойств, которые позволяют настраивать и управлять воспроизведением, но среди них прежде всего можно выделить следующие:</p>
<ul>
<li><p><span class="b">AutoPlay</span>: при значении true аудио начнет автоматически проигрываться после загрузки страницы</p></li>
<li><p><span class="b">IsMuted</span>: при значении true уменьшает громкость о нуля</p></li>
<li><p><span class="b">IsLooping</span>: при значении true композиция повторяется после завершения</p></li>
<li><p><span class="b">Volume</span>: уровень громкости в виде значения double</p></li>
<li><p><span class="b">AreTransportControlsEnabled</span>: при значении true отображаются элементы управления воспроизведением</p></li>
</ul>
<p>Так, чтобы добавить стандартные элементы управления воспроизведения, достаточно использовать свойство <code>AreTransportControlsEnabled</code>:</p>
<pre class="brush:xml;">&lt;MediaElement x:Name="media" Source="audio\\beethoven_fur_elise.mp3" AreTransportControlsEnabled="True" /&gt;</pre>
<p>Тогда элемент будет выглядеть следующим образом:</p>
<img src="pics/15.2.png" alt="Управление MediaElement в UWP" />
<p>Однако, если встроенные элементы нас не устраивают, тогда мы можем сами создать свои компоненты управления, используя методы класса MediaElement:</p>
<ul>
<li><p><span class="b">Play()</span>: запускает воспроизведение</p></li>
<li><p><span class="b">Pause()</span>: приостанавливание воспроизведение</p></li>
<li><p><span class="b">Stop()</span>: полностью останавливает проигрывание</p></li>
</ul>
<p>При работе с MediaElement мы также можем применять его события для отслеживания состояния элемента. Некоторые из событий:</p>
<ul>
<li><p><span class="b">MediaOpened</span>: срабатывает при начале воспроизведения</p></li>
<li><p><span class="b">MediaFailed</span>: генерируется при ошибке открытия файла</p></li>
<li><p><span class="b">MediaEnded</span>: срабатывает при завершении воспроизведения</p></li>
</ul>
<p>Используем обычные кнопки для управления воспроизведения аудио:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>В файле кода C# пропишем обработчики всех событий:</p>
<pre class="brush:c#;">
using Windows.UI.Xaml;
using Windows.UI.Xaml.Controls;
...
<img src="pics/15.3.png" alt="Воспроизведение аудио на Universal Windows Platform" />
...
<div class="nav"><p><a href="14.2.php">Назад</a><a href="./">Содержание</a><a href="13.2.php">Вперед</a></p></div></div>
<div class="socBlock">
