...
<html >
<head>
<title>UWP | Жизненный цикл приложения</title>
<meta charset="utf-8" />
<meta name="description" content="Жизненный цикл приложения в Universal Windows Platform, основные стадии, события жизненного цикла и их обработка">
...
<h1>Приложение и его жизненный цикл</h1><h2>Жизненный цикл приложения</h2><div class="date">Последнее обновление: 11.02.2016</div>
...
<p>Работа приложения UWP  начинается с класса App, который определен в файлах <span class="ii">App.xaml</span> и <span class="ii">App.xaml.cs</span>. Класс App является входной точкой в 
...
исключения, которые могут возникнут в процессе работы. Фактически класс App и представляет приложение.</p>
<p>Приложение может пребывать в пяти различных состояниях:<p>
<ul>
<li><p><span class="b">NotRunning</span>: это состояние возникает, когда пользователь впервые активирует приложение при его установки из магазина, либо когда пользователь закрывает приложение через диспетчер задач, либо завершает его работу, 
выключая компьютер или осуществляя выход из системы</p></li>
<li><p><span class="b">Running</span>: приложение запущено, и пользователь с ним взаимодействует</p></li>
<li><p><span class="b">Suspended</span>: работа приложения приостановлена, оно не активно</p></li>
<li><p><span class="b">Terminated</span>: работа приложения завершается. Переход в это состояние может быть произведен ОС Windows, если к примеру, 
приложение уже находится в приостановленном состоянии Suspended, но в связи с нехваткой памяти для других приложений оно переходится в состояние Terminated</p></li>
<li><p><span class="b">ClosedByUser</span>: состояние возникает при закрытии пользователем приложения на крестик или по нажатию клавиш Alt+F4</p></li>
</ul>
<p>Переходы из одного состояния в другое можно описать следующей схемой:</p>
<img src="pics/8.2.png" alt="Жизненный цикл приложения Universal Windows Platform" />
<p>Первоначально приложение находится в состоянии <span class="b">NotRunning</span>, оно не работает, и пользователь с ним не взаимодействует. Но вот пользователь 
...
Вначале в этом классе вызывается метод <span class="b">OnLaunched()</span>. Если приложение ранее было приостановлено, но пользователь вернулся к нему, 
то у него также вызывается метод <code>OnLaunched()</code>. Стандартное определение метода OnLaunched:</p>
<pre class="brush:c#;">
...
<p>При вызове метода ему передается параметр типа <span class="b">LaunchActivatedEventArgs</span>, который содержит информацию 
...
свойство <span class="b">PreviousExecutionState</span> этого параметра имеет значения <code>ApplicationExecutionState.Terminated</code> или <code>ApplicationExecutionState.ClosedByUser</code>. 
А свойство <span class="b">Kind</span> имеет значение <code>ActivationKind.Launch</code>.</p>
<p>Если предыдущее состояние равно <code>ApplicationExecutionState.Terminated</code>, то, возможно, имеет смысл загрузить ранее сохраненные данные. Собственно поэтому по умолчанию 
в код метода OnLaunched добавлена следующая конструкция:</p>
<pre class="brush:c#;">
if (e.PreviousExecutionState == ApplicationExecutionState.Terminated)
{
    //TODO: Load state from previously suspended application
...
<p>Если же предыдущее состояние приложения было <code>NotRunning</code>, то приложение фактически стартует с чистого листа. То есть разработчик может быть уверен, что 
предыдущий запуск приложения не завершился прерыванием работы со стороны операционной системы.</p>
<p>После того, как сработает метод OnLaunched приложение начинает отображаться на экране, и оно переходит в состояние <span class="b">Running</span>.</p>
<p>Сразу после того, как приложение перейдет в состояние Running, срабатывает метод <span class="b">OnActivated()</span>. По умолчанию в классе App этот 
метод не используется, но мы его можем переопределить. Например:</p>
<pre class="brush:c#;">
...
<p>Здесь опять же мы можем получить из аргумента метода предыдущее состояние. Если оно представляет Terminated, то, к примеру, загрузить ранее сохраненные данные, 
либо выполнить какую-то другую работу.</p>
<h3>Приостановка приложения</h3>
<p>Когда пользователь переключается на другое приложение, либо на стартовый экран, происходит приостановка приложения. Если пользователь после этого 
...
система ждет несколько секунд. И если пользователь в течение этого времени не вернулся в приложение, то оно переходит в состояние Suspended.</p>
<p>Все данные, которые были в приложении до приостановки, сохраняются системой и 
...
Поэтому имеет смысл сохранять критические данные в памяти устройства, чтобы после возобновления работы восстановить их в методе OnLaunched или OnActivated.</p>
<p>При переходе приложения в состояние Suspended генерируется событие <span class="b">Suspending</span>. И если мы посмотрим на определение класса App, то увидим в нем обработчик, который вызывается при переходе приложения в состояние Suspended:</p>
<pre class="brush:c#;">
...
<p>Как правило, в обработчике OnSuspending происходит сохранение важных данных приложения, что может спасти их от потери в случае перевода приложения в 
...
то система сочтет, что приложение перестало отвечать на запросы и должно быть переведено в состояние Terminated.</p>
<p>В начале в обработчике следует получить объект deferral: <code>var deferral = e.SuspendingOperation.GetDeferral();</code>. Этот объект позволит системе дать приложению время 
на выполнение асинхроннных операций.</p>
<p>После выполнения всех операций мы помечаем полученную задачу как завершенную с помощью вызова <code>deferral.Complete()</code></p>
<h3>Возобновление работы</h3>
<p>Когда пользователь возвращается к ранее приостановленному приложению, оно вновь переходит в состояние Running. При этом генерируется событие 
<span class="b">Resuming</span>. И если нам надо при этом выполнить некоторую логику, то мы можем обработать это событие:</p>
<pre class="brush:c#;">
sealed partial class App : Application
{
    public App()
...
<p>После окончания работы с приложением пользователь может сам закрыть приложение с помощью крестика или по нажатию на комбинацию клавиш ALT+F4. 
...
<span class="b">Suspending</span>, которые мы можем обработать, например, чтобы сохранить состояние приложения.</p>
