...
<html >
<head>
<title>UWP | Адаптивные триггеры и шаблоны данных</title>
<meta charset="utf-8" />
<meta name="description" content="Адаптация шаблонов данных в Universal Windows Platform под различные расширения экранов, определение своих элементов для хранения наборов данных">
...
<h2>Адаптивные триггеры и шаблоны данных</h2><div class="date">Последнее обновление: 25.11.2017</div>
...
<p>Шаблон данных, описываемый элементом DataTemplate, имеет одно важное ограничение - мы не можем применять визуальные состояния Visual State к элементам, 
...
данные в зависимости от ширины экрана.</p>
<p>К счастью, эта проблема легко решается с помощью создания своего визуального компонента, в который выносится шаблон данных. Возьмем проект из прошлой темы, 
где у нас был определен класс Phone:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Добавим в этот проект новый элемент по типу <span class="b">User Control</span>, который назовем PhoneTemplate:</p>
<img src="pics/11.4.png" alt="DataTemplate и UserControl в UWP" />
<p>Определим в коде <span class="b">PhoneTemplate.xaml</span> следующую разметку:</p>
<pre class="brush:xml;">
&lt;UserControl
...
<p>Фактически мы определяем в xaml нашего компонента весь тот код, который представляет DataTemplate или шаблон отображения одного элемента Phone. 
И также здесь определены визуальные состояния для окна приложения шириной до 600 единиц и больше 600 единиц.</p>
<p>Кроме того, здесь привязка элементов идет к свойствам объекта Phone, который надо еще определить. Для этого перейдем к коду c# нашего компонента - файлу 
<span class="ii">PhoneTemplate.xaml.cs</span> и определим в нем следующее содержимое:</p>
<pre class="brush:c#;">
...
<p>Во-первых, здесь определяется свойство Phone, которое представляет объект <code>Phone</code>. Причем данные для этого свойства извлекаются из контекста данных DataContext. 
Контекст данных будет устанавливаться из MainPage.</p>
<p>Теперь применим наш компонент PhoneTemplate в коде главной страницы. Для этого изменим MainPage.xaml следующим образом:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Передаваемый в DataTemplate объект Phone как и будет представлять контекст данных для компонента PhoneTemplate.</p>
<p>Код MainPage.xaml.cs остается прежним:</p>
<pre class="brush:c#;">
...
<p>В итоге отображение списка объектов будет изменяться в зависимости от ширины экрана.</p>
...
<div class="nav"><p><a href="11.2.php">Назад</a><a href="./">Содержание</a><a href="4.15.php">Вперед</a></p></div></div>
<div class="socBlock">
