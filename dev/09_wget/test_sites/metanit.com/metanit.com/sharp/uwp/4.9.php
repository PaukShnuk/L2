...
<html >
<head>
<title>UWP | Flyout</title>
<meta charset="utf-8" />
<meta name="description" content="Элемент Flyout в Universal Windows Platform, его основные свойства и методы, создание всплывающих окон, их кастомизация в UWP">
...
<h2>Flyout</h2><div class="date">Последнее обновление: 11.02.2016</div>
...
<p>Элемент <span class="b">Flyout</span> аналогичен контекстным меню, всплывающим подсказкам или диалоговым окнам. Данный элемент используется вместе с другими элементами, для которых надо создать контекстное меню. 
Так, для всех кнопок его можно прикрепить через свойство <code>Button.Flyout</code>, а для всех остальных элементов используется свойство 
<code>FlyoutBase.AttachedFlyout</code>.</p>
<p>Если к кнопке прикрепить Flyout, то он отобразится после нажатия на кнопку. Например:</p>
<pre class="brush:xml;">
&lt;Button Content="Жми"&gt;
    &lt;Button.Flyout&gt;
...
<p>Здесь Flyout фактически представляет диалоговое окно с текстовой надписью и копкой. В коде C# мы можем задать обработчик для кнопки такого диалогового окна:</p>
<pre class="brush:c#;">
...
<p>В данном случае просто закрываем окно. В итоге при нажатии на копку нам отобразится следующее диалоговое окно:</p>
<img src="pics/4.30.png" alt="Flyout в Universal Windows Platform" />
<p>Выше мы рассмотрели установку Flyout для кнопок, однако для других элементов управления все будет немного по другому. Так, Flyout надо программно отображать 
(сам по себе он не отобразится) с помощью метода <code>ShowAt()</code> или <code>ShowAttachedFlyout</code>. А для прикрепления Flyout к другим элементам управления, отличным от кнопок, применяется свойство <span class="b">FlyoutBase.AttachedFlyout</span></p>
<p>Посмотрим на примере TextBlock:</p>
<pre class="brush:xml;">
...
<p>И в файле кода зададим обработчики, которые будут управлять поведением Flyout:</p>
<pre class="brush:c#;">
...
<img src="pics/4.32.png" alt="Flyout в Universal Windows Platform" />
<h3>MenuFlyout</h3>
<p>На основе Flyout существует несколько дополнительных элементов управления: <span class="b">DatePickerFlyout</span> (выпадающее окно для выбора даты), 
<span class="b">TimePickerFlyout</span> и <span class="b">MenuFlyout</span> (контекстное меню).</p>
<p>Применяются они аналогично. Например, используем MenuFlyout:</p>
<pre class="brush:xml;">
&lt;Button Content="Жми"&gt;
    &lt;Button.Flyout&gt;
...
<p>Внутрь MenuFlyout добавляются отдельные пункты меню. Если нам нужен одиночный пункт меню, то применяется элемент <span class="b">MenuFlyoutItem</span>. 
Если мы хотим создать иерархической меню, в котором есть подменю, то используется элемент <span class="b">MenuFlyoutSubItem</span>, в который добавляются 
элементы подменю.</p>
<p>Для красоты можно использовать элемент-разграничитель в виде черты - <span class="b">MenuFlyoutSeparator</span>. И кроме того, если нам нужен пункт меню, 
который может находиться в отмеченном и неотмеченном состоянии, то мы можем воспользоваться элементом <span class="b">ToggleMenuFlyoutItem</span>.</p>
<p>К каждому пункту меню можно прикрепить обработчики событий. Так, в примере выше для последнего пункта задан обработчик события нажатия. Определим его 
в файле кода c#:</p>
<pre class="brush:c#;">
private void Exit_Click(object sender, RoutedEventArgs e)
{
...
<p>В итоге при нажатии на кнопку нам отобразится вот такое контекстное меню:</p>
<img src="pics/4.31.png" alt="Контекстное меню и MenuFlyout в UWP" />
<p>Среди собственной функциональности MenuFlyout следует отметить разве что свойство <span class="b">Placement</span>, которое задает положение контекстного меню и 
может принимать следующие значения:</p>
<ul>
<li><p><span class="b">Bottom</span>: расположение под кнопкой</p></li>
<li><p><span class="b">Fill</span>: меню растягивается на полную ширину</p></li>
<li><p><span class="b">Top</span>: расположение над кнопкой</p></li>
<li><p><span class="b">Left</span>: расположение слева</p></li>
<li><p><span class="b">Right</span>: расположение справа</p></li>
</ul>
...
<div class="nav"><p><a href="4.8.php">Назад</a><a href="./">Содержание</a><a href="4.10.php">Вперед</a></p></div></div>
<div class="socBlock">
