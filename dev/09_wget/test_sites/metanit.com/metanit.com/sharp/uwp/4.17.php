...
<html >
<head>
<title>UWP | GridView</title>
<meta charset="utf-8" />
<meta name="description" content="Элемент GridView в Universal Windows Platform, его основные свойства и методы, создание табличных элементов в UWP">
...
<h2>GridView</h2><div class="date">Последнее обновление: 14.04.2017</div>
...
<p>GridView также, как и ListView, создает отображение данных в виде строк и столбцов. Единственное отличие GridView от ListView состоит в том, 
что GridView использует в качестве панели для расположения элементов класс ItemsWrapGrid и поэтому по умолчанию может создавать дополнительные колонки для отображения данных.</p>
<p>Рассмотрим на примере. Возьмем для представления данных тот же класс Phone:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>И в xaml определим GridView:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Определение GridView также похоже на ListView: здесь мы также должны задать свойство <code>ItemTemplate</code>, установив для него шаблон отображения данных. В данном 
случае опять же задаем объект <span class="b">DataTemplate</span>. В нем определяем грид с тремя столбцами. И в каждом столбце текстовый блок, привязанный к определенному 
свойству объекта Phone с помощью выражения <code>{Binding навание_свойства}</code>. А сами элементы также задаются с помощью свойства <span class="b">Items</span>.</p>
<p>Выделение элемента в GridView порождает событие <span class="b">ItemClick</span> (если, конечно, установлено свойство <code>IsItemClickEnabled="True"</code>). 
Для обработки этого события определим в файле кода обработчик:</p>
<pre class="brush:c#;">
...
<p>Через свойство аргумента <code>e.ClickedItem</code> мы можем получить выбранный в GridView элемент. А так как GridView содержит объекты Phone, то мы можем привести 
выбранный элемент к типу Phone. Ну и далее название смартфона устанавливается в качестве заголовка для GridView.</p>
<img src="pics/4.51.png" alt="GridView в Universal Windows Platform" />
