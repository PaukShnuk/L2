...
<html >
<head>
<title>UWP | SplitView</title>
<meta charset="utf-8" />
<meta name="description" content="Контейнер SplitView в UWP (Universal Windows Platform), открытие и скрытие боковых панелей">
...
<h2>SplitView</h2><div class="date">Последнее обновление: 11.02.2016</div>
...
<p>Элемент <span class="b">SplitView</span> представляет контейнер, который состоит из двух панелей: Pane и Content. Панель Content представляет собой основное содержимое 
контейнера. Она является открытой. А панель Pane может быть скрыта, при необходимости ее можно раскрывать во всю ширину и скрывать обратно:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>С помощью свойств <code>SplitView.Pane</code> и <code>SplitView.Content</code> устанавливается содержимое панелей Pane и Content. Через свойство 
<code>PaneBackground</code> задается цвет панели Pane (стандартное свойство Background относится к панели Content). Чтобы разместить на этих панелях какое-либо содержимое, 
...
<code>Click="Button_Click"</code>. Поэтому перейдем к файлу кода и поместим в него следующий метод:</p>
<pre class="brush:c#;">
...
<p>Свойство <span class="b">IsPaneOpen</span> элемента SplitView управляет видимостью панели Pane. Если оно равно true, то панель Pane находится в раскрытом состоянии. 
И если нам вдруг потребуется, чтобы эта панель была по умолчанию раскрыта, то в разметке XAML у SplitView мы могли бы использовать атрибут <code>IsPaneOpen="True"</code>.</p>
<p>Теперь если мы запустим приложения, то мы увидим только содержимое панели Content:</p>
<img src="pics/3.11.png" alt="Элемент SplitView в Universal Windows Platform" />
<p>Нажмем на кнопку. У нас сработает обработчик, который переключит состояние панели Pane и тем самым раскроет ее:</p>
<img src="pics/3.12.png" />
<p>Обратите внимание, что панель Pane частично перекрывает панель Content и кнопку. Это поведение панели Pane по умолчанию, но его можно переопределить, 
установив у SplitView нужное значение для атрибута <span class="b">DisplayMode</span>:</p>
<ul>
<li><p><code>Overlay</code>: панель Pane перекрывает панель Content (по умолчанию). Закрыть панель Pane можно, нажав на любое место на панели Content</p></li>
<li><p><code>Inline</code>: панель Pane сдвигает панель Content. При этом панель Pane нельзя закрыть, нажав на любое место на панели Content, как в случае с Overlay</p></li>
<li><p><code>CompactOverlay</code>: начальная ширина панели Pane задается с помощью свойства <code>CompactPaneLength</code>, а остальная часть скрывается. 
При раскрытии панель Pane действует также, как и при режиме Overlay</p></li>
<li><p><code>CompactInline</code>: начальная ширина панели Pane задается с помощью свойства <code>CompactPaneLength</code>, а остальная часть скрывается. 
При раскрытии панель Pane действует также, как и при режиме Inline</p></li>
</ul>
<p>Например, используем режим <code>CompactInline</code>:</p>
<pre class="brush:xml;">
...
<p>В скрытом состоянии:</p>
<img src="pics/3.13.png" />
<p>В раскрытом:</p>
<img src="pics/3.14.png" />
<p>Из остальных свойств SplitView следует отметить свойство <span class="b">OpenPaneLength</span>, которое задает ширину панели Pane в раскрытом состоянии.</p>
