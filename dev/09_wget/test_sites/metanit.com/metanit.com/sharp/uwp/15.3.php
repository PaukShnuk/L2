...
<html >
<head>
<title>UWP | Триггеры и условия</title>
<meta charset="utf-8" />
<meta name="description" content="Триггеры в Universal Windows Platform, запуск фоновых задач с помощью триггеров, классы TimeTrigger и SystemTrigger, определение условий для запуска задач">
...
<h2>Триггеры и условия</h2><div class="date">Последнее обновление: 02.03.2016</div>
...
<p>В прошлой теме была продемонстрирована простейшая задача, которая запускалась с помощью простейшего триггера - ApplicationTrigger. 
...
являются TimeTrigger и SystemTrigger.</p>
<p><code>TimeTrigger</code> позволяет запускать задачу периодически через определенный интервал.</p>
<p><code>SystemTrigger</code> запускает задачу в ответ на определенные системные события.</p></li>
<p>Общая форма использования всех триггеров будет аналогична. Например, применим TimeTrigger:</p>
<pre class="brush:c#;">
TimeTrigger timer = new TimeTrigger(15, false);
taskBuilder.SetTrigger(appTrigger);
</pre>
<p>Число 15 указывает количество минут, через которые будет запускаться задача. Меньше 15 минут установить нельзя. И при использовании этого триггера 
также надо будет указать в качестве типа задачи не <code>General</code>, а <code>Timer</code>.</p>
<p>Либо мы можем создать задачу, которая бы запускалась в случае каких-то системных событий. Например, при появлении подключения к интернету. Для этого используется 
триггер <span class="b">SystemTrigger</span>:</p>
<pre class="brush:c#;">
SystemTrigger trigger = new SystemTrigger(SystemTriggerType.InternetAvailable, false);
taskBuilder.SetTrigger(appTrigger);
</pre>
<p>А в качестве типа задачи в манифесте следует указать тип <code>System event</code></p>
<p>Метод <code>RequestAsync()</code> для запуска задачи в этом случае вызывать не надо, так как задача вызывается при генерации системных событий, а именно - появлении подключения к интернету.</p>
<p>Список возможных системных событий, которые мы можем использовать в системном триггере:</p>
<ul>
<li><p><code>SmsReceived</code>: задача запускается при получении смс</p></li>
<li><p><code>UserPresent</code>: задача запускается, когда пользователь становится активен на текущем устройстве</p></li>
<li><p><code>UserAway</code>: задача запускается, когда пользователь становится не активен</p></li>
<li><p><code>NetworkStateChange</code>: задача запускается, когда меняется состояние сети</p></li>
<li><p><code>InternetAvailable</code>: задача запускается, когда становится доступным подключение к интернету</p></li>
<li><p><code>SessionConnected</code>: задача запускается, когда пользователь входит в систему</p></li>
<li><p><code>ServicingComplete</code>: задача запускается, когда система заканчивает обновление приложения</p></li>
<li><p><code>LockScreenApplicationAdded</code>: задача запускается, когда на экран блокировки добавляется плитка приложения</p></li>
<li><p><code>LockScreenApplicationRemoved</code>: задача запускается, когда с экрана блокировки удаляется плитка приложения</p></li>
<li><p><code>TimeZoneChange</code>: запускается при смене часового пояса</p></li>
<li><p><code>OnlineIdConnectedStateChange</code>: запускается, когда аккаунт Microsoft добавляется к аккаунтам на устройстве</p></li>
</ul>
<h3>Использование условий</h3>
<p>Дополнительно к настройкам триггеров мы можем задать ряд условий для запуска задачи. Например, необходимо, чтобы триггер таймера запускал задачу только в случае наличия 
...
<p>Для определения условий применяется метод <code>AddCondition()</code> объекта BackgroundTaskBuilder, а само условие предоставляет одно из значений перечисления 
<span class="b">SystemConditionType</span>. Доступные значения:</p>
<ul>
<li><p><code>UserPresent</code>: пользователь работает с устройством</p></li>
<li><p><code>UserNotPresent</code>: пользователь не активен</p></li>
<li><p><code>InternetAvailable</code>: наличие соединения к интернету</p></li>
<li><p><code>InternetNotAvailable</code>: отсутствие наличия подключения</p></li>
<li><p><code>SessionConnected</code>: пользователь осуществил вход в систему</p></li>
<li><p><code>SessionDisconnected</code>: пользователь не осуществил вход в систему</p></li>
</ul>
...
<div class="nav"><p><a href="15.2.php">Назад</a><a href="./">Содержание</a><a href="14.1.php">Вперед</a></p></div></div>
<div class="socBlock">
