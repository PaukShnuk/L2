...
<html >
<head>
<title>UWP | Системный менеджер навигации</title>
<meta charset="utf-8" />
<meta name="description" content="Менеджер навигации в Universal Windows Platform, работа с объектом SystemNavigationManager, создание кнопки "Назад" в приложении">
...
<h2>Системный менеджер навигации</h2><div class="date">Последнее обновление: 13.02.2016</div>
...
<p>Universal Windows Platform предоставляет такой класс, как <span class="b">Windows.UI.Core.SystemNavigationManager</span>, который позволяет реагировать 
на некоторые системные события навигации. Посмотрим, чем он может нам помочь.</p>
<p>Прежде всего через статический метод <code>GetForCurrentView()</code> мы можем получить объект SystemNavigationManager, который ассоциирован с текущим приложением. 
...
<span class="b">AppViewBackButtonVisibility</span>, которое устанавливает видимость кнопки "Назад". По умолчанию она скрыта, но присвоив ей значение 
<code>AppViewBackButtonVisibility.Visible</code>, мы ее можем открыть:</p>
<pre class="brush:c#;">
Windows.UI.Core.SystemNavigationManager.GetForCurrentView().AppViewBackButtonVisibility =
        Windows.UI.Core.AppViewBackButtonVisibility.Visible;
</pre>
<p>Кроме того, когда пользователь нажимает на эту кнопку, либо пытается перейти назад с помощью голосовой команды, у объекта SystemNavigationManager 
генерируется событие <span class="b">BackRequested</span>. Мы его можем обработать данное событие, чтобы выполнить переход назад:</p>
<pre class="brush:c#;">
...
<p>Так как работа SystemNavigationManager относится в целом ко всему приложению, то соответственно все манипуляции с ним должны происходить в классе App. 
Так, в случае его применения содержимое файла <i>App.xaml.cs</i> может выглядеть таким образом:</p>
<pre class="brush:c#;">
using System;
...
<p>Стоит отметить, что нет смысла показывать копку Назад, например, на главной странице, если еще не было переходов, поэтому она раскрывается только, если в истории 
навигации уже есть просмотренные страницы. Причем для этого используется обработчик события <span class="b">Navigated</span> объекта Frame. То есть при каждом переходе 
будет генерироваться данное событие, и в зависимости от наличия информации в истории переходов будет устанавливаться видимость кнопки.</p>
<p>И теперь после перехода по страницам мы сможем увидеть и использовать кнопку "Назад" в верхней панели приложения:</p>
<img src="pics/9.6.png" alt="Кнопка Назад в приложении Universal Windows Platform" />
