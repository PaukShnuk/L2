...
<html >
<head>
<title>UWP | Сложные свойства в XAML и конвертеры типов</title>
<meta charset="utf-8" />
<meta name="description" content="Сложные свойства в XAML и конвертеры типов в Universal Windows Platform">
...
<h2>Сложные свойства и конвертеры типов</h2><div class="date">Последнее обновление: 30.01.2016</div>
...
<p>В предыдущих темах мы рассмотрели определение элемента Button в коде XAML и установку ряда свойств через атрибуты:</p>
<pre class="brush:xml;">&lt;Button x:Name="messageButton" Width="120" Height="40" Content="Кнопка" HorizontalAlignment="Center"  Background="Red" /&gt;</pre>
<p>Height и Width являются простыми свойствами. Они хранят числовое значение. А например, свойство <code>HorizontalAlignment</code> или <code>Background</code> 
уже являются более сложными по своей структуре. Так, если мы будем определять эту же кнопку в коде c#, то нам надо использовать следующий набор инструкций:</p>
<pre class="brush:c#;">
...
<p>Для определения выравнивания по центру используется перечисление <code>HorizontalAlignment</code>, а для установки фонового цвета - класс 
SolidColorBrush.</p>
<p>В коде XAML мы ничего такого не увидели и устанавливали эти свойства гораздо проще с помощью строк. Дело в том, что по отношению к коду XAML применяются 
специальные объекты - <span class="b">type conventer</span> или конвертеры/преобразователи типов, которые могут преобразовать значения из XAML к тем объектам, которые мы используем в коде C#.</p>
...
<p>Плюсом такого подхода является то, что у объектов мы можем установить дополнительные параметры. Так, в данном случае у объекта SolidColorBrush 
кроме цвета также устанавливается и прозрачность (<code>Opacity="0.5"</code>), что сложно сделать при сокращенном определении свойства <code>Background="Red"</code></p>
