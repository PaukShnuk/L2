...
<html >
<head>
<title>UWP | Введение в компоновку</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в компоновку пользовательского интерфейса в Universal Windows Platform, контейнеры компоновки">
...
<h1>Компоновка</h1><h2>Введение в компоновку</h2><div class="date">Последнее обновление: 11.02.2016</div>
...
<p>Построение графического интерфейса в Universal Windows Platform базируется на концепции компоновки. <span class="b">Компоновка (layout)</span> 
...
"резинового" дизайна и адаптивных интерфейсов, которые проще подстроить под различные разрешения экранов.</p>
<p>В Universal Windows Platform компоновка выполняется с помощью специальных контейнеров. В частности, нам доступны следующие контейнеры компоновки: 
<span class="b">Grid, VariableSizedWrapGrid, StackPanel, SplitView, RelativePanel</span> и <span class="b">Canvas</span>. 
...
комбинировать их. Кроме того, если нам не хватает стандартных контейнеров, мы можем определить свои с нужной нам функциональностью.</p>
<p>Все выше перечисленные контейнеры компоновки наследуются от класса Panel, которое предоставляет ряд базовых свойств и методов. А само дерево наследования можно представить следующим образом:</p>
<img src="pics/3.15.png" alt="Layout в Universal Windows Platform" />
<p>В частности, отличительной чертой контейнеров компоновки является свойство <span class="b">Children</span>, которое включает все вложенные контейнер элементы. 
Например, возьмем стандартный контейнер Grid:</p>
<pre class="brush:xml;">
&lt;Grid x:Name="layoutGrid"&gt;
    &lt;Button Content="Hello" /&gt;
&lt;/Grid&gt;
</pre>
<p>Здесь в грид помещена кнопка. Этот код эквивалентен следующему:</p>
<pre class="brush:xml;">
&lt;Grid x:Name="layoutGrid"&gt;
    &lt;Grid.Children&gt;
        &lt;Button Content="Hello" /&gt;
...
<p>Свойство Children представляет коллекцию <code>UIElementCollection</code>, которая содержит объекты UIElement. Практически все стандартные элементы в Universal 
Windows Platform представляют тип UIElement. И как многие стандартные коллекции, коллекция Children имеет методы для управления содержимым:</p>
<ul>
<li><p><code>Add(UIElement item)</code>: добавляет UIElement в контейнер</p></li>
<li><p><code>Remove(UIElement item)</code>: удаляет UIElement из контейнера</p></li>
<li><p><code>RemoveAt(int index)</code>: удаляет элемент из контейнера по индексу в коллекции Children</p></li>
<li><p><code>Move(int oldIndex, int newIndex)</code>: перемещает элемент в коллекции с одного индекса на другой</p></li>
<li><p><code>Clear()</code>: удаляет все элементы из коллекции</p></li>
</ul>
<p>Например, в коде C# мы могли бы изменять содержимое контейнера компоновки:</p>
<pre class="brush:c#;">
...
<p>Хотя в данном случае мы рассматривали работу с элементами для контейнера Grid, но все то же самое будет характерно и для других контейнеров компоновки.</p>
...
размеры приходится усекать. Затем происходит этап непосредственной расстановки дочерних элементов внутри контейнера.</p>
<p>Теперь рассмотрим подробнее отдельные контейнеры компоновки.</p>
