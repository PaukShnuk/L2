...
<html >
<head>
<title>UWP | Иерархия ресурсов и словари ресурсов</title>
<meta charset="utf-8" />
<meta name="description" content="Иерархия ресурсов в Universal Windows Platform, определение, подключение и использование словарей ресурсов">
...
<h2>Иерархия ресурсов и словари ресурсов</h2><div class="date">Последнее обновление: 11.02.2016</div>
...
<p>При определении и применении ресурсов следует учитывать их иерархию. Ресурсы элемента применяются только к вложенным элементам, но не к внешним 
...
данной страницы.</p>
<p>Но кроме определения ресурсов на уровне отдельных элементов страницы мы также можем определять ресурсы для всего приложения в целом, 
используя отдельные словари ресурсов.</p>
<p>Итак, нажмем правой кнопкой мыши на проект и в контекстном меню выберем <span class="b">Add -&gt; New Item...</span>, и в окне добавления выберем пункт 
<span class="b">Resource Dictionary</span>:</p>
<img src="pics/5.2.png" alt="Словари ресурсов в Universal Windows Platform" />
<p>Оставим у него название по умолчанию Dictionary1.xaml и нажмем на кнопку OK.</p>
<p>После этого в проект добавляется новый файл. Он представляет собой обычный xaml-файл с одним корневым элементом <span class="b">ResourceDictionary</span>:</p>
<pre class="brush:xml;">
&lt;ResourceDictionary
...
<p>После определения файла ресурсов его надо подсоединить к ресурсам приложения. 
Для этого откроем файл <i>App.xaml</i>, который есть в проекте по умолчанию и изменим его:</p>
<pre class="brush:xml;">
&lt;Application
    x:Class="ResourceApp.App"
    xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
    xmlns:local="using:ResourceApp"
...
<p>Здесь элемент <code>ResourceDictionary.MergedDictionaries</code> представляет коллекцию объектов ResourceDictionary, то есть словарей ресурсов, 
которые добавляются к ресурсам приложения. Затем в любом месте приложения, например, в коде MainPage.xaml мы сможем сослаться на этот ресурс:</p>
<pre class="brush:xml;">&lt;Button Content="Hello" HorizontalAlignment="Center" Background="{StaticResource buttonBrush}" /&gt;</pre>
<p>При этом мы можем добавлять в коллекцию ресурсов приложения одновременно множество других словарей или параллельно с ними определять еще какие-либо 
ресурсы:</p>
<pre class="brush:xml;">
&lt;Application
    x:Class="ResourceApp.App"
...
<p>Необязательно добавлять словарь ресурсов через ресурсы приложения. У объекта ResourceDictionary имеется свойство <span class="b">Source</span>, 
