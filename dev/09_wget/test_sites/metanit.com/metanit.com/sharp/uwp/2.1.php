...
<html >
<head>
<title>UWP | Основы XAML</title>
<meta charset="utf-8" />
<meta name="description" content="Основы использования XAML в Universal Windows Platform для декларативного описания графического интерфейса">
...
<h1>Основы XAML</h1><h2>XAML в Universal Windows Platform</h2><div class="date">Последнее обновление: 12.04.2017</div>
...
<p>При разработке приложений UWP с помощью языка C# основной технологией для построения графического интерфейса является <span class="b">XAML</span> 
(eXtensible Application Markup Language). XAML представляет язык разметки на основе XML, который используется для декларативного описания графического интерфейса.</p>
<p>XAML позволяет отделить определение интерфейса или внешнего вида приложения от его логики, которая пишется на C#. Кроме того, декларативное описание 
интерфейса делает код более компактным и понятным.</p>
<p>Если мы возьмем стандартный проект UWP по типу <span class="ii">Blank Application</span>, то как раз увидим файлы, которые используют XAML. Например, в проекте мы 
можем найти файл <span class="ii">MainPage.xaml</span>, который представляет декларативное описание графического интерфейса для единственной страницы в приложении:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Как и в любом документе XML здесь определяется иерархия элементов. Элементом самого верхнего уровня  или корневым элементом 
...
из которых состоит интерфейс, например, кнопки, текстовые поля и т.д., добавляются внутрь элемента Page.</p>
<h3>Пространства имен XAML</h3>
<p>При написании программы на C# мы можем подключать классы из внешних пространств имен с помощью директивы using, например, <code>using System.Windows</code>. 
В XAML также есть свои пространства имен, только они добавляются в документ с помощью атрибута <span class="b">xmlns</span>. В частности, если мы посмотрим на 
содержание элемента Page, то мы как раз можем заметить подключенные пространства имен:</p>
<pre class="brush:xml;">
xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
xmlns:local="using:XamlApp"
...
<p>Рассмотрим все эти пространства.</p>
<ul>
<li><p>Пространство имен <span class="b">http://schemas.microsoft.com/winfx/2006/xaml/presentation</span> 
...
поэтому оно определяется просто с помощью атрибута <code>xmlns</code> без всяких префиксов.</p></li>
<li><p><p>Далее подключается пространство имен <span class="b">"http://schemas.microsoft.com/winfx/2006/xaml"</span>. Оно определяет ряд атрибутов xaml 
таких, как Name или Key. Причем оно объявляется с помощью атрибута <code>xmlns:x</code> - префикс x после двоеточия означает, что те свойства элементов, 
которые содержатся в этом пространстве имен, будут использоваться с префиксом x. Например, x:Name или x:Key.</p>
<p>Так, на второй строке идет следующее выражение: <code>x:Class="XamlApp.MainPage"</code>. Атрибут <code>Class</code> указывает на класс C#, который будет 
соответствовать данному элементу Page. И так как этот атрибут используется с префиксом x, то соответственно определение атрибута <code>Class</code> 
будет извлекаться из пространства имен <code>xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"</code></p></p></li>
<li><p>Третье подключаемое пространство имен <code>xmlns:local="using:XamlApp"</code> представляет текущее пространств имен проекта, который в моем случае 
называется XamlApp. То есть, все определенные в проекте классы, можно будет использовать в XAML с префиксом local.</p></li>
<li><p>Два последних пространства имен <span class="b">xmlns:d="http://schemas.microsoft.com/expression/blend/2008"</span> и <span class="b">xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006"</span> 
...
поэтому на следующей строке можно увидеть атрибут <code>mc:Ignorable="d"</code>. Он указывает, что надо игнорировать пространство имен с префиксом d.</p></li>
</ul>
<h3>Элементы и их атрибуты</h3>
<p>XAML предлагает довольно простую и ясную схему определения различных элементов и их свойств. Каждый элемент, как и любой элемент XML, 
должен иметь открытый и закрытый тег, как в случае с элементом Page:</p>
<pre class="brush:xml;">
&lt;Page&gt; &lt;/Page&gt;
</pre>
...
<p>Но в отличие от элементов обычного документа xml элементы в xaml соответствуют определенному классу из C#. Например, кнопка - элемент Button соответствует 
классу <code>Windows.UI.Xaml.Controls.Button</code>. А атрибуты элемента Button соответствуют свойствам класса Button.</p>
<p>Например, добавим на страницу кнопку:</p>
<pre class="brush:xml;">
&lt;Page
    x:Class="XamlApp.MainPage"
    xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
...
<p>Для кнопки мы можем определить свойства в виде атрибутов. Здесь определены атрибуты <code>x:Name</code> (имя кнопки), Width, Height, FontFamily (используемый шрифт) и 
Content. Причем, атрибут <code>x:Name</code> берется в данном случае из пространства имен <code>http://schemas.microsoft.com/winfx/2006/xaml</code>, 
...
<code>http://schemas.microsoft.com/winfx/2006/xaml/presentation</code>. Из него же берется и сам элемент Button.</p>
<p>Также мы можем определить и другие атрибуты, которые нам нужны. Либо мы в общем можем не определять атрибуты, и тогда они будут использовать значения по умолчанию.</p>
<p>И мы можем запустить проект, и тогда на форме приложения появится добавленная нами кнопка:</p>
<img src="pics/2.1.png" alt="XAML в Universal Windows Platform" />
