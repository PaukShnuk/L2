...
<html >
<head>
<title>UWP | Контекст данных и интерфейс INotifyPropertyChanged</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с привязкой в Universal Windows Platform, установка контекста данных элемента и свойство DataContext, реализация интерфейса INotifyPropertyChanged для автоматического обновления объекта">
...
<h2>Контекст данных и интерфейс INotifyPropertyChanged</h2><div class="date">Последнее обновление: 13.04.2017</div>
...
<p>Одним из способов задания привязки к объекту является использование контекста данных или свойства <span class="b">DataContext</span>. 
...
Посмотрим на примере с объектом Phone:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Свяжем свойства объекта Phone элементами на странице через DataContext:</p>
<pre class="brush:xml;">
&lt;Page
...
<img src="pics/6.3.png" alt="DataContext в Universal Windows Platform" />
<p>Таким образом мы задаем свойству DataContext некоторый ресурс, и затем осуществляется привязка к этому ресурсу.</p>
<p>Каждый объект класса FrameworkElement, а это практически все стандартные элементы управления, может определять свой собственный контекст данных. Например:</p>
<pre class="brush:xml;">
&lt;Page
    x:Class="BindingApp.MainPage"
    xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
...
<p>Здесь определены два элемента StackPanel, каждый из которых использует разные контексты данных: один использует контекст страницы, а другой определяет свой собственный.</p>
<img src="pics/6.8.png" alt="Свойство DataContext в UWP" />
<p>Определение контекста в коде C#:</p>
<pre class="brush:c#;">
...
<p>В этом случае все выражения Binding в XAML автоматически будут привязываться к свойствам этого объекта:</p>
<pre class="brush:xml;">
&lt;Page
    x:Class="BindingApp.MainPage"
    xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
...
<h3>Интерфейс INotifyPropertyChanged</h3>
<p>В примерах выше привязка осуществлялась к объекту класса Phone. Но использование этого класса имело один минус. Даже если мы изменим значения 
его свойств, то содержимое привязанных текстовых блоков это никак не повлияет. Например, добавим на страницу пару кнопок для изменения состояния объекта Phone:</p>
<pre class="brush:xml;">
&lt;Page
    x:Class="BindingApp.MainPage"
    xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
...
<p>А в файле кода C# для этих кнопок определим обработчики Increase и Decrease, в которых будет меняться свойство Price:</p>
<pre class="brush:c#;">
...
<p>В данном случае нажатия на кнопку изменят ресурс, но текстовые блоки, привязанные к этому ресурсу, не изменятся. Чтобы объект мог полноценно 
реализовать механизм привязки, нам надо реализовать в его классе интерфейс INotifyPropertyChanged. И для этого изменим класс Phone следующим образом:</p>
<pre class="brush:c#;">
...
<p>Теперь при изменении значения свойства в объекте Phone срабатывает метод OnPropertyChanged, через который данный объект будет извещать систему об изменении через событие PropertyChanged. А система в свою очередь 
обновляет все привязанные объекты, в частности, элементы TextBlock.</p>
<img src="pics/6.9.png" alt="Интерфейс INotifyPropertyChanged в Universal Windows Platform" />
...
<div class="nav"><p><a href="5.1.php">Назад</a><a href="./">Содержание</a><a href="5.4.php">Вперед</a></p></div></div>
<div class="socBlock">
