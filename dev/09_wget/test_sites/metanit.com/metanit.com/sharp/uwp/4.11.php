...
<html >
<head>
<title>UWP | DispatcherTimer, ProgressBar и ProgressRing</title>
<meta charset="utf-8" />
<meta name="description" content="Элементы DispatcherTimer, ProgressBar и ProgressRing в Universal Windows Platform, их основные свойства и методы, создание таймера и управлением им в UWP, индикатор прогресса">
...
<h2>DispatcherTimer, ProgressBar и ProgressRing</h2><div class="date">Последнее обновление: 11.02.2016</div>
...
<p>DispatcherTimer не является элементом управления, однако мы можем его использовать для процессов в графическом приложении, которые должны выполняться с определенной периодичностью. 
Наиболее важные свойства и методы таймера:</p>
<ul>
<li><p>Свойство <span class="b">Enabled</span> при значении true указывает, что таймер будет запускаться вместе с формой</p></li>
<li><p>Свойство <span class="b">Interval</span> устанавливает интервал таймера в виде объекта TimeSpan</p></li>
<li><p>Метод <span class="b">Start()</span> запускает таймер</p></li>
<li><p>Метод <span class="b">Stop()</span> останавливает таймер</p></li>
<li><p>Событие <span class="b">Tick</span> срабатывает по истечении интервала из свойства Interval</p></li>
</ul>
<p>Далее на примере индикатора процесса рассмотри использование таймера</p>
<h3>ProgressBar</h3>
<p>ProgressBar представляет индикатор выполнения какого-либо процесса. Например, зададим следующий ProgressBar:</p>
<pre class="brush:xml;">
...
<p>Свойство <code>Maximum</code> указывает на максимально возможное значение индикатора. При изменении значения генерируется событие <span class="b">ValueChanged</span>. 
Определим в файле кода для него обработчик:</p>
<pre class="brush:c#;">
using System;
...
<p>В данном случае при запуске приложения будет запускаться таймер, который будет срабатывать раз в секунду. В обработчике таймера происходит приращение 
значения индикатора: <code>progressBar.Value++</code></p>
<p>В ответ на изменение значения индикатора генерируется событие ValueChanged и срабатывает его обработчик, который изменяет текст в текстовом блоке.</p>
<img src="pics/4.37.png" alt="ProgressBar в UWP" />
<h3>ProgressRing</h3>
<p>ProgressRing представляет кольцо с анимацией, которое отображается во время некоторого длительного процесса:</p>
<pre class="brush:xml;">&lt;ProgressRing x:Name="progressRing" IsActive="True" Width="60" Height="60" /&gt;</pre>
<p>Если свойство <code>IsActive</code> равно <code>true</code>, значит кольцо будет отображаться. При завершении процесса мы можем программно отключить отображение, задав 
этому свойству значение <code>false</code></p>
<img src="pics/4.38.png" alt="ProgressRing в UWP" />
