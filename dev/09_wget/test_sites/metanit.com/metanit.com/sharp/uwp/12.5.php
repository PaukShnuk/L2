...
<html >
<head>
<title>UWP | Файловые диалоговые окна</title>
<meta charset="utf-8" />
<meta name="description" content="Файловые диалоговые окна в Universal Windows Platform, открытие и сохранение файлов, классы FileOpenPicker и FileSavePicker">
...
<h2>Файловые диалоговые окна</h2><div class="date">Последнее обновление: 24.02.2016</div>
...
<p>На Universal Windows Platform для работы с файлами мы можем использовать специальные диалоговые окна. В каких ситуациях они могут быть нам полезны? Прежде всего когда 
речь идет об открытии или сохранении файла, и нам нужен простой и понятный визуальный компонент для выбора места файла на устройстве.</p>
<p>Для сохранения файла применяется компонент <span class="b">Windows.Storage.Pickers.FileSavePicker</span>, а для открытия файла .</p>
<p>Для работы с файловыми диалогами определим следующую разметку XAML:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Здесь определены две кнопки на нижней панели, для которых в файле кода C# пропишем обработчики:</p>
<pre class="brush:c#;">
using System;
...
<img src="pics/12.7.png" alt="Диалоговые окна в Universal Windows Platform" />
<p>FileOpenPicker и FileSavePicker имеют похожий набор свойств, которые позволяют кастомизировать диалоговые окна:</p>
<ul>
<li><p><span class="b">SuggestedStartLocation</span>: указывает на каталог по умолчанию, который будет открываться в диалоговом окне. 
В качестве значения можно указать одну из констант перечисления <span class="b">PickerLocationId</span>. Например, каталог Документы - <code>PickerLocationId.DocumentsLibrary</code></p></li>
<li><p><span class="b">SuggestedFileName</span>: устанавливает имя файла по умолчанию</p></li>
<li><p><span class="b">CommitButtonText</span>: устанавливает текст кнопки, которая и выполняет действие сохранения или открытия</p></li>
<li><p><span class="b">DefaultFileExtension</span>: расширение файла, которое будет добавляться к имени файла при сохранении, если другого расширения не будет указано (только для FileSavePicker)</p></li>
<li><p><span class="b">FileTypeChoices</span>: объект IDictionary, который содержит расширения файлов и их описания (только для FileSavePicker)</p></li>
<li><p><span class="b">FileTypeFilter</span>: объект List, который содержит список доступных расширений файлов (только для FileOpenPicker)</p></li>
<li><p><span class="b">ViewMode</span>: формат отображения списка файлов в диалоговом окне при их открытии (только для FileOpenPicker). 
Принимает одну из двух констант: <code>PickerViewMode.Thumbnail</code> (отображение файлов в виде значков) и <code>PickerViewMode.List</code> 
(отображение файлов в виде списка)</p></li>
</ul>
<p>При сохранении файла мы получаем указанное имя файла с расширением с помощью метода <code>PickSaveFileAsync()</code>:</p>
<pre class="brush:c#;">var new_file = await savePicker.PickSaveFileAsync();</pre>
<p>Этот метод возвращает объект StorageFile, получив который, мы можем сохранять в него различное содержимое.</p>
<p>При открытии файла мы получаем выбранный файл с помощью метода <code>PickSingleFileAsync()</code>:</p>
<pre class="brush:c#;">var new_file = await savePicker.PickSaveFileAsync();</pre>
<p>Этот метод также возвращает объект StorageFile, который представляет выбранный файл.</p>
<p>При этом надо подчеркнуть, что сохранение или открытие файла через диалоговые компоненты будет работать как на десктопах, так и на мобильных устройствах, только на смартфонах 
...
проходит следующие экраны:</p>
<img src="pics/12.8.png" alt="Открытие файла на Windows 10 Mobile" />
...
<div class="nav"><p><a href="12.4.php">Назад</a><a href="./">Содержание</a><a href="15.1.php">Вперед</a></p></div></div>
<div class="socBlock">
