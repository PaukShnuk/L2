...
<html >
<head>
<title>UWP | WebView</title>
<meta charset="utf-8" />
<meta name="description" content="Элемент WebView в Universal Windows Platform, его основные свойства и методы, создание мини-веббраузера в UWP, загрузка локальных html-страниц">
...
<h2>WebView</h2><div class="date">Последнее обновление: 12.11.2017</div>
...
<p>WebView представляет элемент для отображения html-кода. Загружаемый html-код может представлять код веб-страниц с веб-сайтов, код локальных страниц или 
код html, который формируется программно. То есть фактически это мини веб-браузер.</p>
<p>Следует отметить ряд особенностей WebView:</p>
<ul>
<li><p>этот элемент не поддерживает большинство событий ввода, которыми обладают другие элементы управления</p></li>
<li><p>для рендеринга он использует движок Microsoft Edge</p></li>
<li><p>он не поддерживает элементы ActiveX, плагины, а также некоторые функциональности HTML5, в частности, AppCache, IndexedDB, программный доступ к 
буферу обмена, геолокацию</p></li>
<li><p>он поддерживает навигацию по ресурсам, использующим протоколы HTTP, HTTPS, ms-appx-web и ms-appdata</p></li>
</ul>
<h3>Адрес ресурса</h3>
<p>За установку ресурса, отображаемого в WebView, отвечает свойство <span class="b">Source</span>. В качестве значения это свойство принимает 
путь к ресурсу. В зависимости от расположения ресурса мы можем использовать различные пути.</p>
<p>Например, загрузим страницу по адресу https://www.microsoft.com:</p>
<pre class="brush:xml;">&lt;WebView x:Name="webView" Source="https://www.microsoft.com" /&gt;</pre>
<img src="pics/4.39.png" alt="WebView в UWP" />
<p>Для просмотра локальных ресурсов следует использовать схемы доступа ms-appx-web и ms-appdata. Например, пусть у нас в проекте есть папка <span class="ii">pages</span>, 
которая содержит html-страницы.</p>
<img src="pics/4.64.png" alt="WebView в Universal Windows Platform" />
<p>Тогда, чтобы обратиться к локальным html-страницам из этой папки, нам надо использовать формат <span class="b">ms-appx-web</span>:</p>
<pre class="brush:xml;">&lt;WebView x:Name="webView" Source="ms-appx-web:///pages/page1.html" /&gt;</pre>
<p>Если необходимая папка с html-страницами располагается на локальном компьютере в локальной папке пользователя или в каталоге temp, то к таким страницам мы можем обратиться 
по следующим путям:</p>
<pre class="brush:c#;">
ms-appdata:///local/pages/page1.html
ms-appdata:///temp/pages/page1.html
</pre>
<h3>Программная навигация</h3>
<p>Для управления элементом мы можем использовать его методы и события. Ряд методов позволяют загружать контент:</p>
<ul>
<li><p><code>NavigateToString</code>: переход к строке кода, который формируется программно. Например:</p>
<pre class="brush:c#;">
string html = "&lt;h2&gt;Hello world45!&lt;/h2&gt;";
webView.NavigateToString(html);</pre></li>
<li><p><code>Navigate</code>: переход по определенному пути. Например:</p>
<pre class="brush:c#;">
...
<h3>События WebView</h3>
<p>Некоторые важные события:</p>
<ul>
<li><p><span class="b">NavigationStarting</span>: генерируется до начала перехода WebView к указанному ресурсу. Мы можем использовать это событие, к примеру, 
чтобы оставить переход к ресурсу:</p>
<pre class="brush:c#;">
...
<li><p><span class="b">ContentLoading</span>: генерируется до начала загрузки контента</p></li>
<li><p><span class="b">DOMContentLoaded</span>: возникает, после того, как WebView закончит загрузку и парсинг контента</p></li>
<li><p><span class="b">NavigationCompleted</span>: возникает после завершения перехода к ресурсу</p></li>
</ul>
<p>Причем все эти события возникают именно в той последовательности, в которой они тут перечислены.</p>
...
<div class="nav"><p><a href="4.11.php">Назад</a><a href="./">Содержание</a><a href="4.20.php">Вперед</a></p></div></div>
<div class="socBlock">
