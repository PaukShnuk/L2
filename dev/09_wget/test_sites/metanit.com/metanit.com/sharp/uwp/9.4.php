...
<html >
<head>
<title>UWP | Восстановление информации о навигации</title>
<meta charset="utf-8" />
<meta name="description" content="Восстановление информации об истории межстраничной навигации в Universal Windows Platform">
...
<h2>Восстановление информации о навигации</h2><div class="date">Последнее обновление: 13.02.2016</div>
...
<p>В прошлой главе, где рассказывалось о жизненном цикле приложения, отмечалось, что система сама, в случае нехватки ресурсов, может перевести приложение в состояние 
...
он продолжал работу именно с той страницей, на которой завершилось выполнение.</p>
<p>Чтобы выйти из этой ситуации можно сохранять и восстанавливать историю навигации. Для этого в классе Frame определено два метода:</p>
<ul>
<li><p><span class="b">GetNavigationState()</span>: возвращает состояние навигации фрейма</p></li>
<li><p><span class="b">SetNavigationState(string state)</span>: восстанавливает состояние навигации, получая состояние в виде сериализованных в строку данных</p></li>
</ul>
<p>Для применения этих методов изменим класс приложения:</p>
<pre class="brush:c#;">
using System;
...
<p>Так как перед переходом в состояние Terminated приложение сначала приостанавливается и переходит в состояние Suspended, то этот переход мы можем обработать, 
...
добавляется элемент с ключом "navHistory". По этому ключу и будет храниться история навигации:</p>
<pre class="brush:c#;">
private void OnSuspending(object sender, SuspendingEventArgs e)
{
    var deferral = e.SuspendingOperation.GetDeferral();
...
<p>При запуске приложения в методе <code>OnLaunched()</code> проверяется предыдущее состояние приложения. Если оно равно Terminated, то история навигации восстанавливается:</p>
<pre class="brush:c#;">
if (e.PreviousExecutionState == ApplicationExecutionState.Terminated)
{
    if (ApplicationData.Current.LocalSettings.Values.ContainsKey("navHistory"))
...
<p>Чтобы проверить работоспособность определенного нами функционала, мы можем эмулировать завершение приложения в Visual Studio после запуска с помощью опции 
<span class="b">Suspend and shutdown</span>, которая становится доступной после запуска проекта в режиме отладки:</p>
<img src="pics/9.7.png" alt="Suspend and shutdown в UWP" />
<p>То есть после запуска проекта переходим на какую-нибудь страницу и далее выбираем опцию <span class="b">Suspend and shutdown</span> для 
...
И даже если мы находились на второй или третьей странице, то при повторном запуске приложения мы также окажемся на этой странице.</p>
