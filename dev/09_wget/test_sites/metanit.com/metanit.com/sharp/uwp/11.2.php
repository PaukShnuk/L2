...
<html >
<head>
<title>UWP | ObservableCollection</title>
<meta charset="utf-8" />
<meta name="description" content="Использование ObservableCollection в Universal Windows Platform, хранение данных в виде коллекции и ее подключение к стандартным элементам UWP">
...
<h2>ObservableCollection</h2><div class="date">Последнее обновление: 14.04.2017</div>
...
<p>Как правило, привязка в элементах управления данными осуществляется не к стандартным спискам, а к объектам класса <span class="b">ObservableCollection</span>. 
...
может уведомлять элементы, которые применяют привязку, в результате чего обновляется не только сам объект ObservableCollection, но и привязанные к нему элементы интерфейса.</p>
<p>К примеру, возьмем проект из прошлой темы и изменим xaml-код страницы MainPage:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Здесь также идет привязка к свойству Phones, которое представляет коллекцию объектов Phone.</p>
<p>Кроме того, во второй строке грида определена форма для добавления данных.</p>
<p>Теперь изменим код в файле <span class="ii">MainPage.xaml.cs</span> следующим образом:</p>
<pre class="brush:c#;">
using System;
...
<p>Теперь свойство Phones представляет объект ObservableCollection. Поэтому при добавлении данных новый элемент автоматически будет отображен в списке:</p>
<img src="pics/11.2.png" alt="Добавление в ObservableCollection" />
<img src="pics/11.3.png" alt="ObservableCollection в UWP" />
<p>В случае, если бы мы использовали класс List, у нас бы не происходило обновление пользовательского интерфейса при добавлении объекта.</p>
