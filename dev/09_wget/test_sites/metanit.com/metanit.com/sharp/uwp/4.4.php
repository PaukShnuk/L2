...
<html >
<head>
<title>UWP | Текстовые поля</title>
<meta charset="utf-8" />
<meta name="description" content="Текстовые поля в Universal Windows Platform, классы TextBlock, TextBox, PasswordBox, их основые свойства и методы, примеры использования, обработка ввода текста">
...
<h2>Текстовые поля</h2><div class="date">Последнее обновление: 11.02.2016</div>
...
<h3>TextBlock</h3>
<p>Элемент TextBlock служит для вывода простой текстовой информации, то есть представляет собой обычную метку. За установку текста метки отвечает свойство <code>Text</code>:</p>
<pre class="brush:xml;">&lt;TextBlock Text="Universal Windows Platfom" /&gt;</pre>
<p>Для настройки шрифта мы можем использовать стандартные свойства <span class="b">FontSize</span> (высота шрифта), <span class="b">FontFamily</span> (семейство шрифтов), 
<span class="b">FontStretch</span> (растяжение шрифта), <span class="b">FontStyle</span> (стиль наклона), <span class="b">FontWeight</span> (вес/толщина шрифта):</p>
<pre class="brush:xml;">
...
<p>С помощью свойств LineHeight, TextWrapping и TextAllignment мы можем задать дополнительные настройки текста.</p>
<p>Свойство LineHeight позволяет указывать высоту строк.</p>
<h4>TextWrapping</h4>
<p>Свойство TextWrapping позволяет переносить текст. Может принимать три значения:</p>
<ul>
<li><p><code>NoWrap</code>: текст не переносится. Значение по умолчанию</p></li>
<li><p><code>Wrap</code>: если ширина текстовой метки не позволяет вместить весь текст, то в ней создается новые строки, на которые переносится текст</p></li>
<li><p><code>WrapWholeWords</code>: текст переносится по границам слов</p></li>
</ul>
<p>Установка в коде:</p>
<pre class="brush:c#;">textBlock.TextWrapping = TextWrapping.WrapWholeWords;</pre>
<h4>TextAllignment</h4>
<p>Свойство TextAllignment выравнивает текст. Может принимать следующие значения:</p>
<ul>
<li><p><code>Center</code>: выравнивание по центру</p></li>
<li><p><code>DetectFromContent</code>: направление выравнивания зависит от текста</p></li>
<li><p><code>Justify</code>: текст не растягивается так, чтобы символы были оптимальным образом распределены по всей ширине блока</p></li>
<li><p><code>Left</code>: выравнивание по левому краю</p></li>
<li><p><code>Right</code>: выравнивание по правому краю</p></li>
</ul>
<p>Установка в коде:</p>
<pre class="brush:c#;">textBlock.TextAlignment = TextAlignment.Justify;</pre>
<p>Например:</p>
<pre class="brush:xml;">
...
<h4>TextLineBounds</h4>
<p>Свойство <span class="b">TextLineBounds</span> устанавливает способ определения высоты для каждой строки текста в элементе. Может принимать следующие значения:</p>
<ul>
<li><p><code>Full</code>: строка имеет стандартную высоту. Значение по умолчанию</p></li>
<li><p><code>TrimToCapHeight</code>: высота строки устанавливается по высоте шапки шрифта.</p></li>
<li><p><code>TrimToBaseline</code>: нижний уровень строки устанавливается по опорной линии текста.</p></li>
<li><p><code>Tight</code>: высота строки устанавливается по шапке шрифта, а нижний уровень - по опорной линии текста</p></li>
</ul>
<img src="pics/4.3.png" alt="TextLineBounds in UWP" />
<p>Установка в коде:</p>
<pre class="brush:c#;">textBlock.TextLineBounds = TextLineBounds.Full;</pre>
<h4>TextTrimming</h4>
<p>Свойство <span class="b">TextTrimming</span> устанавливает, как будет усекаться текст при выходе за пределы ограничивающего прямоугольника. Может принимать следующие значения:</p>
<ul>
<li><p><code>None</code>: текст не усекается. Значение по умолчанию</p></li>
<li><p><code>CharacterEllipsis</code>: текст усекается по границе символа. На месте усечения вставляется многоточие</p></li>
<li><p><code>WordEllipsis</code>: текст усекается по границе слова. На месте усечения вставляется многоточие</p></li>
<li><p><code>Clip</code>: текст обрезается на уровне пикселей</p></li>
</ul>
<p>Установка в коде:</p>
<pre class="brush:c#;">textBlock.TextTrimming = TextTrimming.WordEllipsis;</pre>
<h4>TextReadingOrder и FlowDirection</h4>
<p>Свойство <span class="b">FlowDirection</span> задает направление текста - слева направо или справа налево:</p>
<ul>
<li><p><code>LeftToRight</code>: направление текста слева направо, значение по умолчанию</p></li>
<li><p><code>RightToLeft</code>: направление текста справа налево</p></li>
</ul>
<p>Установка в коде:</p>
<pre class="brush:c#;">textBlock.FlowDirection = FlowDirection.RightToLeft;</pre>
<p>Свойство <span class="b">TextReadingOrder</span> также влияет на порядок чтения:</p>
<ul>
<li><p><code>Default</code>: по умолчанию не переопределяет направление содержимого. Направление задается через FlowDirection</p></li>
<li><p><code>DetectFromContent</code>: корректирует порядок, в котором размещаются блоки двунаправленного текста</p></li>
<li><p><code>UseFlowDirection</code>: то же самое, что и Default</p></li>
</ul>
<p>Установка в коде:</p>
<pre class="brush:c#;">textBlock.TextReadingOrder = TextReadingOrder.UseFlowDirection;</pre>
<h3>TextBox</h3>
<p>Для ввода текста используется элемент <span class="b">TextBox</span>. Он также, как и TextBlock, имеет свойства TextWrapping, TextAllignment и 
TextReadingOrder. Для задания шрифта текста также применяются свойства <code>FontFamily</code>, <code>FontSize</code>. Среди других свойств следует 
выделить <span class="b">MaxLength</span> и <span class="b">PlaceholderText</span></p>
<p><span class="b">MaxLength</span> устанавливает предельное количество вводимых символов.</p>
<p>Свойство <span class="b">PlaceholderText</span> задает текст по умолчанию, который отображается в поле до ввода в него символов:</p>
<pre class="brush:xml;">&lt;TextBox MaxLength="250" PlaceholderText="Введите текст" Height="40" /&gt;</pre>
<p>Еще одно интересное свойство <span class="b">IsSpellCheckEnabled</span> позволяет автоматически проверять орфографию:</p>
<pre class="brush:xml;">&lt;TextBox IsSpellCheckEnabled="True" Height="40" /&gt;</pre>
<p>Если мы введем некорректное значение, то текстовое поле его подчеркнет:</p>
<img src="pics/4.4.png" alt="TextBox in Universal Windows Platform" />
<p>Из событий элемента следует выделить событие <span class="b">TextChanged</span>, которое срабатывает при изменении текста в текстовом поле. Например, 
подключим обработчик события:</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<p>А в файле кода пропишем сам обработчик:</p>
<pre class="brush:c#;">
...
<h3>PasswordBox</h3>
<p>PasswordBox применяется для ввода парольной информации. Это тоже самое текстовое поле, только для ввода символов используется маска. 
Свойство <span class="b">PasswordChar</span> устанавливает символ маски, отображаемый при вводе пароля. Свойство <span class="b">Password</span> 
задает парольную строку, отображаемую по умолчанию при загрузке окна приложения.</p>
<pre class="brush:xml;">
&lt;StackPanel&gt;
...
<img src="pics/4.5.png" alt="PasswordBox in Universal Windows Platform" />
