...
<html >
<head>
<title>UWP | Элемент Bind</title>
<meta charset="utf-8" />
<meta name="description" content="Привязка в Universal Windows Platform с помощью элемента Bind, его возможности и ограничения, обновление привязки и метод Bindings.Update">
...
<h2>Элемент Bind</h2><div class="date">Последнее обновление: 13.04.2017</div>
...
<p>Элемент Bind в некотором отношении аналогичен <code>Binding</code>, только позволяет выполнить привязку к определенным свойствам страницы. 
Bind имеет ряд свойств, которые также определены в Binding: Path, Mode, TargetNullValue. Но в отличие от Binding <code>x:Bind</code> по умолчанию работает по режиму привязки <code>OneTime</code>, то есть привязка осуществляется только один раз. Хотя через свойство Mode можно изменить режим привязки.</p>
<p>Но главное отличие между Bind и Binding состоит в том, что выражение x:Bind выполняет код, который генерируется во время компиляции, а Binding 
обрабатывает привязки во время выполнения. В результате привязки Bind имеет большую производительность.</p>
<p>В качестве источника привязки элемент Bind использует страницу или элемент управления и устанавливает привязку к их свойствам и методам. 
Например, пусть в коде страницы MainPage имеется свойство Phone, которое ссылается на объект Phone:</p>
<pre class="brush:c#;">
...
<p>С помощью Bind мы можем установить привязку к этому свойству:</p>
<pre class="brush:xml;">
&lt;Page
...
<h3>Привязка к событиям</h3>
<p>С помощью <code>x:Bind</code> мы можем привязывать методы объектов к событиям элементов управления. Так, пусть в классе Phone определен метод <code>ShowModel()</code>:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>В данном случае метод просто выводит в окне осообщения название модели.</p>
<p>Тогда мы бы могли определить следующую кнопку, которая бы вызывала этот метод:</p>
<pre class="brush:xml;">&lt;Button Content="Показать модель" Click="{x:Bind Phone.ShowModel}" HorizontalAlignment="Center" /&gt;</pre>
<h3>Метод Bindings.Update</h3>
<p>Как было выше сказано, <code>x:Bind</code> имеет ограничение в том, что он работает по типу привязки OneTime. А это значит, что если мы в процессе работы программы изменим 
...
изменения объекта и при наличии изменений вызывать метод <span class="b">Bindings.Update()</span>. Данный метод обновляет все привязки, которые используются в 
элементе управления.</p>
<p>Например, выведем в XAML информацию об объекте и установим кнопку изменения данных:</p>
<pre class="brush:xml;">
&lt;Page
    x:Class="BindingApp.MainPage"
    xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
...
<p>А в коде C# определим свойство MyPhone и обработчик нажатия кнопки:</p>
<pre class="brush:c#;">
public sealed partial class MainPage : Page
{
...
<p>Метод <code>Bindings.Update()</code> все привязки внутри текущей страницы. Без этого вызова мы не увидем изменения цены объекта.</p>
<p>При использовании объекта Bindings Visual Studio может определять данный объект как некорректный. Однако в данном случае не стоит на это обращать внимание, 
так как при компиляции будет создаваться переменная Bindings, которая собственно и будет использоваться.</p>
