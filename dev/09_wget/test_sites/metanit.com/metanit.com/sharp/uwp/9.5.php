...
<html >
<head>
<title>UWP | Меню "Гамбургер" и панель навигации</title>
<meta charset="utf-8" />
<meta name="description" content="Создание панели навигации в Universal Windows Platform, меню в стиле Гамбургер в UWP">
...
<h2>Меню "Гамбургер" и панель навигации</h2><div class="date">Последнее обновление: 29.02.2016</div>
...
<p>Панель навигации представляет один из распространенных способов навигации по разделам внутри приложения. Вместе с панелью навигации, как правило, 
...
когда в нашем приложении есть несколько различных разделов и когда необходимо, чтобы меню скрывалось, освобождало место основному контенту страницы.</p>
<p>Для примера создадим новый проект и прежде всего добавим в него три пустые страницы XAML по шаблону <span class="b">Blank Page</span>, которые назовем 
<i>home.xaml</i>, <i>share.xaml</i> и <i>settings.xaml</i>:</p>
<img src="pics/9.8.png" alt="Blank XAML Page в UWP" />
<img src="pics/9.9.png" alt="Навигация в UWP" />
<p>И на каждой из страниц определим какой-нибудь содержимое, например, просто выведем какой-нибудь заголовок, чтобы эти страницы визуально друг от друга отличались.</p>
<p>Для создания панели навигации используется элемент <span class="b">SplitView</span>. Так, определим в приложении на главной странице MainPage.xaml следующий интерфейс:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Для создания кнопки "Гамбургер" применяется элемент Button со следующими настройками:</p>
<pre class="brush:xml;">
&lt;Button x:Name="HamburgerButton" FontFamily="Segoe MDL2 Assets" FontSize="36" Content="&amp;#xE700;"
</pre>
<p>Шрифт "Segoe MDL2 Assets" имеет встроенные иконки, которые мы можем использовать в качестве значков на кнопках и других элементах управления. "Гамбургер" 
создается с помощью значения "&amp;#xE700;". Все значения для данного шрифта можно посмотреть в таблице системных шрифтов.</p>
<p>В SplitView в виде скрытого списка ListBox помещено меню навигации. А в справа в SplitView находится элемент Frame, в который будут загружаться выше 
определенные страницы home.xaml, share.xaml и settings.xaml.</p>
<p>Перейдем к файлу кода и определим в нем следующее содержимое:</p>
<pre class="brush:c#;">
...
<p>В коде все относительно просто: в обработчике нажатия кнопки переключаем скрытость/раскрытость панели SplitView с меню навигации, а в обработчике выбора 
элемента списка загружаем нужную страницу во фрейм с помощью метода <code>myFrame.Navigate();</code></p>
<img src="pics/9.10.png" alt="Hamburger Menu in UWP" />
<p>Это был так называемый режим наложения, при котором выдвигающаяся панель меню частично перекрывает основное содержимое. Теперь рассмотрим другой режим - компактный. 
Для этого изменим разметку интерфейса:</p>
<pre class="brush:xml;">
&lt;Page
    x:Class="HamburgerMenuApp.MainPage"
    xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
...
<p>Теперь к каждому пункту меню добавились специальные значки - символы из шрифта Segoe MDL2 Assets. Полный набор значков можно посмотреть в таблице шрифтов или 
в <a href="https://msdn.microsoft.com/ru-ru/library/windows/apps/jj841126.aspx" rel="nofollow">msdn</a></p>
<p>Неполное раскрытие панели меню позволяет увидеть значки и выбирать нужный пункт, не раскрывая полностью панель. Хотя в данном случае мы также можем воспользоваться кнопкой-Гамбургером:</p>
<img src="pics/9.11.png" />
<p>Код C# в данном случае остается без изменений.</p>
<p>Есть еще один режим - раскрытый или закрепленный: панель меню закреплена в раскрытом виде. Панель меню может быть частично раскрыта, либо полностью. В  этом случае кнопка-гамбургер может не потребоваться, 
...
поэтому полностью раскрытый режим рекомендуется только для экранов с шириной о 720px и больше. Для этого режима изменим определение панели SplitView:</p>
<pre class="brush:xml;">
&lt;SplitView Name="mySplitView" Grid.Row="1" OpenPaneLength="200" 
...
<div class="nav"><p><a href="9.4.php">Назад</a><a href="./">Содержание</a><a href="10.1.php">Вперед</a></p></div></div>
<div class="socBlock">
