...
<html >
<head>
<title>UWP | FlipView</title>
<meta charset="utf-8" />
<meta name="description" content="Элемент FlipView в Universal Windows Platform, его основные свойства и методы, эффект перелистывания представлений на странице">
...
<h2>FlipView</h2><div class="date">Последнее обновление: 14.04.2017</div>
...
<p>FlipView представляет элемент, который содержит несколько объектов (например, изображений), которые мы можем пролистывать. 
...
например, фотографий в фотоальбоме или карточек с информацией о товаре.</p>
<p>FlipView наследуется от класса <span class="b">ItemsControl</span> и поэтому может содержать коллекцию элементов любого типа. Поэтому мы 
можем либо определить все ее элементы через свойство Items, либо привязать список объектов к свойству ItemsSource элемента FlipView.</p>
<p>Например, определим простейший FlipView:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Здесь FlipView содержит два элемента StackPanel, каждый из которых представляет как бы отдельную страницу. Фактически эти элементы StackPanel будут помещаться в  И если мы запустим проект, то в итоге 
мы увидим на FlipView содержимое только первого StackPanel. Однако сбоку нам будет доступна кнопка для перехода к следующему StackPanel:</p>
<img src="pics/4.52.png" alt="Элемент FlipView в Universal Windows Platform" />
<p>Теперь определим в FlipView более сложное содержимое. Пусть в проекте есть следующий класс Phone:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Пусть в проекте в папке Assets хранится несколько картинок, которые мы можем использовать для объектов Phone.</p>
<p>В классе MainPage определим источник данных в виде свойства Phones:</p>
<pre class="brush:c#;">
...
<p>В коде xaml у страницы MainPage определим привязку к источнику данных:</p>
<pre class="brush:xml;">
&lt;Page
    x:Class="DataApp.MainPage"
    xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
...
<img src="pics/4.62.png" alt="Элемент FlipView в Universal Windows Platform" />
<h3>Ориентация FlipView</h3>
<p>В качестве панели для расположения элементов в FlipView используется элемент VirtualizingStackPanel - своего рода StackPanel, который имеет по умолчанию горизонтальную 
...
<img src="pics/4.63.png" alt="VirtualizingStackPanel и FlipView в UWP" />
