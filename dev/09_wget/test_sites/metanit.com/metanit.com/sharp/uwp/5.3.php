...
<html >
<head>
<title>UWP | Конвертеры значений</title>
<meta charset="utf-8" />
...
<h2>Конвертеры значений</h2><div class="date">Последнее обновление: 11.02.2016</div>
...
<p>Конвертеры значений (<span class="b">value converter</span>) позволяют преобразовать значения, приходящие от источника привязки, к тому 
...
конвертер значений.</p>
<p>Возьмем, к примеру, привязку элемента DatePicker к TextBlock. Допустим, мы хотим вывести выбранную дату в текстовое поле. Зная основы привязки, мы бы сделали так:</p>
<pre class="brush:xml;">
...
<p>Однако практического результата мы бы не получили:</p>
<img src="pics/6.5.png" />
<p>Все потому что свойство Date объекта DatePicker представляет объект DateTimeOffset, и система просто не знает, как его преобразовать в строку. 
В этом случае нам как раз понадобится конвертер значений.</p>
<p>Теперь добавим в проект следующий класс:</p>
<pre class="brush:c#;">
using System;
...
<p>Конвертер значений должен реализовать интерфейс <span class="b">Windows.UI.Xaml.Data.IValueConverter</span>. В нем определяюся два метода: 
<code>Convert()</code> (преобразует значение от источника привязки в тип, понятный для приемника привязки) и <code>ConvertBack()</code> 
(выполняет противоположную операцию). Оба метода принимают следующие параметры:</p>
<ul>
...
<li><p><code>object parameter</code>: вспомогательный параметр привязки</p></li>
<li><p><code>string language</code>: языковой код текущей культуры, например, "en-US"</p></li>
</ul>
<p>В нашем конвертере значений метод <code>Convert</code> возвращает строковое представление даты в формате "dd.MM.yyyy". То есть мы ожидаем, что в 
качестве параметра value будет передаваться объект DateTimeOffset.</p>
<p>Метод <code>ConvertBack</code> выполняет обратное преобразование - из строки в DateTimeOffset.</p>
<p>Используем наш конвертер в xaml:</p>
<pre class="brush:xml;">
&lt;Page
...
<p>Здесь объект конвертера значений задан как ресурс. Чтобы применить этот конвертер в конструкции привязки используется параметр 
<span class="b">Converter</span> с ссылкой на ресурс: <code>Converter={StaticResource myDateConverter}</code></p>
<p>И теперь текстовый блок будет выводить дату как и должен:</p>
<img src="pics/6.6.png" alt="Конвертер значений в Universal Windows Platform" />
<p>Кроме конвертера мы также можем использовать параметры <span class="b">ConverterParameter</span> и <span class="b">ConverterLanguage</span>. 
В конвертере значений это соответственно параметры <code>object parameter</code> и <code>string language</code>. Применим их, изменим код конвертера:</p>
<pre class="brush:c#;">
public class DateTimeOffsetToStringConverter : IValueConverter
{
    public object Convert(object value, Type targetType, object parameter, string language)
...
<p>В качестве параметра может передаваться любой объект. Если параметр в xaml не используется, то передается null. В данном случае мы проверяем, равен ли 
...
Также если язык имеет код "en-US", то возвращает дату в другом формате.</p>
...
<img src="pics/6.7.png" alt="Value Converter в UWP" />
