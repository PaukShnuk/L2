...
<html >
<head>
<title>UWP | ListView</title>
<meta charset="utf-8" />
<meta name="description" content="Элемент ListView в Universal Windows Platform, его основные свойства и методы, создание сложных списков в UWP">
...
<h2>ListView</h2><div class="date">Последнее обновление: 14.04.2017</div>
...
<p>ListView отображает информацию на множестве строк и столбцов. Он унаследован от класса ListBox, поэтому может вести себя простой список:</p>
<pre class="brush:xml;">
&lt;Page
...
<img src="pics/4.49.png" alt="ListView в Universal Windows Platform" />
<p>Однако, как правило, ListView применяется для отображения сложных по структуре данных. Например, пусть у нас в проекте есть класс Phone:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>И определим код XAML, который будет отображать данные данного типа:</p>
<pre class="brush:xml;">
&lt;Page
    x:Class="DataApp.MainPage"
    xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
...
<img src="pics/4.50.png" alt="ListView в UWP" />
<p>В данном случае для отображения коллекции сложных объектов необходимо настроить шаблон данных, который определяется свойством <span class="b">ItemTemplate</span>. 
В качестве параметра он принимает объект <span class="b">DataTemplate</span>, который устанавливает шаблон списка в виде грида или таблицы. В этой таблице три столбца. 
Для каждого столбца определяется элемент <code>TextBlock</code>, который с помощью выражения <code>{Binding Свойство}</code> привязывается к определенному свойству 
объекта Phone.</p>
<p>А с помощью свойства <code>ListView.Items</code> устанавливается содержимое списка. При этом если мы не зададим шаблон данных для ListView, то он не 
сможет, как отображать эти данные.</p>
<h3>Установка панели ListView</h3>
<p>Можно обратить внимание, что в ListView все элементы располагаются последовательно сверху вниз. Это связано с тем, что в качесте панели для отображения элементов 
ListView использует объект <span class="b">ItemsStackPanel</span>. Это своего рода стек, который мы можем располагать как вертикально, так и горизонтально. 
Например, расположим элементы горизонтально:</p>
<pre class="brush:xml;">
&lt;ListView x:Name="phonesList"
	Height="60" 
...
<p>Для установки панели элементов у всех списоков элементов управления определено свойство <span class="b">ItemsPanel</span>. Оно принимает в качестве значения 
объект <span class="b">ItemsPanelTemplate</span>, который указывает на панель элементов. В данном случае используется горизонтальная панель ItemsStackPanel. 
Но мы можем использовать и другие панели. Например, используем панель <span class="b">ItemsWrapGrid</span>, которая располагает элементы друг за другом, заполняя все пространство ListView.</p>
<pre class="brush:xml;">
&lt;ListView Height="100"
    ScrollViewer.HorizontalScrollMode="Enabled" 
    ScrollViewer.HorizontalScrollBarVisibility="Auto"&gt;
...
<img src="pics/4.61.png" alt="ItemsPanel в ListView в UWP" />
...
<div class="nav"><p><a href="4.15.php">Назад</a><a href="./">Содержание</a><a href="4.18.php">Вперед</a></p></div></div>
<div class="socBlock">
