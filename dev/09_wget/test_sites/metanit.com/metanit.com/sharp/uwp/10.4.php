...
<html >
<head>
<title>UWP | Адаптивный код C#</title>
<meta charset="utf-8" />
<meta name="description" content="Адаптация кода C# в Universal Windows Platform под различные устройства для создания адаптивного дизайна">
...
<h2>Адаптивный код C#</h2><div class="date">Последнее обновление: 20.02.2016</div>
...
<p>Использование адаптивных XAML-представлений и визуальных состояний позволяет создавать адаптивную разметку xaml, которая будет меняться в зависимости от 
...
программно. А десктопах естественно нам эта функция не нужна.</p>
<p>Чтобы сделать код C# адаптивным, нам надо выполнить пару шагов. Вначале нам надо добавить в в проект соответствующие расширения SDK. Допустим, мы хотим добавить 
...
в выпадающем меню выберем пункт <span class="b">Add Reference</span>. После этого нам открывается окно добавления библиотеки в проект. В этом окне 
слева выберем <span class="b">Universal Windows - &gt; Extensions</span>, а в центральном поле выберем пункт <span class="b">Windows Mobile Extensions for the UWP</span>:</p>
<img src="pics/10.5.png" alt="Mobile Extensions for UWP" />
<p>Эти расширения позволяют нам добавить в приложение тот функционал, который отсутствует в универсальной платформе, но который может применяться на конкретных устройствах - 
десктопах, смартфонах или IoT.</p>
<p>Если есть несколько версий одного и того же расширения, то лучше взять более новую версию.</p>
<p>Второй этап заключается в использовании класса <span class="b">Windows.Foundation.Metadata.ApiInformation</span>. Этот класс позволяет проверить наличие на текущем устройстве 
определенных функциональностей. Для этого в нем определены следующие методы:</p>
<ul>
<li><p><code>IsApiContractPresent</code>: возвращает true, если устройство поддерживает определенный API</p></li>
<li><p><code>IsEnumNamedValuePresent</code>: возвращает true, если устройство поддерживает определенную константу из определенного перечисления</p></li>
<li><p><code>IsEventPresent</code>: возвращает true, если устройство поддерживает определенное событие для определенного типа</p></li>
<li><p><code>IsMethodPresent</code>: возвращает true, если устройство поддерживает определенный метод для определенного типа</p></li>
<li><p><code>IsPropertyPresent</code>: возвращает true, если устройство поддерживает определенное свойство для определенного типа</p></li>
<li><p><code>IsReadOnlyPropertyPresent</code>: возвращает true, если устройство поддерживает определенное свойство только для чтения для определенного типа</p></li>
<li><p><code>IsWriteablePropertyPresent</code>: возвращает true, если устройство поддерживает определенное свойство только для записи для определенного типа</p></li>
<li><p><code>IsTypePresent</code>: возвращает true, если устройство поддерживает определенный тип</p></li>
</ul>
<p>Так, наша задача состоит в обработке кнопки Назад на мобильных устройствах. Кнопка Назад представлена классом <span class="b">Windows.Phone.UI.Input.HardwareButtons</span>. 
Поэтому изменим код MainPage.xaml.cs следующим образом:</p>
<pre class="brush:c#;">
...
<pre class="brush:c#;">if (ApiInformation.IsTypePresent("Windows.Phone.UI.Input.HardwareButtons"))</pre>
<p>позволяет проверить на запускаемом устройстве наличие кнопки Назад. Если она есть, то прикрепляем обработчик этой кнопки.</p>
<p>Если бы мы не использовали проверку на наличие типа и просто устанавливали бы обработчик события, то при запуске на десктопе приложение вываливалось бы в ошибку. 
Поэтому вещи, которые являются специфичными для определенной платформы, необходимо проверять перед использованием.</p>
<p>Аналогично мы бы могли использовать для проверки наличия функционала и другие методы класса ApiInformation. Например, проверка на событие:</p>
<pre class="brush:c#;">if (ApiInformation.IsEventPresent(typeof(HardwareButtons).FullName, "BackPressed"))</pre>
<p>Проверка на контракт API:</p>
<pre class="brush:c#;">if (ApiInformation.IsApiContractPresent("Windows.Phone.PhoneContract", 1))</pre>
<p>В данном случае проверяется наличие контракта <code>Windows.Phone.PhoneContract</code>, который применяется на мобильных устройствах, причем проверяется именно 1-я версия. 
Собственно на данный момент это актуальная версия. Более точно версию API можно посмотреть в окне Object Browser.</p>
...
<div class="nav"><p><a href="10.3.php">Назад</a><a href="./">Содержание</a><a href="4.13.php">Вперед</a></p></div></div>
<div class="socBlock">
