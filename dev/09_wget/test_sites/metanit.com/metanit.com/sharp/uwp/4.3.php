...
<html >
<head>
<title>UWP | Кнопки</title>
<meta charset="utf-8" />
<meta name="description" content="Обзор кнопок в Universal Windows Platform, классы Button, ToggleButton, HyperlinkButton, их основые свойства и методы, примеры использования">
...
<h2>Кнопки</h2><div class="date">Последнее обновление: 11.02.2016</div>
...
<h3>Button</h3>
<p>Элемент Button представляет кнопку:</p>
<pre class="brush:xml;">&lt;Button x:Name="button1" Width="100" Height="40" Background="LightGray" Content="Hello UWP" /&gt;</pre>
<p>В основном кнопки служат для инициирования пользователем какого-либо действия по нажатию. Чтобы обработать нажатие кнопки, надо установить 
для ее события <code>Click</code> кнопки обработчик. Например, код xaml:</p>
<pre class="brush:xml;">&lt;Button x:Name="button1" Width="100" Height="40" Background="LightGray" Content="Hello UWP" Click="Button_Click" /&gt;</pre>
<p>Тогда в файле кода c# для данной кнопки можно было бы прописать обработчик:</p>
<pre class="brush:c#;">
...
<p>Создание аналогичной кнопки в коде C#:</p>
<pre class="brush:c#;">
...
<h3>ToggleButton</h3>
<p>ToggleButton представляет простой переключатель, который может пребывать в трех состояниях - отмеченном, неотмеченном и неопределенном. 
Состояние можно установить или получить с помощью свойства <code>IsChecked</code>. Оно принимает три значения: <code>True</code> (отмеченное состояние), 
<code>False</code> (неотмеченное состояние) и <code>x:Null</code> (или <code>null</code> в коде c#) - неопределенное состояние.</p>
<p>Чтобы кнопка могла переключаться во все три состояния, у ней надо установить свойство <code>IsThreeState="True"</code>, иначе для нее будут доступны только 
два первых состояния:</p>
<pre class="brush:xml;">
...
<p>И в файле кода c# зададим вывод в текстовый блок состояния toggleButton1:</p>
<pre class="brush:c#;">
...
<img src="pics/4.17.png" alt="ToggleButton in Universal Windows Platform" />
<p>Как правило, ToggleButton сама по себе редко используется, чаще применяются производные классы в виде CheckBox и RadioButton.</p>
<h3>HyperlinkButton</h3>
<p>HyperlinkButton предназначен для вывода ссылок, во всем остальном это обычная кнопка. Для установки ссылки в элементе определено свойство 
<span class="b">NavigateUri</span>:</p>
<pre class="brush:xml;">&lt;HyperlinkButton NavigateUri="metanit.com" Content="Перейти на сайт" Foreground="Red" /&gt;</pre>
<img src="pics/4.18.png" alt="HyperlinkButton in Universal Windows Platform" />
<p>При нажатии на ссылку открывается браузер Microsoft Edge, который переходит по ссылке.</p>
