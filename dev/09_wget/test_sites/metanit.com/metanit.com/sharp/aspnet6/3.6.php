...
<html >
<head>
<title>ASP.NET Core и C# | Сопоставление запроса с конечной точкой</title>
<meta charset="utf-8" />
<meta name="description" content="Сопоставление запроса с конечной точкой в ASP.NET Core и C#, критерии выбора конечной точки endpoint для обработки запроса на основе шаблона маршрута и его параметров">
...
<h2>Сопоставление запроса с конечной точкой</h2><div class="date">Последнее обновление: 19.12.2021</div>
...
<p>Сопоставление адреса URL или <span class="b">URL matching</span> представляет процесс сопоставления запроса с конечной точкой. Данный процесс основывается 
на пути запроса и полученных в запросе заголовках. Данный процесс проходит ряд этапов:</p>
<ol>
<li><p>Сначала выбираются все конечные точки, шаблон маршрута которых совпадает с путем запроса</p></li>
<li><p>Далее из полученного на предыдущем этапе набора конечных точек удаляются те, 
которые не соответствуют ограничениям маршрута</p></li>
<li><p>Затем из полученного на предыдущем этапе набора конечных точек удаляются те, которые не удавлетворяют политике объекта MatcherPolicy 
(вкратце: класс <span class="b">MatcherPolicy</span> позволяет определить порядок сравнения конечных точек и адреса URL)</p></li>
<li><p>И в самом конце применяется объект <span class="b">EndpointSelector</span> для выбора из полученного на предыдущем этапе списка конечной точки, которая в конечном счете и 
будет обрабатывать запрос</p></li>
</ol>
<p>Приоритет конечных точек зависит от двух факторов:</p>
<ul>
<li><p>Порядо следования в наборе конечных точек</p></li>
<li><p>Приоритетность шаблона маршрута</p></li>
</ul>
<p>Приоритетность шаблонов маршрута зависит от специфичности шаблона. Специфичность шаблона определяется на основе следующих критериев:</p>
<ul>
<li><p>Шаблон маршрута с большим количеством сегментов более специфичен, чем шаблон меньшим количеством сегментов</p></li>
<li><p>Сегмент с текстовым литералом (статический сегмент) более специфичен, чем сегмент с параметром маршрута</p></li>
<li><p>Сегмент с параметром, к которому применяется ограничение маршрута, более специфичен, чем сегмент с параметром без ограничения</p></li>
<li><p>Комплексный сегмент более специфичен, чем сегмент с параметром с ограничением</p></li>
<li><p>Параметр catch-all (параметр, который соответствует неопределенному количеству сегментов) наименее специфичен</p></li>
</ul>
<p>Если в конечном счете осталось две и более конечных точек, которые соответствуют запрошенному адресу, и соответственно система маршрутизации не может 
выбрать, какая из этих конечных точек должна обрабатывать маршрут, то генерируется исключение.</p>
<p>Например, пусть у нас есть два шаблона маршрута: "/hello" и "/{message}".</p>
<pre class="brush:c#;">
...
<p>Оба этих маршрута соответствуют пути запроса "/hello". 
Однако шаблон маршрута "/{message}" более общий - вместо параметра <code>message</code> может идти что угодно. Тогда как шаблон "/hello" состоит из статического сегмента и соответственно 
более конкретный, более специфичный, поэтому конечная точка этого шаблона маршрута и будет выбрана для обработки запроса по пути "/hello".</p>
<img src="./pics/3.20.png" alt="URL matching in ASP.NET Core и C#" />
<p>Другой пример:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Здесь первый шаблон маршрута применяет необязательный параметр message. Второй шаблон маршрута соответствует корню веб-приложения. И в принципе оба 
этих шаблона соответствуют пути запроса "/". Однако второй шаблон представляет статический сегмент, поэтому его конечная точка будет выбрана в конечном счете для обработки запроса:</p>
<img src="./pics/3.21.png" alt="Сопоставление шаблона маршрута с запросом в ASP.NET Core и C#" />
...
<p>Здесь опять же поскольку во втором маршруте первый сегмент представляет статический сегмент, то именно вторая конечная точка будет выбираться для обработки маршрута:</p>
<img src="./pics/3.22.png" alt="Приоритет шаблонов маршрутов в ASP.NET Core и C#" />
