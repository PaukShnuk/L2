...
<html >
<head>
<title>ASP.NET Core и C# | Нефайловые провайдеры конфигурации</title>
<meta charset="utf-8" />
<meta name="description" content="Нефайловые провайдеры конфигурации в ASP.NET Core и C#, хранение настроек в памяти в словарях Dictionary, загрузка переменных среды окружения и аргументов из командной строки">
...
<h2>Нефайловые провайдеры конфигурации</h2><div class="date">Последнее обновление: 21.12.2021</div>
...
<h3>Загрузка аргументов командной строки</h3>
<p>Для загрузки аргументов командной строки в конфигурацию приложения применяется провайдер <span class="b">CommandLineConfigurationProvider</span>. 
Для применения этого провайдера у объекта ConfigurationManager (свойство Configuration объекта WebApplicationBuilder) 
вызывается метод <span class="b">AddCommandLine()</span>, в который передаются аргументы командной строки. 
Но поскольку приложение по умолчанию загружает аргументы командной строки в объект конфигурации, который передается в классы приложения через dependency 
injection, то нам нет смысла явным образом его вызывать.</p>
<p>Каждый аргумент командной строки, который мы хотим использовать в качестве параметра конфигурации, должен представлять пару ключ-значение. 
Есть разные способы определения таких параметров. Самый распространненый - после ключа после знака равно = указывается значение, а через пробел последующие параметры:</p>
<pre class="brush:c#;">
key1=value1 key2=value2 key3=value3
</pre>
<p>Также можно использовать другие способы:</p>
<pre class="brush:c#;">
key1=value1 --key2=value2 /key3=value3
--key1 value1 /key2 value2
key1= key2=value2
</pre>
<p>Перед ключом можно указать префикс -- или /, тогда между ключом и значением можно опустить знак равно = и оставить просто пробел.</p>
<p>Также если мы хотим указать ключ, но не хотим указывать значение, то после знака равно можно не указывать значение.</p>
<h4>Тестирование в Visual Studio</h4>
<p>Для тестирования передачи параметров командной строки изменим в проекте в папке <span class="b">Properties</span> файл 
<span class="b">launchSettings.json</span>.</p>
<img src="./pics/6.7.png" alt="launchSettings.json и установка аргументов командной строки в ASP.NET Core и C#" />
<p>По умолчанию он выглядит примерно следующим образом:</p>
<pre class="brush:js;">
{
  "iisSettings": {
...
<p>Перейдем в этом файле к элементу "profiles". Этот элемент по умолчанию содержит два профиля, которые могут применяться для запуска. Один профиль называется 
по имени проекта (в моем случае HelloApp). Второй профиль называется "IIS". Выберем профиль, который мы используем для запуска проекта и добавим в этот элемент строку</p>
<pre class="brush:js;">
"commandLineArgs": "name=Bob age=37",
</pre>
<p>Параметр "commandLineArgs" позволяет определить данные командной строки, которые будут передаваться приложению. 
В данном случае предполагается, что через командную строку будут передаваться параметр name со значением "Bob" и параметр age со значением "37". 
Например, я запускаю проект как консольное приложение, поэтому я изменяю профиль по имени проекта - HelloApp. Который после изменения будет 
...
<p>Определим в приложении простейший код для получения значений конфигурации:</p>
<pre class="brush:c#;">
...
<p>Здесь важно отменить передачу параметров командной строки - <span class="b">args</span> в метод <code>WebApplication.CreateBuilder(args)</code>. Благодаря этому 
приложение автоматически может подхватить параметры командной строки. И после запуска мы увидим указанные значения параметров конфигурации:</p>
<img src="./pics/6.2.png" alt="Установка конфигурации командной строки в ASP.NET Core и C#" />

<h4>Запуск через консоль</h4>
<p>Используя CLI, мы можем запустить приложение из командной строки и передать ему параметры.</p>
<p>Итак, удалим выше определенные параметры и выполним перестроение проекта. Откроем командную строку и перейдем в консоли с помощью команды 
cd в папку проекта. В моем случае это папка <code>C:\Users\Eugene\Source\Repos\CSharp\ASPNET\HelloApp\HelloApp</code>.</p>
<p>Далее введем следующую команду:</p>
<div class="console">
<pre class="consoletext">
dotnet run name=Tom age=35
</pre>
</div>
<img src="./pics/6.3.png" alt="dotnet run и конфигурация командной строки в ASP.NET Core и C#" />
<p>В итоге приложение подхватит переданные данные:</p>
<img src="./pics/6.4.png" alt="CommandLineConfigurationProvider в ASP.NET Core и C#" />
<h4>Программная симуляция аргументов командной строки</h4>
<p>Также мы можем на уровне кода симулировать передачу параметров командной строки:</p>
<pre class="brush:c#;">
...
<h4>Применение метода AddCommandLine</h4>
<p>Также можно было бы передать параметры командной строки через метод <span class="b">AddCommandLine()</span>:</p>
<pre class="brush:c#;">
...
<h3>Переменные среды окружения как источник конфигурации</h3>
<p>Для загрузки переменных среды окружения в качестве параметров конфигурации применяется провайдер <span class="b">EnvironmentVariablesConfigurationProvider</span>. 
Для его использования у объекта ConfigurationManager вызывается метод <span class="b">AddEnvironmentVariables()</span>. Однако в реальности 
вряд ли его придется часто использовать, так как среда ASP.NET Core уже загружает переменные среды окружения в объект конфигурации по умолчанию.</p>
<p>Например, получим переменную окружения "JAVA_HOME", которая указывает на папку установки java sdk, если эта переменная определена:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/6.5.png" alt="Получение переменных среды из конфигурации в ASP.NET Core и C#" />
<p>Явное применение метода <code>AddEnvironmentVariables()</code>:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<h3>Хранение конфигурации в памяти</h3>
<p>Провайдер <span class="b">MemoryConfigurationProvider</span> позволяет использовать в качестве конфигурации коллекцию 
<code>IEnumerable&lt;KeyValuePair&lt;string, string&gt;&gt;</code>, 
которая хранит данные в виде пары ключ-значение (пример - объект Dictionary). Для добавления источника конфигурации применяется метод <span class="b">AddInMemoryCollection()</span>, в который передается словарь конфигурационных настроек:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
