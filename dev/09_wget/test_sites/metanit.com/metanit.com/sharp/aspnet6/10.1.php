...
<html >
<head>
<title>ASP.NET Core и C# | Введение в Results API</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в Results API в ASP.NET Core и C#, интерфейс IResult, возвращение результата из конечной точки endpoint">
...
<h1>Results API</h1><h2>Введение в Results API</h2><div class="date">Последнее обновление: 24.12.2021</div>
...
<p>Для упрощения отправки ответа ASP.NET Core предоставляет специальный API - <span class="b">Results API</span>. Он реализуется посредством  методов статического класса 
<span class="b">Results</span>:</p>
<ul>
<li><p><span class="b">Accepted()</span>: отправляет клиенту статусный код 202, который указывает, что запрос принят к обработке. Может 
принимать два необязательных параметра: первый параметр представляет адрес для отслеживания состояния запроса, а второй - произвольный объект, оправляемый в ответ клиенту</p></li>	
<li><p><span class="b">AcceptedAtRoute()</span>: также отправляет клиенту статусный код 202. Может 
принимать три необязательных параметра: первый параметр представляет имя маршрута, а второй - параметры для этого маршрута, третий - произвольный объект, оправляемый в ответ клиенту</p></li>	
<li><p><span class="b">BadRequest()</span>: отправляет клиенту статусный код 400, который указывает, что запрос не может быть обработан в силу содержащихся в нем ошибок. 
Может принимать необязательный параметр - произвольный объект, отправляемый в ответ клиенту</p></li>	
<li><p><span class="b">Bytes()</span>: отправляет в ответ клиенту массив байтов, приняется обычно для отправки файлов</p>
<p>Этот метод по своему действию аналогичен методу</p>
<pre class="brush:c#;">File(Byte[], String, String, Boolean, Nullable&lt;DateTimeOffset&gt;, EntityTagHeaderValue)</pre></li>
<li><p><span class="b">Challenge()</span>: создает объект <code>IResult</code>, для которого выполняется метод 
<code>ChallengeAsync(HttpContext, String, AuthenticationProperties)</code></p>
<p>Поведение этого метода зависит от сервиса IAuthenticationService. В качестве возможных ответов могут статусные коды 401 (не авторизирован) и 
403 (доступ запрещен)</p></li>
<li><p><span class="b">Conflict()</span>: отправляет ответ со статусным кодом 409</p></li>
<li><p><span class="b">Content()</span>: отправляет в ответе некоторую строку</p></li>
<li><p><span class="b">Created()</span>: отправляет ответ со статусным кодом 201</p></li>
<li><p><span class="b">CreatedAtRoute()</span>: отправляет ответ со статусным кодом 201</p></li>
<li><p><span class="b">File()</span>: отправляет в ответ файл</p></li>
<li><p><span class="b">Forbid()</span>: создает объект <code>IResult</code>, для которого выполняется метод 
<code>ForbidAsync(HttpContext, String, AuthenticationProperties)</code>. По умолчанию отправляет статусный код 403 (доступ запрещен)</p></li>
<li><p><span class="b">Json()</span>: отправляет ответ в формате JSON</p></li>
<li><p><span class="b">LocalRedirect()</span>: перенаравляет  на один из локальных адресов</p></li>
<li><p><span class="b">NoContent()</span>: отправляет статусный код 204</p></li>
<li><p><span class="b">NotFound()</span>: отправляет ответ со статусным кодом 404</p></li>
<li><p><span class="b">Ok()</span>: отправляет ответ со статусным кодом 200</p></li>
<li><p><span class="b">Problem()</span>: применяет формат <a href="https://datatracker.ietf.org/doc/html/rfc7807" rel="nofollow">ProblemDetails</a> 
для отправки ответа.</p></li>
<li><p><span class="b">Redirect()</span>: перенаравляет на определенный адрес</p></li>
<li><p><span class="b">RedirectToRoute()</span>: перенаравляет на определенный маршрут</p></li>
<li><p><span class="b">SignIn()</span>:  создает объект <code>IResult</code>, для которого выполняется метод 
<code>SignInAsync(HttpContext, String, ClaimsPrincipal, AuthenticationProperties)</code></p></li>
<li><p><span class="b">SignOut()</span>: создает объект <code>IResult</code>, для которого выполняется метод 
<code>SignOutAsync(HttpContext, String, AuthenticationProperties)</code></p></li>
<li><p><span class="b">StatusCode()</span>: отправляет определенный статусный код</p></li>
<li><p><span class="b">Stream()</span>: пишет в ответ поток данных. По своему действию аналогичен методу 
<code>File(Stream, String, String, Nullable&lt;DateTimeOffset&gt;, EntityTagHeaderValue, Boolean)</code></p></li>
<li><p><span class="b">Text()</span>: отправляет в ответ некоторое текстовое содержимое. Аналогичен методу <code>Content(String, String, Encoding)</code></p></li>
<li><p><span class="b">Unauthorized()</span>: отправляет статусный код 401</p></li>
<li><p><span class="b">UnprocessableEntity()</span>: отправляет ответ со статусным кодом 422</p></li>
<li><p><span class="b">ValidationProblem()</span>: отправляет ответ со статусным кодом 400</p></li>
</ul>
<p>Применим один из методов:</p>
<pre class="brush:c#;">
...
<p>В данном случае в приложении определены две конечные точки: одна обрабатывает запросы по маршруту "/hello", а другая - запросы к корню веб-приложения. 
Первая конечная точка для отправки ответа использует метод <code>Results.Text()</code>. Другая конечная точка напрямую возвращает некоторую строку. Однако 
...
метод <code>Reqults.Text</code> добавляет заголовок "Content-Length").</p>
<img src="./pics/10.1.png" alt="Results API и IResult в ASP.NET	Core и C#" />
<p>Подобным обазом можно обрабатывать запрос в отдельном методе, который возвращает объект IResult:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Однако несмотря на то, что из конечных точек мы можем напрямую отправляться некоторый ответ без методов Results, тем не менее методы Results API могут быть полезны, 
когда необходимо установить какие-то дополнительные параметры ответа, например, кодировку, форматирование в json. И конечно мы можем использовать методы Results API 
для отправки в конечных точках статусных кодов, файлов или выполнения редиректов.</p>
<p>Хотя Results API в большей степени предназначено для использования в конечных точках ASP.NET Core, те же методы могут применяться для отправки ответа и в обычных компонентах 
middleware. Например:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
