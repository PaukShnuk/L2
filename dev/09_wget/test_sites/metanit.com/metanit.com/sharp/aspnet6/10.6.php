...
<html >
<head>
<title>ASP.NET Core и C# | Определение своего типа IResult</title>
<meta charset="utf-8" />
<meta name="description" content="Определение своего типа IResult в Results API для отправки кода HTML в ASP.NET Core и C#, реализация метода ExecuteAsync">
...
<h2>Определение своего типа IResult</h2><div class="date">Последнее обновление: 06.02.2022</div>
...
<p>Встроенные типы Result API покрывают различные ситуации. Однако встроенных типов может оказаться недостаточно. И в этом случае мы можем определить 
свой тип IResult.</p>
<p>Например, по умолчанию Results API не предоставляет типа для отправки кода html. Рассмотрим, как мы можем сделать IResult для отправки html.</p>
<p>Для этого определим следующий код приложения:</p>
<pre class="brush:c#;">
...
<p>Ключевым моментом здесь является класс <span class="b">HtmlResult</span>, который собственно и отправляет html-код:</p>
<pre class="brush:c#;">
class HtmlResult: IResult
{
    string htmlCode = "";
...
<p>Он должен реализовать интерфейс <span class="b">IResult</span>, который определяет метод <span class="b">ExecuteAsync()</span></p>
<p>Через конструктор класса получаем отправляемый код html. А в методе <code>ExecuteAsync()</code> через параметр типа HttpContext 
установливаем заголовок и отправляем html-код.</p>
<p>Для создания данного объекта можно определить метод расширения для типа IResultExtensions:</p>
<pre class="brush:c#;">
static class ResultsHtmlExtension
{
    public static IResult Html(this IResultExtensions ext, string htmlCode) =&gt; new HtmlResult(htmlCode);
...
<p>Затем мы можем применить данный метод для отправки ответа при обращении к какой-нибудь конечной точке:</p>
<pre class="brush:c#;">
app.Map("/", () =&gt; Results.Extensions.Html(@"&lt;!DOCTYPE html&gt;...."));
</pre>
<p>Хотя в приницпе также можно было бы напрямую создать объект HtmlResult:</p>
<pre class="brush:c#;">
app.Map("/", () =&gt; new HtmlResult@"&lt;!DOCTYPE html&gt;...."));
</pre>
<p>Но в любом случае при обращении по адресу "/" приложение отправит html-код, который отобразит браузер.</p>
<img src="./pics/10.13.png" alt="Создание своего типа IResult для отправки кода html в ASP.NET Core и C#" />
...
<div class="nav"><p><a href="./10.5.php">Назад</a><a href="./">Содержание</a><a href="./11.1.php">Вперед</a></p></div></div>
<div class="socBlock">
