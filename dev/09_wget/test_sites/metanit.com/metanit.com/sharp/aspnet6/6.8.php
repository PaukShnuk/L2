...
<html >
<head>
<title>ASP.NET Core и C# | Передача конфигурации через IOptions</title>
<meta charset="utf-8" />
<meta name="description" content="Передача конфигурации через IOptions  и проекция конфигурации и ее секций на классы в ASP.NET Core и C#, паттерн Options">
...
<h2>Передача конфигурации через IOptions</h2><div class="date">Последнее обновление: 22.12.2021</div>
...
<p>Фреймворк ASP.NET Core реализует паттерн <span class="b">Options</span>, который позволяет передавать конфигурацию не просто 
как набор настроек в виде пар ключ-значение, а как объекты определенных классов.</p>
<p>Для применения этого паттерна в приложении у объекта IServiceCollection, который представляет коллекцию сервисов приложения, определен метод 
<span class="b">Configure()</span>:</p>
<pre class="brush:c#;">
...
<p>Этот метод реализован как метод расширения для типа IServiceCollection. И все версии метода типизируются типом, объект которого надо передавать через 
механизм внедрения зависимостей. И также все версии метода принимают в качестве одного из параметров объект конфигурации, на основе которой будет создаваться объект TOptions.</p>
<p>Допустим, у нас в проекте определен файл конфигурации <span class="b">person.json</span> со следующим содержимым:</p>
<pre class="brush:js;">
{
  "age": "37",
  "name": "Tom",
...
<p>Данный файл по сути описывает одного пользователя. Элемент name сопоставляется с именем пользвателя, age - с возрастом, languages представляет языки, которыми владеет 
...
Для этого добавим вначале в проект класс <span class="b">Person</span>:</p>
<pre class="brush:c#;">
public class Person
{
...
<p>Для представления компании пользователя определен дополнительный класс Company. Но, как можно заметить, определение класса Person совпадает со структурой json-файла.</p>
<p>И чтобы передать конфигурационные настройки через объект Person, мы можем использовать сервис <span class="b">IOptions&lt;TOptions&gt;</span>:</p>
<pre class="brush:c#;">
...
<p>Прежде всего необходимо связать объект Person, который будет передаваться через механизм внедреия зависимостей, с конфигурацией файла json. 
Для этого метод <code>builder.Services.Configure()</code> типизирует типом Person и в качестве параметра получает конфигурацию приложения 
(свойство builder.Configuration реализует интерфейс IConfiguration и поэтому может передаваться в качестве параметра):</p>
<pre class="brush:c#;">builder.Services.Configure&lt;Person&gt;(builder.Configuration);</pre>
<p>Далее через механиз внедрения зависимостей мы можем получить созданный объект через сервис <code>IOptions&lt;Person&gt;</code></p>:
<pre class="brush:c#;">
app.Map("/", (IOptions&lt;Person&gt; options) =&gt;
{
    Person person = options.Value;  // получаем переданные через Options объект Person
...
<p>Причем через механизм DI передается не просто объект Person, а объект <span class="b">IOptions&ltPerson&gt;</span>, из которого мы можем получим непосредственно 
сам объект Person с помощью свойства <span class="b">Value</span>.</p>
<img src="./pics/6.20.png" alt="Паттерн Options и проекция конфигурации на классы в ASP.NET Core и C#" />
<p>Другой пример: определим в проекте новый класс middleware - <span class="b">PersonMiddleware</span>, который фактически будет выводить информацию о пользователе на веб-станицу:</p>
<pre class="brush:c#;">
using Microsoft.Extensions.Options;
...
<img src="./pics/6.21.png" alt="IOptions и передача конфигурации в класс middleware в ASP.NET Core и C#" />
<h3>Настройка привязки конфгурации</h3>
<p>При необходимости мы можем переопределить настройки с помощью перегрузки метода <code>services.Configure()</code>:</p>
<pre class="brush:c#;">
using Microsoft.Extensions.Options;
...
<p>Также можно передавать отдельные секции конфигурации. Например, передадим секцию Company:</p>
<pre class="brush:c#;">
using Microsoft.Extensions.Options;
...
<img src="./pics/6.22.png" alt="IOptions и привязка объекта к секции конфигурации в ASP.NET Core и C#" />
...
<div class="nav"><p><a href="./6.7.php">Назад</a><a href="./">Содержание</a><a href="./7.1.php">Вперед</a></p></div></div>
<div class="socBlock">
