...
<html >
<head>
<title>ASP.NET Core и C# | Работа со статическими файлами</title>
<meta charset="utf-8" />
<meta name="description" content="Работа со статическими файлами в в ASP.NET Core и C#, файлы по умолчанию, просмотр содержимого каталогов, методы UseDefaultFiles и UseDirectoryBrowser">
...
<h2>Работа со статическими файлами</h2><div class="date">Последнее обновление: 20.12.2021</div>
...
<p>Рассмотрим некоторые возможности, которые предоставляет нам ASP.NET Core для работы со статическими файлами.</p>
<h3>Файлы по умолчанию</h3>
<p>Допустим, в проекте в папке <span class="b">wwwroot</span> располагается файл <span class="b">index.html</span>:</p>
<img src="./pics/5.5.png" alt="Файлы по умолчанию и UseDefaultFiles в ASP.NET Core и C#" />
<p>С помощью специального метода расширения <span class="b">UseDefaultFiles()</span> можно настроить отправку статических веб-страниц по умолчанию без обращения к ним по полному пути:</p>
<pre class="brush:c#;">
...
<p>В этом случае при отправке запроса к корню веб-приложения типа <span class="ii">http://localhost:xxxx/</span> приложение будет искать в папке wwwroot следующие файлы:</p>
<ul>
<li><p>default.htm</p></li>
<li><p>default.html</p></li>
<li><p>index.htm</p></li>
<li><p>index.html</p></li>
</ul>
<p>Если файл будет найден, то он будет отправлен в ответ клиенту.</p>
<img src="./pics/5.6.png" alt="Файл по умолчанию index.html и UseDefaultFiles в ASP.NET Core и C#" />
<p>Если же файл не будет найден, то продолжается обычная обработка запроса с помощью следующих 
компонентов middleware. То есть фактически это будет аналогично, как будто мы обращаемся к файлу: <i>http://localhost/index.html</i></p>
<p>Если же мы хотим использовать файл, название которого отличается от вышеперечисленных, то нам надо в этом случае применить объект <span class="b">DefaultFilesOptions</span>:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>В этом случае в качестве страницы по умолчанию будет использоваться файл <span class="b">hello.html</span>, который должен располагаться в папке wwwroot.</p>
<h3>Метод UseDirectoryBrowser</h3>
<p>Метод <span class="b">UseDirectoryBrowser</span> позволяет пользователям просматривать содержимое каталогов на сайте:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/5.7.png" alt="Метод UseDirectoryBrowser и просмотр содержимого каталога wwwroot в ASP.NET Core и C#" />
<p>Данный метод имеет перегрузку, которая позволяет сопоставить определенный каталог на жестком диске или в проекте с некоторой строкой запроса и тем 
самым потом отобразить содержимое этого каталога:</p>
<pre class="brush:c#;">
...
<p>Чтобы задействовать новый функционал, надо подключить пространство имен <code>using Microsoft.Extensions.FileProviders</code>.</p>
<p>В качестве параметра метод <code>UseDirectoryBrowser()</code> принимает объект <span class="b">DirectoryBrowserOptions</span>, который позволяет настроить сопоставление путей к файлам с 
каталогами. Так, в данном случае путь типа <code>http://localhost:xxxx/pages/</code> будет сопоставляться с каталогом "wwwroot\html".</p>
<img src="./pics/5.8.png" alt="UseDirectoryBrowser и сопоставление пути с каталогом файлов в ASP.NET Core и C#" />
<h3>Сопоставление каталогов с путями</h3>
<p>Перегрузка метода <code>UseStaticFiles()</code> позволяет сопоставить пути с определенными каталогами:</p>
<pre class="brush:c#;">
using Microsoft.Extensions.FileProviders;
...
<p>Первый вызов <code>app.UseStaticFiles()</code> обрабатывает запросы к файлам в папке wwwroot. Второй вызов принимает те же параметры, что и метод <code>app.UseDirectoryBrowser()</code> 
в предыдущем примере. И в отличие от первого вызова он обрабатывает запросы по пути <code>http://localhost:xxxx/pages</code>, сопоставляя данные запросы с 
папкой <span class="b">wwwroot/html</span>. К примеру, по запросу <code>http://localhost:xxxx/pages/index.html</code> мы можем обратиться к файлу <span class="b">wwwroot/html/index.html</span>.</p>
<img src="./pics/5.9.png" alt="UseStaticFiles и сопоставление пути с каталогом файлов в ASP.NET Core и C#" />
<h3>Метод UseFileServer</h3>
<p>Метод <span class="b">UseFileServer()</span> объединяет функциональность сразу всех трех вышеописанных методов 
<code>UseStaticFiles</code>, <code>UseDefaultFiles</code> и <code>UseDirectoryBrowser</code>:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>По умолчанию этот метод позволяет обрабатывать статические файлы и отправлять файлы по умолчанию типа index.html. Если нам надо еще включить просмотр каталогов, 
то мы можем использовать перегрузку данного метода:</p>
<pre class="brush:c#;">app.UseFileServer(enableDirectoryBrowsing: true);</pre>
<p>Еще одна перегрузка метода позволяет более точно задать параметры:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Также можно настроить сопоставление путей запроса с каталогами:</p>
<pre class="brush:c#;">
using Microsoft.Extensions.FileProviders;
...
<p>В этом случае будет разрешен обзор каталога по пути <code>http://localhost:xxxx/pages/</code>, но при этом путь <code>http://localhost:xxxx/html/</code> 
работать не будет.</p>
