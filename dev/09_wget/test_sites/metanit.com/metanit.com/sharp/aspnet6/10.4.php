...
<html >
<head>
<title>ASP.NET Core и C# | Отправка статусных кодов в Results API</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка статусных кодов в Results API в ASP.NET Core и C#, метод StatusCode, NotFound, Ok, BadRequest, Unauthorized">
...
<h2>Отправка статусных кодов в Results API</h2><div class="date">Последнее обновление: 24.12.2021</div>
...
<p>Нередко возникает необходимость отправить в ответ на запрос какой-либо статусный код. Например, если пользователь пытается получить 
доступ к ресурсу, который недоступен или для которого у пользователя нету прав. Либо просто необходимо уведомить пользователя с помощью 
статусного кода об успешном выполнении операции. И для этого Results API предоставляет ряд методов.</p>
<h3>StatusCode</h3>
<p>Метод <span class="b">StatusCode()</span> позволяет отправить любой статусный код, числовой код которого передается в метод в качестве параметра:</p>
<pre class="brush:c#;">
...
<p>Подобным образом мы можем послать клиенту любой другой статусный код. Но для некоторых отдельных статусных кодов имеются свои отдельные методы.</p>
<h3>Метод NotFound</h3>
<p>Метод <span class="b">NotFound()</span> посылает код 404, уведомляя клиента о том, что ресурс не найден. В качестве параметра в метод можно передать 
некоторый объект для отправки клиенту, например, сообщение об ошибке:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/10.6.png" alt="NotFound и Results API в ASP.NET Core и C#" />
<p>Стоит отметить, что сложные объекты по умолчанию сериализуются в формат json.</p>
<h3>Unauthorized</h3>
<p>Метод <span class="b">Unauthorized()</span> посылает код 401, уведомляя пользователя, что он не авторизован для доступа к ресурсу:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<h3>BadRequest</h3>
<p>Метод <span class="b">BadRequest()</span> посылает код 400, который говорит о том, что запрос некорректный. В качестве параметра 
можно передать некоторый объект для отправки клиенту:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>В данном случае если параметр маршрута age меньше 18, то посылаем клиенту статусный код 400 с сообщением "Invalid age". Иначе посылаем некоторое стандартное сообщение</p>
<img src="./pics/10.7.png" alt="BadRequest и Results API в ASP.NET Core и C#" />
<p>Причем опять же посылаемый объект по умолчанию сериализуются в формат json.</p>
<h3>Ok</h3>
<p>Метод <span class="b">Ok()</span> посылает статусный код 200, уведомляя об успешном выполнении запроса. В качестве параметра метод 
принимает отправляемую информацию:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/10.8.png" alt="Ok и Results API в ASP.NET Core и C#" />
