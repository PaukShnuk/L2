...
<html >
<head>
<title>ASP.NET Core и C# | Переадресация в Results API</title>
<meta charset="utf-8" />
<meta name="description" content="Переадресация в Results API в ASP.NET Core и C#, методы LocalRedirect, Redirect и RedirectToRoute">
...
<h2>Переадресация в Results API</h2><div class="date">Последнее обновление: 24.12.2021</div>
...
<h3>LocalRedirect</h3>
<p>Для переадресации на локальный адрес в рамках приложения применяется метод <span class="b">LocalRedirect()</span>:</p>
<pre class="brush:c#;">
public static IResult LocalRedirect (string localUrl, bool permanent = false, bool preserveMethod = false);
</pre>
<p>Параметры метода:</p>
<ul>
<li><p><span class="b">localUrl</span>: локальный адрес для переадресации</p></li>
<li><p><span class="b">permanent</span>: указывает, будет ли переадресация постоянной (отправляется статусный код 301) или временной (
отравляется статусный код 302).</p></li>
<li><p><span class="b">preserveMethod</span>: если равен true, то сохраняется оригинальный метод HTTP-запроса.</p></li>
</ul>
...
<p>В данном случае при обращении по пути "/old" будет осуществляться редирект на адрес "/new". Поскольку второй параметр не задан, то будет выполняться временная переадресация.</p>
<h3>Метод Redirect</h3>
<p>Метод <span class="b">Redirect()</span> также осуществляет переадресацию и принимает те же параметры, что и <code>LocalRedirect()</code>, только адрес для переадресации 
может не только локальным, но и внешним:</p>
<pre class="brush:c#;">
...
<h3>RedirectToRoute</h3>
<p>Метод <span class="b">RedirectToRoute()</span> выполняет переадресацию на определенный маршрут:</p>
<pre class="brush:c#;">
...
<p>Параметры метода:</p>
<ul>
<li><p><span class="b">routeName</span>: название маршрута</p></li>
<li><p><span class="b">routeValues</span>: значения для параметров маршрута</p></li>
<li><p><span class="b">permanent</span>: указывает, будет ли переадресация постоянной (отправляется статусный код 301) или временной (
отравляется статусный код 302).</p></li>
<li><p><span class="b">preserveMethod</span>: если равен true, то сохраняется оригинальный метод HTTP-запроса.</p></li>
<li><p><span class="b">fragment</span>: фрагмент, который добавляется к адресу для переадресации</p></li>
</ul>
