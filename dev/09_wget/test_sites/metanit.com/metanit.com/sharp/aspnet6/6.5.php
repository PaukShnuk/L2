...
<html >
<head>
<title>ASP.NET Core и C# | Анализ конфигурации</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с конфигурацией в ASP.NET Core и C#, получение настроек в файлах json и xml, использование секций, методы GetChildren и GetSection">
...
<h2>Анализ конфигурации</h2><div class="date">Последнее обновление: 21.12.2021</div>
...
<p>Для работы с конфигурацией интерфейс IConfiguration определяет следующие методы:</p>
<ul>
<li><p><span class="b">GetSection(name)</span>: возвращает объект IConfiguration, который представляет только определенную секцию name</p></li>
<li><p><span class="b">GetChildren()</span>: возвращает все подсекции текущего объекта конфигурации в виде набора объектов IConfiguration</p></li>
<li><p><span class="b">GetReloadToken()</span>: возвращает токен - объект IChangeToken, который используется для уведомления при изменении конфигурации</p></li>
<li><p><span class="b">GetConnectionString(name)</span>: эквивалентен вызову <code>GetSection("ConnectionStrings")[name]</code> и предназначается непосредственно 
для работы со строками подключения к различным базам даных</p></li>
<li><p><span class="b">[key]</span>: индексатор, который позволяет получить по определенному ключу key хранящееся значение</p></li>
</ul>
<p>Например, если у нас есть следующая конфигурация в файле <code>config.json</code>:</p>
<pre class="brush:js;">
{
  "ConnectionStrings": {
...
<p>То мы можем с помощью метода <code>GetSection()</code> получить отдельные секции и их значения:</p>
<pre class="brush:c#;">
...
<p>Каждая отдельная секция представляет объект <span class="b">IConfigurationSection</span>. Если секция содержит другие секция, то также можем 
вызвать у ней метод <code>GetSection()</code>. Если же секция содержит только значение, то оно доступно через свойство <code>Value</code>.</p>
<img src="./pics/6.13.png" alt="Получение отдельных секций в конфигурации в ASP.NET Core и C#" />
<p>Также мы могли бы использовать один вызов <code>GetSection()</code>, передав ему полный путь к нужной секции:</p>
<pre class="brush:c#;">string defaultConnection = appConfig.GetSection("ConnectionStrings:DefaultConnection").Value;</pre>
<p>Вложенные секции от родительских отделяются двоеточием.</p>
<p>Также мы могли бы получить нужное значение, используя индексаторы:</p>
<pre class="brush:c#;">string defaultConnection = appConfig["ConnectionStrings:DefaultConnection"];</pre>
<p>Ну и кроме того, для работы непосредственно с секцией "ConnectionStrings" нам доступен метод <code>GetConnectionString()</code>:</p>
<pre class="brush:c#;">string con = appConfig.GetConnectionString("DefaultConnection");</pre>
<p>Используя выше рассмотренные методы, мы можем провести анализ всего файла конфигурации. Например, 
пусть в проекте определен следующий конфигурационный файл <span class="b">project.json</span>:</p>
<pre class="brush:js;">
{
  "projectConfig": {
    "dependencies": {
...
<p>Проанализируем и выведем все его содержимое в браузере:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/6.14.png" alt="Анализ конфигурации json в ASP.NET Core и C#" />
