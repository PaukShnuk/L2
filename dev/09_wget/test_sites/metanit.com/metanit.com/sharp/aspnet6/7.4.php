...
<html >
<head>
<title>ASP.NET Core и C# | Создание провайдера логгирования</title>
<meta charset="utf-8" />
<meta name="description" content="Создание провайдера логгирования в ASP.NET Core и C#, запись лога в текстовый файл, использование объектов ILogger, ILoggerFactory и ILoggerProvider">
...
<h2>Создание провайдера логгирования</h2><div class="date">Последнее обновление: 22.12.2021</div>
...
<p>Стандартная инфраструктура ASP NET Core предоставляет, возможно, не самые удобные способы логгирования - на консоль, в окне Output в Visual Studio. 
Однако в то же время ASP.NET Core позволяет полностью определить свою логику ведения лога. Допустим, мы хотим сохранять сообщения в текстовом файле.</p>
<p>Вначале добавим в проект новый класс <span class="b">FileLogger</span>:</p>
<pre class="brush:c#;">
...
<p>Класс логгера должен реализовать интерфейс <span class="b">ILogger</span>. Этот интерфейс определяет три метода:</p>
<ul>
<li><p><code>BeginScope</code>: этот метод возвращает объект IDisposable, который представляет некоторую область видимости для логгера. В данном 
случае нам этот метод не важен, поэтому возвращаем значение <code>this</code> - ссылку на текущий объект класса, который реализует интерфейс IDisposable.</p></li>
<li><p><code>IsEnabled</code>: возвращает значения true или false, которые указыват, доступен ли логгер для использования. Здесь можно здать различную логику. В частности, 
...
<li><p><code>formatter</code>: функция форматирования, которая с помощью двух предыдущих параметов позволяет получить собственно сообщение для логгирования</p></li>
</ul>
<p>И в данном методе как раз и производится запись в текстовый файл. Путь к этому файлу передается через конструктор</p>
</li>
</ul>
<p>Далее добавим в проект класс <span class="b">FileLoggerProvider</span>:</p>
<pre class="brush:c#;">
...
<p>Этот класс представляет провайдер логгирования. Он должен реализовать интерфейс <span class="b">ILoggerProvider</span>. В этом интерфейсе определны два метода:</p>
<ul>
...
<p>Теперь создадим вспомогательный класс <span class="b">FileLoggerExtensions</span>:</p>
<pre class="brush:c#;">
...
<p>Этот класс добавляет к объекту ILoggingBuilder метод расширения <code>AddFile</code>, который будет добавлять наш провайдер логгирования.</p>
<p>Теперь используем провайдер в файле <span class="b">Program.cs</span>:</p>
<pre class="brush:c#;">
...
<p>За глобальную установку настроек логгирования отвечает свойство <span class="b">Logging</span> класса WebApplicationBuilder. Это свойство представляет 
объект ILoggingBuilder и предоставляет ряд методов для управления логгированием. И в данном случае с помошью выше определенного метода AddFile добавляем логгирование в файл.</p>
<p>Стоит отметить, что глобальная настройка логгирования должна идти до создания объекта WebApplication.</p>
<p>Теперь для логгирования также будет использоваться файл logger.txt, который будет создаваться в папке проекта.</p>
<img src="./pics/7.6.png" alt="Создание провайдера логгирования в ASP.NET Core и C#" />
