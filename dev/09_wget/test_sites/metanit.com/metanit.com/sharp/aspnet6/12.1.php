...
<html >
<head>
<title>ASP.NET Core и C# | Подключение Entity Framework</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение к базе данных через Entity Framework в ASP.NET Core и C#, установка пакета, подключение сервиса через метод AddDbContext, соединение с базой данных MS SQL Server через метод UseSqlServer">
...
<h1>Работа с базой данных</h1><h2>Подключение Entity Framework Core</h2><div class="date">Последнее обновление: 25.12.2021</div>
...
<p>Entity Framework представляет ORM-решение, которое позволяет автоматически связать классы языка C# с таблицами в базе данных. Entity Framework Core 
...
MS SQL Server.</p>
<p>Детально ознакомиться с работой с Entity Framework Core можно в соответствуюшем <a href="../efcore">руководстве</a>, здесь же мы сосредоточимся 
прежде всего на тех моментах, которые характерны для веб-приложения ASP.NET Core.</p>
<p>Для работы с базами данных MS SQL Server через Entity Framework Core необходим пакет <span class="b">Microsoft.EntityFrameworkCore.SqlServer</span>. 
По умолчанию он отсутствует в проекте, поэтому его надо добавить, например, через пакетный менеджер Nuget:</p>
<img src="./pics/12.1.png" alt="Добавление Entity Framework Core в ASP.NET Core и C#" />
<p>Далее добавим в проект класс, которые будет представлять данные. Пусть он будет называться <span class="b">User</span> и будет иметь следующий код:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Этот класс представляет те объекты, которые будут храниться в базе данных.</p>
<p>Для взаимодействия с базой данных через Entity Framework Core необходим контекст данных - класс, унаследованный от класса 
<span class="b">Microsoft.EntityFrameworkCore.DbContext</span>. Поэтому добавим в проект новый класс, который назовем 
<span class="b">ApplicationContext</span> (название класса контекста произвольное):</p>
<pre class="brush:c#;">
...
<p>Свойство DbSet представляет собой коллекцию объектов, которая сопоставляется с определенной таблицей в базе данных. То есть свойство Users будет представлять таблицу, в которой будут храниться объекты User.</p>
<p>Класс DbSet, как и другие типы, является ссылочным. А, начиная с C# 10 и .NET 6 автоматически применяется функциональность ссылочных 
...
их использованием. Чтобы выйти из этой ситуации мы можем инициализировать свойство с помощью выражения <span class="b">null!</span>, которое говорит, что данное свойство в принципе не будет иметь значение null.
Потому что в реальности конструктор базового класса DbContext гарантирует, что все свойства типа DbSet будут инициализированы и соответственно в принципе не будут иметь значение null.</p>
<p>В конструкторе класса ApplicationContext через параметр <code>options</code> будут передаваться настройки контекста данных. 
А в самом конструкторе с помощью вызова <code>Database.EnsureCreated()</code> по определению модели будет создаваться база данных (если она отсутствует).</p>
<p>Кроме того, в методе <span class="b">OnModelCreating()</span> настраивается некоторая начальная конфигурация модели. В частности, с помощью метода 
<code>modelBuilder.Entity&lt;User&gt;().HasData()</code> в базу данных добавляются три начальных объекта</p>
<h3>Конфигурация и строка подключения</h3>
<p>Чтобы подключаться к базе данных, нам надо задать параметры подключения. Это можно сделать либо в коде C#, либо в файле конфигурации. 
Выберем второй способ. Для этого изменим файл <span class="b">appsettings.json</span>. По умолчанию он содержит только настройки логгирования:</p>
<pre class="brush:js;">
{
  "Logging": {
    "LogLevel": {
...
<p>Теперь изменим его, добавив определение строки подключения:</p>
<pre class="brush:js;">
{
  "ConnectionStrings": {
...
<p>В данном случае мы будем использовать упрощенный движок базы данных LocalDB, который представляет легковесную версию SQL Server Express, предназначенную специально для разработки приложений. Об этом говорит параметр <code>Server=(localdb)\\mssqllocaldb</code>. Ну а сама база данных будет 
называться <code>applicationdb</code>.</p>
<h3>Добавление контекста данных EF Core в качестве сервиса</h3>
<p>Контекст данных, через который идет взаимодействие с базой данных, передается в приложение в качестве сервиса через механизм внедрения зависимостей. 
Поэтому для работы с Entity Framework Core необходимо добавить класс контекста данных в коллекцию сервисов приложения:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>Для использования контекста данных ему надо передать строку подключения, которая выше была определена в файл конфигурации appsettings.json. Поэтому сначала считываем 
строку подключения под названием "DefaultConnection":</p>
<pre class="brush:c#;">string connection = builder.Configuration.GetConnectionString("DefaultConnection");</pre>
<p>Для получения строки конфигурации для объекта IConfiguration, который представляет конфигурацию, определен метод расширения <span class="b">GetConnectionString()</span>, 
в который передается название строки подключения.</p>
<p>Для добавления контекста данных в качестве сервиса у объекта коллекции сервисов IServiceCollection определен метод <span class="b">AddDbContext()</span>, который типизируется 
классом контекста данных:</p>
<pre class="brush:c#;">
builder.Services.AddDbContext&lt;ApplicationContext&gt;(options =&gt; options.UseSqlServer(connection));
</pre>
<p>Этот метод в качестве параметра принимает делегат, который настраивает подключение. В частности, с помощью вызова <code>options.UseSqlServer()</code> 
настраивается подключение к серверу MS SQL Server, а в качестве параметра в этот вызов передается строка подключения.</p>
<h3>Обращение к базе данных</h3>
<p>Подключив контекст данных мы можем с его помощью обращаеться к базе данных. Например, в качестве теста получим добавляемые по умолчанию данные:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>Поскольку контекст данных добавлен в сервисы, то мы можем его получить в обработчике конечной точки:</p>
<pre class="brush:c#;">app.MapGet("/", (ApplicationContext db) =&gt; db.Users.ToList());</pre>
<p>В данном случае клиенту отправляется список добавленных в БД по умолчанию объектов User в формате JSON:</p>
<img src="./pics/12.2.png" alt="Взаимодействие с базой данных через Entity Framework Core в ASP.NET Core и C#" />
...
<div class="nav"><p><a href="./11.1.php">Назад</a><a href="./">Содержание</a><a href="./12.2.php">Вперед</a></p></div></div>
<div class="socBlock">
