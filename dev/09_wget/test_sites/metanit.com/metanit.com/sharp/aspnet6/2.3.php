...
<html >
<head>
<title>ASP.NET Core и C# | Метод Run и определение терминального middleware</title>
<meta charset="utf-8" />
<meta name="description" content="Создание компонента middleware в ASP.NET Core и C# с помощью метода Run, терминальный компонент, вынесение middleware в отдельный метод">
...
<h2>Метод Run и определение терминального middleware</h2><div class="date">Последнее обновление: 10.12.2021</div>
...
<p>Самый простой способ добавления middleware в конвейер обработки запроса в ASP.NET Core представляет метод <span class="b">Run()</span>, 
который определен как метод расширения для интерфейса IApplicationBuilder (соответствено его поддерживает и класс WebApplication):</p>
<pre class="brush:c#;">IApplicationBuilder.Run(RequestDelegate handler)</pre>
<p>Метод <code>Run</code> добавляет терминальный компонент - такой компонент, который завершает обработку запроса. Поэтому соответствено он не вызывает никакие другие компоненты и обработку запроса дальше - следующим в конвейере 
компонентам не передает. Поэтому данный метод следует вызывать в самом конце построения конвейера обработки запроса. 
До него же могут быть помещены другие методы, которые добавляют компоненты middleware.</p>
...
<p>Он принимает в качестве параметра контекст запроса HttpContext и возвращает объект Task.</p>
<p>Используем этот метод для определения простейшего компонента:</p>
<pre class="brush:c#;">
...
<p>Здесь для делегата RequestDelegate передается лямбда-выражение, параметр которого - HttpContext можно использовать для отправки ответа. 
В частности, метод <code>context.Response.WriteAsync()</code> позволяет отправить клиенту некоторый ответ - в данном случае отправляется простая строка.</p>
<p>После запуска проекта будет запущено приложение откроется браузер, который выполнит запрос к приложению и получит обратно строку "Hello METANIT.COM".</p>
<img src="./pics/2.2.png" alt="Установка middleware в ASP.NET Core с помощью метода Run" />
<p>Здесь следует сделать пару замечаний. Прежде всего, не стоит путать метод <span class="b">Run()</span>, который определен в классе WebApplication и который запускает приложение, и 
метод расщирения <span class="b">Run()</span>, который встраивает компонент middleware. Это два разных метода, которые выполняют разные задачи. И, как видно из кода выше, 
вызываются оба этих метода.</p>
<p>Второй момент - метод <span class="b">Run()</span>, который запускает приложение, вызывается <span class="b">после</span> добавления компонента middleware. И мы НЕ 
...
<p>При необходимости естественно мы можем вынести код middleware в отдельный метод:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Компоненты middleware создаются один раз и существуют в течение всего жизненного цикла приложения. То есть для последующей обработки запросов используются одни и те же компоненты. Например, определим  в файле <span class="b">Program.cs</span> 
следующий код:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
...
<img src="./pics/2.3.png" alt="Жизненный цикл middleware в ASP.NET Core и C#" />
<p>Однако при последующих запросах мы увидим, что результат переменной х не равен 4.</p>
<img src="./pics/2.4.png" alt="Жизненный цикл приложения в ASP.NET Core и C#" />
