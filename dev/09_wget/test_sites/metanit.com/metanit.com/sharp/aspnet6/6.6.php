...
<html >
<head>
<title>ASP.NET Core и C# | Создание провайдера конфгурации</title>
<meta charset="utf-8" />
<meta name="description" content="Создание провайдера конфгурации в ASP.NET Core и C#, использование источника конфигурации IConfigurationSource и ConfigurationProvider">
...
<h2>Создание провайдера конфгурации</h2><div class="date">Последнее обновление: 21.12.2021</div>
...
<p>Фреймворк ASP.NET Core по умолчанию предоставляет богатый функционал для работы с конфигурацией - для работы с json, xml и так далее. 
Однако, в каких-то ситуациях этого может быть недостаточно. И в этом случае мы можем определить свои источники и провайдеры конфигурации.</p>
<p>Создание конфигурации вовлекает три компонента: <span class="b">IConfigurationSource</span> (определяет источник конфигурации), 
<span class="b">ConfigurationProvider</span> (сам провайдер конфигурации) и некий класс, который добавляет метод расширения к объекту IConfiguration.</p>
<p>Допустим, мы хотим хранить конфигурацию в простом текстовом файле. И для этого добавим новый класс, который назовем <span class="b">TextConfigurationProvider</span>:</p>
<pre class="brush:c#;">
...
<p>Класс TextConfigurationProvider будет представлять провайдер конфигурации и поэтому должен быть унаследован от класса ConfigurationProvider.</p>
<p>В этом классе с помощью StreamReader происходит считывание текстового файла и добавление данных в словарь data. Для загрузки данных переопределяется 
метод <code>Load()</code>, определенный в базовом классе.</p>
<p>В данном случае мы предполагаем, что на первой строке будет располагаться ключ, а на следующей строке значение. И после считывания эти ключ и значение будут 
попадать в словарь data.</p>
<p>После считывания словарь data присваивается свойству <span class="b">Data</span>, которое унаследовано от ConfigurationProvider. Это свойство как раз и хранит все 
те конфигурационные настройки, которые потом используются в программе.</p>
<p>И чтобы получить путь к файлу, он передается через параметр конструктора.</p>
<p>Далее нам надо обращаться к этому провайдеру. Для этого определим класс источника конфигурации, который назовем <span class="b">TextConfigurationSource</span>:</p>
<pre class="brush:c#;">
...
<p>Источник конфигурации должен реализовать интерфейс <span class="b">IConfigurationSource</span>, и в частности, его метод <code>Build</code>. В этот 
метод в качестве параметра передается строитель конфигурации.</p>
<p>В данном случае этот объект нам позволяет получить полный путь к текстовому файлу. Краткое название файла (относительный путь) передается в класс источника через 
конструктор и хранится в свойстве FilePath. После создания полного пути к файлу этот путь передается в конструктор TextConfigurationProvider.</p>
<p>Чтобы задействовать функционал источника конфигурации создадим вспомогательный класс <span class="b">TextConfigurationExtensions</span>:</p>
<pre class="brush:c#;">
...
<p>Этот класс определяет для объекта IConfigurationBuilder метод расширения <span class="b">AddTextFile()</span>, в котором создается источник конфигурации 
TextConfigurationSource, который затем добавляется к строителю конфигурации:</p>
<pre class="brush:c#;">
var source = new TextConfigurationSource(path);
builder.Add(source);
</pre>
<p>Теперь добавим в проект сам файл конфигурации. Пусть он будет называться <span class="b">config.txt</span> и будет иметь следующее содержимое:</p>
<pre class="brush:c#;">
name
Tom
age
...
<p>То есть на первой строке идет ключ настройки, а на второй  ее значение и так далее.</p>
<p>Таким образом, у нас получится следующая структура проекта:</p>
<img src="./pics/6.15.png" alt="Создание провайдера конфигурации в ASP.NET Core и C#" />
<p>И в конце изменим код файла <span class="b">Program.cs</span> для использования конфигурации из текстового файла:</p>
<pre class="brush:c#;">
...
<p>И при обращении к приложению будут использованы настройки из текстового файла:</p>
<img src="./pics/6.16.png" alt="Определение провайдера и источника конфгурации в ASP.NET Core и C#" />
