...
<html >
<head>
<title>ASP.NET Core и C# | Передача зависимостей в конечные точки</title>
<meta charset="utf-8" />
<meta name="description" content="Передача зависимостей и сервисов в конечные точки в ASP.NET Core и C#, получение сервиса в Map">
...
<h2>Передача зависимостей в конечные точки</h2><div class="date">Последнее обновление: 19.12.2021</div>
...
<p>Фреймворк ASP.NET Core предоставляет простой и удобный способ для передачи зависимостей в конечные точки. Все добавляемые в коллекцию сервисов приложения зависимости 
можно получить через параметры делегата, который отвечает за обработку запроса.</p>
...
<p>Здесь класс TimeService выступает в качестве сервиса, свойство <code>Time</code> которого возвращает текущее время в формате "hh:mm:ss".</p>
<p>Этот сервис добавляется в коллекцию сервисов приложения:</p>
<pre class="brush:c#;">builder.Services.AddTransient&lt;TimeService&gt;();</pre>
<p>Далее через параметр делегата, который передается в качестве второго параметра в метод <span class="b">Map()</span> мы можем получить эту зависимость:</p>
<pre class="brush:c#;">
app.Map("/time", (TimeService timeService) =&gt; $"Time: {timeService.Time}");
</pre>
<p>Таким образом, при обращении по адресу "/time" приложение возвратит клиенту текущее время:</p>
<img src="./pics/3.19.png" alt="Внедрение зависимостей и получение сервисов в конечных точках в ASP.NET Core и C#" />
<p>Подобным образом можно получить зависимости, если обаботчик маршрута конечной точки вынесен в отдельный метод:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
