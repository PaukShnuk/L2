...
<html >
<head>
<title>ASP.NET Core и C# | Множественная регистрация сервисов</title>
<meta charset="utf-8" />
<meta name="description" content="Множественная регистрация сервисов в ASP.NET Core и C#, регистрация для одной зависимости нескольких реализаций и установка для нескольких зависимостей одного объекта">
...
<h2>Множественная регистрация сервисов</h2><div class="date">Последнее обновление: 05.02.2022</div>
...
<p>По умолчанию при внедрении зависимостей  в ASP.NET Core одна зависимость сопоставляется с одним типом. Однако бывают ситуации, 
...
реализаций. Вторая ситуация: для нескольких зависимостей необходимо зарегистрировать один и тот же объект.</p>
<h3>Регистрация для одной зависимости нескольких типов</h3>
<p>ASP.NET Core позволяет зарегистрировать для одной зависимости сразу несколько типов. Рассмотрим простейший пример:</p>
<pre class="brush:c#;">
...
<p>Здесь интерфейс IHelloService с помощью свойства Message определяет некоторое сообщение. Этот интерфейс реализуется двумя классами: RuHelloService и EnHelloService, каждый 
...
все эти реализации можно получить в виде коллекции <code>IEnumerable&lt;IHelloService&gt;</code>.</p>
<p>Этот сервис применяется в middleware HelloMiddleware, который внедряется в конвейер обработки запроса и обрабатывает все запросы к приложению. 
И в его конструкторе получаем все зарегистрированные реализации сервиса IHelloService через объект <code>IEnumerable&lt;IHelloService&gt;</code></p>
<pre class="brush:c#;">
public HelloMiddleware(RequestDelegate _, IEnumerable&lt;IHelloService&gt; helloServices)
{
    this.helloServices = helloServices;
...
<p>Соответственно далее все зарегистрированные реализации сервиса IHelloService можно использовать для обработки запроса:</p>
<img src="./pics/4.15.png" alt="Множественная регистрация сервисов в ASP.NET Core и C#" />

<h3>Регистрация одного объекта для нескольких зависимостей</h3>
<p>Теперь рассмотрим другую ситуацию: использование несколькими зависимостями одного и то же объекта. Сначала рассмотрим ситуацию, с которой мы можем столкнуться. 
Допустим, у нас есть следующие объекты:</p>
<pre class="brush:c#;">
...
<p>Здесь интерфейс IGenerator предназначен для генерации некоторого числа, а интерфейс IReader - для чтения некоторого числа. Класс ValueStorage 
...
То есть метод Read считывает текущее значение value, а метод Generate изменяет его.</p>
<p>Используем эти сервисы в приложении:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
...
<p>Здесь для обоих зависимостей - IGenerator и IReader определена одна реализация - ValueStorage. При обращении по адресу "/generate" срабатывает middleware 
GenerateMiddleware, который получает сервис IGenerator и с его помощью генерирует новое значение.</p>
<p>При обращении по всем иным адресам срабатывает middleware ReaderMiddleware, который получает сервис IReader и возвращает текущее значение. Однако при запуске проекта 
...
два разных экземпляра класса ValueStorage:</p>
<img src="./pics/4.16.png" alt="регистрация для нескольких зависимостей одного объекта в ASP.NET Core и C#" />
<p>Для исправления ситуации нам надо определить один объект для обоих зависимостей. Это можно сделать, например, следующим образом:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
...
<p>Теперь определяем один объект ValueStorage в виде синглтон-сервиса:</p>
<pre class="brush:c#;">builder.Services.AddSingleton&lt;ValueStorage&gt;();</pre>
<p>Зетем получаем его из коллекции сервисов и устанавливаем в качестве реализации для обоих зависимостей:</p>
<pre class="brush:c#;">
builder.Services.AddSingleton&lt;IGenerator&gt;(serv =&gt; serv.GetRequiredService&lt;ValueStorage&gt;());
builder.Services.AddSingleton&lt;IReader&gt;(serv =&gt; serv.GetRequiredService&lt;ValueStorage&gt;());
</pre>
<p>Соответственно теперь мы получим другие результаты:</p>
<img src="./pics/4.17.png" alt="один сервис для нескольких зависимостей в ASP.NET Core и C#" />
<p>В качестве альтернативы можно было бы создать объект во вне и передать его сервисам:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./4.6.php">Назад</a><a href="./">Содержание</a><a href="./3.1.php">Вперед</a></p></div></div>
<div class="socBlock">
