...
<html >
<head>
<title>ASP.NET Core и C# | Авторизация по ролям</title>
<meta charset="utf-8" />
<meta name="description" content="Авторизация по ролям в ASP.NET Core и C#, получение и установка ролей пользователя">
...
<h2>Авторизация по ролям</h2><div class="date">Последнее обновление: 12.01.2022</div>
...
<p>Авторизация по ролям позволяет разграничить доступ к ресурсам приложения в зависимости от роли, к которой принадлежит пользователь.</p>
<p>Допустим, у нас есть следующие классы, которые описывают пользователя и его роль:</p>
<pre class="brush:c#;">
class Person
{
...
<p>Класс роли содержит свойство Name, которое хранит название роли. А класс Person хранит email-адрес, пароль и роль пользователя.</p>
<p>В файле <span class="b">Program.cs</span> определим следующий код:</p>
<pre class="brush:c#;">
...
<p>Для упрощения ситуации здесь данные ролей и пользователей определены напрямую в коде в виде списка people:</p>
<pre class="brush:c#;">
var adminRole = new Role("admin");
var userRole  = new Role("user");
var people = new List&lt;Person&gt; 
...
<p>Здесь определены две роли - "admin" и "user" и два пользователя.</p>
<p>Для аутентификации здесь используются куки:</p>
<pre class="brush:c#;">
builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme)
    .AddCookie(options =&gt;
    {
...
<p>Здесь свойство <code>options.AccessDeniedPath</code> указывает на путь, на который будет перенаправляться аутентифицированный пользователь 
при обращении к ресурсу, для доступа к котоому у него нет прав. То есть важно понимать разницу между назначением свойств <code>options.LoginPath</code> и 
<code>options.AccessDeniedPath</code>:</p>
<ul>
<li><p><code>options.LoginPath</code>: определяет путь перенаправления для не аутентифицированного пользователя</p></li>
<li><p><code>options.AccessDeniedPath</code>: определяет путь перенаправления для аутентифицированного пользователя, который не имеет прав для доступа к ресурсу</p></li>
</ul>
<p>В реальности для обоих параметров можно использовать один и тот же путь. Но в данном случае я их разграничил.</p>
<p>Таким образом, при доступе к ресурсу, для которого у пользователя нет прав, пользователь перенаправляется по адресу "/accessdenied". 
Запрос по этому пути обрабатывается следующей конечной точкой:</p>
<pre class="brush:c#;">
app.MapGet("/accessdenied", async (HttpContext context) =&gt;
{
    context.Response.StatusCode = 403;
...
<p>Здесь просто отправляется сообщение о запрете доступа со статусным кодом 403.</p>
<p>Если пользователь не аутентифицирован, то его перенаправляет по пути "/login". GET-запрос по этому пути 
обрабатывается конечной точкой <code>app.MapGet("/login")</code>, которая отправляет пользователю форму для ввода логина и пароля.</p>
<p>Отправленные пользователем в POST-запросе данные логина и пароля будут обрабатываться конечной точкой <code>app.MapPost("/login")</code>. 
Ключевым моментом ее обработчика является установка списка объектов claim, в которых  сохраняется логин пользователя и его роль:</p>
<pre class="brush:c#;">
var claims = new List&lt;Claim&gt;
{
    new Claim(ClaimsIdentity.DefaultNameClaimType, person.Email),
...
<p>Для указания роли здесь применяется тип claim <span class="b">ClaimsIdentity.DefaultRoleClaimType</span>, а в качестве значения для этого типа используется 
имя роли. По сути больше для установки роли для пользователя ничего не нужно.</p>
<p>Чтобы на уровне отдельных ресурсов приложения разграничить доступ в зависимости от роли, свойству <span class="b">Roles</span> 
атрибута <code>Authorize</code> передается набор допустимых ролей:</p>
<pre class="brush:c#;">[Authorize(Roles = "admin")</pre>
<p>Можно передавать несколько ролей через запятую:</p>
<pre class="brush:c#;">[Authorize(Roles = "admin, user")</pre>
<p>Для определения роли текущего пользователя инфрастуктура ASP.NET Core будет использовать значения claim с типом <span class="b">ClaimsIdentity.DefaultRoleClaimType</span>.</p>
<p>Например, обращаться по пути "/admin" могут только пользователи, которые 
принадлежат роли "admin":</p>
<pre class="brush:c#;">app.Map("/admin", [Authorize(Roles = "admin")]() =&gt; "Admin Panel");</pre> 
<p>В то время как по пути "/" могут обращаться представлители ролей "user" и "admin":</p>
<pre class="brush:c#;">
app.Map("/", [Authorize(Roles = "admin, user")](HttpContext context) =&gt;
{
    var login = context.User.FindFirst(ClaimsIdentity.DefaultNameClaimType);
...
<p>Например, залогинимся в приложение пользователем, который имеет роль "user":</p>
<img src="./pics/13.16.png" alt="Авторизация по ролям в ASP.NET Core и C#" />
<p>Однако при попытке пользователя с ролью "user" обратиться по адресу "/admin", он будет переадресован на адрес "/accessdenied":</p>
<img src="./pics/13.17.png" alt="Ограничение доступа по ролям в ASP.NET Core и C#" />
<p>Стоит отметить, что при аутентификации куки при перенаправлении по пути из свойства <code>options.AccessDeniedPath</code> автоматически передается параметр 
<code>ReturnUrl</code>, из которого можно получить путь, к которому пытался обращаться пользователь.</p>
<p>Теперь выйдем из приложения и снова залогинимся, только теперь под пользователем с ролью "admin":</p>
<img src="./pics/13.18.png" alt="Роли пользователей и авторизация в приложении ASP.NET Core и C#" />
<p>Таким образом, мы можем разграничивать доступ в приложении в зависимости от роли пользователя.</p>
...
<div class="nav"><p><a href="./13.6.php">Назад</a><a href="./">Содержание</a><a href="./13.8.php">Вперед</a></p></div></div>
<div class="socBlock">
