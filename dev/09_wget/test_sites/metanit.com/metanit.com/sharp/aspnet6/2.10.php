...
<html >
<head>
<title>ASP.NET Core и C# | Отправка и получение json</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка и получение json в ASP.NET Core и C#, сериализация данных в json и метод Response.WriteAsJsonAsync, десериализация из json и метод Request.ReadFromJsonAsync, создания конвертера в json и тип JsonConverter">
...
<h2>Отправка и получение json</h2><div class="date">Последнее обновление: 12.12.2021</div>
...
<p>JSON является распространенным форматом для передачи данных. Рассмотрим, как мы можем посылать и получить данные json.</p>
<h3>Отправка JSON. Метод WriteAsJsonAsync</h3>
<p>Для отправки json можно воспользоваться методом <span class="b">WriteAsJson()/WriteAsJsonAsync()</span> объекта <span class="b">HttpResponse</span>. 
Этот метод позволяет сериализовать переданные в него объекты в формат JSON и автоматически для заголовка "content-type" устанавливает значение "application/json; charset=utf-8":</p>
<pre class="brush:c#;">
...
<p>В данном случае клиенту отправляется объект типа Person, который представляет класс-record, однако это может быть и обычный класс:</p>
<img src="./pics/2.10.png" alt="Отправка json с помощью WriteAsJson в ASP.NET Core и C#" />
<p>Хотя можно было бы воспользоваться и стандартным методом <code>WriteAsync()</code>:</p>
<pre class="brush:c#;">
app.Run(async (context) =&gt;
{
...
<h3>Получение JSON. Метод ReadFromJsonAsync</h3>
<p>Для получения из запроса объект в формате JSON в классе <span class="b">HttpRequest</span> определен метод <span class="b">ReadFromJsonAsync()</span>. Он позволяет сериализовать данные в объект 
определенного типа.</p>
<p>Например, создадим в проекте папку <span class="b">html</span>, в которой определим новый файл <span class="b">index.html</span>.</p>
<img src="./pics/2.31.png" alt="Отправка объекта json на сервер ASP.NET Core в C#" />
...
<p>Здесь по нажатию на кнопку с помощью функции <span class="b">fetch()</span> по адресу "/api/user" будет отправляться объект со свойствами name и age, 
значения для которых берутся из полей формы. В ответ от сервера веб-страница также получает объект в формате json, в котором имеется свойство <code>text</code> - 
свойство, которое хранит сообщение от сервера.</p>
<p>Теперь в файле <span class="b">Program.cs</span> определим код для получения данных, отправляемых веб-страницей:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>В данном случае, если обращение идет по адресу "/api/user", то получаем данные в формате json. При обращениях по другим адресам просто посылаем 
веб-страницу index.html.</p>
<p>Стоит отметить, что если данные запроса не представляют объект JSON, то вызов метода <code>ReadFromJsonAsync()</code> выльется в ошибку. И в этом случае предпочтительно 
проверять на наличие json в запросе с помощью метода <span class="b">HasJsonContentType()</span> он возвращает <code>true</code>, если клиент прислал json.</p>
<pre class="brush:c#;">
if (request.HasJsonContentType())
{
	var person = await request.ReadFromJsonAsync&lt;Person&gt;();
...
<p>Метод <span class="b">ReadFromJsonAsync()</span> десериализует полученные данные в объект определенного типа - в данном случае типа Person. Поскольку между 
отправляемыми данными и опеделением класса Person есть соответствие, то проблем при десериализации не возникнет. Таким образом, 
результат вызова этого метода - значение переменной <code>person</code> будет представлять объект Person.</p>
<p>И затем в ответ посылаем анонимный объект, который также сериализуется в json с некоторым сообщением, которое хранится в свойстве text. При получении этого сообщения оно выводится на веб-страницу.</p>
<img src="./pics/2.32.png" alt="ReadFromJsonAsync и чтение данных json в ASP.NET Core и C#" />
<h3>Настройка сериализации</h3>
<p>При получении данных в формате json мы можем столкнуться с рядом проблем. Например, данные json не совсем соответствуют определению типа, 
в который надо выполнить десериализацию:</p>
<pre class="brush:c#;">
const response = await fetch("/api/user", {
	method: "POST",
	headers: { "Accept": "application/json", "Content-Type": "application/json" },
...
<p>Здесь названия свойств отправляемого объекта не соответствуют названиям свойств типа Person в C#. Однако объект Person все равно будет создан, просто его свойства получат значения 
по умолчанию (null для свойства Name и 0 для свойства Age).</p>
<p>Другой пример - отправляемые данные не соответствуют по типу:</p>
<pre class="brush:c#;">
const response = await fetch("/api/user", {
	method: "POST",
	headers: { "Accept": "application/json", "Content-Type": "application/json" },
...
<p>Здесь свойство "age" представляет строку и не сможет быть сконвертировано в значение типа <code>int</code>. В итоге при отправке подобных данных на сервере возникнет исключение типа <span class="b">System.Text.Json.JsonException</span>, а 
клиент получит информацию об исключении.</p>
<p>В обоих выше приведенных примерах в зависимости от задачи можно использовать различные решения - обрабатывть исключения, встраивать дополнительные middleware для отлова подобных ситуаций и так далее. 
Одним из решений подобных проблем также может быть настройка сериализации/десериализации с помощью параметра типа <span class="b">JsonSerializerOptions</span>, которое может передаваться в метод 
<span class="b">ReadFromJsonAsync()</span></p>
<pre class="brush:c#;">ReadFromJsonAsync&gt;T&gt;(JsonSerializerOptions options);</pre>
<p>Так, изменим код файла <span class="b">Program.cs</span>:</p>
<pre class="brush:c#;">
...
p>Поскольку настройка параметров сериализации/десериализации - это отдельная большая тема, то пройдемся вкратце по коду, который вовлекается в процесс 
конвертации и прежде всего по конвертеру Person в json.</p>
<h4>Определение конвертера для сериализации/десериализации объекта в json</h4>
<p>Класс конвертера для сериализации/десериализации объекта определенного типа в JSON должен наследоваться от класса <span class="b">JsonConverter&lt;T&gt;</span>. Абстрактный класс JsonConverter типизируется 
типом, для объекта которого надо выполнить сериализацию/десериализацию. В коде выше такой реализацией является класс <code>PersonConverter</code>.</p>
<p>При наследовании класса JsonConverter необходимо реализовать его абстрактные методы <span class="b">Read()</span> (выполняет десериализацию из JSON в Person) и 
<span class="b">Write()</span> (выполняет сериализацию из Person в JSON).</p>
<p>Метод Write, который записывает данные Person в формат JSON, выглядит относительно просто:</p>
<pre class="brush:c#;">
public override void Write(Utf8JsonWriter writer, Person person, JsonSerializerOptions options)
{
	writer.WriteStartObject();
...
<p>Он принимает три параметра:</p>
<ul>
<li><p><span class="b">Utf8JsonWriter</span> - объект, который записывает данные в json</p></li>
<li><p><span class="b">Person</span> - объект, который надо сериализовать</p></li>
<li><p><span class="b">JsonSerializerOptions</span> - дополнительные параметры сериализации</p></li>
</ul>
<p>Сначала с помощью объекта Utf8JsonWriter открываем запись объекта в формате json:</p>
<pre class="brush:c#;">writer.WriteStartObject();</pre>
<p>Последовательно записываем данные объекта Person:</p>
<pre class="brush:c#;">
writer.WriteString("name", person.Name);
writer.WriteNumber("age", person.Age);
</pre>
<p>И завершаем запись объекта:</p>
<pre class="brush:c#;">writer.WriteEndObject();</pre>
<p>Стоит отметить, что в коде выше сереализация из Person в JSON не применяется.</p>
<p>Чтение или десериализация выглядит несколько сложнее. Метод <span class="b">Read()</span> также принимает три параметра:</p>
<ul>
<li><p><span class="b">Utf8JsonReader</span> - объект, который читает данные из json</p></li>
<li><p><span class="b">Type</span> - тип, в который надо выполнить конвертацию</p></li>
<li><p><span class="b">JsonSerializerOptions</span> - дополнительные параметры сериализации</p></li>
</ul>
<p>Результатом метода <span class="b">Read()</span> должен быть десериализованный объект (в данном случае объект типа Person).</p>
<p>В начале определяем данные объекта Person по умолчанию, которые будут применяться, если в процессе десериализации произойдут проблемы:</p>
<pre class="brush:c#;">
 var personName = "Undefined";
var personAge = 0;
</pre>
<p>Далее в цикле считываем каждый токен в строке json с помощью метода <code>Read()</code> объекта Utf8JsonReader:</p>
<pre class="brush:c#;"> while (reader.Read())</pre>
<p>Затем, если считанный токен представляет название свойства, то считываем его и считываем следующий токен:</p>
<pre class="brush:c#;">
if (reader.TokenType == JsonTokenType.PropertyName)
{
	var propertyName = reader.GetString();
...
<p>После этого мы можем узнать, как называется свойство и какое значение оно имеет. Для этого применяем конструкцию <span class="b">switch</span>:</p>
<pre class="brush:c#;">
switch (propertyName)
{
</pre>
<p>Например, мы ожидаем, что json будет содержать свойство с именем "Age" или "age", которое будет хранить некоторое число. Для его получения применяем следующий блок case:</p>
<pre class="brush:c#;">
case "age" or "Age" when reader.TokenType == JsonTokenType.Number:
	personAge = reader.GetInt32();
    break;
</pre>
<p>То есть если свойство называется "Age" или "age" и представляет число (JsonTokenType.Number), то вызываем метод <code>reader.GetInt32()</code></p>
<p>Но свойство "Age/age" также может содержать строку, например, "23". Такая строка может конвертироваться в число. И для подобного случая добавляем 
дополнительный блок case:</p>
<pre class="brush:c#;">
case "age" or "Age" when reader.TokenType == JsonTokenType.String:
	string? stringValue = reader.GetString();
	if (int.TryParse(stringValue, out int value))
...
<p>Подобным образом считываем из json значение для свойства Name:</p>
<pre class="brush:c#;">
 case "Name" or "name":
	string? name = reader.GetString();
    if(name!=null)
...
<p>В конце полученными данными инициализируем объект Person и возвращаем его из метода:</p>
<pre class="brush:c#;">return new Person(personName, personAge);</pre>
<p>Таким образом, мы можем проверить, какие свойства имеет объект json, какие значения они несут и принять решения, передавать эти значения в объект Person. 
И в данном случае, даже если в присланном json не будет нужных свойств, или свойство age будет содержать строку, которая не конвертируется в число, объект Person все равно будет создан.</p>
<p>Чтобы использовать конвертер json, его надо добавить в коллекцию конвертеров:</p>
<pre class="brush:c#;">
 var jsonoptions = new JsonSerializerOptions();
jsonoptions.Converters.Add(new PersonConverter());
var person = await request.ReadFromJsonAsync&lt;Person&gt;(jsonoptions);
