...
<html >
<head>
<title>ASP.NET Core и C# | Сочетание конечных точек с другими middleware</title>
<meta charset="utf-8" />
<meta name="description" content="Сочетание конечных точек endpoint с другими middleware в ASP.NET Core и C#, порядок обработки запроса, метод HttpContext.GetEndpoint">
...
<h2>Сочетание конечных точек с другими middleware</h2><div class="date">Последнее обновление: 19.12.2021</div>
...
<p>Кроме конечных точек запрос в конвейере обработки могут обрабатывать и другие компоненты middleware. При этом надо учитывать общий процесс обработки запроса и вызова конечных точек.</p>
<p>Так, если приложение содержит конечные точки, то система маршрутизации на основе процессе URL matching или сопоставления адреса URL с шаблонами маршрута 
выбирает для обработки определенную конечную точку. Если в приложении есть такая конечная точка, которая соответствует запросу, то компонент 
middleware <span class="b">Microsoft.AspNetCore.Routing.EndpointRoutingMiddleware</span> устанавливает у объекта HttpContext конечную точку для 
будущей обработки запроса, которую можно получить с помощью метода <span class="b">HttpContext.GetEndpoint()</span>. Кроме того, устанавливаются 
значения маршрута, которые можно получить через коллекцию <span class="b">HttpRequest.RouteValues</span></p>
<p>Однако конечная точка начинает обрабатывать запрос только после того, как все middleware в конвейере начнут обработку запроса. Например, 
возьмем следующий код приложения:</p>
<pre class="brush:c#;">
...
<p>Здесь до и после первой конечной точки с помощью метода <code>app.Use()</code> в конвейер встроены два middleware. Для получения общей картины 
выполнения приложения процесс выполнения логгируется на консоль приложения.</p>
<p>Если мы запустим приложение, то при запросе по адресу "/" ожидаемо для обработки запроса будет выбрана первая конечная точка</p>
<img src="./pics/3.25.png" alt="Сочетание конечных точек endpoint и middleware в конвейере обработки запроса в ASP.NET Core и C#" />
<p>Но теперь взглянем на консоль приложения:</p>
<img src="./pics/3.23.png" alt="Порядок выполнения конечных точек и middleware в конвейере обработки запроса в ASP.NET Core и C#" />
<p>Мы видим, что конечная точка выполняется после того, как начнет выполняться компонент middleware, который в коде идет после добавления этой конечной точки.</p>
<p>При этом в компонентах middleware также можно обрабатывать запросы по определенным адресам:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Кроме того, middleware могут быть полезны для некоторого постдействия - выполнения некоторых действий, когда конечная точка уже выбрана. 
Или, наоборот, если ни одна из конечных точек не обработала запрос, и в middleware мы можем обработать эту ситуацию:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Однако если в конце конвейера располагается терминальный компонент, то он будет выполняться даже если конечная точка соответствует запрошенному пути:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>В данном случае по результату программы мы видим, что даже при запросах по адресу "/" и "/about" будет выполняться middleware из метода app.Run:</p>
<img src="./pics/3.3.png" alt="Терминальный middleware и Map в ASP.NET Core и C#" />
<p>Почему такое происходит? Опять же потому что, сначала выполняются все middleware в конвейере, и только потом выполняется конечная точка.</p>
...
<div class="nav"><p><a href="./3.6.php">Назад</a><a href="./">Содержание</a><a href="./5.1.php">Вперед</a></p></div></div>
<div class="socBlock">
