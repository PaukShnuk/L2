...
<html >
<head>
<title>ASP.NET Core и C# | HttpContext.Items</title>
<meta charset="utf-8" />
<meta name="description" content="Хранение состояния приложения в ASP.NET Core и C#, коллекция HttpContext.Items и ее методы, добавление и получение данных в HttpContext.Items">
...
<h1>Состояние приложения. Куки. Сессии</h1><h2>HttpContext.Items</h2><div class="date">Последнее обновление: 23.12.2021</div>
...
<p>Приложение ASP.NET Core может хранить некоторое состояние. Это могут быть как какие-то глобальные данные, так и данные, которые непосредственно 
относятся к запросу и пользователю. И в зависимости от вида данных, существуют различные способы для их хранения.</p>
<p>Один из простейших способов хранения данных представляет коллекция <span class="b">HttpContext.Items</span> - объект типа 
<code>IDictionary&lt;object, object&gt;</code>. Эта коллекция предназначена для таких данных, которые непосредственно связаны с текущим запросом. 
После завершения запроса все данные из HttpContext.Items удаляются. Каждый объект в этой коллекции имеет ключ и значение. И с помощью ключей можно управлять объектами коллекции.</p>
<p>В каком случае мы можем применить данную коллекцию? Например, если у нас обработка запроса вовлекает множество компонентов middleware, и мы хотим, 
чтобы для этих компонентов были доступны общие данные, то как раз можем применить эту коллекцию. Например, пусть в приложении определен 
следующий код:</p>
<pre class="brush:c#;">
...
<img src="./pics/8.1.png" alt="хранение данных в HttpContext.Items в ASP.NET Core и C#" />
<p>Здесь в одном middleware опредляется ключ "text" со значением "Hello from HttpContext.Items":</p>
<pre class="brush:c#;">context.Items["text"] = "Hello from HttpContext.Items";</pre>
<p>В другом middleware этот объект используется для установки отправляемого ответа.</p>
<p><span class="b">HttpContext.Items</span> предоставляет ряд методов для управления элементами:</p>
<ul>
<li><p><code>void Add(object key, object value)</code>: добавляет объект value с ключом key</p></li>
<li><p><code>void Clear()</code>: удаляет все объекты</p></li>
<li><p><code>bool ContainsKey(object key)</code>: возвращает true, если словарь содержит объект с ключом key</p></li>
<li><p><code>bool Remove(object key)</code>: удаляет объект с ключом key, в случае удачного удаления возвращает true</p></li>
<li><p><code>bool TryGetValue(object key, out object value)</code>: возвращает true, если значение объекта с ключом key успешно получено в объект value</p></li>
</ul>
<p>Применение некоторых методов:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
