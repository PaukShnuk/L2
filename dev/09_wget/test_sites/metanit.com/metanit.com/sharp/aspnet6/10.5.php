...
<html >
<head>
<title>ASP.NET Core и C# | Отправка файлов в Results API</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка файлов в Results API в ASP.NET Core и C#, метод File, отправка файла по пути, через объект Stream и массив байт">
...
<h2>Отправка файлов в Results API</h2><div class="date">Последнее обновление: 24.12.2021</div>
...
<p>Для отправки файлов с помощью Results API может применяться метод <span class="b">File()</span>, который имеет ряд версий.</p>
<h3>Отправка файла как массива байтов</h3>
<p>Для отправки файла как массива байтов применяется следующая версия метода:</p>
<pre class="brush:c#;">
...
<ul>
<li><p><span class="b">fileContents</span>: содержимое файла в виде массива байтов</p></li>
<li><p><span class="b">contentType</span>: тип содержимого файла - заголовок <code>Content-Type</code></p></li>
<li><p><span class="b">fileDownloadName</span>: имя файла для загрузки</p></li>
<li><p><span class="b">enableRangeProcessing</span>: значение типа bool. Если оно равно <code>true</code>, то 
допустима обработка данных по частям</p></li>
<li><p><span class="b">lastModified</span>: значение типа <code>Nullable&lt;DateTimeOffset>&gt;</code>, которое указывает на дату последнего изменения файла</p></li>
<li><p><span class="b">entityTag</span>: значение типа <code>EntityTagHeaderValue</code>, которое ассоциировано с файлом</p></li>
</ul>
<p>Например, создадим в проекте новую папку <span class="b">Files</span>, в которую добавим какой-нибудь файл. Например, в моем случае это 
файл <span class="b">forest.png</span>:</p>
<img src="./pics/10.9.png" alt="Отправка файлов Results API в ASP.NET Core и C#" />
<p>Для отправки файла определим следующий код:</p>
<pre class="brush:c#;">
...
<p>И при обращении по пути "/forest" пользователю будет отправлен файл <code>forest.png</code> под именем "winter_forest.png":</p>
<img src="./pics/10.10.png" alt="Отправка файлов и метод File в Results API в ASP.NET Core и C#" />
<h3>Отправка в виде файлового потока</h3>
<p>Еще одна версия метода <span class="b">File()</span> позволяет отправить данные в виде потока Stream:</p>
<pre class="brush:c#;">
...
<p>Здесь меняется только первый параметр, который в данной версии представляет содержимое файла в виде объекта Stream. Остальные параметры остаются те же, 
что и в предыдущей версии</p>
<p>Также, как и в предыдущем примере, отправим файл forest.png из папки Files:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<h3>Отправка файла по определенному пути</h3>
<p>Если у нас есть путь к файлу, то проще использовать третью версию метода <span class="b">File()</span>, которая в качествве первого параметра принимает путь к файлу:</p>
<pre class="brush:c#;">
public static IResult File (string path, string? contentType = default, 
string? fileDownloadName = default, DateTimeOffset? lastModified = default, 
Microsoft.Net.Http.Headers.EntityTagHeaderValue? entityTag = default, bool enableRangeProcessing = false);
</pre>
<p>Остальные параметры те же, что в ранее рассмотренных версиях метода. Однако стоит отметить, что данная версия метода вычисляет пути к файлам 
относительно папки, которая задается параметром WebRootPath - по умолчанию это папка <span class="b">wwwroot</span>. 
То есть файл для отправки по умолчанию должен располагаться в этой папке:</p>
<img src="./pics/10.11.png" alt="Загрузка файла по пути и Results API в ASP.NET Core и C#" />
<p>Используем эту версию метода для отправки файла <code>wwwroot/Files/forest.png</code>:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Но это поведение мы можем переопределить. Допустим, в проекте есть папка Files. И мы хотим, что приложение автоматически подхватывало из нее файлы.</p>
<img src="./pics/10.12.png" alt="Переустановка папки для отправки файла по пути и Results API в ASP.NET Core и C#" />
<p>Для этого добавим новый каталог для статических файлов:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder(
...
<p>В данном случае путь к файлу <code>newRiver.jpg</code> будет расчитываться относительно папки Files.</p>
