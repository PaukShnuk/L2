...
<html >
<head>
<title>ASP.NET Core и C# | Отправка файлов</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка файлов в middleware в ASP.NET Core и C# с помощью метода Response.SendFileAsync, тип IFileInfo">
...
<h2>Отправка файлов</h2><div class="date">Последнее обновление: 10.12.2021</div>
...
<p>Для отправки файлов применяется метод <span class="b">SendFileAsync()</span>, который получает либо путь к файлу в виде строки, либо информацию о файле в виде 
объекта <span class="b">IFileInfo</span>. Например, допустим нам надо отправить файл по адресу "D:\\forest.jpg":</p>
<pre class="brush:c#;">
...
<p>По умолчанию браузер попытается открыть файл. Так, в случае с изображениями они отображаются в браузере:</p>
<img src="./pics/2.11.png" alt="SendFileAsync и отправка файлов в ASP.NET Core и C#" />
<p>Также мы можем использовать относительные пути. Например, добавим в проект какой-нибудь файл (в моем случае это файл <code>forest.jpg</code>):</p>
<img src="./pics/2.13.png" alt="отправка файлов и SendFileAsync в ASP.NET Core и C#" />
<p>Для этого файла в окне свойств установим для опции <span class="b">Copy to Output Directory</span> значение <code>Copy if newer</code> или <code>Copy always</code>, чтобы файл автоматически копировался в выходной каталог при построении приложения. И установим относительный путь относительно корня приложения:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<h3>Отправка html-страницы</h3>
<p>Подобным образом мы можно отправлять и другие типы файлов, например, html-страницу. Так, определим в проекте новую папку, которую назовем <span class="b">html</span>. 
В эту папку добавим новый файл <span class="b">index.html</span>:</p>
<img src="./pics/2.22.png" alt="Отправка html-страницы в ASP.NET Core и C#" />
<p>Определим в файле <span class="b">index.html</span> следующий код:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Определим для отправки веб-страницы следующий код:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>В итоге при обращении к приложению сервер возвратит страницу index.html:</p>
<img src="./pics/2.23.png" alt="Отправка страницы html с помощью SendFileAsync в ASP.NET Core и C#" />
<p>Теперь немного усложним задачу. Добавим в проект в папку <span class="b">html</span> еще пару файлов. Назовем их, к примеру, <span class="b">about.html</span> и 
<span class="b">contact.html</span>.</p>
<img src="./pics/2.24.png" alt="Отправка статических файлов с помощью SendFileAsync в ASP.NET Core и C#" />
<p>Для отправки этих файлов определим следующий код:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Когда приходит запрос, мы сопоставляем путь запроса (path) с файлами в папке html. То есть если <code>path = about.html</code>, то нам надо оправить в ответ 
файл <code>about.html</code>. При этом проверяем наличие файла. Если он есть в папке, то отправляем данный файл. Если нет, то отправляем статусный код 404 и сообщение, что ресус не найден:</p>
<img src="./pics/2.25.png" alt="Отправка статических файлов в ASP.NET Core и C#" />
<p>Стоит отметить, что в ASP.NET Core уже имеется встроенный middleware, который позволяет упростить работу со статическими файлами.</p>
<h3>Загрузка файла</h3>
<p>По умолчанию браузер пытается открыть отправляемый файл, что может быть полезно в случае файлов html - мы можем определить файл html и таким образом отправить 
...
"Content-Disposition" значение "attachment":</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>В этом случае загруженный файл получит имя "my_forest.jpg"</p>
<img src="./pics/2.14.png" alt="SendFileAsync и загрузка файлов в ASP.NET Core и C#" />
<h3>IFileInfo</h3>
<p>В примерах выше применялась версия метода <span class="b">SendFileAsync()</span>, которая получает путь к файлу в виде строки. Также можно использовать другую версию, 
которая получает информацию о файле в виде объекта <span class="b">IFileInfo</span>:</p>
<pre class="brush:c#;">
...
<p>В этом случае сначала необходимо определить объект <span class="b">PhysicalFileProvider</span>, конструктор которого получает каталог для поиска файлов. В 
его метод <code>fileProvider.GetFileInfo()</code> передается путь к файлу в рамках этого каталога. А результатом метода является объект IFileInfo, который передается 
в <code>SendFileAsync()</code></p>
...
<div class="nav"><p><a href="./2.5.php">Назад</a><a href="./">Содержание</a><a href="./2.8.php">Вперед</a></p></div></div>
<div class="socBlock">
