...
<html >
<head>
<title>ASP.NET Core и C# | Конфигурация в файлах JSON, XML и Ini</title>
<meta charset="utf-8" />
<meta name="description" content="Файловые провайдеры конфигурации в ASP.NET Core и C#, хранение настроек в файлах json, ini и xml, appsettings.json, методы AddJsonFile, AddXmlFile и AddIniFile">
...
<h2>Конфигурация в файлах JSON, XML и Ini</h2><div class="date">Последнее обновление: 21.12.2021</div>
...
<p>Как правило, для хранения конфигурации в приложении ASP.NET Core используются файлы json. Для работы с файлами json применяется провайдер 
<span class="b">JsonConfigurationProvider</span>, а для загрузки конфигурации из json применяется метод расширения <span class="b">AddJsonFile()</span>.</p>
...
по умолчанию в приложении и которые мы можем использовать для хранения конфигурационных настроек.</p>
<img src="./pics/6.6.png" alt="Файл appsettings.json и appsettings.Development.json в ASP.NET Core и C#" />
<p>Например, код файла appsettings.json:</p>
<pre class="brush:js;">
{
  "Logging": {
    "LogLevel": {
...
<p>Однако в данном случае для примера возьмем новый файл json. Итак, добавим в проект новый файл <span class="b">config.json</span>:</p>
<img src="./pics/6.8.png" alt="Конфигурация json в ASP.NET Core и C#" />
...
<p>Здесь задается два элемента с ключами "person" и "company". Используем эти настройки в приложении:</p>
<pre class="brush:c#;">
...
<p>Для установки конфигурации из json-файла название файла передается в метод <code>AddJsonFile()</code>.</p>
<img src="./pics/6.9.png" alt="Конфигурация из файла json в ASP.NET Core и C#" />
...
<p>Итак, добавим в проект новый xml-файл, который назовем <span class="b">config.xml</span>. Затем изменим его код на следующий:</p>
<pre class="brush:xml;">
...
<p>Здесь определены два элемента person и company, которые представляют конфигурационные настройки.</p>
...
<img src="./pics/6.10.png" alt="Конфигурация из файла XML в ASP.NET Core и C#" />
...
<p>то мы можем обращаться к этим уровням также, как и в файле json:</p>
<pre class="brush:c#;">
app.Map("/", (IConfiguration appConfig) =&gt;
{
    var personName = appConfig["person:profile:name"];
...
<p>Например, добавим в проект текстовый файл и переименуем его в <span class="b">config.ini</span>.</p>
<img src="./pics/6.11.png" alt="Конфигурация из файла ini в ASP.NET Core и C#" />
<p>Определим в этом файле следующее содержимое:</p>
<pre class="brush:js;">
person="Bob"
company="Microsoft"
</pre>
<p>Подключим конфигурацию из этотого файла в приложении:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
