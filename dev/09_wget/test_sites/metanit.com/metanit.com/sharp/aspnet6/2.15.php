...
<html >
<head>
<title>ASP.NET Core и C# | Классы middleware</title>
<meta charset="utf-8" />
<meta name="description" content="Классы middleware и их добавление в конвейер обработки запроса в ASP.NET Core и C#, создание метода расширения для IApplicationBuilder">
...
<h2>Классы middleware</h2><div class="date">Последнее обновление: 15.12.2021</div>
...
<p>В прошлых темах используемые компоненты middleware фактически представляли методы, то есть так называемые <span class="b">inline middleware</span>. 
Однако также ASP.NET Core позволяет определять middleware в виде отдельных классов.</p>
<p>Итак, добавим в проект новый класс, который назовем <span class="b">TokenMiddleware</span> и который будет иметь следующий код:</p>
<pre class="brush:c#;">
...
<p>Класс middleware должен иметь конструктор, который принимает параметр типа <span class="b">RequestDelegate</span>. Через этот параметр можно получить ссылку на тот делегат запроса, который стоит следующим в конвейере 
обработки запроса.</p>
<p>Также в классе должен быть определен метод, который должен называться либо <span class="b">Invoke</span>, либо <span class="b">InvokeAsync</span>. 
Причем этот метод должен возвращать объект <span class="b">Task</span> и принимать в качестве параметра контекст запроса - объект <span class="b">HttpContext</span>. 
Данный метод собственно и будет обрабатывать запрос.</p>
<p>Суть действия класса заключается в том, что мы получаем из запроса параметр "token". Если полученный токен равен строке "12345678", то передаем запрос 
дальше следующему компоненту, вызвав метод <code>_next.Invoke()</code>. Иначе возвращаем пользователю сообщение об ошибке.</p>
<p>Для добавления компонента middleware, который представляет класс, в конвейер обработки запроса применяется метод <span class="b">UseMiddleware()</span>. 
Так, изменим файл <span class="b">Program.cs</span> следующим образом:</p>
<pre class="brush:c#;">
...
<p>С помощью метода <span class="b">UseMiddleware&lt;T&gt;</span> в конструктор объекта TokenMiddleware будет внедряться объект 
для параметра <code>RequestDelegate next</code>. Поэтому явным образом передавать значение для этого параметра нам не нужно.</p>
<p>Запустим проект. И если мы не передадим через строку запроса параметр token или передадим для него значение, отличное от 
"12345678", то браузер отобразит ошибку:</p>
<img src="./pics/2.46.png" alt="Классы middleware в ASP.NET Core и C#" />
<p>Если же будет передан корректный токен, то вызов <code>app.UseMiddleware&lt;TokenMiddleware&gt;()</code> передаст обработку запроса 
в компонент middleware из <code>app.Run()</code>:</p>
<img src="./pics/2.47.png" alt="Классы middleware и UseMiddleware в ASP.NET Core и C#" />
<h3>Метод расширения для встраивания middleware</h3>
<p>Также нередко для встраивания подобных компонентов middleware определяются специальные методы расширения. 
Так, добавим в проект новый класс, который назовем TokenExtensions:</p>
<pre class="brush:c#;">
...
<p>Здесь создается метод расширения для типа IApplicationBuilder. И этот метод встраивает компонент TokenMiddleware в конвейер 
обработки запроса. Как правило, подобные методы возвращают объект IApplicationBuilder.</p>
<p>Теперь применим этот метод в коде программы в <span class="b">Program.cs</span>:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Изменим класс TokenMiddleware, чтобы он извне получал образец токена для сравнения:</p>
<pre class="brush:c#;">
public class TokenMiddleware
{
    private readonly RequestDelegate next;
...
<p>Образец токена, с которым идет сравнения, устанавливается через конструктор. Чтобы передать его в конструктор, изменим класс TokenExtensions:</p>
<pre class="brush:c#;">
public static class TokenExtensions
{
...
<p>В метод <code>builder.UseMiddleware</code> можно передать набор значений, которые передаются в конструктор компонента middleware.</p>
<p>И при вызове метода расширения UseToken в него можно передать конкретное значение:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/2.48.png" alt="Передача параметров в классы middleware в ASP.NET Core и C#" />
