...
<html >
<head>
<title>ASP.NET Core и C# | Проекция конфигурации на классы</title>
<meta charset="utf-8" />
<meta name="description" content="Проекция конфигурации json и xml на классы в ASP.NET Core и C#,  метод Bind и Get у IConfiguration, привязка отдельных секций конфигурации к объектам классов">
...
<h2>Проекция конфигурации на классы</h2><div class="date">Последнее обновление: 22.12.2021</div>
...
<p>Фреймворк ASP.NET Core позволяет проецировать конфигурационные настройки на классы C#.</p>
<p>Например, определим в проекте новый файл <span class="b">person.json</span>, который будет хранить данные пользователя:</p>
<pre class="brush:js;">
{
  "name": "Tom",
  "age": "22"
...
<p>Под эти данные определим в проекте класс <span class="b">Person</span>:</p>
<pre class="brush:c#;">
public class Person
{
...
<p>Теперь свяжем конфигурацию из файла <span class="b">person.json</span> с объектом класса <span class="b">Person</span>:</p>
<pre class="brush:c#;">
...
<p>Ключевой момент заключается в применении метода <span class="b">Bind</span>:</p>
<pre class="brush:c#;">
var tom = new Person();
app.Configuration.Bind(tom);
</pre>
<p>Для объекта IConfiguration определен метод <span class="b">Bind()</span>, который в качестве параметра принимает объект, который надо связать с данными. 
Стоит отметить, что между конфигурацией в json и классом Person имеется соответствие по названию свойств, благодаря чему может осуществляться связка (регистр в данном случае роли не играет).</p>
<img src="./pics/6.17.png" alt="Привязка конфигурации json и класса C# в ASP.NET Core" />
<p>В качестве альтернативы методу Bind мы могли бы использовать метод <span class="b">Get&lt;T&gt;()</span>, который возвращает объект созданного класса:</p>
<pre class="brush:c#;">Person tom = app.Configuration.Get&lt;Person&gt;();</pre>
<p>В этом случае нам не надо предварительно создавать объект класса Person.</p>
<p>Подобным образом можно выполнять привязку при получении конфигурации через механизм Dependency Injection:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Рассмотрим привязку более сложных по структуре данных. Определим следующий файл <span class="b">person.json</span>:</p>
<pre class="brush:js;">
{
  "age": "28",
  "name": "Tom",
...
<p>Для представления этих данных в коде C# определим следующие классы:</p>
<pre class="brush:c#;">
public class Person
{
    public string Name { get; set; } = "";
...
<p>Теперь выполним в приложении привязку из конфигурации json в объекты классов C#:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/6.18.png" alt="Привязка комплексных объектов из конфигурации json к классам C# в ASP.NET Core" />
<h3>Привязка конфигурации из xml</h3>
<p>Возьмем выше определенные классы Person и Company. И добавим в проект файл <span class="b">person.xml</span>, который будет содержать аналогичные данные:</p>
<pre class="brush:xml;">
...
<p>Обратите внимание на установку в файле xml массивов - они имеют атрибут name, который определяет условный индекс.</p>
<p>Применим конфигурацию из выше определенного файла xml в приложении:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Как можно заметить из кода, меняется только подключение файла с json на xml, а весь остальной код остается прежним.</p>
<h3>Привязка секций конфигурации</h3>
<p>В примерах выше выполнялась привязка корневого объекта конфигурации, однако также можно осуществлять привязку отдельных секций. Например, выше в 
файле json и xml была определена секция company, которая хранит компанию пользователя. Выполним привязку отдельно этой секции к объекту класса Company:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>С помощью метода <code>GetSection()</code> получаем нужную нам секцию конфигурации и затем также можно вызвать методы Bind или Get и выполнить привязку.</p>
<img src="./pics/6.19.png" alt="Привязка секции конфигурации к объекту класса C# в ASP.NET Core" />
