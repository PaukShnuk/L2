...
<html >
<head>
<title>ASP.NET Core и C# | Ограничения маршрутов</title>
<meta charset="utf-8" />
<meta name="description" content="Встроенные ограничения маршрутов route constraints в ASP.NET Core и C#, inline constraints и их использование">
...
<h2>Ограничения маршрутов</h2><div class="date">Последнее обновление: 19.12.2021</div>
...
<p>При обработке запроса система маршрутизации на основании шаблон маршрута автоматически извлекает из строки запроса значения для параметров 
...
а другой параметр начинался строго с определенного символа. И для этого необходимо задать <span class="b">ограничения маршрута</span> (<span class="b">route constraints</span>).</p> 
<p>Ограничения маршрутов выполняюся при парсинге пути запроса, сопоставлении его с шаблоном маршрута и выделении из него значений для параметров маршрута. 
...
путь запроса с определенным маршрутом. Кроме того, ограничения маршрута могут применяться при генерации ссылок.</p>
<p>Вначале рассмотрим следующую ситуацию. Пусть у нас есть следующее приложение:</p>
<pre class="brush:c#;">
...
<p>Первая конечная точка использует маршрут, который подразумевает наличие параметра <code>id</code>. А в обработчике маршрута мы хотим получить значение этого параметра в виде числа. 
То есть мы ожидаем, что в пути запроса будет передаваться число, например, "/users/123". Тем не менее мы можем передать вместо числа и строку:</p>
<img src="./pics/3.12.png" alt="Ограничения маршрутов в ASP.NET Core и C#" />
<p>На скриншоте выше обращение идет по адресу "/users/hello". И этот путь запроса в принципе соответствует шаблону маршрута "/users/{id}". В этом случае 
параметр маршрута <code>id</code> получит значение "hello", а для обработки запроса будет запущено действие</p>
<pre class="brush:c#;">(int id) =&gt; $"User Id: {id}";</pre>
<p>Однако параметр id обработчика представляет тип <code>int</code>, а строка "hello" никак не может быть конвертирована в число. Соответственно мы получим ошибку при преобразовании, 
о чем собственно и говорит скриншот.</p>
<p>Теперь применим ограничения - укажем, что параметр id в маршруте должен представлять тип <code>int</code>:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Для установки ограничения после названия параметра через двоеточие указывается название ограничения. Ограничение <code>int</code> указывает, что параметр должен 
представлять тип int.</p>
<p>В этом случае нам надо передать этому параметру число:</p>
<img src="./pics/3.13.png" alt="Установка ограничений для параметров маршрута в ASP.NET Core и C#" />
<p>Да, мы по-прежнему можем передавать в пути запроса строку вместо числа:</p>
<img src="./pics/3.14.png" alt="Установка ограничений для параметров маршрута в ASP.NET Core и C#" />
<p>Но в этом случае инфраструктура ASP.NET Core просто не сможет сопоставить данный путь запроса с шаблоном маршрута. И клиент получит ошибку 404, то есть 
ресурс не найден.</p>
<p>ASP.NET Core предоставляет следующий ряд ограничений маршрута:</p>
<ul>
<li><p><span class="b">int</span></p>
<p>Соответствие целому числу. Представляет класс IntRouteConstraint</p>
<pre class="brush:c#;">{id:int}</pre></li>
<li><p><span class="b">bool</span>. Представляет класс BoolRouteConstraint</p>
<p>Соответствие значению true или false</p>
<pre class="brush:c#;">{active:bool}</pre></li>
<li><p><span class="b">datetime</span></p>
<p>Соответствие дате и времени. Представляет класс DateTimeRouteConstraint</p>
<pre class="brush:c#;">{date:datetime}</pre></li>
<li><p><span class="b">decimal</span></p>
<p>Соответствие значению decimal. Представляет класс DecimalRouteConstraint</p>
<pre class="brush:c#;">{price:decimal}</pre></li>
<li><p><span class="b">double</span></p>
<p>Соответствие значению типа double. Представляет класс DoubleRouteConstraint</p>
<pre class="brush:c#;">{weight:double}</pre></li>
<li><p><span class="b">float</span></p>
<p>Соответствие значению типа float. Представляет класс FloatRouteConstraint</p>
<pre class="brush:c#;">{height:float}</pre></li>

<li><p><span class="b">guid</span></p>
<p>Соответствие значению типа Guid. Представляет класс GuidRouteConstraint</p>
<pre class="brush:c#;">{id:guid}</pre></li>

<li><p><span class="b">long</span></p>
<p>Соответствие значению типа long. Представляет класс LongRouteConstraint</p>
<pre class="brush:c#;">{id:long}</pre></li>

<li><p><span class="b">minlength(value)</span></p>
<p>Строка должна иметь символов не меньше value. Представляет класс MinLengthRouteConstraint</p>
<pre class="brush:c#;">{name:minlength(3)}</pre></li>
<li><p><span class="b">maxlength(value)</span></p>
<p>Строка должна иметь символов не больше value. Представляет класс MaxLengthRouteConstraint</p>
<pre class="brush:c#;">{name:maxlength(20)}</pre></li>
<li><p><span class="b">length(value)</span></p>
<p>Строка должна иметь ровно столько символов, сколько определено в параметре value. Представляет класс LengthRouteConstraint</p>
<pre class="brush:c#;">{name:length(10)}</pre></li>
<li><p><span class="b">length(min, max)</span></p>
<p>Строка должна иметь символов не меньше min и не больше max. Представляет класс LengthRouteConstraint</p>
<pre class="brush:c#;">{name:length(3, 20)}</pre></li>
<li><p><span class="b">min(value)</span></p>
<p>Число должно быть не меньше value. Представляет класс MinRouteConstraint</p>
<pre class="brush:c#;">{age:min(3)}</pre></li>
<li><p><span class="b">max(value)</span></p>
<p>Число должно быть не больше value. Представляет класс MaxRouteConstraint</p>
<pre class="brush:c#;">{age:max(20)}</pre></li>
<li><p><span class="b">range(min, max)</span></p>
<p>Число должно быть не меньше min и не больше max. Представляет класс RangeRouteConstraint</p>
<pre class="brush:c#;">{age:range(18, 99)}</pre></li>

<li><p><span class="b">alpha</span></p>
<p>Строка должна состоять из одного и более алфавитных символов. Представляет класс AlphaRouteConstraint</p>
<pre class="brush:c#;">{name:alpha}</pre></li>
<li><p><span class="b">regex(expression)</span></p>
<p>Строка должна соответствовать регулярному выражению expression. Представляет класс RegexRouteConstraint</p>
<pre class="brush:c#;">{phone:regex(^\\d{{3}}-\\d{{3}}-\\d{{4}}$)}</pre></li>
<li><p><span class="b">required</span></p>
<p>Параметр является обязательным, и его значение должно быть определено. Представляет класс RequiredRouteConstraint</p>
<pre class="brush:c#;">{name:required}</pre></li>
</ul>
<p>Каждое подобное ограничени представляет определенный класс. Все классы ограничений находятся в пространстве имен 
<a href="https://github.com/aspnet/AspNetCore/tree/main/src/Http/Routing/src/Constraints" rel="nofollow">Microsoft.AspNetCore.Routing.Constraints</a></p>
<p>Ограничения можно комбинировать. При применении нескольких ограничений одновременно, они отделяются друг о друга двоеточием Например:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Первая конечная точка использует шаблон маршрута</p>
<pre class="browser">"/users/{name:alpha:minlength(2)}/{age:int:range(1, 110)}"</pre>
<p>Здесь предполагается, что параметр <code>name</code> принимает только алфавитные символы, а его минимальная длина должна представлять два символа. 
Второй же параметр маршрута - <code>age</code> должен представлять целое число и должен находиться в диапазоне между 1 и 110:</p>
<img src="./pics/3.16.png" alt="Длина строка параметров маршрута в ASP.NET Core и C#" />
<p>Вторая конечная точка использует шаблон маршрута</p>
<pre class="browser">"/phonebook/{phone:regex(^7-\\d{{3}}-\\d{{3}}-\\d{{4}}$)}"</pre>
<p>Параметр <code>phone</code> принимает номер телефона в формате 7-ххх-ххх-хххх, любые другие форматы будут некорректными:</p>
<img src="./pics/3.15.png" alt="Регулярные выражения в ограничениях маршрута в ASP.NET Core и C#" />
