...
<html >
<head>
<title>ASP.NET Core и C# | Обработка ошибок HTTP</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка ошибок HTTP в ASP.NET Core и C#, методы UseDeveloperExceptionPage и UseExceptionHandler, установка обработчика исключений">
...
<h2>Обработка ошибок HTTP</h2><div class="date">Последнее обновление: 23.12.2021</div>
...
стандартную страницу.</p>
<p>Но с помощью компонента <span class="b">StatusCodePagesMiddleware</span> можно добавить в проект отправку информации о статусном коде. 
Для этого применяется метод <span class="b">app.UseStatusCodePages()</span>:</p>
<pre class="brush:c#;">
...
<img src="./pics/9.4.png" alt="метод UseStatusCodePages и отправка информации об ошибке в ASP.NET Core и C#" />
<p>Метод <span class="b">UseStatusCodePages()</span> следует вызывать ближе к началу конвейера обработки запроса, в частности, до 
добавления middleware для работы со статическими файлами и до добавления конечных точек.</p>
<h3>Настройка сообщения</h3>
<p>Сообщение, отправляемое методом <code>UseStatusCodePages()</code> по умолчанию, не очень информативное. Однако одна из версий метода позволяет настроить отправляемое 
пользователю сообщение. В частности, мы можем изменить вызов метода так:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>В качестве первого параметра указывается MIME-тип ответа - в данном случае простой текст (""text/plain""). В качестве второго параметра 
передается собственно то сообщение, которое увидит пользователь. В сообщение мы можем передать код ошибки через плейсхолдер "{0}".</p>
<img src="./pics/9.5.png" alt="метод UseStatusCodePages и настройка сообщения об ошибке в ASP.NET Core и C#" />
<h3>Установка обработчика ошибок</h3>
<p>Еще одна версия метода <span class="b">UseStatusCodePages()</span> позволяет более детально задать обаботчик ошибок. В частности, она принимает делегат, параметр которого - объект <span class="b">StatusCodeContext</span>. 
В свою очередь, объект StatusCodeContext имеет свойство HttpContext, из которого мы можем получить всю информацию о запросе и настроить отправку ответа. Например:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/9.6.png" alt="метод UseStatusCodePages и настройка обработки ошибки HTTP в ASP.NET Core и C#" />

<h3>Методы UseStatusCodePagesWithRedirects и UseStatusCodePagesWithReExecute</h3>
<p>Вместо метода <code>UseStatusCodePages()</code> мы также можем использовать еще пару других, которые также обрабатывают ошибки HTTP.</p>
...
<p>Здесь будет идти перенаправление по адресу "/error/{0}". В качестве параметра через плейсхолдер "{0}" будет передаваться статусный код 
ошибки.</p>
...
<p>В качестве параметра метод <code>UseStatusCodePagesWithReExecute()</code> приминает путь к ресурсу, который будет обрабатывать ошибку. И также с помощью 
плейсхолдера <code>{0}</code> можно передать статусный код ошибки. То есть в данном случае при возникновении ошибки будет вызываться конечная точка</p>
<pre class="brush:c#;">app.Map("/error/{statusCode}", (int statusCode) =&gt; $"Error. Status Code: {statusCode}");</pre>
<p>Формально мы получим тот же ответ, так как так же будет идти перенаправление на путь "/error/404". Но теперь браузер получит оригинальный статусный код 404.</p>
<img src="./pics/9.7.png" alt="метод UseStatusCodePagesWithReExecute и обработки ошибок HTTP в ASP.NET Core и C#" />
<p>Также можно задействовать другую версию метода, которая в качестве второго параметра принимает параметры строки запроса</p>
...
Вместо плейсхолдера {0} опять же будет передаваться статусный код ошибки. Пример использования:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
