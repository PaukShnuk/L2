...
<html >
<head>
<title>ASP.NET Core и C# | Построение конвейера обработки запроса</title>
<meta charset="utf-8" />
<meta name="description" content="Создание конвейера обработки запроса из компонентов middleware в ASP.NET Core и C#, порядок обработки запроса и генерации ответа">
...
<h2>Построение конвейера обработки запроса</h2><div class="date">Последнее обновление: 15.12.2021</div>
...
<p>Обычно для обработки запроса применяется не один, а несколько компонентов middleware. И в этом случае большую роль может играть порядок их 
...
<p>Рассмотрим простейший пример. Определим следующий класс <span class="b">RoutingMiddleware</span> для обработки запроса:</p>
<pre class="brush:c#;">
...
Для этого добавим новый класс <span class="b>"AuthenticationMiddleware:</p>
<pre class="brush:c#;">
...
<p>Поскольку компоненту <code>RoutingMiddleware</code> нет смысла обрабатывать запрос, если пользователь не аутентифицирован, то в конвейере компонент 
<code>AuthenticationMiddleware</code> должен быть помещен перед компонентом <code>RoutingMiddleware</code>:</p>
<pre class="brush:c#;">
...
<img src="./pics/2.49.png" alt="Организация pipeline в ASP.NET Core и C#" />
<p>Добавим еще один компонент middleware, который назовем <span class="b">ErrorHandlingMiddleware</span>:</p>
<pre class="brush:c#;">
...
<p>В отличие от предыдущих двух компонентов <code>ErrorHandlingMiddleware</code> сначала передает запрос на выполнение последующим делегатам, а потом уже 
...
<code>await next.Invoke(context);</code>, а после завершения обработки последующих компонентов вызывается та часть 
кода, которая идет после <code>await next.Invoke(context);</code>. 
И в данном случае для <code>ErrorHandlingMiddleware</code> важен результат обработки запроса последующими компонентами. В частности, он устанавливает сообщения об 
ошибках в зависимости от того, как статусный код установили другие компоненты. Поэтому <code>ErrorHandlingMiddleware</code> должен быть помещен первым из всех трех компонентов:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/2.50.png" alt="Конвейер обработки запроса в ASP.NET Core и C#" />
...
<img src="./pics/2.51.png" alt="порядок обработки запроса в Middleware в ASP.NET Core и C#" />
<p>В первом случае, если указан параметр token, то запрос будет обработан RoutingMiddleware:</p>
<img src="./pics/2.52.png" alt="Этапы обработки запроса в ASP.NET Core и C#" />
<p>Иначе пользователь получит ошибку 403:</p>
<img src="./pics/2.53.png" alt="Цикл обработки запроса в ASP.NET Core и C#" />
