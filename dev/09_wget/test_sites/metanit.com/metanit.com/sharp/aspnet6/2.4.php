...
<html >
<head>
<title>ASP.NET Core и C# | HttpResponse. Отправка ответа</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка ответа в компоненте middleware в ASP.NET Core и C# с помощью класса HttpResponse, установка заголовков и статусных кодов, метод Response.WriteAsync, отправка html и json, Response.WriteAsJson">
...
<h2>HttpResponse. Отправка ответа</h2><div class="date">Последнее обновление: 10.12.2021</div>
...
<p>Все данные запроса передаются в middleware через объект <span class="b">Microsoft.AspNetCore.Http.HttpContext</span>. Этот объект инкапсулирует информацию о запросе, 
позволяет управлять ответом и, кроме того, имеет еще много другой функциональности. Например, возьмем простейшее приложение:</p>
<pre class="brush:c#;">
...
<p>Здесь параметр <code>context</code>, который передается в middleware в методе <code>app.Run()</code> как раз представляет объект <span class="b">HttpContext</span>. 
И через этот объект, точнее через его свойство <span class="b">Response</span> мы можем отправить клиенту некоторый ответ: <code>context.Response.WriteAsync($"Hello METANIT.COM")</code>.</p>
<p>Свойство <span class="b">Response</span> объекта HttpContext представляет объект <span class="b">HttpResponse</span> и устанавливает, что будет отравляться в виде ответа. 
Для установки различных аспектов ответа класс HttpResponse определяет следующие свойства:</p>
<ul>
<li><p><span class="b">Body</span>: получает или устанавливает тело ответа в виде объекта <span class="b">Stream</span></p></li>
<li><p><span class="b">BodyWriter</span>: возвращает объект типа <span class="b">PipeWriter</span> для записи ответа</p></li>
...
<li><p><span class="b">Cookies</span>: возвращает куки, отправляемые в ответе</p></li>
<li><p><span class="b">HasStarted</span>: возвращает <code>true</code>, если отправка ответа уже началась</p></li>
<li><p><span class="b">Headers</span>: возвращает заголовки ответа</p></li>
<li><p><span class="b">Host</span>: получает или устанавливает заголовок <code>Host</code></p></li>
<li><p><span class="b">HttpContext</span>: возвращает объект <span class="b">HttpContext</span>, связанный с данным объектом Response</p></li>
<li><p><span class="b">StatusCode</span>: возвращает или устанавливает статусный код ответа</p></li>
</ul>
<p>Чтобы отправить ответ, мы можем использовать ряд методов класса HttpResponse:</p>
<ul>
<li><p><span class="b">Redirect()</span>: выполняет переадресацию (временную или постоянную) на другой ресурс</p></li>
<li><p><span class="b">WriteAsJson()/WriteAsJsonAsync()</span>: отправляет ответ в виде объектов в формате JSON</p></li>
<li><p><span class="b">WriteAsync()</span>: отправляет некоторое содержимое. Одна из версий метода позволяет указать кодировку. Если кодировка не указана, то по умолчанию применяется кодировка UTF-8</p></li>
<li><p><span class="b">SendFileAsync()</span>: отправляет файл</p></li>
</ul>
<p>Самый простой способ отправки ответа представляет метод <span class="b">WriteAsync()</span>, в который передается отправляемые данные. В качестве дополнительного 
параметра мы можем указать кодировку:</p>
<pre class="brush:c#;">
...
<h3>Установка заголовков</h3>
<p>Для установки заголовков применяется свойство <span class="b">Headers</span>, которое представляет тип <span class="b">IHeaderDictionary</span>. 
Для большинства стандартных заголовков HTTP в этом интерфейсе определены одноименные свойства, например, для заголовка "content-type" определено свойство <code>ContentType</code>. 
Другие, в том числе свои кастомные заголовки можно добавить через метод <code>Append()</code>. Например:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/2.9.png" alt="Установка заголовков в ответе в ASP.NET Core и C#" />
<p>Стоит отметить, что для вывода кириллицы желательно устанавливать заголовок ContentType, в том числе кодировку, которая применяется в отправляемом содержимом (в примере выше это <code>"text/plain; charset=utf-8"</code>).</p>
<p>Также стоит отметить, что вместо</p>
<pre class="brush:c#;">response.Headers.ContentType = "text/plain; charset=utf-8";</pre>
<p>можно было бы написать</p>
<pre class="brush:c#;">response.ContentType = "text/plain; charset=utf-8";</pre>
<h3>Установка кодов статуса</h3>
<p>Для установки статусных кодов применяется свойство <span class="b">StatusCode</span>, которому передается числовой код статуса:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>В данном случае устанавливается код 404, который указывает, что ресурс не найден.</p>
<img src="./pics/2.15.png" alt="Установка кодов статуса в ASP.NET Core и C#" />
<h3>Отправка html-кода</h3>
<p>Если необходимо отправить html-код, то для этого необходимо установить для заголовка <code>Content-Type</code> значение <code>text/html</code>:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/2.8.png" alt="Отправка html кода из ASP.NET Core и C#" />
