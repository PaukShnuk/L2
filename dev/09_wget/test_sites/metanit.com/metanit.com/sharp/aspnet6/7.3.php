...
<html >
<head>
<title>ASP.NET Core и C# | Конфигурация и фильтрация логгирования</title>
<meta charset="utf-8" />
<meta name="description" content="Определение конфигурации логгирования в ASP.NET Core и C#,  хранение настроек логгера в файле json, установка фильтров логгера, метод AddFilter объекта ILoggerFactory, правила фильтрации сообщений при логгировании">
...
<h2>Конфигурация и фильтрация логгирования</h2><div class="date">Последнее обновление: 23.12.2021</div>
...
уровня Warning.</p>
<p>Мы можем, например, убрать логгирование для категории "Microsoft.AspNetCore":</p>
<pre class="brush:js;">
{
  "Logging": {
    "LogLevel": {
...
<p>В данном случае для всех категорий будут логироваться сообщения уровня "Information". Для категории "Microsoft.AspNetCore" логируются сообщения уровня 
"Warning".</p>
...
<p>Следует учитывать, что настройки узла <code>Logging.{имя_провайдера}.LogLevel</code> переопределяют настройки в узле <code>Logging.LogLevel</code></p>
<p>Также можно настроить применяемые фильтры программно с помощью фабрики логгера:</p>
<pre class="brush:c#;">
...
<pre class="brush:c#;">builder.AddFilter("System", LogLevel.Information)</pre>
<p>устанавает для категории "System" уровень <code>LogLevel.Information</code> для всех провайдеров. Это все равно, если бы мы написали в файле appsettings.json:</p>
<pre class="brush:js;">
"Logging": {
    "LogLevel": {
      "System": "Information"
...
<p>Можно определять фильтры сразу и в коде C#, и в файле конфигурации. При создании логгера объект ILoggerFactory выбирает и применяет одно правило для 
каждого провайдера. Выбор нужного правила состоит из ряда этапов:</p>
<ol>
...
<li><p>Если на предыдущем шаге не выбрано никаких правил, то применяется настройка <code>MinimumLevel</code>. Данную опцию также можно определить в 
коде, например, при настройке фабрики логгера:</p>
<pre class="brush:c#;">
var loggerFactory = LoggerFactory.Create(builder =&gt;
{
	builder.AddDebug();
...
<h3>Глобальная настройка логгирования</h3>
<p>Для глобальной установки настроек логгирования у объекта WebApplicationBuilder определено свойсво <span class="b">Logging</span>, которое представляет 
тип <span class="b">ILogingBuilder</span>. Для управления логгирования он предоставляет ряд методов:</p>
<ul>
<li><p><code>AddConfiguration()</code>: добавляет конфигурацию логгера в виде объекта IConfiguration</p></li>
<li><p><code>AddConsole()</code>: добавляет консольный логгер</p></li>
<li><p><code>AddConsoleFormatter()</code>: добавляет объект форматирования для консольного вывода</p></li>
<li><p><code>AddJsonConsole()</code>: добавляет форматирование сообщений консольного логгера в формат "json"</p></li>
<li><p><code>AddSimpleConsole()</code>: добавляет простое форматирование для логгирования на консоль</p></li>
<li><p><code>AddDebug()</code>: добавляет логгер отладки</p></li>
<li><p><code>AddEventLog()</code>: добавляет логгер для вывод в журнал Windows Event Log</p></li>
<li><p><code>AddEventSourceLogger()</code>: добавляет логгер для вывода в лог Event Tracing for Windows</p></li>
<li><p><code>AddFilter()</code>: добавляет фильтрацию для сообщений логгера</p></li>
<li><p><code>AddProvider()</code>: добавляет провайдер логгирования</p></li>
<li><p><code>ClearProviders()</code>: удаляет все зарегистрированные провайдеры логгирования</p></li>
<li><p><code>SetMinimumLevel()</code>: устанавливает минимальный уровень логгирования</p></li>
</ul>
<p>Например:</p>
<pre class="brush:c#;">
