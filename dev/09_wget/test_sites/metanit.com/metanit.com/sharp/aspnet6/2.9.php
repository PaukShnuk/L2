...
<html >
<head>
<title>ASP.NET Core и C# | Переадресация</title>
<meta charset="utf-8" />
<meta name="description" content="Редирект в middleware в ASP.NET Core и C# с помощью метода Response.Redirect, постоянная и временная переадресация">
...
<h2>Переадресация</h2><div class="date">Последнее обновление: 12.12.2021</div>
...
<p>Для выполнения переадресации у объекта <span class="b">HttpResponse</span> определен метод <span class="b">Redirect()</span>:</p>
<pre class="brush:c#;">
void Redirect(string location)
void Redirect(string location, bool permanent)
</pre>
<p>Первая версия выполняет временную переадресацию. В качестве параметра получает адрес для редиректа, а клиенту посылается статусный код 302.</p>
<p>Вторая версия метода также в качестве второго параметра получает булевое значение, которое 
указывает, будет ли переадресация постоянной. Если этот параметр равен <code>true</code>, то переадресация будет постоянной, и в этом случае посылается 
статусный код 301. Если равен <code>false</code>, то переадресация временная, и посылается статусный код 302.</p>
<p>Допустим, у нас было следующее приложение</p>
<pre class="brush:c#;">
...
<p>При обращении по адресу "/old" приложение посылает сообщение "Old Page".</p>
<img src="./pics/2.30.png" alt="Редирект в ASP.NET Core и C#" />
<p>Но затем мы решили сделать переадресацию с адреса "/old" на "/new". Используем для этого первую версию метода Redirect:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Теперь при обращении по адресу "/old" произойдет перенаправление на адрес "/new".</p>
<p>В данном случае применяется редирект на локальный адрес в рамках приложение. Но также можно использовать редирект на внешние ресурсы:</p>
<pre class="brush:c#;">
if (context.Request.Path == "/old")
{
