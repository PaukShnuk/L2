...
<html >
<head>
<title>ASP.NET Core и C# | IWebHostEnvironment и окружение</title>
<meta charset="utf-8" />
<meta name="description" content="Интерфейс IWebHostEnvironment, его свойства и взаимодействие с окружением в ASP.NET Core и C#, проверка стадии, в которой находится приложение, и метод IsDevelopment">
...
<h2>IWebHostEnvironment и окружение</h2><div class="date">Последнее обновление: 20.12.2021</div>
...
<p>Для взаимодействия с окружением, в котором запущено приложение, фреймфорк ASP.NET Core предоставляет интерфейс <span class="b">IWebHostEnvironment</span>. 
Этот интерфейс предлагает ряд свойств, с помощью которых мы можем получить информацию об окружении:</p>
<ul>
<li><p><span class="b">ApplicationName</span>: хранит имя приложения</p></li>
<li><p><span class="b">EnvironmentName</span>: хранит название среды, в которой хостируется приложение</p></li>
<li><p><span class="b">ContentRootPath</span>: хранит путь к корневой папке приложения</p></li>
<li><p><span class="b">WebRootPath</span>: хранит путь к папке, в которой хранится статический контент приложения. По умолчанию это папка wwwroot</p></li>
<li><p><span class="b">ContentRootFileProvider</span>: возвращает реализацию интерфейса <code>Microsoft.AspNetCore.FileProviders.IFileProvider</code>, 
которая может использоваться для чтения файлов из папки ContentRootPath</p></li>
<li><p><span class="b">WebRootFileProvider</span>: возвращает реализацию интерфейса <code>Microsoft.AspNetCore.FileProviders.IFileProvider</code>, 
которая может использоваться для чтения файлов из папки WebRootPath</p></li>
</ul>
<p>При разработке мы можем использовать эти свойства. Но наиболее часто при разработке придется сталкиваться со свойством <span class="b">EnvironmentName</span>. 
...
переменной среды <code>ASPNETCORE_ENVIRONMENT</code>. Текущее значение данного параметра задается в файле <span class="b">launchSettings.json</span>, который располагается в проекте в папке <span class="b">Properties</span>.</p>
<img src="./pics/2.54.png" alt="файл launchSettings.json и переменная ASPNETCORE_ENVIRONMENT в ASP.NET Core и C#" />
<p>Откроем данный файл:</p>
<pre class="brush:js;">
{
  "iisSettings": {
...
<p>Здесь можно увидеть, что переменная "ASPNETCORE_ENVIRONMENT" встречается два раза - для запуска через IISExpress и для запуска через Kestrel. 
В обоих случаях она имеет значение Development. Но мы можем поменять значение этой переменной.</p>
<p>Для определения значения этой переменной для интерфейса IWebHostEnvironment определены специальные методы расширения:</p>
<ul>
<li><p><span class="b">IsEnvironment(string envName)</span>: возвращает <code>true</code>, если имя среды равно значению параметра envName</p></li>
<li><p><span class="b">IsDevelopment()</span>: возвращает <code>true</code>, если имя среды - Development</p></li>
<li><p><span class="b">IsStaging()</span>:  возвращает <code>true</code>, если имя среды - Staging</p></li>
<li><p><span class="b">IsProduction()</span>:  возвращает <code>true</code>, если имя среды - Production</p></li>
</ul>
<p>Данная функциональность позволяет нам выполнять определенный код в зависимости от того, на какой стадии находится приложение. Если приложение в процессе 
разработки, то мы можем выполнять один код, а при разветывании для полноценного использования другой код:</p>
<pre class="brush:c#;">
...
<p>Так, если мы посмотрим на код класса <a href="https://github.com/dotnet/aspnetcore/blob/main/src/DefaultBuilder/src/WebApplicationBuilder.cs" rel="nofollow">WebApplicationBuilder</a>, который применяется для создания приложения, 
то мы там можем увидеть там следующие строки:</p>
<pre class="brush:c#;">
...
<p>Здесь если имя среды имеет значение "Development" (то есть приложение находится в состоянии разработки), то при ошибке разработчик увидит 
...
Таким образом, в зависимости от стадии, на которой находится проект, мы можем скрывать или задействовать часть функционала приложения.</p>
<p>Если мы хотим поменять значение среды, необязательно изменять файл <span class="b">launchSettings.json</span>. Это можно сделать также программно:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
WebApplication app = builder.Build();
...
<h3>Определение своих состояний среды</h3>
<p>Хотя по умолчанию среда может принимать три состояния: Development, Staging, Production, но мы можем при желании вводить новые значения. Например, нам надо отслеживать какие-то дополнительные состояния. Это можно сделать через изменение файла 
<span class="b">launchSettings.json</span> либо программно.</p>
<p>Например, изменим название среды на "Test" (значение может быть произвольное):</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
WebApplication app = builder.Build();
...
<img src="./pics/2.55.png" alt="Окружение и переменные среды в ASP.NET Core и C#" />
<p>Также можно изменить значение "ASPNETCORE_ENVIRONMENT" на "Test" или любое другой в файле <span class="b">launchSettings.json</span> для используемого профиля:</p>
<pre class="brush:js;">
{
  "iisSettings": {
    "windowsAuthentication": false,
...
<div class="nav"><p><a href="./2.16.php">Назад</a><a href="./">Содержание</a><a href="./4.1.php">Вперед</a></p></div></div>
<div class="socBlock">
