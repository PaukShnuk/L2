...
<html >
<head>
<title>ASP.NET Core и C# | Создание и запуск приложения. WebApplication и WebApplicationBuilder</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и запуск приложения в ASP.NET Core, классы WebApplication и WebApplicationBuilder, их свойства и роль в приложении">
...
<h1>Основы в ASP.NET Core</h1><h2>Создание и запуск приложения. WebApplication и WebApplicationBuilder</h2><div class="date">Последнее обновление: 06.12.2021</div>
...
<p>В центре приложения ASP.NET находится класс <span class="b">WebApplication</span>. Например, если мы возьмем проект ASP.NET по типу <span class="b">ASP.NET Core Empty</span>, то в файле <span class="b">Program.cs</span> мы встретим следующий код:</p>
<pre class="brush:c#;">
...
<p>Переменная <code>app</code> в данном коде как раз представляет объект <span class="b">WebApplication</span>. 
Однако для создания этого объекта необходим другой объект - <span class="b">WebApplicationBuilder</span>, который в данном коде представлен переменной <code>builder</code>.</p>
<h3>Класс WebApplicationBuilder</h3>
<p>Создание приложения по умолчанию фактически начинается с класса <span class="b">WebApplicationBuilder</span>. Исходный код доступен по адресу 
<a href="https://github.com/dotnet/aspnetcore/blob/main/src/DefaultBuilder/src/WebApplicationBuilder.cs" rel="nofollow">WebApplicationBuilder.cs</a></p>
<p>Для его создания объекта этого класса вызывается статический метод 
<span class="b">WebApplication.CreateBuilder()</span>:</p>
<pre class="brush:c#;">WebApplicationBuilder builder = WebApplication.CreateBuilder();</pre>
<p>Для инициализации объекта WebApplicationBuilder в этот метод могут передаваться аргументы командной строки, указанные при запуске приложения (доступны через неявно определенный параметр <code>args</code>):</p>
<pre class="brush:c#;">WebApplicationBuilder builder = WebApplication.CreateBuilder(args);</pre>
<p>Либо можно передавать объект <span class="b">WebApplicationOption</span>:</p>
<pre class="brush:c#;">
WebApplicationOptions options = new() { Args = args };
WebApplicationBuilder builder = WebApplication.CreateBuilder(options);
</pre>
<p>Кроме создания объекта WebApplication класс <span class="b">WebApplicationBuilder</span> выполняет еще ряд задач, среди которых можно выделить следующие:</p>
<ul>
<li><p>Установка конфигурации приложения</p></li>
<li><p>Добавление сервисов</p></li>
<li><p>Настройка логгирования в приложении</p></li>
<li><p>Установка окружения приложения</p></li>
<li><p>Конфигурация объектов IHostBuilder и IWebHostBuilder, которые применяются для создания хоста приложения</p></li>
</ul>
...
<p>Для реализации этих задач в классе WebApplicationBuilder определены следующие свойства:</p>
<ul>
<li><p><span class="b">Configuration</span>: представляет объект ConfigurationManager, который применяется для добавления конфигурации к приложению.</p></li>
<li><p><span class="b">Environment</span>: предоставляет информацию об окружении, в котором запущено приложение.</p></li>
<li><p><span class="b">Host</span>: объект <span class="b">IHostBuilder</span>, который применяется для настройки хоста.</p></li>
<li><p><span class="b">Logging</span>: позволяет определить настройки логгирования в приложении.</p></li>
<li><p><span class="b">Services</span>: представляет коллекцию сервисов и позволяет добавлять сервисы в приложение.</p></li>
<li><p><span class="b">WebHost</span>: объект <span class="b">IWebHostBuilder</span>, который позволяет настроить отдельные настройки сервера.</p></li>
</ul>
<h3>Класс WebApplication</h3>
<p>Метод <span class="b">build()</span> класса WebApplicationBuilder создает объект <span class="b">WebApplication</span>:</p>
<pre class="brush:c#;">
WebApplicationBuilder builder = WebApplication.CreateBuilder();
WebApplication app = builder.Build();
</pre>
<p>Класс WebApplication применяется для управления обработкой запроса, установки маршрутов, получения сервисов и т.д. 
Исходный код класса можно найти на Github по адресу <a href="https://github.com/dotnet/aspnetcore/blob/main/src/DefaultBuilder/src/WebApplication.cs" rel="nofollow">WebApplication.cs</a>.</p>
<p>Класс WebApplication применяет три интерфейса:</p>
<ul>
<li><p><span class="b">IHost</span>: применяется для запуска и остановки хоста, который прослушивает входящие запросы</p></li>
<li><p><span class="b">IApplicationBuilder</span>: применяется для установки компонентов, которые участвуют в обработке запроса</p></li>
<li><p><span class="b">IEndpointRouteBuilder</span>: применяется для установки маршрутов, которые сопоставляются с запросами</p></li>
</ul>
<p>Для получения доступа к функциональности приложения можно использовать свойства класса WebApplication:</p>
<ul>
<li><p><span class="b">Configuration</span>: представляет конфигурацию приложения в виде объекта <span class="b">IConfiguration</span></p></li>
<li><p><span class="b">Environment</span>: представляет окружение приложения в виде <span class="b">IWebHostEnvironment</span></p></li>
<li><p><span class="b">Lifetime</span>: позволяет получать уведомления о событиях жизненного цикла приложения</p></li>
<li><p><span class="b">Logger</span>: представляет логгер приложения по умолчанию</p></li>
<li><p><span class="b">Services</span>: представляет сервисы приложения</p></li>
<li><p><span class="b">Urls</span>: представляет набор адресов, которые использует сервер</p></li>
</ul>
<p>Для управления хостом класс WebApplication определяет следующие методы:</p>
<ul>
<li><p><span class="b">Run()</span>: запускает приложение</p></li> 	
<li><p><span class="b">RunAsync()</span>: асинхронно запускает приложение</p></li>
<li><p><span class="b">Start()</span>: запускает приложение</p></li>
<li><p><span class="b">StartAsync()</span>: запускает приложение</p></li> 	
<li><p><span class="b">StopAsync()</span>: останавливает приложение</p></li>
</ul>
<p>Таким образом, после вызова метод Run/Start/RunAsync/StartAsync приложение будет заущено, и мы сможем к нему обращаться:</p>
<pre class="brush:c#;">
WebApplicationBuilder builder = WebApplication.CreateBuilder();
WebApplication app = builder.Build();
app.Run();
</pre>
<p>При необходимости с помощью метода <span class="b">StopAsync()</span> можно программным сособом завершить выполнение приложения:</p>
<pre class="brush:c#;">
WebApplicationBuilder builder = WebApplication.CreateBuilder();
