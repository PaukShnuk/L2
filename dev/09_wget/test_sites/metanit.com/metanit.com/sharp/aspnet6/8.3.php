...
<html >
<head>
<title>ASP.NET Core и C# | Сессии</title>
<meta charset="utf-8" />
<meta name="description" content="Сессии в ASP.NET Core и C#, создание и уничтожение сессий, сохранение и получение данных сессии">
...
<h2>Сессии</h2><div class="date">Последнее обновление: 23.12.2021</div>
...
<p>Сессия представляет собой ряд последовательных запросов, совершенных в одном браузере в течение некоторого времени. 
Сессия может использоваться для сохранения каких-то временных данных, которые должны быть доступны, пока пользователь работает с приложением, и не требуют постоянного хранения.</p>
<p>Для хранения состояния сессии на сервере создается словарь или хеш-таблица, которая хранится в кэше и которая существует для всех запросов из одного браузера в течение некоторого времени. 
...
извлечения нужных данных из сессии. Эти куки удаляются только при завершении сессии. Но если сервер получает куки, которые установлены уже для истекшей сессии, то для этих кук создается новая сессия.</p>
<p>Сервер хранит данные сессии в течение ограниченного промежутка времени после последнего запроса. По умолчанию этот промежуток равен 20 минутам, хотя его также можно изменить. 
</p>
<p>Следует учитывать, что данные сессии специфичны для одного браузера и не разделяются между браузерами. То есть для каждого браузера на одном компьютере 
будет создаваться свой набор данных.</p>
<p>Все сессии работают поверх объекта <span class="b">IDistributedCache</span>, 
и ASP.NET Core предоставляет встроенную реализацию IDistributedCache, которую мы можем использовать. Для этого определим в приложении следующий код:</p>
<pre class="brush:c#;">
...
<p>Сначала добавляем необходимые сервисы:</p>
<pre class="brush:c#;">
builder.Services.AddDistributedMemoryCache();// добавляем IDistributedMemoryCache
builder.Services.AddSession();  // добавляем сервисы сессии
</pre>
<p>Затем с помощью метода <span class="b">UseSession()</span> встраиваем в конвейер обработки запроса middleware для работы с сессиями:</p>
<pre class="brush:c#;">
app.UseSession();   // добавляем middleware для работы с сессиями
...
<p>Если мы вдруг не используем <code>app.UseSession()</code> или попробуем обратиться к сессии до применения этого метода, то получим исключение 
<code>InvalidOperationException</code>.</p>
<p>После вызова <code>app.UseSession()</code> мы сможем управлять сессиями через свойство <span class="b">HttpContext.Session</span>, которое представляет 
объект интерфейса <span class="b">ISession</span>. В данном случае мы проверяем, определен ли в сессии 
ключ <code>"name"</code>. Если ключ определен, то передаем ответ значение по этому ключу. Если ключ не определен, устанавливаем его.</p>
<img src="./pics/8.4.png" alt="Сессии в ASP.NET Core и C#" />
<p>После первого запроса мы сможем через инстументы браузера для разработчиков найти куку <span class="b">.AspNetCore.Session</span>, которая будет представлять идентификатор сессии:</p>
<img src="./pics/8.5.png" alt="Управление сессиями в ASP.NET Core и C#" />
<h3>Управление сессией</h3>
<p>Объект <span class="b">ISession</span> определяет ряд свойств и методов, которые мы можем использовать:</p>
<ul>
<li><p><span class="b">Keys</span>: свойство, представляющее список строк, который хранит все доступные ключи</p></li>
<li><p><span class="b">Clear()</span>: очищает сессию</p></li>
<li><p><span class="b">Get(string key)</span>: получает по ключу key значение, которое представляет массив байтов</p></li>
<li><p><span class="b">GetInt32(string key)</span>: получает по ключу key значение, которое представляет целочисленное значение</p></li>
<li><p><span class="b">GetString(string key)</span>: получает по ключу key значение, которое представляет строку</p></li>
<li><p><span class="b">Set(string key, byte[] value)</span>: устанавливает по ключу key значение, которое представляет массив байтов</p></li>
<li><p><span class="b">SetInt32(string key, int value)</span>: устанавливает по ключу key значение, которое представляет целочисленное значение value</p></li>
<li><p><span class="b">SetString(string key, string value)</span>: устанавливает по ключу key значение, которое представляет строку value</p></li>
<li><p><span class="b">Remove(string key)</span>: удаляет значение по ключу</p></li>
</ul>
<h3>Настройка сессии</h3>
<p>Для разграничения сессий для них устанавливается идентификатор. Каждая сессия имеет свой идентификатор, который сохраняется в куках. 
По умолчанию эти куки имеют название ".AspNet.Session". И также по умолчанию куки имеют настройку 
<code>CookieHttpOnly=true</code>, поэтому они не доступны для клиентских скриптов из браузера. Но мы можем переопределить ряд настроек сессии с помощью 
свойств объекта <span class="b">SessionOptions</span>:</p>
<ul>
<li><p><span class="b">Cookie.Name</span>: имя куки</p></li>
<li><p><span class="b">Cookie.Domain</span>: домен, для которого устаналиваются куки</p></li>
<li><p><span class="b">Cookie.HttpOnly</span>: доступны ли куки только при передаче через HTTP-запрос</p></li>
<li><p><span class="b">Cookie.Path</span>: путь, который используется куками</p></li>
<li><p><span class="b">Cookie.Expiration</span>: время действия куки в виде объекта System.TimeSpan</p></li>
<li><p><span class="b">Cookie.IsEssential</span>: при значении true указывает, что куки критичны и необходимы для работы этого приложения</p></li>
<li><p><span class="b">IdleTimeout</span>: время действия сессии в виде объекта System.TimeSpan прм неактивности пользователя. При каждом новом запросе 
таймаут сбрасывается. Этот параметр не зависит от Cookie.Expiration.</p></li>
</ul>
<p>Для использования этих свойств изменим установку сервисов:</p>
<pre class="brush:c#;">
...
<h3>Хранение сложных объектов</h3>
<p>В случае выше в сессиях хранились простые строки. Если же надо сохранить какой-то сложный объект, то его надо сериализовать в строку, а при получении из сессии - обратно 
десериализовать. Как правило, для этого определяются методы расширения для объекта ISession. В частности, добавим следующий класс:</p>
<pre class="brush:c#;">
...
<p>Метод Set сохраняет в сессию данные, а метод Get извлекает их из сессии.</p>
<p>Допустим, у нас есть класс Person:</p>
<pre class="brush:c#;">
class Person
{
...
<p>В приложении выполним сериализацию и десериализовацию объекта в сессию:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
...
<img src="./pics/8.6.png" alt="Сериализация и десериализация сложных объектов в сессии в ASP.NET Core и C#" />
...
<div class="nav"><p><a href="./8.2.php">Назад</a><a href="./">Содержание</a><a href="./9.1.php">Вперед</a></p></div></div>
<div class="socBlock">
