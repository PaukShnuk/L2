...
<html >
<head>
<title>ASP.NET Core и C# | Создание сервисов</title>
<meta charset="utf-8" />
<meta name="description" content="определение своих сервисов и встраивание их в приложение в коллекцию IServiceCollection в ASP.NET Core и C#">
...
<h2>Создание сервисов</h2><div class="date">Последнее обновление: 18.12.2021</div>
...
<p>Фреймворк ASP.NET Core предоставляет ряд встроенных сервисов, которые мы можем использовать. Но также мы можем создавать свои собственные сервисы. Рассмотрим, как это сделать.</p>
<p>Определим новый интерфейс <span class="b">ITimeService</span>, который предназначен для получения времени:</p>
<pre class="brush:c#;">
...
<p>И также определим два класса, которые будут реализовать данный интерфейс. Первый класс будет называться <span class="b">ShortTimeService</span> 
и будет возвращать текущее время в формате <code>hh:mm</code>(то есть часы и минуты):</p>
<pre class="brush:c#;">
...
<p>Второй класс будет называться <span class="b">LongTimeService</span> - он будет возвращать время в формате <code>hh:mm:ss</code>:</p>
<pre class="brush:c#;">
...
<p>Теперь добавим в коллекцию сервисов сервис ITimeService и используем его в приложении:</p>
<pre class="brush:c#;">
...
<p>Здесь надо выделить два момента. Во-первых, добавление сервиса в коллекцию сервисов приложения:</p>
<pre class="brush:c#;">builder.Services.AddTransient&lt;ITimeService, ShortTimeService&gt;();</pre>
<p>Благодаря вызову <code>AddTransient&lt;ITimeService, ShortTimeService&gt;()</code> система на место объектов интерфейса ITimeService 
будет передавать экземпляры класса ShortTimeService.</p>
<p>Кроме того, сервисы добавляются до создания объекта WebApplication методом <code>Build()</code> объекта WebApplicationBuilder:</p>
<pre class="brush:c#;">
// добавление сервисов
builder.Services.AddTransient&lt;ITimeService, ShortTimeService&gt;();
...
<p>После добавления сервиса его можно получить и использовать в любой части приложения. Для получения сервиса могут применяться различные способы в зависимости от ситуации. 
В данном случае используется свойство <span class="b">app.Services.</span>, которое предоставляет провайдер сервисов - объект <span class="b">IServiceProvider</span>. Для получения сервиса 
у провайдера сервиса вызывается метод <span class="b">GetService()</span>, который типизируется типом сервиса:</p>
<pre class="brush:c#;">var timeService = app.Services.GetService&lt;ITimeService&gt;();</pre>
<p>После получения сервиса мы можем использовать его.</p>
<pre class="brush:c#;">await context.Response.WriteAsync($"Time: {timeService?.GetTime()}");</pre>
<img src="./pics/4.3.png" alt="Тестирование внедрения зависимостей в ASP.NET Core и C#" />
<p>Поскольку метод AddTransient установил зависимость между ITimeService и ShortTimeService, то в браузере выводится текущее время в формате "hh:mm". 
Мы можем поменять тип, сопоставляемый с ITimeService:</p>
<pre class="brush:c#;">
builder.Services.AddTransient&lt;ITimeService, LongTimeService&gt;();
</pre>
<p>И в этом случае мы увидим другое сообщение.</p>
<img src="./pics/4.4.png" alt="механизм Dependency Injection в ASP.NET Core и C#" />
<h3>Сервис как конкретный класс</h3>
<p>При этом необязательно разделять определение сервиса в виде интерфейса и его реализацию. Сам термин "сервис" в данном случае может представлять любой объект, функциональность которого может 
использоваться в приложении.</p>
<p>Например, определим новый класс TimeService:</p>
<pre class="brush:c#;">
...
<p>Данный класс определяет один метод <code>GetTime()</code>, который возвращает текущее время.</p>
<p>Используем этот класс в качестве сервиса:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
...
<p>Для добавления сервиса в эту коллекцию применяется метод <code>AddTransient()</code>:</p>
<pre class="brush:c#;">builder.Services.AddTransient&lt;TimeService&gt;();</pre>
<p>После добавления сервиса мы его можем получить и использовать в любой части приложения.</p>
<h3>Расширения для добавления сервисов</h3>
<p>Нередко для сервисов создают собственные методы добавления в виде методов расширения для интерфейса IServiceCollection. Например, создадим подобный метод 
для сервиса TimeService:</p>
<pre class="brush:c#;">
...
<p>И теперь используем этот метод:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
