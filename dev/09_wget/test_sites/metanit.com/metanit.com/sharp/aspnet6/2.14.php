...
<html >
<head>
<title>ASP.NET Core и C# | Методы Map и MapWhen</title>
<meta charset="utf-8" />
<meta name="description" content="Методы Map и MapWhen и ответвление конвейера обработки запроса в ASP.NET Core и C#, сопоставление запроса с определенным делегатом, создание простейшей системы маршрутизации">
...
<h2>Метод Map</h2><div class="date">Последнее обновление: 14.12.2021</div>
...
<p>Метод <span class="b">Map()</span> применяется для создания ветки конвейера, которая будет обрабатывать запрос по определенному пути. Этот метод реализован как метод расширения 
для типа IApplicationBuilder и имеет ряд перегруженных версий. Например:</p>
<pre class="brush:c#;">
...
<p>В качестве параметра <code>pathMatch</code> метод принимает путь запроса, с которым будет сопоставляться ветка. А параметр <code>configuration</code> 
представляет делегат, в который передается объект IApplicationBuilder и в котором будет создаваться ветка конвейера.</p>
...
<p>В данном случае метод <code>app.Map()</code> создает ответвление конвейера, которое будет обрабатывать запросы по пути "/time":</p>
<pre class="brush:c#;">
appBuilder =&gt;
{
    var time = DateTime.Now.ToShortTimeString();
...
<p>Созданная ветка конвейера содержит два middleware, встраиваемые с помощью методов <code>Use()</code> и <code>Run()</code>. Вначале получаем текущее время и в 
первом middleware логгируем это время на консоль. Во втором - терминальном компоненте middleware отправляем информацию о времени в ответ клиенту.</p>
<img src="./pics/2.44.png" alt="Ответвление конвейера обработки запроса и app.Map в ASP.NET Core и C#" />
<p>При других путях запросах, отличных от "/time", запрос будет обрабатываться основным потоком конвейера, который состоит в данном случае из одного компонента:</p>
<pre class="brush:c#;">
app.Run(async (context) =&gt; await context.Response.WriteAsync("Hello METANIT.COM"));
</pre>
<p>Подобным образом можно создавать ветки для разных путей:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/2.43.png" alt="Сопоставление пути запроса с веткой конвейера обработки запрсоа в методе app.Map в ASP.NET Core и C#" />
<p>При необходимости создание веток конвейера можно вынести в отдельные методы:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<h3>Вложенные методы Map</h3>
<p>Ветка конвейера, которая создается в методе <span class="b">Map()</span>, может иметь вложенные ветки, которые обрабатывают подзапросы. Например:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Здесь ветка создается с помощью вызова</p>
<pre class="brush:c#;">
app.Map("/home", appBuilder =&gt;
</pre>
<p>Эта ветка будет обрабатывать запросы по пути "/home".</p>
<p>Внутри этой ветки создаются две вложенные ветки, которые будут обрабатывать запросы по путям относительно пути основной ветки. 
То есть теперь метод About будет обрабатывать запрос по пути "/home/about", а не "/about".</p>
<img src="./pics/2.45.png" alt="Вложенные ветки конвейера обработки запрсоа в методе app.Map в ASP.NET Core и C#" />
