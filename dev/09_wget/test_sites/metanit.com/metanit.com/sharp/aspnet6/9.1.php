...
<html >
<head>
<title>ASP.NET Core и C# | Обработка исключений</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка ошибок и исключений в ASP.NET Core и C#, методы UseDeveloperExceptionPage и UseExceptionHandler, установка обработчика исключений">
...
<h1>Обработка ошибок</h1><h2>Обработка исключений</h2><div class="date">Последнее обновление: 23.12.2021</div>
...
<p>Для обработки исключений в приложении, которое находится в процессе разработки, предназначен специальный middleware - 
<span class="b">DeveloperExceptionPageMiddleware</span>. Однако вручную нам его не надо добавлять, поскольку оно добавляется автоматически. Так, если мы посмотрим на код класса <a href="https://github.com/dotnet/aspnetcore/blob/main/src/DefaultBuilder/src/WebApplicationBuilder.cs" rel="nofollow">WebApplicationBuilder</a>, 
который применяется для создания приложения, то мы там можем увидеть там следующие строки:</p>
<pre class="brush:c#;">
...
<p>Например, изменим код приложения следующим образом:</p>
<pre class="brush:c#;">
...
<p>В middleware в методе <code>app.Run()</code> симулируется генерация исключения путем деления на ноль. 
И если мы запустим проект, то в браузере мы увидим информацию об исключении:</p>
<img src="./pics/9.1.png" alt="Обработка исключений и UseDeveloperExceptionPage в ASP.NET Core и C#" />
<p>Этой информации достаточно, чтобы определить где именно в коде произошло исключение.</p>
<p>Теперь посмотрим, как все это будет выглядеть для простого пользователя. Для этого изменим код приложения:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Выражение <code>app.Environment.EnvironmentName = "Production"</code> меняет имя окружения с "Development" (которое установлено по умолчанию) на 
"Production". В этом случае среда ASP.NET Core больше не будет расценивать приложение как находящееся в стадии разработки. 
Соответственно middleware из метода <code>app.UseDeveloperExceptionPage()</code> не будет перехватывать исключение. А приложение будет возвращать 
пользователю ошибку 500, которая указывает, что на сервере возникла ошибка при обработке запроса. Однако реально природа подобной ошибки может заключать 
в чем угодно.</p>
<img src="./pics/9.2.png" alt="HTTP ERROR 500 в ASP.NET Core и C#" />
...
Для этих целей можно использовать еще один встроенный middleware - <span class="b">ExceptionHandlerMiddleware </span>, который подключается с помощью метода <span class="b">UseExceptionHandler()</span>. Он перенаправляет 
при возникновении исключения на некоторый адрес и позволяет обработать исключение. Например, изменим код приложения следующим образом:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
из метода <code>app.Map()</code>, в котором пользователю будет отправляться некотоое сообщение со статусным кодом 500.</p>
<img src="./pics/9.3.png" alt="Error Handling и метод UseExceptionHandler в ASP.NET Core и C#" />
<p>Однако данный способ имеет небольшой недостаток - мы можем напрямую обратиться по адресу "/error" и получить тот же ответ от сервера. Но 
в этом случае мы можем использовать другую версию метода <span class="b">UseExceptionHandler()</span>, которая принимает делегат, параметр которого представляет объект IApplicationBuilder:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Следует учитывать, что <code>app.UseExceptionHandler()</code> следует помещать ближе к началу конвейера middleware.</p>
...
<div class="nav"><p><a href="./8.3.php">Назад</a><a href="./">Содержание</a><a href="./9.2.php">Вперед</a></p></div></div>
<div class="socBlock">
