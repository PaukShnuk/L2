...
<html >
<head>
<title>ASP.NET Core и C# | Отправка форм</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка форм в middleware в ASP.NET Core и C# и их получаение с помощью свойства Request.Form, чтение данных формы">
...
<h2>Отправка форм</h2><div class="date">Последнее обновление: 12.12.2021</div>
...
<p>Нередко данные отправляются на сервер с помощью форм html, обычно в запросе типа POST. Для получения подобных данных в классе <span class="b">HttpRequest</span> 
определено свойство <span class="b">Form</span>. Рассмотрим, как мы можем получить подобные данные.</p>
<p>Прежде всего определим в проекте в папке <span class="b">html</span> файл <span class="b">index.html</span></p>
<img src="./pics/2.26.png" alt="Отправка форм в ASP.NET Core и C#" />
...
<p>Здесь определена форма условно для ввода данных пользователя, которая в запросе типа POST (атрибут <code>method="post"</code>) отправляет данные по адресу "postuser" 
(атрибут <code>action="postuser"</code>)</p>
<p>На форме определены два поля ввода. Первое поле предназначено для ввода имени пользователя. Второе поле - для ввода возроста пользователя.</p>
<p>Для получения этих данных определим в файле <span class="b">Program.cs</span> следующий код:</p>
<pre class="brush:c#;">
...
<p>Здесь, если запрошен адрес "/postuser", то предполагается, что отправлена некоторая форма. Сначала получаем отправленную форму в переменную form:</p>
<pre class="brush:c#;">var form = context.Request.Form;</pre>
<p>Свойство <code>Request.Form</code> возвращает объект <span class="b">IFormCollection</span> - своего рода словарь, где по ключу можно получить значение элемента. 
При этом в качестве ключей выступает названия полей форм (значения атрибутов <code>name</code> элементов формы):</p>
<pre class="brush:xml;">&lt;input name="age" type="number" /&gt;</pre>
<p>Так, в данном случае название поля (значение атрибута <code>name</code>) равно "age". Соответственно в Request.Form по этому имени мы можем получить его значение:</p>
<pre class="brush:c#;">string age = form["age"];</pre>
<p>После получения данных формы они отправляются обратно клиенту:</p>
<img src="./pics/2.27.png" alt="Request.Form и получение форм в ASP.NET Core и C#" />
<h3>Получение массивов</h3>
<p>Усложним задачу и добавим в форму на странице <span class="b">index.html</span> несколько полей, которые будут представлять массив:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь добавлено три поля ввода, которые имеют одно и то же имя. Поэтому при их отправке будет формироваться массив из трех значений. Теперь получим эти 
значения в коде C#:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Поскольку параметр "languages" представляет массив, то и сопоствляться он будет с массивом строк:</p>
<pre class="brush:c#;">string[] languages = form["languages"];</pre>
<p>Для вывода на веб-страницу из этого массива формируется код html в виде строки:</p>
<img src="./pics/2.28.png" alt="Отправка массива элементов формы в ASP.NET Core и C#" />
<p>Подобным образом можно передавать значения массива полей других типов, либо полей, которые представляют набор элементов, например, элемента 
<span class="b">select</span>, который поддерживает множественный выбор:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/2.29.png" alt="Отправка массива значений из формы html в ASP.NET Core и C#" />
...
<div class="nav"><p><a href="./2.6.php">Назад</a><a href="./">Содержание</a><a href="./2.9.php">Вперед</a></p></div></div>
<div class="socBlock">
