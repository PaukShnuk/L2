...
<html >
<head>
<title>ASP.NET Core и C# | Создание ветки конвейера. UseWhen и MapWhen</title>
<meta charset="utf-8" />
<meta name="description" content="Создание ветки конвейера обработки запроса с помощью методов UseWhen и MapWhen в ASP.NET Core и C#">
...
<h2>Создание ветки конвейера. UseWhen и MapWhen</h2><div class="date">Последнее обновление: 10.04.2022</div>
...
<h3>UseWhen</h3>
<p>Метод <span class="b">UseWhen()</span> на основании некоторого условия позволяет создать ответвление конвейера при обработке запроса:</p>
<pre class="brush:c#;">
...
<p>Как и <code>Use()</code>, метод <span class="b">UseWhen()</span> реализован как метод расширения для типа IApplicationBuilder.</p>
<p>В качестве параметра он принимает делегат <code>Func&gt;HttpContext,bool&gt;</code> - некоторое условие, которому должен соответствовать запрос. В этот 
делегат передается объект HttpContext. А возвращаемым типом должен быть тип <code>bool</code> - если запрос соответствует условию, то возвращается true, иначе возвращаеся false.</p>
<p>Последний параметр метода - делегат <code> Action&lt;IApplicationBuilder&gt;</code> представляет некоторые действия над объектом IApplicationBuilder, который передается в делегат в качестве параметра.</p>
...
<p>В данном случае метод <code>app.UseWhen()</code> в качестве первого параметра получает следующее условие:</p>
<pre class="brush:c#;">context =&gt; context.Request.Path == "/time"</pre>
<p>Второй параметр определяет действие, в котором создается ответвление конвейера:</p>
<pre class="brush:c#;">
appBuilder =&gt; 
{
    // логгируем данные - выводим на консоль приложения
...
<p>В данном действии в конвейер обработки запроса встраиваются два middleware - с помощью методов <code>Use()</code> и <code>Run()</code>. В первом middleware логгируем это время на консоль приложения. Во втором - терминальном компоненте middleware отправляем информацию о времени в ответ клиенту.</p>
<p>Если мы обращаемся к приложению по пути, который отличается от "/time", то условие в методе <code>UseWhen()</code> ложно, поэтому ответвления конвейера не выполняется. И 
выполняется middleware из метода <code>app.Run()</code>:</p>
<img src="./pics/2.40.png" alt="Ответвление конвейера обработки запроса в ASP.NET Core и C#" />
<p>Однако если мы обращаемся по пути "/time", то условие в методе <code>app.UseWhen()</code> будет истинно. Соответственно будет выполняться ответвление конвейера, который будет обрабатывать запрос. 
В итоге на консоль приложения, а также в браузере будет выводиться текущее время.</p>
<img src="./pics/2.41.png" alt="Ответвление конвейера обработки запроса в ASP.NET Core и C#" />
<p>Стоит отметить, что создание ветки происходит один раз при запуске приложения. Например, в примере выше мы видим, что получение времени производится в обоих middleware во встраиваемой 
ветки. Но что будет, если вынести получение времени во вне и не дублировать в каждом middleware, например, следующим образом:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>В этом случае время будет устанавливаться один раз - при запуске приложения и создании ветки в конвейер. Соответственно вне зависимости от того, сколько раз мы будем обращаться к приложению 
по пути "/time", мы будем получать одно и то же время.</p>
<p>В примере выше ветка конвейера завершалась терминальным компонентом, поэтому остальные действия в основной части конвейера не выполнялись. Однако мы можем также передать запрос на обработку 
из ветки в основной поток конвейера:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>В данном случае, если запрос идет по пути "/time", сначала срабатывает ветка конвейера с компонентом, который логгирует время на консоль. А затем выполняется 
компоненте из <code>app.Run()</code>, который отправляет сообщение "Hello METANIT.COM":</p>
<img src="./pics/2.42.png" alt="Ответвление конвейера обработки запроса с помощью UseWhen в ASP.NET Core и C#" />
<p>Для большей читабельности также можно было бы вынести действия по созданию ветки конвейера в отдельный метод:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<h3>MapWhen</h3>
<p>Метод <span class="b">MapWhen()</span>, как и метод <code>UseWhen()</code>, на основании некоторого условия позволяет создать ответвление 
конвейера:</p>
<pre class="brush:c#;">
...
<p>Метод <span class="b">MapWhen()</span> также реализован как метод расширения для типа IApplicationBuilder, принимает те же параметры, что и <code>UseWhen()</code>, 
и работает во многом аналогичным образом:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Здесь опять же, если запрошен путь "/time", то срабатывает ветка конвейера, созданная методом <code>app.MapWhen()</code>, в которой клиенту отправляется текущее время. 
Если путь запроса другой, то срабатывается основной поток конвейера, в котором отправляется сообщение "Hello METANIT.COM".</p>
...
<div class="nav"><p><a href="./2.7.php">Назад</a><a href="./">Содержание</a><a href="./2.14.php">Вперед</a></p></div></div>
<div class="socBlock">
