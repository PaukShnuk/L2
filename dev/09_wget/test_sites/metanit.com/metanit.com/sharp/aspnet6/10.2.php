...
<html >
<head>
<title>ASP.NET Core и C# | Отправка текста и json в Results API</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка текста и json в Results API в ASP.NET Core и C#, методы Results.Content, Results.Text и Results.Json, настройка сериализации в json и тип ответа">
...
<h2>Отправка текста и json в Results API</h2><div class="date">Последнее обновление: 24.12.2021</div>
...
<h3>Отправка текста и метод Content</h3>
<p>Метод <span class="b">Content()</span> отправляет текстовое содержимое и позволяет при этом задать тип содержимого и кодировку. Одна из версий метода:</p>
<pre class="brush:c#;">
public static IResult Content (string content, string? contentType = default, System.Text.Encoding? contentEncoding = default);
</pre>
<ul>
<li><p>Параметр <span class="b">content</span> задает текстовое содержимое ответа</p></li>
<li><p>Параметр <span class="b">contentType</span> задает MIME-тип ответа</p></li>
<li><p>Параметр <span class="b">contentEncoding</span> задает кодировку</p></li>
</ul>
<pre class="brush:c#;">
...
<p>Если указан только первый параметр, то метод по умолчанию будет использовать в качестве типа содержимого "text/plain", а в качестве кодировки "utf-8"</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/10.2.png" alt="Results.Content и отправка текста в Results API в ASP.NET Core и C#" />
<h3>Text</h3>
<p>Метод <span class="b">Text()</span> работает аналогичным образом, он также отравляет текст и принимает те же параметры:</p>
<pre class="brush:c#;">
...
<h3>Отравка json</h3>
<p>Для отправки данных в формате json применяется метод <span class="b">Results.Json()</span>:</p>
<pre class="brush:c#;">
...
<ul>
<li><p><span class="b">data</span>: отправляемый объект</p></li>
<li><p><span class="b">options</span>: объект <code>System.Text.Json.JsonSerializerOptions?</code>, который задает параметры сериализации</p></li>
<li><p><span class="b">contentType</span>: тип содержимого в виде строки. Если этот параметр не указан, то по умолчанию применяется тип "application/json; charset=utf-8"</p></li>
<li><p><span class="b">statusCode</span>: отправляемый вместе с json код статуса. Если этот параметр не указан, то по умолчанию код статус - 200</p></li>
</ul>
...
<img src="./pics/10.3.png" alt="Results.Json и отправка объектов json в Results API в ASP.NET Core и C#" />
<p>Если надо конкретизировать параметры сериализации в json, то можно использовать второй параметр метода, который представляет тип 
<span class="b">System.Text.Json.JsonSerializerOptions?</span>:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/10.4.png" alt="Results.Json и настройка сериализации в json в ASP.NET Core и C#" />
<p>Нередко формат json также применяется и для отправки ошибок. В этом случае мы можем задать статусный код ошибки с помощью последнего параметра:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>В данном случае посылаем объект с информацией об ошибке и статусный код 500:</p>
<img src="./pics/10.5.png" alt="Results.Json и отправка статусного кода в ASP.NET Core и C#" />
