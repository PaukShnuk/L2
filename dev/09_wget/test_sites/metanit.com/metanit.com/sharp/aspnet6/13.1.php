...
<html >
<head>
<title>ASP.NET Core и C# | Введение в аутентификацию и авторизацию</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в аутентификацию и авторизацию в ASP.NET Core и C#, атрибут Authorize, ограничение доступа к ресурсам, UseAuthentication, UseAuthorization, AddAuthentication, AddAuthorization">
...
<h1>Аутентификация и авторизация</h1><h2>Введение в аутентификацию и авторизацию</h2><div class="date">Последнее обновление: 09.01.2022</div>
...
<p>Важное место в приложении занимает аутентификация и авторизация. <span class="b">Аутентификация</span> 
представляет процесс определения пользователя. <span class="b">Авторизация</span> представляет процесс определения, имеет ли пользователь право доступа 
...
ASP.NET Core имеет встроенную поддержку аутентификации и авторизации.</p>
<h3>Аутентификация</h3>
<p>Для выполнения аутентификации в конвейере обработки запроса отвечает специальный компонент middleware - <span class="b">AuthenticationMiddleware</span>. Для встраивания 
этого middleware в конвейер применяется метод расширения <span class="b">UseAuthentication()</span></p>
<pre class="brush:c#;">
public static IApplicationBuilder UseAuthentication (this IApplicationBuilder app);
</pre>
<p>Следует отметить, что метод <span class="b">UseAuthentication()</span> должен встраиваться в конвейер до любых компонентов middleware, которые используют аутентификацию 
пользователей.</p>
<p>Для выполнения аутентификации этот компонент использует сервисы аутентификации, в частности, сервис <span class="b">IAuthenticationService</span>, 
которые регистрируются в приложении с помощью метода <span class="b">AddAuthentication()</span>:</p>
<pre class="brush:c#;">
public static AuthenticationBuilder AddAuthentication(this IServiceCollection services)
public static AuthenticationBuilder AddAuthentication(this IServiceCollection services, string defaultScheme)
public static AuthenticationBuilder AddAuthentication(this IServiceCollection services, Action&gt;AuthenticationOptions&gt; configureOptions)
</pre>
<p>В качестве параметра вторая версия метода <span class="b">AddAuthentication()</span> принимает схему аутентификации в виде строки. 
Третья версия метода AddAuthentication принимает делегат, который устанавливает опции аутентификации - объект AuthenticationOptions.</p>
<p>Какую бы мы версию метода не использовали, для аутентификации необходима установить схему аутентификации.  Две наиболее расcпространенные схемы аутентификации:</p>
<ul>
<li><p><span class="b">"Cookies"</span>: аутентификация на основе куки. Хранится в константе <code>CookieAuthenticationDefaults.AuthenticationScheme</code></p></li>
<li><p><span class="b">"Bearer"</span>: аутентификация на основе jwt-токенов. Хранится в константе <code>JwtBearerDefaults.AuthenticationScheme</code></p></li>
</ul>
<p><span class="b">Схема аутентификации</span> позволяет выбирать определенный 
обработчик аутентификации. <span class="b">Обработчик аутентификации</span> собственно и выполняет непосредственную аутентификацию пользователей 
на основе данных запросов и исходя из схемы аутентификации.</p>
<p>Например, для аутентификации с помощью куки передается схема "Cookies". Соответственно для аутентификации пользователя будет выбираться встроенный обработчик аутентификации - класс 
<span class="b">Microsoft.AspNetCore.Authentication.Cookies.CookieAuthenticationHandler</span>, который на основе полученных в запросе cookie выполняет 
аутентификацию.</p> 
<p>А если используется схема "Bearer", то это значит, что для аутентификации будет использоваться jwt-токен, а в качестве обработчика аутентификации будет 
...
необходимо добавить в проект через Nuget пакет <span class="b">Microsoft.AspNetCore.Authentication.JwtBearer</span></p>
<p>При чем в ASP.NET Core мы не ограничены встроенными схемами аутентификации и можем создавать свои кастомные схемы и под них своих обработчиков аутентификации.</p>
<p>Кроме применения схемы аутентификации необходимо подключить аутентификацию определенного типа. Для этого можно использовать следуюшие методы:</p>
<ul>
<li><p><span class="b">AddCookie()</span>: подключает и конфигурирует аутентификацию с помощью куки.</p></li>
<li><p><span class="b">AddJwtBearer()</span>: подключает и конфигурирует аутентификацию с помощью jwt-токенов 
(для этого метода необходим Nuget-пакет <span class="b">Microsoft.AspNetCore.Authentication.JwtBearer</span>)</p></li>
</ul>
<p>Оба метода реализованы как методы расширения для типа AuthenticationBuilder, который возвращается методом <code>AddAuthentication()</code>:</p>
<pre class="brush:c#;">
...
<h3>Авторизация</h3>
<p>Авторизация представляет процесс определения прав пользователя в системе, к каким ресурсам приложения он имеет право доступа и при каких условиях.</p>
<p>Хотя авторизация представляет отдельный независимый процесс, тем не менее для нее также необходимо, чтобы приложение также применяло аутентификацию.</p>
<p>Для подключения авторизации необходимо встроить компонент <span class="b">Microsoft.AspNetCore.Authorization.AuthorizationMiddleware</span>. 
Для этого применяется встроенный метод расширения <span class="b">UseAuthorization()</span></p>
<pre class="brush:c#;">public static IApplicationBuilder UseAuthorization(this IApplicationBuilder app)</pre>
<p>Кроме того, для применения авторизации необходимо зарегистрировать сервисы авторизации с помощью метода <span class="b">AddAuthorization()</span>:</p>
<pre class="brush:c#;">
public static IServiceCollection AddAuthorization(this IServiceCollection services)
public static IServiceCollection AddAuthorization(this IServiceCollection services, Action&lt;AuthorizationOptions&gt; configure)
</pre>
<p>Вторая версия метода принимает делегат, который с помощью параметра AuthorizationOptions позволяет сконфигурировать авторизацию.</p>
<p>Ключевым элементом механизма авторизации в ASP.NET Core является атрибут <span class="b">AuthorizeAttribute</span> из пространства имен 
Microsoft.AspNetCore.Authorization, который позволяет ограничить доступ к ресурсам приложения. Например:</p>
<pre class="brush:c#;">
...
<p>Здесь в приложении определены две конечных точки: "/" и "/hello". При этом конечная точка "/hello" применяет атрибут <span class="b">Authorize</span>. 
Атрибут указывается перед обработчиком конечной точки.</p>
<p>Применение данного атрибута означает, что к конечной точке "/hello" имеют доступ только аутентифицированные пользователи.</p>
<p>Если мы обратимся к конечной точке "/", то у нас не возникнет никаких проблем:</p>
<img src="./pics/13.7.png" alt="Аутентификация и авторизация в ASP.NET Core в C#" />
<p>Однако если мы обратимся к ресурсу "/hello", то мы получим ошибку 401, которая говорит о том, что пользователь не авторизован для доступа к этому ресурсу:</p>
<img src="./pics/13.8.png" alt="Атрибут Authorize в ASP.NET Core в C#" />
...
<div class="nav"><p><a href="./12.2.php">Назад</a><a href="./">Содержание</a><a href="./13.2.php">Вперед</a></p></div></div>
<div class="socBlock">
