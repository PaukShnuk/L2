...
<html >
<head>
<title>ASP.NET Core и C# | Параметры маршрута</title>
<meta charset="utf-8" />
<meta name="description" content="Параметры маршрута в ASP.NET Core и C#, получение значений параметров маршрутов в методе Map, необязательные параметры и значения параметров по умолчанию">
...
<h2>Параметры маршрута</h2><div class="date">Последнее обновление: 19.12.2021</div>
...
<p>Шаблон маршрута, который сопоставляется с конечной точкой, может иметь <span class="b">параметры</span>. Параметры имеют имя и определяются в шаблоне маршрута внутри фигурных скобок: <code>{название_параметра}</code></p>
<p>Например, определим конечную точку, которая принимает параметр:</p>
<pre class="brush:c#;">
...
<p>Здесь определено три конечных точки, которые используются шаблоны маршрутов <code>"/users/{id}"</code>, <code>"/users"</code> и <code>"/"</code>. Причем шаблон маршрута 
для первой конечной точки принимает параметр с именем <code>id</code>:</p>
<pre class="brush:c#;">"/users/{id}</pre>
<p>Параметр id будет представлять содержимое последнего сегмента. Например, в пути запроса "/users/134" последний сегмент "134" как раз будет представлять 
параметр id. Если же параметр id не передается, а путь запроса - "/users", то такой запрос обрабатывается второй конечной точкой.</p>
<p>Если маршрут принимает параметр, то в делегате-обработчике маршрута мы можем получить значение этого параметра. Для этого мы можем просто определить действие, 
которое принимает соответствующий параметр:</p>
<pre class="brush:c#;">(string id) =&gt; $"User Id: {id}"</pre>
<p>То есть здесь через параметр <code>id</code> мы получаем одноименный параметр маршрута и далее можем его использовать при обработке запроса. 
Стоит отметить, что название параметра маршрута и название параметра лямбда-выражения совпадают.</p>
<p>Также стоит отметить, что здесь для параметра id указан тип - тип <code>string</code> (то есть здесь мы ожидаем получить через параметр маршрута некоторую строку). Поскольку метод Map может принимать в качестве второго параметра 
делегат RequestDelegate, который тоже принимает один параметр - HttpContext, то, если не указать тип, то делегат-обработчик будет рассматриваться именно как RequestDelegate.</p> 
<img src="./pics/3.4.png" alt="Параметры маршрутов в ASP.NET Core и C#" />
<h3>Определение нескольких параметров</h3>
<p>Подобным образом можно определять и большее количество параметров маршрута:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>В данном случае маршрут первой конечной точки принимает два параметра - id и name:</p>
<img src="./pics/3.5.png" alt="определение параметров маршрутов в ASP.NET Core и C#" />
<p>Следует учитывать, что если параметры маршрута определяются подряд, то между ними должен располагаться разделитель. Например, следующий шаблон маршрута 
работать не будет:</p>
<pre class="brush:c#;">"/users/{id}{name}"</pre>
<p>так как между параметрами <code>id</code> и <code>name</code> нет разделителя.</p>
<p>Обычно в качестве разделителя выступает слеш, который оформляет отдельный сегмент в пути запроса. Но это необязательно. 
В качестве разделителя могут применяться и другие символы. Например, установим в качестве разделителя дефис:</p>
<pre class="brush:c#;">
app.Map(
    "/users/{id}-{name}", 
    (string id, string name) =&gt; $"User Id: {id}    User Name: {name}"
...
<img src="./pics/3.6.png" alt="Разделитель параметров маршрута в ASP.NET Core и C#" />
<p>Более того это может быть набор символов:</p>
<pre class="brush:c#;">
app.Map(
    "/users/{id}and{name}", 
    (string id, string name) =&gt; $"User Id: {id}    User Name: {name}"
...
<img src="./pics/3.7.png" alt="Разделитель параметров в шаблоне маршрута в ASP.NET Core и C#" />
<h3>Вынесение обработчика маршрута в отдельный метод</h3>
<p>Если обработчик маршрута выносится в отдельный метод, то он также может определять параметры, который сопоставляются с параметрами маршрута по имени:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Шаблон маршрута первой конечной точки состоит из двух сегментов, причем второй сегмент представляет параметр id, который помечен как необязательный. 
А это значит, что мы можем в запросе игнорировать значение для этого сегмента. Например, данный шаблон будет соответствовать двум следующим url:</p>
<pre class="browser;">
/users/
/users/23
</pre>
<img src="./pics/3.8.png" alt="Необязательные параметры маршрута в ASP.NET Core и C#" />
<p>Необязательные параметры следует помещать в конце шаблона маршрута, как в случае с параметром id в примере выше. То есть, к примеру, шаблон "/users/{id?}/{name}" не будет работать корректно, если мы для параметра id не передадим значение. 
А вот шаблон "/users/{name}/{id?}" будет работать нормально.</p>
...
<p>Здесь определена одна конечная точка, которая использует следующий шаблон маршрута:</p>
<pre class="browser">"{controller=Home}/{action=Index}/{id?}"</pre>
<p>То есть шаблон состоит из трех параметров. Параметр "controller" имеет значение по умолчанию "Home". Параметр "action" имеет значение по умолчанию "Index". Параметр 
...
<img src="./pics/3.9.png" alt="Значения параметров по умолчанию в ASP.NET Core и C#" />
<h3>Передача произвольного количества параметров в запросе</h3>
<p>Мы можем обозначить любое количество сегментов в запросе, чтобы не быть жестко привязанным к числу сегментов с помощью параметра со знаком * ("звездочка") или ** (две звездочки) (это так называемый catchall-параметр):</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Для первой конечной точки определяется шаблон маршрута, в котором параметр <code>info</code> будет представлять все, что идет после "/users". Это может быть в том числе пустая строка.</p>
<img src="./pics/3.10.png" alt="Произвольное количество параметров в маршруте в ASP.NET Core и C#" />
