...
<html >
<head>
<title>ASP.NET Core и C# | Применение сервисов в классах middleware</title>
<meta charset="utf-8" />
<meta name="description" content="Получение и применение сервисов в классах middleware в ASP.NET Core и C#, жизненный цикл зависимостей">
...
<h2>Применение сервисов в классах middleware</h2><div class="date">Последнее обновление: 18.12.2021</div>
...
<p>После добавления сервисов в коллекцию Services объекта WebApplicationBuilder они становятся доступны приложению, в том числе и в кастомных 
компонентах middleware. В middleware мы можем получить зависимости тремя способами:</p>
<ul>
...
<p>При этом надо учитывать, что компоненты middleware создаются при запуске приложения и живут в течение всего жизненного цикла приложения. То есть 
при последующих запросах инфраструктура ASP.NET Core использует ранее созданный компонент. И это налагает ограничения на использование зависимостей в middleware.</p>
...
<p>Добавим новый компонент <span class="b">TimerMiddleware</span>, который будет использовать этот сервис для вывода времени на веб-страницу:</p>
<pre class="brush:c#;">
...
<p>Если сервис TimeService добавляется в коллекцию сервисов приложения, то мы сможем получить его через конструктор класса TimerMiddleware.</p>
<p>Логика компонента предполагает, что, если запрос пришел по адресу "/time", то с помощью TimeService возвращается текущее время. Иначе мы просто обращаемся 
к следующему middleware в конвейере обработки запроса.</p>
<p>Используем этот компонент в файле Program.cs:</p>
<pre class="brush:c#;">
...
<p>В итоге, если мы обратимся по пути "/time", то приложение выведет текущее время:</p>
<img src="./pics/4.11.png" alt="Сервисы в классах middleware в ASP.NET Core и C#" />
