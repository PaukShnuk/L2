...
<html >
<head>
<title>ASP.NET Core и C# | Загрузка файлов на сервер</title>
<meta charset="utf-8" />
<meta name="description" content="Загрузка файлов на сервер в ASP.NET Core и C#, тип IFormFile, сохранение файла в файловой системе с помощью FileStream, свойство Request.Form.Files, параметр multipart/form-data">
...
<h2>Загрузка файлов на сервер</h2><div class="date">Последнее обновление: 13.12.2021</div>
...
<p>Для тестирования данной возможности определим в проекте папку <span class="b">html</span>, в которой создадим файл <span class="b">index.html</span></p>
<img src="./pics/2.39.png" alt="Как загружать файлы в ASP.NET Core и C#" />
...
<p>В данном случае форма содержит набор элементов с типом <span class="b">file</span>, через которые можно выбрать файлы для загрузки. В данном случае на форме три таких элемента, 
но их может быть и меньше и больше. А благодаря установке атрибута формы <span class="b">enctype="multipart/form-data"</span> браузер будет знать, что вместе с формой надо передать файлы.</p>
<p>Отправляться файлы будут в запросе типа POST на адрес "/upload".</p>
<p>Теперь в файле <span class="b">Program.cs</span> определим код, который будет получать загружаемые файлы:</p>
<pre class="brush:c#;">
...
<p>Здесь если запрос приходит по адресу "/upload", а сам запрос представляет запрос типа POST, то приложение получает коллекцию загруженных файлов с помощью 
свойства <span class="b">Request.Form.Files</span>, которое представляет тип <span class="b">IFormFileCollection</span>:</p>
<pre class="brush:c#;">IFormFileCollection files = request.Form.Files;</pre>
<p>Далее определяем каталог для загружаемых файлов (предполагается, что файлы будут храниться в каталоге "uploads", которая располагается в папке приложения)</p>
<pre class="brush:c#;">var uploadPath = $"{Directory.GetCurrentDirectory()}/uploads";</pre>
<p>Если такой папки нет, то создаем ее. Затем перебираем всю коллекцию файлов.</p>
<pre class="brush:c#;">foreach (var file in files)</pre>
<p>Каждый отдельный файл в этой коллекции представляет тип IFormFile. Для копирования файла 
в нужный каталог создается поток FileStream, в который записывается файл с помощью метода <code>CopyToAsync</code>.</p>
<pre class="brush:c#;">
using (var fileStream = new FileStream(fullPath, FileMode.Create))
{
	await file.CopyToAsync(fileStream);
...
<p>Если запрос идет по другому адресу и/или не представляет тип POST, то отправляем клиенту html-страницу <span class="b">index.html</span>.</p>
<p>Обратимся к приложению и выберем файлы для загрузки:</p>
<img src="./pics/2.36.png" alt="Upload files in ASP.NET Core and C#" />
<p>И после успешной загрузки нам отобразиться соответствующее сообщение:</p>
<img src="./pics/2.37.png" alt="Загрузка файлов на сервер в ASP.NET Core и C#" />
<p>А в каталоге проекта будет создана папка uploads, в которой появятся загуженные файлы:</p>
<img src="./pics/2.38.png" alt="Загрузка файлов на сервер ASP.NET Core и C#" />
...
<div class="nav"><p><a href="./2.11.php">Назад</a><a href="./">Содержание</a><a href="./2.7.php">Вперед</a></p></div></div>
<div class="socBlock">
