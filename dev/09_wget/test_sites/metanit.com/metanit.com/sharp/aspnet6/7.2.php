...
<html >
<head>
<title>ASP.NET Core и C# | Фабрика логгера и провайдеры логгирования</title>
<meta charset="utf-8" />
<meta name="description" content="Фабрика логгера ILoggerFactory в ASP.NET Core и C#, использование ILoggerBuilder для конфигурации логгера, встроенные провайдеры логгирования">
...
<h2>Фабрика логгера и провайдеры логгирования</h2><div class="date">Последнее обновление: 23.12.2021</div>
...
<p>В примерах в прошлой теме мы получали объект логгера, который добавляется через DI. Но мы можем также использовать фабрику логгера для его создания:</p>
<pre class="brush:c#;">
...
<p>В данном случае с помощью метода <span class="b">LoggerFactory.Create</span> создается фабрика логгера в виде объекта <span class="b">ILoggerFactory</span>. 
В качестве параметра метод принимает делегат, который устанавливает некоторые настройки логгирования. В частности, метод <span class="b">AddConsole()</span> объекта ILoggingBuilder 
устанавливает вывод сообщений лога на консоль. Затем метод <code>CreateLogger()</code> фабрики собственно создает логгер:</p>
<pre class="brush:c#;">ILogger logger = loggerFactory.CreateLogger&lt;Program&gt;();</pre>
<p>Метод <code>CreateLogger()</code> типизируется классом, который представляет категорию. В данном случае это класс Program, в котором неявно выполняется данный код. 
Но в качестве альтернативы название категории можно передать в метод в качестве параметра в виде строки:</p>
<pre class="brush:c#;">ILogger logger = loggerFactory.CreateLogger("WebApplication");</pre>
<p>В итоге мы получим тот же вывод сообщений на консоль. Но преимущество использования фабрики логгеров состоит в том, что мы можем дополнительно настроить 
различные параметры логгирования, в частности, провайдер логгирования.</p>
<h3>Получение фабрики логгера через dependency injection</h3>
<p>Как и логгер, фабрика логгера доступна в приложении в виде сервиса, соответственно ее можно получит через механизм внедрения зависимостей:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<h3>Провайдеры логгирования</h3>
<p>В примере выше логгирование шло на консоль. Вообще путь логгирования определяется провайдером логгирования. По умолчанию ASP.NET Core предоставляет 
следующие провайдеры:</p>
<ul>
<li><p><span class="b">Console</span>: вывод информации на консоль. Устанавливается методом <span class="b">AddConsole()</span></p></li>
<li><p><span class="b">Debug</span>: использует для ведения записей лога класс <span class="b">System.Diagnostics.Debug</span> и в частности его 
метод <code>Debug.WriteLine</code>. Соответственно все записи лога мы можем увидеть в окне Output в Visual Studio. Устанавливается методом 
<span class="b">AddDebug()</span>. Стоит отметить, что данный способ работает только при запуске проекта в режиме отладки</p></li>
<li><p><span class="b">EventSource</span>: на Windows введет логгирование в лог <a href="https://docs.microsoft.com/en-us/windows/win32/etw/event-tracing-portal">ETW</a> (Event Tracing for Windows), 
для просмотра которого может использоваться инструмент PerfView (или аналогичный инструменты). 
Хотя данный провайдер задумывался как кроссплатформенный, для Linux и MacOS пока назначение лога не определено. Устанавливается методом <span class="b">AddEventSourceLogger()</span></p></li>
<li><p><span class="b">EventLog</span>: записывает в Windows Event Log, соответственно работает только при запуске на Windows. Устанавливается 
методом <span class="b">AddEventLog()</span></p></li>
</ul>
<p>Например, вместо консоли зададим вывод лога в окне Output в Visual Studio:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/7.5.png" alt="Провайдеры логгирования logging providers в ASP.NET Core и C#" />
