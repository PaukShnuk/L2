...
<html >
<head>
<title>ASP.NET Core и C# | HttpRequest. Получение данных запроса</title>
<meta charset="utf-8" />
<meta name="description" content="Получение данных запроса в компоненте middleware в ASP.NET Core и C# с помощью класса HttpRequest, чтение HTTP-заголовков, определение пути запроса, простейшая система маршрутизации, парсинг строки запроса и QueryString">
...
<h2>HttpRequest. Получение данных запроса</h2><div class="date">Последнее обновление: 10.12.2021</div>
...
<p>Свойство <span class="b">Request</span> объекта HttpContext представляет объект HttpRequest и хранит информацию о запросе в виде следующих свойств:</p>
<ul>
<li><p><span class="b">Body</span>: предоставляет тело запроса в виде объекта <span class="b">Stream</span></p></li>
<li><p><span class="b">BodyReader</span>: возвращает объект типа <span class="b">PipeReader</span> для чтения тела запроса</p></li>
<li><p><span class="b">ContentLength</span>: получает или устанавливает заголовок <code>Content-Length</code></p></li>
<li><p><span class="b">ContentType</span>: получает или устанавливает заголовок <code>Content-Type</code></p></li>
<li><p><span class="b">Cookies</span>: возвращает коллекцию куки (объект Cookies), ассоциированных с данным запросом</p></li>
<li><p><span class="b">Form</span>: получает или устанавливает тело запроса в виде форм</p></li>
<li><p><span class="b">HasFormContentType</span>: проверяет наличие заголовка <code>Content-Type</code></p></li>
<li><p><span class="b">Headers</span>: возвращает заголовки запроса</p></li>
<li><p><span class="b">Host</span>: получает или устанавливает заголовок <code>Host</code></p></li>
<li><p><span class="b">HttpContext</span>: возвращает связанный с данным запросом объект <span class="b">HttpContext</span></p></li>
<li><p><span class="b">IsHttps</span>: возвращает <code>true</code>, если применяется протокол https</p></li>
<li><p><span class="b">Method</span>: получает или устанавливает метод HTTP</p></li>
<li><p><span class="b">Path</span>: получает или устанавливает путь запроса в виде объекта RequestPath</p></li>
<li><p><span class="b">PathBase</span>: получает или устанавливает базовый путь запроса. Такой путь не должен содержать завершающий слеш</p></li>
<li><p><span class="b">Protocol</span>: получает или устанавливает протокол, например, HTTP</p></li>
<li><p><span class="b">Query</span>: возвращает коллекцию параметров из строки запроса</p></li>
<li><p><span class="b">QueryString</span>: получает или устанавливает строку запроса</p></li>
<li><p><span class="b">RouteValues</span>: получает данные маршрута для текущего запроса</p></li>
<li><p><span class="b">Scheme</span>: получает или устанавливает схему запроса HTTP</p></li>
</ul>
<p>Рассмотрим применение некоторых из этих свойств.</p>
<h3>Получение заголовков запроса</h3>
<p>Для получения заголовков применяется свойство <span class="b">Headers</span>, которое представляет тип <span class="b">IHeaderDictionary</span>. 
Например, получим все заголовки запроса и выведем их на веб-страницу:</p>
<pre class="brush:c#;">
...
<img src="./pics/2.16.png" alt="Получение заголовков запроса в ASP.NET Core и C#" />
<p>Для большинства стандартных заголовков HTTP в этом интерфейсе определены одноименные свойства, например, для заголовка "content-type" определено свойство <code>ContentType</code>, 
а для заголовка "accept" - свойство <code>Accept</code>:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Также подобые заголовки, а также какие-то кастомные заголовки, для которых не определены подобные свойства, можно получить как и любой дугой элемент словаря:</p>
<pre class="brush:c#;">var acceptHeaderValue = context.Request.Headers["accept"];</pre>
<p>Для ряда заголовков в классе HttpRequest определены отдельные свойства: Host, Method, ContentType, ContentLength.</p>
<h3>Получение пути запроса</h3>
<p>Свойство <span class="b">path</span> позволяет получить запрошенный путь, то есть адрес, к которому обращается клиент:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/2.17.png" alt="получение пути запроса в ASP.NET Core и C#" />
<p>Это свойство позволяет нам узнать, по какому адресу обращается пользователю. Например, мы можем определить условную обработку запроса в зависимости от запрошенного адреса:</p>
<p>Свойство <span class="b">path</span> позволяет получить запрошенный путь, то есть адрес, к которому обращается клиент:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>В данном случае, если пользователь обращается по адресу "/date", то ему отображается текущая дата, а если обращается по адресу "/time" - текущее время. 
В остальных случаях отображается некоторое универсальное сообщение:</p>
<img src="./pics/2.18.png" alt="получение адреса запроса и маршрутизация в ASP.NET Core и C#" />
<p>Подобным образом можно определить свою систему маршрутизации, однако в ASP.NET Core по умолчанию есть инструменты, которые проще использовать для создания системы маршрутизации в приложении и которые будут рассмотрены в последующих статьях.</p>
<h3>Строка запроса</h3>
<p>Свойство <span class="b">QueryString</span> позволяет получить строку запроса. Строка запроса представляет ту часть запрошенного адреса, которая идет после символа <span class="b">?</span> и 
представляет набор параметров, разделенных символом амперсанда <span class="b">&amp;</span>:</p>
<pre class="brush:c#;">?параметр1=значение1&параметр2=значение2&параметр3=значение3</pre>
<p>Каждому параметру с помощью знака равно передается некоторое значение.</p>
<p>Стоит отметить, что строка запроса (query string) НЕ входит в путь запроса (path):</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/2.19.png" alt="получение строки запроса в ASP.NET Core и C#" />
<p>Так, в данном случае идет обращение по адресу</p>
<pre class="brush:c#;">https://localhost:7256/users?name=Tom&age=37</pre>
<p>Путь запроса или path представляет ту часть адреса, которая идет после домена/порта и до символа ?.</p>
<pre class="brush:c#;">/users</pre>
<p>Строка запроса или query string представляет ту часть адреса, которая идет начиная с символа ?.</p>
<pre class="brush:c#;">?name=Tom&age=37</pre>
<p>То есть в данном случае через строку запроса передаются два параметра. Первый параметр - <code>name</code> имеет значение "Tom". Bторой параметр - <code>age</code> имеет значение 37.</p>
<p>С помощью свойства <span class="b">Query</span> можно получить все параметры строки запроса в виде словаря:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/2.20.png" alt="парсинг строки запроса в ASP.NET Core и C#" />
<p>Соответственно можно вытащить из словаря Query значения отдельных параметров:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<img src="./pics/2.21.png" alt="параметры строки запроса в ASP.NET Core и C#" />
