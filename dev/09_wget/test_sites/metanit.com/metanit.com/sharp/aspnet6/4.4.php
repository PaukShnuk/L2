...
<html >
<head>
<title>ASP.NET Core и C# | Жизненный цикл зависимостей</title>
<meta charset="utf-8" />
<meta name="description" content="Жизненный цикл зависимостей и сервисов в ASP.NET Core и C#, применение методов AddTransient, AddScoped и AddSingleton">
...
<h2>Жизненный цикл зависимостей</h2><div class="date">Последнее обновление: 18.12.2021</div>
...
<p>ASP.NET Core позволяет управлять жизненным циклом внедряемых в приложении сервисов. С точки зрения жизненного цикла 
сервисы могут представлять один из следующих типов:</p>
<ul>
...
<p>Для рассмотрения механизма внедрения зависимостей и жизненного цикла возьмем следующий интерфейс <span class="b">ICounter</span>:</p>
<pre class="brush:c#;">
...
<p>Также определим реализацию этого интерфейса - класс <span class="b">RandomCounter</span></p>
<pre class="brush:c#;">
...
<p>Суть класса RandomCounter состоит в генерации некоторого случайного числа в диапазоне от 0 до 1000000.</p>
<p>И также определим новый класс, который нам более детально поможет разобраться в механизме Depedency Injection. Этот класс назовем 
CounterService и определим в нем следующий код:</p>
<pre class="brush:c#;">
...
<img src="./pics/4.7.png" alt="Жизненный цикл зависимостей и сервисов в ASP.NET Core и C#" />
<p>Теперь на примере этих классов рассмотрим уравление жизненным циклом сервисов.</p><h3>AddTransient</h3>
...
<p>Используем данный метод для добавления сервисов:</p>
<pre class="brush:c#;">
...
<p>Запустим проект:</p>
<img src="./pics/4.8.png" alt="AddTransient в ASP.NET Core и C#" />
...
применения изменим код приложения следующим образом:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
...
<img src="./pics/4.9.png" alt="AddScoped в ASP.NET Core и C#" />
...
<p>Для применения AddSingleton изменим код приложения:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
...
<img src="./pics/4.10.png" alt="AddSingleton в ASP.NET Core и C#" />
<p>Для создания singleton-объектов необязательно полагаться на механизм Depedency Injection. Мы его можем сами создать и передать в нужный метод:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
