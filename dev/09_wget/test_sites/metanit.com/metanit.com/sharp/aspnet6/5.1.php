...
<html >
<head>
<title>ASP.NET Core и C# | Установка каталога статических файлов. UseStaticFiles</title>
<meta charset="utf-8" />
<meta name="description" content="Установка каталога статических файлов и метод UseStaticFiles в ASP.NET Core и C#, ContentRoot и WebRoot, папка wwwroot">
...
<h1>Статические файлы</h1><h2>Установка каталога статических файлов. UseStaticFiles</h2><div class="date">Последнее обновление: 20.12.2021</div>
...
<p>Ранее уже рассматривалась отправка статических файлов. В частности, для отправки файлов мы могли использовать метод <code>SendFileAsync()</code> объекта HttpResponse:</p>
<pre class="brush:c#;">
...
<p>Однако ASP.NET Core также предоставляет специальный встроенный middleware, который подключается с помощью метода <span class="b">UseStaticFiles()</span> и который упрощает работу со статическими файлами.</p>
<p>При использовании этого middleware применяются некоторые условности. В частности, 
по умолчанию для определения пути хранения статических файлов в проекте используются два параметра <span class="b">ContentRoot</span> и 
<span class="b">WebRoot</span>, а сами статические файлы должны помещаться в каталог <code>ContentRoot/WebRoot</code>. На стадии разработки параметр 
"ContentRoot" соответствует каталогу текущего проекта. А параметр "WebRoot" по умолчанию представляет папку <span class="b">wwwroot</span> в рамках 
каталога ContentRoot. То есть, исходя из значений по умолчанию, то статические файлы следует 
располагать в папке "wwwroot", которая должна находиться в текущем проекте. Однако эти параметры при необходимости можно переопределить.</p>
<p>В разных типах проектов ASP NET Core данная папка может уже быть по умолчанию в проекте, а может отсутствовать. Например, в проекте по типу Empty 
данная папка отсутствует, поэтому ее надо добавлять вручную.</p>
<p>Итак, возьмем проект по типу Empty и добавим в него новую папку <span class="b">wwwroot</span>. Далее добавим в папку wwwroot новый файл 
<span class="b">index.html</span>. То есть у нас получится следующий проект:</p>
<img src="./pics/5.1.png" alt="Статические файлы и папка wwwroot в ASP.NET Core и C#" />
<p>Изменим код файла index.html, например, следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Для того, чтобы клиенты могли обращаться к этому файлу, подключим соответствующий компонент middleware с помощью метода <code>UseStaticFiles()</code>:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder();
var app = builder.Build();
...
<p>Для подключения функциональности работы со статическими файлами применяется метод <span class="b">UseStaticFiles()</span>, который реализован 
как метод расширения для типа IApplicationBuilder.</p>
<p>Теперь, если мы обратимся к добавленному файлу, например, по пути <code>/index.html</code>, 
то нам отобразится содержимое данной веб-страницы:</p>
<img src="./pics/5.2.png" alt="Обращение к статическим файлам в ASP.NET Core и C#" />
<p>По всем остальным запросам браузер выводил бы строку "Hello World".</p>
<p>Если бы index.html находился бы в какой-то вложенной папке, например, в <span class="ii">wwwroot/html/</span>, то для обращения к нему мы могли бы использовать путь 
<span class="ii">/html/index.html</span>. То есть middleware для работы со статическими сайтами автоматически сопоставляет запросы с путями к статическим файлам в рамках папки wwwroot.</p>

<h3>Изменение пути к статическим файлам</h3>
<p>Что делать, если нас не устраивает стандартная папка wwwroot. И мы, к примеру, хотим, чтобы все статические файлы в проекте находились в папке 
<span class="b">static</span>. 
Для этого добавим в проект папку <span class="b">static</span> в проект и определим в ней какой-нибудь html-файл. Пусть он будет называться <span class="b">content.html</span>:</p>
<img src="./pics/5.3.png" alt="Изменение пути к webroot в ASP.NET Core и C#" />
<p>Допустим, в этом файле будет какое-нибудь простейшее содержимое:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Чтобы приложение восприняло эту папку, изменим код создания хоста в файле Program.cs:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder(
...
<p>Для добавления пути к файлам используется перегруженная версия метода <span class="b">CreateBuilder()</span>, которая в качестве параметра 
принимает объект <span class="b">WebApplicationOptions</span>. Его свойство <span class="b">WebRootPath</span> позволяет установить папку 
для статических файлов.</p>
<p>И после этого мы также сможем обращаться к статическим файлам из папки <span class="b">static</span>:</p>
<img src="./pics/5.4.png" alt="UseStaticFiles и WebRootPath в ASP.NET Core и C#" />
...
<div class="nav"><p><a href="./3.7.php">Назад</a><a href="./">Содержание</a><a href="./5.2.php">Вперед</a></p></div></div>
<div class="socBlock">
