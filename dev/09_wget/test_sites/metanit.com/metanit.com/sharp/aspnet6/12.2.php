...
<html >
<head>
<title>ASP.NET Core и C# | Основные операции с данными в Entity Framework Core</title>
<meta charset="utf-8" />
<meta name="description" content="Основные операции с данными в Entity Framework Core в приложении Web API в ASP.NET Core и C# на примере базы данных MS SQL Server, простейший CRUD-интерфейс">
...
<h2>Основные операции с данными в Entity Framework Core</h2><div class="date">Последнее обновление: 25.12.2021</div>
...
<p>Рассмотрим, как мы можем выполнять основные операции с данными в приложении ASP.NET Core. За основу возьмем проект Web API, описанный в статье 
<a href="11.1">Пример приложения Web API</a>, но теперь добавим в него взаимодействие с базой данных.</p>
<p>Пусть у нас проекте определен класс <span class="b">User</span>, который будет представлять данные:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Для взаимодействия с базой данных MS SQL Server в качестве контекста данных определим следующий класс <span class="b">ApplicationContext</span>:</p>
<pre class="brush:c#;">
...
<p>Далее в файле <span class="b">Program.cs</span> определим основной код приложения, который будет обрабатывать запросы и подключаться к базе данных:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>Вначале добавляем класс ApplicationContext в сервисы приложения:</p>
<pre class="brush:js;">
var builder = WebApplication.CreateBuilder();
string connection = "Server=(localdb)\\mssqllocaldb;Database=applicationdb;Trusted_Connection=True;";
builder.Services.AddDbContext&lt;ApplicationContext&gt;(options =&gt; options.UseSqlServer(connection));
...
<p>Первая конечная точка обрабатывает запрос типа GET по маршруту "api/users":</p>
<pre class="brush:c#;">
app.MapGet("/api/users", async (ApplicationContext db) =&gt; await db.Users.ToListAsync());
</pre>
<p>Поскольку выше в коде контекст данных ApplicationContext был добавлен в качестве сервиса, то мы можем его получить через параметр обработчика 
конечной точки и через полученный контекст данных получить из БД список объектов User и отправить их клиенту.</p>
...
<p>Здесь через параметр id получаем из пути запроса идентификатор объекта User и по этому идентификатору ищем нужный объект в базе данных, 
используя контекст данных ApplicationContext. Если объект по Id не был найден, то возвращаем с помощью метода <code>Results.NotFound()</code> статусный код 404 с некоторым сообщением в формате JSON. 
...
<p>Здесь если объект по Id не найден в базе данных, то отправляем статусный код 404. Если же объект найден, то с помощью вызова <code>db.Users.Remove(user)</code> указываем, что данный объект надо 
удалить из БД. А с помощью последующего вызова <code>db.SaveChangesAsync()</code> сохраняем изменения в базу данных (то есть удаляем объект). И в конце посылаем удаленный объект клиенту.</p>
...
<p>Здесь мы ожидаем, что в запросе типа POST клиент будет передавать на сервер данные, которые соответствуют определению типа User. И поэтому 
...
конечной точки вместе с сервисом ApplicationContext.</p>
<p>После получения объекта User с помощью метода <code>db.Users.AddAsync(user)</code> указываем, что данный объект надо добавить в БД. А с помощью последующего 
вызова <code>db.SaveChangesAsync()</code> сохраняем изменения в базу данных (то есть добавляем объект). 
После добавления отправляем объект User обратно клиенту.</p>
<p>Если приложению приходит PUT-запрос по адресу "/api/users", то запрос обрабатывает последняя конечная точка:</p>
<pre class="brush:c#;">
app.MapPut("/api/users", async (User userData, ApplicationContext db) =&gt;
{
    // получаем пользователя по id
...
<p>Здесь аналогичным образом получаем отправленные клиентом данные в виде объекта User и сервис ApplicationContext. 
Затем пытаемся найти подобный объект в базе данных. Если объект не найден, отправляем статусный код 404.
Если объект найден, то изменяем его данные, с помощью вызова <code>db.SaveChangesAsync()</code> сохраняем изменения в базу данных и отправляем измененный объект обратно клиенту
<p>Теперь добавим код клиента. Для этого создадим в проекте новую папку <span class="b">wwwroot</span>, в которую добавим новый файл <span class="b">index.html</span></p>
<img src="./pics/12.3.png" alt="Клиент javascript для Web API и Entity Framework в ASP.NET Core и C#" />
<p>Определим в файле <span class="b">index.html</span> следующим код для взаимодействия с веб-приложением ASP.NET Core:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/12.4.png" alt="взаимодействие клиента javascript с базой данных MS SQL Server через Entity Framework в ASP.NET Core и C#" />
...
<img src="./pics/12.5.png" alt="REST, Web API, работа с базой данных MS SQL Server и Entity Framework в ASP.NET Core и C#" />
