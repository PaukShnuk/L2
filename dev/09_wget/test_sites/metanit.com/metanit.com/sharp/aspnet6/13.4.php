...
<html >
<head>
<title>ASP.NET Core и C# | Аутентификация с помощью куки</title>
<meta charset="utf-8" />
<meta name="description" content="Аутентификация с помощью куки в приложении ASP.NET Core и C#, настройка аутентификации и метод AddCookie, установка аутентификационных кук и метод SignInAsync, ClaimsIdentity и ClaimsPrincipal">
...
<h2>Аутентификация с помощью куки</h2><div class="date">Последнее обновление: 10.01.2022</div>
...
<p>Одним из распространенных способов аутентификации в веб-приложенях является аутентификация с помощью куки. 
И ASP.NET Core имеет встроенную поддержку для данного типа аутентификации.<p>
<p>Для примения аутентификации с помощью куки в метод <span class="b">AddAuthentication()</span> передается схема "Cookies":</p>
<pre class="brush:c#;">builder.Services.AddAuthentication("Cookies")</pre>
<p>Чтобы не ошибиться в написании схемы еще можно передавать константу <code>CookieAuthenticationDefaults.AuthenticationScheme</code>, 
которая имеет то же самое значение.</p>
<pre class="brush:c#;">builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme)</pre>
<p>Кроме того, для настройки аутентификации с помщью куки необходимо вызвать метод <span class="b">AddCookie()</span>, который реализован как 
метод расширения для типа AuthenticationBuilder:</p>
<pre class="brush:c#;">
public static AuthenticationBuilder AddCookie(this AuthenticationBuilder builder, Action&lt;CookieAuthenticationOptions&gt; configureOptions);
</pre>
<p>В качестве параметра метод принимает делегат, который с помощью объекта <span class="b">CookieAuthenticationOptions</span> устанавливает настройки аутентификации.</p>
<p>Рассмотрим на примере, как использовать самую простейшую аутентификацию с помощью куки. Для этого определим в файле <span class="b">Program.cs</span> 
следующий код:</p>
<pre class="brush:c#;">
...
<p>Для представления пользователя класс <span class="b">Person</span>:</p>
<pre class="brush:c#;">
record class Person(string Email, string Password);
</pre>
<p>В качестве условной базы данных для теста используется список people:</p>
<pre class="brush:c#;">
var people = new List&lt;Person&gt;
{
    new Person("tom@gmail.com", "12345"),
...
<p>Именно с этими данными мы будем сравнивать присланные от клиента логин и пароль.</p>
<p>Для подключения аутентификации куки регистрируем соответствующие сервисы:</p>
<pre class="brush:c#;">
builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme)
    .AddCookie(options =&gt; options.LoginPath = "/login");
</pre>
<p>Свойство <span class="b">LoginPath</span> класса CookieAuthenticationOptions указывает на путь, по которому неаутентифицированный клиент будет автоматически переадресовываться 
при обращении к ресурсу, для доступа к которому требуется аутентификация.</p>
<p>Конечная точка, которая обрабатывает get-запросы по пути "/login", будет отпаравлять в ответ html-форму для ввода email и пароля:</p>
<pre class="brush:c#;">
app.MapGet("/login", async (HttpContext context) =&gt;
{
    context.Response.ContentType = "text/html; charset=utf-8";
...
<p>В данном случае для простоты весь html-код определен в виде строки, но, естественно можно сделать по-разному, например, опрделить отдельную html-страницу и ее отправлять отправлять клиенту.</p>
<p>На отправляемой форме клиент должен будет заполнить поля "email" и "password", и после нажатия на кнопку отправки значения этих полей 
в запросе типа POST будут получены в обработчике другой конечной точки:</p>
<pre class="brush:c#;">
app.MapPost("/login", async (string? returnUrl, HttpContext context) =&gt;
{
</pre>
<p>Обработчик этой конечной точки принимает два параметра. Прежде всего, это передаваемый через механизм внедрения зависимостей объект контекста запоса 
...
и адрес обработки отправлнных данных совпадает - "/login", то через параметр <code>returnUrl</code> мы можем получить путь, 
по которому изначально обращался клиент. Однако поскольку клиент также может напрямую обратиться к форме логина, то в этом случае данный параметр будет иметь значение null.</p>
<p>В самом же обработчике конечной точки вначале получаем из данных формы отправленные email и пароль:</p>
<pre class="brush:c#;">
var form = context.Request.Form;
if (!form.ContainsKey("email") || !form.ContainsKey("password"))
	return Results.BadRequest("Email и/или пароль не установлены");
...
<p>Получив данные, проверяем, а есть ли объект с такими данными в нашей условной базе данных - списке people:</p>
<pre class="brush:c#;">
Person? person = people.FirstOrDefault(p =&gt; p.Email == email && p.Password == password);
if (person is null) return Results.Unauthorized();
</pre>
<p>Далее производится установка аутентификационных кук, которые будут применяться для определения клиента и его прав в приложении:</p>
<pre class="brush:c#;">
var claims = new List&lt;Claim&gt; { new Claim(ClaimTypes.Name, person.Email) };
// создаем объект ClaimsIdentity
ClaimsIdentity claimsIdentity = new ClaimsIdentity(claims, "Cookies");
...
<p>Для установки кук у класса HttpContext применяется асинхронный метод <span class="b">SignInAsync()</span>. В качестве параметра он принимает 
применяемую схему аутентификации, в нашем случае это "Cookies", то есть значение константы <code>CookieAuthenticationDefaults.AuthenticationScheme</code>. 
А в качестве второго параметра передается объект <span class="b">ClaimsPrincipal</span>, 
который представляет пользователя.</p>
<p>Для правильного создания и настройки объекта ClaimsPrincipal вначале создается список claims - набор объектов Claim - грубо говоря набор данных, которые описывают пользователя. 
...
claim, который в качестве типа принимает константу <code>ClaimTypes.Name</code>, а в качестве значения - email пользователя.</p>
<p>Далее создается объект <span class="b">ClaimsIdentity</span>, который нужен для инициализации ClaimsPrincipal. В ClaimsIdentity передается 
ранее созданный список claims и тип аутентификации, в данном случае "Cookies". Тип аутентификации может представлять произвольную строку.</p></li>
<p>И после вызова метода <code>сontext.SignInAsync</code> будут формироваться аутентификационные куки, которые будут отправлены клиенту и 
при последующих запросах будут передаваться обратно на сервер, десериализоваться и использоваться для аутентификации пользователя.</p>
<p>В самом конце перенаправляем аутентификацированного пользователя обратно на адрес, с которого его перебросило на форму логина:</p>
<pre class="brush:c#;">return Results.Redirect(returnUrl??"/");</pre>
<p>Для выхода из сайта определена конечная точка, которая обрабатывает запросы по пути "/logout":</p>
<pre class="brush:c#;">
app.MapGet("/logout", async (HttpContext context) =&gt;
{
    await context.SignOutAsync(CookieAuthenticationDefaults.AuthenticationScheme);
...
<p>Его основа - вызов метода <span class="b">context.SignOutAsync()</span>, который удаляет аутентификационные куки. В качестве параметра он 
принимает схему аутентификации.</p>
<p>Для тестирования авторизации определена четвертая конечная точка, которая обрабатывает запросы к корню приложения:</p>
<pre class="brush:c#;">
app.Map("/", [Authorize]() =&gt; $"Hello World!");
</pre>
<p>Поскольку эта конечная точка использует атрибут <span class="b">Authorize</span>, то доступ к ней имеют только аутентифицированные пользователи.</p>

<p>Таким образом, когда мы, не будучи аутентифицированными, обращаемся по пути "/", нас перебрасывает на путь "/login", 
где нам надо заполнить форму логина:</p>
<img src="./pics/13.9.png" alt="Аутентификация с помощью куки в ASP.NET Core и C#" />
<p>И после ввода корректных данных и нажатия на кнопку отправки, конечная точка <code>app.MapPost("/login")</code> получит отправленные данные, проверит, 
есть ли с такими данными объект в списке people, и при наличии такого объекта установит аутентификационные куки и 
перенаправит пользователя обратно на адрес "/":</p>
<img src="./pics/13.10.png" alt="Аутентификация и авторизация с помощью куки в ASP.NET Core и C#" />
<p>При этом в самом браузере мы сможем увидеть сохраненные аутентификационные куки, которые называются <code>.AspNetCore.Cookies</code> или <code>.AspNetCore.[Название схемы аутентификации]</code>.</p>
<p>Таким образом, мы можем добавить простейшую систему аутентификации и авторизации с помощью куки в приложение ASP.NET Core</p>
...
<div class="nav"><p><a href="./13.3.php">Назад</a><a href="./">Содержание</a><a href="./13.5.php">Вперед</a></p></div></div>
<div class="socBlock">
