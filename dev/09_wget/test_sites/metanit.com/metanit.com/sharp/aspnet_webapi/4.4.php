...
<html >
<head>
<title>Фильтры исключений в ASP.NET Web API</title>
<meta charset="utf-8" />
...
<h2>Фильтры исключений</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p>
...
<p>Фильтры исключений позволяют обработать исключения, возникшие в методе во время выполнения запроса. Все фильтры исключений реализуют интерфейс 
<code>IExceptionFilter</code> из пространства имен <code>System.Web.Http.Filters</code>:</p>
<pre class="brush:c#;">
...
<p>Метод <code>ExecuteExceptionFilterAsync</code> вызывается при возникновении исключения в методе контроллера. 
В качестве параметров в метод передается контекст <code>HttpActionExecutedContext</code> с полной информацией о запросе, а также токен 
отмены операции cancellationToken.</p>
<p>Создадим свой фильтр исключений:</p>
<pre class="brush:c#;">
using System;
...
<p>С помощью свойства <code>actionExecutedContext.Exception</code> мы можем получить информацию об исключении, возникшем в методе контроллера. 
А используя метод <code>actionExecutedContext.Request.CreateErrorResponse()</code> можно переопределить ответ, который генерирует метод контроллера. В итоге 
пользователь получит сообщение, которое сгенерировал фильтр исключений.</p>
<P>Применим фильтр к методу:</P>
<pre class="brush:c#;">
...
<p>Если мы передадим в метод для параметра id значение больше 2, то пользователь увидит в браузере сообщение об ошибке от фильтра исключений:</p>
<img src="pics/webapi2.png" alt="Обработка исключений в Web API" />
<h3>ExceptionFilterAttribute</h3>
<p>Кроме прямой реализации интерфейса IExceptionFilter мы можем использовать класс <span class="bb">ExceptionFilterAttribute</span>:</p>
<pre class="brush:c#;">
using System;
using System.Net;
using System.Net.Http;
...
<p>Чтобы определить обработку исключения, надо переопределить метод <code>OnExceptionAsync()</code>. И в отличие от ранее определенного фильтра 
...
пользователю. Все эти данные мы можем задать в зависимости от ситуации при использовании атрибута.</p>
<p>Применим фильтр:</p>
<pre class="brush:c#;">
