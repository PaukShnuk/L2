...
<html >
<head>
<title>Переопределение фильтров и глобальные фильтры в ASP.NET Web API</title>
<meta charset="utf-8" />
...
<h2>Переопределение фильтров и глобальные фильтры</h3>
...
<p>Фильтры могут иметь разные области видимости: мы можем определить фильтр для отдельного метода, для всего контроллера, либо для всего приложения, то есть сделать фильтр глобальным.</p>
<p>Чтобы применить фильтр ко всем контроллерам, то есть сделать его глобальным, надо внести соответствующую запись в файл <i>WebApiConfig.cs</i>. 
Например, у нас есть некоторый фильтр:</p>
<pre class="brush:c#;">
...
<p>Тогда в классе WebApiConfig нам надо добавить этот фильтр в коллекцию фильтров приложения:</p>
<pre class="brush:c#;">
...
<p>Переопределение фильтров позволяет отключить действие других фильтров для определенного метода. Это может быть полезно, когда какой-либо фильтр 
применяется ко всему контроллеру или глобально ко всем контроллерам, однако для какого-то определенного метода его надо отключить.</p>
<p>Фильтры переопределений реализуют интерфейс <span class="bb">IOverrideFilter</span> из пространства имен <code>System.Web.Http.Filters</code>:</p>
<pre class="brush:c#;">
...
<p>Его единственное свойство позволяет определить тип фильтров, которые надо переопределить.</p>
<p>Web API уже располагает набором встроенных реализаций фильтров переопределений:</p>
<ul>
<li><p><code>OverrideAuthentication</code>: отключает фильтры аутентификации</p></li>
<li><p><code>OverrideAuthorization</code>: отключает фильтры авторизации</p></li>
<li><p><code>OverrideAction</code>: отключает фильтры методов</p></li>
<li><p><code>OverrideException</code>: отключает фильтры исключений</p></li>
</ul>
<p>Благодаря этому набору встроенных фильтров нам в принципе не надо создавать свои фильтры. Например, отключим авторизацию для одного из методов:</p>
<pre class="brush:c#;">
