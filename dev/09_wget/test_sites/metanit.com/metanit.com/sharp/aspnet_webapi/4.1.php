...
<html >
<head>
<title>Фильтры в ASP.NET Web API</title>
<meta charset="utf-8" />
...
<p>Фильтры добавляют в процесс обработки запроса контроллером дополнительную логику. В Web API определены все те же виды фильтров, что и в 
MVC 5 с аналогичным действием:</p>
<ul>
<li><p><span class="bb">Фильтры аутентификации</span>: представляет интерфейс <code>IAuthenticationFilter</code>. Определяет, 
аутентифицирован ли клиент. Данный фильтр запускается до выполнения любого другого фильтра или метода действий</p></li>
<li><p><span class="bb">Фильтры авторизации</span>: представляет интерфейс <code>IAuthorizationFilter</code>. Определяет, имеет ли пользователь права доступа к данному ресурсу. 
Данный фильтр запускается после фильтра аутентификации, но до любого другого фильтра или метода действия</p></li>
<li><p><span class="bb">Фильтры действий</span>: представляет интерфейс <code>IActionFilter</code>. Применяется к методу до и после обработки запроса</p></li>
<li><p><span class="bb">Фильтры переопределений</span>: представляет интерфейс <code>IOverrideFilter</code>. Переопределяет действие других фильтров по отношению к методу</p></li>
<li><p><span class="bb">Фильтры исключений</span>: представляет интерфейс <code>IExceptionFilter</code>. Используется для обработки исключений, которые возникают в методе при обработке запроса</p></li>
</ul>
<p>Но в отличие от фильтров в MVC 5, фильтры в Web API отличаются тем, что реализуют интерфейс <span class="bb">IFilter</span> из пространства имен 
...
<h2>Фильтры аутентификации</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p>
<p>Фильтры аутентификации проверяют, аутентифицирован ли пользователь. Если к методу применяется несколько фильтров, то фильтр аутентификации вызывается 
до всех остальных фильтров.</p>
<p>В основе всех фильтров аутентификаций лежит реализация интерфейса <span class="bb">IAuthenticationFilter</span> из пространства имен <code>System.Web.Http.Filters</code>:</p>
<pre class="brush:c#;">
...
<p>Метод <code>AuthenticateAsync()</code> вызывается до обработки запроса для идентификации пользователя, а метод <code>ChallengeAsync()</code> 
вызывается уже когда начинается обработка для отправки ответа клиенту.</p>
<p>При рассмотрении аутентификации в Web API важно понимать ее отличие от ASP.NET MVC. Web Api не будет перенаправлять пользователя на страницу логина или ошибки, 
как нередко делается в MVC. Вместо этого клиенту будет отправляться статусный код HTTP 401 (Unauthorized).</p>
<p>Создадим свой фильтр. Для этого определим в проекте папку Filters и добавим в нее новый класс:</p>
<pre class="brush:c#;">
using System;
...
<p>Аутентифицированный пользователь в ASP.NET представлен объектом интерфейса <span class="b">IPrincipal</span>. Контекст метода HttpAuthenticationContext хранит ссылку на 
этот объект. С помощью свойства <code>context.Request.Headers.Authorization</code> мы получаем заголовки авторизации, которые хранят 
данные об авторизации: логин и пароль в зашифрованном виде с применением алгоритма Base64.</p>
<p>Чтобы получить логин и пароль, используем декодирование: <code>Encoding.ASCII.GetString(Convert.FromBase64String(authentication.Parameter))</code>. 
...
встроенный класс <code>GenericPrincipal</code>, но можно сделать свой объект IPrincipal. Кроме того, я определил массив ролей с одной ролью user. Хотя 
роли в данном случае не играют значение, но они требуются для создания объекта GenericPrincipal.</p>
<p>Если данные об авторизации не установлены и объект IPrincipal равен null, то выдаем ошибку <code>UnauthorizedResult</code>. Клиенту при получении 
ошибки будет отображаться окно для ввода аутентификационных данных:</p>
<img src="pics/webapi1.png" alt="Аутентификация в Web Api" />
<p>Хотя тут объекту IPrincipal просто присваивается полученный из заголовков логин и случайная роль, но в реальном приложении мы бы обращались к базе данных 
для получения пользователя и его роли из бд. И в зависимости от результатов выборки производили те или иные действия.</p>
