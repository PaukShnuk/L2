...
<html >
<head>
<title>Определение формата ответа в ASP.NET Web API</title>
<meta charset="utf-8" />
...
<h2>Определение формата ответа</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p>
...
<p>Если мы обратимся к контроллеру Web API для получения некоторого списка объектов, например, по стандартному маршруту <i>api/values/</i>, то 
в IE мы получим эти объекты в формате json, а в Firefox и Chrome - в формате xml. Например, у нас есть следующий метод:</p>
<pre class="brush:c#;">
...
<p>В Firefox мы получим следующий результат:</p>
<pre class="brush:xml;">
&lt;ArrayOfBook&gt;
	&lt;Book&gt;
...
<p>При взаимодействии с клиентом между ним и сервером происходят два процесса: <b>content negotiation</b> (согласование содержимого) и <b>media formatting</b> 
(форматирование или сериализация содержимого). На первом этапе сервер узнает, в каком формате клиенту предпочтительнее отправлять ответ. На втором этапе происходит сериализация данных в этот формат.</p>
<p>Данные о формате клиент передает 
с помощью заголовка <span class="bb">Accept</span>. Различные браузеры отправляют разные заголовки <code>Accept</code>. Например, в IE данный заголовок 
выглядит следующим образом: <code>Accept: text/html, application/xhtml+xml, */*</code>. Часть <code>*/*</code> указывает, что данные могут быть приняты 
...
другим образом, например, <code>Accept: text/html, application/xhtml+xml, application/xml;q=0.9,*/*;q=0.8</code>. Здесь уже явно указывается с помощью 
<code>application/xml</code>, что браузер хочет получать данные в формате xml.</p>
<p>Web API имеет встроенную поддержку для следующих форматов: JSON, BSON, XML. Если же в запросе не передается заголовок Accept, то используется формат по умолчанию - json.</p>
<p>Однако мы можем явно указать приложению, чтобы оно посылало данные в формате json. Для этого в файл <i>WebApiConfig.cs</i> добавим строку:</p>
<pre class="brush:c#;">
using System;
...
<p>Согласование формата на стороне сервера производит специальный класс, который представляет реализацию интерфейса <span class="bb">IContentNegotiator</span>.</p>
<pre class="brush:c#;">
...
<p>Единственный метод интерфейса <code>Negotiate</code> получает тип данных, данные запроса и набор форматировщиков MediaTypeFormatter, которые сериализуют данные 
в ответ клиенту. А в качестве результата метода используется объект ContentNegotiationResult.</p>
<p>Если результат метода равен <code>null</code>, то в ответ клиенту отправляется статусный код <code>406 (Unacceptable)</code>. Этот код указавает на невозможность 
сериализации данных в нужный формат.</p>
<p>По умолчанию Web API уже содержит реализацию данного интерфейса по умолчанию - класс <span class="bb">DefaultContentNegotiator</span>. Но нам может потребоваться переопределить 
стандартное поведение данного класса. Например, нам надо отдавать клиенту с ОС Андроид данные в формате json. Для этого мы можем добавить в проект 
следующий класс:</p>
<pre class="brush:c#;">
using System;
using System.Collections.Generic;
...
<p>Тут мы проверяем, содержит ли строка UserAgent подстроку ""android", и если содержит, отдаем данные в формате json.</p>
<p>Но просто определить класс еще недостаточно. Теперь надо переопределить конфигурацию Web API. Для этого откроем файл <i>WebApiConfig.cs</i>, который 
находится в папке App_Start, и добавим в него одну строку:</p>
<pre class="brush:c#;">
public static class WebApiConfig
{
    public static void Register(HttpConfiguration config)
...
<p>Метод <code>config.Services.Replace()</code> заменяет стандартный тип DefaultContentNegotiator вышеопределенным классом для участия в согласовании содержимого.</p>
<p>И теперь на все браузеры под управлением ОС Андроид будет приходить контент в формате json.</p>
<p>Однако необязательно переопределять класс DefaultContentNegotiator, мы также можем воспользоваться стандартными методами:</p>
<ul>
<li><p><code>Json(content)</code>: сериализует контент в json<p></li>
<li><p><code>Content(status, content, formatter)</code>: сериализует контент в определенный формат с помощью класса formatter<p></li>
<li><p><code>Content(status, content, formatter, mimeType)</code>: то же самое, только используется объект MediaTypeHeaderValue для определения mime-типа<p></li>
<li><p><code>Content(status, content, formatter, mimeString)</code>: аналогично предыдущему, только mime-тип определяется в виде строки<p></li>
</ul>
<p>Эти методы переопределяют стандартный процесс согласования контента. Например, чтобы явным образом отправлять данные в формате json, используем метод Json():</p>
<pre class="brush:c#;">
