...
<html >
<head>
<title>Отправка массивов и сложных данных в ASP.NET Web API</title>
<meta charset="utf-8" />
...
<h2>Отправка массивов и сложных данных</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p>
...
<p>Благодаря использованию привязчиков мы можем отправлять контроллеру Web Api не только простые значения, но и более сложные данные, например, коллекции объектов.</p>
<p>Передача массивов производится также, так и простых объектов. Например, в представлении мы отправляем массив:</p>
<pre class="brush:js;">
var books = [{
            name: "Чайка",
            price: 200,
...
<p>Получение в методе контроллера будет выглядеть так:</p>
<pre class="brush:c#;">
...
<p>Поскольку Web Api имеет встроенного привязчика для коллекций, то вместо массива мы могли бы использовать и список:</p>
<pre class="brush:c#;">
...
<p>Клиентский код на javascript от этого бы не изменился.</p>
<p>Данные могут включать объекты других классов. Например, каждая книга содержит ссылку на автора:</p>
<pre class="brush:c#;">
public class Book
{
...
<p>Отправка данных в javascript:</p>
<pre class="brush:js;">
var books = [{
            name: "Чайка",
            price: 200,
...
<p>Метод контроллера не изменяется: <code>public void Post(List&lt;Book&gt; books)</code></p>
<p>Мы также можем передавать простые массивы из строки запроса, используя атрибут <span class="bb">ModelBinder</span>. Например, у нас есть метод:</p>
<pre class="brush:c#;">
...
<p>Тогда передать массив nums в метод мы можем с помощью, например, такой строки запроса: <i>api/values/getvalue?nums=2&amp;nums=3</i>. После 
этого метод получит массив из двух чисел 2 и 3.</p>
<a name="dictionaryinwebapi"></a>
<h3 >Привязка словарей Dictionary</h3>
<p>Подобным образом мы можем отправлять объекты Dictionary. Например, упростим модель Book:</p>
<pre class="brush:c#;">
public class Book
{
    public string Name { get; set; }
...
<p>Чтобы получить словарь объектов в методе, данный словарь передается в качестве параметра с использованием атрибута ModelBinder:</p>
<pre class="brush:c#;">
...
<p>Тогда в представлении мы можем отправить словарь следующим образом:</p>
<pre class="brush:js;">
function SendItems() {
...
<p>В качестве ключей я выбрал произвольные строковые значения.</p>
<p>Более того мы можем отправить данные и через строку запроса. Например, вышеиспользованные данные отправляются такой строкой запроса:</p>
<pre>
