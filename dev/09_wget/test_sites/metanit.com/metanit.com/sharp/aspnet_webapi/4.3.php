...
<html >
<head>
<title>Фильтры действий в ASP.NET Web API</title>
<meta charset="utf-8" />
...
<h2>Фильтры действий</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p>
...
<p>Фильтры действий выполняются до и после работы метода контроллера. С их помощью мы можем изменить данные запроса, передаваемые в 
объекте <code>HttpRequestMessage</code>, так и внести изменения в результат метода - объект <code>HttpResponseMessage</code>. Фильтры действий реализуют интерфейс IActionFilter:</p>
<pre class="brush:c#;">
...
<p>Параметр <code>actionContext</code> содержит информацию о методе, а также несет ряд данных о запросе.</p>
<p>Параметр <code>cancellationToken</code> представляет токен для отмены операции.</p>
<p>Параметр <code>continuation</code> представляет выполняющийся метод. Вызов этого параметра собственно и предоставляет вызов метода, к которому 
данный фильтр и будет применяться.</p>
<p>Создадим фильтр:</p>
<pre class="brush:c#;">
using System;
...
<p>С помощью класса Stopwatch высчитываем время выполнения метода в секундах. Само выполнение метода представляет выражение <code>await continuation()</code>. 
Его итогом является объект <code>HttpResponseMessage</code>, в который в виде заголовка мы добавляем время выполнения. Затем при обработке ответа на клиенте, либо 
просто в браузере через инструменты разработчика мы можем получить это время.</p>
<p>Применение фильтра к методу:</p>
<pre class="brush:c#;">
...
<p>Хотя данный фильтр и позволяет обрабатывать ряд условий при вызове метода, но более удобным механизмом для создания фильтра действий является 
использование класса <span class="bb">ActionFilterAttribute</span>. Дело в том, что он определяет два метода:</p>
<ul>
<li><p><span class="bb">OnActionExecutingAsync</span>: выполняется до выполнения метода</p></li>
<li><p><span class="bb">OnActionExecutedAsync</span>: выполняется после выполнения метода</p></li>
</ul>
<p>Использование данных методов позволяет разграничить обработку до вызова метода и обработку после:</p>
<pre class="brush:c#;">
using System;
...
<p>Каждый метод возвращает задачу, которая запускается методом <code>Task.Run()</code>. Для отправки заголовков здесь используется контекст метода 
<code>actionExecutedContext</code>, который позволяет манипулировать ответом с помощью свойства <code>actionExecutedContext.Response</code>.</p>
