...
<html >
<head>
<title>Валидация в ASP.NET Web API</title>
<meta charset="utf-8" />
...
<h2>Валидация в Web API</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p>
...
<p>В Web API, как и в MVC, большое значение имеет валидация данных. Однако в Web API она имеет свои особенности.</p>
<p>В первую очередь состояние обработки запроса на сервере мы можем контроллировать с помощью статусных кодов:</p>
<ul>
...
<p>Но мы не ограничены статусными кодами и, как и в MVC, можем использовать для валидации объект <span class="bb">ModelState</span>.</p>
<p>Допустим, мы работаем со следующей моделью:</p>
<pre class="brush:c#;">
public class Book
{
...
<p>В Web API применяются все те же атрибуты валидации, что и в ASP.NET MVC. Общая схема валидации в контроллере Web API, например, при обработке запроса POST, 
...
<p>С помощью объекта ModelState здесь валидируется полученная модель book. Но кроме проверки свойства <span class="bb">ModelState.IsValid</span> 
мы также можем добавить и еще дополнительные проверки. Например:</p>
<pre class="brush:c#;">
if (book.Year == 1984)
    ModelState.AddModelError("book.Year", "Год не должен быть равен 1984");
</pre>
...
по ключу book.Year. Причем по одному ключу мы можем указать множество ошибок.</p>
...
<p>Теперь рассмотрим, как мы можем получить эти ошибки на стороне клиента. Пусть у нас будет следующее представление:</p>
<pre class="brush:xml;">
@{
    Layout = null;
}
...
<p>Для вывода ошибок здесь определен специальный блок с id="errors". При получении ошибки срабатывает функция error, первый параметр которой хранит 
...
Например, чтобы получить ошибки свойства Year, придется использовать вызов <code>response['ModelState']['book.Year']</code>. Получение сообщения об ошибках происходит по тем же ключам, 
которые были определены в контроллере. Причем каждый из таких вызовов представляет собой массив.</p>
<p>И теперь если мы введем некорретные данные, мы получим сообщения об ошибках.</p>
<img src="pics/webapi_validation.png" alt="Валидация в Web API" />
