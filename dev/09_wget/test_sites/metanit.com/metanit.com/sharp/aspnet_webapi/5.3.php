...
<html >
<head>
<title>Создание десктопного клиента на C# для ASP.NET Web API</title>
<meta charset="utf-8" />
...
<h2>Создание десктопного клиента на C# для Web API</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/aspnet5/">Руководство по ASP.NET Core</a></p>
...
<p>В прошлой теме в качестве клиента использовались представления самого же проекта. Однако, как правило, клиентами могут выступать самые разные типы приложений. 
Рассмотрим подключение к Web API из консольного приложения, написанного на C#.</p>
<p>Создадим проект приложения Web API с типом аутентификации Individual User Accounts (или возьмем деланный в прошлой теме). Добавим в решение второй проект - проект 
консольного приложения.</p>
<p>Добавим в проект консольного приложения через Nuget пакет <span class="bb">Microsoft.AspNet.WebApi.Client</span></p>
<img src="pics/webapi_client.png" alt="Создание клиента для Web API на c#" />
<p>Изменим код консольного приложения следующим образом:</p>
<pre class="brush:c#;">
using System;
...
<p>Вначале устанавливаем две глобальные переменные для адреса сайта и токена:</p>
<pre class="brush:c#;">
private const string APP_PATH= "http://localhost:55122";
private static string token;
</pre>
<p>После ввода emailа и пароля вывызвается метод <code>Register()</code>, который отправляет регистрационные данные по маршруту <i>/api/Account/Register</i></p>
<p>Зарегистрировавшись, мы можем получить токен с помощью метода <code>GetTokenDictionary()</code>. Так как возвращаемый сервером объект представляет 
набор ключей и значений в формате json, то его можно преобразовать к словарь Dictionary, а из него по ключу получить сам токен:</p>
<pre class="brush:c#;">
Dictionary&lt;string, string&gt; tokenDictionary = GetTokenDictionary(userName, password);
token = tokenDictionary["access_token"];
</pre>
<p>Получив токен, уже можно отправлять запросы к любым защищенным ресурсам. Например, далее отправляется запрос по маршруту <i>/api/Account/UserInfo</i> для 
получения данных о пользователе. И также отправляется запрос к методу Get контроллера ValuesController.</p>
<p>Для отправки запросов к серверу во всех методах используется класс <span class="bb">HttpClient</span>.</p>
<p>Для упрощения запросов с токеном создан вспомогательный метод <code>CreateClient()</code>, который устанавливает токен в заголовках авторизации 
и возвращает объект HttpClient.</p>
<pre class="brush:c#;">
static HttpClient CreateClient(string accessToken = "")
{
    var client = new HttpClient();
...
<p>Запустив параллельно оба проекта на выполнение, можно протестировать их взаимодействие:</p>
<img src="pics/webapi_client2.png" alt="Тестирование клиента для Web API на c#" />
