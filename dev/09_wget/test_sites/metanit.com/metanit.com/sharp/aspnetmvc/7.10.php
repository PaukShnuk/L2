...
<html >
<head>
<title>ASP.NET Core MVC и C# | Атрибут HtmlTargetElement</title>
<meta charset="utf-8" />
<meta name="description" content="Атрибут HtmlTargetElement, его настройка и его использование для создания tag-хелпера в ASP.NET Core MVC и C#">
...
<h2>Атрибут HtmlTargetElement</h2><div class="date">Последнее обновление: 05.04.2022</div>
...
<p>Применение атрибута HtmlTargetElement позволяет переопределить поведение tag-хелпера. Для этого класс HtmlTargetElementAttribute определяет следующие свойства:</p>
<ul>
<li><p><span class="b">Attributes</span>: указывает, что tag-хелпер применяется только к тем элементам, которые имеют определенные атрибуты.</p></li>
<li><p><span class="b">ParentTag</span>: указывает, что tag-хелпер применяется только к тем элементам, которые определены внутри определенного элемента</p></li>
<li><p><span class="b">TagStructure</span>: указывает, что tag-хелпер применяется только к тем элементам, которые соответствуют определенному значению из перечисления 
<code>TagStructure</code>: Unspecified, NormalOrSelfClosing (стандартный или самозакрывающийся элемент) и WithoutEndTag (элемент без закрывающего тега)</p></li>
</ul>
<h3>Применение tag-хелпера к атрибутам</h3>
<p>Определим следующий класс tag-хелпера:</p>
<pre class="brush:c#;">
...
<p>Атрибут HtmlTargetElement указывает, что класс будет применяться к элементам, у которых установлен атрибут header. Важно, что название хелпера опять 
же соответствует целевому объекту - атрибуту header.</p>
<p>В самом классе происходит замена существующего элемента на элемент <code>&lt;h2&gt;</code> и удаление атрибута header. Все внутреннее содержание, текст, который определен 
в блоках div, при этом сохраняется.
<p>Используем данный tag-хелпер в представлении и для этого определим следующие элементы:</p>
<pre class="brush:xml;">
@addTagHelper *, MvcApp
...
<p>Здесь к блокам div применяется атрибут header, который как бы помечает эти блоки как заголовочные элементы.</p>
<p> В итоге вместо двух блоков div будут созданы следующие заголовки:</p>
<pre class="brush:xml;">
&lt;h2&gt;Hello World&lt;/h2&gt;
&lt;h2&gt;Hello METANIT.COM&lt;/h2&gt;
</pre>
<img src="./pics/7.18.png" alt="атрибут HtmlTargetElement в tag-хелперах в представлениях ASP.NET Core MVC и C#" />
<p>Мы также можем определить набор атрибутов, которым должен соответствовать tag-хелпер:</p>
<pre class="brush:c#;">
...
<p>В этом случае элемент должен иметь сразу два атрибута: header и divtitle.</p>
<pre class="brush:xml;">
&lt;div header divtitle&gt;Hello World&lt;/div&gt;
&lt;div header divtitle&gt;Hello METANIT.COM&lt;/div&gt;
</pre>
<h3>Переопределение имени элемента</h3>
<p>Мы можем переопределить имя элемента, передав в атрибут HtmlTargetElement другое название элемента, которое отличается от имени tag-хелпера. Например:</p>
<pre class="brush:c#;">
...
<p>Данный tag-хелпер будет применяться к элементу "article-header":</p>
<pre class="brush:xml;">&lt;article-header&gt;Hello METANIT.COM&lt;/article-header&gt;</pre>
<h3>Установка родительского тега</h3>
<p>Через свойство <code>ParentTag</code> можно установить элемент, в котором должен использоваться наш tag-хелпер:</p>
<pre class="brush:c#;">
...
<p>В этом случае tag-хелпер будет применяться только к тем элементам header, который расположены внутри элемента <code>form</code>:</p>
<pre class="brush:xml;">
...
<p>В этом примере только второй элемент header будет обрабатываться tag-хелпером.</p>
<h3>Сочетание нескольких условий</h3>
<p>Мы можем определить сразу несколько параметров, чтобы конкретизировать диапазон действия tag-хелпера:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Razor.TagHelpers;
...
<p>В данном случае класс HeaderTagHelper будет применяться к элементу "form-header", который обязательно должен иметь атрибут "form-title" и который 
обязательно должен находиться внутри элемента "form". То есть в следующем случае будет обработан только третий элемент "form-header":</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./7.9.php">Назад</a><a href="./">Содержание</a><a href="./7.11.php">Вперед</a></p></div></div>
<div class="socBlock">
