...
<html >
<head>
<title>ASP.NET Core MVC и C# | Сортировка</title>
<meta charset="utf-8" />
<meta name="description" content="Сортировка данных через Entity Framework в ASP.NET Core MVC и C#,  метод OrderBy и OrderByDescending">
...
<h2>Сортировка данных</h2><div class="date">Последнее обновление: 11.04.2022</div>
...
<p>Рассмотрим, как добавить в приложение сортировку данных. Для работы возьмем новый проект ASP.NET Core по типу 
<span class="b">ASP.NET Core Empty</span>. Назовем его, к примеру, <span class="b">MvcApp</span>.</p>
<p>Далее добавим в проект в папку <span class="b">Models</span> следующие классы <span class="b">User</span> и <span class="b">Company</span>:</p>
<pre class="brush:c#;">
public class User
{
...
Далее добавим в папку <span class="b">Models</span> новый класс <span class="b">UsersContext</span>, через который будем взаимодействовать с бд:</p>
<pre class="brush:c#;">
...
<p>В файле <span class="b">Program.cs</span> установим контекст данных и подключение к базе данных:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
using MvcApp.Models;
...
<p>Итак, в классе User есть три свойства (не считая Id), по которым можно вести сортировку: Name, Age, Company. Сортировка может быть по возрастанию и по убыванию. 
...
Для описания всех этих критериев добавим в папку <span class="b">Models</span> перечисление <span class="b">SortState</span>:</p>
<pre class="brush:c#;">
...
<p>Это перечисление хранит критерии сортировки в виде числовых констант.</p>
<p>Теперь определим в проекте в папке <span class="b">Controllers</span> контроллера <span class="b">HomeController</span> со следующим кодом:</p>
<pre class="brush:c#;">
...
<p>Критерий сортировки передается в метод Index в виде параметра sortOrder. Мы ожидаем, что он будет равен одной из констант перечисления SortState. Если никакое значение не передается, 
по по умолчанию считаем, что используется сортировка по имени по возрастанию: <code>SortState sortOrder = SortState.NameAsc</code></p>
<p>Далее устанавливаем ряд переменных во ViewData, которые будут хранить значения для трех столбцов. Например, для свойства Name определено значение 
<code>ViewData["NameSort"]</code>. Если параметр sortOrder равен константе <code>SortState.NameAsc</code>, то ViewData["NameSort"] присваивается 
значение <code>SortState.NameDesc</code>. То есть если ранее значение ViewData["NameSort"] указывало на сортировку по имени по возрастанию, то теперь 
после сортировки оно будет указывать на сортировку по имени по убыванию. И наоборот.</p>
<p>Далее в методе в конструкции switch..case смотрит на значение sortOrder и производим сортировку с помощью метода <code>OrderBy/OrderByDescending</code>.</p>
<p>В конце передаем полученные данные в представление.</p>
<p>И определим код в представлении <span class="b">Index.cshtml</span>:</p>
<pre class="brush:xml;">
...
<p>В представлении каждый заголовок столбца будет представлять ссылку. То есть мы нажимаем на столбец, и отправляется запрос на сервер, который выполняет фильтрацию.</p>
<p>Каждая ссылка определена с помощью tag-хелпера:</p>
<pre class="brush:xml;">
&lt;a asp-action="Index" asp-route-sortOrder="@ViewBag.NameSort"&gt;Имя&lt;/a&gt;
</pre>
<p>Атрибут <code>asp-route-sortOrder</code> позволяет задать параметр sortOrder, который будет передавать на сервер выбранный критерий сортировки. И в результате 
будет создавать ссылка типа следующей:</p>
<pre class="brush:xml;">http://localhost:xxxxx/?sortOrder=NameAsc</pre>
<p>В итоге весь проект будет выглядеть так:</p>
<img src="./pics/11.7.png" alt="Проект ASP.NET Core MVC на C# для сортировки объектов в Entity Framework" />
<p>После запуска проекта с помощью нажатия на заголовки столбцов в таблице мы сможем отсортировать объекты по нужному столбцу:</p>
<img src="./pics/11.8.png" alt="Сортировка в ASP.NET Core MVC на C# в Entity Framework" />
