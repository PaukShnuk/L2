...
<html >
<head>
<title>ASP.NET Core MVC и C# | Контекст хелпера и получение зависимостей</title>
<meta charset="utf-8" />
<meta name="description" content="Контекст ViewContext и получение зависимостей в tag-хелпере в ASP.NET Core MVC и C#, получение данных запроса">
...
<h2>Контекст хелпера и получение зависимостей</h2><div class="date">Последнее обновление: 05.04.2022</div>
...
<p>Как и другие классы, tag-хелпер может получать зависимости, которые установлены в приложении с помощью механизма dependency injection.</p>
<p>Например, пусть у нас будут определены в проекте следующие сущности:</p>
<pre class="brush:c#;">
...
<p>Зарегистрируем сервис ITimeService в приложении в файле <span class="b">Program.cs</span>:</p>
<pre class="brush:c#;">
...
<p>Теперь мы можем получить сервис ITimeService в конструкторе класса тег-хелпера:</p>
<pre class="brush:c#;">
...
<h3>Контекст представления</h3>
<p>Кроме того, мы можем использовать получение зависимостей в виде свойств. Данная возможность предоставляется фреймворком MVC и 
доступна для ряда объектов, в том числе для объекта <span class="b">ViewContext</span>, который представляет контекст представления, в котором используется 
tag-хелпер.</p>
<p>Используем ViewContext, изменив код хелпера следующим образом:</p>
<pre class="brush:c#;">
...
<p>С помощью следующего свойства ViewContext мы автоматически получаем контекст представления, в котором вызывается хелпер:</p>
<pre class="brush:c#;">
[ViewContext]
[HtmlAttributeNotBound]
public ViewContext? ViewContext { get; set; }
</pre>
<p>Чтобы автоматически получить данную зависимость, к свойству применяется атрибут <code>[ViewContext]</code>.</p>
<p>Кроме того, здесь применяется атрибут <span class="b">HtmlAttributeNotBound</span>. Так как по умолчанию публичные свойства tag-хелпера автоматически наполняются 
через атрибуты элементов, то может возникнуть ситуация, что в представлении для данного элемента будет определен атрибут <code>view-context</code>, например:</p>
<pre class="brush:xml;">&lt;timer view-context="@(new ViewContext())" /&gt;</pre>
<p>Получив объект ViewContext, мы можем извлечь из него различную информацию. В частности, здесь получаем значение параметра font из 
строки запроса и это значение используем для установки шрифта элемента:</p>
<pre class="brush:c#;">
string? font = ViewContext?.HttpContext.Request.Query["font"];
if (string.IsNullOrEmpty(font)) font = "Verdana";
output.Attributes.SetAttribute("style", $"font-family:{font};font-size:18px;");
</pre>
<p>Далее остается определить тег-хелпер в представлении:</p>
<pre class="brush:xml;">&lt;timer /&gt;</pre>
<p>И при выполнении запроса тег-хелпер получит параметр font из стоки запроса (если он указан) и использует его для установки семейства шрифта:</p>
<img src="./pics/7.17.png" alt="ViewContext в Tag Helper в ASP.NET Core MVC и C#" />
...
<div class="nav"><p><a href="./7.8.php">Назад</a><a href="./">Содержание</a><a href="./7.10.php">Вперед</a></p></div></div>
<div class="socBlock">
