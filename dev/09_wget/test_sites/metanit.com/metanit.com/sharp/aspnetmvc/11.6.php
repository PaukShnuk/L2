...
<html >
<head>
<title>ASP.NET Core MVC и C# | Фильтрация</title>
<meta charset="utf-8" />
<meta name="description" content="Фильтрация данных через Entity Framework в ASP.NET Core MVC и C#">
...
<h2>Фильтрация</h2><div class="date">Последнее обновление: 12.04.2022</div>
...
<p>Рассмотрим, как фильтровать данные по определенным критериям в ASP.NET Core MVC. Для работы возьмем следующие модели <span class="b">User</span> и <span class="b">Company</span>:</p>
<pre class="brush:c#;">
public class User
{
...
А затем добавим в проект класс контекста данных <span class="b">UsersContext</span>:</p>
<pre class="brush:c#;">
...
<p>Допустим, при выводе списка пользователей нам надо также предусмотреть возможность для их фильтрации по некоторым критериям, например, по компании и 
...
добавим в проект специальную модель:</p>
<pre class="brush:c#;">
...
<p>Три свойства - для списка пользователей, компаний, введенного для поиска имени. Причем для компаний применяется объект SelectList.</p>
...
<p>Перед таблицей со списком пользователей, определена форма, которая с помощью запросов <span class="b">GET</span> посылает введенные значения на сервер.</p>
<p>Теперь на стороне контроллера определим метод, который будет выводить список пользователей и производить фильтрацию:</p>
<pre class="brush:c#;">
...
<p>При обращении к методу Index вне зависимости были ли переданы параметры name и company, все равно в представление будет выводиться список пользователей. 
Если определенный параметр был передан, то к объекту IQueryable, представляющего выборку, добавляется выражение Where: 
<code>users = users.Where(p =&gt; p.CompanyId == company);</code>.</p>
<p>Для формирования выпадающего списка компаний в представлении, получаем его из БД:</p>
<pre class="brush:c#;">List&lt;Company&gt; companies = db.Companies.ToList();</pre>
<p>Для возможности выбора пользователей из любой компании, добавляем в этот список новый пункт:</p>
<pre class="brush:c#;">companies.Insert(0, new Company { Name = "Все", Id = 0 });</pre>
<p>Все полученные данные используются для формирования объекта UserListViewModel, который передается в представление.</p>
<img src="./pics/11.11.png" alt="Фильтрация в Entity Framework в ASP.NET Core MVC и C#" />
