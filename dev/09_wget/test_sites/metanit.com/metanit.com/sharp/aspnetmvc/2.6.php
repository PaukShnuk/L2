...
<html >
<head>
<title>ASP.NET Core MVC и C# | ContentResult и JsonResult</title>
<meta charset="utf-8" />
<meta name="description" content="ContentResult и JsonResult и отправка ответа в контроллере в виде строки и в формате json в ASP.NET Core MVC и C#">
...
<h2>ContentResult и JsonResult</h2><div class="date">Последнее обновление: 21.03.2022</div>
...
<h3>ContentResult</h3>
<p>ContentResult отправляет клиенту ответ в виде строки. Так, следующий пример:</p>
<pre class="brush:c#;">
public string Index()
{
...
<p>Можно переписать с использованием ContentResult:</p>
<pre class="brush:c#;">
...
<p>Для отправки ContentResult не надо использовать конструктор, так как в контроллере уже определен специальный метод <span class="b">Content()</span>, 
который принимает отправляемую строку и создает объект ContentResult.</p>
<h3>JsonResult</h3>
<p>Одним из наиболее популярных в наше время форматов хранения и передачи данных является формат JSON (JavaScript Object Notation). 
JSON не зависит от языка программирования, он более удобен и легче обрабатывается.</p>
<p>В JSON каждый отдельный объект заключается в фигурные скобки и представляет собой набор пар ключ-значение, разделенных запятыми, где ключом является 
название свойства объекта, а значением соответственно значение этого свойства. Например: <code>{"name":"Tom"}</code>. Здесь "name" является ключом, а "Tom" - значением.</p>
<p>Для отправки объекта в формате json в контроллере имеется метод <span class="b">Json(object obj)</span>, который в качестве параметра принимает отправляемый 
объект. Например:</p>
<pre class="brush:c#;">
...
<p>В данном случае на сторону клиента отправляется строка "Tom".</p>
<p>Допустим, у нас есть следующий класс User:</p>
<pre class="brush:c#;">
record class Person(string Name, int Age);
</pre>
<p>И тогда для отправки клиенту объекта Person мы можем написать следующий метод:</p>
<pre class="brush:c#;">
public IActionResult Index()
{
...
<p>При обращении к методу веб-браузер выведет полное описание объекта в формате json:</p>
<img src="./pics/2.15.png" alt="Отправка JSON в контроллере ASP.NET Core MVC и C#" />
<p>Дополнительная версия метода <span class="b">Json()</span> в качестве второго параметра принимает объект, который задает настройки сериализации в формат json. 
В качестве такого объекта выступает объект типа JsonSerializerOptions:</p>
<pre class="brush:c#;">
...
<p>В данном случае объект JsonSerializerOptions задает с помощью свойств параметры сериализации в json. В частности, значение свойства <code>PropertyNameCaseInsensitive = true</code> 
говорит, что надо учитывать регистр названий свойств. А свойство <code>WriteIndented = true</code> задает установку отступов перед свойствами для более форматированного вывода:</p>
<img src="./pics/2.16.png" alt="Отправка и настройка сериализации JSON в контроллере ASP.NET Core MVC и C#" />
