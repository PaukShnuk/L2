...
<html >
<head>
<title>ASP.NET Core MVC и C# | Частичные представления</title>
<meta charset="utf-8" />
<meta name="description" content="Частичные представления Partial views ASP.NET Core MVC и C#, , класс PartialView и создание частичных представлений, методы Html.PartialAsync и Html.RenderPartialAsync">
...
<h2>Частичные представления</h2><div class="date">Последнее обновление: 23.03.2022</div>
...
<p>Кроме обычных представлений и мастер-страниц в ASP.NET Core MVC также можно использовать частичные представления или <span class="b">partial views</span>. 
...
похожи на секции, которые использовались в прошлой теме, только их код выносится в отдельные файлы.</p>
<p>Частичные представления полезны для создания различных панелей веб-страницы, например, панели меню, блока входа на сайт, каких-то других блоков.</p>
<p>За рендеринг частичных представлений отвечает объект <span class="b">PartialViewResult</span>, который возвращается методом <span class="b">PartialView()</span>. 
Этот метод имеет четыре версии:</p> 
<ul>
<li><p><code>PartialView()</code>: для генерации ответа используется представление, которое по имени совпадает с вызывающим методом</p></li>
<li><p><code>PartialView(string? viewName)</code>: в метод передается имя представления, что позволяет переопределить используемое по умолчанию представление</p></li>
<li><p><code>PartialView(object? model)</code>: передает в представление данные в виде объекта model</p></li>
<li><p><code>PartialView(string? viewName, object? model)</code>: переопределяет имя представления и передает в него данные в виде объекта model</p></li>
</ul>
<p>Итак, определим в контроллере HomeController действие Hello, которое будет использовать частичное представление:</p>
<pre class="brush:c#;">
...
<p>Теперь добавим в папку <span class="b">Views/Home</span> новое представление <span class="b">Hello.cshtml</span>, в котором будет простенькое содержимое:</p>
<pre class="brush:xml;">&lt;h2&gt;Hello View&lt;/h2&gt;</pre>
<img src="./pics/3.21.png" alt="Partial views в ASP.NET Core MVC и C#" />
<p>Теперь обратимся к методу Hello как к обычному действию контроллера, и оно нам вернет частичное представление:</p>
<img src="./pics/3.22.png" alt="Частичные представления в ASP.NET Core MVC и C#" />
<p>По своему действию частичное представление похоже на обычное, только для него по умолчанию не определяется мастер-страница.</p>
<h3>Встраивание частичного представления в обычное</h3>
<p>Теперь рассмотрим, как мы можем встраивать частичные представления в обычные. Для этого изменим представление Index.cshtml:</p>
<pre class="brush:xml;">
&lt;h1&gt;Index View&lt;/h1&gt;

@await Html.PartialAsync("Hello")
</pre>
<p>Метод <code>Html.PartialAsync()</code> встраивает код частичного представления в обычное. Он является асинхронным и возвращает объект 
<span class="b">IHtmlContent</span>, который представляет html-содержимое и который обернут в объект <code>Task&lt;TResult&gt;</code>. В качестве параметра в метод передается имя представления:</p>
<img src="./pics/3.23.png" alt="Html.PartialAsync в ASP.NET Core MVC и C#" />
<p>Обращения к методу <code>Hello()</code> в контроллере при этом не происходит.</p>
<p>Кроме метода <code>Html.PartialAsync()</code> частичное представление можно встроить с помощью другого метода - <span class="b">Html.RenderPartialAsync</span>. 
Этот метод также принимает имя представления, только он используется не в строчных выражениях кода Razor, а в блоке кода, то есть обрамляется фигурными скобками:</p>
<pre class="brush:c#;">@{await Html.RenderPartialAsync("Hello");}</pre>
<p><code>Html.RenderPartialAsync</code> напрямую пишет вывод в выходной поток в асинхронном режиме, поэтому может работать чуть быстрее, чем 
<code>Html.PartialAsync</code>.</p>
<h3>Передача данных в частичное представление</h3>
<p>Одна из перегруженных версий методов <code>Html.PartialAsync / Html.RenderPartialAsync</code> позволяет передать модель в частичное представление. В итоге у нас получится стандартное строго типизированное представление. 
Например, в качестве второго параметра список строк:</p>
<pre class="brush:c#;">
&lt;h1&gt;Welcome&lt;/h1&gt;

@await Html.PartialAsync("Hello", new List&lt;string&gt;{"Hello", "Привет", "Hallo", "Salut"})
</pre>
<p>Поскольку здесь в частичное представление передается список строк, то мы можем использовать модель List&lt;string&gt;, чтобы получить этот список. 
Теперь изменим частичное представление <span class="b">Hello.cshtml</span>:</p>
<pre class="brush:xml;">
...
<p>И на веб-страницу будет передан список строк:</p>
<img src="./pics/3.24.png" alt="Html.PartialAsync и передача данных в частичное представление ASP.NET Core MVC и C#" />
