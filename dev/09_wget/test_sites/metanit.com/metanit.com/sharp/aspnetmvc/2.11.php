...
<html >
<head>
<title>ASP.NET Core MVC и C# | Переопределение контроллеров</title>
<meta charset="utf-8" />
<meta name="description" content="Переопределение контроллеров в ASP.NET Core MVC и C#, базовый класс Controller и его методы, OnActionExecuting и OnActionExecuted">
...
<h2>Переопределение контроллеров</h2><div class="date">Последнее обновление: 21.03.2022</div>
...
<p>Его методы:</p>
<ul>
<li><p>Метод <span class="b">OnActionExecuting()</span> выполняется при вызове метода контроллера до его непосредственного выполнения. В качестве параметра принимает 
объект типа <span class="b">ActionExecutingContext</span>, который имеет следующие свойства:</p>
<ul>
<li><p><span class="b">ActionArguments</span>: объект типа <code>IDictionary&lt;string,object?&gt;</code>, который хранит передаваемые в метод аргументы и их значения (ключи в словаре представляют названия параметров метода)</p></li>
<li><p><span class="b">ActionDescriptor</span>: объект типа <code>ActionDescriptor</code>, который предоставляет информацию о выполняемом действии контроллера</p></li>
<li><p><span class="b">Controller</span>: предоставляет информацию о контроллере</p></li>
<li><p><span class="b">Filters</span>: представляет коллекцию фильтров, применяемых к действию контроллера</p></li>
<li><p><span class="b">HttpContext</span>: контекст запроса</p></li>
<li><p><span class="b">ModelState</span>: объект типа <code>ModelStateDictionary</code>, который хранит состояние модели</p></li>
<li><p><span class="b">Result</span>: объект типа <code>IActionResult</code>, который позволяет получить или установить результат действия контроллера</p></li>
<li><p><span class="b">RouteData</span>: представляет данные маршрута</p></li>
</ul>
</li>
<li><p>Метод <span class="b">OnActionExecuted()</span> выполняется после выполнения метода контроллера. В качестве параметра принимает 
объект типа <span class="b">ActionExecutedContext</span>, который имеет следующие свойства:</p>
<ul>
<li><p><span class="b">ActionDescriptor</span>: объект типа <code>ActionDescriptor</code>, который предоставляет информацию о выполняемом действии контроллера</p></li>
<li><p><span class="b">Canceled</span>: прерывает выполнение действия и фильтров действия</p></li>
<li><p><span class="b">Controller</span>: предоставляет информацию о контроллере</p></li>
<li><p><span class="b">Exception</span>: получает или устанавливает исключение</p></li>
<li><p><span class="b">ExceptionDispatchInfo</span>: получает или устанавливает объект ExceptionDispatchInfo с информацией об исключении</p></li>
<li><p><span class="b">ExceptionHandled</span>: указывает, обработано ли исключение</p></li>
<li><p><span class="b">Filters</span>: представляет коллекцию фильтров, применяемых к действию контроллера</p></li>
<li><p><span class="b">HttpContext</span>: контекст запроса</p></li>
<li><p><span class="b">ModelState</span>: объект типа <code>ModelStateDictionary</code>, который хранит состояние модели</p></li>
<li><p><span class="b">Result</span>: объект типа <code>IActionResult</code>, который позволяет получить или установить результат действия контроллера</p></li>
...
<li><p>Метод <span class="b">OnActionExecutionAsync()</span> представляет асинхронную версию метода OnActionExecuting().</p></li>
</ul>
<p>Возьмем простейшую задачу. Например, мы хотим вывести на консоль приложения имя контроллера и метода, которые обрабатывают запрос.</p>
...
<p>В данном случае переопределяется метод <code>OnActionExecuting()</code>. 
...
<p>В данном случае вначале получаем контроллер и выводим его имя на консоль приложения:</p>
<pre class="brush:c#;">Console.WriteLine($"Controller: {context.Controller.GetType().Name}");</pre>
<p>Свойство <code>context.Controller</code> позволяет получить информацию о текущем контроллере.</p>
<p>Далее получаем информацию о действии, которое обрабатывает запрос:</p>
<pre class="brush:c#;">Console.WriteLine($"Action: {context.ActionDescriptor.DisplayName}");</pre>
<p>Свойство <code>context.ActionDescriptor</code> представляет действие контроллера, в частности, его свойство <code>DisplayName</code> позволяет получить имя действия с учетом 
пространств имен и класса контроллера.</p>
...
<p>И теперь до начала работы метода Index будет срабатывать метод OnActionExecuting(), определенный в базовом классе LogBaseController, который выведет на консоль имя контроллера и его действия:</p>
<img src="./pics/2.21.png" alt="Переопределение контроллеров в ASP.NET Core MVC и C#" />
