...
<html >
<head>
<title>ASP.NET Core MVC и C# | Шаблонные хелперы</title>
<meta charset="utf-8" />
<meta name="description" content="Шаблонные хелперы в ASP.NET Core MVC и C#, Html.Display и Html.DisplayFor, Html.Editor и Html.EditorFor">
...
<h2>Шаблонные хелперы</h2><div class="date">Последнее обновление: 29.03.2022</div>
...
<p>Кроме html-хелперов форм, которые создают различные элементы формы, фреймворк ASP.NET Core MVC также имеет <span class="b">шаблонные (или шаблонизированные) хелперы</span>. В отличие от рассмотренных в прошлой главе html-хелперов они не генерируют 
...
исходя из его типа и метаданных.</p>
<p>В ASP.NET MVC имеются следующие шаблонные хелперы:</p>
<ul>
<li><p><span class="b">Display</span></p>
<p>Создает элемент разметки для отображения значения указанного свойства модели: 
<code>Html.Display("Name")</code></p></li>
<li><p><span class="b">DisplayFor</span></p>
<p>Строго типизированный аналог хелпера Display: 
<code>Html.DisplayFor(m =&gt; m.Name)</code></p></li>
<li><p><span class="b">Editor</span></p>
<p>Создает элемент разметки для редактирования указанного свойства модели: 
<code>Html.Editor("Name")</code></p></li>
<li><p><span class="b">EditorFor</span></p>
<p>Строго типизированный аналог хелпера Editor: 
<code>Html.EditorFor(m =&gt; m.Name)</code></p></li>
<li><p><span class="b">DisplayText</span></p>
<p>Создает выражение для указанного свойства модели в виде простой строки: 
<code>Html.DisplayText("Name")</code></p></li>
<li><p><span class="b">DisplayTextFor</span></p>
<p>Строго типизированный аналог хелпера DisplayText: 
<code>Html.DisplayTextFor(m =&gt; m.Name)</code></p></li>
</ul>
<p>Это были одиночные хелперы, которые генерируют разметку только для одного свойства модели. Но кроме них во фреймворке также 
есть еще несколько шаблонов, которые позволяют создать разом все поля для всех свойств модели:</p>
<ul>
	<li><p><span class="b">DisplayForModel</span></p>
	<p>Создает поля для чтения для всех свойств модели: 
	<code>Html.DisplayForModel()</code></p></li>
	<li><p><span class="b">DisplayTextForModel</span></p>
	<p>Создает поля для чтения для всех свойств модели в виде строки</p></li>
	<li><p><span class="b">EditorForModel</span></p>
	<p>Создает поля для редактирования для всех свойств модели: 
	<code>Html.EditorForModel()</code></p></li>
</ul>
<h3>Html.Display</h3>
<p>Например, пусть у нас есть стандартная модель User:</p>
<pre class="brush:c#;">
public record class User(int Id, string Name, int Age);
</pre>
<p>Пусть в методе контроллера передается объект этой модели в представление:</p>
<pre class="brush:c#;">
...
<p>И для вывода данных используем в представлении хелпер <code>Html.Display</code>:</p>
<pre class="brush:xml;">
...
<img src="./pics/6.6.png" alt="вывод данных модели и Html.Display в ASP.NET Core MVC и C#" />
<p>Аналогичный вывод мы могли бы сделать, применив строго типизированную версию хелпера:</p>
<pre class="brush:xml;">
...
<p>Либо можно было бы вывести все данные разом:</p>
<pre class="brush:c#;">@Html.DisplayForModel()</pre>
<h3>Html.Editor</h3>
<p>Html.Editor работает во многом аналогично. Допустим, у нас есть действие, которое принимает объект модели User:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>Тогда мы могли бы определить следующую форму:</p>
<pre class="brush:xml;">
@model MvcApp.Models.User
...
<img src="./pics/6.7.png" alt="редактирование модели и метод Html.EditorFor в ASP.NET Core MVC и C#" />
<p>Что важно, хелпер <code>Html.Editor</code> смотрит на тип свойства и на основании типа генерирует то или иное поле. Например, для свойства Age 
будет сгенерировано поле <code>&lt;input type="number" /&gt;</code>. И если в прошлой теме нам явным образом приходилось указывать тип для Html.TextBox:</p>
<pre class="brush:c#;">@Html.TextBoxFor(m=&gt;m.Age, "", new { type = "number" })</pre>
<p>Здесь это хелпер сделает автоматически.</p>
