...
<html >
<head>
<title>ASP.NET Core MVC и C# | Добавление MVC в пустой проект</title>
<meta charset="utf-8" />
<meta name="description" content="Создание первого проекта с помощью фреймворка ASP.NET Core MVC на языке программирования C#">
...
<h2>Первый проект на ASP.NET Core MVC</h2><div class="date">Последнее обновление: 16.03.2022</div>
...
<p>В прошлой теме был создан первый проект ASP.NET Core, который по умолчанию применяет паттерн MVC, но осталось за кадром, что именно заставляет паттерн MVC работать в проекте на ASP.NET Core. 
Рассмотрим этот момент и чтобы лучше это понять, возьмем стандартный пустой проект по типу <span class="b">ASP.NET Core Empty</span>, который по умолчанию никакой функциональности MVC в проект не подключает.</p>
<img src="./pics/1.6.png" alt="Подключение MVC в пустой проект ASP.NET Core" />
<p>Допустим, в моем случае проект называется <span class="b">MvcApp</span>. По умолчанию создается примитивный проект ASP.NET Core, в котором файл <span class="b">Program.cs</span> стандартный код:</p>
<pre class="brush:c#;">
...
<p>Теперь задействуем функциональность фреймворка MVC в нашем приложении и изменим файл <span class="b">Program.cs</span> следующим образом:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder(args);
...
<p>Прежде всего надо отметить, что функциональность MVC, в частности, поддержка контроллеров и представлений, подключается в приложение в виде сервиса - в данном случае 
с помощью вызова <code>services.AddControllersWithViews()</code>. После этого мы можем использовать функциональность фреймворка MVC.</p>
<p>Кроме того, чтобы связать приходящие от пользователей запросы с контроллерами применяется метод <code>MapControllerRoute()</code>. 
...
его метода - метод Index.</p>
<h3>Добавление контроллера</h3>
<p>Теперь добавим в проект для хранения контроллеров папку <span class="b">Controllers</span>. И затем в нее добавим новый класс, который назовем HomeController и который 
будет иметь следующий код:</p>
<pre class="brush:c#;">
...
<p>Контроллер наследуется от класса <span class="b">Controller</span> и в данном случае он имеет один метод Index, который с помощью метода 
View обращается к представлению.</p>
<h3>Добавление представления</h3>
<p>Теперь добавим в проект папку <span class="b">Views</span>, которая будет хранить представления. В эту папку вначале добавим каталог <span class="b">Home</span> - каталог, предназначенный непосредственно для представлений 
контроллера Home. И в конце добавим в каталог <span class="b">Views/Home</span> новый элемент по типу <span class="b">Razor View (Empty)</span>, который назовем <span class="b">Index.cshtml</span>.</p>
<img src="./pics/1.7.png" alt="Добавление представление в проект Mvc в ASP NET Core" />
...
<p>Это простейшее представление, которое будет использоваться методом Index контроллера HomeController. То есть в итоге у нас получится следующая структра проекта:</p>
<img src="./pics/1.8.png" alt="Первый проект c ASP.NET Core MVC в C#" />
<p>Теперь запустим приложение на выполнение, и браузер выведет сообщение, определенное в представлении Index.cshtml:</p>
<img src="./pics/1.9.png" alt="запуск проекта на ASP.NET Core MVC и C#" />
<p>Таким образом мы можем добавлять функциональность MVC в свой проект. Однако, как правило, для работы с MVC используется шаблон ASP.NET Core Web App (Model-View-Controller), который по умолчанию содержит некоторую 
базовую функциональность.</p>
<h3>Сервисы MVC</h3>
<p>Функциональность MVC и ее работа в приложении зависит от добавляемых сервисов. В примере выше мы использовали метод <code>AddMvc()</code> для добавления сервисов MVC, 
...
которые мы можем при необходимости использовать:</p>
<ul>
<li><p><span class="b">AddMvc()</span>: добавляет все сервисы фреймворка MVC (в том числе сервисы для работы с аутентификацией и авторизацией, валидацией и т.д.)</p></li>
<li><p><span class="b">AddMvcCore()</span>: добавляет только основные сервисы фреймворка MVC, а всю допалнительную функциональность, типа 
аутентификацией и авторизацией, валидацией и т.д., необходимо добавлять самостоятельно</p></li>
<li><p><span class="b">AddControllersWithViews()</span>: добавляет только те сервисы фреймворка MVC, которые позволяют использовать контроллеры и представления и связанную функциональность. 
При создании проекта по типу <span class="b">ASP.NET Core Web App (Model-View-Controller)</span> используется именно этот метод</p></li>
<li><p><span class="b">AddControllers()</span>: позволяет использовать контроллеры, но без представлений.</p></li>
</ul>
<p>И в зависимости от того, насколько широко нам надо использовать возможности фреймворка MVC, выбирается соответствующий метод. Например, в примере выше мы могли бы использовать 
вместо вызова <code>AddControllersWithViews()</code> метод <code>AddMvc()</code>.</p>
