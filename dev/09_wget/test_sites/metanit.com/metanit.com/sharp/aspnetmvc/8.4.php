...
<html >
<head>
<title>ASP.NET Core MVC и C# | ViewComponentResult и генерация представления</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с представлениями в View Component в ASP.NET Core MVC и C#, использование класса ViewComponentResult">
...
<h2>ViewComponentResult и генерация представления</h2><div class="date">Последнее обновление: 05.04.2022</div>
...
<p>ViewComponentResult позволяет использовать для рендеринга контента частичное представление. Если мы наследуем класс компонента от базового класса 
ViewComponent, то для генерации объекта ViewComponentResult мы можем вызвать унаследованный метод View.</p>
<p>Этот метод имеет четыре перегруженных версии:</p>
<ul>
<li><p><code>View()</code>: для рендеринга контента выбирает представление по умолчанию</p></li>
<li><p><code>View(model)</code>: для рендеринга контента выбирает представление по умолчанию, в которое передает некоторую модель</p></li>
<li><p><code>View(viewName)</code>: для рендеринга контента выбирает представление с именем viewName</p></li>
<li><p><code>View(viewName, model)</code>: для рендеринга контента выбирает представление с именем viewName, в которое передает некоторую модель</p></li>
</ul>
<p>Так, определим следующий класс компонента <span class="b">UsersListViewComponent</span>:</p>
<pre class="brush:c#;">
...
<p>В этом классе определяется условный список пользователей - коллекция строк.</p>
<p>Метод <code>Invoke()</code> возвращает объект <code>IViewComponentResult</code> или фактически представление, которое мы далее создадим. Причем в это предствление передается 
созданный в конструкторе список.</p>
<p>Теперь создадим представление, которое будет выводить переданные из компонента данные. 
...
<span class="b">Default.cshtml</span>. Для поиска файла представления Razor будет просматривать следующие пути в порядке приоритета:</p>
<ul>
<li><p><code>Views/Название_Контроллера/Components/Название_Компонента/Название_Представления.cshtml</code></p></li>
<li><p><code>Views/Shared/Components/Название_Компонента/Название_Представления.cshtml</code></p></li>
</ul>
<p>Допустим, в проекте у нас есть папка <span class="b">Controllers</span>, в которой помещен контроллер <span class="b">HomeController</span>. А в папке <span class="b">Views/Home</span> 
расположено представление <span class="b">Index.cshtml</span> для этого контроллера</p>
<img src="./pics/8.9.png" alt="View Component и представления в ASP.NET Core MVC и C#" />
<p>И, допустим, наш UsersListViewComponent будет использоваться только в представлениях 
этого контроллера HomeController. В этом случае добавим в папку <span class="b">Views/Home</span> новый каталог, который назовем <span class="b">Components</span>. 
А в этот каталог добавим папку, которая называется по имени нашего компонента - <span class="b">UsersList</span>, и определим в ней новое 
представление <span class="b">Default.cshtml</span> со следующим кодом:</p>
<pre class="brush:xml;">
...
<p>Поскольку в представление передается объект List&lt;string&gt;, то оно типизировано типом IEnumerable&lt;string&gt;. В самом представлении 
...
Razor, конструкции и директивы, что и обычное представление.</p>
<p>То есть в итоге представление, которое используется компонентом, должно лежать по адресу <span class="ii">Views/Название_Контроллера/Components/Название_Компонента/Default.cshtml</span>:</p>
<img src="./pics/8.10.png" alt="View Component и генерация представлений в ASP.NET Core MVC и C#" />
<p>Теперь мы можем использовать компонент. Для этого определим в представлении <span class="b">Index.cshtml</span> следующие строки:</p>
<pre class="brush:xml;">
@addTagHelper *, MvcApp
...
<p>Результат работы View Component:</p>
<img src="./pics/8.11.png" alt="Возвращение представления из View Component в ASP.NET Core MVC и C#" />
<p>Выше использовалось имя представления по умолчанию: <span class="b">Default.cshtml</span>. Однако нам необязательно применять именно это имя. 
Если у нас есть другое представление, например, <span class="b">Users.cshtml</span>, то мы можем его использовать следующим образом:</p>
<pre class="brush:c#;">
public IViewComponentResult Invoke()
{
...
<p>Имя представления без расширения <span class="ii">cshtml</span> передается в метод View в качестве первого параметра. Само представление также должно находиться 
по адресу <span class="ii">Views/Название_Контроллера/Components/Название_Компонента/</span></p>
