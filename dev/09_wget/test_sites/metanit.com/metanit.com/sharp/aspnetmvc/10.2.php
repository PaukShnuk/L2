...
<html >
<head>
<title>ASP.NET Core MVC и C# | Область действия фильтров</title>
<meta charset="utf-8" />
<meta name="description" content="Область действия фильтров в ASP.NET Core MVC и C#, фильтры уровня контроллеров и методов, глобальные фильтры, интерфейс IOrderFilter, порядок выполнения">
...
<h2>Область действия фильтров</h2><div class="date">Последнее обновление: 09.04.2022</div>
...
<p>В ASP.NET Core MVC фильтры имеют три области действия:</p>
<ul>
<li><p><span class="b">Метод контроллера</span>. Атрибут применяется к методу контроллера:</p>
<pre class="brush:c#;">
...
<li><p><span class="b">Область действия контроллер</span>.</p>
<p>Атрибут применяется к классу контроллера:</p>
<pre class="brush:c#;">
[SimpleResourceFilter]
public class HomeController : Controller
{
...
<li><p><span class="b">Глобальная область действия</span>, при которой фильтр применяется ко всем методам всех контроллеров приложения.</p>
<p>Для определения фильтра как глобального надо при подключении соответствующих сервисов MVC добавить фильтр в коллекцию фильтров. 
Подключение глобально для всех сервисов MVC:</p>
<pre class="brush:c#;">
...
<p>Установка отдельно для контроллеров и представлений:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder(args);
...
<p>Кроме того, следует отметить, что есть определенная очередность выполнения фильтров в зависимости от области действия:</p>
<table class="tab">
<tr class="tabhead"><td><p>Очередность</p></td><td><p>Область действия</p></td><td><p>Метод фильтра</p></td></tr>
<tr><td><p>1</p></td><td><p>Глобальная</p></td><td><p>On[Stage]Executing</p></td></tr>
<tr><td><p>2</p></td><td><p>Контроллер</p></td><td><p>On[Stage]Executing</p></td></tr>
<tr><td><p>3</p></td><td><p>Действие контроллера</p></td><td><p>On[Stage]Executing</p></td></tr>
<tr><td><p>4</p></td><td><p>Действие контроллера</p></td><td><p>On[Stage]Executed</p></td></tr>
<tr><td><p>5</p></td><td><p>Контроллер</p></td><td><p>On[Stage]Executed</p></td></tr>
<tr><td><p>6</p></td><td><p>Глобальная</p></td><td><p>On[Stage]Executed</p></td></tr>
</table>
<p>Например, создадим в проекте папку <span class="b">Filters</span> и определим в ней три класса фильтров - <span class="b">GlobalResourceFilter</span>, <span class="b">ControllerResourceFilter</span> и <span class="b">ActionResourceFilter</span>:</p>

<img src="./pics/10.4.png" alt="Область действия фильтров в ASP.NET Core MVC и C#" />
<p>Класс <span class="b">GlobalResourceFilter</span> будет выполнять роль глобального фильтра и будет иметь следующий код:</p>
<pre class="brush:c#;">
...
<p>Класс <span class="b">ControllerResourceFilter</span> будет фильтром на уровне контроллера и будет иметь следующий код:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc.Filters;
...
<p>Класс <span class="b">ActionResourceFilter</span> будет выполнять роль фильтра на уровне отдельного действия контроллера и будет иметь следующий код:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc.Filters;
...
<p>Изменим файл <span class="b">Program.cs</span> для подключения классаGlobalResourceFilter в качестве глобального фильтра:</p>
<pre class="brush:c#;">
...
<p>И определим следующий контроллер HomeController:</p>
<pre class="brush:c#;">
...
<p>И после запуска приложения на консоли мы сможем наблюдать очередность выполнения фильтров:</p>
<img src="./pics/10.5.png" alt="Глобальные фильтры в ASP.NET Core MVC и C#" />
<h3>Переопределение порядка фильтров</h3>
<p>ASP.NET Core MVC позволяет переопределить порядок выполнения фильтров. Для этого фильтр должен реализовать интерфейс <span class="b">IOrderedFilter</span>, который определяет свойство <span class="b">Order</span> - это свойство 
устанавливает порядок выполнения фильтра - чем оно меньше, тем раньше будет выполняться фильтр. Посмотрим на примере. Изменим класс <span class="b">ControllerResourceFilter</span> следующим образом:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc.Filters;
...
<p>Теперь класс ControllerResourceFilter через конструктор устанавливает свойство Order.</p>
<p>И также изменим код контроллера:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc;
using MvcApp.Filters;  // пространство имен фильтров
...
<p>Значение <code>int.MinValue</code>, которое передается через конструктор класса ControllerResourceFilter свойству Order, фактически означает, что этот фильтр будет запускаться раньше всех. В чем мы можем 
убедиться, запустив проект:</p>
<img src="./pics/10.6.png" alt="Поряд выполнения фильтров в ASP.NET Core MVC и C#" />
