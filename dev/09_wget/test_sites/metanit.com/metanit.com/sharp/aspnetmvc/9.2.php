...
<html >
<head>
<title>ASP.NET Core MVC и C# | Валидация на стороне клиента</title>
<meta charset="utf-8" />
<meta name="description" content="Валидация модели на стороне клиента в ASP.NET Core MVC и C#, скрипты и атрибуты валидации, jquery.validation.unobtrusive">
...
<h2>Валидация на стороне клиента</h2><div class="date">Последнее обновление: 07.04.2022</div>
...
<p>Кроме валидации на стороне сервера с помощью свойства ModelState в ASP.NET Core MVC можно применять валидацию на стороне клиента, то есть на веб-странице. 
Валидация на стороне клиента позволяет уменьшить количество обращений к серверу и произвести все действия по проверке значений непосредственно при вводе данных.</p>
...
<p>Прежде всего это скрипт jquery, от которого зависят другие скрипты. Второй скрипт - плагин валидации Jquery Validate. Третий скрипт - библиотека jQuery Unobtrusive Validation дополняет 
скрипт валидации, добавляя поддержку data-атрибутов. Актуальная версия всех этих скриптов в будущем может может отличаться от приведенной.</p>
<p>Для рассмотрения валидации на стороне клиента возьмем самый простейший проект ASP.NET Core по типу по типу <span class="b">ASP.NET Core Empty</span>. Пусть весь проект будет выглядеть следующим образом:</p>
<img src="./pics/9.1.png" alt="Основы валидации в ASP.NET Core MVC и C#" />
<p>В папке <span class="b">Models</span> есть следующий класс <span class="b">Person</span>:</p>
<pre class="brush:c#;">
...
<p>Здесь каждое свойство модели помечено атрибутом <span class="b">Required</span>, которое требует обязательного наличия значений.</p>
<p>Также пусть в проекте имеется папка <span class="b">Controllers</span> с контроллером <span class="b">HomeController</span> со следующим кодом:</p>
<pre class="brush:c#;">
...
<p>Post-метод Create получает введенные данные в виде модели Person и проверяет их корректность и в зависимости от результата выводит определенные данные.</p>
<p>И для метода Create в папке <span class="b">Home/Create</span> есть представление <span class="b">Create.cshtml</span>, которое будет содержать форму для ввода данных:</p>
<pre class="brush:xml;">
...
<p>Данное представление является строго типизированным - в качестве модели оно использует класс Person. А в самом конце файла подключаются все необходимые скрипты из CDN. Но естественно скрипты можно загрузить локально в проект и подключать их из проекта.</p>
<p>В самом представлении определена форма ввода данных, для создания которой применяются tag-хелперы. Для отображения ошибок валидации в ASP.NET Core MVC применяются tag-хелперы валидации. При каждом свойстве мы используем хелпер валидации 
<span class="b">&lt;span asp-validation-for="[Название свойства]" /&gt;</span>. 
<p>Например, tag-хелпер</p>
<pre class="brush:xml;">&lt;span asp-validation-for="Name" /&gt;</pre>
<p>как раз представляет хелпер валидации или ValidationMessageTagHelper (а не элемент &lt;span&gt;). 
Атрибуту <span class="b">asp-validation-for</span> передается название свойства (в данном случае свойства Name), для которого будет выводиться сообщение, если свойство не проходит валидацию.</p>

<p>Например, возьмем группу tag-хелперов, которые предназначены для свойства Name:</p>
<pre class="brush:xml;">
&lt;label asp-for="Name"&gt;Name&lt;/label&gt;&lt;br /&gt;
&lt;input type="text" asp-for="Name" /&gt;
&lt;span asp-validation-for="Name" /&gt;
</pre>
<p>Поскольку к свойству Name применяется атрибут <span class="b">[Required]</span>, который требует обязательного наличия значения, то в итоге эта группа хелперов 
сгенерирует следующую разметку:</p>
<pre class="brush:xml;">
...
<p>Если мы запустим проект, ничего не введем в форму ввода и сразу нажмем на кнопку отправки, то данные даже не отправяттся на сервер, а рядом с полями ввода мы увидим 
сообщения об ошибках:</p>
<img src="./pics/9.6.png" alt="Валидация на стороне клиента в представлениях в ASP.NET Core MVC и C#" />
<p>Таким образом, валидация на стороне клиента позволит уменьшить нагрузку на сервер. Тем не менее стоит учитывать, что на стороне клиента по тем или иным причинам может быть отключен javascript, 
...
проверки, которые не могут быть выполнены на стороне клиента.</p>
<p>Стоит отметить, что при валидации нередко данные с ошибками возвращаются обратно клиенту. В этом случае мы можем передавать некорректную модель обратно в представление:</p>
<pre class="brush:c#;">
public IActionResult Create() =&gt; View();
