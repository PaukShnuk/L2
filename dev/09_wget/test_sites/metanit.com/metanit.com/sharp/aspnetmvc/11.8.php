...
<html >
<head>
<title>ASP.NET Core MVC и C# | Tag-хелпер для постраничной навигации</title>
<meta charset="utf-8" />
<meta name="description" content="Создание tag-хелпера для постраничной навигации через Entity Framework в ASP.NET Core MVC и C#">
...
<h2>Tag-хелпер для постраничной навигации</h2><div class="date">Последнее обновление: 12.04.2022</div>
...
<p>В прошлой теме был рассмотрен постраничный вывод. Для создания ссылок применялись встроенные хелперы:</p>
<pre class="brush:xml;">
...
<p>Но нередко для создания постраничной навигации создаются какие-то специальные панельки, где можно увидеть ссылки с номерами страниц, по которым 
можно перейти к нужным страницы, еще какие-то элементы. Все эти элементы нередко должным образом стилизованы.</p>
<p>Нередко элементы навигации - ссылки размещаются в виде ненумерованного списка. В данном случае наша задача - создать компонент навигации наподобие следующего:</p>
<pre class="brush:xml;">
&lt;ul class="pagination"&gt;
...
<p>Для стилизации применяется класс bootstrap - pagination. Перед активной ссылкой располагаются ссылки на предыдущую и следующую страницу.</p>
<p>Вместо того, чтобы определять все необходимые ссылки и элементы напрямую в представлении, удобнее создать специальный tag-хелпер. 
Поэтому возьмем проект из прошлой темы и добавим в него новую папку <span class="b">TagHelpers</span>, а в эту папку добавим новый класс <span class="b">PageLinkTagHelper</span>:</p>
<pre class="brush:c#;">
...
<p>Фреймворк MVC предоставляет ряд сервисов, и один из них - IUrlHelperFactory, который используется для создания ссылки и который мы можем получить в конструкторе.</p>
<p>Всю информацию о пагинации мы получаем через свойство <code>PageModel</code>. Свойство <code>PageAction</code> указывает на метод контроллера, на который будет создаваться 
ссылка.</p>
<p>Для создания ссылки используется объект IUrlHelper, а для его получения нам нужен контекст представления, в котором вызывается tag-хелпер. Получить контекст представления 
мы можем через внедрение зависимости через атрибуты. В частности, чтобы получить контекст представления над свойством ставится атрибут <span class="b">ViewContext</span>:</p>
<pre class="brush:c#;">
[ViewContext]
[HtmlAttributeNotBound]
public ViewContext ViewContext { get; set; } = null!;
</pre>
<p>Чтобы избежать привязки к атрибутам тега, к свойству также применяется атрибут <code>HtmlAttributeNotBound</code>.</p>
<p>В методе Process вначале получаем объект IUrlHelper для создания ссылки:</p>
<pre class="brush:c#;">
IUrlHelper urlHelper = urlHelperFactory.GetUrlHelper(ViewContext);
</pre>
<p>Далее создаем html-элемент <span class="b">ul</span>. Затем нам надо создать максимум три ссылки, если позволяет количество страниц. Так как каждый элемент 
списка ul создается одним и тем же способом, то весь механизм создания элемента списка с ссылкой вынесен в отдельный метод <code>CreateTag()</code>.</p>
<p>Для стилизации ссылок применяются классы pagination, page-item и page-link фреймворка bootstrap, который далее подключим в представлении.</p>
<p>Применим этот хелпер в представлении:</p>
<pre class="brush:xml;">
@using MvcApp.Models
...
<p>Так как название хелпера состоит из нескольких частей: PageLinkTagHelper, то при использовании все эти части разделяются дефисом 
(суффикс TagHelper отбрасывается): <code>page-link</code>. То же самое касается и свойств хелпера. Так, чтобы передать значение для свойства PageModel, 
нам надо использовать атрибут <code>page-model</code>.</p>
<p>Код контроллера остается тем же, что и в прошлой теме:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc;
...
<p>Итоговый проект будет выглядеть следующим образом:</p>
<img src="./pics/11.14.png" alt="Pagination in ASP.NET Core MVC and C#" />
<p>И при запуске проекта мы увидим набор стилизованных ссылок, по котором сможем перемещаться по страницам:</p>
<img src="./pics/11.15.png" alt="Постраничная навигация в ASP.NET Core MVC и C#" />
...
<div class="nav"><p><a href="./11.7.php">Назад</a><a href="./">Содержание</a><a href="./11.9.php">Вперед</a></p></div></div>
<div class="socBlock">
