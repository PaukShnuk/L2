...
<html >
<head>
<title>ASP.NET Core MVC и C# | URL-хелперы</title>
<meta charset="utf-8" />
<meta name="description" content="URL-хелперы Url.Action, Url.RouteUrl, Url.Content и генерация адресов в ASP.NET Core MVC и C#">
...
<h2>URL-хелперы</h2><div class="date">Последнее обновление: 29.03.2022</div>
...
<li><p><code>Content</code></p></li>
<li><p><code>RouteUrl</code></p></li>
</ul>
<p>Хелпер <span class="b">Url.Action</span> похож на Html.ActionLink за тем исключением, что но не возвращает тег якоря. Например, следующий код отображает 
адрес URL, но не саму ссылку:</p>
<pre class="brush:c#;">@Url.Action("Details", "Home", new { name = "Тоm", id = 10 }, null)</pre>
<p>Этот путь затем можно применить при формировании ссылки:</p>
<pre class="brush:c#;">&lt;a href='@Url.Action("Details", "Home", new { name = "Тоm", id = 10 }, null)'&gt;Ссылка&lt;/a&gt;</pre>
...
<pre class="brush:c#;">@Url.RouteUrl(new { controller = "Home", action = "Details", name = "Tom", id = 10 })</pre>
<p>Хелпер <code>Content</code> преобразует относительные пути в абсолютные, например:</p>
<pre class="brush:xml;">
&lt;script src='@Url.Content("~/lib/jquery/dist/jquery.js")'&gt;&lt;/script&gt;
</pre>
<p>Одним из преимуществ URL-хелперов является то, что мы можем их легко применять для генерации путей в коде контроллера. Например:</p>
<pre class="brush:c#;">
public string About()
{
...
<p>При использовании методов <code>Url.Action()</code> и <code>Url.RouteLink()</code>, следует учитывать, что они, также как и Html.ActionLink или AnchorTagHelper 
используют систему маршрутизации.</p>
<p>Например, пусть у нас будут определены следующие маршруты:</p>
<pre class="brush:c#;">
...
<p>То вызов</p>
<pre class="brush:c#;">@Url.Action("Index", "Home")</pre>
<p>в соответствии со вторым маршрутом сгенерирует следующий путь:</p>
<pre>/Store/SubIndex</pre>
<p>А следующий вызов будет соответствовать первому маршруту:</p>
<pre class="brush:c#;">@Url.RouteUrl(new { controller = "Users", action = "List"})</pre>
<p>поэтому будет сгенерирован следующий путь:</p>
<pre>/UserStore</pre>
<p>Или еще один пример, соответствующий первому маршруту:</p>
<pre class="brush:c#;">@Url.Action("List", "Users", new { name = "Tom", id = 10 }, null)</pre>
<p>Его результатом будет следующий путь:</p>
<pre>/UserStore?name=Tom&id=10</pre>
<p>Поэтому при использовании данных хелперов также надо принимать во внимание все определенные в приложении машруты.</p>
