...
<html >
<head>
<title>ASP.NET Core MVC и C# | Работа с данными в Entity Framework</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с базой данных MS SQL Server в ASP.NET Core MVC и C# через Entity Framework, определение моделей и создание базы данных">
...
<h1>Работа с данными в Entity Framework</h1><h2>Подключение Entity Framework Core</h2><div class="date">Последнее обновление: 10.04.2022</div>
...
<p>Entity Framework представляет прекрасное ORM-решение, которое позволяет автоматически связать обычные классы языка C# с таблицами в базе данных. Entity Framework Core 
поддерживает различные СУБД, но в данном случае мы будем работать с базами данных MS SQL Server.</p>
<p>Для работы с Entity Framework вначале создадим новый проект ASP.NET Core по шаблону <span class="b">ASP.NET Core Empty</span>.</p>
<p>Для взаимодействия с MS SQL Server через Entity Framework необходим пакет <span class="b">Microsoft.EntityFrameworkCore.SqlServer</span>. 
По умолчанию он отсутствует в проекте, поэтому его надо добавить, например, через пакетный менеджер Nuget:</p>
<img src="./pics/11.1.png" alt="Добавление Entity Framework Core в проект ASP.NET Core MVC и C#" />
<p>Далее создадим в проекте папку <span class="b">Models</span> и в нее добавим новый класс, который назовем <span class="b">User</span>:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Эта модель представляет те объекты, которые будут храниться в базе данных.</p>
<p>Чтобы взаимодействовать с базой данных через Entity Framework нам нужен контекст данных - класс, унаследованный от класса 
<span class="b">Microsoft.EntityFrameworkCore.DbContext</span>. Поэтому добавим в папку <span class="b">Models</span> новый класс, который назовем <span class="b">ApplicationContext</span>:</p>
<pre class="brush:c#;">
...
<p>Свойство DbSet представляет собой коллекцию объектов, которая сопоставляется с определенной таблицей в базе данных. При этом по умолчанию 
...
User - название класса модели представляет единственное число, а Users - множественное число.</p>
<p>Через параметр <code>options</code> в конструктор контекста данных будут передаваться настройки контекста.</p>
<p>В конструкторе с помощью вызова <code>Database.EnsureCreated()</code> по определению моделей будет создаваться база данных (если она отсутствует).</p>
<p>Чтобы подключаться к базе данных, нам надо задать параметры подключения. Для этого изменим файл <span class="b">appsettings.json</span>, добавив в него 
определение строки подключения:</p>
<pre class="brush:js;">
{
  "ConnectionStrings": {
...
<p>В данном случае мы будем использовать упрощенный движок базы данных LocalDB, который представляет легковесную версию 
SQL Server Express, предназначенную специально для разработки приложений.</p>
<p>И последним шагом в настройке проекта является изменение файла <span class="b">Program.cs</span>:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>Добавление контекста данных в виде сервиса позволит затем получать его в конструкторе контроллера через механизм внедрения зависимостей.</p>
<h3>Получение контекста данных в контроллере</h3>
<p>Поскольку выше в приложении контекст данных добавляется в виде сервиса, то в конструкторе контроллера мы можем получить переданный контекст данных. Например, пусть для 
хранения контроллеров в проекте определена папка <span class="b">Controllers</span>, в которой есть класс контроллера - <span class="b">HomeController</span>:</p>
<pre class="brush:c#;">
...
<p>Для взаимодействия с базой данных в контроллере определяется переменная контекст данных <code>ApplicationContext db</code>. Причем 
поскольку в приложении контекст данных добавляется в виде сервиса, то в конструкторе контроллера мы можем получить переданный контекст данных.</p>
...
<div class="nav"><p><a href="./10.7.php">Назад</a><a href="./">Содержание</a><a href="./11.2.php">Вперед</a></p></div></div>
<div class="socBlock">
