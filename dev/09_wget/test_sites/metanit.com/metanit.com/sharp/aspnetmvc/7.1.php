...
<html >
<head>
<title>ASP.NET Core MVC и C# | Tag-хелперы</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в tag-хелперы в ASP.NET Core MVC и C#, роль файла _ViewImports.cshtml, добавление и удаление tag-хелперов">
...
<h1>Tag-хелперы</h1><h2>Введение в tag-хелперы</h2><div class="date">Последнее обновление: 03.04.2022</div>
...
<p><span class="b">Tag-хелперы</span> представляют собой функциональность, предназначенную для генерации HTML-разметки. Tag-хелперы применяются в представлениях 
...
конечном счете преобразуются в стандартные html-элементы.</p>
<p>Tag-хелперы представляют более удобный способ для генерации html-элементов, нежели обычные html-хелперы, поскольку tag-хелперы во многом выглядят как обычные html-элементы, 
Visual Studio имеет встроенную поддержку IntelliSense для tag-хелперов</p>
<p>Использовать tag-хелперы довольно просто. Рассмотрим на примере примитивного проекта. Пусть у нас есть проект по типу  <span class="b">ASP.NET Core Empty</span>. Определим в 
файле <span class="b">Program.cs</span> подключение необходимых сервисов MVC:</p>
<pre class="brush:c#;">
...
<p>Для тестирования в проекте определим папку <span class="b">Controllers</span>, а в нее поместим следующий контроллер <span class="b">HomeController</span>:</p>
<pre class="brush:c#;">
...
<p>Для представлений этого контроллера создадим в проекте папку <span class="b">Views</span>, а в ней - каталог <span class="b">Home</span>. Затем в папку <span class="b">Views/Home</span> поместим 
новое представление <span class="b">Index.cshtml</span>:</p>
<pre class="brush:xml;">
@addTagHelper *, Microsoft.AspNetCore.Mvc.TagHelpers

&lt;a asp-controller="Home" asp-action="Contacts"&gt;Контакты&lt;/a&gt;
</pre>
<p>Сначала в представлении идет директива <span class="b">addTagHelper</span></p>
<pre class="brush:c#;">@addTagHelper *, Microsoft.AspNetCore.Mvc.TagHelpers</pre>
<p>Первый параметр директивы указывает на tag-хелперы, которые будут доступны в представлении, а второй параметр определяет 
...
библиотеки <span class="b">Microsoft.AspNetCore.Mvc.TagHelpers</span>.</p>
<p>Далее идет собственно tag-хелпер:</p>
<pre class="brush:xml;">&lt;a asp-controller="Home" asp-action="Contacts"&gt;Контакты&lt;/a&gt;</pre>
<p>Внешне данный хелпер напоминает обычную ссылку - стандартный элемент html, однако это не элемент html. И если мы воспользуемся всплывающей подсказкой, 
то увидим, что кроме обычных для элемента <code>&lt;a /&gt;</code> он имеет ряд других:</p>
<img src="./pics/7.1.png" alt="Tag хелперы в ASP.NET Core MVC и C#" />
<p>Данный хелпер создает ссылку, для которой в качестве контроллера используется Home, а в качестве метода Contact. Такой хелпер будет интуитивно более 
понятным и привычным, нежели создание ссылки с помощью Html.ActionLink:</p>
<pre class="brush:c#;">@Html.ActionLink("Контакты", "Contacts", "Home")</pre>
<p>В то же время нам необязательно использовать именно tag-хелперы. Мы можем использовать обычные html-хелперы, если они нам более удобны.</p>
<p>В итоге при запуске проекта вместа данного tag-хелпера будет сформирована гиперссылка, по нажатию на которую запрос будет обрабатываться методом Contacts контроллера Home:</p>
<img src="./pics/7.2.png" alt="добавление tag-хелперов в представление в ASP.NET Core MVC и C#" />

<h3>_ViewImports.cshtml и @addTagHelper</h3>
<p>Выше в представление были подключены tag-хелперы. Но что, если нам надо подключить tag-хелперы в кучу представлений? Вместо того, чтобы прописывать директиву 
<span class="b">@addTagHelper</span> в каждом отдельном представлении, мы можем подключить все хелперы разом. 
Для этого применяется файл <span class="b">_ViewImports.cshtml</span>.</p>
<p>Итак, добавим в проект в папку <span class="b">Views</span> новый файл <span class="b">_ViewImports.cshtml</span>:</p>
<img src="./pics/7.3.png" alt="_ViewImports.cshtml в ASP.NET Core MVC и C#" />

<p>В файле <span class="b">_ViewImports.cshtml</span> определим подключение tag-хелперов:</p>
<pre class="brush:c#;">@addTagHelper *, Microsoft.AspNetCore.Mvc.TagHelpers</pre>
<p>После этого из представления Index.cshtml можно удалить подключение tag-хелперов и оставить только создание ссылки:</p>
<pre class="brush:xml;">&lt;a asp-controller="Home" asp-action="Contacts"&gt;Контакты&lt;/a&gt;</pre>
<p>Также мы можем конкретизировать применение хелперов к определенной группе представлений. Например, если у нас есть каталог <span class="b">Views/Home</span> - 
специально для представлений для контроллера HomeController, 
и мы хотим применить только к ним определенные хелперы. В этом случае мы можем добавить файл <span class="b">_ViewImports.cshtml</span> непосредственно в этот каталог. 
И любой tag-хелпер, добавленный директивой <code>@addTagHelper</code> из файла <span class="ii">Views/Home/_ViewImports.cshtml</span>, будет применяться только к представлениям 
из каталога <span class="ii">Views/Home</span>.</p>
<h3>Удаление tag-хелперов</h3>
<p>Еще одна директива <code>removeTagHelper</code> удаляет ранее добавленные tag-хелперы. Ее применение аналогично:</p>
<pre class="brush:c#;">@removeTagHelper "*, Microsoft.AspNetCore.Mvc.TagHelpers"</pre>
<p>Данная директива может быть полезной, если мы, например, захотим ограничить применение хелперов в каком-то одном представлении или группе представлений. 
Эту директиву также можно определять в файле <span class="ii">_ViewImports.cshtml</span>.</p>
