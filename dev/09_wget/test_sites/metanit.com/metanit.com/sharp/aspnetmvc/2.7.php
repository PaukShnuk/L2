...
<html >
<head>
<title>ASP.NET Core MVC и C# | Переадресация</title>
<meta charset="utf-8" />
<meta name="description" content="Переадресация в ASP.NET Core MVC и C#, использование объекта RedirectResult и методов RedirectToAction и RedirectToRoute">
...
<h2>Переадресация</h2><div class="date">Последнее обновление: 21.03.2022</div>
...
<p>В ASP.NET Core MVC для выполнения переадресации применяются классы <span class="b">RedirectResult</span>, 
...
<p>В данном случае идет переадресация на локальный адрес - методу About контроллера Home.</p>
...
<h3>Переадресация на действие контроллера</h3>
...
<p>Рассмотрим на примере метода <code>RedirectToAction()</code>:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
    public IActionResult Index()
...
<p>Здесь метод Index выполняет переадресацию на метод About контроллера Home.</p>
<p>Теперь передадим при переадресации некоторые значения:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
    public IActionResult Index()
...
About. В прицнипе тут можно было бы не использовать название контроллера, так как Index и About находятся в одном контроллере. В итоге при выполнении вызова</p>
<pre class="brush:c#;">return RedirectToAction("About", "Home", new { name = "Tom", age = 37 });</pre>
<p>будет происходить переадресация по адресу</p>
<pre class="brush:c#;">https://localhost:7288/Home/About?name=Tom&age=37</pre>
<h3>Переадресация на маршрут</h3>
...
<p>Более подробно система маршрутизации будет рассмотрена далее. Сейчас же лишь вкратце коснемся темы, как выполнять переадресацию на маршрут. 
Прежде всего обратимся к файлу <span class="b">Program.cs</span>:</p>
<pre class="brush:c#;">
...
<p>В методе <code>app.MapControllerRoute</code> задается один маршрут, который имеет имя "default". Используя это имя, мы можем выполнить переадресацию на этот маршрут. Например:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
    public IActionResult Index()
...
<p>Здесь в метод RedirectToRoute передается название маршрута - "default". В этом маршруте есть параметры controller, action, которые устанавливаются в анонимном объекте. 
Ну и также передаются параметры name и age, которые принимает метод About.</p>
