...
<html >
<head>
<title>ASP.NET Core MVC и C# | Привязка модели</title>
<meta charset="utf-8" />
<meta name="description" content="Привязка модели model binding в ASP.NET Core MVC и C#, интерфейс IModelBinder, свзывание данных из запроса с параметрами метода">
...
<h2>Привязка модели</h2><div class="date">Последнее обновление: 27.03.2022</div>
...
<p>Привязка модели или <span class="b">Model binding</span> представляет механизм сопоставления значений из HTTP-запроса с параметрами метода контроллера. При этом параметры могут представлять 
как простые типы (int, float и т.д.), так и более сложные типы данных, например, объекты классов.</p>
<p>Чтобы понять смысл привязки, посмотрим на примере. Пусть у нас есть следующий метод:</p>
<pre class="brush:c#;">
...
<p>Допустим, на сервер приходит запрос <span class="ii">https://localhost:7288/Home/Index?name=Tom</span>.</p>
<p>При использовании стандартного маршрута для обслуживания данного запроса будет выбран метод <code>Index</code> контроллера <code>Home</code>. Поскольку данный метод принимает параметр с 
именем <span class="b">name</span>, то механизм привязки по этому имени будет искать в среди пришедших данных значение с ключом <code>name</code>.</p>
<p>Чтобы найти и сопоставить данные из запроса с параметрами метода используется <span class="b">привязчик модели (model binder)</span>, который представляет 
объект интерфейса <span class="b">IModelBinder</span>.</p>
<p>Для поиска значений привязчик модели используется следующие источники в порядке приоритета:</p>
<ul>
<li><p>Данные форм. Хранятся в объекте <code>Request.Form</code></p></li>
<li><p>Данные маршрута, то есть те данные, которые формируются в процессе сопоставления строки запроса маршруту. 
Хранятся в объекте <code>RouteData.Values</code></p></li>
<li><p>Данные строки запроса. Хранятся в объекте <code>Request.Query</code></p></li>
</ul>
<p>Причем все эти источники данных представляют словари, в которых по ключу мы можем получить значение.</p>
<p>То есть в нашем случае, когда на сервер придет запрос <span class="ii">https://localhost:7288/Home/Index?name=Tom</span>, привязчик модели последовательно 
будет просматривать в поиске значения для параметра name следующие пути:</p>
<ul>
<li><p><code>Request.Form["name"]</code></p></li>
<li><p><code>RouteData.Values["name"]</code></p></li>
<li><p><code>Request.Query["name"]</code></p></li>
</ul>
<p>В случае, если параметры метода представляют сложные данные, например, класс, привязчик модели будет действовать подобным образом. 
...
В частности, привязки модели ищет значения с ключами наподобие <code>[имя_параметра].[имя_свойства]</code>. 
Если подобных значений не будет найдено, то привязчик ищет значения просто по имени свойства.</p>
<p>То есть, к примеру, пусть у нас есть следующая модель:</p>
<pre class="brush:c#;">
public record class Person(string Name, int Age, Company? Company);
public record class Company(string Name);
</pre>
<p>И пусть метод принимает в качестве параметра объект данной модели:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
...
<p>В этом случае привязчик модели последовательно будет просматривать те же источники в поиске значений для свойств объекта person. Например, чтобы найти 
значение для свойства Name, привязчик будет искать значение по следующим ключам:</p>
<ul>
<li><p><code>Request.Form["person.Name"]</code></p></li>
<li><p><code>RouteData.Values["person.Name"]</code></p></li>
<li><p><code>Request.Query["person.Name"]</code></p></li>
</ul>
<p>В случае если параметр сам хранит объект сложного типа, как выше класс Person ссылается на класс Company, то привязчик с помощью рекурсии спускается на уровень ниже - на уровень класса Company и 
пытается получить его свойства и найти для них значения.</p>
<p>Для таких типов как коллекции привязчик модели ищет значения с ключами <code>имя_параметра[index]</code> или просто по индексу <code>[index]</code>. 
Если параметр представляет объект Dictionary, то привязчик модели также ищет в источниках запроса значения с ключами <code>имя_параметра[ключ]</code> 
или просто ищет по ключу: <code>[ключ]</code>.</p>
<p>При этом свойства, к которым осуществляется привязка, должны быть объявлены с модификатором <span class="b">public</span> и быть доступными для записи.</p>
<p>Кроме того, если класс не представляет тип record, то он должен иметь конструктор без параметров. Поскольку в примере выше применются классы record, то конструктор без 
параметров можно не определять.</p>
<p>Например, если к вышеопреределенному методу контроллера мы обратимся с запросом</p>
<pre class="brush:c#;">https://localhost:7288/Home/Index?person.Name=Tom&person.Age=37&person.Company.Name=Microsoft</pre>
<p>то ASP.NET Core MVC вполне сможет связать передаваемые значения со свойствами объекта Person:</p>
<img src="./pics/5.5.png" alt="Привязка модели в ASP.NET Core MVC и C#" />
<p>Когда значение для параметра метода найдено, привязчик модели прекращает поиск значений для этого параметра и переходит к поиску значений для следующего параметра. 
...
свойство <code>ModelState.IsValid</code>. Если оно равно <code>false</code>, то привязка завершилась с ошибкой, и полноценно параметры метода мы использовать не сможем.</p>
