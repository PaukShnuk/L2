...
<html >
<head>
<title>ASP.NET Core MVC и C# | ViewComponentContext</title>
<meta charset="utf-8" />
<meta name="description" content="Контекст ViewComponentContext и его применение в View Component в ASP.NET Core MVC и C#, получение данных запроса в компоненте представления">
...
<h2>ViewComponentContext</h2><div class="date">Последнее обновление: 06.04.2022</div>
...
<p>При создании компонента с помощью наследования от класса <span class="b">ViewComponent</span> внутри подобного компонента мы можем использовать через ряд свойств, 
которые позволяют получить различные данные окружения, в том числе данные запроса:</p>
<ul>
<li><p><span class="b">HttpContext</span>: представляет контекст, который описывает полученный запрос, а также отправляемый ответ</p></li>
<li><p><span class="b">ModelState</span>: представляет состояние модели в виде объекта ModelStateDictionary</p></li>
<li><p><span class="b">Request</span>: возвращает контекст запроса в виде объекта HttpRequest</p></li>
<li><p><span class="b">RouteData</span>: возвращает данные маршрута</p></li>
<li><p><span class="b">Url</span>: представляет объект IUrlHelper, который используется для генерации адресов URL</p></li>
<li><p><span class="b">User</span>: представляет текущего пользователя в виде объкта IPrincipal</p></li>
<li><p><span class="b">ViewBag</span>: представляет динамический объект, который может использоваться для передачи данных в представление</p></li>
<li><p><span class="b">ViewContext</span>: описывает контекст главного представления, в котором вызывается компонент</p></li>
<li><p><span class="b">ViewComponentContext</span>: представляет объект ViewComponentContext, который инкапсулирует контекст компонента</p></li>
<li><p><span class="b">ViewData</span>: возвращает объект ViewDataDictionary, который применяется для передачи данных в представление</p></li>
</ul>
<p>Используем некоторые и этих свойств. Пусть код компонента будет выглядеть следующим образом:</p>
<pre class="brush:c#;">
...
<p>Итак, в компоненте определены некоторые начальные данные - список users. В методе Invoke получаем из строки запроса параметр "number", который призван указать, сколько пользователей надо вывести 
на веб-страницу. сли параметр не указан, то просто передаем всех пользователей. А с помощью объектов ViewBag и ViewData передаем данные в представление.</p>
<p>Допустим, данный компонент будет использоваться контроллером HomeController. И для представлений компонента определим папку <span class="b">Views/Home/Components/UsersList</span> и 
в ней определим представление <span class="b">Default.cshtml</span> со следующим кодом:</p>
<pre class="brush:xml;">
...
<img src="./pics/8.14.png" alt="контекст компонента представления в ASP.NET Core MVC и C#" />
<p>Вызовем компонент в главном представлении:</p>
<pre class="brush:xml;">
...
<p>И после этого через параметр number мы сможем изменять количество выводимых пользователей:</p>
<img src="./pics/8.15.png" alt="Контекст в View Component в ASP.NET Core MVC и C#" />
...
<div class="nav"><p><a href="./8.4.php">Назад</a><a href="./">Содержание</a><a href="./9.1.php">Вперед</a></p></div></div>
<div class="socBlock">
