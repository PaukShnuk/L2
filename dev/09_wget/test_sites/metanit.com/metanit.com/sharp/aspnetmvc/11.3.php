...
<html >
<head>
<title>ASP.NET Core MVC и C# | Редактирование и удаление данных</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с базой данных MS SQL Server в ASP.NET Core MVC и C# через Entity Framework, редактирование и удаление данных">
...
<h2>Редактирование и удаление данных</h2><div class="date">Последнее обновление: 11.04.2022</div>
...
<p>Продолжим работу с проектом из прошлой темы и добавим в него возможность редактирования и удаления данных. И прежде всего изменим представление <span class="b">Index.cshtml</span>, добавив в него ссылки:</p>
<pre class="brush:xml;">
...
<p>В таблице кроме вывода имени и возраста каждого пользователя также предусмотрена ссылка на редактирование данных, а также форма с кнопкой на удаление данных. Теперь создадим для каждого действия методы.</p>
<img src="./pics/11.5.png" alt="Редактирование и удаление данных в Entity Framework в ASP.NET Core MVC и C#" />
<h3>Удаление</h3>
<p>Для создания инфраструктуры удаления добавим в контроллер следующий метод:</p>
<pre class="brush:c#;">
[HttpPost]
...
<p>Метод принимает параметр id, с помощью которого получаем удаляемый объект из БД и, если он существует, удаляем его с помощью метода <code>db.Users.Remove()</code>. 
Данный метод генерирует sql-выражение <code>DELETE</code>, которое выполняется вызовом <code>db.SaveChangesAsync()</code>.</p>
<p>Стоит отметить, что данный метод Delete обрабатывает только запросы типа POST. Почему? Дело в том, что использование get-методов не безопасно. 
Например, нам могут прислать письмо с картинкой:</p>
<pre class="brush:xml;">
&lt;img src="http://адрес_нашего_сайта/Home/Delete/1" /&gt;
</pre>
<p>И при открытии письма на сервер будет отправлен get-запрос. И если бы метод Delete обрабатывал бы get-запросы, то объект с id=1 был бы удален из базы данных.</p>
<p>Именно поэтому данный метод обрабатывает запросы POST, а для обращения к этому методу в представлении определена не просто ссылка, а форма с кнопкой, по нажатию на которую выполняется 
post-запрос.</p>
...
<p>И также добавим для этих методов в папку <span class="b">Views/Home</span> представление <span class="b">Edit.cshtml</span>:</p>
<pre class="brush:xml;">
