...
<html >
<head>
<title>ASP.NET Core MVC и C# | Фильтры ресурсов</title>
<meta charset="utf-8" />
<meta name="description" content="Фильтры ресурсов (Resource Filters) в ASP.NET Core MVC и C#, реализация интерфейса IResourceFilter">
...
<h2>Фильтры ресурсов</h2><div class="date">Последнее обновление: 10.04.2022</div>
...
<p><span class="b">Фильтр ресурсов</span> нередко применяется для переопределения результата действия. Это может быть полезно, например, в ситуации с кэшированием: 
в фильтре ресурсов можно получить кэш и сразу установить результат без ненужной повторной генерации результата в методах контроллера.</p>
<p>Для создания фильтра ресурсов надо реализовать либо интерфейс <span class="b">IResourceFilter</span>, либо интерфейс <span class="b">IAsyncResourceFilter</span>.</p>
<p>К примеру, пусть нам надо ограничить доступ к сайту для старых браузеров типа IE, и как раз для этого можно разработать простенький фильтр ресурсов. 
Для этого добавим в проект следующий класс фильтра:</p>
<pre class="brush:c#;">
...
<p>Интерфейс IResourceFilter предоставляет два метода:</p>
<ul>
<li><p><code>OnResourceExecuting()</code>: срабатывает после фильтров авторизации, но до выполнения метода и работы фильтров действий, исключений и результатов</p></li>
<li><p><code>OnResourceExecuted()</code>: срабатывает после выполнения метода и фильтров действий, исключений и результатов</p></li>
</ul>
<p>В качестве параметра в оба метода передается параметр типа <span class="b">ResourceExecutedContext</span>, который позволяет получить данные запроса и управлять 
ответом.</p>
<p>В данном случае проверяем заголовок "User-Agent". Если он содержит соответствующие браузеру Internet Explorer подстроки, то с помощью свойства Result переустанавливаем ответ. И на этом 
обработка запроса завершается, и метод и последующие фильтры не выполняются.</p>
<p>Реализация той же задачи с помощью асинхронной версии:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Mvc.Filters;
using System.Text.RegularExpressions;
...
<p>Асинхронная версия определяет только один метод, который принимает также паараметр ResourceExecutingContext. Второй парамет - объект 
