...
<html >
<head>
<title>ASP.NET Core MVC и C# | Tag-хелперы и модели</title>
<meta charset="utf-8" />
<meta name="description" content="Передача коллекций и сложных объектов в Tag-хелперы в ASP.NET Core MVC и C#, композиции хелперов, использования одного tag-хелпера в другом">
...
<h2>Tag-хелперы и модели</h2><div class="date">Последнее обновление: 05.04.2022</div>
...
<p>С помощью атрибутов тегов мы можем передавать сложные данные - объекты классов, коллекции объектов.</p>
<h3>Коллекции объектов</h3>
<p>Например, мы хотим создать тег для вывода 
в виде списка произвольной коллекции строк. Для этого определим следующий класс tag-хелпера:</p>
<pre class="brush:c#;">
...
<p>Данный хелпер через свойство Elements будет получать коллекцию строк в виде объекта <code>List&lt;string&gt;</code>. А в методе 
Process из этих элементов будет создаваться html-элемент ul.</p>
<p>Теперь определим в представлении следующий код:</p>
<pre class="brush:xml;">
@addTagHelper *, MvcApp
...
<p>В блоке кода создаются два списка: страны и города. Затем эти списки передаются в тег. В итоге веб-страница выведет эти страны и города в виде элементов ul:</p>
<img src="./pics/7.19.png" alt="Передача в tag-хелперы коллекций в ASP.NET Core MVC и C#" />
<h3>Сложные объекты</h3>
<p>Аналогично идет работа со сложными моделями. Например, пусть у нас определена следующая модель Person:</p>
<pre class="brush:c#;">
...
<p>Для вывода информации об одном объекте данной модели создадим следующий класс хелпера:</p>
<pre class="brush:c#;">
...
<p>Также, как и в предыдущем случае, через свойство Person класс получает переданные из представления данные и с их помощью создает разметку html.</p>
<p>Далее определим в представлении следующий код:</p>
<pre class="brush:xml;">
@addTagHelper *, MvcApp
...
<p>Поскольку класс хелпера в названии имеет две части, начинающихся с заглавной буквы: PersonInfo (не считая суффикса TagHelper), то этому классу 
будет соответствовать тег <code>Person-info</code>. При запуске приложение сгенерирует разметку для вывода информации об объекте Person:</p>
<img src="./pics/7.20.png" alt="Tag Helper и модели в ASP.NET Core MVC и C#" />
...
<div class="nav"><p><a href="./7.10.php">Назад</a><a href="./">Содержание</a><a href="./8.1.php">Вперед</a></p></div></div>
<div class="socBlock">
