...
<html >
<head>
<title>ASP.NET Core MVC и C# | CacheTagHelper</title>
<meta charset="utf-8" />
<meta name="description" content="Кэширование в представлениях в  ASP.NET Core MVC и C# с помощью tag-хелпера CacheTagHelper">
...
<h2>CacheTagHelper</h2><div class="date">Последнее обновление: 04.04.2022</div>
...
<p>Класс <span class="b">CacheTagHelper</span> позволяет кэшировать в памяти сервера некоторую часть контента представления. 
Он использует тег <code>&lt;cache&gt;</code>, и весь контент внутри этого тега кэшируется в памяти. 
...
<p>Например, пусть у нас в представлении например, в Index.cshtml, применяется следующий код:</p>
<pre class="brush:xml;">
...
<p>Здесь два раза выводится текущее время. Но второй вызов метода <code>DateTime.Now.ToString</code> помещен в элемент <span class="b">cache</span>, что позволяет кэшировать результат метода. 
И если в течение короткого времени мы несколько раз обратимся к приложению, то увидим, что время из второго вызова <code>DateTime.Now.ToString</code> застыло и не 
изменяется:</p>
<img src="./pics/7.10.png" alt="CacheTagHelper и кэширование контента в представлениях ASP.NET Core MVC в C#" />
