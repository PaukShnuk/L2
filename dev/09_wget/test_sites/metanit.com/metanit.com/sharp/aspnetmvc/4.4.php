...
<html >
<head>
<title>ASP.NET Core MVC и C# | Области</title>
<meta charset="utf-8" />
<meta name="description" content="Области в ASP.NET Core MVC и C#, организация приложения, особенности работы с областями, проекция маршрутов на область и метод MapAreaControllerRoute">
...
<h2>Области</h2><div class="date">Последнее обновление: 26.03.2022</div>
...
<p>Возьмем простейший проект ASP.NET Core по типу Empty и сначала добавим в нее папку <span class="b">Controllers</span>, а в нее поместим примитивный контроллер <span class="b">HomeController</span> 
...
<p>Этот контроллер не принадлежит никакой области.</p>
<p>Теперь добавим область. Как правило, области помещаются в отдельную папку. Поэтому добавим в проект новую папку, которую назовем <span class="b">Areas</span>. В ней будут росполагаться области.</p>
<p>Затем в папку <span class="b">Areas</span> добавим каталог под названием <span class="b">Account</span>. То есть у нас будет область под названием Account.</p>
<p>И далее в папке <span class="b">Areas/Account</span> создадим подкаталоги Controllers и Views. В итоге проект будет выглядеть так:</p>
<img src="./pics/4.13.png" alt="Области в ASP.NET Core MVC и C#" />
...
Без этого атрибута контроллер не будет принадлежать области Account.</p>
<p>Далее в папку <span class="b">Areas/Account/Views</span> добавим подкаталог <span class="b">Home</span> - для представлений нового контроллера. И затем 
в этот подкаталог поместим представление <span class="b">Index.cshtml</span> с одной строкой:</p>
<pre class="brush:xml;">
...
<ul>
<li><p>/Areas/Account/Views/Home/</p></li>
<li><p>/Areas/Account/Views/Shared/</p></li>
<li><p>/Views/Shared/</p></li>
</ul>
<p>Соответственно все три выше указанные папки мы могли бы использовать для хранения представления Index.cshtml для контроллера из области Account.</p>
<p>Таким образом, проект теперь выглядит так:</p>
<img src="./pics/4.14.png" alt="Определение областей в ASP.NET Core MVC и C#" />
<p>Но пока эта область совершенно неактивна. Мы никак не можем к ней обратиться. И теперь добавим в приложение поддержку этой области. Для этого изменим код в файле 
<span class="b">Program.cs</span> следующим образом:</p>
<pre class="brush:c#;">
...
<p>И теперь мы сможем обращаться к области с запросом <span class="ii">http://localhost:xxxx/Account</span>:</p>
<img src="./pics/4.15.png" alt="Areas in ASP.NET Core MVC и C#" />
<p>Если же в запросе не использован префикс "Account", то обращение будет производиться к контроллеру HomeController, который расположен вне области</p>
<img src="./pics/4.16.png" alt="Контроллеры в областях ASP.NET Core MVC и C#" />
<h3>Метод MapAreaControllerRoute</h3>
<p>В примере выше определялся маршрут для всех областей. С помощью метода <span class="b">MapAreaControllerRoute()</span> можно указать маршрут для конкретной области:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder(args);
builder.Services.AddControllersWithViews();
...
Причем шаблон маршрута начинается с префикса области: "store/{controller=Home}/{action=Index}/{id?}". Обратите внимание, что в данном случае для обращения к области Account путь запроса должен начинаться с "profile":</p>
<img src="./pics/4.17.png" alt="Настройка маршрутизации области и MapAreaControllerRoute в ASP.NET Core MVC и C#" />
...
<p>Для обращения к такому методу Index мы можем использовать три адреса: <span class="ii">http://localhost:xxxx/Account</span>, 
<span class="ii">http://localhost:xxxx/Account/Home</span> и <span class="ii">http://localhost:xxxx/Account/Home/Index</span></p>
<p>В этом случае в файле Program.cs можно не определять маршрут для области:</p>
<pre class="brush:c#;">
var builder = WebApplication.CreateBuilder(args);
builder.Services.AddControllersWithViews();
...
<p>Для обращения к такому методу Index мы можем использовать следующие адреса: <span class="ii">http://localhost:xxxx/Profile</span>, 
<span class="ii">http://localhost:xxxx/Profile/Home</span> и <span class="ii">http://localhost:xxxx/Profile/Home/Index</span></p>

<h3>Добавление области</h3>
<p>Для упрощения добавления областей в Visual Studio можно использовать специальный шаблон. В частности, нажмем правой кнопкой мыши на папку <span class="b">Areas</span> 
и в появившемся контекстном меню выберем пункт <span class="b">Add -&gt; Area...</span></p>
<img src="./pics/4.10.png" alt="Добавление области в ASP.NET Core MVC и C#" />
<p>Затем в окне добавления нового элемента выберем пункт <span class="b">MVC Area</span></p>
<img src="./pics/4.11.png" alt="MVC Area in ASP.NET Core MVC и C#" />
<p>Далее будет предложено ввести имя для области</p>
<img src="./pics/4.12.png" alt="Настройка области в ASP.NET Core MVC и C#" />
<p>Затем Visual Studio сгенерирует стартовую структуру каталогов области.</p>
