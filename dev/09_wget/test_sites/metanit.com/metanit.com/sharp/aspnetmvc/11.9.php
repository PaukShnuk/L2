...
<html >
<head>
<title>ASP.NET Core MVC и C# | Объединение сортировки, фильтрации и пагинации</title>
<meta charset="utf-8" />
<meta name="description" content="Объединение сортировки, фильтрации и постраничной навигации через Entity Framework в ASP.NET Core MVC и C#">
...
<h2>Объединение сортировки, фильтрации и пагинации</h2><div class="date">Последнее обновление: 12.04.2022</div>
...
<p>В предыдущих темах было рассмотрено, как по отдельности произвести фильтрацию, сортировку и постраничный вывод. Теперь посмотрим, как 
можно все это объединить в одном приложении.</p>
<p>Для работы с проектом пусть у нас будут определены следующие модели <span class="b">User</span> и <span class="b">Company</span>:</p>
<pre class="brush:c#;">
public class User
{
...
А затем добавим в проект класс контекста данных UsersContext:</p>
<pre class="brush:c#;">
...
<p>И в файле <span class="b">Program.cs</span> настроим использование этого контекста данных:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
using MvcApp.Models;
...
<p>Итак, нам надо произвести три операции: фильтрацию, сортировку и пагинацию. Каждая из этих операций будет представлять свой набор параметров. Для упрощения для каждого набора параметров 
определим собственную модель. Так, для фильтрации добавим модель <span class="b">FilterViewModel</span>:</p>
<pre class="brush:c#;">
...
<p>В данном случае фильтрация будет идти по компаниям и по имени пользователей, поэтому для выбранных значений определены два свойства плюс список компаний.</p>
<p>Для сортировки добавим перечисление <span class="b">SortState</span>, которое будет описывать все возможные варианты сортировки:</p>
<pre class="brush:c#;">
...
<p>И затем добавим новую модель <span class="b">SortViewModel</span>:</p>
<pre class="brush:c#;">
namespace MvcApp.Models
{
...
<p>Здесь для каждого свойства хранится его текущее значение SortState. Кроме того, отдельное свойство Current хранит выбранный критерий сортировки.</p>
<p>Для пагинации добавим новую модель <span class="b">PageViewModel</span>:</p>
<pre class="brush:c#;">
namespace MvcApp.Models
{
...
<p>И в конце добавим общую модель <span class="b">IndexViewModel</span>, которая объединит все эти модели и полученные данные:</p>
<pre class="brush:c#;">
namespace MvcApp.Models
{
...
<p>После создания всех классов проект будет выглядеть следующим образом:</p>
<img src="./pics/11.16.png" alt="Создание проекта для пагинации, фильтрации и сортироки в ASP.NET Core MVC и C#" />
<p>При фильтрации нам надо отдавать новый отфильтрованный набор пользователей. 
...
если мы отправляем новое значение для фильтрации.</p>
<p>При сортировке необходимо учитывать параметры фильтрации, то есть сортировка может идти только в отфильтрованном наборе. В то же время для сортировки 
не играют никакого значения параметры пагинации. И неважно, на какой странице мы находимся, если мы решили отсортировать набор по другому столбцу.</p>
При пагинации необходимо учитывать параметры фильтрации и сортировки, то есть переход по страницам будет идти только в отсортированном и отфильтрованном наборе.</p>
<p>Таким образом, фильтрация ни от чего не зависит. Сортировка зависит от фильтрации. А пагинация зависит от фильтрации и сортировки. Поэтому сначала надо выполнить 
фильтрацию, потом сортировку и в конце пагинацию. Исходя из этого, определим в контроллере <span class="b">HomeController</span> следующий код:</p>
<pre class="brush:c#">
...
<p>В методе Index в качестве параметров принимаем выбранную компанию (ее id), введенное имя для поиска, номер страницы и значение сортировки. Если 
последние два параметра не переданы, то для них устанавливаются значения по умолчанию - первая страница и сортировка по имени по возрастанию.</p>
<p>Код метода Index получился довольно большим и органично разбивается на три секции: фильтрация, сортировка и пагинация. В идеале код каждой отдельной секции можно было бы выделить в отдельный 
класс, но в данном случае я не буду чрезмерно раздувать проект новыми классами.</p>
<p>И для действия Index контроллера HomeController определим следующее представление <span class="b">Index.cshtml</span>:</p>
<pre class="brush:xml;">
...
<p>Так как сортировка зависит от фильтрации, то в ссылки для сортировки передаются выбранные значения при фильтрации.</p>
<p>И аналогично так как пагинация зависит от фильтрации и сортировки, в ссылки для пагинации передаются выбранные значения при фильтрации и сортировки.</p>
<img src="./pics/11.17.png" alt="Сортировка и пагинация и фильтрация через Entity Framework в ASP.NET Core MVC и C#" />
