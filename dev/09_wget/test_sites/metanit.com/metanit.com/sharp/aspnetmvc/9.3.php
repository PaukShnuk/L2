...
<html >
<head>
<title>ASP.NET Core MVC и C# | Атрибуты валидации</title>
<meta charset="utf-8" />
<meta name="description" content="Атрибуты валидации в ASP.NET Core MVC и C# и их применение, атрибут Required, RegularExpression, StringLength, Remote, удаленная валидация, настрока сообщений об ошибках">
...
<h2>Атрибуты валидации</h2><div class="date">Последнее обновление: 07.04.2022</div>
...
<p>С помощью атрибутов валидации модели мы можем управлять валидацией и заключать несложную логику проверки значений свойств уже в атрибуты этих свойств, не прибегая к коду. 
Рассмотрим атрибуты валидации, которые мы можем применить в приложении на ASP.NET Core MVC.</p>
<h3>Атрибут Required</h3>
<p>Применение этого атрибута к свойству модели означает, что данное свойство должно быть обязательно установлено. Допустим, у нас есть следующая модель Person</p>
<pre class="brush:c#;">
...
<p>С помощью свойства <span class="b">ErrorMessage</span> этого атрибута можно настроить выводимое при валидации сообщение. 
А если мы явным образом не установим текст сообщения, то при выводе ошибки будет отображаться стандартный текст сообщения.</p>
<p>Допустим, для ввода данных этой модели будет применяться следующее представление:</p>
<pre class="brush:xml;">
...
<img src="./pics/9.7.png" alt="Атрибут Required в ASP.NET Core MVC и C#" />
<p>Как можно заметить, у свойства <code>int Age</code> не используется атрибут Required, тем не менее если мы не укажем для него значение, оно не пройдет валидацию. 
Потому что для свойств тех типов данных, которые не могут принимать значение <code>null</code>, нам в любом случае надо предоставить значение, как в случае со свойством Age. 
Поэтому для него использовать атрибут Required необязательно.</p>
<h3>Атрибут RegularExpression</h3>
<p>Использование данного атрибута предполагает, что вводимое значение должно соответствовать указанному в этом атрибуте регулярному выражению.</p>
<p>Наиболее распространенный пример - это проверка адреса электронной почты на корректность. Например, класс модели содержит свойство <code>Email</code>:</p>
<pre class="brush:c#;">
[RegularExpression(@"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}", ErrorMessage = "Некорректный адрес")]
public string? Email { get; set; }
</pre>
<p>Если введенное значение не будет соответствовать регулярному выражению, то будет отображено сообщение об ошибке:</p>
<img src="./pics/9.8.png" alt="Атрибут RegularExpression в ASP.NET Core MVC и C#" />
<h3>Атрибут StringLength</h3>
<p>Чтобы пользователь не мог ввести очень длинный текст, применяется атрибут <span class="b">StringLength</span>. Первым параметром в конструкторе атрибута 
идет максимальная допустимая длина строки. Именованные параметры, в частности MinimumLength и ErrorMessage, позволяют задать дополнительные опции отображения.</p>
<pre class="brush:c#;">
public class Person
{
    [Required (ErrorMessage = "Не указано имя")]
...
<img src="./pics/9.9.png" alt="Атрибут StringLength в ASP.NET Core MVC и C#" />
<h3>Атрибут Range</h3>
<p>Атрибут Range определяет минимальные и максимальные значения для числовых данных.</p>
<pre class="brush:c#;">
[Range(1, 110, ErrorMessage = "Недопустимый возраст")]
public int Age { get; set; }
</pre>
<h3>Атрибут Compare</h3>
<p>Атрибут Compare гарантирует, что два свойства объекта модели имеют одно и то же значение. Если, например, надо, чтобы пользователь ввел пароль дважды:</p>
<pre class="brush:c#;">
[Required]
public string? Password { get; set; }
...
<p>Если пользователь введет второй раз другой пароль, отличный от первого, то он увидит ошибку:</p>
<img src="./pics/9.10.png" alt="Проверка ввода пароля в ASP.NET Core MVC и C#" />
<h3>Специальные атрибуты</h3>
<p>Выше мы применяли регулярное выражение для проверки адреса электронной почты. Проверки на корректность электронной почты, адреса url, номера телефона и 
кредитной карты довольно часто встречаются, что для них определены специальные атрибуты:</p>
<ul>
...
<h3>Атрибут Remote</h3>
<p>Атрибут Remote из пространства имен <span class="b">Microsoft.AspNetCore.Mvc;</span> для валидации свойства выполняет запрос на сервер к определенному методу контроллера. И если требуемый метод контроллера вернет значение 
false, то валидация не пройдена. Например:</p>
<pre class="brush:c#;">
[Remote(action: "CheckEmail", controller: "Home", ErrorMessage ="Email уже используется")]
public string Email { get; set; }
</pre>
<p>В данном случае указано, что для проверки значения атрибут будет обращаться к методу CheckEmail контроллера Home. Теперь определим этот метод:</p>
<pre class="brush:c#;">
...
<p>В данном случае метод должен возвращать объект <span class="b">JsonResult</span>. Если переданное через параметр значение будет равно или "admin@mail.ru", или "aaa@gmail.com", 
значит, валидация не пройдена, поэтому отправляем в ответ false. Иначе отправляем true.</p>
<img src="./pics/9.11.png" alt="Удаленная валидация Remote Validation в ASP.NET Core MVC и C#" />
