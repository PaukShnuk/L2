...
<html >
<head>
<title>ASP.NET Core MVC и C# | AnchorTagHelper. Создание ссылок</title>
<meta charset="utf-8" />
<meta name="description" content="Создание ссылок в ASP.NET Core MVC и C# с помощью tag-хелпера AnchorTagHelper, asp-controller, asp-action">
...
<h2>AnchorTagHelper</h2><div class="date">Последнее обновление: 03.04.2022</div>
...
<p><span class="b">AnchorTagHelper</span> представляет тег-хелпер, который позволяет создавать ссылки. Он может принимать ряд специальных атрибутов:</p>
<ul>
...
<h3>asp-action и asp-controller</h3>
<p>Мы можем создавать ссылки в ASP.NET Core различными способами. Например:</p>
<pre class="brush:xml;">
@Html.ActionLink("О сайте","About","Home")
&lt;a href='@Url.Action("About", "Home")'&gt;О сайте&lt;/a&gt;
</pre>
<p>В первом случае используется html-хелпер, во втором - стандартный элемент ссылки с хелпером Url.Content. Еще один способ предоставляют 
tag-хелпер AnchorTagHelper:</p>
<pre class="brush:xml;">
...
<span class="b">asp-controller</span> указывает на название контроллера, а <span class="b">asp-action</span> 
определяет действие, которому будет идти запрос. Если указан атрибут <code>asp-action</code>, но не указан <code>asp-controller</code>, то в качестве 
контроллера используется тот контроллер, который связан с текущим представлением.</p>
<p>Если необходимо установить ссылку на действие контроллера, который находится в другой области, то применяется атрибут <span class="b">asp-area</span>:</p>
<pre class="brush:c#;">&lt;a asp-controller="Home" asp-action="About" asp-area="Service"&gt;О сайте&lt;/a&gt;</pre>
<p>В данном случае предполагается, что контроллер Home находится в области Service.</p>
<p>Если, наоборот, в представлении, которое находится в какой-нибудь области, надо создать ссылку на действие контроллера, который не находится ни в какой области, то указывается пустой атрибут:</p>
<pre class="brush:c#;">&lt;a asp-controller="Home" asp-action="About" asp-area=""&gt;О сайте&lt;/a&gt;</pre>
...
Данный элемент в итоге создает следующую ссылку: <span class="ii">https://localhost.com/Home/About</span></p>
<h3>asp-route- и asp-all-route-data</h3>
<p>А что если у нас метод принимает какие-нибудь параметры, которые надо указать в ссылке:</p>
<pre class="brush:c#;">
public string GetPerson(int id) =&gt; $"Id: {id}";
</pre>
<p>В этом случае мы можем использовать атрибут <span class="b">asp-route-</span>:</p>
<pre class="brush:xml;">
&lt;a asp-controller="Home" asp-action="GetPerson" asp-route-id="5" &gt;Person 5&lt;/a&gt;
</pre>
<p>Если метод принимает несколько параметров, например:</p>
<pre class="brush:c#;">
...
<h3>Влияние системы маршрутизации</h3>
<p>При использовавании данного тег-хелпера может возникнуть вопрос, а какая именно ссылка будет сгенерирована? В реальности ответ на этот вопрос зависиот системы маршрутизации. 
Например, по умолчанию в проекте определен один маршрут (в файле <span class="b">Program.cs</span>):</p>
<pre class="brush:c#;">
app.MapControllerRoute(
	name: "default",
	pattern: "{controller=Home}/{action=Index}/{id?}");
</pre>
<p>Возьмем предпоследний пример:</p>
<pre class="brush:xml;">
&lt;a asp-controller="Home" asp-action="GetPerson" asp-route-id="5"  asp-route-age="18" asp-route-name="tom" &gt;Person 5&lt;/a&gt;
</pre>
<p>В итоге будет создаваться следующая ссылка: <span class="ii">http://localhost:1234/Home/GetPerson/5?name=tom&age=18</span>. Обратите внимание, 
что параметр id является частью шаблона маршрута, поэтому в сгенерированной ссылке он не входит строку запроса.</p>
<p>Если мы немного изменим определение маршрута, убрав сегмент параметра id:</p>
<pre class="brush:c#;">
app.MapControllerRoute(
	name: "default",
	pattern: "{controller=Home}/{action=Index}");
</pre>
<p>То параметр id будет трактоваться как часть строки запроса: <span class="ii">http://localhost:60141/Home/GetPerson?id=5&name=tom&age=18</span></p>
<p>Но это только одна из частностей. Возьмем другой пример. Допустим, в файле <span class="b">Program.cs</span> определено несколько маршрутов:</p>
<pre class="brush:c#;">
app.MapControllerRoute(
        name: "products",
...
запрос <span class="ii">http://localhost:xxxxx/Products/Index</span> будет обрабатываться методом Index контроллера Home.</p>
<p>И теперь следующий тег</p>
<pre class="brush:c#;">
&lt;a asp-controller="Home" asp-action="Index"&gt;Жми здесь&lt;/a&gt;
</pre>
<p>сгенерирует следующую ссылку</p>
<pre class="browser">&lt;a href="/Products/Index"&gt;Жми здесь&lt;/a&gt;</pre>
...
<p>Но возьмем чуть более сложную систему маршрутов:</p>
<pre class="brush:c#;">
app.MapControllerRoute(
		name: "default",
		pattern: "BookStore",
...
<p>Из определения маршрутов ясно, то запрос <span class="ii">http://localhost:xxxxx/BookStore</span> будет обрабатываться методом Index контроллера Book. 
А запрос <span class="ii">http://localhost:xxxxx/Store/SubIndex/</span> будет обрабатываться контроллером Home и его методом Index.</p>
...
следующая ссылка:</p>
<pre class="browser">http://localhost:xxxxx/Store/SubIndex</pre>
...
<pre class="browser">http://localhost:xxxxx/BookStore</pre>
<p>Поэтому если в приложении определено несколько маршрутов, то следует учитывать систему маршрутизации и проверять сгенерированные ссылки, иначе в результате можно получить совсем не то, что ожидалось.</p>
<h3>asp-route</h3>
<p>С помощью параметра <code>asp-route</code> можно сгенерировать ссылку на основании маршрута. Например, пусть у нас есть такой маршрут:</p>
<pre class="brush:c#;">
app.MapControllerRoute(
		name: "book",
		pattern: "BookStore",
...
<p>Возьмем первый маршрут по имени "book":</p>
<pre class="brush:xml;">&lt;a asp-route="book"&gt;Книги&lt;/a&gt;</pre>
<p>Такой тег создаст следующую ссылку:</p>
<pre class="brush:xml;">&lt;a href="/BookStore"&gt;Книги&lt;/a&gt;</pre>
