...
<html >
<head>
<title>ASP.NET Core MVC и C# | Контекст контроллера</title>
<meta charset="utf-8" />
<meta name="description" content="Контекст контроллера в ASP.NET Core MVC и C#, объекты HttpContext и ControllerContext, Request и Response">
...
<h2>Контекст контроллера</h2><div class="date">Последнее обновление: 20.03.2022</div>
...
<p>При обращении к контроллеру срежа ASP.NET создает для этого контроллера контекст, который содержит различные связанные с контроллером данные. Для получения контекста в классе контроллера нам доступно свойство <span class="b">ControllerContext</span>, 
которое представляет одноименный класс ControllerContext. Этот класс определяет ряд важный свойств:</p>
<ul>
<li><p><span class="b">HttpContext</span>: содержит информацию о контексте запроса</p></li>
<li><p><span class="b">ActionDescriptor</span>: возвращает дескриптор действия - объект ActionDescriptor, который описывает вызываемое действие контроллера</p></li>
...
объект, который нам доступен в любом компоненте middleware в ASP.NET Core. Этот объект также доступен через свойство HttpContext класса контроллера. То есть следующие вызовы будут обращаться к одному и тому же объекту:</p>
<pre class="brush:c#;">
...
<h3>Response</h3>
<p>Свойство <span class="b">HttpContext.Response</span> представляет объект <span class="b">HttpResponse</span> и позволяет управлять ответом на запрос, в частности, устанавливать 
заголовки ответа, куки, отправлять в выходной поток некоторый ответ. Этот же объект доступен через свойство <span class="b">Response</span> класса Conroller. 
Среди свойств объекта Response можно выделить следующие:</p>
<ul>
<li><p><span class="b">Body</span>: объект Stream, который применяется для отправки данных в ответ пользователю</p></li>
<li><p><span class="b">Cookies</span>: куки, отправляемые в ответе</p></li>
<li><p><span class="b">ContentType</span>: MIME-тип ответа</p></li>
<li><p><span class="b">Headers</span>: коллекция заголовков ответа</p></li>
<li><p><span class="b">StatusCode</span>: статусный код ответа</p></li>
</ul>
<p>С помощью объекта Response мы можем отправить настроить параметры ответа и отправить его клиенту. Например, отправим из действия контроллера данные клиенту:</p>
<pre class="brush:c#;">
...
<p>Здесь в методе Index вначале устанавливается заголовок Content-Type, а затем с помощью метода <code>WriteAsync()</code> отправляется некоторое простейшее содержимое в 
виде строки с кодом html. Поскольку метод <code>WriteAsync()</code> - асинхронный, то к нему можно применить оператор await, а метод-действие Index определен как асинхронный. Соответственно при обращении к этому действию мы увидим в браузере это содержимое:</p>
<img src="./pics/2.4.png" alt="Response и отправка ответа в asp.net core mvc и c#" />
<h3>Request</h3>
...
<p>Вся основная информация нам доступна из заголовков. Например, получим из запроса все заголовки и выведем их в браузере:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc;
...
<img src="./pics/2.5.png" alt="Request и получение данных запроса в контроллере asp.net core mvc и c#" />
