...
<html >
<head>
<title>ASP.NET Core MVC и C# | Tag-хелпер пагинации с сортировкой и фильтрацией</title>
<meta charset="utf-8" />
<meta name="description" content="Tag-хелпер для постраничной навигации с сортировкой и фильтрацией через Entity Framework в ASP.NET Core MVC и C#">
...
<h2>Tag-хелпер пагинации с сортировкой и фильтрацией</h2><div class="date">Последнее обновление: 12.04.2022</div>
...
<p>В прошлой теме было рассмотрено создание ссылок для постраничного перехода. Теперь посмотрим, как мы можем заменить эти ссылки на специальный tag-хелпер для постраничной навигации.</p>
<p>Добавим в проект новый класс <span class="b">PageLinkTagHelper</span>:</p>
<pre class="brush:c#;">
...
<p>Ранее уже было рассмотрено создание tag-хелпера для постраничной навигации, и в данном случае основная часть повторяется. Единственным исключением является свойство 
PageUrlValues:</p>
<pre class="brush:c#;">
[HtmlAttributeName(DictionaryAttributePrefix = "page-url-")]
public Dictionary&lt;string, object&gt; PageUrlValues { get; set; } = new();
</pre>
<p>Это свойство представляет словарь <code>Dictionary&lt;string, object&gt;</code>, в котором каждой строке будет сопоставлен некоторый объект.</p>
<p>Его отличительной чертой является атрибут <code>[HtmlAttributeName(DictionaryAttributePrefix = "page-url-")]</code>. Он указывает, что при применении 
хелпера в представлении мы сможем передать ему некоторые значения через атрибуты с префиксом "page-url-". Например:</p>
<pre class="brush:xml;">
&lt;page-link page-url-action="Index" page-url-number="2"&gt;&lt;/page-link&gt;
</pre>
<p>В этом случае в словаре PageUrlValues окажутся две пары значений: {"action" : "Index"} и {"number" : 2}. То есть ключи в словаре будут представлять названия атрибутов 
без префикса <code>page-url-</code>, а их значения - значения в этих атрибутах. Таким образом, мы можем передать в словарь PageUrlValues произвольное количество значений.</p>
<p>Далее все эти значения мы можем использовать, к примеру, для создания навигационных ссылок:</p>
<pre class="brush:c#;">
PageUrlValues["page"] = pageNumber;
link.Attributes["href"] = urlHelper.Action(PageAction, PageUrlValues);
</pre>
...
<img src="./pics/11.18.png" alt="Tag-хелпер для постраничной навигации с сортировкой и фильтрацией в ASP.NET Core MVC и C#" />
...
<div class="nav"><p><a href="./11.9.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
