...
<html >
<head>
<title>ASP.NET Core MVC и C# | Создание атрибута валидации. Самовалидация модели</title>
<meta charset="utf-8" />
<meta name="description" content="Создание атрибута валидации. Самовалидация модели в ASP.NET Core MVC и C#, типы IValidatableObject и ValidationAttribute">
...
<h2>Создание атрибута валидации. Самовалидация модели</h2><div class="date">Последнее обновление: 08.04.2022</div>
...
<p>Встроенные атрибуты валидации охватывают большую часть возможных ситуаций, тем не менее этих атрибутов может оказаться недостаточно, особенно в каких-то 
специфических сценариях. Однако .NET позволяет определять собственные атрибуты валидации со своей логикой.</p>
<p>Для создания атрибута валидации необходимо унаследовать класс атрибута от <span class="b">ValidationAttribute</span> и переопределить его метод <span class="b">IsValid()</span>.</p>
<p>Допустим, мы хотим, чтобы вводимое имя пользователя было ограничено некоторым диапазоном имен:</p>
<pre class="brush:c#;">
...
<p>Чтобы применить логику валидации, надо переопределить метод IsValid, предоставленный базовым классом. Параметр этого метода как раз и представляет то значение, 
которое надо валидировать.</p>
<p>Логика атрибута довольно проста - мы получаем массив допустимых имен. А при получении значения валидируемого свойства проверяем, имеется ли оно в этом 
массиве авторов, в зависимости результата возвращаем true или false.</p>
...
<p>Допустим, представление для ввода данных выглядит следующим образом:</p>
<pre class="brush:xml;">
...
<p>И после отправки имени, которое не входит в разрешенный диапазон, мы получим ошибку валидации:</p>
<img src="./pics/9.16.png" alt="Создание атрибута валидации в ASP.NET Core MVC и C#" />
<h3>Атрибуты валидации на уровне модели</h3>
<p>Атрибуты валидации на уровне модели применяются при проверке комбинации свойств. Например, мы не хотим, чтобы имя и пароль пользователя совпадали:</p>
<pre class="brush:c#;">
using System.ComponentModel.DataAnnotations;
...
<p>В данном случае параметр value уже будет представлять всю модель Person в целом. И если переданное значение действительно представляет объект Person и при этом значения его свойств 
Name и Password не равны, то возвращается true. А это значит, что объект Person прошел валидацию.</p>
<p>И теперь мы можем применить этот атрибут ко всей модели:</p>
<pre class="brush:c#;">
[NamePasswordEqual]
public class Person
{
...
<p>Для этого класс модели должен реализовать интерфейс <span class="b">IValidatableObject</span>:</p>
<pre class="brush:c#;">
...
<p>В данном случае нам надо реализовать метод <span class="b">Validate()</span> и возвратить коллекцию объектов <span class="b">ValidationResult</span>, которые и будут содержать все ошибки валидации. 
...
будут считаться ошибками для всей модели, а ошибка для Name будет ошибкой только этого конкретного свойства:</p>
<img src="./pics/9.17.png" alt="ValidationAttribute в ASP.NET Core" />
