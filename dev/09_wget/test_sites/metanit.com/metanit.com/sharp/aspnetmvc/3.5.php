...
<html >
<head>
<title>ASP.NET Core MVC и C# | Файл _ViewImports.cshtml</title>
<meta charset="utf-8" />
<meta name="description" content="Файл _ViewImports.cshtml в ASP.NET Core MVC и C#, подключение в представления пространств имен">
...
<h2>Файл _ViewImports.cshtml</h2><div class="date">Последнее обновление: 23.03.2022</div>
...
<p>Файл <span class="b">_ViewImports.cshtml</span> позволяет по умолчанию подключить в представления некоторый функционал. Сначала рассмотрим ситуацию, с которой мы можем столкнуться. 
Пусть у нас в проекте есть некоторый класс <span class="b">Person:</span></p>
<pre class="brush:c#;">
namespace MvcApp
{
...
<p>Допустим, мы хотим использовать тип Person в представлении Index.cshtml:</p>
<pre class="brush:c#;">
...
<p>Чтобы использовать тип Person в представлении, мы вынуждены импортировать с помощью директивы <code>using</code> пространство имен, где этот тип определен. 
...
увеличит работу, если будут какие-то изменения. Файл <span class="b">_ViewImports.cshtml</span> решает эту проблему</p>
<p>Итак, добавим в проект в папку <span class="b">Views</span> файл <span class="b">_ViewImports.cshtml</span>. Для его добавления в Visual Studio можно применять шаблон 
<span class="b">Razor View Imports</span>:</p>
<img src="./pics/3.19.png" alt="ViewImport in ASP.NET Core MVC и C#" />
<p>Далее добавим в этот файл подключение пространства имен класса Person:</p>
<pre class="brush:c#;">
@using MvcApp @* Подключаем пространство имен класса Person *@
</pre>
<img src="./pics/3.17.png" alt="файл _ViewImports.cshtml в ASP.NET Core MVC и C#" /> 
<p>В этом случае функциональность пространства имен <code>MvcApp</code> будет автоматически подключаться во все представления.</p>
<p>И тогда мы можем убрать из представления Index.cshtml подключение пространства имен:</p>
<pre class="brush:c#;">
@{
    Layout = null;
    Person tom = new Person("Tom", 37);
...
<img src="./pics/3.18.png" alt="файл _ViewImports.cshtml и подключение в представления ASP.NET Core MVC и C#" /> 
<p>Для каждой группы представлений в одной папке мы можем определить свой файл <span class="ii">_ViewImports.cshtml</span>. Например, если мы хотим отдельно подключать ряд пространств имен 
в только в представления контроллера HomeController, тогда нам надо добавить файл _ViewImports.cshtml в каталог <span class="ii">Views/Home</span>:</p>
<img src="./pics/3.20.png" alt="ViewImport in ASP.NET Core MVC и C#" />
<p>И в этмо случае все директивы и выражения из файла <span class="ii">Views/Home/_ViewImports.cshtml</span> будут применяться к представлениям только из папки Views/Home. 
