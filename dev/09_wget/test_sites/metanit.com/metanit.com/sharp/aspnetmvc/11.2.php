...
<html >
<head>
<title>ASP.NET Core MVC и C# | Добавление и вывод данных</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с базой данных MS SQL Server в ASP.NET Core MVC и C# через Entity Framework, добавление и получение данных из бд">
...
<h2>Добавление и вывод данных</h2><div class="date">Последнее обновление: 11.04.2022</div>
...
<p>Продолжим работу с проектом из прошлой темы и определим в контроллере три метода, которые будут добавлять новый объект в базу данных и выводить из нее все объекты:</p>
<pre class="brush:c#;">
...
<p>В методе <code>Index()</code> с помощью вызова <code>db.Users.ToListAsnc()</code> мы будем получать объекты из бд, создавать из них список и передавать в представление.</p>
<p>А в post-методе <code>Create()</code> при помощи вызова <code>db.Users.Add()</code> для данных из объекта user формируется sql-выражение <code>INSERT</code>, а вызов 
<code>db.SaveChangesAsync()</code> выполняет это выражение, тем самым добавляя данные в базу данных.</p>
<p>Далее для хранения представлений добавим в проект папку <span class="b">Views</span>, в нее - папку <span class="b">Home</span>. Затем в папку 
<span class="b">Views/Home</span> добавим новое представление <span class="b">Create.cshtml</span>:</p>
<pre class="brush:xml;">
...
<p>И также добавим в папку <span class="b">Views/Home</span> представление <span class="b">Index.cshtml</span>, которое будет отвечать за вывод объектов:</p>
<pre class="brush:xml;">
...
<p>И также добавим в папку <span class="b">Views/Home</span> новое представление <span class="b">_ViewImports.cshtml</span>:</p>
<pre class="brush:c#;">
@addTagHelper *, Microsoft.AspNetCore.Mvc.TagHelpers
</pre>
<p>Оно будет подключать tag-хелперы в представления.</p>
<p>Итоговая структура проекта:</p>
<img src="./pics/11.4.png" alt="Работа с данными в Entity Framework в ASP.NET Core MVC и C#" />
<p>Запустим приложение и обратимся к методу Create:</p>
<img src="./pics/11.2.png" alt="Добавление данных в Entity Framework в ASP.NET Core MVC и C#" />
<p>Введем в форму какие-либо данные и нажмем на копку:</p>
<img src="./pics/11.3.png" alt="Получение данных их базы данных в Entity Framework в ASP.NET Core MVC и C#" />
<p>После выполнения операций с данными мы можем найти автоматически сгенерированную базу данных в окне SQL Server Object Explorer и посмотреть все те данные, которые она содержит:</p>
<img src="./pics/11.6.png" alt="просмотр базы данных MS SQL Server в Entity Framework в ASP.NET Core MVC и C#" />
