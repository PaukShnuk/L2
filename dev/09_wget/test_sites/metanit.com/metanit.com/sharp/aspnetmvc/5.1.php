...
<html >
<head>
<title>ASP.NET Core MVC и C# | Модели</title>
<meta charset="utf-8" />
<meta name="description" content="Модели в ASP.NET Core MVC и C#, определение и типы моделей, анемичные модели, Rich Domain Model, Fat Model и Thick Model, концепция View Model">
...
<h1>Модели</h1><h2>Введение в определение и применение моделей</h2><div class="date">Последнее обновление: 26.03.2022</div>
...
<p>Все модели оформляются как обычные POCO-классы (plain-old CRL objects), то есть обычные классы на языке C#. Например, если мы работаем с данными пользователей, 
то мы могли бы определить в проекте следующую модель, которая представляет пользователя:</p>
<pre class="brush:c#;">
public class Person
{
...
<p>Модель Person определяет ряд свойств: уникальный идентификатор Id, имя и возраст пользователя. Это классическая <span class="b">анемичная модель</span>. Анемичная модель 
не имеет поведения и хранит только состояние в виде свойств.</p>
<p>В языке C# для представления подобных моделей удобно использовать классы record:</p>
<pre class="brush:c#;">
...
"толстыми" моделями (Rich Domain Model / Fat Model / Thick Model). Например, мы можем уйти от анемичной модели, добавив к ней какое-нибудь поведение:</p>
<pre class="brush:c#;">
public class Person
{
    public int Id { get; set; }
...
<p>Например, создадим новый проект ASP.NET Core по типу <span class="b">ASP.NET Core Empty</span>. Добавим в него папку <span class="b">Models</span>, в которую 
добавим новый класс Person</p>
<pre class="brush:c#;">
...
<p>Эта модель будет описывать данные, которые мы будем использовать. Эти данные могли бы храниться в базе данных, но для простоты мы определим их в контроллере.</p>
<p>Далее добавим в проект папку <span class="b">Controllers</span>, а в нее новый класс - <span class="b">HomeController</span> со следующим кодом:</p>
<pre class="brush:c#;">
...
<p>Список объектов модели Person передается в представление с помощью метода <code>View()</code>.</p>
<p>И в конце добавим в проект папку <span class="b">Views</span>, а в нее - каталог <span class="b">Home</span>. Далее в папку <span class="b">Views/Home</span> 
добавим представление <span class="b">Index.cshtml</span>, которое будет выводить все объекты:</p>
<pre class="brush:c#;">
@using MvcApp.Models
...
<img src="./pics/5.1.png" alt="Модели в ASP.NET Core MVC и C#" />
<p>И при обращении к приложению веб-станица выведет список моделей:</p>
<img src="./pics/5.2.png" alt="Вывод моделей в представлении ASP.NET MVC Core и C#" />
