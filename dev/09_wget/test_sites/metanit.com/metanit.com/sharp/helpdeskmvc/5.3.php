...
<html >
<head>
<title>Создание категорий заявок в HelpDesk</title>
<meta charset="utf-8" />
<meta name="description" content="Категории у нас обозначают тип прецедента. То есь классифицируют проблему по типу">
...
<h2>Создание категорий заявок</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Категории у нас обозначают тип прецедента. То есть классифицируют проблему по типу, например, это проблема с сетью, проблема с оборудованием, 
проблема с программным обеспечением или просто в кружке закончился кофе.</p>
<p>Добавим в контроллер ServiceController методы, связанные с моделью Category:</p>
<pre class="brush:c#;">
...
<p>Как вы наверное заметили, основной функционал для работы с большинством моделей однотипен: просмотр, удаление, добавление, редактирование. Теперь 
добавим представление для метода Categories:</p>
<pre class="brush:xml;">
@model Category
...
<p>Также можем создать несколько категорий для последующего использования:</p>
<img src="pics/17.png" />
<p>Итак, инфраструктура работы со справочниками и вспомогательными моделями создана, теперь мы можем переходить к центральной части системы - работе с заявками.</p>
