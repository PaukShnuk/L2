...
<html >
<head>
<title>Удаление заявок и загрузка файлов в HelpDesk</title>
<meta charset="utf-8" />
<meta name="description" content="Удаление заявок в приложении helpdesk на asp.net mvc, проверка прав на удаление, загрузка прикрепленных к заявке файлов">
...
<h2>Удаление заявок и загрузка файлов</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>Удаление заявок</h3>
<p>На представлении Index у нас уже есть ссылки на метод Delete, который будет удалять заявки. Так, теперь добавим этот метод в контроллер:</p>
<pre class="brush:c#;">
// Удаление заявки
...
<p>Во-первых, любой зарегистрированный пользователь может просто обратиться к данному методу, например, с помощью запроса 
<i>Request/Delete/5</i>. Но так как пятая заявка может не принадлежать данному пользователю, то мы вначале вводим проверку на принадлежность заявки, 
чтобы предотвратить несанкционированное удаление.</p>
<p>Во-вторых, поскольку при организации базы данных у нас таблица объектов жизненного цикла и таблица заявок связаны через внешний ключ, который 
применяет каскадное удаление, то, естественно, нам можно просто удалить объект жизненного цикла, а там по внешнему ключу удалится и связанный с ним 
объект заявки.</p>
<h3>Функция загрузки файлов</h3>
<p>Функция загрузки сохраненных файлов с ошибками будет тривиальна. Добавим в контроллер следующий метод:</p>
<pre class="brush:c#;">
// загружаем файл
...
<p>Объект Request у нас сохраняет в бд краткое имя файла без полного указания пути. Если файл не был прикреплен к заявке, то значение свойства 
File будет равно null. В случае если оно определено, получаем полный путь файла в приложении через метод <code>Server.MapPath("~/Files/" + r.File);</code>. 
Так как предполагается, что файлы у нас в приложении будут сохраняться в папку Files, то мы ее название присоединяем к имени файла для получения полного пути.</p>
<p>Далее получаем расширение файла и в зависимости от него устанавливаем заголовок contentType. И затем метод <code>File</code> возвращает пользователю 
данный файл.</p>
