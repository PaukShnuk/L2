...
<html >
<head>
<title>Работа исполнителей в HelpDesk</title>
<meta charset="utf-8" />
<meta name="description" content="Выполнение исполнителями заявок в приложении helpdesk на asp.net mvc, изменение статуса заявок">
...
<h2>Работа исполнителей</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Заключительной частью создание функционала является добавление логики работы исполнителей. Предполагается, что у нас исполнитель производит 
какую-то работу над проблемой и в зависимости от хода работы, изменяет статус заявки. Если у заявки статус 'Распределена', то потом исполнитель, 
приступив к работе, изменяет ее статус на 'В процессе' и т.д. Вплоть до завершения работы.</p>
<p>Итак, добавим в контроллер RequestController метод ChangeStatus:</p>
<pre class="brush:c#;">
...
<p>Здесь мы получаем текущие заявки для данного исполнителя и изменяем их статус.</p>
<pre class="brush:xml;">
...
<p>При выводе заявок на страницу к каждой заявке прикрепляется форма, на которой выпадающий список возможных статусов, причем можно задать не любой статус, а в зависимости от текущего.</p>
<img src="pics/24.png" />
<br>
