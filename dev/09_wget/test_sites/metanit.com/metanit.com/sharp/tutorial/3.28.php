...
<html >
<head>
<title>C# и .NET | Ковариантность и контравариантность делегатов</title>
<meta charset="utf-8" />
<meta name="description" content="Ковариантность и контравариантность делегатов в C# и .NET, ковариантные и контрвариантные обобщенные делегаты">
...
<h2>Ковариантность и контравариантность делегатов</h2><div class="date">Последнее обновление: 01.01.2022</div>
...
<p>Делегаты могут быть ковариантными и контравариантными. <span class="b">Ковариантность</span> делегата 
предполагает, что возвращаемым типом может быть производный тип. <span class="b">Контрвариантность</span> 
делегата предполагает, что типом параметра может быть более универсальный тип.</p>
<p>Рассмотрим ковариантность и контравариантность на примере следующих классов:</p>
<pre class="brush:c#;">
class Message
{
...
<p>В данном случае класс Message представляет некоторое сообщение и определяет свойство Text для хранения текста сообщения, который устанавливается 
...
оба класса является производными от Message.</p>
<h3>Ковариантность</h3>
<p>Ковариантность позволяет передать делегату метод, возвращаемый тип которого является производный от возвращаемого типа делегат. 
То есть если возвращаемый тип делегата Message, то у метод может иметь в качестве возвращаемого типа класс EmailMessage:</p>
<pre class="brush:c#;">
...
<p>Здесь делегат MessageBuilder возвращает объект Message. Однако благодаря ковариантности данный делегат может указывать на метод, 
который возвращает объект производного типа, например, на метод WriteEmailMessage.</p>
<h3>Контрвариантность</h3>
<p>Контрвариантность позволяет присваить делегату метод, тип параметра которого является более универсальным по отношению к типу параметра 
делегата. Например, возьмем выше определенные классы Message и EmailMessage и используем их в следующем примере:</p>
<pre class="brush:c#;">
...
<p>Несмотря на то, что делегат в качестве параметра принимает объект EmailMessage, ему можно присвоить метод, у которого параметр представляет базовый тип Message. 
...
производного. Однако в реальности в делегат при его вызове мы все равно можем передать только объекты типа EmailMessage, а любой объект типа EmailMessage является объектом типа Message, который используется в методе.</p>
<h3>Ковариантность и контравариантность в обобщенных делегатах</h3>
<p>Обобщенные делегаты также могут быть ковариантными и контравариантными, что дает нам больше гибкости в их использовании.</p>
<p>Например, объявим и используем <span class="b">ковариантный обобщенный делегат</span>:</p>
<pre class="brush:c#;">
...
<p>Благодаря использованию <span class="b">out</span> мы можем присвоить делегату типа <code>MessageBuilder&lt;Message&gt;</code> делегат типа 
<code>MessageBuilder&lt;EmailMessage&gt;</code> или ссылку на метод, который возвращает объект EmailMessage.</p>
<p>Рассмотрим <span class="b">контравариантный обобщенный делегат</span>:</p>
<pre class="brush:c#;">
...
<p>Использование ключевого слова <span class="b">in</span> позволяет присвоить делегату с производным типом (<code>MessageReceiver&lt;EmailMessage&gt;</code>) 
делегат с базовым типом (<code>MessageReceiver&lt;Message&gt;</code>) или метод, который принимает в качестве параметра объект базового типа.</p>
<p>Как и в случае с обобщенными интерфейсами параметр ковариантного типа применяется только к типу значения, которые возвращается делегатом. А 
параметр контравариантного типа применяется только к параметрам делегата.</p>
<p>Причем делегат может одновеменно использовать оба оператора: <span class="b">in</span> и <span class="b">out</span>. Например:</p>
<pre class="brush:c#;">
...
<p>Здесь делегат MessageConverter представляет условное действие, которое конвертирует объект типа M в тип E.</p>
<p>В программе определена переменная converter, которая представляет тип <code>MessageConverter&lt;SmsMessage, Message&gt;</code> - то есть конвертер из типа SmsMessage в любой тип Message, грубо говоря преобразует смс в любой другой тип сообщения.</p>
<p>Этой переменной можно передать метод ConvertToEmail, который из сообщений любого типа создает объект Email-сообщения. Здесь применяется контравариантность: для параметра вместо производного типа SmsMessage 
применяется базовый тип Message. И также есть ковариантность: вместо возвращаемого типа Message используется производный тип EmailMessage.</p>
...
<ul>
<li><a title="Вопросы для самопроверки" href="../questions/2.18.php"><i class="fa fa-lg fa-question"></i><span class="dop-txt">Вопросы для самопроверки</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./3.14.php">Назад</a><a href="./">Содержание</a><a href="./3.33.php">Вперед</a></p></div></div>
<div class="socBlock">
