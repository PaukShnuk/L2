...
<html >
<head>
<title>C# и .NET | Применение рефлексии и исследование типов</title>
<meta charset="utf-8" />
<meta name="description" content="Применение рефлексии в языке программирования C# и .NET для исследования типов, получение методов, свойств, полей класса, метод GetMembers, класс MemberInfo">
...
<h2>Применение рефлексии и исследование типов</h2><div class="date">Последнее обновление: 17.02.2022</div>
...
<h3>Получение всех компонентов типа</h3>
<p>Метод <span class="b">GetMembers()</span> возвращает все доступные компоненты типа в виде объекта <span class="b">MemberInfo</span>. Этот объект 
позволяет извлечь некоторую информацию о компоненте типа. В частности, некоторые его свойства:</p>
<ul>
<li><p><span class="b">DeclaringType</span>: возвращает полное название типа.</p></li>
<li><p><span class="b">MemberType</span>: возвращает значение из перечисления <code>MemberTypes</code>:</p>
<ul>
<li><p><span class="b">MemberTypes.Constructor</span></p></li>
<li><p><span class="b">MemberTypes.Method</span></p></li>
<li><p><span class="b">MemberTypes.Field</span></p></li>
<li><p><span class="b">MemberTypes.Event</span></p></li>
<li><p><span class="b">MemberTypes.Property</span></p></li>
<li><p><span class="b">MemberTypes.NestedType</span></p></li>
</ul>
<li><p><span class="b">Name</span>: возвращает название компонента</p></li>
</ul>
<p>Применим метод <code>GetMembers</code> и выведем все доступные элементы типа:</p>
<pre class="brush:c#;">
...
<p>В данном случае мы получим все общедоступные члены класса Person.</p>
<div class="console">
<pre class="consoletext">
Person Method get_Age
Person Method set_Age
Person Method Print
Person Method SayMessage
...
<p>Обратите внимание, что в данном случае мы получаем только все публичные компоненты класса, и нам не выводится информация о приватной переменной name.</p>
<p>Кроме того, для свойства выводятся методы доступа - геттер (здесь get_Age) и сеттер (здесь set_Age).</p>
<p>Третий момент, который надо отметить, что по умолчанию мы получаем весь функционал, в том числе унаследованный от базовых классов (в данном случае 
функционал базового класса Object).</p>
<h3>BindingFlags</h3>
<p>В примере выше использовалась простая форма метода <code>GetMembers()</code>, которая извлекает все общедоступные публичные методы. Но мы можем использовать 
и другую форму метода: <code>MembersInfo[] GetMembers(BindingFlags)</code>. Перечисление <span class="b">BindingFlags</span> может принимать различные значения:</p>
<ul>
<li><p><span class="b">DeclaredOnly</span>: получает только методы непосредственно данного класса, унаследованные методы не извлекаются</p></li>
<li><p><span class="b">Instance</span>: получает только методы экземпляра</p></li>
<li><p><span class="b">NonPublic</span>: извлекает не публичные методы</p></li>
<li><p><span class="b">Public</span>: получает только публичные методы</p></li>
<li><p><span class="b">Static</span>: получает только статические методы</p></li>
</ul>
<p>Объединяя данные значения с помощью побитовой операции ИЛИ можно комбинировать вывод. Например, получим только компоненты непосредственно самого класса без унаследованных, как 
...
<p>И в данном случае мы получим несколько другой вывод:</p>
<div class="console">
<pre class="consoletext">
Person Method get_Age
Person Method set_Age
...
<h3>Получение одного компонента по имени</h3>
<p>Для получения одного компонента можно использовать метод <code>GetMember()</code>, в который передается имя компонента. И опционально можно передать флаги BindingFlags.</p>
<pre class="brush:c#;">
Type myType = typeof(Person);
...
<p>Стоит отметить, что при получении одного члена типа опять же возвращается массив MemberInfo[], поскольку в классе может быть несколько элементов с одним именем, 
например, несколько перегруженных версий метода Print.</p>
...
<div class="nav"><p><a href="./14.1.php">Назад</a><a href="./">Содержание</a><a href="./14.5.php">Вперед</a></p></div></div>
<div class="socBlock">
