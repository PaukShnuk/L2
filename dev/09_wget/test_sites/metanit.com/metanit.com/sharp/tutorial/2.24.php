...
<html >
<head>
<title>C# и .NET | Условные выражения</title>
<meta charset="utf-8" />
<meta name="description" content="Условные выражения в языке программирования C# и платформе .NET, операции сравнения и логические операции, конъюнкция, дизъюнкция, логическое умножение и отрицание">
...
<h2>Условные выражения</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Отдельный набор операций представляет условные выражения. Такие операции возвращают логическое значение, то есть значение типа <span class="b">bool</span>: 
<span class="b">true</span>, если выражение истинно, и <span class="b">false</span>, если выражение ложно. К подобным операциям относятся операции сравнения и логические операции.</p>
<h3>Операции сравнения</h3>
<p>В операциях сравнения сравниваются два операнда и возвращается значение типа <span class="b">bool</span> - <span class="b">true</span>, если выражение верно, 
и <span class="b">false</span>, если выражение неверно.</p>
<ul>
<li><p><span class="b">==</span></p><p>Сравнивает два операнда на равенство. Если они равны, то операция возвращает <span class="b">true</span>, 
если не равны, то возвращается <span class="b">false</span>:</p>
<pre class="brush:c#;">
int a = 10;
int b = 4;
bool c = a == b; // false
</pre></li>
<li><p><span class="b">!=</span></p><p>Сравнивает два операнда и возвращает true, если операнды не равны, и false, если они равны.</p>
<pre class="brush:c#;">
int a = 10;
int b = 4;
bool c = a != b; 	// true
...
<p>Операция "меньше чем". Возвращает true, если первый операнд меньше второго, и false, если первый операнд больше второго:</p>
<pre class="brush:c#;">
int a = 10;
int b = 4;
bool c = a &lt; b; // false
...
<li><p><span class="b">&gt;</span></p>
<p>Операция "больше чем". Сравнивает два операнда и возвращает true, если первый операнд больше второго, иначе возвращает false:</p>
<pre class="brush:c#;">
int a = 10;
int b = 4;
bool c = a &gt; b;     // true
...
<li><p><span class="b">&lt;=</span></p>
<p>Операция "меньше или равно". Сравнивает два операнда и возвращает true, если первый операнд меньше или равен второму. Иначе возвращает false.</p>
<pre class="brush:c#;">
int a = 10;
int b = 4;
bool c = a &lt;= b;     // false
...
<li><p><span class="b">&gt;=</span></p>
<p>Операция "больше или равно". Сравнивает два операнда и возвращает true, если первый операнд больше или равен второму, иначе возвращается false:</p>
<pre class="brush:c#;">
int a = 10;
int b = 4;
bool c = a &gt;= b;     // true
...
<p>Также в C# определены логические операторы, которые также возвращают значение типа <span class="b">bool</span>. В качестве операндов они принимают значения типа <span class="b">bool</span>. Как правило, применяются к отношениям и объединяют несколько операций сравнения.</p>
<ul>
<li><p><span class="b">|</span></p>
<p>Операция логического сложения или логическое ИЛИ. Возвращает true, если хотя бы один из операндов возвращает true.</p>
<pre class="brush:c#;">
...
<li><p><span class="b">&amp;</span></p>
<p>Операция логического умножения или логическое И. Возвращает true, если оба операнда одновременно равны true.</p>
<pre class="brush:c#;">
...
<li><p><span class="b">||</span></p>
<p>Операция логического сложения. Возвращает true, если хотя бы один из операндов возвращает true.</p>
<pre class="brush:c#;">
bool x1 = (5 &gt; 6) || (4 &lt; 6); // 5 &gt; 6 - false, 4 &lt; 6 - true, поэтому возвращается true
bool x2 = (5 &gt; 6) || (4 &gt; 6); // 5 &gt; 6 - false, 4 &gt; 6 - false, поэтому возвращается false
</pre>
...
<li><p><span class="b">&amp;&amp;</span></p>
<p>Операция логического умножения. Возвращает true, если оба операнда одновременно равны true.</p>
<pre class="brush:c#;">
bool x1 = (5 &gt; 6) &amp;&amp; (4 &lt; 6); // 5 &gt; 6 - false, 4 &lt; 6 - true, поэтому возвращается false
bool x2 = (5 &lt; 6) &amp;&amp; (4 &lt; 6); // 5 &lt; 6 - true, 4 &lt; 6 - true, поэтому возвращается true
</pre>
...
<li><p><span class="b">!</span></p>
<p>Операция логического отрицания. Производится над одним операндом и возвращает true, если операнд равен false. Если операнд равен true, то операция возвращает false:</p>
<pre class="brush:c#;">
bool a = true;
bool b = !a;    // false
</pre>
...
<li><p><span class="b">^</span></p>
<p>Операция исключающего ИЛИ. Возвращает true, если либо первый, либо второй операнд (но не одновременно) равны true, иначе возвращает false</p>
<pre class="brush:c#;">
...
<p>Здесь у нас две пары операций <code>|</code> и <code>||</code> (а также <code>&amp;</code> и <code>&amp;&amp;</code>) выполняют 
похожие действия, однако же они не равнозначны.</p>
<p>В выражении <code>z=x|y;</code> будут вычисляться оба значения - x и y.</p><p>В выражении же <code>z=x||y;</code> сначала будет вычисляться значение x, 
и если оно равно <code>true</code>, то вычисление значения y уже смысла не имеет, так как у нас в любом случае уже z будет равно <code>true</code>. Значение 
y будет вычисляться только в том случае, если x равно <code>false</code></p>
<p>То же самое касается пары операций <code>&amp;/&amp;&amp;</code>. В выражении <code>z=x&amp;y;</code> будут вычисляться оба значения - x и y.</p>
<p>В выражении же <code>z=x&amp;&amp;y;</code> сначала будет вычисляться значение x, 
и если оно равно <code>false</code>, то вычисление значения y уже смысла не имеет, так как у нас в любом случае уже z будет равно <code>false</code>. 
Значение y будет вычисляться только в том случае, если x равно <code>true</code></p>
<p>Поэтому операции <code>||</code> и <code>&amp;&amp;</code> более удобны в вычислениях, так как позволяют сократить время на вычисление значения выражения, и тем самым повышают 
производительность. А операции <code>|</code> и <code>&amp;</code> больше подходят для выполнения поразрядных операций над числами.</p>
...
<div class="nav"><p><a href="./2.2.php">Назад</a><a href="./">Содержание</a><a href="./2.5.php">Вперед</a></p></div></div>
<div class="socBlock">
