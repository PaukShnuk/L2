...
<html >
<head>
<title>C# и .NET | Индексы и диапазоны</title>
<meta charset="utf-8" />
<meta name="description" content="Индексы и диапазоны System.Index и System.Range в языке C# и .NET">
...
<h2>Индексы и диапазоны</h2><div class="date">Последнее обновление: 07.01.2022</div>
...
<p>В C# 8.0 была добавлена новая функциональность - индексы и диапазоны, которые упрощают получение из массивов подмассивов. 
Для этого в C# есть два типа: <span class="b">System.Range</span> и <span class="b">System.Index</span>. Оба типа являются структурами. Тип Range представляет некоторый диапазон значений 
в некоторой последовательность, а тип Index - индекс в последовательности.</p>
<h3>Индексы</h3>
<p>Индекс фактически представляет числовое значение, и при определении индекса мы можем указать это значение:</p>
<pre class="brush:c#;">
Index myIndex = 2;
</pre>
<p>В данном случае индекс представляет третий элемент последовательности (индексация начинается с 0).</p>
<p>С помощью специального оператора <span class="b">^</span> можно задать индекс относительно конца последовательности.</p>
<pre class="brush:c#;">
Index myIndex = ^2;
</pre>
<p>Теперь индекс представляет второй элемент с конца последовательности, то есть предпоследний элемент.</p>
<p>Используем индексы для получения элементов массива:</p>
<pre class="brush:c#;">
...
<p>Фактически для данной задачи индексы не нужны, и мы можем воспользоваться стандартными возможностями массивов:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Bob", "Sam", "Kate", "Alice" };
...
<p>То есть в подобных ситуациях плюсом индексов является большая удобочитаемость. Так, <code>people[^2]</code> более читабельно, чем <code>people[people.Length - 2]</code>.</p>
<h3>Диапазон</h3>
<p>Диапазон представляет часть последовательности, которая ограничена двумя индексами. Начальный индекс включается в диапазон, а конечный индекс НЕ входит в диапазон. 
Для определения диапазона применяется оператор <span class="b">..</span>:</p>
<pre class="brush:c#;">Range myRange1 = 1..4; // по 1-го индекса включая по 4-й индекс не включая</pre>
<p>В данном случае диапазон myRange1 влючает элементы с 1 индекса по 4-й индекс (не включая). При этом элемент по 4-му индексу не 
включается в диапазон. При этом границы диапазона задаются не просто числами, а именно объектами Index. То есть следующие определения диапазонов будут равноценны:</p>
<pre class="brush:c#;">
Index start = 1;
Index end = 4;
...
<p>Практическое применение диапазонов - получим со второго по четвертый элементы массива:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Bob", "Sam", "Kate", "Alice" };
...
<p>Результатом операции <code>people[1..4]</code> является подмассив элементов с 1 по 3 индексы (включая). Консольный вывод:</p>
<div class="console">
...
<p>Мы можем задать для диапазона только конечный индекс. В этом случае начальным индексом по умолчанию будет 0.</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Bob", "Sam", "Kate", "Alice" };
string[] peopleRange = people[..4];     // Tom, Bob, Sam, Kate
</pre>
<p>Либо, наоборот, задать только начальный индекс, тогда конечным индексом будет последний индекс последовательности:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Bob", "Sam", "Kate", "Alice" };
string[] peopleRange = people[1..];     // Bob, Sam, Kate, Alice
</pre>
<p>Используя индексы относительно конца последовательности, можно получать диапазон относительно конца последовательности:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Bob", "Sam", "Kate", "Alice" };
...
<p>Кроме массивов индексы и диапазоны также применяются к объектам Span и ReadOnlySpan:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Bob", "Sam", "Kate", "Alice" };
...
<div class="nav"><p><a href="./2.33.php">Назад</a><a href="./">Содержание</a><a href="./11.1.php">Вперед</a></p></div></div>
<div class="socBlock">
