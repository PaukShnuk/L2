...
<html >
<head>
<title>C# и .NET | Создание своих атрибутов валидации</title>
<meta charset="utf-8" />
<meta name="description" content="Создание своих атрибутов валидации в C# и .NET в System.ComponentModel.DataAnnotations,  класс ValidationAttribute">
...
<h2>Создание своих атрибутов валидации</h2><div class="date">Последнее обновление: 23.02.2022</div>
...
<p>Несмотря на то, что .NET предоставляет нам большой набор встроенных атрибутов валидации, может потребоваться более 
изощренные в плане логики атрибуты. И в этом случае мы можем определить свои классы атрибутов.</p>
<p>Для создания атрибута нам надо унаследовать свой класс от класса <span class="b">ValidationAttribute</span> и реализовать его метод <code>IsValid()</code>:</p>
<p>При создании атрибута надо понимать, к чему именно он будет применяться - к свойству модели или ко всей модели в целом. Рассмотрим обе ситуации.</p>
<h3>Атрибут уровня свойства</h3>
<p>Определим следующий класс:</p>
<pre class="brush:c#;">
...
<p>Класс атрибута наследуется от класса <span class="b">ValidationAttribute</span> и реалует его метод <code>IsValid()</code>. В данный метод передается значение, которое валидируется.</p>
<p>Данный атрибут будет применяться к строковому свойству, поэтому в метод <code>IsValid(object? value)</code> в качестве value будет передаваться строка. 
Поэтому в ходе программы мы можем преобразовать значение <code>value</code> к строке:</p>
<pre class="brush:c#;">if (value is string userName)</pre>
<p>Далее в методе мы проверяем, равно ли значение свойства строке "admin". Если <span class="b">НЕ</span> равно, то возвращаем true. Это значит, что 
свойство, к которому будет применяться данный атрибут, прошло валидацию. Если же строка равна "admin", то возвращаем false и 
устанавливаем сообщение об ошибке. Если же в метод передана не строка, также возвращаем false.</p>
<p>Применим данный атрибут к свойству класса:</p>
<pre class="brush:c#;">
...
<p>В данном случае атрибут UserName применяется к свойству Name класса User:</p>
<pre class="brush:c#;">
[UserName]
public string Name { get; set; }
</pre>
<p>Также, как и другие атрибуты, мы можем использовать эти атрибуты без суффикса <span class="b">Attribute</span>.</p>
<p>В методе Validate валидируем объект User и выводим ошибки валидации на консоль. Поэтому при вызове <code>Validate(new User("admin", 37))</code> 
объект User не пройдет валидацию, так как значени "admin" не соответствует правилам атрибута UserName. И в данном случае мы получим следующий консольный вывод:</p>
<div class="console">
...
<h3>Атрибуты валидации уровня класса</h3>
<p>Подобным образом определим еще один атрибут, который будет применяться ко всему классу User в целом:</p>
<pre class="brush:c#;">
...
<p>Поскольку атрибут будет применяться ко всей модели, то в метод IsValid в качестве параметра <code>value</code> будет передаваться объект User. Как правило, 
...
"Tom" и 37.</p>
<p>Теперь применим этот атрибут:</p>
<pre class="brush:c#;">
using System.ComponentModel.DataAnnotations;
...
<p>Поскольку атрибут UserValidation должен применяться ко всему классу User, то он указывается непосредственно перед определением класса:</p>
<pre class="brush:c#;">
[UserValidation]
public class User
{
...
<p>В итоге если у объекта User одновременно имя будет "Tom", а возраст - 37, то такой объект User не пройдет валидацию. Консольный вывод:</p>
<div class="console">
<pre class="consoletext">
Пользователь прошел валидацию
...
<h3>Передача в атрибут значений</h3>
<p>Выше оба атрибута сравнивали валидируемое значение с некоторым жестко заданным в коде набором значений. Однако мы можем определить 
через конструктор механизм передачи в атрибут значений из вне. Например, изменим атрибут UserValidation:</p>
<pre class="brush:c#;">
public class UserValidationAttribute : ValidationAttribute
{
...
<p>В данном случае значения, относительно которых будет проводиться валидация, передаются через конструктор и сохраняются в свойствах InvalidAge и InvalidName.</p>
<p>При применении атрибута теперь надо указать значения для параметров конструктора UserValidation:</p>
<pre class="brush:c#;">
