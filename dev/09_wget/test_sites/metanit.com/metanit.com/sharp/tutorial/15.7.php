...
<html >
<head>
<title>C# и .NET | Соединение коллекций в LINQ</title>
<meta charset="utf-8" />
<meta name="description" content="Соединение двух коллекций элементов в LINQ в языке программирования C# и .NET по определенному критерию, использование метода Join, GroupJoin и Zip">
...
<h2>Соединение коллекций</h2><div class="date">Последнее обновление: 14.02.2022</div>
...
<p>Соединение в LINQ используется для объединения двух разнотипных наборов в один. Для соединения используется оператор <span class="b">join</span> 
или метод <span class="b">Join()</span>. Как правило, данная операция применяется к двум наборам, которые имеют один общий критерий.</p>
<h3>Оператор join</h3>
<p>Оператор <span class="b">join</span> имеет следующий формальный синтаксис:</p>
<pre class="brush:c#;">
from объект1 in набор1
join объект2 in набор2 on объект2.свойство2 equals объект1.свойство1
</pre>
<p>После оператора join идет выборка объектов из второй коллекции. После оператора <code>on</code> указывается критерий соединения - 
свойство объекта из второй выборки, а после оператора <code>equals</code> - свойство объекта из первой выборки, которому должно быть равно 
свойство объекта из второй выборки. Если эти свойства равны, то оба объекта попадают в финальный результат.</p>
<p>Например, у нас есть два класса:</p>
<pre class="brush:c#;">
record class Person(string Name, string Company);
record class Company(string Title, string Language);
</pre>
<p>Класс Person представляет пользователя и хранит два свойства: Name (имя) и Company (компания пользователя). Класс Company представляет компанию и 
хранит два свойства: Title (название компании) и Language (основной язык программирования в компании)</p>
<p>Объекты обоих классов будет иметь один общий критерий - название компании. Соединим по этому критерию два набора этих классов:</p>
<pre class="brush:c#;">
Person[] people =
{
...
<p>С помощью выражения</p>
<pre class="brush:c#;">join c in companies on p.Company equals c.Title</pre>
<p>объект p из списка people (то есть объект Person) соединяется с объектом c из списка companies (то есть с объектом Company), 
если значение свойства <code>p.Company</code> совпадает со значением свойства <code>c.Title</code>. 
Результатом соединения будет объект анонимного типа, который будет содержать три свойства. В итоге мы получим следующий вывод:</p>
<div class="console">
...
<p>Обратите внимание, что в массиве people есть объект <code>new Person("Bob", "JetBrains")</code>, но в массиве компаний компании с именем "JetBrains" 
нет, соответственно он не попал с результат. Аналогично в списке people нет объектов Person, которые бы соотствовали компании <code>new Company("Oracle", "Java")</code>.</p>

<h3>Метод Join</h3>
<p>В качестве альтернативы можно было бы использовать метод <class="b">Join()</span>:</p>
<pre class="brush:c#;">
...
<p>Метод <code>Join()</code> принимает четыре параметра:</p>
<ul>
<li><p>второй список, который соединяем с текущим</p></li>
<li><p>делегат, который определяет свойство объекта из текущего списка, по которому идет соединение</p></li>
<li><p>делегат, который определяет свойство объекта из второго списка, по которому идет соединение</p></li>
<li><p>делегат, который определяет новый объект в результате соединения</p></li>
</ul>
<p>Перепишим предыдущий пример с использованием метода Join:</p>
<pre class="brush:c#;">
Person[] people =
{
    new Person("Tom", "Microsoft"), new Person("Sam", "Google"),
...
<h3>GroupJoin</h3>
<p>Метод <span class="b">GroupJoin()</span> кроме соединения последовательностей также выполняет и группировку.</p>
<pre class="brush:c#;">
...
<p>Метод <code>GroupJoin()</code> принимает четыре параметра:</p>
<ul>
<li><p>второй список, который соединяем с текущим</p></li>
<li><p>делегат, который определяет свойство объекта из текущей коллекции, по которому идет соединение и по которому будет идти группировка</p></li>
<li><p>делегат, который определяет свойство объекта из второй коллекции, по которому идет соединение</p></li>
<li><p>делегат, который определяет новый объект в результате соединения. Этот делегат получает группу - объект текущей коллекции, по которому шла группировка, и 
набор объектов из второй коллекции, которые сооставляют группу</p></li>
</ul>
<p>Например, возьмем выше определенные массивы people и companies и сгуппируем всех пользователей по компаниям:</p>
<pre class="brush:c#;">
Person[] people =
{
    new Person("Tom", "Microsoft"), new Person("Sam", "Google"),
...
<p>Результатом выполнения программы будет следующий вывод:</p>
<div class="console">
<pre class="consoletext">
Microsoft
Tom
...
<p>Метод GroupJoin, также как и метод Join, принимает все те же параметры. Только теперь в последний параметр - делегат передаются объект компании и 
набор пользователей этой компании.</p>
<p>Обратите внимание, что для компании "Oracle" в массиве people нет пользователей, хотя для нее также создается группа.</p>
<p>Аналогичного результата можно добитьс и с помощью оператора join:</p>
<pre class="brush:c#;">
...
<h3>Метод Zip</h3>
<p>Метод <span class="b">Zip()</span> последовательно объединяет соответствующие элементы текущей последовательности со второй последовательностью, которая передается 
в метод в качестве параметра. То есть первый элемент из первой последовательности объединяется с первым элементом из 
...
Результатом метода является коллекция кортежей, где каждый кортеж хранит пару соответствующих элементов из обоих последовательностей:</p>
<pre class="brush:c#;">
...
<p>Здесь метод Zip объединяет соответствующие элементы из списков courses и students. В результате создается новая коллекция, которая хранит набор кортежей. Каждый 
кортеж в ней имеет два элемента. Первый элемент из свойства First представляет объект текущей коллекции (в данном случае объект Course), а второй элемент (в свойстве Second) 
хранит объект второй последовательности (в данном случае объект Student). Консольный вывод:</p>
<div class="console">
<pre class="consoletext">
...
<div class="nav"><p><a href="./15.6.php">Назад</a><a href="./">Содержание</a><a href="./15.10.php">Вперед</a></p></div></div>
<div class="socBlock">
