...
<html >
<head>
<title>C# и .NET | Сериализация в JSON. JsonSerializer</title>
<meta charset="utf-8" />
<meta name="description" content="Сериализация в JSON в C# и .NET Core, класс JsonSerializer и его использование, пример сериализации и десериализации объектов в JSON-формат, System.Text.Json">
...
<h1>Работа с JSON</h1><h2>Сериализация в JSON. JsonSerializer</h2><div class="date">Последнее обновление: 20.02.2022</div>
...
<p><span class="b">JSON</span> (JavaScript Object Notation) является одним из наиболее популярных форматов для хранения и передачи данных. 
И платформа .NET предоставляет функционал для работы с JSON.</p>
<p>Основная функциональность по работе с JSON сосредоточена в пространстве имен <span class="b">System.Text.Json</span>. 
Ключевым типом является класс <span class="b">JsonSerializer</span>, который и позволяет сериализовать объект в json и, наоборот, десериализовать код 
json в объект C#.</p>
<p>Для сохранения объекта в json в классе JsonSerializer определен статический метод <span class="b">Serialize()</span> и его асинхронный двойник 
<span class="b">SerializeAsyc()</span>, которые имеют ряд перегруженных версий. Некоторые из них:</p>
<ul>
<li><p><code>string Serialize(Object obj, Type type, JsonSerializerOptions options)</code>: сериализует объект obj типа type и возвращает код json в виде строки. 
Последний необязательный параметр options позволяет задать дополнительные опции сериализации</p></li>
<li><p><code>string Serialize&lt;T&gt;(T obj, JsonSerializerOptions options)</code>: типизированная версия сериализует объект obj типа T и возвращает 
код json в виде строки.</p></li>
<li><p><code>Task SerializeAsync(Stream utf8Json, Object obj, Type type, JsonSerializerOptions options)</code>: сериализует объект obj типа type и записывает 
его в поток utf8Json. Последний необязательный параметр options позволяет задать дополнительные опции сериализации</p></li>
<li><p><code>Task SerializeAsync&lt;T&gt;(Stream utf8Json, T obj, JsonSerializerOptions options)</code>: типизированная версия сериализует объект obj типа T 
в поток utf8Json.</p></li>
</ul>
<p>Для десериализации кода json в объект C# применяется метод <span class="b">Deserialize()</span> и его асинхронный двойник <span class="b">DeserializeAsync()</span>, 
которые имеют различные версии. Некоторые из них:</p>
<ul>
<li><p><code>object? Deserialize(string json, Type type, JsonSerializerOptions options)</code>: десериализует строку json в объект типа type и 
возвращает десериализованный объект. Последний необязательный параметр options позволяет задать дополнительные опции десериализации</p></li>
<li><p><code>T? Deserialize&lt;T&gt;(string json, JsonSerializerOptions options)</code>: десериализует строку json в объект типа T и 
возвращает его.</p></li>
<li><p><code>ValueTask&lt;object?&gt; DeserializeAsync(Stream utf8Json, Type type, JsonSerializerOptions options, CancellationToken token)</code>: 
...
дополнительные опции десериализации, а token устанавливает CancellationToken для отмены задачи. Возвращается десериализованный объект, обернутый в ValueTask</p></li>
<li><p><code>ValueTask&lt;T?&gt; DeserializeAsync&lt;T&gt;(Stream utf8Json, JsonSerializerOptions options, CancellationToken token)</code>: 
десериализует данные из потока utf8Json, который представляет объект JSON, в объект типа T. Возвращается десериализованный объект, обернутый в ValueTask</p></li>
</ul>
<p>Рассмотрим применение класса на простом примере. Сериализуем и десериализуем простейший объект:</p>
<pre class="brush:c#;">
...
<p>Здесь вначале сериализуем с помощью метода <code>JsonSerializer.Serialize()</code> объект типа Person в стоку с кодом json. Затем обратно получаем из этой 
строки объект Person посредством метода <code>JsonSerializer.Deserialize()</code>.</p>
...
<p>Хотя в примере выше сериализовался/десериализовался объект класса, но подобным способом мы также можем сериализовать/десериализовать структуры.</p>
<h3>Некоторые замечания по сериализации/десериализации</h3>
<p>Объект, который подвергается десериализации, должен иметь либо конструктор без параметров, либо конструктор, для всех параметров которого 
...
не играет значения).</p>
<p>Сериализации подлежат только публичные свойства объекта (с модификатором public).</p>
<h3>Запись и чтение файла json</h3>
<p>Поскольку методы SerializeAsyc/DeserializeAsync могут принимать поток типа Stream, то соответственно мы можем использовать файловый поток для сохранения 
и последующего извлечения данных:</p>
<pre class="brush:c#;">
using System.Text.Json;
...
<p>В данном случае вначале данные сохраняются в файл user.json и затем считываются из него.</p>
<h3>Настройка сериализации с помощью JsonSerializerOptions</h3>
<p>По умолчанию JsonSerializer сериализует объекты в минимифицированный код. С помощью дополнительного параметра типа <span class="b">JsonSerializerOptions</span> 
можно настроить механизм сериализации/десериализации, используя свойства JsonSerializerOptions. Некоторые из его свойств:</p>
<ul>
<li><p><span class="b">AllowTrailingCommas</span>: устанавливает, надо ли добавлять после последнего элемента в json запятую. Если равно <code>true</code>, запятая добавляется</p></li>
<li><p><span class="b">DefaultIgnoreCondition</span>: устанавливает, будут ли сериализоваться/десериализоваться в json свойства со значениями по умолчанию</p></li>
<li><p><span class="b">IgnoreReadOnlyProperties</span>: аналогично устанавливает, будут ли сериализоваться свойства, предназначенные только для чтения</p></li>
<li><p><span class="b">WriteIndented</span>: устанавливает, будут ли добавляться в json пробелы (условно говоря, для красоты). Если равно <code>true</code> устанавливаются дополнительные пробелы</p></li>
</ul>
<p>Применение:</p>
<pre class="brush:c#;">
using System.Text.Json;
...
<h3>Настройка сериализации с помощью атрибутов</h3>
<p>По умолчанию сериализации подлежат все публичные свойства. Кроме того, в выходном объекте json все названия свойств соответствуют названиям свойств объекта C#. 
Однако с помощью атрибутов <span class="b">JsonIgnore</span> и <span class="b">JsonPropertyName</span>.</p>
<p>Атрибут <span class="b">JsonIgnore</span> позволяет исключить из сериализации определенное свойство. А <span class="b">JsonPropertyName</span> позволяет замещать оригинальное название свойства. 
...
<p>В данном случае свойство Age будет игнорироваться, а для свойства Name будет использоваться псевдоним "firstname". Консольный вывод:</p>
<div class="console">
<pre class="consoletext">
{"firstname":"Tom"}
Name: Tom   Age: 0
...
<p>Обратите внимание, что, поскольку свойство Age не было сериализовано, то при десериализации для него используется значение по умолчанию.</p>
...
<div class="nav"><p><a href="./5.7.php">Назад</a><a href="./">Содержание</a><a href="./16.1.php">Вперед</a></p></div></div>
<div class="socBlock">
