...
<html >
<head>
<title>C# и .NET | Интерфейсы в обобщениях</title>
<meta charset="utf-8" />
<meta name="description" content="Интерфейсы в обобщениях, уроки по C#, обобщенные интерфейсы, ограничения обобщений">
...
<h2>Интерфейсы в обобщениях</h2><div class="date">Последнее обновление: 02.02.2022</div>
...
<h3>Интерфейсы как ограничения обобщений</h3>
<p>Интерфейсы могут выступать в качестве ограничений обобщений. При этом если в качестве ограничения можно указаь только один класс, то интерфейсов можно указать несколько.</p>
<p>Допустим, у нас есть следующие интерфейсы и класс, который их реализует:</p>
<pre class="brush:c#;">
interface IMessage
{
...
<p>Интерфейс IMessage представляет интерфейс сообщения и определяет свойство Text для хранения текста сообщения. Интерфейс IPrintable определяет метод 
Print для условной печати сообщения. И непосредственно класс сообщения  - класс Message реализует эти интерфейсы.</p>
<p>Используем выше перечисленные интерфейсы в качестве ограничений обобщенного класса:</p>
<pre class="brush:c#;">
...
<p>В данном случае класс условного мессенджера использует параметр T - тип, который который реализует сразу два интерфейса IMessage и IPrintable. 
Например, выше определен класс Message, который реализует оба интерфейса, поэтому мы можем данным типом типизировать объекты Messenger:</p>
<pre class="brush:c#;">
// создаем мессенджер
...
<p>Также параметр T может представлять интерфейс, который наследуется от обоих интерфейсов:</p>
<pre class="brush:c#;">
...
<p>В этом случае объекты Messenger мы можем типизировать типом IPrintableMessage:</p>
<pre class="brush:c#;">
...
<p>Как и классы, интерфейсы могут быть обобщенными:</p>
<pre class="brush:c#;">
interface IUser&lt;T&gt;
{
    T Id { get; }
...
<p>Интерфейс IUser типизирован параметром T, который при реализации интерфейса используется в классе User. В частности, переменная _id определена как T, 
что позволяет нам использовать для id различные типы.</p>
<p>Определим две реализации: одна в качестве параметра будет использовать тип int, а другая - тип string:</p>
<pre class="brush:c#;">
...
<p>Также при реализации интерфейса мы можем явным образом указать, какой тип будет использоваться для параметра T:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./3.48.php">Назад</a><a href="./">Содержание</a><a href="./3.22.php">Вперед</a></p></div></div>
<div class="socBlock">
