...
<html >
<head>
<title>C# и .NET | События</title>
<meta charset="utf-8" />
<meta name="description" content="События в c#, их объявление с помощью ключевого слова event и использование на примере, прикрепление обработчиков событий и их удаление">
...
<h2>События</h2><div class="date">Последнее обновление: 31.12.2021</div>
...
<p><span class="b">События</span> сигнализируют системе о том, что произошло определенное 
...
<p>События объявляются в классе с помощью ключевого слова <span class="b">event</span>, после которого указывается тип делегата, который представляет событие:</p>
<pre class="brush:c#;">
delegate void AccountHandler(string message);
event AccountHandler Notify;
</pre>
<p>В данном случае вначале определяется делегат AccountHandler, который принимает один параметр типа string. Затем с помощью ключевого слова <span class="b">event</span> 
...
<pre class="brush:c#;">if(Notify !=null) Notify("Произошло действие");</pre>
<p>Или так:</p>
<pre class="brush:c#;">Notify?.Invoke("Произошло действие");</pre>
<p>В этом случае поскольку событие представляет делегат, то мы можем его вызвать с помощью метода <span class="b">Invoke()</span>, передав в него необходимые значения 
для параметров.</p>
...
события применяется операция <span class="b">+=</span>:</p>
<pre class="brush:c#;">Notify += обработчик события;</pre>
...
В итоге при вызове события <code>Notify?.Invoke()</code> будет вызываться метод DisplayMessage, которому для параметра message будет передаваться строка, которая передается 
в <code>Notify?.Invoke()</code>. В DisplayMessage просто выводим полученное от события сообщение, но можно было бы определить любую логику.</p>
<p>Если бы в данном случае обработчик не был бы установлен, то при вызове события <code>Notify?.Invoke()</code> ничего не происходило, так как событие Notify было бы равно null.</p>
...
<p>С помощью специальных акссесоров <span class="b">add/remove</span> мы можем управлять добавлением и удалением обработчиков. Как правило, подобная функциональность 
...
<pre class="brush:c#;">AccountHandler notify;</pre>
<p>Во второй части определяем акссесоры add и remove. Аксессор <span class="b">add</span> вызывается при добавлении обработчика, то есть при операции +=. 
Добавляемый обработчик доступен через ключевое слово <span class="b">value</span>. Здесь мы можем получить информацию об обработчике (например, 
...
новый класс <span class="b">AccountEventArgs</span> со следующим кодом:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./3.16.php">Назад</a><a href="./">Содержание</a><a href="./3.28.php">Вперед</a></p></div></div>
<div class="socBlock">
