...
<html >
<head>
<title>C# и .NET | Работа с XML </title>
<meta charset="utf-8" />
<meta name="description" content="Работа с XML в языке программирования C# и .NET, xml-документы, их структура, атрибуты и элементы и сопоставление с классами и структурами">
...
<h1>Работа с XML в C#</h1><h2>XML-документы</h2><div class="date">Последнее обновление: 20.02.2022</div>
...
<p>На сегодняшний день XML является одним из распространенных стандартов документов, который позволяет в удобной форме сохранять сложные по структуре данные. 
Поэтому разработчики платформы .NET включили в фреймворк широкие возможности для работы с XML.</p>
<p>Прежде чем перейти непосредственно к работе с XML-файлами, сначала рассмотрим, что представляет собой xml-документ и как он может хранить объекты, используемые в программе на c#.</p>
<p>Например, у нас есть следующий класс:</p>
<pre class="brush:c#;">
class Person
{
...
<p>В программе на C# мы можем создать список объектов класса Person:</p>
<pre class="brush:c#;">
...
<p>Чтобы сохранить список в формате xml мы могли бы использовать следующий xml-файл:</p>
<pre class="brush:xml;">
...
<p>XML-документ объявляет строка <code>&lt;?xml version="1.0" encoding="utf-8" ?&gt;</code>. Она задает версию (1.0) и кодировку (utf-8) xml. Далее идет 
собственно содержимое документа.</p>
<p>XML-документ должен иметь один единственный корневой элемент, внутрь которого помещаются все остальные элементы. В данном случае таким элементом является 
элемент <code>&lt;people&gt;</code>. Внутри корневого элемента <code>&lt;people&gt;</code> задан набор элементов <code>&lt;person&gt;</code>. Вне корневого элемента 
мы не можем разместить элементы <code>person</code>.</p>
<p>Каждый элемент определяется с помощью открывающего и закрывающего тегов, например, <code>&lt;person&gt;</code> и <code>&lt;/person&gt;</code>, внутри которых 
помещается значение или содержимое элементов. Также элемент может иметь сокращенное объявление: <code>&lt;person /&gt;</code> - в конце элемента помещается слеш.</p>
<p>Элемент может иметь вложенные элементы и атрибуты. В данном случае каждый элемент person имеет два вложенных элемента <code>company</code> и 
<code>age</code> и атрибут <code>name</code>.</p>
<p>Атрибуты определяются в теле элемента и имеют следующую форму: <code>название="значение"</code>. Например, <code>&lt;person name="Bill Gates"&gt;</code>, 
в данном случае атрибут называется <code>name</code> и имеет значение <code>Bill Gates</code></p>
<p>Внутри простых элементов помещается их значение. Например, <code>&lt;company&gt;Google&lt;/company&gt;</code> - элемент <code>company</code> имеет значение 
<code>Google</code>.</p>
<p>Названия элементов являются регистрозависимыми, поэтому <code>&lt;company&gt;</code> и <code>&lt;COMPANY&gt;</code> будут представлять разные элементы.</p>
<p>Таким образом, весь список Users из кода C# сопоставляется с корневым элементом <code>&lt;people&gt;</code>, каждый объект Person - с элементом <code>&lt;person&gt;</code>, 
а каждое свойство объекта Person - с атрибутом или вложенным элементом элемента <code>&lt;person&gt;</code></p>
<p>Что использовать для свойств - вложенные элементы или атрибуты? Это вопрос предпочтений - мы можем использовать как атрибуты, так и вложенные элементы. 
Так, в предыдущем примере вполне можно использовать вместо атрибута вложенный элемент:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;people&gt;
  &lt;person&gt;
...
<p>Теперь рассмотрим основные подходы для работы с XML, которые имеются в C#.</p>
...
<div class="nav"><p><a href="./6.5.php">Назад</a><a href="./">Содержание</a><a href="./16.2.php">Вперед</a></p></div></div>
<div class="socBlock">
