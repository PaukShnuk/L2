...
<html >
<head>
<title>C# и .NET | Массив параметров и ключевое слово params</title>
<meta charset="utf-8" />
<meta name="description" content="Массив параметров в программах на языке C#, применение ключевого слова params">
...
<h2>Массив параметров и ключевое слово params</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Во всех предыдущих примерах мы использовали постоянное число параметров. Но, используя ключевое слово 
<span class="b">params</span>, мы можем передавать неопределенное количество параметров:</p>
<pre class="brush:c#;">
...
<p>Сам параметр с ключевым словом <span class="b">params</span> при определении метода должен представлять одномерный массив того типа, данные которого мы собираемся использовать. 
При вызове метода на место параметра с модификатором <span class="b">params</span> мы можем передать как отдельные значения, так и массив 
значений, либо вообще не передавать параметры. Количество передаваемых значений в метод неопределено, однако все эти значения должны соответствовать типу параметра с <span class="b">params</span>.</p>
<p>Если же нам надо передать какие- то другие параметры, то они должны указываться до параметра с ключевм словом params:</p>
<pre class="brush:c#;">
...
<p>Здесь метод Sum имеет обязательный параметр initialValue, поэтому при вызове метода для него нужно обязательно передать значение. Поэтому первое значение при вызове 
метода будет передаваться этому параметру.</p>
<p>Однако после параметра с модификатором <span class="b">params</span> мы НЕ можем указывать другие параметры. То есть 
следующее определение метода недопустимо:</p>
<pre class="brush:c#;">
//Так НЕ работает
void Sum(params int[] numbers, int initialValue)
{}
</pre>
<h3>Массив в качестве параметра</h3>
<p>Также этот способ передачи параметров надо отличать от передачи массива в качестве параметра:</p>
<pre class="brush:c#;">
...
<p>Так как метод <code>Sum</code> принимает в качестве параметра массив без ключевого слова <code>params</code>, то при его вызове нам обязательно надо передать в качестве первого параметра 
массив. Кроме того, в отличие от метода с параметром <span class="b">params</span> после параметра-массива могут располагаться другие параметры.</p>
...
<div class="nav"><p><a href="./2.27.php">Назад</a><a href="./">Содержание</a><a href="./2.11.php">Вперед</a></p></div></div>
<div class="socBlock">
