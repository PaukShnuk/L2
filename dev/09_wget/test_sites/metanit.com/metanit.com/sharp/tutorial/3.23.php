...
<html >
<head>
<title>C# и .NET |Сортировка объектов. Интерфейс IComparable</title>
<meta charset="utf-8" />
<meta name="description" content="Сортировка объектов в C#, применение интерфейса IComparable, использование IComparer и создание своих сортировщиков">
...
<h2>Сортировка объектов. Интерфейс IComparable</h2><div class="date">Последнее обновление: 02.01.2022</div>
...
<p>Большинство встроенных в .NET классов коллекций и массивы поддерживают сортировку. С помощью одного метода, который, как правило, называется 
<code>Sort()</code> можно сразу отсортировать по возрастанию весь набор данных. Например:</p>
<pre class="brush:c#;">
...
<p>Однако метод Sort по умолчанию работает только для наборов примитивных типов, как int или string. Для сортировки наборов сложных объектов 
применяется интерфейс <span class="b">IComparable</span>. Он имеет всего один метод:</p>
<pre class="brush:c#;">
...
<p>Метод <code>CompareTo</code> предназначен для сравнения текущего объекта с объектом, который передается в качестве параметра <code>object? o</code>. 
На выходе он возвращает целое число, которое может иметь одно из трех значений:</p>
<ul>
<li><p>Меньше нуля. Значит, текущий объект должен находиться перед объектом, который передается в качестве параметра</p></li>
<li><p>Равен нулю. Значит, оба объекта равны </p></li>
<li><p>Больше нуля. Значит, текущий объект должен находиться после объекта, передаваемого в качестве параметра</p></li>
</ul>
<p>Например, имеется класс Person:</p>
<pre class="brush:c#;">
...
<p>Здесь в качестве критерия сравнения выбрано свойство Name объекта Person. Поэтому при сравнении здесь фактически идет сравнение значения свойства 
Name текущего объекта и свойства Name объекта, переданного через параметр. Если вдруг объект не удастся привести к типу Person, то выбрасывается исключение.</p>
<p>Применение:</p>
<pre class="brush:c#;">
...
<p>Интерфейс IComparable имеет обобщенную версию, поэтому мы могли бы сократить и упростить его применение в классе Person:</p>
<pre class="brush:c#;">
...
<p>Аналогичным образом мы мошли сравнивать по возрасту:</p>
<pre class="brush:c#;">
class Person : IComparable&lt;Person&gt;
{
    public string Name { get;}
...
<h3>Применение компаратора</h3>
<p>Кроме интерфейса IComparable платформа .NET также предоставляет интерфейс IComparer:</p>
<pre class="brush:c#;">
...
<p>Метод Compare предназначен для сравнения двух объектов o1 и o2. Он также возвращает три значения, в зависимости от результата сравнения: если первый объект больше второго, 
то возвращается число больше 0, если меньше - то число меньше нуля; если оба объекта равны, возвращается ноль.</p>
<p>Создадим компаратор объектов Person. Пусть он сравнивает объекты в зависимости от длины строки - значения свойства Name:</p>
<pre class="brush:c#;">
...
<p>В данном случае используется обобщенная версия интерфейса IComparer, чтобы не делать излишних преобразований типов. Применение компаратора:</p>
<pre class="brush:c#;">
...
<p>Объект компаратора указывается в качестве второго параметра метода <code>Array.Sort()</code>. При этом не важно, реализует ли класс Person интерфейс 
...
а в конце - у которых имена длиннее:</p>
<div class="console">
<pre class="consoletext">
Tom - 37
Kate - 25
...
<div class="nav"><p><a href="./3.22.php">Назад</a><a href="./">Содержание</a><a href="./3.27.php">Вперед</a></p></div></div>
<div class="socBlock">
