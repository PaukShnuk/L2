...
<html >
<head>
<title>Установка службы Windows | C#</title>
<meta charset="utf-8" />
...
<h2>Установка службы</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для установки службы нам понадобится класс установщика. Для его добавления нажмем правой кнопкой мыши на проект и в контекстном меню выберем 
<span class="bb">Add - &gt; Component...</span>. Затем в окне добавления нового элемента выберем пункт <span class="bb">Installer Class</span>:</p>
<img src="./pics/21.2.png" alt="Добавление установщика в Windows службу" />
<p>По умолчанию для файла установщика генерируется название Installer1.cs. Оставим это название.</p>
<p>После добавления узел установщика в окне обозревателя решения (Solution Explorer) будет содержать два файла: сам файл класса Installer1 и 
файл дизайнера Installer1.Designer.cs. Откроем файл кода. Сейчас он выглядит так:</p>
<pre class="brush:c#;">
using System;
...
<p>Теперь изменим его следующим образом:</p>
<pre class="brush:c#;">
using System.ComponentModel;
...
<p>Класс установщика унаследован от класса <span class="bb">System.Configuration.Install.Installer</span>. Атрибут <span class="bb">[RunInstaller(true)]</span> указывает на то, что класс Installer1 должен вызываться при установке сборки, то есть службы.</p>
<p>Он определяет ряд методов: <code>Install()</code> (установка), <code>Commit()</code> (завершает транзакцию установки), <code>Rollback()</code> (восстанавливает состояние компьютера до установки) 
и <code>Uninstall()</code> (удаление). При необходимости мы можем их переопределить. Но в нашем случае мы будем использовать только конструктор.</p>
<p>В конструкторе вызывается метод <code>InitializeComponent()</code>, который призван выполнять начальную инициализацию. Он определен в 
файле дизайнера Installer1.Designer.cs и по сути ничего не делает:</p>
<pre class="brush:c#;">
...
<p>Для установки службы нам также понадобится классы <span class="bb">ServiceInstaller</span> и <span class="bb">ServiceProcessInstaller</span>. Почему именно два класса? 
ServiceProcessInstaller управляет настройкой значений для всех запускаемых служб внутри одного процесса (как было рассмотрено в прошлой теме, метод Main класса 
Program может одновременно запускать несколько служб). Класс ServiceInstaller предназначен для настройки значений для каждой из запускаемых служб. То есть если у нас запускается 
три службы, то для каждой службы создается свой объект ServiceInstaller. Но в нашем случае в прошлой теме мы определили только одну запускаемую службу, поэтому объекты обоих 
классов у нас будут только в одном экземпляре.</p>
<p>Через свойства оба класса позволяют задать настройки службы. Основные свойства ServiceProcessInstaller:</p>
<ul>
<li><p><span class="bb">Username</span>: определяет, под какой именно учетной записью будет запускаться служба</p></li>
<li><p><span class="bb">Password</span>: определяет пароль для запуска службы</p></li>
<li><p><span class="bb">Account</span>: определяет тип учетной записи. Может принимать одно из значений перечисления <span class="bb">ServiceAccount</span>:</p>
<ul>
<li><p><span class="bb">LocalSystem</span>: учетная запись предоставляет широкие привилегии на локальном компьютере и соответствует компьютеру в сети</p></li>
<li><p><span class="bb">LocalService</span>: учетная запись соответствует пользователю без привилегий на локальном компьютере и предоставляет любому удаленному серверу учетные данные анонимного доступа</p></li>
<li><p><span class="bb">NetworkService</span>: учетная запись предоставляет широкие локальные привилегии и также представляет любому удаленному серверу учетные данные компьютера</p></li>
<li><p><span class="bb">User</span>: учетная запись определяется конкретным пользователем в сети</p></li>
</ul>
</li>
<li><p><span class="bb">HelpText</span>: возвращает справку, содержащую информацию об установке службы</p></li>
</ul>
<p>Некоторые важнейшие свойства ServiceInstaller:</p>
<ul>
<li><p><span class="bb">StartType</span>: определяет, как должна запускаться служба - автоматически или вручную. Может принимать следующие значения: 
<span class="bb">ServiceStartMode.Automatic</span> (автоматический запуск), <span class="bb">ServiceStartMode.Manual</span> (вручную) и 
<span class="bb">ServiceStartMode.Disabled</span> (служба по умолчанию отключена)</p></li>
<li><p><span class="bb">DelayedAutoStart</span>: определяет, должна ли служба запускаться не сразу после загрузки операционной системы, а немного позже</p></li>
<li><p><span class="bb">DisplayName</span>: устанавливает имя службы, которое будет отображаться в различных утилитах управления службами</p></li>
<li><p><span class="bb">ServiceName</span>: имя службы, должно совпадать со значением свойства ServiceName у класса службы</p></li>
</ul>
<p>То есть в нашем случае для службы установлен запуск вручную, и ей будут предоставляться широкие привилегии.</p>
<p>В конце оба объекта установщиков надо добавить в коллекцию Installers: <code>Installers.Add()</code></p>
<p>Итак, у нас готов код службы и код инсталяторов. Теперь скомпилируем проект, и в проекте в папке <i>bin/Debug</i> у нас будет лежать исполняемый файл службы, который будет называться по имени проекта и иметь расширение 
exe. Например, в моем случае он называется FileWatcherService.exe.</p>
<p>И теперь нам надо установить службу. Для этого нам надо запустить специальную утилиту <span class="bb">InstallUtil.exe</span> и передать ей имя файла службы.</p>
<p>Утилиту InstallUtil.exe можно найти по пути <i>C:\Windows\Microsoft.NET\Framework64\v4.0.30319</i> для 64-битных платформ и 
в <i>C:\Windows\Microsoft.NET\Framework\v4.0.30319</i> для 32-битных.</p>
<p>Также можно вынести исполняемый файл службы в какое-нибудь другое место, например, я поместил его в папку <i>D://servces</i>. Тогда установка службы 
в командной строке будет выглядеть так:</p>
<img src="./pics/21.3.png" alt="Установка службы Windows с помощью InstallUtil.exe" />
<p>И если мы зайдем после успешной установки в консоль служб, то сможем найти только что установленную службу.</p>
<img src="./pics/21.4.png"  />
<p>Мы ее можем запустить, и она будет выполнять все те действия, которые мы в ней определили.</p>
<p>Если нам служба больше не нужна, и мы хотим ее удалить, то мы можем воспользоваться той же утилитой, передав ему флаг u:</p>
<div class="console">
<pre class="consoletext">
C:\Windows\Microsoft.NET\Framework64\v4.0.30319&gt;InstallUtil.exe /u D://servces/FileSystemWatcher.exe
</div>
</pre>
<p>И после запуска созданная служба будет мониторить все действия с файлами в папке D://Temp и записывать информацию о действиях в текстовый файл.</p>
...
<div class="nav"><p><a href="./21.1.php">Назад</a><a href="./">Содержание</a><a href="./22.1.php">Вперед</a></p></div></div>
<div class="socBlock">
