...
<html >
<head>
<title>C# и .NET | Арифметические операции</title>
<meta charset="utf-8" />
<meta name="description" content="Арифметические операции языка программирования C#, сложение вычитание, деление, получение остатка, умножение, инкремент и декремент">
...
<h2>Арифметические операции языка C#</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>В C# используется большинство операций, которые применяются и в других языках программирования. Операции представляют определенные действия над операндами - 
...
<ul>
<li><p><span class="b">+</span></p><p>Операция сложения двух чисел:</p>
<pre class="brush:c#;">
int x = 10;
int z = x + 12; // 22
</pre>
</li>
<li><p><span class="b">-</span></p><p>Операция вычитания двух чисел:</p>
<pre class="brush:c#;">
int x = 10;
int z = x - 6; // 4
</pre></li>
<li><p><span class="b">*</span></p><p>Операция умножения двух чисел:</p>
<pre class="brush:c#;">
int x = 10;
int z = x * 5; // 50
</pre>
</li>
<li><p><span class="b">/</span></p><p>операция деления двух чисел:</p>
<pre class="brush:c#;">
int x = 10;
int z = x / 5; // 2
...
<pre class="brush:c#;">double z = 10 /  4; //результат равен 2</pre>
...
<pre class="brush:c#;">double z = 10.0 /  4.0; //результат равен 2.5</pre>
</li>
<li><p><span class="b">%</span></p><p>Операция получение остатка от целочисленного деления двух чисел:</p>
<pre class="brush:c#;">
double x = 10.0;
...
<p>Инкремент бывает префиксным: <code>++x</code> - сначала значение переменной x увеличивается на 1, 
а потом ее значение возвращается в качестве результата операции.</p></li>
<p>И также существует постфиксный инкремент: <code>x++</code> - сначала значение переменной x возвращается в качестве результата операции, 
...
<li><p><span class="b">--</span></p>
<p>Операция декремента или уменьшения значения на единицу. Также существует префиксная форма декремента (<code>--x</code>) и 
постфиксная (<code>x--</code>).</p>
<pre class="brush:c#;">
int x1 = 5;
int z1 = --x1; // z1=4; x1=4
Console.WriteLine($"{x1} - {z1}");
...
<pre class="brush:c#;">int d = (c--)-(b*a);</pre>
...
<p>Стоит нам трактовать это выражение как <code>(10 / 5) * 2</code> или как <code>10 / (5 * 2)</code>? Ведь в зависимости от трактовки 
...
<p>Все арифметические операторы являются левоассоциативными, то есть выполняются слева направо. Поэтому выражение <code>10 / 5 * 2</code> 
необходимо трактовать как <code>(10 / 5) * 2</code>, то есть результатом будет 4.</p>
...
<div class="nav"><p><a href="./2.15.php">Назад</a><a href="./">Содержание</a><a href="./2.22.php">Вперед</a></p></div></div>
<div class="socBlock">
