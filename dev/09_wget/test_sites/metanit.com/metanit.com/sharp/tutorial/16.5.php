...
<html >
<head>
<title>C# и .NET | Linq to Xml. Создание документа Xml</title>
<meta charset="utf-8" />
<meta name="description" content="Создание документа Xml в языке программирования C# и .NET c помощью классов и методов Linq to Xml">
...
<h2>Linq to Xml. Создание документа XML</h2><div class="date">Последнее обновление: 21.02.2022</div>
...
<p>Еще один подход к работе с Xml представляет технология <span class="b">LINQ to XML</span>. Вся функциональность LINQ to XML 
содержится в пространстве имен <span class="b">System.Xml.Linq</span>. Рассмотрим основные классы этого пространства имен:</p>
<ul>
<li><p><span class="b">XAttribute</span>: представляет атрибут xml-элемента</p></li>
<li><p><span class="b">XComment</span>: представляет комментарий</p></li>
<li><p><span class="b">XDocument</span>: представляет весь xml-документ</p></li>
<li><p><span class="b">XElement</span>: представляет отдельный xml-элемент</p></li>
</ul>
<p>Ключевым классом является <code>XElement</code>, который позволяет получать вложенные элементы и управлять ими. Среди его методов можно отметить следующие:</p>
<ul>
<li><p><span class="b">Add()</span>: добавляет новый атрибут или элемент</p></li>
<li><p><span class="b">Attributes()</span>: возвращает коллекцию атрибутов для данного элемента</p></li>
<li><p><span class="b">Elements()</span>: возвращает все дочерние элементы данного элемента </p></li>
<li><p><span class="b">Remove()</span>: удаляет данный элемент из родительского объекта</p></li>
<li><p><span class="b">RemoveAll()</span>: удаляет все дочерние элементы и атрибуты у данного элемента</p></li>
</ul>
<p>Итак, используем функциональность LINQ to XML и создадим новый XML-документ:</p>
<pre class="brush:c#;">
...
<p>Чтобы создать документ, нам нужно создать объект класса <span class="b">XDocument</span>. Это объект самого верхнего уровня в хml-документе.</p>
<p>Элементы создаются с помощью конструктора класса <span class="b">XElement</span>. Конструктор имеет ряд перегруженных версий. Первый параметр конструктора 
передает название элемента, например, person. Второй параметр передает значение этого элемента.</p>
<p>Создание атрибута аналогично созданию элемента. Затем все атрибуты и элементы добавляются в элементы person с помощью метода <code>Add()</code>.</p>
<p>Так как документ xml должен иметь один корневой элемент, то затем все элементы person добавляются в один контейнер - элемент people.</p>
<p>В конце корневой элемент добавляется в объект XDocument, и этот объект сохраняется на диске в xml-файл с помощью метода <span class="b">Save()</span>.</p>
<p>Если мы откроем сохраненный файл <span class="b">people.xml</span>, то увидим в нем следующее содержание:</p>
<pre class="brush:xml;">
...
<p>Конструктор класса XElement позволяют задать набор объектов, которые будут входить в элемент. И предыдущий пример мы могли бы сократить следующим способом:</p>
<pre class="brush:c#;">
using System.Xml.Linq;
...
<div class="nav"><p><a href="./16.4.php">Назад</a><a href="./">Содержание</a><a href="./16.6.php">Вперед</a></p></div></div>
<div class="socBlock">
