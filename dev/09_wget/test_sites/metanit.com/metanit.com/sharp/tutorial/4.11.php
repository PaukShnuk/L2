...
<html >
<head>
<title>C# и .NET | Интерфейсы IEnumerable и IEnumerator</title>
<meta charset="utf-8" />
<meta name="description" content="Интерфейсы IEnumerable и IEnumerator в C#, их основные методы и свойства и реализация в классах, цикл foreach">
...
<h2>Интерфейсы IEnumerable и IEnumerator</h2><div class="date">Последнее обновление: 04.01.2022</div>
...
<p>Как мы увидели, основной для большинства коллекций является реализация интерфейсов IEnumerable и IEnumerator. Благодаря такой реализации 
мы можем перебирать объекты в цикле <code>foreach</code>:</p>
<pre class="brush:c#;">
...
<p>Метод <span class="b">MoveNext()</span> перемещает указатель на текущий элемент на следующую позицию в последовательности. Если последовательность еще не закончилась, то возвращает 
true. Если же последовательность закончилась, то возвращается false.</p>
<p>Свойство <span class="b">Current</span> возвращает объект в последовательности, на который указывает указатель.</p>
<p>Метод <span class="b">Reset()</span> сбрасывает указатель позиции в начальное положение.</p>
...
<p>Рассмотрим простешую реализацию <span class="b">IEnumerable</span> на примере:</p>
<pre class="brush:c#;">
using System.Collections;
...
в классе определить публичный метод <span class="b">GetEnumerator</span>, который бы возвращал объект IEnumerator. Например:</p>
<pre class="brush:c#;">
class Week
{
	string[] days = { "Monday", "Tuesday", "Wednesday", "Thursday", 
...
логику перебора объектов. Для этого реализуем интерфейс <span class="b">IEnumerator</span>:</p>
<pre class="brush:c#;">
using System.Collections;
...
<p>В данном случае реализуем интерфейс <code>IEnumerator&lt;string&gt;</code>, соответственно в свойстве Current нам надо возвратить объект string. В этом случае при 
переборе в цикле foreach перебираемые объекты будут автоматически представлять тип <span class="b">string</span>:</p>
<pre class="brush:c#;">
Week week = new Week();
...
<div class="nav"><p><a href="./4.13.php">Назад</a><a href="./">Содержание</a><a href="./4.12.php">Вперед</a></p></div></div>
<div class="socBlock">
