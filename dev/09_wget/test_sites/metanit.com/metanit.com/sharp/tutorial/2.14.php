...
<html >
<head>
<title>C# и .NET | Обработка исключений</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка исключений на C#, использование конструкции try..catch, обработка нескольких типов исключений, оператор throw">
...
<h1>Обработка исключений</h1><h2>Конструкция try..catch..finally</h2><div class="date">Последнее обновление: 30.12.2021</div>
...
<p>Иногда при выполнении программы возникают ошибки, которые трудно предусмотреть или предвидеть, а  иногда и вовсе невозможно. Например, при передачи файла по сети может неожиданно оборваться сетевое подключение. 
такие ситуации называются <span class="b">исключениями</span>. Язык C# предоставляет разработчикам возможности для обработки таких ситуаций. Для этого 
в C# предназначена конструкция <span class="b">try...catch...finally</span>.</p>
<pre class="brush:c#;">
try
{
...
<p>При использовании блока <span class="b">try...catch..finally</span> вначале выполняются все инструкции в блоке <span class="b">try</span>. Если в 
этом блоке не возникло исключений, то после его выполнения начинает выполняться блок <span class="b">finally</span>. И затем конструкция try..catch..finally 
...
начинает искать блок <span class="b">catch</span>, который может обработать данное исключение. Если нужный блок 
...
<img src="./pics/2.10.png" alt="Исключения в C#" />
<p>В этом окошке мы видим, что возникло исключение, которое представляет тип <span class="b">System.DivideByZeroException</span>, 
то есть попытка деления на ноль. С помощью пункта <span class="b">View Details</span> можно посмотреть более детальную информацию об исключении.</p>
...
<span class="b">try...catch...finally</span>. Так, перепишем пример следующим образом:</p>
<pre class="brush:c#;">
try
{
	int x = 5;
...
И дойдя до строки</p>
<pre class="brush:c#;">int y = x / 0;</pre>
<p>выполнение программы остановится. CLR найдет блок <span class="b">catch</span> и передаст управление этому блоку.</p>
...
<p>Следует отметить, что в этой конструкции обязателен блок <span class="b">try</span>. При наличии блока catch мы можем опустить блок finally:</p>
<pre class="brush:c#;">
try
{
	int x = 5;
...
<code>try..catch</code>, чтобы обработать возможную ошибку. Однако гораздо оптимальнее было бы проверить допустимость преобразования:</p>
<pre class="brush:c#;">
Square("12"); // Квадрат числа 12: 144
...
<p>Метод <code>int.TryParse()</code> возвращает <code>true</code>, если преобразование можно осуществить, и <code>false</code> - если нельзя. При допустимости преобразования переменная x 
будет содержать введенное число. Так, не используя <code>try...catch</code> можно обработать возможную исключительную ситуацию.</p>
<p>С точки зрения производительности использование блоков <code>try..catch</code> более накладно, чем применение условных конструкций. Поэтому по возможности 
...
<div class="nav"><p><a href="./3.39.php">Назад</a><a href="./">Содержание</a><a href="./2.28.php">Вперед</a></p></div></div>
<div class="socBlock">
