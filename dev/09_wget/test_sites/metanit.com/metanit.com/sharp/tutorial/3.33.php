...
<html >
<head>
<title>C# и .NET | Делегаты Action, Predicate и Func</title>
<meta charset="utf-8" />
<meta name="description" content="Делегаты Action, Predicate и Func в языке программирования C#">
...
<h2>Делегаты Action, Predicate и Func</h2><div class="date">Последнее обновление: 01.01.2022</div>
...
<p>В .NET есть несколько встроенных делегатов, которые используются в различных ситуациях. И наиболее используемыми, с которыми часто приходится сталкиваться, являются 
<span class="b">Action</span>, <span class="b">Predicate</span> и <span class="b">Func</span>.</p>
<h3>Action</h3>
<p>Делегат <span class="b">Action</span> представляет некоторое действие, которое ничего не возвращает, то есть в качестве возвращаемого типа имеет 
тип  <span class="b">void</span>:</p>
<pre class="brush:c#;">
public delegate void Action()
public delegate void Action&lt;in T&gt;(T obj)
</pre>
<p>Данный делегат имеет ряд перегруженных версий. Каждая версия принимает разное число параметров: от <code>Action&lt;in T1&gt;</code> до 
<code>Action&lt;in T1, in T2,....in T16&gt;</code>. Таким образом можно передать до 16 значений в метод.</p>
<p>Как правило, этот делегат передается в качестве параметра метода и предусматривает вызов определенных действий в ответ на произошедшие действия. Например:</p>
<pre class="brush:c#;">
...
<h3>Predicate</h3>
<p>Делегат <code>Predicate&ltT&gt;</code> принимает один параметр и возвращает значение типа <span class="b">bool</span>:</p>
<pre class="brush:c#;">delegate bool Predicate&lt;in T&gt;(T obj);</pre>
<p>Как правило, используется для сравнения, сопоставления некоторого объекта T определенному условию. 
В качестве выходного результата возвращается значение true, если условие соблюдено, и false, если не соблюдено:</p>
<pre class="brush:c#;">
...
<p>В данном случае возвращается true или false в зависимости от того, больше нуля число или нет.</p>
<h3>Func</h3>
<p>Еще одним распространенным делегатом является <span class="b">Func</span>. Он возвращает результат действия и может принимать параметры. 
Он также имеет различные формы: от <code>Func&lt;out T&gt;()</code>, где T - тип возвращаемого значения, до <code>Func&lt;in T1, in T2,...in T16, out TResult&gt;()</code>, 
то есть может принимать до 16 параметров.</p>
<pre class="brush:c#;">
...
<p>Данный делегат также часто используется в качестве параметра в методах:</p>
<pre class="brush:c#;">
...
<p>Метод <code>DoOperation()</code> в качестве параметра принимает делегат <code>Func&lt;int, int&gt;</code>, то есть ссылку на метод, 
который принимает число int и возвращает также значение int.</p>
<p>При первом вызове метода <code>DoOperation()</code> ему передается ссылка на метод DoubleNumber, который увеличивает число в два раза. 
Во втором случае передается метод SquareNumber - опять же метод, который принимает параметр типа int и возвращает результат в виде значения int.</p>
<p>Другой пример:</p>
<pre class="brush:c#;">
...
<p>Здесь переменная createString представляет лямбда-выражение, которое принимает два числа int и возвращает строку, то есть представляет делегат <code>Func&lt;int, int, string&gt;</code>.</p>
...
<div class="nav"><p><a href="./3.28.php">Назад</a><a href="./">Содержание</a><a href="./3.54.php">Вперед</a></p></div></div>
<div class="socBlock">
