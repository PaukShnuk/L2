...
<html >
<head>
<title>C# и .NET | Переменные-ссылки и возвращение ссылки</title>
<meta charset="utf-8" />
<meta name="description" content="Переменные-ссылки (ref locals) и возвращение ссылки (ref return) в языке программирования C#, получение ссылки на объект в памяти">
...
<h2>Переменные-ссылки и возвращение ссылки</h2><div class="date">Последнее обновление: 11.11.2021</div>
...
<p>Кроме параметров метода, которые с помощью модификатора <span class="b">ref</span> позволяют передавать значение по ссылке, 
C# также позволяет с помощью ключевого слова <span class="b">ref</span> возвращать ссылку из метода и определять переменную, которая будет хранить ссылку.</p>
<h3>Переменная-ссылка</h3>
<p>Для определения локальной переменной-ссылки (ref local) перед ее типом ставится ключевое слово <span class="b">ref</span>:</p>
<pre class="brush:c#;">
int x = 5;
ref int xRef = ref x;
</pre>
<p>Здесь переменная xRef указывает не просто на значение переменной x, а на область в памяти, где располагается эта переменная. Для этого перед x также указывается ref.</p>
<p>При этом мы не можем просто определить переменную-ссылку, нам обязательно надо присвоить ей некоторое значение. Так, следующий код вызовет ошибку:</p>
<pre class="brush:c#;">
ref int xRef;	// ошибка
</pre>
<p>Получив ссылку, мы можем манипулировать значением по этой ссылке. Например:</p>
<pre class="brush:c#;">
int x = 5;
ref int xRef = ref x;
...
<h3>Ссылка как результат функции</h3>
<p>Для возвращения из функции ссылки в сигнатуре функции перед возвращаемым типом, а также после оператора <span class="b">return</span> 
следует указать ключевое слово <span class="b">ref</span>:</p>
<pre class="brush:c#;">
...
<p>В методе <code>Find</code> ищем число в массиве, но вместо самого значения числа возвращаем ссылку на него в памяти. Для этого в сигнатуре метода 
в качестве типа результата функции указывается не просто <span class="b">int</span>, а <span class="b">ref int</span>.</p>
<p>Кроме того, в самом методе после слова return также ставится <code>ref</code>:</p>
<pre class="brush:c#;">return ref numbers[i];</pre>
<p>Тем самым мы получаем не просто значение, а ссылку на объект в памяти.</p>
<p>В методе Main для определения переменной, которая будет содержать ссылку, используется ключевое слово ref. При вызове метода также указывается слово ref:</p>
<pre class="brush:c#;">ref int numberRef = ref Find(7, numbers);</pre>
<p>В итоге переменная numberRef будет содержать ссылку на объект int, и через данную переменную в последствиии мы можем изменить объект по этой ссылке.</p>
<p>Другой пример - возвращение ссылки на максимальное число из двух:</p>
<pre class="brush:c#;">
int a = 5;
int b = 8;
...
<p>Стоит обратить внимание, что параметры метода в этом случае определены с ключевым словом ref.</p>
<p>При определении метода, который возвращает ссылку, следует учитывать, что такой метод естественно не может иметь тип void. Кроме того, такой метод не может 
возвращать:</p>
<ul>
<li><p>Значение <code>null</code></p></li>
<li><p>Константу</p></li>
<li><p>Значение перечисления enum</p></li>
<li><p>Свойство класса или структуры</p></li>
<li><p>Поле для чтения (которое имеет модификатор read-only)</p></li>
</ul>
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/TMoP9zn2k1M" target="_blank"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 20. Переменные-ссылки и получение ссылки в языке C#</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./4.10.php">Назад</a><a href="./">Содержание</a><a href="./3.18.php">Вперед</a></p></div></div>
<div class="socBlock">
