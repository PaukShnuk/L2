...
<html >
<head>
<title>C# и .NET | Архивация и сжатие файлов</title>
<meta charset="utf-8" />
<meta name="description" content="Сжатие файлов и их восстановление, создание архивов zip в языке программирования C# и .NET с помощью классов GZipStream и DeflateStream">
...
<h2>Архивация и сжатие файлов</h2><div class="date">Последнее обновление: 20.02.2022</div>
...
<p>Кроме классов чтения-записи .NET предоставляет классы, которые позволяют сжимать файлы, а также затем восстанавливать их в исходное состояние.</p>
<p>Это классы <span class="b">ZipFile</span>, <span class="b">DeflateStream</span> и <span class="b">GZipStream</span>, которые находятся в пространстве имен 
<span class="b">System.IO.Compression</span> и представляют реализацию одного из алгоритмов сжатия Deflate или GZip.</p>
<h3>GZipStream и DeflateStream</h3>
<p>Для создания объекта GZipStream можно использовать один из его конструкторов:</p>
<ul>
<li></p><code>GZipStream(Stream stream, CompressionLevel level)</code>: stream представляет данные, а level задает уровень сжатия</p></li>
<li></p><code>GZipStream(Stream stream, CompressionMode mode)</code>: mode указывает, будут ли данные сжиматься или, наоборот, восстанавливаться и может принимать два значения:</p>
<ul>
<li><p><code>CompressionMode.Compress</code>: данные сжимаются</p></li>
<li><p><code>CompressionMode.Decompress</code>: данные восстанавливаются</p></li>
</ul>
...
<li></p><code>GZipStream(Stream stream, CompressionLevel level, bool leaveMode)</code>: параметр leaveMode указывает, надо ли оставить открытым поток stream после удаления 
объекта GZipStream. Если значение true, то поток остается открытым</p></li>
<li></p><code>GZipStream(Stream stream, CompressionMode mode, bool leaveMode)</code></p></li>
</ul>
<p>Для управления сжатием/восстанавлением данных GZipStream предоставляет ряд методов. Основые из них:</p>
<ul>
<li></p><code>void CopyTo(Stream destination)</code>: копирует все данные в поток destination</p></li>
<li></p><code>Task CopyToAsync(Stream destination)</code>: асинхронная версия метода CopyTo</p></li>
<li></p><code>void Flush()</code>: очищает буфер, записывая все его данные в файл</p></li>
<li></p><code>Task FlushAsync()</code>: асинхронная версия метода Flush</p></li>
<li><p><code>int Read(byte[] array, int offset, int count)</code>: считывает данные из файла в массив байтов и возвращает количество успешно считанных байтов. 
Принимает три параметра:</p>
<ul>
<li><p><code>array</code> - массив байтов, куда будут помещены считываемые из файла данные</p></li>
<li><p><code>offset</code> представляет смещение в байтах в массиве array, в который считанные байты будут помещены</p></li>
<li><p><code>count</code> - максимальное число байтов, предназначенных для чтения. Если в файле находится меньшее количество байтов, то 
...
<li><p><code>count</code> - максимальное число байтов, предназначенных для чтения. Если в файле находится меньшее количество байтов, то 
все они будут считаны.</p></li>
<li><p><code>Task&lt;int&gt; ReadAsync(byte[] array, int offset, int count)</code>: асинхронная версия метода Read</p></li>
</ul>
</li>
<li><p><code>long Seek(long offset, SeekOrigin origin)</code>: устанавливает позицию в потоке со смещением на количество байт, 
указанных в параметре offset.</p></li>
<li><p><code>void Write(byte[] array, int offset, int count)</code>: записывает в файл данные из массива байтов. Принимает три параметра: 
</p>
<ul>
<li><p><code>array</code> - массив байтов, откуда данные будут записываться в файл</p></li>
<li><p><code>offset</code> - смещение в байтах в массиве array, откуда начинается запись байтов в поток</p></li>
<li><p><code>count</code> - максимальное число байтов, предназначенных для записи</p></li>
</ul>
</li>
<li><p><code>Task WriteAsync(byte[] array, int offset, int count)</code>: асинхронная версия метода Write</li>
</ul>
...
<p>Метод <code>CompressAsync</code> получает название исходного файла, который надо архивировать, и название будущего сжатого файла.</p>
<p>Сначала создается поток для чтения из исходного файла - <code>FileStream sourceStream</code>. Затем создается поток для записи в сжатый файл - 
<code>FileStream targetStream</code>. Поток архивации <code>GZipStream compressionStream</code> инициализируется потоком targetStream и с помощью 
метода <code>CopyToAsync()</code> получает данные от потока sourceStream.</p>
<p>Метод <code>DecompressAsync</code> производит обратную операцию по восстановлению сжатого файла в исходное состояние. Он принимает в качестве параметров 
пути к сжатому файлу и будущему восстановленному файлу.</p>
<p>Здесь в начале создается поток для чтения из сжатого файла <code>FileStream sourceStream</code>, затем поток для записи в восстанавливаемый файл 
<code>FileStream targetStream</code>. В конце создается поток <code>GZipStream decompressionStream</code>,  который с помощью метода 
<code>CopyToAsync()</code> копирует восстановленные данные в поток targetStream.</p>
...
<h3>ZipFile</h3>
<p>Статический класс <span class="b">ZipFile</span> из простанства имен System.IO.Compression предоставляет дополнительные возможности для создания архивов. Он позволяет создавать архив из каталогов. 
Его основные методы:</p>
<ul>
<li><p><code>void CreateFromDirectory(string sourceDirectoryName, string destinationFileName)</code>: архивирует папку по пути sourceDirectoryName в 
файл с названием destinationFileName</p></li>
<li><p><code>void ExtractToDirectory(string sourceFileName, string destinationDirectoryName)</code>: извлекает все файлы из zip-файла sourceFileName в каталог destinationDirectoryName</p></li>
</ul>
...
<p>В данном случае папка "D://test/" методом <code>ZipFile.CreateFromDirectory</code> архивируется в файл test.zip. Затем метод 
<code>ZipFile.ExtractToDirectory()</code> распаковывает данный файл в папку "D://newtest" (если такой папки нет, она создается).</p>
...
<div class="nav"><p><a href="./5.6.php">Назад</a><a href="./">Содержание</a><a href="./6.5.php">Вперед</a></p></div></div>
<div class="socBlock">
