...
<html >
<head>
<title>C# и .NET | Семафоры</title>
<meta charset="utf-8" />
<meta name="description" content="Использование семафоров для синхронизации потоков в приложении на языке программирования C# и .NET. Класс Semaphore и его методы WaitOne и Release">
...
<h2>Семафоры</h2><div class="date">Последнее обновление: 07.02.2022</div>
...
<p><span class="b">Семафоры</span> являются еще одним инструментом, который предлагает нам платформа .NET для управления синхронизацией. 
...
<span class="b">Semaphore</span>.</p>
<p>Для создания семафора применяется один из конструкторов класса Semaphore:</p>
<ul>
<li><p><span class="b">Semaphore (int initialCount, int maximumCount)</span>: параметр <code>initialCount</code> задает начальное количество потоков, 
а <code>maximumCount</code> - максимальное количество потоков, которые имеют доступ к общим ресурсам</p></li>
<li><p><span class="b">Semaphore (int initialCount, int maximumCount, string? name)</span>: в дополнение задает имя семафора</p></li>
<li><p><span class="b">Semaphore (int initialCount, int maximumCount, string? name, out bool createdNew)</span>: последний параметр 
- <code>createdNew</code> при значении true указывает, что новый семафор был успешно создан. Если этот параметр равен false, то семафор с указанным именем уже 
существует</p></li>
</ul>
<p>Для работы с потоками класс Semaphore имеет два основных метода:</p>
<ul>
<li><p><span class="b">WaitOne()</span>: ожидает получения свободного места в семафоре</p></li>
<li><p><span class="b">Release()</span>: освобождает место в семафоре</p></li>
</ul>
<p>Например, у нас такая задача: есть некоторое число читателей, которые приходят в библиотеку три раза в день и что-то там читают. И пусть у нас будет ограничение, 
что единовременно в библиотеке не может находиться больше трех читателей. Данную задачу очень легко решить с помощью семафоров:</p>
<pre class="brush:c#;">
...
<p>В данной программе читатель представлен классом Reader. Он инкапсулирует всю функциональность, связанную с потоками, через переменную 
<code>Thread myThread</code>.</p>
<p>Сам семафор определяется в виде статической переменной sem:</p>
<pre="brush:c#;">static Semaphore sem = new Semaphore(2, 3);</pre>. 
<p>Его конструктор принимает два параметра: первый указывает, какому числу объектов изначально будет доступен семафор, а второй параметр указывает, 
...
в библиотеке, поэтому максимальное число равно 3.</p>
<p>Основной функционал сосредоточен в методе <code>Read</code>, который и выполняется в потоке. В начале для ожидания получения семафора 
используется метод <code>sem.WaitOne()</code>:</p> 
<pre class="brush:c#;">sem.WaitOne();  // ожидаем, когда освободиться место</pre>
<p>После того, как в семафоре освободится место, данный поток заполняет свободное место и начинает выполнять все дальнейшие действия.</p>
<p>После окончания чтения мы высвобождаем семафор с помощью метода <code>sem.Release()</code>:</p>
<pre class="brush:c#;">sem.Release();  // освобождаем место</pre>
<p>После этого в семафоре освобождается одно место, которое заполняет другой поток.</p>
...
<div class="nav"><p><a href="./11.7.php">Назад</a><a href="./">Содержание</a><a href="./12.1.php">Вперед</a></p></div></div>
<div class="socBlock">
