...
<html >
<head>
<title>C# и .NET | Поразрядные операции</title>
<meta charset="utf-8" />
<meta name="description" content="Поразрядные операции в языке программирования C# и платформе .NET, битовое представление числа, конъюнкция, дизъюнкция, инверсия и операции сдвига">
...
<h2>Поразрядные операции</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Особый класс операций представляют поразрядные операции. Они выполняются над отдельными разрядами числа. В этом плане числа рассматриваются в двоичном 
представлении, например, 2 в двоичном представлении 10 и имеет два разряда, число 7 - 111 и имеет три разряда.</p>
...
<li><p><span class="b">^</span> (логическое исключающее ИЛИ)</p><p>Также эту операцию называют XOR, нередко ее применяют для простого шифрования:</p>
<pre class="brush:c#;">
...
<p>Здесь опять же производятся поразрядные операции. Если у нас значения текущего разряда у обоих чисел разные, то возвращается 1, иначе возвращается 0. 
Таким образом, мы получаем из 9^5 в качестве результата число 12. И  чтобы расшифровать число, мы применяем ту же операцию к результату.</p></li>
...
<p>Для записи чисел со знаком в C# применяется <span class="b">дополнительный код</span> (two’s complement), при котором 
...
<ul>
<li><p><code>x&lt;&lt;y</code> - сдвигает число x влево на y разрядов. Например, <code>4&lt;&lt;1</code> сдвигает число 4 (которое в двоичном 
представлении 100) на один разряд влево, то есть в итоге получается 1000 или число 8 в десятичном представлении.</p></li>
<li><p><code>x&gt;&gt;y</code> - сдвигает число x вправо на y разрядов. Например, <code>16&gt;&gt;1</code> сдвигает число 16 
(которое в двоичном представлении 10000) на один разряд вправо, то есть в итоге получается 1000 или число 8 в десятичном представлении.</p></li>
</ul>
...
<div class="nav"><p><a href="./2.3.php">Назад</a><a href="./">Содержание</a><a href="./2.23.php">Вперед</a></p></div></div>
<div class="socBlock">
