...
<html >
<head>
<title>C# и .NET | Выборка элементов в LINQ to XML</title>
<meta charset="utf-8" />
<meta name="description" content="Выборка элементов из xml файла с помощью LINQ to XML в языке программирования C# и .NET, классы XElement и XAttribute">
...
<h2>Выборка элементов в LINQ to XML</h2><div class="date">Последнее обновление: 21.02.2022</div>
...
<p>Большим преимуществом LINQ to XML является то, что эта технология позволяет легко извлекать нужные элементы из документа xml. 
Например, возьмем xml-файл <span class="b">people.xml</span>, созданный в прошлой теме:</p>
<pre class="brush:xml;">
...
<p>Переберем его элементы и выведем их значения на консоль:</p>
<pre class="brush:c#;">
...
<p>Чтобы начать работу с имеющимся xml-файлом, надо сначала загрузить его с помощью статического метода <code>XDocument.Load()</code>, в который передается 
путь к файлу.</p>
<pre class="brush:c#;">XDocument xdoc = XDocument.Load("people.xml");</pre>
<p>Поскольку xml хранит иерархически выстроенные элементы, то и для доступа к элементам надо идти начиная с высшего уровня в этой иерархии и далее вниз. Так, 
для получения элементов person и доступа к ним надо сначала обратиться к корневому элементу, а через него уже к элементам person:</p>
<pre class="brush:c#;">
// получаем корневой узел
XElement? people = xdoc.Element("people");
if (people is not null)
...
<p>Метод <code>Element("имя_элемента")</code> возвращает первый найденный элемент с таким именем. Метод <code>Elements("имя_элемента")</code> возвращает 
коллекцию одноименных элементов. В данном случае мы получаем коллекцию элементов person и поэтому можем перебрать ее в цикле.</p>
<p>Спускаясь дальше по иерархии вниз, мы можем получить атрибуты или вложенные элементы, например, получение элемента &lt;company&gt;</p>
<pre class="brush:c#;">XElement? company = person.Element("company");</pre>
<p>Значение простых элементов, которые содержат один текст, можно получить с помощью свойства <span class="b">Value</span>:</p>
<pre class="brush:c#;">string? companyValue = person.Element("company")?.Value;</pre>
<p>Сочетая операторы Linq и LINQ to XML можно довольно просто извлечь из документа данные и затем обработать их. Например:</p>
<pre class="brush:c#;">
using System.Xml.Linq;
...
<p>В данном случае выбираем все элементы person, у которых вложенный элемент "company" равен "Microsoft". Далее на основе полученной выборке создаем набор анонимных объектов 
с тремя свойствами. Под вывод также можно было бы создать специально какой-нибудь класс или структуру и использовать их вместо анонимного объекта.</p>
<p>Другой пример - выберем элемент person, в котором атрибут name равен "Tom":</p>
<pre class="brush:c#;">
using System.Xml.Linq;
...
<div class="nav"><p><a href="./16.5.php">Назад</a><a href="./">Содержание</a><a href="./16.7.php">Вперед</a></p></div></div>
<div class="socBlock">
