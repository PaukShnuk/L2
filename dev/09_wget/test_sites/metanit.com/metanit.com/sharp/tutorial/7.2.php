...
<html >
<head>
<title>C# и .NET | Операции со строками</title>
<meta charset="utf-8" />
<meta name="description" content="Основные операции со строками в C#: конкатенация, объединение, разделение, поиск, удаление и замена символов и подстрок, основные методы для работы со строками">
...
<h2>Операции со строками</h2><div class="date">Последнее обновление: 05.01.2022</div>
...
<p>Конкатенация строк или объединение может производиться как с помощью операции <code>+</code>, так и с помощью метода <span class="b">Concat</span>:</p>
<pre class="brush:c#;">
string s1 = "hello";
string s2 = "world";
...
<p>Метод <code>Concat</code> является статическим методом класса string, принимающим в качестве параметров две строки. Также имеются другие версии 
метода, принимающие другое количество параметров.</p>
<p>Для объединения строк также может использоваться метод <span class="b">Join</span>:</p>
<pre class="brush:c#;">
string s5 = "apple";
string s6 = "a day";
string s7 = "keeps";
...
<p>Метод Join также является статическим. Использованная выше версия метода получает два параметра: строку-разделитель (в данном случае пробел) и 
массив строк, которые будут соединяться и разделяться разделителем.</p>

<h3>Сравнение строк</h3>
<p>Для сравнения строк применяется статический метод <span class="b">Compare</span>:</p>
<pre class="brush:c#;">
string s1 = "hello";
string s2 = "world";
...
<p>Данная версия метода Compare принимает две строки и возвращает число. Если первая строка по алфавиту стоит выше второй, то возвращается число меньше нуля. 
В противном случае возвращается число больше нуля. И третий случай - если строки равны, то возвращается число 0.</p>
<p>В данном случае так как символ h по алфавиту стоит выше символа w, то и первая строка будет стоять выше.</p>

<h3>Поиск в строке</h3>
<p>С помощью метода <span class="b">IndexOf</span> мы можем определить индекс первого вхождения отдельного символа или подстроки в строке:</p>
<pre class="brush:c#;">
...
<p>Подобным образом действует метод <span class="b">LastIndexOf</span>, только находит индекс последнего вхождения символа или подстроки в строку.</p>
<p>Еще одна группа методов позволяет узнать начинается или заканчивается ли строка на определенную подстроку. Для этого предназначены методы 
<span class="b">StartsWith</span> и <span class="b">EndsWith</span>. Например, в массиве строк хранится список файлов, и нам надо вывести все файлы с 
расширением exe:</p>
<pre class="brush:c#;">
...
<h3>Разделение строк</h3>
<p>С помощью функции <span class="b">Split</span> мы можем разделить строку на массив подстрок. В качестве параметра функция <code>Split</code> 
принимает массив символов или строк, которые и будут служить разделителями. Например, подсчитаем количество слов в сроке, 
разделив ее по пробельным символам:</p>
<pre class="brush:c#;">
...
<p>Это не лучший способ разделения по пробелам, так как во входной строке у нас могло бы быть несколько подряд идущих пробелов и в итоговый массив 
также бы попадали пробелы, поэтому лучше использовать другую версию метода:</p>
<pre class="brush:c#;">
string[] words = text.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
</pre>
<p>Второй параметр <code>StringSplitOptions.RemoveEmptyEntries</code> говорит, что надо удалить все пустые подстроки.</p>

<h3>Обрезка строки</h3>
<p>Для обрезки начальных или концевых символов используется функция <span class="b">Trim</span>:</p>
<pre class="brush:c#;">
...
<p>Функция Trim без параметров обрезает начальные и конечные пробелы и возвращает обрезанную строку. Чтобы явным образом указать, какие 
начальные и конечные символы следует обрезать, мы можем передать в функцию массив этих символов.</p>
<p>Эта функция имеет частичные аналоги: функция <span class="b">TrimStart</span> обрезает начальные символы, а функция <span class="b">TrimEnd</span> обрезает конечные символы.</p>
<p>Обрезать определенную часть строки позволяет функция <span class="b">Substring</span>:</p>
<pre class="brush:c#;">
...
<p>Функция Substring также возвращает обрезанную строку. В качестве параметра первая использованная версия применяет индекс, начиная с 
которого надо обрезать строку. Вторая версия применяет два параметра - индекс начала обрезки и длину вырезаемой части строки.</p>
<h3>Вставка</h3>
<p>Для вставки одной строки в другую применяется функция <span class="b">Insert</span>:</p>
<pre class="brush:c#;">
string text = "Хороший день";
...
<p>Первым параметром в функции Insert является индекс, по которому надо вставлять подстроку, а второй параметр - собственно подстрока.</p>
<h3>Удаление строк</h3>
<p>Удалить часть строки помогает метод <span class="b">Remove</span>:</p>
<pre class="brush:c#;">
string text = "Хороший день";
...
<p>Первая версия метода Remove принимает индекс в строке, начиная с которого надо удалить все символы. Вторая версия принимает еще один параметр - сколько 
символов надо удалить.</p>
<h3>Замена</h3>
<p>Чтобы заменить один символ или подстроку на другую, применяется метод <span class="b">Replace</span>:</p>
<pre class="brush:c#;">
string text = "хороший день";
...
<p>Во втором случае применения функции Replace строка из одного символа "о" заменяется на пустую строку, то есть фактически удаляется из текста. 
Подобным способом легко удалять какой-то определенный текст в строках.</p>
<h3>Смена регистра</h3>
<p>Для приведения строки к верхнему и нижнему регистру используются соответственно функции <span class="bb">ToUpper()</span> и 
<span class="b">ToLower()</span>:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./7.1.php">Назад</a><a href="./">Содержание</a><a href="./7.5.php">Вперед</a></p></div></div>
<div class="socBlock">
