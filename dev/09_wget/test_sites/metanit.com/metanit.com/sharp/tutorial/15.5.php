...
<html >
<head>
<title>C# и .NET | Агрегатные операции в LINQ</title>
<meta charset="utf-8" />
<meta name="description" content="Получение размера списка, нахождение минимального, максимального и среднего значения в наборе, сумма элементов в LINQ в языке C# и .NET, использование методов Sum, Min, Max, Average, Count, Aggregate">
...
<h2>Агрегатные операции</h2><div class="date">Последнее обновление: 13.02.2022</div>
...
<p>К агрегатным операциям относят различные операции над выборкой, например, получение числа элементов, получение минимального, максимального и среднего значения в выборке, а также суммирование 
значений.</p>
<h3>Метод Aggregate</h3>
<p>Метод Aggregate выполняет общую агрегацию элементов коллекции в зависимости от указанного выражения. Например:</p>
<pre class="brush:c#;">
...
<p>Переменная query будет представлять результат агрегации массива. В качестве условия агрегации используется выражение <code>(x,y)=&gt; x - y</code>, 
то есть вначале из первого элемента вычитается второй, потом из получившегося значения вычитается третий и так далее. То есть будет эквивалентно выражению:</p>
<pre class="brush:c#;">
int query = 1 - 2 - 3 - 4 - 5
</pre>
<p>В итоге мы получим число -13. Соответственно мы бы могли использовать любые другие операции, например, сложение:</p>
<pre class="brush:c#;">
int query = numbers.Aggregate((x,y)=&gt; x + y); // аналогично 1 + 2 + 3 + 4 + 5
</pre>
<p>Еще одна версия метода позволяет задать начальное значение, с которого начинается цепь агрегатных операций:</p>
<pre class="brush:c#;">
...
<p>В данном случае объединяются все элементы массива words, но первым элемент агрегатной операции будет строка "Text:".</p>
<h3>Получение размера выборки. Метод Count</h3>
<p>Для получения числа элементов в выборке используется метод <span class="b">Count()</code>:</p>
<pre class="brush:c#;">
int[] numbers = { 1, 2, 3, 4, 10, 34, 55, 66, 77, 88 };
int size = numbers.Count();  // 10
Console.WriteLine(size);
</pre>
<p>Метод <code>Count()</code> в одной из версий также может принимать лямбда-выражение, которое устанавливает условие выборки. Поэтому мы можем в данном 
случае не использовать выражение <code>Where</code>:</p>
<pre class="brush:c#;">
int[] numbers = { 1, 2, 3, 4, 10, 34, 55, 66, 77, 88 };
...
<h3>Получение суммы</h3>
<p>Для получения суммы значений применяется метод <span class="b">Sum</span>:</p>
<pre class="brush:c#;">
int[] numbers = { 1, 2, 3, 4, 10, 34, 55, 66, 77, 88 };
...
<p>Метод <code>Sum()</code> имеет ряд перегрузок. В частности, если у нас набор сложных объектов, как в примере выше, то мы можем указать свойство, значения которого 
будут суммироваться:</p>
<pre class="brush:c#;">
...
<p>В данном случае вычисляется сумма значений свойств Age объектов Person из массива people.</p>
<h3>Максимальное, минимальное и среднее значения</h3>
<p>Для нахождения минимального значения применяется метод <span class="b">Min()</span>, для получения максимального - метод <span class="b">Max()</span>, 
а для нахождения среднего значения - метод <span class="b">Average()</span>. Их действие похоже на методы Sum и Count:</p>
<pre class="brush:c#;">
int[] numbers = { 1, 2, 3, 4, 10, 34, 55, 66, 77, 88 };
...
<p>Если мы работаем со сложными объектами, то в эти методы передается делегат, который принимает свойство, применяемое в вычислениях:</p>
<pre class="brush:c#;">
Person[] people = { new Person("Tom", 37), new Person("Sam", 28), new Person("Bob", 41) };
...
<p>В данном случае для вычислений применяется свойство Age, то есть вычисляется минимальный, максимальный и средний возраст.</p>
...
<div class="nav"><p><a href="./15.4.php">Назад</a><a href="./">Содержание</a><a href="./15.11.php">Вперед</a></p></div></div>
<div class="socBlock">
