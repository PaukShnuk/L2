...
<html >
<head>
<title>C# и .NET | Ограничения обобщений</title>
<meta charset="utf-8" />
<meta name="description" content="Ограничения обобщений в языке программирования C#, ключевое слово where, создание ограничения для классом и методов">
...
<h2>Ограничения обобщений</h2><div class="date">Последнее обновление: 29.12.2021</div>
...
<p>С помощью универсальных параметров мы можем типизировать обобщенные классы любым типом. Однако иногда возникает необходимость конкретизировать тип. 
Например, у нас есть следующий класс Message, который представляет некоторое сообщение:</p>
<pre class="brush:c#;">
class Message
{
...
<p>И, допустим, мы хотим определить метод для отправки сообщений в виде объектов Message. На первый взгляд мы можем определить и использовать следующий метод:</p>
<pre class="brush:c#;">
...
<p>Метод SendMessage в качестве параметра message принимает объект Message и эмулирует его отправку. Вроде все нормально и что-то лучше вряд ли можно придумать. 
Но у класса Message могут быть классы-наследники. Например, класс EmailMessage для email-сообщений, SmsMessage - для sms-сообщений и так далее</p>
<pre class="brush:c#;">
...
<p>Что если мы хотим также отправлять сообщения, которые представляют эти классы? Проблемы вроде нет, поскольку метод SendMessage принимает объект Message и соответственно также и объекты производных классов:</p>
<pre class="brush:c#;">
...
<p>Но здесь мы сталкиваемся с преобразованием типов: от EmailMessage к Message. Кроме того, опять же возможна проблема типобезопасности, если мы 
захотим преобразовать объект message в объект производных классов. И в этом случае чтобы избежать преобразований, мы можем применить обобщения:</p>
<pre class="brush:c#;">
...
<p>Обобщения позволяют избежать преобразований, но теперь мы сталкиваемся с другой проблемой - универсальный параметр T подразумевает любой тип. Но не любой тип 
...
нам достуны только методы типа object.</p>
<p>Таким образом, возникает проблема: надо избежать преобразований типов и соответственно использовать обобщения, а с другой стороны, необходимо обращаться внутри метода к 
функционалу класса Message. И ограничения обобщений позволяют решить эту проблему.</p>
<h3>Ограничения методов</h3>
<p>Ограничения методов указываются после списка параметров после оператора <span class="b">where</span>:</p>
<pre class="brush:c#;">имя_метода&lt;T&gt;(параметры) where T: тип_ограничения</pre>
<p>После оператора <span class="b">where</span> указывается универсальный параметр, для которого применяется ограничение. И через двоеточие указывается тип ограничения - обычно в 
качестве ограничения выступает конкретный тип.</p>
<p>Например, применим ограничения к методу SendMessage, который отправляет объекты Message</p>
<pre class="brush:c#;">
SendMessage(new Message("Hello World"));
...
<p>Выражение <code>where T: Message</code> в определении метода SendMessage говорит, что через универсальный параметр T будут передаваться объекты класса Message и 
производных классов. Благодаря этому компилятор будет знать, что T будет иметь функционал класса Message, и соответственно мы сможем обратиться к методам и свойствам класса Message внутри метода без проблем.</p>
<p>При вызове метода нам необязательно указывать тип в угловых скобках - компилятор на основании переданного значения сам определит каким тиом типизиуется метод:</p>
<pre class="brush:c#;">SendMessage(new EmailMessage("Bye World"));</pre>
<p>Однако это можно сделать и явно</p>
<pre class="brush:c#;">SendMessage&lt;EmailMessage&gt;(new EmailMessage("Bye World"));</pre>
<h3>Ограничения обобщений в типах</h3>
<p>Подобным образом можно определять и ограничения обобщенных типов. Например, ограничения обобщенных классов:</p>
<pre class="brush:c#;">class имя_класса&lt;T&gt; where T: тип_ограничения</pre>
<p>В качестве примера определим класс мессенджера, который будет отправлять сообшения в виде объектов Message:</p>
<pre class="brush:c#;">
...
<p>Здесь для класса Messenger опять же установлено ограничение <code>where T : Message</code>. То есть внутри класса Messenger все объекты типа T можно использовать 
как объекты Message. И в данном случае в классе Messenger в методе SendMessage опять эмулируется отправка сообшений.</p> 
<p>Применим класс для отправки сообщений:</p>
<pre class="brush:c#;">
...
<h3>Типы ограничений и стандартные ограничения</h3>
<p>В качестве ограничений мы можем использовать следующие типы:</p>
<ul>
<li><p>Классы</p></li>
<li><p>Интерфейсы</p></li>
<li><p>class - универсальный параметр должен представлять класс</p></li>
<li><p>struct - универсальный параметр должен представлять структуру</p></li>
<li><p>new() - универсальный параметр должен представлять тип, который имеет общедоступный (public) конструктор без параметров</p></li>
</ul>
<p>Есть ряд стандартных ограничений, которые мы можем использовать. В частности, можно указать ограничение, чтобы использовались только структуры или другие типы значений:</p>
<pre class="brush:c#;">
class Messenger&lt;T&gt; where T : struct
{}
</pre>
<p>При этом использовать в качестве ограничения конкретные структуры в отличие от классов нельзя.</p>
<p>Также можно задать в качестве ограничения ссылочные типы:</p>
<pre class="brush:c#;">
class Messenger&lt;T&gt; where T : class
{}
</pre>
<p>А также можно задать с помощью слова <span class="b">new</span> в качестве ограничения класс или структуру, которые имеют общедоступный конструктор без параметров:</p>
<pre class="brush:c#;">
class Messenger&lt;T&gt; where T : new()
{}
</pre>
<p>Если для универсального параметра задано несколько ограничений, то они должны идти в определенном порядке:</p>
<ol>
<li><p>Название класса, <code>class</code>, <code>struct</code>. Причем мы можем одновременно определить только одно из этих ограничений</p></li>
<li><p>Название интерфейса</p></li>
...
<p>Если класс использует несколько универсальных параметров, то последовательно можно задать ограничения к каждому из них:</p>
<pre class="brush:c#;">
class Messenger&lt;T, P&gt; 
    where T : Message
    where P: Person
...
<p>В данном случае для параметра P будут передаваться объекты типа Person, а для параметра T - объекты Message.</p>
<p>Применим классы:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./3.12.php">Назад</a><a href="./">Содержание</a><a href="./3.39.php">Вперед</a></p></div></div>
<div class="socBlock">
