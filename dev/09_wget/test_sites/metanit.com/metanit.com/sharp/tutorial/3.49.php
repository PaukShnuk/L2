...
<html >
<head>
<title>C# и .NET | Применение интерфейсов</title>
<meta charset="utf-8" />
<meta name="description" content="Применение интерфейсов в C# и .NET Core, их реализация в классах и структурах, интерфейсы в преобразованиях типов">
...
<h2>Применение интерфейсов</h2><div class="date">Последнее обновление: 01.01.2022</div>
...
<p>Интерфейс представляет некое описание типа, набор компонентов, который должен иметь тип данных. И, собственно, мы не можем создавать объекты интерфейса напрямую с 
...
классе и структуре к ним можно применять только модификатор <code>public</code>.</p>
...
<p>В данной программе определен метод <code>DoAction()</code>, который в качестве параметра принимает объект интерфейса 
...
<p>В данном случае интерфейс <code>IMovable</code> определяет реализацию по умолчанию для метода <code>Move</code>. Класс Person не реализует этот метод, поэтому он применяет 
реализацию по умолчанию в отличие от класса <code>Car</code>, который определяет свою реализацию для метода Move.</p>
<p>Стоит отметить, что хотя для объекта класса <code>Person</code> мы можем вызвать метод <code>Move</code> - ведь класс <code>Person</code> применяет интерфейс <code>IMovable</code>, тем не менее мы не можем написать так:</p>
<pre class="brush:c#;">
...
<pre class="brush:c#;">if (hello is Message someMessage) someMessage.Print();</pre>
...
<div class="nav"><p><a href="./3.9.php">Назад</a><a href="./">Содержание</a><a href="./3.44.php">Вперед</a></p></div></div>
<div class="socBlock">
