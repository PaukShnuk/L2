...
<html >
<head>
<title>C# и .NET | Работа с XML с помощью классов System.Xml</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с XML с помощью классов System.Xml XmlNode, XmlDocument, XmlElement в языке программирования C# и .NET">
...
<h2>Работа с XML с помощью классов System.Xml</h2><div class="date">Последнее обновление: 20.02.2022</div>
...
<p>Для работы с XML в C# можно использовать несколько подходов. В первых версиях фреймворка основной функционал работы с XML предоставляло пространство 
имен <span class="b">System.Xml</span>. В нем определен ряд классов, которые позволяют манипулировать xml-документом:</p>
<ul>
<li><p><span class="b">XmlNode</span>: представляет узел xml. В качестве узла может использоваться весь документ, так и отдельный элемент</p></li>
<li><p><span class="b">XmlDocument</span>: представляет весь xml-документ</p></li>
<li><p><span class="b">XmlElement</span>: представляет отдельный элемент. Наследуется от класса XmlNode</p></li>
<li><p><span class="b">XmlAttribute</span>: представляет атрибут элемента</p></li>
<li><p><span class="b">XmlText</span>: представляет значение элемента в виде текста, то есть тот текст, который находится в элементе между его открывающим и закрывающим тегами</p></li>
<li><p><span class="b">XmlComment</span>: представляет комментарий в xml</p></li>
<li><p><span class="b">XmlNodeList</span>: используется для работы со списком узлов</p></li>
</ul>
<p>Ключевым классом, который позволяет манипулировать содержимым xml, является <span class="bb">XmlNode</span>, поэтому рассмотрим некоторые его основные 
методы и свойства:</p>
<ul>
<li><p>Свойство <span class="b">Attributes</span> возвращает объект <code>XmlAttributeCollection</code>, который представляет коллекцию атрибутов</p></li>
<li><p>Свойство <span class="b">ChildNodes</span> возвращает коллекцию дочерних узлов для данного узла</p></li>
<li><p>Свойство <span class="b">HasChildNodes</span> возвращает <code>true</code>, если текущий узел имеет дочерние узлы</p></li>
<li><p>Свойство <span class="b">FirstChild</span> возвращает первый дочерний узел </p></li>
<li><p>Свойство <span class="b">LastChild</span> возвращает последний дочерний узел </p></li>
<li><p>Свойство <span class="b">InnerText</span> возвращает текстовое значение узла</p></li>
<li><p>Свойство <span class="b">InnerXml</span> возвращает всю внутреннюю разметку xml узла</p></li>
<li><p>Свойство <span class="b">Name</span> возвращает название узла. Например, <code>&lt;user&gt;</code> - значение свойства Name равно "user"</p></li>
<li><p>Свойство <span class="b">ParentNode</span> возвращает родительский узел у текущего узла</p></li>
</ul>
<p>Применим эти классы и их функционал. И вначале для работы с xml создадим новый файл. Назовем его <span class="b">people.xml</span> и определим в нем следующее содержание:</p>
<pre class="brush:xml;">
...
<p>Теперь пройдемся по этому документу и выведем его данные на консоль:</p>
<pre class="brush:c#;">
using System.Xml;
...
<p>В итоге я получу следующий вывод на консоли:</p>
<div class="console">
...
<p>Чтобы начать работу с документом xml, нам надо создать объект XmlDocument и затем загрузить в него xml-файл: <code>xDoc.Load("people.xml");</code></p>
<p>При разборе xml для начала мы получаем корневой элемент документа с помощью свойства <code>xDoc.DocumentElement</code>. Далее уже происходит собственно разбор 
узлов документа.</p>
<p>В цикле <code>foreach(XmlNode xnode in xRoot)</code> пробегаемся по всем дочерним узлам корневого элемента. Так как дочерние узлы представляют элементы 
<code>&lt;person&gt;</code>, то мы можем получить их атрибуты: <code>XmlNode attr = xnode.Attributes.GetNamedItem("name");</code> и вложенные элементы: <code>foreach(XmlNode childnode in xnode.ChildNodes)</code></p>
<p>Чтобы определить, что за узел перед нами, мы можем сравнить его название: <code>if(childnode.Name=="company")</code></p>
<p>Подобным образом мы можем создать объекты классов и структур по данным из xml:</p>
<pre class="brush:c#;">
using System.Xml;
...
<p>В данном случае определен класс Person с тремя свойствами. При переборе узлов файла xml значения элементов и их атрибутов передается объекту класса Person.</p>
