...
<html >
<head>
<title>C# и .NET | Проверка наличия и получение элементов в LINQ</title>
<meta charset="utf-8" />
<meta name="description" content="Проверка наличия и получение элементов в LINQ в языке программирования C# и .NET, соответствие коллекции условию, Методы All, Any, Contains, First, FirstOrDefault, Last, LastOrDefault">
...
<h2>Проверка наличия и получение элементов</h2><div class="date">Последнее обновление: 14.02.2022</div>
...
<p>Ряд методов в LINQ позволяют проверить наличие элементов в коллекции и получить их.</p>
<h3>All</h3>
<p>Метод <span class="b">All()</span> проверяет, соответствуют ли все элементы условию. Если все элементы соответствуют условию, то возвращается true. Например:</p>
<pre class="brush:c#;">
...
<p>В первом случае проверяем, все ли строки в массиве people имеют длину в три символа. Поскольку это условие верно, то метод All возвращает true. 
Во втором случае смотрим, все ли строки начинаются с буквы "T". это условие ложно, поэтому метод All возвращает false.</p>
<h3>Any</h3>
<p>Метод <span class="b">Any()</span> действует подобным образом, только возвращает true, если хотя бы один элемент коллекции определенному условию:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Tim", "Bob", "Sam" };
...
<p>Первое выражение вернет <code>false</code>, поскольку все строки имеют длину в 3 символа. Второе выражение возвратит <code>true</code>, 
так как у нас есть в коллекции есть строки, которые начинаются на букву T.</p>
<h3>Contains</h3>
<p>Метод <span class="b">Contains()</span> возвращает <code>true</code>, если коллекция содержит определенный элемент.</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Tim", "Bob", "Sam" };
...
<p>Стоит отметить, что для сравнения объектов применяется реализация метода Equals. Соответственно если мы работаем с объектами своих типов, то мы можем реализовать 
данный метод:</p>
<pre class="brush:c#;">
...
<p>Но стоит отметить, что Contains не всегда может вернуть ожидаемые данные. Например:</p>
<pre class="brush:c#;">
string[] people = { "tom", "Tim", "bOb", "Sam" };
...
<p>В данном случае в массиве нет строки "Tom", а есть строка "tom". Поэтому вызов <code>people.Contains("Tom")</code> возвратит false. Но подобное 
...
<p>Интерфейс IEqualityComparer типизируется типом сравниваемых данных (в данном случае типом String). Для реализации этого интерфейса необходимо 
определить методы Equals и GetHashCode. В методе Equals сравниваем строки в нижнем регистре, а в методе GetHashCode возвращаем возвращаем хеш-код строки в нижнем регистре.</p>
<h3>First/FirstOrdefault</h3>
<p>Метод <span class="b">First()</span> возвращает первый элемент последовательности:</p>
<pre class="brush:c#;">
...
<p>Также в метод First можно передать метод, который представляет условие. В этом случае метод возвращает первый элемент, который соответствует условию:</p>
<pre class="brush:c#;">
...
<p>Здесь выбираем первый элемент, длина которого - 4 символа.</p>
<p>Стоит учитывать, что если коллекция пуста или в коллекции нет элементов, который соответствуют условию, то будет сгенерировано исключение.</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Bob", "Kate", "Tim", "Mike", "Sam" };
...
<p>Метод <span class="b">FirstOrDefault()</span> также возвращает первый элемент и также может принимать условие, только если коллекция пуста или в коллекции не окажется элементов, 
которые соответствуют условию, то метод возвращает значение по умолчанию:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Bob", "Kate", "Tim", "Mike", "Sam" };
...
<p>Стоит учитывать, что для коллекций ссылочных типов FirstOrDefault возвращает значение типа T? (в примере выше - string?), то есть значение, которое может 
быть равно null, а значение по умолчанию - null. Для коллекций числовых типов возвращается непосредственно значение типа T, а значение по умолчанию - 0.</p>
<p>Но мы можем настроить значение по умолчанию, передав его в качестве одного из аргументов:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Bob", "Kate", "Tim", "Mike", "Sam" };
...
<h3>Last и LastOrDefault</h3>
<p>Метод <span class="b">Last()</span> аналогичен по работе методу First, только возвращает последний элемент. Если коллекция не содержит элемент, который соответствуют условию, или 
вообще пуста, то метод генерирует исключение.</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Bob", "Kate", "Tim", "Mike", "Sam" };
...
<p>Метод <span class="b">LastOrDefault()</span> возвращает последний элемент или значение по умолчанию, если коллекция не содержит элемент, который соответствуют условию, или 
вообще пуста:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Bob", "Kate", "Tim", "Mike", "Sam" };
...
<div class="nav"><p><a href="./15.7.php">Назад</a><a href="./">Содержание</a><a href="./15.8.php">Вперед</a></p></div></div>
<div class="socBlock">
