...
<html >
<head>
<title>C# и .NET | Виртуальные методы и свойства</title>
<meta charset="utf-8" />
<meta name="description" content="Переопределение методов в производном классе в языке программирования C#, виртуальные методы и свойства, модификаторы virtual и override, sealed, запрет переопределения">
...
<h2>Виртуальные методы и свойства</h2><div class="date">Последнее обновление: 28.12.2021</div>
...
<p>При наследовании нередко возникает необходимость изменить в классе-наследнике функционал метода, который был унаследован 
от базового класса. В этом случае класс-наследник может переопределять методы и свойства базового класса.</p>
<p>Те методы и свойства, которые мы хотим сделать доступными для переопределения, в базовом классе помечается модификатором <span class="b">virtual</span>. 
Такие методы и свойства называют виртуальными.</p>
<p>А чтобы переопределить метод в классе-наследнике, этот метод определяется с модификатором <span class="b">override</span>. 
Переопределенный метод в классе-наследнике должен иметь тот же набор параметров, что и виртуальный метод в базовом классе.</p>
...
<p>Здесь класс Person представляет человека. Класс Employee наследуется от Person и представляет сотруднника предприятия. Этот класс кроме 
унаследованного свойства Name имеет еще одно свойство - Company.</p>
<p>Чтобы сделать метод Print доступным для переопределения, этот метод определен с модификатором <span class="b">virtual</span>. Поэтому мы можем переопределить этот метод, но можем и не переопределять. 
Допустим, нас устраивает реализация метода из базового класса. В этом случае объекты Employee будут использовать реализацию метода Print из класса Person:</p>
<pre class="brush:c#;">
...
<p>Но также можем переопределить виртуальный метод. Для этого в классе-наследнике определяется метод с модификатором <span class="b">override</span>, 
который имеет то же самое имя и набор параметров:</p>
<pre class="brush:c#;">
class Employee : Person
{
	public string Company { get; set; }
...
<p>Возьмем те же самые объекты:</p>
<pre class="brush:c#;">
Person bob = new Person("Bob");
bob.Print(); // вызов метода Print из класса Person
...
<p>Виртуальные методы базового класса определяют интерфейс всей иерархии, то есть в любом производном классе, который не является прямым наследником от 
...
метод Print.</p>
<p>При переопределении виртуальных методов следует учитывать ряд ограничений:</p>
<ul>
<li><p>Виртуальный и переопределенный методы должны иметь один и тот же модификатор доступа. То есть если виртуальный метод определен с помощью модификатора 
public, то и переопредленный метод также должен иметь модификатор public.</p></li>
<li><p>Нельзя переопределить или объявить виртуальным статический метод.</p></li>
</ul>
<h3>Ключевое слово base</h3>
<p>Кроме конструкторов, мы можем обратиться с помощью ключевого слова <span class="b">base</span> к другим членам базового класса. 
В нашем случае вызов <code>base.Print();</code> будет обращением к методу Print() в классе Person:</p>
<pre class="brush:c#;">
class Employee : Person
{
    public string Company { get; set; }
...
<p>Также как и методы, можно переопределять свойства:</p>
<pre class="brush:c#;">
class Person
{
	int age = 1;
...
<p>В данном случае в классе Person определено виртуальное свойство Age, которое устанавливает значение, если оно больше 0 и меньше 110. 
В классе Employee это свойство переопределено - возраст работника должен быть не меньше 18.</p>
<pre class="brush:c#;">
Person bob = new Person("Bob");
...
<h3>Запрет переопределения методов</h3>
<p>Также можно запретить переопределение методов и свойств. В этом случае их надо объявлять с модификатором <span class="b">sealed</span>:</p>
<pre class="brush:c#;">
class Employee : Person
{
    public string Company { get; set; }
...
<p>При создании методов с модификатором sealed надо учитывать, что sealed применяется в паре с override, то есть только в переопределяемых методах.</p>
<p>И в этом случае мы не сможем переопределить метод Print в классе, унаследованном от Employee.</p>
...
<ul>
<li><a title="Вопросы для самопроверки" href="../questions/2.11.php"><i class="fa fa-lg fa-question"></i><span class="dop-txt">Вопросы для самопроверки</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./3.11.php">Назад</a><a href="./">Содержание</a><a href="./3.41.php">Вперед</a></p></div></div>
<div class="socBlock">
