...
<html >
<head>
<title>C# и .NET | AssemblyLoadContext и динамическая загрузка и выгрузка сборок</title>
<meta charset="utf-8" />
<meta name="description" content="Динамическая загрузка и выгрузка сборок в домене приложения в C# и .NET с помощью класса AssemblyLoadContext">
...
<h2>AssemblyLoadContext и динамическая загрузка и выгрузка сборок</h2><div class="date">Последнее обновление: 22.02.2022</div>
...
<p>В статье <a href="https://metanit.com/sharp/tutorial/14.3.php">Динамическая загрузка сборок и позднее связывание</a> рассматривалось, динамически 
...
Для этого применяется класс <span class="b">AssemblyLoadContext</span> из пространства имен <code>System.Runtime.Loader</code>, который представляет контекст загрузки и выгрузки сборок. 
Рассмотрим, как его использовать.</p>
<p>Допустим, у нас есть консольный проект <span class="b">MyApp</span> со следующим файлом <span class="b">Program.cs</span>:</p>
<pre class="brush:c#;">
class Program
{
...
<p>Эта программа содержит метод Square для вычисления квадрата, и по умолчанию она компилируется в сборку <span class="b">MyApp.dll</span>. 
Загрузим эту сборку, чтобы использовать ее метод Square.</p>
<p>Для создания объекта AssemblyLoadContext применяется следующий конструктор:</p>
<pre class="brush:c#;">public AssemblyLoadContext (string? name, bool isCollectible = false);</pre>
<p>В конструкторе первый параметр устанавливает имя контекста - это может произвольная строка. 
Второй параметр - <code>isCollectible</code> устанавливает, можно ли загруженные сборки выгружать. Значение true указывает, что загруженные 
сборки можно выгружать.</p>
<p>Для загрузки сборок класс AssemblyLoadContext предоставляет ряд методов. Некоторые из них:</p>
<ul>
<li><p><code>Assembly LoadFromAssemblyName (AssemblyName assemblyName)</code>: загружает определенную сборку по имени, которое представлено типом System.Reflection.AssemblyName</p></li>
<li><p><code>Assembly LoadFromAssemblyPath (string assemblyPath)</code>: загружает сборку по определенному пути (путь должен быть абсолютным)</p></li>
<li><p><code>Assembly LoadFromStream (System.IO.Stream stream)</code>: загружает определенную сборку из потока Stream</p></li>
</ul>
<p>Использовав один из этих методов, мы можем получить доступ к сборке через тип Assembly и обращаться к ее функционалу.</p>
<p>После завершения работы со сборкой мы можем вызвать у AssemblyLoadContext метод <span class="b">Unload()</span> и выгрузить контекст со всеми загруженными 
сборками и тем самым снизить потребление памяти и увеличить общую производительность.</p>
<p>Рассмотрим полный пример:</p>
<pre class="brush:c#;">
...
<p>Все эти действия оформляются в виде отдельного метода Square(). В качестве параметра он принимает число, квадрат которого надо вычислить.</p>
<p>Вначале в методе создается объект AssemblyLoadContext:</p>
<pre class="brush:c#;">var context = new AssemblyLoadContext(name: "Square", isCollectible: true);</pre>
<p>Обратите внимание, что параметру isCollectible передается значение true, что позволит выгружать ранее загруженные сборки.</p>
<p>Класс AssemblyLoadContext определяет событие <span class="b">Unloadig</span>, благодаря чему мы можем повесить обработчик и определить момент выгрузки контекста.</p>
<pre class="brush:c#;">context.Unloading += Context_Unloading;</pre>
<p>Далее используется метод <code>LoadFromAssemblyPath</code> для загузки сборки MyApp.dll по абсолютному пути. В данном случае предполагается, что 
файл сборки находится в одной папке с текущим приложением.</p>
<pre class="brush:c#;">Assembly assembly = context.LoadFromAssemblyPath(assemblyPath);</pre>
<p>Получив сборку, с помощью рефлексии обращаемся к методу Square и получаем квадрат числа.</p>
<p>Затем смотрим, какие сборки загружены в текущий домен. Среди них мы сможем найти и MyApp.dll. И в конце выгружаем контекст:</p>
<pre class="brush:c#;">context.Unload();</pre>
<p>Данный метод Square вызывается в методе Main:</p>
<pre class="brush:c#;">
Square(8);
// очистка
GC.Collect();
...
<p>Но обратите внимание, что выгрузка контекста сама по себе не означает немедленной очистки памяти. Вызов метода Unload только инициирует процесс выгрузки, 
...
в конце вызываются методы <code>GC.Collect()</code> и <code>GC.WaitForPendingFinalizers()</code>.</p>
...
<p>Как видно, после выгрузки контекста AssemblyLoadContext сборки MyApp в списке загруженных сборок больше не наблюдается.</p>
...
<div class="nav"><p><a href="./18.2.php">Назад</a><a href="./">Содержание</a><a href="./21.1.php">Вперед</a></p></div></div>
<div class="socBlock">
