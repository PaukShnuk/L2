...
<html >
<head>
<title>C# и .NET | Проверка на null, операторы ?. и ??</title>
<meta charset="utf-8" />
<meta name="description" content="Оператор условного null или элвис-оператор и оператор null-объединения ?. и ??, null guard в языке программирования C# и .NET">
...
<h2>Проверка на null, операторы ?. и ??</h2><div class="date">Последнее обновление: 11.11.2021</div>
...
<h3>Проверка на null. Null guard</h3>
<p>Если мы собираемся использовать переменную или параметр, которые допускают значение <span class="b">null</span>, то есть представляют 
nullable-тип (не важно значимый или ссылочный), то, чтобы избежать возникновения NullReferenceException, мы можем проверить на null:</p>
<pre class="brush:c#;">
...
<p>В данном случае если параметр text не равен null, то вызываем у строки метод <code>ToUpper()</code>, который переводит символы строки в верхний регистр.</p>
<p>Кроме того, с помощью оператора <span class="b">is</span> мы можем проверить значение объекта:</p>
<pre class="brush:c#;">объект is значение</pre>
<p>Если объект слева от оператора <span class="b">is</span> имеет значение справа от оператора. тогда оператор is возвращает <code>true</code>, иначе 
возвращается <code>false</code></p>
<p>Например, проверка параметра/переменной на значение null:</p>
<pre class="b">
void PrintUpper(string? text)
{
...
<p>Или, наоборот, с помощью <span class="b">is not</span> можно проверить отсутствие значения:</p>
<pre class="b">
void PrintUpper(string? text)
{
    if (text is not null) 
...
<p>Также можно проверить на соответствие типу, значение которого мы собираемся использовать:</p>
<pre class="b">
void PrintUpper(string? text)
{
    if (text is string)
...
<p>Подобные проверки еще называются <span class="b">null guard</span> или условно говоря "защита от null".</p>
<h3 id="nullcoalesce">Оператор ??</h3>
<p>Оператор ?? называется оператором <span class="b">null-объединения</span>. Он применяется для установки значений по умолчанию для типов, 
которые допускают значение null:</p>
<pre class="brush:c#;">левый_операнд ?? правый_операнд</pre>
<p>Оператор ?? возвращает левый операнд, если этот операнд не равен <code>null</code>. 
Иначе возвращается правый операнд. При этом левый операнд должен принимать null. Посмотрим на примере:</p>
<pre class="brush:c#;">
string? text = null;
...
<p>Но мы не можем написать следующим образом:</p>
<pre class="brush:c#;">
int x = 44;
int y = x ?? 100;
</pre>
<p>Здесь переменная x представляет значимый тип int и не может принимать значение null, поэтому в качестве левого операнда в операции ?? 
она использоваться не может.</p>
<p>Также можно использовать производный оператора <span class="b">??=</span></p>
<pre class="brush:c#;">
string? text = null;
text ??= "Sam";
// аналогично
...
<h3>Оператор условного null</h3>
<p>Иногда при работе с объектами, которые принимают значение null, мы можем столкнуться с ошибкой: мы пытаемся обратиться к объекту, 
а этот объект равен null. Например, пусть у нас есть следующая система классов:</p>
<pre class="brush:c#;">
class Person
{
...
<p>Объект Person представляет человека. Его свойство Company представляет компанию, где человек работает. Но человек может не работать, поэтому свойство Company имеет 
тип <code>Company?</code>, то есть может иметь значение null.</p>
<p>Класс Company в свою очередь содержит свойство WebSite, которое представляет веб-сайт компании. Но у компании может и не быть собственного веб-сайта. 
Поэтому это свойство имеет тип <code>string?</code>, то есть также допускает значение null.</p>
<p>Допустим, нам надо вывести на консоль заглавными буквами веб-сайт компании, где работает человек (если он, конечно, работает и если у компании, где он работает, есть сайт). На первый взгляд мы можем написать следующую 
конструкцию:</p>
<pre class="brush:c#;">
...
<p>В методе <code>PrintWebSite()</code> принимаем объект Person? и, чтобы избежать исключения NullReferenceException, последовательно проверяем все используемые значения на null, чтобы в конце с помощью метода 
<code>ToUpper()</code> вывести заглавными буквами название сайта.</p>
<p>Хоть это и рабочий способ, но для простого вывода строки получается многоэтажная конструкция, но на самом деле ее можно сократить:</p>
<pre class="brush:c#;">
void PrintWebSite(Person? person)
{
...
<p>Конструкция намного проще, но все равно получается довольно большой. И чтобы ее упростить, в C# есть оператор условного null 
(Null-Conditional Operator)  - оператор <span class="b">?.</span>:</p>
<pre class="brush:c#;">
объект?.компонент
</pre>
<p>Если объект не равен <span class="b">null</span>, то происходит обращение к компоненту объекта - полю, свойству, методу. Если объект 
представляет значение null, обращение к компаненту метода не происходит.</p>
<p>Применим данный оператор, изменив предыдущий пример:</p>
<pre class="brush:c#;">
void PrintWebSite(Person? person)
{
    Console.WriteLine(person?.Company?.WebSite?.ToUpper());
...
<p>Таким образом, если person не равен null, то происходит обращение к его свойству Company. Если свойство Company не равно null, то идет обрашение к свойству 
WebSite объекта Company. Если свойство WebSite не равно null, то идет обращение к методу <code>ToUpper()</code>.</p>
...
<div class="nav"><p><a href="./2.17.php">Назад</a><a href="./">Содержание</a><a href="./4.1.php">Вперед</a></p></div></div>
<div class="socBlock">
