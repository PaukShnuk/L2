...
<html >
<head>
<title>C# и .NET | Атрибуты валидации</title>
<meta charset="utf-8" />
<meta name="description" content="Атрибуты валидации в System.ComponentModel.DataAnnotations в C# и .NET, класс ValidationAttribute, ErrorMessage и настройка сообщений об ошибках">
...
<h2>Атрибуты валидации</h2><div class="date">Последнее обновление: 23.02.2022</div>
...
<p>Для валидации модели мы можем использовать большой набор встроенных атрибутов. Все эти атрибуты представляют классы, унаследованные от класса 
<span class="b">ValidationAttribute</span></p>
<p>Имеется довольно большое количество атрибутов. Основные из них:</p>
<ul>
<li><p><span class="b">Required</span>: данный атрибут указывает, что свойство должно быть обязательно установлено, обязательно должно иметь какое-либо значение. 
Если свойство имеет значение <code>null</code>, то оно не проходит валидацию. Также не проходят валидацию свойства, которые представляют тип string, и которым присваивается 
пустая строка.</p>
<p>Из его свойств следует отметить свойство <code>AllowEmptyStrings</code>. Если оно имеет значение <code>true</code>, то для строковых свойств разрешено использовать 
пустые строки:</p>
<pre class="brush:c#;">
...
<li><p><span class="b">RegularExpression</span>: указывает на регулярное выражение, которому должно соответствовать значение свойства. Например, пусть у 
пользователя определено свойство Phone, которое хранит номер телефона:</p>
<pre class="brush:c#;">
...
<p>В этом случае номер телефона должен быть в формате "+xxxx-xxx-xxxx", например, +1111-111-2345.</p>
</li>
<li><p><span class="b">StringLength</span>: определяет допустимую длину для строковых свойств. В качестве первого параметра он принимает максимально 
допустимую длину строки. С помощью дополнительного свойства <code>MinimumLength</code> можно установить минимально допустимую длину строки</p>
<pre class="brush:c#;">
public class User
{
...
<p>В данном случае значение свойства Name должно иметь длину как минимум в 3 символа и максимум в 20 символов.</p>
</li>
<li><p><span class="b">Range</span>: задает диапазон допустимых числовых значений. В качестве первых двух параметров он принимает минимальное и максимальное значения:</p>
<pre class="brush:c#;">
[Range(1, 100)]
public int Age { get; set; }
</pre>
<p>Здесь значение свойства Age должно находиться в диапазоне от 1 до 100</p></li>
<li><p><span class="b">Compare</span>: позволяет сравнить значение текущего свойства со значением другого свойства, которое передается в этот атрибут.</p>
<p>Например, нередко при регистрации пользователя ему предоставляется два поля для ввода пароля, чтобы не ошибиться. В этом случае мы могли бы определить 
ля регистрации следующий класс:</p>
<pre class="brush:c#;">
public class User
{
    [Required]
...
<p>И если значения свойств Password и ConfirmPassword не будут совпадать, тогда мы получим ошибку валидации.</p>
</li>
<li><p><span class="b">Phone</span>: данный атрибут автоматически валидирует значение свойства, является ли оно телефонным номером. Фактически это встроенная альтернатива 
использованию регулярного выражения, как было показано выше</p>
<pre class="brush:c#;">
[Phone]
public string? Phone { get; set; }
</pre>
</li>
<li><p><span class="b">EmailAddress</span>: определяет, является ли значение свойства электронным адресом</p></li>
<li><p><span class="b">CreditCard</span>: определяет, является ли значение свойства номером кредитной карты</p></li>
<li><p><span class="b">Url</span>: определяет, является ли значение свойства гиперссылкой</p></li>
</ul>
<h3>Настройка сообщения об ошибке</h3>
<p>Класс <span class="b">ValidationAttribute</span> определяет для атрибутов ряд общих свойств и методов, из которых следует выделить свойство <span class="b">ErrorMessage</span>. 
Это свойство позволяет задать сообщение обошибке.</p>
<p>При выводе ошибок валидации .NET использует встроенные локализованные сообщение. А данное свойство как раз и позволяет переопределить сообщение об ошибке:</p>
<pre class="brush:c#;">
using System.ComponentModel.DataAnnotations;
...
<p>Например, возьмем свойство Name:</p>
<pre class="brush:c#;">
[Required(ErrorMessage = "Не указано имя пользователя")]
[StringLength(50, MinimumLength = 3, ErrorMessage = "Недопустимая длина имени")]
public string Name { get; set; }
</pre>
<p>Если для свойства Name будет передана пустая строка, то будет выводиться сообщение об ошибке <code>"Не указано имя пользователя"</code>. Если же 
длина имени меньше 2 или больше 50 символов, то выводится сообщение "Недопустимая длина имени".</p>
...
<h3>Передача в сообщение об ошибке аргументов</h3>
<p>Ряд атрибутов с помощью свойств могут принимать различные параметры. И эти параметры можно использовать в сообщении об ошибке. Например, изменим предыдущий код 
следующим образом:</p>
<pre class="brush:c#;">
using System.ComponentModel.DataAnnotations;
...
<p>Например, возьмем свойство Name и его атрибут StringLength:</p>
<pre class="brush:c#;">
[StringLength(50, MinimumLength = 3, ErrorMessage = "Длина имени должна быть в диапазоне от {2}-{1} символов")]
public string Name { get; set; }
</pre>
<p>В сообщение об ошибке с помощью плейсхолдеров {0}, {1}, {2} и т.д. можно передавать параметры атрибута. Параметр {0} в любом атрибуте представляет имя свойства, которое валидируется. 
То есть в данном случае это было бы "Name". {1} здесь - первый параметр атрибута - максимальное количество символов - 50. {2} - второй параметр - значение свойства MinimumLength. 
То есть в итоге сформируется сообщение Длина имени должна быть в диапазоне от 3-50 символов</p>
<p>Аналогично для свойство Age и его атрибута Range:</p>
<pre class="brush:c#;">
[Range(1, 100, ErrorMessage = "Возраст должен быть в диапазоне {1}-{2}")]
public int Age { get; set; }
</pre>
<p>Здесь параметр {0} представлял бы имя свойства - "Age". {1} здесь - первый параметр атрибута - минимальное значение - 1. {2} - максимальное значение 
100.</p>
