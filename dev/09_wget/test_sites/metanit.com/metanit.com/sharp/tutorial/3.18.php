...
<html >
<head>
<title>C# и .NET | Методы расширения</title>
<meta charset="utf-8" />
<meta name="description" content="Методы расширения (extension methods) в языке программирования C#, добавление к уже существующим классам новых методов">
...
<h2>Методы расширения</h2><div class="date">Последнее обновление: 11.11.2021</div>
...
<p>Методы расширения (extension methods) позволяют добавлять новые методы в уже существующие типы без создания нового производного класса. Эта функциональность бывает особенно 
...
Либо если мы не можем использовать стандартный механизм наследования, например, если классы определенны с модификатором sealed.</p>
<p>Например, нам надо добавить для типа <code>string</code> новый метод:</p>
<pre class="brush:c#;">
string s = "Привет мир";
char c = 'и';
int i = s.CharCount(c);
...
<p>Для того, чтобы создать метод расширения, вначале надо создать статический класс, который и будет содержать этот метод. В данном случае это класс 
<code>StringExtension</code>. Затем объявляем статический метод. Суть нашего метода расширения - подсчет количества определенных символов в строке.</p>
<p>Собственно метод расширения - это обычный статический метод, который в качестве первого параметра всегда принимает такую конструкцию: 
<code>this имя_типа название_параметра</code>, то есть в нашем случае <code>this string str</code>. Так как наш метод будет относиться к типу string, 
то мы и используем данный тип.</p>
<p>Затем у всех строк мы можем вызвать данный метод:</p>
<pre class="brush:c#;">int i = s.CharCount(c);</pre>
<p>Причем нам уже не надо указывать первый параметр. 
Значения для остальных параметров передаются в обычном порядке.</p>
<p>Применение методов расширения очень удобно, но при этом надо помнить, что метод расширения никогда не будет вызван, если он имеет ту же сигнатуру, 
что и метод, изначально определенный в типе.</p>
<p>Также следует учитывать, что методы расширения действуют на уровне пространства имен. То есть, если добавить в проект другое пространство имен, то метод 
не будет применяться к строкам, и в этом случае надо будет подключить пространство имен метода через директиву using.</p>
...
<div class="nav"><p><a href="./2.21.php">Назад</a><a href="./">Содержание</a><a href="./3.21.php">Вперед</a></p></div></div>
<div class="socBlock">
