...
<html >
<head>
<title>C# и .NET | Структуры</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и использование структур в языке программирования C# и .NET, конструкторы и инициализаторы структуры">
...
<h2>Структуры</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Наряду с классами структуры представляют еще один способ создания собственных типов данных в C#. Более того многие примитивные типы, например, 
...
<p>После слова <span class="b">struct</span> идет название структуры и далее в фигурных скобках размещаются элементы структуры - поля, методы и т.д.</p>
...
<p>Для использования структуры ее необходмо инициализировать. Для инициализации создания объектов структуры, как и в случае с классами, применяется вызов конструктура с оператором <span class="b">new</span>. 
...
<pre class="brush:c#;">new название_структуры();</pre>
...
поле <code>age</code> будет иметь значение 0. Для строк это значение <code>null</code>, которое указывает на отсутствие значения. Но далее, если поля доступны 
(а в данном случае поскольку они имеют модификатор <code>public</code> они доступны), мы можем измениь их значения. Так, здесь полю <code>name</code> присваивается строка "Tom". Соответственно 
при выполнении метода <code>Print()</code> мы получим следующий консольный вывод:</p>
<div class="console"><pre class="consoletext">Имя: Tom  Возраст: 0</pre></div>
<h4>Непосредственная иницилизация полей</h4>
<p>Если все поля структуры доступны (как в случае с полями структуры Person, который имеют модификатор <span class="b">public</span>), 
...
<span class="b">with</span>:</p>
<pre class="brush:c#;">
Person tom = new Person { name = "Tom", age = 22 };
Person bob = tom with { name = "Bob" };
bob.Print();    // Имя: Bob  Возраст: 22
</pre>
<p>В данном случае объект bob получает все значения объекта tom, а затем после оператора <span class="b">with</span> в фигурных скобках указывается 
...
<ul>
<li><a title="Вопросы для самопроверки" href="../questions/2.16.php"><i class="fa fa-lg fa-question"></i><span class="dop-txt">Вопросы для самопроверки</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./1.4.php">Назад</a><a href="./">Содержание</a><a href="./2.16.php">Вперед</a></p></div></div>
<div class="socBlock">
