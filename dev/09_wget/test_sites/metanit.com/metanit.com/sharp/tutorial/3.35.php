...
<html >
<head>
<title>C# и .NET | Конструкторы, инициализаторы и деконструкторы</title>
<meta charset="utf-8" />
<meta name="description" content="Конструкторы, инициализаторы и деконструкторы в C# и .NET, метод Deconstruct и декомпозиция объекта на части, ключевое слово this, инициализаторы объектов">
...
<h2>Конструкторы, инициализаторы и деконструкторы</h2><div class="date">Последнее обновление: 11.11.2021</div>
...
<h3>Создание конструкторов</h3>
<p>В прошлой статье для создания объекта использовался конструктор по умолчанию. Однако мы сами можем определить свои конструкторы. Как правило, конструктор выполняет инициализацию объекта. 
...
<p>Конструкторы могут иметь модификаторы, которые указываются перед именем конструктора. Так, в данном случае, чтобы конструктор был доступен вне класса Person, он определен с модификатором <span class="b">public</span>.</p>
<p>Определив конструктор, мы можем вызвать его для создания объекта Person:</p>
<pre class="brush:c#;">Person tom = new Person();  // Создание объекта Person</pre>
<p>В данном случае выражение <code>Person()</code> как раз представляет вызов определенного в классе конструктора (это больше не автоматический конструктор по умолчанию, которого у класса теперь нет). 
...
<p>Ключевое слово <span class="b">this</span> представляет ссылку на текущий экземпляр/объект класса. В каких ситуациях оно нам может пригодиться?</p>
<pre class="brush:c#;">
...
<p>В примере выше во втором и третьем конструкторе параметры называются также, как и поля класса. И чтобы разграничить параметры и поля класса, к полям класса обращение идет через ключевое слово <span class="b">this</span>. 
Так, в выражении</p>
<pre class="b">this.name = name;</pre>
<p>первая часть - <code>this.name</code> означает, что <code>name</code> - это 
...
<code>this</code> было бы необязательно. Также через ключевое слово <span class="b">this</span> можно обращаться к любому полю или методу.</p>
...
<span class="b">this</span>, передавая нужные значения для параметров:</p>
<pre class="brush:c#;">
class Person 
{
    public string name;
...
<p>Обратите внимание, как устанавливается поле <code>company</code>:</p>
<pre class="brush:c#;">company = { title = "Microsoft"}</pre>
...
что не все эти значения нужны. И вместо возвращаемых значений мы можм использовать прочерк <span class="b">_</span>. Например, нам 
...
<div class="nav"><p><a href="./3.1.php">Назад</a><a href="./">Содержание</a><a href="./1.4.php">Вперед</a></p></div></div>
<div class="socBlock">
