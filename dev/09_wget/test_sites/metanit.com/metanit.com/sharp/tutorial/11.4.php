...
<html >
<head>
<title>C# и .NET | Синхронизация потоков</title>
<meta charset="utf-8" />
<meta name="description" content="Использование в многпоточном приложении в C# и .NET общих ресурсов и синхронизация потоков с помощью оператора lock">
...
<h2>Синхронизация потоков</h2><div class="date">Последнее обновление: 07.02.2022</div>
...
<p>Нередко в потоках используются некоторые разделяемые ресурсы, общие для всей программы. Это могут быть общие переменные, файлы, другие ресурсы. Например:</p>
<pre class="brush:c#;">
int x = 0;
...
Для этого используется ключевое слово <span class="b">lock</span>. Оператор <span class="b">lock</span> определяет блок кода, внутри которого весь код блокируется и 
...
<p>Для блокировки с ключевым словом lock используется объект-заглушка, в данном случае это переменная <code>locker</code>. Обычно это переменная типа object. И когда выполнение доходит 
