...
<html >
<head>
<title>C# и .NET | Конструкция using</title>
<meta charset="utf-8" />
<meta name="description" content="Конструкция using в языке программирования C# и .NET, освобождение ресурсов, которые реализуют интерфейс IDisposable и метод Dispose">
...
<h2>Конструкция using</h2><div class="date">Последнее обновление: 18.02.2022</div>
...
<p>В прошлой теме, где рассматривалась реализация метода Dispose, говорилось, что для его вызова можно использовать следующую конструкцию try..catch:</p>
<pre class="brush:c#;">
Test();
...
<p>Однако синтаксис C# также предлагает синонимичную конструкцию для автоматического вызова метод Dispose - конструкцию <span class="b">using</span>:</p>
<pre class="brush:c#;">
using (Person tom = new Person("Tom"))
{
}
</pre>
<p>Конструкция using оформляет блок кода и создает объект некоторого типа, который реализует интерфейс IDisposable, в частности, его метод Dispose. При завершении блока 
кода у объекта вызывается метод Dispose.</p>
<p>Важно, что данная конструкция применяется только для типов, которые реализуют интерфейс IDisposable.</p>
...
<p>Здесь мы видим, что по завершении блока using у объекта Person вызывается метод Dispose. Вне блока кода using объект tom не существует.</p>
<p>Начиная с версии C# 8.0 мы можем задать в качестве области действия всю окружающую область видимости, например, метод:</p>
<pre class="brush:c#;">
Test();
...
<p>В данном случае using сообщает компилятору, что объявляемая переменная должна быть удалена в конце области видимости - то есть в конце метода Test. 
Соответственно мы получим следующий консольный вывод:</p>
<div class="console">
<pre class="consoletext">
Name: Tom
Конец метода Test
...
<h3>Освобождение множества ресурсов</h3>
<p>Для освобождения множества ресурсов мы можем применять вложенные конструкции using. Например:</p>
<pre class="brush:c#;">
void Test()
{
	using (Person tom = new Person("Tom"))
...
<p>В данном случае обе конструкции using создают объекты одного и того же типа, но это могут быть и разные типы данных, главное, чтобы они 
реализовали интерфейс IDisposable.</p>
<p>Мы можем сократить это определение:</p>
<pre class="brush:c#;">
void Test()
{
	using (Person tom = new Person("Tom"))
...
<p>И, как уже было выше сказано, в C# мы можем задать в качестве области действия для объектов, создаваемых в конструкции using, весь метод:</p>
<pre class="brush:c#;">
