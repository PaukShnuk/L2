...
<html >
<head>
<title>C# и .NET | Асинхронные стримы</title>
<meta charset="utf-8" />
<meta name="description" content="Асинхронные стримы в C# 8 и .NET, итераторы в асинхронных операциях, IAsyncEnumerable и IAsyncEnumerator">
...
<h2>Асинхронные стримы</h2><div class="date">Последнее обновление: 11.02.2022</div>
...
<p>Начиная с версии C# 8.0 в C# были добавлены <span class="b">асинхронные стримы</span>, которые упрощают работу со потоками данных в 
...
<ul>
<li><p>метод имеет модификатор <code>async</code></p></li>
<li><p>метод возращает объект <span class="b">IAsyncEnumerable&lt;T&gt;</span>. Интерфейс IAsyncEnumerable определяет метод GetAsyncEnumerator, 
...
<li><p>метод содержит выражения <code>yield return</code> для последовательного получения элементов из асинхронного стрима</p></li>
</ul>
...
<p>Итак, метод <code>GetNumbersAsync()</code> фактически и представляет асинхронный стрим. Этот метод является асинхронным. Его возвращаемый тип - 
<code>IAsyncEnumerable&lt;int&gt;</code>. А его суть сводится к тому, что он возвращает с помощью <code>yield return</code> каждые 100 миллисекунд 
...
<pre class="brush:c#;">await foreach (var number in GetNumbersAsync())</pre>
<p>Важно отметить, что он предваряется оператором <span class="b">await</span>. В итоге, каждый раз когда асинхронный стрим будет возвращать очередное число, цикл будет его получать и выводить на консоль.</p>
...
<div class="nav"><p><a href="./13.4.php">Назад</a><a href="./">Содержание</a><a href="./15.1.php">Вперед</a></p></div></div>
<div class="socBlock">
