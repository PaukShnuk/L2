...
<html >
<head>
<title>C# и .NET | Работа с каталогами</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с каталогами в языке C# и .NET, классы Directory и DirectoryInfo и их основные методы и свойства, получение, удаление, перемещение и создание каталогов, примеры использования">
...
<h2>Работа с каталогами</h2><div class="date">Последнее обновление: 07.01.2022</div>
...
<p>Для работы с каталогами в пространстве имен System.IO предназначены сразу два класса: <span class="b">Directory</span> и 
<span class="b">DirectoryInfo</span>.</p>
...
<ul>
<li><p><span class="b">CreateDirectory(path)</span>: создает каталог по указанному пути path</p></li>
<li><p><span class="b">Delete(path)</span>: удаляет каталог по указанному пути path</p></li>
<li><p><span class="b">Exists(path)</span>: определяет, существует ли каталог по указанному пути path. Если существует, возвращается <code>true</code>, 
если не существует, то <code>false</code></p></li>
<li><p><span class="b">GetCurrentDirectory()</span>: получает путь к текущей папке</p></li>
<li><p><span class="b">GetDirectories(path)</span>: получает список подкаталогов в каталоге path</p></li>
<li><p><span class="b">GetFiles(path)</span>: получает список файлов в каталоге path</p></li>
<li><p><span class="b">GetFileSystemEntries(path)</span>: получает список подкаталогов и файлов в каталоге path</p></li>
<li><p><span class="b">Move(sourceDirName, destDirName)</span>: перемещает каталог</p></li>
<li><p><span class="b">GetParent(path)</span>: получение родительского каталога</p></li>
<li><p><span class="b">GetLastWriteTime(path)</span>: возвращает время последнего изменения каталога</p></li>
<li><p><span class="b">GetLastAccessTime(path)</span>: возвращает время последнего обращения к каталогу</p></li>
<li><p><span class="b">GetCreationTime(path)</span>: возвращает время создания каталога</p></li>
</ul>
...
<pre class="brush:c#;">public DirectoryInfo (string path);</pre>
<p>Основные методы класса <span class="b">DirectoryInfo</span>:</p>
<ul>
<li><p><span class="b">Create()</span>: создает каталог</p></li>
<li><p><span class="b">CreateSubdirectory(path)</span>: создает подкаталог по указанному пути path</p></li>
<li><p><span class="b">Delete()</span>: удаляет каталог</p></li>
<li><p><span class="b">GetDirectories()</span>: получает список подкаталогов папки в виде массива DirectoryInfo</p></li>
<li><p><span class="b">GetFiles()</span>: получает список файлов в папке в виде массива FileInfo</p></li>
<li><p><span class="b">MoveTo(destDirName)</span>: перемещает каталог</p></li>
</ul>
<p>Основные свойства класса <span class="b">DirectoryInfo</span>:</p>
<ul>
<li><p><span class="b">CreationTime</span>: представляет время создания каталога</p></li>
<li><p><span class="b">LastAccessTime</span>: представляет время последнего доступа к каталогу</p></li>
<li><p><span class="b">LastWriteTime</span>: представляет время последнего изменения каталога</p></li>
<li><p><span class="b">Exists</span>: определяет, существует ли каталог</p></li>
<li><p><span class="b">Parent</span>: получение родительского каталога</p></li>
<li><p><span class="b">Root</span>: получение корневого каталога</p></li>
<li><p><span class="b">Name</span>: имя каталога</p></li>
<li><p><span class="b">FullName</span>: полный путь к каталогу</p></li>
</ul>
...
<p>Обратите внимание на использование слешей в именах файлов. Либо мы используем двойной слеш: <code>"C:\\"</code>, либо одинарный, 
но тогда перед всем путем ставим знак @: <code>@"C:\Program Files"</code></p>
...
плейсхолдера: <span class="b">*</span> или символ-звездочка (соответствует любому количеству символов) и <span class="b">?</span> или вопросительный знак (соответствует одному символу)</p>
...
В итоге у нас получится следующий путь: "C:\SomeDir\program\avalon"</p>
...
<p>Если мы просто применим метод <code>Delete</code> к непустой папке, в которой есть какие-нибудь файлы или подкаталоги, то приложение 
нам выбросит ошибку. Поэтому нам надо передать в метод <code>Delete</code> дополнительный параметр булевого типа, который укажет, что папку 
