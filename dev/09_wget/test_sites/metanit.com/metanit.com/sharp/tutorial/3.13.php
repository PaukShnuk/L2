...
<html >
<head>
<title>С# и .NET | Делегаты</title>
<meta charset="utf-8" />
<meta name="description" content="Делегаты в С# и .NET, их сущность и объявление с помощью ключевого слова delegate и вызов методом Invoke, добавление методов в делегат, обобщенные делегаты">
...
<h1>Делегаты, события и лямбды</h1><h2>Делегаты</h2><div class="date">Последнее обновление: 30.12.2021</div>
...
<p>Делегаты представляют такие объекты, которые указывают на методы. То есть делегаты - это указатели на методы и с 
...
<pre class="brush:c#;">delegate void Message(); // 1. Объявляем делегат</pre>
<p>Для использования делегата объявляется переменная этого делегата:</p>
<pre class="brush:c#;">Message mes; // 2. Создаем переменную делегата</pre>
...
<pre class="brush:c#;">mes = Hello; // 3. Присваиваем этой переменной адрес метода</pre>
<p>Затем через делегат вызываем метод, на который ссылается данный делегат:</p>
<pre class="brush:c#;">mes(); // 4. Вызываем метод</pre>
...
<p>Поскольку делегат принимает два параметра типа int, то при его вызове необходимо передать значения для этих параметров: <code>operation(4,5)</code>.</p>
...
Но надо учитывать, что во внимание также принимаются модификаторы <span class="b">ref</span>, <span class="b">in</span> и <span class="b">out</span>. Например, пусть у нас есть делегат:</p>
<pre class="brush:c#;">delegate void SomeDel(int a, double b);</pre>
<p>Этому делегату соответствует, например, следующий метод:</p>
<pre class="brush:c#;">void SomeMethod1(int g, double n) { }</pre>
...
все методы из этого списка последовательно вызываются. И мы можем добавлять в этот список не один, а несколько методов. Для добавления методов в делегат применяется операция <span class="b">+=</span>:</p>
<pre class="brush:c#;">
...
<p>Подобным образом мы можем удалять методы из делегата с помощью операций <span class="b">-=</span>:</p>
<pre class="brush:c#;">
Message? message = Hello; 
message += HowAreYou;
...
не просто как переменная типа <code>Message</code>, а именно <span class="b">Message?</span>, то есть типа, который может представлять как делегат Message, так и значение null.</p>
...
<p>Другой способ вызова делегата представляет метод <span class="b">Invoke()</span>:</p>
<pre class="brush:c#;">
Message mes = Hello;
mes.Invoke(); // Hello
Operation op = Add;
...
<p>В прогамме мы можем определить переменные делегата под определенный метод. Например, делегату <code>Operation&lt;decimal, int&gt;</code> 
соответствует метод, который принимает число int и возвращает число типа decimal. А делегату <code>Operation&lt;int, int&gt;</code> 
...
<p>Здесь метод <span class="b">DoOperation</span> в качестве параметров принимает два числа и некоторое действие в виде делегата Operation. В внутри метода 
...
<p>В данном случае метод <code>SelectOperation()</code> в качестве параметра принимает перечисление типа OperationType. Это перечисление хранит три константы, 
...
Add, Subtract, Multiply. То есть если параметр метода SelectOperation равен <code>OperationType.Add</code>, то возвращается метод Add, который выолняет сложение двух чисел:</p>
<pre class="brush:c#;">case OperationType.Add: return Add;</pre>
<p>При вызове метода SelectOperation мы можем получить из него нужное действие в переменную operation:</p>
<pre class="brush:c#;">Operation operation = SelectOperation(OperationType.Add);</pre>
...
<ul>
<li><a title="Вопросы для самопроверки" href="../questions/2.17.php"><i class="fa fa-lg fa-question"></i><span class="dop-txt">Вопросы для самопроверки</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./2.30.php">Назад</a><a href="./">Содержание</a><a href="./3.43.php">Вперед</a></p></div></div>
<div class="socBlock">
