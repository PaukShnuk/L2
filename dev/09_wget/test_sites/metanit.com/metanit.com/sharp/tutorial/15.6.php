...
<html >
<head>
<title>C# и .NET | Группировка в LINQ</title>
<meta charset="utf-8" />
<meta name="description" content="Группировка элементов в LINQ в языке программирования C# и .NET, оператор group by, метод GroupBy, получения ключа и элементов групп">
...
<h2>Группировка</h2><div class="date">Последнее обновление: 14.02.2022</div>
...
<p>Для группировки данных по определенным параметрам применяется оператор <span class="b">group by</span> и метод <span class="b">GroupBy()</span>. 
<h3>Оператор group by</h3>
<p>Допустим, у нас есть набор из объектов следующего типа:</p>
<pre class="brush:c#;">
record class Person(string Name, string Company);
</pre>
<p>Данный класс представляет пользователя и имеет два свойства: Name (имя пользователя) и Company (компания, где работает пользователь). 
Сгруппируем набор пользователей по компании:</p>
<pre class="brush:c#;">
Person[] people =
{
...
<p>Если в выражении LINQ последним оператором, выполняющим операции над выборкой, является <code>group</code>, то оператор <code>select</code> не применяется.</p>
<p>Оператор <code>group</code> принимает критерий по которому проводится группировка:</p>
<pre class="brush:c#;">group person by person.Company</pre>
<p>в данном случае группировка идет по свойству Company. Результатом оператора <code>group</code> является выборка, которая состоит из групп. 
Каждая группа представляет объект <span>IGrouping&lt;K, V&gt;</span>: параметр <code>K</code> указывает на тип ключа - тип свойства, по которому идет группировка (здесь это тип string). 
А параметр <code>V</code> представляет тип сгруппированных объектов - в данном случае группируем объекты Person.</p>
<p>Каждая группа имеет ключ, который мы можем получить через свойство Key: <code>g.Key</code>. Здесь это будет название компании.</p>
<p>Все элементы внутри группы можно получить с помощью дополнительной итерации. Элементы группы имеют тот же тип, что и тип объектов, которые передавались оператору <code>group</code>, 
то есть в данном случае объекты типа <code>Person</code>.</p>
<p>В итоге мы получим следующий вывод:</p>
<div class="console">
...
<h3>GroupBy</h3>
<p>В качестве альтернативы можно использовать метод расширения <span class="b">GroupBy</span>. Он имеет ряд перегрузок, возьмем самую простую из них:</p>
<pre class="brush:c#;">
GroupBy&lt;TSource,TKey&gt; (Func&lt;TSource,TKey&gt; keySelector);
</pre>
<p>Данная версия получает делегат, которые в качестве параметра принимает каждый элемент коллекции и возвращает критерий группировки.</p>
<p>Перепишем предыдущий пример с помощью метода GroupBy:</p>
<pre class="brush:c#;">
Person[] people =
{
    new Person("Tom", "Microsoft"), new Person("Sam", "Google"),
...
<h3>Создание нового объекта при группировке</h3>
<p>Теперь изменим запрос и создадим из группы новый объект:</p>
<pre class="brush:c#;">
Person[] people =
{
    new Person("Tom", "Microsoft"), new Person("Sam", "Google"),
...
<p>Выражение</p>
<pre class="brush:c#;">group person by person.Company into g</pre>
<p>определяет переменную <code>g</code>, которая будет содержать группу. С помощью этой переменной 
мы можем затем создать новый объект анонимного типа (хотя также можно под данную задачу определить новый класс):</p>
<pre class="brush:c#;">select new { Name = g.Key, Count = g.Count() }</pre>
<p>Теперь результат запроса LINQ будет представлять набор объектов таких анонимных типов, у которых два свойства Name и Count.</p>
...
<p>Аналогичная операция с помощью метода <code>GroupBy()</code>:</p>
<pre class="brush:c#;">
...
<p>Также мы можем осуществлять вложенные запросы:</p>
<pre class="brush:c#;">
Person[] people =
{
    new Person("Tom", "Microsoft"), new Person("Sam", "Google"),
...
<p>Здесь свойство Employees каждой группы формируется с помощью дополнительного запроса, который выбирает всех пользователей в этой группе. Консольный вывод программы:</p>
<div class="console">
<pre class="consoletext">
Microsoft : 3
Tom
...
<p>Аналогичный запрос с помощью метода GroupBy:</p>
<pre class="brush:c#;">
var companies = people
                    .GroupBy(p=&gt;p.Company)
                    .Select(g =&gt; new
...
<div class="nav"><p><a href="./15.11.php">Назад</a><a href="./">Содержание</a><a href="./15.7.php">Вперед</a></p></div></div>
<div class="socBlock">
