...
<html >
<head>
<title>C# и .NET | Подключение пространств имен по умолчанию</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение пространств имен по умолчанию в C# и .NET, ImplicitUsings и GlobalUsings, подключение пространства имен на уровне проекта">
...
<h2>Подключение пространств имен по умолчанию</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Все классы существуют в некотором пространстве имен, и чтобы эти классы использовать, необходимо подключить их пространства имен, либо использовать полное название класса с указанием его пространства имен. 
Однако начиная с Visual Studio 2022 и .NET 6 и C# 10 мы можем просто в файле программы написать:</p>
<pre class="brush:c#;">Console.WriteLine("Hello");</pre>
<p>При этом не подключая явно пространство имен <code>System</code>, где располагается класс <code>Console</code>, тем не менее этот класс будет доступен, и мы его сможем использовать в любом файле кода C# в проекте. 
...
<img src="./pics/3.31.png" alt="ImplicitUsings в C# в Visual Studio" />
...
<p>Здесь строка</p>
<pre class="brush:xml;">&lt;ImplicitUsings&gt;enable&lt;/ImplicitUsings&gt;</pre>
<p>точнее элемент <code>&lt;ImplicitUsings&gt;</code> задает подключение некоторого набора пространств имен по умолчанию. Значение <span class="b">enable</span> указывает, 
что эта настройка будет применяться.</p>
<p>Если мы запустим проект на выполнение, то после компиляции проект в проекте в папке <span class="b">obj/Debug/net6.0</span> 
можно будет увидеть файл <span class="b">[Имя_проекта].GlobalUsings.g.cs</span> (например, у меня проект называется HelloApp, соответственно файл будет 
называться <span class="b">HelloApp.GlobalUsings.g.cs</span>). Если мы откроем этот файл, то увидим там следующее содержимое:</p>
<pre class="brush:c#;">
...
<p>Здесь перечислены все те пространства имен, которые подключаются в проект по умолчанию:</p>
<pre class="brush:c#;">
System
...
<p>Стоит отметить, что это набор пространств имен, которые подключаются в проект консольного приложения по умолчанию, у других типов проектов набор подключаемых пространств может отличаться.</p>
<h3>Отключение пространств по умолчанию</h3>
<p>В проектах, создаваемых в Visual Studio 2022 для .NET 6 и C# 10 эта возможность включена по умолчанию. Тем не менее при необходимости мы ее можем отключить. Для этого изменим эту настройку следующим образом:</p>
<pre class="brush:xml;">&lt;ImplicitUsings&gt;disable&lt;/ImplicitUsings&gt;</pre>
<p>После этого Visual Studio 2022 подчеркнет нам класс <code>Console</code>, так как он нигде не определен в проекте, а его пространство имен не подключено:</p>
<img src="./pics/3.32.png" alt="ImplicitUsings в .NET 6 и C# 10 в Visual Studio 2022" />
<p>В этом случае нам потребуется либо подключить пространство имен <code>System</code>, где располагается класс Console, либо использовать его полное имя:</p>
<pre class="brush:c#;">
...
<h3>Подключение и отключение пространств по отдельности</h3>
<p>Вместо подключения по умолчанию некоторого набора пространств имен по умолчанию также можно подключать и отключать по отдельности пространств имен. Так, 
изменим файл проекта следующим образом:</p>
<pre class="brush:xml;">
&lt;Project Sdk="Microsoft.NET.Sdk"&gt;
...
<p>Здесь атрибут <span class="b">Include</span> элемента <span class="b">Using</span> подключает пространства имен глобально по всему проекту. Например:</p>
<pre class="brush:c#;">&lt;Using Include="System" /&gt;</pre>
<p>Подключает пространство System. Соответственно после этого нам не надо его подключать в файлы кода в проекте.</p>
<p>Обратная ситуация - отключение некоторых ненужных пространств имен из тех, которые подключаются по умолчанию. Для этого 
применяется атрибут <span class="b">Remove</span> элемента <span class="b">Using</span>:</p>
<pre class="brush:xml;">
&lt;Project Sdk="Microsoft.NET.Sdk"&gt;
...
<p>В данном случае отключаются пространства "System" и "System.Threading.Tasks"</p>
...
<div class="nav"><p><a href="./3.52.php">Назад</a><a href="./">Содержание</a><a href="./3.46.php">Вперед</a></p></div></div>
<div class="socBlock">
