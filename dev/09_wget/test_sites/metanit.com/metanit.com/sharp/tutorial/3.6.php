...
<html >
<head>
<title>C# и .NET | Статические члены и модификатор static</title>
<meta charset="utf-8" />
<meta name="description" content="Статические поля, свойства и методы, особенности их использования в C#, модификатор static, статические конструкторы">
...
<h2>Статические члены и модификатор static</h2><div class="date">Последнее обновление: 27.12.2021</div>
...
<p>Кроме обычных полей, методов, свойств классы и структуры могут иметь статические поля, методы, свойства. Статические поля, методы, свойства относятся ко всему классу/всей структуре и 
для обращения к подобным членам необязательно создавать экземпляр класса / структуры.</p>
<h3>Статические поля</h3>
<p>Статические поля хранят состояние всего класса / структуры. Статическое поле определяется как и обычное, только перед типом поля указывается ключевое слово 
<span class="b">static</span>. Например, рассмотрим класс Person, который представляет человека:</p>
<pre class="brush:c#;">
Person bob = new(68);
...
<p>Причем в самом классе мы можем использовать это поле как и любые другие. Так, в методе <code>СheckAge()</code>, который поверяет пенсионный статус человека, 
для проверки используем это поле:</p>
<pre class="brush:c#;">if (age &gt;= retirementAge)</pre>
...
<img src="./pics/static.png" alt="Статические методы класса в C#" />
...
<p>Подобным образом мы можем создавать и использовать статические свойства:</p>
<pre class="brush:c#;">
Person bob = new(68);
bob.СheckAge();
...
<p>В данном случае доступ к статической переменной retirementAge опосредуется с помощью статического свойства <code>RetirementAge</code>.</p>
<p>Таким образом, переменные и свойства, которые хранят состояние, общее для всех объектов класса / структуры, следует определять как статические.</p>

<p>Нередко статические поля и свойства применяются для хранения счетчиков. Например, мы хотим иметь счетчик, который позволял бы узнать, 
сколько объектов Person создано:</p>
<pre class="brush:c#;">
var tom = new Person();
var bob = new Person();
var sam = new Person();
...
<p>В данном случае в классе Person счетчик хранится в приватной переменной counter, значение которой увеличивается на единицу при создании объекта в конструкторе. 
А с помощью статического свойства Counter, которое доступно только для чтения, мы можем получить значение счетчика.</p>
<h3>Статические методы</h3>
<p>Статические методы определяют общее для всех объектов поведение, которое не зависит от конкретного объекта. Для обращения к статическим методам также применяется имя класса / структуры:</p>
<pre class="brush:c#;">
Person bob = new(68);
...
<p>В данном случае в классе Person определен статический метод <code>CheckRetirementStatus()</code>, который в качестве параметра принимает объект Person и проверяет его пенсионный статус.</p>
<p>Следует учитывать, что статические методы могут обращаться только к статическим членам класса. Обращаться к нестатическим методам, полям, свойствам внутри статического метода мы не можем.</p>

<h3>Статический конструктор</h3>
<p>Кроме обычных конструкторов у класса также могут быть статические конструкторы. Статические конструкторы имеют следующие отличительные черты:</p>
<ul>
<li><p>Статические конструкторы не должны иметь модификатор доступа и не принимают параметров</p></li>
<li><p>Как и в статических методах, в статических конструкторах нельзя использовать ключевое слово this для ссылки на текущий объект класса и 
можно обращаться только к статическим членам класса</p></li>
<li><p>Статические конструкторы нельзя вызвать в программе вручную. Они выполняются автоматически при самом первом создании объекта данного 
класса или при первом обращении к его статическим членам (если таковые имеются)</p></li>
</ul>
<p>Статические конструкторы обычно используются для инициализации статических данных, либо же выполняют действия, которые требуется выполнить только один раз</p>
<p>Определим статический конструктор:</p>
<pre class="brush:c#;">
Console.WriteLine(Person.RetirementAge);
...
<p>В данном случае с помощью встроенной структуры <span class="b">DateTime</span> получаем текущий год. Для этого используется свойство <code>DateTime.Now.Year</code>. 
если он равен 2022, устанавливаем один пенсионный возраст. При другом значении текущего года устанавливается другое значение пенсионного возраста.</p>
<h3>Статические классы</h3>
<p>Статические классы объявляются с модификатором static и могут содержать только статические поля, свойства и методы. Например, 
определим класс, который выполняет ряд арифметических операций:</p>
<pre class="brush:c#;">
...
<ul>
<li><a title="Вопросы для самопроверки" href="../questions/2.10.php"><i class="fa fa-lg fa-question"></i><span class="dop-txt">Вопросы для самопроверки</span></a></li>
</ul>
</fieldset></div>
