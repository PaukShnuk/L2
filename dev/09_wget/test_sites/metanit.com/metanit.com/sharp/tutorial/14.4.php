...
<html >
<head>
<title>C# и .NET | Атрибуты</title>
<meta charset="utf-8" />
<meta name="description" content="Атрибуты в языке программирования C# и .NET и класс System.Attribute, создание своих атрибутов и применение их к классам, ограничение атрибутов и AttributeUsage">
...
<h2>Атрибуты в .NET</h2><div class="date">Последнее обновление: 17.02.2022</div>
...
<p>Атрибуты в .NET представляют специальные инструменты, которые позволяют встраивать в сборку дополнительные метаданные. Атрибуты могут применяться 
как ко всему типу (классу, интерфейсу и т.д.), так и к отдельным его частям (методу, свойству и т.д.). Основу атрибутов составляет класс <code>System.Attribute</code>, 
...
классы атрибутов, которые будут определять метаданные других типов.</p>
<p>Допустим, нам надо проверять пользователя на соответствие некоторым возрастным ограничениям. Создадим свой атрибут, который будет хранить пороговое значение возраста, с которого разрешены некоторые 
действия:</p>
<pre class="brush:c#;">
...
<p>По сути это обычный класс, унаследованный от System.Atribute. В нем определено два конструктора: с параметром и без. В качестве параметра второй конструктор атрибута принимает некий пороговый возраст и сохраняет его в свойстве.</p>
<p>Теперь применим его к некоторому классу:</p>
<pre class="brush:c#;">
[AgeValidation(18)]
public class Person
{
...
<p>Данный класс Person применяет атрибут. Для этого имя атрибута указывается в квадратных скобках непосредственно перед определением класса. 
Причем суффикс <span class="ii">Attribute</span> указывать необязательно. 
Обе записи <code>[AgeValidation(18)]</code> и <code>[AgeValidationAttribute(18)]</code> будут равноправны.</p>
<p>Если конструктор атрибута предусматривает использование параметров (<code>public AgeValidationAttribute(int age)</code>), то после имени атрибута 
мы можем указать значения для параметров конструктора. В данном случае передается значение для параметра <code>age</code>.  То есть фактически мы говорим, что в 
AgeValidationAttribute свойство Age будет иметь значение 18.</p>
<p>В качестве альтернативы можно использовать именованные параметры для всех свойств атрибута, если класс атрибута имеет конструктор без параметров: 
<code>[AgeValidation(Age = 18)]</code></p>
<p>Теперь получим атрибут класса Person и используем его для проверки объектов данного класса:</p>
<pre class="brush:c#;">
...
<p>В данном случае в методе ValidateUser через параметр получаем некоторый объект Person и с помощью метода GetCustomAttributes вытаскиваем из типа Person 
...
пользователя. Если пользователь прошел проверку по возрасту, то возвращаем true, иначе возвращаем false. Если атрибут не применяется, возвращаем true.</p>
<h3>Ограничение применения атрибута</h3>
<p>С помощью атрибута <code>AttributeUsage</code> можно ограничить типы, к которым будет применяться атрибут. Например, мы хотим, чтобы выше определенный атрибут 
мог применяться только к классам:</p>
<pre class="brush:c#;">
...
<p>Ограничение задает перечисление <span class="b">AttributeTargets</span>, которое может принимать еще ряд значений:</p>
<ul>
<li><p><code>All</code>: используется всеми типами</p></li>
<li><p><code>Assembly</code>: атрибут применяется к сборке</p></li>
<li><p><code>Constructor</code>: атрибут применяется к конструктору</p></li>
<li><p><code>Delegate</code>: атрибут применяется к делегату</p></li>
<li><p><code>Enum</code>: применяется к перечислению</p></li>
<li><p><code>Event</code>: атрибут применяется к событию</p></li>
<li><p><code>Field</code>: применяется к полю типа</p></li>
<li><p><code>Interface</code>: атрибут применяется к интерфейсу</p></li>
<li><p><code>Method</code>: применяется к методу</p></li>
<li><p><code>Property</code>: применяется к свойству</p></li>
<li><p><code>Struct</code>: применяется к структуре</p></li>
</ul>
<p>С помощью логической операции ИЛИ можно комбинировать эти значения. Например, пусть атрибут может применяться к классам и структурам: 
<code>[AttributeUsage(AttributeTargets.Class | AttributeTargets.Struct)]</code></p>
...
<div class="nav"><p><a href="./14.3.php">Назад</a><a href="./">Содержание</a><a href="./9.1.php">Вперед</a></p></div></div>
<div class="socBlock">
