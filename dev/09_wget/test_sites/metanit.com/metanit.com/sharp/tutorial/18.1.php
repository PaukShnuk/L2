...
<html >
<head>
<title>C# и .NET | Процессы</title>
<meta charset="utf-8" />
<meta name="description" content="Процессы и домены приложения в C# и .NET, функциональность System.Diagnostics, получение списка процессов в системе, имя и id процесса, выделенная память процесса, класс Process, ProcessModule  и ProcessThread, запуск приложения с помощью метода Process.Start">
...
<h1>Процессы и домены приложения</h1><h2>Процессы</h2><div class="date">Последнее обновление: 22.02.2022</div>
...
<p>При запуске приложения операционная система создает для него отдельный процесс, которому выделяется определённое адресное пространство в памяти и который 
...
C# точкой входа в программу является метод <code>Main</code>. Вызов этого метода автоматически создает главный поток. А из главного потока могут запускаться 
вторичные потоки.</p>
<p>В .NET процесс представлен классом <span class="b">Process</span> из пространства имен <span class="b">System.Diagnostics</span>. Этот класс позволяет управлять 
уже запущенными процессами, а также запускать новые. В данном классе определено ряд свойств и методов, позволяющих получать информацию о процессах и управлять ими:</p>
<ul>
<li><p>Свойство <span class="b">Handle</span>: возвращает дескриптор процесса</p></li>
<li><p>Свойство <span class="b">Id</span>: получает уникальный идентификатор процесса в рамках текущего сеанса ОС</p></li>
<li><p>Свойство <span class="b">MachineName</span>: возвращает имя компьютера, на котором запущен процесс</p></li>
<li><p>Свойство <span class="b">MainModule</span>: представляет основной модуль - исполняемый файл программы, представлен объектом типа <span class="b">ProcessModule<span></p></li>
<li><p>Свойство <span class="b">Modules</span>: получает доступ к коллекции ProcessModuleCollection, которая в виде объектов ProcessModule хранит набор модулей (например, файлов dll и exe), 
загруженных в рамках данного процесса</p></li>
<li><p>Свойство <span class="b">ProcessName</span>: возвращает имя процесса, которое нередко совпадает с именем приложения</p></li>
<li><p>Свойство <span class="b">StartTime</span>: возвращает время, когда процесс был запущен</p></li>
<li><p>Свойство <span class="b">PageMemorySize64</span>: возвращает объем памяти, который выделен для данного процесса</p></li>
<li><p>Свойство <span class="b">VirtualMemorySize64</span>: возвращает объем виртуальной памяти, который выделен для данного процесса</p></li>
<li><p>Метод <span class="b">CloseMainWindow()</span>: закрывает окно процесса, который имеет графический интерфейс</p></li>
<li><p>Метод <span class="b">GetProcesses()</span>: возвращающий массив всех запущенных процессов</p></li>
<li><p>Метод <span class="b">GetProcesses()</span>: возвращающий массив всех запущенных процессов</p></li>
<li><p>Метод <span class="b">GetProcessesByName()</span>: возвращает процессы по его имени. Так как можно запустить несколько копий одного приложения, 
то возвращает массив</p></li>
<li><p>Метод <span class="b">GetProcessById()</span>: возвращает процесс по Id. Так как можно запустить несколько копий одного приложения, 
то возвращает массив</p></li>
<li><p>Метод <span class="b">Kill()</span>: останавливает процесс</p></li>
<li><p>Метод <span class="b">Start()</span>: запускает новый процесс</p></li>
</ul>
<p>Например, исследуем текущий процесс:</p>
<pre class="brush:c#;">
...
<p>Получим все запущенные процессы:</p>
<pre class="brush:c#;">
using System.Diagnostics;
...
<p>Получим id процессов, который представляют запущенные экземпляры Visual Studio:</p>
<pre class="brush:c#;">
using System.Diagnostics;
...
<h3>Потоки процесса</h3>
<p>Свойство <span class="b">Threads</span> представляет коллекцию потоков процесса - объект <code>ProcessThreadCollection</code>, каждый поток в которой 
является объектом <span class="b">ProcessThread</span>. В данном классе можно выделить следующие свойства:</p>
<ul>
<li><p><span class="b">CurrentPriority</span>: возвращает текущий приоритет потока</p></li>
<li><p><span class="b">Id</span>: идентификатор потока</p></li>
<li><p><span class="b">IdealProcessor</span>: позволяет установить процессор для обработки потока</p></li>
<li><p><span class="b">PriorityLevel</span>: уровень приоритета потока</p></li>
<li><p><span class="b">StartAddress</span>: адрес в памяти функции, запустившей поток</p></li>
<li><p><span class="b">StartTime</span>: время запуска потока (поддерживается только на Windows и Linux)</p></li>
</ul>
<p>Например, получим все потоки процесса Visual Studio:</p>
<pre class="brush:c#;">
using System.Diagnostics;
...
<h3>Модули процесса</h3>
<p>Одно приложение может использовать набор различных сторонних библиотек и модулей. Для их получения класс Prosess имеет свойство <code>Modules</code>, 
которое представляет объект <code>ProcessModuleCollection</code>. Каждый отдельный модуль представлен классом <span class="b">ProcessModule</span>, 
у которого можно выделить следующие свойства:</p>
<ul>
<li><p><span class="b">BaseAddress</span>: адрес модуля в памяти</p></li>
<li><p><span class="b">FileName</span>: полный путь к файлу модуля</p></li>
<li><p><span class="b">EntryPointAddress</span>: адрес функции в памяти, которая запустила модуль</p></li>
<li><p><span class="b">ModuleName</span>: название модуля (краткое имя файла)</p></li>
<li><p><span class="b">ModuleMemorySize</span>: возвращает объем памяти, необходимый для загрузки модуля</p></li>
</ul>
<p>Получим все модули, используемые Visual Studio:</p>
<pre class="brush:c#;">
using System.Diagnostics;
...
<h3>Запуск нового процесса</h3>
<p>С помощью статического метода <span class="b">Process.Start()</span> можно запустить новый процесс. Например:</p>
<pre class="brush:c#;">
...
<p>В данном случае запускается браузер Google Chrome</p>
<p>При обращении к исполняемому файлу .NET запускает приложение.</p>
<p>Однако при запуске некоторых программ может потребоваться передать им различные параметры. В этом случае можно использовать перегруженную версию 
метода, передавая в качестве второго параметра параметры:</p>
<pre class="brush:c#;">
Process.Start(@"C:\Program Files\Google\Chrome\Application\chrome", "https://metanit.com");
</pre>
<p>Чтобы отделить настройку параметров запуска от самого запуска можно использовать класс <span class="b">ProcessStartInfo</span>:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./6.4.php">Назад</a><a href="./">Содержание</a><a href="./18.2.php">Вперед</a></p></div></div>
<div class="socBlock">
