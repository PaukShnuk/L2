...
<html >
<head>
<title>C# и .NET | Конструкция if..else и тернарная операция</title>
<meta charset="utf-8" />
<meta name="description" content="Условные конструкции в языке C#, выражение if..else, выражения else if, тернарный оператор">
...
<h2>Конструкция if..else и тернарная операция</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Условные конструкции - один из базовых компонентов многих языков программирования, которые направляют работу программы по одному из путей 
в зависимости от определенных условий. Одной из таких конструкций в языке программирования C# является конструкция <code>if..else</code></p>
<p>Конструкция if/else проверяет истинность некоторого условия и в зависимости от результатов проверки выполняет определенный код.</p>
<p>Ее простейшая форма состоит из блока <span class="b">if</span>:</p>
<pre class="brush:c#;">
if(условие)
{
...
<p>После ключевого слова <code>if</code> ставится условие. Условие должно представлять значение типа <span class="b">bool</span>. 
Это может быть непосредственно значение типа bool или результат условного выражения или другого выражения, которое возвращает значение тиа bool. 
И если это условие истинно (равно <code>true</code>), то срабатывает код, который помещен далее после условия внутри фигурных скобок.</p>
<p>Например:</p>
<pre class="brush:c#;">
int num1 = 8;
int num2 = 6;
...
<p>В данном случае у нас первое число больше второго, поэтому выражение <code>num1 &gt; num2</code> истинно и возвращает 
<code>true</code>, следовательно, управление переходит к строке <code>Console.WriteLine("Число {num1} больше числа {num2}");</code></p>
<p>Если блок if содержит одну инструкцию, то мы можем его сократить, убрав фигурные скобки:</p>
<pre class="brush:c#;">
int num1 = 8;
int num2 = 6;
if (num1 &gt; num2)
...
<p>В данном случае блок if будет выполняться, если <code>num1 &gt; num2</code> равно <code>true</code> и <code>num1==8</code> равно <code>true</code>.</p>
...
<p>Блок <code>else</code> выполняется, если условие после if ложно, то есть равно <code>false</code>. Если блок else содержит толко одну 
инструкцию, то оять же мы можем его сократить, убрав фигурные скобки:</p>
<pre class="brush:c#;">
int num1 = 8;
int num2 = 6;
if(num1 &gt; num2)
...
<h4>else if</h4>
<p>Но в примере выше при сравнении чисел мы можем насчитать три состояния: первое число больше второго, первое число меньше второго и числа равны. 
Используя конструкцию <code>else if</code>, мы можем обрабатывать дополнительные условия:</p>
<pre class="brush:c#;">
int num1 = 8;
int num2 = 6;
if(num1 &gt; num2)
...
<p>При необходимости можно добавить несколько выражений <code>else if</code>:</p>
<pre class="brush:c#;">
string name = "Alex";
...
<p>Тернарную операция также позволяет проверить некоторое условие и в зависимости от его истинности выполнить некоторые действия. Она имеет следующий синтаксис:</p>
<pre class="brush:c#;">[первый операнд - условие] ? [второй операнд] : [третий операнд]</pre>
<p>Здесь сразу три операнда. В зависимости от условия тернарная операция возвращает второй или третий операнд: если условие равно <code>true</code>, то 
...
<p>Здесь первый операнд (то есть условие) представляет выражение <code>x &lt; y</code>. Если оно равно <code>true</code>, то возвращается 
второй операнд - <code>(x+y)</code>, то есть результат операции сложения. Если условие равно <code>false</code>, то возвращается 
третий операнд - <code>(x-y)</code>.</p>
<p>Результат тернарной операции (то есть второй или третий операнд в зависимости от условия) присваивается переменной z.</p>
...
<div class="nav"><p><a href="./2.24.php">Назад</a><a href="./">Содержание</a><a href="./2.6.php">Вперед</a></p></div></div>
<div class="socBlock">
