...
<html >
<head>
<title>C# и .NET | XPath</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие с xml файлами в языке программирования C# и .NET с помощью язык запросов XPath, методы SelectSingleNode и SelectNodes класса XmlElement">
...
<h2>XPath</h2><div class="date">Последнее обновление: 20.02.2022</div>
...
<p><span class="b">XPath</span> представляет язык запросов в XML. Он позволяет выбирать элементы, соответствующие определенному селектору.</p>
<p>Рассмотрим некоторые наиболее распространенные селекторы:</p>
<dl>
  <dt><p><code>.</code></p></dt>
   <dd><p>выбор текущего узла</p></dd>
   <dt><p><code>..</code></p></dt>
   <dd><p>выбор родительского узла</p></dd>
    <dt><p><code>*</code></p></dt>
   <dd><p>выбор всех дочерних узлов текущего узла</p></dd>
    <dt><p><code>person</code></p></dt>
   <dd><p>выбор всех узлов с определенным именем, в данном случае с именем "person"</p></dd>
    <dt><p><code>@name</code></p></dt>
   <dd><p>выбор атрибута текущего узла, после знака @ указывается название атрибута (в данном случае "name")</p></dd>
    <dt><p><code>@+</code></p></dt>
   <dd><p>выбор всех атрибутов текущего узла</p></dd>
    <dt><p><code>element[3]</code></p></dt>
   <dd><p>выбор определенного дочернего узла по индексу, в данном случае третьего узла</p></dd>
    <dt><p><code>//person</code></p></dt>
   <dd><p>выбор в документе всех узлов с именем "person"</p></dd>
   <dt><p><code>person[@name='Tom']</code></p></dt>
   <dd><p>выбор элементов с определенным значением атрибута. В данном случае выбираются все элементы "person" с атрибутом name='Tom'</p></dd>
   <dt><p><code>person[company='Microsoft']</code></p></dt>
   <dd><p>выбор элементов с определенным значением вложенного элемента. В данном случае выбираются все элементы "person", у которых дочерний элемент 
"company" имеет значение 'Microsoft'</p></dd>
<dt><p><code>//person/company</code></p></dt>
   <dd><p>выбор в документе всех узлов с именем "company", которые находятся в элементах "person"</p></dd>
</dl>
<p>Действие запросов XPath основано на применении двух методов класса <span class="b">XmlElement</span>:</p>
<ul>
<li><p><span class="b">SelectSingleNode()</span>: выбор единственного узла из выборки. Если выборка по запросу содержит несколько узлов, то выбирается первый</p></li>
<li><p><span class="b">SelectNodes()</span>: выборка по запросу коллекции узлов в виде объекта <code>XmlNodeList</code></p></li>
</ul>
<p>Для запросов возьмем xml-документ из прошлых тем:</p>
<pre class="brush:xml;">
...
<p>Теперь выберем все узлы корневого элемента, то есть все элементы person:</p>
<pre class="brush:c#;">
using System.Xml;
...
<p>Выберем все узлы <code>&lt;person&gt;</code>:</p>
<pre class="brush:c#;">
XmlNodeList? personNodes = xRoot?.SelectNodes("person");
</pre>
<p>Выведем на консоль значения атрибутов name у элементов person:</p>
<pre class="brush:c#;">
using System.Xml;
...
<p>Результатом выполнения будет следующий вывод:</p>
<pre>
Tom
Bob
</pre>
<p>Выберем узел, у которого атрибут name имеет значение "Tom":</p>
<pre class="brush:c#;">
XmlNode childnode = xRoot.SelectSingleNode("person[@name='Tom']");
if (childnode != null)
    Console.WriteLine(childnode.OuterXml);
</pre>
<p>Выберем узел, у которого вложенный элемент "company" имеет значение "Microsoft":</p>
<pre class="brush:c#;">
XmlNode? tomNode = xRoot?.SelectSingleNode("person[@name='Tom']");
Console.WriteLine(tomNode?.OuterXml);
</pre>
<p>Допустим, нам надо получить только компании. Для этого надо осуществить выборку вниз по иерархии элементов:</p>
<pre class="brush:c#;">
using System.Xml;
