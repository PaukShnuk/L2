...
<html >
<head>
<title>C# и .NET | Преобразование типов</title>
<meta charset="utf-8" />
<meta name="description" content="Преобразование типов, от базового к производному и от производного к базовому, ключевые слова is и as, upcating и downcasting">
...
<h2>Преобразование типов</h2><div class="date">Последнее обновление: 28.12.2021</div>
...
<p>В предыдущей главе мы говорили о преобразованиях объектов простых типов. Сейчас затронем тему преобразования объектов классов. Допустим, у нас есть 
...
<img src="./pics/3.12.png" alt="Иерархия наследования в языке C#" />
...
<img src="./pics/3.15.png" alt="Преобразование типов в C#" />
...
<p>Во-первых, можно использовать ключевое слово <span class="b">as</span>. С помощью него программа пытается преобразовать выражение к определенному типу, при этом не 
...
<p>Стоит отметить, что переменная <code>employee</code> здесь определяется не просто как переменная Employee, а именно <span class="b">Employee?</span> - после названия типа ставится вопросительный знак. Что указывает, 
что переменная может хранить как значение null, так и значение Employee.</p>
<p>Второй способ заключается в проверке допустимости преобразования с помощью ключевого слова <span class="b">is</span>:</p>
<pre class="brush:c#;">значение is тип</pre>
<p>Если значение слева от оператора представляет тип, указаный справа от оператора, то оператор is возвращает <code>true</code>, иначе возвращается <code>false</code>.</p>
<p>Причем оператор <span class="b">is</span> позволяет автоматически преобразовать значение к типу, если это значение представляет данный тип. Например:</p>
<pre class="brush:c#;">
Person person = new Person("Tom");
...
<p>Выражение <code>if (person is Employee employee)</code> проверяет, является ли переменная person объектом типа Employee. И если person является объектом 
...
<p>Однако, если person не является объектом Employee, как в данном случае, то такая проверка вернет значение <code>false</code>, и преобразование 
не сработает.</p>
<p>Оператор <span class="b">is</span> также можно применять и без преобразования, просто проверяя на соответствие типу:</p>
<pre class="brush:c#;">
Person person = new Person("Tom");
...
<ul>
<li><a title="Вопросы для самопроверки" href="../questions/2.5.php"><i class="fa fa-lg fa-question"></i><span class="dop-txt">Вопросы для самопроверки</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./3.7.php">Назад</a><a href="./">Содержание</a><a href="./3.19.php">Вперед</a></p></div></div>
<div class="socBlock">
