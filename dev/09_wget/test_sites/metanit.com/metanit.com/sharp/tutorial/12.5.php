...
<html >
<head>
<title>C# и .NET | Отмена задач. CancellationToken</title>
<meta charset="utf-8" />
<meta name="description" content="Отмена выполнения задач Task и параллельных операций в языке программирования C#, класс CancellationToken и CancellationTokenSource и его использование, свойство IsCancellationRequested и метод ThrowIfCancellationRequested ">
...
<h2>Отмена задач и параллельных операций. CancellationToken</h2><div class="date">Последнее обновление: 09.02.2022</div>
...
<p>Параллельное выполнение задач может занимать много времени. И иногда может возникнуть необходимость прервать выполняемую задачу. Для этого 
платформа .NET предоставляет структуру <span class="b">CancellationToken</span> из пространства имен <code>System.Threading</code>.</p>
<p>Общий алгоритм отмены задачи обычно предусматривает следующий порядок действий:</p>
<ol><li><p>Создание объекта <span class="b">CancellationTokenSource</span>, который управляет и посылает уведомление об отмене токену.</p></li>
<li><p>С помощью свойства <span class="b">CancellationTokenSource.Token</span> получаем собственно токен - объект структуры <span class="b">CancellationToken</span> 
и передаем его в задачу, которая может быть отменена.</p>
<pre class="brush:c#;">
CancellationTokenSource cancelTokenSource = new CancellationTokenSource(); 
CancellationToken token = cancelTokenSource.Token;
</pre>
<p>Для передачи токена в задачу можно применять один из конструкторов класса Task:</p>
<pre class="brush:c#;">
CancellationTokenSource cancelTokenSource = new CancellationTokenSource(); 
CancellationToken token = cancelTokenSource.Token;
...
<li><p>Определяем в задаче действия на случай ее отмены.</p></li>
<li><p>Вызываем метод <span class="b">CancellationTokenSource.Cancel()</span>, который устанавливает для свойства <span class="b">CancellationToken.IsCancellationRequested</span> значение true. 
Стоит понимать, что сам по себе метод <span class="b">CancellationTokenSource.Cancel()</span> не отменяет задачу, он лишь посылает уведомление об отмене через установку свойства 
<code>CancellationToken.IsCancellationRequested</code>. Каким образом будет происходить выход из задачи, это решает сам разработчик.</p></li>
<li><p>Класс CancellationTokenSource реализует интерфейс IDisposable. 
...
<p>Теперь касательно третьего пункта - определения действий отмены задачи. Как именно завершить задачу? Конкретные действия на лежат целиком на разработчике, 
тем не менее есть два общих варианта выхода:</p>
<ul>
<li><p>При получении сигнала отмены выйти из метода задачи, например, с помощью оператора return или построив логику метода соответствующим образом. Но следует учитывать, 
что в этом случае задача перейдет в состояние <code>TaskStatus.RanToCompletion</code>, а не в состояние <code>TaskStatus.Canceled</code>.</p>
<li><p>При получении сигнала отмены сгенерировать исключение <span class="b">OperationCanceledException</span>, вызвав у токена метод 
<span class="b">ThrowIfCancellationRequested()</span>. После этого задача перейдет в состояние <code>TaskStatus.Canceled</code>.</p></li>
</ul>
<h3>Мягкий выход из задачи без исключения OperationCanceledException</h3>
<p>Сначала рассмотрим первый - "мягкий" вариант завершения:</p>
<pre class="brush:c#;">
CancellationTokenSource cancelTokenSource = new CancellationTokenSource();
CancellationToken token = cancelTokenSource.Token;
...
<p>В данном случае задача task вычисляет и выводит на консоль квадраты чисел от 1 до 9. Для отмены задачи нам надо создать и использовать токен. Вначале создается объект <code>CancellationTokenSource</code>:</p>
<pre class="brush:c#;">
CancellationTokenSource cancelTokenSource = new CancellationTokenSource();
</pre>
<p>Затем из него получаем сам токен:</p>
<pre class="brush:c#;">
CancellationToken token = cancelTokenSource.Token;
</pre>
<p>Чтобы отменить операцию, необходимо вызвать метод <code>Cancel()</code> у объекта CancellationTokenSource:</p>
<pre class="brush:c#;">
cancelTokenSource.Cancel();
</pre>
<p>В данном случае отмена задачи вызывается через секунду, чтобы задача произвела некоторые действия.</p>
<p>В самом методе задачи в цикле мы можем отловить сигнал отмены с помощью проверки свойства <code>token.IsCancellationRequested</code>:</p>
<pre class="brush:c#;">
...
<p>Если был вызван метод <code>cancelTokenSource.Cancel()</code>, то выражение <code>token.IsCancellationRequested</code> возвращает true.</p>
<p>После завершения задачи проверяем ее статус:</p>
<pre class="brush:c#;">Console.WriteLine($"Task Status: {task.Status}");</pre>
<p>Поскольку задача успешно завершена, у задачи должен быть статус <code>RanToCompletion</code></p>
<p>И в конце у объекта CancellationTokenSource вызываем метод Dispose:</p>
<pre class="brush:c#;">cancelTokenSource.Dispose();</pre>
...
<h3>Отмена задачи с помощью генерации исключения</h3>
<p>Второй способ завершения задачи представляет генерация исключения <span class="b">OperationCanceledException</span>. Для этого применяется метод 
<span class="b">ThrowIfCancellationRequested()</span> объекта CancellationToken:</p>
<pre class="brush:c#;">
CancellationTokenSource cancelTokenSource = new CancellationTokenSource();
CancellationToken token = cancelTokenSource.Token;
...
<p>Здесь опять же проверяем значение свойства IsCancellationRequested, и если оно равно true, генерируем исключение:</p>
<pre class="brush:c#;">
if (token.IsCancellationRequested)
    token.ThrowIfCancellationRequested(); // генерируем исключение
</pre>
<p>Чтобы обработать исключение, помещаем весь код работы с задачей в конструкцию try..catch и также с помощью вызова <code>cancelTokenSource.Cancel()</code> посылаем 
сообщение об отмене задачи.</p>
<p>Стоит отметить, что генерируемое исключение будет спрятано в объекте AggregateException, который по сути представляет набор исключений. Если причина исключения 
состояла в отмене задачи, то мы можем найти в этом наборе исключений исключение типа <span class="b">TaskCanceledException</span></p>
<pre class="brush:c#;">
catch (AggregateException ae)
{
    foreach (Exception e in ae.InnerExceptions)
...
<p>Класс TaskCanceledException является производным от OperationCanceledException. Исключение типа TaskCanceledException возникает, если для задачи устанавливается статус 
...
<p>Стоит отметить, что исключение возникает только тогда, когда мы останавливаем текущий поток и ожидаем завершения задачи с помощью методов  Wait или WaitAll. Если эти методы не используются 
для ожидания задачи, то для нее просто устанавливается состояние Canceled. Например, в следующем случае исключение не возникнет:</p>
<pre class="brush:c#;">
CancellationTokenSource cancelTokenSource = new CancellationTokenSource();
CancellationToken token = cancelTokenSource.Token;
...
<h3>Регистрация обработчика отмены задачи</h3>
<p>Выше для проверки сигнала отмены применялось свойство IsCancellationRequested. Но есть и другой способ узнать о том, что был послан сигнал отмены задачи. Метод 
<span class="b">Register()</span> позволяет зарегистрировать обработчик отмены задачи в виде делегата Action:</p>
<pre class="brush:c#;">
CancellationTokenSource cancelTokenSource = new CancellationTokenSource();
CancellationToken token = cancelTokenSource.Token;
...
<p>Здесь обработчик отмены представлен лямбда-выражением:</p>
<pre class="brush:c#;">
token.Register(() =&gt; 
{ 
    Console.WriteLine("Операция прервана"); 
...
<p>Поскольку действие задачи представляет цикл, который выполняется при значении i меньше 10, то установка этой переменной в обработчике отмены приведет к выходу из цикла и 
соответственно завершению задачи.</p>
<h3>Передача токена во внешний метод</h3>
<p>Если операция, которая выполняется в задаче, представляет внешний метод, то ему можно передавать в качестве одного из параметров:</p>
<pre class="brush:c#;">
CancellationTokenSource cancelTokenSource = new CancellationTokenSource();
CancellationToken token = cancelTokenSource.Token;
...
<h3>Отмена параллельных операций Parallel</h3>
<p>Для отмены выполнения параллельных операций, запущенных с помощью методов <span class="b">Parallel.For()</span> и <span class="b">Parallel.ForEach()</span>, 
можно использовать перегруженные версии данных методов, которые принимают в качестве параметра объект <span class="b">ParallelOptions</span>. 
Данный объект позволяет установить токен:</p>
<pre class="brush:c#;">
CancellationTokenSource cancelTokenSource = new CancellationTokenSource();
CancellationToken token = cancelTokenSource.Token;
...
<p>В параллельной запущеной задаче через 400 миллисекунд происходит вызов <code>cancelTokenSource.Cancel()</code>, в 
результате программа выбрасывает исключение OperationCanceledException, и выполнение параллельных операций прекращается.</p>
...
<div class="nav"><p><a href="./12.4.php">Назад</a><a href="./">Содержание</a><a href="./13.3.php">Вперед</a></p></div></div>
<div class="socBlock">
