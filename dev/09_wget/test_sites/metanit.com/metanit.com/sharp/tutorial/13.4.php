...
<html >
<head>
<title>C# и .NET | Обработка ошибок в асинхронных методах</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка ошибок в асинхронных методах в программе на языке программирования C# и .NET, использование блока try..catch вместе с async и await.">
...
<h2>Обработка ошибок в асинхронных методах</h2><div class="date">Последнее обновление: 11.02.2022</div>
...
<p>Обработка ошибок в асинхронных методах, использующих ключевые слова <code>async</code> и <code>await</code>, имеет свои особенности.</p>
<p>Для обработки ошибок выражение <span class="b">await</span> помещается в блок try:</p>
<pre class="brush:c#;">
try
{
...
вызова <code>await PrintAsync("Hi")</code> будет сгенерировано исключение, 
...
<span class="b">IsFaulted</span> имеет значение <code>true</code>. Кроме того, свойство <code>Exception</code> объекта Task 
...
<p>И если мы передадим в метод строку с длиной меньше 3 символов, то <code>task.IsFaulted</code> будет равно true.</p>
<h3>Обработка нескольких исключений. WhenAll</h3>
<p>Если мы ожидаем выполнения сразу нескольких задач, например, с помощью <code>Task.WhenAll</code>, то мы можем получить сразу несколько исключений одномоментно 
для каждой выполняемой задачи. В этом случае мы можем получить все исключения из свойства <code>Exception.InnerExceptions</code>:</p>
<pre class="brush:c#;">
...
<p>Хотя блок <code>catch</code> через переменную <code>Exception ex</code> будет получать одно перехваченное исключение, но с помощью 
коллекции <code>Exception.InnerExceptions</code> мы сможем получить информацию обо всех возникших исключениях.</p>
...
<div class="nav"><p><a href="./13.5.php">Назад</a><a href="./">Содержание</a><a href="./13.8.php">Вперед</a></p></div></div>
<div class="socBlock">
