...
<html >
<head>
<title>C# и .NET | Указатели</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и использование указателей в языке C# и .NET, неуправляемый код, операции с указателями, операция разыменовывания и получение адреса переменной">
...
<h2>Указатели</h2><div class="date">Последнее обновление: 19.02.2022</div>
...
<p>Если вы программировали на С/С++, то, возможно, вы знакомы с таким понятием как <span class="b">указатели</span>. Указатели позволяют 
получить доступ к определенной ячейке памяти и произвести определенные манипуляции со значением, хранящимся в этой ячейке.</p>
<p>В языке C# указатели редко используются, однако в некоторых случаях можно прибегать к ним для оптимизации приложений. 
...
И с точки зрения CLR такой код не безопасен, так как среда не может проверить данный код, поэтому повышается вероятность различного рода ошибок.</p>
<p>Чтобы использовать небезопасный код в C#, надо первым делом указать проекту, что он будет работать с небезопасным кодом. Для этого надо установить 
...
<span class="b">Unsafe code</span> (Небезопасный код):</p>
<img src="./pics/8.1.png" alt="установка флага unsafe в c#" />
<p>Теперь мы можем приступать к работе с небезопасным кодом и указателями.</p>
<h3>Ключевое слово unsafe</h3>
<p>Блок кода или метод, в котором используются указатели, помечается ключевым словом <span class="b">unsafe</span>:</p>
<pre class="brush:c#;">
...
<p>Метод, использующий указатели:</p>
<pre class="brush:c#;">
unsafe void Test()
{
...
<p>Также с помощью unsafe можно объявлять структуры и классы:</p>
<pre class="brush:c#;">
unsafe struct State
{ 
...
<h3>Операции * и &amp;</h3>
<p>Ключевой при работе с указателями является операция <code>*</code>, которую еще называют операцией разыменовывания. Операция разыменовывания 
позволяет получить или установить значение по адресу, на который указывает указатель. Для получения адреса переменной 
применяется операция <code>&amp;</code>:</p>
<pre class="brush:c#;">
unsafe
{
...
<p>При объявлении указателя указываем тип <code>int* x;</code> - в данном случае объявляется указатель на целое число. Но кроме типа 
<code>int</code> можно использовать и другие: <code>sbyte, byte, short, ushort, int, uint, long, ulong, char, float, double, decimal или bool</code>. 
Также можно объявлять указатели на типы <code>enum</code>, структуры и другие указатели.</p>
<p>Выражение <code>x = &amp;y;</code> позволяет нам получить адрес переменной y и установить на него указатель x. До этого указатель x не на что не указывал.</p>
<p>После этого все операции с y будут влиять на значение, получаемое через указатель x и наоборот, так как они указывают на одну и ту же область в памяти.</p>
<p>Для получения значения, которое хранится в области памяти, на которую указывает указатель x, используется выражение <code>*x</code>.</p>
<h3>Получение адреса</h3>
<p>Используя преобразование указателя к целочисленному типу, можно получить адрес памяти, на который указывает указатель:</p>
<pre class="brush:c#;">
int* x; // определение указателя
int y = 10; // определяем переменную
...
<p>Для получения адреса используется преобразование в тип uint, long или ulong. 
...
Соответственно на 64-разрядных системах диапазон доступных адресов гораздо больше, поэтому в данном случае лучше использовать ulong, чтобы избежать ошибки переполнения.</p>

<h3>Указатель на другой указатель</h3>
<p>Объявление и использование указателя на указатель:</p>
<pre class="brush:c#;">
unsafe
{
    int* x; // определение указателя
...
<div class="nav"><p><a href="./8.5.php">Назад</a><a href="./">Содержание</a><a href="./8.4.php">Вперед</a></p></div></div>
<div class="socBlock">
