...
<html >
<head>
<title>C# и .NET | Проекция данных</title>
<meta charset="utf-8" />
<meta name="description" content="Проекция данных в LINQ в языке программирования C# и .NET, методы Select, SelectMany, объединение выборок, оператор let, использование в запросах linq переменных">
...
<h2>Проекция данных</h2><div class="date">Последнее обновление: 13.02.2022</div>
...
<p>Проекция позволяет преобразовать объект одного типа в объект другого типа. Для проекции используется оператор <span class="b">select</span>. 
Допустим, у нас есть набор объектов следующего класса, представляющего пользователя:</p>
<pre class="brush:c#;">
record class Person(string Name, int Age);
</pre>
<p>Но, допустим, нам нужен не весь объект, а только его свойство Name:</p>
<pre class="brush:c#;">
...
<p>Результат выражения LINQ будет представлять набор строк, поскольку выражение <code>select p.Name</code> выбирает в результирующую выборку только значения свойства Name.</p>
<div class="console">
...
<p>В качестве альтернативы мы могли бы использовать метод расширения <span class="b">Select()</span>:</p>
<pre class="brush:c#;">Select(Func&lt;TSource,TResult&gt; selector)</pre>
<p>Этот метод принимает функцию преобразования в виде делегата Func&lt;TSource,TResult&gt;. Функция преобразования получает каждый объект выборки типа TSource и с его помощью 
создает объект TResult. Метод Select возвращает коллекцию преобразованных объектов.</p>
<p>Перепишем предыдущий пример с применением метода Select:</p>
<pre class="brush:c#;">
var people = new List&lt;Person&gt;
{
    new Person ("Tom", 23),
...
<p>Аналогично можно создать объекты другого типа, в том числе анонимного:</p>
<pre class="brush:c#;">
var people = new List&lt;Person&gt;
{
    new Person ("Tom", 23),
...
<p>Здесь оператор <code>select</code> создает объект анонимного типа, используя текущий объект Person. И теперь результат будет содержать набор объектов 
данного анонимного типа, в котором определены два свойства: FirstName и Year (год рождения). Консольный вывод программы:</p>
<div class="console">
<pre class="consoletext">
Tom - 1999
Bob - 1995
...
<h3>Переменые в запросах и оператор let</h3>
<p>Иногда возникает необходимость произвести в запросах LINQ какие-то дополнительные промежуточные вычисления. Для этих целей мы можем задать 
в запросах свои переменные с помощью оператора <span class="b">let</span>:</p>
<pre class="brush:c#;">
var people = new List&lt;Person&gt;
{
    new Person ("Tom", 23),
...
<p>В данном случае создаются две переменных. Переменная <code>name</code>, значение которой равно <code>$"Mr. {p.Name}"</code>.</p>
<p>Возможность определения переменных наверное одно из главных преимуществ операторов LINQ по сравнению с методами расширения.</p>


<h3>Выборка из нескольких источников</h3>
<p>В LINQ можно выбирать объекты не только из одного, но и из большего количества источников. Например, возьмем классы:</p>
<pre class="brush:c#;">
record class Course(string Title);  // учебный курс
record class Student(string Name);  // студент
</pre>
<p>Класс Course  представляет учебный курс и хранит его название. Класс Student представляет студента и хранит его имя.</p>
<p>Допустим, нам надо из списка курсов и списка студентов получить набор пар студент-курс (условно говоря сущность, которая представляет учебу студента на данном курсе):</p>
<pre class="brush:c#;">
...
<p>Таким образом, при выборке из двух источников каждый элемент из первого источника будет сопоставляться с каждым элементом из второго источника. 
То есть получиться 4 пары.</p>

<h3>SelectMany и сведение объектов</h3>
<p>Метод <span class="b">SelectMany</span> позволяет свести набор коллекций в одну коллекцию. Он имеет ряд перегруженных версий. Возьмем одну из них:</p>
<pre class="brush:c#;">
...
<p>Первая версия метода принимает функцию преобразования в виде делегата <code>Func&lt;TSource,IEnumerable&lt;TResult&gt;&gt;</code>. Функция преобразования получает каждый объект выборки типа TSource и с его помощью 
создает набор объектов TResult. Метод SelectMany возвращает коллекцию преобразованных объектов.</p>
<p>Вторая версия принимает функцию преобразования в виде делегата <code>Func&lt;TSource,IEnumerable&lt;TResult&gt;&gt;</code>. Функция преобразования 
получает каждый объект выборки типа TSource и возвращает некоторую промежуточную коллекцию типа TCollection. 
Второй параметр - то же функция функция преобразования в виде делегата <code>Func&lt;TSource,TCollection,TResult&gt;</code>, которая получает два параметра - каждый элемент 
текущей выборки и каждый элемент промежуточной коллекции и на их основе создает некоторый объект типа TResult.</p>
...
<p>Здесь нам дан список компаний, в каждой компании имеет набор сотрудников в виде списка объектов Person. И на выходе мы получаем список сотрудников всех компаний, то есть 
по сути коллекцию объектов Person. Консольный вывод:</p>
<div class="console">
<pre class="consoletext">
Tom
Bob
...
<p>Аналогичный пример с помощью операторов LINQ:</p>
<pre class="brush:c#;">
var companies = new List&lt;Company&gt;
{
    new Company("Microsoft", new List&lt;Person&gt; {new Person("Tom"), new Person("Bob")}),
...
<p>Теперь добавим к сотрудникам их компанию:</p>
<pre class="brush:c#;">
var companies = new List&lt;Company&gt;
{
    new Company("Microsoft", new List&lt;Person&gt; {new Person("Tom"), new Person("Bob")}),
...
<p>Здесь применяется другая версия метода SelectMany. Первый делегат в виде <code>c =&gt; c.Staff</code> создает промежуточную коллекцию - фактически просто возвращаем 
набор сотрудников каждой компании. Второй делегат - <code>(c, emp)=&gt; new { Name = emp.Name, Company = c.Name }</code> 
получает каждую компанию и каждый элемент промежуточной коллекции - объект Person и на их основе создает анонимный объект с двумя свойствами Name и Company. Консольный вывод программы:</p>
<div class="console">
<pre class="consoletext">
Tom - Microsoft
Bob - Microsoft
...
<p>Аналогичный пример с помощью операторов запросов:</p>
<pre class="brush:c#;">
var companies = new List<Company>
{
    new Company("Microsoft", new List&lt;Person&gt; {new Person("Tom"), new Person("Bob")}),
...
<div class="nav"><p><a href="./15.1.php">Назад</a><a href="./">Содержание</a><a href="./15.2.php">Вперед</a></p></div></div>
<div class="socBlock">
