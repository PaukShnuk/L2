...
<html >
<head>
<title>C# и .NET | LINQ</title>
<meta charset="utf-8" />
<meta name="description" content="Язык интегрированных запросов LINQ в языке программирования C# и .NET, запросы к источнику данных, основные методы Language-Integrated Query">
...
<h1>LINQ</h1><h2>Основы LINQ</h2><div class="date">Последнее обновление: 12.02.2022</div>
...
<p><span class="b">LINQ</span> (Language-Integrated Query) представляет простой и удобный язык запросов к источнику данных. В качестве источника данных может выступать 
...
типа источника LINQ позволяет применить ко всем один и тот же подход для выборки данных.</p>
<p>Существует несколько разновидностей LINQ:</p>
<ul>
<li><p><span class="b">LINQ to Objects</span>: применяется для работы с массивами и коллекциями</p></li>
<li><p><span class="b">LINQ to Entities</span>: используется при обращении к базам данных через технологию Entity Framework</p></li>
<li><p><span class="b">LINQ to XML</span>: применяется при работе с файлами XML</p></li>
<li><p><span class="b">LINQ to DataSet</span>: применяется при работе с объектом DataSet</p></li>
<li><p><span class="b">Parallel LINQ (PLINQ)</span>: используется для выполнения параллельных запросов</p></li>
</ul>
<p>В этой главе речь пойдет прежде всего о <span class="b">LINQ to Objects</span>, но в последующих материалах также будут затронуты и другие разновидности LINQ. 
Основная часть функциональности LINQ сосредоточена в пространстве имен <span class="b">System.LINQ</span>. В проектах под .NET 6 данное пространство имен подключается по умолчанию.</p>
<p>В чем же удобство LINQ? Посмотрим на простейшем примере. Выберем из массива строки, которые начинаются на определенную букву, например, букву "T", и отсортируем полученный список:</p>
<pre class="brush:c#;">
...
<p>Для отфильтрованных строк создается специальный список. Затем в цикле проходим по всем элементам массива и, если они соответствуют условию (начинаются на букву T), 
то добавляем их в этот список. Затем сортируем список по возрастанию. И в конце элементы полученного списка выводим на консоль:</p>
<div class="console">
...
<p>Хотя подобный подход вполне работает, однако LINQ позволяет значительно сократить код с помощью интуитивно понятного и краткого синтаксиса.</p>
<p>Для работы с колекциями можно использовать два способа:</p>
<ul>
<li><p>Операторы запросов LINQ</p><li>
<li><p>Методы расширений LINQ</p><li>
</ul>
<p>Рассмотрим оба способа</p>
<h3>Операторы запросов LINQ</h3>
<p>Операторы запросов LINQ в каком-то роде частично напоминают синтаксис запросов SQL, поэтому если вы работали когда-нибудь с sql-запросами, то будет проще 
понять общую концепцию. Итак, изменим предыдущий пример, применив операторы запросов LINQ:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Bob", "Sam", "Tim", "Tomas", "Bill" };
...
<p>Прежде всего, как мы видим, код стал меньше и проще, а результат будет тем же. В принципе все выражение можно было бы записать в одну строку:</p> 
<pre class="brush:c#;">var selectedPeople = from p in people where p.ToUpper().StartsWith("T") orderby p  select p;</pre> 
<p>Но для более понятной логической разбивки я поместил каждое отдельное подвыражение на отдельной строке.</p>
<p>Простейшее определение запроса LINQ выглядит следующим образом:</p>
<pre class="brush:c#;">
from переменная in набор_объектов
select переменная;
</pre>
<p>Итак, что делает этот запрос LINQ? Выражение <code>from p in people</code> проходит по всем элементам массива people и определяет каждый элемент как 
<code>p</code>. Используя переменную <code>p</code> мы можем проводить над ней разные операции.</p>
<p>Несмотря на то, что мы не указываем тип переменной <code>p</code>, выражения LINQ являются строго типизированными. То есть среда автоматически распознает, что 
набор people состоит из объектов string, поэтому переменная <code>p</code> будет рассматриваться в качестве строки.</p>
<p>Далее с помощью оператора <code>where</code> проводится фильтрация объектов, и если объект соответствует критерию (в данном случае начальная буква 
должна быть "T"), то этот объект передается дальше.</p>
<p>Оператор <code>orderby</code> упорядочивает по возрастанию, то есть сортирует выбранные объекты.</p>
<p>Оператор <code>select</code> передает выбранные значения в результирующую выборку, которая возвращается LINQ-выражением.</p>
<p>В данном случае результатом выражения LINQ является объект <code>IEnumerable&lt;T&gt;</code>. Нередко результирующая выборка определяется с помощью 
ключевого слова <code>var</code>, тогда компилятор на этапе компиляции сам выводит тип.</p>
<h3>Методы расширения LINQ</h3>
<p>Кроме стандартного синтаксиса <code>from .. in .. select</code> для создания запроса LINQ мы можем применять специальные методы расширения, которые определены 
для интерфейса <code>IEnumerable</code>. Как правило, эти методы реализуют ту же функциональность, что и операторы LINQ типа <code>where</code> или <code>orderby</code>.</p>
<p>Например:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Bob", "Sam", "Tim", "Tomas", "Bill" };
...
<p>Запрос <code>people.Where(p=&gt;p.ToUpper().StartsWith("T")).OrderBy(p =&gt; p)</code> будет аналогичен предыдущему. Он состоит из цепочки методов Where 
и OrderBy. В качестве аргумента эти методы принимают делегат или лямбда-выражение.</p>
<p>И хотя ряд действий мы можем реализовать как с помощью операторов запросов LINQ, так и с помощью методов расширений LINQ, но не каждый метод расширения 
имеет аналог среди операторов LINQ. И в этом случае можно сочетать оба подхода. Например, используем стандартный синтаксис linq 
и метод расширения Count(), который возвращает количество элементов в выборке:</p>
<pre class="brush:c#;">
int number = (from p in people where p.ToUpper().StartsWith("T") select p).Count();
Console.WriteLine(number); // 3
</pre>
<h3>Список используемых методов расширения LINQ</h3>
<ul>
...
<li><p><span class="b">Join</span>: соединяет две коллекции по определенному признаку</p></li>
<li><p><span class="b">Aggregate</span>: применяет к элементам последовательности агрегатную функцию, которая сводит их к одному объекту</p></li>
<li><p><span class="b">GroupBy</span>: группирует элементы по ключу</p></li>
<li><p><span class="b">ToLookup</span>: группирует элементы по ключу, при этом все элементы добавляются в словарь</p></li>
<li><p><span class="b">GroupJoin</span>: выполняет одновременно соединение коллекций и группировку элементов по ключу</p></li>
<li><p><span class="b">Reverse</span>: располагает элементы в обратном порядке</p></li>
<li><p><span class="b">All</span>: определяет, все ли элементы коллекции удовлятворяют определенному условию</p></li>
<li><p><span class="b">Any</span>: определяет, удовлетворяет хотя бы один элемент коллекции определенному условию</p></li>
<li><p><span class="b">Contains</span>: определяет, содержит ли коллекция определенный элемент</p></li>
<li><p><span class="b">Distinct</span>: удаляет дублирующиеся элементы из коллекции</p></li>
<li><p><span class="b">Except</span>: возвращает разность двух коллекцию, то есть те элементы, которые создаются только в одной коллекции</p></li>
<li><p><span class="b">Union</span>: объединяет две однородные коллекции</p></li>
<li><p><span class="b">Intersect</span>: возвращает пересечение двух коллекций, то есть те элементы, которые встречаются в обоих коллекциях</p></li>
<li><p><span class="b">Count</span>: подсчитывает количество элементов коллекции, которые удовлетворяют определенному условию</p></li>
<li><p><span class="b">Sum</span>: подсчитывает сумму числовых значений в коллекции</p></li>
<li><p><span class="b">Average</span>: подсчитывает cреднее значение числовых значений в коллекции</p></li>
<li><p><span class="b">Min</span>: находит минимальное значение</p></li>
<li><p><span class="b">Max</span>: находит максимальное значение</p></li>
<li><p><span class="b">Take</span>: выбирает определенное количество элементов</p></li>
<li><p><span class="b">Skip</span>: пропускает определенное количество элементов</p></li>
...
<li><p><span class="b">Concat</span>: объединяет две коллекции</p></li>
<li><p><span class="b">Zip</span>: объединяет две коллекции в соответствии с определенным условием</p></li>
<li><p><span class="b">First</span>: выбирает первый элемент коллекции</p></li>
<li><p><span class="b">FirstOrDefault</span>: выбирает первый элемент коллекции или возвращает значение по умолчанию</p></li>
<li><p><span class="b">Single</span>: выбирает единственный элемент коллекции, если коллекция содержит больше или меньше одного элемента, то генерируется исключение</p></li>
<li><p><span class="b">SingleOrDefault</span>: выбирает единственный элемент коллекции. Если коллекция пуста, возвращает значение по умолчанию. Если в коллекции больше одного элемента, генерирует исключение</p></li>
<li><p><span class="b">ElementAt</span>: выбирает элемент последовательности по определенному индексу</p></li>
<li><p><span class="b">ElementAtOrDefault</span>: выбирает элемент коллекции по определенному индексу или возвращает значение по умолчанию, если индекс вне допустимого диапазона</p></li>
<li><p><span class="b">Last</span>: выбирает последний элемент коллекции</p></li>
<li><p><span class="b">LastOrDefault</span>: выбирает последний элемент коллекции или возвращает значение по умолчанию</p></li>
</ul>
...
<div class="nav"><p><a href="./13.8.php">Назад</a><a href="./">Содержание</a><a href="./15.2.php">Вперед</a></p></div></div>
<div class="socBlock">
