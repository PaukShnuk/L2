...
<html >
<head>
<title>C# и .NET | FileStream. Чтение и запись файла</title>
<meta charset="utf-8" />
<meta name="description" content="Чтение и запись файла в языке программирования C# и .NET, класс FileStream и его методы, примеры использования, чтение и запись файла в произвольном месте">
...
<h2>FileStream. Чтение и запись файла</h2><div class="date">Последнее обновление: 20.02.2022</div>
...
<p>Класс <span class="b">FileStream</span> представляет возможности по считыванию из файла и записи в файл. Он позволяет работать как с 
...
<pre class="brush:c#;">FileStream(string filename, FileMode mode)</pre>
<p>Здесь в конструктор передается два параметра: путь к файлу и перечисление <span class="b">FileMode</span>. Данное перечисление указывает на режим доступа к 
файлу и может принимать следующие значения:</p>
<ul>
<li><p><span class="b">Append</span>: если файл существует, то текст добавляется в конец файл. Если файла нет, то он создается. 
Файл открывается только для записи.</p></li>
<li><p><span class="b">Create</span>: создается новый файл. Если такой файл уже существует, то он перезаписывается</p></li>
<li><p><span class="b">CreateNew</span>: создается новый файл. Если такой файл уже существует, то приложение выбрасывает ошибку</p></li>
<li><p><span class="b">Open</span>: открывает файл. Если файл не существует, выбрасывается исключение</p></li>
<li><p><span class="b">OpenOrCreate</span>: если файл существует, он открывается, если нет - создается новый</p></li>
<li><p><span class="b">Truncate</span>: если файл существует, то он перезаписывается. Файл открывается только для записи.</p></li>
</ul>
...
<span class="b">Close()</span>, который вызывает метод Dispose:</p>
<pre class="brush:c#;">
...
<p>Рассмотрим наиболее важные свойства класса <span class="b">FileStream</span>:</p>
<ul>
<li><p>Свойство <span class="b">Length</span>: возвращает длину потока в байтах</p></li>
<li><p>Свойство <span class="b">Position</span>: возвращает текущую позицию в потоке</p></li>
<li><p>Свойство <span class="b">Name</span>: возвращает абсолютный путь к файлу, открытому в FileStream</p></li>
</ul>
<p>Для чтения/записи файлов можно применять следующие методы класса <span class="b">FileStream</span>:</p>
<ul>
<li><p><code>void CopyTo(Stream destination)</code>: копирует данные из текущего потока в поток destination</p>
<li><p><code>Task CopyToAsync(Stream destination)</code>: асинхронная версия метода CopyTo</p>
<li><p><code>void Flush()</code>: сбрасывает содержимое буфера в файл</p>
<li><p><code>Task FlushAsync()</code>: асинхронная версия метода Flush</p>
...
<p>И при чтении, и при записи для создания и удаления объекта FileStream используется конструкция <code>using</code>, 
по завершению которой у созданного объекта FileStream автоматически вызывается метод <code>Dispose</code>, и, таким образом, объект уничтожается.</p>
...
И при записи, и при чтении применяется объект кодировки <code>Encoding.Default</code> из пространства имен <code>System.Text</code>. 
В данном случае мы используем два его метода: <code>GetBytes</code> для получения массива байтов из строки и 
<code>GetString</code> для получения строки из массива байтов.</p>
...
<p>С помощью метода <span class="b">Seek()</span> мы можем управлять положением курсора потока, начиная с которого производится считывание или запись в файл. Этот метод принимает два параметра: 
offset (смещение) и позиция в файле. Позиция в файле описывается тремя значениями:</p>
<ul>
<li><p><span class="b">SeekOrigin.Begin</span>: начало файла</p></li>
<li><p><span class="b">SeekOrigin.End</span>: конец файла</p></li>
<li><p><span class="b">SeekOrigin.Current</span>: текущая позиция в файле</p></li>
</ul>
...
<pre class="brush:c#;">fstream.Seek(-5, SeekOrigin.End)</pre>
<img src="./pics/5.3.png" alt="чтение и запись файлов через FileStream в C#" />
...
<pre class="brush:c#;">fstream.Seek(0, SeekOrigin.Begin);</pre>
