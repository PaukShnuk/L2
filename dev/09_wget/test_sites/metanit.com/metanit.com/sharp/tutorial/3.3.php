...
<html >
<head>
<title>C# и .NET | Поля и структуры для чтения</title>
<meta charset="utf-8" />
<meta name="description" content="Поля и структуры для чтения в C# и .NET, модификатор readonly, отличие полей для чтения и констант">
...
<h2>Поля и структуры для чтения</h2><div class="date">Последнее обновление: 27.12.2021</div>
...
<h3>Поля для чтения и модификатор readonly</h3>
<p><span class="b">Поля для чтения</span> представляют такие поля класса или структуры, значение которых нельзя изменить. Таким полям можно 
...
нельзя, можно только считывать их значение.</p>
<p>Поле для чтения объявляется с ключевым словом <span class="b">readonly</span>:</p>
<pre class="brush:c#;">
...
<p>Здесь поле <code>name</code> хранит имя пользователя и представляет поле для чтения. Ему можно присвоить значение непосредственно при объявлении:</p>
<pre class="brush:c#;">public readonly string name = "Undefined";</pre>
<p>Либо можно присвоить значение в конструкторе</p>
<pre class="brush:c#;">
public Person(string name)
{
	this.name = name;
...
<p>В остальных местах программы, в том числе в самом классе Person присвоить значение полю для чтения нельзя.</p>
<h3>Сравнение констант</h3>
<ul>
<li><p>Константы должны быть определены во время компиляции, а поля для чтения могут быть определены во время выполнения программы.</p>
<p>Соответственно значение константы можно установить только при ее определении.</p>
<p>Поле для чтения можно инициализировать либо при его определении, либо в конструкторе класса.</p></li>
<li><p>Константы не могут использовать модификатор static, так как уже неявно являются статическими. Поля для чтения могут быть как статическими, так и не статическими.</p></li>
</ul>
<h3>Структуры для чтения</h3>
<p>Кроме полей для чтения в C# можно определять структуры для чтения. Для этого они предваряются модификатором <span class="b">readonly</span>:</p>
<pre class="brush:c#;">
readonly struct Person { }
</pre>
<p>Особенностью таких структур является то, что все их поля должны быть также полями для чтения:</p>
<pre class="brush:c#;">
readonly struct Person
{
...
<p>То же самое касается и свойств, которые должны быть доступны только для чтения:</p>
<pre class="brush:c#;">
readonly struct Person
{
...
<ul>
<li><a title="Вопросы для самопроверки" href="../questions/2.8.php"><i class="fa fa-lg fa-question"></i><span class="dop-txt">Вопросы для самопроверки</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./3.6.php">Назад</a><a href="./">Содержание</a><a href="./3.50.php">Вперед</a></p></div></div>
<div class="socBlock">
