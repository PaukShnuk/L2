...
<html >
<head>
<title>C# и .NET | Преобразования базовых типов данных</title>
<meta charset="utf-8" />
<meta name="description" content="Правила преобразований типов данных в C#, явные (explicit) и неявные (implicit) преобразования. Использование ключевого слова checked">
...
<h2>Преобразования базовых типов данных</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>При рассмотрении типов данных указывалось, какие значения может иметь тот или иной тип и сколько байт памяти он может занимать. В прошлой теме 
были расмотрены арифметические операции. Теперь применим операцию сложения к данным разных типов:</p>
<pre class="brush:c#;">
byte a = 4;
int b = a + 70;
</pre>
<p>Результатом операции вполне справедливо является число 74, как и ожидается.</p>
<p>Но теперь попробуем применить сложение к двум объектам типа <span class="b">byte</span>:</p>
<pre class="brush:c#;">
byte a = 4;
byte b = a + 70;  // ошибка
</pre>
<p>Здесь поменялся только тип переменной, которая получает результат сложения - с int на byte. Однако при попытке скомпилировать программу мы получим ошибку на этапе компиляции. 
И если мы работаем в Visual Studio, среда подчеркнет вторую строку красной волнистой линией, указывая, что в ней ошибка.</p>
<p>При операциях мы должны учитывать диапазон значений, которые может хранить тот или иной тип. Но в данном случае число 74, которое мы ожидаем получить, вполне укладывается 
в диапазон значений типа byte, тем не менее мы получаем ошибку.</p>
<p>Дело в том, что операция сложения (да и вычитания) возвращает значение типа int, если в операции участвуют целочисленные типы данных с разрядностью меньше 
или равно int (то есть типы byte, short, int). Поэтому результатом операции <code>a + 70</code> будет объект, который имеет длину в памяти 4 байта. 
Затем этот объект мы пытаемся присвоить переменной b, которая имеет тип byte и в памяти занимает 1 байт.</p>
<p>И чтобы выйти из этой ситуации, необходимо применить операцию преобразования типов. <span class="b">Операция преобразования</span> типов предполагает указание в скобках того типа, к которому надо преобразовать значение:</p>
<pre class="brush:c#;">
(тип_данных_в_который_надо_преобразовать)значение_для_преобразования;
</pre>
<p>Так, изменим предыдущий пример, применив операцию преобразования типов:</p>
<pre class="brush:c#;">
byte a = 4;
byte b = (byte)(a + 70);
</pre>
<h3>Сужающие и расширяющие преобразования</h3>
<p>Преобразования могут сужающие (narrowing) и расширяющие (widening). Расширяющие преобразования расширяют размер объекта в памяти. Например:</p>
<pre class="brush:c#;">
byte a = 4;				// 0000100
ushort b = a;	// 000000000000100
</pre>
<p>В данном случае переменной типа ushort присваивается значение типа byte. Тип byte занимает 1 байт (8 бит), 
и значение переменной a в двоичном виде можно представить как:</p>
<pre class="brush:c#;">00000100</pre>
<p>Значение типа ushort занимает 2 байта (16 бит). И при присвоении переменной b значение переменной a расширяется до 2 байт</p>
<pre class="brush:c#;">0000000000000100</pre>
<p>То есть значение, которое занимает 8 бит, <span class="b">расширяется</span> до 16 бит.</p>
<p>Сужающие преобразования, наоборот, сужают значение до типа меньшей разядности. Во втором листинге статьи мы как раз имели дело с сужающими преобразованиями:</p>
<pre class="brush:c#;">
ushort a = 4;
byte b = (byte) a;
</pre>
<p>Здесь переменной b, которая занимает 8 бит, присваивается значение ushort, которое занимает 16 бит. То есть из <code>0000000000000100</code> 
получаем <code>00000100</code>. Таким образом, значение сужается с 16 бит (2 байт) до 8 бит (1 байт).</p>
<h3>Явные и неявные преобразования</h3>
<h4>Неявные преобразования</h4>
<p>В случае с расширяющими преобразованиями компилятор за нас выполнял все преобразования данных, то есть преобразования были неявными 
(<span class="b">implicit conversion</span>). Такие преобразования не вызывают каких-то затруднений. Тем не менее стоит сказать пару слов об общей механике подобных преобразований.</p>
<p>Если производится преобразование от безнакового типа меньшей разрядности к безнаковому типу большой разрядности, 
то добавляются дополнительные биты, которые имеют значени 0. Это называется <span class="b">дополнение нулями</span> или zero extension.</p>
<pre class="brush:c#;">
byte a = 4;				// 0000100
ushort b = a;	// 000000000000100
</pre>
<p>Если производится преобразование к знаковому типу, то битовое представление дополняется нулями, если число положительное, 
и единицами, если число отрицательное. Последний разряд числа содержит знаковый бит - 0 для положительных и 1 для 
отрицательных чисел. При расширении в добавленные разряды компируется знаковый бит.</p>
<p>Рассмотрим преобразование положительного числа:</p>
<pre class="brush:c#;">
sbyte a = 4;			// 0000100
short b = a;	// 000000000000100
</pre>
<img src="./pics/2.7.png" alt="Sing bit in conversions in c#" />
<p>Преобразование отрицательного числа:</p>
<pre class="brush:c#;">
sbyte a = -4;			// 1111100
short b = a;	// 111111111111100
</pre>
<img src="./pics/2.8.png" alt="Sing bit  negative number in conversions in c#" />
<h4>Явные преобразования</h4>
<p>При явных преобразованиях (<span class="b">explicit conversion</span>) мы сами должны применить операцию преобразования (операция <code>()</code>). 
Суть операции преобразования типов состоит в том, что перед значением указывается в скобках тип, к которому надо привести данное значение:</p>
<pre class="brush:c#;">
int a = 4;
int b = 6;
byte c = (byte)(a+b);
...
<p><span class="b">byte</span> -&gt; <span class="b">short</span> -&gt; <span class="b">int</span> -&gt; <span class="b">long</span> -&gt; <span class="b">decimal</span></p>
<p><span class="b">int</span> -&gt; <span class="b">double</span></p>
<p><span class="b">short</span> -&gt; <span class="b">float</span> -&gt; <span class="b">double</span></p>
<p><span class="b">char</span> -&gt; <span class="b">int</span></p>
<p>Все безопасные автоматические преобразования можно описать следующей таблицей:</p>
<table class="tab">
<tr class="tabhead"><td><p>Тип</p></td><td><p>В какие типы безопасно преобразуется</p></td></tr>
<tr><td><p>byte</p></td><td><p>short, ushort, int, uint, long, ulong, float, double, decimal</p></td></tr>
<tr><td><p>sbyte</p></td><td><p>short, int, long, float, double, decimal</p></td></tr>
<tr><td><p>short</p></td><td><p>int, long, float, double, decimal</p></td></tr>
<tr><td><p>ushort</p></td><td><p>int, uint, long, ulong, float, double, decimal</p></td></tr>
<tr><td><p>int</p></td><td><p>long, float, double, decimal</p></td></tr>
<tr><td><p>uint</p></td><td><p>long, ulong, float, double, decimal</p></td></tr>
<tr><td><p>long</p></td><td><p>float, double, decimal</p></td></tr>
<tr><td><p>ulong</p></td><td><p>float, double, decimal</p></td></tr>
<tr><td><p>float</p></td><td><p>double</p></td></tr>
<tr><td><p>char</p></td><td><p>ushort, int, uint, long, ulong, float, double, decimal</p></td></tr>
</table>
<img src="./pics/2.9.png" alt="Преобразования типов в c#" />
<p>В остальных случаях следует использовать явные преобразования типов.</p>
<p>Также следует отметить, что несмотря на то, что и double, и decimal могут хранить дробные данные, а decimal имеет большую разрядность, чем double, но все равно 
значение double нужно явно приводить к типу decimal:</p>
<pre class="brush:c#;">
double a = 4.0;
decimal b = (decimal)a;
</pre>
<h3>Потеря данных и ключевое слово checked</h3>
<p>Рассмотрим другую ситуацию, что будет, например, в следующем случае:</p>
<pre class="brush:c#;">
int a = 33;
int b = 600;
byte c = (byte)(a+b);
...
<p>Результатом будет число 121, так число 633 не попадает в допустимый диапазон для типа byte, и старшие биты будут усекаться. В итоге получится число 121. 
...
либо мы можем выбрать вместо byte другой тип данных, например, int.</p>
<p>Однако ситуации разные могут быть. Мы можем точно не знать, какие значения будут 
иметь числа a и b. И чтобы избежать подобных ситуаций, в c# имеется ключевое слово <code>checked</code>:</p>
<pre class="brush:c#;">
try
{
    int a = 33;
...
<p>При использовании ключевого слова <code>checked</code> приложение выбрасывает исключение о переполнении. Поэтому для его обработки 
...
в блок try мы включаем действия, в которых может потенциально возникнуть ошибка, а в блоке catch обрабатываем ошибку.</p>
...
<ul>
<li><a title="Вопросы для самопроверки" href="../questions/1.2.php"><i class="fa fa-lg fa-question"></i><span class="dop-txt">Вопросы для самопроверки</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./2.23.php">Назад</a><a href="./">Содержание</a><a href="./2.24.php">Вперед</a></p></div></div>
<div class="socBlock">
