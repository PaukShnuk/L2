...
<html >
<head>
<title>C# и .NET | Динамическая загрузка сборок и позднее связывание</title>
<meta charset="utf-8" />
<meta name="description" content="Динамическая загрузка сборок в языке программирования C# и .NET с помощью метода Type.LoadFrom(), динамическое создание экземпляра классом System.Activator и позднее связывание, вызов метода">
...
<h2>Динамическая загрузка сборок и позднее связывание</h2><div class="date">Последнее обновление: 17.02.2022</div>
...
<p>При создании приложения для него определяется набор сборок, которые будут использоваться. В проекте указываются 
ссылки на эти сборки, и когда приложение выполняется, при обращении к функционалу этих сборок они автоматически подгружаются.</p>
<p>Но также мы можем сами динамически подгружать другие сборки, на которые в проекте нет ссылок.</p>
<p>Для управления сборками в пространстве имен <code>System.Reflection</code> имеется класс <span class="b">Assembly</span>. С его помощью можно загружать сборку, 
исследовать ее.</p>
<p>Чтобы динамически загрузить сборку в приложение, надо использовать статические методы <span class="b">Assembly.LoadFrom()</span> или <span class="b">Assembly.Load()</span>.</p>
<p>Метод <code>LoadFrom()</code> принимает в качестве параметра путь к сборке.</p>
<p>Допустим, у нас есть два проекта:</p>
<img src="./pics/14.1.png" alt="Загрузка сборок в C# и .NET"> 
<p>Пусть в проекте MyApp, который компилируется в сборку MyApp.dll, имеется файл Program.cs со следующим кодом:</p>
<pre class="brush:c#;">
...
<p>В другом проект исследуем сборку MyApp.dll на наличие в ней различных типов:</p>
<pre class="brush:c#;">
...
<p>В данном случае для исследования указывается сборка MyApp.dll. Здесь использован относительный путь, так как сборка находится 
в одной папке с приложением - в проекте в каталоге <span class="ii">bin/Debug/net6.x</span>. Можно в принципе в качестве имени указать и имя текущего приложение. В этом случае 
программа будет исследовать саму себя. В любом случае стоит учитывать, что загрузке подлежат (по крайней мере в .NET 6.0) сборки с расширением dll, но не exe.</p>
...
<p>Как видно из вывода, полное название сборки: <code>MyApp, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null</code>. А сама сборка MyApp.dll содержит 
пять типов - кроме класса Person и неявно определяемого класса Program добавляется еще три автоматически генерируемых класса.</p>
<p>Метод <code>Load()</code> действует аналогично, только в качестве его параметра передается дружественное имя сборки, которое нередко совпадает с именем 
приложения: <code>Assembly asm = Assembly.Load("MyApp");</code></p>
<p>Получив все типы сборки с помощью метода <code>GetTypes()</code>, мы опять же можем применить к каждому типу все те методы, которые были рассмотрены 
в прошлой теме.</p>
<h3>Позднее связывание</h3>
<p>С помощью динамической загрузки мы можем реализовать технологию позднего связывания. <span class="b">Позднее связывание</span> позволяет создавать 
экземпляры некоторого типа, а также использовать его во время выполнения приложения.</p> 
<p>Использование позднего связывания менее безопасно в том плане, что при жестком кодировании всех типов (ранее связывание) на этапе компиляции 
...
неизвестен, и его могут разработать и подключить сторонние разработчики.</p>
<p>Ключевую роль в позднем связывании играет класс <span class="b">System.Activator</span>. С помощью его статического метода <span class="b">Activator.CreateInstance()</span> 
можно создавать экземпляры заданного типа.</p>
<p>Например, динамически загрузим сборку и вызовем у ней некоторый метод. Допустим, загружаемая сборка MyApp.exe представляет следующую программу:</p>
<pre class="brush:c#;">
class Program
{
...
<p>В данном случае мы явным образом определили класс Program с методом Main. И кроме того, в классе Program определен статический метод 
Square, который в качестве параметра принимает число и возвращает его квадрат.</p>
<p>Теперь динамически подключим сборку с этой программой в другой программе и вызовем ее методы.</p>
<p>Пусть наша основная программа будет выглядеть так:</p>
<pre class="brush:c#;">
using System.Reflection;
...
<p>Сначала получаем ссылку на исследуемую сборку в переменную asm:</p>
<pre class="brush:c#;">Assembly asm = Assembly.LoadFrom("MyApp.dll")</pre>
<p>Затем с помощью метода GetType получаем тип - класс Program, который находится в сборке MyApp.dll:</p>
<pre class="brush:c#;">Type? t = asm.GetType("Program");</pre>
<p>Получив тип, создаем его экземпляр:</p>
<pre class="brush:c#;">object? obj = Activator.CreateInstance(t)</pre>
<p>Результат создания - объект класса Program представляет собой переменную <code>obj</code>.</p>
<p>И в конце остается вызвать метод. Во-первых, получаем сам метод:</p>
<pre class="brush:c#;">MethodInfo? square = t.GetMethod("Square", BindingFlags.NonPublic | BindingFlags.Static);</pre>
<p>Поскольку метод Square приватный и статический, то в качестве второго параметра в метод передаются флаги <code>BindingFlags.NonPublic | BindingFlags.Static</code></p>
<p>И потом с помощью метода <code>Invoke</code> вызываем его:</p>
<pre class="brush:c#;">
object? result = square?.Invoke(obj, new object[] { 7 });
</pre>
<p>Здесь первый параметр представляет объект, 
для которого вызывается метод, а второй - набор параметров в виде массива object[].</p>
<p>Так как метод Square возвращает некоторое значение, то мы можем его получить из метода в виде объекта типа object.</p>
<P>Если бы метод не принимал параметров, то вместо массива объектов использовалось бы значение <code>null</code>: <code>method.Invoke(obj, null)</code></P>
<p>В сборке MyApp.exe в классе Program также есть и другой метод - метод Main, который также выполняет некоторую работу. Вызовем теперь его:</p>
<pre class="brush:c#;">
using System.Reflection;
...
<p>Так как метод Main является статическим и не публичным, то к нему также применяется битовая маска <code>BindingFlags.NonPublic | BindingFlags.Static</code>. 
И поскольку он в качестве параметра принимает массив строк, то при вызове метода передается соответствующее значение: <code>main.Invoke(obj, new object[]{new string[]{}})</code></p>
...
<div class="nav"><p><a href="./14.6.php">Назад</a><a href="./">Содержание</a><a href="./14.4.php">Вперед</a></p></div></div>
<div class="socBlock">
