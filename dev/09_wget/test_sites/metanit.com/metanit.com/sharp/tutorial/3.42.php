...
<html >
<head>
<title>C# и .NET | Переопределение и скрытие методов</title>
<meta charset="utf-8" />
<meta name="description" content="Чем отличаются переопределение и скрытие методов базового класса в производном в языке программирования C# и .NET">
...
<h2>Различие переопределения и скрытия методов</h2><div class="date">Последнее обновление: 29.12.2021</div>
...
<p>Ранее было рассмотрена два способа изменения функциональности методов, унаследованных от базового класса - скрытие и переопределение. В чем разница между двумя этими способами?</p>
<h3>Переопределение</h3>
<p>Возьмем пример с переопределением методов:</p>
<pre class="brush:c#;">
class Person
{
...
<p>Используем классы в программе:</p>
<pre class="brush:c#;">
Person tom = new Employee("Tom", "Microsoft");
tom.Print();		// Tom работает в Microsoft
</pre>
<p>При вызове <code>tom.Print()</code> выполняется реализация метода Print из класса Employee, несмотря на то, что переменная <code>tom</code> - переменная типа Person.</p> 
<p>Для работы с виртуальными методами компилятор формирует таблицу виртуальных методов (<span class="b">Virtual Method Table</span> или <span class="b">VMT</span>). 
В нее записывается адреса виртуальных методов. Для каждого класса создается своя таблица.</p>
<p>Когда создается объект класса, то компилятор передает в конструктор объекта специальный код, который связывает объект и таблицу VMT.</p>
<p>А при вызове виртуального метода из объекта берется адрес его таблицы VMT. Затем из VMT извлекается адрес метода и ему передается управление. То есть процесс выбора реализации метода производится 
во время выполнения программы. Собственно так и выполняется виртуальный метод. Следует учитывать, что так как среде выполнения вначале необходимо получить из таблицы VMT адрес нужного метода, то это немного замедляет выполнение программы.</p>
<h3>Скрытие</h3>
<p>Теперь возьмем те же классы Person и Employee, но вместо переопределения используем сокрытие:</p>
<pre class="brush:c#;">
class Person
{
	public string Name { get; set; }
...
<p>И посмотрим, что будет в следующем случае:</p>
<pre class="brush:c#;">
Person tom = new Employee("Tom", "Microsoft");
tom.Print();		// Tom
</pre>
<p>Переменная tom представляет тип Person, но хранит ссылку на объект Employee. Однако при вызове метода Print будет выполняться та версия метода, 
...
Поэтому при вызове <code>tom.Print()</code> вызывается метод Print из класса Person.</p>
...
<div class="nav"><p><a href="./3.41.php">Назад</a><a href="./">Содержание</a><a href="./3.8.php">Вперед</a></p></div></div>
<div class="socBlock">
