...
<html >
<head>
<title>C# и .NET | Коллекция Dictionary</title>
<meta charset="utf-8" />
<meta name="description" content="Словарь Dictionary&lt;K, V&gt; в C#, его основные методы и свойства, использование ключей словаря, класс KeyValuePair&lt;TKey, TValue&gt;, инициализация, примеры использования">
...
<h2>Коллекция Dictionary&lt;K, V&gt;</h2><div class="date">Последнее обновление: 04.01.2022</div>
...
<p>Еще один распространенный тип коллекции представляют словари. Словарь хранит объекты, которые представляют пару ключ-значение. 
Класс словаря <span class="b">Dictionary&lt;K, V&gt;</span> типизируется двумя типами: параметр <span class="b">K</span> представляет тип ключей, 
а параметр <span class="b">V</span> предоставляет тип значений.</p>
...
<p>Здесь словарь <code>people</code> в качестве ключей принимает значения типа int, а в качестве значений - строки.</p>
...
Поскольку при объявлении словаря people для ключей указан тип <code>int</code>, а для значений - тип <code>string</code>, то в элементе словаря 
...
<h3>KeyValuePair</h3>
<p>Стоит отметить, что каждый элемент в словаре представляет структуру <span class="b">KeyValuePair&lt;TKey, TValue&gt;</span>, где параметр <span class="b">TKey</span> представляет тип ключа, 
а параметр <span class="b">TValue</span> - тип значений элементов. Эта структура предоставляет свойства <code>Key</code> и <code>Value</code>, 
...
<p>Конструктор типа <code>KeyValuePair</code> принимает два параметра - ключ элемента и его значения. То есть в данном случае создается один такой 
элемент  - <code>mike</code> с ключом 56 и значением "Mike". И этот элемент добавляется в список employees, которым затем инициализируется словарь.</p>
...
<p>Для перебора словаря можно применять цикл <span class="b">foreach</span>:</p>
<pre class="brush:c#;">
var people = new Dictionary&lt;int, string&gt;()
{
    [5] = "Tom",
...
<pre class="brush:c#;">словарь[ключ]</pre>
...
<ul>
<li><p><span class="b">void Add(K key, V value)</span>: добавляет новый элемент в словарь</p></li>
<li><p><span class="b">void Clear()</span>: очищает словарь</p></li>
<li><p><span class="b">bool ContainsKey(K key)</span>: проверяет наличие элемента с определенным ключом и возвращает true при его наличии в словаре</p></li>
<li><p><span class="b">bool ContainsValue(V value)</span>: проверяет наличие элемента с определенным значением и возвращает true при его наличии в словаре</p></li>
<li><p><span class="b">bool Remove(K key)</span>: удаляет по ключу элемент из словаря</p>
<p>Другая версия этого метода позволяет получить удленный элемент в выходной параметр: <span class="b">bool Remove(K key, out V value)</span></p></li>
<li><p><span class="b">bool TryGetValue(K key, out V value)</span>: получает из словаря элемент по ключу key. При успешном получении передает 
значение элемента в выходной параметр value и возвращает true</p></li>
<li><p><span class="b">bool TryAdd(K key, V value)</span>: добавляет в словарь элемент с ключом key и значением value. При успешном добавлении 
возвращает true</p></li>
</ul>
<p>Из свойств следует отметить свойство <span class="b">Count</span>, которое возвращает количество элементов в словаре.</p>
