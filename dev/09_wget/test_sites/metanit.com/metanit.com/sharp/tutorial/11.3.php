...
<html >
<head>
<title>C# и .NET | Потоки с параметрами и ParameterizedThreadStart</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и запуск потоков в языке программирования C# и .NET, передача параметров в потоки с помощью делегата ParameterizedThreadStart. Способ передачи параметров с использованием ThreadStart">
...
<h2>Потоки с параметрами и ParameterizedThreadStart</h2><div class="date">Последнее обновление: 06.02.2022</div>
...
<p>В предыдущей статье было рассмотрено, как запускать в отдельных потоках методы без параметров. А что, если нам надо передать какие-нибудь параметры в поток?</p>
<p>Для этой цели используется делегат <span class="b">ParameterizedThreadStart</span>, который передается в конструктор класса Thread:</p>
<pre class="brush:c#;">public delegate void ParameterizedThreadStart(object? obj);</pre> 
<p>Применение делегата ParameterizedThreadStart во многом похоже на работу с <code>ThreadStart</code>. Рассмотрим на примере:</p>
<pre class="brush:c#;">
...
<p>При создании потока в конструктор класса Thread передается объект делегата ParameterizedThreadStart <code>new Thread(new ParameterizedThreadStart(Print))</code>, 
либо непосредственно метод, который соответствует этому делегату (<code>new Thread(Print)</code>), в том числе в виде лямбда-выражения 
(<code>new Thread(message =&gt; Console.WriteLine(message))</code>)</p>
<p>Затем при запуске потока в метод <code>Start()</code> передается значение, которое передается параметру метода Print. И в данном случае мы получим следующий 
консольный вывод:</p>
<div class="console">
...
качестве единственного параметра принимает объект типа <code>object?</code>. Поэтому если мы хотим использовать данные других типов, в 
...
<p>Но тут опять же есть одно ограничение: метод <code>Thread.Start</code> не является типобезопасным, то есть мы можем передать в него любой тип, 
