...
<html >
<head>
<title>C# и .NET | Сериализация в XML. XmlSerializer</title>
<meta charset="utf-8" />
<meta name="description" content="Сериализация в XML в языке программирования C# и .NET, класс XmlSerializer и его использование, пример сериализации и десериализации объектов в xml-файл">
...
<h2>Сериализация в XML. XmlSerializer</h2><div class="date">Последнее обновление: 21.02.2022</div>
...
<p>Для удобного сохранения и извлечения объектов из файлов xml может использоваться класс <span class="b">XmlSerializer</span> 
из пространства имен <code>System.Xml.Serialization</code>. Данный класс упрощает сохранение сложных объектов в формат xml и последующее их извлечение.</p>
<p>Для создания объекта XmlSerializer можно применять различные конструкторы, но почти все они требуют указания типа данных, которые будут сериализоваться и десериализоваться:</p>
<pre class="brush:c#;">
...
<p>В данном случае XmlSerializer будет работать только с объектами класса Person.</p>
<p>Следует учитывать, что XmlSerializer предполагает некоторые ограничения. Например, класс, подлежащий сериализации, должен иметь стандартный конструктор 
без параметров и должен иметь модификатор доступа <span class="b">public</span>. Также сериализации подлежат только открытые члены. 
Если в классе есть поля или свойства с модификатором <code>private</code>, то при сериализации они будут игнорироваться. Кроме того, свойства должны иметь общедоступные геттеры и сеттеры.</p>
<h3>Сериализация</h3>
<p>Для сериализации (то есть сохранения в форма xml) применяется метод <span class="b">Serialize()</span>. Данный метод имеет ряд версий. Возьмем самую простую из них:</p>
<pre class="brush:c#;">void Serialize (Stream stream, object? o);</pre>
<p>В качестве первого параметра передается поток Stream (например, объект FileStream), в который будет идти сериализация. А второй параметр представляет собственно тот объект, 
который будет сохраняться в формат xml. Например:</p>
<pre class="brush:c#;">
...
<p>Итак, класс Person общедоступный, имеет общедоступные свойства и конструктор без параметров, поэтому объекты этого класса подлежат сериализации. При создании объекта XmlSerializer 
передаем в конструктор тип класса Person.</p>
<p>В метод Serialize передается файловый поток для сохранения данных в файл <span class="b">person.xml</span> и сохраняемый в этот файл объект Person. 
И если по завершению программы мы откроем файл <span class="b">person.xml</span>, то увидим содержание нашего объекта:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0"?&gt;
...
<p>Для десериализации данных xml в объект кода C# применяется метод <span class="b">Deserialize()</span>. Отметим одну из версий этого метода:</p>
<pre class="brush:c#;">object? Deserialize (Stream stream);</pre>
<p>В качестве параметра в метод передается объект Stream, который содержит данные в формате xml. Результатом метода является десериализованный объект.</p>
<p>Например, десериализуем данные из выше созданного файла <span class="b">person.xml</span>:</p>
<pre class="brush:c#;">
using System.Xml.Serialization;
...
<h3>Сериализация и десериализация коллекций</h3>
<p>Подобным образом мы можем сериализовать массив или коллекцию объектов:</p>
<pre class="brush:c#;">
using System.Xml.Serialization;
...
<p>Но это был простой объект. Однако с более сложными по составу объектами работать так же просто. Например:</p>
<pre class="brush:c#;">
using System.Xml.Serialization;
...
<p>Класс Person содержит свойство Company, которое будет хранить объект класса Company. Члены класса Company объявляются с модификатором public, 
кроме того также присутствует стандартный конструктор без параметров. В итоге после сериализации мы получим следующий xml-документ:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0"?&gt;
...
<div class="nav"><p><a href="./16.7.php">Назад</a><a href="./">Содержание</a><a href="./18.1.php">Вперед</a></p></div></div>
<div class="socBlock">
