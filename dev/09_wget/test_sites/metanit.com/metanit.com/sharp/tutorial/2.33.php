...
<html >
<head>
<title>C# и .NET | Span</title>
<meta charset="utf-8" />
<meta name="description" content="Типы Span и ReadOnlySpan в языке программирования C# и .NET, оптимизация памяти при работе с массивами">
...
<h2>Span</h2><div class="date">Последнее обновление: 07.01.2022</div>
...
<p>Тип <span class="b">Span</span> представляет непрерывную область памяти. Цель данного типа - повысить производительность и эффективность использования 
памяти. Span позволяет избежать дополнительных выделений памяти при операции с наборами данных. Поскольку Span является структурой, то объект этого типа располагаетс в стеке, а не в хипе.</p>
<h3>Создание Span</h3>
<p>Для создания объекта Span можно применять один из его конструкторов:</p>
<ul>
<li><p><code>Span()</code>: создает пустой объект Span</p></li>
<li><p><code>Span(T[] array)</code>: создает объект Span из массива array</p></li>
<li><p><code>Span(void* pointer, int length)</code>: создает объект Span, который получает length байт памяти, начиная с указателя pointer</p></li>
<li><p><code>Span(T[] array, int start, int length)</code>: создает объект Span, который получает из массива array length элементов, начиная с индекса start</p></li>
</ul>
<p>Например, простейшее создание Span:</p>
<pre class="brush:c#;">
string[] people = new string[] { "Tom", "Alice", "Bob" };
Span&lt;string&gt; peopleSpan = new Span&lt;string&gt;(people);
</pre>
<p>Мы также можем непосредственно присвоить массив, и он неявно будет преобразован в Span:</p>
<pre class="brush:c#;">
string[] people = new string[] { "Tom", "Alice", "Bob" };
Span&lt;string&gt; peopleSpan = people;
</pre>
<p>Далее мы можем получать, устанавливать или перебирать данные также, как в массиве:</p>
<pre class="brush:c#;">
string[] people = new string[] { "Tom", "Alice", "Bob" };
Span&lt;string&gt; peopleSpan = people;
...
<p>Для создания производных объектов Span применяется метод Slice, который из Spana выделяет часть и возвращает ее в виде другого объекта Span. 
...
<h3>ReadOnlySpan</h3>
<p>Структура <span class="b">ReadOnlySpan</span> аналогична Span, только предназначена для неизменяемых данных. Например:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./20.5.php">Назад</a><a href="./">Содержание</a><a href="./2.32.php">Вперед</a></p></div></div>
<div class="socBlock">
