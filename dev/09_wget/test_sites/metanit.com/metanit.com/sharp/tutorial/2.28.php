...
<html >
<head>
<title>C# и .NET | Блок catch и фильтры исключений</title>
<meta charset="utf-8" />
<meta name="description" content="Определение блока catch в конструкции try...catch в C#, фильтр исключений и оператор when">
...
<h2>Блок catch и фильтры исключений</h2><div class="date">Последнее обновление: 20.01.2022</div>
...
<h3>Определение блока catch</h3>
<p>За обработку исключения отвечает блок <span class="b">catch</span>, который может иметь следующие формы:</p>
<ul>
<li><pre class="brush:c#;">
catch
{
...
<p>Фактически этот случай аналогичен предыдущему за тем исключением, что здесь используется переменная. В данном случае в переменную <code>ex</code>, которая представляет тип DivideByZeroException, помещается 
информация о возникшем исключени. И с помощью свойства <code>Message</code> мы можем получить сообщение об ошибке.</p>
...
после выражения catch идет выражение <span class="b">when</span>, после которого в скобках указывается 
условие:</p>
<pre class="brush:c#;">
catch when(условие)
{
...
<span class="b">y == 0</span>, то именно этот блок будет обрабатывать данное исключение - условие, указанное после оператора <span class="b">when</span> 
возвращает true.</p>
...
<p>В данном случае будет выброшено исключение, так как x=0. Условие первого блока catch - <span class="b">y == 0</span> 
...
<div class="nav"><p><a href="./2.14.php">Назад</a><a href="./">Содержание</a><a href="./2.29.php">Вперед</a></p></div></div>
<div class="socBlock">
