...
<html >
<head>
<title>C# и .NET | Лямбды</title>
<meta charset="utf-8" />
<meta name="description" content="Лямбды в c#, создание лямбда-выражений и их использование, лямбда-оператор =&gt;, добавление методов в лямбда-выражение, лямбда как параметр и результат метода">
...
<h2>Лямбды</h2><div class="date">Последнее обновление: 19.01.2022</div>
...
<p>Лямбда-выражения представляют упрощенную запись анонимных методов. Лямбда-выражения позволяют создать емкие лаконичные методы, которые могут 
возвращать некоторое значение и которые можно передать в качестве параметров в другие методы.</p>
<p>Ламбда-выражения имеют следующий синтаксис: слева от лямбда-оператора <code>=&gt;</code> определяется список параметров, а справа блок выражений, использующий эти параметры:</p> 
<pre class="brush:c#;">(список_параметров) =&gt; выражение</pre>
...
идут пустые скобки. А справа от лямбда-оператора идет выполняемое выражение - <code>Console.WriteLine("Hello")</code></p>
...
<p>Выше мы определили переменную hello, которая представляет делегат Message. Но начиная с версии <span class="b">C# 10</span> мы можем применять неявную типизацию 
 (определение переменной с помощью оператора <span class="b">var</span>) при определении лямбда-выражения:</p>
<pre class="brush:c#;">
...
Например, выше определенное лямбда-выражение hello по умолчанию компилятор будет рассматривать как переменную встроенного делегата <span class="b">Action</span>, который не принимает 
...
<p>В данном случае компилятор видит, что лямбда-выражение sum представляет тип Operation, а значит оба параметра лямбды представляют тип <code>int</code>. 
...
<pre class="brush:c#;">var sum = (x, y) =&gt; Console.WriteLine($"{x} + {y} = {x + y}");	// ! Ошибка</pre>
...
<p>Если лямбда-выражение содержит несколько выражение, тогда нужно использовать оператор <span class="b">return</span>, как в обычных методах:</p>
<pre class="brush:c#;">
...
В цикле проходим по всем числам и складываем их. Причем складываем только те числа, для которых делегат <code>IsEqual func</code> 
...
<p>А выражение <code>x &gt; 5</code> представляет условие, которому должно соответствовать число. Если число соответствует этому условию, то 
...
<div class="nav"><p><a href="./3.15.php">Назад</a><a href="./">Содержание</a><a href="./3.14.php">Вперед</a></p></div></div>
<div class="socBlock">
