...
<html >
<head>
<title>C# и .NET | Класс Parallel</title>
<meta charset="utf-8" />
<meta name="description" content="Класс Parallel и его применение для параллельного выполнения задач в языке программирования C# и .NET, методы Parallel.ForEach и Parallel.For">
...
<h2>Класс Parallel</h2><div class="date">Последнее обновление: 08.02.2022</div>
...
<p>Класс <span class="b">Parallel</span> также является частью TPL и предназначен для упрощения параллельного выполнения кода. 
<span class="b">Parallel</span> имеет ряд методов, которые позволяют распараллелить задачу.</p>
<p>Одним из методов, позволяющих параллельное выполнение задач, является метод <code>Invoke</code>:</p>
<pre class="brush:c#;">
...
<p>Метод <code>Parallel.Invoke</code> в качестве параметра принимает массив объектов <code>Action</code>, то есть мы можем передать в данный 
...
трех методов. Опять же как и в случае с классом Task мы можем передать либо название метода, либо лямбда-выражение.</p>
<p>И таким образом, при наличии нескольких ядер на целевой машине данные методы будут выполняться параллельно на различных ядрах. Пример консольного вывода программы:</p>
<div class="console">
...
<h3>Parallel.For</h3>
<p>Метод Parallel.For позволяет выполнять итерации цикла параллельно. Он имеет следующее определение:</p>
<pre class="brush:c#;">For(int, int, Action&lt;int&gt;)</pre> 
<p>Первый параметр метода задает начальный индекс элемента в цикле, а второй параметр - конечный индекс. Третий параметр - делегат Action - указывает 
на метод, который будет выполняться один раз за итерацию:</p>
<pre class="brush:c#;">
...
<p>В данном случае в качестве первого параметра в метод Parallel.For передается число 1, а в качестве второго - число 5. Таким образом, метод 
...
представляет тип Action&lt;int&gt;, то этот метод в качестве параметра должен принимать объект int.</p>
<p>А в качестве значения параметра в этот метод передается счетчик, который проходит в цикле от 1 до 4 включительно. И метод Square, таким образом, вызывается 4 раза. 
Пример консольного вывода:</p>
<div class="console">
<pre class="consoletext">
Выполняется задача 1
Выполняется задача 2
...
<p>Метод <span class="b">Parallel.ForEach</span> осуществляет итерацию по коллекции, реализующей интерфейс <span class="b">IEnumerable</span>, 
подобно циклу foreach, только осуществляет параллельное выполнение перебора. Он имеет следующее определение:</p> 
<pre class="brush:c#;">ParallelLoopResult ForEach&lt;TSource&gt;(IEnumerable&lt;TSource&gt; source,Action&lt;TSource&gt; body)</pre>
<p>где первый параметр представляет перебираемую коллекцию, а второй параметр - делегат, выполняющийся один раз за итерацию для каждого перебираемого элемента коллекции.</p>
<p>На выходе метод возвращает структуру <code>ParallelLoopResult</code>, которая содержит информацию о выполнении цикла.</p>
<pre class="brush:c#;">
...
<p>В данном случае поскольку мы используем коллекцию объектов int, то и метод, который мы передаем в качестве второго параметра, должен в качестве параметра принимать значение 
int. Пример консольного вывода:</p>
<div class="console">
<pre class="consoletext">
Выполняется задача 1
Выполняется задача 3
...
<h3>Выход из цикла</h3>
<p>В стандартных циклах for и foreach предусмотрен преждевременный выход из цикла с помощью оператора <code>break</code>. В методах Parallel.ForEach и 
Parallel.For мы также можем, не дожидаясь окончания цикла, выйти из него:</p>
<pre class="brush:c#;">
...
<p>Здесь метод Square, который обрабатывает каждую итерацию, принимает дополнительный параметр - объект <span class="b">ParallelLoopState</span>. 
И если счетчик в цикле достигнет значения 5, вызывается метод Break. Благодаря чему система осуществит выход и прекратит выполнение метода Parallel.For при первом удобном случае.</p>
<p>Методы Parallel.ForEach и Parallel.For возвращают объект <span class="b">ParallelLoopResult</span>, наиболее значимыми свойствами которого являются два следующих:</p>
<ul>
<li><p><span class="b">IsCompleted</span>: определяет, завершилось ли полное выполнение параллельного цикла</p></li>
<li><p><span class="b">LowestBreakIteration</span>: возвращает индекс, на котором произошло прерывание работы цикла</p></li>
</ul>
<p>Так как у нас на индексе равном 5 происходит прерывание, то свойство IsCompleted будет иметь значение <code>false</code>, а 
<code>LowestBreakIteration</code> будет равно 5.</p>
...
<div class="nav"><p><a href="./12.3.php">Назад</a><a href="./">Содержание</a><a href="./12.5.php">Вперед</a></p></div></div>
<div class="socBlock">
