...
<html >
<head>
<title>C# и .NET | Последовательное и параллельное выполнение. Task.WhenAll и Task.WhenAny</title>
<meta charset="utf-8" />
<meta name="description" content="Последовательный вызов асинхронных методов в программе на C# и .NET с помощью оператора await, параллельный запуск асинхронных методов с Task.WhenAll и Task.WhenAny">
...
<h2>Последовательное и параллельное выполнение. Task.WhenAll и Task.WhenAny</h2><div class="date">Последнее обновление: 11.02.2022</div>
...
<p>Асинхронный метод может содержать множество выражений await. Когда система встречает в блоке 
...
<p>Однако .NET позволяет упростить отслеживание выполнения набора задач с помощью метода <span class="b">Task.WhenAll</span>. Этот метод принимает набор 
асинхронных задач и ожидает завершения всех этих задач. Этот метод является аналогом статического метода <code>Task.WaitAll()</code>, однако 
...
<p>Если нам надо дождаться, когда будет выполнена хотя бы одна задача из некоторого набора задач, то можно применять метод <span class="b">Task.WhenAny()</span>. 
Это аналог метода <code>Task.WaitAny()</code> - он завершает выполнение, когда завершается хотя бы одна задача. Но для ожидания выполнения к Task.WhenAny() применяется 
оператор await:</p>
<pre class="brush:c#;">
// определяем и запускаем задачи
var task1 = PrintAsync("Hello C#");
...
<p>Задачи, передаваемые в <code>Task.WhenAll</code> и <code>Task.WhenAny</code>, могут возвращать некоторое значение. 
...
<div class="nav"><p><a href="./13.7.php">Назад</a><a href="./">Содержание</a><a href="./13.4.php">Вперед</a></p></div></div>
<div class="socBlock">
