...
<html >
<head>
<title>C# и .NET | Сортировка в LINQ</title>
<meta charset="utf-8" />
<meta name="description" content="Сортировка в LINQ и C# по возрастанию и убыванию, методы orderby и OrderByDescending">
...
<h2>Сортировка</h2><div class="date">Последнее обновление: 13.02.2022</div>
...
<h3>Оператор orderby и метод OrderBy</h3>
<p>Для сортировки набора данных в LINQ можно применять оператор <span class="b">orderby</span>:</p>
<pre class="brush:c#;">
...
<p>Оператор <code>orderby</code> принимает критерий сортировки. В данном случае в качестве критерия выступает само число. Результат работы программы:</p>
<div class="console">
...
<p>Если числа сортируются стандартным образом, как принято в математике, то строки сортируются исходя из алфавитного порядка:</p>
<pre class="brush:c#;">
...
<p>Вместо оператора orderby можно применять метод расширения <span class="b">OrderBy()</span>:</p>
<pre class="brush:c#;">
OrderBy (Func&lt;TSource,TKey&gt; keySelector)
OrderBy (Func&lt;TSource,TKey&gt; keySelector, IComparer&lt;TKey&gt;? comparer);
</pre>
<p>Первая версия метода получает делегат, который через параметр получает элемент коллекции и который возвращает значение, применяемое для сортировки. 
Вторая версия позволяет также задать принцип сортировки через реализацию интерфейса IComparer.</p>
<p>Перепишем предыдущие два примера с помощью метода OrderBy:</p>
<pre class="brush:c#;">
int[] numbers = { 3, 12, 4, 10 };
...
<h3>Сортировка сложных объектов</h3>
<p>Возьмем посложнее пример. Допустим, надо отсортировать выборку сложных объектов. Тогда в качестве критерия мы можем указать свойство класса объекта:</p>
<pre class="brush:c#;">
...
<p>По умолчанию оператор <code>orderby</code> и метод OrderBy производят сортировку по возрастанию. С помощью ключевых слов <span class="b">ascending</span> (сортировка 
по возрастанию) и <span class="b">descending</span> (сортировка по убыванию) для оператора <code>orderby</code> можно явным образом указать направление сортировки. 
Например, отсортируем массив чисел по убыванию:</p>
<pre class="brush:c#;">
int[] numbers = { 3, 12, 4, 10 };
var orderedNumbers = from i in numbers
...
<p>Для сортировки по убыванию можно применять метод <span class="b">OrderByDescending()</span>, который работает аналогично OrderBy за исключением направления сортировки:</p>
<pre class="brush:c#;">
int[] numbers = { 3, 12, 4, 10 };
...
<h3>Множественные критерии сортировки</h3>
<p>В наборах сложных объектов иногда встает ситуация, когда надо отсортировать не по одному, а сразу по нескольким полям. Для этого в запросе LINQ все критерии 
указываются в порядке приоритета через запятую:</p>
<pre class="brush:c#;">
var people = new List&lt;Person&gt;
{
    new Person("Tom", 37),
...
<p>Для разных критериев сортировки можно установить направление:</p>
<pre class="brush:c#;">
// с помощью оператора orderby
var sortedPeople1 = from p in people
...
<p>С помощью методов расширения то же самое можно сделать через метод <span class="b">ThenBy()</span>(для сортировки по возрастанию) и <span class="b">ThenByDescending()</span> (для сортировки по убыванию):</p>
<pre class="brush:c#;">
var sortedPeople2 = people.OrderBy(p =&gt; p.Name).ThenByDescending(p=&gt;p.Age);
</pre>
<p>Результат будет аналогичен предыдущему.</p>
<h3>Переопределение критерия сортировки</h3>
<p>С помощью реализации IComparer мы можем переопределить критерии сортировки, если они нас не устраивают. Например, строки по умолчанию сортируются в алфавитном порядке. 
Но что, если мы хотим сортировать строки исходя из их длины? Решим данную задачу:</p>
<pre class="brush:c#;">
...
<p>Интерфейс IComparer типизируется типов сортируемых данных (в данном случае типом String). Для реализации этого интерфейса необходимо определить метод Compare. 
Он возвращает число: если первый параметр больше второго, то число больше 0, если меньше - то число меньше 0. Если оба параметра равны, то возвращается 0.</p>
<p>В данном случае, если параметр равен null, будем считать что длина строки равна 0. И с помощью разницы длин строк из обоих параметров определяем, какой из них больше.</p>
<p>Результат программы:</p>
<div class="console">
<pre class="consoletext">
Tom
