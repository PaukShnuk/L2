...
<html >
<head>
<title>C# и .NET | Мьютексы</title>
<meta charset="utf-8" />
<meta name="description" content="Использование мьютексов для синхронизации потоков в приложении на языке программирования C# и .NET. Класс Mutex и его методы WaitOne и ReleaseMutex">
...
<h2>Мьютексы</h2><div class="date">Последнее обновление: 07.02.2022</div>
...
<p>Еще один инструмент управления синхронизацией потоков представляет класс <span class="b">Mutex</span> или мьютекс, 
который также располагается в пространстве имен <code>System.Threading</code>.</p>
<p>Так, возьмем пример с оператором lock из одной из предыдущих тем, в котором применялась синхронизация потоков:</p>
<pre class="brush:c#;">
int x = 0;
...
<p>И перепишем данный пример, используя мьютексы:</p>
<pre class="brush:c#;">
int x = 0;
Mutex mutexObj = new();
...
<p>Сначала создаем объект мьютекса:</p>
<pre class="brush:c#;">Mutex mutexObj = new Mutex()</pre>
<p>Основную работу по синхронизации выполняют методы <span class="b">WaitOne()</span> и <span class="b">ReleaseMutex()</span>. 
Метод <code>mutexObj.WaitOne()</code> приостанавливает выполнение потока до тех пор, пока не будет получен мьютекс mutexObj.</p>
<p>Изначально мьютекс свободен, поэтому его получает один из потоков.</p>
<p>После выполнения всех действий, когда мьютекс больше не нужен, поток освобождает его с помощью метода <code>mutexObj.ReleaseMutex()</code>. А мьютекс получает 
один из ожидающих потоков.</p>
<p>Таким образом, когда выполнение дойдет до вызова <code>mutexObj.WaitOne()</code>, поток будет ожидать, пока не освободится мьютекс. И после его получения 
продолжит выполнять свою работу.</p>
