...
<html >
<head>
<title>C# и .NET | Dynamic Language Runtime</title>
<meta charset="utf-8" />
<meta name="description" content="Динамическая общеязыковая среда Dynamic Language Runtime в языке программирования C# и .NET, различие между языками со статической и динамической типизацией, определение переменных с помощью ключевого слова dynamic и динамическая типизация, использование dynamic в методах">
...
<h1>Dynamic Language Runtime</h1>
<h2>DLR в C#. Ключевое слово dynamic</h2><div class="date">Последнее обновление: 18.02.2022</div>
...
<p>Хотя C# относится к статически типизированным языкам, в последних версиях языка были добавлены некоторые динамические возможности. Так, начиная с .NET 4.0 
...
<p>Ключевым моментом использования DLR в C# является применение типов <span class="b">dynamic</span>. Это ключевое слово позволяет опустить 
...
отличается от применения ключевого слова <span class="b">var</span>. Для переменной, объявленной с помощью ключевого слова <code>var</code>, 
тип выводится во время компиляции и затем во время выполнения больше не меняется.</p>
<p>Также можно найти общее между использованием <code>dynamic</code> и типом object. Если в предыдущем примере мы заменим dynamic на object: 
<code>object x = 3;</code>, то результат будет тот же. Однако и тут есть различия. Например:</p>
<pre class="brush:c#;">
object obj = 24;
dynamic dyn = 24;
obj += 4; // так нельзя
...
<p>На строке <code>obj += 4;</code> мы увидим ошибку, так как операция += не может быть применена к типам object и int. С переменной, объявленной 
...
<p>В классе Person определено динамическое свойство Age, поэтому при задании значения этому свойству мы можем написать и <code>person.Age=22</code>, и 
<code>person.Age="twenty-two"</code>. Оба варианта будут допустимыми. А через параметр age в конструкторе этому свойству можно передать любое значение.</p>
<p>Также есть метод <code>GetSalary</code>, который возвращает значение dynamic. Например, в зависимости от параметра мы можем вернуть или строковое представление 
...
<div class="nav"><p><a href="./14.4.php">Назад</a><a href="./">Содержание</a><a href="./9.2.php">Вперед</a></p></div></div>
<div class="socBlock">
