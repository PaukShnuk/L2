...
<html >
<head>
<title>C# и .NET | Создание потоков. Делегат ThreadStart</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и запуск новых потоков в языке программирования C# и .NET, метод Start класса Thread, применение делегата ThreadStart">
...
<h2>Создание потоков. Делегат ThreadStart</h2><div class="date">Последнее обновление: 06.02.2022</div>
...
<p>Язык C# позволяет запускать и выполнять в рамках приложения несколько потоков, которые будут выполняться одновременно.</p>
<p>Для создания потока применяется один из конструкторов класса <span class="b">Thread</span>:</p>
<ul>
<li><p><span class="b">Thread(ThreadStart)</span>: в качестве параметра принимает объект делегата ThreadStart, который представляет выполняемое в потоке действие</p></li>
<li><p><span class="b">Thread(ThreadStart, Int32)</span>: в дополнение к делегату ThreadStart принимает числовое значение, которое устанавливает размер стека, выделяемого 
под данный поток</p></li>
<li><p><span class="b">Thread(ParameterizedThreadStart)</span>: в качестве параметра принимает объект делегата ParameterizedThreadStart, который представляет выполняемое в потоке действие</p></li>
<li><p><span class="b">Thread(ParameterizedThreadStart, Int32)</span>: вместе с делегатом ParameterizedThreadStart принимает числовое значение, которое устанавливает размер стека для 
...
<pre class="brush:c#;">public delegate void ThreadStart();</pre>
...
<p>Для запуска нового потока применяется метод <span class="b">Start</span> класса Thread:</p>
<pre class="brush:c#;">
