...
<html >
<head>
<title>C# и .NET | Конструкция switch</title>
<meta charset="utf-8" />
<meta name="description" content="Конструкция switch в языке программирования C# и .NET, оператор case, сопоставление со значениями, блок default, получение результата из конструкции switch">
...
<h2>Конструкция switch</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Конструкция <span class="b">switch/case</span> оценивает некоторое выражение и сравнивает его значение с набором значений. 
...
сравнивается со значениями, помещенными после оператора <span class="b">сase</span>. И если совпадение будет найдено, то будет 
выполняться определенный блок <span class="b">сase</span>.</p>
<p>В конце каждого блока сase должен ставиться один из операторов перехода: 
<span class="b">break</span>, <span class="b">goto case</span>, <span class="b">return</span> или <span class="b">throw</span>. 
Как правило, используется оператор <span class="b">break</span>. При его применении другие блоки <code>case</code> выполняться не будут.</p>
<p>Например:</p>
<pre class="brush:c#;">
...
<p>В данном случае конструкция <code>switch</code> последовательно сравнивает значение переменной <code>name</code> с набором значений, 
которые указаны после операторов <code>case</code>. Поскольку здесь значение переменной <code>name</code> - строка "Tom", то будет выполняться блок</p>
<pre class="brush:c#;">
case "Tom":
	Console.WriteLine("Ваше имя - Tom");
	break;
...
<div class="console"><pre class="consoletext">Ваше имя - Tom</pre></div>
<p>Если значение переменной <code>name</code> не совпадает ни с каким значением после операторов <code>case</code>, то ни один из блоков case не выполняется. 
Однако если даже в этом случае нам все равно надо выполнить какие-нибудь действия, то мы можем добавить в конструкцию <span class="b">switch</span> необязательный 
блок <span class="b">default</span>. Например:</p>
<pre class="brush:c#;">
string name = "Alex";
...
<p>Конструкция <span class="b">switch</span> позволяет возвращать некоторое значение. Для возвращения значения в блоках <span class="b">case</span> может применятся 
оператор <span class="b">return</span>. Например, определим следующий метод:</p>
<pre class="brush:c#;">
...
<p>В метод <code>DoOperation()</code> передается числовой код операции и два операнда. В зависимости от кода операции над операндами 
...
<p>Хотя конструкция <span class="b">switch</span> в примере выше прекрасно работает, тем не менее мы ее можем сократить и получить результат неосредственно из конструкции 
<span class="b">switch</span>:</p>
<pre class="brush:c#;">
int DoOperation(int op, int a, int b)
{
...
<p>Теперь не требуется оператор <span class="b">case</span>, а после сравниваемого значения ставится оператор стрелка <span class="b">=&gt;</span>. 
Значение справа от стрелки выступает в качестве возвращаемоего значения. Кроме того, вместо оператора <span class="b">default</span> 
...
<p>Обращаю внимание, что данное упрощение касается лишь таких конструкций <span class="b">switch</span>, которые <span class="b">возвращают</span> некоторые значения, как в примере выше.</p>
...
<div class="nav"><p><a href="./2.20.php">Назад</a><a href="./">Содержание</a><a href="./2.12.php">Вперед</a></p></div></div>
<div class="socBlock">
