...
<html >
<head>
<title>C# и .NET | Преобразование типов и класс Convert</title>
<meta charset="utf-8" />
<meta name="description" content="Преобразование типов с помощью методов Parse и TryParse в языке программирования C# и .NET, класс Convert">
...
<h2>Преобразование типов и класс Convert</h2><div class="date">Последнее обновление: 06.01.2022</div>
...
<h3>Методы Parse и TryParse</h3>
<p>Все примитивные типы имеют два метода, которые позволяют преобразовать строку к данному типу. Это методы <span class="b">Parse()</span> и <span class="b">TryParse()</span>.</p>
<p>Метод <span class="b">Parse()</span> в качестве параметра принимает строку и возвращает объект текущего типа. Например:</p>
<pre class="brush:c#;">
int a = int.Parse("10");
...
<p>Стоит отметить, что парсинг дробных чисел зависит от настроек текущей культуры. В частности, для получения числа double я передаю строку "23,56" с запятой в качестве разделителя. 
Если бы я передал точку вместо запятой, то приложение выдало ошибку выполнения. На компьютерах с другой локалью, наоборот, использование запятой вместо точки выдало бы ошибку.</p>
<p>Чтобы не зависеть от культурных различий мы можем установить четкий формат с помощью класса <span class="b">NumberFormatInfo</span> и его свойства 
<span class="b">NumberDecimalSeparator</span>:</p>
<pre class="brush:c#;">
...
<p>В данном случае в качестве разделителя устанавливается точка. Однако тем не менее потенциально при использовании метода Parse мы можем столкнуться с ошибкой, например, 
при передачи алфавитных символов вместо числовых. И в этом случае более удачным выбором будет применение метода <span class="b">TryParse()</span>. 
Он пытается преобразовать строку к типу и, если преобразование прошло успешно, то возвращает <span class="b">true</span>. Иначе возвращается false:</p>
<pre class="brush:c#;">
...
<p>Если преобразование пройдет неудачно, то исключения никакого не будет выброшено, просто метод TryParse возвратит false, а переменная number будет содержать значение по умолчанию.</p>
<h3>Convert</h3>
<p>Класс <span class="b">Convert</span> представляет еще один способ для преобразования значений. Для этого в нем определены следующие статические методы:</p>
<ul>
<li><p><span class="b">ToBoolean(value)</span></p></li>
<li><p><span class="b">ToByte(value)</span></p></li>
<li><p><span class="b">ToChar(value)</span></p></li>
<li><p><span class="b">ToDateTime(value)</span></p></li>
<li><p><span class="b">ToDecimal(value)</span></p></li>
<li><p><span class="b">ToDouble(value)</span></p></li>
<li><p><span class="b">ToInt16(value)</span></p></li>
<li><p><span class="b">ToInt32(value)</span></p></li>
<li><p><span class="b">ToInt64(value)</span></p></li>
<li><p><span class="b">ToSByte(value)</span></p></li>
<li><p><span class="b">ToSingle(value)</span></p></li>
<li><p><span class="b">ToUInt16(value)</span></p></li>
<li><p><span class="b">ToUInt32(value)</span></p></li>
<li><p><span class="b">ToUInt64(value)</span></p></li>
</ul>
<p>В качестве параметра в эти методы может передаваться значение различных примитивных типов, необязательно строки:</p>
<pre class="brush:c#;">
...
<p>Однако опять же, как и в случае с методом Parse, если методу не удастся преобразовать значение к нужному типу, то он выбрасывает исключение FormatException.</p>
...
<div class="nav"><p><a href="./20.2.php">Назад</a><a href="./">Содержание</a><a href="./20.5.php">Вперед</a></p></div></div>
<div class="socBlock">
