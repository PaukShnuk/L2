...
<html >
<head>
<title>C# и .NET | Итераторы и оператор yield</title>
<meta charset="utf-8" />
<meta name="description" content="Использование ключевого слова yield в C#, создание итераторов для перебора коллекций, именнованные итераторы, оператор yield break">
...
<h2>Итераторы и оператор yield</h2><div class="date">Последнее обновление: 19.01.2022</div>
...
<span class="b">Итератор</span> по сути представляет блок кода, который использует оператор <span class="b">yield</span> для перебора 
набора значений. Данный блок кода может представлять тело метода, оператора или блок get в свойствах.</p>
<p>Итератор использует две специальных инструкции:</p>
<ul>
<li><p><span class="b">yield return</span>: определяет возвращаемый элемент</p></li>
<li><p><span class="b">yield break</span>: указывает, что последовательность больше не имеет элементов</p></li>
</ul>
...
<p>В классе Numbers метод <span class="b">GetEnumerator()</span> фактически представляет итератор. С помощью оператора 
<span class="b">yield return</span> возвращается некоторое значение (в данном случае квадрат числа).</p>
<p>В программе с помощью цикла foreach мы можем перебрать объект Numbers как обычную коллекцию. При получении каждого элемента в цикле foreach 
будет срабатывать оператор yield return, который будет возвращать один элемент и запоминать текущую позицию.</p>
<p>Благодаря итераторам мы можем пойти дальше и легко реализовать перебор числа в цикле foreach:</p>
<pre class="brush:c#;">
...
<p>В данном случае итератор реализован как метод расширения для типа int или System.Int32. В методе итератора фактически 
возвращаем все целочисленные значения от 0 до текущего числа. Консольный вывод:</p>
<div class="console">
...
<p>Другой пример: пусть у нас есть коллекция Company, которая представляет компанию и которая хранит в массиве personnel штат сотрудников - объектов Person. Используем оператор yield 
для перебора этой коллекции:</p>
<pre class="brush:c#;">
class Person
{
...
<p>Метод <code>GetEnumerator()</code> представляет итератор. И когда мы будем осуществлять перебор в объекте Company в цикле foreach, то будет идти обращение к вызову <code>yield return personnel[i];</code>. При 
обращении к оператору <code>yield return</code> будет сохраняться текущее местоположение. И когда метод foreach перейдет к следующей итерации 
для получения нового объекта, итератор начнет выполнения с этого местоположения.</p>
<p>Ну и в основной программе в цикле foreach выполняется собственно перебор, благодаря реализации итератора:</p>
<pre class="brush:c#;">
...
<p>Хотя при реализации итератора в методе <code>GetEnumerator()</code> применялся перебор массива в цикле for, но это необязательно делать. 
Мы можем просто определить несколько вызовов оператора <code>yield return</code>:</p>
<pre class="brush:c#;">
public IEnumerator&lt;Person&gt; GetEnumerator()
{
...
<p>В этом случае при каждом вызове оператора <code>yield return</code> итератор также будет запоминать текущее местоположение и при последующих вызовах начинать с него.</p>
<h3>Именованный итератор</h3>
<p>Выше для создания итератора мы использовали метод <code>GetEnumerator</code>. Но оператор <code>yield</code> можно использовать 
внутри любого метода, только такой метод должен возвращать объект интерфейса <code>IEnumerable</code>. Подобные методы еще называют 
<span class="b">именованными итераторами</span>.</p>
<p>Создадим такой именованный итератор в классе Company и используем его:</p>
<pre class="brush:c#;">
class Person
{
    public string Name { get; }
...
<p>Определенный здесь итератор - метод <code>IEnumerable GetPersonnel(int max)</code> в качестве параметра принимает количество выводимых объектов. 
...
оператор <span class="b">yield break</span>. Этот оператор прерывает выполнение итератора.</p>
<p>Применение итератора:</p>
<pre class="brush:c#;">
var people = new Person[]
{
    new Person("Tom"), 
...
<p>Вызов <code>microsoft.GetPersonnel(3)</code> будет возвращать набор из не более чем 5 объектов Person. Но так как у нас всего три таких объекта, 
то в методе <code>GetPersonnel</code> после трех операций сработает оператор <code>yield break</code>.</p>
...
<div class="nav"><p><a href="./4.11.php">Назад</a><a href="./">Содержание</a><a href="./7.1.php">Вперед</a></p></div></div>
<div class="socBlock">
