...
<html >
<head>
<title>C# и .NET | Замыкания</title>
<meta charset="utf-8" />
<meta name="description" content="Замыкания в языке программирования C# и .NET, определение замыканий с помощью локальных функций и лямбда-выражений, лексическое окружение функции">
...
<h2>Замыкания</h2><div class="date">Последнее обновление: 19.11.2021</div>
...
<p><span class="b">Замыкание</span> (closure) представляет объект функции, который запоминает свое лексическое окружение даже в том случае, 
когда она выполняется вне своей области видимости.</p>
...
<p>В языке C# реализовать замыкания можно разными способами - с помощью локальных функций и лямбда-выражений.</p>
<p>Рассмотрим создание замыканий через локальные функции:</p>
<pre class="brush:c#;">
...
<p>Здесь метод <code>Outer</code> в качестве возвращаемого типа имеет тип <code>Action</code>, то есть метод возвратить функцию, которая не 
принимает параметров и имеет тип void.</p>
<pre class="brush:c#;">Action Outer()</pre>
<p>Внутри метода Outer определена переменная x - это и есть лексическое окружение для внутренней функции:</p>
<pre class="brush:c#;">int x = 5;</pre>
<p>Также внутри метода Outer определена внутренняя функция - локальная функция Inner, которая обращается к своему лексическому окружению - переменной x - увеличивает ее значение на единицу и выводит на консоль:</p>
<pre class="brush:c#;">
void Inner()
{
	x++;
...
<p>Эта локальная функция возвращается методом Outer:</p>
<pre class="brush:c#;">return Inner;</pre>
<p>В программе вызываем метод Outer и получаем в переменную <code>fn</code> локальную функцию Inner:</p>
<pre class="brush:c#;">var fn = Outer();</pre>
<p>Переменная <code>fn</code> и представляет собой замыкание, то есть объединяет две вещи: функцию и 
...
окружение и может к нему обращаться и изменять, что мы увидим по консольному выводу:</p>
<pre class="brush:c#;">
fn();   // 6
fn();   // 7
fn();   // 8
</pre>
<h3>Реализация с помощью лямбда-выражений</h3>
<p>С помощью лямбд можно сократить определение замыкания:</p>
<pre class="brush:c#;">
var outerFn = () =&gt;
{
    int x = 10;
...
<h3>Применение параметров</h3>
<p>Кроме внешних переменных к лексическому окружению также относятся параметры окружающего метода. Рассмотрим использование параметров:</p>
<pre class="brush:c#;">
var fn = Multiply(5);  
...
<p>Здесь внешняя функция - метод Multiply возвращает функцию, которая принимает число int и возвращает число int. Для этого определен делегат Operation, 
который будет представлять возвращаемый тип:</p>
<pre class="brush:c#;">delegate int Operation(int n);</pre>
<p>Хотя также можно было бы использовать встроенный делегат <code>Func&lt;int, int&gt;</code>.</p>
<p>Вызов метода <code>Multiply()</code> возвращает локальную функцию, которая соответствует сигнатуре делегата Operation:</p>
<pre class="brush:c#;">
int Inner(int m)
{
	return n * m;
...
<p>Эта функция запоминает окружение, в котором она была создана, в частности, значение параметра n. Кроме того, сама принимает параметр и возвращает произведение 
параметров n и m.</p>
<p>В итоге при вызове метода Multiply определяется переменная fn, которая получает локальную функцию Inner и ее лексическое окружение - значение параметра n:</p>
<pre class="brush:c#;">var fn = Multiply(5);</pre>
<p>В данном случае параметр n равен 5.</p>
<p>При вызове локальной функции, например, в случае:</p>
<pre class="brush:c#;">Console.WriteLine(fn(6));   // 30</pre>
<p>Число 6 передается для параметра m локальной функции, которая возвращает произведение n и m, то есть 5 * 6 = 30.</p>
<p>Также можно было бы сократить весь этот код с помощью лямбд:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./3.33.php">Назад</a><a href="./">Содержание</a><a href="./3.9.php">Вперед</a></p></div></div>
<div class="socBlock">
