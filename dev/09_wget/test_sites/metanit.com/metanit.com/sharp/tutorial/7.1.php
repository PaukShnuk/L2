...
<html >
<head>
<title>C# и .NET | Строки</title>
<meta charset="utf-8" />
<meta name="description" content="Работа со строками в C#, класс String, доступ к символам строки, основные методы для работы со строками">
...
<h1>Работа со строками</h1><h2>Строки и класс String</h2><div class="date">Последнее обновление: 05.01.2022</div>
...
<p>Довольно большое количество задач, которые могут встретиться при разработке приложений, так или 
...
из текста и т.д. Поэтому в этом плане работе со строками уделяется особое внимание.</p>
<p>В языке C# строковые значения представляет тип <span class="b">string</span>, а вся функциональность работы с данным типом сосредоточена в классе 
<span class="b">System.String</span>. Собственно <code>string</code> является псевдонимом для класса String. Объекты этого класса представляют текст как последовательность символов Unicode. Максимальный размер объекта String может составлять в памяти 2 ГБ, или около 1 миллиарда символов.</p>
<h3>Создание строк</h3>
<p>Создавать строки можно, как используя переменную типа string и присваивая ей значение, так и применяя один из конструкторов класса String:</p>
<pre class="brush:c#;">
string s1 = "hello";
...
<p>Конструктор String имеет различное число версий. Так, вызов конструктора</p>
<pre class="brush:c#;">new String('a', 6)</pre>
<p>6 раз повторит объект из первого параметра, то есть фактически создаст строку "aaaaaa".</p>
<p>Еще один конструктор принимает массив символов, из которых создается строка</p>
<pre class="brush:c#;">string s3 = new String(new char[] { 'w', 'o', 'r', 'l', 'd' });</pre>
<p>Третий использованный выше в примере конструктор позволяет создать строку из части массива символов. Второй параметр передает начальный индекс, с которого извлкаются символы, а третий параметр 
указывает на количество символов:</p>
<pre class="brush:c#;">string s4 = new String(new char[] { 'w', 'o', 'r', 'l', 'd' }, 1, 3); // orl</pre>
<h3>Строка как набор символов</h3>
<p>Так как строка хранит коллекцию символов, в ней определен индексатор для доступа к этим символам:</p>
<pre class="brush:c#;">
public char this[int index] {get;}
</pre>
<p>Применяя индексатор, мы можем обратиться к строке как к массиву символов и получить по индексу любой из ее символов:</p>
<pre class="brush:c#;">
...
<p>Используя свойство <span class="b">Length</span>, как и в обычном массиве, можно получить длину строки.</p>
<h3>Перебор строк</h3>
<p>Класс String реализует интерфейс IEnumerable, благодаря чему строку можно перебрать в цикле foreach как набор объектов char. Также можно с помощью 
других типов циклов перебрать строку, применяя обращение к символам по индексу:</p>
<pre class="brush:c#;">
string message = "hello";
...
<p>В отличие от других классов строки сравниваются по значению их символов, а не по ссылкам:</p>
<pre class="brush:c#;">
...
<p>Основная функциональность класса String раскрывается через его методы, среди которых можно выделить следующие:</p>
<ul>
<li><p><span class="b">Compare</span>: сравнивает две строки с учетом текущей культуры (локали) пользователя</p></li>
<li><p><span class="b">CompareOrdinal</span>: сравнивает две строки без учета локали</p></li>
<li><p><span class="b">Contains</span>: определяет, содержится ли подстрока в строке</p></li>
<li><p><span class="b">Concat</span>: соединяет строки</p></li>
<li><p><span class="b">CopyTo</span>: копирует часть строки, начиная с определенного индекса в массив</p></li>
<li><p><span class="b">EndsWith</span>: определяет, совпадает ли конец строки с подстрокой</p></li>
<li><p><span class="b">Format</span>: форматирует строку</p></li>
<li><p><span class="b">IndexOf</span>: находит индекс первого вхождения символа или подстроки в строке</p></li>
<li><p><span class="b">Insert</span>: вставляет в строку подстроку</p></li>
<li><p><span class="b">Join</span>: соединяет элементы массива строк</p></li>
<li><p><span class="b">LastIndexOf</span>: находит индекс последнего вхождения символа или подстроки в строке</p></li>
<li><p><span class="b">Replace</span>: замещает в строке символ или подстроку другим символом или подстрокой</p></li>
<li><p><span class="b">Split</span>: разделяет одну строку на массив строк</p></li>
<li><p><span class="b">Substring</span>: извлекает из строки подстроку, начиная с указанной позиции</p></li>
<li><p><span class="b">ToLower</span>: переводит все символы строки в нижний регистр</p></li>
<li><p><span class="b">ToUpper</span>: переводит все символы строки в верхний регистр</p></li>
<li><p><span class="b">Trim</span>: удаляет начальные и конечные пробелы из строки</p></li>
</ul>
<p>Разберем работу этих методов.</p>
...
<div class="nav"><p><a href="./4.12.php">Назад</a><a href="./">Содержание</a><a href="./7.2.php">Вперед</a></p></div></div>
<div class="socBlock">
