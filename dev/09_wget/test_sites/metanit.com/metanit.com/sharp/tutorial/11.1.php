...
<html >
<head>
<title>C# и .NET | Введение в многопоточность. Класс Thread</title>
<meta charset="utf-8" />
<meta name="description" content="Создание многопоточных программ в языке программирования C#, пространство имен System.Threading, класс Thread, его методы и свойства, состояние и приоритет потоков">
...
<h1>Многопоточность</h1><h2>Введение в многопоточность. Класс Thread</h2><div class="date">Последнее обновление: 06.02.2022</div>
...
<p>Одним из ключевых аспектов в современном программировании является <span class="b">многопоточность</span>. Ключевым понятием при работе с многоопоточностью является поток. 
...
<p>Основной функционал для использования потоков в приложении сосредоточен в пространстве имен <span class="b">System.Threading</span>. В нем определен 
класс, представляющий отдельный поток - класс <span class="b">Thread</span>.</p>
...
<ul>
<li><p><span class="b">ExecutionContext</span>: позволяет получить контекст, в котором выполняется поток</p></li>
<li><p><span class="b">IsAlive</span>: указывает, работает ли поток в текущий момент</p></li>
<li><p><span class="b">IsBackground</span>: указывает, является ли поток фоновым</p></li>
<li><p><span class="b">Name</span>: содержит имя потока</p></li>
<li><p><span class="b">ManagedThreadId</span>: возвращает числовой идентификатор текущего потока</p></li>
<li><p><span class="b">Priority</span>: хранит приоритет потока - значение перечисления <span class="b">ThreadPriority</span>:</p>
<ul>
<li><p><span class="b">Lowest</span></p></li>
<li><p><span class="b">BelowNormal</span></p></li>
<li><p><span class="b">Normal</span></p></li>
<li><p><span class="b">AboveNormal</span></p></li>
<li><p><span class="b">Highest</span></p></li>
</ul>
...
<li><p><span class="b">ThreadState</span> возвращает состояние потока - одно из значений перечисления <span class="b">ThreadState</span>:</p>
<ul>
<li><p><span class="b">Aborted</span>: поток остановлен, но пока еще окончательно не завершен</p></li>
<li><p><span class="b">AbortRequested</span>: для потока вызван метод Abort, но остановка потока еще не произошла</p></li>
<li><p><span class="b">Background</span>: поток выполняется в фоновом режиме</p></li>
<li><p><span class="b">Running</span>: поток запущен и работает (не приостановлен)</p></li>
<li><p><span class="b">Stopped</span>: поток завершен</p></li>
<li><p><span class="b">StopRequested</span>: поток получил запрос на остановку</p></li>
<li><p><span class="b">Suspended</span>: поток приостановлен</p></li>
<li><p><span class="b">SuspendRequested</span>: поток получил запрос на приостановку</p></li>
<li><p><span class="b">Unstarted</span>: поток еще не был запущен</p></li>
<li><p><span class="b">WaitSleepJoin</span>: поток заблокирован в результате действия методов Sleep или Join</p></li>
</ul>
...
Start его статус имеет значение <code>Unstarted</code>. Запустив поток, мы изменим его статус на <code>Running</code>. 
Вызвав метод Sleep, статус изменится на <code>WaitSleepJoin</code>.</p>
</li>
</ul>
<p>Кроме того статическое свойство <span class="b">CurrentThread</span> класса Thread позволяет получить текущий поток</p>
...
<ul>
<li><p>Статический метод <span class="b">GetDomain</span> возвращает ссылку на домен приложения </p></li>
<li><p>Статический метод <span class="b">GetDomainID</span> возвращает id домена приложения, в котором выполняется текущий поток </p></li>
<li><p>Статический метод <span class="b">Sleep</span> останавливает поток на определенное количество миллисекунд</p></li>
<li><p>Метод <span class="b">Interrupt</span> прерывает поток, который находится в состоянии WaitSleepJoin</p></li>
<li><p>Метод <span class="b">Join</span> блокирует выполнение вызвавшего его потока до тех пор, пока не завершится поток, для которого был вызван данный метод</p></li>
<li><p>Метод <span class="b">Start</span> запускает поток</p></li>
</ul>
...
<div class="nav"><p><a href="./2.32.php">Назад</a><a href="./">Содержание</a><a href="./11.2.php">Вперед</a></p></div></div>
<div class="socBlock">
