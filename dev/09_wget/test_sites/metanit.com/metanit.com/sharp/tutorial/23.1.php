...
<html >
<head>
<title>C# и .NET | Нововведения в C# 11</title>
<meta charset="utf-8" />
<meta name="description" content="Новвоведения в языке программирования C# 11, паттерны списков list pattern, slice pattern, многострочный текст, обобщенные атрибуты, оператор !! и проверка параметра на null">
...
<h2>Нововведения в C# 11</h2><div class="date">Последнее обновление: 22.04.2022</div>
...
<h3>Паттерны списков</h3>
<p>Паттерны списков (list pattern) позволяют сопоставлять выражения со списками и массивами. Можно сопоставлять для списка-паттерна можно 
установить все элементы массивы:</p>
<pre class="brush:c#;">
[2, 3, 4, 5] // этот паттерн соответствует любому массиву/списку, 
            // который равен [2, 3, 4, 5]
</pre>
<p>Также с помощью символа подчеркивания <span class="b">_</span> можно задать произвольный элемент, значение которого не имеет значения:</p>
<pre class="brush:c#;">
[_, 5] // этот паттерн соответствует любому массиву/списку из двух элементов
        // где второй элемент равен 5 (например, [1, 5] или [25, 5])
</pre>
<p>И, кроме того, с помощью slice-паттерна <span class="b">..</span> можно задать произвольное количество элементов:</p>
<pre class="brush:c#;">
...
<h3>Обобщения в атрибутах</h3>
<p>В C# 11 можно определять обобщенные классы атрибутов:</p>
<pre class="brush:c#;">
class C
{
    [Reader&lt;int&gt;]
...
<h3>Многострочные строки</h3>
<p>С помощью трех пар двойных кавычек можно оформить многострочный текст, в том числе с применением интерполяции:</p>
<pre class="brush:c#;">
Print();
PrintValue("hello");
...
<div class="nav"><p><a href="./22.4.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
...
$(function() { 
  ...
  $("#toggle-theme").on("click", function() {
	...
	$("li:contains('Глава 27.')").removeClass('lastExpandable').addClass('lastCollapsable').children('ul').css({"display":"block"});
	$("li:contains('Глава 27.')").children('div').removeClass('lastExpandable-hitarea').addClass('lastCollapsable-hitarea');
