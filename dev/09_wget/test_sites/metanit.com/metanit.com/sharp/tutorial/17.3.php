...
<html >
<head>
<title>C# и .NET | Обработка ошибок и отмена параллельных операций в Parallel LINQ</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка ошибок и отмена параллельных операций в Parallel LINQ  в языке программирования C# и .NET. Исключение AggregateException">
...
<h2>Обработка ошибок и отмена операции</h2><div class="date">Последнее обновление: 15.02.2022</div>
...
<p>При выполнении параллельных операций также могут возникать ошибки, обработка которых имеет свои особенности. При параллельной обработке коллекция 
разделяется а части, и каждая часть обрабатывается в отдельном потоке. Однако если возникнет ошибка в одном из потоков, то система прерывает выполнение всех потоков.</p>
<p>При генерации исключений все они агрегируются в одном исключении типа <span class="b">AggregateException</span></p>
<p>Например, пусть в метод факториала передается массив объектов, который содержит не только числа, но и строки:</p>
<pre class="brush:c#;">
...
<p>Запустим проект <span class="b">без отладки</span>. И так как массив содержит строку, то попытка приведения закончится неудачей, и на консоль будет выведено сообщение об ошибке. 
При запуске приложения в Visual Studio в режиме отладки выполнение остановится на строке преобразования. 
А после продолжения также сработает перехват исключения в блоке catch, и на консоль будет выведено сообщение об ошибке.</p>
<h3>Прерывание параллельной операции</h3>
<p>Вполне вероятно, что нам может потребоваться прекратить операцию до ее завершения. В этом случае мы можем использовать метод <span class="b">WithCancellation()</span>, 
которому в качестве параметра передается токен <span class="b">CancellationToken</span>:</p>
<pre class="brush:c#;">
...
<p>В параллельной запущенной задаче вызывается метод <code>cts.Cancel()</code>, что приводит к завершению операции и генерации исключения <span class="b">OperationCanceledException</span>:</p>
<div class="console">
...
<p>При этом также имеет смысл обрабатывать исключение AggregateException, так как если параллельно возникает еще одно исключение, то это исключение, 
а также OperationCanceledException помещаются внутрь одного объекта AggregateException.</p>
...
<div class="nav"><p><a href="./17.2.php">Назад</a><a href="./">Содержание</a><a href="./14.1.php">Вперед</a></p></div></div>
<div class="socBlock">
