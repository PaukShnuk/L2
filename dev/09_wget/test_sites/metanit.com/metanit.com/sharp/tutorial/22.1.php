...
<html >
<head>
<title>C# и .NET | Валидация модели</title>
<meta charset="utf-8" />
<meta name="description" content="Валидация модели в C# и .NET Core с помощью System.ComponentModel.DataAnnotations, проверка вводимых данных на корректность, метод Validator.TryValidateObject">
...
<h1>Валидация модели</h1><h2>Основы валидации модели</h2><div class="date">Последнее обновление: 22.02.2022</div>
...
<p>Большую роль в приложении играет валидация модели или проверка вводимых данных на корректность. Например, у нас есть класс пользователя, 
...
например, миллион лет.</p>
<p>Например, пусть у нас есть проект консольного приложения, в котором есть клас User:</p>
<pre class="brush:c#;">
public class User
{
...
<p>В программе мы можем проверять вводимые данные с помощью условных конструкций:</p>
<pre class="brush:c#;">
CreateUser("Tom", 37);
CreateUser("b", -4);
CreateUser("", 130);
...
<p>Здесь предполагается, что имя должно иметь больше 1 символа, а возраст должен быть в диапазоне от 1 до 100. Однако в классе может быть гораздо больше 
...
в виде атрибутов из пространства имен <span class="b">System.ComponentModel.DataAnnotations</span>.</p>
<p>Итак, изменим касс User следующим образом:</p>
<pre class="brush:c#;">
...
<p>Все правила валидации модели в System.ComponentModel.DataAnnotations определяются в виде атрибутов. В данном случае используются три атрибута: 
классы RequiredAttribute, StringLengthAttribute и RangeAttribute. В коде необязательно использовать суффикс <code>Attribute</code>, поэтому он, 
...
а атрибут Range устанавливает диапазон приемлемых значений.</p>
<p>Теперь изменим код программы:</p>
<pre class="brush:c#;">
using System.ComponentModel.DataAnnotations;
...
<p>Здесь определен метод CreateUser, который принимает два значения и с их помощью создает объект User. В этом методе используются классы ValidationResult, Validator и ValidationContext, которые предоставляются пространством имен System.ComponentModel.DataAnnotations 
и которые управляют валидацией.</p>
<p>Вначале мы создаем контекст валидации - объект <span class="b">ValidationContext</span>. В качестве первого параметра в конструктор этого класса 
передается валидируемый объект, то есть в данном случае объект User.</p>
<pre class="brush:c#;">var context = new ValidationContext(user);</pre>
<p>Собственно валидацию производит класс <span class="b">Validator</span> и его метод <code>TryValidateObject()</code>. В этот метод передается 
валидируемый объект (в данном случае объект user), контекст валидации, список объектов <span class="b">ValidationResult</span> и булевый параметр, 
который указывает, надо ли валидировать все свойства.</p>
<pre class="brush:c#;">
var results = new List&lt;ValidationResult&gt;();
if (!Validator.TryValidateObject(user, context, results, true))
{
...
<p>Если метод <code>Validator.TryValidateObject()</code> возвращает <code>false</code>, значит объект не проходит валидацию. Если модель не проходит валидацию, 
...
имеет два ключевых свойства: <code>MemberNames</code> - список свойств, для которых возникла ошибка, и <code>ErrorMessage</code> - собственно сообщение об ошибке.</p>
<p>Для тестирования три раза вызываем метод CreateUser, передавая в него сначала корректные, а потом некорректные данные:</p>
<pre class="brush:c#;">
CreateUser("Tom", 37);
CreateUser("b", -4);
CreateUser("", 130);
</pre>
<p>И мы получим следующий консольный вывод:</p>
<div class="console">
...
<p>В первом вызове метода CreateUser передаются корректные данные, поэтому никаких ошибок при валидации не возникнет.</p>
<p>А во втором вызове CreateUser валидация завершится неудачно, так как свойствам User переданы некорректные значения. Например, свойству Name передается 
значение "b", что не соответствует правилам атрибута <code>[StringLength(50, MinimumLength = 3)]</code>. Также значение свойства Age - -4 не соответствует 
правилам атрибута <code>[Range(1, 100)]</code>. Соответственно консоль отобразит ошибки для свойств Name и Age.</p>

<p>В третьем вызове CreateUser валидация также завершится неудачно - значение свойства Age - 130 также не соответствует 
правилам атрибута <code>[Range(1, 100)]</code>. Но, кроме того, свойству Name передается пустая строка - значение, которое не соответствует правилу атрибута 
<code>[Required]</code>. Данный атрибут требует обязательного наличия значения.</p>
<p>Если применяются типы record, то атрибуты валидации можно указать непосредственно перед определением свойства:</p>
<pre class="brush:c#;">
using System.ComponentModel.DataAnnotations;
...
<p>В этом случае перед названием атрибута указывается оператор <span class="b">property: </span></p>
<p>Таким образом, вместо кучи условных конструкций для проверки значений свойств модели мы можем использовать один метод <code>Validator.TryValidateObject()</code>, 
а все правила валидации определить в виде атрибутов.</p>
...
<div class="nav"><p><a href="./18.3.php">Назад</a><a href="./">Содержание</a><a href="./22.2.php">Вперед</a></p></div></div>
<div class="socBlock">
