...
<html >
<head>
<title>C# и .NET | Получение части коллекции в LINQ</title>
<meta charset="utf-8" />
<meta name="description" content="Получение части коллекции в LINQ в языке программирования C# и .NET, методы Skip и Take, TakeWhile и SkipWhile, создание постраничного вывода в C# и .NET">
...
<h2>Методы Skip и Take</h2><div class="date">Последнее обновление: 14.02.2022</div>
...
<p>Ряд методов в LINQ позволяют получить часть коллекции, в частности, такие методы как Skip, Take, SkipWhile, TakeWhile.</p>
<h3>Skip</h3>
<p>Метод <span class="b">Skip()</span> пропускает определенное количество элементов. Количество пропускаемых элементов передается в качестве параметра в метод:</p>
<pre class="brush:c#;">
...
<p>В данном случае пропускаем первые два элемента.  Консольный вывод:</p>
<div class="console">
...
<p>Если необходимо пропустить определенное количество элементов с конца коллекции, то применяется метод <span class="b">SkipLast()</span>:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Sam", "Bob", "Mike", "Kate" };
...
<p>В данном случае пропускаем последние два элемента.  Консольный вывод:</p>
<div class="console">
<pre class="consoletext">
Tom
Sam
...
<h3>SkipWhile</h3>
<p>Метод <span class="b">SkipWhile()</span> пропускает цепочку элементов, начиная с первого элемента, пока они удовлетворяют определенному условию:</p>
<pre class="brush:c#;">SkipWhile(Func&lt;TSource, bool&gt; predicate);</pre>
<p>В метод передается делегат, который представляет условие, он получает каждый элемент коллекции и возвращает значение true, если элемент соответствует условию. Например:</p>
<pre class="brush:c#;">
...
<p>Здесь метод SkipWhile пропускает элементы, длина которых равна 3 символам. Первые два элемента массива people ("Tom", "Sam") соответствуют этому условию 
...
не будет пропущен и будет включен в выходную коллекцию:</p>
<div class="console">
<pre class="consoletext">
Mike
Kate
...
<p>Если в массиве первый элемент имел бы длину больше или меньше 3 символов, то цепочка пропускаемых элементов прервалась бы уже на первом элементе, и поэтому 
метод SkipWhile возвратил бы все элементы массива.</p>
<h3>Take</h3>
<p>Метод <span class="b">Take()</span> извлекает определенное число элементов. Количество извлекаемых элементов передается в метод в качестве параметра. 
Например, извлечем три первых элемента:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Sam", "Mike", "Kate", "Bob" };
...
<p>Метод <span class="b">TakeLast()</span> извлекает определенное количество элементов с конце коллекции:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Sam", "Mike", "Kate", "Bob" };
...
<h3>TakeWhile</h3>
<p>Метод <span class="b">TakeWhile()</span> выбирает цепочку элементов, начиная с первого элемента, пока они удовлетворяют определенному условию:</p>
<pre class="brush:c#;">TakeWhile(Func&lt;TSource, bool&gt; predicate);</pre>
<p>В метод передается делегат, который представляет условие, он получает каждый элемент коллекции и возвращает значение true, если элемент соответствует условию. Например:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Sam", "Mike", "Kate", "Bob" };
...
<p>Здесь метод TakeWhile выбирает элементы, длина которых равна 3 символам. Первые два элемента массива people ("Tom", "Sam") соответствуют этому условию 
...
не будет включен в выходную коллекцию:</p>
<div class="console">
<pre class="consoletext">
Tom
Sam
...
<p>Если бы первый элемент в массиве имел бы длину больше или меньше 3 символов, то в этом случае метод TakeWhile возвратил бы нам 0 элементов.</p>
<h3>Постраничный вывод</h3>
<p>Совмещая оба метода - Take и Skip, мы можем выбрать определенное количество элементов начиная с определенного элемента. Например, 
выберем два элемента, начиная со четвертого (то есть пропустим 3 первых элемента):</p>
<pre class="brush:c#;">
