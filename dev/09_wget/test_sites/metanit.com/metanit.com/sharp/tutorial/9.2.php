...
<html >
<head>
<title>C# и .NET | DynamicObject и ExpandoObject</title>
<meta charset="utf-8" />
<meta name="description" content="Использование среды Dynamic Language Runtime в C# и .NET, определение динамических объектов, типы DynamicObject и ExpandoObject">
...
<h2>DynamicObject и ExpandoObject</h2><div class="date">Последнее обновление: 18.02.2022</div>
...
<p>Интересные возможности при разработке в C# и .NET с использованием DLR предоставляет пространство имен <span class="b">System.Dynamic</span> и в частности класс <span class="bb">ExpandoObject</span>. 
...
<h3>DynamicObject</h3>
<p>На ExpandoObject по своему действию похож другой класс - <span class="b">DynamicObject</span>. Он также позволяет задавать динамические объекты, но 
...
<p>Для использования <span class="b">DynamicObject</span> надо создать свой класс, унаследовав его от DynamicObject и реализовав его методы:</p>
<ul>
<li><p><code>TryBinaryOperation()</code>: выполняет бинарную операцию между двумя объектами. Эквивалентно стандартным бинарным операциям, например, 
сложению <code>x + y</code>)</p></li>
<li><p><code>TryConvert()</code>: выполняет преобразование к определенному типу. Эквивалентно базовому преобразованию в C#, например, <code>(SomeType) obj</code></p></li>
<li><p><code>TryCreateInstance()</code>: создает экземпляр объекта</p></li>
<li><p><code>TryDeleteIndex()</code>: удаляет индексатор</p></li>
<li><p><code>TryDeleteMember()</code>: удаляет свойство или метод</p></li>
<li><p><code>TryGetIndex()</code>: получает элемент по индексу через индексатор. В C# может быть эквивалентно следующему выражению <code>int x = collection[i]</code></p></li>
<li><p><code>TryGetMember()</code>: получаем значение свойства. Эквивалентно обращению к свойству, например, <code>string n = person.Name</code></p></li>
<li><p><code>TryInvoke()</code>: вызов объекта в качестве делегата</p></li>
<li><p><code>TryInvokeMember()</code>: вызов метода</p></li>
<li><p><code>TrySetIndex()</code>: устанавливает элемент по индексу через индексатор. В C# может быть эквивалентно следующему выражению <code>collection[i] = x;</code></p></li>
<li><p><code>TrySetMember()</code>: устанавливает свойство. Эквивалентно присвоению свойству значения, например: <code>person.Name = "Tom"</code></p></li>
<li><p><code>TryUnaryOperation()</code>: выполняет унарную операцию подобно унарным операциям в C#: <code>x++</code></p></li>
</ul>
...
то в качестве второго параметра используется массив <code>object[]</code> - он хранит переданные в метод или индексатор аргументы.</p>
...
В этом случае применяется третий параметр <code>out object vaue</code>, который предназначен для хранения возвращаемого объекта.</p>
<p>Например, определение метода <code>TryInvokeMember()</code>:</p>
<pre class="brush:c#;">
public virtual bool TryInvokeMember (InvokeMemberBinder binder, object?[]? args, out object? result)
</pre>
<p>Параметр <code>InvokeMemberBinder binder</code> является связывателем - получает свойства и методы объекта, <code>object?[]? args</code> хранит передаваемые аргументы, 
<code>out object? result</code> предназначен для хранения выходного результата.</p>
...
<p>Для хранения всех членов класса, как свойств, так и методов, определен словарь <code>Dictionary&lt;string, object&gt; members</code>. 
Ключами здесь являются названия свойств и методов, а значениями - значения этих свойств.</p>
<p>В методе <code>TrySetMember()</code> производится установка свойства:</p>
<pre class="brush:c#;">
...
<p>Для получения значения свойства переопределен метод <code>TryGetMember</code>:</p>
<pre class="brush:c#;">
...
<p>Для вызова методов определен метод <code>TryInvokeMember</code>:</p>
<pre class="brush:c#;">
public override bool TryInvokeMember(InvokeMemberBinder binder, object?[]? args, out object? result)
{
    result = null;
...
<p>Сначала с помощью bindera получаем метод и затем передаем ему аргумент <code>args[0]</code>, предварительно приведя его к типу int, и 
...
<p>Выражение <code>person.Name = "Tom"</code> будет вызывать метод <code>TrySetMember</code>, в который в качестве второго параметра будет передаваться строка "Tom".</p>
<p>Выражение <code>return person.Age;</code> вызывает метод <code>TryGetMember</code>.</p>
<p>Также у объекта person определен метод <code>IncrementAge</code>, который представляет действия лямбда-выражения 
<code>(int n) =&gt; { person.Age += n; return person.Age; };</code>. Это выражение принимает число n, увеличивает на это число свойство Age и возвращает новое значение person.Age. 
И при вызове этого метода будет происходить обращение к методу <code>TryInvokeMember</code>. И, таким образом, произойдет приращение значения свойства person.Age.</p>
