...
<html >
<head>
<title>C# и .NET | Метод AsOrdered в Parallel LINQ</title>
<meta charset="utf-8" />
<meta name="description" content="Упорядочивание результата операции в Parallel LINQ в языке программирования C# и .NET с помощью метода AsOrdered, применение метода AsUnordered">
...
<h2>Метод AsOrdered</h2><div class="date">Последнее обновление: 15.02.2022</div>
...
<p>При выполнении параллельного запроса порядок данных в результирующей выборки может быть не предсказуем. Например:</p>
<pre class="brush:c#;">
...
<p>В данном случае программа вычисляет квадраты чисел, которые больше 0. Однако в результате работы программы мы можем получить неупорядоченный вывод:</p>
<div class="console">
...
<p>То есть данные склеиваются в общий набор неупорядоченно.</p>
<p>Если в запросе применяются операторы или методы сортировки в запросе, данные автоматически упорядочиваются:</p>
<pre class="brush:c#;">
var squares = from n in numbers.AsParallel()
                 where n &gt; 0
...
<p>Однако не всегда оператор <code>orderby</code> или метод <code>OrderBy</code> используются в запросах. Более того они упорядочивают результирующую выборку в соответствии с результатами, а не в соответствии с исходной последовательностью. 
В этих случаях мы может применять метод 
<span class="b">AsOrdered()</span>:</p>
<pre class="brush:c#;">
var squares = from n in numbers.AsParallel().AsOrdered()
              where n &gt; 0
              select Square(n);
</pre>
<p>В этом случае результат уже будет упорядоченным в соответствии с тем, как элементы располагаются в исходной последовательности:</p>
<div class="console">
<pre class="consoletext">
1
4
...
<p>В то же время надо понимать, что упорядочивание в параллельной операции приводит к увеличению издержек, поэтому подобный запрос будет выполняться медленнее, чем 
неупорядоченный. И если задача не требует возвращение упорядоченного набора, то лучше не применять метод AsOrdered.</p>
<p>Кроме того, если в программе предстоят какие-нибудь манипуляции с полученным набором, однако упорядочивание больше не требуется, мы можем применить 
метод <span class="b">AsUnordered()</span>:</p>
<pre class="brush:c#;">
int[] numbers = new int[] { -2, -1, 0, 1, 2, 3, 4, 5, 6, 7, 8, };
var squares = from n in numbers.AsParallel().AsOrdered()
              where n &gt; 0
