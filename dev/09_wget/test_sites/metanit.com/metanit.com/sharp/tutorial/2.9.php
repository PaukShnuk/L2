...
<html >
<head>
<title>C# и .NET | Параметры методов</title>
<meta charset="utf-8" />
<meta name="description" content="Использование параметров в методах на языке C#, аргументы, формальные и фактические параметры, необязательные и именованные параметры">
...
<h2>Параметры методов</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>В прошлой теме был определен метод <code>SayHello</code>, который выводит на консоль некоторое сообщение:</p>
<pre class="brush:c#;">
void SayHello()
{
...
<p>Но минусом подобного метода является то, что он выводит одно и то же сообщение. И было бы неплохо, если бы мы могли бы динамически определять, 
какое сообщение будет выводить метод на экран, то есть передать из вне в метод это сообщение. Для этого в языке C# мы можем использовать параметры.</p>
<p><span class="b">Параметры</span> позволяют передать в метод некоторые входные данные. Параметры определяются через заятую в скобках после названия метода в виде:</p>
<pre class="brush:c#;">
...
<p>Определение параметра состоит из двух частей: сначала идет тип параметра и затем его имя.</p>
<p>Например, определим метод PrintMessage, который получает извне выводимое сообщение:</p>
<pre class="brush:c#;">
...
<p>Здесь метод <code>PrintMessage()</code> принимает один параметр, который называется <code>message</code> и имеет тип <code>string</code>.</p>
<p>Чтобы выполнить метод, который имеет параметры, при вызове после имени метода в скобках ему передаются значения для его параметров, например:</p>
<pre class="brush:c#;">PrintMessage("Hello work");</pre>
<p>Здесь параметру <code>message</code> передается строка "Hello work". Значения, которые передаются параметрам, еще называются <span class="b">аргументами</span>. 
То есть передаваемая строка "Hello work" в данном случае является аргументом.</p>
<p>Иногда можно встретить такие определения как <span class="b">формальные параметры</span> и <span class="b">фактические параметры</span>. 
Формальные параметры - это собственно параметры метода (в данном случае message), а фактические параметры - значения, которые 
передаются формальным параметрам. То есть фактические параметры - это и есть аргументы метода.</p>
<p>Определим еще один метод, который складывает два числа:</p>
<pre class="brush:c#;">
...
<p>Метод Sum имеет два параметра: x и y. Оба параметра представляют тип int. Поэтому при вызове данного метода нам обязательно надо 
передать на место этих параметров два числа. Внутри метода вычисляется сумма переданных чисел и выводится на консоль.</p>
<p>При вызове метода Sum значения передаются параметрам по позиции. Например, в вызове <code>Sum(10, 15)</code> число 10 
передается параметру x, а число 15 - параметру y.</p>
<p>Также параметры могут использоваться в сокращеной версии метода:</p>
<pre class="brush:c#;">
void Sum(int x, int y) =&gt; Console.WriteLine($"{x} + {y} = { x + y }");

Sum(10, 15);    // 10 + 15 = 25
</pre>
<p>Передаваемые параметру значения могут представлять значения переменных или результат работы сложных выражений, которые возвращают 
...
<p>Если параметрами метода передаются значения переменных, то таким переменным должно быть присвоено значение. Например, следующая программа не скомпилируется:</p>
<pre class="brush:c#;">
void Sum(int x, int y)
{
...
<h3>Соответствие параметов и аргументов по типу данных</h3>
<p>При передаче значений параметрам важно учитывать тип параметров: между аргументами и параметрами должно быть соответствие по типу. Например:</p>
<pre class="brush:c#;">
...
<p>В данном случае первый параметр метода <code>PrintPerson()</code> представляет тип <code>string</code>, поэтому при вызове метода 
мы должны передать этому параметру значение типа <code>string</code>, то есть строку. Второй параметр представляет тип <code>int</code>, 
поэтому должны передать ему целое число, которое соответствует типу <code>int</code>.</p>
<pre class="brush:c#;">
PrintPerson("Tom", 24);
</pre>
<p>Также мы можем передать параметрам значения тех типов, которые автоматически могут быть преобразованы в тип параметров. Например:</p>
<pre class="brush:c#;">
void PrintPerson(string name, int age)
{
    Console.WriteLine($"Name: {name}  Age: {age}");
...
<p>Здесь параметру типа <code>int</code> передается значение типа <code>byte</code>, но комилятор может автоматически преобразовать значение типа byte к тиу int. Поэтому здесь ошибки не возникнет. 
Какие преобразования типов могут быть выполнены автоматически, рассматривалось в одной из предыдущих тем: <a href="./2.2.php">Преобразования базовых типов данных</a></p>
<p>Данные других типов мы передать параметров не можем. Например, следующий вызов метода PrintPerson будет ошибочным:</p>
<pre class="brush:c#;">PrintPerson(45, "Bob"); // Ошибка! несоответствие значений типам параметров</pre>
<h3>Необязательные параметры</h3>
<p>По умолчанию при вызове метода необходимо предоставить значения для всех его параметров. Но C# также позволяет использовать необязательные параметры. 
Для таких параметров нам необходимо объявить значение по умолчанию. Также следует учитывать, что после необязательных параметров все последующие параметры также должны быть необязательными:</p>
<pre class="brush:c#;">
...
<p>Здесь параметры age и company являются необязательными, так как им присвоены значения. Поэтому при вызове метода мы можем не передавать для них данные:</p>
<pre class="brush:c#;">
void PrintPerson(string name, int age = 1, string company = "Undefined")
{
    Console.WriteLine($"Name: {name}  Age: {age}  Company: {company}");
...
<p>В предыдущих примерах при вызове методов значения для параметров передавались в порядке объявления этих параметров в методе. То есть аргументы передавались параметрам <span class="b">по позиции</span>. 
Но мы можем нарушить подобный порядок, используя именованные параметры:</p>
<pre class="brush:c#;">
void PrintPerson(string name, int age = 1, string company = "Undefined")
{
    Console.WriteLine($"Name: {name}  Age: {age}  Company: {company}");
...
<p>Для передачи значений параметрам о имени при вызове метода указывается имя параметра и через двоеточие его значение: <code>name:"Tom"</code></p>
<p>Консольный вывод программы:</p>
<div class="console"><pre class="consoletext">
Name: Tom  Age: 37  Company: Microsoft
...
<div class="nav"><p><a href="./2.8.php">Назад</a><a href="./">Содержание</a><a href="./2.34.php">Вперед</a></p></div></div>
<div class="socBlock">
