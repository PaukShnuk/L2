...
<html >
<head>
<title>C# и .NET | Коллекция Stack</title>
<meta charset="utf-8" />
<meta name="description" content="Стек Stack&lt;T&gt; в C#, добавление в стек с помощью метода Push и удаление из стека через метод Pop, пример использования">
...
<h2>Коллекция Stack&lt;T&gt;</h2><div class="date">Последнее обновление: 04.01.2022</div>
...
<p>Класс <code>Stack&lt;T&gt;</code> представляет коллекцию, которая использует алгоритм LIFO ("последний вошел - первый вышел"). 
...
<p>Для перебора стека можно использовать стандартный цикл <span class="b">foreach</span>. Причем в цикле в соответствии с аалгоритмом стека LIFO 
...
<p>Для получения количества элементов стека применяется свойство <span class="b">Count</span>.</p>

<h3>Методы Stack</h3>
<p>В классе Stack можно выделить следующие методы:</p>
<ul>
<li><p><span class="b">Clear</span>: очищает стек</p></li>
<li><p><span class="b">Contains</span>: проверяет наличие в стеке элемента и возвращает true при его наличии</p></li>
<li><p><span class="b">Push</span>: добавляет элемент в стек в верхушку стека</p></li>
<li><p><span class="b">Pop</span>: извлекает и возвращает первый элемент из стека</p></li>
<li><p><span class="b">Peek</span>: просто возвращает первый элемент из стека без его удаления</p></li>
</ul>
...
<img src="./pics/4.1.png" alt="Stack в C#" />
...
<ul>
<li><p><span class="b">bool TryPop(out T result)</span>: удаляет из стека первый элемент и передает его в переменную result, возвращает <code>true</code>, если очередь не пуста и элемент успешно получен.</p></li>
<li><p><span class="b">bool TryPeek(out T result)</span>: передает в переменную result первый элемент стека без его извлечения, 
возвращает <code>true</code>, если элемент успешно получен.</p></li>
</ul>
