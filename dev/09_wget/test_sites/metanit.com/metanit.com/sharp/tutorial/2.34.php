...
<html >
<head>
<title>C# и .NET | Возвращение значения и оператор return</title>
<meta charset="utf-8" />
<meta name="description" content="Возвращение значения из метода и оператор return в языке программирования C#, оператор return в void-методах">
...
<h2>Возвращение значения и оператор return</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Метод может возвращать значение, какой-либо результат. В примере выше были определены два метода, которые имели тип <span class="b">void</span>. Методы с таким типом не возвращают никакого значения. 
Они просто выполняют некоторые действия.</p>
<p>Но методы также могут возвращать некоторое значение. Для этого применяется оператор 
<span class="b">return</span>, после которого идет возвращаемое значение:</p>
<pre class="brush:c#;">return возвращаемое значение;</pre>
<p>Например, определим метод, который возвращает значение типа <code>string</code>:</p>
<pre class="brush:c#;">
string GetMessage()
{
	return "Hello";
...
<p>Метод <code>GetMessage</code> имеет тип <code>string</code>, следовательно, он должен возвратить строку. Поэтому в теле метода 
используется оператор <span class="b">return</span>, после которого указана возвращаемая строка.</p>

<p>При этом методы, которые в качестве возвращаемого типа имеют любой тип, кроме <span class="b">void</span>, обязательно должны использовать 
оператор return для возвращения значения. Например, следующее определение метода некорректно:</p>
<pre class="brush:c#;">
string GetMessage()
{
...
<p>Также между возвращаемым типом метода и возвращаемым значением после оператора return должно быть соответствие. 
Например, в следующем случае возвращаемый тип - <code>string</code>, но метод возвращает число (тип int), поэтому такое определение 
метода некорректно:</p>
<pre class="brush:c#;">
string GetMessage()
{
...
<p>Результат методов, который возвращают значение, мы можем присвоить переменным или использовать иным образом в программе:</p>
<pre class="brush:c#;">
string GetMessage()
{
	return "Hello";
...
<p>Метод <code>GetMessage()</code> возвращает значение типа <code>string</code>. Поэтому мы можем присвоить это значение какой-нибудь 
переменной типа string: <code>string message = GetMessage();</code></p>
<p>Либо даже передать в качестве значения параметру другого метода:</p>
<pre class="brush:c#;">
string GetMessage()
{
	return "Hello";
...
<p>В вызове <code>PrintMessage(GetMessage())</code> сначада вызывается метод <code>GetMessage()</code> и его результат передается параметру 
message метода PrintMessage</p>
<p>После оператора return также можно указывать сложные выражения или вызовы других методов, которые возвращают определенный результат. 
Например, определим метод, который возвращает сумму чисел:</p>
 <pre class="brush:c#;">
int Sum(int x, int y)
{
    return x + y;
...
<p>Метод <code>Sum()</code> имеет тип <code>int</code>, следовательно, он должен возвратить значение типа int - целое число. 
Поэтому в теле метода используется оператор <span class="b">return</span>, после которого указано возвращаемое число (в данном случае результат суммы переменных x и y).</p>
<h3>Сокращенная версия методов с результатом</h3>
<p>Также мы можем сокращать методы, которые возвращают значение:</p>
<pre class="brush:c#;">
string GetMessage()
{
	return "hello";
...
<p>аналогичен следующему методу:</p>
<pre class="brush:c#;">int Sum(int x, int y) =&gt; x + y;</pre>
<h3>Выход из метода</h3>
<p>Оператор return не только возвращает значение, но и производит выход из метода. Поэтому он должен определяться после остальных инструкций. 
Например:</p>
<pre class="brush:c#;">
string GetHello()
{
...
<p>С точки зрения синтаксиса данный метод корректен, однако его инструкция <code>Console.WriteLine("After return")</code> не имеет 
смысла - она никогда не выполнится, так как до ее выполнения оператор <span class="b">return</span> возвратит значение и произведет выход из метода.</p>
<p>Однако мы можем использовать оператор <span class="b">return</span> и в методах с типом <code>void</code>. В этом случае после оператора return не ставится 
никакого возвращаемого значения (ведь метод ничего не возвращает). Типичная ситуация - в зависимости от опеределенных условий произвести выход из метода:</p>
<pre class="brush:c#;">
...
<p>Здесь метод <code>PrintPerson()</code> в качестве параметров принимает имя и возраст пользователя. Однако в методе вначале мы проверяем, 
...
с помощью оператора <span class="b">return</span> выходим из метода. После этого метод заканчивает свою работу.</p>
...
<div class="nav"><p><a href="./2.9.php">Назад</a><a href="./">Содержание</a><a href="./2.27.php">Вперед</a></p></div></div>
<div class="socBlock">
