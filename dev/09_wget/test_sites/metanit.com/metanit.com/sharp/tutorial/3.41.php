...
<html >
<head>
<title>C# и .NET | Скрытие методов и свойств</title>
<meta charset="utf-8" />
<meta name="description" content="Скрытие методов и свойств базового класса в производном классе в языке программирования C# и .NET, method shadowing и hiding, оператор new">
...
<h2>Скрытие методов и свойств</h2><div class="date">Последнее обновление: 29.12.2021</div>
...
<p>В прошлой теме было рассмотрено определение и переопределение виртуальных методов. Другим способом изменить функциональность метода, унаследованного 
от базового класса, является <span class="b">скрытие</span> (shadowing / hiding).</p>
<p>Фактически скрытие метода/свойства представляет определение в классе-наследнике метода или свойства, которые соответствует по имени и набору 
параметров методу или свойству базового класса. Для скрытия членов класса применяется ключевое слово <span class="b">new</span>. 
Например:</p>
<pre class="brush:c#;">
class Person
{
...
<p>Здесь определен класс Person, представляющий человека, и класс Employee, представляющий работника предприятия. Employee наследует от Person 
...
И мы хотели бы в методе Print выводить информацию о компании вместе с именем на консоль. Для этого определяется метод Print с ключевым словом <span class="b">new</span>, 
который скрывает реализацию данного метода из базового класса.</p>
<p>В каких ситуациях можно использовать скрытие? Например, в примере выше метод Print в базовом классе не является виртуальным, мы не можем его переопределить, 
но, допустим, нас не устраивает его реализация для производного класса, поэтому мы можем воспользоваться сокрытием, чтобы определить нужный нам функционал.</p>
<p>Используем эти классы в программе в методе Main:</p>
<pre class="brush:c#;">
...
<p>При этом если мы хотим обратиться именно к реализации свойства или метода в базовом классе, то опять же мы можем использовать 
ключевое слово <span class="b">base</span> и через него обращаться к функциональности базового класса.</p>
<pre class="brush:c#;">
class Employee : Person
{
	public string Company { get; set; }
...
<h3>Скрытие свойств</h3>
<p>Подобным обазом мы можем организовать скрытие свойств:</p>
<pre class="brush:c#;">
Person bob = new Person("Bob");
...
<p>В данном случае в классе Employee переопределено свойство Name. В блоке <code>get</code> нем мы берем значение свойства из базовового класса P
erson и присоединяем к нему "Mr./Ms.". В блоке <code>set</code> передаем полученное значение в реализацию свойства Name базового класса Person</p>
<h3>Скрытие переменных и констант</h3>
<p>В отличие от переопределения C# позволяет применять скрытие к переменным (как к статическим, так и нестатическим) и константам, также используя ключевое слово <span class="b">new</span>:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./3.19.php">Назад</a><a href="./">Содержание</a><a href="./3.42.php">Вперед</a></p></div></div>
<div class="socBlock">
