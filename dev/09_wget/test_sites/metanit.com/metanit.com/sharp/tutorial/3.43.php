...
<html >
<head>
<title>С# и .NET | Применение делегатов</title>
<meta charset="utf-8" />
<meta name="description" content="Применение делегатов в С# и .NET, механизм событий и методов обратного вызова на основе делегата">
...
<h2>Применение делегатов</h2><div class="date">Последнее обновление: 31.12.2021</div>
...
<p>В прошлой теме подробно были рассмотрены делегаты. Однако данные примеры, возможно, не показывают истинной силы делегатов, так как нужные нам методы в данном случае мы можем вызвать и напрямую 
без всяких делегатов. Однако наиболее сильная сторона делегатов состоит в том, что они позволяют <span class="b">делегировать</span> выполнение 
...
<p>В переменной sum хранится сумма на счете. С помощью конструктора устанавливается начальная сумма на счете. Метод <code>Add()</code> служит для добавления на счет, 
а метод <code>Take</code> - для снятия денег со счета.</p>
<p>Допустим, в случае вывода денег с помощью метода <code>Take</code> нам надо как-то уведомлять об этом самого владельца счета и, может 
...
<pre class="brush:c#;">Console.WriteLine($"Со счета списано {sum} у.е.");</pre>
...
<p>Для делегирования действия здесь определен делегат <span class="b">AccountHandler</span>. Этот делегат соответствует любым методам, которые имеют 
тип <span class="b">void</span> и принимают параметр типа <span class="b">string</span>.</p>
<pre class="brush:c#;">public delegate void AccountHandler(string message);</pre>
<p>В классе Account определяем переменную <code>taken</code>, которая представляет этот делегат:</p>
<pre class="brush:c#;">AccountHandler? taken;</pre>
...
<p>Поскольку делегат AccountHandler в качестве параметра принимает строку, то при вызове переменной <code>taken()</code> мы можем передать в этот вызов 
...
<p>Здесь через метод RegisterHandler переменной taken в классе Account передается ссылка на метод <code>PrintSimpleMessage</code>. Этот метод 
...
<p>Через параметр <code>message</code> метод PrintSimpleMessage получит переданное из делегата сообщение и выведет его на консоль:</p>
<div class="console">
...
<p>В первом методе объединяет делегаты <code>taken</code> и <code>del</code> в один, который потом 
присваивается переменной <code>taken</code>. Во втором методе из переменной <code>taken</code> удаляется делегат <code>del</code>.</p> 
...
<p>В строке <code>account.UnregisterHandler(PrintColorMessage);</code> этот метод удаляется из списка вызовов делегата, поэтому этот метод 
