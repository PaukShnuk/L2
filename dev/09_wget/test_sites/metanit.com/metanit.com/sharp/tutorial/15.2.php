...
<html >
<head>
<title>C# и .NET | Фильтрация коллекции</title>
<meta charset="utf-8" />
<meta name="description" content="Фильтрация выборки и коллекций в LINQ в языке программирования C# и .NET, метод Where и OfType, фильтрация по типу данных ">
...
<h2>Фильтрация коллекции</h2><div class="date">Последнее обновление: 13.02.2022</div>
...
<p>Для выбора элементов из некоторого набора по условию используется метод <span class="b">Where</span>:</p>
<pre class="brush:c#;">Where&lt;TSource&gt; (Func&lt;TSource,bool&gt; predicate)</pre>
<p>Этот метод принимает делегат <code>Func&lt;TSource,bool&gt;</code>, который в качестве параметра принимает каждый элемент последовательности и возвращает 
значение bool. Если элемент соответствует некоторому условию, то возвращается true, и тогда этот элемент передаетсяв коллекцию, которая возвращается из метода Where.</p>
<p>Например, выберем все строки, длина которых равна 3:</p>
<pre class="brush:c#;">
...
<p>Если выражение в методе Where для определенного элемента будет равно true (в данном случае выражение <code>p.Length == 3</code>), то данный элемент попадает в результирующую выборку.</p>
<p>Аналогичный запрос с помощью операторов LINQ:</p>
<pre class="brush:c#;">
string[] people = { "Tom", "Alice", "Bob", "Sam", "Tim", "Tomas", "Bill" };
...
<p>Другой пример - выберем все четные элементы, которые больше 10.</p>
<p>Фильтрация с помощью операторов LINQ:</p>
<pre class="brush:c#;">
...
<h3>Выборка сложных объектов</h3>
<p>Допустим, у нас есть класс пользователя:</p>
<pre class="brush:c#;">
record class Person(string Name, int Age, List&lt;string&gt; Languages);
</pre>
<p>Свойство Name представляет имя, свойство Age - возраст пользователя, а список Languages - список языков, которыми владеет пользователь.</p>
<p>Создадим набор пользователей и выберем из них тех, которым больше 25 лет:</p>
<pre class="brush:c#;">
...
<p>Аналогичный запрос с помощью метода расширения <code>Where</code>:</p>
<pre class="brush:c#;">
var selectedPeople = people.Where(p=&gt; p.Age &gt; 25);
</pre>
<h3>Сложные фильтры</h3>
<p>Теперь рассмотрим более сложные фильтры. Например, в классе пользователя есть список языков, которыми владеет пользователь. Что если нам надо отфильтровать пользователей 
по языку:</p>
<pre class="brush:c#;">
...
<p>Для создания аналогичного запроса с помощью методов расширения применяется метод <span class="b">SelectMany</span>:</p>
<pre class="brush:c#;">
...
<p>Метод <code>SelectMany()</code> в качестве первого параметра принимает последовательность, которую надо проецировать, а в качестве второго 
...
к которым потом применяется фильтр с помощью Where.</p>
<h3>Фильтрация по типу данных</h3>
<p>Дополнительный метод расширения - <span class="b">OfType()</span> позволяет отфильтровать данные коллекции по определенному типу:</p>
<pre class="brush:c#;">
var people= new List&lt;Person&gt;
{
    new Student("Tom"),
...
<p>В данном случае список people содержит объекты трех типов  -  класса Person и производных типов Student и Employee. И в примере производится фильтрация данных 
типа Student - для этого метод <code>OfType()</code> типизируется типом Student. Консольный вывод:</p>
<div class="console">
<pre class="consoletext">
Tom
Bob
...
<div class="nav"><p><a href="./13.6.php">Назад</a><a href="./">Содержание</a><a href="./15.3.php">Вперед</a></p></div></div>
<div class="socBlock">
