...
<html >
<head>
<title>C# и .NET | Класс AutoResetEvent</title>
<meta charset="utf-8" />
<meta name="description" content="Использование класса AutoResetEvent и его методов WaitAll, WaitOne, Set, WaitAny для синхронизации потоков в приложении на языке программирования C# и .NET">
...
<h2>Класс AutoResetEvent</h2><div class="date">Последнее обновление: 07.02.2022</div>
...
<p>Класс <span class="b">AutoResetEvent</span> также служит целям синхронизации потоков. Этот класс представляет событие синхронизации потоков, 
который позволяет при получении сигнала переключить данный объект-событие из сигнального в несигнальное состояние.</p>

<p>Для управления синхронизацией класс AutoResetEvent предоставляет ряд методов:</p>
<ul>
<li><p><span class="b">Reset()</span>: задает несигнальное состояние объекта, блокируя потоки.</p></li>
<li><p><span class="b">Set();</span>: задает сигнальное состояние объекта, позволяя одному или несколким ожидающим потокам продолжить работу.</p></li>
<li><p><span class="b">WaitOne()</span>: задает несигнальное состояние и блокирует текущий поток, пока текущий объект AutoResetEvent не получит сигнал.</p></li>
</ul>
<p>Событие синхронизации может находиться в сигнальном и несигнальном состоянии. Если состояние события несигнальное, 
поток, который вызывает метод WaitOne, будет заблокирован, пока состояние события не станет сигнальным. Метод Set, наоборот, задает сигнальное состояние события.</p>
<p>Так, в одной из предыдущих тем для синхронизации потоков применялся оператор lock:</p>
<pre class="brush:c#;">
int x = 0;
...
<p>Перепишем этот пример с использованием <span class="b">AutoResetEvent</span>:</p>
<pre class="brush:c#;">
...
<p>Во-первых, создаем переменную типа <code>AutoResetEvent</code>. Передавая в конструктор значение <code>true</code>, мы тем самым указываем, 
что создаваемый объект изначально будет в сигнальном состоянии.</p>
<p>Когда начинает работать поток, то первым делом срабатывает определенный в методе Print вызов <code>waitHandler.WaitOne()</code>. Метод 
...
И так все потоки у нас переводятся в состояние ожидания.</p>
<p>После завершения работы вызывается метод <code>waitHandler.Set</code>, который уведомляет все ожидающие потоки, что объект waitHandler снова находится в сигнальном состоянии, 
и один из потоков "захватывает" данный объект, переводит в несигнальное состояние и выполняет свой код. А остальные потоки снова ожидают.</p> 
<p>Так как в конструкторе AutoResetEvent мы указываем, что объект изначально находится в сигнальном состоянии, то первый из очереди потоков захватывает данный объект 
и начинает выполнять свой код.</p>
<p>Но если бы мы написали <code>AutoResetEvent waitHandler = new AutoResetEvent(false)</code>, тогда объект изначально был бы в несигнальном состоянии, 
а поскольку все потоки блокируются методом <code>waitHandler.WaitOne()</code> до ожидания сигнала, то у нас попросту случилась бы блокировка программы, и программа 
не выполняла бы никаких действий.</p>
<p>Если у нас в программе используются несколько объектов <code>AutoResetEvent</code>, то мы можем использовать для отслеживания состояния этих объектов 
статические методы <span class="b">WaitAll</span> и <span class="b">WaitAny</span>, которые в качестве параметра принимают массив объектов класса WaitHandle - базового класса 
для AutoResetEvent.</p>
<p>Так, мы тоже можем использовать WaitAll в вышеприведенном примере. Для этого надо строку</p>
<pre class="brush:c#;">waitHandler.WaitOne();</pre>
<p>заменить на следующую:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./11.5.php">Назад</a><a href="./">Содержание</a><a href="./11.7.php">Вперед</a></p></div></div>
<div class="socBlock">
