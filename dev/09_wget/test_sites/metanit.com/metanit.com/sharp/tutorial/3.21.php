...
<html >
<head>
<title>C# и .NET | Частичные классы и методы</title>
<meta charset="utf-8" />
<meta name="description" content="Partial classes и partial methods в C# и .NET, частичные классы и методы">
...
<h2>Частичные классы и методы</h2><div class="date">Последнее обновление: 04.09.2021</div>
...
<p>Классы могут быть частичными. То есть мы можем иметь несколько файлов с определением одного и того же класса, и при компиляции все 
эти определения будут скомпилированы в одно.</p>
<p>Например, определим в проекте два файла с кодом. Не столь важно как эти файлы будут называться. Например, PersonBase.cs и PersonAdditional.cs. 
В одном из этих файлов (без разницы в каком именно) определим следующий класс:</p>
<pre class="brush:c#;">
...
<p>А в другом файле определим следующий класс:</p>
<pre class="brush:c#;">
public partial class Person
{
	public void Eat()
...
<p>Таким образом, два файла в проекте содержит определение одного и того же класса Person, которые содержат два разных метода. И оба определенных здесь класса 
являются частичными. Для этого они определяются с ключевым словом <span class="b">partial</span>.</p>
<img src="./pics/3.14.png" alt="Partial classes in C#" />
<p>Затем мы можем использовать все методы класса Person:</p>
<pre class="brush:c#;">
class Program
{
...
<h3>Частичные методы</h3>
<p>Частичные классы могут содержать частичные методы. Такие методы также опреляются с ключевым словом <span class="b">partial</span>. Причем 
определение частичного метода без тела метода находится в одном частичном классе, а реализация этого же метода - в другом частичном классе.</p>
<p>Например, изменим выше определенные классы Person. Первый класс:</p>
<pre class="brush:c#;">
public partial class Person
{
	partial void Read();
...
<p>В первом классе определен метод <code>Read()</code>. Причем на момент определения первого класса неизвестно, 
что представляет собой этот метод, какие действия он будет выполнять. Тем не менее мы знаем список его параметров и можем вызвать в первом классе.</p>
<p>Второй класс уже непосредственно определяет тело метода <code>Read()</code>.</p>
<pre class="brush:c#;">
class Program
{
	static void Main(string[] args)
...
<p>Стоит отметить, что по умолчанию к частичным методам применяется ряд ограничений:</p> 
<ul>
<li><p>Они не могут иметь модификаторы доступа</p></li>
<li><p>Они имеют тип <span class="b">void</span></p></li>
<li><p>Они не могут иметь out-параметры</p></li>
<li><p>Они не могут иметь модификаторы <code>virtual, override, sealed, new</code> или <code>extern</code></p></li>
</ul>
<p>Если же они не соответствуют какому-то из этих ограничений, то для них должна быть предоставлена реализация. Как, например, 
в следующем примере частичные методы применяют модификатор <code>public</code>:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./3.18.php">Назад</a><a href="./">Содержание</a><a href="./3.20.php">Вперед</a></p></div></div>
<div class="socBlock">
