...
<html >
<head>
<title>C# и .NET | Самовалидация модели</title>
<meta charset="utf-8" />
<meta name="description" content="Самовалидация модели в языке программирования C# и .NET, проверка данных на корректность, реализация интерфейса IValidatableObject и его метода Validate">
...
<h2>Самовалидация модели</h2><div class="date">Последнее обновление: 23.02.2022</div>
...
<p>Нам необязательно определять правила валидации модели в виде атрибутов. Мы можем применить к классу интерфейс <span class="b">IValidatableObject</span> 
и реализовать его метод <code>Validate()</code>:</p>
<pre class="brush:c#;">
...
<p>Метод Validate в качестве параметра получает объект ValidationContext, который собственно и проводит непосредственную валидацию. В качестве результата 
метод должен возвращать коллекцию объектов ValidationResult, которые представляют результат валидации.</p>
<p>Фактически при применении этого интерфейса класс будет сам себя валидировать.</p>
<p>Итак, реализуем этот интерфейс в классе User:</p>
<pre class="brush:c#;">
...
<p>Здесь в методе <code>Validate()</code> проверяем значения свойств и, если свойство не проходит валидацию, добавляем в список errors соответствующее 
сообщение об ошибке.</p>
<p>И в основной части программы мы также можем применять валидацию к объекту User:</p>
<pre class="brush:c#;">
using System.ComponentModel.DataAnnotations;
...
<div class="nav"><p><a href="./22.3.php">Назад</a><a href="./">Содержание</a><a href="./23.1.php">Вперед</a></p></div></div>
<div class="socBlock">
