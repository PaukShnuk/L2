...
<html >
<head>
<title>C# и .NET | Делегаты и анонимные методы в запросах LINQ</title>
<meta charset="utf-8" />
<meta name="description" content="Делегаты в запросах LINQ в языке программирования C# и .NET, передача в методы расширения LINQ других методов">
...
<h2>Делегаты в запросах LINQ</h2><div class="date">Последнее обновление: 14.02.2022</div>
...
<p>Если мы обратимся к определению многих методов расширений LINQ, то увидим, что в качестве параметра многие из них принимают делегаты например, 
<span class="b">Func&lt;TSource, bool&gt;</span>, например, определение метода Where:</p>
<pre class="brush:c#;">
...
<p>Хотя, как правило, в качестве делегата в подобные методы удобно передавать лямбда-выражения. Но тем не менее мы также можем передать полноценные методы. 
Например:</p>
<pre class="brush:c#;">
...
<p>Здесь метод LenghtIs3 проверяет, равна ли длина строки 3 символам. Так как в данном случае набор элементов, к которому применяется метод Where, содержит объекты string, то в метод в качестве параметра передается 
объект этого типа. Возвращаемый тип должен представлять тип <code>bool</code>: если true, то объект string соответствует условию и попадает в выходную коллекцию.</p>

<p>Рассмотрим другой пример. Пусть метод <code>Select()</code> применяется к коллекции целых чисел и преобразует каждое число в его квадрат:</p>
<pre class="brush:c#;">
...
<p>Метод Select в качестве параметра принимает тип <code>Func&lt;TSource, TResult&gt; selector</code>. Так как у нас набор объектов <code>int</code>, то входным 
параметром делегата также будет объект типа int. В качестве типа выходного параметра выберем int, так как здесь квадрат числа - это целочисленное значение.</p>
...
<div class="nav"><p><a href="./15.8.php">Назад</a><a href="./">Содержание</a><a href="./17.1.php">Вперед</a></p></div></div>
<div class="socBlock">
