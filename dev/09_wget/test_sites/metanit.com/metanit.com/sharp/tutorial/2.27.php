...
<html >
<head>
<title>C# и .NET | Параметры по ссылке и значению. Выходные параметры</title>
<meta charset="utf-8" />
<meta name="description" content="Передача параметров по ссылке и значению в методах на языке C#, применение модификаторов ref и out, выходные параметры">
...
<h2>Передача параметров по ссылке и значению. Выходные параметры</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Существует два способа передачи параметров в метод в языке C#: <span class="b">по значению</span> и 
<span class="b">по ссылке</span>.</p>
<h3>Передача параметров по значению</h3>
<p>Наиболее простой способ передачи параметров представляет передача по значению, по сути это обычный способ передачи параметров:</p>
<pre class="brush:c#;">
...
<p>При передаче аргументов параметрам по значению параметр метода получает не саму переменную, а ее копию и далее работает с этой копией независимо 
от самой переменной.</p>
<p>Так, выше при вызове метод Increment получает копию переменной number и увеличивает значение этой копии. Поэтому в самом методе 
...
То есть изменяется копия, а сама переменная не изменяется.</p>
<h3>Передача параметров по ссылке и модификатор ref</h3>
<p>При передаче параметров по ссылке перед параметрами используется модификатор <span class="b">ref</span>:</p>
<pre class="brush:c#;">
...
<p>При передаче значений параметрам по ссылке метод получает адрес переменной в памяти. И, таким образом, если в методе изменяется значение параметра, передаваемого по ссылке, то также изменяется и значение переменной, которая передается на его место..</p>
<p>Так, в метод Increment передается ссылка на саму переменную number в памяти. И если значение параметра n в Increment изменяется, то 
это приводит и к изменению переменной number, так как и параметр n и переменная number указывают на один и тот же адрес в памяти.</p>
<p>Обратите внимание, что модификатор <code>ref</code> указывается как перед параметром при объявлении метода, так и при вызове метода 
перед аргументом, который передается параметру.</p>

<h3>Выходные параметры. Модификатор out</h3>
<p>Выше мы использовали входные параметры. Но параметры могут быть также выходными. Чтобы сделать параметр выходным, перед ним ставится 
модификатор <span class="b">out</span>:</p>
<pre class="brush:c#;">
...
<p>Здесь результат возвращается не через оператор <code>return</code>, а через выходной параметр result. Использование 
в программе:</p>
<pre class="brush:c#;">
void Sum(int x, int y, out int result)
{
...
<p>Причем, как и в случае с <span class="b">ref</span> ключевое слово <span class="b">out</span> используется как при определении 
метода, так и при его вызове.</p>
<p>Также обратите внимание, что методы, использующие такие параметры, обязательно должны присваивать им определенное значение. То есть 
следующий код будет недопустим, так как в нем для out-параметра не указано никакого значения:</p>
<pre class="brush:c#;">
void Sum(int x, int y, out int result)
{
...
<p>Прелесть использования подобных параметров состоит в том, что по сути мы можем вернуть из метода не одно значение, а несколько. 
Например:</p>
<pre class="brush:c#;">
...
<p>Здесь у нас есть метод GetRectangleData, который получает ширину и высоту прямоугольника (параметры width и height).
 А два выходных параметра мы используем для подсчета площади и периметра прямоугольника.</p>
<p>При этом можно определять переменные, которые передаются <span class="b">out</span>-параметрам в непосредственно при вызове метода. 
То есть мы можем сократить предыдущий пример следующим образом:</p>
<pre class="brush:c#;">
void GetRectangleData(int width, int height, out int rectArea, out int rectPerimetr)
{
...
<p>При этом, если нам неизвестен тип значений, которые будут присвоены параметрам, то мы можем для их определения использовать оператор <span class="b">var</span>:</p>
<pre class="brush:c#;">
...
<h3>Входные параметры. Модификатор in</h3>
<p>Кроме выходных параметров с модификатором out метод может использовать входные параметры с модификатором <span class="b">in</span>. 
Модификатор <span class="b">in</span> указывает, что данный параметр будет передаваться в метод по ссылке, однако внутри метода его 
значение параметра нельзя будет изменить. Например, возьмем следующий метод:</p>
<pre class="brush:c#;">
...
<p>В данном случае через входные параметры <code>width</code> и <code>height</code> в метод передаются значения, но в самом методе мы не можем 
изменить значения этих параметров, так как они определены с модификатором <span class="b">in</span>.</p>
<p>Передача по ссылке в некоторых случаях может увеличить произодительность, а использование оператора <span class="b">in</span>, что переданные параметрам переменне ельзя будет изменить в этом методе.</p>
...
<div class="nav"><p><a href="./2.34.php">Назад</a><a href="./">Содержание</a><a href="./2.10.php">Вперед</a></p></div></div>
<div class="socBlock">
