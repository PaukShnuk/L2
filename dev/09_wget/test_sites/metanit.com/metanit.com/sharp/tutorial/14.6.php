...
<html >
<head>
<title>C# и .NET | Исследование полей и свойств с помощью рефлексии</title>
<meta charset="utf-8" />
<meta name="description" content="Применение рефлексии в языке программирования C# и .NET для исследования свойств и полей класса, типы PropertyInfo и FieldInfo, методы GetFields, GetProperties, получение и изменение значений приватных полей и свойств">
...
<h2>Исследование полей и свойств с помощью рефлексии</h2><div class="date">Последнее обновление: 17.02.2022</div>
...
<h3>Получение информации о полях</h3>
<p>Для извлечения всех полей применяется метод <span class="b">GetFields()</span>, который возвращает массив объектов класса 
<span class="b">FieldInfo</span>.</p>
<p>Некоторые основные свойства и методы класса <span class="b">FieldInfo</span>:</p>
<ul>
<li><p>Свойство <span class="b">IsAssembly</span>: возвращает true, если поле имеет модификатор доступа <code>protected</code></p></li>
<li><p>Свойство <span class="b">IsFamilyAndAssembly</span>: возвращает true, если поле имеет модификатор доступа <code>private protected</code></p></li>
<li><p>Свойство <span class="b">IsFamilyOrAssembly</span>: возвращает true, если поле имеет модификатор доступа <code>protected internal</code></p></li>
<li><p>Свойство <span class="b">IsAssembly</span>: возвращает true, если поле имеет модификатор доступа <code>internal</code></p></li>
<li><p>Свойство <span class="b">IsPrivate</span>: возвращает true, если поле имеет модификатор доступа <code>private</code></p></li>
<li><p>Свойство <span class="b">IsPublic</span>: возвращает true, если поле имеет модификатор доступа <code>public</code></p></li>
<li><p>Свойство <span class="b">IsStatic</span>: возвращает true, если поле статическое</p></li>
<li><p>Метод <span class="b">GetValue()</span>: возвращает значение поля</p></li>
<li><p>Метод <span class="b">SetValue()</span>: устанавливает значение поля</p></li>
</ul>
<p>Например, получим все поля класса:</p>
<pre class="brush:c#;">
...
<p>Чтобы получить и статические, и не статические, и публичные, и непубличные поля, в метод <code>GetFields()</code> передается набор 
флагов</p>
<pre class="brush:c#;">BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static</pre>
...
<h3>Получение и изменение значения поля</h3>
<p>Для получения одного поля по имени применяется метод <span class="b">GetField()</span>, в который передается имя поля:</p>
<pre class="brush:c#;">var name = myType.GetField("name", BindingFlags.Instance | BindingFlags.NonPublic);</pre>
<p>В качестве второго необязательного параметра передается набор флагов.</p>
<p>Причем рефлексия позволяет получать значения и изменять их даже у привтаных полей. Например, получим и изменим значение поля name:</p>
<pre class="brush:c#;">
using System.Reflection;
...
<h3>Свойства</h3>
<p>Для извлечения всех свойств типа применяется соответственно метод <span class="b">GetProperties()</span>, который возвращает массив объектов PropertyInfo. 
Для получения одного свойства по имени применяется метод <span class="b">GetProperty()</span>, в который передается название свойства и который возвращает объект 
PropertyInfo?.</p>
<p>Некоторый основной функционал класса <span class="b">PropertyInfo</span>:</p>
<ul>
<li><p>Свойство <span class="b">Attributes</span>: возвращает коллекцию атрибутов свойства</p></li>
<li><p>Свойство <span class="b">CanRead</span>: возвращает true, если свойство доступно для чтения</p></li>
<li><p>Свойство <span class="b">CanWrite</span>: возвращает true, если свойство доступно для записи</p></li>
<li><p>Свойство <span class="b">GetMethod</span>: возвращает get-акссесор в виде объекта <code>MethodInfo?</code></p></li>
<li><p>Свойство <span class="b">SetMethod</span>: возвращает set-акссесор в виде объекта <code>MethodInfo?</code></p></li>
<li><p>Свойство <span class="b">PropertyType</span>: возвращает тип свойства</p></li>
<li><p>Метод <span class="b">GetValue()</span>: возвращает значение свойства</p></li>
<li><p>Метод <span class="b">SetValue()</span>: устанавливает значение свойства</p></li>
</ul>
<p>Используем некоторые свойства PropertyInfo для получения информации о свойствах:</p>
<pre class="brush:c#;">
using System.Reflection;
...
<p>С помощью методов PropertyInfo можно манипулировать значением свойства. Например, получим и изменим значение свойства:</p>
<pre class="brush:c#;">
using System.Reflection;
...
<p>Для получения значения свойства в метод <code>GetValue()</code> объекта PropertyInfo передается объект, у которого вызывается свойства. Результатом метода 
является значение свойства. Для установки значения в метод <code>SetValue()</code> объекта PropertyInfo передается объект, у которого устанавливается свойство, и 
собственно новое значение свойства.</p>
...
<div class="nav"><p><a href="./14.5.php">Назад</a><a href="./">Содержание</a><a href="./14.3.php">Вперед</a></p></div></div>
<div class="socBlock">
