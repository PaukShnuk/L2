...
<html >
<head>
<title>C# и .NET | Локальные функции</title>
<meta charset="utf-8" />
<meta name="description" content="Локальные функции в C#, их определение и использование, контекст функции">
...
<h2>Локальные функции</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Локальные функции представляют функции, определенные внутри других методов. Локальная функция, как правило, содержит действия, которые применяются только в рамках ее 
метода.</p>
<p>Например, определим метод который сравнивают сумму чисел двух массивов:</p>
<pre class="brush:c#;">
...
<p>Здесь метод <code>Compare</code> принимает два массива и последовательно вычисляет сумму их элементов, чтобы узнать в каком массиве сумма чисел больше. Несмотря на то, что 
все работает, здесь есть один недостаток: здесь повторяется действия, которые вычисляют сумму массива:</p>
<pre class="brush:c#;">
int numbers1Sum = 0;
...
<p>К тому а что, если мы захотим сравнивать сумму только положительных или четных чисел или как-то иначе изменить логику сравнения? В этом лучше вынести повторяющиеся 
...
Для этого изменим метод Compare следующим образом:</p>
<pre class="brush:c#;">
void Compare(int[] numbers1, int[] numbers2)
{
...
<p>Здесь подсчет суммы вынесен в локальную функцию <code>Sum</code>, которая принимает массив и возвращает его сумму. И далее в рамках метода Compare 
мы сможем ее использовать для вычисления суммы массива. При этом неважно, определена локальная функция до или после использования. Но вне ее метода локальная функция не может использоваться.</p>
<h3>Статические локальные функции</h3>
<p>Локлальные функции могут быть статическими. Такие функции определяются с помощью ключевого слова <span class="b">static</span>. 
Их особенностью является то, что они не могут обращаться к переменным окружения, то есть метода, в котором статическая функция определена.</p> 
<p>Сначала определим локальную функцию, которая имеет доступ окружению:</p>
<pre class="brush:c#;">
int Sum(int[] numbers)
{
    int limit = 0;
...
<p>Здесь функция Sum вычисляет сумму чисел массива, которые соответствуют условию в локальной функции <code>IsPassed()</code>. Эта локальная функция 
проверяет, больше ли переданное число чем значение переменной <code>limit</code>, определенной в методе Sum. То есть локальная функция IsPassed может обращаться к данным определенным в 
окружающей функции Sum.</p>
<p>Теперь сделаем функцию IsPassed статической:</p>
<pre class="brush:c#;">
int Sum(int[] numbers)
{
    int result = 0;
...
<p>Модификатор <span class="b">static</span> указывается перед типом локальной функции. Теперь функция IsPassed не может обращаться к 
переменной limit, и в этом случае нам надо либо передать это значение в виде параметра, либо определить переменную limit непосредственно в локальной функции.</p>
...
<div class="nav"><p><a href="./2.11.php">Назад</a><a href="./">Содержание</a><a href="./3.45.php">Вперед</a></p></div></div>
<div class="socBlock">
