...
<html >
<head>
<title>C# и .NET | Использование IronPython</title>
<meta charset="utf-8" />
<meta name="description" content="Использование динамических языков IronPython и IronRuby в C# и .NET, написание клиентских сценариев">
...
<h2>Использование IronPython в .NET</h2><div class="date">Последнее обновление: 18.02.2022</div>
...
<p>Одним из ключевых достоинств среды DLR является поддержка таких динамических языков как <span class="b">IronPython</span> и 
<span class="b">IronRuby</span>. Казалось бы, зачем нам нужны еще языки, тем более которые применяются в рамках другого языка C#?</p>
...
того нажмем в окне проекта на узел <span class="b">Dependencies</span> правой кнопкой мыши и выберем в появившемся списке пункт <span class="b">Manage NuGet Packages...</span> (Управление NuGet-пакетами):</p>
<img src="./pics/9.1.png" alt="Пакетный менеджер NuGet в Visual Studio" />
...
первый - пакет <span class="b">DynamicLanguageRuntime</span> необходимо установить.</p>
<img src="./pics/9.2.png" alt="Пакет DynamicLanguageRuntime в NuGet" />
<p>После этого в проект в узел Dependencies добавляется библиотека <span class="b">Microsoft.Scripting</span>.</p>
<p>Теперь также нам надо добавить пакет <span class="b">IronPython</span>. Для этого введем в поле поиска "IronPython" и после этого установим одноименный 
пакет:</p>
<img src="./pics/9.3.png" alt="Пакет IronPython в NuGet" />
...
<p>Здесь используется выражение <code>print('hello, world')</code> языка Python, которое выводит на консоль строку. Для создания движка, выполняющего скрипт, 
применяется класс <span class="b">ScriptEngine</span>. А его метод <code>Execute()</code> выполняет скрипт.</p>
<p>Мы также могли бы определить файл <span class="b">hello.py</span>, то есть обычный текстовый файл с кодом на языке Python, со следующим содержимым:</p>
<pre class="brush:c#;">
...
<img src="./pics/9.4.png" alt="файл с кодом Python в проекте на C#" />
...
напишем простейший скрипт <span class="b">hello2.py</span>, который использует переменные:</p>
<pre class="brush:py;">
x = 10
z = x + y
print(z)
...
<p>Объект ScriptScope с  помощью метода <code>SetVariable</code> позволяет установить переменные в скрипте, а с помощью метода <code>GetVariable()</code> - 
получить их.</p>
<p>Консольный вывод:</p>
<div class="console">
<pre class="consoletext">
...
<p>Определим в файле <span class="b">hello.py</span> функцию для вычисления квадрата числа:</p>
<pre class="brush:c#;">
def square(n):
   return n * n
</pre>
...
<p>Получить объект функции можно также, как и переменную: <code>scope.GetVariable("square");</code>. Затем с этим объектом работаем также, как и с 
...
<div class="nav"><p><a href="./9.2.php">Назад</a><a href="./">Содержание</a><a href="./8.1.php">Вперед</a></p></div></div>
<div class="socBlock">
