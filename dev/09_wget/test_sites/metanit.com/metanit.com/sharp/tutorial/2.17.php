...
<html >
<head>
<title>C# и .NET | Null и значимые типы</title>
<meta charset="utf-8" />
<meta name="description" content="Null и значимые типы в языке программирования C#, использование структуры System.Nullable&lt;T&gt;, присвоение типам значений null, использование операции ??">
...
<h2>Null и значимые типы</h2><div class="date">Последнее обновление: 11.11.2021</div>
...
<p>В отличие от ссылочных типов переменным/параметрам значимых типов нельзя напрямую присвоить значение <span class="b">null</span>. Тем не менее 
...
может отсутствовать. То есть, если значение в базе данных есть - получим число, если нет - то null.</p>
<p>Чтобы присвоения переменной или параметру значимого типа значения <span class="b">null</span>, эти переменная/параметр значимого типа должны представлять тип <span class="b">nullable</span>. Для этого 
после названия типа указывается знак вопроса <span class="b">?</span></p>
<pre class="brush:c#;">
int? val = null;
Console.WriteLine(val);
</pre>
<p>Здесь переменная <code>val</code> представляет не просто тип <code>int</code>, а тип <span class="b">int?</span> - тип, переменные/параметры 
которого могут принимать как значения типа <code>int</code>, так и значение <code>null</code>. В данном случае мы передаем ей значение 
<span class="b">null</span>. Но также можно передать и значение типа <code>int</code>:</p>
<pre class="brush:c#;">
int? val = null;
IsNull(val);    // null
val = 22;
...
<p>Однако если переменная/параметр представляет значимый не nullable-тип, то присвоить им значение <span class="b">null</span> не получится:</p>
<pre class="brush:c#;">int val = null;  // ! ошибка, переменная val НЕ представляет тип nullable </pre>
<p>Стоит отметить, что фактически запись <code>?</code> для значимых типов является упрощенной формой использования структуры <span class="b">System.Nullable&lt;T&gt;</span>. Параметр 
<code>T</code> в угловых скобках представляет универсальный параметр, вместо которого в программе подставляется конкретный тип 
данных. Следующие виды определения переменных будут эквивалентны:</p>
<pre class="brush:c#;">
int? number1 = 5;
Nullable&lt;int&gt; number2 = 5;
</pre>
<h4>Свойства Value и HasValue и метод GetValueOrDefault</h4>
<p>Структура <span class="b">Nullable&lt;T&gt;</span> имеет два свойства:<p>
<ul>
<li><p><span class="b">Value</span> - значение объекта</p></li>
<li><p><span class="b">HasValue</span>: возвращает <code>true</code>, если объект хранит некоторое значение, и <code>false</code>, если объект равен <code>null</code>.</p>
</ul>
<p>Мы можем использовать эти свойства для проверки наличия и получения значения:</p>
<pre class="brush:c#;">
PrintNullable(5);       // 5
...
<p>Однако если мы попробуем получить через свойство Value значение переменной, которая равна null, то мы столкнемся с ошибкой:</p>
<pre class="brush:c#;">
int? number = null;
Console.WriteLine(number.Value);	// ! Ошибка
Console.WriteLine(number);			// Ошибки нет - просто ничего не выведет
</pre>
<p>Также структура <span class="b">Nullable&lt;T&gt;</span> имеет метод <span class="b">GetValueOrDefault()</span>. Он возвращает значение переменной/параметра, если они не равны <span class="b">null</span>. 
Если они равны <span class="b">null</span>, то возвращается значение по умолчанию. Значение по умолчанию можно передать в метод. Если в метод не передается данных, то возвращается значение по умолчанию для данного типа данных 
(например, для числовых данных это число 0).</p>
<pre class="brush:c#;">
...
<h3>Преобразование значимых nullable-типов</h3>
<p>Рассмотрим возможные преобразования:</p>
<ul>
<li>
<p>явное преобразование от T? к T</p>
<pre class="brush:c#;">
int? x1 = null;
if(x1.HasValue)
{
...
<li>
<p>неявное преобразование от T к T?</p>
<pre class="brush:c#;">
int x1 = 4;
int? x2 = x1;
Console.WriteLine(x2);
...
<li>
<p>неявные расширяющие преобразования от V к T?</p>
<pre class="brush:c#;">
int x1 = 4;
long? x2 = x1;
Console.WriteLine(x2);
</pre>
<p>явные сужающие преобразования от V к T?</p>
<pre class="brush:c#;">
long x1 = 4;
int? x2 = (int?)x1;
</pre>
</li>
<li><p>Подобным образом работают явные сужающие преобразования от V? к T?</p>
<pre class="brush:c#;">
long? x1 = 4;
int? x2 = (int?)x1;
</pre></li>
<li><p>явные сужающие преобразования от V? к T</p>
<pre class="brush:c#;">
long? x1 = null;
if (x1.HasValue) 
{ 
...
<h3>Операции с nullable-типами</h3>
<p>nullable-типы поддерживают тот же набор операций, что и их не-nullable двойники. Но следует учитывать, что если в операции участвует nullable-тип, то 
результатом также будет значение nullable-типа</p>
<pre class="brush:c#;">
int? x = 5;
int z = x + 7;          // нельзя
int? w = x + 7;         // можно
...
<p>В арифметических операциях, если один из операндов равен null, то результатом операции также будет null:</p>
<pre class="brush:c#;">
int? x = null;
int? w = x + 7;         // w = null
</pre>
<p>В операциях сравнения, если хотя бы один из операндов равен null, то возвращается <code>false</code></p>
...
<div class="nav"><p><a href="./3.50.php">Назад</a><a href="./">Содержание</a><a href="./3.26.php">Вперед</a></p></div></div>
<div class="socBlock">
