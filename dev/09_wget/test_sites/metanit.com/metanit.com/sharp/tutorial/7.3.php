...
<html >
<head>
<title>C# и .NET | StringBuilder</title>
<meta charset="utf-8" />
<meta name="description" content="Класс StringBuilder в C#, его основные методы и примеры использования, преимущества перед применением класса String">
...
<h2>Класс StringBuilder</h2><div class="date">Последнее обновление: 05.01.2022</div>
...
<p>Хотя класс System.String предоставляет нам широкую функциональность по работе со строками, все таки он имеет свои недостатки. Прежде всего, 
...
существенными.</p>
<p>Чтобы выйти из этой ситуации во фреймворк .NET был добавлен новый класс <span class="b">StringBuilder</span>, который находится в пространстве имен 
<code>System.Text</code>. Этот класс представляет динамическую строку.</p>
<h3>Создание StringBuilder</h3>
<p>Для создания объекта StringBuilder можно использовать ряд его конструкторов. Прежде всего можно создать пустой StringBuilder:</p>
<pre class="brush:c#;">
using System.Text;

StringBuilder sb = new StringBuilder();
</pre>
<p>Можно сразу инициализировать объект определенной строкой:</p>
<pre class="brush:c#;">
StringBuilder sb = new StringBuilder("Привет мир");
</pre>
<p>С помощью метода <code>ToString()</code> мы можем получить строку, которая хранится в StringBuilder:</p>
<pre class="brush:c#;">
var sb = new StringBuilder("Hello World");
Console.WriteLine(sb.ToString());    //  Hello World
</pre>
<p>Либо можно просто передать объект StringBuilder:</p>
<pre class="brush:c#;">
var sb = new StringBuilder("Hello World");
Console.WriteLine(sb);    //  Hello World
</pre>
<h3>Длина и емкость StringBuilder</h3>
<p>Для хранения длины строки в классе StringBuilder определенно свойство <span class="b">Length</span>. Однако есть и вторая величина - 
емкость выделенной памяти. Это значение хранится в свойстве <code>Capacity</code>. Емкость - это выделенная память под объект. Установка емкости 
позволяет уменьшить выделения памяти и тем самым повысить производительность.</p>
<p>Если строка, которая передается в конструктор StringBuilder, имеет длину 16 символов или меньше, то начальная ёмкость в StringBuilder равна 16. 
Если начальная строка больше 16 символов, то в качестве начальной емкости StringBuilder будет использовать длину строки.</p>
<p>Например, посмотрим, что содержат данные свойства:</p>
<pre class="brush:c#;">
using System.Text;
...
<p>Хотя в данном случае длина равна 10 символов, но реально емкость будет составлять 
...
При увеличении строки в StringBuilder, когда количество символов превосходит начальную емкость, то емкость увеличивается в два и более раз.</p>
<p>Если у нас заранее известен максимальный размер объекта, то мы можем таким образом сразу задать емкость с помощью одного из конструкторов и 
тем самым избежать последующих издержек при дополнительном выделении памяти.</p>
<pre class="brush:c#;">
var sb = new StringBuilder(32);
</pre>
<p>StringBuilder также позволяет сразу задать строку и емкость:</p>
<pre class="brush:c#;">var sb = new StringBuilder("Привет мир", 32);</pre>
<h3>Операции со строками в StringBuilder</h3>
<p>Для операций над строками класс StringBuilder определяет ряд методов:</p>
<ul>
<li><p><span class="b">Append</span>: добавляет подстроку в объект StringBuilder</p></li>
<li><p><span class="b">Insert</span>: вставляет подстроку в объект StringBuilder, начиная с определенного индекса</p></li>
<li><p><span class="b">Remove</span>: удаляет определенное количество символов, начиная с определенного индекса</p></li>
<li><p><span class="b">Replace</span>: заменяет все вхождения определенного символа или подстроки на другой символ или подстроку</p></li>
<li><p><span class="b">AppendFormat</span>: добавляет подстроку в конец объекта StringBuilder</p></li>
</ul>
<p>Теперь посмотрим на примере метода <span class="b">Append()</span> использование и преимущества класса StringBuilder:</p>
<pre class="brush:c#;">
using System.Text;
...
<p>При создании объекта StringBuilder выделяется память по умолчанию для 16 символов, так как длина начальной строки меньше 16.</p>
<p>Дальше применяется метод <code>Append</code> - этот метод добавляет к строке подстроку. Так как при объединении строк их общая длина - 22 символа - 
превышает начальную емкость в 16 символов, то начальная емкость удваивается - до 32 символов.</p>
<p>Если бы итоговая длина строки была бы больше 32 символов, то емкость расширялась бы до размера длины строки.</p>
<p>Далее опять применяется метод Append, однако финальная длина уже будет 28 символов, что меньше 32 символов, и дополнительная память не будет выделяться.</p>
<p>Используем остальные методы StringBuilder:</p>
<pre class="brush:c#;">
...
<p>Когда надо использовать класс String, а когда StringBuilder?</p>
<p>Microsoft рекомендует использовать класс String в следующих случаях:</p>
<ul>
<li><p>При небольшом количестве операций и изменений над строками</p></li>
<li><p>При выполнении фиксированного количества операций объединения. В этом случае компилятор может объединить все операции объединения в одну</p></li>
<li><p>Когда надо выполнять масштабные операции поиска при построении строки, например IndexOf или StartsWith. Класс StringBuilder не имеет подобных методов.</p></li>
</ul>
<p>Класс StringBuilder рекомендуется использовать в следующих случаях:</p>
<ul>
<li><p>При неизвестном количестве операций и изменений над строками во время выполнения программы</p></li>
<li><p>Когда предполагается, что приложению придется сделать множество подобных операций</p></li>
</ul>
...
<div class="nav"><p><a href="./7.5.php">Назад</a><a href="./">Содержание</a><a href="./7.4.php">Вперед</a></p></div></div>
<div class="socBlock">
