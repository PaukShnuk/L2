...
<html >
<head>
<title>C# и .NET | Работа с классом Task</title>
<meta charset="utf-8" />
<meta name="description" content="Использование класса Task для создания асинхронных задач в C#, фабрика задач TaskFactory, возвращение результата из задач, свойства класса Task">
...
<h2>Работа с классом Task</h2><div class="date">Последнее обновление: 08.02.2022</div>
...
<h3>Вложенные задачи</h3>
<p>Одна задача может запускать другую - вложенную задачу. При этом эти задачи выполняются независимо друг от друга. Например:</p>
<pre class="brush:c#;">
...
<p>Несмотря на то, что здесь мы ожидаем выполнения внешней задачи, но вложенная задача может завершить выполнение 
даже после завершения метода Main:</p>
<div class="console">
...
<p>При этом внутренняя задача может даже не начать свое выполнение к завершению работы основного потока программы. 
То есть в данном случае внешняя и вложенная задачи выполняются независимо друг от друга.</p>
<p>Если необходимо, чтобы вложенная задача выполнялась как часть внешней, необходимо использовать значение 
<span class="b">TaskCreationOptions.AttachedToParent</span>:</p>
<pre class="brush:c#;">
var outer = Task.Factory.StartNew(() =&gt;      // внешняя задача
{
    Console.WriteLine("Outer task starting...");
...
<p>В данном случае вложенная задача прикреплена к внешней и выполняется как часть внешней задачи. И внешняя задача завершится только когда завершатся все прикрепленные 
к ней вложенные задачи.</p>
<h3>Массив задач</h3>
<p>Также как и с потоками, мы можем создать и запустить массив задач. Можно определить все задачи в массиве непосредственно через объект Task:</p>
<pre class="brush:c#;">
...
<p>Либо также можно использовать методы Task.Factory.StartNew или Task.Run и сразу запускать все задачи:</p>
<pre class="brush:c#;">
...
<p>Но в любом случае мы опять же можем столкнуться с тем, что все задачи из массива могут завершиться после того, как отработает метод Main, в котором запускаются эти задачи:</p>
<pre class="brush:c#;">
...
<p>Один из возможных консольных выводов программы:</p>
<div class="console">
<pre class="consoletext">
Завершение метода Main
</pre>
</div>
<p>Если необходимо завершить выполнение программы или вообще выполнять некоторый код лишь после того, как все задачи из массива завершатся, 
то применяется метод <span class="b">Task.WaitAll(tasks)</span>:</p>
<pre class="brush:c#;">
Task[] tasks = new Task[3];
for(var i = 0; i &lt; tasks.Length; i++)
{
...
<p>В этом случае сначала завершатся все задачи, и лишь только потом будет выполняться последующий код из метода Main:</p>
<div class="console">
<pre class="consoletext">
Завершение метода Main
Task3 finished
...
<p>В то же время порядок выполнения самих задач в массиве также недетерминирован.</p>
<p>Также мы можем применять метод <span class="b">Task.WaitAny(tasks)</span>. Он ждет, пока завершится хотя бы одна из массива задач.</p>
<h3>Возвращение результатов из задач</h3>
<p>Задачи могут не только выполняться как процедуры, но и возвращать определенные результаты:</p>
<pre class="brush:c#;">
int n1 =4, n2 = 5;
...
<p>Во-первых, чтобы получать из задачи не который результат, необходимо типизировать объект Task тем типом, объект которого мы хотим получить из задачи. 
Например,  в примере выше мы ожидаем из задачи sumTask получить число типа int, соответственно типизируем объект Task данным типом - <code>Task&lt;int&gt;</code>.</p>
<p>И, во-вторых, в качестве задачи должен выполняться метод, который возвращает данный тип объекта. Так, в данном случае 
у нас в качестве задачи выполняется метод <code>Sum</code>, которая принимаетдва числа и на выходе возвращает их сумму - значение типа int.</p>
<p>Возвращаемое число будет храниться в свойстве Result: <code>sumTask.Result</code>. Нам не надо его приводить к типу int, оно уже само по себе 
будет представлять число.</p>
<pre class="brush:c#;">int result = sumTask.Result;</pre>
<p>При этом при обращении к свойству Result текущий поток останавливает выполнение и ждет, когда будет получен результат из выполняемой задачи.</p>
<p>Другой пример:</p>
<pre class="brush:c#;">
...
<p>В данном случае задача defaultPersonTask возвращает объект типа Person, который мы можем получить из свойства Result.</p>
...
<div class="nav"><p><a href="./12.1.php">Назад</a><a href="./">Содержание</a><a href="./12.3.php">Вперед</a></p></div></div>
<div class="socBlock">
