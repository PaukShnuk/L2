...
<html >
<head>
<title>C# и .NET | Типы исключений. Класс Exception</title>
<meta charset="utf-8" />
<meta name="description" content="Типы исключений в C#, класс Exception и его свойства, отображение сообщения об исключении, IndexOutOfRangeException">
...
<h2>Типы исключений. Класс Exception</h2><div class="date">Последнее обновление: 30.12.2021</div>
...
<p>Базовым для всех типов исключений является тип <span class="b">Exception</span>. Этот тип определяет ряд свойств, с помощью которых можно получить информацию об исключении.</p>
<ul>
<li><p><span class="b">InnerException</span>: хранит информацию об исключении, которое послужило причиной текущего исключения</p></li>
<li><p><span class="b">Message</span>: хранит сообщение об исключении, текст ошибки</p></li>
<li><p><span class="b">Source</span>: хранит имя объекта или сборки, которое вызвало исключение</p></li>
<li><p><span class="b">StackTrace</span>: возвращает строковое представление стека вызывов, которые привели к возникновению исключения</p></li>
<li><p><span class="b">TargetSite</span>: возвращает метод, в котором и было вызвано исключение</p></li>
</ul>
...
<img src="./pics/2.11.png" alt="Обработка исключений и класс Exception в C#" />
<p>Однако так как тип Exception является базовым типом для всех исключений, то выражение <code>catch (Exception ex)</code> 
...
<ul>
<li><p><span class="b">DivideByZeroException</span>: представляет исключение, которое генерируется при делении на ноль</p></li>
<li><p><span class="b">ArgumentOutOfRangeException</span>: генерируется, если значение аргумента находится вне диапазона 
допустимых значений</p></li>
<li><p><span class="b">ArgumentException</span>: генерируется, если в метод для параметра передается некорректное значение</p></li>
<li><p><span class="b">IndexOutOfRangeException</span>: генерируется, если индекс элемента массива или коллекции находится вне диапазона 
допустимых значений</p></li>
<li><p><span class="b">InvalidCastException</span>: генерируется при попытке произвести недопустимые преобразования типов</p></li>
<li><p><span class="b">NullReferenceException</span>: генерируется при попытке обращения к объекту, который равен null (то есть по сути неопределен)</p></li>
</ul>
...
<pre class="brush:c#;">numbers[7] = 9;</pre>
...
Однако поскольку после генерации IndexOutOfRangeException управление переходит в соответствующий блок catch, то деление на ноль <code>int y = x / 0</code> 
...
<p>И в данном случае блок <code>catch (Exception ex){}</code> будет обрабатывать все исключения кроме DivideByZeroException 
...
<div class="nav"><p><a href="./2.28.php">Назад</a><a href="./">Содержание</a><a href="./2.31.php">Вперед</a></p></div></div>
<div class="socBlock">
