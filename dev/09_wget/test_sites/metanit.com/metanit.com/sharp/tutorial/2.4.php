...
<html >
<head>
<title>C# и .NET | Массивы</title>
<meta charset="utf-8" />
<meta name="description" content="Массивы языка программирования C#, определение массивов, операции над ними, их перебор с помощью цикла for/foreach и использование">
...
<h2>Массивы</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Массив представляет набор однотипных данных. Объявление массива похоже на объявление переменной за тем исключением, что после указания типа ставятся квадратные скобки:</p>
<pre class="brush:c#;">тип_переменной[] название_массива;</pre>
<p>Например, определим массив целых чисел:</p>
<pre class="brush:c#;">int[] numbers;</pre>
<p>После определения переменной массива мы можем присвоить ей определенное значение:</p>
<pre class="brush:c#;">int[] nums = new int[4];</pre>
<p>Здесь вначале мы объявили массив nums, который будет хранить данные типа <code>int</code>. Далее используя операцию <code>new</code>, 
мы выделили память для 4 элементов массива: <code>new int[4]</code>. Число 4 еще называется <span class="b">длиной массива</span>. 
...
<p>Подобным образом можно определять массивы и других типов, например, массив значений типа <code>string</code>:</p>
<pre class="brush:c#;">string[] people = { "Tom", "Sam", "Bob" };</pre>
<h3>Индексы и получение элементов массива</h3>
<p>Для обращения к элементам массива используются <span class="b">индексы</span>. Индекс представляет номер элемента в массиве, при этом нумерация начинается с 
...
<p>каждый массив имеет свойство <span class="b">Length</span>, которое хранит длину массива. Например, получим длину выше созданного массива numbers:</p>
<pre class="brush:c#;">
int[] numbers = { 1, 2, 3, 5 };

Console.WriteLine(numbers.Length);  // 4
</pre>
<p>Для получения длины массива после названия массива через точку указывается свойство <code>Length</code>: <code>numbers.Length</code>.</p>
<h3>Получение элементов с конца массива</h3>
<p>Благодаря наличию свойства <code>Length</code>, мы можем вычислить индекс последнего элемента массива - это длина массива - 1. Например, если длина массива - 4 (то есть 
массив имеет 4 элемента), то индекс последнего элемента будет равен 3. И, используя свойство <code>Length</code>, мы можем легко получить элементы с конца массива:</p>
<pre class="brush:c#;">
int[] numbers = { 1, 2, 3, 5};
...
<p>Однако при подобном подходе выражения типа <code>numbers.Length - 1</code>, смысл которых состоит в том, чтобы получить какой-то определенный элемент с конца массива, 
утяжеляют код. И, начиная, с версии C# 8.0 в язык был добавлен специальный оператор <span class="b">^</span>, 
с помощью которого можно задать индекс относительно конца коллекции.</p>
<p>Перепишем предыдущий пример, применяя оператор <span class="b">^</span>:</p>
<pre class="brush:c#;">
int[] numbers = { 1, 2, 3, 5};
...
<p>Для перебора массивов мы можем использовать различные типы циклов. Например, цикл <span class="b">foreach</span>:</p>
<pre class="brush:c#;">
...
<p>Здесь в качестве контейнера выступает массив данных типа <code>int</code>. Поэтому мы объявляем переменную с типом <code>int</code></p>
...
<p>В то же время цикл <span class="b">for</span> более гибкий по сравнению с <span class="b">foreach</span>. Если <span class="b">foreach</span> последовательно извлекает элементы контейнера и только для чтения, 
...
<p>Также можно использовать и другие виды циклов, например, <code>while</code>:</p>
<pre class="brush:c;">
int[] numbers = { 1, 2, 3, 4, 5 };
int i = 0;
...
<p>Массивы характеризуются таким понятием как <span class="b">ранг</span> или количество измерений. Выше мы рассматривали 
...
В частности, у каждого массива есть метод <span class="b">GetUpperBound(номер_размерности)</span>, который возвращает индекс последнего 
...
<pre class="brush:c#;">numbers.GetUpperBound(0) + 1</pre>
<p>можно получить количество строк таблицы, представленной двухмерным массивом. А через</p>
<pre class="brush:c#;">numbers.Length / количество_строк</pre>
...
<p>От многомерных массивов надо отличать <span class="b">массив массивов</span> или так называемый "зубчатый массив":</p>
<pre class="brush:c#;">
int[][] nums = new int[3][];
...
<p>Здесь две группы квадратных скобок указывают, что это <span class="b">массив массивов</span>, то есть такой массив, который в свою очередь содержит в себе 
...
<code>new int[3][]</code>. В данном случае у нас массив nums содержит три массива. Причем размерность каждого из этих массивов может не совпадать.</p>
...
<tr><td><p>1</p></td><td><p>2</p></td><td><p>3</p></td><td><p>4</p></td><td><p>5</p></td></tr>
</tbody></table>
...
<ul>
<li><p><span class="b">Ранг</span> (rank): количество измерений массива</p></li>
<li><p><span class="b">Длина измерения</span> (dimension length): длина отдельного измерения массива</p></li>
<li><p><span class="b">Длина массива</span> (array length): количество всех элементов массива</p></li>
</ul>
<p>Например, возьмем массив</p>
<pre class="brush:c#;">int[,] numbers = new int[3, 4];</pre>
...
<img src="./pics/array.png" alt="Массивы в языке C#" />
...
<ul>
<li><a title="Вопросы для самопроверки" href="../questions/1.8.php"><i class="fa fa-lg fa-question"></i><span class="dop-txt">Вопросы для самопроверки</span></a></li>
<li><a title="Упражнения" href="../practice/1.3.php"><i class="fa fa-lg fa-pencil"></i><span class="dop-txt">Упражнения</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./2.6.php">Назад</a><a href="./">Содержание</a><a href="./2.7.php">Вперед</a></p></div></div>
<div class="socBlock">
