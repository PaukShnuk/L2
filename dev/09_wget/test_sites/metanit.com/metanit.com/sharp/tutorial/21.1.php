...
<html >
<head>
<title>C# и .NET | Службы Windows</title>
<meta charset="utf-8" />
<meta name="description" content="Создание службы Windows в C# и .NET Framework">
...
<h1>Службы Windows</h1><h2>Создание службы для Windows</h2><div class="date">Последнее обновление: 16.10.2019</div>
...
<p>Одним из важнейших компонентов ОС Windows являются службы. Фактически это отдельные приложения, которые не имеют графического интерфейса и 
...
открыть панель служб и сами увидеть все установленные и запущенные службы:</p>
<img src="./pics/windowsservice.png" alt="Создание службы Windows в C#" />
<p>Рассмотрим, как создавать свои службы в C#. В качестве реализуемой задачи выберем наблюдение за изменениями в определенной папке в файловой системе. 
Теперь создадим для ее выполнения службу.</p>
<p>Вначале создадим новый проект, который будет иметь тип <span class="bb">Windows Service</span>. Назовем проект FileWatcherService:</p>
<img src="./pics/createwindowsservice.png" alt="Проект Windows Service в Visual Studio 2015" />
<p>После этого Visual Studio генерирует проект, который имеет все необходимое. Хотя в принципе нам необязательно выбирать именно этот тип проекта, 
можно было бы создать проект библиотеки классов, и затем в нем определить все необходимые классы.</p>
<p>Итак, новый проект выглядит следующим образом:</p>
<img src="./pics/21.1.png" />
<p>Здесь также есть файл Program.cs и есть собственно узел службы <span class="bb">Service1.cs</span>.</p> 
<p>Служба представляет обычное приложение, но она не запускаетс сама по себе. Все вызовы и обращения к ней проходят через менеджер управления службами 
(Service Control Manager или SCM). Когда служба запускается автоматически при старте системы или вручную, то SCM обращается к методу Main в классе Program:</p>
<pre class="brush:c#;">
...
<p>Метод Main по умолчанию определен таким образом, чтобы запускать сразу несколько служб, которые определены в массиве ServicesToRun. Однако по умолчанию 
проект содержит только одну службу Service1. Сам запуск производится с помощью метода Run: <code>ServiceBase.Run(ServicesToRun)</code>.</p>
<p>Сама запускаемая служба представлена узлом Service1.cs. Однако на самом деле 
это не простой файл кода. Если мы откроем этот узел, то увидим в нем файл дизайнера службы <span class="bb">Service1.Designer.cs</span> и класс Service1.</p>
<p>Класс Service1 собственно представляет службу. По умолчанию он имеет следующий код:</p>
<pre class="brush:c#;">
using System;
...
<p>Класс службы должен наследоваться от базового класса <span class="bb">ServiceBase</span>. Этот класс определяет ряд методов, важнейшие из которых 
метод <span class="bb">OnStart()</span>, который запускает действия, выпоняемые службой, и метод <span class="bb">OnStop()</span>, останавливающий 
службу.</p>
<p>После того, как SCM вызовет метод Main  и зарегистрирует службу, происходит непосредственный ее вызов через запуск метода OnStart.</p>
<p>Когда в консоли служб или через командную строку мы посылаем команду на остановку службы, то SCM обращается к методу OnStop для ее остановки.</p>
<p>Кроме этих двух методов в классе службы можно переопределить еще несколько методов базового класса ServiceBase:</p>
<ul>
<li><p><span class="bb">OnPause</span>: вызывается при приостановке службы</p></li>
<li><p><span class="bb">OnContinue</span>: вызывается при возобновлении работы службы после ее приостановки</p></li>
<li><p><span class="bb">OnShutdown</span>: вызывается при завершении работы Windows</p></li>
<li><p><span class="bb">OnPowerEvent</span>: вызывается при изменении режима электропитания</p></li>
<li><p><span class="bb">OnCustomCommand</span>: вызывается при получении службой пользовательской команды от Менеджера Управления Службами (Service Control Manager / SCM)</p></li>
</ul>
<p>В конструкторе класса Service1 вызывается метод <code>InitializeComponent()</code>, который определен в файле дизайнера Service1.Designer.cs:</p>
<pre class="brush:c#;">
namespace FileWatcherService
{
...
<p>Единственное, что надо в нем отметить, это установка названия службы (свойство ServiceName):</p>
<pre class="brush:c#;">this.ServiceName = "Service1";</pre>
<p>Это то название, которое будет отображаться в консоли служб после установки данной службы. Мы можем его изменить, а можем и оставить как есть.</p>
<p>Теперь изменим код службы следующим образом:</p>
<pre class="brush:c#;">
using System;
using System.ServiceProcess;
using System.IO;
...
<p>Ключевым классом, который инкапсулирует всю функциональность, является класс Logger. С помощью объекта FileSystemWatcher он будет вести мониторинг изменений в папке <i>D://Temp</i>. 
В методе <code>Start()</code> устанавливается, что мы будем отслеживать изменения через объект FileSystemWatcher. И вся работа будет идти, пока булевая переменная enabled равна <code>true</code>. А метод <code>Stop()</code> позволит 
завершить работу класса.</p>
<p>События FileSystemWatcher позволяют отслеживать все изменения в наблюдаемой папке. 
При этом будет вестись запись изменений в файл templog.txt. Чтобы не было гонки ресурсов 
за файл templog.txt, в который вносятся записи об изменениях, процедура записи блокируется заглушкой <code>lock(obj)</code>.</p>
<p>В итоге после создания, изменения, переименования и удаления файл лога будет содержать что-то наподобие:</p>
<pre>
...
<p>В самом классе службы Service1 в конструкторе устанавливается ряд опций:</p>
<pre class="brush:c#;">
...
<p>В методе <code>OnStart()</code> для запуска объекта Logger вызывется новый поток:</p>
<pre class="brush:c#;">
protected override void OnStart(string[] args)
{
    logger = new Logger();
...
<p>Новый поток нужен, так как текущий поток обрабатывает только команды SCM и должен возвращаться из метода OnStart как можно быстрее.</p>
<p>Когда от менеджера SCM поступает команда на остановку службы, срабатывает метод OnStop, который вызывает метод <code>logger.Stop()</code>. Дополнительная задержка 
позволит потоку логгера остановиться:</p>
<pre class="brush:c#;">
protected override void OnStop()
{
    logger.Stop();
...
<p>Однако самого класса службы еще недостаточно. Нам необходимо еще создать устанощик службы.</p>
...
<div class="nav"><p><a href="./2.32.php">Назад</a><a href="./">Содержание</a><a href="./21.2.php">Вперед</a></p></div></div>
<div class="socBlock">
