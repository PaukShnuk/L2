...
<html >
<head>
<title>C# и .NET | Records</title>
<meta charset="utf-8" />
<meta name="description" content="Тип Record, отличие от обычных классов, сравнение и метод Equals, оператор with и инициализация, позиционные positional records">
...
<h2>Records</h2><div class="date">Последнее обновление: 02.01.2022</div>
...
<p>Records представляют новый ссылочный тип, который появился в C#9. Ключевая особенность records состоит в том, что они могут представлять неизменяемый (immutable) тип, 
...
не будут изменяться. В .NET в принципе уже есть неизменяемые типы, например, String.</p>
<p>Стоит отметить, что начиная с версии C# 10 добавлена поддержка структур record, 
соответственно мы можем создавать record-классы и record-структуры.</p>
<p>Для определения records используется ключевое слово <span class="b">record</span>. Если определяется класс record, то ключевое слово <code>class</code> можно неиспользовать 
при определении типа:</p>
<pre class="brush:c#;">
...
<p>При определении структуры record при объявлении типа надо использовать ключевое слово <span class="b">struct</span>:
<pre class="brush:c#;">
...
<p>Хотя типы record предназначены для создания неизменяемых типов, однако одно только применение ключевого слова <span class="b">record</span> не гарантирует 
неизменяесть объектов record. Они являются неизменяемыми (immutable) только при определенных условиях. Например, мы можем написать так:</p>
<pre class="brush:c#;">
...
<p>При выполнении этого кода не возникнет никакой ошибки, мы спокойно сможем изменять значения свойств объекта Person. Чтобы сделать 
его действительно неизменяемым, для свойств вместо обычных сеттеров надо использовать модификатор <span class="b">init</span>.</p>
<pre class="brush:c#;">
var person = new Person("Tom");
...
<p>В данном случае мы получим ошибку при попытке изменить значение свойств объекта Person.</p>
<p>Во многим records похожи на обычные классы и структуры, например, они могут абстрактными, их также можно наследовать либо запрещать наследование с помощью оператора sealed. 
Тем не менее есть и ряд отличий. Рассмотрим некоторые основные отличия records от стандартных классов и структур.</p>
<h3>Сравнение на равенство</h3>
<p>При определении record компилятор генерирует метод <span class="b">Equals()</span> для сравнения с другим объектом. 
При этом сравнение двух records производится на основе их значений. Например, рассмотрим следующий пример:</p>
<pre class="brush:c#;">
...
<p>В данном случае при сравнении двух объектов record Person мы увидим, что они равны, так как их значения (значения свойств Name) 
равны. Однако в случае с объектами класса User, которые имеют те же одинаковые значения мы увидим, что они не равны. 
Так как сравнение records производится <span class="b">по значению</span>.</p>
<p>Кроме того, для record уже по умолчанию реализованы операторы <span class="b">==</span> и <span class="b">!=</span>, 
которые также сравнивают две record по значению:</p>
<pre class="brush:c#;">
var person1 = new Person("Tom");
var person2 = new Person("Tom");
...
<h3>Оператор with</h3>
<p>В отличие от классов records поддерживают инициализацию с помощью оператора <span class="b">with</span>. Он позволяет создать 
одну record на основе другой record:</p>
<pre class="brush:c#;">
...
<p>После record, значения которой мы хотим скопировать, указывается оператор <span class="b">with</span>, после которого в 
...
Age значение из tom, а свойство Name изменяется.</p>
<p>Эта возможность может быть особенно актуальна, если в record, которую мы хотим скопировать, множество свойств, из которых мы хотим поменять 
одно-два.</p>
<p>Если надо скопировать значения всех свойств, то можно оставить пустые фигурные скобки:</p>
<pre class="brush:c#;">
var person1 = new Person("Tom", 37);
var person2 = person1 with { };
</pre>
<h3>Позиционные records</h3>
<p>Records могут принимать данные для свойств через конструктор, и в этом случае мы можем сократить их определение. Например, 
пусть у нас есть следующая record Person:</p>
<pre class="brush:c#;">
public record struct Person
{
    public string Name { get; init; }
...
<p>Кроме конструктора здесь реализован деконструктор, который позволяет разложить объект Person на кортеж значений. И мы 
могли бы применить ее, например, следующим образом:</p>
<pre class="brush:c#;">
...
<p>Выше определенную record Person можно сократить до позиционной record:</p>
<pre class="brush:c#;">
public record Person(string Name, int Age);
</pre>
<p>Это все определение типа. То есть мы говорим, что для типа Person будет создаваться конструктор, который принимает два параметра и присваивает 
их значения соответственно свойствам Name и Age, и что также автоматически будет создаваться деконструктор. 
Ее использование будет аналогично:</p>
<pre class="brush:c#;">
var person = new Person("Tom", 37);
...
<p>При необходимости также можно совмещать стандартное определение свойств и определение свойств через конструктор:</p>
<pre class="brush:c#;">
...
<h3>Позиционные структуры для чтения</h3>
<p>Следует отметить различие между позиционными классами и структурами record. Свойства класса record, которые устанавливаются через параметры конструктора, 
по умолчанию будут иметь модификатор <span class="b">init</span>. То есть после установки их значений через конструктор, мы больше не сможем их изменить:</p>
<pre class="brush:c#;">
var person = new Person("Tom", 37);
...
<p>Стоит отметить, что это относится только к тем свойствам, которые устанавливаются через конструктор.</p>
<p>Однако для позиционных структур record свойства будут иметь стандартные сеттеры, которые позволят изменять значения свойств:</p>
<pre class="brush:c#;">
var person = new Person("Tom", 37);
person.Name = "Bob";
...
<p>Чтобы для подобных свойств структуры record использовался модификатор <span class="b">init</span> вместо обычных сеттеров, такую структуру надо определить с ключевым словом 
<span class="b">readonly</span>:</p>
<pre class="brush:c#;">
var person = new Person("Tom", 37);
...
<h3>ToString</h3>
<p>Небольшим преимуществом типов record также является то, что для них уже по умолчанию реализован метод <span class="b">ToString()</span>, 
который выводит состояние объекта в отформатированном виде:</p>
<pre class="brush:c#;">
var person = new Person("Tom", 37);
...
<h3>Наследование</h3>
<p>Как и обычные классы record-классы могут наследоваться:</p>
<pre class="brush:c#;">
var tom = new Person("Tom", 37);
...
<p>В данном случае класс record Employee наследуется от Person.</p>
...
<div class="nav"><p><a href="./2.19.php">Назад</a><a href="./">Содержание</a><a href="./3.34.php">Вперед</a></p></div></div>
<div class="socBlock">
