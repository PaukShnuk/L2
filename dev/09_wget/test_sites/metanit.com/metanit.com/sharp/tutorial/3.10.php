...
<html >
<head>
<title>C# и .NET | Класс System.Object</title>
<meta charset="utf-8" />
<meta name="description" content="Класс System.Object в языке C# и .NET, его методы Equals, ToString, GetHashCode, GetType и их переопределение в производных классах">
...
<h2>Класс System.Object и его методы</h2><div class="date">Последнее обновление: 29.12.2021</div>
...
<p>Все классы в .NET, даже те, которые мы сами создаем, а также базовые типы, такие как 
<span class="b">System.Int32</span>, являются неявно производными от класса Object. Даже если мы не указываем класс Object в качестве базового, 
...
<h3>ToString</h3>
<p>Метод <code>ToString</code> служит для получения строкового представления данного объекта. Для базовых типов просто будет выводиться их строковое 
значение:</p>
<pre class="brush:c#;">
int i = 5;
...
<p>Для переопределения метода <code>ToString()</code> в классе Clock, который представляет часы, используется ключевое слово 
<span class="b">override</span> (как и при обычном переопределении виртуальных или абстрактных методов). В данном случае 
метод <code>ToString()</code> выводит в строке значения свойств Hours, Minutes, Seconds.</p>
...
Стоит отметить, что базовая реализация возвращает не просто строку, а объект <span class="b">string?</span> - то есть это может 
быть строка string, либо значение <span class="b">null</span>, которое указывает на отсутствие значения. И в реальности в качестве возвращаемого типа для метода 
мы можем использовать как <span class="b">string</span>, так и <span class="b">string?</span></p>  
<p>Если же имя у объекта Person установлено, то возвращается значение свойства Name. Для проверки строки на наличие значения применяется метод <code>String.IsNullOrEmpty()</code>.</p>
...
<code>Console.WriteLine()</code> по умолчанию выводит именно строковое представление объекта. Поэтому, если нам надо вывести строковое представление объекта на консоль, то при передаче 
...
<p>Метод <span class="b">GetHashCode</span> позволяет возвратить некоторое числовое значение, которое будет соответствовать данному объекту или его хэш-код. 
...
<p>Этот метод возвращает объект <span class="b">Type</span>, то есть тип объекта.</p>
<p>С помощью ключевого слова <span class="b">typeof</span> мы получаем тип класса и сравниваем его с типом объекта. И если этот объект представляет тип Client, 
...
<p>Стоит отметить, что проверку типа можно скоратить с помощью оператора <span class="b">is</span>:</p>
<pre class="brush:c#;">
object person = new Person { Name = "Tom" };
if (person is Person)
    Console.WriteLine("Это реально класс Person");
</pre>
<p>В отличие от методов ToString, Equals, GetHashCode метод <span class="b">GetType()</span> не переопределяется.</p>
<h3>Метод Equals</h3>
<p>Метод Equals позволяет сравнить два объекта на равенство. В качестве параметра он принимает объект для сравнения в виде типа <code>object</code> и возврашает 
<code>true</code>, если оба объекта равны:</p>
<pre class="brush:c#;">public override bool Equals(object? obj) {......}</pre>
...
<div class="nav"><p><a href="./3.8.php">Назад</a><a href="./">Содержание</a><a href="./3.12.php">Вперед</a></p></div></div>
<div class="socBlock">
