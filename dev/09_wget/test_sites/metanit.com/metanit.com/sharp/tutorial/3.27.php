...
<html >
<head>
<title>C# и .NET | Ковариантность и контравариантность обобщенных интерфейсов</title>
<meta charset="utf-8" />
<meta name="description" content="Ковариантность и контравариантность обобщенных интерфейсов в C# и .NET, понятие инвариантности, применение ключевых слов in и out">
...
<h2>Ковариантность и контравариантность обобщенных интерфейсов</h2><div class="date">Последнее обновление: 02.01.2022</div>
...
<p>Понятия ковариантности и контравариантности связаны с возможностью использовать в приложении вместо некоторого типа другой тип, который находится ниже или выше в иерархии наследования.</p>
<p>Имеется три возможных варианта поведения:</p>
<ul>
<li><p><span class="b">Ковариантность</span>: позволяет использовать более конкретный тип, чем заданный изначально</p></li>
<li><p><span class="b">Контравариантность</span>: позволяет использовать более универсальный тип, чем заданный изначально</p></li>
<li><p><span class="b">Инвариантность</span>: позволяет использовать только заданный тип</p></li>
</ul>
<p>C# позволяет создавать ковариантные и контравариантные обобщенные интерфейсы. Эта функциональность 
повышает гибкость при использовании обобщенных интерфейсов в программе. По умолчанию все обобщенные интерфейсы являются инвариантными.</p>
<p>Для рассмотрения ковариантных и контравариантных интерфейсов возьмем следующие классы:</p>
<pre class="brush:c#;">
class Message
{
...
<p>Здесь определен класс сообщения Message, который получает через конструктор текст и сохраняет его в свойство Text. 
А класс EmailMessage представляет условное email-сообщение и просто вызывает конструктор базового класса, передавая ему текст сообщения.</p>
<h3>Ковариантные интерфейсы</h3>
<p>Обобщенные интерфейсы могут быть ковариантными, если к универсальному параметру применяется ключевое слово <span class="b">out</span>. Такой параметр должен представлять 
тип объекта, который возвращается из метода. Например:</p>
<pre class="brush:c#;">
...
<p>Здесь обобщенный интерфейс <code>IMessenger</code> представляет интерфейс мессенджера и определяет метод <code>WriteMessage()</code> 
...
Ключевое слово <span class="b">out</span> в определении интерфейса указывает, что данный интерфейс будет ковариантным.</p>
<p>Класс EmailMessenger, который представляет условную программу для отправки email-сообщений, 
реализует этот интерфейс и возвращает из метода <code>WriteMessage()</code> объект EmailMessage.</p>
<p>Применим данные типы в программе:</p>
<pre class="brush:c#;">
...
<p>То есть мы можем присвоить более общему типу <code>IMessenger&lt;Message&gt;</code> объект более конкретного типа <code>EmailMessenger</code> или 
<code>IMessenger&lt;EmailMessage&gt;</code>.</p>
<p>В то же время если бы мы не использовали ключевое слово <code>out</code>:</p>
<pre class="brush:c#;">interface IMessenger&lt;T&gt;</pre>
<p>то мы столкнулись бы с ошибкой в строке</p>
<pre class="brush:c#;">
...
<p>Поскольку в этом случае невозможно было бы привести объект <code>IMessenger&lt;EmailMessage&gt;</code> к типу <code>IMessenger&lt;Message&gt;</code></p>
<p>При создании ковариантного интерфейса надо учитывать, что универсальный параметр может использоваться только в качестве типа значения, возвращаемого методами интерфейса. 
Но не может использоваться в качестве типа аргументов метода или ограничения методов интерфейса.</p>
<h3>Контравариантные интерфейсы</h3>
<p>Для создания контравариантного интерфейса надо использовать ключевое слово <span class="b">in</span>. Например, возьмем те же классы Message и EmailMessage 
и определим следующие типы:</p>
<pre class="brush:c#;">
...
<p>Здесь опять же интерфейс IMessenger представляет интерфейс мессенджера и определяет метод <code>SendMessage()</code> для отправки условного сообщения. 
Ключевое слово <span class="b">in</span> в определении интерфейса указывает, что этот интерфейс - <span class="b">контравариантный</span>.</p>
<p>Класс SimpleMessenger представляет условную программу отправки сообщений и реализует этот интерфейс. 
...
<code>IMessenger&lt;Message&gt;</code>.</p>
<p>Применим эти типы в программе:</p>
<pre class="brush:c#;">
...
<p>Так как интерфейс IMessenger использует универсальный параметр с ключевым словом <span class="b">in</span>, то он является контравариантным, 
поэтому в коде мы можем переменной типа <code>IMessenger&lt;EmailMessage&gt;</code> передать объект <code>IMessenger&lt;Message&gt;</code> или 
<code>SimpleMessenger</code></p>
<p>Если бы ключевое слово <span class="b">in</span> не использовалось бы, то мы не смогли бы это сделать. То есть объект интерфейса с более универсальным типом приводится к объекту интерфейса с более конкретным типом.</p>
<p>При создании контрвариантного интерфейса надо учитывать, что универсальный параметр контрвариантного типа может применяться только к 
аргументам метода, но не может применяться к возвращаемому результату метода.</p>
<h3>Совмещение ковариантности и контравариантности</h3>
<p>Также мы можем совместить ковариантность и контравариантность в одном интерфейсе. Например:</p>
<pre class="brush:c#;">
...
<p>Фактически здесь объединены два предыдущих примера. Благодаря ковариантности/контравариантности объект класса SimpleMessenger может представлять типы 
<code>IMessenger&lt;EmailMessage, Message&gt;</code>, <code>IMessenger&lt;Message, EmailMessage&gt;</code>, 
<code>IMessenger&lt;Message, Message&gt;</code> и <code>IMessenger&lt;EmailMessage, EmailMessage&gt;</code>. 
...
<div class="nav"><p><a href="./3.23.php">Назад</a><a href="./">Содержание</a><a href="./3.36.php">Вперед</a></p></div></div>
<div class="socBlock">
