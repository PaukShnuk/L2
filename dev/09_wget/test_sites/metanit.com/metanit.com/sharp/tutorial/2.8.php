...
<html >
<head>
<title>C# и .NET | Методы</title>
<meta charset="utf-8" />
<meta name="description" content="Определение и использование методов языке C#, процедуры и функции, возвращаемые значения, оператор return">
...
<h2>Методы</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Если переменные хранят некоторые значения, то методы содержат собой набор инструкций, которые выполняют определенные действия. 
По сути метод - это именованный блок кода, который выполняет некоторые действия.</p>
...
<p>Ранее мы уже использовали как минимум один метод - <span class="b">Console.WriteLine()</span>, который выводит информацию на консоль. 
Теперь рассмотрим, как мы можем создавать свои методы.</p>
<h3>Определение метода</h3>
...
<p>Здесь определен метод <code>SayHello</code>, который выводит некоторое сообшение. К названиям методов предъявляются в принципе те же требования, что и к названиям переменных. Однако, как правило, 
названия методов начинаются с большой буквы.</p>
<p>Перед названием метода идет возвращаемый тип данных. Здесь это тип <span class="b">void</span>, который указыает, 
что фактически ничего не возвращает, он просто производит некоторые действия.</p>
...
<p>После списка параметров в круглых скобках идет блок кода, который представляет набор выполняемых методом инструкций. В данном случае блок метода SayHello 
содержит только одну инструкцию, которая выводит строку на консоль:</p>
<pre class="brush:c#;">Console.WriteLine("Hello");</pre>
<p>Но если мы запустим данный проект, то мы не увидим никакой строки, которую должен выводить метод SayHello. Потому что после определения 
метод еще надо вызвать, чтобы он выполнил свою работу.</p>
<h3>Вызов методов</h3>
...
<pre class="brush:c#;">SayHello();</pre>
...
<img src="./pics/2.13.png" alt="методы в C# и .NET" />
...
<p>Преимуществом методов является то, что их можно повторно и многократно вызывать в различных частях программы. Например, в примере 
выше два раза вызывается метод SayHello.</p>
<p>При этом в данном случае нет разницы, сначала определяется метод, а потом вызывается или наоборот. Например, мы могли бы написать и так:</p>
<pre class="brush:c#;">
SayHello(); // Hello
SayHello(); // Hello
...
<p>Определим и вызовем еще несколько методов:</p>
<pre class="brush:c#;">
void SayHelloRu()
{
...
<p>Здесь определены три метода <code>SayHelloRu()</code>, <code>SayHelloEn()</code> и <code>SayHelloFr()</code>, которые также имеют 
тип <code>void</code>, не принимают никаких параметров и также выводит некоторую строку на консоль. Условно говоря, они выводят приветствие на 
определенном языке.</p>
<p>В конструкции <code>switch</code> проверяется значение переменной <code>language</code>, которая условно хранит код языка, 
и в зависимости от ее значения вызывается определенный метод. Так, в данном случае на консоль будет выведено</p>
<div class="console"><pre class="consoletext">Hello</pre></div>
<h3>Сокращенная запись методов</h3>
<p>Если метод в качестве тела определяет только одну инструкцию, то мы можем сократить определение метода. Например, допустим у нас есть метод:</p>
<pre class="brush:c#;">
void SayHello()
{
	Console.WriteLine("Hello");
...
<p>Мы можем его сократить следующим образом:</p>
<pre class="brush:c#;">void SayHello() =&gt; Console.WriteLine("Hello");</pre>
<p>То есть после списка параметров ставится оператор <span class="b">=&gt;</span>, после которого идет выполняемая инструкция.</p>
