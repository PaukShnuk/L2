...
<html >
<head>
<title>C# и .NET | Pattern matching</title>
<meta charset="utf-8" />
<meta name="description" content="Pattern matching, Преобразование типов и Паттерн типов в языке программирования C# и .NET, type pattern и constant pattern">
...
<h1>Pattern matching</h1><h2>Паттерн типов</h2><div class="date">Последнее обновление: 07.01.2022</div>
...
<p><span class="b">Pattern matching</span> фактически выполняет сопоставление некоторого значения с некоторым шаблоном. И если 
сопоставление прошло успешно, то выполняются определенные действия. Язык C# позволяет выполнять различные типы сопоставлений.</p>
<p>Паттерн типов или <span class="b">type pattern</span> позволяет проверить некоторое значение на соответствие некоторому типу:</p>
<pre class="brush:c#;">значение is тип переменная_типа</pre>
<p>Например, у нас есть следующие классы:</p>
<pre class="brush:c#;">
class Employee
{
...
<p>Класс Employee представляет работника, а класс Manager - менеджера. Оба класса реализуют метод Work. Кроме того, класс Manager определяет свойство IsOnVacation.</p>
<p>С помощью паттерна типов проверим, представляет ли объект Employee класс Manager:</p>
<pre class="brush:c#;">
...
<p>Здесь в методе UseEmployee значение <code>emp</code> сопоставляется с типом Manager. То есть в данном случае в качестве шаблона выступает тип Manager. 
...
И далее мы можем вызвать у него методы и свойства.</p>
<p>Также мы можем использовать <span class="b">constant pattern</span> - сопоставление с некоторой константой:</p>
<pre class="brush:c#;">
var message = "hello";
...
<p>Подобным образом, например, можно проверить значение на null:</p>
<pre class="brush:c#;">
...
<p>Кроме конструкции if сопоставление паттернов может применяться в конструкции switch:</p>
<pre class="brush:c#;">
Employee bob = new Employee();
Employee tom = new Manager();
UseEmployee(tom);   // Manager works
...
<p>С помощью выражения <span class="b">when</span> можно вводить дополнительные условия в конструкцию case:</p>
<pre class="brush:c#;">
...
<p>В этом случае опять же преобразуем объект emp в объект типа Manager и в случае удачного преобразования смотрим на значение свойства IsOnVacation: 
если оно равно false, то выполняется данный блок case.</p>
...
<div class="nav"><p><a href="./3.51.php">Назад</a><a href="./">Содержание</a><a href="./3.55.php">Вперед</a></p></div></div>
<div class="socBlock">
