...
<html >
<head>
<title>C# и .NET | Создание библиотеки классов</title>
<meta charset="utf-8" />
<meta name="description" content="Создание библиотеки классов в C# и .NET Core и подключение ее в проект">
...
<h2>Создание библиотеки классов</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Нередко различные классы и структуры оформляются в виде отдельных библиотек, которые компилируются в файлы dll и затем могут подключаться в другие проекты. 
Благодаря этому мы можем определить один и тот же функционал в виде библиотеки классов и подключать в различные проекты или передавать на использование другим разработчикам.</p>
<p>Создадим и подключим библиотеку классов.</p>
<p>Возьмем имеющийся проект консольного приложения C#, например, созданный в прошлых темах. В структуре проекта нажмем правой кнопкой на название решения и далее в появившемся 
контекстном меню выберем <span class="b">Add -&gt; New Project...</span> (Добавить новый проект):</p>
<img src="./pics/3.16.png" alt="Создание библиотеки классов в C#" />
<p>Далее в списке шаблонов проекта найдем пункт <span class="b">Class Library</span>:</p>
<img src="./pics/3.17.png" alt="Библиотека классов в C# и .NET" />
<p>Затем дадим новому проекту какое-нибудь название, например, MyLib:</p>
<img src="./pics/3.18.png" alt="Class Library in .NET" />
<p>Далее будет предложено выбрать версию фреймворка, под которую создается библиоека:</p>
<img src="./pics/3.18.png" alt=".NET 6.0 (Long-term support)" />
<p>Оставим здесь <span class="b">.NET 6.0 (Long-term support)</span> и нажмем на кнопку Create. После этого в решение будет добавлен новый проект, в моем случае с названием MyLib:</p>
<img src="./pics/3.19.png" alt="Добавление нового проекта в C# и .NET" />
<p>По умолчанию новый проект имеет один пустой класс Class1 в файле Class1.cs. Мы можем этот файл удалить или переименовать, как нам больше нравится.</p>
<p>Например, переименуем файл Class1.cs в Person.cs, а класс Class1 в Person. Определим в классе Person простейший код:</p>
<pre class="brush:c#;">
public class Person
{
	string name;
...
<img src="./pics/3.20.png" alt="Новый проект в C# и .NET Core" />
<p>Теперь скомпилируем библиотеку классов. Для этого нажмем правой кнопкой на проект библиотеки классов и в контекстном меню выберем пункт <span class="b">Rebuild</span>:</p>
<img src="./pics/3.21.png" alt="Компиляция библиотеки классов в C# и .NET Core" />
<p>После компиляции библиотеки классов в папке проекта в каталоге <span class="ii">bin/Debug/net6.0</span> мы сможем найти 
скомпилированный файл dll (MyLib.dll). Подключим его в основной проект. Для этого в основном проекте нажмем правой кнопкой на узел <span class="b">Dependencies</span> 
и в контекстном меню выберем пункт <span class="b">Add Project Reference...</span>:</p>
<img src="./pics/3.22.png" alt="Добавление библиотеки классов в проекте на C# и .NET Core" />
<p>Далее нам откроется окно для добавления библиотек. В этом окне выберем пункт Solution,который позволяет увидеть все библиотеки классов из проектов текущего решения, 
поставим отметку рядом с нашей библиотекой и нажмем на кнопку OK:</p>
<img src="./pics/3.23.png" />
<p>Если наша библиотека вдруг представляет файл dll, который не связан ни с каким проектом в нашем решении, то с помощью кнопки <span class="b">Browse</span> 
мы можем найти местоположение файла dll и также его подключить.</p>
<p>После успешного подключения библиотеки в главном проекте изменим файл <span class="b">Program.cs</span>, чтобы он использовал класс Person из библиотеки классов:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./3.53.php">Назад</a><a href="./">Содержание</a><a href="./3.2.php">Вперед</a></p></div></div>
<div class="socBlock">
