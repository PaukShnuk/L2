...
<html >
<head>
<title>C# и .NET | Рефлексия</title>
<meta charset="utf-8" />
<meta name="description" content="Использование рефлексии для получения информации о типах из сборки в языке программирования C# и .NET, класс System.Type и его роль, ключевые классы, применяемые в рефлексии">
...
<h1>Рефлексия</h1><h2>Введение в рефлексию. Класс System.Type</h2><div class="date">Последнее обновление: 17.02.2022</div>
...
<p><span class="b">Рефлексия</span> представляет собой процесс выявления типов во время выполнения приложения. Каждое приложение содержит 
...
как раз и позволяет определить все эти составные элементы приложения. То есть основная задача рефлексии - это исследование типов.</p>
<p>Основной функционал рефлексии сосредоточен в пространстве имен <span class="b">System.Reflection</span>. В нем мы можем выделить следующие основные классы:</p>
<ul>
<li><p><span class="b">Assembly</span>: класс, представляющий сборку и позволяющий манипулировать этой сборкой</p></li>
<li><p><span class="b">AssemblyName</span>: класс, хранящий информацию о сборке</p></li>
<li><p><span class="b">MemberInfo</span>: базовый абстрактный класс, определяющий общий функционал для классов EventInfo, FieldInfo, 
MethodInfo и PropertyInfo</p></li>
<li><p><span class="b">EventInfo</span>: класс, хранящий информацию о событии</p></li>
<li><p><span class="b">FieldInfo</span>: хранит информацию об определенном поле типа</p></li>
<li><p><span class="b">MethodInfo</span>: хранит информацию об определенном методе</p></li>
<li><p><span class="b">PropertyInfo</span>: хранит информацию о свойстве</p></li>
<li><p><span class="b">ConstructorInfo</span>: класс, представляющий конструктор</p></li>
<li><p><span class="b">Module</span>: класс, позволяющий получить доступ к определенному модулю внутри сборки</p></li>
<li><p><span class="b">ParameterInfo</span>: класс, хранящий информацию о параметре метода</p></li>
</ul>
<p>Эти классы представляют составные блоки типа и приложения: методы, свойства и т.д. Но чтобы получить информацию о членах типа, нам надо 
воспользоваться классом <span class="b">System.Type</span>.</p>
<p>Класс <span class="b">Type</span> представляет изучаемый тип, инкапсулируя всю информацию о нем. С помощью его свойств и методов можно получить эту информацию. 
Некоторые из его свойств и методов:</p>
<ul>
<li><p>Метод <span class="b">FindMembers()</span> возвращает массив объектов MemberInfo данного типа</p></li>
<li><p>Метод <span class="b">GetConstructors()</span> возвращает все конструкторы данного типа в виде набора объектов ConstructorInfo</p></li>
<li><p>Метод <span class="b">GetEvents()</span> возвращает все события данного типа в виде массива объектов EventInfo</p></li>
<li><p>Метод <span class="b">GetFields()</span> возвращает все поля данного типа в виде массива объектов FieldInfo</p></li>
<li><p>Метод <span class="b">GetInterfaces()</span> получает все реализуемые данным типом интерфейсы в виде массива объектов Type</p></li>
<li><p>Метод <span class="b">GetMembers()</span> возвращает все члены типа в виде массива объектов MemberInfo</p></li>
<li><p>Метод <span class="b">GetMethods()</span> получает все методы типа в виде массива объектов MethodInfo</p></li>
<li><p>Метод <span class="b">GetProperties()</span> получает все свойства в виде массива объектов PropertyInfo</p></li>
<li><p>Свойство <span class="b">Name</span> возвращает имя типа</p></li>
<li><p>Свойство <span class="b">Assembly</span> возвращает название сборки, где определен тип</p></li>
<li><p>Свойство <span class="b">Namespace</span> возвращает название пространства имен, где определен тип</p></li>
<li><p>Свойство <span class="b">IsArray</span> возвращает true, если тип является массивом</p></li>
<li><p>Свойство <span class="b">IsClass</span> возвращает true, если тип представляет класс</p></li>
<li><p>Свойство <span class="b">IsEnum</span> возвращает true, если тип является перечислением</p></li>
<li><p>Свойство <span class="b">IsInterface</span> возвращает true, если тип представляет интерфейс</p></li>
</ul>
<h3>Получение типа</h3>
<p>Чтобы управлять типом и получать всю информацию о нем, нам надо сперва получить данный тип. Это можно сделать тремя способами: с помощью оператора 
<span class="b">typeof</span>, с помощью метода <span class="b">GetType()</span> класса Object и применяя статический метод <code>Type.GetType()</code>.</p>
<p>Получение типа через <span class="b">typeof</span>:</p>
<pre class="brush:c#;">
...
<p>Здесь определен класс Person с некоторой функциональностью. И чтобы получить его тип, используется выражение <code>Type myType = typeof(Person);</code></p>
<p>Получение типа с помощью метода <span class="b">GetType</span>, унаследованного от класса <code>Object</code>:</p>
<pre class="brush:c#;">
Person tom = new Person("Tom");
Type myType = tom.GetType();
</pre>
<p>В отличие от предыдущего примера здесь, чтобы получить тип, надо создавать объект класса.</p>
<p>И третий способ получения типа - статический метод <span class="b">Type.GetType()</span>:</p>
<pre class="brush:c#;">
Type? myType = Type.GetType("Person", false, true);
</pre>
<p>Первый параметр указывает на полное имя класса с пространством имен. Второй параметр указывает, будет ли генерироваться исключение, если класс не удастся найти. В данном случае значение <code>false</code> означает, 
что исключение не будет генерироваться. И третий параметр указывает, надо ли учитывать регистр символов в первом параметре. Значение <code>true</code> 
означает, что регистр игнорируется. Поскольку указанный тип может отсутствовать, то метод возвращает объект nullable-типа</p>
<p>В данном случае класс основной программы и класс Person находятся в глобальном пространстве имен. Однако если тип располагается в другом пространстве имен, 
то его также надо указать:</p>
<pre class="brush:c#;">
...
<p>В качестве альтернативы можно применять оператор typeof, передавая в него имя типа с указанием пространства имен:</p>
<pre class="brush:c#;">Type myType = typeof(PeopleTypes.Person);</pre>
<p>Если нужный нам тип находится в другой сборке dll, то после полного имени класса через запятую указывается имя сборки:</p>
<pre class="brush:c#;">
 Type myType = Type.GetType("PeopleTypes.Person, MyLibrary", false, true);
</pre>
<p>Теперь исследуем тип и получим некоторую информацию о нем.</p>
<pre class="brush:c#;">
...
<h3>Поиск реализованных интерфейсов</h3>
<p>Чтобы получить все реализованные типом интерфейсы, надо использовать метод <span class="b">GetInterfaces()</span>, который возвращает массив объектов Type:</p>
<pre class="brush:c#;">
Type myType = typeof(Person);
...
<p>Так как каждый интерфейс представляет объект Type, то для каждого полученного интерфейса можно также применить выше рассмотренные методы для извлечения информации о свойствах и методах</p>
<p>Но пока все примеры выше никак не использовали рефлексию. В следующих темах рассмотрим, как можно с помощью рефлексии получать компоненты типа и обращаться к ним, 
например, изменять значения приватных полей класса.</p>
...
<div class="nav"><p><a href="./17.3.php">Назад</a><a href="./">Содержание</a><a href="./14.2.php">Вперед</a></p></div></div>
<div class="socBlock">
