...
<html >
<head>
<title>C# и .NET | Позиционный паттерн</title>
<meta charset="utf-8" />
<meta name="description" content="Pattern matching и Позиционный паттерн в языке программирования C# и .NET, сопоставление со значения по позиции, роль деконструктора класса">
...
<h2>Позиционный паттерн</h2><div class="date">Последнее обновление: 07.01.2022</div>
...
<p>Позиционный паттерн применяется к типу, у которого определен метод деконструктора. Например, определим следующий класс:</p>
<pre class="brush:c#;">
...
<p>Теперь используем позиционный паттерн и в зависимости от значений объекта MessageDetails возвратим определенное сообщение:</p>
<pre class="brush:c#;">
...
<p>Фактически этот паттерн похож на пример с кортежами выше, только теперь вместо кортежа в конструкцию switch передается объект MessageDetails. 
...
<p>Также мы можем взять значения объекта MessageDetails и использовать их при создании результата метода:</p>
<pre class="brush:c#;">
string GetWelcome(MessageDetails details) =&gt; details switch
{
	("english", "morning", _) =&gt; "Good morning",
...
<p>В предпоследней инструкции в конструкции switch получаем по позиции значения из MessageDetails в переменные lang, datetime и status и используем 
их для создания сообщения:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./3.56.php">Назад</a><a href="./">Содержание</a><a href="./3.58.php">Вперед</a></p></div></div>
<div class="socBlock">
