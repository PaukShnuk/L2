...
<html >
<head>
<title>C# и .NET | Область видимости и контекст переменных </title>
<meta charset="utf-8" />
...
<h2>Область видимости (контекст) переменных и констант</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Каждая переменная доступна в рамках определенного контекста или области видимость. Вне этого контекста переменная уже не существует.</p>
<p>Существуют различные контексты:</p>
<ul>
<li><p>Контекст класса. Переменные, определенные на уровне класса, доступны в любом методе этого класса. Их еще называют глобальными переменными или полями<p></li>
<li><p>Контекст метода. Переменные, определенные на уровне метода, являются локальными и доступны только в рамках данного метода. В других методах они недоступны<p></li>
<li><p>Контекст блока кода. Переменные, определенные на уровне блока кода, также являются локальными и доступны только в рамках данного блока. Вне 
своего блока кода они не доступны.<p></li>
</ul>
<p>Например, пусть класс Program определен следующим образом:</p>
<pre class="brush:c#;">
Person tom = new();
tom.PrintName();
tom.PrintSurname();
...
<p>Здесь определенно четыре переменных: type, name, shortName и surname. Каждая из них существует в своем контексте. Переменная <code>type</code> 
существует в контексте всего класса Person и доступна в любом месте и блоке кода в методах PrintName и PrintSurname.</p>
<p>Переменная <code>name</code> существует только в рамках метода PrintName. Также как и переменная <code>surname</code> существует в 
рамках метода PrintSurname. В методе PrintName мы не можем обратиться к переменной <code>surname</code>, так как она в другом контексте.</p>
<p>Переменная <code>shortName</code> существует только в блоке кода, границами которого являются открывающая и закрывающая фигурные 
скобки. Вне его границ переменная shortName не существует и к ней нельзя обратиться.</p>
<p>Нередко границы различных контекстов можно ассоциировать с открывающимися и закрывающимися фигурными скобками, как в данном случае, которые задают пределы блока кода, метода, класса.</p>
<p>При работе с переменными надо учитывать, что локальные переменные, определенные в методе или в блоке кода, скрывают переменные уровня класса, если их имена совпадают:</p>
<pre class="brush:c#;">
class Person 
{
...
<p>При объявлении переменных также надо учитывать, что в одном контексте нельзя определить несколько переменных с одним и тем же именем.</p>
...
<ul>
<li><a title="Вопросы для самопроверки" href="../questions/1.6.php"><i class="fa fa-lg fa-question"></i><span class="dop-txt">Вопросы для самопроверки</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./2.16.php">Назад</a><a href="./">Содержание</a><a href="./3.25.php">Вперед</a></p></div></div>
<div class="socBlock">
