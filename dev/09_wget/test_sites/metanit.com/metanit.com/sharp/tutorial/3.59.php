...
<html >
<head>
<title>C# и .NET | Паттерны списков</title>
<meta charset="utf-8" />
<meta name="description" content="Паттерны списков (list pattern) в Pattern matching в языке программирования C# и .NET, slice-pattern, сопоставление массивов и списков">
...
<h2>Паттерны списков</h2><div class="date">Последнее обновление: 27.02.2022</div>
...
<p>Паттерны списков (list pattern) позволяют сопоставлять выражения со списками и массивами.  Данный паттерн пока доступен в Preview-версии 
C# 11.</p>
<p>Полное совпадение с массивом/списком:</p>
<pre class="brush:c#;">
...
<p>Аналогично вместо массивов можно применять списки:</p>
<pre class="brush:c#;">
...
<p>Аналогичным образом паттерны списков можно использовать в конструкции <span class="b">if</span>:</p>
<pre class="brush:c#;">
...
<h4>Подстановка _</h4>
<p>С помощью паттерна <span class="b">_</span> можно обозначить одиночный элемент, который имеет любое значение. Например, паттерн 
<span class="b">[2, _, 5]</span> соответствует любому массиву из трех элементов, в котором между 2 и 5 располагается произвольное значение. 
А массив <span class="b">[_, _]</span> соответствует любому массиву из двух произвольных элементов</p>
...
<h4>slice-паттерн</h4>
<p>Для передачи произвольного количества элементов массива/списка применяется slice-паттерн <span class="b">..</span>. 
Например, паттерн <span class="b">[1, 2, .., 5]</span> 
...
произвольных целых чисел. То есть паттерн <span class="b">[1, 2, .., 5]</span> будет соответствовать таким массивам как</p>
<pre class="brush:c#;">
int[] arr1 = {  1, 2, 3, 4, 5 };
int[] arr2 = { 1, 2, 5 };
int[] arr3 = { 1, 2, 66, 77, 88, 5 };
</pre>
<p>С помощью паттерна <span class="b">..</span> можно задавать произвольное количество элементов как в начале, так и в конце массива/списка. 
Например, паттерн <span class="b">[2,..]</span> представляет массив, который начинается на 2. А паттерн <span class="b">[.., 5]</span> 
представляет массив, который заканчивается элементом 5. Паттерн <span class="b">[..]</span> представляет массив, который содержит 
произвольное количество элементов. Например:</p>
<pre class="brush:c#;">
...
<p>slice-паттерн можно сочетать с символов подстановки _, например:</p>
<pre class="brush:c#;">
int GetNumber(int[] values) =&gt; values switch
{
    [_, .., _] =&gt; 1,
...
<p>В данном случае паттерн <span class="b">[_, .., _]</span> предполагает массив, который состоит как минимум из двух произвольных элементов, и между первым и последним 
элементром может находиться произвольное количество других элементов:</p>
<pre class="brush:c#;">
...
<h3>Получение элементов в переменные</h3>
<p>Отдельные значения массива/списка можно получить в переменные, например:</p>
<pre class="brush:c#;">
int[] numbers = { 2, 3, 5 };
...
<p>В данном случае получаем первый элемент массива в переменную first, второй элемент - в переменную second, а последний элемент - в 
переменную last.</p>
<p>Пример с различными массивами:</p>
<pre class="brush:c#;">
Console.WriteLine(GetData(new[] { 1, 2, 3 }));        // First: 1  Second: 2  Last: 3
Console.WriteLine(GetData(new[] { 2, 4, 6, 8 }));    // First: 2  Second: 4  Last: 8
Console.WriteLine(GetData(new[] { 1, 2 }));          // First: 1  Second: 2  Last: 3
...
<p>В данном случае получаем первый элемент массива в переменную first, второй элемент - в переменную second, а последний элемент - в 
переменную last.</p>
<p>При этом значения, которые проектируются на паттерн <span class="b">..</span>, также можно получить в переменную. Например, в паттерне 
<span class="b">[2, .. var middle, 5]</span> элементы, которые проектируются на <span class="b">..</span>, можно передаются в переменную 
middle. Несколько примеров:</p>
<pre class="brush:c#;">
...
<h3>Свойства коллекций</h3>
<p>Стоит отметить, что, поскольку массивы и списки - обычные классы C#, которые имеют свойства, то для них мы также можем применять 
...
если массив имеет три элемента, то разложить его на три переменных:</p>
<pre class="brush:c#;">
int[] numbers = { 2, 3, 5 };
...
<div class="nav"><p><a href="./3.58.php">Назад</a><a href="./">Содержание</a><a href="./4.5.php">Вперед</a></p></div></div>
<div class="socBlock">
