...
<html >
<head>
<title>C# и NET | Определение операторов</title>
<meta charset="utf-8" />
<meta name="description" content="Определение и перегрузка операторов в классах и структурах C#, operator overloading, правила перегрузки унарных и бинарных операторов">
...
<h1>Дополнительные возможности ООП в C#</h1><h2>Определение операторов</h2><div class="date">Последнее обновление: 28.12.2021</div>
...
<p>Наряду с методами в классах и структурах мы можем также определять операторы. Например, пусть у нас есть следующий класс Counter:</p>
<pre class="brush:c#;">
class Counter
{
...
<p>Данный класс представляет некоторый счетчик, значение которого хранится в свойстве Value.</p>
<p>И допустим, у нас есть два объекта класса Counter - два счетчика, которые мы хотим сравнивать или складывать на основании их свойства Value, 
используя стандартные операции сравнения и сложения:</p>
<pre class="brush:c#;">
...
<p>Но на данный момент ни операция сравнения, ни операция сложения для объектов Counter не доступны. Эти операции могут использоваться 
...
классов и структур компилятор не знает. И для этого нам надо выполнить перегрузку нужных нам операторов.</p>
<p>Определение операторов заключается в определении в классе, для объектов которого мы хотим определить оператор, специального метода:</p>
<pre class="brush:c#;">
public static возвращаемый_тип operator оператор(параметры)
{  }
</pre>
<p>Этот метод должен иметь модификаторы <span class="b">public static</span>, так как перегружаемый оператор будет 
...
получить объект типа bool, который указывает истинно ли условное выражение или ложно. Но в зависимости от задачи возвращаемые типы могут быть любыми.</p>
<p>Затем вместо названия метода идет ключевое слово <span class="b">operator</span> и собственно сам оператор. И далее в скобках перечисляются 
параметры. Бинарные операторы принимают два параметра, унарные - один параметр. И в любом случае один из параметров должен 
представлять тот тип - класс или структуру, в котором определяется оператор.</p>
<p>Например, перегрузим ряд операторов для класса Counter:</p>
<pre class="brush:c#;">
class Counter
{
	public int Value { get; set; }
...
<p>Поскольку все определенные операторы - бинарные - то есть проводятся над двумя объектами, то для каждой перегрузки предусмотрено 
по два параметра.</p>
<p>Так как в случае с операцией сложения мы хотим сложить два объекта класса Counter, то оператор принимает два объекта этого класса. И так как мы хотим в результате сложения 
...
которого объединяет значения свойства Value обоих параметров:</p>
<pre class="brush:c#;">
public static Counter operator +(Counter counter1, Counter counter2)
{
	return new Counter { Value = counter1.Value + counter2.Value };
...
<p>Также определены две операции сравнения. Если мы определяем одну из этих операций сравнения, то мы также должны определить вторую из этих операций. 
Сами операторы сравнения сравнивают значения свойств Value и в зависимости от результата сравнения возвращают либо true, либо false.</p>
<p>Теперь используем перегруженные операторы в программе:</p>
<pre class="brush:c#;">
Counter counter1 = new Counter { Value = 23 };
Counter counter2 = new Counter { Value = 45 };
bool result = counter1 &gt; counter2;
...
<p>Стоит отметить, что так как по сути определение оператора представляет собой метод, то этот метод мы также можем перегрузить, то есть 
создать для него еще одну версию. Например, добавим в класс Counter еще один оператор:</p>
<pre class="brush:c#;">
...
<p>Данный метод складывает значение свойства Value и некоторое число, возвращая их сумму. И также мы можем применить этот оператор:</p>
<pre class="brush:c#;">
Counter counter1 = new Counter { Value = 23 };
int result = counter1 + 27; // 50
Console.WriteLine(result);
</pre>
<p>Следует учитывать, что не все операторы можно определить. 
В частности, мы можем определить логику для следующих операторов:</p>
<ul>
<li><p>унарные операторы +x, -x, !x, ~x, ++, --, true, false 	</p></li>
<li><p>бинарные операторы +, -, *, /, %</p></li>
<li><p>операции сравнения  ==, !=, &lt;, &gt;, &lt;=, &gt;=</p></li>
<li><p>поразрядные операторы  &, |, ^, &lt;&lt;, &gt;&gt;</p></li>
<li><p>логические операторы &&, ||</p></li>
</ul>
<p>Кроме того, есть несколько операторов, которые надо определять парами:</p>
<ul>
<li><p>== и !=</p></li>
<li><p>&lt; и &gt;</p></li>
<li><p>&lt;= и &gt;=</p></li>
</ul>
<p>И есть ряд операторов, которые нельзя перегрузить, например, операцию равенства <code>=</code> или тернарный оператор <code>?:</code>, а также ряд других. Полный список перегружаемых операторов можно найти в <a href="https://docs.microsoft.com/en-us/dotnet/csharp/language-reference/operators/operator-overloading#overloadable-operators" rel="nofollow">документации msdn</a></p>
<h3>Определение инкремента и декремента</h3>
<p>Следует учитывать, что в коде оператора не должны изменяться те объекты, которые передаются в оператор через параметры. Например, мы можем определить для класса 
Counter оператор инкремента:</p>
<pre class="brush:c#;">
...
<p>Поскольку оператор унарный, он принимает только один параметр - объект того класса, в котором данный оператор определен. Но это 
<span class="b">неправильное</span> определение инкремента, так как оператор не должен менять значения своих параметров.</p>
<p>И более корректное определение оператора инкремента будет выглядеть так:</p>
<pre class="brush:c#;">
public static Counter operator ++(Counter counter1)
{
...
<p>То есть возвращается новый объект, который содержит в свойстве Value инкрементированное значение. При этом нам не надо определять отдельно операторы для префиксного и для постфиксного инкремента (а также декремента), так как одна реализация 
будет работать в обоих случаях.</p>
<pre class="brush:c#;">
...
<p>При операции постфиксного инкремента (counter1++) компилятор сначала создает временную переменную, в которую сохраняет текущий объект. Затем текущий объект замещает значением, полученным из функции оператора. 
В качестве результата операции возвращается значение временной переменной. При префиксном инкременте (++counter1) компилятор возвращает новое значение, полученное из функции оператора.</p>
<h3>Определение операций true и false</h3>
<p>Отдельно стоит отметить определение операторов <span class="b">true</span> и <span class="b">false</span>. Эти операторы определяются, когда мы хотим использовать объект типа в качестве условия. 
Например, определим данные операторы в классе Counter:</p>
<pre class="brush:c#;">
class Counter
{
	public int Value { get; set; }
...
<p>Также стоит отметить, что если мы хотим использовать операцию отрицания, типа <code>if (!counter)</code>, то нам также необходимо определить для типа операцию <span class="b">!</span>:</p>
<pre class="brush:c#;">
...
<p>Операция отрицания фактически синонимична операции <code>false</code>, поэтому содержит аналогичное условие.</p>
...
<ul>
<li><a title="Вопросы для самопроверки" href="../questions/2.15.php"><i class="fa fa-lg fa-question"></i><span class="dop-txt">Вопросы для самопроверки</span></a></li>
<li><a title="Упражнения" href="../practice/2.4.php"><i class="fa fa-lg fa-pencil"></i><span class="dop-txt">Упражнения</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./3.27.php">Назад</a><a href="./">Содержание</a><a href="./3.37.php">Вперед</a></p></div></div>
<div class="socBlock">
