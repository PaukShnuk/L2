...
<html >
<head>
<title>C# и .NET | Копирование объектов. Интерфейс ICloneable | </title>
<meta charset="utf-8" />
<meta name="description" content="Клонирование объектов в C#, применение интерфейса ICloneable, глубокое и неглубокое копирование, метод MemberwiseClone">
...
<h2>Копирование объектов. Интерфейс ICloneable</h2><div class="date">Последнее обновление: 02.01.2022</div>
...
<p>Поскольку классы представляют ссылочные типы, то это накладывает некоторые ограничения на их использование. В частности, допустим, у нас есть 
следующий класс:</p>
<pre class="brush:c#;">
class Person
{
...
<p>Чтобы переменная ищи указывала на новый объект, но со значениями из tom, мы можем применить клонирование с помощью реализации интерфейса <span class="b">ICloneable</span>:</p>
<pre class="brush:c#;">
...
<p>Для сокращения кода копирования мы можем использовать специальный метод <span class="b">MemberwiseClone()</span>, который возвращает копию объекта:</p>
<pre class="brush:c#;">
class Person : ICloneable
{
    public string Name { get; set; }
...
<p>Этот метод реализует <span class="b">поверхностное (неглубокое) копирование</span>. Однако данного копирования может быть недостаточно. Например, 
...
надо применять <span class="b">глубокое копирование</span>:</p>
<pre class="brush:c#;">
class Person : ICloneable
{
    public string Name { get; set; }
...
<div class="nav"><p><a href="./3.40.php">Назад</a><a href="./">Содержание</a><a href="./3.23.php">Вперед</a></p></div></div>
<div class="socBlock">
