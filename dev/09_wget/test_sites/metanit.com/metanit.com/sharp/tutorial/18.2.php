...
<html >
<head>
<title>C# и .NET | Домены приложений</title>
<meta charset="utf-8" />
<meta name="description" content="Домены приложений в C# и .NET, класс AppDomain и его методы и свойства, получение всех загруженных сборок и текущего каталога приложения">
...
<h2>Домены приложений</h2><div class="date">Последнее обновление: 22.02.2022</div>
...
<p>При запуске приложения, написанного на C#, операционная система создает процесс, а среда CLR создает внутри этого процесса логический контейнер, который называется 
<span class="b">доменом приложения</span> и внутри которого работает запущенное приложение.</p>
<p>Для управления домена платформа .NET предоставляет класс <span class="b">AppDomain</span>. Рассмотрим некоторые основные методы и свойства данного класса:</p>
<ul>
<li><p>Свойство <span class="b">BaseDirectory</span>: базовый каталог, который используется для получения сборок (как правило, каталог самого приложения)</p></li>
<li><p>Свойство <span class="b">CurrentDomain</span>: домен текущего приложения</p></li>
<li><p>Свойство <span class="b">FriendlyName</span>: имя домена приложения</p></li>
<li><p>Свойство <span class="b">SetupInformation</span>: представляет объект <code>AppDomainSetup</code> и хранит конфигурацию домена приложения</p></li>
<li><p>Метод <span class="b">ExecuteAssembly()</span>: запускает сборку exe в рамках текущего домена приложения</p></li>
<li><p>Метод <span class="b">GetAssemblies()</span>: получает набор сборок .NET, загруженных в домен приложения</p></li>
</ul>
<p>Получим имя и базовый каталог текущего домена и выведем все загруженные в домен сборки:</p>
<pre class="brush:c#;">
