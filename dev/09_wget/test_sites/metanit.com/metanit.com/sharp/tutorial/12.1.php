...
<html >
<head>
<title>C# и .NET | Параллельное программирование и библиотека TPL</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в параллельное программирование и библиотеку TPL в языке C# и .NET. Класс Task, создание задач и их использование">
...
<h1>Параллельное программирование и библиотека TPL</h1><h2>Задачи и класс Task</h2><div class="date">Последнее обновление: 08.02.2022</div>
...
<p>В эпоху многоядерных машин, которые позволяют параллельно выполнять сразу несколько процессов, стандартных средств работы с потоками в .NET 
...
основной функционал которой располагается в пространстве имен <span class="b">System.Threading.Tasks</span>. 
...
Поэтому обычно рекомендуется использовать именно TPL и ее классы для создания многопоточных приложений, хотя стандартные средства и класс Thread по-прежнему находят широкое применение.</p>
<p>В основе библиотеки TPL лежит концепция задач, каждая из которых описывает отдельную продолжительную операцию. 
В библиотеке классов .NET задача представлена специальным классом  - классом <span class="b">Task</span>, который находится в пространстве 
имен <span class="b">System.Threading.Tasks</span>. Данный класс описывает отдельную задачу, которая запускается асинхронно в одном из 
потоков из пула потоков. Хотя ее также можно запускать синхронно в текущем потоке.</p>
<p>Для определения и запуска задачи можно использовать различные способы.</p>
<ul>
<li>
<p>Первый способ создание объекта Task и вызов у него метода Start:</p>
<pre class="brush:c#;">
Task task = new Task(() =&gt; Console.WriteLine("Hello Task!"));
task.Start();
</pre>
<p>В качестве параметра объект Task принимает делегат Action, то есть мы можем передать любое действие, которое соответствует данному делегату, 
например, лямбда-выражение, как в данном случае, или ссылку на какой-либо метод. То есть в данном случае при выполнении задачи на консоль будет выводиться строка "Hello Task!".</p>
<p>А метод <code>Start()</code> собственно запускает задачу.</p>
</li>
<li><p>Второй способ заключается в использовании статического метода <span class="b">Task.Factory.StartNew()</span>. Этот метод также в качестве 
параметра принимает делегат Action, который указывает, какое действие будет выполняться. При этом этот метод сразу же запускает задачу:</p>
<pre class="brush:c#;">
Task task = Task.Factory.StartNew(() =&gt; Console.WriteLine("Hello Task!"));
</pre>
<p>В качестве результата метод возвращает запущенную задачу.</p></li>
<li><p>Третий способ определения и запуска задач представляет использование статического метода <span class="b">Task.Run()</span>:</p>
<pre class="brush:c#;">
Task task = Task.Run(() =&gt; Console.WriteLine("Hello Task!"));
</pre>
<p>Метод <code>Task.Run()</code> также в качестве параметра может принимать делегат Action - выполняемое действие и возвращает объект Task.</p>
</li>
</ul>
<p>Определим небольшую программу, где используем все эти способы:</p>
<pre class="brush:c#;">
...
<p>Итак, в данном коде задачи создаются и запускаются, но при выполнении приложения на консоли мы можем не увидеть ничего. Почему? 
...
до того, как все три или даже хотя бы одна из трех задач начнет выполнение. Чтобы этого не произошло, мы можем программным образом ожидать завершения задачи.</p>
<h3>Ожидание завершения задачи</h3>
<p>Чтобы приложение ожидало завершения задачи, можно использовать метод <span class="b">Wait()</span> объекта Task:</p>
<pre class="brush:c#;">
Task task1 = new Task(() =&gt; Console.WriteLine("Task1 is executed"));
task1.Start();
...
<p>Консольный вывод не детерминирован, поскольку задачи не выполняются последовательно. Первая запущенная задача может завершить свое выполнение после последней задачи.</p>
<p>Стоит отметить, что метод <span class="b">Wait()</span> блокирует вызывающий поток, в котором запущена задача, пока эта задача не завершит свое выполнение. 
Например:</p>
<pre class="brush:c#;">
...
<p>Для эмуляции долговременной работы здесь в задаче task1 устанавливается задержка на 1 секунду. В итоге, когда выполнение дойдет до вызова <code>task1.Wait()</code> 
основной поток остановит свое выполнение и будет ждать завершения задачи. И мы получим следующий консольный вывод:</p>
<div class="console">
<pre class="consoletext">
Main Starts
Task Starts
...
<p>Если подобное поведение не принципиально, то ожидание завершения задачи можно поместить в конец метода Main:</p>
<pre class="brush:c#;">
Console.WriteLine("Main Starts");
// создаем задачу
Task task1 = new Task(() =&gt;
...
<p>В этом случае приложение все равно будет ждать завершения задачи, однако другие синхронные действия в основном потоке не будут блокироваться и ожидать завершения задачи.</p>
<h3>Синхронный запуск задачи</h3>
<p>По умолчанию задачи запускаются асинхронно. Однако с помощью метода <span class="b">RunSynchronously()</span> можно запускать синхронно:</p>
<pre class="brush:c#;">
Console.WriteLine("Main Starts");
// создаем задачу
Task task1 = new Task(() =&gt;
...
<h3>Свойства класса Task</h3>
<p>Класс Task имеет ряд свойств, с помощью которых мы можем получить информацию об объекте. Некоторые из них:</p>
<ul>
<li><p><span class="b">AsyncState</span>: возвращает объект состояния задачи</p></li>
<li><p><span class="b">CurrentId</span>: возвращает идентификатор текущей задачи (статическое свойство)</p></li>
<li><p><span class="b">Id</span>: возвращает идентификатор текущей задачи</p></li>
<li><p><span class="b">Exception</span>: возвращает объект исключения, возникшего при выполнении задачи</p></li>
<li><p><span class="b">Status</span>: возвращает статус задачи. Представляет перечисление <code>System.Threading.Tasks.TaskStatus</code>, которое имеет следующие значения:</p>
<ul>
<li><p><code>Canceled</code>: задача отменена</p></li>
<li><p><code>Created</code>: задача создана, но еще не запущена</p></li>
<li><p><code>Faulted</code>: в процессе работы задачи произошло исключение</p></li>
<li><p><code>RanToCompletion</code>: задача успешно завершена</p></li>
<li><p><code>Running</code>: задача запущена, но еще не завершена</p></li>
<li><p><code>WaitingForActivation</code>: задача ожидает активации и постановки в график выполнения</p></li>
<li><p><code>WaitingForChildrenToComplete</code>: задача завершена и теперь ожидает заврешения прикрепленных к ней дочерних задач</p></li>
<li><p><code>WaitingToRun</code>: задача поставлена в график выполнения, но еще не начала свое выполнение</p></li>
</ul>
</li>
<li><p><span class="b">IsCompleted</span>: возвращает true, если задача завершена</p></li>
<li><p><span class="b">IsCanceled</span>: возвращает true, если задача была отменена</p></li>
<li><p><span class="b">IsFaulted</span>: возвращает true, если задача завершилась при возникновении исключения</p></li>
<li><p><span class="b">IsCompletedSuccessfully</span>: возвращает true, если задача завершилась успешно</p></li>
</ul>
<p>Используем некоторые из этих свойств:</p>
<pre class="brush:c#;">
Task task1 = new Task(() =&gt;
{
...
<div class="nav"><p><a href="./11.8.php">Назад</a><a href="./">Содержание</a><a href="./12.2.php">Вперед</a></p></div></div>
<div class="socBlock">
