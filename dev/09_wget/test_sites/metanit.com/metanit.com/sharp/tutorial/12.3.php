...
<html >
<head>
<title>C# и .NET | Задачи продолжения</title>
<meta charset="utf-8" />
<meta name="description" content="Использование задач продолжения (continuation task) в языке программирования C# и .NET, метод ContinueWith, последовательное выполнения цепочки задач Task, получение данных из предыдущей задачи">
...
<h2>Задачи продолжения</h2><div class="date">Последнее обновление: 08.02.2022</div>
...
<p>Задачи продолжения или <span class="b">continuation task</span> позволяют определить задачи, которые выполняются после завершения других задач. 
Благодаря этому мы можем вызвать после выполнения одной задачи несколько других, определить условия их вызова, передать из предыдущей задачи в следующую некоторые данные.</p>
<p>Задачи продолжения похожи на методы обратного вызова, но фактически являются обычными задачами Task. Посмотрим на примере:</p>
<pre class="brush:c#;">
Task task1 = new Task(() =&gt;
{
...
<p>Первая задача задается с помощью лямбда-выражения, которое просто выводит id этой задачи. Вторая задача - задача продолжения задается 
с помощью метода <span class="b">ContinueWith</span>, который в качестве параметра принимает делегат <code>Action&lt;Task&gt;</code>. То есть 
метод PrintTask, который передается в вызов ContinueWith, должен принимать параметр типа Task.</p>
<p>Благодаря передачи в метод параметра Task, мы можем получить различные свойства предыдущей задачи, как например, в данном случае получает ее Id.</p>
<p>И после завершения задачи task1 сразу будет вызываться задача task2. Консольный вывод программы:</p>
<div class="console">
...
<p>Также мы можем передавать конкретный результат работы предыдущей задачи:</p>
<pre class="brush:c#;">
...
<p>В данном случае задача sumTask выполняет метод Sum и возвращает его результат. Задача printTask является задачей продолжения, выполняется сразу после 
sumTask и получает ее результат. Так, в вызове</p>
<pre class="brush:c#;">Task printTask = sumTask.ContinueWith(task =&gt; PrintResult(task.Result));</pre>
<p>Параметр task в лямбда-выражении фактически представляет задачу sumTask, из которой извлекается результат.</p>
<p>Подобным образом можно построить целую цепочку последовательно выполняющихся задач:</p>
<pre class="brush:c#;">
...
<p>здесь друг за другом выполняются задачи task1, task2, task3, task4. Консольный вывод программы:</p>
<div class="console">
<pre class="consoletext">
Current Task: 1
