...
<html >
<head>
<title>C# и .NET | Работа с датами и временем</title>
<meta charset="utf-8" />
<meta name="description" content="Структура DateTime в C# и .NET, работа с датами и временем">
...
<h1>Работа с датами и временем</h1><h2>Структура DateTime</h2><div class="date">Последнее обновление: 06.01.2022</div>
...
<p>Для работы с датами и временем в .NET предназначена структура <span class="b">DateTime</span>. Она представляет дату и время от 00:00:00 1 января 0001 года 
до 23:59:59 31 декабря 9999 года.</p>
<p>Для создания нового объекта DateTime также можно использовать конструктор. Пустой конструктор создает начальную дату:</p>
<pre class="brush:c#;">
DateTime dateTime = new DateTime();
Console.WriteLine(dateTime); // 01.01.0001 0:00:00
</pre>
<p>То есть мы получим минимально возможное значение, которое также можно получить следующим образом:</p>
<pre class="brush:c#;">
Console.WriteLine(DateTime.MinValue);
</pre>
<p>Чтобы задать конкретную дату, нужно использовать один из конструкторов, принимающих параметры:</p>
<pre class="brush:c#;">
...
<p>Если необходимо получить текущую время и дату, то можно использовать ряд свойств DateTime:</p>
<pre class="brush:c#;">
...
<p>Свойство <code>DateTime.Now</code> берет текущую дату и время компьютера, <code>DateTime.UtcNow</code> - дата и время относительно времени по 
Гринвичу (GMT) и <code>DateTime.Today</code> - только текущая дата.</p>
<p>При работе с датами надо учитывать, что по умолчанию для представления дат применяется григорианский календарь. Но что будет, если мы захотим получить день недели для 
5 октября 1582 года:</p>
<pre class="brush:c#;">
DateTime someDate = new DateTime(1582, 10, 5);
Console.WriteLine(someDate.DayOfWeek);</pre>
<p>Консоль высветит значение Tuesday, то есть вторник. Однако, как может быть известно из истории, впервые переход с юлианского календаря на григорианский 
...
Таким образом, фактически выкинули 10 дней. То есть после 4 октября шло 15 октября.</p>
<p>В большинстве случаев данный факт вряд ли как-то повлияет на вычисления, однако при работе с очень давними датами данный аспект следует учитывать.</p>
<h3>Операции с DateTime</h3>
<p>Основные операции со структурой DateTime связаны со сложением или вычитанием дат. Например, надо к некоторой дате прибавить или, наоборот, 
отнять несколько дней.</p>
<p>Для добавления дат используется ряд методов:</p>
<ul>
<li><p><code>Add(TimeSpan value)</code>: добавляет к дате значение TimeSpan<p></li>
<li><p><code>AddDays(double value)</code>: добавляет к текущей дате несколько дней<p></li>
<li><p><code>AddHours(double value)</code>: добавляет к текущей дате несколько часов<p></li>
<li><p><code>AddMinutes(double value)</code>: добавляет к текущей дате несколько минут<p></li>
<li><p><code>AddMonths(int value)</code>: добавляет к текущей дате несколько месяцев<p></li>
<li><p><code>AddYears(int value)</code>: добавляет к текущей дате несколько лет<p></li>
</ul>
<p>Например, добавим к некоторой дате 3 часа:</p>
<pre class="brush:c#;">
DateTime date1 = new DateTime(2015, 7, 20, 18, 30, 25); // 20.07.2015 18:30:25
Console.WriteLine(date1.AddHours(3)); // 20.07.2015 21:30:25
</pre>
<p>Для вычитания дат используется метод <span class="b">Subtract(DateTime date)</span>:</p>
<pre class="brush:c#;">
DateTime date1 = new DateTime(2015, 7, 20, 18, 30, 25); // 20.07.2015 18:30:25
DateTime date2 = new DateTime(2015, 7, 20, 15, 30, 25); // 20.07.2015 15:30:25
Console.WriteLine(date1.Subtract(date2)); // 03:00:00
</pre>
<p>Здесь даты различаются на три часа, поэтому результатом будет дата "03:00:00".</p>
<p>Метод Substract не имеет возможностей для отдельного вычитания дней, часов и так далее. Но это и не надо, так как мы можем передавать в метод 
AddDays() и другие методы добавления отрицательные значения:</p>
<pre class="brush:c#;">
// вычтем три часа
DateTime date1 = new DateTime(2015, 7, 20, 18, 30, 25);  // 20.07.2015 18:30:25
Console.WriteLine(date1.AddHours(-3)); // 20.07.2015 15:30:25
</pre>
<p>Кроме операций сложения и вычитания еще есть ряд методов форматирования дат:</p>
<pre class="brush:c#;">
...
<p>Метод <code>ToLocalTime()</code> преобразует время UTC в локальное время, добавляя смещение относительно времени по Гринвичу. 
Метод <code>ToUniversalTime()</code>, наоборот, преобразует локальное время во время UTC, то есть вычитает смещение относительно времени по Гринвичу. 
Остальные методы преобразуют дату к определенному формату.</p>
...
<div class="nav"><p><a href="./7.4.php">Назад</a><a href="./">Содержание</a><a href="./19.2.php">Вперед</a></p></div></div>
<div class="socBlock">
