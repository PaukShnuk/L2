...
<html >
<head>
<title>C# и .NET | Паттерны кортежей</title>
<meta charset="utf-8" />
<meta name="description" content="Pattern matching и Паттерны кортежей в языке программирования C# и .NET, сопоставление кортежей">
...
<h2>Паттерны кортежей</h2><div class="date">Последнее обновление: 07.01.2022</div>
...
<p>Паттерны кортежей позволяют сравнивать значения кортежей. Например, передадим конструкци switch кортеж с названием языка и времени дня и в 
зависимости от переданных данных возвратим определенное сообщение:</p>
<pre class="brush:c#;">
...
<p>Здесь в метод передаются два значения, из которых создается кортеж (можно и сразу передать в метод кортеж). Далее в конструкции switch с 
помощью круглых скобок определяются значения, которым должны соответствовать элементы кортежа. Например, выражение <code>("english", "morning") =&gt; "Good morning"</code> 
будет выполняться, если одновременно lang="english" и datetime="morning".</p>
<p>Применение:</p>
<pre class="brush:c#;">
...
<p>Нам не обязательно сравнивать все значения кортежа, мы можем использовать только некоторые элементы кортежа. В случае, если мы не хотим использовать 
элемент кортежа, то вместо него ставим прочерк:</p>
<pre class="brush:c#;">
...
<p>Теперь кортеж состоит из трех элементов. Но первые четыре выражения не используют последний элемент кортежа, допустим, он не важен, 
поэтому вместо него ставится прочерк <code>("english", "morning", _)</code>.</p>
<p>А в предпоследнем примере, наоборот, не важны первые два элемента, а важен третий элемент: <code>(_, _, "admin") =&gt;</code>.</p>
<p>Но в любом случае нам надо указать конкретные значения или прочерки для всех элементов кортежа.</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./3.55.php">Назад</a><a href="./">Содержание</a><a href="./3.57.php">Вперед</a></p></div></div>
<div class="socBlock">
