...
<html >
<head>
<title>C# и .NET | Паттерн свойств</title>
<meta charset="utf-8" />
<meta name="description" content="Pattern matching и Паттерн свойств в языке программирования C# и .NET, сопоставление объекта со свойствами, свойства вложенных объектов">
...
<h2>Паттерн свойств</h2><div class="date">Последнее обновление: 07.01.2022</div>
...
<p>Паттерн свойств позволяет сравнивать со значениями определенных свойств объекта. Например, пусть у нас будет следующий класс:</p>
<pre class="brush:c#;">
class Person
{
...
<p>Например, в зависимости от языка пользователя выведем ему определенное сообщение, применив паттерн свойств:</p>
<pre class="brush:c#;">
...
<p>Здесь метод SayHello в качестве параметра принимает объект Person и сопоставляет его с некоторым паттерном. В качестве паттерна выступает выражение 
<code>Person { Language: "french" }</code>. То есть параметр person должен представлять объект Person, у которого значение свойства Language равно "french".</p>
<p>При этом можно задействовать набор свойств. Например, добавим проверку по свойству Status:</p>
<pre class="brush:c#;">
Person tom = new Person { Language = "english", Status = "user", Name = "Tom" };
Person pierre = new Person { Language = "french", Status = "user", Name = "Pierre" };
...
<p>Теперь выражение if проверяет, соответствует ли параметр person объекту Person, у которого свойства Language и Status имеют определенные значения.</p>
<p>Подобным образом можно применять паттерн свойств в конструкции switch:</p>
<pre class="brush:c#;">
...
<p>Паттерны свойств предполагают использование фигурных скобок, внутри которых указываются свойства и через двоеточие их значение <code>{свойство: значение}</code>. 
И со значением свойства в фигурных скобках сравнивается свойство передаваемого объекта. При этом в фигурных скобках мы можем указать несколько свойств и их значений <code>{ Language: "german", Status: "admin" }</code> - тогда 
свойства передаваемого объекта должны соответствовать всем этим значениям.</p>
<p>Можно оставить пустые фигурные скобки, как в последнем случае <code>{ } =&gt; "undefined!"</code> - передаваемый объект будет 
соответствовать пустым фигурным скобкам, если он не соответствует всем предыдущим значениям, или например, если его свойства не указаны или имеют значение null.</p>
<p>То есть в данном случае, если у объекта Person p выполняется равенство Language = "english", будет возвращаться строка "Hello!".</p>
<p>Если у объекта Person p одновременно выполняются два равенства Language = "german" и Role="admin", 
будет возвращаться строка "Hallo, admin!".</p>
<p>Если у объекта Person p выполняется равенство Language = "french", будет возвращаться строка "Salut!".</p>
<p>Если объект Person будет сопоставляться с пустыми фигурными скобками <code>{}</code>, и будет возвращаться строка "undefined".</p>
<p>Последняя проверка проверяет значение на null.</p>
<p>Применение:</p>
<pre class="brush:c#;">
Person pierre = new Person { Language = "french", Status = "user", Name = "Pierre" };
...
<p>Кроме того, мы можем определять в паттерных свойств переменные, передавать этим переменным значения объекта и использовать при возвращении значения:</p>
<pre class="brush:c#;">
string GetMessage(Person? p) =&gt; p switch
{
	{ Language: "german", Status: "admin" } =&gt; "Hallo, admin!",
...
<p>Так, подвыражение <code>Name: var name</code> говорит, что надо передать в переменную name значение свойства Name. Затем ее можно применить 
при генерации выходного значения: <code>=&gt; $"Salut, {name}!"</code></p>
<p>Применение:</p>
<pre class="brush:c#;">
Person pierre = new Person { Language = "french", Status = "user", Name = "Pierre" };
string message = GetMessage(pierre);
...
<p>Стоит отметить, что начиная с версии C# 10 было упрощено сопоставление со свойствами вложенных объектов. Допустим, у нас есть следующие классы:</p>
<pre class="brush:c#;">
class Employee
{
...
<p>Класс Company определяет свойство Title, которое хранит название компании. Класс Employee определяет сотрудника компании и в 
свойстве Company хранит компанию. Применим паттерн свойств на основе свойств вложенного объекта Company:</p>
<pre class="brush:c#;">
...
<p>В методе PrintCompany объект employee сопоставляется с паттерном <code>Employee { Company:{Title: "Microsoft" } }</code>. То есть 
сотрудник компании должен представлять объект Employee, у которого название компании равно "Microsoft"</p>
<p>Однако мы также можем сократить данный паттерн следующим образом:</p>
<pre class="brush:c#;">
void PrintCompany(Employee employee)
{
    if (employee is Employee { Company.Title: "Microsoft" })
...
<div class="nav"><p><a href="./3.34.php">Назад</a><a href="./">Содержание</a><a href="./3.56.php">Вперед</a></p></div></div>
<div class="socBlock">
