...
<html >
<head>
<title>C# и .NET | Изменение документа XML</title>
<meta charset="utf-8" />
<meta name="description" content="Изменение документа XML в языке программирования C# и .NET с помощью методов класса XmlNode, добавление, изменение и удаление узлов в файле xml">
...
<h2>Изменение XML-документа</h2><div class="date">Последнее обновление: 20.02.2022</div>
...
<p>Для редактирования xml-документа (изменения, добавления, удаления элементов) мы можем воспользоваться методами класса <span class="b">XmlNode</span>:</p>
<ul>
<li><p><span class="b">AppendChild</span>: добавляет в конец текущего узла новый дочерний узел</p></li>
<li><p><span class="b">InsertAfter</span>: добавляет новый узел после определенного узла</p></li>
<li><p><span class="b">InsertBefore</span>: добавляет новый узел до определенного узла</p></li>
<li><p><span class="b">RemoveAll</span>: удаляет все дочерние узлы текущего узла</p></li>
<li><p><span class="b">RemoveChild</span>: удаляет у текущего узла один дочерний узел и возвращает его</p></li>
</ul>
<p>Класс <span class="b">XmlDocument</span> добавляет еще ряд методов, которые позволяют создавать новые узлы:</p>
<ul>
<li><p><span class="b">CreateNode</span>: создает узел любого типа</p></li>
<li><p><span class="b">CreateElement</span>: создает узел типа XmlDocument</p></li>
<li><p><span class="b">CreateAttribute</span>: создает узел типа XmlAttribute</p></li>
<li><p><span class="b">CreateTextNode</span>: создает узел типа XmlTextNode</p></li>
<li><p><span class="b">CreateComment</span>: создает комментарий</p></li>
</ul>
<p>Возьмем xml-документ people.xml из прошлой темы:</p>
<pre class="brush:xml;">
...
<p>Добавим в этот xml-документ новый элемент &lt;person&gt;:</p>
<pre class="brush:c#;">
using System.Xml;
...
<p>Добавление элементов происходит по одной схеме. Сначала создаем элемент (<code>xDoc.CreateElement("person")</code>). Если элемент сложный, то есть содержит в 
...
(<code>XmlText companyText = xDoc.CreateTextNode("Facebook");</code>).</p>
<p>Затем все элементы добавляются в основной элемент person, а тот добавляется в корневой элемент (<code>xRoot.AppendChild(personElem);</code>).</p>
<p>Чтобы сохранить измененный документ на диск, используем метод <code>Save</code>: <code>xDoc.Save("people.xml")</code></p>
<p>После этого в xml-файле появится следующий элемент:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;people&gt;
  &lt;person name="Tom"&gt;
...
<p>Удалим первый узел xml-документа:</p>
<pre class="brush:c#;">
using System.Xml;
