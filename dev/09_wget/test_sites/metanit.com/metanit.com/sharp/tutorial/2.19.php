...
<html >
<head>
<title>C# и .NET | Кортежи</title>
<meta charset="utf-8" />
<meta name="description" content="Кортежи в C#, определение кортежей, возвращаение из функций и передача в метод в качестве параметра, именованные кортежи">
...
<h2>Кортежи</h2><div class="date">Последнее обновление: 02.01.2022</div>
...
<p>Кортежи предоставляют удобный способ для работы с набором значений, который был добавлен в версии C# 7.0.</p>
<p>Кортеж представляет набор значений, заключенных в круглые скобки:</p>
<pre class="brush:c#;">
var tuple = (5, 10);
</pre>
<p>В данном случае определен кортеж tuple, который имеет два значения: 5 и 10. В дальнейшем мы можем обращаться к каждому из этих значений через поля 
с названиями:</p>
<pre class="brush:c#;">Item[порядковый_номер_поля_в_кортеже]</pre>
<p>Например:</p>
<pre class="brush:c#;">
var tuple = (5, 10);
...
<p>В данном случае тип определяется неявно. Но мы также можем явным образом указать для переменной кортежа тип:</p>
<pre class="brush:c#;">(int, int) tuple = (5, 10);</pre>
<p>Так как кортеж содержит два числа, то в определении типа нам надо указать два числовых типа. Или другой пример определения кортежа:</p>
<pre class="brush:c#;">(string, int, double) person = ("Tom", 25, 81.23);</pre>
<p>Первый элемент кортежа в данном случае представляет строку, второй элемент - тип int, а третий - тип double.</p>
<p>Мы также можем дать названия полям кортежа:</p>
<pre class="brush:c#;">
var tuple = (count:5, sum:10);
Console.WriteLine(tuple.count); // 5
Console.WriteLine(tuple.sum); // 10
</pre>
<p>Теперь чтобы обратиться к полям кортежа используются их имена, а не названия Item1 и Item2.</p>
<p>Мы даже можем выполнить декомпозицию кортежа на отдельные переменные:</p>
<pre class="brush:c#;">
var (name, age) = ("Tom", 23);
Console.WriteLine(name);    // Tom
Console.WriteLine(age);     // 23
</pre>
<p>Одной из задач, которую позволяет элегантно решить кортеж - это обмен значениями:</p>
<pre class="brush:c#;">
string main = "Java";
string second = "C#";
...
<p>Что можно использовать, например, при простейшей сортировке массива:</p>
<pre class="brush:c#;">
...
<h3>Кортеж как результат метода</h3>
<p>Кортежи могут выступать в качестве результата функции. Например, одной из распространенных ситуаций является возвращение из функции двух и более значений, в 
то время как функция может возвращать только одно значение. И кортежи представляют оптимальный способ для решения этой задачи:</p>
<pre class="brush:c#;">
var tuple = GetValues();
...
<p>Здесь определен метод <code>GetValues()</code>, который возвращает кортеж. Кортеж определяется как набор значений, помещенных в круглые скобки. 
И в данном случае мы возвращаем кортеж из двух элементов типа int, то есть два числа.</p>
<p>Другой пример:</p>
<pre class="brush:c#;">
...
<h3>Кортеж как параметр метода</h3>
<p>И также кортеж может передаваться в качестве параметра в метод:</p>
<pre class="brush:c#;">
...
<p>Здесь в метод PrintPerson передается кортеж из двух элементов, первый из которых предоставляет строку, а второй - значение типа int.</p>
...
<div class="nav"><p><a href="./3.20.php">Назад</a><a href="./">Содержание</a><a href="./3.51.php">Вперед</a></p></div></div>
<div class="socBlock">
