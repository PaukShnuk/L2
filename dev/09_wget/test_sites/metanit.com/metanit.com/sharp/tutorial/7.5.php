...
<html >
<head>
<title>C# и .NET | Форматирование и интерполяция строк</title>
<meta charset="utf-8" />
<meta name="description" content="Форматирование и интерполяция строк C#, использование методов Format и ToString, применение описателей и спецификаторов для настройки форматирования">
...
<h2>Форматирование и интерполяция строк</h2><div class="date">Последнее обновление: 05.01.2022</div>
...
<p>При выводе строк в консоли с помощью метода Console.WriteLine для встраивания значений в строку мы можем применять форматирование 
вместо конкатенации:</p>
<pre class="brush:c#;">
string name = "Tom";
int age = 23;
...
<p>В строке "Имя: {0}  Возраст: {1}" на место <code>{0}</code> и <code>{1}</code> затем будут вставляться в порядке следования значения переменныйх <code>name</code> и <code>age</code></p>
<p>То же самое форматирование в строке мы можем сделать не только в методе Console.WriteLine, но и в любом месте программы с помощью метода <span class="b">string.Format</span>:</p>
<pre class="brush:c#;">
string name = "Tom";
int age = 23;
...
<p>Метод <code>Format</code> принимает строку с плейсхолдерами типа {0}, {1} и т.д., а также набор аргументов, которые вставляются на место 
данных плейсхолдеров. В итоге генерируется новая строка.</p>
<h3>Спецификаторы форматирования</h3>
<p>В методе Format могут использоваться различные спецификаторы и описатели, которые позволяют настроить вывод данных. Рассмотрим основные описатели. 
Все используемые форматы:</h4>
<table class="tab">
<tr><td><p><span class="b">C / c</span></p></td><td><p>Задает формат денежной единицы, указывает количество десятичных разрядов после запятой</p></td></tr>
<tr><td><p><span class="b">D / d</span></p></td><td><p>Целочисленный формат, указывает минимальное количество цифр</p></td></tr>
<tr><td><p><span class="b">E / e</span></p></td><td><p>Экспоненциальное представление числа, указывает количество десятичных разрядов после запятой</p></td></tr>
<tr><td><p><span class="b">F / f</span></p></td><td><p>Формат дробных чисел с фиксированной точкой, указывает количество десятичных разрядов после запятой</p></td></tr>
<tr><td><p><span class="b">G / g</span></p></td><td><p>Задает более короткий из двух форматов: F или E</p></td></tr>
<tr><td><p><span class="b">N / n</span></p></td><td><p>Также задает формат дробных чисел с фиксированной точкой, определяет количество разрядов после запятой</p></td></tr>
<tr><td><p><span class="b">P / p</span></p></td><td><p>Задает отображения знака процентов рядом с число, указывает количество десятичных разрядов после запятой</p></td></tr>
<tr><td><p><span class="b">X / x</span></p></td><td><p>Шестнадцатеричный формат числа</p></td></tr>
</table>
<h4>Форматирование валюты</h4>
<p>Для форматирования валюты используется описатель "C":</p>
<pre class="brush:c#;">
double number = 23.7;
...
<p>Число после описателя указывает, сколько чисел будет использоваться после разделителя между целой и дробной частью. При выводе также добавляется 
...
Также обратите внимание на округление в первом примере.</p>
<h4>Форматирование целых чисел</h4>
<p>Для форматирования целочисленных значение применяется описатель "d":</p>
<pre class="brush:c#;">
int number = 23;
...
<p>Число после описателя указывает, сколько цифр будет в числовом значении. Если в исходном числе цифр меньше, то к нему добавляются нули.</p>
<h4>Форматирование дробных чисел</h4>
<p>Для форматирования дробны чисел используется описатель F, число после которого указывает, сколько знаков будет использоваться после разделителя между целой 
и дробной частью. Если исходное число - целое, то к нему добавляются разделитель и нули.</p>
<pre class="brush:c#;">
int number = 23;
...
<h4>Формат процентов</h4>
<p>Описатель "P" задает отображение процентов. Используемый с ним числовой спецификатор указывает, сколько знаков будет после запятой:</p>
<pre class="brush:c#;">
decimal number = 0.15345m;
Console.WriteLine("{0:P1}", number);// 15,3%
</pre>
<h4>Настраиваемые форматы</h4>
<p>Используя знак #, можно настроить формат вывода. Например, нам надо вывести некоторое число в формате телефона +х (ххх)ххх-хх-хх:</p>
<pre class="brush:c#;">
...
<p>Метод <code>ToString()</code> не только получает строковое описание объекта, но и может осуществлять форматирование. Он поддерживает те же описатели, что 
используются в методе Format:</p>
<pre class="brush:c#;">
long number = 19876543210;
...
<h3>Интерполяция строк</h3>
<p>Интерполяция строк призвана упростить форматирование строк. Так, перепишем пример с выводом значений переменных в строке:</p>
<pre class="brush:c#;">
string name = "Tom";
int age = 23;
...
<p>Знак доллара перед строкой указывает, что будет осуществляться интерполяция строк. Внутри строки опять же используются плейсхолдеры <code>{...}</code>, 
только внутри фигурных скобок уже можно напрямую писать те выражения, которые мы хотим вывести.</p>
<p>Интерполяция по сути представляет более лаконичное форматирование. При этом внутри фигурных скобок мы можем указывать не только свойства, но и различные выражения языка C#:</p>
<pre class="brush:c#;">
int x = 8;
int y = 7;
...
<p>Также внутри фигурных скобок можно выполнять более сложные выражения, например, вызывать методы:</p>
<pre class="brush:c#;">
int x = 8;
int y = 7;
...
<p>Уже внутри строки можно применять форматирование. В этом случае мы можем применять все те же описатели, что и в методе <code>Format</code>. Например, выведем номер телефона в формате +x xxx-xxx-xx-xx:</p>
<pre class="brush:c#;">
long number = 19876543210;
Console.WriteLine($"{number:+# ### ### ## ##}"); // +1 987 654 32 10
</pre>
<p>Добавляем пространство до и после форматируемого вывода:</p>
<pre class="brush:c#;">
string name = "Tom";
int age = 23;
...
<div class="nav"><p><a href="./7.2.php">Назад</a><a href="./">Содержание</a><a href="./7.3.php">Вперед</a></p></div></div>
<div class="socBlock">
