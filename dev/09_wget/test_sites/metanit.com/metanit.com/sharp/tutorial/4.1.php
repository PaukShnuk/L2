...
<html >
<head>
<title>C# и .NET | Псевдонимы типов и статический импорт</title>
<meta charset="utf-8" />
<meta name="description" content="Установка псевдонимов классов и структур в языке программирования C# с помощью директивы using, статический импорт функциональности класса с using static">
...
<h2>Псевдонимы типов и статический импорт</h2><div class="date">Последнее обновление: 06.01.2022</div>
...
<h3>Псевдонимы</h3><p>Для различных классов и структур мы можем использовать псевдонимы. Затем в программе вместо названия типа используется его псевдоним. Например, 
для вывода строки на экран применяется метод <code>Console.WriteLine()</code>. Но теперь зададим для класса <span class="b">Console</span> 
псевдоним:</p>
<pre class="brush:c#;">
...
<p>С помощью выражения <code>using printer = System.Console</code> указываем, что псевдонимом для класса System.Console будет имя <code>printer</code>. Это выражение 
не имеет ничего общего с подключением пространств имен в начале файла, хотя и использует оператор <code>using</code>. При этом используется полное имя класса 
с учетом пространства имен, в котором класс определен. И далее, чтобы вывести 
строку, применяется выражение <code>printer.WriteLine("Laudate omnes gentes laudate")</code>.</p>
<p>Данная возможность может применяться, когда имена классов и структур довольно длинные, и мы хотим их сократить для большей лаконичности кода, 
либо когда мы хотим иметь более выразительные на наш субъективный взгляд имена типов.</p>
<p>И еще пример. Определим класс и для него псевдоним:</p>
<pre class="brush:c#;">
using printer = System.Console;
using User = Person;
...
<p>Класс называется Person, но в программе для него используется псевдоним User.</p>
<h3>Статический импорт</h3>
<p>Также в C# имеется возможность импорта статической функциональности классов. Например, импортируем возможности класса <code>Console</code>:</p>
<pre class="brush:c#;">
using static System.Console;

WriteLine("Hello from C# 8.0");
</pre>
<p>Выражение <span class="b">using static</span> подключает в программу все статические методы и свойства, а также константы. И после этого мы можем 
не указывать название класса при вызове метода.</p>
<p>Подобным образом можно определять свои классы и импортировать их:</p>
<pre class="brush:c#;">
using static System.Console;
...
<div class="nav"><p><a href="./3.26.php">Назад</a><a href="./">Содержание</a><a href="./3.7.php">Вперед</a></p></div></div>
<div class="socBlock">
