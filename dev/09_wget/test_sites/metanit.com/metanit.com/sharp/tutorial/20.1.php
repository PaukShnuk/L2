...
<html >
<head>
<title>C# и .NET | Отложенная инициализация и тип Lazy</title>
<meta charset="utf-8" />
<meta name="description" content="Отложенная инициализация и тип Lazy в C# и .NET">
...
<h1>Дополнительные классы и структуры .NET</h1><h2>Отложенная инициализация и тип Lazy</h2><div class="date">Последнее обновление: 06.01.2022</div>
...
<p>Приложение может использовать множество классов и объектов. Однако есть вероятность, что не все создаваемые объекты будут использованы. 
Особенно это касается больших приложений. Например:</p>
<pre class="brush:c#;">
class Reader
{
...
<p>Есть класс Library, представляющий библиотеку и хранящий некоторый набор книг в виде массива. Есть класс читателя Reader, который хранит ссылку на объект 
...
Library, чтобы получить эту книгу.</p>
<p>Но что если читателю вообще не придется читать обычную книгу, а только электронные, например, в следующем случае:</p>
<pre class="brush:c#;">
Reader reader = new Reader();
reader.ReadEbook();
</pre>
<p>В этом случае объект library в классе читателя никак не будет использоваться 
и будет только занимать место памяти. Хотя надобности в нем не будет.</p>
<p>Для подобных случаев в .NET определен специальный класс <span class="bb">Lazy&lt;T&gt;</span>. Изменим класс читателя следующим образом:</p>
<pre class="brush:c#;">
class Reader
{
...
<p>Класс Library остается прежнем. Но теперь класс читателя содержит ссылку на библиотеку в виде объекта <code>Lazy&lt;Library&gt;</code>. А чтобы 
обратиться к самой библиотеке и ее методам, надо использовать выражение <code>library.Value</code> - это и есть объект Library.</p>
<p>Что меняет в поведении класса Reader эта замена? Рассмотрим его применение:</p>
<pre class="brush:c#;">
Reader reader = new Reader();
reader.ReadEbook();
reader.ReadBook();
</pre>
<p>Непосредственно объект Library задействуется здесь только на третьей строке в методе <code>reader.ReadBook()</code>, который вызывает в свою очередь 
метод <code>library.Value.GetBook()</code>. Поэтому вплоть до третьей строки объект Library, используемый читателем, не будет создан. Если мы не будем применять 
в программе метод <code>reader.ReadBook()</code>, то объект библиотеки тогда вобще не будет создан, и мы избежим лишних затрат памяти. Таким образом, Lazy&ltT&gt; гарантирует нам, 
что объект будет создан только тогда, когда в нем есть необходимость.</p>
...
<div class="nav"><p><a href="./19.3.php">Назад</a><a href="./">Содержание</a><a href="./20.2.php">Вперед</a></p></div></div>
<div class="socBlock">
