...
<html >
<head>
<title>C# и .NET | Указатели на структуры, члены классов и массивы</title>
<meta charset="utf-8" />
<meta name="description" content="Использование указателей на структуры в языке программирования C# и .NET, ключевое слово stackalloc, фиксация указателей и оператор fixed">
...
<h2>Указатели на структуры, члены классов и массивы</h2><div class="date">Последнее обновление: 19.02.2022</div>
...
<h3>Указатели на типы и операция -&gt;</h3>
<p>Кроме указателей на простые типы можно использовать указатели на структуры. А для доступа к полям структуры, на которую указывает указатель, используется операция -&gt;:</p>
<pre class="brush:c#;">
unsafe
{
...
<p>Обращаясь к указателю <code>p-&gt;X = 30;</code> мы можем получить или установить значение свойства структуры, на которую указывает указатель. 
Обратите внимание, что просто написать <code>p.X=30</code> мы не можем, так как <code>p</code> - это не структура Point, а указатель на структуру.</p>
<p>Альтернативой служит операция разыменования: <code>(*p).X = 30;</code></p>
<p>Стоит отметить, что указатель может указывать только на те структуры, которые не имеют полей ссылочных типов (в том числе полей, которые генерируются компилятором автоматически 
для автосвойств).</p>
<h3>Указатели на массивы и stackalloc</h3>
<p>С помощью ключевого слова <span class="b">stackalloc</span> можно выделить память под массив в стеке. Смысл выделения памяти в стеке в повышении быстродействия кода. 
Посмотрим на примере вычисления квадратов чисел:</p>
<pre class="brush:c#;">
unsafe
{
    const int size = 7;
...
<p>Оператор <code>stackalloc</code> принимает после себя массив, на который будет указывать указатель. <code>int* square = stackalloc int[size];</code>.</p>
<p>Для манипуляций с массивом создаем указатель p: <code>int* p = square;</code>, который указывает на первый элемент массива, в котором всего 7 элементов. То есть с 
помощью указателя p мы сможем перемещаться по массиву square.</p>
<p>Далее в цикле происходит подсчет квадратов чисел от 1 до 7. В цикле для установки значения (квадрата числа - i * i) по адресу, который хранит указатель, 
выполняется выражение:</p>
<pre class="brush:c#;">*p = i * i;</pre>
<p>Затем происходит инкремент указателя <code>p++</code>, и указатель p смещается вперед на следующий элемент в массиве 
square.</p>
<p>Чуть более сложный пример - вычисление факториала:</p>
<pre class="brush:c#;">
unsafe
{
    const int size = 7;
...
<p>Также с помощью оператора <code>stackalloc</code> выделяется память для 7 элементов массива. 
И также для манипуляций с массивом создаем указатель p: <code>int* p = factorial;</code>, который указывает на первый элемент массива, в котором всего 7 элементов</p>
<p>Далее начинаются уже сами операции с указателем и подсчет факториала. Так как факториал 1 равен 1, то присваиваем первому элементу, на который указывает 
указатель p, единицу с помощью операции разыменования: <code>*(p++)= 1;</code></p>
<p>Для установки некоторого значения по адресу указателя применяется выражение: <code>*p=1</code>. 
Но кроме этого тут происходит также инкремент указателя <code>p++</code>. То есть сначала первому элементу массива присваивается единица, потом 
указатель p смещается и начинает указывать уже на второй элемент. Мы могли бы написать это так:</p>
<pre class="brush:c#;">
*p= 1;
p++;
</pre>
<p>Чтобы получить предыдущий элемент и сместиться назад, можно использовать операцию декремента: <code>Console.WriteLine(*(--p));</code>. 
Обратите внимание, что операции <code>*(--p)</code> и <code>*(p--)</code> различаются, так как в первом случае сначала идет смещение указателя, а затем его разыменовывание. 
А во втором случае - наоборот.</p>
<p>Затем вычисляем факториал всех остальных шести чисел: <code>*p = p[-1] *i;</code>. Обращение к указателям как к массивам представляет альтернативу 
операции разыменовывания для получения значения. В данном случае мы получаем значение предыдущего элемента.</p>
<p>И в заключении, используя указатель factorial, выводим факториалы всех семи чисел.</p>
<h3>Оператор fixed и закрепление указателей</h3>
<p>Ранее мы посмотрели, как создавать указатели на типы значений, например, int или структуры. Однако кроме структур в C# есть еще и классы, 
...
периодически очищающий кучу. Чтобы фиксировать на все время работы указатели на объекты классов используется оператор <code>fixed</code>.</p>
...
<p>Зафиксируем указатель с помощью оператора fixed:</p>
<pre class="brush:c#;">
unsafe
{
...
<p>Оператор fixed создает блок, в котором фиксируется указатель на поле объекта person. После завершения блока fixed закрепление с переменных снимается, 
и они могут быть подвержены сборке мусора.</p>
<p>Кроме адреса переменной можно также инициализировать указатель, используя массив, строку или буфер фиксированного размера:</p>
<pre class="brush:c#;">
unsafe 
{
...
<p>При инициализации указателей на строку следует учитывать, что указатель должен иметь тип <code>char*</code>.</p>
...
<div class="nav"><p><a href="./8.3.php">Назад</a><a href="./">Содержание</a><a href="./5.1.php">Вперед</a></p></div></div>
<div class="socBlock">
