...
<html >
<head>
<title>C# и .NET | Очередь Queue</title>
<meta charset="utf-8" />
<meta name="description" content="Очередь Queue&lt;T&gt; в C#, добавление в очередь с помощью метода Enqueue и удаление из очереди через метод Dequeue, пример использования">
...
<h2>Очередь Queue</h2><div class="date">Последнее обновление: 04.01.2022</div>
...
<p>Класс <span class="b">Queue&lt;T&gt;</span> представляет обычную очередь, которая работает по алгоритму FIFO ("первый вошел - первый вышел").</p>
<h3>Создание очереди</h3>
<p>Для создания очереди можно использовать один из трех ее конструкторов. Прежде всего можно создать пустую очередь:</p>
<pre class="brush:c#;">
Queue&lt;string&gt; people = new Queue&lt;string&gt;();
</pre>
<p>При создании пустой очереди можно указать емкость очереди:</p>
<pre class="brush:c#;">
Queue&lt;string&gt; people = new Queue&lt;string&gt;(16);
</pre>
<p>Также можно инициализировать очередь элементами из другой коллекции или массивом:</p>
<pre class="brush:c#;">
...
<p>Для перебора очереди можно использовать стандартный цикл <span class="b">foreach</span>.</p>
<p>Для получения количества элементов в очереди в классе определено свойство <span class="b">Count</span>.</p>

<h3>Методы Queue</h3>
<p>У класса <code>Queue&lt;T&gt;</code> можно отметить следующие методы:</p>
<ul>
<li><p><span class="b">void Clear()</span>: очищает очередь</p></li>
<li><p><span class="b">bool Contains(T item)</span>: возвращает <code>true</code>, если элемент item имеется в очереди</p></li>
<li><p><span class="b">T Dequeue()</span>: извлекает и возвращает первый элемент очереди</p></li>
<li><p><span class="b">void Enqueue(T item)</span>: добавляет элемент в конец очереди</p></li>
<li><p><span class="b">T Peek()</span>: просто возвращает первый элемент из начала очереди без его удаления</p></li>
</ul>
<p>Посмотрим применение очереди на практике:</p>
<pre class="brush:c#;">
...
<p>Стоит отметить, что если с помощью методов Peek или Enqueue мы попытаемся получить первый элемент очереди, которая пуста, 
то программа выдаст исключение. Соответственно перед получением элемента мы можем проверять количество элементов в очереди:</p>
<pre class="brush:c#;">
if(people.Count &gt; 0)
{
...
<ul>
<li><p><span class="b">bool TryDequeue(out T result)</span>: передает в переменную result первый элемент очереди с его удалением из очереди, 
возвращает <code>true</code>, если очередь не пуста и элемент успешно получен.</p></li>
<li><p><span class="b">bool TryPeek(out T result)</span>: передает в переменную result первый элемент очереди без его извлечения из очереди, 
возвращает <code>true</code>, если очередь не пуста и элемент успешно получен.</p></li>
</ul>
...
<p>Очереди - довольно часто встречаемая стуктура в реальной жизни. Например, очередь пациентов на прием к врачу. Реализуем данную ситуацию:</p>
<pre class="brush:c#;">
...
<p>Здесь класс врача - класс Doctor в методе TakePatients принимает очередь пациентов в виде объектов Person. И пока в очереди есть объекты извлекает по одному объекту. Консольный вывод:</p>
<pre class="brush:c#;">
Осмотр пациента Tom
Осмотр пациента Bob
Осмотр пациента Sam
