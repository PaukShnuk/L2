...
<html >
<head>
<title>C# и .NET | Объединение, пересечение и разность коллекций</title>
<meta charset="utf-8" />
<meta name="description" content="Операции LINQ в C# и .NET, которые генерируют множества, методы Except, Intersect, Distinct, пересечение и разность множеств">
...
<h2>Объединение, пересечение и разность коллекций</h2><div class="date">Последнее обновление: 13.02.2022</div>
...
<p>LINQ предоставляет несколько методов для работы с коллекциями как с множествами, а именно находить их разность, объединение и пересечение.</p>
<h3>Разность последовательностей</h3>
<p>С помощью метода <span class="b">Except()</span> можно получить разность двух последовательностей:</p>
<pre class="brush:c#;">
...
<p>В данном случае из массива <code>soft</code> убираются все элементы, которые есть в массиве <code>hard</code>. Результатом операции будут 
два элемента:</p>
<div class="console">
...
<h3>Пересечение последовательностей</h3>
<p>Для получения пересечения последовательностей, то есть общих для обоих наборов элементов, применяется метод <span class="b">Intersect</span>:</p>
<pre class="brush:c#;">
string[] soft = { "Microsoft", "Google", "Apple"};
string[] hard = { "Apple", "IBM", "Samsung"};
...
<p>Так как оба набора имеют только один общий элемент, то соответственно только он и попадет в результирующую выборку:</p>
<div class="console">
<pre class="consoletext">
Apple
</pre>
...
<p>Для удаления дублей в наборе используется метод <span class="b">Distinct</span>:</p>
<pre class="brush:c#;">
...
<p>Для объединения двух последовательностей используется метод <span class="b">Union</span>. Его результатом является новый набор, в котором имеются элементы, как из 
первой, так и из второй последовательности. Повторяющиеся элементы добавляются в результат только один раз:</p>
<pre class="brush:c#;">
string[] soft = { "Microsoft", "Google", "Apple"};
string[] hard = { "Apple", "IBM", "Samsung"};
...
<p>Результатом операции будет следующий набор:</p>
<div class="console">
<pre class="consoletext">
Microsoft
Google
...
<p>Если же нам нужно простое объединение двух наборов, то мы можем использовать метод <span class="b">Concat</span>:</p>
<pre class="brush:c#;">
var result = soft.Concat(hard);
</pre>
<p>В этом случае те элементы, которые встречаются в обоих наборах, дублируются в резутирующей последовательности.</P>
<p>Последовательное применение методов Concat и Distinct будет подобно действию метода Union.</p>
<h3>Работа со сложными объектами</h3>
<p>Для сравнения объектов в последовательностях применяются реализации методов GetHeshCode() и Equals(). Поэтому если мы хотим работать с 
последовательностями, которые содержат объекты своих классов и структур, то нам необходимо определить для них подобные методы:</p>
<pre class="brush:c#;">
...
<p>Здесь объекты Person сравниваются исходя из значения их свойства Name - если имена равны, то и объекты Person равны. Консольный вывод:</p>
<div class="console">
<pre class="consoletext">
Tom
Bob
