...
<html >
<head>
<title>C# и .NET | Глобальные пространства имен</title>
<meta charset="utf-8" />
<meta name="description" content="Глобальные пространства имен в C# и .NET, файл GlobalUsings.cs, директива global, подключение пространства имен на уровне проекта">
...
<h2>Глобальные пространства имен</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Если мы хотим использовать какое-нибудь пространство имен в файлах кода в проекте, то по умолчанию нам надо его подключать во все файлы, где мы планируем его использовать.</p>
<p>Например, пусть у нас в проекте есть три файла с кодом:</p>
<img src="./pics/3.33.png" alt="Глобальные пространства имен в C# и .NET" />
<p>В файле <span class="b">Base.cs</span> определяется класс Company в пространстве <span class="b">Base</span>:</p>
<pre class="brush:c#;">
namespace Base;
class Company
{
...
<p>В файле <span class="b">Person.cs</span> определен класс Person, который использует класс Company:</p>
<pre class="brush:c#;">
using Base;
...
<p>Чтобы использовать класс Company мы подключаем его пространство имен: <code>using Base;</code></p>
<p>В классе <span class="b">Program.cs</span> используются классы Person и Company:</p>
<pre class="brush:c#;">
using Base;
...
<p>Опять же чтобы использовать класс Company, подключается его пространство имен: <code>using Base;</code></p>
<p>И таких файлов, где надо подключать пространство Base (или какое-то другое) может быть множество. Это не очень удобно. И в .NET 6 для этого предложена 
концепция <span class="b">глобальных пространств имен</span>, который подключаются один раз но сразу во все файлы кода в проекте. Для этого нам достаточно в одном файле происать директиву:</p>
<pre class="brush:c#;">global using подключаемое_пространство_имен;</pre>
<p>Например, изменим файл <span class="b">Program.cs</span> следующим образом:</p>
<pre class="brush:c#;">
global using Base;
...
<p>Теперь пространство Base подключается во все файлы кода в проекте. И из файла <span class="b">Person.cs</span> мы можем убрать строку</p>
<pre class="brush:c#;">using Base;</pre>
<h3>Определение глобальных пространств в отдельном файле</h3>
<p>Однако вышеуказанный подход опять же может быть не очень удобным, поскольку проще определить глобальные пространство имен, которые подключаются во весь проект 
...
пространств имен. Например, добавим в проект файл, который назовем <span class="b">GlobalUsings.cs</span> и в котором определим следующее содержимое:</p>
<pre class="brush:c#;">
global using System.Text;
global using System.Reflection;
global using Base;
</pre>
<p>И этот набор пространств имен будет автоматически подключаться во все файлы кода в проекте.</p>
<img src="./pics/3.34.png" alt="Файл GlobalUsings.cs и глобальные пространства имен в C# и .NET и в Visual Studio 2022" />
...
<div class="nav"><p><a href="./3.25.php">Назад</a><a href="./">Содержание</a><a href="./3.53.php">Вперед</a></p></div></div>
<div class="socBlock">
