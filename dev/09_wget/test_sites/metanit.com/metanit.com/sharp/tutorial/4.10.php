...
<html >
<head>
<title>C# и .NET | Индексаторы</title>
<meta charset="utf-8" />
<meta name="description" content="Индексаторы в C#, их определение и использование для создания собственных коллекций, индексаторы и двухмерные массивы">
...
<h2>Индексаторы</h2><div class="date">Последнее обновление: 28.12.2021</div>
...
<p><span class="b">Индексаторы</span> позволяют индексировать объекты и обращаться к данным по индексу. Фактически с помощью индексаторов 
мы можем работать с объектами как с массивами. По форме они напоминают свойства со стандартными блоками <code>get</code> и 
<code>set</code>, которые возвращают и присваивают значение.</p>
<p>Формальное определение индексатора:</p>
<pre class="brush:c#;">
...
<p>В отличие от свойств индексатор не имеет названия. Вместо него указывается ключевое слово <span class="b">this</span>, 
после которого в квадратных скобках идут параметры. Индексатор должен иметь как минимум один параметр.</p>
<p>Посмотрим на примере. Допустим, у нас есть класс <span class="b">Person</span>, который представляет человека, и класс <span class="b">Company</span>, 
который представляет некоторую компанию, где работают люди. Используем индексаторы для определения класса Company:</p>
<pre class="brush:c#;">
class Person
{
...
<p>Для хранения персонала компании в классе определен массив <code>personal</code>, который состоит из объектов Person. Для доступа к этим объектам определен индексатор:</p>
<pre class="brush:c#;">public Person this[int index]</pre>
<p>Индексатор в принципе подобен стандартному свойству. Во-первых, для индексатора определяется тип в данном случае тип Person. Тип индексатора определяет, какие объекты будет получать и возвращать индексатор.</p>
<p>Во-вторых, для индексатора определен параметр <code>int index</code>, через который обращаемся к элементам внутри объекта Company.</p>
<p>Для возвращения объекта в индексаторе определен блок <span class="b">get</span>:</p>
<pre class="brush:c#;">
get =&gt; personal[index];
</pre>
<p>Поскольку индексатор имеет тип Person, то в блоке get нам надо возвратить объект этого типа с помощью оператора return. Здесь мы 
можем определить разнообразную логику. В данном случае просто возвращаем объект из массива personal.</p>
<p>В блоке <span class="b">set</span>, как и в обычном свойстве, получаем через параметр <span class="b">value</span> переданный объект 
Person и сохраняем его в массив по индексу.</p>
<pre class="brush:c#;">
set =&gt; personal[index] = value;
</pre>
<p>После этого мы можем работать с объектом Company как с набором объектов Person:</p>
<pre class="brush:c#;">
...
<p>Стоит отметить, что если индексатору будет передан некорректный индекс, который отсутствует в массиве person, то мы получим исключение, как и в случае обращения напрямую к элементам 
массива. В этом случае можно предусмотреть какую-то дополнительную логику. Например, проверять переданный индекс:</p>
<pre class="brush:c#;">
class Company
{
    Person[] personal;
...
<p>Здесь в блоке get если переданный индекс имеется в массиве, то возвращаем объект по индексу. Если индекса нет в массиве, то генерируем исключение. 
Аналогично в блоке set устанавливаем значение по индексу, если индекс есть в массиве.</p>
<h3>Индексы</h3>
<p>Индексатор получает набор индексов в виде параметров. Однако индексы необязательно должны представлять тип int, устанавливаемые/возвращаемые значения 
необязательно хранить в массиве. Например, мы можем рассматривать объект как хранилище атрибутов/свойств и передавать имя атрибута в виде строки:</p>
<pre class="brush:c#;">
User tom = new User();
...
<p>В данном случае индексатор в классе User в качестве индекса получает строку, которая хранит название атрибута (в данном случае название поля класса).</p>
<p>В блоке <span class="b">get</span> в зависимости от значения строкового индекса возвращается значение того или иного поля класса. Если передано неизвестное название, 
то генерируется исключение. В блоке <span class="b">set</span> похожая логика - по индексу узнаем, для какого поля надо установить значение.</p>
<h3>Применение нескольких параметров</h3>
<p>Также индексатор может принимать несколько параметров. Допустим, у нас есть класс, в котором хранилище определено в виде двухмерного массива или матрицы:</p>
<pre class="brush:c#;">
class Matrix
{
...
<p>Теперь для определения индексатора используются два индекса - i и j. И в программе мы уже должны обращаться к объекту, используя два индекса:</p>
<pre class="brush:c#;">
...
<p>Следует учитывать, что индексатор не может быть статическим и применяется только к экземпляру класса. Но при этом индексаторы могут быть виртуальными и абстрактными и 
могут переопределяться в произодных классах.</p>
<h3>Блоки get и set</h3>
<p>Как и в свойствах, в индексаторах можно опускать блок get или set, если в них нет необходимости. 
Например, удалим блок set и сделаем индексатор доступным только для чтения:</p>
<pre class="brush:c#;">
class Matrix
{
    int[,] numbers = new int[,] { { 1, 2, 4 }, { 2, 3, 6 }, { 3, 4, 8 } };
...
<p>Также мы можем ограничивать доступ к блокам get и set, используя модификаторы доступа. Например, сделаем блок set приватным:</p>
<pre class="brush:c#;">
class Matrix
{
    int[,] numbers = new int[,] { { 1, 2, 4 }, { 2, 3, 6 }, { 3, 4, 8 } };
...
<h3>Перегрузка индексаторов</h3>
<p>Подобно методам индексаторы можно перегружать. В этом случае также индексаторы должны отличаться по количеству, типу или порядку используемых параметров. 
Например:</p>
<pre class="brush:c#;">
...
<p>В данном случае класс Company содержит две версии индексатора. Первая версия получает и устанавливает объект Person по индексу, 
а вторая - только получае объект Person по его имени.</p>
...
<ul>
<li><a title="Упражнения" href="../practice/2.2.php"><i class="fa fa-lg fa-pencil"></i><span class="dop-txt">Упражнения</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./3.37.php">Назад</a><a href="./">Содержание</a><a href="./2.21.php">Вперед</a></p></div></div>
<div class="socBlock">
