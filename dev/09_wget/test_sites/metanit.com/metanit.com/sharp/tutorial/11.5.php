...
<html >
<head>
<title>C# и .NET | Мониторы</title>
<meta charset="utf-8" />
<meta name="description" content="Использование мониторов для синхронизации потоков в приложении на языке программирования C# и .NET. Использование класса Monitor и его методов Enter, TryEnter, Exit">
...
<h2>Мониторы</h2><div class="date">Последнее обновление: 07.02.2022</div>
...
<p>Наряду с оператором lock для синхронизации потоков мы можем использовать мониторы, представленные классом 
<span class="b">System.Threading.Monitor</span>. Для управления синхронизацией этот класс предоставляет следующите методы:</p>
<ul>
<li><p><span class="b">void Enter(object obj)</span>: получает в экслюзивное владение объект, передаваемый в качестве параметра.</p>	
<p><span class="b">void Enter(object obj, bool acquiredLock)</span>: дополнительно принимает второй параметра - логическое значение, которое указывает, получено ли владение 
над объектом из первого параметра</p></li>
<li><p><span class="b">void Exit(object obj)</span>: освобождает ранее захваченный объект</p></li>
<li><p><span class="b">bool IsEntered(object obj)</span>: возвращает true, если монитор захватил объект obj</p></li>
<li><p><span class="b">void Pulse (object obj)</span>: уведомляет поток из очереди ожидания, что текущий поток освободил объект obj</p></li>
<li><p><span class="b">void PulseAll(object obj)</span>: уведомляет все потоки из очереди ожидания, что текущий поток освободил объект obj. После чего один из потоков из очереди 
ожидания захватывает объект obj.</p></li>
<li><p><span class="b">bool TryEnter (object obj)</span>: пытается захватить объект obj. Если владение над объектом успешно получено, то возвращается значение true</p></li>
<li><p><span class="b">bool Wait (object obj)</span>: освобождает блокировку объекта и переводит поток в очередь ожидания объекта. Следующий поток в очереди готовности объекта блокирует данный объект. А все потоки, которые вызвали метод Wait, остаются в очереди ожидания, пока не получат сигнала от метода Monitor.Pulse или Monitor.PulseAll, посланного владельцем блокировки.</p></li>
</ul>
<p>Стоит отметить, что фактически конструкция оператора lock инкапсулирует в себе синтаксис использования мониторов. Например, в прошлой теме для синхронизации потоков 
применялся оператор <span class="b">lock</span>:</p>
<pre class="brush:c#;">
int x = 0;
...
<p>Фактически данный пример будет эквивалентен следующему коду:</p>
<pre class="brush:c#;">
int x = 0;
object locker = new();  // объект-заглушка
// запускаем пять потоков
...
<p>Метод <span class="b">Monitor.Enter</span> принимает два параметра - объект блокировки и значение типа bool, которое указывает на результат блокировки (если он равен true, то 
...
метода <code>Monitor.Exit</code> происходит освобождение объекта locker, если блокировка осуществлена успешно, и он становится доступным для других потоков.</p>
