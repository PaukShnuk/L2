...
<html >
<head>
<title>C# и .NET | DateOnly и TimeOnly</title>
<meta charset="utf-8" />
<meta name="description" content="Структуры DateOnly и TimeOnly в C# и .NET, методы для работы с датами и временем">
...
<h2>DateOnly и TimeOnly</h2><div class="date">Последнее обновление: 06.01.2022</div>
...
<p>Для упрощения работы с датами и временем в .NET 6 были добавлены две дополнительные структуры - <span class="b">DateOnly</span> и 
<span class="b">TimeOnly</span>.</p>
<h3>DateOnly</h3>
<p>Структура <span class="b">DateOnly</span> представляет дату. Для создания структуры можно использовать ряд ее конструкторов.</p>
<pre class="brush:c#;">
DateOnly()
DateOnly(int year, int month, int day)
DateOnly(int year, int month, int day, System.Globalization.Calendar calendar)
</pre>
<p>При использовании конструктора без параметров структура инициализируется датой 01.01.0001:</p>
<pre class="brush:c#;">
DateOnly someDate = new DateOnly();
Console.WriteLine(someDate); // 01.01.0001
</pre>
<p>Вторая версия конструктора принимает год, месяц и число, которыми инициализируется структура:</p>
<pre class="brush:c#;">
DateOnly someDate = new DateOnly(2022,1,6); // 6 января 2022 года
Console.WriteLine(someDate); // 06.01.2022
</pre>
<p>Третья версия конструктора в дополнение к году, месяцу и числу, также принимает объект календаря - объект <span class="b">System.Globalization.Calendar</span>, 
...
Например, расcчитаем дату относительно юлианского календаря:</p>
<pre class="brush:c#;">
...
<p>В данном случае для .NET переданная в конструктор дата - 06.01.2022 расценивается как дата юлианского календаря. При выводе на консоль мы видим тот же день 
только относительно григорианского календаря.</p>
<h4>Свойства DateOnly</h4>
<p>С помощью свойств структуры можно получить отдельные составляющие даты:</p>
<ul>
<li><p><span class="b">Day</span>: возвращает день даты</p></li>
<li><p><span class="b">DayNumber</span>: возвращает количество прошедших дней с 1 января 0001 года относительно григорианского календаря</p></li>
<li><p><span class="b">DayOfWeek</span>: возвращает день недели</p></li>
<li><p><span class="b">DayOfYear</span>: возвращает день года</p></li>
<li><p><span class="b">MaxValue</span>: возвращает максимально возможную дату (статическое свойство)</p></li>
<li><p><span class="b">MinValue</span>: возвращает самую раннюю возможную дату (статическое свойство)</p></li>
<li><p><span class="b">Month</span>: возвращает месяц</p></li>
<li><p><span class="b">Year</span>: возвращает год</p></li>
</ul>
...
<h4>Методы DateOnly</h4>
<p>С помощью методов DateOnly можно производить некоторые операции с датами. Некоторые из них:</p>
<ul>
<li><p><span class="b">AddDays(int days)</span>: добавляет к дате некоторое количество дней</p></li>
<li><p><span class="b">AddMonths(int months)</span>: добавляет к дате некоторое количество месяцев</p></li>
<li><p><span class="b">AddYears(int years)</span>: добавляет к дате некоторое количество лет</p></li>
<li><p><span class="b">ToDateTime(TimeOnly)</span>: возвращает объект <code>DateTime</code>, 
который в качестве даты исппользует текущий объект DateOnly, а в качестве времени - значение параметра в виде TimeOnly</p></li>
<li><p><span class="b">ToLongDateString()</span>: выводит текущий объект DateOnly в виде подробной даты</p></li>
<li><p><span class="b">ToShortDateString()</span>: выводит текущий объект DateOnly в виде сжатой даты</p></li>
</ul>
<p>Также в классе есть ряд статических методов. Некоторые из них:</p>
<ul>
<li><p><span class="b">FromDateTime(DateTime dateTime)</span>: на основе значения DateTime, переданного через параметр, создает и возвращает объект DateOnly</p></li>
<li><p><span class="b">FromDayNumber(int days)</span>: на основе количества дней создает и возвращает объект DateOnly</p></li>
<li><p><span class="b">Parse(string date)</span>: конвертирует строковое представление даты в объект DateOnly</p></li>
<li><p><span class="b">ParseExact(string date, string format)</span>: конвертирует строковое представление даты в объект DateOnly, применяя определенный формат</p></li>
<li><p><span class="b">TryParse(String, DateOnly)</span>: конвертирует строковое представление даты в объект DateOnly. При успешной конвертации возвращает true, а параметр типа DateOnly содержит созданную дату</p></li>
<li><p><span class="b">TryParseExact(String, String, DateOnly)</span>: конвертирует строковое представление даты в объект DateOnly, применяя определенный формат. При успешной конвертации возвращает true, а параметр типа DateOnly содержит созданную дату</p></li>
</ul>
<p>Пример некоторых операций:</p>
<pre class="brush:c#;">
...
<h3>TimeOnly</h3>
<p>Структура <span class="b">TimeOnly</span> представляет время в диапазоне от 00:00:00 до 23:59:59.9999999. Для создания структуры можно использовать ряд ее конструкторов.</p>
<pre class="brush:c#;">
TimeOnly()
TimeOnly(long ticks)
...
<p>При использовании конструктора без параметров структура инициализируется временем 0.00:</p>
<pre class="brush:c#;">
TimeOnly time = new TimeOnly();
Console.WriteLine(time); // 0:00
</pre>
<p>Дополнительно с помощью других версий конструктора можно установить количество часов, минут, секунд и миллисекунд:</p>
<pre class="brush:c#;">
...
<h4>Свойства TimeOnly</h4>
<p>С помощью свойств структуры можно получить отдельные составляющие времени:</p>
<ul>
<li><p><span class="b">Hour</span>: возвращает количество часов</p></li>
<li><p><span class="b">Minute</span>: возвращает количество минут</p></li>
<li><p><span class="b">Second</span>: возвращает количество секунд</p></li>
<li><p><span class="b">Millisecond</span>: возвращает количество миллисекунд</p></li>
<li><p><span class="b">Ticks</span>: возвращает количество тиков</p></li>
<li><p><span class="b">MaxValue</span>: возвращает максимально возможное время (статическое свойство)</p></li>
<li><p><span class="b">MinValue</span>: возвращает минимально возможное время (статическое свойство)</p></li>
</ul>
<p>Применение свойств:</p>
<pre class="brush:c#;">
...
<h4>Методы TimeOnly</h4>
<p>С помощью методов TimeOnly можно производить некоторые операции с временем. Некоторые из них:</p>
<ul>
<li><p><span class="b">AddHours(double hours)</span>: добавляет к времени некоторое количество часов</p></li>
<li><p><span class="b">AddMinutes(double minutes)</span>: добавляет к времени некоторое количество минут</p></li>
<li><p><span class="b">Add(TimeSpan value)</span>: добавляет время из объекта TimeSpan</p></li>
<li><p><span class="b">ToLongTimeString()</span>: выводит текущий объект TimeOnly в виде подробного времени</p></li>
<li><p><span class="b">ToShortTimeString()</span>: выводит текущий объект TimeOnly в виде сжатого времени</p></li>
</ul>
<p>Также в классе есть ряд статических методов. Некоторые из них:</p>
<ul>
<li><p><span class="b">FromDateTime(DateTime dateTime)</span>: на основе значения DateTime, переданного через параметр, создает и возвращает объект TimeOnly</p></li>
<li><p><span class="b">FromTimeSpan(TimeSpan value)</span>: на основе объекта TimeSpan создает и возвращает объект TimeOnly</p></li>
<li><p><span class="b">Parse(string time)</span>: конвертирует строковое представление времени в объект TimeOnly</p></li>
<li><p><span class="b">ParseExact(string timee, string format)</span>: конвертирует строковое представление времени в объект TimeOnly, применяя определенный формат</p></li>
<li><p><span class="b">TryParse(string time, TimeOnly result)</span>: конвертирует строковое представление времени в объект TimeOnly. При успешной конвертации возвращает true, а параметр типа TimeOnly содержит  сконвертированное время</p></li>
<li><p><span class="b">TryParseExact(string time, string format, TimeOnly result)</span>: конвертирует строковое представление времени в объект TimeOnly, применяя определенный формат. При успешной конвертации возвращает true, а параметр типа TimeOnly содержит сконвертированное время</p></li>
</ul>
<p>Пример некоторых операций:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./19.2.php">Назад</a><a href="./">Содержание</a><a href="./20.1.php">Вперед</a></p></div></div>
<div class="socBlock">
