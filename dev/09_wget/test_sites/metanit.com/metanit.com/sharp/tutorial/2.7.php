...
<html >
<head>
<title>C# и .NET | Задачи с массивами</title>
<meta charset="utf-8" />
<meta name="description" content="Задачи с массивами в языке программирования C#, программа сортировки массива, инверсия массивов, нахождение максимального числа">
...
<h2>Задачи с массивами</h2><div class="date">Последнее обновление: 10.11.2021</div>
...
<p>Познакомившись с циклами, переменными, условными конструкциями и массивами, рассмотрим несколько задач для работы с массивами.</p>
<h3>Количество положительных чисел</h3>
<p>Найдем количество положительных чисел в массиве:</p>
<pre class="brush:c#;">
...
<p>Здесь создаем вспомогательную переменную <code>result</code>, которая будет содержать количество положительных чисел. В цикле прохожим по массиву и, если его элемент больше нуля, 
добавляем к переменной <code>result</code> единицу.</p>
<h3>Инверсия массива</h3>
<p>Вторая задача - инверсия массива, то есть переворот его в обратном порядке:</p>
<pre class="brush:c#;">
int[] numbers = { -4, -3, -2, -1,0, 1, 2, 3, 4 };
...
<p>Поскольку нам надо изменять элементы массива, то для этого используется цикл for. Алгоритм решения задачи подразумевает перебор элементов до середины массива, 
которая в программе представлена переменной k, и обмен значений элемента, который имеет индекс i, и элемента с индексом n-i-1.</p>
<h3>Программа сортировки массива</h3>
<p>Теперь возьмем задачу посложнее - простейшую сортировку массива:</p>
<pre class="brush:c#;">
...
<p>Для сортировки массива выполняем проходы по массиву и сравниваем элементы. Поскольку нам надо последовательно сравнивать каждый элемент массива с каждым (за исключением сравния с самим собой), 
то здесь применятся вложенный цикл.</p>
<p>Во внешнем цикле мы берем элемент, который будем сравнивать:</p>
<pre class="brush:c#;">for (int i = 0; i &lt; nums.Length - 1; i++)</pre>
<p>Далее запускаем вложенный цикл, который начинается, со следующего элемента, и из которого извлекаем элементы, с которыми будем сравнивать тот элемент, которые берется из массива во внешнем цикле:</p>
<pre class="brush:c#;">for (int j = i + 1; j &lt; nums.Length; j++)</pre>
<p>Если элемент с меньшим индексом больше элемента с большим индексом, то меняем элементы местами.</p>
<pre class="brush:c#;">
if (nums[i] &gt; nums[j])
{
	temp = nums[i];
...
<p>В конце выводим все элементы.</p>
