...
<html >
<head>
<title>Xamarin Forms | Привязка</title>
<meta charset="utf-8" />
<meta name="description" content="Основы привязки данных в Xamarin Forms, примеры использования привязки, объект Binding">
...
<h1>Привязка в Xamarin</h1><h2>Введение в привязку</h2><div class="date">Последнее обновление: 12.01.2021</div>
...
<p>Привязка данных (data binding) является одним из ключевых моментов платформы Xamarin Forms.</p>
<p>Привязка данных состоит из двух компонентов: источника (source) и цели (target). Привязка осуществляется от свойства источника к свойству цели. 
И когда происходит изменение источника, механизм привязки автоматически обновляет также и цель.</p>
<img src="./pics/3.24.png" alt="Структура привязки в Xamarin Forms" />
<p>Цель привязки должна представлять объект <span class="b">BindableObject</span>, а свойство, к которому осуществляется привязка, должно быть свойством 
<span class="b">BindableProperty</span>. Поскольку большинство визуальных элементов в Xamarin Forms наследуются от класса BindableObject, 
то в качестве цели привязки будут, как правило, выступать визуальные элементы.</p>
<p>А вот источником привязки может выступать любой объект языка C#. Однако, надо понимать, что цель привязки должна автоматически изменяться при 
...
в качестве подобного механизма извещения выступает интерфейс <span class="b">INotifyPropertyChanged</span>. То есть нужно реализовать данный интерфейс 
в объекте-источнике.</p>
<p>Объект BindableObject как раз реализует <code>INotifyPropertyChanged</code>. Поэтому если источником привязки является стандартный визуальный элемент 
...
класса C#, то, как писалось выше, этот класс должен реализовать INotifyPropertyChanged.</p>
<p>Для установки привязки у объекта цели устанавливается свойство <span class="b">BindingContext</span>. В качестве значения оно принимает 
источник привязки.</p>
<p>Рассмотрим на примерах, как устанавливается привязка в коде C# и в коде XAML.</p>
<h3>Привязка в коде C#</h3>
<p>Для привязки в коде после установки у цели привязки свойства <span class="b">BindingContext</span> необходимо также у цели привязки 
вызвать метод <span class="b">SetBinding()</span>, который свяжет свойство цели привязки со свойством источника.</p>
<pre class="brush:c#;">
...
<p>Выражение <code>label.SetBinding(Label.TextProperty, "Text")</code> устанавливает привязку свойства TextProperty у цели - элемента label к 
свойству <code>Text</code>.</p>
<p>Причем, что важно у объекта цели label привязка устанавливается именно у свойства <code>BindableProperty</code>, которым является <code>TextProperty</code>, 
а не просто для свойства <code>Text</code>.</p>
<p>В итоге при вводе данных в текстовое поле будет изменяться значение свойства Text у объекта entry. А это изменение автоматически скажется на объекте 
label и изменит его свойство TextProperty.</p>
<img src="./pics/3.1.png" alt="Binding in Xamarin Forms" />
<h3>Привязка в XAML</h3>
<p>Аналогично можно установить привязку и в xaml-коде.</p>
<pre class="brush:xml;">
...
<p>В XAML действует тот же принцип. Для установки привязки для объекта цели задается свойство <span class="b">BindingContext</span>. Но теперь его значение 
имеет другую форму: <code>{x:Reference Name=entryBox}</code>. После расширения разметки <code>x:Reference</code> идет свойство <code>Name</code> и его значение - 
имя элемента источника. То есть в данном случае источником выступает объект с именем entryBox.</p>
<p>Далее устанавливается сама привязка к свойству:</p>
<pre class="brush:xml;">Text="{Binding Path=Text}"</pre>
<p>Здесь также устанавливается привязка к свойству Text. Выражение привязки заключается в фигурные скобки и состоит из слова <span class="b">Binding</span>, 
после которого идет свойство <code>Path</code> и его значение - свойство объекта источника. То есть свойство Text объекта label привязано к свойству Text объекта entryBox.</p>
...
<div class="nav"><p><a href="./9.5.php">Назад</a><a href="./">Содержание</a><a href="./4.4.php">Вперед</a></p></div></div>
<div class="socBlock">
