...
<html >
<head>
<title>Xamarin Forms | WebView</title>
<meta charset="utf-8" />
<meta name="description" content="Веб-бразуер в Xamarin Forms и элемент WebView для просмотра информации из интернета">
...
<h2>WebView</h2><div class="date">Последнее обновление: 10.01.2021</div>
...
<p>WebView представляет собой мини-веббраузер, позволяющий просматривать различные типы контента:</p>
<ul>
<li><p>В первую очередь WebView позволяет загружать из среды интернет веб-сайты и взаимодействовать с ними. WebView имеет полную поддержку HTML, CSS и 
JavaScript</p></li>
<li><p>Различные типы документов, например, документы pdf. Однако каждая платформа может поддерживать свой набор типов документов. К примеру, на 
iOS и Android мы сможем просмотреть файл pdf, а вот на Windows Phone/Windows Mobile нет</p></li>
<li><p>Строки, которые содержат код html, при просмотре будут должным образом интерпретированы, как в обычном браузере</p></li>
<li><p>Локальные файлы - мы можем добавить в проект локальные файлы html и запускать их в WebView</p></li>
</ul>
<h3>Просмотр данных из интернета</h3>
<p>Для загрузки данных нам надо передать свойству <span class="b">Source</span> элемента WebView определенный путь:</p>
<pre class="brush:c#;">
...
<img src="./pics/2.35.png" alt="WebView in Xamarin" />
<p>Аналог в xaml:</p>
<pre class="brush:xml;">
&lt;StackLayout&gt;
...
<h4>Примечение для iOS</h4>
<p>Начиная с версии iOS 9 система позволяет взаимодействовать только с теми серверами, которые применяют SSL. 
Для всех остальных серверов, которые не применяют SSL, надо вносить соответствующие значения в файл <span class="b">Info.plist</span>.</p>
<pre class="brush:xml;">
...
<p>В данном случае дается разрешение для работы с сайтом xamarin.com. Но если нам вообще надо снять ограничение, то мы можем использовать более универсальную, но менее 
безопасную настройку:</p>
<pre class="brush:xml;">
&lt;key&gt;NSAppTransportSecurity&lt;/key&gt;
    &lt;dict&gt;
...
<p>Чтобы внести изменения в файл, надо его открыть как xml-файл в обычном текстовом редакторе.</p>
<h3>HTML-строки</h3>
<pre class="brush:c#;">
class MainPage : ContentPage
{
...
<img src="./pics/2.64.png" alt="Промотр html-кода в WebView в Xamarin Forms" />
<h3>Локальные html-файлы</h3>
<p>WebView может запускать файлы HTML, CSS и Javascript, которые встроены в приложение. Например, пусть у нас есть такой html-файл <i>index.html</i>:</p>
<pre class="brush:xml;">
&lt;html&gt;
  &lt;head&gt;
...
<p>Этот файл использует таблицу стилей из файла styles.css и файл изображения image2.png. Также надо учесть, что данный файл имеет кодировку utf-8.</p>
<p>Теперь чтобы использовать этот и сопутствующие файлы, нам надо добавить их во все три проекта. Но перед добавлением настроеим главный проект. Допустим, 
у нас все загружается в классе страницы MainPage:</p>
<pre class="brush:c#;">
...
<p>Во-первых, зедсь определяется вспомогательный интерфейс IBaseUrl, через который, используя внедрение зависимостей, мы будем получать путь к фйлу.</p>
<p>Во-вторых, так как нам надо установить путь, мы используем объект <span class="b">UrlWebViewSource</span>. А через вызов <code>DependencyService.Get&lt;IBaseUrl&gt;().Get()</code> 
мы будем получать от каждой отдельной платформы ее путь к файлу index.html.</p>
<p>Теперь настроим остальные проекты.</p>
<h4>Android</h4>
<p>В проект для Android в папку Assets добавим все необходимые файлы html, css, js, файлы изображений. А в сам этот проект добавим дополнительный класс <span class="b">BaseUrl_Android</span>:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>То есть здесь мы реализуем интерфейс IBaseUrl и возвращаем путь к папке с файлами.</p>
<img src="./pics/2.65.png" alt="Локальные файлы html в Android в Xamarin Forms" />
<h4>iOS</h4>
<p>В проект для iOS аналогично добавляем все используемые файлы в папку Resources. И затем в проект добавляем класс <span class="b"></span>:</p>
<pre class="brush:c#;">
using Foundation;
using HelloApp.iOS;
using Xamarin.Forms;
...
<p>Опять же это реализация интерфейса IBaseUrl, которая возвращает путь к файлам.</p>
<img src="./pics/2.66.png" alt="Локальные файлы html в iOS в Xamarin Forms" />
<h4>UWP</h4>
<p>Далее в проект для UWP также добавляем все необходимые файлы, можно прямо в корень проекта, можно специально создать для них подкаталг. 
И затем добавляем в проект новый класс <span class="b">BaseUrl_Windows</span>:</p>
<pre class="brush:c#;">
using HelloApp.UWP;
using Xamarin.Forms;
...
<img src="./pics/2.67.png" alt="Локальные файлы html в UWP в Xamarin Forms" />
<p>Теперь вне зависимости от типа устройства главный проект будет получать корректный путь и должным образом устанавливать источник ресурса для WebView:</p>
<img src="./pics/2.68.png" alt="Local files html in WebView in Xamarin Forms" />
...
<div class="nav"><p><a href="./3.14.php">Назад</a><a href="./">Содержание</a><a href="./3.8.php">Вперед</a></p></div></div>
<div class="socBlock">
