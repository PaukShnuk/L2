...
<html >
<head>
<title>Xamarin Forms | Стилизация текста</title>
<meta charset="utf-8" />
<meta name="description" content="Стилизация текста в Xamarin Forms, свойства FontFamily и FontSize">
...
<h2>Стилизация текста</h2><div class="date">Последнее обновление: 09.01.2021</div>
...
<p>Если мы работаем с текстом, то для стилизации его вывода мы можем использовать ряд свойств:</p>
<ul>
<li><p><span class="b">FontFamily</span>: устанавливает применяемое семейство шрифтов</p></li>
<li><p><span class="b">FontSize</span>: устанавливает высоту шрифта</p></li>
<li><p><span class="b">FontAttributes</span>: устанавливает дополнительные визуальные эффекты шрифта - выделение жирным или курсивом</p></li>
</ul>
<p>Для установки семейства используемых шрифтов свойству <span class="b">FontFamily</span> в качестве значения передается название шрифта. Однако 
при установке шрифта надо учитывать, что данный шрифт должен поддерживаться на всех используемых платформах. Например, установка в коде:</p>
<pre class="brush:c#;">
Label header = new Label() { Text = "Xamarin Forms in Arial" };
header.FontFamily = "Arial";
</pre>
<p>Установка в XAML:</p>
<pre class="brush:xml;">
&lt;Label Text="Xamarin Forms in Arial" FontFamily="Arial" /&gt;
</pre>
<h3>Свойство FontSize</h3>
<p>Для установки высоты шрифта используется значение типа double:</p>
<pre class="brush:c#;">
...
<p>В то же время может возникнуть вопрос, какой лучше шрифт использовать на том или ином устройстве. Для этого в классе <span class="b">Device</span> 
определен статический метод <span class="b">GetNamedSize()</span>. Этот метод в качестве первого параметра требует одну из констант из перечисления 
<span class="b">NamedSize</span>:</p>
<ul>
...
<li><p><code>Subtitle</code></p></li>
<li><p><code>Title</code></p></li>
<li><p><code>Body</code></p></li>
<li><p><code>Caption</code></p></li>
<li><p><code>Header</code></p></li>
</ul>
<p>Все эти константы представляют разные размеры шрифтов от маленького до большого, характерные непосрественно для платформы, на которой запускается приложение.</p>
<p>В качестве второго параметра передается тип элемента, для которого устанавливается шрифт:</p>
<pre class="brush:c#;">
...
<h3>FontAttributes</h3>
<p>FontAttributes позволяет выделить текст жирным или курсивом. Для этого он принимает значение из перечисления <span class="b">FontAttributes</span>:</p>
<ul>
<li><p><code>Bold</code>: выделение жирным</p></li>
<li><p><code>Italic</code>: выделение курсивом</p></li>
</ul>
<p>Применение:</p>
<pre class="brush:c#;">
Label header = new Label() { Text = "Привет из Xamarin Forms" };
...
<p>Свойство Text элемента Label позволяет устанавливать простой текст, который мы можем стилизовать с помощью ранее рассмотренных свойств. 
Однако текст с каким-то более сложным форматированием так не задать. И для этого в элементе Label определено вспомогательное свойство <span class="b">FormattedText</span>, 
которое допускает сложное форматирование.</p>
<p>Свойство <code>FormattedText</code> хранят не просто строку текста, а объект типа <span class="b">FormattedString</span>, 
который инкапсулирует объекты типа <span class="b">Span</span>. Каждый объект Span как раз и представляет кусок некоторым образом стилизованного текста. Для стилизации 
объекта Span применются все те же свойства:</p>
<ul>
<li><p><code>Text</code></p></li>
<li><p><code>FontFamily</code></p></li>
<li><p><code>FontSize</code></p></li>
<li><p><code>FontAttributes</code></p></li>
<li><p><code>ForegroundColor</code></p></li>
<li><p><code>BackgroundColor</code></p></li>
</ul>
<p>Например,создадим текст со сложынм форматированием в коде:</p>
<pre class="brush:c#;">
...
<img src="./pics/2.56.png" alt="Форматирование текста в Xamarin Forms" />
<h3>Перевод строки</h3>
<p>Для переноса текста на новую строку используется значение "\n". Например:</p>
<pre class="brush:c#;">
...
<img src="./pics/2.57.png" alt="Перенос текста на новую строку в Xamarin Forms" />
...
<div class="nav"><p><a href="./3.18.php">Назад</a><a href="./">Содержание</a><a href="./3.7.php">Вперед</a></p></div></div>
<div class="socBlock">
