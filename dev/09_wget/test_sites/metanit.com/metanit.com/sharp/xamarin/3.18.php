...
<html >
<head>
<title>Xamarin Forms | Работа с цветом</title>
<meta charset="utf-8" />
<meta name="description" content="Установка цвета фона и цвета текста в Xamarin Forms">
...
<h2>Работа с цветом</h2><div class="date">Последнее обновление: 09.01.2021</div>
...
<p>За установку цвета фона и текста элементов отвечают свойства <span class="b">BackgroundColor</span> и <span class="b">TextColor</span> 
соответственно. В качестве значение они принимают структуру <code>Color</code>:</p>
<pre class="brush:c#;">
...
<img src="./pics/2.55.png" alt="Установка цвета в Xamarin Forms" />
<p>Кроме встроенных констант типа <code>Color.Red</code> также для установки цвета мы можем указать и другие значения, используя структуру 
<span class="b">Color</span>. Для этого нам надо передать значение для одной из компонент красного, зеленого, синего цветов, смесь которых даст 
финальный цвет. Передаваемое значение имеет тип double и должно находиться в диапазоне от 0.0 до 1.0.</p>
<p>Чтобы передать эти значения, можно использовать один из конструкторов структуры Color:</p>
<ul>
<li><p><code>new Color(double grayShade)</code>: устанавливает тон серого цвета</p></li>
<li><p><code>new Color(double r, double g, double b)</code>: устанавливает компоненты красного, зеленого и синего</p></li>
<li><p><code>new Color(double r, double g, double b, double a)</code>: добавляет еще один параметр - <code>a</code>, который передает прозрачность 
и имеет значение от 0.0 (полностью прозрачный) до 1.0 (не прозрачный)</p></li>
</ul>
<p>Использование конструктора:</p>
<pre class="brush:c#;">
Label header = new Label() { Text = "Привет из Xamarin Forms" };
header.BackgroundColor = new Color(0.9, 0.9, 0.8); //rgb
</pre>
<p>Также для установки цвета мы можем использовать ряд статических методов:</p>
<ul>
<li><p><span class="b">Color.FromHex(string hex)</span>: возвращает объект Color, созданный по переданному в качестве параметра шестнадцатеричному значению</p></li>
<li><p><span class="b">Color.FromRgb(double r, double g, double b)</span>: возвращает объект Color, для которого также устанавливаются компоненты красного, зеленого и синего</p></li>
<li><p><span class="b">Color.FromRgb(int r, int g, int b)</span>: аналогичен предыдущей версии метода, только теперь компоненты красного, зеленого и синего 
имеют целочисленные значения от 0 до 255</p></li>
<li><p><span class="b">Color.FromRgba(double r, double g, double b, double a)</span>: добавляет параметр прозрачности со значением от 0.0 (полностью прозрачный) до 1.0 (не прозрачный)</p></li>
<li><p><span class="b">Color.FromRgba(int r, int g, int b, int a)</span>: добавляет параметр прозрачности со значением от 0 (полностью прозрачный) до 255 (не прозрачный)</p></li>
<li><p><span class="b">Color.FromHsla(double h, double s, double l, double a)</span>: устанавливает последовательно параметры <code>h</code> (hue - тон цвета), <code>s</code> (saturation - 
насыщенность), <code>l</code> (luminosity - яркость) и прозрачность.</p></li>
</ul>
<p>Например:</p>
<pre class="brush:c#;">
Label header = new Label() { Text = "Привет из Xamarin Forms" };
...
<p>В xaml мы можем задавать цвет с помощью шестнадцатеричных значений также, как в HTML/CSS:</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./3.17.php">Назад</a><a href="./">Содержание</a><a href="./3.19.php">Вперед</a></p></div></div>
<div class="socBlock">
