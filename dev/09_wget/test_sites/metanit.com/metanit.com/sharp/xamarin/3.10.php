...
<html >
<head>
<title>Xamarin Forms | Выбор даты и времени. DatePicker и TimePicker</title>
<meta charset="utf-8" />
<meta name="description" content="Выбор даты и времени в Xamarin Forms с помощью DatePicker и TimePicker">
...
<h2>Выбор даты и времени. DatePicker и TimePicker</h2><div class="date">Последнее обновление: 10.01.2021</div>
...
<h3>DatePicker</h3>
<p>DatePicker предназначен для выбора дат. Для его управления могут использоваться следующие свойства:</p>
<ul>
<li><p><span class="b">MaximumDate</span>: максимальная возможная дата, по умолчанию равна 31 декабря 2100</p></li>
<li><p><span class="b">MinimumDate</span>: минимальная возможная дата, по умолчанию равна 1 января 1900</p></li>
<li><p><span class="b">Date</span>: выбранная дата, по умолчанию равна значению <code>DateTime.Today</code></p></li>
<li><p><span class="b">Format</span>: определяет формат даты</p></li>
</ul>
<p>Например:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>Чтобы отследить выбор даты, мы можем использовать обработчик события <code>DateSelected</code></p>
<p>Но при создании элемента надо учитывать, что в конечном счете Xamarin использует стандартные механизмы для отображения этого элемента на каждой конкретной мобильной платформе. 
Так, на Android он будет выглядеть следующим образом:</p>
<img src="./pics/2.27.png" alt="DatePicker in Xamarin" />
<p>При установке дат в XAML необходимо передавать значение в формате, которое бы будет понятно для <code>DateTime.Parse()</code>. 
Наиболее простой способ заключается в использовании краткого формата даты "MM/dd/yyyy". Так, аналогичный пример в xaml выглядел бы следующим образом:</p>
<pre class="brush:xml;">
...
<p>И в этом случае нам надо задать обработчик события выбора даты с файле кода:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<h3>TimePicker</h3>
<p>TimePicker представляет элемент управления для отображения времени:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>Если необходимо отследить выбор времени в TimePicker, то можно подписаться на событие <code>PropertyChanged</code>.</p>
<p>Конкретное отображение TimePickera также будет зависеть от конкретной платформы. Например, на Android элемент будет выглядеть так:</p>
<img src="./pics/2.28.png" alt="TimePicker in Xamarin" />
<p>По существу этот тот же элемент Entry с дополнительными возможностями для установки времени.</p>
<p>Создание TimePicker в xaml:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
