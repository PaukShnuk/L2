...
<html >
<head>
<title>Xamarin Forms | Глобализация и локализация</title>
<meta charset="utf-8" />
<meta name="description" content="Глобализация и локализация в Xamarin Forms с помощью ресурсов">
...
<h1>Глобализация и локализация</h1><h2>Добавление локализации</h2><div class="date">Последнее обновление: 14.01.2021</div>
...
<p>Хотя можно создать приложение на одном языке, однако чем больше языков оно будет поддерживать, тем больше оно будет распространено, и в итоге тем больше 
будет прибыль от приложения. Рассмотрим, как создать в Xamarin Forms многоязычное приложение.</p>
<p>Допустим, у нас определена в приложении страница с простеньким интерфейсом:</p>
<pre class="brush:xml;">
&lt;StackLayout&gt;
...
<p>Рассмотрим, как можно локализовать эту форму на несколько языков.</p>
<p>Чтобы определить в приложении локализацию, нам надо использовать файлы ресурсов. Основной файл ресурсов (RESX) будет содержать все строковые значения для языка по умолчанию. Добавим в проект файл ресурсов. Для этого нажмем на проект 
правой кнопкой мыши и выберем <span class="b">Add -&gt; New Item...</span>. В окне добавления нового элемента выберем тип <span class="b">Resources Files</span>. 
Назовем его просто <span class="b">Resource.resx</span>:</p>
<img src="./pics/8.1.png" alt="Resources in Xamarin" />
<p>После добавления в Visual Studio новый файл автоматически откроется в дизайнере ресурсов, который представляет собой таблицу. Таблица имеет несколько столбцов. В столбце Name указывается название ресурса, а в столбце Value - его значение. Опционально можно в третьем столбце 
...
То есть мы можем создать три ресурса для текстовых значений этих элементов. Добавим эти элементы в таблицу:</P>
<img src="./pics/8.2.png" alt="Resource Editor in Xamarin" />
<p>Пусть это будет англоязычная культура по умолчанию, хотя это может быть любая другая культура.</p>
<p>Чтобы добавить поддержку нескольких языков, нам надо создать файлы ресурсов для соответствующих языков. Согласно соглашениям по именованию названия 
...
<span class="b">Resource.resx</span>, то для немецкого языка, код которого - <code>de</code>, название файла будет <span class="ii">Resource.de.resx</span>, для русского, например, - 
<span class="b">Resource.ru.resx</span>, для испанского - <span class="b">Resource.es.resx</span> и т.д.</p>
<p>Некоторые языковые культуры имеют разные ответвления, например, португальский в Португалии будет отличаться от португальского в Бразилии. Чтобы 
...
португальского: <span class="ii">Resource.pt-BR.resx</span></p>
<p>Скопируем файл ресурсов и вставим его в проект. Переименуем новый файл в <span class="b">Resource.ru.resx</span>. Изменим в нем определения ресурсов:</p>
<img src="./pics/8.3.png" />
<p>Попутно можно определить файлы ресурсов и для других языков. Добавление ресуросов происходит только в главный проект, остальные проекты мы вообще не трогаем. В итоге в главном 
проекте будет примерно следующая структура:</p>
<img src="./pics/8.5.png" alt="Localization in Xamarin.Forms" />
<p>Теперь для того, чтобы локализовать элементы интерфейса в коде, мы можем присвоить их тексту определенный ресурс из файла ресурсов. Например, в конструкторе 
класса страницы в файле кода с# можно прописать следующее:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>То есть название класса ресурса, которое совпадает с именем файла ресурса - "Resource", после которого идет конкретный ресурс. Причем значения для ресурсов 
будут браться из того файла, который соответствует языковой культуре мобильного устройства. Например:</p>
<img src="./pics/8.4.png" alt="Localization in Xamarin" />
