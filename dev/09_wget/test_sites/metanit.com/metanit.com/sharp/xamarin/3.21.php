...
<html >
<head>
<title>Xamarin Forms | Таймеры</title>
<meta charset="utf-8" />
<meta name="description" content="Использование таймеров в Xamarin Forms, метод Device.StartTimer">
...
<h2>Таймеры</h2><div class="date">Последнее обновление: 11.01.2021</div>
...
<p>Применить таймер в Xamarin Forms можно разными способами. Первый способ представляет использование метода 
<span class="b">Device.StartTimer()</span>, который определен в пространстве имен Xamarin.Forms. Он имеет следующее определение:</p>
<pre class="brush:c#;">Device.StartTimer(TimeSpan interval, Func&lt;bool&gt; callback);</pre>
<p>Первый параметр задает временный интервал, через который будет действовать метод из второго параметра - функция обратного вызова. Причем эта функция должна возвращать булевое значение: 
true - если надо, чтобы таймер продолжал выполнение, и false, если надо, чтобы таймер остановился. Например:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>Здесь определена кнопка, на котороую выводится текущее время. Для вывода времени запущен таймер, причем он срабатывает раз в секунду 
(<code>TimeSpan.FromSeconds(1)</code>). По таймеру срабатывает метод OnTimerTick, который возвращает значение переменной alive. Через нажатие на кнопку мы можем 
проконтроллировать значение этой переменной и тем самым остановить или снова запустить таймер.</p>
<img src="./pics/2.72.png" alt="Таймеры в Xamarin Forms" />
<p>Еще одну форму таймера представляет симуляция таймера с помощью временной задержки <span class="b">Task.Delay()</span>:</p>
<pre class="brush:c#;">
using System;
...
<p>Этот практически аналогичный пример за тем исключением, что тут явным образом запускается асинхронный метод DisplayTime(), который отрабатывает 
во вторичном потоке. Поэтому, несмотря на то, что он запускается в конструкторе, после завершения конструктора он продолжит работу.</p>
...
<div class="nav"><p><a href="./3.8.php">Назад</a><a href="./">Содержание</a><a href="./12.1.php">Вперед</a></p></div></div>
<div class="socBlock">
