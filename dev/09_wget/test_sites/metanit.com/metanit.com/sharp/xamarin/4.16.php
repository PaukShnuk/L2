...
<html >
<head>
<title>Xamarin Forms | Производительность ListView</title>
<meta charset="utf-8" />
<meta name="description" content="Производительность ListView в Xamarin Forms, стратегии кэширования элементов RecycleElement и RetainElement">
...
<h2>Производительность ListView</h2><div class="date">Последнее обновление: 12.01.2021</div>
...
<p>Хотя ListView является двольно мощным элементом для отображения данных, он все таки имеет некоторые ограничения. 
...
Однако существуют приемы и способы, которые позволят повысить роизводительность и постараться нивелировать возможный отрицательный эффект при подобных действиях.</p>
<h3>Кэширование</h3>
<p>Элемент ListView в Xamarin Forms опирается на нативные реализации этого элемента на Android, iOS, UWP, и на каждой из этих платформ 
...
не создавать тысячи объектов, которые номинально имеются в списке, тем самым уменьшая потребление памяти.</p>
<p>В Xamarin.Forms существуют две стратегии кэширования, которые описываются перечислением <span class="b">ListViewCachingStrategy</span>. Для каждой стратегии в этом 
перечислении определено соответствующее значение:</p>
<ul>
<li><p><span class="b">RetainElement</span></p></li>
<li><p><span class="b">RecycleElement</span></p></li>
</ul>
<h4>RetainElement</h4>
<p>По умолчанию к ListView в Xamarin.Forms применяется значение <span class="b">RetainElement</span>. Оно означает, что ListView будет создавать 
ячейку для каждого элемена в списке.</p>
<p>В каких случаях данная стратегия может быть предпочтительной:</p>
<ul>
<li><p>Когда каждая ячейка ListView имеет большое количество привязок (20-30 и более)</p></li>
<li><p>Когда шаблон ячейки часто меняется</p></li>
<li><p>Когда другая стратегия RecycleElement при тех же условиях показывает худшие результаты</p></li>
</ul>
<h4>RecycleElement</h4>
<p>RecycleElement позволяет повторно использовать одни и те же ячейки, вместо их создания каждый раз, когда они попадают в зону видимости. Эта 
стратегия может быть предпочтительной в следующих случаях:</p>
<ul>
<li><p>Когда каждая ячейка ListView имеет небольшое количество привязок</p></li>
<li><p>Когда свойство BindingContext ячейки определяет все ее данные</p></li>
<li><p>Когда ячейки во многом аналогичны, а их шаблон остается неизменным</p></li>
</ul>
<p>Для установки стратегии кэширования у ListView в XAML применяется атрибут <span class="b">CachingStrategy</span>:</p>
<pre class="brush:xml;">
...
<p>В коде C# аналогом является передача в конструктор значения ListViewCachingStrategy:</p>
<pre class="brush:c#;">
ListView listView = new ListView(ListViewCachingStrategy.RecycleElement);
</pre>
<h3>Техники по оптимизации производительности</h3>
<ul>
<li><p>Оптимальнее к свойству <span class="b">ItemsSource</span> привязывать объект <code>IList&lt;T&gt;</code> вместо 
<code>IEnumerable&lt;T&gt;</code>, так как коллекции <code>IEnumerable&lt;T&gt;</code> не поддерживают произвольный доступ.</p></li>
<li><p>Где возможно, используйте встроенные типы ячеек -  TextCell или ImageCell вместо ViewCell.</p></li>
<li><p>Используйте как можно меньше элементов.</p></li>
<li><p>Если необходимо вывести данные различных типов, используйте элемент TableView вместо ListView.</p></li>
<li><p>Ограничено используйте метод <code>Cell.ForceUpdateSize</code>, так как он снижает производительность.</p></li>
<li><p>На Android избегайте установки видимости или цвета разделителя элементов в ListView, после инициализации ListView.</p></li>
<li><p>Избегайте изменения ячеек, которые основаны на BindingContext</p></li>
<li><p>Избегайте глубоковложенных иерархий элементов. Вместо подобных иерархий используйте AbsoluteLayout или Grid, чтобы уменьшить уровни вложенности элементов.</p></li>
<li><p>Избегайте установки любого значения структуры <code>LayoutOptions</code>, кроме значения <code>Fill</code>, так как на при этом значении на вычисления тратятся наименьшие ресурсы.</p></li>
<li><p>Избегайте размещения ListView внутри ScrollView, так как ListView имеет свой собственный скроллинг</p></li>
<li><p>Если необходимо построение комплексной сложной ячейки, то лучше для этой цели лучше создать новый элемент с помощью <a href="./14.1.php">рендереров</a>. Это поможет снизить отрицательный эффект от вычислений при скроллинге в списке.</p></li>
</ul>
<p>AbsoluteLayout в ряде случаев может производить компоновку элементов вообще без вычислений, что делает данный элемент довольно производительным. 
...
RelativeLayout передавайте применяемым ограничениям (Constraints) прямые значения, вместо использования выражений.</p>
...
<div class="nav"><p><a href="./4.10.php">Назад</a><a href="./">Содержание</a><a href="./4.11.php">Вперед</a></p></div></div>
<div class="socBlock">
