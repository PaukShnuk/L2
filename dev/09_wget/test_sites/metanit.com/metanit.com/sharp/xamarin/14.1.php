...
<html >
<head>
<title>Xamarin Forms | Рендеринг элементов управления</title>
<meta charset="utf-8" />
<meta name="description" content="Рендеринг элементов управления в Xamarin Forms и создание нового элемента и его рендерера">
...
<h1>Рендеринг элементов управления</h1><h2>Создание нового элемента</h2><div class="date">Последнее обновление: 13.01.2021</div>
...
<p>За визуализацию элементов управления в Xamarin Forms на конкретных платформах отвечают специальные классы - рендереры (renderer). Например, для рендеринга элемета Button предназначен класс 
...
суффикс "Renderer".</p>
<p>Для каждой платформы есть своя реализация классов рендереров, которая опирается на нативные элементы управления, которые имеются на платформе. Так, элемент Label, доступный в 
...
На UWP есть аналогичный элемент - TextBlock. И задача рендерера для элемента Label использовать нужный нативный элемент для рендеринга Label.</p>
<p>При создании своего рендерера у нас есть три различные стратегии:</p>
<ul>
<li><p>Создание с нуля нового класса для элемента управления и рендерера для него</p></li>
<li><p>Наследования от уже имеющегося класса элемента</p></li>
<li><p>Создание своего рендерера имеющегося элемента</p></li>
</ul>
<p>Так, создадим новый элемент и для него рендерер. Вначале добавим в главный проект новый класс, который назовем 
<span class="b">HeaderView</span>:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>Пусть данный элемент будет выполнять функцию заголовка.</p>
<p>Для создания своего элемента нам надо унаследовать свой класс от класса <span class="b">View</span>. Больше пока нам ничего не потребуется.</p>
<p>После этого мы уже можем использовать данный класс на странице. Например:</p>
<pre class="brush:c#;">
...
<p>Но если мы запустим приложение, то мы увидим пустую страницу, так как для элемента HeaderView еще не определен рендерер.</p>
<p>При запуске приложения Xamarin.Forms использует механизм рефлексии для поиска в загруженных библиотеках 
атрибута <span class="b">ExportRenderer</span>. Данный атрибут указывает на наличие рендерера для элемента Xamarin.Forms.</p>
<p>При этом важно понимать, что мы не можем создать какой-то общий рендерер сразу для всех платформ. На каждой платформе есть свой набор визуальных элементов, 
свои механизмы для их создания, и наша задача - задействовать эти элементы для каждой из платформ при определении рендерера. 
Поэтому при создании рендерера могут потребоваться знания о том, какие элементы доступны на той или иной платформе, как они работают и т.д.</p>
<p>Итак, добавим в проект для Android новый класс, который назовем <span class="b">HeaderViewRenderer</span>:</p>
<pre class="brush:c#;">
...
<p>Атрибут ExportRenderer принимает два параметра: тип элемента, для которого создается рендерер, и сам тип рендерера.</p>
<p>Сам класс рендерера наследуется от базового класса <span class="b">ViewRenderer</span>, который типизируется двумя параметрами (<code>ViewRenderer&lt;HeaderView, TextView&gt;</code>). 
...
для создания визуального элемента. В нашем случае таким классом является <span class="b">TextView</span>, который в Android представляет простую метку с текстом.</p>
<p>Основная работа во ViewRenderer производится в методе <span class="b">OnElementChanged()</span>. Этот метод вызывается при создании объекта HeaderView и создает 
нативный элемент управления, используемый для рендеринга HeaderView.</p>
<p>У класса ViewRender можно выделить два важнейших свойства, которые мы можем использовать:</p>
<ul>
<li><p><span class="b">Control</span>: указывает на создаваемый нативный элемент для данной платформы. В нашем случае TextView.</p></li>
<li><p><span class="b">Element</span>: указывает на элемент из Xamarin.Forms, для которого создается нативный объект. В нашем случае HeaderView.</p></li>
</ul>
<p>И в примере выше в методе <code>OnElementChanged()</code> вначале проверяем свойство <code>Control</code>, которое определено в базовом классе ViewRenderer. Это свойство 
должно представлять создаваемый элемент, то есть в данном случае TextView.</p>
<p>При первом вызове метода <code>OnElementChanged()</code> свойство Control имеет значение <code>null</code>. Поэтому нам надо создать объект TextView.</p>
<p>Для настройки TextView мы можем применять различные его свойства и методы. В данном случае устанавливается свойство <code>Text</code>, а также зеленый цвет текста и 
высота шрифта в 25 единиц.</p>
<p>После его создания объект передается в метод <span class="b">SetNativeControl()</span>. После этого устанавливается свойство Control, 
которое в качестве значения приобретает созданный TextView.</p>
<p>Теперь если мы запустим приложение на Android, то увидим наш элемент HeaderView, за которым фактически будет скрываться нативный элемент TextView.</p>
<img src="./pics/13.1.png" alt="Customm Renderer in Xamarin" />
<p>Далее добавим новый класс HeaderViewRenderer в проект для iOS:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>Здесь применяется похожая схема: также используется атрибут ExportRenderer, а класс рендерера наследуется от ViewRenderer. Только теперь для создания визуального 
элемента применяется нативный класс <span class="b">UILabel</span>, который имеется на iOS.</p>
<p>И также добавим новый класс <span class="b">HeaderViewRenderer</span> в проект для UWP:</p>
<pre class="brush:c#;">
...
<p>На UWP для создания элемента применяется класс TextBlock из пространства имен <code>Windows.UI.Xaml.Controls</code>.</p>
<p>И теперь наш элемент HeaderView будет работать на всех трех платформах.</p>
...
<div class="nav"><p><a href="./13.1.php">Назад</a><a href="./">Содержание</a><a href="./14.2.php">Вперед</a></p></div></div>
<div class="socBlock">
