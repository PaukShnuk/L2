...
<html >
<head>
<title>Xamarin Forms | Сообщения и MessagingCenter</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка сообщений из одной страницы в другую в Xamarin Forms, класс MessagingCenter и его методы">
...
<h2>Сообщения и MessagingCenter</h2><div class="date">Последнее обновление: 13.01.2021</div>
...
<p>Xamarin Forms поддерживает такую функциональность, как отправку сообщений. Для этого в Xamarin определен класс 
<span class="b">MessagingCenter</span>, который поддерживает ряд методов:</p>
<ul>
<li><p><span class="b">Send&lt;TSender&gt;(TSender sender, string message)</span>: посылает сообщение message, в качестве отправителя выступает объект TSender</p></li>
<li><p><span class="b">Subscribe&lt;TSender&gt;(object subscriber, string message, Action&lt;TSender&gt; callback)</span>: 
устанавливает подписку для объекта subscriber на получение сообщения message. При получении события будет вызываться действие callback</p></li>
<li><p><span class="b">Unsubscribe&lt;TSender&gt;(object subscriber, string message)</span>: прекращает подписку для объекта subscriber на сообщение message</p></li>
</ul>
<p>MessagingCenter работает по принципу подписки или Publisher-Subscriber:</p>
<img src="./pics/5.15.png" alt="MessagingCenter in Xamarin Forms and Publisher-Subscriber" />
<p>Рассмотрим применение сообщений на примере. Добавим в проект класс страницы, который назовем MessagePage:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>Здесь определена одна кнопка для возврата на предыдущую страницу. При возврате с помощью метода <code>MessagingCenter.Send</code> отправляется сообщение. 
Название сообщения - "LabelChange". В качестве отправителя установлен текущий объект, поэтому метод типизирован типом <code>Page</code>. Хотя можно было бы и конкретизировать 
тип отправителя:</p>
<pre class="brush:c#;">
MessagingCenter.Send&lt;MessagePage&gt;(this, "LabelChange");
</pre>
<p>Таким образом, сообщение отправляется. Теперь подпишемся на это событие. Пусть у нас есть главная страница MainPage:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>Здесь определена кнопка для перехода к странице MessagePage, и также определен метод подписки на сообщение, в котором вызывается 
<code>MessagingCenter.Subscribe</code>. Причем метод MessagingCenter.Subscribe типизирован именно тем типом, который является отправителем, то есть тип Page.</p>
<p>Получателем является текущий объект, поэтому в метод передается в качестве первого параметра this.</p>
<p>Название сообщения должно совпадать с тем, которое используется при отправке, то есть в нашем случае "LabelChange".</p>
<p>Третий параметр - действие просто выполняет установку текста метки, хотя тут могла бы быть и более сложная логика.</p>
<p>Ну и в главном классе приложения App страница MainPage должна передаваться в NavigationPage:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>Теперь запустим приложение и перейдем с MainPage на MessagePage и обратно:</p>
<img src="./pics/5.12.png" alt="Отправка сообщений и MessagingCenter в Xamarin" />
...
<div class="nav"><p><a href="./5.3.php">Назад</a><a href="./">Содержание</a><a href="./3.22.php">Вперед</a></p></div></div>
<div class="socBlock">
