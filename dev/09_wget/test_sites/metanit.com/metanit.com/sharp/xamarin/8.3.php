...
<html >
<head>
<title>Xamarin Forms | Локализация XAML</title>
<meta charset="utf-8" />
<meta name="description" content="Локализация элементов в XAML в Xamarin Forms">
...
<h2>Локализация XAML</h2><div class="date">Последнее обновление: 14.01.2021</div>
...
<p>В прошлых темах мы рассмотрели локализацию в коде C#, но гораздо проще было бы локализовать элементы интерфейса сразу же в xaml. Для этого возьмем проект из прошлой темы.</p>
<p>Чтобы добавить в приложение возможность локализации элементов xaml, надо добавить класс расширения разметки. Итак, добавим в главный проект следующий 
класс:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>В строке <code>const string ResourceId = "HelloApp.Resource";</code> устанавливаются ресурсы для локализации. Так как у меня пространство имен проекта 
называется <code>HelloApp</code>, а файл ресурсов по умолчанию размещен в корне проекта и называется <code>Resource.resx</code>, 
то полное имя ресурса будет <code>HelloApp.Resource</code>.</p>
<p>Для извлечения ресурсов используется класс <code>ResourceManager</code> и его метод <code>GetString()</code></p>
<p>Теперь применим данный класс в xaml:</p>
<pre class="brush:xml;">
...
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
  ...
  &lt;StackLayout&gt;
	...
	&lt;Label x:Name="myLabel" FontSize="Large" Text="{local:Translate Header}" /&gt;
    &lt;Entry x:Name="myEntry" Placeholder="{local:Translate LoginText}" /&gt;
    &lt;Button x:Name="myButton" FontSize="Large"  Text="{local:Translate SaveButton}" /&gt;
  &lt;/StackLayout&gt;
&lt;/ContentPage&gt;
</pre>
<p>Чтобы получить доступ к классу, надо подключить текущее пространство имен проекта: <code>xmlns:local="clr-namespace:HelloApp;assembly=HelloApp"</code></p>
<p>Затем мы можем использовать привязку у тех свойствам элементов, которые надо локализовать: <code>Text="{local:Translate Header}"</code>. В данном случае 
local ссылается на текущее пространство имен, "Translate" ссылается на класс <code>TranslateExtension</code> - по умолчанию суффикс Extension отбрасывается. 
И затем указывается называется имя ресурса, к которому идет привязка.</p>
<p>В итоге мы можем убрать локализацию из класса приложения App, которая была определена в прошлых темеах, теперь он будет просто создавать стартовую страницу:</p>
<pre class="brush:c#;">
