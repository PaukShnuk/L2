...
<html >
<head>
<title>Xamarin Forms | Взаимодействие с веб-сервисом</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие с веб-сервисом из мобильного приложения на Xamarin Forms">
...
<h2>Взаимодействие с веб-сервисом</h2><div class="date">Последнее обновление: 14.01.2021</div>
...
<p>Теперь создадим проект Xamarin Forms, который будет взаимодействовать с сервисом ASP NET Core из прошлой темы. 
Первым делом добавим в проект через Nuget пакет <span class="b">System.Text.Json</span> для работы с json.</p>
<p>После добавления выше перечисленных пакетов определим в главном проекте класс, который будет представлять основную модель приложения - класс Friend:</p>
<pre class="brush:c#;">
...
<p>И также добавим в проект вспомогательный класс, через который мы будем взаимодействовать с сервером - класс FriendService:</p>
<pre class="brush:c#;">
...
<p>Для обращения к веб-сервису здесь применяется класс HttpClient и его методы. В данном случае предполагается, что веб-сервис запущен по адресу 
<span>http://192.168.1.18:3000</span>, в вашем случае адрес может быть иным.</p>
<p>При локальном тестировании, когда веб-приложение и мобильное приложение запускаются одновременно на локальной машине, советую 
запускать веб-приложение по протоколу http, а не https, чтобы у HttpClient не возникло проблем с сертификатами.</p>
<p>Первый метод <code>GetClient()</code> используется для создания и конфигурации объекта HttpClient. Здесь мы можем сразу установить различные заголовки. В данном 
случае мы устанавливаем заголовок Accept, чтобы сервер отдавал нам данные в формате json.</p>
<p>Метод Get загружает все данные и преобразует их в объекты Friend посредством метода <code>JsonSerializer.Deserialize()</code>:</p>
<pre class="brush:c#;">
public async Task&lt;IEnumerable&lt;Friend&gt;&gt; Get()
{
    HttpClient client = GetClient();
...
<p>Метод Add посылает серверу новые данные:</p>
<pre class="brush:c#;">
public async Task&lt;Friend&gt; Add(Friend friend)
{
    HttpClient client = GetClient();
...
<p>Для отправки объект сериализуется с помощью метода <code>JsonSerializer.Serialize()</code> в формат json.</p>
<p>Так как созданный в прошлой теме сервер после добавления возвращает добавленный объект, то соответственно здесь мы его можем получить и возвратить в качестве результата метода. 
При этом мы ожидаем, что ответ от сервера будет иметь статусный код 200. Если же статусный код другой, например, 40х, то возвращаем null.</p>
<p>Методы  Update и Delete, предназначенные для обновления и удаления данных, в целом аналогичны методу Add.</p>
