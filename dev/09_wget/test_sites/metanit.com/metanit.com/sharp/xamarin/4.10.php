...
<html >
<head>
<title>Xamarin Forms | Группировка в ListView</title>
<meta charset="utf-8" />
<meta name="description" content="Группировка в списке в Xamarin Forms, получение и настройка групп">
...
<h2>Группировка в ListView</h2><div class="date">Последнее обновление: 12.01.2021</div>
...
<p>Элемент ListView в Xamarin поддерживает возможности группировки. Рассмотрим, как мы можем сгруппировать элементы в списке</p>
<p>Для группировки в начале добавим в проект вспомогательный класс, который назовем Grouping:</p>
<pre class="brush:c#;">
...
<p>Класс Grouping типизирован двумя параметрами. Параметр K представляет тип ключа группы, который будет храниться в свойстве <code>Name</code>. 
...
А в конструкторе мы получаем все необходимые данные.</p>
<p>В качестве объектов возьмем опять же класс Phone:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>В коде страницы MainPage создадим список групп:</p>
<pre class="brush:c#;">
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
...
<p>В конструкторе переменная <code>phones</code> определяет общие данные, по которым создается коллекция групп в виде свойства <code>PhoneGroups</code>. Группировка 
в данном случае идет по свойству Company объекта Phone.</p>
<p>А в коде xaml у MainPage пропишем выражения привязки:</p>
<pre class="brush:xml;">
...
<p>Привязка ListView здесь идет к свойству PhoneGroups, которое содержит группы. Установка свойства <code>IsGroupingEnabled="True"</code> добавляет в ListView поддержку групп.</p>
<p>С помошью свойства <span class="b">GroupDisplayBinding</span> можно задать то значение, которое будет отображаться для каждой группы. В нашем случае идет 
привязка к имени группы, которое представляет критерий группировки.</p>
<p>И после запуска приложения все данные в списке будут сгруппированы по компаниям:</p>
<img src="./pics/3.21.png" alt="Группировка в ListView в Xamarin Forms" />
<p>Однако по умолчанию заголовки групп выглядят не очень хорошо, мало отличимы от основного содержимого элементов. И в этом случае мы можем настроить шаблон отображения 
заголовков групп. Для этого изменим разметку xaml:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<p>Свойство <span class="b">GroupHeaderTemplate</span> позволяет в корне изменить отображение заголовка, определив ему свой шаблон DataTemplate:</p>
<img src="./pics/3.10.png" alt="Группировка списков в Xamarin Forms" />
...
<div class="nav"><p><a href="./4.15.php">Назад</a><a href="./">Содержание</a><a href="./4.16.php">Вперед</a></p></div></div>
<div class="socBlock">
