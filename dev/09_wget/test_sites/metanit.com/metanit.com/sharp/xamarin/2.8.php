...
<html >
<head>
<title>Xamarin Forms | Запуск на iOS из Visual Studio</title>
<meta charset="utf-8" />
<meta name="description" content="Тестирование проекта Xamarin Forms для iOS из Visual Studio на Mac OS X">
...
<h2>Запуск на iOS из Visual Studio</h2><div class="date">Последнее обновление: 25.05.2019</div>
...
<p>Рассмотрим, как мы можем компилировать приложения под iOS с помощью Mac OS из Visual Studio. Прежде всего, на машине Mac OS X должны быть установлены 
...
последняя версия XCode.</p>
<p>Для подключения к Mac OS Visual Studio применяет SSH и компоненты Xamarion.iOS.</p>
<p>Допустим, нам надо скомпилировать тот же проект, что и в предыдущих темах для Windows 10 и Android. Прежде всего нам надо открыть доступ извне на 
самой машине под управлением Mac OS X. Для этого на Mac OS вызовем окно поиска и введем в него "Общий доступ", чтобы быстро перейти к нужным нам настройкам:</p>
<img src="./pics/2.44.png" alt="Общий доступ на Mac OS для Xamarin" />
<p>И перейдем в папку "Общий доступ". Далее в открывшихся настройках надо включить опцию "Удаленный вход":</p>
<img src="./pics/2.45.png" alt="Удаленный вход для Xamarin на Mac OS" />
<p>В этом окне надо отметить IP-адрес в рамках подсети, по которому мы будем подключаться к макбуку. В моем случае 192.168.0.100. 
Это были все необходимые настройки для Mac OS. Теперь перейдем к Visual Studio. Перейдем к пункту меню <span class="b">Tools-&gt;Options</span> 
и далее в открывшемся окне к пункту <span class="b">Xamarin-&gt;iOS Settings</span>.</p>
<img src="./pics/2.46.png" alt="iOS Settings в Visual Studio для Xamarin" />
<p>В этом окне стоит обратить внимание на поле "Remote Simulator to Windows". По умолчанию оно отмечено. Это значит, что тестируемое приложение 
будет запускаться на симуляторе iOS непосредственно в Windows. Если же мы уберем отметку с этого поля, то приложение будет запускаться на подключенном Mac OS.</p>
<p>Итак, нажмем в этом окне на кнопку "Pair to Mac". И откроется диалоговое окно "Pair to Mac" для подключения к Mac OS.</p>
<img src="./pics/2.47.png" alt="Подключение к Mac OS в Visual Studio" />
<p>В этом окне нажмем на кнопку "Add Mac..." и появившееся небольшое окошко введем IP-адрес нашего макбука в рамках подсети. Адрес можно найти в окне общего доступа на макбуке, где устанавливается 
удаленный вход. В моем случае это 192.168.0.100.</p>
<p>После этого откроется диалоговое окно, в котором надо будет ввести аутентификационные данные для подключения к Mac OS (то есть логин и пароль пользователя 
на машине Mac OS):</p>
<img src="./pics/2.48.png" alt="Подключение к Mac OS в Xamarin" />
<p>Если вы вдруг не уверены в правильности вводимого логина, то его можно узнать на Mac OS, введя в терминал команду <span class="b">whoami</span>.</p>
<p>При подключении Visual Studio может сообщить, что на версия Mono на MacOS и на Windows не совпадает, и предложит доустановить некоторые пакеты, чтобы привести все в соответствие. 
В этом случае надо все предложенные пакеты Mono, естественно, установить.</p>
<p>После успешного логина и подключения все окна можно закрыть. И затем мы сможем использовать удаленную машину Mac OS для компиляции приложения. Для этого назначим проект для iOS в качестве 
стартового. В Visual Studio мы сможем выбрать нужный симулятор iOS и запустить проект:</p>
<img src="./pics/2.49.png" alt="Запуск проекта для iOS в Visual Studio" />
<p>После этого на Mac OS запустится на симуляторе наш проект:</p>
<img src="./pics/2.50.png" alt="Компиляция проекта Xamarin для iOS" />
<p>Стоит отметить, что несмотря на то, что мы можем запустить приложения на симуляторе непосредственно в Windows, но все равно нам необходим Mac OS 
