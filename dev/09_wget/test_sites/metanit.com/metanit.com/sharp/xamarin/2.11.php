...
<html >
<head>
<title>Xamarin Forms | Расширения разметки XAML</title>
<meta charset="utf-8" />
<meta name="description" content="Расширения разметки XAML в Xamarin Forms, x:Static, x:Type и x:Array, интерфейс IMarkupExtension, создание своих расширений разметки xaml">
...
<h2>Расширения разметки XAML</h2><div class="date">Последнее обновление: 08.01.2021</div>
...
<p>Расширения разметки (Markup extensions) дают большую гибкость над тем, как устанавливать значения атрибутов в XAML.</p>
<p>Все классы расширений должны реализовать интерфейс <span class="b">IMarkupExtension</span>. Он имеет две версии:</p>
<pre class="brush:c#;">
...
<p>Метод <code>ProvideValue()</code> вызывается во время загрузки кода xaml и возвращает собственно то значение, которое присваивается атрибуту элемента.</p>
<p>В Xamarin Forms есть ряд встроенных расширений:</p>
<ul>
<li><p>x:Static</p></li>
<li><p>x:Type</p></li>
<li><p>x:Array</p></li>
<li><p>x:Null</p></li>
<li><p>x:Reference</p></li>
<li><p>StaticResource</p></li>
<li><p>DynamicResource</p></li>
<li><p>Binding</p></li>
<li><p>ConstraintExpression</p></li>
</ul>
<p>Расмотрим некоторые из них. А в последующих темах так или иначе затронем и остальные.</p>
<h3>x:Static</h3>
<p><span class="b">x:Static</span> позволяет привязать к атрибуту значение константы, статической переменной, статического свойства или значения перечисления. 
Например, в коде C# у страницы определим константу или статическую переменную:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>Тогда мы сможем использовать их значения следующим образом:</p>
<pre class="brush:xml;">
...
<p>Чтобы ссылаться на типы и их члены, которые определены в текущем проекте в коде C#, нам надо подключить пространство имен текущего проекта:</p>
<pre class="brush:xml;">xmlns:local="clr-namespace:HelloApp"</pre>
<p>В данном случае весь функционал, который определен в коде C# в текущем проекте, будет сопоставляться с префиксом <span class="b">local</span>. После 
параметра <span class="b">clr-namespace</span> указывается пространство имен текущего проекта. Обычно оно совпадает с названием проекта (в моем случае HelloApp). 
После этого в коде XAML мы можем через префикс local ссылаться на типы и их доступные переменные и свойства:</p>
<pre class="brush:xml;">Text="{x:Static local:MainPage.HEADER}"</pre>
<img src="./pics/2.94.png" alt="x:Static расширение в Xamarin Forms" />
<h3>x:Array и x:Type</h3>
<p>Расширение разметки <span class="b">x:Array</span> позволяет определить массив данных. А расширение <span class="b">x:Type</span> позволяет указать тип данных. Например, 
воспользуемся элементом ListView для вывода содержимого массива:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<p>Сложное свойство <code>ListView.ItemsSource</code> в качестве значения принимает массив. В данном случае в определении массива атрибут <code>Type="{x:Type x:String}"</code> указывает, что массив будет содержать данные типа String.</p>
<img src="./pics/1.16.png" alt="x:Array и x:Type в Xamarin Forms" />
<h3>Создание расширений XAML</h3>
<p>Иногда может возникнуть необходимость создать какое-то свое расширение для XAML. В этом случае нам надо создать класс, который будет реализовать интерфейс 
<span class="b">IMarkupExtension</span>.</p>
<p>К примеру, мы хотим чтобы через XAML можно было установить для фона элемента цвет из четырех компонент - ARGB. В этом случае мы можем создать следующий класс:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>Затем используем этот класс в XAML:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<p>Свойства <code>TextColor</code> и <code>BackgroundColor</code> в качестве значения принимают объект Color. И именно подобный объект генерируется 
методом <code>ProvideValue()</code> из класса ARGBColor. То есть значение "{local:ARGBColor Alpha=255, Red=0, Green77, Blue=64}" 
по сути будет возвращать должным образом инициализованный объект Color.</p>
<p>В результате мы увидим на странице метку, к которой применены цвета по настройкам ARGB:</p>
<img src="./pics/1.17.png" alt="XAML Markup Extensions in Xamarin Forms" />
...
<div class="nav"><p><a href="./2.12.php">Назад</a><a href="./">Содержание</a><a href="./3.1.php">Вперед</a></p></div></div>
<div class="socBlock">
