...
<html >
<head>
<title>Xamarin Forms | Подключение к существующей базе данных SQLite</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение к существующей базе данных SQLite в Xamarin Forms, особенности настройки проектов">
...
<h2>Подключение к существующей базе данных</h2><div class="date">Последнее обновление: 14.01.2021</div>
...
<p>В прошлой теме мы подключались к базе данных SQLite, которая автоматически создавалась (при ее отсутствии) при запуске приложения. Однако это не всегда бывает 
...
Поэтому рассмотрим, как мы можем добавить в проект уже имеющуюся базу данных SQLite и использовать ее.</p>
<p>Вначале создадим саму базу данных. Для работы с SQLite установим специальный инструмент - 
<a href="http://sqlitebrowser.org/" rel="nofollow">DB Browser for SQLite</a>.</p> 
<p>Откроем DB Browser for SQLite и для создания новой базы данных нажмем на кнопку <span class="b">New Database</span>. В открывшемся окне выберем место расположения файла базы 
данных и укажем для нее имя, например, <span class="b">friends5</span>.</p>
<p>Далее нам будет предложено добавить в базу данных таблицы. Возьмем проект из прошлой темы. В нем была модель Friend:</p>
<pre class="brush:c#;">
using SQLite;
...
<p>Для хранения объектов данной модели определим в базе данных новую таблицу Users и добавим четыре столбца _id, Name, Email, Phone, как на скриншоте::</p>
<img src="./pics/6.10.png" alt="Создание базы данных SQLite для Xamarin Forms" />
<p>Эта таблица имеет столбцы, которые соответствуют свойствам класса Friend.</p>
<p>Можно тут же добавить некоторые начальные данные в таблицу.</p>
<img src="./pics/6.16.png" alt="базы данных SQLite для Xamarin Forms" />
<p>Теперь нам надо определить логику передачи файла базы данных в каталог приложения. Это можно сделать различными способами, которые в каких-то 
ситуациях могут оказаться по своему более оптимальными или менее оптимальными. В данном случае рассмотрим простейший способ.</p>
<p>После создания базы данных добавим ее в главный проект решения и установим в окне Properties (Свойства) у файла базы данных в поле 
<span class="b">Build Action</span> значение <span class="b">Embedded Resource</span>:</p>
<img src="./pics/6.11.png" alt="Добавление базы данных SQLite для Xamarin Android" />
<p>Далее откроем файл App.xaml.cs и определим логику копирования базы данных:</p>
<pre class="brush:c#;">
using System;
using System.IO;
using Xamarin.Forms;
...
<p>В данном случае наша задача скопировать файл базы данных в папку . Для этого вначале получаем встроенный ресурс посредством метода 
<code>assembly.GetManifestResourceStream($"HelloApp.{DATABASE_NAME}"))</code>. "HelloApp" в данном случае это название текущей сборки - 
по сути название проекта, соответственно если название проекта другое, то оно может изменяться.</p>
<p>Далее данные из полученного потока с помощью объекта FileStream сохраняем в нужное нам место.</p>
<p>При этом нам НЕ надо ничего добавлять в другие проекты или изменять их код.</p>
<p>Таким образом, при запуске приложения и обращения к базе данных произойдет копирование файла в нужное место и затем новосозданный файл будет использоваться для  
работы с приложением.</p>
<p>Весь остальной код в данном случае остается тем же, что и вв прошлой теме. В частности, код репозитория - класса <span class="b">FriendAsyncRepository</span>:</p>
<pre class="brush:c#;">
...
<p>Xaml-код страницы <span class="b">MainPage.xaml</span> выглядит следующим образом:</p>
<pre class="brush:xml;">
...
<p>Код <span class="b">MainPage.xaml.cs</span>:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>Интерфейс страницы <span class="b">FriendPage.xaml</span> для отображения одного объекта:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<p>Файл связанного кода <span class="b">FriendPage.xaml.cs</span>:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
