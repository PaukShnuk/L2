...
<html >
<head>
<title>Xamarin Forms | Добавление событий</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление событий в новый элемент в Xamarin Forms и применение их при рендеринге">
...
<h2>Добавление событий</h2><div class="date">Последнее обновление: 13.01.2021</div>
...
<p>В прошлой теме в элемент HeaderView были добавлены свойства. Теперь добавим также и события. В частности, событие нажатия.</p>
<p>Для вызова события нам необязательно писать какой-то код в самом элементе. В нем мы можем только определить нужные события, а затем их генерировать в 
рендерере.</p>
<p>Так, определим в классе HeaderView событие <code>TappedOrClickEvent</code>:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>На странице используем это событие:</p>
<pre class="brush:xml;">
...
<p>А в файле кода страницы пропишем обработчик:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>Теперь изменим рендерер для Android:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>Ключевым моментом для обработки события здесь является обработка события Touch нативного элемента TextView. В его обработчике по сути мы переадресуем вызовы 
элементу HeaderView:</p>
<pre class="brush:c#;">Element.FireClick(System.EventArgs.Empty);</pre>
<p>Это общая схема обработки событий, которая работает вне зависимости от события элемента: в обработчиках событий нативного элемента мы генерируем события элемента 
из Xamarin.Forms.</p>
<p>Также изменим код рендерера для iOS:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>Здесь для обработки нажатия используется класс <span class="b">UITapGestureRecognizer</span>.</p>
<p>И изменим код рендерера для UWP:</p>
<pre class="brush:c#;">
...
<img src="./pics/13.4.png" alt="Events in Custom Renderer in Xamarin.Forms" />
