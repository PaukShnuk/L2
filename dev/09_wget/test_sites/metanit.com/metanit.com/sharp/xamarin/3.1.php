...
<html >
<head>
<title>Xamarin Forms | Контейнеры компоновки</title>
<meta charset="utf-8" />
<meta name="description" content="Элементы компоновки в Xamarin Forms, добавления множества элементов на страницу">
...
<h1>Контейнеры компоновки</h1><h2>Элементы компоновки</h2><div class="date">Последнее обновление: 09.01.2021</div>
...
<p>В Xamarin мы можем использовать ряд элементов. Их объединяет то, что все они унаследованы от общего класса <span class="b">View</span> 
и поэтому наследуют ряд общих свойств.</p>
<p>Кроме обычных элементов типа кнопок и текстовых полей, в Xamarin Forms также имеются контейнеры, которые позволяют скомпоновать содержимое, расположить его определенным образом.</p>
<p>Для определения содержимого страницы класс страницы <span class="b">ContentPage</span> имеет свойство <span class="b">Content</span>. По умолчанию этому свойству 
присваивается один элемент Label.</p>
<p>Но свойство <code>Content</code> имеет ограничение - для него можно установить только один элемент. 
...
который наследуется от базового класса <span class="b">Layout&lt;T&gt;</span>:</p>
<ul>
<li><p><span class="b">StackLayout</span></p></li>
<li><p><span class="b">AbsoluteLayout</span></p></li>
<li><p><span class="b">RelativeLayout</span></p></li>
<li><p><span class="b">Grid</span></p></li>
<li><p><span class="b">FlexLayout</span></p></li>
</ul>
<p>Все элементы компоновки имеют свойство <span class="b">Children</span>, позволяющее задать или получить вложенные элементы.</p>
<p>Например, установка вложенных элементов в коде C#:</p>
<pre class="brush:c#;">
...
<img src="./pics/2.58.png" alt="Контейнеры компоновки в Xamarin Forms" />
<p>Аналогично в XAML мы могли бы написать:</p>
<pre class="brush:xml;">
...
<p>Поскольку коллекция Children представляет собой обычный список, то он поддерживает операции по управлению элементами. В частности, мы можем 
динамически добавить новые элементы. Например:</p>
<pre class="brush:c#;">stackLayout.Children.Add(new Label { Text = "Третья метка" });</pre>
<p>Хотя в данном случае коллекция элементов рассматривалась на примере контейнера StackLayout, но тот же принцип работы с элементами будет характерен и для других элементов компоновки.</p>
