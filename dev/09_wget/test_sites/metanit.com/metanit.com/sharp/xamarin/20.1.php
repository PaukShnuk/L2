...
<html >
<head>
<title>Xamarin Forms | Flyout</title>
<meta charset="utf-8" />
<meta name="description" content="Создание одностраничного мобильного приложения с Flyout в Xamarin Forms, создание меню и вкладок, классы FlyoutItem, Tab и FlyoutContent">
...
<h1>Flyout</h1><h2>Первое приложение с Flyout</h2><div class="date">Последнее обновление: 12.01.2021</div>
...
<p>Flyout позволяет создавать одностраничные приложения. Создадим проект с использованием Flyout. Для упрощения создания подобного проекта Visual Studio 
предлагает готовый шаблон. Поэтому при создании нового проекта Xamarin Forms можно выбрать шаблон <span class="b">Flyout</span>:</p>
<img src="./pics/shell1.png" alt="Flyout в Xamarin Forms" />
<p>Однако по умолчанию этот тип проекта добавляет множество дополнительных файлов и создает проект с уже довольно разветвленной структурой. 
Поэтому, чтобы более детально разобраться в Flyout создадим простейший проект по типу <span class="b">Blank</span>:</p>
<img src="./pics/shell2.png" alt="Мобильное приложение Flyout в Xamarin Forms" />
<p>Следует отметить, что на данный момент использование Flyout доступно только в проектах для Android и iOS (для UWP недоступно), поэтому в окне выше пукт для Windows(UWP) не надо отмечать.</p>
<p>После создания проекта мы увидим стандартную простейшую структуру:</p>
<img src="./pics/shell3.png" alt="Использование AppFlyout в Xamarin Forms" />
<p>Итак, по умолчанию у нас есть стандартная страница MainPage, которая представляет класс ContentPage. В приложении Flyout основной класс страницы, 
который загружается классом App при старте приложения, должен представлять класс <span class="b">Shell</span>.</p>
<p>Поэтому перейдем к файлу <span class="b">MainPage.xaml</span> и изменим его следующим образом:</p>
<pre class="brush:xml;">
...
<p>И также изменим файл <span class="b">MainPage.xaml.cs</span>:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>Теперь определение класса MainPage и в XAML, и в C# представляет класс Shell.</p>
<h3>Наполнение элементами</h3>
<p>Flyout/Shell представляет простую систему наполнения страницы элементами. Контейнерами верхнего уровня являются объекты 
<span class="b">FlyoutItem</span> и <span class="b">TabBar</span>, которые позволяют создать механизм навигации, перехода по вкладкам внутри страницы.</p>
<p><code>FlyoutItem</code> и <code>TabBar</code> содержат объект <span class="b">Tab</span>, который предназначен для группировки контента в виде одной нижней вкладки.</p>
<p>Объект <span class="b">Tab</span> содержит объекты <span class="b">ShellContent</span>, которые представляют верхние вкладки. В ShellContent 
добавляются стандартные страницы <span class="b">ContentPage</span>, в которые собственно и помещаются визуальные компоненты типа контейнеров компоновки, кнопок, текстовых полей и других 
элементов.</p>
<p>Простейший пример Flyout:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;Shell xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<p>Два элемента FlyoutItem представляют два пункта в боковом меню:</p>
<img src="./pics/shell4.png" alt="FlyoutItem in Shell in Xamarin Forms" />
<p>На каждом FlyoutItem есть по два элемента Tab, которые образуют нижние вкладки</p>
<img src="./pics/shell5.png" alt="Tab in Shell in Xamarin Forms" />
<p>В каждом элементе Tab есть по два элемента ShellContent, которые образуют верхние вкладки. А ShellContent с помощью ContentPage собственно определяет 
содержимое.</p>
<img src="./pics/shell6.png" alt="ShellContent in Flyout in Xamarin Forms" />
<p>Таким образом, Shell может хранить несколько FlyoutItem. Один FlyoutItem может хранить несколько вкладок Tab. А одна вкладка Tab может хранить 
несколько вкладок ShellContent.</p>
