...
<html >
<head>
<title>Xamarin Forms | Основные операции с SQLite</title>
<meta charset="utf-8" />
<meta name="description" content="Использование SQLite.NET для работы с базой данных SQLite в Xamarin Forms, CRUD интерфейс">
...
<h1>Базы данных SQLite</h1><h2>Основные операции с SQLite</h2><div class="date">Последнее обновление: 14.01.2021</div>
...
<img src="./pics/6.1.png" alt="SQLite in Xamarin Forms" />
...
<ul>
<li><p>Автор: <span class="b">SQLite-net</span></p></li>
<li><p>Id: <span class="b">sqlite-net-pcl</span></p></li>
</ul>

<p>Вначале определим класс, объекты которого будут храниться в базе данных. Добавим в главный проект следующий класс Friend:</p>
<pre class="brush:c#;">
using SQLite;
...
<p>Класс Friend выступает в качестве модели приложения. Этот класс использует атрибуты, которые позволяют настроить его отображение на таблицу в бд. 
Для настройки мы можем использовать следующие атрибуты:</p>
<ul>
<li><p><span class="b">[PrimaryKey]</span>: применяется к свойству типа <code>int</code> и 
указывает, что столбец в таблице, который соответствует этому свойству, будет выполнять роль первичного ключа. Составные ключи не поддерживаются</p></li>
<li><p><span class="b">[AutoIncrement]</span>: применяется к свойству типа <code>int</code> и 
указывает, что столбец в таблице, который соответствует этому свойству, будет инкрементировать значение на единицу при добавлении нового элемента</p></li>
<li><p><span class="b">[Column(name)]</span>: задает сопоставление свойства со столбцом в таблице, который имеет имя name</p></li>
<li><p><span class="b">[Table(name)]</span>: устанавливает название таблицы, которая будет соотвествовать данному классу</p></li>
<li><p><span class="b">[MaxLength(value)]</span>: устанавливает максимальную длину для строковых свойств</p></li>
<li><p><span class="b">[Ignore]</span>: указывает, что свойство будет игнорироваться. Это может быть полезно, если значение данного свойства не надо 
хранить в базе данных, и данное свойство не должно сопоставляться со столбцами из таблицы в бд</p></li>
<li><p><span class="b">[Unique]</span>: гарантирует, что столбец, который соответствует свойству с этим атрибутом, будет иметь уникальные неповторяющиеся значения</p></li>
</ul>
<p>При запросах к базе данных будет происходить автоматическое сопоставление типов данных из SQLite с типами данных из C#. Сопоставление типов 
можно описать следующей таблицей:</p>
<table class="tab">
<tr class="tabhead"><td><p>C#</p></td><td><p>SQLite</p></td></tr>
<tr><td><p>int, long</p></td><td><p>integer, bigint</p></td></tr>
<tr><td><p>bool</p></td><td><p>integer (1 = true)</p></td></tr>
<tr><td><p>enum</p></td><td><p>integer</p></td></tr>
<tr><td><p>float</p></td><td><p>real</p></td></tr>
<tr><td><p>double</p></td><td><p>real</p></td></tr>
<tr><td><p>decimal</p></td><td><p>real</p></td></tr>
<tr><td><p>string</p></td><td><p>varchar, text</p></td></tr>
<tr><td><p>DateTime</p></td><td><p>numeric, text</p></td></tr>
<tr><td><p>byte[]</p></td><td><p>blob</p></td></tr>
</table>
<h3>Класс репозитория</h3>
<p>Также создадим класс репозитория, через который будут идти все операции с данными:</p>
<pre class="brush:c#;">
...
<p>В конструкторе класса происходит создание подключения и базы данных (если она отсутствует). В качестве параметра извне будет передаваться путь к базе данных. 
При желании путь к бд можно определить и в самом конструкторе.</p>
<p>Для всех операций с данными используются методы, определенные в классе SQLiteConnection:</p>
<ul>
<li><p><span class="b">Insert</span>: добавляет объект в таблицу</p></li>
<li><p><span class="b">Get&lt;T&gt;</span>: позволяет получить элемент типа T по id</p></li>
<li><p><span class="b">Table&lt;T&gt;</span>: возвращает все объекты из таблицы</p></li>
<li><p><span class="b">Delete&lt;T&gt;</span>: удаляет объект по id</p></li>
<li><p><span class="b">Update&lt;T&gt;</span>: обновляет объект</p></li>
<li><p><span class="b">Query&lt;T&gt;</span>: выполняет SQL-выражение и возвращает строки из таблицы в виде объектов типа T (относится к выражениям SELECT)</p></li>
<li><p><span class="b">Execute</span>: выполняет SQL-выражение, но ничего не возвращает (относится к операциям. где не надо возвращать результат -
 UPDATE, INSERT, DELETE)</p></li>
</ul>
<p>Если предполагается, что к базе данных может обращаться сразу несколько потоков, то для блокирования одновременных операций с бд в классе репозитории 
можно использовать блок <span class="b">lock</span> с заглушкой, например:</p>
<pre class="brush:c#;">
SQLiteConnection database;
...
<p>Создаваемое подключение будет общим для всего приложения, поэтому изменим файл <span class="b">App.xaml.cs</span> следующим образом:</p>
<pre class="brush:c#;">
using System;
using System.IO;
using Xamarin.Forms;
...
<p>Статический объект репозитория, создаваемый при создании главной страницы приложения, будет доступен из любого места приложения.</p>
<p>Поскольку в нашем приложении мы будем переходить по страницам - к странице добавления или просмотра, то в качестве главной страницы устанавливается 
объект <code>NavigationPage</code>.</p>
<h3>Добавление страниц приложения</h3>
<p>Теперь на главной странице MainPage.xaml следующий код:</p>
<pre class="brush:xml;">
...
<p>Элемент ListView будет выводить список объектов, а при нажатии на элемент списка, будет срабатывать обработчик <code>OnItemSelected</code>. 
И также для добавления нового объекта определена кнопка.</p>
<p>И изменим файл кода <span class="b">MainPage.xaml.cs</span>:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>Остальные оба обработчика - OnItemSelected и CreateFriend предусматривают переход на страницу FriendPage, которая будет отвечать за работу с одним объектом из бд. 
Через свойство <code>BindingContext</code> мы можем установить полученный объект в качестве контекста страницы, а на самой странице использовать выражения привязки 
для изменения значений свойств объекта.</p>
<p>Теперь добавим эту страницу <span class="b">FriendPage</span>. В ее коде xaml пропишем следующий интерфейс:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<p>А в файле связанного кода <span class="b">FriendPage.xaml.cs</span> добавим обработчики нажатия кнопок:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>Обработчики используют методы репозитория для сохранения и удаления объекта и после этого осуществляют переход назад на главную страницу.</p>
<p>В итоге главный проект будет выглядеть следующим образом:</p>
<img src="./pics/6.8.png" alt="Работа с базой данных SQLite в Xamarin Forms" />
<p>Теперь с главной страницы мы можем попасть на страницу добавления и создать там новые объекты:</p>
<img src="./pics/6.5.png" alt="Create in SQLite and Xamarin" />
<p>После добавления все объекты будут отображаться в списке на главной странице:</p>
<img src="./pics/6.6.png" alt="Read in SQLite and Xamarin" />
...
<div class="nav"><p><a href="./11.2.php">Назад</a><a href="./">Содержание</a><a href="./7.4.php">Вперед</a></p></div></div>
<div class="socBlock">
