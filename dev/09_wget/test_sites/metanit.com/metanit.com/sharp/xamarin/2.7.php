...
<html >
<head>
<title>Xamarin Forms | Первое приложение в Visual Studio for Mac</title>
<meta charset="utf-8" />
<meta name="description" content="Установка Visual Studio for Mac и iOS.Xamarin для Mac OS, создание, настройка и запуск первого проекта">
...
<h2>Первое приложение в Visual Studio for Mac</h2><div class="date">Последнее обновление: 25.05.2019</div>
...
<p>Если рабочей машиной является MacOS, то в качестве среды разработки мы можем использовать среду разработки 
<span class="b">Visual Studio for Mac</span>. 
Ее можно загрузить по следующему адресу: <a href="https://visualstudio.microsoft.com/downloads/" rel="nofollow">https://visualstudio.microsoft.com/downloads/</a> (если заходить из под MacOS)</p>
<p>Но даже если нашей основной средой разработки является Visual Studio на Windows, а VS for Mac даже не предполагается использовать, но при этом мы 
...
Потому что эти средства потребуются для подключения Visual Studio из Windows к MacOS.</p>
<p>Прежде всего перед установкой  Visual Studio for Mac, необходимо установить среду XCode, которую можно загрузить из AppStore.</p>
<p>Итак, после загрузки инсталлятора VS for Mac запустим его:</p>
<img src="./pics/mac1.png" alt="Установка Visual Studio for Mac" />
<p>Нажмем на кнопку Install. И далее запустится окно для выбора опций для установки:</p>
<img src="./pics/mac2.png" alt="Настройка Visual Studio for Mac" />
<p>Выберем все необходимые компоненты и нажмем на кнопку установки.</p>
<p>После завершения установки откроем Visual Studio и создадим первый проект. Для этого нажмем на кнопку <span class="b">New...</span, либо перейдем в меню <span class="b">File</span> -&gt; <span class="b">New Solution</span></p>
<img src="./pics/mac4.png" alt="Первый проект в Visual Studio for Mac" />
<p>Далее нам откроется окно выбора шаблона проекта:</p>
<img src="./pics/mac5.png" alt="Первый проект Xamarin Forms в Visual Studio for Mac" />
<p>В левой части окна нам надо выбрать пункт <span class="b">Multiplatform</span> -&gt; <span class="b">App</span>. В центральной части нам отобразится ряд шаблонов. которые мы могли бы 
также использовать в Visual Studio на Windows:</p>
<ul>
<li><p><span class="b">Blank Forms App</span>: пустой шаблон, создающий проект с минимальной функциональностью</p></li>
<li><p><span class="b">Tabbed Forms App</span>: проект приложения, которое использует вкладки для навигации между страницами</p></li>
<li><p><span class="b">Shell Forms App</span>: шаблон одностраничного приложения</p></li>
<li><p><span class="b">Master Detail Forms App</span>: проект для специально для тех случаев, когда должна быть функциональность для представления списка объектов и 
функциональность для вывода информации по каждому отдельному элементу списка</p></li>
</ul>
<p>Выберем в качестве шаблона <span class="b">Blank Forms App</span> и нажмем на кнопку Next.</p>
<p>Далее откроется окно, где надо будет указать название проиложения и идентификатора организации:</p>
<img src="./pics/mac6.png" alt="Настройка проекта в Visual Studio for Mac" />
<p>В моем случае приложение будет называться HelloApp, а идентификатор организации - com.metanit.</p>
<p>Поле <span class="b">Target Platforms</span> позволяет указать целевые платформы, для которых будет создаваться проект. 
По умолчанию отмечены платформы iOS и Android.</p>
<p>В поле <span class="b">Shared Code</span> можно выбрать стратегию создания общего кода для проектов. Здесь есть два варианта:</p>
<ul>
    <li><p><span class="b">Use .NET Standard</span></p></li>
    <li><p><span class="b">Use Shared Library</span></p></li>
</ul>
<p>По умолчанию отмечена первая опция, собственно ее и оставим.</p>
<p>Оставим все остальные опции по умолчанию и нажмем на кнопку Next. После этого нам откроется окно настройки проекта:</p>
<img src="./pics/mac7.png" alt="Настройка проекта Xamarin Forms в Visual Studio for Mac" />
<p>Здесь можно задать имя для проекта. По умолчанию оно совпадает с ранее указанным именем приложения. Хотя в принципе его можно изменить.</p>
<p>Оставим здесь все настройки по умолчанию и для создания проекта нажмем на кнопку Create. После этого будет создан проект Xamarin Forms.</p>
<p>В отличие от версии на Windows созданное решение будет иметь только два проекта - для iOS и Android.</p>
<img src="./pics/mac8.png" alt="Настройка проекта Xamarin Forms в Visual Studio for Mac" />
<p>В созданный проект по умолчанию, как и в версии для Windows, добавляются файлы приложения <span class="b">App.xaml</span> и <span class="b">App.xaml.cs</span>, с которых 
собственно и начинается выполнение программы. 
А также файл интерфейса в xaml единственной страницы - <span class="b">HelloAppPage.xaml</span> и файл связанного с ней кода C# - 
<span class="b">HelloAppPage.xaml.cs</span>.</p>
 <p>Запустим проект iOS на выполнение, выбрав нужный эмулятор и нажав вверху на черную стрелочку над структурой проекта. И отобразится 
     эмулятор iPhone, который отобразит содержимое страницы HelloAppPage.xaml:</p>
<img src="./pics/mac9.png" alt="Создание приложения Xamarin Forms на MacOS" />
