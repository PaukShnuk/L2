...
<html >
<head>
<title>Xamarin Forms | AbsoluteLayout</title>
<meta charset="utf-8" />
<meta name="description" content="Контейнер AbsoluteLayout в Xamarin Forms, установка абсолютных и пропорциональных размеров дл элементов">
...
<h2>AbsoluteLayout</h2><div class="date">Последнее обновление: 09.01.2021</div>
...
<p><span class="b">AbsoluteLayout</span> позволяет задавать вложенным элементам абсолютные координаты расположения на странице и подходит больше для тех 
случаев, когда нам нужны элементы с точными координатами.</p>
<p>Для создания абсолютного позиционирования нам надо определить прямоугольную область, которую будет занимать элемент. Как правило, для этого используется 
структура Rectangle, которая представляет прямоугольник:</p>
<pre class="brush:c#;">
...
<p>Если нам неизвестна точная ширина и длина элемента, то мы можем ограничиться позицией, с которой начинается элемент, в виде структуры <span class="b">Point</span>:</p>
<pre class="brush:c#;">
int x = 10;	// позиция координаты X на странице
int y = 10;	// позиция координаты Y на странице
Point point = new Point(x, y);
</pre>
<p>Непосредственно для добавления элементов в контейнер AbsoluteLayout мы можем использовать метод <code>AbsoluteLayout.Children.Add()</code>, который имеет ряд версий:</p>
<ul>
<li><p><code>Add(View view)</code>: просто добавляет элемент в контейнер</p></li>
<li><p><code>Add(View view, Point point)</code>: элемент будет располагаться на странице, начиная с точки point</p></li>
<li><p><code>Add(View view, Rectangle rect)</code>: элемент будет располагаться на странице в области, ограниченной прямоугольником rect</p></li>
</ul>
<p>Например, добавим пару элементов:</p>
<pre class="brush:c#;">
...
<p>В данном случае одна метка накладывается на элемент BoxView, который представляет прямоугольник, заполненный цветом. Таким образом, AbsoluteLayout позволяет нам осуществлять 
перекрытие одних элементов другими.</p>
<img src="./pics/2.60.png" alt="Абсолютное позиционирование в Xamarin Forms" />
<p>Для установки позиции в качестве альтернативы мы можем использовать статический метод <span class="b">AbsoluteLayout.SetLayoutBounds()</span>, 
который в качестве параметров принимает элемент и прямоугольную область, выделяемую для этого элемента:</p>
<pre class="brush:c#;">
public class MainPage : ContentPage
{
    public MainPage()
...
<p>Для задания автоматических ширины и длины в данном случае мы можем использовать значение <span class="b">AbsoluteLayout.AutoSize</span>. Поэтому для последней метки 
устанавливается только точка верхнего левого угла занимаемой области, а высота и ширина этой области расчитываются автоматически исходя из ширины и высоты элемента.</p>
<p>Для создания абсолютного позиционирования в XAML у элемента устанавливается свойство <span class="b">AbsoluteLayout.LayoutBounds</span>, 
которое фактически представляет прямоугольную область:</p>
<pre class="brush:xml;">
...
<p>В случае со второй меткой нам неизвестная точная ширина и длина, поэтому вместо конкретных размеров мы можем использвать автоматические 
размеры в виде значения <code>AutoSize</code>.</p>
<h3>Флаг AbsoluteLayoutFlags. Пропорциональные значения</h3>
<p>Часто бывает сложно подобрать точные размеры для элементов. И в этом случае мы можем установить пропорциональные размеры. Для этого используется 
перечисление <span class="b">AbsoluteLayoutFlags</span>, которое может принимать следующие значения:</p>
<ul>
<li><p><code>None</code>: все значения интерпретируются как абсолютные</p></li>
<li><p><code>All</code>: все значения интерпретируются как пропорциональные</p></li>
<li><p><code>WidthProportional</code>: пропорциональной считается только ширина прямоугольной области элемента, а все остальные значения рассматриваются как абсолютные</p></li>
<li><p><code>HeightProportional</code>: пропорциональной считается только высота прямоугольной обласи элемента, а все остальные значения рассматриваются как абсолютные</p></li>
<li><p><code>XProportional</code>: пропорциональной считается только координата X элемента, а все остальные значения рассматриваются как абсолютные</p></li>
<li><p><code>YProportional</code>: пропорциональной считается только координата Y элемента, а все остальные значения рассматриваются как абсолютные</p></li>
<li><p><code>PositionProportional</code>: пропорциональными считаются только координаты X и Y позиции элемента</p></li>
<li><p><code>SizeProportional</code>: пропорциональными считаются только ширина и высота прямоугольной области элемента</p></li>
</ul>
<p>При использовании пропорциональных значений все эти значения рассчитываются в диапазоне от 0.0 до 1.0. Например, если мы хотим, чтобы элемент занимал половину ширины 
контейнера и четверть высоты, то для ширины нужно установить значение 0.5, а для высоты - 0.25.</p>
<p>Таким образом, ширина элемента будет вычисляться по следющей формуле:</p>
<pre class="brush:c#;">ширина_элемента = пропорциональная_ширина_элемента * ширина_контейнера</pre>
<p>Аналогично с координатами. Например, координата X будет вычисляться по следующей формуле:</p>
<pre class="brush:c#;">координатаХ_элемента = (ширина_контейнера - ширина_элемента) * пропорциональное_значениеХ_элемента</pre>
<p>Например, пусть контейнер AbsoluteLayout имеет ширину в 400 единиц, и пусть вложенный элемент имеет ширину в 100 единиц и пропорциональное значение 
координаты X равное 0.2. Тогда реальная позиция координаты Х будет равна (400-100) * 0.2 = 60 пикселей от левого края контейнера AbsoluteLayout.</p>
<p>Для установки нужного флага для определенного элемента используется статический метод <span class="b">AbsoluteLayout.SetLayoutFlags()</span>.</p>
<pre class="brush:c#;">
public class MainPage : ContentPage
{
    public MainPage()
...
<img src="./pics/2.61.png" alt="Пропорциональные размеры в Xamarin Forms" />
<p>Аналогичный код в XAML будет выглядеть следующим образом:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<div class="nav"><p><a href="./3.2.php">Назад</a><a href="./">Содержание</a><a href="./3.5.php">Вперед</a></p></div></div>
<div class="socBlock">
