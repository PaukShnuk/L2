...
<html >
<head>
<title>Xamarin Forms | SQLite</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение к базе данных SQLite в Xamarin Forms с помощью библиотеки SQLite.NET PCL">
...
<h1>Работа с SQLite</h1><h2>Настройка проекта для работы с SQLite.NET</h2><div class="date">Последнее обновление: 05.01.2019</div>
...
<p>Кроме обычных файлов для хранения данных мы можем использовать локальные базы данных. Наиболее популярной системой баз данных для локальных приложений является 
SQLite, поэтому рассмотрим общие принципы работы с этой СУБД.</p>
<p>Итак, создадим новый проект по типу Blank. Назовем его <span class="b">SQLiteApp</span>.</p> 
<img src="./pics/6.15.png" alt="База данных SQLite и операции с данными в Xamarin Forms" />
<p>Для работы с SQLite можно использовать разные подходы и библиотеки. Рекомендуемой библиотекой является <span class="b">SQLite.NET</span>, 
...
с помощью менеджера NuGet эту библиотеку <span class="b">sqlite-net-pcl</span>:</p>
<img src="./pics/6.1.png" alt="SQLite in Xamarin" />
<p>Следует отметить, что в Nuget можно найти много похожих библиотек с подобным названием. У нужной библиотеки будут следующие данные:</p>
<ul>
<li><p>Автор: <span class="b">Frank A. Krueger</span></p></li>
<li><p>Id: <span class="b">sqlite-net-pcl</span></p></li>
</ul>
<p>При работе с SQLite в Xamarin есть один минус - полный путь к базе данных на каждой отдельной платформе будет отличаться. И чтобы решить эту задачу, нам надо использовать 
внедрение зависимостей. Определим интерфейс, который будет определять метод для получения пути к бд:</p>
<pre class="brush:c#;">
namespace SQLiteApp
{
...
<p>Единственный метод интерфейса в качестве параметра принимает название файла базы данных.</p>
<h3>Настройка проекта для iOS</h3>
<p>В проект для iOS также через NuGet добавим ту же самую библиотеку <span class="b">sqlite-net-pcl</span>:</p>
<img src="./pics/6.2.png" alt="SQLite в Xamarin iOS" />
<p>После добавления библиотек добавим в проект для iOS следующий класс:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
using System.IO;
...
<p>Подобные действия проведем в проекте для Androida. Сначала добавим ту же самую библиотеку через NuGet:</p>
<img src="./pics/6.3.png" alt="SQLite в Xamarin Android" />
<p>Далее добавим в проект класс, реализующий интерфейс ISQLite:</p>
<pre class="brush:c#;">
using System;
...
<p>Данный класс аналогично создает подключение к базе данных.</p>
<h3>Настройка проекта для UWP</h3>
<p>В проект для UWP также установим ту же самую библиотеку. И также добавим такой же класс в проект для UWP:</p>
<pre class="brush:c#;">
using System.IO;
...
<div class="nav"><p><a href="./11.2.php">Назад</a><a href="./">Содержание</a><a href="./7.2.php">Вперед</a></p></div></div>
<div class="socBlock">
