...
<html >
<head>
<title>Xamarin Forms | Триггеры данных</title>
<meta charset="utf-8" />
<meta name="description" content="Триггеры данных в Xamarin Forms, отслеживание изменения свойств объектов, класс DataTrigger">
...
<h2>Триггеры данных</h2><div class="date">Последнее обновление: 12.01.2021</div>
...
<p>Триггеры данных (data triggers) позволяют отслеживать изменение свойств объектов. В Xamarin Forms триггеры данных представлены классом <span class="b">DataTrigger</span>. Рассмотрим, 
как его использовать.</p>
<p>В качестве объекта, за свойствами которого будет следить триггер данных, возьмем следующий класс Phone:</p>
<pre class="brush:c#;">
...
<p>Важно отметить, что если мы хотим динамически отслеживать изменение свойств объекта, то класс, представляющий этот объект, должен реализовать интерфейс 
INotifyPropertyChanged.</p>
<p>В коде xaml у главной страницы определим пользовательский интерфейс с привязкой к объекту Phone:</p>
<pre class="brush:xml;">
...
<p>Здесь установлена привязка трех элементов Entry к разным свойствам объекта Phone, который определен в ресурсах. Причем для второго элемента Entry добавлен 
триггер данных:</p>
<pre class="brush:xml;">
&lt;Entry Grid.Row="3" Text="{Binding Path=Company}"&gt;
  &lt;Entry.Triggers&gt;
...
<p>У триггера данных в обязательном порядке должно быть установлено свойство <code>TargetType</code>, которое указывает на тип элемента, к которому применяется 
триггер.</p>
<p>Также у триггера через свойство <code>Binding</code> задается привязка к определенному свойству объекта Phone (в данном случае свойство Company). А 
через свойство <code>Value</code> определяется значение, при котором будет срабатывать триггер. То есть в данном случае триггер будет срабатывать, 
когда свойство Company объекта Phone получит значение "Microsoft".</p>
<p>Вся работа триггера заключается в установке свойств объекта Entry. Для этого у триггера в коллекцию Setters добавляются объекты Setter, 
...
свойство BackgroundColor у Entry получит значение "#80CBC4", свойство TextColor - значение "#004D40".</p>
<p>В данном случае триггер привязан к тому же свойству, к которому привязан и объект Entry. Однако это необязательно, они могут быть привязан к разным свойствам. И теперь, если мы 
запустим приложения и введем в поле, которое представляет компанию, строку "Microsoft", то сработает наш триггер данных:</p>
<img src="./pics/3.11.png" alt="DataTrigger in Xamarin Forms" />
<p>Создание аналогичного триггера данных в коде c#:</p>
<pre class="brush:c#;">
...
<p>Таким образом, триггеры данных позволяют внести некоторое дополнительное форматирование в зависимости от значения свойства объекта. 
Особенно полезной такая функция может оказаться при отображении списков объектов.</p> 
<p>Допустим, в коде страницы определено свойство Phones, которое содержит 
некоторый набор объектов:</p>
<pre class="brush:c#;">
...
<p>В коде XAML у страницы применим триггер данных, чтобы выделить цветом те объекты, которые принадлежат определенной компании:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<img src="./pics/3.12.png" alt="Триггер данных в Xamarin Forms" />
...
<div class="nav"><p><a href="./4.16.php">Назад</a><a href="./">Содержание</a><a href="./5.1.php">Вперед</a></p></div></div>
<div class="socBlock">
