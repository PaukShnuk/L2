...
<html >
<head>
<title>Xamarin Forms | Объект Binding</title>
<meta charset="utf-8" />
<meta name="description" content="Создание привязки данных с помощью объекта Binding в Xamarin Forms, TwoWay и OneWay, однонаправленная и двунаправленная привязка">
...
<h2>Объект Binding</h2><div class="date">Последнее обновление: 12.01.2021</div>
...
<p>Свойство <span class="b">BindingContext</span> не является единственным способом для установки источника привязки. Также мы можем инкапсулировать все 
опции привязки в объекте <span class="b">Binding</span>. Например:</p>
<pre class="brush:c#;">
...
<p>Здесь у объекта Binding настраиваются ряд свойств. В частности, свойство <code>Source</code> указывает на источник привязки, а свойство 
<code>Path</code> - на свойство источника. В данном случае источником служит объект Entry, а свойством - его свойство Text.</p>
<p>Далее в метод <code>SetBinding()</code> в качестве второго параметра передается объект Binding, инкапсулирующий все опции привязки.</p>
<p>В итоге мы получаем то же самое, что и в прошлой теме: свойство Text объекта Label привязано к свойству Text объекта Entry.</p>
<p>Тоже самое можно прописать и в XAML:</p>
<pre class="brush:xml;">
...
<p>Таким образом, нам необязательно использовать свойство BindingContext для установки привязки к источнику. Более того использование объекта Binding 
...
вышеописанный способ.</p>
<p>Также мы можем одновременно устанавливать и BindingContext, и использовать объект Binding для определения опций привязки, и в этом случае 
объект Binding будет переопределять BindingContext, если, к примеру, в BindingContext указан один источник привязки, а в объекте Binding другой.</p>
<h3>Режим привязки</h3>
<p>Режим привязки определяет в какую сторону буде работать привязка. Все возможные режимы задаются с помощью значений из перечисления <span class="b">BindingMode</span>:</p>
<ul>
<li><p><code>Default</code>: значение по умолчанию. Для разных элементов и свойств может отличаться</p></li>
<li><p><code>OneWay</code>: при изменении в источнике изменяется цель</p></li>
<li><p><code>OneTime</code>: привязка установливается только однократно и больше не действует. То есть 
цель привязки получает начальное значение из источника привязки. Но при дальнейшем изменении в источнике привязка больше не действует</p></li>
<li><p><code>OneWayToSource</code>: при изменении в цели привязки изменяется источник (то есть обратная привязка)</p></li>
<li><p><code>TwoWay</code>: изменения в источнике воздействуют на цель привязки, а изменении в цели воздействуют на источник (то есть двусторонняя привязка)</p></li>
</ul>
<img src="./pics/3.25.png" alt="Режимы привязки в Xamarin Forms" />
<p>По умоланию стандртным значением для всех элементов и их свойств является значение <code>OneWay</code>, то есть односторонняя привязка от источника к цели. 
Однако ряд элементов и их свойств по умолчанию используют двустороннюю привязку:</p>
<table class="tab">
<tr class="tabhead"><td><p>Элемент</p></td><td><p>Свойство</p></td></tr>
<tr><td><p>Stepper</p></td><td><p>Value</p></td></tr>
<tr><td><p>Switch</p></td><td><p>IsToggled</p></td></tr>
<tr><td><p>Entry</p></td><td><p>Text</p></td></tr>
<tr><td><p>Editor</p></td><td><p>Text</p></td></tr>
<tr><td><p>SearchBar</p></td><td><p>Text</p></td></tr>
<tr><td><p>DatePicker</p></td><td><p>Date</p></td></tr>
<tr><td><p>TimePicker</p></td><td><p>Time</p></td></tr>
</table>
<p>Например, пусть у нас два связанных текстовых поля:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<p>Так как по умолчанию стандартным значением привязки для свойства Text объекта Entry является <code>TwoWay</code>, то при вводе в первое или второе 
текстовое поле, все изменения будут автоматически отображаться в другом поле.</p>
<p>Но если нам такое поведение не нужно, и мы хотим использовать именно односороннюю привязку от источника к цели, то мы можем изменить выражение привязки 
следующим образом:</p>
<pre class="brush:xml;">
&lt;Entry x:Name="entry1" BindingContext="{x:Reference Name=entry2}"
           Text="{Binding Path=Text, Mode=OneWay}" /&gt;
</pre>
<p>Для задания режима привязки в коде C# надо установить свойство Mode у объекта Binding:</p>
<pre class="brush:c#;">
...
<h3>StringFormat</h3>
<p>Свойство <span class="b">StringFormat</span> класса Binding позволяет дополнительно отформатировать значение:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<img src="./pics/3.4.png" alt="Форматирование привязки в Xamarin Forms" />
<p>Свойству <code>StringFormat</code> передается некоторая строка в ординарных кавычках, которая указывает, как будет форматироваться значение. 
Плейсхолдер {0} указывает на начальное значение, которое передается от привязки.</p>
