...
<html >
<head>
<title>Xamarin Forms | Настройки приложения</title>
<meta charset="utf-8" />
<meta name="description" content="Хранение данных в настройках приложения на Xamarin Forms, плагин Settings">
...
<h2>Настройки приложения</h2><div class="date">Последнее обновление: 13.01.2021</div>
...
<p>Каждая система предполагает механизм настроек пользователя, которые можно сохранять для последующего использования. Например, на Android это 
...
<span class="b">Xamarin Essentials</span>, который добавляется в проект Xamarin Forms по умолчанию.</p>
<p>Применять плагин довольно просто. Все настройки доступны через свойство <span class="b">Preferences</span>, который предоставляет для 
работы с настройками следующие методы:</p>
<ul>
<li><p><span class="b">void Set(key, value)</span>: сохранение значения value по ключу key</p>
<li><p><span class="b">string Get(key, defaultValue)</span>: получение значения по ключу key. Если же настроек с таким ключом нет, то возвращается значение 
по умолчанию - defaultValue</p>
<li><p><span class="b">void Remove(key)</span>: удаляет настройку с ключом key</p>
<li><p><span class="b">void Clear()</span>: удаляет все настройки</p>
<li><p><span class="b">bool ContainsKey(key)</span>: возвращает true, если имеется настройка с ключом key</p>
</ul>
<p>Например, для сохранения применяется 
метод <code>Set()</code>:</p>
<pre class="brush:c#;">
// сохраняем значение Tom по ключу name
Preferences.Set("name", "Tom");
</pre>
<p>Получение сохраненного значения по ключу:</p>
<pre class="brush:c#;">
...
<p>Для примера определим на странице текстовое поле для ввода, кнопку для сохранения введенного значения в настройках и метку для вывода из настроек:</p>
<pre class="brush:xml;">
...
<p>В файле кода страницы пропишем получение и сохранение данных в настройках:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
using System;
...
<p>При первой загрузке приложения, когда настройки еще не установлены, метка будет выводить значение по умолчанию. 
После установки значения метка будет брать это значение из настроек:</p>
<img src="./pics/7.3.png" alt="Preferences в Xamarin Forms" />
...
<div class="nav"><p><a href="./6.2.php">Назад</a><a href="./">Содержание</a><a href="./6.1.php">Вперед</a></p></div></div>
<div class="socBlock">
