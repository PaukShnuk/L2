...
<html >
<head>
<title>Xamarin Forms | Создание веб-сервиса для мобильного приложения</title>
<meta charset="utf-8" />
<meta name="description" content="Создание веб-сервиса для взаимодействия с приложением на Xamarin Forms">
...
<h2>Создание веб-сервиса</h2><div class="date">Последнее обновление: 14.01.2021</div>
...
<p>В прошлой теме мы рассмотрели, как отправлять запрос к внешнему серверу. Теперь рассмотрим, как мы можем отправлять запросы самых различных типов - 
...
по типу <span class="b">ASP.NET Core Web Application</span></p>
<img src="./pics/10.8.png" alt="Веб-сервис для Xamarin Forms" />
<p>Назовем <span class="b">SocialService</span>:</p>
<img src="./pics/10.14.png" alt="Xamarin и ASP NET Core" />
<p>Далее в открывшемся окне в качеcтве шаблона проекта выберем <span class="b">ASP.NET Core Empty</span>.</p>
<img src="./pics/10.9.png" alt="ASP.NET Core Web api для Xamarin Forms" />
<p>После создания проекта добавим в него через Nuget пакет <span class="b">Microsoft.EntityFrameworkCore.SqlServer</span></p>
<img src="./pics/10.10.png" alt="Добавление Entity Framework в ASP.NET Web api" />
<p>Затем добавим в проект новую папку <span class="b">Models</span>. А в папку <span class="b">Models</span> - новый класс, 
который будет представлять модель используемых данных - класс <span class="b">Friend</span>:</p>
<pre class="brush:c#;">
...
<p>Затем добавим в папку Models класс <span class="b">ApplicationContext</span> - класс контекста данных, через который будем подключаться к базе данных:</p>
<pre class="brush:c#;">
...
<p>Далее добавим в проект новую папку, которую назовем <span class="b">Controllers</span>. Затем в эту папку добавим новый 
элемент по типу <span class="b">API Controller - Empty</span>, который назовем <span class="b">FriendsController</span>:</p>
<img src="./pics/10.11.png" alt="Добавление контроллера в проект ASP.NET Web API" />
<p>Изменим код этого контроллера на следующий:</p>
<pre class="brush:c#;">
...
<img src="./pics/10.19.png" alt="Проект ASP.NET Web API для Xamarin" />
<p>И изменим класс <span class="b">Startup</span>, который имеется в проекте по умолчанию, следующим образом:</p>
<pre class="brush:c#;">
...
<p>Это типовой веб-сервис, реализующий стандартный CRUD-интерфейс, достаточный для демонстрации сетевых возможностей приложения на Xamarin. Но естественно конкретные сервисы, с которыми придется взаимодействовать, могут отличаться по функционалу.</p>
<p>После того, как сервис готов, его можно запустить с помощю одного из способов, описанных на странице <a href="./../mvc5/24.3.php">http://metanit.com/sharp/mvc5/24.3.php</a>.</p>
...
<div class="nav"><p><a href="./10.1.php">Назад</a><a href="./">Содержание</a><a href="./10.5.php">Вперед</a></p></div></div>
<div class="socBlock">
