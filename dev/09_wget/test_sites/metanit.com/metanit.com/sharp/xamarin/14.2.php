...
<html >
<head>
<title>Xamarin Forms | Добавление свойств</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление свойств в новый элемент в Xamarin Forms и применение их при рендеринге">
...
<h2>Добавление свойств</h2><div class="date">Последнее обновление: 13.01.2021</div>
...
<p>В прошлой теме был создан новый элемент HeaderView. Но на данный момент он выглядит слишком просто:</p>
<pre class="brush:c#;">
public class HeaderView : View
{}
</pre>
<p>В нем пока нет никакой функциональности. Теперь изменим его, добавив пару свойств:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>Класс определяет два свойства: Text и TextColor. Фактически данные свойства выступают в качестве обертки над <a href="./4.4.php">BindableProperty</a>. Тем самым мы сможем 
использовать свойства в механизме привязки.</p>
<p>На странице определим пару элементов HeaderView:</p>
<pre class="brush:xml;">
...
<p>Затем нам надо изменить код рендереров для каждой платформы, чтобы через свойства в HeaderView можно было установить параметры визуализации.</p>
<p>Изменим код рендерера для Android следующим образом:</p>
<pre class="brush:c#;">
...
<p>Во время вызова метода <code>OnElementChanged()</code> ожидается, что объект из Xamarin.Forms (в нашем случае HeaderView) уже создан, а его свойства установлены. 
Хотя как правило так и происходит, но это не обязательно должно происходить. С помощью передаваемого в качестве параметра объект <code>ElementChangedEventArgs</code> мы можем 
получить тот элемент Xamarin.Forms, для которого создается нативный объект TextView. Если свойство <code>NewElement</code> не равно значению <code>null</code>, 
то значит объект HeaderView создан, а его свойства установлены. В этом случае свойство 
<code>Element</code> из ViewRenderer ссылается на тот же объект, что и NewElement. И мы можем передать значения его свойств в создаваемый нативный объект.</p>
<p>Для установки значений здесь используются два дополнительных метода <code>SetText()</code> и <code>SetTextColor()</code>.</p>
<p>При использовании свойств важно, чтобы при изменении их значений у объекта Xamarin Forms автоматически менялись также и значения свойств нативного объекта.</p>
<p>Если свойство элемента из Xamarin.Forms представляет BindableProperty, то любое его изменение вызовет событие <code>PropertyChanged</code>. Поэтому 
рендерер также будет уведомлен об изменении, и в этом случае у ViewRenderer будет вызван метод <span class="b">OnElementPropertyChanged()</span>.</p>
<p>В методе <code>OnElementPropertyChanged()</code> с помощью передаваемого параметра PropertyChangedEventArgs мы можем получить изменяемое свойство:</p>
<pre class="brush:c#;">
if (e.PropertyName == HeaderView.TextColorProperty.PropertyName)
{
    //.............
...
<p>Если мы запустим приложение, то к обоим элементам HeaderView будут применены те свойства, которые мы для них указали:</p>
<img src="./pics/13.2.png" alt="Properties in custom renderers in Xamarin Forms" />
<p>Далее изменим реализацию рендерера в проекте для iOS:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>Здесь аналогичный код, отличаются только конкретные классы, используемые при рендеринге.</p>
<p>И также изменим код рендерера для UWP:</p>
<pre class="brush:c#;">
...
<p>Таким образом, мы можем определить в кастомном элементе свойства и затем использовать их для рендеринга этого элемента.</p>
