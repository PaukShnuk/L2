...
<html >
<head>
<title>Xamarin Forms | Контекстное меню</title>
<meta charset="utf-8" />
<meta name="description" content="Контекстное меню для списка ListView с использованием MVVM  в Xamarin Forms">
...
<h2>Контекстное меню</h2><div class="date">Последнее обновление: 14.01.2021</div>
...
<p>Ключевым элементом для работы с наборами данных является ListView. И нередко бывает важно продумать, какие инструменты использовать для управления данными в 
...
<span class="b">ContextActions</span>.</p>
<p>Итак, для работы с контекстным меню создадим новый проект. В качестве объектов с которыми будем работать, возьмем класс Phone</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Далее добавим в проект модель <span class="b">PhoneViewModel</span>, через которую мы будем взаимодействовать с объектами Phone:</p>
<pre class="brush:c#;">
...
<p>Кроме свойств-надстроек над свойствами объекта Phone здесь есть свойство, которое представляет модель PhonesListViewModel - объект списка, в котором будет находиться 
текущий объект PhoneViewModel. И далее добавим в проект новый класс <span class="b">PhonesListViewModel</span>:</p>
<pre class="brush:c#;">
...
<p>Для хранения данных определена коллекция Phones, которая инициализируется в конструкторе.</p>
<p>Кроме этой коллекции в классе определены три команды. Команда MoveToTopCommand поднимает элемент на одну позицию вверх в списку (в реальности это выглядит как 
перемещение ближе к началу коллекции Phones). Команда MoveToBottomCommand, наоборот, опускает элемент на одну позицию вниз. А команда RemoveCommand удаляет элемент из списка.</p>
<p>В коде C# у главной страницы MainPage определим контекст данных:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>В коде XAML определим список для вывода объектов, к каждому из которых прикрепим контекстное меню:</p>
<pre class="brush:xml;">
...
<p>Для определения меню применяется свойство <span class="b">ContextActions</span> объекта ViewCell, который представляет собой отдельный элемент. 
Каждый пункт в этом меню представляет элемент <span class="b">MenuItem</span>. У MenuItem можно определить команду и параметр, который будет пределаться команде при 
ее выполнении. В данном примере будут вызываться команды из PhonesListViewModel, которым будет передаваться текущий объект, для которого вызвано контекстное меню.</p>
<p>И если мы запустим приложение и одним долгим касанием нажмем на какой-нибудь элемент	списка, то отобразится контекстное меню:</p>
<img src="./pics/11.5.png" alt="Контекстное меню в MVVM в Xamarin Forms" />
<p>При этом надо отметить, что на разных ОС (даже на разных вериях одной ОС Android) визуально контекстное меню может выглядеть иначе.</p>
<p>Хотя здесь элементы меню используют команды и их параметры, однако нам в принцпе не обязательно использовать контекстное меню именно в контексте MVVM. Так, 
у каждого элемента меню есть стандартное событие <code>Clicked</code>, для которого мы можем определить в коде обработчик. Например:</p>
<pre class="brush:xml;">
&lt;MenuItem Text="Show" Clicked="Show" /&gt;
</pre>
<p>А в коде прописать какой-нибудь метод обработки:</p>
<pre class="brush:c#;">
...
<p>В тоже время в MVVM использование контекстного меню имеет свои плюсы, в частности, возможность передачи параметра.</p>
...
<div class="nav"><p><a href="./11.1.php">Назад</a><a href="./">Содержание</a><a href="./7.2.php">Вперед</a></p></div></div>
<div class="socBlock">
