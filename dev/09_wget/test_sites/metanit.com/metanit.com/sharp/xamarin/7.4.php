...
<html >
<head>
<title>Xamarin Forms | Асинхронное подключение к SQLite</title>
<meta charset="utf-8" />
<meta name="description" content="Асинхронное подключение к базе данных SQLite в Xamarin Forms, класс SQLiteAsyncConnection и его методы">
...
<h2>Асинхронное подключение к SQLite</h2><div class="date">Последнее обновление: 14.01.2021</div>
...
<p>В прошлой теме для подключения к базе данных SQLite мы использовали синхронные методы, которые на время вызова и выполнения операций с данными 
блокировали основной поток графического приложения. Однако библиотека SQLite.NET предоставляет также асинхронный API. Он предоставляет методы, которые во многом похожи на синхронные.</p>
<p>Асинхронное подключение в SQLite.NET представляет объект класса <span class="b">SQLiteAsyncConnection</span>. А весь асинхронный API раскрывается через методы этого класса:</p>
<ul>
<li><p><span class="b">CreateTableAsync()</span>: создание таблицы</p></li>
<li><p><span class="b">DropTableAsync()</span>: удаление таблицы</p></li>
<li><p><span class="b">Table&lt;T&gt;()</span>: получение данных из таблицы</p></li>
<li><p><span class="b">GetAsync&lt;T&gt;()</span>: получение объекта</p></li>
<li><p><span class="b">DeleteAsync()</span>: удаление объекта</p></li>
<li><p><span class="b">UpdateAsync()</span>: обновление существующего объекта</p></li>
<li><p><span class="b">InsertAsync()</span>: добавление нового элемента</p></li>
<li><p><span class="b">ExecuteAsync()</span>: выполнение sql-выражения</p></li>
<li><p><span class="b">ExecuteScalarAsync()</span>: выполнение sql-выражения и получение результата</p></li>
<li><p><span class="b">FindAsync()</span>: поиск объекта в БД</p></li>
<li><p><span class="b">QueryAsync()</span>: выполнение запроса SQL</p></li>
<li><p><span class="b">InsertAllAsync()</span>: добавление нескольких объектов</p></li>
<li><p><span class="b">UpdateAllAsync()</span>: обновление нескольких объектов</p></li>
</ul>
<p>Для применения асинхронного API возьмем проект из прошлой темы и добавим в него новый класс <span class="b">FriendsAsyncRepository</span>:</p>
<pre class="brush:c#;">
...
<p>В отличие от синхронной версии здесь создание таблицы вынесено в отдельный метод.</p>
<p>И далее похожим образом мы сможем использовать этот репозиторий. Установим его в классе App:</p>
<pre class="brush:c#;">
using System;
using System.IO;
using Xamarin.Forms;
...
<p>И затем используем на главной странице приложения:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>Xaml-код страницы MainPage выглядит следующим образом:</p>
<pre class="brush:xml;">
...
<p>При переходе на любую страницу у нее вызывается метод <code>OnAppearing()</code>, поэтому тут мы можем установить привязку и настроить другие начальные данные.</p>
<p>Интерфейс страницы <span class="b">FriendPage</span> для отображения одного объекта:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<p>А в файле связанного кода <span class="b">FriendPage.xaml.cs</span> определим обработчики нажатия кнопок:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>Часто данные извлекаются и обрабатываются в фоновом потоке. В этом случае, чтобы после выполнения операции <span class="b">await</span> обработка продолжилась в фоновом потоке, 
применяется метод <span class="b">ConfigureAwait(false)</span>:</p>
<pre class="brush:c#;">
