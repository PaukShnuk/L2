...
<html >
<head>
<title>Xamarin Forms | Наследование элемента и рендерера</title>
<meta charset="utf-8" />
<meta name="description" content="Наследование элемента и рендерера в Xamarin Forms, создание кастомного рендеринга">
...
<h2>Наследование элемента и рендерера</h2><div class="date">Последнее обновление: 13.01.2021</div>
...
<p>Нам необязательно создавать новый элемент с нуля. Мы можем унаследовать свой класс элемента от уже готового класса. К примеру, переопределим элемент Button, 
добавив в главный проект следующий класс:</p>
<pre class="brush:c#;">
public class CustomButton : Button
{
}
</pre>
<p>Пусть этот элемент выводится на страницу:</p>
<pre class="brush:xml;">
...
<p>И далее добавим класс рендерера для этого элемента в проект для UWP:</p>
<pre class="brush:c#;">
using HelloApp;
using HelloApp.UWP;
...
<p>Так как для элемента Button уже есть свой рендерер - ButtonRenderer, то мы можем наследовать от этого класса. И фактически через механизм наследования мы также наследуем 
всю логику по реализации нативного элемента управления. Поэтому после вызова <code>base.OnElementChanged(e)</code> у нас уже будет сформирован нативный элемент, 
который при желании мы можем подкорректировать, например, как здесь, установив цвет фона и границы. Либо же можно полностью переопределить элемент, создав свой нативный объект Button.</p>
<p>Но в любом случае нам также надо использовать атрибут <code>ExportRenderer</code>.</p>
<p>Но в данном случае необязательно наследовать класс рендерера от ButtonRenderer. Это может быть и ViewRenderer, особенно если мы хотим сопоставить с элементом 
CustomButton не нативный класс Button, а какой-то другой. Например:</p>
<pre class="brush:c#;">
using HelloApp;
using HelloApp.UWP;
using Xamarin.Forms.Platform.UWP;
...
<p>Здесь в качестве нативного элемента выступает объект Border - специальные элемент в UWP, представляющий границу вокруг некоторого содержимого. Содержимым 
же в данном случае выступает объект ContentPresenter. В частности через его свойство <code>Content</code> устанавливается текст кнопки.</p>
<p>Также стоит отметить, что для поддержки событий нажатия на наш элемент CustomButton здесь при нажатии на элемент Border срабатывает делегат:</p>
<pre class="brush:c#;">
buttonBorder.Tapped += (o, args) =&gt; { ((Xamarin.Forms.IButtonController)Element).SendClicked(); };
</pre>
<p>Благодаря этому при нажатии сработает событие Clicked у элемента CustomButton.</p>
<p>В итоге мы получим следующую кнопку:</p>
<img src="./pics/13.3.png" alt="Click event in custom renderer in Xamarin Forms" />
<p>Аналогичным образом можно создать рендереры и для других платформ.</p>
