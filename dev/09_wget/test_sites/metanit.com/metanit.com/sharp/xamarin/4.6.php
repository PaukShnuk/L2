...
<html >
<head>
<title>Xamarin Forms | Конвертеры значений</title>
<meta charset="utf-8" />
<meta name="description" content="Конвертеры значений в Xamarin Forms, их роль в привязке данных, интерфейс IValueConverter">
...
<h2>Конвертеры значений</h2><div class="date">Последнее обновление: 13.01.2021</div>
...
<p>Привязка позоляет связать два свойства, которые совпадают по типу данных. Например, свойство Text у класса Entry и свойство Text класса Label 
...
особенно когда необходимо применить к значению привязки дополнительное форматирование.</p>
<p>Например, пусть у нас элемент Label привязан к элементу DatePicker и выводит выбранную дату. По умолчанию дата выводится в формате 
"MM/dd/yyyy hh:mm:ss". Это не очень удобный формат. Возможно, мы захотим использовать совсем другие форматы.</p>
<p>Для преобразования одного значения к другому в процессе привязки используются пециальные классы - конвертеры значений. 
К примеру, добавим в наш проект новый класс конвертера значений:</p>
<pre class="brush:c#;">
using System;
...
<p>Прежде всего класс конвертера значений должен реализовать интерфейс <span class="b">IValueConverter</span>. Этот интерфейс определяет два 
метода: <code>Convert()</code>, который преобразует пришедшее от привязки значение в тот тип, который понимается приемником привязки, и 
<code>ConvertBack()</code>, который выполняет противоположную операцию.</p>
...
<p>В примере выше метод Convert возвращает дату в виде строки в формате "dd.MM.yyyy". То есть мы ожидаем, что в качесве параметра value 
будет передаваться объект DateTime.</p>
<p>Метод ConvertBack в данном случае не имеет значения, и поэтому он просто возвращает строковое представление текущей даты.</p>
...
<p>Так как класс конвертера располагается в пространстве имен текущего проекта, то, чтобы класс был доступен в xaml, надо подключить текущее пространство имен.</p>
<pre class="brush:xml;">xmlns:local="clr-namespace:HelloApp;assembly=HelloApp"</pre>
<p>В моем случае пространством имен является HelloApp, и оно проецируется на суффикс <code>local</code></p>
<p>Затем в ресурсах создается объект привязки с ключом <code>dateConverter</code>.</p>
<p>И далее в самом выражении привязки мы подключем данный ресурс в качестве конвертера:</p>
<pre class="brush:xml;">Converter={StaticResource dateConverter}</pre>
<p>При этом здесь привязка идет к элементу выбора даты для двух объектов Label. Но только второй объект применяет конвертер:</p>
<img src="./pics/3.5.png" alt="Value Converter in Xamarin Forms" />
<p>Возьмем другой случай. Допустим, нам надо привязать цвет метки к вводимому в текстовое поле значение. Для этого определим следующий конвертер:</p>
<pre class="brush:c#;">
using System;
using System.Globalization;
using Xamarin.Forms;
...
<p>Если вводится определенная строка, то конвертер возвращает определенный цвет. Теперь применим этот конвертер в XAML:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<img src="./pics/3.6.png" alt="Color Converter in Xamarin" />
