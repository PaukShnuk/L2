...
<html >
<head>
<title>Xamarin Forms | Entity Framework Core</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с SQLite через Entity Framework Core в Xamarin Forms, определение модели и контекста данных">
...
<h1>Entity Framework Core</h1><h2>Создание контекста данных</h2><div class="date">Последнее обновление: 11.01.2021</div>
...
<p>Для взаимодействия с SQLite в Xamarin Forms мы можем использовать Entity Framework Core, что значительно облегчает работу с базой данных. 
...
он будет называться <span class="b">EFCoreApp</span></p>
<img src="./pics/efcore4.png" alt="Entity Framework Core и Sqlite в C# и .NET в Xamarin.Forms" />
<p>И первым делом добавим в главный проект решения через Nuget пакет <span class="b">Microsoft.EntityFrameworkCore.Sqlite</span>:</p>
<img src="./pics/efcore1.png" alt="Работа с Entity Framework Core в C# и .NET в Xamarin.Forms" />
<p>При добавлении пакета следует учитывать версию .NET Standard, на которую он ориентирован. Так, последняя на данный момент версия пакета - 
...
В этом случае нам надо либо установить Microsoft.EntityFrameworkCore.Sqlite более ранних версий, например, 3.1, 3.0, 2.2.</p>
<p>Если же мы все-таки хотим использовать версию пакета Microsoft.EntityFrameworkCore.Sqlite 5.0 или выше, то можно изменить вверсию 
...
проекте для UWP, поскольку он также нацелен на .NET Standard 2.0.</p>
<p>После добавления пакета определим в главном проекте класс <span class="b">Friend</span> - модель, с которой мы будем работать:</p>
<pre class="brush:c#;">
public class Friend
{
...
<p>Также в главный проект добавим новый класс <span class="b">ApplicationContext</span>:</p>
<pre class="brush:c#;">
...
<p>Данный класс представляет контекст данных, через который мы будем взаимодействовать с базой данных SQLite. Через конструктор объект этого класса 
получает в переменную _databasePath путь к базе данных. Стоит отметить, что в зависимости от целевой ОС путь к базе данных может отличаться.</p>
<p>Поскольку путь для разных ОС отличается, нам необходимо найти способ получения этого пути. И для этого воспользуемся механизмом внедрения зависимостей. 
В частности, определим в главном проекте интерфейс <span class="b">IPath</span>:</p>
<pre class="brush:c#;">
...
<p>Через метод <code>GetDatabasePath</code> мы будем получать из разных ОС путь к базе данных.</p>
<p>Теперь добавим в проекты для конкретных ОС реализацию этого интерфейса. В частности, в проект для Android добавим класс <span class="b">AndroidDbPath</span>:</p>
<pre class="brush:c#;">
using System;
...
<p>В проект для iOS добавим класс <span class="b">IosDbPath</span>:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
using System.IO;
...
<p>И в проект для UWP добавим класс <span class="b">UwpDbPath</span>:</p>
<pre class="brush:c#;">
using EFCoreApp.UWP;
using Xamarin.Forms;
using System.IO;
...
<img src="./pics/efcore2.png" alt="Работа с Entity Framework Core и SQLite в Xamarin.Forms" />
<p>Теперь создадим простейший CRUD-интерфейс для работы с БД через Entity Framework Core.</p>
...
<div class="nav"><p><a href="./18.1.php">Назад</a><a href="./">Содержание</a><a href="./19.2.php">Вперед</a></p></div></div>
<div class="socBlock">
