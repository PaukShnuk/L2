...
<html >
<head>
<title>Xamarin Forms | Паттерн Model-View-ViewModel</title>
<meta charset="utf-8" />
<meta name="description" content="Использование паттерна Model-View-ViewModel в приложении на Xamarin Forms">
...
<h1>MVVM</h1><h2>Паттерн Model-View-ViewModel</h2><div class="date">Последнее обновление: 13.01.2021</div>
...
<p>Паттерн MVVM (Model - View - ViewModel) основывается на разделении функциональной части приложения на три ключевых компонента:</p>
<ul>
<li><p><span class="b">View</span> - представление или пользовательский интерфейс</p></li>
<li><p><span class="b">Model</span> - модель или данные, которые используются в приложении</p></li>
<li><p><span class="b">ViewModel</span> - промежуточный слой между представлением и данными, который обеспечивает их взаимодействие</p></li>
</ul>
<p>Преимуществом использования данного паттерна является меньшая связанность между компонентами и разделение ответственности между ними. То есть Model отвечает 
за данные, View отвечает за графический интерфейс, а ViewModel - за логику приложения.</p>
<p>Легкость реализации паттерна MVVM  в Xamarin Forms стала возможной благодаря ранее рассмотренному механизму привязки.</p>
<p>Рассмотрим простейший пример. Определим класс данных или модели:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Также добавим в проект класс, который назовем PhoneViewModel со следующим содержимым:</p>
<pre class="brush:c#;">
...
<p>Это и будет компонент ViewModel, который связывает данные и визуальный интерфейс. По большому счету она представляет обертку над классом Phone, 
...
например, по получению объекта из базы данных.</p>
<p>Важно, что данный класс реализует интерфейс <span class="b">INotifyPropertyChanged</span>, что позволяет уведомлять систему об изменении его свойств с помощью события 
<code>PropertyChanged</code>.</p>
<p>Теперь создадим визуальную часть. Определим на главной странице MainPage.xaml следующее содержимое:</p>
<pre class="brush:xml;">
...
<p>Здесь определена привязка к свойству ViewModel.</p>
<p>А в конструкторе страницы в файле кода c# пропишем в качестве контекста данных для страницы определенную ранее ViewModel:</p>
<pre class="brush:c#;">
...
<p>И при запуске приложение выведет нам все данные о viewmodel, переданной во view:</p>
<img src="./pics/3.2.png" alt="MVVM in Xamarin Forms" />
<p>При использовании паттерна в качестве представления обычно выступает страница, и, как правило, одно представление (одна страница) связана с одной моделью представления (ViewModel). 
При этом одну и ту же VıewModel могут использовать несколько представлений.</p>
<p>Стоит отметить, что в этом отношении ViewModel ничего не знает о представлении, что это, какие элементы управления оно содержит. ViewModel только определяет 
логику обработку без какой-либо связи с графическим интерфейсом.</p>
...
<div class="nav"><p><a href="./6.1.php">Назад</a><a href="./">Содержание</a><a href="./4.3.php">Вперед</a></p></div></div>
<div class="socBlock">
