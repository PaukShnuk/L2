...
<html >
<head>
<title>Xamarin Forms | Запуск на Android</title>
<meta charset="utf-8" />
<meta name="description" content="Тестирование приложения на Xamarin Forms на Android, настройка версии SDK">
...
<h2>Запуск на Android</h2><div class="date">Последнее обновление: 07.01.2021</div>
...
<p>Итак, в прошлых темах мы создали первый проект, теперь запустим его на ОС Android. Так как главный проект компилируется в библиотеку dll, 
то нам надо запустить собственно тот проект, который предназначен для этой операционной системы. То есть в данном случае <span class="b">HelloApp.Android</span>.</p>
<p>Если рабочая машина, на которой ведется разработка, поддерживает виртуализацию, то мы можем для отладки приложений использовать эмуляторы: эмулятор Android 
от Microsoft или Android Player.</p>
<p>Если у нас есть устройство с ОС Android, то мы можем его использовать для тестирования. Для этого надо подключить это устройство к компьютеру 
с помощью USB-кабеля. А на самом мобильном устройстве установить режим разработчика в параметрах.</p>
<p>Вполне возможно, что мобильное устройство подключено к компьютеру, должным образом настроено, но Visual Studio его все равно не видит. 
В этом случае можно перезугрузить сервер Adb. Для этого надо в Visual Studio перейти в меню к пункту <span class="b">Tools -&gt; Android -&gt; Restart Adb Server</span>:</p>
<img src="./pics/androidadb.png" alt="Подключение смартфона с Android в Xamarin Visual Studio" />
<p>Перед самим запуском можно настроить некоторые параметры, которые связаны с Android. Так, если мы перейдем в Visual Studio в меню 
<span class="b">Tools -&gt; Options</span> и в появившемся окне выберем пункт <span class="b">Xamarin -&gt; Android Settings</span>. Здесь мы можем настроить 
расположение Java SE SDK и Android SDK, а также настройки эмулятора и прочее.</p>
<img src="./pics/2.39.png" alt="Android SDK для Xamarin" />
<p>Далее мы можем перейти в свойства проекта для Android, и в настройках установить версию среды Android, которая будет использоваться для компиляции приложения:</p>
<img src="./pics/2.6.png" alt="Настройка Android для Xamarin" />
<p>Следует учитывать, что последняя версия Xamarin Forms не поддерживает версии ниже Android 10 в качестве целевой версии Android.</p>
<p>Важную роль играют настройки манифеста, где мы можем установить различные разрешения, иконку и название приложения, целевую и минимальную версию Android, 
которые будут поддерживаться приложением, и прочие настройки:</p>
<img src="./pics/1.27.png" alt="Настройка версии Android для Xamarin" />
<p>Версию среды стоит учитывать, если у нас определенное мобильное устройство, которое мы хотим использовать для тестирования. Если его версия Android 
ниже минимальной, то Visual Studio может просто не увидеть это устройство, даже если оно подключено через USB к компьютеру.</p>
<p>Если же все настроено правильно, то в панели инструментов Visual Studio отобразит подключенное устройство:</p>
<img src="./pics/2.9.png" alt="Тестирование Xamarin на смартфоне Android" />
<p>В моем случае это устройство Nexus 5X. Попутно в списке могут отображаться и другие подключенные устройства и эмуляторы.</p>

<p>Если необходимо установить какую-то новую версию Android API или какие-то компоненты из Android SDK, можно перейти к меню 
<span class="b">Tools -&gt; Android -&gt; Android SDK Manager</span> и там отметить нужные компоненты для установки.</p>
<p>И в конце мы можем запустить проект. В соответствии с имеющимся кодом он будет отображать метку с текстом в центре экрана:</p>
<img src="./pics/2.10.png" alt="Первое приложение на Xamarin под Android" />
<p>Стоит отметить, что в процессе развертывания приложения на устройстве Xamarin устанавливает ряд пакетов
<span class="b">Xamarin Android API-xx Support</span>, что можно увидеть через окно вывода Output в Visual Studio или на смартфоне в диспетчере приложений:</p>
<img src="./pics/2.11.png" alt="Mono Android Support" />
...
<div class="nav"><p><a href="./2.2.php">Назад</a><a href="./">Содержание</a><a href="./2.6.php">Вперед</a></p></div></div>
<div class="socBlock">
