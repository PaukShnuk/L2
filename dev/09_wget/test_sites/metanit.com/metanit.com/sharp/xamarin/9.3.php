...
<html >
<head>
<title>Xamarin Forms | Триггеры</title>
<meta charset="utf-8" />
<meta name="description" content="Триггеры в Xamarin Forms, триггеры свойств и событий">
...
<h2>Триггеры</h2><div class="date">Последнее обновление: 12.01.2021</div>
...
<p>Триггеры в Xamarin Forms позволяют декларативно задать некоторые действия, которые выполняются при изменении свойств визуального объекта.</p>
<h3>Триггеры свойств</h3>
<p>Простые триггеры свойств определяются как элементы стиля с помощью объекта Trigger. Они следят за значением свойств и в случае их изменения с помощью объекта Setter устанавливают значение других свойств.</p>
<p>Например, пусть по переходу к текстовому полю Entry текст в нем приобретает красный цвет:</p>
<pre class="brush:xml;">
...
<p>Стиль может содержать несколько триггеров, и все они определяются в элементе <code>Style.Triggers</code>.</p>
<p>У каждого триггера устанавливаются три свойства:</p>
<ul>
<li><p><span class="b">Property</span>: свойство, на изменение которого должен отслеживать триггер</p></li>
<li><p><span class="b">Value</span>: значение свойства, при котором должен срабатывать триггер</p></li>
<li><p><span class="b">TargetType</span>: тип объектов, к которым применяется триггер</p></li>
</ul>
<p>То есть триггер в данном случае будет срабатывать, когда свойство <code>IsFocused</code> элемента <code>Entry</code> приобритет значение <code>true</code>.</p>
<p>Работа триггера будет заключаться в установки ряда свойств элемента Entry. Здесь у Entry устанавливается красный текст:</p>
<pre class="brush:xml;">&lt;Setter Property="TextColor" Value="Red" /&gt;</pre>
<p>Таким образом, при получении фокуса сработает триггер, который окрасит текст в красный цвет.</p>
<img src="./pics/9.4.png" alt="Триггеры в Xamarin Forms" />
<p>Зато, когда мы установим фокус в другом месте приложения на какой-то другой элемент, то триггер уже действовать не будет, а текст в Entry приобретет 
свой стандартный цвет:</p>
<img src="./pics/9.5.png" alt="Триггеры свойств в Xamarin" />
<h4>Триггеры свойств в коде C#</h4>
<p>Определим триггеры свойств в коде C#:</p>
<pre class="brush:c#;">
...
<p>Вначале определяем триггер с помощью объекта Trigger. Через свойство <code>Property</code> указываем свойства элемента Entry, которое будет отслеживаться. 
А с помощью свойства <code>Value</code> устанавливаем значение, при получении которого сработает триггер.</p>
<pre class="brush:c#;">
var trigger = new Trigger(typeof(Entry))
{
    Property = Entry.IsFocusedProperty,
...
<p>То есть когда свойство <code>IsFocused</code> получит значение <code>true</code>, сработает триггер.</p>
<p>Далее через коллекцию <code>Setters</code> определяем свойства и их значения, которые будут устанавливаться во время действия триггера - установка зеленого 
цвета фона и белого цвета для введенных символов.</p>
<p>В конце триггер добавляется в коллекцию Triggers элемента Entry:</p>
<pre class="brush:c#;">entry.Triggers.Add(trigger);</pre>
<p>В итоге при получении фокуса тестовое поле будет окрашиваться в зеленый цвет, а введенные символы - в белый:</p>
<img src="./pics/9.8.png" alt="Триггеры свойств в коде C# в Xamarin Forms" />
<p>Но пойдем чуть дальше и добавим к триггеру дополнительные действия. Действие в триггере - это обычный класс, который наследуется от класса 
<span class="b">TriggerAction</span> и который реализует метод <code>Invoke()</code>. Так, определим следующий класс:</p>
<pre class="brush:c#;">
...
<p>Поскольку это действие будет применяться к элементу Entry, то базовый класс типизируется типом Entry. В метод Invoke в качестве параметра передается 
...
наполовину прозрачным.</p>
<p>Теперь используем это действие:</p>
<pre class="brush:c#;">
...
<p>Класс Trigger определяет два специальных свойства: <span class="b">EnterActions</span> (хранит действия, которые применяются при срабатывании триггера) и 
<span class="b">ExitActions</span> (хранит действия, которые выполняются, когда триггер перестает действовать). Причем в обе эти коллекции-свойства передается FocusTriggerAction, 
что позволит включить действие при включении триггера и отключать при отключении триггера.</p>
<h3>Триггеры событий</h3>
<p>Триггеры событий вызываются в ответ на события элемента. Но чтобы использовать триггер событий, нам надо вначале создать действие, которое будет вызываться триггером. 
.</p>
<p>Итак, добавим в наш проект следующий класс:</p>
<pre class="brush:c#;">
...
<p>В данном случае мы будем ожидать ввод только цифр в поле Entry. Если же будут введены нецифровые символы, то поле окрасится в красный цвет.</p>
<p>Теперь применим это действие:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<p>Триггер событий также определяется в элементе <code>Style.Triggers</code>, только теперь он представлен объектом <span class="b">EventTrigger</span>.</p>
<p>Атрибут <code>Event</code> этого объекта указывает на событие, при возникновении которого будет вызываться триггер. В данном случае это событие 
<code>TextChanged</code>, то есть изменение текста в поле Entry.</p>
<p>И в самом EventTrigger определяется наше действие EntryValidation.</p>
<p>В итоге, если мы введем в поле нецифровые символы, то это поле окрасится в красный цвет:</p>
<img src="./pics/9.6.png" alt="Триггер событий в Xamarin Forms" />
<h4>Триггеры событий в коде</h4>
<p>У визуальных элементов есть свойство <span class="b">Triggers</span>, которое мы можем использовать для добавления триггера:</p>
<pre class="brush:c#;">
public partial class MainPage : ContentPage
{
    public MainPage()
...
<p>Вначале собственно создается триггер - объект EventTrigger, и с помощью свойства <code>Event</code> у него устанавливается событие.</p>
<p>Далее в коллекцию <code>Actions</code> в триггере добавляется объект EntryValidation, а затем сам триггер добавляется в коллекцию Triggers у элемента Entry.</p>
<p>Результат работы будет аналогичен действию кода в xaml.</p>
