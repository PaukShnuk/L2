...
<html >
<head>
<title>Xamarin Forms | Текстовые поля</title>
<meta charset="utf-8" />
<meta name="description" content="Текстовые поля Label, Entry и Editor в Xamarin Forms">
...
<h2>Текстовые поля</h2><div class="date">Последнее обновление: 09.01.2021</div>
...
<p>В Xamarin Forms текстовые поля представлены несколькими классами:</p>
<ul>
<li><p><span class="b">Label</span>: текстовая метка для вывода текста</p></li>
<li><p><span class="b">Entry</span>: однострочное текстовое поле</p></li>
<li><p><span class="b">Editor</span>: многострочное текстовое поле</p></li>
</ul>
<h3>Label</h3>
<p>Label представляет обычную текстовую метку, которая выводит информацию с помощью свойства Text. Label удобен для создания заголоков и меток к элементам ввода.</p>
<p>Label довольно простой элемент, однако даже с ним могут возникать некоторые сложности. Например, кнопки определяют событие нажатия, 
...
специальным классом <span class="b">TapGestureRecognizer</span>, который позволяет обрабатывать нажатия:</p>
<pre class="brush:c#;">
...
<p>TapGestureRecognizer представляет специальный класс, который позволяет распознать нажатия. С помощью свойства <code>NumberOfTapsRequired</code> 
...
<code>Tapped</code>, в обработчике которого мы будем переключать цвет текста и фона метки.</p>
<p>И чтобы связать объект TapGestureRecognizer с меткой, необходимо добавить этот объект в коллекцию <code>GestureRecognizers</code>:</p>
<pre class="brush:c#;">label.GestureRecognizers.Add(tapGesture);</pre>
<img src="./pics/2.78.png" alt="TapGestureRecognizer в Xamarin Forms" />
<h3>Тектовое поле Entry</h3>
<p>Entry представляет текстовое поле для ввода однострочной информации:</p>
<pre class="brush:c#;">
...
<img src="./pics/2.21.png" alt="Entry in Xamarin" />
<p>Обработать ввод пользователя мы можем с помощью обработки события <code>TextChanged</code></p>
...
<p>Основные свойства элемента Entry, которые мы можем использовать:</p>
<ul>
<li><p><span class="b">Text</span>: введенный в поле текст</p></li>
<li><p><span class="b">TextColor</span>: цвет вводимого текста</p></li>
<li><p><span class="b">Placeholder</span>: текст-заменитель, который отображается в поле до ввода и исчезает при начале печати</p></li>
<li><p><span class="b">IsPassword</span>: при значении <code>true</code> указывает, то поле будет предназначено для ввода пароля, и поэтому все 
вводимые символы будут заменяться звездочкой</p></li>
</ul>
<p>Также класс Entry определяет два события:</p>
<ul>
<li><p><span class="b">TextChanged</span>: возникает при вводе символов в поле</p></li>
<li><p><span class="b">Completed</span>: возникает при завершении ввода</p></li>
</ul>
<h3>Формат клавиатуры</h3>
<p>Среди свойств текстового поля ввода наиболее интересным является свойство <span class="b">Keyboard</span>. Оно позволяет задать формат клавиатуры для 
...
пользователю сразу числовую раскладку клавиатуры. Данное свойство принимает одно из значений перечисления Keyboard:</p>
<ul>
<li><p>Default</p></li>
<li><p>Text</p></li>
<li><p>Chat</p></li>
...
<p>Каждое значение предоставляет свою раскладку клавиатуры, предназначенную для ввода определенной информации. Например, 
<code>entry.Keyboard=Keyboard.Telephone</code> - при вводе нам автоматически представляется раскладка, содержащая только те знаки, которые используются при наборе телефонного номера.</p>
<h3>Editor</h3>
<p>В отличие от Entry <span class="b">Editor</span>  представляет собой многострочное поле ввода, но принципы его работы аналогичны. Он также имеет все те же 
свойства и события. Например:</p>
<pre class="brush:c#;">
public MainPage()
{
    StackLayout stackLayout = new StackLayout();
...
<img src="./pics/2.22.png" alt="Многострочное поле Editor in Xamarin" />
<p>Реализация в xaml:</p>
<pre class="brush:xml;">
&lt;StackLayout&gt;
...
<div class="nav"><p><a href="./3.7.php">Назад</a><a href="./">Содержание</a><a href="./3.3.php">Вперед</a></p></div></div>
<div class="socBlock">
