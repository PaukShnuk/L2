...
<html >
<head>
<title>Xamarin Forms | Создание интерфейса для работы с веб-сервисом</title>
<meta charset="utf-8" />
<meta name="description" content="Создание интерфейса мобильногого приложения на Xamarin Forms для работы с веб-сервисом">
...
<h2>Создание интерфейса для работы с веб-сервисом</h2><div class="date">Последнее обновление: 14.01.2021</div>
...
<p>Продолжим проект из прошлой темы и создадим для него графический интерфейс. Итак, по умолчанию у нас уже есть 
страница <span class="b">MainPage.xaml</span>, которую мы будем использовать для вывода списка друзей. И также добавим в 
проект новую страницы XAML, которую назовем <span class="b">FriendPage.xaml</span> и которая будет применяться 
для добавления/изменения друга.</p>
<p>И также добавим в проект класс ApplicationViewModel, через который будет идти взаимодействие графического интерфейса и веб-сервера:</p>
<pre class="brush:c#;">
...
<p>В этом классе определяется коллекция Friends, где будут храниться все друзья, а также переменная friendsService для взаимодействия с сервером.</p>
<p>Также в классе определяется ряд команд и свойство IsBusy, которое будет указывать, находятся ли данные в процессе загрузки.</p>
<p>В конструкторе определяем пустой список Friends и команды</p>
<p>Начальные данные в список Friends загружаются дс помощью метода <code>GetFriends()</code>. В этом методе <code>GetFriends()</code> идет обращение 
к серверу через метод <code>Get()</code> объекта friendsService. Но в данном случае мы будем загружать данные только один раз - при старте приложения. И для 
этого в классе определен флаг initialized, который приобретает значение true после начальной загрузки.</p>
<p>Через свойство <code>SelectedFriend</code> мы будем отслеживать выделенный элемент в списке. Когда пользователь будет выбирать какой-то объект в списке, 
...
<p>Команда CreateFriendCommand будет вызывать метод <code>CreateFriend()</code>, который будет передавать новый объект на страницу FriendPage:</p>
<pre class="brush:c#;">
private async void CreateFriend()
{
    await Navigation.PushAsync(new FriendPage(new Friend(), this));
...
<p>Команда сохранения SaveFriendCommand будет получать созданный/отредактированный объект Friend и в зависимости от режима (добавление или редактирование) 
использовать один из методов класса FriendService для отправки запроса.</p>
<p>Так как наш веб-сервис, который был создан в позапрошлой теме, при добавлении или изменении возвращает добавленный/измененный объект, то, получв этот объект от сервера, 
мы можем его добавить в коллекцию Friends или изменить элемент в этой коллекции.</p>
<p>Изменение:</p>
<pre class="brush:c#;">
Friend updatedFriend = await friendsService.Update(friend);
// заменяем объект в списке на новый
...
<p>Команда DeleteFriendCommand выполняет метод DeleteFriend, который во многом аналогичен редактированию объекта:</p>
<pre class="brush:c#;">
Friend deletedFriend = await friendsService.Delete(friend.Id);
if(deletedFriend!=null)
{
...
<p>Это основные моменты класса ApplicationViewModel, но надо сказать, что многое зависит от сервера и его API. В данном случае веб-сервис делаем мы сами, и 
...
мобильный траффик.</p>
<p>Однако используемый нами веб-сервис может не представлять подобной функциональности, как возвращение измененного объекта, либо к данным на сервере могут одновременно 
обращаться несколько пользователей и соответственно изменять эти данные. И в этом случае надежнее будет заново загружать данные после каждой произведенной операции.</p>
<p>Далее изменим страницу FriendPage. В коде Xaml определим простейший интерфейс:</p>
<pre class="brush:xml;">
...
<p>А в коде FriendPage.xaml.cs пропишем передачу на страницу объектов Friend и ApplicationViewModel и установку контекста:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>Далее на странице MainPage.xaml определим вывод списка друзей:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<p>А в коде MainPage.xaml.cs пропишем установку контекста страницы и загрузку начальных данных:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>И в конце установим страницу NavigationPage в качестве главной в классе App:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<img src="./pics/10.20.png" alt="веб-сервис в Xamarin Forms" />
<p>Запустим веб-сервис и мобильное приложение и нажмем на кнопку добавления. Введем какие-нибудь данные:</p>
<img src="./pics/10.12.png" alt="Добавление на веб-сервис в Xamarin Forms" />
<p>И после добавления новый объект отобразится на главной странице:</p>
<img src="./pics/10.13.png" alt="Работа с веб-сервисом в Xamarin Forms" />
...
<div class="nav"><p><a href="./10.5.php">Назад</a><a href="./">Содержание</a><a href="./13.1.php">Вперед</a></p></div></div>
<div class="socBlock">
