...
<html >
<head>
<title>Xamarin Forms | Свойство Wrap в FlexLayout</title>
<meta charset="utf-8" />
<meta name="description" content="Свойство Wrap в FlexLayout в Xamarin Forms, перенос элементов на новую строку и столбец">
...
<h2>Свойство Wrap</h2><div class="date">Последнее обновление: 11.01.2021</div>
...
<p>Свойство <span class="b">Wrap</span> позволяет установить правила переноса элементов, если они не помещаются по ширине или высоте контейнера. 
Это свойство принимает одно из значений из перечисления <span class="b">FlexWrap</span>:</p>
<ul>
<li><p><span class="b">NoWrap</span>: элементы не переносятся и ужимаются до таких размеров, достаточных для вмещения в контейнер. Это значение по умолчанию</p></li>
<li><p><span class="b">Wrap</span>: элементы переносятся на новую строку (при расположении в строку) или на новый столбец при расположении в столбик</p></li>
<li><p><span class="b">Reverse</span>: элементы переносятся в обратном порядке</p></li>
</ul>
<p>Вначале возьмем значение по умолчанию - "NoWrap":</p>
<pre class="brush:xml;">
...
<p>Здесь кнопки расположены в строку. Каждая кнопка имеет ширину в 150 единиц, однако при такой ширине все кнопки могут не уместиться на 
экране устройства:</p>
<img src="./pics/flexlayout6.png" alt="NoWrap в FlexLayout в Xamarin Forms" />
<p>Из скриншота мы видим, что все элементы уместились по ширине экрана, но их ширина далеко не 150 единиц.</p>
<p>Такое поведение не всегда может быть желательным. Возможно, мы ожидаем, что неумещающиеся элементы будут просто переноситься дальше на следующую строку, 
что было бы вполне естественно. Для этого нам надо использовать значение <span class="b">Wrap</span>:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<p>В данном случае мы видим, что элементы сохраняют свою ширину и переносятся на новую строку. В итоге при ширине экрана в 400 единиц 
контейнер будет располагать элементы в три строки.</p>
<p>Подобным образом мы можем управлять переносом элементов при располжении в столбик. В этом случае элементы переносятся в новый столбец.</p>
<p>При альбомной ориентации устройства:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<img src="./pics/flexlayout8.png" alt="FlexLayout column Wrap in Xamarin Forms" />
