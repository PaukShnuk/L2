...
<html >
<head>
<title>Xamarin Forms | Определение языковой культуры</title>
<meta charset="utf-8" />
<meta name="description" content="Определение языковой культуры в Xamarin Forms, настройка поддерживаемых языков">
...
<h2>Определение языковой культуры</h2><div class="date">Последнее обновление: 14.01.2021</div>
...
<p>В прошлой теме был создан проект и ряд ресурсов, позволяющих локализовать приложение. Однако просто установки в коде нужного ресурса бывает недостаточно. Нужно 
явным образом указать требуемую языковую культуру для определенной платформы.</p>
<p>Поскольку каждая платформа требует определенного подхода, то нам надо использовать класс <code>DependencyService</code>, который позволит сопоставить нужные зависимости.</p>
<p>Вначале добавим в главный проект интерфейс ILocalize:</p>
<pre class="brush:c#;">
...
<p>Его единственный метод будет определять языковую культуру мобильного устройства.</p>
<p>Теперь добавим в проект для iOS следующий класс Localize:</p>
<pre class="brush:c#;">
using Foundation;
using Xamarin.Forms;
...
<p>Реализация этого класса использует массив <code>NSLocale.PreferredLanguages</code> для определения языковой культуры.</p>
<p>При получении языка на устройстве iOS нам следует учитывать следующий аспект. Пользователь теоретически может установить в качестве культуры, например, 
...
<code>try...catch</code>, где в случае неудачи получить установленную культуры мы получаем предпочтительную культуру - "en-US".</p>
<p>Некоторые системные элементы управления iOS переводит автоматически (например, элемент Picker). Но чтобы указать системе на необходимость перевода, 
нам надо внести в файл <span class="b">Info.plist</span>, который имеется в проекте, соответствующие определения языковых культур. Info.plist представляет 
обычный xml-файл, поэтому откроем его в каком-нибудь текстовом редакторе. Он выглядит примерно так:</p>
<pre class="brush:xml;">
...
<p>В пределах элемента <code>dict</code> добавим определения языков. В моем случае язык по умолчанию английский (en) и две дополнительных языковых культуры: 
русскоязычная (ru) и немецкая (de). Поэтому я добавляю следующие строки:</p>
<pre class="brush:xml;">
...
<h3>Настройка проекта для Android</h3>
<p>В проект для Androida добавим следующий класс:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>Опять же это реализация интерфейса ILocalize, которая использует объект <code>Java.Util.Locale.Default</code> для определения культуры.</p>
<h3>Настройка проекта для UWP</h3>
<p>Также добавим реалзицию интерфейса ILocalize в проект для UWP:</p>
<pre class="brush:c#;">
using System.Globalization;
using Xamarin.Forms;
...
<p>Языковая культура приложения должна определяться как можно раньше. В частности, это можно сделать при открытии стартовой страницы приложения. Итак, 
добавим в конструктор класса <span class="b">App</span> в главном проекте определения языковой культуры:</p>
<pre class="brush:c#;">
...
<p>И поскольку фреймворк автоматически распознает язык на устройстве Windows 10, нам нет нужды устанавливать его вручную, поэтому сначала в коде проверяем 
тип платформы <code>if (Device.RuntimePlatform != Device.UWP)</code></p>
