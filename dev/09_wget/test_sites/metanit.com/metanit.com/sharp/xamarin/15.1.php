...
<html >
<head>
<title>Xamarin Forms | Работа с камерой</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с камерой в Xamarin Forms, создание и просмотр фотографий">
...
<h1>Работа с мультимедиа</h1><h2>Работа с камерой</h2><div class="date">Последнее обновление: 13.01.2021</div>
...
<p>Работа с камерой на Xamarin вовлекает платформо-зависимый код, специфичный для каждой платформы. Но к счастью нам 
не придется писать обобщенный код, который связал бы все платформы. Благодаря классу <span class="b">MediaPicker</span> из пакета 
<span class="b">Xamarin.Essentials</span>, который по умолчанию добавляется в проект Xamarin Forms, съемка/просмотр фото/видео производятся 
довольно просто. 
<p>Однако первым делом для работы с камерой необходимо установить соответствующие разрешения для каждой платформы.</p>
<h3>Установка разрешений</h3>
<h4>Android</h4>
<p>В проекте для Android в файл <span class="b">AssemblyInfo.cs</span> в папке <span class="b">Properties</span> необходимо добавить 
следующие строки:</p>
<pre class="brush:c#;">
...
<p>Либо добавить в файл <span class="b">AndroidManifest.xml</span>, который располагается там же в папке <span class="b">Properties</span>, 
в узел <span class="b">manifest</span> следующее содержимое:</p>
<pre class="brush:xml;">
...
<h4>iOS</h4>
<p>Для работы на iOS требуется дополнительно добавить в файл <span class="b">Info.plist</span> следующие строки:</p>
<pre class="brush:c#;">
...
<p>Внимание! Текст между тегами <code>&lt;string&gt;....&lt;/string&gt;</code> следует заменить на текст, которые затем будет отображаться 
пользователям в качестве информации о вашем приложении.</p>
<h4>UWP</h4>
<p>Для проекта UWP необходимо отметить в файле <span class="b">Package.appxmanifest</span> в секции <span class="b">Capabilities</span> 
пункты <span class="b">Microphone</span> и <span class="b">Webcam</span>.</p>
<h3>Класс MediaPicker</h3>
<p><span class="b">MediaPicker</span> предоставляет следующие методы для работы с камерой:</p>
<ul>
<li><p><span class="b">PickPhotoAsync</span>: открывает на устройстве проводник для выбора фото</p></li>
<li><p><span class="b">CapturePhotoAsync</span>: открывает приложение камеры для съемки фото</p></li>
<li><p><span class="b">PickVideoAsync</span>: открывает проводник для выбора файла видео</p></li>
<li><p><span class="b">CaptureVideoAsync</span>: открывает приложение камеры для съемки видео</p></li>
</ul>
<p>Все методы возвращают объект <span class="b">FileResult</span> - наследник класса <span class="b">Stream</span>, с которым можно 
работать как с файловым потоком, например, сохранить его данные в файл.</p>

<p>Изменим главную страницу MainPage таким образом, чтобы она имела кнопки для съемки и выбора фото и областью просмотра снятой фотографии:</p>
<pre class="brush:c#;">
using System;
using System.IO;
...
<p>В обработчике первой кнопки будет производиться выбор фото. Сначала получаем само фото в виде объкта FileResult</p>
<pre class="brush:c#;">var photo = await MediaPicker.PickPhotoAsync()</pre>
<p>Затем загружаем его в ImageView:</p>
<pre class="brush:c#;">img.Source = ImageSource.FromFile(photo.FullPath);</pre>

<p>В обработчике второй кнопки делаем съемку и сохраняем фото:</p>
<pre class="brush:c#;">
var photo = await MediaPicker.CapturePhotoAsync(new MediaPickerOptions 
{ 
	Title = $"xamarin.{DateTime.Now.ToString("dd.MM.yyyy_hh.mm.ss")}.png"
...
<p>В данном случае в метод MediaPicker.CapturePhotoAsync передается необязательный параметр - объект <span class="b">MediaPickerOptions </span>, 
который с помощью свойства Title позволяет настроить название файла.</p>
<p>Получив объект FileResult, мы можем сохранить его в качестве файла по некоторому пути. В данном случае сохраняем в локальную папку приложения:</p>
<pre class="brush:c#;">
var newFile = Path.Combine(FileSystem.AppDataDirectory, photo.FileName);
using (var stream = await photo.OpenReadAsync())
using (var newStream = File.OpenWrite(newFile))
...
<p>Запустим приложение. И у нас должна отобразиться кнопка для съемки фото:</p>
<img src="./pics/14.2.png" alt="Камера в Xamarin Forms" />
<p>При нажатии на первую кнопку система сначала запросит, нужно ли предоставить приложению разрешения для использования камеры:</p>
<img src="./pics/14.1.png" alt="Съемка фото и видео в Xamarin" />
<p>После установки разрешения нас перебросит в стандартное приложении камеры, которое имеется на устройстве. Сделаем фото, и приложение предложит сохранить сделанное фото:</p>
<img src="./pics/14.3.png" alt="Take a photo in Xamarin Forms" />
<p>После сохранения фото будет отображаться в элементе Image:</p>
<img src="./pics/14.4.png" alt="Работа с камерой и фото в Xamarin Forms" />
<p>Либо мы можем нажать на вторую кнопку и выбрать в галерее необходимую фотографию для просмотра.</p>
...
<div class="nav"><p><a href="./14.4.php">Назад</a><a href="./">Содержание</a><a href="./17.1.php">Вперед</a></div></div>
<div class="socBlock">
