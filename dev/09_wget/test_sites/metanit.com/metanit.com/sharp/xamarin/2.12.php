...
<html >
<head>
<title>Xamarin Forms | Метод LoadFromXaml и загрузка XAML</title>
<meta charset="utf-8" />
<meta name="description" content="Метод LoadFromXaml и загрузка XAML в Xamarin Forms, динамическая загрузка и изменение элементов на странице">
...
<h2>Метод LoadFromXaml и загрузка XAML</h2><div class="date">Последнее обновление: 08.01.2021</div>
...
<p>В прошлых темах были рассмотренны генерируемые по умолчанию при создании проекта файлы MainPage.xaml и MainPage.xaml.cs. Если файл MainPage.xaml 
...
загрузка XAML?</p>
<p>По умолчанию файл MainPage.xaml.cs содержит следующее определение класса MainPage:</p>
<pre class="brush:c#;">
...
<p>По сути, все, что делает класс - это вызов метода <span class="b">InitializeComponent()</span>, благодаря которому страница получает определенный в связанном 
xaml-файле графический интерфейс.</p>
<p>При построении проекта Visual Studio парсит файлы с кодом XAML и генерирует файл, название которого заканчивается на 
<span class="ii">.g.cs</span> (например, <span class="ii">MainPage.xaml.g.cs</span>, его можно найи в папке проекта в подкаталоге 
obj\Debug\netstandard2.0). То есть генерируется обычный класс C#, который содержит определение метода <span class="b">InitializeComponent()</span>:</p>
<pre class="brush:c#;">
...
<p>Мы видим, что метод <code>InitializeComponent()</code> вызывает другой метод - <span class="b">LoadFromXaml()</span>. 
Этот метод извлекает файл XAML (или его скомпилированную бинарную версию) из файла dll, в который скомпилировано приложение.</p>
<p>После извлечения кода графического интерфейса метод инициализирует все объекты, определенные в файле XAML, устанавливает структуру элементов (какой элемент какие элементы содержит), 
прикрепляет обработчики событий, определенные в файле кода C#, и создает дерево объектов. Таким образом, код из XAML используется для построения интерфейса.</p>
<h3>Динамическая загрузка XAML</h3>
<p>Однако мы сами можем загружать код XAML, причем динамически в процессе выполнения программы, используя тот же самый метод <span class="b">LoadFromXaml()</span>. 
...
версии интерфейса. Но вместе с тем стоит отметить, что динамическая загрузка XAML снижает производительность приложения, поэтому по возможности ее следует избегать.</p>
<p>Например, динамически загрузим интерфейс для всей страницы:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>Метод <span class="b">LoadFromXaml()</span> является методом расширения, определенным для всех визуальных элементов управления в пространстве имен 
<span class="b">Xamarin.Forms.Xaml</span>. После загрузки страницы она будет выглядеть следующим образом:</p>
<img src="./pics/2.95.png" alt="LoadFromXaml в Xamarin" />
<p>Может возникнуть необходимость, обращаться к элементам подобного интерфейса в коде C#. Для нахождения элементов по имени можно применять метод 
<span class="b">FindByName()</span>. Например, изменим тест метки:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
using Xamarin.Forms.Xaml;
...
namespace HelloApp
{
    public partial class MainPage : ContentPage
    {
        public MainPage()
        {
            ...
            this.LoadFromXaml(pageXAML);
            Label lbl = this.FindByName&lt;Label&gt;("lbl");
            lbl.Text = "Changed";
        }
    }
}
</pre>
<p>Метод <code>FindByName()</code> типизированный - он типизируется типом элемента, который надо найти. Например, в даном случае мы ищем элемент Label с именем 
lbl. Соответственно метод <code>FindByName()</code> тизируется типом Label, а в качестве параметра в него предается строка lbl.</p>
<p>Загружать XAML мы можем не только для страницы в целом, но и для отдельных элементов. Например, определим в MainPage.xaml следующую разметку:</p>
<pre class="brush:xml;">
...
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             ...
             x:Class="HelloApp.MainPage"&gt;
    &lt;StackLayout&gt;
        &lt;Label x:Name="lbl" Text="Hello"  FontSize="24" /&gt;
        &lt;Button Text="Change" Clicked="Button_Clicked" FontSize="24" /&gt;
    &lt;/StackLayout&gt;
&lt;/ContentPage&gt;
</pre>
<p>Здесь определены метка и кнопка. У кнопки задан обработчик события нажатия.</p>
<p>Теперь изменим файл <span class="b">MainPage.xaml.cs</span>:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
using Xamarin.Forms.Xaml;
...
<p>В обработчике нажатия кнопки здесь изменяется разметка элемента Label.</p>
<img src="./pics/2.93.png" alt="LoadFromXaml в Xamarin Forms" />
