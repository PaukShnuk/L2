...
<html >
<head>
<title>Xamarin Forms | Навигация</title>
<meta charset="utf-8" />
<meta name="description" content="Переход между страницами в Xamarin Forms, методы PushAsync и PopAsync">
...
<h1>Навигация</h1><h2>Основы навигации</h2><div class="date">Последнее обновление: 13.01.2021</div>
...
<p>Для поддержки навигации на каждой странице Page в Xamarin Forms определено свойство <span class="b">Navigation</span>. Это свойство предтавляет интерфейс <span class="b">INavigation</span>, 
в котором есть следующие методы:</p>
<pre class="brush:c#;">
Task PushAsync(Page page)
Task PushModalAsync(Page page)
</pre>
<p>В качестве параметра здесь передается объект Page - страница, на которую надо осуществить переход.</p>
<p>Второй метод имеет в своем названии слово "Modal" и осуществляет переход на модальную страницу.Обычно модальные страницы используются, когда приложению 
нужно получить некоторую информацию от пользователя. При этом до получения информации нельзя возвращаться на предыдущую страницу.</p>
<p>Также имеются методы для возврата на предыдущую страницу:</p>
<pre class="brush:c#;">
Task&lt;Page&gt; PopAsync()
Task&lt;Page&gt; PopModalAsync()
</pre>
<p>Итак, рассмотрим простейший пример навигации, используя все эти методы. Создадим новый проект по типу Xamarin Forms Portable.</p>
<p>Так как наше приложение будет производить навигацию, то есть переходы между страницами, то вначале добавим эти вспомогательные страницы.</p>
<p>Итак, добавим в проект новый элемент <span class="b">ContentPage (C#)</span>, который назовем <span class="b">CommonPage.cs</span>:</p>
<img src="./pics/5.3.png" alt="ContentPage in Xamarin Forms" />
<p>Определим в этом классе следующий код:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>На этой странице определяется единственная кнопка, по нажатию на которую осуществляется переход назад с помощью метода <code>Navigation.PopAsync()</code>. Поскольку 
метод является асинхронным, то перед ним ставится ключевое слово <code>await</code>, а сам обработчик кнопки помечается с помощью ключевого слова <code>async</code>.</p>
<p>Также стоит отметить, что у страницы устанавливается заголовок через свойство <code>Title</code>.</p>
<p>Подобным образом добавим вторую страницу, которую назовем <span class="b">ModalPage</span>. Определим в ней следующий код:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>Данная страница выглядит во многом аналогично предыдущий за тем исключением, что переход назад здесь производится с помощью метода <code>Navigation.PopModalAsync()</code>. 
То есть эта страница будет модальной.</p>
<p>И изменим код главной страницы MainPage:</p>
<pre class="brush:c#;">
using System;
using Xamarin.Forms;
...
<p>Итак, здесь две кнопки. Одна производит переход на CommonPage с помощью метода <code>Navigation.PushAsync()</code>, а другая осуществляет переход на 
ModalPage посредством вызова <code>Navigation.PushModalAsync()</code></p>
<p>Но чтобы с MainPage можно было бы переходить к CommonPage или ModalPage, нам надо в качестве главной страницы использовать объект NavigationPage, а не ContentPage, как установлено по умолчанию. 
Поэтому перейдем к классу App и изменим код установки главной страницы:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>И чтобы использовать механизм навигации в приложении, необходимо обернуть объект MainPage в NavigationPage:</p>
<pre class="brush:c#;">MainPage = new NavigationPage(new MainPage());</pre>
<p>Иначе мы не сможем использовать навигацию в приложении. Сам же класс NavigationPage визуально только добавляет заголовок страницы, который устанавливается с 
помощью свойства <code>Title</code> и который отображается рядом с иконкой приложения. Хотя в зависимости от операционной системы отображение заголовка может отличаться.</p>
<p>Итак, запустим приложение:</p>
<img src="./pics/5.4.png" alt="Навигация в Xamarin Forms" />
<p>Попробуем перейти на обычную страницу, нажав на первую кнопку:</p>
<img src="./pics/5.5.png" alt="Navigation in Xamarin Forms" />
<p>Как из скриншота можно увидеть, что в данном случае даже не нужна кнопка для возвращаения назад, так как мы можем вернуться через стрелку рядом с заголовком.</p>
<p>Вернемся назад и нажмем на вторую кнопку для перехода к ModalPage:</p>
<img src="./pics/5.6.png" alt="Modal Page in Xamarin Forms" />
<p>Так как для перехода к этой странице использовался метод <code>Navigation.PushModalAsync()</code>, то данная страница будет модальной. Основное отличие 
модальной страницы в визуальном плане состоит в отстутсвии заголовка, несмотря на то, что в коде мы устанавливаем свойство <code>Title</code> у ModalPage.</p>
<p>Ну и для возврата с этой страницы назад нам надо использовать метод <code>Navigation.PopModalAsync()</code></p>
<p>При в реальности дл создания модальной страницы необязательно создавать специальный класс страницы, как здесь класс ModalPage. Так, мы могли бы использовать в качестве 
модальной и CommonPage.</p>
<h3>Анимация в постраничных переходах</h3>
<p>Все ранее расмотренные методы навигации в качестве второго параметра принимают логическое значение:</p>
<pre class="brush:c#;">
...
<p>И если оно равно <code>true</code>, то при переходе на другую страницу осуществляется анимация, если, конечно, целевая платформа поддерживает подобную анимацию.</p>
<p>По умолчанию анимация включена, поэтому, чтобы ее отключить, нам надо использовать одну из этих версий, передавая значение false.</p>
