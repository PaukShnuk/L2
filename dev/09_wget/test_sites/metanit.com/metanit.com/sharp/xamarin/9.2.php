...
<html >
<head>
<title>Xamarin Forms | Стили</title>
<meta charset="utf-8" />
<meta name="description" content="Использование стилей в Xamarin Forms, стили в XAML и в коде C#, объект Style и Setter, наследование стилей и свойство BasedOn">
...
<h2>Стили</h2><div class="date">Последнее обновление: 12.01.2021</div>
...
<p>Стили позволяют определить набор некоторых свойств и их значений, которые потом могут применяться к элементам. Основная их задача - создать стилевое единообразие 
для элементов интерфейса. Стили хранятся в ресурсах и отделяют значения свойств элементов от пользовательского интерфейса.</p>
<p>Чтобы понять, как стили упрощают нам работу, рассмотрим простой пример:</p>
<pre class="brush:xml;">
...
<p>Здесь определены две кнопки, которые фактически имеют один и тот же стиль: одни и те же цвет фона и текста, а также размер текста. Единственное отличие состоит в тексте кнопки.</p>
<p>Однако в данном случае мы вынуждены повторяться и повторно определять один и те же свойства и одни и те же значения для каждого из элементов.</p>
<img src="./pics/9.1.png" alt="Стили в Xamarin Forms" />
<p>Теперь применим стили:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<p>Стиль создается как ресурс с помощью объекта Style и, как любой другой ресурс, он обязательно должен иметь ключ. Атрибут <span class="b">TargetType</span> 
указывает, к какому типу оносится стиль. В данном случае это тип <code>Button</code>.</p>
<p>С помощью коллекции Setters определяется группа свойств, входящих в стиль. В нее входят объекты Setter, которые имеют следующие свойства:</p>
<ul>
<li><p><span class="b">Property</span>: указывает на свойство, к которому будет применять данный сеттер. При этом свойство должно представлять тип BindableProperty</p></li>
<li><p><span class="b">Value</span>: собственно значение свойства</p></li>
</ul>
<p>Поскольку стиль определяется как ресурс, то для его установки используются расширения StaticResource или DynamicResource (если стиль динамический):</p>
<pre class="brush:xml;">
&lt;Button Text="iOS" Style="{StaticResource buttonStyle}" /&gt;
&lt;Button Text="Android" Style="{DynamicResource buttonStyle}" /&gt;
</pre>
<p>Иногда свойство может представлять сложный объект, либо же значение формируется сложным способом. Например, у структуры Color есть статический метод <code>Color.FromRgb()</code>, 
который создает цвет по трем значениям. В этом случае мы можем расписать формирование объекта:</p>
<pre class="brush:xml;">
&lt;Style x:Key="buttonStyle" TargetType="Button"&gt;
...
<p>Также в качестве значения можно устанавливать ссылку на другой ресурс:</p>
<pre class="brush:xml;">
&lt;ContentPage.Resources&gt;
    &lt;ResourceDictionary&gt;
...
<h3>TargetType</h3>
<p>Если нам надо создать общий стиль для элементов определенного типа, то можно не задавать ключ ресурса, а достаточно установить у стиля атрибут 
<span class="b">TargetType</span>, в который передается тип элементов:</p>
<pre class="brush:xml;">
&lt;ContentPage.Resources&gt;
  &lt;ResourceDictionary&gt;
    &lt;Color x:Key="greenColor"&gt;#004D40&lt;/Color&gt;
...
<p>Теперь у кнопок не надо будет указывать ресурс стиля, так как стиль будет автоматически применяться ко всем объектам типа, который указан в атрибуте TargetType.</p>
<h3>Переопределение стилей</h3>
<p>Стиль позволяет задать некоторые начальные значения. Однако элемент может переопределить отдельные значения из стиля:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<p>В данном случае кнопка получает все значения из стиля buttonStyle, однако переопределяет цвет текста, так как прямое использование атрибутов элемента имеет приоритет 
над приеняемым стилем.</p>
<h3>Установка стилей в коде</h3>
<p>Для создания стиля в коде используется объект Style:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>В конструктор объекта Style передается тип, для которого предназначен данный стиль - аналогично использованию атрибута TargetType в XAML.</p>
<p>При создании стиля в коде следует учитывать, что в качестве свойств указываются именно BindableProperty (как правило называется по имени обычного свойства 
с суффиксом <span class="ii">Property</span>). Например:</p>
<pre class="brush:c#;">Property = Button.TextColorProperty</pre>
<p>А не просто TextColor. Причем в начале идет тип (в данном случае Button), а потом идет название свойства (здесь TextColorProperty).</p>
<p>Правда, здесь надо отметить, что в коде помещать стиль в ресурсы не имеет смысла, так как мы можем напрямую присвоить кнопке или другому элементу 
определенный стиль:</p>
<pre class="brush:c#;">
public partial class MainPage : ContentPage
{
    public MainPage()
...
<p>С помощью свойства <span class="b">BasedOn</span> можно наследовать один стиль от другого. Например:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
<p>Здесь у стиля <code>greenButtonStyle</code> атрибут <code>BasedOn</code> указывает на другой стиль <code>baseButtonStyle</code>. И таким образом, 
...
значение свойства TextColor.</p>
<p>При наследовании стилей важно, чтобы тип элементов, указанный в качестве значения атрибута <span class="b">TargetType</span>, совпадал. Например, в данном случае 
оба стиля применяется к элементам типа Button.</p>
<p>Наследование в коде осуществляется с помощью установки у стиля свойства BasedOn:</p>
<pre class="brush:c#;">
...
<p>Xamarin Forms предоставляет ряд встроенных стилей, которые мы можем использовать:</p>
<ul>
<li><p><code>BodyStyle</code>: стиль для текста страницы</p></li>
<li><p><code>CaptionStyle</code>: стиль для подписей к изображениям и т.д.</p></li>
<li><p><code>ListItemDetailTextStyle</code>: стиль для детального описания в списке</p></li>
<li><p><code>ListItemTextStyle</code>: стиль для текста элемента списка</p></li>
<li><p><code>SubtitleStyle</code>: стиль для подзаголовка</p></li>
<li><p><code>TitleStyle</code>: стиль для заголовка страницы</p></li>
</ul>
<p>В то же время эти стили имеют ограничения: они применяются только к элементу Label и устанавливают стилевые настройки только для текста метки.</p>
<p>Применение стилей:</p>
<pre class="brush:xml;">&lt;Label Text="Заголовок" Style="{DynamicResource TitleStyle}" /&gt;</pre>
