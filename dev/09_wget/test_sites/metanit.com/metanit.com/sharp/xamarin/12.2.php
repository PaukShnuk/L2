...
<html >
<head>
<title>Xamarin Forms | Платформозависимость в XAML</title>
<meta charset="utf-8" />
<meta name="description" content="Платформозависимость в XAML в Xamarin Forms, класс OnPlatform и его использование">
...
<h2>Платформозависимость в XAML</h2><div class="date">Последнее обновление: 11.01.2021</div>
...
<p>Для определения платформозависимых значений свойств применяется класс <span class="b">OnPlatform&lt;T&gt;</span>. Например, нам надо для каждой платформы 
установить свой текст и цвет текста элемента Label:</p>
<pre class="brush:xml;">
...
<p>В качестве значения свойства Text и TextColor принимают объект <code>OnPlatform</code>. Чтобы указать, значения какого типа должны быть переданы, применяется 
атрибут <span class="b">x:TypeArguments</span>. Например, свойство Text в качестве значения принимает строку, то есть объект типа String, поэтому устанавливаем 
<code>x:TypeArguments="x:String"</code>. А свойство TextColor принимает объект Color, поэтому пишем <code>x:TypeArguments="Color"</code></p>
<p>Следующий набор атрибутов: <code>iOS</code>, <code>Android</code> и <code>WinPhone</code> устанавливают значения для каждой операционной системы. 
Значение WinPhone распространяется не только на Windows Phone, но и на десктопные версии Windows.</p>
<p>Также мы можем расписать определение OnPlatform более подробно, если, к примеру, мы должны передавать какой-то сложный объект. Например, свойство Padding, 
которое устанавливает внутренние отступы страницы, принимает объект Thickness:</p>
<pre class="brush:xml;">
&lt;ContentPage.Padding&gt;
...
<p>Правда, в данном случае надо заметить, что настройки для Windows и Android принимают значения по умолчанию, то есть нулевые отступы от каждой стороны страницы. 
Изменения касаются лишь iOS. Поэтому в данном случае мы можем сократить установку Padding:</p>
<pre class="brush:xml;">
&lt;ContentPage.Padding&gt;
    &lt;OnPlatform x:TypeArguments="Thickness"
         iOS="0,20,0,0"
