...
<html >
<head>
<title>Xamarin Forms | BindableObject и BindableProperty</title>
<meta charset="utf-8" />
<meta name="description" content="Роль классов BindableObject и BindableProperty в механизме привязки в Xamarin Forms">
...
<h2>BindableObject и BindableProperty</h2><div class="date">Последнее обновление: 12.01.2021</div>
...
<p>Для поддержки привязки данных Xamarin Forms определяет класс <span class="b">BindableObject</span>. Все остальные визуальные элементы, как кнопки, метки, текстовые поля, контейнеры компоновки и так далее 
наследуются от этого класса.</p>
<p>Отличительной особенностью класса BindableObject является то, что он содержит специальные типы свойств <span class="b">BindableProperty</span>. 
...
которые имеют похожее назначение.</p>
<p>Например, в Xamarin Forms есть класс Label, который является потомком класса BindableObject и у которого есть свойство <code>Text</code>. Через 
это свойство мы можем установить текст метки. Но в реальности это свойство выглядит следующим образом:</p>
<pre class="brush:c#;">
...
<p>Как правило, для каждого свойства BindableProperty создается обертка-обычное свойство. И название BindableProperty обычно имеет название обычного свойства + суффикс 
<code>Property</code>. Например, Text и TextProperty или TextColor и TextColorProperty.</p>
<p>Поэтому, если, допустим, у нас есть элемент Label, и мы хотим присвоить ему некоторый текст, то мы можем сдеать это двумя способами:</p>
<pre class="brush:c#;">
...
<p>Для установки значения свойства через BindableProperty у объекта BindableObject вызывается метод <code>SetValue()</code>. В качестве первого 
параметра в метод передается само свойство (то есть в данном случае Label.TextProperty). Второй параметр передает значение для этого свойства.</p>
<p>Аналогично для получения значения свойства мы также можем применять два способа:</p>
<pre class="brush:c#;">
...
<p>Таким образом, определяются BindableObject и BindableProperty.</p>
<h3>Создание свойства BindableProperty</h3>
<p>Допустим, мы хотим определить свое свойство BindableProperty в каком-то своем классе. Например, мы хотим расширить функционал класса Label, чтобы он включал некоторый 
тег, который присваивается метке. Для этого своздадим свой класс, производный от Label:</p>
<pre class="brush:c#;">
...
<p>Данный класс располагается в главном проекте решения.</p>
<p>Для определения свойства BindableProperty используется метод <span class="b">BindableProperty.Create()</span>. Этот метод возвращает объект 
BindableProperty и принимает в данном случае четыре параметра по порядку:</p>
<ul>
<li><p>Имя обычного свойства, которое будет оберткой. В данном случае свойство будет называться "Tag"</p></li>
<li><p>Возвращаемый тип свойства. В данном случае тип string</p></li>
<li><p>Название типа, в котором объявляется это свойство. Здесь тип TagLabel</p></li>
<li><p>Значение по умолчанию. Здесь строка "0"</p></li>
</ul>
<p>Это не все возможные параметры. Другие перегруженные версии метода BindableProperty.Create() могут принимать еще шесть параметров по порядку:</p>
<ul>
<li><p><code>defaultBindingMode</code> - режим привязки</p></li>
<li><p><code>validateValue</code> - метод, который проверяет новое значение на корректность</p></li>
<li><p><code>propertyChanged</code> - метод, который вызывается при изменении свойства</p></li>
<li><p><code>propertyChanging</code> - метод, который вызывается перед изменением свойства</p></li>
<li><p><code>coerceValue</code> - метод корректировки нового значения</p></li>
<li><p><code>defaultValueCreator</code> - метод-генератор значения по умоланию</p></li>
</ul>
<p>После определения класса и свойства они могут участвовать в привязке данных. Так, пусть у нас будет следующий код страницы:</p>
<pre class="brush:c#;">
...
<p>Итак, здесь объект нашего класса TagLabel привязан к объекту entry. Причем сразу два свойства - Text и Tag у объекта TagLabel привязаны к 
свойству Text объекта Entry.</p>
<p>Также здесь есть простой объект Label, свойство Text которого привязано к свойству Tag объекта TagLabel. Поэтому при вводе символов в текстовое поле Entry 
синхронно будет изменяться значение свойства Tag у tagLabel, а это в свою очередь вызовет изменение свойства Text у простого объекта Label.</p>
<img src="./pics/3.3.png" alt="Объект BindableObject и BindableProperty в Xamarin" />
<p>Таким образом, определив свое свойство по типу BindableProperty впоследствии мы сможем осуществлять к нему привязку.</p>
<p>Определение аналогичной функциональности в XAML-коде:</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./4.1.php">Назад</a><a href="./">Содержание</a><a href="./4.5.php">Вперед</a></p></div></div>
<div class="socBlock">
