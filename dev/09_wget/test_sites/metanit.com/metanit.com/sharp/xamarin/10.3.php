...
<html >
<head>
<title>Xamarin Forms | Класс HttpClient и отправка запросов</title>
<meta charset="utf-8" />
<meta name="description" content="Класс HttpClient и отправка запросов в Xamarin Forms">
...
<h2>Класс HttpClient и отправка запросов</h2><div class="date">Последнее обновление: 13.06.2018</div>
...
<p>Для взаимодействия с веб-сервисами в .NET используется класс <span class="b">HttpClient</span> из пространства имен <span class="b">System.Net.Http</span>. 
С помощью его методов можно посылать определенный запрос к серверу.</p>


<h3>Работа с HttpClient</h3>
<p>HttpClient предоставляет следующие методы:</p>
<ul>
<li><p><span class="b">GetAsync()</span>: отправляет запрос типа Get</p></li>
<li><p><span class="b">DeleteAsync()</span>: отправляет запрос типа Delete</p></li>
<li><p><span class="b">PostAsync()</span>: отправляет запрос типа Post</p></li>
<li><p><span class="b">PutAsync()</span>: отправляет запрос типа Put</p></li>
<li><p><span class="b">SendAsync()</span>: отправляет запрос любого типа в зависимости от настроек заголовков запроса. Выше определенные методы 
являются частными случаями данного метода</p></li>
<li><p><span class="b">GetByteArrayAsync()</span>: отправляет массив байтов в запросе типа Get</p></li>
<li><p><span class="b">GetStreamAsync()</span>: отправляет поток Stream в запросе типа Get</p></li>
<li><p><span class="b">GetStringAsync()</span>: отправляет строку в запросе типа Get</p></li>
</ul>

<p>При отправке запроса для его настройки мы можем применять объект класса <span class="b">HttpRequestMessage</span>. В частности, для конфигурации запроса 
можно использовать следующие свойства класса:</p>
<ul>
<li><p><span class="b">Content</span>: отправляемые данные</p></li>
<li><p><span class="b">Headers</span>: заголовки http</p></li>
<li><p><span class="b">Method</span>: метод отправки(Get/Post/Put/Delete)</p></li>
<li><p><span class="b">RequestUri</span>: адрес сервера</p></li>
</ul>
<p>Например:</p>
<pre class="brush:c#;">
...
<p>После выполнения запроса мы можем получить ответ в виде объекта класса <span class="b">HttpResponseMessage</span>. Для обработки ответа мы можем воспользоваться 
следующими свойствами данного класса:</p>
<ul>
<li><p><span class="b">Content</span>: полученные данные</p></li>
<li><p><span class="b">Headers</span>: заголовки ответа, полученные от сервера</p></li>
...
<p>Если сервер отправляет нам данные в виде json, то, получив данные в виде строки с помощью метода <code>responseContent.ReadAsStringAsync()</code>, мы 
можем их десериализовать из json с помощью специальных инструментов или библиотек, как Json.NET.</p>
<p>Используя методы <code>GetAsync()/PostAsync()/PutAsync()/DeleteAsync()</code> соответственно уже не надо указывать метод запроса. Например:</p>
<pre class="brush:c#;">
HttpClient client = new HttpClient();
...
<p>Если сервер посылает ответ в виде простой строки, массива байт или файла, то в данном случае проще использовать методы 
<code>GetStringAsync()/GetByteArrayAsync()/GetStreamAsync()</code>. Например, получим от сервера строку:</p>
<pre class="brush:c#;">
HttpClient client = new HttpClient();
string response = await client.GetStringAsync("http://somesite.com");
</pre>
<p>В то же время в данном случае мы не сможем отследить статусный код ответа от сервера.</p>
<h3>Отправка и получение данных</h3>
<p>Данные запроса и ответа представлены в виде объекта класса <span class="b">HttpContent</span>. Этот класс является абстрактным, и фактически мы будем работать 
с его производными классами:</p>
<ul>
<li><p><span class="b">StringContent</span>: применяется, если отправка или получение данных происходит в виде строки</p></li>
<li><p><span class="b">ByteArrayContent</span>: применяется, если отправка или получение данных происходит в виде массива байтов</p></li>
<li><p><span class="b">StreamContent</span>: применяется, если отправка или получение данных происходит, как правило, в виде файла, например, изображения</p></li>
</ul>
<p>Если мы хотим получить данные в виде строки, массива байт или файла, то соответственно мы можем использовать один из методов <code>GetStringAsync()/GetByteArrayAsync()/GetStreamAsync()</code>, 
либо можно обрабатывать свойство Content объекта HttpResponseMessage.</p>
<p>Для отправки данных мы можем задействовать свойство Content объекта HttpRequestMessage:</p>
<pre class="brush:c#;">
...
<p>Либо в качестве альтернативы для отправки данных можно использовать специальные методы <code>PostAsync()/PutAsync()</code>:</p>
<pre class="brush:c#;">
var friend = new Friend { Id = 1, Name = "Иван Иванов" };
// сериализация объекта с помощью Json.NET
string json = JsonConvert.SerializeObject(friend);
...
<div class="nav"><p><a href="./10.2.php">Назад</a><a href="./">Содержание</a><a href="./10.1.php">Вперед</a></p></div></div>
<div class="socBlock">
