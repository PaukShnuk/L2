...
<html >
<head>
<title>Xamarin Forms | Команды и взаимодействие с пользователем в MVVM</title>
<meta charset="utf-8" />
<meta name="description" content="Использование команд для взаимодействия с пользовательским интерфейсом в MVVM в Xamarin Forms">
...
<h2>Команды и взаимодействие с пользователем в MVVM</h2><div class="date">Последнее обновление: 13.01.2021</div>
...
<p>В прошлой теме описывался паттерн MVVM, позволяющий отображать связанные данные. Однако, как правило, необходимо не только отображать данные, но 
...
паттерна MVVM.</p>
<p>Ключевой идеей паттерна является взаимодействие с моделью через ViewModel, то есть в данном случае использование событий визуальных компонентов и их 
обработчиков нежелательно. Чтобы решить эту задачу, в платформе Xamarin Forms имеется механизм команд.</p>
<p>Механизм команд раскрывается через интерфейс <span class="b">ICommand</span>:</p>
<pre class="brush:c#;">
...
<p>Интерфейс ICommand определен в пространстве имен <code>System.Windows.Input</code> и расположен в сборке <code>System.ObjectModel</code>.</p>
<p>Встроенную поддержку команд имеют следующие элементы управления:</p>
<ul>
<li><p><code>Button</code></p></li>
<li><p><code>MenuItem</code></p></li>
<li><p><code>ToolbarItem</code></p></li>
<li><p><code>SearchBar</code></p></li>
<li><p><code>TextCell</code></p></li>
<li><p><code>ImageCell</code></p></li>
<li><p><code>ListView</code></p></li>
<li><p><code>TapGestureRecognizer</code></p></li>
</ul>
<p>Итак, для применения команд возьмем проект приложения из прошлой темы и определим в нем простейшую команду. Для этого добавим в проект класс 
<span class="b">IncreasePriceCommand</span>:</p>
<pre class="brush:c#;">
using System;
...
<p>Команда через конструктор принимает объект PhoneViewModel. Метод <code>CanExecute</code> возвращает true, если у цена во viewModel меньше 100000. Здесь можно определить любое условие. 
В прицнипе данный метод вообще не обязательно использовать.</p>
<p>Метод <code>Execute()</code> собственно выполняет действие команды. В нашем случае будет вызываться метод IncreasePrice, который мы определим в ViewModel. Стоит отметить, что метод принимает 
параметр типа object, то есть мы можем извне передавать в команду различные данные.</p>
<p>Теперь изменим класс PhoneViewModel, чтобы он использовал данную команду:</p>
<pre class="brush:c#;">
...
<p>ViewModel содержит команду Increase, которая представляет тип IncreasePriceCommand.</p>
<p>И также изменим код страницы в xaml, добавив в него кнопки с привязкой к вышеопределенным командам:</p>
<pre class="brush:xml;">
...
<p>С помощью свойства Command и выражения привязки кнопка связывается с командой Increase. И при нажатии на кнопку происходит действие, заложенное в связанной команде, то есть произойдет увеличение цены товара.</p>
<img src="./pics/3.26.png" alt="Команды в Xamarin Forms" />
<p>Также стоит отметить, что если мы хотим использовать команды, то нам необязательно определять новый класс команды. В Xamarin.Forms есть встроенный класс Command, 
...
изменим класс PhoneViewModel следующим образом:</p>
<pre class="brush:c#;">
using System.ComponentModel;
using System.Windows.Input;
using Xamarin.Forms;
...
<p>И приложение будет также работать.</p>
...
<div class="nav"><p><a href="./4.2.php">Назад</a><a href="./">Содержание</a><a href="./11.1.php">Вперед</a></p></div></div>
<div class="socBlock">
