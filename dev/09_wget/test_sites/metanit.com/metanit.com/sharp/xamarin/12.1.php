...
<html >
<head>
<title>Xamarin Forms | Платформо-зависимый код. Класс Device</title>
<meta charset="utf-8" />
<meta name="description" content="Класс Device в Xamarin Forms, RuntimePlatform и определение кода, специфического для каждой платформы">
...
<h1>Платформо-зависимый код</h1><h2>Класс Device</h2><div class="date">Последнее обновление: 11.01.2021</div>
...
<p>Несмотря на то, что проект Xamarin Forms позволяет обеспечить единую логику приложения и единый графический интерфейс, иногда возникает необходимость 
подстроить некоторые аспекты под конкретную операционную систему. Одним из механизмов, который позволяет это сделать, представляет класс <span class="b">Device</span>.</p>
<h3>Свойство RuntimePlatform</h3>
<p>Прежде всего, свойство <span class="b">RuntimePlatform</span> класса Device позволяет получить платформу, на которой запущено приложение. 
В качестве значения свойство хранит название платформы. В частности, это может быть одна из встроенных констант класса Device:</p>
<ul>
<li><p><code>Device.Android</code></p></li>
<li><p><code>Device.iOS</code></p></li>
<li><p><code>Device.UWP</code></p></li>
<li><p><code>Device.WPF</code></p></li>
<li><p><code>Device.Tizen</code></p></li>
<li><p><code>Device.GTK</code></p></li>
<li><p><code>Device.macOS</code></p></li>
</ul>
<p>Например, определим в коде метку, текст которой зависит от текущей пдатформы:</p>
<pre class="brush:c#;">
...
<h3>Свойство Idiom</h3>
<p>Свойство <span class="b">Idiom</span> класса Device позволяет проверить тип устройства. В качестве значения оно принимает 
одно из значений перечисления <span class="b">TargetIdiom</span>:</p>
<ul>
<li><p><code>TargetIdiom.Phone</code>: устройства iPhone, iPod и Android с шириной менее 600 dip</p></li>
<li><p><code>TargetIdiom.Tablet</code>: устройства iPad, Windows и Android с шириной более 600 dip</p></li>
<li><p><code>TargetIdiom.Desktop</code>: настольные компьютеры</p></li>
<li><p><code>TargetIdiom.TV</code>: телевизоры на платформе Tizen</p></li>
<li><p><code>TargetIdiom.Watch</code>: часы на платформе Tizen</p></li>
<li><p><code>TargetIdiom.Unsupported</code>: не поддерживается</p></li>
</ul>
<p>В зависимости от результатов проверки значения свойства можно выполнить тот или иной набор инструкций:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./3.21.php">Назад</a><a href="./">Содержание</a><a href="./12.2.php">Вперед</a></p></div></div>
<div class="socBlock">
