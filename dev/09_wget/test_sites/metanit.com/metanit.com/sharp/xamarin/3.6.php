...
<html >
<head>
<title>Xamarin Forms | Контейнер Grid</title>
<meta charset="utf-8" />
<meta name="description" content="Создание сетки элементов с помощью контейнера Grid в Xamarin Forms">
...
<h2>Контейнер Grid</h2><div class="date">Последнее обновление: 09.01.2021</div>
...
<p>Контейнер <span class="b">Grid</span> располагает вложенные элементы в виде таблицы:</p>
<pre class="brush:c#;">
...
<img src="./pics/2.19.png" alt="Grid in Xamarin" />
<p>С помощью свойств <code>RowDefinitions</code> и <code>ColumnDefinitions</code> грид создает набор строк и столбцов соответственно. Так, 
здесь задается три строки и три столбца, то есть в итоге 9 ячеек.</p>
<P>Для высоты строк и ширины столбцов задается значение <code>new GridLength(1, GridUnitType.Star)</code>. Число 1 указывает на то, что 
...
условно будет равно 1+1+1=3, а одна строка или столбец будет занимать 1/3). А параметр <code>GridUnitType.Star</code> как раз указывает, что размеры будут 
вычисляться пропорционально.</P>
<p>Чтобы добавить элемент в определенную ячейку, мы указываем номер строки и столбца: <code>grid.Children.Add(new BoxView { Color = Color.Blue }, 0, 1);</code> - 
...
поэтому 1 будет представлять второй столбец или строку)</p>
<p>Употребление Grid в xaml:</p>
<pre class="brush:xml;">
...
<p>Использование звездочки в xaml аналогично параметру <code>GridUnitType.Star</code>. Но кроме пропорциональных размеров мы можем задать 
автоматические размеры или абсолютные размеры, например:</p>
<pre class="brush:xml;">
&lt;Grid&gt;
  &lt;Grid.RowDefinitions&gt;
...
<p>Аналогичное определение разных типов размеров в коде C#:</p>
<pre class="brush:c#;">
Grid grid = new Grid
{
    RowDefinitions =
...
<h3>Отступы</h3>
<p>Класс Grid определяет два специальных свойства для создания отступов:</p>
<ul>
<li><p><span class="b">ColumnSpacing</span>: определяет пространство между столбцами</p></li>
<li><p><span class="b">RowSpacing</span>: определяет пространство между строками</p></li>
</ul>
<p>Например:</p>
<pre class="brush:xml;">
...
<p>С помощью свойства <span class="b">ColumnSpan</span> можно объединить несколько столбцов, а с помощью свойства <span class="b">RowSpan</span> - 
объединить ячейки:</p>
<pre class="brush:c#;">
...
<img src="./pics/2.62.png" alt="Объединение ячеек в гриде Xamarin Forms" />
...
<div class="nav"><p><a href="./3.5.php">Назад</a><a href="./">Содержание</a><a href="./3.17.php">Вперед</a></p></div></div>
<div class="socBlock">
