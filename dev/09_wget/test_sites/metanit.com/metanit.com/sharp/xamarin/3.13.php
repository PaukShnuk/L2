...
<html >
<head>
<title>Xamarin Forms | Переключатель Switch</title>
<meta charset="utf-8" />
<meta name="description" content="Создание переключателя checkbox с помощью элемента switch в Xamarin Forms">
...
<h2>Переключатель Switch</h2><div class="date">Последнее обновление: 10.01.2021</div>
...
<p>Элемент <span class="b">Switch</span> представляет переключатель, который может находиться в двух состояниях: включенном и выключенном.</p>
<p>Среди свойств класса Switch стоит выделить следующие:</p>
<ul>
<li><p><span class="b">IsToggled</span>: указывает, находится ли Switch во включенном состоянии (значение true) или 
выключенном (значение false)</p></li>
<li><p><span class="b">ThumbColor</span>: цвет кнопки переключателя</p></li>
<li><p><span class="b">OnColor</span>: цвет переключателя во включенном состоянии</p></li>
</ul>
<p>Создадим элемент Switch s коде C#:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
using System;
...
<p>Если надо установить переключатель в определенное состояние, то применяется свойство <code>IsToggled</code>. По умолчанию оно имеет значение false. Чтобы отследить смену состояния, 
мы можем обработать событие <code>Toggled</code></p>
<img src="./pics/2.33.png" alt="Switch и checkbox в Xamarin Forms" />
<p>Аналог в xaml:</p>
<pre class="brush:xml;">
 &lt;StackLayout&gt;
