...
<html >
<head>
<title>Xamarin Forms | Создание класса ячейки для ListView</title>
<meta charset="utf-8" />
<meta name="description" content="Создание класса ячейки  на основе ViewCell для ListView в Xamarin Forms">
...
<h2>Создание класса ячейки для ListView</h2><div class="date">Последнее обновление: 12.01.2021</div>
...
<p>Если визуальное представление, используемое для вывода данных в ListView, повторяется и применяется в отдельных частях приложения, то мы можем 
вынести это представление в отдельный класс, который унаследован от ViewCell.</p>
<p>Например, нам бы хотелось, чтобы каждая ячейка была наподобие ImageCell, только с возможностью устанавливать высоту и ширину изображения.</p>
<p>Для этого добавим в проект новый класс CustomCell, который будет представлять отдельную ячейку:</p>
<pre class="brush:c#;">
...
<p>Ячейка будет состоять из двух элементов Label, расположенных вертикально, и элемента Image.</p>
<p>Для получения данных извне и привязки данных к элементам Label и Image определены свойства BindableProperty.</p>
<p>Для обработки изменения контекста привязки переопределен метод <code>OnBindingContextChanged</code>.</p>
<p>Определим в коде C# страницу, которая будет выводить список:</p>
<pre class="brush:c#;">
...
<img src="./pics/3.20.png" alt="ViewCell in Xamarin.Forms" />
<p>Использование CustomCell в xaml:</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./4.13.php">Назад</a><a href="./">Содержание</a><a href="./4.9.php">Вперед</a></p></div></div>
<div class="socBlock">
