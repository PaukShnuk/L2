...
<html >
<head>
<title>Xamarin Forms | ObservableCollection</title>
<meta charset="utf-8" />
<meta name="description" content="Динамическое добавление и удаление объектов в списке в Xamarin Forms, применение коллекции ObservableCollection">
...
<h2>ObservableCollection</h2><div class="date">Последнее обновление: 12.01.2021</div>
...
<p>При работе со списком объектов мы можем столкнуться с проблемой добавления или удаления объектов в этом списке. Так, возьмем пример из прошлой темы и 
добавим в код визуального интерфейса пару кнопок, которые будут выполнять добавление и удаление:</p>
<pre class="brush:xml;">
...
<p>В файл кода C# добавим к странице обработчики кнопок:</p>
<pre class="brush:c#;">
using System;
...
<p>В данном случае типом коллекции является стандартный класс List, который поддерживает добавление и удаление с помощью методов <code>Add()</code> и <code>Remove()</code>. 
...
Хотя в реальности коллекция Phones будет изменяться. Более того, мы можем столкнуться с исключением.</p>
<p>Чтобы решить эту проблемы в качестве типа коллекции, как правило, используется не класс List, а класс <span class="b">ObservableCollection</span> из 
пространства имен <code>System.Collections.ObjectModel</code>. За счет реализации интерфейса <code>INotifyCollectionChanged</code> при добавлении или удалении 
объектов в ObservableCollection автоматически будут изменяться все привязанные к этой коллекции объекты, в том числе и ListView.</p>
<p>Итак, изменим определение коллекции Phones:</p>
<pre class="brush:c#;">
...
<p>И теперь у нас не возникнет проблем с добавлением или удалением.</p>
<img src="./pics/3.9.png" alt="ObservableCollection в Xamarin Forms" />
...
<div class="nav"><p><a href="./4.8.php">Назад</a><a href="./">Содержание</a><a href="./4.15.php">Вперед</a></p></div></div>
<div class="socBlock">
