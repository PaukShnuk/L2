...
<html >
<head>
<title>Razor Pages и C# | Модель страницы Razor</title>
<meta charset="utf-8" />
<meta name="description" content="Модель страницы Razor в проекте ASP.NET Core и C#, директива model и подключение класса модели, свойство Model, класс PageModel">
...
<h2>Модель страницы Razor</h2><div class="date">Последнее обновление: 14.04.2022</div>
...
<p>Вместе со страницей Razor в проект добавляется файл связанного кода на языке C#. Этот код определяется в виде класса, который по умолчанию называется по имени 
страницы плюс суффикс "Model" (например, для страницы Index - класс IndexModel) и который помещается в файл с именем 
<span class="b">[файл_страницы_razor].cs</span>.</p>
<img src="./pics/2.11.png" alt="Model in Razor Page в ASP.NET Core и C#" />
<p>Например, при добавлении страницы <span class="b">Index.cshtml</span> вместе с ней будет добавляться файл <span class="b">Index.cshtml.cs</span>, 
который будет содержать определение класса <span class="b">IndexModel</span></p>
<pre class="brush:c#;">
...
<p>Класс модели страницы Razor должен обязательно наследоваться от абстрактного класса <span class="b">PageModel</span>. 
По умолчанию данный класс содержит пустой метод <span class="b">OnGet()</span>, который призван обрабатывать get-запросы. Здесь мы можем определить какие-то данные, какую-то логику, которая будет применяться на 
странице razor.</p>
<p>К пример, изменим этот класс следующим образом:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc.RazorPages;
...
<p>В данном случае класс IndexModel определяет свойство Message и метод PrintTime, который возвращает текущее время.</p>
<p>Для подключения класса-модели на страницу Razor применяется директива <span class="b">@model</span>, после которой идет имя класса. Например, изменим определение страницы 
...
<p>Обратите внимание, что после директивы <code>@model</code> указывается полное имя класса с учетом пространства имен.</p>
<p>После подключения модели мы можем обращаться к ее функционалу через свойство <span class="b">Model</span>. Например, с помощью выражения <code>@Model.Message</code> мы 
можем обратиться к свойству Message в модели.</p>
<p>В итоге при обращении к странице Index и генерации ответа будет использоваться функционал модели IndexModel:</p>
<img src="./pics/2.12.png" alt="Подключение модели на странице Razor Page в ASP.NET Core и C#" />
