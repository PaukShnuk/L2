...
<html >
<head>
<title>Razor Pages и C# | Добавление и получение данных из базы данных</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление и получение данных из базы данных через Entity Framework в Razor Pages в ASP.NET Core и C#">
...
<h2>Добавление и получение данных из базы данных</h2><div class="date">Последнее обновление: 18.04.2022</div>
...
<p>Продолжим работу с проектом из прошлой темы, где уже настроено подключение и создана база данных. Теперь определим в проекте в папке 
<span class="b">Pages</span> новую страницу Razor, которую назовем <span class="b">Index.cshtml</span>. После ее добавления в папке <span class="b">Pages</span> появятся два файла: 
<span class="b">Index.cshtml</span> и <span class="b">Index.cshtml.cs</span>. Эта страница будет предназначена для вывода список пользователей из базы данных.</p>
<p>Изменим файл <span class="b">Index.cshtml.cs</span> следующим образом:</p>
<pre class="brush:c#;">
...
<p>Поскольку в прошлой статье контекст данных ApplicationContext был добавлен в приложение в качестве сервиса, то в конструкторе модели страницы мы можем получить этот контекст и затем использовать его для взаимодействия с базой данных.</p>
...
<p>Теперь аналогичным образом добавим в папку <span class="b">Pages</span> еще одну страницу Razor - <span class="b">Create.cshtml</span>. И вначале изменим 
файл <span class="b">Create.cshtml.cs</span> следующим образом:</p>
<pre class="brush:c#;">
...
<p>Класс CreateModel также через конструктор получает контекст данных для последующего добавления объекта в БД. Сам добавляемый объект определен в виде свойства Person, к которому применяется 
атрибут <span class="b">[BindProperty]</span>. А это значит, что система автоматически будет связывать данные запроса со свойствами этого объекта.</p>
<p>В методе <span class="b">OnPostAsync()</span> происходит добавление объекта в базу данных, после чего идет редирект на страницу Index.</p>
<p>И поскольку данные будут отправляться из формы в запросе Post, чтобы избежать необходимости устанавливать antiforgery-токен, к классу CreateModel применяется атрибут <span class="b">[IgnoreAntiforgeryToken]</span></p>
...
<img src="./pics/4.2.png" alt="проект для для добавления данных в бд через Entity Framework в RazorPages в ASP.NET Core и C#" />
<p>Запустим проект. Вначале у нас нет объектов в бд, поэтому перейдем к странице Create.cshtml и добавим один объект:</p>
<img src="./pics/4.3.png" alt="Добавление в базу данных в Razor Pages и C#" />
<p>И после добавления страница Index.cshtml отобразит добавленный объект:</p>
<img src="./pics/4.4.png" alt="Вывод объектов из базы данных в Razor Pages и C#" />
