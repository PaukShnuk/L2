...
<html >
<head>
<title>Razor Pages и C# | Подключение к базе данных</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение к базе данных через Entity Framework в Razor Pages в ASP.NET Core и C#, настройка подключения и создание бд">
...
<h1>Работа с базой данных через Entity Framework</h1><h2>Подключение к базе данных в Razor Pages</h2><div class="date">Последнее обновление: 18.04.2022</div>
...
<p>Для работы с базой данных один из распространенных подходов в ASP.NET Core является использование фреймворка Entity Framework. 
<span class="b">Entity Framework</span> представляет ORM-решение, которое позволяет автоматически связать обычные классы языка C# с таблицами в базе данных. Entity Framework Core 
...
Пусть у нас есть пустой проект для ASP.NET Core по типу <span class="b">ASP.NET Core Empty</span>.</p>
<p>Прежде всего для взаимодействия с MS SQL Server через Entity Framework добавим в проект через Nuget пакет <span class="b">Microsoft.EntityFrameworkCore.SqlServer</span>.</p>
<img src="./pics/4.1.png" alt="добавление Entity Framework Core в проект razor pages и C#" />
<p>Создадим в проекте новую папку <span class="b">Models</span>, в которую затем добавим класс <span class="b">User</span> со следующим кодом:</p>
<pre class="brush:c#;">
...
<p>Этот класс определяет данные, с которыми мы далее будем работать.</p>
<p>И также добавим в папку <span class="b">Models</span> класс контекста данных <span class="b">ApplicationContext</span>:</p>
<pre class="brush:c#;">
...
<p>Свойство <span class="b">DbSet</span> представляет собой коллекцию объектов, которая сопоставляется с определенной таблицей в базе данных. При этом по умолчанию название 
...
название класса модели представляет единственное число, а Users - множественное число.</p>
...
<p>Чтобы подключаться к базе данных, нам надо задать параметры подключения. Для этого изменим файл appsettings.json, добавив в него определение строки подключения::</p>
<pre class="brush:js;">
{
  "ConnectionStrings": {
...
<p>В данном случае мы будем использовать упрощенный движок базы данных LocalDB, который представляет легковесную версию SQL Server Express, предназначенную специально для разработки приложений.</p>
<p>И в конце подключим контекст данных ApplicationContext в приложение в виде сервиса и для этого изменим файл <span class="b">Program.cs</span>:</p>
<pre class="brush:c#;">
using Microsoft.EntityFrameworkCore;
...
<p>С помощью метода <span class="b">AddDbContext</span> класс контекста добавляется в приложение в виде сервиса, и у него устанавливается строка подключения. 
Добавление контекста данных в виде сервиса позволит затем получать его в конструкторе контроллера через механизм внедрения зависимостей.</p>
