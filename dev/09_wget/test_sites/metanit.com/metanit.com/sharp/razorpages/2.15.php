...
<html >
<head>
<title>Razor Pages и C# | ViewBag и ViewData</title>
<meta charset="utf-8" />
<meta name="description" content="Передача данных на страницу Razor Pages в ASP.NET Core и C# с помощью ViewBag и ViewData">
...
<h2>ViewBag и ViewData</h2><div class="date">Последнее обновление: 18.04.2022</div>
...
<p>Для передачи данных из кода c# на страницу Razor можно использовать ряд способов. Самый распространенный и предпочтительный - использование свойств модели страницы. 
Тем не менее есть и другие способы, в частности, примение объектов <span class="b">ViewBag</span> и <span class="b">ViewData</span>.</p>
<h3>ViewData</h3>
<p><span class="b">ViewData</span> представляет словарь из пар ключ-значение.</p>
...
<p>В коде модели IndexModel в файле <span class="b">Index.cshtml.cs</span> определим следующий код:</p>
<pre class="brush:c#;">
...
<p>В данном случае в словаре ViewData определяется элемент с ключом "Message", значением которого является строка "Razor Pages on METANIT.COM". 
На странице <span class="b">Index.cshtml</span> получим это значение:</p>
<pre class="brush:xml;">
@page 
...
<p>Здесь получаем из словаря ViewData элемент с ключом "Message" и выводим его в заголовке.</p>
<img src="./pics/2.46.png" alt="Передача данных через ViewData на страницу Razor Pages в ASP.NET Core и C#" />
<p>Подобным образом через ViewData можно передавать и более комплексные данные, только стоит учитывать, что в этом случае может потрбеоваться приведение типов. 
Например, определим в коде модели передачу списка строк:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc.RazorPages;
...
<p>На странице Index.cshtml выведем данные из списка:</p>
<pre class="brush:xml;">
@page 
...
<img src="./pics/2.47.png" alt=" ViewData в Razor Pages в ASP.NET Core и C#" />
<p>Как видно, здесь потребовалось преобразование данных. С этой точки зрения использование свойств модели вместо ViewData было бы более предпочтительным.</p>
<p>При этом данные ViewData можно определять непосредственно на самой странице Razor:</p>
<pre class="brush:xml;">
@page 
...
<p>В конкретно данном случае применение ViewData не имеет большого смысла, посколько можно было бы просто определить переменную, тем не менее в ряде сценариев может потребоваться передать данные 
из страницы Razor в другие представления или страницы.</p>
<h3>ViewBag</h3>
<p><span class="b">ViewBag</span> во многом подобен ViewData. Он позволяет динамически определить различные свойства и присвоить им любое значение. Так, мы могли бы переписать предыдущий пример следующим образом::</p>
<pre class="brush:xml;">
@page 
...
<p>Стоит отметить, что свойство ViewBag можно использовать только на странице Razor, а в классе модели оно не доступно.</p>
<p>На странице Razor также можно получать все данные, переданные через ViewData, через <span class="b">ViewBag</span>:</p>
<pre class="brush:xml;">
@page 
...
<div class="nav"><p><a href="./2.14.php">Назад</a><a href="./">Содержание</a><a href="./3.1.php">Вперед</a></p></div></div>
<div class="socBlock">
