...
<html >
<head>
<title>Razor Pages и C# | Отправка статусных кодов</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка статусных кодов 404, 401, 403, 400, 200 в Razor Pages в ASP.NET Core и C#, методы StatusCode, NotFound, Unauthorized, BadRequest">
...
<h2>Отправка статусных кодов в Razor Pages</h2><div class="date">Последнее обновление: 17.04.2022</div>
...
<p>Нередко возникает необходимость отправить в ответ на запрос какой-либо статусный код. 
...
можно применять различные методы и классы результатов.</p>
<h3>StatusCodeResult</h3>
<p><span class="b">StatusCodeResult</span> позволяет отправить любой статусный код клиенту. Для создания этого результата используется метод <span class="b">StatusCode()</span>, в который передается отправляемый код статуса.</p>
...
<p>В файле <span class="b">Index.cshtml.cs</span> определим следующую модель:</p>
<pre class="brush:c#;">
...
<p>В данном случае, если значение параметра name не равно строке "admin", то посылаем в ответ пользователю статусный код 401, который говорит о том, что доступ запрещен. Иначе возвращаем 
страницу Razor.</p>
<p>На самой странице <span class="b">Index.cshtml</span> определим какое-нибудь простейшее содержимое для тестирования:</p>
<pre class="brush:c#;">
@page 
...
<p>То есть если странице не будет передан параметр name со значением "admin", то пользователь увидит сообщение об ошибке 401. Иначе же ему отобразиться заголовок из страницы Razor.</p>
<img src="./pics/2.42.png" alt="Отправка статусных кодов в Razor Pages и C#" />

<p>Подобным образом мы можем послать браузеру любой другой статусный код. Но для отдельных кодов статуса предназначены свои отдельные классы.</p>

<h3>UnauthorizedResult и UnauthorizedObjectResult</h3>
<p><span class="b">UnauthorizedResult</span> посылает код 401, уведомляя пользователя, что он не автризован для доступа к ресурсу:</p>
<pre class="brush:c#;">
public IActionResult OnGet(string? name)
{
...
<p>Для создания ответа используется метод <span class="b">Unauthorized()</span>.</p>
<p><span class="b">UnauthorizedObjectResult</span> также посылает код 401, только позволяет добавить в ответ некоторый объект с информацией об ошибке. Конструктор этого класса принимает произвольный объект, 
который отправляется клиенту в формате json:</p>
<pre class="brush:c#;">
public IActionResult OnGet(string? name)
{
    if(name != "admin") 
...
<h3>HttpNotFoundResult и HttpNotFoundObjectResult</h3>
<p><span class="b">NotFoundResult</span> и <span class="b">NotFoundObjectResult</span> посылает код 404, уведомляя браузер о том, что ресурс 
не найден. Второй класс в дополнении к статусному коду позволяет отправить доплнительную информацию, которая потом отобразится в браузере.</p>
<p>Объекты обоих классов создаются методом <span class="b">NotFound</span>. Для первого класса - это метод без параметров, для второго класса - метод, 
который в качестве параметра принимает отправляемую информацию. Например, используем NotFoundObjectResult:</p>
<pre class="brush:c#;">
public IActionResult OnGet(string? name)
{
...
<h3>BadResult и BadObjectResult</h3>
<p><span class="b">BadResult</span> и <span class="b">BadObjectResult</span> посылают код 400, что говорит о том, что запрос некорректный. 
Второй класс в дополнении к статусному коду позволяет отправить доплнительную информацию, которая потом отобразится в браузере.</p>
<p>Эти классы можно применять, например, если в запросе нет каких-то параметров или данные представляют совсем не те типы, которые мы ожидаем получить, и т.д.</p>
<p>Объекты обоих классов создаются методом <span class="b">BadRequest</span>. Для первого класса - это метод без параметров, для второго класса - метод, 
который в качестве параметра принимает отправляемую информацию:</p>
<pre class="brush:c#;">
public IActionResult OnGet(string? name)
{
...
<h3>OkResult и OkObjectResult</h3>
<p><span class="b">OkResult</span> и <span class="b">OkObjectResult</span> посылают код 200, уведомляя об успешном выполнении запроса. 
Первый класс просто отправляет статусный код 200:</p>
<pre class="brush:c#;">
public IActionResult OnGet(string? name)
{
...
<p>Второй класс в дополнении к статусному коду позволяет отправить доплнительную информацию, которая передается через конструктор и которая отправляется в формате json:</p>
<pre class="brush:c#;">
public IActionResult OnGet(string? name)
{
