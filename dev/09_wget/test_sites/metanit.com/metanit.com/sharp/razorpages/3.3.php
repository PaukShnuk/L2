...
<html >
<head>
<title>Razor Pages и C# | Введение в tag-хелперы</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в tag-хелперы в Razor Pages в ASP.NET Core и C#, роль файла _ViewImports.cshtml, добавление и удаление tag-хелперов, директива addTagHelper">
...
<h2>Введение в tag-хелперы</h2><div class="date">Последнее обновление: 18.04.2022</div>
...
<p>Для определения пользовательского интерфейса можно применять стандартные элементы html, а также конструкции Razor. Но кроме того для упрощения создания интерфейса 
ASP.NET Core предоставляет специальный инструмент, который называется <span class="b">tag-хелперы</span>. Tag-хелперы представляют функциональность, предназначенную для генерации 
...
<p>Использовать tag-хелперы довольно просто. Допустим, у нас в проекте в папке <span class="b">Pages</span> есть две страницы: <span class="b">Index.cshtml</span> 
и <span class="b">About</span></p>
<img src="./pics/3.10.png" alt="Подключение tag-хелперов в Razor Pages в ASP.NET Core и C#" />
<p>На странице <span class="b">Index.cshtml</span> определим следующий код:</p>
<pre class="brush:xml;">
@page
...
<p>Сначала на странице идет директива <span class="b">addTagHelper</span></p>
<pre class="brush:c#;">@addTagHelper *, Microsoft.AspNetCore.Mvc.TagHelpers</pre>
<p>Первый параметр директивы указывает на tag-хелперы, которые будут доступны на странице Razor, а второй параметр определяет 
...
<pre class="brush:xml;">&lt;a asp-page="About"&gt;О приложении&lt;/a&gt;</pre>
...
<img src="./pics/3.11.png" alt="Tag хелперы в Razor Pages ASP.NET Core и C#" />
<p>Данный хелпер создает ссылку, которая указывает на страницу About.cshtml. В итоге при запуске проекта вместа данного tag-хелпера будет 
сформирована гиперссылка, по нажатию на которую уйдет запрос на страницу About.cshtml:</p>
<img src="./pics/3.12.png" alt="добавление tag-хелперов на страницу Razor Pages в ASP.NET Core и C#" />

<h3>_ViewImports.cshtml и @addTagHelper</h3>
<p>Выше на странице Index.cshtml были подключены tag-хелперы. Но что, если нам надо подключить tag-хелперы на множество страниц Razor? Вместо того, чтобы прописывать директиву 
<span class="b">@addTagHelper</span> на каждой отдельной странице, мы можем подключить все хелперы разом. 
Для этого, как и для подключения различных пространств имен, применяется файл <span class="b">_ViewImports.cshtml</span>.</p>
<p>Итак, добавим в проект в папку <span class="b">Pages</span> новый файл <span class="b">_ViewImports.cshtml</span>:</p>
<img src="./pics/3.13.png" alt="_ViewImports.cshtml в ASP.NET Core MVC и C#" />
<p>В файле <span class="b">_ViewImports.cshtml</span> определим подключение tag-хелперов:</p>
<pre class="brush:c#;">@addTagHelper *, Microsoft.AspNetCore.Mvc.TagHelpers</pre>
<p>После этого из кода страницы <span class="b">Index.cshtml</span> можно удалить подключение tag-хелперов и оставить только создание ссылки:</p>
<pre class="brush:xml;">
@page
...
<p>Данная директива может быть полезной, если мы, например, захотим ограничить применение хелперов в какой-то одной странице или группе страниц Razor. 
Эту директиву также можно определять в файле <span class="b">_ViewImports.cshtml</span>.</p>
