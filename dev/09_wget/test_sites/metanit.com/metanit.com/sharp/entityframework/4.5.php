...
<html >
<head>
<title>Группировка в LINQ to Entities | C# Entity Framework 6</title>
<meta charset="utf-8" />
...
<h2>Группировка</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Чтобы сгруппировать данные по определенным параметрам используются оператор <span class="bb">group by</span> или метод <span class="bb">GroupBy()</span>. 
Например, сгруппируем модели телефонов по производителю:</p>
<pre class="brush:c#;">
...
<p>В данном случае критерием для объединения групп является название компании, то есть столбец Name из связанной таблицы Companies. Критерий, по которому проводится 
группировка, является ключом. Ключ мы можем получить через свойство <span class="bb">Key</span>, которое есть у группы.</p>
...
<p>Аналогично работает метод <code>GroupBy()</code>:</p>
<pre class="brush:c#;">
var groups = db.Phones.GroupBy(p=&gt;p.Company.Name);
</pre>
<p>Кроме свойства Key у группы есть метод <code>Count()</code>, который возвращает количество элементов в данной группе. Например, сформируем новый элемент, 
который будет содержать ключ группы и количество ее элементов:</p>
<pre class="brush:c#;">
var groups = from p in db.Phones
...
<p>В моем случае я получу следующий вывод:</p>
<pre>
