...
<html >
<head>
<title>Автоматизация Code First и EF Power Tools | Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Автоматизация создания классов и контекста в Code First в Entity Framework к уже существующей базой данных с помощью EF Power Tools ">
...
<h2>Автоматизация Code First и EF Power Tools</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Кроме вышеописанной функциональности Microsoft предлагает нам полезный инструмент, также призванный автоматизировать данный процесс. Этот инструмент называется 
<span class="bb">EF Power Tools</span>.</p>
<p>EF Power Tools представляет собой надстройку к Visual Studio. Эту надстройку, а также краткое описание можно найти на странице 
<a href="http://visualstudiogallery.msdn.microsoft.com/72a60b14-1581-4b9b-89f2-846072eff19d/" rel="nofollow">Entity Framework Power Tools </a>. Для установки достаточно нажать на этой веб-странице 
на кнопку "Загрузка" и дальше следовать инструкциям.</p>
<p>В то же время есть некоторые ограничения: надстройка Entity Framework Power Tools работает только в полных версиях Visual Studio. В экспресс же версиях не работает.</p>
<p>Итак, если у вас полнофункциональная версия Visual Studio 2012 или 2013, то вы можете нажать в окне Solution Explorer (Обозреватель решений) 
на проект правой кнопкой мыши и увидеть в контекстном меню пункт <b>Entity Framework</b>:</p>
<img alt="EF Power Tools" src="pics/2.17.png" />
<p>Выберем пункт <span class="bb">Entity Framework –&gt; Reverse Engineer Code First</span>. Затем откроется окно настройки подключения, где нам надо задать сервер и 
базу данных, с которой нам надо взаимодействовать:</p>
<img src="pics/2.18.png" />
<p>Я выбрал базу данных, которая была создана в прошлой теме.</p>
<p>Затем нажмем ОК. После этого в проект будет добавлена папка Models, в которой будут находиться все созданные классы. По умолчанию для каждой таблицы 
создается свой класс, и также генерируется класс контекста данных:</p>
<img src="pics/2.19.png" />
<p>Поскольку в моей базе данных была одна таблица Users, то автоматически был создан класс User, который отражает структуру таблицы:</p>
<pre class="brush:c#;">
using System;
...
<p>Контекст данных userstoredbContext:</p>
<pre class="brush:c#;">
...
<p>Он имеет два конструктора. Статический конструктор призван выполнить начальную инициализацию данных. В данном случае он ничего не выполняет.</p>
<p>Стандартный конструктор обращается к конструктору базового класса (то есть класса DbContext) и  передает ему название строки подключения 
(<code>base("Name=userstoredbContext")</code>).</p>
<p>Для взаимодействия с таблицей Users класс контекста имеет одноименное свойство <code>public DbSet&lt;User&gt; Users { get; set; }</code></p>
<p>И в методе <code>OnModelCreating</code> выполняются действия при создании моделей. В данном случае с помощью класса <code>UserMap</code> 
настраивается конфигурация связей между классами и базой данных.</p>
<p>Этот класс UserMap выполняет сопоставление таблиц и их столбцов с классами и их свойствами:</p>
<pre class="brush:c#;">
...
<p>Остальная работа с базой данных будет происходить также, как и при стандартном подходе Code First. То есть, если нам надо добавить в таблицу новый 
объект User, то мы пишем:</p>
<pre class="brush:c#;">
User p = new User { Name = "Имя", Age = 30 };
userstoredbContext.Users.Add(p);
userstoredbContext.SaveChanges();
</pre>
<p>Конечно, в реальности базы данных, как правило, обладают более сложной структурой, и поэтому структура генерируемых классов также будет сложнее 
класса User. Но на данном примере уже понятно, что мы можем значительно автоматизировать часть работы по созданию классов моделей.</p>
