...
<html >
<head>
<title>Миграции | Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Миграции базы данных в Entity Framework 6">
...
<h2>Миграции</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 27.09.2016</div>
...
<p>Миграции позоляют вносить изменения в базу данных при изменениях моделей и контекста данных. Так, пусть у нас есть следующая модель Phone и контекст данных:</p>
<pre class="brush:c#;">
...
<p>Мы можем использовать этот контекст данных для работы с БД, добавлять и удалять данные. Но в какой-то момент, возможно, нам захочется что-то изменить, 
например, добавить в модель Phone новое свойство:</p>
<pre class="brush:c#;">public class Phone
{
    public int Id { get; set; }
    public string Name { get; set; }
...
<p>И если мы попытаемся обратиться к базе данных после изменения модели, то мы получим ошибку:</p>
<img src="pics/3.18.png" alt="Миграции в Entity Framework 6" />
<p>Для решения этой проблемы нам надо изменить базу данных таким образом, чтобы она вновь соответствовала моделям и контексту данных. И для этого мы можем 
применить миграции.</p>
<p>Для этого в Visual Studio перейдем к окну <span class="b">Package Manager Console</span>, которое можно найти внизу VS. Если такого окна нет, то его можно открыть, 
перейдя к меню <span class="b">View</span>-&gt;<span class="b">Other Window</span>-&gt;<span class="b">Package Manager Console</span></p>
<p>Для добавления функционала миграций введем в это окно следующую команду:</p>
<pre>enable-migrations</pre>
<p>После ввода команды нажмем на Enter. И в результате выполнения данной команды в проект будет добавлена папка <span class="ii">Migrations</span>, 
в которой будут два файла: <span class="ii">Configration.cs</span> (содержит базовую конфигурацию миграций) и файл начальной миграции, название которого может отличаться. 
Файл начальной миграции устанавливает, как база данных определяется на данный момент. То есть в моем случае он будет выглядеть так:</p>
<pre class="brush:c#;">
...
<p>Далее выполним в Package Manager Console следующую команду:</p>
<pre>add-migration "AddCompanyMigration"</pre>
<img src="pics/3.19.png" alt="Добавение миграции Entity Framework в проект" />
<p>И после выполнения этой команды в папку Migrations будет добавлена новая миграция:</p>
<pre class="brush:c#;">
using System;
using System.Data.Entity.Migrations;
...
<p>В миграции определяются два метода: <code>Up()</code> и <code>Down()</code>. В методе Up с помощью вызова метода AddColumn добавляется новый столбец 
Company в уже имеющуюся таблицу dbo.Phones. Метод Down удаляет столбец на случай, если они существуют. Фактически эти методы равнозначны выражению ALTER 
в языке SQL, которое меняет структуру базы данных и ее таблиц.</p>
<p>И в завершении чтобы выполнить миграцию, применим этот класс, набрав в той же консоли команду:</p>
<pre>update-database</pre>
<p>Эта команда обновит базу данных, добавив в нее новый столбец. Причем данные, которые уже были в таблицы, сохранятся.</p>
<p>Если база данных уже используется в производстве, развернута на сервере, где бы не можем произвести миграции, то мы можем сгенерировать по миграции скрипт. 
Для этого надо ввести следующую команду:</p>
<pre>update-database -script</pre>
<p>В итоге в моем случае будет сгенерирован следующий скрипт SQL:</p>
<pre class="brush:sql;">
ALTER TABLE [dbo].[Phones] ADD [Company] [nvarchar](max)
INSERT [dbo].[__MigrationHistory]([MigrationId], [ContextKey], [Model], [ProductVersion])
VALUES (N'201609271310287_AddCompanyMigration', N'CodeFirstApp.PhoneContext',  0x1F8B0800000000000400CD57DB6EDB38107D2FB0FF207CC35C38C0E0000 , N'6.1.3-40302')
</pre>
<p>Затем данный скрипт можно выполнить для используемой базы данных.</p>
<h3>Методы миграций</h3>
<p>Основу миграции составляют ряд методов, которые позволяют удалять, добавлять столбцы и таблицы, изменять настройки столбцов и так далее. Основные методы:</p>
<ul>
<li><p><span class="b">CreateTable</span>: добавляет таблицу</p></li>
<li><p><span class="b">DropTable</span>: удаляет таблицу</p></li>
<li><p><span class="b">AddColumn</span>: добавляет столбец</p></li>
<li><p><span class="b">DropColumn</span>: удаляет столбец</p></li>
<li><p><span class="b">AlterColumn</span>: изменяет настройки столбца</p></li>
<li><p><span class="b">AddForeignKey</span>: добавляет внешний ключ</p></li>
<li><p><span class="b">DropForeignKey</span>: удаляет внешний ключ</p></li>
<li><p><span class="b">AddPrimaryKey</span>: добавляет первичный ключ</p></li>
<li><p><span class="b">DropPrimaryKey</span>: удаляет первичный ключ</p></li>
<li><p><span class="b">CreateIndex</span>: добавляет индекс</p></li>
<li><p><span class="b">DropIndex</span>: удаляет индекс</p></li>
<li><p><span class="b">CreateStoredProcedure</span>: создает хранимую процедуру</p></li>
<li><p><span class="b">DropStoredProcedure</span>: удаляет хранимую процедуру</p></li>
<li><p><span class="b">AlterStoredProcedure</span>: изменяет хранимую процедуру</p></li>
</ul>
<p>Далее от этих методов можно строить цепочки дополнительных методов. Например, создание таблицы:</p>
<pre class="brush:c#;">
CreateTable(
