...
<html >
<head>
<title>Database First | Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Использования подхода Database First в Entity Framework для подключения к уже существующей базе данных">
...
<h2>Database First</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Database First был первым подходом, который появился в Entity Framework. Данный подход во многом похож на Model First и подходит для тех случаев, 
когда разработчик уже имеет готовую базу данных. </p>
<p>Чтобы Entity Framework мог получить доступ к базе данных, в системе должен быть установлен соответствующий провайдер. Так, Visual Studio уже 
поддерживает соответствующую инфраструктуру для СУБД MS SQL Server. Для остальных СУБД, например, MySQL, Oracle и других надо устанавливать соответствующие провайдеры. 
Список провайдеров для наиболее распространенных СУБД можно найти на странице <a href="http://msdn.microsoft.com/ru-RU/data/dd363565.aspx" rel="nofollow">ADO.NET Data Providers</a>.</p>
<p>Итак, создадим новый проект по типу Console Application. Его функциональность будет той же самой, что и у предыдущих проектов, только подход к использованию Entity Framework будет 
отличаться.</p>
<p>После создания нового проекта, чтобы задействовать базу данных, нам надо ее иметь. Создадим новую бд или возьмем уже имеющуюся.</p>
<p>В Visual Studio в окне Solution Explorer нажмем на проект правой кнопкой мыши и выберем в <span class="bb">Add - &gt; New Item</span>. Далее в 
появившемся окне добавления нового элемента выберем <span class="bb">ADO.NET Entity Data Model</span>. Дадим новому компоненту какое-либо название, например, 
<span class="bb">User</span>:</p>
<img alt="Entity Data Model" src="pics/2.1.png" />
<p>После этого нам откроется окно мастера создания модели. Если вы работаете с Visual Studio 2013 с пакетом обновления SP2, SP3, то откроется следующее 
окно мастера модели:</p>
<img src="pics/2.2.png" />
<p>В нем нам надо выбрать опцию <span class="bb">EF Designer from database</span>.</p>
<p>Если нашей целевой средой является Visual Studio 2013 без пакетов обновлений, то окно мастера будет выглядеть следующим образом:</p>
<img alt="Database First" src="pics/2.12.png" />
<p>В этом случае надо выбрать пункт <span class="bb">Generate from database</span> (Создание модели по имеющейся базе данных).</p>
<p>Затем откроется окно следующего шага по созданию модели, на котором надо будет установить подключение к базе данных:</p>
<img alt="Database First подключение к бд" src="pics/2.13.png" />
<p>В выпадающем списке выберем одно из доступных подключений. Если в списке нет предпочтительных подключений, то можно нажать на кнопку 
<span class="bb">New Connection</span> и установить новое подключение.</p>
<p>Также внизу указывается название <b>контекста данных</b>, который будет использоваться для доступа к данным. По умолчанию у меня контекст имеет название 
<span class="bb">userstoredbEntities</span>. Можно изменить, а можно и оставить.</p>
<p>Выбрав подключение, переходим к следующему шагу. Если у нас Visual Studio 2013 без пакетов обновления, то будет предложено также выбрать версию Entity Framework. Выберем шестую версию:</p>
<img alt="Выбор версии Entity Framework" src="pics/2.14.png" />
<p>В версиях Visual Studio 2013 SP2, SP3 по умолчанию используется EF 6, поэтому этот шаг пропускается.</p>
<p>Далее Visual Studio извлекает всю информацию о базе данных:</p>
<img src="pics/2.15.png" />
<p>Раскроем узел Tables. Он отображает все таблицы, имеющиеся в базе данных. В моем случае имеется только одна таблица Users. Отметим все подузлы 
в ветке Tables.</p>
<p>В поле <span class="bb">Model Namespace</span> установим предпочтительное имя модели и нажмем Finish. После этого Entity Framework сгенерирует модель по  
базе данных и добавит ее в проект.</p>
<p>Visiual Studio отобразит нам схему модели. В моем случае в бд есть только одна таблица Users, поэтому на схеме отображается только одна сущность User:</p>
<img src="pics/2.3.png" />
<p>После выделения сущности в правом нижнем углу Visual Studio мы увидим свойства для этой сущности:</p>
<img src="pics/2.16.png" />
<p>Свойство Name в окне свойств указывает на класс, которым будет представлена данная сущность (то есть классом User). А свойство 
<span class="bb">Entity Set Name</span> указывает на имя набора объектов (то есть свойство DbSet контекста данных) - в данном случае Users.</p>
<p>И теперь определим минимальный код для получения данных в коде приложения:</p>
<pre class="brush:c#;">
