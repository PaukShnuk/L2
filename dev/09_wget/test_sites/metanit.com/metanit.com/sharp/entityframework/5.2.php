...
<html >
<head>
<title>Хранимые функции | Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Хранимые функции в Entity Framework, создание tvf-функций в базе данных и обращение к ним из кода на c#">
...
<h2>Хранимые функции</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Особое внимание при работе с sql-запросами представляют хранимые функции и процедуры. Рассмотрим вызов хранимой функции в приложении на C#.</p>
...
<p>Теперь в Visual Studio в окне Database Explorer откроем базу данных. 
Для этого нажмем в окне Database Explorer на кнопку <span class="bb">Connect to Database</span>:</p>
<img src="pics/5.1.png" />
<p>Выберем нужную базу данных. В моем случае это база данных phonesdb:</p>
<img src="pics/5.2.png" />
<p>После открытия базы данных в окне Database Explorer найдем узел Functions и нажмем на него правой кнопкой мыши. В появившемся контекстном меню выберем 
<span class="bb">Add New -&gt; Table-valued Function</span>:</p>
<img src="pics/5.3.png" />
<p>После этого Visual Studio генерирует и октрывает файл скрипта со следующим содержимым:</p>
<pre class="brush:sql;">
...
<p>С помощью выражения <code>CREATE FUNCTION [dbo].[GetPhonesByPrice]</code> создается новая функция GetPhonesByPrice. Далее после названия определяется список параметров. Пусть наша функция принимает 
только один параметр <code>@price</code>, который имеет тип <code>int</code>, то есть целочисленное значение.</p>
...
таблицы. В данном случае они совпадают с определением таблицы Phones. То есть таблица будет содержать объекты класса Phone.</p>
...
строки, у которых столбец Price содержит меньшее значение, чем в параметре @price.</p>
...
<p>После этого в окне Database Explorer в узле Functions появится подузел добавленной функции. И мы ее можем уже использовать. Но перед обращением к ней из кода 
c# мы ее протестируем, чтобы убедиться, что она работает как надо. Для этого нажмем на функцию правой кнопкой мыши и в появившемся меню выберем пункт <span class="b">Execute</span>:</p>
...
<p>Теперь похожим образом создадим новую функцию, которая будет вычислять сумму с учетом скидки. Код функции:</p>
<pre class="brush:sql;">
...
<p>Функция принимает в качестве параметра процент скидки, например, 10 %. И на выходе она возвращает таблицу из двух полей - названия модели и цены с учетом скидки.</p>
<p>Так как функция фактически будет возвращать новый объект с двумя свойствами - Name и Price, при этом Price уже имеет тип decimal, то нам нужен соответствующий класс C#. 
Добавим в проект следующий класс:</p>
<pre class="brush:c#;">
...
<p>И теперь результат функции мы можем получить следующим образом:</p>
<pre class="brush:c#;">
using(PhoneContext db = new PhoneContext())
{
	// скидка - 15%
