...
<html >
<head>
<title>Сопоставление операций с хранимыми процедурами | Entity Framework</title>
<meta charset="utf-8" />
<meta name="description" content="Сопоставление операций CRUD с хранимыми процедурами в Entity Framework 6">
...
<h2>Сопоставление операций Entity Framework с хранимыми процедурами</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Кроме определения отношений между сущностями Fluent API позволяет производить еще ряд действий, в частности, сопоставить стандартные 
операции Entity Framework, такие как добавление, обновление, удаление, с хранимыми процедурами. Подобная функциональность может быть 
необходима, если мы хотим переопределить эти операции, либо если у нас уже есть база данных с хранимыми процедурами, и нам надо их использовать 
вместо стандартного механизма Entity Framework.</p>
<p>Допустим, у нас есть следующая модель Phone:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>С помощью метода MapToStoredProcedures осуществляется сопоставление операции с определенной сущностью с хранимыми процедурами. Так, 
первый блок осуществляет сопоставление операции добавления сущности Phone с хранимой процедурой InsertPhone:</p>
<pre class="brush:c#;">
modelBuilder.Entity&lt;Phone&gt;()
    .MapToStoredProcedures(b =&gt; b.Insert(sp =&gt; sp.HasName("InsertPhone")
        .Parameter(pm =&gt; pm.Name, "Model")
...
<p>Название процедуры задается с помощью метода <code>HasName()</code>. С помощью метода <code>Parameter()</code> происходит сопоставление определенных 
свойств модели Phone с параметрами процедуры. Так, в данном случае свойство Name сопоставляется с параметром model, а свойство Price с параметром price. 
И еще один метод <code>Result</code> позволяет указать свойство модели, которое будет принимать результат процедуры. В данном случае это свойство Id, которое получает 
идентификатор добавленного в бд объекта.</p>
<p>Пусть у нас уже есть база данных и таблица Phones, которая соответствует модели Phone и которая определяется следующим скриптом:</p>
<pre class="brush:sql;">
...
<p>И теперь добавим к этой таблице хранимую процедуру (подробнее механизм добавления процедуры описан в статье <a href="//metanit.com/sharp/entityframework/5.3.php">Хранимые процедуры</a>):</p>
<pre class="brush:sql;">
...
<p>Во-первых, название процедуры - InsertPhone - соответствует тому названию, которое используется выше в коде контекста. Во-вторых, процедуры имеет 
все те параметры, которые используются в коде контекста.</p>
<p>Для добавления используется выражение INSERT, а для возвращения результата выражение <code>SELECT SCOPE_IDENTITY() AS id</code>.</p>
<p>При этом добавление в бд в коде c# будет происходить как обычно:</p>
<pre class="brush:c#;">
...
<p>Только теперь Entity Framework будет для добавления использовать не собственное выражение, а код хранимой процедуры.</p>
<p>Подобным образом можно создать и использовать хранимую процедуру для обновления объекта:</p>
<pre class="brush:sql;">
