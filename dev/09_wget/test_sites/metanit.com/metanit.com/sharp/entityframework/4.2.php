...
<html >
<head>
<title>Выборка и проекция в LINQ to Entities | Entity Framework 6</title>
<meta charset="utf-8" />
...
<h2>Выборка и проекция из базы данных</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для выборки применяется метод Where. Выберем из бд все модели, производитель которых - "Samsung":</p>
<pre class="brush:c#;">
...
<p>Для выборки одного объекта мы можем использовать метод <span class="bb">Find()</span>. Данный метод не является методом Linq, он определен у класса DbSet:</p>
<pre class="brush:c#;">Phone myphone = db.Phones.Find(3); // выберем элемент с id=3</pre>
<p>Но в качестве альтернативы мы можем использовать методы Linq <span class="bb">First()/FirstOrDefault()</span>. Они получают первый элемент выборки, который 
соответствует определенному условию. Использование метода <code>FirstOrDefault()</code> является более гибким, так как если выборка пуста, то он вернет значение null. 
...
<p>Теперь сделаем проекцию. Допустим, нам надо добавить в результат выборки название компании. Мы можем использовать метод <span class="bb">Include</span> 
для подсоединения к объекту связанных данных из другой таблицы: <code>var phones = db.Phones.Include(p=&gt;p.Company)</code>. Но не всегда нужны все свойства 
выбираемых объектов. В этом случае мы можем применить метод <span class="bb">Select</span> для проекции извлеченных данных на новый тип:</p>
<pre class="brush:c#;">
using(PhoneContext db = new PhoneContext())
{
...
<p>В итоге метод Select из полученных данных спроецирует новый тип. В даном случае мы получим данные анонимного типа, но это также может быть определенный пользователем тип. Например:</p>
<pre class="brush:c#;">
public class Model
{
