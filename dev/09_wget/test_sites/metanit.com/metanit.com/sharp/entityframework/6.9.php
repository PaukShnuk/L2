...
<html >
<head>
<title>Конвенции | Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Конфигурация моделей в Entity Framework 6 с помощью класса EntityTypeConfiguration, добавление конфигураций">
...
<h2>Конвенции</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 26.09.2016</div>
...
<p>По умолчанию в Entity Framework действует ряд условностей. В частности, таблицы должны называться по имени моделей во множественном числе и т.д. С помощью 
аннотаций данных и Fluent API мы можем переопределить это поведение. Однако Entity Framework позволяет не только просто переопределить это поведение, но и 
создать свои собственные условности или конвенции (conventions).</p>
<p>К примеру, возьмем следующие модели:</p>
<pre class="brush:c#;">
...
<p>Эти модели содержат свойство Name, которое в нашем случае обязательно должно быть установлено. И, возможно, мы захотим ограничить это свойство по длине строки. 
Для этого мы могли бы использовать атрибуты <code>[Required]</code> и <code>[MaxLength]</code>. Однако здесь мы сталкиваемся с практически однотипными настройками. 
И если моделей десяток и более, то установка атрибутов и последующее возможное изменение (например, изменение предельной длины строки) может занять некоторое время. 
И в этом случае удобнее использовать конвенции, то есть создать свои условности в отношении свойства Name.</p>
<p>Для этого определим следующий класс контекста данных:</p>
<pre class="brush:c#;">
...
<p>С помощью метода <code>modelBuilder.Properties()</code> мы можем установить конвенции для определенного рода свойств. Например, для текстовых свойств. 
Сами условности устанавливаются с помощью метода <code>Configure()</code>. Так, выражение:</p>
<pre class="brush:c#;">modelBuilder.Properties&lt;string&gt;().Configure(s =&gt; s.HasMaxLength(150));</pre>
<p>указывает, что все строковые столбцы в базе данных будут иметь максимальную длину в 150 символов.</p>
<p>Если необходимо установить правила для столбцов с определенным условием (например, с названием "Name"), то можно применить выражение <code>Where</code>:</p>
<pre class="brush:c#;">modelBuilder.Properties&lt;string&gt;().Where(s =&gt; s.Name == "Name")</pre>
<p>Важную роль играет порядок определения конвенций: все последующие конвенции переопределяют предыдущие. Так, в примере выше сначала для всех сток устанавливается 
максимальная длина в 150 символов, и только потом отдельно для свойства Name устанавливает максимальная длина в 30 символов. Если бы определение конвенций в данном случае поменялось бы местами, 
то правило для 150 символов переопределило бы правило для Name.</p>
<p>В итоге в базе данных для столбцов будут действовать описанные ограничения:</p>
<img src="pics/6.12.png" alt="Conventions in Entity Framework" />
<p>Подобных конвенций можно насоздавать множество. И чтобы сократить код метода <code>OnModelCreating()</code>, мы можем вынести все конвенции в отдельные классы.</p>
<p>Для этого нам надо определить классы, производные от класса <span class="b">Convention</span> из пространства имен 
<code>System.Data.Entity.ModelConfiguration.Conventions</code>:</p>
<pre class="brush:c#;">
...
<p>С помощью метода <code>modelBuilder.Conventions.Add()</code> объекты конвенций добавляются в контекст. И опять же здесь играет большое значение порядок 
определения конвенций: последующие переопределяют предыдущие.</p>
<h3>Переопределение существующих конвенций</h3>
<p>По умолчанию Entity Framework уже применяет ряд конвенций, в частности, конвенцию IdKeyDiscoveryConvention, которая требует наличия свойства под названием 
Id или [Название_модели]Id. Но, допустим, мы не хотим называть идентификатор id, а как-то по другому, например, "Key". В этом случае мы можем переопределить 
существующую конвенцию. Для этого создадим следующий класс:</p>
<pre class="brush:c#;">
using System;
...
<p>Далее применим эту конвенцию:</p>
<pre class="brush:c#;">
using System;
using System.Collections.Generic;
using System.Linq;
...
<p>В результате теперь вместо "Id" в качестве идентификатора глобально будет использоваться "Key".</p>
...
<div class="nav"><p><a href="6.8.php">Назад</a><a href="./">Содержание</a><a href="7.1.php">Вперед</a></p></div></div>
<div class="socBlock">
