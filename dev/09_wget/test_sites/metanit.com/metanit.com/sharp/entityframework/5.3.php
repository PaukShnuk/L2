...
<html >
<head>
<title>Хранимые процедуры | Entity Framework 6</title>
<meta charset="utf-8" />
...
<h2>Хранимые процедуры</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Работа с хранимыми процедурами во многом аналогична работе с функциями базы данных. Итак, возьмем бд из прошлой темы и добавим в нее новую процедуру. 
Для этого откроем подключение к базе данных в окне Database Explorer и найдем в его структуре узел 
<span class="bb">Stored Procedures</span> (Хранимые процедуры). Нажмем на этот узел правой кнопкой мыши и в появившемся меню выберем <span class="bb">Add New Stored Procedure</span>:</p>
<img src="pics/5.9.png" alt="Добавление хранимой процедуры" />
<p>После этого Visual Studio сгенерирует код процедуры по умолчанию:</p>
<pre class="brush:sql;">
...
<p>Изменим ее следующим образом:</p>
<pre class="brush:sql;">
...
<img src="pics/5.10.png" alt="Создание хранимой процедуры в Entity Framework" />
<p>И затем в появившемся окошке нажимаем кнопку Update Database:</p>
<img src="pics/5.11.png" />
<p>После этого в узле <span class="bb">Stored Procedures</span> появится новая хранимая процедура. Перед ее использованием мы можем проверить ее, чтобы убедиться, 
что она работает как надо и получает нужные данные. Для этого нажмем на нее правой кнопкой мыши и выберем в меню <span class="bb">Execute</span>:</p>
<img src="pics/5.12.png" alt="Тестирование хранимой процедуры" />
<p>После этого нам откроется окно настройки входного параметра. В поле Value введем какое-нибудь имя, по которому будем осуществлять поиск и нажмем ОК:</p>
<img src="pics/5.13.png" />
<p>И Visual Studio автоматически генерирует и запускает скрипт на языке SQL с вызовом процедуры и передачи ей параметра:</p>
...
<p>Параметр в методе <code>SqlQuery</code> принимает название процедуры, после которого идет перечисление параметров: <code>GetPhonesByCompany @name</code></p>
<p>И так как процедура возвращает строки из таблицы Phones, то метод <code>SqlQuery</code> мы можем типизировать классом Phone: <code>db.Database.SqlQuery&lt;Phone&gt;(...)</code></p>
