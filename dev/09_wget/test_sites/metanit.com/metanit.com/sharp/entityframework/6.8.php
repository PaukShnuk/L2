...
<html >
<head>
<title>Конфигурация моделей | Entity Framework 6</title>
<meta charset="utf-8" />
...
<h2>Конфигурация моделей</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 26.09.2016</div>
...
<p>С помощью Fluent API мы можем задать многочисленные настройки для моделей и их свойств. Однако, если настроек очень много, то они могут утяжелять 
класс контекста. И для вынесения их во вне можно использовать конфигурации, которые представлены классом <span class="b">EntityTypeConfiguration&lt;T&gt;</span>.</p>
<p>К примеру, пусть у нас есть следующий класс контекста:</p>
<pre class="brush:c#;">
...
<p>Вся конфигурация здесь определена в методе <code>OnModelCreating()</code>. В принципе он не содержит много кода, однако при наличии гораздо большего количества 
сущностей и более изощренной их конфигурации с помощью Fluen API данный метод мог бы сильно раздуться в размерах. И теперь изменим определение контекста, применив конфигурации:</p>
<pre class="brush:c#;">
...
<p>Теперь конфигурация моделей вынесена в отдельные классы. А для добавления конкретных конфигураций в контекст используется метод <code>modelBuilder.Configurations.Add()</code>, 
которому передается нужный объект конфигурации. В итоге по своему действию первый и второй варианты контекста будут идентичны.</p>
