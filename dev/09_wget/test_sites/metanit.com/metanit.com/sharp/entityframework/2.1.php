...
<html >
<head>
<title>Автоматизация Code First | Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Автоматизация Code First в Entity Framework для работы с базой данных">
...
<h2>Автоматизация Code First</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Вручную создавать классы по уже готовой бд со всеми полями и связями между собой довольно утомительно, особенно если таблиц в БД очень много. В обновленных 
версиях Visual Studio 2013 с пакетами обновления SP3 мы можем автоматизировать этот процесс.</p>
<p>Для этого добавим в проект новый элемент <span class="bb">ADO.NET Entity Data Model</span>:</p>
<img src="pics/2.1.png" />
<p>Нажмем OK и нам откроется мастер создания модели. Здесь нам надо выбрать пункт <span class="bb">Code First from database</span>:</p>
<img src="pics/2.34.png" />
<p>Далее на следующем шаге настройки модели надо будет установить подключение к имеющейся базе данных.</p>
<img src="pics/2.24.png" />
<p>Нажмем на кнопку <span class="bb">New Connection</span> и в следующем окне настроек подключения выберем сервер и базу данных, с которой мы хотим работать:</p>
<img src="pics/2.18.png" />
<p>После этого в окне мастера настройки модели появится выбранное подключение. И также здесь мы можем установить название подключения, 
которое будет использоваться в файле конфигурации App.config. Изменим его, например, на UserContext:</p>
<img src="pics/2.35.png" />
<p>Нажмем Next, и на следующем шаге нам будет предложено выбрать те таблицы из бд, по которым нам надо создать модели:</p>
<img src="pics/2.36.png" />
<p>И затем нажмем Finish. После этого будут сгенерированы классы моделей. Например, в моем случае по единственной таблице в бд будет сгенерирован следующий класс:</p>
<pre class="brush:c#;">
...
<p>И также надо отметить, что в файле <i>App.config</i> появилось определение подключения:</p>
<pre class="brush:xml;">
 &lt;connectionStrings&gt;
...
<p>Для полноценной работы нам осталось добавить класс контекста данных:</p>
<pre class="brush:c#;">
using System;
using System.Collections.Generic;
...
<p>И теперь мы можем взаимодействовать с базой данных:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="2.6.php">Назад</a><a href="./">Содержание</a><a href="2.3.php">Вперед</a></p></div></div>
<div class="socBlock">
