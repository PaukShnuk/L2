...
<html >
<head>
<title>Code First к существующей базе данных | Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Использование подхода Code First в Entity Framework для работы с уже существующей базой данных, Code Second, настройка подключения">
...
<h1>Взаимодействие с данными. Подходы</h1><h2>Code First к существующей базе данных</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 16.05.2018</div>
...
<p>В первой главе при создании первого приложения с Entity Framework мы использовали подход <span class="bb">Code First</span>. Этот подход очень прост и удобен. 
Но он также и очень гибкий. Так, вполне часто распространена ситуация, когда база данных уже имеется. И здесь опять же поможет Code First. 
Иногда программисты называют данный подход <span class="b">Code Second</span>. Посмотрим на примере.</p>
<p>Вначале создадим новый проект. Затем создадим тестовую базу данных. 
В Visual Studio выберем в меню пункт <span class="b">View-&gt;SQL Server Object Explorer</span>. В открывшемся окне SQL Server Object Explorer создадим новую базу данных. 
Для этого раскроем узел сервера, нажмем на узел Databases правой кнопкой мыши и в появившемся контекстном меню выберем <span class="b">Add New Database</span>.</p>
<img src="pics/2.37.png" alt="Connect to Database in Visual Studio" />
<p>Далее отобразится окно, в котором в качестве имени базы данных введем userstoredb:</p>
<img src="pics/2.7.png" />
<p>После создания БД добавим в нее таблицу. Нажмем правой кнопкой мыши на узел Tables и в появившемся контекстном меню выберем Add New Table.</p>
<img src="pics/2.8.png" /> 
<p>Затем в центральном поле в режиме дизайнера создадим следующее определение таблицы:</p>
<img src="pics/2.10.png" />
<p>В поле T-SQL (или графически) определим структуру и имя таблицы, типы столбцов и после этого нажмем в верхнем левом углу на кнопку Update.</p>
<p>В новом окне нам будет выдана некоторая информация об изменениях, производимых в бд:</p>
<img src="pics/2.11.png" />
<p>Нажмем на кнопку Update Database. И после этого будет создана таблица Users.</p>
<p>Мы можем добавить некоторые данные в таблицу. Для этого нажмем на таблицу в окне Server Explorer правой кнопкой мыши и выберем пункт 
<span class="b">View Data</span>.</p>
<img src="pics/2.38.png" />
<p>У нас откроется форма для работы с данными, в которую введем пару строк:</p>
<img src="pics/2.20.png" />
<p>База данных готова. Теперь нам надо добавить подключение в файл конфигурации приложения. В Solution Explorer найдем файл App.config и откроем его. 
Перед закрывающим тегом <code>&lt;/configuration&gt;</code> добавим новую секцию <span class="bb">connectionStrings</span>:</p>
<pre class="brush:xml;">
...
<p>Теперь определим классы модели данных и контекста. Добавим класс модели User:</p>
<pre class="brush:c#;">
public class User
{
...
<p>И также добавим класс контекста данных:</p>
<pre class="brush:c#;">
using System;
...
<p>В конструкторе контекста данных мы передаем в конструктор базового класса имя строки подключения из файла конфигурации App.config. Так как мы определили 
там строку подключения UserDB (<code>&lt;add name="UserDB"</code>), то именно это значение и используется в конструкторе.</p>
<p>Однако, как вариант, мы могли не использовать конструктор в классе контекста данных, а определить в качестве имени строки подключения название этого класса, например: 
<code>&lt;add name="UserContext" connectionString="....</code></p>
<p>И для получения данных определим следующий код в консольном приложении:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="1.2.php">Назад</a><a href="./">Содержание</a><a href="2.6.php">Вперед</a></p></div></div>
<div class="socBlock">
