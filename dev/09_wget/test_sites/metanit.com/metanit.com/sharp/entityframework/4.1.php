...
<html >
<head>
<title>LINQ to Entities | Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="LINQ to Entities в Entity Framework, создание запросов к базе данных, его отличие от LINQ to Objects">
...
<h2>Введение в LINQ to Entities</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Ранее мы использовали ряд операций для получения данных из БД. В основе подобных операций лежит технология LINQ (Language Integrated Query), или точнее 
<b>LINQ to Entities</b>. LINQ to Entities предлагает простой и интуитивно понятный подход для получения данных с помощью выражений, 
которые по форме близки выражениям языка SQL.</p>
<P>Хотя при работе с базой данных мы оперируем запросами LINQ, но база данных понимает только запросы на языке SQL. Поэтому между LINQ to Entities и 
базой данных есть проводник, который позволяет им взаимодействовать. Этим проводником является провайдер <span class="bb">EntityClient</span>. Он создает интерфейс для взаимодействия с провайдером ADO.NET для 
SQL Serverа.</p>
<p>Для начала взаимодействия с базой данных создается объект <span class="bb">EntityConnection</span>. Через объект <span class="bb">EntityCommand</span> 
он отправляет запросы, а с помощью объекта <span class="bb">EntityDataReader</span> считывает извлеченные из БД данные. Однако разработчику не надо напрямую взаимодействовать с 
этими объектами, фреймворк все сделает за него. Задача же разработчика сводится в основном к написанию запросов к базе данных с помощью LINQ.</p>
<p>Прежде чем приступить к обзору основных запросов в LINQ to Entities, для работы с материалом этой главы создадим новые модели по связи один-ко-многим:</p>
<pre class="brush:c#;">
...
<p>У нас здесь модель телефона и модель компании-производителя. Теперь создадим контекст данных и инициализатор базы данных начальными данными:</p>
<pre class="brush:c#;">
...
<p>Чтобы база данных уже содержала некоторые данные, в инициализаторе бд создается несколько объектов. Чтобы задействовать инициализатор, он вызывается в статическом конструкторе 
контекста данных: <code>Database.SetInitializer(new MyContextInitializer());</code></p>
<p>Для создания запросов в Linq to Entities, так же, как и в Linq to Objects, мы можем применять операторы LINQ и методы расширения LINQ.</p>
<p>Например, используем некоторые операторы LINQ:</p>
<pre class="brush:c#;">
...
<p>И тот же запрос с помощью методов расширений LINQ:</p>
<pre class="brush:c#;">
using(PhoneContext db = new PhoneContext())
{
...
<p>Оба запроса в итоге транслируются в одной выражение sql:</p>
<pre class="brush:sql;">
...
<p>Важно понимать различие между Linq to Entities и Linq to Objects:</p>
<pre class="brush:c#;">var phones = db.Phones.Where(p=&gt; p.CompanyId == 1).ToList().Where(p=&gt; p.Id&lt;10);</pre>
<p>Здесь используются два метода Where, но их реализация будет различной. В первом случае, <code>db.Phones.Where(p=&gt; p.CompanyId == 1)</code> транслируется 
в выражение SQL, которое было рассмотрено выше. Далее метод <code>ToList()</code> по результатам запроса создает список в памяти компьютера. После этого мы 
уже имеем дело со списком в памяти, а не с базой данных. И далее вызов <code>Where(p=&gt; p.Id&lt;10)</code> будет обращаться к списку в памяти и будет представлять 
Linq to Object.</p>
<p>А теперь рассмотрим некоторые приемы применения LINQ к запросам из базы данных.</p>
...
<div class="nav"><p><a href="3.13.php">Назад</a><a href="./">Содержание</a><a href="4.2.php">Вперед</a></p></div></div>
<div class="socBlock">
