...
<html >
<head>
<title>Связь один ко многим | C# Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Связь один-ко-многим в Entity Framework реализуется, если одна модель хранит ссылку на один объект другой модели, а вторая модель может ссылаться на коллекцию объектов первой модели">
...
<h2>Связь один ко многим</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 17.12.2015</div>
...
<p>Связь один-ко-многим реализуется, если одна модель хранит ссылку на один объект другой модели, а вторая модель может ссылаться на коллекцию объектов первой модели. 
Например, в одной команде играет несколько игроков:</p>
<pre class="brush:c#;">
public class Player
{
...
<img src="pics/3.17.png" alt="One-to-Many relationship in Entity Framework" />
<p>Редактирование:</p>
<pre class="brush:c#;">
//редактирование
...
<p>При удалении объектов, связанных отношением "один-ко-многим" нам надо учитывать то, что по умолчанию даже если внешний ключ допускает значение null 
...
В этом случае нам надо установить для внешнего ключа TeamId в таблице игроков ограничение <code>ON DELETE SET NULL</code>. Данное ограничение позволит 
при удалении связанного объекта устанавливать для внешнего ключа значение null.</p>
<p>Установку ограничения можно сделать вручную, изменим схему базы данных, либо программно. Рассмотрим, как это сделать программно. Для этого перед удалением нам надо выполнить 
соответствующую команду SQL, чтобы добавить ограничение:</p>
<pre class="brush:c#;">
 ...
 <p>Здесь добавляется ограничение "Players_Teams" в таблицу игроков, которая называется, как правило, dbo.Players или просто Players. Это ограничение указывает, что для 
 внешнего ключа TeamId из таблицы Players, который связан со столбцом Id из таблицы dbo.Teams, устанавливается правило "ON DELETE SET NULL", то есть установка null по удалению.</p>
<p>И после этого мы можем выполнить удаление:</p>
<pre class="brush:c#;">
//удаление игрока
...
<p>При удалении команды свойство Team у объектов Player получает значение null.</p>
