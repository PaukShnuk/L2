...
<html >
<head>
<title>AsNoTracking | Entity Framework 6</title>
<meta charset="utf-8" />
...
<h2>Метод AsNoTracking</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Когда контекст данных извлекает данные из базы данных, Entity Framework помещает извлеченные объекты в кэш и отслеживает изменения, которые происходят 
с этими объектами вплоть до использования метода <code>SaveChanges()</code>, который фиксирует все изменения в базе данных. Но нам не всегда необходимо отслеживать изменения. 
Например, нам надо просто вывести данные для просмотра.</p>
<p>Чтобы данные не помещались в кэш, применяется метод <span class="bb">AsNoTracking()</span>. При его применении возвращаемые из запроса данные не кэшируются. 
А это означает, что Entity Framework не производит какую-то дополнительную обработку и не выделяет дополнительное место для хранения извлеченных из БД объектов.</p>
<p>Метод AsNoTracking() применяется к набору IQueryable:</p>
<pre class="brush:c#;">
...
<p>Небольшой пример. У нас в базе данных есть есть несколько моделей Phones:</p>
<pre class="brush:c#;">
...
<p>Мы увидим, что в наборе phones первый элемент имеет название "Samsung Galaxy Ace 2".</p>
...
<p>Так как при получении первого элемента используется AsNoTracking, он не будет отслеживаться, и поэтому вызов <code>db.SaveChanges()</code> никак не повлияет 
на базу данных, а первый элемент сохранит свое первоначальное значение - "Samsung Galaxy Note".</p>
<p>Если мы обратимся к тестам сравнения простого запроса и запроса с использованием AsNoTracking, то мы увидим преимущество как по скорости выполнения, 
так и по использованной памяти.</p>
<h5>Сравнение по скорости выполнения</h5>
<img src="pics/4.3.png" alt="AsNoTracking and Execution Speed in Entity Framework" />
<h5>Сравнение по использованию памяти</h5>
<img src="pics/4.4.png" alt="AsNoTracking and Memory Usage in Entity Framework" />
<p>Правда, различия будут заметны на больших объемах данных в сотни и тысячи объектов.</p>
<p>Когда следует использовать AsNoTracking? Если нам надо просто вывести данные для отображения без необходимости их дальнейшего обновления, 
тогда как раз тот случай, когда мы можем использовать AsNoTracking.</p>
...
<div class="nav"><p><a href="1.4.php">Назад</a><a href="./">Содержание</a><a href="5.1.php">Вперед</a></p></div></div>
<div class="socBlock">
