...
<html >
<head>
<title>Model First | Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="создание первого приложения на C# с использованием Entity Framework Применение подхода Model First">
...
<h2>Model First</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Model First представляет еще один подход к работе с Entity Framework. Суть данного подхода состоит в том, что сначала делается модель, а потом по ней 
создается база данных.</p>
<p>Итак, создадим новый проект по типу Console  Application. И затем добавим в проект новый элемент. Нажмем правой кнопкой мыши на проект в окне Solution Explorer и в появившемся списке выберем 
<span class="bb">Add -&gt; New Item</span>. И затем в окне добавления нового элемента выберем <span class="bb">ADO.NET Entity Data Model</span>:</p>
<img src="pics/2.1.png" />
<p>Поскольку модель будет описывать человека, то назовем ее User. Нажмем OK и нам откроется мастер создания модели. Если у нас Visual Studio с пакетами обновления 
SP2, SP3, то мастер создания модели выглядит следующим образом:</p>
<img src="pics/2.4.png" />
<p>Окно нам предлагает четыре варианта создания модели, из которых нам надо выбрать <span class="b">Empty EF Designer Model</span>.</p>
<p>Нажмем кнопку Finish, и перед нами откроется пустое окно создания модели.</p>
<img src="pics/2.21.png" />
<p>Перетащим на это поле с панели Toolbox (Панель Инструментов) в левой части элемент Entity. Теперь у нас на поле создания модели имеется небольшая схема будущей 
модели, в которой сейчас по умолчанию указано лишь одно поле - Id. Во-первых, переименуем сущность. По умолчанию она называется Entity1. Выделим схему и 
перейдем к окну свойств в правом нижнем углу:</p>
<img src="pics/2.22.png" />
<p>Здесь изменим значение свойства Name на User. Это у нас будет имя сущности. И также изменим 
значение свойства Entity Set Name на Users. Это у нас будет название набора объектов User.</p>
<p>Далее создадим несколько свойств. Сущность у нас будет простая и будет содержать всего два свойства для имени и возраста. Итак, выделим схему сущности и 
нажмем на правую кнопку мыши. В выпадающем списке выберем <span class="bb">Add New -&gt; Scalar Property</span>. После этого будет добавлено новое свойство. 
Scalar Property подразумевают свойства на основе простейших типов int, float, string и т.д. Добавим два свойства - Name и Age. По умолчанию все добавляемые 
свойства имеют тип string. Однако мы можем изменить тип в окне свойств.</p>
<p>Таким образом, у нас должна получиться следующая схема сущности:</p>
<img src="pics/2.3.png" />
<p>После создания диаграммы модели перестроим проект с помощью опции Rebuild.</p>
<p>Теперь по модели мы можем сгенерировать код и базу данных. Вначале сгенерируем код модели. Для этого нажмем на диаграмму модели правой кнопкой мыши и 
выберем пункт <span class="bb">Add Code Generation Item</span>:</p>
<img src="pics/2.31.png" />
<p>Далее нам будет предложено выбрать версию EF. Выберем шестую версию:</p>
<img src="pics/2.32.png" />
<p>После этого в структуре проекта мы можем увидеть узел <i>User.tt</i>, который в качестве подузла будет содержать класс модели в файле User.cs:</p>
<img src="pics/2.33.png" />
<p>Также здесь мы можем найти файл контекста данных <i>User.Context.cs</i>, который выглядит в моем случае следующим образом:</p>
<pre class="brush:c#;">
...
<p>Теперь сгенерируем базу данных по нашей модели. Итак, нажмем на диаграмму модели правой кнопкой мыши и 
в выпадающем списке выберем <span class="bb">Generate Database from Model</span> (Сгенерировать базу данных по модели). Перед нами откроется 
мастер создания подключения.</p>
<img src="pics/2.24.png" />
<p>Нажмем на кнопку <span class="bb">New Connection</span> (Новое подключение). Далее открывается новое окно, где будет предложено настроить подключение и создать базу данных:</p>
<img src="pics/2.25.png" />
<p>Здесь нам надо ввести имя сервера. А также название бд. В качестве имени базы данных введем usersdb, а в качестве сервера: <span class="bb">(localdb)\v11.0</span>. 
Нажмем OK и затем Visual Studio установит в качестве подключения модели только что созданную базу данных:</p>
<img src="pics/2.26.png" />
<p>После этого будет сгенерирован скрипт базы данных:</p>
<img src="pics/2.28.png" />
<p>Нажмем Finish (Готово). У нас автоматически откроется в Visual Studio файл скрипта User.edmx.sql. И в завершении нам надо будет запустить этот скрипт. Для этого нажмем в верхнем левом углу на зеленую кнопку Execute (Выполнить):</p>
<img src="pics/2.29.png" />
<p>После этого в нижнем окне Visual Studio нам сообщит об успешном (или неуспешном) создании базы данных. Если мы откроем окно Database Explorer 
(его можно открыть, выбрав в меню View-&gt;Other Windows), то мы увидим нашу базу данных. А раскрыв узел, также увидим, что она содержит всю ту схему, которую 
мы определили в модели:</p>
<img src="pics/2.30.png" />
<p>Это все была работа по созданию модели и базы данных. И в конце определим минимальный код для работы с базой данных:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="2.4.php">Назад</a><a href="./">Содержание</a><a href="1.3.php">Вперед</a></p></div></div>
<div class="socBlock">
