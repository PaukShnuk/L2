...
<html >
<head>
<title>Связь многие ко многим | C# Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Создание приложения со связью многие-ко-многим в Entity Framework, построение базы данных, особенности взаимодействия объектов">
...
<h2>Связь многие ко многим</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Еще одним способом ассоциации объектов является связь многие-ко-многим. Например, у нас есть модель футболистов и есть модель команд. 
На протяжении всей жизни футболист может поиграть в различных командах, а в одной команде может поиграть множество разных футболистов. На уровне моделей это выглядит так:</p>
<pre class="brush:c#;">
public class Team
{
...
<p>Обе модели имеют свойства-коллекции, через которые и будет осуществляться связь многие-ко-многим. В итоге, если мы используем CodeFirst, то 
автоматически будет создаваться база данных со следующей схемой:</p>
<img src="pics/3.9.png" alt="many to many in Entity Framework" />
<p>То есть создается промежуточная таблица, которая хранит наборы пар Player-Team. И если бы мы использовали подход Database First, то нам надо было также создать 
эту таблицу.</p>
<p>Используем модели. Добавление и вывод:</p>
<pre class="brush:c#;">
...
<p>При добавление одной модели в список к другой важно помнить, что это список уже должен быть создан, иначе будет выброшено исключение. В данном случае мы создаем список в конструкторе 
обоих моделей. Также допустимо создание списка непосредственно в программе.</p>
<p>Редактирование:</p>
<pre class="brush:c#;">
...
<p>Удаление игрока из списка команды будет означать удаление строки из таблицы TeamPlayers, в которой id игрока сопоставляется id команды.</p>
<p>Удаление же игрока или команды вообще из базы данных приводит к тому, что все строки в таблице TeamPlayers, которые содержат id удаленного объекта, также будут удалены:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="3.3.php">Назад</a><a href="./">Содержание</a><a href="3.4.php">Вперед</a></p></div></div>
<div class="socBlock">
