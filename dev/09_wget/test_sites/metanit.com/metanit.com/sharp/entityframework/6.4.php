...
<html >
<head>
<title>Работа с комплексными типами | Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Комплексные типы в Entity Framework 6, разбиение одного класса на две таблицы">
...
<h2>Работа с комплексными типами</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Каждый тип сопоставляется Entity Framework с отдельной таблицей. Однако бывают случаи, когда определенный класс существует не сам по себе, а несет некоторую 
дополнительную информацию по отношению к другой главной модели. Например:</p>
<pre class="brush:c#;">
...
<p>Класс PhoneInfo содержит некоторую дополнительную информацию по отношению к модели Phone, а модель Phone создает объект PhoneInfo с данными по умолчанию. 
Такие типы еще называют комплексными.</p>
<p>Комплексные типы имеют ряд ограничений:</p>
<ul>
<li><p>Они не имеют ключей</p></li>
<li><p>Они могут содержать свойства только примитивных типов</p></li>
<li><p>При использовании моделями модель может использовать только один объект типа, но не коллекцию (то есть коллекцию PhoneInfo модель Phone содержать не может)</p></li>
</ul>
<p>Чтобы обозначить связь между двумя типами, необходимо использовать атрибут <span class="bb">ComplexType</span>. Он позволяет создать из разных классов одну сущность.</p>
<p>Например:</p>
<pre class="brush:c#;">
[ComplexType]
public class PhoneInfo
...
<p>Класс Phone остается без изменений. Тогда мы можем использовать данные типы следующим образом:</p>
<pre class="brush:c#;">
...
<p>Тогда при создании таблицы она будет объединять столбцы для обоих моделей, представляя как бы единую сущность:</p>
<img src="pics/6.4.png" />
<p>Альтернативный вариант применению атрибута представляет использование метода <span class="bb">ComplexType</span> в FluentAPI:</p>
<pre class="brush:c#;">
