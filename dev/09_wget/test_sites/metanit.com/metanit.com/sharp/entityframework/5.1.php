...
<html >
<head>
<title>SQL в EntityFramework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Выполнение запросов SQL в базе данных через EntityFramework 6, свойство Database">
...
<h1>SQL в EntityFramework</h1><h2>Работа с SQL</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В большинстве случаев разработчики смогут создать эффективные запросы с помощью методов и операторов LINQ. Однако в Entity Framework доступно также прямое выполнение 
sql-запросов.</p>
<p>Для осуществления прямых sql-запросов к базе данных можно воспользоваться свойством <span class="bb">Database</span>, которое имеется у класса контекста данных. 
Данное свойство позволяет получать информацию о базе данных, подключении и осуществлять запросы к БД. Например, получим строку подключения:</p>
<pre class="brush:c#;">
...
<p>Непосредственно для создания запроса нам надо использовать метод <b>SqlQuery</b>, который принимает в качестве параметра sql-выражение.</p>
<p>Для примера возьмем 
базу данных, созданную в прошлой теме, которая описывается следующими моделями и контекстом данных:</p>
<pre class="brush:c#;">
...
<p>Итак, получим все модели из таблицы Companies:</p>
<pre class="brush:c#;">
using(PhoneContext db = new PhoneContext())
{
...
данный вызов типизируется классом Company: <code>db.Database.SqlQuery&lt;Company&gt;()</code></p>
<p>Другая версия метода <code>SqlQuery()</code> позволяет использовать параметры. Например, выберем из бд все модели, которые в названии имеют подстроку "Samsung":</p>
<pre class="brush:c#;">
using(PhoneContext db = new PhoneContext())
{
...
<p>Класс <b>SqlParameter</b> из пространства имен System.Data.SqlClient позволяет задать параметр, который затем передается в запрос sql.</p>
<p>Метод <code>SqlQuery()</code> осуществляет выборку из БД, но кроме выборки нам, возможно, придется удалять, обновлять уже существующие или вставлять 
новые записи. Для этой цели применяется метод <span class="bb">ExecuteSqlCommand()</span>, который возвращает количество затронутых командой строк:</p>
<pre class="brush:c#;">
// вставка
...
<div class="nav"><p><a href="4.8.php">Назад</a><a href="./">Содержание</a><a href="5.2.php">Вперед</a></p></div></div>
<div class="socBlock">
