...
<html >
<head>
<title>Управление транзакциями | C# Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Управление транзакциями в Entity Framework 6">
...
<h2>Управление транзакциями</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 22.12.2017</div>
...
<p>Когда мы вызываем при добавлении, обновлении, удалении метод <code>SaveChanges()</code>, то фактически Entity Framework проводит транзакцию. 
...
откатить все сделанные из этих операций назад, если какая-нибудь из этих операций завершилась с ошибкой.</p>
<p>Для создания транзакции используется выражение <code>var transaction = db.Database.BeginTransaction()</code>, и так как класс 
<span class="b">DbContextTransaction</span> реализует интерфейс IDisposable, то весь код транзакции обертываем в конструкцию <code>using</code></p>
<p>После операций с БД вызывается метод <code>transaction.Commit()</code> для коммита транзакции.</p>
...
<p>В данном случае и метод <code>db.Database.ExecuteSqlCommand</code> и <code>db.SaveChanges()</code> определяют две разные транзакции. Однако поскольку мы оборачиваем оба этих метода в 
блок <code>using (var transaction = db.Database.BeginTransaction())</code> оба этих метода будут объединены в одну транзакцию. И если 
вызов <code>db.Database.ExecuteSqlCommand</code> пройдет неудачно, то вызов <code>db.SaveChanges()</code> не будет выполняться.</p>
<p>Однако если, допустим, у нас возникнет исключение параллелизма или любое другое исключение при редактировании или добавлении, то есть одна из 
...
помощью метода <code>transaction.Rollback()</code></p>
