...
<html >
<head>
<title>Связь многие ко многим. Практический пример | Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Применение связи многие ко многим в приложении на C# и Entity Framework 6">
...
<h2>Связь многие ко многим. Практический пример</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Итак, создадим новый проект по типу Windows Forms и добавим в него эти модели. После этого также добавим через NuGet пакет Entity Framework 
и следующий класс контекста данных:</p>
<pre class="brush:c#;">
...
<p>Теперь само приложение. На главной форме у нас будет в таблице выводиться список игроков:</p>
<img src="pics/3.13.png" alt="Связь многие ко многим в Entity Framework" />
<p>Основной функционал будет таким же, как и в прошлой теме: один элемент DataGridView и четыре кнопки. Кроме того нам потребуется две дополнительные формы: 
одна для создания команды, а другая - для создания/редактирования игрока.</p>
<p>Добавим в проект новую форму. Назовем ее, к примеру, <span class="bb">PlayerForm</span>:</p>
<img src="pics/3.14.png" />
<p>Здесь текстовое поле для указания имени, поле NumericUpDown для возраста и элемент ListBox, в котором будут выводиться команды. Установим у этих 
элементов значение свойства <span class="b">Modifier</span> равным <code>Protected Internal</code>.</p>
<p>Также есть две кнопки. Установим у кнопки 'ОК' значение свойства <span class="b">DialogResult</span> равным <code>OK</code>, 
а у кнопки 'Отмена' - равным <code>Cancel</code>.</p>
<p>Форма для добавления команд, назовем ее TeamForm, будет выглядеть следующим образом:</p>
<img src="pics/3.15.png" />
<p>Произведем у этой формы ту же настройку с полями и кнопками, как и у предыдущей формы.</p>
<p>Теперь изменим код главной формы, которая отображает игроков, на следующий:</p>
...
<div class="nav"><p><a href="3.8.php">Назад</a><a href="./">Содержание</a><a href="3.9.php">Вперед</a></p></div></div>
<div class="socBlock">
