...
<html >
<head>
<title>Наследование в Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Приемы наследования в Entity Framework 6, TPH (Table Per Hierarchy / Таблица на одну иерархию классов)">
...
<h1>Наследование в Entity Framework</h1>
<h2>Подход TPH</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>При использовании подхода TPH (Table Per Hierarchy / Таблица на одну иерархию классов) для одной иерархии классов используется одна таблица. Данные базовых 
и производных классов сохраняются в одну таблицу, а для их отличия создается специальный столбец.</p>
<p>Например, создадим мини-иерархию из двух классов:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Здесь класс Smartphone наследуется от Phone, определяя одно свойство в дополнение к унаследованным. И при работе будет создана такая таблица:</p>
<img src="pics/6.8.png" alt="Inheritance in Entity Framework" />
<p>Кроме всех свойств классов Phone и Smartphone здесь также появляется еще один столбец - Discriminator. Он имеет тип <code>nvarchar</code> (то есть строка) и имеет длину в 128 символов. 
Данный столбец и будет определять относится строка к типу Phone или Smartphone.</p>
<p>Теперь используем в программе:</p>
<pre class="brush:c#;">
...
<p>Также обратите внимание на вывод консоли:</p>
<img src="pics/6.9.png" alt="Наследование in Entity Framework" />
<p>Так как объект Smartphone также является и объектом Phone, то он также извлекается через db.Phones.</p>
