...
<html >
<head>
<title>Подход TPC | Наследование в Entity Framework 6</title>
<meta charset="utf-8" />
...
<h2>Подход TPC</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Подход TPC (Table Per Concrete Type / Таблица на каждый отдельный тип) предполагает создание для каждой модели по отдельной таблицы. Столбцы в каждой таблице создаются 
по всем свойствам, в том числе и унаследованным.</p>
<p>Чтобы применить подход, изменим объявления моделей и контекст следующим образом:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Во-первых, обратите внимание, что у класса Phone в качестве типа ключа используется не int, а <span class="bb">Guid</span>. Это поможет нам избежать некоторых проблем 
с ключами. Хотя также можно было бы использовать <code>int</code> с ручной установкой Id при создании объекта.</p>
<p>Во-вторых, при настройке сопоставления моделей и таблиц у каждой модели вызывается метод <span class="bb">MapInheritedProperties()</span>, 
который указывает Entity Frameworkу, что в таблицу для данной модели надо включить также наследуемые свойства, а не только те, которые определены непосредственно в этой модели.</p>
<p>При генерации базы данных у нас будут созданы две таблицы с полным набором столбцов:</p>
<img src="pics/6.11.png" />
<p>Применение моделей:</p>
<pre class="brush:c#;">
...
<p>Несмотря на то, что объект Smartphone никак не связан с таблицей Phones, при извлечении данных он также будет находится в наборе <code>db.Phones</code>, 
потому что наследование все равно будет действовать.</p>
