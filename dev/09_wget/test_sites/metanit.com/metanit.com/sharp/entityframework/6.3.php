...
<html >
<head>
<title>Аннотации | Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Аннотации данных в Entity Framework 6, основные атрибуты и конфигурация моделей">
...
<h2>Аннотации</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 26.09.2016</div>
...
<p>Аннотации представляют настройку сопоставления моделей и таблиц с помощью атрибутов. Большинство классов аннотаций располагаются в пространстве 
<code>System.ComponentModel.DataAnnotations</code>, которое нам надо подключить в файл c# перед использованием аннотаций.</p>
<h3>Настройка ключа</h3>
<p>Для установки свойства в качестве первичного ключа применяется атрибут <code>[Key]</code>:</p>
<pre class="brush:c#;">
public class Phone
{
    [Key]
...
<p>Теперь свойство Ident будет рассматриваться в качестве первичного ключа.</p>
<p>Чтобы установить ключа в качестве идентификатора, можно использовать атрибут <code>DatabaseGenerated(DatabaseGeneratedOption.Identity)</code>:</p>
<pre class="brush:c#;">
[Key,DatabaseGenerated(DatabaseGeneratedOption.Identity)]
public int Ident { get; set; }
</pre>
<p>Если свойство, помеченное атрибутом <code>Key</code>, представляет тип <code>int</code>, то к нему атрибут <code>DatabaseGenerated(DatabaseGeneratedOption.Identity)</code> 
будет применяться автоматически, и его можно в принципе не указывать.</p>
<h3>Атрибут Required</h3>
<p>Атрибут <span class="bb">Required</span> указывает, что данное свойство обязательно для установки, то есть будет иметь определение NOT NULL в БД:</p>
<pre class="brush:c#;">
public class Phone
{
    [Key]
...
<p>Если мы не установим свойство Name у объекта Phone и попытаемся добавить этот объект в бд, то получим ошибку. А столбец Name будет определен как NOT NULL.</p>
<h3>MaxLength и MinLength</h3>
<p>Атрибуты <span class="bb">MaxLength</span> и <span class="bb">MinLength</span> устанавливают максимальное и минимальное количество символов в строке-свойстве:</p>
<pre class="brush:c#;">
public class Phone
{
    [Key]
...
<h3>Атрибут NotMapped</h3>
<p>По умолчанию все публичные свойства сопоставляются с определенными столбцами в таблицами. Но такое поведение не всегда необходимо. Иногда требуется, наоборот, 
исключить определенное свойство, чтобы для него не создавался столбец в таблице. И для этих целей есть атрибут NotMapped:</p>
<pre class="brush:c#;">
public class Phone
{
    [Key]
...
<p>Чтобы задействовать атрибут NotMapped, надо подключить пространство имен <code>System.ComponentModel.DataAnnotations.Schema</code></p>
<h3>Сопоставление с таблицей и столбцами</h3>
<p>Entity Framework при создании и сопоставлении таблиц и столбцов использует имена моделей и их свойств. Но мы можем переопределить это поведение с 
помощью атрибутов <span class="bb">Table</span> и <span class="bb">Column</span>:</p>
<pre class="brush:c#;">
[Table("Mobiles")]
public class Phone
{
...
<p>Теперь сущность Phone будет сопоставляться с таблицей Mobiles, а свойство Name со столбцом ModelName:</p>
<img src="pics/6.5.png" />
<h3>Установка внешнего ключа</h3>
<p>Чтобы установить внешний ключ для связи с другой сущностью, используется атрибут <span class="bb">ForeignKey</span>:</p>
<pre class="brush:c#;">
public class Phone
{
    public int Id { get; set; }
...
<p>В данном случае внешним ключом для связи с моделью Company будет служить свойство CompId.</p>
<h3>Настройка индекса</h3>
<p>Для установки индекса для столбца к соответствующему свойству модели применяется атрибут <span class="b">[Index]</span></p>
<pre class="brush:c#;">
public class Phone
{
	[Index]
...
<h3>ConcurrencyCheck</h3>
<p>Атрибут <span class="bb">ConcurrencyCheck</span> позволяет решить проблему параллелизма, когда с одной и той же записью в таблице могут работать одновременно 
несколько пользователей. Например:</p>
<pre class="brush:c#;">
public class Phone
{
    public int Id { get; set; }
...
<p>Теоретически возможна ситуация, когда два пользователя пытаются изменить значение, например:</p>
<pre class="brush:c#;">
...
<p>В обычном режиме Entity Framework при обновлении смотрит на Id и если Id записи в таблице совпадает с Id в передаваемой модели phone, то строка в таблице обновляется. 
...
то запись обновляется. Если же не совпадает (то есть кто-то уже успел обновить), то EF генерирует исключение <span class="bb">DbUpdateConcurrencyException</span>.</p>
