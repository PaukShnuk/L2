...
<html >
<head>
<title>Навигационные свойства и загрузка данных | Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Использование навигационных свойств и внешних ключей в Entity Framework. Связь двух таблиц, типы загрузок: lazy loading и eager loading">
...
<h2>Навигационные свойства и загрузка данных</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 28.09.2016</div>
...
<p>В предыдущей теме в качестве модели был использован класс Player, представляющий футболиста и содержащий четыре свойства. Однако эта была 
очень простая модель. В реальности в нашей базе данных может быть не одна, а несколько таблиц, которые связаны между собой различными связями.</h2>
<p>Допустим, для каждого футболиста может быть определена футбольная команда, в которой он играет. И, наоборот, в одной футбольной команде могут играть несколько 
футболистов. То есть в данном случае у нас связь <b>один-ко-многим (one-to-many)</b>.</p>
<p>Например, у нас определен следующий класс футбольной команды Team:</p>
<pre class="brush:c#;">
class Team
{
...
<p>А класс Player, описывающий футболиста, мог бы выглядеть следующим образом:</p>
<pre class="brush:c#;">
class Player
{
    public int Id { get; set; }
...
<p>Кроме обычных свойств типа Name, Position и Age здесь также определен внешний ключ. Внешний ключ состоит из обычного свойства и навигационного.</p>
<p>Свойство <code>public Team Team { get; set; }</code> в классе Player называется <b>навигационным свойством</b> - при получении данных об игроке оно 
будет автоматически получать данные из БД.</p>
<p>Аналогично в классе Team также имеется навигационное свойство - <code>Players</code>, через которое мы можем получать игроков данной команды.</p>
<p>Вторая часть внешнего ключа - свойство TeamId. Чтобы в связке с навигационным свойством образовать внешний ключ оно должно принимать одно из 
следующих вариантов имени:</p>
<ul>
<li><p><i>Имя_навигационного_свойства+Имя ключа из связанной таблицы</i> - в нашем случае имя навигационного свойства Team, а ключа из модели Team - Id, поэтому в нашем случае нам надо обозвать свойство TeamId, что собственно и было сделано в вышеприведенном коде.</p></li>
<li><p><i>Имя_класса_связанной_таблицы+Имя ключа из связанной таблицы</i> - в нашем случае класс Team, а ключа из модели Team - Id, поэтому опять же в этом случае получается TeamId.</p></li>
</ul>
<p>Как уже было сказано, внешний ключ позволяет получать связанные данные. Например, после генерации базы данных с помощью Code First таблица Players 
будет иметь следующее определение:</p>
<pre class="brush:sql;">
...
<p>При определении внешнего ключа нужно иметь в виду следующее. Если тип обычного свойства во внешнем ключе определяется как <code>int?</code>, 
то есть допускает значения <code>null</code>, то при создании базы данных соответствующее поле так будет принимать значения NULL: <code>[TeamId] INT NULL</code>.</p>
<p>Однако если мы изменим в классе Player тип TeamId на просто <code>int</code>: <code>public int TeamId { get; set; }</code>, то в этом случае 
соответствующее поле имело бы ограничение <code>NOT NULL</code>, а внешний ключ определял бы каскадное удаление:</p>
<pre class="brush:sql;">
CREATE TABLE [dbo].[Players] (
    [Id]       INT            IDENTITY (1, 1) NOT NULL,
    [Name]     NVARCHAR (MAX) NULL,
...
<h3>Способы загрузки и получения связанных данных</h3>
<p>В Entity Framework есть три способа загрузки данных:</p>
<ul>
...
<h4>Eager Loading</h4>
<p>Суть Eager Loading заключается в том, чтобы использовать для подгрузки связанных по внешнему ключу данных метод <b>Include</b>. Например, получим всех 
игроков с их командами:</p>
<pre class="brush:c#;">
...
<p>Без использования метода Include мы бы не могли бы получить связанную команду и ее свойства: <code>p.Team.Name</code></p>
<p>Соответственно чтобы подгрузить к командам все данные по игрокам, мы можем написать так:</p>
<pre class="brush:c#;">
using(SoccerContext db = new SoccerContext())
{
...
<p>Явная загрузка предусмативает применение метода <span class="b">Load()</span> для загрузки данных в контекст. Например:</p>
<pre class="brush:c#;">
using(SoccerContext db = new SoccerContext())
{
...
<p>Чтобы подгрузить данные, здесь идет обращение к методу <code>db.Entry()</code>, в который передается нужный объект. 
Для подгрузки связанного объекта, который не представляет коллекцию, используется метод <span class="b">Reference()</span>. В этот метод переается навигационное свойство, 
по которому надо подгрузить данные.</p>
<p>Если связанные объект представляет коллекцию, то применяется метод <code>Collection()</code>, в который также передается навигационное свойство в виде строки.</p>
<h4>Lazy Loading</h4>
<p>Еще один способ представляет так называемая "ленивая загрузка" или <b>lazy loading</b>. При таком способе подгрузки при первом обращении к объекту, 
если связанные данные не нужны, то они не подгружаются. Однако при первом же обращении к навигационному свойству эти данные автоматически подгружаются из бд.</p>
<p>При использовании ленивой загрузки надо иметь в виду некоторые моменты при объявлении классов. Так, классы, использующие ленивую загрузку должны 
быть публичными, а их свойства должны иметь модификаторы <span class="b">public</span> и <span class="b">virtual</span>. Например, классы Player и Team 
могут иметь следующее определение:</p>
<pre class="brush:c#;">
public class Player
{
    public int Id { get; set; }
...
<p>В этом случае нам не потребуется использовать какие-то дополнительные методы, как Include или Load:</p>
<pre class="brush:c#;">
using (SoccerContext db = new SoccerContext())
{
...
<div class="nav"><p><a href="3.5.php">Назад</a><a href="./">Содержание</a><a href="3.6.php">Вперед</a></p></div></div>
<div class="socBlock">
