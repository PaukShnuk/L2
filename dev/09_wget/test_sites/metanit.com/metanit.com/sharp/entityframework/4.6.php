...
<html >
<head>
<title>Операции с множествами в LINQ to Entities | C# Entity Framework 6</title>
<meta charset="utf-8" />
...
<h2>Операции с множествами: объединение, пересечение, разность</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>Объединение</h3>
<p>Для объединения двух выборок используется метод <span class="bb">Union()</span>:</p>
<pre class="brush:c#;">
...
<p>Метод Union в качестве параметра принимает результаты второй выборки и объединяет ее с исходной.</p>
<p>При этом мы не можем объединить две разнородные выборки, например, таблицу, моделей телефонов и таблицу производителей телефонов. Однако уместна следующая запись:</p>
<pre class="brush:c#;">
...
<h3>Пересечение</h3>
<p>Чтобы найти пересечение выборок, то есть те элементы, которые присутствуют сразу в двух выборках, используется метод <span class="bb">Intersect()</span>:</p>
<pre class="brush:c#;">
using(PhoneContext db = new PhoneContext())
{
	var phones = db.Phones.Where(p =&gt; p.Price &lt; 25000)
...
<h3>Разность</h3>
<p>Если нам надо найти элементы первой выборки, которые отсутствуют во второй выборке, то мы можем использовать метод <span class="bb">Except</span>:</p>
<pre class="brush:c#;">
using(PhoneContext db = new PhoneContext())
{
