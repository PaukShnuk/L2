...
<html >
<head>
<title>Связь один к одному | Entity Framework 6</title>
<meta charset="utf-8" />
...
<h2>Связь один-к-одному</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Строго говоря в Entity Framework нет как таковой связи один-к-одному, так как ожидается, что обработчик будет использовать связь 
...
данный тип отношений.</p>
<p>Рассмотрим стандартный пример подобных отношений: есть класс пользователя User, который хранит логин и пароль, то есть данные учетных записей. А все данные профиля, такие 
как имя, возраст и так далее, выделяются в класс профиля UserProfile.</p>
<pre class="brush:c#;">
...
<p>В этой связи между классами класс UserProfile является дочерним или подчиненным по отношению к классу User. И чтобы установить связь одни к одному, 
...
Id, то и в UserProfile также свойство называется Id. Если бы в классе User свойство называлось бы UserId, то такое же название должно было быть и в UserProfile.</p>
<p>И в классе UserProfile над этим свойством Id устанавливаются два атрибута: <code>[Key]</code>, который показывает, то это первичный ключ, и 
<code>[ForeignKey]</code>, который показывает, что это также и внешний ключ. Причем внешний ключ к таблице объектов User.</p>
<p>Соответственно классы User и UserProfile имеют ссылки друг на друга.</p>
<p>В классе контекста определяются свойства для взаимодействия с талицами в бд:</p>
<pre class="brush:c#;">
...
<p>Для этих классов контекст данных будет создавать следующую таблицу UserProfiles:</p>
<pre>
...
<p>Посмотрим, как работать с моделями с такой связью. Добавление и получение:</p>
<pre class="brush:c#;">
...
Если же будет удален объект UserProfile, на объект User это никак не повлияет:</p>
<pre class="brush:c#;">
using (UserContext db = new UserContext())
{
...
<div class="nav"><p><a href="3.2.php">Назад</a><a href="./">Содержание</a><a href="3.7.php">Вперед</a></p></div></div>
<div class="socBlock">
