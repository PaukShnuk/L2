...
<html >
<head>
<title>Строка подключения | Entity Framework 6</title>
<meta charset="utf-8" />
<meta name="description" content="Строка подключения в Entity Framework, определение строки подключения в Code First, Database First и Model-First и ее параметры">
...
<h2>Строка подключения</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/efcore/">Руководство по Entity Framework Core</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В предыдущей теме использовался следующий контекст данных:</p>
<pre class="brush:c#;">
...
<p>В конструктор базового класса передается наименование подключения - DefaultConnection. И так как в данном случае испльзовался подход Code First, 
то при обращении к базе данных, если ее нет, создавалась новая база данных с названием DefaultConnection на MS SQL Serverе. А после создания все запросы 
также шли к этой базе данных. Мы можем подключиться к MS SQL Server через Visual Studio или воспользоваться специальной средой 
<b>SQL Server Management Studio</b>, чтобы ее увидеть:</p>
<img src="pics/3.16.png" alt="SQL Server Management Studio" />
<p>Но в реальности может потребоваться динамически изменять местоположение или название базы данных, к которой идет подключения. В этом случае нам надо указать 
стрку подключения. По умолчанию Visual Studio добавляет в проекты файл конфигурации. В проектах для десктопных приложений (Windows Forms, 
консольные приложения и т.д.) этот файл называется <span class="bb">App.config</span>, в проектах веб-приложений (ASP.NET) это файл <span class="bb">Web.config</span>. 
Но вне зависимости от типа проекта файл конфигурации имеет определенную структуру и элементы, среди которых также есть также элемент 
<code>connectionStrings</code>, определяющий строки подключения.</p>
<p>Итак, что делать, если мы хотим определить базу данных со случайным именем, которая будет находиться на MS SQL Servere? Добавим в файл 
App.config перед закрывающим тегом <code>configuration</code> следующий код:</p>
<pre class="brush:xml;">
 &lt;connectionStrings&gt;
...
<p>Элемент <code>add</code> добавляет в секцию <code>connectionStrings</code> определение строки подключения к БД. Таких элементов <code>add</code> 
можно определить несколько, если мы планируем использовать различные подключения.</p>
<p>Атрибут <code>name="DefaultConnection"</code> указывает на название подключения. Имя должно согласоваться с контекстом данных, который использует 
данное подключение. Так, в конструктор базового класса (<code>base("DefaultConnection")</code>) мы передаем название DefaultConnection, 
поэтому и подключение носит такое же название. Но если бы определили бы контекст без вызова конструктора базового класса:</p>
<pre class="brush:c#;">
class SoccerContext : DbContext
{
    public SoccerContext() 
...
<p>В этом случае название подключения должно было бы отражать название контекста: <code>name="SoccerContext"</code></p>
<p>Следующий элемент подключения определяет параметры строки подключения. Этот элемент разбивается на несколько частей:</p>
<ul>
<li><p><span class="bb">Data Source</span>: название сервера. Для MS SQL Express этот параметр имеет значение <code>.\SQLEXPRESS</code></p></li>
<li><p><span class="bb">Initial Catalog</span>: название каталога базы данных. В данном случае <code>Players</code>, поэтому при использовании 
подхода Code First на сервере будет созадваться база данных Players.mdf</p></li>
<li><p><span class="bb">Integrated Security</span>: устанавливает проверку подлинности</p></li>
</ul>
<p>И последний элемент устанавливает провайдер: <code>providerName="System.Data.SqlClient"</code></p>
<p>И таким образом, в процессе работы приложения будет создана (если еще не существует) и использоваться база данных Players.</p>
<h3>Строка подключения в Model First и Database First</h3>
<p>При использовании подходов Model First и Database First строка подключения выглядит иначе. Например:</p>
<pre class="brush:xml;">
 &lt;connectionStrings&gt;
...
<p>Здесь для нас важны следующие параметры строки подключения:</p>
<ul>
<li><p>параметр <code>metadata</code>, содержащий метаданные модели (
так как в данном случае модель называется Person, то метаданные включают ресурсы Person.csdl, Person.ssdl, Person.msl)</p></li>
<li><p>параметр <code>data source</code> также устанавливает сервер MS SQL</p></li>
<li><p>а параметр <code>initial catalog</code> также устанавливает каталог базы данных</p></li>
</ul>
<p>И в случае изменения базы данных, ее положения, наименования, или использования другой модели, нам надо соответственно изменить эти параметры.</p>
...
<div class="nav"><p><a href="3.1.php">Назад</a><a href="./">Содержание</a><a href="3.2.php">Вперед</a></p></div></div>
<div class="socBlock">
