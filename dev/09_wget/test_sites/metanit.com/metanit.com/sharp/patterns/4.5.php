...
<html >
<head>
<title>Заместитель (Прокси) | Паттерны в C# и .NET</title>
<meta charset="utf-8" />
...
<h2>Заместитель (Прокси)</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Паттерн Заместитель (Proxy) предоставляет объект-заместитель, который управляет доступом к другому объекту. То есть создается объект-суррогат, который может 
выступать в роли другого объекта и замещать его.</p>
<h3>Когда использовать прокси?</h3>
<ul>
<li><p>Когда надо осуществлять взаимодействие по сети, а объект-проси должен имитировать поведения объекта в другом адресном пространстве. Использование прокси позволяет снизить накладные издержки 
при передачи данных через сеть. Подобная ситуация еще называется <span class="bb">удалённый заместитель (remote proxies)</span></p></li>
<li><p>Когда нужно управлять доступом к ресурсу, создание которого требует больших затрат. Реальный объект создается только тогда, когда он действительно может понадобится, а до этого все 
запросы к нему обрабатывает прокси-объект. Подобная ситуация еще называется <span class="bb">виртуальный заместитель (virtual proxies)</span></p></li>
<li><p>Когда необходимо разграничить доступ к вызываемому объекту в зависимости от прав вызывающего объекта. Подобная ситуация еще называется <span class="bb">защищающий заместитель (protection proxies)</span></p></li>
<li><p>Когда нужно вести подсчет ссылок на объект или обеспечить потокобезопасную работу с реальным объектом. Подобная ситуация 
называется <span class="bb">"умные ссылки" (smart reference)</span></p></li>
</ul>
<p>С помощью UML паттерн может быть описан так:</p>
<img src="pics/proxy.png" alt="Паттерн Заместитель или Прокси в C# и .NET" />
<p>На C# паттерн формально может выглядеть следующим образом:</p>
<pre class="brush:c#;">
class Client
{
    void Main()
...
<h3>Участники паттерна</h3>
<ul>
<li><p><span class="bb">Subject</span>: определяет общий интерфейс для Proxy и RealSubject. Поэтому Proxy может использоваться вместо RealSubject</p></li>
<li><p><span class="bb">RealSubject</span>: представляет реальный объект, для которого создается прокси</p></li>
<li><p><span class="bb">Proxy</span>: заместитель реального объекта. Хранит ссылку на реальный объект, контролирует к нему доступ, 
может управлять его созданием и удалением. При необходимости Proxy переадресует запросы объекту RealSubject</p></li>
<li><p><span class="bb">Client</span>: использует объект Proxy для доступа к объекту RealSubject</p></li>
</ul>
<p>Рассмотрим применение паттерна. Допустим, мы взаимодействуем с базой данных через Entity Framework. У нас есть модель и контекст данных:</p>
<pre class="brush:c#;">
class Page
{
...
<p>Класс Page представляет отдельную страницу книги, у которой есть номер и текст. Взаимодействие с базой данных может уменьшить производительность приложения. Для оптимизации приложения мы можем использовать паттерн Прокси. 
Для этого определим репозиторий и его прокси-двойник:</p>
<pre class="brush:c#;">
class Program
{
...
<p>Итак, здесь определен общий интерфейс IBook для реального объекта и для его прокси-класса. Он определяет один метод <code>GetPage()</code> 
для получения страницы по номеру.</p>
<p>Реальный объект BookStore использует контекст данных для извлечения информации о странице из базы данных. Действие же прокси-класса 
...
и если там страницы не окажется, то идет обращение к реальному объекту BookStore и его методу. То есть фактически будет реализована функциональность кэша страниц.</p>
<p>Клиент, в роли которого в данном случае выступает класс Program, вообще не будет знать, использует ли он функционал класса BookStore или 
его прокси.</p>
<p>В то же время паттерн Прокси имеет недостаток: поскольку иногда будет выполняться сначала функционал прокси, а потом функционал реального объекта, например, 
если страницы не окажется в списке-кэше, то это может привести к замедлению выполнения программы.</p>
