...
<html >
<head>
<title>Адаптер (Adapter) | Паттерны в C# и .NET</title>
<meta charset="utf-8" />
...
<h2>Адаптер (Adapter)</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Паттерн Адаптер (Adapter) предназначен для преобразования интерфейса одного класса в интерфейс другого. Благодаря реализации данного 
паттерна мы можем использовать вместе классы с несовместимыми интерфейсами.</p>
<h3>Когда надо использовать Адаптер?</h3>
<ul>
<li><p>Когда необходимо использовать имеющийся класс, но его интерфейс не соответствует потребностям</p></li>
<li><p>Когда надо использовать уже существующий класс совместно с другими классами, интерфейсы которых не совместимы</p></li>
</ul>
<p>Формальное определение паттерна на UML выглядит следующим образом:</p>
<img src="pics/adapter.png" alt="Паттерн адаптер объектов в C#" />
<p>Формальное описание адаптера объектов на C# выглядит таким образом:</p>
<pre class="brush:c#;">
class Client
{
...
<h3>Участники</h3>
<ul>
<li><p><span class="bb">Target</span>: представляет объекты, которые используются клиентом</p></li>
<li><p><span class="bb">Client</span>: использует объекты Target для реализации своих задач</p></li>
<li><p><span class="bb">Adaptee</span>: представляет адаптируемый класс, который мы хотели бы использовать у клиента вместо объектов Target</p></li>
<li><p><span class="bb">Adapter</span>: собственно адаптер, который позволяет работать с объектами Adaptee как с объектами Target.</p></li>
</ul>
<p> То есть клиент ничего не знает об Adaptee, он знает и использует только объекты Target. И благодаря адаптеру мы можем на клиенте 
использовать объекты Adaptee как Target</p>
<p>Теперь разберем реальный пример. Допустим, у нас есть путешественник, который путешествует на машине. Но в какой-то момент ему приходится 
...
путешественника использование класса верблюда не предусмотрено, поэтому нам надо использовать адаптер:</p>
<pre class="brush:c#;">
class Program
{
...
<p>В данном случае в качестве клиента применяется класс Driver, который использует объект ITransport. Адаптируемым является класс верблюда Camel, 
который нужно использовать в качестве объекта ITransport. И адптером служит класс CamelToTransportAdapter.</p>
<p>Возможно, кому-то покажется надуманной проблема использования адаптеров особенно в данном случае, так как мы могли бы применить 
...
подключения к различным системам баз данных, как раз и реализуют паттерн адаптер (например, класс <code>System.Data.SqlClient.SqlDataAdapter</code>).</p>
