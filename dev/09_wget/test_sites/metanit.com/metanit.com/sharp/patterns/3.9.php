...
<html >
<head>
<title>Посредник (Mediator) | Паттерны в C# и .NET</title>
<meta charset="utf-8" />
...
<h2>Посредник (Mediator)</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Паттерн Посредник (Mediator) представляет такой шаблон проектирования, который обеспечивает взаимодействие множества объектов 
без необходимости ссылаться друг на друга. Тем самым достигается слабосвязанность взаимодействующих объектов.</p>
<p>Когда используется паттерн Посредник?</p>
<ul>
<li><p>Когда имеется множество взаимосвязаных объектов, связи между которыми сложны и запутаны.</p></li>
<li><p>Когда необходимо повторно использовать объект, однако повторное использование затруднено в силу сильных связей с другими объектами.</p></li>
</ul>
<p>Схематично с помощью UML паттерн можно описать следующим образом:</p>
<img src="pics/mediator.png" alt="Паттерн Mediator в C# и .NET" />
<p>Формальная структура классов и связей между ними с применением паттерна на языке C#:</p>
<pre class="brush:c#;">
...
<h3>Участники</h3>
<ul>
<li><p><span class="bb">Mediator</span>: представляет интерфейс для взаимодействия с объектами Colleague</p></li>
<li><p><span class="bb">Colleague</span>: представляет интерфейс для взаимодействия с объектом Mediator</p></li>
<li><p><span class="bb">ConcreteColleague1</span> и <span class="bb">ConcreteColleague2</span>: конкретные классы коллег, 
которые обмениваются друг с другом через объект Mediator</p></li>
<li><p><span class="bb">ConcreteMediator</span>: конкретный посредник, реализующий интерфейс типа Mediator</p></li>
</ul>
<p>Рассмотрим реальный пример. Система создания программных продуктов включает ряд акторов: заказчики, программисты, тестировщики и так далее. 
...
выполняет роль посредника. В этом случае процесс взаимодействия между объектами мы могли бы описать так:</p>
<pre class="brush:c#;">
class Program
{
...
<p>Класс менеджера - ManagerMediator в методе <code>Send()</code> проверяет, от кого пришло сообщение, и в зависимости от 
отправителя перенаправляет его другому объекту с помощью методов <code>Notify()</code>, определенных в классе Colleague.</p>
...
<p>В итоге применение паттерна Посредник дает нам следующие преимущества:</p>
<ul>
<li><p>Устраняется сильная связанность между объектами Colleague</p></li>
<li><p>Упрощается взаимодействие между объектами: вместо связей по типу "все-ко-всем" применяется связь "один-ко-всем"</p></li>
<li><p>Взаимодействие между объектами абстрагируется и выносится в отдельный интерфейс</p></li>
<li><p>Централизуется управления отношениями между объектами</p></li>
</ul>
