...
<html >
<head>
<title>Хранитель (Memento) | Паттерны в C# и .NET</title>
<meta charset="utf-8" />
...
<h2>Хранитель (Memento)</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Паттерн Хранитель (Memento) позволяет выносить внутреннее состояние объекта за его пределы для последующего возможного 
восстановления объекта без нарушения принципа инкапсуляции.</p>
<p>Когда использовать Memento?</p>
<ul>
<li><p>Когда нужно сохранить состояние объекта для возможного последующего восстановления</p></li>
<li><p>Когда сохранение состояния должно проходить без нарушения принципа инкапсуляции</p></li>
</ul>
<p>То есть ключевыми понятиями для данного паттерна являются <span class="bb">сохранение внутреннего состояния</span> и <span class="bb">инкапсуляция</span>, 
...
В этом случае как раз и может помочь паттерн Хранитель.</p>
<p>С помощью диаграмм структуру паттерна можно изобразить следующим образом:</p>
<img src="pics/memento.png" alt="Паттерн Memento в C# и .NET" />
<p>Формальная структура паттерна на языке C#:</p>
<pre class="brush:c#;">
class Memento
{
...
<h3>Участники</h3>
<ul>
<li><p><span class="bb">Memento</span>: хранитель, который сохраняет состояние объекта Originator и предоставляет полный доступ только 
этому объекту Originator</p></li>
<li><p><span class="bb">Originator</span>: создает объект хранителя для сохранения своего состояния</p></li>
<li><p><span class="bb">Caretaker</span>: выполняет только функцию хранения объекта Memento, в то же время у него нет полного доступа к хранителю и никаких других операций над хранителем, кроме собственно сохранения, 
он не производит</p></li>
</ul>
<p>Теперь рассмотрим реальный пример: нам надо сохранять состояние игрового персонажа в игре:</p>
<pre class="brush:c#;">
class Program
{
...
<p>Здесь в роли Originator выступает класс Hero, состояние которого описывается количество патронов и жизней. Для хранения 
состояния игрового персонажа предназначен класс HeroMemento. С помощью метода <code>SaveState()</code> объект Hero может 
сохранить свое состояние в HeroMemento, а с помощью метода <code>RestoreState()</code> - восстановить.</p>
<p>Для хранения состояний предназначен класс GameHistory, причем все состояния хранятся в стеке, что позволяет с легкостью извлекать последнее сохраненное состояние.</p>
<p>Использование паттерна Memento дает нам следующие преимущества:</p>
<ul>
<li><p>Уменьшение связанности системы</p></li>
<li><p>Сохранение инкапсуляции информации</p></li>
<li><p>Определение простого интерфейса для сохранения и восстановления состояния</p></li>
</ul>
<p>В то же время мы можем столкнуться с недостатками, в частности, если требуется сохранение большого объема информации, то 
возрастут издержки на хранение всего объема состояния.</p>
