...
<html >
<head>
<title>Интерпретатор (Interpreter) | Паттерны в C# и .NET</title>
<meta charset="utf-8" />
...
<h2>Интерпретатор (Interpreter)</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Паттерн Интерпретатор (Interpreter) определяет представление грамматики для заданного языка и интерпретатор предложений этого языка. Как правило, 
данный шаблон проектирования применяется для часто повторяющихся операций.</p>
<p>Хотя паттерн требует понимания теории формальных языков и грамматик, на самом деле он не так сложен в понимании.</p>
<p>С помощью диаграмм UML паттерн можно описать так:</p>
<img src="pics/interpreter.png" alt="Паттерн Interpreter в C# и .NET" />
<p>На языке C# формальная структура программы могла бы выглядеть следующим образом:</p>
<pre class="brush:c#;">
class Client
{
    void Main()
...
<h3>Участники</h3>
<ul>
<li><p><span class="bb">AbstractExpression</span>: определяет интерфейс выражения, объявляет метод <code>Interpret()</code></p></li>
<li><p><span class="bb">TerminalExpression</span>: терминальное выражение, реализует метод <code>Interpret()</code> для терминальных 
символов грамматики. Для каждого символа грамматики создается свой объект TerminalExpression</p></li>
<li><p><span class="bb">NonterminalExpression</span>: нетерминальное выражение, представляет 
правило грамматики. Для каждого отдельного правила грамматики создается свой объект NonterminalExpression.</p></li>
<li><p><span class="bb">Context</span>: содержит общую для интерпретатора информацию. Может использоваться объектами терминальных и 
нетерминальных выражений для сохранения состояния операций и последующего доступа к сохраненному состоянию</p></li>
<li><p><span class="bb">Client</span>: строит предложения языка с данной грамматикой в виде абстрактного синтаксического дерева, 
узлами которого являются объекты TerminalExpression и NonterminalExpression</p></li>
</ul>
<p>Методы Interpret в нетерминальных выражениях позволяют реализовать правила грамматики. При этом мы легко может добавить 
...
следует выбирать другие способы проектирования приложения.</p>
<p>Например, нам надо разработать программ для вычислений простейших операций сложения и вычитания с помощью переменных: <code>x + y - z</code>. Для этого 
можно определить следующую грамматику:</p>
<pre>
...
<p>Реализация этой грамматики на языке C# будет следующая программа:</p>
<pre class="brush:c#;">
class Program
{
...
<p>В данном случае все действия интерпретатора можно описать следующим деревом:</p>
<img src="pics/interpretertree.png" alt="Синтаксическое дерево в Interpeter" />
<p>Класс Context определяет методы для установки значений переменных и для получения их значений.</p>
<p>В качестве интерпретатора используется интерфейс <code>IExpression</code>. Его реализация - класс NumberExpression предназначен 
для выражения отдельных переменных - это терминальные объекты.</p>
<p>Другие реализации интерфейса - классы AddExpression и SubtractExpression представляют нетерминальные объекты. 
Они реализуют простейшие операции сложения и вычитания и могут рекурсивно обращаться к методам Interpret используемых 
терминальных и нетерминальных объектов.</p>
<p>Клиент, в роли которого выступает класс Program, инициализирует контекст и для вычисления выражения x + y - z создается объект SubtractExpression, который в качестве параметров принимает другие объекты IExpression.</p>
