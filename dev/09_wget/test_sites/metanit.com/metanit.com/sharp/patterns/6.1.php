...
<html >
<head>
<title>Паттерны в C# и .NET | Fluent Builder</title>
<meta charset="utf-8" />
<meta name="description" content="Паттерн Fluent Builder в C# и .NET, создание объектов, ограничение конструкторов, способы реализации">
...
<h1>Дополнительные паттерны</h1><h2>Fluent Builder</h2><div class="date">Последнее обновление: 19.10.2017</div>
...
<p>Паттерн Fluent Builder позволяет упростить процесс создания сложных объектов с помощью методов-цепочек, которые наделяют объект каким-то определенным качеством. 
Применение данного паттерна делает процесс конструирования объектов более прозрачным, а код более читабельным.</p>
<p>Исторически fluent builder прежде всего позволял решить проблему перегруженных конструкторов.</p>
<p>Возьмем следующий класс User:</p>
<pre class="brush:c#;">
public class User
{
...
<p>С какими проблемами мы можем столкнуться? Если необходимо иницилизировать множества свойств объекта, то конструктор может 
принимать много параметров. В данном случае для упрощения примера используются только четыре параметра и свойства, но 
соответственно их может быть гораздо больше. Такой код сложнее поддерживать. Мы зависим от параметров и действий, 
которые с этими параметрами производятся в конструкторе. Кроме того, конструктор может содержать дополнительную логику по проверке и установке значений свойств и переменных. Если надо выполнить 
много подобных действий, то код конструктора раздувается.</p>
<p>В качестве одного из решений данной проблемы можно использовать паттерн Fluent Builder. Для этого определим следующий класс UserBuilder:</p>
<pre class="brush:c#;">
...
<p>С помощью ряда методов класс инициализирует различные свойства объекта User. Причем каждый подобный метод возвращает текущий объект 
с помощью вызова <span class="b">return this</span>. При этом для этой цели можно использовать не только методы, но и свойства.</p>
<p>Для возвращения созданного объекта обычно определяется метод под названием Build.</p>
<p>Также изменим класс User:</p>
<pre class="brush:c#;">
public class User
{
	public string Name { get; set; }        // имя
...
<p>Класс больше не имеет никаких конструкторов, кроме конструктора по умолчанию, и также определяет статический метод, который возвращает объект 
UserBuilder.</p>
...
<p>В первом случае напрямую используется объект UserBuilder, а во втором - статический метод User.CreateBuilder. 
Такой подход является более интуитивно понятным в плане того, что и для какого свойства объекта устанавливается.</p>
<p>C# позволяет определять перегрузку операций преобразования, поэтому вместо возвращения объекта через метод Build 
определим операцию преобразования:</p>
<pre class="brush:c#;">
public class UserBuilder
{
	private User user;
...
<p>Теперь метод Build можно не использовать:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="5.5.php">Назад</a><a href="./">Содержание</a><a href="./7.1.php">Вперед</a></p></div></div>
<div class="socBlock">
