...
<html >
<head>
<title>Прототип (Prototype) | Паттерны в C# и .NET</title>
<meta charset="utf-8" />
...
<h2>Прототип (Prototype)</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Паттерн Прототип (Prototype) позволяет создавать объекты на основе уже ранее созданных объектов-прототипов. То есть по сути данный паттерн предлагает 
технику клонирования объектов.</p>
<p>Когда использовать Прототип?</p>
<ul>
<li><p>Когда конкретный тип создаваемого объекта должен определяться динамически во время выполнения</p></li>
<li><p>Когда нежелательно создание отдельной иерархии классов фабрик для создания объектов-продуктов из параллельной иерархии классов 
(как это делается, например, при использовании паттерна Абстрактная фабрика)</p></li>
<li><p>Когда клонирование объекта является более предпочтительным вариантом нежели его создание и инициализация с помощью конструктора. Особенно когда известно, что объект может принимать небольшое ограниченное число 
возможных состояний.</p></li>
</ul>
<p>На языке UML отношения между классами при применении данного паттерна можно описать следующим образом:</p>
<img src="pics/prototype.png" alt="Паттерн Prototype в C# и .NET" />
...
<h3>Участники</h3>
<ul>
<li><p><span class="bb">Prototype</span>: определяет интерфейс для клонирования самого себя, который, как правило, представляет метод 
<code>Clone()</code></p></li>
<li><p><span class="bb">ConcretePrototype1</span> и <span class="bb">ConcretePrototype2</span>: конкретные реализации прототипа. Реализуют метод <code>Clone()</code></p></li>
<li><p><span class="bb">Client</span>: создает объекты прототипов с помощью метода <code>Clone()</code></p></li>
</ul>
<p>Рассмотрим клонирование на примере фигур - прямоугольников и кругов:</p>
<pre class="brush:c#;">
class Program
{
...
<p>Но в данном случае надо заметить, что фреймворк .NET предлагает функционал для копирования в виде метода <span class="bb">MemberwiseClone()</span>. 
Например, мы могли бы изменить реализацию метода <code>Clone()</code> в классах прямоугольника и круга следующим образом:</p>
<pre class="brush:c#;">
public IFigure Clone()
{
...
<p>В то же время надо учитывать, что метод <code>MemberwiseClone()</code> осуществляет неполное копирование - то есть копирование значимых типов. 
...
пусть фигура круг имеет свойство ссылочного типа:</p>
<pre class="brush:c#;">
class Point
{
...
<p>В этом случае при изменении значений в свойстве Point начальной фигуры автоматически бы изменилось соответствующее значение и у клонированной 
фигуры:</p>
<pre class="brush:c#;">
...
<p>Чтобы вручную не создавать у клонированного объекта вложенный объект Point, здесь используются механизмы бинарной 
сериализации. И в этом случае все классы, объекты которых подлежат копированию, должны быть помечены атрибутом Serializable.</p>
