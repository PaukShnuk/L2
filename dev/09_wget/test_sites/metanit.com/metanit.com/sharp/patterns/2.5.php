...
<html >
<head>
<title>Строитель (Builder) | Паттерны в C# и .NET</title>
<meta charset="utf-8" />
...
<h2>Строитель (Builder)</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<h3>Когда использовать паттерн Строитель?</h3>
<ul>
<li><p>Когда процесс создания нового объекта не должен зависеть от того, из каких частей этот объект состоит и как эти части связаны между собой</p></li>
<li><p>Когда необходимо обеспечить получение различных вариаций объекта в процессе его создания</p></li>
</ul>
<p>Формально в UML паттерн мог бы выглядеть следующим образом:</p>
<img src="pics/builder.png" alt="Паттерн Строитель в C# и .NET" />
...
<h3>Участники</h3>
<ul>
<li><p><span class="bb">Product</span>: представляет объект, который должен быть создан. В данном случае все части объекта заключены 
в списке parts.</p></li>
<li><p><span class="bb">Builder</span>: определяет интерфейс для создания различных частей объекта Product</p></li>
<li><p><span class="bb">ConcreteBuilder</span>: конкретная реализация Buildera. Создает объект Product и определяет интерфейс 
для доступа к нему</p></li>
<li><p><span class="bb">Director</span>: распорядитель - создает объект, используя объекты Builder</p></li>
</ul>
<p>Рассмотрим применение паттерна на примере выпечки хлеба. Как известно, даже обычный хлеб включает множество компонентов. 
Мы можем использовать для представления хлеба и его компонентов следующие классы:</p>
<pre class="brush:c#;">
//мука
class Flour
{
...
<p>Хлеб может иметь различную комбинацию компонентов: ржаной и пшеничной муки, соли, пищевых добавок. И нам надо обеспечить выпечку 
...
И для этой задачи применим паттерн Builder:</p>
<pre class="brush:c#;">
class Program
{
...
<p>В данном случае с помощью конкретных строителей RyeBreadBuilder и WheatBreadBuilder создаются объекты Bread с определенным набором. 
В роли распорядителя выступает класс пекаря Baker, который вызывает методы конкретных строителей для построения нового объекта.</p>
