...
<html >
<head>
<title>Компоновщик (Composite) | Паттерны в C# и .NET</title>
<meta charset="utf-8" />
...
<h2>Компоновщик (Composite)</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Паттерн Компоновщик (Composite) объединяет группы объектов в древовидную структуру по принципу "часть-целое и позволяет клиенту 
одинаково работать как с отдельными объектами, так и с группой объектов.</p>
<p>Образно реализацию паттерна можно представить в виде меню, которое имеет различные пункты. Эти пункты могут содержать подменю, в которых, в свою 
...
однообразно можем работать как с пунктом меню, так и со всем меню в целом.</p>
<h3>Когда использовать компоновщик?</h3>
<ul>
<li><p>Когда объекты должны быть реализованы в виде иерархической древовидной структуры</p></li>
<li><p>Когда клиенты единообразно должны управлять как целыми объектами, так и их составными частями. То есть целое и его части должны 
реализовать один и тот же интерфейс</p></li>
</ul>
<p>С помощью UML шаблон компоновщик можно представить следующим образом:</p>
<img src="pics/composite.png" alt="Паттерн Компоновщик в C# и .NET" />
<p>Формальное определение паттерна на C# могло бы выглядеть так:</p>
<pre class="brush:c#;">
class Client
{
    public void Main()
...
<h3>Участники</h3>
<ul>
<li><p><span class="bb">Component</span>: определяет интерфейс для всех компонентов в древовидной структуре</p></li>
<li><p><span class="bb">Composite</span>: представляет компонент, который может содержать другие компоненты и реализует механизм 
для их добавления и удаления</p></li>
<li><p><span class="bb">Leaf</span>: представляет отдельный компонент, который не может содержать другие компоненты</p></li>
<li><p><span class="bb">Client</span>: клиент, который использует компоненты</p></li>
</ul>
<p>Рассмотрим простейший пример. Допустим, нам надо создать объект файловой системы. Файловую систему составляют папки и файлы. 
Каждая папка также может включать в себя папки и файлы. То есть получается древовидная иерархическая структура, 
где с вложенными папками нам надо работать также, как и с папками, которые их содержат. Для реализации данной задачи и воспользуемся паттерном Компоновщик:</p>
<pre class="brush:c#;">
class Program
{
...
<p>В итоге подобная система обладает неплохой гибкостью: если мы захотим добавить новый вид компонентов, нам достаточно унаследовать новый класс от Component.</p>
<p>И также применяя компоновщик, мы легко можем обойти все узлы древовидной структуры.</p>
