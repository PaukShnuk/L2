...
<html >
<head>
<title>Состояние (State) | Паттерны в C# и .NET</title>
<meta charset="utf-8" />
...
<h2>Состояние (State)</h2><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Состояние (State) - шаблон проектирования, который позволяет объекту изменять свое поведение в зависимости от внутреннего состояния.</p>
<h3>Когда применяется данный паттерн?</h3>
<ul>
<li><p>Когда поведение объекта должно зависеть от его состояния и может изменяться динамически во время выполнения</p></li>
<li><p>Когда в коде методов объекта используются многочисленные условные конструкции, выбор которых зависит от текущего состояния объекта</p></li>
</ul>
<p>UML-диаграмма данного шаблона проектирования предлагает следующую систему:</p>
<img src="pics/state.png" alt="Паттерн Состояние в C# и .NET" />
...
<ul>
<li><p><span class="bb">State</span>: определяет интерфейс состояния</p></li>
<li><p>Классы <span class="bb">StateA</span> и <span class="bb">StateB</span> - конкретные реализации состояний</p></li>
<li><p><span class="bb">Context</span>: представляет объект, поведение которого должно динамически изменяться в соответствии с состоянием. 
Выполнение же конкретных действий делегируется объекту состояния</p></li>
</ul>
<p>Например, вода может находиться в ряде состояний: твердое, жидкое, парообразное. Допустим, нам надо определить 
класс Вода, у которого бы имелись методы для нагревания и заморозки воды. Без использования паттерна Состояние мы 
могли бы написать следующую программу:</p>
<pre class="brush:c#;">
class Program
{
...
<p>Вода имеет три состояния, и в каждом методе нам надо смотреть на текущее состояние, чтобы произвести действия. В итоге с трех состояний уже 
...
чтобы сделать программу более гибкой, в данном случае мы можем применить паттерн Состояние:</p>
<pre class="brush:c#;">
class Program
{
    static void Main(string[] args)
...
<p>Таким образом, реализация паттерна Состояние позволяет вынести поведение, зависящее от текущего состояния объекта, в отдельные классы, и избежать 
