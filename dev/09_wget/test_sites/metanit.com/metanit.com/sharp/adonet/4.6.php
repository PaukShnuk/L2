...
<html >
<head>
<title>Методы ExecuteCommand и ExecuteQuery. Хранимые процедуры в LINQ to SQL и C#</title>
<meta charset="utf-8" />
<meta name="description" content="Использование методов ExecuteCommand и ExecuteQuery для выполнения запросов, хранимые процедуры в LINQ to SQL и C#">
...
<h2>Методы ExecuteCommand и ExecuteQuery. Хранимые процедуры</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для непосредственного выполнения кода sql в классе DataContext определены методы <code>ExecuteCommand()</code> и 
<code>ExecuteQuery()</code>.</p>
<p>Метод <code>ExecuteCommand()</code> принимает выполняемое sql-выражение и список параметров:</p>
<pre class="brush:c#;">
...
<p>В данном случае удаляется объект по id, который равен 40. Возвращаемое методом число указывает на количество затронутых командой строк.</p>
<p>Метод <code>ExecuteQuery()</code> имеет похожее применение, только в этом случае он возвращает результат выполнения запроса SELECT:</p>
<pre class="brush:c#;">
DataContext db = new DataContext(connectionString);
IEnumerable&lt;User&gt; users = db.ExecuteQuery&lt;User&gt;("SELECT * FROM Users WHERE Age&gt;{0}", 23);
</pre>
<h3>Хранимые процедуры</h3>
<p>Допустим, у нас в базе данных определена следующая хранимая процедура:</p>
<pre class="brush:sql;">
...
<p>Данная процедура по переданному имени вычисляет минимальный и максимальный возраст для пользователей с данным именем.</p>
<p>Чтобы облегчить работу с хранимыми процедурами мы можем создать свой подкласс, унаследовав его от DataContext:</p>
<pre class="brush:c#;">
...
<p>Для выполнения хранимой процедуры определяется функция GetAgeRange. И чтобы она сопоставлялась с процедурой к ней применяется атрибут 
<span class="b">[Function]</span> с указанием имени процедуры.</p>
<p>Функция GetAgeRange принимает ряд параметров. И к каждому параметру принимает атрибут <span class="b">Parameter</span>, который позволяет 
сопоставить параметр функции с параметром в хранимой процедуре. Так как параметры minAge и maxAge являются выходными, то они определяются 
с ключевым словом <span class="b">ref</span>.</p>
<p>Применим эту функцию в программе:</p>
<pre class="brush:c#;">
string connectionString = @"Data Source=.\SQLEXPRESS;Initial Catalog=usersdb;Integrated Security=True";
...
<img src="pics/l2sstoredprocedure.png" alt="Хранимые процедуры в LINQ to SQL" />
...
<div class="nav"><p><a href="4.5.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
