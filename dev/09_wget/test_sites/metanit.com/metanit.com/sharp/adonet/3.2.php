...
<html >
<head>
<title>Постраничный просмотр в SqlDataAdapter в ADO.NET и C#</title>
<meta charset="utf-8" />
<meta name="description" content="Создание постраничной навигации с помощью SqlDataAdapter в ADO.NET и C#">
...
<h2>Постраничный просмотр в SqlDataAdapter</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>В прошлой теме была рассмотрена загрузка данных через SqlDataAdapter в DataSet с последующим отображением в DataGridView. Однако если 
в базе данных очень много строк, то для более комфортной работы может потребоваться разбить эти данные на отдельные куски или страницы. 
Используя DataGridView очень легко сделать постраничный просмотр объект. Для этого определим на форме DataGridView и две кнопки - backButton и nextButton 
соответственно для перехода назад и вперед.</p>
<p>Затем определим следующий код формы:</p>
<pre class="brush:c#;">
using System;
using System.Data;
...
<p>Переменная pageSize определяет количество строк на одной странице, а переменная pageNumber будет хранить номер текущей просматриваемой страницы.</p>
<p>Запрос к базе данных будет создаваться с помощью функции <code>GetSql()</code>.</p>
<p>В обработчике кнопки Вперед увеличивается текущая страница на единицу и производится запрос. В обработчике кнопки Назад уменьшается счетчик текущей страницы.</p>
<img src="pics/sqlpaging.png" alt="Постраничная навигация с MS SQL Server" />
