...
<html >
<head>
<title>Отношения между таблицами в DataSet в ADO.NET и C#</title>
<meta charset="utf-8" />
<meta name="description" content="Отношения между объектами в таблицах DataSet в ADO.NET и C#, использование внешних ключей для создания отношений, класс ForeignKeyConstraint ">
...
<h2>Отношения между таблицами в DataSet</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>DataSet может содержать множество таблиц, которые могут быть связаны различными отношениями. Например, пусть у нас в dataset будет определена 
таблица производителей смартфонов и таблица самих смартфонов, которая связана с первой:</p>
<pre class="brush:c#;">
...
<img src="pics/datarelation.png" alt="Отношения между таблицами в DataSet" />
<p>Ключевым моментом здесь является установка отношения между таблицами:</p>
<pre class="brush:c#;">ds.Relations.Add("PhonesCompanies", companiesTable.Columns["Id"], phonesTable.Columns["CompanyId"]);</pre>
<p>Первым параметром здесь идет название отношения, по которому потом на него можно будет ссылаться. 
Вторым параметром идет главный столбец - id компании в таблице компаний. Третий параметр - подчиненный столбец - id компании в таблице 
смартфонов. Подчиненный столбец зависит от главного. Вместо этих параметров мы можем передать объект DataRelation, 
который также будет иметь данные значения.</p>
<p>Затем используя отношение, мы можем получить для одной строки из таблицы компаний связанные строки из таблицы смартфонов:</p>
<pre class="brush:c#;">DataRow[] rows = apple.GetChildRows(ds.Relations["PhonesCompanies"]);</pre>
<h3>Внешние ключи</h3>
<p>Кроме простого отношения между таблицами мы также можем задать внешние ключи, как в обычных базах данных. Для этого непосредственно перед строкой:</p>
<pre class="brush:c#;">ds.Relations.Add("PhonesCompanies", companiesTable.Columns["Id"], phonesTable.Columns["CompanyId"]);</pre>
...
<p>Внешний ключ представляет объект ForeignKeyConstraint, в конструктор которого передается два параметра: главный столбец и зависимый столбец. 
В данном случае это те же самые столбцы.</p>
<p>Также у него мы можем установить ряд свойств. Через свойство <code>ConstraintName</code> устанавливается название внешнего ключа. 
Свойства <code>DeleteRule</code> и <code>UpdateRule</code> определют поведение при удалении или обновлении данных соответственно в главной таблице (то есть в 
данном случае в таблице компаний). Эти свойства принимают одно из значений перечисления Rule:</p>
<ul>
<li><p><code>Cascade</code>: происходит автоматическое изменение или удаление строк в подчиненной таблице</p></li>
<li><p><code>None</code>: строки в подчиненной таблице не изменяются</p></li>
<li><p><code>SetDefault</code>: происходит установка значений по умолчанию для связанных строк (которое определяется через DataColumn.DefaultValue)</p></li>
<li><p><code>SetNull</code>: для связанных строк устанавливается значение DBNull</p></li>
</ul>
<p>С помощью метода <code>ds.Tables["Phones"].Constraints.Add(foreignKey);</code> внешний ключ добавляется к таблице смартфонов. И затем 
через выражение <code>ds.EnforceConstraints = true;</code> происходит применение внешнего ключа.</p>
<p>И если мы удалим, например, одну из строк из таблицы компаний, то в таблице смартфонов все строки, которые связаны с удаленной, в столбце 
CompanyId будут иметь значение null.</p>
