...
<html >
<head>
<title>DataSet и XML в C#</title>
<meta charset="utf-8" />
<meta name="description" content="Загрузка xml-файла в DataSet и обратное сохранение данных в xml в ADO.NET и C#">
...
<h2>DataSet и XML</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Встроенные возможности DataSet позволяют производить некоторые операции с xml-документами, в частности, сохранять данные из dataset в xml и, наоборот, 
загружать из xml в dataset. Подобные возможности позволяют при необходимости использовать dataset в качестве посредника между базой данных 
и xml-файлами.</p>
<p>Сохранение данных из dataset в xml осуществляется с помощью метода <span class="bb">WriteXml</span>. Например, загрузим данные из БД в DataSet и 
затем сохраним их в xml:</p>
<pre class="brush:c#;">
...
<p>В метод <code>WriteXml()</code> передается имя файла, который после записи будет выглядеть примерно следующим образом:</p>
<pre class="brush:xml;">
...
<p>Для считывания данных из xml применяется метод <span class="b">ReadXml()</span>:</p>
<pre class="brush:c#;">
