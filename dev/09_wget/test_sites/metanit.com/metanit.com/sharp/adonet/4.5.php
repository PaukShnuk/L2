...
<html >
<head>
<title>Удаление в LINQ to SQL и C#</title>
<meta charset="utf-8" />
<meta name="description" content="Удаление объектов в LINQ to SQL и C#, метод DeleteOnSubmit">
...
<h2>Удаление в LINQ to SQL</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для удаления объекта из базы данных применяется метод <span class="b">DeleteOnSubmit()</span> или <span class="b">DeleteAllOnSubmit()</span>, 
если удаляется список объектов.</p>
<pre class="brush:c#;">
...
<p>В данном случае удаляется последний элемент из таблицы. В итоге при удалении будет сгенерировано и выполнено следующее 
sql-выражение:</p>
<pre class="brush:sql;">
...
<p>Правда, у данного способа есть большой недостаток: если мы захотим удалить объект по какому-либо признаку, например, по id, 
то вначале нам надо получить этот объект и потом уже передать его в метод <code>DeleteOnSubmit()</code>. При этом будут происходить фактически два обращения к бд: 
на получение объекта и на его удаление. Чтобы избежать ненужного запроса мы можем использовать хранимые процедуры или прямые запросы sql.</p>
