...
<html >
<head>
<title>Операции с данными в LINQ to SQL и C#</title>
<meta charset="utf-8" />
<meta name="description" content="Основные операции с данными в LINQ to SQL и C#, добавление, удаление, редактирование, вывод и фильтрация объектов">
...
<h2>Операции с набором в LINQ to SQL</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Чтобы применить фильтрацию и сортировку в LINQ to SQL, мы можем воспользоваться стандартными операторами и выражениями LINQ. Например, 
выведем всех пользователей, у которых возраст больше 25 и упорядочим по имени:</p>
<pre class="brush:c#;">
...
<p>Операторы where и orderby позволяют произвести сортировку. Причем в результате действия этого запроса будет сформировано 
sql-выражение:</p>
<pre class="brush:sql;">
...
<p>Которое и возвратит все необходимые данные.</p>
<h3>Группировка</h3>
<p>Для группировки данных применяется оператор <code>group by</code> или аналогичный метод <code>GroupBy()</code>:</p>
<pre class="brush:c#;">
string connectionString = @"Data Source=.\SQLEXPRESS;Initial Catalog=usersdb;Integrated Security=True";
DataContext db = new DataContext(connectionString);
...
<img src="pics/l2sgroup.png" />
<p>В результате выполнения выражения linq будет сформировано и выполнено следующее sql-выражение:</p>
<pre class="brush:sql;">
...
<p>Здесь сначала с помощью метода <code>Count()</code> получаем общее количество объектов в базе данных. Затем с помощью метода <code>Skip()</code> 
пропускаем нужное число элементов, а через метод <code>Take()</code> извлекаем нужную порцию объектов.</p>
