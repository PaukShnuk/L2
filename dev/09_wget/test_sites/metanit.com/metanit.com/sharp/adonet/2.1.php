...
<html >
<head>
<title>Создание базы данных в MS SQL Server</title>
<meta charset="utf-8" />
<meta name="description" content="Создание базы данных MS SQL Server для работы в ADO.NET">
...
<h1>C# и MS SQL Server</h1>
<h2>Создание базы данных</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Для работы с базами данных нам естественно прежде всего надо иметь какую-нибудь базу данных. В данном случае мы будем рассматривать 
...
для установки можно найти по адресу <a href="https://www.microsoft.com/en-US/download/details.aspx?id=42299">https://www.microsoft.com/en-US/download/details.aspx?id=42299</a>. 
Вместе с сервером также устанавливается специальная программа SQL Server Management Studio, которая используется для управления базами данных на сервере.</p>
<p>Для начала создадим простейшую базу данных на MS SQL Server. Для этого откроем SQL Server Management Studio и нажмем на узел Databases 
правой кнопкой мыши. После этого в открывшемся контекстном меню выберем <span class="bb">New Database</span>:</p>
<img src="pics/sqlservermanagementstudio.png" alt="Создание базы данных в SQL Server Management Studio" />
<p>После этого нам открывается окно для создания базы данных:</p>
<img src="pics/createdatabase.png" />
<p>В нем в поле Database Name нам надо ввести название базы данных. Введем <span class="bb">usersdb</span>. Больше здесь вводить ничего не требуется, 
и поэтому нажмем на ОК.</p>
<p>После этого в узле Databases появляется новый элемент, который представляет только что созданную базу данных usersdb. Раскроем его и нажмем правой кнопкой 
мыши на его подузел Tables:</p>
<img src="pics/createtable.png" alt="Создание таблицы в SQL Server Management Studio" />
<p>Затем нам открывается дизайнер таблицы:</p>
<img src="pics/newtable.png" />
<p>В нем надо указать три столбца: Id, Name и Age, которые будут представлять соответственно уникальный идентификатор пользователя, 
его имя и возраст. У первого и третьего столбца надо указать тип <code>int</code> (то есть целочисленный), а у столбца Name - тип <code>nvarchar</code> (строковый).</p>
<p>Кроме того, в окне свойств таблицы в поле Name надо ввести имя таблицы - Users, а в поле Identity ввести Id, то есть тем самым указывая, 
что столбец Id будет идентификатором.</p>
<p>И в конце нам надо установить курсор на столбец Id и в панели инструментов программы нажать на золотой ключик. После этого напротив поля Id также 
должен появиться золотой ключик. Этот ключик будет указывать, что столбец Id будет выполнять роль первичного ключа.</p>
<p>После этого нажмем на сохранение и затем на клавишу F5 (обновление), и в узле нашей базы данных появится новая таблица, которая будет называться dbo.Users:</p>
<img src="pics/newtable2.png" />
<p>Итак, мы создали базу данных и таблицу, которые будут выполнять функции по хранению данных. И теперь мы можем использовать инфраструктуру 
ADO.NET для подключения к ним.</p>
