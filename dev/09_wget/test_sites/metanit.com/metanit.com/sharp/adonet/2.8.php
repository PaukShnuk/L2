...
<html >
<head>
<title>Получение скалярных значений в ADO.NET и C#</title>
<meta charset="utf-8" />
<meta name="description" content="Получение результатов операций Min, Max, Count, Sum в MS SQL Server в ADO.NET и C#">
...
<h2>Получение скалярных значений</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>При отправке запросов мы можем использовать специальные встроенные функции SQL, например, Min, Max, Sum, Count и т.д., которые не 
выполняют операции с объектами и не извлекают объекты, а возвращают какое-то определенное значение. Например, функция 
Count подсчитывает количество объектов. И для работы с такими функциями в SqlCommand определен специальный метод <code>ExecuteScalar</code>. 
Например, найдем число всех объектов в таблице и получим минимальный возраст пользователей:</p>
<pre class="brush:c#;">
...
находит минимальное значение столбца Age. В качестве результата метод <code>ExecuteScalar()</code> возвращает объект типа  <code>object</code>.</p>
<p>И если мы захотим получать данные асинхронным образом, то мы можем использовать асинхронную версию этого метода - <span class="bb">ExecuteScalarAsync()</span>.</p>
