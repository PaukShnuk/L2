...
<html >
<head>
<title>Добавление в LINQ to SQL и C#</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление объектов в LINQ to SQL и C#, метод InsertOnSubmit">
...
<h2>Добавление в LINQ to SQL</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Чтобы добавить новый объект в базу данных, необходимо вызвать у таблицы в контексте данных метод <span class="b">InsertOnSubmit()</span> 
или <span class="b">InsertAllOnSubmit()</span>(если надо добавить список объектов).</p>
<pre class="brush:c#;">
...
<img src="pics/l2sadd.png" alt="Добавление в LINQ to SQL" />
<p>При добавлении генерируется следующее sql-выражение:</p>
<pre class="brush:sql;">
...
<p>Причем данное выражение возвращает id добавленной записи, поэтому после добавления у объекта User мы сможем получить Id:</p>
<pre class="brush:c#;">
User user1 = new User { FirstName = "Ronald", Age = 34 };
db.GetTable&lt;User&gt;().InsertOnSubmit(user1);
db.SubmitChanges();
...
<p>Здесь важно отметить, что если в базе данных значение какого-либо столбца, например, столбца для Id, должно генерироваться автоматически, 
то в классе модели атрибут Colunm над соответствующим свойством должен иметь значение <code>IsDbGenerated = true</code>:</p>
<pre class="brush:c#;">
