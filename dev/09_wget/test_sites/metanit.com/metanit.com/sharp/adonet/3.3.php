...
<html >
<head>
<title>SqlCommandBuilder и сохранение изменений DataSet в базе данных в C#</title>
<meta charset="utf-8" />
<meta name="description" content="Сохранение всех изменений из DataSet в базу данных с помощью SqlCommandBuilder в ADO.NET и C#">
...
<h2>SqlCommandBuilder и сохранение изменений DataSet в базе данных</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Получив данные в DataSet, мы можем производить с ними различными операции: удалять, изменять, добавлять новые записи. Однако все делаемые нами изменения 
автоматически не будут сохраняться в БД. Для этого нам еще надо вызвать метод <span class="bb">Update</span> объекта SqlDataAdapter, который заполнял DataSet.</p>
<p>Для модификации данных в БД в соответствии с изменениями в DataSet <code>SqlDataAdapter</code> использует команды <code>InsertCommand</code>, 
<code>UpdateCommand</code> и <code>DeleteCommand</code>. Мы можем сами определить для этих команд sql-выражения, либо мы можем воспользоваться 
классом <span class="b">SqlCommandBuilder</span>, который позволяет автоматически сгенерировать нужные выражения. Используем SqlCommandBuilder:</p>
<pre class="brush:c#;">
...
<img src="pics/sqlcommandbuilder.png" alt="SqlCommandBuilder в ADO.NET" />
<p>Здесь после загрузки данных создается новая строка, которая затем добавляется в DataTable. При вызове у адаптера метода <code>Update()</code> 
...
делает SqlCommandBuilder. Для применения этого класса достаточно вызвать его конструктор, в который передается нужный адаптер:</p>
<pre class="brush:c#;">SqlCommandBuilder commandBuilder = new SqlCommandBuilder(adapter);</pre>
<p>Причем больше нигде в коде вы этот объект не вызываем.</p>
<p>При необходимости мы можем получить sql-выражения используемых команд:</p>
<pre class="brush:c#;">
...
<p>В моем случае команда обновления будет выглядеть так:</p>
<pre class="brush:sql;">UPDATE [Users] SET [Name]=@p1, [Age]=@p2 WHERE (([Id]=@p3) AND ([Name]=@p4) AND ([Age]=@p5))</pre>
<p>Команда вставки:</p>
<pre class="brush:sql;">INSERT INTO [Users] ([Name],[Age]) VALUES (@p1, @p2)</pre>
<p>Команда удаления:</p>
<pre class="brush:sql;">DELETE FROM [Users] WHERE (([Id]=@p1) AND ([Name]=@p2) AND ([Age]=@p3))</pre>
