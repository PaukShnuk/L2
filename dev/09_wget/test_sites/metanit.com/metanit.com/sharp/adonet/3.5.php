...
<html >
<head>
<title>Все операции с БД в графическом приложении в C#</title>
<meta charset="utf-8" />
<meta name="description" content="Выборка, сохранение, обновление и удаление в базу данных MS SQL Server в графическом приложении на C#">
...
<h2>Все операции с БД в графическом приложении</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Ранее мы рассмотрели, как удобно загружать данные в приложении Windows Forms в элемент DataGridView через DataSet. Теперь определим полнофункциональную 
форму, через которую мы сможем производить все стандартные CRUD операции в базе данных.</p>
<p>Итак, определим форму, на которой будет элемент DataGridView и три кнопки для добавления, удаления и сохранения изменений. Форма в итоге будет 
выглядеть примерно следующим образом:</p>
<img src="pics/datagridview.png" alt="DataSet и DataGridView" />
<p>Код формы будет выглядеть следующим образом:</p>
<pre class="brush:c#;">
using System;
using System.Data;
...
<p>Здесь для добавления объекта мы будем обращаться к хранимой процедуре sp_CreateUser, которая была добавлена в базу данных в прошлой теме.</p>
<p>В конструкторе данные загружаются в DataSet, первая таблица которого устанавливается в качестве источника данных для dataGridView1:</p>
<pre class="brush:c#;">dataGridView1.DataSource = ds.Tables[0];</pre>
<p>Также в конструкторе устанавливается полное выделение строки и запрет на ручное добавление новых строк:</p>
<pre class="brush:c#;">
dataGridView1.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
dataGridView1.AllowUserToAddRows = false;
</pre>
<p>В обработчике кнопки добавления создается новая строка, которая добавляется в таблицу объекта DataSet. И так как мы ранее установили 
привязку к источнику данных, то автоматически новая строка также будет добавляться и в dataGridView1:</p>
<pre class="brush:c#;">
private void addButton_Click(object sender, EventArgs e)
{
    DataRow row = ds.Tables[0].NewRow(); // добавляем новую строку в DataTable
...
<p>В обработчике кнопки удаления удаляются выделенные строки в dataGridView1. Опять же в силу привязки к источнику данных будет также происходить удаление 
и из таблицы в DataSet:</p>
<pre class="brush:c#;">
private void deleteButton_Click(object sender, EventArgs e)
{
    foreach(DataGridViewRow row in dataGridView1.SelectedRows)
...
<p>Для обновления на не нужна никакая кнопка, так как мы можем нажать на любую ячейку таблицы (кроме заблокированного для изменения столбца Id) и изменить в ней данные. 
...
чтобы бд синхронизировалась, пользователю надо будет нажать на кнопку сохранения, обработчик которой выглядит следующим образом:</p>
<pre class="brush:c#;">
private void saveButton_Click(object sender, EventArgs e)
{
    using (SqlConnection connection = new SqlConnection(connectionString))
...
<p>Как в прошлой теме здесь устанавливается у адаптера команда на добавление <code>InsertCommand</code> и затем вызывается метод <code>Update()</code>. 
В итоге мы можем добавить несколько строк, удалить, изменить, и потом один раз мы нажмем на кнопку, и все изменения будут применены к базе данных.</p>
