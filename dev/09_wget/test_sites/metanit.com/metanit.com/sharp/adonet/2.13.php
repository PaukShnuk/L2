...
<html >
<head>
<title>Транзакции в MS SQL Server и C#</title>
<meta charset="utf-8" />
<meta name="description" content="Использование транзакций в MS SQL Server и ADO.NET для проведения операций в базе данных">
...
<h2>Транзакции</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Транзакции позволяют выполнять ряд операций в виде одного целостного пакета. И если хотя бы одна из этих операций завершится неудачно, 
то произойдет откат выполнения остальных операций.</p>
<p>Для начала транзакции надо вызвать метод <code>BeginTransaction()</code> объекта <code>SqlConnection</code>:</p>
<pre class="brush:c#;">
...
<p>Для завершения всех операции после их выполнения вызывается метод <code>Commit()</code> объекта SqlTransaction:</p>
<pre class="brush:c#;">transaction.Commit();</pre>
<p>Если в ходе выполнения произошла ошибка, то мы можем откатить транзакцию, вызвав метод <code>Rollback()</code>:</p>
<pre class="brush:c#;">transaction.Rollback();</pre>
