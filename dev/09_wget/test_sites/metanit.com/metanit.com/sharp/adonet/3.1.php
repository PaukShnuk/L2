...
<html >
<head>
<title>SqlDataAdapter и DataSet в ADO.NET и C#</title>
<meta charset="utf-8" />
<meta name="description" content="Отключенный уровень в ADO.NET, загрузка данных из базы данных MS SQL Server в DataSet в ADO.NET и C#">
...
<h1>Работа с SqlDataAdapter и DataSet</h1>
<h2>SqlDataAdapter и DataSet</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<ul>
<li><p>Можно использовать конструктор без параметров, а команду SELECT и подключение установить позже</p></li>
<li><p>Можно передать в конструктор объект SqlCommand</p></li>
<li><p>Можно в конструкторе установить sql-выражение SELECT и объект SqlConnection</p></li>
<li><p>Можно в конструкторе установить sql-выражение SELECT и строку подключения</p></li>
</ul>
<p>Рассмотрим, как получить данные в DataSet через SqlDataAdapter. Для работы с DataSet особенно удобно использовать элементы 
...
проект по типу Windows Forms Application.</p>
<p>Добавим на единственную форму в проекте элемент DataGridView и определим следующий код формы:</p>
<pre class="brush:c#;">
using System.Data;
...
<p>В конструкторе формы в DataGridView загружаются данные. Для загрузки данных создается объект SqlDataAdapter, который принимает объект подключения и sql-выражение SELECT. 
Затем создается объект DataSet и с помощью метода <code>adapter.Fill()</code> в него загружаются данные. Дальше происходит установка источника 
данных для DataGridView:</p>
<pre class="brush:c#;">dataGridView1.DataSource = ds.Tables[0];</pre>
<p>В качестве источника устанавливается одна из таблиц в DataSet. Каждая таблица представляет объект DataTable, и в DataSet может быть определено 
несколько таких таблиц. Но в данном случае при выборке в DataSet есть только одна таблица, которую мы можем получить из коллекции Tables по индексу.</p>
<img src="pics/sqldataadapter.png" alt="SqlDataAdapter в ADO.NET" />
