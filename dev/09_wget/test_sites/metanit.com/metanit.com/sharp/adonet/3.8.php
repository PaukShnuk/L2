...
<html >
<head>
<title>LINQ to DataSet в C#</title>
<meta charset="utf-8" />
<meta name="description" content="Выборка с помощью LINQ to DataSet в ADO.NET и C#">
...
<h2>LINQ to DataSet</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
<p>Одним из преимуществ использования DataSet является то, что мы можем использовать для работы с данными в DataSet технологию LINQ.</p>
<p>Например, выведем из таблицы объекты по какому-нибудь условию:</p>
<pre class="brush:c#;">
...
<img src="pics/linqtodataset.png" alt="LINQ to DataSet" />
<p>В данном случае отображаются все смартфоны стоимостью больше 200 единиц. Чтобы проводить операции LINQ над таблицами, их надо привести 
к объекту IEnumerable: <code>ds.Tables["Phones"].AsEnumerable()</code>. По имени столбцов мы можем получить значения в соответствующих ячейках строк: 
<code>phone["Price"]</code>.</p>
<p>Чтобы избежать необходимости преобразований в запросах, мы можем использовать типизированный метод <code>Field&lt;T&gt;()</code>, 
в который передается название столбца, для которого надо получить значение:</p>
<pre class="brush:c#;">
var query = from phone in ds.Tables["Phones"].AsEnumerable()
            from company in ds.Tables["Companies"].AsEnumerable()
