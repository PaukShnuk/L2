...
<html >
<head>
<title>Параметризация запросов в MS SQL Server и C#</title>
<meta charset="utf-8" />
<meta name="description" content="Параметризация запросов к базе данных MS SQL Server в ADO.NET и C#">
...
<h2>Параметризация запросов</h2><p style="color:red;">Данное руководство устарело. Актуальное руководство: <a href="https://metanit.com/sharp/adonetcore/">по ADO.NET и работе с базами данных в .NET 6</a></p><div class="date">Последнее обновление: 31.10.2015</div>
...
выражение:</p>
<pre class="brush:c#;">string sqlExpression = String.Format("INSERT INTO Users (Name, Age) VALUES ('{0}', {1})", name, age);</pre>
...
данные.</p>
<p>Чтобы выйти из этой ситуации, в sql-командах используются параметры:</p>
<pre class="brush:c#;">
...
<p>Для определения параметров используется объект <span class="b">SqlParameter</span>. Этот объект имеет ряд конструкторов, 
но в данном случае передается название параметра и его значение. Причем название параметров начинается со знака @ и должно совпадать с тем названием, 
которое используется в sql-выражении (то есть в "INSERT INTO Users (Name, Age) VALUES (@name, @age)"). После определения параметра 
он добавляется в коллекцию параметров команды.</p>
