...
<html >
<head>
<title>ASP.NET Core | ClaimPrincipal и объекты Claim</title>
<meta charset="utf-8" />
<meta name="description" content="Классы ClaimPrincipal, ClaimsIdentity и Claim и их роль в авторизации в ASP.NET Core">
...
<h2>ClaimPrincipal и объекты Claim</h2><div class="date">Последнее обновление: 13.12.2019</div>
...
<p>В прошлой теме был рассмотрен объект <code>HttpContext.User</code>, который представляет интерфейс IPrincipal. А у этого объекта есть свойство <code>Identity</code>, 
которое представляет интерфейс <code>IIdentity</code>. Для обоих этих интерфейсов ASP.NET Core предоставляет реализации по умолчанию - классы 
<code>ClaimPrincipal</code> и <code>ClaimsIdentity</code>. То есть свойство <code>User</code> (<code>HttpContext.User</code>) фактически представляет 
объект <span class="b">ClaimsPrincipal</span>.</p>
<p>Ключевым моментом этих двух классов является то, что они позволяют работать с объектами claim.</p>
...
<p>Каждый объект claim представляет класс <span class="b">Claim</span>, который определяет следующие свойства:</p>
<ul>
...
<p>Так в позапрошлой теме для создания аутентификационных куки также использовался объект Claim:</p>
<pre class="brush:c#;">
...
<p>Для создания claima ему в конструктор передается тип и значения. Тип <code>ClaimsIdentity.DefaultNameClaimType</code> фактически представляет логин. 
А <code>userName</code>, в данном случае будет представлять значение, которое затем мы сможем получить через выражение <code>User.Identity.Name</code></p>
 
<p>Для работы с объектами Claim в классе ClaimsPrincipal есть следующие свойства и методы:</p>
<ul>
<li><p><span class="b">Identity</span>: возвращает объект <code>ClaimsIdentity</code>, который реализует интерфейс <code>IIdentity</code> и представляет текущего пользователя</p></li>
...
<li><p><span class="b">FindAll(predicate)</span>: возвращает все объекты claim, которые соответствуют определенному условию</p></li>
...
<p>Например, опять же в позапрошлой теме все эти классы применялись для аутентификации пользователя:</p>
<pre class="brush:c#;">
var claims = new List&lt;Claim&gt;
{
...
<p>Для создания объекта ClaimsIdentity в его конструктор передается набор claim, тип аутентификации (ApplicationCookie), тип для claima, представляющего логин, 
и тип для claima, представляющего роль.</p>
<p>Созданный объект ClaimsIdentity передается в конструктор ClaimsPrincipal. И фактически этот объект ClaimsPrincipal и будет представлять то, что мы потом в любом 
контроллере сможем получить через <code>HttpContext.User</code>.</p>
