...
<html >
<head>
<title>ASP.NET Core | Нефайловые провайдеры конфигурации</title>
<meta charset="utf-8" />
<meta name="description" content="Провайдеры конфигурации в ASP.NET Core, хранение настроек в памяти в словарях Dictionary, загрузка переменных среды окружения и аргументов из командной строки">
...
<h2>Нефайловые провайдеры конфигурации</h2><div class="date">Последнее обновление: 14.11.2019</div>
...
<p>Провайдер CommandLineConfigurationProvider обеспечивает передачу аргументов командной строки в конфигурацию приложения. Для применения этого провайдера у 
объекта вызывается метод <span class="b">AddCommandLine()</span>, в который передаются аргументы командной строки. 
...
<p>Также если мы хотим указать ключ, но не хотим указывать значение, то поле знака равно можно не указывать значение.</p>
<h4>Тестирование в Visual Studio</h4>
<p>Для тестирования передачи параметров командной строки перейдем к настройкам проекта на вкладку <span class="b">Debug</span>:</p>
<img src="./pics/config2.png" alt="Параметры конфигурации командной строки в ASP.NET Core" />
<p>В поле <span class="b">Application arguments</span> укажем, например, два параметра:</p>
<pre>
name=Bob age=37
</pre>
<p>В данном случае два параметра name и age. После изменения сохраним настройки.</p>
<p>Определим в классе Startup какой-нибудь простейший код для получения значений конфигурации:</p>
<pre class="brush:c#;">
...
<p>После запуска мы увидим указанные значения параметров конфигурации:</p>
<img src="./pics/config3.png" alt="Установка конфигурации командной строки в ASP.NET Core" />
...
cd в папку проекта. В моем случае это папка C:\Users\Eugene\Source\Repos\ASPNETCore\ConfigurationApp\ConfigurationApp.</p>
...
<img src="./pics/config4.png" alt="dotnet run и конфигурация командной строки в ASP.NET Core" />
<p>В итоге приложение подхватит переданные данные:</p>
<img src="./pics/config5.png" alt="CommandLineConfigurationProvider в ASP.NET Core" />
<h3>Применение метода AddCommandLine</h3>
<p>Мы можем на уровне кода симулировать передачу параметров командной строки:</p>
<pre class="brush:c#;">
public class Startup
{
	public Startup()
...
<p>Для загрузки переменных среды окружения в качестве параметров конфигурации применяется провайдер EnvironmentVariablesConfigurationProvider. 
Для его использования у объекта ConfigurationBuilder вызывается метод <span class="b">AddEnvironmentVariables()</span>. Однако в реальности 
...
<p>Провайдер MemoryConfigurationProvider позволяет использовать в качестве конфигурации коллекцию IEnumerable&lt;&lt;string, string&gt;&gt;, 
...
<div class="nav"><p><a href="./2.6.php">Назад</a><a href="./">Содержание</a><a href="./2.12.php">Вперед</a></p></div></div>
<div class="socBlock">
