...
<html >
<head>
<title>ASP.NET Core | Хранение ресурсов в базе данных</title>
<meta charset="utf-8" />
<meta name="description" content="Хранение ресурсов локализации в базе данных в ASP.NET Core, локализация через контекст данных Entity Framework Core">
...
<h2>Хранение ресурсов в базе данных</h2><div class="date">Последнее обновление: 18.12.2019</div>
...
<p>В предыдущих статьях для хранения ресурсов, используемых при локализации, применялись файлы ресурсов resx. Однако мы также можем использовать для хранения ресурсов 
любое хранилище, например, базу данных. Рассмотрим как это сделать.</p>
<p>Вначале определим модели. Добавим в проект в папку Models два класса <span class="b">Culture</span> и <span class="b">Resource</span>:</p>
<p>Класс Resource:</p>
<pre class="brush:c#;">
...
<p>Класс ресурса содержит свойства для хранения ключа, значения по ключу и ссылку на культуру, к которой принадлежит ресурс.</p>
<p>Класс Culture:</p>
<pre class="brush:c#;">
...
<p>Класс культуры хранит ее название и список связанных ресурсов.</p>
...
А затем добавим в папку Models класс контекста данных <span class="b">LocalizationContext</span>, через который мы будем работать с базой данных:</p>
<pre class="brush:c#;">
...
<p>Теперь нам надо создать инфраструктуру, которая бы извлекала из базы данных через контекст нужный ресурс. Для этого добавим в корень проекта 
новый класс <span class="b">EFStringLocalizer</span>:</p>
<pre class="brush:c#;">
...
<p>С помощью индексаторов возвращаем из базы данных по ключу объект Resource, из которого формируем экземпляр класса LocalizedString. 
Для обращения к базе данных через встроенный механизм внедрения зависимостей получаем в конструкторе контекст данных.</p>
<p>Также добавим в корень проекта еще один класс, который назовем <span class="b">EFStringLocalizerFactory</span>. Этот класс будет представлять фабрику объектов IStringLocalizer:</p>
<pre class="brush:c#;">
using EFLocalizationApp.Models;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Localization;
...
<p>Фабрика EFStringLocalizerFactory будет определена как синглтон (так как нет смысла создавать для каждого запроса отдельный объект фабрики), 
поэтому контекст данных, через который фабрика взаимодействует с БД, создается непосредственно внутри нее. 
В частности, в методе CreateStringLocalizer определяется контекст данных, происходи инициализация БД и возвращается объект 
EFStringLocalizer.</p>
<p>При создании объекта IStringLocalizer инфраструктурой MVC у фабрики будут вызываться методы Create, которые собственно и будут возвращает 
объект EFStringLocalizer. И этот объект потом будет использоваться для локализации.</p>
<p>Итоге получится следующий проект:</p>
<img src="./pics/localization32.png" alt="Localization in ASP.NET Core and Entity Framework Core" />
<p>Теперь все это используем в классе Startup:</p>
<pre class="brush:c#;">
...
<p>С локализацией контроллеров и представлений все относительно просто. Нам просто надо установить зависимость для IStringLocalizer:</p>
<pre class="brush:c#;">services.AddTransient&lt;IStringLocalizer, EFStringLocalizer&gt;();</pre>
<p>С аннотациями данных все сложнее, так как для аннотаций данных необходимо создать через фабрику объект IStringLocalizer. Но чтобы использовать нужную фабрику, 
надо установить зависимость для IStringLocalizerFactory:</p>
<pre class="brush:c#;">
...
<p>Для применения EFStringLocalizer в представлениях, необходимо инжектировать сервис IViewLocalizer:</p>
<pre class="brush:xml;">
...
<p>Для использования для аннотаций данных ничего дополнительно не надо делать.</p>
<p>А в контроллере также через конструктор передается сервис IStringLocalizer:</p>
<pre class="brush:c#;">
...
<p>К примеру, обращение к действию Test при установке различных культур:</p>
<img src="./pics/localization30.png" alt="Локализация из базы данных в ASP.NET Core" />
<img src="./pics/localization31.png" alt="Локализация с помощью Entity Framework Core в ASP.NET Core MVC" />
...
<div class="nav"><p><a href="./28.8.php">Назад</a><a href="./">Содержание</a><a href="./30.1.php">Вперед</a></p></div></div>
<div class="socBlock">
