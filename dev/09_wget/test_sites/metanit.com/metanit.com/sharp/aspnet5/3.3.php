...
<html >
<head>
<title>ASP.NET Core | Создание контроллера и инициализатора базы данных в MVC</title>
<meta charset="utf-8" />
<meta name="description" content="Создание контроллера и представления в ASP.NET MVC Core и инициализация базы данных">
...
<h2>Создание контроллера и инициализатора базы данных</h2><div class="date">Последнее обновление: 16.11.2019</div>
...
<p>Чтобы у нас в базе данных уже были начальные данные при запуске приложения, нам нужен некий класс. который бы выполнял роль инициализатора базы данных. 
Для этого добавим в проект новый класс, который назовем SampleData и который будет иметь следующий код:</p>
<pre class="brush:c#;">
using System.Linq;
...
<p>Данный класс определяет один статический метод <code>Initialize()</code>, в котором происходит добавление трех начальных элементов - объектов Phone. 
Для добавления объектов в бд в метод Initialize передается контекст данных. И если данные в таблице Phones в бд отсутствуют (<code>if (!context.Phones.Any())</code>), то добавляются три объекта.</p>
<p>Чтобы инициализатор базы данных вызывался при старте приложения, изменим класс Program следующим образом:</p>
<pre class="brush:c#;">
using System;
...
<p>В данном случае получаем контекст из сервисов и вызываем инициализатор. В случае ошибки осуществляем логгирования через соответствующий сервис.</p>
<p>Итак, начальные данные определены, и теперь мы хотим их выводить на веб-страницу, чтобы пользователи видели набор товаров и смогли бы один из них выбрать.</p>
<p>Для этого перейдем к папке Controllers, где находится единственный контроллер HomeController. Изменим его содержимое следующим образом:</p>
<pre class="brush:c#;">
using System.Linq;
...
<p>Во-первых, здесь удалены все ненужные нам методы - все кроме метода Index, который будет использоваться для передачи пользователю данных о товарах.</p>
<p>Во-вторых, добавлен конструктор, в котором получаем контекст данных. Здесь применяется встроенный механизм внедрения зависимостей. Так как, в методе <code>ConfigureServices()</code> 
контекст данных был добавлен в качестве сервиса:</p>
<pre class="brush:c#;">
services.AddDbContext&lt;MobileContext&gt;(options =&gt;
		options.UseSqlServer(Configuration.GetConnectionString("DefaultConnection")));
</pre>
<p>То через конструктор мы можем получить эту зависимость и использовать ее.</p>
<p>Затем вызывается метод <code>View()</code>, который генерирует представление. И в этот метод передаются все объекты 
из таблицы Phones в базе данных. Для передачи данных нам достаточно использовать такую конструкцию: <code>db.Phones.ToList()</code>.</p>
<p>За построение визуального интерфейса в MVC отвечают представления. И теперь создадим само представление для вывода списка смартфонов. Для представлений в 
проекте предназначена папка Views. По умолчанию в этой папке уже есть подкаталог для представлений контроллера Home, в котором три представления: About.cshtml, Contact.cshtml и Index.chtml.</p>
<p>Каждый метод контроллера по умоланию использует одноименное представление. То есть метод Index будет использовать представление Index.cshtml. Откроем 
это представление и изменим его код следующим образом:</p>
<pre class="brush:xml;">
...
<p>Представления напоминают html-страницы, так как могут содержать и часто содержат очень много кода html. Но кроме собственно html они также содержат специальные инструкции, 
...
код на языке c#. Далее мы подробнее разберем синтаксис движка Razor, а пока достаточно знать, что после символа <span class="b">@</span> идут выражения 
на языке C#.</p>
<p>Первая строка устанавливает модель представления - та сущность, которая будет доступна в представлении через объект Model. В данном случае это объект 
<code>IEnumerable&lt;MobileStore.Models.Phone&gt;</code>, так как в контроллере в методе Index мы передаем список смартфонов, а список - объект List представляет 
интерфейс IEnumerable.</p>
<p>Далее идет блок кода, в котором выражение <code>Layout = null</code> указывает, что мастер-страница не будет применяться к этому представлению. Далее мы добавим к нему мастер-страницу и узнаем, зачем она нужна, а пока обойдемся без нее.</p>
<p>Практически весь остальной код представляет собой стандартный код веб-страницы на языке html: создание обычной таблицы, которая выводит информацию о продаваемых смартфонах. Здесь 
также используется конструкция <code>@foreach (var phone in Model)</code>. То есть тут мы создаем цикл. В нем мы пробегаемся по всем элементам в объекте 
...
<code>@phone.Name</code> и помещаем его в ячейку таблицы.</p>
<p>В последнюю колонку таблицы для каждого элемента добавляется ссылка <code>&lt;a href="~/Home/Buy/@phone.Id"&gt;Купить&lt;/a&gt;</code>. При нажатии на эту ссылку методу Buy контроллера HomeController 
будет отправляться запрос, в котором вместо @phone.Id будет указан id смартфона. Пока у нас, правда, отсутствует метод Buy, но скоро мы его создадим.</p>
<h3>Основы маршрутизации</h3>
<p>В методе <code>Configure()</code> класса Startup устанавливаются настройки маршрутизации приложения:</p>
<pre class="brush:c#;">
...
<p>Здесь добавляется маршрут с именем <code>default</code>. Поле <code>pattern</code> указывает, что запрос к приложению должен иметь двух-трехсегментную структуру. 
Вначале идет имя контроллера, потом имя метода и потом может идти необязательный параметр id.</p>
<p>То есть чтобы обратиться к контроллеру HomeController или отправить ему запрос, нам надо указать в строке запроса его имя - Home и далее через 
слеш указать метод контроллера, к которому отправляется запрос, например, <span class="ii">Home/Index</span>. По умолчанию при запуске проекта или при обращении к сайту 
система mvc будет вызывать действие Index контроллера HomeController.</p>
<p>Теперь запустим приложение на выполнение. По умолчанию, как и определено выше маршрутом default, сработает обращение к методу Index контроллера Home, 
который возвратит пользователю страницу с данными:</p>
<img src="./pics/mvc11.png" alt="Первое приложение на ASP.NET MVC Core" />
<p>А через интерфейс Visual Studio в окне SQL Server Object Explorer мы можем увидеть созданную базу данных:</p>
<img src="./pics/mvc10.png" />
