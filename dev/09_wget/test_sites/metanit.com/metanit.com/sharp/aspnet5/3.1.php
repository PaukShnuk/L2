...
<html >
<head>
<title>ASP.NET Core | Введение в MVC</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в ASP.NET Core MVC, основные компоненты паттерна, модель, контроллер, представление, их взаимодействие">
...
<h1>ASP.NET Core MVC</h1><h2>Введение в MVC</h2><div class="date">Последнее обновление: 16.11.2019</div>
...
<p>Одним из отличительных моментов платформы ASP.NET Core является применение паттерна MVC. Причем последняя версия MVC-фреймворка, который применяется в ASP.NET Core, имеет номер 5.0. 
Поэтому важно не путать ASP.NET MVC 5, который применяется в ASP.NET 4.5-4.8, и фреймворк MVC, который применяется в ASP.NET Core. Хотя во многих аспектах эти фремйоворки будут совпадать.</p>
<p>Также неверно отождествлять ASP.NET Core всецело с фреймворком ASP.NET Core MVC. Фреймворк ASP.NET Core MVC работает поверх платформы 
ASP.NET Core, и предназначен для того, чтобы упростить создание приложения. Но мы можем и не использовать MVC, а применять чистый ASP.NET Core и на нем всецело выстраивать логику приложения.</p>
<p>Сам паттерн MVC не является какой-то новой идеей в архитектуре приложений, он появился еще в конце 1970-х годов в компании Xerox как способ организации 
компонентов в графическом приложение на языке Smalltalk.</p>
<p>Концепция паттерна MVC предполагает разделение приложения на три компонента:</p>
<ul>
<li><p><span class="b">Модель (model)</span>: описывает используемые в приложении данные, а также логику, которая связана непосредственно с данными, например, 
логику валидации данных. Как правило, объекты моделей хранятся в базе данных.</p>
<p>В MVC модели представлены двумя основными типами: модели представлений, которые используются представлениями для отображения и передачи данных, и 
модели домена, которые описывают логику управления данными.</p>
<p>Модель может содержать данные, хранить логику управления этими данными. В то же время модель не должна содержать логику взаимодействия с пользователем и не должна 
определять механизм обработки запроса. Кроме того, модель не должна содержать логику отображения данных в представлении.</p></li>
<li><p><span class="b">Представление (view)</span>: отвечают за визуальную часть или пользовательский интерфейс, нередко html-страница, через который пользователь взаимодействует с приложением. Также представление может 
содержать логику, связанную с отображением данных. В то же время представление не должно содержать логику обработки запроса пользователя или управления данными.</p></li>
<li><p><span class="b">Контроллер (controller)</span>: представляет центральный компонент MVC, который обеспечивает связь между пользователем и приложением, представлением и хранилищем данных. Он содержит 
...
обработки отправляет пользователю определенный вывод, например, в виде представления, наполненного данными моделей.</p></li>
</ul>
<p>Отношения между компонентами паттерна можно описать следующей схемой:</p>
<img src="./pics/mvc.png" alt="Паттерн MVC в программировании" />
<p>В этой схеме модель является независимым компонентом - любые изменения контроллера или представления никак не влияют на модель. 
...
а из контроллера генерировать представления, но при этом нередко их можно изменять независимо друг от друга.</p>
<p>Такое разграничение компонентов приложения позволяет реализовать концепцию разделение ответственности, при которой каждый компонент отвечает за свою строго очерченную сферу. 
...
Допустим, если нам важна визуальная часть или фронтэнд, то мы можем тестировать представление независимо от контроллера. Либо мы можем сосредоточиться на бэкэнде и тестировать контроллер.</p>
<p>Однако неверно полностью ассоциировать всю платформу ASP.NET Core с MVC. MVC - это лишь паттерн, который реализуется в рамках платформы. 
...
класс построить всю обработку запроса. Но естественно применение MVC облегчает разработку приложений.</p>
<p>Для создания проекта, применяющего MVC, нам надо выбрать шаблон <span class="b">ASP.NET Core Web App (Model-View-Controller)</span>:</p>
<img src="./pics/mvc1.png" alt="Фреймворк ASP.NET MVC" />
<img src="./pics/mvc2.png" alt="Первый проект на ASP.NET MVC Core" />
<p>Оставим все настройки по умолчанию и нажмем на ОК. И Visual Studio создаст новый проект MVC.</p>
<p>Структура создаваемого проекта будет отличаться от структуры проекта по типу Empty. В частности, мы увидим ряд новых папок и файлов:</p>
<img src="./pics/mvcstructure1.png" alt="Структура проекта ASP.NET MVC Core" />
<ul>
<li><p><span class="b">Dependencies</span>: все добавленные в проект пакеты и библиотеки</p></li>
<li><p><span class="b">wwwroot</span>: этот узел (на жестком диске ему соответствует одноименная папка) предназначен для хранения статических файлов - 
...
доступ запрещен.</p>
<li><p><span class="b">Controllers</span>: папка для хранения контроллеров, используемых приложением</p></li>
<li><p><span class="b">Models</span>: каталог для хранения моделей</p></li>
<li><p><span class="b">Views</span>: каталог для хранения представлений</p></li>
<li><p><span class="b">appsettings.json</span>: хранит конфигурацию приложения</p></li>
<li><p><span class="b">Program.cs</span>: файл, определяющий класс Program, который инициализирует и запускает хост с приложением.</p></li>
<li><p><span class="b">Startup.cs</span>: файл, определяющий класс Startup, с которого начинается работа приложения. То есть это входная точка в приложение.</p></li>
</ul>
<p>Фактически эта та же структура, что и у проекта по типу Empty за тем исключением, что здесь также добавлены по умолчанию папки для ключевых компонентов фреймворка MVC: контроллеров 
и представлений. А также есть дополнительные узлы и файлы для управления зависимостями клиентской части приложения.</p>
<p>И если мы запустим проект на выполнение, то сработает запрос к контроллеру по умолчанию - классу HomeController, который выберет для генерации ответа 
нужное представление. И в итоге из представления будет создана html-страница, которую мы увидим в своем веб-браузере:</p>
<img src="./pics/mvc5.png" alt="Первый проект на ASP.NET MVC Core" />
...
<div class="nav"><p><a href="./11.8.php">Назад</a><a href="./">Содержание</a><a href="./3.6.php">Вперед</a></p></div></div>
<div class="socBlock">
