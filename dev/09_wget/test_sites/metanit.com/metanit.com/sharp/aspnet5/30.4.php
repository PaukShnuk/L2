...
<html >
<head>
<title>ASP.NET Core | Контекст хаба, подключение и отключение клиентов в SignalR</title>
<meta charset="utf-8" />
<meta name="description" content="Контекст хаба, подключение и отключение клиентов в SignalR в ASP.NET Core, класс HubCallerContext и его свойства">
...
<h2>Контекст хаба, подключение и отключение клиентов</h2><div class="date">Последнее обновление: 24.12.2019</div>
...
<p>В классе Hub определено свойство <span class="b">Context</span>, которое представляет тип <span class="b">HubCallerContext</span> 
и который позволяет нам получить некоторую информацию о подключении к хабу. Данная информация хранится в следующих его свойствах:</p>
<ul>
<li><p><span class="b">ConnectionId</span>: возвращает уникальный идентификатор подключения в виде строки.</p></li>
<li><p><span class="b">ConnectionAborted</span>: возвращает объект CancellationToken, который извещает о закрытии подключения</p></li>
<li><p><span class="b">User</span>: возвращает объект <span class="b">ClaimsPrincipal</span>, ассоциированный с текущим 
пользователем. По сути это аналог свойства <code>HttpContext.User</code>, которое доступно в констроллерах</p></li>
<li><p><span class="b">UserIdentifier</span>: возвращает идентификатор пользователя. По умолчанию индентификатор пользователя представляет клейм 
<span class="b">ClaimTypes.NameIdentifier</span> объекта <span class="b">ClaimsPrincipal</span>, 
который ассоциирован с данным подключением.</p></li>
<li><p><span class="b">Items</span>: возвращает словарь значений, ассоциированных с текущим подключением. Данный словарь позволяет хранить данные 
для определенного клиента между его запросами</p></li>
<li><p><span class="b">Features</span>: возвращает коллекцию возможностей HTTP, ассоциированных с текущим подключением</p></li>
</ul>
<p>Также класс HubCallerContext определяет пару методов:</p>
<ul>
<li><p><span class="b">Abort()</span>: принудительно завершает текущее подключение</p></li>
<li><p><span class="b">GetHttpContext()</span>: возвращает объект <span class="b">HttpContext</span> для текущего подключения или 
<span class="b">null</span>, если подключение не ассоциировано с запросом HTTP.</p></li>
</ul>
<h3>Подключение и отключение клиентов</h3>
<p>Класс Hub определяет два метода, которые мы можем переопределить в классах-наследниках:</p>
<ul>
<li><p><span class="b">OnConnectedAsync()</span>: срабатывает при подключении нового клиента</p></li>
<li><p><span class="b">OnDisconnectedAsync(Exception exception)</span>: срабатывает при отключении клиента, в качестве параметра передается сообщение об ошибке, которая описывает, 
почему произошло отключение.</p></li>
</ul>
<p>Реализуем эти методы и определим следующий класс хаба:</p>
<pre class="brush:c#;">
...
<p>В данном случае вместо условного ника пользователя будет использоваться идентификатор подключения. В методах OnConnectedAsync и 
OnDisconnectedAsync мы будем уведомлять клиентов, о том, что какой-то клиент подключился или отключился от приложения.</p>
<p>Для теста определим следующую веб-страницу index.html:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь в javascript в функции "Send" получаем обычные сообщения от клиентов, а в функции "Notify" диагностические уведомления.</p>
<p>Таким образом, при подключение и отключение мы сможем уведомлять всех клиентов:</p>
<img src="./pics/signalr9.png" alt="Подключение и отключение клиентов в SignalR" />
<p>Стоит отметить, что методы OnConnectedAsync срабатывает еще до первой отправки сообщения пользователем. Метод OnDisconnectedAsync срабатывает, 
если клиент закроет вкладку браузера или перезагрузит страницу.</p>
<p>Также стоит отметить, что если мы откроем в одном браузере две вкладки со страницей, то у нас будут два отдельных подключенных клиента, которые будут иметь разные идентификаторы.</p>
<h3>Получение информации о контексте HTTP</h3>
<p>С помощью метода <code>GetHttpContext()</code> контекста хаба мы можем получить различную информацию о запросе, например, информацию о браузере пользователя, 
его ip, куки и т.д., вообщем различные заголовки http. Например:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.SignalR;
using System;
using System.Diagnostics;
...
<p>Следует отметить, что получить заголовки http таким образом мы можем, а вот установить заголовки или поменять их значение или установить те же куки нельзя.</p>
...
<div class="nav"><p><a href="./30.3.php">Назад</a><a href="./">Содержание</a><a href="./30.5.php">Вперед</a></p></div></div>
<div class="socBlock">
