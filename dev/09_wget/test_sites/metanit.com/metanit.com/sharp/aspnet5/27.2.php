...
<html >
<head>
<title>ASP.NET Core | Чтение и добавление данных в MongoDB</title>
<meta charset="utf-8" />
<meta name="description" content="Чтение и добавление данных в MongoDB в ASP.NET Core, подключение Mongo в контроллере">
...
<h2>Чтение и добавление данных</h2><div class="date">Последнее обновление: 29.12.2019</div>
...
<p>В прошлой теме был определен класс ProductService, который внедряется в приложение в виде сервиса и который позволяет взаимодействовать с базой данных. 
Теперь определим логику обработки запросов пользователя в контроллере.</p>
<p>Но вначале добавим в папку <span class="b">Models</span> две дополнительные модели, которые упростят работу с данными. 
Первая модель будет содержать значения для фильтрации данных и будет называться <span class="b">FilterViewModel</span>:</p>
<pre class="brush:c#;">
...
<p>Так как метод GetProducts класса ProductService принимает три критерия фильтрации: минимальную и максимальную цену, а также название модели, 
то класс FilterViewModel принимает три соответствующих свойства.</p>
<p>Вторая модель представляет данные, выводимые на главную страницу. Эта модель называется <span class="b">IndexViewModel</span>:</p>
<pre class="brush:c#;">
...
<p>По умолчанию в проекте уже есть контроллер <span class="b">HomeController</span>. Собственно его и будем использовать для обработки запросов. 
Определим в нем следующий код:</p>
<pre class="brush:c#;">
...
<p>Поскольку ProductService внедряется в приложение в виде сервиса, то в конструкторе контроллера мы можем его получить.</p>
<p>В методе Index получаем введенные пользователем данные в виде модели FilterViewModel и передаем эти данные в вызов метода <code>db.GetProducts</code>, 
через который извлекаем из базы данных все соответствующие критериям фильтрации документы.</p>
<p>Через действие Create добавляем пришедшие данные в бд посредством обращения к методу <code>db.Create()</code>.</p>
<p>Практически нам ничего не надо делать в контроллере кроме как вызывать методы класса ProductService, который и выполняет всю основную работу.</p>
<p>Теперь определим для метода Index представление <span class="b">Index.cshtml</span>, в котором будут выводиться данные:</p>
<pre class="brush:xml;">
@model MongoApp.Models.IndexViewModel
...
&lt;form method="get"&gt;
    &lt;div class="form-group"&gt;
        &lt;label for="Filter.Name"&gt;Поиск по соответствию&lt;/label&gt;
        &lt;input type="text" name="Filter.Name" value="@Model.Filter.Name" class="form-control"  /&gt;
    &lt;/div&gt;
    &lt;div class="form-group"&gt;
        &lt;label for="Filter.MinPrice"&gt;Минимальная цена&lt;/label&gt;
        &lt;input type="number" name="Filter.MinPrice" value="@Model.Filter.MinPrice" class="form-control"  /&gt;
    &lt;/div&gt;
    &lt;div class="form-group"&gt;
        &lt;label for="Filter.MaxPrice"&gt;Максимальная цена&lt;/label&gt;
        &lt;input type="number" name="Filter.MaxPrice" value="@Model.Filter.MaxPrice" class="form-control" /&gt;
    &lt;/div&gt;
    &lt;div class="form-group"&gt;
    ...
    @foreach (var item in Model.Products)
    {
        &lt;tr&gt;
            ...
            &lt;td&gt;
                ...
                &lt;a asp-controller="Home" asp-action="AttachImage" asp-route-id="@item.Id"&gt;Изображение&lt;/a&gt;
            &lt;/td&gt;
        &lt;/tr&gt;
    }
&lt;/table&gt;
</pre>
<p>А для метода Create определим представление <span class="b">Create.cshtml</span> с формой для добавления данных:</p>
<pre class="brush:xml;">
@model MongoApp.Models.Product
...
<img src="./pics/mongo3.png" alt="Проект ASP.NET Core для работы с MongoDB" />
<p>Запустим сервер MongoDB и затем запустим проект. При первом обращении к приложению у нас естественно нет никаких данных, и на главной странице мы увидим пустую таблицу. 
Поэтому обратимся к методу Create добавим какие-нибудь данные:</p>
<img src="./pics/mongo4.png" alt="Добавление в MongoDB и ASP.NET Core MVC" />
<p>И таким образом добавим несколько объектов Product:</p>
<img src="./pics/mongo5.png" alt="Извлечение данных из MongoDB в ASP.NET Core MVC" />
...
<div class="nav"><p><a href="./27.1.php">Назад</a><a href="./">Содержание</a><a href="./27.3.php">Вперед</a></p></div></div>
<div class="socBlock">
