...
<html >
<head>
<title>ASP.NET Core | Установка приложения в виде службы Windows</title>
<meta charset="utf-8" />
<meta name="description" content="Установка приложения в виде службы Windows ASP.NET Core, параметры публикации и настройка пула приложения">
...
<h2>Установка приложения в виде службы Windows</h2><div class="date">Последнее обновление: 12.06.2018</div>
...
<p>ASP.NET Core можно развертывать в виде обычной службы Windows без каких-либо веб-серверов, в частности, IIS.<p>
<p>Создадим новый проект ASP.NET Core 2.1 любого типа. Прежде всего, нам надо добавить в проект через Nuget пакет 
<span class="b">Microsoft.AspNetCore.Hosting.WindowsServices</span>.</p>
<p>После создания проекта обратимся к файлу <span class="b">Program.cs</span>, который во всех проектах выглядит идентично:</p>
<pre class="brush:c#;">
using System;
...
<p>Чтобы запустить приложение в виде службы у объекта IWebHost вызывается метод <span class="b">RunAsService()</span>.</p>
<h3>Публикация</h3>
<p>Теперь нам надо опубликовать приложение в файловой системе. Мы можем это сделать через консоль с помощью команды <code>dotnet publish</code>. 
Для этого вначале в командной строке/терминале надо перейти к папке проекта и из нее запустить команду:</p>
<div class="console">
<pre class="consoletext">
dotnet publish --configuration Release --runtime win10-x64 --output c:\myapp
</pre>
</div>
<p>Поскольку приложение будет устанавливаться в виде службы Windows и должно иметь исполняемый файл, то указывается 
параметр <code>--runtime</code>. В данном случае служба будет устанавливаться на Windows 10 с 64-битной архитектурой. Поэтому 
для этого параметра указано значение <code>win10-x64</code>.</p>
<p>Параметр <code>--output</code> указывает, где будет опубликовано приложение - то есть в данном случае в папке <span class="ii">c:\myapp</span>.</p>
<img src="./pics/service1.png" alt="ASP.NET Core app as a Windows Service" />
<p>Также можно поизвести публикацию с помощью графических средств в Visual Studio.</p>
<h3>Создание службы</h3>
<p>После публикации с помощью консольной утилиты <span class="b">sc.exe</span> создадим службу:</p>
<div class="console"><pre class="consoletext">sc create НАЗВАНИЕ_СЛУЖБЫ binPath= "ПУТЬ К ИСПОЛНЯЕМОМУ ФАЙЛУ EXE"</pre></div>
<p>После команды <code>create</code> указывается имя службы. Службу можно назвать как угодно.</p>
<p>Параметр <code>binPath</code> указывает на путь к исполняемому файлу (в том числе имя самого файла). 
Причем между знаком равно и путем к файлу в кавычках должен идти пробел.</p>
<p>Например, ранее приложение было опубликовано в папке c:\myapp. Как правило, название исполняемого файла соответствует названию проекта, то есть в моем случае 
в папке c:\myapp после публикации находится исполняемый файл <code>ServiceHostingApp.exe</code>. И, допустим, служба буде называться 
MyAspService. В этом случае команда на создание службы будет выглядеть следующим образом:</p>
<div class="console"><pre class="consoletext">sc create MyAspService binPath= "c:\myapp\servicehostingapp.exe"</pre></div>
<img src="./pics/service2.png" alt="Установка приложения ASP NET Core в виде службы" />
<h3>Запуск службы</h3>
<p>После установки службы запустим ее с помощью команды:</p>
<div class="console"><pre class="consoletext">sc start MyAspService</pre></div>
<p>Команде <code>start</code> передается имя ранее установленной службы - в моем случае это MyAspService.</p>
<p>После установки мы можем обратиться обратиться к нашему веб-приложению из браузера по адресу <span class="ii">http://localhost:5000</span>:</p>
<img src="./pics/service3.png" alt="Приложение Asp.Net Core в виде службы Windows" />
...
<div class="nav"><p><a href="./20.1.php">Назад</a><a href="./">Содержание</a><a href="./22.1.php">Вперед</a></p></div></div>
<div class="socBlock">
