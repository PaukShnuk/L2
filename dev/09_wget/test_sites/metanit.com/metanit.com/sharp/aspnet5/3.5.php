...
<html >
<head>
<title>ASP.NET Core | Добавление мастер-страницы и стилизации в MVC</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление мастер-страницы в ASP.NET Core MVC, стилизация представлений">
...
<h2>Добавление мастер-страницы и стилизации</h2><div class="date">Последнее обновление: 16.11.2019</div>
...
<p>Итак, у нас есть уже примитивный функционал для вывода на веб-страницу товаров и оформления заказов. Но рассмотрим еще один аспект - создание единообразного вида приложения.</p>
<p>У нас есть два представления, которые фактически напоминают веб-страницу. И если мы захотим применить к каждой веб-странице какие-либо стили, 
...
так как если нам надо внести изменения, то придется менять все представления, которых может быть в проекте множество.</p>
<p>Гораздо более оптимальный способ преставляет использование мастер-страниц. По умолчанию в проекте ASP.NET MVC уже имеется мастер-страница, 
которая называется <span class="b">_Layout.cshtml</span> и которая находится в папке <span class="ii">Views/Shared</span>:</p>
<img src="./pics/mvc16.png" alt="Layout в ASP.NET MVC Core" />
<p>Мастер-страница предствляет обычное представление, в которое включает в себя другие отдельные представления. Откроем файл <i>_Layout.cshtml</i> 
и изменим его следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
    ...
    &lt;body&gt;
        ...
        &lt;div class="container body-content"&gt;
            ...
            &lt;hr /&gt;
            &lt;footer&gt;
                &lt;p&gt;&copy; 2015 - MobileStore&lt;/p&gt;
            &lt;/footer&gt;
        &lt;/div&gt;
    &lt;/body&gt;
&lt;/html&gt;
</pre>
<p>Вначале подключаются стили библиотеки Bootstrap, которая по умолчанию уже имеется в проекте в папке <i>wwwroot/lib/bootstrap/dist/css/</i>.</p>
<p>После секции head на мастер-странице идет создание меню. Так как у нас всего два представления, то в качестве одного единственного пункта меню указывается ссылка на главную страницу. 
Для создания меню здесь применяются стандартные классы Bootstrap.</p>
<p>Далее в основной части идет вызов метода <span class="b">RenderBody()</span> - с помощью этого метода в это место будет подставляться разметка уже 
конкретных представлений.</p>
...
<p>Вначале объявляется заголовок с помощью выражения <code>ViewData["Title"] = "Каталог товаров"</code>. Этот заголовок затем вставляется на мастер-страницу.</p>
<p>Для создания таблицы здесь используется класс bootstrap <code>table-condensed</code>. Поскольку данное представление будет внедряться в мастер-страницу, то соответственно 
все стили и скрипты на мастер-страницы для представления Index.cshtml также будут доступны.</p>
<p>Представление <i>Buy.cshtml</i>:</p>
<pre class="brush:xml;">
@{
...
<p>Здесь та же сама форма для создания заказа, только теперь к элементам применяются классы bootstrap.</p>
<p>И если мы теперь запустим приложение, то увидим, что веб-страницы выглядят немного иначе за счет использования мастер-страницы и стилей bootstrap:</p>
<img src="./pics/mvc17.png" alt="Применение стилизации в ASP.NET MVC Core" />
...
<div class="nav"><p><a href="./3.4.php">Назад</a><a href="./">Содержание</a><a href="./5.1.php">Вперед</a></p></div></div>
<div class="socBlock">
