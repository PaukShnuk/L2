...
<html >
<head>
<title>ASP.NET Core | Передача конфигурации через IOptions</title>
<meta charset="utf-8" />
<meta name="description" content="Определение конфигурации и ее передача в middleware и другие классы в ASP.NET Core через dependency injection">
...
<h2>Передача конфигурации через IOptions</h2><div class="date">Последнее обновление: 14.11.2019</div>
...
<p>С помощью механизма внедрения зависимостей можно передавать конфигурацию как сервис различные компоненты middleware и прочие классы приложения. 
Но кроме того, с помощью объекта IOptions можно передавать конфигурацию не просто как набор настроек в виде пар ключ-значение, а как объекты определенных классов.</p>
<p>Допустим, у нас проект ASP.NET Core по типу Empty. Определим в проекте файл конфигурации <span class="b">person.json</span> со следующим содержимым:</p>
<pre class="brush:js;">
{
  "age": 18,
  "name": "Tom",
...
<p>И чтобы передать конфигурационные настройки через объект Person, мы можем использовать сервис <span class="b">IOptions&lt;TOptions&gt;</span>. 
Для этого определим в проекте новый класс PersonMiddleware, который фактически будет выводить информацию о пользователе на веб-станицу:</p>
<pre class="brush:c#;">
...
<p>Через конструктор система будет передавать в контроллер объект IOptions&ltPerson&gt;. Но чтобы эта передача произошла, нам еще надо изменить класс Startup:</p>
<pre class="brush:c#;">
...
<p>В методе <code>ConfigureServices()</code> по данным конфигурации из свойства AppConfiguration создается объект Person:</p>
<pre class="brush:c#;">
services.Configure&lt;Person&gt;(AppConfiguration);
</pre>
<p>В итоге PersonMiddleware при получении запроса отправит данные объекта Person, а по сути данные из файла конфигурации.</p>
<img src="./pics/2.37.png" alt="Options в ASP.NET Core" />
