...
<html >
<head>
<title>ASP.NET Core | Конфигурация и фильтрация логгирования</title>
<meta charset="utf-8" />
<meta name="description" content="Определение конфигурации логгирования ASP.NET Core в файле json, установка фильтров логгера, метод AddFilter объекта ILoggerFactory, правила фильтрации сообщений при логгировании">
...
<h2>Конфигурация и фильтрация логгирования</h2><div class="date">Последнее обновление: 06.11.2019</div>
...
<p>Для логгера можно задать конфигурацию с помощью одного из следующих источников:</p>
<ul>
<li><p>Файлы (json, xml)</p></li>
<li><p>Аргументы командной строки</p></li>
<li><p>Переменные среды окружения</p></li>
<li><p>Объекты .NET</p></li>
<li><p>Незашифрованное хранилище Secret Manager</p></li>
</ul>
<p>Также можно создать свой провайдер конфигурации логгера.</p>
<p>Например, по умолчанию в проект по типу Empty добавляется файл <span class="b">appsettings.json</span>, который предназначен для конфигурации проекта и в том числе 
может определять и по умолчанию уже содержит конфигурацию логгера:</p>
<pre class="brush:js;">
{
  "Logging": {
    "LogLevel": {
...
<p>Узел <span class="b">"Logging"</span> задает настройки логгирования. Далее узел <span class="b">LogLevel</span> задает <span class="b">минимальный уровень</span> 
логгирования для трех категорий. Категория Default представляет универсальное применение ко всем категориям. 
То есть по умолчанию будут логгироваться все сообщения уровня Information. Но для логгера с категорией Microsoft будут логгироваться только сообщения 
уровня Warning, а для категории "Microsoft.Hosting.Lifetime" - уровня Information.</p>
<p>Мы можем, например, убрать логгирование для категории "Microsoft.Hosting.Lifetime":</p>
<pre class="brush:js;">
{
  "Logging": {
    "LogLevel": {
...
<p>Или установить логгирование связки различных уровней и категорий для различных провайдеров:</p>
<pre class="brush:js;">
{
  "Logging": {
    "Debug": {
...
<p>Здесь определяются четыре правила. Одно правило определено для провайдера Debug:</p>
<pre class="brush:js;">
"Debug": {
	"LogLevel": {
        "Default": "Debug"
...
<p>То есть для провайдера Debug определено логгирование сообщения уровня Debug для всех категорий.</p>
<p>Для провайдера Console определено два правила:</p>
<pre class="brush:js;">
"Console": {
	"LogLevel": {
        "Default": "Information",
...
<p>В данном случае для всех категорий будут логироваться сообщения уровня "Information". Для категории "Microsoft" логируются сообщения уровня 
"Warning".</p>
<p>И также определено одно общее правило для всех провайдеров:</p>
<pre class="brush:js;">
"LogLevel": {
	"Default": "Error"
}
</pre>
<p>Логгирование сообщений уровня "Error" для всех категорий.</p>
<p>Следует учитывать, что настройки узла <code>Logging.{ммя_провайдера}.LogLevel</code> переопределяют настройки в узле <code>Logging.LogLevel</code></p>
<p>Также можно настроить применяемые фильтры с помощью фабрики логгера:</p>
<pre class="brush:c#;">
...
<p>Фильтр логгирования задается с помощью метода <span class="b">AddFilter()</span>, который позволяет задать для определенной категории определенный уовень логгирования. Например:</p>
<pre class="brush:c#;">builder.AddFilter("System", LogLevel.Debug)</pre>
<p>устанавает для категории "System" уровень <code>LogLevel.Debug</code> для всех провайдеров. Это все равно, если бы мы написали в файле appsettings.json:</p>
<pre class="brush:js;">
"Logging": {
    "LogLevel": {
      "System": "Debug"
...
<p>Второй вызов AddFilter обобщенный - обобщенный класс указывает на провайдер, для которого задается фильтр. То есть для провайдера DebugLoggerProvider 
для категории "Microsoft" устанавается уровень <code>LogLevel.Trace</code>:</p>
<pre class="brush:c#;">AddFilter&lt;DebugLoggerProvider&gt;("Microsoft", LogLevel.Trace);</pre>
<p>Этот вызов аналогичен следующему определению в файле конфигурации</p>
<pre class="brush:js;">
"Logging": {
    "Debug": {
      "LogLevel": {
...
<p>Можно определять фильтры сразу и в коде C#, и в файле конфигурации. При создании объекта ILogger ILoggerFactory выбирает и применяет одно правило для 
каждого провайдера. Выбор нужного правило состоит из ряда этапов:</p>
<ol>
<li><p>Выбираются все правила, которые соответствуют провайдеру. Если таких правил нет, то выбираются все правила, общие для всех провайдеров.</p></li>
<li><p>Из выбранных на предыдущем шаге правил выбирается правило, которое имеет наиболее длинное соответствие имени категории. Если такого нет, 
то выбираются все правила, общие для всех категорий.</p></li>
<li><p>Если на предыдущем шаге выбрано несколько правил, из них выбирается самое последнее (Порядковый номер правила соответствует его порядку определения 
в файле конфигурации или в коде. Причем правила из файла конфигурации предшествуют правилам, определенным в коде).</p></li>
<li><p>Если на предыдущем шаге не выбрано никаких правил, то применяется настройка <code>MinimumLevel</code>. Данную опцию можно опредееленить в коде, например, 
при настройке фабрики логгера:</p>
<pre class="brush:c#;">
var loggerFactory = LoggerFactory.Create(builder =&gt;
{
	builder.AddDebug();
...
<p>Для установки базовой конфигурации логгирования в классе Program при создании объекта IWebHostBuilder мы можем вызвать метод <span class="b">ConfigureLogging()</span>. 
Например, по умолчанию минимальный уровень сообщений представлен константой LogLevel.Information. А это значит, что сообщения, которые относятся к уровням 
Trace и Debug, будут игнорироваться. Установим в качестве минимального уровня Trace, изменив файл Program.cs следующим образом:</p>
<pre class="brush:c#;">
...
<p>После этого в файле Startup мы также сможем использовать логгирование, как раньше, только теперь мы сможем увидеть в логе также и отладочную информацию.</p>
...
<div class="nav"><p><a href="./2.29.php">Назад</a><a href="./">Содержание</a><a href="./2.27.php">Вперед</a></p></div></div>
<div class="socBlock">
