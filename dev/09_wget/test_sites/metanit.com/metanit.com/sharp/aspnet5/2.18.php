...
<html >
<head>
<title>ASP.NET Core | Конвейер обработки запроса</title>
<meta charset="utf-8" />
<meta name="description" content="Конвейер обработки запроса из компонентов middleware в ASP.NET Core, терминальные компоненты">
...
<h2>Конвейер обработки запроса</h2><div class="date">Последнее обновление: 01.11.2019</div>
...
<p>Как правило, для обработки запроса применяется не один, а несколько компонентов middleware. И в этом случае большую роль может играть порядок их 
помещения в конвейер обработки запроса, а также то, как они взаимодействуют с другими компонентами.</p>
<p>Кроме того, каждый компонент middleware может обрабатывать запрос до и после последующих в конвейере компонентов. Данное обстоятельство позволяет предыдущим компонентам корректировать результат обработки последующих компонентов.</p>
<p>Рассмотрим простейший пример. Определим следующий middleware для обработки запроса:</p>
<pre class="brush:c#;">
...
<p>Этот компонент в зависимости от строки запроса возвращает либо определенную строку, либо устанавливает код ошибки.</p>
<p>Допустим, мы хотим, чтобы пользователь был аутентифицирован при обращении к нашему приложению.  
Для этого добавим новый класс AuthenticationMiddleware:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Http;
using System.Threading.Tasks;
...
<p>Условно будем считать, что если в строке запроса есть параметр <code>token</code> и он имеет какое-нибудь значение, то пользователь 
аутентифицирован.  А если он не аутентифицирован, то надо необходимо ограничить доступ пользователям к приложению. 
Если пользователь не аутентифицирован, то устанавливаем статусный код 403, иначе передаем выполнение запроса следующему в конвейере делегату.</p>
<p>Поскольку компоненту RoutingMiddleware нет смысла обрабатывать запрос, если пользователь не аутентифицирован, то в конвейере компонент 
AuthenticationMiddleware должен быть помещен перед компонентом RoutingMiddleware:</p>
<pre class="brush:c#;">
...
<p>Таким образом, если мы сейчас запустим проект и обратимся по пути /index или /about и не передадим параметр token, то мы получим ошибку. Если же обратимся по пути /index или /about и передадим значение 
параметра token, то увидим искомый текст:</p>
<img src="./pics/2.39.png" alt="Организация pipeline в asp net core" />
<p>Добавим еще один компонент middleware, который назовем ErrorHandlingMiddleware:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Http;
using System.Threading.Tasks;
...
<p>В отличие от предыдущих двух компонентов ErrorHandlingMiddleware сначала передает запрос на выполнение последующим делегатам, а потом уже 
...
<code>await _next.Invoke(context);</code>, а после завершения обработки последующих компонентов вызывается та часть 
кода, которая идет после <code>await _next.Invoke(context);</code>. 
И в данном случае для ErrorHandlingMiddleware важен результат обработки запроса последующими компонентами. В частности, он устанавливает сообщения об 
ошибках в зависимости от того, как статусный код установили другие компоненты. Поэтому ErrorHandlingMiddleware должен быть помещен первым из всех трех компонентов:</p>
<pre class="brush:c#;">
public class Startup
{
    public void Configure(IApplicationBuilder app)
...
<p>Схематично конвейер обработки запроса будет выглядеть следующим образом:</p>
<img src="./pics/2.20.png" alt="Конвейер обработки запроса в ASP.NET Core" />
<p>В то же время, если к приложению обратится пользователь, не указав в строке запроса параметр token, то AuthenticationMiddleware не будет передавать дальше запрос на обработку, 
а конвейер обработки будет выглядеть так:</p>
<img src="./pics/2.21.png" alt="Middleware pipeline in ASP.NET Core" />
<p>В первом случае, если указан параметр token, то запрос будет обработан RoutingMiddleware:</p>
<img src="./pics/2.28.png" alt="Этапы обработки запроса в ASP.NET Core" />
<p>Иначе пользователь получит ошибку 403:</p>
<img src="./pics/2.29.png" alt="Цикл обработки запроса в ASP.NET Core" />
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://www.youtube.com/watch?v=hVQDxEJL6tE"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Видеокурс по ASP.NET Core. 10. Создание конвейера из компонентов middleware</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./2.4.php">Назад</a><a href="./">Содержание</a><a href="./2.21.php">Вперед</a></p></div></div>
<div class="socBlock">
