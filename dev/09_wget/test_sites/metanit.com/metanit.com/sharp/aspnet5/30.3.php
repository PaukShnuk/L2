...
<html >
<head>
<title>ASP.NET Core | Клиент javascript в SignalR</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и конфигурация клиента javascript в SignalR в ASP.NET Core, HubConnection и HubConnectionBuilder, взаимодействовие с хабом сервера">
...
<h2>Клиент javascript</h2><div class="date">Последнее обновление: 24.12.2019</div>
...
<h3>Установка</h3>
<p>Клиент javascript является одним из распространненых типов клиентов в SignalR, который может взаимодействовать 
с хабом на сервере. Для создания клиента js необходимо добавить в проект специальную библиотеку.</p>
<h4>Установка через npm</h4>
<p>При использовании npm надо установить пакет <span class="b">@microsoft/signalr</span>:</p>
<pre class="brush:js;">npm install @microsoft/signalr</pre>
<p>Определение пакета в файла package.json:</p>
<pre class="brush:js;">
{
  "version": "1.0.0",
  "name": "signalrapp",
...
<h4>Получение из CDN</h4>
<p>Также можно использовать сети CDN, которые содержат данную библиотеку. В частности, подключить SignalR на страницу 
можно по одному из следующих адресов:</p>
<pre class="brush:xml;">
...
<h4>Установка через LibMan</h4>
<p>В Visual Studio также можно использовать менеджер LibMan для загрузки SignalR в проект:</p>
<img src="./pics/signalr3.png" alt="Установка SignalR через LibMan" />
<h3>Подключение к хабу</h3>
<p>Допустим, на стороне сервера определен следующий хаб:</p>
<pre class="brush:c#;">
...
<p>Чтобы подключиться к хабу, применяется объект <span class="b">signalR.HubConnectionBuilder</span>:</p>
<pre class="brush:js;">
const hubConnection = new signalR.HubConnectionBuilder()
	.withUrl("/chat")
	.build();
</pre>
<p>Через его метод <span class="b">withUrl</span> передается адрес, по которому доступен хаб. А метод <span class="b">build()</span> 
собственно создает объек подключения - объект <span class="b">HubConnection</span>, через который мы можем взаимодействовать с хабом.</p>
<h3>Отправка данных хабу</h3>
<p>Для отправки данных хабу у объекта HubConnection применяется метод <span class="b">invoke()</span>. Например, выше в классе хаба 
бы определен метод Send, который принимает два параметра. Обратимся к этому методу:</p>
<pre class="brush:js;">hubConnection.invoke('Send', "Hello SignalR", "Tom");</pre>
<p>Первый параметр метода представляет имя метода хаба, к которому идет обращения. Например, в данном случае это 
метод Send. Все последующие параметры передают данные для параметров метода хаба. Так, метод Send в хабе принимает два строковых параметра. 
Соответственно в методе invoke мы можем передать для них данные.</p>
<p>Параметры передаются по позиции: второй аргумент метода invoke передает значение для первого параметра метода Send, третий аргумент в invoke - для второго параметра в Send и так далее.</p>
<h3>Вызов метода клиента из хаба</h3>
<p>Клиент может обращаться к методу хаба, но и заб может вызывать метод на клиенте. Для этого объект HubConnection с помощью метода 
<span class="b">on</span> должен зарегистрировать функцию, которую сможет вызывать хаб. Например, выше в методе Send в хабе 
полученные данные транслировались на функцию Receive всем подключенным клиентам:</p>
<pre class="brush:c#;">await Clients.All.SendAsync("Receive", message, userName);</pre>
<p>Первый аргумент функции SendAsync как раз представляет название функции в коде клиента, которая будет вызываться. Последующие 
аргументы передают данные, которые получит функция на клиенте.</p>
<p>В этом случае на клиенте мы могли бы так определить функцию Receive:</p>
<pre class="brush:js;">
...
<p>Первый параметр функции on - это название вызываемой хабом функции - в данном случае Receive. Второй параметр - функция, 
которая получает переданные хабом данные.</p>
<h3>Логгирование</h3>
<p>У объекта <code>signalR.HubConnectionBuilder</code> есть метод <span class="b">configureLogging()</span>, в который передается 
...
<p>Все возможные уровни логгирования:</p>
<ul>
<li><p><code>signalR.LogLevel.None</code>: логгирование отключено</p></li>
<li><p><code>signalR.LogLevel.Critical</code>: логгирование сообщений об ошибках, которые относятся ко всему приложению в целом</p></li>
<li><p><code>signalR.LogLevel.Error</code>: логгирование сообшений об ошибках, которые относятся к текущей операции</p></li>
<li><p><code>signalR.LogLevel.Warning</code>: логгирование сообщений, которые не представляют ошибки</p></li>
<li><p><code>signalR.LogLevel.Information</code>: логгирование информационных сообщений</p></li>
<li><p><code>signalR.LogLevel.Debug</code>: логгирование диагностических сообщений, используемых при отладке</p></li>
<li><p><code>signalR.LogLevel.Trace</code>: логгирование диагностических сообщений с детальной информацией</p></li>
</ul>
<h3>Конфигурация клиента</h3>
<h4>Настройка типа транспорта</h4>
<p>Для настройки транспорта в методе <span class="b">withUrl()</span> у дополнительного параметра устанавливается свойство 
<code>transport</code>, которому передаются значения объекта <span class="b">HttpTransportType</span>: WebSockets, LongPolling или ServerSentEvents. 
По умолчанию используются все эти виды транспорта, но можно указать только один из них:</p>
<pre class="brush:c#;">
const hubConnection = new signalR.HubConnectionBuilder()
...
<p>Установка нескольких типов транспорта:</p>
<pre class="brush:c#;">
const hubConnection = new signalR.HubConnectionBuilder()
...
<p>Единственное, что надо учитывать, что сервер тоже должен поддерживать устанавливаемые виды транспорта.</p>
<h4>Установка времени таймаута</h4>
<p>Свойство <span class="b">serverTimeoutInMilliseconds</span> объекта <span class="b">HubConnection</span> представляет таймаут, в течение которого подключение считается активным. 
...
метод <span class="b">onclose()</span>. Рекомендуется устанавливать это значение в два раза большим, чем праметр KeepAliveInterval на стороне сервера. 
По умолчанию равно 30 секунд (30000 миллисекунд).</p>
<p>Например, установим таймаут в виде 10 минут:</p>
<pre class="brush:js;">
...
<div class="nav"><p><a href="./30.2.php">Назад</a><a href="./">Содержание</a><a href="./30.4.php">Вперед</a></p></div></div>
<div class="socBlock">
