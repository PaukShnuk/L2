...
<html >
<head>
<title>ASP.NET Core | RequestLocalizationMiddleware</title>
<meta charset="utf-8" />
<meta name="description" content="Компонент RequestLocalizationMiddleware в ASP.NET Core, определение и установка культуры с помощью параметров строки запроса, заголовка accept-language и куки, метод app.UseRequestLocalization">
...
<h2>RequestLocalizationMiddleware</h2><div class="date">Последнее обновление: 18.12.2019</div>
...
<p>В прошлой теме было рассмотрено получение и установка культуры с помощью самописного функционала. Однако ASP.NET Core уже по умолчанию предоставляет специальный компонент middleware, который упрощает нам локализацию - 
<span class="b">RequestLocalizationMiddleware</span>. Пр каждом запросе этот компонент автоматически устанавливает значения культуры на основе пришедших в запросе данных.</p>
<p>Для применения RequestLocalizationMiddleware используется специальный метод расширения <span class="b">app.UseRequestLocalization</span>. Используем его, изменив файл 
<span class="b">Startup.cs</span>:</p>
<pre class="brush:c#;">
...
<p>Метод <code>UseRequestLocalization()</code> инициализирует объект <span class="b">RequestLocalizationOptions</span> с помощью ряда значений. 
В частности, у RequestLocalizationOptions устанавливается текущая культура с помощью свойства <span class="b">DefaultRequestCulture</span>, 
а также список поддерживаемых культур посредством свойств <span class="b">SupportedCultures</span> и <span class="b">SupportedUICultures</span>.</p>
<p>При каждом запросе в объекте RequestLocalizationOptions перебирается список провайдеров. Первый попавшийся провайдер, 
который определит культуру, будет использоваться для обработки запроса.</p>
<p>По умолчанию в RequestLocalizationOptions определено несколько провайдеров, каждый из которых связан с определенным источником данных:</p>
<ul>
<li><p><span class="b">QueryStringRequestCultureProvider</span></p></li>
<li><p><span class="b">CookieRequestCultureProvider</span></p></li>
<li><p><span class="b">AcceptLanguageHeaderRequestCultureProvider</span></p></li>
</ul>
<h3>QueryStringRequestCultureProvider</h3>
<p><span class="b">QueryStringRequestCultureProvider</span> устанавливает текущую культуру на основании строки запроса. При этом в запросе должны присутствовать 
параметры <code>culture</code> и <code>ui-culture</code>. Например, запрос может выглядеть следующим образом:</p>
<pre class="brush:js;">http://localhost:5000/?culture=en-US&ui-culture=en-US</pre>
<p>В данном случае будет использоваться культура "en-US".</p>
<p>Если один из этих двух параметров опущен, то другой применяется сразу для CurrentCulture и CurrentUICulture.</p>
<h3>CookieRequestCultureProvider</h3>
<p>CookieRequestCultureProvider извлекает код культуры из куки. По умолчанию такая куки имеет имя ".AspNetCore.Culture", а сам формат хранимого в ней значения выглядит следующим образом:</p>
<pre class="brush:js;">c=%LANGCODE%|uic=%LANGCODE%</pre>
<p>Здесь <code>c</code> передает CurrentCulture, а <code>uic</code> - CurrentUICulture. Например: <code>c=de-DE|uic=de-DE</code>.</p>
<p>Также кука может хранить только одно значение, тогда оно будет распространяться на оба параметра.</p>
<h3>AcceptLanguageHeaderRequestCultureProvider</h3>
<p>AcceptLanguageHeaderRequestCultureProvider извлекает код культуры из заголовка <span class="b">Accept-Language</span>, который передается в 
запросе. Обратите внимание, что браузеры могут автоматически настраивать этот заголовок. И даже если вы установите в определенную культуру 
по умолчанию (как в случае выше это "ru-RU"), значение этого заголовка ее сбрасывает. Например, на руссифцированной операционной системе установлен англоязычный браузер, который 
посылает заголовок "accept-language: en-US,en;q=0.9". В итоге и культура будет "en-US".</p>
<h3>Приоритет провайдеров</h3>
<p>Наиболее приоритетным является <code>QueryStringRequestCultureProvider</code>. Если в строке запроса не найдено определение культуры, 
тогда используется провайдер <code>CookieRequestCultureProvider</code>, который использует куки. Если в куках не найдено определение культуры, тогда применяется 
<code>AcceptLanguageHeaderRequestCultureProvider</code>, который использует заголовок Accept-Language.</p>
<p>Например, возьмем метод контроллера, который выводит культуру:</p>
<pre class="brush:c#;">
...
<p>И передадим приложению через строку запроса языковой код:</p>
<img src="./pics/localization3.png" alt="RequestLocalizationMiddleware в ASP.NET Core" />
<p>Стоит отметить, что не все переданные значения будут использоваться для установки культуры, а только те, которые определены в свойствах SupportedCultures 
и SupportedUICultures при настройке в классе Startup. Если будет передана не поддерживаемая культура, то вместо нее будет применяться культура по умолчанию.</p>
...
<div class="nav"><p><a href="./28.1.php">Назад</a><a href="./">Содержание</a><a href="./28.3.php">Вперед</a></p></div></div>
<div class="socBlock">
...
$(function() { 
  ...
  $("#toggle-theme").on("click", function() {
	...
	$("li:contains('Глава 28.')").removeClass('expandable').addClass('collapsable').children('ul').css({"display":"block"});
	$("li:contains('Глава 28.')").children('div').removeClass('expandable-hitarea').addClass('collapsable-hitarea');
