...
<html >
<head>
<title>ASP.NET Core | Куки</title>
<meta charset="utf-8" />
<meta name="description" content="Установка, получение и удаление кук cookie в ASP.NET Core, коллекция IRequestCookieCollection, Request.Cookies и Response.Cookies">
...
<h2>Куки</h2><div class="date">Последнее обновление: 03.11.2019</div>
...
<p>Куки представляют самый простой способ сохранить данные пользователя. Куки храняться на компьютере пользователя и могу устанавливаться как на сервере, так и на клиенте. 
Так как куки посылаются с каждым запросом на сервер, то их максимальный размер ограничен 4096 байтами.</p>
<p>Для работы с куками также можно использовать контекст запроса HttpContext, который передается в качестве параметра в компоненты middleware, а также доступен в контроллерах и 
RazorPages.</p>
<p>Чтобы получить куки, которые приходят вместе с запросом к приложению, нам надо использовать коллекцию <span class="b">Request.Cookies</span> 
объекта HttpContext. Эта коллекция представляет объект <span class="b">IRequestCookieCollection</span>, 
в котором каждый элемент - это объект <span class="b">KeyValuePair&lt;string, string&gt;</span>, то есть некоторую пару ключ-значение.</p>
<p>Для этой коллекции определено несколько методов:</p>
<ul>
<li><p><code>bool ContainsKey(string key)</code>: возвращает true, если в коллекции кук есть куки с ключом key</p></li>
<li><p><code>bool TryGetValue(string key, out string value)</code>: возвращает true, если удалось получить значение куки с ключом key в переменную value</p></li>
</ul>
<p>Стоит отметить, что куки - это строковые значения. Неважно, что вы пытаетесь сохранить в куки - все это необходимо приводить к строке и соответственно получаете из 
кук вы тоже строки.</p>
<p>Например, получим куку "name":</p>
<pre class="brush:c#;">
if (context.Request.Cookies.ContainsKey("name"))
	string name = context.Request.Cookies["name"];
</pre>
<p>Повторюсь, что коллекция context.Request.Cookies служит только для получения значений кук.</p>
<p>Для установки кук, которые отправляются в ответ клиенту, применяется объект <span class="b">context.Response.Cookies</span>, 
который представляет интерфейс <span class="b">IResponseCookies</span>. Этот интерфейс определяет два метода:</p>
<ul>
<li><p><span class="b">Append(string key, string value)</span>: добавляет для куки с ключом key значение value</p></li>
<li><p><span class="b">Delete(string key)</span>: удаляет куку по ключу</p></li>
</ul>
<p>Например, объединим установку и получение кук в методе Configure класса Startup:</p>
<pre class="brush:c#;">
...
<p>При получении запроса мы смотрим, установлена ли кука "name":</p>
<pre class="brush:c#;">if (context.Request.Cookies.ContainsKey("name"))</pre>
<p>Если кука не установлена (например, при первом обращении к приложению), то устанавливаем ее и отправляем пользователю в ответ строку "Hello World!".</p>
<pre class="brush:c#;">
context.Response.Cookies.Append("name", "Tom");
await context.Response.WriteAsync("Hello World!");
</pre>
<p>Если кука установлена, то получаем ее значение и отправляем его пользователю</p>
<pre class="brush:c#;">
string name = context.Request.Cookies["name"];
await context.Response.WriteAsync($"Hello {name}!");
</pre>
<img src="./pics/2.41.png" alt="Куки в ASP.NET Core" />
<p>После установки кук в результате первого запроса в браузере будет установлена кука name. И через средства разработчика в веб-браузере мы сможем 
увидеть ее значение (если браузер поддерживает подобное). Например, в Google Chrome посмотреть куки можно на вкладке <span class="b">Application -&gt; Cookies</span>:</p>
<img src="./pics/2.42.png" alt="Установка и получение cookie куки в ASP.NET Core" />
<p>В примере выше мы также могли бы применить метод <code>TryGetValue()</code> для получения кук:</p>
<pre class="brush:c#;">
app.Run(async (context) =&gt;
{
	string login;
...
<p>Стоит отметить, что пользователь через подобные инструменты (если веб-браузер позволяет) может вручную поменять значение кук, либо вовсе удалить их.</p>
...
<div class="nav"><p><a href="./2.11.php">Назад</a><a href="./">Содержание</a><a href="./2.26.php">Вперед</a></p></div></div>
<div class="socBlock">
