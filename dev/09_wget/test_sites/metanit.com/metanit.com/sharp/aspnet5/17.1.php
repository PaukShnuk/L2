...
<meta charset="utf-8" />
<meta name="description" content="Обработка ошибок и исключений в ASP.NET Core, определение страниц с ошибками">
...
<h2>Обработка ошибок</h2><div class="date">Последнее обновление: 06.11.2019</div>
...
<p>Ошибки в приложении можно условно разделить на два типа: исключения, которые возникают в процессе выполнения кода (например, деление на 0), и 
стандартные ошибки протокола HTTP (например, ошибка 404).</p>
<p>Обычные исключения могут быть полезны для разработчика в процессе создания приложения, но простые пользователи не должны будут их видеть.</p>
<h3>UseDeveloperExceptionPage</h3>
<p>Если мы создаем проект ASP.NET Core, например, по типу Empty (да и в других типах проектов), то в классе Startup мы можем найти в начале метода <code>Configure()</code> следующие строки:</p>
<pre class="brush:c#;">
...
<p>Если приложение находится в состоянии разработки, то с помощью middleware <code>app.UseDeveloperExceptionPage()</code> приложение перехватывает исключения и 
выводит информацию о них разработчику.</p>
<p>Например, изменим класс Startup следующим образом:</p>
<pre class="brush:c#;">
...
<p>В middleware app.Run симулируется генерация исключения при делении ноль. И если мы запустим проект, то в браузере мы увидим 
информацию об исключении:</p>
<img src="./pics/17.1.png" alt="Обработка исключений в ASP.NET Core" />
<p>Этой информации достаточно, чтобы определить где именно в коде произошло исключение.</p>
<p>Теперь посмотрим, как все это будет выглядеть для простого пользователя. Для этого изменим метод Configure:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
{
...
<p>Выражение <code>env.EnvironmentName = "Production";</code> устанавливает режим развертывания вместо режима разработки. В этом случае выражение <code>if (env.IsDevelopment())</code> будет возвращать false, и мы увидим в браузере что-то наподобие "HTTP ERROR 500"</p>
<img src="./pics/17.9.png" alt="HTTP ERROR 500 в ASP.NET Core" />
<h3>UseExceptionHandler</h3>
<p>Это не самая лучшая ситуация, и нередко все-таки возникает необходимость дать пользователям некоторую информацию о том, что же все-таки произошло. Либо потребуется как-то обработать данную ситуацию. 
Для этих целей можно использовать еще один встроенный middleware в виде метода <span class="b">UseExceptionHandler()</span>. Он перенаправляет 
при возникновении исключения на некоторый адрес и позволяет обработать исключение. Например, изменим метод Configure следующим образом:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
{
	env.EnvironmentName = "Production";
...
<p>Метод <code>app.UseExceptionHandler("/error");</code> перенаправляет при возникновении ошибки на адрес "/error".</p>
<p>Для обработки пути по определенному адресу здесь использовался метод <code>app.Map()</code>. В итоге при возникновении исключения будет срабатывать делегат 
из метода app.Map.</p>
<img src="./pics/17.3.png" alt="Error Handling in ASP.NET Core" />
<p>Следует учитывать, что оба middleware - <code>app.UseDeveloperExceptionPage()</code> и <code>app.UseExceptionHandler()</code> 
следует помещать ближе к началу конвейера middleware.</p>
<h3>Обработка ошибок HTTP</h3>
<p>В отличие от исключений стандартный функционал проекта ASP.NET Core почти никак не обрабатывает ошибки HTTP, например, в случае если ресурс не найден. 
...
Но с помощью компонента <span class="b">StatusCodePagesMiddleware</span> можно добавить в проект отправку информации о статусном коде. 
Для этого добавим в метод <code>Configure()</code> класса Startup вызов <code>app.UseStatusCodePages()</code>:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
{
	if (env.IsDevelopment())
...
<p>Здесь мы можем обращаться только по адресу "/hello". При обращении ко всем остальным адресам браузер отобразит базовую информацию об ошибке:</p>
<img src="./pics/17.4.png" alt="UseStatusCodePages в ASP.NET Core" />
<p>Данный метод позволяет настроить отправляемое пользователю сообщение. В частности, мы можем изменить вызов метода так:</p>
<pre class="brush:c#;">app.UseStatusCodePages("text/plain", "Error. Status code : {0}");</pre>
<p>В качестве первого параметра указывается MIME-тип ответа, а в качестве второго - собственно то сообщение, которое увидит пользователь. В сообщение мы можем 
передать код ошибки через плейсхолдер "{0}".</p>
<p>Вместо метода <code>app.UseStatusCodePages()</code> мы также можем использовать еще пару других, которые также обрабатываю ошибки HTTP.</p>
<p>С помощью метода <code>app.UseStatusCodePagesWithRedirects()</code> можно выполнить переадресацию на определенный метод, который непосредственно обработает статусный код:</p>
<pre class="brush:c#;">app.UseStatusCodePagesWithRedirects("/error?code={0}");</pre>
<p>Здесь будет идти перенаправление по адресу "/error?code={0}". В качестве параметра через плейсхолдер "{0}" будет передаваться статусный код 
ошибки.</p>
<p>Но теперь при обращении к несуществующему ресурсу клиент получит статусный код 302 / Found. То есть формально несуществующий ресурс будет существовать, просто статусный код 302 
будет указывать, что ресурс перемещен на другое место - по пути "/error/404".</p>
<p>Подобное поведение может быть неудобно, особенно с точки зрения поисковой индексации, и в этом случае мы можем применить другой метод 
<span class="b">app.UseStatusCodePagesWithReExecute()</span>:</p>
<pre class="brush:c#;">app.UseStatusCodePagesWithReExecute("/error", "?code={0}");</pre>
<p>Первый параметр метода указывает на путь перенаправления, а второй задает параметры строки запроса, которые будут передаваться при перенаправлении. 
Вместо плейсхолдера {0} опять же будет передаваться статусный код ошибки. Формально мы получим тот же ответ, так как так же будет идти перенаправление на путь "/error?code=404". Но теперь браузер получит оригинальный статусный код 404.</p>
...
<h3>Настройка обработки ошибок в web.config</h3>
<p>Еще один способ обработки кодов ошибок представляет собой определение и настройка в файле конфигурации <span class="ii">web.config</span> элемента 
<span class="b">httpErrors</span>. Этот способ в принципе использовался и в других версиях ASP.NET. 
В ASP.NET Core он также доступен, однако имеет очень ограниченное действие. В частности, мы его можем использовать только при развертывании на IIS, а также не можем использовать ряд настроек.</p>
<p>Итак, добавим в корень проекта новый элемент <span class="b">Web Configurarion File</span>, который естественно назовем <span class="b">web.config</span>:</p>
<img src="./pics/17.8.png" alt="Обработка ошибок в web.config" />
...
<p>Также для обработки ошибок добавим в корень проекта новый файл <span class="ii">404.html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>По аналогии можно добавить файл <span class="ii">403.html</span> для ошибки 403.</p>
<p>Итак, элемент httpErrors имеет ряд настроек. Для тестирования настроек локально, необходимо установить атрибут <code>errorMode="Custom"</code>. 
Если тестирование необязательно, и приложение уже развернуто для использования, то можно установить значение <code>errorMode="DetailedLocalOnly"</code>.</p>
<p>Значение <code>existingResponse="Replace"</code> позволит отобразить ошибку по оригинальному запрошенному пути без переадресации.</p>
<p>Внутри элемента httpErrors с помощью отдельных элементов <span class="b">error</span> устанавливается обработка ошибок. Атрибут <code>statusCode</code> 
задает статусный код, атрибут <code>path</code> - адрес url, который будет вызываться, а атрибут <code>responseMode</code> указывает, как будет обрабатываться ответ вызванному url. 
Атрибут <code>responseMode</code> имеет значение <code>File</code>, что позволяет рассматривать адрес url из атрибута path как статическую страницу и использовать ее в качестве ответа</p>
<p>Настройки элемента <code>httpErrors</code> могут наследоваться с других уровней, например, от файла конфигурации <code>machine.config</code>. И чтобы удалить 
все унаследованные настройки, применяется элемент <code>&lt;clear /&gt;</code>. Чтобы удалить настройки для отдельных ошибок, применяется элемент 
<code>&lt;remove /&gt;</code>.</p>
<p>Для тестирования используем следующий класс Startup:</p>
<pre class="brush:c#;">
public class Startup
{
	public void Configure(IApplicationBuilder app)
...
<p>И после обращения к несуществующему ресурсу в приложении отобразится содержимое из файла <span class="ii">404.html</span>.</p>
<img src="./pics/17.6.png" alt="Настройка обработки ошибок в web.config в ASP.NET Core" />
...
<div class="nav"><p><a href="./2.14.php">Назад</a><a href="./">Содержание</a><a href="./18.6.php">Вперед</a></p></div></div>
<div class="socBlock">
