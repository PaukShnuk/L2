...
<html >
<head>
<title>ASP.NET Core | Локализация представлений</title>
<meta charset="utf-8" />
<meta name="description" content="Локализация представлений в ASP.NET Core, создание ресурсов для представления, сервис IViewLocalizer, метод AddViewLocalization">
...
<h2>Локализация представлений</h2><div class="date">Последнее обновление: 18.12.2019</div>
...
<p>Для локализации строк в представлении применяется сервис <span class="b">IViewLocalizer</span>, который по своему действию аналогичен сервису IStringLocalizer. 
Встроенная реализация этого интерфейса - класс ViewLocalizer в зависимости от пути к локализуемому представлению пытается найти нужный файл ресурсов.</p>
<p>При локализации представлений применяются те же соглашения по именованию, что и при локализации контроллеров. То есть имя файла ресурсов, предназначенного для 
представления, должно отражать его путь в проекте. Например, пусть нам надо локализовать представление <span class="ii">Views/Home/Index.cshtml</span>. 
В этом случае ViewLocalizer будет искать для него файл ресурсов по следующим путям:</p>
<ul>
<li><p><span class="ii">[папка_ресурсов]/Views/Home/Index.[код_культуры].resx</span></p></li>
<li><p><span class="ii">[папка_ресурсов]/Views.Home.Index.[код_культуры].resx</span></p></li>
</ul>
<p>Например, добавим для представления <span class="ii">Views/Home/Index.cshtml</span> ресурсы в папку Resources. Воспользуемся первым вариантов:</p>
<img src="./pics/localization11.png" alt="Локализация представлений в ASP.NET Core" />
<p>Как видно из скриншота, папка ресурсов в данном случае называется Resources, и в ней с прошлой темы остались ресурсы для локализации контроллеров. 
Использование структуры папок позволяет избежать мешанины в одной папке ресурсов для локализуемых сущностей и должным образом их структуризировать.</p>
<p>В данном случае я определил три файла ресурсов:</p>
<ul>
<li><p><span class="ii">Resources/Views/Home/Index.ru.resx</span> (для русского языка)</p></li>
<li><p><span class="ii">Resources/Views/Home/Index.en.resx</span> (для английского языка)</p></li>
<li><p><span class="ii">Resources/Views/Home/Index.de.resx</span> (для немецкого языка)</p></li>
</ul>
<p>Каждый из файлов ресурсов имеет два ресурса с одинаковыми ключами Header и Message, но значениями на соответствующем языке. Например, ресурс для английского языка:</p>
<img src="./pics/localization12.png" alt="Ресурсы представлений в ASP.NET Core MVC" />
<p>А аналогичный ресурс для русского языка выглядит следующим образом:</p>
<img src="./pics/localization13.png" alt="Локализация представлений на языки в ASP.NET Core MVC" />
<p>После добавления ресурсов сделаем ребилд (перестроение) проекта.</p>
<p>Используем эти ресурсы в представлении <span class="ii">Views/Home/Index.cshtml</span>:</p>
<pre class="brush:xml;">
...
<p>Первой строкой подключаем пространство имен, где определен IViewLocalizer. Второй строкой инжектируем данный сервис в представление и даем ему название 
<code>Localizer</code>.</p>
<p>И в конце добавим данный сервис в приложение, изменив класс <span class="b">Startup</span>:</p>
<pre class="brush:c#;">
...
<p>Для добавления сервиса IViewLocalizer в методе ConfigureServices вызывается метод <span class="b">AddViewLocalization()</span>.</p>
<p>Также следует отметить, что в методе Configure подключен RequestLocalizationMiddleware, который определяет набор допустимых культур и культуру по умолчанию.</p>
<p>Запустим приложение и передадим русскоязычную культуру:</p>
<img src="./pics/localization14.png" alt="View localization in ASP.NET Core" />
<p>Передадим другую культуру, например, англоязычную:</p>
<img src="./pics/localization15.png" alt="View localization in ASP.NET Core MVC" />
...
<div class="nav"><p><a href="./28.4.php">Назад</a><a href="./">Содержание</a><a href="./28.6.php">Вперед</a></p></div></div>
<div class="socBlock">
