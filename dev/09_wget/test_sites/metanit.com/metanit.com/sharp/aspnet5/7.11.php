...
<html >
<head>
<title>ASP.NET Core | Передача данных во View Component</title>
<meta charset="utf-8" />
<meta name="description" content="Передача параметров и dependency injection во View Component в ASP.NET Core MVC">
...
<h2>Передача данных во View Component</h2><div class="date">Последнее обновление: 27.11.2019</div>
...
<p>В View Component при необходимости можно передавать данные. Во-первых, View Component может принимать некоторые параметры, которые ему передаются при вызове 
...
Рассмотрим, оба случая передачи данных.</p>
<h3>Передача параметров в View Component</h3>
<p>Через параметры метода Invoke/InvokeAsync мы можем получать извне некоторые данные. Например, определим следующий компонент:</p>
<pre class="brush:c#;">
...
<p>В методе Invoke компонент принимает один параметр - includeSeconds, который представляет тип bool. Если этот параметр равен true, то строка со временем также содерджит секунды, 
если параметр равен false - секунды опускаются.</p>
<p>Теперь обратимся к этому компоненту в представлении:</p>
<pre class="brush:xml;">
&lt;p&gt;С секундами: &lt;br /&gt;
...
<p>Для передачи значений параметрам метода Invoke/InvokeAsync в методе <code>Component.InvokeAsync()</code> в качестве второго аргумента указывается 
анонимный объект, который устанавливает значения всех параметров.</p>
<p>При использовании тег-хелпера параметры компонента определяются как атрибуты тега, которым присваивается необходимое значение. Причем если у нас исползуется 
camelcase, при котором каждое подслово в составе составного слова пишется с большой буквы, например, includeSeconds, то в названии атрибута все подслова разделяются дефисом и начинаются со строчной буквы.</p>
<p>Результат выполнения:</p>
<img src="./pics/viewcomponent10.png" alt="Параметры в View Component в ASP.NET Core" />
<p>Подобным образом мы можем использовать и больщее количество параметров. Например, изменим код компонента следующим образом:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc;
using System;
namespace ViewComponentsApp.Components
...
<p>Новый параметр <code>format24</code> указывает в каком формате будет выводиться время - в 24-часовом или в 12 часовом.</p>
<p>Обратимся к компоненту в представлении:</p>
<pre class="brush:xml;">
...
<img src="./pics/viewcomponent11.png" alt="Parameters in View Component in ASP.NET Core" />

<p>Несмотря на то, что мы вроде равным образом можем обращаться к View Component через метод Component.InvokeAsync или через tag-хелпер, 
тем не менее при передаче параметров между этими способами есть различия. В частности, при использовании тег-хелпера мы должны определить все параметры для View Component. 
Но рассмотрим следующую ситуацию. View Component устанавливает значения по умолчанию для своих параметров:</p>
<pre class="brush:c#;">
public string Invoke(bool includeSeconds = false, bool format24 = true)
{
	// остальное содержание метода
</pre>
<p>То есть по умолчанию применяется 24-часовой формат, а секунды не включаются.</p>
<p>Попробуем при обращении к компоненту опустить некоторые параметры (ведь теперь они имеют значения по умолчанию):</p>
<pre class="brush:xml;">
&lt;p&gt;
...
<p>При запуске приложения мы увидим, что метод Component.InvokeAsync сработал как надо, а тег-хелпер нет.</p>
<img src="./pics/viewcomponent12.png" alt="Передача параметров в View Component в ASP.NET Core" />
<p>При этом View Component может принимать более сложные данные. Например, определим в проекте в папке Models новый класс User:</p>
<pre class="brush:c#;">
public class User
{
...
<p>В папке Components определим новый View Component - класс PersonViewComponent:</p>
<pre class="brush:c#;">
...
<p>Данный компонент принимает объкт User и возвращает его данные в виде строки.</p>
<p>Теперь применим данный компонент в представлении:</p>
<pre class="brush:xml;">
...
<img src="./pics/viewcomponent5.png" alt="Передача сложных данных в View Component в ASP.NET Core MVC" />
<h3>Внедрение зависимостей</h3>
<p>Как и другие классы, View Component может получать внеденные в приложение зависимости.</p>
<p>Например, определим в проекте интерфейс и класс сервиса:</p>
<pre class="brush:c#;">
...
<p>Зарегистрируем зависимость от ITimeService в классе Startup:</p>
<pre class="brush:c#;">
...
<p>Изменим код компонента Timer:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc;
using ViewComponentsApp.Services;
namespace ViewComponentsApp.Components
...
<p>При запуске приложения и вызове компонента ему автоматически будет передаваться нужный объект ITimeService. И также в представлении мы сможем его использовать:</p>
<pre class="brush:xml;">
&lt;div&gt;
    @await Component.InvokeAsync("Timer")
&lt;/div&gt;
...
<div class="nav"><p><a href="./7.6.php">Назад</a><a href="./">Содержание</a><a href="./7.12.php">Вперед</a></p></div></div>
<div class="socBlock">
