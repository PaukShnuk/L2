...
<html >
<head>
<title>ASP.NET Core | Создание провайдера конфгурации</title>
<meta charset="utf-8" />
<meta name="description" content="Создание провайдера конфгурации в ASP.NET Core, использование источника конфигурации IConfigurationSource и ConfigurationProvider">
...
<h2>Создание провайдера конфгурации</h2><div class="date">Последнее обновление: 14.11.2019</div>
...
<p>Для работы с конфигурацией в нашем распоряжение есть ряд функционала - для работы с json, xml и так далее. Однако, в каких-то ситуациях, возможно, 
этого функционала окажется недостаточно. И в этом случае мы можем определить свои источники и провайдеры конфигурации.</p>
...
<p>Допустим, мы хотим хранить конфигурацию в простом текстовом файле. И для этого добавим новый класс, который назовем TextConfigurationProvider:</p>
<pre class="brush:c#;">
...
<p>В этом классе с помощью FileStream и StreamReader происходит считывание текстового файла и добавление данных в словарь data. Для загрузки данных переопределяется 
...
<p>Далее нам надо обращаться к этому провайдеру. Для этого определим класс источника конфигурации, который назовем TextConfigurationSource:</p>
<pre class="brush:c#;">
using Microsoft.Extensions.Configuration;
...
<p>Источник конфгурации должен реализовать интерфейс <span class="b">IConfigurationSource</span>, и в частности, его метод <code>Build</code>. В этот 
метод в качестве параметра передается строитель конфгурации - то есть тот объект, с помощью которого в конструкторе класса Startup мы будем создавать конфигурацию.</p>
...
<p>Чтобы задействовать функционал источника конфигурации создадим вспомогательный класс TextConfigurationExtensions:</p>
<pre class="brush:c#;">
using Microsoft.Extensions.Configuration;
using System;
...
<p>Этот класс определяет для объекта IConfigurationBuilder метод расширения <code>AddTextFile()</code>, в котором создается источник конфигурации 
TextConfigurationSource, который затем добавляется к строителю конфигурации:</p>
<pre class="brush:c#;">
var source = new TextConfigurationSource(path);
builder.Add(source);
</pre>
<p>Теперь добавим в проект сам файл конфигурации. Пусть он будет называться <i>config.txt</i> и будет иметь следующее содержимое:</p>
<pre class="brush:c#;">
color
red
text
...
<img src="./pics/2.13.png" alt="Переопределение конфигурации в ASP.NET Core" />
<p>И в конце изменим клас Startup, чтобы он использовал конфигурацию из текстового файла:</p>
<pre class="brush:c#;">
...
<img src="./pics/2.11.png" alt="Работа с конфигурацией в ASP.NET Core" />
