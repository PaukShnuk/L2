...
<html >
<head>
<title>ASP.NET Core | Пакетный менеджер NPM</title>
<meta charset="utf-8" />
<meta name="description" content="Пакетный менеджер NPM в ASP.NET Core, добавление библиотек javascript и css, добавление npm в Visual Studio 2015">
...
<h2>Пакетный менеджер NPM</h2><div class="date">Последнее обновление: 16.12.2019</div>
...
<p>В прошлой теме было рассмотрено, как для управления пакетами клиентской стороны (то есть библиотеками javascript и css) в ASP.NET Core 
можно использовать менеджер Libman. Но мы также можем использовать и другой более привычный для мира фронтенд-разработки менеджер пакетов - NPM.</p>
<p>К примеру создадим новый проект ASP.NET Core по типу Empty. В приницпе можно выбрать в качестве типа Web Application, но в данном случае разницы никакой нет, так как он 
тоже по умолчанию не содержит функциональность NPM.</p>
<p>В первую очередь добавим в проект новый элемент <span class="b">npm Configuration File</span>. Назовем его <span class="b">package.json</span>:</p>
<img src="./pics/npm1.png" alt="NPM Configuration File in ASP.NET Core" />
<p>После добавления откроем этот файл:</p>
<pre class="brush:js;">
{
	"version": "1.0.0",
	"name": "asp.net",
...
<p>Изменим этот файл, добавив в него пакеты jquery и bootstrap:</p>
<pre class="brush:js;">
{
	"version": "1.0.0",
	"name": "asp.net",
...
<p>Определение каждого пакета содержится в виде: <code>"название_пакета" : "версия"</code>.</p>
<p>Сохраним файл, и после этого в проект будут добавлены все необходимые пакеты. Во-первых, в проекте в узле <span class="b">Dependencies</span> 
повится подузел npm, который будет содержать ссылки на добавленные пакеты.</p>
<p>Во-вторых, если мы нажмем на кноку <span class="b">Show All Files</span>, то также в проекте мы увидим скрытый узел <span class="b">node_modules</span>, 
который будет представлять физическую папку, в которой хранятся все файлы добавленных пакетов:</p>
<img src="./pics/npm2.png" alt="Пакетный менеджер NPM в ASP.NET Core" />
<p>Теперь используем добавленные библиотеки в проекте. Для этого добавим в проект папку <span class="b">wwwroot</span> и определим в ней новый файл <span class="ii">index.html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>На странице подключаются ранее добавленные библиотеки с помощью путей, начинающихся с названи каталога <code>node_modules</code>. Однако по умолчанию 
...
index.html не существуют. Поэтому изменим класс Startup:</p>
<pre class="brush:c#;">
using System.IO;
...
<p>Последний вызов <code>app.UseFileServer()</code> позволит сопоставлять все запросы с "/node_modules" с каталогом "node_modules".</p>
<p>В реальности, конечно, в большинстве случаев для библиотек из node_modules используют минификацию/бандлинг с помощью Grunt/Gulp, либо с помощью BundleConfig из первой тему этой главы с последующим копированием в папку wwwroot, поэтому не придется 
прибегать к проекции запросов на каталог node_modules. Но тем не менее так мы тоже можем обращаться к файлам в каталоге node_modules.</p>
<p>И теперь при обращении к приложению мы увидим нашу страницу index.html, к которой будут применяться стили bootstrap:</p>
<img src="./pics/npm3.png" alt="Подключение к node_modules в ASP.NET Core" />
...
<div class="nav"><p><a href="./13.7.php">Назад</a><a href="./">Содержание</a><a href="./13.1.php">Вперед</a></p></div></div>
<div class="socBlock">
