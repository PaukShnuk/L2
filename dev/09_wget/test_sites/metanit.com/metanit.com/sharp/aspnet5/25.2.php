...
<html >
<head>
<title>ASP.NET Core | Взаимодействие с React.JS</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие React.JS и ASP.NET Core Web API, отправка запросов с помощью AJAX">
...
<h2>Взаимодействие React.JS и ASP.NET Core</h2><div class="date">Последнее обновление: 21.11.2019</div>
...
<p>Продолжим работу с проектом из прошлой темы и рассмотрим непосредственно взаимодействие ASP.NET Core и React.JS. 
Для этого используем контроллеры Web API.</p>
<p>Добавим в проект папку <span class="b">Models</span> и в ней определим класс модели Phone, с которой будем работать:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Далее для контроллеров добавим в проект папку <span class="b">Controllers</span>, а в нее добавим новый контроллер PhonesController:</p>
<pre class="brush:c#;">
using System;
...
<p>В роли источника данных здесь используется статический список, который инициализируется двумя объектами Phone.</p>
<p>Для управления данными здесь применяются три метода: <code>Get()</code> возвращает список объектов, <code>Post</code> добавляет один объект и 
<code>Delete()</code> удаляет объект по id.</p>
<p>В итоге финальный проект будет выглядеть следующим образом:</p>
<img src="./pics/react7.png" alt="React in ASP.NET Core 3" />
<p>И для обращения к этим методам изменим файл <span class="b">app.jsx</span>:</p>
<pre class="brush:js;">
...
<p>И также изменим класс Startup, чтобы задействовать контроллеры:</p>
<pre class="brush:c#;">
...
<p>Веб-страница <span class="ii">index.html</span> остается той же, что и в прошлой теме:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/react5.png" alt="Использование React JS в ASP.NET Core Web API" />
...
<div class="nav"><p><a href="./25.1.php">Назад</a><a href="./">Содержание</a><a href="./32.1.php">Вперед</a></p></div></div>
<div class="socBlock">
