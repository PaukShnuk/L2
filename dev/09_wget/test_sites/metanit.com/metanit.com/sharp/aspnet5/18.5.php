...
<html >
<head>
<title>ASP.NET Core | Передача параметров в фильтры и установка зависимостей</title>
<meta charset="utf-8" />
<meta name="description" content="Передача параметров и установка зависимостей в фильтрах в ASP.NET Core, применение классов ServiceFilterAttribute и TypeFilterAttribute">
...
<h2>Передача параметров в фильтры и установка зависимостей</h2><div class="date">Последнее обновление: 11.12.2019</div>
...
<p>Фильтры могут принимать некоторые значения из вне. Для их приема мы можем определить в классе фильтра конструктор с соответствующими параметрами. Например:</p>
<pre class="brush:c#;">
...
<p>Здесь конструктор фильтра принимает два параметра. При применении фильтра мы можем передать значения для этих параметров:</p>
<pre class="brush:c#;">
...
<p>Если фильтр устанавливается глобально, то также при его установке необходимо передать ему нужные значения:</p>
<pre class="brush:c#;">
...
<h3>Фильтры и Dependency Injection</h3>
<p>Как и любые классы в ASP.NET Core, фильтры также могут принимать зависимости, устанавливаемые при старте приложения. Однако, если фильтр применяется непосредственно 
к контроллеру или его методу, то установка зависимостей через встроенный механизм Dependency Injection не будет работать. 
Все потому, что при применении атрибутов все значения должны быть переданы напрямую в их конструктор. Чтобы обойти это ограничение, 
надо использовать один из классов <span class="b">ServiceFilterAttribute</span> или <span class="b">TypeFilterAttribute</span>.</p>
<p>Например, пусть класс фильтра получает зависимость ILoggerFactory, которая позволяет создать объект логгера для логгирования событий приложения:</p>
<pre class="brush:c#;">
...
<p>Класс <span class="b">ServiceFilterAttribute</span> извлекает экземпляр фильтра напрямую из DI. Для этого при применении ему передается тип фильтра:</p>
<pre class="brush:c#;">
...
<p>Но чтобы данный фильтр стал доступен, его еще надо зарегистрировать в методе <code>ConfigureServices()</code> класса Startup:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
    services.AddControllersWithViews();
...
<p>Класс <span class="b">TypeFilterAttribute</span> создает объект фильтра с помощью фабрики 
<code>Microsoft.Extensions.DependencyInjection.ObjectFactory</code>, а с помощью механизма DI устанавливает все зависимости для создаваемого фильтра. 
В этом случае класс фильтра больше не надо регистрировать в ConfigureServices().</p>
<p>Применение TypeFilterAttribute аналогично ServiceFilterAttribute:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./18.1.php">Назад</a><a href="./">Содержание</a><a href="./18.2.php">Вперед</a></p></div></div>
<div class="socBlock">
