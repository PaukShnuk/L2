...
<html >
<head>
<title>ASP.NET Core | EnvironmentTagHelper</title>
<meta charset="utf-8" />
<meta name="description" content="Применение EnvironmentTagHelper в ASP.NET Core MVC, создание профиля отладки">
...
<h2>EnvironmentTagHelper</h2><div class="date">Последнее обновление: 06.12.2019</div>
...
<p>Тег <span class="b">EnvironmentTagHelper</span> используется для генерации определенной разметки html в зависимости от состояния приложения: находится ли приложение в процессе разработки, тестирования или уже опубликовано на сервере. 
Состояние проекта задается с помощью среды окружения <span class="b">ASPNETCORE_ENVIRONMENT</span>. Нередко данный тег-хелпер используется совместно с  LinkTagHelper и ScriptTagHelper. Его ключевой атрибут <span class="b">names</span> 
позволяет установить названия состояний среды, при которых применяется данный тег.</p>
<p>Например, мы могли бы подключать css-стили bootstrap с использованием данного тег-хелпера следующим образом:</p>
<pre class="brush:xml;">
...
<p>Атрибут <code>names</code> через запятую позволяет перечислить, к каким состояниям среды приложения применяется этот тег. 
В данном случае, если проект на стадии Development (то есть в разработке), то подключается локальный файл bootstrap.css. Если 
же проект находится в процессе тестирования (Staging) или уже опубликован (Production), то подключается файл bootstrap.min.css из CDN.</p>
<p>При обработке данного тега фреймворк использует значение переменной окружения <span class="b">ASPNETCORE_ENVIRONMENT</span>. При разработке по умолчанию эта переменная имеет 
значение "Development". В частности, мы можем увидеть ее значение, открыв свойства проекта и далее вкладку Debug:</p>
<img src="./pics/environmenttaghelper.png" alt="environmenttaghelper in ASP.NET Core" />
<p>Значение переменной ASPNETCORE_ENVIRONMENT привязано к определенному профилю. По умолчанию проект ASP.NET Core предлагает два профиля. Один называется 
<span class="b">IISExpress</span>, а другой по имени проекта.</p>
<p>Например, в данном случае проект находится в состоянии Development, поэтому выше определенный тег environment будет генерировать следующую разметку:</p>
<pre class="brush:xml;">&lt;link rel="stylesheet" href="/lib/bootstrap/dist/css/bootstrap.css" /&gt;</pre>
<p>Если же мы изменим значение ASPNETCORE_ENVIRONMENT на <code>Production</code>:</p>
<img src="./pics/environmenttaghelper1.png" alt="Переменная ASPNETCORE_ENVIRONMENT в ASP.NET Core" />
<p>В этом случае тег environment сгенерирует следующую разметку:</p>
<pre class="brush:xml;">&lt;link rel="stylesheet" href="https://ajax.aspnetcdn.com/ajax/bootstrap/4.3.0/css/bootstrap.min.css" /&gt;</pre>
<p>Кроме окна свойств проекта мы можем поменять значение ASPNETCORE_ENVIRONMENT для определенного профиля в файле <span class="b">launchSettings.json</span>, 
который находится в проекте в папке <span class="b">Properties</span> и который по умолчанию выглядит примерно так:</p>
<pre class="brush:js;">
{
  "iisSettings": {
...
<p>Здесь мы можем поменять значение <code>"ASPNETCORE_ENVIRONMENT"</code> на любое предпочтительное. При этом обращаю внимание, что это значение 
...
пребывает проект.</p>
<p>Также обращаю внимание, что тег environment касается не только подключения стилей или скриптов, но и добавление любой разметки html. Например, определим в представлении 
следующий код:</p>
<pre class="brush:xml;">
...
<img src="./pics/environmenttaghelper2.png" alt="Profile ASPNETCORE_ENVIRONMENT in ASP.NET Core" />
...
<div class="nav"><p><a href="./10.4.php">Назад</a><a href="./">Содержание</a><a href="./10.7.php">Вперед</a></p></div></div>
<div class="socBlock">
