...
<html >
<head>
<title>ASP.NET Core | Создание своего маршрута</title>
<meta charset="utf-8" />
<meta name="description" content="Создание своего маршрута в ASP.NET Core, реализация интерфейса IRouter">
...
<h2>Создание своего маршрута</h2><div class="date">Последнее обновление: 16.11.2019</div>
...
<p>По умолчанию каждому маршруту сопоставляется некоторый шаблон, которому должна соответствовать строка запроса. Однако мы можем уйти от определения шаблонов и 
построить собственную логику применения маршрута к запросу через реализацию интерфейса <span class="b">Microsoft.AspNetCore.Routing.IRouter</span>.</p>
<p>Этот интерфейс определяет два метода:</p>
<pre class="brush:c#;">
...
<p>Метод <code>RouteAsync()</code> предназначен для обработки входящих запросов. А метод <code>GetVirtualPath()</code> применяется для генерации ссылок в соответствии 
с данным маршрутом.</p>
<p>К примеру, добавим в проект класс AdminRoute:</p>
<pre class="brush:c#;">
...
<p>Данный класс реализует метод <code>RouteAsync()</code>, который обрабатывает входящий запрос. В качестве параметра в RouteAsync передается объект 
<span class="b">RouteContext</span>, через который мы можем получить доступ к данным запроса. В частности, здесь мы смотрим на первый сегмент строки запроса - 
если он начинается со строки "/Admin", то устанавливается обработчик маршрута, который отправляет в качестве ответа сообщение "Привет admin!".</p>
<p>Обработчик маршрута здесь также представляет делегат RequestDelegate, которому в качестве параметра из вызывающей среды передается объект RequestContext.</p>
<p>Метод <code>GetVirtualPath()</code> в данном случае нам не понабодится, поэтому он остается нереализованным.</p>
<p>Также стоит отметить, что обработка запроса в выше определенном методе RouteAsync существенно отличается от, скажем, следующей реализации метода, которая 
вроде бы делает то же самое:</p>
<pre class="brush:c#;">
public async Task RouteAsync(RouteContext context)
{
    string url = context.HttpContext.Request.Path.Value.TrimEnd('/');
...
<p>Во втором случае запрос после обработки будет передаваться дальше по списку маршрутов и компонентов middleware, которые имеются в конвейере. А в первом случае запрос будет 
обрабатываться только тем делегатом, который установлен для свойства <code>context.Handler</code>.</p>
<p>Теперь применим этот маршрут в классе Startup:</p>
<pre class="brush:c#;">
...
<p>С помощью вызова <code>routeBuilder.Routes.Add(new AdminRoute())</code> маршрут добавляется на первое место в списке маршрутов. И в случае, если в запросе 
первый сегмент будет начинаться с "Admin", то он и будет соответствовать этому маршруту.</p>
<img src="./pics/routing7.png" alt="Создание маршрута в ASP.NET Core" />
...
<div class="nav"><p><a href="./11.4.php">Назад</a><a href="./">Содержание</a><a href="./3.1.php">Вперед</a></p></div></div>
<div class="socBlock">
