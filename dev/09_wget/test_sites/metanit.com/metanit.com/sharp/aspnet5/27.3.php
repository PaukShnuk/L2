...
<html >
<head>
<title>ASP.NET Core | Редактирование, удаление и работа с изображениями в MongoDB</title>
<meta charset="utf-8" />
<meta name="description" content="Редактирование и удаление данных в MongoDB в ASP.NET Core, работа с изображениями в Mongo, вывод картинок из базы данных в представление">
...
<h2>Редактирование, удаление и работа с изображениями</h2><div class="date">Последнее обновление: 29.12.2019</div>
...
<p>В прошлой теме был создан функционал для вывода и добавления объектов Product. Теперь определим остальные действия для управления объектами - редактирование, 
удаление, прикрепление изображений. Для этого добавим в контроллер HomeController следующие методы:</p>
<pre class="brush:c#;">
...
<p>Действие <code>Edit()</code> изменяет документ в базе данных через вызов метода Update класса ProductService.</p>
<p>Действие <code>Delete()</code> удаляет документ по id, вызывая метод Remove кдасса ProductService.</p>
<p>Действие <code>AttachImage()</code> получает отправленный пользователем файл и передает файловый поток в метод StoreImage класса ProductService.</p>
<p>И действие <code>GetImage()</code> просто возвращает изображение по id.</p>
<p>Теперь добавим представления для двух из этих действий. Представление <span class="b">Edit.cshtml</span> с формой для редактирования объекта Product:</p>
<pre class="brush:c#;">
@model MongoApp.Models.Product
@{
    ViewData["Title"] = "Редактировать модель";
}
...
<p>И представление <span class="b">AttachImage.cshtml</span> для прикрепления нового изображения и просмотра текущего изображения при его наличии:</p>
<pre class="brush:c#;">
@model MongoApp.Models.Product
...
@if (Model.HasImage())
{
    &lt;img src="@Url.Action("GetImage", new { id=@Model.ImageId})" style="width:150px;height:200px;" /&gt;
}
</pre>
<p>В случае, если изображение уже прикреплено к модели, то оно выводится на страницу с помощью хелпера <code>Url.Action("GetImage", new { id=@Model.ImageId})"</code>. 
Иначе с помощью формы выще мы можем прикрепить его:</p>
<img src="./pics/mongo6.png" alt="GridFS в MongoDB и ASP.NET Core MVC" />
...
<div class="nav"><p><a href="./27.2.php">Назад</a><a href="./">Содержание</a><a href="./25.1.php">Вперед</a></p></div></div>
<div class="socBlock">
