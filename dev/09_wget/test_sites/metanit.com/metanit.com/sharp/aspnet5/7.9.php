...
<html >
<head>
<title>ASP.NET Core | Файл _ViewImports.cshtml</title>
<meta charset="utf-8" />
<meta name="description" content="Файл _ViewImports.cshtml в ASP.NET Core MVC, подключение в представления пространств имен">
...
<h2>Файл _ViewImports.cshtml</h2><div class="date">Последнее обновление: 13.03.2017</div>
...
<p>По умолчанию проект ASP.NET Core по типу Web Application содержит в папке <span class="ii">Views</span> такой файл как <span class="b">_ViewImports.cshtml</span>.</p>
<img src="./pics/views14.png" alt="_ViewImports.cshtml в ASP.NET Core" />
<p>Он имеет по умолчанию следующее содержимое:</p>
<pre class="brush:c#;">
@using MvcViewsApp
@addTagHelper *, Microsoft.AspNetCore.Mvc.TagHelpers
</pre>
<p>Здесь используются только директивы синтаксиса Razor. Первая строка добавляет функциональность пространства имен MvcViewsApp (то есть текущего проекта). 
Вторая строка добавляет функциональность встроенных tag-хелперов из пространства имен <code>Microsoft.AspNetCore.Mvc.TagHelpers</code>. Про tag-хелперы 
<a href="10.1.php">далее</a> рассказывается более подробно. Сейчас же нас интересует прежде всего первая строка.</p>
<p>Если мы хотим использовать в представлении какие-то типы, то мы должны указывать их с полным названием пространства имен.</p>
<p>Например, добавим в проект новую папку Models и определим в ней новый класс Phone:</p>
<pre class="brush:c#;">
...
<img src="./pics/views15.png" alt="Подключение типов в представление в ASP.NET Core MVC" />
<p>Допустим, мы хотим использовать тип Phone в представлении Index.cshtml:</p>
<pre class="brush:c#;">
...
<p>Чтобы использовать тип Phone в представлении, мы вынуждены импортировать с помощью директивы <code>using</code> пространство имен, где этот тип определен. 
...
увеличит работу, если будут какие-то изменения.</p>
<p>Файл <span class="ii">_ViewImports.cshtml</span> решает эту проблему. В частности, добавим в него подключение пространства имен <code>RazorApp.Models</code>:</p>
<pre class="brush:c#;">
@using MvcViewsApp
@using MvcViewsApp.Models
...
<p>В этом случае функциональность <code>MvcViewsApp.Models</code> будет автоматически подключаться во все представления.</p>
<p>Для каждой группы представлений в одной папке мы можем определить свой файл <span class="ii">_ViewImports.cshtml</span>. Так пускай, в проекте будет добавлен 
новый контроллер OtherController, в папке Views для его представлений будет добавлена подпапка <span class="ii">Other</span>:</p>
<img src="./pics/views16.png" alt="ViewImport in ASP.NET Core MVC" />
<p>В эту папку Views/Other добавим новый файл _ViewImports.cshtml. Для его добавления можно выбрать спецальный шаблон:</p>
<img src="./pics/views17.png" alt="ViewImport Template in ASP.NET Core" />
<p>И теперь все директивы и выражения из файла <span class="ii">Views/Other/_ViewImports.cshtml</span> будут применяться к представлениям только из папки Views/Other. 
Кроме того, ко всем представлениям во всех папках продолжит применяться глобальный файл <span class="ii">Views/_ViewImports.cshtml</span></p>
