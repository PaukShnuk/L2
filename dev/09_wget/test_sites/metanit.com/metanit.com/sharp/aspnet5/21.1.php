...
<html >
<head>
<title>ASP.NET Core | Отправка email</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка email в ASP.NET Core с помощью MailKit">
...
<h1>Дополнительные статьи</h1><h2>Отправка email в ASP.NET Core</h2><div class="date">Последнее обновление: 15.03.2017</div>
...
<p>На данный момент .NET Core по умолчанию не предоставляет готового функционала для работы с протоколом SMTP, поэтому нам надо использовать сторонние решения, 
...
случае мы уйдем от кроссплатформенности в строну одноплатформенного решения.</p>
<p>В данном случае мы будем использовать MailKit как наиболее простое и в то же время довольно насыщенное в плане функциональности решение. 
Для его установки добавим в проект через NuGet соответствующий пакет:</p>
<img src="./pics/email8.png" alt="MailKit в ASP.NET Core" />
<p>После сохранения файла project.json и добавления всех необходимых пакетов добавим в проект новый класс. Назовем его EmailService и определим в нем следующий код:</p>
<pre class="brush:c#;">
using MimeKit;
...
<p>В классе определен асинхронный метод <code>SendEmailAsync()</code>, который в качестве параметров принимает email получателя сообщения, тему письма и его текст.</p>
<p>Для создания объекта отправляемого сообщения используется класс <span class="b">MimeMessage</span>. Для определения отправителя в его коллекцию From 
добавляется объект MailboxAddress. Теоретически может быть указано несколько отправителей.</p>
<p>Все получатели письма добавляются в коллекцию To также в виде объекта MailboxAddress.</p>
<p>Тело сообщения, которое представлено свойством Body, может представлять как простой текст, так и какое-то другое содержимое. Например, в данном случае указано, 
что сообщение будет иметь формат html.</p>
<p>Для простого текста мы могли бы указать:</p>
<pre class="brush:c#;">
...
<p>Либо использовать значение <code>MimeKit.Text.TextFormat.Plain</code>.</p>
<p>Непосредственно отправление производится с помощью класса <code>SmtpClient</code>. Причем весь процесс разбивается на ряд этапов:</p>
<ol>
<li><p>Подключение к серверу</p></li>
<li><p>Аутентификация (при ее необходимости)</p></li>
<li><p>Собственно отправка сообщения</p></li>
<li><p>Отключение</p></li>
</ol>
<p>При подключении в метод <code>ConnectAsync()</code> передаются smtp-адрес сервера, номер порта, и логическое значение, указывающее, будет ли применяться SSL. В данном случае 
SSL не применяется, поэтому стоит значение <code>false</code>.</p>
<p>Для аутентификации в метод <code>AuthenticateAsync()</code> передаются наши логин и пароль на сервере.</p>
<p>Хотя здесь все методы асинхронные, но это необязательно. Мы можем использовать и синхронный API:</p>
<pre class="brush:c#;">
using (var client = new SmtpClient())
{
...
<p>После определения класса мы можем его использовать, например, в контроллере:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./32.1.php">Назад</a><a href="./">Содержание</a><a href="./21.3.php">Вперед</a></p></div></div>
<div class="socBlock">
