...
<html >
<head>
<title>ASP.NET Core | Состояние приложения. Куки. Сессии</title>
<meta charset="utf-8" />
<meta name="description" content="Хранение состояния приложения в ASP.NET Core, коллекция HttpContext.Items, использование сессий и куки">
...
<h1>Состояние приложения. Куки. Сессии</h1><div class="date">Последнее обновление: 03.11.2019</div>
...
<p>Часть данных, используемых в приложении, можно отнести к его состоянию. Это могут быть как какие-то глобальные данные, так и данные, которые непосредственно 
относятся к запросу и пользователю. И в зависимости от вида данных, существуют различные способы для их хранения.</p>
<h2>HttpContext.Items</h2>
<p>В объекте <span class="b">HttpContext</span> определена коллекция <span class="b">Items</span>, которая представляет собой словарь типа 
...
После завершения запроса все данные из HttpContext.Items удаляются. Каждый объект в этой коллекции имет ключ и значение. И с помощью ключей можно управлять объектами коллекции.</p>
...
чтобы для этих компонентов были доступны общие данные, то как раз можем применить эту коллекцию. Например, пусть метод Configure в классе Startup будет определен 
следующим образом:</p>
<pre class="brush:c#;">
...
<p>Здесь в одном middleware опредляется ключ "text" со значением "Text from HttpContext.Items":</p>
<pre class="brush:c#;">context.Items["text"] = "Text from HttpContext.Items";</pre>
<p>В другом middleware этот объект используется для установки отправляемого ответа.</p>
<p>Поскольку HttpContext.Items предоставляет ряд методов для управления элементами:</p>
<ul>
...
<div class="nav"><p><a href="./6.3.php">Назад</a><a href="./">Содержание</a><a href="./2.25.php">Вперед</a></p></div></div>
<div class="socBlock">
