...
<html >
<head>
<title>ASP.NET Core MVC | Переопределение контроллеров</title>
<meta charset="utf-8" />
<meta name="description" content="Переопределение функционала контроллеров в ASP.NET Core MVC, базовый класс Controller и его методы, OnActionExecuting и OnActionExecuted">
...
<h2>Переопределение контроллеров</h2><div class="date">Последнее обновление: 18.11.2019</div>
...
<p>Как правило, для создания контроллера достаточно унаследовать свой класс от базового класса Controller. Однако если нам необходимо, чтобы 
...
Либо мы также можем переопределить некоторые методы базового класса Controller, если они нас не устраивают.</p>
<p>Что мы можем в контроллере переопределить? В базовом классе Controller среди всех прочих методов есть три интересных метода:</p>
<pre class="brush:c#;">
[NonAction]
...
<p>Метод <span class="b">OnActionExecuting()</span> выполняется при вызове метода контроллера до его непосредственного выполнения.</p>
<p>Метод <span class="b">OnActionExecuted()</span> выполняется после выполнения метода контроллера.</p>
<p>Метод <span class="b">OnActionExecutionAsync()</span> представляет асинхронную версию метода OnActionExecuting().</p>
<p>Возьмем простейшую задачу. Например, мы решили, что наше приложение больше не будет поддерживать Internet Explorer, и поэтому хотим всем пользователям 
с этим браузером выводить соответствующие сообщения. А для всех остальных пользователей обработка запроса пусть протекает в стандартном режиме.</p>
<p>Для решения этой задачи добавим в папку Controllers следующий класс контроллера:</p>
<pre class="brush:c#;">
using System.Linq;
...
<p>Так как нам надо переопределить поток выполнения программы до начала выполнения действия, то переопределяется метод <code>OnActionExecuting()</code>. 
В качестве параметра он принимает контекст выполнения метода - объект <code>ActionExecutingContext</code>, из которого мы можем получить самую различную информацию, в том числе и 
обратиться к контексту запроса.</p>
<p>Первым делом проверяем, установлен ли заголовок "User-Agent":</p>
<pre class="brush:c#;">context.HttpContext.Request.Headers.ContainsKey("User-Agent")</pre>
<p>Если установлен, то содержит ли этот User-Agent подстроки "MSIE" или "Trident":</p>
<pre class="brush:c#;">if(useragent.Contains("MSIE") || useragent.Contains("Trident"))</pre>
<p>И далее переопределяется результат метода в виде переадресации:</p>
<pre class="brush:c#;">context.Result = Content("Internet Explorer не поддерживается");</pre>
<p>Также мы бы могли переопределить какие-либо другие действия или определить новые общие для всех действия.</p>
<p>И теперь мы можем унаследовать контроллеры от нашего базового класса:</p>
<pre class="brush:c#;">
...
<p>И теперь до начала работы метода Index будет срабатывать метод OnActionExecuting(), определенный в базовом классе HelloBaseController.</p>
<img src="./pics/controllers9.png" alt="Overriding controllers in ASP.NET Core MVC" />
