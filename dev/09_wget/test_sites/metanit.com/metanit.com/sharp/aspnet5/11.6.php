...
<html >
<head>
<title>ASP.NET Core | Атрибуты маршрутизации</title>
<meta charset="utf-8" />
<meta name="description" content="Атрибуты маршрутизации или Attribute-Based Routing в ASP.NET Core, особенности использования атрибутов">
...
<h2>Атрибуты маршрутизации</h2><div class="date">Последнее обновление: 25.11.2019</div>
...
<p>Фреймворк MVC позволяет использовать в приложении маршрутизацию на основе атрибутов. Такой тип маршрутизации еще называется 
<span class="b">Attribute-Based Routing</span>. Атрибуты предоставляют более гибкий способ определения маршрутов. 
Маршруты, определенные с помощью атрибутов, имеют приоритет по сравнению с маршрутами, определенными в классе Startup.</p>
<p>Маршрутизация на основе атрибутов может применяться независимо от того, какой компонент отвечает в приложении за маршрутизацию - RouterMiddleware 
или EndpointRoitingMiddleware+EndpointMiddleware.</p>
<p>Для определения маршрута, необходимо использовать атрибут <span class="b">[Route]</span>:</p>
<pre class="brush:c#;">
...
<img src="./pics/routing14.png" alt="Attribute-Based Routing in ASP.NET Core" />
<p>Если в проекте планируется использовать только маршрутизацию на основе атрибутов, то в классе Startup мы можем не определять никаких маршрутов. 
Например, при использовании EndpointMiddleware и конечных точек можно применить метод <code>MapControllers</code>:</p>
<pre class="brush:c#;">
...
<p>Аналогично можно не определять маршруты и при использовании RouterMiddleware:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
{
	if (env.IsDevelopment())
...
<p>В качестве параметра атрибут Route принимает шаблон URL, с которым будет сопоставляться запрошенный адрес. И даже если у нас определен стандартный маршрут в классе Startup:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
{
	if (env.IsDevelopment())
...
<img src="./pics/routing5.png" alt="Атрибуты маршрутизации в ASP.NET MVC Core" />
<p>Определение маршрутов с помощью атрибутов подчиняется тем же правилам, что определение маршрутов в классе Startup. Например, используем параметры 
и ограничения:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
...
<p>И к такому методу мы сможем обратиться с запросом типа <span class="ii">http://localhost:xxxx/10/lumia</span>.</p>
<img src="./pics/routing15.png" alt="Атрибуты контроллеров и маршрутизация в ASP.NET MVC Core" />
...
<p>От всех параметров шаблона маршрутов в атрибутах отличаются два параметра controller и action, которые ссылаются соответственно на контроллер и его действие. 
При использовании их надо помещать в квадратные скобки, а не в фигурные, как другие параметры:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
...
<p>Допустимо также использование их по отдельности или добавление к ним других параметров в фигурных скобках:</p>
<pre class="brush:c#;">
[Route("[controller]")]
[Route("[action]")]
...
<div class="nav"><p><a href="./11.13.php">Назад</a><a href="./">Содержание</a><a href="./11.9.php">Вперед</a></p></div></div>
<div class="socBlock">
