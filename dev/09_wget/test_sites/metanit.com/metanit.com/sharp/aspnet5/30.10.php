...
<html >
<head>
<title>ASP.NET Core | Пользователи в SignalR</title>
<meta charset="utf-8" />
<meta name="description" content="Управление пользователями в хабах и клиентах в SignalR в ASP.NET Core, получение данных о пользователях">
...
<h2>Пользователи</h2><div class="date">Последнее обновление: 28.12.2019</div>
...
<p>Пользователь в SignalR представляет по умолчанию объект <span class="b">ClaimsPrincipal</span>. По сути это аутентифицированный пользователь, 
...
пользователь. Используя идентификатор <code>ClaimTypes.NameIdentifier</code> из ClaimsPrincipal, можно отправлять сообщение определенным пользователям.</p>
<p>Концепция пользователя и концепция клиента(подключения) в SignalR отличаются. Пользователь - это учетная запись, под которой осуществлен вход в приложение. 
...
пользователь увидит отправленное сообщение во всех браузерах и устройствах, в которых он зашел в приложение.</p>
<p>Подключение, в отличие от пользователя, создается даже на каждую отдельную вкладку браузера. Например, если в одном браузере открыть две вкладки и на обеих вкладках 
подключиться к хабу, то на каждой вкладке будут разные подключения с разными идентификаторами.</p>
<p>Если пользователь авторизован, то внутри хаба мы можем получить этого пользователя через свойство <span class="b">Context.User</span>:</p>
<pre class="brush:c#;">
...
<p>Это свойство представляет объект ClaimsPrincipal, поэтому мы можем также, как и в представлениях или контроллерах, получить из него формальное 
имя пользователя (<code>user.Identity.Name</code>), получить роль или значения других клеймов, если они сохранены в аутентификационных куках или токене, узнать, принадлежит ли 
пользователь определенной роли, а также получить другую сопутствующую информацию.</p>
<p>Но важно учитывать, что некоторые пользователи, которые обращаются к хабу, могут быть аутентифицированы, некоторые могут быть анонимными. 
...
Либо применять атрибут авторизации, который исключает обращения анонимных пользователей.</p>
<p>В хабе мы можем не только получить текущего пользователя, но и отправить сообщения определенным пользователям. Для этого можно использовать ряд методов:</p>
<ul>
<li><p><span class="b">Clients.User(string userId)</span>: вызывает метод у пользователя по id</p></li>
<li><p><span class="b">Clients.Users(IReadOnlyList&lt;string&gt; userIds)</span>: вызывает метод у пользователей, id которых передаются в метод</p></li>
</ul>
<p>В оба метода передается некий id пользователя. Что это за id? В реальности мы сами можем определить, что будет использоваться в качестве id. Для этого добавим в проект новый класс 
<span class="b">CustomUserIdProvider</span>:</p>
<pre class="brush:c#;">
...
<p>Данный класс реализует интерфейс <span class="b">IUserIdProvider</span>, который определяет метод <span class="b">GetUserId()</span>. Этот метод как раз и возвращает id пользователя. 
В данном случае он возвращает значение <code>User.Identity.Name</code>. Здесь <code>connection.User</code> - это текущий пользователь. Здесь 
мы можем возвратить любое другое значение, например, роль пользователя, все зависит от конкретного приложения.</p>
<p>Далее этот метод необходимо добавить в сервисы в методе ConfigureServices класса Startup:</p>
<pre class="brush:c#;">
...
<p>Определим следующий класс хаба:</p>
<pre class="brush:c#;">
...
<p>При подключение пользователя у всех подключененных клиентов будет вызываться функция Notify, которой передается сообщение общего характера. 
...
или так <code>Context.User.Identity.Name</code> или так <code>Context.UserIdentifier</code>. Далее сообщение отправляется 
получателю, которому предназначено сообщение, и текущему пользователю, которой собственно и отправляет сообщение.</p>
<p>Для тестирования определим следующую веб-страницу:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>На веб-странице вначале осуществляем вход в приложение, как было описано в прошлой статье, затем отправляем сообщение определенному пользователю.</p>
<p>Таким образом, пользователи смогут отправлять сообщения только определенным пользователям:</p>
<img src="./pics/signalr18.png" alt="Управление пользователями в SignalR в ASP.NET Core" />
...
<div class="nav"><p><a href="./30.9.php">Назад</a><a href="./">Содержание</a><a href="./30.11.php">Вперед</a></p></div></div>
<div class="socBlock">
