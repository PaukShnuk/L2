...
<html >
<head>
<title>ASP.NET Core | Применение правил для Apache</title>
<meta charset="utf-8" />
<meta name="description" content="Применение правил IIS для URL Rewriting в ASP.NET Core, метод AddApacheModRewrite">
...
<h2>Применение правил для Apache</h2><div class="date">Последнее обновление: 22.12.2019</div>
...
<p>Для написания правил для URL Rewriting мы также можем использовать стандартные правила для веб-сервера Apache, которые обычно помещаются в файл htaccess. 
...
правилом лучше, конечно же, тестировать.</p>
<p>Итак, добавим в корень проекта ASP.NET Core новый текстовый файл, который назовем <span class="b">rewrite.txt</span>. Определим в этом файле следующее содержимое:</p>
<pre class="brush:js;">
...
<p>Для переопределения адресов применяется слово <span class="b">RewriteRule</span>, которое в качестве параметра принимает выражение, которому должен соответствовать адрес, и ссылку, 
на которую будет осуществлен переход.</p>
<p>Первое выражение RewriteRule получает запрошенный путь, сравнивает его с регулярным выражением, и если сравнение прошло успешно, то преобразует адрес. 
Последующим RewriteRule передается результат предыдущих преобразований.</p>
<p>Регулярные выражения могут иметь группы, ограниченные круглыми скобками. В преобразованном адресе на каждую группу мы можем ссылаться через выражения типа $1, где число 1 - это порядковый номер группы. 
Например, в регулярном выражении "(.*)/$" $1 передает ту часть пути, которая идет до конечного слеша.</p>
<p>В выражении "Home/Products/([0-9]+)/([0-9a-z-]+)" уже две группы, для обращения к которым можно использовать токены $1 и $2 соответственно.</p>
<p>Если надо выполнить переадресацию, то в конце правила используется выражение <code>[R=301]</code> или <code>[R]</code>. В случае с третьим правилом никакой переадресации нет, будет просто подмена 
одного адреса на другой.</p>
<p>Кроме того, поскольку при определении правил Apache по умолчанию учитывается регистр (то есть Home и home - это не одно и то же), то чтобы регистр не учитывался, 
также используется флаг <code>[NC]</code>.</p>
<p>Более подробно про правила URL Rewriting, которые применяются в Apache, можно посмотреть в <a href="http://httpd.apache.org/docs/current/mod/mod_rewrite.html" rel="nofollow">официальной документации</a>.</p>
<p>Для использования этого файла применяется метод <span class="b">AddApacheModRewrite()</span>:</p>
<pre class="brush:c#;">
...
<p>В метод <code>AddApacheModRewrite</code> передается провайдер файлов, который позволит сформировать путь к файлу правил (в данном случае это  env.ContentRootFileProvider), 
и также название самого файла.</p>
<p>Также начиная с версии ASP.NET Core 2.0, можно использовать другой способ получения конфигурации Apache из файла:</p>
<pre class="brush:c#;">
using System.IO;
