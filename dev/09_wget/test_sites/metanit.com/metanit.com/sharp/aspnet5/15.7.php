...
<html >
<head>
<title>ASP.NET Core | Пример авторизации на основе Claims</title>
<meta charset="utf-8" />
<meta name="description" content="Пример авторизации на основе Claims в ASP.NET Core MVC">
...
<h2>Пример авторизации на основе Claims</h2><div class="date">Последнее обновление: 13.12.2019</div>
...
<p>Создадим новый проект ASP.NET Core по типу WebApplication (Model-View-Controller), который назовем ClaimsApp.</p>
<p>Затем добавим в проект новую папку для моделей, которую назовем Models. И определим в этой папке класс пользователя:</p>
<pre class="brush:c#;">
public class User
{
...
И также добавим в папку Models новый класс <span class="b">ApplicationContext</span>, который будет представлять контекст данных:</p>
<pre class="brush:c#;">
...
<p>Здесь устанавливаются две политики - "OnlyForLondon" и "OnlyForMicrosoft". Первая политика требует, чтобы claim с типом <code>ClaimTypes.Locality</code> 
...
<p>Для тестирования авторизации добавим в папку Controllers новый контроллер <span class="b">AccountController</span>, 
который будет выполнять регистрацию пользователей.</p>
<p>Для регистрации определим в папке Models дополнительную модель <span class="b">RegisterModel</span>:</p>
<pre class="brush:c#;">
...
<p>А в контроллере AccountController определим действие регистрации:</p>
<pre class="brush:c#;">
...
<img src="./pics/claims3.png" alt="Авторизация claims в ASP.NET Core" />
<p>Для тестирования доступа изменим контроллер HomeController:</p>
<pre class="brush:c#;">
...
<p>Здесь метод <code>Index</code> доступен только для тех пользователей, которые удовлетворяют политике "OnlyForLondon", а метод About - для пользователей, 
соответствующих политике "OnlyForMicrosoft".</p>
<p>И пусть в представлении для метода Index контроллера Home выводятся все объекты Claim для текущего пользователя:</p>
<pre class="brush:xml;">
@using System.Security.Claims
...
<p>Запустим проект и зарегистрируем нового пользователя:</p>
<img src="./pics/claims1.png" alt="Создание нового пользователя в ASP.NET Core" />
<p>И после регистрации перейдем к методу Index контроллера HomeController:</p>
<img src="./pics/claims2.png" alt="Claims based authorization in ASP.NET Core" />
...
<div class="nav"><p><a href="./15.6.php">Назад</a><a href="./">Содержание</a><a href="./15.8.php">Вперед</a></p></div></div>
<div class="socBlock">
