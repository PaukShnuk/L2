...
<html >
<head>
<title>ASP.NET Core | Конфигурация CORS</title>
<meta charset="utf-8" />
<meta name="description" content="Конфигурация CORS в ASP.NET Core, объект CorsPolicyBuilder и его методы, ограничения на запросы от разных адресов">
...
<h2>Конфигурация CORS</h2><div class="date">Последнее обновление: 22.12.2019</div>
...
<p>Для работы с CORS в прошлой теме применялся следующий класс Startup:</p>
<pre class="brush:c#;">
...
<p>В вызове app.UseCors с помощью методов объекта <span class="b">CorsPolicyBuilder</span> можно настроить конфигурацию CORS:</p>
<ul>
<li><p><span class="b">AllowAnyOrigin()</span>: принимаются запросы с любого адреса</p></li>
<li><p><span class="b">AllowAnyHeader()</span>: принимаются запросы с любыми заголовками</p></li>
<li><p><span class="b">AllowAnyMethod()</span>: принимаются запросы любого типа (GET/POST)</p></li>
<li><p><span class="b">AllowCredentials()</span>: разрешается принимать идентификационные данные от клиента (например, куки)</p></li>
<li><p><span class="b">WithHeaders()</span>: принимаются только те запросы, которые используют содержат определенные заголовки</p></li>
<li><p><span class="b">WithMethods()</span>: принимаются запросы только определенного типа</p></li>
<li><p><span class="b">WithOrigins()</span>: принимаются запросы только с определенных адресов</p></li>
<li><p><span class="b">WithExposedHeaders()</span>: позволяет серверу отправлять на сторону клиента свои заголовки</p></li>
</ul>
<h3>Определение адреса</h3>
<p>Метод <span class="b">AllowAnyOrigin()</span> позволяет установить взаимодействие с любым приложением по любому адресу. Однако подобное поведение может 
быть нежелательным. В этом случае мы можем ограничить круг адресов с помощью метода <span class="b">WithOrigins()</span>:</p>
<pre class="brush:c#;">
app.UseCors(builder =&gt; builder.WithOrigins("http://example.com", "http://google.com"));
</pre>
<p>При чем, что важно, в конце названия домена не должно быть конечного слеша.</p>
<h3>Определение метода запроса</h3>
<p>Метод <span class="b">AllowAnyMethod()</span> позволяет принимать запросы любого типа (GET/POST). Также можно настроить принятие только определенного типа запросов:</p>
<pre class="brush:c#;">
app.UseCors(builder =&gt; builder.WithOrigins("https://localhost:44321").WithMethods("GET"));
</pre>
<h3>Определение заголовков</h3>
<p>Для разрешения запросов с любыми заголовками применяется метод <span class="b">AllowAnyHeader()</span>. Следует отметить, что вместе с этим методом лучше также указывать и 
метод <span class="b">AllowAnyMethod()</span> или <span class="b">WithMethods()</span> для указания типа запроса:</p>
<pre class="brush:c#;">
app.UseCors(builder =&gt; builder.WithOrigins("https://localhost:44321")
							.AllowAnyHeader()
                            .AllowAnyMethod());
</pre>
<p>Если необходимо принимать запросы только с определенными 
заголовоками, то все требуемые заголовки надо передать в метод <span class="b">WithHeaders()</span>:</p>
<pre class="brush:c#;">
app.UseCors(builder =&gt; builder.WithOrigins("https://localhost:44321")
							.AllowAnyMethod()
							.WithHeaders("accept", "content-type", "origin", "custom-header"));
</pre>
<p>Если в метод передаются заголовки, отличные от "*", то следует указать по крайней мере три заголовка "accept", "content-type", "origin", а также какие-то свои заголовки, 
например, как в данном случае "custom-header".</p>
<h3>Отправка заголовков</h3>
<p>Если сервер отправляет какие-то свои заголовки, то по умолчанию клиент их не получает. Чтобы на стороне сервера указать, какие заголовки может получать клиент, 
следует использовать метод <span class="b">WithExposedHeaders()</span>:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app)
{
	app.UseCors(builder =&gt; builder.WithOrigins("https://localhost:44321")
...
<p>Сервер устанавливает заголовок custom-header и отправляет его клиенту. Чтобы клиент получил этот заголовок, он передается в метод WithExposedHeaders.</p>
<p>Затем на стороне клиента можно получить значение этого заголовка:</p>
<pre class="brush:js;">
...
<h3>Передача идентификационных данных</h3>
<p>По умолчанию браузер не посылает никаких идентификационных данных. Подобные данные включают куки, а также данные HTTP-аутентификации. 
...
<span class="b">withCredentials</span> равным true.</p>
<pre class="brush:js;">
var request = new XMLHttpRequest();
request.open("GET", "https://localhost:44313/");
request.withCredentials = true;
</pre>
<p>Для получения данных на стороне сервера применяется метод <span class="b">AllowCredentials()</span>. Этот метод устанавливает заголовок <code>Access-Control-Allow-Credentials</code>, 
...
есть обязательно нужно указать набор адресов, с которыми будет взаимодействовать сервер. Например:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app)
{
	app.UseCors(builder =&gt; builder.WithOrigins("https://localhost:44321")
...
<p>На стороне клиента отправка куки могла бы выглядеть следующим образом:</p>
<pre class="brush:js;">
var btn = document.getElementById("btn");
...
<div class="nav"><p><a href="./31.1.php">Назад</a><a href="./">Содержание</a><a href="./31.3.php">Вперед</a></p></div></div>
<div class="socBlock">
...
$(function() { 
  ...
  $("#toggle-theme").on("click", function() {
	...
	$("li:contains('Глава 30.')").removeClass('expandable').addClass('collapsable').children('ul').css({"display":"block"});
	$("li:contains('Глава 30.')").children('div').removeClass('expandable-hitarea').addClass('collapsable-hitarea');
