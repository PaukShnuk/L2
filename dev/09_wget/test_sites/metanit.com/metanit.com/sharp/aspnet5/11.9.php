...
<html >
<head>
<title>ASP.NET Core | Области</title>
<meta charset="utf-8" />
<meta name="description" content="Области в ASP.NET Core, организация приложения, особенности работы с областями">
...
<h2>Области</h2><div class="date">Последнее обновление: 26.11.2019</div>
...
<p>ASP.NET Core MVC позволяет организовать приложение в виде областей, где каждая область представляет какой-то свой сегмент приложения. Например, одна область 
...
собственную структуру каталогов. Это позволяет разрабатывать и тестировать код для каждой области отдельно независимо от других областей.</p>
<p>Возьмем простой проект ASP.NET Core с MVC и добавим в него области. Как правило, области помещаются в отдельную папку. Поэтому добавим в проект новую папку, 
которую назовем <span class="ii">Areas</span>. Затем в эту папку добавим каталог под названием <span class="ii">Store</span>. То есть у нас будет область под названием Store.</p>
<p>И далее в папке <span class="ii">Areas/Store</span> создадим подкаталоги Controllers и Views. В итоге проект будет выглядеть так:</p>
<img src="./pics/areas1.png" alt="Области в ASP.NET Core" />
<p>Затем в папку Controllers добавим новый контроллер HomeController:</p>
<pre class="brush:c#;">
...
<p>Для ассоциации контроллера с определенной областью к этому контроллеру применяется атрибут <span class="b">Area</span>, в который передается название области. 
Без этого атрибута контроллер не будет принадлежать области Store.</p>
<p>Далее в папку <span class="ii">Areas/Store/Views</span> добавим подкаталог <span class="ii">Home</span> - для представлений нового контроллера. И затем 
в этот подкаталог поместим представление <span class="ii">Index.cshtml</span> с одной строкой:</p>
<pre class="brush:xml;">
&lt;h2&gt;HomeController из области Store&lt;/h2&gt;
</pre>
<p>Вообще поиск представления для контроллера из области ведется по следующим путям:</p>
<ul>
<li><p>/Areas/Store/Views/Home/</p></li>
<li><p>/Areas/Store/Views/Shared/</p></li>
<li><p>/Views/Shared/</p></li>
</ul>
<p>Соответственно все три выше указанные папки мы могли бы использовать для хранения представления Index.cshtml для контроллера из области Store.</p>
<p>Таким образом, проект теперь выглядит так:</p>
<img src="./pics/areas2.png" alt="Добавление областей в ASP.NET Core MVC" />
<p>Но пока эта область совершенно неактивна. Мы никак не можем к ней обратиться. И теперь добавим в приложение поддержку этой области. Для этого изменим класс Startup:</p>
<pre class="brush:c#;">
...
<p>Для сопоставления с областью в первом маршруте определен сегмент <code>area</code>. Ограничение <code>exists</code> 
используется для того, чтобы маршрут сопоставлялся только с теми областями, которые определены в приложении.</p>
<p>И теперь мы сможем обращаться к области с запросом <span class="ii">http://localhost:xxxx/Store</span>:</p>
<img src="./pics/areas3.png" alt="Areas in ASP.NET Core" />
<p>В данном случае определяется маршрут для всех областей. С помощью метода <span class="b">MapAreaControllerRoute()</span> 
можно указать маршрут для конкретной области:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app)
{
	app.UseDeveloperExceptionPage();
...
<p>Метод <code>MapAreaControllerRoute()</code> принимает три обязательных параметра: имя маршрута, имя области (areaName) и шаблон маршрута. 
Причем шаблон маршрута начинается с названия области: "store/{controller=Home}/{action=Index}/{id?}"</p>
<p>Подобным образом можно определить маршруты для нескольких областей:</p>
<pre class="brush:c#;">
app.UseEndpoints(endpoints =&gt;
{
	// маршрут для области store
...
<p>Несмотря на то, что в примере выше шаблон маршрута начинается с названия области и при обращении к методу контроллеру мы вводим в начале название области, 
в принципе это необязательно. Например, определим следующий маршрут:</p>
<pre class="brush:c#;">
app.UseEndpoints(endpoints =&gt;
{
	// маршрут для области store
...
<p>Теперь шаблон маршрута для области Store начинается с токена "products", однако параметр areaName по прежнему позволяет связать данный маршрут с 
областью Store. Только теперь в адресной строке нам придется вводить вначале "Products", а не "Store":</p>
<img src="./pics/areas4.png" alt="Настройка маршрутизации области и MapAreaControllerRoute" />

<h3>Определение областей через RouterMiddleware и UseMvc</h3>
<p>Подобным образом мы можем использовать области при маршрутизациии через RouterMiddleware и метод UseMvc:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Builder;
using Microsoft.Extensions.DependencyInjection;
...
<p>Определение маршрутов для конкретных областей:</p>
<pre class="brush:c#;">
app.UseMvc(routes =&gt;
{
	routes.MapAreaRoute(
...
<h3>Настройка маршрутизации через атрибуты</h3>
<p>Также мы могли бы задать маршрут через атрибут маршрутизации, в котором параметр <code>area</code> представлял бы область:</p>
<pre class="brush:c#;">
[Area("Store")]
public class HomeController : Controller
{
...
<p>Для обращения к такому методу Index мы можем использовать три адреса: <span class="ii">http://localhost:xxxx/Store</span>, 
<span class="ii">http://localhost:xxxx/Store/Home</span> и <span class="ii">http://localhost:xxxx/Store/Home/Index</span></p>
<p>В этом случае в классе Startup можно не определять маршрут для области:</p>
<pre class="brush:c#;">
app.UseEndpoints(endpoints =&gt;
{
	endpoints.MapControllerRoute(
...
<p>Также можно использовать префиксы:</p>
<pre class="brush:c#;">
[Area("Store")]
public class HomeController : Controller
{
...
<p>Для обращения к такому методу Index мы можем использовать следующие адреса: <span class="ii">http://localhost:xxxx/Products</span>, 
<span class="ii">http://localhost:xxxx/Products/Home</span> и <span class="ii">http://localhost:xxxx/Products/Home/Index</span></p>
<h3>Работа с областями</h3>
<p>При работе с областями следует учитывать ряд моментов.</p>
<p>Если в контроллеры области используют представления, то система будет искать эти представления по следующим путям:</p>
<pre>
...
<p>Поэтому все представления, которые находятся вне описанных выше путей, будут недоступны для области. К примеру, в главном проекте функциональность 
tag-хелперов добавляется с помощью файла <span class="ii">_ViewImports.cshtml</span>. В области он будет недоступен. И чтобы задействовать tag-хелперы в 
...
представления области эту строку, где надо использовать tag-хелперы.</p>
<p>Другой важный момент - это генерация ссылок. При генерации ссылок в представлениях отдельных областей используются все те хелперы, например, Html.ActionLink или AncorTagHelper, однако 
тут есть и некоторые особенности.</p>
<p>Чтобы сгенерировать ссылку на какое-либо действие контроллера, которые находятся внутри области, то мы указываем действие и контроллер (если действие находится в другом контроллере):</p>
<pre class="brush:xml;">
@Html.ActionLink("Онлайн-магазин", "Index", "Home")
// или
&lt;a asp-action="Index" asp-controller="Home"&gt;Онлайн-магазин&lt;/a&gt;
</pre>
<p>В итоге будет сгенерирована ссылка: <code>&lt;a href="Store/Home/Index/"&gt;Онлайн-магазин&lt;/a&gt;</code> (если текущее представление расположено в области Store).</p>
...
<p>Сгенерированная ссылка будет выглядеть так: <code>&lt;a href="Store/Book/List/"&gt;Все книги&lt;/a&gt;</code></p>
<p>Если же метод и контроллер, на которые мы хотим сделать ссылку, находятся в основной части приложения, то для параметра area определяем пустую строку:</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./11.6.php">Назад</a><a href="./">Содержание</a><a href="./8.1.php">Вперед</a></p></div></div>
<div class="socBlock">
