...
<html >
<head>
<title>ASP.NET Core | Фильтры действий</title>
<meta charset="utf-8" />
<meta name="description" content="Фильтры действий (Action Filters) в ASP.NET Core, ообенности определения, реализация интерфейсов IActionFilter и IAsyncActionFilter">
...
<h2>Фильтры действий</h2><div class="date">Последнее обновление: 11.12.2019</div>
...
<p>Фильтры действий (Action Filters) реализуют либо интерфейс <span class="b">IActionFilter</span>, либо интерфейс <span class="b">IAsyncActionFilter</span>. 
Фильтры действий выполняются после фильтров авторизации и ресурсов и уже после того, как произошла привязка модели. 
Поэтому данные фильтры являются прекрасным местом для исследования результатов привязки модели, а также модификации входных данных в метод контроллера или его результата.</p>
<p>К примеру, добавим в проект следующий фильтр:</p>
<pre class="brush:c#;">
...
<p>В данном случае у нас определен фильтр пробелов, который удаляет пробелы из выходного кода html. Основная работа перенесена во вспомогательный класс 
SpaceCleaner. А ключевыми здесь являются методы <code>OnActionExecuting()</code> и <code>OnActionExecuted()</code>.</p>
<p>Метод <code>OnActionExecuting()</code> вызывается до выполнения метода контроллера. В качестве параметра он принимает контекст выполнения - объект 
<span class="b">ActionExecutingContext</span>, который имеет ряд свойств. Посредством изменения значения 
<code>ActionExecutingContext.ActionArguments</code> можно манипулировать параметрами метода. 
Либо через значение <code>ActionExecutingContext.Controller</code> можно управлять контроллером.</p>
<p>Кроме того, метод <code>OnActionExecuting()</code> может завершить обработку запроса путем установки свойства <code>ActionExecutingContext.Result</code>.</p>
<p>Метод <code>OnActionExecuted()</code> вызывается после выполнения метода и получает в качестве параметра объект <span class="b">ActionExecutedContext</span>. На этом этапе мы можем увидеть результат выполнения и как-то 
его изменить через свойство <code>ActionExecutedContext.Result</code>.</p>
<p>В случае с нашим фильтром метод <code>OnActionExecuting</code> никак не задействуется, поскольку удалять пробелы из выходного кода html мы можем после выполнения метода. 
А это значит, что всю логику нам надо определить в методе <code>OnActionExecuted</code>.</p>
...
<p>При реализации интерфейса <span class="b">IAsyncActionFilter</span> необходимо определить его метод <code>OnActionExecutionAsync()</code>, который объединяет возможности 
методов <code>OnActionExecuting()</code> и <code>OnActionExecuted()</code>. А вызов <code>await next()</code> для передаваемого в качестве параметра 
объекта <span class="b">ActionExecutionDelegate</span> позволит выполнить последующие фильтры действий.</p>
<p>К примеру, добавим новый фильтр, который назовем CheckFilterAttribute:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc.Filters;
using System;
using System.Threading.Tasks;
...
<p>Данный метод применяет интерфейс IAsyncActionFilter, поэтому реализует только один метод <code>OnActionExecutionAsync()</code>. 
В качестве параметра опять же передается контекст выполнения ActionExecutingContext и делегат ActionExecutionDelegate, через который мы можем вызвать другие фильтры действий.</p>
<p>Суть фильтра довольно проста - для параметра id в качестве значения устанавливается число 34.</p>
<p>Применение фильтра будет аналогично:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
    [CheckFilter]
...
<p>При обращении к методу вне зависимости от того, какое значение будет передано для id, фильтр действий все равно его поменяет на число 34.</p>
<img src="./pics/filters4.png" alt="Action Filters in ASP.NET Core MVC" />
