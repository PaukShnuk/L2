...
<html >
<head>
<title>ASP.NET Core | Привязка свойств RazorPage к параметрам запроса</title>
<meta charset="utf-8" />
<meta name="description" content="Передача параметров с Razor Pages в ASP.NET Core, атрибут BindProperty, привязка свойств и параметров строки запроса и форм">
...
<h2>Привязка свойств RazorPage к параметрам запроса</h2><div class="date">Последнее обновление: 05.12.2019</div>
...
<p>Для рассмотрения механизма передачи параметров возьмем проект ASP.NET Core по типу Web Application, который использует Razor Pages и добавим в его папку Pages новую 
страницу Razor, которую назовем Person.cshtml:</p>
<img src="./pics/razorpages9.png" alt="Передача параметров в Razor Page" />
<p>После этого в проект в папку Pages будет добавлена страница <span class="b">Person.cshtml</span> и связанный с ней файл кода 
<span class="b">Person.cshtml.cs</span>. Допустим, данная страница предназначена для ввода данных пользователя, и нам их надо получить при отправке.</p>
<p>Самый простой способ получения данных - использование параметров в метода OnGet/OnPost/OnPut/OnDelete. Поэтому изменим файл <span class="ii">Person.cshtml.cs</span> следующим образом:</p>
<pre class="brush:c#;">
...
<p>На самой странице Person.cshtml определим форму отправки данных:</p>
<pre class="brush:xml;">
@page
@model PersonModel
@{
...
<p>При этом элементы формы называются также, как и параметры - name и age.</p>
<p>Обратимся к этой странице:</p>
<img src="./pics/razorpages10.png" alt="Передача параметров в OnPost Razor Page" />
<p>Метод OnPost получит переданные значения через параметры и переустановит свойство Message:</p>
<img src="./pics/razorpages11.png" alt="Параметры в Razor Page" />
<p>Хотя такой способ работает, но мы можем также использовать и другой способ, который представляет применение атрибута <span class="b">BindProperty</span>. 
Для его использования изменим код Person.cshtml.cs:</p>
<pre class="brush:c#;">
...
<p>Теперь можно убрать параметры из метода OnPost. Все значения будут передаваться по имени свойствам Name и Age. Поэтому результат работы 
приложения не изменится.</p>
<p>Атрибут <span class="b">BindProperty</span> имеет ряд свойств, через которые мы можем управлять им:</p>
<ul>
<li><p><span class="b">Type</span>: тип привязчика модели</p></li>
<li><p><span class="b">Name</span>: имя данных в запросе, если оно отличается</p></li>
<li><p><span class="b">SupportGet</span>: поддерживает ли запросы get</p></li>
</ul>
<p>Например:</p>
<pre class="brush:c#;">
...
<p>Причем по умолчанию свойство <span class="b">SupportGet</span> имеет значение false. Поэтому если мы хотим передавать параметры через строку запроса типа:</p>
<pre class="brush:c#;">http://localhost:5000/Person?name=Tom&age=23</pre>
<p>То у свойств для этого атрибута надо установить параметр <code>SupportsGet=true</code>:</p>
<pre class="brush:c#;">
public class PersonModel : PageModel
{
	public string Message { get; set; }
...
<p>Однако поскольку оба свойства представляют фактически одну сущность, то естественно лучше работать с ними, как со свойствами одного объекта. 
Поэтому создадим в проекте новую папку <span class="b">Models</span>, а в нее добавим новый класс, который назовем <span class="b">Person</span>.</p>
<pre class="brush:c#;">
public class Person
{
	public string Name { get; set; }
...
<p>Теперь изменим файл Person.cshtml.cs:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Mvc.RazorPages;
...
<p>Теперь все данные будут передаваться свойствам объекта Person. А приложение будет работать также, как и раньше.</p>
...
<div class="nav"><p><a href="./29.3.php">Назад</a><a href="./">Содержание</a><a href="./29.5.php">Вперед</a></p></div></div>
<div class="socBlock">
