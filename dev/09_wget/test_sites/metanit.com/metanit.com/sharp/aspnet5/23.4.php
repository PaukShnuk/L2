...
<html >
<head>
<title>ASP.NET Core | Создание клиента для WEB API</title>
<meta charset="utf-8" />
<meta name="description" content="Создание клиента для WEB API в ASP.NET Core MVC, AJAX запросы из javascript к серверу">
...
<h2>Создание клиента для WEB API</h2><div class="date">Последнее обновление: 30.12.2019</div>
...
<p>Продолжим работу с проектом из прошлой теме. В ней был создан и протестирован контроллер UsersController. Теперь создадим для него визуальную часть, которая будет представлять 
веб-страницу. То есть из веб-страницы мы будем отправлять запросы к контроллеру и обрабатывать ответ от контроллера.</p>
<p>Для создания веб-клиента добавим в проект папку <span class="b">wwwroot</span> и затем в ней определим новый элемент <span class="b">HTML Page</span>, который назовем "index.html":</p>
<img src="./pics/webapi12.png" alt="Добавление веб-страницы в ASP NET Core Web API" />
<p>Затем изменим класс Startup:</p>
<pre class="brush:c#;">
...
<p>Здесь в метод <code>Configure()</code> были добавлены два вызова для работы со статическими файлами:</p>
<pre class="brush:c#;">
app.UseDefaultFiles();
app.UseStaticFiles();
</pre>
<p>Благодаря этому мы сможем обратиться напрямую к веб-странице, например, по пути <span class="b">http://localhost:xxxx/index.html</span>. 
Для этого изменим файл <span class="b">index.html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>И выделенный пользователь добавляется в форму над таблицей. Эта же форма применяется и для добавления объекта. С помощью скрытого поля, которое хранит id пользователя, 
мы можем узнать, какое действие выполняется - добавление или редактирование. Если id равен 0, то выполняется функция CreateUser, которая отправляет данные в POST-запросе:</p>
<pre class="brush:js;">
async function CreateUser(userName, userAge) {
...
<p>При нажатии на ссылку "Удалить" выполняется DELETE-запрос, который по id удаляет пользователя.</p>
<img src="./pics/webapi16.png" alt="Представление для ASP.NET Core Web API" />
