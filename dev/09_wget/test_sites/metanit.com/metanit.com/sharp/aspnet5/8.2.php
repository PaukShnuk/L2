...
<html >
<head>
<title>ASP.NET Core | Модели представления View Model</title>
<meta charset="utf-8" />
<meta name="description" content="Модели представления в ASP.NET MVC Core, View Model и передача сложных моделей в представление">
...
<h2>Модель представления</h2><div class="date">Последнее обновление: 29.11.2019</div>
...
<p>Приложения на ASP.NET MVC бывают разными: маленькими, большими, простыми, либо со сложной логикой. И в зависимости от сложности проекта мы можем 
использовать одну и ту же модель для хранения данных в базе данных, для передачи данных в представление и получения данных из представления. 
...
объекты сразу двух моделей. И в этом случае мы можем воспользоваться моделями представления.</p>
<p>Рассмотрим простейший пример работы с моделями. Допустим, нам надо выводить на страницу список смартфонов и фильтровать их по компаниям. Наподобие 
следующего:</p>
<img src="./pics/models1.png" alt="Модели в ASP.NET MVC Core" />
<p>Возьмем проект из прошлой темы, где создавались модели Phone и Company.</p>
<pre class="brush:c#;">
...
<p>Но если вернуться к скриншоту веб-страницы, которую нам надо сделать, то там есть не просто список смартфонов, но и список компаний, 
причем в списке компаний имеется дополнительный элемент, позволяющий выбрать смартфоны всех компаний.</p>
<p>Очевидно, что этих двух моделей - Phone и Company для решения поставленной задачи нам недостаточно. И нам надо создадить 
...
проект новую папку <span class="b">ViewModels</span>. В принципе модели представлений не обязательно определять именно в папке ViewModels, 
это может быть любая папка, в том числе и имеющаяся по умолчанию папка Models. Далее в каталог ViewModels поместим модель <span class="b">CompanyModel</span>:</p>
<pre class="brush:c#;">
...
<p>Эта модель упрощает передачу списка компаний в представление.</p>
<p>И также добавим в ViewModels собственно модель представления, которую назовем <span class="b">IndexViewModel</span>:</p>
<pre class="brush:c#;">
...
<p>С помощью этой модели мы сможем передать в представление сразу и список компаний, и список смартфонов.</p>
<p>В итоге проект будет иметь следующую структуру:</p>
<img src="./pics/models2.png" alt="Добавление моделей представлений в ASP.NET Core" />
<p>Теперь изменим код в HomeController следующим образом:</p>
<pre class="brush:c#;">
using System.Collections.Generic;
using System.Linq;
...
<p>В метод Index передается опциональный параметр <code>companyId</code>, который передает идентификатор выбранной компании. Если он не равен 0 и определен, то производим фильтрацию по компаниям.</p>
<p>И в конце изменим представление <span class="ii">Index.cshtml</span>, которое будет выводить все объекты:</p>
<pre class="brush:c#;">
...
<p>И теперь у нас получится веб-страница, как на первом скриншоте, на которой используется фильтрация.</p>
<p>В данном случае продемонстрирован наглядный пример, где с помощью только одной простой моделей типа Phone и Company было бы сложно передать данные. 
И поэтому было бы более оптимально прибегнуть к комплексной модели представления (IndexViewModel).</p>
