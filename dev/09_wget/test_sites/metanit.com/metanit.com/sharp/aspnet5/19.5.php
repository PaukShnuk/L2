...
<html >
<head>
<title>ASP.NET Core | Создание атрибута валидации. Самовалидация модели</title>
<meta charset="utf-8" />
<meta name="description" content="Создание атрибута валидации в ASP.NET Core с помощью класса ValidationAttribute, самовалидация модели и интерфейс IValidatableObject">
...
<h2>Создание атрибута валидации. Самовалидация модели</h2><div class="date">Последнее обновление: 08.12.2019</div>
...
<p>Встроенные атрибуты валидации охватывают большую часть возможных ситуаций, которые могут возникнуть, но все же их бывает недостаточно, особенно в каких-то 
специфических сценариях. Однако mvc фреймворк позволяет создавать собственные атрибуты валидации и закладывать в них свою логику.</p>
<p>Для этого необходимо унаследовать свой атрибут от класса <span class="b">ValidationAttribute</span> и переопределить его метод <code>IsValid()</code>.</p>
...
<p>В данном случае параметр value уже будет представлять всю модель Person в целом. И теперь мы можем применить этот атрибут ко всей модели:</p>
<pre class="brush:c#;">
[NamePasswordEqual]
public class Person
{
...
<p>В данном случае нам надо реализовать метод Validate и возвратить коллекцию объектов ValidationResult, которые и будут содержать все ошибки валидации. 
...
<img src="./pics/validation11.png" alt="ValidationAttribute в ASP.NET Core" />
...
<div class="nav"><p><a href="./10.5.php">Назад</a><a href="./">Содержание</a><a href="./19.6.php">Вперед</a></p></div></div>
<div class="socBlock">
