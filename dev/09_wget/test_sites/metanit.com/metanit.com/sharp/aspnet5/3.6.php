...
<html >
<head>
<title>ASP.NET Core | Добавление MVC в пустой проект</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление фреймворка MVC в пустой проект ASP.NET Core">
...
<h2>Добавление MVC в пустой проект</h2><div class="date">Последнее обновление: 16.11.2019</div>
...
<p>В прошлой теме был создан первый проект ASP.NET Core MVC, но осталось за кадром, что именно заставляет паттерн MVC работать в проекте на ASP.NET Core? В чем ключевое 
отличие проекта по типу Empty, который функциональность MVC не содержит, от проекта по типу Web Application? Рассмотрим этот момент.</p>
<p>И в начале создадим новый проект ASP.NET Core по типу Empty - то есть тот, который mvc по умолчанию не использует:</p>
<img src="./pics/mvc18.png" />
<p>Теперь задействуем функциональность фреймворка MVC в нашем приложении. MVC 
подключается в приложение в качестве сервиса. Поэтому откроем класс Startup и изменим его следующим образом:</p>
<pre class="brush:c#;">
...
<p>Во-первых, в методе <code>ConfigureServices()</code> с помощью вызова <code>services.AddMvc()</code> добавляются всее сервисы MVC. 
После этого мы можем использовать фреймворк.</p>
<p>Во-вторых, в методе <code>Configure()</code> через вызовы <code>app.UseRouting()</code> и <code>app.UseEndpoints()</code> встраивается система маршрутизации, 
...
тем (<a href="https://metanit.com/sharp/aspnet5/11.1.php">Основы маршрутизации в ASP.NET Core</a>). В данном же случае стоит отметить, 
что мы используем метод <code>MapControllerRoute()</code> интерфейса IEndpointRouteBuilder для связи контроллеров с маршрутом "{controller=Home}/{action=Index}/{id?}", 
...
<p>Контроллер должен быть унаследован от класса Controller и в данном случае он имеет один метод Index, который с помощью метода View обращается к представлению.</p>
...
контроллера Home. И в конце добавим в каталог <span class="b">Views/Home</span> новый элемент по типу <span class="b">Razor View</span>, который назовем <span class="b">Index.cshtml</span>.</p>
<img src="./pics/mvc25.png" alt="Mvc Razor View" />
...
<img src="./pics/mvc21.png" />
<p>Теперь запустим приложение на выполнение, и браузер выведет сообщение, определенное в представлении:</p>
<img src="./pics/mvc22.png" />
<p>Таким образом мы можем добавлять функциональность MVC в свой проект. Однако, как правило, для работы с MVC используется шаблон Web Application (Model-View-Controller), который по умолчанию содержит некоторую 
...
При создании проекта по типу <span class="b">Web Application (Model-View-Controller)</span> используется именно этот метод</p></li>
...
вместо вызова <code>services.AddMvc()</code> метод <code>services.AddControllersWithViews()</code>.</p>
