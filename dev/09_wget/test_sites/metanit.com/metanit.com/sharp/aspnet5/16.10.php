...
<html >
<head>
<title>ASP.NET Core | Валидация пользователя</title>
<meta charset="utf-8" />
<meta name="description" content="Валидация пользователя в ASP.NET Core Identity с помощью объекта IUserValidator, переопределение механизма валидации">
...
<h2>Валидация пользователя</h2><div class="date">Последнее обновление: 15.12.2019</div>
...
<p>Кроме проверки пароля в ASP.NET Core Identity также валидируется и пользователь. В частности, если мы введем при регистрации некорректные данные для поля Email, 
то получим ошибки валидации пользователя:</p>
<img src="./pics/password4.png" alt="Валидация пользователя в ASP.NET Core" />
<p>В данном случае ошибка говорит о том, что email некорректен и должен содержать только алфавитно-цифровые символы.</p>
<p>За валидацию пользователя по умолчанию отвечает класс <span class="b">UserOptions</span>, который определяет следующие свойства:</p>
<ul>
<li><p><code>AllowedUserNameCharacters</code>: если равно true, то юзернейм должен содержать только алфавитно-цифровые символы</p></li>
<li><p><code>RequireUniqueEmail</code>: если равно true, то email пользователя должен быть уникальным</p></li>
</ul>
<p>Для применения этих свойств изменим установку контекста данных в методе <code>ConfigureServices()</code> в классе Startup:</p>
<pre class="brush:c#;">
...
<p>Свойство <code>opts.User</code> как раз и представляет класс <code>UserOptions</code>.</p>
<p>Этот способ работает, но все же он имеет недостатки:</p>
<img src="./pics/password3.png" alt="UserOptions в ASP.NET Core" />
<p>Несмотря на то, что разрешены только алфавитные символы, в сообщении все равно говорится, что email должен содержать алфавитные или цифровые символы. И во-вторых, 
сообщения отображаются на английском языке. И в этом случае мы можем полностью переопределить логику валидации, создав свой класс валидатора.</p>
<p>Класс валидатора должен реализовать интерфейс <code>IUserValidator&lt;T&gt;</code>:</p>
<pre class="brush:c#;">
public interface IUserValidator&lt;TUser&gt; where TUser : class {
	Task&lt;IdentityResult&gt; ValidateAsync(UserManager&lt;TUser&gt; manager, TUser user);
}
</pre>
<p>Метод <code>ValidateAsync()</code> вызывается при валидации адреса электронной почты. В качестве параметров в метод передаются объект UserManager и 
валидируемый пользователь.</p>
<p>Теперь создадим класс-валидатор CustomUserValidator:</p>
<pre class="brush:c#;">
...
<p>Здесь проверяются пара условий: наличие слова "admin" и домен электронного адреса. В зависимости от результатов проверки добавляются соответствующие ошибки в 
коллекцию errors. И если ошибок не будет найдено, что значит валидация прошла успешно, и возвращается значение <code>IdentityResult.Success</code>.</p>
<p>Теперь применим наш валидатор. Для этого добавим установку валидатора в метод <code>ConfigureServices()</code> класса Startup:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
...
<p>И в этом случае, если мы введем некорректные значения, то увидим сообщения из нашего валидатора:</p>
<img src="./pics/password5.png" alt="Создание валидатора пользователя в ASP.NET Core" />
<p>Также для создания валидатора мы можем унаследовать свой класс от класса <code>UserValidator</code>:</p>
<pre class="brush:c#;">
...
<p>Этот класс также реализует интерфейс IUserValidator, поэтому для его переопределения мы можем изменить метод <code>ValidateAsync()</code>. Результат будет тем же, что и в предыдущем случае.</p>
...
<div class="nav"><p><a href="./16.9.php">Назад</a><a href="./">Содержание</a><a href="./16.13.php">Вперед</a></p></div></div>
<div class="socBlock">
