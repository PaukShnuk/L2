...
<html >
<head>
<title>ASP.NET Core | Логгирование</title>
<meta charset="utf-8" />
<meta name="description" content="Логгирование в ASP.NET Core, использование сервиса ILogger, уровни и категории логгирования, LogInformation, вывод информации в лог">
...
<h1>Логгирование</h1><h2>Ведение лога и ILogger</h2><div class="date">Последнее обновление: 06.11.2019</div>
...
уже предоставляет нам такой объект. Например, возьмем стандартный проект по типу Empty и добавим механизм логгирования. Для этого перейдем к классу Startup и изменим его метод <code>Configure()</code>:</p>
<pre class="brush:c#;">
...
<p>Средой выполнения в метод Configure передается объект ILogger, который представляет логгер. А метод логгера <code>logger.LogInformation</code> передает на консоль некоторую информацию. 
По умолчанию информация логгируется на консоль, поэтому для тестирования логгера нам надо запустить приложение как консольное:</p>
<img src="./pics/logger2.png" />
<p>При обращении к приложению с помощью следующего запроса <span class="ii">http://localhost:xxxxx/index</span> на консоль будет выведена информация, переданная логгером:</p>
<img src="./pics/logger3.png" alt="Тестирование логгера в ASP.NET Core" />
<p>Важно, что, если мы используем класс Startup для логгирования, то получить объект логгера мы можем только в методе <code>Configure()</code>, 
но никак не в методе <code>ConfigureServices()</code> или конструкторе класса Startup, поскольку инфраструктура логгирования зависит от конфигурации и контейнера DI, который окончательно устанавливаются 
лишь после завершения работы метода ConfigureServices.</p>
<h3>Категория логгера</h3>
<p>В примере выше в метод передается не просто объект ILogger, а объект ILogger&lt;T&gt; или точнее ILogger&lt;Startup&gt;. В данном случае "Startup" 
...
можем увидеть, в каком классе именно было создано сообщение лога. Поэтому, как правило, в качестве категории указывается текущий класс, но в принципе это необязательно. Например, мы можем написать так:</p>
<pre class="brush:c#;">
public class Startup
{
...
<img src="./pics/logger7.png" alt="Категория логгирования в ASP.NET Core" />
<p>В данном случае в качестве категории выступает класс Program, примем, обратите внимание, полное имя этого класса. Тем не менее в качестве категории, как правило, задается 
имя текущего класса.</p>
...
<li><p><code>string format</code>: строковое сообщения для лога, которое моет содержать параметры</p></li>
...
<pre class="brush:c#;">logger.Log(LogLevel.Information, "Requested Path: {0}", context.Request.Path);</pre>
...
<img src="./pics/logger5.png" alt="Уровни логгирования в ASP.NET Core" />
...
<div class="nav"><p><a href="./2.26.php">Назад</a><a href="./">Содержание</a><a href="./2.29.php">Вперед</a></p></div></div>
<div class="socBlock">
