...
<html >
<head>
<title>ASP.NET Core | Создание ограничений маршрутов</title>
<meta charset="utf-8" />
<meta name="description" content="Создание ограничений маршрутов в ASP.NET Core, реализация интерфейса IRouteConstraint">
...
<h2>Создание ограничений маршрутов</h2><div class="date">Последнее обновление: 16.11.2019</div>
...
<p>Чтобы создать собственное ограничение, нужно реализовать интерфейс <span class="b">IRouteConstraint</span> с одним единственным методом Match, который имеет 
...
IRouteConstraint.Match, чтобы определить, применяется ли данное ограничение к данному запросу или нет. Например, создадим ограничение, которое не 
будет пропускать запросу по некоторому url. Итак, добавим в приложение следующий класс:</p>
<pre class="brush:c#;">
...
<p>Здесь мы говорим, что если запрашиваемый ресурс совпадает со значением свойства <code>httpContext.Request.Path</code>, то запрос не будет 
сопоставляться с маршрутом. Тогда определение маршрута может выглядеть следующим образом:</p>
<pre class="brush:c#;">
...
<p>В случае выше было определено общее ограничение, которое применялось в целом к запросу. Но также можно задавать и ограничения для 
отдельных параметров. Допустим, создадим ограничение по должности:</p>
<pre class="brush:c#;">
...
<p>С помощью массива positions устанавливается допустимый набор значений, которые могут использовать в запросе.</p>
<p>В методе <code>Match()</code> с помощью параметра <code>routeKey</code> мы получаем параметр маршрута, для которого действует ограничение. А словарь 
<code>values</code> содержит набор значений маршрута. Используя выражение <code>values[routeKey]</code> мы можем получить значение для параметра и что-то сделать с ним. 
В данном случае мы смотрим, а содержится ли это значение в массиве positions.</p>
<p>Применим это ограничение:</p>
<pre class="brush:c#;">
...
<p>Теперь для соответствия маршруту параметр id должен иметь одно из следующих значений: "admin", "director" или "accountant". То есть строка запроса 
могла бы выглядеть так: <span class="ii">http://localhost:59406/Home/Index/admin</span></p>
<h3>Создание inline-ограничений</h3>
<p>Для создания inline-ограничений нам надо зарегистрировать их в методе ConfigureServices:</p>
<pre class="brush:c#;">
...
<p>Основную работу по добавлению inline-ограничения выполняют следующие строки:</p>
<pre class="brush:c#;">
services.Configure&lt;RouteOptions&gt;(options =&gt; 
    options.ConstraintMap.Add("position", typeof(PositionConstraint)));
</pre>
<p>Они устанавливают сопоставление ограничения "position" с классом PositionConstraint.</p>
...
<div class="nav"><p><a href="./11.3.php">Назад</a><a href="./">Содержание</a><a href="./11.8.php">Вперед</a></p></div></div>
<div class="socBlock">
