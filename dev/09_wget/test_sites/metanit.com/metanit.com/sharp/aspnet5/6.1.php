...
<meta charset="utf-8" />
<meta name="description" content="Dependency Injection и механизм внедрения зависимостей в ASP.NET Core">
...
<h1>Dependency Injection</h1><h2>Сервисы и метод ConfigureServices</h2><div class="date">Последнее обновление: 07.11.2019</div>
...
<p>Dependency injection (DI) или внедрение зависимостей представляет механизм, который позволяет сделать взаимодействующие в приложении 
...
более адаптируемой и расширяемой.</p>
<p>Нередко для установки зависимостей в подобных системах используются специальные контейнеры - IoC-контейнеры (Inversion of Control). 
...
созданием этих объектов.</p>
<p>И если раньше в ASP.NET 4 и других предыдщих версиях надо было использовать различные внешние IoC-контейнеры для установки зависимостей, 
такие как Ninject, Autofac, Unity, Windsor Castle, StructureMap, то ASP.NET Core уже имеет встроенный контейнер внедрения зависимостей, который представлен 
интерфейсом <span class="b">IServiceProvider</span>. А сами зависимости еще называются сервисами, собственно поэтому контейнер можно назвать 
<span class="b">провайдером сервисов</span>. Этот контейнер отвечает за сопоставление зависимостей с конкретными типами и за внедрение зависимостей в 
различные объекты.</p>
<h3>Установка встроенных сервисов фреймворка</h3>
<p>Для рассмотрения работы механизма Dependency Injection создадим новый проект ASP.NET Core 3.0 по типу Empty:</p>
<img src="./pics/di.png" alt="Новый проект ASP.NET Core для Dependency Injection" />
<p>За установку сервисов в приложении отвечает метод <span class="b">ConfigureServices</span>, определенный в классе Startup. В проекте Empty этот метод 
фактически пустой:</p>
<pre class="brush:c#;">
...
<p>Через параметр IServiceCollection сервисы и добавляются в проект. Несмотря на то, что метод пустой, IServiceCollection уже содержит ряд сервисов по умолчанию</p>
<img src="./pics/di1.png" alt="Сервисы в ASP.NET Core" />
<p>Как видно на скриншоте, в коллекции IServiceCollection 78 сервиса, который мы можем использовать в приложении. Это такие сервисы, как <span class="b">ILogger&lt;T&gt;</span>, 
<span class="b">ILoggerFactory</span>, <span class="b">IWebHostEnvironment</span> и ряд других. Они добавляются по умолчанию инфраструктурой ASP.NET Core. И мы их можем использовать 
в любой части приложения. Например, в том же пустом проекте следующее определение метода Configure:</p>
<pre class="brush:c#;">
...
<p>Через параметры метода Configure можно получить добавленные в приложении сервисы и, в частности, через параметр передается сервис IWebHostEnvironment, 
который используется для определения окружения.</p>
<h3>Регистрация встроенных сервисов ASP.NET Core</h3>
<p>Кроме ряда подключаемых по умолчанию сервисов ASP.NET Core имеет еще ряд встроенных сервисов, которые мы можем подключать в приложение при необходимости. 
...
имеющих общую форму <code>Add[название_сервиса]</code>.
<p>Например:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
...
<p>Для объекта IServiceCollection определено ряд методов расширений, которые начинаются на <span class="ii">Add</span>, как, например, <code>AddMvc()</code>. Эти методы добавляют 
в объект IServiceCollection соответствующие сервисы. Например, <code>AddMvc()</code> добавляет в приложение сервисы MVC, благодаря чему мы сможем их использовать 
в методе <code>Configure()</code>.</p>
<h3>Информация о сервисах</h3>
<p>Каждый сервис в коллекции IServiceCollection представляет объект <span class="b">ServiceDescriptor</span>, который несет некоторую информацию. 
В частности, наиболее важные свойства этого объекта:</p>
<ul>
<li><p><span class="b">ServiceType</span>: тип сервиса</p></li>
<li><p><span class="b">ImplementationType</span>: тип реализации сервиса</p></li>
<li><p><span class="b">Lifetime</span>: жизненный цикл сервиса</p></li>
</ul>
<p>Например, получим все сервисы, которые добавлены в приложение:</p>
<pre class="brush:c#;">
...
<img src="./pics/di3.png" alt="Получение всех сервисов их IServiceCollection ASP.NET Core" />
...
<div class="nav"><p><a href="./18.6.php">Назад</a><a href="./">Содержание</a><a href="./2.19.php">Вперед</a></p></div></div>
<div class="socBlock">
