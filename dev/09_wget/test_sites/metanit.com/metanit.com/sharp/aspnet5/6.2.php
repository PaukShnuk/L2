...
<html >
<head>
<title>ASP.NET Core | Жизненный цикл зависимостей</title>
<meta charset="utf-8" />
<meta name="description" content="Жизненный цикл объектов в ASP.NET Core и внедрение зависимостей, применение методов AddTransient, AddScoped и AddSingleton">
...
<h2>Жизненный цикл зависимостей</h2><div class="date">Последнее обновление: 07.11.2019</div>
...
<p>Используя различные методы внедрения зависимостей, можно управлять жизненным циклом создаваемых сервисов. Сервисы, которые создаются механизмом 
Depedency Injection, могут представлять один из следующих типов:</p>
<ul>
<li><p><span class="b">Transient</span>: при каждом обращении к сервису создается новый объект сервиса. В течение одного запроса 
...
наиболее подходит для легковесных сервисов, которые не хранят данных о состоянии</p></li>
<li><p><span class="b">Scoped</span>: для каждого запроса создается свой объект сервиса. То есть если в течение одного запроса есть несколько обращений к одному сервису, 
то при всех этих обращениях будет использоваться один и тот же объект сервиса.</p></li>
<li><p><span class="b">Singleton</span>: объект сервиса создается при первом обращении к нему, все последующие запросы используют один и тот же ранее созданный объект сервиса</p></li>
</ul>
<p>Для создания каждого типа сервиса предназначен соответствующий метод <span class="b">AddTransient()</span>, <span class="b">AddScoped()</span> и <span class="b">AddSingleton()</span>.</p>
<p>Для рассмотрения механизма внедрения зависимостей и жизненного цикла возьмем проект ASP.NET Core по типу Empty. Вначале определим простейший сервис. 
Для этого добавим в проект папку, которую назовем <span class="ii">Services</span>. И в нее затем добавим интерфейс ICounter:</p>
<pre class="brush:c#;">
...
<p>И добавим в эту же папку Services класс RandomCounter, который будет реализовать данный интерфейс:</p>
<pre class="brush:c#;">
using System;
...
<p>Суть данного класса будет состоять в генерации некоторого случайного числа в диапазоне от 0 до 1000000.</p>
<p>И также в папку Services добавим новый класс, который нам более детально поможет разобраться в механизме Depedency Injection. Этот класс назовем 
CounterService, и он будет содержать следующий код:</p>
<pre class="brush:c#;">
...
<p>Данный класс просто устанавливает объект ICounter, передаваемый через конструктор.</p>
<p>Для работы с сервисами определим компонент middleware, который назовем <span class="b">CounterMiddleware</span>:</p>
<pre class="brush:c#;">
...
<p>Для получения зависимостей здесь используется метод InvokeAsync, в котором передаются две зависимости ICounter и CounterService. 
В самом методе выводятся значения Value из обоих зависимостей. Причем сервис CounterService сам использует зависимость ICounter.</p>
<p>То есть структура проекта будет выглядеть следующим образом:</p>
<img src="./pics/dependencyinjection.png" alt="Depedency Injection in ASP.NET Core" />
<p>Для тестирования сервисов изменим код класса Startup:</p>
<pre class="brush:c#;">
...
<p>В методе Configure CounterMiddleware встраивается в конвейер обработки запроса.</p>
<p>Для того, чтобы передать нужные зависимости, необходимо вызвать методе <code>ConfigureServices()</code> один из методов, который определяет жизненный цикл зависимостей.</p>
<h3>AddTransient</h3>
<p>Метод <code>AddTransient()</code> создает transient-объекты. Такие объекты создаются при каждом обращении к ним. Данный метод имеет ряд перегруженных версий:</p>
<ul>
<li><p>AddTransient(Type serviceType)</p></li>
<li><p>AddTransient(Type serviceType, Type implementationType)</p></li>
<li><p>AddTransient(Type serviceType, Func&lt;IServiceProvider,object&gt; implementationFactory)</p></li>
<li><p>AddTransient&lt;TService&gt;()</p></li>
<li><p>AddTransient&lt;TService, TImplementation&gt;()</p></li>
<li><p>AddTransient&lt;TService&gt;(Func&lt;IServiceProvider,TService&gt; implementationFactory)</p></li>
<li><p>AddTransient&lt;TService, TImplementation&gt;(Func&lt;IServiceProvider,TImplementation&gt; implementationFactory)</p></li>
</ul>
<p>Для использования в методе <code>ConfigureServices()</code> пусть будут определены следующие вызовы:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
    services.AddTransient&lt;ICounter, RandomCounter&gt;();
...
<p>Запустим проект:</p>
<img src="./pics/addtransient.png" alt="AddTransient в ASP.NET Core DI" />
<p>В нашем случае CounterMiddleware получает объект ICounter, для которого создается один экземпляр класса RandomCounter. 
...
Поэтому генерируемые случайные числа обоими экземплярами не совпадают. Таким образом, применение AddTransient создаст два разных объекта RandomCounter.</p>
<p>При втором и последующих запросах к контроллеру будут создаваться новые объекты RandomCounter.</p>
<h3>AddScoped</h3>
<p>Метод AddScoped создает один экземпляр объекта для всего запроса. Он имеет те же перегруженные версии, что и AddTransient. Для его 
применения изменим метод <code>ConfigureServices()</code> в классе Startup (весь остальной код остается прежним):</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
...
<img src="./pics/addscoped.png" alt="AddScoped в ASP.NET Core DI" />
<p>Теперь в рамках одного и того же запроса и CounterMiddleware и сервис CounterService будут использовать один и тот же объект RandomCounter. 
При следующем запросе к приложению будет генерироваться новый объект RandomCounter.</p>
<h3>AddSingleton</h3>
<p>AddSingleton создает один объект для всех последующих запросов, при этом объект создается только тогда, когда он непосредственно необходим. 
Этот метод имеет все те же перегруженые версии, что и AddTransient и AddScoped.</p>
<p>Для применения AddSingleton изменим метод <code>ConfigureServices()</code>:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
...
<p>Как можно заметить, все три метода внедрения зависимостей имеют один и тот же принцип использования, различается только название метода. 
И в этом случае при нескольких последовательных запросах мы получим следующий результат:</p>
<img src="./pics/addsingleton.png" alt="AddSingleton в ASP.NET Core DI" />
<p>Для создания singleton-объектов необязательно полагаться на механизм DI. Мы его можем сами создать и передать в нужный метод:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
...
<p>Работа приложения в данном случае будет аналогична предыдущему примеру.</p>
<h3>Использование фабрики сервисов</h3>
<p>Ряд перегруженных версий у всех трех методов в качестве параметра могут принимать фабрику сервисов - <code>Func&lt;IServiceProvider,object&gt; implementationFactory</code>, 
которая управляет созданием объектов сервисов. Фабрика позволяет нам применить более сложную логику по созданию сервиса, например, добавить условия создания:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
...
<p>Или к примеру у нас есть зависимость IMessageSender и есть две ее реализации - EmailMessageSender и SmsMessageSender:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./6.4.php">Назад</a><a href="./">Содержание</a><a href="./2.20.php">Вперед</a></p></div></div>
<div class="socBlock">
