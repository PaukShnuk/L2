...
<html >
<head>
<title>ASP.NET Core | Кэширование статических файлов</title>
<meta charset="utf-8" />
<meta name="description" content="Кэширование статических файлов в ASP.NET Core, StaticFilesMiddleware, установка заголовков кэширования Cache-Control, добавление версии к файлам">
...
<h2>Кэширование статических файлов</h2><div class="date">Последнее обновление: 20.12.2019</div>
...
<p>Кроме кэширования результатов действий контроллера для оптимизации работы приложения можно применять кэширование статического контекта. 
В данном случае статический контент - это файлы изображений, скрипты javascript, файлы стилей css, какие-то другие файлы, например, аудио-файлы, используемые на странице. Как правило, 
подобные файлы в проекте размещаются в папке <span class="b">wwwroot</span>.</p>
<p>Для кэширования статических файлов при их отправке следует установить соответствующие заголовки. Для этого StaticFilesMiddleware передается 
объект <span class="b">StaticFileOptions</span>, у которого устанавливается свойство <span class="b">OnPrepareResponse</span/>:</p>
<pre class="brush:c#;">
...
<p>Свойство OnPrepareResponse представлят делегат Action - некоторое действие, которое в качестве параметра принимает объект 
<span class="b">StaticFileResponseContext</span>. Через этот объект можно установить заголовки ответа, который будут посылаться вместе со статическими файлами. 
В частности, в данном случае устанавливается заголовок "Cache-Control", который задает параметры кэширования. Так, значение "public" 
указывает, что файл будет кэшироваться как в кэше браузера, так и на промежуточных прокси-серверах.</p>
<p>А параметр <code>max-age</code> задает время кэширования в секундах. Так, в данном случае файл кэшируется 10 минут (600 секунд).</p>
<p>То есть в данном случае метод Configure мог бы выглядеть следующим образом:</p>
<pre class="brush:c#;">
...
<p>При отправке запроса браузер получит вместе с файлами заголовки ответа, которые устанавливают параметры кэширования:</p>
<img src="./pics/caching7.png" alt="Кэширование статических файлов в ASP.NET Core" />
<p>При повторной отправке запроса браузер будет доставать файлы из кэша:</p>
<img src="./pics/caching8.png" alt="Кэширование статических файлов в ASP.NET Core MVC" />
<h3>Версии файлов</h3>
<p>У кэширования есть недостаток: если мы изменим, например, содержимое файла css или javascript, то при повторном обращении к приложению 
браузер продолжит извлекать нужные файлы из кэша (если срок кэширования не истек). И таким образом, мы будем использовать старые версии файлов.</p>
<p>Для решения этой проблемы можно добавлять к статическим файлам версию и при каждом изменении файла соответственно менять версию файла:</p>
<pre class="brush:xml;">
&lt;link rel="stylesheet" href="/css/site.css?v=123" /&gt;
&lt;script src="/js/site.js?v=123"&gt;&lt;/script&gt;
&lt;img src="/images/banner1.svg?v=123" /&gt;
</pre>
<p>Если мы используем представления в рамках проекта MVC или RazorPages, то для автоматической генерации версии можно воспользоваться встроенными tag-хелперами LinkTagHelper, 
ScriptTagHelper и ImageTagHelper, которые с помощью атрибута <span class="b">asp-append-version="true"</span> позволяют автоматически 
добавлять версию при изменении файла:</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./14.3.php">Назад</a><a href="./">Содержание</a><a href="./2.7.php">Вперед</a></p></div></div>
<div class="socBlock">
