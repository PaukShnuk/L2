...
<html >
<head>
<title>ASP.NET Core | Первое приложение на MVC</title>
<meta charset="utf-8" />
<meta name="description" content="Создание первого приложения на ASP.NET MVC Core, добавление базы данных MS SQL Server">
...
<h2>Первое приложение. Добавление моделей и базы данных</h2><div class="date">Последнее обновление: 16.11.2019</div>
...
<p>Создадим первое простенькое приложение на ASP.NET Core MVC, которое призвано дать понимание, как работает данная платформа. Вначале создадим новый проект. 
Назовем его <span class="b">MobileStore</span>. В качестве шаблона проекта выберем Web Application (View-Model-Controller):</p>
<img src="./pics/mvc6.png" alt="Веб-сайт на ASP.NET Core MVC" />
<p>Наше приложение будет очень простым и будет представлять магазин смартфонов. Какие основные функции у любого магазина - 
предоставлять товар на продажу и принимать заказы на товар. В связи с этим мы можем выделить прежде всего две модели: модель смартфона и модель заказа.</p>
<p>Итак, в проекте уже есть папка для хранения моделей - папка <span class="b">Models</span>, поэтому поместим в нее новый класс, который назовем <span class="b">Phone</span> и который будет представлять модель смартфона:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Модель телефона имеет четыре свойства: Id, название, производитель и цена.</p>
<p>И также добавим в папку Models новый класс, который будет представлять заказ:</p>
<pre class="brush:c#;">
public class Order
{
...
<p>Здесь все свойства довольно простые кроме двух последних. Два последних свойства вместе представляют внешний ключ на связанную модель Phone.</p>
<p>После определения моделей надо выбрать хранилище данных для этих моделей. Мы будем использовать MS SQL Server. Для работы с MS SQL Server компания Microsoft 
рекомендует использовать ORM-технологию Entity Framework, хотя ее использование необязательно. Мы также можем применять другие ORM-технологии или доступные средства ADO.NET. 
Преимущество фреймворка Entity Framework состоит в том, что он позволяет абстрагироваться от структуры конкретной базы данных и вести все операции с данными 
через модель.</p>
<p>В данном случае для взаимодействия с MS SQL Server через Entity Framework нам нужен пакет 
<span class="b">Microsoft.EntityFrameworkCore.SqlServer</span>. Поэтому добавим данный пакет через пакетный менеджер NuGet:</p>
<img src="./pics/mvc23.png" alt="Microsoft.EntityFrameworkCore.SqlServer" />
<p>Чтобы взаимодействовать с базой данных нам нужен контекст данных. Причем Entity Framework Core использует подход Code First, при котором нам надо сначала определить 
модели и контекст данных, а потом уже исходя и этих моделей и класса контекста будет создаваться бд и все ее таблицы.</p>
<p>Поэтому добавим в папку Models новый класс, который назовем <span class="b">MobileContext</span> и который будет иметь следующий код:</p>
<pre class="brush:c#;">
...
<p>Чтобы создать контекст, нам надо унаследовать новый класс от класса <span class="b">DbContext</span>. Свойства наподобие 
<code>public DbSet&lt;Phone&gt; Phones { get; set; }</code> помогают получать из БД набор данных определенного типа (например, набор объектов Phone). Фактически 
каждое свойство DbSet будет соотноситься с отдельной таблицей в базе данных.</p>
<p>По умолчанию у нас база данных отсутствуют. Поэтому в конструктор MobileContext определен вызов <code>Database.EnsureCreated()</code>, который 
при отсутствии базы данных автоматически создает ее. Если база данных уже есть, то ничего не происходит.</p>
<p>Чтобы подключаться к базе данных, нам надо задать параметры подключения. Для этого изменим файл <span class="b">appsettings.json</span>. По умолчанию он содержит только настройки 
логгирования:</p>
<pre class="brush:js;">
{
  "Logging": {
...
<p>В данном случае мы будем использовать упрощенный движок базы данных LocalDB, который представляет легковесную версию SQL Server Express, предназначенную специально для разработки приложений. 
Об этом говорит параметр <code>Server=(localdb)\\mssqllocaldb</code>. Ну а сама база данных будет называться <code>mobilestoredb</code>.</p>
<p>И последним шагом в настройке проекта является изменение файла Startup.cs. В нем нам надо изменить метод <code>ConfigureServices()</code>:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./3.6.php">Назад</a><a href="./">Содержание</a><a href="./3.3.php">Вперед</a></p></div></div>
<div class="socBlock">
