...
<html >
<head>
<title>ASP.NET Core | Валидация на стороне сервера</title>
<meta charset="utf-8" />
<meta name="description" content="Валидация на стороне сервера в ASP.NET Core, свойство ModelState.Invalid, добавление ошибок валидаци и метод ModelState.AddModelError">
...
<h2>Валидация на стороне сервера</h2><div class="date">Последнее обновление: 08.12.2019</div>
...
<p>Валидация на стороне сервера, то есть в контроллере, осуществляется посредством помощью проверки свойства <span class="b">ModelState.IsValid</span>.</p>
<p>Объект ModelState сохраняет все значения, которые пользователь ввел для свойств модели, а также все ошибки, связанные с каждым свойством и с моделью в 
целом. Если в объекте ModelState имеются какие-нибудь ошибки, то свойство ModelState.IsValid возвратит false:</p>
<pre class="brush:c#;">
[HttpPost]
...
<p>Допустим, при отправке модели Person в контроллер для свойства Name значение не указано. Поскольку к свойству применяется атрибут Required, то после 
привязки модели при валидации мы получим следующие значения:</p>
<pre class="brush:c#;">
...
<p>Мы также можем проверять корректность значений отдельных свойств модели:</p>
<pre class="brush:c#;">
[HttpPost]
public IActionResult Create(Person person)
{
...
<p>В данном случае метод <code>ModelState.AddModelError</code> добавляет для свойства, указанного в качестве первого параметра (в данном случае <code>Name</code>) 
ошибку "Недопустимая длина строки". При использовании хелперов впоследствии мы можем вывести данное сообщение об ошибке:</p>
<pre class="brush:xml;">
&lt;div&gt;
...
<img src="./pics/validation8.png" alt="ModelState в ASP.NET Core" />
...
<p>В данном случае мы не хотим, чтобы имя и пароль совпадали. И при их совпадении устанавливаем ошибку для всей модели. Подобные ошибки часто выводятся в верху формы ввода. И для 
подобных ошибок мы можем использовать следующий хелпер:</p>
<pre class="brush:xml;">&lt;div class="validation" asp-validation-summary="ModelOnly"&gt;&lt;/div&gt;</pre>
<img src="./pics/validation9.png" alt="Валидация модели в ASP.NET Core" />
...
Но мы можем настроить это поведение, определив нужное количество ошибок в методе <code>ConfigureServices()</code> класса Startup:</p>
<pre class="brush:c#;">
