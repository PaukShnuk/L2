...
<html >
<head>
<title>ASP.NET Core | Сессии</title>
<meta charset="utf-8" />
<meta name="description" content="Сессии в ASP.NET Core, создание и уничтожение сессий">
...
<h2>Сессии</h2><div class="date">Последнее обновление: 03.11.2019</div>
...
<p>Чтобы использовать сессии, необходимо сконфигурировать их параметры в классе Startup. Все сессии работают поверх объекта <span class="b">IDistributedCache</span>, 
и ASP.NET Core предоставляет встроенную реализацию IDistributedCache, которую мы можем использовать. И для этого изменим класс Startup следующим образом:</p>
<pre class="brush:c#;">
...
<p>В методе <code>ConfigureServices()</code> добавляются необходимые сервисы:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
    services.AddDistributedMemoryCache();
...
<p>В методе <code>Configure()</code> механизм работы с сессиями встраивается в конвейер обработки запроса:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app)
{
...
<p>После вызова <code>app.UseSession()</code> мы сможем использовать сессии у объекта HttpContext. В данном случае мы проверяем, определен ли в сессии 
...
<img src="./pics/2.43.png" alt="Сессии в ASP.NET Core" />
...
<img src="./pics/2.44.png" alt="Управление сессиями в ASP.NET Core" />
<h3>Управление сессией</h3>
<p>Объект Session определяет ряд свойств и методов, которые мы можем использовать:</p>
<ul>
...
<p>Для разграничения сессий для них устанавливается идентификатор. Каждая сессия имеет свой идентификатор, который сохрнаяется в куках. 
...
<p>Для использования этих свойств изменим код метода <code>ConfigureServices()</code>:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
    services.AddDistributedMemoryCache();
...
частности, добавим следующий класс:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Http;
...
<p>Сохраним в сессию и обратно десериализуем объект в методе Configure:</p>
<pre class="brush:c#;">
public class Startup
{
	public void ConfigureServices(IServiceCollection services)
...
<div class="nav"><p><a href="./2.25.php">Назад</a><a href="./">Содержание</a><a href="./2.10.php">Вперед</a></p></div></div>
<div class="socBlock">
