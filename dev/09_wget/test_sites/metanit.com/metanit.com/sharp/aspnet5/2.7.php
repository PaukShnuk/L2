...
<html >
<head>
<title>ASP.NET Core | Сервер</title>
<meta charset="utf-8" />
<meta name="description" content="Настройка и запуск веб-серверов в ASP.NET Core, Kestrel, WebListener, HTTP.sys и IIS">
...
<h1>Сервер и публикация приложения</h1><h2>Сервер</h2><div class="date">Последнее обновление: 28.12.2019</div>
...
<p>Традиционно приложения ASP.NET развертывались на веб-сервере IIS. Однако поскольку ASP.NET Core имеет кроссплатформенную природу, потребовалось 
отвязать ASP.NET Core от IIS и в целом от Windows. И на данный момент для развертывания приложения ASP.NET Core поддерживает развертывание приложения на следующих веб-серверах:</p>
 IIS и 
IIS Express, а также предоставляет возможность запускать приложение без IIS в рамках собственного процесса с помощью двух дополнительных http-серверов, которые идут вместе с ASP.NET Core:</p>
<ul>
<li><p>IIS HTTP Server</p></li>
<li><p>Microsoft.AspNetCore.Server.HttpSys (или просто WebListener) (в предыдущих версиях ASP.NET Core назывался WebListener)</p></li>
<li><p>Microsoft.AspNetCore.Server.Kestrel (или просто Kestrel)</p></li>
</ul>
<p> HTTP.sys и Kestrel представляют два дополнительных http-серверов, которые идут вместе с ASP.NET Core. HTTP.sys работает только на платформе Windows, а Kestrel является кроссплатформенным.</p>
<p>Кроме того, если приложение использует Kestrel, то в качестве прокси-сервера оно может использовать также IIS, Apache и Nginx. То есть Apache, Nginx или IIS будут 
...
позволяет нам задействовать возможности, которые есть у этих веб-серверов, но отсутствуют у Kestrel.</p>


<h3>IIS и IIS Express</h3>
<p>По умолчанию приложения ASP.NET работают с сервером IIS. Однако надо заметить, что по сравнению с предыдущими версиями ASP.NET при работе 
с ASP.NET Core IIS не использует инфраструктуру <span class="b">System.Web</span>, что значительно повышает производительность приложения. Кроме того, 
...
приложениями.</p>
<h3>AspNetCoreModule</h3>
<p>Хостирование приложений ASP.NET Core на IIS происходит с помощью нативного модуля IIS под названием <span class="b">AspNetCoreModule</span>, 
...
хостируется приложение, и перенаправляет все запросы от IIS к этому хостирующему процессу.</p>
<img src="./pics/2.14.png" alt="Перенаправление запросов от IIS к Kestrel" />
<p>При получении первого запроса к приложению AspNetCoreModule запускает процесс для этого приложения и перезапускает процесс, если приложение падает.</p>
<p>Вначале все входящие запросы проходят через драйвер Http.Sys, который перенаправляет их на IIS на основной порт (80) или на порт SSL (443). 
...
запрос и возвращает IIS результат обработки, который затем посылается HTTP-клиенту (например, веб-бразеру).</p>
<p>Для использования модуля он должен быть установлен. Он распространяется в рамках пакета <a href="https://go.microsoft.com/fwlink/?linkid=844461" rel="nofollow">ASP.NET Core Server Hosting Bundle</a>. 
Но при установки Visual Studio с ASP.NET Core модуль AspNetCoreModule уже автоматически устанавливается для IIS Express / IIS, поэтому разработчикам, как правило, не придется его дополнительно доустанавливать.</p>
<p>Также для использования модуля непосредственно в приложении в проекте должен быть установлен Nuget-пакет 
<span class="b">Microsoft.AspNetCore.Server.IISIntegration</span>.</p>
<p>Для интеграции с IIS для объекта WebHostBuilder вызывается метод <span class="b">UseIISIntegration()</span>. 
Этот метод ищет переменные окружения, которые устаналиваются модулем AspNetCoreModule, если подобных переменных не установлено, то метод по сути ничего не делает.</p>
<p>Мы можем вызвать этот метод явно в файле Program.cs:</p>
<pre class="brush:c#;">
...
<p>Однако это необязательно делать, поскольку при запуске приложения через IIS система автоматически вызывает данный метод.</p>
<p>Среди преимуществ использования именно IIS, а не Kestrel следует отметить работу со статическими файлами. В стандартном веб-приложении ASP.NET Core 
...
В частности, IIS кэширует ранее считанный с диска статический файл и при последующих обращениях к нему берет этот файл из кэша, тем самым увеличивая производительность.</p>

<h3>Kestrel</h3>
<p>Kestrel представляет кроссплатформенный веб-сервер, основанный на кросплатформенной библиотеке асинхронного ввода/вывода libuv. 
Kestrel по умолчанию включается в проект ASP.NET Core.</p>
<p>При инициализации хоста у объекта WebHostBuilder вызывается метод <code>UseKestrel()</code>, который позволяет задействовать 
Kestrel. Несмотря на то, что по умолчанию исходный код в файле Program.cs не содержит этого вызова, этот метод вызывается автоматически.</p>
<p>Однако при настройке хоста мы можем явным образом вызвать метод UseKestrel для конфигурации сервера:</p>
<pre class="brush:c#;">
using System;
...
<p>Для настройки Kestrela применяются свойства и методы объекта <span class="b">KestrelServerOptions</span>. В частности, метод <span class="b">Listen</span> 
позволяет установить порт, по которому Kestrel будет запускаться.</p>
<p>Свойство <span class="b">Limits</span> устанавливает предельные значения для различных конфигурационных настроек. Так, свойство <code>MaxConcurrentConnections</code> 
задает максимально количество оновременно открытых соединений.</p>
<p>Свойство <code>MaxRequestBodySize</code> устанавливает максимальный размер для запроса в байтах.</p>
<p>Свойство <code>MinRequestBodyDataRate</code> задает минимальную скорость передачи данных в запросе в байтах в секунду.</p>
<p>Свойство <code>MinResponseDataRate</code> задает минимальную скорость передачи данных в исходящем потоке в байтах в секунду.</p>
<p>При развертывании на Windows Kestrel может применять IIS в качестве прокси-сервера, а при 
развертывании на Linux как прокси-серверы могут использоваться Apache и Nginx. Но также Kestrel может работать самостоятельно внтури своего процесса без IIS.</p>
<p>Так, по умолчанию в Visual Studio доступны две возможности для запуска: с проксированием через IIS и напрямую.</p>
<p>При выборе возможностей вариантов запуска мы можем по умолчанию встретить два профиля:</p>
<ul>
<li><p>IIS Express (запуск с проксированием через IIS Express)</p></li>
<li><p>Профиль, который совпадает с названием проекта (в моем случае это HelloApp) - тот пункт, который позволяет запускать приложение в отдельном процессе 
без всякого проксирования через IIS. Так как в файле Program.cs установлен в качестве сервера Kestrel (методом <code>UseKestrel()</code>), то в данном случае 
приложение будет запускать именно Kestrel. Причем по умолчанию приложение будет запускаться на 5000-порту.</p></li>
</ul>
<img src="./pics/webserverlistener.png" alt="Профиль Kestrel в ASP.NET Core" />
<p>И если мы выберем для запуска второй пункт, то у нас запустится консольное приложение <code>dotnet.exe</code>, которое запустит наше приложение. 
И после этого мы сможем обращаться к нашему приложению из любого браузера:</p>
<img src="./pics/kestrel.png" alt="Kestrel web server in ASP.NET Core" />
<p>Запуск приложения с помощью Kestrel довольно прост, нам не надо устанавливать и настраивать другие веб-серверы. Однако специалисты Microsoft рекомендуют такой способ 
...
доступны напрямую. Кроме того, веб-серверы позволяет управлять нагрузкой ко всем приложениям, и предоставляют другие функции по управлению приложениями.</p>
<p>Собственно при создании проекта ASP.NET Core в Visual Studio такой способ используется по умолчанию.</p>
<h3>HTTP.sys</h3>
<p>HTTP.sys представляет HTTP-сервер для ASP.NET Core, который работает только в ОС Windows.  Ранне данный сервер назывался WebListener. Он 
запускается поверх драйвера ядра <span class="b">Http.Sys</span>. Весь функционал сервера сосредоточен в пакете <span class="b">Microsoft.AspNetCore.Server.HttpSys</span>.</p>
<p>Итак, изменим файл Program.cs, чтобы в нем использовался не Kestrel, а HttpSys:</p>
<pre class="brush:c#;">
public class Program
{
	public static void Main(string[] args)
...
<p>Если метод <code>UseKestrel()</code> задает в качестве сервера Kestrel, то вызов метода <code>UseHttpSys()</code> аналогичным образом устанавливает 
в качестве сервера HTTP.sys.</p>
<p>Теперь запустим приложение, выбрав профиль, который совпадает с названием проекта:</p>
<img src="./pics/webserverlistener.png" alt="Профиль Kestrel в ASP.NET Core" />
<p>И у нас запустится та же самая консоль, что и при работе через Kestrel, только теперь все вызовы к приложению будут идти через WebListener.</p>
<img src="./pics/kestrel.png" alt="Kestrel web server in ASP.NET Core" />
<p>Если Kestrel рекомендуется запускать вместе с проксирующими веб-серверами типа IIS, Apache, то для HTTP.sys не требуется 
проксирующего веб-сервера (более того IIS не может работать с HTTP.sys), так как Http.Sys позволяет обеспечить безопасность и надежность.</p>
...
<div class="nav"><p><a href="./14.4.php">Назад</a><a href="./">Содержание</a><a href="./20.1.php">Вперед</a></p></div></div>
<div class="socBlock">
...
$(function() { 
  ...
  $("#toggle-theme").on("click", function() {
	...
	$("li:contains('Глава 25.')").removeClass('expandable').addClass('collapsable').children('ul').css({"display":"block"});
	$("li:contains('Глава 25.')").children('div').removeClass('expandable-hitarea').addClass('collapsable-hitarea');
