...
<html >
<head>
<title>ASP.NET Core | Редактирование и удаление моделей в Entity Framework</title>
<meta charset="utf-8" />
<meta name="description" content="Редактирование и удаление в базе данных MS SQL Server в ASP.NET Core через Entity Framework">
...
<h2>Основные операции с моделями. Редактирование и удаление</h2><div class="date">Последнее обновление: 09.12.2019</div>
...
<p>Продолжим работу с проектом из прошлой темы. И изменим представление Index.cshtml, добавив в него ссылки:</p>
<pre class="brush:xml;">
...
<p>Теперь создадим для каждого действия методы.</p>
<h3>Вывод информации о модели</h3>
<p>Добавим в контроллер следующий метод:</p>
<pre class="brush:c#;">
...
<p>Метод получает id объекта, о котором надо вывести информацию. Затем найденный объект передается в представление. Теперь 
добавим в папку <span class="ii">Views/Home</span> новое представление <span class="b">Details.cshtml</span>:</p>
<pre class="brush:xml;">
...
<p>Для редактирования объекта добавим в контроллер следующую пару методов:</p>
<pre class="brush:c#;">
...
<p>GET-версия метода Edit возвращает форму с данными объекта, которые пользователь может отредактировать. А post-версия Edit получает отредактированные 
данные в виде объекта user и с помощью метода <code>db.Users.Update(user)</code> для этих данных будет генерироваться 
sql-выражение <code>UPDATE</code>, которое будет выполнено вызовом <code>db.SaveChangesAsync()</code></p>
<p>И также добавим для этих методов в папку <span class="ii">Views/Home</span> представление <span class="b">Edit.cshtml</span>:</p>
<pre class="brush:xml;">
@model EFDataApp.Models.User
@{
...
<p>Эта форма аналогична форме в представлении для создания нового объекта за исключением атрибута <code>asp-route-id="@Model.Id"</code>. 
...
Entity Framework не сможет обновить соответствующую модель в базе данных.</p>
<h3>Удаление</h3>
<p>Для создания инфраструктуры удаления добавим в контроллер следующие методы:</p>
<pre class="brush:c#;">
[HttpGet]
[ActionName("Delete")]
...
<p>Здесь надо отметить, что оба метода принимают один единственный параметр id, и из-за этого мы не можем назвать методы одинаково, как в случае с Edit или Create. 
И поэтому первый метод назван ConfirmDelete. Однако используемый над ним атрибут <code>ActionName("Delete")</code> указывает, что 
этот метод также относится к действию Delete, и поэтому мы к нему можем обращаться с запросом Home/Delete, а не Home/ConfirmDelete.</p>
<p>В первом методе удаляемый объект просто извлекается из БД и передается в представление. Во втором методе опять же по id получаем удаляемый объект 
и удаляем его с помощью метода <code>db.Users.Remove()</code>. Данный метод генерирует sql-выражение <code>DELETE</code>, которое выполняется вызовом 
<code>db.SaveChangesAsync()</code>.</p>
<p>И затем добавим в папку <span class="ii">Views/Home</span> новое представление <span class="ii">Delete.cshtml</span>:</p>
<pre class="brush:xml;">
@model EFDataApp.Models.User
@{
...
<p>Причем надо отметит, что на форме нет никакого поля для параметра id и в теге формы также не устанавливается параметр id, как, например, 
в представлении Edit, так как данный параметр берется из строки запроса.</p>
<p>Может возникнуть вопрос, почему бы не сделать удаление сразу в методе ConfirmDelete? Дело в том, что использование get-методов не безопасно. 
Например, нам могут прислать письмо с картинкой:</p>
<pre class="brush:xml;">
&lt;img src="http://адрес_нашего_сайта/Home/Delete/1" /&gt;
</pre>
<p>И при открытии письма на сервер будет отправлен get-запрос к методу ConfirmDelete, и объект с id=1 будет удален из базы данных.</p>
<p>И еще надо отметить, что в post-методе Delete мы можем произвести небольшую оптимизацию. Иногда бывает важно узнать перед удалением, а есть 
...
И мы можем оптимизировать метод следующим образом:</p>
<pre class="brush:c#;">
[HttpPost]
public async Task&lt;IActionResult&gt; Delete(int? id)
{
...
<p>В данном случае нам важен только id объекта. Поэтому создаем новый объект user и устанавливаем у него состояние Deleted: 
<code>db.Entry(user).State = EntityState.Deleted;</code>. Это выражение опять же сгенерирует sql-выражение <code>DELETE</code>.</p>
...
<div class="nav"><p><a href="./12.2.php">Назад</a><a href="./">Содержание</a><a href="./12.4.php">Вперед</a></p></div></div>
<div class="socBlock">
