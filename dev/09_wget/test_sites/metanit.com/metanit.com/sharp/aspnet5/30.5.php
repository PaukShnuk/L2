...
<html >
<head>
<title>ASP.NET Core | Взаимодействие с клиентами в SignalR</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие хаба с клиентами в SignalR в ASP.NET Core, отправка сообщений подключениям">
...
<h2>Взаимодействие с клиентами</h2><div class="date">Последнее обновление: 24.12.2019</div>
...
<p>Для взаимодействия с клиентами в классе Hub определено свойство <span class="b">Clients</span>. Оно представляет всех подключенных клиентов 
...
то мы можем использовать данный объект. Например:</p>
<pre class="brush:c#;">
...
<p>В данном случае у всех подключенных клиентов вызывается функция Receive, которой передается строка message.</p>
<p>У объекта Clients есть следующие свойства:</p>
<ul>
<li><p><span class="b">All</span>: представляет всех подключенных клиентов</p></li>
<li><p><span class="b">Caller</span>: представляет только текущего клиента, который обратился к хабу</p></li>
<li><p><span class="b">Others</span>: представляет всех клиентов за исключением текущего клиента, который обратился к хабу</p></li>
</ul>
<p>Кроме свойств объект Clients имеет ряд методов:</p>
<ul>
<li><p><span class="b">AllExcept(IReadOnlyList&lt;string&gt; connectionIds)</span>: представляет всех клиентов за исключением тех, 
id которых передаются в метод</p></li>
<li><p><span class="b">Client(string connectionId)</span>: вызывает метод у клиента по id подключения</p></li>
<li><p><span class="b">Clients(IReadOnlyList&lt;string&gt; connectionIds)</span>: вызывает метод у клиентов, id которых передаются в метод</p></li>
<li><p><span class="b">Group(string groupName)</span>: вызывает метод у клиентов определенной группы</p></li>
<li><p><span class="b">GroupExcept(string groupName, IReadOnlyList&lt;string&gt; connectionIds)</span>: вызывает метод у
клиентов группы по имени groupName за исключением тех клиентов, id которых передаются в качестве второго параметра</p></li>
<li><p><span class="b">Groups(IReadOnlyList&lt;string&gt; groupNames)</span>: вызывает метод у клиентов групп, названия которых передаются в метод</p></li>
<li><p><span class="b">OthersInGroup(string OthersInGroup)</span>: вызывает метод у клиентов определенной группы за исключением текущего клиента</p></li>
<li><p><span class="b">User(string userId)</span>: вызывает метод у пользователя по id</p></li>
<li><p><span class="b">Users(IReadOnlyList&lt;string&gt; userIds)</span>: вызывает метод у пользователей, id которых передаются в метод</p></li>
</ul>
<p>Обратите внимание, что концепция клиентов и концепция пользователей в данном случае отличается. Клиент по сути представляет отдельное подключение, с которым 
...
приложения.</p>
<p>Например, определим следующий хаб:</p>
<pre class="brush:c#;">
...
<p>При вызове метода Send у текущего клиента будет вызываться функция Notify, а у всех остальных клиентов - функция Receive.</p>
<p>Для теста определим следующую страницу index.html:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>И после условного добавления товара клиент получит одно сообщение:</p>
<img src="./pics/signalr10.png" alt="Отправка сообщений сервера клиенту в ASP NET Core SignalR" />
<p>А все остальные подключенные клиенты получат своего рода push-уведомление о произведенных действиях:</p>
<img src="./pics/signalr11.png" alt="Push уведомления в ASP NET Core SignalR" />
<p>Также мы могли бы переписать метод Send в хабе следующим образом:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.SignalR;
using System;
...
<p>В данном случае метод Send выполняет все те же действия, только теперь для определения клиентов для отправки сообщений применяется идентификатор клиента, который получаем 
из контекста хаба.</p>
...
<div class="nav"><p><a href="./30.4.php">Назад</a><a href="./">Содержание</a><a href="./30.6.php">Вперед</a></p></div></div>
<div class="socBlock">
