...
<html >
<head>
<title>ASP.NET Core | Применение сервисов в middleware</title>
<meta charset="utf-8" />
<meta name="description" content="Применение сервисов в компонентах middleware в ASP.NET Core">
...
<h2>Применение сервисов в middleware</h2><div class="date">Последнее обновление: 07.11.2019</div>
...
<p>После добавления сервисов в классе Startup они становятся доступны приложению, в том числе и в кастомных компонентах middleware. В 
middleware мы можем получить зависимости тремя способами:</p>
<ul>
<li><p>Через конструктор</p></li>
<li><p>Через параметр метода Invoke/InvokeAsync</p></li>
<li><p>Через свойство HttpContext.RequestServices</p></li>
</ul>
<p>При этом надо учитывать, что компоненты middleware создаются при создании класса Startup и живут в течение всего жизненного цикла приложения. То есть 
при последующих запросах asp.net core использует уже ранее созданный компонент. И это налагает ограничения на использование зависимостей в middleware.</p>
<p>В частности, если конструктор передается transient-сервис, который создается при каждом обращении к нему, то при последующих запросах 
мы будем использовать тот же самый сервис, так как конструктор middleware вызывается один раз - при создании приложения.</p>
<p>Например, определим сервис TimeService:</p>
<pre class="brush:c#;">
...
<p>В конструкторе устанавливается свойство, которое хранит текущее время в виде строки.</p>
<p>Добавим новый компонент TimerMiddleware, который будет использовать этот сервис для вывода времени на веб-страницу:</p>
<pre class="brush:c#;">
...
<p>Если сервис TimeService добавляется в методе <code>ConfigureServices()</code>, то мы сможем получить его через конструктор класса TimerMiddleware.</p>
<p>Логика компонента предполагает, что, если строка запроса равна строке "time", то с помощью TimeService возвращается текущее время. Иначе мы просто обращаемся 
к следующему делегату в конвейере обработки запроса.</p>
<p>Используем этот компонент в классе Startup:</p>
<pre class="brush:c#;">
...
<p>В итоге, если мы используем в адресной строке путь "time", то приложение выведет текущее время:</p>
<img src="./pics/2.19.png" alt="Сервисы в middleware в ASP.NET Core" />
<p>Однако сколько бы мы раз не обращались по этому пути, мы все время будем получать одно и то же время, так как объект TimerMiddleware был создан еще при первом запросе. 
Поэтому передача через конструктор middleware больше подходит для сервисов с жизненным циклом Singleton, которые создаются один раз для всех 
последующих запросов.</p>
<p>Если же в middleware необходимо использовать сервисы с жизненным циклом Scoped или Transient, то лучше их передавать через параметр метода Invoke/InvokeAsync:</p>
<pre class="brush:c#;">
public class TimerMiddleware
{
	private readonly RequestDelegate _next;
...
<p>Также можно получать сервисы через свойство HttpContext.RequestServices в тех ситуациях, когда необходимость использования сервисов может зависеть от 
среды выполнения:</p>
<pre class="brush:c#;">
using DIApp.Services;
using Microsoft.AspNetCore.Http;
...
<div class="nav"><p><a href="./6.2.php">Назад</a><a href="./">Содержание</a><a href="./6.5.php">Вперед</a></p></div></div>
<div class="socBlock">
