...
<html >
<head>
<title>ASP.NET Core | Конфигурация по умолчанию и Dependency Injection</title>
<meta charset="utf-8" />
<meta name="description" content="Конфигурация по умолчанию в ASP.NET Core, файл appsettings.json, передача конфигурации в качестве сервиса в классы и middleware">
...
<h2>Конфигурация по умолчанию и Dependency Injection</h2><div class="date">Последнее обновление: 14.09.2017</div>
...
<h3>Объединение источников конфигурации</h3>
<p>При необходимости мы можем использовать сразу несколько источников конфигурации:</p>
<pre class="brush:c#;">
...
<p>Но в данном случае может возникнуть вопрос, как объединить все кастомные конфигурации с той конфигурацией, которая передается по умолчанию в приложения (из файла 
appsettings.json, параметров командной строки и переменных среды окружения)? Для этого мы можем использовать метод <span class="b">AddConfiguration()</span>, 
в который передается добавляемая конфигурация:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Builder;
using Microsoft.AspNetCore.Hosting;
using Microsoft.AspNetCore.Http;
...
<p>В данном случае конфигурация по умолчанию, которая передается в конструктор класса Startup, объединяется с настроками в памяти и файлом conf.json. В итоге создается новый объект конфигурации. 
В дальнейшем мы можем использовать его в классе Startup, например, в методе Configure.</p>
<p>Тем не менее в примере мы можем столкнуться с одной проблемой - через механизм dependency injection мы можем получить по прежнему только 
конфигурацию по умолчанию, которая передается в конструктор Startup. Поэтому в этом случае нам надо передать в контейнер зависимостей новый объект конфигурации, 
который мы сами создаем - как обычно в методе ConfigureServices.</p>
<p>Вначале определим для тестирования компонент middleware:</p>
<pre class="brush:c#;">
...
<p>В данном случае через сервис IConfiguration мы ожидаем получить параметры с ключами name и age.</p>
<p>Теперь внедрим конфигурацию в классе Startup:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Builder;
using Microsoft.Extensions.Configuration;
using Microsoft.Extensions.DependencyInjection;
...
<p>В методе ConfigureServices в метод <code>AddTransient()</code> через фабрику сервисов передается объект, который будет использоваться в качестве сервиса IConfiguration. 
...
приложении наш объект конфигурации.</p>
<img src="./pics/config7.png" alt="Сервис IConfiguration в ASP.NET Core" />
...
<div class="nav"><p><a href="./2.12.php">Назад</a><a href="./">Содержание</a><a href="./2.17.php">Вперед</a></p></div></div>
<div class="socBlock">
