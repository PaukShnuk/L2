...
<html >
<head>
<title>ASP.NET Core | Ресурсы и локализация в контроллерах</title>
<meta charset="utf-8" />
<meta name="description" content="Файлы ресурсов в ASP.NET Core, локализация в контроллерах с помощью сервиса IStringLocalizer, создание ресурсов для контроллеров">
...
<h2>Ресурсы и локализация в контроллерах</h2><div class="date">Последнее обновление: 18.12.2019</div>
...
<p>В прошлой теме мы познакомились с сервисом IStringLocalizer, который эффективно позволяет локализовать строки. Однако все используемые ресурсы 
были жестко определены в коде. Но, как правило, для хранения локализуемых строк применяются файлы ресурсов. 
Файл ресурсов позволяет отделить локализуемые строки, которые должны изменяться в зависимости от текущий языковой культуры, от кода приложения. 
Файлы ресурсов resx использовались широко и в предыдущих версиях ASP.NET и вообще в различных технологиях на платформе .NET. Но в ASP.NET Core их применение имеет свои особенности.</p>
<p>В прошлой теме для локализации была создана реализация интерфейса IStringLocalizer, но ASP.NET Core также уже имеет встроенную реализацию - ResourceManagerStringLocalizer, 
которая как раз и использует файлы ресурсов.</p>
<p>При определении ресурсов следует учитывать условности в наименовании файлов ресурсов. Так, файлы ресурсов должны называться по имени файла с учетом пространства имен, но без 
названия проекта, если оно совпадает с именем сборки. Например, ресурс для класса Startup должен представлять файл <span class="ii">Startup.[код_культуры].resx</span>.</p>
<p>Определим в проекте ресурсы для контроллера HomeController. Для этого добавим в проект новую папку <span class="b">Resources</span>. 
А в нее добавим файл ресурса <span class="b">Controllers.HomeController.ru.resx</span>:</p>
<img src="./pics/localization6.png" alt="Локализация и файлы ресурсов в ASP.NET Core" />
<p>Для добавления ресурсов в Visual Studio имеется встроенный шаблон <span class="b">Resources File</span>. 
Обратите внимание на название файла. По умолчанию HomeController находится в папке Controllers. Поэтому файл ресурсов называется 
Controllers.HomeController.ru.resx, а суффикс "ru" в конце файла указывает, что ресурсы будут предназначены для русскоязычной культуры.</p>
<p>Для контроллеров также допустимо размещение в папке ресурсов в подкаталоге Controllers. В этом случае файл ресурсов должен отражать только имя контроллера: <span class="ii">HomeController.ru.resx</span>. 
То есть в данном случае для контроллера HomeController допустимо создание файлов ресурсов со следующими путями:</p>
<ul>
<li><p><span class="ii">Resources/Controllers.HomeController.ru.resx</span></p></li>
<li><p><span class="ii">Resources/Controllers/HomeController.ru.resx</span></p></li>
</ul>
<p>После добавления файла определим в нем несколько ресурсов:</p>
<img src="./pics/localization7.png" alt="Добавление ресурсов resx в ASP.NET Core" />
<p>В данном случае определены два ресурса: Header и Message.</p>
<p>Добавим в папку Resources аналогичные файлы ресурсов для других языков:</p>
<img src="./pics/localization8.png" alt="Локализация ресурсов resx в ASP.NET Core MVC" />
<p>То есть получится несколько файлов, которые в названии отличаются только кодом культуры. И все файлы ресурсов имеют одни и те же ключи, только 
им соответствуют строки на соответствующем языке:</p>
<img src="./pics/localization9.png" />
<p>После добавления ресурсов лучше выполнить ребилд (перестроение) проекта.</p>
<p>Далее изменим класс <span class="b">Startup</span>:</p>
<pre class="brush:c#;">
...
<p>Ключевым моментом здесь является добавление сервисов локализации в методе ConfigureServices:</p>
<pre class="brush:c#;">services.AddLocalization(options =&gt; options.ResourcesPath = "Resources");</pre>
<p>Свойство options.ResourcesPath здесь указывает на каталог, где размещаются ресурсы. В нашем случае это папка Resources.</p>
<p>Используем эти ресурсы в контроллере HomeController:</p>
<pre class="brush:c#;">
...
<p>Сервис IStringLocalizer добавляется вместе с другими сервисами локализации, мы его можем использовать для получения значений. Только в отличие от прошлой темы, где 
все ресурсы были жестко определены в коде, теперь IStringLocalizer будет извлекать ресурсы из файлов ресурсов.</p>
<p>Также стоит отметить, что теперь объект IStringLocalizer должен типизироваться типом, для которого он используется, то есть типом HomeController.</p>
<p>Для теста пусть в представлении выводятся значения из ViewData:</p>
<pre class="brush:xml;">
&lt;h1&gt;@ViewData["Title"]&lt;/h1&gt;
&lt;h3&gt;@ViewData["Message"]&lt;/h3&gt;
</pre>
<p>Запустим приложение и передадим ему англоязычную культуру:</p>
<img src="./pics/localization10.png" alt="Локализация ресурсов в ASP.NET Core MVC" />
...
<div class="nav"><p><a href="./28.3.php">Назад</a><a href="./">Содержание</a><a href="./28.5.php">Вперед</a></p></div></div>
<div class="socBlock">
