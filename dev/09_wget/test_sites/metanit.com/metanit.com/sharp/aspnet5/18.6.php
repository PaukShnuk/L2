...
<html >
<head>
<title>ASP.NET Core | Работа с HTTPS</title>
<meta charset="utf-8" />
<meta name="description" content="Отладка приложения ASP.NET Core по HTTPS, методы UseHttpsRedirection и UseHsts, отправка заголовка Strict-Transport-Security">
...
<h2>Работа с HTTPS</h2><div class="date">Последнее обновление: 06.11.2019</div>
...
<p>Начиная с версии ASP.NET Core 2.1 были добавлены новые возможности для работы с приложением по протоколу HTTPS. Рассмотрим их.</p>
<h3>Отладка по https</h3>
<p>Создадим новый проект ASP.NET Core по шаблону Empty.</p>
<img src="./pics/https.png" alt="Https в ASP.NET Core" />
<p>При создании проекта мы можем отметить поле <span class="b">Configure for HTTPS</span>. Установка этого поля позволит по умолчанию включить отладку 
проекта по HTTPS. Правда, если мы даже не отметим это поле, мы все равно можем потом добавить в проект поддержку https через его свойства.</p>
<p>При установке отметки после создания проекта он уже сконфигурирован на запуск через HTTPS. В частности, если мы зайдем в Visual Studio в 
свойства проекта на вкладку Debug, то мы увидим, что на этой вкладке установлено поле <span class="b">Enable SSL</span>:</p>
<img src="./pics/https2.png" alt="Configure HTTPS and SSL in ASP.NET Core" />
<p>В процессе работы с проектом мы можем свободно включать или отключать это поле в зависимости от того, нужна ли нам отладка по Https.</p>
<p>Соответственно после запуска такого проекта он будет запускаться по протоколу HTTPS:</p>
<img src="./pics/https3.png" alt="Отладка проекта ASP.NET Core по HTTPS" />
<h3>UseHttpsRedirection</h3>
<p>Метод <span class="b">UseHttpsRedirection()</span> объекта IApplicationBuilder добавляет для проекта переадресацию на тот же ресурс только по протоколу 
https (если приложение имеет поддержку SSL). В частности, изменим в классе Startup метод Configure следующим образом:</p>
<pre class="brush:c#">
...
<p>С помощью метода <span class="b">AddHttpsRedirection()</span> мы можем настроить параметры переадресации. Например, изменим в классе Startup метод ConfigureServices 
следующим образом:</p>
<pre class="brush:c#;">
...
<p>Для настройки переадресации в метод AddHttpsRedirection передается функция, которая в качестве параметра принимает объект 
<span class="b">HttpsRedirectionOptions</span>. Этот объект имеет два свойства:</p>
<ul>
<li><p><span class="b">RedirectStatusCode</span>: статусный код переадресации - одно из значений перечисления StatusCodes. По умолчанию 
применяется значение <code>StatusCodes.Status307TemporaryRedirect</code></p></li>
<li><p><span class="b">HttpsPort</span>: устанавливает порт. Данная опция необходима, если порт отличается от стандартного 443. 
В данном случае это порт 44344. При запуске в Visual Studio через HTTPS данная опция не учитывается, поскольку порт устанавливает сама Visual Studio.</p></li>
</ul>
<h3>HTTP Strict Transport Security Protocol (HSTS)</h3>
<p>Когда пользователь первый раз обращается к сайту по протоколу http, даже если на самом сайте установлен сертификат ssl и переадресация на https, 
то пользователь при первом запросе все равно будет обращаться к сайту по незащищенной версии, то есть по протоколу http. Затем будет идти переадресация. 
Обращение по незащищенному протоколу уже может спровоцировать ряд атак. В связи с чем применяются специальный заголовок 
<span class="b">Strict-Transport-Security</span>. Этот заголовок отправляется сервером клиенту. Заголовок сообщает браузеру, что к приложению надо обращаться по протоколу https, а не по протоколу http. 
Кроме того, передаст параметры подобного обращения. Пример заголовка Strict-Transport-Security:</p>
<pre class="brush:c#;">
Strict-Transport-Security: max-age=63072000; includeSubDomains; preload 
</pre>
<p>Заголовок имеет три параметра:</p>
<ul>
<li><p><code>max-age</code>: задает максимальное действие заголовка (обязательный параметр)</p></li>
<li><p><code>includeSubDomains</code>: если этот параметр установлен, то действие применяется ко всем поддоменам</p></li>
<li><p><code>preload</code>: если этот параметр установлен, то будет использоваться специальный preload-список доменов, обращение к которым безопасно.</p></li>
</ul>
<p>Стоит отметить, что отправка этого заголовка браузеру - это не то же самое, что и переадресация. При переадресации устанавливается редирект одних отдельных адресов на другие. 
А заголовок Strict-Transport-Security устанавливает глобально политику работы с определенным доменом и его поддоменами.</p>
<p>В ASP.NET Core 2.1+ С помощью метода <span class="b">UseHsts()</span> объекта IApplicationBuilder мы можем отправить браузеру заголовок 
Strict-Transport-Security. Так, изменим метод <code>Configure</code> в классе Startup следующим образом:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
{
	if (env.IsDevelopment())
...
<p>Стоит отметить, что метод UseHsts вызывается, если только приложение уже развернуто для полноценного использования, потому что в процессе разработки 
использование данного метода может создавать неудобства, так как заголовки кэшируются.</p>
<p>Дополнительно, с помощью метода <span class="b">AddHsts</span> в методе ConfigureServices мы можем настроить параметры заголовка 
Strict-Transport-Security. Так, изменим, к примеру, метод ConfigureServices в классе Startup следующим образом:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
...
<p>В метод <span class="b">AddHsts()</span> передается функция, которая в качестве параметра принимает объект <span class="b">HstsOptions</span>. 
Этот объект имеет следующие свойства:</p>
<ul>
<li><p><span class="b">Preload</span>: устанавливает параметр preload</p></li>
<li><p><span class="b">MaxAge</span>: устанавливает параметр max-age</p></li>
<li><p><span class="b">IncludeSubDomains</span>: устанавливает параметр includeSubDomains</p></li>
<li><p><span class="b">ExcludedHosts</span>: добавляет список доменов, которые надо исключить из действия заголовка</p></li>
</ul>
...
<div class="nav"><p><a href="./17.1.php">Назад</a><a href="./">Содержание</a><a href="./6.1.php">Вперед</a></p></div></div>
<div class="socBlock">
