...
<html >
<head>
<title>ASP.NET Core | Авторизация</title>
<meta charset="utf-8" />
<meta name="description" content="Авторизация в ASP.NET Core, атрибут Authorize и AllowAnonymous, интерфейс IPrincipal и роль в аутентификации пользователя">
...
<h2>Авторизация</h2><div class="date">Последнее обновление: 13.12.2019</div>
...
<p>Ключевым инструментом для авторизации является атрибут <span class="b">AuthorizeAttribute</span> из пространства имен <code>Microsoft.AspNetCore.Authorization</code>. 
Например, в прошлой теме данный атрибут ограничивал доступ к методу Index контроллера HomeController:</p>
<pre class="brush:c#;">
...
<p>В этом случае доступ к методу Index имеют только те пользователи, которые залогинились в приложении. Анонимные пользователи же в данном случае при доступе 
к методу Index будут переадресованы на форму входа в приложение.</p>
<p>При этом атрибут <code>Authorize</code> не указывает, как пользователь должен аутентифицироваться, он только выполняет проверку. 
Для проверки статуса пользователя атрибут использует свойство <code>User</code> объекта HttpContext, которое устанавливается инфраструктурой ASP.NET.</p>
<p>Свойство <code>HttpContext.User</code> представляет объект интерфейса <span class="b">IPrincipal</span>, который определен в пространстве имен 
<code>System.Security.Principal</code>. Этот интерфейс определяет метод <code>IsInRole()</code> и свойство <code>Identity</code>.</p>
<p>Свойство <span class="b">Identity</span> возвращает объект интерфейса <span class="b">IIdentity</span>, который связан с текущим запросом.</p>
<p>Метод <span class="b">IsInRole()</span> в качестве параметра принимает роль и возвращает <code>true</code>, если текущий пользователь принадлежит данной роли.</p>
<p>Объект <code>IIdentity</code>, в свою очередь, предоставляет информацию о текущем пользователе через следующие свойства:</p>
<ul>
<li><p><span class="b">AuthenticationType</span>: тип аутентификации в строковом виде</p></li>
<li><p><span class="b">IsAuthenticated</span>: возвращает <code>true</code>, если пользователь аутентифицирован</p></li>
<li><p><span class="b">Name</span>: возвращает имя пользователя. Как правило, в качестве подобного имени используется логин, по которому пользователь входит в приложение</p></li>
</ul>
<p>Для определения аутентифицирован ли пользователь, ASP.NET Core использует аутентификационные куки.</p>
<p>Мы можем применять эти свойства в контроллерах или представлениях:</p>
<pre class="brush:c#;">
public IActionResult Index()
{
...
<p>Атрибут AllowAnonymous позволяет открыть доступ для анонимных пользователей. Обычно он применяется к методам контроллера, который уже защищен атрибутом 
Authorize. Например:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Authorization;
...
<p>Здесь доступ ко всей функциональности контроллера Home имеют только аутентифицированные пользователи, за исключением доступа к методу Index: 
к нему могут обращаться абсолютно все пользователи приложения.</p>
