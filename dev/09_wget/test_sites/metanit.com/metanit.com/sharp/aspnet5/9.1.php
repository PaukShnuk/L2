...
<html >
<head>
<title>ASP.NET Core | HTML-хелперы</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в HTML-хелперы в ASP.NET Core MVC, создание своих хелперов и использование их в представлениях, класс TagBuilder">
...
<h1>HTML-хелперы</h1><h2>Создание HTML-хелперов</h2><div class="date">Последнее обновление: 30.11.2019</div>
...
<p>Для вывода контента в представлении мы можем использовать стандартные html-элементы, которые позволяют создавать блоки, списки, таблицы и т.д. Но кроме 
собственно html-элементов для создания разметки мы можем использовать специальные методы - html-хелперы. Вообще helper можно перевести с 
английского как "вспомогательный метод". И фактически html-хелперы представляют собой вспомогательные методы, цель которых - генерация html-разметки.</p>
<p>Для создания простейшего html-хелпера возьмем проект ASP.NET Core по типу <span class="b">Web Application (Model-View-Controller)</span> и добавим в него папку <span class="b">App_Code</span>. 
Затем в эту папку добавим новый класс <span class="b">ListHelper</span>:</p>
<pre class="brush:c#;">
...
<p>В новом классе хелпера определен один статический метод <code>CreateList</code>, принимающий в качестве первого параметра объект, для которого 
создается метод. Так как данный метод расширяет функциональность html-хелперов, которые представляет интерфейс 
<span class="b">Microsoft.AspNetCore.Mvc.Rendering.IHtmlHelper</span>, то именно объект этого типа и передается в данном случае в качестве первого 
параметра. Второй параметр метода CreateList - массив строк-значений, которые потом будут выводиться в списке.</p>
<p>В самом методе просто пробегаемся по массиву строк и формируем из них разметку html в виде строки. Результатом метода является объект <span class="b">HtmlString</span>, 
который в конструкторе получает разметку html в виде строки.</p>
<p>Этот очень простой метод уже может упростить работу с разметкой. Рассмотрим его использование. Допустим, нам надо в представлении вывести массив строк в списке:</p>
<pre class="brush:xml;">
@{
...
<p>Поскольку html-хелпер представляет метод расширения для объекта IHtmlHelper, то для его применения нам достаточно написать <code>Html.CreateList</code> 
и передать в метод необходимые параметры. Либо мы можем вызвать его как метод класса, в котором он определен: <code>ListHelper.CreateList</code></p>
<p>И теперь, если мы захотим создать список <code>&lt;ul&gt;</code>, нам достаточно будет написать одну строку с вызовом хелпера, передав ему массив:</p>
<img src="./pics/htmlhelper1.png" alt="Html Helper in ASP.NET Core" />
<p>При отсутствии подобного хелпера, то нам бы пришлось по сути дублировать один и тот же html-код для создания списка. Однако этот хелпер еще довольно простой, а если нам приходится создавать по сто раз более сложную, но однотипную разметку html, тогда хелперы окажутся еще более полезными.</p>
<h3>TagBuilder</h3>
<p>Для создания html-тегов в хелпере мы можем использовать класс <span class="b">Microsoft.AspNetCore.Mvc.Rendering.TagBuilder</span>. Так, перепишем код хелпера 
следующим образом:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Html;
using Microsoft.AspNetCore.Mvc.Rendering;
...
<p>В конструктор TagBuilder передается элемент, для которого создается тег. TagBuilder имеет ряд свойств и методов, которые можно использовать:</p>
<ul>
<li><p>Свойство <span class="b">InnerHtml</span> позволяет установить или получить содержимое тега в виде строки. Чтобы манипулировать этим свойством, 
можно вызвать один из методов:</p>
<ul>
<li><p><code>Append(string text)</code>: добавление строки теста внутрь элемента</p></li>
<li><p><code>AppendHtml(IHtmlContent html)</code>: добавление в элемент кода html в виде объекта IHtmlContent - это может быть другой объект TagBuilder</p></li>
<li><p><code>Clear()</code>: очистка элемента</p></li>
<li><p><code>SetContent(string text)</code>: установка текста элемента</p></li>
<li><p><code>SetHtmlContent(IHtmlContent html)</code>: установка внутреннего кода html в виде объекта IHtmlContent</p></li>
</ul>
</li>
<li><p>Свойство <span class="b">Attributes</span> позволяет управлять атрибутами элемента</p></li>
<li><p>Метод <span class="b">MergeAttribute()</span> позволяет добавить к элементу один атрибут</p></li>
<li><p>Метод <span class="b">AddCssClass()</span> позволяет добавить к элементу класс css</p></li>
<li><p>Метод <span class="b">WriteTo()</span> позволяет создать из элемента и его внутреннего содержимого строку при помощью объектов TextWriter и HtmlEncoder.</p></li>
</ul>
<p>В итоге мы получим тот же самый список, что и ранее.</p>
