...
<html >
<head>
<title>ASP.NET Core | View Component</title>
<meta charset="utf-8" />
<meta name="description" content="View Component в ASP.NET Core MVC, создание и использование компонента, атрибут ViewComponent">
...
<h1>View Component</h1><h2>Определение компонента представлений</h2><div class="date">Последнее обновление: 26.11.2019</div>
...
<p>View Component (или компонент представлений) представляет новую концепцию, которая появилась в ASP.NET Core MVC, и которая во многом схожа с 
частичными представлениями. При этом View Component предназначен для таких задач, которые, возможно, сложно решить с помощью одних частичных представлений, например, 
создание динамических меню, облако тегов, панель логина, корзина покупок и так далее.</p>
...
HTTP-запросы, а генерируемый им контент включается в код родительского представления, в котором вызывается компонент. Нередко View Component используется на мастер-странице _layout.cshtml.</p>
<p>И подобно контроллерам view component должен представлять класс с модификатором <code>public</code>. Кроме того, класс компонента не должен быть вложенным или абстрактным.</p>
...
<p>Для работы с компонентами представлений создадим новый проект ASP.NET Core по типу Web Application (Model-View-Controller).</p>
...
плюс в самом классе должен быть определен метод <code>Invoke()</code> (если он должен выполнять действия в синхронном режиме) или <code>InvokeAsync</code> (если он должен выполнять действия в асинхронном режиме).</p>
...
<p>Теперь мы можем использовать компонент. При этом его использоваие не зависит от способа его определения. Есть два способа применения компонента в представлении.</p>
<h4>Метод Component.InvokeAsync</h4>
<p>По умолчанию у нас уже есть контроллер HomeController, а значит в папке Views есть каталог Home, в котором находится представление Index.cshtml. Добавим в него вызов определенного нами компонента:</p>
<pre class="brush:xml;">@await Component.InvokeAsync("Timer")</pre>
<p>В качестве параметра в метод <code>InvokeAsync()</code> передается название компонента. В нашем случае это "Timer". Пи этом не важно, какой из трех вышеописанных способов 
...
<img src="./pics/viewcomponent4.png" alt="View Component in ASP.NET Core MVC" />
<h4>Tag-хелпер vc</h4>
...
<p>Однако перед использованием тега нобходимо зарегистрировать наш компонент в качестве tag-хелпера. Для этого в файл <span class="b">_ViewImports.cshtml</span> 
в папке <span class="b">Views</span> добавим следующую директиву:</p>
<pre class="brush:xml;">
@addTagHelper *, [Название_сборки_проекта]
</pre>
<p>Название сборки проекта как правило совпадает с названием самого проекта. То есть в моем случае проект называется ViewComponents, поэтому я 
добавляю в файл _ViewImports.cshtml строку</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./10.9.php">Назад</a><a href="./">Содержание</a><a href="./7.11.php">Вперед</a></p></div></div>
<div class="socBlock">
