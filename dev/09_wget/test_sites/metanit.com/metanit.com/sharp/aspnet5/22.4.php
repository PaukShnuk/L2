...
<html >
<head>
<title>ASP.NET Core | Фреймворк Moq и moq-объекты</title>
<meta charset="utf-8" />
<meta name="description" content="Фреймворк Moq в ASP.NET Core, использование moq-объектов для имитации репозитория">
...
<h2>Фреймворк Moq и moq-объекты</h2><div class="date">Последнее обновление: 21.12.2019</div>
...
<p>Нередко методы контроллера или функционал тестируемых классов использует какие-то другие внешние классы. Например, при работе с базой данных может использоваться 
контекст данных или репозиторий, которые определены как отдельные классы.</p>
<p>Например, пусть в проекте ASP.NET Core определена модель User:</p>
<pre class="brush:c#;">
...
<p>Для работы с данными добавим общий интерфейс репозитория:</p>
<pre class="brush:c#;">
...
<p>Этот интерфейс может реализоваться разными классами, которые используют разные источники данных - базу данных, файлы xml, список объектов в памяти, внешние сервисы и 
так далее. Нам это не важно, при необходимости мы можем предусмотреть и установить через встроенный механизм Dependency Injection любую реализацию для данного интерфейса.</p>
<p>Далее используем интерфейс в контроллере HomeController:</p>
<pre class="brush:c#;">
...
<p>В конструкторе контроллер получает нужную реализацию интерфейса IRepository. Для получения данных предусмотрен метод Index, который передает их в представление.</p>
<p>Отмечу, что данном случае нам можно не создавать реализацию интерфейса IRepository, устанавливать зависимости в классе Startup. Мы можем протестировать, имея лишь интерфейс репозитория.</p>

<p>Вначале добавим через NuGet в <span class="b">проект тестов</span> пакет <span class="b">Moq</span>:</p>
<img src="./pics/moq.png" alt="Добавление Moq в проект ASP.NET Core" />
<p>Теперь протестируем этот контроллер, определив следующий тест:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc;
...
<p>Moq предназначен для имитации объектов или для создания так называемых фейковых объектов. В данном случае имитируется функциональность репозитория. 
Для этого объект Mock типизируется соответствующим типом: <code>var mock = new Mock&lt;IRepository&gt;()</code></p>
<p>Затем выполняется настройка mock объекта с помощью метода <code>Setup</code>. Так как нам надо имитировать возвращение методом <code>GetAll()</code> 
набора объектов, то данный метод вызывается в методе Setup, а с помощью метода <code>Returns</code> определяем данный набор объектов. Набор объектов, который используется 
для теста, возвращается с помощью вспомогательного метода GetTestUsers.</p>
<p>Поскольку контроллер HomeController теперь в конструкторе принимает объект репозитория, то мы можем передать в конструктор мок-объект, который имитирует функциональность 
репозитория: <code>HomeController controller = new HomeController(mock.Object)</code></p>
<p>Для проверки работоспособности метода Index здесь проверяются три условия. Является ли возвращаемый результат объектом ViewResult:</p>
<pre class="brush:c#;">var viewResult = Assert.IsType&lt;ViewResult&gt;(result);</pre>
<p>Передается ли в представление в качестве модели объект IEnumerable&lt;User&gt;:</p>
<pre class="brush:c#;">var model = Assert.IsAssignableFrom&lt;IEnumerable&lt;User&gt;&gt;(viewResult.Model);</pre>
<p>И также проверяется количество объектов, которые передаются в представление:</p>
<pre class="brush:c#;">Assert.Equal(GetTestUsers().Count, model.Count());</pre>
<p>Запустим тест и убедимся, что он проходит:</p>
<img src="./pics/unittest8.png" alt="Moq в ASP.NET Core" />
...
<div class="nav"><p><a href="./22.3.php">Назад</a><a href="./">Содержание</a><a href="./22.5.php">Вперед</a></p></div></div>
<div class="socBlock">
