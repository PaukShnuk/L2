...
<html >
<head>
<title>ASP.NET Core | Создание ограничений для авторизации</title>
<meta charset="utf-8" />
<meta name="description" content="Создание ограничений для политики авторизации в ASP.NET Core MVC с помощью интерфейсов IAuthorizationHandler и IAuthorizationRequirement">
...
<h2>Создание ограничений для политики авторизации</h2><div class="date">Последнее обновление: 13.12.2019</div>
...
возможности. В частности, в прошлом проекте класс User имел свойство Year, указывающее на год рождения пользователя:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Что если мы хотим ограничить доступ в зависимости от возраста пользователя. Для этого нам надо создать собственное ограничение.</p>
<p>Возьмем проект из прошлой темы и добавим в него класс ограничения, который назовем AgeRequirement:</p>
<pre class="brush:c#;">
...
<p>Класс ограничения должен реализовать интерфейс <span class="b">IAuthorizationRequirement</span>. С помощью свойства Age устанавливается минимально допустимый возраст.</p>
<p>Сам класс ограничения только устанавливает некоторые лимиты, больше он ничего не делает. Чтобы его использовать при обработке запроса, нам надо 
добавить специальный класс - обработчик. Итак, добавим в проект новый класс AgeHandler:</p>
<pre class="brush:c#;">
using System;
...
<img src="./pics/claims12.png" alt="Restrictions for Claims Policy Authorization in ASP.NET Core MVC" />
<p>Далее изменим класс Startup, чтобы применить эти классы:</p>
<pre class="brush:c#;">
...
<p>Здесь надо отметить два момента. Во-первых, в методе <code>ConfigureServices()</code> происходит установка зависимости для IAuthorizationHandler:</p>
<pre class="brush:c#;">services.AddTransient&lt;IAuthorizationHandler, AgeHandler&gt;();</pre>
<p>Во-вторых, в коллекцию <code>Requirements</code> добавляется кастомное ограничение:</p>
<pre class="brush:c#;">opts.AddPolicy("AgeLimit", policy =&gt; policy.Requirements.Add(new AgeRequirement(18)));</pre>
<p>Теперь нам остается установить нужный объект Claim. Для этого можно изменить в контроллере AccountController метод Authenticate, который вызывается при регистрации:</p>
<pre class="brush:c#;">
...
<p>Полный код контроллера AccountController в итоге должен выглядеть так:</p>
<pre class="brush:c#;">
...
<p>И далее мы можем использовать созданную политику для ограничения доступа:</p>
<pre class="brush:c#;">
...
<p>Теперь к методу Index смогут обратиться только те, кто удовлетворяет ограничению AgeLimit 
(в данном случае кому исполнилось 18 лет), в то время как к методу About смогут обратиться все желающие.</p>
...
<div class="nav"><p><a href="./15.7.php">Назад</a><a href="./">Содержание</a><a href="./23.7.php">Вперед</a></p></div></div>
<div class="socBlock">
