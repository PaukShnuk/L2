...
<html >
<head>
<title>ASP.NET Core | Регистрация и создание пользователей в Identity</title>
<meta charset="utf-8" />
<meta name="description" content="Регистрация и создание пользователей в ASP.NET Core Identity, создание функционала по регистрации">
...
<h2>Регистрация и создание пользователей в Identity</h2><div class="date">Последнее обновление: 08.01.2021</div>
...
<p>Продолжим работу с проектом из прошлой темы и добавим в него функционал регистрации пользователей.</p>
<p>Пользователи в приложении представлены классом User, который наследует множество свойств. Однако нам необязательно всех их устанавливать. Достаточно 
установить ключевые свойства вроде логина и пароля. Для этого лучше воспользоваться вспомогательной моделью, 
которая установит все необходимые свойства.</p>
<p>Итак, добавим в проект новую папку, которую назовем <span class="b">ViewModels</span>. Затем в этой папке определим новый класс <span class="b">RegisterViewModel</span>, который будет 
представлять регистрирующегося пользователя:</p>
<pre class="brush:c#;">
...
<p>Для работы с учетными записями пользователей добавим в папку <span class="b">Controllers</span> новый контроллер <span class="b">AccountController</span> 
и определим в нем метод для регистрации пользователей:</p>
<pre class="brush:c#;">
...
<p>Поскольку в классе Startup были добавлены сервисы Identity, то здесь в контроллере через конструктор мы можем их получить. В данном случае мы получаем сервис 
по управлению пользователями - <code>UserManager</code> и сервис <code>SignInManager</code>, который позволяет аутентифицировать пользователя и устанавливать или удалять его куки.</p>
<p>С помощью метода <code>_userManager.CreateAsync</code> пользователь добавляется в базу данных. В качестве параметра передается сам пользователь и его пароль.</p>
<p>Данный метод возвращает объект <code>IdentityResult</code>, с помощью которого можно узнать успешность выполненной операции. Вполне возможно, что переданные значения 
не удовлетворяют требованиям, и тогда пользователь не будет добавлен в базу данных. В случае удачного добавления с помощью метода <code>_signInManager.SignInAsync()</code> 
...
указывающее, надо ли сохранять куки в течение продолжительного времени. И далее выполняем переадресацию на главную страницу приложения.</p>
<p>Если добавление прошло неудачно, то добавляем к состоянию модели с помощью метода <code>ModelState</code> все возникшие при добавлении ошибки, и отправленная модель возвращается в представление.</p>
<p>Для представлений этого контроллера в каталоге Views определим подкаталог <span class="b">Account</span>, в который добавим новое представление <span class="b">Register.cshtml</span>. 
Это представление будет служить для регистрации пользователя:</p>
<pre class="brush:xml;">
...
<p>В итоге вся структура проекта на данный момент будет выглядеть следующим образом:</p>
<img src="./pics/customidentity7.png" alt="Структура проекта в ASP.NET Identity Core" />
<p>Запустим приложение и обратимся к действию регистрации. Введем какие-нибудь данные:</p>
<img src="./pics/customidentity5.png" alt="Регистрация в ASP.NET Core Identity" />
<p>После удачной регистрации нас переадресует на главную страницу.</p>
<p>После добавленяи первого пользователя через окно <span class="b">SQL Server Objects Explorer</span> мы можем увидеть созданную базу данных:</p>
<img src="./pics/customidentity3.png" alt="Добавление базы данных в ASP.NET Identity Core" />
<p>А в самой базе данных в таблице <span class="b">dbo.AspNetUsers</span> мы сможем увидеть добавленного пользователя.</p>
<img src="./pics/customidentity6.png" alt="Добавление пользователя в ASP.NET Core Identity" />
...
<div class="nav"><p><a href="./16.2.php">Назад</a><a href="./">Содержание</a><a href="./16.4.php">Вперед</a></p></div></div>
<div class="socBlock">
