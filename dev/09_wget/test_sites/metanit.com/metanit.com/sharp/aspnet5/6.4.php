...
<html >
<head>
<title>ASP.NET Core | Передача зависимостей</title>
<meta charset="utf-8" />
<meta name="description" content="Передача зависимостей и Dependency Injection в ASP.NET Core, установка зависимостей через контроллеры и методы">
...
<h2>Передача зависимостей</h2><div class="date">Последнее обновление: 07.11.2019</div>
...
В ASP.NET Core мы можем получить добавленные в приложения сервисы различными способами;</p>
<ul>
<li><p>Через конструктор класса (за исключением конструктора класса Startup)</p></li>
<li><p>Через параметр метода Configure класса Startup</p></li>
...
<li><p>Через свойство ApplicationServices объекта IApplicationBuilder в классе Startup</p></li>
</ul>
<p>Для работы возьмем проект ASP.NET Core по типу Empty и определим в нем типы IMessageSender и EmailMessageSender:</p>
<pre class="brush:c#;">
...
<h3>Параметр метода Configure</h3>
<p>Метод Configure класса Startup позволяет напрямую получать зависимость в качестве параметра метода. Например:</p>
<pre class="brush:c#;">
...
<p>Данный способ естественно может применяться только в классе Startup.</p>
<h3>Конструкторы</h3>
<p>Встроенная в ASP.NET Core система внедрения зависимостей использует конструкторы классов для передачи всех зависимостей. Соответственно 
в конструкторе контроллера мы можем получить зависимость. Конструкторы являются наиболее предпочтительным вариантом для получения зависимостей.</p>
<p>Например, пусть в проекте определен класс MessageService:</p>
<pre class="brush:c#;">
...
<p>Здесь через конструктор класса передается зависимость от IMessageSender. Причем здесь неизвестно, что это будет за реализация интерфейса IMessageSender.</p>
<p>Изменим класс Startup для использования MessageService:</p>
<pre class="brush:c#;">
public class Startup
{
	public void ConfigureServices(IServiceCollection services)
...
<p>Чтобы использовать класс MessageService, он внедряется в приложение в виде сервиса. Поскольку это самодостаточная зависимость, которая представляет 
конкретный класс, то метод <code>services.AddTransient</code> типизируется одним этим типом MessageService. То есть классы, использующие сервисы, сами 
могут выступать в качестве сервисов.</p>
<p>Но так как класс MessageService использует зависимость IMessageSender, которая передается через конструктор, то нам надо также установить и эту зависимость:</p>
<pre class="brush:c#;">services.AddTransient&lt;IMessageSender, EmailMessageSender&gt;();</pre>
<p>И когда при обработке запроса будет использоваться класс MessageService, для создания объекта этого класса будет вызываться провайдер сервисов. 
Провайдер сервисов проверят конструктор класса MessageService на наличие зависимостей. Затем создает объекты для всех используемых 
зависимостей и передает их в конструктор.</p>
<p>В методе Configure сервис MessageService передается в качестве параметра и участвует в обработке запроса.</p>
<p>А результатом работы приложения будет вывод строки:</p>
<img src="./pics/di2.png" alt="Тестирование внедрения зависимостей в ASP.NET Core" />
<p>Данный способ может применяться для передачи зависимостей в конструктор любого класса кроме класса Startup.</p>
<h3>HttpContext.RequestServices</h3>
<p>Объект <span class="b">HttpContext.RequestServices</span> предоставляет доступ к всем внедренным зависимостям с помощью своих методов:</p>
<ul>
...
в рамках ASP.NET Core в принципе мы можем использовать подобную функциональность.</p>
<p>Например, изменим класс Startup:</p>
<pre class="brush:c#;">
public class Startup
{
	public void ConfigureServices(IServiceCollection services)
...
<h3>ApplicationServices</h3>
<p>Еще один похожий способ представляет получение сервисов через свойство <span class="b">ApplicationServices</span> объекта IApplicationBuilder, 
который передается в качестве параметра в метод Configure класса Startup:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app)
{
	app.Run(async (context) =&gt;
...
<p>Подобно тому, как зависимости передаются в метод Configure в классе Startup, точно также их можно передавать в метод Invoke компонента middleware. 
...
<p>Применим компонент для обработки запроса в классе Startup:</p>
<pre class="brush:c#;">
public class Startup
{
	public void ConfigureServices(IServiceCollection services)
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://www.youtube.com/watch?v=J2RKFhVQdrE"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Видеокурс по ASP.NET Core. 13. Получение сервисов и Dependency Injection</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="./2.19.php">Назад</a><a href="./">Содержание</a><a href="./6.2.php">Вперед</a></p></div></div>
<div class="socBlock">
