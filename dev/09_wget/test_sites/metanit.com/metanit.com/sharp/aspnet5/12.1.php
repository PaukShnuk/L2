...
<html >
<head>
<title>ASP.NET Core | Работа с данными в Entity Framework</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с базой данных MS SQL Server в ASP.NET Core через Entity Framework, создание и миграции бд">
...
<h1>Работа с данными в Entity Framework</h1>
<h2>Подключение и создание базы данных в Entity Framework Core</h2><div class="date">Последнее обновление: 09.12.2019</div>
...
нацелен в первую очередь на работу с СУБД MS SQL Server, однако поддерживает также и ряд других СУБД. В данном случае мы будем работать с базами данных в MS SQL Server.</p>
<p>Также стоит отметить, что здесь мы будем использовать <span class="b">Entity Framework Core</span> - кроссплатформенное решение на базе .NET Core, 
...
<a href="https://metanit.com/sharp/entityframeworkcore/">руководстве</a>.</p>
<p>Для работы с Entity Framework вначале создадим новый проект ASP.NET Core по шаблону ASP.NET Core Web App (Model-View-Controller). Пусть он будет называться <span class="b">EFDataApp</span>.</p>
<img src="./pics/efcore3.png" alt="Entity Framework Core в ASP.NET Core" />
...
<img src="./pics/efcore1.png" alt="Entity Framework Core в ASP.NET Core" />
<p>Далее добавим в проект класс модели, с которой мы будем работать. По умолчанию в проекте уже есть предназначенная для моделей папка Models. 
Определим в этой папке определим новый класс User:</p>
<pre class="brush:c#;">
public class User
{
...
<span class="b">Microsoft.EntityFrameworkCore.DbContext</span>. Поэтому добавим в папку  Models новый класс, который назовем <span class="b">ApplicationContext</span> (название класса контекста произвольное):</p>
<pre class="brush:c#;">
...
<p>И последним шагом в настройке проекта является изменение файла <span class="b">Startup.cs</span>. В нем нам надо изменить метод <code>ConfigureServices()</code>:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./19.6.php">Назад</a><a href="./">Содержание</a><a href="./12.2.php">Вперед</a></p></div></div>
<div class="socBlock">
