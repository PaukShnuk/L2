...
<html >
<head>
<title>ASP.NET Core | Работа с формами в MVC</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с элементами формам в ASP.NET MVC Core, отправка данных из формы на контроллер, элементы input, select">
...
<h2>Работа с формами</h2><div class="date">Последнее обновление: 19.11.2019</div>
...
<p>Формы представляют одну из форм передачи наборов данных на сервер. Как правило, для создания форм и их элементов в MVC применяются либо html-хелперы, либо 
tag-хелперы, которые рассматриваются далее. Однако в данном случае мы рассмотрим взаимодействие на примере стандартных тегов html, которые создают элементы формы.</p>
<p>В одной из прошлых тем было рассмотрено, как MVC связывает отправленные значения формы с параметрами метода. Например, у нас есть действие Login:</p>
<pre class="brush:c#;">
...
<p>Одно действие расщеплено на два метода: GET-версию, которая отдает представление с формой ввода, и POST-версию, которая принимает введенные в эту форму данные.</p>
<p>Теперь создадим само представление. Добавим в папку Views/Home новый файл <span class="ii">Login.cshtml</span>:</p>
<pre class="brush:xml;">
@{
...
<p>Чтобы инфраструктура MVC могла автоматически связать данные из формы с параметрами метода, значения атрибутов <span class="b">name</span> у полей формы 
совпадают с именами параметров.</p>
<p>Таким образом, когда форма отправится на сервер, при обработке запроса фреймворк MVC автоматически свяжет значения полей формы с параметрами:</p>
<img src="./pics/views9.png" alt="Отправка форм в ASP.NET Core" />
<img src="./pics/views10.png" alt="Формы в ASP.NET MVC Core" />
<p>В HTML мы можем использовать ряд встроенных элементов. Для ввода строк используется элементы <code>&lt;input type="text" /&gt;</code> (для однострочного текста) 
и <code>textarea</code> (для многострочного текста).</p>
<p>Для ввода парольной информации можно применять элемент  <code>&lt;input type="password" /&gt;</code></p>
<p>Для ввода чисел (которые соответствуют типа int, short, byte, long, float, double) используется элемент  <code>&lt;input type="number" /&gt;</code></p>
<p>Например, метод принимает ряд значений:</p>
<pre class="brush:c#;">
[HttpPost]
...
<p>Тогда форма могла бы выглядеть следующим образом:</p>
<pre class="brush:xml;">
&lt;form method="post"&gt;
    &lt;p&gt;
        &lt;label&gt;Логин:&lt;/label&gt;&lt;br /&gt;
...
<img src="./pics/views11.png" alt="Элементы html в ASP.NET Core" />
<h3>Элемент checkbox и тип bool</h3>
<p>Для передачи логических значений true или false удобно использовать элемент <code>&lt;input type="checkbox" /&gt;</code>. Например, пусть метод принимает 
объект типа <code>bool</code>:</p>
<pre class="brush:c#;">
...
<p>Тогда мы могли бы определить следующий элемент input:</p>
<pre class="brush:xml;">
&lt;label&gt;Женат/Замужем:&lt;/label&gt;&lt;br /&gt;
&lt;input type="checkbox" name="isMarried" value="true" /&gt;
</pre>
<p>Для связи с параметром метода элемент input называется по имени параметра - <code>isMarried</code>. И, кроме того, это поле имеет по умолчанию значение <code>true</code>. 
Это значение необязательно указывает, что параметр <code>isMarried</code> будет равен <code>true</code>.</p>
<h3>Радиокнопки</h3>
<p>Радиокнопки или переключатели позволяют выбрать один из нескольких вариантов. Для их создания используется элемент <code>&lt;input type="radio" /&gt;</code>:</p>
<pre class="brush:xml;">
&lt;form method="post"&gt;
...
<img src="./pics/views12.png" alt="rudiobuttons in asp.net core" />
<p>Все радиокнопки имеют одинаковое имя - color и отличаются только значением, которое хранится в атрибуте value. В контроллере мы могли бы получить выбранное 
значение в виде обычной строки:</p>
<pre class="brush:c#;">
...
<h3>Список select</h3>
<p>Элемент <code>select</code> предназначен для создания двух видов списков - выпадающих и развернутых.</p>
<p>В выпадающем списке мы можем выбрать один элемент:</p>
<pre class="brush:xml;">
&lt;form method="post"&gt;
...
<img src="./pics/views13.png" alt="Список select в ASP.NET Core" />
<p>При этом отображаемый текст элемента списка необязательно должен совпадать с его значением. Например:</p>
<pre class="brush:xml;">&lt;option value="iPhone 6S"&gt;Apple&lt;/option&gt;</pre>
<p>Здесь текст "Apple" - это то, что мы увидим в списке, а "iPhone 6S" - это значение, которое при выборе данного элемента отправится на сервер.</p>
<p>Чтобы получить выбранный в этом списке элемент, параметр метода контроллера должен называться также, как и элемент select:</p>
<pre class="brush:c#;">
...
<p>Для создания списка со множественным выбором к элементу select добавляется атрибут <code>multiple</code>:</p>
<pre class="brush:xml;">
&lt;form method="post" &gt;
...
<p>Но теперь, чтобы получить все выбранные значения в этом списке, метод контроллера должен в качестве параметра принимать массив или список:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./7.7.php">Назад</a><a href="./">Содержание</a><a href="./7.10.php">Вперед</a></p></div></div>
<div class="socBlock">
