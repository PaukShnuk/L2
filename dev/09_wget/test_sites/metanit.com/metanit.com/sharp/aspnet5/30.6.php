...
<html >
<head>
<title>ASP.NET Core | IHubContext в SignalR</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка сообщений из контроллеров в SignalR в ASP.NET Core, сервис IHubContext">
...
<h2>IHubContext</h2><div class="date">Последнее обновление: 25.12.2019</div>
...
<p>Класс хаба - не единственное место, где мы можем отправлять сообщения клиентам. SignalR позволяет это делать также 
в контроллерах, страницах Razor Pages, компонентах middleware, сервисах благодаря внедрению зависимости <span class="b">IHubContext</span>. Объект IHubContext реализует часть функциональности стандартного класса хаба, в частности, в нем определены такие же свойства 
<span class="b">Groups</span> и <span class="b">Clients</span>, которые позволяют манипулировать группами и отправлять сообщения всем подключенным клиентам.</p>
<p>Например, определим хаб ChatHub:</p>
<pre class="brush:c#;">
...
<p>Несмотря на то, что класс хаба пустой, он нам понадобится для типизиации объекта IHubContext, кроме того, взаимодействие с клиентами будет идти через данный хаб.</p>
<p>Далее добавим в классе Startup функциональность SignalR и MVC:</p>
<pre class="brush:c#;">
...
<p>Определим следующий контроллер HomeController:</p>
<pre class="brush:c#;">
using System;
...
<p>Благодаря встроенному механизму внедрения зависимостей мы можем получить объект IHubContext в конструкторе контроллера. Основное ограничение в данном случае - 
этот объект должен быть типизирован классом хаба.</p>
<p>Затем в методах контроллера мы можем использовать данный IHubContext, например, отправив сообщение подключенным клиентам. К примеру, в методе 
Create происходит условное добавление некоторого товара, и все подключенные клиенты получают уведомление </p>
<p>Для тестирования определим представление <span class="b">Index.cshtml</span>:</p>
<pre class="brush:xml;">
...
<p>В представлении определена форма для отправки данных на метод Create и блок для вывода сообщений, получаемых от хаба. Причем стоит отметить, что 
подключение идет по адресу "/chat" - то есть фактически к хабу ChatHub. При получении уведомлений от хаба будет вызываться функция notify.</p>
<p>И после отправки формы все остальные подключенные клиенты увидят информацию об отправленных данных.</p>
<img src="./pics/signalr8.png" alt="HubContext в SignalR в ASP.NET Core" />
<p>Подобным образом можно получать сервис IHubContext в других сервисах, компонентах middleware и страницах Razor Pages.</p>
<p>Однако стоит отметить, что данный способ отправки сообщений клиентам имее некоторые недостатки. Например, у свойства Clients доступно только свойство All - 
...
пересылать тем или иным образом, например, через куки или через параметры запроса.</p>
<p>К примеру, определим следующий контроллер:</p>
<pre class="brush:c#;">
using System;
using System.Threading.Tasks;
using Microsoft.AspNetCore.Mvc;
...
<p>Теперь метод Create также получае id подключение - параметр connectionId и посылает клиентам в зависимости от id подключения разные сообщения.</p>
<p>Также изменим представление:</p>
<pre class="brush:xml;">
@{
    Layout = null;
}
...
<p>Теперь страница отправляет ajax-запросы к методу Create контроллера Home. После подключения к хабу через функцию 
<code>hubConnection.start()</code> получаем id подключения. И затем при отправке включаем его в тело запроса.</p>
<img src="./pics/signalr23.png" alt="SignalR и HubContext в ASP.NET Core MVC" />
...
<div class="nav"><p><a href="./30.5.php">Назад</a><a href="./">Содержание</a><a href="./30.7.php">Вперед</a></p></div></div>
<div class="socBlock">
