...
<html >
<head>
<title>ASP.NET Core | Обработчики страницы Razor Pages</title>
<meta charset="utf-8" />
<meta name="description" content="Обработчики страницы Razor Pages в ASP.NET Core, обработка одной страницей разных запросов">
...
<h2>Обработчики страницы Razor Pages</h2><div class="date">Последнее обновление: 05.12.2019</div>
...
<p>Для обработки запроса каждая страница может использовать методы OnGet/OnPost/OnPut и так далее, то есть такие методы, которые предназначены для определенного вида запросов. 
...
В этом случае мы можем добавить в код страницы дополнительные обработчики запросов.</p>
<p>Например, пусть в проекте определен класс Person, который представляет данные:</p>
<pre class="brush:c#;">
public class Person
{
...
<p>И пусть в проекте есть страница Razor <span class="ii">Person.cshtml</span> и связанный с ней файл кода <span class="ii">Person.cshtml.cs</span>.</p>
<img src="./pics/razorpages12.png" alt="BindProperty в Razor Pages" />
<p>Пусть <span class="ii">Person.cshtml.cs</span> содержит следующее определение класса PersonModel:</p>
<pre class="brush:c#;">
...
<p>Класс содержит некоторые начальные данные в переменной people - список объектов Person. При стандартном get-запросе в свойство DisplayedPeople передается весь 
этот список.</p>
<p>И также определены два метода - обработчики для фильтрации списка по имени и возрасту объекта Person. Так как оба этих обработчики будут также обрабатывать запросы get, 
то их название начинается с префикса <span class="b">OnGet</span>.</p>
<p>На странице <span class="ii">Person.cshtml</span> выводится переданный из PersonModel список:</p>
<pre class="brush:xml;">
@page
...
<p>И при обычном запросе к странице сработает метод OnGet, поэтому на страницу будет выведен полный список:</p>
<img src="./pics/razorpages15.png" alt="Обработчики в Razor Pages в ASP.NET Core" />
<p>Теперь обратимся к конкретному обработчику. Для этого в строку запроса передается параметр "handler" с указанием имени обработчика. 
Причем именем обработчика считается та часть названия метода, которая идет после префикса <span class="ii">OnGet</span>:</p>
<img src="./pics/razorpages16.png" alt="Handlers в Razor Pages в ASP.NET Core" />
<p>В данном случае обращение идет к обработчику OnGetByName. Поскольку он принимает параметр name, то через строку запроса для этого параметра передается значение: 
<span class="ii">http://localhost:44306/Person?handler=ByName&name=Tom</span>.</p>
<h3>Передача обработчика через параметр маршрута</h3>
<p>Не всегда может устроить добавление названия обработчика через строку запроса, наподобие <span class="ii">?handler=ByName</span>. В этом случае можно 
передать название обработчика через параметр маршрута. Для этого изменим код страницы Person.cshtml:</p>
<pre class="brush:xml;">
@page "{handler?}"
...
<p>Вопросительный знак после названия параметра "{handler?}" указывает, что данный параметр является необязательным. И в этом случае можно просто указать в url 
последним сегментом название обработчика:</p>
<img src="./pics/razorpages17.png" alt="Передача handler через параметр маршрута в Razor Pages" />
<h3>Обработка форм и запросов POST</h3>
<p>Подобным образом мы можем использовать обработчики для для обработки запросов POST - посылать данные по адесам, которые отличаются по параметру handler и прочим параметрам строки запроса. 
Но кроме того, мы можем задавать явным образом нужный обработчик с помощью параметра <span class="b">asp-page-handler</span>. Например, изменим код PersonModel:</p>
<pre class="brush:c#;">
using System.Collections.Generic;
using System.Linq;
using Microsoft.AspNetCore.Mvc.RazorPages;
...
<p>Изменим код страницы Person.cshtml:</p>
<pre class="brush:xml;">
@page "{handler?}"
...
<img src="./pics/razorpages18.png" alt="POST handlers in Razor Pages in ASP.NET Core" />
...
<div class="nav"><p><a href="./29.5.php">Назад</a><a href="./">Содержание</a><a href="./29.7.php">Вперед</a></p></div></div>
<div class="socBlock">
