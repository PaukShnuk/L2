...
<html >
<head>
<title>ASP.NET Core | Content negotiation</title>
<meta charset="utf-8" />
<meta name="description" content="Content negotiation в Web API в ASP.NET Core MVC, согласование типа контента, отправка xml вместо json">
...
<h2>Content negotiation</h2><div class="date">Последнее обновление: 15.03.2017</div>
...
<p>Когда метод контроллера возвращает ответ, то инфраструктура MVC определяет, в каком формате этот ответ лучше отправить клиенту. 
Формат контента зависит от ряда факторов: какой формат принимает клиент, какой формат может генерировать MVC, политика форматирования, 
возвращаемый методом тип.</p>
<p>Иногда метод контроллера возвращает результат в конкретном формате. Например, с помощью метода <code>Content</code> мы можем возвратить объект string, 
а с помощью метода <code>Json()</code> можно возвратить объект, сериализованый в формат json. Однако контроллеры в Web API для возвращения результат а используют самые различные методы, а не только Json и Content. Например:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>Первый метод <code>Get</code> возвращает коллекция элементов <code>IEnumerable&lt;User&gt;</code>. Однако здесь следует правило, если метод возвращает 
стандартный POCO-объект, то на самом деле неявно возвращается объект <span class="b">ObjectResult</span>, в который оборачивается POCO-объект. 
То есть в случае с методом Get мы могли бы написать:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>Когда же мы передаем в метод, который отправляет определенный статусный код, например, в метод <code>Ok()</code>, то он также возвращает объект 
класса ObjectResult, точнее один из наследников этого класса. Например, выражение <code>return Ok(user);</code> возвращает объект OkObjectResult.</p>
<p>Использование типа ObjectResult во всех случаях имеет важное значение, так как он реализует такую функциональность, как <span class="b">Content negotiation</span>.</p>
<p>Content negotiation предполагает процесс согласования между сервером и клиентом по поводу типа контента, который отправляется клиенту.</p>
<p>Если метод возвращает ответ в виде строки, то есть объекта <code>string</code>, эта строка отправляется клиенту как есть, а для 
заголовка <code>Content-Type</code> устанавливается значение <code>text/plain</code>. Данные простейших типов, как int или DateTime, при оправке также форматируются в строку.</p>
<p>А для объектов классов отправляемые данные в ObjectResult по умолчанию форматируются в формат JSON, а для заголовка <code>Content-Type</code> устанавливается значение 
<code>application/json</code>.</p>
<h3>Заголовок Accept</h3>
<p>Большинство браузеров включают в запрос заголовок <code>Accept</code>, который указывает набор форматов, предпочтительных для получения ответа. 
В частности, браузер Google Chrome в запросе отправляет следующий заголовок Accept:</p>
<pre class="brush:js;">
Accept: text/html,application/xhtml + xml,application/xml;q = 0.9,image/webp,*/*;q = 0.8
</pre>
<p>Этот заголовок указывает, что предпочтительными форматами для браузера являются HTML, XHTML, XML и WEBP (для изображений). 
...
<code>1.0</code>.А, к примеру, значение <code>q = 0.9</code> для формата <code>application/xml</code> указывает, что браузер принимает данные в XML, но 
предпочитает форматы HTML и XHTML (для которых по умолчанию вес 1.0). Две звездочки в конце <code>*/*</code> говорят о том, что браузер может принимать 
любой формат, но в соответствии с его весом <code>q = 0.8</code> он будет менее предпочтительным по сравнению с ранее указанными форматами.</p>
<p>То есть данным заголовком Google Chrome говорит, что он предпочитает данные в формате HTML или XHTML, а изображения - в формате WEBP. Если форматы HTML и XHTML недоступны, 
то можно отправить данные в XML. Если же ни один из этих форматов недоступен для отправки ответа, тогда браузер может принять ответ в любом формате.</p>
<h3>Форматирование ответа в xml</h3>
<p>Добавим через NuGet в проект пакет <span class="b">Microsoft.AspNetCore.Mvc.Formatters.Xml</span>:</p>
<img src="./pics/webapi22.png" alt="Microsoft.AspNetCore.Mvc.Formatters.Xml" />
<p>Добавим форматировщик XmlSerializerFormatters к сервисам MVC в классе Startup.cs:</p>
<pre class="brush:c#;">
...
<p>В качестве альтернативы можно также использовать другой способ:</p>
<pre class="brush:c#;">
...
<p>Вне зависимости от того, какой из этих двух способов будет применяться, для сериализации ответа будет использоваться класс 
<span class="b">System.Runtime.Serialization.DataContractSerializer</span>.</p>
<p>В качестве альтернативы для сериализации данных в xml можно использовать класс <span class="b">System.Xml.Serialization.XmlSerializer</span>:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
...
<p>В данном случае разница между <code>XmlDataContractSerializerOutputFormatter</code> и <code>XmlSerializerOutputFormatter</code> будет небольшая 
за тем исключением, что XmlSerializerOutputFormatter на клиентские приложения на .NET, которые используют старые версии фреймворка при работе с XML.</p>
<p>В этом случае, если клиент будет отправлять в запросе, например, к методу:</p>
<pre class="brush:c#;">
[HttpGet]
public IEnumerable&lt;User&gt; Get()
{
...
<p>следующий заголовок</p>
<pre class="brush:js;">
Accept: application/xml
</pre>
<p>То сервер будет отправлять данные в формате xml.</p>
<p>Например, обратимся из Fiddlera к методу Get, отправив в запросе заголовок "Accept: application/xml":</p>
<img src="./pics/webapi19.png" alt="Content Negotiation в ASP.NET Core Web API" />
<p>Если бы мы не указали заголовок Accept, то данные по прежнему передавались бы в формате json.</p>
<h3>Атрибут Produces</h3>
<p>Мы можем переопределить систему согласования типа контента с помощью атрибута <span class="b">Produces</span>. 
Этот атрибут выступает своего рода фильтром, который изменяет тип контента для объекта ObjectResult. В качестве значения в этот атрибут передается тип 
содержимого:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>В данном случае, даже если приложение использует сериализацию ответа в формат XML, а клиент в запросе указывает заголовок "Accept: application/xml", 
данные все равно будут отправляться в формате json.</p>
<img src="./pics/webapi18.png" alt="Атрибут Produce в ASP.NET Core Web API" />
<h3>Получение типа контента из строки запроса</h3>
<p>Заголовок Accept - не единственный способ указать серверу, в каком формате надо отправлять данные клиенту. Еще один способ представляет использование 
строки запроса. Так, изменим в классе Startup добавление сервисов MVC:</p>
<pre class="brush:c#;">
...
<p>В методе <code>AddMvcOptions()</code> устанавливается сопоставление формата из строки запроса с определенным медиа-типом:</p>
<pre class="brush:c#;">opts.FormatterMappings.SetMediaTypeMappingForFormat("xml", new MediaTypeHeaderValue("application/xml"));</pre>
<p>Теперь к методу контроллера надо применить атрибут <span class="b">FormatFilter</span>:</p>
<pre class="brush:c#;">
[HttpGet]
[FormatFilter]
public IEnumerable&lt;User&gt; Get()
...
<p>При запросе мы можем указать формат xml, передав в строке запроса параметр <code>format=xml</code>:</p>
<img src="./pics/webapi20.png" alt="Формат xml в ASP.NET Core Web API" />
<p>Атрибут <code>FormatFilter</code> может применяться как к отдельным методам, так и ко всему контроллеру.</p>
<p>При этом серверу уже не важно, какой формат контента передается в запросе в заголовке Accept - он все равно не будет учитываться.</p>
...
<div class="nav"><p><a href="./23.5.php">Назад</a><a href="./">Содержание</a><a href="./18.1.php">Вперед</a></p></div></div>
<div class="socBlock">
