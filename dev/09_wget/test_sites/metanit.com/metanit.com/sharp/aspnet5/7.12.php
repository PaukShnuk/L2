...
<html >
<head>
<title>ASP.NET Core | Генерация контента в View Component</title>
<meta charset="utf-8" />
<meta name="description" content="Генерация контента в View Component в ASP.NET Core MVC, создание html-фрагментов с помощью класса HtmlContentViewComponentResult">
...
<h2>Генерация контента в View Component</h2><div class="date">Последнее обновление: 28.11.2019</div>
...
генерации результата не всегда является удобным, и как правило в качестве результата в View Component используется объект интерфейса <span class="b">IViewComponentResult</span>. 
Имеется три встроенных класса, которые реализуют этот интерфейс:</p>
<ul>
<li><p><span class="b">ViewViewComponentResult</span>: используется для генерации представления Razor с возможностью передачи модели. Для создания 
...
<p>Для возврата содержимого в виде обычной строки используется объект <span class="b">ContentViewComponentResult</span>. Для его создания применяется метод <code>Content()</code>, который в качестве параметра принимает строку.</p>
...
так как фреймворк MVC автоматически преобразовывает строку в ContentViewComponentResult. Например:</p>
<pre class="brush:c#;">
using System;
...
<p>Стоит отметить, что в подобном случае необходимо наследовать класс компонента от ViewComponent, иначе мы не сможем воспользоваться методом Content.</p>
...
<img src="./pics/viewcomponent6.png" alt="IViewComponentResult в ASP.NET Core" />
...
<img src="./pics/viewcomponent7.png" alt="HtmlContentViewComponentResult в ASP.NET Core" />
...
разметки html, то в этом случае лучше использовать ViewViewComponentResult, который будет рассмотрен в следующей теме.</p>
