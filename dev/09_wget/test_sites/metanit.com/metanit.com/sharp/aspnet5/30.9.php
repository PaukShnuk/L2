...
<html >
<head>
<title>ASP.NET Core | Аутентификация и авторизация и токены в SignalR</title>
<meta charset="utf-8" />
<meta name="description" content="Аутентификация и авторизация с помощью токенов в хабах и клиентах в SignalR в ASP.NET Core">
...
<h2>Аутентификация и авторизация с помощью токенов</h2><div class="date">Последнее обновление: 28.12.2019</div>
...
<p>Использование токена при аутентификации и авторизации в SignalR будет несколько отличаться от использования кук. Частности, при установке соединения в SignalR в клиенте javascript необходимо дополнительно передавать токен:</p>
<pre class="brush:js;">
...
<p>В качестве второго параметра в метод <code>withUrl</code> передается объект, у которого параметр <span class="b">accessTokenFactory</span> 
...
могут быть и более сложные.</p> 
<p>Например, пусть в проекте будут следующие классы контекста данных, пользователя и роли:</p>
<pre class="brush:c#;">
...
<p>Для хранения настроек токена пусть в проекте будет определен вспомогательный класс AuthOptions:</p>
<pre class="brush:c#;">
...
<p>Для аутентификации пусть будет определен контроллер AccountController:</p>
<pre class="brush:c#;">
...
<p>Метод Token будет получать логин и пароль и возвращать токен и имя пользователя.</p>
<p>Пусть у нас будет следующий хаб ChatHub:</p>
<pre class="brush:c#;">
...
<p>Поскольку к классу применяется атрибут Authorize, то доступ к хабу установлен только для аутентифицировнных пользователей.</p>
<p>Для тестирования возьмем следующую веб-страницу index.html:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Изначально токен не установлен, он будет установлен только после прохождении аутентификации на сервере. 
По нажатию на кнопку loginBtn отправляется запрос, который должен вернуть токен и имя пользователя. И только после получения токена запускатся соединение:</p>
<pre class="brush:js;">hubConnection.start()</pre>
<p>В итоге проект может выглядеть следующим образом:</p>
<img src="./pics/signalr25.png" alt="Token-based authentication and authorization in SignalR ASP.NET Core" />
<p>SignalR на стороне клиент не добавляет токен в заголовки запроса. И при обращении к хабу токен фактически будет посылаться как параметр строки запроса. И в этом случае нам необходима дополнительная конфигурация 
в классе Startup:</p>
<pre class="brush:c#;">
using System.Threading.Tasks;
using AuthSignalRApp.Models;
...
<p>В отличие, скажем, от стандартной работы с токеном в webapi здесь нам еще надо получить токен из строки запроса, если запрос обращен к хабу:</p>
<pre class="brush:c#;">
options.Events = new JwtBearerEvents
{
	OnMessageReceived = context =&gt;
...
<img src="./pics/signalr15.png" alt="Токен авторизации в SignalR в ASP.NET Core" />
...
<div class="nav"><p><a href="./30.8.php">Назад</a><a href="./">Содержание</a><a href="./30.10.php">Вперед</a></p></div></div>
<div class="socBlock">
