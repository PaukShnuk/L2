...
<html >
<head>
<title>ASP.NET Core | Создание и конфигурация хабов SignalR</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и конфигурация хабов SignalR в ASP.NET Core, методы AddSignalR, MapHub, MapHub">
...
<h2>Создание и конфигурация хабов</h2><div class="date">Последнее обновление: 24.12.2019</div>
...
<p>Ключевой сущностью в SignalR, через которую клиенты обмениваются сообщеними с сервером и между собой, является <span class="b">хаб</span> (hub), 
который представляет некоторый класс, унаследованный от абстрактного класса <span class="b">Microsoft.AspNetCore.SignalR.Hub</span>.</p>
<p>Простейший хаб:</p>
<pre class="brush:c#;">
...
<p>Класс хаба может определять различные методы с различными параметрами, однако клиент может обращаться только к публичным методам хаба. 
Например, в данном случае определен публичный метод <code>Send</code>, который принимает параметр типа string. А это значит, 
...
метод просто транслирует полученную строку всех подключенным клиентам.</p>
<h3>AddSignalR()</h3>
<p>Сам по себе SignalR не будет работать, чтобы хаб был доступен для использования, в методе <span class="b">ConfigureServices()</span> класса 
<span class="b">Startup</span> вызывает метод <code>services.AddSignalR()</code>:</p>
<pre class="brush:c#;">
...
<p>Перегузка этого метода принимает в качестве параметра делегат Action, который имеет параметр типа <span class="b">HubOptions</span>. Данный параметр 
позволяет сконфигурировать подключение хаба. В частности, класс HubOptions определяет ряд свойств:</p>
<ul>
<li><p><span class="b">ClientTimeoutInterval</span>. Определяет время, в течение которого клиент должен отправить серверу сообщение. 
Если в течение данного времени никаких сообщенй от клиента на сервер не пришло, то сервер закрывает соединение.</p>
<p>По умолчанию равно 30 секунд.</p></li>
<li><p><span class="b">HandshakeTimeout</span>. После подключения к серверу клиент должен отправить серверу в качестве самого первого сообщения специальное сообщение - HandshakeRequest. 
...
Если в течение этого периода клиент не отправит первое сообщение, то подключение закрывается.</p>
<p>По умолчанию равно 15 секунд.</p></li>
<li><p><span class="b">KeepAliveInterval</span>: если в течение этого периода сервер не отправит никаких сообшений, то  
...
изменить на стророне клиента параметр serverTimeoutInMilliseconds (клиент javascript)/ ServerTimeout(клиент .NET), которое рекомендуется устанавливать в два раза больше, чем KeepAliveInterval.</p>
<p>По умолчанию равно 15 секунд.</p></li>

<li><p><span class="b">SupportedProtocols</span> определяет поддерживаемые протоколы. По умолчанию поддерживаются все протоколы.</p></li>
<li><p><span class="b">EnableDetailedErrors</span> при значении <span class="b">true</span> возвращает клиенту детальное 
...
информацию, то по умолчанию имеет значение <span class="b">false</span>.</p></li>
</ul>
<p>Мы можем настроить параметры глобально для всех хабов в приложении, либо для хабов отдельного типа. Глобальная 
настройка хабов:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
...
<p>Настройка только для хаба ChatHub:</p>
<pre class="brush:c#;">
...
<h3>MapHub</h3>
<p>Чтобы сервер мог сопоставить запросы с определенным хабом, в методе <span class="b">Configure()</span> класса Startup 
для сопоставления определенного маршрута с определенным хабом вызывается метод <span class="b">MapHub()</span>:</p>
<pre class="brush:c#;">
...
<p>С помощью метода <span class="b">MapHub()</span> можно сопоставить URL с хабом. В данном случае 
все запросы по адресу "/chat" будут обрабатываться хабом ChatHub.</p>
<p>Перегрузка метода MapHub дополнительно принимает делегат с параметром типа 
<span class="b">HttpConnectionDispatcherOptions</span>, благодаря чему мы можем настроить различные настройки подключения. 
В частности, в HttpConnectionDispatcherOptions есть следующие свойства:</p>
<ul>
<li><p><span class="b">ApplicationMaxBufferSize</span>: максимальный размер буфера в байтах, в который сервер помещает 
получаемые от клиента данные.</p>
<p>По умолчанию равно 32 килобайта.</p></li>
<li><p><span class="b">AuthorizationData</span>: представляет список (объект IList) объектов IAuthorizeData, которые определяют, 
авторизован ли клиент для подключения к хабу.</p></li>

<li><p><span class="b">TransportMaxBufferSize</span> максимальный размер буфера в байтах, в который сервер помещает 
данные для отправки клиенту.</p>
<p>По умолчанию равно 32 килобайта.</p></li>

<li><p><span class="b">Transports</span> представляет битовую маску из значений перечисления <span class="b">HttpTransportType</span>, 
которая устанавливает допустимые типы транспорта. По умолчанию применяются все типы транспорта</p></li>

<li><p><span class="b">LongPolling</span> представляет объект <span class="b">LongPollingOptions</span>, 
который настраивает транспорт LongPolling. Этот класс имеет только одно свойство <span class="b">PollTimeout</span>, 
которое устанавливает периодичность опроса</p></li>
<li><p><span class="b">WebSockets</span> представляет объект <span class="b">Microsoft.AspNetCore.Http.Connections.WebSocketOptions</span>, 
который настраивает транспорт WebSocket.</p></li>
</ul>
<p>Применим некоторые свойства:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Builder;
using Microsoft.Extensions.DependencyInjection;
...
<div class="nav"><p><a href="./30.1.php">Назад</a><a href="./">Содержание</a><a href="./30.3.php">Вперед</a></p></div></div>
<div class="socBlock">
