...
<html >
<head>
<title>ASP.NET Core | Инициализация БД ролями и пользователями</title>
<meta charset="utf-8" />
<meta name="description" content="Создание инициализатора для доабвления ролей и пользователей по умолчанию в базу данных в ASP.NET Core Identity">
...
<h2>Инициализация БД ролями и пользователями</h2><div class="date">Последнее обновление: 15.12.2019</div>
...
<p>Нередко возникает необходимость, чтобы база данных к моменту первого обращения приложения уже содержала начальные 
данные, например, базовые роли и учетную запись администратора.</p>
<p>Для инициализации базы данных  начальными ролями и пользователями определим новый класс RoleInitializer:</p>
<pre class="brush:c#;">
...
<p>В классе определен метод <code>InitializeAsync()</code>, который добавляет в базу данных две роли - "admin" и "user", а также одного пользователя - 
администратора. Для добавления здесь применяются методы классов UserManager и RoleManager, которые нам доступны через коллекцию сервисов приложения.</p>
<p>Для краткости все данные (логин и пароль администратора) здесь определены в коде, но естественно можно указать значения и в файле конфигурации и затем передавать в 
метод.</p>
<p>Используем данных класс в классе Program при старте приложения:</p>
<pre class="brush:c#;">
using System;
using System.Threading.Tasks;
...
<p>Здесь с помощью метода <code>services.GetRequiredService()</code> получаем сервисы UserManager и RoleManager и передаем их в метод RoleInitializer.InitializeAsync. 
...
начиная с которой и была добавлена поддержка асинхронных методов Main.</p>
<p>В итоге при первом запуске в базу данных будут добавлены все необходимые данные, и мы сможем зайти из под администратора, используя указанные в коде логин и пароль.</p>
...
<div class="nav"><p><a href="./13.6.php">Назад</a><a href="./">Содержание</a><a href="./13.6.php">Вперед</a></p></div></div>
<div class="socBlock">
