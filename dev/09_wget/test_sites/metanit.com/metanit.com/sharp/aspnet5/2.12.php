...
<html >
<head>
<title>ASP.NET Core | Файловые провайдеры конфигурации</title>
<meta charset="utf-8" />
<meta name="description" content="Файловые провайдеры конфигурации в ASP.NET Core, хранение настроек в файлах json, ini и xml, appsettings.json">
...
<h2>Файловые провайдеры конфигурации</h2><div class="date">Последнее обновление: 14.11.2019</div>
...
<h3>Конфигурация в JSON</h3>
<p>Зачастую для хранения конфигурации в ASP.NET Core используются файлы json. Для работы с файлами json применяется провайдер <span class="b">JsonConfigurationProvider</span>, 
а для загрузки конфигурации из json для класса ConfigurationBuilder доступен метод расширения <span class="b">AddJsonFile()</span>.</p>
<p>По умолчанию в проекте уже есть файл конфигурации json - <span class="ii">appsettings.json</span>, а также <span class="ii">appsettings.Development.json</span>, которые загружаются 
по умолчанию в приложении и которые мы можем использовать для хранения конфигурационных настроек. Например, код файла appsettings.json:</p>
<pre class="brush:js;">
{
  "Logging": {
    "LogLevel": {
...
<p>Здесь определяются настройки логгирования (элемент "Logging") и разрешенные хосты (элемент "AllowedHosts"). Одни элементы могут иметь вложенные элементы. 
Аналогичным образом можно задать другие необходимые настройки или удалить ранее определенные.</p>
<p>Однако в данном случае для примера возьмем новый файл json. Итак, добавим в проект новый файл <span class="ii">conf.json</span>:</p>
<img src="./pics/2.9.png" alt="Конфигурация json в ASP.NET Core" />
<p>И определим в нем следующее содержимое:</p>
<pre class="brush:js;">
{
  "color": "red",
  "text": "welcome!"
...
<p>Здесь задается два элемента с ключами color и text. Затем используем этот файл в классе Startup:</p>
<pre class="brush:c#;">
...
<p>Для установки конфигурации из json-файла нам надо передать его название в метод <code>builder.AddJsonFile()</code>. Затем в методе Configure настройки конфигурации используются для создания html-разметки, которая отправляется в ответ клиенту.</p>
<p>При определении настроек в файле json нам надо учитывать, что они должны иметь уникальные ключи. Но при этом мы можем использовать для конфигурации 
более чем одного файла:</p>
<pre class="brush:c#;">
var builder = new ConfigurationBuilder()
				.AddJsonFile("conf.json")
				.AddJsonFile("myconfig.json");
</pre>
<p>И если во втором файле есть настройки, которые имеют тот же ключ, что и настройки первого файла, то происходит переопределение настроек: 
настройки из второго файла заменяют настройки первого.</p>
<p>Но json может хранить также более сложные по составу объекты, например:</p>
<pre class="brush:js;">
{
  "color": "red",
...
<p>И чтобы обратиться к этой настройке, нам надо использовать знак двоеточия для обращения к иерархии настроек:</p>
<pre class="brush:c#;">
string text = AppConfiguration["namespace:class:method"];
</pre>
<h3>Конфигурация в XML</h3>
<p>За использование конфигурации в XML-файле отвечает провайдер <span class="b">XmlConfigurationProvider</span>. Для загрузки xml-файла 
применяется метод расширения <span class="b">AddXmlFile()</span>.</p>
<p>Итак, добавим в проект новый xml-файл, который назовем <span class="ii">config.xml</span>. Затем изменим его код на следующий:</p>
<pre class="brush:xml;">
...
<p>Здесь определены два элемента color и text, которые представляют конфигурационные настройки.</p>
<p>Обратите внимание, что у файла xml в свойствах должно быть выставлено копирование при компиляции в выходную папку приложения:</p>
<img src="./pics/config6.png" alt="XML configuration in ASP.NET Core" />
<p>Теперь используем этот файл:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Builder;
using Microsoft.AspNetCore.Hosting;
using Microsoft.AspNetCore.Http;
...
<p>Если у нас файл конфигурации имеет разные уровни вложенности, например:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8" ?&gt;
&lt;configuration&gt;
  &lt;color&gt;blue&lt;/color&gt;
...
<p>то мы можем обращаться к этим уровням также, как и в файле json. Например, получение текста из элемента text-&gt;description-&gt;data:</p>
<pre class="brush:c#;">var text = AppConfiguration["text:description:data"];</pre>
<h3>Конфигурация в ini-файлах</h3>
<p>Для работы с конфигурацией INI применяется провайдер <span class="b">IniConfigurationProvider</span>. А для загрузки конфигурации из INI-файла нам надо использовать метод расширения <span class="b">AddIniFile()</span>.</p>
<p>Например, добавим в проект текстовый файл и переименуем его в <span class="b">conf.ini</span>. Определим в нем следующее содержимое:</p>
<pre class="brush:js;">
color="blue"
text="Hello ASP.NET Core 2.0"
</pre>
<p>Как и в случае с xml-файлом необходимо указать копирование в целевой каталог приложения.</p>
<p>Подключим этот файл в классе Startup:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Builder;
using Microsoft.AspNetCore.Hosting;
using Microsoft.AspNetCore.Http;
...
<div class="nav"><p><a href="./2.16.php">Назад</a><a href="./">Содержание</a><a href="./2.23.php">Вперед</a></p></div></div>
<div class="socBlock">
