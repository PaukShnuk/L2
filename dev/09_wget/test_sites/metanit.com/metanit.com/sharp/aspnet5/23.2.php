...
<html >
<head>
<title>ASP.NET Core | Создание контроллера Web API</title>
<meta charset="utf-8" />
<meta name="description" content="Создание контроллера Web API в ASP.NET Core MVC, архитектура REST, методы протокола HTTP и их обработка в контроллере">
...
<h2>Создание контроллера</h2><div class="date">Последнее обновление: 12.12.2019</div>
...
<p>Создадим простейшее приложение на Web API, которое будет выполнять все основные операции с данными. Для этого создадим 
проект по типу Empty:</p>
<img src="./pics/webapi5.png" alt="Первое приложение ASP.NET Core Web API" />
<p>Далее добавим в проект новую папку <span class="b">Models</span>, а в нее поместим новый класс User:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Для взаимодействия с MS SQL Server через Entity Framework через пакетный менеджер Nuget добавим в проект пакет <span class="b">Microsoft.EntityFrameworkCore.SqlServer</span>.</p>

<p>Также добавим в папку Models новый класс <span class="b">UsersContext</span> для взаимодействия с базой данных:</p>
<pre class="brush:c#;">
...
<p>Далее добавим в проект новую папку <span class="b">Controllers</span>, а в ней создадим новый api-контроллер. 
Для этого при добавлении нового элемента в проект можно использовать шаблон <span class="b">API Controller Class</span>:</p>
<img src="./pics/webapi6.png" alt="Добавление контроллера в ASP.NET Core Web API" />
<p>Назовем новый элемент <span class="b">UsersController</span>.</p>
<p>После его создания изменим его код следующим образом:</p>
<pre class="brush:c#;">
...
<p>Прежде всего к контроллеру применяется атрибут <span class="b">[ApiController]</span>, который позволяет использовать ряд дополнительных возможностей, в частности, в плане привязки модели и ряд других. 
Также к контроллеру применяется атрибут маршрутизации, который указывает, как контроллер будет сопоставляться с запросами.</p>
<p>В конструкторе контроллера получаем контекст данных и используем его для операций с данными. Также в конструкторе контроллера добавляем ряд начальных данных.</p>
<p>Контроллер API предназначен преимущественно для обработки запросов протокола HTTP: Get, Post, Put, Delete, Patch, Head, Options. В данном случае для каждого типа запросов в контроллере определен свои методы. Так, метод <code>Get()</code> обрабатывает запросы типа GET и возвращает 
коллекцию объектов из бд.</p>
<p>Если запрос Get содержит параметр id (идентификатор объекта), то он обрабатывается другим методом - <code>Get(int id)</code>, 
который возвращает объект по переданному id.</p>
<p>Запросы типа Post обрабатываются методом <code>Post(User user)</code>, который получает из тела запроса отправленные данные и добавляет их в базу данных.</p>
<p>Метод <code>Put(User user)</code> обрабатывает запросы типа Put - получает данные из запроса и изменяет ими объект в базе данных.</p>
<p>И метод <code>Delete(int id)</code> обрабатывает запросы типа Delete, то есть запросы на удаление - получает из запроса параметр id и по данному идентификатору 
удаляет объект из БД.</p>
<p>В итоге у нас получится следующий проект:</p>
<img src="./pics/webapi24.png" alt="Проект ASP.NET Core Web API" />
<p>Теперь, чтобы это все использовать, изменим код класса Startup:</p>
<pre class="brush:c#;">
...
<p>Чтобы задействовать контроллеры, в методе <code>ConfigureServices()</code> вызывается метод <span class="b">services.AddControllers()</span>.</p>
<p>Чтобы подключить маршрутизацию контроллеров на основе атрибутов, в методе <code>Configure()</code> вызывается метод <span class="b">endpoints.MapControllers()</span>. 
После этого мы сможем обращаться к контроллеру через запрос <span class="ii">api/users</span>, поскольку к контроллеру применяется атрибут маршрутизации 
<code>[Route("api/[controller]")]</code>, где параметр "controller" указывает на название контроллера.</p>
<p>Запустим приложение и обратимся по пути <span class="ii">api/users</span>:</p>
<img src="./pics/webapi7.png" alt="Первое приложение на ASP.NET Web API" />
<p>Поскольку запрос из адресной строки браузера представляет GET-запрос, то его будет обрабатывать метод</p>
<pre class="brush:c#;">
[HttpGet]
public async Task&lt;ActionResult&lt;IEnumerable&lt;User&gt;&gt;&gt; Get()
{
...
<p>Этот метод возвратит всех пользователей из базы данных. Поэтому в браузере мы увидим все те данные, которые были добавлены в конструкторе.</p>
<p>Передадим параметр id:</p>
<img src="./pics/webapi23.png" alt="ASP.NET Web API in ASP.NET Core" />
<p>Поскольку это также запос типа Get, но теперь также передается параметр id, то сработает следующий метод:</p>
<pre class="brush:c#;">
[HttpGet("{id}")]
...
<div class="nav"><p><a href="./23.1.php">Назад</a><a href="./">Содержание</a><a href="./23.3.php">Вперед</a></p></div></div>
<div class="socBlock">
