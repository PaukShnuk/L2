...
<html >
<head>
<title>ASP.NET Core | Основные классы в Identity</title>
<meta charset="utf-8" />
<meta name="description" content="Основные классы в ASP.NET Core Identity, IdentityDbContext, UserManager, RoleManager, IdentityUser и IdentityRole">
...
<h2>Основные классы в ASP.NET Core Identity</h2><div class="date">Последнее обновление: 14.12.2019</div>
...
<p>ASP.NET Core Identity использует множество различных классов, предназначенных для различных задач. Рассмотрим основные из этих классов.</p>
<h3>Контекст данных IdentityDbContext</h3>
<p>Для работы с базой данных ASP NET Identity использует контекст данных, который наследуется от класса <span class="b">IdentityDbContext</span> из 
пространства имен <code>Microsoft.AspNetCore.Identity.EntityFrameworkCore</code>:</p>
<pre class="brush:c#;">
...
<p>По умолчанию данный класс наследует весь функционал от IdentityDbContext. Так, мы можем получить содержимое таблиц из бд с помощью следующих свойств:</p>
<ul>
<li><p><span class="b">Users</span>: набор объектов IdentityUser, соответствует таблице пользователей</p></li>
<li><p><span class="b">Roles</span>: набор объектов IdentityRole, соответствует таблице ролей</p></li>
<li><p><span class="b">RoleClaims</span>: набор объектов IdentityRoleClaim, соответствует таблице связи ролей и объектов claims</p></li>
<li><p><span class="b">UserLogins</span>: набор объектов IdentityUserLogin, соответствует таблице связи пользователей с их логинами их внешних сервисов</p></li>
<li><p><span class="b">UserClaims</span>: набор объектов IdentityUserClaim, соответствует таблице связи пользователей и объектов claims</p></li>
<li><p><span class="b">UserRoles</span>: набор объектов IdentityUserRole, соответствует таблице, которая сопоставляет пользователей и их роли</p></li>
<li><p><span class="b">UserTokens</span>: набор объектов IdentityUserToken, соответствует таблице токенов пользователей</p></li>
</ul>
<p>По умолчанию таблица, которая соответствует определенному набору, называется по имени набор плюс префикс <span class="ii">AspNet</span>. Например, 
таблица пользователей называется <span class="ii">AspNetUsers</span>.</p>
<p>Если нам надо хранить в базе данных объекты каких-то других классов, то в классе контекста можно определить для них свойство по типу DbSet&lt;T&gt;.</p>
<p>И мы можем напрямую работать с этим контекстом данных. Например, вывести всех пользователей:</p>
<pre class="brush:c#;">
...
<p>С помощью стандартного механизма внедрения зависимостей контекст данных внедряется в контроллер, и, получив в конструкторе, мы сможем его использовать.</p>
<p>В представлении выведем список объектов IdentityUser:</p>
<pre class="brush:xml;">
...
<h3>IdentityUser</h3>
<p>В ASP.NET Core Identity пользователь представлен классом <span class="b">IdentityUser</span> из пространства имен 
<code>Microsoft.AspNetCore.Identity.EntityFrameworkCore</code>. Этот класс предоставляет базовую информацию о пользователе с помощью следующих свойств:</p>
<ul>
<li><p><span class="b">Id</span>: уникальный идентификатор пользователя</p></li>
<li><p><span class="b">UserName</span>: ник пользователя</p></li>
<li><p><span class="b">Email</span>: электронный адрес пользователя</p></li>
<li><p><span class="b">Logins</span>: коллекция логинов, которые использовались пользователем для входа через сторонние сервисы (Google, Facebook и т.д.)</p></li>
<li><p><span class="b">Claims</span>: коллекция клеймов или дополнительных объектов, которые используются для авторизации пользователя</p></li>
<li><p><span class="b">PasswordHash</span>: хеш пароля. В базе данных напрямую не хранится пароль, а только его хеш.</p></li>
<li><p><span class="b">Roles</span>: набор ролей, к которым принадлежит пользователь</p></li>
<li><p><span class="b">PhoneNumber</span>: номер телефона</p></li>
<li><p><span class="b">SecurityStamp</span>: некоторое специальное значение, которое меняется при смене аутентификационных данных, например, пароля</p></li>
<li><p><span class="b">AccessFailedCount</span>: количество неудачных входов пользователя в систему</p></li>
<li><p><span class="b">EmailConfirmed</span>: подтвержден ли адрес электронной почты</p></li>
<li><p><span class="b">PhoneNumberConfirmed</span>: подтвержден ли номер телефона</p></li>
</ul>

<h3>Менеджер пользователей UserManager</h3>
<p>Как правило, для управления пользователями используется не контекст данных, а специальный класс - <span class="b">UserManager&lt;T&gt;</span> из пространства имен 
<code>Microsoft.AspNetCore.Identity</code>. Основные из его методов и свойств:</p>
<ul>
<li><p><span class="b">ChangePasswordAsync(user, old, new)</span>: изменяет пароль пользователя</p></li>
<li><p><span class="b">CreateAsync(user)</span>: создает нового пользователя</p></li>
<li><p><span class="b">DeleteAsync(user)</span>: удаляет пользователя</p></li>
<li><p><span class="b">FindByIdAsync(id)</span>: ищет пользователя по id</p></li>
<li><p><span class="b">FindByEmailAsync(email)</span>: ищет пользователя по email</p></li>
<li><p><span class="b">FindByNameAsync(name)</span>: ищет пользователя по нику</p></li>
<li><p><span class="b">UpdateAsync(user)</span>: обновляет пользователя</p></li>
<li><p><span class="b">Users</span>: возвращает всех пользователей</p></li>
<li><p><span class="b">AddToRoleAsync(user, role)</span>: добавляет для пользователя user роль role</p></li>
<li><p><span class="b">GetRolesAsync (user)</span>: возвращает список ролей, к которым принадлежит пользователь user</p></li>
<li><p><span class="b">IsInRoleAsync(user, name)</span>: возвращает true, если пользователь user принадлежит роли name</p></li>
<li><p><span class="b">RemoveFromRoleAsync(user, name)</span>: удаляет роль name у пользователя user</p></li>
</ul>
<p>После добавления сервисов Identity в классе Startup мы можем получить объект UserManager через механизм внедрения зависимостей, например, через конструктор:</p>
<pre class="brush:c#;">
using System.Linq;
...
<h3>IdentityRole и RoleManager</h3>
<p>По умолчанию роль в ASP.NET Core Identity представлена классом <span class="b">IdentityRole</span>, который определяет три свойства:</p>
<ul>
<li><p><code>Id</code>: идентификатор роли</p></li>
<li><p><code>Name</code>: название роли</p></li>
<li><p><code>Users</code>: коллекция объектов <code>IdentityUserRole</code>, через которые пользователи ассоциированы с данной ролью</p></li>
</ul>
<p>Также, как и с пользователями, с ролями мы можем работать напрямую через контекст данных. Например:</p>
<pre class="brush:c#;">
using System.Linq;
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Identity;
...
<p>И в представлении мы можем вывести все роли:</p>
<pre claims="brush:xml;">
...
<p>Но, как правило, за работу с ролями в ASP.NET Core Identity отвечает класс <span class="b">RoleManager&lt;T&gt;</span>, где параметр T представляет класс, описывающий роль. 
Этот класс представляет ряд методов ля управления ролями:</p>
<ul>
<li><p><span class="b">CreateAsync(role)</span>: создает новую роль</p></li>
<li><p><span class="b">DeleteAsync(role)</span>: удаляет роль</p></li>
<li><p><span class="b">FindByIdAsync(id)</span>: возвращает роль по id</p></li>
<li><p><span class="b">FindByNameAsync(name)</span>: возвращает роль по названию</p></li>
<li><p><span class="b">RoleExistsAsync(name)</span>: возвращает true, если роль с данным именем существует</p></li>
<li><p><span class="b">UpdateAsync(role)</span>: обновляет роль</p></li>
<li><p><span class="b">Roles</span>: возвращает все роли</p></li>
</ul>
<p>И также мы можем получить объект RoleManager в контроллерах через механизм внедрения зависимостей:</p>
<pre class="brush:c#;">
using System.Linq;
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Identity;
...
<p>Чтобы воспользоваться классом RoleManager, нам надо изменить подключение сервисов Identity в классе Startup в методе ConfigureServices. 
По умолчанию оно выглядит так:</p>
<pre class="brush:c#;">
services.AddDefaultIdentity&lt;IdentityUser&gt;(options =&gt; options.SignIn.RequireConfirmedAccount = true)
		.AddEntityFrameworkStores&lt;ApplicationDbContext&gt;();
</pre>
<p>Изменим следующим образом:</p>
<pre class="brush:c#;">
services.AddIdentity&lt;IdentityUser, IdentityRole&gt;(options =&gt; options.SignIn.RequireConfirmedAccount = true) 
		.AddEntityFrameworkStores&lt;ApplicationDbContext&gt;();
</pre>
<p>Метод AddIdentity типизируется классом роли - IdentityRole, тем самым также будет внедряться сервис RoleManager, который мы сможем получить в контроллере.</p>
...
<div class="nav"><p><a href="./16.1.php">Назад</a><a href="./">Содержание</a><a href="./16.2.php">Вперед</a></p></div></div>
<div class="socBlock">
