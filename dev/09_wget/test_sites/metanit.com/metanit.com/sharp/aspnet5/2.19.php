...
<html >
<head>
<title>ASP.NET Core | Создание своих сервисов</title>
<meta charset="utf-8" />
<meta name="description" content="Сервисы и метод ConfigureServices в ASP.NET Core, определение своих сервисов и встраивание их в приложение">
...
<h2>Создание своих сервисов</h2><div class="date">Последнее обновление: 07.11.2019</div>
...
<p>Итак, мы можем использовать встроенные сервисы как IWebHostEnvironment или сервисы, которые предоставляет ASP.NET Core и которые надо дополнительно добавлять в приложение типа MVC. 
Но также мы можем создавать свои собственные сервисы. Рассмотрим, как это сделать.</p>
<p>Добавим в проект новую папку Services. А в ней определим новый интерфейс IMessageSender:</p>
<pre class="brush:c#;">
...
<p>И добавим в папку Services два класса, которые будут реализовать данный интерфейс. Первый класс будет называться EmailMessageSender:</p>
<pre class="brush:c#;">
...
<p>Второй класс будет называться SmsMessageSender:</p>
<pre class="brush:c#;">
...
<p>Используем функциональные возможности интерфейса IMessageSender в классе Startup:</p>
<pre class="brush:c#;">
...
<p>С помощью вызова <code>services.AddTransient&lt;IMessageSender, EmailMessageSender&gt;();</code> в методе ConfigureServices система на место объектов интерфейса IMessageSender будет передавать экземпляры класса EmailMessageSender. 
После добавления в ConfigureServices сервисы можно получить и использовать в любой части приложения. И через параметр метода Configure мы можем получить сервис и использовать его.</p>
<p>И теперь запустим приложение:</p>
<img src="./pics/di2.png" alt="Тестирование внедрения зависимостей в ASP.NET Core" />
<p>Поскольку метод AddTransient установил зависимость между IMessageSender и EmailMessageSender, то в браузере выводится сообщение "Sent by email." 
Мы можем поменять тип, сопоставляемый с IMessageSender:</p>
<pre class="brush:c#;">
...
<p>Используем этот класс в качестве сервиса в классе Startup:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Builder;
using Microsoft.AspNetCore.Hosting;
using Microsoft.AspNetCore.Http;
...
<pre class="brush:c#;">services.AddTransient&lt;TimeService&gt;();</pre>
<p>После добавления сервиса мы его можем получить и использовать в любой части приложения. В частности, мы его можем получить с помощью 
параметра метода <code>Configure</code>. В итоге браузер выведет текущее время:</p>
<img src="./pics/di4.png" alt="DependencyInjection in Configure ASP.NET Core" />
...
<div class="nav"><p><a href="./6.1.php">Назад</a><a href="./">Содержание</a><a href="./6.4.php">Вперед</a></p></div></div>
<div class="socBlock">
