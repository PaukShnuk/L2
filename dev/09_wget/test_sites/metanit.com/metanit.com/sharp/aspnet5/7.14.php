...
<html >
<head>
<title>ASP.NET Core | Асинхронные операции в View Component</title>
<meta charset="utf-8" />
<meta name="description" content="Асинхронные операции в View Component в ASP.NET Core MVC, метод InvokeAsync">
...
<h2>Асинхронные операции в View Component</h2><div class="date">Последнее обновление: 28.11.2019</div>
...
<p>Иногда возникает необходимость в компоненте выполнить некоторую асинхронную операцию, например, для обращения к базе данных, к внешнему сетевому ресурсу, чтению файла и 
...
метод <code>Invoke</code>, только предназначен для асинхронных операций и обычно возвращает объект Task. При вызове этого метода 
...
<p>Для примера возьмем асинхронное считывание из файла. Для этого определим в проекте папку Files, а в нее добавим файл 
<span class="ii">header.html</span> с каким-нибудь простеньким кодом:</p>
<pre class="brush:xml;">
&lt;h2&gt;Изучаем View Component.&lt;/h2&gt;
&lt;h3&gt;Асинхронный метод InvokeAsync&lt;/h3&gt;
</pre>
<p>Далее добавим в проект новый компонент Header:</p>
<pre class="brush:c#;">
...
<p>Метод InvokeAsync помечается с помощью ключевого слова async и содержит выражение await - <code>await reader.ReadToEndAsync()</code>. 
Считанное содержимое затем возвращается в виде html-фрагмента через объект HtmlContentViewComponentResult.</p>
<p>Определим вызов компонента Header в представлении Index.cshtml:</p>
<pre class="brush:xml;">
...
<p>Запустим на выполнение и увидим вставку на веб-станицу кода из файла header.html:</p>
<img src="./pics/viewcomponent8.png" alt="InvokeAsync в ViewComponent в ASP.NET Core" />
...
к InvokeAsync мы определим синхронный метод Invoke, например:</p>
<pre class="brush:c#;">
public class Header : ViewComponent
{
    public async Task&lt;IViewComponentResult&gt; InvokeAsync()
...
<p>То в этом случае мы получим ошибку, так как движок представлений не будет знать, какой метод выбрать для обработки.</p>
