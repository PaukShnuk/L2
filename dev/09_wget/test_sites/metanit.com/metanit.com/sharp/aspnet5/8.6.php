...
<html >
<head>
<title>ASP.NET Core | Создание привязчика модели</title>
<meta charset="utf-8" />
<meta name="description" content="Создание привязчика модели в ASP.NET Core MVC, интерфейс IModelBinder и IModelBinderProvider">
...
<h2>Создание привязчика модели</h2><div class="date">Последнее обновление: 29.11.2019</div>
...
<p>Привязчики модели (model binders), которые нам доступны по умолчанию в рамках ASP.NET Core MVC, охватывают большинство типов .NET Core. 
...
<p>Создадим новый проект ASP.NET Core по типу <span class="b">Web Application (Model-View-Controller)</span> и назовем его <span class="b">CustomModelBinderApp</span>. 
Добавим в проект в папку <span class="b">Models</span> специальную модель Event, которая представляет некоторое событие:</p>
<pre class="brush:c#;">
using System;
...
<p>Пусть в контроллере HomeController предусмотрен функционал для добавления одного объекта Event в список объектов:</p>
<pre class="brush:c#;">
using System;
...
<p>Но допустим, мы хотим отдельно вводить дату и отдельно время для события. И для добавления события определим следующее представление <span class="b">Create.cshtml</span>:</p>
<pre class="brush:xml;">
&lt;form method="post"&gt;
    &lt;p&gt;
...
<p>В представлении <span class="ii">Index.cshtml</span> добавленные события будут выводиться на веб-страницу:</p>
<pre class="brush:xml;">
...
<img src="./pics/models12.png" alt="Привязчик моделей в ASP.NET Core" />
<p>Но даже если мы введем данные и отправим, по умолчанию система привязки asp.net core не сможет никак соединить значения полей Date и Time в 
одно свойство типа DateTime:</p>
<img src="./pics/models13.png" alt="Model Binder in ASP.NET Core MVC" />
...
<p>Итак, добавим в папку Infrastructure новый класс <span class="b">CustomDateTimeModelBinderProvider</span>:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc.ModelBinding;
...
<img src="./pics/models15.png" alt="Провайдер привязки модели в ASP.NET Core MVC" />
<p>И последний шаг - добавление этого провайдера в коллекцию провайдеров привязчиков модели. Для этого откроем класс Startup. В его методе 
<span class="b">ConfigureServices()</span> есть вызов, который добавляет все сервисы фреймворка MVC:</p>
<pre class="brush:c#;">services.AddControllersWithViews();</pre>
...
<img src="./pics/models14.png" alt="Custom Model Binder in ASP.NET Core" />
...
<p>Далее добавим в папку Infrastructure для этого привязчика класс провайдера, который назовем <span class="b"></span>:</p>
<pre class="brush:c#;">
using CustomModelBinderApp.Models;
using Microsoft.AspNetCore.Mvc.ModelBinding;
...
<p>И для встраивания провайдера в приложение изменим метод ConfigureServices в классе Startup:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
	services.AddControllersWithViews(opts =&gt;
