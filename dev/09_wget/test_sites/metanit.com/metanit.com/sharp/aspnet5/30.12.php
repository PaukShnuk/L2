...
<html >
<head>
<title>ASP.NET Core | Клиент на Xamarin Forms для SignalR</title>
<meta charset="utf-8" />
<meta name="description" content="Клиентское приложение на Xamarin Forms для SignalR в ASP.NET Core, подключение мобильного приложения к серверу">
...
<h2>Клиент на Xamarin Forms</h2><div class="date">Последнее обновление: 08.12.2018</div>
...
<p>Xamarin Forms позволяет создавать клиентские приложения, которые могут взаимодействовать с хабом SignalR на стороне сервера. 
Рассмотрим, как это сделать.</p>
<h3>Создание сервера</h3>
<p>Сначала определим код сервера, с которым будет взаимодействовать клиент на Xamarin. Для этого создадим проект ASP.NET Core по типу 
Empty.</p>
<p>Определим в проекте следующий простейший класс хаба:</p>
<pre class="brush:c#;">
...
<p>В методе Send хаб будет принимать имя пользователя и его сообщение и транслировать его на функцию Receive всех подключенных клиентов.</p>
<p>В классе Startup определим следующий код:</p>
<pre class="brush:c#;">
...
<p>И также для теста определим в папке <span class="b">wwwroot</span> простейшую веб-страницу <span class="b">index.html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Общий проект сервера:</p>
<img src="./pics/signalr19.png" alt="SignalR Hub для работы с Xamarin Forms" />
<h3>Создание клиента Xamarin Forms</h3>
<p>Теперь создадим клиентское приложение на Xamarin. Для этого опеделим новый проект Xamarin Forms. И прежде всего добавим 
во все проекты решения через пакетный менеджер Nuget пакет <span class="b">Microsoft.AspNetCore.SignalR.Client</span>.</p>
<img src="./pics/signalr20.png" alt="Microsoft.AspNetCore.SignalR.Client в Xamarin Forms" />
<p>В главном проекте определим класс <span class="b">MessageData</span>, который будет представляет полученные с сервера данные:</p>
<pre class="brush:c#;">
...
<p>То есть объект MessageData будет содержать данные о сообщении и отправившем его пользователе.</p>
<p>Также добавим в главный проект класс <span class="b">ChatViewModel</span>, который будет выполнять роль модели представления и 
через который будет идти взаимодействие с сервером:</p>
<pre class="brush:c#;">
...
<p>Раберем код этого класса. Для взаимодействия с хабом нам потребует класс <span class="b">HubConnection</span>, который предоставляет нам функционал для 
подключения к хабу и отправки сообщений.</p>
<p>Свойства UserName и Message представляют соответственно имя пользователя и текст сообщения, которые будут отправляться на сервер. 
Свойство Messages представляет объект ObservableCollection&lt;MessageData&gt; - полученные с сервера сообщения.</p>
<p>Чтобы извещать пользователя о процессе отправки, определено свойство IsBusy - если оно равно <code>true</code>, то приложение находится в процессе оправки сообщения.</p>
<p>Свойство <code>IsConnected</code> указывает, подключено ли приложение к хабу.</p>
<p>Непосредственно для отправки сообщений определена команда <code>SendMessageCommand</code>.</p>
<p>В конструкторе ChatViewModel с помощью класса <code>HubConnectionBuilder</code> создается объект HubConnection. Для его инициализации 
через метод <code>WithUrl()</code> передается адрес хаба:</p>
<pre class="brush:c#;">
hubConnection = new HubConnectionBuilder()
	.WithUrl("http://192.168.0.103:3000/chat")
	.Build();
</pre>
<p>В каждом конктретном случае адрес будет отличаться.</p>
<p>Затем определяется команда отправки сообщений:</p>
<pre class="brush:c#;">
SendMessageCommand = new Command(async () =&gt; await SendMessage(), () =&gt; IsConnected);
</pre>
<p>При выполнении команды будет вызываться метод SendMessage. Кроме того, команда будет доступна, если свойство IsConnected равно true, то есть 
если мы подключены к хабу.</p>
<p>Получив объект HubConnection, мы можем выполнить его настройку. Так, далее устанавливается обрабатчик события завершения подключения:</p>
<pre class="brush:c#;">
hubConnection.Closed += async (error) =&gt;
{
	SendLocalMessage(String.Empty, "Подключение закрыто...");
...
<p>При закрытии подключения, которое может происходить по самым разным причинам,  коллекцию Messages добавляется диагностическое сообщение 
для пользователя (поэтому вместо имени пользователя используется пустая строка String.Empty) и затем через 5 секунд мы повторно пытаемся подключиться к хабу.</p>
<p>Кроме того, нам надо настроить прием сообщений. Для этого применяется метод <code>On</code>:</p>
<pre class="brush:c#;">
hubConnection.On&lt;string, string&gt;("Receive", (user, message) =&gt;
{
	SendLocalMessage(user, message);
...
<p>В классе хаба мы транслируем всем подключенным клиентам на функцию Receive две строки: <code>this.Clients.All.SendAsync("Receive", username, message);</code>. 
Поэтому в данном случае метод <code>On()</code> типизирован двумя объектами string - для получения имени пользователя и его сообщения. 
Первый парамет метода указавает название функции - Receive, а второй параметр представляет лямбда-выражение, в котором мы получаем от сервера данные.</p>
<p>В методе <code>Connect()</code> осуществляется подключение к хабу. Для этого применяется вызов <code>hubConnection.StartAsync()</code>. 
После его успешного выполнения мы можем взаимодействовать с сервером.</p>
<p>В методе <code>Disconnect()</code> происходит отключение от сервера. Для этого применяется вызов <code>hubConnection.StopAsync()</code></p>
<p>Метод <code>SendMessage()</code> предназначен для отправки сообщений хабу. Это осуществляется посредством вызова 
<code>hubConnection.InvokeAsync("Send", UserName, Message);</code> - на хабе вызывается метод Send, которому передаются значения UserName и Message.</p>
<p>Теперь используем этот класс. Для этого определим к следующий интерфейс на странице <span class="b">MainPage.xaml</span>:</p>
<pre class="brush:xml;">
...
<p>Элемент ActivityIndicator извещает пользователя об процессе отправки сообщений. Для ввода данных определены два текстовых поля. 
И по нажатию на кнопку вызывается команда SendMessageCommand, которая оправляет введенные данные.</p>
<p>Для отображения сообщений определен элемент ListView.</p>
<p>В файле <span class="b">MainPage.xaml.cs</span> определим привязку ChatViewModel к странице:</p>
<pre class="brush:c#;">
using Xamarin.Forms;
...
<p>В методе OnAppearing, то есть когда начинается отображение страницы, осуществляется подключение к хабу. В методе OnDisappearing, когда 
пользователь покидает страницу или приложение переходит в фоновый режим, то выполняется отключение от хаба.</p>
<p>Итоговый проект для Xamarin:</p>
<img src="./pics/signalr21.png" alt="Проект Xamarin для работы с SignalR" />
<p>Запустим сначала приложение ASP.NET Core, а затем приложение на Xamarin. Чтобы сделать приложение ASP.NET Core доступным для всех устройств, 
подключенных к одной и той же локальной сети, можно использовать описанный <a href="https://metanit.com/sharp/mvc5/24.3.php">здесь</a> второй способ.</p>
<img src="./pics/signalr22.png" alt="Xamarin Forms и ASP.NET Core SignalR" />
...
<div class="nav"><p><a href="./30.11.php">Назад</a><a href="./">Содержание</a><a href="./31.1.php">Вперед</a></p></div></div>
<div class="socBlock">
