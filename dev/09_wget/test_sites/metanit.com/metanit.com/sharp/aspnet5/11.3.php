...
<html >
<head>
<title>ASP.NET Core | Ограничения маршрутов</title>
<meta charset="utf-8" />
<meta name="description" content="Встроенные ограничения маршрутов в ASP.NET Core, inline constraints и их использование">
...
<h2>Ограничения маршрутов</h2><div class="date">Последнее обновление: 16.11.2019</div>
...
<p>При обработке запроса система маршрутизации автоматически выдергивает из строки запроса данные для параметров маршрута, не зависимости от их содержимого, 
основываясь на сегментной структуре адреса URL. Однако это не всегда бывает удобно. Например, мы хотим, чтобы какой-то параметр представлял только числа, 
а другой параметр начинался строго с определенного символа. И для этого нам надо задать ограничения маршрута.</p> 
<p>Для задания ограничений маршрутов применяется одна из перегруженных версий метода <code>MapRoute()</code> класса RouteBuilder. Например, нам надо 
...
<p>С помощью последнего параметра устанавливаются ограничения маршрута. И если мы направим приложению запрос <span class="ii">Book/Index</span>, 
то маршрут не будет обрабатываться связанным с ним обработчиком, так как имя первого параметра "controller" попадает под ограничение. А запрос <span class="ii">Home/Index</span> нормально был бы обработан.</p>
<img src="./pics/routing13.png" alt="Route Constraints in ASP.NET Core" />
<p>Подобным образом мы можем задать ограничения и для других параметров. Например, пусть параметр id состоит из двух цифр:</p>
<pre class="brush:c#;">
routeBuilder.MapRoute("default", 
        "{controller}/{action}/{id?}", 
        new { action = "Index" },
...
<p>Теперь даже запрос <span class="ii">Home/Index/1</span> не будет сопоставляться с маршрутом, так как параметр id должен состоять из двух цифр.</p>
<p>Фактически в обоих случаях применяется класс <span class="b">RegexRouteConstraint</span> из пространства <span class="ii">Microsoft.AspNetCore.Routing.Constraints</span>, который устанавливает ограничение в виде регулярного выражения:</p>
<pre class="brush:c#;">
routeBuilder.MapRoute("default", 
        "{controller}/{action}/{id?}", 
        null,
...
<p>Кроме этого класса ASP.NET Core предоставляет еще ряд классов ограничений, которые находятся в пространстве имен 
<a href="https://github.com/aspnet/AspNetCore/tree/master/src/Http/Routing/src/Constraints" rel="nofollow">Microsoft.AspNet.Routing.Constraints</a></p>
<ul>
<li><p><span class="b">AlphaRouteConstraint</span>: параметр должен состоять только из алфавитных символов английского алфавита</p>
<pre class="brush:c#;">new {controller=new AlphaRouteConstraint()}</pre>
</li>
<li><p><span class="b">BoolRouteConstraint</span>: параметр должен иметь значение true или false</p>
<pre class="brush:c#;">new {id=new BoolRouteConstraint()}</pre>
</li>
<li><p><span class="b">DateTimeRouteConstraint</span>: параметр должен предоставлять объект DateTime</p>
<pre class="brush:c#;">new {id=new DateTimeRouteConstraint()}</pre>
</li>
<li><p><span class="b">DecimalRouteConstraint</span>: параметр должен предоставлять объект decimal</p>
<pre class="brush:c#;">new {id=new DecimalRouteConstraint()}</pre>
</li>
<li><p><span class="b">DoubleRouteConstraint</span>: параметр должен предоставлять объект double</p>
<pre class="brush:c#;">new {id=new DoubleRouteConstraint()}</pre>
</li>
<li><p><span class="b">FloatRouteConstraint</span>: параметр должен предоставлять объект float</p>
<pre class="brush:c#;">new {id=new FloatRouteConstraint()}</pre></li>
<li><p><span class="b">GuidRouteConstraint</span>: параметр должен предоставлять объект Guid</p>
<pre class="brush:c#;">new {id=new GuidRouteConstraint()}</pre></li>
<li><p><span class="b">HttpMethodRouteConstraint</span>: запрос должен представлять определенный тип - Get, Post и т.д.</p>
<pre class="brush:c#;">
routeBuilder.MapRoute("default",
     "{controller}/{action}/{id?}",
     null,
...
<p>В конструктор HttpMethodRouteConstraint передаются названия допустимых типов запросв. В данном случае маршрут будет обрабатываться только для POST-запросов.</p>
</li>
<li><p><span class="b">IntRouteConstraint</span>: параметр должен предоставлять объект int</p>
<pre class="brush:c#;">new {id=new IntRouteConstraint()}</pre></li>
<li><p><span class="b">LengthRouteConstraint</span>: строка, представляющая параметр, должна иметь определенную длину</p>
<pre class="brush:c#;">
new 
{
...
<li><p><span class="b">LongRouteConstraint</span>: параметр должен предоставлять объект long</p>
<pre class="brush:c#;">new {id=new LongRouteConstraint()}</pre></li>
<li><p><span class="b">MaxLengthRouteConstraint / MinLengthRouteConstraint</span>: определяют максимальную и минимальную длину параметра в символах</p>
<pre class="brush:c#;">
new 
{
...
<li><p><span class="b">MaxRouteConstraint / MinRouteConstraint</span>: определяют максимальное и минимальное числовое значение для параметра</p>
<pre class="brush:c#;">new {id=new MinRouteConstraint(4)} // минимальное значение - 4</pre></li>
<li><p><span class="b">RangeRouteConstraint</span>: параметр должен предоставлять числовое значение int в определенном диапазоне</p>
<pre class="brush:c#;">new {id=new RangeRouteConstraint(3, 100)} // диапазон от 3 до 100</pre></li>
<li><p><span class="b">RegexRouteConstraint</span>: задает регулярное значение, которому должен соответствовать параметр</p></li>
<li><p><span class="b">OptionalRouteConstraint</span>: определяет ограничение для необязательного параметра. И если параметр имется в запросе, то вступает в силу 
внутреннее ограничение, задаваемое через свойство InnerConstraint класса OptionalRouteConstraint</p></li>
<li><p><span class="b">RequiredRouteConstraint</span>: указывает, что параметр должен обязательно иметь какое-нибудь значение</p></li>
<li><p><span class="b">StringRouteConstraint</span>: указывает, что параметр должен обязательно соответствовать определенной строке</p>
<pre class="brush:c#;">new { controller = new StringRouteConstraint("Store") } // controller должен быть равен "Store"</pre></li>
</ul>
<h3>Составные ограничения</h3>
<p>С помощью специального класса <span class="b">CompositeRouteConstraint</span> можно установить сложное ограничение, которое будет включать несколько 
простых:</p>
<pre class="brush:c#;">
...
<p>В данном случае устанавливается, что параметр <code>action</code> должен одновременно состоять только из алфавитных символов и также должен иметь 
длину не менее 5 символов.</p>
<h3>Строчный синтаксис ограничений</h3>
<p>ASP.NET Core позволяет также указывать ограничения прямо при определении параметров поле двоеточия в виде сокращений, это так называемые 
строчные ограничения (inline constraints):</p>
<pre class="brush:c#;">
routeBuilder.MapRoute("default", "{controller:regex(^H.*)}/{action}/{id?}");
</pre>
<p>Ограничение <code>regex</code> фактически применяет класс RegexRouteConstraint. Все подобные встраиваемые ограничения называются по имени классов без суффикса 
<code>RouteConstraint</code>.</p>
<p>Все строчные ограничения:</p>
<ul>
<li><p><span class="b">int</span></p>
<p>Соответствие числу</p>
<pre class="brush:c#;">{id:int}</pre></li>
<li><p><span class="b">bool</span></p>
<p>Соответствие значению true или false</p>
<pre class="brush:c#;">{active:bool}</pre></li>

<li><p><span class="b">datetime</span></p>
<p>Соответствие дате и времени</p>
<pre class="brush:c#;">{date:datetime}</pre></li>

<li><p><span class="b">decimal</span></p>
<p>Соответствие значению decimal</p>
<pre class="brush:c#;">{price:decimal}</pre></li>

<li><p><span class="b">double</span></p>
<p>Соответствие значению типа double</p>
<pre class="brush:c#;">{weight:double}</pre></li>
<li><p><span class="b">float</span></p>
<p>Соответствие значению типа float</p>
<pre class="brush:c#;">{height:float}</pre></li>

<li><p><span class="b">guid</span></p>
<p>Соответствие значению типа Guid</p>
<pre class="brush:c#;">{id:guid}</pre></li>

<li><p><span class="b">long</span></p>
<p>Соответствие значению типа long</p>
<pre class="brush:c#;">{id:long}</pre></li>

<li><p><span class="b">minlength(value)</span></p>
<p>Строка должна иметь символов не меньше value</p>
<pre class="brush:c#;">{name:minlength(3)}</pre></li>
<li><p><span class="b">maxlength(value)</span></p>
<p>Строка должна иметь символов не больше value</p>
<pre class="brush:c#;">{name:maxlength(20)}</pre></li>

<li><p><span class="b">length(value)</span></p>
<p>Строка должна иметь ровно столько символов, сколько определено в параметре value</p>
<pre class="brush:c#;">{name:length(10)}</pre></li>

<li><p><span class="b">length(min, max)</span></p>
<p>Строка должна иметь символов не меньше min и не больше max</p>
<pre class="brush:c#;">{name:length(3, 20)}</pre></li>

<li><p><span class="b">min(value)</span></p>
<p>Число должно быть не меньше value</p>
<pre class="brush:c#;">{age:min(3)}</pre></li>
<li><p><span class="b">max(value)</span></p>
<p>Число должно быть не больше value</p>
<pre class="brush:c#;">{age:max(20)}</pre></li>
<li><p><span class="b">range(min, max)</span></p>
<p>Число должно быть не меньше min и не больше max</p>
<pre class="brush:c#;">{age:range(18, 99)}</pre></li>

<li><p><span class="b">alpha</span></p>
<p>Строка должна состоять из одно и более алфавитных символов</p>
<pre class="brush:c#;">{name:alpha}</pre></li>
<li><p><span class="b">regex(expression)</span></p>
<p>Строка должна соответствовать регулярному выражению expression</p>
<pre class="brush:c#;">{phone:regex(^\d{{3}}-\d{{3}}-\d{{4}}$)}</pre></li>
<li><p><span class="b">required</span></p>
<p>Параметр является обязательным, и его значение должно быть определено</p>
<pre class="brush:c#;">{name:required}</pre></li>
</ul>
<p>Например, установим ряд ограничений:</p>
<pre class="brush:c#;">
routeBuilder.MapRoute("default", "{controller:length(4)}/{action:alpha}/{id:range(4,100)}");
</pre>
<p>Если нам надо установить составное ограничение, то все ограничения перечисляются через двоеточие:</p>
<pre class="brush:c#;">
routeBuilder.MapRoute(
    name: "default",
...
<div class="nav"><p><a href="./11.4.php">Назад</a><a href="./">Содержание</a><a href="./11.7.php">Вперед</a></p></div></div>
<div class="socBlock">
