...
<html >
<head>
<title>ASP.NET Core MVC | Отправка статусных кодов</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка статусных кодов в ASP.NET Core MVC">
...
<h2>Отправка статусных кодов</h2><div class="date">Последнее обновление: 17.11.2019</div>
...
<p>Иногда возникает необходимость отправить в ответ на запрос какой-либо статусный код. 
Например, если пользователь пытается получить доступ к ресурсу, который недоступен или для которого у пользователя нету прав. Либо 
нам нужно просто уведомить браузер пользователя с помощью статусного кода об успешном выполнении операции, как иногда применяется в ajax-запросах. 
И фреймворк ASP.NET Core MVC предоставляет множество различных классов, которые можно использовать для отправки статусного кода.</p>
<h3>StatusCodeResult</h3>
<p><span class="b">StatusCodeResult</span> позволяет отправить любой статусный код клиенту:</p>
<pre class="brush:c#;">
...
<p>Для создания этого результата используется метод <span class="b">StatusCode()</span>, в который передается отправляемый код статуса.</p>
...
<img src="./pics/httpnotfound.png" alt="NotFoundResult и ошибка 404 в ASP.NET MVC Core" />
...
<p><span class="b">UnauthorizedObjectResult</span> также посылает код 401, только позволяет добавить в ответ некоторый объект с информацией об ошибке:</p>
<pre class="brush:c#;">
...
<img src="./pics/controllers8.png" alt="Статусные коды в ASP.NET Core" />
...
Второй класс в дополнении к статусному коду позволяет отправить доплнительную информацию, которая потом отобразится в браузере.</p>
<p>Объекты обоих классов создаются методом <span class="b">Ok()</span>. Для первого класса - это метод без параметров, для второго класса - метод, 
который в качестве параметра принимает отправляемую информацию:</p>
<pre class="brush:c#;">
public IActionResult Index()
{
