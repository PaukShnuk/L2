...
<html >
<head>
<title>ASP.NET Core | Создание провайдера логгирования</title>
<meta charset="utf-8" />
<meta name="description" content="Создание провайдера логгирования в ASP.NET Core, запись лога в текстовый файл, использование объектов ILogger, ILoggerFactory и ILoggerProvider">
...
<h2>Создание провайдера логгирования</h2><div class="date">Последнее обновление: 06.11.2019</div>
...
<p>Стандартная инфраструктура ASP NET Core предоставляет, возможно, не самый удобные способы логгирования - на консоль, в окне Output в Visual Studio. 
...
<ul>
<li><p><code>BeginScope</code>: этот метод возвращает объект IDisposable, который представляет некоторую область видимости для логгера. В данном случае нам 
этот метод не важен, поэтому возвращаем значение <code>null</code></p></li>
...
<li><p><code>formatter</code>: функция форматирвания, которая с помощью двух предыдущих параметов позволяет получить собственно сообщение для логгирования</p></li>
</ul>
...
<p>Этот класс добавляет к объекту ILoggerFactory метод расширения <code>AddFile</code>, который будет добавлять наш провайдер логгирования.</p>
<p>Теперь используем провайдер в классе Startup:</p>
<pre class="brush:c#;">
...
<p>Теперь для логгирования будет использоваться файл logger.txt, который будет создаваться в папке проекта.</p>
...
<div class="nav"><p><a href="./2.28.php">Назад</a><a href="./">Содержание</a><a href="./11.1.php">Вперед</a></p></div></div>
<div class="socBlock">
