...
<html >
<head>
<title>ASP.NET Core | Частичные представления в MVC</title>
<meta charset="utf-8" />
<meta name="description" content="Partial views в ASP.NET MVC Core, класс PartialView и создание частичных представлений">
...
<h2>Частичные представления</h2><div class="date">Последнее обновление: 19.11.2019</div>
...
<p>В приложениях на ASP.NET MVC кроме обычных представлений и мастер-страниц можно также использовать частичные представления или <span class="b">partial views</span>. 
...
<p>За рендеринг частичных представлений отвечает объект <span class="b">PartialViewResult</span>, который возвращается методом <code>PartialView</code>. 
Итак, определим в контроллере HomeController новое действие GetMessage:</p>
<pre class="brush:c#;">
...
<p>Теперь добавим в папку <span class="ii">Views/Home</span> новое представление <span class="ii">_GetMessage.cshtml</span>, в котором будет простенькое содержимое:</p>
<pre class="brush:xml;">&lt;h3&gt;Частичное представление&lt;/h3&gt;</pre>
<p>Теперь обратимся к методу GetMessage 
как к обычному действию контроллера, и оно нам вернет частичное представление:</p>
<img src="./pics/views5.png" alt="Частичные представления в ASP.NET MVC Core" />
...
<code>IHtmlContent<code>, который представляет html-содержимое и который обернут в объект <code>Task&lt;TResult&gt;</code>. В качестве параметра в метод передается имя представления:</p>
<img src="./pics/views6.png" alt="Partial Views in ASP.NET MVC Core" />
<p>Обращения к методу <code>GetMessage()</code> в контроллере при этом не происходит.</p>
...
<pre class="brush:c#;">@{await Html.RenderPartialAsync("_GetMessage");}</pre>
...
<pre class="brush:c#;">@await Html.PartialAsync("_GetMessage", new List&lt;string&gt; { "Lumia 950", "iPhone 6S", "Samsung Galaxy s 6", "LG G 4" })</pre>
...
Теперь изменим частичное представление <span class="b">_GetMessage.cshtml</span>:</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./7.9.php">Назад</a><a href="./">Содержание</a><a href="./7.7.php">Вперед</a></p></div></div>
<div class="socBlock">
