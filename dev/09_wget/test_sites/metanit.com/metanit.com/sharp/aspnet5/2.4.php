...
<html >
<head>
<title>ASP.NET Core | Создание компонентов middleware</title>
<meta charset="utf-8" />
<meta name="description" content="Создание компонентов middleware для обработки запросов в ASP.NET Core">
...
<h2>Создание компонентов middleware</h2><div class="date">Последнее обновление: 30.10.2019</div>
...
<p>Кроме использования делегатов в методах Run/Use/Map мы можем создавать свои компоненты middleware в виде отдельных классов, 
которые затем добавляются в конвейер с помощью метода <code>UseMiddleware()</code>. Например, напишем свой примитивный компонент. 
Для этого добавим в новый проект новый класс TokenMiddleware:</p>
<pre class="brush:c#;">
...
Так, изменим метод Configure класса Startup следующим образом:</p>
<pre class="brush:c#;">
...
<img src="./pics/custommiddleware2.png" alt="Custom middleware in asp net core" />
<p>Если же будет передан корректный токен, то метод <code>app.UseToken()</code> передаст управление методу <code>app.Run()</code>:</p>
<img src="./pics/custommiddleware3.png" alt="Создание компонента middleware in asp net core" />
...
<img src="./pics/custommiddleware1.png" alt="Custom middleware in ASP.NET Core" />
<p>Теперь применим этот метод в методе <code>Configure()</code> класса Startup:</p>
<pre class="brush:c#;">
public class Startup
{
    public void Configure(IApplicationBuilder app)
...
<p>Затем в методе <code>Configure()</code> уже можно передать в метод расширения UseToken конкретное значение:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app)
{
    app.UseToken("555555");
...
<div class="nav"><p><a href="./2.22.php">Назад</a><a href="./">Содержание</a><a href="./2.18.php">Вперед</a></p></div></div>
<div class="socBlock">
