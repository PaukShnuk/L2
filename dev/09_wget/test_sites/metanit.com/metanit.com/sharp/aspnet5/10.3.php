...
<html >
<head>
<title>ASP.NET Core | LinkTagHelper и ScriptTagHelper</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение скриптов и стилей в ASP.NET Core MVC с помощью tag-хелперов LinkTagHelper и ScriptTagHelper">
...
<h2>LinkTagHelper и ScriptTagHelper</h2><div class="date">Последнее обновление: 05.12.2019</div>
...
<p>Для подключения внешних файлов скриптов применяется тег-хэлпер <span class="b">ScriptTagHelper</span>. Тег, представляющий данный класс, может принимать ряд атрибутов:</p>
<ul>
...
<p>Данный элемент представляет не просто стандартный тег script, но  и класс тег-хэлпера ScriptTagHelper. Атрибут <code>src</code> 
...
<code>asp-fallback-test</code>. Он тестирует загрузку с помощью выражения <code>window.jQuery</code>. Если объект <code>window.jQuery</code> определен, 
то загрузка скрипта прошла успешно. Если же нет, то загружается скрипт, который указан в атрибуте <code>asp-fallback-src</code>.</p>
<p>Рассмотрим другие атрибуты. Допустим, у нас в проекте в папке wwwroot/js определено четыре скрипта и подпапка util с двумя скриптами:</p>
<img src="./pics/scripttagbuilder.png" alt="scripttagbuilder in ASP.NET Core" />
<p>Теперь подключим все скрипты:</p>
<pre class="brush:xml;">
...
<p>В данном случае подключаются все файлы css из каталога wwwroot/css и всех его подкаталогов:</p>
<img src="./pics/linktaghelper.png" alt="Класс LinkTagHelper в ASP.NET Core" />
...
<p>Для решения этой проблемы мы можем использовать в ScriptTagHelper и LinkTagHelper параметр <code>asp-append-version</code>:</p>
<pre class="brush:xml;">
