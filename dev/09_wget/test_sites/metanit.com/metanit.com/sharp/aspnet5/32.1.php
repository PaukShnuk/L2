...
<html >
<head>
<title>ASP.NET Core | Введение в gRPC</title>
<meta charset="utf-8" />
<meta name="description" content="Сервисы gRPC в ASP.NET Core, определение и реализация сервиса, файлы proto, протокол protobuf, реализация клиента .NET Core">
...
<h1>Сервисы gRPC</h1><h2>Введение в gRPC</h2><div class="date">Последнее обновление: 23.01.2020</div>
...
<p>gRPC представляет фреймворк, который использует протокол RPC (Remote Procedure Call) для обмена сообщениями между клиентом и сервером. 
...
в режиме реального времени. gRPC не является частью ASP.NET Core или .NET, более того gRPC представляет технологию, которая не привязана к конкретному языку. Но тем не менее 
мы можем использовать gRPC в связке с ASP.NET Core для создания приложений.</p>
<p>Visual Studio поддерживает специальный шаблон для создания серверных приложений ASP.NET Core с gRPC - <span class="b">gRPC Service</span>.</p>
<img src="./pics/grpc3.png" alt="Сервер gRPC на ASP.NET Core" />
<p>Используя данный тип проектов создадим новый проект, который, пусть называется SimpleGrpcService.</p>
<img src="./pics/grpc4.png" alt="Проект сервиса gRPC на ASP.NET Core" />
<p>Структура проекта gRPC:</p>
<ul>
<li><p>Папка <span class="b">Protos</span> содержит файлы с определением сервисов и сообщений, используемых для взаимодействия по сети</p>
<p>По умолчанию в этой папке определен один файл <span class="b">greet.proto</span>.</p></li>
<li><p>Папка <span class="b">Services</span> содержит файлы с реализацией сервисов</p>
<p>По умолчанию в этой папке определен один файл <span class="b">GreeterService.cs</span>.</p></li>
<li><p>Файл <span class="b">appsettings.json</span> - стандартный файл конфигурации проекта в ASP.NET Core.</p></li>
<li><p>Файл <span class="b">Program.cs</span> содержит стандартный класс Program, с которого начинается выполнение приложения ASP.NET Core.</p></li>
<li><p>Файл <span class="b">Startup.cs</span> содержит стандартный класс Startup приложения ASP.NET Core.</p></li>
</ul>
<h3>Определение сервиса</h3>
<p>Таким образом, мы видим, что по сравнению с обычными проектами ASP.NET Core здесь два новый элемента - каталоги Protos и Services. 
...
Так, по умолчанию в папке Protos есть файл <span class="b">greet.proto</span> со следующим кодом:</p>
<pre class="brush:c#;">
syntax = "proto3";
...
<p>Определение этого файла может напоминать синтаксис C#, но в реальности это синтаксис <span class="b">proto</span>, который используется 
для описания сервиса gPRC. Хотя в целом это си-подобный синтаксис, поэтому ориентироваться в нем не так сложно.</p>
<p>Самая первая строка определяет тип используемого синтаксиса:</p>
<pre class="brush:c#;">syntax = "proto3";</pre>
<p>В данном случаем применяется синтаксис "proto3". На сегодняшний момент в природе также имеется версия "proto2", которая является более старой.</p>
<p>Далее указывается пространство имен, которое будет использоваться с этим сервисом:</p>
<pre class="brush:c#;">option csharp_namespace = "TestGrpcService";</pre>
<p>По умолчанию это название проекта. И соответственно генерируемые классы будут помещаться в даное пространство имен.</p>
<p>Следующая строка с помощью оператора <span class="b">package</span> определяет название пакета:</p>
<pre class="brush:c#;">package greet;</pre>
<p>В данном случае пакет называется "great". Установка пакета позволяет разрешить конфликты имен при наличие сущностей с одинаковыми именами.</p> 
<p>Далее собственно определяется сервис:</p>
<pre class="brush:c#;">
service Greeter {
  rpc SayHello (HelloRequest) returns (HelloReply);
}
</pre>
<p>Сервис определяется с помощью ключевого слова <span class="b">service</span>, после которого указывается название сервиса. То есть в данном случае 
сервис называется "Greeter".</p>
<p>С помощью ключевого слова <span class="b">rpc</span> в сервисе определяется метод <code>SayHello</code>. Данный метод отправляет 
сообщение HelloRequest и сообщение HelloReply.</p>
<p>Далее идет определение используемых сообщений:</p>
<pre class="brush:c#;">
message HelloRequest {
  string name = 1;
}
...
<p>Сообщение представляет специальную сущность, которая содержит пересылаемые данные. Сообщение определяет набор полей, для которых 
определен тип. Каждое поле представляет некоторый кусочек информации, посылаемой в сообщении. Так, в обоих сообщениях определены два поля типа string, то есть в каждом сообщении будут отправляться некоторая строка.</p>
<p>Proto3 поддерживает использование многих стандартных примитивных типов, которые применяются в ряде наиболее популярных языков программирования, например, 
<code>bool, int32, float, double</code> и <code>string</code>. Таким образом, можно пересылать различные по характеру данные. 
Также в качестве типа полей сообщений можно использовать ранее определенные типы сообщений.</p>
<p>Каждому полю в сообщении присваивается уникальное число, например, в примере выше полям обоих сообщений присваивается единица 
(в <code>string name = 1</code> или в <code>string message = 1</code>). Эти значения позволяют идентифицировать непосредственные значения полей в бинарном формате при кодировании и получении 
сообщений. При использовании чисел от 1 до 15 в качестве значения в бинаром представлении в сообщение добавляется дополнительный 
...
лучше указывать значения от 1 до 15. Допустимые значения: от 1 до 536870911 (з исключением диапазона чисел от 19000 до 19999)..</p>
<p>В дальнейшем компилятор сгенерирует класс, который сооветствует определению в файле <code>*.proto</code>.</p>
<p>Подробное руководство по синтаксису protobuf можно найти на <a href="https://developers.google.com/protocol-buffers/docs/proto3" rel="nofollow">https://developers.google.com/protocol-buffers/docs/proto3</a> и 
<a href="https://developers.google.com/protocol-buffers/docs/csharptutorial" rel="nofollow">https://developers.google.com/protocol-buffers/docs/csharptutorial</a></p>
<p>Используя подобный подход, мы можем абстрагироваться от конкретного языка и описывать взаимодействия сервера и клиент в терминах языка Proto. А затем для каждого конкреного языка 
(C#, Java и т.д.) определить соответствующую реализацию.</p>
<h3>Реализация сервиса</h3>
<p>В папке <span class="b">Services</span> по умолчанию в файле <span class="b">GreeterService.cs</span> определен класс GreeterService, 
который представляет реализацию сервиса gPRC на языке C#:</p>
<pre class="brush:c#;">
...
<p>Класс сервиса (в данном случае GreeterService) наследуется от класса Greeter.GreeterBase. Greeter.GreeterBase - абстрактный класс, который автоматически генерируется по определению сервиса greeter в файле <code>greeter.proto</code>.</p>
<p>Класс GreeterService по сути представляет обычный класс C#. Так, по умолчанию он имеет конструктор, который посредством dependency injection получает 
объект логгера и может его использовать для логирования.</p>
<p>Но основной момент класса сервиса - это реализация метода SayHello, коорый по сути отвечает за обмен сообщениями. Его первый параметр представляет 
класс HelloRequest, который соответствует определению входящего сообщения в файле proto. То есть это те данные, которые мы получаем от клиента:</p>
<pre class="brush:c#;">
message HelloRequest {
  string name = 1;
}
</pre>
<p>Поскольку сообщение содержит одно поле name, которое представляет строку, то в коде C# мы можем получить эти данные через свойство Name.</p>
<p>Второй параметр метода - объект ServerCallContext, хранит информацию, связанную с контекстом, в котором работает сервер.</p>
<p>Возвращаемое значение метода - объект класса HelloReply, который представляет другое сообщение:</p>
<pre class="brush:c#;">
message HelloReply {
  string message = 1;
}
</pre>
<p>Это те данные, которые мы посылаем клиенту в ответ. Поскольку сообщение содержит строковое поле message, то в соответствующем классе C# мы можем обращаться к нему 
через свойство Message:</p>
<pre class="brush:c#;">
new HelloReply
{
	Message = "Hello " + request.Name
...
<p>Класс HelloReply, как и все классы, которые представляют сообщения или сервисы из файла proto, также генерируется автоматически.</p>
<h3>Подключение gRPC</h3>
<p>Для того, чтобы задействовать сервис gRPC, в классе Startup, во-первых, в методе ConfigureServices добавляются необходимые сервисы:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
	services.AddGrpc();	// добавление сервисов gRPC
}
</pre>
<p>Во-вторых, в методе Configure сервис gRPC встраивается в систему маршрутизации для обработки запроса:</p>
<pre class="brush:c#;">
...
<p>Для встраивания сервиса применяется метод <span class="b">MapGrpcService()</span/>, который типизируется типом встраиваемого сервиса.</p>
<p>В конце стоит отметить, что по умолчанию сервер gRPC будет запускаться по адресу https://localhost:5001, и собственно по этому адресу клиенты смогут обращаться к серверу.</p>
<p>Это был примитивная стандартная инфрастуктура сервиса gRPC. Теперь рассмотрим, как создать для него клиентское приложение.</p>
<h3>Клиент для gRPC-сервиса</h3>
<p>Содадим клиентское приложение для вышеопределенного сервиса gRPC. Для этого создадим новый проект консольного приложения 
(<span class="b">Console App (.NET Core)</span>:</p>
<img src="./pics/grpc1.png" alt="Создание клиента для сервиса gRPC в ASP.NET Core" />
<p>Для создания клиентского приложения для gRPC необходимо через Nuget установить следующие пакеты:</p>
<ul>
<li><p>Grpc.Net.Client</p></li>
<li><p>Google.Protobuf</p></li>
<li><p>Grpc.Tools</p></li>
</ul>
<img src="./pics/grpc2.png" alt="Пакеты gRPC в ASP.NET Core" />
<p>Далее создадим в проекте консольного приложения новую папку <span class="b">Protos</span>, а в нее скопируем из проекта сервиса файл <span class="b">greet.proto</span>, 
который содержит определение используемого сервиса.</p>
<p>Далее нам надо отредактировать файл проекта. Для этого нажмем на название проекта правой кнопки мыши и выберем в контекстном меню пункт 
<span class="b">Edit Project File</span>:</p>
<img src="./pics/grpc5.png" />
<p>После добавления пакетов он выглядит примерно следующим образом:</p>
<pre class="brush:xml;">
...
<p>В узел <code>&lt;ItemGroup&gt;</code> добавим слеудющий элемент:</p>
<pre class="brush:xml;">
&lt;Protobuf Include="Protos\greet.proto" GrpcServices="Client" /&gt;
</pre>
<p>То есть после изменения файл проекта будет выглядеть следующим образом:</p>
<pre class="brush:xml;">
&lt;Project Sdk="Microsoft.NET.Sdk"&gt;
...
<p>После этого выполним построение проекта консольного приложения, чтобы были сгенерированы необходимые классы. Все сгенерированные файлы можно найти 
в проекте в каталоге <code>\obj\Debug\netcoreapp3.1</code>.</p>
<p>После построения проекта изменим код класса Program следующим образом:</p>
<pre class="brush:c#;">
...
<p>Допустим, программа будет спрашивать у пользователя имя, отправлять его сервису gRPC и отображать ответ сервиса.</p>
<p>В файле greet.proto определено, что генерируемые классы будут помещаться в пространство имен SimpleGrpcService:</p>
<pre class="brush:c#;">option csharp_namespace = "SimpleGrpcService";</pre>
<p>Поэтому в коде клиентского приложения для подключения всех необходимых классов, связанных с сервисом grpc и автосгенерированных при построении 
проекта, применяется соответствующая директива using:</p>
<pre class="brush:c#;">using SimpleGrpcService; </pre>
<p>В каждом конкретном случае пространство имен может отличаться (по умолчанию оно равно названию проекта сервиса).</p>
<p>Для обмена сообщениями с сервером с помощью метода <code>GrpcChannel.ForAddress()</code> создается канал - объект <span class="b">GrpcChannel</span>:</p>
<pre class="brush:c#;">using var channel = GrpcChannel.ForAddress("https://localhost:5001");</pre>
<p>В метод передается адрес сервиса. В данном случае поскольку сервис будет запускаться по адресу "https://localhost:5001", то 
соответственно передается данный адрес.</p>
<p>Обратите внимание, что канал создается с помощью конструкции using, которая использует новый синтаксис C# 8.0.</p>
<p>Для обращения к серверу необходимо создать объект клиента:</p>
<pre class="brush:c#;">var client = new Greeter.GreeterClient(channel);</pre>
<p>В конструктор клиента передается объект GrpcChannel.</p>
<p>Название конкретного класса клиента зависит от определения сервиса и устанавливается по шаблону <code>[имя_сервиса].[имя_сервиса]Client</code>. 
...
Greeter.GreeterClient.</p>
<p>Далее собственно выполняется взаимодействие с сервисом. Для этого вызывается метод <code>client.SayHelloAsync()</code>, определение которого в целом 
совпадает с определением метода SayHello в сервисе Greeter за тем исключением, что метод клиента асинхронный:</p>
<pre class="brush:c#;">var reply = await client.SayHelloAsync(new HelloRequest { Name = name });</pre>
<p>В метод передается объект HelloRequest, который представляет отправляемую сервису информацию. Определение класса HelloRequest совпадает 
с определением сообщения HelloRequest из файла proto.</p>
<p>Возвращает метод ответ сервера в виде объекта HelloReply, обернутого в объект Grpc.Core.AsyncUnaryCall. Класс HelloReply совпадает с 
определением сообщения HelloReply из файла proto, поэтому через свойство Message мы можем собственно получить ответ от сервера.</p>
<pre class="brush:c#;"> Console.WriteLine("Ответ сервера: " + reply.Message);</pre>
<p>Протестируем приложения. Запустим сервер gRPC. По умолчанию он запускается по адресу https://localhost:5001.</p>
<p>Затем запустим консольный клиент и в консоли введем некоторое строковое значение:</p>
<img src="./pics/grpc6.png" alt="Взаимодействие клиента и сервера gRPC в ASP.NET Core" />
...
<div class="nav"><p><a href="./25.2.php">Назад</a><a href="./">Содержание</a><a href="./21.1.php">Вперед</a></p></div></div>
<div class="socBlock">
...
$(function() { 
  ...
  $("#toggle-theme").on("click", function() {
	...
	$("li:contains('Глава 34.')").removeClass('expandable').addClass('collapsable').children('ul').css({"display":"block"});
	$("li:contains('Глава 34.')").children('div').removeClass('expandable-hitarea').addClass('collapsable-hitarea');
