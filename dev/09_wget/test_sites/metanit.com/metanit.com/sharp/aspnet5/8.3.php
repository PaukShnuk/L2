...
<html >
<head>
<title>ASP.NET Core | Привязка модели</title>
<meta charset="utf-8" />
<meta name="description" content="Привязка модели в ASP.NET MVC Core, интерфейс IModelBinder, свзывание данных из запроса с параметрами метода">
...
<h2>Привязка модели</h2><div class="date">Последнее обновление: 29.11.2019</div>
...
<p>Допустим, на сервер приходит запрос <span class="ii">http://localhost:8000/home/index?name=volga</span>.</p>
<p>Для обслуживания данного запроса будет выбран метод <code>Index</code> контроллера <code>Home</code>. Поскольку данный метод принимает параметр с 
...
<p>То есть в нашем случае, когда на сервер придет запрос <span class="ii">http://localhost:8000/home/index?name=volga</span>, привязчик модели последовательно 
...
<p>В этом случае привязчик модели последовательно будет просматривать те же источники в поиске значений для свойств объекта myPhone. Например, чтобы найти 
значение для свойства Name, привязчик будет искать значение по следующим ключам:</p>
<ul>
<li><p><code>Request.Form["myPhone.Name"]</code></p></li>
<li><p><code>RouteData.Values["myPhone.Name"]</code></p></li>
<li><p><code>Request.Query["myPhone.Name"]</code></p></li>
</ul>
<p>В случае если параметр сам хранит объект сложного типа, как выше класс Phone ссылается на класс Company, то привязчик с помощью рекурсии спускается на уровень ниже - на уровень класса Company и 
...
<p>При этом свойства, к которым осуществляется привязка, должны быть объявлены с модификатором <span class="b">public</span> и быть доступными для записи. А сам класс должен иметь общедоступный 
...
и затем у созданного объекта будут устанавливаться свойства.</p>
