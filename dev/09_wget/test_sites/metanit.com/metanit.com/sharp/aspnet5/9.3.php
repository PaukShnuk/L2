...
<html >
<head>
<title>ASP.NET Core | Строго типизированные хелперы</title>
<meta charset="utf-8" />
<meta name="description" content="Строго типизированные хелперы форм в ASP.NET Core, Html.TextBoxFor, Html.DropDownListFor, Html.LabelFor">
...
<h2>Строго типизированные хелперы</h2><div class="date">Последнее обновление: 30.11.2019</div>
...
<p>В ASP.NET MVC Core кроме базовых html-хелперов форм мы можем использовать их двойники - строго типизированные хелперы. Этот вид хелперов принимает в 
качестве параметра лямбда-выражение, в котором указывается то свойство модели, к которому должен быть привязан данный хелпер. Важно учитывать, что 
...
тем же самым, что указан для всего представления с помощью директивы <span class="b">@model</span>.</p>
<p>Посмотрим на примере. Возьмем следующую модель User:</p>
<pre class="brush:c#;">
public class User
{
...
<p>Для создания объекта этой модели мы могли бы использовать следующую форму:</p>
<pre class="brush:xml;">
...
<p>Перепишем этот пример с использованием строго типизированных хелперов::</p>
<pre class="brush:xml;">
...
<img src="./pics/htmlhelper2.png" alt="строго типизированные хелперы в ASP.NET Core" />
<p>Строго типизированный хелпер похож на обычный, только в конце прибавляется суффикс <span class="b">For</span>: <code>LabelFor</code>. 
...
модель, которая будет использоваться: <code>@model HtmlHelpersApp.Models.User</code>. То есть, в вызове 
<code>@Html.TextBoxFor(m=&gt;m.Name)</code> параметр <code>m</code> представляет переменную модели User. А лямбда-выражение <code>m=&gt;m.Age</code> 
указывает, что данный хелпер будет генерировать поле для свойства <code>Age</code>. 
Таким образом, хелпер <code>@Html.TextBoxFor(m=&gt;m.Age, "", new { type = "number" })</code> сгенерирует поле:</p>
<pre class="brush:xml;">&lt;input data-val="true" data-val-required="The Age field is required." id="Age" name="Age" type="number" value=""&gt;</pre>
<p>Кроме стандартных атрибутов строго типизированные хелперы также добавляют атрибуты валидации типа <code>data-val</code> и <code>data-val-required</code>, которые будут рассмотрены в соответствующей теме.</p>
<p>Для каждого базового встроенного хелпера имеется свой строго типизированный хелпер:</p>
<ul>
<li><p><span class="b">Html.HiddenFor</span></p>
<p>Выражение <code>@Html.HiddenFor(m=&gt; m.Id)</code> создает разметку:</p> 
<p><code>&lt;input id="Id" name="Id" type="hidden" value="[значение_m.Id]" /&gt;</code></p></li>
<li><p><span class="b">Html.CheckBoxFor</span></p>
<p>Выражение <code>@Html.CheckBoxFor(m=&gt;m.Enable, false)</code> формирует два элемента:</p>
<pre class="brush:xml;">
&lt;input id="Enable" name="Enable" type="checkbox" value="true" /&gt;
&lt;input name="Enable" type="hidden" value="false"&gt;
</pre></li>
<li><p><span class="b">Html.LabelFor</span></p>
<p>Хелпер <code>@Html.LabelFor(m =&gt; m.Name,"Имя")</code> генерирует разметку:</p> 
<p><code>&lt;label for="Name"&gt;Имя&lt;/label&gt;</code></p></li>
<li><p><span class="b">Html.PasswordFor</span></p>
<p>Хелпер <code>@Html.PasswordFor(m =&gt; m.Password)</code> создает разметку:</p> 
<p><code>&lt;input id="Password" name="Password" type="password" /&gt;</code></p></li>
<li><p><span class="b">Html.RadioButtonFor</span></p>
<p><code>@Html.RadioButtonFor(m =&gt; m.Option, "val")</code> генерирует разметку:</p> 
<p><code>&lt;input id="Option" name="Option" type="radio" value="val" /&gt;</code></p></li>
<li><p><span class="b">Html.TextBoxFor</span></p>Выражение <p><code>@Html.TextBoxFor(m =&gt; m.Name)</code> создает разметку:</p> 
<p><code>&lt;input id="Name" name="Name" type="text" /&gt;</code></p></li>
<li><p><span class="b">Html.TextAreaFor</span></p>
<p>Хелпер <code>@Html.TextAreaFor(m =&gt; m.Name,10,9,null)</code> генерирует код:</p> 
<p><code>&lt;textarea cols="9" id="Name" name="Name" rows="10" &gt;&lt;/textarea&gt;</code></p></li>

<li><p><span class="b">Html.DropDownListFor</span></p>
<p>Создает выпадающий список select с одиночным выбором для установки свойства модели</p></li>
<li><p><span class="b">Html.ListBoxFor</span></p>
<p>Создает список select со множественным выбором для определенного свойства модели. Подходит, если свойство модели представляет массив или список</p></li>
</ul>
