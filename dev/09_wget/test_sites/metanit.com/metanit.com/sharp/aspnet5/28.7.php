...
<html >
<head>
<title>ASP.NET Core | Переключение языка приложения</title>
<meta charset="utf-8" />
<meta name="description" content="Переключение языка приложения в ASP.NET Core, определение ресурсов для атрибутов моделей, метод AddDataAnnotationsLocalization">
...
<h2>Переключение языка приложения</h2><div class="date">Последнее обновление: 18.12.2019</div>
...
<p>Ранее мы рассмотрели, как в ASP.NET Core устанавливать культуру в приложении. В частности, при использовании RequestLocalizationMiddleware мы можем задействовать 
для установки культуры три механизма: параметры строки запроса, куки и заголовок accept-language. Используем куки для установки культуры. Для этого определим 
в контроллере специальный метод <span class="b">SetLanguage</span>:</p>
<pre class="brush:c#;">
using System;
...
<p>Этот метод в качестве параметров принимает код устанавливаемой культуры и адрес, на который надо вернуться после установки культуры.</p>
<p>В самом методе происходит установка куки с помощью метода <code>Response.Cookies.Append</code>. При рассмотрении RequestLocalizationMiddleware упоминалось, 
что куки, применяемая для локализации, по умолчанию имеет имя ".AspNetCore.Culture" и формат "c=%LANGCODE%|uic=%LANGCODE%". И мы могли бы написать:</p>
<pre class="brush:c#;">Response.Cookies.Append(".AspNetCore.Culture", "c=ru|uic=ru");</pre>
<p>Но в данном случае мы используем методы провайдера CookieRequestCultureProvider.</p>
<p>Непосредственно для установки культуры на уровне пользовательского интерфейса определим выпадающий список, в котором пользователь из списка доступных языков 
сможет выбрать нужный. Для этой цели создадим частичное представление в папке <span class="ii">Views/Shared</span>, 
которое пусть называется <span class="ii">_SelectLanguagePartial.cshtml</span>, и определим в нем следующий код:</p>
<pre class="brush:c#;">
...
<p>С помощью объекта <span class="b">IOptions&lt;RequestLocalizationOptions&gt;</span> получаем список поддерживаемых культур и создаем из них список, 
который потом передается в элемент select для построения выпадающего списка.</p>
<p>Также получаем сервис <span class="b">IRequestCultureFeature</span>, который позволяет получить текущую культуру.</p>
<p>Далее встроим это представление на мастер-станицу <span class="ii">_Layout.cshtml</span>, например, где нибудь вверху перед основным содержанием:</p>
<pre class="brush:xml;">
...
<p>В частичном представлении "_SelectLanguagePartial.cshtml" мы получаем объект <span class="b">IOptions&lt;RequestLocalizationOptions&gt;</span>, 
который необходим для построения списка доступных языков. Однако на данный момент этот объект пока еще нигде не определяется и не передается в приложение. 
Поэтому изменим класс <span class="b">Startup</span> следующим образом:</p>
<pre class="brush:c#;">
...
<p>В методе <code>ConfigureServices()</code> с помощью вызова <code>services.Configure()</code> устанавливается объект <span class="b">RequestLocalizationOptions</span>, 
который затем мы сможем получить в классах приложения. В частности, этому объекту передается список поддерживаемых культур и культура по умолчанию.</p>
<p>Через менахизм DependencyInjection этот объект будет передаваться в RequestLocalizationMiddleware.</p>
<p>Для динамического переключения языков все готово. Теперь мы можем протестировать этот функционал. Пусть в представлении <span class="ii">Views/Home/Index.cshtml</span> 
с помощью сервиса IViewLocalizer выводятся значения ресурсов:</p>
<pre class="brush:xml;">
@using Microsoft.AspNetCore.Mvc.Localization
...
<p>Пусть для этого представления в папке Resources будет определено несколько файлов ресурсов:</p>
<img src="./pics/localization21.png" alt="Динамическое переключение языков в ASP.NET Core" />
<p>Каждый файл ресурсов определяет два ресурса Header и Message. Например, файл ресурсов для русскоязычной локали:</p>
<img src="./pics/localization22.png" />
<p>И файл для английского языка:</p>
<img src="./pics/localization23.png" />
<p>Запустим приложение и выберем из списка русский язык:</p>
<img src="./pics/localization24.png" />
<p>После выбора любого другого языка интерфейс переключится на нужную локаль, при условии если для нее имеются файл ресурсов. 
Например, переключимся на английский язык:</p>
<img src="./pics/localization25.png" />
<p>Таким образом, мы можем создать простейший механизм для переключения языков в приложении ASP.NET Core MVC.</p>
...
<div class="nav"><p><a href="./28.6.php">Назад</a><a href="./">Содержание</a><a href="./28.8.php">Вперед</a></p></div></div>
<div class="socBlock">
