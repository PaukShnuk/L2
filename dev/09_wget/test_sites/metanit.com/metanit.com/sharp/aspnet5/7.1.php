...
<html >
<head>
<title>ASP.NET Core MVC | Представления</title>
<meta charset="utf-8" />
<meta name="description" content="Представления views в ASP.NET Core MVC, пути к представлениям, создание новых представлений">
...
<h1>Представления</h1><h2>Введение в представления</h2><div class="date">Последнее обновление: 18.11.2019</div>
...
<p>В большинстве случаев при обращении к веб-приложению пользователь ожидает получить веб-страницу с какими-нибудь данными. В MVC для этого, как 
...
от кода html к коду на языке C#.</p>
<p>Например, возьмем простейшее представление:</p>
<pre class="brush:xml;">
@{
    Layout = null;
}
...
<p>Данное представление напоминает обычную страницу html. Здесь могут быть определены все стандартные элементы разметки html, здесь могут подключаться стили, скрипты. 
...
html-страниц, которые видит пользователь в своем браузере.</p>
<p>Для хранения представлений в проекте ASP.NET MVC предназначена папка <span class="ii">Views</span>:</p>
<img src="./pics/views2.png" alt="Представления в ASP.NET MVC Core" />
<p>В этой папке уже есть некоторая подструктура. Во-первых, как правило, для каждого контроллера в проекте создается подкаталог в папке Views, который называется по имени 
...
Views есть подкаталог Home с представлениями для методов контроллера HomeController.</p>
<p>Также здесь есть папка <span class="b">Shared</span>, которая хранит общие представления для всех контроллеров. 
По умолчанию это файлы <span class="b">_Layout.cshtml</span> (используется в качестве мастер-страницы), <span class="b">Error.cshtml</span> 
(использутся для отображения ошибок) и <span class="b">_ValidationScripsPartial.cshtml</span> 
(частичное представление, которое подключает скрипты валидации формы).</p>
<p>И в корне каталога <span class="ii">Views</span> также можно найти два файла <span class="b">_ViewImports.cshtml</span> и <span class="b">_ViewStart.cshtml</span>. Эти файлы 
содержат код, который автоматически добавляется ко всем представлениям. <span class="ii">_ViewImports.cshtml</span> устанавливает некоторые общие для всех представлений пространства имен, 
а <span class="ii">_ViewStart.cshtml</span> устанавливает общую мастер-страницу.</p>
<p>При необходимости мы можем добавлять в каталог Views какие-то свои представления, каталоги для представлений. И они необязательно должны быть связаны с контроллерами и их методами. 
Для добавления представления нужно правой кнопкой мыши на подкаталог в папке Views (или на саму папку Views) и в контекстном меню выбрать <span class="b">Add -&gt; New Item</span>. Затем в появившемся 
окне добавления нового элемента выбрать компонент <span class="b">Razor View</span>:</p>
<img src="./pics/views3.png" alt="Добавление представления в ASP.NET MVC Core" />
<h3>ViewResult</h3>
<p>За работу с представлениями отвечает объект <span class="b">ViewResult</span>. Он производит рендеринг представления в веб-страницу и 
возвращает ее в виде ответа клиенту.</p>
<p>Чтобы возвратить объект ViewResult используется метод <span class="b">View</span>:</p>
<pre class="brush:c#;">
...
<p>Вызов метода View возвращает объект ViewResult. Затем уже ViewResult производит рендеринг определенного представления в ответ. По умолчанию контроллер 
производит поиск представления в проекте по следующим путям:</p>
<pre>
/Views/Имя_контроллера/Имя_представления.cshtml
/Views/Shared/Имя_представления.cshtml
</pre>
<p>Согласно настройкам по умолчанию, если название представления не указано явным образом, то в качестве представления будет использоваться то, имя которого 
...
<i>/Views/Home/</i>.</p>
<p>Метод <code>View()</code> имеет четыре перегруженных версии:</p>
<ul>
<li><p><code>View()</code>: для генерации ответа используется представление, которое по имени совпадает с вызывающим методом</p></li>
<li><p><code>View(string viewName)</code>: в метод передается имя представления, что позволяет переопределить используемое по умолчанию представление</p></li>
<li><p><code>View(object model)</code>: передает в представление данные в виде объекта model</p></li>
<li><p><code>View(string viewName, object model)</code>: переопределяет имя представления и передает в него данные в виде объекта model</p></li>
</ul>
<p>Вторая версия метода позволяет переопределить используемое представление. Если представление находится в той же папке, которая предназначена для данного контроллера, 
то в метод View() достаточно передать название представления без расширения:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
    public IActionResult Index()
...
<p>В этом случае метод Index будет использовать представление <i>Views/Home/About.cshtml</i>. Если же представление находится в другой папке, 
то нам надо передать полный путь к представлению:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
    public IActionResult Index()
...
<div class="nav"><p><a href="./5.10.php">Назад</a><a href="./">Содержание</a><a href="./7.2.php">Вперед</a></p></div></div>
<div class="socBlock">
