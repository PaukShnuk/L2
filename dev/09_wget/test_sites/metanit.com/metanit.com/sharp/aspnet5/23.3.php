...
<html >
<head>
<title>ASP.NET Core | Тестирование контроллера Web API</title>
<meta charset="utf-8" />
<meta name="description" content="Тестирование контроллера Web API в ASP.NET Core MVC, использование PowerShell и Fiddler">
...
<h2>Тестирование контроллера</h2><div class="date">Последнее обновление: 12.12.2019</div>
...
<p>В прошлой теме был создан контроллер Web API, и протестирована работа метода GET. Однако напрямую из строки браузера кроме запросов GET другие типы 
...
консольного десктопного приложения, но создание клиента может занять довольно много времени, тогда как нам просто надо протестировать обработку запросов.</p>
<p>Для тестирования контроллера Web API можно применять специальные инструменты, которые устанавливаются в виде отдельных приложений, либо в виде 
расширений для браузеров, например, <span class="b">Fiddler</span> или <span class="b">Postman</span>. 
Однако Visual Studio предоставляет еще один способ - ввод команд в окне <span class="b">Package Manager Console</span>, которое для отправки запросов использует оболочку PowerShell.</p>
<h3>Использование PowerShell</h3>
<h4>Тестирование GET-запросов</h4>
<p>Например, в моем случае приложение запускается по адресу <span class="ii">http://localhost:54427</span>. Поэтому для тестирования GET-запроса на получение 
списка объектов от контроллера UsersController я должен ввести в Package Manager Console следующую команду:</p>
<pre>Invoke-RestMethod http://localhost:54427/api/users -Method GET</pre>
<img src="./pics/webapi8.png" alt="Тестирование контроллера Web API в ASP.NET Core" />
<p>Для тестирования другого GET-метода, который возвращает пользователя по id, введем другой запрос:</p>
<pre>Invoke-RestMethod http://localhost:54427/api/users/1 -Method GET</pre>
<p>И Package Manager Console выведет:</p>
<pre>
id name age
-- ---- ---
 1 Tom   26
</pre>
<h4>Тестирование POST-запросов</h4>
<p>Для тестирования POST-запроса необходимо передать объект в формате json с указанием некоторой дополнительной информацией. В моем случае команда 
выглядела бы так:</p>
<pre>
...
<p>Аргумент <code>-Body</code> в этой команде указывает на тело запроса - тот объект, который будет отправляться на сервер. В моем случае это объект 
класса User, поэтому все его значения имеют ключи, которые соответствуют названиям свойств класса User. 
Кроме того, указываем с помощью флага <code>ConvertTo-Json</code>, что объект будет отправляться в формате JSON. 
А дополнительный аргумент <code>-ContentType</code> устанавливает в запросе заголовок <code>Content-Type</code>. В итоге эта команда возвратит 
...
<p>После этого мы можем повторно получить список пользователей и увидеть в нем добавленный объект:</p>
<img src="./pics/webapi9.png" alt="Тестирование POST запросов в Visual Studio" />
<h4>Тестирование PUT-запросов</h4>
<p>Для тестирования PUT-запроса передается так же объект, только в данном случае уже надо указать id редактируемого объекта:</p>
<pre>Invoke-RestMethod http://localhost:54427/api/users -Method PUT -Body (@{id = 3; name = "Bob Marley"; age = 44} | ConvertTo-Json) -ContentType "application/json"</pre>
<img src="./pics/webapi10.png" alt="Тестирование PUT запросов в Visual Studio" />
<h4>Тестирование запросов DELETE</h4>
<p>В запросе DELETE необходимо передать id удаляемого объекта:</p>
<pre>Invoke-RestMethod http://localhost:54427/api/users/3 -Method DELETE</pre>
<img src="./pics/webapi11.png" alt="Тестирование DELETE запросов в Visual Studio" />
<h3>Fiddler</h3>
<p>Теперь рассмотрим, как использовать <a href="http://www.telerik.com/fiddler" rel="nofollow">Fiddler</a>. Загрузим установочный пакет по ссылке и установим.</p>
<p>После установки запустим проект в Visual Studio на выполнение. Параллельно откроем программу Fiddler и перейдем на вкладку <span class="b">Composer</span>. 
...
то в моем случае это будет запрос GET и адрес <span class="ii">http://localhost:54427/api/users/</span>.</p>
<img src="./pics/fiddler1.png" alt="Тестирование запросов в Fiddler" />
<p>После ввода адреса нажмем на кнопку Execute. После осуществления запроса в левом поле-списке запросов выберем сделанный только что запрос, и 
на вкладке <span class="b">Inspectors</span> можно будет увидеть результат запроса - список пользователей, который Fiddler получает в сериализованном виде:</p>
<img src="./pics/fiddler2.png" alt="Fiddler в ASP.NET Core Web API" />
...
<div class="nav"><p><a href="./23.2.php">Назад</a><a href="./">Содержание</a><a href="./23.4.php">Вперед</a></p></div></div>
<div class="socBlock">
