...
<html >
<head>
<title>ASP.NET Core | Изменение пароля</title>
<meta charset="utf-8" />
<meta name="description" content="Изменение пароля в ASP.NET Core Identity, создание хеша пароля и его валидация с помощью объектов IPasswordHasher и IPasswordValidator">
...
<h2>Изменение пароля</h2><div class="date">Последнее обновление: 15.12.2019</div>
...
<p>В прошлой теме было рассмотрено редактирование пользователя. Однако процесс редактирования не включал изменение пароля. В этой же теме рассмотрим, как 
отдельно сменить пароль пользователя.</p>
<p>Вначале создадим модель представления ChangePasswordViewModel:</p>
<pre class="brush:c#;">
...
<p>Здесь определены свойства для Id и электронной почты пользователя, а также для нового пароля.</p>
<p>Далее добавим в контроллер UsersController, который был создан в прошлой теме, новое действие ChangePassword:</p>
<pre class="brush:c#;">
...
<p>Таблица пользователей не хранит оригинальный пароль, а вместо него хранит хеш пароля. Для его получения мы можем воспользоваться методом 
<code>HashPassword()</code> сервиса <span class="b">IPasswordHasher</span>, который добавляется вместе со всеми сервисами ASP.NET Core Identity через механизм внедрения зависимостей.</p>
<p>Однако перед созданием хеша нам надо валидировать пароль с помощью валидатора. Валидатор пароля также можно получить из коллекции сервисов приложения 
в виде объекта <span class="b">IPasswordValidator</span>.</p>
<p>Затем добавим представление <span class="ii">ChangePassword.cshtml</span>:</p>
<pre class="brush:xml;">
...
<p>И в конце добавим на представление <span class="ii">Index.cshtml</span> ссылку для смены пароля:</p>
<pre class="brush:xml;">
...
<img src="./pics/customidentity12.png" alt="Смена пароля в ASP.NET Core Identity" />
<h3>Второй вариант</h3>
<p>В предыдущем случае мы явным образом создавали хеш пароля и обновляли пользователя. Однако мы можем воспользоваться и другим методом - 
методом <code>ChangePasswordAsync()</code> класса UserManager. Он в качестве параметра принимает объект пользователя, старый и новый пароль. То есть, 
чтобы сменить пароль на новый, нам надо указать также и старый, что может быть целесообразно в различных ситуациях.</p>
<p>Для этого изменим модель ChangePasswordViewModel:</p>
<pre class="brush:c#;">
public class ChangePasswordViewModel
{
    public string Id { get; set; }
...
<p>Изменим post-версию метода ChangePassword:</p>
<pre class="brush:c#;">
[HttpPost]
public async Task&lt;IActionResult&gt; ChangePassword(ChangePasswordViewModel model)
{
...
<p>И соответственно изменим представление <span class="ii">ChangePassword.cshtml</span>:</p>
<pre class="brush:xml;">
@model CustomIdentityApp.ViewModels.ChangePasswordViewModel
@{
    ViewBag.Title = "Изменение пароля";
...
<div class="nav"><p><a href="./16.7.php">Назад</a><a href="./">Содержание</a><a href="./16.9.php">Вперед</a></p></div></div>
<div class="socBlock">
