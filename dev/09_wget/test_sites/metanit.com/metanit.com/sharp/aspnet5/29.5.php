...
<html >
<head>
<title>ASP.NET Core | Параметры маршрутов в Razor Pages</title>
<meta charset="utf-8" />
<meta name="description" content="Параметры маршрутов в Razor Pages в ASP.NET Core, атрибут BindProperty, ограничения параметров">
...
<h2>Параметры маршрутов в Razor Pages</h2><div class="date">Последнее обновление: 05.12.2019</div>
...
<p>Кроме получения данных из параметры запроса и из отправленных форм мы также можем получать данные из параметров маршрута. И в данном случае 
важно понимать отличие параметров строки запроса от параметров маршрута. Например:</p>
<pre class="brush:c#;">
localhost:5000/Index?id=23
localhost:5000/Index/23
</pre>
<p>Параметры, которые идут после вопросительного знака в url представляют параметры строки запроса. А данные, которые разделены в url слешами, представляют 
параметры маршрута. Один url может содержать как параметры маршрута, так и параметры запроса.</p>
...
<p>К примеру, у нас есть страница Person.cshtml. Используем на ней параметр id:</p>
<pre class="brush:c#;">
@page "{id}"
@{
...
<p>С помощью свойства <code>RouteData.Values</code>, как и в контроллерах MVC, мы можем получить параметры маршрута.</p>
<img src="./pics/razorpages13.png" alt="Параметры маршрута в Razor Pages в ASP.NET Core" />
<p>Также мы можем получить параметры маршрута через одноименные параметры метода OnGet:</p>
<pre class="brush:c#;">
@page "{id}"
@functions{
...
<p>Либо если мы используем PageModel, то мы могли бы также получить параметр маршрута через OnGet:</p>
<pre class="brush:c#;">
...
<p>Тогда в коде страницы Person.cshtml достаточно получить свойство Id у модели:</p>
<pre class="brush:xml;">
@page "{id}"
@model PersonModel
&lt;h2&gt;Id = @Model.Id&lt;/h2&gt;
...
<p>В данном случае получаем три параметра id, name и age. Изменим код PersonModel, чтобы получить эти параметры:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Mvc.RazorPages;
...
<img src="./pics/razorpages14.png" alt="Получение параметров маршрутов в ASP.NET Core UserPages" />
...
<p>В данном случае параметры id и age должны обязательно представлять число, а параметр name может содержать только алфавитные символы, причем не менее трех символов. 
Подробнее про ограничения маршрутов в MVC можно посмотреть в статье <a href="11.3.php">Ограничения маршрутов</a>.</p>
...
<div class="nav"><p><a href="./29.4.php">Назад</a><a href="./">Содержание</a><a href="./29.6.php">Вперед</a></p></div></div>
<div class="socBlock">
