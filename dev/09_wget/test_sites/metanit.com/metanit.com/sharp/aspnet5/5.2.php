...
<html >
<head>
<title>ASP.NET Core | Передача данных в контроллер</title>
<meta charset="utf-8" />
<meta name="description" content="Передача данных в контроллер ASP.NET Core MVC, отправка массивов и сложных объектов">
...
<h2>Передача данных в контроллер</h2><div class="date">Последнее обновление: 17.11.2019</div>
...
<p>Определение в методах контроллера параметров ничем не отличается от определения параметров в языке C#. Параметры могут представлять примитивные типы, как int или string, а могут 
представлять и более сложные классы.</p>
<p>Передавать значения для параметров можно различными способами. При отправке GET-запроса значения передаются через строку запроса. Стандартный get-запрос принимает примерно следующую форму: <i>название_ресурса?параметр1=значение1&параметр2=значение2</i>. 
Например, определим следующий метод:</p>
<pre class="brush:c#;">
...
<p>Чтобы передать значение для параметра id, нам надо отправить запрос типа <i>http://localhost:57086/Home/Hello?id=9</i>:</p>
<img src="./pics/controllers4.png" alt="Параметры метода контроллера в ASP.NET MVC Core" />
<p>Если метод принимает несколько параметров:</p>
<pre class="brush:c#;">
...
<i>Home/Square/</i> параметры a и h будут равны 0. Но на случай подобной ситуации мы можем использовать параметры по умолчанию, которые будут работать, 
...
<p>Система привязки MVC, которую мы позже рассмотрим, сопоставляет параметры запроса и параметры метода по имени. То есть, если в строке запроса 
идет параметр <code>a</code>, то его значение будет передаваться именно параметру метода, который также называется <code>a</code>. При этом должно быть также 
...
Например, определим рядом с контроллером класс Geometry:</p>
<pre class="brush:c#;">
...
<p>Класс Geometry определяет два свойства и метод для подсчета площади. И теперь в контроллере метод Area принимает параметр типа Geometry. Как в этом случае мы 
можем передать контроллеру данные? Для этого нам надо отправить запрос наподобие следующего <i>http://localhost:57086/Home/Area?altitude=10&height=3</i>. 
Здесь параметры строки запроса должны соответствовать по имени свойствам объекта. Регистр названий при этом не учитывается:</p>
<img src="./pics/controllers5.png" />
<h3>Передача массивов</h3>
<p>Допустим, метод принимает массив чисел:</p>
<pre class="brush:c#;">
...
<p>Чтобы передать значения для массива, нам надо использовалась строку запроса наподобие <i>http://localhost:57086/Home/Sum?nums=1&nums=2&nums=3</i>. 
В этом случае в массиве nums окажется три элемента.</p>
<p>Теперь изменим метод Sum, чтобы он принимал массив объектов ранее созданного класса Geometry:</p>
<pre class="brush:c#;">
...
<p>Данный метод подсчитывает сумму всех площадей в массиве geoms. И чтобы передать в этот метод данные, нам надо использовать запрос типа 
<i>http://localhost:57086/Home/Sum?geoms[0].altitude=10&geoms[0].height=3&geoms[1].altitude=16&geoms[1].height=2</i>. 
В этом случае в массиве geoms будут два элемента Geometry.</p>
<h3>Передача данных в запросе POST</h3>
...
<p>Для передачи POST-запросов определим в представлении Index.cshtml, которое должно быть по умолчанию в папке Views/Home, простенькую форму:</p>
<pre class="brush:xml;">
...
<p>Форма устанавливает метод отправки - post, адрес отправки - Home/Area и два поля ввода чисел.</p>
<p>И чтобы метод Area принимал отправляемые данные, нам надо изменить его код следующим образом:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
	public IActionResult Index()
...
<p>Чтобы система могла связать параметры метода и данные формы, необходимо, чтобы атрибуты <code>name</code> у полей формы соответствовали названиям параметров.</p>
<p>Причем здесь действуют те же правила привязки, поэтому мы также может с той же формы получать более сложные объекты:</p>
<pre class="brush:c#;">
[HttpPost]
public string Area(Geometry geometry)
{
...
у которого можно получить как данные строки запроса, так и данные отправленных форм.</p>
<p>Данные строки запроса доступны через свойство <span class="b">Request.Query</span>, которое представляет объект <code>IQueryCollection</code>. 
Например:</p>
<pre class="brush:c#;">
public string Area()
{
...
<p>В данном случае метод <code>Area</code> обрабатывает GET-запросы, и мы можем к нему обратиться через запрос типа <i>http://localhost:57086/Home/Area?altitude=20&height=4</i>.</p>
<p>Для получения данных отправленных форм можно использовать свойство <span class="b">Request.Form</span>. Это свойство представляет объект <code>IFormsCollection</code>, 
но работает аналогично Request.Query:</p>
<pre class="brush:c#;">
[HttpPost]
public string Area()
{
