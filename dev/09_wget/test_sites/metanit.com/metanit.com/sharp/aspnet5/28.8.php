...
<html >
<head>
<title>ASP.NET Core | Общие ресурсы локализации</title>
<meta charset="utf-8" />
<meta name="description" content="Общие ресурсы локализации приложения в ASP.NET Core, SharedResource">
...
<h2>Общие ресурсы локализации</h2><div class="date">Последнее обновление: 18.12.2019</div>
...
<p>В прошлых темах для каждой сущности использовались свои ресурсы: для представлений - одни, для контроллеров - другие, для атрибутов модели - третьи. 
Но также мы можем использовать и некоторые общие ресурсы для различных сущностей. Для этого добавим в корень проекта простой класс 
SharedResource:</p>
<pre class="brush:c#;">
...
<p>Этот класс будет пустым.</p>
<p>Затем определим для него общие файлы ресурсов. Для этого в проекте в папку Resources добавим несколько файлов ресурсов для тех языков, которые мы хотим использовать. 
При этом названия файла ресурсов должны соответствовать пути к файлу SharedResource в рамках проекта. Поскольку этот класс находится в корне проекта, 
то файлы ресурсов будут иметь имя <span class="ii">SharedResource.[код_культуры].resx</span>. Путь к данным файлам ресурсов должен отражать путь к классу SharedResource 
внутри проекта (как и в случае с ресурсами для контроллеров и представлений).</p>
<p>Например, в моем случае это будут три файла:</p>
<ul>
<li><p>SharedResource.ru.resx</p></li>
<li><p>SharedResource.en.resx</p></li>
<li><p>SharedResource.de.resx</p></li>
</ul>
<img src="./pics/localization26.png" alt="Общие ресурсы локализации в ASP.NET Core" />
<p>Например, тестовый файл ресурсов для русскоязычной локали:</p>
<img src="./pics/localization27.png" />
<p>И тестовый файл для английского языка:</p>
<img src="./pics/localization28.png" />
<p>Теперь как мы эти ресурсы можем использовать в приложении? В контроллере мы можем передать данные ресурсы через конструктор 
посредством параметра типа IStringLocalizer&lt;SharedResource&gt;:</p>
<pre class="brush:c#;">
...
<p>В представлении мы можем передать ресурсы через сервис типа <span class="b">IHtmlLocalizer&lt;SharedResource&gt;</span>:</p>
<pre class="brush:xml;">
...
<p>Для применения ресурсов к аннотациям данных надо изменить вызов метода <code>AddDataAnnotationsLocalization()</code> в классе Startup:</p>
<pre class="brush:c#;">
...
<p>Ключевым моментом здесь является создание объекта IStringLocalizer для ресурса SharedResource с помощью фабрики IStringLocalizerFactory 
в методе AddDataAnnotationsLocalization:</p>
<pre class="brush:c#;">
services.AddControllersWithViews()
		.AddDataAnnotationsLocalization(options =&gt; {
			options.DataAnnotationLocalizerProvider = (type, factory) =&gt;
...
<p>Таким образом, мы охватим локализацией из общего ресурса контроллеры, представления и аннотации данных.</p>
<p>И если мы обратимся к выше указанному в контроллере действию Test и передадим ему определенную культуру, то он будет использовать соответствующие ресурсы:</p>
<img src="./pics/localization29.png" alt="Локализация с SharedResource в ASP.NET Core MVC" />
...
<div class="nav"><p><a href="./28.7.php">Назад</a><a href="./">Содержание</a><a href="./28.9.php">Вперед</a></p></div></div>
<div class="socBlock">
