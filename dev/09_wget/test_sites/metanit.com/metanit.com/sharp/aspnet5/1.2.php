...
<html >
<head>
<title>ASP.NET Core | Начало работы</title>
<meta charset="utf-8" />
<meta name="description" content="Начало работы с ASP.NET Core, установка Visual Studio 2019, настройка среды ASP.NET, знакомство со структурой проекта">
...
<h2>Начало работы с ASP.NET Core</h2><div class="date">Последнее обновление: 15.12.2020</div>
...
<p>Для разработки под ASP.NET Core мы можем использовать различный инструментарий. Если нашей рабочей платформой является Windows, 
...
<span class="b">Visual Studio Code</span>. Данный редактор также может работать и под Windows. В рамках данного руководства преимущественно 
будет использоваться среда Visual Studio 2019.</p> 
<p>Программу для установки Visual Studio 2019 можно загрузить со страницы <a href="https://www.visualstudio.com/downloads/" rel="nofollow">https://www.visualstudio.com/downloads/</a>. 
В данном случае не важно, какой выпуск VS использовать - бесплатный Community или платные Professional или Enterprise. Все эти выпуски имеют встроенные средства 
для создания приложений на ASP.NET Core. В рамках этого руководства будет использоваться бесплатный выпуск VS 2019 Community.</p>
<p>Итак, загрузим установщик VS 2019 и запустим его. Вначале нам предлагается установить ряд опций. И так как мы будем работать с ASP.NET Core, то выбрать в программе для 
установке пункт <span class="b">ASP.NET и разработка веб-приложений</span>:</p>
<img src="./pics/1.6.png" alt="Установка ASP.NET для Visual Studio 2019" />
<p>Также при выборе этого пункта в поле справа можно выбрать также необязательные компоненты, которые будут устанавливаться вместе с ASP.NET. Можно выбрать все компоненты.</p>
<p>Кроме того, чуть ниже также в программе установщика нам надо выбрать другой пункт <span class="b">Кроссплатформенная разработка .NET Core</span>:</p>
<img src="./pics/1.7.png" alt="Установка .NET Core для Visual Studio 2019" />
<p>Отметив все необходимые нам опции, выполним установку Visual Studio.</p>
<p>После установки откроем Microsoft Visual Studio 2019 и при создании проекта выберем пункт <span class="b">ASP.NET Core Web Application</span> - 
тип проекта для создания веб-приложения ASP.NET Core:</p>
<img src="./pics/1.1.png" alt="Создание первого проекта ASP.NET Core" />
<p>На следующем шаге дадим какое-нибудь имя проекту, например, HelloApp, и определим для него местоположение на жестком диске:</p>
<img src="./pics/1.8.png" alt="Первый проект ASP.NET Core 5.0" />
<p>После этого отобразится окно выбора шаблона нового приложения:</p>
<img src="./pics/1.2.png" alt="Типы проектов ASP.NET Core" />
<p>Здесь нам доступно несколько типов проектов:</p>
<ul>
<li><p><span class="b">ASP.NET Core Empty</span>: пустой шаблон с самой минимальной функциональностью для создания приложений с нуля</p></li>
<li><p><span class="b">ASP.NET Core Web API</span>: проект веб-приложения, который использует архитектуру REST для создания веб-сервиса</p></li>
<li><p><span class="b">ASP.NET Core Web App</span>: проект, который для обработки запросов по умолчанию использует Razor Pages</p></li>
<li><p><span class="b">ASP.NET Core Web App(Model-View-Controller)</span>: проект, который использует архитектуру MVC</p></li>
<li><p><span class="b">ASP.NET Core with Angular</span>: проект, предназначенный специально для работы с Angular 2+.</p></li>
<li><p><span class="b">ASP.NET Core with React.js</span>: проект, который использует React.JS</p></li>
<li><p><span class="b">ASP.NET Core with React.js and Redux</span>: проект, который использует React.JS и Redux</p></li>
</ul>
<p>Кроме того, здесь мы можем указать версию ASP.NET Core в выпадающем списке, но в данном случае оставим значение по умолчанию - ASP.NET Core 5.0.</p>
<p>Также здесь можно указать тип аутентификации, который по умолчанию используется в проекте, и подключить контейнер Docker.</p>
<p>Также здесь есть флажок "Configure for HTTPS". При установке этого флажка проект при отладке и тестировании по умолчанию будет запускаться по протоколу HTTPS. В данном случае 
установка и неустановка этого флажка не имеет значения. Кроме того, даже если мы установили эту отметку, то впоследствии через свойства проекта можно отменить запуск через HTTPS или, наоборот, заново установить.</p>
<p>Среди этих шаблонов выберем <span class="b">ASP.NET Core Empty</span>, все остальные значения оставим по умолчанию и нажмем на кнопку ОК. И Visual Studio создает новый проект:</p>
<img src="./pics/1.3.png" alt="Начало работы с ASP.NET Core" />
<h3>Структура проекта ASP.NET Core</h3>
<p>Рассмотрим базовую структуру стандартного проекта ASP.NET Core. Проект <span class="b">ASP.NET Core Empty</span> содержит очень простую структуру - необходимый минимум для запуска приложения:</p>
<ul>
<li><p><span class="b">Connected Services</span>: подключенные сервисы из Azure</p></li>
<li><p><span class="b">Dependencies</span>: все добавленные в проект пакеты и библиотеки, иначе говоря зависимости</p></li>
<li><p><span class="b">Properties</span>: узел, который содержит некоторые настройки проекта. В частности, в файле launchSettings.json описаны 
настройки запуска проекта, например, адреса, по которым будет запускаться приложение.</p></li>
<li><p><span class="b">appsettings.json</span>: файл конфигурации проекта в формате json</p>
<li><p><span class="b">Program.cs</span>: главный файл приложения, с которого и начинается его выполнение. Код этого файла настривает и запускает веб-хост, в рамках которого разворачивается приложение</p></li>
<li><p><span class="b">Startup.cs</span>: файл, который определяет класс Startup и который содержит логику обработки входящих запросов</p></li>
</ul>
<p>Данная структура, конечно, не представляет проект полнофункционального приложения. И если мы запустим проект, то в браузере увидим только строку "Hello World!", 
которая отправляется в ответ клиенту с помощью класса Startup:</p>
<img src="./pics/1.4.png" alt="Первое приложение на ASP.NET Core" />
<p>При создании других типов проектов ASP.NET структура будет отличаться, соответственно начальный проект будет иметь больше функционала, однако это тот каркас, от которого мы можем отталкиваться, 
добавляя в него какие-то свои файлы и папки.</p>
