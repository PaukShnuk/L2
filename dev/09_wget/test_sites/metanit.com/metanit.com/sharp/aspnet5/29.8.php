...
<html >
<head>
<title>ASP.NET Core | Переадресация и создание ссылок в Razor Pages</title>
<meta charset="utf-8" />
<meta name="description" content="Переадресация, генерация адресов и ссылок в Razor Pages в ASP.NET Core, метод RedirectToPage и Url.Page">
...
<h2>Переадресация и создание ссылок в Razor Pages</h2><div class="date">Последнее обновление: 05.12.2019</div>
...
<p>Для переадресации на страницу Razor применяется метод <span class="b">RedirectToPage()</span>. В этот метод передается относительный путь к 
странице. При этом передаваемый путь объединяется с путем к текущей странице, из которой вызывается данный метод.</p>
<p>Например, текущая страница расположена по пути <span class="ii">Pages/Products/Create.cshtml</span>.</p>
<img src="./pics/razorpages32.png" alt="Переадресация в Razor Pages в ASP.NE Core" />
<p>И из нее вызывается метод RedirectToPage, к примеру:</p>
<pre class="brush:c#;">
...
<p>Рассмотрим некоторые возможные сопоставления передаваемых путей и страниц Razor:</p>
<table>
<tr><td><p>Вызов метода</p></td><td><p>На какую страницу ведет</p></td></tr>
<tr><td><p>RedirectToPage("/Index")</p></td><td><p><span class="ii">Pages/Index</span></p></td></tr>
<tr><td><p>RedirectToPage("./Index")</p></td><td><p><span class="ii">Pages/Products/Index</span></p></td></tr>
<tr><td><p>RedirectToPage("../Index")</p></td><td><p><span class="ii">Pages/Index</span></p></td></tr>
<tr><td><p>RedirectToPage("Index")</p></td><td><p><span class="ii">Pages/Products/Index</span></p></td></tr>
<tr><td><p>RedirectToPage("/Edit")</p></td><td><p><span class="ii">Pages/Edit</span></p></td></tr>
<tr><td><p>RedirectToPage("Edit")</p></td><td><p><span class="ii">Pages/Products/Edit</span></p></td></tr>
</table>

<h3>Генерация адреса на страницу</h3>
<p>Для генерации адреса на страницу Razor применяется метод <span class="b">Url.Page()</span>. Этот метод имеет ряд перегрузок, позволяющих задать различные параметры. 
Некоторые из версий:</p>
<pre class="brush:c#;">
...
<p>Параметр <code>pageName</code> указывает на путь к странице. Параметр <code>pageHandler</code> представляет обработчик, определенный для страницы. 
А параметр <code>values</code> представляет параметры строки запроса, которые передаются странице.</p>
<p>Например, страница Person принимает два параметра:</p>
<pre class="brush:c#;">
...
<p>Создание в адреса на эту страницу с последующей переадресацией в коде C#:</p>
<pre class="brush:c#;">
...
<p>Здесь полагается, что станица Index находится в одном каталоге со страницей Person. То есть в данном случае будет сгенерирован адрес <span class="ii">Person?name=Tom&amp;age=34</span>.</p>
<p>Создание адреса на страницу в представлении:</p>
<pre class="brush:xml;">&lt;a href='@Url.Page("/Person", new {name="Bob", age=32 })'&gt;Person&lt;/a&gt;</pre>
<h3>Создание ссылок</h3>
<p>Для создания ссылки в тег-хелпере AnchorTagHelper применяется параметр <span class="b">asp-page</span>. Например, ссылка на ту же страницу Person:</p>
<pre class="brush:xml;">&lt;a asp-page="Person" asp-route-name="Bob" asp-route-age="32">Person&lt;/a&gt;</pre>
<p>Дополнительно с помощью параметра <span class="b">asp-handler</span> можно указать обработчик на странице.</p>
...
<div class="nav"><p><a href="./29.7.php">Назад</a><a href="./">Содержание</a><a href="./29.9.php">Вперед</a></p></div></div>
<div class="socBlock">
