...
<html >
<head>
<title>ASP.NET Core | Контроллеры</title>
<meta charset="utf-8" />
<meta name="description" content="Контроллеры в ASP.NET Core MVC, действия контроллеров, типы запросов">
...
<h1>Контроллеры</h1><h2>Контроллеры и их действия</h2><div class="date">Последнее обновление: 17.11.2019</div>
...
<p>Центральным звеном в архитектуре ASP.NET Core MVC является контроллер. При получении запроса система маршрутизации выбирает для обработки запроса нужный контроллер 
и передает ему данные запроса. Контроллер обрабатывает эти данные и посылает обратно результат обработки.</p>
<p>В ASP.NET Core MVC контроллер представляет обычный класс на языке C#, который наследуется от абстрактного базового класса <span class="b">Microsoft.AspNetCore.Mvc.Controller</span>. 
По умолчанию проект ASP.NET Core MVC содержит как минимум один контроллер - HomeController:</p>
<pre class="brush:c#;">
...
<p>В данном случае мы видим, что констроллер имеет конструктор, через который посредством механизма dependency injection передается сервис ILogger, используемый для 
логгирования. Также контроллер определяет три метода - Index, Privacy и Error.</p>
<p>При использовании контроллеров существуют некоторые условности. Во-первых, в проекте контроллеры помещаются в каталог <span class="ii">Controllers</span>. 
И во-вторых, по соглашениям об именовании названия контроллеров обычно оканчиваются на суффикс "Controller", 
остальная же часть до этого суффикса считается именем контроллера, например, HomeController. Но в принципе эти условности необязательны.</p>
<p>Но есть также и обязательные условности, которые предъявляются к контроллерам. В частности, класс контроллера должен удовлетворять как минимум одному из следующих 
условий:</p>
<ul>
<li><p>Класс контроллера имеет суффикс "Controller"</p>
<pre class="brush:c#;">
...
<li><p>Класс контроллера наследуется от класса, который имеет суффикс "Controller"</p>
<pre class="brush:c#;">
...
<li><p>К классу контроллера  применяется атрибут <code>[Controller]</code></p>
<pre class="brush:c#;">
[Controller]
public class Home
{
...
<p>Если нам нужен еще один контроллер, то мы можем добавить в папку <span class="b">Controllers</span> новый класс, который будет наследоваться от класса <code>Controller</code>. Либо мы 
можем использовать готовый шаблон <span class="b">Controller Class</span>:</p>
<img src="./pics/controllers2.png" alt="Добавление контроллера в ASP.NET Core" />
<p>Контроллер, как и любой класс на языке C#, может иметь поля, свойства, методы. По умолчанию HomeController имеет четыре метода, которые можно назвать 
<span class="b">действиями</span>. Действия контроллера - это публичные методы, которые могут сопоставляться с запросами. Например, стандартный контроллер 
содержит метод Index - он имеет модификатор public и поэтому может использоваться для обработки запроса.</p>
<p>Чтобы обратиться контроллеру из веб-браузера, нам надо в адресной строке набрать <span class="b">адрес_сайта/Имя_контроллера/Действие_контроллера</span>. 
Так, по запросу <span class="b">адрес_сайта/Home/Index</span> система маршрутизации по умолчанию вызовет метод Index контроллера HomeController для обработки 
входящего запроса. Например:</p>
<img src="./pics/controllers1.png" alt="Обращение к методу контроллера ASP.NET MVC Core" />
<p>Однако такое сопоставление строки url с названием контроллера и его метода происходит благодаря системе маршрутизации. Если мы обратимся к классу Startup и 
его методу <code>Configure()</code>, то мы можем найти там определение единственного для приложения маршрута:</p>
<pre class="brush:c#;">
...
<p>Метод <code>endpoints.MapControllerRoute</code> добавляет один маршрут с именем 
<span class="b">default</span> и шаблоном <code>"{controller=Home}/{action=Index}/{id?}"</code>. Данный шаблон устанавливает трехсегментную структуру строки запроса: 
<span class="ii">controller/action/id</span>. То есть в начале идет название контроллера, затем название действия, и далее может идти необязательный параметр id.</p>
<p>Собственно поэтому система может соотнести запрос типа <span class="ii">localhost:xxxx/Home/Index</span> с контроллером и его действием.</p>
<p>Однако не все методы контроллера являются действиями. Контроллер также может иметь непубличные методы - такие методы не рассматриваются как действия и 
соответственно не могут соотноситься с запросами. Например, определим в контроллере следующий метод:</p>
<pre class="brush:c#;">
...
<p>Поскольку его модификатор отличается от <code>public</code>, то мы не сможем обратиться к этому методу с запросом <span class="ii">localhost:xxxx/Home/Hello</span>. 
...
При этом если мы изменим модификатор метода на <code>public</code>, то метод Hello станет полноценным действием:</p>
<img src="./pics/controllers3.png" alt="Действия контроллера" />
<a name="action"></a>
<h3>Атрибуты NonController, ActionName и NonAction</h3>
<p>Возможно, сопоставление по умолчанию бывает не всегда удобно. Например, у нас есть класс в папке Controllers, но мы не хотим, чтобы он мог обрабатывать 
запрос и использоваться как контроллер. Чтобы указать, что этот класс не является контроллером, нам надо использовать над ним атрибут <span class="b">[NonController]</span>:</p>
<pre class="brush:c#;">
[NonController]
public class HomeController : Controller
{
...
<p>Аналогично, если мы хотим, чтобы какой-либо публичный метод контроллера не рассматривался как действие, то мы можем использовать над ним атрибут <span class="b">NonAction</span>:</p>
<pre class="brush:c#;">
[NonAction]
public string Hello()
{
...
<p>Атрибут <span class="b">[ActionName]</span> позволяет для метода задать другое имя действия. Например:</p>
<pre class="brush:c#;">
[ActionName("Welcome")]
public string Hello()
{
...
<p>В этом случае чтобы обратиться к этому методу, надо отправить запрос <i>localhost:xxxx/Home/Welcome</i>. А запрос <i>localhost:xxxx/Home/Hello</i> работать не будет.</p>
<a name="httpmethod"></a>
<h3>Типы запросов</h3>
<p>В прошлой главе для обработки запросов использовалась следующая пара методов:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>Несмотря на то, что здесь два разных метода, но они в соответствии с именем образуют одно действие Buy. Допустимо определять в контроллере методы с одним и 
тем же именем, только в этом случае они должны различаться по параметрам, как в данном случае.</p>
<p>Кроме того, методы в рамках одного действия могут обслуживать разные запросы. Для указания типа запроса HTTP нам надо применить к методу один из атрибутов: 
<span class="b">[HttpGet]</span>, <span class="b">[HttpPost]</span>, <span class="b">[HttpPut]</span>, <span class="b">[HttpDelete]</span> и <span class="b">[HttpHead]</span>. 
Если атрибут явным образом не указан, то метод может обрабатывать все типы запросов: GET, POST, PUT, DELETE.</p>
...
<div class="nav"><p><a href="./3.5.php">Назад</a><a href="./">Содержание</a><a href="./5.2.php">Вперед</a></p></div></div>
<div class="socBlock">
