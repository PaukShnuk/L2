...
<html >
<head>
<title>ASP.NET Core | Отправка файлов</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка файлов в ASP.NET Core MVC, классы PhysicalFileResult, FileContentResult, FileStreamResult, VirtualFileResult">
...
<h2>Отправка файлов</h2><div class="date">Последнее обновление: 18.11.2019</div>
...
<p>Для примера добавим в корень проекта папку <span class="ii">Files</span>, в которой находится файл <span class="ii">book.pdf</span>. И также 
добавим в папку wwwroot папку <span class="ii">Files</span>, в которой находится файл <span class="ii">hello.txt</span>:</p>
<img src="./pics/physicalfileresult.png" alt="Отправка файла по физическому пути в ASP.NET Core MVC 6" />
<h3>Загрузка файла по пути. PhysicalFileResult</h3>
<p>Воспользуемся <code>PhysicalFileResult</code> для отправки файла book.pdf клиенту:</p>
<pre class="brush:c#;">
...
<p>Чтобы получить полный физический путь каталога относительно проекта, воспользуемся сервисом <span class="b">IWebHostEnvironment</span>. Он автоматически передается 
в контроллер, и через конструктор контроллера мы его можем получить. Свойство <span class="b">ContentRootPath</span> данного сервиса указывает на 
физический путь к каталогу проекта.</p>
...
<p>Похожим образом работает и класс FileContentResult, только используется метод <span class="b">File()</span>, а вместо имени файла передается массив байтов, в который был считан файл:</p>
<pre class="brush:c#;">
