...
<html >
<head>
<title>ASP.NET Core | Возвращение результата в Razor Pages</title>
<meta charset="utf-8" />
<meta name="description" content="Возвращение результата в Razor Pages в ASP.NET Core, IActionResult, метод Page">
...
<h2>Возвращение результата в Razor Pages</h2><div class="date">Последнее обновление: 05.12.2019</div>
...
<p>Все ранее рассмотренные методы OnGet и OnPost, предназначенные для обработки GET и POST-запросов, ничего не возвращали, то есть имели в качестве возвращаемого типа 
тип void и просто выполняли некоторое действие:</p>
<pre class="brush:c#;">
public void OnGet()
{
	// обработка запроса
...
<p>В реальности данные методы по умолчанию возвращали связанную с классом страницу Razor. Однако иногда возникает необходимость возвратить некоторый результат, например, статусный код ошибки или сделать редирект. И подобные методы, как и 
методы контроллеров, также могут возвращать объекты IActionResult. Для этого в классе PageModel определено ряд методов:</p>
<ul>
...
<p>Большинство этих методов и возвращаемых ими результатов было рассмотрено ранее в теме <a href="https://metanit.com/sharp/aspnet5/5.3.php">Результаты действий контроллеров</a> и 
ряде последующих статей. В данном случае рассмотрим некоторые моменты, которые могут вызвать трудности. Например, в зависимости от результата надо возвратить 
либо статусный код ошибки, либо страницу Razor.</p>
<p>Например, пусть у нас есть класс Person, который представляет данные:</p>
<pre class="brush:c#;">
public class Person
{
...
<p>Допустим, мы хотим в PageModel получить из списка по имени нужный объект Person:</p>
<pre class="brush:c#;">
...
<p>Метод OnGet возвращает IActionResult. В зависимости от того, найден ли пользователь, этот IActionResult будет представлять либо NotFoundResult, 
либо PageResult. Причем метод Page() возвращает текущую страницу, как если бы у нас был бы просто определен метод OnGet с типом void:</p>
<pre class="brush:c#;">
...
<p>И на самой странице Razor выводится полученный объект:</p>
<pre class="brush:xml;">
@page
...
<img src="./pics/razorpages31.png" alt="Result Types in Razor Pages in ASP.NET Core" />
...
<div class="nav"><p><a href="./29.6.php">Назад</a><a href="./">Содержание</a><a href="./29.8.php">Вперед</a></p></div></div>
<div class="socBlock">
