...
<html >
<head>
<title>ASP.NET Core | Контекст контроллера</title>
<meta charset="utf-8" />
<meta name="description" content="Контекст контроллера в ASP.NET Core MVC, объекты HttpContext и ControllerContext, Request и Response">
...
<h2>Контекст контроллера</h2><div class="date">Последнее обновление: 18.11.2019</div>
...
<p>Кроме тех данных, которые передаются через параметры метода, в контроллере мы можем получить различную информацию, связанную с контекстом контроллера, в том 
числе контекст запроса и все его данные. Для получения контекста в контроллере нам доступно свойство <span class="b">ControllerContext</span>, 
...
<p>Для получения информации о запросе нас прежде всего будет интересовать свойство HttpContext, которое представляет объект <code>Microsoft.AspNetCore.Http.HttpContext</code>. 
Это свойство также доступно через свойство HttpContext в контроллере. То есть следующие вызовы будут обращаться к одному и тому же объекту:</p>
<pre class="brush:c#;">
...
<h3>Request</h3>
<p>Свойство <code>HttpContext.Request</code> представляет объект <code>HttpRequest</code> и предоставляет разнообразную информацию о запросе. Этот же объект 
доступен через свойство <code>Request</code> класса Conroller. Среди свойств объекта Request можно выделить следующие:</p>
<ul>
...
<p>Вся основная информация нам доступна из заголовков. Например, получим все имеющиеся заголовки и выведем их в браузере:</p>
<pre class="brush:c#;">
...
<p>Получим значения определенных заголовков:</p>
<pre class="brush:c#;">
string userAgent = Request.Headers["User-Agent"].ToString();
string referer = Request.Headers["Referer"].ToString();
</pre>
<h3>Response</h3>
<p>Свойство <code>HttpContext.Response</code> представляет объект <code>HttpResponse</code> и позволяет управлять ответом на запрос, в частности, устанавливать 
заголовки ответа, куки, отправлять в выходной поток некоторый ответ. Этот же объект доступен через свойство <code>Response</code> класса Conroller. 
...
<p>Также с помощью объекта Response мы можем отправить ответ клиенту с помощью метода <code>WriteAsync()</code>:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Http;
...
<p>Для использования метода <code>WriteAsync()</code> необходимо импортировать пространство имен <code>Microsoft.AspNetCore.Http</code>.</p>
<p>То есть фактически нам необязательно отправлять какой-то результат обработки запроса в виде ViewResult или других объектов. Мы вполне может в простых ситуациях 
обойтись методом <code>Response.WriteAsync()</code>.</p>
