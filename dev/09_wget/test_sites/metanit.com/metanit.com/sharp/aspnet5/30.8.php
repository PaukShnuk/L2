...
<html >
<head>
<title>ASP.NET Core | Аутентификация и авторизация в SignalR</title>
<meta charset="utf-8" />
<meta name="description" content="Аутентификация и авторизация в хабах и клиентах в SignalR в ASP.NET Core">
...
<h2>Аутентификация и авторизация на основе куки</h2><div class="date">Последнее обновление: 28.12.2019</div>
...
<p>Хабы, как и контроллеры, могут использовать встроенные механизмы аутентификации и авторизации для разграничения доступа.</p>
<p>Возьмем простейший проект, где применяется простейшая аутентификация и авторизация на основе куки со следующей структурой:</p>
<img src="./pics/signalr24.png" alt="Role Authorization and Cookie Authentification in SignalR" />
<h3>Определение базовой части </h3>
<p>Пусть в папке Models находятся контекст данных ApplicationContext и две модели Role и User, чьи объекты хранятся в базе данных:</p>
<pre class="brush:c#;">
...
<p>И также для передачи данных из представления в контроллер в папке Models имеет модель представления LoginModel:</p>
<pre class="brush:c#;">
...
<p>Контроллер AccountController в папке Controllers управляет аутентификацией:</p>
<pre class="brush:c#;">
using System.Collections.Generic;
...
<p>Для аутентификации в папке <span class="ii">Views/Account</span> определим представление Login.cshtlm:</p>
<pre class="brush:xml;">
...
<h3>Определение хаба SignalR</h3>
<p>Для тестирования аутентификации и авторизации в SignalR определим следующий класс хаб:</p>
<pre class="brush:c#;">
...
<p>Для организации доступа к хабу и его методам можно применять атрибут <span class="b">AuthorizeAttribute</span>. В данном случае функциональность 
...
не смогут вызвать методы хаба, даже если в их распоряжении будет код javascript, который обращается к этим методам.</p>
<p>Кроме того, для метода Notify указано еще одно ограничение - доступ только для тех пользователей, которые принадлежат роли "admin". В остальном оба метода 
одинаковы - принимают два параметра и вызывают на клиенте функцию "Receive".</p>
<p>Для взаимодействия с хабом в папке <span class="ii">Views/Account</span> определим следующее представление Index.cshtml:</p>
<pre class="brush:xml;">
...
<p>С помощью проверки на роль <code>@if (User.IsInRole("admin"))</code> добвляются блоки html и код javascript, который будет у условного 
...
на стороне хаба его роль все равно бы проверялась.</p>
<p>И класс Startup выглядит следующим образом:</p>
<pre class="brush:c#;">
using System;
using System.Collections.Generic;
using System.Linq;
...
<p>Пример работы хаба:</p>
<img src="./pics/signalr14.png" alt="Авторизация и аутентификация в SignalR" />
...
<div class="nav"><p><a href="./30.7.php">Назад</a><a href="./">Содержание</a><a href="./30.9.php">Вперед</a></p></div></div>
<div class="socBlock">
