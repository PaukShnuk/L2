...
<html >
<head>
<title>ASP.NET Core | Создание правил URL Rewriting</title>
<meta charset="utf-8" />
<meta name="description" content="Создание правил URL Rewriting в ASP.NET Core, класс RewriteContext, интерфейс IRule">
...
<h2>Создание правил URL Rewriting</h2><div class="date">Последнее обновление: 22.12.2019</div>
...
<p>Если необходимо использовать какую-то более сложную логику по переопределению строки запроса, то в этом случае мы можем 
определить свои правила с помощью методов или целых классов.</p>
<p>Например, пусть ранее сайт использовал технологию php, но затем мигрировал на asp.net, а все документы php были сконвертированы 
в документы html. То есть ранее сайт, к примеру, использовал адрес <span class="ii">http://localhost:1234/some.php</span>, 
то теперь документ перемещен по адресу <span class="ii">http://localhost:1234/html/some.html</span>. Расмотрим на примере создания своих правил, 
как мы можем решить проблему адресов.</p>
<p>Пусть нужные документы находятся в проекте в папке webroot/html:</p>
<img src="./pics/urlrewriting4.png" alt="Миграция с PHP на ASP.NET Core" />
<p>Допустим, там находится следующая страница some.html:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Вначале рассмотрим простой рерайт без переадресации и определим правило в виде отдельного метода. Для этого изменим класс Startup следующим образом:</p>
<pre class="brush:c#;">
...
<p>Для применения правила у объекта RewriteOptions вызывается метод Add, в который передается делегат <span class="b">Action&lt;RewriteContext&gt;</span>. В данном случае передаем ссылку на метод 
RewritePHPRequests.</p>
<p>В методе RewritePHPRequests вначале получаем объекты запроса, ответа и запрошенный путь. Если запрошенный путь уже содержит gпуть к каталогу html, то отклоняем его, устанавливая в качестве кода 
ответа статусный код 404. Для завершения выполнения задается значение <code>context.Result = RuleResult.EndResponse</code>. 
Тем самым мы предотвращаем прямой доступ к папке html (допустим, необходимо скрыть путь к документам html).</p>
<p>Если запрошенный адрес заканчивается на ".php", то выпоняем ряд преобразований, получая путь к html-документу в папке webroot/html. И затем 
устанавливаем новое значение у свойства <span class="b">request.Path</span>. Из него последующие компоненты middleware будут брать информацию о запрошенном пути и обработать его соответствующим образом.</p>
<p>Запустим приложение на выполнение и обратимся по адресу http://localhost:xxxx/some.php:</p>
<img src="./pics/urlrewriting5.png" alt="Изменение php на asp.net core" />
<p>В этом случае произойдет обращение к документу webroot/html/some.html.</p>
<p>Рассмотрим другой способ. Допустим, нам надо не просто переопределить адрес внутри приложения, а выполнить постоянную переадресацию, уведомляя браузеры пользователей и 
поисковики, что адрес документа окончательно изменился. Для этого определим новый класс RedirectPHPRequests (хотя можно было бы и в виде метода определить):</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Rewrite;
...
<p>Класс правила должен реализовать интерфейс <span class="b">IRule</span>, который определяет метод ApplyRule.</p>
<p>В конструкторе получаем расширение, которое стоит использовать вместо php, а также путь к документам в рамках проекта.</p>
<p>В методе ApplyRule если вдруг запрошенный адрес начинается с названия каталог, где лежать файлы html, то завершаем выполнение. Так как нет смысла выполнять переадресацию, ведь запрос уже идет к файлам в нужном каталоге. 
...
устанавливается статусный код постоянной переадресации 301.</p>
<p>Применим этот класс в классе Startup:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app)
{
	app.UseDeveloperExceptionPage();
...
<p>Перегруженная версия метода Add класса RewriteOptions принимает объект IRule, в качестве которого в данном случае передается объект RedirectPHPRequests, </p>
...
<div class="nav"><p><a href="./24.3.php">Назад</a><a href="./">Содержание</a><a href="./28.1.php">Вперед</a></p></div></div>
<div class="socBlock">
