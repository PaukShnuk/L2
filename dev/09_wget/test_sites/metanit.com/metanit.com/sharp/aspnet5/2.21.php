...
<html >
<head>
<title>ASP.NET Core | IWebHostEnvironment и окружение</title>
<meta charset="utf-8" />
<meta name="description" content="Интерфейс IWebHostEnvironment, его свойства и взаимодействие с окружением в ASP.NET Core">
...
<h2>IWebHostEnvironment и окружение</h2><div class="date">Последнее обновление: 01.11.2019</div>
...
<p>Для взаимодействия со средой, в которой запущено приложение, применяются объекты, реализующие интерфейс <span class="b">IHostingEnvironment</span>. 
Этот интерфейс предлагает ряд свойств, с помощью которых мы можем получить информацию об окружении:</p>
<ul>
<li><p><span class="b">ApplicationName</span>: возвращает имя приложения</p></li>
<li><p><span class="b">EnvironmentName</span>: возвращает описание среды, в которой хостируется приложение</p></li>
<li><p><span class="b">ContentRootPath</span>: возвращает путь к корневой папке приложения</p></li>
<li><p><span class="b">WebRootPath</span>: возвращает путь к папке, в которой хранится статический контент приложения, как правило, это папка wwwroot</p></li>
...
переменной среды <code>ASPNETCORE_ENVIRONMENT</code>. Ее текущее значение можно посмотреть в свойствах проекта на вкладке Debug:</p>
<img src="./pics/2.22.png" alt="Environment Name in ASP.NET Core" />
<p>Здесь же также можно изменить значение этой переменной.</p>
<p>Кроме того, в проекте в папке <span class="b">Properties</span> есть файл <span class="b">launchSettings.json</span>, который также содержит описания переменных сред:</p>
<pre class="brush:js;">
{
  "iisSettings": {
...
<p>Здесь можно увидеть, что переменная "ASPNETCORE_ENVIRONMENT" встречается два раза - для запуска через IISExpress и для запуска через Kestrel. В обоих случаях 
она имеет значение Development. Здесь мы также можем поменять значение этой переменной. Причем файл launchSettings.json переопределяет настройки проекта. 
Например, если в launchSettings.json мы определим <code>"ASPNETCORE_ENVIRONMENT": "Production"</code>, то имя среды нашего проекта будет "Production", 
хотя в настройках проекта может стоять "Development".</p>
...
<p>Например, при создании нового проекта в методе <code>Configure()</code> класса Startup можно найти следующие строки:</p>
<pre class="brush:c#;">
...
<p>Таким образом, если имя среды имеет значение "Development", то есть приложение находится в состоянии разработки, то при ошибке разработчик увидит детальное описание ошибки. 
Если же приложение развернуто на хостинге и соответственно имеет другое имя хостирующей среды, то простой пользователь при ошибке ничего не увидит. Таким образом, 
в зависимости от стадии, на которой находится проект, мы можем скрывать или задействовать часть функционала приложения.</p>
<h3>Определение своих состояний среды</h3>
<p>Хотя по умолчанию среда может принимать три состояния: Development, Staging, Production, но мы можем при желании вводить новые значения. Например, нам надо отслеживать какие-то дополнительные состояния. 
К примеру, изменим в файле launchSettings.json значение "ASPNETCORE_ENVIRONMENT" на "Test" (значение может быть произвольное):</p>
<pre class="brush:js;">
{
  "iisSettings": {
    "windowsAuthentication": false,
...
<p>Далее мы можем использовать это состояния для выполнения различных действий. Например, используем состояние "Test" в классе Startup:</p>
<pre class="brush:c#;">
...
<img src="./pics/2.40.png" alt="Окружение и переменные среды в ASP.NET Core" />
...
<div class="nav"><p><a href="./2.18.php">Назад</a><a href="./">Содержание</a><a href="./2.5.php">Вперед</a></p></div></div>
<div class="socBlock">
