...
<meta charset="utf-8" />
<meta name="description" content="Передача данных в контроллер в ASP.NET Core MVC, отправка массивов, списков, словарей Dictionary и сложных объектов">
...
<h2>Передача данных в контроллер</h2><div class="date">Последнее обновление: 29.11.2019</div>
...
<p>Рассмотрим некоторые ситуации передачи данных в контроллер.</p>
<h3>Передача массивов</h3>
<p>Пусть у нас есть метод, который принимает массив:</p>
<pre class="brush:c#;">
...
<p>Этот метод обрабатывает запросы GET, поэтому данные можно передать ему через строку запроса через название параметра:</p>
<pre class="browser">http://localhost:54274/Home/GetData?items=volga&items=dnepr&items=don</pre>
<p>Можно явным образом указать индексы элементов:</p>
<pre class="browser">http://localhost:54274/Home/GetData?items[0]=volga&items[2]=dnepr&items[1]=don</pre>
<p>Можно просто ограничиться индексами:</p>
<pre class="browser">http://localhost:54274/Home/GetData?[0]=volga&[2]=dnepr&[1]=don</pre>
...
<p>При отправке значения всех трех полей образуют массив items. И также можно явным образом указать индексы:</p>
<pre class="brush:c#;">
&lt;form method="post"&gt;
    &lt;p&gt;
...
<p>Или вовсе ограничиться индексами:</p>
<pre class="brush:c#;">
&lt;form method="post"&gt;
    &lt;p&gt;
...
<p>Аналогично массивам будут передаваться и списки типа List.</p>
<h3>Передача словарей Dictionary</h3>
<p>Пусть некоторый метод получает в качестве параметра объект Dictionary:</p>
<pre class="brush:c#;">
...
<pre class="browser">http://localhost:54274/Home/GetData?items[germany]=berlin&items[france]=paris&items[spain]=madrid</pre>
...
<p>Аналогично мы могли бы отправить данные через форму:</p>
<pre class="brush:xml;">
&lt;form method="post"&gt;
    &lt;p&gt;Германия:
...
<h3>Отправка сложных объектов</h3>
<p>Пусть у нас есть две модели:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Пусть у нас есть метод, в котором мы хотим получить объект Phone:</p>
<pre class="brush:c#;">
...
<p>С помощью строки запроса мы можем передать ему значения следующим образом:</p>
<pre class="browser">http://localhost:54274/Home/GetPhone?myPhone.Price=24000&myPhone.Name=Nexus5X&myPhone.Manufacturer.Name=LG</pre>
<img src="./pics/models4.png" alt="Установка модели в ASP.NET Core" />
<p>То есть в данном случае параметр метода называется <code>myPhone</code>, поэтому, например, для установки цены в строке запроса используется параметр 
<code>myPhone.Price=24000</code>. Но необязательно использовать имя параметра, ограничившись только названиями свойств:</p>
<pre class="browser">http://localhost:54274/Home/GetPhone?Price=24000&Name=Nexus5X&Manufacturer.Name=LG</pre>
<p>Аналогичным образом мы могли бы передать значения через форму:</p>
<pre class="brush:xml;">
&lt;form method="post"&gt;
    &lt;p&gt;
...
<p>Здесь опять же идет связка по атрибуту <code>name</code>. Значение <code>name="myPhone.Price"</code> указывает, что данное поле будет привязано к 
свойству Price параметра myPhone. Однако опять же необязательно использовать название параметра:</p>
<pre class="brush:xml;">
&lt;form method="post"&gt;
    &lt;p&gt;
        &lt;label&gt;Модель&lt;/label&gt;&lt;br /&gt;
...
<h3>Передача массивов и списков сложных объектов</h3>
<p>Продолжим работу с теми же моделями Phone и Company, и, допустим, у нас есть метод, который принимает массив или список объектов Phone:</p>
<pre class="brush:c#;">
...
<p>С помощью строки запроса мы можем передать данные следующим образом:</p>
<pre class="browser">http://localhost:54274/Home/GetPhones?phones[0].Name=Lumia950&phones[0].Price=30000&phones[0].Manufacturer.Name=Microsoft&phones[1].Name=iPhone6S&phones[1].Price=50000&phones[1].Manufacturer.Name=Apple</pre>
<p>Опять же можно сократить, убрав название параметра и оставив только индексы:</p>
<pre class="browser">http://localhost:54274/Home/GetPhones?[0].Name=Lumia950&[0].Price=30000&[0].Manufacturer.Name=Microsoft&[1].Name=iPhone6S&[1].Price=50000&[1].Manufacturer.Name=Apple</pre>
<p>Но, конечно, подобные данные, лучше все таки передавать через формы. Передача через формы будет аналогична. Допустим, нам надо передать два объекта:</p>
<pre class="brush:xml;">
&lt;form method="post"&gt;
    @for(int i=0;i&lt;2;i++)
    {
...
<p>Здесь создается два набора полей, каждый из которых предназначен для одного объекта Phone:</p>
<img src="./pics/models5.png" alt="Передача сложных моделей в ASP.NET Core" />
<img src="./pics/models6.png" alt="Отправка сложных моделей в ASP.NET MVC Core" />
<p>Также для установки названий полей можно опустить название параметра метода:</p>
<pre class="brush:xml;">
&lt;form method="post"&gt;
    @for(int i=0;i&lt;2;i++)
    {
