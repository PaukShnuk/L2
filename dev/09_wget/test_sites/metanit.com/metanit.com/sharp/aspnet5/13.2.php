...
<html >
<head>
<title>ASP.NET Core | Grunt</title>
<meta charset="utf-8" />
<meta name="description" content="Использование Grunt в ASP.NET Core для минификации и бандлинга файлов javascript и css">
...
<h2>Grunt</h2><div class="date">Последнее обновление: 17.12.2019</div>
...
<p>Grunt представляет еще один инструмент для автоматизации задач на JavaScript, который позволяет минифицировать скрипты, 
компилировать код TypeScript в javascript и ряд других вещей. Хотя в проектах на ASP.NET по умолчанию для автоматизации используется BundlerMinifier, Visual Studio также имеет полноценную поддержку для Grunt.</p>
<p>Для работы с Grunt создадим новый проект по типу Web Application (Model-View-Controller). Новый проект по умолчанию не содержит никакой функциональности Grunt, 
поэтому нам ее надо добавить. Grunt также, как и Gulp, можно добавить через npm. Поэтому вначале добавим конфигурационный файл для NPM, который назовем package.json:</p>
<img src="./pics/grunt6.png" alt="Конфгурационный файл NPM" />
<p>Далее откроем добавленный файл <span class="ii">package.json</span> и изменим его содержание следующим образом:</p>
<pre class="brush:js;">
{
	"version": "1.0.0",
	"name": "asp.net",
...
<p>После сохранения файла в проект будут добавлены пакеты grunt. А в узле <span class="ii">Dependencies/npm</span> мы можем увидеть все установленные модули:</p>
<img src="./pics/grunt1.png" alt="Grunt in ASP.NET 5" />
<p>Что представляют все эти модули?</p>
<ul>
<li><p><span class="b">grunt</span>: основной пакет инструментария Grunt</p></li>
<li><p><span class="b">grunt-contrib-clean</span>: задача, которая удаляет файлы и папки</p></li>
<li><p><span class="b">grunt-contrib-copy</span>: задача, которая управляет копированием файлов и папок в новое место</p></li>
<li><p><span class="b">grunt-contrib-concat</span>: задача, которая соединяет все файлы в один</p></li>
<li><p><span class="b">grunt-contrib-uglify</span>: задача, которая минимизирует файлы JavaScript</p></li>
<li><p><span class="b">grunt-contrib-cssmin</span>: задача, которая минимизирует файлы CSS</p></li>
<li><p><span class="b">grunt-contrib-watch</span>: задача, которая отслеживает изменения в файлах</p></li>
</ul>
<p>Вообще в Grunt имеется гораздо больше задач, но в данном случае мы ограничимся этими.</p>
<p>Grunt настраивается с помощью файла <span class="b">gruntfile.js</span>, который регистрирует задачи, запускаемые в зависимости от различных событий 
в Visual Studio. Поэтому добавить в корень проекта новый файл javascript, который назовем <span class="b">gruntfile.js</span>:</p>
<img src="./pics/grunt2.png" alt="Grunt.js in Visual Studio" />
<p>После добавления изменим содержимое файла gruntfile.js следующим образом:</p>
<pre class="brush:js;">
...
<p>Разберем этот файл по секциям. Вначале идет задача <code>concat</code>:</p>
<pre class="brush:js;">
concat: {
    build: {
        src: ['wwwroot/js/*'],
...
<p><code>Build</code> указывает на общее название задачи, которое потом будет использоваться для вызова задачи concat. Параметр <code>src</code> 
указывает на набор файлов, который надо соединить в один. А параметр <code>dest</code> указывает на название нового файла, в который файлы из src 
будут объединяться. Здесь применяется синтаксис подстановок. С помощью звездочки указывается, что все файлы из папки <code>wwwroot/js/</code> 
будут соединяться в файл <span class="ii">wwwroot/js/app.js</span></p>
<p>Затем идет задача <code>uglify</code>:</p>
<pre class="brush:js;">
uglify: {
    build: {
        files: {
...
<p>Задача uglify управляет минимизацией файлов javascript. Параметр <code>files</code> указывает набор пар "минимизированный файл: [массив оригинальных файлов]". 
Например, из файла <span class="ii">wwwroot/js/app.js</span> будет создаваться минимизированный файл <i>wwwroot/bundles/app.js</i>. Из нескольких файлов можно создать один минимизированный файл, 
что позволяет одновременно провести их соединение.</p>
<p>Далее задача <code>clean</code> удаляет ранее созданный файл wwwroot/js/app.js, так как он был минимизирован в файл в другом месте:</p>
<pre class="brush:js;">
clean: ["wwwroot/js/app.js"],
</pre>
<p>Затем минимизируем css-файлы задачей <code>cssmin</code>:</p>
<pre class="brush:js;">
cssmin: {
    build: {
        files: {
...
<p>Принцип минимизации здесь аналогичен задаче uglify.</p>
<p>После определения задач идет код для загрузки используемых модулей:</p>
<pre class="brush:js;">
grunt.loadNpmTasks('grunt-contrib-cssmin');
grunt.loadNpmTasks('grunt-contrib-concat');
grunt.loadNpmTasks('grunt-contrib-uglify');
...
<p>И в самом конце регистрируется общая задача <code>build</code>, которая будет запускать все выше определенные задачи:</p>
<pre class="brush:js;">grunt.registerTask('build', ['concat', 'uglify', 'cssmin', 'clean']);</pre>
<p>После сохранения файла gruntfile.js нажмем на него правой кнопкой мыши и в выпадающем меню выберем <span class="b">Task Runner Explorer</span>. </p>
<img src="./pics/grunt7.png" alt="Grunt task runner in Visual Studio" />

<p>И в этом окне откроется список задач Grunt. (Если окно Task Runner не отображает задач Grunt, то следует нажать на кнопку обновления в окне Task Runner). 
В самом низу окна найдем пункт <span class="b">Alias Tasks</span>. Нажмем правой кнопкой на <code>build</code>, который располагается под этим пунктом, и в контекстном меню выберем команду <code>Run</code>:</p>
<img src="./pics/grunt4.png" alt="Run Grunt in Visual Studio" />
<p>После запуска консоль отобразит нам диагностическую информацию</p>
<img src="./pics/grunt8.png" />
<p>Подобным образом можно запускать отдельные задачи. Например, при построении видно, что на момент выполнения задачи "clean" еще не существует файл app.js, 
который надо удалить, поэтому данную задачу можно запустить отдельно от других после того, как остальные задачи отработают.</p>
<p>И после выполнения всех задач в папке wwwroot появятся новые файлы:</p>
<img src="./pics/grunt5.png" alt="Планировщик задач Grunt в Visual Studio и ASP.NET Core" />
<p>Все эти созданные файлы мы можем затем подключить в представлении или на мастер-странице:</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./13.1.php">Назад</a><a href="./">Содержание</a><a href="./13.4.php">Вперед</a></p></div></div>
<div class="socBlock">
