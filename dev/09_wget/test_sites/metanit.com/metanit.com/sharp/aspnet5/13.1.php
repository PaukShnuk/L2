...
<html >
<head>
<title>ASP.NET Core | Gulp</title>
<meta charset="utf-8" />
<meta name="description" content="Использоваие планировщика задач Gulp в ASP.NET Core для минификации файлов">
...
<h2>Gulp</h2><div class="date">Последнее обновление: 16.12.2019</div>
...
<p>Gulp представляет инструмент разработки, который использует язык JavaScript и позволяет автоматизировать процесс разработки. В частности, 
Gulp можно использовать для минификации и бандлинга (объединения) скриптов и ряда других задач.</p>
<p>Для работы с Gulp создадим новый проект ASP.NET Core по типу Web Application (Model-View-Controller). По умолчанию в ASP.NET Core для бадлинга и минификации применяется 
встроенный инструмент BundlerMinifier. Однако мы можем использовать и другие инструменты, в том числе и Gulp.</p>
<p>По умолчанию проект ASP.NET Core не имеет поддержки Gulp, и нам ее надо добавить. Gulp распространяется как модуль Node.js, поэтому вначале необходимо 
добавить поддержку npm. Для этого добавим в проект новый элемент <span class="b">npm Configuration File</span>, который будет представлять конфигурационный файл npm и который 
назовем <span class="ii">package.json</span>:</p>
<img src="./pics/gulp7.png" alt="Добавление npm Configuration File в ASP.NET Core" />
<p>Изменим этот файл следующим образом:</p>
<pre class="brush:js;">
{
  "version": "1.0.0",
  "name": "asp.net",
...
<p>Поскольку функциональность Gulp нам нужна только для разработки - минимизировать и объединять скрипты и стили, то соответствующие пакеты помещаем в 
узел "devDependencies".</p>
<p>Сохраним файл, и в узле <span class="ii">Dependencies/npm</span> мы увидим добавленные зависимости:</p>
<img src="./pics/gulp1.png" alt="Gulp in Visual Studio 2019" />
<p>Затем нам надо добавить файл конфигурации для самого Gulp. Для этого добавим в корень проекта javascript-файл, который 
назовем <span class="b">gulpfile.js</span>:</p>
<img src="./pics/gulp8.png" alt="Добавление Gulp в ASP.NET Core" />
<p>Определим в этом файле следующее содержимое:</p>
<pre class="brush:js;">
"use strict";
...
<p>Файл <span class="ii">gulpfile.js</span> содержит код JavaScript, который определяет набор задач для менеджера gulp. В начале файла идет вызов функций <code>require</code>, 
которые регистрируют необходимые модули Node. Это все те модули, которые мы можем увидеть в узле <span class="ii">Dependencies/npm</span> 
(gulp, rimraf, gulp-concat, gulp-cssmin и gulp-uglify). Для каждого модуля определяется своя переменная:</p>
<pre class="brush:js;">
"use strict";
...
<ul>
<li><p><span class="b">gulp</span>: основной модуль gulp</p></li>
<li><p><span class="b">rimraf</span>: модуль для удаления</p></li>
<li><p><span class="b">gulp-concat</span>: модуль для соединения файлов</p></li>
<li><p><span class="b">gulp-cssmin</span>: модуль минимизации файлов css</p></li>
<li><p><span class="b">gulp-uglify</span>: модуль минимизации файлов javascript с помощью утилиты UglifyJS</p></li>
</ul>
<p>Затем дополнительно определяются пути для файлов CSS и JavaScript:</p>
<pre class="brush:js;">
var paths = {
    webroot: "./wwwroot/"
};
...
<p>Параметр <code>paths.js</code> указывает на путь к обрабатываемым js-файлам, а <code>paths.minJs</code> определяет маску для минимизированных файлов js.</p>
<p><code>paths.css</code> аналогично указывает на обрабатываемые файлы css, а <code>paths.minCss</code> - в какие файлы они будут минимизироваться.</p>
<p><code>paths.concatJsDest</code> определяет путь к файлу, в который будут объединяться минимизированные js-файлы. Параметр <code>paths.concatCssDest</code> 
представляет аналогичную опцию для css-файлов.</p>
<p>После этого определяются задачи gulp. По умолчанию определено шесть задач:</p>
<pre class="brush:js;">
gulp.task("clean:js", function (cb) {
    rimraf(paths.concatJsDest, cb);
});
...
<ul>
<li><p><span class="b">clean:js</span>: задача, которая использует модуль <code>rimraf</code> для удаления ненужных файлов javascript</p></li>
<li><p><span class="b">clean:css</span>: задача, которая использует модуль <code>rimraf</code> для удаления ненужных файлов css</p></li>
<li><p><span class="b">clean</span>: задача, которая вызывает две ранее определенные задачи clean:js и clean:css</p></li>
<li><p><span class="b">min:js</span>: задача, которая минифицирует файлы javascript и объединяет их в один файл</p></li>
<li><p><span class="b">min:css</span>: задача, которая минифицирует файлы css и объединяет их в один файл</p></li>
<li><p><span class="b">min</span>: задача, которая вызывает две предыдущие задачи min:js и min:css</p></li>
</ul>
<p>Причем все задачи будут выполняться именно в том порядке, в котором они объявлены. Каждая задача использует ряд встроенных функций Gulp:</p>
<ul>
<li><p><span class="b">task</span>: определяет задачу. Имеет следующий синтаксис <code>gulp.task(name[, deps], fn) { }</code>, где <code>name</code> - имя задачи, 
<code>deps</code> - набор задач, которые будут выполняться перед текущей задачей, и <code>fn</code> - функция, собственно представляющая код задачи</p></li>
<li><p><span class="b">run</span>: имеет следующий синтаксис <code>gulp.run(tasks) { }</code> и представляет набор выполняемых задач</p></li>
<li><p><span class="b">watch</span>: имеет синтаксис <code>gulp.watch(glob [, opts], tasks) { }</code>, следит за изменениями в файлах, соответствующих параметру 
<code>glob</code>, и при наличии изменений выполняет задачу.</p></li>
<li><p><span class="b">src</span>: имеет синтаксис <code>gulp.src(globs[, options])</code> и предоставляет файлы, которые соответствуют параметру glob. А параметр <code>options</code> 
устанавливает дополнительные опции для сопоставления файлов</p></li>
<li><p><span class="b">dest</span>: имеет следующий синтаксис <code>gulp.dest(path[, options]) { }</code>, с помощью параметра <code>path</code> устанавливает 
путь к создаваемым файлам</p></li>
</ul>
<p>Теперь запустим эти задачи. Для этого нажмем на файл gulpfile.js правой кнопкой мыши и в контекстном меню выберем пункт <span class="b">Task Runner Explorer</span>:</p>
<img src="./pics/gulp2.png" alt="Gulp in ASP.NET Core" />
<p>И внизу Visual Studio нам откроется соответствующее окно:</p>
<img src="./pics/gulp3.png" alt="Task Runner Explorer in Visual Studio" />
<p>В этом окне отображатся все определенные нами задачи из файла gulpfile.js. Если они не отображаются, то надо нажать на кнопку обновления.</p>
<p>Нажмем в этом окне правой кнопкой мыши на задачу <span class="b">min</span> и в появившемся контекстном меню выберем <span class="b">Run</span>:</p>
<img src="./pics/gulp4.png" alt="Запуск задач Gulp в Visual Studio" />
<p>После выполнения этой задачи в папках, где хранятся файлы стилей и скриптов приложения, появятся минимизированные версии файлов.</p>
<p>С помощью дополнительных инструментов в Visual Studio мы можем настроить выполнение задач Gulp. Так, опять же нажмем на задачу <span class="b">min</span> 
и в контекстном меню выберем <span class="b">Bindings -&gt; Before Build</span>:</p>
<img src="./pics/gulp5.png" alt="Настройка Gulp в Visual Studio" />
<p>Данная опция позволит автоматически запускать задачи перед построением проекта. Либо при желании мы можем установить одну из других опций: 
<code>After Build</code>, <code>Clean</code> и <code>Project Open</code>, либо выбрать сразу несколько опций.</p>
...
<div class="nav"><p><a href="./13.5.php">Назад</a><a href="./">Содержание</a><a href="./13.2.php">Вперед</a></p></div></div>
<div class="socBlock">
