...
<html >
<head>
<title>ASP.NET Core | Локализация аннотаций данных</title>
<meta charset="utf-8" />
<meta name="description" content="Локализация аннотаций данных в ASP.NET Core, определение ресурсов для атрибутов моделей, метод AddDataAnnotationsLocalization">
...
<h2>Локализация аннотаций данных</h2><div class="date">Последнее обновление: 18.12.2019</div>
...
<p>Для локализации аннотаций данных применяется ранее рассмотренный в прошлых темах сервис <span class="b">IStringLocalizer&lt;T&gt;</span>. И также все необходимые 
ресурсы локализации определяются с помощью файлов ресурсов. Посмотрим на примере.</p>
<p>Пусть в проекте для моделей будет определена папка <span class="b">ViewModels</span>, в которой будет расположена модель <span class="b">ProductViewModel</span>:</p>
<pre class="brush:c#;">
...
<p>Это вполне обычная модель, к которой применяются аннотации данных. Но в данном случае значение ErrorMessage в атрибутах Required/Range/StringLength рассматривается 
не только как сообщение об ошибке, но и как название ресурса, который содержит нужное значение. То же самое касается и свойства Name в атрибуте Display. 
То есть в данном случае применяются 6 атрибутов, которые фактически используют шесть ресурсов: Name, Price, NameLength, NameRequired, PriceRange и PriceRequired. 
И через эти ресурсы мы можем управлять локализацией аннотаций.</p>
<p>Теперь собственно определим файлы ресурсов. К наименованию файлов ресурсов для моделей применяются те же правила, что и к наименованию файлов ресурсов для 
контроллеров и представлений. То есть название файла ресурсов должно отражать полный путь к модели в рамках проекта. Например, в нашем случае модель находится по пути 
<span class="ii">ViewModels/ProductViewModel.cs</span>, поэтому файлы ресурсов для нее могут располагаться по следующим путям:</p>
<ul>
<li><p><span class="ii">[папка_ресурсов]/ViewModels.ProductViewModel.[код_культры].resx</span></p></li>
<li><p><span class="ii">[папка_ресурсов]/ViewModels/ProductViewModel.[код_культры].resx</span></p></li>
</ul>
<p>Выберем второй способ. Определим в проекте в папке Resources подкаталог ViewModels, а в него добавим файлы ресурсов для различных локалей:</p>
<img src="./pics/localization16.png" alt="Локализация аннотаций данных в ASP.NET Core" />
<p>В данном случае я определил три файла ресурсов:</p>
<ul>
<li><p><span class="ii">Resources/ViewModels/ProductViewModel.ru.resx</span></p></li>
<li><p><span class="ii">Resources/ViewModels/ProductViewModel.en.resx</span></p></li>
<li><p><span class="ii">Resources/ViewModels/ProductViewModel.de.resx</span></p></li>
</ul>
<p>Каждый файл ресурсов содержит те ресурсы, которые применяются в атрибутах модели ProductViewModel. Например, файл для русскоязычной локали:</p>
<img src="./pics/localization17.png" alt="Аннотации данных и файлы ресурсов в ASP.NET Core" />
<p>И файл ресурсов для англоязычной локали:</p>
<img src="./pics/localization18.png" alt="Аннотации данных и локализация в ASP.NET Core MVC" />
<p>Далее для подключения локализации аннотаций в классе <span class="b">Startup</span> необходимо вызвать метод <span class="b">AddDataAnnotationsLocalization()</span> в методе ConfigureServices:</p>
<pre class="brush:c#;">
...
<p>Для тестирования определим в контроллере HomeController действие Create:</p>
<pre class="brush:c#;">
...
<p>И для действия создадим представление <span class="b">Create.cshtml</span> с формой для добавления объекта ProductViewModel:</p>
<pre class="brush:xml;">
...
<p>Запустим приложение и установим русскоязычную культуру. Попробуем отправить заведомо некорректные данные:</p>
<img src="./pics/localization19.png" />
<p>Теперь установим англоязычную локаль:</p>
<img src="./pics/localization20.png" />
...
<div class="nav"><p><a href="./28.5.php">Назад</a><a href="./">Содержание</a><a href="./28.7.php">Вперед</a></p></div></div>
<div class="socBlock">
