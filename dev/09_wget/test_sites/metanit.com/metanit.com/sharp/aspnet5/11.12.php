...
<html >
<head>
<title>ASP.NET Core | RouterMiddleware</title>
<meta charset="utf-8" />
<meta name="description" content="Использование маршрутизации в ASP.NET Core, роль компонента RouterMiddleware, обработчик маршрутов RouteHandler и построитель RouteBuilder">
...
<h2>RouterMiddleware</h2><div class="date">Последнее обновление: 15.11.2019</div>
...
<p>В прошлой теме были рассмотрены стандартный механизм маршрутизации в ASP.NET Core с помощью компонентов EndpointRoutingMiddleware и EndpointMiddleware. 
Еще один способ построения маршрутизации представляет компонент называется <span class="b">RouterMiddleware</span>. 
При обработке запроса RouterMiddleware сравнивает запрашиваемый адрес Url с зарегистрированными маршрутами, и если один из маршрутов подходит, то 
вызывается обработчик этого маршрута.</p>
<p>Для добавления RouterMiddleware изменим класс Startup:</p>
<pre class="brush:c#;">
...
<p>В методе <code>Configure()</code> происходит настройка маршрута и определение обработчика, который будет обрабатывать этот запрос по этому маршруту:</p>
<pre class="brush:c#;">
var myRouteHandler = new RouteHandler(Handle);
var routeBuilder = new RouteBuilder(app, myRouteHandler);
routeBuilder.MapRoute("default", "{controller}/{action}");
...
<p>Класс <span class="b">RouteHandler</span> представляет встроенный обработчик маршрута. В качестве параметра в него передается делегат <span class="b">RequestDelegate</span>, 
который и будет обрабатывать запрос. В данном случае на место делегата передается ссылка на метод <code>Handle()</code>. Метод, который представляет делегат 
...
строку "Hello ASP.NET Core!"</p>
<p>Для построения маршрута применяется объект <span class="b">RouteBuilder</span>. В его конструктор передается сервис IApplicationBuilder, который в данном случае мы можем 
получить из параметра метода <code>Configure()</code>, и вышеопределенный обработчик маршрута.</p>
<p>Далее идет собственно определение самого маршрута в методе <code>routeBuilder.MapRoute()</code>. В данном случае "default" задает имя маршрута, а строка 
<code>"{controller}/{action}"</code> представляет <span class="b">шаблон маршрута</span>  - некоторый шаблон Url, с которым будет сопоставляться 
запрошенный адрес URL. Шаблон URL может состоять из одного и более сегментов. Если в шаблоне используется несколько сегментов, то они разделяются слешами.</p>
<p>Каждый такой сегмент шаблона содержит параметр. Эти параметры называются <span class="b">параметрами маршрута</span>. Каждый параметр заключается в фигурные скобки. В данном случае 
это параметры controller и action. Но вообще параметры не обязательно должны иметь именно такие названия, они могут иметь различные имена, включающие 
любые алфавитно-цифровые символы.</p>
<p>И последняя строка подключает RouterMiddleware в конвейер обработки запроса:</p>
<pre class="brush:c#;">app.UseRouter(routeBuilder.Build());</pre>
<p>Вызов <code>routeBuilder.Build()</code> возвращает объект IRouter, который затем переходит в RouterMiddleware и используется для обработки запросов.</p>
<p>После этого обработчик маршрута готов к использованию и может участвовать в обработке входящих запросов.</p>
<p>При этом после этого вызова могут идти другие компоненты middleware, которые могут участвовать в обработке запроса. Так, в данном случае в конце вызывается метод 
<code>app.Run()</code>, который отправляет в выходной поток строку "Hello World".</p>
<p>Теперь запустим приложение на выполнение. По умолчанию приложение открывается по адресу типа <span class="ii">http://localhost:xxxx/</span> - то есть название домена и порта.</p>
<img src="./pics/routing1.png" alt="Маршрутизация в ASP.NET Core" />
<p>При этом надо учитывать, что RouterMiddleware не передает выполнение запроса дальше, если хотя бы один из определенных маршрутов совпал со строкой запроса.</p>
<h3>URL matching</h3>
<p>Когда к приложению, которое использует маршруты, приходит запрос, то начинается процесс <span class="b">URL matching</span> - 
процесс сопоставления строки запроса URL маршрутам. Причем в приложении может использоваться множество маршрутов. И RouterMiddleware последовательно перебирает 
все маршруты и сравнивает их шаблон с запрошенным адресом URL. Если шаблон маршрута и строка URL совпадают, то для обработки запроса выбирается обработчик 
данного маршрута. Если обработчик установлен, то он вызывается, и далее другие маршруты не вызываются. Если шаблон маршрута и запрошенный адрес URL не совпадают, 
тогда вызывается следующий маршрут.</p>
<p>Если все маршруты были вызваны, но обработчик для запроса так и не был найден, RouterMiddleware вызывает следующий в конвейере обработки запроса компонент 
middleware.</p>
<p>Так, в данном случае запрошенный URL выглядит следующим образом: <code>http://localhost:59406</code>. Этот URL не имеет сегментов после домена и порта 
(домен и порт в сегментной структуре не учитываются). 
А наш единственный маршрут задает шаблон из двух сегментов: <code>{controller}/{action}</code>. То есть в процессе URL matching компонент 
RouterMiddleware не найдет нужный маршрут и просто передаст выполнение запроса дальше методу <code>app.Run()</code>, который и выведет строку "Hello World!".</p>
<p>Теперь обратимся к нашему приложению с другим запросом <span class="ii">http://localhost:59406/home/index</span>:</p>
<img src="./pics/routing2.png" alt="Основы маршрутизации в ASP.NET Core" />
<p>В этом случае строка URL уже имеет два сегмента: <code>home/index</code>. Теперь система маршрутизации сможет сопоставить адрес запроса с единственным 
маршрутом, так как адрес запрос также состоит из двух сегментов. Для инкапсулирования всей информации о маршрутизации в RouterMiddleware создается объект <span class="b">RouteContext</span></p>
<p>Для полного понимания процесса мы можем обратиться к исходным кодам классов RouteHandler и RouterMiddleware. Так, при удачном сопоставлении запроса определенному маршруту во встроенном классе RouteHandler у контекста маршрутизации устанавливается свойство 
<span class="b">Handler</span>. И этому свойству присваивается тот делегат RequestDelegate, который мы передаем через объект RouteHandler:</p>
<pre class="brush:c#;">
...
<p>И затем в методе Invoke класса RouterMiddleware вызывается этот делегат:</p>
<pre class="brush:c#;">
...
<p>Поэтому при удачном сопоставлении с запросом система маршрутизации вызовет делегат RequestDelegate, который в данном случае указывает на метод <code>Handle()</code> и который отправит в ответ на запрос строку "Hello ASP.NET Core".</p>
...
<div class="nav"><p><a href="./11.1.php">Назад</a><a href="./">Содержание</a><a href="./11.2.php">Вперед</a></p></div></div>
<div class="socBlock">
