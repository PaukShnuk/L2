...
<meta charset="utf-8" />
<meta name="description" content="Введение в систему аутентификации и авторизации ASP.NET Core Identity и ее использование в ASP.NET Core">
...
<h1>ASP.NET Core Identity</h1><h2>Введение в ASP.NET Core Identity</h2><div class="date">Последнее обновление: 14.12.2019</div>
...
<p>ASP.NET Identity представляет встроенную в ASP.NET систему аутентификации и авторизации. Данная система позволяет пользователям создавать учетные записи, аутентифицироваться, управлять учетными записями или использовать для входа на сайт 
учетные записи внешних провайдеров, таких как Facebook, Google, Microsoft, Twitter и других.</p>
<p>Так, создадим новый проект ASP.NET Core по типу Web Application (Model-View-Controller) с типом аутентификации <span class="b">Individual User Accounts</span>.:</p>
<img src="./pics/identity1.png" alt="ASP.NET Core Identity в Visual Studio" />
<p> Для выбора типа аутентификации в окне выбора типа проекта в секции <span class="b">Authentication</span> нажмем на ссылку <span class="b">Change</span>. При нажати на эту ссылку откроется окно установки типа аутентифкации:</p>
<img src="./pics/identity8.png" alt="ASP.NET Core Identity и тип аутентификации" />
<p>В этом окне соответственно надо указать <span class="b">Individual User Accounts</span> и в центральной части 
<span class="b">Store user accounts in-app</span> - сохранение учетных данных пользователей в локальной базе данных.</p>
<p>В итоге будет следующий проект:</p>
<img src="./pics/identity9.png" alt="ASP NET Core Idenity 2.1" />
<p>По большому счету сообственно от Identity здесь по умолчанию добавлено не так много функционала. В частности, добавлен 
класс ApplicationDbContext - контекст данных для работы с Identity. В папке <span class="b">Areas-&gt;Identity-&gt;Pages</span> 
можно найти файл <span class="b">_ViewStart.cshtml</span>. И есть некоторые изменения в файле Startup.</p>
<p>Однако в отличие от предыдущих версий ASP.NET Core в проект по умолчанию не добавляется никаких контроллеров, моделей, представлений, 
...
контекстном меню выберем пункт <span class="b">Add -&gt; New Scaffolded Item</span></p>
<img src="./pics/identity10.png" alt="Scaffold ASP.NET Core Identity" />
<p>Далее в открывшемся окне выберем <span class="b">Idenity</span>:</p>
<img src="./pics/identity11.png" alt="Добавление в проект ASP.NET Core Identity" />
<p>Затем отроется окно, где необходимо указать, какой именно функционал будет добавлен. Здесь очень много различных опций:</p>
<img src="./pics/identity12.png" alt="ASP.NET Core Identity Razor Pages" />
<p>В частности, к примеру, я отметил пункты <code>Account/Login</code> и <code>Account/Register</code>, то есть функционал для регистрации и входа в приложение. 
Кроме того, в поле <code>Data context class</code> надо указать класс контекста данных. По умолчанию при создании в проект уже добавляется 
класс контекста, который можно указать в этом поле.</p>
<p>Опционально также можно указать путь внутри проекта к мастер-станице и класс пользователя. Но можно также оставить эти поля пустыми, как в моем случае.</p>
<p>После указания всех настроек нажмем на кноку Add, и в проект будет добавлен весь указанный функционал.</p>
<img src="./pics/identity13.png" />
<p>Поскольку я отметил пункты <code>Account/Login</code> и <code>Account/Register</code>, то в папке <span class="b">Areas/Idenity/Pages/Account</span> 
можно будет найти Razor-страницы Login и Register, выполняющих соответствующие функции.</p>
<p>Вне зависимости от того, какие функции мы отметили при добавлении Identity, весь функционал по работе с учетными записями помещается 
в папку Areas/Identity. Начиная с версии ASP.NET Core 2.1 это страницы Razor Pages, которые выполняют какую-то определенную задачу - вход в систему, регистрацию и так далее.</p>
<p>Но чтобы все эти станицы могли выполнять свою роль, в классе Startup добавляется конфигурация Identity. В его методе <code>ConfigureServices()</code> в приложение добавляются сервисы Identity:</p>
<pre class="brush:c#;">
...
<p>Во-первых, здесь добавляются сервисы Entity Framework, которые используются для работы с базой данных, хранящей учетные записи. Все настройки подключения 
заданы в файле <span class="ii">appsettings.json</span> в узле <code>ConnectionStrings</code>.</p>
<p>А в методе <code>Configure()</code> функционал аутентификации и авторизации на основе системы Identity становится доступным через вызовы методов <span class="b">UseAuthentication</span> 
и <span class="b">app.UseAuthorization()</span>, как и в любой системе аутентификации и авторизации в ASP.NET Core:</p>
<pre class="brush:c#;">
...
<p>При начале работы с проектом нам надо учитывать, что он использует Entity Framework, поэтому прежде всего нам надо выполнить миграции, чтобы создать базу данных. 
В проекте уже по умолчанию есть миграции, которые нам осталось применить. Для этого откроем окно <span class="b">Package Manager Console</span> 
и введем в командную строку следующую команду:</p>
<pre class="brush:js;">Update-Database</pre>
<img src="./pics/identity6.png" />
<p>Запустим проект на выполнение. И в меню веб-страницы мы увидим дополнительные пункты для регистрации и входа на сайт:</p>
<img src="./pics/identity4.png" alt="Create Application with ASP.NET Core Identity" />
<p>Нажмем на кнопку регистрации и перед нами откроется форма для регистрации нового пользователя. Введем в нее какие-нибудь данные:</p>
<img src="./pics/identity5.png" alt="Регистрация в ASP.NET Core Identity" />
<p>И после регистрации в базу данных будет добавлен новый пользователь, а нас перенаправят на страницу подтверждения регистрации:</p>
<img src="./pics/identity16.png" alt="Подтверждение регистрации в ASP.NET Core Identity"  />
<p>На этой странице нажмем на ссылку "Click here to confirm your account". По умолчанию при создании пользователя в базе данных в таблице пользователей в поле "EmailConfirmed" устанавливается значение False. 
...
<p>Далее с логином и паролем созданного пользователя мы сможем заходить в приложение.</p>
<img src="./pics/identity14.png" alt="Login in ASP.NET Core Identity"  />
<img src="./pics/identity15.png" alt="Вход в систему ASP.NET Core MVC Identity"  />
<p>При желании мы можем найти базу данных в окне <span class="b">SQL Server Object Explorer</span> по имени, заданному в файле appsettings.json. 
В частности, в таблице <span class="b">dbo.AspNetUsers</span>, которая предназначена для хранения пользователей, мы как раз сможем обнаружить данные добавленного пользователя.</p>
<img src="./pics/identity7.png" />
...
<div class="nav"><p><a href="./23.7.php">Назад</a><a href="./">Содержание</a><a href="./16.11.php">Вперед</a></p></div></div>
<div class="socBlock">
