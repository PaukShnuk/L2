...
<html >
<head>
<title>ASP.NET Core MVC | Мастер-страницы</title>
<meta charset="utf-8" />
<meta name="description" content="Мастер-страницы layout в ASP.NET MVC Core, создание единообразного вида приложения, секции section в представлениях">
...
<h2>Мастер-страницы</h2><div class="date">Последнее обновление: 19.11.2019</div>
...
<p>Когда у нас в проекте много представлений, и все они содержат какие-то общие элементы, то вместо того, чтобы пописывать все эти элементы в каждом представлении, 
...
не изменяя всех остальных представлений. В ASP.NET MVC таким шаблоном являются мастер-страницы.</p>
<p>Мастер-страницы применяются для создания единообразного, унифицированного вида сайта. По сути мастер-страницы - это те же самые представления, которе могут 
...
<p>По умолчанию при создании нового проекта ASP.NET MVC Core в проект уже добавляется мастер-страница под названием <span class="b">_Layout.chtml</span>, 
которую можно найти в каталоге <span class="b">Views/Shared</span>. В приложении из шестой главы мы ее изменили следующим образом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Код мастер-страницы напоминает полноценную веб-страницу: здесь присутсвуют основные теги <code>&lt;html&gt;</code>, <code>&lt;head&gt;</code>, 
<code>&lt;body&gt;</code> и так далее. И также здесь могут использоваться конструкции Razor. Фактически это то же самое представление. 
Главное же отличие от обычных представлений состоит в использовании метода <span class="b">@RenderBody()</span>, который является плейсхолдером 
...
<h3>ViewStart</h3>
<p>По умолчанию представления уже подключают мастер-страницу за счет файла <span class="b">_ViewStart.cshtml</span>. Этот файл можно найти в проекте в 
папке <i>Views</i>.  Код этого файла добавляется в самое начало кода преставлений при их запуске. При этом файлы представлений, к которым применяется  _ViewStart.cshtml, должны находиться с 
этим файлов в одном каталоге.</p>
<p>По умолчанию файл <code>_ViewStart.cshtml</code> содержит следующий код:</p>
<pre class="brush:c#;">
@{
    Layout = "_Layout";
}
...
<p>Если вдруг мы хотим глобально по всему проекту поменять мастер-страницу на другой файл, который расположен в какой-то другой папке, например, в корне каталога Views, то нам надо использовать полный путь к файлу:</p>
<pre class="brush:c#;">
@{
...
<p>При необходимости мы можем использовать несколько мастер-страниц. Добавим в папку Views новую мастер-страницу. Для этого нажмем правой кнопкой мыши 
на папку Views и в выпадающем меню выберем <span class="b">Add -&gt; New Item</span>. Для более быстрого создания мастер-страницы Visual Studio предлагает 
специальный шаблон <span class="b">Razor Layout</span>:</p>
<img src="./pics/views4.png" alt="Добавление layout в ASP.NET MVC Core" />
<p>Назовем файл <span class="b">_Master.cshtml</span>.</p>
...
свойство Layout. Мы можем вообще не использовать мастер-страницу, тогда нам надо присвоить значение <code>null</code>:</p>
<pre class="brush:xml;">
@{
    Layout = null;
}
...
<p>Либо можно использовать какую-нибудь уже имеющуюся мастер-страницу, указав к ней полный путь:</p>
<pre class="brush:xml;">
@{
...
Например, добавим к мастер-странице <code>_Master.cshtml</code> секцию footer:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
...
<div class="nav"><p><a href="./7.3.php">Назад</a><a href="./">Содержание</a><a href="./7.9.php">Вперед</a></p></div></div>
<div class="socBlock">
