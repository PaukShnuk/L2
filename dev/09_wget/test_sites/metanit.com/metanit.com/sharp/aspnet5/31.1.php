...
<html >
<head>
<title>ASP.NET Core | CORS и кросс-доменные запросы</title>
<meta charset="utf-8" />
<meta name="description" content="CORS и кросс-доменные запросы в ASP.NET Core, отправка ajax-запроса, методы AddCors и UseCors">
...
<h1>CORS и кросс-доменные запросы</h1><h2>Начало работы с CORS</h2><div class="date">Последнее обновление: 14.02.2020</div>
...
<p>По умолчанию в целях безопасности браузер ограничивает ajax-запросы между различными доменами. Однако нередко возникает ситуация, когда необходимо 
выполнять запросы из приложения с одного адреса (или домена) к приложению, которое размещено по другому адресу. Для этого нам надо использовать технику, которая называется CORS (Cross Origin Resource Sharing).</p>
<p>В начале создадим новый проект ASP.NET Core по типу Empty:</p>
<img src="./pics/cors1.png" alt="CORS в ASP.NET Core" />
<p>Этот проект будет получать запросы от других приложений и отправлять им ответ. Для этого изменим его класс Startup следующим образом:</p>
<pre class="brush:c#;">
...
<p>Для подключения сервисов CORS в приложение в методе ConfigureServices вызывается метод <span class="b">services.AddCors()</span>.</p>
<p>Чтобы задействовать CORS для обработки запроса в методе Configure вызывается метод <span class="b">UseCors()</span>. Для настройки конфигурации этот метод использует делегат, в 
который передается объект <span class="b">CorsPolicyBuilder</span>. И с помощью этого объекта можно выполнить настройку CORS.</p>
<pre class="brush:c#;">app.UseCors(builder =&gt; builder.AllowAnyOrigin());</pre>
<p>С помощью метода <code>AllowAnyOrigin()</code> мы указываем, что приложение может обрабатывать запросы от приложений по любым адресам.</p>
<p>Стоит отметить, что метод <code>app.UseCors()</code> должен вызываться между методами <code>app.UseRouting()</code> и <code>app.UseEndpoints()</code>.</p>
<p>В итоге всем подключенным клиентам данное приложение будет отправлять строку "Hello World!".</p>
<p>Для тестирования создадим второй проект также ASP.NET Core по типу Empty. Определим во втором проекте следующий класс Startup:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Builder;
...
<p>Здесь мы будем работать только со статическими файлами. В частности, определим во втором проекте в папке wwwroot html-страницу <span class="b">index.html</span>, 
которая будет загружаться по умолчанию:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В данном случае по нажатию на кнопку будет выполняться ajax-запрос к первому приложению, который, в моем случае, будет запускаться по адресу "http://localhost:49352/". 
Полученный от первого приложения ответ будет загружаться в блок <code>&lt;div id="result"&gt;</code></p>
<p>Запустим оба проекта и во втором приложении нажмем на кнопку, чтобы получить ответ от первого приложения:</p>
<img src="./pics/cors2.png" alt="CORS в ASP NET Core 3.0" />
...
<div class="nav"><p><a href="./30.12.php">Назад</a><a href="./">Содержание</a><a href="./31.2.php">Вперед</a></p></div></div>
<div class="socBlock">
