...
<html >
<head>
<title>ASP.NET Core | Создание tag-хелперов</title>
<meta charset="utf-8" />
<meta name="description" content="Создание tag-helpers в ASP.NET Core MVC, файл _ViewImports.cshtml, подключение tag-хелпера, асинхронный метод ProcessAsync">
...
<h1>Работа с tag-хелперами</h1><h2>Создание tag-хелперов</h2><div class="date">Последнее обновление: 06.12.2019</div>
...
<p>Чтобы создать свой tag-хелпер, нам надо унаследовать класс от класса <span class="b">TagHelper</span>, переопределив его метод <code>Process</code>.</p>
<h3>Определение tag-хелпера</h3>
<p>Для примера создадим какой-нибудь простейший тег-хелпер. Допустим, пусть он будет выводить текущее время. Для создания хелпера вначале создадим новый проект ASP.NET Core по типу <span class="b">ASP.NET Core Web App (Model-View-Controller)</span>. И затем добавим в него новую папку, которую назовем <span class="b">TagHelpers</span>. 
Далее в эту папку добавим новый класс <span class="b">TimerTagHelper</span>:</p>
<pre class="brush:c#;">
...
<img src="./pics/customtaghelper2.png" alt="Создание taghelper в ASP.NET Core MVC" />
<p>По умолчанию  tag-хелперы применяют соглашения об наименовании, согласно которым класс должен оканчиваться на суффикс <span class="ii">TagHelper</span>. 
...
<p>Чтобы задействовать класс хелпера в представлении, нам надо подключить его функциональность в представление. Для этого изменим представление 
<span class="b">Views/_ViewImports.cshtml</span> следующим образом:</p>
<pre class="brush:c#;">
@using TagHelpersApp
...
<p>В данном случае предполагается, что проект приложения называется TagHelpersApp. В этом случае первые три директивы должны быть в файле по умолчанию.</p>
<p>Для добавления хелпера используется директива <span class="b">addTagHelper</span>. Директива использует синтаксис подстановочных знаков, 
определяя, какие tag-хелперы будут загружаться в представление. И также указывается сборка, которая содержит классы хелперы. То есть директива</p>
<pre class="brush:c#;">@addTagHelper *, TagHelpersApp</pre>
<p>Здесь первая часть до запятой (в данном случае символ звездочки) указывает, какие tag-хелперы будут загружаться в представление (символ 
...
сборка TagHelpersApp (так как хелперы определены в текущем проекте).</p>
<h3>Использование tag-хелпера</h3>
<p>Теперь определим в каком-нибудь имеющемся представлении тег:</p>
<pre class="brush:xml;">
@{
...
<p>При этом не важно, что он пустой. Можно добавить в него какой-нибудь текст, но он не имеет значение. Главное, что этот тег называется по имени класса без суффикса TagHelper. И в итоге вместо этого тега будет сгенерирован 
элемент <code>&lt;div&gt;</code>, в котором будет выводиться время:</p>
<img src="./pics/customtaghelper.png" alt="Создание tag helper в ASP.NET Core MVC" />
...
<img src="./pics/customtaghelper1.png" alt="TagHelper in ASP.NET Core" />
