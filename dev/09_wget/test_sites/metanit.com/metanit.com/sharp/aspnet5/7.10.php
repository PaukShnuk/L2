...
<html >
<head>
<title>ASP.NET Core | Создание движка представлений</title>
<meta charset="utf-8" />
<meta name="description" content="Создание движка представлений в ASP.NET Core MVC, реализация интерфейсов IView и IViewEngine">
...
<h2>Создание движка представлений</h2><div class="date">Последнее обновление: 19.11.2019</div>
...
определенных в этом же классе <a href="https://github.com/aspnet/AspNetCore/blob/master/src/Mvc/Mvc.ViewFeatures/src/ViewEngines/ViewEngineResult.cs" rel="nofollow">ViewEngineResult</a>:</p>
<ul>
...
для генерации ответа клиенту. В качестве параметра в метод передается объект <a href="https://github.com/aspnet/AspNetCore/blob/master/src/Mvc/Mvc.ViewFeatures/src/Rendering/ViewContext.cs" rel="nofollow">ViewContext</a>, 
...
<p>Чтобы понять, как складывается взаимодействие всех этих классов, создадим небольшой пример. Создадим новый проект ASP.NET Core по типу Empty.</p>
<img src="./pics/viewengine4.png" alt="Проект для View Engine в ASP.NET Core MVC" />
...
добавим в нее простенький контроллер:</p>
<pre class="brush:c#;">
...
<img src="./pics/viewengine1.png" alt="View Engine in ASP.NET Core" />
<p>Теперь изменим класс Startup:</p>
<pre class="brush:c#;">
...
<p>Для установки движка представлений в методе <code>ConfigureServices()</code> применяется объект MvcViewOptions. В MVC может использоваться одновременно несколько движков представлений, чтобы 
...
<img src="./pics/viewengine2.png" alt="Движок представлений в ASP.NET Core" />
<p>Если же нужный html-файл не будет найден, то мы увидим ошибку с перечислением путей, по которым велся поиск файла:</p>
<img src="./pics/viewengine3.png" alt="Реализация движка представлений в ASP.NET Core" />
...
<div class="nav"><p><a href="./7.8.php">Назад</a><a href="./">Содержание</a><a href="./11.5.php">Вперед</a></p></div></div>
<div class="socBlock">
