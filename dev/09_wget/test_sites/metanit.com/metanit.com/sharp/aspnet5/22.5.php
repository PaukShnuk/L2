...
<html >
<head>
<title>ASP.NET Core | Тестирование контроллера</title>
<meta charset="utf-8" />
<meta name="description" content="Тестирование контроллера в ASP.NET Core MVC, использование Moq и xUnit">
...
<h2>Тестирование контроллера</h2><div class="date">Последнее обновление: 21.12.2019</div>
...
<p>В прошлой теме в проект был добавлен Moq фреймворк и протестировано получение данных из репозитория. Теперь определим в контроллере HomeController еще пару действий:</p>
<pre class="brush:c#;">
...
<p>Здесь добавлены методы <code>GetUser()</code>, который возвращает объект из репозитория по id, и метод <code>AddUser()</code>, который добавляет объект в репозиторий.</p>
<p>Для их тестирования определим следующие тесты:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc;
...
<p>Для post-версии метода AddUser контроллера HomeController существуют два варианта событий. При валидной модели она добавляется в репозиторий, и 
происходит передаресация на метод Index:</p>
<pre class="brush:c#;">
if (ModelState.IsValid)
{
    repo.Create(user);
...
<p>Для тестирования этой ситуации в классе тестов определен метод <code>AddUserReturnsARedirectAndAddsUser()</code>:</p>
<pre class="brush:c#;">
public void AddUserReturnsARedirectAndAddsUser()
{
    // Arrange
...
<p>В случае переадресации возвращаемым типом должен быть RedirectToActionResult. Его свойство ActionName указывает на метод для переадресации - он должен представлять 
метод Index.</p>
<p>В тестах мы легко можем взять результат тестируемых методов и сравнить этот результат с определенным значением, чтобы понять, правильно ли все работает. 
Но не все методы возвращают определенные значения. Некоторые методы возвращают тип void, однако и тоже необходимо тестировать. И чтобы протестировать 
подобные методы, в классе Mock определен метод Verify. Так, в данном случае мы верифицируем, что метод Create у репозитория точно был вызван:</p>
<pre class="brush:c#;">mock.Verify(r =&gt; r.Create(newUser));</pre>
<p>Вторая ситуация в методе AddUser в контроллере представляет передачу обратно в представление некорректной модели:</p>
<pre class="brush:c#;">
public IActionResult AddUser(User user)
{
    if (ModelState.IsValid)
...
<p>Для тестирования данной ситуации определен метод <code>AddUserReturnsViewResultWithUserModel()</code>:</p>
<pre class="brush:c#;">
public void AddUserReturnsViewResultWithUserModel()
{
    // Arrange
...
<p>Для имитации некорретной модели здесь добавляется ошибка модели с помощью вызова <code>controller.ModelState.AddModelError()</code>. 
Для проверки теста мы устанавливаем тип результата и сравниваем переданную обратно в представление модель - она должна быть эквивалентна исходной модели newUser.</p>
<p>Стоит отметить, что здесь в тесте не будет работать проверка <code>if(ModelState.IsValid)</code>, потому что в тесте не учитывается механизм привязки модели, 
а поэтому и ошибок модели в контроллере нет.</p>
<p>Метод GetUser в контроллере HomeController подразумевает три различные ситуации. Первая ситуация представляет некорректную передачу id:</p>
<pre class="brush:c#;">
if (!id.HasValue)
    return BadRequest();
</pre>
<p>Для тестирования этого куска кода определен метод <code>GetUserReturnsBadRequestResultWhenIdIsNull()</code>:</p>
<pre class="brush:c#;">
public void GetUserReturnsBadRequestResultWhenIdIsNull()
{
    // Arrange
...
<p>На выходе из метода должен возвращаться объект <code>BadRequestResult</code>, что мы и проверяем.</p>
<p>Вторая ситуация связана с тем, что в репозитории может не оказаться объект с переданным id, поэтому метод возвращает NotFoundResult:</p>
<pre class="brush:c#;">
User user = repo.Get(id.Value);
if (user == null)
    return NotFound();
</pre>
<p>Для ее тестирования предназначен метод <code>GetUserReturnsNotFoundResultWhenUserNotFound()</code>:</p>
<pre class="brush:c#;">
public void GetUserReturnsNotFoundResultWhenUserNotFound()
{
    int testUserId = 1;
...
<p>Тест проверяет возвращаемый результат - он должен представлять объект <code>NotFoundResult</code>.</p>
<p>Третья ситуация в методе GetUser контроллера HomeController связана с получением объекта из репозиторяи и передачи его в представление:</p>
<pre class="brush:c#;">return View(user);</pre>
<p>Чтобы протестировать эту ситуацию, определен метод <code>GetUserReturnsViewResultWithUser()</code>:</p>
<pre class="brush:c#;">
public void GetUserReturnsViewResultWithUser()
{
    // Arrange
...
<p>Здесь для метода репозитория Get в качестве возвращаемого результата устанавливается объект с id=1, который имеется в списке, создаваемом в методе GetTestUsers.</p>
<p>Сам тест проверяет тип результата - ViewResult, тип модели - User, а также значения модели, которые нам известны.</p>
<p>Ну и в конце запустим тесты и убедимся, что методы контроллера соответствуют этим тестам:</p>
<img src="./pics/unittest9.png" alt="Тестирование в ASP.NET Core и Moq" />
...
<div class="nav"><p><a href="./22.4.php">Назад</a><a href="./">Содержание</a><a href="./24.1.php">Вперед</a></p></div></div>
<div class="socBlock">
