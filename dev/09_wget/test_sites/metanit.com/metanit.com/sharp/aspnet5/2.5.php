...
<html >
<head>
<title>ASP.NET Core | Статические файлы</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка запросов к статическим файлам в ASP.NET Core, путь webroot и каталог wwwroot, метод UseStaticFiles">
...
<h2>Статические файлы</h2><div class="date">Последнее обновление: 02.11.2019</div>
...
<p>Для определения пути хранения статических файлов в проекте используются два параметра <span class="b">ContentRoot</span> и <span class="b">WebRoot</span>. 
Статические файлы должны располагаться в каталоге <span class="ii">ContentRoot/WebRoot</span>. По умолчанию "ContentRoot" представляет каталог текущего проекта, 
а "WebRoot" по умолчанию представляет каталог <span class="b">wwwroot</span>. То есть если использовать значения по умолчанию, то статические файлы следует 
располагать в папке "wwwroot", которая должна находиться в текущем проекте. Но естественно эти параметры можно переопределить.</p>
<p>В разных типах проектов ASP NET Core данная папка может уже быть по умолчанию в проекте, а может отсутствовать. Например, в проекте по типу Empty данная 
папка отсутствует, поэтому ее надо добавлять вручную.</p>
...
<span class="b">index.html</span>. Для его добавления нажмем на wwwroot правой кнопкой мыши и далее в контекстном меню выберем <span class="b">Add -&gt;New Item</span>. Затем в окне добавления 
нового элемента найдем шаблон для html-файлов:</p>
<img src="./pics/2.3.png" alt="Добавление html-файла в проект ASP.NET Core" />
<p>Структура проекта после добавления папки и файла index.html:</p>
<img src="./pics/2.31.png" alt="Статические файлы в ASP.NET Core" />
...
<p>Но чтобы приложение могло бы отдавать статические файлы клиенту, нам надо добавить в конвейер обработки запроса в методе <code>Configure()</code> 
класса <code>Startup</code> компонент middleware в виде метода расширения <span class="b">UseStaticFiles()</span>:</p>
<pre class="brush:c#;">
...
<p>Теперь, если мы обратимся к добавленному файлу, например, по пути <span class="ii">http://localhost:58666/index.html</span>, 
то нам отобразится содержимое данной веб-страницы:</p>
<img src="./pics/2.7.png" alt="Обращение к статическим файлам в ASP.NET Core" />
<p>По всем остальным запросам браузер выводил бы строку "Hello World".</p>
<p>Если бы index.html находился бы в какой-то вложенной папке, например, в <span class="ii">wwwroot/html/</span>, то для обращения мы можем использовать следующий запрос: 
<span class="ii">http://localhost:58666/html/index.html</span></p>
...
<span class="ii">static</span>. 
Для этого добавим папку static в проект, и затем в ней определим какой-нибудь html-файл. Пусть он будет называться <span class="ii">content.html</span>:</p>
<img src="./pics/2.32.png" alt="Изменение пути к webroot в ASP.NET Core" />
...
<p>Для объекта IWebHostBuilder определен метод <span class="b">UseWebRoot()</span>, который позволяет переопределить папку.</p>
<p>И после этого мы также сможем обращаться к статическим файлам, только теперь они будут браться из папки static, а не из wwwroot.</p>
...
<div class="nav"><p><a href="./2.21.php">Назад</a><a href="./">Содержание</a><a href="./2.14.php">Вперед</a></p></div></div>
<div class="socBlock">
