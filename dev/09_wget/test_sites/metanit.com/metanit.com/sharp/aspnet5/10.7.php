...
<html >
<head>
<title>ASP.NET Core | CacheTagHelper</title>
<meta charset="utf-8" />
<meta name="description" content="Кэширование в представлениях в ASP.NET Core MVC, использование класса CacheTagHelper">
...
<h2>CacheTagHelper</h2><div class="date">Последнее обновление: 06.12.2019</div>
...
<p><span class="b">CacheTagHelper</span> обертывает контент представления и позволяет кэшировать его в памяти сервера. 
Он использует тег <code>&lt;cache&gt;</code>, и весь контент, определенный внутри этого тега, кэшируется в памяти. 
...
тогда Razor обрабатывает контент, а тег-хелпер сохраняет контент в memory cache для последующего использования.</p>
<p>Например, пусть у нас есть частичное представление <span class="b">TimeView.cshtml</span>, которое просто выводит текущее время:</p>
<pre class="brush:xml;">
&lt;p&gt;Время из DateView: &lt;b&gt;@DateTime.Now.ToString("HH:mm:ss")&lt;/b&gt;&lt;/p&gt;
</pre>
<p>Пусть оно подключается в какое-нибудь другое представление, например, в Index.cshtml:</p>
<pre class="brush:xml;">
...
<p>Здесь вызов частичного представления TimeView помещен в элемент <span class="b">cache</span>, что позволяет кэшировать содержимое частичного представления. 
И основное представление также выводит текущее время, что позволяет нам сравнить два результата.</p>
<p>И если в течение короткого времени (20 минут) мы несколько раз обратимся к приложению, то увидим, что время из частичного представления застыло и не 
изменяется:</p>
<img src="./pics/cachetaghalper.png" alt="CacheTagHelper in ASP.NET Core MVC" />

<p>Рассмотрим атрибуты, которые использует данный тег.</p>
<h3>expires-after</h3>
<p><code>expires-after</code> указывает, на какое время контент будет кэшироваться. В качестве значения атрибут принимает объект 
<span class="b">TimeSpan</span>:</p>
<pre class="brush:xml;">
&lt;cache expires-after="@TimeSpan.FromMinutes(10)"&gt;  
    @await Html.PartialAsync("TimeView")
&lt;/cache&gt;  
</pre>
<p>В данном случае контент кэшируется на 10 минут.</p>
<h3>expires-on</h3>
<p><code>expires-on</code> указывает, когда именно истечет срок хранения контента в кэше. В качестве значения атрибут принимает объект 
<span class="b">DateTime</span>:</p>
<pre class="brush:xml;">
&lt;cache expires-on="@DateTime.Now.AddDays(1)"&gt;  
    @await Html.PartialAsync("TimeView")
&lt;/cache&gt;  
</pre>
<p>Здесь срок кэширования истекает через день.</p>
<h3>expires-sliding</h3>
<p><code>expires-sliding</code> определяет, через какое время с момента последнего посещения контент будет удаляться из кэша. 
В качестве значения атрибут принимает объект <span class="b">TimeSpan</span>:</p>
<pre class="brush:xml;">
...
<h3>vary-by-user</h3>
<p><code>vary-by-user</code> позволяет кэшировать контент отдельно для каждого залогиненного пользователя (для которого установлено значение <code>User.Identity.Name</code>). При кэшировании к ключу контента в кэше 
добавляется логин пользователя. В качестве значения атрибут принимает логическое значение true (надо кэшировать по пользователю) или false:</p>
<pre class="brush:xml;">
...
<h3>vary-by-route</h3>
<p>Данный атрибут позволяет кэшировать различные версии одного и того же контента в зависимости от параметров маршрута. 
В качестве значения атрибут принимает названия параметров через запятую, которые будут учитываться при кэшировании. 
И затем в кэше для контента к ключу будет добавляться значение параметров:</p>
<pre class="brush:xml;">
&lt;cache expires-after="@TimeSpan.FromMinutes(15)" vary-by-route="id"&gt;  

&lt;/cache&gt;  
</pre>
<p>В данном случае будут создаваться разные версии одного и того же контента для разных значений параметра id.</p>
<h3>vary-by-query</h3>
<p><code>vary-by-query</code> позволяет кэшировать различные версии контента в зависимости от значений параметров, переданных в запросе. 
В качестве значения атрибуту передается список параметров через запятую. При кэшировании к ключу контента в кэше 
добавляется значение этих параметров.</p>
<pre class="brush:xml;">
&lt;cache vary-by-query="name"&gt;  

&lt;/cache&gt;  
</pre>
<p>Здесь для каждого значения параметра name будут кэшироваться свои копии контента.</p>
<h3>vary-by-cookie</h3>
<p>Данный атрибут позволяет кэшировать различные версии одного и того же контент в зависимости от значений, которые хранятся в куках. 
В качестве значения атрибут принимает названия куков через запятую, которые будут учитываться при кэшировании. 
В ходе кэширования значения этих кук будут добавляться к ключам контента в кэше.</p>
<pre class="brush:xml;">
&lt;cache vary-by-cookie="pubid"&gt;  

&lt;/cache&gt;  
</pre>
<p>Здесь предполагается, что наше приложение использует куку "pubid". И для каждого ее значения будут кэшироваться свои копии контента.</p>
<h3>vary-by-header</h3>
<p><code>vary-by-header</code> позволяет кэшировать различные версии контента в зависимости от значений заголовков запроса. 
...
может использоваться заголовок User-Agent:</p>
<pre class="brush:xml;">
...
<h3>vary-by</h3>
<p><code>vary-by</code> позволяет кэшировать различные версии контента в зависимости от произвольного строкового значения. 
Например, кэшируем в зависимости от значения <code>ViewBag.Id</code>:</p>
<pre class="brush:xml;">
...
<h3>priority</h3>
<p>Определяет приоритет кэшируемого контента. Приоритет может иметь значение, если для размещения кэша не хватает памяти. 
В этом случае из кэша могут удаляться некоторые объекты. И чем ниже приоритет, тем больше вероятность что данные объекты будут удалены при нехватке памяти.</p>
<p>Атрибут <code>priority</code> принимает одно из значений перечисления <span class="bb">Microsoft.Extensions.Caching.Memory.CacheItemPriority</span>:</p>
<ul>
<li><p><span class="b">Low</span>: низкий приоритет</p></li>
<li><p><span class="b">High</span>: высокий приоритет</p></li>
<li><p><span class="b">NeverRemove</span>: контент никогда не удаляется из кэша</p></li>
<li><p><span class="b">Normal</span>: средний приоритет</p></li>
</ul>
<p>Например:</p>
<pre class="brush:xml;">
...
<div class="nav"><p><a href="./10.6.php">Назад</a><a href="./">Содержание</a><a href="./10.8.php">Вперед</a></p></div></div>
<div class="socBlock">
