...
<html >
<head>
<title>ASP.NET Core | Фильтры</title>
<meta charset="utf-8" />
<meta name="description" content="Фильтры в ASP.NET Core, filter pipeline, типы фильтров, их определение и применение">
...
<h1>Фильтры</h1><h2>Введение в фильтры</h2><div class="date">Последнее обновление: 11.12.2019</div>
...
<p>Фильтры позволяют выполнять некоторые действия до или после определенной стадии обработки запроса. В ASP.NET Core имеются следующие типы фильтров:</p>
<ul>
...
<li><p><span class="b">Фильтры RazorPages</span>: применяется только к страницам RazorPages, выполняются перед и после обработки запроса страницей Razor Page</p></li>
<li><p><span class="b">Фильтры исключений</span>: определяют действия в отношении необработанных исключений</p></li>
<li><p><span class="b">Фильтры результатов действий</span>: фильтр применяется к результатам методов контроллера и страниц Razor Pages, выполняется как до, 
...
<img src="./pics/filters1.png" alt="Filter Pipeline in ASP.NET Core" />
...
<tr><td><p>Фильтр RazorPages</p></td><td><p>IPageFilter</p></td><td><p>IAsyncPageFilter</p></td></tr>
...
<p>Асинхронные интерфейсы фильтров называются <code>IAsync[Stage]Filter</code>, и они определяют только один метод - <code>On[Stage]ExecutionAsync</code> (исключение - фильтр RazorPages, где определяется два метода)</p>
<pre class="brush:c#;">
...
<p>К примеру, создадим свой фильтр. Для этого создадим новый проект ASP.NET Core по типу <span class="b">ASP.NET Core Web App (Model-View-Controller)</span>. 
Затем в этот проект добавим папку <span class="b">Filters</span>, в которой будут храниться фильтры. И далее добавим в эту папку новый класс 
...
который выполняется до методов контроллера или страницы Razor Pages и соответственно до выполнения результата и формирования ответа пользователю. 
...
<p>Также стоит отметить, что класс фильтра также наследует класс Attribute, так как фильтры реализуются в основном именно как атрибуты, что упрощает их применение.</p>
<h3>Применение фильтров</h3>
<p>Фильтры могут иметь различную область действия:</p>
<ul>
<li><p>Метод контроллера. Атрибут применяется к методу контроллера:</p>
<pre class="brush:c#;">
...
<li><p>Весь контроллер или страница RazorPage.</p>
...
<p>Атрибут применяется к классу страницы Razor Page:</p>
<pre class="brush:c#;">
[SimpleResourceFilter]
...
<li><p>Глобальная область действия, при которой фильтр применяется ко всем методам всех контроллеров.</p>
<p>Для определения фильтра как глобального нам надо изменить в методе <code>ConfigureServices()</code> класса Startup подключение соответствующих сервисов MVC. 
Подключение глобально для всех сервисов MVC (и для контроллеров, и для Razor Pages):</p>
<pre class="brush:c#;">
...
<p>Установка отдельно для контроллеров:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
...
<p>Установка отдельно для RazorPages:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
...
<p>В нашем случае выберем глобальную область действия и определим фильтр как глобальный:</p>
<pre class="brush:c#;">
services.AddControllersWithViews(options=&gt; options.Filters.Add(new SimpleResourceFilter()));
</pre>
<p>Таким образом, мы встроили наш фильтр в конвейер фильтров.</p>
<p>Запустим приложение, и после получения ответа в куках окажутся данные с ключом "LastVisit":</p>
<img src="./pics/filters3.png" alt="Применение фильтров в ASP.NET Core" />
<p>Если бы мы не использовали бы фильтр, то в каждом методе нам надо было бы определить строку кода:</p>
...
<p>Это фильтр ресурсов, который устанавливает свойство <code>Result</code>. Значение этого свойства фактически будет использоваться в качестве результата 
...
<p>При обращении к методу Index сработает фильтр FakeNotFoundResourceFilter, который установит результат обработки запроса. И после него фильтр SimpleResourceFilter 
уже не будет действовать.</p>
...
<div class="nav"><p><a href="./23.6.php">Назад</a><a href="./">Содержание</a><a href="./18.5.php">Вперед</a></p></div></div>
<div class="socBlock">
