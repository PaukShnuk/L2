...
<html >
<head>
<title>ASP.NET Core | Подключение React</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение React в ASP.NET Core">
...
<h1>React.JS</h1><h2>Подключение React в ASP.NET Core</h2><div class="date">Последнее обновление: 21.11.2019</div>
...
<p>В руководстве по <a href="//metanit.com/web/react">React</a> были рассмотрены базовые моменты работы с React.JS, компоненты, их определение, состояние и так далее. 
В рамках же этой главы в данной же главе рассмотрим непосредственно использование React в приложении на ASP.NET Core.</p>
<p>Итак, создадим новый проект ASP.NET Core. В качестве типа проекта выберем тип Empty:</p>
<img src="./pics/react1.png" alt="Создание проекта ASP.NET Core для работы с React JS" />
<p>Первым делом добавим через Nuget необходимые для работы с React пакеты:</p>
<ul>
<li><p>React.AspNet</p></li>
<li><p>JavaScriptEngineSwitcher.Extensions.MsDependencyInjection</p></li>
<li><p>JavaScriptEngineSwitcher.ChakraCore</p></li>
<li><p>JavaScriptEngineSwitcher.ChakraCore.Native.win-x64</p></li>
<li><p>JavaScriptEngineSwitcher.ChakraCore.Native.win-x86</p></li>
<li><p>JavaScriptEngineSwitcher.ChakraCore.Native.osx-x64</p></li>
<li><p>JavaScriptEngineSwitcher.ChakraCore.Native.linux-x64</p></li>
</ul>
<p>Первый пакет представляет библиотеку <a href="https://reactjs.net" rel="nofollow">ReactJS.NET</a>, которая позволяет связать react и 
инфраструктуру .NET. Остальные пакеты необходимы для работы движка ChakraCore:</p>
<img src="./pics/react2.png" alt="Установка React.AspNet" />
<img src="./pics/react6.png" alt="Установка React для ASP.NET Core" />
<p>Далее добавим в проект папку <span class="b">wwwroot</span> для хранения статических файлов. Затем добавим эту папку новый файл <span class="b">index.html</span>:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Здесь определен элемент div, в который будет загружаться компоненты React, а также подключаются из cdn библиотеки самого React.JS и файл app.jsx, который мы далее создадим.</p>
<p>Далее добавим в папку wwwroot новый каталог <span class="b">js</span>, в который добавим новый файл <span class="b">app.jsx</span>. То есть вся структура проекта 
будет выглядеть следующим образом:</p>
<img src="./pics/react3.png" alt="Структура проекта React в ASP.NET Core" />
...
<p>А метод <span class="b">ReactDOM.render</span> производит рендеринг компонента на веб-странице в элементе с id="content".</p>
<p>И в конце изменим конфигурацию в классе <span class="b">Startup</span>:</p>
<pre class="brush:c#;">
...
<p>Чтобы задействовать ReactJS.NET в сервисы приложения добавляется соответствующий сервис:</p>
<pre class="brush:c#;">
services.AddSingleton&lt;IHttpContextAccessor, HttpContextAccessor&gt;();
services.AddReact();
</pre>
<p>Кроме того, добавляется сервис JsEngineSwitcher, который позволяет задействовать определенный движок JS - в данном случае ChakraCora.</p>
<pre class="brush:c#;">
services.AddJsEngineSwitcher(options =&gt; options.DefaultEngineName = ChakraCoreJsEngine.EngineName).AddChakraCore();
</pre>
<p>Затем в методе Configure вызываются сервисы React:</p>
<pre class="brush:c#;">app.UseReact(config =&gt; { });</pre>
<p>Запустим проект и обратимся к главной странице index.html:</p>
<img src="./pics/react4.png" alt="Используем React JS в ASP.NET Core" />
...
<div class="nav"><p><a href="./27.3.php">Назад</a><a href="./">Содержание</a><a href="./25.2.php">Вперед</a></p></div></div>
<div class="socBlock">
...
$(function() { 
  ...
  $("#toggle-theme").on("click", function() {
	...
	$("li:contains('Глава 33.')").removeClass('expandable').addClass('collapsable').children('ul').css({"display":"block"});
	$("li:contains('Глава 33.')").children('div').removeClass('expandable-hitarea').addClass('collapsable-hitarea');
