...
<html >
<head>
<title>ASP.NET Core | Политики CORS</title>
<meta charset="utf-8" />
<meta name="description" content="Установка политик CORS в ASP.NET Core в методе AddCors, объект CorsPolicyBuilder и его методы">
...
<h2>Политики CORS</h2><div class="date">Последнее обновление: 22.12.2019</div>
...
<p>Кроме непосредственной установки конфигурации CORS в app.UseCors, можно определять политики CORS. Политики определяют набор правил взаимодействия сервера и клиента. А 
чтобы задействовать нужную политику, в метод app.UseCors передается ее название:</p>
<pre class="brush:c#;">
...
<p>В метод AddCors передается делегат, который принимает объект <span class="b">CorsOptions</span> - настройки CORS. У этого объекта вызывается метод 
<span class="b">AddPolicy()</span>, который устанавливает политику. Первый параметр метода - произвольное название политики (в данном случае AllowLocalhost44321). 
Второй параметр - делегат, который для создания политики использует объект <span class="b">CorsPolicyBuilder</span>. То есть это тот же самый объект, методы которого были рассмотрены 
в прошлой теме. Поэтому для него можно использовать все те же самые методы:</p>
<pre class="brush:c#;">
services.AddCors(options =&gt; options.AddPolicy("AllowLocalhost44321", builder =&gt; builder
                    .WithOrigins("https://localhost:44321")
                    .AllowAnyHeader()
...
<p>Чтобы применить эту политику в метод app.UseCors передается ее название:</p>
<pre class="brush:c#;">app.UseCors("AllowLocalhost44321");</pre>
<p>При этом можно определить набор политик и применять одну из них, меняя ее по мере необходимости:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./31.2.php">Назад</a><a href="./">Содержание</a><a href="./31.5.php">Вперед</a></p></div></div>
<div class="socBlock">
