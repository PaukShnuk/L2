...
<html >
<head>
<title>ASP.NET Core | Проекция конфигурации на классы</title>
<meta charset="utf-8" />
<meta name="description" content="Проекция конфигурации json и xml на классы в проекте ASP.NET Core, метод Bind и Get у IConfiguration, привязка отдельных секций конфигурации к объектам классов C#">
...
<h2>Проекция конфигурации на классы</h2><div class="date">Последнее обновление: 14.11.2019</div>
...
<p>В ASP.NET Core мы можем не просто получать конфигурационные настройки в виде разрозненных пар ключ-значение, но и проецировать данные конфигурации 
на классы C#.</p>
...
<p>Под него в проекте определим класс Person:</p>
<pre class="brush:c#;">
public class Person
{
...
<p>Теперь изменим класс Startup и свяжем конфигурацию из файла json с объектом класса Person:</p>
<pre class="brush:c#;">
...
<p>Ключевой момент заключается в применении метода Bind:</p>
<pre class="brush:c#;">
var tom = new Person();
AppConfiguration.Bind(tom);
</pre>
...
<img src="./pics/config9.png" alt="Привязка конфигурации json и класса C# в ASP.NET Core" />
...
<pre class="brush:c#;">Person tom = AppConfiguration.Get&lt;Person&gt;();</pre>
<p>В этом случае нам не надо предварительно создавать объект класса Person.</p>
<h3>Привязка сложных объектов</h3>
<p>Рассмотрим привязку более сложных по структуре данных. Определим следующий файл <span class="ii">person.json</span>:</p>
<pre class="brush:js;">
{
  "age": 18,
  "name": "Tom",
...
<p>Теперь выполним привязку из конфигурации json в объекты классов C# в классе Startup:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Builder;
using Microsoft.AspNetCore.Hosting;
using Microsoft.AspNetCore.Http;
...
<img src="./pics/config8.png" alt="Привязка комплексных объектов из конфигурации json к классам C# в ASP.NET Core" />
...
<p>В классе Startup изменим конструктор, установив применение конфигурации из выше определенного файла xml:</p>
<pre class="brush:c#;">
public Startup()
{
	var builder = new ConfigurationBuilder()
...
<p>Весь остальной код остается прежним.</p>
...
<img src="./pics/config10.png" alt="Привязка секции конфигурации к объекту класса C# в ASP.NET Core" />
...
<div class="nav"><p><a href="./2.15.php">Назад</a><a href="./">Содержание</a><a href="./6.3.php">Вперед</a></p></div></div>
<div class="socBlock">
