...
<html >
<head>
<title>ASP.NET Core | Бандлинг и минификация</title>
<meta charset="utf-8" />
<meta name="description" content="Бандлинг и минификация скриптов и стилей с помощью BundlerMinifier в ASP.NET Core">
...
<h1>Клиентская разработка</h1><div class="date">Последнее обновление: 16.12.2019</div>
...
<p>При работе со скриптами и стилями в проектах веб-приложений в самых различных технологиях нередко используются различные средства автоматизации задач. Например, одной из распространенных задач, где применяются эти средства, являются бандлинг и минификация, которые 
...
разработки.</p>
<h2>Бандлинг и минификация</h2>
<p>У нас в проекте может быть множество файлов скриптов, стилей. Однако чем больше разных скриптов и стилей подключено на веб-странице, тем больше запросов надо совершить браузеру, а 
серверу обработать. Что увеличивает нагрузку на сервер.</p>
<p>Скрипты и стили могут содержать множество строк. И опять же чем больше в файле строк, тем больше серверу надо передать байт на сторону клиенту. Поэтому также с ростом 
размера файлов возрастает нагрузка на сервер.</p>
<p>Для решения этих проблем применяются два механизма: бандлинг и минификация.</p>
<p><span class="b">Бандлинг</span> представляет соединение скриптов или стилей в один общий файл или бандл.</p>
<p><span class="b">Минификация</span> представляет сокращение содержимого скриптов или стилей.</p>
<p>Для проведения бандлинга и минификации в веб-разработке могут применяться самые различные утилиты, инструменты, например, Grunt, Gulp. Но рекомендуемым способом именно 
по отношению к ASP.NET Core является использование расширения Visual Studio - BundlerMinifier.</p>
<p>Для работы с BundlerMinifier нам надо загрузить и установить данное расширение по 
<a href="https://visualstudiogallery.msdn.microsoft.com/9ec27da7-e24b-4d56-8064-fd7e88ac1c40" rel="nofollow">ссылке</a>.</p>
<h3>Конфигурация BundlerMinifier</h3>
<p>Конфигурация BundlerMinifier задается с помощью файла <span class="b">bundleconfig.json</span>. Он определяется в в следующем формате:</p>
<pre class="brush:js;">
[
  {
...
<p>Фактически файл определяет массив бандлов. Для настройки каждого отдельного бандла применяется ряд опций:</p>
<pre class="brush:js;">
{
...
<p>Параметр <code>outputFileName</code> задает путь выходного файла, который будет формироваться в результате объединения файлов.</p>
<p>Параметр <code>inputFiles</code> определяет через запятую набор файлов, которые будут объединяться. То есть в данном случае из файла "wwwroot/js/site.js" 
будет формироваться файл "wwwroot/js/site.min.js"</p>
<p>Дополнительный параметр <code>minify</code> указывает, будут ли минифицироваться включаемые в бандл файлы. Значение <code>enabled: true</code> включает минификацию. 
А значение <code>"renameLocals": true</code> позволяет сократить имена локальных переменных.</p>
<p>Последний параметр <code>sourceMap</code> указывает, надо ли генерировать файл-карту сопоставления исходного и выходного файлов.</p>
<h3>Применение BundlerMinifier</h3>
<p>Создадим новый проект ASP.NET Core по типу ASP.NET Core Web App (Model-View-Controller). Для работы с BundlerMinifier добавим в проект ряд дополнительных стилей и скриптов:</p>
<img src="./pics/bundle1.png" alt="BundlerMinifier в ASP.NET Core" />
<p>Допустим, в папке wwwroot/css будут два файла стилей: site.css и style2.css. А в папке wwwroot/js - site.js, site2.js, site3.js.</p>
<p>Также добавим в проект файл <span class="b">bundleconfig.json</span> (если он отсутствует в проекте). Затем изменим его следующим образом:</p>
<pre class="brush:js;">
[
  {
...
<p>Также надо отметить, что нам необязательно исправлять файл вручную. Мы можем также выделить все нужные элементы, нажать на выделенный элемент правой кнопкой мыши и 
в контекстном меню выбрать пункт <span class="b">Bundler &amp; Minifier -&gt; Bundle and Minify Files</span>:</p>
<img src="./pics/bundle2.png" alt="Автоматизация BundlerMinifier в ASP.NET Core" />
<p>После выбора этой опции в диалоговом окне надо будет ввести расположение для выходного файла, в который будет собираться бандл. По умолчанию дается имя <code>bundle.js</code>, как указано 
в файле bundleconfig.json, но его можно изменить.</p>
<p>После этого в папке с js-скриптами появятся новые файлы - полная и минимизированная версия файла bundle.js, который объединяется все скрипты.</p>
<img src="./pics/bundle4.png" alt="Bundling в ASP.NET Core MVC" />

<p>Теперь перейдем в меню <span class="b">View -&gt; Other Windows -&gt; Task Runner Explorer</span>. После этого внизу Visual Studio откроется окно 
Task Runner Explorer. В этом окне нажмем правой кнопкой мыши на пункт <span class="b">Update all files</span>:</p>
<img src="./pics/bundle3.png" alt="Task Runner Explorer в Visual Studio 2019" />
<p>При открытии контекстого меню нам надо установить привязку с помощью пункта Binding. Здесь имеются четыре типа привязки:</p>
<ul>
<li><p><code>Before Build</code>: бандлы будут собираться перед построением проекта</p></li>
<li><p><code>After Build</code>: бандлы будут собираться после построения проекта</p></li>
<li><p><code>Clean</code>: бандлы будут собираться при очистке проекта</p></li>
<li><p><code>Project Open</code>: бандлы будут собираться при открытии проекта</p></li>
</ul>
<p>К примеру выберем опцию <code>Before Build</code>. И далее мы сможем использовать бандлы, подключив стандартным образом:</p>
<pre class="brush:xml;">
&lt;link rel="stylesheet" href="~/css/bundle.min.css" /&gt;
&lt;script src="~/js/bundle.min.js"&gt;&lt;/script&gt;
</pre>
<p>Причем не важно, что на данный момент нет таких файлов. Они будут формироваться автоматически при каждом построении проекта.</p>
<p>Стоит отметить, что скрипты и стили не обязательно должны располагаться в каталоге wwwroot. Например, мы можем дополнительно определить в проекте 
специальную папку, например, назовем ее <span class="ii">static</span>, и расположить их в этой папке. А бандлы формировать и подключать из wwwroot:</p>
<img src="./pics/bundle5.png" alt="Подключение бандлов стилей и скриптов в ASP.NET Core MVC" />
<p>В этом случае мы могли бы настроить файл bundleconfig.json следующим образом:</p>
<pre class="brush:js;">
[
  {
    "outputFileName": "wwwroot/css/bundle.css",
...
<div class="nav"><p><a href="./16.12.php">Назад</a><a href="./">Содержание</a><a href="./13.7.php">Вперед</a></p></div></div>
<div class="socBlock">
