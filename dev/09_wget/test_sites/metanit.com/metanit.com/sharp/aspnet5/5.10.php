...
<html >
<head>
<title>ASP.NET Core | Передача зависимостей в контроллер</title>
<meta charset="utf-8" />
<meta name="description" content="Передача зависимостей в контроллер в ASP.NET Core MVC, объекты HttpContext и ControllerContext, Request и Response">
...
<h2>Передача зависимостей в контроллер</h2><div class="date">Последнее обновление: 18.11.2019</div>
...
<p>Например, пусть в проекте определен интерфейс ITimeService и его реализация SimpleTimeService:</p>
<pre class="brush:c#;">
...
<p>И в классе Startup происходит регистрация сервиса ITimeService:</p>
<pre class="brush:c#;">
...
<p>Когда приходит запрос к контроллеру, инфраструктура MVC вызывает провайдер сервисов для создания объекта HomeController. 
...
зависимостей и передает их в конструкторо HomeController для создания объекта контроллера, который затем обрабатывает запрос.</p>
<p>Например, получим зависимость в конструкторе контроллера:</p>
<pre class="brush:c#;">
...
<h3>Передача зависимостей в методы. FromServices</h3>
<p>Иногда зависимость используется только в одном методе. И в этом случае нет необходимости передавать ее в контроллер, поскольку она напрямую может 
...
<p>Атрибут FromServices предоставляется инфраструктурой MVC - он определен в пространстве имен Microsoft.AspNetCore.Mvc.</p>
<h3>HttpContext.RequestServices</h3>
<p>В методах контроллера можно обращаться к объекту контекста запроса через свойство HttpContext, а через свойство HttpContext.RequestServices 
...
<div class="nav"><p><a href="./5.9.php">Назад</a><a href="./">Содержание</a><a href="./7.1.php">Вперед</a></p></div></div>
<div class="socBlock">
