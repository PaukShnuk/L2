...
<html >
<head>
<title>ASP.NET Core | Основы валидации</title>
<meta charset="utf-8" />
<meta name="description" content="Основы валидации модели в ASP.NET Core, скрипты и атрибуты валидации, свойство ModelState.IsValid">
...
<h1>Метаданные и валидация модели</h1><h2>Основы валидации</h2><div class="date">Последнее обновление: 08.12.2019</div>
...
<p>Важную роль в ASP.NET Core играет валидация входных данных. Валидация позволяет проверить входные данные на наличие неправильных, корректных значений и должным 
образом обработать эти значения.</p>
<p>Для рассмотрения валидации создадим новый проект ASP.NET Core по типу Web Application (Model-View-Controller), который назовем ValidationApp. Вначале добавим в проект новую папку Models для моделей и определим  
...
<p>Здесь каждое свойство модели помечено атрибутом Required, который находится в пространстве имен <code>System.ComponentModel.DataAnnotations</code>.</p>
<p>Затем определим в контроллере HomeController действие Create, через которое мы будем добавлять на сервер объект модели Person:</p>
<pre class="brush:c#;">
...
<p>Если данные для модели были введены правильно, то через метод <code>Content()</code> эти данные будут выведены в браузере. Если же были введены некорректные данные, 
то возвращаем объект Person в метод View.</p>
<p>Далее определим само представление <span class="b">Create.cshtml</span>, которое будет содержать форму для ввода данных:</p>
<pre class="brush:xml;">
...
<p>Это стандартное строго типизированное представление, которое использует tag-хелперы для создания полей для свойств модели Person. 
И кроме того, здесь определена секция <span class="b">scripts</span>. В этой секции подключаются скрипты клиентской валидации либо из внешних CDN, либо непосредственно из проекта. 
В частности, в папке <span class="b">wwwroot/lib</span> по умолчанию уже добавлены соответствующие скрипты:</p>
<img src="./pics/validation1.png" alt="Скрипты валидации в ASP.NET Core" />
<p>И поскольку эти скрипты зависят от библиотеки jquery, то нам надо подключать их именно после этой библиотеки. На мастер-странице <span class="ii">_Layout.cshtml</span> 
jquery уже подключена, и после ее подключения определена секция scripts, поэтому в представлении Create.cshtml подключение скриптов клиентской валидации определено именно в секции scripts.</p>
<pre class="brush:xml;">
...
<p>Хотя это необязательно, главное, чтобы перед этими скриптами была подключена jquery.</p>
<p>Теперь запустим приложение и обратимся к методу Create. И ничего не вводя в поля ввода, нажмем на кнопку отправки:</p>
<img src="./pics/validation2.png" alt="Основы валидации в ASP.NET Core" />
<p>По факту форма даже не отправляется, а справа от поле ввода мы увидим сообщения о необходимости ввода данных.</p>
<p>В данном случае мы задействовали механизм валидации, который используется в mvc по умолчанию. Каковы же основные моменты валидации в данном случае:</p>
<ol>
<li><p>Использование атрибутов валидации при объявлении модели. Мы указали для каждого свойства атрибут <code>Required</code>, благодаря чему фреймворк знает, 
что данное свойство обязательно должно содержать некоторое значение.</p></li>
<li><p>Использование tag-хелперов валидации. При каждом свойстве мы используем хелпер валидации <code>&lt;span asp-validation-for="[Название свойства]" /&gt;</code>:</p>
<pre class="brush:xml;">
&lt;div&gt;
    &lt;label asp-for="Name"&gt;&lt;/label&gt;&lt;br /&gt;
    &lt;input type="text" asp-for="Name" /&gt;
...
<p>Благодаря чему и отображается сообщение об ошибке.</p>
</li>
<li><p>Использование скриптов валидации в представлении:</p>
<pre class="brush:xml;">
&lt;script src="~/lib/jquery-validation/dist/jquery.validate.min.js"&gt;&lt;/script&gt;
&lt;script src="~/lib/jquery-validation-unobtrusive/jquery.validate.unobtrusive.min.js"&gt;&lt;/script&gt;
</pre>
<p>Несмотря на то что, в принципе валидация может работать и без этих скриптов, однако же эти скрипты позволяют валидировать значение прямо на клиенте без их отправки 
на сервер</p>
<p>В Visual Studio 2019 в папку Views/Shared по умолчанию добавляется частичное представление <span class="b">_ValidationScriptsPartial.cshtml</span>, 
...
использующие клиентскую валидацию.</p></li>
<li><p>Валидация на стороне сервера. Если у пользователя отключен javascript а браузере, или в представлении не добавлены скрипты клиентской валидации, 
то форма благополучно отправится на сервер. Но во фреймворке предусмотрена также валидация на стороне сервера:</p>
<pre class="brush:c#;">
if (ModelState.IsValid)
    return Content($"{person.Name} - {person.Email}");
else
...
<p>С помощью свойства <span class="b">ModelState.IsValid</span> мы узнаем, проходит модель валидацию или нет, и зависимости от результата совершаем те или иные действия.</p>
</li>
</ol>
...
<div class="nav"><p><a href="./7.15.php">Назад</a><a href="./">Содержание</a><a href="./19.2.php">Вперед</a></p></div></div>
<div class="socBlock">
