...
<html >
<head>
<title>ASP.NET Core | Определение культуры</title>
<meta charset="utf-8" />
<meta name="description" content="Определение культуры в ASP.NET Core, класс CultureInfo и его свойства CurrentCulture и CurrentUICulture, установка локали">
...
<h1>Глобализация и локализация</h1><h2>Определение культуры</h2><div class="date">Последнее обновление: 18.12.2019</div>
...
<p>Культура в ASP.NET Core, как и в целом в .NET, определяется объектом класса <span class="b">CultureInfo</span>, который находится в пространстве имен 
<span class="b">System.Globalization</span>. Объект CultureInfo определяет функционал, который зависит от культурного контекста, например, форматирование 
дат, времени, чисел, валюты, работа с календарем. И при запуске приложения каждый поток в .NET определяет два объекта: <span class="b">CultureInfo.CurrentCulture</span> - текущую языковую 
культуру и <span class="b">CultureInfo.CurrentUICulture</span> - языковая культура для пользовательского интерфейса. 
ASP.NET Core использует эти свойства для рендеринга значений, которые зависят от настройки культуры. Например, в зависимости от культуры может меняться отображение даты и времени.</p>
<p>Каждая культура описывается в специальном формате <span class="b">RFC 4646</span>, который позволяет представить культуру в виде:</p>
<pre class="brush:c#;">культура-субкультура</pre>
<p>Здесь "культура" представляет двузначный код ISO 639 в нижнем регистре, который ассоциирован с определенным языком. Например, "de" - немецкий язык или 
"es" - испанский. После дефиса идет второй параметр - "субкультура", который представляет код ISO 3166 в верхнем регистре, ассоциированный со страной или регионом. 
Например, английский язык в США и английский в Великобритании имеют свои особенности, поэтому для них существуют разные коды: "en-US" и "en-GB". Но если 
нам не важны региональные особенности, связанные с определенным языком, то мы можем представить культуру с помощью одного кода ISO 639, например, "en".</p>
<p>Теперь собственно как мы можем работать с культурой в рамках приложения ASP.NET Core. В данном случае нас будет интересовать, как используя культуру, сделать 
локализацию приложения, чтобы оно стало доступным для различных языков.</p>
<p>Вначале просто рассмотрим процесс получения и установки культуры. Для этого возьмем проект ASP.NET Core по типу <span class="b">Web Application (Model-View-Controller)</span> 
и определим в нем следующий контроллер:</p>
<pre class="brush:c#;">
using System;
...
<p>Метод GetCulture принимает в виде параметра код и с его помощью устанавливает текущую культуру. Затем установленные значения выводятся в браузере.</p>
<p>Если методу GetCulture не передать параметра code, то он будет использовать культуру по умолчанию:</p>
<img src="./pics/localization1.png" alt="Глобализация и локализация в ASP.NET Core" />
<p>Изменим текущую культуру на немецкоязычную, то есть на "de":</p>
<img src="./pics/localization2.png" alt="CultureInfo.CurrentCulture в ASP.NET Core" />
<p>Как мы видим, получение и установка культуры не вызывают никакой сложности. И мы вполне можем написать свой компонент middleware, который сможет взять тот же код, который передается 
методу GetCulture, из различных источников - куки, сессии, строки запроса, отправленных форм и т.д., и установить культуру.</p>
<p>Например, добавим в проект новый класс, который назовем CultureMiddleware:</p>
<pre class="brush:c#;">
...
<p>Этот компонент получает из строки запроса параметр lang и использует его значение для установки культуры. Но кроме параметров мы можем использовать множество других источников для установки культуры - куки, 
сессии, кэш, данные отправленных форм, какие-то другие хранилища.</p>
<p>Для упрощения его внедрения в конвейер определен метод расширения UseCulture.</p>
<p>Используем этот middleware в классе Startup:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Builder;
...
<p>Таким образом, при передаче параметра lang еще до начала работы контроллера в приложении уже будет установлена необходимая языковая культура.</p>
<img src="./pics/localization33.png" alt="Установка культуры и локали в ASP.NET Core MVC" />
...
<div class="nav"><p><a href="./24.4.php">Назад</a><a href="./">Содержание</a><a href="./28.2.php">Вперед</a></p></div></div>
<div class="socBlock">
