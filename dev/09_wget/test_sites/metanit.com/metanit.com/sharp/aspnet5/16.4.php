...
<html >
<head>
<title>ASP.NET Core | Авторизация пользователей в Identity</title>
<meta charset="utf-8" />
<meta name="description" content="Авторизация пользователей в ASP.NET Core Identity, создание функционала входа и выхода в веб-приложение">
...
<h2>Авторизация пользователей в Identity</h2><div class="date">Последнее обновление: 14.12.2019</div>
...
<p>Для создания механизма авторизации пользователей в приложении вначале добавим в проект из прошлой темы в папку ViewModels специальную модель <span class="b">LoginViewModel</span>:</p>
<pre class="brush:c#;">
...
<p>В прошлой теме у нас уже был добавлен контроллер AccountController. Теперь же добавим в него следующие три метода:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>В Get-версии метода Login мы получаем адрес для возврата в виде параметра returnUrl и передаем его в модель LoginViewModel.</p>
<p>В Post-версии метода Login получаем данные из представления в виде модели LoginViewModel. Всю работу по аутентификации пользователя выполняет метод 
<code>signInManager.PasswordSignInAsync()</code>. Этот метод принимает логин и пароль пользователя. Третий параметр метода указывает, надо ли сохранять 
устанавливаемые куки на долгое время.</p>
<p>Данный метод также возвращает IdentityResult, с помощью которого можно узнать, завершилась ли аутентификация успешно. Если она завершилось успешно, 
то используем свойство ReturnUrl модели LoginViewModel для возврата пользователя на предыдущее место. Для этого нужно еще удостовериться, что адрес возврата 
принадлежит приложению с помощью метода <code>Url.IsLocalUrl()</code>. Это позволит избежать перенаправлений на нежелательные сайты. Если же адрес возврата не 
установлен или не принадлежит приложению, выполняем переадресацию на главную страницу.</p>
<p>Третий метод - метод Logout выполняет выход пользователя из приложения. За выход отвечает метод <code>_signInManager.SignOutAsync()</code>, который очищает аутентификационные куки.</p>
<p>И теперь определим в проекте в папке <span class="ii">Views/Account</span> представление <span class="b">Login.cshtml</span>, через которое будет осуществляться 
вход в приложение:</p>
<pre class="brush:xml;">
...
<p>Для проверки логина для простоты определим в представлении Index.cshtml контроллера HomeController вывод имени пользователя и ссылки на вход и регистрацию:</p>
<pre class="brush:xml;">
@{
...
<p>Запустим проект и выполним вход в приложение:</p>
<img src="./pics/customidentity8.png" alt="Авторизация в ASP.NET Identity Core" />
<p>И после удачной авторизации на главной странице увидим свое имя и кнопку для выхода:</p>
<img src="./pics/customidentity9.png" alt="Выход из веб-приложения в ASP.NET Identity Core" />
...
<div class="nav"><p><a href="./16.3.php">Назад</a><a href="./">Содержание</a><a href="./16.7.php">Вперед</a></p></div></div>
<div class="socBlock">
