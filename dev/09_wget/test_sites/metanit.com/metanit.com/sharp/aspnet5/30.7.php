...
<html >
<head>
<title>ASP.NET Core | Передача сложных объектов в SignalR</title>
<meta charset="utf-8" />
<meta name="description" content="Отправка сложных объектов в хабах и клиентах в SignalR в ASP.NET Core">
...
<h2>Отправка сложных объектов</h2><div class="date">Последнее обновление: 25.12.2019</div>
...
<p>В примерах в прошлых темах из хаба в клиент и из клиента в хаб передавались довольно простые данные - одна или две строки. Однако SignalR, 
как и MVC, может связывать наборы данных со сложными объектами. Это значит, что мы можем получать в хабе данных более сложных типов, чем просто строки.</p>
<p>Например, определим следующий класс хаба:</p>
<pre class="brush:c#;">
...
<p>Здесь хаб получает данные в виде объекта User, изменяет значение его свойства Age и отправляет обратно текущему клиенту.</p>
<p>Также определим следующую веб-страницу для взаимодействия с хабом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>Код javascript получает введенные в два поля на странице данные, формирует из них объект со свойствами "name" и "age" и отправляет хабу. Стоит обратить внимание, 
что отправляемые объект соответствует определению класса User, благодаря чему SignalR сможет связать эти данные с объектом User.</p>
<p>Обратите внимание на то, что при отправке для значения age выполняется преобразование <code>parseInt(age)</code>.</p>
<img src="./pics/signalr12.png" alt="Отправка объектов в SignalR в ASP.NET Core" />
<p>Подобным образом можно отправлять массивы и коллекции, в том числе коллекции сложных данных. Например, определим хаб, который принимает список строк:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.SignalR;
...
<p>Полученный список отправляет обратно клиенту.</p>
<p>На клиенте определим отправку и получение набора строк:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>На странице три поля ввода, имя которых равно "items". В коде javascript считываем значения этих полей, добавляем их в массив и отправляем хабу.</p>
<p>В функции Receive, наоборот, получаем пришедшие данные и выводим на страницу.</p>
<img src="./pics/signalr13.png" alt="Отправка массивов в SignalR в ASP.NET Core" />
...
<div class="nav"><p><a href="./30.6.php">Назад</a><a href="./">Содержание</a><a href="./30.8.php">Вперед</a></p></div></div>
<div class="socBlock">
