...
<html >
<head>
<title>ASP.NET Core | Валидация на стороне клиента</title>
<meta charset="utf-8" />
<meta name="description" content="Валидация на стороне клиента в ASP.NET Core, подключение скриптов валидации, применение tag-хелперов, стилизация сообщений об ошибках">
...
<h2>Валидация на стороне клиента</h2><div class="date">Последнее обновление: 08.12.2019</div>
...
<p>Валидация на стороне клиента позволяет уменьшить количество обращений к серверу и произвести все действия по проверке значений непосредственно при вводе данных.</p>
<p>Для определения валидации на стороне клиента необходимо подключить в представлении специальные скрипты валидации:</p>
<pre class="brush:xml;">
&lt;script src="https://ajax.aspnetcdn.com/ajax/jquery.validate/1.17.0/jquery.validate.min.js"&gt;&lt;/script&gt;
&lt;script src="https://ajax.aspnetcdn.com/ajax/jquery.validation.unobtrusive/3.2.10/jquery.validate.unobtrusive.min.js"&gt;&lt;/script&gt;
</pre>
<p>Актуальная версия скриптов в будущем может отличаться. Но по умолчанию эти же скрипты по умолчанию уже есть в проекте в папке <span class="b">wwwroot/lib</span>, 
и мы можем совместить подключение из CDN и подключение из проекта (в случае есть из CDN не удастся подключиться):</p>
<pre class="brush:xml;">
...
<p>Единственное, что надо учитывать, что эти скрипты зависят от основной библиотеки jquery, поэтому перед подключением скриптов валидации должно идти подключение jquery.</p>
<p>Кроме того, в проекте по типу Web Application (Model-View-Controller) по умолчанию в папке <span class="b">Views/Shared</span> добавляется 
файл частичного представления <span class="b">_ValidationScriptsPartial</span>, который опять же содержит подключения выше упомянутых скриптов:</p>
<pre class="brush:xml;">
&lt;script src="~/lib/jquery-validation/dist/jquery.validate.min.js"&gt;&lt;/script&gt;
&lt;script src="~/lib/jquery-validation-unobtrusive/jquery.validate.unobtrusive.min.js"&gt;&lt;/script&gt;
</pre>
<p>Поэтому в принципе мы можем подключать скрипты через данное частичное представление.</p>
<img src="./pics/validation13.png" alt="Client validation in ASP NET Core MVC" />
<p>Для валидации и отображения ошибок фреймворк MVC использует атрибуты валидации в дополнение к метаданным свойств модели. Когда MVC производит рендеринг 
элементов формы с помощью tag-хелперов или html-хелперов, к элементам формы добавляются data-атрибуты HTML 5.</p>
<p>Например, пусть в модели определено свойство Name:</p>
<pre class="brush:c#;">
[Required (ErrorMessage = "Не указано имя")]
public string Name { get; set; }
</pre>
<p>Тогда следующее определение поля ввода с помощью tag-хелперов:</p>
<pre class="brush:xml;">
&lt;label asp-for="Name"&gt;&lt;/label&gt;&lt;br /&gt;
&lt;input type="text" asp-for="Name" /&gt;
&lt;span asp-validation-for="Name"&gt;&lt;/span&gt;
</pre>
<p>сгенерирует следующую разметку:</p>
<pre class="brush:xml;">
...
<p>Когда происходит валидация, то при отображении ошибок соответствующим полям присваиваются определенные классы css. Используя эти классы, мы можем настроить 
отображение сообщений. Например, добавим в представление или в подключаемый файл css следующие стили:</p>
<pre class="brush:css;">
...
<p>Класс <code>validation-summary-errors</code> добавляется для сводки ошибок всей модели над формой. Класс <code>field-validation-error</code> добавляется 
к элементам <code>&lt;span&gt;</code>, которые отображают текст ошибки. И класс <code>input-validation-error</code> добавляется для полей ввода, которые содержат 
некорректные данные. Управляя этими классами, мы можем должным образом стилизовать элементы формы.</p>
<img src="./pics/validation10.png" alt="Стилизация ошибок валидации" />
...
<div class="nav"><p><a href="./19.3.php">Назад</a><a href="./">Содержание</a><a href="./10.5.php">Вперед</a></p></div></div>
<div class="socBlock">
