...
<html >
<head>
<title>ASP.NET Core | Методы Методы Map и MapWhen</title>
<meta charset="utf-8" />
<meta name="description" content="Методы Map и MapWhen в ASP.NET Core, сопоставление запроса с определенным делегатом, создание простейшей системы маршрутизации">
...
<h2>Методы Map и MapWhen</h2><div class="date">Последнее обновление: 30.10.2019</div>
...
<h3>Метод Map</h3>
<p>Метод Map (и методы расширения <code>MapXXX()</code>) применяется для сопоставления пути запроса с определенным делегатом, который будет 
обрабатывать запрос по этому пути. Например:</p>
<pre class="brush:c#;">
...
<p>Теперь обращения к приложению типа <span class="ii">http://localhost:xxxx/about</span> будут обрабатываться с помощью метода <code>About</code>, 
а запросы типа <span class="ii">http://localhost:xxxx/index</span> - методом <code>Index</code>. 
А все остальные запросы будут обрабатываться делегатом из <code>app.Run()</code>.</p>
<img src="./pics/runmiddleware2.png" alt="Обработка запроса методом app.Map в ASP.NET Core" />
<h3>Вложенные методы Map</h3>
<p>Метод Map может иметь вложенные методы Map, которые обрабатывают подмаршруты. Например:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app)
{
    app.Map("/home", home =&gt;
...
<p>Теперь метод About будет обрабатывать запрос не <span class="ii">http://localhost:xxxx/about</span>, а <span class="ii">http://localhost:xxxx/home/about</span></p>
<h3>Метод MapWhen</h3>
<p>Похожим образом работает метод <span class="b">MapWhen()</span>. Он принимает в качестве параметра делегат <code>Func&lt;HttpContext, bool&gt;</code> и 
обрабатывает запрос, если функция, передаваемая в качестве параметра возвращает true.</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app)
{
     app.MapWhen(context =&gt; {
...
<img src="./pics/2.5.png" alt="Обработка запроса методом app.MapWhen в ASP.NET Core" />
<p>В данном случае если в запросе указан параметр id и он имеет значение 5, то запрос обрабатывается функцией <code>HandleId()</code>. К подобным запросам будут 
относиться, например, запрос <span class="ii">http://localhost:55234/?id=5</span> или <span class="ii">http://localhost:55234/product?id=5&amp;name=phone</span>, 
так как обе строки запроса содержат параметр id равный 5. А все остальные запросы также будут обрабатываться делегатом, передаваемым в метод <code>app.Run()</code>.</p>
