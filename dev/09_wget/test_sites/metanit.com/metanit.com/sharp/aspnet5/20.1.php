...
<html >
<head>
<title>ASP.NET Core | Публикация на IIS</title>
<meta charset="utf-8" />
<meta name="description" content="Публикация на IIS веб-приложения ASP.NET Core, параметры публикации и настройка пула приложения">
...
<h2>Публикация на IIS</h2><div class="date">Последнее обновление: 26.03.2018</div>
...
<p>После завершения работы над приложением приложение его можно опубликовать, чтобы оно стало доступно широкому кругу пользователей. 
Как правило, для хостирования приложения будет применяться один из внешних хостингов, список которых можно посмотреть <a href="//metanit.com/sharp/articles/mvc/6.php">здесь</a>. 
В данном же случае рассмотрим основные моменты публикации и развертывания приложения на локальном компьютере.</p>
<p>Традиционно веб-сервер IIS (Internet Information Services) применялся для развертывания веб-приложений. Для хостирования веб-приложений ASP.NET Core 
...
Хостирование приложений ASP.NET Core на IIS происходит с помощью нативного модуля <span class="b">AspNetCoreModule</span>, который сконфигурирован таким 
...
приложение, и перенаправляет все запросы от IIS к этому хостирующему процессу.</p>
<p>При разработке в Visual Studio публиковать приложения очень легко - среда разработки имеет для этого весь необходимый инструментарий. 
Так, возьмем какой-нибудь проект и в Visual Studio нажмем на него правой кнопкой мыши и в появившемся контекстном меню выберем пункт <span class="b">Publish</span>:</p>
<img src="./pics/publish1.png" alt="Публикация приложения ASP.NET Core в Visual Studio" />
<p>И перед нами откроется окно публикации приложения:</p>
<img src="./pics/publish6.png" alt="Публикация в приложения ASP.NET Core MVC на IIS" />
<p>Здесь нам доступно несколько вариантов публикации:</p>
<ul>
<li><p><span class="b">Microsoft Azure App Service</span>: публикация в облаке Azure</p></li>
<li><p><span class="b">IIS, FTP, etc</span>: публикация через FTP</p></li>
<li><p><span class="b">Folder</span>: публикация в виде отдельного пакета в файловой системе текущей рабочей машины</p></li>
<li><p><span class="b">Import Profile</span>: импорт профиля, который содержит настройки публикации</p></li>
<li><p><span class="b">Microsoft Azure Virtual Machines</span>: публикация в облаке Azure, по сравнению с первой опцией обладает большими возможностями по управлению инфраструктурой развертывания</p></li>
</ul>
<p>В данном случае выберем опцию <span class="b">Folder</span> для создания пакета для публикации в файловой системе. И также укажем путь, по которому будет находиться 
пакет. В моем случае это каталог "C:\CoreApp". И в конце нажмем на кнопку Publish.</p>
<p>Далее откроется окно, где будут оображаться выбранные настройки конфигурации, и для продолжения публикации нажмем в нем кнопку Publish:</p>
<img src="./pics/publish7.png" alt="Публикация в файловой системе приложения ASP.NET Core MVC" />
<p>И после окончания публикации по указанному пути (в моем случае это каталог C:\CoreApp) появятся опубликованные файлы.</p>
<h3>Настройка IIS</h3>
<p>Прежде всего нам надо включить функциональность Web Server (IIS) и настроить роли сервера. Для этого перейдем по пути <span class="b">Панель управления -&gt; 
Программы и компоненты -&gt; Включение или отключение компонентов Windows</span>. В списке компонентов найдем <span class="b">Службы IIS (Internet Information Services)</span> и отметим ее:</p>
<img src="./pics/iis1.png" alt="Включение IIS на ОС Windows" />
<p>Здесь также надо отметить подпункт "Службы Интернета" (World Wide Web Services) и все его подпункты, а также подпункт "Консоль управления IIS" 
(IIS Management Console).</p>
<p>Нажмем на ОК, и весь необходимый функционал будет добавлен в операционную систему.</p>
<p>Затем нам необходимо установить специальный пакет <span class="b">.NET Core Windows Server Hosting</span>. Его можно найти, перейдя на страницу <a href="https://www.microsoft.com/net/download/all" rel="nofollow">https://www.microsoft.com/net/download/all</a>. 
Далее на этой странице надо выбрать нужную версию .NET Core Runtime (<span class="b">.NET Core Runtime &gt; .NET Core Runtime x.y.z</span>. Далее 
на странице выбранной версии .NET Core Runtime перейти к подразделу <span class="b">Windows</span> и выбрать <span class="b">Server Hosting Installer</span>. 
После этого загрузится нужный пакет. Этот пакет устанавливает .NET Core Runtime, .NET Core Library и модуль ASP.NET Core Module. 
Данный модуль, как говорилось выше, как раз и создает проксирование между IIS и сервером Kestrel.</p>
<p>После установки этого пакета выполним в командной строке команду <span class="b">iisreset</span> или вручную перезапустим IIS, чтобы сервер применил изменения.</p>
<h3>Конфигурация сервера</h3>
<p>Для конфигурации IIS перейдем к консоли управления веб-сервером. Для этого перейдем по пути 
<span class="b">Панель управления -&gt; Администрирование -&gt; Диспетчер служб IIS</span>:</p>
<img src="./pics/iis2.png" alt="Диспетчер служб IIS в Windows" />
<p>Нажмем правой кнопкой на узел "сайты" и в контекстном меню выберем пункт "Добавить веб-сайт...". После этого нам откроется окно для добавления нового сайта:</p>
<img src="./pics/iis3.png" alt="Добавление приложения на IIS" />
<p>В поле "Физический путь" здесь укажем каталог, в котором опубликовано приложение. А в качестве имени узла определим "localhost". Нажмем на OK, и приложение будет запущено.</p>
<p>Теперь перейдем к пункту <span class="b">Пулы приложений</span>. Выберем пул нашего приложения и справа нажмем на ссылку <span class="b">Основные настройки</span>:</p>
<img src="./pics/iis4.png" alt="Пул приложений в IIS" />
<p>В открывшемся окне для параметра <span class="b">Версия среды CLR .NET</span> установим значение <span class="b">Без управляемого кода</span>:</p>
<img src="./pics/iis5.png" alt="Настройка среды пула приложений в IIS" />
<p>После этого пул будет перезапущен, и мы сможем обращаться к нашему приложению по адресу localhost.</p>
<img src="./pics/iis6.png" alt="Развертывание приложения ASP.NET Core на IIS" />
...
<div class="nav"><p><a href="./2.7.php">Назад</a><a href="./">Содержание</a><a href="./20.2.php">Вперед</a></p></div></div>
<div class="socBlock">
