...
<html >
<head>
<title>ASP.NET Core MVC | Результаты действий</title>
<meta charset="utf-8" />
<meta name="description" content="Результаты действий в ASP.NET Core MVC, обработка запросов и создание ответа">
...
<h2>Результаты действий</h2><div class="date">Последнее обновление: 17.11.2019</div>
...
<p>В данном случае использовался класс Geometry из прошлой темы.</p>
...
<p>Итак, создадим свой класс результата действий. Для этого вначале добавим в проект папку <span class="b">Util</span>, в которой будет храниться классы 
результатов. Затем в эту папку добавим новый класс, который назовем <span class="b">HtmlResult</span>:</p>
<pre class="brush:c#;">
...
<p>Здесь определен метод <code>GetHtml()</code>, который возвращает объект HtmlResult. При обращении к этому объекту будет срабатывать его 
метод <code>ExecuteResultAsync()</code>, в котором будет происходить генерация html-страницы. И чтобы обратиться к этому методу в браузере, нам надо отправить 
запрос типа <i>http://localhost:57086/Home/GetHtml</i>:</p>
<img src="./pics/controllers6.png" alt="Создание результата действий и IActionResult в ASP.NET MVC" />
<p>Однако в большинстве случаев нам не придется создавать свои классы результатов, потому что фреймворк ASP.NET MVC Core итак предоставляет довольно большое количество 
классов результатов для самых различных ситуаций:</p>
<ul>
<li><p><span class="bb">ContentResult</span>: пишет указанный контент напрямую в ответ в виде строки</p></li>
