...
<html >
<head>
<title>ASP.NET Core | Менеджер Libman</title>
<meta charset="utf-8" />
<meta name="description" content="Пакетный менеджер Менеджер Libman в ASP.NET Core, добавление библиотек javascript, управление клиентскими пакетами">
...
<h2>Менеджер Libman</h2><div class="date">Последнее обновление: 16.12.2019</div>
...
<p>Начиная с версии 15.8 в Visual Studio была добавлена такая функциональность как <span class="b">Microsoft Library Manager</span> или 
сокращенно <span class="b">Libman</span>. Этот менеджер позволяет управлять пакетами клиентских библиотек и в 
некотором смысле может рассматриваться как замена безвозвратно ушедшему от нас пакетному менеджеру Bower.</p>
<p>К примеру, возьмем в любой проект ASP.NET Core в Visual Studio (начиная с версии 15.8) и в структуре проекта 
нажмем на узел <span class="b">wwwroot/lib</span> правой кнопкой мыши:</p>
<img src="./pics/libman1.png" alt="Libman in ASP.NET Core" />
<p>И затем в контентном меню выберем пункт <span class="b">Add -&gt; Client-Side Library...</span></p>
<p>После этого нам откроется окно добавления пакетов клиентских библиотек.</p>
<img src="./pics/libman2.png" alt="Library Manager in ASP.NET Core in Visual Studio" />
<p>Данное окно предоставляет ряд опций для добавления библиотек. Первая опция <span class="b">Provider</span> предоставляет список провайдеров 
пакетов. Сейчас доступно три провайдера:</p>
<ul>
<li><p><span class="b">cdnjs</span> - добавление из репозитория CDNJS</p></li>
<li><p><span class="b">filesystem</span> - добавление с локального диска</p></li>
<li><p><span class="b">unpkg</span> - добавление из репозитория unpkg</p></li>
</ul>
<p>Второе поле - поле <span class="b">Library</span> предназначено для ввода названия библиотеки. При вводе нескольких символов появится список подходящих результатов, 
из которого можно выбрать нужную библиотеку.</p>
<img src="./pics/libman3.png" />
<p>Либо можно полностью ввести название пакета в формате [название]@[версия].</p>
<p>Третья опция имеет два альтернативных варианта:</p>
<ul>
<li><p><span class="b">Include all library files</span> указывает, что добавляться будут все файлы. И в поле ниже мы можем увидеть 
список всех добавляемых файлов</p></li>
<li><p><span class="b">Choose specific files</span> позволяет выбрать только нужные нам файлы библиотеки</p></li>
</ul>
<p>Последнее поле - поле <span class="b">Target Location</span> позволяет указать место добавления файлов библиотеки.</p>
<p>После нажатия на кнопку Install выбранная библиотека будет добавлена в проект.</p>
<p>После первого использования Libman в проекте появится новый файл <span class="b">libman.json</span>.</p>
<img src="./pics/libman4.png" alt="libman.json in Visual Studio" />
<p>Если мы откроем файл, то там будет содержимое типа следующего:</p>
<pre class="brush:js;">
{
  "version": "1.0",
...
<p>Узел libraries содержит набор добавленных библиотек. Если мы хотим удалить библиотеку, достаточно удалить узел нужной библиотеки 
из libraries и сохранить файл. И библиотека будет удалена из проекта.</p>
<p>Аналогично мы можем изменить даные библиотеки, например, изменить версию или расположения, и затем после сохранения файла 
все изменения будут применены к проекту.</p>
<p>Стоит отметить, что Libman не управляет теми пакетами, которые добавляются в проект по умолчанию (jquery и bootstrap).</p>
...
<div class="nav"><p><a href="./13.6.php">Назад</a><a href="./">Содержание</a><a href="./13.5.php">Вперед</a></p></div></div>
<div class="socBlock">
