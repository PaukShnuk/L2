...
<html >
<head>
<title>ASP.NET Core | Модели</title>
<meta charset="utf-8" />
<meta name="description" content="Модели в ASP.NET MVC Core, определение и использование моделей, концепция View Model">
...
<h1>Модели</h1><h2>Модели в ASP.NET MVC Core</h2><div class="date">Последнее обновление: 29.11.2019</div>
...
<p>Одним из ключевых компонентов паттерна MVC являются <span class="b">модели</span>. Ключевая задача моделей - описание структуры и логики используемых данных.</p>
<p>Как правило, все используемые сущности в приложении выделяются в отдельные модели, которые и описывают структуру каждой сущности. В зависимости от задач и 
предметной области мы можем выделить различное количество моделей в приложении.</p>
<p>Все модели оформляются как обычные POCO-классы (plain-old CRL objects), то есть обычные классы на языке C#. Например, если мы работаем с приложением 
интернет-магазина мобильных телефонов, то мы могли бы определить в проекте следующую модель, представляющую телефон:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Модель Phone определяет ряд свойств: уникальный идентификатор Id, название, компанию производителя и цену. Это классическая <span class="b">анемичная модель</span>. Анемичная модель 
не имеет поведения и хранит только состояние в виде свойств.</p>
<p>Однако модель необязательно должна состоять только из свойств. Кроме того, она может иметь конструктор, какие-нибудь методы, 
...
"толстыми" моделями (Rich Domain Model / Fat Model / Thick Model). Например, мы можем уйти от анемичной модели, модифицировав ее следующим образом:</p>
<pre class="brush:c#;">
public class Phone
{
...
<p>Но какой бы способ описания сущности не был выбран, главное помнить, что его предназначение состоит прежде всего описывать 
данные. И модель должна описывать только одну сущность, следуя принципу единой ответственности.</p>
<p>В приложении ASP.NET MVC Core модели можно разделить по степени применения на несколько групп:</p>
<ul>
<li><p>Модели, объекты которых хранятся в специальных хранилищах данных (например, в базах данных, файлах xml и т.д.)</p></li>
<li><p>Модели, которые используются для передачи данных представление или наоборот, для получения данных из представления. Такие модели еще называтся <span class="b">моделями представления</span></p></li>
<li><p>Вспомогательные модели для промежуточных вычислений</p></li>
</ul>
<p>Как правило, для хранения моделей создается в проекте отдельная папка <span class="b">Models</span>. Модели представления нередко помещаются в отдельную папку, которая 
нередко называется <span class="b">ViewModels</span>. В реальности, это могут быть каталоги с любыми называниями, можно помещать модели 
хоть в корень проекта, но более распространенным стилем являются названия Models и ViewModels.</p>
<p>Например, создадим новый проект ASP.NET Core по типу Web Application (Model-View-Controller). По умолчанию он уже содержит папку Models, в которой по умолчанию 
определен класс ErrorViewModel - модель, используемая для вывода информации об ошибках в приложении.</p>
<p>Вначале добавим в эту папку новый класс Company:</p>
<pre class="brush:c#;">
...
<p>И также добавим в папку Models класс Phone</p>
<pre class="brush:c#;">
public class Phone
{
    public int Id { get; set; }
...
<img src="./pics/models16.png" alt="Модели в ASP.NET Core MVC" />
<p>Эти модели будут описывать данные, которые мы будем использовать. Эти данные могут храниться в базе данных, но для простоты мы определим их в контроллере. 
Изменим контроллер HomeController следующим образом:</p>
<pre class="brush:c#;">
...
<p>Список объектом модели передается в представление с помощью метода <code>View()</code>.</p>
<p>И в конце изменим представление <i>Index.cshtml</i>, которое будет выводить все объекты:</p>
<pre class="brush:c#;">
...
<p>И при обращении к приложению веб-станица выведет список моделей:</p>
<img src="./pics/models3.png" alt="Модели в ASP.NET MVC Core" />
...
<div class="nav"><p><a href="./11.9.php">Назад</a><a href="./">Содержание</a><a href="./8.2.php">Вперед</a></p></div></div>
<div class="socBlock">
