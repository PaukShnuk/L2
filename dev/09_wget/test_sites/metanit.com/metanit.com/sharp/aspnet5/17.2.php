...
<html >
<head>
<title>ASP.NET Core | Фильтры исключений</title>
<meta charset="utf-8" />
<meta name="description" content="Фильтры исключений в ASP.NET Core, интерфейсы IExceptionFilter и IAsyncExceptionFilter">
...
<h2>Фильтры исключений</h2><div class="date">Последнее обновление: 11.12.2019</div>
...
<p>Фильтры исключений обрабатывают необработанные исключения, в том числе те, которые возникли при создании контроллера и привязки модели. 
...
В частности,фильтры исключений обрабатывают необработанные исключения, которые возникают при создании объекта контроллера или страницы 
RazorPage, при привязке модели, при выполнении фильтров действий или методов контроллера или страницы RazorPage. Но фильтры исключений  не 
обрабатывают исключения, которые возникают в фильтрах ресурсов, результатов или при выполнении результата.</p>
<p>В этом плане фильтры исключений менее гибки, чем, скажем, компоненты middleware, которые также предназначены для обработки исключений.</p>
<p>Фильтры исключений (Exception Filters) должны реализовать либо интерфейс <span class="b">IExceptionFilter</span>, либо интерфейс <span class="b">IAsyncExceptionFilter</span>. 
Также мы можем создать класс фильтра исключений, унаследовав его от абстрактного класса <span class="b">ExceptionFilterAttribute</span>.</p>
<p>В отличие от других фильтров фильтры исключений имеют только один метод - <code>OnException()</code> (для реализации IExceptionFilter) и 
<code>OnExceptionAsync()</code> (для реализации IAsyncExceptionFilter). В метод <code>OnException()</code> в качестве параметра передается объект <span class="b">ExceptionContext</span>, который содержит всю информацию о возникшем 
исключении.</p>
<p>При установке свойства <code>context.ExceptionHandled</code> равным <code>true</code> мы можем получить эффект, как будто мы обработали исключение. 
И в этому случае обработка запроса продолжится, как будто никакого исключения и не было, а браузеру будет отправлен статусный код 200 OK.</p>
<p>Например, определим следующий класс фильтра исключений:</p>
<pre class="brush:c#;">
...
<p>С помощью параметра ExceptionContext получаем разные сведения об исключении, а также имя действия, в котором возникло исключение, и создаем из полученной информации 
объект ContentResult. Чтобы исключение считалось обработанным, устанавливаем <code>context.ExceptionHandled = true</code></p>
<p>Применим этот фильтр к методу, в котором симулируем ошибочную ситуацию:</p>
<pre class="brush:c#;">
...
<p>При обращении к этому методу у нас возникнет исключение - деление числа на 0. Исключение будет перехвачено фильтром исключений, и в итоге мы увидим в своем 
браузере информацию об исключении:</p>
<img src="./pics/17.7.png" alt="Фильтры исключений в ASP.NET Core" />
...
<div class="nav"><p><a href="./18.4.php">Назад</a><a href="./">Содержание</a><a href="./18.7.php">Вперед</a></p></div></div>
<div class="socBlock">
