...
<html >
<head>
<title>ASP.NET Core | Создание юнит-тестов</title>
<meta charset="utf-8" />
<meta name="description" content="Создание юнит-тестов в ASP.NET Core, использование фреймворка xUnit, тестирование контроллера MVC">
...
<h2>Создание юнит-тестов</h2><div class="date">Последнее обновление: 21.12.2019</div>
...
<p>Возьмем тот же проект из прошлый темы (либо создадим новый) и изменим в главном проекте код контроллера HomeController:</p>
<pre class="brush:c#;">
...
<p>Контроллер имеет только один метод, который устанавливает значение <code>ViewData["Message"]</code> и генерирует объект ViewResult, который использует представление Index. Теперь протестируем этот метод.</p>
<p>Для этого в проект тестов добавим новый класс, который назовем HomeControllerTests:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc;
...
<p>Этот простейший стандартный тест, который призван дать начальное понимание работы тестов.</p>
<p>Метод <code>IndexViewResultNotNull</code> тестирует результат метода - возвращаемый объект ViewResult не должен иметь значение null. 
Метод <code>IndexViewNameEqualIndex</code> проверяет название вызываемого представления с помощью вызова Assert.Equal. 
А метод <code>IndexViewDataMessage</code> проверяет значение по ключу "Message" в словаре ViewData.</p>
<img src="./pics/unittest7.png" alt="Проект юнит-тестов в ASP.NET Core" />
<p>Тесты в xUnit определяются в виде методов, к которым применяются атрибуты Fact и Theory. Атрибут <span class="b">Fact</span> указывает, что методы представляет тест.</p>
<p>Каждый метод содержит три логических части - Arrange, Act и Assert. Они помечены соответствующими комментариями. Вкратце рассмотрим, что они делают.</p>
<p>Секция Arrange выполняет начальную инициализацию контекста тестов, а именно создает объект контроллера: <code>HomeController controller = new HomeController()</code>.</p>
<p>Далее в секции Act выполняет само действие, которое надо протестировать, а именно генерация представления: <code>ViewResult result = controller.Index() as ViewResult</code>. 
Так как метод Index контроллера возвращает объект ActionResult, то его еще надо привести к объекту ViewResult.</p>
<p>Чтобы проверить результат текста в секции Assert вызывается один из методов класса <code>Assert</code>.</p>
<p>Модель тестов <span class="b">Arrange-Act-Assert</span> представляет целую парадигму тестирования, которая используется многими 
фреймворками юнит-тестов:</p>
<ul>
<li><p><span class="b">Arrange</span>: устанавливает начальные условия для выполнения теста</p></li>
<li><p><span class="b">Act</span>:  выполняет тест (обычно представляет одну строку кода)</p></li>
<li><p><span class="b">Assert</span>: верифицирует результат теста</p></li>
</ul>
<p>Секции Arrange и Act представляют обычный код на языке C#. А секция Assert использует одноименный класс Assert, который определен в библиотеке xUnit.net.</p>
<p>Для проверки результата в классе Assert определено ряд методов, среди которых можно выделить следующие:</p>
<ul>
<li><p><span class="b">All(collection, action)</span>: метод подтверждает, что все элементы коллекции collection соответствуют действию action</p></li>
<li><p><span class="b">Contains(expectedSubString, actualString)</span>: метод подтверждает, что строка actualString содержит expectedSubString</p></li>
<li><p><span class="b">DoesNotContain(expectedSubString, actualString)</span>: метод подтверждает, что строка actualString не содержит строку expectedSubString</p></li>
<li><p><span class="b">DoesNotMatch(expectedRegexPattern, actualString)</span>: метод подтверждает, что строка actualString не соответствует регулярному выражению expectedRegexPattern</p></li>
<li><p><span class="b">Matches(expectedRegexPattern, actualString)</span>: метод подтверждает, что строка actualString соответствует регулярному выражению expectedRegexPattern</p></li>
<li><p><span class="b">Equal(expected, result)</span>: метод сравнивает результат теста в виде значения <code>result</code> и ожидаемое значение 
<code>expected</code> и подтверждает их равенство</p></li>
<li><p><span class="b">NotEqual(expected, result)</span>: метод сравнивает результат теста в виде значения <code>result</code> и ожидаемое значение 
<code>expected</code> и подтверждает их неравенство</p></li>
<li><p><span class="b">Empty(collection)</span>: метод подтверждает, что коллекция collection пустая</p></li>
<li><p><span class="b">NotEmpty(collection)</span>: метод подтверждает, что коллекция collection не пустая</p></li>
<li><p><span class="b">True(result)</span>: метод подтверждает, что результат теста равен <code>true</code></p></li>
<li><p><span class="b">False(result)</span>: метод подтверждает, что результат теста равен <code>false</code></p></li>
<li><p><span class="b">IsType(expected, result)</span>: метод подтверждает, что результат теста имеет тип <code>expected</code></p></li>
<li><p><span class="b">IsNotType(expected, result)</span>: метод подтверждает, что результат теста не представляет тип <code>expected</code></p></li>
<li><p><span class="b">IsNull(result)</span>: метод подтверждает, что результат теста имеет значение <code>null</code></p></li>
<li><p><span class="b">IsNotNull(result)</span>: метод подтверждает, что результат теста не равен <code>null</code></p></li>
<li><p><span class="b">InRange(result, low, high)</span>: метод подтверждает, что результат теста находится в диапазоне между low и high</p></li>
<li><p><span class="b">NotInRange(result, low, high)</span>: метод подтверждает, что результат теста не принадлежит диапазону от low до high</p></li>
<li><p><span class="b">Same(expected, actual)</span>: метод подтверждает, что ссылки <code>expected</code> и <code>actual</code> указывают на один и тот же объект в памяти</p></li>
<li><p><span class="b">NotSame(expected, actual)</span>: метод подтверждает, что ссылки <code>expected</code> и <code>actual</code> указывают на разные объекты в памяти</p></li>
<li><p><span class="b">Throws(exception, expression)</span>: метод подтверждает, что выражение <code>expression</code> генерирует исключение <code>exception</code></p></li>
</ul>
<p>Теперь запустим тест на выполнение через меню <span class="b">Test-&gt;Run All Tests</span>:</p>
<img src="./pics/unittest4.png" alt="Запуск юнит-тестов в ASP.NET Core" />
<p>После этого откроется окно <span class="b">Test Explorer</span> или обозреватель тестов. Если все нормально, то обозреватель тестов сигнализирует 
нам зеленым цветом, что все тесты успешно пройдены. Если же нет, то не пройденные тесты будут отмечены красным цветом:</p>
<img src="./pics/unittest5.png" alt="xunit и тесты в ASP.NET Core" />
<p>Из скриншота вида, что один тест - IndexViewDataMessage не пройден. Потому что мы ожидаем, что <code>ViewData["Message"]</code> будет иметь значение 
"Hello world!". Однако в методе Index контроллера HomeController передается совсем другое значение: <code>ViewData["Message"] = "Hello!"</code>. То есть 
имеющаяся логика не соответствует нашим ожиданиям, и в нем имеется ошибка. Теперь изменим код метода Index:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
    public IActionResult Index()
...
<p>И заново запустим тесты:</p>
<img src="./pics/unittest6.png" alt="Unit test in ASP.NET Core MVC" />
<p>Теперь все тесты пройдены.</p>
<p>В принципе необязательно было бы разделять все ключевые действия по разным методам. Можно было бы объединить все тесты в один:</p>
<pre class="brush:c#;">
[Fact]
...
<p>В этом случае тест был пройден, если подтверждены все используемые выражения Assert.</p>
...
<div class="nav"><p><a href="./22.2.php">Назад</a><a href="./">Содержание</a><a href="./22.4.php">Вперед</a></p></div></div>
<div class="socBlock">
...
$(function() { 
  ...
  $("#toggle-theme").on("click", function() {
	...
	$("li:contains('Глава 26.')").removeClass('expandable').addClass('collapsable').children('ul').css({"display":"block"});
	$("li:contains('Глава 26.')").children('div').removeClass('expandable-hitarea').addClass('collapsable-hitarea');
