...
<html >
<head>
<title>ASP.NET Core | Tag-хелперы</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в TagHelpers в ASP.NET Core, роль файла _ViewImports.cshtml, добавление и удаление tag-хелперов">
...
<h1>Tag-хелперы</h1><h2>Введение в tag-хелперы</h2><div class="date">Последнее обновление: 02.12.2019</div>
...
<p>Tag-хелперы представляют собой функциональность, предназначенную для генерации HTML-разметки. Tag-хелперы используются в представлениях и 
выглядят как обычные html-элементы или атрибуты, однако при работе приложения они обрабатываются движком Razor на стороне сервера и в 
...
<p>Использовать tag-хелперы довольно просто. Например, определим в представлении следующий код:</p>
...
<img src="./pics/taghelper1.png" alt="Tag хелперы в ASP.NET MVC Core" />
...
<p>Проект ASP.NET MVC Core уже по умолчанию подключает функциональность tag-хелперов в представления с помощью 
установки в файле <span class="b">_ViewImports.cshtml</span> следующей директивы:</p>
<pre class="brush:c#;">@addTagHelper *, Microsoft.AspNetCore.Mvc.TagHelpers</pre>
<p>Первый параметр директивы указывает на tag-хелперы, которые будут доступны во всех представлениях из папки Views, а второй параметр определяет 
библиотеку хелперов. В данном случае директива использует синтаксис подстановок - знак звездочки ("*") означает, что все хелперы из 
библиотеки <span class="b">Microsoft.AspNetCore.Mvc.TagHelpers</span>.</p>
<p>Если вдруг у нас не окажется подобной директивы, то ее добавление в представления позволяет использовать все встроенные tag-хелперы.</p>
<p>По умолчанию эта директива определяется в файле <span class="ii">_ViewImports.cshtml</span>, который находится в папке <span class="ii">Views</span>. Однако мы можем конкретизировать 
применение хелперов к определенной группе представлений. Например, если у нас есть каталог <span class="ii">Views/Home</span> - специально для представлений для контроллера HomeController, 
и мы хотим применить только к ним определенные хелперы. В этом случае мы можем добавить файл <i>_ViewImports.cshtml</i> непосредственно в этот каталог. 
...
Эта директива также определяется в файле <span class="ii">_ViewImports.cshtml</span>.</p>
