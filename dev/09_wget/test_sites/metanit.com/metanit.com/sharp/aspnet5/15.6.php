...
<html >
<head>
<title>ASP.NET Core | Авторизация на основе Claims</title>
<meta charset="utf-8" />
<meta name="description" content="Авторизация на основе Claims (Claims-based authorization) в ASP.NET Core">
...
<h2>Авторизация на основе Claims</h2><div class="date">Последнее обновление: 13.12.2019</div>
...
<p>Роли позволяют разграничить доступ, однако для создания авторизации функциональности ролей бывает недостаточно. Например, что если мы хотим разграничить доступ на основе 
...
<p>Все применяемые политики добавляются в методе <code>ConfigureServices()</code> класса Startup с помощью метода <code>services.AddAuthorization()</code>. 
...
<p>Фактически данные методы задают ограничения, которым должен соответствовать пользователь, обращающийся к приложению. После установки ограничений политики в ConfigureServices() мы 
можем их применять для разграничения доступа:</p>
<pre class="brush:c#;">
...
должны соответствовать пользователи. И если пользователи соответствуют тем ограничениям, которые были установлены для политики в методе <code>ConfigureServices()</code>, 
то для них будет разрешен доступ к методу Index.</p>
