...
<html >
<head>
<title>ASP.NET Core | Создание и вывод моделей в Entity Framework</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление и получение данных в ASP.NET Core из базы данных MS SQL Server через Entity Framework">
...
<h2>Основные операции с моделями. Создание и вывод</h2><div class="date">Последнее обновление: 09.12.2019</div>
...
<p>Вначале изменим имеющийся по умолчанию контроллер HomeController:</p>
<pre class="brush:c#;">
using System.Linq;
...
можем получить переданный контекст данных.</p>
<p>Теперь добавим в контроллер три метода, которые будут добавлять новый объект в базу данных и выводить из нее все объекты:</p>
<pre class="brush:c#;">
...
<p>С помощью метода <code>db.Users.ToListAsnc()</code> мы будем получать объекты из бд, создавать из них список и передавать в представление.</p>
<p>А при помощи метода <code>db.Users.Add()</code> для данных из объекта user формируется sql-выражение <code>INSERT</code>, а метод 
<code>db.SaveChangesAsync()</code> выполняет это выражение, тем самым добавляя данные в базу данных.</p>
<p>А в папку <span class="ii">Views/Home</span> добавим новое представление <span class="b">Create.cshtml</span>:</p>
<pre class="brush:xml;">
...
<p>И также изменим (или добавим при отсутствии) представление <span class="b">Index.cshtml</span>, которое будет отвечать за вывод объектов:</p>
<pre class="brush:xml;">
...
<img src="./pics/createmodel.png" alt="Создание модели в Entity Framework ASP.NET Core" />
<p>Введем в форму какие-либо данные и нажмем на копку:</p>
<img src="./pics/indexmodel.png" alt="Вывод моделей в Entity Framework в ASP.NET Core" />
...
<img src="./pics/serverexplorer.png" alt="Просмотр базы данных в SQL Server Object Explorer" />
