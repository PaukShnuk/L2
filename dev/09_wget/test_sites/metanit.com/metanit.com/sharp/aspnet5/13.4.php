...
<html >
<head>
<title>ASP.NET Core | Препроцессоры Less и Sass</title>
<meta charset="utf-8" />
<meta name="description" content="Применение препроцесоров Less и Sass для работы со стилями CSS в ASP.NET Core">
...
<h2>Препроцессоры Less и Sass</h2><div class="date">Последнее обновление: 17.12.2019</div>
...
<p>Для упрощения работы с стилями css особенно в крупных проектах нередко используются препроцессоры Less и Sass, которые позволяют создавать более 
комплексные таблицы стилей. Less и Saas позволяют уменьшить дублирование кода css, более оптимально организовать стили. И 
Visual Studio предоставляет встроенную поддержку для Less и Sass, а также ряд связанных с этими препроцессорами расширений, которые могут помочь при разработке. Рассмотрим, как
 мы можем применять эти препроцессоры в ASP.NET Core.</p>
<h3>Less</h3>
<p>Для работы с <span class="b">Less</span> возьмем стандартный проект ASP.NET Core по типу Web Application (Model-View-Controller) 
и вначале добавим в <span class="b">корень проекта</span> новую папку, которая будет хранить файлы Less. Пусть эта папка называется <span class="b">Less</span>. Затем в эту папку добавим новый элемент по типу <span class="b">LESS Style Sheet</span> и назовем 
его <span class="b">styles.less</span>:</p>
<img src="./pics/less1.png" alt="LESS in Visual Studio" />
<p>Определим в файле <span class="ii">styles.less</span> следующее содержимое:</p>
<pre class="brush:css;">
@base-color:#f7f7fa;
...
<p><code>@base-color</code> и остальные элементы, которые начинаются с символа альфы @, являются переменными. Каждая переменная несет определенное значение. 
После определения переменных идут стандартные определения стилей css, правда, вместо конкретных значений они ссылаются на переменные.</p>
<p>Файл с кодом less сам по себе ничего не дает. В итоге нам надо его превратить в файл css. Для этого можно использовать менеджеры автоматизации задач, 
такие как Gulp или Grunt. В данном случае мы будем использовать Gulp.</p>
<p>Для этого вначале добавим в проект новый элемент по типу <span class="b">npm Configuration File</span>, который назовем package.json</p>
<img src="./pics/less3.png" alt="Конфигурация LESS и Gulp в ASP.NET Core" />
<p>Далее добавим в этот файл функционал Gulp для работы с Less:</p>
<pre class="brush:js;">
{
  "version": "1.0.0",
  "name": "asp.net",
...
<p>После сохранения файла в проект будет добавлен соответствующий пакет.</p>
<p>Затем нам надо определить конфигурацию Gulp. Для этого добавим в <span class="b">корень проекта</span> новый javascript-файл, 
который назовем <span class="b">gulpfile.js</span> и определим в нем следующий код:</p>
<pre class="brush:js;">
/// &lt;binding Clean='clean' /&gt;
"use strict";
...
<p>Итоговый проект:</p>
<img src="./pics/less5.png" alt="Less в ASP.NET Core MVC" />
<p>Затем через окно Task Runner Explorer запустим задачу Gulp:</p>
<img src="./pics/less2.png" alt="LESS в ASP.NET MVC" />
<p>И после этого в папке wwwroot/css появится следующий файл <i>styles.css</i>:</p>
<pre class="brush:css;">
body {
  font-family: Verdana;
...
<h3>Sass</h3>
<p>Использование Sass аналогично Less за тем исключением, что немного отличается синтаксис. Вначале добавим в проект новую папку, которую назовем <span class="b">Saas</span> 
и которая будет хранить файлы Sass. Затем в эту папку добавим новый элемент <span class="b">SCSS Style Sheet (SASS)</span>, который назовем <span class="b">styles2.scss</span>:</p>
<img src="./pics/sass1.png" alt="Sass in Visual Studio" />
<p>В этом файле определим следующий код:</p>
<pre class="brush:css;">
$base-color:#f7f7fa;
$article-font-size:13px;
$font:Verdana;
...
<p>Как можно заметить, данный код похож на код в файле styles.less, только объявление переменных начинается не со знака @, а со знака $.</p>
<p>Как и в случае с Less прежде всего нам надо добавить в файл <span class="ii">package.json</span> информацию о модуле gulp, который позволит работать с Sass:</p>
<pre class="brush:js;">
{
  "version": "1.0.0",
  "name": "asp.net",
...
<p>И также изменим файл <span class="ii">gulpfile.js</span>:</p>
<pre class="brush:js;">
/// &lt;binding Clean='clean' /&gt;
"use strict";
...
<p>Также перейдем к окну Task Runner Explorer и запустим определенную выше задачу:</p>
<img src="./pics/sass2.png" alt="SASS в ASP.NET MVC" />
<p>И после выполнения задачи в папке <span class="ii">wwwroot/css</span> появится новый файл <span class="ii">styles2.css</span>, который по своему содержимому не будет отличаться от ранее созданного через Less файла styles.css.</p>
