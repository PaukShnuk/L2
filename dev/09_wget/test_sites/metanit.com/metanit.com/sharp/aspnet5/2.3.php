...
<html >
<head>
<title>ASP.NET Core | Методы Use, Run и делегат RequestDelegate</title>
<meta charset="utf-8" />
<meta name="description" content="Конвейер обработки запроса в ASP.NET Core, определение компонентов middleware, методы Use, Run интерфейса IApplicationBuilder, делегат RequestDelegate">
...
<h2>Методы Use, Run и делегат RequestDelegate</h2><div class="date">Последнее обновление: 30.10.2019</div>
...
<p>Для конфигурации конвейера обработки запроса применяются методы <span class="b">Run</span>, <span class="b">Map</span> и <span class="b">Use</span>. Рассмотрим вначале метод 
Run и для этого возьмем проект ASP.NET Core по типу Empty.</p>
<p>Изменим код класса Startup следующим образом:</p>
<pre class="brush:c#;">
...
<p>Метод <code>Run</code> представляет собой простейший способ для добавления компонентов middleware в конвейер. Однако компоненты, определенные через 
метод Run, не вызывают никакие другие компоненты и дальше обработку запроса не передают.</p>
<p>В качестве параметра метод Run принимает делегат RequestDelegate. Этот делегат имеет следующее определение:</p>
<pre class="brush:c#;">public delegate Task RequestDelegate(HttpContext context);</pre>
<p>Он принимает в качестве параметра контекст запроса HttpContext и возвращает объект Task. Поэтому в методе Run 
делегат в качестве параметра context принимает контекст запроса - объект HttpContext.</p>
<p>Данный метод определяет один единственный делегат запроса, который обрабатывает все запросы к приложению. Суть этого делегата заключается 
в отправке в ответ на запросы сообщения "Hello World!". Причем так как данный метод не передает обработку запроса далее по конвейеру, 
то его следует помещать в самом конце. До него же могут быть помещены другие методы.</p>
<p>В итоге при запуске проекта в браузере мы увидим приветствие:</p>
<img src="./pics/1.4.png" />
<h3>Метод Use</h3>
<p>Метод Use также добавляет компоненты middleware, которые также обрабатывают запрос, но в нем может быть вызван следующий в конвейере запроса компонент middleware. Например, 
изменим метод <code>Configure()</code> следующим образом:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app)
{
    int x = 5;
...
<p>В данном случае мы используем перегрузку метода Use, которая в качестве параметров принимает контекст запроса - объект HttpContext и 
делегат <code>Func&lt;Task&gt;</code>, который представляет собой ссылку на следующий в конвейере компонент middleware.</p>
<p>Метод app.Use реализует простейшую задачу - умножение двух чисел и затем передает обработку запроса следующим компонентам middleware в конвейере.</p>
<p>То есть при вызове <code>await next.Invoke()</code> обработка запроса перейдет к тому компоненту, который установлен в методе <code>app.Run()</code>.</p>
<p>В итоге в веб-браузере мы увидим следующее сообщение:</p>
<img src="./pics/runmiddleware1.png" alt="Обработка запроса методом app.Use в ASP.NET Core" />
<p>Если бы мы не использовали вызов <code>await next.Invoke()</code> или закомментировали бы его, то обращения к следующему компоненту в конвейере не произошло бы.</p>
<p>Однако в большинстве случаев мы можем использовать не просто методы Use, а методы расширений <code>app.UseXXX</code>, например, 
<code>UseStaticFiles()</code> или <code>UseMvc()</code>.</p>
<p>При использовании метода Use и передаче выполнения следующему делегату следует учитывать, что не рекомендуется вызывать метод 
<code>next.Invoke</code> после метода <code>Response.WriteAsync()</code>. Компонент middleware должен либо генерировать ответ с помощью Response.WriteAsync, 
либо вызывать следующий делегат посредством <code>next.Invoke</code>, но не выполнять оба этих действия одновременно. Так как согласно документации 
...
могут привести к нарушению тела ответа, например, футер страницы HTML запишется в CSS-файл.<p>
<p>То есть к примеру следующая обработка запроса не рекомендуется:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app)
{
    app.Use(async (context, next) =&gt;
...
<h3>Выполнение app.Use</h3>
<p>Если компоненты middleware в app.Use использует вызов <code>next.Invoke()</code> для передачи обработки дальше по конвейеру, то выполнение такого компонента 
фактически делится на две части: до <code>next.Invoke()</code> и после <code>next.Invoke()</code>. Например, определим в методе Configure следующий код:</p>
<pre class="brush:c#;">
public void Configure(IApplicationBuilder app)
{
	int x = 2;
...
<p>Здесь  определена переменная x, которая равна 2. Последующие вызовы компонентов middleware увеличивают ее значение в два раза. 
Каким образом будет происходить обработка запроса:</p>
<ol>
<li><p>Вызов компонента app.Use</p></li>
<li><p>Увеличение переменной x в два раза: <code>x = x * 2;</code>. Теперь х равно 4.</p></li>
<li><p>Вызов <code>await next.Invoke()</code>. Управление переходит следующему компоненту в конвейере - к app.Run.</p></li>
<li><p>Увеличение переменной x в два раза: <code>x = x * 2;</code>. Теперь х равно 8.</p></li>
<li><p>Метод app.Run закончил свою работу, и управление обработкой возвращается к app.Use. Начинает выполняться та часть кода, которая 
идет после <code>await next.Invoke()</code>.</p></li>
<li><p>Увеличение переменной x в два раза: <code>x = x * 2;</code>. Теперь х равно 16.</p></li>
<li><p>Отправка ответа клиенту с помощью вызова <code>await context.Response.WriteAsync($"Result: {x}")</code></p></li>
</ol>
...
<div class="nav"><p><a href="./2.2.php">Назад</a><a href="./">Содержание</a><a href="./2.22.php">Вперед</a></p></div></div>
<div class="socBlock">
