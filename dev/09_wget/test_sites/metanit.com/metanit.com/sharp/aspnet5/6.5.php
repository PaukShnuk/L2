...
<html >
<head>
<title>ASP.NET Core | Singleton-объекты и scoped-сервисы</title>
<meta charset="utf-8" />
<meta name="description" content="Singleton-объекты и scoped-сервисы в ASP.NET Core">
...
<h2>Singleton-объекты и scoped-сервисы</h2><div class="date">Последнее обновление: 07.11.2019</div>
...
<p>Все объекты, которые используются в ASP.NET Core, имеет три варианта жизненного цикла. Singleton-объекты создаются один раз при запуске приложения, 
...
middleware или сервисы, которые регистрируются с помощью метода <code>AddSingleton()</code>.</p>
<p>Transient-объекты создаются каждый раз, когда нам требуется экземпляр определенного класса. А scoped-объекты создаются по одному на каждый запрос.</p>
<p>Одни объекты или сервисы с помощью встроенного механизма dependency injection можно передать в другие объекты. 
...
мы не можем передавать scoped-сервисы в конструктор singleton-объектов.</p>
<p>Например, пусть будут опеделены следующие классы:</p>
<pre class="brush:c#;">
...
<p>TimeService получает через конструктор сервис ITimer и использует его для получения текущего времени.</p>
<p>Также пусть будет определен компонент middleware TimerMiddleware:</p>
<pre class="brush:c#;">
...
<p>Компонент TimerMiddleware получает сервис TimeService и отправляет в ответ клиенту информацию о текущем времени.</p>
<p>TimerMiddleware является singleton-объектом. И теперь в классе Startup зарегистрируем сервис TimeService как scoped-объект:</p>
<pre class="brush:c#;">
...
<p>Если мы запустим приложение, то мы столкнемся с ошибкой типа "InvalidOperationException: Cannot resolve scoped service 'DIApp.TimeService' from root provider.":</p>
<img src="./pics/dierror1.png" alt="Cannot resolve scoped service from root provider в ASP.NET Core 2" />
<p>То есть на момент создания объекта TimerMiddleware scoped-сервис TimeService еще не установлен, соответственно он использоваться не может. А без создания объекта TimeService 
нельзя создать объект TimerMiddleware.</p>
<p>Аналогичная ситуация может возникнуть, если TimeService добавляется как Transient, а сервис ITimer определен как Scoped:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
...
<p>В этом случае для создания объекта TimeService надо получить сервис ITimer, но на момент вызова конструктора TimerMiddleware сервис ITimer еще неопределен:</p>
<img src="./pics/dierror2.png" alt="Cannot resolve from root provider because it requires scoped service" />
<p>Для выхода из этой ситуации ни TimeService, ни ITimer не должны иметь жизненный цикл Scoped. То есть это может быть Transient или Singleton.</p>
<p>Рассмотрим еще одну ситуацию, с которой можно столкнуться в любой части приложения, а не только в конструкторе middleware, когда 
сервис TimeService представляет singleton, а ITimer - scoped-объект:</p>
<pre class="brush:c#;">
public void ConfigureServices(IServiceCollection services)
{
	services.AddScoped&lt;ITimer, Timer&gt;();
...
<p>И, допустим, эти сервисы используются в TimerMiddleware непосредственно при обработке запроса в методе Invoke/InvokeAsync:</p>
<pre class="brush:c#;">
public class TimerMiddleware
{
	private readonly RequestDelegate _next;
...
<p>При запуске приложения мы опять же столкнемся с ошибкой, только немного другой "Cannot consume scoped service 'DIApp.ITimer' from singleton 'DIApp.TimeService'"</p>
<img src="./pics/dierror3.png" alt="Cannot consume scoped service from singleton in ASP.NET Core" />
<p>Но суть будет та же самая - мы не можем по умолчанию передавать в конструктор singleton-объекта scoped-сервис.</p>
...
<div class="nav"><p><a href="./2.20.php">Назад</a><a href="./">Содержание</a><a href="./2.6.php">Вперед</a></p></div></div>
<div class="socBlock">
