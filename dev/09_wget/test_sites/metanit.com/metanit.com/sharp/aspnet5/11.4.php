...
<html >
<head>
<title>ASP.NET Core | Работа с маршрутами</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с маршрутами в ASP.NET Core, добавление новых маршрутов, обработка их параметров">
...
<h2>Работа с маршрутами</h2><div class="date">Последнее обновление: 15.11.2019</div>
...
<p>Рассмотрим, как происходит работа с маршрутами. При удачном сопоставлении адреса URL определенному маршруту у объекта <span class="b">RouteContext</span> устанавливаются свойства 
<span class="b">Handler</span> и <span class="b">RouteData</span>. Как мы посмотрели ранее, через свойство <code>Handler</code> устанавливается и потом 
вызывается делегат, который обрабатывает запрос по маршруту.</p>
<p>А свойство <code>RouteData</code> предназначено для хранения информации о маршруте и его значениях. Оно представляет объект одноименного класса <span class="b">RouteData</span>, 
который имеет ряд свойств, содержащих информацию об обрабатываемом маршруте:</p>
<ul>
<li><p><span class="b">Values</span> представляет словарь значений маршрута. Эти значения получены с помощью сегментирования строки запроса URL. 
При обработке мы можем использовать эти значения.</p></li>
<li><p><span class="b">DataTokens</span> содержит набор дополнительных данных, которые связаны с обрабатываемым маршрутом. Однако если данные в 
RouteData.Values должны быть легко преобразованы в строки и обратно, то в <code>RouteData.DataTokens</code> могут быть данные любых типов.</p></li>
<li><p><span class="b">Routers</span> хранит список маршрутов, которые использовались для успешного сопоставления с запросом. 
Первый объект в этом списке представляет коллекцию маршрутов, которая применяется для генерации URL. А последний элемент этого списка - собственно тот маршрут, 
который совпал с запросом.</p></li>
</ul>
<p>И все эти данные мы можем получить при обрабтке запроса. Однако делегат RequestDelegate, который обрабатывает запрос в качестве параметра принимает не 
объект RouteContext, а объект HttpContext. Например:</p>
<pre class="brush:c#;">
...
<p>Но у объекта HttpContext есть метод расширения <span class="b">GetRouteData()</span>, который позволяет получить объект RouteData:</p>
<pre class="brush:c#;">RouteData routeData = context.GetRouteData();</pre>
<p>Для получения отдельных данных из словаря RouteData.Values также можно использовать еще один метод класса HttpContext - метод <span class="b">context.GetRouteValue()</span></p>
<pre class="brush:c#;">string controller = context.GetRouteValue("controller").ToString();</pre>
<p>Например, получим параметры запроса:</p>
<pre class="brush:c#;">
...
<img src="./pics/routing4.png" alt="Параметры маршрута в ASP.NET Core" />
<h3>Добавление маршрутов</h3>
<p>Чтобы добавить новые маршруты, нам надо последовательно вызвать несколько методов <code>routeBuilder.MapRoute()</code>, передав в них все необходимые параметры. 
Например:</p>
<pre class="brush:c#;">
var myRouteHandler = new RouteHandler(Handle);
var routeBuilder = new RouteBuilder(app, myRouteHandler);
...
<p>Теперь если запрос соответствует первому маршруту, то он выбирается для обработки, и другие маршруты не учитываются. Если же запрос не соответствует 
маршруту, то система маршрутизации сопоставляет запрос со следующими маршрутами.</p>
<p>При определении маршрутов важно их определять так, чтобы у системы маршрутизации не возникало неоднозначности, какой маршрут использовать. Возьмем два маршрута, определенные выше, 
и запрос <span class="ii">http://localhost:5634/index/lumia-2015</span>. На первый взгляд он соответствует первому маршруту. И параметры маршрутизации распределятся 
следующим образом:</p>
<table class="tab">
<tr><td><p>action</p></td><td><p>index</p></td></tr>
<tr><td><p>name</p></td><td><p>lumia</p></td></tr>
<tr><td><p>year</p></td><td><p>2015</p></td></tr>
</table>
<p>Но этому же запросу также соответствует и второй маршрут, так как этот маршрут соответствует трехсегментному запросу, в котором последний сегмент необязателен:</p>
<table class="tab">
<tr><td><p>controller</p></td><td><p>index</p></td></tr>
<tr><td><p>action</p></td><td><p>lumia-2015</p></td></tr>
</table>
<p>Причем система маршрутизации сопоставляет маршруты с запросом в том порядке, в котором они определены. Поэтому если мы определяем маршруты, которые могут пересекаться, 
то вначале надо определять те маршруты, которые имеют больший приоритет или более специфичные. А больше общие или стандартные маршруты должны определяться ближе к концу.</p>
...
<div class="nav"><p><a href="./11.2.php">Назад</a><a href="./">Содержание</a><a href="./11.3.php">Вперед</a></p></div></div>
<div class="socBlock">
