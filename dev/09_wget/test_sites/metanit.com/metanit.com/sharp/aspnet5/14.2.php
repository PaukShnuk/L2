...
<html >
<head>
<title>ASP.NET Core | Атрибут ResponseCache</title>
<meta charset="utf-8" />
<meta name="description" content="Кэширование с помощью атрибута ResponseCache в ASP.NET Core, профили кэширования">
...
<h2>Атрибут ResponseCache</h2><div class="date">Последнее обновление: 20.12.2019</div>
...
<p>Атрибут <span class="b">ResponseCache</span> представляет еще один инструмент кэширования. Он предполагает установку в ответе клиенту 
заголовков кэширования, которые определяют, как клиент и промежуточные прокси-серверы должны кэшировать ответ на своей стороне. При этом ResponseCache 
не выполняет кэширование на сервере! Его суть - только в установке отправляемых заголовков.</p>
<p>Заголовок кэширования - это обычные заголовки, которые определяются спецификацией протокола HTTP и которые позволяют управлять кэшированием. 
То есть это заголовки <span class="b">Cache-Control</span>, <span class="b">Pragma</span> и <span class="b">Vary</span>.</p>
<p>Основным из них является заголовок <code>Cache-Control</code>, который может принимать следующие значения:</p>
<ul>
<li><p><code>public</code>: ответ будет кэшироваться везде - и на машине клиента, и на промежуточных прокси-серверах</p></li>
<li><p><code>private</code>: ответ будет кэшироваться только на компьютере клиента, но промежуточные прокси-серверы не будут выполнять кэширование</p></li>
<li><p><code>no-cache</code>: ответ нигде не будет кэшироваться</p></li>
<li><p><code>max-age</code>: время кэширования</p></li>
</ul>
<p>Для настройки заголовоков ответа атрибут ResponseCacheAttribute определяет следующие свойства:</p>
<ul>
<li><p><span class="b">Duration</span>: устанавливает максимальное время кэширования в секундах. Является обязательным, если свойство <code>NoStore</code> 
не равно true. Duration добавляет к заголовку <code>Cache-Control</code> значение <code>max-age</code> с устанавливаемым временным промежутком в секундах</p></li>
<li><p><span class="b">Location</span>: определяет место кэширования. Принимает одно из значений из перечисления <span class="b">ResponseCacheLocation</span>:</p>
<ul>
<li><p><code>Any</code>: ответ кэшируется везде (в том числе и на прокси-серверах). Является значением по умолчанию</p></li>
<li><p><code>Client</code>: ответ кэшируется только на компьютере клиента</p></li>
<li><p><code>None</code>: ответ нигде не кэшируется</p></li>
</ul></li>
<li><p><span class="b">NoStore</span>: определяет, будет ли ответ кэшироваться. Если равен <code>true</code>, то ответ не кэшируется, а значения свойств 
<code>Location</code> и <code>Duration</code> игнорируются. Кроме того, заголовку <code>Cache-Control</code> добавляется значение <code>no-store</code></p></li>
<li><p><span class="b">VaryByHeader</span>: устанавливает заголовок <code>vary</code></p></li>
<li><p><span class="b">CacheProfileName</span>: определяет имя профиля кэширования</p></li>
<li><p><span class="b">Order</span>: устанавливает порядок данного атрибута при применении к методу контроллера одновременно нескольких атрибутов</p></li>
</ul>
<p>Атрибут ResponseCache может применяться к классам и методам контроллеров, а также к методам RazorPages.</p>
<p>Для практического применения возьмем проект по типу Web Application (Model-View-Controller) и применим атрибут к какому-нибудь методу контроллера данный атрибут. 
Сначала отключим любое кэширование:</p>
<pre class="brush:c#;">
...
<p>Здесь атрибут применяется к стандартному методу Index. Значение <code>NoStore =true</code> добавляет к заголовку Cache-Control значение <code>no-store</code>, 
а значение <code>Location =ResponseCacheLocation.None</code> добавляет к Cache-Control значение <code>no-cache</code></p>
<p>Теперь обратимся в браузере к этому методу:</p>
<img src="./pics/caching3.png" alt="Response Caching ASP.NET MVC Core" />
<p>Как видно, браузер клиента получил установленные заголовки:</p>
<pre>
Cache-Control: no-store,no-cache
Pragma: no-cache
</pre>
<p>Мы можем перезагрузить страницу, и опять будет идти обращение к серверу, поскольку кэширование не выполняется. 
Теперь изменим атрибут:</p>
<pre class="brush:c#;">
...
<p>Теперь будет выполняться кэширование, так как я явным образом использовал значение <code>Location =ResponseCacheLocation.Any</code> (оно применяется по умолчанию, 
поэтому его можно опускать). Кроме того, срок жизни кэша будет составлять 300 секунд, то есть 5 минут.</p>
<p>Перезапустим приложение, и теперь заголовок Cache-Control будет иметь другое значение:</p>
<pre>
Cache-Control: public,max-age=300
</pre>
<img src="./pics/caching4.png" />
<p>И если в течение следующих 300 секунд мы снова обратимся к методу Index, например, перейдем по ссылке в нашем приложении, которая ведет на этот метод, то ответ будет извлекаться из кэша браузера:</p>
<img src="./pics/caching5.png" alt="Кэширование в ASP.NET Core" />
<h3>Профили кэширования</h3>
<p>Если у нас все методы контроллера должны отправлять одни и те же заголовки кэширования, то мы можем применить данный атрибут ко всему сразу контроллеру. 
...
профиль кэширования. Для этого перейдем к методу <code>ConfigureServices</code> класса Startup и изменим его следующим образом:</p>
<pre class="brush:c#;">
...
<p>Каждый профиль кэширования представлен объектом <span class="b">CacheProfile</span> из пространства имен <i>Microsoft.AspNetCore.Mvc</i>. Для 
добавления профиля применяется метод <code>options.CacheProfiles.Add()</code>, в который передается название профиля и собственно профиль.</p>
<p>Объект профиля устанавливает все те свойства, что и атрибут ResponseCacheAttribute.</p>
<p>Здесь добавляется два профиля, при необходимости мы можем добавить профилей, сколько нам надо. И затем в атрибуте ResponseCache нам надо указать имя нужного профиля через параметр <span class="b">CacheProfileName</span>:</p>
<pre class="brush:c#;">
