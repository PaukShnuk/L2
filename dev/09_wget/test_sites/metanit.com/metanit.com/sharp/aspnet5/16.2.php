...
<html >
<head>
<title>ASP.NET Core | Добавление Identity в проект с нуля</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление системы авторизации Identity в проект ASP.NET Core с нуля">
...
<h2>Добавление Identity в проект с нуля</h2><div class="date">Последнее обновление: 08.01.2021</div>
...
<p>Проект с типом аутентификации Individual User Accounts позволяет автоматически добавить все необходимые файлы для работы с ASP.NET Core Identity. Однако, как правило, 
редко востребован весь стандартный функционал, кроме того, нередко вознкает необходимость более детально настроить различные компоненты, из которых 
состоит Identity. И в реальности мы можем выбрать любой другой тип проекта и в него уже добавить вручную и подкорректировать 
функционал ASP.NET Core Identity. Это даст нам больший контроль над тем кодом, который размещается в проекте. Также это может быть актуально если у нас уже есть рабочий проект, 
но в него необходимо добавить поддержку ASP.NET Core Identity.</p>
<p>Итак, вначале создадим проект ASP.NET Core по шаблону ASP.NET Core Web App (Model-View-Controller) но без аутентификации:</p>
<img src="./pics/customidentity1.png" alt="Добавление ASP.NET Core Identity" />
<p>Вначале добавим в папку <span class="b">Models</span> новый класс <span class="b">User</span>:</p>
<pre class="brush:c#;">
...
<p>Класс User представляет пользователя и наследуется от класса IdentityUser, перенимая все его свойства. Кроме того, для примера здесь добавлено свойство Year, 
которое будет представлять год рождения пользователя. При желании можно определить любые другие свойства.</p>
<p>Для взаимодействия с MS SQL Server через ASP.NET Core Identity добавим в проект через Nuget пакеты <span class="b">Microsoft.AspNetCore.Identity.EntityFrameworkCore</span> 
и <span class="b">Microsoft.EntityFrameworkCore.SqlServer</span>.</p>
<img src="./pics/customidentity17.png" alt="ASP.NET Core Identity и Entity Framework Core" />
<p>Далее добавим в папку Models класс контекста данных <span class="b">ApplicationContext</span>. Так как мы используем Identity, то класс контекста данных будет наследоваться не от 
DbContext, а от IdentityDbContext:</p>
<pre class="brush:c#;">
...
<p>У нас есть контекст и модели, и теперь нам необходима база данных, которая будет хранить все данные. Вначале определим в файле <span class="ii">appsettings.json</span> 
строку подключения:</p>
<pre class="brush:js;">
"ConnectionStrings": {
    "DefaultConnection": "Server=(localdb)\\mssqllocaldb;Database=usersstoredb;Trusted_Connection=True;"
  },
</pre>
<p>Далее нам надо изменить класс Startup, чтобы применить все необходимые сервисы для работы с Identity и базой данных:</p>
<pre class="brush:c#;">
...
<p>В методе <code>ConfigureServices()</code> добавляются сервисы для Entity Framework Core:</p>
<pre class="brush:c#;">
services.AddDbContext&lt;ApplicationContext&gt;(options =&gt;
    options.UseSqlServer(Configuration.GetConnectionString("DefaultConnection")));
</pre>
<p>Далее добавляются специфичные для Identity сервисы:</p>
<pre class="brush:c#;">
services.AddIdentity&lt;User, IdentityRole&gt;()
    .AddEntityFrameworkStores&lt;ApplicationContext&gt;();
</pre>
<p>Метод <code>AddIdentity()</code> позволяет установить некоторую начальную конфигурацию. Здесь мы указываем тип пользователя и тип роли, которые будут использоваться системой Identity. 
В качестве типа пользователя выступает созданный нами выше класс User, а в качестве типа роли взят стандартный класс <code>IdentityRole</code>, 
который находится в пространстве имен <code>Microsoft.AspNetCore.Identity.EntityFrameworkCore</code>.</p>
<p>Метод <code>AddEntityFrameworkStores()</code> устанавливает тип хранилища, которое будет применяться в Identity для хранения данных. В качестве типа хранилища 
здесь указывается класс контекста данных.</p>
<p>Затем, чтобы использовать Identity, в методе <code>Configure()</code> устанавливается компонент middeware - <code>UseAuthentication</code>. Причем это middleware 
вызывается перед <code>app.UseEndpoints()</code>, тем самым гарантируя, что ко времени обращения к системе маршрутизации, контроллерам и их методам, куки должным образом обработаны и установлены.</p>
<p>Теперь система Identity подключена в проект, и мы можем с ней работать.</p>
...
<div class="nav"><p><a href="./16.11.php">Назад</a><a href="./">Содержание</a><a href="./16.3.php">Вперед</a></p></div></div>
<div class="socBlock">
