...
<html >
<head>
<title>ASP.NET Core MVC | Переадресация</title>
<meta charset="utf-8" />
<meta name="description" content="Переадресация в ASP.NET MVC Core, использование объекта RedirectResult и методов RedirectToAction и RedirectToRoute">
...
<h2>Переадресация</h2><div class="date">Последнее обновление: 17.11.2019</div>
...
<p>В ASP.NET Core MVC для создания переадресации используются классы <span class="b">RedirectResult</span>, 
<span class="b">LocalRedirectResult</span>, <span class="b">RedirectToActionResult</span> и <span class="b">RedirectToRouteResult</span>. Но в зависимости 
от типа переадресации их применение будет отличаться.</p> 
<p>Протокол HTTP поддерживает два типа переадресации:</p>
<ul>
<li><p><span class="b">постоянная переадресация</span>. При постоянной переадресации сервер будет отправлять браузеру статусный код 301. При данном типе 
...
Поэтому данный способ можно использовать, если вы полностью уверены, что документ на старое место уже не возвратится..<p></li>
<li><p><span class="b">временная переадресация</span>. При временной переадресации сервер будет отправлять браузеру статусный код 302. 
При этом считается, что запрашиваемый документ временно перемещен на другую страницу.<p></li>
</ul>
<p>В обоих случаях для создания переадресации может использоваться объект <span class="b">RedirectResult</span>, однако метод, возвращающий данный объект, будет отличаться.</p>
<p>Для временной переадресации применяется метод <span class="b">Redirect</span>:</p>
<pre class="brush:c#;">
...
<p>В данном случае идет переадресация на локальный адрес - методу About контроллера Home. Причем обращаю внимание на знак тильды в начале пути "~". В ASP.NET 
данный знак ссылается на корень приложения. А использование пути без тильды "/Home/About" было бы не совсем корректно в некоторых слуаях. Например, 
при обращении к адресу Home/Index в данном случае будет идти переадресация на ресурс Home/Home/About. Поэтому рекомендую всегда использовать в начале локального адреса тильду.</p>
<p>Ну и кроме того, также можно обращаться к внешнему ресурсу: <code>return Redirect("http://microsoft.com")</code></p>
...
<p>Если же мы передадим в эти методы адрес внешнего ресурса, например:</p>
<pre class="brush:c#;">
public IActionResult Index()
{
...
<p>То в этом случае метод выбросит исключение.</p>
<p>Для создания переадресации на определенный метод контроллера используется объект <span class="b">RedirectToActionResult</span>. Опять же для его генерации используется пара методов для временной и постоянной переадресации: 
<span class="b">RedirectToAction</span> и <span class="b">RedirectToActionPermanent</span></p>
<p>Оба этих метода имеют несколько версий:</p>
<ul>
<li><p><code>RedirectToAction(string actionName)</code>/<code>RedirectToActionPermanent(string actionName)</code>: actionName - метод в рамках текущего контроллера, на который надо выполнить переадресацию</p></li>
<li><p><code>RedirectToAction(string actionName, string controllerName)</code>/<code>RedirectToActionPermanent(string actionName, string controllerName)</code>: 
переадресация выполняется на метод actionName контроллера controllerName</p></li>
<li><p><code>RedirectToAction(string actionName, object routeValues)</code>/
<code>RedirectToActionPermanent(string actionName, object routeValues)</code>: здесь также учитываются дополнительные параметры запроса, которые представляет объект routeValues</p></li>
<li><p><code>RedirectToAction(string actionName, string controllerName, object routeValues)</code>/
<code>RedirectToActionPermanent(string actionName, string controllerName, object routeValues)</code>: объединение двух предыдущих версий</p></li>
</ul>
<p>Рассмотрим на примере последней версии метода <code>RedirectToAction()</code>:</p>
<pre class="brush:c#;">
...
<p>В качестве последнего параметра в метод <code>RedirectToAction</code> передается анонимный объект, каждое свойство которого имеет то же имя, что и параметры метода 
Area. В прицнипе тут можно было бы не использовать название контроллера, так как Index и Area находятся в одном контроллере.</p>
<p>И последний класс, который используется для создания переадресации - <span class="b">RedirectToRouteResult</span>. Для переадресации он использует маршруты. Для создания объекта этого класса имеются 
два метода: <span class="b">RedirectToRoute</span> и <span class="b">RedirectToRoutePermanent</span> (соответственно для временной и постоянной переадресации).</p>
<p>Эти методы имеют по три перегруженных версии:</p>
<ul>
<li><p><code>RedirectToRoute(string routeName)</code>/<code>RedirectToRoutePermanent(string routeName)</code>: routeName - название маршрута</p></li>
<li><p><code>RedirectToRoute(object routeValues)</code>/<code>RedirectToRoutePermanent(object routeValues)</code>: routeValues - параметры маршрута</p></li>
<li><p><code>RedirectToRoute(string routeName, object routeValues)</code>/
<code>RedirectToRoutePermanent(string routeName, object routeValues)</code>: передает маршруту routeName параметры в виде объекта routeValues</p></li>
</ul>
<p>Рассмотрим последний вариант:</p>
<pre class="brush:c#;">
public class HomeController : Controller
{
    public IActionResult Index()
...
<p>В классе Startup по умолчанию определен один маршрут:</p>
<pre class="brush:c#;">
...
<p>Поэтому в метод RedirectToRoute передается название "default". В этом маршруте есть параметры controller, action, которые устанавливаются в анонимном объекте. 
Ну и также передаются параметры altitude и height, которые принимает метод Area.</p>
