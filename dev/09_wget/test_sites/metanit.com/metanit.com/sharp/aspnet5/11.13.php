...
<html >
<head>
<title>ASP.NET Core | Маршрутизация с помощью RouterMiddleware. Метод UseMvc</title>
<meta charset="utf-8" />
<meta name="description" content="Маршрутизация в ASP.NET Core MVC с помощью RouterMiddleware. Метод UseMvc, применение IRouteBuilder, определение маршрутов, MvcRouteHandler">
...
<h2>Маршрутизация с помощью RouterMiddleware. Метод UseMvc</h2><div class="date">Последнее обновление: 23.11.2019</div>
...
<p>Вместо использования конечных точек и EndpointMiddleware для созданию системы маршрутизации в приложении мы можем применять метод <span class="b">UseMvc()</span>. 
Он встраивает в конвейер обработки запроса компонент <span class="b">RouterMiddleware</span>, который используется для сопоставления запросов с маршрутами.</p>
<p>Стоит отметить, что мы можем использовать либо новую форму маршрутизации на основе конечных точек на основе EndpointRoutingMiddleware + EndpointMiddleware, 
либо старую форму маршрутизации с использованием RouterMiddleware. Поэтому если мы хотим использовать метод <code>UseMvc()</code>, то необходимо при подключении сервисов 
...
сервисов MVC, все эти методы в качестве параметра могут принимать делегат <span class="b">Action&lt;MvcOptions&gt;</span>. И с помощью свойства 
<span class="b">EnableEndpointRouting</span> объекта MvcOptions можно отключить использование конечных точек:</p>
<pre class="brush:c#;">
...
<p>Применим метод <code>UseMvc()</code> для определения маршрутов:</p>
<pre class="brush:c#;">
...
<p>Обратите внимание, что теперь естественно не надо использовать методы UseRouting и UseEndpoints для встраивания в конвейер системы маршрутизации.</p>
<p>Метод <code>app.UseMvc()</code> в качестве параметра принимает объект <span class="b">Action&lt;IRouteBuilder&gt;</span>, который добавляет 
...
действие Index.</p>
<p>Хотя данный способ является альтернативой использованию конечных точек, но в целом определение маршрутов будет выглядеть также, как и в методе UseEndpoints. Например, 
определим несколько маршрутов:</p>
<pre class="brush:c#;">
app.UseMvc(routes =&gt;
{
...
<p>Первый маршрут сопоставляется с запросом типа <span class="ii">http://localhost:xxxx/api/get</span>, и для обработки такого запроса будет выбираться метод About контроллера Home. Второй маршрут стандартный.</p>
<p>Поскольку второй маршрут является стандартным и применяется многими разработчиками, то для его создания определен специальный метод:</p>
<pre class="brush:c#;">
app.UseMvcWithDefaultRoute();
</pre>
<p>То есть этот вызов метода равносилен конструкции:</p>
<pre class="brush:c#;">
app.UseMvc(routes =&gt;
{
    routes.MapRoute(
...
<p>Поэтому вместо определения стандартного маршрута через <code>app.UseMvc()</code>, мы можем применять метод <code>app.UseMvcWithDefaultRoute()</code>.</p>
<h3>Анатомия метода UseMvc</h3>
<p>Метод UseMvc имеет следующее определение:</p>
<pre class="brush:c#;">
...
<p>Исходный код можно найти на <a href="https://github.com/aspnet/AspNetCore/blob/master/src/Mvc/Mvc.Core/src/Builder/MvcApplicationBuilderExtensions.cs" rel="nofollow">гитхабе</a></p>
<p>Как можно увидеть, данный метод полагается на компонент RouterMiddleware, который был рассмотрен в статье <a href="https://metanit.com/sharp/aspnet5/11.12.php">RouteMiddlevare</a>. 
Для создания маршрута применяетс класс RouteBuilder, а для обработки маршрутов фреймворк предоставляет встроенный класс <span class="b">MvcRouteHandler</span>, 
исходный код которого можно найти <a href="https://github.com/aspnet/AspNetCore/blob/master/src/Mvc/Mvc.Core/src/Routing/MvcRouteHandler.cs" rel="nofollow">здесь</a>.</p>
<h3>Применение своего обработчика маршрутов</h3>
<p>Хотя по умолчанию все маршруты в UseMvc обрабатываются с помощью MvcRouteHandler, но его применение необязательно, и мы можем задать свою логику обработки маршрутов. Например, 
изменим класс Startup следующим образом:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Builder;
using Microsoft.AspNetCore.Hosting;
using Microsoft.AspNetCore.Http;
...
<p>Для первого маршрута с шаблоном "api/get" здесь задан свой обработчик в виде асинхронного делегата, который отправляет в выходной поток одну строку. Остальные маршруты 
будут обрабатываться стандартным MvcRouteHandler.</p>
<img src="./pics/routing6.png" alt="Маршруты в ASP.NET Core MVC" />
...
<div class="nav"><p><a href="./11.5.php">Назад</a><a href="./">Содержание</a><a href="./11.6.php">Вперед</a></p></div></div>
<div class="socBlock">
