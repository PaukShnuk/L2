...
<html >
<head>
<title>ASP.NET Core | Обработка запросов в Razor Pages</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка запросов. Передача форм с Razor Pages в ASP.NET Core, методы OnGet и OnPost, асинхронная обработка запроса">
...
<h2>Обработка запросов. Передача форм</h2><div class="date">Последнее обновление: 13.12.2019</div>
...
можно определять соответствующие методы, которые называются по шаблону <span class="ii">On[Тип_запросы]</span>, либо можно использовать 
их асинхронные двойники <span class="ii">On[Тип_запросы]</span>. Например, запрос Get обрабатывается методами <span class="b">OnGet()</span> и <span class="b">OnGetAsync()</span>, 
а запросы Post - методами <span class="b">OnPost()</span> и <span class="b">OnPostAsync()</span>.</p>
<p>Возьмем проект RazorPages и добавим в него новую страницу <span class="b">Exchange.cshtml</span>:</p>
<img src="./pics/razorpages30.png" alt="OnGet и OnPost в Razor Page в ASP.NET Core" />
<p>Допустим, наша страница будет предоставлять информацию об обменном курсе. Пусть файл Exchange.cshtml.cs будет обрабатывать get и post запросы следующим образом:</p>
<pre class="brush:c#;">
...
<p>И также изменим страницу Exchange.cshtml:</p>
<pre class="brush:xml;">
@page
@model ExchangeModel
@{
...
<p>При получении запроса get приложение просто будет отдавать данную страницу с формой ввода. При получении запроса post класс ExchangeModel 
получит отправленное значение и подсчитает результат, который потом выводится на страницу. При этом, чтобы система могла связать параметры запроса со значениями формы также, как и в MVC 
поля форм (их атрибут name) должны называться также, как и параметры методов OnPost или OnGet.</p>
<p>При запуске страница отобразит приглашение к вводу:</p>
<img src="./pics/razorpages7.png" alt="OnGet в Razor Pages" />
<p>А при отправке введенного значения отобразится результат:</p>
<img src="./pics/razorpages8.png" alt="OnPost в Razor Pages" />
<p>Для обработки запроса можно использовать класс, производный от PageModel. Однако страница Razor уже сама по себе представляет модель. И мы 
можем всю логику обработки и свойства модели определить напрямую в странице. Например, изменим страницу Exchange.cshtml:</p>
<pre class="brush:c#;">
@page
...
<p>Теперь класс ExchangeModel никак не используется, вся логика определена непосредственно на странице, поэтому работать она будет также как и ранее.</p>
<p>Однако не все такой подход может показаться привлекательным из-за того, что визуальное представление смешивается с логикой и представлением данных. 
Поэтому, как правило, подход с классом, производным от PageModel, является более предпочтительным.</p>
...
<div class="nav"><p><a href="./29.2.php">Назад</a><a href="./">Содержание</a><a href="./29.4.php">Вперед</a></p></div></div>
<div class="socBlock">
