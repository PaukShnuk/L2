...
<html >
<head>
<title>ASP.NET Core | Валидация пароля</title>
<meta charset="utf-8" />
<meta name="description" content="Валидация пароля в ASP.NET Core Identity с помощью объекта IPasswordValidator, переопределение механизма валидации">
...
<h2>Валидация пароля</h2><div class="date">Последнее обновление: 15.12.2019</div>
...
<p>Система ASP.NET Identity уже имеет ряд ограничений на вводимые данные, которые нам надо использовать. 
...
Identity уже заложен ряд ограничений:</p>
<img src="./pics/password1.png" alt="Валидация пароля в ASP.NET Core" />
<p>При вводе пароля срабатывает встроенная логика, согласно которой длина пароля не может быть меньше 6 символов, пароль должен содержать как минимум один цифровой и один 
...
минимальную длину пароля? Или что если мы хотим, чтобы в пароле могли бы использоваться только цифровые или только алфавитные символы?</p>
<p>И несмотря на то, что встроенная логика валидации скрыта от наших глаз, мы ее можем переопределить с помощью объекта 
<span class="b">Microsoft.AspNetCore.Identity.PasswordOptions</span>.</p>
<p>Чтобы его применить, необходимо изменить код добавления контекста в классе Startup в методе <code>ConfigureServices()</code>, который по умолчанию имеет следующее содержимое:</p>
<pre class="brush:c#;">
services.AddIdentity&lt;User, IdentityRole&gt;()
		.AddEntityFrameworkStores&lt;ApplicationContext&gt;();
</pre>
<p>Итак, изменим данный код следующим образом:</p>
<pre class="brush:c#;">
...
<p>Класс <span class="b">PasswordOptions</span> предоставляет следующие свойства:</p>
<ul>
<li><p><code>RequiredLength</code>: минимальная длина пароля</p></li>
<li><p><code>RequireNonLetterOrDigit</code>: если равно true, то пароль должен будет иметь как минимум один символ, который не является алфавитно-цифровым</p></li>
<li><p><code>RequireDigit</code>: если равно true, то пароль должен будет иметь как минимум одну цифру</p></li>
<li><p><code>RequireLowercase</code>: если равно true, то пароль должен будет иметь как минимум один символ в нижнем регистре</p></li>
<li><p><code>RequireUppercase</code>: если равно true, то пароль должен будет иметь как минимум один символ в верхнем регистре</p></li>
</ul>
<p>Если мы меняем длину пароля, то соответственно нам надо изменить определение модели регистрации, если она устанавливает ограничение на длину:</p>
<pre class="brush:c#;">
...
<p>Для большинства случаев настроек класса PasswordOptions вполне хватит, мы можем настроить минимальную длину пароля, ввод алфавитных, цифровых или не алфавитно-цифровых 
символов. Однако если нам потребуется более сложная логика валидации, то придется определять свой класс валидатора.</p>
<p>Объект валидатора представляет объект интерфейса <code>IPasswordValidator&lt;T&gt;</code>:</p>
<pre class="brush:c#;" >
...
<p>Метод <code>ValidateAsync()</code> вызывается при валидации пароля. В качестве параметров он получает объект UserManager, 
объект пользователя и сам пароль, который надо валидировать.</p>
<p>Результат метода представляет объект <code>IdentityResult</code>, через статус которого можно узнать об успешности валидации: если 
свойство <code>Succeeded</code> равно <code>true</code>, то валидация прошла успешно.</p>
<p>Итак, создадим новый класс, который назовем и который будет иметь следующий код:</p>
<pre class="brush:c#;">
using System;
...
<p>Для проверки на длину в классе определено свойство RequiredLength. Кроме проверки количества символов пароль здесь также проверяется на соответствие 
...
в методе различные способы проверок.</p>
<p>Если какая-то из проверок не будет пройдена, то в список ошибок добавляется очередная ошибка в виде объекта IdentityError. При отстутствии ошибок возвращается результат <code>IdentityResult.Success</code>.</p>
<p>Для использования класса перейдем в класс Startup и изменим его метод <code>ConfigureServices()</code>:</p>
<pre class="brush:c#;">
...
<p>Первый вызов добавляет зависимость для интерфейса IPasswordValidator в виде объекта CustomPasswordValidator.</p>
<p>И теперь, если мы введем некорректные с точки зрения нашего валидатора значения, то получим соответствующие сообщения:</p>
<img src="./pics/password2.png" alt="Создание валидатора пароля в ASP.NET Core" />
...
<div class="nav"><p><a href="./16.8.php">Назад</a><a href="./">Содержание</a><a href="./16.10.php">Вперед</a></p></div></div>
<div class="socBlock">
