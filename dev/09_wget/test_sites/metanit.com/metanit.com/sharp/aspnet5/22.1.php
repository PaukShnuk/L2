...
<meta charset="utf-8" />
<meta name="description" content="Введение в юнит-тесты в ASP.NET Core, Test-Driven-Development, принципы тестирования">
...
<h1>Тестирование</h1><h2>Введение в юнит-тесты</h2><div class="date">Последнее обновление: 21.12.2019</div>
...
<p>До запуска приложения в производство, когда оно станет доступно пользователям, важно убедиться, что данное приложение функционирует, как и должно, что 
в нем нет ошибок. Для проверки приложения мы можем использовать различные схемы и механизмы тестирования. Одним из таких механизмов являются юнит-тесты.</p>
<p>Юнит-тесты позволяют быстро и автоматически протестировать отдельные компоненты приложения независимо от остальной его части. Не всегда юнит-тесты могут 
покрыть весь код приложения, но тем не менее они позволяют существенно уменьшить количество ошибок уже на этапе разработки.</p>
<p>Мы не должны тестировать код используемого фреймворка или используемых зависимостей. Тестировать надо только тот код, который написали мы сами.</p>
<p>Надо отметить, что в целом концепция юнит-тестов не является непреложным требованием к веб-разработке, да и вообще к разработке. Кто-то считает, что юнит-тесты 
...
Некоторые не используют юнит-тесты.</p>
<p>Но тем не менее юнит-тесты несут потенциальные преимущества при разработке, к которым следует отнести не только собственно проверку результата и тестирование кода, но 
...
нам надо, чтобы они были слабосвязанными. А подобное построение приложения в дальнейшем может положительно сказаться на его последующей модификации и поддержке.</p>
<p>Большинство юнит-тестов так или иначе имеют ряд следующих признаков:</p>
<p><span class="b">Тестирование небольших участков кода ("юнитов")</span></p>
<p>Для создания юнит-тестов выбираются небольшие участки кода, которые надо протестировать. Тестируемый участок, как правило, должен быть меньше класса. 
В большинстве случаев тестируется отдельный метод класса или даже часть функционала метода. Упор на небольшие участки позволяет довольно быстро писать простенькие тесты.</p>
...
<p><span class="b">Тестирование в изоляции от остального кода</span></p>
...
ошибок именно в этом изолированном коде. Что упрощает и повышает контроль над отдельными компонентами программы.</p>
<p><span class="b">Автоматизация тестов</span></p>
<p>Создание юнит-тестов для небольших участков кода ведет к тому, что количество этих юнит-тестов становится очень велико. 
...
<p><span class="b">Тестирование только общедоступных конечных точек</span></p>
<p>Даже небольшие изменения в классе могут привести к неудаче многих юнит-тестов, поскольку реализация используемого класса изменилась. 
...
<h3>Фрейморки тестирования</h3>
<p>Для написания юнит-тестов мы можем сами создавать весь необходимый функционал, использовать какие-то свои способы тестирования, однако, как правило, для этого применяются 
специальные фреймворки. Некоторые из них:</p>
<ul>
<li><p><b>xUnit.net</b>: фреймворк тестирования для платформы .NET. Наиболее популярный фреймворк для работы именно с .NET Core и ASP.NET Core</p></li>
<li><p><b>MS Test</b>: фреймворк юнит-тестирования от компании Microsoft, который по умолчанию включен в Visual Studio и который также можно использовать с .NET Core</p></li>
<li><p><b>NUnit</b>: портированный фреймворк с JUnit для платформы .NET</p></li>
</ul>
<p>Данные фреймворки предоставляют несложный API, который позволяет быстро написать и автоматически проверить тесты.</p>
<h3>Разработка через тестирование (Test-Driven-Development)</h3>
<p>Отдельно стоит сказать о концепции <span class="b">TDD</span> или <span class="b">разработка через тестирование</span>. TDD представляет 
процесс применения юнит-тестов, при котором сначала пишутся тесты, а потом уже программный код, достаточный для выполнения этих тестов.</p>
...
<p>Порядок написания кода при TDD довольно прост:</p>
<ul>
...
<div class="nav"><p><a href="./20.2.php">Назад</a><a href="./">Содержание</a><a href="./22.2.php">Вперед</a></p></div></div>
<div class="socBlock">
