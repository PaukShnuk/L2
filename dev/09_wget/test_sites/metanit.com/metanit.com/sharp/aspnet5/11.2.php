...
<html >
<head>
<title>ASP.NET Core | Определение маршрутов</title>
<meta charset="utf-8" />
<meta name="description" content="Создание маршрутов в ASP.NET Core, соответствие маршрута строке запроса URL, установка обработчика маршрута">
...
<h2>Определение маршрутов</h2><div class="date">Последнее обновление: 15.11.2019</div>
...
<p>В прошлой теме были рассмотрены базовые моменты работы с маршрутов. Теперь рассмотрим различные способы определения маршрутов.</p>
<p>Для добавления маршрута в прошлой теме использовался метод <code>MapRoute()</code>. Он имеет ряд перегруженных версий, которые позволяют установить различные параметры:</p>
<ul>
<li><p><code>MapRoute(string template, RequestDelegate handler)</code>: устанавливает маршрут с шаблоном template, который будет обрабатываться делегатом handler</p></li>
<li><p><code>MapRoute(string name, string template)</code>: устанавливает маршрут с именем name и шаблоном template</p></li>
<li><p><code>MapRoute(string name, string template, object defaults)</code>: добавляет значения по умолчанию в виде объекта defaults</p></li>
<li><p><code>MapRoute(string name, string template, object defaults, object constraints)</code>: добавляет ограничения к маршруту в виде объекта constraints</p></li>
<li><p><code>MapRoute(string name, string template, object defaults, object constraints, object dataTokens)</code>: добавляет токены в виде объекта dataTokens</p></li>
</ul>
<p>Так первая версия позволяет сразу задать обработчик маршрута для определенного шаблона. Например, применим первую версию метода MapRoute:</p>
<pre class="brush:c#;">
...
<p>Здесь определено два маршрута, причем каждый из них имеет свой обработчик. Первый маршрут соответствует запросу с двумя сегментами, а второй - с тремя сегментами. 
Эта версия метода MapRoute тем и удобна, что мы можем легко повесить на каждый маршрут свой обработчик.</p>
<img src="./pics/routing12.png" alt="RouteBuilder и определение маршрутов в ASP.NET Core" />
<h3>URL matching</h3>
<p>Ранее в предыдущей теме использовалась вторая версия:</p>
<pre class="brush:c#;">routeBuilder.MapRoute("default", "{controller}/{action}");</pre>
<p>Такая версия определяет маршрут с именем "default" и шаблоном URL "{controller}/{action}", который состоит из двух сегментов. С этим шаблоном затем сопоставляется запрошенный адрес URL. 
При удачном сопоставлении запускается обработчик маршрута, который обрабатывает запрос.</p>
<p>Но важно заметить, что здесь в шаблоне URL используются <code>параметры маршрута</code>. Каждый параметр заключается в фигурные скобки. Когда приходит запрос, 
система маршрутизации при удачном сопоставлении запроса с маршрутом будет выделять из запроса значения для этих параметров.</p>
<p>Например, при запросе <span class="ii">http://localhost:xxxx/Home/Index</span> значения для параметров распределятся так:</p>
<table class="tab">
<tr class="tabhead"><td><p>controller</p></td><td><p>action</p></td></tr>
<tr><td><p>Home</p></td><td><p>Index</p></td></tr>
</table>
<p>Или запрос <i>http://localhost:xxxx/Book/Order</i> сформирует следующие значения:</p>
<table class="tab">
<tr class="tabhead"><td><p>controller</p></td><td><p>action</p></td></tr>
<tr><td><p>Book</p></td><td><p>Order</p></td></tr>
</table>
<h3>Статические сегменты</h3>
<p>Однако сегменты шаблона необязательно должны представлять параметры URL, это также могут быть константные значения. Например, пусть у нас будет определен 
следующий маршрут:</p>
<pre class="brush:c#;">routeBuilder.MapRoute("default", "store/{action}");</pre>
<p>Шаблон опять же состоит из двух сегментов, но первый сегмент представляет константное значение "store", поэтому такой маршрут будет соответствовать, например, 
такому запросу <span class="ii">http://localhost:xxxx/Store/Order</span> или любому другому запросу, который состоит из двух сегментов, и первым сегментов обязательно идет "Store", 
а второй сегмент по-прежнему может представлять любое значение для параметра "action".</p>
<p>Или к примеру маршрут:</p>
<pre class="brush:c#;">routeBuilder.MapRoute("default", "api/{controller}/{action}/{id?}");</pre>
<p>Этот маршрут определяет в шаблоне статический сегмент "api", поэтому запрос в качестве первого сегмента обязательно должен иметь строку "api". 
И подобному маршруту будет соответствовать, например, следующий запрос:</p>
<pre>http://localhost:56130/api/Home/Index/1</pre>
<h3>Необязательные параметры</h3>
<p>При использовании параметров в шаблоне URL мы можем их помечать как необязательные с помощью знака вопроса. Например, определим следующий маршрут:</p>
<pre class="brush:c#;">routeBuilder.MapRoute("default", "{controller}/{action?}/{id?}");</pre>
<p>Здесь шаблон состоит из трех сегментов, и последний сегмент id помечен как необязательный. А это значит, что мы можем в запросе игнорировать значение 
для этого сегмента. Например, данный шаблон будет соответствовать двум следующим url:</p>
<pre>
localhost:56130/Home/Index/
localhost:56130/Home/Index/2
</pre>
<p>Но при определении необязательных параметров надо учитывать одну вещь: мы можем определять необязательные параметры, начиная с конца, как в случае 
с параметром id. То есть шаблон <span class="ii">{controller}/{action?}/{id}</span> будет некорректным. А вот шаблон <span class="ii">{controller}/{action?}/{id?}</span> 
будет работать нормально.</p>
<h3>Значения для параметров по умолчанию</h3>
<p>Выше были рассмотрены версии метода <code>MapRoute()</code>, одна из которых позволяет задавать для параметров значения по умолчанию, если 
вдруг в запрошенный адрес url не содержит нужных значений:</p>
<pre class="brush:c#;">routeBuilder.MapRoute("default", "{controller}/{action}/{id?}", new { controller = "home", action = "index" });</pre>
<p>Третий параметр устанавливает значения по умолчанию. В итоге при различных запросах у нас получатся следующие значения:</p>
<table class="tab">
<tr class="tabhead"><td><p>Запрос</p></td><td><p>Параметры запроса</p></td></tr>
<tr><td><p>http://localhost:56130/</p></td><td><p>controller=Home</p>
<p>action=Index</p></td></tr>
<tr><td><p>http://localhost:56130/Book</p></td><td><p>controller=Book</p>
<p>action=Index</p></td></tr>
<tr><td><p>http://localhost:56130/Book/Show</p></td><td><p>controller=Book</p>
<p>action=Show</p></td></tr>
<tr><td><p>http://localhost:56130/Book/Show/2</p></td><td><p>controller=Book</p><p>action=Show</p><p>id=2</p></td></tr>
</table>
<p>В качестве альтернативы мы можем задать значения по умолчанию сразу же при определении параметров:</p>
<pre class="brush:c#;">routeBuilder.MapRoute("default", "{controller=Home}/{action=Index}/{id?}");</pre>
<h3>Передача произвольного количества параметров в запросе</h3>
<p>Мы можем обозначить любое количество сегментов в запросе, чтобы не быть жестко привязанным к числу сегментов с помощью параметра со знаком * ("звездочка"):</p>
<pre class="brush:c#;">routeBuilder.MapRoute("default", "{controller=Home}/{action=Index}/{id?}/{*catchall}");</pre>
<p>Параметр <code>catchall</code> будет соответствовать всем тем сегментам строки запроса, которые будут идти после id. То есть, например, при 
запросе <i>http://localhost:56130/Home/Index/1/name/book/order</i> значения распределятся следующим образом:</p>
<table class="tab">
<tr><td><p>controller</p></td><td><p>Home</p></td></tr>
<tr><td><p>action</p></td><td><p>Index</p></td></tr>
<tr><td><p>id</p></td><td><p>1</p></td></tr>
<tr><td><p>catchall</p></td><td><p>name/book/order</p></td></tr>
</table>
<h3>Использование префиксов</h3>
<p>Сегменты строки запроса необязательно полностью должны нести только значения для параметров или констант, определенных в шаблоне маршруте. 
...
Например, мы можем определить следующий маршрут:</p>
<pre class="brush:c#;">routeBuilder.MapRoute("default", "Ru{controller=Home}/{action=Index}-en/{id?}");</pre>
<p>Первый сегмент обязательно должен начинаться с префикса "Ru", а вся остальная часть сегмента считается параметром "controller". 
...
И такому шаблону будет соответствовать, к примеру, следующий запрос:</p>
<pre>http://localhost:56130/RuHome/Index-en/1</pre>
<h3>Несколько параметров в сегменте</h3>
<p>И также в одном сегменте может быть несколько параметров. Например, определим следующий маршрут:</p>
<pre class="brush:c#;">routeBuilder.MapRoute("default", "{controller=Home}/{action=Index}/{name}-{year}");</pre>
<p>Здесь последний сегмент включает два параметра name и year, которые разделены дефисом. И такому шаблону соответствует следующий запрос:</p>
<pre>http://localhost:56130/Store/Order/lumia-2015</pre>
<p>Распределение параметров:</p>
<table class="tab">
<tr><td><p>controller</p></td><td><p>Store</p></td></tr>
<tr><td><p>action</p></td><td><p>Order</p></td></tr>
<tr><td><p>name</p></td><td><p>lumia</p></td></tr>
<tr><td><p>year</p></td><td><p>2015</p></td></tr>
</table>
<h3>Маршруты для разных типов запросов</h3>
<p>Кроме метода <code>MapRoute()</code> для создания маршрута мы можем использовать еще ряд методов, которые предназначены для создания маршрутов 
для определенных типов запросов: GET, POST, PUT, DELETE. Это методы:</p>
<ul>
<li><p><span class="b">MapGet</span></p></li>
<li><p><span class="b">MapDelete</span></p></li>
<li><p><span class="b">MapPost</span></p></li>
<li><p><span class="b">MapPut</span></p></li>
<li><p><span class="b">MapVerb</span></p></li>
</ul>
<p>По окончанию названия уже можно предположить, для какого типа запроса данный метод предназначен. Все они, кроме MapVerb, имеют следующие две формы:</p>
<pre class="brush:c#;">
...
<p>А метод <code>MapVerb()</code> добавляет в качестве первого параметра название типа запроса http, например, "GET" или "POST":</p>
<pre class="brush:c#;">
...
<h3>Установка middleware для обработки запросов</h3>
<p>Еще ряд методов позволяют установить для обработки запросов middleware в виде делегата <span class="b">Action&lt;IApplicationBuilder app&gt;</span>. Это:</p>
<ul>
<li><p><span class="b">MapMiddlewareGet</span></p></li>
<li><p><span class="b">MapMiddlewareDelete</span></p></li>
<li><p><span class="b">MapMiddlewarePost</span></p></li>
<li><p><span class="b">MapMiddlewarePut</span></p></li>
<li><p><span class="b">MapMiddlewareRoute</span></p></li>
<li><p><span class="b">MapMiddlewareVerb</span></p></li>
</ul>
<p>Суффикс Get/Delete/Post/Put на конце имени метода указывает, какой именно тип запроса обрабатывается методом. MapMiddlewareRoute является универсальным, 
а MapMiddlewareVerb позволяет указать тип запроса в качестве первого параметра.</p>
<p>Например, применим MapMiddlewareGet:</p>
<pre class="brush:c#;">
public class Startup
{
	public void Configure(IApplicationBuilder app)
...
<p>Все методы, за исключением MapMiddlewareVerb, принимают  те же параметры, что и MapMiddlewareGet:</p>
<pre class="brush:c#;">MapMiddlewareGet()</pre>
<p>MapMiddlewareVerb дополнительно в качестве первого параметра принимает тип запроса.</p>
...
<div class="nav"><p><a href="./11.12.php">Назад</a><a href="./">Содержание</a><a href="./11.4.php">Вперед</a></p></div></div>
<div class="socBlock">
