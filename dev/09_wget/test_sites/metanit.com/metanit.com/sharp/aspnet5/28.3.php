...
<html >
<head>
<title>ASP.NET Core | Локализация строк. IStringLocalizer</title>
<meta charset="utf-8" />
<meta name="description" content="Локализация строк с помощью сервиса IStringLocalizer в ASP.NET Core, динамическая локализация в контроллере">
...
<h2>Локализация строк. IStringLocalizer</h2><div class="date">Последнее обновление: 18.12.2019</div>
...
<p>В прошлых темах было рассмотрено, как получать и устанавливать текущую языковую культуру. И теперь посмотрим, как мы можем в зависимости от культуры 
локализовать определенные значения.</p>
<p>ASP.NET Core предоставляет ряд встроенных сервисов, которые автоматически позволяют локализовать строки. И одним из таких сервисов является 
<span class="b">IStringLocalizer</span> и <span class="b">IStringLocalizer&lt;T&gt;</span>. Чтобы детальнее понять, как работает этот сервис, определим его реализацию. 
Для этого добавим в проект новый класс, который назовем CustomStringLocalizer:</p>
<pre class="brush:c#;">
...
<p>Этот класс реализует интерфейс IStringLocalizer. Для демонстрации его работы реализован только индексатор, который по некоторому ключу возвращает 
объект <span class="b">LocalizedString</span>.</p>
<p>Для простоты здесь определены словари для трех языков, каждом из которых по два ресурса HEADER и MESSAGE. Затем все три словаря объединяются в один общий словарь ресурсов.</p>
<p>В реализованном индексаторе по текущей культуре получаем нужный языковой словарь, а из этого словаря по ключу name извлекаем нужное значение. В конце по ключу и значению создается объект LocalizedString.</p>
<p>Встроим сервис в конвейер ASP.NET Core. Для этого изменим класс <span class="b">Startup</span>:</p>
<pre class="brush:c#;">
using System.Globalization;
...
<p>Сервис IStringLocalizer устанавливается в методе ConfigureServices, а в методе Configure вызывается рассмотренный в прошлой теме компонент RequestLocalizationMiddleware.</p>
<p>Используем сервис в контроллере HomeController:</p>
<pre class="brush:c#;">
using System;
...
<p>Для тестирования сервиса в методе Index в представление через объект ViewData передаются два значения.</p>
<p>И определим простейшее представление для метода Index для вывода этих значений:</p>
<pre class="brush:xml;">
&lt;h1&gt;@ViewData["Title"]&lt;/h1&gt;
&lt;h3&gt;@ViewData["Message"]&lt;/h3&gt;
</pre>
<p>Запустим приложение и вначале посредством параметра culture через сроку запроса установим одну культуру:</p>
<img src="./pics/localization4.png" alt="IStringLocalizer в ASP.NET Core" />
<p>Затем установим другую культуру:</p>
<img src="./pics/localization5.png" alt="Сервис IStringLocalizer в ASP.NET Core MVC" />
<p>Таким образом, при изменении культуры сервис IStringLocalizer автоматически будет выбирать нужный ресурс.</p>
...
<div class="nav"><p><a href="./28.2.php">Назад</a><a href="./">Содержание</a><a href="./28.4.php">Вперед</a></p></div></div>
<div class="socBlock">
