...
<html >
<head>
<title>ASP.NET Core | Группы в SignalR</title>
<meta charset="utf-8" />
<meta name="description" content="Группы в SignalR в ASP.NET Core, добавление и удаление пользователей из групп">
...
<h2>Группы</h2><div class="date">Последнее обновление: 25.12.2019</div>
...
<p>Группы в SignalR представляют коллекцию подключений, которые ассоциированы с именем группы. SignalR позволяет добавлять и удалять клиентов из группы. 
Один клиент может входить сразу в несколько групп.</p>
<p>Для работы с группами в классе Hub определено свойство <span class="b">Groups</span>, которое представляет объект <span class="b">IGroupManager</span>. 
Он имеет два метода:</p>
<ul>
<li><p><span class="b">AddToGroupAsync(connectionId, groupName)</span>: добавляет клиента с идентификатором connectionId в группу с именем groupName</p></li>
<li><p><span class="b">RemoveFromGroupAsync(connectionId, groupName)</span>: удаляет клиента с идентификатором connectionId из группы</p></li>
</ul>
<p>Также у объекта IHubCallerClients определено ряд методов, которые позволяют выбрать определенные группы для отправки им сообщения.</p>
...
<p>В хабе определено два метода. В методе Enter обратившегося клиента добавляют в группу "cats". Имя групп может быть любым, важно только учитывать, что оно 
регистрозависимое. В методе Send ретранслируют сообщение клиента всем участникам группы cats.</p>
<p>Для вызова метода у клиентов в группах можно использовать ряд методов:</p>
<ul>
...
<p>Для тестирования можно определить следующую веб-страницу:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<img src="./pics/signalr16.png" alt="Группы в SignalR в ASP.NET Core" />
<p>При работе с группами стоит учитывать, группа никак не связана с аутентификацией и авторизацией. Например, в примере хаба выше любой подключившийся к 
...
Группа в SignalR - это не более чем условное объединение клиентов, подключенных к хабу. И мы не можем узнать, какие пользователи состоят в той или иной группе, получить всех пользователей определенной группы.</p>
<p>И если, к примеру, необходимо, создавать какие-нибудь групповые чаты, где могут послать сообщения и соответственно видеть эти сообщения только участники данного чата, 
необходимо использовать стандартные механизмы авторизации и аутентификации. Например:</p>
<pre class="brush:c#;">
...
<p>В данном случае в группу "admins" могут посылать сообщения только авторизованные пользователи, у которых роль - "admin".</p>
...
<div class="nav"><p><a href="./30.10.php">Назад</a><a href="./">Содержание</a><a href="./30.12.php">Вперед</a></p></div></div>
<div class="socBlock">
