...
<html >
<head>
<title>ASP.NET Core | Загрузка файлов на сервер</title>
<meta charset="utf-8" />
<meta name="description" content="Загрузка файлов на сервер в ASP.NET Core, сохранение файлов в базе данных и в файловой системе, отправка коллекции файлов">
...
<h2>Загрузка файлов на сервер</h2><div class="date">Последнее обновление: 29.12.2019</div>
...
<p>Рассмотрим, как загружать файлы на сервер в ASP.NET Core. Все загружаемые файлы в ASP.NET Core представлены типом <span class="b">IFormFile</span> из пространства имен <code>Microsoft.AspNetCore.Http</code>. 
Соответственно для получения отправленного файла в контроллере необходимо использовать IFormFile. Затем с помощью методов IFormFile мы можем произвести различные манипуляции 
 файлом - получит его свойства, сохранить, получить его поток и т.д. Некоторые его свойства и методы:</p>
<ul>
<li><p><code>ContentType</code>: тип файла</p></li>
<li><p><code>FileName</code>: название файла</p></li>
<li><p><code>Length</code>: размер файла</p></li>
<li><p><code>CopyTo/CopyToAsync</code>: копирует файл в поток</p></li>
<li><p><code>OpenReadStream</code>: открывает поток файла для чтения</p></li>
</ul>
<p>При работе с файлами прежде всего следует выбрать способ сохранения файлов. Существует два способа  - в бд и в файловой системе. В данном случае рассмотрим оба способа на примере проекта 
ASP.NET Core <span class="b">MVC</span>.</p>
<h3>Сохранение в файловой системе</h3>
<p>Определим простую модель FileModel:</p>
<pre class="brush:c#;">
...
<p>Свойство Name здесь хранит имя файла, а свойство Path - путь к файлу в файловой системе.</p>
<p>Определим контекст данных ApplicationContext:</p>
<pre class="brush:c#;">
...
<p>Для хранения файлов создадим в проекте в каталоге <span class="b">wwwroot</span> папку <span class="ii">Files</span>.</p>
<p>Далее определим в контроллере методы, которые будет получать и выводить полученные файлы в виде картинок (предполагается, что загружаются изображения):</p>
<pre class="brush:c#;">
using System.Linq;
...
<p>Для получения полного пути к каталогу wwwroot применяется свойство <span class="b">WebRootPath</span> объекта IWebHostEnvironment. Для копирования файла 
в папку Files создается поток FileStream, в который записывается файл с помощью метода <code>CopyToAsync</code>.</p>
<p>Затем изменим представление <span class="ii">Index.cshtml</span>:</p>
<pre class="brush:xml;">
...
<p>Благодаря установке атрибута формы <code>enctype="multipart/form-data"</code> браузер будет знать, что вместе с формой надо передать файл.</p>
<p>После формы идет отображение файлов с помощью тега img на веб-странице.</p>
<h3>Множественная загрузка файлов</h3>
<p>При множественной загрузке файлов создаем в представлении набор элементов с типом file с одинаковым значением атрибута name:</p>
<pre class="brush:xml;">
&lt;form asp-action="AddFile" asp-controller="Home" method="post" enctype="multipart/form-data"&gt;
...
<p>Теперь в метода контроллера мы уже будем получать коллекцию объектов IFormFile - IFormFileCollection:</p>
<pre class="brush:c#;">
[HttpPost]
...
<h3>Загрузка в базу данных</h3>
<p>Пусть у нас есть следующая модель Person:</p>
<pre class="brush:c#;">
public class Person
{
    public int Id { get; set; }
...
<p>Для простоты примера в модели Person всего три свойства: id, имя пользователя и массив байтов файла, который будет храниться в бд и который будет представлять аватар пользователя. 
В базе данных свойство Avatar будет сопоставляться со столбцом с типом <code>varbinary(max)</code>.</p>
<p>Пусть у нас будет следующий контекст данных FilesContext:</p>
<pre class="brush:c#;">
...
<p>Для создания модели Person определим вспомогательную модель PersonViewModel:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Http;
...
<p>Далее определим следующее представление <span class="ii">Index.cshtml</span>:</p>
<pre class="brush:xml;">
...
<p>Опять же для упрощения примера одно и то же представление будет выводить форму для добавления нового объекта и список имеющихся объектов.</p>
<p>При выводе списка объектов, если файл представляет изображение, то мы можем с помощью метода <code>Convert.ToBase64String</code> преобразовать массив байтов 
в нужный формат для вывода изображения.</p>
<p>В итоге все будет выглядеть примерно так:</p>
<img src="./pics/uploadfiles.png" alt="Upload files in ASP.NET Core" />
<p>Далее изменим контроллер HomeController:</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Mvc;
using System.Linq;
using FileUploadApp.Models;
...
<p>В данном случае объект IFormFile с помощью класса BinaryReader считывается в массив байтов, который мы затем можем сохранить в базу данных.</p>
...
$(function() { 
  ...
  $("#toggle-theme").on("click", function() {
	...
	$("li:contains('Глава 35.')").removeClass('expandable').addClass('collapsable').children('ul').css({"display":"block"});
	$("li:contains('Глава 35.')").children('div').removeClass('expandable-hitarea').addClass('collapsable-hitarea');
	$("li:contains('Глава 35.')").removeClass('lastExpandable').addClass('lastCollapsable').children('ul').css({"display":"block"});
	$("li:contains('Глава 35.')").children('div').removeClass('lastExpandable-hitarea').addClass('lastCollapsable-hitarea');
