...
<html >
<head>
<title>ASP.NET Core | Маршрутизация в MVC</title>
<meta charset="utf-8" />
<meta name="description" content="Маршрутизация в ASP.NET Core MVC, определение маршрутов и получение данных маршрута, объект RouteData">
...
<h1>Маршрутизация в ASP.NET Core MVC</h1><div class="date">Последнее обновление: 23.11.2019</div>
...
<p>В приложении ASP.NET Core MVC мы также можем использовать систему маршрутизации, которая работает в целом в ASP.NET Core и основные принципы которой 
были рассмотрены в предыдущих темах, в частности, <a href="https://metanit.com/sharp/aspnet5/11.1.php">здесь</a>. 
В частности, в рамках MVC мы также можем использовать два подхода к определению системы маршрутизации. Первый представляет определение и 
использование конечных точек с помощью компонентов EndpointRoutingMiddleware и EndpointMiddleware и встраивающих их методов UseRouting и UseEndpoints. Данный подход применяется по умолчанию, когда, 
например, мы создаем новый проект для MVC. Второй подход предполагает использование RouterMiddleware и метода UseMvc. Далее мы рассмотрим оба подхода.</p>
<h2>Маршрутизация в MVC с помощью конечных точек</h2>
<p>Чтобы задействовать систему маршрутизации на основе конечных точек в приложении MVC, нам надо добавить в классе Startup в методе <code>ConfigureServices()</code> сервисы фреймворка 
MVC и с помощью соответствующего middleware в методе <code>Configure()</code> определить как минимум один маршрут:</p>
<pre class="brush:c#;">
...
<p>Как и в целом в ASP.NET Core, в MVC по умолчанию для определения системы маршрутизации используется связка EndpointRoutingMiddleware и EndpointMiddleware. 
В частности, в классе Startup метод <code>app.UseRouting()</code> добавляет в конвейер обработки запроса компонент EndpointRoutingMiddleware, 
который определяет конечную точку, которая будет обрабатывать текущий запрос. А метод <code>app.UseEndpoints</code> встраивает компонент EndpointMiddleware, 
который определяет набор конечных точек, которые будут сопоставляться с определенными маршрутами и будут обрабатывать соответствующие маршрутам входящие запросы.</p>
<p>Метод <code>app.UseEndpoints</code> в качестве параметра принимает объект <span class="b">IEndpointRouteBuilder</span>, который добавляет 
конечные точки. Для добавления маршрута, который будет сопоставляться с конечными точками, применяется метод <span class="b">MapControllerRoute()</span>. 
Он определяет один маршрут:</p>
<pre class="brush:c#;">
endpoints.MapControllerRoute(
		name: "default",
		pattern: "{controller=Home}/{action=Index}/{id?}");
</pre>
<p>Параметр <code>name</code> определяет название маршрута - оно произвольно и в данное случае имеет значение "default". А параметр <code>pattern</code> 
определяет шаблон маршрута, с которым будут сопоставляться входящие маршруты.</p>
<p>Шаблон маршрута использует три параметра. Параметр "controller" будет сопоставляться по имени с одним из контроллеров приложения, а параметр "action" - с действием этого контроллера. 
Например, при запросе <span class="ii">http://localhost:3456/Home/Index</span> система выберет для обработки запроса контроллер Home - имя контроллера без префикса Controller и его 
действие Index.</p>
<p>Данный тип маршрутизации еще называют <span class="b">Convention-Based Routing</span>, то есть маршрутизация, основанная на условностях в определениях маршрутов.</p>
<h3>Добавление маршрутов</h3>
<p>Для добавления маршрутов в MVC мы можем применять следующие методы IEndpointRouteBuilder:</p>
<ul>
<li><p><span class="b">MapControllerRoute()</span> определяет произвольный маршрут и принимает следующие параметры:</p>
<pre class="brush:c#;">MapControllerRoute(string name, string pattern, [object defaults = null], 
[object constraints = null], [object dataTokens = null])</pre>
<p>Его параметры:</p>
<ul>
<li><p><code>name</code>: название машрута</p></li>
<li><p><code>pattern</code>: шаблон машрута</p></li>
<li><p><code>defaults</code>: значения параметров маршрутов по умолчанию</p></li>
<li><p><code>constraints</code>: ограничения маршрута</p></li>
<li><p><code>dataTokens</code>: определения токенов маршрута</p></li>
</ul>
<p>Первые два параметра являются обязательными, остальные необязательные.</p>
</li>

<li><p><span class="b">MapDefaultControllerRoute()</span> определяет стандартный маршрут, фактически эквивалентен вызову</p>
<pre class="brush:c#;">
endpoints.MapControllerRoute(
	name: "default",
	pattern: "{controller=Home}/{action=Index}/{id?}");</pre>
<p>Поскольку это довольно часто используемое определение маршрута, то для него и был введен отдельный метод.</p>
</li>
<li><p><span class="b">MapAreaControllerRoute()</span> определяет маршрут, который также учитывает область приложения. Имеет следующие параметры:</p>
<pre class="brush:c#;">MapAreaControllerRoute(string name, string areaName, string pattern, [object defaults = null], 
[object constraints = null], [object dataTokens = null])</pre>
<p>Обязательный параметр <code>areaName</code> позволяет определить область, с которой будет связан маршрут.</p>
</li>
<li><p><span class="b">MapControllers()</span> сопоставляет действия контроллера с запросами, используя маршрутизацию на основе атрибутов. Про атрибуты маршрутизации 
будет сказано в последующих статьях.</p></li>
<li><p><span class="b">MapFallbackToController()</span> определяет действие контроллера, которое будет обрабатывать запрос, если все остальые определенные маршруты не соответствуют запросу. 
Принимает имя контроллера и его метода:</p>
<pre class="brush:c#;">MapFallbackToController(string action, string controller)</pre></li>
</ul>
<p>Определение шаблонов маршрутов, использование ограничений, необязательных параметров и параметров по умолчанию в MVC будет происходить также, как и уже было рассмотрено на примере ASP.NET Core в целом. 
Единственное существенное отличие состоит в том, что MVC добавляет поддержку атрибутов маршрутизации, которые мы далее рассмотрим.</p>
<h3>Получение параметров маршрутов в контроллере</h3>
<p>Мы можем получить в контроллере все параметры маршрута, используя объект <span class="b">RouteData</span>:</p>
<pre class="brush:c#;">
...
<div class="nav"><p><a href="./7.10.php">Назад</a><a href="./">Содержание</a><a href="./11.13.php">Вперед</a></p></div></div>
<div class="socBlock">
