...
<html >
<head>
<title>ASP.NET Core | Добавление методов контроллера и представлений в MVC</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление методов контроллера и представлений в проект ASP.NET Core MVC">
...
<h2>Добавление методов контроллера и представлений</h2><div class="date">Последнее обновление: 16.11.2019</div>
...
<p>В прошлой теме мы создали  функционал для вывода объектов на веб-страницу, чтобы пользователь смог увидеть все товары и их информацию. 
Теперь добавим функциональность для оформления заказов. В прошлой теме у нас была определена ссылка на метод Buy контроллера HomeController, который должен 
выполнять функции по покупке товара. Поэтому добавим в контроллер HomeController следующие методы:</p>
<pre class="brush:c#;">
[HttpGet]
...
<p>Хотя мы добавили два метода, но в целом они составляют одно действие Buy. С помощью атрибутов <code>[HttpGet]</code> и <code>[HttpPost]</code> мы можем указать, какой метод какой тип запроса 
...
второй - при получении запроса POST.</p>
<p>Так как предполагается, что в метод Buy будет передаваться id смартфона, который пользователь хочет купить, то нам надо определить в методе соответствующий 
параметр: <code>public IActionResult Buy(int? id)</code>.</p>
<p>Если в метод вдруг не передано id смартфона, хотя сам метод вызван, в этом случае с помощью вызова <code>RedirectToAction("Index")</code> выполняем переадресацию на метод Index, который выводит список товаров.</p>
<p>Чтобы передать id смартфона в представление применяется объект ViewBag.</p>
<p><span class="b">ViewBag</span> представляет такой объект, который позволяет определить любую переменную и передать ей некоторое значение, а затем в 
представлении извлечь это значение. Так, мы определяем переменную ViewBag.PhoneId, которая и будет хранить id выбранного смартфона.</p>
<p>Теперь добавим представление для этого метода, которое должно выводить форму для оформления заказа. Для этого нажмем правой кнопкой мыши на папку <i>Views/Home</i> 
и в контекстном меню выберем <span class="b">Add -&gt; New Item</span>:</p>
<img src="./pics/mvc12.png" alt="Добавление представление в проект ASP.NET MVC Core" />
<p>И затем в открывшемся окне добавления нового элемента выберем <span class="b">Razor View</span> и назовем новый элемент <span class="b">Buy.cshtml</span>, так как 
по умолчанию названия представлений (не считая расширения файла cshtml) должны соответствовать названию вызывающих их методов:</p>
<img src="./pics/mvc13.png" alt="Добавление в проект ASP.NET Core элемента MVC View Page" />
<p>Созданное по умолчанию представление изменим следующим образом:</p>
<pre class="brush:xml;">
@{
    Layout = null;
}
...
<p>Данное представление представляет собой форму для ввода данных. И так как в методе мы установили переменную во ViewBag, то в представлении мы ее можем получить. 
Поскольку нам не надо менять ее значение, то мы его вкладываем в скрытое поле в начале формы.</p>
<p>В итоге при переходе на главной странице по ссылке "/Home/Buy/1" контроллер будет получать запрос к действию Buy, передавая ему в качестве параметра id значение 1. 
И так как такой запрос представляет тип GET, пользователю будет возвращаться данное представление с формой.</p>
<p>После заполнения формы и нажатия на кнопку форма будет оправляться запросом POST, так как мы его определили в строке 
<code>&lt;form method="post"&gt;</code>. Контроллер снова будет получать запрос к действию Buy, но теперь для обработки запроса будет выбираться метод 
<code>public string Buy(Order order)</code>.</p>
<p>Несмотря на то, что на форме у нас простые поля ввода, в post-версии метода Buy мы получаем именно модель Order. 
Как система MVC угадывает, что мы передали с запросом post информацию о модели Order, а не набор разрозненных значений полей формы? 
Обратите внимание на поля ввода:</p>
<pre class="brush:xml;">
&lt;input type="hidden" value="@ViewBag.PhoneId" name="PhoneId" /&gt;
...
<p>Значение атрибута name у этих полей ввода соответствует названию одного из свойств модели Order. После нажатия кнопки и отправки запроса приложение 
получает значения этих полей. Для связывания приходящих на сервер данных с параметрами методов фреймворк MVC использует специальный компонент 
<span class="b">привязчик модели</span> (model binder), который по названию пришедших данных сопоставит их со свойствами модели Order и создаст объект этой модели.</p>
<p>Снова запустим проект. На главной странице выберем какой-нибудь смартфон и нажмем на ссылку "Купить". На форме заполним поля и нажмем кнопку "Отправить".</p>
<img src="./pics/mvc14.png" alt="Отправка данных в ASP.NET MVC Core" />
<p>После нажатия кнопки информация о покупке попадет в базу данных, а в браузер отобразит уведомление:</p>
<img src="./pics/mvc15.png" alt="Обработка данных в ASP.NET MVC Core" />
