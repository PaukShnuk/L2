...
<html >
<head>
<title>ASP.NET Core | Глобальная и локальная настройка CORS</title>
<meta charset="utf-8" />
<meta name="description" content="Глобальная и локальная установка политик CORS в ASP.NET Core для отдельных конечных точек и маршрутов, метод RequireCors">
...
<h2>Глобальная и локальная настройка CORS</h2><div class="date">Последнее обновление: 22.12.2019</div>
...
<p>Если мы используем в приложении для создания системы маршрутизации метод <code>app.UseEndpoints()</code>, 
...
CORS, либо конкретизировать для каждого ресурса свои настройки.</p>
<p>В прошлых темах использовалась глобальная настройка CORS, которая предполагает, что настройки CORS передаются в метод <code>app.UseCors()</code>:</p>
<pre class="brush:c#;">
...
<p>С помощью метода <span class="b">RequireCors()</span> у объекта IEndpointConventionBuuilder мы можем установить настройки CORS для каждого конкретного маршрута. Данный метод 
в качестве параметра принимает либо название политики CORS, либо делегат с параметром CorsPolicyBuilder, с помощью который устанавливается конфигурацию CORS.</p>
<pre class="brush:c#;">
using Microsoft.AspNetCore.Builder;
using Microsoft.AspNetCore.Http;
using Microsoft.Extensions.DependencyInjection;
...
<p>Таким образом, мы можем определить для каждого свою конфигурацию CORS.</p>
...
<div class="nav"><p><a href="./31.5.php">Назад</a><a href="./">Содержание</a><a href="./31.4.php">Вперед</a></p></div></div>
<div class="socBlock">
