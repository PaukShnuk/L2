...
<html >
<head>
<title>ASP.NET Core | Создание проекта юнит-тестов. Добавление xUnit</title>
<meta charset="utf-8" />
<meta name="description" content="Создание проекта юнит-тестов в ASP.NET Core, добавление в проект фреймворка xUnit">
...
<h2>Создание проекта юнит-тестов. Добавление xUnit</h2><div class="date">Последнее обновление: 21.12.2019</div>
...
<p>Создадим новый проект ASP.NET Core по типу ASP.NET Core Web App (Model-View-Controller):</p>
<img src="./pics/unittest1.png" alt="Юнит-тесты в ASP.NET Core" />
<p>Пусть проект будет называться UnitTestApp.</p>
<p>Теперь для него добавим проект юнит-тестов.</p>
<p>Согласно условностям название проекта юнит-тестов называется <span class="ii">[название_приложения].Tests</span>.
Итак, добавим в <span class="b">решение</span> новый проект по типу <span class="b">Class Library (.NET Core)</span>:</p>
<img src="./pics/unittest2.png" alt="Добавление проекта юнит-тестов в ASP.NET Core" />
<p>Назовем его <span class="b">UnitTestApp.Tests</span></p>
<img src="./pics/unittest14.png" alt="Добавление проекта юнит-тестов в ASP.NET Core" />
<p>Стоит отметить, что начиная с Visual Studio 2019 специально для тестов были добавлены три типа проектов: 
<span class="b">xUnit Test Project(.NET Core)</span>, <span class="b">MSTest Test Project(.NET Core)</span> и 
<span class="b">NUnit Test Project(.NET Core)</span>, которые для тестирования используют соответственно фреймворк xUnit, MSTest Framework и NUnit. 
В реальности разница между ними небольшая. И если мы используем VS 2019, то сразу можем выбрать один из этих проектов. Однако в данном случае мы возьмем 
именно проект библиотеки классов и вручную добавим в нее все нужные зависимости.</p>
<p>В итоге после создания проекта все решение будет иметь следующую структуру:</p>
<img src="./pics/unittest3.png" alt="Тестирование в ASP.NET Core MVC" />
<p>Удалим из проекта тестов созданный по умолчанию файл Class1.cs, так как он нам не нужен.</p>
<p>Теперь добавим фреймворк юнит-тестов. Существует множество фреймворков. В данном случае мы будем использовать 
<a href="https://xunit.net/" rel="nofollow">xUnit</a> как наиболее распространенный фреймворк в связке с ASP.NET Core (хотя можно использовать и другие фреймворки, 
которые более предпочтительны).</p>
<p>Чтобы использовать xUnit, в проект тестов (то есть UnitTestApp.Tests) необходимо добавить через NuGet три пакета:</p>
<ul>
<li><p><span class="b">Microsoft.NET.Test.Sdk</span></p></li>
<li><p><span class="b">xunit</span>: функционал фреймворка тестирования</p></li>
<li><p><span class="b">xunit.runner.visualstudio</span>: обеспечивает интеграцию с Visual Studio</p></li>
</ul>
<img src="./pics/unittest10.png" alt="Добавление xunit в ASP.NET Core" />
<p>Причем вначале лучще добавлять Microsoft.NET.Test.Sdk, так как он содержит ряд зависимостей,используемых в xunit.runner.visualstudio.</p>
<p>Как было отмечено выше, в Visual Studio 2019 мы могли бы выбрать для создания проекта тестов шаблон <span class="b">xUnit Test Project(.NET Core)</span> - он по 
умолчанию включает все выше описанные пакеты.</p>
<p>После добавления этих пакетов также в проекте тестов необходимо добавить ссылку на главный проект:</p>
<img src="./pics/unittest11.png" alt="Добавление проекта тестов в ASP.NET Core" />
<p>Таким образом, проект тестов настроен, и мы можем приступать к тестированию.</p>
