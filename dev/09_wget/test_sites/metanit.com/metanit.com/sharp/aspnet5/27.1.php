...
<html >
<head>
<title>ASP.NET Core | Создание проекта и подключение MongoDB</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение MongoDB в ASP.NET Core, создание репозитория, драйвер для языка программирования C# для Mongo, основные операции с базой данных, IMongoCollection">
...
<h1>Работа с MongoDB</h1><h2>Создание проекта и подключение MongoDB</h2><div class="date">Последнее обновление: 29.12.2019</div>
...
<p>Одной из наиболее популярных систем баз данных является MongoDB. Рассмотрим, как мы можем взаимодействовать с MongoDB в рамках ASP.NET Core.</p>
<p>И прежде всего надо отметить, что предварительно должен быть установлен сервер MongoDB. О процессе установки можно прочитать в статье 
<a href="https://metanit.com/nosql/mongodb/1.2.php">Начало работы с MongoDB</a>.</p>
<p>Итак, создадим новый проект ASP.NET Core по типу Web Application (Model-View-Controller), который назовем <span class="b">MongoApp</span>.</p>
<img src="./pics/mongo1.png" alt="MongoDB и ASP.NET Core" />
<p>Для взаимодействия с сервером Mongo необходимо установить драйвер. Для этого перейдем к пакетному менеджеру Nuget и через него установим пакет 
<span class="b">MongoDB.Driver</span>. Кроме того, для работы с файлами потребуется установить пакет <span class="b">MongoDB.Driver.GridFS</span/>:</p>
<img src="./pics/mongo2.png" alt="MongoDB.Driver для ASP.NET Core" />
<p>Вначале создадим модель, которая будет представлять данные. Для этого добавим в проект в папку <span class="b">Models</span> новый класс 
Product:</p>
<pre class="brush:c#;">
using MongoDB.Bson;
...
<p>В MongoDB уникальный идентификатор документа имеет тип <span class="b">ObjectId</span>, и по умолчанию для него в документе выделяется поле с именем "_id". 
И чтобы связать свойство Id класса Product с этим полем, над данным свойством установлен атрибут <span class="b">[BsonRepresentation(BsonType.ObjectId)]</span>.</p>
<p>Весь код взаимодействия с базой данных вынесем в отдельный класс. Для этого добавим в папку Models новый класс <span class="b">ProductService</span> 
...
<p>Здесь представлены все стандартные методы CRUD, а также методы управления изображениями. Ключевыми объектами в ProductService являются 
IMongoDatabase (представляет базу данных) и IGridFSBucket (представляет файловое хранилище GridFS).</p>
<p>База данных в MongoDB состоит из коллекций. Все объекты Product будут размещатся в коллекции Products. Для удобного доступа к этой коллекции в 
классе определено одноименное приватная переменнаяя <code>Products</code>, которая представляет объект <span class="b">IMongoCollection</span>.</p>
<p>Метод <code>GetProducts()</code> возвращает все документы - объекты Product из базы данных. Кроме того, он позволяет отфильтровать выборку, используя три переданных в метод параметра - минимальную и максимальную цену и название модели. 
Для этого создается набор фильтров, которые объединяются в один объект FilterDefinition. Затем этот фильтр передается в метод Find:</p>
<pre class="brush:c#;">return await Products.Find(filter).ToListAsync();</pre>
<p>Метод <code>GetProduct()</code> возвращает только один объект Product по id. Для этого также у коллекции вызывается метод Find, только в качестве критерия выборки служит значение 
поля _id:</p>
<pre class="brush:c#;">return await Products.Find(new BsonDocument("_id", new ObjectId(id))).FirstOrDefaultAsync();</pre>
<p>Для фильтрации документов создается объект <span class="b">BsonDocument</span>. Он принимает два параметра: название поля, по которому вести фильтрацию, 
- это поле "_id", и значение, которое должно имеь поле, - объект ObjectId.</p>
<p>Метод <code>Create()</code> добавляет данные в коллекцию и для этого вызывает у нее метод <span class="b">InsertOneAsync()</span>, передавая в него добавляемый объект:</p>
<pre class="brush:c#;">await Products.InsertOneAsync(p);</pre>
<p>Метод <code>Update()</code> обновляет один документ в коллекции, передавая в метод <span class="b">ReplaceOneAsync()</span> критерий фильтрации - объект ObjectId 
и заменяемый объект:</p>
<pre class="brush:c#;">await Products.ReplaceOneAsync(new BsonDocument("_id", new ObjectId(p.Id)), p);</pre>
<p>И метод <code>Remove()</code> удаляет документ по id:</p>
<pre class="brush:c#;">await Products.DeleteOneAsync(new BsonDocument("_id", new ObjectId(id)));</pre>
<p>Методы <code>StoreImage()</code> и <code>GetImage()</code> сохраняют и возвращают изображение соответственно. Они работают с объектом IGridFSBucket. 
StoreImage загружает данные из потока через вызов метода <span class="b">gridFS.UploadFromStreamAsync()</span>, возвращая идентификатор загуженного файла. 
А GetImage, наоборот, извлекает данные в виде массива байтов посредством функции <span class="b">gridFS.DownloadAsBytesAsync(new ObjectId(id));</span>, используя идентификатор файла.</p>
<p>Более подробно весь этот функционал и все возможности по взаимодействию программы на C# с базой данных на MongoDB был описан в материале <a href="https://metanit.com/nosql/mongodb/4.1.php">C# и MongoDB</a>.</p>
<p>И далее добавим ProductService в качестве сервиса в конвейер ASP.NET Core, используя механизм внедрения зависимостей. Для этого изменим класс Startup:</p>
<pre class="brush:c#;">
...
<p>Для внедрения сервиса <code>ProductService</code> в методе <code>ConfigureServices()</code> осуществляется вызов <code>services.AddTransient&lt;ProductService&gt;();</code>.</p>
<p>Теперь создадим основной функционал приложения.</p>
...
<div class="nav"><p><a href="./26.1.php">Назад</a><a href="./">Содержание</a><a href="./27.2.php">Вперед</a></p></div></div>
<div class="socBlock">
...
$(function() { 
  ...
  $("#toggle-theme").on("click", function() {
	...
	$("li:contains('Глава 32.')").removeClass('expandable').addClass('collapsable').children('ul').css({"display":"block"});
	$("li:contains('Глава 32.')").children('div').removeClass('expandable-hitarea').addClass('collapsable-hitarea');
