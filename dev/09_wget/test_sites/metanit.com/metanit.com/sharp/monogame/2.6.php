...
<html >
<head>
<title>Обработка нажатий клавиатуры в MonoGame</title>
<meta charset="utf-8" />
...
<h2>Обработка нажатий клавиатуры</h2><div class="date">Последнее обновление: 14.11.2015</div>
...
<p>В предыдущих темах рассматривалось автоматическое движение спрайта. Теперь добавим возможность управлять спрайтом с помощью клавиатуры.</p>
<p>Для взаимодействия с клавиатурой в MonoGame имеется класс <span class="b">Keyboard</span>, который определен в пространстве имен 
<span class="b">Microsoft.Xna.Framework.Input</span>.</p>
<p>Класс Keyboard определяет статический метод <span class="b">GetState()</span>, который возвращает текущее состояние клавиатуры в виде объекта 
<span class="b">KeyboardState</span>. Эта структура содержит три ключевых метода, которые позволяют нам определить, нажата ли определенная клавиша:</p>
<ul>
<li><p><code>GetPressedKeys()</code>: возвращает массив нажатых клавиш в виде объекта Key[]</p></li>
<li><p><code>IsKeyDown(Keys key)</code>: возвращает true, если нажата определенная клавиша, иначе возвращает false</p></li>
<li><p><code>IsKeyUp(Keys key)</code>: возвращает true, если была отпущена определенная клавиша</p></li>
</ul>
<p>По умолчанию при создании проекта в коде уже задействуется этот класс - в методе Update:</p>
<pre class="brush:c#;">
...
<p>То есть если нажата клавиша Escape, то осуществляется выход из приложения.</p>
<p>Таким образом, нам надо в методе Update просто инспектировать состояние клавиатуры на нажатие клавиши, и если определенная клавиша нажата, 
то производить какие-либо действия, например, изменение координаты отрисовки спрайта.</p>
<p>Возьмем обычный какой-нибудь обычный спрайт и определим следующий код приложения:</p>
<pre class="brush:c#;">
...
<p>Основную работу выполняет следующий участок кода:</p>
<pre class="brush:c#;">
if (keyboardState.IsKeyDown(Keys.Left))
    position.X -= speed;
if (keyboardState.IsKeyDown(Keys.Right))
...
<p>Если нажата определенная клавиша, то происходит увеличение или уменьшение соответствующей координаты текущей позиции спрайта.</p>
