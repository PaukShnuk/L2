...
<html >
<head>
<title>Перемещение объектов в MonoGame</title>
<meta charset="utf-8" />
...
<h2>Перемещение объектов</h2><div class="date">Последнее обновление: 14.11.2015</div>
...
<p>Перемещение спрайтов осуществляется довольно просто. Необходимо лишь перерисовать спрайт на новой позиции, и у пользователя возникнет 
иллюзия движения. Определим следующий код игры:</p>
<pre class="brush:c#;">
...
<p>Здесь добавляются две дополнительных переменных: <code>Vector2 position</code> для хранения текущей позиции спрайта и 
<code>float speed</code> для шага перемещения.</p>
<p>Поскольку за обновление состояния игры отвечает метод <code>Update()</code>, который, как и метод <code>Draw()</code>, вызывается 
60 раз в секунду, то в Update производим изменение переменной <code>position</code> на значение переменной <code>speed</code>. В методе <code>Draw()</code> данная переменная будет использоваться для отрисовки. 
Если спрайт достиг границ окна, то меняет направление движения на противоположное.</p>
