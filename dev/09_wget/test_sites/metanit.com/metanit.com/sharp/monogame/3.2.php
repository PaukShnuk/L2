...
<html >
<head>
<title>Звуковые эффекты в MonoGame</title>
<meta charset="utf-8" />
...
<h2>Звуковые эффекты</h2><div class="date">Последнее обновление: 14.11.2015</div>
...
<p>Еще один способ введения аудио в приложение представляют звуковые эффекты, которые представлены классом SoundEffect из пространства имен 
Microsoft.Xna.Framework.Audio.</p>
<p>Первым делом добавим нужный аудиофайл в проект:</p>
<img src="pics/2.19.png" alt="SoundEffect in Monogame" />
<p>Здесь также можно добавить mp3-файл, однако надо учитывать, что для mp3-файла по умолчанию в опции Processor устанавливается значение 
<code>Song - MonoGame</code>, что позволяет воспроизводить файл как объект Song с помощью MediaPlayer (как было рассмотрено в прошлой теме). 
Однако поскольку в данном случае мы будем работать с этим файлом как со звуковым эффектам, то нам надо поменять значение этой опции 
на <span class="b">Sound Effect - MonoGame</span>.</p>
<p>Теперь, допустим, пускай по нажатию клавиши S будет проигрываться добавленный звук:</p>
<pre class="brush:c#;">
...
<p>Ключевым методом объекта SoundEffect является метод <code>Play()</code>, который позволяет воспроизвести файл. Однако кроме объекта 
SoundEffect можно использовать и производный объект SoundEffectInstance:</p>
<pre class="brush:c#;">
if (Keyboard.GetState().IsKeyDown(Keys.S))
{
...
<p>SoundEffectInstance дает больший контроль над воспроизведением аудио. Свойство IsLooped позволяет задать повторение мелодии после ее окончания. А свойство 
Volume позволяет установить звук с помощью величины из диапазона от 0.0f до 1.0f.</p>
<p>Методы <code>Play()</code> (начать воспроизведение), <code>Stop()</code> (остановить воспроизведение), 
<code>Pause</code> (приостановить), <code>Resume()</code> (возобновить воспроизведение) позволяют управлять проигрыванием звукового файла.</p>
<p>При этом мы можем использовать множество различных звуков в приложении. И если мы вдруг захотим задать для всех звуков одну и ту же громкость, 
то можем использовать статическое свойство <code>MasterVolume</code> класса SondEffect:</p>
<pre class="brush:c#;">SoundEffect.MasterVolume=0.5f;</pre>
<p>Данное свойство принимает значение типа float от 0.0f до 1.0f.</p>
