...
<html >
<head>
<title>Наложение спрайтов в MonoGame</title>
<meta charset="utf-8" />
...
<h2>Наложение спрайтов</h2><div class="date">Последнее обновление: 14.11.2015</div>
...
<p>Если у нас в приложении используется несколько текстур, то может возникнуть вопрос их наложения, если их границы пересекаются.</p>
<p>По умолчанию каждая новая текстура рисуется поверх предыдущей (если они пересекаются). Но мы можем это действие переопределить. 
Например, выведем одну и ту же текстуру два раза:</p>
<pre class="brush:c#;">
...
<p>Да последовательных метода <code>spriteBatch.Draw()</code> выведут два объекта. Второй объект в синим цвете будет перекрывать границы первого:</p>
<img src="pics/2.12.png" alt="Наложение объектов в MonoGame" />
<p>Теперь, допустим, нам надо, чтобы первый объект был поверх второго.</p>
<p>За z-порядок или порядок отрисовки спрайтов отвечает последний параметр метода <code>spriteBatch.Draw()</code>, который называется 
layerDepth и который принимает значение типа float от 0 до 1. 0 и 1 - это как бы крайние точки, между которыми располагаются объекты. 0 представляет передний план, а 1 - задний.</p>
<p>Чтобы изменить порядок спрайтов, также надо использовать перегруженные версии метода <span class="b">spriteBatch.Begin()</span>. 
Данный метод может принимать параметр типа <code>SpriteSortMode</code> - перечисление, в котором определены следующие значения:</p>
<ul><li><p><code>Deferred</code>: спрайты не отображаются до вызова <code>SpriteBatch.End</code>.
Затем все вызовы метода <code>spriteBatch.Draw()</code> направляются на графическое устройство в том порядке, в каком они были сделаны. Данный режим применяется по умолчанию.</p></li>
<li><p><code>Immediate</code>: метод <code>spriteBatch.Begin</code> сразу после вызова устанавливает настройки для графического устройства, 
а новые вызовы <code>spriteBatch.Draw()</code> сразу же выводят спрайты на экран также в том же порядке, в каком вызываются 
для них методы <code>spriteBatch.Draw()</code>.</p></li>
<li><p><code>Texture</code>: то же самое, что и режим <code>Deferred</code> за тем исключением, что производит сортировку спрайтов 
перед их отображением на экране.</p></li>
<li><p><code>BackToFront</code>: то же самое, что и режим <code>Deferred</code>, при этом спрайты располагаются по порядку в соответствии с указанным значением параметра layer depth в методе <code>Draw</code>.</p></li>
<li><p><code>FrontToBack</code>: то же самое, но теперь спрайты располагаются в обратном порядке по сравнению со значениями, указанными в параметре 
layer depth метода <code>spriteBatch.Draw()</code>.</p></li></ul>
<p>Теперь изменим метод <code>Draw()</code>:</p>
<pre class="brush:c#;">
protected override void Draw(GameTime gameTime)
{
    GraphicsDevice.Clear(Color.CornflowerBlue);
...
<p>Поскольку у первого объекта layerDepth равно 0, то он помещается на передний план. У второго объекта данное значение равно 1, поэтому он уходит на задний план.</p>
<img src="pics/2.13.png" alt="Наложение спрайтов в MonoGame" />
