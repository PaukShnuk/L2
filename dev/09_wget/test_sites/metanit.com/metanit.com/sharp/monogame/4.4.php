...
<html >
<head>
<title>Вращение, перемещение и масштабирование в MonoGame</title>
<meta charset="utf-8" />
...
<h2>Вращение, перемещение и масштабирование</h2><div class="date">Последнее обновление: 14.11.2015</div>
...
<p>При создании трехмерной сцены в прошлой теме мы рассмотрели, что за позицию объекта в пространстве отвечает мировая матрица, поэтому если 
нам надо переместить объект или повернуть, то соответственно нам надо применить некоторые математические преобразования именно к этой матрице.</p>
<p>К счастью нам не придется сильно погружаться в математические аспекты операций с матрицами, так как инфраструктура MonoGame уже предоставляет нам 
для этого готовые методы.</p>
<h3>Перемещение</h3>
<p>Для перемещения объекта по любой из осей мы можем применять метод <span class="b">Matrix.CreateTranslation(x, y, z)</span>. Этот метод принимает три параметра, 
которые указывают, насколько надо сместить объект по каждой и осей. Так, изменим код из прошлой темы и применим этот метод:</p>
<pre class="brush:c#;">
...
<p>Здесь был обновлен метод <code>Update()</code>, поскольку все вычисления в процессе работы программы должны идти именно в этом методе. 
В нем проверяется нажатие определенной клавиши (вверх, вниз, вправо, влево, клавиши S и W). И если клавиша нажата, к матрице worldMatrix 
применяется преобразование в виде смещения. Таким образом мы можем сделать перемещение. Также мы можем использовать другой способ: явно изменять 
определенную координату. Для этого нам надо вынести позицию объекта в отдельную переменную-вектор:</p>
<pre class="brush:c#;">
...
<p>А в методе Update по нажатию клавиши у этого вектора изменять соответствующее измерение:</p>
<pre class="brush:c#;">
protected override void Update(GameTime gameTime)
{
    //..........................
...
<h3>Вращение</h3>
<p>Для осуществления вращения мы можем применять методы <span class="b">Matrix.CreateRotationX(angle)</span>, 
<span class="b">Matrix.CreateRotationY(angle)</span> и <span class="b">Matrix.CreateRotationZ(angle)</span> для поворота вокруг осей X, Y и Z соответственно. 
Например, применим вращение вокруг оси Y:</p>
<pre class="brush:c#;">
using Microsoft.Xna.Framework;
using Microsoft.Xna.Framework.Graphics;
using Microsoft.Xna.Framework.Input;
...
<p>В данном случае в метод <code>Matrix.CreateRotationY()</code> передается угол в радианах, вычисляемый с помощью выражения <code>MathHelper.ToRadians(1)</code>, 
то есть фактически 1 градус.</p>
<img src="pics/4.5.png" alt="Перемещение и вращение в MomoGame" />
<h3>Обратная поверхность</h3>
<p>При повороте на 180 градусов мы не увидим треугольника, так как по умолчанию работает механизм "backface culling" или сокрытие обратной поверхности. 
Для увеличения производительности отображается только та часть графического примитива, которая повернута к камере.</p>
<p>Чтобы отключить данный режим, нам надо добавить в метод Initialize() следующие строки:</p>
<pre class="brush:c#;">
protected override void Initialize()
{
     //.......................
...
<p>Однако данный режим обычно не отключают в целях производительности.</p>
<h3>Масштабирование</h3>
<p>Для масштабирования объекта применяется метод <span class="b">Matrix.CreateScale()</span>. В качестве параметра он принимает величину, на которую надо увеличить 
объект:</p>
<pre class="brush:c#;">
...
<p>В данном случае по нажатию на клавишу + (плюс) будет происходить увеличение в 1.01 раз, а по нажатию на клавишу - (минус) - увеличение в 0.99, то есть 
фактически уменьшение.</p>
