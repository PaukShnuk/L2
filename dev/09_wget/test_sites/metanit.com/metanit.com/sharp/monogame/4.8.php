...
<html >
<head>
<title>Текстурирование в MonoGame</title>
<meta charset="utf-8" />
...
<h2>Текстурирование</h2><div class="date">Последнее обновление: 14.11.2015</div>
...
<p>Также, как и в обычной двухмерной игре, в трехмерной сцене мы также можем использовать текстуры. Только теперь наша задача будет состоять в том, чтобы совместить 
вершины текстуры с вершинами треугольника.</p>
<p>Вершины текстуры имеют следующие условные координаты:</p>
<img src="pics/4.12.jpg" alt="Texture coordinates in MonoGame" />
<p>То есть верхний левый угол текстуры будет иметь условно координаты (0,0), а правый нижний угол - (1,1). Точка в середине текстуры соответственно имеет координату (0.5,0.5). 
Таким образом, манипулируя координатами от 0.0 до 1.0 мы можем получить любую точку текстуры.</p>
<p>И если у нас треугольники строятся по следующим четырем точкам пространства (-1,1,1), (1,1,1), (1,-1,1) и (-1,-1,1), то мы получим примерно следующее наложение текстуры на поверхности треугольников:</p>
<img src="pics/4.13.png" alt="Texture mapping in MonoGame" />
<p>Теперь применим наложение текстуры в программе:</p>
<pre class="brush:c#;">
...
<p>Чтобы выполнить маппинг текстуры на поверхность трехмерного объекта, применяются объекты типа <span class="b">VertexPositionTexture</span>, 
которые принимают трехмерный вектор - координаты точки пространства и двухмерный вектор - координата текстуры.</p>
<p>Далее в методе <code>LoadContent()</code> загружается нужная текстура и настраивается эффект:</p>
<pre class="brush:c#;">
texture = Content.Load&lt;Texture2D&gt;("stone");
effect.TextureEnabled = true;
effect.Texture = texture;
</pre>
<p>В методе <code>Draw()</code> происходит отрисовка:</p>
<pre class="brush:c#;">
GraphicsDevice.DrawUserIndexedPrimitives&lt;VertexPositionTexture&gt;
	(PrimitiveType.TriangleList, verts, 0, 8, textureIndices, 0, 12);
</pre>
<p>И в результате мы получим текстурированный куб:</p>
<img src="pics/4.14.png" alt="Текстурированный куб в monogame" />
<p>Правда, в отличие от простого создания куба, как было показано в прошлой теме, здесь нам надо гораздо больше вершин, чтобы получить 
полностью текстурированный куб. Так как в данном случае оттекстурированы должным образом только четыре треугольника, которые составляют лицевую и заднюю сторону куба.</p>
