...
<html >
<head>
<title>Вопросы по C# | Интерфейсы</title>
<meta charset="utf-8" />
<meta name="description" content="Проверка знаний и вопросы по теме Интерфейсы, наследование и реализация интерфейсов в C#">
...
<h2>Интерфейсы</h2><div class="date">Последнее обновление: 18.07.2018</div>
...
<p>Что неправильно в определении следующих интерфейсов:</p>
<pre class="brush:c#;">
...
<p>Варианты ответов (ответов может быть несколько):</p>
<ul>
<li><p>Интерфейсы не могут иметь модификатор доступа internal</p></li>
<li><p>Интерфейсы не могут иметь модификатор abstract</p></li>
<li><p>Интерфейс IRunAction должен определить метод Move, так как базовый интерфейс IAction является абстрактным</p></li>
<li><p>От абстрактного интерфейса нельзя наследоваться</p></li>
</ul>
</div>
<div class="answer">
...
<p>Интерфейсы не могут иметь модификатор abstract.</p>
</div>
</div>
</div>
...
<p>Почему данная программа не скомпилируется? Варианты ответов:</p>
<ul>
<li><p>Потому что метод Move в IRunAction определен с ключевым словом new</p></li>
<li><p>Потому что в обоих интерфейсах определен метод Move, и программа не знает, какой именно из этих методов вызывать</p></li>
<li><p>Потому что метод Move в классе RunAction определен с модификатором internal</p></li>
<li><p>Потому что класс RunAction должен определить две реализации метода Move для каждого из интерфейсов</p></li>
</ul>
</div>
<div class="answer">
...
<div class="answer-content">
<p>При реализации метода интерфейса в классе данный метод может иметь только модификатор public. Определение реализации с модификатором 
internal некорректно.</p>
</div>
</div>
</div>
...
<p>Класс Tester реализует интерфейсы IFoo и IBar:</p>
<pre class="brush:c#;">
interface IFoo
{
	void Execute();
...
<p>Метод <code>Execute()</code> какого именно интерфейса реализует класс Tester?</p>
</div>
<div class="answer">
<input type="checkbox" id="checkbox-3" name="checkbox-accordion" />
<label for="checkbox-3">Ответ</label>
<div class="answer-content">
<p>Класс Tester реализует метод Execute сразу обоих интерфейсов.</p>
</div>
</div>
</div>
...
<p>Реализация метода <code>Execute()</code> из какого интерфейса будет вызвана в следующем коде:</p>
<pre class="brush:c#;">
Tester t = new Tester();
t.Execute();
</pre>
...
<div class="answer-content">
<p>Выше приведенный код не будет скомпилирован. Необходимо явно указать, реализацию какого интерфейса мы хотим использовать, например, 
с помощью операции приведения типов:</p>
<pre class="brush:c#;">
Tester t = new Tester();
(t as IFoo).Execute();
</pre>
...
<ul>
<li><a title="Теоретический материал" href="../tutorial/3.9.php"><i class="fa fa-lg fa-file-text-o"></i><span class="dop-txt">Статья по теме</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="2.6.php">Назад</a><a href="./">Содержание</a><a href="2.17.php">Вперед</a></p></div></div>
<div class="socBlock">
