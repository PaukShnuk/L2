...
<html >
<head>
<title>Вопросы по C# | Перегрузка операций преобразования типов</title>
<meta charset="utf-8" />
<meta name="description" content="Проверка знаний и вопросы по теме Перегрузка операций преобразования типов в C#">
...
<p>Что выведет на консоль следующий код и почему:</p>
<pre class="brush:c#;">
class Clock
{
...
<p>Программа не скомпилируется, потому что в классе Clock определен оператор, который предусматривает явное преобразование от 
типа Clock к типу int. Поэтому на строке <code>int x = clock</code> мы получим ошибку. Для преобразования необходимо применить операцию 
приведения типов:</p>
<pre class="brush:c#;">int x = (int)clock;</pre>
</div>
</div>
</div>
...
<p>При проектировании следующих классов допущены ошибки. Какие?</p>
<pre class="brush:c#;">
class Dollar
{
...
<p>Варианты ответов (правильных вариантов может быть несколько):</p>
<ol>
<li><p>Нельзя определить оператор преобразования в тип decimal</p></li>
<li><p>Нельзя определить все операторы преобразования как неявные (с ключевым словом implicit). Необходимо сделать 
один или несколько операторов явными (определить с ключевым словом explicit)</p></li>
<li><p>Слово <span class="b">operator</span> лишнее, так как ключевые слова implicit и explicit уже говорят о том, 
что определяется оператор преобразования типов</p></li>
<li><p>Все операторы преобразования типов должны быть определены как статические (с ключевым словом <span class="b">static</span>)</p></li>
<li><p>Необходимо добавить операторы преобразования типов также и в класс Dollar, иначе преобразования раотать не будут</p></li>
<li><p>Оператор преобразования типов должен преобразовывать из типа или в тип, в котором этот оператор определен.</p></li>
</ol>
</div>
<div class="answer">
...
<div class="answer-content">
<p>Все операторы должны быть определены как статические, то есть с ключевым словом <span class="b">static</span>. Кроме того, оператор преобразования 
типов должен преобразовывать из типа или в тип, в котором этот оператор определен (то есть нельзя определить оператор преобразования из Dollar в decimal в классе Rouble)</p>
</div>
</div>
</div>
...
<p>Когда необходимо объявлять оператор преобразования как явный (с ключевым словом explicit), и когда как неявный (с ключевым словом implicit)?</p>
</div>
<div class="answer">
<input type="checkbox" id="checkbox-3" name="checkbox-accordion" />
<label for="checkbox-3">Ответ</label>
<div class="answer-content">
<p>Объявлять оператор преобразования как явный или неявный решается произвольным образом. Здесь нет четких критериев.</p>
</div>
</div>
</div>
...
<div class="nav"><p><a href="2.5.php">Назад</a><a href="./">Содержание</a><a href="2.11.php">Вперед</a></p></div></div>
<div class="socBlock">
