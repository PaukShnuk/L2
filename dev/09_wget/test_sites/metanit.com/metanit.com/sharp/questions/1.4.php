...
<html >
<head>
<title>Вопросы по C# | Параметры</title>
<meta charset="utf-8" />
<meta name="description" content="Проверка знаний и вопросы по теме Параметры в C#">
...
<h2>Параметры</h2><div class="date">Последнее обновление: 17.08.2018</div>
...
<p>Какой результат работы будет у следующей программы и почему:</p>
<pre class="brush:c#;">
class Program
{
...
<p>Приведенная выше программа не скомпилируется, так как перед передачей переменной x типа int в качестве ref-аргумента ей должно быть присвоено начальное значение.</p>
</div>
</div>
</div>
...
<p>Какое число (или какие числа) выведет на консоль следующая программа и почему:</p>
<pre class="brush:c#;">
class Program
{
	static void Main(string[] args)
...
<div class="answer-content">
<p>На консоль будет выведено число 121.</p>
<ol>
<li><p>Вначале выполняется инкремент переменной i (<code>i++</code>).</p></li>
<li><p>Затем ее увеличенное значение (то есть число 1) передается в метод <code>Calculate()</code>.</p></li> 
<li><p>Выполняется вызов метода Calculate(). В этом методе в вызове Console.Write(i++) переданное через параметр значение увеличивается на единицу, но так как это постфиксный инкремент, 
то возвращается число 1 и это же число выводится на консоль.</p></li>
<li><p>Далее метод Calculate возвращает число 2.</p></li> 
<li><p>Выполнение возвращается к строке <code>Console.Write(i++ + Calculate(i));</code>. Итак, здесь выражение <code>Calculate(i)</code> 
возвращает 2. Но выражение <code>i++</code>, хотя и увеличивает значение переменной i на единицу, возвращает ее значение до увеличения, то есть число 0, 
так как это постфиксный инкремент. Поэтому на консоль будет выведено число 0 + 2 = 2.</p>
<p>Так как ранее уже было выведено число 1, то после выполнение этого кода на консоли фактически будет число 12.</p></li>
<li><p>В конце выполняется строка <code>Console.WriteLine(i)</code>, которая добавляет к выведенному числу значение переменной i, то есть 1.</p></li> 
</ol>
<p>Таким образом, на консоль будет выведено число 121.</p>
</div>
</div>
</div>
...
<ul>
<li><a title="Теоретический материал" href="../tutorial/2.27.php"><i class="fa fa-lg fa-file-text-o"></i><span class="dop-txt">Статья по теме</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="1.8.php">Назад</a><a href="./">Содержание</a><a href="1.5.php">Вперед</a></p></div></div>
<div class="socBlock">
