...
<html >
<head>
<title>Вопросы по C# | Классы и объекты</title>
<meta charset="utf-8" />
<meta name="description" content="Проверка знаний и вопросы по теме Классы и объекты в C#, конструкторы и инициализаторы">
...
<h2>Классы и объекты</h2><div class="date">Последнее обновление: 03.08.2018</div>
...
<p>Какое значение поле name будет иметь при выполнение следующего кода и почему?</p>
<pre class="brush:c#;">
...
<p>Поле name объекта tom будет иметь значение "Bob". В самом начале выполняется начальная инициализация полей класса, и полю name присваивается 
значение "Sam". Далее выполняется конструктор, который устанавливает для переменной name значение "Tom". И в последнюю 
очередь выполняется инициализатор, который устанавливает для поля name значение "Bob".</p>
</div>
</div>
</div>
...
<p>Какое значения будут иметь поля name, age и email после выполнения следующего кода и почему? В каком порядке будут вызываться конструкторы класса Person?</p>
<pre class="brush:c#;">
...
<li><p>name: Bob</p></li>
<li><p>age: 31</p></li>
<li><p>email: tom@gmail.com</p></li>
</ul>
<p>Порядок выполнения конструкторов:</p>
<ol>
<li><p>Вначале вызывается конструктор с тремя параметрами: <code>Person(string name, int age, string email)</code>. Он вызывает конструктор с двумя параметрами 
<code>Person(string name, int age)</code>. Этому конструктору передаются значения name: "Bob", age: 31</p></li>
<li><p>Далее вызывается конструктор с двумя параметрами: <code>Person(string name, int age)</code>. Он вызывает конструктор с одним параметром
<code>Person(string name)</code>. Этому конструктору передаются значения name: "Bob"</p></li>
<li><p>Далее вызывается конструктор с одним параметром: <code>Person(string name)</code>. Он получает строку "Bob" и устанавливает ее в качестве значения для поля name</p></li>
<li><p>Затем выполняется конструктор с двумя параметрами: <code>Person(string name, int age)</code>. Он устанавливает число 31 в качестве значения для поля age</p></li>
<li><p>Далее выполняется конструктор с тремя параметрами: <code>Person(string name, int age, string email)</code>. Он устанавливает строку "tom@gmail.com" 
в качестве значения для поля email</p></li>
</ol>
</div>
</div>
</div>
...
<ul>
<li><a title="Теоретический материал" href="../tutorial/3.1.php"><i class="fa fa-lg fa-file-text-o"></i><span class="dop-txt">Статья по теме</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="1.7.php">Назад</a><a href="./">Содержание</a><a href="2.16.php">Вперед</a></p></div></div>
<div class="socBlock">
