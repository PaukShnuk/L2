...
<html >
<head>
<title>Вопросы по C# | Преобразование типов</title>
<meta charset="utf-8" />
<meta name="description" content="Проверка знаний и вопросы по теме Преобразование типов в C#">
...
<h2>Преобразование типов</h2><div class="date">Последнее обновление: 18.07.2018</div>
...
<p>Что будет выведено на консоль в результате выполнения следующей программы и почему?:</p>
<pre class="brush:c#;">
using System;
...
<p>При выполнении программа сгенерирует исключение System.NullReferenceException, потому что поскольку переменная tom НЕ представляет 
объект Employee, то преобразование <code>tom as Employee</code> возвратит null.</p>
</div>
</div>
</div>
...
<div class="answer-content">
<p>На консоль будет выведена строка "Bob". Переменная tom хранит ссылку на объект Employee. Переменная bob после присвоения также 
хранит эту же ссылку на объект Employee. Поэтому преобразование <code>bob as Employee</code> возвратит объект Employee, которая будет присвоена переменной 
...
из этих переменных приведет к тому, что другие переменные будут указывать на этот измененный объект.</p>
</div>
</div>
</div>
...
<p>Для преобразования объекта Person к типу Employee мы можем использовать следующие два способа:</p>
<pre class="brush:c#;">
Person tom = new Person { Name = "Tom" };
Employee empl1 = tom as Employee;
Employee empl2 = (Employee)tom;
</pre>
<p>Какой из способов является более предпочтительным и почему?</p>
</div>
<div class="answer">
<input type="checkbox" id="checkbox-3" name="checkbox-accordion" />
<label for="checkbox-3">Ответ</label>
<div class="answer-content">
<p>Использование оператора <span class="b">as</span> является более предпочтительным, поскольку в случае неудачного преобразования этот оператор 
возвращает значение <span class="b">null</span>. При доступе к объекту через переменную мы можем проверить ее значение на null.</p>
<p>А операция преобразования <code>(тип)</code> в случае неудачного преобразования генерирует исключение. Мы можем отлавливать данное исключение через 
конструкцию try...catch. Однако, как правило, обработка исключений является более затратной операций по сравнению с простой проверкой в блоке if.</p>
</div>
</div>
</div>
...
<ul>
<li><a title="Теоретический материал" href="../tutorial/3.11.php"><i class="fa fa-lg fa-file-text-o"></i><span class="dop-txt">Статья по теме</span></a></li>
</ul>
</fieldset></div>
<div class="nav"><p><a href="2.4.php">Назад</a><a href="./">Содержание</a><a href="2.14.php">Вперед</a></p></div></div>
<div class="socBlock">
