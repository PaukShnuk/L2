...
<html >
<head>
<title>ADO.NET и C# | Получение скалярных значений в SQLite</title>
<meta charset="utf-8" />
<meta name="description" content="Получение скалярных значений в SQLite в Microsoft.Data.Sqlite в C# и .NET. Метод ExecuteScalar класса SqliteCommand, получение id после добавления объекта">
...
<h2>Получение скалярных значений в SQLite</h2><div class="date">Последнее обновление: 03.03.2021</div>
...
<p><span class="b">SqliteCommand</span> с помощью специального метода <span class="b">ExecuteScalar()</span> позволяет получать из базы данных скалярный результат, 
что может быть полезно в различных ситуациях. Например, мы хотим при добавлении нового объекта получить его идентификатор, чтобы потом каким-то образом использовать 
в программе. И для этого можно как раз применить метод <span class="b">ExecuteScalar()</span>:</p>
<pre class="brush:c#;">
using System;
...
<p>Для получения идентификатора добавленнего объекта на стороне базы данных применяется встроенная функция <span class="b">last_insert_rowid()</span>. 
И для ее выполнения в код SQL-запроса добавляет подзапрос "SELECT last_insert_rowid();"</p>
<p>Следует учитывать, что метод <code>ExecuteScalar()</code> возвращает результат в виде объекта типа <span class="b">object</span>. В данном случае это не имеет большого 
значения, так как здесь полученный результат просто выводится на консоль, соответственно значение object автоматически приводится к типу string.</p>
<p>Другим примером применения функции может служить использование специальных встроенные агрегатные функции SQLite, например, 
<code>Min, Max, Sum, Count</code> и т.д., которые не выполняют операции с объектами и не извлекают объекты, 
а возвращают какое-то определенное значение. Например, функция <code>Count</code> подсчитывает количество объектов, а <code>Max</code> подсчитывает максмальное значение на некотором диапазоне значений.</p>
<p>Например, найдем число всех объектов в таблице, получим минимальный и средний возраст пользователей:</p>
<pre class="brush:c#;">
using System;
using Microsoft.Data.Sqlite;
...
<p>Выражение "SELECT COUNT(*) FROM Users" количество объектов в таблице Users, а выражение "SELECT MIN(Age) FROM Users" 
находит минимальное значение столбца Age. Подобным образом выражение "SELECT AVG(Age) FROM Users" получает среднее значение для столбца Age. В качестве результата метод <code>ExecuteScalarAsync()</code> возвращает объект типа 
<code>object</code>, который затем можно пробразовать к типу int или другому числовому типу и использовать как числовые данные.</p>
