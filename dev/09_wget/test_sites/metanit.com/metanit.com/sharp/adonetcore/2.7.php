...
<html >
<head>
<title>ADO.NET и MS SQL Server | Получение скалярных значений</title>
<meta charset="utf-8" />
<meta name="description" content="Получение скалярных результатов операций Min, Max, Count, Avg, Sum в MS SQL Server в .NET и C# с помощью SqlCommand, методы ExecuteScalarAsync и ExecuteScalar">
...
<h2>Получение скалярных значений</h2><div class="date">Последнее обновление: 09.02.2021</div>
...
<p>При отправке запросов мы можем использовать специальные встроенные агрегатные функции SQL, например, Min, Max, Sum, Count и т.д., 
которые не выполняют операции с объектами и не извлекают объекты, а возвращают какое-то определенное значение. Например, функция 
Count подсчитывает количество объектов. Подробнее про работу агрегатных функций в MS SQL Server можно посмотреть в статье 
<a href="https://metanit.com/sql/sqlserver/5.1.php">Агрегатные функции</a></p>
<p>И для работы с такими функциями в SqlCommand определен специальный метод <code>ExecuteScalar()/ExecuteScalarAsync()</code>, 
который возвращает результат в виде объекта типа <span class="b">object</span>. Например, найдем число всех объектов в таблице, 
получим минимальный и средний возраст пользователей:</p>
<pre class="brush:c#;">
...
<code>object</code>, который затем можно пробразовать к типу int и использовать как числовые данные.</p>
