...
<html >
<head>
<title>ADO.NET и MS SQL Server | Выходные параметры хранимых процедур</title>
<meta charset="utf-8" />
<meta name="description" content="Использование выходных параметров хранимых процедур MS SQL Server для получения результатов запросов в ADO.NET и C#, ParameterDirection.Output">
...
<h2>Выходные параметры хранимых процедур</h2><div class="date">Последнее обновление: 11.02.2021</div>
...
<p>Теперь применим добавленную процедуру для получения диапазона возрастов по имени:</p>
<pre class="brush:c#;">
using System;
using System.Data;
using System.Threading.Tasks;
...
<p>Здесь вначале выводим список пользователей из базы данных с помощью метода GetAllUsersAsync, а затем с помощью вызова GetAgeRangeAsync ищем диапазон возрастов по имени пользователя.</p>
<p>При обращении к хранимой процедуре в методе GetAgeRangeAsync определяем входной параметр nameParam и два выходных параметры - minAgeParam и maxAgeParam. 
Выходные параметры команды определяются также, как и входные за тем исключением, что для свойства <code>Direction</code> 
нам надо установить значение <span class="b">ParameterDirection.Output</span>.</p>
<pre class="brush:c#;">
SqlParameter minAgeParam = new SqlParameter
{
	ParameterName = "@minAge",
...
<p>По умолчанию все параметры имеют значение 
<code>ParameterDirection.Input</code>, которое указывает, что они являются входными.</p>
<p>После выполнения команды с помощью метода <code>ExecuteNonOueryAsync()</code> мы можем получить параметры по названию или по их индексу в коллекции 
