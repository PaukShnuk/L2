...
<html >
<head>
<title>ADO.NET и C# | Сопоставление типов C# и SQLite. Типизация SqliteDataReader</title>
<meta charset="utf-8" />
<meta name="description" content="Сопоставление типов данных C# и SQLite в Microsoft.Data.Sqlite. Типизация SqliteDataReader и его методы">
...
<h2>Сопоставление типов C# и SQLite. Типизация SqliteDataReader</h2><div class="date">Последнее обновление: 02.03.2021</div>
...
<p>Для получения результатов с помощью класса SqliteDataReader в прошлой теме применялся метод <code>GetValue</code>, 
который возвращал значение определенного столбца в текущей ячейки в виде объекта типа <span class="b">object</span>. 
Однако при создании таблицы в одной из прошлых тем использовался следующий скрипт sql:</p>
<pre class="brush:sql;">
CREATE TABLE Users(_id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, Name TEXT NOT NULL, Age INTEGER NOT NULL)
</pre>
<p>То есть, мы знаем, что во третьем столбце хранится возраст пользователя, который представляет целое число, 
и в программе мы хотели бы получать это значение именно как целое число, а не объект типа object. Ведь, поскольку 
GetValue возвращает объект типа object, то, чтобы использовать значение столбца, к примеру, как число, нам надо 
его привести к типу int. Но класс <span class="b">SqliteDataReader</span> представляет ряд методов, которые позволяют сразу получить данные 
определенных типов.</p>
<p>Итак, получим из таблицы Users, которая была создана в прошлых темах, данные с помощью дополнительных методов SqliteDataReader:</p>
<pre class="brush:c#;">
using System;
...
типа int и string соответственно. Причем поскольку мы знаем, что в столбце _id хранится число, то мы можем получить его именно с помощью 
...
<p>SQLite имеет только четыре примитивных типа: <span class="b">INTEGER</span>, <span class="b">REAL</span>, <span class="b">TEXT</span> и <span class="b">BLOB</span>. 
С помощью пакета Microsoft.Data.Sqlite типы данных C# и SQLite сопоставляются следующим образом:</p>
<table class="tab">
<tr class="tabhead"><td><p>Тип C#</p></td><td><p>Тип SQLite</p></td></tr>
<tr><td><p>bool</p></td><td><p>INTEGER (0 или 1)</p></td></tr>
<tr><td><p>sbyte</p></td><td><p>INTEGER</p></td></tr>
<tr><td><p>byte</p></td><td><p>INTEGER</p></td></tr>
<tr><td><p>byte[]</p></td><td><p>BLOB</p></td></tr>
<tr><td><p>char</p></td><td><p>TEXT (UTF-8)</p></td><td></tr>
<tr><td><p>DateTime</p></td><td><p>TEXT (формат yyyy-MM-dd HH:mm:ss.FFFFFFF)</p></td></tr>
<tr><td><p>DateTimeOffset</p></td><td><p>TEXT (формат yyyy-MM-dd HH:mm:ss.FFFFFFF)</p></td></tr>
<tr><td><p>TimeSpan</p></td><td><p>TEXT (формат d.hh:mm:ss.fffffff)</p></td></tr>
<tr><td><p>decimal</p></td><td><p>TEXT (формат 0.0###########################)</p></td></tr>
<tr><td><p>float</p></td><td><p>REAL</p></td></tr>
<tr><td><p>double</p></td><td><p>REAL</p></td></tr>
<tr><td><p>Guid</p></td><td><p>TEXT (формат 00000000-0000-0000-0000-000000000000)</p></td></tr>
<tr><td><p>int</p></td><td><p>INTEGER</p></td></tr>
<tr><td><p>short</p></td><td><p>INTEGER</p></td></tr>
<tr><td><p>long</p></td><td><p>INTEGER</p></td></tr>
<tr><td><p>uint</p></td><td><p>INTEGER</p></td></tr>
<tr><td><p>ushort</p></td><td><p>INTEGER</p></td></tr>
<tr><td><p>ulong</p></td><td><p>INTEGER</p></td></tr>
<tr><td><p>string</p></td><td><p>TEXT</p></td></tr>
</table>
<p>И для получения данных определенного типа данных мы можем применять соответствующий метод класса <code>SqliteDataReader</code>.</p>
