...
<html >
<head>
<title>ADO.NET и MS SQL Server | Подключение к MS SQL Server</title>
<meta charset="utf-8" />
<meta name="description" content="Microsoft.Data.SqlClient и создание подключения с помощью объекта SqlConnection для работы с базой данных MS SQL Server в C#, открытие и закрытие подключения">
...
<h2>Подключение к MS SQL Server</h2><div class="date">Последнее обновление: 08.02.2021</div>
...
<p>Для работы с базой данных MS SQL Server в .NET 5 и выше (а также .NET Core 3.0/3.1) необходимо установить в проект через nuget пакет <span class="b">Microsoft.Data.SqlClient</span>:</p>
<img src="./pics/1.1.png" alt="Microsoft.Data.SqlClient и подключение к MS SQL Server в .NET Core и в NET 6" />
<p>Для создания подключения к MS SQL Server применяется класс <span class="b">SqlConnection</span> из пространства имен 
Microsoft.Data.SqlClient.</p>
<p>Этот класс имеет три конструктора:</p>
<pre class="brush:c#;">
SqlConnection() 	
SqlConnection(String) 	
SqlConnection(String, SqlCredential) 
</pre>
<p>Второй и третий конструкторы в качестве первого параметра принимают строку подключения. Третий конструктор также принимает объект 
<code>SqlCredential</code>, который фактически представляет логин и пароль.</p>
<p>Теперь проверим подключение на примере сервера LocalDB:</p>
<pre class="brush:c#;">
...
<p>В данном случае подключение осуществляется к серверу LocalDB и его базе данных <code>master</code> (по умолчанию база данных master уже 
должна быть на любом MS SQL Servere).</p>
<p>Для начала взаимодействия с базой данных нам надо открыть подключение с помощью методов <code>Open()</code> (синхронный) или 
<code>OpenAsync()</code> (асинхронный).</p>
<p>По окончании работы с SqlConnection необходимо закрыть подключение к серверу, вызвав метод <span class="b">Close()/CloseAsync()</span> или 
<span class="b">Dispose()/DisposeAsync()</span>. В данном случае вначале проверяем, что подключение открыто и, если оно открыто, вызываем 
асинхронный метод <code>OpenAsync()</code>.</p>
<p>В итоге, если указана валидная строка подключения, то мы должны увидеть на консоли следующие строки:</p>
<div class="console">
...
<p>Вместо явного закрытия подключения также можно использовать конструкцию <code>using</code>, которая автоматически закрывает подключение:</p>
<pre class="brush:c#;">
using Microsoft.Data.SqlClient;
using System;
using System.Threading.Tasks;
