...
<html >
<head>
<title>ADO.NET и MS SQL Server | Выходные параметры запросов</title>
<meta charset="utf-8" />
<meta name="description" content="Использование выходных параметров для получения результатов запросов к базе данных MS SQL Server в ADO.NET и C#, свойство Direction типа SqlParameter, ParameterDirection, как получить id добавленной строки в базе данных">
...
<h2>Выходные параметры запросов</h2><div class="date">Последнее обновление: 11.02.2021</div>
...
<p>Параметры, которые используются в командах, могут быть нескольких типов. Тип параметра задается с помощью свойства 
<code>Direction</code> объекта SqlParameter. Данное свойство принимает одно из значений перечисления <span class="b">ParameterDirection</span>:</p>
<ul>
<li><p><code>Input</code>: параметр является входным, то есть предназначен для передачи значений в sql-выражение запроса. Это значение 
по умолчанию для всех параметров<p></li>
<li><p><code>InputOutput</code>: параметр может быть как входным, так и выходным.<p></li>
<li><p><code>Output</code>: параметр является выходным, то есть используется для возвращения запросом каких-либо значений<p></li>
<li><p><code>ReturnValue</code>: параметр представляет результат выполнения выражения или хранимой процедуры<p></li>
</ul>
<p>Используем выходные параметры для возвращения id строки, которая была добавлена:</p>
<pre class="brush:c#;">
using System;
using System.Data;
...
<p>Здесь к sql-выражению добавляется операция присвоения параметру id идентификатора добавленной строки: <code>SET @id=SCOPE_IDENTITY()</code>. 
Выражение <code>SCOPE_IDENTITY()</code> представляет встроенную функцию MS SQL Server, которая возвращает идентификатор добавленной строки.</p>
<pre class="brush:c#;">string sqlExpression = "INSERT INTO Users (Name, Age) VALUES (@name, @age);SET @id=SCOPE_IDENTITY()";</pre> 
<p>Для определения параметра используются его свойства. <code>ParameterName</code> задает имя параметра, <code>SqlDbType</code> указывает на тип параметра, 
а свойство <code>Direction</code> определяет тип параметра.</p>
<pre class="brush:c#;">
SqlParameter idParam = new SqlParameter
{
	ParameterName = "@id",
...
<p>После выполнения команды параметр получает значение, которое мы можем получить через его свойство <code>Value</code>:</p>
<pre class="brush:c#;">Console.WriteLine($"Id нового объекта: {idParam.Value}");</pre>
...
<div class="nav"><p><a href="2.8.php">Назад</a><a href="./">Содержание</a><a href="2.19.php">Вперед</a></p></div></div>
<div class="socBlock">
