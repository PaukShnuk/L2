...
<html >
<head>
<title>ADO.NET и C# | Чтение результатов запроса и SqliteDataReader</title>
<meta charset="utf-8" />
<meta name="description" content="Чтение результатов запроса  к БД SQLite и SqliteDataReader и SqliteCommand в .NET и C#, Microsoft.Data.SQLite">
...
<h2>Чтение результатов запроса и SqliteDataReader</h2><div class="date">Последнее обновление: 02.03.2021</div>
...
<p>Для чтения данных из базы данных SQLite применяется метод - <span class="b">ExecuteReader()</span> класса <span class="b">SqliteCommand</span>. 
Этот метод возвращает объект <span class="b">SqliteDataReader</span>, который используется для чтения данных.</p>
<p>Основные свойства класса SqliteDataReader, которые мы можем использовать при получении данных:</p>
<ul>
<li><p><span class="b">HasRows</span>: указывает, содержит ли SqliteDataReader как минимум одну строку</p></li> 
...
<p>Среди методов класса SqliteDataReader, которые применяются при работе с данным классом, следует выделить следующие:</p>
<ul>
<li><p><span class="b">Close()</span>: закрывает объект SqliteDataReader</p></li> 	
...
<p>Например, выведем на консоль все данные из таблицы Users, которая была созданна в прошлой теме:</p>
<pre class="brush:c#;">
using System;
...
Получив при выполнении запроса объект <code>SqliteDataReader</code>, мы можем считать все полученные данные.</p>
<p>Но вначале мы проверяем, а есть ли вообще данные с помощью свойства <code>HasRows</code>. И если данные есть, 
в цикле <code>while (reader.Read())</code> в порядке следования столбов получаем данные с помощью метода <code>GetValue()</code>, 
который возвращает данные в виде объекта типа <code>object</code>. Причем столбцы в выборке идут именно в том порядке, в котором они определены в таблице.</p>
<pre class="brush:sql;">
CREATE TABLE Users(_id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, Name TEXT NOT NULL, Age INTEGER NOT NULL)
</pre> 
<p>Например, столбец _id идет первым, поэтому для его получения применяется метод <code>reader.GetValue(0)</code>. 
А столбец Age идет третьим, поэтому его значения получаем с помощью <code>reader.GetValue(2)</code>.</p>
...
<p>В этом случае результат будет аналогичным. Однако здесь следует учитывать регистр передаваемых названий столбцов. Так, в таблице Users определены 
столбцы "_id" (с маленькой буквы), "Name" (с большой буквы), "Age" (с большой буквы), соответственно, мы пишем <code>reader["_id"]</code>, 
а не <code>reader["_Id"]</code>; и <code>reader["Name"]</code>, а не <code>reader["name"]</code>.</p>
