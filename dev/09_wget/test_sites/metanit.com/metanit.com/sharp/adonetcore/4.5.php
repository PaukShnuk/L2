...
<html >
<head>
<title>ADO.NET и C# | Параметризация запросов к БД Sqlite</title>
<meta charset="utf-8" />
<meta name="description" content="Параметризация запросов к БД Sqlite в Microsoft.Data.Sqlite в C# и .NET. Тип SqliteParameter и его методы, SqliteType">
...
<h2>Параметризация запросов к БД Sqlite</h2><div class="date">Последнее обновление: 02.03.2021</div>
...
<p>Пакет <code>Microsoft.Data.Sqlite</code> поддерживает параметризацию запросов. Для определения параметров применяется класс 
<span class="b">SqliteParameter</span>. Этот класс имеет ряд конструкторов, наиболее используемые из них:</p>
<ul>
<li><p><span class="b">SqliteParameter()</span></p></li>	
<li><p><span class="b">SqliteParameter(String, Object)</span>: первый параметр конструктора передает имя, а второй - значение параметра</p></li>		
<li><p><span class="b">SqliteParameter(String, SqliteType)</span>: первый параметр конструктора передает имя параметра, а второй - его тип 
в виде объекта <span class="b">SqliteType</span></p></li>
<li><p><span class="b">SqliteParameter(String, SqliteType, Int32)</span>: первый параметр конструктора передает имя параметра, второй - его тип, а третий - максимальный размер значения параметра в байтах</p></li>
<li><p><span class="b">SqliteParameter(String, SqliteType, Int32, String)</span>: конструктор последовательно принимает значения 
для имя параметра, его типа, максимального размера и имени столбца в таблице</p></li>
</ul>
<p>Для конфигурации параметров можно использовать их свойства, среди которых следует выделить следующие:</p>
<ul>
<li><p><span class="b">SqliteType</span>: задает или устанавливает тип параметра в виде типа SqliteType</p></li>	
...
<li><p><span class="b">Size</span>: хранит максимальный размер данных параметра в байтах</p></li>
<li><p><span class="b">Value</span>: хранит значение параметра</p></li>
</ul>
<p>Следует учитывать, что все параметры в Microsoft.Data.Sqlite являются входными.</p>
<p>После определения параметров они добавляются в коллекцию <span class="b">Parameters</span> объекта <span class="b">SqliteCommand</span>.</p>
...
<p>При выполнении команды на место параметров в sql-выражении подставляются их значения.</p>
...
<p>С помощью дополнительных конструкторов и свойств класса <span class="b">SqliteParameter</span> можно дополнительно определить тип и размер параметра.</p>
<p>Для описания типа параметра используется перечисление <span class="b">SqliteType</span>, которое определяет всего 
четыре значения:</p>
<ul>
<li><p><code>SqliteType.Integer</code></p></li>
<li><p><code>SqliteType.Real</code></p></li>
<li><p><code>SqliteType.Text</code></p></li>
<li><p><code>SqliteType.Blob</code></p></li>
</ul>
<p>Эти значения соответствуют соответствующим типам данных SQLite.</p>
...
<p>В данном случае параметр nameParam имеет тип <code>SqliteType.TEXT</code> и длину в 4 символа. Поэтому, несмотря на то, что на сохранение 
в базу данных этому параметру передается строка "Daniel", из-за ограничения по длине она будет усечена до 4 символов.</p>
<p>Результат работы программы:</p>
<div class="console">
<pre class="consoletext">
Добавлено объектов: 1
