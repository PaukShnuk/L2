...
<html >
<head>
<title>Flutter | Expanded</title>
<meta charset="utf-8" />
<meta name="description" content="Виджет Expanded в Flutter, заполнение доступного пространства в Row и Column, желтая полоска в Row, флекс-фактор flex-factor">
...
<h2>Expanded</h2><div class="date">Последнее обновление: 22.06.2020</div>
...
<p>Контейнер <span class="b">Expanded</span> позволяет своему вложенному виджету child заполнить доступное пространство (или его часть) других контейнеров - Row и Column.</p>
<p>Для создания виджета Expanded применяется следующий конструктор:</p>
<pre class="brush:dart;">
Expanded({Key key, int flex: 1, @required Widget child})
</pre>
<p>Параметр <code>child</code> представляет вложенный виджет, а параметр <code>flex</code> задает флекс-фактор - величину, в соответствии с 
которой виджету Expanded будет отводиться определенное пространство контейнера.</p>
<p>Сначала рассмотрим проблему, с которой мы можем столкнуться. Допустим, в контейнере Row мы хотим вывести относительно длинный текст:</p>
<pre class="brush:dart;">
...
<p>Если мы запустим приложение, то мы увидим что-то типа следующего:</p>
<img src="./pics/2.23.png" alt="Row и Expanded в Flutter" />
<p>Поскольку ширина контейнера Row недостаточна, чтобы вместить строку текста, то мы увидим желто-черную полоску и сообщение о превышении размера на столько 
пикселей. Тем не менее все остальное пространство Row под сторой текста у нас пусто. И было бы неплохо, чтобы вместо этой полоски мы видели бы перенос текста ниже, чтобы он заполнял все пространство Row. 
Для этого воспользуемся виджетом Expanded:</p>
<pre class="brush:dart;">
import 'package:flutter/material.dart';
...
<img src="./pics/2.24.png" alt="Заполнение Row и Expanded в Flutter" />
<h3>Флекс-фактор</h3>
<p>Флекс-фактор указывает на часть пространства контейнера, которая буде отдаваться виджету Expanded. При вычислении данной части пространства 
флекс-фактор данного виджета Expanded делиться на сумму флекс-факторов всех элементов. Полученное значение умножается на общее доступное простанство контейнера.</p>
<p>Например, пусть у нас есть следующий контейнер Row:</p>
<pre class="brush:dart;">
import 'package:flutter/material.dart';
...
<p>Таким образом, у нас три виджета Expanded, каждый из которых содержит виджет Container с определенной цветовой окраской. Первый виджет Expanded имеет флекс-фактор 3, 
второй - 1, третий - 2. Сумма всех флекс-факторов равна 3 + 1 + 2 = 6. Поэтому первый виджет Expanded получить 3/6 или 1/2 пространства Row, 
второй виджет Expanded - 1/6 пространства Row, а третий виджет Expanded - 2/6 или 1/3 пространства Row.</p>
<img src="./pics/2.25.png" alt="Flex-factor in Expanded in Flutter" />
<p>Если flex-factor явным образом не указан, то по умолчанию он равен 1.</p>
<p>То же самое касается и при расположении в контейнере Column, только пространство будет делиться пропорционально по вертикали:</p>
<pre class="brush:dart;">
import 'package:flutter/material.dart';
...
<img src="./pics/2.26.png" alt="Flex-factor, Expanded и Column в Flutter" />
