...
<html >
<head>
<title>Flutter | Выбор элемента в списке</title>
<meta charset="utf-8" />
<meta name="description" content="Выбор элемента в списке ListView с помощью конструктора GestureDetector во Flutter, обработка выделения элементов">
...
<h2>Выбор элемента в списке</h2><div class="date">Последнее обновление: 19.02.2021</div>
...
<p>По умолчанию ListView не поддерживает отслеживание выделенного элемента, и в такой ситуации нам надо создавать 
самим всю инфраструктуру обработки выделения элемента. В этом случае мы можем воспользоваться ранее рассмотренным 
виджетом <span class="b">GestureDetector</span>, который позволяет отслеживать нажатия.</p>
<p>Например, определим следующий код:</p>
...
<p>Итак, в данном случае определен новый виджет - UsersList, который может хранить состояние. Все его состояние инкапсулировано 
в рамках класса <code>_UsersListState</code></p>
<p>_UsersListState хранит список <code>users</code> - собственно данные, которые будут выводиться в ListView:</p>
<pre class="brush:dart;">final List&lt;String&gt; users = ["Tom", "Alice", "Sam", "Bob", "Kate"];</pre>
<p>Для отслеживания выделенного элемента нам потребуется его индекс. И для хранения индекса выделенного элемента из списка 
users определим отдельную переменную:</p>
<pre class="brush:dart;">int selectedIndex = -1;</pre>
<p>По умолчанию она равна -1, что будет выступать в качестве индикатора, что ни одного элемента еще не выбрано. 
После выделения элемента в ListView эта переменная будет хранить индекс выделенного элемента.</p>
<p>Организационно все виджеты будут помещаться в контейнер <code>Column</code>, в котором первый виджет - Text будет 
выводить значение выделенного элемента (если такой имеется):</p>
<pre class="brush:dart;">
Text(
  selectedIndex==-1?"Не выбрано": "Выбрано: ${users[selectedIndex]}",
  style: TextStyle(fontSize: 30)),
</pre>
<p>Чтобы у ListView не было проблем с заполнением пространства контейнера помещаем этот виджет в контейнер <span class="b">Expander</span>. 
А само наполнение ListView для наглядности вынесено в отдельный метод <code>_createListView</code>. напомню, что в конструкторе ListView 
параметр itemBuilder принимает функцию, которая в качестве второго параметра получает индекс элемента:</p>
<pre class="brush:dart;">Widget _createListView(BuildContext context, int index) {</pre> 
<p>Так из параметра метода _createListView мы можем получить индекс текущего элемента и что-то с ним сделать.</p>
<p>Контейнером верхнего уровня для элемента в ListView служит виджет <span class="b">GestureDetector</span>, который позволяет обрабатывать нажатия.</p>
<p>При нажатии будет срабатывать функция, которая передается через параметр <code>onTap</code> виджета GestureDetector:</p>
<pre class="brush:dart;">
onTap: () {
        setState(() {
          // устанавливаем индекс выделенного элемента
...
<p>Здесь мы изменяем состояние виджета, передавая в переменную <code>selectedIndex</code> индекс нажатого элемента.</p>
<p>Сам GestureDetector содержит виджет <code>Container</code>, а тот - виджет <code>Text</code>, который выводит строку 
из списка <code>users</code>.</p>
<p>Чтобы выделить отмеченный элемент, в виджете Container фоновый цвет устанавливается в зависимости от того, является ли текущий 
индекс индексом выделенного элемента:</p>
<pre class="brush:dart;">color: index == selectedIndex ? Colors.black12: Colors.white60,</pre>
<p>В итоге мы сможем выделять элементы в ListView и получать их:</p>
<img src="./pics/4.11.png" alt="Выделение элементов в списке ListView в Flutter" />
