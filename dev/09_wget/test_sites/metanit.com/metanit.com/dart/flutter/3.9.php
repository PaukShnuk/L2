...
<html >
<head>
<title>Flutter | GestureDetector и обработка нажатий</title>
<meta charset="utf-8" />
<meta name="description" content="GestureDetector и обработка нажатий виджетов во Flutter, параметр onTap, изменение состояния в StatefulWidget">
...
<h2>GestureDetector и обработка нажатий</h2><div class="date">Последнее обновление: 06.02.2021</div>
...
<p>Во Flutter мы можем добавить возможность обработки нажатию для другого виджета. Для этого применяется класс 
<span class="b">GestureDetector</span>.</p>
<p>Конструктор данного класса содержит много параметров, поэтому рассмотрим наиболее основные. В частности, параметр 
<code>child</code> позволяет установить вложенный виджет, для которого добавляется возможность нажатия. Если 
<code>GestureDetector</code> не имеет вложенного виджета, то он занимает все пространство своего родительского 
контейнера, делая всю его площадь кликабельной.</p>
<p>Для обработки все возможных нажатий конструктор GestureDetector предоставляет различные параметры. Действие для простого нажатия 
можно задать с помощью параметра <span class="b">onTap</span>.</p>
<p>Например, сделаем с помощью GestureDetector кликабельным виджет Container:</p>
...
<p>В данном случае параметр класса <code>onTap</code> указывает на метод <code>_increaseValue()</code>, который изменяет 
состояние виджета Counter. В частности, увеличивает значение переменной <code>_value</code> на единицу.</p>
<p>Вложенным виджетом для GestureDetector является на элемент Container, который, в свою очередь, содержит виджет 
Text. А Text выводит значение переменной _value. То есть по нажатию на область, занимаемую виджетом Container, будет срабатывает метод <code>_increaseValue()</code>.</p>
<img src="./pics/3.251.png" alt="Виджет GestureDetector и обработка нажатий в Flutter" />
<p>Подобным образом можно добавить нажатия и к другим виджетами с более сложной иерархией или создать собственные виджеты, 
которые поддерживают обработку нажатий.</p>
