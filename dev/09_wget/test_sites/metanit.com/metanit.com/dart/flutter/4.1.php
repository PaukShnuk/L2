...
<html >
<head>
<title>Flutter | Виджет ListView</title>
<meta charset="utf-8" />
<meta name="description" content="Виджет ListView, горизонтальные и вертикальные списки во Flutter">
...
<h1>TextEditingController</h1><h2>Виджет ListView</h2><div class="date">Последнее обновление: 19.02.2021</div>
...
<p>Виджет <span class="b">ListView</span> представляет прокручиваемый список элементов.</p>
<p>Класс ListView имеет несколько конструкторов с большим количеством параметров, поэтому отметим только некоторые 
параметры:</p>
<ul>
<li><p><span class="b">children</span>: объект <code>List&lt;Widget&gt;</code>, который представляет список виджетов, добавляемых в 
ListView</p></li>
<li><p><span class="b">scrollDirection</span>: устанавливает направление элементов. 
Представляет перечисление <code>Axis</code>, которое определяет две константы:</p>
<ul>
<li><p><span class="b">Axis.horizontal</span>: устанавливает горизонтальный список - элементы располагаются слева направо (или справо налево)</p></li>
<li><p><span class="b">Axis.vertical</span>: устанавливает вертикальный список - элементы располагаются сверху вниз</p></li>
</ul>
<p>Значение по умолчанию - <code>Axis.vertical</code>.</p>
</li>
<li><p><span class="b">padding</span>: устанавливает отступы элементов от границ ListView, представляет объект 
<code>EdgeInsetsGeometry</code></p></li>
<li><p><span class="b">reverse</span>: если равен <code>true</code>, располагает элементы в обратном порядке</p></li>
<li><p><span class="b">physics</span>: задает параметры скроллинга с помощью объекта <span class="b">ScrollPhysics</span></p></li>
</ul>
<p>Определим простейший список:</p>
...
<p>В данном случае все элементы ListView представлены виджетом <code>Text</code>:</p>
<img src="./pics/4.1.png" alt="ListView in Flutter и Dart" />
<p>В реальности это не самый оптимальный способ построения списка, учитывая, что в данном случае меняется только 
надпись в виджете Text. Но далее мы посмотрим, как можем эту структуру упростить.</p>
<p>По умолчанию создается вертикальный список, где все элементы расположены в столбик. Теперь определим горизонтальный 
список:</p>
<pre class="brush:dart;">\
import 'package:flutter/material.dart';
...
<p>Для создания горизонтального списка параметру <code>scrollDirection</code> передается значение <span class="b">Axis.horizontal</span>. 
Однако если мы посмотрим на результат, то мы увидим, что элементы стоят вплотную друг к другу без каких-нибудь отступов.</p>
<img src="./pics/4.2.png" alt="Горизонтальный список horizontal ListView in Flutter и Dart" />
<p>В этом случае мы можем обернуть виджет Text в какой-то другой виджет, который поддерживает применение отступов:</p>
<pre class="brush:dart;">
import 'package:flutter/material.dart';
...
<p>В данном случае для создания отступов виджет Text обертывается в Padding, который устанавливает равные отступы справа и слева:</p>
<img src="./pics/4.3.png" alt="Горизонтальный список ListView в Flutter и Dart" />
