...
<html >
<head>
<title>Flutter | RichText</title>
<meta charset="utf-8" />
<meta name="description" content="Виджет RichText в Flutter, InlineSpan и TextSpan, стилизация текста и TextStyle">
...
<h2>RichText</h2><div class="date">Последнее обновление: 30.01.2021</div>
...
<p>Виджет <span class="b">RichText</span> позволяет выводить текст с разным стилевым оформлением. Это достигается благодаря тому, что каждый отдельный 
кусок текста в RichText представляет объект <span class="b">TextSpan</span>, для которого можно настроить свой стиль.</p>
<p>Для создания RichText применяется следующий конструктор:</p>
...
<p>Большинство параметров конструктора совпадают с параметрами конструктора класса Text. Соответственно мы их можем использовать также, как и при создании 
и настройке виджета Text. Рассмотрим лишь те параметры, которые отличаются.</p>
<p>Прежде всего, текст задается с помощью параметра <code>text</code>, который представляет объект <span class="b">InlineSpan</span>. В реальности, как правило, 
этому параметру передается объект класса <span class="b">TextSpan</span> (который унаследован от InlineSpan). По сути TextSpan - это некий кускок текста.</p>
<p>Для создания TextSpan применяется следующий конструктор:</p>
<pre class="brush:dart;">
TextSpan({String text, List&lt;InlineSpan&gt; children, TextStyle style, GestureRecognizer recognizer, String semanticsLabel})
</pre>
<p>Первый параметр - <code>text</code> представляет собственно текст объекта. Но кроме этого, с помощью параметра <code>children</code> 
можно вложить дополнительные объекты InlineSpan (в том числе объекты TextSpan), которые также могут содержать текст. Третий параметр - <code>style</code> 
позволяет задать для текста определенный стиль.</p>
<p>Рассмотрим простейший RichText:</p>
<pre class="brush:dart;">
...
<img src="./pics/2.27.png" alt="RichText и TextSpan в Flutter" />
<p>В данном случае весь текст в RichText инкапсулирован во вложенном виджете TextSpan, который, в свою очередь, сам содержит еще объекта 
TextSpan. Фактически текст из обоих вложенных TextSpan просто добавляется к тексту из родительского TextSpan.</p>
<p>Следует учитывать, что стиль родительского TextSpan автоматически распространяется на все дочерние объкты TextSpan. Так, в примере выше мы видим, что 
установка высоты шрифта <code>style: TextStyle(fontSize: 20)</code> фактически применяется ко всем объктам TextSpan. На уровне каждого дочернего TextSpan же можно 
определить свой специфический стиль или изменить унаследованные от родительского TextSpan стилевые настройки.</p>
