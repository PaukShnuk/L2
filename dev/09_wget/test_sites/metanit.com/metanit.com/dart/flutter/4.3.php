...
<html >
<head>
<title>Flutter | Добавление в список разделителя</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление разделителя в виджета ListView с помощью конструктора ListView.separated() во Flutter, параметр separatorBuilder, виджеты Divider и VerticalDivider">
...
<h2>Добавление в список разделителя</h2><div class="date">Последнее обновление: 19.02.2021</div>
...
<p>При наполнении ListView мы можем по своему стилизовать виджет, который применяется для каждого элемента списка - задать 
отступы, границы, какую-то графику. Но если речь идет о простом создании разделителя между элементами в ListView, 
то мы можем воспользоваться еще одним конструктором класса - <span class="b">ListView.separated</span>. 
Этот конструктор также, как и конструктор <code>ListView.builder</code> позволяет задать с помощью параметра <code>itemBuilder</code> 
функцию для создания элементов в ListView. Но кроме того с помощью дополнительного параметра <span class="b">separatorBuilder</span> 
он позволяет задать функцию для создания границы между элементами.</p>
<p>Параметр <span class="b">separatorBuilder</span>  также представляет функцию типа <code>IndexedWidgetBuilder</code>, 
то есть функцию <span class="b">Widget Function(BuildContext context, int index)</span>.</p>
<p>Итак, применим конструктор <code>ListView.separated</code>:</p>
...
<p>Для создания разделителя здесь применяется встроенный виджет <span class="b">Divider()</span>, который по сути представляет полоску:</p>
<img src="./pics/4.7.png" alt="ListView.separated и separatorBuilder при создании ListView в Flutter и Dart" />

<p>И Divider предоставляет ряд параметров, которые позволяют настроить разделитель:</p>
<ul>
<li><p><span class="b">double height</span>: высота разделителя</p></li>
<li><p><span class="b">double thickness</span>: ширина разделителя</p></li>
<li><p><span class="b">double indent</span>: отступ перед разделителем</p></li>
<li><p><span class="b">edouble ndIndent</span>: отступ после разделителя</p></li>
<li><p><span class="b">Color color</span>: цвет разделителя</p></li>
</ul>
<p>Например, настроим разделитель:</p>
<pre class="brush:dart;">
import 'package:flutter/material.dart';
...
<p>Здесь разделитель принимает ширину в 20 единиц и толщину, равную 3. Кроме того, использует светло-красный цвет.</p>
<img src="./pics/4.9.png" alt="Настройка Divider в ListView и ListView.separated в Flutter" />

<p>Для горизонтального списка можно применять другой встроенный виджет <span class="b">VerticalDivider</span>:</p>
<pre class="brush:dart;">
import 'package:flutter/material.dart';
...
<img src="./pics/4.8.png" alt="Граница между элементами в горизонтальном списке ListView и ListView.separated в Flutter" />
<p>VerticalDivider предоставляет те же параметры, что и Divider,  только вместо параметра <code>height</code>, применяет 
параметр <code>width</code>, который настраивает ширину разделителя.</p>
<p>Несмотря на то, что Flutter предоставляет встроенные виджеты для создания разделителя - Divider и VerticalDivider, в реальности 
мы сами можем определить свой виджет, который будет выполнять функцию границы между элементами в ListView. 
Например, используем виджет Container:</p>
<pre class="brush:dart;">
import 'package:flutter/material.dart';
...
<p>Поскольку Container имеет в данном случае высоту 1, то по сути он будет представлять тонкую полоску. Дополнительно здесь устанавливается 
отступ в 10 единиц от соседних виджетов, в качестве которых выступают виджеты-элементы списка, опять же представленные классом 
Container с вложенным виджетом Text.</p>
<img src="./pics/4.10.png" alt="Граница между элементами в списке ListView и ListView.separated в Flutter" />
