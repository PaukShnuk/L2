...
<html >
<head>
<title>Flutter | MaterialApp и Scaffold</title>
<meta charset="utf-8" />
<meta name="description" content="Классы MaterialApp и Scaffold в Flutter, стиль Material Design, содание верхней панели с заголовком AppBar">
...
<h2>MaterialApp и Scaffold</h2><div class="date">Последнее обновление: 03.02.2021</div>
...
<p>Виджет <span class="b">MaterialApp</span> предназначен для создания графического интерфейса в 
стиле material design. Кроме собственно интерфейса в стиле material design это виджет предоставляет много другой 
функциональности, например, позволяет определить навигацию, различные части приложения и т.д. Поэтому вначале 
рассмотрим некоторые базовые моменты использования MaterialApp.</p>
<p>Конструктор MaterialApp имеет приличное количество параметров, среди которых следует выделить параметр 
<span class="b">home</span>. Этот параметр задает базовый виджет, который будет отображаться в MaterialApp при загрузке. 
Например:</p>
...
<img src="./pics/3.14.png" alt="MaterialApp в Flutter и Dart" />
<h3>Scaffold</h3>
<p>Несмотря на то, что для параметра <code>home</code> можно использовать любой виджет, в реальности, как правило, 
в данном качестве используется виджет <span class="b">Scaffold</span>. Это еще один виджет, который применяется для 
создания интерфейса в стиле Material Design.</p>
<p>Конструктор класса Scaffold также имеет довольно много параметров, из которых прежде всего следует выделить 
параметр <span class="b">body</span> - он задает основное содержимое Scaffold в виде другого виджета. Например:</p>
<pre class="brush:dart;">
import 'package:flutter/material.dart';
...
<img src="./pics/3.15.png" alt="MaterialApp и Scaffold в Flutter и Dart" />
<h3>AppBar</h3>
<p>Из других параметров конструктора Scaffold следует отметить параметр <span class="b">appBar</span>, который задает 
верхнюю панель с заголовком и возможными иконками. В качестве значения этот параметр принимает объект <span class="b">AppBar</span>.</p>
<p><span class="b">AppBar</span> представляет еще один класс, который применяется для придания приложению стиля 
Material Design.</p>
<p>Виджет AppBar также предоставляет богатую функциональность. И пока остановимся только на заголовке, который 
устанавливается с помощью параметра <span class="b">text</span> конструктора AppBar. Этот параметр в качестве значения принимает виджет 
<span class="b">Text</span>:</p>
<pre class="brush:dart;">
import 'package:flutter/material.dart';
...
<img src="./pics/3.16.png" alt="MaterialApp и Scaffold и AppBar в Flutter и Dart" />
<p>Как видно со скриншота, в данном случае нам не надо устанавливать сверху никаких отступов, Scaffold автоматически 
позиционирует основной контент ниже панели AppBar. Кроме того, виджеты предоставляют некоторые цвета по умолчанию.</p>
<p>При этом поскольку заголовок в AppBar представляет не просто строку, а виджет Text, то соответственно мы можем применить некоторую 
стилизацию, например, цвет текста и т.д.</p>
