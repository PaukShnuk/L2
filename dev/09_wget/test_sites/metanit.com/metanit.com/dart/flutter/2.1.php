...
<html >
<head>
<title>Flutter | Введение в виджеты</title>
<meta charset="utf-8" />
<meta name="description" content="Введение в виджеты в приложении на Flutter, функция runApp, иерархия виджетов">
...
<h2>Введение в виджеты</h2><div class="date">Последнее обновление: 15.06.2020</div>
...
<p>Центральным элементом приложения на Flutter являются виджеты. Фактически это те визуальные компоненты, из которых состоит графический интерфейс.</p>
<p>Рассмотрим простейший пример. Прежде всего создадим новый проект Flutter.</p>
<img src="./pics/2.1.png" alt="Введение в виджеты в Flutter" />
<p>По умолчанию код приложения размещается в файле <span class="b">lib/main.dart</span>. Откроем этот файл и изменим его содержимое:</p>
...
<p>Выполнение приложения Flutter начинается с функции <span class="b">main</span>. Для создания графического интерфейса в этой функции вызывается 
другая встроенная функция - <span class="b">runApp(Widget app)</span>. Она прикрепляет определенный виджет к экрану. То есть по сути это, что мы видим при запуске приложения на экране устройства. 
Прикрепляемый виджет передается в функцию <code>runApp()</code> в качестве параметра.</p>
<p>Все виджеты наследуется от класса <span class="b">Widget</span>. В данном случае в функцию runApp передается виджет <span class="b">Text</span>, который предназначен для 
...
строка 'Привет Flutter!'. И также устанавливаются свойство <span class="b">textDirection</span>, которое задает направление текста и принимает значение 
<code>TextDirection.ltr</code> (направление текста слева направо), и свойство <span class="b">textAlign</span>, которое задает выравнивание текста по горизонтали и принимает 
значение <code>TextAlign.center</code> (то есть текст выравнивается по центру).</p>
<p>После изменения кода Android Studio может подчеркивать папку с файлами тестов как некорректную, поскольку мы изменили код программы, не изменив файл тестов:</p>
<img src="./pics/2.4.png" alt="Android Studio и виджеты в Flutter" />
<p>Но на данный момент можно не обращать внимание на этот момент. Более того файл <span class="b">widget_test.dart</span> можно пока вовсе удалить.</p>
<p>Итак, мы определили самое минимальное приложение Flutter. В итоге при запуске приложения мы увидим вверху по центру устройства искомый текст:</p>
<img src="./pics/2.2.png" alt="Widget в Flutter" />
<p>Одни виджеты могут включать другие, образуя тем самым иерархическую систему в виде дерева. Например, разместим текст по центру экрана по вертикали. Для этого 
воспользуемся виджетом <span class="b">Align</span>:</p>
<pre class="brush:dart;">
import 'package:flutter/material.dart';
...
<p>Теперь корневым виджетом, который передается в функцию runApp, является виджет <span class="b">Align</span>. С помощью свойства <span class="b">alignment</span> 
задается выравнивание вложенных элементов. В данном случае значение <code>Alignment.center</code> указывает, что выравнивание будет идти по центру как по горизонтали, 
так и по вертикали.</p>
<p>Другое свойство - <span class="b">child</span> представляет вложенный виджет - в данном случае ранее использовавшийся виджет Text.</p>
<p>Когда мы запустим приложение, по сути мы увидим виджет Align, внутри которого расположен виджет Text:</p>
<img src="./pics/2.3.png" alt="Иерархия виджетов в Flutter" />
