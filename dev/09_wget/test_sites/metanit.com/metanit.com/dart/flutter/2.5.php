...
<html >
<head>
<title>Flutter | ConstrainedBox</title>
<meta charset="utf-8" />
<meta name="description" content="Виджет ConstrainedBox в Flutter, установка размеров элементов, BoxConstraints">
...
<h2>ConstrainedBox</h2><div class="date">Последнее обновление: 21.06.2020</div>
...
<p>Виджет <span class="b">ConstrainedBox</span> задает параметры прямоугольной области (ширину и высоту), в которой размещается вложенный виджет. 
Для создания объекта ConstrainedBox применяется следующий конструктор:</p>
<pre class="brush:dart;">
ConstrainedBox({Key key, @required BoxConstraints constraints, Widget child})
</pre>
<p>Обязательный параметр constraints представляет класс <span class="b">BoxConstraints</span>, который имеет следующие конструкторы:</p>
<ul>
<li><p><code>BoxConstraints({double minWidth: 0.0, double maxWidth: double.infinity, double minHeight: 0.0, 
double maxHeight: double.infinity})</code>: получает минимальную ширину (minWidth), максимальную ширину (maxWidth), 
минимальную (minHeight) и максимальную (maxHeight) длину</p></li>
<li><p><code>BoxConstraints.expand({double width, double height})</code>: принимает ширину и длину, до которых может растягиваться контейнер</p></li>
<li><p><code>BoxConstraints.loose(Size size)</code>: создает контейнер, который должен иметь размер не больше, чем параметр size</p></li>
<li><p><code>BoxConstraints.tight(Size size)</code>: создает контейнер, который должен иметь в точности тот же размер, что и параметр size</p></li>
<li><p><code>BoxConstraints.tightFor({double width, double height})</code>: принимает точные значения ширины и длины, которые должен иметь контейнер</p></li>
<li><p><code>BoxConstraints.tightForFinite({double width: double.infinity, double height: double.infinity})</code>: принимает точные значения ширины и длины, если они не равны <code>double.infinity</code></p></li>
</ul>
<p>Данный тип контейнеров применяется, когда нам надо ограничить виджет определенной областью.</p>
<p>Например, виджет Text по умолчанию растягивается по всей длине и ширине контейнера:</p>
<pre class="brush:dart;">
...
<img src="./pics/2.12.png" alt="Установка ширины и высоты ConstrainedBox в Flutter" />
<p>Используем ConstrainedBox, чтобы ограничить область текста на экране:</p>
<pre class="brush:dart;">
import 'package:flutter/material.dart';
...
<p>В данном случае с помощью конструктора <code>BoxConstraints.tightFor(width: 300, height: 100)</code> область ConstrainedBox и соответственно 
его содержимого ограничивается прямоугольником шириной 300 и высотой в 100 единиц.</p>
<img src="./pics/2.13.png" alt="BoxConstraints и установка размеров в Flutter" />
<p>Если содержимое виджета занимает больше места, то оно усекается до видимой области. Например, попробуем вывести многострочный текст:</p>
<pre class="brush:dart;">
import 'package:flutter/material.dart';
...
<p>В данном случае последняя строка не впишеется в отведенную для виджета область, поэтому мы ее не увидим:</p>
<img src="./pics/2.14.png" alt="Ограничение размеров виджетов в Flutter" />
