...
<html >
<head>
<title>Dart | Структура программы</title>
<meta charset="utf-8" />
<meta name="description" content="Структура программы на языке программирования Dart, синтаксис языка, функция main, инструкции и блоки кода, типы комментариев">
...
<h1>Основы языка Dart</h1><h2>Структура программы</h2><div class="date">Последнее обновление: 12.05.2020</div>
...
<p>Входной точкой в программу на языке Dart является функция <span class="b">main</span>. Именно с нее начинается 
выполнение программы. Она обязательно должна присутствовать в программе. Ее простейшее определение может быть следующим:</p>
<pre class="brush:dart;">
void main(){

}</pre>
<p>Слово <code>void</code> вначале определения функции указывает, что она не возвращает никакого значения, просто выполняет некоторые действия. 
Далее идет собственно имя функции - <span class="b">main</span> и затем пустые скобки, которые указывают, что функция не принимет параметров. Хотя мы также можем использовать и другую форму функции main, которая принимает параметры - и через эти параметры передать функции данные извне при запуске программы.</p>
<p>После пустых скобок идет блок функции в виде открывающей и закрывающей фигурных скобок {}, внутри которых помещается набор выполняемых инструкций - те действия, 
которые и будет выполнять программа.</p>
<p><span class="b">Инструкции</span> (statement) являются основным строительным блоком в программе на Dart. Каждая инструкция выполняет некоторое действие, 
например, вызовы методов, объявление переменных и присвоение им значений. После завершения инструкции в Dart ставится точка с запятой (;). Данный знак указывает на 
конец инструкции. Например:</p>
<pre class="brush:dart;">
print("Hello Dart!");
</pre>
<p>Данная строка представляет вызов функции <code>print</code>, которая выводит на консоль строку "Hello Dart!". В данном случае вызов 
функции является инструкцией и поэтому завершается точкой с запятой.</p>
...
<p>Функция main по сути также является блоком кода и также может содержать другие блоки кода:</p>
<pre class="brush:dart;">
void main() {
...
<h3>Комментарии</h3>
<p>Код программы может содержать комментарии. Комментарии позволяют понять смыл программы, что делают те или иные ее части. При компиляции комментарии 
игнорируются и не оказывают никакого влияние на работу приложения и на его размер.</p>
<p>В Dart есть два типа комментариев: однострочный и многострочный. Однострочный комментарий размещается на одной строке после двойного слеша //. 
