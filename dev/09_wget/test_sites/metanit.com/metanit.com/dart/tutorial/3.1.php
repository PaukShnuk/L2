...
<html >
<head>
<title>Dart | Определение функций</title>
<meta charset="utf-8" />
<meta name="description" content="Определение и вызов функций в языке программирования Dart">
...
<h1>Функциональное программирование</h1><h2>Определение функций</h2><div class="date">Последнее обновление: 17.06.2019</div>
...
<p>Функции представляют некоторые действия. Мы можем оформить набор операторов в виде одной функции и многократно вызывать ее в программе. Формальное определение функции:</p>
...
<p>Вначале указывается возвращаемый тип функции. Если функция не возвращает никакого значения, то используется тип <span class="b">void</span>.</p>
<p>Затем идет имя функции, которое представляет произвольное название, к которому применяются те же правила, что и к именованию переменных.</p>
<p>После имени функции в скобках идет перечисление параметров. Функция может не иметь параметров, в этом случае указываются пустые скобки.
<p>Далее в фигурных скобках идет тело функции, которое содержит выполняемые выражения.</p>
<p>Например, определим функцию, которую назовем hello:</p>
<pre class="brush:dart;">
void main() {
}
void hello(){
...
<p>Функция hello не возвращает никакого результата, а просто выполняет некоторые действия, поэтому функция имеет тип void. Все, что она делает - это выводит строку Hello.</p>
<p>Но сама по себе функция не будет выполняться. По умолчанию среда выполняет только функцию main. Поэтому, чтобы выполнить функцию hello, ее необходимо вызвать в функции main. Вызов функции осуществляется в форме:</p>
<pre class="brush:dart;">имя_функции(аргументы);</pre>
<p>После имени функции указываются скобки, в которых перечисляются аргументы - значения для параметров функции. Например, вызовем функцию hello два раза:</p>
<pre class="brush:dart;">
void main() {
...
<p>Если функция состоит из одного выражения, то ее можно сократить следующим образом:</p>
<pre class="brush:dart;">
[тип] имя_функции(параметры) =&gt; выполняемое_выражение
</pre>
<p>В данном случае после списка параметров в скобках идет =&gt;, а затем выполняемое выражение. Например, выше функция hello фактически выполняла 
только одно выражение, поэтому мы можем ее сократить:</p>
<pre class="brush:dart;">
void main() {
