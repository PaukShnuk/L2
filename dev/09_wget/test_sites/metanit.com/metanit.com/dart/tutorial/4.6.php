...
<html >
<head>
<title>Dart | Миксины</title>
<meta charset="utf-8" />
<meta name="description" content="Миксины и наследование интерфейсов в языке программирования Dart, операторы mixin и with">
...
<h2>Миксины</h2><div class="date">Последнее обновление: 23.01.2021</div>
...
<p>Еще одно решение проблемы множественного наследования в Dart представляют миксины. По сути миксин представляет обычный класс за тем исключением, 
что он не должен содержать конструктор, иначе он не может быть использован в качестве миксина. Для применения миксинов применяется оператор <span class="b">with</span>:</p>
...
<p>В данном случае класс Worker выступает в роли миксина. Для использования его функционала при определении класса Employee применяется оператор <span class="b">with</span>:</p>
<pre class="brush:dart;">class Employee extends Person with Worker{</pre>
<p>После этого класс Employee может использовать поля и методы из класса Worker. При этом класс Employee может наследовать от других классов, как в данном случае 
от класса Person.</p>
<p>При этом поскольку тип Worker определен как обычный класс, мы можем использовать его независимо:</p>
<pre class="brush:dart;">
Worker tom = Worker();
tom.company = "Apple";
tom.work();
</pre>
<p>В то же время мы можем указать, что данный тип будет использоваться только как миксин (если мы не планируем использовать его в качестве отдельного класса). 
В этом случае для опрделения типа применяется ключевое слово <span class="b">mixin</span>:</p>
<pre class="brush:dart;">
mixin Worker{
	String company = "";
	void work(){
