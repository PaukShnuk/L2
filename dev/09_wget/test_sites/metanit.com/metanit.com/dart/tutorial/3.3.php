...
<html >
<head>
<title>Dart | Возвращение результата из функции</title>
<meta charset="utf-8" />
<meta name="description" content="Возвращение результата из функции в языке программирования Dart, оператор return, особенности void-функций">
...
<h2>Возвращение результата из функции</h2><div class="date">Последнее обновление: 17.06.2019</div>
...
<p>Функция может возвращать определенное значение. Для этого используется оператор return. Например, возвратим из функции сумму двух чисел:</p>
<pre class="brush:dart;">
int sum(int a, int b){
	return a + b;
}
</pre>
<p>Возвращаемый тип ставится перед именем функции. То есть в данном случае функция возвращает значение типа int. После оператора return в теле функции указывается 
какое именно значение будет возвращено. В данном случае это результат выражения (a + b). Причем тип значения после оператора return должен соответствовать типу функции. 
В данном случае функция имеет тип int и возвращает сумму a+b, которая тоже представляет тип int.</p>
<p>Кроме того, оператор return завершает выполнение функции, поэтому все остальные инструкции в функции должны идти до оператора return.</p>
<p>Результат функции мы можем присвоить переменной и затем использовать в программе:</p>
<pre class="brush:dart;">
void main() {
...
<p>Здесь результат функции sum, то есть сумма двух чисел присваивается переменной c.</p>
<p>При определении функции мы можем не указывать возвращаемый тип. Тогда он выводится исходя из значения после оператора return:</p>
<pre class="brush:dart;">
void main() {
...
<p>Если функция не имеет оператора return, то в тело функции неявно добавляется инструкция <code>return null;</code>. Например, функция </p>
<pre class="brush:dart;">
sum(int a, int b){
	print(a + b);
}
</pre>
<p>Фактически будет эквивалентна</p>
<pre class="brush:dart;">
sum(int a, int b){
	print(a + b);
	return null;
...
<p>Если функция не должна возвращать никакого значения, то функция имеет тип void. Причем мы даже можем определить переменную типа void и присвоить ей 
результат void-функции, но ничего сделать c void-объектом мы не сможем:</p>
<pre class="brush:dart;">
void main() {
