...
<html >
<head>
<title>Dart | Статические члены классов</title>
<meta charset="utf-8" />
<meta name="description" content="Статические поля, константы и методы классов в языке программирования Dart">
...
<h2>Статические члены классов</h2><div class="date">Последнее обновление: 23.01.2021</div>
...
<p>Кроме обычных методов и полей класс может иметь статические поля, методы, константы. Значения таких полей, методов и констант относятся в целом ко всему классу, а 
не к отдельным объектам. Для определения статических переменных, констант и методов перед их объявлением указывается ключевое слово <span class="b">static</span>.</p>
<h3>Статические поля</h3>
<p>Статические поля хранят состояние всего класса. Статическое поле определяется как и обычное, только впереди ставится ключевое слово <span class="b">static</span>. Например, рассмотрим класс Employee, который представляет работника:</p>
...
<p>В классе работника определены поля для хранения имени и возраста - у каждого отдельного работника может быть свое имя и свой возраст. Кроме того, 
определено статическое поле для хранения пенсионного возраста retirementAge. Оно относится не к конкретному работнику, а ко всем работникам, то есть ко всему классу Employee в целом, так как нельзя для каждого отдельного работника 
установить свою пенсионную группу. Затем в самом классе мы можем использовать это поле как и любые другие.</p>
<p>Если мы хотим обратиться к этому полю вне класса, например, изменить его значение, то для обращения применяется имя класса:</p>
<pre class="brush:dart;">Employee.retirementAge = 65;</pre>
<h3>Статические константы</h3>
<p>Если пенсионный возраст менять не планируется, то мы можем определить как статическую константу:</p>
<pre class="brush:dart;">
void main (){
...
<p>Для определения статической константы можно использовать либо выражение <code>static const</code>, либо <code>static final</code>:</p>
<pre class="brush:dart;">
static const int retirementAge = 65;
static final int retirementAge = 65;
</pre>
<h3>Статические методы</h3>
<p>Статические методы также относятся ко всему классу и предваряются ключевым словом <code>static</code>. Как правило, статические методы выполняют такие вычисления, 
которые не затрагивают состояние или поведение объекта:</p>
<pre class="brush:dart;">
void main (){
...
<p>В данном случае для методов sum, subtract, multiply не имеет значения, какой именно экземпляр класса Operation используется. Эти методы работают только с параметрами, не затрагивая состояние класса. Поэтому их можно определить как статические.</p>
<p>Стоит отметить, что Google не рекомендует создавать классы, которые содержать только статические методы и переменные/константы, как выше определенный класс 
Operation. Так как в этом случае мы просто можем определить обычные функции и также их использовать:</p>
<pre class="brush:dart;">
void main (){
...
<p>Статические методы могут использоваться для управления статическими полями, то есть состоянием, которое относится ко всему классу. В то же время в статических методах мы не можем использовать 
нестатические методы и поля класса:</p>
<pre class="brush:dart;">
void main (){
...
class Employee{
	...
	static void setRetirementAge(int value){
...
<p>В данном случае статический метод setRetirementAge позволяет управлять статической переменной retirementAge.</p>
<p>В чем преимущества статических полей и методов перед нестатическими? Статические поля и методы потребляют меньше памяти, чем нестатические. Нестатическая переменная после инициализации (при первом присвоении 
...
Соответственно потреблять память они начинают только тогда, когда непосредственно начинают использоваться.</p>
