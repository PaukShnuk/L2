...
<html >
<head>
<title>Dart | Переопределение операторов</title>
<meta charset="utf-8" />
<meta name="description" content="Переопределение операторов в языке программирования Dart, списов перегружаемых операторов">
...
<h2>Переопределение операторов</h2><div class="date">Последнее обновление: 15.05.2020</div>
...
<p>Dart позволяет изменить поведение ряда встроенных операторов, что позволяет нам воспользоваться дополнительными возможностями 
по работе с объектами. Например, пусть у нас есть следующий класс Counter:</p>
...
<p>Данный класс представляет некоторый счетчик, значение которого хранится в поле value.</p>
<p>И допустим, у нас есть два объекта класса Counter - два счетчика, которые мы хотим сравнивать или складывать на основании их поля value, 
...
<p>Но по умолчанию ни операция сравнения, ни операция сложения для объектов Counter не доступны. Эти операции можно использовать 
с объектами примитивных типов, например, int. Но как складывать объекты классов, определенных разработчиком, Dart не знает. 
И для этого нам надо переопределить соответствующие операторы.</p>
<p>Переопределение операторов заключается в определении в классе, для объектов которого мы хотим определить оператор, специального метода:</p>
<pre class="brush:dart;">
возвращаемый_тип operator оператор(параметр) {  }
</pre>
<p>Возвращаемый тип представляет тот тип, объекты которого мы хотим получить. К примеру, в результате сложения двух объектов Counter 
мы ожидаем получить новый объект Counter. А в результате сравнения двух мы хотим получить объект типа bool, который указывает истинно ли 
условное выражение или ложно. Но в зависимости от задачи возвращаемые типы могут быть любыми. Затем вместо названия метода идет ключевое 
слово <span class="b">operator</span> и собственно сам оператор. И далее в скобках указывается параметр.</p>
<p>Например, перегрузим оператор сложения для класса Counter:</p>
<pre class="brush:dart;">
void main (){
...
<p>в случае с операцией сложения мы хотим сложить два объекта класса Counter, то оператор принимает в качестве параметра объект этого класса. И так как мы хотим в результате сложения получить новый объект Counter, то данный класс также используется в качестве возвращаемого типа. Все действия этого оператора сводятся к созданию, нового объекта, поле value которого объединяет значения поля value обоих объектов.</p>
<p>Следует отметить, что мы не можем переопределить абсолютно все имеющиеся в Dart операторы, а только некоторые из них, а именно: &lt; 	+ 	| 	[]  &gt; 	/ 	^ 	[]=  &lt;= 	~/ 	&amp; 	~  &gt;= 	* 	&lt;&lt; 	==  – 	% 	&gt;&gt; </p>
<p>Например, переопределим для класса Counter еще и операторы &gt; и &lt;:</p>
<pre class="brush:dart;">
void main (){
...
<p>Поскольку операции &lt; и &gt; возвращают значение типа bool, то и здесь при опеределении оператора возвращаемый тип bool, а фактическое действие 
оператора сводится к сравнению значений полей value обоих объектов.</p>
<p>Хотя в примерах выше в качестве параметра передавался объект того же типа - Counter, это необязательно, в зависимости от нашей задачи параметр 
может представлять любой тип. Например, изменим код следующим образом:</p>
<pre class="brush:dart;">
void main (){
...
<p>Теперь оператор сложения позволяет складывать объект Counter и обычное число и возвращает новый объект Counter. А оператор вычитания 
позволяет вычитать из Counter число и возвращает другое - разность поля value и вычитаемого числа.</p>
