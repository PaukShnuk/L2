...
<html >
<head>
<title>Dart | Абстрактные классы и методы</title>
<meta charset="utf-8" />
<meta name="description" content="Абстрактные классы и методы в языке программирования Dart, оператор abstract">
...
<h2>Абстрактные классы и методы</h2><div class="date">Последнее обновление: 23.01.2021</div>
...
<p>Абстрактные классы представляют классы, определенные с ключевым словом <span class="b">abstract</span>:</p>
<pre class="brush:dart;">
abstract class Figure {
}
</pre>
<p>Абстрактные классы похожи на обычные классы (также могут определять поля, методы, конструкторы) за тем исключением, что мы не можем создать напрямую объект абстрактного класса, используя его конструктор.</p>
<pre class="brush:dart;">
void main (){
...
<p>Как правило, абстрактные классы описывают сущности, которые в реальности не имеют конкретного воплощения. Например, геометрическая фигура может 
представлять круг, квадрат, треугольник, но как таковой геометрической фигуры самой по себе не существует. Есть конкретные фигуры, с которыми мы и 
работаем. В то же время все фигуры могут иметь какой-то общий функционал, например, методы вычисления периметра, площади и т.д.</p>
<pre class="brush:dart;">
void main (){
...
<p>Здесь абстрактный класс геометрической фигуры определяет метод <code>calculateArea()</code>, который выводит на консоль площадь фигуры. Класс 
прямоугольника определяет свою реализацию для этого метода.</p>
<h3>Абстрактные методы</h3>
<p>В примере выше метод calculateArea в базовом классе Figure не выполняет никакой полезной работы, так как у абстрактной фигуры не может быть площади. 
И в этом случае подобный метод лучше определить как абстрактный:</p>
<pre class="brush:dart;">
void main (){
...
<p>Абстрактный метод определяется также, как и обычный, только вместо тела метода после списка параметров идет точка с запятой: <code>void calculateArea();</code>.</p>
<p>Важно отметить, что абстрактные методы могут быть определены только в абстрактных классах. Кроме того, если базовый класс определяет абстрактный метод, то класс-наследник 
обязательно должен его реализовать, то есть определить тело метода.</p>
