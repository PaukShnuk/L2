...
<html >
<head>
<title>Dart | Фабричный конструктор</title>
<meta charset="utf-8" />
<meta name="description" content="Фабричный конструктор класса в языке программирования Dart, вызов конструктора не создает нового объекта, оператор factory">
...
<h2>Фабричный конструктор</h2><div class="date">Последнее обновление: 23.01.2021</div>
...
<p>По умолчанию конструкторы класса создают и возвращают новый объект этого класса. Но, возможно, не всегда потребуется создавать новый объект класса. 
Возможно, мы захотим использовать и возвращать из конструктора уже имеющийся объект. Для этой цели в языке Dart применяются фабричные конструкторы 
или конструкторы, которые предваряются ключевым словом <span class="b">factory</span>.</p>
...
<p>В данном случае определен класс Application, который представляет приложение. Допустим, наша задача, чтобы одновременно было запущено только одно приложение. 
Для этой цели определяем в классе статическую переменную app, которая представляет тип Application. И также определяем переменную name для хранения 
условного имени приложения.</p>
<p>В классе Application определен именованный конструктор <code>Application.fromName</code>, который устанавливает name.</p>
<p>И также определен фабричный конструктор:</p>
<pre class="brush:dart;">
factory Application(String name) {
        if(app.name == ""){
            app = Application.fromName(name);
...
<p>Фабричный конструктор, как и обычные конструкторы, может принимать параметры. В данном случае передаем в конструктор для установки name.</p>
<p>В самом конструкторе проверяем, если переменная name равна пустой строке, то создаем новый объект Application с помощью конструктора Application.fromName и 
присваиваем его переменной app. Если объект уже был создан, то просто выводим некоторую информацию. И в конце возвращаем созданный объект.</p>
<p>То есть фабричный конструктор создает новый объект Application, если его нет (переменная name представляет пустую строку), либо возвращает уже имеющийся. 
То есть мы получаем объект, который существует в единственном числе.</p>
<p>Также обратите внимание, что в фабричных конструкторах нельзя обращаться к текущему объекту через ключевое слово <span class="b">this</span>:</p>
<pre class="brush:dart;">
//print("В приложении ${this.name} открыта новая вкладка");
</pre>
<p>В функции main определяем две переменных это класса:</p>
<pre class="brush:dart;">
void main (){
...
Приложение Internet Browser запущено
В приложении Internet Browser открыта новая вкладка
Приложение Internet Browser
</pre>
</div>
<p>По консольному выводу мы видим, что конструктор Application.fromName, который создает новый объект Application, срабатывает только один раз. 
Также мы видим, что при установке name у второй переменной, меняется name и у первой переменной. Потому что в данном случае существует только один объект Application, 
а обе переменных - myApp1 и myApp2 указывают на один и тот же объект.</p>
<p>Где может применяться подобная техника? Прежде всего это различные варианты кэширования, когда вместо того, чтобы тратить дополнительные 
ресурсы на создание нового объекта, мы можем взять из кэша уже существующий. А если он отсутствует, то создать его.</p>
