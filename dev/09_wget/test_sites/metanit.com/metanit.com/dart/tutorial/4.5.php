...
<html >
<head>
<title>Dart | Реализация интерфейсов</title>
<meta charset="utf-8" />
<meta name="description" content="Реализация интерфейсов в языке программирования Dart, отличие от наследования, оператор implements">
...
<h2>Реализация интерфейсов</h2><div class="date">Последнее обновление: 23.01.2021</div>
...
<p>Наследование в языке Dart имеет важное ограничение: мы не можем наследовать класс сразу от нескольких классов, например, следующим образом:</p>
...
<p>Для решения этой проблемы в Dart применяется реализация интерфейсов. При этом важно понимать, что интерфейс - это не отдельная сущность, как в некоторых 
языках программирования (например, тип interface в C# или Java), а тот же класс. То есть класс в Dart одновременно выступает в роли интерфейса, и другой 
класс может реализовать данный интерфейс.</p>
<p>Интерфейс представляет синтаксический контракт, которому должны следовать реализующие этот интерфейс классы. То есть, если класс-интерфейс 
определяет какие-нибудь поля и методы, то класс, реализующий данный интерфейс, должен также определить эти поля и методы.</p>
<p>Для реализации интерфейсов применяется оператор <span class="b">implements</span>:</p>
<pre class="brush:dart;">
void main (){
...
<p>В данном случае класс Employee применяет интерфейс класса Person (<code>Employee implements Person</code>). Применение интерфейса класса Person означает, что класс Employee 
должен реализовать все поля и методы, которые определены в классе Person. Поэтому класс Employee должен определить поле name и метод display. По сути интерфейс - это 
контракт, что класс должен содержать определенный функционал (в данном случае поле name и метод display). Если класс Employee не определил бы все поля и методы, 
которые есть в классе Person, то мы столкнулись бы с ошибкой.</p>
<p>В то же время из приведенного выше примеры видно, что класс Employee не обязан реализовать конструктор и может вообще не определять никакого конструктора.</p>
<p>Подобным образом мы можем реализовать сразу несколько интерфейсов:</p>
<pre class="brush:dart;">
void main (){
...
<p>В данном случае класс Employee реализует интерфейс классов Person и Worker.</p>
<h3>Наследование классов vs реализация интерфейсов</h3>
<p>При наследовании производный класс не обязан определять те же поля и методы, которые есть в базовом классе (за исключением абстрактных методов). 
Если базовом классе определяется конструктор, то производный класс обязан определить свой конструктор, при котором вызывается конструктор базового класса. В производном классе 
мы можем обращаться к реализации базового класса с помощью ключевого слова <span class="b">super</span>. Не поддерживается множественное наследование.</p>
<p>При реализации интерфейса производный класс должен определить все поля и методы, которые определены в классе интерфейса. Если в базовом есть конструктор, 
то производный класс НЕ обязан определять свой конструктор. В производном классе мы НЕ можем обращаться к методам реализованного интерфейса с помощью ключевого слова <span class="b">super</span>. 
Поддерживается множественная реализация интерфейсов.</p>
