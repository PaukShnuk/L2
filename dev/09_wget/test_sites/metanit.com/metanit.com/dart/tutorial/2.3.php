...
<html >
<head>
<title>Dart | Примитивные типы данных</title>
<meta charset="utf-8" />
<meta name="description" content="Примитивные типы данных на языке программирования Dart, int, bool, double, String, Runes, Symbol, интерполяция строк">
...
<h2>Примитивные типы данных</h2><div class="date">Последнее обновление: 21.01.2021</div>
...
<p>Тип данных определяет, какие именно значения может хранить объект. Dart имеет ряд примитивных встроенных типов:</p>
<ul>
<li><p>логический тип bool</p></li>
<li><p>Числовые типы (int, double)</p></li>
<li><p>Строковые типы (String, Runes)</p></li>
<li><p>Symbol</p></li>
</ul>
<h3>bool</h3>
<p>Тип <span class="b">bool</span> представляет два значения: <span class="b">true</span> и <span class="b">false</span>.</p>
<pre class="brush:dart;">
var isEnabled = false;
bool isAlive = true;
</pre>
<h3>int</h3>
<p>Тип <span class="b">int</span> представляет целые числа, которые занимают не более 64 бит, точный размер зависит от платформы. Например, для Dart VM значения варьируются от -2<sup>63</sup> до 2<sup>63</sup> - 1. 
Dart, который компилируется в JavaScript, использует систему числовых типов JavaScript с диапазоном значений от -2<sup>53</sup> до 2<sup>53</sup> - 1.</p>
<p>Любые целые числа трактуются как значения типа int:</p>
<pre class="brush:dart;">
int x = 8;
var y = -5;
</pre>
<h3>double</h3>
<p>Тип <span class="b">double</span> представляет числа с плавающей точкой, которые занимают в памяти 64 бита.</p>
<p>Все дробные числа с точкой в качестве разделителя целой и дробной части трактуются как значения типа double:</p>
<pre class="brush:dart;">
double x = 8.8;
var y = -5.3;
var z = 0.09;
</pre>
<p>Переменной типа double также можно присвоить целочисленное значение, в этом случае оно автоматически преобразуется в дробное:</p>
<pre class="brush:dart;">
double x = 8;	// x = 8.0
print(x);	// 8.0
</pre>
<h3>String</h3>
<p>Строки представлены типом <span class="b">String</span>, который представляет последовательность символов в кодировке UTF-16. Для определения строки 
можно использовать одинарные и двойные кавычки:</p>
<pre class="brush:dart;">
String tom = "Tom";
String sam = 'Sam';
var kate = "Kate";
...
<p>Если необходимо определить многострочную строку, то она заключается в тройные кавычки:</p>
<pre class="brush:dart;">
void main() {
	var multiline = '''
		Многострочная
...
<p>С помощью интерполяции мы можем вводить в строку значения других переменных:</p>
<pre class="brush:dart;">
String name = "Tom";
int age = 35;
String info = "Name: $name  Age: $age";
</pre>
<p>Для ввода значений в строку перед переменной ставится знак доллара $ ($name, $age), в итоге вместо $name будет вставляться значение переменной name, 
а вместо $age - значение переменной $age.</p>
<h3>Runes</h3>
<p>Тип <span class="b">Runes</span> также представляет строки, но в отличие от String, Runes - это последовательность символов в кодировке Unicode. Поскольку по умолчанию все строки в 
кавычках (как одинарных, так и двойных) представляют тип String, то для определения переменной Runes требуется специальный синтаксис:</p>
<pre class="brush:dart;">
void main() {
...
<p>Для определения значения типа Runes применяется конструктор данного типа. То есть в начале идет название типа 
- Runes, после которого в скобках указывается передаваемый конструктору аргумент. Мы можем передать в конструктор либо набор кодовых единиц в кодировке UTF-16 
(new Runes('\u041F\u0440\u0438\u0432\u0435\u0442')), либо обычный набор символов (new Runes('Привет')).</p>
<p>Чтобы получить текстовое представление объекта Runes, применяется метод <span class="b">String.fromCharCodes</span>.</p>
<h3>Symbol</h3>
<p>Тип <span class="b">Symbol</span>, наименее используемый из всех типов, представляет символьные идентификаторы, которые, 
как правило, применяются для ссылки на какие-то элементы API, например, библиотеки и классы. Для определения объекта этого 
типа применяется симол решетки #:</p>
<pre class="brush:dart;">
...
<div class="nav"><p><a href="./2.2.php">Назад</a><a href="./">Содержание</a><a href="./2.10.php">Вперед</a></p></div></div>
<div class="socBlock">
