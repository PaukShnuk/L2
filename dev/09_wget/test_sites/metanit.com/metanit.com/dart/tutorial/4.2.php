...
<html >
<head>
<title>Dart | Константы и константные конструкторы классов</title>
<meta charset="utf-8" />
<meta name="description" content="Константы и константные конструкторы классов в языке программирования Dart, оператор final">
...
<h2>Константы и константные конструкторы классов</h2><div class="date">Последнее обновление: 23.01.2021</div>
...
<h3>Константы классов</h3>
<p>Кроме обычных полей-переменных классы могут определять константы. Для определения констант можно использовать ключевое слово <span class="b">final</span>:</p>
...
<p>Значения констант классов должны быть установлены до вызова конструктора. Это может происходить при определении констант, как в примере выше:</p>
<pre class="brush:dart;">
final minAge = 1;
</pre>
<p>Другим вариантом является установка значения константы через параметр констуктора:</p>
<pre class="brush:dart;">
void main (){
...
<p>Третим вариантом является установка значений констант с помощью инициализатора:</p>
<pre class="brush:dart;">
void main (){
...
<p>В данном случае константа получает значение параметра minAge + 1, то есть значение констант могут представлять динамически вычисляемые значения.</p>
<h3>Константные конструкторы</h3>
<p>Классы могут содержать константные конструкторы. Такие конструкторы призваны создавать объекты, которые не должны изменяться. 
Константные конструкторы предваряются ключевым словом <span class="b">const</span>. Класс, который определяет подобный конструктор, 
не должен содержать переменных, но может определять константы. Кроме того, константые конструкторы не имеют тела:</p>
<pre class="brush:dart;">
void main (){
...
<p>При вызове константного конструктора перед его вызовом ставится ключевое слово <span class="b">const</span>: <code>Person tom = const Person("Tom", 23);</code></p>
