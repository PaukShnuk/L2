...
<html >
<head>
<title>Dart | Арифметические операции</title>
<meta charset="utf-8" />
<meta name="description" content="Арифметические операции на языке программирования Dart">
...
<h2>Арифметические операции</h2><div class="date">Последнее обновление: 13.05.2020</div>
...
<p>Dart поддерживает все распространенные арифметические операции. В арифметических операциях участвуют числами. Есть бинарные арифметические операции 
(производятся над двумя операндами) и унарные (выполняются над одним операндом). К бинарным операциям относят следующие:</p>
<ul>
<li><p><code>+</code></p>
<p>операция сложения двух чисел:</p>
...
<li><p><code>-</code></p>
<p>операция вычитания двух чисел:</p>
<pre class="brush:dart;">
int a = 10;
int b = 7;
int c = a - b;  // 3
...
<li><p><code>*</code></p><p>операция умножения двух чисел</p>
<pre class="brush:dart;">
int a = 10;
int b = 7;
int c = a * b;  // 70
...
<li><p><code>/</code></p><p>операция деления двух чисел:</p>
<pre class="brush:dart;">
int a = 20;
int b = 5;
double c = a / b;  // 4.0
...
<p>При делении стоит учитывать, что результат операции всегда представляет тип <span class="b">double</span>.</p>
</li>
<li><p><code>~/</code></p><p>операция целочисленного деления двух чисел. Результат операции всегда представляет тип <span class="b">int</span>:</p>
<pre class="brush:dart;">
int a = 33;
int b = 5;
int c = a ~/ b;  // 6
...
<li><p><code>%</code></p><p>получение остатка от деления двух чисел:</p>
<pre class="brush:dart;">
int a = 33;
int b = 5;
int c = a % b;  // 3
...
<p>Также есть две унарные арифметические операции, которые производятся над одним числом: ++ (инкремент) и -- (декремент). Каждая из операций имеет две разновидности: префиксная и постфиксная:</p>
<ul>
<li><p><span class="b">++ (префиксный инкремент)</span></p>
<p>Предполагает увеличение переменной на единицу, например, <code>z=++y</code> (вначале значение переменной y увеличивается на 1, 
а затем ее значение присваивается переменной z)</p>
<pre class="brush:dart;">
int a = 8;
int b = ++a;
print(a);	// 9
...
<li><p><span class="b">++ (постфиксный инкремент)</span></p>
<p>Также представляет увеличение переменной на единицу, например, <code>z=y++</code> (вначале значение переменной y присваивается 
переменной z, а потом значение переменной y увеличивается на 1)</p>
<pre class="brush:dart;">
int a = 8;
int b = a++;
print(a);	// 9
...
<li><p><code>-- (префиксный декремент)</code></p>
<p>уменьшение переменной на единицу, например, <code>z=--y</code> (вначале значение переменной y уменьшается на 1, 
а потом ее значение присваивается переменной z)</p>
<pre class="brush:dart;">
int a = 8;
int b = --a;
print(a);	// 7
...
<li><p><code>-- (постфиксный декремент)</code></p>
<p><code>z=y--</code> (сначала значение переменной y присваивается переменной z, а затем значение переменной y  уменьшается на 1)</p>
<pre class="brush:dart;">
int a = 8;
int b = a--;
print(a);	// 7
...
<h3>Приоритет арифметических операций</h3>
<p>Одни операции имеют больший приоритет чем другие и поэтому выполняются вначале. Операции в порядке уменьшения приоритета:</p>
<p>++ (постфиксный инкремент), -- (постфиксный декремент)</p>
<p>++ (префиксный инкремент), -- (префиксный декремент)</p>
<p>* (умножение), / (деление), % (остаток от деления)</p>
<p>+ (сложение), - (вычитание)</p>
<p>Приоритет операций следует учитывать при выполнении набора арифметических выражений:</p>
<pre class="brush:dart;">
int a = 8;
int b = 7;
int c = a + 5 * ++b;      
...
<p>Вначале будет выполняться операция инкремента <code>++b</code>, которая имеет больший приоритет - она увеличит значение переменной b и возвратит его в 
качестве результата. Затем выполняется умножение <code>5 * ++b</code>, и только в последнюю очередь выполняется сложение <code>a + 5 * ++b</code></p>
<p>Скобки позволяют переопределить порядок вычислений:</p>
<pre class="brush:dart;">
int a = 8;
int b = 7;
int c = (a + 5) * ++b;    
...
<p>Хотя операция сложения имеет меньший приоритет, но вначале будет выполняться именно сложение, а не умножение, так как операция сложения заключена в скобки.</p>
