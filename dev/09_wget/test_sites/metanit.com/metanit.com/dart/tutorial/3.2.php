...
<html >
<head>
<title>Dart | Параметры функции</title>
<meta charset="utf-8" />
<meta name="description" content="Параметры функции в языке программирования Dart, именнованные и позиционные необязательные параметры">
...
<h2>Параметры функции</h2><div class="date">Последнее обновление: 22.01.2021</div>
...
<p>Функция может принимать параметры. Параметры перечисляются в скобках после имени функции имеют следующее определение:</p>
<pre class="brush:dart;">тип название_параметра</pre>
<p>Например, определим функцию, которая складывает два числа:</p>
...
<p>Функция sum принимает два параметра, которые имеют имена a и b и представляют тип <code>int</code>. В функции main вызываем функцию sum и передаем ей два аргумента. 
...
аргумент - второму параметру и так далее. В данном случае параметр a в качестве значения получит число 2, а параметр b - число 5.</p>
<p>Количество передаваемых аргументов должно соответствовать количеству параметров. Кроме того между типами аргументов и типами параметров должно быть соответствие. 
Например, определим еще одну функцию:</p>
<pre class="brush:dart;">
void main() {
...
<p>Функция showPerson принимает два параметра. Первый параметр представляет тип String, а второй представляет тип int. Поэтому при вызове функции 
необходимо сначала передать значение типа String (то есть строку), а затем значение типа int (то есть целое число)</p>
<p>При определении функции мы можем не указывать тип параметров. Тогда Dart сам выводит их тип исходя из передаваемых значений:</p>
<pre class="brush:dart;">
void main() {
...
<p>Обратите внимание, что теперь при определении функции неизвестно, какой именно тип представляют параметры name и age. Поэтому во втором вызове функции 
<code>showPerson(29, "Alice")</code> Dart будет трактовать параметр name как параметр типа int, потому что ему передается целое число. А параметр age будет трактоваться как строка.</p>
<h3>Необязательные параметры</h3>
<p>Ряд параметров мы можем сделать необязательными, то есть мы можем не передавать для них никаких значений. Для этого параметр заключается в квадратные скобки. Но при этом ему необходимо предоставить 
значение по умолчанию, которое будет использоваться, если параметру не передано никакого значения:</p>
<pre class="brush:dart;">
void main() {
...
<p>В данном случае параметр age является необязательным. Если мы не передадим ему никакого значения, то он по умолчанию будет иметь значение 22. 
Консольный вывод:</p>
...
<p>Однако если необязательный параметр представляет nullable-тип, то есть может принимать значение null, тогда можно не указывать для него значение по умолчанию - 
в этом случае значение по умолчанию будет <span class="b">null</span>:</p>
<pre class="brush:dart;">
void main() {
...
<p>Dart поддерживает передачу параметров по имени. Например:</p>
<pre class="brush:dart;">
void main() {
...
<p>При определении функции список параметров заключается в фигурные скобки, при этом для каждого параметра, если его тип не поддерживает значение null, 
указывается значение по умолчанию.</p>
<p>При вызове функции указывается имя параметра и через двоеточие его значение (<code>name: "Tom"</code>), при этом мы можем указывать параметры в любой последовательности. 
Например, во втором вызове вначале указывается значение для параметра age, а потом для параметра name (<code>showPerson(age: 29, name: "Alice")</code>). 
Более того мы можем вообще не передавать параметрам значение, тогда подобный параметр будет иметь значение по умолчанию, как в случае с параметром age в третьем вызове (<code>showPerson(name: "Kate")</code>).</p>
<p>Консольный вывод:</p>
<div class="console"><pre class="consoletext">
Name: Tom
Age: 35
...
<p>Если параметр представляет nullable-тип, то есть поддерживает значение null, то мы можем не указывать значение по умолчанию, тогда значением по умолчанию будет null:</p>
<pre class="brush:dart;">
void main() {
...
<h3>Константы в функциях</h3>
<p>Параметры позволяют передать значения локальным переменным, которые определены в функциях. Но также можно передавать через параметры значения локальным <span class="b">final</span>-константам, поскольку 
они определяются во время выполнения. Но при этом мы не можем передавать значения <span class="b">const</span>-константам, так как их значения должны известны 
уже во время компиляции. В связи с чем при разных вызовах функции мы можем передать <span class="b">final</span>-константам разные значения:</p>
<pre class="brush:dart;">
void main() {
