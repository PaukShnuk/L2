...
<html >
<head>
<title>Dart | Оператор throw и создание своих типов исключений</title>
<meta charset="utf-8" />
<meta name="description" content="Оператор throw и генерация исключения в языке программирования Dart, создание своих типов исключений">
...
<h2>Оператор throw и создание своих типов исключений</h2><div class="date">Последнее обновление: 24.01.2021</div>
...
<p>Оператор <span class="b">throw</span> позволяет нам самим сгенерировать исключение в необходимом месте. Рассмотрим следующий код:</p>
...
<p>Например, при создании нового пользователя - объекта класса Person мы хотим предупредить ситуацию, когда в конструктор для возраста 
передается недопустимое (например, отрицательное значение). В этом случае мы можем с помощью оператора throw сгененировать исключение:</p>
<pre class="brush:dart;">throw Exception("Недопустимый возраст");</pre>
<p>В конструктор Exception передается сообщение, которое можно получить при обработке исключения.</p>
<p>Таким образом, при выполнении следующей строи в блоке try будет возникать исключение:</p>
<pre class="brush:dart;">
Person tom = Person("Tom", -45);
</pre>
<p>А в блоке on..catch можно обработать это исключение:</p>
<pre class="brush:dart;">
on Exception catch(e){
	print(e);
}
</pre>
<p>С помощью аргумента e в блоке catch мы можем получить сообщение об ошибке. Консольный вывод:</p>
<div class="console">
...
<h3>Создание своих исключений</h3>
<p>Встроенная библиотека Dart обеспечивает большим количеством разных типов исключений. Тем не менее для каких-то отдельных ситуаций, 
возможно, мы захотим создать свои собственные типы исключений. Например, в примере выше с возрастом пользователя мы можем все действия оформить 
в виде отдельного исключения. Например:</p>
<pre class="brush:dart;">
void main (){
...
<p>Здесь определен класс исключения AgeException, который реализует интерфейс типа Exception. (Фактически мы можем определить класс без реализации Exception). 
И в этом классе определен свой метод getErrorMessage, который возвращает сообщение об ошибке.</p>
<p>При обработке исключения мы можем использовать этот метод для вывода сообщения об ошибке:</p>
<pre class="brush:dart;">
on AgeException catch(e){
	print(e.getErrorMessage());
}
</pre>
<p>В данном случае параметр e в catch представляет объект AgeException, у которого можно вызвать метод getErrorMessage.</p>
