...
<html >
<head>
<title>Java | Арифметические операции</title>
<meta charset="utf-8" />
<meta name="description" content="Арифметические операции в языке программирования Java и их приоритеты: сложение, умножение, деление, вычитание, инкремент и декремент, постфиксный и префиксный">
...
<h2>Арифметические операции</h2><div class="date">Последнее обновление: 30.10.2018</div>
...
<p>Большинство операций в Java аналогичны тем, которые применяются в других си-подобных языках. Есть унарные операции (выполняются над одним операндом), 
...
участвующее в операции. Рассмотрим все виды операций.</p>
<p>В арифметических операциях участвуют числа. В Java есть бинарные арифметические операции (производятся над двумя операндами) и унарные (выполняются над одним операндом). К бинарным операциям относят 
следующие:</p>
<ul>
<li><p><code>+</code></p>
...
<p>При делении стоит учитывать, что если в операции участвуют два целых числа, то результат деления будет округляться до целого числа, даже если 
результат присваивается переменной float или double:</p>
<pre class="brush:java;">
double k = 10 / 4;     // 2
...
<h3>Ассоциативность операций</h3>
<p>Кроме приоритета операции отличаются таким понятием как 
<span class="b">ассоциативность</span>. Когда операции имеют один и тот же приоритет, порядок вычисления определяется 
ассоциативностью операторов. В зависимости от ассоциативности есть два типа операторов:</p>
<ul>
...
<p>Так, некоторые операции, например, операции умножения и деления, имеют один и тот же приоритет. Какой же тогда будет результат в выражении:</p>
<pre class="brush:java;">int x = 10 / 5 * 2;</pre>
<p>Стоит нам трактовать это выражение как <code>(10 / 5) * 2</code> или как <code>10 / (5 * 2)</code>? Ведь в зависимости от трактовки мы получим разные результаты.</p>
<p>Все арифметические операторы (кроме префиксного инкремента и декремента) являются левоассоциативными, то есть выполняются слева направо. Поэтому выражение <code>10 / 5 * 2</code> необходимо трактовать как <code>(10 / 5) * 2</code>, 
то есть результатом будет 4.</p>
<h3>Операции с числами с плавающей точкой</h3>
<p>Следует отметить, что числа с плавающей точкой не подходят для финансовых и других вычислений, где ошибки при округлении могут быть критичными. Например:</p>
<pre class="brush:java;">
double d = 2.0 - 1.1;
System.out.println(d);
</pre>
<p>В данном случае переменная d будет равна не 0.9, как можно было бы изначально предположить, а 0.8999999999999999. 
...
Поэтому в таких случаях обычно применяется класс BigDecimal, который позволяет обойти подобные ситуации.</p>
