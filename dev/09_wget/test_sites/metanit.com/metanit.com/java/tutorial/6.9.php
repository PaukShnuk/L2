...
<html >
<head>
<title>Java | Буферизация символьных потоков. BufferedReader и BufferedWriter</title>
<meta charset="utf-8" />
<meta name="description" content="Считывание текстового файла в буфер и запись из буфера в языке программирования Java с помощью потоков BufferedReader и BufferedWriter">
...
<h2>Буферизация символьных потоков. BufferedReader и BufferedWriter</h2><div class="date">Последнее обновление: 26.04.2018</div>
...
<h3>Запись текста через буфер и BufferedWriter</h3>
<p>Класс <span class="b">BufferedWriter</span> записывает текст в поток, предварительно буферизируя записываемые символы, тем самым снижая количество 
обращений к физическому носителю для записи данных.</p>
<p>Класс BufferedWriter имеет следующие конструкторы:</p>
...
<h3>Чтение текста и BufferedReader</h3>
<p>Класс <span class="b">BufferedReader</span> считывает текст из символьного потока ввода, буферизируя прочитанные символы. Использование буфера 
призвано увеличить производительность чтения данных из потока.</p>
<p>Класс BufferedReader имеет следующие конструкторы:</p>
<pre class="brush:java;">
...
<p>Так как BufferedReader наследуется от класса <code>Reader</code>, то он может использовать все те методы для чтения из потока, которые определены в 
Reader. И также BufferedReader определяет свой собственный метод <span class="b">readLine()</span>, который позволяет считывать из потока построчно.</p>
<p>Рассмотрим применение BufferedReader:</p>
<pre class="brush:java;">
import java.io.*;
...
<h3>Считывание с консоли в файл</h3>
<p>Соединим оба класса BufferedReader и BufferedWriter для считывания с консоли в файл. Для этого определим следующий код программы:</p>
<pre class="brush:java;">
import java.io.*;
...
<p>Здесь объект BufferedReader устанавливается для чтения с консоли с помощью объекта <code>new InputStreamReader(System.in)</code>. 
В цикле while считывается введенный текст. И пока пользователь не введет строку "ESC", объект BufferedWriter будет записывать текст файл.</p>
