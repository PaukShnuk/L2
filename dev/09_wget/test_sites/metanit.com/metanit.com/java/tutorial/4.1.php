...
<html >
<head>
<title>Java | Обработка исключений</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка исключений в Java, использование оператора throws, конструкция try...catch">
...
<h1>Обработка исключений</h1><div class="date">Последнее обновление: 23.04.2018</div>
...
<p>В теме <a href="2.10.php">Введение в обработку исключений</a> были рассмотрены некоторые основные моменты обработки исключений. Сейчас более 
подробно поговорим о других моментах, связанных с обработкой исключений.</p>
<h2>Оператор throws</h2>
<p>Иногда метод, в котором может генерироваться исключение, сам не обрабатывает это исключение. В этом случае в объявлении метода используется 
оператор <span class="b">throws</span>, который надо обработать при вызове этого метода. Например, у нас имеется метод вычисления факториала, 
и нам надо обработать ситуацию, если в метод передается число меньше 1:</p>
...
<p>С помощью оператора <code>throw</code> по условию выбрасывается исключение. В то же время метод сам это исключение не обрабатывает с помощью 
try..catch, поэтому в определении метода используется выражение <code>throws Exception</code>.</p>
...
<p>В качестве альтернативы мы могли бы и не использовать оператор throws, а обработать исключение прямо в методе:</p>
<pre class="brush:java;">
...
<div class="nav"><p><a href="3.18.php">Назад</a><a href="./">Содержание</a><a href="4.2.php">Вперед</a></p></div></div>
<div class="socBlock">
