...
<html >
<head>
<title>Java | Ограничения обобщений</title>
<meta charset="utf-8" />
<meta name="description" content="Ограничения обобщений (generics) в языке программирования Java, интерфейсы в качестве ограничений, примеры использования">
...
<h2>Ограничения обобщений</h2><div class="date">Последнее обновление: 23.04.2018</div>
...
<p>Когда мы указываем универсальный параметр у обобщений, то по умолчанию он может представлять любой тип. Однако иногда необходимо, чтобы параметр соответствовал только 
некоторому ограниченному набору типов. В этом случае применяются ограничения, которые позволяют указать базовый класс, которому должен соответствовать параметр.</p>
<p>Для установки ограничения после универсального параметра ставится слово <span class="b">extends</span>, после которого указывается базовый класс ограничения:</p>
<pre class="brush:java;">
class Account{ }
class Transaction&lt;T extends Account&gt;{ }
</pre>
<p>К примеру, в данном случае для параметра T в Transaction ограничением является класс Account. То есть на место параметра T мы можем передать либо класс Account, либо 
один из его классов-наследников.</p>
...
<p>В данном случае класс Transaction, который представляет операцию перевода средств между двумя счетами, типизирован параметром T, у которого в качестве 
ограничения установлен класс Account. При создании объекта Transaction в его конструктор передаются два объекта Account - два счета, между которыми надо 
осуществить перевод, и сумма перевода.</p>
<p>При этом важно понимать, что поскольку мы установили подобное ограничение, то компилятор будет распознавать объекты типа T как объекты типа Account. 
И в этом случае мы можем вызывать у объектов типа T методы класса Account. И мы бы не смогли бы это сделать, если бы мы не задали подобного ограничения:</p>
<pre class="brush:java;">
class Transaction&lt;T&gt;{
...
<h3>Обобщенные типы в качестве ограничений</h3>
...
<p>В данном случае ограничением для Transaction является тип Account, который типизирован типом String.</p>
<h3>Интерфейсы в качестве оганичений</h3>
...
<p>Также можно установить сразу несколько ограничений. Например, пусть класс Transaction может работать только с объектами, которые одновременно 
реализуют интерфейс Accountable и являются наследниками класса Person:</p>
<pre class="brush:java;">
class Person{}
interface Accountable{}
...
<div class="nav"><p><a href="3.11.php">Назад</a><a href="./">Содержание</a><a href="3.15.php">Вперед</a></p></div></div>
<div class="socBlock">
