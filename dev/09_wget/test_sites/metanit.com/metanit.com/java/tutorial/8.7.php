...
<html >
<head>
<title>Java | Обмен между потоками. Класс Exchanger</title>
<meta charset="utf-8" />
<meta name="description" content="Обмен между потоками в языке программирования Java, класс Exchanger">
...
<h2>Обмен между потоками. Класс Exchanger</h2><div class="date">Последнее обновление: 28.04.2018</div>
...
<p>Класс <span class="b">Exchanger</span> предназначен для обмена данными между потоками. Он является типизированным и типизируется типом данных, которыми потоки должны обмениваться.</p>
<p>Обмен данными производится с помощью единственного метода этого класса <span class="b">exchange()</span>:</p>
<pre class="brush:java;">
V exchange(V x) throws InterruptedException
V exchange(V x, long timeout, TimeUnit unit) throws InterruptedException, TimeoutException
</pre>
<p>Параметр x представляет буфер данных для обмена. Вторая форма метода также определяет параметр timeout - время ожидания и unit - тип временных единиц, применяемых для параметра timeout.</p>
...
<p>В классе PutThread отправляет в буфер сообщение "Hello Java!":</p>
<pre class="brush:java;">message=exchanger.exchange(message);</pre>
<p>Причем в ответ метод exchange возвращает данные, которые отправил в буфер другой поток. То есть происходит обмен данными. Хотя нам необязательно 
получать данные, мы можем просто их отправить:</p>
<pre class="brush:java;">exchanger.exchange(message);</pre>
<p>Логика класса GetThread аналогична - также отправляется сообщение.</p>
