...
<html >
<head>
<title>Java | Создание и выполнение потоков</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и выполнение потоков в языке программирования Java, наследование от класса Thread, интерфейс Runnable, ожидание окончания потока с помощью метода join, выход из потока">
...
<h2>Создание и выполнение потоков</h2><div class="date">Последнее обновление: 27.04.2018</div>
...
<p>Для создания нового потока мы можем создать новый класс, либо наследуя его от класса Thread, либо реализуя в классе интерфейс <span class="b">Runnable</span>.</p>
<h3>Наследование от класса Thread</h3>
<p>Создадим свой класс на основе Thread:</p>
...
<p>Класс потока называется JThread. Предполагается, что в конструктор класса передается имя потока, которое затем передается 
в конструктор базового класса. В конструктор своего класса потока мы можем передать различные данные, но главное, чтобы в нем вызывался конструктор 
базового класса Thread, в который передается имя потока.</p>
<p>И также в JThread переопределяется метод <span class="b">run()</span>, код которого собственно и будет представлять весь тот код, который 
выполняется в потоке.</p>
<p>В методе main для запуска потока JThread у него вызывается метод <span class="b">start()</span>, после чего начинается выполнение того кода, который определен в методе run:</p>
<pre class="brush:java;">
new JThread("JThread").start();
</pre>
...
<p>Здесь в методе main в конструктор JThread передается произвольное название потока, и затем вызывается метод <code>start()</code>. По сути этот метод 
как раз и вызывает переопределенный метод <code>run()</code> класса JThread.</p>
<p>Обратите внимание, что главный поток завершает работу раньше, чем порожденный им дочерний поток JThread.</p>
...
<h3>Ожидание завершения потока</h3>
<p>При запуске потоков в примерах выше Main thread завершался до дочернего потока. Как правило, более распространенной ситуацией является случай, 
когда Main thread завершается самым последним. Для этого надо применить метод <span class="b">join()</span>. В этом случае текущий поток будет ожидать завершения потока, для которого вызван метод join:</p>
<pre class="brush:java;">
public static void main(String[] args) {
...
<p>Метод <code>join()</code> заставляет вызвавший поток (в данном случае Main thread) ожидать завершения вызываемого потока, для которого и применяется 
метод join (в данном случае JThread).</p>
<p>Консольный вывод:</p>
<div class="console">
<pre class="consoletext">
Main thread started...
...
<p>Если в программе используется несколько дочерних потоков, и надо, чтобы Main thread завершался после дочерних, то для каждого дочернего потока надо вызвать метод join.</p>
<h3>Реализация интерфейса Runnable</h3>
<p>Другой способ определения потока представляет реализация интерфейса <span class="b">Runnable</span>. Этот интерфейс имеет один метод <span class="b">run</span>:</p>
<pre class="brush:java;">
interface Runnable{
...
<p>После определения объекта Runnable он передается в один из конструкторов класса Thread:</p>
<pre class="brush:java;">Thread(Runnable runnable, String threadName)</pre>
<p>Для реализации интерфейса определим следующий класс MyThread:</p>
<pre class="brush:java;">
...
<p>Реализация интерфейса Runnable во многом аналогична переопределению класса Thread. Также в методе <code>run</code> определяется простейший код, 
который усыпляет поток на 500 миллисекунд.</p>
<p>В методе main вызывается конструктор Thread, в который передается объект MyThread. И чтобы запустить поток, вызывается метод <code>start()</code>. 
В итоге консоль выведет что-то наподобие следующего:</p>
<div class="console">
<pre class="consoletext">
Main thread started...
Main thread finished...
...
<p>Поскольку Runnable фактически представляет функциональный интерфейс, который определяет один метод, то объект этого интерфейса мы можем 
представить в виде лямбда-выражения:</p>
<pre class="brush:java;">
public class Program {
...
<div class="nav"><p><a href="8.1.php">Назад</a><a href="./">Содержание</a><a href="8.4.php">Вперед</a></p></div></div>
<div class="socBlock">
