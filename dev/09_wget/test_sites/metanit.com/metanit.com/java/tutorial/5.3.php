...
<html >
<head>
<title>Java | Класс LinkedList</title>
<meta charset="utf-8" />
<meta name="description" content="Класс LinkedList в языке программирования Java, создание связанного списка, интерфейсы Queue и Dequeue и их методы">
...
<h2>Класс LinkedList</h2><div class="date">Последнее обновление: 24.04.2018</div>
...
<p>Обобщенный класс LinkedList&lt;E&gt; представляет структуру данных в виде связанного списка. Он наследуется от класса <code>AbstractSequentialList</code> и реализует интерфейсы <code>List, Dequeue</code> и <code>Queue</code>. 
То есть он соединяет функциональность работы со списком и фукциональность очереди.</p>
<p>Класс LinkedList имеет следующие конструкторы:</p>
<ul>
<li><p><code>LinkedList()</code>: создает пустой список</p></li>
<li><p><code>LinkedList(Collection&lt;? extends E&gt; col)</code>: создает список, в который добавляет все элементы коллекции col</p></li>
</ul>
<p>LinkedList содержит все те методы, которые определены в интерфейсах List, Queue, Deque. Некоторые из них:</p>
<ul>
<li><p><span class="b">addFirst() / offerFirst()</span>: добавляет элемент в начало списка</p></li>
<li><p><span class="b">addLast() / offerLast()</span>: добавляет элемент в конец списка</p></li>
<li><p><span class="b">removeFirst() / pollFirst()</span>: удаляет первый элемент из начала списка</p></li>
<li><p><span class="b">removeLast() / pollLast()</span>: удаляет последний элемент из конца списка</p></li>
<li><p><span class="b">getFirst() / peekFirst()</span>: получает первый элемент</p></li>
<li><p><span class="b">getLast() / peekLast()</span>: получает последний элемент</p></li>
</ul>
...
<p>Здесь создаются и используются два списка: для строк и для объектов класса Person. При этом в дополнение к методам addFirst/removeLast и т.д., 
нам также доступны стандартные методы, определенные в интерфейсе Collection: <code>add()</code>, <code>remove</code>, <code>contains</code>, 
<code>size</code> и другие. Поэтому мы можем использовать разные методы для одного и того же действия. Например, добавление в самое начало списка 
можно сделать так: <code>states.addFirst("Spain");</code>, а можно сделать так: <code>states.add(0, "Spain");</code></p>
...
<div class="nav"><p><a href="5.7.php">Назад</a><a href="./">Содержание</a><a href="5.4.php">Вперед</a></p></div></div>
<div class="socBlock">
