...
<html >
<head>
<title>Java | Работа со строками. Класс String</title>
<meta charset="utf-8" />
<meta name="description" content="Работа со строками в языке Java, класс String и его методы">
...
<h1>Работа со строками</h1><h2>Введение в строки. Класс String</h2><div class="date">Последнее обновление: 31.10.2018</div>
...
<p>Строка представляет собой последовательность символов. Для работы со строками в Java определен класс String, который предоставляет ряд методов для манипуляции строками. 
Физически объект String представляет собой ссылку на область в памяти, в которой размещены символы.</p>
<p>Для создания новой строки мы можем использовать один из конструкторов класса String, либо напрямую присвоить строку в двойных кавычках:</p>
...
<p>При работе со строками важно понимать, что объект String является неизменяемым (<span class="b">immutable</span>). То есть при любых операциях 
над строкой, которые изменяют эту строку, фактически будет создаваться новая строка.</p>
<p>Поскольку строка рассматривается как набор символов, то мы можем применить метод <span class="b">length()</span> для нахождения длины строки или длины набора символов:</p>
<pre class="brush:java;">
String str1 = "Java";
System.out.println(str1.length()); // 4
</pre>
<p>А с помощью метода <span class="b">toCharArray()</span> можно обратно преобразовать строку в массив символов:</p>
<pre class="brush:java;">
...
<p>В этом случае длина строки, возвращаемая методом length(), равна 0.</p>
<p>Класс String имеет специальный метод, который позволяет проверить строку на пустоту - <span class="b">isEmpty()</span>. Если строка пуста, он возвращает true:</p>
<pre class="brush:java;">
String s = "";   // строка не указывает на объект
if(s.isEmpty()) System.out.println("String is empty");
</pre>
<p>Переменная String может не указывать на какой-либо объект и иметь значение <span class="b">null</span>:</p>
<pre class="brush:java;">
String s = null;   // строка не указывает на объект
if(s == null) System.out.println("String is null");
</pre>
<p>Значение null не эквивалентно пустой строке. Например, в следующем случае мы столкнемся с ошибкой выполнения:</p>
<pre class="brush:java;">
String s = null;   // строка не указывает на объект
if(s.length()==0) System.out.println("String is empty");	// ! Ошибка
</pre>
<p>Так как переменная не указывает ни на какой объект String, то соответственно мы не можем обращаться к методам объекта String. 
Чтобы избежать подобных ошибок, можно предварительно проверять строку на null:</p>
<pre class="brush:java;">
String s = null;   // строка не указывает на объект
if(s!=null && s.length()==0) System.out.println("String is empty");
</pre>
<h3>Основные методы класса String</h3>
<p>Основные операции со строками раскрывается через методы класса String, среди которых можно выделить следующие:</p>
<ul>
<li><p><span class="b">concat()</span>: объединяет строки</p></li>
<li><p><span class="b">valueOf()</span>: преобразует объект в строковый вид</p></li>
<li><p><span class="b">join()</span>: соединяет строки с учетом разделителя</p></li>
<li><p><span class="b">сompareTo()</span>: сравнивает две строки</p></li>
<li><p><span class="b">charAt()</span>: возвращает символ строки по индексу</p></li>
<li><p><span class="b">getChars()</span>: возвращает группу символов</p></li>
<li><p><span class="b">equals()</span>: сравнивает строки с учетом регистра</p></li>
<li><p><span class="b">equalsIgnoreCase()</span>: сравнивает строки без учета регистра</p></li>
<li><p><span class="b">regionMatches()</span>: сравнивает подстроки в строках</p></li>
<li><p><span class="b">indexOf()</span>: находит индекс первого вхождения подстроки в строку</p></li>
<li><p><span class="b">lastIndexOf()</span>: находит индекс последнего вхождения подстроки в строку</p></li>
<li><p><span class="b">startsWith()</span>: определяет, начинается ли строка с подстроки</p></li>
<li><p><span class="b">endsWith()</span>: определяет, заканчивается ли строка на определенную подстроку</p></li>
<li><p><span class="b">replace()</span>: заменяет в строке одну подстроку на другую</p></li>
<li><p><span class="b">trim()</span>: удаляет начальные и конечные пробелы</p></li>
<li><p><span class="b">substring()</span>: возвращает подстроку, начиная с определенного индекса до конца или до определенного индекса</p></li>
<li><p><span class="b">toLowerCase()</span>: переводит все символы строки в нижний регистр</p></li>
<li><p><span class="b">toUpperCase()</span>: переводит все символы строки в верхний регистр</p></li>
</ul>
...
<div class="nav"><p><a href="6.13.php">Назад</a><a href="./">Содержание</a><a href="7.2.php">Вперед</a></p></div></div>
<div class="socBlock">
