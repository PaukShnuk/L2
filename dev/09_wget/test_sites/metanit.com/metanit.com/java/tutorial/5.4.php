...
<html >
<head>
<title>Java | Интерфейс Set и класс HashSet</title>
<meta charset="utf-8" />
<meta name="description" content="Неупорядоченная коллекция уникальных элементов в языке программирования Java, интерфейс Set и класс HashSet, операции с хэш-таблицей">
...
<h2>Интерфейс Set и класс HashSet</h2><div class="date">Последнее обновление: 24.04.2018</div>
...
<p>Интерфейс <span class="b">Set</span> расширяет интерфейс Collection и представляет набор уникальных элементов. Set не добавляет 
новых методов, только вносит изменения унаследованные. В частности, метод <span class="b">add()</span> добавляет элемент в коллекцию и возвращает true, 
если в коллекции еще нет такого элемента.</p>
<p>Обобщенный класс <span class="b">HashSet</span> представляет хеш-таблицу. Он наследует свой функционал от класса <code>AbstractSet</code>, а также реализует интерфейс <code>Set</code>.</p>
<p>Хеш-таблица представляет такую структуру данных, в которой все объекты имеют уникальный ключ или хеш-код. Данный ключ позволяет уникально идентифицировать объект в 
таблице.</p>
<p>Для создания объекта HashSet можно воспользоваться одним из следующих конструкторов:</p>
<ul>
<li><p><code>HashSet()</code>: создает пустой список</p></li>
<li><p><code>HashSet(Collection&lt;? extends E&gt; col)</code>: создает хеш-таблицу, в которую добавляет все элементы коллекции col</p></li>
<li><p><code>HashSet(int capacity)</code>: параметр capacity указывает начальную емкость таблицы, которая по умолчанию равна 16</p></li>
<li><p><code>HashSet(int capacity, float koef)</code>: параметр koef или коэффициент заполнения, значение которого должно быть в пределах от 0.0 до 1.0, указывает, 
...
емкости на 3/4 произойдет ее расширение.</p></li>
</ul>
<p>Класс HashSet не добавляет новых методов, реализуя лишь те, что объявлены в родительских классах и применяемых интерфейсах:</p>
