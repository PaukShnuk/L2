...
<html >
<head>
<title>Java | Получение подпотока и объединение потоков в Stream API</title>
<meta charset="utf-8" />
<meta name="description" content="Получение подпотока и объединение потоков в Stream API в языке программирования Java, методы takeWhile, dropWhile">
...
<h2>Получение подпотока и объединение потоков</h2><div class="date">Последнее обновление: 30.04.2018</div>
...
<h3>takeWhile</h3>
<p>Метод <span class="b">takeWhile()</span> выбирает из потока элементы, пока они соответствуют условию. Если попадается элемент, который не соответствует условию, 
то метод завершает свою работу. Выбранные элементы возвращаются в виде потока.</p>
...
<p>При этом несмотря на то, что в потоке больше отрицательных чисел, но метод завершает работу, как только обнаружит первое число, которое не соответствует условию. 
В этом и состоит отличие, например, от метода <code>filter()</code>.</p>
...
<h3>dropWhile</h3>
<p>Метод <span class="b">dropWhile()</span> выполняет обратную задачу - он пропускает элементы потока, которые соответствуют условию до тех пор, пока не встретит элемент, который НЕ соответствует 
условию:</p>
<pre class="brush:java;">
Stream&lt;Integer&gt; numbers = Stream.of(-3, -2, -1, 0, 1, 2, 3, -4, -5);
...
<h3>concat</h3>
<p>Статический метод <span class="b">concat()</span> объединяет элементы двух потоков, возвращая объединенный поток:</p>
<pre class="brush:java;">
import java.util.stream.Stream;
...
<h3>distinct</h3>
<p>Метод <span class="b">distinct()</span> возвращает только ункальные элементы в виде потока:</p>
<pre class="brush:java;">
...
<div class="nav"><p><a href="10.8.php">Назад</a><a href="./">Содержание</a><a href="10.4.php">Вперед</a></p></div></div>
<div class="socBlock">
