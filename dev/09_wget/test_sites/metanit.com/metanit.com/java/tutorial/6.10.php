...
<html >
<head>
<title>Java | Сериализация</title>
<meta charset="utf-8" />
<meta name="description" content="Сериализация и десериализация сложных объектов в языке программирования Java, сохранение объектов классов в файл и чтение их из файла">
...
<h2>Сериализация</h2><div class="date">Последнее обновление: 26.04.2018</div>
...
<p><span class="b">Сериализация</span> представляет процесс записи состояния объекта в поток, соответственно процесс извлечения или восстановления состояния объекта из потока называется 
<span class="b">десериализацией</span>. Сериализация очень удобна, когда идет работа со сложными объектами.</p>
<h3>Интерфейс Serializable</h3>
<p>Сразу надо сказать, что сериализовать можно только те объекты, которые реализуют интерфейс <span class="b">Serializable</span>. 
Этот интерфейс не определяет никаких методов, просто он служит указателем системе, что объект, реализующий его, может быть сериализован.</p>
<h3>Сериализация. Класс ObjectOutputStream</h3>
<p>Для сериализации объектов в поток используется класс <span class="b">ObjectOutputStream</span>. Он записывает данные в поток.</p>
<p>Для создания объекта ObjectOutputStream в конструктор передается поток, в который производится запись:</p>
<pre class="brush:java;">
ObjectOutputStream(OutputStream out)
</pre>
<p>Для записи данных ObjectOutputStream использует ряд методов, среди которых можно выделить следующие:</p>
<ul>
<li><p><span class="b">void close()</span>: закрывает поток</p></li>
<li><p><span class="b">void flush()</span>: очищает буфер и сбрасывает его содержимое в выходной поток</p></li>
<li><p><span class="b">void write(byte[] buf)</span>: записывает в поток массив байтов</p></li>
<li><p><span class="b">void write(int val)</span>: записывает в поток один младший байт из val</p></li>
<li><p><span class="b">void writeBoolean(boolean val)</span>: записывает в поток значение boolean</p></li>
<li><p><span class="b">void writeByte(int val)</span>: записывает в поток один младший байт из val</p></li>
<li><p><span class="b">void writeChar(int val)</span>: записывает в поток значение типа char, представленное целочисленным значением</p></li>
<li><p><span class="b">void writeDouble(double val)</span>: записывает в поток значение типа double</p></li>
<li><p><span class="b">void writeFloat(float val)</span>: записывает в поток значение типа float</p></li>
<li><p><span class="b">void writeInt(int val)</span>: записывает целочисленное значение int</p></li>
<li><p><span class="b">void writeLong(long val)</span>: записывает значение типа long</p></li>
<li><p><span class="b">void writeShort(int val)</span>: записывает значение типа short</p></li>
<li><p><span class="b">void writeUTF(String str)</span>: записывает в поток строку в кодировке UTF-8</p></li>
<li><p><span class="b">void writeObject(Object obj)</span>: записывает в поток отдельный объект</p></li>
</ul>
<p>Эти методы охватывают весь спектр данных, которые можно сериализовать.</p>
<p>Например, сохраним в файл один объект класса Person:</p>
<pre class="brush:java;">
import java.io.*;
...
<h3>Десериализация. Класс ObjectInputStream</h3>
<p>Класс ObjectInputStream отвечает за обратный процесс - чтение ранее сериализованных данных из потока. В конструкторе он принимает ссылку на поток ввода:</p>
<pre class="brush:java;">
ObjectInputStream(InputStream in)
</pre>
<p>Функционал ObjectInputStream сосредоточен в методах, предназначенных для чтения различных типов данных. Рассмотрим основные методы этого класса:</p>
<ul>
<li><p><span class="b">void close()</span>: закрывает поток</p></li>
<li><p><span class="b">int skipBytes(int len)</span>: пропускает при чтении несколько байт, количество которых равно len</p></li>
<li><p><span class="b">int available()</span>: возвращает количество байт, доступных для чтения</p></li>
<li><p><span class="b">int read()</span>: считывает из потока один байт и возвращает его целочисленное представление</p></li>
<li><p><span class="b">boolean readBoolean()</span>: считывает из потока одно значение boolean</p></li>
<li><p><span class="b">byte readByte()</span>: считывает из потока один байт</p></li>
<li><p><span class="b">char readChar()</span>: считывает из потока один символ char</p></li>
<li><p><span class="b">double readDouble()</span>: считывает значение типа double</p></li>
<li><p><span class="b">float readFloat()</span>: считывает из потока значение типа float</p></li>
<li><p><span class="b">int readInt()</span>: считывает целочисленное значение int</p></li>
<li><p><span class="b">long readLong()</span>: считывает значение типа long</p></li>
<li><p><span class="b">short readShort()</span>: считывает значение типа short</p></li>
<li><p><span class="b">String readUTF()</span>: считывает строку в кодировке UTF-8</p></li>
<li><p><span class="b">Object readObject()</span>: считывает из потока объект</p></li>
</ul>
<p>Например, извлечем выше сохраненный объект Person из файла:</p>
<pre class="brush:java;">
import java.io.*;
...
<h3>Исключение данных из сериализации</h3>
<p>По умолчанию сериализуются все переменные объекта. Однако, возможно, мы хотим, чтобы некоторые поля были исключены из сериализации. 
Для этого они должны быть объявлены с модификатором <span class="b">transient</span>. Например, исключим из сериализации объекта Person переменные 
height и married:</p>
<pre class="brush:java;">
class Person implements Serializable{
...
<div class="nav"><p><a href="6.9.php">Назад</a><a href="./">Содержание</a><a href="6.11.php">Вперед</a></p></div></div>
<div class="socBlock">
