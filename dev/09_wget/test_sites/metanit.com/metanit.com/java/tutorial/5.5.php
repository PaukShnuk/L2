...
<html >
<head>
<title>Java | SortedSet, NavigableSet, TreeSet</title>
<meta charset="utf-8" />
<meta name="description" content="Упорядоченные множества в языке программирования Java, интерфейсы SortedSet и NavigableSet, класс дерева TreeSet">
...
<h2>SortedSet, NavigableSet, TreeSet</h2><div class="date">Последнее обновление: 24.04.2018</div>
...
<h3>SortedSet</h3>
<p>Интерфейс <span class="b">SortedSet</span> предназначен для создания коллекций, который хранят элементы в отсортированном виде (сортировка по 
возрастанию). SortedSet расширяет интерфейс Set, поэтому такая коллекция опять же хранит только уникальные значения. 
SortedSet предоставляет следующие методы:</p>
<ul>
<li><p><span class="b">E first()</span>: возвращает первый элемент набора</p></li>
<li><p><span class="b">E last()</span>: возвращает последний элемент набора</p></li>
<li><p><span class="b">SortedSet&lt;E&gt; headSet(E end)</span>: возвращает объект SortedSet, который содержит все элементы первичного набора до элемента end</p></li>
<li><p><span class="b">SortedSet&lt;E&gt; subSet(E start, E end)</span>: возвращает объект SortedSet, который содержит все элементы первичного набора между элементами start и end</p></li>
<li><p><span class="b">SortedSet&lt;E&gt; tailSet(E start)</span>: возвращает объект SortedSet, который содержит все элементы первичного набора, начиная с элемента start</p></li>
</ul>
<h3>NavigableSet</h3>
<p>Интерфейс <span class="b">NavigableSet</span> расширяет интерфейс <span class="b">SortedSet</span> и позволяет извлекать элементы на основании их значений. 
NavigableSet определяет следующие методы:</p>
<ul>
<li><p><span class="b">E ceiling(E obj)</span>: ищет в наборе наименьший элемент e, который больше obj (e &gt;=obj). Если такой элемент найден, 
то он возвращается в качестве результата. Иначе возвращается null.</p></li>
<li><p><span class="b">E floor(E obj)</span>: ищет в наборе наибольший элемент e, который меньше элемента obj (e &lt;=obj). Если такой элемент найден, 
то он возвращается в качестве результата. Иначе возвращается null.</p></li>
<li><p><span class="b">E higher(E obj)</span>: ищет в наборе наименьший элемент e, который больше элемента obj (e &gt;obj). Если такой элемент найден, 
то он возвращается в качестве результата. Иначе возвращается null.</p></li>
<li><p><span class="b">E lower(E obj)</span>: ищет в наборе наибольший элемент e, который меньше элемента obj (e &lt;obj). Если такой элемент найден, 
то он возвращается в качестве результата. Иначе возвращается null.</p></li>
<li><p><span class="b">E pollFirst()</span>: возвращает первый элемент и удаляет его из набора</p></li>
<li><p><span class="b">E pollLast()</span>: возвращает последний элемент и удаляет его из набора</p></li>
<li><p><span class="b">NavigableSet&lt;E&gt; descendingSet()</span>: возвращает объект NavigableSet, который содержит все элементы первичного набора NavigableSet в обратном порядке</p></li>
<li><p><span class="b">NavigableSet&lt;E&gt; headSet(E upperBound, boolean incl)</span>: возвращает объект NavigableSet, который содержит все 
элементы первичного набора NavigableSet до upperBound. Параметр incl при значении true, позволяет включить в выходной набор элемент upperBound</p></li>
<li><p><span class="b">NavigableSet&lt;E&gt; tailSet(E lowerBound, boolean incl)</span>: возвращает объект NavigableSet, который содержит все 
элементы первичного набора NavigableSet, начиная с lowerBound. Параметр incl при значении true, позволяет включить в выходной набор элемент lowerBound</p></li>
<li><p><span class="b">NavigableSet&lt;E&gt; subSet(E lowerBound, boolean lowerIncl, E upperBound, boolean highIncl)</span>: возвращает объект 
NavigableSet, который содержит все элементы первичного набора NavigableSet от lowerBound до upperBound.</p></li>
</ul>
<h3>TreeSet</h3>
<p>Обобщенный класс TreeSet&lt;E&gt; представляет структуру данных в виде дерева, в котором все объекты хранятся в отсортированном виде по возрастанию. 
TreeSet является наследником класса <code>AbstractSet</code> и реализует интерфейс <code>NavigableSet</code>, а следовательно, и интерфейс <code>SortedSet</code>.</p>
<p>В классе TreeSet определены следующие конструкторы:</p>
<ul>
<li><p><code>TreeSet()</code>: создает пустое дерево</p></li>
<li><p><code>TreeSet(Collection&lt;? extends E&gt; col)</code>: создает дерево, в которое добавляет все элементы коллекции col</p></li>
<li><p><code>TreeSet(SortedSet &lt;E&gt; set)</code>: создает дерево, в которое добавляет все элементы сортированного набора set</p></li>
<li><p><code>TreeSet(Comparator&lt;? super E&gt; comparator)</code>: создает пустое дерево, где все добавляемые элементы впоследствии будут 
отсортированы компаратором.</p></li>
</ul>
<p>TreeSet поддерживает все стандартные методы для вставки и удаления элементов:</p>
...
<p>И поскольку при вставке объекты сразу же сортируются по возрастанию, то при выводе в цикле for мы получим отсортированный набор:</p>
<div class="console">
...
<p>Так как TreeSet реализует интерфейс NavigableSet, а через него и SortedSet, то мы можем применить к структуре дерева различные методы:</p>
<pre class="brush:java;">
import java.util.*;
