...
<html >
<head>
<title>Java | Группировка в Stream API</title>
<meta charset="utf-8" />
<meta name="description" content="Группировка данных в Stream API в языке программирования Java, метод Collectors.groupingBy">
...
<h2>Группировка</h2><div class="date">Последнее обновление: 30.04.2018</div>
...
<p>Чтобы сгруппировать данные по какому-нибудь признаку, нам надо использовать в связке метод <code>collect()</code> объекта Stream и метод 
<span class="b">Collectors.groupingBy()</span>. Допустим, у нас есть следующий класс:</p>
...
<p>И, к примеру, у нас есть набор объектов Phone, которые мы хотим сгруппировать по компании:</p>
<pre class="brush:java;">
...
<p>Итак, для создания групп в метод <code>phoneStream.collect()</code> передается вызов функции <code>Collectors.groupingBy()</code>, 
которая с помощью выражения <code>Phone::getCompany</code> группирует объекты по компании. В итоге будет создан объект Map, в котором ключами являются названия компаний, 
а значениями - список связанных с компаниями телефонов.</p>
<h3>Метод Collectors.partitioningBy</h3>
<p>Метод <span class="b">Collectors.partitioningBy()</span> имеет похожее действие, только он делит элементы на группы по принципу, соответствует ли элемент определенному условию. 
Например:</p>
<pre class="brush:java;">
...
<p>В данном случае с помощью условия <code>p-&gt;p.getCompany()=="Apple"</code> мы смотрим, принадлежит ли телефон компании Apple. Если телефон принадлежит этой 
компании, то он попадает в одну группу, если нет, то в другую.</p>
<h3>Метод Coollectors.counting</h3>
<p>Метод <span class="b">Collectors.counting</span> применяется в <code>Collectors.groupingBy()</code> для вычисления количества элементов в каждой группе:</p>
<pre class="brush:java;">
...
<h3>Метод Collectors.summing</h3>
<p>Метод Collectors.summing применяется для подсчета суммы. В зависимости от типа данных, к которым применяется метод, он имеет следующие формы: 
<code>summingInt()</code>, <code>summingLong()</code>, <code>summingDouble()</code>. Применим этот метод для подсчета стоимости всех смартфонов по компаниям:</p>
<pre class="brush:java;">
...
<p>С помощью выражения <code>Collectors.summingInt(Phone::getPrice))</code> мы указываем, что для каждой компании будет вычислять совокупная цена 
...
<h3>Методы maxBy и minBy</h3>
<p>Методы maxBy и minBy применяются для подсчета минимального и максимального значения в каждой группе. В качестве параметра эти методы принимают 
функцию компаратора, которая нужна для сравнения значений. Например, найдем для каждой компании телефон с минимальной ценой:</p>
<pre class="brush:java;">
...
<p>В качестве возвращаемого значения операции группировки используется объект <code>Map&lt;String, Optional&lt;Phone&gt;&gt;</code>. Опять же поскольку 
группируем по компаниям, то ключом будет выступать строка, а значением - объект <code>Optional&lt;Phone&gt;</code>.</p>
<h3>Метод summarizing</h3>
<p>Методы <code>summarizingInt() / summarizingLong() / summarizingDouble()</code> позволяют объединить в набор значения соответствующих типов:</p>
<pre class="brush:java;">
...
<p>Метод <code>Collectors.summarizingInt(Phone::getPrice))</code> создает набор, в который помещаются цены для всех телефонов каждой из групп. Данный набор 
инкапсулируется в объекте <span class="b">IntSummaryStatistics</span>. Соответственно если бы мы применяли методы <code>summarizingLong()</code> или <code>summarizingDouble()</code>, то 
соответственно бы получали объекты LongSummaryStatistics или DoubleSummaryStatistics.</p>
<p>У этих объектов есть ряд методов, который позволяют выполнить различные атомарные операции над набором:</p>
<ul>
<li><p><span class="b">getAverage()</span>: возвращает среднее значение</p></li>
<li><p><span class="b">getCount()</span>: возвращает количество элементов в наборе</p></li>
<li><p><span class="b">getMax()</span>: возвращает максимальное значение</p></li>
<li><p><span class="b">getMin()</span>: возвращает минимальное значение</p></li>
<li><p><span class="b">getSum()</span>: возвращает сумму элементов</p></li>
<li><p><span class="b">accept()</span>: добавляет в набор новый элемент</p></li>
</ul>
...
<h3>Метод mapping</h3>
<p>Метод mapping позволяет дополнительно обработать данные и задать функцию отображения объектов из потока на какой-нибудь другой тип данных. Например:</p>
<pre class="brush:java;">
...
<p>Выражение <code>Collectors.mapping(Phone::getName, Collectors.toList())</code> указывает, что в группу будут выделятся названия смартфонов, причем 
группа будет представлять объект List.</p>
...
<div class="nav"><p><a href="10.6.php">Назад</a><a href="./">Содержание</a><a href="10.9.php">Вперед</a></p></div></div>
<div class="socBlock">
