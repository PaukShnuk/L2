...
<html >
<head>
<title>Java | Классы ByteArrayInputStream и ByteArrayOutputStream</title>
<meta charset="utf-8" />
<meta name="description" content="Чтение и запись массива байтов в языке программирования Java с помощью классов ByteArrayInputStream и ByteArrayOutputStream">
...
<h2>Классы ByteArrayInputStream и ByteArrayOutputStream</h2><div class="date">Последнее обновление: 26.04.2018</div>
...
<p>Для работы с массивами байтов - их чтения и записи используются классы ByteArrayInputStream и ByteArrayOutputStream.</p>
<h3>Чтение массива байтов и класс ByteArrayInputStream</h3>
...
<p>В качестве параметров конструкторы используют массив байтов <code>buf</code>, из которого производится считывание, смещение относительно начала массива 
<code>offset</code> и количество считываемых символов <code>length</code>.</p>
...
<h3>Запись массива байт и класс ByteArrayOutputStream</h3>
<p>Класс ByteArrayOutputStream представляет поток вывода, использующий массив байтов в качестве места вывода.</p>
<P>Чтобы создать объект данного класса, мы можем использовать один из его конструкторов:</P>
<pre class="brush:java;">
ByteArrayOutputStream() 
ByteArrayOutputStream(int size)
</pre>
<p>Первая версия создает массив для хранения байтов длиной в 32 байта, а вторая версия создает массив длиной <code>size</code>.</p>
...
<p>Как и в других потоках вывода в классе <code>ByteArrayOutputStream</code> определен метод <code>write</code>, который записывает в поток некоторые данные. 
В данном случае мы записываем в поток массив байтов. Этот массив байтов записывается в объекте ByteArrayOutputStream в защищенное поле <code>buf</code>, 
которое представляет также массив байтов (<code>protected byte[] buf</code>).</p>
<p>Так как метод <code>write</code> может сгенерировать исключение, то вызов этого метода помещается в блок try..catch.</p>
<p>Используя методы <code>toString()</code> и <code>toByteArray()</code>, можно получить массив байтов buf в виде текста или непосредственно в виде массива байт.</p>
<p>С помощью метода <code>writeTo</code> мы можем вывести массив байт в другой поток. Данный метод в качестве параметра принимает объект <code>OutputStream</code>, 
в который производится запись массива байт:</p>
<pre class="brush:java;">
ByteArrayOutputStream baos = new ByteArrayOutputStream();
String text = "Hello Wolrd!";
byte[] buffer = text.getBytes();
...
<p>После выполнения этой программы в папке с программой появится файл hello.txt, который будет содержать строку "Hello Wolrd!".</p>
<p>И в заключении также надо сказать, что как и для объектов ByteArrayInputStream, для ByteArrayOutputStream не надо явным образом закрывать поток с помощью метода 
<code>close</code>.</p>
