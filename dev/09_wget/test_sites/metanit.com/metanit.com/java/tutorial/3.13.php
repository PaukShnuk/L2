...
<html >
<head>
<title>Java | Ссылочные типы и копирование объектов</title>
<meta charset="utf-8" />
<meta name="description" content="Ссылочные типы в Java, их отличие от примитивных типов, клонирование объектов, полное и неполное копирование, интерфейс Clonable">
...
<h2>Ссылочные типы и копирование объектов</h2><div class="date">Последнее обновление: 23.04.2018</div>
...
<p>При работе с объектами классов надо учитывать, что они все представляют ссылочные типы, то есть указывают на какой-то объект, 
расположенный в памяти. Чтобы понять возможные трудности, с которыми мы можем столкнуться, рассмотрим пример:</p>
...
<p>Здесь создаем два объекта Person и один присваиваем другому. Но, несмотря на то, что мы изменяем только объект bob, вместе с ним изменяется и 
объект tom. Потому что после присвоения они указывают на одну и ту же область в памяти, где собственно данные об объекте Person и его полях и хранятся.</p>
<img src="./pics/3.2.png" alt="Клонирование объектов в Java" />

<p>Чтобы избежать этой проблемы, необходимо создать отдельный объект для переменной bob, например, с помощью метода <code>clone</code>:</p>
<pre class="brush:java;">
...
<p>Для реализации клонирования класс Person должен применить интерфейс <span class="b">Cloneable</span>, который определяет метод <code>clone</code>. 
Реализация этого метода просто возвращает вызов метода <code>clone</code> для родительского класса - то есть класса Object с преобразованием к типу Person.</p>
<p>Кроме того, на случай если класс не поддерживает клонирование, метод должен выбрасывать исключение <span class="b">CloneNotSupportedException</span>, 
что определяется с помощью оператора <span class="b">throws</span>.</p>
...
<p>Однако данный способ осуществляет <span class="b">неполное копирование</span> и подойдет, если клонируемый объект не содержит сложных объектов. Например, 
пусть класс Book имеет следующее определение:</p>
<pre class="brush:java;">
...
<p>Если мы попробуем изменить автора книги, нас постигнет неудача:</p>
<pre class="brush:java;">
try{
...
<p>В этом случае, хотя переменные book и book2 будут указывать на разные объекты в памяти, но эти объекты при этом будут указывать на один объект 
Author.</p>
<img src="./pics/3.3.png" alt="Неполное копирование объектов в Java" />
<p>И в этом случае нам необходимо выполнить полное копирование. Для этого надо определить метод клонирования у класса Author:</p>
<pre class="brush:java;">
...
<p>И затем исправим метод clone в классе Book следующим образом:</p>
<pre class="brush:java;">
public Book clone() throws CloneNotSupportedException{
...
<div class="nav"><p><a href="3.15.php">Назад</a><a href="./">Содержание</a><a href="3.18.php">Вперед</a></p></div></div>
<div class="socBlock">
