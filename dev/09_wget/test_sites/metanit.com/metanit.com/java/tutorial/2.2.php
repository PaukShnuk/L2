...
<html >
<head>
<title>Java | Преобразования типов данных</title>
<meta charset="utf-8" />
<meta name="description" content="Правила преобразований базовых типов данных в Java, расширяющие и сужающие преобразования, приведение типов в арифметических операциях">
...
<h2>Преобразования базовых типов данных</h2><div class="date">Последнее обновление: 29.10.2018</div>
...
<p>Каждый базовый тип данных занимает определенное количество байт памяти. Это накладывает ограничение на операции, в которые вовлечены различные типы данных. Рассмотрим следующий пример:</p>
<pre class="brush:java;">
int a = 4;
byte b = a;	// ! Ошибка
</pre>
<p>В данном коде мы столкнемся с ошибкой. Хотя и тип byte, и тип int представляют целые числа. Более того, значение переменной a, 
...
компиляции. Поскольку в данном случае мы пытаемся присвоить некоторые данные, которые занимают 4 байта, переменной, которая занимает всего один байт.</p>
<p>Тем не менее в программе может потребоваться, чтобы подобное преобразование было выполнено. В этом случае необходимо использовать операцию 
преобразования типов (операция <code>()</code>):</p>
<pre class="brush:java;">
int a = 4;
byte b = (byte)a;  // преобразование типов: от типа int к типу byte
System.out.println(b); // 4
</pre>
<p>Операция преобразования типов предполагает указание в скобках того типа, к которому надо преобразовать значение. 
Например, в случае операции <code>(byte)a</code>, идет преобразование данных типа int в тип byte. В итоге мы получим значение типа byte.</p>
<h3>Явные и неявные преобразования</h3>
<p>Когда в одной операции вовлечены данные разных типов, не всегда необходимо использовать операцию преобразования типов. 
Некоторые виды преобразований выполняются неявно, автоматически.</p>
<h4>Автоматические преобразования</h4>
<img src="./pics/2.9.png" alt="Преобразования типов в языке Java" />
<p>Стрелками на рисунке показано, какие преобразования типов могут выполняться автоматически. Пунктирными стрелками показаны автоматические преобразования 
с потерей точности.</p>
<p>Автоматически без каких-либо проблем производятся <span class="b">расширяющие преобразования</span> (widening) - они расширяют представление объекта в памяти. Например:</p>
<pre class="brush:java;">
byte b = 7;
int d = b;	// преобразование от byte к int
</pre>
<p>В данном случае значение типа byte, которое занимает в памяти 1 байт, расширяется до типа int, которое занимает 4 байта.</p>
<p>Расширяющие автоматические преобразования представлены следующими цепочками:</p>
<p><span class="b">byte</span> -&gt; <span class="b">short</span> -&gt; <span class="b">int</span> -&gt; <span class="b">long</span></p>
...
<h4>Автоматические преобразования с потерей точности</h4>
<p>Некоторые преобразования могут производиться автоматически между типами данных одинаковой разрядности или даже от типа данных с большей разрядностью к 
типа с меньшей разрядностью. Это следующие цепочки преобразований: <code>int -&gt; float</code>, <code>long -&gt; float</code> и  <code>long -&gt; double</code>. 
Они производятся без ошибок, но при преобразовании мы можем столкнуться с потерей информации.</p>
<p>Например:</p>
<pre class="brush:java;">
int a = 2147483647;
...
<p>Во всех остальных преобразованиях примитивных типов явным образом применяется операция преобразования типов. Обычно это сужающие преобразования (narrowing) от 
типа с большей разрядностью к типу с меньшей разрядностью:</p>
<pre class="brush:java;">
long a = 4;
int b = (int) a;
</pre>
<h3>Потеря данных при преобразовании</h3>
<p>При применении явных преобразований мы можем столкнуться с потерей данных. Например, в следующем коде у нас не возникнет никаких проблем:</p>
<pre class="brush:java;">
int a = 5;
byte b = (byte) a;
System.out.println(b);		// 5
</pre>
<p>Число 5 вполне укладывается в диапазон значений типа byte, поэтому после преобразования переменная b будет равна 5. Но что будет в следующем случае:</p>
<pre class="brush:java;">
int a = 258;
byte b = (byte) a;
System.out.println(b);		// 2
</pre>
<p>Результатом будет число 2. В данном случае число 258 вне диапазона для типа byte (от -128 до 127), поэтому произойдет усечение значения. Почему результатом будет именно число 2?</p>
<p>Число a, которое равно 258, в двоичном системе будет равно <code>00000000 00000000 00000001 00000010</code>. 
Значения типа byte занимают в памяти только 8 бит. Поэтому двоичное представление числа int усекается до 8 правых разрядов, то есть <code>00000010</code>, 
что в десятичной системе дает число 2.</p>
<p></p>
<h3>Усечение рациональных чисел до целых</h3>
<p>При преобразовании значений с плавающей точкой к целочисленным значениям, происходит усечение дробной части:</p>
<pre class="brush:java;">
double a = 56.9898;
int b = (int)a;
</pre>
<p>Здесь значение числа b будет равно 56, несмотря на то, что число 57 было бы ближе к 56.9898. Чтобы избежать подобных казусов, надо применять 
функцию округления, которая есть в математической библиотеке Java:</p>
<pre class="brush:java;">
double a = 56.9898;
int b = (int)Math.round(a);
</pre>
<h3>Преобразования при операциях</h3>
<p>Нередки ситуации, когда приходится применять различные операции, например, сложение и произведение, над значениями разных типов. Здесь также 
действуют некоторые правила:</p>
<ul>
<li><p>если один из операндов операции относится к типу <code>double</code>, то и второй операнд преобразуется к типу <code>double</code></p></li>
<li><p>если предыдущее условие не соблюдено, а один из операндов операции относится к типу <code>float</code>, то и второй операнд преобразуется к типу <code>float</code></p></li>
<li><p>если предыдущие условия не соблюдены, один из операндов операции относится к типу <code>long</code>, то и второй операнд преобразуется к типу <code>long</code></p></li>
<li><p>иначе все операнды операции преобразуются к типу <code>int</code></p></li>
</ul>
...
<p>Так как в операции участвует значение типа double, то и другое значение приводится к типу double и сумма двух значений <code>a+b</code> будет 
представлять тип double.</p>
<p>Другой пример:</p>
<pre class="brush:java;">
byte a = 3;
short b = 4;
...
<p>Две переменных типа byte и short (не double, float или long), поэтому при сложении они преобразуются к типу <code>int</code>, и их сумма 
<code>a+b</code> представляет значение типа int. Поэтому если затем мы присваиваем эту сумму переменной типа byte, то нам опять надо сделать 
преобразование типов к byte.</p>
