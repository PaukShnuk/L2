...
<html >
<head>
<title>Java | Потоки ввода-вывода</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с файлами в Java, основные классы файловых потоков и их методы, InputStream и OutputStream, Writer и Reader">
...
<h1>Потоки ввода-вывода. Работа с файлами</h1><h2>Потоки ввода-вывода</h2><div class="date">Последнее обновление: 25.04.2018</div>
...
<p>Отличительной чертой многих языков программирования является работа с файлами и потоками. В Java основной функционал работы с потоками 
сосредоточен в классах из пакета <span class="b">java.io</span>.</p>
<p>Ключевым понятием здесь является понятие <span class="b">потока</span>. Хотя понятие "поток" в программировании довольно перегружено и может обозначать 
множество различных концепций. В данном случае применительно к работе с файлами и вводом-выводом мы будем говорить о потоке (stream), как об абстракции, 
которая используется для чтения или записи информации (файлов, сокетов, текста консоли и т.д.).</p>
<p>Поток связан с реальным физическим устройством с помощью системы ввода-вывода Java. У нас может быть определен поток, который связан с файлом и через который 
мы можем вести чтение или запись файла. Это также может быть поток, связанный с сетевым сокетом, с помощью которого можно получить или отправить данные в сети. 
Все эти задачи: чтение и запись различных файлов, обмен информацией по сети, ввод-ввывод в консоли мы будем решать в Java с помощью потоков.</p>
<p>Объект, из которого можно считать данные, называется <span class="b">потоком ввода</span>, а объект, в который можно записывать данные, - 
<span class="b">потоком вывода</span>. Например, если надо считать содержание файла, то применяется поток ввода, а если надо записать в файл - то поток вывода.</p>
<p>В основе всех классов, управляющих потоками байтов, находятся два абстрактных класса: <span class="b">InputStream</span> (представляющий потоки ввода) и 
<span class="b">OutputStream</span> (представляющий потоки вывода)</p>
<p>Но поскольку работать с байтами не очень удобно, то для работы с потоками символов были добавлены абстрактные классы <span class="b">Reader</span> (для 
чтения потоков символов) и <span class="b">Writer</span> (для записи потоков символов).</p>
<p>Все остальные классы, работающие с потоками, являются наследниками этих абстрактных классов. Основные классы потоков:</p>
<img src="./pics/6.1.png" alt="Классы потоков в Java" />
<h3>Потоки байтов</h3>
<h4>Класс InputStream</h4>
<p>Класс InputStream является базовым для всех классов, управляющих байтовыми потоками ввода. Рассмотрим его основные методы:</p>
<ul>
<li><p><code>int available()</code>: возвращает количество байтов, доступных для чтения в потоке</p></li>
<li><p><code>void close()</code>: закрывает поток</p></li>
<li><p><code>int read()</code>: возвращает целочисленное представление следующего байта в потоке. Когда в потоке не останется доступных для 
чтения байтов, данный метод возвратит число -1</p></li>
<li><p><code>int read(byte[] buffer)</code>: считывает байты из потока в массив buffer. После чтения возвращает число считанных байтов. Если ни одного байта не 
было считано, то возвращается число -1</p></li>
<li><p><code>int read(byte[] buffer, int offset, int length)</code>: считывает некоторое количество байтов, равное length, из потока в массив buffer. 
При этом считанные байты помещаются в массиве, начиная со смещения offset, то есть с элемента <code>buffer[offset]</code>. 
Метод возвращает число успешно прочитанных байтов.</p></li>
<li><p><code>long skip(long number)</code>: пропускает в потоке при чтении некоторое количество байт, которое равно number</p></li>
</ul>
<h4>Класс OutputStream</h4>
<p>Класс OutputStream является базовым классом для всех классов, которые работают с бинарными потоками записи. Свою функциональность он реализует через следующие методы:</p>
<ul>
<li><p><code>void close()</code>: закрывает поток</p></li>
<li><p><code>void flush()</code>: очищает буфер вывода, записывая все его содержимое</p></li>
<li><p><code>void write(int b)</code>: записывает в выходной поток один байт, который представлен целочисленным параметром b</p></li>
<li><p><code>void write(byte[] buffer)</code>: записывает в выходной поток массив байтов buffer.</p></li>
<li><p><code>void write(byte[] buffer, int offset, int length)</code>: записывает в выходной поток некоторое число байтов, равное <code>length</code>, из 
массива <code>buffer</code>, начиная со смещения <code>offset</code>, то есть с элемента <code>buffer[offset]</code>.</p></li>
</ul>
<h3>Абстрактные классы Reader и Writer</h3>
<p>Абстрактный класс Reader предоставляет функционал для чтения текстовой информации. Рассмотрим его основные методы:</p>
<ul>
<li><p><code>absract void close()</code>: закрывает поток ввода</p></li>
<li><p><code>int read()</code>: возвращает целочисленное представление следующего символа в потоке. Если таких символов нет, и достигнут конец файла, 
то возвращается число -1</p></li>
<li><p><code>int read(char[] buffer)</code>: считывает в массив buffer из потока символы, количество которых равно длине массива buffer. Возвращает 
количество успешно считанных символов. При достижении конца файла возвращает -1</p></li>
<li><p><code>int read(CharBuffer buffer)</code>: считывает в объект CharBuffer из потока символы. Возвращает 
количество успешно считанных символов. При достижении конца файла возвращает -1</p></li>
<li><p><code>absract int read(char[] buffer, int offset, int count)</code>: считывает в массив buffer, начиная со смещения offset, из потока символы, количество которых равно count</p></li>
<li><p><code>long skip(long count)</code>: пропускает количество символов, равное count. Возвращает число успешно пропущенных символов</p></li>
</ul>

<p>Класс Writer определяет функционал для всех символьных потоков вывода. Его основные методы:</p>
<ul>
<li><p><code>Writer append(char c)</code>: добавляет в конец выходного потока символ c. Возвращает объект Writer</p></li>
<li><p><code>Writer append(CharSequence chars)</code>: добавляет в конец выходного потока набор символов chars. Возвращает объект Writer</p></li>
<li><p><code>abstract void close()</code>: закрывает поток</p></li>
<li><p><code>abstract void flush()</code>: очищает буферы потока</p></li>
<li><p><code>void write(int c)</code>: записывает в поток один символ, который имеет целочисленное представление</p></li>
<li><p><code>void write(char[] buffer)</code>: записывает в поток массив символов</p></li>
<li><p><code>absract void  write(char[] buffer, int off, int len)  </code>: записывает в поток только несколько символов из массива buffer. Причем количество символов 
равно len, а отбор символов из массива начинается с индекса off</p></li>
<li><p><code>void write(String str)</code>: записывает в поток строку</p></li>
<li><p><code>void write(String str, int off, int len)</code>: записывает в поток из строки некоторое количество символов, которое равно len, причем отбор символов 
из строки начинается с индекса off</p></li>
</ul>
<p>Функционал, описанный классами Reader и Writer, наследуется непосредственно классами символьных потоков, в частности классами 
<code>FileReader</code> и <code>FileWriter</code> соответственно, предназначенными для работы с текстовыми файлами.</p>
<p>Теперь рассмотрим конкретные классы потоков.</p>
...
<div class="nav"><p><a href="5.10.php">Назад</a><a href="./">Содержание</a><a href="6.3.php">Вперед</a></p></div></div>
<div class="socBlock">
