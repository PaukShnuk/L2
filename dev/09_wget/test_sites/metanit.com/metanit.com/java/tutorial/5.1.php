...
<html >
<head>
<title>Java | Типы коллекций. Интерфейс Collection</title>
<meta charset="utf-8" />
<meta name="description" content="Коллекции в языке программирования Java, типы коллекций, интерфейс Collection и его методы">
...
<h1>Коллекции</h1><h2>Типы коллекций. Интерфейс Collection</h2><div class="date">Последнее обновление: 30.04.2018</div>
...
<p>Для хранения наборов данных в Java предназначены массивы. Однако их не всегда удобно использовать, прежде всего потому, что они имеют фиксированную длину. 
Эту проблему в Java решают коллекции. Однако суть не только в гибких по размеру наборах объектов, но в и том, что классы коллекций реализуют различные алгоритмы 
и структуры данных, например, такие как стек, очередь, дерево и ряд других.</p> 
...
<p>Хотя в Java существует множество коллекций, но все они образуют стройную и логичную систему. Во-первых, в основе всех коллекций лежит 
применение того или иного интерфейса, который определяет базовый функционал. Среди этих интерфейсов можно выделить следующие:</p>
<ul>
<li><p><span class="b">Collection</span>: базовый интерфейс для всех коллекций и других интерфейсов коллекций</p></li>
<li><p><span class="b">Queue</span>: наследует интерфейс Collection и представляет функционал для структур данных в виде очереди</p></li>
<li><p><span class="b">Deque</span>: наследует интерфейс Queue и представляет функционал для двунаправленных очередей</p></li>
<li><p><span class="b">List</span>: наследует интерфейс Collection и представляет функциональность простых списков</p></li>
<li><p><span class="b">Set</span>: также расширяет интерфейс Collection и используется для хранения множеств уникальных объектов</p></li>
<li><p><span class="b">SortedSet</span>: расширяет интерфейс Set для создания сортированных коллекций</p></li>
<li><p><span class="b">NavigableSet</span>: расширяет интерфейс SortedSet для создания коллекций, в которых можно осуществлять поиск по соответствию</p></li>
<li><p><span class="b">Map</span>: предназначен для созданий структур данных в виде словаря, где каждый элемент имеет определенный ключ и значение. 
В отличие от других интерфейсов коллекций не наследуется от интерфейса Collection</p></li>
</ul>
<p>Эти интерфейсы частично реализуются абстрактными классами:</p>
<ul>
<li><p><span class="b">AbstractCollection</span>: базовый абстрактный класс для других коллекций, который применяет интерфейс Collection</p></li>
<li><p><span class="b">AbstractList</span>: расширяет класс AbstractCollection и применяет интерфейс List, предназначен для создания коллекций в виде списков</p></li>
<li><p><span class="b">AbstractSet</span>: расширяет класс AbstractCollection и применяет интерфейс Set для создания коллекций в виде множеств</p></li>
<li><p><span class="b">AbstractQueue</span>: расширяет класс AbstractCollection и применяет интерфейс Queue, предназначен для создания коллекций в виде очередей и стеков</p></li>
<li><p><span class="b">AbstractSequentialList</span>: также расширяет класс AbstractList и реализует интерфейс List. Используется для создания связанных списков</p></li>
<li><p><span class="b">AbstractMap</span>: применяет интерфейс Map, предназначен для создания наборов по типу словаря с объектами в виде пары "ключ-значение"</p></li>
</ul>
<p>С помощью применения вышеописанных интерфейсов и абстрактных классов в Java реализуется широкая палитра классов коллекций - списки, множества, 
очереди, отображения и другие, среди которых можно выделить следующие:</p>
<ul>
<li><p><span class="b">ArrayList</span>: простой список объектов</p></li>
<li><p><span class="b">LinkedList</span>: представляет связанный список</p></li>
<li><p><span class="b">ArrayDeque</span>: класс двунаправленной очереди, в которой мы можем произвести вставку и удаление как в начале коллекции, 
так и в ее конце</p></li>
<li><p><span class="b">HashSet</span>: набор объектов или хеш-множество, где каждый элемент имеет ключ - уникальный хеш-код</p></li>
<li><p><span class="b">TreeSet</span>: набор отсортированных объектов в виде дерева</p></li>
<li><p><span class="b">LinkedHashSet</span>: связанное хеш-множество</p></li>
<li><p><span class="b">PriorityQueue</span>: очередь приоритетов</p></li>
<li><p><span class="b">HashMap</span>: структура данных в виде словаря, в котором каждый объект имеет уникальный ключ и некоторое значение</p></li>
<li><p><span class="b">TreeMap</span>: структура данных в виде дерева, где каждый элемент имеет уникальный ключ и некоторое значение</p></li>
</ul>
<p>Схематично всю систему коллекций вкратце можно представить следующим образом:</p>
<img src="./pics/5.1.png" alt="Интерфейсы коллекций в Java" />
<h3>Интерфейс Collection</h3>
<p>Интерфейс Collection является базовым для всех коллекций, определяя основной функционал:</p>
...
<p>Интерфейс Collection является обобщенным и расширяет интерфейс Iterable, поэтому все объекты коллекций можно перебирать в цикле по типу 
<code>for-each</code>.</p>
<p>Среди методов интерфейса Collection можно выделить следующие:</p>
<ul>
<li><p><code>boolean add (E item)</code>: добавляет в коллекцию объект item. При удачном добавлении возвращает true, при неудачном - false</p></li>
<li><p><code>boolean addAll (Collection&lt;? extends E&gt; col)</code>: добавляет в коллекцию все элементы из коллекции col. При удачном добавлении возвращает true, при неудачном - false</p></li>
<li><p><code>void clear ()</code>: удаляет все элементы из коллекции</p></li>
<li><p><code>boolean contains (Object item)</code>: возвращает true, если объект item содержится в коллекции, иначе возвращает false</p></li>
<li><p><code>boolean isEmpty ()</code>: возвращает true, если коллекция пуста, иначе возвращает false</p></li>
<li><p><code>Iterator&lt;E&gt; iterator ()</code>: возвращает объект Iterator для обхода элементов коллекции</p></li>
<li><p><code>boolean remove (Object item)</code>: возвращает true, если объект item удачно удален из коллекции, иначе возвращается false</p></li>
<li><p><code>boolean removeAll (Collection&lt;?&gt; col)</code>: удаляет все объекты коллекции col из текущей коллекции. Если текущая коллекция изменилась, возвращает true, 
иначе возвращается false</p></li>
<li><p><code>boolean retainAll (Collection&lt;?&gt; col)</code>: удаляет все объекты из текущей коллекции, кроме тех, которые содержатся в коллекции col. 
Если текущая коллекция после удаления изменилась, возвращает true, иначе возвращается false</p></li>
<li><p><code>int size ()</code>: возвращает число элементов в коллекции</p></li>
<li><p><code>Object[] toArray ()</code>: возвращает массив, содержащий все элементы коллекции</p></li>
</ul>
<p>Все эти и остальные методы, которые имеются в интерфейсе Collection, реализуются всеми коллекциями, поэтому в целом общие принципы работы с коллекциями 
будут одни и те же. Единообразный интерфейс упрощает понимание и работу с различными типами коллекций. Так, добавление элемента будет производиться с помощью метода <code>add</code>, который принимает добавляемый элемент в качестве параметра. 
Для удаления вызывается метод <code>remove()</code>. Метод <code>clear</code> будет очищать коллекцию, а метод <code>size</code> возвращать количество элементов в коллекции.</p>
...
<div class="nav"><p><a href="4.3.php">Назад</a><a href="./">Содержание</a><a href="5.2.php">Вперед</a></p></div></div>
<div class="socBlock">
