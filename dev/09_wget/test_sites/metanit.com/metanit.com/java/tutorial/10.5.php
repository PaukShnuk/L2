...
<html >
<head>
<title>Java | Метод reduce</title>
<meta charset="utf-8" />
<meta name="description" content="Операции сведения данных из потоков в Stream API в Java, метод reduce">
...
<h2>Метод reduce</h2><div class="date">Последнее обновление: 06.11.2020</div>
...
<p>Метод <span class="b">reduce</span> выполняет терминальные операции сведения, возвращая некоторое значение - результат операции.  
Он имеет следующие формы:</p>
<pre class="brush:java;">
Optional&lt;T&gt; reduce(BinaryOperator&lt;T&gt; accumulator)
T reduce(T identity, BinaryOperator&lt;T&gt; accumulator)
U reduce(U identity, BiFunction&lt;U,? super T,U&gt; accumulator, BinaryOperator&lt;U&gt; combiner)
</pre>
<p>Первая форма возвращает результат в виде объекта <span class="b">Optional&lt;T&gt;</span>. Например, вычислим произведение набора чисел:</p>
<pre class="brush:java;">
...
<p>Объект <code>BinaryOperator&lt;T&gt;</code> представляет функцию, которая принимает два элемента и выполняет над ними некоторую операцию, возвращая результат. 
При этом метод <code>reduce</code> сохраняет результат и затем опять же применяет к этому результату и следующему элементу в наборе бинарную операцию. 
Фактически в данном случае мы получим результат, который будет равен: <code>n1 op n2 op n3 op n4 op n5 op n6</code>, где op - это операция (в данном случае умножения), 
а n1, n2, ... - элементы из потока.</p>
...
<p>Вторая версия метода <code>reduce()</code> принимает два параметра:
<pre class="brush:java;">
T reduce(T identity, BinaryOperator&lt;T&gt; accumulator)
</pre>
<p>Первый параметр - <code>T identity</code> - элемент, который предоставляет начальное значение для функции из второго параметра, 
а также предоставляет значение по умолчанию, если поток не имеет элементов.</p>
<p>Второй параметр - <code>BinaryOperator&lt;T&gt; accumulator</code>, как и первая форма метода reduce, представляет ассоциативную функцию, которая запускается 
для каждого элемента в потоке и принимает два параметра. Первый параметр 
представляяет промежуточный результат функции, а второй параметр - следующий элемент в потоке. Фактически код этого метода будет равноценен следующей записи:</p>
<pre class="brush:java;">
T result = identity;
...
<p>То есть при первом вызове функция accumulator в качестве первого параметра принимает значение identity, а в качестве второго параметра - 
первый элемент потока. При втором вызове первым параметром служит результат первого вызова функции accumulator, а вторым параметром - второй элемент 
в потоке и так далее. Например:</p>
<pre class="brush:java;">
...
<p>В предыдущих примерах тип возвращаемых объектов совпадал с типом элементов, которые входят в поток. Однако это не всегда удобно. Возможно, мы захотим возвратить 
результат, тип которого отличается от типа объектов потока. Например, пусть у нас есть следующий класс Phone, представляющий телефон:</p>
<pre class="brush:java;">
class Phone{
...
<p>И мы хотим найти сумму цен тех телефонов, у которых цена меньше определенного значения. Для этого используем третью версию метода reduce:</p>
<pre class="brush:java;">
...
<p>Опять же здесь в качестве первого параметра идет значение по умолчанию - 0. Второй параметр производит бинарную операцию, которая получает промежуточное значение - суммарную 
цену текущего и предыдущего телефонов. Третий параметр представляет бинарную операцию, которая суммирует все промежуточные вычисления.</p>
...
<div class="nav"><p><a href="10.11.php">Назад</a><a href="./">Содержание</a><a href="10.12.php">Вперед</a></p></div></div>
<div class="socBlock">
