...
<html >
<head>
<title>Java | Структура программы</title>
<meta charset="utf-8" />
<meta name="description" content="Структура программы на языке Java, инструкции и блоки кода, метод main и его роль, использование комментариев">
...
<h1>Основы программирования на Java</h1><h2>Структура программы</h2><div class="date">Последнее обновление: 12.04.2018</div>
...
<p>Основным строительным блоком программы на языке Java являются <span class="b">инструкции</span> (statement). 
...
конец инструкции. Например:</p>
<pre class="brush:java;">
System.out.println("Hello Java!");
</pre>
<p>Данная строка представляет вызов метода <code>System.out.println</code>, который выводит на консоль строку "Hello Java!". В данном случае вызов 
метода является инструкцией и поэтому завершается точкой с запятой.</p>
<p>Кроме отдельных инструкций распространенной конструкцией является блок кода. Блок кода содержит набор инструкций, он заключается в фигурные скобки, а инструкции помещаются между открывающей и закрывающей фигурными скобками:</p>
<pre class="brush:java;">
{
...
<p>В этом блоке кода две инструкции, которые выводят на консоль определенную строку.</p>
<h3>Выполнение программы. Метод main</h3>
<p>Java является объектно-ориентированным языком, поэтому всю программу можно представить как набор взаимодействующих между собой классов и объектов. 
В первой главе при создании первого приложения программа была определена следующим образом:</p>
<pre class="brush:java;">
...
<p>То есть основу нашей программы составляет класс Program. При определении класса вначале идет модификатор доступа <span class="b">public</span>, 
который указывает, что данный класс будет доступен всем, то есть мы сможем его запустить из командной строки. Далее идет ключевое слово <span class="b">class</span>, 
а затем название класса. После названия класса идет блок кода, в котором расположено содержимое класса.</p>
<p>Входной точкой в программу на языке Java является метод <span class="b">main</span>, который определен в классе Program. Именно с него начинается 
выполнение программы. Он обязательно должен присутствовать в программе. При этом его заголовок может быть только таким:</p>
<pre class="brush:java;">public static void main (String args[])</pre>
<p>При запуске приложения виртуальная машина Java ищет в главном классе программы метод main с подобным заголовком, и после его обнаружения запускает его.</p>
<p>Вначале заголовка метода идет модификатор <code>public</code>, который указывает, что метод будет доступен извне. Слово <code>static</code> указывает, что метод 
main - статический, а слово <code>void</code> - что он не возвращает никакого значения. Далее в скобках у нас идут параметры метода - <code>String args[]</code> - 
это массив args, который хранит значения типа String, то есть строки. При запуске программы через этот массив мы можем передать в программу различные данные.</p>
<p>После заголовка метода идет его блок, который содержит набор выполняемых инструкций.</p>
<h3>Комментарии</h3>
<p>Код программы может содержать комментарии. Комментарии позволяют понять смысл программы, что делают те или иные ее части. При компиляции комментарии 
игнорируются и не оказывают никакого влияния на работу приложения и на его размер.</p>
<p>В Java есть два типа комментариев: однострочный и многострочный. Однострочный комментарий размещается на одной строке после двойного слеша //. 
А многострочный комментарий заключается между символами <span class="b">/* текст комментария */</span>. Он может размещаться на нескольких строках. 
Например:</p>
<pre class="brush:java;">
/*
