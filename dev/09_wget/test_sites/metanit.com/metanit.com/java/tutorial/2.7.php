...
<html >
<head>
<title>Java | Методы</title>
<meta charset="utf-8" />
<meta name="description" content="Определение и использование методов языке Java, процедуры и функции, возвращаемые значения">
...
<h2>Методы</h2><div class="date">Последнее обновление: 17.04.2018</div>
...
<p>По умолчанию главный класс любой программы на Java содержит метод main, который служит точкой входа в программу:</p>
<pre class="brush:java;">
public static void main(String[] args) {
	System.out.println("привет мир!");
}
</pre>
<p>Ключевые слова <code>public</code> и <code>static</code> являются модификаторами. Далее идет тип возвращаемого значения. 
Ключевое слово <code>void</code> указывает на то, что метод ничего не возвращает.</p>
<p>Затем идут название метода - main и в скобках параметры метода - <code>String[] args</code>. И в фигурные скобки заключено тело метода - все действия, 
которые он выполняет.</p>
...
<p>Здесь определены два дополнительных метода: hello и welcome, каждый из которых выводит некоторую строку на консоль. Методы определяются внутри класса - в данном случае внутри класса Program, 
в котором определен метод main.</p>
<p>Но если мы скомпилируем и запустим данную программу, то мы ничего не увидим на консоли. В примере выше мы определили два метода, но мы их нигде не вызываем. По умолчанию в программе Java 
выполняется только метод main и все его содержимое. Поэтому, если мы хотим, чтобы другие методы тоже выполнялись, их надо вызвать в методе main.</p>
...
<p>В методе main вызывается один раз метод hello и два раза метод welcome. В этом и заключается одно из преимуществ методов: мы можем вынести некоторые общие действия в 
отдельный метод и затем вызывать многократно их в различных местах программы. Поскольку оба метода не имеют никаких параметров, то после их названия при вызове ставятся пустые скобки.</p>
<p>Также следует отметить, что чтобы вызвать в методе main другие методы, которые определены в одном классе с методом main, они должны иметь модификатор 
<code>static</code>.</p>
...
<div class="nav"><p><a href="2.4.php">Назад</a><a href="./">Содержание</a><a href="2.16.php">Вперед</a></p></div></div>
<div class="socBlock">
