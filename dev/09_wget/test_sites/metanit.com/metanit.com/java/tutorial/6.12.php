...
<html >
<head>
<title>Java | Работа с ZIP архивами</title>
<meta charset="utf-8" />
<meta name="description" content="Создание, упаковка и распаковка zip архивов в языке программирования Java">
...
<h2>Работа с ZIP-архивами</h2><div class="date">Последнее обновление: 26.04.2018</div>
...
<p>Кроме общего функционала для работы с файлами Java предоставляет функциональность для работы с таким видом файлов как zip-архивы. 
Для этого в пакете <span class="ii">java.util.zip</span> определены два класса - <span class="b">ZipInputStream</span> и <span class="b">ZipOutputStream</span></p>
<h3>ZipOutputStream. Запись архивов</h3>
<p>Для создания архива используется класс ZipOutputStream. Для создания объекта ZipOutputStream в его конструктор передается поток вывода:</p>
<pre class="brush:java;">
ZipOutputStream(OutputStream out)
</pre>
<p>Для записи файлов в архив для каждого файла создается объект <code>ZipEntry</code>, в конструктор которого передается имя архивируемого файла. А чтобы 
добавить каждый объект ZipEntry в архив, применяется метод <span class="bb">putNextEntry()</span>.</p>
<p>Создадим архив:</p>
<pre class="brush:java;">
import java.io.*;
...
<p>После добавления объекта ZipEntry в поток нам также надо добавить в него и содержимое файла. Для этого используется метод write, записывающий в поток 
массив байтов: <code>zout.write(buffer);</code>. В конце надо закрыть ZipEntry с помощью метода <code>closeEntry()</code>. После этого можно добавлять в архив 
новые файлы - в этом случае все вышеописанные действия для каждого нового файла повторяются.</p>
<h3>Чтение архивов. ZipInputStream</h3>
...
<pre class="brush:java;">ZipInputStream(InputStream in)</pre>
<p>Для считывания файлов из архива ZipInputStream использует метод <span class="b">getNextEntry()</span>, который возвращает объект <span class="b">ZipEntry</span>. 
Объект ZipEntry представляет отдельную запись в zip-архиве. Например, считаем какой-нибудь архив:</p>
<pre class="brush:java;">
import java.io.*;
import java.util.zip.*;
...
<p>ZipInputStream в конструкторе получает ссылку на поток ввода. И затем в цикле выводятся все файлы и их размер в байтах, которые находятся в данном архиве.</p>
<p>Затем данные извлекаются из архива и сохраняются в новые файлы, которые находятся в той же папке и которые начинаются с "new".</p>
