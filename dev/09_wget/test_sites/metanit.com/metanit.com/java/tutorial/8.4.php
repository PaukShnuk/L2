...
<html >
<head>
<title>Java | Завершение и прерывание потока</title>
<meta charset="utf-8" />
<meta name="description" content="Завершение и прерывание потоков в языке программирования Java, отслеживание состояния булевой переменной, методы interrupt и isInterrupted, исключение InterruptedException">
...
<h2>Завершение и прерывание потока</h2><div class="date">Последнее обновление: 27.04.2018</div>
...
<p>Примеры потоков ранее представляли поток как последовательный набор операций. После выполнения 
последней операции завершался и поток. Однако нередко имеет место и другая организация потока в виде бесконечного цикла. Например, поток сервера в бесконечном 
цикле прослушивает определенный порт на предмет получения данных. И в этом случае мы также можем предусмотреть механизм завершения потока. </p>
<h3>Завершение потока</h3>
<p>Распространенный способ завершения потока представляет опрос логической переменной. И если она равна, например, false, то поток завершает бесконечный цикл и заканчивает свое выполнение.</p>
...
<p>Итак, вначале запускается дочерний поток: <code>new Thread(myThread,"MyThread").start()</code>. Затем на 1100 миллисекунд останавливаем Main thread и 
потом вызываем метод <code>myThread.disable()</code>, который переключает в потоке флаг isActive. И дочерний поток завершается.</p>
<h3>Метод interrupt</h3>
<p>Еще один способ вызова завершения или прерывания потока представляет метод <span class="b">interrupt()</span>. Вызов этого метода устанавливает 
у потока статус, что он прерван. Сам метод возвращает true, если поток может быть прерван, в ином случае возвращается false.</p>
<p>При этом сам вызов этого метода НЕ завершает поток, он только устанавливает статус: в частности, метод <span class="b">isInterrupted()</span> класса Thread будет возвращать значение true. 
Мы можем проверить значение возвращаемое данным методом и прозвести некоторые действия. Например:</p>
<pre class="brush:java;">
...
<p>В классе, который унаследован от Thread, мы можем получить статус текущего потока с помощью метода <span class="b">isInterrupted()</span>. 
И пока этот метод возвращает false, мы можем выполнять цикл. А после того, как будет вызван метод interrupt, <code>isInterrupted()</code> возвратит true, и соответственно 
произойдет выход из цикла.</p>
...
<p>Если основная функциональность заключена в классе, который реализует интерфейс Runnable, то там можно проверять статус потока с помощью метода 
<span class="b">Thread.currentThread().isInterrupted()</span>:</p>
<pre class="brush:java;">
class MyThread implements Runnable {
...
<p>Однако при получении статуса потока с помощью метода <code>isInterrupted()</code> следует учитывать, что если мы обрабатываем в цикле исключение 
<span class="b">InterruptedException</span> в блоке catch, то при перехвате исключения статус потока автоматически сбрасывается, и после этого 
isInterrupted будет возвращать false.</p>
<p>Например, добавим в цикл потока задержку с помощью метода sleep:</p>
<pre class="brush:java;">
public void run(){
...
<p>Когда поток вызовет метод interrupt, метод sleep сгенерирует исключение InterruptedException, и управление перейдет к блоку catch. Но если мы проверим статус потока, 
то увидим, что метод isInterrupted возвращает false. Как вариант, в этом случае мы можем повторно прервать текущий поток, опять же вызвав метод interrupt(). 
Тогда при новой итерации цикла while метода isInterrupted возвратит true, и поизойдет выход из цикла.</p>
<p>Либо мы можем сразу же в блоке catch выйти из цикла с помощью break:</p>
<pre class="brush:java;">
while(!isInterrupted()){
...
<p>Если бесконечный цикл помещен в конструкцию try...catch, то достаточно обработать InterruptedException:</p>
<pre class="brush:java;">
public void run(){
...
<div class="nav"><p><a href="8.2.php">Назад</a><a href="./">Содержание</a><a href="8.3.php">Вперед</a></p></div></div>
<div class="socBlock">
