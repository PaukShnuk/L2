...
<html >
<head>
<title>Java | Зависимые модули</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение модулей в языке программирования Java, оператор requires">
...
<h2>Зависимые модули</h2><div class="date">Последнее обновление: 01.05.2018</div>
...
<p>Одни модули могут зависить от других, например, как-то использовать их. Например, возьмем проект из прошлой темы с простейшим модулем:</p>
<img src="./pics/11.1.png" alt="Модули в Java 9" />
<p>В файле <span class="b">Hello.java</span> определим вывод сообщения не на консоль, а через графическое окно, которое предоставляется инфраструктурой Swing:</p>
...
<p>Для вывода сообщения используется статический метод <code>showMessageDialog()</code> класса <code>JOptionPane</code>, 
который расположен в пакете <span class="ii">javax.swing.JOptionPane</span>. Чтобы использовать данный класс, он импортируется в начале файла. 
...
где определен данный класс.</p>
<p>Как узнать, в каком модуле расположен тот или иной класс? Если речь идет о встроенных классах, то для этого надо смотреть документацию, причем сразу по Java (JDK) 9+. В начале описания 
класса, как правило, указывается, к какому модулю он принадлежит.</p>
<img src="./pics/11.4.png" alt="поиск модулей в Java 9" />
<p>Итак, документация нам сообщает, что класс JOptionPane расположен в модуле <span class="b">java.desktop</span>. Теперь укажем нашему модулю, что нам надо использовать модуль 
java.desktop. Для этого перейдем к определению модуля в файле <span class="b">module-info.java</span> и изменим его следующим образом:</p>
<pre class="brush:java;">
module demo {
    requires java.desktop;
}
</pre>
<p>После оператора <span class="b">requires</span> указывается название модуля, от которого зависит наш модуль. Соответственно если наш модуль использует 
несколько других модулей, то также с помощью оператора <code>require</code> можно указать все используемые модули.</p>
<p>Далее скомпилируем модуль и запустим его на выполнение. После запуска нам отобразится графическое окно:</p>
<img src="./pics/11.3.png" alt="requires в модулях в Java 9" />
