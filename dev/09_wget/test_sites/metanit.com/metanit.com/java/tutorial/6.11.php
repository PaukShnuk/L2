...
<html >
<head>
<title>Java | Класс File. Работа с файлами и каталогами</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с файлами и каталогами в языке программирования Java, класс File и его методы, создание, удаление и переименование файлов и папок ">
...
<h2>Класс File. Работа с файлами и каталогами</h2><div class="date">Последнее обновление: 26.04.2018</div>
...
<p>Класс File, определенный в пакете java.io, не работает напрямую с потоками. Его задачей является управление информацией о файлах и каталогах. Хотя на уровне операционной системы 
файлы и каталоги отличаются, но в Java они описываются одним классом File.</p>
<p>В зависимости от того, что должен представлять объект File - файл или каталог, мы можем использовать один из конструкторов для создания объекта:</p>
...
<p>Класс File имеет ряд методов, которые позволяют управлять файлами и каталогами. Рассмотрим некоторые из них:</p>
<ul>
<li><p><span class="b">boolean createNewFile()</span>: создает новый файл по пути, который передан в конструктор. В случае удачного создания возвращает 
true, иначе false</p></li>
<li><p><span class="b">boolean delete()</span>: удаляет каталог или файл по пути, который передан в конструктор. При удачном удалении возвращает 
true.</p></li>
<li><p><span class="b">boolean exists()</span>: проверяет, существует ли по указанному в конструкторе пути файл или каталог. И если файл или каталог существует, 
то возвращает true, иначе возвращает false</p></li>
<li><p><span class="b">String getAbsolutePath()</span>: возвращает абсолютный путь для пути, переданного в конструктор объекта</p></li>
<li><p><span class="b">String getName()</span>: возвращает краткое имя файла или каталога</p></li>
<li><p><span class="b">String getParent()</span>: возвращает имя родительского каталога</p></li>
<li><p><span class="b">boolean isDirectory()</span>: возвращает значение true, если по указанному пути располагается каталог</p></li>
<li><p><span class="b">boolean isFile()</span>: возвращает значение true, если по указанному пути находится файл</p></li>
<li><p><span class="b">boolean isHidden()</span>: возвращает значение true, если каталог или файл являются скрытыми</p></li>
<li><p><span class="b">long length()</span>: возвращает размер файла в байтах</p></li>
<li><p><span class="b">long lastModified()</span>: возвращает время последнего изменения файла или каталога. Значение представляет количество 
миллисекунд, прошедших с начала эпохи Unix</p></li>
<li><p><span class="b">String[] list()</span>: возвращает массив файлов и подкаталогов, которые находятся в определенном каталоге</p></li>
<li><p><span class="b">File[] listFiles()</span>: возвращает массив файлов и подкаталогов, которые находятся в определенном каталоге</p></li>
<li><p><span class="b">boolean mkdir()</span>: создает новый каталог и при удачном создании возвращает значение true</p></li>
<li><p><span class="b">boolean renameTo(File dest)</span>: переименовывает файл или каталог</p></li>
</ul>
<h3>Работа с каталогами</h3>
<p>Если объект File представляет каталог, то его метод <code>isDirectory()</code> возвращает <code>true</code>. И поэтому мы можем получить его 
содержимое - вложенные подкаталоги и файлы с помощью методов <code>list()</code> и <code>listFiles()</code>. Получим все подкаталоги и файлы в определенном каталоге:</p>
<pre class="brush:java;">
import java.io.File;
...
<p>Теперь выполним еще ряд операций с каталогами, как удаление, переименование и создание:</p>
<pre class="brush:java;">
import java.io.File;
...
<p>Работа с файлами аналогична работе с каталога. Например, получим данные по одному из файлов и создадим еще один файл:</p>
<pre class="brush:java;">
import java.io.File;
...
<p>При создании нового файла метод <code>createNewFile()</code> в случае неудачи выбрасывает исключение <span class="b">IOException</span>, поэтому нам надо 
его отлавливать, например, в блоке try...catch, как делается в примере выше.</p>
...
<div class="nav"><p><a href="6.10.php">Назад</a><a href="./">Содержание</a><a href="6.12.php">Вперед</a></p></div></div>
<div class="socBlock">
