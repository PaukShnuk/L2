...
<html >
<head>
<title>Java | Интерфейсы SortedMap и NavigableMap. Класс TreeMap</title>
<meta charset="utf-8" />
<meta name="description" content="Класс TreeMap в языке программирования Java, древовидные отображения, интерфейсы SortedMap и NavigableMap, сортировка объектов в дереве">
...
<h2>Интерфейсы SortedMap и NavigableMap. Класс TreeMap</h2><div class="date">Последнее обновление: 25.04.2018</div>
...
<p>Для создания отображений Java также предоставляет ряд дополнительных интерфейсов: SortedMap и NavigableMap</p>
<h3>SortedMap</h3>
<p>Интерфейс <span class="b">SortedMap</span> расширяет Map и создает отображение, в котором все элементы отсортированы в порядке возрастания их ключей. 
SortedMap добавляет ряд методов:</p>
<ul>
<li><p><code>K firstKey()</code>: возвращает ключ первого элемента отображения</p></li>
<li><p><code>K lastKey()</code>: возвращает ключ последнего элемента отображения</p></li>
<li><p><code>SortedMap&lt;K, V&gt; headMap(K end)</code>: возвращает отображение SortedMap, которые содержит все элементы оригинального SortedMap вплоть до элемента с ключом end</p></li>
<li><p><code>SortedMap&lt;K, V&gt; tailMap(K start)</code>: возвращает отображение SortedMap, которые содержит все элементы оригинального SortedMap, начиная с 
элемента с ключом start</p></li>
<li><p><code>SortedMap&lt;K, V&gt; subMap(K start, K end)</code>: возвращает отображение SortedMap, которые содержит все элементы оригинального 
SortedMap вплоть от элемента с ключом start до элемента с ключом end</p></li>
</ul>
<h3>NavigableMap</h3>
<p>Интерфейс <span class="b">NavigableMap</span> расширяет интерфейс SortedMap и обеспечивает возможность получения элементов отображения относительно других элементов. 
Его основные методы:</p>
<ul>
<li><p><code>Map.Entry&lt;K, V&gt; ceilingEntry(K obj)</code>: возвращает элемент с наименьшим ключом k, который больше или равен ключу obj 
(k &gt;=obj). Если такого ключа нет, то возвращается null.</p></li>
<li><p><code>Map.Entry&lt;K, V&gt; floorEntry(K obj)</code>: возвращает элемент с наибольшим ключом k, который меньше или равен ключу obj (k &lt;=obj). 
Если такого ключа нет, то возвращается null.</p></li>
<li><p><code>Map.Entry&lt;K, V&gt; higherEntry()</code>: возвращает элемент с наименьшим ключом k, который больше ключа obj (k &gt;obj). 
Если такого ключа нет, то возвращается null.</p></li>
<li><p><code>Map.Entry&lt;K, V&gt; lowerEntry()</code>: возвращает элемент с наибольшим ключом k, который меньше ключа obj (k &lt;obj). 
Если такого ключа нет, то возвращается null.</p></li>
<li><p><code>Map.Entry&lt;K, V&gt; firstEntry()</code>: возвращает первый элемент отображения</p></li>
<li><p><code>Map.Entry&lt;K, V&gt; lastEntry()</code>: возвращает последний элемент отображения</p></li>

<li><p><code>Map.Entry&lt;K, V&gt; pollFirstEntry()</code>: возвращает и одновременно удаляет первый элемент из отображения</p></li>
<li><p><code>Map.Entry&lt;K, V&gt; pollLastEntry()</code>: возвращает и одновременно удаляет последний элемент из отображения</p></li>

<li><p><code>K ceilingKey(K obj)</code>: возвращает наименьший ключ k, который больше или равен ключу obj (k &gt;=obj). Если такого ключа нет, то возвращается null.</p></li>
<li><p><code>K floorKey(K obj)</code>: возвращает наибольший ключ k, который меньше или равен ключу obj (k &lt;=obj). Если такого ключа нет, то возвращается null.</p></li>
<li><p><code>K lowerKey(K obj)</code>: возвращает наибольший ключ k, который меньше ключа obj (k &lt;obj). Если такого ключа нет, то возвращается null.</p></li>
<li><p><code>K higherKey(K obj)</code>: возвращает наименьший ключ k, который больше ключа obj (k &gt;obj). Если такого ключа нет, то возвращается null.</p></li>

<li><p><code>NavigableSet&lt;K&gt; descendingKeySet()</code>: возвращает объект NavigableSet, который содержит все ключи отображения в обратном порядке</p></li>
<li><p><code>NavigableMap&lt;K, V&gt; descendingMap()</code>: возвращает отображение NavigableMap, которое содержит все элементы в обратном порядке</p></li>
<li><p><code>NavigableSet&lt;K&gt; navigableKeySet()</code>: возвращает объект NavigableSet, который содержит все ключи отображения</p></li>

<li><p><code>NavigableMap&lt;K, V&gt; headMap(K upperBound, boolean incl)</code>: возвращает отображение NavigableMap, которое содержит все 
элементы оригинального NavigableMap вплоть от элемента с ключом upperBound. Параметр incl при значении true указывает, что элемент с ключом upperBound также включается в выходной набор.</p></li>
<li><p><code>NavigableMap&lt;K, V&gt; tailMap(K lowerBound, boolean incl)</code>: возвращает отображение NavigableMap, которое содержит все 
элементы оригинального NavigableMap, начиная с элемента с ключом lowerBound. Параметр incl при значении true указывает, что элемент с ключом lowerBound также включается в выходной набор.</p></li>
<li><p><code>NavigableMap&lt;K, V&gt; subMap(K lowerBound, boolean lowIncl, K upperBound, boolean highIncl)</code>: возвращает отображение 
NavigableMap, которое содержит все 
элементы оригинального NavigableMap от элемента с ключом lowerBound до элемента с ключом upperBound. Параметры lowIncl и highIncl 
при значении true включают в выходной набор элементы с ключами lowerBound и upperBound соответственно.</p></li>
</ul>
<h3>TreeMap</h3>
<p>Класс TreeMap&lt;K, V&gt; представляет отображение в виде дерева. Он наследуется от класса AbstractMap и реализует интерфейс NavigableMap, а следовательно, также и 
интерфейс SortedMap. Поэтому в отличие от коллекции HashMap в TreeMap все объекты автоматически сортируются по возрастанию их ключей.</p>
<p>Класс TreeMap имеет следующие конструкторы:</p>
<ul>
<li><p><code>TreeMap()</code>: создает пустое отображение в виде дерева</p></li>
<li><p><code>TreeMap(Map&lt;? extends K,​? extends V&gt; map)</code>: создает дерево, в которое добавляет все элементы из отображения map</p></li>
<li><p><code>TreeMap(SortedMap&lt;K, ? extends V&gt; smap)</code>: создает дерево, в которое добавляет все элементы из отображения smap</p></li>
<li><p><code>TreeMap(Comparator&lt;? super K&gt; comparator)</code>: создает пустое дерево, где все добавляемые элементы впоследствии будут 
отсортированы компаратором.</p></li>
</ul>
...
<p>Кроме собственно методов интерфейса Map класс TreeMap реализует методы интерфейса NavigableMap. Например, мы можем получить 
все объекты до или после определенного ключа с помощью методов <code>headMap</code> и <code>tailMap</code>. Также мы можем получить первый и последний элементы 
и провести ряд дополнительных манипуляций с объектами.</p>
...
<div class="nav"><p><a href="5.8.php">Назад</a><a href="./">Содержание</a><a href="5.10.php">Вперед</a></p></div></div>
<div class="socBlock">
