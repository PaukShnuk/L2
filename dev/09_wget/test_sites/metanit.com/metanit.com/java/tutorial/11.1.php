...
<html >
<head>
<title>Java | Модульность. Создание модуля</title>
<meta charset="utf-8" />
<meta name="description" content="Модульность в языке программирования Java, создание простейшего модуля">
...
<h1>Модульность</h1><h2>Создание модуля</h2><div class="date">Последнее обновление: 01.05.2018</div>
...
<p>С выходом JDK 9 в языке Java появилась новая возможность - модульность. Модульность позволяет разбить код на отдельные структурные единицы - 
...
модуля.</p>
<p>До Java 9 было несколько уровней инкапсуляции функционала. Первый уровень представлял класс, в котором мы могли определить переменные и методы с 
...
оказалось недостаточно. И модуль стал следующим уровнем инкапсуляции, который объединял несколько пакетов.</p>
<p>Модуль состоит из группы пакетов. Также модуль включает список все пакетов, которые входят в модуль, и список всех модулей, от которых зависит данный модуль. 
Дополнительно (но необязательно) он может включать файлы ресурсов или файлы нативных библиотек.</p>
<p>В качестве названия модуля может использоваться произвольный идентификатор из алфавитно-цифровых символов и знаков подчеркивания. Но рекомендуется, 
чтобы название модуля соответствовало названию, которого начинаются пакеты этого модуля.</p>
<p>Определим и используем простейший модуль. Допустим, файлы с исходными кодами помещаются в папку C:\java (либо какую-нибудь другую папку на жестком диске). 
Создадим в этой папке каталог, который назовем <span class="b">demo</span>. Этот каталог будет представлять модуль.</p>
<p>В каталоге demo определим новый файл <span class="b">module-info.java</span> со следующим кодом:</p>
<pre class="brush:java;">
module demo {
}
</pre>
<p>Этот файл представляет дескриптор модуля (module descriptor). Этот файл может содержать только определение модуля.</p>
<p>С помощью ключевого слова <span class="b">module</span> определяется модуль, который называется demo, то есть так же, как и каталог, в котором данный файл расположен. 
После имени модуля с помощью фигурных скобок можно определить тело модуля, но в данном случае код модуля не содержит никаких инструкций.</p>
<p>Далее в каталоге demo создадим папку <span class="b">com</span>. В папке com создадим папку <span class="b">metanit</span>, а в папке com/metanit - 
папку <span class="b">hello</span>.</p>
<p>В папке <span class="ii">com/metanit/hello</span> определим новый файл <span class="b">Hello.java</span>:</p>
<pre class="brush:java;">
...
<p>Название пакета файла - <span class="ii">com.metanit.hello</span> отражает структуру папок, в которых расположен файл. Сам файл определяет класс Hello, 
который в методе main выводит на консоль строку.</p>
<p>В итоге у нас получится следующая стуктура проекта:</p>
<img src="./pics/11.1.png" alt="Модули в Java 9" />
<p>Теперь скомпилируем все это. Для этого вначале перейдем в командной строке/терминале к папке, в которой находится модуль demo.</p>
<p>Затем для компиляции модуля выполним следующую команду:</p>
<div class="console">
<pre class="consoletext">
javac demo/module-info.java demo/com/metanit/hello/Hello.java
</pre>
</div>
<p>После компиляции модуля demo выполним программу с помощью следующей команды:</p>
<div class="console">
<pre class="consoletext">
java  --module-path demo --module demo/com.metanit.hello.Hello
</pre>
</div>
<p>Параметр <span class="ii">--module-path</span> указывает на путь к модулю, а <span class="ii">--module</span> - на главный класс модуля.</p>
<p>При наборе команды вместо параметра <span class="ii">--module-path</span> можно указать его сокращение <span class="ii">-p</span>, а вместо 
параметра <span class="ii">--module</span> - сокращение <span class="ii">-m</span>.</p>
<p>И на консоли отобразится сообщение "Hello Demo Module!"</p>
<img src="./pics/11.2.png" alt="Компиляция и выполнение модуля в Java" />
...
<div class="nav"><p><a href="10.10.php">Назад</a><a href="./">Содержание</a><a href="11.2.php">Вперед</a></p></div></div>
<div class="socBlock">
