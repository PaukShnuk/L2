...
<html >
<head>
<title>Java | Интерфейс Map и класс HashMap</title>
<meta charset="utf-8" />
<meta name="description" content="Организация набор, каждый элемент которых имеет ключ и значение, в языке программирования Java, интерфейс Map и класс HashMap">
...
<h2>Интерфейс Map и класс HashMap</h2><div class="date">Последнее обновление: 25.04.2018</div>
...
<p>Интерфейс <span class="b">Map&lt;K, V&gt;</span> представляет отображение или иначе говоря словарь, где каждый элемент представляет пару "ключ-значение". При этом 
все ключи уникальные в рамках объекта Map. Такие коллекции облегчают поиск элемента, если нам известен ключ - уникальный идентификатор объекта.</p>
<p>Следует отметить, что в отличие от других интерфейсов, которые представляют коллекции, интерфейс Map НЕ расширяет интерфейс Collection.</p>
<p>Среди методов интерфейса Map можно выделить следующие:</p>
<ul>
<li><p><code>void clear()</code>: очищает коллекцию</p></li>
<li><p><code>boolean containsKey(Object k)</code>: возвращает true, если коллекция содержит ключ k</p></li>
<li><p><code>boolean containsValue(Object v)</code>: возвращает true, если коллекция содержит значение v</p></li>
<li><p><code>Set&lt;Map.Entry&lt;K, V&gt;&gt; entrySet()</code>: возвращает набор элементов коллекции. Все элементы представляют объект <code>Map.Entry</code></p></li>
<li><p><code>boolean equals(Object obj)</code>: возвращает true, если коллекция идентична коллекции, передаваемой через параметр obj</p></li>
<li><p><code>boolean isEmpty</code>: возвращает true, если коллекция пуста</p></li>
<li><p><code>V get(Object k)</code>: возвращает значение объекта, ключ которого равен k. Если такого элемента не окажется, то возвращается значение <code>null</code></p></li>
<li><p><code>V getOrDefault(Object k, V defaultValue)</code>: возвращает значение объекта, ключ которого равен k. Если такого элемента не окажется, 
то возвращается значение <code>defaultVlue</code></p></li>
<li><p><code>V put(K k, V v)</code>: помещает в коллекцию новый объект с ключом k и значением v. Если в коллекции уже есть объект с подобным ключом, 
...
то возвращается значение <code>null</code></p></li>
<li><p><code>V putIfAbsent(K k, V v)</code>: помещает в коллекцию новый объект с ключом k и значением v, если в коллекции еще нет элемента с подобным ключом.</p></li>
<li><p><code>Set&lt;K&gt; keySet()</code>: возвращает набор всех ключей отображения</p></li>
<li><p><code>Collection&lt;V&gt; values()</code>: возвращает набор всех значений отображения</p></li>
<li><p><code>void putAll(Map&lt;? extends K, ? extends V&gt; map)</code>: добавляет в коллекцию все объекты из отображения map</p></li>
<li><p><code>V remove(Object k)</code>: удаляет объект с ключом k</p></li>
<li><p><code>int size()</code>: возвращает количество элементов коллекции</p></li>
</ul>
<p>Чтобы положить объект в коллекцию, используется метод <code>put</code>, а чтобы получить по ключу - метод <code>get</code>. Реализация интерфейса 
Map также позволяет получить наборы как ключей, так и значений. А метод <code>entrySet()</code> возвращает набор всех элементов в виде объектов 
<span class="b">Map.Entry&lt;K, V&gt;</span>.</p>
<p>Обобщенный интерфейс <code>Map.Entry&lt;K, V&gt;</code> представляет объект с ключом типа K и значением типа V и определяет следующие методы:</p>
<ul>
<li><p><code>boolean equals(Object obj)</code>: возвращает true, если объект obj, представляющий интерфейс <code>Map.Entry</code>, идентичен текущему</p></li>
<li><p><code>K getKey()</code>: возвращает ключ объекта отображения</p></li>
<li><p><code>V getValue()</code>: возвращает значение объекта отображения</p></li>
<li><p><code>V setValue(V v)</code>: устанавливает для текущего объекта значение v</p></li>
<li><p><code>int hashCode()</code>: возвращает хеш-код данного объекта</p></li>
</ul>
<p>При переборе объектов отображения мы будем оперировать этими методами для работы с ключами и значениями объектов.</p>
<h3>Классы отображений. HashMap</h3>
<p>Базовым классом для всех отображений является абстрактный класс <span class="b">AbstractMap</span>, 
который реализует большую часть методов интерфейса Map. Наиболее распространенным классом отображений является <span class="b">HashMap</span>, 
который реализует интерфейс Map и наследуется от класса AbstractMap.</p>
...
<p>Чтобы добавить или заменить элемент, используется метод put, либо replace, а чтобы получить его значение по ключу - метод get. С помощью других методов 
интерфейса Map также производятся другие манипуляции над элементами: перебор, получение ключей, значений, удаление.</p>
