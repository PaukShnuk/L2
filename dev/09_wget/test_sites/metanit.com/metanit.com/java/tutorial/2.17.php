...
<html >
<head>
<title>Java | Оператор return. Результат метода</title>
<meta charset="utf-8" />
<meta name="description" content="Оператор return в языке программирования Java, возвращение результата из метода и выход из метода">
...
<h2>Оператор return. Результат метода</h2><div class="date">Последнее обновление: 18.04.2018</div>
...
<p>Методы могут возвращать некоторое значение. Для этого применяется оператор <span class="b">return</span>.</p>
<pre class="brush:java;">return возвращаемое_значение;</pre>
<p>После оператора return указывается возвращаемое значение, которое является результатом метода. Это может быть литеральное значение, значение переменной или 
какого-то сложного выражения.</p>
<p>Например:</p>
...
<code>int</code>, поэтому этот тип указывается перед названием метода. Причем если в качестве возвращаемого типа для метода определен любой 
другой, отличный от <code>void</code>, то метод обязательно должен использовать оператор <span class="b">return</span> для возвращения значения.</p>
<p>При этом возвращаемое значение всегда должно иметь тот же тип, что значится в определении функции. И если функция 
возвращает значение типа <code>int</code>, то после оператора return стоит целочисленное значение, 
которое является объектом типа <code>int</code>. Как в данном случае это сумма значений параметров метода.</p>
<p>Метод может использовать несколько вызовов оператора return для возваращения разных значений в зависимости от некоторых условий:</p>
<pre class="brush:java;">
public class Program{
...
<p>Здесь метод daytime возвращает значение типа String, то есть строку, и в зависимости от значения параметра hour возвращаемая строка будет различаться.</p>
<h3>Выход из метода</h3>
<p>Оператор return применяется для возвращаения значения из метода, но и для выхода из метода. В подобном качестве оператор return применяется в методах, которые ничего не возвращают, 
то есть имеют тип <code>void</code>:</p>
<pre class="brush:java;">
public class Program{
...
<p>Если переданное в метод datetime значение больше 24 или меньше 0, то просто выходим из метода. Возвращаемое значение после return указывать в этом случае не нужно.</p>
