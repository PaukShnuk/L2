...
<html >
<head>
<title>Java | Создание своих классов исключений</title>
<meta charset="utf-8" />
<meta name="description" content="Создание своих классов исключений в Java, примеры использования">
...
<h2>Создание своих классов исключений</h2><div class="date">Последнее обновление: 23.04.2018</div>
...
<p>Хотя имеющиеся в стандартной библиотеке классов Java классы исключений описывают большинство исключительных ситуаций, которые могут возникнуть 
при выполнении программы, все таки иногда требуется создать свои собственные классы исключений со своей логикой.</p>
<p>Чтобы создать свой класс исключений, надо унаследовать его от класса Exception. Например, у нас есть класс, вычисляющий факториал, и нам надо 
выбрасывать специальное исключение, если число, передаваемое в метод, меньше 1:</p>
...
<p>Здесь для определения ошибки, связанной с вычислением факториала, определен класс <code>FactorialException</code>, который наследуется от Exception 
...
<code>super(message)</code>. Кроме того, отдельное поле предназначено для хранения числа, факториал которого вычисляется.</p>
<p>Для генерации исключения в методе вычисления факториала выбрасывается исключение с помощью оператора throw: <code>throw new FactorialException("Число не может быть меньше 1", num)</code>. 
...
<code>public static int getFactorial(int num) throws FactorialException</code></p>
<p>Теперь используем класс в методе main:</p>
<pre class="brush:java;">
