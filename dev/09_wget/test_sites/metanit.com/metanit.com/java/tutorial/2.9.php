...
<html >
<head>
<title>Java | Консольный ввод-вывод</title>
<meta charset="utf-8" />
<meta name="description" content="Вывод на консоль и ввод с консоли в языке java, метод System.out.println и объект System.in, класс Scanner, ввод дробных чисел">
...
<h2>Консольный ввод/вывод в Java</h2><div class="date">Последнее обновление: 27.10.2018</div>
...
<p>Наиболее простой способ взаимодействия с пользователем представляет консоль: мы можем выводить на консоль некоторую информацию или, 
наоборот, считывать с консоли некоторые данные. Для взаимодействия с консолью в Java применяется класс <span class="b">System</span>, 
а его функциональность собственно обеспечивает консольный ввод и вывод.</p>
<h3>Вывод на консоль</h3>
<p>Для создания потока вывода в класс System определен объект <span class="b">out</span>. В этом объекте определен метод <span class="b">println</span>, 
который позволяет вывести на консоль некоторое значение с последующим переводом курсора консоли на следующую строку. Например:</p>
...
<p>В метод <code>println</code> передается любое значение, как правило, строка, которое надо вывести на консоль. И в данном случае мы получим 
следующий вывод:</p>
<div class="console">
...
<p>При необходимости можно и не переводить курсор на следующую строку. В этом случае можно использовать метод 
<span class="b">System.out.print()</span>, который аналогичен println за тем исключением, 
что не осуществляет перевода на следующую строку.</p>
<pre class="brush:java;">
public class Program {
...
<p>Но с помощью метода System.out.print также можно осуществить перевод каретки на следующую строку. Для этого надо использовать escape-последовательность 
<i>\n</i>:</p>
<pre class="brush:java;">
System.out.print("Hello world \n");
</pre>
<p>Нередко необходимо подставлять в строку какие-нибудь данные. Например, у нас есть два числа, и мы хотим вывести их 
значения на экран. В этом случае мы можем, например, написать так:</p>
<pre class="brush:java;">
public class Program {
...
<p>Но в Java есть также функция для форматированного вывода, унаследованная от языка С: <code>System.out.printf()</code>. С ее помощью мы можем переписать 
...
<p>В данном случае символы <span class="b">%d</span> обозначают спецификатор, вместо которого подставляет один из аргументов. Спецификаторов 
и соответствующих им аргументов может быть множество. В данном случае у нас только два аргумента, поэтому вместо первого <code>%d</code> 
подставляет значение переменной x, а вместо второго - значение переменной y. Сама буква <span class="b">d</span> означает, что данный спецификатор 
будет использоваться для вывода целочисленных значений.</p>
<p>Кроме спецификатора %d мы можем использовать еще ряд спецификаторов для других типов данных:</p>
<ul><li><p><span class="b">%x</span>: для вывода шестнадцатеричных чисел</p></li>
<li><p><span class="b">%f</span>: для вывода чисел с плавающей точкой</p></li>
<li><p><span class="b">%e</span>: для вывода чисел в экспоненциальной форме, например, <code>1.3e+01</code></p></li>
<li><p><span class="b">%c</span>: для вывода одиночного символа</p></li>
<li><p><span class="b">%s</span>: для вывода строковых значений</p></li>
</ul>
<p>Например:</p>
<pre class="brush:java;">
public class Program {
...
<p>При выводе чисел с плавающей точкой мы можем указать количество знаков после запятой, для этого используем спецификатор на <code>%.2f</code>, 
где <code>.2</code> указывает, что после запятой будет два знака. В итоге мы получим следующий вывод:</p>
<div class="console">
<pre class="consoletext">
...
<p>Для получения ввода с консоли в классе System определен объект <span class="b">in</span>. Однако непосредственно через объект 
System.in не очень удобно работать, поэтому, как правило, используют класс <span class="b">Scanner</span>, который, в 
свою очередь использует <code>System.in</code>. Например, напишем маленькую программу, которая осуществляет ввод чисел:</p>
<pre class="brush:java;">
...
<p>Так как класс <code>Scanner</code> находится в пакете <span class="b">java.util</span>, то мы вначале его импортируем с помощью инструкции <code>import java.util.Scanner</code>.</p> 
<p>Для создания самого объекта Scanner в его конструктор передается объект <code>System.in</code>. После этого мы можем получать вводимые значения. 
Например, в данном случае вначале выводим приглашение к вводу и затем получаем вводимое число в переменную num.</p>
<p>Чтобы получить введенное число, используется метод <span class="b">in.nextInt();</span>, который возвращает 
введенное с клавиатуры целочисленное значение.</p>
...
<p>Класс Scanner имеет еще ряд методов, которые позволяют получить введенные пользователем значения:</p>
<ul>
<li><p><span class="b">next()</span>: считывает введенную строку до первого пробела</p></li>
<li><p><span class="b">nextLine()</span>: считывает всю введенную строку</p></li>
<li><p><span class="b">nextInt()</span>: считывает введенное число int</p></li>
<li><p><span class="b">nextDouble()</span>: считывает введенное число double</p></li>
<li><p><span class="b">nextBoolean()</span>: считывает значение boolean</p></li>
<li><p><span class="b">nextByte()</span>: считывает введенное число byte</p></li>
<li><p><span class="b">nextFloat()</span>: считывает введенное число float</p></li>
<li><p><span class="b">nextShort()</span>: считывает введенное число short</p></li>
</ul>
<p>То есть для ввода значений каждого примитивного типа в классе Scanner определен свой метод.</p>
...
<p>Здесь последовательно вводятся данные типов String, int, float и потом все введенные данные вместе выводятся на консоль. Пример работы программы:</p>
<div class="console">
<pre class="consoletext">
Input name: Tom
Input age: 34
...
<p>Обратите внимание, что для ввода значения типа float (то же самое относится к типу double) применяется число "1,7", где разделителем является 
...
<div class="nav"><p><a href="2.12.php">Назад</a><a href="./">Содержание</a><a href="2.3.php">Вперед</a></p></div></div>
<div class="socBlock">
