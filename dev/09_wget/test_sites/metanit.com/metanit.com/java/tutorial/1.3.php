...
<html >
<head>
<title>Java | Первая программа в NetBeans</title>
<meta charset="utf-8" />
<meta name="description" content="Начало работы с IDE NetBeans. Создание первой программы на Java">
...
<h2>Первая программа в NetBeans</h2><div class="date">Последнее обновление: 28.09.2020</div>
...
<p>Еще одной популярной средой программирования под Java является Netbeans. Проект NetBeans во многом поддерживается и спонсируется 
компанией Oracle, при этом его разработка ведется независимым сообществом разработчиков (Apache NetBeans Community). 
NetBeans является кроссплатформенным программным обеспечением, поэтому есть версии среды для различных платформ и ОС. Более подробную информацию можно найти на официальном сайте 
даной среды <a href="https://netbeans.apache.org/" rel="nofollow">https://netbeans.apache.org/</a>.</p>
<p>На данный момент последней версией является 12-я версия. Итак, установим последнюю версию NetBeans, загрузив со страницы <a href="https://netbeans.apache.org/download/nb121/nb121.html" rel="nofollow">https://netbeans.apache.org/download/nb121/nb121.html</a>.</p> 
<img src="./pics/netbeans1.png" alt="Загрузка Apache Netbeans" />
<p>На странице загрузки мы можем найти два варианта: скомпилированные бинарные файлы в виде архива (пункт Binaries) и установщики для 
каждой конкретной операционной системы (пункт Installers). Загрузим среду в скомпилированном, выбрав пункт <span class="b">Binaries</span>.</p>
<p>После выбора этой опции нас переадресует на страницу с непосредственной ссылкой на зеркало для загрузки:</p>
<img src="./pics/netbeans3.png" alt="Загрузка Apache Netbeans" />
<p>После загрузки распакуем архив в папку, где мы хотим разместить среду Netbeans. Стоит отметить, что после распаковки архива мы можем уже сразу запустить среду, не надо ничего 
устанавливать. В архиве уже готовые для использования файлы. Для запуска в распакованном архиве в папке <span class="b">bin</span> мы 
можем найти собственно запускаемые файлы. В случае с ОС Windows это netbeans.exe и netbeans64.exe</p>
<img src="./pics/netbeans2.png" alt="Запуск Netbeans" />
<p>Запустим netbeans.exe или netbeans64.exe:</p>
<img src="./pics/1.3.png" alt="NetBeans" />
<p>NetBeans довольно интеллектуально понятна, и с ней очень легко работать. Создадим новый проект. Для этого выберем в меню пункт 
<span class="b">File-&gt;New Project...</span>. После этого перед нами откроется окно создания нового проекта:</p>
<img src="./pics/1.4.png" alt="Создание нового проекта в NetBeans" />
<p>В окне создания нового проекта выберем в левой колонке первый пункт - <span class="b">Java with Ant</span>, а в правой в качестве типа проекта - 
<span class="b">Java Application</span>. И нажмем кнопку <span class="b">Next &gt;</span></p>
<p>При первом создании проекта для Java нам будет предложено активировать соответствующую функцию:</p>
<img src="./pics/1.23.png" alt="Создание нового проекта в NetBeans" />
<p>И на данном этапе нажмем на кноку <span class="b">Activate</span>.</p>
<p>Затем откроется окно настроек проекта:</p>
<img src="./pics/1.5.png" alt="Настройка проекта в NetBeans" />
<p>В поле <span class="b">Project Name</span> дадим проекту какое-нибудь название (в моем случае это HelloApp). Для всех остальных полей можно оставить значения по умолчанию. 
...
в одноименном пакете helloapp. И в завершении создания проекта нажмем на кнопку "Finish". И перед нами откроется новый проект в Netbeans:</p>
<img src="./pics/1.6.png" alt="Новый проект в NetBeans" />
<p>Узкое окно слева отображает все открытые проекты и их структуру. В данном случае у нас открыт пока только один проект. А большое окно справа представляет 
...
<p>Здесь, во-первых, много комментариев, которые нам не нужны и которые мы можем удалить. Во-вторых, в начале файла указывается, что класс будет находиться в пакете 
helloapp с помощью директивы package: <code>package helloapp;</code>. Затем, как уже рассматривалось в прошлой теме, у нас есть класс программы - класс 
HelloApp, который имеет метод main.</p>
<p>Изменим код по умолчанию на следующий, чтобы программа выводила простейшую строку на консоль:</p>
<pre class="brush:java;">
package helloapp;
...
<p>Теперь запустим проект на выполнение. Для этого на панели инструментов NetBeans нажмем на зеленую стрелочку (либо можно нажать на клавишу F6, либо выбрать в меню 
пункт Run -&gt; Run Project). И внизу NetBeans откроется окно вывода, в котором мы сможем наблюдать результаты нашей программы:</p>
<img src="./pics/1.9.png" alt="Java в NetBeans 12" />
<p>Фактически это окно вывода эквивалентно консоли за некоторыми исключениями.</p>
...
<div class="nav"><p><a href="1.5.php">Назад</a><a href="./">Содержание</a><a href="1.4.php">Вперед</a></p></div></div>
<div class="socBlock">
