...
<html >
<head>
<title>Java | Создание потока данных</title>
<meta charset="utf-8" />
<meta name="description" content="Создание потока данных в Stream API в Java, методы stream и parallelStream интерфейса Collection">
...
<h2>Создание потока данных</h2><div class="date">Последнее обновление: 02.05.2018</div>
...
<p>Для создания потока данных можно применять различные методы. В качестве источника потока мы можем использовать коллекции. В частности, в JDK 8 в интерфейс 
<span class="b">Collection</span>, который реализуется всеми классами коллекций, были добавлены два метода для работы с потоками:</p>
<ul>
<li><p><code>default Stream&lt;E&gt; stream</code>: возвращается поток данных из коллекции</p></li>
<li><p><code>default Stream&lt;E&gt; parallelStream</code>: возвращается параллельный поток данных из коллекции</p></li>
</ul>
<p>Так, рассмотрим пример с ArrayList:</p>
...
<p>Здесь с помощью вызова <code>cities.stream()</code> получаем поток, который использует данные из списка cities. С помощью каждой промежуточной операции, 
которая применяется к потоку, мы также можем получить поток с учетом модификаций. Например, мы можем изменить предыдущий пример следующим образом:</p>
<pre class="brush:java;">
ArrayList&lt;String&gt; cities = new ArrayList&lt;String&gt;();
Collections.addAll(cities, "Париж", "Лондон", "Мадрид");
...
<p>Важно, что после использования терминальных операций другие терминальные или промежуточные операции к этому же потоку не могут быть применены, поток уже употреблен. 
...
<ol>
<li><p>Создание потока</p></li>
<li><p>Применение к потоку ряда промежуточных операций</p></li>
<li><p>Применение к потоку терминальной операции и получение результата</p></li>
</ol>
<p>Кроме вышерассмотренных методов мы можем использовать еще ряд способов для создания потока данных. Один из таких способов представляет метод 
<span class="b">Arrays.stream(T[] array)</span>, который создает поток данных из массива:</p>
<pre class="brush:java;">
...
<p>Для создания потоков IntStream, DoubleStream, LongStream можно использовать соответствующие перегруженные версии этого метода:</p>
<pre class="brush:java;">
...
<p>И еще один способ создания потока представляет статический метод <span class="b">of(T..values)</span> класса Stream:</p>
<pre class="brush:java;">
