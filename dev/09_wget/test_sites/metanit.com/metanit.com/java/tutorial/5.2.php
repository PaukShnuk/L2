...
<html >
<head>
<title>Java | Класс ArrayList и интерфейс List</title>
<meta charset="utf-8" />
<meta name="description" content="Интерфейс List в языке программирования Java, создание списков, класс ArrayList и его методы">
...
<h2>Класс ArrayList и интерфейс List</h2><div class="date">Последнее обновление: 24.04.2018</div>
...
<p>Для создания простых списков применяется интерфейс <span class="b">List</span>, который расширяет функцональность интерфейса Collection. 
Некоторые наиболее часто используемые методы интерфейса List:</p>
<ul>
<li><p><span class="b">void add(int index, E obj)</span>: добавляет в список по индексу index объект obj</p></li>
<li><p><span class="b">boolean addAll(int index, Collection&lt;? extends E&gt; col)</span>: добавляет в список по индексу index все элементы коллекции col. 
Если в результате добавления список был изменен, то возвращается true, иначе возвращается false</p></li>
<li><p><span class="b">E get(int index)</span>: возвращает объект из списка по индексу index</p></li>
<li><p><span class="b">int indexOf(Object obj)</span>: возвращает индекс первого вхождения объекта obj в список. Если объект не найден, то возвращается -1</p></li>
<li><p><span class="b">int lastIndexOf(Object obj)</span>: возвращает индекс последнего вхождения объекта obj в список. Если объект не найден, то возвращается -1</p></li>
<li><p><span class="b">ListIterator&lt;E&gt; listIterator ()</span>: возвращает объект ListIterator для обхода элементов списка</p></li>
<li><p><span class="b">static &lt;E&gt; List&lt;E&gt; of(элементы)</span>: создает из набора элементов объект List</p></li>
<li><p><span class="b">E remove(int index)</span>: удаляет объект из списка по индексу index, возвращая при этом удаленный объект</p></li>
<li><p><span class="b">E set(int index, E obj)</span>: присваивает значение объекта obj элементу, который находится по индексу index</p></li>
<li><p><span class="b">void sort(Comparator&lt;? super E&gt; comp)</span>: сортирует список с помощью компаратора comp</p></li>
<li><p><span class="b">List&lt;E&gt; subList(int start, int end)</span>: получает набор элементов, которые находятся в списке между индексами start и end</p></li>
</ul>
<p>По умолчанию в Java есть встроенная реализация этого интерфейса - класс <span class="b">ArrayList</span>. Класс ArrayList представляет 
обобщенную коллекцию, которая наследует свою функциональность от класса AbstractList и применяет интерфейс List. 
Проще говоря, ArrayList представляет простой список, аналогичный массиву, за тем исключением, что количество элементов в нем не фиксировано.</p>
<p>ArrayList имеет следующие конструкторы:</p>
<ul>
<li><p><code>ArrayList()</code>: создает пустой список</p></li>
<li><p><code>ArrayList(Collection &lt;? extends E&gt; col)</code>: создает список, в который добавляются все элементы коллекции col.</p></li>
<li><p><code>ArrayList (int capacity)</code>: создает список, который имеет начальную емкость capacity</p></li>
</ul>
<p>Емкость в ArrayList представляет размер массива, который будет использоваться для хранения объектов. При добавлении элементов фактически происходит 
перераспределение памяти - создание нового массива и копирование в него элементов из старого массива. Изначальное задание емкости ArrayList позволяет снизить подобные перераспределения 
памяти, тем самым повышая производительность.</p>
<p>Используем класс ArrayList и некоторые его методы в программе:</p>
...
<p>Здесь объект ArrayList типизируется классом String, поэтому список будет хранить только строки. Поскольку класс ArrayList применяет интерфейс 
Collection&lt;E&gt;, то мы можем использовать методы данного интерфейса для управления объектами в списке.</p> 
<p>Для добавления вызывается метод <code>add</code>. 
С его помощью мы можем добавлять объект в конец списка: <code>people.add("Tom")</code>. Также мы можем добавить объект на определенное место в 
списке, например, добавим объект на второе место (то есть по индексу 1, так как нумерация начинается с нуля): <code>people.add(1, "Bob")</code></p>
<p>Метод <code>size()</code> позволяет узнать количество объектов в коллекции.</p>
<p>Проверку на наличие элемента в коллекции производится с помощью метода <code>contains</code>. А удаление с помощью метода <code>remove</code>. 
И так же, как и с добавлением, мы можем удалить либо конкретный элемент <code>people.remove("Tom");</code>, либо элемент по индексу 
<code>people.remove(0);</code> - удаление первого элемента.</p>
<p>Получить определенный элемент по индексу мы можем с помощью метода <code>get()</code>: <code>String person = people.get(1);</code>, 
а установить элемент по индексу с помощью метода <code>set</code>: <code>people.set(1, "Robert");</code></p>
<p>С помощью метода <code>toArray()</code> мы можем преобразовать список в массив объектов.</p>
<p>И поскольку класс ArrayList реализует интерфейс Iterable, то мы можем пробежаться по списку в цикле аля for-each: <code>for(String person : people)</code>.</p>
<p>Хотя мы можем свободно добавлять в объект ArrayList дополнительные объекты, в отличие от массива, однако в реальности ArrayList использует для 
...
установить это количество, либо в конструкторе: <code>ArrayList&lt;String&gt; people = new ArrayList&lt;String&gt;(25);</code>, 
либо с помощью метода <code>ensureCapacity</code>: <code>people.ensureCapacity(25);</code></p>
...
<div class="nav"><p><a href="5.1.php">Назад</a><a href="./">Содержание</a><a href="5.7.php">Вперед</a></p></div></div>
<div class="socBlock">
