...
<html >
<head>
<title>Классы DataOutputStream и DataInputStream | Java</title>
<meta charset="utf-8" />
<meta name="description" content="Запись в файл данных примитивных типов в языке программирования Java, классы DataInputStream и DataOutputStream">
...
<h2>Классы DataOutputStream и DataInputStream</h2><div class="date">Последнее обновление: 26.04.2018</div>
...
<p>Классы DataOutputStream и DataInputStream позволяют записывать и считывать данные примитивных типов.</p>
<h3>Запись данных и DataOutputStream</h3>
<p>Класс DataOutputStream представляет поток вывода и предназначен для записи данных примитивных типов, таких, как int, double и т.д. Для записи каждого из 
примитивных типов предназначен свой метод:</p>
<ul>
<li><p><span class="bb">writeBoolean(boolean v) </span>: записывает в поток булевое однобайтовое значение</p></li>
<li><p><span class="bb">writeByte(int v)</span>: записывает в поток 1 байт, которые представлен в виде целочисленного значения</p></li>
<li><p><span class="bb">writeChar(int v)</span>: записывает 2-байтовое значение char</p></li>
<li><p><span class="bb">writeDouble(double v)</span>: записывает в поток 8-байтовое значение double</p></li>
<li><p><span class="bb">writeFloat(float v)</span>: записывает в поток 4-байтовое значение float</p></li>
<li><p><span class="bb">writeInt(int v)</span>: записывает в поток целочисленное значение int</p></li>
<li><p><span class="bb">writeLong(long v)</span>: записывает в поток значение long</p></li>
<li><p><span class="bb">writeShort(int v)</span>: записывает в поток значение short</p></li>
<li><p><span class="bb">writeUTF(String str)</span>: записывает в поток строку в кодировке UTF-8</p></li>
</ul>
<h3>Считывание данных и DataInputStream</h3>
<p>Класс DataInputStream действует противоположным образом - он считывает из потока данные примитивных типов. Соответственно для каждого примитивного типа определен 
свой метод для считывания:</p>
<ul>
<li><p><span class="bb">boolean readBoolean()</span>: считывает из потока булевое однобайтовое значение</p></li>
<li><p><span class="bb">byte readByte()</span>: считывает из потока 1 байт</p></li>
<li><p><span class="bb">char readChar()</span>: считывает из потока значение char</p></li>
<li><p><span class="bb">double readDouble()</span>: считывает из потока 8-байтовое значение double</p></li>
<li><p><span class="bb">float readFloat()</span>: считывает из потока 4-байтовое значение float</p></li>
<li><p><span class="bb">int readInt()</span>: считывает из потока целочисленное значение int</p></li>
<li><p><span class="bb">long readLong()</span>: считывает из потока значение long</p></li>
<li><p><span class="bb">short readShort()</span>: считывает значение short</p></li>
<li><p><span class="bb">String readUTF()</span>: считывает из потока строку в кодировке UTF-8</p></li>
<li><p><span class="bb">int skipBytes(int n)</span>: пропускает при чтении из потока n байтов</p></li>
</ul>
...
<p>Здесь мы последовательно записываем в файл данные объекта Person.</p>
<p>Объект DataOutputStream в конструкторе принимает поток вывода: <code>DataOutputStream (OutputStream out)</code>. В данном случае в качестве потока вывода 
используется объект <code>FileOutputStream</code>, поэтому вывод будет происходить в файл. И с помощью выше рассмотренных методов типа <code>writeUTF()</code> 
производится запись значений в бинарный файл.</p>
<p>Затем происходит чтение ранее записанных данных. Объект DataInputStream в конструкторе принимает поток для чтения: <code>DataInputStream(InputStream in)</code>. 
Здесь таким потоком выступает объект <code>FileInputStream</code></p>
...
<div class="nav"><p><a href="6.6.php">Назад</a><a href="./">Содержание</a><a href="6.8.php">Вперед</a></p></div></div>
<div class="socBlock">
