...
<html >
<head>
<title>Java | Перечисления enum</title>
<meta charset="utf-8" />
<meta name="description" content="Перечисления enum в Java, их объявление, конструкторы, методы, поля и примеры использования">
...
<h2>Перечисления enum</h2><div class="date">Последнее обновление: 22.04.2018</div>
...
<p>Кроме отдельных примитивных типов данных и классов в Java есть такой тип как <span class="b">enum</span> или перечисление. 
Перечисления представляют набор логически связанных констант. Объявление перечисления происходит с помощью оператора enum, после которого идет 
название перечисления. Затем идет список элементов перечисления через запятую:</p>
...
<p>Само перечисление объявлено вне класса, оно содержит четыре жанра книг. Класс Book кроме обычных переменных содержит также переменную типа нашего перечисления. 
В конструкторе мы ее также можем присвоить, как и обычные поля класса.</p>
<p>С помощью конструкции switch..case можно проверить принадлежность значения bookType определенной константе перечисления.</p>
<h3>Методы перечислений</h3>
<p>Каждое перечисление имеет статический метод <span class="b">values()</span>. Он возвращает массив всех констант перечисления:</p>
<pre class="brush:java;">
public class Program{
...
<p>Метод <span class="b">ordinal()</span> возвращает порядковый номер определенной константы (нумерация начинается с 0):</p>
<pre class="brush:java;">
System.out.println(Type.BELLETRE.ordinal());	// 1
</pre>
<h3>Конструкторы, поля и методы перечисления</h3>
...
по умолчанию приватный, то есть имеет модификатор private. Любой другой модификатор будет считаться ошибкой. Поэтому создать константы перечисления с помощью 
конструктора мы можем только внутри перечисления.</p>
...
<div class="nav"><p><a href="3.16.php">Назад</a><a href="./">Содержание</a><a href="3.9.php">Вперед</a></p></div></div>
<div class="socBlock">
