...
<html >
<head>
<title>Java | Сортировка в Stream API</title>
<meta charset="utf-8" />
<meta name="description" content="Сортировка данных в Stream API в языке программирования Java, метод sorted">
...
<h2>Сортировка</h2><div class="date">Последнее обновление: 30.04.2018</div>
...
<p>Коллекции, на основе которых нередко создаются потоки, уже имеют специальные методы для сортировки содержимого. Однако класс Stream также включает возможность сортировки. 
Такую сортировку мы можем задействовать, когда у нас идет набор промежуточных операций с потоком, которые создают новые наборы данных, и нам надо эти наборы отсортировать.</p>
<p>Для простой сортировки по возрастанию применяется метод <span class="b">sorted()</span>:</p>
...
<p>Однако данный метод не всегда подходит. Уже по консольному выводу мы видим, что метод сортирует объекты по возрастанию, но при этом 
заглавные и строчные буквы рассматриваются отдельно.</p>
<p>Кроме того, данный метод подходит только для сортировки тех объектов, которые реализуют интерфейс <span class="b">Comparable</span>.</p>
<p>Если же у нас классы объектов не реализуют этот интерфейс или мы хотим создать какую-то свою логику сортировки, то мы можем использовать другую версию метода 
<code>sorted()</code>, которая в качестве параметра принимает компаратор.</p>
<p>Например, пусть у нас есть следующий класс Phone:</p>
<pre class="brush:java;">
class Phone{
...
<p>Отсортируем поток обектов Phone:</p>
<pre class="brush:java;">
...
<p>Здесь определен класс компаратора PhoneComparator, который сортирует объекты по полю name. В итоге мы получим следующий вывод:</p>
<div class="console">
<pre class="consoletext">
...
<div class="nav"><p><a href="10.3.php">Назад</a><a href="./">Содержание</a><a href="10.13.php">Вперед</a></p></div></div>
<div class="socBlock">
