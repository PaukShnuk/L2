...
<html >
<head>
<title>Java | Объекты как параметры методов</title>
<meta charset="utf-8" />
<meta name="description" content="Объекты как параметры методов в языке программирования Java, передача данных в методы по значению и по ссылке ">
...
<h2>Объекты как параметры методов</h2><div class="date">Последнее обновление: 19.04.2018</div>
...
<p>Объекты классов, как и данные примитивных типов, могут передаваться в методы. Однако в данном случае есть одна особенность - при передаче 
...
Пусть у нас есть следующий класс Person:</p>
...
<p>Здесь в метод changeName передается объект Person, у которого изменяется имя. Так как в метод будет передаваться копия ссылки на область памяти, 
...
после выполнения метода у объекта kate, который передается в метод, будет изменено имя с "Kate" на "Alice".</p>
...
<p>В метод changePerson также передается копия ссылки на объект Person. Однако в самом методе мы изменяем не отдельные значения объекта, а пересоздаем объект 
с помощью конструктора и оператора new. В результате в памяти будет выделено новое место для нового объекта Person, и ссылка на этот объект будет привоена параметру p:</p>
<pre class="brush:java;">
static void changePerson(Person p){
	p = new Person("Alice");	// p указывает на новый объект
...
<p>То есть после создания нового объекта Person параметр p и переменная kate в методе main будут хранить ссылки на разные объекты. 
Переменная kate, которая передавалась в метод, продолжит хранить ссылку на старый объект в памяти. Поэтому ее значение не меняется.</p>
...
<div class="nav"><p><a href="3.4.php">Назад</a><a href="./">Содержание</a><a href="3.12.php">Вперед</a></p></div></div>
<div class="socBlock">
