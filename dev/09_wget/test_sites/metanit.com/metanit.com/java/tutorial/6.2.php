...
<html >
<head>
<title>Java | Закрытие потоков</title>
<meta charset="utf-8" />
<meta name="description" content="Закрытие файловых потоков в языке программирования Java, конструкция try..with resources">
...
<h2>Закрытие потоков</h2><div class="date">Последнее обновление: 25.04.2018</div>
...
<p>При завершении работы с потоком его надо закрыть с помощью метода <span class="b">close()</span>, который определен в интерфейсе 
<span class="b">Closeable</span>. Метод close имеет следующее определение:</p>
<pre class="brush:java;">
void close() throws IOException
</pre>
<p>Этот интерфейс уже реализуется в классах InputStream и OutputStream, а через них и во всех классах потоков.</p>
...
<p>Есть два способа закрытия файла. Первый традиционный заключается в использовании блока <code>try..catch..finally</code>. Например, считаем данные из файла:</p>
<pre class="brush:java;">
import java.io.*;
...
<p>Поскольку при открытии или считывании файла может произойти ошибка ввода-вывода, то код считывания помещается в блок try. И чтобы быть уверенным, что 
поток в любом случае закроется, даже если при работе с ним возникнет ошибка, вызов метода <code>close()</code> помещается в блок <code>finally</code>. 
И, так как метод <code>close()</code> также в случае ошибки может генерировать исключение IOException, то его вызов также помещается во вложенный блок 
<code>try..catch</code></p>
<p>Начиная с Java 7 можно использовать еще один способ, который автоматически вызывает метод close. Этот способ заключается в использовании конструкции 
<span class="b">try-with-resources</span> (try-с-ресурсами). Данная конструкция работает с объектами, которые реализуют интерфейс <code>AutoCloseable</code>. 
Так как все классы потоков реализуют интерфейс <code>Closeable</code>, который в свою очередь наследуется от <code>AutoCloseable</code>, то их также можно использовать в данной 
конструкции</p>
<p>Итак, перепишем предыдущий пример с использованием конструкции try-with-resources:</p>
<pre class="brush:java;">
import java.io.*;
...
<p>Синтаксис конструкции следующий: <code>try(название_класса имя_переменной = конструктор_класса)</code>. Данная конструкция также не исключает 
использования блоков <code>catch</code>.</p>
<p>После окончания работы в блоке try у ресурса (в данном случае у объекта <code>FileInputStream</code>) автоматически вызывается метод close().</p>
...
<div class="nav"><p><a href="6.3.php">Назад</a><a href="./">Содержание</a><a href="6.4.php">Вперед</a></p></div></div>
<div class="socBlock">
