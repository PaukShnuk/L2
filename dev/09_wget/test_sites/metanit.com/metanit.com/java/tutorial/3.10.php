...
<html >
<head>
<title>Java | Иерархия наследования и преобразование типов</title>
<meta charset="utf-8" />
<meta name="description" content="Преобразование типов в Java, от базового к производному и от производного к базовому, создание иерархии наследования из классов и интерфейсов, оператор instanceof">
...
<h2>Иерархия наследования и преобразование типов</h2><div class="date">Последнее обновление: 28.03.2021</div>
...
<p>В прошлой главе говорилось о преобразованиях объектов простых типов. Однако с объектами классов все происходит немного по-другому. 
Допустим, у нас есть следующая иерархия классов:</p>
...
<p>В этой иерархии классов можно проследить следующую цепь наследования: Object (все классы неявно наследуются от типа Object) -&gt; Person -&gt; Employee|Client.</p>
<img src="./pics/3.12.png" alt="Преобразование типов в языке Java" />
<p>Суперклассы обычно размещаются выше подклассов, поэтому на вершине наследования находится класс Object, а в самом низу Employee и Client.</p>
...
<p>Это так называемое восходящее преобразование (от подкласса внизу к суперклассу вверху иерархии) или upcasting. Такое преобразование осуществляется 
автоматически.</p>
<p>Обратное не всегда верно. Например, объект Person не всегда является объектом Employee или Client. Поэтому нисходящее преобразование или downcasting 
от суперкласса к подклассу автоматически не выполняется. В этом случае нам надо использовать операцию преобразования типов.</p>
<pre class="brush:java;">
Object sam = new Employee("Sam", "Oracle");
...
<p>В данном случае переменная sam приводится к типу Employee. И затем через объект emp мы можем обратиться к функционалу объекта Employee.</p>
<p>Мы можем преобразовать объект Employee по всей прямой линии наследования от Object к Employee.</p>
...
<p>В данном случае переменная типа Object хранит ссылку на объект Client. Мы можем без ошибок привести этот объект к типам Person или Client. Но при попытке преобразования 
к типу Employee мы получим ошибку во время выполнения. Так как kate не представляет объект типа Employee.</p>
<h3>Оператор instanceof</h3>
<p>В примере выше мы явно видим, что переменная kate - это ссылка на объект Client, а не Employee. Однако нередко данные приходят извне, и мы можем точно не знать, какой именно 
объект эти данные представляют. Соответственно возникает большая вероятность столкнуться с ошибкой. И перед тем, как провести преобразование типов, 
мы можем проверить, а можем ли мы выполнить приведение с помощью оператора <span class="b">instanceof</span>:</p>
<pre class="brush:java;">
Object kate = new Client("Kate", "DeutscheBank", 2000);
...
<p>Выражение <code>kate instanceof Employee</code> проверяет, является ли переменная kate объектом типа Employee. Но так как в данном случае явно 
не является, то такая проверка вернет значение <code>false</code>, и преобразование не сработает.</p>
<p>А выражение <code>kate instanceof Client</code> возвратило бы <code>true</code>:</p>
<pre class="brush:java;">
Object kate = new Client("Kate", "DeutscheBank", 2000);
...
<p>Следует отметить, что начиная с версии Java 16 мы можем упростить преобразование типов следующим образом:</p>
<pre class="brush:java;">
Object kate = new Client("Kate", "DeutscheBank", 2000);
...
<p>Выражение</p>
<pre class="brush:java;">kate instanceof Client clientKate</pre>
<p>Проверяет, представляет ли переменная <code>kate</code> класс <code>Client</code>, и если представляет (то есть оператор <code>instanceof</code> 
возвращает <code>true</code>), то создает переменную <code>clientKate</code> типа <code>Client</code>. И в дальнейшем мы можем использовать эту переменную clientKate и производить 
с ней различные операции.</p>
...
<div class="nav"><p><a href="3.6.php">Назад</a><a href="./">Содержание</a><a href="3.7.php">Вперед</a></p></div></div>
<div class="socBlock">
