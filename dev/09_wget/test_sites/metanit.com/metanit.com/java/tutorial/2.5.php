...
<html >
<head>
<title>Java | Условные конструкции</title>
<meta charset="utf-8" />
<meta name="description" content="Условные конструкции в языке Java, выражение if..else, использование конструкции switch..case, тернарный оператор">
...
<h2>Условные конструкции</h2><div class="date">Последнее обновление: 17.04.2018</div>
...
<p>Одним из фундаментальных элементов многих языков программирования являются <span class="bb">условные конструкции</span>. Данные конструкции 
позволяют направить работу программы по одному из путей в зависимости от определенных условий.</p>
<p>В языке Java используются следующие условные конструкции: <code>if..else</code> и <code>switch..case</code></p>
...
<p>После ключевого слова <code>if</code> ставится условие. И если это условие выполняется, то срабатывает код, который помещен в далее в 
блоке if  после фигурных скобок. В качестве условий выступает операция сравнения двух чисел.</p>
<p>Так как, в данном случае первое число больше второго, то выражение <code>num1 &gt; num2</code> истинно и возвращает значение 
<code>true</code>. Следовательно, управление переходит в блок кода после фигурных скобок и начинает выполнять содержащиеся там инструкции, а 
конкретно метод <code>System.out.println("Первое число больше второго");</code>. Если бы первое число оказалось бы меньше второго или равно ему, то инструкции в блоке if не выполнялись бы.</p>
...
<p>Но при сравнении чисел мы можем насчитать три состояния: первое число больше второго, первое число меньше второго и числа равны. 
С помощью выражения <code>else if</code>, мы можем обрабатывать дополнительные условия:</p>
<pre class="brush:java;">
int num1 = 6;
int num2 = 8;
if(num1&gt;num2){
...
<p>Здесь блок if будет выполняться, если <code>num1 &gt; num2</code> равно <code>true</code> и одновременно <code>num1&gt;7</code> равно <code>true</code>.</p>
<h3>Конструкция switch</h3>
<p>Конструкция <span class="b">switch/case</span> аналогична конструкции <code>if/else</code>, так как позволяет обработать сразу несколько условий:</p>
<pre class="brush:java;">
int num = 8;
switch(num){
...
<p>После ключевого слова <span class="b">switch</span> в скобках идет сравниваемое выражение. Значение этого выражения последовательно сравнивается со значениями, помещенными после операторов 
<span class="b">сase</span>. И если совпадение найдено, то будет выполняет соответствующий блок <span class="b">сase</span>.</p>
<p>В конце блока сase ставится оператор <span class="b">break</span>, чтобы избежать выполнения других блоков. Например, если бы убрали оператор <code>break</code> в следующем случае:</p>
<pre class="brush:java;">
case 8: 
    System.out.println("число равно 8");
    num++;
...
<p>то выполнился бы блок <code>case 8</code>, (поскольку переменная num равна 8). Но так как в этом блоке оператор break отсутствует, то начал бы выполняться блок <code>case 9</code>.</p>
<p>Если мы хотим также обработать ситуацию, когда совпадения не будет найдено, то можно добавить блок <span class="bb">default</span>, 
как в примере выше. Хотя блок <span class="b">default</span> необязателен.</p>
<p>Также мы можем определить одно действие сразу для нескольких блоков case подряд:</p>
<pre class="brush:java;">
int num = 3;
int output = 0;
switch(num){
...
<p>Тернарную операция имеет следующий синтаксис: <code>[первый операнд - условие] ? [второй операнд] : [третий операнд]</code>. 
Таким образом, в этой операции участвуют сразу три операнда. 
В зависимости от условия тернарная операция возвращает второй или третий операнд: если условие равно <code>true</code>, то 
возвращается второй операнд; если условие равно <code>false</code>, то третий. Например:</p>
<pre class="brush:java;">
int x=3;
int y=2;
int z = x&lt;y? (x+y) : (x-y);
...
<p>Здесь результатом тернарной операции является переменная z. Сначала проверяется условие <code>x&lt;y</code>. 
И если оно соблюдается, то z будет равно второму операнду - (x+y), иначе z будет равно третьему операнду.</p>
...
<li><a title="Упражнения" href="../practice/1.1.php"><i class="fa fa-lg fa-pencil"></i><span class="dop-txt">Упражнения</span></a></li>
</ul>
</fieldset></div>
