...
<html >
<head>
<title>Java | Параллельные операции над массивами</title>
<meta charset="utf-8" />
<meta name="description" content="Параллельные операции над массивами в языке программирования Java">
...
<h2>Параллельные операции над массивами</h2><div class="date">Последнее обновление: 30.04.2018</div>
...
<p>В JDK 8 к классу <span class="b">Arrays</span> было добавлено ряд методов, которые позволяют в параллельном режиме совершать обработку элементов массива. 
И хотя данные методы формально не входят в Stream API, но реализуют схожую функциональность, что и параллельные потоки:</p>
<ul>
<li><p><span class="b">parallelPrefix()</span>: вычисляет некоторое значение для элементов массива (например, сумму элементов)</p></li>
<li><p><span class="b">parallelSetAll()</span>: устанавливает элементы массива с помощью лямбда-выражения</p></li>
<li><p><span class="b">parallelSort()</span>: сортирует массив</p></li>
</ul>
...
<p>В метод <code>Arrays.parallelSetAll</code> передается два параметра: изменяемый массив и функция, которая устанавливает элементы массива. Эта 
функция перебирает все элементы и в качестве параметра получает индекс текущего перебираемого элемента. Выражение <code>i -&gt; i*10</code> означает, 
что по каждому индексу в массиве будет хранится число, равное i * 10. В итоге мы получим следующий вывод:</p> 
<div class="console">
...
<p>Рассмотрим более сложный пример. Пусть у нас есть следующий класс Phone:</p>
<pre class="brush:java;">
class Phone{
...
<p>Теперь произведем манипуляции с массивом объектов Phone:</p>
<pre class="brush:java;">
...
оператор <span class="b">return</span>. В этом лямбда-выражении опять же функция получает индексы перебираемых элементов, и по этим индексам мы можем обратиться 
...
<p>Если же нам надо как-то по-другому отсортировать объекты, например, по модулю числа, или у нас более сложные объекты, то мы можем создать свой компаратор и передать его в качестве второго параметра в 
<code>Arrays.parallelSort()</code>. Например, возьмем выше определенный класс Phone и создадим для него компаратор:</p>
<pre class="brush:java;">
import java.util.Arrays;
...
<h3>Метод parallelPrefix</h3>
<p>Метод <code>parallelPrefix()</code> походит для тех случаев, когда надо получить элемент массива или объект того же типа, что и элементы массива, который обладает некоторыми признаками. 
Например, в массиве чисел это может быть максимальное, минимальное значения и т.д. Например, найдем произведение чисел:</p>
<pre class="brush:java;">
...
<p>То есть, как мы видим из консольного вывода, лямбда-выражение из <code>Arrays.parallelPrefix</code>, которое представляет бинарную функцию, получает два элемента и 
выполняет над ними операцию. Результат операции сохраняется и передается в следующий вызов бинарной функции.</p>
...
<div class="nav"><p><a href="10.9.php">Назад</a><a href="./">Содержание</a><a href="11.1.php">Вперед</a></p></div></div>
<div class="socBlock">
