...
<html >
<head>
<title>Java | Блокировки. ReentrantLock</title>
<meta charset="utf-8" />
<meta name="description" content="Блокировки. ReentrantLock в многопоточных программах на Java, пакет java.util.concurrent.locks">
...
<h2>Блокировки. ReentrantLock</h2><div class="date">Последнее обновление: 28.04.2018</div>
...
<p>Для управления доступом к общему ресурсу в качестве альтернативы оператору synchronized мы можем использовать блокировки. Функциональность 
блокировок заключена в пакете <span class="b">java.util.concurrent.locks</span>.</p>
<p>Вначале поток пытается получить доступ к общему ресурсу. Если он свободен, то на него накладывает блокировку. 
После завершения работы блокировка с общего ресурса снимается. Если же ресурс не свободен и на него уже наложена блокировка, то поток ожидает, пока эта блокировка не будет снята.</p>
<p>Классы блокировок реализуют интерфейс <span class="b">Lock</span>, который определяет следующие методы:</p>
<ul>
<li><p><span class="b">void lock()</span>: ожидает, пока не будет получена блокировка</p></li>
<li><p><span class="b">void lockInterruptibly() throws InterruptedException</span>: ожидает, пока не будет получена блокировка, если поток не прерван</p></li>
<li><p><span class="b">boolean tryLock()</span>: пытается получить блокировку, если блокировка получена, то возвращает true. Если блокировка не получена, 
то возвращает false. В отличие от метода <code>lock()</code> не ожидает получения блокировки, если она недоступна</p></li>
<li><p><span class="b">void unlock()</span>: снимает блокировку</p></li>
<li><p><span class="b">Condition newCondition()</span>: возвращает объект Condition, который связан с текущей блокировкой</p></li>
</ul>
<p>Организация блокировки в общем случае довольно проста: для получения блокировки вызывается метод <code>lock()</code>, а после окончания работы с общими ресурсами 
вызывается метод <code>unlock()</code>, который снимает блокировку.</p>
<p>Объект Condition позволяет управлять блокировкой.</p>
<p>Как правило, для работы с блокировками используется класс <span class="b">ReentrantLock</span> из пакета <span class="ii">java.util.concurrent.locks</span>. 
Данный класс реализует интерфейс Lock.</p>
<p>Для примера возьмем код из темы про оператор synchronized и перепишем данный код с использованием заглушки ReentrantLock:</p>
...
<p>Здесь также используется общий ресурс CommonResource, для управления которым создается пять потоков. На входе в критическую секцию устанавливается 
заглушка:</p>
<pre class="brush:java;">locker.lock();</pre>
<p>После этого только один поток имеет доступ к критической секции, а остальные потоки ожидают снятия блокировки. В блоке finally после всей 
окончания основной работы потока эта блокировка снимается. Причем делается это обязательно в блоке finally, так как в случае 
возникновения ошибки все остальные потоки окажутся заблокированными.</p>
<p>В итоге мы получим вывод, аналогичный тому, который был в случае с оператором synchronized:</p>
<div class="console">
