...
<html >
<head>
<title>Java | Параметры методов</title>
<meta charset="utf-8" />
<meta name="description" content="Параметры методов в языке программирования Java, неопределенное количество параметров">
...
<h2>Параметры методов</h2><div class="date">Последнее обновление: 18.04.2018</div>
...
<p>Поскольку метод sum принимает два значения типа int, то на место параметров надо передать два значения типа int. Это могут быть и числовые литералы, и переменные типов данных, которые представляют тип int или 
могут быть автоматически преобразованы в тип int. Значения, которые передаются на место параметров, еще называются <span class="b">аргументами</span>. 
Значения передаются параметрам по позиции, то есть первый аргумент первому параметру, второй аргумент - второму параметру и так далее.</p>
...
<p>Метод display принимает два параметра. Первый параметр представляет тип String, а второй - тип int. Поэтому при вызове метода вначале в него надо передать строку, а затем число.</p>
<h3>Параметры переменной длины</h3>
<p>Метод может принимать параметры переменной длины одного типа. Например, нам надо передать в метод набор числел и вычислить их сумму, но мы точно не знаем, сколько именно чисел будет передано - 3, 4, 5 или больше. 
Параметры переменной длины позволяют решить эту задачу:</p>
<pre class="brush:java;">
public class Program{
...
<p>Троеточие перед названием параметра <code>int ...nums</code> указывает на то, что он будет необязательным и будет представлять массив. Мы можем передать в метод sum 
одно число, несколько чисел, а можем вообще не передавать никаких параметров. Причем, если мы хотим передать несколько параметров, то необязательный параметр должен указываться в конце:</p>
<pre class="brush:java;">
public static void main(String[] args) {
...
<div class="nav"><p><a href="2.7.php">Назад</a><a href="./">Содержание</a><a href="2.17.php">Вперед</a></p></div></div>
<div class="socBlock">
