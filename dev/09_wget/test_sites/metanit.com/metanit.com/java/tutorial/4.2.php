...
<html >
<head>
<title>Java | Классы исключений</title>
<meta charset="utf-8" />
<meta name="description" content="Классы исключений в Java, класс Throwable, Exception, иерархия исключений, проверяемые и непроверяемые исключения">
...
<h2>Классы исключений</h2><div class="date">Последнее обновление: 23.04.2018</div>
...
<p>Базовым классом для всех исключений является класс <span class="b">Throwable</span>. От него уже наследуются два класса: <span class="b">Error</span> 
и <span class="b">Exception</span>. Все остальные классы являются производными от этих двух классов.</p>
<p>Класс Error описывает внутренние ошибки в исполняющей среде Java. Программист имеет очень ограниченные возможности для обработки подобных ошибок.</p>
<p>Собственно исключения наследуются от класса Exception. Среди этих исключений следует выделить класс <span class="b">RuntimeException</span>. 
RuntimeException является базовым классом для так называемой группы <span class="b">непроверяемых исключений</span> (unchecked exceptions) - 
...
Такие исключения являются следствием ошибок разработчика, например, неверное преобразование типов или выход за пределы массива.</p>
<p>Некоторые из классов непроверяемых исключений:</p>
<ul>
<li><p><span class="b">ArithmeticException</span>: исключение, возникающее при делении на ноль</p></li>
<li><p><span class="b">IndexOutOfBoundException</span>: индекс вне границ массива</p></li>
<li><p><span class="b">IllegalArgumentException</span>: использование неверного аргумента при вызове метода</p></li>
<li><p><span class="b">NullPointerException</span>: использование пустой ссылки</p></li>
<li><p><span class="b">NumberFormatException</span>: ошибка преобразования строки в число</p></li>
</ul>
<p>Все остальные классы, образованные от класса Exception, называются проверяемыми исключениями (checked exceptions).</p>
<p>Некоторые из классов проверяемых исключений:</p>
<ul>
<li><p><span class="b">CloneNotSupportedException</span>: класс, для объекта которого вызывается клонирование, не реализует интерфейс Cloneable</p></li>
<li><p><span class="b">InterruptedException</span>: поток прерван другим потоком</p></li>
<li><p><span class="b">ClassNotFoundException</span>: невозможно найти класс</p></li>
</ul>
<p>Подобные исключения обрабатываются с помощью конструкции try..catch. Либо можно передать обработку методу, который будет вызывать данный метод, 
указав исключения после оператора throws:</p>
...
<img src="./pics/4.1.png" alt="Иерархия классов исключений в Java" />
<p>Поскольку все классы исключений наследуются от класса Exception, то все они наследуют ряд его методов, которые позволяют получить информацию о 
характере исключения. Среди этих методов отметим наиболее важные:</p>
<ul>
<li><p>Метод <span class="b">getMessage()</span> возвращает сообщение об исключении</p></li>
<li><p>Метод <span class="b">getStackTrace()</span> возвращает массив, содержащий трассировку стека исключения</p></li>
<li><p>Метод <span class="b">printStackTrace()</span> отображает трассировку стека</p></li>
</ul>
<p>Например:</p>
<pre class="brush:java;">
try{
