...
<html >
<head>
<title>Введение в обработку исключений | Java</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка исключений в Java, использование конструкции try..catch, обработка нескольких типов исключений, оператор throw">
...
<h2>Введение в обработку исключений</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
Подобные ситуации называются <span class="bb">исключениями</span>.</p>
<p>В языке Java предусмотрены специальные средства для обработки подобных ситуаций. Одним из таких средств является конструкция <span class="bb">try...catch...finally</span>. 
...
программы останавливается. И чтобы подобной остановки не произошло, и надо использовать блок try..catch. Например:</p>
<pre class="brush:java;">
int[] numbers = new int[3];
numbers[4]=45;
System.out.println(numbers[4]);
</pre>
<p>Так как у нас массив numbers может содержать только 3 элемента, то при выполнении инструкции <code>numbers[4]=45</code> консоль отобразит 
исключение, и выполнение программы будет завершено. Теперь попробуем обработать это исключение:</p>
<pre class="brush:java;">
try{
    int[] numbers = new int[3];
	numbers[4]=45;
...
<p>При использовании блока <span class="bb">try...catch</span> вначале выполняются все инструкции между операторами try и catch. 
Если в блоке try вдруг возникает исключение, то обычный порядок выполнения останавливается и переходит к инструкции сatch. 
Поэтому когда выполнение программы дойдет до строки <code>numbers[4]=45;</code>, программа остановится и перейдет к блоку <code>catch</code></p>
<p>Выражение <span class="bb">catch</span> имеет следующий синтаксис: <code>catch (тип_исключения имя_переменной)</code>. 
В данном случае объявляется переменная <code>ex</code>, которая имеет тип <code>Exception</code>. Но если возникшее исключение не 
...
<p>Но так как тип <code>Exception</code> является базовым классом для всех исключений, то выражение <code>catch (Exception ex)</code> 
...
помощью метода <code>printStackTrace()</code>, определенного в классе Exception.</p>
<p>После завершения выполнения блока catch программа продолжает свою работу, выполняя все остальные инструкции после блока catch.</p>
<p>Конструкция try..catch также может иметь блок <code>finally</code>. Однако этот блок необязательный, и его можно при обработке исключений 
опускать. Блок <code>finally</code> выполняется в любом случае, возникло ли исключение в блоке try или нет:</p>
<pre class="brush:java;">
try{
    int[] numbers = new int[3];
	numbers[4]=45;
...
<p>В Java имеется множество различных типов исключений, и мы можем разграничить их обработку, включив дополнительные блоки catch:</p>
<pre class="brush:java;">
int[] numbers = new int[3];
try{
    numbers[6]=45;
...
<p>Если у нас возникает исключение определенного типа, то оно переходит к соответствующему блоку catch.</p>
<h3>Оператор throw</h3>
<p>Чтобы сообщить о выполнении исключительных ситуаций в программе, можно использовать оператор <span class="bb">throw</span>. То есть с помощью 
...
мы хотим, чтобы, если число больше 30, то возникало исключение:</p>
<pre class="brush:java;">
package firstapp;
...
<p>Здесь для создания объекта исключения используется конструктор класса Exception, в который передается сообщение об исключении. И если 
число х окажется больше 29, то будет выброшено исключение и управление перейдет к блоку catch.</p>
<p>В блоке catch мы можем получить сообщение об исключении с помощью метода <code>getMessage()</code>.</p>
