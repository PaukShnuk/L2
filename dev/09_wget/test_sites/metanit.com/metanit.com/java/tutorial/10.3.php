...
<html >
<head>
<title>Java | Фильтрация, перебор элементов и отображение</title>
<meta charset="utf-8" />
<meta name="description" content="Фильтрация, перебор элементов и отображение с помощью Stream API  в Java">
...
<h2>Фильтрация, перебор элементов и отображение</h2><div class="date">Последнее обновление: 02.05.2018</div>
...
<h3>Перебор элементов. Метод forEach</h3>
<p>Для перебора элементов потока применяется метод <code>forEach()</code>, который представляет терминальную операцию. В качестве параметра он принимает 
объект <code>Consumer&lt;? super String&gt;</code>, который представляет действие, выполняемое для каждого элемента набора. Например:</p>
<pre class="brush:java;">
Stream&lt;String&gt; citiesStream = Stream.of("Париж", "Лондон", "Мадрид","Берлин", "Брюссель");
citiesStream.forEach(s-&gt;System.out.println(s));
</pre>
<p>Фактически это будет аналогично перебору всех элементов в цикле for и выполнению с ними действия, а именно вывод на консоль. В итоге консоль выведет:</p>
<div class="console">
...
<p>Кстати мы можем сократить в данном случае применение метода forEach следующим образом:</p>
<pre class="brush:java;">
Stream&lt;String&gt; citiesStream = Stream.of("Париж", "Лондон", "Мадрид","Берлин", "Брюссель");
...
<h3>Фильтрация. Метод filter</h3>
<p>Для фильтрации элементов в потоке применяется метод <span class="b">filter()</span>, который представляет промежуточную операцию. Он принимает в 
качестве параметра некоторое условие в виде объекта <code>Predicate&lt;T&gt;</code> и возвращает новый поток из элементов, которые удовлетворяют этому условию:</p>
<pre class="brush:java;">
Stream&lt;String&gt; citiesStream = Stream.of("Париж", "Лондон", "Мадрид","Берлин", "Брюссель");
citiesStream.filter(s-&gt;s.length()==6).forEach(s-&gt;System.out.println(s));
</pre>
<p>Здесь условие <code>s.length()==6</code> возвращает true для тех элементов, длина которых равна 6 символам. То есть в итоге программа выведет:</p>
<div class="console">
<pre class="consoletext">
Лондон
Мадрид
...
<p>Рассмотрим еще один пример фильтрации с более сложными данными. Допустим, у нас есть следующий класс Phone:</p>
<pre class="brush:java;">
class Phone{
...
<h3>Отображение. Метод map</h3>
<p>Отображение или маппинг позволяет задать функцию преобразования одного объекта в другой, то есть получить из элемента одного типа элемент другого типа. Для 
отображения используется метод <code>map</code>, который имеет следующее определение:</p>
<pre class="brush:java;">
&lt;R&gt; Stream&lt;R&gt; map(Function&lt;? super T, ? extends R&gt; mapper)
</pre>
<p>Передаваемая в метод map функция задает преобразование от объектов типа T к типу R. И в результате возвращается новый поток с преобразованными объектами.</p>
<p>Возьмем вышеопределенный класс телефонов и выполним преобразование от типа Phone к типу String:</p>
<pre class="brush:java;">
Stream&lt;Phone&gt; phoneStream = Stream.of(new Phone("iPhone 6 S", 54000), new Phone("Lumia 950", 45000),
                new Phone("Samsung Galaxy S 6", 40000));
...
<p>Для преобразования объектов в типы Integer, Long, Double определены специальные методы <span class="b">mapToInt()</span>, <span class="b">mapToLong()</span> и 
<span class="b">mapToDouble()</span> соответственно.</p>
<h3>Плоское отображение. Метод flatMap</h3>
<p>Плоское отображение выполняется тогда, когда из одного элемента нужно получить несколько. Данную операцию выполняет метод <span class="b">flatMap</span>:</p>
<pre class="brush:java;">
&lt;R&gt; Stream&lt;R&gt; flatMap(Function&lt;? super T, ? extends Stream&lt;? extends R&gt;&gt; mapper)
</pre>
<p>Например, в примере выше мы выводим название телефона и его цену. Но что, если мы хотим установить для каждого телефона цену со скидкой и цену без 
скидки. То есть из одного объекта Phone нам надо получить два объекта с информацией, например, в виде строки. Для этого применим flatMap:</p>
<pre class="brush:java;">
Stream&lt;Phone&gt; phoneStream = Stream.of(new Phone("iPhone 6 S", 54000), new Phone("Lumia 950", 45000),
                new Phone("Samsung Galaxy S 6", 40000));
...
<div class="nav"><p><a href="10.2.php">Назад</a><a href="./">Содержание</a><a href="10.8.php">Вперед</a></p></div></div>
<div class="socBlock">
