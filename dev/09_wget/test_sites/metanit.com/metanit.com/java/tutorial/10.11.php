...
<html >
<head>
<title>Java | Операции сведения в Stream API</title>
<meta charset="utf-8" />
<meta name="description" content="Операции сведения данных из потоков в Stream API в Java, методы count, min, max, anyMatch, allMatch, noneMatch">
...
<h3>findFirst и findAny</h3>
<p>Метод <span class="b">findFirst()</span> извлекает из потока первый элемент, а <span class="b">findAny()</span> извлекает случайный объект из потока (нередко так же первый):</p>
<pre class="brush:java;">
ArrayList&lt;String&gt; names = new ArrayList&lt;String&gt;();
names.addAll(Arrays.asList(new String[]{"Tom", "Sam", "Bob", "Alice"}));
...
<p>Еще одна группа операций сведения возвращает логическое значение true или false:</p>
<ul>
<li><p><code>boolean allMatch(Predicate&lt;? super T&gt; predicate)</code>: возвращает true, если все элементы потока удовлетворяют условию в предикате</li>
<li><p><code>boolean anyMatch(Predicate&lt;? super T&gt; predicate)</code>: возвращает true, если хоть один элемент потока удовлетворяют условию в предикате</li>
<li><p><code>boolean noneMatch(Predicate&lt;? super T&gt; predicate)</code>: возвращает true, если ни один из элементов в потоке не удовлетворяет условию в предикате</li>
</ul>
...
<h3>min и max</h3>
<p>Методы <span class="b">min()</span> и <span class="b">max()</span> возвращают соответственно минимальное и максимальное значение. Поскольку данные в потоке 
могут представлять различные типы, в том числе сложные классы, то в качестве параметра в эти методы передается объект интерфейса 
<a href="https://metanit.com/java/tutorial/5.6.php">Comparator</a>, который указывает, как сравнивать объекты:</p>
<pre class="brush:java;">
Optional&lt;T&gt; min(Comparator&lt;? super T&gt; comparator)
Optional&lt;T&gt; max(Comparator&lt;? super T&gt; comparator)
</pre>
<p>Оба метода возвращают элемент потока (минимальный или максимальный), обернутый в объект Optional.</p>
...
<p>Интерфейс Comparator - это функциональный интерфейс, который определяет один метод compare, принимающий два сравниваемых объекта и возвращающий число (если первый объект больше, возвращается положительное число, иначе 
...
compare интерфейса Comparator.  Поскольку сравниваются числа, то в метод передается в качестве компаратора статический метод <code>Integer.compare()</code>.</p>
<p>При этом методы min и max возвращают именно Optional, и чтобы получить непосредственно результат операции из Optional, необходимо вызвать метод <code>get()</code>.</p>
...
<p>В данном случае мы находим минимальный и максимальный объект Phone: фактически объекты с максимальной и минимальной ценой. Для определения функциональности 
сравнения в классе Phone реализован статический метод <code>compare</code>, который соответствует сигнатуре метода compare интерфейса Comparator. 
И в методах min и max применяем этот статический метод для сравнения объектов.</p>
...
<div class="nav"><p><a href="10.4.php">Назад</a><a href="./">Содержание</a><a href="10.5.php">Вперед</a></p></div></div>
<div class="socBlock">
