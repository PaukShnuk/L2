...
<html >
<head>
<title>Java | Форматируемый вывод. PrintStream и PrintWriter</title>
<meta charset="utf-8" />
<meta name="description" content="Вывод данных на консоль и считывание данных с консоли в языке программирования Java, Форматируемый ввод и вывод, классы PrintStream и PrintWriter">
...
<h2>Форматируемый ввод и вывод. PrintStream и PrintWriter</h2><div class="date">Последнее обновление: 26.04.2018</div>
...
<h3>Класс PrintStream</h3><p>Класс PrintStream - это именно тот класс, который используется для вывода на консоль. Когда мы выводим на консоль некоторую информацию с помощью 
вызова <code>System.out.println()</code>, то тем самым мы задействует <span class="b">PrintStream</span>, так как переменная <code>out</code> в 
классе System как раз и представляет объект класса PrintStream, а метод <code>println()</code> - это метод класса PrintStream.</p>
<p>Но PrintStream полезен не только для вывода на консоль. Мы можем использовать данный класс для записи информации в поток вывода. Для этого PrintStream 
определяет ряд конструкторов:</p> 
...
<p>Параметр <code>outputStream</code> - это объект OutputStream, в который производится запись. Параметр <code>autoFlushingOn</code> при значении 
true позволяет автоматически записывать данные в поток вывода. По умолчанию этот параметр равен false. Параметр <code>charSet</code> позволяет указать кодировку символов.</p>
<p>В качестве источника для записи данных вместо OutputStream можно использовать объект File или строковый путь, по которому будет создаваться файл.</p>
<p>Для вывода информации в выходной поток PrintStream использует следующие методы:</p>
<ul>
<li><p><code>println()</code>: вывод строковой информации с переводом строки</p></li>
<li><p><code>print()</code>: вывод строковой информации без перевода строки</p></li>
<li><p><code>printf()</code>: форматированный вывод</p></li>
</ul>
...
<p>В данном случае применяется форма конструктора PrintStream, которая в качестве параметра принимает поток вывода: <code>PrintStream (OutputStream out)</code>. Кроме того, 
мы могли бы использовать ряд других форм конструктора, например, указывая названия файла для записи: <code>PrintStream (string filename)</code></p>
<p>В качестве потока вывода используется объект <code>FileOutputStream</code>. С помощью метода <code>println()</code> производится запись информации 
в выходной поток - то есть в объект FileOutputStream. (В случае с выводом на консоль с помощью <code>System.out.println()</code> в качестве потока вывода выступает 
консоль)</p>
<p>Кроме того, как и любой поток вывода и наследник класса OutputStream он имеет метод <code>write</code>:</p>
<pre class="brush:java;">
import java.io.*;
...
<h3>PrintWriter</h3>
<p>На PrintStream похож другой класс <span class="b">PrintWriter</span>. Его можно использовать как для вывода информации на консоль, так и в файл или 
любой другой поток вывода. Данный класс имеет ряд конструкторов:</p>
<ul>
<li><p><code>PrintWriter(File file)</code>: автоматически добавляет информацию в указанный файл</p></li>
<li><p><code>PrintWriter(File file, String csn)</code>: автоматически добавляет информацию в указанный файл с учетом кодировки csn</p></li>
<li><p><code>PrintWriter(OutputStream out)</code>: для вывода информации используется существующий объект OutputStream, автоматически сбрасывая в него данные</p></li>
<li><p><code>PrintWriter(OutputStream out, boolean autoFlush)</code>: для вывода информации используется существующий объект OutputStream, второй параметр указывает, 
надо ли автоматически добавлять в OutputStream данные</p></li>
<li><p><code>PrintWriter(String fileName)</code>: автоматически добавляет информацию в файл по указанному имени</p></li>
<li><p><code>PrintWriter(String fileName, String csn)</code>: автоматически добавляет информацию в файл по указанному имени, используя кодировку csn</p></li>
<li><p><code>PrintWriter(Writer out)</code>: для вывода информации используется существующий объект Writer, в который автоматически идет запись данных</p></li>
<li><p><code>PrintWriter(Writer out, boolean autoFlush)</code>: для вывода информации используется существующий объект Writer, второй параметр указывает, 
надо ли автоматически добавлять в Writer данные</p></li>
</ul>
<p>PrintWriter реализует интерфейсы Appendable, Closable и Flushable, и поэтому после использования представляемый им поток надо закрывать.</p>
<p>Для записи данных в поток он также используется методы <span class="b">printf()</span> и <span class="b">println()</span>.</p>
...
<p>В качестве потока вывода здесь применяется System.out, а на консоль будет выведена строка "Hello world!"</p>
