...
<html >
<head>
<title>Интерфейсы в механизме обратного вызова | Java</title>
<meta charset="utf-8" />
<meta name="description" content="Оосбенности наследования классов в обобщениях в Java, различные способы наследования и преобразование обобщенных классов">
...
<h2>Интерфейсы в механизме обратного вызова</h2><div class="date">Последнее обновление: 28.12.2015</div>
...
<p>Одним из распространенных способов использования интерфейсов в Java является создание обратного вызова. Суть обратного вызова состоит в том, 
...
запускает печать документа на принтере и т.д.</p>
...
<p>Итак, здесь у нас определен класс Button, который в конструкторе принимает объект интерфейса EventHandler и в методе click (имитация нажатия) вызывает 
метод execute этого объекта.</p>
<p>Далее определяется реализация EventHandler в виде класса <code>ButtonClickHandler</code>. И в основной программе объект этого класса передается в конструктор Button. 
Таким образом, через конструктор мы устанавливаем обработчик нажатия кнопки. И при каждом вызове метода <code>button.click()</code> будет вызываться этот обработчик.</p>
<p>В итоге программа выведет на консоль следующий результат:</p>
<div class="console">
...
<p>Но казалось бы, зачем нам выносить все действия в интерфейс, его реализовать, почему бы не написать проще сразу в классе Button:</p>
<pre class="brush:java;">
class Button{
...
<p>Дело в том, что на момент определения класса нам не всегда бывают точно известны те действия, которые должны производиться. Особенно если класс Button 
...
кнопок - объектов Button и для каждого объекта надо определить свое действие. Например, изменим главный класс программы:</p>
<pre class="brush:java;">
public class EventsApp {
...
<p>Здесь у нас две кнопки - одна для включения-выключения телевизора, а другая для печати на принтере. Вместо того, чтобы создавать отдельные 
...
кнопки телевизора хранит дополнительное состояние в виде логической переменной <code>on</code>.</p>
<p>В итоге консоль выведет нам следующий результат:</p>
<div class="console">
<pre class="consoletext">
Телевизор включен!
...
<p>И в завершении надо сказать, что интерфейсы в данном качестве особенно широко используются в различных графических API - AWT, Swing, JavaFX, 
где обработка событий объектов - элементов графического интерфейса особенно актуальна.</p>
