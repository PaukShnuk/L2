...
<html >
<head>
<title>Java | Лямбды как параметры и результаты методов</title>
<meta charset="utf-8" />
<meta name="description" content="Лямбды как параметры методов в языке программирования Java, ссылки на методы в качестве параметров, возвращение из метода лямбда-выражения">
...
<h2>Лямбды как параметры и результаты методов</h2><div class="date">Последнее обновление: 25.04.2018</div>
...
<h3>Лямбды как параметры методов</h3>
...
<p>Функциональный интерфейс <code>Expression</code> определяет метод <code>isEqual()</code>, который возвращает true, если в отношении числа n действует 
какое-нибудь равенство.</p>
<p>В основном классе программы определяется метод <code>sum()</code>, который вычисляет сумму всех элементов массива, соответствующих некоторому условию. А 
само условие передается через параметр <code>Expression func</code>. Причем на момент написания метода sum мы можем абсолютно не знать, какое именно условие будет использоваться. 
Само же условие определяется в виде лямбда-выражения:</p>
<pre class="brush:java;">Expression func = (n)-&gt; n%2==0;</pre>
<p>То есть в данном случае все числа должны быть четными или остаток от их деления на 2 должен быть равен 0. Затем это лямбда-выражение передается в вызов метода 
sum.</p>
...
<h3>Ссылки на метод как параметры методов</h3>
<p>Начиная с JDK 8 в Java можно в качестве параметра в метод передавать ссылку на другой метод. В принципе данный способ аналогичен передаче в метод 
лямбда-выражения.</p>
<p>Ссылка на метод передается в виде <span class="b">имя_класса::имя_статического_метода</span> (если метод статический) или 
<span class="b">объект_класса::имя_метода</span> (если метод нестатический). Рассмотрим на примере:</p>
<pre class="brush:java;">
...
<p>Здесь также определен функциональный интерфейс Expression, который имеет один метод. Кроме того, определен класс ExpressionHelper, 
который содержит два статических метода. В принципе их можно было определить и в основном классе программы, но я вынес их в отдельный класс.</p>
<p>В основном классе программы LambdaApp определен метод <code>sum()</code>, который возвращает сумму элементов массива, соответствующих некоторому условию. 
Условие передается в виде объекта функционального интерфейса Expression.</p>
<p>В методе <code>main</code> два раза вызываем метод <code>sum</code>, передавая в него один и тот же массив чисел, но разные условия. 
Первый вызов метода <code>sum</code>:</p>
<pre class="brush:java;">System.out.println(sum(nums, ExpressionHelper::isEven));</pre>
<p>На место второго параметра передается <code>ExpressionHelper::isEven</code>, то есть ссылка на статический метод <code>isEven()</code> 
класса ExpressionHelper. При этом методы, на которые идет ссылка, должны совпадать по параметрам и результату с методом функционального интерфейса.</p>
<p>При втором вызове метода <code>sum</code> отдельно создается объект Expression, который затем передается в метод:</p>
<pre class="brush:java;">
Expression expr = ExpressionHelper::isPositive;
System.out.println(sum(nums, expr));
</pre>
<p>Использование ссылок на методы в качестве параметров аналогично использованию лямбда-выражений.</p>
...
<p>При использовании конструкторов методы функциональных интерфейсов должны принимать тот же список параметров, что и конструкторы класса, и должны возвращать 
объект данного класса.</p>
<h3>Лямбды как результат методов</h3>
...
<p>В данном случае определен функциональный интерфейс Operation, в котором метод execute принимает два значения типа int и возвращает значение типа int.</p>
<p>Метод action принимает в качестве параметра число и в зависимости от его значения возвращает то или иное лямбда-выражение. Оно может представлять 
либо сложение, либо вычитание, либо умножение, либо просто возвращает 0. Стоит учитывать, что формально возвращаемым типом метода action является интерфейс Operation, 
а возвращаемое лямбда-выражение должно соответствовать этому интерфейсу.</p>
<p>В методе main мы можем вызвать этот метод action. Например, сначала получить его результат - лямбда-выражение, которое присваивается переменной Operation. 
А затем через метод execute выполнить это лямбда-выражение:</p>
<pre class="brush:java;">
Operation func = action(1);
int a = func.execute(6, 5);
System.out.println(a);			// 11
