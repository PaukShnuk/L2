...
<html >
<head>
<title>Java | Класс Object</title>
<meta charset="utf-8" />
<meta name="description" content="Класс Object в Java, его методы equals, toString, hashCode, getClass и их переопределение в производных классах">
...
<h2>Класс Object и его методы</h2><div class="date">Последнее обновление: 21.04.2018</div>
...
<p>Хотя мы можем создать обычный класс, который не является наследником, но фактически все классы наследуются от класса <b>Object</b>. 
Все остальные классы, даже те, которые мы добавляем в свой проект, являются неявно производными от класса Object. 
Поэтому все типы и классы могут реализовать те методы, которые определены в классе Object. Рассмотрим эти методы.</p>
<h3>toString</h3>
<p>Метод <code>toString</code> служит для получения представления данного объекта в виде строки. При попытке вывести строковое представления 
какого-нибудь объекта, как правило, будет выводиться полное имя класса. Например:</p>
...
<p>Полученное мной значение (в данном случае <code>Person@7960847b</code>) вряд ли может служить хорошим строковым описанием объекта. 
Поэтому метод <code>toString()</code> нередко переопределяют. Например:</p>
<pre class="brush:java;">
public class Program{
...
<p>Метод <span class="b">hashCode</span> позволяет задать некоторое числовое значение, которое будет соответствовать данному объекту или его хэш-код. 
По данному числу, например, можно сравнивать объекты.</p>
<p>Например, выведем представление вышеопределенного объекта:</p>
<pre class="brush:java;">
Person tom = new Person("Tom");
...
<h3>Получение типа объекта и метод getClass</h3>
...
<h3>Метод equals</h3>
<p>Метод equals сравнивает два объекта на равенство:</p>
<pre class="brush:java;">
public class Program{
...
<p>Метод equals принимает в качестве параметра объект любого типа, который мы затем приводим к текущему, если они являются объектами 
одного класса.</p>
<p>Оператор <span class="b">instanceof</span> позволяет выяснить, является ли переданный в качестве параметра объект объектом определенного класса, 
в данном случае класса Person. Если объекты принадлежат к разным классам, то их сравнение не имеет смысла, и возвращается значение false.</p>
<p>Затем сравниваем по именам. Если они совпадают, возвращаем true, что будет говорить, что объекты равны.</p>
...
<div class="nav"><p><a href="3.8.php">Назад</a><a href="./">Содержание</a><a href="3.11.php">Вперед</a></p></div></div>
<div class="socBlock">
