...
<html >
<head>
<title>Java и Android | ConstraintLayout</title>
<meta charset="utf-8" />
<meta name="description" content="Контейнер ConstraintLayout в Android, установка ограничений и позиционирование элементов, layout_constraintLeft_toLeftOf, сдвиг виджетов">
...
<h2>ConstraintLayout</h2><div class="date">Последнее обновление: 14.10.2021</div>
...
<p><span class="b">ConstraintLayout</span> представляет контейнер, который позволяет создавать гибкие и масштабируемые визуальные интерфейсы.</p>
<p>Для позиционирования элемента внутри ConstraintLayout необходимо указать ограничения (constraints). Есть несколько типов ограничений. В частности, для установки 
позиции относительно определенного элемента испльзуются следующие ограничения:</p>
<ul>
<li><p><span class="b">layout_constraintLeft_toLeftOf</span>: левая граница позиционируется относительно левой границы другого элемента</p></li>
<li><p><span class="b">layout_constraintLeft_toRightOf</span>: левая граница позиционируется относительно правой границы другого элемента</p></li>
<li><p><span class="b">layout_constraintRight_toLeftOf</span>: правая граница позиционируется относительно левой границы другого элемента</p></li>
<li><p><span class="b">layout_constraintRight_toRightOf</span>: правая граница позиционируется относительно правой границы другого элемента</p></li>
<li><p><span class="b">layout_constraintTop_toTopOf</span>: верхняя граница позиционируется относительно верхней границы другого элемента</p></li>
<li><p><span class="b">layout_constraintTop_toBottomOf</span>: верхняя граница позиционируется относительно нижней границы другого элемента</p></li>
<li><p><span class="b">layout_constraintBottom_toBottomOf</span>: нижняя граница позиционируется относительно нижней границы другого элемента</p></li>
<li><p><span class="b">layout_constraintBottom_toTopOf</span>: нижняя граница позиционируется относительно верхней границы другого элемента</p></li>
<li><p><span class="b">layout_constraintBaseline_toBaselineOf</span>: базовая линия позиционируется относительно базовой линии другого элемента</p></li>
<li><p><span class="b">layout_constraintStart_toEndOf</span>: элемент начинается там, где завершается другой элемент</p></li>
<li><p><span class="b">layout_constraintStart_toStartOf</span>: элемент начинается там, где начинается другой элемент</p></li>
<li><p><span class="b">layout_constraintEnd_toStartOf</span>: элемент завершается там, где начинается другой элемент</code></p></li>
<li><p><span class="b">layout_constraintEnd_toEndOf</span>: элемент завершается там, где завершается другой элемент</code></p></li>
</ul>
<p>Возможно, по поводу четырех последних свойств возникло некоторое непонимание, что подразумевается под началом или завершением элемента. Дело в том, 
...
Например, при левосторонней ориентации начало - слева, а завершение - справа, поэтому атрибут <code>layout_constraintStart_toEndOf</code> 
фактически будет аналогичен атрибуту <code>layout_constraintLeft_toRightOf</code>. А при правосторонней ориентации - атрибуту 
<code>layout_constraintRight_toLeftOf</code>, так как начало справа, а завершение - слева</p>
<p>Каждое ограничение устанавливает позиционирование элемента либо по горизонтали, либо по вертикали. 
И для определения позиции элемента в ConstraintLayout необходимо указать как минимум одно ограничение по горизонтали и одно ограничение по вертикали.</p>
<p>Позиционирования может производиться относительно границ самого контейнера ContentLayout (в этом случае ограничение имеет значение <span class="b">parent</span>), 
либо же относительно любого другого элемента внутри ConstraintLayout, тогда в качестве значения ограничения указывается id этого элемента.</p>
...
<p>В данном случае у элемента TextView установлено два ограничение: одно погоризонтальной линии (<code>app:layout_constraintLeft_toLeftOf="parent"</code>), 
...
поэтому они принимают значение <span class="b">parent</span>, то есть ConstraintLayout.</p>
<p>Ограничение <code>app:layout_constraintLeft_toLeftOf="parent"</code> устанавливает левую границу TextView у левой границы контейнера.</p>
<p>Ограничение <code>app:layout_constraintTop_toTopOf="parent"</code> устанавливает верхнюю границу TextView у верхней границы контейнера.</p>
<p>В итоге TextView будет располагаться в верхнем левом углу контейнера.</p>
<img src="./pics/constraintlayout.png" alt="Расположение элементов в ConstraintLayout в Android Studio" />
<p>Стоит обратить внимание, что все эти атрибуты ограничений берутся из пространства имен "http://schemas.android.com/apk/res-auto", которое проецируется 
на префикс <span class="b">app</span>.</p>
...
<p>Здесь для текстового поля ввода EditText устанавливаются два ограничения относительно родительского контейнера ConstraintLayout, 
поэтому ограничения имеют значение <span class="b">parent</span>, а сам EditText выравнивается по левой и верхней границе контейнера. 
...
Для этого в качестве значения атрибута передается id EditText:</p>
<pre class="brush:xml;">app:layout_constraintLeft_toRightOf="@id/editText"</pre>
<img src="./pics/constraintlayout2.png" alt="Позиционирование в ConstraintLayout в Android" />
...
<img src="./pics/constraintlayout3.png" alt="Позиционирование в ConstraintLayout в Android" />
<p>Более того мы можем позиционировать оба элемента один относительно другого:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<h4>Позиционирование в центре</h4>
...
<img src="./pics/constraintlayout4.png" alt="Позиционирование в центре ConstraintLayout в Android" />
<h3>Сдвиг</h3>
<p>Если элементы расположены по центру, ConstraintLayout позволяет их сдвигать по горизонтали и по вертикали. Для сдвига по горизонтали применяется атрибут 
<span class="b">layout_constraintHorizontal_bias</span>, а для сдвига по вертикали - атрибут <span class="b">layout_constraintVertical_bias</span>. 
В качестве значения они принимают число с плавающей точкой от 0 до 1. Значение по умолчанию - 0.5 (расположение по центру). Например:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>Первый TextView сдвигается на 20% от левой границы контейнера (значение по умолчанию - 0.5, поэтому при значении 0.2 элемент сдвигается влево). 
Второй TextView сдвигается на 90% от левой границы контейнера. Например, значение 1 означало бы, что элемент придвинут к правой границе, а значение 0 - к левой</p>
<img src="./pics/constraintlayout9.png" alt="Bias сдвиг элементов и виджетов в ConstraintLayout в Android Studio" />
<p>Аналогично работает атрибут <span class="b">layout_constraintVertical_bias</span>, который сдвигает по вертикали.</p>
...
<div class="nav"><p><a href="./3.10.php">Назад</a><a href="./">Содержание</a><a href="./3.7.php">Вперед</a></p></div></div>
<div class="socBlock">
