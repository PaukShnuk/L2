...
<html >
<head>
<title>Java и Android | Создание провайдера контента. Часть 3</title>
<meta charset="utf-8" />
<meta name="description" content="Создание провайдера контента и его использование в Android, класс  ContentResolver и его методы, добавление, удаление, получение и обновление данных">
...
<h2>Создание провайдера контента. Часть 3</h2><div class="date">Последнее обновление: 18.10.2021</div>
...
<p>В прошлой теме был определен провайдер контента. Рассмотрим, как его использовать. Сначала определим простейший визуальный интерфейс для 
тестирования возможностей провайдера в файле <span class="b">activity_main.xml</span>:</p>
<pre class="brush:xml;">
...
<p>Здесь определен набор кнопок для вывода списка друзей, а также добавления, обновления и удаления. Каждая кнопка будет вызывать соответсствующий метод в классе MainActivity.</p>
<p>Теперь изменим код класса <span class="b">MainActivity</span>. Для упрощения результаты будем выводить в окне <span class="b">Logcat</span> с помощью метода <span class="b">Log.d()</span>:</p>
...
<p>Разберем отдельные действия, выполняемые в данном коде.</p>
...
<p>Взаимодействие с провайдером контента осуществляется через объект <span class="b">ContentResolver</span>. Для получения данных вызывается метод <span class="b">query()</span>, по сути 
он представляет вызов метод query провайдера контента. В метод query передается uri - путь к данным, projection - набор столбцов для извлечения, выражение выборки и параметры для него и 
название столбца, по которому проводится сортировка.</p>
<p>Метод возвращает курсор Cursor, который с помощью метода <code>moveToNext()</code> можно перебрать и получить отдельные данные. Метод <code>getColumnName()</code> 
возвращает название столбца, а <code>getString()</code> - собственно значение этого столбца:</p>
<img src="./pics/contentprovider1.png" alt="Custom Content Provider in Android and Java" />
<p>Получение одного объекта по id:</p>
<pre class="brush:java;">
String[] projection = {
	FriendsContract.Columns._ID,
	FriendsContract.Columns.NAME,
...
<p>В данном случае получаем объект с _id=2.</p>
...
<p>Для добавления применяется метод <code>insert</code>, который принимает путь URI и добавляемые данные в виде ContentValues.</p>
...
<p>В данном случае обновляются данные у всех объектов, у которых "Name=Sam". Критерий обновления передается через третий параметр.</p>
<p>Естественно с помощью выражения SQL можно задать любою логику выборки объектов для обновления. И для большего удобства мы можем вводить в него данные 
с помощью параметров, которые задаются знаком вопроса:</p>
<pre class="brush:java;">
ContentResolver contentResolver = getContentResolver();
ContentValues values = new ContentValues();
values.put(FriendsContract.Columns.NAME, "Sam");
...
<p>В этом случае с помощью четвертого параметра передается массив значений для параметров выражения выборки.</p>
<p>Но в примерах выше обновлялись все строки в бд, которые имели, например, имя "Sam". Но также можно обновлять и один объект по id. 
Например, обновим строку с _id=3:</p>
<pre class="brush:java;">
ContentResolver contentResolver = getContentResolver();
ContentValues values = new ContentValues();
values.put(FriendsContract.Columns.NAME, "Sam");
...
<p>Удаление данных по общему условию:</p>
<pre class="brush:java;">
ContentResolver contentResolver = getContentResolver();
String selection = FriendsContract.Columns.NAME + " = ?";
String[] args = {"Sam"};
...
<p>В данном случае удаляются все строки, у которых Name=Sam.</p>
<p>Удаление по id:</p>
<pre class="brush:java;">
ContentResolver contentResolver = getContentResolver();
int count = contentResolver.delete(FriendsContract.buildFriendUri(2), null, null);
</pre>
<p>В данном случае удаляется строка с _id=2.</p>
