...
<html >
<head>
<title>Java и Android | RelativeLayout</title>
<meta charset="utf-8" />
<meta name="description" content="Контейнер RelativeLayout в  Android, определение в файле xml и в коде java, его основные свойства и примеры использвания">
...
<h2>RelativeLayout</h2><div class="date">Последнее обновление: 14.10.2021</div>
...
<p><span class="b">RelativeLayout</span> представляет объект <code>ViewGroup</code>, который располагает дочерние элементы относительно 
позиции других дочерних элементов разметки или относительно области самой разметки RelativeLayout. Используя относительное позиционирование, мы можем установить элемент по правому краю или в центре 
или иным способом, который предоставляет данный контейнер. Для установки элемента в файле xml мы можем применять следующие атрибуты:</p>
<ul>
<li><p><span class="b">android:layout_above</span>: располагает элемент над элементом с указанным Id</p></li>
<li><p><span class="b">android:layout_below</span>: располагает элемент под элементом с указанным Id</p></li>
<li><p><span class="b">android:layout_toLeftOf</span>: располагается слева от элемента с указанным Id</p></li>
<li><p><span class="b">android:layout_toRightOf</span>: располагается справа от элемента с указанным Id</p></li>
<li><p><span class="b">android:layout_toStartOf</span>: располагает начало текущего элемента, где начинается элемент с указанным Id</p></li>
<li><p><span class="b">android:layout_toEndOf</span>: располагает начало текущего элемента, где завершается элемент с указанным Id</p></li>

<li><p><span class="b">android:layout_alignBottom</span>: выравнивает элемент по нижней границе другого элемента с указанным Id</p></li>
<li><p><span class="b">android:layout_alignLeft</span>: выравнивает элемент по левой границе другого элемента с указанным Id</p></li>
<li><p><span class="b">android:layout_alignRight</span>: выравнивает элемент по правой границе другого элемента с указанным Id</p></li>
<li><p><span class="b">android:layout_alignStart</span>: выравнивает элемент по линии, у которой начинается другой элемент с указанным Id</p></li>
<li><p><span class="b">android:layout_alignEnd</span>: выравнивает элемент по линии, у которой завершается другой элемент с указанным Id</p></li>
<li><p><span class="b">android:layout_alignTop</span>: выравнивает элемент по верхней границе другого элемента с указанным Id</p></li>
<li><p><span class="b">android:layout_alignBaseline</span>: выравнивает базовую линию элемента по базовой линии другого элемента с указанным Id</p></li>

<li><p><span class="b">android:layout_alignParentBottom</span>: если атрибут имеет значение true, то элемент прижимается к нижней границе контейнера</p></li>
<li><p><span class="b">android:layout_alignParentRight</span>: если атрибут имеет значение true, то элемент прижимается к правому краю контейнера</p></li>
<li><p><span class="b">android:layout_alignParentLeft</span>: если атрибут имеет значение true, то элемент прижимается к левому краю контейнера</p></li>
<li><p><span class="b">android:layout_alignParentStart</span>: если атрибут имеет значение true, то элемент прижимается к начальному краю контейнера (при левосторонней ориентации текста - левый край)</p></li>
<li><p><span class="b">android:layout_alignParentEnd</span>: если атрибут имеет значение true, то элемент прижимается к конечному краю контейнера (при левосторонней ориентации текста - правый край)</p></li>
<li><p><span class="b">android:layout_alignParentTop</span>: если атрибут имеет значение true, то элемент прижимается к верхней границе контейнера</p></li>
<li><p><span class="b">android:layout_centerInParent</span>: если атрибут имеет значение true, то элемент располагается по центру родительского контейнера</p></li>
<li><p><span class="b">android:layout_centerHorizontal</span>: при значении true выравнивает элемент по центру по горизонтали</p></li>
<li><p><span class="b">android:layout_centerVertical</span>: при значении true выравнивает элемент по центру по вертикали</p></li>
</ul>
<p>Например, позиционирование относительно контейнера RelativeLayout:</p>
<pre class="brush:xml;">
...
<img src="./pics/relativelayout.png" alt="RelativeLayout в Android Studio" />
<p>Для позиционирования относительно другого элемента, нам надо указать id этого элемента. Так, поместим на RelativeLayout текстовое поле и кнопку:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;RelativeLayout 
	xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>В данном случае поле EditText располагается по центру в RelativeLayout, а кнопка помещается под EditText и выравнивается по его правой границе:</p>
<img src="./pics/relativelayout2.png" alt="Позиционирование RelativeLayout в Android" />
<h3>Программное создание RelativeLayout</h3>
<p>Создадим элемент RelativeLayout программно в коде Java:</p>
...
<p>Чтобы задать положение элемента в контейнере, применяется класс <span class="b">RelativeLayout.LayoutParams</span>. Через конструктор 
устанавливаются значения для для ширины и высоты. Например, у элемента EditText для ширины устанавливается значение <code>MATCH_PARENT</code>, 
а для высоты - <code>WRAP_CONTENT</code>.</p>
<p>С помощью метода <span class="b">addRule()</span> мы можем добавлять дополнительные правила для позиционирования элемента. Этот метод в качестве параметра принимает
числовую константу, которая представляет параметр позиционирования и которая аналогична атрибуту. Например, атрибуту <code>android:layout_centerInParent</code> 
будет соответствовать константа <code>CENTER_IN_PARENT</code>, а атрибуту <code>android:layout_alignRight</code> константа <code>ALIGN_RIGHT</code>.</p>
<p>Стоит отметить, что в целях упрощения кода для установки id у EditText вызывается метод <span class="b">generateViewId();</span>, который позволяет программно сгенерировать 
id для элемента управления.</p>
<p>Затем установленный id передается в качестве второго параметра в метод addRule при установке правил для кнопки:</p>
<pre class="brush:java;">buttonParams.addRule(RelativeLayout.BELOW, editText.getId());</pre>
<p>Тем самым мы указываем относительно какого элемента надо задать расположение.</p>
