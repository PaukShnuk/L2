...
<html >
<head>
<title>Java и Android | Выпадающий список Spinner</title>
<meta charset="utf-8" />
<meta name="description" content="Создание выпадающего списка в Android и Java, элемент Spinner, подключение источника данных, обработка выбора элемента">
...
<h2>Выпадающий список Spinner</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p><span class="b">Spinner</span> представляет собой выпадающий список. Определим в файле разметки <span class="b">activity_main.xml</span> 
элемент <span class="b">Spinner</span>:</p>
<pre class="brush:xml;">
...
<p>В качестве источника данных, как и для ListView, для Spinner может служить простой список или массив, соданный программно, либо ресурс 
<code>string-array</code>. Взаимодействие с источником данных также будет идти через адаптер. В данном случае определим источник программно в виде массива в коде MainActivity:</p>
...
<p>Используемый при создании ArrayAdapter ресурс <span class="b">android.R.layout.simple_spinner_item</span> предоставляется платформой и 
является стандартной разметкой для создания выпадающего списка.</p>
<p>С помощью метода <code>adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)</code> устанавливаются дополнительные визуальные возможности списка. 
А передаваемый в метод ресурс <span class="b">android.R.layout.simple_spinner_dropdown_item</span> используется для визуализации выпадающего списка и также предоставляется платформой.</p>
<img src="./pics/spinner1.png" alt="Элемент Spinner в Android и Java" />
<h3>Обработка выбора элемента</h3>
<p>Используя слушатель <span class="b">OnItemSelectedListener</span>, в частности его метод <code>onItemSelected()</code>, мы можем обрабатывать 
выбор элемента из списка. Вначале добавим в разметку интерфейса текстовое поле, которое будет выводить выбранный элемент:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>И изменим код MainActivity, определив для элемента Spinner слушатель <span class="b">OnItemSelectedListener</span>:</p>
<pre class="brush:java;">
package com.example.listapp;
...
<p>Метод <code>onItemSelected</code> слушателя OnItemSelectedListener получает четыре параметра:</p>
<ul>
<li><p><code>parent</code>: объект Spinner, в котором произошло событие выбора элемента</p></li>
<li><p><code>view</code>: объект View внутри Spinnera, который представляет выбранный элемент</p></li>
<li><p><code>position</code>: индекс выбранного элемента в адаптере</p></li>
<li><p><code>id</code>: идентификатор строки того элемента, который был выбран</p></li>
</ul>
<p>Получив позицию выбранного элемента, мы можем найти его в списке:</p>
<pre class="brush:java;">String item = (String)parent.getItemAtPosition(position);</pre>
<p>Для установки слушателя OnItemSelectedListener в классе Spinner применяется метод <span class="b">setOnItemSelectedListener</span>.</p>
<img src="./pics/spinner2.png" alt="Выбор элемента в списке Spinner в Android и Java с помощью OnItemSelectedListener" />
...
<div class="nav"><p><a href="./5.3.php">Назад</a><a href="./">Содержание</a><a href="./4.2.php">Вперед</a></p></div></div>
<div class="socBlock">
