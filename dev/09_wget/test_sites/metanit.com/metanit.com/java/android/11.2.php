...
<html >
<head>
<title>Java и Android | Аудио</title>
<meta charset="utf-8" />
<meta name="description" content="Воспроизведение аудио в Android, добавление аудиофайла, класс MediaPlayer и его методы">
...
<h2>Воспроизведение аудио</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>Для воспроизведения музыки и других аудиоматериалов Android предоставляет класс <span class="b">MediaPlayer</span>.</p>
<p>Чтобы воспроизводить аудио, MediaPlayer должен знать, какой именно ресурс (файл) нужно производить. Установить нужный ресурс для воспроизведения 
можно тремя способами:</p>
<ul>
<li><p>в метод <code>create()</code> объекта MediaPlayer передается id ресурса, представляющего аудиофайл</p></li>
<li><p>в метод <code>create()</code> объекта MediaPlayer передается объект Uri, представляющего аудиофайл</p></li>
<li><p>в метод <code>setDataSource()</code> объекта MediaPlayer передается полный путь к аудиофайлу</p></li>
</ul>
<p>После установки ресурса вызывается метод <code>prepare()</code> или <code>prepareAsync()</code> (асинхронный вариант prepare()). Этот метод подготавливает 
аудиофайл к воспроизведению, извлекая из него первые секунды. Если мы воспроизводим файл из сети, то лучше использовать prepareAsync().</p>
<p>Для управления воспроизведением в классе MediaPlayer определены следующие методы:</p>
<ul>
<li><p><code>start()</code>: запускает аудио</p></li>
<li><p><code>pause()</code>: приостанавливает воспроизведение</p></li>
<li><p><code>stop()</code>: полностью останавливает воспроизведение</p></li>
</ul>
<p>Итак, создадим новый проект. Как и в случае с видео, аудиофайл должен находиться в папке <span class="b">res/raw</span>, 
поэтому добавим в проект в Android Studio такую папку. Для этого нажмем на папку res правой кнопкой мыши и в появившемся меню 
...
<p>И скопируем в нее какой-нибудь аудио-файл.</p>
<img src="./pics/audio1.png" alt="Добавление аудио-файла в Android Studio" />
<p>Для управления аудиопотоком определим в файле <span class="b">activity_main.xml</span> три кнопки:</p>
<pre class="brush:xml;">
...
<p>И изменим код класса <span class="b">MainActivity</span>:</p>
...
<p>Обработчик каждой кнопки кроме вызова определенного метода у <span class="b">MediaPlayer</span> также переключает доступность кнопок.</p>
<p>И если запуск и приостановка воспроизведения особых сложностей не вызывает, то при обработки полной остановки воспроизведения мы можем столкнуться с рядом трудностей. 
В частности, когда мы выходим из приложения - полностью закрываем его через диспетчер приложений либо нажимаем на кнопку Назад, то у нас для текущей Activity 
вызывается метод onDestroy, activity уничтожается, но <span class="b">MediaPlayer</span> продолжает работать. Если мы вернемся к приложению, то activity будет создана заново, но с помощью 
кнопок мы не сможем управлять воспроизведением. Поэтому в даном случае переопределяем метод onDestroy, в котором завершаем воспроизведение.</p>
<p>Для корректного завершения также определен обработчик естественного завершения 
воспроизведения <code>OnCompletionListener</code>, действие которого будет аналогично нажатию на кнопку "Стоп".</p>
<img src="./pics/audio2.png" alt="Воспроизведение аудио в Android и MediaPlayer" />
<p>Добавим к воспроизведению индикатор громкости. Для этого в файле <span class="b">activity_main.xml</span> определим <span class="b">SeekBar</span>:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>И далее изменим код класса <span class="b">MainActivity</span>:</p>
<pre class="brush:java;">
package com.example.mediaapp;
...
<p>Для управления громкостью звука применяется класс <span class="b">AudioManager</span>. А в с помощью вызова <code>audioManager.setStreamVolume(AudioManager.STREAM_MUSIC, progress, 0);</code> 
в качестве второго параметра можно передать нужное значение громкости.</p>
<img src="./pics/audio3.png" alt="Громкость в MediaPlayer в Android и Java" />
...
<div class="nav"><p><a href="./11.1.php">Назад</a><a href="./">Содержание</a><a href="./12.3.php">Вперед</a></p></div></div>
<div class="socBlock">
