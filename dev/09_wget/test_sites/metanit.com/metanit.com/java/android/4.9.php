...
<html >
<head>
<title>Java и Android | Button</title>
<meta charset="utf-8" />
<meta name="description" content="Кнопки Button в Android, обработка нажатий, onClick и слушатель View.OnClickListener">
...
<h2>Button</h2><div class="date">Последнее обновление: 15.10.2021</div>
...
<p>Одним из часто используемых элементов являются кнопки, которые представлены классом <span class="b">android.widget.Button</span>. Ключевой особенностью 
кнопок является возможность взаимодействия с пользователем через нажатия.</p>
<p>Некоторые ключевые атрибуты, которые можно задать у кнопок:</p>
<ul>
<li><p><code>text</code>: задает текст на кнопке</p></li>
<li><p><code>textColor</code>: задает цвет текста на кнопке</p></li>
<li><p><code>background</code>: задает фоновый цвет кнопки</p></li>
<li><p><code>textAllCaps</code>: при значении true устанавливает текст в верхнем регистре. По умолчанию как раз и применяется значение true</p></li>
<li><p><code>onClick</code>: задает обработчик нажатия кнопки</p></li>
</ul>
<p>Итак, изменим код в <span class="b">activity_main.xml</span> следующим образом:</p>
<pre class="brush:xml;">
...
<p>При помощью атрибута  <code>android:onClick</code> можно задать метод в коде java, который будет обрабатывать нажатия кнопки. Так, 
в вышеприведенном примере это метод <code>sendMessage</code>. Теперь перейдем к коду MainActivity и пропишем в нем такой метод:</p>
...
<p>При создании метода обработки нажатия следует учитывать следующие моменты:</p>
<ul>
<li><p>Метод должен объявляться с модификатором <code>public</code></p></li>
<li><p>Должен возвращать значение <code>void</code></p></li>
<li><p>В качестве параметра принимать объект <code>View</code>. Этот объект View и представляет собой нажатую кнопку</p></li>
</ul>
<p>В данном случае после нажатия на кнопку в TextView выводится текст из EditText.</p>
<img src="./pics/button1.png" alt="onClick и Button нажатие кнопки в Java и Android" />
<p>Аналогичный пример полностью в коде MainActivity:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>При программном создании кнопки мы можем определить у нее слушатель нажатия <span class="b">View.OnClickListener</span> и с помощью его метода onClick также обработать нажатие:</p>
<pre class="brush:java;">
button.setOnClickListener(new View.OnClickListener() {
    public void onClick(View v) {
        // Обработка нажатия
...
<div class="nav"><p><a href="./4.8.php">Назад</a><a href="./">Содержание</a><a href="./4.13.php">Вперед</a></p></div></div>
<div class="socBlock">
