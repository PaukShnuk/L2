...
<html >
<head>
<title>Java и Android | FrameLayout</title>
<meta charset="utf-8" />
<meta name="description" content="Контейнер компоновки FrameLayout в Android и его использование, атрибут android_gravity">
...
<h2>FrameLayout</h2><div class="date">Последнее обновление: 14.10.2021</div>
...
<p>Контейнер FrameLayout предназначен для вывода на экран одного помещенного в него визуального элемента. 
Если же мы поместим несколько элементов, то они будут накладываться друг на друга. Тем не менее также можно располагать в FrameLayout несколько элементов.</p>
<p>Допустим, вложим в FrameLayout два элемента TextView:</p>
<pre class="brush:xml;">
...
<p>Здесь оба элемента позиционируются в одно и то же место - в левый верхний угол контейнера FrameLayout, и чтобы избежать наложения, в данном случае у второго TextView 
устанавливается отступ сверху в 50 единиц.</p>
<img src="./pics/framelayout2.png" alt="FrameLayout в Android" />
<p>Нередко FrameLayout применяется для создания производных контейнеров, например, ScrollView, который обеспечивает прокрутку.</p>
<p>Элементы управления, которые помещаются в FrameLayout, могут установить свое позиционирование с помощью атрибута <span class="b">android:layout_gravity</span>:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;FrameLayout 
	xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>При указании значения мы можем комбинировать ряд значений, разделяя их вертикальной чертой: <code>bottom|center_horizontal</code></p>
<img src="./pics/framelayout1.png" alt="Gravity в FrameLayout в Android" />
<p>Программное создание FrameLayout в коде MainActivity:</p>
