...
<html >
<head>
<title>Java и Android | DatePickerDialog и TimePickerDialog</title>
<meta charset="utf-8" />
<meta name="description" content="Диалоговые окна DatePickerDialog и TimePickerDialog для выбора даты и времени в Android и Java">
...
<h1>Диалоговые окна</h1><h2>DatePickerDialog и TimePickerDialog</h2><div class="date">Последнее обновление: 18.10.2021</div>
...
<p>По умолчанию в Android уже определены два диалоговых окна - <span class="b">DatePickerDialog</span> и <span class="b">TimePickerDialog</span>, которые позволяют выбрать дату и время.</p>
<p>Кроме установки даты DatePickerDialog позволяет обработать выбор даты с помощью слушателей <span class="b">OnDateChangedListener</span> 
и <span class="b">OnDateSetListener</span>. Что позволяет использовать выбранную дату далее в приложении.</p>
<p>Подобным образом TimePickerDialog позволяет обработать выбор времени с помощью слушателей <span class="b">OnTimeChangedListener</span> и 
<span class="b">OnTimeSetListener</span></p>
<p>Пи работе с данными компонентами надо учитывать, что отсчет месяцев в DatePickerDialog начинается с нуля, то есть январь будет иметь номер 0, а декабрь - 11. 
И аналогично в TimePickerDialog отсчет секунд и минут будет идти с 0 до 59, а часов - с 0 до 23.</p>
<p>Используем DatePickerDialog и TimePickerDialog в приложении. Определим следующую разметку интерфейса в <span class="b">activity_main.xml</span>:</p>
<pre class="brush:xml;">
...
<p>Здесь определены две кнопки для выбора даты и времени и текстовое поле, отображающее выбранные дату и время. И изменим код <span class="b">MainActivity</span>:</p>
...
<p>Ключевым классом здесь является <span class="b">java.util.Calendar</span>, который хранится в стандартной библиоетке классов Java. В методе 
<code>setInitialDateTime()</code> мы получаем из экземпляра этого класса количество миллисекунд <code>dateAndTime.getTimeInMillis()</code> 
и с помощью форматирования выводим на текстовое поле.</p>
<p>Метод <code>setDate()</code>, вызываемый по нажатию на кнопку, отображает окно для выбора даты. При создании окна его объекту передается обработчик выбора даты 
<code>DatePickerDialog.OnDateSetListener</code>, который изменяет дату на текстовом поле.</p>
<p>Аналогично метод <code>setTime()</code> отображает окно для выбора времени. Объект окна использует обработчик выбора времени 
<code>TimePickerDialog.OnTimeSetListener</code>, который изменяет время на текстовом поле.</p>
<p>И поле запуска, нажав на кнопку изменения времени, мы сможем установить время:</p>
<img src="./pics/TimePickerDialog1.png" alt="TimePickerDialog in Android" />
<p>Аналогично работает окно установки даты:</p>
<img src="./pics/DatePickerDialog1.png" alt="DatePickerDialog in Android" />
...
<div class="nav"><p><a href="./16.1.php">Назад</a><a href="./">Содержание</a><a href="./18.2.php">Вперед</a></p></div></div>
<div class="socBlock">
