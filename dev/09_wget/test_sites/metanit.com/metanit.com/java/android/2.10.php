...
<html >
<head>
<title>Java и Android | Взаимодействие между Activity</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие между Activity в Android, управление стеком activity">
...
<h2>Взаимодействие между Activity</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>В прошлых темах мы рассмотрели жизненный цикл activity и запуск новых activity с помощью объекта Intent. Теперь рассмотрим некоторые особенности взаимодействия между 
activity в одном приложении. Допустим, у нас есть три activity: MainActivity, SecondActivity и ThirdActivity.</p>
<img src="./pics/activity6.png" alt="Back stak и управление стеком из Activity в Android и Java" />
<p>С помощью Intent, например, по нажатию кнопки MainActivity запускает SecondActivity:</p>
<pre class="brush:java;">
Intent intent = new Intent(this, SecondActivity.class);
startActivity(intent);
</pre>
<p>На SecondActivity тоже есть кнопка, которая запускает ThirdActivity:</p>
<pre class="brush:java;">
Intent intent = new Intent(this, ThirdActivity.class);
startActivity(intent);
</pre>
<p>На ThirdActivity также есть кнопка, которая возвращается к первой activity - MainActivity:</p>
<pre class="brush:java;">
Intent intent = new Intent(this, MainActivity.class);
startActivity(intent);
</pre>
<img src="./pics/activities.png" alt="Взаимодействие между Activity в Android и Java" />
<p>Если мы последовательно запустим все activity: из главной MainActivity запустим SecondActivity, из SecondActivity - ThirdActivity, 
то в итоге у нас сложится следующий стек activity:</p>
<table>
<tr><td>ThirdActivity</td></tr>
<tr><td>SecondActivity</td></tr>
<tr><td>MainActivity</td></tr>
</table>
<p>Если после этого из ThirdActivity мы захотим обратиться к MainActivity, то метод  startActivity() запустит новый объект MainActivity (а не вернется к уже существующему), 
и стек уже будет выглядеть следующим образом:</p>
<table>
<tr><td>MainActivity</td></tr>
<tr><td>ThirdActivity</td></tr>
<tr><td>SecondActivity</td></tr>
...
<p>То есть у нас будут две независимые копии MainActivity. Такое положение нежелательно, если мы просто хотим перейти к существующей. 
И этот момент надо учитывать.</p>
<p>Если мы нажмем на кнопку <span class="b">Back (Назад)</span>, то текущая activity, которая находится на вершине стека, удаляется из стека, 
и предыдущая activity оказывается на вершине стека и возобновляет свою работу. И таким образом с помощью кнопки Back (Назад) мы сможем перейти к 
предыдущей activity в стеке. Например, в случае выше если мы нажмем на кнопку Назад, то MainActivity на вершине стека завершает свою работу, и на экране начинает 
отображаться ThirdActivity</p>
<table>
<tr><td>ThirdActivity</td></tr>
<tr><td>SecondActivity</td></tr>
<tr><td>MainActivity</td></tr>
</table>
<p>Тем не менее иногда возникает необходимость упавлять переходом между activity. Например, в данном случае нам нежелательно при нажатии на кнопку в ThirdActivity запускать новую копию 
MainActivity вместо того, чтобы просто перейти к MainActivity, которая была запущена первой и находится в самом низу стека. Рассмотрим, какие возможности предоставляет нам Android. </p>
<h3>Управление стеком activity</h3>
<p>Для управления стеком из activity Android предлагает нам использовать флаги - константы, определенные в классе <span class="b">Intent</span>. Применение определенного флага 
позволит нам определенным образом изменить положение в стеке для определенных activity.</p>
<p>Например, возьмем предыдущую задачу, когда после нажатия на кнопку в ThirdActivity запускается новый экземпляр MainActivity. Но мы хотим не запускать новую, а перейти к уже существующей.</p>
<table>
<tr><td>MainActivity</td></tr>
<tr><td>ThirdActivity</td></tr>
<tr><td>SecondActivity</td></tr>
...
<p>Чтобы выйти из этой ситуации, мы можем использовать флаг <span class="b">Intent.FLAG_ACTIVITY_REORDER_TO_FRONT</span>:</p>
<pre class="brush:java;">
Intent intent = new Intent(this, MainActivity.class);
intent.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
startActivity(intent);
</pre>
<p>флаг <span class="b">Intent.FLAG_ACTIVITY_REORDER_TO_FRONT</span> перемещает activity, к которой осуществляется переход на вершину стека, если она ужее есть в стеке. 
И в этом случае после перехода из ThirdActivity к MainActivity стек будет выглядеть следующим образом:</p>
<table>
<tr><td>MainActivity</td></tr>
<tr><td>ThirdActivity</td></tr>
<tr><td>SecondActivity</td></tr>
</table>
<p>Если же нам просто надо перейти из ThirdActivity к MainActivity, как если бы мы перешли назад с помощью кнопки Back, то мы можем использовать 
флаги <span class="b">Intent.FLAG_ACTIVITY_CLEAR_TOP</span> и <span class="b">Intent.FLAG_ACTIVITY_SINGLE_TOP</span>:</p>
<pre class="brush:java;">
Intent intent = new Intent(this, MainActivity.class);
intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
startActivity(intent);
</pre>
<p>Флаг <span class="b">Intent.FLAG_ACTIVITY_CLEAR_TOP</span> очищает все activity кроме той, которая запускается (если она уже есть в стеке). А флаг <span class="b">Intent.FLAG_ACTIVITY_SINGLE_TOP</span> указывает, что если в вершине стеке уже есть activity, которую надо запустить, то она НЕ запускается (то она может существовать в 
стеке только в единичном виде).</p>
<p>В этом случае после перехода из ThirdActivity к MainActivity стек будет полностью очищен, и там останется одна MainActivity.</p>
<p>Еще один флаг - <span class="b">Intent.FLAG_ACTIVITY_NO_HISTORY</span> позволит не сохранять в стеке запускаемую activity. Например, при запуске SecondActivity мы не хотим ее сохранять в стеке:</p>
<pre class="brush:java;">
Intent intent = new Intent(this, SecondActivity.class);
intent.addFlags(Intent.FLAG_ACTIVITY_NO_HISTORY);
startActivity(intent);
</pre>
<p>В этом случае при переходе по цепочке MainActivity -&gt; SecondActivity -&gt; ThirdActivity стек будет выглядеть следующим образом:</p>
<table>
<tr><td>MainActivity</td></tr>
<tr><td>ThirdActivity</td></tr>
</table>
...
<div class="nav"><p><a href="./2.11.php">Назад</a><a href="./">Содержание</a><a href="./2.9.php">Вперед</a></p></div></div>
<div class="socBlock">
