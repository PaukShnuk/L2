...
<html >
<head>
<title>Java и Android | EditText</title>
<meta charset="utf-8" />
<meta name="description" content="EditText, его основные свойства и использование в Android, обработка ввода текста">
...
<h2>EditText</h2><div class="date">Последнее обновление: 15.10.2021</div>
...
<p>Элемент <span class="b">EditText</span> является подклассом класса <code>TextView</code>. Он также представляет текстовое поле, но теперь уже с возможностью ввода 
и редактирования текста. Таким образом, в EditText мы можем использовать все те же возможности, что и в TextView.</p>
<p>Из тех атрибутов, что не рассматривались в теме про TextView, следует отметить атрибут <span class="b">android:hint</span>. Он позволяет задать текст, который будет отображаться в качестве подсказки, если элемент EditText пуст. 
Кроме того, мы можем использовать атрибут <span class="b">android:inputType</span>, который позволяет задать клавиатуру для ввода. В частности, среди его значений можно выделить следующие:</p>
<ul>
<li><p><code>text</code>: обычная клавиатура для ввода однострочного текста</p></li>
<li><p><code>textMultiLine</code>: многострочное текстовое поле</p></li>
<li><p><code>textEmailAddress</code>: обычная клавиатура, на которой присутствует символ <code>@</code>, ориентирована на ввод email</p></li>
<li><p><code>textUri</code>: обычная клавиатура, на которой присутствует символ <code>/</code>, ориентирована на ввод интернет-адресов</p></li>
<li><p><code>textPassword</code>: клавиатура для ввода пароля</p></li>
<li><p><code>textCapWords</code>: при вводе первый введенный символ слова представляет заглавную букву, остальные - строчные</p></li>
<li><p><code>number</code>: числовая клавиатура</p></li>
<li><p><code>phone</code>: клавиатура в стиле обычного телефона</p></li>
<li><p><code>date</code>: клавиатура для ввода даты</p></li>
<li><p><code>time</code>: клавиатура для ввода времени</p></li>
<li><p><code>datetime</code>: клавиатура для ввода даты и времени</p></li>
</ul>
<p>Используем EditText:</p>
<pre class="brush:xml;">
...
<p>Первое поле здесь обычное однострочное, а второе - многострочное. Чтобы во втором поле текст выравнивался по верху, дополнительно устанавливается атрибут <code>android:gravity="top"</code>.</p>
<img src="./pics/edittext1.png" alt="EditText в Java и Android" />
<p>Одной из возможностей элемента EditText также является возможность обработать введенные символы по мере ввода пользователя. Для этого определим в файле 
<span class="b">activity_main.xml</span> следующую разметку:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>Предполагается, что введенные в EditText символы тут же будут отображаться в элементе TextView. И для этого также изменим код MainActivity:</p>
...
<p>С помощью метода <span class="b">addTextChangedListener()</span> здесь к элементу EditText добавляется слушатель ввода текста - объект 
<span class="b">TextWatcher</span>. Для его использования нам надо реализовать три метода, но в реальности нам хватит реализации метода <span class="b">onTextChanged</span>, 
который вызывается при изменении текста. Введенный текст передается в этот метод в качестве параметра CharSequence. В самом методе просто передаем этот текст в элемент 
TextView.</p>
<p>В итоге при вводе в EditText все символы также будут отображаться в TextView:</p>
<img src="./pics/edittext2.png" alt="TextChangedListener и EditText в Android" />
...
<div class="nav"><p><a href="./4.1.php">Назад</a><a href="./">Содержание</a><a href="./4.9.php">Вперед</a></p></div></div>
<div class="socBlock">
