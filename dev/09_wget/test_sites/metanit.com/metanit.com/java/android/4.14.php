...
<html >
<head>
<title>Java и Android | Snackbar</title>
<meta charset="utf-8" />
<meta name="description" content="Всплывающие окна в Android и Java, класс Snackbar, его настройка и методы, отличие от класса Toast">
...
<h2>Snackbar</h2><div class="date">Последнее обновление: 15.10.2021</div>
...
<p>Элемент <span class="b">Snackbar</span> в некотором роде похож на Toast: он также позволяет выводить всплывающие сообщения, но теперь сообщения растягиваются по ширине экрана.</p>
<p>Для применения Snackbar добавим в файл <span class="b">activity_main.xml</span> определение кнопки, по нажатию на которую будет появляться Snackbar:</p>
<pre class="brush:xml;">
...
<p>Здесь определена кнопка, по нажатию на которую будет оображаться сообщение.</p>
...
<p>Snackbar создается с помощью метода <span class="b">make()</span>, в который передаются три параметра: объект View, к которому прикрепляется всплывающее ообщение, 
само сообщение в виде строки и параметр, который указывает, сколько будет отображаться сообщение. Последний параметр может принимать числовое значение - количество 
миллисекунд, либо одну из трех констант: <span class="b">Snackbar.LENGTH_INDEFINITE</span> (отображение в течение неопределенного периода времени), 
<span class="b">Snackbar.LENGTH_LONG</span> (долгое отображение) или <span class="b">Snackbar.LENGTH_SHORT</span> (недолгое отображение).</p>
<p>После создания Snackbar отображается с помощью метода <span class="b">show</span>:</p>
<img src="./pics/snackbar3.png" alt="Snackbar в Android и Java" />
<p>При этом в отличие от Toast мы не можем повлиять на позицию сообщения, оно отображается внизу экрана и занимает всю нижнюю часть.</p>
<h3>Прикрепление обработчика события</h3>
<p>Snackbar позволяет добавить виджету действие, чтобы пользователь мог как-то прореагировать на сообщение. Например, изменим код MainActivity следующим образом:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>Для добавления действия у Snackbar применяется метод <span class="b">setAction()</span>. Первый параметр представляет текст кнопки в сообщении, на которую 
может нажать пользователь - в данном случае это "Next...". Второй параметр представляет реализацию интерфейса <span class="b">View.OnClickListener</span> (тот же самый, 
который используется для обработки нажатия кнопки). В методе <span class="b">onClick()</span> сообственно выполняем действия, которые вызываются при нажатии 
на кнопку в сообщении. В данном случае для простоты просто отображаем всплывающее сообщение в виде объекта Toast</p>
<img src="./pics/snackbar1.png" alt="Обработка нажатия click в Snackbar в Android и Java" />
<h3>Настройка визуального вида</h3>
<p>Ряд методов Snackbar позволяет настроить внешний вид:</p>
<ul>
<li><p><span class="b">setTextColor()</span>: настраивает цвет текста</p></li>
<li><p><span class="b">setBackgroundTint()</span>: настраивает цвет фона</p></li>
<li><p><span class="b">setActionTextColor()</span>: настраивает цвет текста кнопки в всплывающем сообщении</p></li>
</ul>
<pre class="brush:java;">
...
<img src="./pics/snackbar2.png" alt="Настройка визуального вида Snackbar в Android и Java" />
...
<div class="nav"><p><a href="./4.10.php">Назад</a><a href="./">Содержание</a><a href="./4.3.php">Вперед</a></p></div></div>
<div class="socBlock">
