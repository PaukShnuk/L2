...
<html >
<head>
<title>Java и Android | Асинхронная загрузка данных</title>
<meta charset="utf-8" />
<meta name="description" content="Асинхронная загрузка данных в провайдере контента в Android и Java, интерфейс LoaderManager">
...
<h2>Асинхронная загрузка данных</h2><div class="date">Последнее обновление: 18.10.2021</div>
...
<p>В базе данных может быть много данных, и их загрузка может занять некоторое время. В этом случае можно воспользоваться асинхронной загрузкой данных. Для этого 
класс activity или фрагмента должен реализовать интерфейс <span class="b">LoaderManager.LoaderCallbacks&lt;Cursor&gt;</span>.</p>
<p>Возьмем проект из прошлой темы, где реализован провайдер контента AppProvider, и изменим класс MainActivity:</p>
...
<p>Интерфейс LoaderManager.LoaderCallbacks&lt;Cursor&gt; предполагает реализацию трех методов. Метод <code>onCreateLoader()</code> загружает курсор. 
...
такого кода использует константа LOADER_ID.</p>
<p>В самом методе создается объект <span class="b">CursorLoader</span>. В его конструктор передается несколько параметров:</p>
<ul>
<li><p>объект Context (текущая activity)</p></li>
<li><p>набор столбцов, которые надо получить</p></li>
<li><p>выражение для выборки данных</p></li>
<li><p>значения для параметров для выражения выборки</p></li>
<li><p>столбец, по которому идет сортировка</p></li>
</ul>
<p>Метод <code>onLoadFinished</code> вызывается при загрузке курсора. Через второй параметр мы можем собственно получить курсор и через него загруженные данные. 
И соответственно в этом методе мы может перебрать курсор, получить данные и вывести их в элементах графического интерфейса или на консоль.</p>
<p>И метод <code>onLoaderReset</code> предназначен для сброса загрузчика.</p>
<p>Чтобы запустить загрузку данных, в методе onCreate вызывается метод <code>LoaderManager.getInstance(this).initLoader(LOADER_ID, null, this);</code>. 
Первый параметр - числовой код, а второй - объект Bundle. Это те значения, которые мы можем получить в методе onCreateLoader. И третий - объект Context.</p>
<p>В итоге при запуске MainActivity данные асинхронно будут загружены из базы данных:</p>
<img src="./pics/contentprovider1.png" alt="Content Provider in Android and Java" />
...
<div class="nav"><p><a href="./20.5.php">Назад</a><a href="./">Содержание</a><a href="./13.3.php">Вперед</a></p></div></div>
<div class="socBlock">
