...
<html >
<head>
<title>Java и Android | RecyclerView</title>
<meta charset="utf-8" />
<meta name="description" content="Элемент RecyclerView в Android и Java, оптимизация работы со списками, ViewHolder, LinearLayoutManager и layoutManager, компоновка списка">
...
<h2>RecyclerView</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>Элемент <span class="b">RecyclerView</span> предназначен для оптимизации работы со списками и во многом позволяет повысить 
производительность по сравнению со стандартным ListView.</p>
<p>Для представления данных добавим в проект в ту же папку, где расположен класс MainActivity, новый класс Java, который назовем <span class="b">State</span>:</p>
...
<p>Класс State содержит поля для хранения названия и столицы страны, а также ссылку на ресурс изображения флага страны. В данном случае предполагается, 
что в папке <span class="b">res/drawable</span> будут располагаться файлы изображений флагов для используемых стран.</p>
<p>Допустим, мы хотим вывести список объектов State с помощью <span class="b">RecyclerView</span>. Для этого добавим в папку <span class="b">res/layout</span> 
новый файл <span class="b">list_item.xml</span>:</p>
<pre class="brush:xml;">
...
<p>Этот файл определяет разметку для вывода одного объекта State.</p>
<p>Как и в случае с ListView, для вывода сложных объектов в <span class="b">RecyclerView</span> необходимо определить свой адаптер. Поэтому добавим в ту же папку, где расположен 
класс MainActivity и State, новый класс Java, который назовем <span class="b">StateAdapter</span>:</p>
<pre class="brush:java;">
package com.example.listapp;
...
<p>Адаптер, который используется в RecyclerView, должен наследоваться от абстрактного класса <span class="b">RecyclerView.Adapter</span>. Этот класс 
определяет три метода:</p>
<ul>
<li><p><span class="b">onCreateViewHolder</span>: возвращает объект ViewHolder, который будет хранить данные по одному объекту State.</p></li>
<li><p><span class="b">onBindViewHolder</span>: выполняет привязку объекта ViewHolder к объекту State по определенной позиции.</p></li>
<li><p><span class="b">getItemCount</span>: возвращает количество объектов в списке</p></li>
</ul>
<p>Для хранения данных в классе адаптера определен статический класс ViewHolder, который использует определенные в <span class="b">list_item.xml</span> элементы управления.</p>
<p>Теперь определим в файле <span class="b">activity_main.xml</span> элемент RecyclerView:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>Следует учитывать, что RecyclerView расположен в пакете <span class="b">androidx.recyclerview.widget</span> и является частью 
тулкита <span class="b">Android Jetpack</span>, поэтому при использовании виджета указывается полное его название с учетом пакета (в принципе как и для ConstraintLayout):</p>
<pre class="brush:xml;">&lt;androidx.recyclerview.widget.RecyclerView ....</pre>
<p>Для RecyclerView следует установить атрибут <span class="b">app:layoutManager</span>, который указает на тип менеджера компоновки. Менеджер 
компоновки представляет объект, который представлен классом <span class="b">LayoutManager</span>. По умолчанию библиотека RecyclerView предоставляет 
три реализации данного менеджера:</p>
<ul>
<li><p><span class="b">LinearLayoutManager</span>: упорядочивает элементы в виде списка с одной колонкой</p></li>
<li><p><span class="b">GridLayoutManager</span>: упорядочивает элементы в виде грида со столлбцами и строками. Грид может упорядочивать элементы 
по горизонтали (горизонтальный грид) или по вертикали (вертикальный грид)</p></li>
<li><p><span class="b">StaggeredGridLayoutManager</span>: аналогичен GridLayoutManager, однако не требует установки для каждого элемента в строке 
имели одну и ту же высоту (для вертикального грида) и одну и ту же ширну (для горизонтального грида)</p></li>
</ul>
<p>В данном случае с помощью значения <span class="b">androidx.recyclerview.widget.LinearLayoutManager</span> указываем, что элементы будут располагаться 
в виде простого списка. Обратите внимание, что класс LinearLayoutManager также расположен в библиотеке RecyclerView и поэтому при указании значения 
указывается полное название класса с именем его пакета.</p>
<p>И в конце изменим класс <span class="b">MainActivity</span>:</p>
<pre class="brush:java;">
package com.example.listapp;
...
<p>С помощью метода <code>setInitialData()</code> устанавливается набор начальных данных. В данном случае имеется в виду, что в папке res/drawables 
находится ряд ресурсов изображений для объектов State.</p>
<p>Как и в случае с выводом списка через ListView здесь сначала получаем элемент <span class="b">RecyclerView</span>, создаем адаптер и устанавливаем адаптер для RecyclerView.</p>
<p>Весь проект в итоге будет выглядеть следующим образом:</p>
<img src="./pics/recyclerview3.png" alt="Проект для RecyclerView в Android и Java" />
<p>В результате RecyclerView выведет набор объектов:</p>
<img src="./pics/recyclerview4.png" alt="Список RecyclerView в Android и Java" />
...
<div class="nav"><p><a href="./4.2.php">Назад</a><a href="./">Содержание</a><a href="./5.12.php">Вперед</a></p></div></div>
<div class="socBlock">
