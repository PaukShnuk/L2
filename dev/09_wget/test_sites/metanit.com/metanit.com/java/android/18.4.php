...
<html >
<head>
<title>Java и Android | Взаимодействие диалогового окна с Activity</title>
<meta charset="utf-8" />
<meta name="description" content="Взаимодействие диалогового окна AlertDialog и DialogFragment с Activity в Android и Java">
...
<h2>Взаимодействие с Activity</h2><div class="date">Последнее обновление: 18.10.2021</div>
...
<p>Взаимодействие между Activity и фрагментом производится, как правило, через интерфейс. К примеру, в прошлой теме MainActivity выводила список объектов, и теперь определим 
удаление из этого списка через диалоговое окно.</p>
<p>Для этого добавим в проект интерфейс <span class="b">Removable</span>:</p>
...
<p>Единственный метод интерфейса remove получает удаляемый объект в виде параметра name.</p>
<p>Теперь реализуем этот интерфейс в коде <span class="b">MainActivity</span>:</p>
<pre class="brush:java;">
package com.example.dialogsapp;
...
<p>Метод remove просто удаляет из адаптера переданный элемент.</p>
<p>Файл <span class="b">activity_main.xml</span> по прежнему определяет только элемент ListView:</p>
<pre class="brush:xml;">
...
<p>И в конце определим фрагмент <span class="b">CustomDialogFragment</span>:</p>
<pre class="brush:java;">
package com.example.dialogsapp;
...
<p>Метод <code>onAttach()</code> вызывается в начале жизненного цикла фрагмента, и именно здесь мы можем получить контекст фрагмента, в качестве которого 
выступает класс MainActivity. Так как MainActivity реализует интерфейс Removable, то мы можем преобразовать контекст к данному интерфейсу.</p>
<p>Затем в обработчике кнопки ОК вызывается метод remove объекта Removable, который удаляет переданный во фрагмент объект phone.</p>
<img src="./pics/dialog5.png" alt="Взаимодействие с MainActivity в диалоговом окне AlertDialog и DialogFragment в Android и Java" />
