...
<html >
<head>
<title>Android | Класс AsyncTask</title>
<meta charset="utf-8" />
<meta name="description" content="Многопоточность и асинхронность в Android на примере индикации процесса, класс AsyncTask и его методы">
...
<h2>Класс AsyncTask</h2><div class="date">Последнее обновление: 26.02.2017</div>
...
<p>В прошлых статьях был описан общий подход, который применяется сейчас для запуска в приложении нового потока и 
обновления в нем пользовательского интерфейса. Рассмотрим другой подход, который представляет класс AsyncTask. Хотя примение 
AsyncTask в современных приложениях Android устарел. Тем не менее, поскольку он по прежнему широко применяется, также рассмотрим его.</p>
<p>Чтобы использовать AsyncTask, нам надо:</p>
<ol>
<li><p>Создать класс, производный от AsyncTask (как правило, для этого создается внутренний класс в activity или во фрагменте)</p></li>
<li><p>Переопределить один или несколько методов AsyncTask для выполнения некоторой работы в фоновом режиме</p></li>
<li><p>При необходимости создать объект AsyncTask и вызывать его метод <code>execute()</code> для начала работы</p></li>
</ol>
<p>Итак, создадим простейшее приложение с использованием AsyncTask. Определим следующую разметку интерфейса в файле <span class="b">activity_main.xml</span>:</p>
<pre class="brush:xml;">
...
<p>Здесь определена кнопка для запуска фонового потока, а также текстовое поле и прогрессбар для индикации выполнения задачи. Кроме того, здесь определены дополнительная кнопка, которая 
увеличивает числов кликов, и текстовое поле, оторое выводит число кликов.</p>
<p>Далее определим в классе <span class="b">MainActivity</span> следующий код:</p>
...
<p>Класс задачи ProgressTask определен как внутренний класс. Он наследуется не просто от AsyncTask, а от его типизированной версии <code>AsyncTask&lt;Void, Integer, Void&gt;</code>. 
Она типизируется тремя типами:</p>
<ul>
<li><p>Класс для хранения информации, которая нужна для обработки задачи</p></li>
<li><p>Тип объектов, которые используются для индикации процесса выполнения задачи</p></li>
<li><p>Тип результата задачи</p></li>
</ul>
<p>Эти типы могут быть представлены разными классами. В данном случае сущность задачи будет состоять в переборе массива integers, представляющего набор элементов 
Integer. И здесь нам не надо передавать в задачу никакой объект, поэтому первый тип идет как <span class="b">Void</span>.</p>
<p>Для индикации перебора используются целые числа, которые показывают, какой объект из массива мы в данный момент перебираем. 
Поэтому в качестве второго типа используется Integer.</p>
<p>В качестве третьего типа используется опять Void, поскольку в данном случае не надо ничего возвращать из задачи.</p>
<p>AsyncTask содержит четыре метода, которые можно переопределить:</p>
<ul>
<li><p>Метод <span class="b">doInBackground()</span>: выполняется в фоновом потоке, должен возвращать определенный результат</p></li>
<li><p>Метод <span class="b">onPreExecute()</span>: вызывается из главного потока перед запуском метода <code>doInBackground()</code></p></li>
<li><p>Метод <span class="b">onPostExecute()</span>: выполняется из главного потока после завершения работы метода <code>doInBackground()</code></p></li>
<li><p>Метод <span class="b">onProgressUpdate()</span>: позволяет сигнализировать пользователю о выполнении фонового потока</p></li>
</ul>
<p>Так как метод doInBackground() не принимает ничего и не возвращает ничего, то в качестве его параметра используется <code>Void...</code> - массив Void, 
и в качестве возвращаемого типа - тоже Void. Эти типы соответствуют первому и третьему типам в <code>AsyncTask&lt;Void, Integer, Void&gt;</code>.</p>
<p>Метод doInBackground() перебирает массив и при каждой итерации уведомляет систему с помощью метода <span class="bb">publishProgress(item)</span>. Так 
как в нашем случае для индикации используются целые числа, то параметр <code>item</code> должен представлять целое число.</p>
<p>Метод <code>onProgressUpdate(Integer... items)</code> получает переданное выше число и применяет его для настройки текстового поля и прогрессбара.</p>
<p>Метод <code>onPostExecute()</code> выполняется после завершения задачи и в качестве параметра принимает объект, возвращаемый методом doInBackground() - то есть 
в данном случае объект типа Void. Чтобы сигнализировать окончание работы, здесь выводится на экран всплывающее сообщение.</p>
<p>Запустим приложение. Запустим задачу, нажав на кнопку:</p>
<img src="./pics/asynctask11.png" alt="AsyncTask в Android" />
<p>При этом пока выполняется задача, мы можем параллельно нажимать на вторую кнопку и увеличивать число кликов, либо выполнять какую-то другую работу в приложении.</p>
