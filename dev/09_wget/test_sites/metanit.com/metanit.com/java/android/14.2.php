...
<html >
<head>
<title>Java и Android | Добавление, удаление и обновление данных в SQLite</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление, удаление и обновление данных в SQLite в Android и Java с помощью SQLiteDatabase, класс ContentValues">
...
<h2>Добавление, удаление и обновление данных в SQLite</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>Продолжим работу с проектом из прошлой темы, где мы получаем данные. Теперь добавим в него стандартную CRUD-логику (создание, обновление, удаление).</p>
<p>Чтобы не нагромождать форму с главной activity, все остальные действия по работе с данными будут происходить на другом экране. Добавим в проект 
новый класс activity, который назовем UserActivity:</p>
<img src="./pics/sqlite7.png" alt="Добавление activity для SQLite в Android и Java" />
<p>В файле <span class="b">activity_user.xml</span> определим универсальую форму для добавления/обновления/удаления данных:</p>
<pre class="brush:xml;">
...
<p>И также изменим код <span class="b">UserActivity</span>:</p>
...
<p>При обновлении или удалении объекта из списка из главной activity в UserActivity будет передаваться id объекта:</p>
<pre class="brush:java;">
long userId=0;
//.................
Bundle extras = getIntent().getExtras();
...
<p>Если из MainActivity не было передано id, то устанавливаем его значение 0, следовательно, у нас будет добавление, а не редактирование/удаление</p>
<p>Если id определен, то получаем по нему из базы данных объект для редактирования/удаления:</p>
<pre class="brush:java;">
if (id &lt; 0) {
...
<p>Иначе просто скрываем кнопку удаления.</p>
<p>Для выполнения операций по вставке, обновлению и удалению данных SQLiteDatabase имеет методы <code>insert()</code>, <code>update()</code> и <code>delete()</code>. 
Эти методы вызываются в обработчиках кнопок:</p>
<pre class="brush:java;">db.delete(DatabaseHelper.TABLE, "_id = ?", new String[]{String.valueOf(id)});</pre>
<p>В метод delete() передается название таблицы, а также столбец, по которому происходит удаление, и его значение. В качестве критерия можно выбрать несколько 
столбцов, поэтому третьим параметром идет массив. Знак вопроса ? обозначает параметр, вместо которого подставляется значение из третьего параметра.</p>
<h3>ContentValues</h3>
<p>Для добавления или обновления нам надо создать объект <span class="b">ContentValues</span>. Данный объект представляет словарь, который содержит 
набор пар "ключ-значение". Для добавления в этот словарь нового объекта применяется метод <span class="b">put</span>. Первый параметр метода - это ключ, а второй - 
значение, например:</p>
<pre class="brush:java;">
ContentValues cv = new ContentValues();
cv.put("NAME", "Tom");
...
<p>В качестве значений в метод <code>put</code> можно передавать строки, целые числа, числа с плавающей точкой</p>
<p>В данном же случае добавляются введенные в текстовое поля значения:</p>
<pre class="brush:java;">
ContentValues cv = new ContentValues();
cv.put(DatabaseHelper.COLUMN_NAME, nameBox.getText().toString());
cv.put(DatabaseHelper.COLUMN_YEAR, Integer.parseInt(yearBox.getText().toString()));
</pre>
<p>При обновлении в метод update() передается название таблицы, объект ContentValues и критерий, по которому происходит обновление (в данном случае столбец id):</p>
<pre class="brush:java;">db.update(DatabaseHelper.TABLE, cv, DatabaseHelper.COLUMN_ID + "=" + userId, null);</pre>
<p>Метод insert() принимает название таблицы, объект ContentValues с добавляемыми значениями. Второй параметр является необязательным: он передает 
столбец, в который надо добавить значение NULL:</p>
<pre class="brush:java;">db.insert(DatabaseHelper.TABLE, null, cv);</pre>
<p>Вместо этих методов, как в прошлой теме, можно использовать метод <code>execSQL()</code> с точным указанием выполняемого sql-выражения. 
В то же время методы delete/insert/update имеют преимущество - они возвращают id измененной записи, по которому мы можем узнать об успешности операции, или -1 в случае неудачной операции:</p>
<pre class="brush:java;">
...
<p>После каждой операции выполняется метод <code>goHome()</code>, который возвращает на главную activity.</p>
<p>После этого нам надо исправить код MainActivity, чтобы она инициировала выполнение кода в UserActivity. Для этого изменим код 
<span class="b">activity_main.xml</span>:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>В данном случае была добавлена кнопка для вызова UserActivity.</p>
<p>И также изменим код класса <span class="b">MainActivity</span>:</p>
<pre class="brush:java;">
package com.example.sqliteapp;
...
<p>При нажатии на кнопку запускается UserActivity, при этом не передается никакого id, то есть в UserActivity id будет равен нулю, значит будет идти добавление данных:</p>
<pre class="brush:java;">
public void add(View view){
    Intent intent = new Intent(this, UserActivity.class);
    startActivity(intent);
...
<p>Другую ситуацию представляет обработчик нажатия на элемент списка - при нажатии также будет запускаться UserActivity, но теперь будет передаваться 
id выбранной записи:</p>
<pre class="brush:java;">
public void onItemClick(AdapterView&lt;?&gt; parent, View view, int position, long id) {
    Intent intent = new Intent(getApplicationContext(), UserActivity.class);
    intent.putExtra("id", id);
...
<p>Запустим приложение и нажмем на кнопку, которая должен перенаправлять на UserActivity:</p>
<img src="./pics/sqlite8.png" alt="Добавление в SQLite в Android и Java" />
<p>При нажатии в MainActivity на элемент списка этот элемент попадет на UserActivity, где его можно будет удалить или подредактировать:</p>
<img src="./pics/sqlite9.png" alt="Редактирование в SQLite в Android и Java" />
