...
<html >
<head>
<title>Java и Android | Первый проект в Android Studio</title>
<meta charset="utf-8" />
<meta name="description" content="Создание первого проекта приложения для ОС Android в Создание первого проекта в Android Studio, структура проекта и его основные части">
...
<h2>Первый проект в Android Studio</h2><div class="date">Последнее обновление: 14.10.2021</div>
...
<p>Теперь создадим первое приложение в среде Android Studio для операционной системы Android. Откроем Android Studio и на начальном экране выберем пункт 
<span class="b">New Project</span>:</p>
<img src="./pics/1.8.png" alt="Первый проект в Android Studio" />
<p>При создании проекта Android Studio вначале предложит нам выбрать шаблон проекта:</p>
<img src="./pics/android_activity3.png" alt="Выбор Empty Activity в Android Studio" />
<p>Android Studio предоставляет ряд шаблонов для различных ситуаций. Выберем в этом списке шаблон <span class="b">Empty Activity</span>, 
который предосавляет самый простейший фукционал, необходимый для начала, и нажмем на кнопку <span class="b">Next</span>.</p>
<p>После этого отобразится окно настроек нового проекта:</p>
<img src="./pics/androidprojectsettings3.png" alt="Настройка проекта в Android Studio" />
<p>В окне создания нового проекта мы можем установить его начальные настройки:</p>
<ul>
...
<li><p>В поле <span class="b">Language</span> в качестве языка программирования укажем <span class="b">Java</span> (будьт внимательны, так как по умолчанию в этом поле стоит Kotlin)</p></li>
...
<img src="./pics/androidstudio3.png" alt="Новый проект в Android Studio" />
<p>Вначале вкратце рассмотрим структуру проекта, что он уже имеет по умолчанию</p>
<img src="./pics/1.9.png" alt="Структура проекта в Android Studio" />
<p>Проект Android может состоять из различных модулей. По умолчанию, когда мы создаем проект, создается один модуль - <span class="b">app</span>. Модуль имеет три подпапки:</p>
<ul>
...
<li><p><span class="b">java</span>: хранит файлы кода на языке java, которые структурированы по отдельным пакетам. Так, в папке <code>com.example.helloapp</code> (название 
которого было указано на этапе создания проекта) имеется по умолчанию файл <span class="b">MainActivity.java</span> с кодом на языке Java, который представляет класс 
MainActivity, запускаемый по умолчанию при старте приложения</p></li>
...
<ul>
	...
	<li><p>папка <span class="b">layout</span> предназначена для хранения файлов, определяющих графический интерфейс. 
	По умолчанию здесь есть файл <span class="b">activity_main.xml</span>, который определяет интерфейс для класса MainActivity в виде xml</p></li>
...
<p>Отдельный элемент <span class="b">Gradle Scripts</span> содержит ряд скриптов, которые используются при построении приложения.</p>
<p>Во всей этой структуре следует выделить файл MainActivity.java, который открыт в Android Studio и который содержит логику приложения и собственно с него начинается выполнение 
приложения. И также выделим файл <span class="b">activity_main.xml</span>, который определяет графический интерфейс - по сути то, что увидит пользователь 
на своем смартфоне после загрузки приложения.</p>
<h3>Возможные проблемы</h3>
<p>Для создания приложения используется Java. А для построения приложения применяется инфраструктура Gradle. Однако текущая используемая версия 
...
<span class="b">Unsupported class file major version 61</span>:</p>
<img src="./pics/1.17.png" alt="Unsupported class file major version 61 в Android Studio"/>
<p>Эта ошибка говорит о том, что версия JDK 17 несовместима с текущей версией Gradle. И надо использовать меньшую версию.</p>
<p>Для решения этой проблемы перейдем в студии к меню <span class="b">File -&gt;Settings</span> (на MacOS это пункт <span class="b">Android Studio -&gt; Preferences</span>)</p>
<img src="./pics/1.18.png" alt="Unsupported class file major version в Android Studio"/>
<p>Затем в открывшемся окне настроек перейдем к пункту меню <span class="b">Build, Execution, Deployment -&gt; Build Tools -&gt; Gradle</span> 
и далее найдем поле <span class="b">Gradle JDK</span>, где изменим версию JDK. Она должна иметь версию 11 и выше. 
Как правило, вместе с Android Studio устанавливается и поддерживаемая версия JDK - на данный момент это JDK 11. И ее можно выбрать в списке JDK:</p>

<img src="./pics/1.19.png" alt="Установка версии JDK в Android Studio"/>
<p>Наиболее оптимальный пункт для выбора версий JDK, которая идет вместе с Android Studio, называется <span class="b">Embedded JDK version...</span>. 
Как видно на скриншоте, это версия 11, но при последующих обновлениях Android Studio эта версия может измениться.</p>
<p>После сделанных изменений сначала нажмем на кнопку <span class="b">Apply</span>, а затем на кнопку <span class="b">OK</span>. И повторим запуск проекта.</p>
<h3>Запуск проекта</h3>
<p>Созданный выше проект уже содержит некоторый примитивный функционал. Правда, этот функционал почти ничего не делает, только выводит на 
экран строку "Hello world!". Тем не менее это уже фактически приложение, которое мы можем запустить.</p>
<p>Для запуска и тестирования приложения мы можем использовать эмуляторы или реальные устройства. Но в идеале лучше тестировать на реальных устройствах. К тому же эмуляторы требуют 
больших аппаратных ресурсов, и не каждый компьютер может потянуть требования эмуляторов. А для использования мобильного устройства для тестирования может потребоваться разве что установить необходимый драйвер.</p>
<h4>Режим разработчика на телефоне</h4>
<p>По умолчанию опции разработчика на смартфонах скрыты. Чтобы сделать их доступными, надо зайти в <span class="b">Settings &gt; About phone (Настройки &gt; О телефоне)</span> 
(в Android 8 это в <span class="b">Settings &gt; System &gt; About phone (Настройки &gt; Система &gt; О телефоне)</span>) и семь 
раз нажать <span class="b">Build Number (Номер сборки)</span>.</p>
<img src="./pics/1.11.png" alt="Включение параметров разработчика на Android" />

<p>Теперь необходимо включить отладку по USB. Для этого перейдем в 
<span class="b">Settings &gt; System &gt; Advanced &gt; Developer options</span> или 
<span class="b">Настройки &gt; Система &gt; Дополнительно &gt; Для разработчиков</span> 
(в Android 8 это в <span class="b">Settings &gt; System &gt; Developer options</span> или <span class="b">Настройки &gt; Система &gt; Для разработчиков</span> ).</p>
<img src="./pics/1.12.png" alt="Параметры разработчика на Android" />
<p>И включим возможность отладки по USB:</p>
<img src="./pics/1.13.png" alt="Отладка по USB на Android" />
<h4>Запуск приложения</h4>
<p>Подключим устройство с ОС Android (если мы тестируем на реальном устройстве) и запустим проект, нажав на зеленую стрелочку на панели инструментов.</p>
<img src="./pics/1.5.png" alt="Запуск приложения в Android Studio"/>
<p>Выберем устройство и нажмем на кнопку OK. И после запуска мы увидим наше приложение на экране устройства:</p>
