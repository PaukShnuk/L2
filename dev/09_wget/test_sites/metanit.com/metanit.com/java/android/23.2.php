...
<html >
<head>
<title>Java и Android | Получение xml по сети</title>
<meta charset="utf-8" />
<meta name="description" content="Получение xml по сети в приложении с помощью HttpsURLConnection на Android и Java и парсинг с помощью класса XmlPullParser">
...
<h2>Получение xml по сети</h2><div class="date">Последнее обновление: 18.10.2021</div>
...
<p>Рассмотрим получение данных в формате xml по сети. Допустим, на некотором сайте https://example.com находится файл <span class="b">users.xml</span> со следующим содержимым:</p>
<pre class="brush:xml;">
...
<p>То есть сам файл доступен по адресу https://example.com/users.xml. Но это необязательно должен быть именно файл, это может быть любой ресурс, который 
динамически генерирует данные в xml.</p>
<p>Возьмем стандартный проект Android и вначале определим в нем класс <span class="b">User</span>, который будет представлять загружаемые данные:</p>
...
<p>Далее определим класс <span class="b">UserXmlParser</span>:</p>
<pre class="brush:java;">
package com.example.xmlapp;
...
<p>То есть в итоге получится следующий проект:</p>
<img src="./pics/xml3.png" alt="Парсинг xml из сети в Android" />
<p>Для парсинга xml здесь используется класс <span class="b">XmlPullParser</span>, который уже рассматривался в прошлой теме. Единственное отличие заключается в том, 
что для создания объекта этого класса применяется класс <span class="b">XmlPullParserFactory</span>:</p>
<pre class="brush:java;">
XmlPullParserFactory factory = XmlPullParserFactory.newInstance();
XmlPullParser xpp = factory.newPullParser();
</pre>
<p>Для работы определим простейший визуальный интефейс в файле <span class="b">activity_main.xml</span>:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>Здесь определен элемент TextView для отображения некоторой дополнительной информации о состоянии загрузки файла и элемент ListView для отображения загруженных 
объектов.</p>
<p>Далее изменим класс <span class="b">MainActivity</span>:</p>
<pre class="brush:java;">
package com.example.xmlapp;
...
<p>При создании MainActivity будет запускаться дополнительный поток, который вызывает метод <code>download()</code>. Этот метод с помощью 
...
вместо применяется класса HttpsURLConnection класс <code>java.net.HttpURLConnection</code>).</p>
<pre class="brush:java;">
String content = download("https://example.com/users.xml");
</pre>
<p>Затем загруженное содержимое передается в метод <code>parse()</code>, класса UserXmlParser, который формирует список объектов.</p>
<pre class="brush:java;">
UserXmlParser parser = new UserXmlParser();
if(parser.parse(content)){
	//...................
</pre>
<p>Затем загруженный список передается в адаптер ArrayAdapter, а через него в ListView для отображения на экране устройства:</p>
<pre class="brush:java;">
ArrayAdapter&lt;User&gt; adapter = new ArrayAdapter(getBaseContext(), android.R.layout.simple_list_item_1, parser.getUsers());
usersList.setAdapter(adapter);
</pre>
<p>В завершении надо добавить в файл манифеста <span class="b">AndroidManifest.xml</span> разрешения на взаимодействие с сетью:</p>
<pre class="brush:xml;">&lt;uses-permission android:name="android.permission.INTERNET"/&gt;</pre>
<p>И после запуска приложения в окне Logcat мы увидим полученные с сервера данные:</p>
<img src="./pics/xml6.png" alt="XmlPullParser и парсинг xml-файла и HttpsURLConnection в Android и Java" />
...
<div class="nav"><p><a href="./23.1.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
