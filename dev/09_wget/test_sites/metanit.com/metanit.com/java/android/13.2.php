...
<html >
<head>
<title>Java и Android | Размещение файлов во внешнем хранилище</title>
<meta charset="utf-8" />
<meta name="description" content="Размещение файлов во внешнем хранилище в Android и Java, метод getExternalFilesDir">
...
<h2>Размещение файлов во внешнем хранилище</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>В прошлой теме мы рассмотрели сохранение и чтение файлов из каталога приложения. По умолчанию такие файлы доступны только самому приложения. 
Однако мы можем помещать и работать с файлами из внешнего хранилища приложения. Это также позволит другим программам открывать данные файлы и при необходимости изменять.</p>
<p>Весь механизм работы с файлами будет таким же, как и при работе с хранилищем приложения. Ключевым отличием здесь будет получение и использование пути к 
внешнему хранилищу через метод <span class="b">getExternalFilesDir()</span> класса Context.</p>

<p>Итак, пусть в файле <span class="b">activity_main.xml</span> будет такая же разметка интерфейса:</p>
<pre class="brush:xml;">
...
<p>А код класса <span class="b">MainActivity</span> будет выглядеть следующим образом:</p>
...
<p>С помощью выражения <code>getExternalFilesDir(null)</code> получаем доступ к папке приложения во внешнем хранилище и устанавливаем объект файла:</p>
<pre class="brush:java;">
private File getExternalPath() {
    return new File(getExternalFilesDir(null), FILE_NAME);
}
</pre>
<p>В качестве параметра передается тип папки, но в данном случае он нам не важен, поэтому передается значение <code>null</code></p>
<p>Все остальные действия по записи/чтению файла будут такими же, как и в предыдущей теме в случае с работой с локальной папкой файла.</p>
<img src="./pics/filesystem2.png" alt="Запись файла во внешнем хранилищ в Android и Java" />
<p>И после операции записи на смартфоне через <span class="b">Device File Explorer</span> мы сможем увидеть созданный файл в папке <span class="b">storage/self/primary/Android/data/[название_пакета]/files</span>:</p>
<img src="./pics/filesystem4.png" alt="Файл во внещнем хранилище в Device File Explorer в Android Studio" />
...
<div class="nav"><p><a href="./13.1.php">Назад</a><a href="./">Содержание</a><a href="./14.5.php">Вперед</a></p></div></div>
<div class="socBlock">
