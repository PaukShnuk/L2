...
<html >
<head>
<title>Java и Android | Жизненный цикл фрагментов</title>
<meta charset="utf-8" />
<meta name="description" content="Жизненный цикл фрагментов в Android и Java, добавление фрагмента в Android Studio, методы onCreate, onCreateView и onViewCreated">
...
<h2>Жизненный цикл фрагментов</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>Каждый класс фрагмента наследуется от базового класса Fragment и имеет свой жизненный цикл, состоящий из ряда этапов:</p>
<img src="./pics/fragment_lyfecycle.png" alt="Жизненный цикл фрагментов в Android" />
<p>Каждый этап жизненного цикла описывается одной из констант перечисления <span class="b">Lifecycle.State</span>:</p>
<ul>
<li><p><span class="b">INITIALIZED</span></p></li>
<li><p><span class="b">CREATED</span></p></li>
<li><p><span class="b">STARTED</span></p></li>
<li><p><span class="b">RESUMED</span></p></li>
<li><p><span class="b">DESTROYED</span></p></li>
</ul>
<p>Стоит отметить, что представление фрагмента (его визуальный интерфейс или View) имеет отдельный жизненный цикл.</p>
...
<ul>
<li><p>При создании фрагмент находится в состоянии <span class="b">INITIALIZED</span>. Чтобы фрагмент прошел все остальные этапы жизненного цикла, фрагмент необходимо передать 
в объект <span class="b">FragmentManager</span>, который далее определяет состояние фрагмента и переводит фрагмент из одного состояния в другое.</p></li>
<li><p>Когда фрагмент добавляется в FragmentManager и прикрепляется к определенному классу Activity, у фрагмента вызывается метод <span class="b">onAttach()</span>. 
Данный метод вызывается до всех остальных методов жизненного цикла. После этого фрагмент переходит в состояние <span class="b">CREATED</span></p></li>
<li><p><span class="b">onCreate()</span>: происходит создание фрагмента. В этом методе мы можем получить ранее сохраненное состояние фрагмента 
через параметр метода <code>Bundle savedInstanceState</code>. (Если фрагмент создается первый раз, то этот объект имеет значение <code>null</code>) 
Этот метод вызывается после вызова соответствующего метода onCreate() у activity.</p>
<pre class="brush:java;">public void onCreate(Bundle savedInstanceState)</pre>
</li>
<li><p><span class="b">onCreateView()</span>: фрагмент создает представление (View или визуальный интерфейс). В этом методе мы можем установить, какой именно изуальный интерфейс будет использовать 
фрагмент. При выполнении этого метода представление фрагмента переходит в состояние <span class="b">INITIALIZED</span>. А сам фрагмент 
все еще находится в состоянии <span class="b">CREATED</span></p>
<pre class="brush:java;">
public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
</pre>
<p>Первый параметр - объект <span class="b">LayoutInflater</span> позволяет получить содержимое ресурса layout и передать его во фрагмент.</p> 
<p>Второй параметр - объект <span class="b">ViewGroup</span> представляет контейнер, в которой будет загружаться фрагмент.</p>
<p>Третий параметр - объект <span class="b">Bundle</span> представляет состояние фрагмента. (Если фрагмент загружается первый раз, то равен null)</p>
<p>На выходе метод возвращает созданное с помощью <span class="b">LayoutInflater</span> представление в виде объекта View - собственно представление фрагмента</p>
</li>
<li><p><span class="b">onViewCreated()</span>: вызывается после создания представления фрагмента.</p>
<pre class="brush:java;">
public void onViewCreated (View view,  Bundle savedInstanceState)
</pre>
<p>Первый параметр - объект <span class="b">View</span> - представление фрагмента, которое было создано посредством метода onCreateView.</p> 
<p>Второй параметр - объект <span class="b">Bundle</span> представляет состояние фрагмента. (Если фрагмент загружается первый раз, то равен null)</p>
</li>
<li><p><span class="b">onViewStateRestored()</span>: получает состояние представления фрагмента. После выполнения этого метода представление фрагмента переходит в состояние 
<span class="b">CREATED</span></p>
<pre class="brush:java;">
public void onViewStateRestored (Bundle savedInstanceState)
</pre>
</li>
<li><p><span class="b">onStart()</span>: вызывается, когда фрагмент становится видимым и вместе с представлением переходит в состояние <span class="b">STARTED</span></p>
<pre class="brush:java;">
public void onStart ()
</pre>
</li>
<li><p><span class="b">onResume()</span>: вызывается, когда фрагмент становится активным, и пользователь может с ним взаимодействовать. 
При этом фрагмент и его представление переходят в состояние <span class="b">RESUMED</span></p>
<pre class="brush:java;">
public void onResume ()
</pre>
</li>
<li><p><span class="b">onPause()</span>: фрагмент продолжает оставаться видимым, но уже не активен и вместе с представлением переходит в состояние <span class="b">STARTED</span></p>
<pre class="brush:java;">
public void onPause ()
</pre>
</li>
<li><p><span class="b">onStop()</span>: фрагмент больше не является видимым и вместе с представлением переходит в состояние <span class="b">CREATED</span></p>
<pre class="brush:java;">
public void onStop ()
</pre>
<p>На этом этапе жизненного цикла мы можем сохранить состояние фрагмента с помощью метода <span class="b">onSaveInstanceState()</span>. Однако стоит учитывать, 
что вызов этого метода зависит от версии API. До API 28 <code>onSaveInstanceState()</code> вызывается до <code>onStop()</code>, 
а начиная API 28 после <code>onStop()</code>.</p>
</li>
<li><p><span class="b">onDestroyView()</span>: уничтожается представление фрагмента. Представление переходит в состояние <span class="b">DESTROYED</span></p></li>
<li><p><span class="b">onDestroy()</span>: окончательно уничтожение фрагмента - он также переходит в состояние <span class="b">DESTROYED</span></p></li>
<li><p>Метод <span class="b">onDetach()</span> вызывается, когда фрагмент удаляется из FragmentManager и открепляется от класса Activity. 
Этот метод вызывается после всех остальных методов жизненного цикла.</p></li>
</ul>
<p>В коде класса фрагмента мы можем переопределить все или часть из этих методов. Например, пусть у нас будет определен следующий проект:</p>
<img src="./pics/fragment_creation.png" alt="Fragment Lifecycle жизненный цикл фрагмента в Android Studio и Java" />
<p>В каталоге <span class="b">res/layout</span> определен файл layout для фрагмента - <span class="b">fragment_content.xml</span> со следующей разметкой:</p>
<pre class="brush:xml;">
...
<p>Класс фрагмента использует данный файл для установки представления, а также определяет методы для управления жизненным циклом:</p>
<pre class="brush:java;">
...
<p>В отличие от прошлой статьи, где рассматривалось создание фрагмента, здесь фрагмент устанавливает представление в методе 
<span class="b">onCreateView</span>. Для этого в метод <code>inflate()</code> объекта LayoutInflater передается идентификатор ресурса layout и контейнер - объект ViewGroup, 
в который будет загружаться фрагмент. В итоге метод <code>inflate()</code> возвращает созданное представление.</p>
<pre class="brush:java;">return inflater.inflate(R.layout.fragment_content, container, false);</pre>
<p>При выполнении метода <span class="b">onViewCreated()</span> представление уже создано и оно передается в качестве первого параметра - объекта 
View, через который с помощью идентификаторов мы можем получить визуальные элементы - TextView и Button, которые определены в представлении.</p>
<p>Для остальных методов жизненного цикла установлено простое логгирование с помощью метода <code>Log.d()</span>.</p>
<p>Пусть в файле <span class="b">activity_main.xml</span> происходит добавление фрагмента:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>И класс <span class="b">MainActivity</span>:</p>
<pre class="brush:java;">
package com.example.fragmentapp;
...
<p>Если мы запустим проект, то на экране устройства мы увидим визуальный интерфейс, определенный для фрагмента.</p>
<img src="./pics/fragment_creation2.png" alt="Жизненный цикл фрагмента в Android Studio и Java, onViewCreated" />
<p>А в окне <span class="b">Logcat</span> в Android Studio можно будет наблюдать логгирование методов жизненного цикла</p>
<img src="./pics/fragment_creation3.png" alt="Методы и состояния хизненного цикла фрагмента в Android Studio и Java" />
...
<div class="nav"><p><a href="./8.1.php">Назад</a><a href="./">Содержание</a><a href="./8.5.php">Вперед</a></p></div></div>
<div class="socBlock">
