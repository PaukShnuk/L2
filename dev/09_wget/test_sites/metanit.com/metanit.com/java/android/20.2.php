...
<html >
<head>
<title>Java и Android | Добавление контактов</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление контактов в Android и Java, WRITE_CONTACTS">
...
<h2>Добавление контактов</h2><div class="date">Последнее обновление: 18.10.2021</div>
...
<p>Продолжим работу с проектом из прошлой темы и добавим в него возможность добавления новых контактов. 
Добавление контактов представляет собой запрос на изменение списка контактов, то есть его запись. Поэтому нам надо установить соответствующее разрешение 
в файле манифеста. Возьмем проект из прошлой темы и добавим в него в файл <span class="b">AndroidManifest.xml</span> разрешение <span class="b">android.permission.WRITE_CONTACTS</span>:</p>
<pre class="brush:xml;">
...
<p>Для добавления контакта добавим изменим файл <span class="b">activity_main.xml</span>, определив в нем текстовое поле для ввода данных:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>В коде <span class="b">MainActivity</span> пропишем обработчик <code>onAddContact</code> с добавлением контакта:</p>
...
<p>Сразу стоит отметить, что для работы с контактами не надо отдельно получать разрешения на чтение и отдельно на изменение контактов. 
Пользователь один раз согласие для установки сразу двух разрешений. Однако на уровне кода нам необходимо перечислить через запятую устанавливаемые разрешения:</p>
<pre class="brush:java;">
 // вызываем диалоговое окно для установки разрешений
...
<p>Однако мы опять же можем управлять разрешением, например, установить доступность кнопки:</p>
<pre class="brush:java;">addBtn.setEnabled(READ_CONTACTS_GRANTED);</pre>
<p>Если разрешение не получено, то переменная READ_CONTACTS_GRANTED будет иметь значение false, и соответственно кнопка будет недоступна, и мы не сможем добавить 
новый контакт.</p>
<p>Весь код добавления находится в обработчике нажатия кнопки <code>onAddContact</code>. В Android контакты 
...
добавить не можем, но это и не нужно.</p>
<p>Данные контакта представляют объект <span class="b">ContentValues</span>, который состоит из ключей и их значений, то есть объект словаря. После его создания 
происходит добавление в него пары элементов:</p>
<pre class="brush:java;">
contactValues.put(RawContacts.ACCOUNT_NAME, newContact);
contactValues.put(RawContacts.ACCOUNT_TYPE, newContact);
</pre>
<p>Здесь устанавливается название и тип контакта. В качестве ключей выставляются значения <code>RawContacts.ACCOUNT_NAME</code> и 
<code>RawContacts.ACCOUNT_TYPE</code>, а в качестве их значения - текст из текстового поля.</p>
<p>Далее этот объект добавляется в таблицу RawContacts с помощью метода <code>insert()</code>:</p>
<pre class="brush:java;">
Uri newUri = getContentResolver().insert(RawContacts.CONTENT_URI, contactValues);
</pre>
<p>Метод insert() возвращает URI - ссылку на добавленный объект в таблице, у которого мы можем получить id. Затем после очистки мы подготавливаем объект 
для доабвления в таблицу Data, вновь наполняя его данными:</p>
<pre class="brush:java;">
...
<p>И опять добавление производит метод insert():</p>
<pre class="brush:java;">
getContentResolver().insert(Data.CONTENT_URI, contactValues);
</pre>
<p>Перед запуском, если ранее (в прошлой теме) приложение было установлено, то его необходимо удалить, чтобы установить для приложения новые разрешения (разрешение на запись контактов).</p>
<p>Запустим приложение и добавим новый контакт:</p>
<img src="./pics/addcontact1.png" alt="Новый контакт в Android" />
