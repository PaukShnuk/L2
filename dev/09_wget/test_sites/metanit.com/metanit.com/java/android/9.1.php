...
<html >
<head>
<title>Java и Android | Перелистывание страниц и ViewPager2</title>
<meta charset="utf-8" />
<meta name="description" content="Перелистывание страниц и ViewPager2 в Android и Java, PagerAdapter, PageFragment, FragmentStateAdapter">
...
<h1>Перелистывание страниц и ViewPager2</h1><h2>ViewPager2 и разделение приложения на страницы</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>Нередко можно встретить приложения, которые реализуют систему перелистывания, а само приложение предстает в виде набора страниц, 
которые можно пролистывать влево и вправо. В приложении Android для создания подобного эффекта можно использовать элемент <span class="b">ViewPager2</span> 
из комплекта JetPack. Для создания эффекта страниц ViewPager2 использует фрагменты.</p>
<p>Итак, создадим новый проект. Добавим в папку <span class="b">res/layout</span> файл разметки для фрагмента, который будет представлять страницу. 
Назовем его <span class="b">fragment_page.xml</span> и определим в нем следующий код:</p>
<pre class="brush:xml;" >
...
<p>Фрагмент будет отображать текстовое поле с номером страницы.</p>
<p>Теперь добавим в проект сам класс фрагмента. Назовем его <span class="b">PageFragment</span>:</p>
...
<p>Переменная <code>pageNumber</code> указывает на номер текущей страницы. Номер страницы будет передаваться извне через фабричный метод 
<code>newInstance()</code>. Передача номера происходит путем добавления значения в аргумент "num"</p>
<p>Затем при создании фрагмента в методе <code>onCreate()</code> этот номер будет извлекаться из аргумента "num" (если аргументы определены):</p>
<pre class="brush:java;">pageNumber = getArguments() != null ? getArguments().getInt("num") : 1;</pre>
<p>В методе <code>onCreateView()</code> полученный номер страницы будет отображаться в текстовом поле.</p>
<p>Сам по себе фрагмент еще не создает функциональность постраничной навигации. Для этого нам нужен один из классов <span class="b">PagerAdapter</span>. 
Android SDK содержит ряд встроенных реализаций PagerAdapter, в частности, класс <span class="b">FragmentStateAdapter</span>. 
Этот класс являются абстрактным, поэтому напрямую мы его использовать не можем, и нам нужно создать класс-наследник. Для этого добавим в 
проект новый класс, который назовем <span class="b">MyAdapter</span>:</p>
<pre class="brush:java;">
package com.example.viewpagerapp;
...
<p>Класс FragmentStateAdapter определяет два метода:</p>
<ul>
<li><p><code>int getItemCount()</code>: возвращает количество страниц, которые будут в ViewPager2 (в нашем случае 10)</p></li>
<li><p><code>Fragment createFragment(int position)</code>: по номеру страницы, передаваемому в качестве параметра position, возвращает объект фрагмента</p></li>
</ul>
<p>Стоит отметить, что в качестве параметра конструктор FragmentStateAdapter принимает контекст выполнения - обычно это объект FragmentActivity, но также это 
может быть объект Fragment</p>
<p>В завершении установим в файле <span class="b">activity_main.xml</span> элемент ViewPager2:</p>
<pre class="brush:xml;">
...
<p>Класс MainActivity наследуется от AppCompatActivity - класса, который в свою очередь наследуется от FragmentActivity, и поэтому ее текущий объект мы можем передать в качестве параметра в конструктор MyAdapter (а через него - в конструктор 
FragmentStateAdapter). Чтобы перелистывание заработало, для ViewPager2 устанавливается адаптер MyAdapter.</p>
<p>И запустив проект, мы сможем с помощью перелистывания перемещаться по страницам:</p>
<img src="./pics/pageviewer1.png" alt="Перелистывание на Android и Java и ViewPager2" />
...
<div class="nav"><p><a href="./14.6.php">Назад</a><a href="./">Содержание</a><a href="./9.2.php">Вперед</a></p></div></div>
<div class="socBlock">
