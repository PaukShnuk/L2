...
<html >
<head>
<title>Java и Android | Создание интерфейса в коде java</title>
<meta charset="utf-8" />
<meta name="description" content="Создание графического интерфейса в приложении Android, иерархия визуальных элементов, создание интерфейса в xml и коде java">
...
<h1>Основы создания интерфейса</h1><h2>Введение в создание интерфейса</h2><div class="date">Последнее обновление: 14.11.2021</div>
...
<p>Графический интерфейс пользователя представляет собой иерархию объектов <span class="b">android.view.View</span> и <span class="b">android.view.ViewGroup</span>. 
Каждый объект ViewGroup представляет контейнер, который содержит и упорядочивает дочерние объекты <span class="b">View</span>. В частности, к контейнерам относят 
такие элементы, как RelativeLayout, LinearLayout, GridLayout, ConstraintLayout и ряд других.</p>
<p>Простые объекты <span class="b">View</span> представляют собой элементы управления и прочие виджеты, например, кнопки, текстовые поля и т.д., через которые пользователь взаимодействует с программой:</p>
<img src="./pics/viewgroup.png" alt="Иерархия визуальных компонентов" />
<p>Большинство визуальных элементов, наследующихся от класса View, такие как кнопки, текстовые поля и другие, располагаются в пакете <code>android.widget</code></p>
<p>При определении визуального у нас есть три стратегии:</p>
<ul>
<li><p>Создать элементы управления программно в коде java</p></li>
<li><p>Объявить элементы интерфейса в XML</p></li>
<li><p>Сочетание обоих способов - базовые элементы разметки определить в XML, а остальные добавлять во время выполнения</p></li>
</ul>
<p>Сначала рассмотрим первую стратегию - определение интерейса в коде Java.</p>
<h2>Создание интерфейса в коде java</h2>
<p>Для работы с визуальными элементами создадим новый проект. В качестве шаблона проекта выберем <span class="b">Empty Activity</span>:</p>
<img src="./pics/3.2.png" alt="Empty Activity в Android Studio" />
<p>Пусть он будет называться ViewsApp:</p>
<img src="./pics/3.1.png" alt="Графическое приложение в Android" />
<p>И после создания проекта два основных файла, которые будут нас интересовать при создании визуального интерфейса - это класс <span class="b">MainActivity</span> и определение интерфейса 
для этой activity в файле <span class="b">activity_main.xml</span>.</p>
<img src="./pics/3.4.png" alt="MainActivity и activity_main.xml в Android Studio" />

<p>Определим в классе <span class="b">MainActivity</span> простейший интерфейс:</p>
...
<p>При создании виджетов в коде Java применяется их конструктор, в который передается контекст данного виджета, а точнее объект <span class="b"> android.content.Context</span>, 
в качестве которого выступает текущий класс MainActivity.</p>
<pre class="brush:java;">TextView textView = new TextView(this);</pre>
<p>Здесь весь интерфейс представлен элементом TextView, которое предназначено для выводa текста. С помощью методов, которые, как правило, начинаются на 
<span class="b">set</span>, можно установить различные свойства TextView. Например, в данном случае метод <code>setText()</code> устанавливает текст в поле, 
а <code>setTextSize()</code> задает высоту шрифта.</p>
<p>Для установки элемента в качестве интерфейса приложения в коде Activity вызывается метод <span class="b">setContentView()</span>, в который передается визуальный элемент.</p>
<p>Если мы запустим приложение, то получим следующий визуальный интерфейс:</p>
<img src="./pics/3.5.png" alt="Интерфейс в коде java в Android Studio" />
<p>Подобным образом мы можем создавать более сложные интерейсы. Например, TextView, вложенный в ConstraintLayout:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>Для каждого контейнера конкретные действия по добавлению и позиционированию в нем элемента могут отличаться. В данном случае контейнеров выступает класс ConstraintLayout, 
поэтому для определения позиционирования и размеров элемента необходимо создать объект <span class="b">ConstraintLayout.LayoutParams</span>.  (Для LinearLayout 
...
<span class="b">ViewGroup.LayoutParams.WRAP_CONTENT</span>, которая устанавливает размеры элемента, необходимые для размещения а экране его содержимого.</p>
<p>Далее определяется позиционирование. В зависимости от типа контейнера набор устанавливаемых свойств может отличаться. Так, строка кода</p>
<pre class="brush:java;">layoutParams.leftToLeft = ConstraintLayout.LayoutParams.PARENT_ID;</pre>
<p>указывает, что левая граница элемента будет выравниваться по левой ганице контейнера.</p>
<p>А строка кода</p>
<pre class="brush:java;">layoutParams.topToTop = ConstraintLayout.LayoutParams.PARENT_ID;</pre>
<p>указывает, что верхняя граница элемента будет выравниваться по верхней ганице контейнера. В итоге элемент будет размещен в левом верхнем углу ConstraintLayout.</p>
<p>Для установки всех этих значений для конкретного элемента (TextView) в его метод <code>setLayoutParams()</code> передается объект <span class="b">ViewGroup.LayoutParams</span> (или один из его наследников, 
например, ConstraintLayout.LayoutParams).</p>
<pre class="brush:java;">textView.setLayoutParams(layoutParams);</pre>
<p>Все классы контейнеров, которые наследуются от <span class="b">android.view.ViewGroup</span> (RelativeLayout, LinearLayout, GridLayout, ConstraintLayout и т.д.), 
имеют метод <code>void addView(android.view.View child)</code>, который позволяет добавить в контейнер другой элемент - обычный виджет типа TextView или другой контейнер. 
И в данном случае посредством данного метода TextView добавляется в ConstraintLayout:</p>
<pre class="brush:java;">constraintLayout.addView(textView);</pre>
<p>Опять же отмечу, что для конкретного контейнера конкретные действия могут отличаться, но как правило для всех характерно три этапа:</p>
<ul>
<li><p>Создание объекта ViewGroup.LayoutParams и установка его свойств</p></li>
<li><p>Передача объекта ViewGroup.LayoutParams в метод <code>setLayoutParams()</code> элемента</p></li>
<li><p>Передача элемента для добавления в метод <code>addView()</code> объекта контейнера</p></li>
</ul>
<p>Хотя мы можем использовать подобный подход, в то же время более оптимально определять визуальный интерейс в файлах xml, а всю связанную логику определять в классе 
activity. Тем самым мы достигнем разграничения интерфейса и логики приложения, их легче будет разрабатывать и впоследствии модифицировать. И в следующей теме мы это рассмотрим.</p>
...
<div class="nav"><p><a href="./1.3.php">Назад</a><a href="./">Содержание</a><a href="./2.8.php">Вперед</a></p></div></div>
<div class="socBlock">
