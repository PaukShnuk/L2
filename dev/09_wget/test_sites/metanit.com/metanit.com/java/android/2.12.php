...
<html >
<head>
<title>Java и Android | Передача данных между Activity. Сериализация</title>
<meta charset="utf-8" />
<meta name="description" content="Передача данных между Activity в Android и Java, сериализация и интерфейс Serializable">
...
<h2>Передача данных между Activity. Сериализация</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>Для передачи данных между двумя Activity используется объект <span class="b">Intent</span>. Через его метод <span class="b">putExtra()</span> 
можно добавить ключ и связанное с ним значение.</p>
<p>Например, передача из текущей activity в SecondActivity строки "Hello World" с ключом "hello":</p>
...
<p>Для передачи данных применяется метод <span class="b">putExtra()</span>, который в качестве значения позволяет передать данные простейших типов - String, int, float, double, long, short, byte, 
char, массивы этих типов, либо объект интерфейса Serializable.</p>
<p>Чтобы получить отправленные данные при загрузке SecondActivity, можно воспользоваться методом <span class="b">get()</span>, в который передается ключ объекта:</p>
<pre class="brush:java;">
Bundle arguments = getIntent().getExtras();
String name = arguments.get("hello").toString();	// Hello World
</pre>
<p>В зависимости от типа отправляемых данных при их получении мы можем использовать ряд методов объекта Bundle. Все они в качестве параметра принимают 
ключ объекта. Основные из них:</p>
<ul>
...
<p>Пусть у нас в проекте будет определено две activity: MainActivity и SecondActivity.</p>
<p>В коде SecondActivity определим получение данных:</p>
<pre class="brush:java;">
...
<p>В данном случае в SecondActivity получаем все данных из объекта Bundle и выводим их в текстовое поле TextView. Предполагается, что данной activity 
будут передаваться три элемента - две строки с ключами name и company и число с ключом price.</p>
<p>Теперь определим передачу в SecondActivity данных. Например, определим для MainActivity следующий интерфейс в файле <span class="b">activity_main.xml</span>:</p>
<pre class="brush:xml;">
...
<p>Здесь определены три текстовых поля для ввода данных и кнопка.</p>
<p>В классе MainActivity определим следующее содержимое:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>В обработчике нажатия кнопки получаем введенные в текстовые поля EditText данные и передаем их в объект Intent с помощью метода <code>putExtra()</code>. 
Затем запускаем SecondActivity.</p>
<p>В итоге при нажатии на кнопку запустится SecondActivity, которая получит некоторые введенные в текстовые поля данные.</p>
<img src="./pics/activity3.png" alt="putExtra in Android и getString in Activity in Android Java" />
<h3>Передача сложных объектов</h3>
<p>В примере выше передавались простые данные - числа, строки. Но также мы можем передавать более сложные данные. В этом случае используется механизм сериализации. 
Для этого нажмем на папку пакета, где находятся классы MainActivity и SecondActivity, правой кнопкой мыши и в контекстном меню выберем 
<span class="b">New -&gt; Java Class</span>:</p>
<img src="./pics/activity4.png" alt="Добавление класса Java в Android Studio" />
<p>Назовем новый класс <span class="b">User</span> - пусть он будет представлять пользователя.</p>
<img src="./pics/activity5.png" alt="Добавление класса Java в Android и передача сложных объектов в MainActivity" />
<p>Пусть класс User имеет следующий код:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>Стоит отметить, что данный класс реализует интерфейс <span class="b">Serializable</span>. Теперь изменим код MainActivity:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>Теперь вместо трех разрозненных данных передается один объект User. В качестве ключа используется результат метода <code>User.class.getSimpleName()</code>, 
который по сути возвращает название класса.</p>
<p>И изменим класс SecondActivity:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>Для получения данных применяется метод <code>getSerializable()</code>, поскольку класс User реализует интерфейс Serializable. Таким образом, 
мы можем передать один единственый объект вместо набора разрозненных данных.</p>
