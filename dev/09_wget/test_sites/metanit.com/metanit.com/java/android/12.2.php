...
<html >
<head>
<title>Java и Android | PreferenceFragmentCompat</title>
<meta charset="utf-8" />
<meta name="description" content="PreferenceActivity и PreferenceFragment и PreferenceFragmentCompat в Android и Java">
...
<h2>PreferenceFragmentCompat</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>Для упрощения работы с группой настроек Android предоставляет специальный тип фрагмента - <span class="b">PreferenceFragmentCompat</span>. 
Рассмотрим как ее использовать.</p>
<p>Создадим новый проект и вначале определим в файле <span class="b">build.gradle</span> нужные зависимости для работы с PreferenceFragmentCompat:</p>
<pre class="brush:java;">
implementation "androidx.fragment:fragment:1.3.6"
implementation "androidx.preference:preference:1.1.1"
</pre>
<p>Для определения настроек добавим в папку <span class="b">res</span> подпапку <span class="b">xml</span>.</p>
<img src="./pics/preferences1.png" alt="Настройки и PreferenceFragment в Android и Java" />
<p>Затем в папку <span class="b">res/xml</span> 
добавим новый файл, который назовем <span class="b">settings.xml</span>. И изменим его следующим образом:</p>
<pre class="brush:xml;">
...
<p>Здесь в корневом элементе <span class="b">PreferenceScreen</span> устанавливаются элементы <span class="b">EditTextPreference</span> и 
<span class="b">CheckBoxPreference</span>. Через каждый из этих элементов мы можем взаимодействовать с определенной настройкой.</p>
<p>Вобще в данном случае мы можем использовать ряд различных типов настроек:</p>
<ul>
<li><p><code>EditTextPreference</code>: используется элемент EditText для ввода текстового значения</p></li>
<li><p><code>CheckBoxPreference</code>: используется элемент CheckBox для установки логических значений true или false</p></li>
<li><p><code>SwitchPreference</code>: используется элемент Switch для установки логических значений true или false ("on" и "off")</p></li>
<li><p><code>RingtonePreference</code>: использует диалоговое окно для установки рингтона из списка рингтонов для установки логических значений true или false</p></li>
<li><p><code>ListPreference</code>: использует список для выбора одного из предопределенных значений</p></li>
<li><p><code>MultiSelectListPreference</code>: также использует список для выбора значений, но позволяет выбрать несколько элементов</p></li>
</ul>
<p>Для каждого элемента настройки необходимо определить, как минимум, три атрибута:</p>
<ul>
<li><p><code>android:key</code>: устанавливает ключ настройки в SharedPreferences</p></li>
<li><p><code>android:title</code>: название настройки для пользователя</p></li>
<li><p><code>android:summary</code>: краткое описание по данной настройке для пользователя</p></li>
</ul>
<p>Далее добавим новый класс Java, который назовем <span class="b">SettingsFragment</span>:</p>
<pre class="brush:java;">
...
<p>Фрагмент SettingsFragment наследуется от класса PreferenceFragmentCompat. В его методе <code>onCreatePreferences</code> вызывается метод 
<span class="b">addPreferencesFromResource()</span>, в который передается id ресурса xml с настройками (в данном случае ранее определенный ресурс R.xml.settings).</p>


<p>И теперь добавим в проект специальную activity для установки настроек. Назовем ее <span class="b">SettingsActivity</span>. 
В итоге проект будет выглядеть следующим образом:</p>
<img src="./pics/preferences6.png" alt="Настройки и PreferenceFragmentCompat в Android" />
<p>В файле layout для SettingsActivity - <span class="b">activity_settings.xml</span> пропишем следующий интерфейс:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>Здесь определен FragmentContainerView с id = settings_container - именно тот элемент, в который будет загружаться фрагмент SettingsFragment.</p>
<p>В коде <span class="b">SettingsActivity</span> определим загррузку фрагмента:</p>
<pre class="brush:java;">
package com.example.settingsapp;
...
<p>SettingsActivity в качестве разметки интерфейса будет использовать ресурс <code>R.layout.activity_settings</code>.</p>
<p>При запуске SettingsActivity будет загружать фрагмент SettingsFragment в элемент с id settings_container.</p>
<p>Далее перейдем к главной activity - MainActivity. В файле <span class="b">activity_main.xml</span> определим текстовое поле и кнопку:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>В методе <code>onResume()</code> получаем все настройки. Если настройка enabled равно true, то отображаем текстовое поле с логином.</p>
<p>В методе <code>setPrefs()</code>, который срабатывает при нажатии на кнопку, выполняется переход к SettingsActivity.</p>
<p>При первом запуске настроек не будет, и логин не будет отображаться. Перейдем на страницу настроек и установим там логин и включим его отображение, а 
затем вернемся на главную activity:</p>
<img src="./pics/preferences7.png" alt="Установка настроек в PreferenceFragmentCompat в Android" />
<p>При этом вручную нам ничего не надо сохранять, все настройки автоматически сохраняются функционалом PreferenceFragmentCompat.</p>
...
<div class="nav"><p><a href="./12.1.php">Назад</a><a href="./">Содержание</a><a href="./13.1.php">Вперед</a></p></div></div>
<div class="socBlock">
