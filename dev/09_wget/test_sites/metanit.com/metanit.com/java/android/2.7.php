...
<html >
<head>
<title>Java и Android | Ресурсы Color и установка цвета</title>
<meta charset="utf-8" />
<meta name="description" content="Ресурсы Color и установка цвета в Android, метод ContextCompat.getColor">
...
<h2>Ресурсы Color и установка цвета</h2><div class="date">Последнее обновление: 15.10.2021</div>
...
<p>В приложении Android также можно определять ресурсы цветов (Color). Они должны храниться в файле по пути <span class="b">res/values</span> и 
также, как и ресурсы строк, заключены в тег <code>&lt;resources&gt;</code>. Так, по умолчанию при создании самого простого проекта в папку 
<span class="b">res/values</span> добавляется файл <span class="b">colors.xml</span>:</p>
<pre class="brush:xml;">
...
<p>Цвет определяется с помощью элемента <code>&lt;color&gt;</code>. Его атрибут <code>name</code> устанавливает название цвета, которое будет 
использоваться в приложении, а  шестнадцатеричное число - значение цвета.</p>
<p>Для задания цветовых ресурсов можно использовать следующие форматы:</p>
<ul>
<li><p>#RGB (#F00 - 12-битное значение)</p></li>
<li><p>#ARGB (#8F00 - 12-битное значение с добавлением альфа-канала)</p></li>
<li><p>#RRGGBB (#FF00FF - 24-битное значение)</p></li>
<li><p>#AARRGGBB (#80FF00FF - 24-битное значение с добавлением альфа-канала)</p></li>
</ul>
<p>Чтобы не трогать и не портить данный файл, определим свой новый файл ресурсов и для этого добавим в папку <span class="b">res/values</span> новый файл ресурсов, 
который назовем <span class="b">my_colors.xml</span>.</p>
<img src="./pics/2.14.png" alt="Использование цвета в Android Studio и ресурсы color" />
<p>Изменим файл <span class="b">my_colors.xml</span>, добавив в него пару цветов:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;resources&gt;
...
<p>Применим цвета в файле <span class="b">activity_main.xml</span>:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>С помощью атрибута <code>android:textColor</code> устанавливается цвет текста в TextView, а атрибут <code>android:background</code> устанавливает 
фон TextView. В качестве значения они используют цвет, например, в том же шестнадцатеричном формате. Для получения самого цвета после <span class="b">"@color/"</span> указывается имя ресурса.</p>
<img src="./pics/2.3.png" alt="Использование цвета в Android и Java и ресурсы color" />
<p>Также можно использовать цветовые ресурсы в коде MainActivity:</p>
...
<p>Для получения цвета из ресурсов применяется метод <span class="b">resources.getColor()</span>, который принимает два параметра. Первый параметр - идентификатор ресурса, 
цвет которого надо получить. Второй параметр представляет тему. Но поскольку в данном случае тема не важна, для этого параметра передаем значение <code>null</code></p>
<p>Следует учитывать, что метод <span class="b">resources.getColor()</span> с двумя параметрами, который использован выше, доступен, если для минимальная версия Android не ниже Android 6.0 (или Android 23). 
Однако минимальная версия Android ниже, то можно использовать устаревшую версию с одним параметром:</p>
<pre class="brush:java;">
...
<div class="nav"><p><a href="./2.6.php">Назад</a><a href="./">Содержание</a><a href="./2.1.php">Вперед</a></p></div></div>
<div class="socBlock">
