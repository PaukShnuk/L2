...
<html >
<head>
<title>Java и Android | LinearLayout</title>
<meta charset="utf-8" />
<meta name="description" content="Контейнер LinearLayout в Android, атрибут layout_weight, горизонтальное и вертикальное расположение элементов">
...
<h2>LinearLayout</h2><div class="date">Последнее обновление: 14.10.2021</div>
...
<p>Контейнер <span class="b">LinearLayout</span> представляет простейший контейнер - объект <code>ViewGroup</code>, который упорядочивает все дочерние элементы в 
...
<span class="b">android:orientation</span>.</p>
<p>Если, например, ориентация разметки вертикальная (<code>android:orientation="vertical"</code>), то все элементы располагаются в столбик - 
по одному элементу на каждой строке. Если ориентация горизонтальная (<code>android:orientation="horizontal"</code>), то элементы располагаются в одну строку. 
Например, расположим элементы в горизонтальный ряд:</p>
<pre class="brush:xml;">
...
<img src="./pics/linearlayout1.png" alt="LinearLayout в Android Studio" />
<p>Если бы мы указали для LinearLayout атрибут <span class="b">android:orientation="vertical"</span>, то элементы размещались бы по вертикали:</p>
<img src="./pics/linearlayout2.png" alt="Вертикальный LinearLayout в Android Studio" />
<h3>Вес элемента</h3>
<p>LinearLayout поддерживает такое свойство, как вес элемента, которое передается атрибутом <span class="b">android:layout_weight</span>. 
...
Например, если один элемент у нас будет иметь для свойства <code>android:layout_weight</code> значение 2, а другой - значение 1, то в сумме они дадут 3, поэтому первый элемент будет занимать 2/3 оставшегося пространства, а второй - 1/3.</p>
<p>Если все элементы имеют значение <code>android:layout_weight="1"</code>, то все эти элементы будут равномерно распределены по всей площади контейнера:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;LinearLayout xmlns:android="http://schemas.android.com/apk/res/android"
    android:layout_width="match_parent"
...
<p>В данном случае LinearLayout имеет вертикальную ориентацию, поэтому все элементы будут располагаться сверху вниз. Все три элемента имеют значение 
<code>android:layout_weight="1"</code>, поэтому сумма весов всех элементов будет равна 3, а каждый элемент получит по трети пространства в LinearLayout:</p>
<img src="./pics/linearlayout3.png" alt="layout_weight в Android" />
<p>При этом так как у нас вертикальный стек, то нам надо также установить для свойства <code>layout_height</code> значение <span class="b">0dp</span>. Если бы 
LinearLayout имел горизонтальную ориентацию, то для свойства <code>layout_width</code> надо было бы установить значение <span class="b">0dp</span>.</p>
<p>Еще один атрибут <span class="b">android:weightSum</span> позволяет указать сумму весов всех элементов. Например:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;LinearLayout xmlns:android="http://schemas.android.com/apk/res/android"
    android:layout_width="match_parent"
...
<p>LinearLayout здесь задает сумму весов равную 7. То есть все пространство по вертикали (так как вертикальная ориентация) условно делится 
на семь равных частей.</p>
<p>Первый TextView имеет вес 1, то есть из этих семи частей занимает только одну. Второй TextView имеет вес 3, то есть занимает три части из семи. 
И третий имеет вес 2. Итоговая сумма составляет 6. Но так как LinearLayout задает вес 7, то одна часть будет свободна от всех элементов.</p>
<img src="./pics/linearlayout4.png" alt="weightSum в LinearLayout" />


<h3>Программное создание LinearLayout</h3>
<p>Создание LinearLayout в коде java:</p>
...
<img src="./pics/linearlayout5.png" alt="Программное создание LinearLayout в Android и Java" />
<p>Дополнительная версия конструктора <span class="b">LinearLayout.LayoutParams()</span> в качестве третьего параметра позволяет указать вес элемента:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<img src="./pics/linearlayout6.png" alt="Программное создание LinearLayout с layout_weight в Android и Java" />

<h3>Layout_gravity</h3>
<p>Атрибут <span class="b">layout_gravity</span> позволяет устанавливать позиционирование относительно LinearLayout. Он принимает следуюшие значения:</p>
<ul>
<li><p><code>top</code>: выравнивает элемент по верхней границе контейнера</p></li>
<li><p><code>bottom</code>: выравнивает элемент по нижней границе контейнера</p></li>
<li><p><code>left</code>: выравнивает элемент по левой границе контейнера</p></li>
<li><p><code>right</code>: выравнивает элемент по правой границе контейнера</p></li>
<li><p><code>center_vertical</code>: выравнивает элемент по центру по вертикали</p></li>
<li><p><code>center_horizontal</code>: выравнивает элемент по центру по горизонтали</p></li>
<li><p><code>center</code>: элемент позиционируется в центре</p></li>
...
<li><p><code>clip_vertical</code>: обрезает верхнюю и нижнюю границу элемента</p></li>
<li><p><code>clip_horizontal</code>: обрезает правую и левую границу элемента</p></li>
...
<p>В данном случае первый элемент TextView будет позиционироваться по левой стороне контейнера (<code>android:layout_gravity="left"</code>), 
второй TextView по центру (<code>android:layout_gravity="center"</code>), 
третий - по правой стороне (<code>android:layout_gravity="right"</code>) и четвертый - по центру (<code>android:layout_gravity="center"</code>)</p>
<img src="./pics/gravity3.png" alt="layout_gravity в LinearLayout в Android" />
<p>Стоит учитывать ориентацию контейнера. Например, при вертикальной ориентации все элементы будут представлять вертикальный стек,
идущий сверху вниз. Поэтому значения, которые относятся к позиционированию элемента по вертикали (например, top или bottom) никак не будут влиять на элемент. Также при горизонтальной ориентации 
LinearLayout не окажут никакого влияния значения, которые позиционируют элемент по горизонтали, например, left и right.</p>
<p>Для установки программно параметра <span class="b">layout_gravity</span> надо задать поле <span class="b">gravity</span> у объекта <span class="b">LinearLayout.LayoutParams</span>:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>В качестве значения передается одна из констант класса Gravity, которые аналогичны значениям атрибута.</p>
...
<div class="nav"><p><a href="./3.16.php">Назад</a><a href="./">Содержание</a><a href="./3.3.php">Вперед</a></p></div></div>
<div class="socBlock">
