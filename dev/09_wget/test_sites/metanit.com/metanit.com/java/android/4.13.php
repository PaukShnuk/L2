...
<html >
<head>
<title>Java и Android | Приложение Калькулятор</title>
<meta charset="utf-8" />
<meta name="description" content="Создиние простейшего калькулятора под Android с помощью языка программирования Java">
...
<h2>Приложение Калькулятор</h2><div class="date">Последнее обновление: 15.10.2021</div>
...
<p>Зная некоторые основы компоновки и такие элементы как TextView, EditText и Button, уже можно составить более менее полноценное приложение. В данном случае мы сделаем простенький калькулятор.</p>
<p>Для этого создадим новый проект и определим в файле <span class="b">activity_main.xml</span> следующий интерфейс:</p>
<pre class="brush:xml;">
...
<p>В итоге весь интерфейс будет выглядеть следующим образом:</p>
<img src="./pics/calculator.png" alt="Калькулятор на Android и Java" />
<p>Корневой контейнер компоновки представляет элемент ConstraintLayout . Сверху в нем определены два текстовых поля TextView: одно для вывода результата вычислений и одно для вывода текущего знака операции.</p>
<p>Затем идет элемент EditText, предназначенный для ввода чисел.</p>
<p>И далее расположены четыре элемента LinearLayout с горизонтальными рядами кнопок. Чтобы все кнопки занимали равное пространство внутри контейнера, для них установлены атрибуты 
<code>android:layout_weight="1"</code> и <code>android:layout_width="0dp"</code>.</p>
<p>Кроме того, для числовых кнопок в качестве обработчика нажатия установлен метод <code>onNumberClick</code>, а для кнопок со знаками операций атрибут <code>onClick</code> 
указывает на метод <code>onOperationClick</code>.</p>
...
<p>Разберем этот код. Вначале в методе <code>onCreate()</code> получаем все поля из activity_main.xml, текст которых будет изменяться:</p>
<pre class="brush:java;">
resultField = findViewById(R.id.resultField);
numberField = findViewById(R.id.numberField);
operationField = findViewById(R.id.operationField);
</pre>
<p>Результат операции будет попадать в переменную operand, которая представляет тип Double, а знак операции - в переменную lastOperation:</p>
<pre class="brush:java;">
Double operand = null;
String lastOperation = "=";
</pre>
<p>Так как при переходе от портретной ориентации к альбомной или наоборот мы можем потерять все введенные данные, то чтобы их не потерять, мы их сохраняем в методе 
<code>onSaveInstanceState()</code> и обратно получаем в методе <code>onRestoreInstanceState()</code>.</p>
<p>При нажатии на числовую кнопку будет вызываться метод <code>onNumberClick</code>, в котором добавляем введенную цифру или знак запятой к тексту в поле numberField:</p>
<pre class="brush:java;">
Button button = (Button)view;
numberField.append(button.getText());
...
<p>При этом если последняя операция представляла собой получение результата (знак "равно"), то мы сбрасываем переменную operand.</p>
<p>В методе <code>onOperationClick</code> происходит обработка нажатия на кнопку со знаком операции:</p>
<pre class="brush:java;">
Button button = (Button)view;
String op = button.getText().toString();
String number = numberField.getText().toString();
...
<p>Здесь получаем ранее введенное число и введенную операцию и передаем их в метод <code>performOperation()</code>. Так как в метод передается не просто строка, а число 
...
используется конструкция try...catch.</p>
<p>Кроме того, так как разделителем целой и дробной части в Double в java является точка, то нам надо заменить запятую на точку, так как предполагается, что мы используем в качестве разделителя запятую.</p>
<p>А методе <code>performOperation()</code> выполняем собственно операцию. При вводе первой операции, когда операнд еще не установлен, мы просто устанавливаем операнд:</p>
<pre class="brush:java;">
if(operand ==null){
    operand = number;
}
</pre>
<p>При вводе второй и последующих операций применяем предыдущую операцию, знак которой хранится в переменной lastOperation, к операнду operand и второму числу, которое было введено в числовое поле. 
Полученный результат операции сохраняем в переменной operand.</p>
...
<div class="nav"><p><a href="./4.9.php">Назад</a><a href="./">Содержание</a><a href="./4.10.php">Вперед</a></p></div></div>
<div class="socBlock">
