...
<html >
<head>
<title>Java и Android | Фрагменты</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление и использование фрагментов в Android Studio и Java, добавление фрагмента в коде MainActivity, AndroidX Fragment library">
...
<h1>Фрагменты</h1><h2>Введение во фрагменты</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>Организация приложения на основе нескольких activity не всегда может быть оптимальной. Мир ОС Android довольно сильно фрагментирован и состоит 
из многих устройств. И если для мобильных аппаратов с небольшими экранами взаимодействие между разными activity выглядит довольно неплохо, то 
на больших экранах - планшетах, телевизорах окна activity смотрелись бы не очень в силу большого размера экрана. Собственно поэтому и появилась концепция фрагментов.</p>
<p>Фрагмент представляет кусочек визуального интерфейса приложения, который может использоваться повторно и многократно.  У фрагмента может быть собственный 
файл layout, у фрагментов есть свой собственный жизненный цикл. Фрагмент существует в контексте activity и имеет свой жизненный цикл, вне activity обособлено он существовать не может. Каждая activity может 
иметь несколько фрагментов.</p>
<img src="./pics/fragment.png" alt="Фрагменты в Android" />
<p>Для начала работы с фрагментами создадим новый проект с пустой MainActivity. И вначале создадим первый фрагмент. Но сразу стоит отметить, что не вся функциональность 
фрагментов по умолчанию может быть доступна в проекте, поскольку располагается в отдельной библиотеке - <span class="b">AndroidX Fragment library</span>. И вначале необходимо подключить к проекту 
эту библиотеку в файле <span class="b">build.gradle</span>.</p>
<img src="./pics/fragments9.png" alt="Подключение фрагментов и AndroidX Fragment library в Android и Java" />
<p>Найдем в нем секцию <span class="b">dependencies</span>, которая выглядит по умолчанию примерно так:</p>
...
<p>В ее начало добавим строку</p>
<pre class="brush:java;">
implementation "androidx.fragment:fragment:1.3.6"
</pre>
<p>То есть в моем случае получится </p>
<pre class="brush:java;">
dependencies {
...
<img src="./pics/fragments10.png" alt="Подключение фрагментов и AndroidX Fragment library в Android и Java и Gradle" />
<p>И затем нажмем на появившуюся ссылку <span class="b">Sync Now</span>.</p>

<p>Фактически фрагмент - это обычный класс Java, который наследуется от класса <span class="b">Fragment</span>. Однако как и класс Activity, фрагмент может использовать xml-файлы layout для определения графического интерфейса. 
И таким образом, мы можем добавить по отдельности класс Java, который представляет фрагмент, и файл xml для хранения в нем разметки интерфейса, который будет использовать 
фрагмент.</p>

<p>Итак, добавим в папку <span class="b">res/layout</span> новый файл <span class="b">fragment_content.xml</span> и определим в нем следующий код:</p>
<pre class="brush:xml;">
...
<p>Фрагменты содержат те же элементы управления, что и activity. В частности, здесь определены кнопка и текстовое поле, которые будут составлять интерфейс фрагмента.</p>
<p>Теперь создадим сам класс фрагмента. Для этого добавим в одну папку с <span class="b">MainActivity</span> новый класс. Для этого нажмем на папку правой кнопкой мыши и выберем в 
меню <span class="b">New -&gt; Java Class</span>. Назовем новый класс <span class="b">ContentFragment</span> и определим у него следующее содержание:</p>
<pre class="brush:java;">
...
<p>Класс фрагмента должен наследоваться от класса <span class="b">Fragment</span>.</p>
<p>Чтобы указать, что фрагмент будет использовать определенный xml-файл layout, идентификатор ресурса layout передается в вызов 
конструктора родительского класса (то есть класса Fragment).</p>
<p>Весь проект будет выглядеть следующим образом:</p>
<img src="./pics/fragments4.png" alt="Фрагменты в Android Studio и Java" />
<h3>Добавление фрагмента в Activity</h3>
<p>Для использования фрагмента добавим его в <span class="b">MainActivity</span>. Для этого изменим файл <span class="b">activity_main.xml</span>, которая определяет интерфейс для 
MainActivity:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>Для добавления фрамента применяется элемент <span class="b">FragmentContainerView</span>. По сути FragmentContainerView представляет объект 
View, который расширяет класс <span class="b">FrameLayout</span> и предназначен специально для работы с фрагментами. Собственно кроме фрагментов он больше ничего 
содержать не может.</p>
<p>Его атрибут <span class="b">android:name</span> указывает на имя класса фрагмента, который будет использоваться. В моем случае полное имя класса фрагмента 
с учетов пакета <span class="b">com.example.fragmentapp.ContentFragment</span>.</p>
<p>Код класса <span class="b">MainActivity</span> остается тем же, что и при создании проекта:</p>
<pre class="brush:java;">
package com.example.fragmentapp;
...
<p>Если мы запустим приложение, то мы увидим фактически тот же самый интерфейс, который мы могли бы сделать и через activity, только в данном случае 
интерфейс будет определен во фрагменте:</p>
<img src="./pics/fragments5.png" alt="Создание фрагмента для Android и Java" />

<p>Стоит отметить, что Android Studio представляет готовый шаблон для добавления фрагмента. Собственно воспользуемся этим способом.</p>
<p>Для этого нажмем на папку, где находится класс <span class="b">MainActivity</span>, правой кнопкой мыши 
и в появившемся меню выберем <span class="b">New -&gt; Fragment -&gt; Fragment(Blank)</span>:</p>
<img src="./pics/fragments3.png" alt="Добавление фрагмента в Android Studio" />
<p>Данный шаблон предложить указать класс фрагмента и название файла связанного с ним класса разметки интерфейса.</p>
<img src="./pics/fragments11.png" alt="Добавление фрагмента в Android Studio и Java" />
<h3>Добавление логики к фрагменту</h3>
<p>Фрагмент определяет кнопку. Теперь добавим к этой кнопки некоторое действие. Для этого изменим класс ContentFragment:</p>
<pre class="brush:java;">
package com.example.fragmentapp;
...
<p>Здесь переопределен метод <span class="b">onViewCreated</span> класса Fragment, который вызывается после создания объекта View для визуального интерфейса, который представляет 
данный фрагмент. Созданный объект View передается в качестве первого параметра. И далее мы можем получить конкретные элементы управления в рамках этого объекта 
View, в частности, TextView и Button, и выполнить с ними некоторые действия. В данном случае в обработчике нажатия кнопки в 
текстовом поле выводится текущая дата.</p>
<img src="./pics/fragments12.png" alt="Добавление фрагмента в AndroidX Fragment Library и Java" />

<h3>Добавление фрагмента в коде</h3>
<p>Кроме определения фрагмента в xaml-файле интерфейса мы можем добавить его динамически в activity.</p>
<p>Для этого изменим файл <span class="b">activity_main.xml</span>:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.fragment.app.FragmentContainerView
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>И также изменим класс <span class="b">MainActivity</span>:</p>
<pre class="brush:java;">
package com.example.fragmentapp;
...
<p>Метод <code>getSupportFragmentManager()</code> возвращает объект <code>FragmentManager</code>, который управляет фрагментами.</p>
<p>Объект FragmentManager с помощью метода <code>beginTransaction()</code> создает объект <span class="b">FragmentTransaction<span>.</p>
<p>FragmentTransaction выполняет два метода: add() и commit(). Метод add() добавляет фрагмент: <code>add(R.id.fragment_container_view, new ContentFragment())</code> - 
первым аргументом передается ресурс разметки, в который надо добавить фрагмент (это определенный в <span class="b">activity_main.xml</span> элемент 
<code>androidx.fragment.app.FragmentContainerView</code>). И метод <code>commit()</code> подтвержает и завершает операцию добавления.</p>
<p>Итоговый результат такого добавления фрагмента будет тем же, что и при явном определении фрагмента через элемент <code>FragmentContainerView</code> 
в разметке интерфейса.</p>
...
<div class="nav"><p><a href="./7.2.php">Назад</a><a href="./">Содержание</a><a href="./8.3.php">Вперед</a></p></div></div>
<div class="socBlock">
