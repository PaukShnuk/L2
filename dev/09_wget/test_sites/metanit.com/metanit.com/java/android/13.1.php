...
<html >
<head>
<title>Java и Android | Работа с файловой системой</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с файловой системой в Android и Java, запись и чтение файлов, текстовый редактор, Device File Explorer, просмотр файлов смартфона в Android Studio">
...
<h1>Работа с файловой системой</h1><h2>Чтение и сохранение файлов</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>Работа с настройками уровня activity и приложения позволяет сохранить небольшие данные отдельных типов (string, int), но для работы с большими массивами данных, такими как графически файлы, файлы мультимедиа и т.д., 
нам придется обращаться к файловой системе.</p>
<p>ОС Android построена на основе Linux. Этот факт находит свое отражение в работе с файлами. Так, в путях к файлам в качестве разграничителя 
в Linux использует слеш "/", а не обратный слеш "\" (как в Windows). А все названия файлов и каталогов являются регистрозависимыми, то есть "data" это не то же самое, что и "Data".</p>
<p>Приложение Android сохраняет свои данные в каталоге <span class="b">/data/data/&lt;название_пакета&gt;/</span> и, как правило, относительно 
этого каталога будет идти работа.</p>
<p>Для работы с файлами абстрактный класс <code>android.content.Context</code> определяет ряд методов:</p>
<ul>
<li><p><span class="b">boolean deleteFile (String name)</span>: удаляет определенный файл</p></li>
<li><p><span class="b">String[] fileList ()</span>: получает все файлы, которые содержатся в подкаталоге <span class="ii">/files</span> в каталоге приложения</p></li>
<li><p><span class="b">File getCacheDir()</span>: получает ссылку на подкаталог <span class="ii">cache</span> в каталоге приложения</p></li>
<li><p><span class="b">File getDir(String dirName, int mode)</span>: получает ссылку на подкаталог в каталоге приложения, если такого подкаталога нет, то он создается </p></li>
<li><p><span class="b">File getExternalCacheDir()</span>: получает ссылку на папку <span class="ii">/cache</span> внешней файловой системы устройства</p></li>
<li><p><span class="b">File getExternalFilesDir(String type)</span>: получает ссылку на каталог <span class="ii">/files</span> внешней файловой системы устройства</p></li>
<li><p><span class="b">File getFileStreamPath(String filename)</span>: возвращает абсолютный путь к файлу в файловой системе</p></li>
<li><p><span class="b">FileInputStream openFileInput(String filename)</span>: открывает файл для чтения</p></li>
<li><p><span class="b">FileOutputStream openFileOutput (String name, int mode)</span>: открывает файл для записи</p></li>
</ul>
<p>Все файлы, которые создаются и редактируются в приложении, как правило, хранятся в подкаталоге <span class="ii">/files</span> в каталоге приложения.</p>
<p>Для непосредственного чтения и записи файлов применяются также стандартные классы Java из пакета java.io.</p>
<p>Итак, применим функционал чтения-записи файлов в приложении. Пусть у нас будет следующая примитивная разметка layout:</p>
<pre class="brush:xml;">
...
<p>Поле EditText предназначено для ввода текста, а TextView - для вывода ранее сохраненного текста. Для сохранения и восстановления текста добавлены 
две кнопки.</p>
<p>Теперь в коде Activity пропишем обработчики кнопок с сохранением и чтением файла:</p>
...
<p>При нажатии на кнопку сохранения будет создаваться поток вывода <code>FileOutputStream fos = openFileOutput(FILE_NAME, MODE_PRIVATE)</code></p>
<p>В данном случае введенный текст будет сохраняться в файл "content.txt". При этом будет использоваться режим MODE_PRIVATE</p>
<p>Система позволяет создавать файлы с двумя разными режимами:</p>
<ul>
<li><p><span class="b">MODE_PRIVATE</span>: файлы могут быть доступны только владельцу приложения (режим по умолчанию)</p></li>
<li><p><span class="b">MODE_APPEND</span>: данные могут быть добавлены в конец файла</p></li>
</ul>
<p>Поэтому в данном случае если файл "content.txt" уже существует, то он будет перезаписан. Если же нам надо было дописать файл, тогда надо было бы 
использовать режим MODE_APPEND:</p>
<pre class="brush:java;">FileOutputStream fos = openFileOutput(FILE_NAME, MODE_APPEND);</pre>
<p>Для чтения файла применяется поток ввода <span class="b">FileInputStream</span>:</p>
<pre class="brush:java;">FileInputStream fin = openFileInput(FILE_NAME);</pre>
<p>Подробнее про использование потоков ввода-вывода можно прочитать в руководстве по Java: <a href="//metanit.com/java/tutorial/6.3.php">https://metanit.com/java/tutorial/6.3.php</a></p>
<p>В итоге после нажатия кнопки сохранения весь текст будет сохранен в файле /data/data/название_пакета/files/content.txt</p>
<img src="./pics/filesystem1.png" alt="Сохранение и открытие файлов в Android и Java" />
<p>Где физически находится созданный файл? Чтобы увидеть его на подключенном устройстве перейдем в Android Stud	 в меню к пункту 
<span class="b">View -&gt; Tool Windows -&gt; Device File Explorer</span></p>
<img src="./pics/filesystem.png" alt="Device File Explorer в Android Studio" />
<p>После этого откроектся окно Device File Explorer для просмотра файловой системы устройства. И в папке <span class="b">data/data/[название_пакета_приложения]/files</span> 
мы сможем найти сохраненный файл.</p>
<img src="./pics/filesystem3.png" alt="Просмотр файлов смартфона в Android Studio" />
