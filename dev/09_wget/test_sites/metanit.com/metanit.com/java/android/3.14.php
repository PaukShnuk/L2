...
<html >
<head>
<title>Java и Android | Вложенные layout</title>
<meta charset="utf-8" />
<meta name="description" content="Вложенные layout в Android, элемент include и встраивание разметки графического интерфейса в другую разметку">
...
<h2>Вложенные layout</h2><div class="date">Последнее обновление: 15.10.2021</div>
...
<p>Одна layout может содержать другую layout. Для этого применяется элемент <span class="b">include</span>.</p>
<p>Например, добавим в папку <span class="b">res/layout</span> два файла layout, которые пусть будут называться <span class="b">text_panel.xml</span> и 
<span class="b">button_panel.xml</span>:</p>
<img src="./pics/include1.png" alt="Include layouts in Android" />
<p>В файле <span class="b">text_panel.xml</span> определим следующий код:</p>
<pre class="brush:xml;">
...
<p>По сути здесь просто определено поле TextView для вывода текста.</p>
<p>В файле <span class="b">button_panel.xml</span> определим следующую разметку:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>Здесь определена кнопка, нажатия которой мы будем обрабатывать.</p>
<p>Основным файлом разметки, который определяет интерфейс приложения, по-прежнему является <span class="b">activity_main.xml</span>. 
Изменим его:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
...
<p>С помощью ConstraintLayout весь интерфейс здесь организуется в виде вертикального стека. С помощью элементов <span class="b">include</span> внутрь ConstraintLayout 
добавляется содержимое файлов text_panel.xml и button_panel.xml. Для указания названия файла применяется атрибут <span class="b">layout</span>.</p>
<p>Это все равно, что если бы мы напрямую вместо элемента include добавили содержимое файлов. Однако такой способ имеет свои преимущества. Например, 
какая-то часть разметки, группа элементов управления может повторяться в различных activity. И чтобы не определять по сто раз эти элементы, можно вынести их 
в отдельный файл layout и с помощью include подключать их.</p>
<p>После добавления в ConstraintLayout к элементам <span class="b">include</span> можно применять все те стандартные атрибуты, которые применяются 
в этом контейнере к вложенным элементам, например, настроить размеры, расположение. Также стоит отметить, что добавлять внешние layout можно не только в 
ConstraintLayout, но и в другие контейнеры (LinearLayout, RelativeLayout и т.д.)</p>
<p>Также изменим код <span class="b">MainActivity</span>:</p>
...
<p>В MainActivity мы можем обращаться к элементам во вложенных файлах layout. Например, мы можем установить обработчик нажатия кнопки, в котором при нажатии изменять 
текст в TextView.</p>
<img src="./pics/include2.png" alt="Вложенные файлы layout в Activity в Android" />
<p>При этом мы несколько раз можем добавлять в один файл layout другой файл layout. Для этого вначале изменим файл <span class="b">button_panel.xml</span> 
следующим образом:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
...
<p>И изменим файл <span class="b">activity_main.xml</span>:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>Теперь файл button_panel.xml добавляется два раза. Важно, что при добавлении этого файла 
каждому элементу include присвоен определенный id. По этому id мы сможем узнать, о каком именно элементе include идет речь.</p>
<p>Также изменим MainActivity:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>Здесь вначале мы получаем отдельные элементы include по id. Затем в рамках этих элементов получаем кнопку. После этого мы можем установить у кнопко любой текст и 
повесить обработчик события нажатия. И таким образом, поведение обеих кнопок будет различаться.</p>
<img src="./pics/include3.png" alt="вложение layout с помощью include в android" />
