...
<html >
<head>
<title>Java и Android | WebView</title>
<meta charset="utf-8" />
<meta name="description" content="WebView в Android и Java, загрузка и просмотр интернет-ресурса, loadData и loadUrl">
...
<h1>Работа с сетью. WebView</h1><h2>WebView</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>WebView представляет простейший элемент для рендеринга html-кода, базирующийся на движке WebKit. Благодаря этому мы можем использовать WebView 
как примитивный веб-браузер, просматривая через него контент из сети интернет. Использование движка WebKit гарантирует, что отображение контента будет 
происходить примерно такжe, как и в других браузерах, построенных на этом движке - Google Chrome и Safari.</p>

<p>Некоторые основные методы класса WebView:</p>
<ul>
<li><p><span class="b">boolean canGoBack()</span>: возвращает true, если перед текущей веб-страницей в истории навигации WebView еще есть страницы</p></li>
<li><p><span class="b">boolean canGoForward()</span>: возвращает true, если после текущей веб-страницей в истории навигации WebView еще есть страницы</p></li>
<li><p><span class="b">void	clearCache(boolean includeDiskFiles)</span>: очищает кэш WebView</p></li>
<li><p><span class="b">void	clearFormData()</span>: очищает данный автозаполнения полей форм</p></li>
<li><p><span class="b">void	clearHistory()</span>: очищает историю навигации WebView</p></li>
<li><p><span class="b">String getUrl()</span>: возвращает адрес текущей веб-страницы</p></li>
<li><p><span class="b">void	goBack()</span>: переходит к предыдущей веб-странице в истории навигации</p></li>
<li><p><span class="b">void	goForward()</span>: переходит к следующей веб-странице в истории навигации</p></li>
<li><p><span class="b">void	loadData(String data, String mimeType, String encoding)</span>: загружает в веб-браузере данные в виде html-кода, используя указанный 
mime-тип и кодировку</p></li>
<li><p><span class="b">void	loadDataWithBaseURL (String baseUrl, String data, String mimeType, String encoding, String historyUrl)</span>: 
также загружает в веб-браузере данные в виде html-кода, используя указанный mime-тип и кодировку, как и метод <code>loadData()</code>. Однако кроме того, 
в качестве первого параметра принимает валидный адрес, с которым ассоциируется загруженные данные.</p>
<p>Зачем нужен этот метод, если есть <code>loadData()</code>? Содержимое, загружаемое методом <code>loadData()</code>, в качестве значения для 
<code>window.origin</code> будет иметь значение <code>null</code>, и таким образом, источник загружаемого содержимого не сможет пройти проверку на достоверность. 
Метод <code>loadDataWithBaseURL()</code> с валидными адресами (протокол может быть и HTTP, и HTTPS) позволяет установить источник содержимого.</p>
</li>
<li><p><span class="b">void	loadUrl(String url)</span>: загружает веб-страницу по определенному адресу</p></li>
<li><p><span class="b">void	postUrl(String url, byte[] postData)</span>: отправляет данные с помощью запроса типа "POST" по определенному адресу</p></li>
<li><p><span class="b">void	zoomBy(float zoomFactor)</span>: изменяет масштаб на опредленный коэффициент</p></li>
<li><p><span class="b">boolean zoomIn()</span>: увеличивает масштаб</p></li>
<li><p><span class="b">boolean zoomOut()</span>: уменьшает масштаб</p></li>
</ul>
<p>Работать с WebView очень просто. Определим данный элемент в разметке layout:</p>
<pre class="brush:xml;">
...
<p>Для получения доступа к интернету из приложения, необходимо указать в файле манифеста AndroidManifest.xml соответствующее разрешение:</p>
<pre class="brush:xml;">&lt;uses-permission android:name="android.permission.INTERNET"/&gt;</pre>
<p>Чтобы загрузить определенную страницу в WebView, через метод <code>loadUrl()</code> надо установить ее адрес:</p>
...
<img src="./pics/webview.png" alt="Элемент WebView в Android и Java" />
<p>Вместо определения элемента в layout мы можем создать WebView в коде Activity:</p>
<pre class="brush:java;">
WebView browser = new WebView(this);
setContentView(browser);
browser.loadUrl("http://metanit.com");
</pre>
<p>Кроме загрузки конкретной страницы из интернета с помощью метод <code>loadData()</code>:</p>
<pre class="brush:java;">
WebView browser= findViewById(R.id.webBrowser);
browser.loadData("&lt;html&gt;&lt;body&gt;&lt;h2&gt;Hello, Android!&lt;/h2&gt;&lt;/body&gt;&lt;/html&gt;", "text/html", "UTF-8");
</pre>
<p>Первым параметром метод принимает строку кода html, во втором - тип содержимого, а в третьем - кодировку.</p>
<img src="./pics/webview2.png" alt="Load data in WebView в Android и Java" />
<h4>JavaScript</h4>
<p>По умолчанию в WebView отключен javascript, чтобы его включить надо применить метод <span class="b">setJavaScriptEnabled(true)</span> объекта WebSettings:</p>
<pre class="brush:java;">
...
<div class="nav"><p><a href="./10.4.php">Назад</a><a href="./">Содержание</a><a href="./15.1.php">Вперед</a></p></div></div>
<div class="socBlock">
