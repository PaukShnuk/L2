...
<html >
<head>
<title>Java и Android | Сервисы</title>
<meta charset="utf-8" />
<meta name="description" content="Сервисы в Java и Android, класс Service, методы onCreate, onStartCommand, onBind, bindService, startService">
...
<h1>Сервисы</h1><h2>Введение в сервисы Android</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>Сервисы представляют собой особую организацию приложения. В отличие от activity они не требуют наличия визуального интерфейса. Сервисы позволяют 
выполнять долговременные задачи без вмешательства пользователя.</p>
<p>Все сервисы наследуются от класса <span class="b">Service</span> и проходят следующие этапы жизненного цикла:</p>
<ul>
<li><p>Метод <code>onCreate()</code>: вызывается при создании сервиса</p></li>
<li><p>Метод <code>onStartCommand()</code>: вызывается при получении сервисом команды, отправленной с помощью метода <code>startService()</code></p></li>
<li><p>Метод <code>onBind()</code>: вызывается при закреплении клиента за сервисом с помощью метода <code>bindService()</code></p></li>
<li><p>Метод <code>onDestroy()</code>: вызывается при завершении работы сервиса</p></li>
</ul>
<p>Создадим простейшее приложение с сервисом. Наш сервис будет воспроизводить музыкальный файл. И вначале добавим в проект в каталог <span class="b">res</span> 
папку <span class="b">raw</span>. Для этого нажмем правой кнопкой мыши на каталог <span class="b">res</span> и в контекстном меню выберем пункт 
<span class="b">New -&gt; Android Resource Directory</span>.</p>
<img src="./pics/androidservice1.png" alt="Добавление папки raw для сервисов в Android Studio и Java" />
<p>Далее укажем в качестве типа папки - <span class="b">raw</span>:</p>
<img src="./pics/androidservice2.png" alt="Добавление папки ресурсов raw для сервисов в Android Studio и Java" />
<p>И поместим в эту папку (<span class="b">res/raw</span>) какой-нибудь mp3-файл.</p>
<p>Затем добавим новый класс сервиса. Назовем его <span class="b">MediaService</span>. В итоге получится следующий проект:</p>
<img src="./pics/androidservice3.png" alt="Добавление сервисов в Android Studio и Java" />
<p>Для воспроизведения аудио-файла определим в классе <span class="b">MediaService</span> следующий код:</p></p>
...
<p>Для воспроизведения музыкального файла сервис будет использовать компонент <span class="b">MediaPlayer</span>.</p>
<p>В сервисе переопределяются все четыре метода жизненного цикла. Но по сути метод <code>onBind()</code> не имеет никакой реализации.</p>
<p>В методе <code>onCreate()</code> инициализируется медиа-проигрыватель с помощью музыкального ресурса, который добавлен в папку res/raw.</p>
<p>В методе <code>onStartCommand()</code> начинается воспроизведение.</p>
<p>Метод <code>onStartCommand()</code> может возвращать одно из значений, которое предполагает различное поведение в случае, если процесс сервиса был неожиданно 
завершен системой:</p>
<ul>
<li><p><span class="bb">START_STICKY</span>: в этом случае сервис снова возвращается в запущенное состояние, как будто если бы снова был бы вызван метод 
onStartCommand() без передачи в этот метод объекта Intent</p></li>
<li><p><span class="bb">START_REDELIVER_INTENT</span>: в этом случае сервис снова возвращается в запущенное состояние, как будто если бы снова был бы вызван метод 
onStartCommand() с передачей в этот метод объекта Intent</p></li>
<li><p><span class="bb">START_NOT_STICKY</span>: сервис остается в остановленном положении</p></li>
</ul>
<p>Метод <code>onDestroy()</code> завершает воспроизведение.</p>
<p>Чтобы управлять сервисом, изменим activity. Сначала добавим в файл <span class="b">activity_main.xml</span> пару кнопок для управления сервисом:</p>
<pre class="brush:xml;">
...
<p>И изменим код <span class="b">MainActivity</span>:</p>
<pre class="brush:java;">
package com.example.soundserviceapp;
...
<p>Для запуска сервиса используется объект Intent:</p>
<pre class="brush:java;">Intent i=new Intent(this, MediaService.class);</pre>
<p>Для запуска сервиса в классе Activity определен метод <span class="b">startService()</span>, в который передается объект Intent. Этот метод 
будет посылать команду сервису и вызывать его метод <code>onStartCommand()</code>, а также указывать системе, что сервис должен продолжать работать 
до тех пор, пока не будет вызван метод stopService().</p>
<p>Метод <span class="b">stopService()</span> также определен к классе Activity и принимает объект Intent. Он останавливает работу сервиса, вызывая 
его метод <code>onDestroy()</code></p>
<p>И в конце нам надо зарегистрировать сервис в файле манифеста:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>Регистрация сервиса производится в узле <code>application</code> с помощью добавления элемента <code>&lt;service&gt;</code>. В нем определяется атрибут 
<code>android:name</code>, который хранит название класса сервиса. И кроме того может принимать еще ряд атрибутов:</p>
<ul>
<li><p><code>android:enabled</code>: если имеет значение "true", то сервис может ли создаваться системой. Значение по умолчанию - "true".</p></li>
<li><p><code>android:exported</code>: указывает, могут ли компоненты других приложений обращаться к сервису. Если имеет значение "true", то могут, 
если имеет значение "false", то нет.</p></li>
<li><p><code>android:icon</code>: значок сервиса, представляет собой ссылку на ресурс drawable</p></li>
<li><p><code>android:isolatedProcess</code>: если имеет значение true, то сервис может быть запущен как специальный процесс, изолированный от остальной системы.</p></li>
<li><p><code>android:label</code>: название сервиса, которое отображается пользователю</p></li>
<li><p><code>android:permission</code>: набор разрешений, которые должно применять приложение для запуска сервиса</p></li>
<li><p><code>android:process</code>: название процесса, в котором запущен сервис. Как правило, имеет то же название, что и пакет приложения.</p></li>
</ul>
<p>Запустим приложение и нажмем на кнопку запуска сервиса:</p>
<img src="./pics/androidservice.png" alt="Сервисы startService в Android и Java" />
<p>После этого начнется воспроизведение добавленной нами в приложение мелодии.</p>
...
<div class="nav"><p><a href="./9.2.php">Назад</a><a href="./">Содержание</a><a href="./18.1.php">Вперед</a></p></div></div>
<div class="socBlock">
