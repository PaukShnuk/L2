...
<html >
<head>
<title>Java и Android | Создание графического интерфейса</title>
<meta charset="utf-8" />
<meta name="description" content="Класс Activity, определение графического интерфейса и ресурсы в Android, классы MainActivity и AppCompatActivity, файл activity_main.xml">
...
<h2>Создание графического интерфейса</h2><div class="date">Последнее обновление: 14.10.2021</div>
...
<p>В прошлой теме мы рассмотрели создание простого приложения, который предлагает Android Studio по умолчанию и 
которое просто выводит на экран строку Hello Android.</p>
<img src="./pics/1.4.png" alt="Тестирование приложения в Android Studio"/>
<p>Но почему у нас выводится именно эта строка? Почему у нас вообще создается именно такой визуальный интерфейс?</p>
<p>Выполнение приложения Android по умолчанию начинается с класса <span class="b">MainActivity</span>, который по умолчанию открыт в Android Studio:</p>
...
<p>Каждый отдельный экран или страница в приложении описывается таким понятием как activity. В литературе могут использоваться различные термины: 
экран, страница, активность. В данном случае я буду использовать понятие "activity". Так вот, если мы запустим приложение на устройстве, 
то на экране мы по сути увидим определенную activity, которая предсталяет данный интерфейс.</p>
<p>Класс MainActivity по сути представляет обычный класс java, в начале которого идет определение пакета данного класса:</p>
<pre class="brush:java;">package com.example.helloapp;</pre>
<p>Далее идет импорт классов из других пакетов, функциональность которых используется в MainActivity:</p>
<pre class="brush:java;">
import androidx.appcompat.app.AppCompatActivity;
import android.os.Bundle;
</pre>
<p>Затем идет собственно определение класса:</p>
<pre class="brush:java;">public class MainActivity extends AppCompatActivity </pre>
<p>По умолчанию MainActivity наследуется от класса AppCompatActivity, который выше подключен с помощью директивы импорта. 
Класс <span class="b">AppCompatActivity</span> по сути представляет отдельный экран (страницу) приложения или его визуальный интерфейс. И MainActivity наследует 
весь этот функционал.</p>
<p>По умолчанию MainActivity содержит только один метод <code>onCreate()</code>, в котором фактически и создается весь интерфейс приложения:</p>
<pre class="brush:java;">
protected void onCreate(Bundle savedInstanceState) {
	super.onCreate(savedInstanceState);
	setContentView(R.layout.activity_main);
...
<p>В метод <span class="b">setContentView()</span> передается ресурс разметки графического интерфейса:</p>
<pre class="brush:java;">
setContentView(R.layout.activity_main);
</pre>
<p>Именно здесь и решается, какой именно визуальный интерфейс будет иметь MainActivity. Но что в данном случае представляет ресурс 
<span class="b">R.layout.activity_main</span>? Это файл <span class="b">activity_main.xml</span> из папки <code>res/layout</code> 
(в принципе можно заметить, что название ресурса соответствует названию файла), который также 
по умолчанию открыт в Android Studio:</p>
<img src="./pics/graphicview2.png" alt="Режим дизайнера в Android Studio" />
<h3>Файл activity_main.xml</h3>
<p>Android Studio позволяет работать с визуальным интерфейсом как в режиме кода, так и в графическом режиме. Так, по умолчанию файл открыт в графическом 
режиме, и мы наглядно можем увидеть, как у нас примерно будет выглядеть экран приложения. И даже набрасать с панели инструментов какие-нибудь 
элементы управления, например, кнопки или текстовые поля.</p>
<p>Но также мы можем работать с файлом в режиме кода, поскольку <span class="b">activity_main.xml</span> - это обычный текстовый файл с разметкой xml. 
Для переключения к коду нажмем на кнопку <span class="b">Code</span> над графическим представлением. (Дополнительно с помощью кнопки <span class="b">Split</span> 
можно переключиться на комбинированное представление код + графический дизайнер)</p>
<img src="./pics/graphicview3.png" alt="Переключене к коду layout activity_main.xml в Android Studio" />
<p>Здесь мы увидим, что на уровне кода файл <span class="b">activity_main.xml</span> содержит следующую разметку:</p>
<pre class="brush:xml;">
...
<p>Весь интерфейс представлен элементом-контейнером <span class="b">androidx.constraintlayout.widget.ConstraintLayout</span>:</p>
<pre class="brush:xml;">
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
    xmlns:app="http://schemas.android.com/apk/res-auto"
...
<p>ConstraintLayout позволяет расположить вложенные элементы в определенных местах экрана. Вначале элемента ConstraintLayout идет определение пространств имен XML:</p>
<pre class="brush:xml;">
xmlns:android="http://schemas.android.com/apk/res/android"
xmlns:app="http://schemas.android.com/apk/res-auto"
xmlns:tools="http://schemas.android.com/tools"
</pre>
<p>Каждое пространство имен задается следующим образом: <code>xmlns:префикс="название_ресурса"</code>. Например, в</p>
<pre class="brush:xml;">
xmlns:android="http://schemas.android.com/apk/res/android"
</pre>
<p>Название ресурса (или URI - Uniform Resource Indicator) - <code>"http://schemas.android.com/apk/res/android"</code>. И этот ресурс сопоставляется с 
префиксом <code>android</code> (<code>xmlns:android</code>).</p>
<p>Зачем эти пространства имен нужны? Каждый ресурс или URI определяет некоторую функциональность, которая используется в приложении, например, предоставляют 
теги и атрибуты, которые необходимые для построения приложения.<p>
<ul>
<li><p><code>xmlns:android="http://schemas.android.com/apk/res/android"</code>: содержит основные атрибуты, которые предоставляются платформой Android, 
применяются в элементах управления и определяют их визуальные свойства (например, размер, позиционирование)</p></li>
<li><p><code>xmlns:app="http://schemas.android.com/apk/res-auto"</code>: содержит атрибуты, которые определены в рамках приложения</p></li>
<li><p><code>xmlns:tools="http://schemas.android.com/tools"</code>: применяется для работы с режиме дизайнера в Android Studio</p></li>
</ul>
<p>И чтобы упростить работу с этими ресурсами, применяются префиксы. Например, дальше мы видим:</p>
<pre class="brush:xml;">
android:layout_width="match_parent"
android:layout_height="match_parent"
tools:context=".MainActivity"&gt;
</pre>
<p><span class="b">android:layout_width</span> определяет ширину контейнера. Этот атрибут (<code>layout_width</code>) расположен в ресурсе "http://schemas.android.com/apk/res/android". 
И поскольку этот ресурс сопоставляется с префиксом <code>android</code>, то для обращения к атрибуту перед ним через двоеточие указывается префикс 
данного ресурса.</p>
<p>Значением атрибута <code>android:layout_weight</code> является <span class="b">"match_parent"</span>. Это значит, что элемент (ConstraintLayout) 
будет растягиваться по всей ширине контейнера (экрана устройства).</p>
<p>Атрибут <span class="b">android:layout_height="match_parent"</span> определяет высоту контейнера и также определен в "http://schemas.android.com/apk/res/android". 
Значение <code>"match_parent"</code> указывает, что  ConstraintLayout будет растягивается по всей длине контейнера (экрана устройства).</p>
<p>Атрибут <span class="b">tools:context</span> определяет, какой класс activity (экрана приложения) связан с текущим определением интерфейса. 
В данном случае это класс MainActivity. Это позволяет использовать в Android Studio различные возможности в режиме дизайнера, которые зависят от класса activity.</p>
<h4>TextView</h4>
<p>Текстовое поле устанавливает текст с помощью атрибута <code>android:text</code>.</p>
<pre class="brush:xml;">
&lt;TextView
        android:layout_width="wrap_content"
        android:layout_height="wrap_content"
...
<ul>
<li><p><span class="b">android:layout_width</span> устанавливает ширину виджета. Значение <code>wrap_content</code> задает 
для виджета величину, достаточную для отображения в контейнере.</p></li>
<li><p><span class="b">android:layout_height</span> устанавливает высоту виджета. Значение <code>wrap_content</code> аналогично установке ширины 
задает для виджета высоту, достаточную для отображения в контейнере</p></li>
<li><p><span class="b">android:text</span> устанавливает текст, который будет выводиться в TextView (в данном случае это строка "Hello World!")</p></li>
<li><p><span class="b">app:layout_constraintLeft_toLeftOf="parent"</span>: указывает, что левая граница элемента будет выравниваться по левой стороне 
контейнера ConstraintLayout</p>
<p>Обратите внимание, что этот атрибут определен в пространстве имен с префиксом app, то есть в "http://schemas.android.com/apk/res-auto".</p></li>
</li>
<li><p><span class="b">app:layout_constraintTop_toTopOf="parent"</span>: указывает, что верхняя граница элемента будет выравниваться по верхней стороне 
контейнера ConstraintLayout</p></li>
<li><p><span class="b">app:layout_constraintRight_toRightOf="parent"</span>: указывает, что правая граница элемента будет выравниваться по правой стороне 
контейнера ConstraintLayout</p></li>
<li><p><span class="b">app:layout_constraintBottom_toBottomOf="parent"</span>: указывает, что нижняя граница элемента будет выравниваться по нижней стороне 
контейнера ConstraintLayout</p></li>
</ul>

<p>Стоит отметить, что последние четыре атрибута вместе будут приводить к расположению TextView по центру экрана.</p>
<p>Таким образом, при запуске приложения сначала запускается класс MainActivity, который в качестве графического интерфейса устанавливает разметку из файла 
activity_main.xml. И поскольку в этой разметке прописан элемент TextView, который представляет некоторый текст, то мы и увидим его текст на экране смартфона.</p>
