...
<html >
<head>
<title>Java и Android | Ресурсы изображений</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление изображений в Android Studio и Java, ресурсы drawable, вывод картинок с помощью ImageView">
...
<h1>Работа с изображениями</h1><h2>Ресурсы изображений</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>Одним из наиболее распространенных источников ресурсов являются файлы изображений. Android поддерживает следующие форматы файлов: 
<span class="ii">.png</span> (предпочтителен), <span class="ii">.jpg</span> (приемлем), <span class="ii">.gif</span> (нежелателен). Для графических файлов в проекте уже по умолчанию создана папка 
<span class="b">res/drawable</span>. По умолчанию она уже содержит ряд файлов - пару файлов иконок:</p>
<img src="./pics/imageview1.png" alt="Работа с изображениями в Android" />
<p>При добавлении графических файлов в эту папку для каждого из них Android создает ресурс 
<span class="b">Drawable</span>. После этого мы можем обратиться к ресурсу следующим образом в коде Java:</p>
...
<p>Например, добавим в проект в папку <span class="b">res/drawable</span> какой-нибудь файл изображения. Для этого скопируем на жестком диске какой-нибудь файл 
с расширением png или jpg и вставим его в папку <span class="b">res/drawable</span> (для копирования в проект используется простой Copy-Paste)</p>
<p>Далее нам будет предложено выбрать папку - <span class="b">drawable</span> или <span class="b">drawable-24</span>. Для добавления обычных файлов 
изображений выберем <span class="b">drawable</span>:</p>
<img src="./pics/imageview2.png" alt="Добавление изображений в папку drawable Android Studio" />

<p>Здесь сразу стоит учесть, 
что файл изображения будет добавляться в приложение, тем самым увеличивая его размер. Кроме того, большие изображения отрицательно влияют на производительность. 
Поэтому лучше использовать небольшие и оптимизрованные (сжатые) графические файлы. Хотя, также стоит отметить, что все файлы изображений, которые добавляются в эту папку, могут автоматически оптимизироваться с помощью утилиты <span class="b">aapt</span> 
во время построения проекта. Это позволяет уменьшить размер файла без потери качества.</p>
<p>При копировании файла нам будет предложено установить для него новое имя.</p>
<img src="./pics/imageview3.png" alt="Ресурсы drawable в Android и Java" />
<p>Можно изменить название файла, а можно оставить так как есть. В моем случае файл называется <span class="b">dubi2.png</span>. И затем нажмем на кнопку Refactor. И после этого в папку drawable будет добавлен 
выбранный нами файл изображения.</p>
<img src="./pics/imageview4.png" alt="Добавление изображений в Android Studio" />
<p>Для работы с изображениями в Android можно использовать различные элементы, но непосредственно для вывода изображений предназначен <span class="b">ImageView</span>. 
Поэтому изменим файл <span class="b">activity_main.xml</span> следующим образом:</p>
<pre class="brush:xml;">
...
<p>В данном случае для отображения файла в ImageView у элемента устанавливается атрибут <span class="b">android:src</span>. В его значении 
указывается имя графического ресурса, которое совпадает с именем файла без расширения. 
И после этого уже в Preview или в режиме дизайнере в Android Studio можно будет увидеть применение изображения, либо при запуске приложения:</p>
<img src="./pics/imageview5.png" alt="ImageView и drawable в Android Studio и Java" />
<p>Если бы мы создавали ImageView в коде java и из кода применяли бы ресурс, то activity могла бы выглядеть так:</p>
<pre class="brush:java;">
...
<p>В данном случае ресурс drawable напрямую передается в метод <code>imageView.setImageResource()</code>, и таким образом устанавливается изображение. В результате 
мы получим тот же результат.</p>
<pre class="brush:java;">imageView.setImageResource(R.drawable.dubi2);</pre>
<p>Однако может возникнуть необходимость как-то обработать ресурс перед использованием или использовать его в других сценариях. В этом случае мы можем сначала 
получить его как объект Drawable и затем использовать для наших задач:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>Для получения ресурса применяется метод <code>ResourcesCompat.getDrawable()</code>, в который передается объект Resources, идентификатор ресурса и тема. 
В данном случае тема нам не важна, поэтому для нее передаем значение null. Возвращается ресурс в виде объекта <span class="b">Drawable</span>:</p>
<pre class="brush:java;">Drawable drawable = ResourcesCompat.getDrawable(res, R.drawable.dubi2, null);</pre>
<p>Затем, например, можно также передать ресурс объекту ImageView через его метод <code>setImageDrawable()</code></p>
<pre class="brush:java;">imageView.setImageDrawable(drawable);</pre>
...
<div class="nav"><p><a href="./2.10.php">Назад</a><a href="./">Содержание</a><a href="./4.5.php">Вперед</a></p></div></div>
<div class="socBlock">
