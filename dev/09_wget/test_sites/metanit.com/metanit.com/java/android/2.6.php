...
<html >
<head>
<title>Java и Android | Ресурсы dimension</title>
<meta charset="utf-8" />
<meta name="description" content="Ресурсы dimension, их получение и использование в Android, определение размеров, метод getDimension, перевод в пиксели">
...
<h2>Ресурсы dimension</h2><div class="date">Последнее обновление: 15.10.2021</div>
...
<p>Определение размеров должно находиться в папке <span class="b">res/values</span> в файле с любым произвольным именем. Общий синтаксис определения 
ресурса следующий:</p>
<pre class="brush:xml;">
...
<code>&lt;dimen&gt;</code> обозначает ресурс и в качестве значния принимает некоторое значение размера в одной из принятых единиц измерения  
(dp, sp, pt, px, mm, in). Более подробно установка размеров рассматривалась в одной из прошлых статей - <a href="https://metanit.com/java/android/3.13.php">Определение размеров</a></p>
...
<img src="./pics/2.19.png" alt="Ресурс Dimension в Android Studio и Java, dimens.xml" />
...
<p>Здесь определены два ресурса для отступов <code>horizontal_margin</code> и <code>vertical_margin</code>, которые хранят соответственно значения 64dp и 32dp, 
и ресурс text_size, который хранит высоту шрифта - 32sp. Названия ресурсов могут быть произвольными.</p>
<p>Используем ресурс в файле activity_main.xml:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>Ресурсы dimension используются для таких атриубтов визуальных элементов, которые в качестве значения требуют указание числового значения. Например, 
атриубут <code>android:layout_height</code> или <code>android:textSize</code>. Для получения ресурса в xml после <code>"@dimen/"</code> 
указывается имя ресурса.</p>
<img src="./pics/2.13.png" alt="Dimensions в Android и Java" />
<p>Для получения ресурсов в коде java применяется метод <span class="b">getDimension()</span> класса Resources. Например, определим в коде Java аналогичный визуальный интерфейс:</p>
...
<p>При программном получении ресурсов dimen с помощью метода <code>getDimension()</code> следует учитывать, что он возвращает значения в физических пикселях, 
...
поскольку большинство методов в Java принимают именно пиксели, а не dp или другие единицы, как например, метод <code>setMargins()</code>, который устанавливает отступы.</p>
<p>Однако метод <code>setTextSize()</code> принимает именно <span class="b">sp</span>, поэтому с помощью дополнительного параметра необходимо указать, 
что в данном случае имеются в виду физические пиксели, а не sp:</p>
<pre class="brush:java;">textView.setTextSize(TypedValue.COMPLEX_UNIT_PX, textSize);</pre>
<p>Либо, как вариант, с помощью класса <span class="b">TypedValue</span> программно перевести физические пиксели в sp или другую единицу измерения.</p>
