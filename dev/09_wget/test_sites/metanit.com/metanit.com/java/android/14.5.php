...
<html >
<head>
<title>Java и Android | Подключение к базе данных SQLite</title>
<meta charset="utf-8" />
<meta name="description" content="Подключение к базе данных SQLite в Android и Java, основные классы android.database.sqlite, SQLiteDatabase, rawQuery, openOrCreateDatabase">
...
<h1>Работа с базами данных SQLite</h1><h2>Подключение к базе данных SQLite</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>В Android имеется встроенная поддержка одной из распространенных систем управления базами данных - SQLite. 
Для этого в пакете <span class="b">android.database.sqlite</span> определен набор классов, которые позволяют работать с базами данных SQLite. 
И каждое приложение может создать свою базу данных.</p>
<p>Чтобы использовать SQLite в Android, надо создать базу данных с помощью выражение на языке SQL. После этого база данных будет храниться в каталоге приложения 
по пути:</p>
<pre class="brush:js;">
DATA/data/[Название_приложения]/databases/[Название_файла_базы_данных]
</pre>
<p>ОС Android по умолчанию уже содержит ряд встроенных бад SQLite, которые используются стандартными программами - для списка контактов, для хранения 
фотографий с камеры, музыкальных альбомов и т.д.</p>
<p>Основную функциональность по работе с базами данных предоставляет пакет <span class="b">android.database</span>. Функциональность непосредственно для 
работы с SQLite находится в пакете <span class="b">android.database.sqlite</span>.</p>
<p>База данных в SQLite представлена классом <span class="b">android.database.sqlite.SQLiteDatabase</span>. Он позволяет выполнять запросы к бд, выполнять с ней 
различные манипуляции.</p>
<p>Класс <span class="b">android.database.sqlite.SQLiteCursor</span> предоставляет запрос и позволяет возвращать набор строк, которые соответствуют этому запросу.</p>
<p>Класс <span class="b">android.database.sqlite.SQLiteQueryBuilder</span> позволяет создавать SQL-запросы.</p>
<p>Сами sql-выражения представлены классом <span class="b">android.database.sqlite.SQLiteStatement</span>, которые позволяют с помощью плейсхолдеров 
вставлять в выражения динамические данные.</p>
<p>Класс <span class="b">android.database.sqlite.SQLiteOpenHelper</span> позволяет создать базу данных со всеми таблицами, если их еще не существует.</p>
<p>В SQLite применяется следующая система типов данных:</p>
<ul>
<li><p><span class="b">INTEGER</span>: представляет целое число, аналог типу <code>int</code> в java</p></li>
<li><p><span class="b">REAL</span>: представляет число с плавающей точкой, аналог <code>float</code> и <code>double</code> в java</p></li>
<li><p><span class="b">TEXT</span>: представляет набор символов, аналог <code>String</code> и <code>char</code> в java</p></li>
<li><p><span class="b">BLOB</span>: представляет массив бинарных данных, например, изображение, аналог типу <code>int</code> в java</p></li>
</ul>
<p>Сохраняемые данные должны представлять соответствующие типы в java.</p>
<h3>Создание и открытие базы данных</h3>
<p>Для создания или открытия новой базы данных из кода Activity в Android мы можем вызвать метод <span class="b">openOrCreateDatabase()</span>. Этот метод 
может принимать три параметра:</p>
<ul>
<li><p>название для базы данных</p></li>
<li><p>числовое значение, которое определяет режим работы (как правило, в виде константы <code>MODE_PRIVATE</code>)</p></li>
<li><p>необязательный параметр в виде объекта <code>SQLiteDatabase.CursorFactory</code>, который представляет фабрику создания курсора для работы с бд</p></li>
</ul>
<p>Например, создание базы данных <code>app.db</code>:</p>
<pre class="brush:java;">SQLiteDatabase db = getBaseContext().openOrCreateDatabase("app.db", MODE_PRIVATE, null);</pre>
<p>Для выполнения запроса к базе данных можно использовать метод <span class="b">execSQL</span> класса SQLiteDatabase. В этот метод передается SQL-выражение. 
Например, создание в базе данных таблицы users:</p>
<pre class="brush:java;">
SQLiteDatabase db = getBaseContext().openOrCreateDatabase("app.db", MODE_PRIVATE, null);
db.execSQL("CREATE TABLE IF NOT EXISTS users (name TEXT, age INTEGER)");
</pre>
<p>Если нам надо не просто выполнить выражение, но и получить из бд какие-либо данные, то используется метод <span class="b">rawQuery()</span>. 
Этот метод в качестве параметра принимает SQL-выражение, а также набор значений для выражения sql. Например, получение всех объектов из базы данных:</p>
<pre class="brush:java;">
SQLiteDatabase db = getBaseContext().openOrCreateDatabase("app.db", MODE_PRIVATE, null);
db.execSQL("CREATE TABLE IF NOT EXISTS users (name TEXT, age INTEGER)");
...
<p>Метод <code>db.rawQuery()</code> возвращает объект Cursor, с помощью которого мы можем извлечь полученные данные.</p>
<p>Возможна ситуация, когда в базе данных не будет объектов, и для этого методом <code>query.moveToFirst()</code> пытаемся переместиться к первому 
объекту, полученному из бд. Если этот метод возвратит значение false, значит запрос не получил никаких данных из бд.</p>
<p>Теперь для работы с базой данных сделаем простейшее приложение. Для этого создадим новый проект.</p>
<p>В файле <span class="b">activity_main.xml</span> определим простейший графический интерфейс:</p>
<pre class="brush:xml;">
...
<p>А в классе <span class="b">MainActivity</span> определим взаимодействие с базой данных:</p>
<pre class="brush:java;">
...
<p>По нажатию на кнопку здесь вначале создается в базе данных app.db новая таблица users, а затем в нее добавляются два объекта в базу данных с помощью SQL-выражения INSERT.</p>
<p>Далее с помощью выражения SELECT получаем всех добавленных пользователей из базы данных в виде курсора Cursor.</p>
<p>Вызовом <code>query.moveToNext()</code> перемещаемся в цикле <code>while</code> последовательно по всем объектам.</p>
<p>Для получения данных из курсора применяются методы <code>query.getString(0)</code> и <code>query.getInt(1)</code>. В скобках в методы передается 
...
будет идти строкое значение, которое получаем с помощью метода <code>getString()</code>, а следующим - первым столбцом идет числовое значение, для 
которого применяется метод <code>getInt()</code>.</p>
<p>После завершения работы с курсором и базой данных мы закрываем все связанные объекты:</p>
<pre class="brush:java;">
query.close();
db.close();
</pre>
<p>Если мы не закроем курсор, то можем столкнуться с проблемой утечки памяти.</p>
<p>И если мы обратимся к приложению, то после нажатия на кнопку в текстовое поле будут выведены добавленные данные:</p>
<img src="./pics/sqlitedatabase.png" alt="Работа с классом SQLiteDatabase в Android и Java" />
...
<div class="nav"><p><a href="./13.2.php">Назад</a><a href="./">Содержание</a><a href="./14.1.php">Вперед</a></p></div></div>
<div class="socBlock">
