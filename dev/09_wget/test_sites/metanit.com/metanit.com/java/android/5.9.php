...
<html >
<head>
<title>Java и Android | Добавление и удаление в ArrayAdapter и ListView</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление и удаление в ListView в Android и Java, методы ArrayAdapter, обработка выделения элемента в списке, обновление адаптера с помощью метода notifyDataSetChanged">
...
<h2>Добавление и удаление в ArrayAdapter и ListView</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>После привязки ListView к источнику данных через адаптер мы можем работать с данными - добавлять, удалять, изменять только через адаптер. ListView служит только 
для отображения данных.</p>
<p>Для управления данными мы можем использовать методы адаптера или напрямую источника данных. Например, класс ArrayAdapter предоставляет следующие методы 
для управления данными:</p>
<ul>
<li><p><span class="b">void	add(T object)</span>: добавляет элемент object в конец массива</p></li>
<li><p><span class="b">void	addAll(T... items)</span>: добавляет все элементы items в конец массива</p></li>
<li><p><span class="b">void	addAll(Collection&lt;? extends T&gt; collection)</span>: добавляет коллекцию элементов collection в конец массива</p></li>
<li><p><span class="b">void	clear()</span>: удаляет все элементы из списка</p></li>
<li><p><span class="b">void	insert(T object, int index)</span>: добавляет элемент object в массив по индексу index</p></li>
<li><p><span class="b">void	remove(T object)</span>: удаляет элемент object из массива</p></li>
</ul>
<p>Однако после применения вышеуказанных методов изменения коснутся только массива, выступающего источником данных. Чтобы синхронизировать изменения 
с элементом <code>ListView</code>, надо вызвать у адаптера метод <code>notifyDataSetChanged()</code>.</p>
<p>Например, определим в файле <span class="b">activity_main.xml</span> следующие элементы:</p>
<pre class="brush:xml;">
...
<p>Для вывода списка предназначен ListView с возможностью множественного выбора элементов. Для добавления и удаления определены две кнопки. Для 
ввода нового объекта в список предназначено поле EditText.</p>
...
<p>С добавлением все относительно просто: получаем введенную строку и добавляем в список с помощью метода <span class="b">adapter.add()</span>. Чтобы обновить ListView 
после добавления вызывается метод <span class="b">adapter.notifyDataSetChanged()</span>.</p>
<p>А для удаления создается дополнительный список selectedUsers, который будет содержать выделенные элементы. Для получения выделенных элементов и добавления 
их в список используется слушатель <span class="b">AdapterView.OnItemClickListener</span>, метод <code>onItemClick()</code> которого вызывается при установке или снятия отметки с элемента, то есть 
при любом нажатии на элемент.</p>
<p>По нажатию на кнопку удаления пробегаемся по списку выделенных элементов и вызываем для каждого из них метод <span class="b">adapter.remove()</span>.</p>
<img src="./pics/listview2.png" alt="Добавление в ListView и ArrayAdapter в Android и Java" />
<img src="./pics/listview.png" alt="удаление в ListView и ArrayAdapter в Android и Java" />
...
<div class="nav"><p><a href="./5.2.php">Назад</a><a href="./">Содержание</a><a href="./5.6.php">Вперед</a></p></div></div>
<div class="socBlock">
