...
<html >
<head>
<title>Java и Android | Провайдеры контента. Контакты</title>
<meta charset="utf-8" />
<meta name="description" content="Провайдеры контента (Content Providers) в Android, типы провайдеров, получение контактов пользователя и установка разрешений в Android и Java">
...
<h1>Провайдеры контента</h1><div class="date">Последнее обновление: 18.10.2021</div>
...
<p>Наше приложение может сохранять разнообразую информацию о пользователе, какие-то связанные данные в файлах или настройках. Однако ОС Android уже хранит 
ряд важной информации, связанной с пользователем, к которой имеем доступ и которую мы можем использовать. Это и списки контактов, и файлы сохраненных 
изображений и видеоматериалов, и какие-то отметки о звонках и т.д., то есть некоторый контент. А для доступа к этому контенту в OC Android определены 
<span class="b">провайдеры контента (content provider)</span></p>
<p>В Android имеются следующие встроенные провайдеры, определенные в пакете <span class="b">android.content</span>:</p>
<ul>
<li><p><span class="b">AlarmClock</span>: управление будильником</p></li>
<li><p><span class="b">Browser</span>: история браузера и закладки</p></li>
<li><p><span class="b">CalendarContract</span>: каледарь и информаци о событиях</p></li>
<li><p><span class="b">CallLog</span>: информация о звонках</p></li>
<li><p><span class="b">ContactsContract</span>: контакты</p></li>
<li><p><span class="b">MediaStore</span>: медиа-файлы</p></li>
<li><p><span class="b">SearchRecentSuggestions</span>: подсказки по поиску</p></li>
<li><p><span class="b">Settings</span>: системные настройки</p></li>
<li><p><span class="b">UserDictionary</span>: словарь слов, которые используются для быстрого набора</p></li>
<li><p><span class="b">VoicemailContract</span>: записи голосовой почты</p></li>
</ul>
<a name="contacts"></a>
<h2>Работа с контактами</h2>
<p>Контакты в Android обладают встроенным API, который позволяет получать и изменять список контактов. Все контакты хранятся в базе данных SQLite, 
однако они не представляют единой таблицы. Для контактов отведено три таблицы, связанных отношением один-ко-многим: таблица для хранения информации о людях, таблица их телефонов и 
и таблица адресов их электронных почт. Но благодаря Android API мы можем абстрагироваться от связей между таблицами.</p>
<p>Общая форма получения контактов выглядит следующим образом:</p>
...
<p>Все контакты и сопутствующий функционал хранятся в специальных базах данных SQLite. Но нам не надо напрямую работать с ними. Мы можем воспользоваться 
объектом класса <span class="b">Cursor</span>. Чтобы его получить, сначала вызывается метод <code>getContentResolver()</code>, который возвращает 
объект <span class="b">ContentResolver</span>. Затем по цепочке вызывается метод <code>query()</code>. В этот метод передается ряд параметров, 
первый из которых представляет URI - ресурс, который мы хотим получить. Для обращения к базе данных контактов используется константа 
<span class="b">ContactsContract.Contacts.CONTENT_URI</span></p>
<p>Метод <code>contactsCursor.moveToNext()</code> позволяет последовательно перемещаться по записям контактов, считывая по одному контакту через 
вызов <code>contactsCursor.getString()</code>.</p>
<p>Таким образом, получать контакты не сложно. Главная сложность в работе с контактами, да и с любыми другими провайдерами контента, заключается в установке 
разрешений. До Android API 23 достаточно было установить соответствующее разрешение в файле манифеста приложения. Начиная же с API 23 (Android Marshmallow) 
Google изменил схему работы с разрешениями. И теперь пользователь сам должен решить, будет ли он давать разрешения приложению. В связи с чем разработчики должны добавлять дополнительный код.</p>
<p>Итак, для доступа к контактам нам надо установить разрешение <span class="b">android.permission.READ_CONTACTS</span> в файле манифеста приложения:</p>
<pre class="brush:xml;">
...
<p>Для вывода списка контактов в файле <span class="b">activity_main.xml</span> определим следующую разметку интерфейса:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>Для вывода списка контактов воспользуемся элементом ListView. И в классе <span class="b">MainActivity</span> получим контакты:</p>
<pre class="brush:java;">
package com.example.contactsapp;
...
<p>Кроме собственно загрузки контактов и передачи их через адаптер ArrayAdapter в список ListView  здесь добавлено много кода по управлению разрешениями. 
Вначале определена переменная <span class="b">READ_CONTACTS_GRANTED</span>, которая указывает, было ли выдано разрешение. И здеь есть два варианта действий.</p>
<p>Первый вариант предполагает, что устройство имеет версию Android ниже Marshmallow (ниже API 23). Для этого мы просто узнаем, есть ли разрешение для 
<code>READ_CONTACTS</code> и если оно есть, то устанавливаем для переменной READ_CONTACTS_GRANTED значение <code>true</code>:</p>
<pre class="brush:java;">
int hasReadContactPermission = ContextCompat.checkSelfPermission(this, Manifest.permission.READ_CONTACTS);
if(hasReadContactPermission == PackageManager.PERMISSION_GRANTED){
    READ_CONTACTS_GRANTED = true;
...
<p>Иначе нам надо отобразить пользователю диалоговое окно, где он решить, надо ли дать приложению разрешение:</p>
<pre class="brush:java;">
ActivityCompat.requestPermissions(this, 
								new String[]{Manifest.permission.READ_CONTACTS}, 
								REQUEST_CODE_READ_CONTACTS);
</pre>
<p>В этот метод передаются три параметра. Первый - текущий контекст, то есть текущий объект Activity.</p>
<p>Второй параметр представляет набор разрешений, которые надо получить, в виде массива строк. Нам надо получить в данном случае только одно разрешение - 
Manifest.permission.READ_CONTACTS.</p>
<p>Третий параметр представляет код запроса, через который мы сможем получить ответ пользователя.</p>
<p>Если мы хотим получить выбор пользователя при установке разрешений, то нам надо переопределить в классе Activity метод <span class="b">onRequestPermissionsResult</span>:</p>
<pre class="brush:java;">
public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults){
...
<p>Первый параметр метода requestCode - это тот код запроса, который передавался в качестве третьего параметра в 
<code>ActivityCompat.requestPermissions()</code>. Второй параметр - массив строк, для которых устанавливались разрешения. То есть одномоментно мы можем устанавливать 
сразу несколько разрешений.</p>
<p>Третий параметр собственно хранит числовые коды разрешений. Так мы запрашиваем только одно разрешение, то первый элемент массива будет хранить его код. 
Через условное выражение мы можем проверить этот код: <code>grantResults[0] == PackageManager.PERMISSION_GRANTED</code>. И в зависимости от результата 
проверки изменить переменную READ_CONTACTS_GRANTED.</p>
<p>И при запуске приложения нам сначала отобразится окно для выдачи разрешения, а после выдачи подтверждений список контактов:</p>
<img src="./pics/read_contacts1.png" alt="Чтение контактов в Android" />
<p>После выдачи разрешения при повторных запусках приложения повторять разрешение не нужно, поэтому метод <code>onRequestPermissionsResult()</code> в таком случае 
будет срабатывать только один раз. А переменная READ_CONTACTS_GRANTED в этом случае уже будет иметь значение true.</p>
<p>Другоая ситуация - если мы отклоним разрешение. В этом случае при повторном запуске приложения повторно будет отображаться данное окно.</p>
...
<div class="nav"><p><a href="./19.2.php">Назад</a><a href="./">Содержание</a><a href="./20.2.php">Вперед</a></p></div></div>
<div class="socBlock">
