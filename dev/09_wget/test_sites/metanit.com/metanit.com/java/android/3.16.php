...
<html >
<head>
<title>Java и Android | Программное создание ConstraintLayout и позиционионирование</title>
<meta charset="utf-8" />
<meta name="description" content="Программное создание ConstraintLayout и позиционионирование в Java в Android, класс ConstraintLayout.LayoutParams и его параметры, PARENT_ID">
...
<h2>Программное создание ConstraintLayout и позиционионирование</h2><div class="date">Последнее обновление: 14.10.2021</div>
...
<p>Для создания контейнера в коде Java применяется одноименный класс <span class="b">ConstraintLayout</span>, для создания объекта которого в конструктор передаются 
значения для ширины и высоты элемента:</p>
<pre class="brush:java;">
ConstraintLayout.LayoutParams layoutParams = new ConstraintLayout.LayoutParams
                (ConstraintLayout.LayoutParams.WRAP_CONTENT , ConstraintLayout.LayoutParams.WRAP_CONTENT);
</pre>
<p>Первый параметр устанавливает щирину элемента, а второй - высоту. <code>ConstraintLayout.LayoutParams.WRAP_CONTENT</code> указывает, что элемент будет иметь те размеры, которые необходимы для того, чтобы вывести на 
экран его содержимое. Кроме <code>ConstraintLayout.LayoutParams.WRAP_CONTENT</code> можно применять константу 
<code>ConstraintLayout.LayoutParams.MATCH_CONSTRAINT</code>, которая аналогична применению значения "0dp" в атрибутах <code>layout_width</code> и <code>layout_height</code> 
и которая растягивает элемент по ширине или высоте контейнера.</p>
<p>Также можно использовать точные размеры, например:</p>
<pre class="brush:java;">
ConstraintLayout.LayoutParams layoutParams = new ConstraintLayout.LayoutParams
                (ConstraintLayout.LayoutParams.MATCH_CONSTRAINT, 200);
</pre>
<p>Для настройки позиционирования внутри ConstraintLayout применяется класс <span class="b">ConstraintLayout.LayoutParams</span>. Он имеет довольно много функционала. 
Рассмотрим в данном случае только те поля, которые позволяют установить расположение элемента:</p>
<ul>
<li><p><span class="b">baselineToBaseline</span>: выравнивает базовую линию элемента по базовой линии другого элемента, id которого присваивается свойству.</p></li>
<li><p><span class="b">bottomToBottom</span>: выравнивает нижнюю границу элемента по нижней границе другого элемента.</p></li>
<li><p><span class="b">bottomToTop</span>: выравнивает нижнюю границу элемента по верхней границе другого элемента.</p></li>
<li><p><span class="b">leftToLeft</span>: выравнивает левую границу элемента по левой границе другого элемента.</p></li>
<li><p><span class="b">leftToRight</span>: выравнивает левую границу элемента по правой границе другого элемента.</p></li>
<li><p><span class="b">rightToLeft</span>: выравнивает правую границу элемента по левой границе другого элемента.</p></li>
<li><p><span class="b">rightToRight</span>: выравнивает правую границу элемента по правой границе другого элемента.</p></li>
<li><p><span class="b">startToEnd</span>: выравнивает начало элемента по завершению другого элемента.</p></li>
<li><p><span class="b">startToStart</span>: выравнивает начало элемента по началу другого элемента.</p></li>
<li><p><span class="b">topToBottom</span>: выравнивает верхнюю границу элемента по нижней границе другого элемента.</p></li>
<li><p><span class="b">topToTop</span>: выравнивает верхнюю границу элемента по верхней границе другого элемента.</p></li>
<li><p><span class="b">endToEnd</span>: выравнивает заврешение элемента по завершению другого элемента.</p></li>
<li><p><span class="b">endToStart</span>: выравнивает завершение элемента по началу другого элемента.</p></li>
</ul>
<p>В качестве значения эти поля принимают id (идентификатор) элемента, относительно которого выполняется позиционирование. Если расположение устанавливается 
относительно контейнера ConstraintLayout, то применяется константа <span class="b">ConstraintLayout.LayoutParams.PARENT_ID</span></p>
...
<p>В данном случае значение <code>ConstraintLayout.LayoutParams.WRAP_CONTENT</code> для ширины и высоты указывает, что элемент будет иметь те размеры, которые необходимы для того, чтобы вывести на 
экран его содержимое.</p>
<p>Далее выравниваем левую границу элемента по левой стороне контейнера:</p>
<pre class="brush:java;">layoutParams.leftToLeft = ConstraintLayout.LayoutParams.PARENT_ID;</pre>
<p>Эта установка аналогична использованию атрибута <code>app:layout_constraintLeft_toLeftOf="parent"</code>.</p>
<p>Затем выравниваем верхнюю границу элемента по верхней стороне контейнера:</p>
<pre class="brush:java;">layoutParams.topToTop = ConstraintLayout.LayoutParams.PARENT_ID;</pre>
<p>Эта установка аналогична использованию атрибута <code>app:layout_constraintTop_toTopOf="parent"</code>.</p>
<p>И в конце применяем объект ConstraintLayout.LayoutParams к TextView:</p>
<pre class="brush:java;">constraintLayout.addView(textView);</pre>
<p>В итоге элемент TextView будет расположен в верхнем левом углу ConstraintLayout:</p>
<img src="./pics/constraintlayout.png" alt="Расположение элементов в ConstraintLayout в Android Studio" />
<p>Рассмотрим другой пример - установку расположения элементов относительно друг друга:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>При расположении одного элемента относительно другого, нам нужно знать id вторрого элемента. Если элемент определен в коде Java, то вначале надо сгенерировать идентификатор:</p>
<pre class="brush:java;">
editText.setId(View.generateViewId());
button.setId(View.generateViewId());
</pre>
<p>Затем можно применять идентификаторы элементов для установки позиционирование позиционионирование. Так, правая граница EditText выравнивается по 
левой границе кнопки:</p>
<pre class="brush:java;">
editTextLayout.rightToLeft = button.getId();
</pre>
<p>А левая граница кнопки выравнивается по правой границе элемента EditText:</p>
<pre class="brush:java;">
buttonLayout.leftToRight = editText.getId();
</pre>
...
<div class="nav"><p><a href="./3.15.php">Назад</a><a href="./">Содержание</a><a href="./3.2.php">Вперед</a></p></div></div>
<div class="socBlock">
