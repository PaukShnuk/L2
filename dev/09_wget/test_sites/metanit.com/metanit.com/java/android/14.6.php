...
<html >
<head>
<title>Java и Android | Модель, репозиторий и работа с базой данных</title>
<meta charset="utf-8" />
<meta name="description" content="Модель, репозиторий и работа с базой данных SQLite через ArrayAdapter в Android и Java, SQLiteOpenHelper и SQLiteDatabase">
...
<h2>Модель, репозиторий и работа с базой данных</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>В прошлых темах было рассмотрено взаимодействие с базой данных через класс SimpleCursorAdapter. Но есть и другие способы работы с данными, когда мы абстрагируемся от структуры таблицы и работаем 
через модель, а все взаимодействие с базой данных производится фактически через реализацию паттерна репозиторий.</p>
<p>Так, создадим новый проект с пустой MainActivity и прежде всего добавим в него класс модели, который назовем <span class="b">User</span>:</p>
...
<p>В данном проекте мы будем работть фактически с теми же данными, что и ранее с данными пользователей, у которых есть уникальный идентификатор, 
имя и год рождения. И модель User как раз описывает эти данные.</p>
<p>Для взаимодействия с базой данных SQLite добавим новый класс <span class="b">DatabaseHelper</span>:</p>
<pre class="brush:java;">
package com.example.databaseadapterapp;
...
<p>Также для работы с базой данных добавим в проект класс <span class="b">DatabaseAdapter</span>:</p>
<pre class="brush:java;">
package com.example.databaseadapterapp;
...
<p>Фактически данный класс выполняет роль репозитория данных. Чтобы взамодействовать с БД он определяет методы <code>open()</code> и <code>close()</code>, 
которые соответственно открывают и закрывают подключение к базе данных.</p>
<p>Непосредственно для работы с данными в классе определены методы <code>insert()</code> (добавление), <code>delete()</code> (удаление), 
<code>update()</code> (обновление), <code>getUsers()</code> (получение всех пользователей из таблицы) и <code>getUser()</code> (получение одного пользователя по id).</p>
<p>В качестве пользовательского интерфейса будем отталкиваться от того функционала, который использовался в прошлых темах. Так, добавим в проект новый 
класс Activity - <span class="b">UserActivity</span>. В итоге весь проект будет выглядеть следующим образом:</p>
<img src="./pics/db1.png" alt="Модель и репозиторий в Android и Java" />
<p>В файле <span class="b">activity_user.xml</span> в папке <span class="b">res/layout</span> определим для UserActivity простейший интерфейс:</p>
<pre class="brush:xml;">
...
<p>В классе <span class="b">UserActivity</span> опредлим логику добавления/изменения/удаления пользователя:</p>
<pre class="brush:java;">
package com.example.databaseadapterapp;
...
<p>Эта activity используется для добавления/редактирования/удаления одного объекта User. Если в UserActivity передается параметр id, то 
значит мы находимся в режиме редактирования пользователя, поэтому обращаемся к методу <code>getUser()</code> класса DatabaseAdapter для получения 
нужного пользователя.</p>
<p>Для добавления/изменения/удаления пользователя по нажатию на кнопку вызывается соответствующий метод класса DatabaseAdapter.</p>
<p>В файле <span class="b">activity_main.xml</span> в папке <span class="b">res/layout</span> определим визуальный интерфейс для MainActivity:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>Здесь имеется элемент ListView для вывода объектов из таблицы и кнопка для перехода к UserActivity для добавления пользователя.</p>
...
<p>В переопределенном методе <code>onResume()</code> через объект DatabaseAdapter получаем всех пользователей из базы данных и через ArrayAdapter выводим их в ListView.</p>
<p>При нажатии на элемент ListView запускаем UserActivity, передавая ей id выделенного пользователя.</p>
<p>При нажатии на кнопку просто вызываем UserActivity.</p>
<p>При запуске MainActivity отобразит список пользователей из базы данных, а при переходе к UserActivity мы сможем подредактировать или добавить пользователей:</p>
<img src="./pics/db2.png" alt="Работа с данными в SQLite через репозиторий в Android и Java" />
...
<div class="nav"><p><a href="./14.4.php">Назад</a><a href="./">Содержание</a><a href="./9.1.php">Вперед</a></p></div></div>
<div class="socBlock">
