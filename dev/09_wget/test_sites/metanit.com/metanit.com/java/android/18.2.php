...
<html >
<head>
<title>Java и Android | DialogFragment и создание диалоговых окон</title>
<meta charset="utf-8" />
<meta name="description" content="DialogFragment и создание своих диалоговых окон в Android и Java, класс AlertDialog">
...
<h2>DialogFragment и создание диалоговых окон</h2><div class="date">Последнее обновление: 18.10.2021</div>
...
<p>Для создания своих диалоговых окон используется компонент <span class="b">AlertDialog</span> в связке с классом фрагмента <span class="b">DialogFragment</span>. 
Рассмотрим их применение.</p>
<p>Вначале добавим в проект новый класс фрагмента, который назовем <span class="b">CustomDialogFragment</span>:</p>
...
<p>Класс фрагмента содержит всю стандартную функциональность фрагмента с его жизненным циклом, но при этом наследуется от 
класса <span class="b">DialogFragment</span>, который добавляет ряд дополнительных функций. И для его создания мы можем использвоать два способа:</p>
<ul>
<li><p>Переопределение метода <code>onCreateDialog()</code>, который возвращает объект <code>Dialog</code>.</p></li>
<li><p>Использование стандартного метода <code>onCreateView()</code>.</p></li>
</ul>
<p>Для создания диалогового окна в методе <code>onCreateDialog()</code> применяется класс <code>AlertDialog.Builder</code>. С помощью своих методов 
он позволяет настроить отображение диалогового окна:</p>
<ul>
<li><p><code>setTitle</code>: устанавливает заголовок окна</p></li>
<li><p><code>setView</code>: устанавливает разметку интерфейса окна</p></li>
<li><p><code>setIcon</code>: устанавливает иконку окна</p></li>
<li><p><code>setPositiveButton</code>: устанавливает кнопку подтверждения действия</p></li>
<li><p><code>setNeutralButton</code>: устанавливает "нейтральную" кнопку, действие которой может отличаться от действий подтверждения или отмены</p></li>
<li><p><code>setNegativeButton</code>: устанавливает кнопку отмены</p></li>
<li><p><code>setMessage</code>: устанавливает текст диалогового окна, но при использовании setView данный метод необязателен или может рассматриваться 
в качестве альтернативы, если нам надо просто вывести сообщение.</p></li>
<li><p><code>create</code>: создает окно</p></li>
</ul>
<p>В данном же случае диаговое кно просто выводит некоторое сообщение.</p>
<p>Для вызова этого диалогового окна в файле <span class="b">activity_main.xml</span> определим кнопку:</p>
<pre class="brush:xml;">
...
<p>В коде <span class="b">MainActivity</span> определим обработчик нажатия кнопки, который будет запускать диалоговое окно:</p>
<pre class="brush:java;">
package com.example.dialogsapp;
...
<p>Для вызова диалогового окна создается объект фрагмента CustomDialogFragment, затем у него вызывается метод <code>show()</code>. В этот метод передается 
менеджер фрагментов FragmentManager и строка - произвольный тег.</p>
<p>И поле нажатия на кнопку мы сможем ввести данные в диалоговое окно:</p>
<img src="./pics/dialog1.png" alt="DialogFragment и AlertDialog в Android и Java" />
<p>Теперь немного кастомизируем диалоговое окно:</p>
<pre class="brush:java;">
public Dialog onCreateDialog(Bundle savedInstanceState) {
...
<img src="./pics/dialog2.png" alt="Диалоговые окна в Android и Java" />
<p>Здесь добавляетcя иконка, которая в качестве изображения использует встроенный ресурс <code>android.R.drawable.ic_dialog_alert</code> и устанавливаются 
две кнопки. Для каждой кнопки можно установить текст и обработчик нажатия. В данном случае для обработчика нажатия передается null, то есть обработчик не установлен.</p>
<p>Теперь добавим в папку <span class="b">res/layout</span> новый файл <span class="b">dialog.xml</span>:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>И изменим создание диалогового окна:</p>
<pre class="brush:java;">
public Dialog onCreateDialog(Bundle savedInstanceState) {
...
<p>Метод <code>setView()</code> устанавливает в качестве интерфейса окна ранее добавленный ресурс layout <span class="b">dialog.xml</span>.</p>
<img src="./pics/dialog3.png" alt="Создание диалоговых окон в Android" />
<p>При этом, как можно увидеть на скриншоте, кнопки и заголовок с иконкой не входят в разметку.</p>
