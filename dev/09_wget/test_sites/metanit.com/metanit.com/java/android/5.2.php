...
<html >
<head>
<title>Java и Android | Выбор элемента в ListView</title>
<meta charset="utf-8" />
<meta name="description" content="Выбор элемента в ListView в Android и Java, использование слушателя OnItemClickListener, обработка выделения элемента в списке, множественный выбор">
...
<h2>Выбор элемента в ListView</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>В прошлых темах было рассмотрено, как можно загружать данные в ListView, связывать его с источником данных. 
Но кроме простого вывода списка элементов ListView позволяет выбирать элемент и обрабатывать его выбор. Рассмотрим, как это сделать. 
Определим следующую разметку в файле <span class="b">activity_main.xml</span>:</p>
<pre class="brush:xml;">
...
<p>Теперь свяжем список ListView с источником данных и закрепим за ним слушатель нажатия на элемент списка:</p>
...
<p>Итак, метод <code>setAdapter</code> связывает элемент ListView с определенным адаптером. Далее для обработки выбора элемента списка 
устанавливается слушатель <span class="b">OnItemClickListener</span>. Этот слушатель имеет один метод <span class="b">onItemClick</span>, через параметры которого 
мы можем получить выделенный элемент и сопутствующие данные. Так, он принимает следующие параметры:</p>
<ul>
<li><p><span class="b">parent</span> : нажатый элемент AdapterView (в роли которого в данном случае выступает наш элемент ListView)</p></li>
<li><p><span class="b">view</span> : нажатый виджет внутри AdapterView</p></li>
<li><p><span class="b">position</span> : индекс нажатого виждета внутри AdapterView</p></li>
<li><p><span class="b">id</span> : идентификатор строки нажатого элемента</p></li>
</ul>
<p>Используя эти параметры, мы можем разными способами получить выделенный элемент.</p>
<p>Например, в данном случае получая индекс нажатого виджета, который соответствует индексу элемента в массиве строк, мы можем устанавить соответствующий элемент в массиве строк и таким образом 
получить его текст:</p>
<pre class="brush:java;">
countriesList.setOnItemClickListener(new AdapterView.OnItemClickListener(){
	@Override
	public void onItemClick(AdapterView&lt;?&gt; parent, View v, int position, long id)
...
<img src="./pics/listview1.png" alt="элемент ListView в Android" />
<p>Также мы можем получить выделенный элемент из AdapterView, который передается в качестве первого параметра - <code>AdapterView&lt;?&gt; parent</code>. 
...
получить выделенный элемент так:</p>
<pre class="brush:java;">
countriesList.setOnItemClickListener(new AdapterView.OnItemClickListener(){
	@Override
	public void onItemClick(AdapterView&lt;?&gt; parent, View v, int position, long id)
...
<p>Метод <code>getItemAtPosition</code> возвращает выделенный элемент по индексу. Это может актуально, если мы используем в качестве источника данных не массив строк, созданный в коде Java, 
а, например, ресурс <code>&lt;string-array&gt;</code>, заданный в файле xml.</p>
<p>В-третьих, мы можем использовать выделенный элемент, который передается в качестве второго параметра - <code>View v</code>. Так, в данном случае 
адаптер использует в качестве типа разметки ресурс - <span class="b">android.R.layout.simple_list_item_1</span>, а это значит, что выделенный элемент 
представляет элемент <span class="b">TextView</span>, в котором выводится данный текст. Поэтому в данном случае мы также могли бы получить выделенный 
элемент так:</p>
<pre class="brush:java;">
countriesList.setOnItemClickListener(new AdapterView.OnItemClickListener(){
	@Override
...
<h3>Множественный выбор в списке</h3>
<p>Иногда требуется выбрать не один элемент, как по умолчанию, а несколько. Для этого, во-первых, в разметке списка надо установить 
атрибут <code>android:choiceMode="multipleChoice"</code>:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>Теперь определим в коде MainActivity обработку выбора элементов списка:</p>
<pre class="brush:java;">
package com.example.listapp;
...
<p>Ресурс <code>android.R.layout.simple_list_item_multiple_choice</code> представляет стандартную разметку, предоставляемую фреймворком, 
для создания списка с множественным выбором.</p>
<p>А при выборе элементов мы получаем все выбранные позиции в объект <span class="b">SparseBooleanArray</span>, затем пробегаемся по всему массиву, и если позиция 
элемента в массиве есть в SparseBooleanArray, то есть она отмечена, то добавляем отмеченный элемент в строку.</p>
<img src="./pics/multiplechoicelist1.png" alt="Множественный выбор в ListView в Android и Java" />
