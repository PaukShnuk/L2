...
<html >
<head>
<title>Java и Android | Оптимизация адаптера и View Holder</title>
<meta charset="utf-8" />
<meta name="description" content="Оптимизация адаптера для работы с ListView в Android и Java, паттерн View Holder, создание плавной и производительной прокрутки списка">
...
<h2>Оптимизация адаптера и View Holder</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>В прошлой теме был создан кастомный адаптер, который позволял работать со сложными списками объектов:</p>
...
<p>Но этот адаптер имеет один очень большой минус - при прокрутке в ListView, если в списке очень много объектов, то для каждого элемента, когда он попадет в зону видимости, 
будет повторно вызываться метод getView, в котором будет заново создаваться новый объект View. Соответственно будет увеличиваться потребление памяти и снижаться производительность. 
Поэтому оптимизируем код метода <span class="b">getView</span>:</p>
<pre class="brush:java;">
public View getView(int position, View convertView, ViewGroup parent) {
...
<p>Параметр <code>convertView</code> указывает на элемент View, который используется для объекта в списке по позиции position. Если ранее уже создавался View для этого объекта, 
то параметр convertView уже содержит некоторое значение, которое мы можем использовать.</p>
<p>В этом случае мы будем повторно использовать уже созданные объекты и увеличим производительность, однако этот код можно еще больше оптимизировать. 
Дело в том, что получение элементов по id тоже относительно затратная операция. Поэтому дальше оптимизируем код <span class="b">StateAdapter</span>, изменив его следующим образом:</p>
<pre class="brush:java;">
import android.content.Context;
import android.view.LayoutInflater;
import android.view.View;
...
<p>Для хранения ссылок на используемые элементы ImageView и TextView определен внутренний приватный класс ViewHolder, который в конструкторе получает объект View, 
содержащий ImageView и TextView.</p>
<p>В методе <code>getView</code>, если convertView равен null (то есть если ранее для объекта не создана разметка) создаем объект ViewHolder, который сохраняем в тег в convertView:</p>
<pre class="brush:java;">convertView.setTag(viewHolder);</pre>
<p>Если же разметка для объекта в ListView уже ранее была создана, то обратно получаем ViewHolder из тега:</p>
<pre class="brush:java;">viewHolder = (ViewHolder) convertView.getTag();</pre>
<p>Затем также для ImageView и TextView во ViewHolder устанавливаются значения из объекта State:</p>
<pre class="brush:java;">
viewHolder.imageView.setImageResource(state.getFlagResource());
viewHolder.nameView.setText(state.getName());
viewHolder.capitalView.setText(state.getCapital());
</pre>
<p>И теперь ListView особенно при больших списках будет работать плавнее и производительнее, чем в прошлой теме:</p>
<img src="./pics/customarrayadapter3.png" alt="Переопределение ArrayAdapter в Android и Java и ViewHolder" />
...
<div class="nav"><p><a href="./5.6.php">Назад</a><a href="./">Содержание</a><a href="./5.10.php">Вперед</a></p></div></div>
<div class="socBlock">
