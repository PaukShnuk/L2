...
<html >
<head>
<title>Java и Android | Checkbox</title>
<meta charset="utf-8" />
<meta name="description" content="Элемент CheckBox в Android и Java, обработка установка отметки во флажок, метод onCheckboxClicked и onCheckedChanged, OnCheckedChangeListener">
...
<h2>Checkbox</h2><div class="date">Последнее обновление: 15.10.2021</div>
...
<p>Элементы Checkbox представляют собой флажки, которые могут находиться в отмеченном и неотмеченном состоянии. Флажки позволяют производить множественный выбор из нескольких значений. Итак, определим в файле разметки 
<span class="b">activity_main.xml</span> элемент CheckBox:</p>
<pre class="brush:xml;">
...
<p>Атрибут <code>android:onClick</code>, как и в случае с простыми кнопками, позволяет задать обработчик нажатия на флажок. Определим обработчик нажатия в коде 
MainActivity:</p>
...
<p>В качестве параметра в обработчик нажатия onCheckboxClicked передается нажатый флажок. Обработчик срабатывает при каждом нажатии на checkBox. То есть и когда 
мы устанавливаем флажок, и когда мы снимем отметку. С помощью метода <span class="b">isChecked()</span> можно узнать, 
выделен ли флажок - в этом случае метод возвращает true.</p>
<img src="./pics/checkbox3.png" alt="Элемент Checkbox в Android и Java" />

<p>Подобным образом можно использовать несколько флажков:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>На каждый флажок можно повесить свой обработчик нажатия. А можно сделать один, как в данном случае. В этом случае мы 
можем обработать несколько флажков в коде java с помощью конструкции <code>switch...case</code></p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>С помощью конструкции switch...case можно получить id нажатого флажка и выполнить соответствующие действия.</p>
<img src="./pics/checkbox1.png" alt="Элемент Checkbox в Android" />
<p>Правда, если нам просто надо взять текст из выбранного флажка, то необязательно в данном случае использовать конструкцию switch, так как мы можем сократить весь код следующим образом:</p>
<pre class="brush:java;">
public void onCheckboxClicked(View view) {
    // Получаем флажок
...
<p>Однако в данном случае остается проблема: в текстовом поле отображается только один выделенный элемент. Изменим код MainActivity, чтобы отображать оба выделенных элемента:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<img src="./pics/checkbox2.png" alt="Выбор CheckBox в Android" />
<h3>OnCheckedChangeListener</h3>
<p>Применение слушателя <span class="b">OnCheckedChangeListener</span> представляет альтернативный способ отслеживания изменения флажка. 
Этот слушатель срабатывает, когда мы устанавливаем или убираем отметку на флажке. Например, определим следующий checkbox:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>В коде MainActivity подключим обработчик изменения состояния:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>Слушатель OnCheckedChangeListener определен в базовом классе <span class="b">CompoundButton</span> и определяет один метод - <span class="b">onCheckedChanged</span>. 
Первый параметр этого метода <code>buttonView</code> - сам измененный флажок CheckBox. А второй параметр <code>isChecked</code> указывает, отмечен ли флажок.</p>
<p>При изменении состояния флажка будет выводиться во всплывающем окне соответствующее уведомление:</p>
<img src="./pics/checkbox3.png" alt="CheckBox и OnCheckedChangeListener в Android" />
...
<div class="nav"><p><a href="./4.14.php">Назад</a><a href="./">Содержание</a><a href="./4.11.php">Вперед</a></p></div></div>
<div class="socBlock">
