...
<html >
<head>
<title>Java и Android | Загрузка данных и класс HttpURLConnection</title>
<meta charset="utf-8" />
<meta name="description" content="Загрузка данных из интернета и класс HttpURLConnection в Android и Java, работа с протоколом https">
...
<h2>Загрузка данных и класс HttpURLConnection</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>На сегодняшний день если не все, то большинство Android-устройств имеют доступ к сети интернет. А большое количество мобильных приложений так или иначе взаимодействуют с средой интернет: загружают файлы, авторизуются и получают информацию с 
внешних веб-сервисов и т.д. Рассмотрим, как мы можем использовать в своем приложении доступ к сети интернет.</p>
<p>Среди стандартных элементов нам доступен виджет WebView, который может загружать контент с определенного url-адреса. Но этим возможности работы с сетью в Android 
не ограничиваются. Для получения данных с определенного интернет-ресурса мы можем использовать классы <span class="b">HttpUrlConnection</span> 
(для протокола HTTP) и <span class="b">HttpsUrlConnection</span> (для протокола HTTPS) из стандартной библиотеки Java.</p>
<p>Итак, создадим новый проект с пустой MainActivity. Первым делом для работы с сетью нам надо установить в файле манифеста <span class="b">AndroidManifest.xml</span> соответствующее разрешение:</p>
<pre class="brush:xml;">&lt;uses-permission android:name="android.permission.INTERNET"/&gt;</pre>

<p>В файле <span class="b">activity_main.xml</span>, который представляет разметку для MainActivity, определим следующий код:</p>
<pre class="brush:xml;">
...
<p>Здесь определена кнопка для загрузки данных, а сами данные для примера загружаются одновременно в виде строки в текстовое поле и в элемент WebView. Так как данных может 
быть очень много, то текстовое поле помещено в элемент ScrollView.</p>

<p>Поскольку загрузка данных может занять некоторое время, то обращение к интернет-ресурсу определим в отдельном потоке и для этого изменим код 
<span class="b">MainActivity</span> следующим образом:</p>
...
<p>Непосредственно для самой загрузки определен метод <code>getContent()</code>, который будет загружать веб-страницу с помощью класса 
<span class="b">HttpsURLConnection</span> и возвращать код загруженной страницы в виде строки.</p>
<p>Вначале создается элемент <code>HttpsURLConnection</code>:</p>
<pre class="brush:java;">
URL url=new URL(path);
connection =(HttpsURLConnection)url.openConnection();
...
<p>После подключение происходит считывание со входного потока:</p>
<pre class="brush:java;">
stream = connection.getInputStream();
reader= new BufferedReader(new InputStreamReader(stream));
</pre>
<p>Используя входной поток, мы можем считать его в строку.</p>
<p>Этот метод <code>getContent()</code> затем будет вызываться в обработчике нажатия кнопки:</p>
<pre class="brush:java;">
...
<p>Поскольку загрузка может занять долгое время, то метод <code>getContent()</code> в отдельном потоке с помощью объектов Thread и Runnable. Для примера в данном 
случае обращение идет к ресурсу "https://stackoverflow.com/".</p>
<p>Запустим приложение и нажмем на кнопку. И при наличии интернета приложение загрузит гравную страницу с "https://stackoverflow.com/" и 
отобразит ее в WebView и TextView:</p>
<img src="./pics/http1.png" alt="HttpsURLConnection и загрузка данных из интернета в Android и Java" />
<p>Конечно, данный способ вряд ли подходит для просмотра интернет-страниц, однако таким образом, мы можем получать какие-либо данные (не интернет-страницы) от 
различных веб-сервисов, например, в формате xml или json (например, различные курсы валют, показатели погоды), используя специальные api, и затем 
после обработки показывать их пользователю.</p>
...
<div class="nav"><p><a href="./4.6.php">Назад</a><a href="./">Содержание</a><a href="./11.1.php">Вперед</a></p></div></div>
<div class="socBlock">
