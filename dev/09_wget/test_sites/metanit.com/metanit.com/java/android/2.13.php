...
<html >
<head>
<title>Java и Android | Parcelable</title>
<meta charset="utf-8" />
<meta name="description" content="Передача данных между Activity с помощью объектов Parcel в Android, сериализация и интерфейс Parcelable">
...
<h2>Parcelable</h2><div class="date">Последнее обновление: 12.12.2020</div>
...
<p>Возможность сериализации объектов предоставляется напрямую инфраструктурой языка Java. Однако Android также предоставляет интерфейс <span class="b">Parcelable</span>, 
который по сути также позволяет сериализовать объекты, как и Serializable, но является более оптимизированным для Android. И подобные объекты Parcelable 
также можно передавать между двумя activity или использовать каким-то иным образом.</p>
<p>Например, в прошлой теме данные передавались между activity в виде объектов User, которые использовали сериализацию. Теперь пусть класс User 
применяет интерфейс Parcelable:</p>
...
<p>Интерфейс <span class="b">android.os.Parcelable</span> предполагает реализацию двух методов: <code>describeContents()</code> и <code>writeToParcel()</code>. 
Первый метод описывает контент и возвращает некторое числовое значение. Второй метод пишет в объект Parcel содержимое объекта User.</p>
<p>Для записи данных объекта в Parcel используется ряд методов, каждый из которых предназначен для определенного типа данных. Основные методы:</p>
<ul>
<li><p><span class="b">writeString()</span></p></li>
<li><p><span class="b">writeInt()</span></p></li>
<li><p><span class="b">writeFloat()</span></p></li>
<li><p><span class="b">writeDouble()</span></p></li>
<li><p><span class="b">writeByte()</span></p></li>
<li><p><span class="b">writeLong()</span></p></li>
<li><p><span class="b">writeIntArray()</span></p></li>
<li><p><span class="b">writeValue()</span> (записывает объект типа Object)</p></li>
<li><p><span class="b">writeParcelable()</span> (записывает объект типа Parcelable)</p></li>
</ul>
<p>Кроме того, объект Parcelable должен содержать статическое поле CREATOR, которое представляет объект <code>Creator&lt;User&gt;</code>. 
Причем этот объект реализует два метода. Они нужны для создания их ранее сериализованных данных исходных объектов типа User.</p>
<p>Так, метод <code>newArray()</code> создает массив объект User.</p>
<p>Метод <span class="b">createFromParcel</span> создает из Parcel новый объект типа User. То есть этот метод противоположен по действию методу writeToParcel. 
Для получения данных из Parcel применяются методы типа <code>readString()</code>, <code>readInt()</code>, <code>readParcelable()</code> и так далее - для чтения определенных типов данных.</p>
<p>Причем важно, что данные в createFromParcel считываются из объекта Parcel именно в том порядке, в котором они добавляются в этот объект в методе writeToParcel.</p>
<p>Допустим в activity, которая называется <span class="b">SecondActivity</span> мы будем получать объект User:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>Для получения объекта Parcelable, переданного в activity, применяется метод <code>getParcelable()</code>. Причем никакого приведения типов не требуется.</p>
<p>Для тестирования передачи Parcelable определим в файле <span class="b">activity_main.xml</span> простейший интерфейс для MainActivity:</p>
<pre class="brush:xml;">
...
<p>А в коде <span class="b">MainActivity</span> определим передачу данных в SecondActivity:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<div class="nav"><p><a href="./2.12.php">Назад</a><a href="./">Содержание</a><a href="./2.11.php">Вперед</a></p></div></div>
<div class="socBlock">
