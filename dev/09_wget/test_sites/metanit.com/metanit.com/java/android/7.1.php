...
<html >
<head>
<title>Java и Android | Создание меню</title>
<meta charset="utf-8" />
<meta name="description" content="Определение меню в Android в файлах xml и в коде java,  метод onOptionsItemSelected и onCreateOptionsMenu, элементы меню, создание подменю, обработка событий">
...
<h1>Меню</h1><h2>Создание меню</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>Меню в приложениях представляет класс <span class="b">android.view.Menu</span>, и каждая activity ассоциируется с объектом этого типа. 
Объект android.view.Menu может включать различное количество элементов, а те в свою очередь могут хранить подэлементы.</p>
<h3>Определение меню в xml</h3>
<p>Меню, как и файлы интерфейса или изображений, также представляет собой ресурс. Однако при создании нового проекта с Empty Activity по умолчанию 
нет никаких ресурсов меню, поэтому при необходимости их нужно добавлять вручную. Так, для определения ресурсов меню в проекте 
нажмем правой кнопкой мыши в проекте на каталог <span class="b">res</span> и далее в открывшемся списоке выберем пункт <span class="b">New -&gt; Android Resource File</span>:</p>
<img src="./pics/menu1.png" alt="Добавление ресурсов меню в Android Studio" />
<p>Далее в появившемся окне укажем для имени файла название <span class="b">main_menu</span>, а для поля <span class="b">Resource Type</span> 
(тип ресурса) выберем <span class="b">Menu</span>:</p>
<img src="./pics/menu2.png" alt="Создание ресурса меню в Android Studio" />
<p>После этого в каталоге res будет создан подкаталог menu, в котором будет находиться файл <span class="b">main_menu.xml</span>.</p>
<img src="./pics/menu3.png" alt="Определение меню в Android Studio и Java" />
<p>По умолчанию этот файл определяет один пустой элемент menu:</p>
<pre class="brush:xml;">
...
<p>Изменим содержимое файла, определив несколько пунктов:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;menu xmlns:android="http://schemas.android.com/apk/res/android"&gt;
    &lt;item
...
<p>Тег <code>&lt;menu&gt;</code> является корневым узлом файла и определяет меню, состоящее из одного или нескольких элементов 
<code>&lt;item&gt;</code> и <code>&lt;group&gt;</code>.</p>
<p>Элемент <code>&lt;item&gt;</code> представляет объект MenuItem, которой является одним из элементов меню. 
Этот элемент может содержать внутренний подэлемент <code>&lt;menu&gt;</code>, с помощью которого создается подменю.</p>
<p>Элемент <code>&lt;item&gt;</code> включает следующие атрибуты, которые определяют его внешний вид и поведение:</p>
<ul>
<li><p><span class="b">android:id</span>: уникальный id элемента меню, который позволяет его опознать при выборе пользователем и найти 
через поиск ресурса по id</p></li>
<li><p><span class="b">android:icon</span>: ссылка на ресурс drawable, который задает изображение для элемента (<code>android:icon="@drawable/ic_help"</code>)</p></li>
<li><p><span class="b">android:title</span>: ссылка на ресурс строки, содержащий заголовок элемента. По умолчанию имеет значение "Settings"</p></li>
<li><p><span class="b">android:orderInCategory</span>: порядок следования элемента в меню</p></li>
</ul>
<h3>Наполнение меню элементами</h3>
<p>Мы определили меню с тремя элементами, но само определение элементов в файле еще не создает меню. Это всего лишь декларативное описание. 
Чтобы вывести его на экран, нам надо использовать его в классе Activity. Для этого надо переопределить метод <span class="b">onCreateOptionsMenu</span>. 
Итак, перейдем к классу MainActivity и изменим его следующим образом:</p>
...
<p>Метод <code>getMenuInflater</code> возвращает объект <code>MenuInflater</code>, у которого вызывается метод <code>inflate()</code>. 
...
объект menu, переданный в качестве второго параметра.</p>
<p>Запустим приложение по умолчанию и нажмем на кнопку меню в правом верхнем углу:</p>
<img src="./pics/menu4.png" alt="Меню по умолчанию в Android" />
<h3>Обработка нажатий в меню</h3>
<p>Если мы нажмем на любой из пунктов меню, то ничего не произойдет. Чтобы привязать к меню действия, нам надо переопределить в классе activity 
<span class="b">onOptionsItemSelected</span>.</p>
<p>Для вывода выбранного элемена меню в файле <span class="b">activity_main.xml</span> определим текстовое поле с id=header:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>И изменим класс MainActivity:</p>
<pre class="brush:java;">
package com.example.menuapp;
...
<p>Чтобы понять, какой пункт меню выбран, вначале получаем его идентификатор <code>int id = item.getItemId()</code>. Затем пробегаемся в 
...
TextView.</p>
<img src="./pics/menuitems.png" alt="Меню и onOptionsItemSelected в Android и Java" />
<p>Стоит отметить, что в данном случае, если наша задача заключалась, чтобы просто в выводе текста выбранного пункта меню, то мы вместо конструкции switch просто могли написать так:</p>
<pre class="brush:java;">headerView.setText(item.getTitle());</pre>
<h3>Программное создание меню</h3>
<p>Кроме определения элементов меню в xml, можно также создать меню программным способом. Для добавления новых пунктов меню используется метод <span class="b">add()</span> класса <span class="b">Menu</span>.</p>
<p>Например, изменим код <span class="b">MainActivity</span>:</p>
<pre class="brush:java;">
package com.example.menuapp;
...
<p>Использованная версия метода <code>add()</code> принимает заголовок для пункта меню.</p>
