...
<html >
<head>
<title>Java и Android | Передача данных в диалоговое окно</title>
<meta charset="utf-8" />
<meta name="description" content="Передача данных в диалоговое окно из класса Activity в Android и Java, класс AlertDialog, DialogFragment">
...
<h2>Передача данных в диалоговое окно</h2><div class="date">Последнее обновление: 18.10.2021</div>
...
<p>Передача данных в диаговое окно, как и в любой фрагмент, осуществляется с помощью объекта Bundle.</p>
<p>Так, определим в файле <span class="b">activity_main.xml</span> список ListView:</p>
<pre class="brush:xml;">
...
<p>В классе <span class="b">MainActivity</span> определим для этого списка данные:</p>
...
<p>В обработчике нажатия на элемент в списке получаем выбранный элемент и добавляем его в объект Bundle. Далее через метод <code>dialog.setArguments()</code> 
передаем данные из Bundle во фрагмент.</p>
<p>Теперь определим следующий класс фрагмента <span class="b">CustomDialogFragment</span>:</p>
<pre class="brush:java;">
package com.example.dialogsapp;
...
<p>С помощью метода <code>getArguments()</code> получаем переданный в MainActivity объект Bundle. И так как была передана строка, то для ее извлечения 
применяется метод <code>getString()</code>.</p>
<p>И при нажатии элемент списка будет передаваться в диалоговое окно:</p>
<img src="./pics/dialog4.png" alt="Передача данных в диалоговое окно AlertDialog и DialogFragment в Android и Java" />
<p>В данном случае реального удаления не происходит, и в следующей статье рассмотрим, как добавить логику удаления и взаимодействия с Activity.</p>
