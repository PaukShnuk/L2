...
<html >
<head>
<title>Java и Android | TableLayout</title>
<meta charset="utf-8" />
<meta name="description" content="Контейнер TableLayout в Android и Java, организация элементов и виджетов в виде таблицы">
...
<h2>TableLayout</h2><div class="date">Последнее обновление: 14.10.2021</div>
...
<p>Контейнер <span class="b">TableLayout</span> структурирует элементы управления в виде таблицы по столбцам и строкам. 
Определим в файле <span class="b">activity_main.xml</span> элемент TableLayout, который будет включать две строки и два столбца:</p>
<pre class="brush:xml;">
...
<img src="./pics/tablelayout3.png" alt="Разметка TableLayout в Android" />
<p>Используя элемент <span class="b">TableRow</span>, мы создаем отдельную строку. Как разметка узнает сколько столбцов надо создать? 
Android находит строку с максимальным количеством виджетов одного уровня, и это количество будет означать количество столбцов. 
Например, в данном случае у нас определены две строки и в каждой по два элемента. Если бы в какой-нибудь из них было бы три виджета, то 
соответственно столбцов было бы также три, даже если в другой строке осталось бы два виджета.</p>
<p>Причем элемент TableRow наследуется от класса LinearLayout, поэтому мы можем к нему применять тот  же функционал, что и к LinearLayout. В частности, 
для определения пространства для элементов в строке используется атрибут <code>android:layout_weight</code>.</p>
<p>Если какой-то элемент должен быть растянут на ряд столбцов, то мы можем растянуть его с помощью атрибута <span class="b">layout_span</span>, 
который указывает на какое количество столбцов надо растянуть элемент:</p>
<pre class="brush:xml;">
&lt;TableLayout xmlns:android="http://schemas.android.com/apk/res/android"
    android:layout_width="match_parent"
    android:layout_height="match_parent"&gt;
...
<img src="./pics/tablelayout4.png" alt="Растягиваем элемент на несколько столбцов в TableLayout в Android" />
<p>Также можно растянуть элемент на всю строку, установив у него атрибут <code>android:layout_weight="1"</code>:</p>
<pre class="brush:xml;">
&lt;TableRow&gt;
    &lt;Button
        android:text="Отправить"
...
<h3>Программное создание TableLayout</h3>
<p>Создадим TableLayout программным образом, переложив на код java самый первый пример из данной статьи:</p>
