...
<html >
<head>
<title>Java и Android | Видео</title>
<meta charset="utf-8" />
<meta name="description" content="Работа с видео в Android и Java, добавление видеофайла в Android Studio, поддерживаемые форматы, элемент VideoView, управление воспроизведением видео с помощью MediaController">
...
<h1>Работа с мультимедиа</h1><h2>Работа с видео</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>Для работы с видеоматериалами в стандартном наборе виджетов Android определен класс VideoView, который позволяет воспроизводить видео.</p>
<p>Какие типы видеофайлов можно использовать? Android поддерживает большинство распространенных типов видеофайлов, в частности, 3GPP (.3gp), 
WebM (.webm), Matroska (.mkv), MPEG-4 (.mp4).</p>
<p>VideoView может работать как с роликами, размещенными на мобильном устройстве, так и с видеоматериалами из сети. В данном случае используем видеоролик, размещенный локально. 
Для этого добавим в проект какой-нибудь видеоролик. Обычно видеоматериалы помещают в проекте в папку <span class="b">res/raw</span>. По умолчанию  
проект не содержит подобной папки, поэтому добавим в каталог res подпапку raw. Для этого нажмем на папку res правой кнопкой мыши и в появившемся меню 
выберем <span class="b">New -&gt; Android Resource Directory</span>:</p>
<img src="./pics/video1.png" alt="Добавление папки raw в проект Android Studio" />
<p>Затем в появившемся окне в качестве типа папки укажем <span class="b">raw</span> (что также будет использоваться в качестве названия папки):</p>
<img src="./pics/video2.png" alt="Создание папки raw в проект Android Studio" />
<p>После добавления папки raw скопируем в нее какой-нибудь видеофайл:</p>
<img src="./pics/video3.png" alt="Добавление видеофайла в Android Studio" />
<p>Теперь определим функционал для его воспроизведения. Для этого в файле <span class="b">activity_main.xml</span> укажем следующий код:</p>
<pre class="brush:xml;">
...
<p>Для управления воспроизведением видео здесь определены три кнопки: для запуска видео, для паузы и для его остановки.</p>
<p>И также изменим код <span class="b">MainActivity</span>:</p>
...
<p>Во-первых, чтобы управлять потоком воспроизведения, нам надо получить объект VideoView: <code>videoPlayer = findViewById(R.id.videoPlayer);</code></p>
<p>Чтобы указать источник воспроизведения, необходим объект <span class="b">Uri</span>. В данном случае с помощью выражения 
<code>Uri myVideoUri= Uri.parse("android.resource://" + getPackageName() + "/" + R.raw.cats);</code> получаем адрес видеоролика внутри пакета приложения.</p>
<p>Строка URI имеет ряд частей: сначала идет <code>Uri-схема</code> (<code>http://</code> или как здесь <code>android.resource://</code>), 
затем название пакета, получаемое через метод getPackageName(), и далее непосредственно название ресурса видео из папки <span class="b">res/raw</span>, 
которое совпадает с названием файла.</p>
<p>Затем этот Uri устанавливается у videoPlayerа: <code>videoPlayer.setVideoURI(myVideoUri);</code></p>
<p>Чтобы управлять видеопотоком, обработчики нажатия кнопок вызывают соответствующее действие:</p>
<pre class="brush:java;">
public void play(View view){
        videoPlayer.start();
    }
...
<p>Метод <code>videoPlayer.start()</code> начинает или продолжает воспроизведение.</p>
<p>Метод <code>videoPlayer.pause()</code> приостанавливает видео.</p>
<p>Метод <code>videoPlayer.stopPlayback()</code> полностью останавливает видео.</p>
<p>Метод <code>videoPlayer.resume()</code> позволяет снова начать воспроизведение видео с начала после его полной остановки.</p>
<p>При запуске приложения мы сможем с помощью кнопок управляь воспроизведением:</p>
<img src="./pics/video4.png" alt="Воспроизведение видео в Android и VideoView" />
<h3>MediaController</h3>
<p>С помощью класса <span class="b">MediaController</span> мы можем добавить к VideoView дополнительно элементы управления. Для этого изменим код MainActivity:</p>
<pre class="brush:java;">
package com.example.mediaapp;
...
<p>И если мы запустим приложения, то при касании по VideoView внизу появятся инструменты для управления видео. В прицнипе теперь и кнопки, которые мы создали ранее, не нужны:</p>
<img src="./pics/video5.png" alt="VideoView и MediaController в Android" />
<h3>Воспроизведение файла из интернета</h3>
<p>VideoView поддерживает воспроизведение файла из интернета. Но чтобы это стало возможно, необходимо в файле <span class="b">AndroidManifest.xml</span> 
установить разрешение <span class="b">android.permission.INTERNET</span>, так как мы получаем данные из интернета:</p>
<pre class="brush:xml;">
&lt;uses-permission android:name="android.permission.INTERNET" /&gt;
</pre>

<p>Далее изменим класс MainActivity:</p>
<pre class="brush:java;">
package com.example.mediaapp;
...
<p>Здесь нам надо в метод <code>videoPlayer.setVideoPath()</code> передать интернет-адрес воспроизводимого файла.</p>
...
<div class="nav"><p><a href="./10.2.php">Назад</a><a href="./">Содержание</a><a href="./11.2.php">Вперед</a></p></div></div>
<div class="socBlock">
