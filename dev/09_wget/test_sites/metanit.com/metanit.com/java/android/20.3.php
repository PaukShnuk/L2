...
<html >
<head>
<title>Java и Android | Создание провайдера контента. Часть 1</title>
<meta charset="utf-8" />
<meta name="description" content="Создание провайдера контента в Android и Java, определение контракта и базы данных, класс SQLiteOpenHelper">
...
<h2>Создание провайдера контента. Часть 1. Определение контракта</h2><div class="date">Последнее обновление: 18.10.2021</div>
...
<p>Контент-провайдеры (content providers) позволяют обращаться одним приложениям к данным других приложений. И мы таже можем сделать, чтобы другие приложения могули обращаться 
к данным нашего приложения через некоторый API. Для этого нам надо создать свой контент-провайдер. Рассмотрим как это сделать.</p>
<p>Вначале добавим в проект класс <span class="b">FriendsContract</span>, который будет описывать основные значения, столбцы, адреса uri, используемые в контент-провайдере.</p>
...
<p>С помощью константы TABLE_NAME определяется имя таблицы, к которой будет происходить обращение. А вложенный статический класс Columns описывает 
столбцы этой таблицы. То есть таблица будет называться "friends", а столбцы - "_id", "Name", "Email", "Phone". То есть условно говоря в таблице будут храниться данные о друзьях - 
имя, электронный адрес и номер телефона.</p>
<p>Константа CONTENT_AUTHORITY описывает название контент-провайдера. То есть в моем случае провайдер будет называться "com.example.friendsprovider". С помощью имени провайдера создается 
константа CONTENT_AUTHORITY_URI - универсальный локатор или своего рода путь, через который мы будем обращаться к провайдеру при выполнении с ним различных операций.</p>
<p>Также класс определяет две константы CONTENT_TYPE и CONTENT_ITEM_TYPE, которые определяют тип возвращаемого содержимого. Здесь есть два варианта: 
возвращение набора данных и возвращение одного объекта. Значение, определяющее набор данных, строится по принципу <code>"vnd.android.cursor.dir/vnd.[name].[table]"</code>, где в качестве <code>[name]</code> 
обычно выступает глобально уникальный идентификатор, например, название провайдера или имя пакета провайдера. А в качестве <code>[type]</code>, как правило, 
используется имя таблицы. По похожей схеме строится второе значение, только вместо "dir" ставится "item".</p>
<p>Также в классе определяется вспомогательная константа CONTENT_URI, которая описывает путь для доступа к таблице friends. И также определяем два вспомогательных 
метода: <code>buildFriendUri()</code> (возвращает uri для доступа к объекту по опреленному id) и <code>getFriendId</code> (для извлечения id из 
переданного пути uri).</p>
<p>Далее добавим в проект новый класс <span class="b">AppDatabase</span>:</p>
<pre class="brush:java;">
package com.example.friendsproviderapp;
...
<p>Данный класс по принципу синглтона организует доступ к базе данных и, кроме того, создает саму базу данных и добавляет в нее начальные данные.</p>
...
<div class="nav"><p><a href="./20.2.php">Назад</a><a href="./">Содержание</a><a href="./20.4.php">Вперед</a></p></div></div>
<div class="socBlock">
