...
<html >
<head>
<title>Java и Android | Ресурс string-array и ListView</title>
<meta charset="utf-8" />
<meta name="description" content="Определение ресурса string-array в Android и Java, связь ListView с массивом строк">
...
<h2>Ресурс string-array и ListView</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>В прошлой теме было рассмотрено, как выводить массив строк с помощью ArrayAdapter в ListView. При этом массив строк определялся программно в коде java. 
Однако подобный массив строк гораздо удобнее было бы хранить в файле xml в виде ресурса.</p>
<p>Ресурсы массивов строк представляют элемент типа <span class="b">string-array</span>. Они могут находится в каталоге 
<span class="b">res/values</span> в xml-файле с произвольным именем.</p>
<p>Определения массивов строк имеют следующий синтаксис:</p>
<pre class="brush:xml;">
...
<p>Массив строк задается с помощью элемента &lt;string-array&gt;, атрибут <code>name</code> которого может иметь произвольное значение, 
по которому затем будут ссылаться на этот массив. Все элементы массива представляют набор значений <code>&lt;item&gt;</code></p>
<p>Например, добавим в папку <span class="b">res/values</span> новый файл. Для этого нажмем правой кнопкой мыши на данный каталог и появившемся меню выберем пункт 
<span class="b">New -&gt; Value Resource file</span>:</p>
<img src="./pics/string_array.png" alt="Ресурс String-Array в Android Studio" />
<p>В появившемся окне назовем файл как countries:</p>
<img src="./pics/string_array2.png" alt="String-Array в Android Studio" />
<p>После добавления файла в папку res/values изменим его содержимое следующим образом:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;resources&gt;
...
<p>В файле разметки <span class="b">activity_main.xml</span> остается определение элемента <span class="b">ListView</span>:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>Теперь свяжем ресурс и ListView в коде MainActivity:</p>
...
<p>Для получения ресурса в коде java применяется выражение <code>R.array.название_ресурса</code>.</p>
<p>Но нам необязательно добавлять список строк в ListView программно. У этого элемента есть атрибут <span class="b">entries</span>, который в качестве значения 
может принимать ресурс string-array:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout 
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>В этом случае код MainActivity мы можем сократить до стандартного:</p>
<pre class="brush:java;">
package com.example.listapp;
...
<p>А результат будет тот же самый:</p>
