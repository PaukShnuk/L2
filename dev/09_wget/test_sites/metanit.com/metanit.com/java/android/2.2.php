...
<html >
<head>
<title>Java и Android | Введение в Intent. Запуск Activity</title>
<meta charset="utf-8" />
<meta name="description" content="Intent и Intent-фильтры в Android и Java, добавление и запуск Activity, взаимодействие между разными activity">
...
<h2>Введение в Intent. Запуск Activity</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
контекстном меню выберем <span class="b">New-&gt;Activity-&gt;Empty Activity</span>:</p>
<img src="./pics/2.15.png" alt="Добавление Empty Activity в Android Studio" />
<p>Новый класс Activity назовем <span class="b">SecondActivity</span>, а все остальные настройки оставим по умолчанию:</p>
<img src="./pics/2.16.png" alt="Установка Empty Activity в Android Studio" />
<p>И после этого в проект будет добавлена новая Activity - SecondActivity:</p>
<img src="./pics/2.17.png" alt="MainActivity в Android Studio" />
...
<p>Все используемые классы activity должны быть описаны в файле AndroidManifest.xml с помощью элемента <code>&lt;activity&gt;</code>. Каждый подобный 
...
<p>Чтобы из MainActivity запустить SecondActivity, надо вызвать метод <code>startActivity()</code>:</p>
<pre class="brush:java;">
Intent intent = new Intent(this, SecondActivity.class);
startActivity(intent);
</pre>
<p>В качестве параметра в метод startActivity передается объект Intent. Для своего создания Intent в конструкторе принимает два параметра: 
контекст выполнения (в данном случае это текущий объект MainActivity) и класс, который используется объектом Intent и представляет передаваемые в 
задачу данные (фактически класс activity, которую мы будем запускать).</p>
<p>Теперь рассмотрим реализацию перехода от одной Activity к другой. Для этого в файле <span class="b">activity_main.xml</span> (то есть в интерфейсе для 
MainActivity) определим кнопку:</p>
<pre class="brush:xml;">
...
<p>И определим для кнопки в классе <span class="b">MainActivity</span> обработчик нажатия, по которому будет производиться переход к новой Activity:</p>
<pre class="brush:java;">
...
<p>В обработчике нажатия будет запускаться SecondActivity.</p>
<p>Далее изменим код <span class="b">SecondActivity</span>:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>Запустим приложение и перейдем от одной Activity к другой:</p>
<img src="./pics/2.18.png" alt="Intent и startActivity в Android и Java" />
...
<div class="nav"><p><a href="./2.3.php">Назад</a><a href="./">Содержание</a><a href="./2.12.php">Вперед</a></p></div></div>
<div class="socBlock">
