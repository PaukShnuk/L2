...
<html >
<head>
<title>Java и Android | Группы в меню и подменю</title>
<meta charset="utf-8" />
<meta name="description" content="Создание подменю и групп в меню в Android, программное создание меню в коде Java">
...
<h2>Группы в меню и подменю</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<h3>Создание подменю</h3>
<p>Для создания подменю в файле разметки меню определим внутренний элемент <code>menu</code>:</p>
<pre class="brush:xml;">
...
<p>После нажатия на меню отобразятся элементы верхнего уровня, по нажатию на которые мы можем перейти к подменю:</p>
<img src="./pics/submenu1.png" alt="Подменю в Android" />
<h3>Группы в меню</h3>
<p>Использование элемента <code>group</code> позволяет оформить элементы меню в группу:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;menu xmlns:android="http://schemas.android.com/apk/res/android"&gt;
...
<p>В определении группы мы можем установить атрибут <span class="b">android:checkableBehavior</span>. Этот атрибут может принимать следующие 
значения: <code>single</code>(у каждого элемента создается радиокнопка), <code>all</code> (для каждого элемента создается флажок) и <code>none</code>.</p>
<p>В данном случае для каждого элемента будет создаваться радиокнопка (визуально кружок). И для первого элемента устанавливается отмеченная радиокнопка (<code>android:checked="true"</code>).</p>
<p>В файле разметки интерфейса <span class="b">activity_main.xml</span> также пусть будет определено текстовое поле:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>А в классе MainActivity определим выделение радиокнопки у выбранного пункта меню:</p>
...
<img src="./pics/submenu2.png" alt="Группы в меню в Android" />
<h3>Программное создание групп в меню и подменю</h3>
<p>Также группы и подменю можно создавать программным способом. Так, изменим код <span class="b">MainActivity</span>:</p>
<pre class="brush:java;">
package com.example.menuapp;
...
<p>Использованная здесь версия метода <code>add()</code> добавляет пункт в меню, принимая следующие параметры: номер группы, id, порядок элемента в меню и заголовок элемента.</p>
<img src="./pics/programmenu1.png" alt="Программное создание меню в Android" />
