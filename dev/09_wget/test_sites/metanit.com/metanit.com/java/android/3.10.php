...
<html >
<head>
<title>Java и Android | Внутренние и внешние отступы</title>
<meta charset="utf-8" />
<meta name="description" content="Внутренние и внешние отступы margin и padding в Android 7">
...
<h2>Внутренние и внешние отступы</h2><div class="date">Последнее обновление: 14.10.2021</div>
...
<p>Параметры разметки позволяют задать отступы как от внешних границ элемента до границ контейнера, так и внутри самого элемента между его границами и содержимым.</p>
<h3>Padding</h3>
<p>Для установки внутренних отступов применяется атрибут <span class="b">android:padding</span>. Он устанавливает отступы контента от всех четырех сторон контейнера. Можно устанавливать отступы только от 
одной стороны контейнера, применяя следующие атрибуты: <span class="b">android:paddingLeft, android:paddingRight, android:paddingTop</span> и 
<span class="b">android:paddingBottom</span>.</p>
<pre class="brush:xml;">
...
<p>У контейнера ConstraintLayout установлен только один общий внутренний отступ в 50 единиц. Вложенный элемент TextView позиционируется в левом верхнем углу контейнера 
(благодаря атрибутам <code>app:layout_constraintLeft_toLeftOf="parent"</code> и <code>app:layout_constraintTop_toTopOf="parent"</code>). . 
...
же отступы будут действовать справа и снизу, если элемент будет примыкать к нижней или правой границе контейнера.</p>
<img src="./pics/3.12.png" alt="Padding в Android Studio" />
<p>Установка одного отступа</p>
<pre class="brush:xml;">android:padding="50dp"</pre>
<p>Будет аналогична установке четырех отступов</p>
<pre class="brush:xml;">
...
<p>Подобным образом можно установить отступы в других элементах. Например, установим внутри TextView сверху и снизу от внутреннего содержимого 
(то есть текста) отступы в 60 единиц и отступы слева и справа в 40 единиц:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<img src="./pics/3.15.png" alt="Установка внутренних отступов в Android Studio" />
<p>Стоит отметить, что вместо атрибутов <code>android:paddingLeft</code> и <code>android:paddingRight</code> можно применять атрибуты 
<code>android:paddingStart</code> и <code>android:paddingEnd</code>, которые разработаны специально адаптации приложения для работы как для языков с левосторонней ориентацией, 
так и правосторонней ориентацией (арабский, фарси).</p>
<h3>Margin</h3>
<p>Для установки внешних отступов используется атрибут <code>layout_margin</code>. Данный атрибут имеет 
модификации, которые позволяют задать отступ только от одной стороны: <code>android:layout_marginBottom, android:layout_marginTop, 
android:layout_marginLeft</code> и <code>android:layout_marginRight</code> (отступы соответственно от нижней, верхней, левой и правой границ):</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<img src="./pics/3.13.png" alt="Внешний отступ Margin в Android" />
<h3>Программная установка отступов</h3>
<p>Для программной установки внутренних отступов у элементы вызывается метод <span class="b">setPadding(left, top, right, bottom)</span>, в который передаются 
четыре значения для каждой из сторон. Также можно по отдельности задать отступы с помощью методов 
<span class="b">getPaddingLeft(), getPaddingTop(), getPaddingRight()</span> и <span class="b">getPaddingBottom()</span>.</p>
<p>Для установки внешних отступов необходимо реализовать объект <span class="b">LayoutParams</span> для того контейнера, который применяется. 
И затем вызвать у этого объекта LayoutParams метод <span class="b">setMargins(left, top, right, bottom)</span>:</p>
...
<p>Поскольку в данном случае элемент TextView добавляется в контейнер типа ConstraintLayout, то для его позиционирования применяется объект 
<span class="b">ConstraintLayout</span>.LayoutParams (соответственно для LinearLayout это будет LinearLayout.LayoutParams), у которого вызывается метод <span class="b">setMargins()</span>.</p>
<img src="./pics/3.16.png" alt="Programmaticaly set Margin in Android" />
<p>Но если посмотреть на последний скриншот, то можно увидеть, что, несмотря на то, что отступы вроде бы заданы также, что и в предпоследнем примере в файле 
layout, однако в реальности на экране мы увидим отступы со совсем другими значениями. Дело в том, что методы <code>setPadding()</code> и <code>setMargins()</code> 
принимают значения в пикселях, тогда как в файле layout применялись единицы dp. И чтобы использовать dp также в коде, необходимо выполнить преобразования:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<img src="./pics/3.17.png" alt="Программная установка отступов margin и padding в Java и Android" />
