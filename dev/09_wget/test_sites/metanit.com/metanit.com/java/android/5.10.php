...
<html >
<head>
<title>Java и Android | Сложный список с кнопками</title>
<meta charset="utf-8" />
<meta name="description" content="Сложный список с кнопками в Android, обработка кнопок в ListView">
...
<h2>Сложный список с кнопками</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>Ранее были расмотрены кастомные адаптеры, которые позволяют выводить в списки сложные данные. Теперь пойдем дальше и рассмотрим, как мы 
можем добавить в списки другие элементы, например, кнопки, и обрабатывать их события.</p>
<p>Для этого вначале определим следующий класс <span class="b">Product</span>:</p>
...
<p>Данный класс хранит название, количество продукта, а также единицу измерения. И объекты этого классы будем выводить в список.</p>
<p>Для этого в папку <span class="b">res/layout</span> добавим новый файл <span class="b">list_item.xml</span>:</p>
<pre class="brush:xml;">
...
<p>Здесь определены два текстовых поля для вывода названия и количества продукта и две кнопки для добавления и удаления однйо единицы продукта.</p>
<p>Теперь добавим класс адаптера, который назовем <span class="b">ProductAdapter</span>:</p>
<pre class="brush:java;">
package com.example.listapp;
...
<p>Для каждой кнопки здесь определен обработчик нажатия, в котором мы уменьшаем, либо увеличиваем количество продукта на единицу и затем переустанавливаем текст в 
сооветствующем текстовом поле.</p>
<p>Далее в файле <span class="b">activity_main.xml</span> определим элемент ListView:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<img src="./pics/customlist2.png" alt="ListView с кнопками в Android и Java" />
<p>И после запуска приложения мы сможем управлять количеством продуктов через кнопки:</p>
<img src="./pics/customlist.png" alt="ListView с кнопками в Android и Java" />
...
<div class="nav"><p><a href="./5.8.php">Назад</a><a href="./">Содержание</a><a href="./5.3.php">Вперед</a></p></div></div>
<div class="socBlock">
