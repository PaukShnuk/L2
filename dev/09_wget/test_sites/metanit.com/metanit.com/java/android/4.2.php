...
<html >
<head>
<title>Java и Android |  Виджет автодополнения AutoCompleteTextView</title>
<meta charset="utf-8" />
<meta name="description" content="Виджет автодополнения AutoCompleteTextView и MultiAutoCompleteTextView в Android и Java, обработка выбора элемента">
...
<h2>Виджет автодополнения AutoCompleteTextView</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p><span class="b">AutoCompleteTextView</span> представляет элемент, созданный на основе класса <code>EditText</code> и обладающий возможностью автодополнения</p>
<p>Во-первых, объявим в ресурсе разметке данный элемент:</p>
<pre class="brush:xml;">
...
<p>Атрибут <span class="b">android:completionHint</span> позволяет задать надпись, которая отображается внизу списка, а свойство 
<span class="b">android:completionThreshold</span> устанавливает, какое количество символов надо ввести, чтобы начало работать автодополнение. То есть в данном случае 
уже после ввода одного символа должен появться список с подстановками.</p>
<p>Как и в случае с элементами ListView и Spinner, AutoCompleteTextView подключается к источнику данных через адаптер. 
Источником данных опять же может служить массив или список объектов, либо ресурс string-array.</p>
<p>Теперь подключим к виджету массив строк в классе MainActivity:</p>
...
<p>После ввода в текстовое поле одной буквы отобразится список с вариантами автодополнения, где можно выбрать предпочтительный:</p>
<img src="./pics/autocomplete1.png" alt="Элемент AutoCompleteTextView в Android и Java" />
<h3>MultiAutoCompleteTextView</h3>
<p>Этот виджет дополняет функциональность элемента AutoCompleteTextView. <span class="b">MultiAutoCompleteTextView</span> позволяет использовать 
автодополнения не только для одной строки, но и для отдельных слов. Например, если вводится слово и после него ставится запятая, то 
автозаполнение все равно будет работать для вновь вводимых слов после запятой или другого разделителя.</p>
<p>MultiAutoCompleteTextView имеет такую же форму объявления, как и  AutoCompleteTextView:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>Чтобы включить MultiAutoCompleteTextView в коде, надо установить токен разделителя:</p>
<pre class="brush:java;">
package com.example.listapp;
...
<p>Здесь в качестве разделителя используется встроенный разделитель на основе запятой <span class="b">CommaTokenizer()</span>. При необходимости мы можем создать свои 
разделители.</p>
<img src="./pics/multiautocomplete1.png" alt="Элемент MultiAutoCompleteTextView в Android и Java" />
