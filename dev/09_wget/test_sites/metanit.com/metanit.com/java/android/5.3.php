...
<html >
<head>
<title>Java и Android | ListActivity</title>
<meta charset="utf-8" />
<meta name="description" content="Применение класса ListActivity в Android и Java, работа с ListView и AdapterView, обработка нажатия в списке и AdapterView.OnItemClickListener">
...
<h2>ListActivity</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>Для упрощения доступа к элементам списка используется класс <span class="b">ListActivity</span>. ListActivity представляет собой 
класс, унаследованный от Activity и разработанный специально для работы со списками.</p>
<p>Итак, посмотрим на примере. Во-первых, определим в файле разметки <span class="b">activity_main.xml</span> элемент ListView:</p>
<pre class="brush:xml;">
...
<p>Обратите внимание на объявление идентификатора ListView: <code>android:id="@android:id/list"</code>. Подобное объявление обязательно, 
чтобы ListActivity распознала список и могла бы им управлять.</p>
<p>Кроме ListView в файле разметки интерфейса также могут быть и другие элементы. Но в данном случае ограничимся только элементом ListView.</p>
<p>Далее изменим код класса <span class="b">MainActivity</span>:</p>
...
<p>Теперь класс MainActivity расширяет базовый класс ListActivity.</p>
<p>Здесь как и в случае с ListView мы создаем адаптер ArrayAdapter, только устанавливаем его через метод <code>setListAdapter</code>, 
который определен у ListActivity.</p>
<p>Далее создается объект слушателя <span class="b">OnItemClickListener</span>, который будет обрабатывать выбор элементов списка. Его единственный 
метод <code>onItemClick</code> аналогичен тому, что разбирался в прошлом разделе, за тем исключением, что здесь мы выводим сообщение с текстом 
выбранного элемента.</p>
<p>В конце мы используем метод <code>getListView()</code>, который возвращает объект ListView. И затем устанавливаем для него вышеопределенный 
слушатель.</p>
<img src="./pics/listactivity1.png" alt="ListActivity в Android и Java" />
