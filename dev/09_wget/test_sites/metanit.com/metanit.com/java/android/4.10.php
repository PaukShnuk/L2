...
<html >
<head>
<title>Java и Android | Всплывающие окна. Toast</title>
<meta charset="utf-8" />
<meta name="description" content="Всплывающие окна в Android, класс Toast и его методы, позиционирование окон на экране">
...
<h2>Всплывающие окна. Toast</h2><div class="date">Последнее обновление: 15.10.2021</div>
...
<p>Для создания простых уведомлений в Android используется класс <span class="b">Toast</span>. Фактически Toast представляет всплывающее окно с некоторым текстом, 
которое отображается в течение некоторого времени.</p>
<p>Объект Toast нельзя создать в коде разметки xml, например, в файл activity_main.xml</span>. Toast можно использовать только в коде java.</p>
<p>Так, определим в файле разметки activity_main.xml кнопку:</p>
<pre class="brush:xml;">
...
<p>В обработчике отображается всплывающее окно. Для его создания применяется метод <span class="b">Toast.makeText()</span>, в который передается три параметра: 
текущий контекст (текущий объект activity), отображаемый текст и время отобажения окна.</p>
<p>В качестве времени показа окна мы можем использовать целочисленное значение - колическо миллисекунд или встроенные константы 
<span class="b">Toast.LENGTH_LONG</span> (3500 миллисекунд) и <span class="b">Toast.LENGTH_SHORT</span> (2000 миллисекунд).</p>
<p>Для самого отображения окна вызывается метод <span class="b">show()</span>:</p>
<img src="./pics/toast1.png" alt="Toast в Android и Java" />
<p>По умолчанию окно отображается внизу интерфейса с центрированием по центру. Но мы можем кастомизировать позиционирование окна с помощью методов 
<span class="b">setGravity()</span> и <span class="b">setMargin()</span>. Так, изменим метод onClick:</p>
<pre class="brush:java;">
public  void onClick(View view){
...
<p>Первый параметр метода setGravity указывает, в какой части контейнера надо позиционировать Toast, второй и третий параметр устанавливают отступы от этой позиции по горизонтали и вертикали сооветственно:</p>
<img src="./pics/toast2.png" alt="Позиционирование toast в Android и Java" />
<p>Метод <span class="b">setMargin()</span> принимает два параметра: отступ от левой границы контейнера в процентах от шиирины контейнера и отступ от 
верхней границы в процентах от длины контейнера.</p>

<h3>Определение интерфейса для Toast</h3>
<p>Android поддерживает кастомизацию визуального интерфейса всплывающих окон. Для этого добавим в проект в папку <span class="b">res/layout</span> 
новый файл layout <span class="b">custom_toast.xml</span>:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
...
<img src="./pics/toast4.png" alt="layout для toast в Android и Java" />
<p>В коде MainActivity настроим отображение Toast:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>Здесь по нажатию на кнопку с помощью объекта <span class="b">LayoutInflater</span> загружаем выше определенный интерфейс из файла custom_toast.xml. Затем настраиваем 
отображение окна, его текст и выводим всплывающее окно по центру экрана.</p>
<img src="./pics/toast3.png" alt="Кастомизация и настройка toast в Android и Java" />
