...
<html >
<head>
<title>Java и Android | RadioButton</title>
<meta charset="utf-8" />
<meta name="description" content="Элемент RadioButton в Android и Java, создание группы радиокнопок и их переключение, класс RadioGroup и слушатель OnCheckedChangeListener, метод onRadioButtonClicked">
...
<h2>RadioButton</h2><div class="date">Последнее обновление: 15.10.2021</div>
...
<p>Схожую с флажками функциональность предоставляют переключатели, которые представлены классом <span class="b">RadioButton</span>. Но в отличие от флажков 
единовременно в группе переключателей мы можем выбрать только один переключатель.</p>
<p>Чтобы создать список переключателей для выбора, вначале надо создать объект <span class="b">RadioGroup</span>, который будет включать в себя 
все переключатели:</p>
<pre class="brush:xml;">
...
<p>Поскольку класс <code>RadioGroup</code> является производным от <code>LinearLayout</code>, то мы также можем задать вертикальную или 
горизонтальную ориентацию списка, при том включив в него не только собственно переключатели, но и другие объекты, например, кнопку или TextView.</p>
<p>В классе MainActivity определим обработку выбора переключателей:</p>
...
<img src="./pics/radiobutton1.png" alt="Элемент RadioButton в Android 7" />
<h3>OnCheckedChangeListener</h3>
<p>Кроме обработки нажатия на каждый отдельный переколючатель мы можем в целом повесить на весь RadioGroup с его переключателями слушатель <span class="b">OnCheckedChangeListener</span> 
и обрабатывать в нем нажатия. Для этого уберем из разметки у переключателей атрибуты <code>android:onClick</code>, а у элемента RadioGroup определим id:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>Далее в коде MainActivity повесим на объект <code>RadioGroup</code> слушатель <span class="b">OnCheckedChangeListener</span>:</p>
<pre class="brush:java;">
package com.example.viewapp;
...
<p>Слушатель RadioGroup.OnCheckedChangeListener определяет метод <span class="b">onCheckedChanged()</span>, в который передается объект RadioGroup и id выделенного переключателя. 
Далее также мы можем проверить id и выполнить определенную обработку.</p>
...
<div class="nav"><p><a href="./4.11.php">Назад</a><a href="./">Содержание</a><a href="./4.4.php">Вперед</a></p></div></div>
<div class="socBlock">
