...
<html >
<head>
<title>Java и Android | TimePicker</title>
<meta charset="utf-8" />
<meta name="description" content="Элемент TimePicker в Android и Java, установка времени, OnTimeChangedListener">
...
<h2>TimePicker</h2><div class="date">Последнее обновление: 15.10.2021</div>
...
<p><span class="b">TimePicker</span> представляет виджет для выбора времени, который может отображать время либо в 24-часовом, либо в 12-часовом формате.</p>
<p>Среди атрибутов TimePicker следует выделить <span class="b">timePickerMode</span>, который позволяет режим отображения и может принимать одно из двух значений: 
<span class="b">clock</span> (отображение в виде часов) и <span class="b">spinner</span> (отображение в виде спиннера).</p>
<p>Среди методов TimePicker можно отметить следующие:</p>
<ul>
<li><p><code>int getHour()</code>: возвращает час (в 24-часом формате)</p></li>
<li><p><code>int getMinute()</code>: возвращает минуты</p></li>
<li><p><code>boolean is24HourView()</code>: возвращает true, если используется 24-часовой формат</p></li>
<li><p><code>void setHour(int hour)</code>: устанавливает час для TimePicker</p></li>
<li><p><code>void setIs24HourView(Boolean is24HourView)</code>: устанавливает 24-часовой формат</p></li>
<li><p><code>void setMinute(int minute)</code>: устанавливает минуты</p></li>
<li><p><code>void setOnTimeChangedListener(TimePicker.OnTimeChangedListener onTimeChangedListener)</code>: 
устанавливает слушатель изменения времени в TimePicker в виде объекта <span class="b">TimePicker.OnTimeChangedListener</span></p></li>
</ul>
<p>Определим TimePicker в <span class="b">activity_main.xml</span>:</p>
<pre class="brush:xml;">
...
<p>Применим некоторые методы TimePicker для управления его поведением:</p>
...
<p>Для добавления слушателя изменения времени в TimePicker применяется метод <code>setOnTimeChangedListener()</code>, в который передается объект 
<span class="b">TimePicker.OnTimeChangedListener</span>. Он имеет один метод - <span class="b">onTimeChanged()</span>, который вызывается при каждом изменении 
времени в TimePicker. Этот метод принимает три параметра - сам элемент TimePicker, <code>hourOfDay</code> - установленный час и <code>minute</code> - 
установленные минуты. В данном случае просто передаем значение выбранного времени в TextView.</p>
<img src="./pics/timepicker1.png" alt="OnTimeChangedListener в TimePicker в Android и Java" />
<p>По умолчанию TimePicker отображается в режиме "clock" или часы. Применим режим "spinner":</p>
<pre class="brush:xml;">
&lt;TimePicker android:id="@+id/timePicker"
	android:layout_width="wrap_content"
	android:layout_height="wrap_content"
...
<img src="./pics/timepicker2.png" alt="Spinner в TimePicker в Android и Java" />
...
<div class="nav"><p><a href="./4.4.php">Назад</a><a href="./">Содержание</a><a href="./4.7.php">Вперед</a></p></div></div>
<div class="socBlock">
