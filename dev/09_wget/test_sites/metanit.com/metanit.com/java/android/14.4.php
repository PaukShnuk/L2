...
<html >
<head>
<title>Java и Android | Динамический поиск по базе данных SQLite</title>
<meta charset="utf-8" />
<meta name="description" content="Динамический живой поиск по базе данных SQLite в Android и Java, live search">
...
<h2>Динамический поиск по базе данных SQLite</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>Рассмотрим, как мы можем создать в приложении на Android динамический поиск по базе данных SQLite.</p>
<p>Итак, создадим новый проект с пустой MainActivity. Для этого проекта возьмем базу данных из прошлой темы (или создадим новую). Данная база данных называется 
<span class="ii">cityinfo</span> и имеет одну таблицу users с тремя полями _id, name, age:</p>
<pre class="brush:sql;">
CREATE TABLE `users` (
...
<p>И также добавим в проект в Android Studio папку <span class="b">assets</span>, а в папку assets - только что созданную базу данных:</p>
<img src="./pics/sqlitesearch1.png" alt="База данных SQLite в Android Studio" />
<p>В моем случае база данных называется "cityinfo.db".</p>
<p>Как показано выше на скриншоте, добавив в проект в одну папку с MainActivity новый класс <span class="b">DatabaseHelper</span>:</p>
...
<p>Перейдем к файлу <span class="b">activity_main.xml</span>, который определяет визуальный интерфейс, и изменим его следующим образом:</p>
<pre class="brush:xml;">
...
<p>Итак, у нас будет элемент ListView для отображения списка и текстовое поле для фильтрации.</p>
<p>Теперь изменим код <span class="b">MainActivity</span>:</p>
<pre class="brush:java;">
package com.example.livesearchapp;
...
<p>Прежде всего надо отметить, что для фильтрации данных в адаптере, нам надо получить фильтр адаптера, а у этого фильтра выполнить метод <code>filter()</code>:</p>
<pre class="brush:java;">userAdapter.getFilter().filter(s.toString());</pre>
<p>В этот метод <code>filter()</code> передается ключ поиска.</p>
<p>Для текстового поля мы можем отслеживать изменения содержимого с помощью слушателя:</p>
<pre class="brush:java;">
userFilter.addTextChangedListener(new TextWatcher() {
...
<p>В слушателе TextWatcher в методе <code>onTextChanged</code> как раз и вызывается метод <code>filter()</code>, в который передется введенная пользователем в текстовое поле 
последовательность символов.</p>
<p>Сам вызызов метода <code>filter()</code> мало на что влияет. Нам нало еще определить провайдер фильтрации адаптера, которые и будет инкапсулировать реальную логику фильтрации:</p>
<pre class="brush:java;">
userAdapter.setFilterQueryProvider(new FilterQueryProvider() {
    @Override
    public Cursor runQuery(CharSequence constraint) {
...
<p>Сущность этого провайдера заключается в выполнении SQL-выражений к бд, а именно конструкций "select from" и "select from where like". Данные простейшие выражения выполняют 
регистрозависимую фильтрацию. В результате адаптаре получает отфильтрованные данные.</p>
<p>Следует также отметить следующий код:</p>
<pre class="brush:java;">
if(!userFilter.getText().toString().isEmpty())
    userAdapter.getFilter().filter(userFilter.getText().toString());
</pre>
<p>Данный код нам нужен при смене ориентации (например, с портретной на альбомную). И если ориентация устройства изменена, но в текстовом поле все же есть 
некоторые текст-фильтр, то выполняется фильтрация. Иначе бы она не выполнялась.</p>
<p>И после запуска мы сможем насладиться фильтрацией данных:</p>
<img src="./pics/sqlitefilter1.png" alt="Live Search Фильтрация данных в Android  и Java и SQLite" />
...
<div class="nav"><p><a href="./14.3.php">Назад</a><a href="./">Содержание</a><a href="./14.6.php">Вперед</a></p></div></div>
<div class="socBlock">
