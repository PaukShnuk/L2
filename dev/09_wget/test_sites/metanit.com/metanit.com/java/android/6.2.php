...
<html >
<head>
<title>Java и Android | Темы</title>
<meta charset="utf-8" />
<meta name="description" content="Использование тем в Android и Java, их отличие от стилей, применение тем к приложению и к отдельной активности, задание собственных тем">
...
<h2>Темы</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>Кроме применение отдельных стилей к отдельным элементам, мы можем задавать стили для всего приложения или activity в виде тем. 
Тема предтавляет коллекцию атрибутов, которые применяются в целом ко всему приложению, классу activity или иерархии виджетов.</p>
<p>Мы можем сами создать тему. Однако Android уже предоставляет несколько предустановленных тем для стилизации приложения, 
например, <code>Theme.AppCompat.Light.DarkActionBar</code> и ряд других.</p>
<p>По умолчанию приложение уже применяет темы. Так, откроем файл <span class="b">AndroidManifest.xml</span>. В нем мы можем увидеть следующее определение элемента application, представляющего приложение:</p>
<pre class="brush:xml;">
&lt;application
...
<p>Задание темы происходит с помощью атрибута <code>android:theme</code>. В данном случае используется ресурс, который называется в моем случае 
<span class="b">Theme.ViewApp</span>. По умолчанию файлы тем определены в папке <span class="b">res/values</span>. 
В частности, здесь можно найти условный каталог <span class="b">themes</span>, в котором по умолчанию есть два элемента: <span class="b">themes.xml</span>:</p>
<img src="./pics/theme1.png" alt="Темы и стили в Android Studio и Java" />
<p>Один файл представляет светлую тему, а другой - темную. Например, откроем файл themes.xml со светлой темой:</p>
<pre class="brush:xml;">
...
<p>Здесь мы можем увидеть, что тема определяется как и стиль с помощью элемента <span class="b">style</span>.. 
Атрибут <span class="b">parent</span> указывает на родительскую тему, от которой текущая тема берет все стилевые характеристики. 
То есть тема "Theme.ViewApp" использует другую тему - "Theme.MaterialComponents.DayNight.DarkActionBar". И кроме того, определяет ряд своих собственных стилей.</p>
<p>Также можно заметить, что здесь определяются не только характеристики для атрибутов, но и семантические имена, например, <code>colorPrimary</code>, которому сопоставлен ресурс "@color/purple_500".</p>
<p>При необходимости мы можем изменить эти характеристики или дополнить тему новыми стилевыми характеристиками. Например, изменим цвет свойства <code>colorPrimary</code>, которое применяется в том числе 
в качестве фонового цвета заголовка и кнопки:</p>
<pre class="brush:xml;">&lt;item name="colorPrimary"&gt;#1565C0&lt;/item&gt;</pre>
<p>И соответственно изменится цвет по умолчанию для фона заголовка и кнопки:</p>
<pre class="brush:xml;">
...
<img src="./pics/themeeditor1.png" alt="Theme Editor in Android Studio" />

<h3>Создание собственной темы</h3>
<p>Вместо использования встроенных тем мы можем создать свою. Для этого добавим в папку <span class="b">res/values</span> новый файл <span class="b">mythemes.xml</span> и определим в нем следующее содержимое:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;resources&gt;
...
<img src="./pics/themedialog1.png" alt="Определение своей темы" />
<p>Итак, мы создали стиль "MyTheme", который унаследован от стиля <code>Theme.AppCompat.Light</code>. В этом стиле мы переопределили 
два свойства: высоту шрифта (textSize) - 28sp, а также цвет текста (textColor) - <code>#FF018786</code>.</p>
<p>Теперь определим этот стиль в качестве темы приложения в файле <span class="b">AndroidManifest.xml</span>:</p>
<pre class="brush:xml;">
&lt;application
        android:allowBackup="true"
        android:icon="@mipmap/ic_launcher"
...
<p>Пусть у нас будет следующая разметка в <span class="b">activity_main.xml</span></p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>Как видно, для элементов TextView не устанавливается атрибут <code>textSize</code> и <code>textColor</code>, однако поскольку они определены в теме, 
которая применяется глобально к нашему приложению, то элементы TextView будут подхватывать эти стилевые характеристики:</p>
<img src="./pics/customtheme1.png" alt="Создание новой темы для Android" />
<h3>Применение темы к activity</h3>
<p>Выше темы применялись глобально ко всему приложению. Но также можно применить их к отдельному классу Activity. Для этого надо подкоррективать файл манифеста 
AndroidManifest. Например:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>Атрибут <span class="b">android:theme</span> элемента <code>&lt;activity&gt;</code> указывает на применяемую к MainActivity тему. То есть глобально к приложению применяется 
тема "Theme.ViewApp", а к MainActivity - "MyTheme".</p>
<h3>Применение темы к иерархии виджетов</h3>
<p>Также можно применить тему к иерархии виджетов, установив атрибут <span class="b">android:theme</span> у элемента, к которому (включая его 
вложенные элементы) мы хотим применить тему. Например, примение темы к ConstraintLayout и ее элементам:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<div class="nav"><p><a href="./6.1.php">Назад</a><a href="./">Содержание</a><a href="./7.1.php">Вперед</a></p></div></div>
<div class="socBlock">
