...
<html >
<head>
<title>Java и Android | Ресурсы XML и их парсинг</title>
<meta charset="utf-8" />
<meta name="description" content="Ресурсы XML и их парсинг в приложении на Android и Java, класс XmlPullParser">
...
<h1>Работа с XML</h1><h2>Ресурсы XML и их парсинг</h2><div class="date">Последнее обновление: 18.10.2021</div>
...
<p>Одним из распространенных форматов хранения и передачи данных является <span class="b">xml</span>. Рассмотрим, как с ним работать в приложении 
на Android.</p>
<p>Приложение может получать данные в формате xml различными способами - из ресурсов, из сети и т.д. В данном случае рассмотрим ситуацию, когда файл 
xml хранится в ресурсах.</p>
<p>Возьмем стандартный проект Android по умолчанию и в папке <span class="b">res</span> создадим каталог <span class="b">xml</span>. Для этого нажмем на каталог <span class="b">res</span> 
правой кнопкой мыши и в контекстном меню выберем <span class="b">New -&gt; Android Resource Directory</span>:</p>
<img src="./pics/xml4.png" alt="Добавление папки xml в Android Studio" />
<p>В появившемся окне в качестве типа ресурсов укажем <span class="b">xml</span>:</p>
<img src="./pics/xml5.png" alt="Добавление папки ресурсов xml в Android Studio" />
<p>В этот каталог добавим новый файл, 
который назовем <span class="b">users.xml</span> и который будет иметь следующее содержимое:</p>
<pre class="brush:xml;">
...
<p>Это обычный файл xml, который хранит набор элементов user. Каждый элемент характеризуется наличием двух подэлементов - name и age. Условно говоря, 
каждый элемент описывает пользователя, у которого есть имя и возраст.</p>
<p>В папку, где находится основной класс MainActivity, добавим новый класс, который назовем <span class="b">User</span>:</p>
...
<p>Этот класс описывает товар, информация о котором будет извлекаться из xml-файла.</p>
<p>И в ту же папку добавим новый класс <span class="b">UserResourceParser</span>:</p>
<img src="./pics/xml1.png" alt="XML-ресурсы в Android Studio и их парсинг с помощью Java" />
<p>Определим для класса UserResourceParser следующий код:</p>
<pre class="brush:java;">
package com.example.xmlapp;
...
<p>Данный класс выполняет функции парсинга xml. Распарсенные данные будут храниться в переменной users. Непосредственно сам парсинг осуществляется 
с помощью функции <code>parse</code>. Основную работу выполняет передаваемый в качестве параметра объект <span class="b">XmlPullParser</span>. Этот 
класс позволяет пробежаться по всему документу xml и получить его содержимое.</p>
<p>Когда данный объект проходит по документу xml, при обнаружении определенного тега он генерирует некоторое событие. Есть четыре события, которые описываются 
следующими константами:</p>
<ul>
<li><p><code>START_TAG</code>: открывающий тег элемента</p></li>
<li><p><code>TEXT</code>: прочитан текст элемента</p></li>
<li><p><code>END_TAG</code>: закрывающий тег элемента</p></li>
<li><p><code>END_DOCUMENT</code>: конец документа</p></li>
</ul>
<p>С помощью метода <code>getEventType()</code> можно получить первое событие и потом последовательно считывать документ, пока не дойдем до его конца. 
Когда будет достигнут конец документа, то событие будет представлять константу <code>END_DOCUMENT</code>:</p>
<pre class="brush:java;">
int eventType = xpp.getEventType();
while(eventType != XmlPullParser.END_DOCUMENT){
	//......................
...
<p>Для перехода к следующему событию применяется метод <code>next()</code>.</p>
<p>При чтении документа с помощью метода <code>getName()</code> можно получить название считываемого элемента.</p>
<pre class="brush:java;">String tagName = xpp.getName();</pre>
<p>И в зависимости от названия тега и события мы можем выполнить определенные действия. Например, если это открывающий тег элемента user, 
то создаем новый объект User и устанавливаем, что мы находимся внутри элемента user:</p>
<pre class="brush:java;">
case XmlPullParser.START_TAG:
	if("user".equalsIgnoreCase(tagName)){
		inEntry = true;
...
<p>Если событие <code>TEXT</code>, то считано содержимое элемента, которое мы можем прочитать с помощью метода <code>getText()</code>:</p>
<pre class="brush:java;">
case XmlPullParser.TEXT:
	textValue = xpp.getText();
	break;
</pre>
<p>Если закрывающий тег, то все зависит от того, какой элемент прочитан. Если прочитан элемент user, то добавляем объект User в коллекцию ArrayList и 
сбрываем переменную inEntry, указывая, что мы вышли из элемента user:</p>
<pre class="brush:java;">
case XmlPullParser.END_TAG:
	if(inEntry){
		if("user".equalsIgnoreCase(tagName)){
...
<p>Если прочитаны элементы name и age, то передаем их значения переменным name и age объекта User:</p>
<pre class="brush:java;">
else if("name".equalsIgnoreCase(tagName)){
	currentUser.setName(textValue);
} else if("age".equalsIgnoreCase(tagName)){
...
<p>Теперь изменим класс MainActivity, который будет загружать ресурс xml:</p>
<pre class="brush:java;">
package com.example.xmlapp;
...
<p>Вначале получаем ресурс xml с помощью метода <span class="b">getXml()</span>, в который передается название ресурса. Данный метод возвращает 
объект <span class="b">XmlPullParser</span>, который затем используется для парсинга. Для простоты просто выводим данные в окне <span class="b">Logcat</span>:</p>
<img src="./pics/xml2.png" alt="XmlPullParser и парсинг xml-файла в Android и Java" />
...
<div class="nav"><p><a href="./13.3.php">Назад</a><a href="./">Содержание</a><a href="./23.2.php">Вперед</a></p></div></div>
<div class="socBlock">
...
$(function() { 
  ...
  $("#toggle-theme").on("click", function() {
	...
	$("li:contains('Глава 23.')").removeClass('lastExpandable').addClass('lastCollapsable').children('ul').css({"display":"block"});
	$("li:contains('Глава 23.')").children('div').removeClass('lastExpandable-hitarea').addClass('lastCollapsable-hitarea');
