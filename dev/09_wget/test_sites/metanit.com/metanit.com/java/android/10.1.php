...
<html >
<head>
<title>Java и Android | Многопоточность и асинхронность</title>
<meta charset="utf-8" />
<meta name="description" content="Многопоточность и асинхронность в Android и Java, View.post, Runnable, Thread, доступ к виджетам в потоке">
...
<h1>Многопоточность и асинхронность</h1><h2>Создание потоков и визуальный интерфейс</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>Когда мы запускаем приложение на Android, система создает поток, который называется основным потоком приложения или UI-поток. Этот поток обрабатывает все 
изменения и события пользовательского интерфейса. Однако для вспомогательных операций, таких как отправка или загрузка файла, продолжительные 
вычисления и т.д., мы можем создавать дополнительные потоки.</p>
<p>Для создания новых потоков нам доcтупен стандартный функционал класса <span class="b">Thread</span> из базовой библиотеки Java из пакета 
<code>java.util.concurrent</code>, которые особой трудности не представляют. Тем не менее трудности могут возникнуть при обновлении визуального интерфейса из потока.</p>
<p>Например, создадим простейшее приложение с использованием потоков. Определим следующую разметку интерфейса в файле <span class="b">activity_main.xml</span>:</p>
<pre class="brush:xml;">
...
<p>Здесь определена кнопка для запуска фонового потока, а также текстовое поле для отображения некоторых данных, которые будут генерироваться в запущенном потоке.</p>
...
<p>Итак, здесь к кнопке прикреплен обработчик нажатия, который запускает новый поток. Создавать и запускать поток в Java можно различными способами. 
В данном случае сами действия, которые выполняются в потоке, определяются в методе <span class="b">run()</span> объекта <span class="b">Runnable</span>:</p>
<pre class="brush:java;">
Runnable runnable = new Runnable() {
	@Override
	public void run() {
...
<p>Для примера получаем текущее время и пытаемся отобразить его в элементе TextView.</p>
<p>Далее определяем объект потока - объект <span class="b">Thread</span>, который принимает объект Runnable. И с помощью метода <span class="b">start()</span> 
запускаем поток:</p>
<pre class="brush:java;">
// Определяем объект Thread - новый поток
Thread thread = new Thread(runnable);
...
<p>Вроде ничего сложного. Но если мы запустим приложение и нажмем на кнопку, то мы столкнемся с ошибкой:</p>
<img src="./pics/asynctask1.png" alt="Многопоточность в Android и Java" />
<p>Поскольку изменять состояние визуальных элементов, обращаться к ним мы можем только в основном потоке приложения или UI-потоке.</p>
<p>Для решения этой проблемы - взаимодействия во вторичных потоках с элементами графического интерфейса класс <span class="b">View()</span> определяет 
метод <span class="b">post()</span>:</p>
<pre class="brush:java;">boolean post (Runnable action)</pre>
<p>В качестве параметра он принимает задачу, которую надо выполнить, и возвращает логическое значение - <code>true</code>, если задача Runnable 
успешно помещена в очередь сообщение, или <code>false</code>, если не удалось разместить в очереди</p>
<p>Также у класса View есть аналогичный метод:</p><span class="b">postDelayed()</span>:</p>
<pre class="brush:java;">boolean postDelayed (Runnable action, long millsec)</pre>
<p>Он также запускает задачу, только через определенный промежуток времени в миллисекундах, который указывается во втором параметре.</p>
<p>Так, изменим код <span class="b">MainActivity</span> следующим образом</p>
<pre class="brush:java;">
package com.example.threadapp;
...
<p>Теперь для обновления TextView применяется метод post:</p>
<pre class="brush:java;">
textView.post(new Runnable() {
		public void run() {
			textView.setText(time);
...
<p>То есть здесь в методе <code>run()</code> передаемого в метод <code>post()</code> объекта Runnable мы можем обращаться к элементам визуального интерфейса и 
взаимодействовать с ними.</p>
<img src="./pics/asynctask2.png" alt="Многопоточность в Android и Java, Runnable и метод View.post" />
<p>Подобным образом можно работать и с другими виджетами, которые наследуются от класса <span class="b">View</span>.</p>
...
<div class="nav"><p><a href="./8.2.php">Назад</a><a href="./">Содержание</a><a href="./10.2.php">Вперед</a></p></div></div>
<div class="socBlock">
