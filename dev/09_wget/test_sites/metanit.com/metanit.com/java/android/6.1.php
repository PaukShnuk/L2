...
<html >
<head>
<title>Java и Android | Стили</title>
<meta charset="utf-8" />
<meta name="description" content="Определение и применение стилей в Android и Java, ресурс style, файл styles.xml">
...
<h1>Стили и темы</h1><h2>Стили</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>Мы можем настроить элемент с помощью различных атрибутов, которые задают высоту, ширину, цвет фона, текста и так далее. Но если у нас 
несколько элементов используют одни и те же настройки, то мы можем объединить эти настройки в стили.</p>
<p>Например, пусть у нас есть несколько элементов TextView:</p>
<pre class="brush:xml;">
...
<img src="./pics/styles1.png" alt="Стили в Android" />
<p>Все эти TextView имеют одинаковый набор свойств, и, к примеру, если нам захочется изменить цвет текста, то придется менять его у всех трех TextView. 
Данный подход не оптимален, а более оптимальный подход представляет использование стилей, которые определяются в проекте в папке <span class="b">res/values</span>.</p>
<p>Итак, добавим в проект в папку <span class="b">res/values</span> новый элемент <span class="b">Value Resourse File</span>, который  
назовем <span class="b">styles.xml</span>:</p>
<img src="./pics/styles2.png" alt="styles.xml в Android Studio" />
<p>Определим в файле <span class="b">styles.xml</span> следующее содержимое:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;resources&gt;
...
<p>Здесь определен новый стиль TextViewStyle, который с помощью элементов item задает значения для атрибутов TextView.</p>
<p>Стиль задается с помощью элемента <code>&lt;style&gt;</code>. Атрибут name указывает на название стиля, через которое потом можно 
ссылаться на него. Необязательный атрибут <code>parent</code> устанавливает для данного стиля родительский стиль, от которого дочерний стиль 
будет наследовать все свои характеристики.</p>
<p>С помощью элементов <code>item</code> устанавливаются конкретные свойства виджета, который принимает в качестве значения атрибута name 
имя устанавливаемого свойства.</p>
<p>Теперь применим стиль, изменим файл activity_main.xml:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>Используя определение <code>style="@style/TextViewStyle"</code> текстовое поле связывается с определением стиля. Итоговый результат буде тот же, что и раньше, только 
кода становится меньше. А если мы захотим поменять какие-то характеристики, то достаточно изменить нужный элемент item в определении стиля.</p>
...
<div class="nav"><p><a href="./5.12.php">Назад</a><a href="./">Содержание</a><a href="./6.2.php">Вперед</a></p></div></div>
<div class="socBlock">
