...
<html >
<head>
<title>Java и Android | Tween-анимация</title>
<meta charset="utf-8" />
<meta name="description" content="Tween-анимация Java и Android, AnimationUtils.loadAnimation, startAnimation ImageView">
...
<h2>Tween-анимация</h2><div class="date">Последнее обновление: 18.10.2021</div>
...
<p>Tween-анимация представляет анимацию различных свойств объекта, при которой система сама расчитывает некоторые промежуточные значения с помощью 
определенного алгоритма, который называется интерполяцией. В Android алгоритм интерполяции определяется встроенным классом <span class="b">Animation</span>.</p>
<p>От данного класса наследуются классы, которые описывают конкретные типы анимаций, такие как <span class="b">AlphaAnimation</span> (изменение прозрачности), 
<span class="b">RotateAnimation</span> (анимация вращения), <span class="b">ScaleAnimation</span> (анимация масштабирования), 
<span class="b">TranslateAnimation</span> (анимация перемещения).</p>
<p>Мы можем определить анимацию как в коде java, так и в файле xml. Для хранения ресурсов анимации в папке res предназначена папка <span class="b">anim</span>. По умолчанию 
данная папка отсуствует в проекте, поэтому создадим ее. Для этого нажмем правой кнопкой мыши на папку <span class="b">res</span> и в появившемся контекстном 
меню выберем пункт <span class="b">New -&gt; Android Resource Directory</span></p>:
<img src="./pics/tween_animation1.png" alt="Tween-анимация в Android Studio" />
<p>Затем в появившемся окне укажем тип ресурсов - <span class="b">anim</span></p>
<img src="./pics/tween_animation2.png" alt="Resource anim in Android Studio" />
<p>Далее добавим в нее новый xml-файл, который назовем <span class="b">common_animation.xml</span>:</p>
<img src="./pics/tween_animation.png" alt="Tween-анимация в Android" />
<p>Определим в этом файле следующее содержание:</p>
<pre class="brush:xml;">
...
<p>Здесь задействуются четыре типа анимаций: элемент <code>scale</code> представляет масштабирование, элемент <code>rotate</code> - вращение, 
элемент <code>alpha</code> - изменение прозрачности, элемент <code>translate</code> - перемещение. Если бы мы использовали одну анимацию, то могли бы определить 
один корневой элемент по типу анимации. Но так как мы используем набор, то все анимации помещаются в элемент <span class="b">set</span>, который 
представляет класс <code>AnimationSet</code></p>
<p>Все виды анимаций принимают ряд общих свойств. В частности, свойство <span class="b">android:repeatMode</span>, которое указывает на редим выполнения. 
Если имеет значение <code>reverse</code>, то анимация выполняется также и в обратную сторону</p>
<p>Свойство <span class="b">android:repeatCount</span> указывает на количество повторов анимации. Значение <code>infinite</code> устанавливает 
бесконечное число повторов.</p>
<p>Время анимации задается с помощью свойства <span class="b">android:duration</span></p>
<p>Для всех анимаций также характерно указание начальной и конечной точки трансформации.</p>
<h3>Анимация масштабирования</h3>
<p>Для анимация масштабирования задается начальное масштабирование 
по оси х (<code>android:fromXScale</code>) и по оси y (<code>android:fromYScale</code>) и конечные значения масштабирования <code>android:toXScale</code> 
и <code>android:toYScale</code>. Например, так как android:fromXScale=1.0, а android:toXScale=0.5, то по ширине будет происходить сжатие на 50%.</p>
<p>Также при масштабировании устанавливаются зачения <code>android:pivotX</code> и <code>android:pivotY</code>, которые указывают на центр масшабирования или опорную точку.</p>
<h3>Анимация вращения</h3>
<p>Для анимации вращения задается начальное (<code>android:fromDegrees</code>) и конечное значения поворота (<code>android:toDegrees</code>).</p>
<p>С помощью свойств <code>android:pivotX</code> и <code>android:pivotY</code> также, как и при масштабировании, задается опорная точка вращения.</p>
<h3>Анимация прозрачности</h3>
<p>Для анимации прозрачности задается начальное значение прозрачности (<code>android:fromAlpha</code>) и финальное значение, устанавливамое при завершении анимации 
(<code>android:toAlpha</code>). Все значения варьируютс в диапазоне от 1.0 (непрозрачный) до 0.0 (полностью прозрачный)</p>
<h3>Анимация перемещения</h3>
<p>Для перемещения также устанавливаются начальные (<code>android:fromXDelta</code> и <code>android:fromYDelta</code>) и конечные значения 
(<code>android:toXDelta</code> и <code>android:toYDelta</code>)</p>
<p>Для всех анимаций начальные и конечные значения указывают некий диапазон, в котором будут ранжироваться значения. Само вычисление значений на этом промежутке 
...
<code>set</code> определено свойство <code>android:interpolator="@android:anim/linear_interpolator"</code>.</p>
<p>Кроме данного значения свойство <span class="b">android:interpolator</span> может принимать еще ряд других: bounce_interpolator, cycle_interpolator и т.д.</p>
<p>Данная анимация будет применяться к элементу <span class="b">ImageView</span>, который определим в файле разметки интерфейса:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>Для демонстрации анимации добавим в папку <span class="b">res/drawable</span> какой-нибудь графический файл. В моем случае это файл <span class="b">dubi2.png</span>.</p>
<p>Теперь определим и запустим анимацию в классе <span class="b">MainActivity</span>:</p>
...
<p>Сначала определяем анимацию по тому файлу <span class="b">common_animation.xml</span>, который содержит набор анимаций:</p>
<pre class="brush:java;">
Animation animation = AnimationUtils.loadAnimation(this, R.anim.common_animation);
</pre>
<p>А потом запускаем ее:</p>
<pre class="brush:java;">
img.startAnimation(animation);
</pre>
<img src="./pics/animations.png" alt="Анимации в Android и Java" />
...
<div class="nav"><p><a href="./19.1.php">Назад</a><a href="./">Содержание</a><a href="./20.1.php">Вперед</a></p></div></div>
<div class="socBlock">
