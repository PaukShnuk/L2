...
<html >
<head>
<title>Java и Android | Обработка выбора элемента в RecyclerView</title>
<meta charset="utf-8" />
<meta name="description" content="Обработка выбора элемента в RecyclerView в Android и Java, нажатие на ViewHolder, OnClickListener">
...
<h2>Обработка выбора элемента в RecyclerView</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>При работе с виджетом <span class="b">RecyclerView</span> неизбежно встанет вопрос, а как обработать выбор элемента в RecyclerView. 
И тут надо заметить, что в отличие от других типов виджетов для работы со списками RecyclerView по умолчанию не предоставляет какого-то специального метода, 
с помощью которого можно было бы определить слушатель нажатия на элемент в списке. Поэтому всю инфраструктуру необходимо определять самому разработчику. 
Но к счастью в реальности все не так сложно. Для примера возьмем проект из прошлой темы:</p>
<img src="./pics/recyclerview3.png" alt="Проект для RecyclerView в Android и Java" />
<p>Итак, для представления данных в проекте есть класс <span class="b">State</span>, который представляет государство:</p>
...
<p>В папке <span class="b">res/layout</span> для вывода одного объекта State в списке определен следующий файл <span class="b">list_item.xml</span>:</p>
<pre class="brush:xml;">
...
<p>Теперь перейдем к классу <span class="b">StateAdapter</span> и следующим образом определим его код:</p>
<pre class="brush:java;">
package com.example.listapp;
...
<p>Здесь я остановлюсь на тех моментах, которые были добавлены по сравнению с кодом из прошлой статьи.</p>
<p>Прежде всего нам надо определить интерфейс слушателя события нажатия. Для этого в классе StateAdapter определен интерфейс:</p>
<pre class="brush:java;">
interface OnStateClickListener{
	void onStateClick(State state, int position);
}
</pre>
<p>Интерфейс определяет один метод <code>onStateClick()</code>, который, как предполагается, будет вызываться при выборе объекта State и который 
будет получать выбранный объект State и его позицию в списке.</p>
<p>Следующий момент - определение в классе адаптера переменной для хранения объекта этого интерфейса и получение для нее значения в конструкторе:</p>
<pre class="brush:java;">
private final OnStateClickListener onClickListener;
...
<p>Таким образом, вне кода адаптера мы можем установить любой объект слушателя и передать его в адаптер.</p>
<p>И третий момент - вызов метода слушателя при нажатии на элемент в списке в методе <span class="b">onBindViewHolder</span>:</p>
<pre class="brush:java;">
public void onBindViewHolder(StateAdapter.ViewHolder holder, int position) {
	State state = states.get(position);
	holder.flagView.setImageResource(state.getFlagResource());
...
<p>Класс <span class="b">ViewHolder</span> имеет поле <span class="b">itemView</span>, которое представляет интерфейс для одного объекта в списке и фактически 
объект <span class="b">View</span>. А у этого объекта есть метод <code>setOnClickListener()</code>, через который можно подлючить стандартный слушатель 
нажатия <span class="b">OnClickListener</span> и в его методе <code>onClick()</code> вызвать метод нашего интерфейса, передав ему необходимые данные - выбранный объект State и его 
позицию в списке.</p>
<p>Может возникнуть вопрос, а почему бы сразу тут и не обработать нажатие на элемент? К чему создавать дополнительный интерфейс, его переменную и вызывать его метод? 
Конечно, мы можем попыться прямо тут обработать нажатия, но это не является хорошей или распространенной практикой, поскольку, возможно, мы захотим 
определить обработку нажатия в классе MainActivity исходя из того, кода, который там определен (или из какого-то другого места извне).</p>
<p>В файле <span class="b">activity_main.xml</span> остается тот же самый визуальный интерфейс:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>При создании адаптера ему передается определенный в классе MainActivity слушатель:</p>
<pre class="brush:java;">
StateAdapter.OnStateClickListener stateClickListener = new StateAdapter.OnStateClickListener() {
	@Override
	public void onStateClick(State state, int position) {
...
<p>Здесь просто выводится всплывающее сообщение о выбранном элементе списка.</p>
<img src="./pics/recyclerview1.png" alt="OnClickListener в ViewHolder и RecyclerView в Android и Java" />
...
<div class="nav"><p><a href="./5.11.php">Назад</a><a href="./">Содержание</a><a href="./6.1.php">Вперед</a></p></div></div>
<div class="socBlock">
