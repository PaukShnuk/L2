...
<html >
<head>
<title>Java и Android | Публикация приложения</title>
<meta charset="utf-8" />
<meta name="description" content="Публикация приложения, создание подписанного файла apk в Android Studio">
...
<h1>Публикация приложения</h1><h2>Создание подписанного apk</h2><div class="date">Последнее обновление: 30.10.2015</div>
...
<p>После создания приложения, его тестирования и отладки мы можем приступить к его публикации. Суть публикации заключается в создании файла с 
расширением <i>.apk</i>, которое будет представлять приложение, и его последующее размещение в Google Play Market или на других внешних сайтах. 
...
<i>Название_проекта\app\build\outputs\apk</i>. По умолчанию файл называется <i>app-debug.apk</i> и представляет debug-версию.</p>
<p>Но для полноценно публикации данного файла может оказаться недостаточно. И нам еще дополнительно надо произвести 
некоторую подготовку проекта к релизу. Для это следует указать в файле манифеста у элемента <code>&lt;manifest&gt;</code> установлены атрибуты <span class="bb">android:versionCode</span> и 
<span class="bb">android:versionName</span>. Также в файле манифеста элемент <code>&lt;application&gt;</code> не должен содержать атрибута <span class="bb">android:debuggable</span></p>
<p>Кроме того, на этом этапе можно установить иконку для приложения, которая будет отображаться на рабочем экране гаджета, название приложения 
(атрибут <span class="bb">android:label</span> у элемента), а также можно задать лицензионное соглашение.</p>
<p>В файле манифеста также следует определить название пакета (атрибут package элемента <code>&lt;manifest&gt;</code>), которое будет использоваться для приложения в дальнейшем. По умолчанию при разработке в 
Android Studio пакеты приложений начинаются с <i>com.example</i>. Не стоит оставлять данное название, так как название пакета будет служить уникальным идентификатором вашего приложения. Например, 
ниже в моем случае названием пакета служит "com.maverics.eugene.telephonelist":</p>
<pre class="brush:xml;">
...
<p>При этом если в файлах кода java название пакета в начале файла также должно соответствовать пакету приложения.</p>
<h3>Установка требований</h3>
<p>На этапе подготовки к релизу также можно установить требования к API. Например, наше приложение имеет определеную минимальную версию ОС Android, 
поэтому мы можем установить в файле манифеста соответствующие атрибуты у элемента <code>&lt;uses-sdk&gt;</code></p>
<ul>
<li><p><code>android:minSdkVersion</code> — минимальная версия Android</p></li>
<li><p><code>android:targetSdkVersion</code> — оптимальная версия API</p></li>
<li><p><code>android:maxSdkVersion</code> — максимальная версия системы</p></li>
</ul>
<p>Например, пусть минимальная версия Jelly Beans 4.1.2, а оптимальная KitKat 4.4.4:</p>
<pre class="brush:xml;">
&lt;manifest xmlns:android="http://schemas.android.com/apk/res/android"
    package="com.maverics.eugene.telephonelist"
    android:versionName="1.0"
...
<p>Когда все уже готово, приложение для Android должно быть подписано сертификатом, благодаря которому можно идентифицировать автора приложения. 
...
нам надо произвести дополнительно ряд действий.</p>
<p>При создании сертификата следует помнить, что при обновлении приложения система будет сравнивать сертификаты старой и новой версии. 
...
В этом случае чтобы его установить, новая версия должна будет иметь другой название пакета, нежели старая.</p>
<p>Во-первых, в Android Studio выберем в меню пункт <span class="bb">Build -&gt; Generate Signed APK</span>. После этого нам откроется окно мастера:</p>
<img src="./pics/signedapk.png" alt="Generate Signed APK" />
<p>Нажмем на кнопку <code>Create new...</code>. После этого нам откроется окно создания ключа:</p>
<img src="./pics/keystore.png" alt="Создание APK в Android" />
<p>Введем в поле <code>Key store path</code> путь к файлу сетификата, который будет создан. Если указанной папки не существует, то ее надо создать или определить 
существующую папку.</p>
<p>В поле Password/Confirm указываем пароль.</p>
<p>В поле <code>Alias</code> указываем псевдоним. Можно поставить произвольное название.</p>
<p>В поле <code>First and Last Name</code> вписываем имя и фамилию. И далее пишим подразделение, организацию, город, страну и код страны.</p>
<p>В конце нажимаем OK.</p>
<p>После этого автоматически обновится первое окошко:</p>
<img src="./pics/signedapk2.png" alt="Создание подписанного APK и сертификата" />
<p>Далее нажмем на кнопку Next:</p>
<img src="./pics/signedapk3.png" />
<p>Финальное окно покажет нам путь к каталогу, где будет находиться подписанное приложение apk в release-версии. Нажмем на Finish.</p>
<p>Теперь по указанному пути можно будет найти подписанный apk, который будет иметь название <i>app-release.apk</i>:</p>
<img src="./pics/signedapk4.png" />
<p>Мы можем переименовать файл, сохранив его расширение и выложить в Play Market или на любой сайт или сразу загрузить на мобильное устройство. 
...
<code>Безопасность-&gt;Неизвестные источники (Разрешить установку приложений из других источников)</code></p>
...
<div class="nav"><p><a href="./20.6.php">Назад</a><a href="./">Содержание</a><a href="./21.2.php">Вперед</a></p></div></div>
<div class="socBlock">
