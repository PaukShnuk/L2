...
<html >
<head>
<title>Java и Android | Cell-анимация</title>
<meta charset="utf-8" />
<meta name="description" content="Cell-анимация фреймов AnimationDrawable в Java и Android">
...
<h1>Анимация</h1><h2>Cell-анимация</h2><div class="date">Последнее обновление: 18.10.2021</div>
...
<p><span class="b">Cell animation</span> или <span class="b">анимация фреймов</span> представляет собой технику анимации, при 
которой ряд изображений или кадров/фреймов последовательно сменяют друг друга за короткий промежуток времени. 
Подобная техника довольно распространена при создании мультфильмов. Например, имеется следующий набор изображений:</p>
<img src="./pics/a1.png" />
<img src="./pics/a2.png" />
<img src="./pics/a3.png" />
<img src="./pics/a4.png" />
...
<img src="./pics/a6.png" />
<img src="./pics/a7.png" />
<p>При достаточно быстрой смене кадров получится динамический эффект зайца, забрасывающего мяч в баскетбольную корзину. Теперь рассмотрим, как сделать 
подобную анимацию в приложении Android.</p>
<p>Во-первых, нам надо добавить все эти изображения в проект в папку <span class="b">res/drawable</span>. И в эту же папку добавим новый xml-файл. Назовем его <span class="b">rabit_animation.xml</span> 
и поместим в него следующее содержимое:</p>
<pre class="brush:xml;">
...
<img src="./pics/cellanimation1.png" alt="Анимация фреймов animation-list в Android и Java" />
<p>Анимация определяется с помощью корневого элемента <span class="b">animation-list</span>, который содержит набор ключевых кадров в виде элементов item.</p>
<p>Свойство <code>android:oneshot="false"</code> в определении корневого элемента указывает, что анимация будут продолжатся циклически. А при значении <code>true</code> 
анимация срабатывала только один раз.</p>
<p>Каждый элемент аннимации устанавливает ссылку на ресурс изображения с помощью свойства <code>android:drawable</code>, а также 
с помощью свойства <span class="b">android:duration</span> устанавливает время в миллисекундах, которое будет отображаться изображение.</p>
<p>В разметке интерфейса для отображения анимации используется элемент ImageView:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
...
<p>Далее изменим код <span class="b">MainActivity</span>, чтобы запустить анимацию:</p>
...
<p>С помощью метода <span class="b">setBackgroundResource()</span> объекта ImageView устанавливается ресурс анимации. Затем из ImageView получаем собственно объект анимации <span class="b">AnimationDrawable</span> и по нажатию 
на ImageView запускаем анимацию с помощью метода <span class="b">start()</span>.</p>
<img src="./pics/cellanimation2.png" alt="Анимация фреймов AnimationDrawable в Android и Java" />
<p>Стоит отметить, что метод <code>start()</code> объекта AnimationDrawable не вызывается напрямую из <code>onCreate()</code> 
класса MainActivity, так как при выполнении метода <code>onCreate()</code> объект AnimationDrawable еще полностью не определен. 
...
Если же необходимо автоматически запустить анимацию при запуске приложения, то можно это делать в методе <span class="b">onStart()</span> 
класса Activity.</p>
...
<div class="nav"><p><a href="./18.4.php">Назад</a><a href="./">Содержание</a><a href="./19.2.php">Вперед</a></p></div></div>
<div class="socBlock">
