...
<html >
<head>
<title>Java и Android | Сохранение состояния приложения</title>
<meta charset="utf-8" />
<meta name="description" content="Сохранение и восстановление состояния в Android и Java, методы объекта Bundle, использование методов onRestoreInstanceState и onSaveInstanceState">
...
<h1>Настройки и состояние приложения</h1><h2>Сохранение состояния приложения</h2><div class="date">Последнее обновление: 17.10.2021</div>
...
<p>В одной из предыдущих тем был рассмотрен жизненный цикл Activity в приложении на Android, где после создания Activity вызывался метод <span class="b">onRestoreInstanceState</span>, 
который восстанавливал ее состояние, а перед завершением работы вызывался метод <span class="b">onSaveInstanceState</span>, который сохранял состояние Actiity. 
Оба этих метода в качестве параметра принимают объект <span class="b">Bundle</span>, который как раз и хранит состояние activity:</p>
<pre class="brush:java;">
protected void onRestoreInstanceState(Bundle saveInstanceState);
protected void onSaveInstanceState(Bundle saveInstanceState);
</pre>
<p>В какой ситуации могут быть уместно использование подобных методов? Банальная ситуация - переворот экрана и переход от портретной ориентации к альбомной и наоборот. 
...
при изменении ориентации экрана их значения могут быть сброшены до значений по умолчанию.</p>
<p>Чтобы точнее понять проблему, с которой мы можем столкнуться, рассмотрим пример. Изменим файл <span class="b">activity_main</span> следующим образом:</p>
<pre class="brush:xml;">
...
<p>Здесь определено поле EditText, в которое вводим имя. И также определена кнопка для его сохранения.</p>
<p>Далее для вывода сохраненного имени предназначено поле TextView, а для получения сохраненного имени - вторая кнопка.</p>
<p>Теперь изменим класс <span class="b">MainActivity</span>:</p>
<pre class="brush:java;">
...
<p>Для хранения имени в программе определена переменная name. При нажатии на первую кнопку сохраняем текст из EditText в переменную name, а при нажатии на вторую кнопку - 
обратно получаем текст из переменной name в поле TextView.</p>
<p>Запустим приложение введем какое-нибудь имя, сохраним и получим его в TextView:</p>
<img src="./pics/state1.png" alt="Состояние activity в Android и Java" />
<p>Но если мы перейдем к альбомному режиму, то TextView окажется пустым, несмотря на то, что в него вроде бы уже получили нужное значение:</p>
<img src="./pics/state2.png" alt="Состояние Bundle activity в Android и Java" />
<p>И даже если мы попробуем заново получить значение из переменной name, то мы увидим, что она обнулилась:</p>
<img src="./pics/state3.png" alt="Сохранение состояния activity в Android и Java" />
<p>Чтобы избежать подобных ситуаций как раз и следует сохранять и восстанавливать состояние activity. Для этого изменим код MainActivity:</p>
<pre class="brush:java;">
package com.example.settingsapp;
...
<p>В методе <code>onSaveInstanceState()</code> сохраняем состояние. Для этого вызываем у параметра Bundle метод <span class="b">putString(key, value)</span>, 
первый параметр которого - ключ, а второй - значение сохраняемых данных. В данном случае мы сохраняем строку, поэтому вызываем метод <code>putString()</code>. Для 
сохранения объектов других типов данных мы можем вызвать соответствующий метод:</p>
<ul>
<li><p><span class="b">put()</span>: универсальный метод, который добавляет значение типа Object. Соответственно поле получения данное значение необходимо 
преобразовать к нужному типу</p></li>
<li><p><span class="b">putString()</span>: добавляет объект типа String</p></li>
<li><p><span class="b">putInt()</span>: добавляет значение типа int</p></li>
<li><p><span class="b">putByte()</span>: добавляет значение типа byte</p></li>
<li><p><span class="b">putChar()</span>: добавляет значение типа char</p></li>
<li><p><span class="b">putShort()</span>: добавляет значение типа short</p></li>
<li><p><span class="b">putLong()</span>: добавляет значение типа long</p></li>
<li><p><span class="b">putFloat()</span>: добавляет значение типа float</p></li>
<li><p><span class="b">putDouble()</span>: добавляет значение типа double</p></li>
<li><p><span class="b">putBoolean()</span>: добавляет значение типа boolean</p></li>
<li><p><span class="b">putCharArray()</span>: добавляет массив объектов char</p></li>
<li><p><span class="b">putIntArray()</span>: добавляет массив объектов int</p></li>
<li><p><span class="b">putFloatArray()</span>: добавляет массив объектов float</p></li>
<li><p><span class="b">putSerializable()</span>: добавляет объект интерфейса Serializable</p></li>
<li><p><span class="b">putParcelable()</span>: добавляет объект Parcelable</p></li>
</ul>
<p>Каждый такой метод также в качестве первого параметра принимает ключа, а в качестве второго - значение.</p>
<p>В методе <span class="b">onRestoreInstanceState</span> происходит обратный процесс - с помощью метода <span class="b">getString(key)</span> по ключу получаем из сохраненного состояния строку по ключу. 
Соответственно для получения данных других типов мы можем использовать аналогичные методы:</p>
<ul>
<li><p><span class="b">get()</span>: универсальный метод, который возвращает значение типа Object. Соответственно поле получения данное значение необходимо 
преобразовать к нужному типу</p></li>
<li><p><span class="b">getString()</span>: возвращает объект типа String</p></li>
<li><p><span class="b">getInt()</span>: возвращает значение типа int</p></li>
<li><p><span class="b">getByte()</span>: возвращает значение типа byte</p></li>
<li><p><span class="b">getChar()</span>: возвращает значение типа char</p></li>
<li><p><span class="b">getShort()</span>: возвращает значение типа short</p></li>
<li><p><span class="b">getLong()</span>: возвращает значение типа long</p></li>
<li><p><span class="b">getFloat()</span>: возвращает значение типа float</p></li>
<li><p><span class="b">getDouble()</span>: возвращает значение типа double</p></li>
<li><p><span class="b">getBoolean()</span>: возвращает значение типа boolean</p></li>
<li><p><span class="b">getCharArray()</span>: возвращает массив объектов char</p></li>
<li><p><span class="b">getIntArray()</span>: возвращает массив объектов int</p></li>
<li><p><span class="b">getFloatArray()</span>: возвращает массив объектов float</p></li>
<li><p><span class="b">getSerializable()</span>: возвращает объект интерфейса Serializable</p></li>
<li><p><span class="b">getParcelable()</span>: возвращает объект Parcelable</p></li>
</ul>
<p>Для примера рассмотрим сохранение-получение более сложных данных. Например, объектов определенного класса. Пусть у нас есть класс <span class="b">User</span>:</p>
<pre class="brush:java;">
package com.example.settingsapp;
...
<p>Класс User реализует интерфейс <span class="b">Serializable</span>, поэтому мы можем сохранить его объекты с помощью метода <code>putSerializable()</code>, а получить 
с помощью метода <code>getSerializable()</code>.</p>
<p>Пусть у нас будет следующий интерфейс в <span class="b">activity_main.xml</span>:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="utf-8"?&gt;
&lt;androidx.constraintlayout.widget.ConstraintLayout
    xmlns:android="http://schemas.android.com/apk/res/android"
...
<p>Здесь определены два поля ввода для имени и возраста соответственно.</p>
<p>В классе <span class="brush:java;">MainActivity</span> пропишем логику сохранения и получения данных:</p>
<pre class="brush:java;">
package com.example.settingsapp;
...
<p>Здесь также сохраняем данные в переменную User, которая предварительно инициализированна некоторыми данными по умолчанию. А при нажатии на кнопку получения 
получем данные из переменной и передаем их для вывода в текстовое поле.</p>
<img src="./pics/state4.png" alt="onRestoreInstanceState и onSaveInstanceState 	 в Android и Java" />
...
<div class="nav"><p><a href="./11.2.php">Назад</a><a href="./">Содержание</a><a href="./12.1.php">Вперед</a></p></div></div>
<div class="socBlock">
