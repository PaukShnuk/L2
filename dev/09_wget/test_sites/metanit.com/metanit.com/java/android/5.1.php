...
<html >
<head>
<title>Java и Android | ListView и ArrayAdapter</title>
<meta charset="utf-8" />
<meta name="description" content="Адаптеры и списки. ArrayView, ArrayAdapter в Android и Java, связь ListView с источником данных">
...
<h1>Адаптеры и списки</h1><h2>ListView и ArrayAdapter</h2><div class="date">Последнее обновление: 16.10.2021</div>
...
<p>Android представляет широкую палитру элементов,которые представляют списки. Все они является наследниками класса 
<span class="b">android.widget.AdapterView</span>. Это такие виджеты как ListView, GridView, Spinner. Они могут выступать контейнерами для других элементов управления</p>
<img src="./pics/adapters.png" alt="Адаптеры в Android" />
<p>При работе со списками мы имеем дело с тремя компонентами. Во-первых, это визуальный элемент или виджет, который на экране представляет список 
(ListView, GridView) и который отображает данные. Во-вторых, это источник данных - массив, объект ArrayList, база данных и т.д., в котором находятся 
сами отображаемые данные. И в-третьих, это адаптер - специальный компонент, который связывает источник данных с виджетом списка.</p>
<p>Одним из самых простых и распространенных элементов списка является виджет <span class="b">ListView</span>. 
Рассмотрим связь элемента ListView с источником данных с помощью одного из таких адаптеров - класса <span class="b">ArrayAdapter</span>.</p>
<p>Класс <span class="b">ArrayAdapter</span> представляет собой простейший адаптер, который связывает массив данных с набором элементов 
<code>TextView</code>, из которых, к примеру, может состоять <code>ListView</code>. То есть в данном случае источником данных выступает массив объектов. 
ArrayAdapter вызывает у каждого объекта метод <code>toString()</code> для приведения к строковому виду и полученную строку устанавливает в элемент TextView.</p>
<p>Посмотрим на примере. Итак, разметка приложения может выглядеть так:</p>
<pre class="brush:xml;">
...
<p>Здесь также определен элемент ListView, который будет выводить список объектов. 
Теперь перейдем к коду activity и свяжем ListView через ArrayAdapter с некоторыми данными:</p>
...
<p>Здесь вначале получаем по id элемент ListView и затем создаем для него адаптер.</p>
<p>Для создания адаптера использовался следующий конструктор <code>ArrayAdapter&lt;String&gt;(this,android.R.layout.simple_list_item_1, countries)</code>, где </p>
<ul>
<li><p><span class="b">this</span> : текущий объект activity</p></li>
<li><p><span class="b">android.R.layout.simple_list_item_1</span> : файл разметки списка, который фреймворк представляет по умолчанию. Он 
...
мы можем создать свою и потом в коде изменить id на id нужной нам разметки</p></li>
<li><p><span class="b">countries</span> : массив данных. Здесь необязательно указывать именно массив, это может быть список <code>ArrayList&lt;T&gt;</code>.</p></li>
</ul>
<p>В конце неоходимо установить для ListView адаптер с помощью метода <code>setAdapter()</code>.</p>
<p>В итоге мы получим следующее отображение:</p>
<img src="./pics/arrayadapter1.png" alt="Адаптер ArrayAdapter в Android и Java" />
...
<div class="nav"><p><a href="./4.15.php">Назад</a><a href="./">Содержание</a><a href="./5.7.php">Вперед</a></p></div></div>
<div class="socBlock">
