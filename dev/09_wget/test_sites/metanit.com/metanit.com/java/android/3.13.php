...
<html >
<head>
<title>Java и Android | Определение размеров</title>
<meta charset="utf-8" />
<meta name="description" content="Определение размеров в Android, метод getDimension и TypedValue.applyDimension, dp, sp, px, mm, pt, точки, дюймы, пиксели, device-independent pixels">
...
<h2>Определение размеров</h2><div class="date">Последнее обновление: 14.10.2021</div>
...
<ul>
<li><p><code>px</code>:  пиксели текущего экрана. Однако эта единица измерения не рекомендуется, так как реальное представление внешнего вида 
...
пикселей на экране может также меняться</p></li>
<li><p><code>dp</code>:  (device-independent pixels) независимые от плотности экрана пиксели. Абстрактная единица измерения, основанная на физической плотности экрана 
...
Общая формула для получения количества физических пикселей из dp: <span class="b">px = dp * (dpi / 160)</span></p></li>
<li><p><code>sp</code>:  (scale-independent pixels) независимые от масштабирования пиксели. Допускают настройку размеров, производимую пользователем. 
Рекомендуются для работы со шрифтами.</p></li>
<li><p><code>pt</code>:  1/72 дюйма, базируются на физических размерах экрана</p></li>
<li><p><code>mm</code>:  миллиметры</p></li>
<li><p><code>in</code>:  дюймы</p></li></ul>
...
<img src="./pics/2.20.png" alt="Разрешение экрана в Android" />
<p>Используя же стандартные физические пиксели мы можем столкнуться с проблемой, что размеры элементов также будут сильно варьироваться в зависимости от 
плотности пикселей устройства. Например, возьмем 3 устройства с различными характеристиками экрана Nexus 4, Nexus 5X и Nexus 6P и выведем на экран квадрат размером 300px на 300px:</p>
<img src="./pics/2.21.png" alt="Физические пиксели в Android" />
<p>В одном случае квадрат по ширине будет занимать 40%, в другом - треть ширины, в третьем - 20%.</p>
<p>Теперь также возьмем квадрат со сторонами 300х300, но теперь вместо физических пикселей используем единицы dp:</p>
<img src="./pics/2.22.png" alt="независимые от плотности пиксели в Android" />
<p>Теперь же размеры квадрата на разных устройствах выглядят более консистентно.</p>
<p>Для упрощения работы с размерами все размеры разбиты на несколько групп:</p>
<ul>
<li><p><span class="b">ldpi (low)</span>: ~120dpi</p></li>
<li><p><span class="b">mdpi (medium)</span>: ~160dpi</p></li>
<li><p><span class="b">hdpi (high)</span>: ~240dpi (к данной группе можно отнести такое древнее устройство как Nexus One)</p></li>
<li><p><span class="b">xhdpi (extra-high)</span>: ~320dpi (Nexus 4)</p></li>
<li><p><span class="b">xxhdpi (extra-extra-high)</span>: ~480dpi (Nexus 5/5X, Samsung Galaxy S5)</p></li>
<li><p><span class="b">xxxhdpi (extra-extra-extra-high)</span>: ~640dpi (Nexus 6/6P, Samsung Galaxy S6)</p></li>
</ul>
<h3>Установка размеров</h3>
<p>Основная проблема, связанная с размерами, связана с их установкой в коде Java. Например, некоторые методы принимают в качестве значения физические 
...
метод <span class="b">TypedValue.applyDimension()</span>, который принимает три параметра:</p>
<pre class="brush:java;">
public static float applyDimension(int unit,
                                   float value,
                                   android.util.DisplayMetrics metrics)
</pre>
<p>Параметр <code>unit</code> представляет тип единиц, из которой надо получить значение в пикселях. Тип единиц описывается одной из констант <span class="b">TypedValue</span>:</p>
<ul>
<li><p><code>COMPLEX_UNIT_DIP</code> - dp или независимые от плотности экрана пиксели</p></li>
<li><p><code>COMPLEX_UNIT_IN</code> - in или дюймы</p></li>
<li><p><code>COMPLEX_UNIT_MM</code> - mm или миллиметры</p></li>
<li><p><code>COMPLEX_UNIT_PT</code> - pt или точки</p></li>
<li><p><code>COMPLEX_UNIT_PX</code> - px или физические пиксели</p></li>
<li><p><code>COMPLEX_UNIT_SP</code> - sp или независимые от масштабирования пиксели (scale-independent pixels)</p></li>
</ul>
<p>Параметр <code>value</code> представляет значение, которое надо преобразовать</p>.
<p>Параметр <code>metrics</code> представляет информацию о метрике, в рамках коорой надо выполнить преобразование.</p>
<p>В итоге метод возвращает преобразованное значение. Рассмотрим абстрактный пример. Например, нам надо получить из 60dp обычные физические пиксели:</p>
<pre class="brush:java;">
int valueInDp = 60;
int valueInPx = (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, valueInDp, getResources().getDisplayMetrics());
</pre>
<p>В качестве третьго аргумента передается вызов метода <code>getResources().getDisplayMetrics()</code>, который позволяет получить информацию о метрике, 
связанной с текущим устройством. В итоге мы получим из 60dp некоторое количество пикселей.</p>
...
<div class="nav"><p><a href="./2.8.php">Назад</a><a href="./">Содержание</a><a href="./3.9.php">Вперед</a></p></div></div>
<div class="socBlock">
