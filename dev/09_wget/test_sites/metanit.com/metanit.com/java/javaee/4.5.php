...
<html >
<head>
<title>Java EE | web.xml и маппинг сервлетов</title>
<meta charset="utf-8" />
<meta name="description" content="Файл web.xml и маппинг сервлетов в Java EE, параметр servlet-mapping">
...
<h2>web.xml и маппинг сервлетов</h2><div class="date">Последнее обновление: 13.09.2018</div>
...
<p>В предыдущих статьях при определении сервлетов применялась аннотация <span class="b">@WebServlet</span>, которая устанавливала конечную точку, 
с которой сопоставлялся сервлет. Например, следующий сервлет</p>
<pre class="brush:java;">
@WebServlet("/hello")
public class HelloServlet extends HttpServlet {
</pre>
<p>сопоставляется с путем "/hello". То есть сервлет HelloServlet будет обрабатывать запросы типа "http://localhost:8081/[название_приложения]/hello" или 
"http://somesite.com/[название_приложения]/hello".</p>
<p>Но есть и другой способ сопоставления путей и сервлетов. Он представляет использование файла web.xml.</p>
<p>Файл <span class="b">web.xml</span> хранит информацию о конфигурации приложения. Он не является обязательной частью приложения, 
как можно увидеть из предыдущих статей, тем не менее он широко используется для настройки конфигурации.</p>
<p>Данный файл должен располагаться в папке <span class="b">WEB-INF</span> приложения. При запуске Tomcat считывает его содержимое и 
использует считанную конфигурацию. Если же файл содержит ошибки, то Tomcat отображает ошибку.</p>
<p>web.xml имеет определенную структуру. Все вложенные узлы, которые определяют конфигурацию, помещаются в корневой узел <span class="b">&lt;web-app&gt;</span>.</p>
<pre class="brush:xml;">
...
<p>У элемента web-app определяется ряд атрибутов. В данном случае атрибуты xmlns и xmlns:xsi указывают на используемые пространства имен xml. 
Атрибут version указывает на версию спецификации сервлетов или Servlet API, которая используется в приложении. Последняя версия API сервлетов - 4-я версия.</p>
<p>С помощью элемента <span class="b">&lt;servlet-mapping&gt;</span> можно задать сопоставление сервлета с запрашиваемым URL.</p>
<p>Например, добавим в проект в Eclipse в папку <span class="ii">WebConent/WEB-INF</span> новый файл <span class="b">web.xml</span>:</p>
<img src="pics/servlet16.png" alt="servlet mapping в web.xml" />
...
<p>Прежде всего вначале с помощью элемента <span class="b">&lt;servlet&gt;</span> определяется сервлет. Элемент 
<span class="b">&lt;servlet-name&gt;</span> задает имя сервлета, на которое будет проецироватья класс, указанный в элементе 
<span class="b">&lt;servlet-class&gt;</span>. То есть, допустим, у нас проекте есть класс сервлета HelloServlet, который будет 
проецироваться на имя HelloWorld. Имя может быть произвольным и может совпадать с названием класса.</p>
<p>Затем в элементе <span class="b">&lt;servlet-mapping&gt;</span> сервлет с именем HelloWorld (по сути сервлет HelloServlet) 
сопоставляется с путем "/welcome".</p>
<p>Допустим, сервлет HelloServlet будет выглядеть следующим образом:</p>
<pre class="brush:java;">
...
<p>Теперь, чтобы обратиться к этому сервлету, надо использовать путь "/welcome":</p>
<img src="pics/servlet18.png" alt="Маппинг сервлетов в Java EE" />
