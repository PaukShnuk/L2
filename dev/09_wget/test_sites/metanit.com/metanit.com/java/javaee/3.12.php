...
<html >
<head>
<title>Java EE | Основные возможности JSTL</title>
<meta charset="utf-8" />
<meta name="description" content="Основные возможности JSTL в Java EE, использование циклов и условных выражений в страницах JSP">
...
<h2>Основные возможности JSTL</h2><div class="date">Последнее обновление: 28.09.2018</div>
...
<p>Рассмотрим некоторые основные возможности JSTL.</p>
<h3>Цикл</h3>
...
<p>В данном случае параметр "items" указывает на коллекцию, элементы которой выводятся. А параметр "var" задает переменную, 
через которую доступен текущий перебираемый элемент.</p>
...
<img src="pics/jstl5.png" alt="Цикл в JSTL в JSP" />
...
<p>В данном случае если атрибут isVisible равен true, то выводится код, который расположен между тегами 
<code>&lt;c:if&gt;</code> и <code>&lt;/c:if&gt;</code></p>
...
<p>В данном случае тег <code>c:choose</code> проверяет значение атрибута "val". Для проверки применяются вложенные теги 
<code>c:when</code>, которые аналогичны блокам case в конструкции switch..case. С помощью их параметра <code>test</code> 
...
внутри данного элемента c:when. Таким образом мы можем определить несколько блоков c:when. Дополнительный тег <code>&lt;c:otherwise&gt;</code> 
выполняется, если условия проверки значения во всех тегах c:when ложно.</p>
<h3>Тег url</h3>
<p>Тег <code>&lt;c:url&gt;</code> позволяет создать адрес относительно корня приложения. Этот тег может применяться, например, при создании 
ссылок.</p>
<p>Например,</p>
<pre class="brush:xml;">
&lt;%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %&gt;
...
<img src="pics/jstl6.png" alt="url in JSTL in JSP" />
<h3>Редирект</h3>
<p>С помощью тега <code>redirect</code> можно установить редирект на другой адрес. Например, в случае если атрибут val не определен, то 
делаем редирект на страницу "notfound.jsp":</p>
<pre class="brush:xml;">
&lt;c:if test="${val == null}"&gt;
...
<div class="nav"><p><a href="3.11.php">Назад</a><a href="./">Содержание</a><a href="5.1.php">Вперед</a></p></div></div>
<div class="socBlock">
