...
<html >
<head>
<title>Java EE | Получение данных в сервлете</title>
<meta charset="utf-8" />
<meta name="description" content="Получение данных в сервлете в Java EE, отправка данных через строку запросу и форму, методы getParameter и getParameterValues">
...
<h2>Получение данных в сервлете</h2><div class="date">Последнее обновление: 07.09.2018</div>
...
<p>В методы doGet и doPost сервлета, которые обрабатывают запрос, в качестве одного из параметров передается объект 
<span class="b">HttpServletRequest</span>, с помощью которого можно получить отправленные сервлету данные, то есть параметры запроса. Для этого в классе HttpServletRequest 
определены два метода:</p>
<ul>
...
<p>Передавать значения в сервлет можно различными способами. При отправке GET-запроса значения передаются через строку запроса. 
Стандартный get-запрос принимает примерно следующую форму: <code>название_ресурса?параметр1=значение1&параметр2=значение2.</code></p>
...
<p>В данном случае мы предполагаем, что в сервлет в get-запросе передается значение/параметр по имени id. С помощью вызова <code>request.getParameter("id")</code> 
...
метод getParameter возвратить строку.</p>
<p>Запустим приложение и обратимся к сервлету:</p>
<img src="pics/servlet7.png" alt="Запрос GET к сервлету" />
<p>В случае выше значение для параметра id не указано, мы просто вводим в адресной строке браузера название ресурса - в данном случае <code>http://localhost:8081/helloapp/hello</code>, 
где helloapp - название приложения, а hello - конечная точка, к которой примонтирован сервлет. Поэтому метод getParameter("id") возвращает значение null.</p>
<p>Теперь передадим значение для параметра id:</p>
<img src="pics/servlet8.png" alt="Передача данных в сервлет" />
<p>Параметры в строке запроса указываютя после названия ресурса после знака вопроса: <code>http://localhost:8081/helloapp/hello?id=5</code>. 
В данном случае параметр id равен 5.</p>
...
<p>В данном случае сервлет получает два параметра: name и age. Мы можем передать значения для них, набрав в адресной строке, например, 
<code>http://localhost:8081/helloapp/hello?name=Tom&amp;age=34</code>. При передаче некольких параметров они отделяются друг от друга знаком амперсанда &amp;. 
Подобным образом мы можем передать и большее количество параметров.</p>
<img src="pics/servlet9.png" alt="Параметры сервлета Java" />
<h3>Передача массивов</h3>
<p>В примерах выше передавались одиночные данные - отдельные строки. Но также мы можем передавать наборы значений. Например, изменим сервлет 
следующим образом:</p>
<pre class="brush:java;">
import java.io.PrintWriter;
import java.io.IOException;
...
<p>Если в сервлет передается массив значений, то для его получения у объекта HttpServletRequest применяется метод 
<span class="b">getParameterValues()</span>, который получает название параметра и возвращает массив строк. В данном случае мы ожидаем, что параметр будет называться 
nums. Затем значения из этого параметра в цикле передаются в ответ клиенту.</p>
<p>Запустим сервлет и передадим ему значения</p>
<img src="pics/servlet10.png" alt="Передача в сервлет Java массив значений" />
<p>При передаче массива через строку запроса указываются несколько значений с одним и тем же именем: <code>http://localhost:8081/helloapp/hello?nums=1&amp;nums=2&amp;nums=3</code>.</p>
<h3>Получение данных из форм</h3>
<p>Еще одним распространенным способом отправки данных является отправка форм. Добавим в проект страницу <span class="b">index.html</span> 
...
<p>Как правило, формы отправлются с помощью запроса POST, поэтому у элемента формы определен 
атрибут <code>method="POST"</code>. Сама форма будет отправляться на ресурс "/hello", с которым будет 
сопоставляться сервлет. На самой форме есть множество полей ввода, в том числе набор чекбоксов, из 
которых можно выбрать сразу несколько значений.</p>
<img src="pics/servlet11.png" alt="Отправка форм в сервлет" />
...
<p>Данный сервлет будет обрабатывать запросы к по адресу "/hello", на который отпавляется форма. Поскольку отправка формы 
осущетвляется с помощью метода POST, то для обработки запроса определен метод <span class="b">doPost</span>. Метод doPost принимает те же параметры, что и 
метод doGet.</p>
<p>С помощью методов <code>getParameter()</code> и <code>getParameterValues()</code> также получаем 
значения параметров. В данном случае названия параметров представляют названия полей ввода отправленной формы.</p>
<p>Причем поскольку на форме несколько чекбоксов, которые имеют одно и то же имя, то соответственно сервлету передается набор значений, 
который получаем с помощью метода <code>getParameterValues()</code>. Все остальные поля представляют одиночные значения, поэтому для 
получения их данных применяется метод <code>getParameter()</code>.</p>
<img src="pics/servlet12.png" alt="Отправка данных в сервлет Java" />
