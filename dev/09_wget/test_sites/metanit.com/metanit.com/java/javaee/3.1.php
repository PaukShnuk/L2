...
<html >
<head>
<title>Java EE | Что такое JSP. Первое приложение</title>
<meta charset="utf-8" />
<meta name="description" content="Что такое Java Server Pages, первое приложение, размещение страниц JSP на Apache Tomcat">
...
<h1>Java Server Pages</h1><h2>Что такое JSP. Первое приложение</h2><div class="date">Последнее обновление: 02.09.2018</div>
...
<p><span class="b">Java Server Pages</span> представляет технологию, которая позволяет создавать динамические веб-страницы. Изначально JSP (вместе с сервлетами) 
...
место другой технологии - JSF, тем не менее JSP продолжают широко использоваться.</p>
<p>По сути Java Server Page или JSP представляет собой html-код с вкраплениями кода Java. В то же время станицы jsp - это не стандартные html-страницы. Когда приходит запрос к определенной 
...
JSP видит в своем браузере обычную html-страницу.</p>
<p>Как и обычные статические веб-страницы, файлы JSP необходимо размещать на веб-сервере, к которому обычные пользователи могут обращаться по 
...
JSP (<span class="b">JSP engine</span>), который также называют JSP-контейнером. Есть множество движков JSP, и все они реализуют одну и ту же спецификацию и в целом работают одинаково. 
Однако тем не менее при переносе кода с одного веб-сервера на другой могут потребоваться небольшие изменения.</p>
<img src="pics/webserver1.png" alt="JSP Engine" />
<p>В данном случае для работы с JSP мы будем использовать Apache Tomcat, который одновременно является и веб-сервером и контейнером сервлетов и JSP.</p>
<p>Создадим простейшую страницу JSP. Для этого где-нибудь на жестком диске определим файл <span class="b">index.jsp</span>. 
Все станицы JSP имеют расширение <span class="ii">jsp</span>. Откроем этот файл в любом текстовом редакторе и определим в нем 
следующий код:</p>
<pre class="brush:xml;">
&lt;%
...
<p>С помощью тегов <code>&lt;% ... %&gt;</code> мы можем определить код Java на странице JSP. В данном случае мы просто определяем 
переменную типа String, которая называется header.</p>
<p>Затем идет стандартный код страницы html. Чтобы внедрить код java внутрь html-страницы применяются теги <code>&lt;%=  %&gt;</code> - 
...
выражение <code>new java.util.Date()</code>, которое возвращает текущую дату.</p>
<p>Для тех, кто знаком с веб-разработкой на PHP, это может напоминать оформление файлов php, которые также содержать код html и код php.</p>
<p>Теперь поместим данный файл на сервер - в данном случае в контейнер Tomcat. Перейдем в Apache Tomcat к папке 
<span class="ii">webapps\ROOT</span>. Удалим из нее все содержимое и поместим нашу страницу <span class="b">index.jsp</span>, которая была 
создана выше.</p>
<img src="pics/jsp1.png" alt="Java Server Pages" />
<p>Запустим Apache Tomcat (если он не запущен), и обратимся к приложению по адресу <span class="ii">http://localhost:xxxx/index.jsp</span>, 
где xxxx - номер порта, по которому запущен Tomcat:</p>
<img src="pics/jsp2.png" alt="Введение в Java Server Page" />
<p>В итоге Tomcat получит запрос к странице index.jsp, обработает код на java, сгенерирует html-страницу и отправит ее пользователю.</p>
<p>По умолчанию Apache Tomcat настроен таким образом, что все запросы к корню приложения по умолчанию обрабатываются страницей 
index.jsp, поэтому мы также можем обращаться к ней по адресу <span class="ii">http://localhost:xxxx</span></p>
...
<div class="nav"><p><a href="4.10.php">Назад</a><a href="./">Содержание</a><a href="3.2.php">Вперед</a></p></div></div>
<div class="socBlock">
