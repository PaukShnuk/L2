...
<html >
<head>
<title>Java EE | Работа с JSP в Eclipse</title>
<meta charset="utf-8" />
<meta name="description" content="Создание первого приложения на JSP в Eclipse">
...
<h2>Работа с JSP в Eclipse</h2><div class="date">Последнее обновление: 02.09.2018</div>
...
<p>Хотя мы вполне можем создавать страницы JSP в обычном текстовом редакторе, но также мы можем использовать и среды разработки, например, 
Eclipse.</p>
<p>Для работы с JSP в Eclipse создадим новый проект. Для этого перейдем к пункту меню <span class="b">File -&gt; New -&gt; Dynamic Web Project</span></p>
<img src="pics/jsp3.png" alt="Java Server Pages in Eclipse" />
<p>Далее нам откроется окно создания нового проекта.</p>
<img src="pics/jsp4.png" alt="Dynamic Web Project in Eclipse" />
<p>В поле <span class="b">Project Name</span> укажем для проекта какое-нибудь имя, например, <span class="b">hellojsp</span> 
и нажмем на кнопку Finish для создания проекта.</p>
<p>После этого будет создан новый проект, который по умолчанию будет иметь некоторое содержимое. Все файлы с исходным кодом, в 
том числе jsp-страницы, помещаются в проекте в каталог <span class="b">Web Content</span>. Нажмем на него правой кнопкой мыши и 
в контекстном меню выберем пункт <span class="b">New -&gt; JSP File</span></p>
<img src="pics/jsp5.png" alt="Добавление JSP в Eclipse" />
<p>Далее откроется окно добавления нового файла:</p>
<img src="pics/jsp6.png" alt="JSP в Eclipse" />
<p>Укажем для нового файла имя, например, <span class="b">hello.jsp</span> и нажмем на кнопку Finish. После этого в проект будет добвлен новый файл hello.jsp.</p>
<p>Чтобы открыть данный файл, нажмем на него. Заменим его код на следующий:</p>
<pre class="brush:xml;">
&lt;%
...
<p>Теперь запустим данный файл. Для этого нажмем на него правой кнопкой мыши и в контекстном меню выберем <span class="b">Run As -&gt; Run on Server</span>:</p>
<img src="pics/jsp7.png" alt="Запуск JSP в Eclipse" />
<p>Далее откроется окно для выбора сервера. Среди серверов уже значится Apache Tomcat 9.0, соответственно он и будет использоваться. 
Опционально можно отметить поле <span class="b">Always use this server when running this project</span>, что позволит в дальнейшем 
упростить запуск проекта:</p>
<img src="pics/jsp8.png" />
<p>После нажатия на кнопку Finish будет запущен Apache Tomcat, и в эмуляторе браузера внутри Eclipse произойдет обращение к странице hello.jsp:</p>
<img src="pics/jsp9.png" />
<p>Следует учитывать, что если ранее уже был запущен Tomcat, например, при автозапуске и вручную через панель служб, то надо его остановить. Иначе при запуске из Eclipse 
мы столкнемся с ошибкой, что Eclipse не может запустить Tomcat, так как его порт уже занят.</p>
<p>Несмотря на то, что по умолчанию срабатывает эмулятор браузера внутри Eclipse, но мы можем взять адрес приложения и обратится к нему 
в любом стандартном веб-браузере:</p>
<img src="pics/jsp10.png" />
