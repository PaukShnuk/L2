...
<html >
<head>
<title>Java EE | Сервлеты в Eclipse</title>
<meta charset="utf-8" />
<meta name="description" content="Сервлеты Java EE в Eclipse, добавление и настройка сервлета">
...
<h2>Сервлеты в Eclipse</h2><div class="date">Последнее обновление: 07.09.2018</div>
...
<p>Ручная компиляция сервлетов, кончено, занятие очень увлекательное, но гораздо проще работать с сервлетами в рамках IDE, например, Eclipse. 
Рассмотрим работу с сервлетами в Eclipse.</p>
<p>Создадим в Eclipse проект по типу <span class="b">Dynamic Web Project</span>. Допустим, он будет называться helloapp.</p>
<p>Как и стандартные классы, сервлеты помещаются в проекте в Eclipse в папку <span class="b">Java Resources/src</span>. При этом мы можем 
добавить обычный класс Java и затем нужным нам образом изменить его код. Либо мы можем вопользоваться готовым шаблоном под названием <span class="b">Servlet</span>, который 
представляет базовую структуру сервлета. Используем этот шаблон. Нажмем правой кнопкой мыши на папку <span class="b">Java Resources/src</span> и 
в контекстном меню выберем пункт <span class="b">File -&gt; Servlet</span>:</p>
<img src="pics/servlet4.png" alt="Servlet in Eclipse" />
<p>Далее нам откроется окно настройки сервлета.</p>
<img src="pics/servlet5.png" alt="Добавление сервлета в Eclipse" />
<p>В поле Class Name укажем для класса сервлета имя - HelloServlet. Также можно указать пакет для сервлета в поле Java Package. Если не указать названиепакета, то будет 
применяться пакет по умолчанию. Затем нажмем на кнопку Finish. И в проект будет добавлен сервлет с некоторым стандартным содержимым. 
Изменим его код на следующий:</p>
...
<p>Здесь определен точно такой же сервлет, как и в прошлой теме. Теперь запустим проект. Обратимся к сервлету, и он отправит в ответ html-код, который мы увидим в браузере:</p>
<img src="pics/servlet6.png" />
...
<div class="nav"><p><a href="4.8.php">Назад</a><a href="./">Содержание</a><a href="4.3.php">Вперед</a></p></div></div>
<div class="socBlock">
