...
<html >
<head>
<title>Java EE | Параметры инициализации сервлетов</title>
<meta charset="utf-8" />
<meta name="description" content="Параметры инициализации сервлетов в Java EE, параметр context-param и init-param">
...
<h2>Параметры инициализации сервлетов</h2><div class="date">Последнее обновление: 13.09.2018</div>
...
<p>Элемент <span class="b">&lt;context-param&gt;</span> в файле web.xml задает параметр, которым инициализируются сервлеты. Внутри этого элемента 
с помощью элемента <span class="b">&lt;param-name&gt;</span> задается имя параметра, а с помощью элемента <span class="b">&lt;param-value&gt;</span> - 
значение параметра.</p>
<p>Например, определим следующий файл web.xml:</p>
<pre class="brush:xml;">
...
<p>В данном случае параметр называется "message" и имеет значение "Hello Servlets". Этот параметр доступен всем сервлетам в приложении.</p>
<p>Для получения параметра внутри сервлета необходимо обратиться к контексту сервлета. Для его получения внутри 
класса сервлета определен метод <span class="b">getServletContext()</span>. Он возвращает объект  ServletContext. Затем у этого объекта 
вызывается метод <span class="b">getInitParameter()</span>, в который передается название параметра инициализации.</p>
...
<img src="pics/servlet17.png" alt="Параметры сервлетов Java EE" />
<p>Стоит отметить, что этот параметр будет доступен для всех сервлетов в приложении. Соответственно если мы изменим в web.xml значение параметра, 
то оно изменится для всех сервлетов.</p>
<h3>Параметры для отдельного сервлета</h3>
<p>Кроме определения общих для всех сервлетов параметров, мы можем определить параметры непосредственно для конкретного сервлета. Для этого внутри 
элемента <span class="b">&lt;servlet&gt;</span> определяется элемент <span class="b">&lt;init-param&gt;</span></p>
<p>Например, изменим web.xml следующим образом:</p>
<pre class="brush:xml;">
&lt;?xml version="1.0" encoding="UTF-8"?&gt;
&lt;web-app xmlns="http://xmlns.jcp.org/xml/ns/javaee"
  xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
...
<p>Здесь также задан параметр message со значением "Hello Servlets". Чтобы получить этот параметр в классе сервлете, необходимо 
использовать метод <span class="b">getServletConfig()</span>, который возвращает объект ServletConfig. У него затем 
вызывается метод <span class="b">getInitParameter()</span>, в который передается имя параметра. Например:</p>
<pre class="brush:java;">
import java.io.IOException;
import java.io.PrintWriter;
