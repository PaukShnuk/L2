...
<html >
<head>
<title>Java EE | Структура Apache Tomcat</title>
<meta charset="utf-8" />
<meta name="description" content="Веб-контейнер Apache Tomcat, его структура, размещеие веб-страниц">
...
<h2>Структура Apache Tomcat</h2><div class="date">Последнее обновление: 02.09.2018</div>
...
<p>После установки в папке Apache Tomcat на жестком диске можно найти ряд файлов и папок.</p>
<img src="pics/tomcat10.png" alt="Каталоги Apache Tomcat"/>
<p>Основные каталоги:</p>
<ul>
<li><p>Папка <span class="b">bin</span>: хранит различные скрипты, в частности, для запуска, перезагрузки и т.д.</p></li>
<li><p>Папка <span class="b">conf</span>: хранит конфигурационные файлы, наиболее важным из которых является файл <span class="b">server.xml</span>, который 
определяет основную часть конфигурации.</p></li>
<li><p>Папка <span class="b">logs</span>: папка по умолчанию для хранения лог-файлов.</p></li>
<li><p>Папка <span class="b">webapps</span>: папка, где хранятся собственно файлы приложений.</p></li>
</ul>
<p>Таким образом, все файлы приложений будут помещаться в папку webapps. По умолчанию после установки в ней можно найти ряд стандартных папок:</p>
<img src="pics/tomcat11.png" alt="Сервер Apache Tomcat"/>
<p>Ключевая папка здесь - это папка <span class="b">ROOT</span>. Так, все содержимое, которое мы видим в браузере по адресу http://localhost:8081, как раз представляет 
файлы из этой папки.</p>
<p>Для примера создадим в каталоге webapps новую папку, которую назовем <span class="b">test</span>.</p>
<p>Далее создадим где-нибудь на жестком диске новый файл <span class="b">hello.html</span> со следующим содержимым:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
    &lt;head&gt;
...
<p>Этот обычный файл с кодом html. Затем скопируем этот файл в выше созданную папку test.</p>
<img src="pics/tomcat12.png" />
<p>После этого мы сможем обратиться к этому файлу в браузере по адресу http://localhost:8081/test/hello.html. То есть в начале указывается 
локальный адрес localhost, затем номер порта (в данном случае 8081), далее название папки - test, и далее название файла - hello.html.</p>
<img src="pics/tomcat13.png" />
<p>Стотит отметить, что при обращении к файлам в папке ROOT, нам не надо указывать название папки. Например, скопируем тот же файл hello.html 
в папку ROOT. Тогда к этому файлу мы можем обратиться по адресу http://localhost:8081/hello.html</p>
<img src="pics/tomcat14.png" />
<p>Несмотря на то, что по умолчанию папка ROOT уже содержит некоторое содержимое справочного характера, тем не менее мы можем все это содержимое спокойно удалить и 
размещать в этой папке только файлы непосредственно нашего приложения. То есть мы вполне можем использовать папку ROOT. Но как правило, приходится работать 
с несколькими приложениями, для которых лучше создавать отдельную папку в каталоге webapps.</p>
