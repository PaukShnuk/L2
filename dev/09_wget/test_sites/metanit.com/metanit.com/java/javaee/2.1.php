...
<html >
<head>
<title>Java EE | Установка Apahe Tomcat</title>
<meta charset="utf-8" />
<meta name="description" content="Веб-сервер Apache Tomcat, его установка для Java EE">
...
<h1>Apache Tomcat</h1><h2>Установка Tomcat</h2><div class="date">Последнее обновление: 02.09.2018</div>
...
<p><span class="b">Tomcat</span> представляет веб-контейнер сервлетов и предназначен для работы с рядом технологий Java EE, в частности, с JSP, сервлетами и рядом других. Нередко 
Tomcat называют веб-сервером.</p>
<p>В данном случае мы будем использовать последнюю на сегодняшний день (29.08.2018) 9-ю версию. Для установки Tomcat перейдем на официальный сайт 
данного контейнера на странцу загрузок - <a href="https://tomcat.apache.org/download-90.cgi" rel="nofollow">https://tomcat.apache.org/download-90.cgi</a>. 
На данной странице мы можем увидеть различные опции для загрузки: в виде архива, который достаточно распаковать, либо в виде инсталлятора.</p>
<img src="pics/tomcat1.png" alt="Веб-контейнер Apache Tomcat" />
<p>В данном случае большой разницы не будет, какой именно пакет использовать. Но в моем случае для упрощения настройки я выберу пункт <span class="b">32-bit/64-bit Windows Service Installer</span>.</p>
<p>После выбора этой опции на компьютер будет загружен инсталлятор, который надо запустить.</p>
<img src="pics/tomcat2.png" alt="Установка Apache Tomcat" />
<p>Вначале надо принять лицензионное соглашение:</p>
<img src="pics/tomcat3.png" alt="Веб-сервер Apache Tomcat" />
<p>Затем надо настроить устанавливаемые компоненты:</p>
<img src="pics/tomcat4.png" alt="Компоненты Apache Tomcat" />
<p>Здесь можно выбрать те компоненты, которые мы хотим установить, в частности, можно выбрать пункт <span class="b">Service Startup</span>, и тогда Tomcat будет запускаться 
автоматически при запуске системы. Можно выбрать все, а можно ограничиться теми компонентами, которые уже выбраны по умолчанию.</p>
<p>Далее будет предложено настроить порты и ряд дополнительных моментов конфигурации Tomcat:</p>
<img src="pics/tomcat5.png" alt="Настройка Apache Tomcat" />
<p>Здесь стоит обратить внимание на пункт <span class="b">HTTP/1.1 Connector Port</span>. Он указывает, по какому порту будет запускаться приложение. 
Укажем в этом поле номер 8081.</p>
<p>Затем надо будет указать версию java, которая будет использоваться:</p>
<img src="pics/tomcat6.png" alt="Конфигурация Apache Tomcat" />
<p>По умолчанию инсталлятор должен определять путь к Java. Но естественно при необходимости его можно изменить. И в конце надо будет указать 
путь к устанавливаемому веб-контейнеру на жестком диске:</p>
<img src="pics/tomcat7.png" />
<p>Также можно оставить путь по умолчанию. И после этого собственно произойдет установка контейнера.</p>
<img src="pics/tomcat8.png" alt="Запуск Apache Tomcat"/>
<p>После установки на финальном экране оставим отмеченнным пункт <span class="b">Run Apache Tomcat</span> и нажмем на кнопку Finish. После этого Tomcat будет запущен, и 
мы сможем к нему обращаться.</p>
<p>Убедимся, что Tomcat работает. Для этого обратимся в строке браузера по адресу <span class="ii">http://localhost:8081</span>. В данном случае 8081 - это тот порт, который был указан 
на этапе установке выше. Если все Tomcat установлен и запущен правильно, то в браузере мы увидим некоторое стандартное содержимое:</p>
<img src="pics/tomcat9.png" alt="Проверка Apache Tomcat"/>
