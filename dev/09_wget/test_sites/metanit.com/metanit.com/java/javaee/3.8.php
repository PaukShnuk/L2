...
<html >
<head>
<title>Java EE | Передача данных из сервлета в jsp</title>
<meta charset="utf-8" />
<meta name="description" content="Передача данных из сервлета в jsp в Java EE, scope, контекста запроса и приложения, ServletContext, HttpServletRequest">
...
<h2>Передача данных из сервлета в jsp</h2><div class="date">Последнее обновление: 23.09.2018</div>
...
<p>Нередко страница jsp обрабатывает запрос вместе сервлетом. В этом случае сервлет определяет логику, а jsp - визуальную часть. 
...
как передать данные из сервлета в jsp?</p>
<p>Есть несколько способов передачи данных из сервлета в jsp, которые заключаются в использовании определенного контекста или scope. 
Есть несколько контекстов для передачи данных:</p>
<ul>
<li><p><span class="b">request</span> (контекст запроса): данные сохраняются в HttpServletRequest</p></li>
<li><p><span class="b">session</span> (контекст сессии): данные сохраняются в HttpSession</p></li>
<li><p><span class="b">application</span> (контекст приложения): данные сохраняются в ServletContext</p></li>
</ul>
<p>Данные из контекста запроса доступны только в пределах текущего запроса. Данные из контекста сессии доступны только в пределах текущего 
сеанса. А данные из контекста приложения доступны постоянно, пока работает приложение.</p>
<p>Но вне зависимости от выбранного способа передача данных осуществляется с помощью метода <span class="b">setAttribute(name, value)</span>, 
где name - строковое название данных, а value - сами данные, которые могут представлять различные данные.</p>
<p>Наиболее распространенный способ передачи данных из сервлета в jsp представляют атрибуты запроса. То есть у объекта HttpServletRequest, который передается в 
сервлет, вызывается метод <span class="b">setAttribute()</span>. Этот метод устанавливает атрибут, который можно получить в jsp.</p>
<p>Например, добавим в проект страницу <span class="b">basic.jsp</span> со следующим кодом:</p>
<pre class="brush:xml;">
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;
...
<p>В basic.jsp можно получить атрибуты name и age и вывести их значение на страницу. Для вывода атрибутов применяется специальный 
синтаксис EL: в фигурные скобки {} заключается выводимое значение</p>
<h3>Контекст запроса</h3>
<p>Определим сервлет HelloServlet, который передает запрос basic.jsp и передает ей данные:</p>
...
<p>Сервлет устанавливает два атрибута - "name" и "age" через объект HttpServletRequest и затем перенаправляет запрос странице basic.jsp.</p>
<p>Если мы обратимся к сервлету HelloServlet, то он передаст запрос и данные странице basic.jsp.</p>
<img src="pics/jsp27.png" alt="Передача данных из сервлета в jsp" />
<h3>Контекст приложения</h3>
<p>Использование контекста приложения представляет применение объекта <span class="b">ServletContext</span>, который можно получить в сервлете с 
помощью метода <span class="b">getServletContext()</span>:</p>
<pre class="brush:java;">
import java.io.IOException;
...
<p>Код страницы basic.jsp при этом не меняется.</p>
<h3>Контекст сессии</h3>
<p>Подобным образом можно передать данные в jsp через сессию:</p>
<pre class="brush:java;">
import java.io.IOException;
...
<div class="nav"><p><a href="3.6.php">Назад</a><a href="./">Содержание</a><a href="3.9.php">Вперед</a></p></div></div>
<div class="socBlock">
