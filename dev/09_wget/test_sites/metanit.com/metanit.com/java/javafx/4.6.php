...
<meta charset="utf-8" />
<meta name="description" content="ScrollPane в JavaFX, создание полос прокрутки и управление ими">
...
<h2>ScrollPane</h2><div class="date">Последнее обновление: 20.06.2018</div>
...
<p>Элементы управления не всегда могут помещаться в отведенное им пространство. В этом случае мы можем использовать полосы прокрутки для 
удобного обращения к подобным элементам управления. Для создания возможности прокрутки в JavaFX применяется класс <span class="b">javafx.scene.control.ScrollPane</span>.</p>
<p>ScrollPane определяет два конструктора:</p>
<ul>
<li><p><code>ScrollPane()</code>: создает пустой объект ScrollPane</p></li>
<li><p><code>ScrollPane(Node content)</code>: создает объект ScrollPane, в который помещается узел content</p></li>
</ul>
<p>Для управления прокручиваемым содержимым ScrollPane определяет ряд методов. Некоторые из них:</p>
<ul>
<li><p><code>void setContent(Node content)</code>: устанавливает содержимое в ScrollPane</p></li>
<li><p><code>void setPrefViewportHeight(double height)</code>: устанавливает предпочтительную высоту видимого пространства в ScrollPane</p></li>
<li><p><code>void setPrefViewportWidth(double width)</code>: устанавливает предпочтительную ширину видимого пространства в ScrollPane</p></li>
</ul>
<p></p>
...
<img src="./pics/4.12.png" alt="ScrollPane in JavaFX" />
<p>С помощью методов <span class="b">setVvalue(double value)</span> и <span class="b">setHvalue(double value)</span> можно установить 
положение ползунка прокрутки по вертикали и по горизонтали соответственно. В качестве значения в методы передается число от 0 до 1, где 0 - это начало, а 1 - конец. Например, определим 
положение ползунка прокрутки по середине:</p>
<pre class="brush:java;">
ScrollPane scrollPane = new ScrollPane(textLbl);
scrollPane.setPrefViewportHeight(150);
scrollPane.setPrefViewportWidth(200);
