...
<html >
<head>
<title>JavaFX | Кнопки и метки</title>
<meta charset="utf-8" />
<meta name="description" content="Кнопки и метки в JavaFX, классы Button и Label и их использование">
...
<h1>Элементы управления</h1><h2>Кнопки и метки</h2><div class="date">Последнее обновление: 16.06.2018</div>
...
<p>Наиболее часто используемые элементы управления в JavaFX - это, без сомнения, кнопки и метки. Кнопки 
представлены классом <span class="b">javafx.scene.control.Button</span> и позволяют по нажатию выполнять некоторые действия. А метки представлены 
классы <span class="b">javafx.scene.control.Label</span>, и их задача - вывод текстовой информации. Класс Button унаследован от классов ButtonBase, Labeled, Region, Control, Parent и Node, 
а класс Label - от Labeled, Region, Control, Parent и Node. JavaFX позволяет создавать самые разнообразные кнопки и метки: с текстом, графикой, и тем и другим, с различными стилями.</p>
<p>Класс Button имеет три конструктора:</p>
<ul>
<li><p><code>Button()</code>: создает кнопку без надписи</p></li>
<li><p><code>Button​(String text)</code>: создает кнопку с определенной надписью</p></li>
<li><p><code>Button​(String text, Node graphic)</code>: создает кнопку с определенной надписью и иконкой</p></li>
</ul>
<p>Класс Label имеет три похожих конструктора:</p>
<ul>
<li><p><code>Label()</code>: создает метку без надписи</p></li>
<li><p><code>Label​(String text)</code>: создает метку с определенной надписью</p></li>
<li><p><code>Label(String text, Node graphic)</code>: создает метку с определенной надписью и иконкой</p></li>
</ul>
<p>Ключевой возможностью кнопки является способность реагировать на нажатия пользователей и по нажтию выполнять некоторое действие. 
Для обработки нажатий в базовом классе ButtonBase определен метод <span class="b">setOnAction()</span>, который устанавливает обработчик нажатия:</p>
<pre class="brush:java;">
final void setOnAction(EventHandler&lt;ActionEvent&gt; handler)
</pre>
<p>В качестве параметра в этот метод передается реализация интерфейса EventHandler:</p>
<pre class="brush:java;">
public interface EventHandler&lt;T extends Event&gt;{
    void handle​(T event);
}
</pre>
<p>Используем кнопку в программе:</p>
<pre class="brush:java;">
...
<p>В классе определена переменная-счетчик counter. И по нажатию на кнопку будет увеличиваться ее значение.</p>
<img src="./pics/4.1.png" alt="Кнопки Button в JavaFX" />
