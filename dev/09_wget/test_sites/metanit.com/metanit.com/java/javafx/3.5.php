...
<meta charset="utf-8" />
<meta name="description" content="Панель компоновки BorderPane в JavaFX, позиционирование элементов вдоль границ контейнера">
...
<h2>BorderPane</h2><div class="date">Последнее обновление: 07.06.2018</div>
...
<p>Контейнер <span class="b">javafx.scene.layout.BorderPane</span> позволяет прижать вложенные элементы управления к одной из сторон контейнера: по верхнему, нижнему, левому или правому краю или расположить по центру.</p>
<img src="./pics/3.12.png" alt="BorderPane в JavaFX" />
<p>Класс BorderPane имеет следующие конструкторы:</p>
<ul>
<li><p><span class="b">BorderPane()</span>: создает пустой контейнер BorderPane</p></li>
<li><p><span class="b">BorderPane​(Node center)</span>: создает контейнер BorderPane, в котором по центру располагается элемент center</p></li>
<li><p><span class="b">BorderPane​(Node center, Node top, Node right, Node bottom, Node left)</span>: создает контейнер BorderPane, 
в котором по центру располагается элемент center, по верхнему краю - элемент top, по правому краю - элемент right, по нижнему краю - элемент bottom  и по нижнему краю - узел left</p></li>
</ul>
<p>Расположение элемента по центру:</p>
...
<img src="./pics/3.13.png" alt="Центрирование в BorderPane in JavaFX" />
<p>Некоторые из основных методов BorderPane:</p>
<ul>
<li><p><code>Node getBottom​()</code>: получает узел, который расположен по нижнему краю BorderPane</p></li>
<li><p><code>void setBottom​(Node child)</code>: располагает узел по нижнему краю</p></li>
<li><p><code>Node getCenter​()</code>: получает узел, который расположен по центру</p></li>
<li><p><code>void setCenter(Node child)</code>: располагает узел по центру</p></li>
<li><p><code>Node getLeft()</code>: получает узел, который расположен по левому краю</p></li>
<li><p><code>void setLeft​(Node child)</code>: располагает узел по левому краю</p></li>
<li><p><code>Node getRight()</code>: получает узел, который расположен по правому краю</p></li>
<li><p><code>void setRight(Node child)</code>: располагает узел по правому краю</p></li>
<li><p><code>Node getTop()</code>: получает узел, который расположен по верхнему краю</p></li>
<li><p><code>void setTop​(Node child)</code>: располагает узел по верхнему краю</p></li>
<li><p><code>static Pos getAlignment(Node child)</code>: получает параметры выравнивания элемента child в виде объекта Pos</p></li>
<li><p><code>static void setAlignment​(Node child, Pos value)</code>: задает выравнивание для элемента child</p></li>
<li><p><code>static void setMargin​(Node child, Insets value)</code>: устанавливает для элемента child внешние отступы от границ контейнера</p></li>
<li><p><code>static Insets getMargin​(Node child)</code>: получает внешние отступы элемента от границ контейнера</p></li>
</ul>
<p>При размещении элементов, следует учитывать, что для каждого элемента задается по умолчанию определенное выравнивание:</p>
<ul>
<li><p>top: Pos.TOP_LEFT</p></li>
<li><p>bottom: Pos.BOTTOM_LEFT</p></li>
<li><p>left: Pos.TOP_LEFT</p></li>
<li><p>right: Pos.TOP_RIGHT</p></li>
<li><p>center: Pos.CENTER</p></li>
</ul>
<p>С помощью метода setAlignment мы можем переопределить выравнивание по умолчанию:</p>
<pre class="brush:java;">
import javafx.application.Application;
import javafx.stage.Stage;
import javafx.scene.Scene;
...
<p>В данном случае выравнивание все элементов идет по центру исходя из того, к какой стороне контейнера они примыкают:</p>
<img src="./pics/3.14.png" alt="Alignment in BorderPane in JavaFX" />
<p>Благодаря методам BorderPane нам необязательно определять элементы для центра, или для каждой из сторон. Мы можем установить элементы только для тех сторон, 
которые действительно необходимы:</p>
<pre class="brush:java;">
import javafx.application.Application;
import javafx.stage.Stage;
import javafx.scene.Scene;
...
<img src="./pics/3.15.png" alt="Позиционирование в BorderPane в JavaFX" />
