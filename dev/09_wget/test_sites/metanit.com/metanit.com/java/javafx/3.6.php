...
<html >
<head>
<title>JavaFX | StackPane</title>
<meta charset="utf-8" />
<meta name="description" content="Панель компоновки StackPane в JavaFX, расположение элементов в виде стека">
...
<h2>StackPane</h2><div class="date">Последнее обновление: 07.06.2018</div>
...
<p>Панель компоновки <span class="b">javafx.scene.layout.StackPane</span> позволяет располагать элементы поверх других элементов, то есть в виде стека, где каждый новый элемент наслаивается поверх другого. 
По умолчанию StackPane пытается масштабировать каждый вложенный элемент таким образом, чтобы он заполнил все пространство контейнера. Если вложенный элемент 
не может быть отмасштабирован, то он располагается по центру контейнера.</p>
<p>StackPane имеет два конструктора:</p>
<ul>
<li><p><span class="b">StackPane()</span>: создает пустой объект StackPane</p></li>
<li><p><span class="b">StackPane​(Node... children)</span>: создает объект StackPane, заполненный узлами children</p></li>
</ul>
<p>Создадим простейший StackPane:</p>
...
<img src="./pics/3.16.png" alt="StackPane в JavaFX" />
<p>По умолчанию элементы центрируются в контейнере. Поскольку кнопка добавляется последней, то она накладывается поверх остальных. Однако 
вряд ли такой интерфейс можно считать оптимальным. Поскольку нет смысла располагать кнопки и метки поверх друг друга. Но с помощью статического 
метода <span class="b">setAlignment()</span> можно опредленным образом позиционировать вложенный элемент:</p>
<pre class="brush:java;">
import javafx.application.Application;
import javafx.stage.Stage;
import javafx.scene.Scene;
...
<p>С помощью значения перечисления Pos StackPane может расположить элемент по каком-то определенном месте контейнера.</p>
<img src="./pics/3.17.png" alt="Alignment в StackPane в JavaFX" />
<p>Среди методов StackPane следует отметить следующие:</p>
<ul>
<li><p><code>Pos getAlignment()</code>: возвращает параметры выравнивания элемента<p></li>
<li><p><code>static Pos getAlignment​(Node child)</code>: возвращает параметры выравнивания для элемента child внутри StackPane<p></li>
<li><p><code>static Insets getMargin​(Node child)</code>: возвращает для элемента child отступы от границ контейнера StackPane<p></li>
<li><p><code>void setAlignment​(Pos value)</code>: устанавливает параметры выравнивания для StackPane<p></li>
<li><p><code>static void setAlignment​(Node child, Pos value)</code>: устанавливает параметры выравнивания для элемента child внутри StackPane<p></li>
<li><p><code>sstatic void setMargin​(Node child, Insets value)</code>: устанавливает для элемента child отступы от границ контейнера StackPane<p></li>
</ul>
<p>Например, устаовим отступы элментов от границ StackPane:</p>
<pre class="brush:java;">
import javafx.application.Application;
import javafx.stage.Stage;
import javafx.scene.Scene;
...
<img src="./pics/3.18.png" alt="Insects в StackPane в JavaFX" />
<p>Для установки отступов применяется объет Insects, в конструктор которого передается значение отступов от верхней, правой, нижней и левой границы контейнера соответственно.</p>
