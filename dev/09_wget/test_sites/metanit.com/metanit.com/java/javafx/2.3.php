...
<html >
<head>
<title>JavaFX | Скрипты в FXML</title>
<meta charset="utf-8" />
<meta name="description" content="Скрипты JavaScript и определение обработчиков событий в FXML в JavaFX">
...
<h2>Скрипты в FXML</h2><div class="date">Последнее обновление: 13.06.2018</div>
...
<p>Для создания сложной, которая используется для обработки событий элементов управления, лучше определять классы контроллеров, как было показано в прошлой теме. 
Однако в каких-то простых случаях можно несколько упростить организацию приложения и определить обработчики прямо в FXML, не создавая отдельного класса 
контроллера.</p>
<p>Например, определим в FXML следующий интерфейс:</p>
<pre class="brush:xml;">
...
<p>Фактически такой способ работы с FXML напоминает работу с веб-страницей, которая содержит код javascript. И чтобы в FXML 
это тоже было возможно, вначало файла помещается следующая декларация:</p>
<pre class="brush:xml;">&lt;?language JavaScript?&gt;</pre>
<p>Но чтобы элемент language стал доступен, в корневом узле подключается пространство имен <code>xmlns="http://javafx.com/fxml/2.2"</code>. Причем данное 
пространство имен указывается для атрибута xmlns, поэтому к его возможностям можно обращаться без префикса.</p>
<p>В конце файла определяется элемент <code>fx:script</code>, который содержит логику обработки. Здесь определяются переменная x, которая хранит счетчик нажатий, 
и функция pressButton, которая будет вызываться при нажатии на кнопку.</p>
<p>В скрипте мы можем обращаться к элементам FXML по их id и соответственно вызывать их методы. Как в случае с кнопкой устанавливается ее текст.</p>
<p>Для прикрепления функции pressButton в качестве обработчика события к кнопке, вызов этой функции присваивается атрибуту onAction:</p>
<pre class="brush:xml;">
onAction="pressButton()" 
</pre>
<p>Стоит обратить внимание, что это отличается от установки ссылки на метод контроллера. То есть в данном случае перед именем функции не ставится 
знак решетки и после имени идут скобки, которые представляют вызов функции.</p>
<p>При запуске приложения мы сможем нажимать на кнопку и тем самым увеличивать счетчик нажатий:</p>
<img src="./pics/2.13.png" alt="JavaScript in FXML" />
