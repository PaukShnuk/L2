...
<meta charset="utf-8" />
<meta name="description" content="Панель компоновки VBox в JavaFX, методы класса, расположение элементов">
...
<h2>VBox</h2><div class="date">Последнее обновление: 07.06.2018</div>
...
<p>Контейнер <span class="b"></span> организует вложенные элементы в виде вертикального ряда.</p>
<p>Для создания объекта VBox применяется один из следующих конструкторов:</p>
<ul>
<li><p><code>VBox()</code>: определяет пустой объект VBox</p></li>
<li><p><code>VBox​(double spacing)</code>: определяет пустой объект VBox, элементы которого будут находиться друг от друга на расстоянии spacing. По умолчанию 
это значение равно 0</p></li>
<li><p><code>VBox​(double spacing, Node... children)</code>: определяет объект VBox, заполненный элементами nodes, которые находятся друг от друга на расстоянии spacing</p></li>
<li><p><code>VBox​(Node... children)</code>: определяет объект VBox, заполненный элементами nodes</p></li>
</ul>
<p>Определим простой VBox:</p>
...
<p>Если места для элементов не хватает, но они не переносятся на другой ряд.</p>
<img src="./pics/3.8.png" alt="VBox в JavaFX" />
<p>Некоторые из методов VBox:</p>
<ul>
...
<li><p><span class="b">VBox.getVgrow​(Node child)</span>: статический метод возвращает для элемента child ограничение vgrow в виде объекта Priority</p></li>
<li><p><span class="b">VBox.setVgrow​(Node child, Priority value)</span>: статический метод устанавливает для элемента child ограничение vgrow</p></li>
<li><p><span class="b">setFillWidth​(boolean value)</span>: при передачи значения true расширяет VBox по всей ширине контейнера</p></li>
<li><p><span class="b">isFillWidth()</span>: возвращает логическое значение true или false, которое указывает, развернут ли VBox по всей ширине контейнера</p></li>
<li><p><span class="b">VBox.setMargin​(Node child, Insets value)</span>: статический метод устанавливает для элемента child внешние отступы от границ контейнера</p></li>
<li><p><span class="b">VBox.getMargin​(Node child)</span>: статический метод получает внешние отступы элемента от границ контейнера</p></li>
</ul>
<p>Особо выделим метод <span class="b">VBox.setVgrow​()</span>, который позволяет установить для элемента ограничение vgrow. 
...
<p>Например, растянем кнопку по всей высоте контейнера:</p>
<pre class="brush:java;">
import javafx.application.Application;
import javafx.stage.Stage;
import javafx.scene.Scene;
...
<p>Для кнопки с помощью setMaxHeight задается максимальная высота, которая принимает максимально допустимое значение для типа double. А в качестве 
ограничения устанавливается значение <span class="b">Priority.ALWAYS</span>, которое поволяет автоматически масштабировать кнопку, чтобы заполнить 
пространство контейнера.</p>
<img src="./pics/3.9.png" alt="vgrow в VBox в JavaFX" />
<p>Подобным образом мы можем растягивать несколько элементов или комбинировать их с элементами, которое не должны растягиваться:</p>
<pre class="brush:java;">
import javafx.application.Application;
import javafx.stage.Stage;
import javafx.scene.Scene;
...
<p>Две кнопки будут заполнять равномерно пространство VBox, тогда как метка label занимает только то место, которое ей необходимо.</p>
<img src="./pics/3.10.png" />
<p>Однако при использовании VBox, как видно из скриншота, мы можем столкнуться с проблемой неравномерной ширины элементов. Элементы автоматически нерятягиваются по всей ширине контейнера. 
Чтобы выйти из этой проблемы, мы можем задать максимальную ширину для элементов:</p>
<pre class="brush:java;">
okBtn.setMaxWidth(100);
cancelBtn.setMaxWidth(100);
...
<img src="./pics/3.11.png" alt="Равномерное растяжение в VBox в JavaFX" />
