...
<meta charset="utf-8" />
<meta name="description" content="Slider в JavaFX, вертикальный и горизонтальный слайдеры, управление делениями на шкале слайдера и перемещением бегунка">
...
<h2>Slider</h2><div class="date">Последнее обновление: 20.06.2018</div>
...
<p>Класс <span class="b">javafx.scene.control.Slider</span> определяет элемент управления в виде числовой шкалы с делениями, на которой можно выбрать определеное значение, проще говоря 
слайдер.</p>
<p>Для создания слайдера можно использовать один из двух конструкторов класса:</p>
<ul>
<li><p><code>Slider()</code>: создает слайдер с настройками по умолчанию</p></li>
<li><p><code>Slider​(double min, double max, double value)</code>: создает слайдер, для которого установлены минимальное, максимальное и текущее 
значения</p></li>
</ul>
<p>Некоторые наиболее используемые методы слайдера:</p>
<ul>
<li><p><code>void setOrientation(Orientation how)</code>: устанавливает ориентацию слайдера - горизонтальную или вертикальную</p></li>
<li><p><code>void setValue​(double value)</code>: устанавливает текущее значение слайдера</p></li>
<li><p><code>double getValue()</code>: возвращает текущее значение слайдера</p></li>
<li><p><code>void setShowTickMarks​(boolean value)</code>: значение true (по умолчанию) делает деления на шкале слайдера видимыми, соответственно 
значение false делает деления невидимыми</p></li>
<li><p><code>void setMinorTickCount(int val)</code>: устанавливает количество делений между двумя числовыми отметками на шкале</p></li>
<li><p><code>void setMajorTickUnit(int val)</code>: устанавливает расстояниие между двумя числовыми отметками на шкале</p></li>
<li><p><code>void setShowTickLabels(boolean on)</code>: устанавливает видимость числовых меток на шкале</p></li>
<li><p><code>void setBlockIncrement(double val)</code>: устанавливает, насколько значений будет перемещаться бегунок слайдера 
с помощью клавиатурных клавиш Вперед и Назад при горизонтальной ориентации и клавиш Вверх и Вниз при вертикальной ориентации</p></li>
<li><p><code>void setSnapToTicks(boolean on)</code>: при передачи значения true позволяет переходить ровно по делениям (то есть вместо значения типа 4.5677687 будет 5)</p></li>
</ul>
<p>Создадим слайдер:</p>
...
<img src="./pics/4.13.png" alt="Slider in JavaFX" />
<p>В данном случае чтобы вывести текущее значение слайдера на элемент Label, необходимо нажать на кнопку. Однако мы можем сразу динамически 
изменять текст на метке при изменении значения слайдера. Для этого мы можем прослушивать изменения значения с помощью слушателя ChangeListener:</p>
<pre class="brush:java;">
import javafx.application.Application;
import javafx.stage.Stage;
import javafx.scene.Scene;
...
<img src="./pics/4.14.png" alt="Бегунок слайдер в JavaFX" />
<p>В примерах выше использовались горизонтальные слайдеры, но также можно задать вертикальную ориентацию слайдера:</p>
<pre class="brush:java;">
import javafx.geometry.Orientation;
//...............
Slider slider = new Slider(0.0, 20.0, 10.0);
