...
<html >
<head>
<title>JavaFX | Класс Scene</title>
<meta charset="utf-8" />
<meta name="description" content="Класс Scene в JavaFX, создание графического интерфейса, scene graph, добавление элементов управления в окно приложения">
...
<h2>Класс Scene</h2><div class="date">Последнее обновление: 26.09.2021</div>
...
<p>Класс <span class="b">javafx.scene.Scene</span> представляет контейнер для всех графических элементов внутри объекта Stage в виде графа, который называется 
<span class="b">Scene Graph</span>. Все узлы этого графа, то есть по сути все вложенные элементы должны представлять класс <span class="b">javafx.scene.Node</span>. 
Но корневой узел этого графа должен представлять объект класса, который унаследован от <span class="b">javafx.scene.Parent</span>. По сути Parent - 
это контейнер, который может содержать другие элементы.</p>
<img src="./pics/2.3.png" alt="Элементы в Scene в JavaFX" />
<p>Для установки корневого узла в Scene применяется один из конструкторов объекта Scene. Основные из них:</p>
<ul>
<li><p><code>Scene(Parent root)</code>: создает Scene с корневым узлом root<p></li>
<li><p><code>Scene(Parent root, double width, double height)</code>: создает Scene с корневым узлом root, с шириной width и высотой height<p></li>
<li><p><code>Scene(Parent root, Paint fill)</code>: создает Scene с корневым узлом root и устанавливает фоновый цвет<p></li>
<li><p><code>Scene(Parent root, double width, double height, Paint fill)</code>: создает Scene с корневым узлом root, с шириной width и высотой height и устанавливает фоновый цвет<p></li>
</ul>
<p>При этом все конструкторы принимают в качестве первого параметра корневой узел. То есть при создании объекта Scene нам та или иначе придется установить и корневой узел.</p>
<p>Для установки сцены у объекта Stage вызывается метод <code>setScene()</code>, в который передается объект Scene. Например, создадим простейшую сцену:</p>
...
<p>В данном случае в качестве корневого узла используется объект класса <span class="b">Group</span>, который унаследован от класса Parent. 
Однако поскольку Group не содержит никаких элементов, то по сути мы увидим пустое окно.</p>
<p>Немного изменим код программы:</p>
<pre class="brush:java;">
import javafx.application.Application;
import javafx.stage.Stage;
import javafx.scene.Scene;
...
<p>Здесь в качестве корневого узла выступает класс FlowPane, который располагает вложенные элементы подряд и который, как и Group, унаследован от класса Parent. 
FlowPane содержит объект Label (текстовая метка) и объект Group. А объект Group, в свою очередь, содержит объект Button (кнопка).</p>
<p>Визуально это будет выглядеть следующим образом:</p>
<img src="./pics/2.11.png" alt="Элементы в Scene Graph в JavaFX" /> 
<p>А сам Scene Graph в данном случае будет представлять следующую структуру:</p>
<img src="./pics/2.12.png" /> 
<h3>Настройка фонового цвета окна</h3>
<p>С помощью Scene мы можем настроить фоновый цвет и размеры окна. Для этого мы можем использовать один из конструкторов:</p>
<pre class="brush:java;">
import javafx.application.Application;
import javafx.stage.Stage;
import javafx.scene.Scene;
...
<p>В данном случае вся сцена занимает 400 единиц в ширину и 150 единиц в длину и будет окрашена в синий цвет. Цвет представляет объект 
<span class="b">javafx.scene.paint.Color</span>.</p>
<img src="./pics/2.2.png" alt="Создание графического интерфейса и класс Scene в JavaFX" />
<p>Также для установки цвета можно применять метод <span class="b">setFill(Paint value)</span>.</p>
...
<div class="nav"><p><a href="./1.4.php">Назад</a><a href="./">Содержание</a><a href="./1.6.php">Вперед</a></p></div></div>
<div class="socBlock">
