...
<html >
<head>
<title>JavaFX | Панели компоновки</title>
<meta charset="utf-8" />
<meta name="description" content="Панели компоновки в JavaFX, класс Pane, типы контейнеров, добавление элементов в контейнер, ObservableList">
...
<h1>Панели компоновки</h1><h2>Типы панелей компоновки</h2><div class="date">Последнее обновление: 07.06.2018</div>
...
<p>Обычно все элементы управления, которые используются в приложении, помещаются в специальные контейнеры - панели компоновки (layout panes). 
Все панели встроенные компоновки наследуются от класса <span class="b">javafx.scene.layout.Pane</span>, который, в свою очередь, наследуется от Region.</p>
<img src="./pics/2.5.png" alt="Панели компоновки LayoutPanes в JavaFX" />
<p>Панели компоновки позволяют упорядочить вложенные элементы определенным образом и могут определять их размеры. В JavaFX есть несколько встроенных 
панелей компоновки:</p>
<ul>
<li><p><span class="b">AnchorPane</span>: позволяет прижимать вложенные элементы управления к одной из сторон контейнера или размещать их по центру</p></li>
<li><p><span class="b">BorderPane</span>: поволяет упорядочить вложенные элементы управления относительно одной из сторон контейнера</p></li>
<li><p><span class="b">FlowPane</span>: размещает  все вложенные элементы либо по горизонтали (если элементы не помещаются, то они переносятся на новую строку), либо 
по вертикали (если элементы не помещаются, то они переносятся в новый столбик)</p></li>
<li><p><span class="b">GridPane</span>: образует сетку из строк и столбцов и размещает вложенные элементы ячейках полученной таблицы</p></li>
<li><p><span class="b">HBox</span>: размещает все элементы в виде одной строки, то есть горизонтальный стек</p></li>
<li><p><span class="b">VBox</span>: размещает все элементы в виде одного столбца, то есть вертикальный стек</p></li>
<li><p><span class="b">StackPane</span>: располагает одни элементы поверх других</p></li>
<li><p><span class="b">TilePane</span>: размещает все вложенные элементы в виде тайлов</p></li>
</ul>
<p>Поскольку все эти классы наследуются от класса Pane, то они все они имеют один важный метод - <span class="b">getChildren()</span>, который возвращает 
коллекцию вложенных в контейнер элементов в виде объекта интерфейса <span class="b">ObservableList&lt;Node&gt;</span>. Интерфейс ObservableList, в свою очередь, 
расширяет интерфейсы Collection&lt;E&gt;, Iterable&lt;E&gt;, List&lt;E&gt;, Observable.</p>
<p>Каждый вложенный элемент представляет объект Node. Используя методы ObservableList, мы можем добавлять узлы Node в эту коллекцию, а по сути в контейнер компоновки.</p>
<p>Например, возьмем простейшую панель компоновки - FlowPane и добавив в нее два элемента - Label и Button:</p>
...
<img src="./pics/3.30.png" alt="Добавление элементов в Pane в JavaFX" />
<p>Подобным образом можно добавлять данные во все другие панели компоновки.</p>
