...
<html >
<head>
<title>F# | Функция как параметр и результат другой функции</title>
<meta charset="utf-8" />
<meta name="description" content="Функция как параметр и результат другой функции в языке программирования F#">
...
<h2>Функция как параметр и результат другой функции</h2><div class="date">Последнее обновление: 23.07.2021</div>
...
<p>В языке F# одна функция может быть параметром и результатом другой функции.</p>
<h3>Функция как параметр функции</h3>
<p>Рассмотрим функцию, которая принимает другую функцию в качестве параметра:</p>
<pre class="brush:f#;">
let sum x y = x + y
...
<p>Здесь функция <code>operation</code> принимает три параметра. Причем третий параметр представляет функцию, которая принимает два параметра.</p>
<p>Далее мы можем вызвать функцию operation, передав ей в качестве параметра определенную функцию:</p>
<pre class="brush:f#;">let result1 = operation 10 6 sum </pre>
<h3>Функция как результат функции</h3>
<p>Рассмотрим случай, когда функция возвращает другую функцию:</p>
<pre class="brush:f#;">
let sum x y = x + y
let subtract x y = x - y
...
<p>Здесь функция <code>operation</code> принимает один параметр и в зависимости от его значения возвращает одну из функций: sum, subtract, multiply или empty. 
При чем все возвращаемые функции должны иметь один и тот же тип.</p>
<p>Затем мы можем вызвать функцию operation и получить ее результат:</p>
<pre class="brush:f#;">let selection = operation 1</pre>
<p>Поскольку здесь функции operation передается число 1, то она возвратит функцию sum. Соответственно значение <code>selection</code> будет представлять функцию 
sum.</p>
<p>Далее через <code>selection</code> мы фактически может обращаться к этой функции sum:</p>
<pre class="brush:f#;">let result1 = selection 10 7       // 17</pre>
<p>Но мы можем укоротить получение и вызов функции:</p>
<pre class="brush:f#;">
let result2 = operation 2 10 7      // 3
printfn $"result2 = {result2}"    // result2 = 3
</pre>
<p>Здесь <code>operation 2</code> опять же представляет получение из operation функции (в данном случае функции subtract). И дальше передаются значения <code>10 7</code> для параметров 
функции-результата.</p>
