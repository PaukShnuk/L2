...
<html >
<head>
<title>F# | Результат функции</title>
<meta charset="utf-8" />
<meta name="description" content="Возврашение результата из функций в языке программирования F#, типизация результата функции">
...
<h2>Результат функции</h2><div class="date">Последнее обновление: 21.07.2021</div>
...
<p>В языке F# функция возвращает некоторый результат. Причем любая функция возвращает результат.</p>
<p>Какое значение возвращает функция? Компилятор использует последнее выражение в теле функции для определения возвращаемого значения и его типа.</p>
<p>Например, возьмем следующую функцию</p>
<pre class="brush:f#;">let sum x y = x + y</pre>
<p>Здесь тело функции состоит из одного выражения: <code>x + y</code>. Поэтому функция <code>sum</code> будет возвращать сумму значений параметров x и y. По умолчанию компилятор расценивает 
эти параметры как параметры типа <code>int</code> (так как к ним применяется арифметическая операция), поэтому возвращаемое функцией значение также будет иметь тип <code>int</code>.</p>
<p>При вызове функции мы можем получить ее возвращаемое значение:</p>
<pre class="brush:f#;">
let sum x y = x + y
...
<p>Здесь результат вызова функции <code>sum 10 30</code> передается значению <code>result</code>. То есть здесь result будет хранить число 40.</p>
<p>Теперь усложним функцию:</p>
<pre class="brush:f#;">
let sum x y = 
    printfn $"x = {x}"
    printfn $"y = {y}"
...
<p>Теперь функция sum состоит из четырех выражений. На первых двух строках выводим значения параметров. На третей строке вычисляем их сумму и передаем значению result. 
А вот последней строкой является</p>
<pre class="brush:f#;">result</pre>
<p>Поэтому это значение и будет возвращаться функцией в качестве возвращаемого результата. То есть фактически функция опять же возвращает сумму параметров. Таким образом, значение на последней будет результатом функции.</p>
<p>Если последнее выражение функции представляет вызов другой функции, то результатом будет результат этого вызова функции:</p>
<pre class="brush:f#;">
...
<p>В данном случае функция <code>printMessage</code> возвращает результат, который возвращается вызовом <code>printfn $"Message: {message}"</code>. Здесь значение типа 
<code>unit</code></p>
<h3>Типизация типа результата</h3>
<p>Также мы можем явно указать тип результата функции. Для этого тип результата функции указывается после ее параметров через двоеточие:</p>
<pre class="brush:f#;">let имя_функции параметр1 параметр2 ... параметрN : тип_результата = действия_фукции</pre>
<p>Например:</p>
<pre class="brush:f#;">
let sum x y : int64 = 
    printfn $"x = {x}"
...
<p>Здесь функция sum возвращает значение типа <code>int64</code>. Поскольку компилятор видит, что при операции сложения параметров должно сформироваться значение типа <code>int64</code>, 
то параметры также будут рассматриваться как значения типа <code>int64</code>. Соответственно при вызове функции ей надо передать значения этого типа.</p>
