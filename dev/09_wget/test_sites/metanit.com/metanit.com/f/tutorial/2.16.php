...
<html >
<head>
<title>F# | Оператор type и определение типа</title>
<meta charset="utf-8" />
<meta name="description" content="Определение типа с помощью оператора type в языке программирования F#, пример использования">
...
<h2>Оператор type и определение типа</h2><div class="date">Последнее обновление: 27.07.2021</div>
...
<p>Язык F# позволяет создавать новые определения типов и их псевдонимы. Для применяется оператор <span class="b">type</span>. 
Он имеет следующий формальный синтаксис:</p>
<pre class="brush:f#;">type название_типа = определение_типа</pre>
<p>После ключевого слова <span class="b">type</span> идет название типа. А после знака равно собственно определение нового типа.</p>
<p>Вначале рассмотрим ситуацию, где мы могли бы это применить:</p>
<pre class="brush:f#;">
...
<p>В данном случае функция <code>action</code> в качестве третьего параметра принимает функцию, которая выполняет некоторые действия над первым и вторым параметром. 
И данном случае мы явно устанавливаем тип этой функции: <code>float -&gt;float-&gt; float</code> - то есть функция принимает два параметра типа float и возращает значение float.</p>
<p>Определение типа функции выглядит несколько громоздко. Но ситуация усложнится, если мы хотим задейтвовать этот тип многократно. Например, определим еще значение этого типа:</p>
<pre class="brush:f#;">
...
<p>Теперь изменим этот код, применим определение типа с помощью оператора <span class="b">type</span>:</p>
<pre class="brush:f#;">
...
<p>Здесь определяется тип <code>operation</code>, который по факту представляет тип <code>float -&gt;float-&gt; float</code>. То есть фактически мы определяем псевдоним (хотя <code>type</code> также может применяться для 
определения новых типов, что будет рассмотрено далее). Далее через идентификатор <code>operation</code> мы можем ссылаться на тип <code>float -&gt;float-&gt; float</code>.</p>
