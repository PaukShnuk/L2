...
<html >
<head>
<title>F# | Конструкция if..then..else</title>
<meta charset="utf-8" />
<meta name="description" content="Конструкция if..then..else на языке программирования F#, проверка условия, альтернативные условия elif, многострочные блоки then, возвравщение результата из if..else">
...
<h2>Конструкция if..then..else</h2><div class="date">Последнее обновление: 22.07.2021</div>
...
<p>Конструкция <span class="b">if</span> проверяет некоторое условие и в зависимости от того, какой результат возвращает это условие - true или false, 
выполняет те или иный действия. Формальное определение конструкции:</p>
<pre class="brush:f#;">
if условие 
then действия_если_условие_верно
[ else действия_если_условие_не_верно ]
</pre>
<p>Само условие указывается после оператора <span class="b">if</span>. Условие представляет любое выражение, которое возвращает значение типа <code>bool</code>, то есть либо значение 
<code>true</code>, либо значение <code>false</code>.</p>
<p>После условия идет ключевое слово <span class="b">then</span>, после которого указываются действия, выполняемые, если условие истинно (то есть возвращает <code>true</code>).</p>
<p>Например:</p>
<pre class="brush:f#;">
let x = 20
if x &gt; 10 then printfn "x больше 10"
</pre>
<p>Здесь конструкция <code>if</code> проверяет условие <code>x &gt; 10</code>, то есть больше ли значение <code>x</code>, чем число 10. И если это условие возвращает <code>true</code>, 
то выполняются действия после оператора <span class="b">then</code>: <code>printfn "x больше 10"</code>.</p>
<p>Поскольку в данном случае это условие истинно, то есть значение x действительно больше  10, то на консоль будет выведена строка <code>"x больше 10"</code></p>
<h3>else</h3>
<p>Если мы хотим также определить альтернативные действия, которые выполняются, если условие НЕверно, то в этом случае можно 
использовать необязательный оператор <span class="b">else</code>. После него указываются действия, выполняемые, если условие возвращает <code>false</code>. 
Например, в примере выше значение x может быть больше 10, а может быть меньш или равно. Отразим это в коде:</p>
<pre class="brush:f#;">
let x = 8
...
<p>В данном случае условие <code>&gt; 10</code> неверное, поскольку значение x равно 8. Поэтому блок <code>then</code> игнорируется и будет выполняться блок <code>else</code>, 
который выведет на консоль строку <code>"x меньше или равен 10"</code>.</p>
<h3>elif</h3>
<p>Если надо проверить альтернативные дополнительные условия, то можно использовать оператор <span class="b">elif</span>, после которого указываются дополнительные условия:</p>
<pre class="brush:f#;">
elif дополнительное_условие then действия_если_дополнительное_условие_истинно</pre>
<p>Так, в примере выше значение x имеет три варианта: больше 10, меньше 10 и равно 10. Определим соответствующую конструкцию if:</p>
<pre class="brush:f#;">
let x = 8
...
<p>В данном случае условие из блока <code>elif</code> - <code>x &lt; 10</code> истинно, поэтому будет выполняться код <code>printfn "x меньше 10"</code></p>
<p>Выражений <code>elif</code> может быть произвольное количество. Например:</p>
<pre class="brush:f#;">
let x = 8
...
<h3>Многострочные then и else</h3>
<p>Если блоки <code>then</code> и <code>else</code> должны выполнять несколько выражений (а не одно выражение, как в примерах выше), то каждое выражение 
располагается на следующей строке и предваряется отступом (как и в случае с многострочной функцией):</p>
<pre class="brush:f#;">
let country = "france"
...
<p>Но конструкция <span class="b">if..then</span> не просто выполняет некоторые действия в зависимости от условия. Она еще, подобно функции возвращает определенное значение:</p>
<pre class="brush:f#;">
let n1 = 10
let n2 = 4
let operation = "+"
...
<p>В данном случае в зависимости от значения operation (знака операции) выполняем над числами n1 и n2 ту или иную операцию. Возращаемый результат указывается 
после <code>then/else</code>. Затем результат передается значению <code>result</code>.</p>
<p>Если после операторов <code>then/else</code> указано несколко выражений, то результат определяет последнее выражение:</p>
<pre class="brush:f#;">
let countryCode = "de"
...
<p>В данном случае из if возвращается в завимости от условия одна из строк: "France"/ "Germany"/ "USA".</p>
<p>Стоит отметить, что если конструкция <code>if</code> не использует <code>else</code>, то она должна возвращать значение типа <code>unit</code>:</p>
<pre class="brush:f#;">
let countryCode = "fr"
...
<p>Здесь значение <code>result</code> представляет тип <code>unit</code> (функция <code>printfn</code> возвращает значение типа <code>unit</code>).</p>
