...
<html >
<head>
<title>F# | Конструкторы классов</title>
<meta charset="utf-8" />
<meta name="description" content="Конструкторы классов в языке программирования F#, создание объектов, определение первичного и вторичных конструкторов, оператор new, вызов цепочки конструкторов">
...
<h2>Конструкторы</h2><div class="date">Последнее обновление: 28.07.2021</div>
...
<p>Для создания объекта класса необходим вызов конструктора. Есть два типа конструкторов: первичные или основные и вторичные или дополнительные.</p>
<h3>Первичный конструктор</h3>
<p>Каждый класс имеет один первичный конструктор. Как и обычная функция, конструктор может иметь параметры. Например:</p>
<pre class="brush:f#;">
type Person() = do printfn "Создание объекта класса Person"

let tom = Person()
</pre>
<p>Перечисление параметров идет в скобках после имени класса. В примере выше параметры не используются, поэтому после имени класса указаны пустые скобки. 
Теперь определим пару параметров:</p>
<pre class="brush:f#;">
...
<p>Здесь первичный конструктор имеет два параметра: name и age. Параметры определяются в скобках через запятую. При вызове конструктора мы должны передать ему значения для его параметров.</p>
<p>Стоит отметить, что в определении класса после знака равно (=) фактически идут действия, выполняемые первичным конструктором. Поэтому мы можем обратиться к значениям параметров:</p>
<pre class="brush:f#;">do printfn $"Person name: {name}  age: {age}"</pre>
<p>Результат работы программы:</p>
<div class="console"><pre class="consoletext">
Person name: Tom  age: 37
Person name: Bob  age: 41
</pre></div>
<p>При этом параметры конструктора также можно типизировать, а при вызове конструктора передавать параметрам значения по имени:</p>
<pre class="brush:f#;">
...
<p>Класс может иметь только один первичный конструктор. Но в дополнение к нему можно определить несколько вторичных конструкторов. Они определяются с помощью ключевого слова 
<span class="b">new</span>. Вторичный конструктор должен вызывать первичный. Например, определим вторичный конструктор в классе Person:</p>
<pre class="brush:f#;">
type Person (name, age) = 
    do printfn $"Person name: {name}  age: {age}"
...
<p>Строка</p>
<pre class="brush:f#;">new() = Person("Undefined", 1)</pre>
<p>представляет определение вторичного конструктора - оно начинается с ключевого слова <span class="b">new</span>, после которого в скобках идут параметры этого конструктора. 
В данном случае вторичный конструктор не имеет параметров. После знака равно идет вызов первичного конструктора. Вызов первичного конструктора 
представляет имя класса, после которого в скобках передаются значения для параметров первичного конструктора. То есть фактически также, как мы создаем объект класса.</p>
<p>Поскольку мы определили второй конструктор, мы можем его использовать для создания объекта класса Person:</p>
<pre class="brush:f#;">let bob = Person()</pre>
<p>В данном случае будет выполняться вызов <code>Person("Undefined", 1)</code>.</p>
<p>Консольный вывод программы:</p>
<div class="console"><pre class="consoletext">
Person name: Tom  age: 37
Person name: Undefined  age: 1
</pre></div>
<p>Вторичных конструкторов может быть множество. Так, добавим еще один вторичный конструктор:</p>
<pre class="brush:f#;">
type Person (name, age) = 
    do printfn $"Person name: {name}  age: {age}"
    new() = Person("Undefined", 1)
...
<p>Здесь добавлен второй вторичный конструктор, который принимает один параметр:</p>
<pre class="brush:f#;">new(name) = Person(name, 1)</pre>
<p>Он также вызывает первичный конструктор, только его первому параметру передает значение своего единственного параметра.</p>
<p>После этого мы также сможем использовать этот конструктор для создания объектов:</p>
<pre class="brush:f#;">let sam = Person("Sam") </pre>
<p>Консольный вывод программы:</p>
<div class="console"><pre class="consoletext">
Person name: Tom  age: 37
Person name: Undefined  age: 1
...
<h3>Цепочка вызовов конструкторов</h3>
<p>Выше в примере оба вторичных конструктора вызывали первичный, однако они также могут вызывать и один из вторичных конструкторов, главное, чтобы цепочка вызовов конструкторов в 
конечном счете привела к вызову первичного конструктора:</p>
<pre class="brush:f#;">
type Person (name, age) = 
    do printfn $"Person name: {name}  age: {age}"
...
<p>Таким образом, создания объекта:</p>
<pre class="brush:f#;">let bob = Person()</pre>
<p>Приведет к вызову вторичного конструктора</p>
<pre class="brush:f#;">new() = Person("Undefined") </pre>
<p>Который вызовет другой вторичный конструктор:</p>
<pre class="brush:f#;">new(name) = Person(name, 1) </pre>
<p>А этот вызов, в свою очередь, приведет к вызову первичного конструктора.</p>
<h3>Определение действий во вторичном конструкторе</h3>
<p>Выше мы увидели, как вызывать некоторые действия в первичном конструкторе (с помощью выражения <span class="b">do</span>), но что, если 
мы хотим также определить некоторые действия во вторичном конструкторе? В этом случае используется выражение <span class="b">then</span>:</p>
<pre class="brush:f#;">
type Person (name, age) = 
    do printfn $"Person name: {name}  age: {age}"
...
<p>Теперь первый вторичный конструктор выглядит следующим образом:</p>
<pre class="brush:f#;">
new(name) =             // определение вторичного конструтора
	Person(name, 1) 
	then                // действия выполняемые во вторичном конструкторе
...
<p>После оператора <span class="b">then</span> помещаются все дополнительные действия, выполняемые этим конструктором.</p>
<p>Консольный вывод программы:</p>
<div class="console"><pre class="consoletext">
Person name: Sam  age: 1
...
<div class="nav"><p><a href="./3.1.php">Назад</a><a href="./">Содержание</a><a href="./3.3.php">Назад</a></p></div></div>
<div class="socBlock">
