...
<html >
<head>
<title>F# | Пространства имен</title>
<meta charset="utf-8" />
<meta name="description" content="Пространства имен в языке программирования F#, определение внешних и внутренних пространств имен, оператор namespace">
...
<h2>Пространства имен</h2><div class="date">Последнее обновление: 30.07.2021</div>
...
<p>Пространства имен позволяют сгруппировать код в отдельные логические области.</p>
<p>Для организации пространства имен применяется ключевое слово <span class="b">namespace</span>, после которого идет название название пространства имен:</p>
<pre class="brush:f#;">namespace название_пространства_имен</pre>
<p>При помещении кода в пространство имен определение пространства имен должно быть первым выражением в файле. В этом случае весь остальной код файла будет 
входить в данное пронстранство имен. При этом пространства имен напрямую не могут содержать значения и функции или вызовы функций. Они могут содержать только типы и модули. 
Поэтому если надо определить какие-то значения или функции, то внутри пространства имен для этого определяется локальный модуль.</p>
<p>Например, пусть у нас в проекте есть файл <span class="b">Hello.fs</span>:</p>
<img src="./pics/2.6.png" alt="Пространства имен namespace в F#" />
<p>Определим в нем пространство имен с названием <code>HelloMessages</code>:</p>
<pre class="brush:f#;">
...
<p>В этом пространстве имен определен тип <code>MessageFunc</code>, который представляет тип функции, которая принимает параметр типа <code>unit</code> и возвращает 
значение типа <code>unit</code>.</p>
<p>Также в пространстве имен определен модуль, который содержит три значения и одну функцию.</p>
<p>Для обращения к типам и модулям пространства имен перед ними через точку указывается название содержащего их пространства имен:</p>
<pre class="brush:f#;">
пространство_имен.тип
пространство_имен.модуль
</pre>
<p>Теперь используем выше определенное пространство имен HelloMessages в главном Файле проекта <span class="b">Program.fs</span>:</p>
<pre class="brush:f#;">
...
<p>Здесь определяется значение <code>sayGoodMorning</code>, которое имеет тип <code>HelloMessages.MessageFunc</code> и представляет функцию, которая 
обращается к значению <code>HelloMessages.Messages.goodMorning</code>. Затем вызываем эту функцию. И также здесь вызывается функция <code>HelloMessages.Messages.printHello</code>.</p>
<p>Но прописывать перед названиями модулей и типов названия пространств имен увеличивает объем кода, и мы можем, как и в случае с модулями, подключить 
все пространство имен с помощью оператора <span class="b">open</span>:</p>
<pre class="brush:f#;">
...
<h3>Несколько пространств имен в одном файле</h3>
<p>В одном файле может быть определено больше одного пространства имен. В этом случае все типы и модули, которые идут до определения пространства имен, принадлежат предыдущему пространству имен:</p>
<pre class="brush:f#;">
namespace HelloMessages
...
<p>Здесь определено два пространства имен. Пространству имен <code>HelloMessages</code> принадлежит тип <code>MessageFunc</code>, тогда как модуль Messages принадлежит 
пространству имен <code>EngMessages</code>.</p>
<p>Обращения к этим пространствам имен:</p>
<pre class="brush:f#;">
open HelloMessages  // подключаем пространство имен HelloMessages
...
<p>Пространства имен могут быть вложенными. В этом случае при определении вложенного пространства имен перед его названием указывается имя родительского пространства имен:</p>
<pre class="brush:f#;">
namespace HelloMessages
...
<p>Здесь пространство <code>EngMessages</code> является вложенным по отношению к HelloMessages. И чтобы обратиться, например, к функции <code>printHello</code> из EngMessages, 
нам надо указать все пространства имен:</p>
<pre class="brush:f#;">HelloMessages.EngMessages.Messages.printHello()</pre>
<h3>Неявная пространства имен</h3>
<p>При опредлении модуля уровня файла неявно также определяется одноименное пространство имен:</p>
<pre class="brush:f#;">
module Hello
...
<p>В данном случае автоматически неявно будут определено пространство имен Hello для всей функциональности модуля Hello.</p>
<p>Но также при определении модуля можно явно указать пространство имен, к которому принадлежит модуль уровня файла:</p>
<pre class="brush:f#;">
...
<p>Название пространства имен указывается перед именем модуля через точку. То есть в данном случае модуль Hello принадлежит пространству имен HelloMessages.</p>
<p>Использование такого модуля в Program.fs:</p>
<pre class="brush:f#;">
