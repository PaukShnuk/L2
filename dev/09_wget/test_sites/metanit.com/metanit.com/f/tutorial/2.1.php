...
<html >
<head>
<title>F# | Оператор let. Привязка значений</title>
<meta charset="utf-8" />
...
<h1>Основы программирования на F#</h1><h2>Оператор let. Привязка значений</h2><div class="date">Последнее обновление: 18.07.2021</div>
...
<p>Возможно, наиболее важным оператором, без которого сложно обойтись, является оператор <span class="b">let</span>. Этот оператор позволяет 
определить значения (value) и связать их с данными или действиями. То есть по сути значения представляют некоторые данные или действия, которые мы можем использовать в программе.</p>
<p>Общая форма определения значения:</p>
<pre class="brush:f#;">
let название_значения = данные/действия
</pre>
<p>После названия значения через знак равно(=) указываются связываемые с этим значением данные или действия.</p>
<p>В качестве названия значения может выступать произвольное название. Например, определим идентификатор <span class="b">name</span>:</p>
<pre class="brush:f#;">let name</pre>
<p>Однако подобное выражение неполное: нам надо что-то связать с этим значением. Что можно связать со значением? Это могут быть какие-то конкретные данные - числа, строки, 
хотя это также могут быть и целые действия. В данном случае мы сначала рассмотрим привязку данных. Например, передадим значению <code>name</code> какую-нибудь строку:</p>
<pre class="brush:f#;">let name = "Tom"</pre>
<p>Таким образом, мы определили значение <code>name</code>, которое связано со строкой "Tom" (по сути хранит строку "Tom").</p>
<p>Теперь определим программу, где используем этот значение:</p>
<pre class="brush:f#;">
let name = "Tom"
printfn "%s" name
</pre>
<p>В данном случае сначала определяем значение <code>name</code>, которое хранит строку "Tom". Затем мы передаем это значение для вывода на консоль 
функции <code>printfn()</code>. Для этого после названия функции <code>printfn</code> сначала идет строка "%s". <code>%s</code> указывает, что на месте этих символов 
(если точнее этого спецификатора) будет вставляться строка (значение <code>name</code> у нас представляет строку). 
Далее после "%s" указывается, что будет вставляться вместо <code>%s</code>, то есть значение <code>name</code>.</p>
<p>Общий вид этой программы в Visual Studio:</p>
<img src="./pics/2.1.png" alt="Values или определение значений в F#" />
<p>Запустим данную программу, и на консоль будет выведено значение <code>name</code>, то есть строка "Tom"</p>
<div class="console"><pre class="consoletext">Tom</pre></div>
<p>Таким образом, как мы видим, значения позволяют сохранять данные, которые далее могут использоваться в программе.</p>
<p>Подобным образом можно определять и другие значения, например:</p>
<pre class="brush:f#;">
let name = "Tom"
printfn "Name: %s" name
...
<p>Здесь также определяется значение <span class="b">age</span>, которое представляет число 37. Поскольку оно представляет целое число, то для его 
вывода на консоль нам надо использовать спецификатор <code>%d</code>. Таким образом, в последней строке кода при выводе на консоль в строку 
"Age %d" вместо <code>%d</code> будет вставляться значение <code>age</code>. Консольный вывод программы:</p>
<div class="console">
...
<h3>Изменяемые значения</h3>
<p>Все значения по умолчанию являются <span class="b">неизменяемыми</span> (immutable). Например, определим следующую программу:</p>
<pre class="brush:f#;">
let name = "Tom"
printfn "%s" name    // Tom
...
<p>Здесь мы видим, что несмотря на строку кода <code>name = "Bob"</code> данные значения <code>name</code> не изменились. В этом плане значения в F# похожи на константы в некоторых языках программирования.</p>
<p>Однако иногда необходимо все таки менять значения. В этом случае значение определяется с помощью ключевого слова <span class="b">mutable</span>:</p>
<pre class="brush:f#;">
...
<p>При этом чтобы присвоить значению новые данные применяется оператор <span class="b">&lt;-</code>.</p>
<p>Консольный вывод программы:</p>
<div class="console">
<pre class="consoletext">
Tom
