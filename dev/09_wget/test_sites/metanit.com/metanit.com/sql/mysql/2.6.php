...
<html >
<head>
<title>MySQL | Изменение таблиц и столбцов</title>
<meta charset="utf-8" />
<meta name="description" content="Изменение таблиц и столбцов в MySQL, выражение ALTER TABLE, добавление и удаление первичных и внешних ключей, добавление и удаление столбцов, изменение типа столбца">
...
<h2>Изменение таблиц и столбцов</h2><div class="date">Последнее обновление: 11.05.2018</div>
...
<p>Если таблица уже была ранее создана, и ее необходимо изменить, то для этого применяется команда <span class="b">ALTER TABLE</span>. Ее 
сокращенный формальный синтаксис:</p>
<pre class="brush:sql;">
...
<p>Вообще данная команда поддерживает гораздо больше опций и возможностей. Все их можно посмотреть в документации. Рассмотрим лишь основные сценарии, 
с которыми мы можем столкнуться.</p>
...
<p>В данном случае столбец Address имеет тип VARCHAR и для него определен атрибут NULL.</p>
...
<h3>Изменение значения по умолчанию</h3>
<p>Установим в таблице Customers для столбца Age значение по умолчанию 22:</p>
<pre class="brush:sql;">
ALTER TABLE Customers
ALTER COLUMN Age SET DEFAULT 22;
</pre>
<h3>Изменение типа столбца</h3>
<p>Изменим в таблице Customers тип данных у столбца FirstName на <code>CHAR(100)</code> и установим для него атрибут <code>NULL</code>:</p>
<pre class="brush:sql;">
ALTER TABLE Customers
MODIFY COLUMN FirstName CHAR(100) NULL;
</pre>
<h3>Добавление и удаление внешнего ключа</h3>
...
<p>При добавлении ограничений мы можем указать для них имя, используя оператор <span class="b">CONSTRAINT</span>, после которого указывается имя 
ограничения:</p>
<pre class="brush:sql;">
ALTER TABLE Orders
...
<p>В данном случае ограничение внешнего ключа называется orders_customers_fk. Затем по этому имени мы можем удалить ограничение:</p>
<pre class="brush:sql;">
ALTER TABLE Orders
DROP FOREIGN KEY orders_customers_fk;
</pre>
<h3>Добавление и удаление первичного ключа</h3>
<p>Добавим в таблицу Products первичный ключ:</p>
<pre class="brush:sql;">
...
<p>Теперь удалим первичный ключ:</p>
<pre class="brush:sql;">
ALTER TABLE Products
DROP PRIMARY KEY;
</pre>
