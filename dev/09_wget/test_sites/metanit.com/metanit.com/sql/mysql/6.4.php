...
<html >
<head>
<title>MySQL | Функции CASE, IF, IFNULL, COALESCE</title>
<meta charset="utf-8" />
<meta name="description" content="Функции CASE, IF, IFNULL, COALESCE, создание условных конструкций в MySQL">
...
<h2>Функции CASE, IF, IFNULL, COALESCE</h2><div class="date">Последнее обновление: 26.05.2018</div>
...
<h3>CASE</h3>
<p>Функция <span class="b">CASE</span> проверяет истинность набора условий и в зависимости от результата проверки может возвращать тот или иной результат. 
Эта функция принимает следующую форму:</p>
<pre class="brush:sql;">
CASE
...
<p>Выполним запрос к этой таблице и используем функцию CASE:</p>
<pre class="brush:sql;">
...
<img src="pics/8.3.png" alt="Функция CASE в MySQL" />
<h3>Функция IF</h3>
<p>Функция <span class="b">IF</span> в зависимости от результата условного выражения возвращает одно из двух значений. Общая форма функции выглядит 
следующим образом:</p>
<pre class="brush:sql;">IF(условие, значение_1, значение_2)</pre>
...
<img src="pics/8.4.png" alt="Функция IF в MySQL" />
<h3>IFNULL</h3>
<p>Функция <span class="b">IFNULL</span> проверяет значение некоторого выражения. Если оно равно NULL, то функция возвращает значение, которое передается в 
качестве второго параметра:</p>
<pre class="brush:sql;">IFNULL(выражение, значение)</pre>
<p>Например, возьмем следующую таблицу</p>
<pre class="brush:sql;">
...
<p>И применим при получении данных функцию IFNULL:</p>
<pre class="brush:sql;">
...
<img src="pics/8.5.png" alt="Функция IFNULL в MySQL" />
<h3>COALESCE</h3>
<p>Функция <span class="b">COALESCE</span> принимает список значений и возвращает первое из них, которое не равно NULL:</p>
<pre class="brush:sql;">
COALESCE(выражение_1, выражение_2, выражение_N)
</pre>
<p>Например, выберем из таблицы Clients пользователей и в контактах у них определим либо телефон, либо электронный адрес, если они не равны NULL:</p>
<pre class="brush:sql;">
SELECT FirstName, LastName,
        COALESCE(Phone, Email, 'не определено') AS Contacts
FROM Clients;
</pre>
<p>То есть в данном случае возвращается телефон, если он определен. Если он не определен, то возвращается электронный адрес. Если и электронный адрес не определен, то возвращается строка "не определено".</p>
