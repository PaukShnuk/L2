...
<html >
<head>
<title>MySQL | Операторы фильтрации</title>
<meta charset="utf-8" />
<meta name="description" content="Операторы фильтрации данных из БД в MySQL, IN, BETWEEN, IS NULL, LIKE, REGEXP, использование регулярных выражений">
...
<h2>Операторы фильтрации</h2><div class="date">Последнее обновление: 16.05.2018</div>
...
<h3>Оператор IN</h3>
<p>Оператор <span class="b">IN</span> определяет набор значений, которые должны иметь столбцы:</p>
...
<img src="pics/4.4.png" alt="Оператор IN в mysql" />
<p>Оператор <span class="b">NOT</span>, наоборот, позволяет выбрать все строки, столбцы которых не имеют определенных значений:</p>
<pre class="brush:sql;">
SELECT * FROM Products
WHERE Manufacturer NOT IN ('Samsung', 'HTC', 'Huawei');
</pre>
<img src="pics/4.5.png" alt="Оператор NOT IN в mysql" />
...
<img src="pics/4.6.png" alt="Оператор BETWEEN в mysql" />
...
<img src="pics/4.7.png" alt="Оператор LIKE в MySQL" />
<p><span class="b">REGEXP</span> позволяет задать регулярное выражение, которому должно соответствовать значение столбца. В этом плане REGEXP представляет 
более изощренный и комплексный способ фильтрации, нежели оператор LIKE. REGEXP имеет похожий синтаксис:</p>
<pre class="brush:sql;">
WHERE выражение [NOT] REGEXP регулярное выражение
</pre>
<p>Регулярное выражение может принимать следующие специальные символы:</p>
<ul>
<li><p><span class="b">^</span>: указывает на начало строки</p></li>
<li><p><span class="b">$</span>: указывает на конец строки</p></li>
<li><p><span class="b">.</span>: соответствует любому одиночному символу</p></li>
<li><p><span class="b">[символы]</span>: соответствует любому одиночному символу из скобок</p></li>
<li><p><span class="b">[начальный_символ-конечный_символ]</span>: соответствует любому одиночному символу из диапазона символов</p></li>
<li><p><span class="b">|</span>: отделяет два шаблона строки, и значение должно соответствовать одну из этих шаблонов</p></li>
</ul>
<p>Примеры REGEXP:</p>
<ul>
<li><p><code>WHERE ProductName REGEXP 'Phone'</code>: строка должна содержать "Phone", например, <code>iPhone X</code>, <code>Nokia Phone N</code>, <code>iPhone</code></p></li>
<li><p><code>WHERE ProductName REGEXP '^Phone'</code>: строка должна начинаться с "Phone", например, <code>Phone 34</code>, <code>PhoneX</code></p></li>
<li><p><code>WHERE ProductName REGEXP 'Phone$'</code>: строка должна заканчиваться на "Phone", например, <code>iPhone</code>, <code>Nokia Phone</code></p></li>
<li><p><code>WHERE ProductName REGEXP 'iPhone [78]';</code>: строка должна содержать либо <code>iPhone 7</code>, либо <code>iPhone 8</code></p></li>
<li><p><code>WHERE ProductName REGEXP 'iPhone [6-8]';</code>: строка должна содержать либо <code>iPhone 6</code>, либо <code>iPhone 7</code>, либо <code>iPhone 8</code></p></li>
</ul>
<p>Например, найдем товары, названия которых содержат либо "Phone", либо "Galaxy":</p>
<pre class="brush:sql;">
SELECT * FROM Products
WHERE ProductName REGEXP 'Phone|Galaxy';
</pre>
<img src="pics/4.8.png" alt="Оператор REGEXP в MySQL" />
<h3>IS NULL</h3>
