...
<html >
<head>
<title>MySQL | Оператор EXISTS</title>
<meta charset="utf-8" />
<meta name="description" content="Оператор EXISTS в MySQL, применение в подзапросах">
...
<h2>Оператор EXISTS</h2><div class="date">Последнее обновление: 22.05.2018</div>
...
<pre class="brush:sql;">WHERE [NOT] EXISTS (подзапрос)</pre>
<p>Например, найдем все товары из таблицы Products, на которые есть заказы в таблице Orders:</p>
<pre class="brush:sql;">
SELECT * FROM Products
WHERE EXISTS 
(SELECT * FROM Orders WHERE Orders.ProductId = Products.Id)
</pre>
<img src="pics/6.5.png" alt="Оператор EXISTS в MySQL" />
...
Например, найдем все товары из таблицы Products, на которые не было заказов в таблице Orders:</p>
<pre class="brush:sql;">
SELECT * FROM Products
WHERE NOT EXISTS (SELECT * FROM Orders WHERE Products.Id = Orders.ProductId)
</pre>
<p>Стоит отметить, что для получения подобного результата можно было бы использовать и опеатор <span class="b">IN</span>:</p>
<pre class="brush:sql;">
SELECT *
FROM Products
WHERE Id NOT IN (SELECT ProductId FROM Orders)
</pre>
<p>Но поскольку при применении EXISTS не происходит выборка строк, то его использование более оптимально и эффективно, чем использование оператора IN.</p>
