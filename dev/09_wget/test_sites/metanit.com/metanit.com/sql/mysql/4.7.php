...
<html >
<head>
<title>MySQL | Подзапросы</title>
<meta charset="utf-8" />
<meta name="description" content="Подзапросы в MySQL, коррелирующие и некоррелирующие подзапросы">
...
<h2>Подзапросы</h2><div class="date">Последнее обновление: 22.05.2018</div>
...
<p>Например, создадим таблицы для товаров и заказов:</p>
<pre class="brush:sql;">
...
<p>Таблица Orders содержит данные о купленным товарам из таблицы Products.</p>
...
<p>При добавлении данных в таблицу Orders как раз используются подзапросы. Например, первый заказ был сделан на товар Galaxy S8. Соответственно в 
...
Но на момент написания запроса нам может быть неизвестен ни Id покупателя, ни Id товара, ни цена товара. В этом случае можно выполнить подзапрос в виде</p>
<pre class="brush:sql;">
(SELECT Price FROM Products WHERE ProductName='iPhone 8')
</pre>
<p>Подзапрос выполняет команду SELECT и заключается в скобки. В данном же случае при добавлении одного товара выполняется два подзапроса. Каждый 
подзапрос возвращает одного скалярное значение, например, числовой идентификатор.</p>
...
<img src="pics/6.1.png" alt="подзапросы в MySQL" />
...
<p>В данном случае для каждой строки из таблицы Orders будет выполняться подзапрос, результат которого зависит от столбца ProductId. И каждый подзапрос может 
возвращать различные данные.</p>
<img src="pics/6.3.png" alt="Correlated subquery in MySQL" />
...
<img src="pics/6.4.png" alt="Коррелирующий подзапрос в MySQL" />
<p>Здесь определено два коррелирующих подзапроса. Первый подзапрос определяет спецификацию столбца AvgPrice. 
