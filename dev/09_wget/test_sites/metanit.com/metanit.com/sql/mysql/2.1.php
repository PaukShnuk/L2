...
<html >
<head>
<title>MySQL | Создание и удаление базы данных</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и удаление базы данных в MySQL, команды CREATE DATABASE и DROP DATABASE">
...
<h1>Определение структуры данных</h1><h2>Создание и удаление базы данных</h2><div class="date">Последнее обновление: 04.05.2018</div>
...
<p>Для создания базы данных используется команда <span class="b">CREATE DATABASE</span>. Она имеет следующий синтаксис:</p>
<pre class="brush:sql;">
CREATE DATABASE [IF NOT EXISTS] имя_базы_даных;
</pre>
<p>В конце команды указывается имя базы данных.</p>
<p>Первая форма <code>CREATE DATABASE имя_базы_даных</code> пытается создать базу данных, но если такая база данных уже существует, то операция возвратит ошибку.</p>
<p>Вторая форма <code>CREATE DATABASE IF NOT EXISTS имя_базы_даных</code> пытается создать базу данных, если на сервере отсутствует бд с таким именем.</p>
<p>Например, в MySQL Workbench CE (или в MySQL Command Line Client) выполним следующую команду:</p>
<pre class="brush:sql;">
CREATE DATABASE productsdb;
</pre>
<p>Она создаст на сервере бд productsdb.</p>
<img src="pics/2.1.png" alt="CREATE DATABASE in MySQL" />
<h3>Установка базы данных</h3>
<p>После создания БД с ней производятся различные операции: создание таблиц, добавление и получение данных и т.д. Но чтобы установить производить эти операции, надо 
установить определенную базу данных в качестве используемой. Для этого применяется оператор <span class="b">USE</span>:</p>
<pre class="brush:sql;">USE productsdb;</pre>
<h3>Удаление базы данных</h3>
<p>Для удаления базы данных применяется команда <span class="b">DROP DATABASE</span>, которая имеет следующий синтаксис:</p>
<pre class="brush:sql;">
DROP DATABASE [IF EXISTS] имя_базы_даных;
</pre>
<p>Первая форма <code>DROP DATABASE имя_базы_даных</code> пытается удалить базу данных, но если такая база данных отсутствует на сервере, то операция возвратит ошибку.</p>
<p>Вторая форма <code>DROP DATABASE IF EXISTS имя_базы_даных</code> пытается удалить базу данных, если на сервере имеется бд с таким именем.</p>
<p>Например, удалим выше созданную базу данных productsdb:</p>
<pre class="brush:sql;">
