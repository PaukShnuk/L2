...
<html >
<head>
<title>MySQL | Добавление данных. Команда INSERT</title>
<meta charset="utf-8" />
<meta name="description" content="Добавление данных в MySQL, команда INSERT, установка значений по умолчанию">
...
<h1>Основные операции с данными</h1><h2>Добавление данных. Команда INSERT</h2><div class="date">Последнее обновление: 11.05.2018</div>
...
<p>Для добавления данных в БД в MySQL используется команда <span class="b">INSERT</span>, которая имеет следующий формальный синтаксис:</p>
<pre class="brush:sql;">
...
<p>Например, пусть в базе данных productsdb есть следующая таблица Products:</p>
<pre class="brush:sql;">
...
<p>В данно случае значения будут передаваться столбцам по позиции. То есть стобцу ProductName передается строка "iPhone X", столбцу Manufacturer - 
строка "Apple" и так далее.</p>
<p>Важно, чтобы между значениями и типами данных столбцов было соответствие. Так, столбец ProductName представляет тип <code>varchar</code>, то есть 
строку. Соответственно этому столбцу мы можем передать строковое значение в одинарных кавычках. А стобец ProductCount представляет тип <code>int</code>, 
то есть целое число, поэтому данному столбцу нужно передать целые числа, но никак не строки.</p>
<p>После удачного выполнения в MySQL Workbench в поле вывода должны появиться зеленый маркер и сообщение "1 row(s) affected":</p>
<img src="pics/3.1.png" alt="INSERT и добавление данных в MySQL" />
<p>Необязательно при добавлении данных указывать значения абсолютно для всех столбцов таблицы. Например, в примере выше не указано значение 
для стобца Id. Но поскольку для данного столбца определен атрибут <code>AUTO_INCREMENT</code>, то его значение будет автоматически генерироваться.</p>
<p>Также мы можем опускать при добавлении такие столбцы, которые поддерживают значение NULL или для которых указано значение по умолчанию, то есть для них определены атрибуты 
<code>NULL</code> или <code>DEFAULT</code>. Так, в таблице Products столбец ProductCount имеет значение по умолчанию - число 0. Поэтому мы можем при добавлении опустить 
этот столбец, и ему будет передаваться число 0:</p>
<pre class="brush:sql;">
INSERT Products(ProductName, Manufacturer, Price) 
VALUES ('Galaxy S9', 'Samsung', 63000);
</pre>
<p>С помощью ключевых слов <code>DEFAULT</code> и <code>NULL</code> можно указать, что в качестве значения будет использовать значение по умолчанию или 
NULL соответственно:</p>
<pre class="brush:sql;">
