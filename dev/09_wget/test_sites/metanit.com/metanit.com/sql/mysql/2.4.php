...
<html >
<head>
<title>MySQL | Атрибуты столбцов и таблиц</title>
<meta charset="utf-8" />
<meta name="description" content="Атрибуты столбцов и таблиц в MySQL, NOT NULL, DEFAULT, установка значения по умолчанию, первичные ключи, PRIMARY KEY, UNIQUE, автогенерация значений">
...
<h2>Атрибуты столбцов и таблиц</h2><div class="date">Последнее обновление: 11.05.2018</div>
...
<p>С помощью атрибутов можно настроить поведение столбцов. Рассмотрим, какие атрибуты мы можем использовать.</p>
<h3>PRIMARY KEY</h3>
<p>Атрибут <span class="b">PRIMARY KEY</span> задает первичный ключ таблицы.</p>
<pre class="brush:sql;">
USE productsdb;
...
<p>Первичный ключ уникально идентифицирует строку в таблице. В качестве первичного ключа необязательно должны выступать столбцы с типом int, они 
могут представлять любой другой тип.</p>
...
<p>Первичный ключ может быть составным. Такой ключ использовать сразу несколько столбцов, чтобы уникально идентифицировать 
...
<p>Атрибут <span class="b">AUTO_INCREMENT</span> позволяет указать, что значение столбца будет автоматически увеличиваться при добавлении новой строки. 
Данный атрибут работает для столбцов, которые представляют целочисленный тип или числа с плавающей точкой.</p>
<pre class="brush:sql;">
CREATE TABLE Customers
(
...
<h3>UNIQUE</h3>
<p>Атрибут <span class="b">UNIQUE</span> указывает, что столбец может хранить только уникальные значения.</p>
<pre class="brush:sql;">
CREATE TABLE Customers
(
    Id INT PRIMARY KEY AUTO_INCREMENT,
...
<p>В данном случае столбец Phone, который представляет телефон клиента, может хранить только уникальные значения. 
...
<p>Чтобы указать, может ли столбец принимать значение <span class="b">NULL</span>, при определении столбца ему можно задать атрибут 
<span class="b">NULL</span> или <span class="b">NOT NULL</span>. Если этот атрибут явным образом не будет использован, то по умолчанию столбец 
будет допускать значение NULL. Исключением является тот случай, когда столбец выступает в роли первичного ключа - в этом случае по умолчанию столбец имеет значение NOT NULL.</p>
<pre class="brush:sql;">
CREATE TABLE Customers
(
    Id INT PRIMARY KEY AUTO_INCREMENT,
...
<p>В данном случае столбец Age по умолчанию будет иметь атрибут NULL.</p>
<h3>DEFAULT</h3>
<p>Атрибут <span class="b">DEFAULT</span> определяет значение по умолчанию для столбца. Если при добавлении данных для столбца не будет предусмотрено 
значение, то для него будет использоваться значение по умолчанию.</p>
<pre class="brush:sql;">
CREATE TABLE Customers
(
    Id INT PRIMARY KEY AUTO_INCREMENT,
...
<h3>CHECK</h3>
<p>Атрибут <span class="b">CHECK</span> задает ограничение для диапазона значений, которые могут храниться в столбце. Для этого после CHECK 
...
<p>Кроме проверки возраста здесь также проверяется, что столбцы Email и Phone не могут иметь пустую строку в качестве значения (пустая строка не эквивалентна значению NULL).</p>
...
<p>С помощью ключевого слова <span class="b">CONSTRAINT</span> можно задать имя для ограничений. Они указываются после ключевого слова CONSTRAINT перед атрибутами на уровне таблицы:</p>
<pre class="brush:sql;">
CREATE TABLE Customers
(
    Id INT AUTO_INCREMENT,
...
<p>В данном случае ограничение для PRIMARY KEY называется customers_pk, для UNIQUE - customer_phone_uq, а для CHECK - customer_age_chk. Смысл установки 
...
<p>Установить имя можно для ограничений PRIMARY KEY, CHECK, UNIQUE, а также FOREIGN KEY, который рассматриватся далее.</p>
