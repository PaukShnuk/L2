...
<html >
<head>
<title>MySQL | Обновление данных. Команда UPDATE</title>
<meta charset="utf-8" />
<meta name="description" content="Обновление данных в MySQL, команда UPDATE, оператор SET, установка значений, условия обновления">
...
<h2>Обновление данных. Команда UPDATE</h2><div class="date">Последнее обновление: 11.05.2018</div>
...
<p>Команда <span class="b">UPDATE</span> применяется для обновления уже имеющихся строк. Она имеет следующий формальный синтаксис:</p>
<pre class="brush:sql;">
UPDATE имя_таблицы
...
<p>Однако при выполнении данного запроса в MySQL Workbench мы можем столкнуться с ошибкой:</p>
<img src="pics/3.12.png" alt="Safe UPDATE mode in MySQL Workbench" />
<p>Ошибка говорит о том, что мы находимся в безопасном режиме. И чтобы его отключить, в MySQL Workbench надо перейти в меню <span class="b">Edit -&gt; Preferences</span> 
и в открывшемся окне перейти к пункту <span class="b">SQL Editor</span>:</p>
<img src="pics/3.13.png" alt="Disable safe mode in MySQL Workbench" />
<p>В открывшейся вкладке в самом низу надо снять флажок с поля "Safe Updates (reject UPDATEs and DELETEs with no restrictions)" и затем сохранить изменения, 
нажав на кнопку OK. После этого надо переподключиться к серверу.</p>
<p>Используем выражение WHERE и изменим название производителя с "Samsung" на "Samsung Inc.":</p>
<pre class="brush:sql;">
UPDATE Products
...
<p>При обновлении вместо конкретных значений и выражений мы можем использовать ключевые слова <span class="b">DEFAULT</span> и <span class="b">NULL</span> 
для установки соответственно значения по умолчанию или NULL:</p>
<pre class="brush:sql;">
UPDATE Products
