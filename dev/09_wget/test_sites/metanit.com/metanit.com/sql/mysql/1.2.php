...
<html >
<head>
<title>MySQL | Консольный клиент MySQL Command Line Client</title>
<meta charset="utf-8" />
<meta name="description" content="Консольный клиент MySQL Command Line Client, подключение к серверу и выполнение запросов к базе данных">
...
<h2>Консольный клиент MySQL Command Line Client</h2><div class="date">Последнее обновление: 04.06.2021</div>
...
<p>При установке сервера MySQL также устанавливается консольный клиент для работы с базами данных. Например, в Windows в меню Пуск можно найти 
программу <span class="b">MySQL 8.0 Command Line Client</span>. Это и есть собственно консольный клиент:</p>
<img src="./pics/1.15.png" alt="MySQL 8.0 Command Line Client" />
<p>Причем клиент устанавливается сразу в двух вариантах - с поддержкой Unicode и без.</p>
<p>Запустим MySQL 8.0 Command Line Client - Unicode. Вначале нам отобразится предложение ввести пароль:</p>
<img src="./pics/1.16.png" alt="Ввод пароля в MySQL Command Line Client" />
<p>Здесь необходимо ввести пароль, который был установлен при установке MySQL для пользователя root. 
И после удачного подключения можно будет отправлять серверу команды через данный консольный клиент.</p>
<p>Для начала посмотрим, какие базы данные есть на сервере по умолчанию. Для этого введем команду <span class="b">show databases;</span></p>
<img src="./pics/1.17.png" alt="Show databases в MySQL" />
<p>После выполнения этой команды мы увидим, что на сервере по умолчанию уже есть ряд баз данных, которые выполняют административные функции.</p>
<p>Теперь создадим базу данных с помощью следующей команды языка SQL:</p>
<div class="console"><pre class="consoletext">create database test;</pre></div>
...
данном случае база данных будет называться "test".</p>
...
<div class="console"><pre class="consoletext">use test;</pre></div>
...
<img src="./pics/1.18.png" alt="Создание базы данных и таблицы в MySQL Command Line Client" />
...
<img src="./pics/1.19.png" alt="Консольный клиент MySQL Command Line Client" />
<p>Таким образом в общих чертах мы можем работать с консольным клиентом MySQL Command Line Client.</p>
