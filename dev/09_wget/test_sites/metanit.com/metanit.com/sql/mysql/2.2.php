...
<html >
<head>
<title>MySQL | Создание и удаление таблиц</title>
<meta charset="utf-8" />
<meta name="description" content="Создание, переименование и удаление таблиц в MySQL, команды CREATE TABLE и DROP TABLE">
...
<h2>Создание и удаление таблиц</h2><div class="date">Последнее обновление: 04.05.2018</div>
...
<p>Для создания таблиц используется команда <span class="b">CREATE TABLE</span>. Эта команды применяет ряд операторов, которые определяют столбцы 
таблицы и их атрибуты. Общий формальный синтаксис команды CREATE TABLE:</p>
<pre class="brush:sql;">
...
<p>После команды CREATE TABLE идет название таблицы. Имя таблицы выполняет роль ее идентификатора в базе данных, поэтому оно должно быть 
...
Атрибуты столбцов, а также атрибуты таблицы указывать необязательно.</p>
<p>Создадим простейшую таблицу. Для этого выполним следующий скрипт:</p>
<pre class="brush:sql;">
...
<p>Таблица не может создаваться сама по себе. Она всегда создается в определенной базе данных. Вначале здесь создается база данных productsdb. 
...
<span class="b">USE</span>.</p>
<p>Далее собственно идет создание таблицы, которая называется Customers. Она определяет четыре столбца: Id, Age, FirstName, LastName. Первые два столбца представляют идентификатор клиента и его возраст и имеют тип <code>INT</code>, то есть будут хранить числовые значения. 
Следующие столбцы представляют имя и фамилию клиента и имеют тип <code>VARCHAR(20)</code>, то есть представляют строку длиной не более 20 символов. 
В данном случае для каждого столбца определены имя и тип данных, при этом атрибуты столбцов и таблицы в целом отсутствуют.</p>
<img src="pics/2.2.png" alt="CREATE TABLE и создание таблиц в MySQL" />
<p>И в результате выполнения этой команды будет создана база данных productsdb, в которой будет создана таблица Customers.</p>
<h3>Переименование таблиц</h3>
<p>Если после создания таблицы мы захотим ее переименовать, то для этого нужно использовать команду <span class="b">RENAME TABLE</span>, 
которая имеет следующий синтаксис:</p>
<pre class="brush:sql;">
RENAME TABLE старое_название TO новое_название;
</pre>
<p>Например, переименуем таблицу Customers в Clients:</p>
<pre class="brush:sql;">
RENAME TABLE Customers TO Clients;
</pre>
<h3>Полное удаление данных</h3>
<p>Для полного удаления данных, очистки таблицы применяется команда <span class="b">TRUNCATE TABLE</span>. Например, очистим таблицу Clients:</p>
<pre class="brush:sql;">
TRUNCATE TABLE Clients;
</pre>
<h3>Удаление таблиц</h3>
<p>Для удаления таблицы из БД применяется команда <span class="b">DROP TABLE</span>, после которой указывается название удаляемой таблицы. Например, 
удалим таблицу Clients:</p>
<pre class="brush:sql;">
DROP TABLE Clients;
</pre>
