...
<html >
<head>
<title>MySQL | Outer Join</title>
<meta charset="utf-8" />
<meta name="description" content="Внешнее соединение таблиц в MySQL с помощью оператора Outer Join, правостороннее и левостороннее соединение">
...
<h2>Outer Join</h2><div class="date">Последнее обновление: 22.05.2018</div>
...
<p>В предыдущей теме рассматривля Inner Join или внутреннее соединение таблиц. Но также в MySQL мы можем использовать и так называемое 
...
<p>Перед оператором <span class="b">JOIN</span> указывается одно из ключевых слов <span class="b">LEFT</span> или 
<span class="b">RIGHT</span>, которые определяют тип соединения:</p>
<ul>
...
<li><p><span class="b">RIGHT</span>: выборка будет содержать все строки из второй или правой таблицы</p></li>
 ...
 <p>Также перед оператором JOIN может указываться ключевое слово <span class="b">OUTER</span>, но его применение необязательно. 
 Далее после JOIN указывается присоединяемая таблица, а затем идет условие соединения.</p>
<p>Например, соединим таблицы Orders и Customers:</p>
<pre class="brush:sql;">
...
<img src="pics/7.6.png" alt="Левостороннее соединение OUTER JOIN в MySQL" />
...
не включаются в выходную выборку. Left Join выбирает все строки первой таблицы и затем присоединяет к ним строки правой таблицы. К примеру, возьмем таблицу Customers и добавим к покупателям информацию об их заказах:</p>
<pre class="brush:sql;">
#INNER JOIN
...
<img src="pics/7.7.png" alt="INNER join vs left join в MySQL"/>
<p>В случае с LEFT JOIN MySQL выбирает сначала всех покупателей из таблицы Customers, затем сопоставляет их с заказами из таблицы Orders через 
условие <code>Orders.CustomerId = Customers.Id</code>. Однако не у всех покупателей есть заказы. В этом случае покупателю для соответствующих столбцов 
устанавливаются значения NULL.</p>
<p>Изменим в примере выше тип соединения для OUTER JOIN с левостороннего на правостороннее:</p>
<pre class="brush:sql;">
SELECT FirstName, CreatedAt, ProductCount, Price 
FROM Customers RIGHT JOIN Orders 
ON Orders.CustomerId = Customers.Id;
</pre>
<p>Теперь будут выбираться все строки из Orders (из правой таблицы), а к ним уже будет присоединяться связанные по условию строки из таблицы Customers:</p>
<img src="pics/7.8.png" alt="OUTER JOIN в MySQL" />
...
<img src="pics/7.9.png" alt="Правостороннее соединение OUTER Join в MySQL" />
...
у которых цена больше 45000, и отсортируем по дате заказа:</p>
<pre class="brush:sql;">
SELECT Customers.FirstName, Orders.CreatedAt, 
       Products.ProductName, Products.Manufacturer
FROM Orders 
