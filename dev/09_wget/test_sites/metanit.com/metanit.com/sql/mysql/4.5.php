...
<html >
<head>
<title>MySQL | Агрегатные функции</title>
<meta charset="utf-8" />
<meta name="description" content="Агрегатные функции в MySQL, AVG, COUNT, SUM, Min, Max, поиск суммы, среднего, максимально и минимального значения, количество строк">
...
<h2>Агрегатные функции</h2><div class="date">Последнее обновление: 21.05.2018</div>
...
<p>Агрегатные функции вычисляют некоторые скалярные значения в наборе строк. В MySQL есть следующие агрегатные функции:</p>
<ul>
...
<p>Все агрегатные функции принимают в качестве параметра выражение, которое представляет критерий для определения значений. Зачастую, в качестве 
выражения выступает название столбца, над значениями которого надо проводить вычисления.</p>
<p>Выражения в функциях <span class="b">AVG</span> и <span class="b">SUM</span> должно представлять числовое значение (например, столбец, который хранит числовые значения). 
Выражение в функциях <span class="b">MIN</span>, <span class="b">MAX</span> и <span class="b">COUNT</span> может представлять числовое или строковое значение или дату.</p>
<p>Все агрегатные функции за исключением <code>COUNT(*)</code> игнорируют значения NULL.</p>
<h3>Avg</h3>
<p>Функция <span class="b">Avg</span> возвращает среднее значение на диапазоне значений столбца таблицы.</p>
<p>Например, пусть есть следующая таблица товаров Products:</p>
<pre class="brush:sql;">
...
<img src="pics/5.1.png" alt="Функция avg и поиск среднего значения в MySQL" />
...
<img src="pics/5.2.png" alt="Функция count в MySQL и вычисление количества строк" />
...
<img src="pics/5.3.png" alt="MIN и MAX в MySQL" />
...
<h3>All и Distinct</h3>
<p>По умолчанию все вышеперечисленных пять функций учитывают все строки выборки для вычисления результата. Но выборка может содержать повторяющие значения. 
...
этого применяется оператор <span class="b">DISTINCT</span>.</p>
<pre class="brush:sql;">
SELECT COUNT(DISTINCT Manufacturer) FROM Products
</pre>
<p>По умолчанию вместо DISTINCT применяется оператор <span class="b">ALL</span>, который выбирает все строки:</p>
<pre class="brush:sql;">
SELECT COUNT(ALL Manufacturer) FROM Products
</pre>
<p>В данном случае мы видим, что производители могут повторяться в таблице, так как некоторые товары могут иметь одних и тех же производителей. Поэтому 
чтобы подсчитать количество уникальных производителей, необходимо использовать оператор DISTINCT.</p>
<p>Так как этот оператор неявно подразумевается при отсутствии DISTINCT, то его можно не указывать.</p>
