...
<html >
<head>
<title>MySQL | Графический клиент MySQL Workbench</title>
<meta charset="utf-8" />
<meta name="description" content="Графический клиент MySQL Workbench, подключение к серверу и выполнение запросов к базе данных, создание бд и таблиц">
...
<h2>Графический клиент MySQL Workbench</h2><div class="date">Последнее обновление: 04.06.2021</div>
...
<p>Для упрощения работы с сервером MySQL в базовый комплект установки входит такой инструмент как <span class="b">MySQL Workbench</span>. 
...
управлять ими. Так, на Windows после установки в меню Пуск мы можем найти значок программы и запустить ее:</p>
<img src="pics/1.20.png" alt="MySQL Workbench CE" />
<p>Нам откроется следующее окно, где мы можем увидеть поле с названием запущенного локально экземпляра MySQL:</p>
<img src="pics/1.21.png" alt="Local Instance MySQL" />
<p>Нажмем на него, и нам отобразится окно для ввода пароля:</p>
<img src="pics/1.22.png" alt="Ввод пароля в MySQL Workbench CE" />
<p>Здесь надо ввести пароль, который был установлен для пользователя root при установке MySQL.</p>
<p>После успешного логина нам откроется содержимое сервера:</p>
<img src="pics/1.23.png" alt="Базы данных в MySQL Workbench CE" />
<p>В частности, в левой части в окне SCHEMAS можно увидеть доступные базы данных.</p>
<p>Теперь посмотрим, как мы можем выполнять в этой программе запросы к бд. 
Вначале создадим саму БД. Для этого нажмем над списком баз данных на значок "SQL" с плюсом:</p>
<img src="pics/1.25.png" alt="Создание скрипта в MySQL Workbench CE" />
...
<p>Данная команда создает базу данных usersdb.</p>
<p>Для выполнения скрипта в панели инструментов нажмем на значок молнии:</p>
<img src="pics/1.24.png" alt="Выполнение скрипта в MySQL Workbench CE" />
<p>После этого внизу программы в поле вывода в случае удачного выполнения мы увидим зеленый маркер и отчет о выполнении.</p>
<p>Таким образом, бд создана. Теперь добавим в нее таблицу и какие-нибудь данные. Для этого изменим код в поле ввода скрипта на следующий:</p>
<pre class="brush:sql;">
USE usersdb;
CREATE TABLE users (
...
<p>Все команды отделяются друг от друга точкой с запятой. Первая комнда - <code>USE</code> устанавливает в качестве используемой базу данных usersdb, которая 
была создана выше. Вторая команда - <code>CREATE TABLE</code> создает в бд таблицу users, в которой будет три столбца: id, firstname и age. 
Третья команда - <code>INSERT INTO</code> добавляет в таблицу users одну строку. Для выполнения этих команд также нажмем на значок молнии.</p>
<img src="pics/1.26.png" alt="Создание таблицы и добавление данных в MySQL Workbench CE" />
<p>И в конце получим все данные из таблицы users с помощью следующих команд:</p>
<pre class="brush:sql;">
USE usersdb;
SELECT * FROM users;
</pre>
<img src="pics/1.27.png" alt="Получение данных из БД в MySQL Workbench CE" />
<p>Таким образом, мы можем осуществлять запросы к БД в программе MySQL Workbench CE.</p>
