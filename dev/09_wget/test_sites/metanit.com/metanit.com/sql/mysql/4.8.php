...
<html >
<head>
<title>MySQL | Подзапросы в основных командах SQL</title>
<meta charset="utf-8" />
<meta name="description" content="Подзапросы в основных командах в MySQL">
...
<h2>Подзапросы в основных командах SQL</h2><div class="date">Последнее обновление: 22.05.2018</div>
...
<ol>
<li><p>В условии в выражении WHERE</p></li>
<li><p>В условии в выражении HAVING</p></li>
<li><p>В качестве таблицы для выборки в выражении FROM</p></li>
<li><p>В качестве спецификации столбца в выражении SELECT</p></li>
</ol>
...
<h4>Оператор IN</h4>
<p>Нередко подзапросы применяются вместе с оператором <span class="b">IN</span>, который выбирает из набора значений. И подзапрос как раз 
может предоставить требуемый набор значений. Например, выберем все товары из таблицы Products, на которые есть заказы в таблице Orders:</p>
<pre class="brush:sql;">
SELECT * FROM Products
WHERE Id IN (SELECT ProductId FROM Orders)
</pre>
<p>То есть подзапрос в данном случае выбирает все идентификаторы товаров из Orders, затем по этим идентификаторам извлекаютя товары из Products.</p>
<p>Добавив оператор <span class="b">NOT</span>, мы можем выбрать те товары, на которые нет заказов в таблице Orders:</p>
<pre class="brush:sql;">
SELECT * FROM Products
WHERE Id NOT IN (SELECT ProductId FROM Orders)
</pre>
<p>Стоит отметить, что это не самый эффективный способ для извлечения связанных данных из других таблиц, так как для сведения данных из разных таблиц 
можно использовать оператор JOIN, который рассматривается в следующей главе.</p>
...
<p>Допустим, если данный подзапрос возвращает значения vаl1, val2 и val3, то условие фильтрации фактически было бы аналогично 
...
<p>Как работает оператор ALL:</p>
<ul>
<li><p><code>x &gt; ALL (1, 2)</code> эквивалентно <code>x &gt; 2</code></p></li>
<li><p><code>x &lt; ALL (1, 2)</code> эквивалентно <code>x &lt; 1</code></p></li>
<li><p><code>x = ALL (1, 2)</code> эквивалентно <code>(x = 1) AND (x = 2)</code></p></li>
<li><p><code>x &lt;&gt; ALL (1, 2)</code> эквивалентно <code>x NOT IN (1, 2)</code></p></li>
</ul>
<p>Операторы ANY и SOME условие в операции сравнения должно быть истинным для хотя бы одного из значений, возвращаемых 
подзапросом. По своему действию оба этих оператора аналогичны, поэтому можно применять любой из них. 
Например, в следующем случае получим товары, которые стоят меньше самого дорогого товара компании Apple:</p>
<pre class="brush:sql;">
SELECT * FROM Products
...
<p>Как работает оператор ANY (а также SOME):</p>
<ul>
<li><p><code>x &gt; ANY (1, 2)</code> эквивалентно <code>x &gt; 1</code></p></li>
<li><p><code>x &lt; ANY (1, 2)</code> эквивалентно <code>x &lt; 2</code></p></li>
<li><p><code>x = ANY (1, 2)</code> эквивалентно <code>x IN (1, 2)</code></p></li>
<li><p><code>x &lt;&gt; ANY (1, 2)</code> эквивалентно <code>(x &lt;&gt; 1) OR (x &lt;&gt; 2)</code></p></li>
</ul>
...
<img src="pics/6.2.png" alt="Подзапросы в SELECT в MySQL" />
...
<p>В команде DELETE подзапросы также применяются как часть условия. Так, удалим все заказы на Galaxy S8:</p>
<pre class="brush:sql;">
DELETE FROM Orders
