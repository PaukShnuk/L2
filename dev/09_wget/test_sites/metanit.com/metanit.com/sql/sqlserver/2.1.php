...
<html >
<head>
<title>MS SQL Server и T-SQL | Начало работы</title>
<meta charset="utf-8" />
<meta name="description" content="Создание базы данных в SQL Server Management Studio, системные базы данных и их назначение">
...
<h1>Начало работы с MS SQL Server</h1><h2>Создание базы данных</h2><div class="date">Последнее обновление: 26.06.2017</div>
...
<p>Базу данных часто отождествляют с набором таблиц, которые хранят данные. Но это не совсем так. Лучше сказать, что база данных 
представляет хранилище объектов. Основные из них:</p>
<ul>
<li><p><span class="b">Таблицы</span>: хранят собственно данные</p></li>
<li><p><span class="b">Представления</span> (Views): выражения языка SQL, которые возвращают набор данных в виде таблицы</p></li>
<li><p><span class="b">Хранимые процедуры</span>: выполняют код на языке SQL по отношению к данным к БД (например, получает данные или изменяет их)</p></li>
<li><p><span class="b">Функции</span>: также код SQL, который выполняет определенную задачу</p></li>
</ul>
<p>В SQL Server используется два типа баз данных: системные и пользовательские. Системные базы данных необходимы серверу SQL для корректной работы. 
...
которые мы будем создавать и с которыми мы будем работать.</p>
<h3>Системные базы данных</h3>
<p>В MS SQL Server по умолчанию создается четыре системных баз данных:</p>
<ul>
<li><p><span class="b">master</span>: эта главная база данных сервера, в случае ее отсутствия или повреждения сервер не сможет работать. 
...
ряд другой информации.</p></li>
<li><p><span class="b">model</span>: эта база данных представляет шаблон, на основе которого создаются другие базы данных. То есть когда мы 
создаем через SSMS свою бд, она создается как копия базы model.</p></li>
<li><p><span class="b">msdb</span>: хранит информацию о работе, выполняемой таким компонентом как планировщик SQL. Также она хранит информацию о бекапах баз данных.</p></li>
<li><p><span class="b">tempdb</span>: эта база данных используется как хранилище для временных объектов. Она заново пересоздается при каждом запуске сервера.</p></li>
</ul>
<p>Все эти базы можно увидеть через SQL Server Management Studio в узле <span class="b">Databases -&gt; System Databases</span>:</p>
<img src="pics/18.png" alt="Системные базы данных в MS SQL Server 2016" />
<p>Эти базы данных не следует изменять, за исключением бд model.</p>
<p>Если на этапе установки сервера был выбран и установлен компонент PolyBase, то также на сервере по умолчанию будут расположены еще три базы данных, которые используется этим компонентом: 
DWConfiguration, DWDiagnostics, DWQueue.</p>
<h3>Создание базы данных в SQL Management Studio</h3>
<p>Теперь создадим свою базу данных. Для этого мы можем использовать скрипт на языке SQL, либо все сделать с помощью графических средств в 
...
на узел <span class="b">Databases</span>. Затем в появившемся контекстном меню выберем пункт <span class="b">New Database</span>:</p>
<img src="pics/19.png" alt="Создание базы данных в SQL Server Management Studio" />
<p>После этого нам открывается окно для создания базы данных:</p>
<img src="pics/20.png" alt="Определение базы данных в SQL Server Management Studio" />
<p>В поле <span class="b">Database</span> необходимо ввести название новой бд. Пусть у нас база данных называется <span class="b">university</span>.</p>
<p>Следующее поле <span class="b">Owner</span> задает владельца базы данных. По умолчанию оно имеет значение <span class="ii">&lt;defult&gt;</span>, то есть владельцем будет тот, кто создает эту базу данных. 
Оставим это поле без изменений.</p>
<p>Далее идет таблица для установки общих настроек базы данных. Она содержит две строки - первая для установки настроек для главного файла, где будут храниться данные, и 
вторая строка для конфигурации файла логгирования. В частности, мы можем установить следующие настройки:</p>
<ul>
<li><p><span class="b">Logical Name</span>: логическое имя, которое присваивается файлу базы данных.</p></li>
<li><p><span class="b">File Type</span>: есть несколько типов файлов, но, как правило, основная работа ведется с файлами данных 
(ROWS Data) и файлом лога (LOG)</p></li>
<li><p><span class="b">Filegroup</span>: обозначет группу файлов. Группа файлов может хранить множество файлов 
и может использоваться для разбиения базы данных на части для размещения в разных местах.</p></li>
<li><p><span class="b">Initial Size (MB)</span>: устанавливает начальный размер файлов при создании (фактический размер может отличаться от этого значения).</p></li>
<li><p><span class="b">Autogrowth/Maxsize</span>: при достижении базой данных начального размера SQL Server использует это значение 
для увеличения файла.</p></li>
<li><p><span class="b">Path</span>: каталог, где будут храниться базы данных.</p></li>
<li><p><span class="b">File Name</span>: непосредственное имя физического файла. Если оно не указано, то применяется логическое имя.</p></li>
</ul>
<p>После ввода названия базы данных нажмем на кнопку ОК, и бд будет создана.</p>
<p>После этого она появится среди баз данных сервера. Если эта бд впоследствии не потребуется, то ее можно удалить, нажав на нее 
правой кнопкой мыши и выбрав в контекстном меню пункт Delete:</p>
<img src="pics/21.png" alt="Удаление базы данных в MS SQL Server" />
