...
<html >
<head>
<title>MS SQL Server и T-SQL | Триггер INSTEAD OF</title>
<meta charset="utf-8" />
<meta name="description" content="Триггер INSTEAD OF в T-SQL в MS SQL Server, изменение результатов операции">
...
<h2>Триггер INSTEAD OF</h2><div class="date">Последнее обновление: 09.11.2017</div>
...
<p>Триггер INSTEAD OF срабатывает вместо операции с данными. Он определяется в принципе также, как триггер AFTER, за тем исключением, 
что он может определяться только для одной операции - INSERT, DELETE или UPDATE. И также он может применяться как для таблиц, так и для представлений (триггер AFTER применяется только для таблиц).</p>
<p>Например, создадим следующие базу данных и таблицу:</p>
<pre class="brush:sql;">
...
<p>Здесь таблица содержит столбец IsDeleted, который указывает, удалена ли запись. То есть вместо жесткого удаления полностью из базы данных 
мы хотим выполнить мягкое удаление, при котором запись остается в базе данных.</p>
<p>Определим триггер для удаления записи:</p>
<pre class="brush:sql;">
USE prods
GO
...
<p>Добавим некоторые данные в таблицу и выполним удаление из нее:</p>
<pre class="brush:sql;">
USE prods;
...
<p>Таким образом, удаляемые записи на самом деле не будут удаляться, просто у них будет устанавливаться значение для столбца IsDeleted:</p>
<img src="pics/139.png" alt="Trigger INSTEAD OF в T-SQL" />
...
<div class="nav"><p><a href="12.2.php">Назад</a><a href="./">Содержание</a></p></div></div>
<div class="socBlock">
