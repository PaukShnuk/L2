...
<html >
<head>
<title>MS SQL Server и T-SQL | Извлечение диапазона строк</title>
<meta charset="utf-8" />
<meta name="description" content="Извлечение диапазона строк в T-SQL в MS SQL Server 2016, оператор TOP, OFFSET, FETCH">
...
<h2>Извлечение диапазона строк</h2><div class="date">Последнее обновление: 13.07.2017</div>
...
<h3>Оператор TOP</h3>
<p>Оператор <span class="b">TOP</span> позволяет выбрать определенное количество строк из таблицы:</p>
<pre class="brush:sql;">
SELECT TOP 4 ProductName
FROM Products
</pre>
<img src="pics/44.png" alt="SELECT TOP в T-SQL" />
<p>Дополнительный оператор <span class="b">PERCENT</span> позволяет выбрать процентное количество строк из таблицы. 
Например, выберем 75% строк:</p>
<pre class="brush:sql;">
SELECT TOP 75 PERCENT ProductName
FROM Products
</pre>
<h3>OFFSET и FETCH</h3>
<p>Оператор TOP позволяет извлечь определенное количество строк, начиная с начала таблицы. Для извлечения набора строк из любого места, применяются операторы 
<span class="b">OFFSET</span> и <span class="b">FETCH</span>. Важно, что эти операторы применяются только в отсортированном наборе данных после выражения ORDER BY.</p>
<pre class="brush:sql;">
ORDER BY выражение 
	OFFSET смещение_относительно_начала {ROW|ROWS}
	[FETCH {FIRST|NEXT} количество_извлекаемых_строк {ROW|ROWS} ONLY]
</pre>
<p>Например, выберем все строки, начиная с третьей:</p>
<pre class="brush:sql;">
SELECT * FROM Products
ORDER BY Id 
	OFFSET 2 ROWS;
</pre>
<p>Число после ключевого слова OFFSET указывает, сколько строк необходимо пропустить.</p>
<img src="pics/55.png" alt="ORDER BY и OFFSET в T-SQL" />
<p>Теперь выберем только три строки, начиная с третьей:</p>
<pre class="brush:sql;">
SELECT * FROM Products
ORDER BY Id 
	OFFSET 2 ROWS
...
<p>После оператора FETCH указывается ключевое слово <span class="b">FIRST</span> или <span class="b">NEXT</span> (какое именно в данном случае не имеет значения) и 
затем указывается количество строк, которое надо получить.</p>
<img src="pics/56.png" alt="OFFSET и FETCH в T-SQL и постраничная навигация" />
<p>Данная комбинация операторов, как правило, используется для постраничной навигации, когда необходимо получить определенную страницу с данными.</p>
