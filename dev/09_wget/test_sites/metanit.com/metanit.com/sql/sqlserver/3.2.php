...
<html >
<head>
<title>MS SQL Server и T-SQL | Создание и удаление таблиц</title>
<meta charset="utf-8" />
<meta name="description" content="Создание, удаление и переименование таблиц в базе данных MS SQL Server 2016 с помощью языка T-SQL, CREATE TABLE, определение столбцов">
...
<h2>Создание и удаление таблиц</h2><div class="date">Последнее обновление: 09.07.2017</div>
...
<p>Для создания таблиц применяется команда <span class="b">CREATE TABLE</span>. С этой командой можно использовать ряд операторов, которые определяют столбцы таблицы и их атрибуты. 
И кроме того, можно использовать ряд операторов, которые определяют свойства таблицы в целом. Одна база данных может содержать до 2 миллиардов таблиц.</p>
<p>Общий синтаксис создания таблицы выглядит следующим образом:</p>
<pre class="brush:sql;">
...
<p>После команды <code>CREATE TABLE</code> идет название создаваемой таблицы. Имя таблицы выполняет роль ее идентификатора в базе данных, 
...
символов $ и знака подчеркивания. Причем первым символом должна быть буква или знак подчеркивания.</p>
<p>Имя объекта не может включать пробелы и не может представлять одно из ключевых слов языка Transact-SQL. Если идентификатор все же содержит пробельные символы, то его следует заключать в кавычки. 
Если необходимо в качестве имени использовать ключевые слова, то эти слова помещаются в квадратные скобки.</p>
<p>Примеры корректных идентификаторов:</p>
<pre class="brush:sql;">
Users
tags$345
users_accounts
...
<p>После имени таблицы в скобках указываются параметры всех столбцов и в самом конце атрибуты, которые относятся ко 
всей таблице. Атрибуты столбцов и атрибуты таблицы являются необязательными компонентами, и их можно не указывать.</p>
<p>В самом просто виде команда CREATE TABLE должна содержать как минимум имя таблицы, имена и типы столбцов.</p>
<p>Таблица может содержать от 1 до 1024 столбцов. Каждый столбец должен иметь уникальное в рамках текущей таблицы имя, и ему должен быть назначен тип данных.</p>
<p>Например, определение простейшей таблицы Customers:</p>
<pre class="brush:sql;">
...
<p>В данном случае в таблице Customers определяются шесть столбцов: Id, FirstName, LastName, Age, Email, Phone. Первые два столбца представляют идентификатор клиента и его возраст и имеют 
тип <span class="b">INT</span>, то есть будут хранить числовые значения. Следующие два столбца представляют имя и фамилию клиента и имеют тип 
<span class="b">NVARCHAR(20)</span>, то есть представляют строку UNICODE длиной не более 20 символов. Последние два столбца Email и Phone представляют адрес электронной почты и телефон клиента и имеют тип 
<span class="b">VARCHAR(30/20)</span> - они также хранят строку, но не в кодировке UNICODE.</p>
<h3>Создание таблицы в SQL Management Studio</h3>
<p>Создадим простую таблицу на сервере. Для этого откроем SQL Server Management Studio и нажмем правой кнопкой мыши на название сервера. В появившемся 
контекстном меню выберем пункт <span class="b">New Query</span>.</p>
<img src="pics/32.png" alt="Создание таблицы в SQL Server Management Studio" />
<p>Таблица создается в рамках текущей базы данных. Если мы запускаем окно редактора SQL как это сделано выше - из под названия сервера, то база данных по умолчанию не установлена. 
И для ее установки необходимо применить команду <span class="b">USE</span>, после которой указывается имя базы данных. Поэтому введем в поле редактора 
SQL-команд следующие выражения:</p>
<pre class="brush:sql;">
USE usersdb;
...
<p>То есть в базу данных добавляется таблица Customers, которая была рассмотрена ранее.</p>
<p>Также можно открыть редактор из под базы данных, также нажав на нее правой кнопкой мыши и выбрав New Query:</p>
<img src="pics/33.png" alt="Создание таблицы в MS SQL Server 2016" />
<p>В этом случае в качестве текущей будет рассматриваться та база данных, из под которой был открыт редактор, и дополнительно ее устанавливать с помощью команды USE 
не потребуется.</p>
...
<h3>Переименование таблицы</h3>
<p>Для переименования таблиц применяется системная хранимая процедура "sp_rename". Например, переименование таблицы Users в UserAccounts в базе данных usersdb:</p>
<pre class="brush:sql;">
USE usersdb;
