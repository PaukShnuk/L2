...
<html >
<head>
<title>MS SQL Server и T-SQL | Табличные переменные</title>
<meta charset="utf-8" />
<meta name="description" content="Табличные переменные в T-SQL в MS SQL Server, определение переменных, которые хранят таблицы">
...
<h2>Табличные переменные</h2><div class="date">Последнее обновление: 14.08.2017</div>
...
<p>Табличные переменные (table variable) позволяют сохранить содержимое целой таблицы. Формальный синтаксис определения подобной переменной 
во многом похож на создание таблицы:</p>
<pre class="brush:sql;">
...
<p>В данном случае переменная @ABrends будет содержать два столбца.</p>
<p>В дальнейшем мы сможем работать с этой переменной как с обычной таблицей, то есть добавлять в нее данные, изменять, удалять и извлекать их:</p>
<pre class="brush:sql;">
DECLARE @ABrends TABLE (ProductId INT,  ProductName NVARCHAR(20))
...
<img src="pics/118.png" alt="табличные переменные в T-SQL и MS SQL Server" />
<p>Однако следует учитывать, что такие переменные не полностью эквивалентны таблицам. Они живут в пределах одного пакета, после завершения работы которого 
они удаляются.  То есть они носят временный характер, и физически их данные нигде не хранятся на жестком диске.</p>
