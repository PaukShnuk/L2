...
<html >
<head>
<title>MS SQL Server и T-SQL | Сортировка. ORDER BY</title>
<meta charset="utf-8" />
<meta name="description" content="Сортировка по возрастанию и убыванию в T-SQL в MS SQL Server 2016, оператор ORDER BY">
...
<h2>Сортировка. ORDER BY</h2><div class="date">Последнее обновление: 13.07.2017</div>
...
<p>Оператор <span class="b">ORDER BY</span> позволяет отсортировать извлекаемые значения по определенному столбцу:</p>
<pre class="brush:sql;">
SELECT *
FROM Products
ORDER BY ProductName
</pre>
<p>В данном случае строки сортируются по возрастанию значения столбца ProductName:</p>
<img src="pics/40.png" alt="ORDER BY и сортировка в T-SQL" />
<p>Сортировку также можно проводить по псевдониму столбца, который определяется с помощью оператора AS:</p>
<pre class="brush:sql;">
SELECT ProductName, ProductCount * Price AS TotalSum
FROM Products
ORDER BY TotalSum
</pre>
<img src="pics/41.png" alt="ORDER BY и сортировка в MS SQL Server 2016" />
<p>По умолчанию применяется сортировка по возрастанию. С помощью дополнительного оператора <span class="b">DESC</span> можно задать сортировку по убыванию.</p>
<pre class="brush:sql;">
SELECT ProductName
FROM Products
ORDER BY ProductName DESC
</pre>
<img src="pics/42.png" alt="ORDER BY DESC и сортировка по убыванию в T-SQL" />
<p>По умолчанию вместо DESC используется оператор <span class="b">ASC</span>:</p>
<pre class="brush:sql;">
SELECT ProductName
FROM Products
ORDER BY ProductName ASC
</pre>
<p>Если необходимо отсортировать сразу по нескольким столбцам, то все они перечисляются после оператора ORDER BY:</p>
<pre class="brush:sql;">
SELECT ProductName, Price, Manufacturer
FROM Products
ORDER BY Manufacturer, ProductName
</pre>
<p>В этом случае сначала строки сортируются по столбцу Manufacturer по возрастанию. Затем если есть две строки, в которых столбец Manufacturer имеет одинаковое значение, то 
они сортируются по столбцу ProductName также по возрастанию. Но опять же с помощью ASC и DESC можно отдельно для разных столбцов определить сортировку по возрастанию и 
убыванию:</p>
<pre class="brush:sql;">
SELECT ProductName, Price, Manufacturer
FROM Products
...
<img src="pics/43.png" alt="Сортировка по нескольким столбцам в T-SQL" />
...
<img src="pics/54.png" alt="Сортировка по сложному выражению в ORDER BY в T-SQL" />
