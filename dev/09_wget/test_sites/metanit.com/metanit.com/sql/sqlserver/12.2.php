...
<html >
<head>
<title>MS SQL Server и T-SQL | Триггеры для операций INSERT, UPDATE, DELETE</title>
<meta charset="utf-8" />
<meta name="description" content="Создание триггеров для операций INSERT, UPDATE, DELETE в T-SQL в MS SQL Server">
...
<h2>Триггеры для операций INSERT, UPDATE, DELETE</h2><div class="date">Последнее обновление: 09.11.2017</div>
...
<p>Для рассмотрения операций с триггерами определим следующую базу данных productsdb:</p>
<pre class="brush:sql;">
...
<p>Здесь определены две таблиц: Products - для хранения товаров и History - для хранения истории операций с товарами.</p>
<h3>Добавление</h3>
<p>При добавлении данных (при выполнении команды <span class="b">INSERT</span>) в триггере мы можем получить добавленные данные из 
виртуальной таблицы <span class="b">INSERTED</span>.</p>
<p>Определим триггер, который будет срабатывать после добавления:</p>
<pre class="brush:sql;">
USE productsdb
GO
...
<p>Этот триггер будет добавлять в таблицу History данные о добавлении товара, которые берутся из виртуальной таблицы INSERTED.</p>
<p>Выполним добавление данных в Products и получим данные из таблицы History:</p>
<pre class="brush:sql;">
USE productsdb;
...
<img src="pics/136.png" alt="INSERT Trigger in MS SQL Server" />
<h3>Удаление данных</h3>
<p>При удалении все удаленные данные помещаются в виртуальную таблицу <span class="b">DELETED</span>:</p>
<pre class="brush:sql;">
USE productsdb
GO
...
<p>Здесь, как и в случае с предыдущим триггером, помещаем информацию об удаленных товарах в таблицу History.</p>
<p>Выполним команду на удаление:</p>
<pre class="brush:sql;">
USE productsdb;
...
<img src="pics/137.png" alt="DELETE Trigger in MS SQL Server" />
<h3>Изменение данных</h3>
<p>Триггер обновления данных срабатывает при выполнении операции UPDATE. И в таком триггере мы можем использовать две виртуальных таблицы. 
Таблица INSERTED хранит значения строк после обновления, а таблица DELETED хранит те же строки, но до обновления.</p>
<p>Создадим триггер обновления:</p>
<pre class="brush:sql;">
USE productsdb
GO
...
<p>И при обновлении данных сработает данный триггер:</p>
<img src="pics/138.png" alt="UPDATE Trigger in MS SQL Server" />
