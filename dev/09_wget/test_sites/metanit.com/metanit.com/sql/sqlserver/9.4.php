...
<html >
<head>
<title>MS SQL Server и T-SQL | Циклы</title>
<meta charset="utf-8" />
<meta name="description" content="Циклы в T-SQL в MS SQL Server, конструкция WHILE, вычисление факториала">
...
<h2>Циклы</h2><div class="date">Последнее обновление: 14.08.2017</div>
...
<p>Для выполнения повторяющихся операций в T-SQL применяются циклы. В частности, в T-SQL есть цикл <span class="b">WHILE</span>. 
Этот цикл выполняет определенные действия, пока некоторое условие истинно.</p>
<pre class="brush:sql;">
WHILE условие
	{инструкция|BEGIN...END}
</pre>
<p>Если в блоке WHILE необходимо разместить несколько инструкций, то все они помещаются в блок BEGIN...END.</p>
<p>Например, вычислим факториал числа:</p>
<pre class="brush:sql;">
...
<p>То есть в данном случае пока переменная @number не будет равна 0, будет продолжаться цикл WHILE. Так как @number равна 5, то цикл сделает пять 
проходов. Каждый проход цикла называется итерацией. В каждой итерации будет переустанавливаться значение переменных @factorial и @number.</p>
<img src="pics/125.png" alt="Факториал в T-SQL" />
<p>Другой пример - рассчитаем баланс счета через несколько лет с учетом процентной ставки:</p>
<pre class="brush:sql;">
USE productsdb;
...
<p>Здесь создается временная таблица #Accounts, в которую добавляется в цикле пять строк с данными.</p>
<img src="pics/126.png" alt="WHILE в T-SQL и MS SQL Server" />
<h3>Операторы BREAK и CONTINUE</h3>
<p>Оператор BREAK позволяет завершить цикл, а оператор CONTINUE - перейти к новой итерации.</p>
<pre class="brush:sql;">
DECLARE @number INT
SET @number = 1
...
<p>Когда переменная @number станет равна 4, то с помощью оператора CONTINUE произойдет переход к новой итерации, поэтому последующая строка <code>PRINT 'Конец итерации'</code> 
не будет выполняться, хотя цикл продолжится.</p>
<p>Когда переменная @number станет равна 7, то оператор BREAK произведет выход из цикла, и он завершится.</p>
<img src="pics/127.png" alt="BREAK и CONTINUE в T-SQL и MS SQL Server" />
