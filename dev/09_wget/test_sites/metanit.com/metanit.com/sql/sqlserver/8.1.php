...
<html >
<head>
<title>MS SQL Server и T-SQL | Функции для работы со строками</title>
<meta charset="utf-8" />
<meta name="description" content="Функции для работы со строками в T-SQL в MS SQL Server, LEN, RTRIM, LTRIM, LOWER, UPPER, REPLACE, REVERSE">
...
<h1>Встроенные функции</h1><h2>Функции для работы со строками</h2><div class="date">Последнее обновление: 29.07.2017</div>
...
<p>Для работы со строками в T-SQL можно применять следующие функции:</p>
<ul>
<li><p><span class="b">LEN</span>: возвращает количество символов в строке. В качестве параметра в функцию передается строка, для которой надо найти длину:</p>
<pre class="brush:sql;">SELECT LEN('Apple')  -- 5</pre>
</li>
...
<li><p><span class="b">CHARINDEX</span>: возвращает индекс, по которому находится первое вхождение подстроки в строке. В качестве 
первого параметра передается подстрока, а в качестве второго - строка, в которой надо вести поиск:</p>
<pre class="brush:sql;">SELECT CHARINDEX('pl', 'Apple')	-- 3</pre>
</li>
<li><p><span class="b">PATINDEX</span>: возвращает индекс, по которому находится первое вхождение определенного шаблона в строке:</p>
<pre class="brush:sql;">SELECT PATINDEX('%p_e%', 'Apple')	-- 3</pre></li>
...
<li><p><span class="b">SUBSTRING</span>: вырезает из строки подстроку определенной длиной, начиная с определенного индекса. 
Певый параметр функции - строка, второй - начальный индекс для вырезки, и третий параметр - количество вырезаемых символов:</p>
<pre class="brush:sql;">SELECT SUBSTRING('Galaxy S8 Plus', 8, 2)	-- S8</pre></li>
...
<li><p><span class="b">CONCAT</span>: объединяет две строки в одну. В качестве параметра принимает от 2-х и более строк, которые надо соединить:</p>
<pre class="brush:sql;">SELECT CONCAT('Tom', ' ', 'Smith')	-- Tom Smith</pre></li>
...
<img src="pics/103.png" alt="Строковые функции в T-SQL и MS SQL Server" />
