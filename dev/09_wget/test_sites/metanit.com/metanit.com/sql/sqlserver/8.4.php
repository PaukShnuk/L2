...
<html >
<head>
<title>MS SQL Server и T-SQL | Преобразование данных</title>
<meta charset="utf-8" />
<meta name="description" content="Преобразование данных в T-SQL в MS SQL Server, функции CAST и CONVERT">
...
<p>Когда мы присваиваем значение одного типа столбцу, который хранит данные другого типа, либо выполняем операции, которые вовлекают данные разных типов, 
...
SQL Server может выполнять неявные преобразования от типа с меньшим приоритетом к типу с большим приоритетом. Таблица приоритетов (чем выше, тем больший приоритет):</p>
<table>
...
<p>В тех случаях, когда необходимо выполнить преобразования от типов с высшим приоритетом к типам с низшим приоритетом, то надо выполнять явное приведение типов. 
...
<pre class="brush:sql;">CAST(выражение AS тип_данных)</pre>
...
<img src="pics/105.png" alt="Функция CAST в T-SQL и MS SQL Server" />
<h3>Convert</h3>
<p>Большую часть преобразований охватывает функция CAST. Если же необходимо какое-то дополнительное форматирование, то можно использовать функцию <span class="b">CONVERT</span>. Она имеет следующую форму:</p>
<pre class="brush:sql;">CONVERT(тип_данных, выражение [, стиль])</pre>
<p>Третий необязательный параметр задает стиль форматирования данных. Этот параметр представляет числовое значение, которое для разных типов данных имеет разную интерпретацию. 
...
<img src="pics/106.png" alt="Функция Convert в T-SQL и MS SQL Server" />
<h3>TRY_CONVERT</h3>
<p>При использовании функций CAST и CONVERT SQL Server выбрасывает исключение, если данные нельзя привести к определенному типу. Например:</p>
<pre class="brush:sql;">SELECT CONVERT(int, 'sql')</pre>
...
<ul>
<li><p><span class="b">STR(float [, length [,decimal]])</span>: преобразует число в строку. Второй параметр указывает на длину строки, а третий - сколько знаков в дробной части числа надо оставлять</p></li>
<li><p><span class="b">CHAR(int)</span>: преобразует числовой код ASCII в символ. Нередко используется для тех ситуаций, когда необходим символ, который нельзя ввести с клавиатуры</p></li>
