...
<html >
<head>
<title>MS SQL Server и T-SQL | Определение триггеров</title>
<meta charset="utf-8" />
<meta name="description" content="Создание и удаление триггеров в T-SQL в MS SQL Server, типы триггеров AFTER и INSTEAD OF">
...
<h1>Триггеры</h1><h2>Определение триггеров</h2><div class="date">Последнее обновление: 09.11.2017</div>
...
<p>Триггеры представляют специальный тип хранимой процедуры, которая вызывается автоматически при выполнении определенного действия 
над таблицей или представлением, в частности, при добавлении, изменении или удалении данных, то есть при выполнении команд INSERT, UPDATE, DELETE.</p>
<p>Формальное определение триггера:</p>
<pre class="brush:sql;">
...
<p>Для создания триггера применяется выражение <span class="b">CREATE TRIGGER</span>, после которого идет имя триггера. 
Как правило, имя триггера отражает тип операций и имя таблицы, над которой производится операция.</p>
<p>Каждый триггер ассоциируется с определенной таблицей или представлением, имя которых указывается после слова <span class="b">ON</span>.</p>
<p>Затем устанавливается тип триггера. Мы можем использовать один из двух типов:</p>
<ul>
<li><p><span class="b">AFTER</span>: выполняется после выполнения действия. Определяется только для таблиц.</p></li>
<li><p><span class="b">INSTEAD OF</span>: выполняется вместо действия (то есть по сути действие - добавление, изменение или удаление - вообще не выполняется). Определяется для таблиц и представлений</p></li>
</ul>
<p>После типа триггера идет указание операции, для которой определяется триггер: <span class="b">INSERT</span>, <span class="b">UPDATE</span> или 
<span class="b">DELETE</span>.</p>
<p>Для триггера AFTER можно применять сразу для нескольких действий, например, UPDATE и INSERT. В этом случае операции 
указываются через запятую. Для триггера INSTEAD OF можно определить только одно действие.</p>
<p>И затем после слова AS идет набор выражений SQL, которые собственно и составляют тело триггера.</p>
<p>Создадим триггер. Допустим, у нас есть база данных productsdb со следующим определением:</p>
<pre class="brush:sql;">
...
<p>Определим триггер, который будет срабатывать при добавлении и обновлении данных:</p>
<pre class="brush:sql;">
USE productdb;
GO
...
<p>Допустим, в таблице Products хранятся данные о товарах. Но цена товара нередко содержит различные надбавки типа налога на добавленную 
...
чистую цену. С помощью триггера мы можем поправить цену товара на некоторую величину.</p>
<p>Таким образом, триггер будет срабатывать при любой операции INSERT или UPDATE над таблицей Products. Сам триггер будет изменять цену товара, 
...
товарах.</p>
<p>И после добавления товара в таблицу Products в реальности товар будет иметь несколько большую цену, чем та, которая была определена при добавлении:</p>
<img src="pics/135.png" alt="Триггеры в MS SQL Server" />
<h3>Удаление триггера</h3>
<p>Для удаления триггера необходимо применить команду <span class="b">DROP TRIGGER</span>:</p>
<pre class="brush:sql;">DROP TRIGGER Products_INSERT_UPDATE</pre>
<h3>Отключение триггера</h3>
<p>Бывает, что мы хотим приостановить действие триггера, но удалять его полностью не хотим. В этом случае его можно временно отключить с 
помощью команды <span class="b">DISABLE TRIGGER</span>:</p>
<pre class="brush:sql;">DISABLE TRIGGER Products_INSERT_UPDATE ON Products</pre>
<p>А когда триггер понадобится, его можно включить с помощью команды <span class="b">ENABLE TRIGGER</span>:</p>
<pre class="brush:sql;">ENABLE TRIGGER Products_INSERT_UPDATE ON Products</pre>
...
<div class="nav"><p><a href="11.3.php">Назад</a><a href="./">Содержание</a><a href="12.2.php">Вперед</a></p></div></div>
<div class="socBlock">
