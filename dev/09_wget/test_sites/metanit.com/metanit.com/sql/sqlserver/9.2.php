...
<html >
<head>
<title>MS SQL Server и T-SQL | Переменные в запросах</title>
<meta charset="utf-8" />
<meta name="description" content="Переменные в запросах в T-SQL в MS SQL Server, передача значения переменной в запрос и получения из запроса значения в переменную">
...
<h2>Переменные в запросах</h2><div class="date">Последнее обновление: 14.08.2017</div>
...
<p>Через переменные мы можем передавать данные в запросы. И также мы можем получать данные, которые являются результатом запросов, в переменные. 
Например, при выборке из таблиц с помощью команды <span class="b">SELECT</span> мы можем извлекать данные в переменную с помощью следующего синтаксиса:</p>
<pre class="brush:sql;">
...
<p>Кроме того, в выражении <span class="b">SET</span> значение, присваиваемое переменной, также может быть результатом команды SELECT.</p>
<p>Например, пусть у нас будут следующие таблицы:</p>
<pre class="brush:sql;">
...
<p>Используем переменные при извлечении данных:</p>
<pre class="brush:sql;">
...
<p>В данном случае переменная @count будет содержать сумму всех значений из столбца ProductCount таблицы Orders, то есть общее количество проданных товаров.</p>
<p>Переменные @min и @max хранят соответственно минимальное и максимальное значения столбца Price из таблицы Products, а переменная 
@dif - разницу между этими значениями. И подобно простым значениям, переменные также могут участвовать в операциях.</p>
<img src="pics/116.png" alt="Переменные в запросах в T-SQL" />
<p>Другой пример:</p>
<pre class="brush:sql;">
...
<p>Здесь извлекаемые данные из двух таблиц Products и Orders группируются по столбцам Id и ProductName из таблицы Products. Затем данные 
фильтруются по столбцу Id из Products. А извлеченные данные попадают в переменные @sum, @name, @prodid.</p>
<img src="pics/117.png" alt="Переменные в запросах SELECT в MS SQL Server" />
