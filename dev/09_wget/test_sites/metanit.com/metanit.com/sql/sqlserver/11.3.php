...
<html >
<head>
<title>MS SQL Server и T-SQL | Выходные параметры хранимых процедур</title>
<meta charset="utf-8" />
<meta name="description" content="Выходные параметры в хранимых процедурах в T-SQL в MS SQL Server, получение результата процедуры и оператор RETURN">
...
<h2>Выходные параметры и возвращение результата</h2><div class="date">Последнее обновление: 14.08.2017</div>
...
<p>Выходные параметры позволяют возвратить из процедуры некоторый результат. Выходные параметры определяются с помощью ключевого слова <span class="b">OUTPUT</span>. Например, определим еще одну процедуру:</p>
<pre class="brush:sql;">
USE productsdb;
GO
...
<p>При вызове процедуры для выходных параметров передаются переменные с ключевым словом OUTPUT:</p>
<pre class="brush:sql;">
USE productsdb;
...
<img src="pics/133.png" alt="Выходные параметры в процедурах в MS SQL Server" />
<p>Также можно сочетать входные и выходные параметры. Например, определим процедуру, которая добавляет новую строку в таблицу и возвращает ее id:</p>
<pre class="brush:sql;">
USE productsdb;
GO
...
<p>С помощью глобальной переменной <span class="b">@@IDENTITY</span> можно получить идентификатор добавленной записи.</p>
<p>При вызове этой процедуры ей также по позиции передаются все входные и выходные параметры:</p>
<pre class="brush:sql;">
USE productsdb;
...
<p>Кроме передачи результата выполнения через выходные параметры хранимая процедура также может возвращать какое-либо значение типа <span class="b">INT</span> с помощью 
оператора <span class="b">RETURN</span>. Хотя данная возможность во многом нивелирована использованием выходных параметров, через которые можно возвращать результат, 
тем не менее, если надо возвратить из процедуры одно значение, то вполне можно использовать оператор RETURN.</p>
<p>Например, возвратим среднюю цену на товары:</p>
<pre class="brush:sql;">
USE productsdb;
GO
...
<p>После оператора RETURN указывается возвращаемое значение. В данном случае это значение переменной @avgPrice.</p>
<p>Вызовем данную процедуру:</p>
<pre class="brush:sql;">
USE productsdb;
...
<p>Для получения результата процедуры ее значение сохраняется в переменную (в данном случае в переменную @result):</p>
<img src="pics/134.png" alt="Оператор RETURN в хранимых процедурах в MS SQL Server" />
<p>Стоит отметить, что <code>RETURN</code> возвращает только целочисленные значения.</p>
