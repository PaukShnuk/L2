...
<html >
<head>
<title>MS SQL Server и T-SQL | Оператор EXISTS</title>
<meta charset="utf-8" />
<meta name="description" content="Оператор EXISTS и проверка на существование T-SQL в MS SQL Server 2016">
...
<h2>Оператор EXISTS</h2><div class="date">Последнее обновление: 20.07.2017</div>
...
<p>Оператор <span class="b">EXISTS</span> позволяет проверить, возвращает ли подзапрос какое-либо значение. Как правило, этот 
оператор используется для индикации того, что какая-либо строка удовлетворяет условию. То есть фактически оператор EXISTS не возвращает строки, а лишь указывает, что в 
базе данных есть как минимум одна строка, которые соответствует данному запросу. Поскольку возвращения набора строк не 
...
<p>Например, найдем всех покупателей из таблицы Customer, которые делали заказы:</p>
<pre class="brush:sql;">
SELECT *
FROM Customers
...
<img src="pics/99.png" alt="Оператор EXISTS в T-SQL" />
<p>Другой пример - найдем все товары из таблицы Products, на которые не было заказов в таблице Orders:</p>
<pre class="brush:sql;">
SELECT *
FROM Products
WHERE NOT EXISTS (SELECT * FROM Orders WHERE Products.Id = Orders.ProductId)
</pre>
<img src="pics/98.png" alt="Оператор EXISTS в T-SQL" />
<p>Стоит отметить, что для получения подобного результата ы могли бы использовать и опеатор <span class="b">IN</span>:</p>
<pre class="brush:sql;">
SELECT *
FROM Products
