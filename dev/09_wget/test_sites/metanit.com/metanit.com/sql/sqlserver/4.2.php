...
<html >
<head>
<title>MS SQL Server и T-SQL | Выборка данных. Команда SELECT</title>
<meta charset="utf-8" />
<meta name="description" content="Выборка данных в T-SQL в MS SQL Server 2016, команда SELECT, оператор DISTINCT и AS, псевдонимы столбцов">
...
<h2>Выборка данных. Команда SELECT</h2><div class="date">Последнее обновление: 13.07.2017</div>
...
<p>Для получения данных применяется команда <span class="b">SELECT</span>. В упрощенном виде она имеет следующий синтаксис:</p>
...
<img src="pics/36.png" alt="SELECT и выборка из базы данных MS SQL Server" />
<p>Получение всех столбцов с помощью символа звездочки * считается не очень хорошей практикой, так как, как правило, не все столбцы бывают нужны. И более 
...
<img src="pics/37.png" alt="SELECT и выборка по столбцам из базы данных MS SQL Server" />
...
<p>Здесь при выборке будут создаваться три столбца. Первый столбец представляет результат объединения двух столбцов ProductName и 
Manufacturer. Второй столбец - стандартный столбец Price. А третий столбец представляет значение столбца Price, умноженное на значение столбца ProductCount.</p>
<img src="pics/38.png" alt="SELECT и анонимные столбцы в T-SQL" />
...
<p>В данном случае результатом выборки являются данные по 3-м столбцам. Первый столбец ModelName объединяет столбцы 
...
ProductCount и Price. При этом, как в случае со столбцом Price, необязательно определять название результирующего столбца с помощью AS.</p>
<img src="pics/39.png" alt="Оператор AS в T-SQL и псевдонимы столбцов" />
<h3>DISTINCT</h3>
<p>Оператор <span class="b">DISTINCT</span> позволяет выбрать уникальные строки. Например, в нашем случае в таблице может быть по несколько товаров 
от одних и тех же производителей. Выберем всех производителей:</p>
<pre class="brush:sql;">
SELECT DISTINCT Manufacturer
FROM Products
</pre>
<img src="pics/45.png" alt="SELECT DISTINCT в T-SQL" />
<p>В данном случае критерием разграничения строк является столбец Manufacturer. Поэтому в результирующей выборке будут только уникальные 
значения Manufacturer. И если, к примеру, в базе данных есть два товара с производителем Apple, то это название будет встречаться в результирующей выборке только один раз.</p>
<h3>Выборка с добавлением</h3>
<h4>SELECT INTO</h4>
<p>Выражение <span class="b">SELECT INTO</span> позволяет выбрать из одной таблицы некоторые данные в другую таблицу, при этом вторая таблица создается автоматически. 
Например:</p>
<pre class="brush:sql;">
...
<p>После выполнения этой команды в базе данных будет создана еще одна таблица ProductSummary, которая будет иметь два столбца 
ModelName и Price, а данные для этих столбцов будут взяты из таблицы Products:</p>
<img src="pics/57.png" alt="SELECT INTO в T-SQL" />
<p>При выполнении этой команды таблица, в которую идет выборка (в данном случае ProductSummary), не должна существовать в базе данных.</p>
<p>Но, допустим, мы потом решили добавить все данные из таблицы Products в уже существующую таблицу ProductSummary. 
В этом случае можно опять же использовать команду <span class="b">INSERT</span>:</p>
<pre class="brush:sql;">
INSERT INTO ProductSummary
SELECT ProductName + ' (' + Manufacturer + ')' AS ModelName, Price
FROM Products
</pre>
<p>Здесь добавляемые значения фактически представляют результат выборки из таблицы Products.</p>
