...
<html >
<head>
<title>MS SQL Server и T-SQL | Внешние ключи</title>
<meta charset="utf-8" />
<meta name="description" content="Внешние ключи в MS SQL Server 2016, их определение через T-SQL, FOREIGN KEY REFERENCES, выражения ON DELETE и ON UPDATE, каскадное удаление">
...
<h2>Внешние ключи</h2><div class="date">Последнее обновление: 09.07.2017</div>
...
<p>Внешние ключи применяются для установки связи между таблицами. Внешний ключ устанавливается для столбцов из зависимой, подчиненной таблицы, и 
указывает на один из столбцов из главной таблицы. Хотя, как правило, внешний ключ указывает на первичный ключ из связанной главной таблицы, но это необязательно должно быть непременным условием. Внешний ключ также может указывать на какой-то другой столбец, 
который имеет уникальное значение.</p>
...
<p>Для создания ограничения внешнего ключа на уровне столбца после ключевого слова 
<span class="b">REFERENCES</span> указывается имя связанной таблицы и в круглых скобках имя связанного столбца, на который будет указывать внешний ключ. 
Также обычно добавляются ключевые слова <span class="b">FOREIGN KEY</span>, но в принципе их необязательно указывать. После выражения REFERENCES идет выражение <span class="b">ON DELETE</span> и <span class="b">ON UPDATE</span>.</p>
...
<p>С помощью оператора <span class="b">CONSTRAINT</span> можно задать имя для ограничения внешнего ключа. Обычно это имя начинается с префикса 
"FK_":</p>
<pre class="brush:sql;">
CREATE TABLE Orders
(
...
<p>В данном случае ограничение внешнего ключа CustomerId называется "FK_Orders_To_Customers".</p>
<h3>ON DELETE и ON UPDATE</h3>
<p>С помощью выражений <span class="b">ON DELETE</span> и <span class="b">ON UPDATE</span> можно установить действия, которые выполняться соответственно 
при удалении и изменении связанной строки из главной таблицы. И для определения действия мы можем использовать следующие опции:</p>
<ul>
...
<li><p><span class="b">NO ACTION</span>: предотвращает какие-либо действия в зависимой таблице при удалении или изменении связанных строк в главной таблице. 
