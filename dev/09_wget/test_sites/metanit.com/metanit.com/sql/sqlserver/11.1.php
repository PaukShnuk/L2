...
<html >
<head>
<title>MS SQL Server и T-SQL | Хранимые процедуры</title>
<meta charset="utf-8" />
<meta name="description" content="Хранимые процедуры в T-SQL в MS SQL Server, их определение и использование, команда CREATE PROCEDURE и EXEC">
...
<h1>Хранимые процедуры</h1><h2>Создание и выполнение процедур</h2><div class="date">Последнее обновление: 14.08.2017</div>
...
<p>Нередко операция с данными представляет набор инструкций, которые необходимо выполнить в определенной последовательности. Например, при добавлении данных покупки товара 
...
И в этом случае более оптимально будет инкапсулировать все эти действия в один объект - <span class="b">хранимую процедуру</span> (stored procedure).</p>
<p>То есть по сути хранимые процедуры представляют набор инструкций, которые выполняются как единое целое. 
...
изменить код процедуры. То есть процедура также упрощает управление кодом.</p>
<p>Также хранимые процедуры позволяют ограничить доступ к данным в таблицах и тем самым уменьшить вероятность преднамеренных или неосознанных 
нежелательных действий в отношении этих данных.</p>
<p>И еще один важный аспект - производительность. Хранимые процедуры обычно выполняются быстрее, чем обычные SQL-инструкции. Все потому что код процедур компилируется один раз при первом ее запуске, а затем сохраняется в скомпилированной форме.</p>
<p>Для создания хранимой процедуры применяется команда <span class="b">CREATE PROCEDURE</span> или <span class="b">CREATE PROC</span>.</p>
<p>Таким образом, хранимая процедура имеет три ключевых особенности: упрощение кода, безопасность и производительность.</p>
...
<p>Поскольку команда CREATE PROCEDURE должна вызываться в отдельном пакете, то после команды USE, которая устанавливает текущую базу данных, используется 
команда <span class="b">GO</span> для определения нового пакета.</p>
<p>После имени процедуры должно идти ключевое слово AS.</p>
<p>Для отделения тела процедуры от остальной части скрипта код процедуры нередко помещается в блок BEGIN...END:</p>
<pre class="brush:sql;">
USE productsdb;
GO
CREATE PROCEDURE ProductSummary AS
...
<p>После добавления процедуры мы ее можем увидеть в узле базы данных в SQL Server Management Studio в подузле <span class="b">Programmability -&gt; Stored Procedures</span>:</p>
<img src="pics/130.png" alt="Создание хранимых процедур в MS SQL Server" />
<p>И мы сможем управлять процедурой также и через визуальный интерфейс.</p>
<h3>Выполнение процедуры</h3>
<p>Для выполнения хранимой процедуры вызывается команда <span class="b">EXEC</span> или <span class="b">EXECUTE</span>:</p>
<pre class="brush:sql;">
EXEC ProductSummary
</pre>
<img src="pics/131.png" alt="Выполнение хранимых процедур в MS SQL Server" />
<h3>Удаление процедуры</h3>
<p>Для удаления процедуры применяется команда <span class="b">DROP PROCEDURE</span>:</p>
<pre class="brush:sql;">DROP PROCEDURE ProductSummary</pre>
...
<div class="nav"><p><a href="10.4.php">Назад</a><a href="./">Содержание</a><a href="11.2.php">Вперед</a></p></div></div>
<div class="socBlock">
