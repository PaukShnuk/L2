...
<html >
<head>
<title>MS SQL Server и T-SQL | Неявное соединение таблиц</title>
<meta charset="utf-8" />
<meta name="description" content="Неявное соединение таблиц в T-SQL в MS SQL Server 2016 с помощью команды SELECT">
...
<h1>Соединение таблиц</h1><h2>Неявное соединение таблиц</h2><div class="date">Последнее обновление: 20.07.2017</div>
...
<p>Для сведения данных из разных таблиц мы можем использовать стандартную команду SELECT. Допустим, у нас есть следующие таблицы, которые связаны между собой связями:</p>
<pre class="brush:sql;">
USE productsdb;
...
и и по какой цене он был куплен (Price). И кроме того, таблица также хранит в виде столбца CreatedAt дату покупки.</p>
...
<p>При такой выборке для каждой строки из таблицы Orders будет совмещаться с каждой строкой из таблицы Customers. То есть, получится перекрестное соединение. 
Например, в Orders три строки, а в Customers то же три строки, значит мы получим 3 * 3 = 9 строк:</p>
 <img src="pics/64.png" alt="Неявное соединение таблиц без JOIN в SQL" />
<p>То есть в данном случае мы получаем прямое (декартово) произведение двух групп. Но вряд ли это тот результат, который хотелось бы видеть. 
...
<img src="pics/65.png" alt="Соединение таблиц без JOIN в T-SQL" />
...
<p>Поскольку надо соединить три таблицы, то применяются как минимум два условия. Ключевой таблицей остается Orders, из которой извлекаются все заказы, а затем к ней подсоединяется 
...
<img src="pics/66.png" alt="Соединение таблиц без JOIN в MS SQL Server 2016" />
