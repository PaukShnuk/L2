...
<html >
<head>
<title>MS SQL Server и T-SQL | Переменные</title>
<meta charset="utf-8" />
<meta name="description" content="Определение и использование переменных в T-SQL в MS SQL Server, операторы DECLARE и SET">
...
<h1>Переменные и управляющие конструкции</h1><h2>Переменные в T-SQL</h2><div class="date">Последнее обновление: 14.08.2017</div>
...
<p>Переменная представляет именованный объект, который хранит некоторое значение. Для определения переменных применяется выражение <span class="b">DECLARE</span>, 
после которого указывается название и тип переменной. При этом название локальной переменной должно начинаться с символа <span class="b">@</span>:</p>
<pre class="brush:sql;">
DECLARE @название_переменной тип_данных
</pre>
<p>Например, определим переменную name, которая будет иметь тип NVARCHAR:</p>
<pre class="brush:sql;">
DECLARE @name NVARCHAR(20)
</pre>
<p>Также можно определить через запятую сразу несколько переменных:</p>
<pre class="brush:sql;">
DECLARE @name NVARCHAR(20), @age INT
</pre>
<p>С помощью выражения <span class="b">SET</span> можно присвоить переменной некоторое значение:</p>
<pre class="brush:sql;">
DECLARE @name NVARCHAR(20), @age INT;
SET @name='Tom';
SET @age = 18;
</pre>
<p>Так как @name предоставляет тип NVARCHAR, то есть строку, то этой переменной соответственно и присваивается строка. А переменной @age присваивается число, так как она 
представляет тип INT.</p>
<p>Выражение <span class="b">PRINT</span> возвращает сообщение клиенту. Например:</p>
<pre class="brush:sql;">
PRINT 'Hello World'
</pre>
<p>И с его помощью мы можем вывести значение переменной:</p>
<pre class="brush:sql;">
DECLARE @name NVARCHAR(20), @age INT;
SET @name='Tom';
SET @age = 18;
...
<p>При выполнении скрипта внизу SQL Server Management Studio отобразится значение переменных:</p>
<img src="pics/115.png" alt="Функция Print в T-SQL" />
<p>Также можно использовать для получения значения команду <span class="b">SELECT</span>:</p>
<pre class="brush:sql;">
DECLARE @name NVARCHAR(20), @age INT;
SET @name='Tom';
SET @age = 18;
...
<img src="pics/129.png" alt="Получение переменных в T-SQL и MS SQL Server" />
