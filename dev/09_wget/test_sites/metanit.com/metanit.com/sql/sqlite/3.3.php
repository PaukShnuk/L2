...
<html >
<head>
<title>SQLite | Фильтрация данных. Оператор WHERE</title>
<meta charset="utf-8" />
<meta name="description" content="Фильтрация данных в SQLite, оператор WHERE, условие фильтрации, логические операторы AND, OR, NOT и операторы сравнения">
...
<h2>Фильтрация данных. Оператор WHERE</h2><div class="date">Последнее обновление: 27.11.2021</div>
...
<p>Нередко при получении данных из БД выбираются только те данные, которые соответствуют некоторому определенному условию. 
Для фильтрации данных в команде SELECT применяется оператор <span class="b">WHERE</span>, после которого указывается условие:</p>
...
<p>К примеру, возьмем следующую таблицу products, которая хранит некоторый набор товаров:</p>
<pre class="brush:sql;">
...
<p>Выберем из этой таблицы все товары, производителем которых является компания Samsung:</p>
<pre class="brush:sql;">
SELECT * FROM products
WHERE company = 'Samsung';
</pre>
<img src="./pics/3.10.png" alt="Оператор WHERE в SQLite" />
<p>Стоит отметить, что для SQLite <span class="b">важен</span> регистр символов, и, к примеру, строка "Samsung" НЕ будет эквивалентна строке "SAMSUNG" 
или "samSunG".</p>
...
<img src="pics/3.11.png" alt="Фильтрация данных с помощью WHERE в SQLite" />
<h3>Логические операторы</h3>
<p>Логические операторы позволяют объединить несколько условий. В SQLite можно использовать следующие логические операторы:</p>
<ul>
...
<img src="./pics/3.12.png" alt="Логический оператор AND в SQLite" />
...
<img src="./pics/3.13.png" alt="Логический оператор OR в SQLite" />
...
<img src="./pics/3.14.png" alt="Оператор NOT в SQLite" />
<h3>Приоритет операций</h3>
<p>В одном условии при необходимости мы можем объединять несколько логических операций. При этом следует учитывать, что самой приоритетной операцией, 
...
<p>В конце вычисляется оператор OR - либо цена должна быть меньше или равна 30000 и <span class="b">одновременно</span> количество товаров должно быть больше 2, либо производителем должен быть Samsung.</p>
<img src="./pics/3.15.png" alt="Приоритет логических операций в SQLite" />
<p>С помощью скобок можно переопределить приоритет операций:</p>
<pre class="brush:sql;">
SELECT * FROM products
WHERE company ='Samsung' OR NOT (price &gt; 30000 AND product_count &gt; 2);
</pre>
<p>В данном случае находим товары, у которых либо производитель Samsung, либо <span class="b">одновременно</span> цена товара меньше или равна 30000 и количество товаров меньше 3.</p>
<img src="./pics/3.16.png" alt="приоритет логических операций AND, OR, NOT в SQLite" />
