...
<html >
<head>
<title>SQLite | Получение диапазона строк. Оператор LIMIT</title>
<meta charset="utf-8" />
<meta name="description" content="Оператор LIMIT и получение диапазона строк при выборке из базы данных в SQLIte, установка смещения и оператор OFFSET">
...
<h2>Получение диапазона строк. Оператор LIMIT</h2><div class="date">Последнее обновление: 29.11.2021</div>
...
<p>Оператор <span class="b">LIMIT</span> позволяет задать ограничение на количество строк и имеет следующий синтаксис:</p>
<pre class="brush:sql;">LIMIT [смещение,] количество_строк</pre>
<p>Если оператору LIMIT передается один параметр, то он указывает на количество извлекаемых строка. Если передается два параметра, то первый параметр устанавливает 
смещение относительно начала, то есть сколько строк нужно пропустить, а второй параметр также указывает на количество извлекаемых строк.</p>
<p>Также можно использовать другую форму, где смещение указывается после оператора <span class="b">OFFSET</span>:</p>
<pre class="brush:sql;">LIMIT количество_строк OFFSET смещение</pre>
<p>Допустим, у нас есть следующая таблица users</p>
<pre class="brush:sql;">
...
<p>Например, выберем первые три строки:</p>
<pre class="brush:sql;">
SELECT * FROM users
LIMIT 3;
</pre>
<img src="./pics/4.13.png" alt="Оператор LIMIT в SQLite" />
<p>Теперь используем второй параметр и укажем смещение, с которой должна происходить выборка:</p>
<pre class="brush:sql;">
SELECT * FROM users
LIMIT 2, 3;
</pre>
<p>В данном случае пропускаются две первые строки и извлекаются следующие 3 строки:</p>
<img src="./pics/4.14.png" alt="LIMIT и OFFSET в SQLite" />
<p>В качестве альтернативы можно задать смещение после оператора <span class="b">OFFSET</span>:</p>
<pre class="brush:sql;">
SELECT * FROM users
LIMIT 3 OFFSET 2;
</pre>
<p>Как правило, оператор LIMIT используется вместе с оператором ORBER BY:</p>
<pre class="brush:sql;">
SELECT * FROM users
ORDER BY name
LIMIT 2, 3;
