...
<html >
<head>
<title>SQLite | Типы данных</title>
<meta charset="utf-8" />
<meta name="description" content="Типы данных в SQLite, INTEGER, REAL, TEXT, NULL, NUMERIC, BLOB, storage class и type affinity">
...
<h2>Типы данных</h2><div class="date">Последнее обновление: 26.11.2021</div>
...
<p>При определении столбцов таблицы для них необходимо указать тип данных. Каждый столбец имеет определенный тип данных. 
Для хранения данных в в SQLite применяются следующие типы:</p>
<ul>
<li><p><span class="b">NULL</span>: указывает фактически на отсутствие значения</p></li>
<li><p><span class="b">INTEGER</span>: представляет целое число, которое может быть положительным и отрицательным и в зависимости от своего значения может занимать 1, 2, 3, 4, 6 или 8 байт</p></li>
<li><p><span class="b">REAL</span>: представляет число с плавающей точкой, занимает 8 байт в памяти</p></li>
<li><p><span class="b">TEXT</span>: строка текста в одинарных кавычках, которая сохраняется в кодировке базы данных (UTF-8, UTF-16BE или UTF-16LE)</p></li>
<li><p><span class="b">BLOB</span>: бинарные данные</p></li>
</ul>
<p>Стоит отметить, что SQLite оперирует концепцией <span class="b">классов хранения</span> или <span class="b">storage class</span>. И по сути все эти пять типов называются классами хранения. 
Концепция классов хранения несколько шире, чем тип данных. Например, класс <span class="b">INTEGER</span> по сути объединяет 6 различных целочисленных типов 
данных разной длины. Однако это больше относится к внутренней работе SQLite. И внешне, например, на уровне определения таблицы и работы с данными мы будем работать с типом 
<span class="b">INTEGER</span>, а не со всеми реальными типами, которые скрываются за этим названием. Поэтому фактически классы хранения ассоциируются с типа данных. И выше 
представленные пять классов хранения также обычно называют типами данных и данные можно применять при определении столбцов:</p>
<pre class="brush:sql;">
CREATE TABLE users
(
    id INTEGER,
...
<p>Кроме того, мы можем применять идентификатор <span class="b">NUMERIC</span>. Этот идентификатор не представляет отдельного типа данных. А фактически представляет столбец, 
который может хранить данные всех пяти выше перечисленных типов (в терминологии SQLite NUMERIC еще называется <span class="b">type affinity</span>). Например:</p>
<pre class="brush:sql;">
CREATE TABLE users
(
    id INTEGER,
