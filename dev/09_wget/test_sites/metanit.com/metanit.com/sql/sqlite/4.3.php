...
<html >
<head>
<title>SQLite | Сортировка. ORDER BY</title>
<meta charset="utf-8" />
<meta name="description" content="Сортировка данных в SQLite, оператор ORDER BY, сортировка по нескольким столбцам по возрастанию и убыванию и по NULL, NULLS LAST">
...
<h2>Сортировка. ORDER BY</h2><div class="date">Последнее обновление: 28.11.2021</div>
...
<p>Оператор <span class="b">ORDER BY</span> сортируют значения по одному или нескольких столбцам. Например, упорядочим выборку из таблицы products 
по столбцу price:</p>
<pre class="brush:sql;">
SELECT * FROM products
ORDER BY price;
</pre>
<img src="./pics/4.9.png" alt="Оператор order by и сортировка в SQLite" />
...
<img src="./pics/4.10.png" alt="Сортировка по производным столбцам в SQLite" />
<p>В качестве критерия сортировки также можно использовать вычисляемое выражение на основе столбцов:</p>
<pre class="brush:sql;">
...
<img src="./pics/4.11.png" alt="Сортировка по убыванию в SQLite" />
...
<p>При сортировке сразу по нескольким столбцам все эти столбцы указываются через запятую после оператора <span class="b">ORDER BY</span>:</p>
<pre class="brush:sql;">
SELECT name, price, company
FROM products
ORDER BY company, name;
</pre>
<p>Здесь строки сначала сортируются по столбцу company по возрастанию. Затем если есть две строки, в которых столбец company 
имеет одинаковое значение, то они сортируются по столбцу name также по возрастанию. Но опять же с помощью ASC и DESC можно отдельно для 
...
<img src="./pics/4.12.png" alt="Сортировка по нескольким столбцам в SQLite" />
<h3>Сортировка по NULL</h3>
<p><span class="b">NULL</span> представляет специальное значение, которое не сравнивается с другими значениями. По умолчанию NULL оценивается как 
наименьшее из значений. Это значит, что при сортировке по возрастанию значение NULL будет предшествовать всем остальным значениям.</p>
<p>Допустим, у нас есть следующая таблица пользователей:</p>
<pre class="brush:sql;">
...
<p>Столбец company здесь хранит компанию, где работает пользователь. Но пользователь может не работать, и соответственно столбец будет иметь значение NULL. 
Отсортируем пользователей по компании:</p>
<pre class="brush:sql;">
SELECT * FROM users
ORDER BY company;
</pre>
<img src="./pics/4.15.png" alt="Сортировка по NULL в SQLite" />
<p>Однако поведение по умолчанию, когда NULL идет перед остальными значениями при сортировке по возрастанию или в конце при сортировке по убыванию может быть 
нежелательным. В этом случае мы можем использовать дополнительные операторы: <span class="b">NULLS FIRST</span> и <span class="b">NULLS LAST</span>. 
<span class="b">NULLS FIRST</span> указывает, что значение NULL идет перед всеми остальными значениями, а <span class="b">NULLS LAST</span> - после.</p>
<p>Например, выведем значения NULL при сортировке по возрастанию после остальных значений:</p>
<pre class="brush:sql;">
SELECT * FROM users
ORDER BY company NULLS LAST;
</pre>
<img src="./pics/4.16.png" alt="Сортировка по NULL и NULLS LAST в SQLite" />
