...
<html >
<head>
<title>SQLite | Подзапросы в основных командах SQL</title>
<meta charset="utf-8" />
<meta name="description" content="Подзапросы в основных командах SELECT, INSERT, UPDATE, DELETE в SQLite">
...
<h2>Подзапросы в основных командах SQL</h2><div class="date">Последнее обновление: 29.11.2021</div>
...
<p>Рассмотрим применение подзапросов в основных командах SQLite - SELECT, INSERT, UPDATE, DELETE. И для этого возьмем следующие таблицы:</p>
<pre class="brush:sql;">
...
<p>Рассмотрим некоторые из этих случаев. Например, получим всех пользователей, у которых возраст выше среднего:</p>
<pre class="brush:sql;">
SELECT * FROM users
WHERE age &gt; (SELECT AVG(age) FROM users);
</pre>
<p>Чтобы получить нужных пользователей, вначале надо выполнить подзапрос на получение среднего возраста: <code>SELECT AVG(age) FROM users</code>.</p>
<h4>Операторы IN и BETWEEN</h4>
<p>Нередко подзапросы применяются для получения набора значений, которые затем используются операторами <span class="b">IN</span> и <span class="b">BETWEEN</span>. 
Например, пользователей определенных компаний:</p>
<pre class="brush:sql;">
SELECT * FROM users
WHERE company_id IN (SELECT id FROM companies 
				WHERE name='Microsoft' OR name='Google');
</pre>
<p>Здесь оператору <span class="b">IN</span> передается набор значений, который является результатом подзапроса</p>
<pre class="brush:sql;">SELECT id FROM companies WHERE name='Microsoft' OR name='Google'</pre>
<p>Другой пример - выберем всех пользователей из таблицы users, у которых возраст либо максимальный, либо минимальный:</p>
<pre class="brush:sql;">
SELECT * FROM users
WHERE age IN ((SELECT MIN(age) FROM users), 
			(SELECT MAX(age) FROM users));
</pre>
<p>То есть в данном случае оператору IN передаются два значения, которые являются результатами двух подзапросов.</p>
<p>Аналогичным образом мы можем получать значения для оператора <span class="b">BETWEEN</span>. Например, выберем всех пользователей, у которых возраст в диапазоне 
от среднего до максимального:</p>
<pre class="brush:sql;">
SELECT * FROM users
WHERE age BETWEEN 
...
<img src="./pics/6.2.png" alt="Подзапросы в SELECT в SQLite" />
<h3>Подзапросы в команде INSERT</h3>
<p>В команде INSERT подзапросы могут применяться для определения значения, которое вставляется в один из столбцов, что в принципе уже ранее рассматривалось:</p>
<pre class="brush:sql;">
INSERT INTO companies (name) VALUES  ('Microsoft'), ('Google'), ('JetBrains');
...
<p>Так, увеличим в таблице users возраст пользователей, которые работают в компании Microsoft, на 3:</p>
<pre class="brush:sql;">
UPDATE users
SET age = age + 3
WHERE company_id =(SELECT id FROM companies WHERE name='Microsoft');
</pre>
<p>Или установим для пользователя с id=1 максимальный возраст:</p>
<pre class="brush:sql;">
UPDATE users
...
<p>В команде DELETE подзапросы также применяются как часть условия. Так, удалим всех пользователей, которые работают в Microsoft:</p>
<pre class="brush:sql;">
DELETE FROM users
WHERE company_id=(SELECT id FROM companies WHERE name='Microsoft');
</pre>
