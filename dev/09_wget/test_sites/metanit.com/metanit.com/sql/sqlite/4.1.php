...
<html >
<head>
<title>SQLite | Выборка уникальных значений. Оператор DISTINCT</title>
<meta charset="utf-8" />
<meta name="description" content="Оператор DISTINCT в SQLite и выборка уникальных значений из базы данных">
...
<h1>Запросы</h1><h2>Выборка уникальных значений. Оператор DISTINCT</h2><div class="date">Последнее обновление: 28.11.2021</div>
...
<p>К примеру, разные товары могут иметь одних и тех же производителей, и, допустим, у нас следующая таблица товаров:</p>
<pre class="brush:sql;">
...
<img src="./pics/4.1.png" alt="Выборка уникальных значений в SQLite" />
<p>Но при таком запросе производители повторяются. Теперь применим оператор <span class="b">DISTINCT</span> для выборки уникальных значений:</p>
<pre class="brush:sql;">
SELECT DISTINCT company FROM products;
</pre>
<img src="./pics/4.2.png" alt="Оператор DISTINCT в SQLite" />
<p>Также мы можем задавать выборку уникальных значений по нескольким столбцам:</p>
<pre class="brush:sql;">
SELECT DISTINCT company, product_count FROM products;
</pre>
<p>Здесь для выборки используются столбцы company и product_count. Из пяти строк только для двух строк эти столбцы имеют повторяющиеся значения. 
Поэтому в выборке будет 4 строки:</p>
<img src="./pics/4.3.png" alt="Оператор DISTINCT и несколько столбцов в SQLite" />
<p>Стоит отметить, что SQLite рассматривает значения <span class="b">NULL</span> как повторяющиеся. 
Поэтому если оператор DISTINCT производит уникальную выборку по столбцу, для которого в нескольких строках значится значение NULL, то оператор DISTINCT 
отберет из них только одну строку.</p>
