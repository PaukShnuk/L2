...
<html >
<head>
<title>SQLite | Функции для работы со строками</title>
<meta charset="utf-8" />
<meta name="description" content="Функции для работы со строками в SQLite, replace, substr, substring, ltrim, rtrim, lenth, получение и замена подстроки">
...
<h2>Функции для работы со строками</h2><div class="date">Последнее обновление: 01.12.2021</div>
...
<p>Для работы со строками в SQLite можно применять следующие функции:</p>
<ul>
...
<pre class="brush:sql;">SELECT length('обороноспособность');	-- 18</pre>
</li>
<li><p><span class="b">ltrim</span>: удаляет начальные пробелы из строки. В качестве параметра принимает строку:</p>
<pre class="brush:sql;">SELECT ltrim('  Apple');</pre>
<p>Может принимать второй параметр - подстроку, справа от которой надо вырезать подстроку:</p>
<pre class="brush:sql;">SELECT ltrim('iPhone 13 Pro', 'iPhone');  -- 13 Pro</pre></li>
<li><p><span class="b">rtrim</span>: удаляет конечные пробелы из строки. В качестве параметра принимает строку:</p>
<pre class="brush:sql;">SELECT rtrim('Apple  ');</pre>
<p>Может принимать второй параметр - подстроку, слева от которой надо вырезать подстроку:</p>
<pre class="brush:sql;">SELECT rtrim('iPhone 13 Pro', 'Pro');  -- iPhone 13</pre></li>
<li><p><span class="b">trim</span>: удаляет начальные и конечные пробелы из строки. В качестве параметра принимает строку:</p>
<pre class="brush:sql;">SELECT trim('   Apple  ');</pre>
<p>Может принимать второй параметр - подстроку, слева или справа от которой надо вырезать подстроку:</p>
<pre class="brush:sql;">SELECT trim('iPhone 13 Pro', 'Pro');  -- iPhone 13</pre></li>
<li><p><span class="b">substr</span> и <span class="b">substring</span>: вырезает из строки подстроку определенной длиной, начиная с определенного индекса. 
Певый параметр функции - строка, второй - начальный индекс для вырезки, и третий параметр - количество вырезаемых символов:</p>
<pre class="brush:sql;">
SELECT substr('обороноспособность', 8, 6),	-- способ
	substring('обороноспособность', 2, 3);	-- бор
</pre></li>
<p>Третий параметр можно опустить, тогда в результат поадают все символы с индекса, который передается во втором параметре:</p>
<pre class="brush:sql;">
SELECT substr('обороноспособность', 8),	-- способность
	substring('обороноспособность', 2);	-- бороноспособность
</pre>
<li><p><span class="b">replace</span>: заменяет одну подстроку другой в рамках строки. Первый параметр функции - строка, второй - подстрока, которую надо заменить, а третий - подстрока, на которую надо заменить:</p>
<pre class="brush:sql;">SELECT replace('iPhone 12 Pro', '12 Pro', '13');	-- iPhone 13</pre></li>
...
<li><p><span class="b">instr</span>: возвращает индекс, начиная с которого в строке встречается подстрока. Индексация начинается с 1, то есть первый символ имеет индекс 0. Если подстрока не 
встречается в строке, то возвращается 0. Первый параметр функции - строка, а второй параметр - подстрока, индекс которой надо найти:</p>
<pre class="brush:sql;">SELECT instr('обороноспособность', 'но');  -- 6</pre>
</li>
</ul>
