...
<html >
<head>
<title>SQLite | Ограничения столбцов и таблиц</title>
<meta charset="utf-8" />
<meta name="description" content="Ограничения столбцов и таблиц в SQLite, UNIQUE, PRIMARY KEY, AUTOINCREMENT, CHECK, DEFAULT, значение по умолчанию, первичный ключ, оператор CONSTRAINT">
...
<h2>Ограничения столбцов и таблиц</h2><div class="date">Последнее обновление: 26.11.2021</div>
...
<p>При определении столбцов и таблиц для них можно задать ограничения. Ограничения позволяют настроить поведение столбцов и таблиц. Ограничения столбцов указываются после типа столбца:</p>
<pre class="brush:sql;">имя_столбца тип_столбца ограничения_столбца</pre>
<p>Ограничения таблицы указываются после определения всех столбцов.</p>
<p>Рассмотрим, какие ограничения столбцов мы можем использовать.</p>
 <h3>PRIMARY KEY</h3>
 <p>Атрибут <span class="b">PRIMARY KEY</span> задает первичный ключ таблицы. Первичный ключ уникально идентифицирует строку в таблице. Например:</p>
 <pre class="brush:sql;">
 CREATE TABLE users
(
...
<p>Здесь столбец id выступает в качестве первичного ключа, он будет уникально идентифицировать строку и его значение должно быть уникальным. То есть у нас не может 
быть таблице users более одной строки, где в столбце id было бы одно и то же значение.</p>
...
<p>Первичный ключ может быть составным. Такой ключ использовать сразу несколько столбцов, чтобы уникально идентифицировать строку в таблице. Например:</p>
<pre class="brush:sql;">
CREATE TABLE users
(
    id INTEGER,
...
<p>В данном случае в качестве первичного ключа выступает связка столбцов id и name. То есть в таблице users не может быть двух строк, где для обоих из этих полей одновременно были бы одни и те же значения.</p>
<h3>AUTOINCREMENT</h3>
<p>Ограничение <span class="b">AUTOINCREMENT</span> позволяет указать, что значение столбца будет автоматически увеличиваться при добавлении новой строки. 
Данное ограничение работает для столбцов, которые представляют тип <span class="b">INTEGER</span> с ограничением <span class="b">PRIMARY KEY</span>:</p>
<pre class="brush:sql;">
DROP TABLE users;
CREATE TABLE users
(
...
<p>В данном случае значение столбца id каждой новой добавленной строки будет увеличиваться на единицу.</p>
<h3>UNIQUE</h3>
<p>Ограничение <span class="b">UNIQUE</span> указывает, что столбец может хранить только уникальные значения.</p>
<pre class="brush:sql;">
CREATE TABLE users
(
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
<p>В данном случае столбец email, который представляет телефон пользователя, может хранить только уникальные значения. 
И мы не сможем добавить в таблицу две строки, у которых значения для этого столбца будет совпадать.</p>
<p>Также мы можем определить это ограничение на уровне таблицы:</p>
<pre class="brush:sql;">
CREATE TABLE users
(
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
<p>В данном случае уникальность значений устанавлена сразу для двух столбцов - name и email.</p>
<h3>NULL и NOT NULL</h3>
<p>По умолчанию любой столбец, если он не представляет первичный ключ, может принимать значение <span class="b">NULL</span>, то есть фактически отсутствие формального 
...
для него следует установить ограничение <span class="b">NOT NULL</span>:</p>
<pre class="brush:sql;">
CREATE TABLE users
(
    id INTEGER PRIMARY KEY,
...
<p>В данном случае столбец name не допускает значение NULL.</p>
<h3>DEFAULT</h3>
<p>Ограничение <span class="b">DEFAULT</span> определяет значение по умолчанию для столбца. 
Если при добавлении данных для столбца не будет предусмотрено значение, то для него будет использоваться значение по умолчанию.</p>
<pre class="brush:sql;">
CREATE TABLE users
(
    id INTEGER PRIMARY KEY,
...
<p>Здесь столбец age в качестве значения по умолчанию имеет число 18.</p>
<h3>CHECK</h3>
<p>Ограничение <span class="b">CHECK</span> задает ограничение для диапазона значений, которые могут храниться в столбце. Для этого после CHECK 
...
<p>Кроме проверки возраста здесь также проверяется, что столбец name не может иметь пустую строку в качестве значения (пустая строка не эквивалентна 
значению NULL).</p>
...
<p>Также CHECK можно использовать на уровне таблицы:</p>
<pre class="brush:sql;">
CREATE TABLE users
(
    id INTEGER PRIMARY KEY,
...
<p>С помощью оператора <span class="b">CONSTRAINT</span> можно задать имя для ограничений. Они указываются после ключевого слова CONSTRAINT перед ограничениями на уровне таблицы:</p>
<pre class="brush:sql;">
CREATE TABLE users
(
    id INTEGER,
...
<p>В данном случае ограничение для PRIMARY KEY называется users_pk, для UNIQUE - user_phone_uq, а для CHECK - user_age_chk. Смысл установки 
имен ограничений заключается в том, что впоследствии через эти имена мы сможем управлять ограничениями - удалять или изменять их.</p>
