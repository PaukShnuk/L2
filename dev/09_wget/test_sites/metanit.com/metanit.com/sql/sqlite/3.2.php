...
<html >
<head>
<title>SQLite | Выборка данных. Команда SELECT</title>
<meta charset="utf-8" />
<meta name="description" content="Получение данных в SQLite, команда SELECT, псевдонимы столбцов, оператор AS, операции над полученными данными">
...
<h2>Выборка данных. Команда SELECT</h2><div class="date">Последнее обновление: 27.11.2021</div>
...
<p>Для получения данных в SQLite применяется команда <span class="b">SELECT</span>. В упрощенном виде она имеет следующий синтаксис:</p>
<pre class="brush:sql;">
SELECT список_столбцов FROM имя_таблицы
</pre>
<p>Прежде всего стоит отметить, что действие команды SELECT не ограничено только получением данных из БД. Так, мы даже можем выполнять с помощью SELECT простейшие вычисления. 
Например, выполним следующую команду:</p>
<pre class="brush:sql;">SELECT 5 * 6;</pre>
<img src="./pics/3.8.png" alt="SELECT и простейшие вычисления в SQLite" />
<p>или даже набор вычислений:</p>
<pre class="brush:sql;">
SELECT 5 * 6, 
	5 + 6, 
	5 - 6;
</pre>
<img src="./pics/3.9.png" alt="SELECT и арифметические операции в SQLite" />

<p>Но обычно команда SELECT применяется именно для полученияданных. Например, пусть ранее была создана следующая таблица users, и в нее добавлены некоторые начальные данные:</p>
<pre class="brush:sql;">
CREATE TABLE users
(
...
<img src="pics/3.4.png" alt="Команда SELECT и получение данных в SQLite" />
<p>Если необходимо получить данные не из всех, а из каких-то конкретных столбцов, тогда спецификации этих столбцов перечисляются через запятую после SELECT:</p>
<pre class="brush:sql;">
SELECT id, name FROM users;
</pre>
<img src="pics/3.5.png" alt="Выборка данных из отдельных столбцов в SQLite" />
<p>Спецификация столбца необязательно должна представлять его название. Это может быть любое выражение, например, результат арифметической операции. Допустим, у нас есть 
следующая таблица:</p>
<pre class="brush:sql;">
...
<p>Таблица products хранит товары. Столбец name хранит название товара, company - производителя, product_count - количество товара в штуках, а price - цену товара.</p>
<p>Например, получим для каждого товара совокупную цену и для этого выполним следующий запрос:</p>
<pre class="brush:sql;">
SELECT name, product_count * price
FROM products;
</pre>
<p>Здесь при выборке будут создаваться два столбца. Причем второй столбец представляет значение столбца product_count, умноженное на значение столбца price.</p>
<img src="./pics/3.6.png" alt="Производные столбцы в SQLite" />
...
<p>Здесь для первого столбца определяется псевдоним title, хотя в реальности он будет представлять столбец name. 
Второй столбец - total_sum хранит произведение столбцов product_count и price.</p>
<img src="pics/3.7.png" alt="Псевдонимы столбцов и оператор AS в SQLite" />
