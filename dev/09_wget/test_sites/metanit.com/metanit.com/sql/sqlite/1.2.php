...
<html >
<head>
<title>SQLite | Консольный клиент sqlite3</title>
<meta charset="utf-8" />
<meta name="description" content="Консольный клиент sqlite3, создание и управление базами данных SQLite в консоли">
...
<h2>Консольный клиент sqlite3</h2><div class="date">Последнее обновление: 26.11.2021</div>
...
<p>Для работы с SQLite разработчики данной СУБД предоставляют консольный клиент <span class="b">sqlite3</span>. Рассмотрим вкратце, как с ним работать.</p>
<p>Прежде всего нам надо загрузить sqlite3. Для этого перейдем на страницу <a href="https://www.sqlite.org/download.html" rel="nofollow">https://www.sqlite.org/download.html</a>. 
Название необходимого нам пакета начинается с <span class="b">sqlite-tools</span>. И на странице загрузки мы можем найти версии для Windows, Linux, MacOS:</p>
<img src="./pics/1.1.png" alt="Загрузка консольной утилиты sqlite3" />
<p>Загрузим нужную нам версию и распакуем ее.</p>
<img src="./pics/1.2.png" alt="Консольная утилита sqlite-tools" />
<p>В распакованной папке мы сможем найти три утилиты, из которых файл с названием <span class="b">sqlite3</span> и представляет собственно 
консольную оболочку для работы с бд SQLite. Запустим ее:</p>
<img src="./pics/1.3.png" alt="Консольная утилита sqlite-tools" />
<h3>Открытие базы данных</h3>
<p>Для открытия базы данных необходимо ввести команду <span class="b">.open</span>, после которой указывается путь к базе данных. Например,</p>
<div class="console"><pre class="consoletext">.open test.db</pre></div>
<p>В данном случае будет открыта база данных под названием "test.db', которая находится в той же папке, что и консольная утилита. Если базы данных не существует, 
то она создается.</p>
<img src="./pics/1.4.png" alt="Открытие и создание базы данных в sqlite3" />
<p>Также можно передать абсолютный путь:</p>
<div class="console"><pre class="consoletext">sqlite&gt;.open C:\\sqlite\\test.db</pre></div>
<p>После открытия мы сможем работать с этой бд.</p>
<h3>Создание таблицы</h3>
<p>Для создания таблицы после открытия базы данных необходимо ввести команды <span class="b">CREATE TABLE</span>, после которой указываются название таблицы и спецификация ее столбцов:</p>
<div class="console"><pre class="consoletext">sqlite&gt;create table users(name text, age integer);</pre></div>
<p>В данном случае создается таблица <code>users</code>, в которой два столбца: столбец <code>name</code>, который имеет тип <code>text</code>, и столбец <code>age</code>, который имеет 
тип <code>integer</code></p>
<p>Обратите внимание, что команда завершается точкой с запятой. И все команды SQL должны завершаться точкой с запятой, благодаря чему sqlite может идентифицировать, 
что выполняется sql-команда.</p>
<h3>Операции с данными</h3>
<p>Для добавления данных применяется команда <span class="b">INSERT INTO</span>. Например, добавим в таблицу users одну строку:</p>
<div class="console"><pre class="consoletext">sqlite&gt;insert into users values ('Tom', 37);</pre></div>
<p>Теперь получим ранее добавленны данные. Для этого используем команду <span class="b">SELECT</span>:</p>
<div class="console"><pre class="consoletext">sqlite&gt;select * from users;</pre></div>
<p>И sqlite выведет нам все данные из таблицы users:</p>
<img src="./pics/1.5.png" alt="Работа с данными через консольный клиент sqlite3" />
