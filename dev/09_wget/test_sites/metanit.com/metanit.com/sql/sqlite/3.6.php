...
<html >
<head>
<title>SQLite | Замена данных. Команда REPLACE</title>
<meta charset="utf-8" />
<meta name="description" content="Замена данных в БД SQLite, команда REPLACE, ">
...
<h2>Замена данных. Команда REPLACE</h2><div class="date">Последнее обновление: 27.11.2021</div>
...
<p>Команда <span class="b">REPLACE</span> по сути является сокращением от <span class="b">INSERT OR REPLACE</span>. Ее идея состоит в следующем. 
Пи вставке данных может нарушаются ограничения <span class="b">UNIQUE</span> или <span class="b">PRIMARY KEY</span>, например, когда мы пытаемся добавить для столбца, который должен иметь уникальные значения, 
данные, которые уже есть в таблице. Этот конфликт ограничений призвана разрешить команда <span class="b">REPLACE</span>.</p>
<p>Эта команда сначала удаляет строку, которая вызвала конфликт на уникальность данных, 
и затем вместо нее вставляет новую строку. То есть фактически все выглядит как замена строки.</p>
<p>Если происходит конфликт с ограничением <span class="b">NOT NULL</span> (в столбец, для которого задано ограничение <code>NOT NULL</code>, 
вставляется значение NULL), команда REPLACE заменяется вставляемое значение NULL значением по умолчанию, которое принято для этого столбца. Если 
для столбца не установлено значение по умолчанию, то выполнение запроса отменяется.</p>
<p>Если конфликтов с ограничениями не происходит, то команда <span class="b">REPLACE</span> по сути действует аналогично команде <span class="b">INSERT</span>.</p>
<p>Команда REPLACE имеет следующий формальный синтаксис:</p>
<pre class="brush:sql;">
INSERT OR REPLACE INTO table(столбец1, столбец2, ... столбецN)
VALUES(значение1, значение2, ... значениеN);
</pre>
<p>Вместо <span class="b">INSERT OR REPLACE</span> можно писать просто <span class="b">REPLACE</span>.</p>
<p>Например, возьмем следующую таблицу users, которая хранит пользователей:</p>
<pre class="brush:sql;">
...
<p>Здесь для столбца name задано ограничение <span class="b">UNIQUE</span>, то есть не может быть двух строк, у которых в этом столбце будет одно и то же значение.</p>
<p>Посмотрим эту команду в деле. Выше в определении таблице были добавлены начальные данные, где для одного из пользователей задано имя "Tom". И вначале попробуем 
добавить нового пользователя с тем же именем с помощью стандартной команды <span class="b">INSERT</span>:</p>
<pre class="brush:sql;">INSERT INTO users (name, age) VALUES ('Tom', 22);</pre>
<img src="./pics/3.19.png" alt="Команда REPLACE OR INSERT в SQLite" />
<p>И при выполнении мы столкнемся с ошибкой, которая говорит, данный запрос нарушает ограничение по уникальности столбца name.</p>
<p>Теперь попробуем добавить те же самые данные, только с помощью команды <span class="b">REPLACE</span>:</p>
<pre class="brush:sql;">
REPLACE INTO users (name, age) VALUES ('Tom', 22);
</pre>
<p>Эта команда удалить ранее добавленную стоку, где name = "Tom', и вставит текущие данные:</p>
<img src="./pics/3.20.png" alt="Команда REPLACE INTO в SQLite" />
...
<div class="nav"><p><a href="./3.5.php">Назад</a><a href="./">Содержание</a><a href="./4.1.php">Вперед</a></p></div></div>
<div class="socBlock">
