...
<html >
<head>
<title>SQLite | Подзапросы</title>
<meta charset="utf-8" />
<meta name="description" content="Подзапросы в SQLite, коррелирующие и некоррелирующие подзапросы к таблицам, correlated subquery, ">
...
<h2>Подзапросы</h2><div class="date">Последнее обновление: 29.11.2021</div>
...
<p>Подзапросы представляют выражения SELECT, которые встроены в другие запросы SQL. Рассмотрим простейший пример применения подзапросов.</p>
<p>Например, определим таблицы для пользователей и компаний, которых работают пользователи:</p>
<pre class="brush:sql;">
...
<p>Таблица users содержит поле <code>company_id</code>, которое будет представлять id компании из таблицы companies.</p>
...
<img src="./pics/6.1.png" alt="подзапросы в SQLite" />
<p>При добавлении данных в таблицу users как раз используются подзапросы. Например, первый пользователь работает в компании "Microsoft". 
Соответственно в таблицу users нам надо сохранить id компании, у которой поле name хранит строку "Microsoft". 
Но на момент написания запроса нам может быть неизвестен ни id компании. В этом случае можно выполнить подзапрос в виде</p>
<pre class="brush:sql;">
(SELECT id FROM companies WHERE name='Microsoft')
</pre>
<p>Подзапрос выполняет команду SELECT и заключается в скобки. В данном же случае при добавлении одного пользователя выполняется один подзапрос, который 
возвращает одного скалярное значение - числовой идентификатор комании.</p>
<p>В примере выше подзапросы выполнялись к другой таблице, но могут выполняться и к той же, для которой вызывается основной запрос. Например, найдем 
пользователей из таблицы users, возраст которых выше среднего:</p>
<pre class="brush:sql;">
SELECT * FROM users
WHERE age &gt; (SELECT AVG(age) FROM users);
</pre>
<h3>Коррелирующие и некоррелирующие подзапросы</h3>
<p>Подзапросы бывают коррелирующими и некоррелирующими. В примерах выше команды SELECT фактически выполняли один подзапрос для всех строк, извлекаемых 
...
запроса.</p>
<p>Но также можно использовать и <span class="b">коррелирующие подзапросы</span> (correlated subquery), результаты которых зависят от строк, 
которые выбираются в основном запросе.</p>
<p>Например, выберем всех пользователей из таблицы users, добавив к ним информацию о компании:</p>
<pre class="brush:sql;">
SELECT name, age, 
...
<p>В данном случае для каждой строки из таблицы users будет выполняться подзапрос, результат которого зависит от столбца company_id. И каждый подзапрос может 
возвращать различные данные.</p>
<img src="./pics/6.3.png" alt="Correlated subquery in SQLite" />
...
Например, выберем из таблицы users тех пользователей, возраст которых выше среднего возраста пользователей той же компании:</p>
<pre class="brush:sql;">
SELECT name,
	age,
...
<img src="./pics/6.4.png" alt="Коррелирующий подзапрос в SQLite" />
<p>Здесь определено два коррелирующих подзапроса. Первый подзапрос определяет спецификацию столбца <code>avg_age</code>. 
...
также может отличаться.</p>
<p>Второй подзапрос аналогичен, только он используется для фильтрации извлекаемых из таблицы users. И также он будет выполняться для каждой строки.</p>
<p>Чтобы избежать двойственности при фильтрации в подзапросе при сравнении id компаний (<code>subuser.company_id=user.company_id</code>) для внешней выборки установлен псевдоним <code>user</code>, а для выборки из 
подзапросов определен псевдоним <code>subuser</code>.</p>
