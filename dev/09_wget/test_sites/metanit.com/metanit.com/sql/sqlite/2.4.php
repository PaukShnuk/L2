...
<html >
<head>
<title>SQLite | Внешние ключи FOREIGN KEY</title>
<meta charset="utf-8" />
<meta name="description" content="Внешние ключи FOREIGN KEY в SQLite, связь двух таблиц, каскадное удаление">
...
<h2>Внешние ключи FOREIGN KEY</h2><div class="date">Последнее обновление: 26.11.2021</div>
...
<p>Внешние ключи позволяют установить связи между таблицами. Внешний ключ устанавливается для столбцов из зависимой, подчиненной таблицы, и 
указывает на один из столбцов из главной таблицы. Как правило, внешний ключ указывает на первичный ключ из связанной главной таблицы.</p>
...
<p>Для создания ограничения внешнего ключа после <span class="b">FOREIGN KEY</span> указывается столбец таблицы, который будет представляет 
внешний ключ. А после ключевого слова <span class="b">REFERENCES</span> указывается имя 
связанной таблицы, а затем в скобках имя связанного столбца, на который будет указывать внешний ключ. После выражения <span class="b">REFERENCES</span> 
идут выражения <span class="b">ON DELETE</span> и <span class="b">ON UPDATE</span>, которые задают действие при удалении и обновлении строки из 
главной таблицы соответственно.</p>
...
<p>В данном случае определены таблицы companies и users. companies является главной и представляет компании, где может работать пользователь. 
users является зависимой и представляет пользователей. Таблица users через столбец company_id связана с таблицей companies и ее столбцом id. То есть 
столбец company_id является <span class="b">внешним ключом</span>, который указывает на столбец id из таблицы companies.</p>
<p>С помощью оператора <span class="b">CONSTRAINT</span> можно задать имя для ограничения внешнего ключа:</p>
<pre class="brush:sql;">
CREATE TABLE users
(
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
соответственно при удалении и изменении связанной строки из главной таблицы. В качестве действия могут использоваться следующие опции:</p>
<ul>
...
<li><p><span class="b">SET NULL</span>: при удалении или обновлении связанной строки из главной таблицы устанавливает для столбца внешнего ключа 
значение <span class="b">NULL</span>. (В этом случае столбец внешнего ключа должен поддерживать установку NULL)</p></li>
<li><p><span class="b">RESTRICT</span>: отклоняет удаление или изменение строк в главной таблице при наличии связанных строк в зависимой таблице.</p></li>
<li><p><span class="b">NO ACTION</span>: то же самое, что и <code>RESTRICT</code>.</p></li>
<li><p><span class="b">SET DEFAULT</span>: при удалении связанной строки из главной таблицы устанавливает для столбца внешнего ключа значение по 
умолчанию, которое задается с помощью атрибуты DEFAULT.</p></li>
</ul>
<h3>Каскадное удаление</h3>
<p>Каскадное удаление позволяет при удалении строки из главной таблицы автоматически удалить все связанные строки из зависимой таблицы. 
Для этого применяется опция <span class="b">CASCADE</span>:</p>
<pre class="brush:sql;">
CREATE TABLE users
(
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
<p>Подобным образом работает и выражение <span class="b">ON UPDATE CASCADE</span>. При изменении значения первичного ключа автоматически изменится 
...
качестве первичных ключей столбцы с изменяемыми значениями, то на практике выражение ON UPDATE используется редко.</p>
