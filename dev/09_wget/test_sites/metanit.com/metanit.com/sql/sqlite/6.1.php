...
<html >
<head>
<title>SQLite | Функции CASE и IIF</title>
<meta charset="utf-8" />
<meta name="description" content="выражение CASE и функция IIF и создание условных конструкций в SQLite">
...
<h1>Встроенные функции</h1><h2>Функции CASE и IIF</h2><div class="date">Последнее обновление: 30.11.2021</div>
...
<h3>CASE</h3>
<p>Выражение <span class="b">CASE</span> проверяет истинность набора условий и в зависимости от результата проверки может возвращать тот или иной результат. 
Это выражение имеет следующую форму:</p>
<pre class="brush:sql;">
CASE
...
<p>Выражение <span class="b">CASE</span> последовательно просматривает все выражения с операторами <span class="b">WHEN</span>. 
После оператора <span class="b">WHEN</span> указывается условие. Если оно истинно, то возвращается результат, который указан после оператора <span class="b">THEN</span>. 
Если условие ложно, то выражение CASE переходит к следующего оператору WHEN.</p>
<p>В конце можно указать необязательный оператор <span class="b">ELSE</span>, который возвращает результат, если условие всех предыдущих операторов 
WHEN оказалось ложным.</p>
<p>Завершается определение CASE оператором <span class="b">END</span>.</p>
<p>Возьмем для примера следующую таблицу products:</p>
<pre class="brush:sql;">
...
<p>Выполним запрос к этой таблице и используем функцию <span class="b">CASE</span>:</p>
<pre class="brush:sql;">
...
<img src="./pics/8.3.png" alt="Функция CASE..WHEN в SQLite" />
<h3>Функция IIF</h3>
<p>Функция <span class="b">IIF</span> в зависимости от результата условного выражения возвращает одно из двух значений. Общая форма функции выглядит 
следующим образом:</p>
<pre class="brush:sql;">IIF(условие, значение_1, значение_2)</pre>
<p>Если условие, передаваемое в качестве первого параметра, верно, то возвращается первое значение, иначе возвращается второе значение. Например:</p>
<pre class="brush:sql;">
SELECT name, company, items_count,
    IIF(items_count &gt; 2, 'Много товара', 'Мало товара') AS status
FROM products;
</pre>
<img src="./pics/8.4.png" alt="Функция IIF в SQLite" />
<p>Функции <span class="b">IIF</span> могут быть вложенными:</p>
<pre class="brush:sql;">
SELECT name, company, items_count,
...
<p>В выражении <code>IIF(items_count == 1, 'Товар заканчивается', IIF(items_count==2...</code> функция IIF проверяет, равно ли значение items_count числу 1. 
Если равно, то возвращается строка "Товар заканчивается". Если не равно то возвращается результат далее вызываемой функции IIF: <code>IIF(items_count==2...</code></p>
...
<div class="nav"><p><a href="./5.6.php">Назад</a><a href="./">Содержание</a><a href="./6.4.php">Вперед</a></p></div></div>
<div class="socBlock">
