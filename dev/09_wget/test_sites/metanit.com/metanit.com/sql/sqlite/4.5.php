...
<html >
<head>
<title>SQLite | Агрегатные функции</title>
<meta charset="utf-8" />
<meta name="description" content="Агрегатные функции в SQLite, AVG, COUNT, SUM, Min, Max, поиск суммы, среднего, максимально и минимального значения, количество строк">
...
<h2>Агрегатные функции</h2><div class="date">Последнее обновление: 29.11.2021</div>
...
<p>Агрегатные функции вычисляют некоторые скалярные значения на основе набора строк. В SQLite определены следующие агрегатные функции:</p>
<ul>
<li><p><span class="b">AVG</span>: вычисляет среднее значение</p></li>
<li><p><span class="b">SUM</span>: вычисляет сумму значений</p></li>
<li><p><span class="b">MIN</span>: вычисляет наименьшее значение</p></li>
<li><p><span class="b">MAX</span>: вычисляет наибольшее значение</p></li>
<li><p><span class="b">COUNT</span>: вычисляет количество строк в запросе</p></li>
</ul>
<p>Все агрегатные функции в качестве параметра принимают выражение, которое представляет критерий для определения значений. Обычно это название столбца, над значениями которого надо проводить вычисления.</p>
<p>Для рассмотрения агрегатных функций возьмем следующую таблицу products, которая хранит набор товаров:</p>
<pre class="brush:sql;">
...
<h3>Avg</h3>
<p>Функция <span class="b">Avg</span> возвращает среднее значение на диапазоне значений столбца таблицы. При этом строки со значением NULL игнорируются.</p>
<p>Например, найдем среднюю цену товаров из базы данных:</p>
<pre class="brush:sql;">
SELECT AVG(price) AS average_price FROM products;
</pre>
<p>Для поиска среднего значения в качестве выражения в функцию передается столбец price. Для получаемого значения устанавливается псевдоним average_price, хотя 
в принципе устанавливать псевдоним необязательно.</p>
<img src="./pics/5.1.png" alt="Функция avg и поиск среднего значения в SQLite" />
<p>На этапе выборки можно применять фильтрацию. Например, найдем среднюю цену для товаров определенного производителя:</p>
<pre class="brush:sql;">
SELECT AVG(price) FROM products
WHERE company='Apple';
</pre>
<p>Также можно находить среднее значение для более сложных выражений. Например, найдем среднюю сумму всех товаров, учитывая их количество:</p>
<pre class="brush:sql;">
SELECT AVG(price * product_count) FROM products;
</pre>
<p>Дополнительно функция <span class="b">AVG</span> может принимать два оператора:</p>
<pre class="brush:sql;">AVG([ALL | DISTINCT] выражение);</pre>
<p>Оператор <span class="b">ALL</span> указывает, что для вычисления среднего значения будут учитываться все значения, а оператор <span class="b">DISTINCT</span> указывает, 
что учитываются только уникальные значения. Например, вычисление средней цены товаров на основе уникальных значений:</p>
<pre class="brush:sql;">
SELECT AVG(DISTINCT price) FROM products;
</pre>
<h3>Count</h3>
<p>Функция <span class="b">Count</span> вычисляет количество строк в выборке. Есть две формы этой функции. Первая форма <code>COUNT(*)</code> подсчитывает число строк в выборке:</p>
<pre class="brush:sql;">
SELECT COUNT(*) FROM products;
</pre>
<img src="./pics/5.2.png" alt="Функция count в SQLite и вычисление количества строк" />
...
<p>Однако выражение выше подсчитывает количество всех строк, где в столбце company нет значения NULL, то есть количество в том числе повторяющихся значений. 
Чтобы одсчитать количество только уникальных значений, необходимо использовать оператор <span class="b">DISTINCT</span>:</p>
<pre class="brush:sql;">
SELECT COUNT(DISTINCT company) FROM products;
</pre>
<h3>Min и Max</h3>
<p>Функции <span class="b">Min</span> и <span class="b">Max</span> вычисляют минимальное и максимальное значение по столбцу соответственно. 
Например, найдем минимальную цену среди товаров:</p>
<pre class="brush:sql;">
SELECT MIN(price), MAX(price) FROM products;
</pre>
<img src="./pics/5.3.png" alt="Функции MIN и MAX в SQLite" />
<p>Данные функции также игнорируют значения NULL и не учитывают их при подсчете.</p>
<h3>Sum</h3>
<p>Функция <span class="b">Sum</span> вычисляет сумму значений столбца. Если столбец имеет значение NULL, то оно игнорируется. Например, подсчитаем общее количество товаров:</p>
<pre class="brush:sql;">
...
<p>По умолчанию функция <span class="b">SUM</span> вычисляет сумму всех значений, среди которых могут быть повторяющиеся значения. Если необходимо исключить 
повторяющиеся значения, то можно применять оператор <span class="b">DISTINCT</span>. Например, суммарная цена всех товаров на основе уникальных значений:</p>
<pre class="brush:sql;">
