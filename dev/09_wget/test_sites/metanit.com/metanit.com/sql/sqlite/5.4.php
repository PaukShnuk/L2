...
<html >
<head>
<title>SQLite | UNION</title>
<meta charset="utf-8" />
<meta name="description" content="Объединение однотипных выборок и таблиц в SQLite с помощью оператора UNION, версия UNION ALL, исключение повторяющихся строк">
...
<h2>UNION</h2><div class="date">Последнее обновление: 30.11.2021</div>
...
<p>Оператор <span class="b">UNION</span> позволяет объединить две однотипных выборки. Эти выборки могут быть из разных таблиц или из одной и той же 
таблицы. Формальный синтаксис объединения:</p>
<pre class="brush:sql;">
SELECT_выражение1
UNION [ALL] SELECT_выражение2
[UNION [ALL] SELECT_выражениеN]
</pre>
<p>Например, пусть в базе данных будут две отдельные таблицы для клиентов банка (таблица clients) и для сотрудников банка (таблица employees):</p>
<pre class="brush:sql;">
...
именем (name) и возрастом (age). Выберем сразу всех клиентов банка и его сотрудников из обеих таблиц:</p>
<pre class="brush:sql;">
SELECT name, age 
FROM clients
UNION SELECT name, age FROM employees;
</pre>
<p>Здесь из первой таблицы выбираются два значения - имя и возраст клиента. Из второй таблицы employees также 
выбираются два значения - имя и возраст сотрудников. То есть при объединении количество выбираемых столбцов и их тип 
совпадают для обеих выборок.</p>
<img src="./pics/7.10.png" alt="оператор UNION в SQLite" />
<p>При этом названия столбцов объединенной выборки будут совпадать с названия столбцов первой выборки. Если же в одной выборке больше столбцов, чем в другой, то они не смогут быть 
объединены. Например, в следующем случае объединение завершится с ошибкой:</p>
<pre class="brush:sql;">
SELECT name, age , account_sum
FROM clients
UNION SELECT name, age FROM employees;
</pre>
<p>Если оба объединяемых набора содержат в строках идентичные значения, то при объединении повторяющиеся строки по умолчанию удаляются. 
Например, в случае с таблицами clients и employees сотрудники банка могут быть одновременно его клиентами и содержаться в обеих таблицах. 
Если же необходимо при объединении сохранить все, в том числе повторяющиеся строки, то для этого необходимо использовать оператор <span class="b">ALL</span>:</p>
<pre class="brush:sql;">
SELECT name, age 
FROM clients
UNION ALL SELECT name, age FROM employees;
</pre>
<img src="./pics/7.12.png" alt="Обединение повторяющихся строк в SQLite с помощью UNION ALL" />
<p>Объединять выборки можно и из одной и той же таблицы. Например, в зависимости от суммы на счете клиента нам надо начислять ему определенные 
проценты:</p>
<pre class="brush:sql;">
...
<img src="./pics/7.13.png" alt="Объединение выборок из той же таблицы в SQLite с помощью UNION" />
