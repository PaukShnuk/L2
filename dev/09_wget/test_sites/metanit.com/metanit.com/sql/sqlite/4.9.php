...
<html >
<head>
<title>SQLite | Оператор EXISTS</title>
<meta charset="utf-8" />
<meta name="description" content="Оператор EXISTS в SQLite, проверка наличия данных в подзапросах">
...
<h2>Оператор EXISTS</h2><div class="date">Последнее обновление: 29.11.2021</div>
...
<p>Оператор <span class="b">EXISTS</span> проверяет, возвращает ли подзапрос какое-либо значение. Как правило, этот оператор используется для 
индикации того, что как минимум одна строка в таблице удовлетворяет некоторому условию. Поскольку возвращения набора строк не 
происходит, то подзапросы с подобным оператором выполняются довольно быстро.</p>
<p>Применение оператора имеет следующий формальный синтаксис:</p>
<pre class="brush:sql;">EXISTS (подзапрос)</pre>
<p>Если подзапрос возвращает хотя бы одну строку, то оператор <span class="b">EXISTS</span> возвращает true или 1. 
Если результат подзапроса не содержит строк, то оператор <span class="b">EXISTS</span> возвращает false или 0.</p>
<p>Рассмотрим применение оператора <span class="b">EXISTS</span> на примере следующих таблиц:</p>
<pre class="brush:sql;">
...
<p>Простейший пример - посмотрим, есть ли в таблице companies компании с определенными названиями:</p>
<pre class="brush:sql;">
SELECT 
...
<p>Здесь мы видим, что компания "Microsoft" есть в таблице companies, соответственно подзапрос возвращает данные, а оператор EXISTS возвращает 1. 
А вот компании "Apple" нет, поэтому оператор EXISTS возвращает 0:</p>
<img src="./pics/6.6.png" alt="Оператор EXISTS в SQLite" />

<p>Как правило, EXISTS используется при составлении условий в запросах. Например, выберем все комании из таблицы companies, у которых есть 
работники в таблице users:</p>
<pre class="brush:sql;">
SELECT * FROM companies
WHERE EXISTS 
(SELECT * FROM users WHERE users.company_id = companies.id);
</pre>
<img src="./pics/6.5.png" alt="Оператор EXISTS и подзапросы в SQLite" />
<p>Стоит отметить, что для получения подобного результата можно было бы использовать и оператор<span class="b">IN</span>:</p>
<pre class="brush:sql;">
SELECT * FROM companies
WHERE id IN (SELECT company_id FROM users);
</pre>
<p>Но поскольку при применении EXISTS не происходит выборка строк, то его использование более оптимально и эффективно, чем использование оператора IN, поскольку IN должен пробежать все строки из результата подзапроса, 
чтобы проверить соответствие значению.</p>
<p>Если мы хотим узнать, наоброт, есть ли в таблице строки, которые НЕ удовлетворяют условию, то можно использовать операторы <code>NOT EXISTS</code>. 
Например, найдем все комании из таблицы companies, у которых нет работников в таблице users:</p>
<pre class="brush:sql;">
SELECT * FROM companies
WHERE NOT EXISTS 
(SELECT * FROM users WHERE users.company_id = companies.id);
...
<div class="nav"><p><a href="./4.8.php">Назад</a><a href="./">Содержание</a><a href="./5.1.php">Вперед</a></p></div></div>
<div class="socBlock">
