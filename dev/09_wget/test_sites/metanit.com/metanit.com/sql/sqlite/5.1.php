...
<html >
<head>
<title>SQLite | Неявное соединение таблиц</title>
<meta charset="utf-8" />
<meta name="description" content="Неявное соединение таблиц в SQLite с помощью команды SELECT, одновременная выборка из нескольких таблицы">
...
<h1>Соединение таблиц</h1><h2>Неявное соединение таблиц</h2><div class="date">Последнее обновление: 30.11.2021</div>
...
<p>Нередко возникает необходимость объединенить в одном запросе данные сразу из нескольких таблиц. 
Для объединения данных из разных таблиц можно применять разные способы. В данной статье рассмотрим не самый распространный, однако довольно простой способ, который представляет неявное соединение таблиц.</p>
...
<p>Здесь таблицы products и customers связаны с таблицей orders связью один ко многим. Таблица orders в виде внешних ключей product_id и customer_id 
содержит ссылки на столбцы id из соответственно таблиц products и customers. Также она хранит количество купленного товара (items_count) 
и и по какой цене он был куплен (price). И кроме того, таблицы также хранит в виде столбца created_at дату покупки.</p>
...
соединение. Например, в orders три строки, а в customers тоже три строки, значит мы получим 3 * 3 = 9 строк:</p>
<img src="./pics/7.1.png" alt="Неявное соединение таблиц без JOIN в SQLite" />
...
<p>Чтобы решить задачу, необходимо использовать выражение <span class="b">WHERE</span> и фильтровать строки при условии, что поле customer_id из orders соответствует полю Id из customers:</p>
<pre class="brush:sql;">
SELECT * FROM orders, customers
WHERE orders.customer_id = customers.id;
</pre>
<img src="./pics/7.2.png" alt="Соединение таблиц без JOIN в SQLite" />
<p>Теперь объединим данные по трем таблицам orders, customers и products. То есть получим все заказы и добавим информацию по клиенту и связанному товару:</p>
<pre class="brush:sql;">
...
данные по клиенту по условию <code>orders.customer_id = customers.id</code> и данные по товару по условию <code>orders.product_id=products.id</code></p>
<img src="./pics/7.3.png" alt="Соединение таблиц без JOIN в SQLite" />
